...
TL;DR: 本文开发了一个基于深度学习的自动化狂犬病诊断系统，使用荧光图像分析和四种深度学习架构，通过数据增强技术优化模型性能，最终部署了在线工具以实现快速可靠的狂犬病检测。


<details>
  <summary>Details</summary>
Motivation: 狂犬病在非洲和亚洲许多国家仍是重大公共卫生问题，准确诊断对流行病学监测至关重要。目前的金标准诊断方法依赖荧光显微镜，需要熟练的实验室人员进行结果解读，而这种专业人员在样本量低的地区往往稀缺。

Method: 开发了一个自动化AI诊断系统，使用荧光图像分析，通过迁移学习应用四种深度学习架构：EfficientNetB0、EfficientNetB2、VGG16和Vision Transformer (ViTB16)。评估了三种不同的数据增强策略，在155张显微图像数据集（123张阳性，32张阴性）上进行实验，采用分层3折交叉验证选择最佳模型。

Result: TrivialAugmentWide被证明是最有效的数据增强技术，能保留关键荧光模式同时提高模型鲁棒性。EfficientNetB0模型结合几何和颜色增强，在裁剪图像上实现了最佳分类性能。尽管面临类别不平衡和数据集有限的限制，研究证实了深度学习自动化狂犬病诊断的可行性。

Conclusion: 该研究证明了优化深度学习模型在转变狂犬病诊断方面的潜力，开发的在线工具为实际应用提供了便利，为未来医学影像应用建立了框架，有望改善公共卫生结果。
...
TL;DR: TactileEval：一个自动化触觉图形质量评估的三阶段流程，通过建立五类质量分类法、收集结构化标注、训练视觉Transformer模型，实现85.70%的测试准确率，并提供基于AI的自动编辑修复功能。


<details>
  <summary>Details</summary>
Motivation: 当前触觉图形数据集仅提供粗略的整体质量评分，缺乏可操作的修复信号，需要专家验证才能供视障学习者使用。研究旨在自动化这一验证过程，提高触觉图形的可访问性。

Method: 1. 基于TactileNet数据集的专家自由文本评论建立五类质量分类法（视角、部件完整性、背景杂乱、纹理分离、线条质量）；2. 通过亚马逊Mechanical Turk收集14,095个结构化标注，涵盖66个对象类别和6个家族；3. 训练可复现的ViT-L/14特征探针模型；4. 开发ViT引导的自动编辑流程，通过家族特定提示模板生成针对性修正。

Result: 模型在30个不同任务上达到85.70%的整体测试准确率，难度排序一致表明分类法捕捉了有意义的感知结构。基于评估结果，开发了通过gpt-image-1图像编辑实现针对性修正的自动化编辑管道。

Conclusion: TactileEval首次实现了触觉图形质量评估的自动化，通过结构化质量分类法和深度学习模型，为触觉图形提供了可操作的修复信号，显著提高了视障学习者获取触觉图形的效率和质量。
...
TL;DR: KD-Judge是一个基于知识驱动的功能性健身动作自动评判框架，通过LLM将规则手册转换为可执行的机器可读表示，结合确定性规则系统进行重复次数评估，并在边缘设备上实现实时高效执行。


<details>
  <summary>Details</summary>
Motivation: 功能性健身动作在训练、比赛和健康项目中广泛应用，但重复次数标准的主观人为判断、时间限制和规则演变导致一致执行困难。现有AI方法主要依赖学习评分或基于参考的比较，缺乏明确的基于规则的透明性和确定性重复级别验证。

Method: 提出KD-Judge框架：1) 使用基于LLM的检索增强生成和思维链规则结构化管道，将非结构化规则手册标准转换为可执行的机器可读表示；2) 结合姿态引导的运动学推理的确定性规则评判系统评估重复有效性和时间边界；3) 引入双重策略缓存机制，在边缘设备上减少冗余和不必要计算。

Result: 在CFRep数据集上进行评估，实现了可靠的规则结构化性能和准确的重复级别评估，执行速度快于实时（RTF < 1）。启用缓存策略后，在资源受限的边缘设备上，相比无缓存基线，预录制场景获得3.36倍加速，直播场景获得15.91倍加速。

Conclusion: KD-Judge实现了透明、高效和可扩展的基于规则的重复级别分析，能够补充实际中的人工评判，为功能性健身动作提供可靠的自动化评估解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在指令跟随智能体方面表现出强大的感知和推理能力，但在环境理解方面仍存在局限性，经常在交互中失败或依赖环境元数据执行任务。需要解决智能体在环境理解方面的不足，以实现更可靠的任务执行。

Method: 提出EUEA框架，微调四个核心技能：1)物体感知用于识别相关物体，2)任务规划用于生成交互子目标，3)动作理解用于判断成功可能性，4)目标识别用于确定目标完成。引入恢复步骤利用这些核心技能采样替代动作纠正失败案例，以及GRPO阶段优化不一致的技能预测。

Result: 在ALFRED任务上，该方法显著优于行为克隆基线，平均成功率提升8.86%。恢复步骤和GRPO阶段进一步提供3.03%的增益。技能级分析揭示了闭源和开源VLM在环境理解方面的关键局限性。

Conclusion: EUEA框架通过微调核心技能和优化策略，有效提升了指令跟随智能体的环境理解能力和任务执行可靠性，为智能体与环境交互所需的能力提供了重要见解。
...
Method: 采用原生统一的多模态架构，将大型语言模型的认知能力与扩散变换器的高保真像素合成相结合；通过大规模多模态数据扩展、系统化的细粒度标注引擎和精选的强化学习数据来提升专业能力。

Result: 实现了超长复杂文本渲染、超多样化人像生成、调色板引导生成、多主体身份保持、连贯序列视觉生成、精确多模态交互编辑、原生Alpha通道生成和高效4K合成等专业能力。在人类评估中超越Seedream 5.0 Lite和GPT Image 1.5，在挑战性任务上与Nano Banana Pro持平。
...
TL;DR: 提出SGAP-Gaze模型，通过融合驾驶员面部特征和交通场景图像，使用基于Transformer的注意力机制来估计驾驶员注视点，在UD-FSG数据集上相比现有方法减少23.5%的平均像素误差。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶员注视估计模型仅使用面部信息预测注视点或3D注视方向向量，缺乏对周围交通场景的考虑。作者认为场景图像能提供周围交通线索，有助于改进注视估计模型。

Method: 提出SGAP-Gaze（Scene-Grid Attention based Point-of-Gaze estimation network）模型：1）创建UD-FSG数据集，包含同步的驾驶员面部和交通场景图像；2）模型整合驾驶员面部、眼睛、虹膜和场景上下文信息；3）提取面部特征融合成注视意图向量；4）使用基于Transformer的注意力机制计算空间场景网格上的注意力分数，融合面部和场景特征来预测注视点。

Result: SGAP-Gaze在UD-FSG数据集上达到104.73的平均像素误差，在LBW数据集上达到63.48的平均像素误差，相比最先进的驾驶员注视估计模型减少23.5%的平均像素误差。空间像素分布分析显示，SGAP-Gaze在所有空间范围内（包括场景外围区域）都获得更低的平均像素误差。

Conclusion: 结果表明，将多模态注视线索与场景感知注意力机制相结合，能够在真实驾驶环境中构建鲁棒的驾驶员注视点估计模型，特别是在外围区域这种罕见但关键的注意力理解方面表现优异。
...
Method: MMCORE使用预训练的视觉语言模型通过可学习的查询标记预测语义视觉嵌入，然后将这些嵌入作为扩散模型的调节信号。这种流线型设计避免了自回归模型与扩散模型的深度融合，也无需从头训练。

Result: MMCORE在文本到图像合成和交错图像生成方面表现出色，在空间推理和视觉接地等复杂场景中展现出强大的多模态理解能力。在广泛的文本到图像和单/多图像编辑基准测试中，始终优于最先进的基线方法。

Conclusion: MMCORE成功地将视觉语言模型的丰富语义理解能力转移到视觉生成过程中，通过简洁高效的设计实现了高质量的多模态图像生成与编辑，同时显著降低了计算开销。
...
TL;DR: UniCon3R提出了一种统一的端到端框架，用于从单目视频进行在线人-场景4D重建，通过显式建模人-场景接触来纠正重建结果，提高物理合理性。
...
Method: 显式建模交互：从人体姿态和场景几何推断3D接触，并将接触作为纠正线索来生成最终姿态。这是一个统一的端到端前馈框架，用于在线人-场景4D重建。

Result: 在RICH、EMDB、3DPW和SLOPER4D等标准人中心视频基准测试中，UniCon3R在物理合理性和全局人体运动估计方面优于现有方法，同时实现实时在线推理。

Conclusion: 接触不仅是一个外部度量，更是一个强大的内部先验，为物理基础的联合人-场景重建建立了新范式。
...
Method: 采用两阶段训练流程：1）推理蒸馏：使用GPT-5.1为未标注伤口图像生成思维链推理，在较小的学生模型（Qwen3-VL-4B-Thinking）中初始化伤口特定推理能力；2）强化学习后训练：在小型标注感染数据集上使用Group Relative Policy Optimization来优化分类推理。

Result: 在异质性伤口数据集上，Infection-Reasoner达到86.8%准确率、86.4%敏感度和87.1%特异度，优于包括GPT-5.1在内的多个基线模型。推理质量评估显示，多模态大语言模型评委的视觉支持一致性分数在0.722-0.903之间，专家评审认为61.8%的推理正确，32.4%部分正确。

Conclusion: Infection-Reasoner是一个紧凑的推理视觉语言模型，能够在有限专家标注数据下实现准确的慢性伤口感染分类，并生成高质量的解释推理，为临床决策提供支持。
...
TL;DR: 提出了一种上下文感知的生成框架CrackForward，通过合成逼真的裂纹生长模式来解决裂纹检测中标注数据稀缺的问题，相比传统方法能更好地模拟裂纹形态特征。


<details>
  <summary>Details</summary>
Motivation: 结构健康监测中可靠的裂纹检测和分割至关重要，但高质量标注数据的稀缺性构成了主要挑战。现有方法主要操纵纹理或背景内容，无法充分模拟裂纹的形态特征。

Method: 提出了CrackForward框架，结合方向性裂纹延伸与学习的增厚和分支来显式建模裂纹形态。包含两个关键创新：(1)上下文引导的裂纹扩展模块，使用局部方向线索和自适应随机游走模拟真实传播路径；(2)两阶段U-Net风格生成器，学习再现空间变化的裂纹特征如厚度、分支和生长。

Result: 实验结果表明，生成的样本保留了目标阶段的饱和度和厚度特征，并提高了多种裂纹分割架构的性能。这表明结构感知的合成裂纹生成比传统数据增强方法能提供更具信息量的训练数据。

Conclusion: 提出的上下文感知生成框架能够有效合成逼真的裂纹生长模式，为解决裂纹检测中数据稀缺问题提供了新思路，通过显式建模裂纹形态特征，生成的合成数据能显著提升裂纹分割模型的性能。
...
TL;DR: ToolsRL：通过工具监督的强化学习课程框架，让多模态大语言模型有效掌握视觉工具使用以解决复杂视觉推理任务


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型如何有效掌握工具使用来解决复杂视觉推理任务的问题。当前模型在工具使用方面存在学习效率低、优化冲突等问题，需要更有效的训练方法。

Method: 提出工具监督的强化学习框架ToolsRL，采用两阶段课程学习：第一阶段通过专门设计的工具特定奖励优化工具调用能力；第二阶段在掌握工具使用的基础上，结合准确性目标奖励进行训练。使用简单、原生、可解释的视觉工具（放大、旋转、翻转、绘制点/线），这些工具监督数据容易收集。

Result: 实验表明，工具监督的课程训练是高效的，ToolsRL能够在复杂视觉推理任务中实现强大的工具使用能力。该方法避免了异质任务之间的潜在优化冲突。

Conclusion: ToolsRL框架通过工具监督的强化学习课程，有效解决了多模态大语言模型掌握视觉工具使用的挑战，为复杂视觉推理任务提供了有效的解决方案。
...
TL;DR: 提出一种通过学习参数化相机token来实现文本到图像生成中精确相机控制的方法，解决了现有模型难以通过自然语言进行精确相机控制的问题。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型仅通过自然语言难以实现精确的相机控制，需要一种能够提供全局场景理解并实现精确相机控制的框架。

Method: 通过微调图像生成模型，在包含3D渲染图像（用于几何监督）和真实感增强（用于外观和背景多样性）的策划数据集上，学习参数化相机token来实现视角条件化的文本到图像生成。

Result: 定性和定量实验表明，该方法在保持图像质量和提示保真度的同时，达到了最先进的精度。与之前方法不同，该方法学习的视角token能够分解几何表示，并可迁移到未见过的物体类别。
...
TL;DR: DistortBench是一个用于评估视觉语言模型（VLMs）在无参考失真感知能力的诊断基准，包含13,500个四选一问题，涵盖27种失真类型和5个严重程度级别。评估18个VLMs后发现，即使最好的模型准确率也只有61.9%，低于人类基准，表明低层次视觉感知是当前VLMs的主要弱点。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在需要对低层次图像失真敏感的应用场景中越来越重要，如内容审核、图像恢复和质量监控等，但目前对这些模型识别失真类型和严重程度的能力了解不足。

Method: 提出了DistortBench诊断基准，包含13,500个四选一问题，覆盖27种失真类型、6个感知类别和5个严重程度级别。其中25种失真继承KADID-10k的校准，2种旋转失真使用基于角度的单调级别。评估了18个VLMs，包括17个开源模型和1个专有模型。

Result: 最佳模型准确率仅为61.9%，略低于人类多数投票基准的65.7%（个体平均60.2%）。分析还发现：模型规模与性能呈弱且非单调的缩放关系；大多数基础-思考模型对性能下降；不同模型家族表现出不同的严重程度响应模式。

Conclusion: 当前视觉语言模型在低层次视觉感知方面存在显著弱点，DistortBench可作为衡量和改进VLMs低层次视觉感知能力的有用基准。
...
TL;DR: 提出一种在线直接边缘映射技术，绕过图像重建步骤来分类场景和目标，降低SAR处理的计算和存储成本


<details>
  <summary>Details</summary>
Motivation: 现代防御应用依赖低成本小型无人机，但SAR中的自动目标识别通常需要后处理，机载系统需存储大量回波信号，带来高存储和计算负担

Method: 提出在线直接边缘映射技术，绕过传统的图像重建步骤，通过重建场景为边缘图来分类场景和目标，边缘图具有稀疏性，需要更少的测量和计算资源

Result: 该方法比传统的后向投影等SAR重建算法需要更少的测量和计算能力，通过边缘图的稀疏性降低了存储和处理需求

Conclusion: 提出的在线直接边缘映射技术能够有效降低SAR自动目标识别的计算和存储成本，特别适合资源受限的无人机机载系统
...
TL;DR: 研究开发了一个基于信息感官价值理论的计算模型，通过多模态特征分析预测短视频的感官和行为参与度，发现感官价值与感官参与度正相关，但与行为参与度呈倒U型关系。


<details>
  <summary>Details</summary>
Motivation: 当前媒体环境中充斥着大量感官刺激强烈的短视频，但现有研究主要关注单一模态特征的影响，缺乏对多模态特征集体影响观众参与度的系统性理解。

Method: 基于信息感官价值理论框架，开发并测试了一个计算模型，通过对1,200个短视频进行多模态特征分析和人工评估，并在三个短视频平台的14,492个未见过视频数据集上进行验证。

Result: 信息感官价值与感官参与度呈正相关，但与行为参与度呈现倒U型关系：较高的感官价值引发更强的感官刺激，但适中的感官价值能优化行为参与度。
...
TL;DR: 提出一种结合Mosaic策略和HSV色彩空间适应的数据增强管道，用于提升轻量级YOLOv11 Nano模型在无人机检测任务中的性能，特别是在边缘设备部署场景下。


<details>
  <summary>Details</summary>
Motivation: 无人机视觉检测在监控系统中至关重要，但由于无人机体积小和环境挑战，检测难度大。虽然深度学习模型已取得显著进展，但在边缘设备上部署需要轻量级模型（如YOLOv11 Nano），这些模型学习能力有限，需要有效的数据增强方法来提升性能。

Method: 提出一种高效且上下文感知的数据增强管道，结合Mosaic策略和HSV色彩空间适应。Mosaic策略通过拼接多张图像来增加训练样本的多样性，HSV色彩空间适应则调整图像的色调、饱和度和亮度以增强模型对不同光照条件的鲁棒性。

Result: 在四个标准数据集上的实验结果表明，与Copy-Paste等重量级和实例级方法相比，该方法不仅避免了合成伪影和过拟合问题，还显著提高了所有场景下的平均精度（mAP）。在雾天条件下的泛化能力评估显示，该方法在精度和稳定性之间达到了最佳平衡，适合实时系统，而MixUp等方法仅在特定应用中有效。

Conclusion: 提出的数据增强管道能有效提升轻量级无人机检测模型的性能，特别是在边缘设备部署场景下。该方法在避免合成伪影和过拟合的同时，显著提高了检测精度，并在不同环境条件下表现出良好的泛化能力和稳定性，适合实时监控系统应用。
...
<details>
  <summary>Details</summary>
Motivation: 自动化野生动物监测面临两大挑战：小尺寸稀有物种检测困难和大规模专家标注成本高昂。草原犬鼠作为典型案例，体型小、分布稀疏、与背景视觉差异小，对传统检测模型构成严重挑战。

Method: RareSpot+整合了三种关键技术：1）多尺度一致性损失，通过对齐不同检测头的中间特征图来增强小目标定位；2）上下文感知增强，合成生态上合理的困难样本提高鲁棒性；3）地理空间主动学习模块，利用草原犬鼠与洞穴的空间先验知识，结合测试时增强和元不确定性模型，减少冗余标注。

Result: 在2平方公里的航空数据集上，RareSpot+相比基线mAP@50提升了35.2%（绝对提升0.13）。在HerdNet、AED等多个野生动物基准测试中表现出强大的检测器级可迁移性。主动学习模块仅使用1.7%未标注图块的标注预算，就将草原犬鼠AP提升了14.5%。

Conclusion: RareSpot+不仅显著提升了小目标稀有物种的检测性能，还通过地理空间主动学习大幅降低了标注成本。该框架超越了单纯的目标检测，能够支持空间生态分析（如聚类和共现分析），将视觉检测与定量生态学联系起来。
...
TL;DR: 通过使用人类显著性注视图微调ViT-B/16的自注意力权重，可以缩小视觉Transformer与人类注意力特征之间的认知差距，且不影响模型分类性能。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的视觉Transformer（ViT）在处理图像时与人类注意力特征存在显著差异，研究者希望探索是否可以通过人类监督来缩小这种认知差距。

Method: 使用Google的ViT-B/16模型，利用人类显著性注视图微调其自注意力权重，并与随机打乱的对照组进行比较，以分离语义相关信号与通用人类监督的影响。

Result: 微调显著提高了五个显著性指标的匹配度，并诱导出三种典型的人类偏向：逆转了基线模型对大物体的反人类偏向，放大了对生命体的偏好，降低了极端注意力熵。贝叶斯分析显示这种认知对齐不会损害模型在ImageNet、ImageNet-C和ObjectNet上的分类性能。同样的方法应用于ResNet-50 CNN反而降低了对齐度和准确率。

Conclusion: 研究表明，基于生物学的先验可以作为人类对齐注意力的免费涌现特性被注入，从而提高Transformer的可解释性，且ViT的模块化自注意力机制特别适合分离空间优先级与表征逻辑。
...
TL;DR: ACFamNet Pro通过改进FamNet，结合多注意力头和残差连接，在细菌菌落计数任务上取得9.64%的平均归一化绝对误差，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 细菌菌落自动计数对疫苗和抗生素开发很重要，但面临四个挑战：菌落尺寸小、对象聚集、数据标注成本高、跨物种泛化能力有限。现有FamNet方法对前两个挑战有效，但对小尺寸菌落和跨物种泛化的效果未知。

Method: 提出ACFamNet解决前三个挑战，使用带对齐的新型感兴趣区域池化和优化特征工程处理小尺寸和聚集对象。进一步提出ACFamNet Pro解决所有四个挑战，在ACFamNet基础上增加多注意力头和残差连接，实现对象动态加权和改进梯度流。

Result: ACFamNet Pro在5折交叉验证下达到9.64%的平均归一化绝对误差，分别比ACFamNet和FamNet提升2.23%和12.71%。

Conclusion: ACFamNet Pro通过多注意力头和残差连接增强，有效解决了细菌菌落计数的四个主要挑战，在准确性和泛化能力上显著优于现有方法。
...
Method: 引入训练时的跨视角正则化方案，在图像域通过几何对齐约束联合监督多视角编辑，然后通过前馈3D高斯泼溅模型将编辑后的视角提升为3D表示

Result: 实验显示具有竞争力的编辑保真度，相比优化方法显著改善跨视角一致性，推理时间减少数个数量级

Conclusion: 提出的前馈框架能够实现高效、一致的3D场景编辑，避免了传统迭代优化方法的计算负担和视角不一致问题
...
TL;DR: iTARFlow是一种改进的归一化流生成模型，结合了自回归生成和扩散式迭代去噪，在ImageNet多分辨率图像建模中取得竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 归一化流作为基于似然的经典方法近年来重新受到关注，TARFlow已证明其在图像建模任务中的潜力。本文旨在进一步推进归一化流生成模型的发展，使其成为扩散模型等方法的可行替代方案。

Method: 提出迭代TARFlow（iTARFlow），在训练时保持端到端的基于似然的目标，在采样时采用自回归生成和受扩散模型启发的迭代去噪过程。

Result: iTARFlow在ImageNet的64、128和256像素分辨率上取得了竞争性性能，展示了其作为强大生成模型的潜力，并分析了模型产生的特征性伪影。
...
TL;DR: 本文提出了一种内存受限的3D高斯泼溅训练框架，通过动态优化高斯原语的数量来解决训练过程中的内存峰值问题，在保持渲染质量的同时显著降低内存消耗。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）在新型视图合成中表现出色，但由于训练过程中高斯原语的不可控密集化，导致内存占用巨大，特别是在内存受限的边缘设备上部署时成为关键瓶颈。现有方法在训练后修剪冗余高斯，但无法解决训练早期高斯数量激增导致的内存峰值问题。

Method: 提出了一种系统化的内存受限训练框架，通过迭代增长和修剪来动态优化高斯原语。该框架交替进行低影响高斯的增量修剪和带有自适应高斯补偿的新原语战略增长，在保持接近恒定的低内存使用的同时，逐步提升渲染保真度。
...
Conclusion: 提出的内存受限训练框架有效解决了3DGS训练过程中的内存峰值问题，使得在内存受限的边缘设备上进行高质量3D场景重建成为可能，为3DGS的实际部署提供了可行的解决方案。
...
TL;DR: PASTA提出了一种针对Vision Transformers的双重隐蔽补丁式后门攻击方法，通过触发辐射效应和多位置触发插入策略，实现在任意补丁位置激活后门，同时保持像素和注意力域的隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 现有针对ViT的补丁式后门攻击通常假设推理时触发位置固定，忽略了ViT的自注意力机制可以捕获补丁间的长程依赖关系。研究发现补丁式触发可以通过激活相邻补丁的后门实现高攻击效果（触发辐射效应），但现有最大化触发注意力的方法往往牺牲了视觉和注意力隐蔽性。

Method: 提出PASTA攻击框架：1）多位置触发插入策略增强触发辐射效应；2）通过双层优化问题构建自适应后门学习框架，模型和触发相互迭代适应以避免局部最优；3）在像素和注意力域同时保持隐蔽性。

Result: 在四个数据集上，PASTA在任意补丁位置平均达到99.13%的攻击成功率，显著提升了视觉隐蔽性（144.43倍）和注意力隐蔽性（18.68倍），对最先进的ViT防御方法的鲁棒性提高了2.79倍，优于CNN和ViT基线方法。

Conclusion: PASTA展示了在Vision Transformers中实现高效且隐蔽的后门攻击的可能性，通过利用触发辐射效应和自适应学习框架，在保持隐蔽性的同时实现了高攻击成功率和鲁棒性，揭示了ViT后门攻击的新方向。
...
TL;DR: FurnSet框架通过显式识别和利用场景中重复的物体实例来改进单视图3D场景重建，使用集合感知的自注意力机制聚合互补信息，结合场景级和物体级条件指导重建，并通过布局优化提升对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有单视图3D场景重建方法通常独立重建物体或依赖隐式场景上下文，未能充分利用真实场景中常见的重复实例。这些重复实例包含互补的观测信息，可以显著提升重建质量。

Method: 提出FurnSet框架：1) 引入每个物体的CLS令牌；2) 使用集合感知的自注意力机制识别和分组相同实例，聚合跨实例的互补观测；3) 结合场景级和物体级条件指导物体重建；4) 使用物体点云进行布局优化，采用3D和2D投影损失进行场景对齐。

Result: 在3D-Future和3D-Front数据集上的实验表明，该方法显著提升了场景重建质量，证明了利用重复实例进行稳健3D场景重建的有效性。

Conclusion: 通过显式识别和利用场景中的重复物体实例，FurnSet框架能够更有效地进行3D场景重建，为单视图重建任务提供了新的思路和方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有长视频理解系统存在修正成本过高的问题：多模态流水线产生不透明的端到端输出，缺乏可检查的中间状态，迫使标注者重新查看原始视频并从头重建时间逻辑。核心瓶颈不仅是生成质量，还缺乏能让人类工作量与每个错误范围成比例的监督接口。

Method: 提出IMPACT-CYCLE系统，将长视频理解重构为共享语义记忆的迭代声明级维护。该系统包含结构化、版本化的状态，编码类型化声明、声明依赖图和溯源日志。角色专门化的智能体在明确权限合约下运行，将验证分解为局部对象关系正确性、跨时间一致性和全局语义连贯性，修正仅限于结构依赖的声明。当自动化证据不足时，系统将升级到具有最终否决权的人类仲裁作为监督权威；依赖闭包重新验证确保修正成本与错误范围成比例。

Result: 在VidOR数据集上的实验显示，下游推理能力显著提升（VQA：从0.71到0.79），人类仲裁成本降低4.8倍，工作量显著低于手动标注。

Conclusion: IMPACT-CYCLE通过引入结构化语义记忆和监督式多智能体系统，解决了长视频理解中错误修正成本不成比例的问题，实现了人类工作量与错误范围的成比例关系，显著提升了系统效率和准确性。
...
TL;DR: GSCompleter提出了一种免蒸馏的插件方法，通过"生成-注册"范式解决3D高斯泼溅在稀疏视图下的性能退化问题，替代了传统不稳定的"修复-蒸馏"方法。
...
Method: 提出GSCompleter，采用免蒸馏的"生成-注册"工作流程：1) 合成可信的2D参考图像；2) 通过稳健的立体锚机制将其显式提升为度量尺度的3D基元；3) 通过新颖的射线约束注册策略将这些基元无缝集成到全局上下文中。

Result: 该方法在三个不同的基准测试中实现了卓越的3DGS补全性能，提升了各种基线的质量和效率，并取得了新的SOTA结果。

Conclusion: 通过将场景补全转向稳定的"生成-注册"工作流程，GSCompleter为3D高斯泼溅的稀疏视图外推问题提供了更有效和稳定的解决方案。
...
TL;DR: HumanScore：评估AI生成视频中人体运动质量的系统框架，包含六个可解释指标，用于分析13个SOTA模型的运动生成质量


<details>
  <summary>Details</summary>
Motivation: 当前视频生成技术快速发展，但缺乏系统评估人体运动和动态渲染质量的方法。现有方法主要关注视觉真实感，而忽略了运动生物力学和运动学合理性等关键方面。

Method: 提出HumanScore框架，定义六个可解释指标：运动学合理性、时间稳定性、生物力学一致性等。通过精心设计的提示词收集多样化运动数据，评估13个最先进的视频生成模型。

Result: 分析揭示了感知合理性与运动生物力学保真度之间的差距，识别了常见失败模式（如时间抖动、解剖学上不合理的姿势、运动漂移），并基于定量和物理意义标准产生了稳健的模型排名。

Conclusion: HumanScore为评估AI生成视频中人体运动质量提供了系统框架，超越了单纯的视觉真实感评估，能够识别模型在运动生成方面的具体缺陷，为改进视频生成技术提供了重要诊断工具。
...
TL;DR: SFS是一个结合FastSAM快速分割能力和语义标注管道的实时语义分割框架，在保持准确性的同时实现20倍加速，支持闭集和开放词汇分割。
...
Method: 1. 基于FastSAM（CNN重新实现的SAM）进行快速掩码生成；2. 集成Semantic-Segment-Anything标注策略为每个掩码分配语义类别；3. 支持闭集和开放词汇分割，后者使用CLIP-based语义头；4. 整个框架在保持准确性的同时大幅降低计算成本和内存占用。

Result: 在Cityscapes数据集上达到70.33 mIoU，在ADE20K上达到48.01 mIoU，与先前SAM-based方法精度相当；推理速度比SSA快约20倍；在开放词汇分割任务中超越近期模型。

Conclusion: SFS实现了实用的实时语义分割，具备"分割一切"能力，显著提升了基础分割模型在机器人等实时应用场景中的适用性，为实际部署提供了高效解决方案。
...
TL;DR: WildFireVQA：首个结合RGB和热成像数据的大规模空中野火监测视觉问答基准，包含6,097个样本和207,298个多选题，涵盖野火监测的六个关键任务类别。


<details>
  <summary>Details</summary>
Motivation: 现有空中视觉问答基准缺乏针对野火监测的多模态推理评估，特别是缺少基于热测量数据的接地推理。野火监测需要及时、可操作的情境感知，而现有基准无法评估野火特定的多模态推理能力。

Method: 构建包含RGB图像、彩色映射热成像可视化和辐射热TIFF数据的6,097个样本数据集。采用多模态大语言模型答案生成与传感器驱动确定性标注相结合的方法，辅以人工验证、帧内和帧间一致性检查，确保标注可靠性。建立RGB、热成像和检索增强三种设置下的评估协议。

Result: 实验表明，在当前模型中，RGB仍是最强的模态，而检索到的热上下文对更强的多模态大语言模型有提升作用。这凸显了基于温度推理的价值，也暴露了现有多模态大语言模型在安全关键野火场景中的局限性。
...
TL;DR: 提出DualGaze-VLM框架，通过构建对象级驾驶员注意力数据集G-W3DA和双分支视觉语言模型，实现精确的对象级注意力预测，显著提升空间对齐指标。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶员注意力预测数据集仅提供场景级全局注视而非细粒度对象级标注，无法支持基于文本的认知建模，导致视觉语言模型出现文本-视觉解耦和视觉偏差幻觉问题。
...
Result: 在W3DA基准测试中，DualGaze-VLM在空间对齐指标上持续超越现有SOTA模型，在安全关键场景下相似度(SIM)提升高达17.8%；视觉图灵测试显示88.22%的人类评估者认为其生成的注意力热图真实可信。
...
TL;DR: 该论文提出了一种空间-时间相干相关性学习算法，用于在保持说话口型的同时修改面部表情，无需成对的训练样本。


<details>
  <summary>Details</summary>
Motivation: 现有的语音保留面部表情操纵方法依赖于难以获取的成对训练样本，限制了其在现实场景中的应用。作者发现不同情绪下表达相同内容时，局部面部动画具有高度相关性，这为SPFEM提供了有价值的监督信号。

Method: 提出空间-时间相干相关性学习算法，包括：1) 空间相干相关性度量，确保相邻局部区域在情绪变化时保持相似的视觉相关性；2) 时间相干相关性度量，确保跨帧的特定区域在不同情绪下保持相似的相关性；3) 相关性感知自适应策略，优先处理更具挑战性的区域。

Result: 通过将空间-时间相干相关性度量作为额外损失函数来监督生成过程，能够在修改面部表情的同时更好地保留说话内容相关的面部动画。

Conclusion: 该方法利用不同情绪下相同内容表达时的局部面部动画相关性，通过空间-时间相干相关性学习实现了无需成对样本的语音保留面部表情操纵，扩展了SPFEM在现实场景中的应用。
...
Conclusion: 本文提供了一个全面的生物启发颜色恒常性技术框架，将光照估计问题重新定义为灰度锚点检测，并通过实验验证了该方法的有效性，为生物启发方法在颜色恒常性领域的进一步探索奠定了基础。
...
TL;DR: 该论文提出了一种无需训练、任务感知的编辑定位框架，通过利用图像编辑模型中的源图像和目标图像流，针对不同编辑任务（添加、移除、替换）进行精确的编辑区域定位，解决了现有指令图像编辑方法中的过度编辑问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于指令的图像编辑方法在处理文本指令时经常出现过度编辑问题，对不相关区域引入不必要的修改。作者发现这一限制源于缺乏明确的编辑定位机制，且不同编辑操作（添加、移除、替换）具有不同的空间模式，但现有模型通常以任务无关的方式处理定位问题。

Method: 提出了一种无需训练、任务感知的编辑定位框架：1）从源图像和目标图像流中获取基于注意力的编辑线索；2）基于这些注意力线索构建特征质心，将token划分为编辑区域和非编辑区域；3）引入统一的掩码构建策略，针对不同编辑任务选择性地利用源图像和目标图像流。

Result: 在EdiVal-Bench上的大量实验表明，该框架在保持强大指令跟随性能的同时，显著提高了非编辑区域的一致性，并在Step1X-Edit和Qwen-Image-Edit等强大的图像编辑骨干模型上取得了显著改进。

Conclusion: 通过任务感知的编辑定位框架，有效解决了指令图像编辑中的过度编辑问题，为不同编辑任务提供了精确的定位机制，提高了编辑质量和非编辑区域的保护能力。
...
<details>
  <summary>Details</summary>
Motivation: 定向目标检测面临角度边界不连续(ABD)和循环模糊(CA)问题，现有连续角度编码方法仍存在显著循环误差，主要源于非正交解码机制的结构噪声放大。

Method: 提出傅里叶级数编码器(FSC)，通过将角度映射到最小正交傅里叶基并显式施加几何流形约束，建立连续、可逆且数学鲁棒的角度编码-解码范式，防止特征模量崩溃。

Result: 在三个大规模数据集上的实验表明，FSC实现了高度竞争力的整体性能，在高精度检测方面取得显著改进，无需启发式截断即可实现严格边界连续性和优越噪声免疫力。

Conclusion: FSC通过建立结构稳定的角度表示，从根本上解决了定向目标检测中的角度边界不连续和循环模糊问题，为高精度定向检测提供了轻量级即插即用解决方案。
...
TL;DR: MambaLiteUNet：一个轻量高效的皮肤病变分割框架，集成Mamba状态空间模型到U-Net架构，通过三个关键模块增强局部-全局特征交互，在保持高精度的同时大幅减少参数和计算量。


<details>
  <summary>Details</summary>
Motivation: 现有分割模型虽然通过减少参数和计算复杂度提高了效率，但在皮肤癌早期诊断和治疗规划中，难以准确描绘精细的病变边界和纹理模式，需要更平衡精度与效率的解决方案。

Method: 提出MambaLiteUNet框架，将Mamba状态空间模型集成到U-Net架构中，包含三个关键模块：自适应多分支Mamba特征融合（AMF）、局部-全局特征混合（LGFM）和交叉门控注意力（CGA），以增强特征交互并保留空间细节。

Result: 在ISIC2017、ISIC2018、HAM10000和PH2基准测试中，平均IoU达到87.12%，平均Dice分数达到93.09%，优于现有最先进模型。相比U-Net，IoU和Dice分别提高7.72和4.61个百分点，同时参数减少93.6%，GFLOPs减少97.6%。在六个未见病变类别的域泛化测试中，IoU达到77.61%，Dice达到87.23%，表现最佳。
...
TL;DR: X-Cache是一种用于实时驾驶世界模型的训练免费加速方法，通过跨连续生成块而非去噪步骤进行缓存，实现了71%的块跳过率和2.6倍的加速，同时保持最小性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有的驾驶世界模型基于自回归视频扩散技术，虽然实现了高保真、可控的多摄像头生成，但其推理成本成为交互部署的瓶颈。现有的扩散缓存方法专为具有多个去噪步骤的离线视频生成设计，不适用于实时交互场景。
...
Result: 在基于多块因果DiT构建的X-world驾驶世界模型上，X-Cache实现了71%的块跳过率，2.6倍的实时加速，同时保持最小性能下降。

Conclusion: X-Cache为实时驾驶世界模型提供了一种有效的训练免费加速方法，通过创新的跨块缓存策略解决了现有方法在交互部署中的局限性，显著提升了推理效率。
...
TL;DR: 该论文提出了一个名为DCI（双重因果推断）的新框架，用于解决医学视觉问答中的跨模态混淆问题，通过结合后门调整和工具变量学习来处理可观察和不可观察的混淆因素。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉问答方法容易过度拟合表面的跨模态相关性，忽略了多模态医学数据中固有的偏见，导致模型容易受到跨模态混淆效应的影响，从而无法提供可信的诊断推理。

Method: 提出DCI框架，构建结构因果模型，使用后门调整缓解可观察的跨模态偏见，同时使用从共享潜在空间学习的工具变量来补偿不可观察的混淆因素。通过互信息约束确保工具变量的有效性。

Result: 在SLAKE、SLAKE-CP、VQA-RAD和PathVQA四个基准数据集上的实验表明，该方法持续优于现有方法，特别是在分布外泛化方面表现突出。定性分析确认DCI显著增强了跨模态推理的可解释性和鲁棒性。

Conclusion: DCI框架通过双重因果推断机制有效提取去混淆表示，捕捉真实的因果关系，解决了医学视觉问答中的跨模态混淆问题，提高了模型的泛化能力和诊断可靠性。
...
Method: UniCVR采用两阶段方法：第一阶段通过对比学习在多源数据集（约350万样本）上训练MLLM作为组合查询嵌入器，桥接MLLM与冻结VLP图库编码器的异构嵌入空间，并提出基于聚类的硬负样本采样策略；第二阶段引入MLLM引导的双层重排序机制，对少量顶级候选进行自适应预算子集评分，然后通过双层重评分方案利用相关信号，以最小计算开销产生更准确的最终排名。

Result: 在涵盖所有三个任务的五个基准测试中进行的广泛实验表明，UniCVR实现了最先进的性能，验证了其有效性和泛化能力。

Conclusion: UniCVR是首个统一的零样本组合视觉检索框架，通过结合MLLM的组合查询理解能力和VLP模型的结构化视觉检索优势，成功解决了三个相关但孤立研究的任务，展示了强大的性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 外科视频的细粒度时空推理至关重要，但多模态大语言模型在该领域的能力尚未得到充分探索，需要建立一个评估基准来弥合MLLM能力与临床推理需求之间的差距。
...
Conclusion: SurgCoT提供了一个可重复的测试平台，能够缩小MLLM能力与临床推理需求之间的差距，促进外科视频时空推理能力的发展。
...
TL;DR: HyLaR框架通过混合潜在推理，将离散文本生成与连续视觉潜在表示交织，使用DePO优化策略在混合空间中实现有效强化学习，提升MLLMs的细粒度感知和多模态理解能力。


<details>
  <summary>Details</summary>
Motivation: 传统CoT推理在视觉任务中需要将信号离散化以适应LLM输入，导致早期语义崩溃和细粒度细节丢失。外部工具虽然可以缓解这一问题，但引入了僵化的瓶颈，将推理限制在预定义操作中。最近的潜在推理范式虽然内部化视觉状态来克服这些限制，但优化混合离散-连续动作空间仍然具有挑战性。
...
Conclusion: HyLaR通过混合潜在推理框架有效解决了视觉CoT推理中的语义崩溃和细粒度信息丢失问题，DePO优化策略在混合离散-连续空间中实现了有效的强化学习，显著提升了多模态大语言模型的推理能力。
...
Method: 通过将视觉任务的输出空间参数化为RGB图像，将感知任务重构为图像生成问题。在Nano Banana Pro基础上，使用原始训练数据和少量视觉任务数据进行指令微调，构建通用模型Vision Banana。

Result: Vision Banana在多种2D和3D视觉任务上达到SOTA结果，在分割任务上超越或媲美Segment Anything Model 3，在深度估计上超越Depth Anything系列，且轻量级指令微调不损害基础模型的图像生成能力。

Conclusion: 图像生成预训练是通用视觉学习器，图像生成可以作为视觉任务的统一通用接口，类似于文本生成在语言理解和推理中的角色。这可能标志着计算机视觉的重大范式转变。
...
<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法主要针对单人物场景，缺乏对多人协同操纵中载荷诱导动力学、自然交互姿势和稳定状态的考虑，需要开发专门的多人物协同操纵生成方法。

Method: 1. 基于物体可操作性和空间配置的生成模型，推导显式操纵策略引导运动流向成功操纵；2. 设计对抗交互先验，提升个体姿势自然性和人际交互真实性；3. 引入稳定性驱动模拟，通过采样优化细化不稳定交互状态，直接调整向量场回归。

Result: 实验表明，相比现有最佳的人-物交互基线方法，本方法在接触精度、穿透率和分布保真度方面表现更优，实现了更高准确性和更自然的协同操纵运动。

Conclusion: 提出的流匹配框架能够生成符合目标、自然且有效的协同操纵运动，通过显式操纵策略、对抗交互先验和稳定性驱动模拟的综合设计，显著提升了多人协同操纵的质量和稳定性。
...
Method: 引入跨模态渐进式临床推理基准，包含两个推理任务：1）六阶段渐进式推理链（从图像质量评估到临床决策），2）整合六种成像模态的跨模态推理任务。基准包含来自51个公开数据集的26,415张图像和177,868个专家验证问答对。

Result: 对21个多模态大语言模型的评估揭示了在渐进式推理和跨模态整合方面存在关键差距，表明现有模型在临床诊断工作流程中的推理能力仍有不足。

Conclusion: X-PCR基准为评估多模态大语言模型的临床推理能力提供了全面框架，揭示了当前模型在渐进式推理和跨模态整合方面的局限性，为未来改进提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在多对象生成时经常出现幻觉问题（某些实体被遗漏），现有解决方案主要优化潜在表示但低估了初始生成种子的重要性，而使用多种子多次迭代会增加时间和能耗成本

Method: HEaD+框架整合交叉注意力图、文本信息和新的"预测最终图像"输入，在扩散过程中早期检测错误生成，决定是否继续当前生成或使用不同种子重新开始；还提出集成定位模块预测对象质心位置并验证空间关系

Result: 在包含最多七个对象的45,000张生成图像数据集上训练，HEaD+在四个对象情况下将完整生成概率提高6-8%，在追求完整图像时将生成时间减少高达32%，同时提高关系一致性结果

Conclusion: HEaD+通过早期检测幻觉问题，在探索多种子生成的同时节省时间，显著提高了多对象文本到图像生成的效率和准确性
...
TL;DR: SignDATA是一个用于手语数据预处理的配置驱动工具包，可将异构手语语料库标准化为可比较的输出格式，支持姿态和视频两种处理流程，提供可复现的预处理层。


<details>
  <summary>Details</summary>
Motivation: 手语数据集因标注模式、剪辑时序、手语者取景和隐私限制等因素难以进行一致的预处理。现有工作通常只报告下游模型，而将原始视频转换为训练就绪的姿态或视频数据的预处理流程仍然分散、后端特定且文档不完善。

Method: 提出SignDATA配置驱动预处理工具包，支持两种端到端处理流程：姿态流程（包含采集、清单化、人物定位、剪辑、裁剪、关键点提取、归一化和WebDataset导出）和视频流程（用裁剪后的视频打包替换姿态提取）。系统提供可互换的MediaPipe和MMPose后端、类型化作业模式、实验级覆盖以及基于配置和清单感知哈希的每阶段检查点。
...
<details>
  <summary>Details</summary>
Motivation: 组合图像检索任务依赖昂贵且易错的三元组标注，本文发现噪声三元组对应问题，特别是"硬噪声"（参考图像和目标图像高度相似但修改文本错误）对现有噪声对应学习方法构成独特挑战，因为它打破了传统的"小损失假设"。

Method: 提出ConeSep方法：1）几何保真量化，理论建立并实际估计噪声边界以精确定位噪声对应；2）负边界学习，为每个查询学习"对角线负组合"作为嵌入空间中的显式语义对立锚点；3）基于边界的定向反学习，将噪声校正过程建模为最优传输问题，优雅避免反学习反弹。
...
TL;DR: ScanVLA模型利用视觉语言模型融合多模态特征，通过历史增强扫描路径解码器和分割LoRA辅助，显著提升了目标指引导向的扫描路径预测性能。


<details>
  <summary>Details</summary>
Motivation: 目标指引导向的扫描路径预测需要有效融合视觉和语言信息，现有方法在多模态信息融合方面存在不足，需要更精确的位置感知能力。

Method: 1. 使用视觉语言模型提取对齐的视觉和语言特征表示；2. 提出历史增强扫描路径解码器，直接利用历史注视点位置信息；3. 采用冻结的分割LoRA作为辅助组件，精确定位目标对象。

Result: 大量实验结果表明，ScanVLA在目标指引导向下显著优于现有的扫描路径预测方法，且不增加大量计算和时间成本。

Conclusion: ScanVLA通过有效融合多模态信息和增强位置感知能力，为目标指引导向的扫描路径预测提供了有效的解决方案。
...
TL;DR: MPD是一个双阶段框架，通过语义感知的组件解耦和可解释的参数更新，有效减少大型视觉语言模型的幻觉生成，同时保持其通用生成能力。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然具有强大的生成能力，但经常产生幻觉，影响输出可靠性。基于标注数据的微调成本高昂，而现有的基于表示的方法虽然高效，但会降低模型的通用生成能力。

Method: 提出MPD双阶段框架：1）语义感知的组件解耦，提取纯幻觉组件；2）可解释的参数更新，选择性地修改与幻觉最相关的参数。

Result: 在LLaVA-Bench和MME上的实验表明，MPD将幻觉减少了23.4%，同时保持了97.4%的通用生成能力，且没有额外的计算成本。

Conclusion: MPD框架能够有效缓解大型视觉语言模型的幻觉问题，同时保持其通用生成能力，达到了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有线性注意力变体通常使用高斯核替代softmax来降低复杂度，但这些近似缺乏理论依据，并且容易过度抑制中程token交互。需要一种有理论基础的替代方案来解决Transformer在高分辨率视觉任务中的二次复杂度问题。

Method: 1) 使用拉普拉斯核作为softmax的原则性替代；2) 引入可证明的单射特征映射来保留细粒度token信息；3) 采用Nyström近似进行核矩阵近似；4) 使用Newton-Schulz迭代求解系统，避免昂贵的矩阵求逆和SVD；5) 开发定制CUDA实现支持高效前向和反向传播。

Result: 在ImageNet上的实验表明，LaplacianFormer在保持强性能的同时实现了良好的效率平衡，同时提高了注意力的表达能力。

Conclusion: LaplacianFormer通过拉普拉斯核提供了一种有理论基础的softmax注意力替代方案，结合高效的计算方法，为高分辨率视觉任务中的Transformer扩展提供了可行的解决方案。
...
TL;DR: SpaCeFormer：一种无提议的空间曲线Transformer，用于开放词汇3D实例分割，速度比多阶段2D+3D流水线快2-3个数量级（0.14秒/场景），同时构建了最大的开放词汇3D实例分割数据集SpaCeFormer-3M。
...
Method: 提出SpaCeFormer：无提议的空间曲线Transformer，结合空间窗口注意力和Morton曲线序列化实现空间一致性特征，使用RoPE增强的解码器直接从学习查询预测实例掩码，无需外部提议。同时构建SpaCeFormer-3M数据集（3.0M多视图一致标注，604K实例，7.4K场景）。

Result: 速度达0.14秒/场景，比多阶段流水线快2-3个数量级；在ScanNet200上实现11.1零样本mAP，比之前最佳无提议方法提升2.8倍；在ScanNet++和Replica上分别达到22.9和24.1 mAP，超越所有先前方法（包括使用多视图2D输入的方法）。

Conclusion: SpaCeFormer通过空间曲线Transformer架构实现了快速高效的开放词汇3D实例分割，无需外部提议，同时在多个基准测试中达到最先进性能，为机器人和AR/VR应用提供了核心能力。
...
TL;DR: 提出FTF两阶段检索框架，通过文本无关召回和文本引导重排，在遥感图像文本检索中同时实现细粒度对齐和高效检索。
...
Method: 提出快速-精细两阶段检索框架：1) 文本无关召回阶段使用粗粒度表示进行高效候选选择；2) 文本引导重排阶段使用无参数平衡文本引导交互块增强细粒度对齐；3) 设计跨模态和模态内损失联合优化多粒度表示的跨模态对齐。

Result: 在公共基准测试上的广泛实验表明，FTF在保持竞争力的检索准确性的同时，相比现有方法显著提高了检索效率。

Conclusion: FTF框架有效解决了遥感图像文本检索中细粒度对齐与检索效率的平衡问题，通过两阶段分解实现了高效且准确的跨模态检索。
...
TL;DR: 提出了CCTVBench基准测试，用于评估交通视频QA模型的对比一致性，要求模型在真实事故视频和反事实场景中做出正确判断，并引入C-TCD方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 安全关键的交通推理需要对比一致性：模型必须在事故发生时检测到真实危险，同时在几乎相同的反事实场景下可靠地拒绝看似合理但错误的假设。现有模型在这方面的表现存在不足。

Method: 1) 构建CCTVBench基准：基于真实事故视频和世界模型生成的反事实对应视频，配以最小差异、互斥的假设问题；2) 提出C-TCD对比解码方法，在推理时使用语义互斥的对立视频作为对比输入。

Result: 实验显示开源和专有视频LLM在标准QA指标和四元组对比一致性之间存在巨大差距，其中"以上都不是"的不可靠拒绝是关键瓶颈。C-TCD方法在实例级QA和对比一致性方面均有改进。

Conclusion: CCTVBench为安全关键交通推理提供了有效的评估框架，揭示了现有模型的对比一致性缺陷，C-TCD方法为解决这一问题提供了有前景的方向。
...
TL;DR: DynamicRad提出了一种统一的稀疏注意力范式，通过径向局部性先验实现自适应选择，采用双模式策略（静态比率和动态阈值），结合离线贝叶斯优化和语义运动路由器，在视频扩散模型中实现高效推理，获得1.7-2.5倍加速和超过80%的有效稀疏度。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型中自然时空能量衰减提供了效率提升的路径，但仅依赖刚性静态掩码可能在复杂动态中丢失关键的长程信息。需要一种既能保持效率又能保留重要信息的自适应稀疏注意力机制。

Method: 提出DynamicRad统一稀疏注意力范式，基于径向局部性先验进行自适应选择。采用双模式策略：静态比率模式用于速度优化执行，动态阈值模式用于质量优先过滤。通过离线贝叶斯优化管道结合语义运动路由器，将提示嵌入映射到最优稀疏度区域，最小化运行时开销。

Result: 在HunyuanVideo和Wan2.1-14B上的实验表明，DynamicRad在效率-质量帕累托前沿上取得突破，实现1.7-2.5倍推理加速，有效稀疏度超过80%。在某些长序列设置中，动态模式甚至匹配或超过密集基线，而掩码感知LoRA进一步改善了长时程一致性。

Conclusion: DynamicRad通过统一的稀疏注意力范式成功解决了视频扩散模型中效率与质量之间的权衡问题，通过自适应选择和离线优化实现了显著的推理加速，同时保持了生成质量，为视频生成模型的效率优化提供了有效解决方案。
...
TL;DR: Video-ToC提出了一种基于树状线索推理的视频理解框架，通过结构化推理模式增强模型感知能力，解决现有视频大语言模型在复杂视频理解中的推理能力不足和幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型在复杂视频理解方面存在局限，主要表现为推理能力有限和可能产生幻觉。这些方法主要依赖预训练的内在推理逻辑，缺乏对输入视频内容的感知自适应能力。

Method: 提出了三个关键创新：1）树引导的视觉线索定位机制，通过结构化推理模式增强模型的细粒度感知能力；2）推理需求奖励机制，基于推理需求估计动态调整强化学习的奖励值，实现按需激励更有效的推理策略；3）自动化标注流程，构建了Video-ToC-SFT-1k和Video-ToC-RL-2k数据集，分别用于监督微调和强化学习训练。

Result: 在六个视频理解基准测试和一个视频幻觉基准测试上的广泛评估表明，Video-ToC优于基线方法和近期方法。

Conclusion: Video-ToC通过树状线索推理框架有效提升了视频理解能力，解决了现有视频大语言模型在复杂推理任务中的局限性，为视频理解提供了新的解决方案。
...
TL;DR: RWoDSN是一种新的点云特征点提取方法，通过两阶段上下文相关分析，结合Disk Sampling Neighborhood描述符和随机游走，有效提取轮廓特征点，在多个指标上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 点云中能够完整勾勒模型形状的特征点对于点云处理任务至关重要，广泛应用于计算机图形学和计算机辅助设计。现有方法在提取特征点时需要考虑尖锐到平滑的过渡、大到小的尺度变化以及纹理到细节的特征。

Method: 提出两阶段上下文相关分析方法：第一阶段引入Disk Sampling Neighborhood（DSN）描述符，保持矩阵结构并维护法向邻域关系；第二阶段在DSN上执行随机游走（RWoDSN），生成基于图的DSN，同时考虑每个点周围局部表面的空间分布、拓扑属性和几何特征。

Result: 实验结果显示，RWoDSN方法的召回率达到0.769，比当前最先进方法高出22%，精确度为0.784。在八个评估指标上显著优于多种传统和深度学习方法。

Conclusion: RWoDSN方法通过创新的DSN描述符和随机游走策略，能够有效提取点云特征点，在多个性能指标上表现出色，为点云处理任务提供了可靠的基础。
...
TL;DR: 本文提出了RefAerial数据集，这是首个面向航拍图像的指称检测大规模挑战性数据集，并开发了SCS框架来解决航拍图像中的尺度变化问题。


<details>
  <summary>Details</summary>
Motivation: 现有指称检测数据集主要针对地面图像，其中物体通常较大且位于相对简单的场景中。航拍图像具有低但多样的物体-场景比例、大量目标和干扰物、复杂细粒度的指称描述等特点，现有方法在航拍图像上性能严重下降。

Method: 1) 开发了RefAerial数据集，包含4个特点：低但多样的物体-场景比例、大量目标和干扰物、复杂细粒度的指称描述、航拍视角的多样广阔场景；2) 设计了REA-Engine半自动标注引擎；3) 提出了SCS框架，包含混合粒度注意力机制和两阶段综合到敏感解码策略。

Result: 提出的SCS框架在RefAerial数据集上取得了显著性能，在传统地面指称检测数据集上也表现出有前景的性能提升。

Conclusion: RefAerial是首个面向航拍图像指称检测的大规模挑战性数据集，提出的SCS框架有效解决了航拍图像中的尺度变化问题，为航拍图像理解提供了新基准。
...
TL;DR: 论文提出了EVIAN框架，通过分解评估范式对视觉-语言模型训练数据进行细粒度质量审核，发现逻辑一致性是数据质量最关键因素，且小规模高质量数据训练效果优于大规模低质量数据。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型训练数据质量参差不齐，传统过滤方法依赖粗粒度评分，无法检测逻辑谬误或事实错误等细微语义缺陷，这成为开发更可靠模型的主要瓶颈。

Method: 提出"分解-评估"范式，将模型响应分解为视觉描述、主观推理和事实主张三个认知组件；构建包含30万样本的大规模基准数据集，系统注入多种细微缺陷；实现EVIAN框架，沿图像-文本一致性、逻辑一致性和事实准确性三个正交维度评估数据质量。

Result: 实验结果表明：1) 使用EVIAN筛选的小规模高质量数据训练模型，性能优于使用数量级更大数据集训练的模型；2) 将复杂审核任务分解为可验证子任务可实现稳健的数据筛选；3) 逻辑一致性是数据质量评估中最关键的因素。

Conclusion: 挑战了当前以规模为中心的训练范式，证明数据质量比数量更重要；提出的分解评估框架为视觉-语言模型训练数据提供了细粒度的质量审核方法，逻辑一致性应成为数据筛选的核心指标。
...
TL;DR: GSI-Bench：首个评估生成式空间智能的基准，通过空间约束下的图像编辑来衡量多模态模型的空间推理能力，包含真实和合成数据集，实验表明生成式训练能显著提升空间智能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的空间智能评估主要关注理解能力，但缺乏对生成式空间智能（GSI）的评估，即模型在图像生成过程中是否能够尊重和操作3D空间约束。

Method: 提出了GSI-Bench基准，包含两个互补组件：GSI-Real（通过3D先验引导的生成和过滤流程构建的高质量真实数据集）和GSI-Syn（具有可控空间操作和全自动标注的大规模合成基准），并设计了统一的评估协议。

Result: 实验表明，在GSI-Syn上微调统一多模态模型能显著提升合成和真实任务的表现，并且有趣的是，还能改善下游的空间理解能力。

Conclusion: 这是首次明确证据表明生成式训练能够切实增强空间推理能力，为推进多模态模型的空间智能开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 视线跟随在计算机视觉中已被广泛研究，但从未在手术室这一复杂高风险环境中探索。手术室中的视觉注意力对手术流程分析至关重要，本研究旨在将视线跟随概念引入手术领域，以理解临床角色、手术阶段和团队沟通。

Method: 扩展4D-OR和Team-OR数据集，添加视线跟随标注；提出基于视线热图的方法用于角色和阶段识别；为团队沟通检测训练自监督的时空模型，编码基于视线的片段特征，再输入到时间活动检测模型中。
...
<details>
  <summary>Details</summary>
Motivation: 实际生物识别系统（如欧洲出入境系统）需要在机场等非受控环境中采集人脸图像，无法保证标准背景，同时需要考虑无障碍需求，需要在传统办公室环境外进行图像采集

Method: 评估多种分割技术、三类变形攻击检测方法和四种人脸识别模型，使用包含受控和野外采集图像的数据集

Result: 分割技术对人脸识别性能和图像质量有系统性影响，同时也会影响变形攻击检测性能，揭示了预处理技术需要谨慎部署

Conclusion: 在操作型生物识别系统中部署背景校正预处理技术时需要仔细考虑，平衡可用性驱动的图像标准化与大规模生物识别系统的可靠性要求
...
TL;DR: 提出了一种结构增强的时序滑动窗口系统，用于在盲扫超声中检测胎儿腹部平面，通过分割先验增强腹部结构，结合时序信息稳定关键帧定位，提高生物测量可靠性。
...
Result: 广泛实验结果表明，结构增强的时序滑动策略显著改善并稳定了解剖学意义平面的检测，使盲扫超声中的生物测量更加可靠。

Conclusion: 该方法通过结合结构增强和时序信息，有效解决了盲扫超声中胎儿腹部平面检测的挑战，为低资源环境下的胎儿生长受限筛查提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统时间激光散斑对比成像需要长序列获取稳定统计，易受采集干扰且时间分辨率受限，需要从极少帧实现稳健重建的方法

Method: 1. 基于相位相关的配准稳定原始散斑序列；2. 利用物理先验校正运动；3. 条件扩散模型联合配准序列和校正先验进行逆重建

Result: 相比直接从少数帧重建和基线方法，在自建视网膜LSCI系统数据上显示改进的结构连续性和统计稳定性，在极端挑战性案例中仍有效

Conclusion: 为从极有限帧实现可靠的视网膜tLSCI重建提供了实用且物理基础的方法，代码和模型权重将公开
...
TL;DR: RSRCC是首个面向遥感变化问答的基准数据集，包含12.6万个问题，专注于局部化、变化特定的语义推理，填补了传统变化检测与自然语言描述之间的空白。


<details>
  <summary>Details</summary>
Motivation: 传统变化检测只能识别变化位置，无法用自然语言解释变化内容。现有的遥感变化描述数据集通常只描述图像级别的整体差异，缺乏对局部化语义推理的探索。为了填补这一空白，需要创建专门用于细粒度推理监督的遥感变化问答基准。

Method: 采用分层半监督筛选流程构建RSRCC数据集：1）从语义分割掩码中提取候选变化区域；2）使用图像-文本嵌入模型进行初步筛选；3）通过检索增强的视觉语言筛选和Best-of-N排序作为关键的最终歧义解决阶段，验证候选变化。这一流程能够可扩展地过滤噪声和模糊候选，同时保留语义上有意义的变化。

Result: 创建了RSRCC基准数据集，包含12.6万个问题，分为8.7万个训练样本、1.71万个验证样本和2.2万个测试样本。这是首个专门为细粒度推理监督设计的遥感变化问答基准，数据集已在Hugging Face平台公开。

Conclusion: RSRCC填补了遥感变化检测与自然语言理解之间的重要空白，通过局部化、变化特定的问答任务，为遥感图像的细粒度语义推理提供了新的监督基准，有望推动遥感领域更深入的变化理解研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决杂乱场景中6D物体姿态估计面临的严重遮挡和传感器噪声问题，现有方法在遮挡情况下定位精度和空间对齐效果不佳。

Method: 采用两阶段框架：1) MAPP阶段：通过掩码感知对应关系将2D对应提升到3D空间，建立可靠关键点匹配，基于对应级评分生成几何一致姿态假设，选择top-K候选；2) 细化阶段：集成AMPR模块的张量化渲染-比较管道，通过重建完整物体几何和动态调整ROI来缓解遮挡下的定位误差和空间错位，GPU加速的RGB-XYZ重投影实现单次前向传播同时细化所有姿态假设。

Result: 在BOP基准测试中达到76.5%的平均召回率(AR)，比FoundationPose提升3.1% AR，多物体推理速度提升43倍。

Conclusion: MAPRPose通过掩码感知对应关系和模态驱动ROI细化，有效解决了遮挡严重场景下的6D物体姿态估计问题，在精度和速度方面均达到最先进水平。
...
TL;DR: 论文提出当前视觉语言模型存在"功能性失明"问题，依赖语言先验而非真正视觉理解，并提出了量化评估视觉模态贡献的新方法
...
Method: 提出了一种根本性的信息论方法：模态翻译协议，通过翻译语义负载而非消融数据来量化揭示"视觉代价"。制定了三个新指标：视觉代价、视觉诅咒和视觉谬误，最终形成语义充分性准则。还提出了一个挑衅性的多模态缩放发散定律假设

Result: 通过提出的模态翻译协议和语义充分性准则，能够量化评估视觉模态的真实贡献，揭示当前模型依赖语言先验而非真正视觉理解的问题

Conclusion: 挑战KDD社区放弃追求虚幻的"多模态增益"，将语义充分性准则从被动诊断约束提升为主动架构蓝图，为下一代AI系统提供真正看到数据、实现真正多模态推理所需的严格、可信基础
...
TL;DR: R-CoV是一种视觉验证链方法，通过区域感知的验证步骤来减少大型视觉语言模型中的物体幻觉问题


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多种多模态理解和推理任务中表现出色，但仍然存在物体幻觉问题，即在视觉输入中声称存在不存在的物体。为了解决这一挑战，研究者受到人类理解复杂视觉信息方式的启发——人类通常会关注特定图像区域或细节——试图从LVLMs中引出类似的区域级处理能力。

Method: R-CoV包含六个步骤：初始响应生成、实体提取、坐标生成、区域描述、验证执行和最终响应生成。该方法无需训练，也不依赖外部检测模型，可以无缝集成到各种LVLMs中。

Result: 在多个广泛使用的幻觉基准测试和多个LVLMs上的大量实验表明，R-CoV能够显著减轻LVLMs中的物体幻觉问题。

Conclusion: R-CoV是一种简单而有效的后处理方法，通过区域感知的验证链机制，能够有效检测和缓解大型视觉语言模型中的物体幻觉问题，且无需额外训练或外部模型支持。
...
Method: 重新审视视觉领域的自监督学习（SSL），将广泛使用的SSL任务重新构建为一组可验证的视觉谜题，用于RL后训练，无需人工或外部模型监督。
...
TL;DR: GeoRelight是一个统一的多模态扩散Transformer模型，通过联合解决几何估计和重光照任务，避免了顺序管道的误差累积问题，实现了更好的物理一致性。
...
Method: 提出了GeoRelight模型，包含两个关键技术贡献：1) isotropic NDC-Orthographic Depth (iNOD)，一种与潜在扩散模型兼容的无失真3D表示；2) 结合合成和自动标注真实数据的混合数据训练策略。使用多模态扩散Transformer联合解决几何估计和重光照任务。

Result: 通过联合解决几何和重光照问题，GeoRelight在性能上优于顺序模型和先前忽略几何的系统。

Conclusion: 几何估计和重光照是相互促进的任务，通过统一的扩散Transformer模型联合解决这两个问题，能够实现更好的重光照效果和物理一致性。
...
TL;DR: OMIBench是一个专门评估大型视觉语言模型在多图像推理能力上的基准测试，聚焦于奥林匹克竞赛级别的多学科问题，现有模型表现有限。


<details>
  <summary>Details</summary>
Motivation: 当前奥林匹克级别的多模态推理基准主要关注单图像分析，缺乏对跨多图像上下文信息利用的评估，需要专门的基准来评估模型在多图像分布式证据下的推理能力。

Method: 开发了OMIBench基准，包含来自生物、化学、数学和物理奥林匹克竞赛的问题，这些问题需要跨多个图像的证据进行推理。基准提供了手动标注的推理过程和两种评估协议：精确答案匹配和语义答案匹配。

Result: 在OMIBench上的广泛实验显示，现有模型存在显著性能差距。即使是最强的大型视觉语言模型（如Gemini-3-Pro）在基准上也只能达到约50%的准确率。

Conclusion: OMIBench为研究和改进大型视觉语言模型的多图像推理能力提供了专门的资源，揭示了当前模型在处理需要跨多图像分布式证据的复杂推理任务上的局限性。
...
TL;DR: 提出Render-in-the-Loop新范式，通过视觉反馈循环改进SVG生成，解决现有MLLMs在SVG生成中忽视中间视觉状态的问题


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs生成SVG采用"盲目绘制"的开环方法，仅生成符号代码序列而不感知中间视觉结果，未能充分利用MLLMs的视觉先验知识，将SVG生成视为分离的文本序列建模任务而非集成的视觉空间任务

Method: 提出Render-in-the-Loop范式，将SVG合成重构为逐步、视觉上下文感知的过程；通过细粒度路径分解构建密集多步视觉轨迹；引入视觉自反馈训练策略；提出Render-and-Verify推理机制过滤退化冗余基元

Result: 在标准MMSVGBench上优于强基线模型，展示了Render-in-the-Loop范式在Text-to-SVG和Image-to-SVG任务上的显著数据效率和泛化能力

Conclusion: Render-in-the-Loop范式通过视觉反馈循环显著提升了MLLMs生成SVG的质量和效率，解决了现有方法忽视中间视觉状态的问题
...
TL;DR: Amodal SAM是一个统一框架，利用SAM模型进行amodal图像和视频分割，通过轻量级适配器、合成训练数据和新的学习目标，在保持SAM泛化能力的同时扩展其amodal分割能力。


<details>
  <summary>Details</summary>
Motivation: 现有amodal分割方法主要关注训练域内的分割，缺乏对新颖物体类别和未见场景的泛化能力。需要开发一个能够有效扩展到新场景的amodal分割框架。

Method: 提出Amodal SAM框架，包含三个关键改进：1) 轻量级空间完成适配器实现遮挡区域重建；2) 目标感知遮挡合成(TAOS)管道生成多样合成训练数据；3) 新的学习目标强制执行区域一致性和拓扑正则化。

Result: 在标准基准测试中实现了最先进的性能，同时展现出对新场景的鲁棒泛化能力。该框架将推动amodal分割系统在无约束真实环境中的实际应用。

Conclusion: Amodal SAM通过结合SAM的强大泛化能力和专门设计的amodal分割组件，为实际amodal分割系统的发展提供了有前景的方向，能够有效处理新颖物体和未见场景。
...
Method: 提出GeoRect4D统一框架：1）退化感知反馈机制，结合基于锚点的鲁棒动态3D高斯溅射基底与单步扩散校正器来生成高保真细节；2）结构锁定机制和时空协调注意力保持物理合理性；3）渐进优化策略，使用随机几何净化消除漂浮物，并通过生成蒸馏将纹理细节注入显式表示。

Result: 在多个数据集上的广泛实验表明，GeoRect4D在重建保真度、感知质量和时空一致性方面达到最先进性能。

Conclusion: GeoRect4D通过闭环优化有效耦合显式3D一致性与生成式细化，成功解决了稀疏视图动态重建中的几何崩溃、轨迹漂移和漂浮伪影问题，实现了高质量、物理合理的动态场景重建。
...
TL;DR: LLaDA2.0-Uni是一个统一的多模态离散扩散大语言模型，支持视觉理解和生成，通过语义离散化、MoE架构和扩散解码器实现文本与图像的联合处理。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型通常将理解和生成分离，缺乏统一的框架。本文旨在开发一个原生集成的模型，同时支持多模态理解和生成，为下一代统一基础模型建立可扩展的范式。

Method: 1. 使用SigLIP-VQ将连续视觉输入离散化为语义token；2. 采用MoE架构的离散扩散大语言模型作为主干；3. 使用扩散解码器将视觉token重建为高质量图像；4. 通过前缀感知优化和少步蒸馏提升推理效率；5. 基于大规模数据和多阶段训练流程。

Result: 模型在多项任务中表现出色：1. 多模态理解能力与专业视觉语言模型相当；2. 在图像生成和编辑方面表现强劲；3. 支持交错生成和推理；4. 推理效率通过优化得到显著提升。

Conclusion: LLaDA2.0-Uni成功实现了多模态理解和生成的统一框架，为下一代基础模型提供了有前景且可扩展的范式。代码和模型已开源，支持进一步研究和应用。
...
TL;DR: LEXIS-Flow：通过离散交互特征流形和扩散模型，从单张RGB图像重建3D人-物交互，使用连续邻近场表示实现物理合理的重建。
...
Method: 提出InterFields表示法编码身体和物体表面的密集连续邻近关系；通过VQ-VAE学习离散交互特征流形LEXIS；开发LEXIS-Flow扩散框架，利用LEXIS特征估计人体和物体网格及其InterFields。

Result: 在Open3DHOI和BEHAVE数据集上，LEXIS-Flow在重建、接触和邻近质量方面显著优于现有SotA基线，提高了泛化能力，重建结果被认为更真实。

Conclusion: LEXIS-Flow通过结构化交互模式和连续邻近场表示，实现了更物理合理、更真实的3D人-物交互重建，推动了整体3D场景理解的发展。
...
<details>
  <summary>Details</summary>
Motivation: Transformer OCR模型在拉丁和CJK文字上表现良好，但在非洲音节文字系统（特别是提格里尼亚语的Ge'ez文字）上的应用有限，现有模型无法处理这种新文字系统

Method: 1. 从预训练模型开始；2. 扩展字节级BPE分词器以覆盖230个Ge'ez字符；3. 引入词感知损失加权解决拉丁中心BPE惯例在新文字系统中产生的系统词边界错误

Result: 未修改的模型在Ge'ez文字上无法产生可用输出。适配后的TrOCR-Printed变体在GLOCR数据集的5000张合成测试图像上达到0.22%字符错误率和97.20%完全匹配准确率。消融实验证实词感知损失加权是关键组件，相比仅扩展词汇表，将CER降低了两个数量级

Conclusion: 成功将TrOCR适配到提格里尼亚语Ge'ez文字，通过词汇扩展和词感知损失加权解决了新文字系统的识别问题，训练时间短（单8GB消费级GPU不到3小时），所有代码、模型权重和评估脚本均已公开
...
TL;DR: 该研究构建了全球首个基于Sentinel-1 SAR时间序列的海上风电基础设施监测数据集，包含2016-2025年间15,606个时间序列和1,480万次事件，提供分析就绪的1D后向散射剖面、基线语义标签和专家标注基准数据集。


<details>
  <summary>Details</summary>
Motivation: 海上风电行业快速发展，需要独立、高时间分辨率的全球基础设施监测。现有开放数据集缺乏时间密集和语义细粒度的建设与运营动态信息，无法满足对部署和运营阶段的持续监测需求。

Method: 基于更新的目标检测工作流程，从2016Q1到2025Q1的Sentinel-1 SAR时间序列数据中提取基础设施位置；构建了15,606个时间序列，包含1,480万次事件作为分析就绪的1D SAR后向散射剖面；开发基于规则的分类器生成基线语义标签；创建专家标注的553个时间序列基准数据集。

Result: 基线分类器在事件级评估中达到宏观F1分数0.84，在压缩编辑相似性-质量阈值曲线下面积(AUC)为0.785，表明时间一致性良好。数据集支持全球部署动态分析、区域部署模式差异识别、船舶交互和运营事件检测。

Conclusion: 该研究创建的数据集填补了海上风电基础设施时间密集监测的空白，为全球尺度分析提供了标准化数据基础，同时为开发和时间序列分类方法比较提供了基准参考，支持更精细的基础设施监测应用。
...
TL;DR: 该研究对伏尼契手稿的字符序列进行了系统分析，发现了独特的双向结构特征，并测试了两种生成模型，为未来研究提供了定量基准。


<details>
  <summary>Details</summary>
Motivation: 伏尼契手稿的字符序列来源不明且难以进行语言分析，研究者希望系统分析其结构特征，并评估现有生成模型能否复制这些特征。

Method: 1. 系统分析手稿的字符序列结构；2. 与四种对比语言（英语、法语、希伯来语、阿拉伯语）进行比较；3. 测试两类结构化生成器：参数化槽位生成器和Cardan格栅实现Rugg的胡言假说；4. 使用四特征联合标准进行评估。

Result: 1. 发现伏尼契手稿具有独特的双向结构：词内字符级右到左优化和词边界左到右依赖，这在对比语言中未观察到；2. 两类生成器在其完整测试参数空间中均无法同时复制所有四个特征。

Conclusion: 研究结果为未来伏尼契手稿的生成或密码分析模型提供了首个定量基准，表明手稿具有类似密码的结构约束，这些约束难以通过简单的位置或频率机制单独复制。
...
TL;DR: 研究发现幻觉神经元（H-neurons）在不同知识领域间不具通用性，幻觉机制具有领域特异性，而非单一通用机制


<details>
  <summary>Details</summary>
Motivation: 先前研究发现稀疏的"幻觉神经元"能预测大语言模型产生幻觉的情况，但尚不清楚这些神经元是否能在不同知识领域间通用

Method: 采用系统性的跨领域转移协议，在6个不同领域（通用QA、法律、金融、科学、道德推理、代码漏洞）和5个开放权重模型（3B到8B参数）上进行实验，评估H-neurons的跨领域泛化能力

Result: H-neurons在领域内AUROC达到0.783，但跨领域转移时仅0.563（delta=0.220，p<0.001），在所有测试模型中表现一致，表明幻觉神经元不具有跨领域通用性

Conclusion: 幻觉不是单一机制，而是涉及领域特异性神经元群体，神经元级幻觉检测器需要按领域校准，不能一次训练后通用
...
TL;DR: 该研究提出一个量化语言模型认知-修辞分离的框架，发现LLM生成的文本在修辞强度与认知基础之间存在系统性错配，表现为形式-意义分歧显著增加、修辞设备分布更均匀等特征。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）表现出系统性的校准错误，其修辞强度与认知基础不成比例。本研究旨在测试这一假设，并建立一个量化这种分离的框架。

Method: 设计了一个三元认知-修辞标记（ERM）分类法，通过形式-意义分歧（FMD）、真实-表现认知比率（GPR）和修辞设备分布熵（RDDE）等复合指标进行操作化。该方法应用于225篇议论文本（约60万词符），涵盖人类专家、非专家和LLM生成的子语料库。

Result: LLM生成的文本表现出一致的、模型无关的认知特征：三连词使用率是专家的近两倍，而人类作者使用反问词的频率是LLM的两倍以上；表现性犹豫标记在LLM输出中的密度是人类的两倍；LLM文本的FMD显著高于人类组，修辞设备分布更均匀。

Conclusion: 研究结果与Grice语用学、关联理论和Brandom推理主义的理论直觉一致。该标注流程完全可自动化，可作为AI生成内容中认知校准错误的轻量级筛查工具，以及LLM生成文本检测的理论驱动特征集。
...
TL;DR: TTKV：受人类记忆系统启发的KV缓存管理框架，通过分层存储和基于时间接近度的分配策略，显著减少内存占用和提升推理效率


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存方法将不同时间的KV状态视为同等重要，采用统一精度和访问方式，这与人类记忆系统（记忆清晰度、回忆频率和时近性不同）形成鲜明对比，导致内存占用随上下文长度线性增长，成为严重可扩展性瓶颈

Method: 提出TTKV框架，将人类记忆系统映射到KV缓存：1) 分层布局：使用HBM和DRAM分离快速和慢速内存；2) 分层内容：基于时间接近度将较新的KV状态分配到更快、更高精度的层级；3) 分层交互：采用块级流式注意力在访问慢速层级时重叠通信和计算

Result: 在128K上下文任务中，TTKV将跨层流量减少5.94倍，相比强基线实现高达76%的延迟降低和2倍的吞吐量提升

Conclusion: TTKV通过模拟人类记忆系统的分层特性，有效解决了KV缓存的内存可扩展性问题，为大规模语言模型的高效推理提供了创新解决方案
...
<details>
  <summary>Details</summary>
Motivation: 日本建筑许可审查需要跨修订周期交叉引用大型PDF文档集，手动执行这一过程既劳动密集又容易出错。

Method: 算法结合了最长公共子序列(LCS)结构对齐、七阶段共识匹配管道和动态规划最优对齐阶段，能够稳健地在修订版本间配对页面，即使页面顺序、编号或内容发生重大变化。随后采用多层差异引擎，包括文本级、表格级和像素级视觉差异检测，生成高亮差异报告。

Result: 在真实建筑许可文档集上的评估显示，在手动标注的基准测试中达到F1=0.80和精确率=1.00，匹配对零误报。

Conclusion: 该混合多阶段页面匹配算法能够有效自动化日本建筑许可文档的比较过程，显著提高审查效率和准确性。
...
TL;DR: CoAuthorAI是一个人类在环的写作系统，通过检索增强生成、专家设计的层次化大纲和自动参考文献链接，帮助LLMs从文章写作扩展到书籍长度的科学写作任务。
...
Method: CoAuthorAI结合检索增强生成、专家设计的层次化大纲和自动参考文献链接，采用人类在环的方法，允许专家在句子级别迭代精炼文本，确保连贯性和准确性。

Result: 在500个多领域文献综述章节评估中，CoAuthorAI达到最大软标题召回率98%；在100篇文章的人类评估中，生成内容满意度达82%。使用该系统与Kexin Technology的LUFFA AI模型合作完成的《AI for Rock Dynamics》已在Springer Nature出版。

Conclusion: 系统化的人机协作可以扩展大型语言模型的能力，从文章写作延伸到完整书籍的创作，实现更快、更可靠的科学出版。
...
Method: 提出PR-CAD渐进式精炼框架：1）构建涵盖完整CAD生命周期的高保真交互数据集，包含多种CAD表示和描述；2）设计适合LLM的CAD表示；3）开发强化学习增强的推理框架，将意图理解、参数估计和精确编辑定位集成到单一智能体中。

Result: 在公开基准测试中，PR-CAD在生成和精炼场景中实现了最先进的可控性和忠实度，证明生成和编辑任务之间存在强相互增强关系，且显著提高了CAD建模效率。

Conclusion: PR-CAD提供了一个"一体化"解决方案，统一了CAD设计创建和精炼，通过渐进式精炼框架实现了可控且忠实的文本到CAD建模，提高了用户友好性和建模效率。
...
TL;DR: 本研究评估了电子健康记录集成的大型语言模型在生成出院小结草稿方面的应用效果，结果显示该技术能显著减少文档记录时间和行政工作量，用户接受度高。


<details>
  <summary>Details</summary>
Motivation: 撰写出院小结以传递医疗信息是一个重要但耗时的过程，大型语言模型可能为此提供帮助，但需要在实际临床环境中评估其效果。

Method: 采用前瞻性混合方法试点研究，在学术医院的临床实践中评估电子健康记录集成的大型语言模型生成出院小结草稿的效果，共涉及21名住院医师和4名医师助理在9周内生成的379份出院小结。

Result: 58.5%的入院记录中复制了LLM生成的文本，29.1%的最终出院信函中可追溯到可识别的LLM内容；86.9%的用户自我报告文档记录时间减少，60.9%报告行政工作量减少；试点阶段后使用意向高达91.3%。

Conclusion: LLM辅助的出院小结生成在临床实践中显示出积极效果，支持进一步实施该应用场景；准确测量用户在出院小结上的文档记录时间仍然具有挑战性，但这对未来LLM辅助文档的外部评估是必要的。
...
<details>
  <summary>Details</summary>
Motivation: 结核病是南非医疗系统的重大负担，需要开发专门的大语言模型来帮助患者和医疗提供者，减轻医疗系统压力

Method: 1) 文献回顾了解医疗领域LLM开发策略；2) 收集南非结核病指南、相关文献和医疗基准数据集；3) 使用QLoRA算法在BioMistral-7B上进行微调；4) 结合GraphRAG实现检索增强生成

Result: 开发的领域特定LLM在词汇、语义和知识层面的上下文对齐方面表现优于基础BioMistral-7B模型和通用LLM

Conclusion: 针对特定医疗领域开发的大语言模型能够更好地适应当地医疗需求，为结核病护理提供更准确、相关的支持
...
TL;DR: 提出SDSR框架，利用LLM的首位偏见而非对抗它，通过在结构化数据文件开头嵌入人工编写的导航元数据，结合双重引导策略，显著提升长上下文中的信息检索准确率。


<details>
  <summary>Details</summary>
Motivation: LLM在处理长输入上下文时存在"中间迷失效应" - 中间信息比边界信息获得更少关注，这限制了将大型结构化知识库直接嵌入LLM上下文的知识检索应用。RAG虽然通过检索相关片段解决了可扩展性问题，但引入了大量基础设施开销，且不适用于语义边界由人工定义而非统计学习的库。

Method: 提出自我描述结构化检索(SDSR)框架：1) 在结构化数据文件的开头位置嵌入人工编写的导航元数据，利用LLM的首位偏见；2) 采用双重引导策略，结合文件内元数据和系统提示中的显式路由规则。

Result: 在四轮基准测试中，使用190个技能库（从36个扩展到119个类别，通过对抗性干扰注入），测试四种条件：(A)无引导(65%准确率)、(B)仅文件内摘要、(C)仅提示提示、(D)两者结合(100%准确率)。版本D在119个类别中实现了100%的主要路由准确率(20/20)。

Conclusion: SDSR框架通过利用LLM的首位偏见而非对抗它，显著提升了长上下文中的信息检索性能。发现基本不对称性：主要路由可通过显式规则解决，而次要跨类别路由需要在数据结构中显式编码架构意图。SDSR还可扩展到半结构化语料库，展示如何通过交叉引用编码在具有可恢复文档结构的领域中无需向量数据库即可操作。
...
<details>
  <summary>Details</summary>
Motivation: 当前大音频语言模型（LALMs）在多语言语音理解方面取得进展，但针对非英语语言（特别是韩语）的评估基准仍然稀缺，需要专门的韩语基准测试来评估模型的语音理解和语音忠实度能力。

Method: 构建了KoALa-Bench基准测试，包含6个任务：4个评估基础语音理解能力（自动语音识别、语音翻译、语音问答、语音指令跟随），2个评估语音忠实度（基于观察到LALMs常未能充分利用语音模态）。基准还整合了韩国高考听力题和韩国文化领域内容，以反映韩国特定知识。

Result: 对6个模型（包括白盒和黑盒模型）进行了广泛实验，建立了公开可用的基准测试、评估代码和排行榜，为韩语LALMs评估提供了标准化工具。

Conclusion: KoALa-Bench填补了韩语大音频语言模型评估基准的空白，为研究社区提供了全面的评估框架，有助于推动韩语语音理解技术的发展。
...
TL;DR: 研究分析了41种说服策略与捐款行为的关系，发现策略类别本身对捐款结果的解释力很弱，只有"内疚诱导"策略显著降低捐款率，而"互惠"策略与捐款正相关。


<details>
  <summary>Details</summary>
Motivation: 需要了解哪些具体说服策略与捐款依从性相关，这需要在整个语料库上进行细粒度策略标注，并进行多重比较校正的统计检验。

Method: 使用三个开源大语言模型(Qwen3:30b, Mistral-Small-3.2, Phi-4)对PersuasionForGood语料库中的10,600个说服者话语进行标注，采用包含41种策略、11个类别的分类体系，分析策略与捐款结果的关系。

Result: 策略类别仅能解释捐款结果约1.5%的方差；内疚诱导是唯一显著降低捐款率的策略（约降低23个百分点），这一效应在三个模型中均得到复现；互惠是最稳健的正相关因素；目标对象的情感和兴趣能预测是否捐款，但与捐款金额的相关性较弱。
...
TL;DR: HumorRank：基于锦标赛的LLM幽默生成评估框架，通过配对比较和Bradley-Terry模型生成统一排名，发现幽默质量取决于喜剧机制掌握而非模型规模


<details>
  <summary>Details</summary>
Motivation: 现有LLM幽默评估方法产生孤立、不可比较的指标，缺乏统一模型排名，难以追踪系统进展

Method: 使用SemEval-2026 MWAHAHA测试数据集，基于GTVH理论进行自动化配对评估，通过自适应瑞士锦标赛和Bradley-Terry最大似然估计生成全局一致的幽默生成能力排名

Result: HumorRank产生统计上可靠的模型分层，显示幽默质量由喜剧机制掌握程度驱动而非模型规模

Conclusion: HumorRank为LLM生成幽默提供了可扩展、可解释的基准测试方法学
...
TL;DR: 本文系统分析了LLM后训练量化中的两种失败模式：信号退化和计算崩溃，并提出了相应的诊断和修复策略。


<details>
  <summary>Details</summary>
Motivation: 虽然4位量化被认为是理想的权衡点，但将精度降低到2位通常会导致灾难性的性能下降。目前尚不清楚这两种情况下的失败机制是否根本不同，因此需要进行系统性的机理分析。

Method: 进行系统的机理分析，识别出两种性质不同的失败模式：信号退化（计算模式保持完整但信息精度受损）和计算崩溃（关键组件失效导致早期层信号破坏）。基于此诊断，进行机制感知的干预实验。

Result: 研究发现有针对性的免训练修复可以缓解信号退化，但对计算崩溃无效。计算崩溃需要结构重建而非简单的补偿。

Conclusion: 该研究为PTQ失败提供了系统的诊断框架，表明解决计算崩溃需要结构重建而非简单的补偿，为高效部署LLM提供了重要的理论指导。
...
TL;DR: 使用大语言模型对Reddit帖子进行抑郁症风险评估，通过多标签情感分类和加权严重指数计算，在零样本设置下达到与专门微调模型竞争的性能，并应用于大规模社交媒体数据分析。
...
Method: 基于大语言模型（LLMs）的系统，对Reddit帖子进行抑郁症风险评估，通过多标签分类识别八种与抑郁症相关的情感，并计算加权严重指数。在DepressionEmo数据集（约6000条帖子）上进行零样本评估，并将方法应用于2024-2025年期间从四个subreddit收集的469,692条评论。

Result: 最佳模型gemma3:27b达到micro-F1=0.75和macro-F1=0.70，与专门微调的BART模型（micro-F1=0.80，macro-F1=0.76）性能相当。实际应用分析显示不同社区间存在一致且时间稳定的风险特征，r/depression和r/anxiety之间存在明显差异。

Conclusion: 研究证明了基于大语言模型的大规模心理健康监测方法的可行性，提供了一种成本效益高、可扩展的解决方案。
...
TL;DR: 该论文针对大语言模型在否定语义理解上的困难，提出了一种自动增强常识知识库的方法，通过添加否定关系创建了两个包含200多万个三元组的新语料库，并证明在此语料上预训练能提升模型的否定理解能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理自然语言理解任务中的否定语义时存在困难，而现有的常识知识研究缺乏对否定关系的深入探讨。作者旨在解决模型在涉及否定的常识知识理解方面的挑战。

Method: 提出了一种新颖的自动增强方法，将否定关系添加到现有的常识知识语料库中，创建了两个包含超过200万个"如果-那么"关系三元组的新语料库。

Result: 成功构建了两个大规模包含否定关系的常识知识语料库，并证明在大语言模型上使用这些语料进行预训练能够有效提升模型对否定语义的理解能力。

Conclusion: 否定关系在常识知识理解中具有重要价值，通过自动增强现有语料库并添加否定关系，可以有效改善大语言模型在否定语义理解方面的表现，为相关研究提供了新的资源和方向。
...
TL;DR: 研究大语言模型在文本生成中如何回忆关系知识，重点关注通过线性探针进行关系分类的潜在表示，发现注意力头对残差流的贡献是最佳特征，探针准确性与关系特异性、实体连接度等特征相关。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明注意力头和MLP如何交互以解析主语、谓语和宾语，但尚不清楚哪些表示支持可靠的线性关系分类，以及为什么某些关系类型比其他类型更容易被线性捕获。

Method: 系统评估从注意力头和MLP贡献中提取的不同潜在表示，使用线性探针进行关系分类，并通过特征归因分析探针行为，研究关系特异性、实体连接度等特征对探针准确性的影响。

Result: 发现每个注意力头对残差流的贡献是线性关系分类的较强特征；探针准确性与关系特异性、实体连接度以及信号在注意力头间的分布程度存在明显相关性；通过token级特征归因可以详细揭示探针行为。

Conclusion: 注意力头对残差流的贡献是关系分类的有效特征，探针性能与关系类型特征密切相关，特征归因方法有助于深入理解探针工作机制，为理解大语言模型如何编码关系知识提供了新见解。
...
TL;DR: 该研究分析了厄瓜多尔和秘鲁6,323个COVID-19开放式回答的渐进式健康素养标注，发现标注分歧主要源于问题概念难度而非标注者个体差异，且关键社会效应在不同标注一致性水平上呈现不同甚至相反的方向。


<details>
  <summary>Details</summary>
Motivation: 传统NLP标注流程假设每个实例存在单一潜在真实标签，通过标签聚合解决分歧。透视主义方法挑战这一观点，认为分歧可能具有信息价值而非错误。本研究旨在通过大规模渐进式健康素养标注分析，探讨标注分歧的结构和意义。

Method: 收集厄瓜多尔和秘鲁6,323个开放式COVID-19回答，由多位标注者使用比例正确性分数独立标注（反映回答与公共卫生指南的符合程度）。采用方差分解分析分歧来源，通过一致性分层分析考察关键社会效应在不同标注一致性水平上的变化。

Result: 方差分解显示问题级概念难度比标注者身份解释了更多方差，表明分歧主要由任务本身而非个体标注者驱动。一致性分层分析发现国家、教育水平和城乡差异等关键社会效应在不同标注一致性水平上幅度变化，有时甚至方向相反。

Conclusion: 渐进式健康素养评估包含认知稳定和不稳定成分，聚合这些成分可能掩盖重要的推断差异。在渐进式解释性任务中，强透视主义建模不仅概念上合理，统计上也是有效推断的必要条件。
...
TL;DR: TriEx是一个三视角可解释性框架，用于分析LLM智能体在交互式部分可观测环境中的决策过程，通过第一人称自我推理、第二人称对手信念状态和第三人称环境审计三种视角提供证据锚定的解释。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体在交互式、部分可观测环境中的可解释性特别具有挑战性，因为决策依赖于不断演变的信念和其他智能体。现有解释通常是自由形式的叙述，难以进行系统比较和验证。

Method: TriEx框架通过三种对齐的视角来检测顺序决策：1) 与行动绑定的结构化第一人称自我推理；2) 随时间更新的显式第二人称对手信念状态；3) 基于环境衍生参考信号的第三人称审计。将解释从自由叙述转变为证据锚定的对象。

Result: 在不完美信息战略游戏的受控测试环境中，TriEx能够扩展分析解释的忠实度、信念动态和评估者可靠性，揭示了智能体所说、所信和所做之间的系统性不匹配。

Conclusion: 可解释性是一个依赖于交互的属性，需要多视角、基于证据的评估框架。TriEx为LLM智能体提供了系统化的解释分析工具，有助于理解智能体在复杂交互环境中的决策过程。
...
TL;DR: 研究发现大型语言模型在描述和评估城市时并非文化中立，而是以西方文化为基准，形成文化不平等的参考框架，影响对城市的感知和评价。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地用于描述、评估和解释城市空间，但尚不清楚它们是否从文化中立的角度进行这些操作。本研究旨在测试前沿LLMs在城市感知方面是否存在文化偏见。

Method: 使用平衡的全球街景样本，设计中性提示和不同区域文化立场的提示，通过开放式描述和结构化地点判断来测试LLMs的城市感知。同时与区域人类文本-图像基准进行比较。

Result: 中性提示在实践中并不中性；与欧洲和北美相关的提示比许多非西方提示更接近基线，表明模型感知围绕文化不平等的参考框架组织。文化提示还改变了情感评价，产生基于情感的群体内偏好。与人类基准比较显示，文化相近的提示可以提高与人类描述的一致性，但无法恢复人类的语义多样性，并保留了情感提升的风格。

Conclusion: LLMs并非从"无立场"的角度感知城市，而是通过文化不平等的基线来感知，这种基线塑造了什么是普通、熟悉和积极评价的内容，影响了城市感知和评价。
...
TL;DR: 提出POP自博弈框架，让LLM同时生成评估标准、输入输出对，并基于内容丰富的预训练语料库减少奖励攻击和模式崩溃，在多种开放任务上提升性能


<details>
  <summary>Details</summary>
Motivation: 自博弈在可验证任务（如数学、编程）中表现良好，但需要扩展到更现实的开放任务；传统方法需要昂贵的人工或专有模型生成高质量输入输出对，自博弈能最小化监督成本

Method: 提出POP自博弈框架：1）让同一LLM为每个示例合成评估标准（rubric）和输入输出对；2）基于内容丰富的预训练语料库进行基础化，确保生成-验证差距，减少奖励攻击，防止模式崩溃

Result: 在Qwen-2.5-7B模型上，POP提升了预训练和指令调优模型的性能，覆盖从长格式医疗问答到创意写作和指令遵循等多种任务

Conclusion: POP框架成功将自博弈扩展到开放任务，通过让LLM生成评估标准和内容，结合预训练语料库基础化，有效提升了模型在各种现实任务上的表现
...
TL;DR: UL-XCoT是一个高效的多语言推理框架，通过语言不变统一逻辑空间选择候选语言、监控逻辑空间轨迹动态剪枝低质量推理路径，显著减少解码token成本超过50%，同时保持竞争力准确率。
...
Method: UL-XCoT采用三个关键技术：(1)在语言不变统一逻辑空间为每个查询选择小候选语言集；(2)解码时监控逻辑空间轨迹动态剪枝低质量推理路径；(3)通过投票聚合剩余高质量轨迹。

Result: 在18种语言的PolyMath和29种语言的MMLU-ProX-Lite数据集上，UL-XCoT在保持竞争力准确率的同时，相比先前采样基线减少超过50%解码token成本，在低资源语言上表现更稳定。
...
TL;DR: SCG-MEM提出了一种基于认知图式的生成式记忆架构，通过模式约束生成避免结构幻觉，相比传统稠密检索方法在LoCoMo基准上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于稠密检索的智能体记忆系统依赖语义重叠或实体匹配，难以区分语义相似但上下文不同的实例，导致检索噪声。而直接使用开放式生成进行记忆访问又会产生"结构幻觉"问题，生成不存在的记忆键导致查找失败。

Method: 提出SCG-MEM架构，将记忆访问重新定义为模式约束生成。通过维护动态认知图式，严格约束LLM解码只生成有效的记忆条目键，提供形式化保证避免结构幻觉。支持通过同化（将输入融入现有图式）和顺应（用新概念扩展图式）进行记忆更新，并构建关联图支持多跳推理。

Result: 在LoCoMo基准测试中，SCG-MEM在所有类别上都显著优于基于检索的基线方法。

Conclusion: 基于建构主义认识论，记忆本质上由认知图式组织，有效回忆必须是在这些图式结构内进行的生成过程。SCG-MEM通过模式约束生成实现了这一理念，为智能体记忆系统提供了更可靠的方法。
...
TL;DR: 该研究探讨了使用大语言模型进行定性分析时可能存在的偏见问题，特别是LLM在解释人生故事时可能存在的种族和性别偏见。


<details>
  <summary>Details</summary>
Motivation: 随着越来越多的研究探索使用大语言模型加速或扩展文本数据的定性分析，虽然可以直接比较LLM与人类标签在演绎编码中的准确性，但评估LLM在归纳性主题分析等抽象方法中的伦理和效果更具挑战性。研究者希望了解LLM作为意义解释者如何影响研究的结论和视角。

Method: 研究者与心理学家合作，研究LLM对人类人生故事所做的抽象性主张。提出了一种基于摘要的流程，用于揭示LLM在解释这些人生故事时可能采用的视角偏见。

Result: 研究表明，该流程能够识别出LLM在解释人生故事时存在的种族和性别偏见，这些偏见可能导致代表性伤害。

Conclusion: 建议在涉及LLM解释研究参与者书面文本或转录语音的未来研究中采用这种分析方法，以描绘研究的立场性画像，提高研究的伦理性和有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型表示模型（如VLM2Vec）在多模态理解方面表现出色，但在细粒度语义理解方面存在不足，难以区分高度相似的商品。电商任务（如相同产品检索）需要更精细的语义理解能力。

Method: AFMRL将产品细粒度理解定义为属性生成任务，利用多模态大语言模型（MLLM）从产品图像和文本中提取关键属性。采用两阶段训练框架：1）属性引导对比学习（AGCL），使用MLLM生成的关键属性在图像-文本对比学习中识别困难样本并过滤噪声负样本；2）检索感知属性强化（RAR），将属性集成后表示模型的检索性能改进作为奖励信号，在多模态微调中增强MLLM的属性生成能力。

Result: 在大规模电商数据集上的广泛实验表明，该方法在多个下游检索任务上达到了最先进的性能，验证了利用生成模型推进细粒度表示学习的有效性。

Conclusion: AFMRL通过将生成模型与表示学习相结合，成功提升了电商场景下的细粒度多模态理解能力，为利用生成模型增强表示学习提供了有效框架。
...
TL;DR: 本文提出了一个三层信任框架，整合了AI研究和心理健康实践中的不同视角，用于评估心理健康支持AI系统的可信度，并指出了当前评估与实际需求之间的差距。


<details>
  <summary>Details</summary>
Motivation: 心理健康支持AI系统的"可信"概念在不同学科间定义松散且操作不一致。AI研究关注技术标准（如鲁棒性、可解释性、安全性），而治疗实践者强调治疗保真度（如适当性、共情、长期用户结果）。需要弥合这种碎片化的视角。

Method: 提出了一个三层信任框架，涵盖人本导向、AI导向和交互导向的信任，整合了关键利益相关者（如从业者、研究人员、监管者）的观点。使用该框架系统回顾了心理健康领域的现有AI驱动研究，并审查了从自动指标到临床验证方法的"可信"评估实践。

Result: 通过框架分析，突出了当前NLP测量内容与实际心理健康环境需求之间的关键差距。识别了技术导向评估与临床实践需求之间的不匹配。

Conclusion: 为构建社会技术对齐且真正可信的心理健康支持AI制定了研究议程，强调需要弥合技术评估与临床实践之间的鸿沟，实现更全面的可信AI系统。
...
TL;DR: Duluth团队在SemEval-2026 Task 6中提出基于DeBERTa-V3-base的系统，通过LLM数据增强和多种技术改进，在政治问题回避检测任务中取得第8名成绩


<details>
  <summary>Details</summary>
Motivation: 解决政治访谈中问题-回答对的两级清晰度分类任务，应对训练数据中的类别不平衡问题，提升对细微政治话语任务的处理能力

Method: 基于DeBERTa-V3-base模型，结合焦点损失、分层学习率衰减和布尔话语特征；使用Gemini 3和Claude Sonnet 4.5生成合成样本来增强少数类数据

Result: 在Task 1评估集上获得0.76的宏观F1分数，在40个团队中排名第8；最佳系统得分为0.89，参与者平均分为0.70；错误分析显示主要混淆在"矛盾回答"和"清晰回答"之间

Conclusion: LLM数据增强能显著提升少数类召回率，但系统在区分细微语义差异方面仍有挑战，这与人类标注者的分歧模式相似
...
TL;DR: 本文提出LAURA方法解决多语言检索增强生成中的语言偏见问题，通过语言无关的效用驱动重排对齐，显著提升跨语言证据检索效果。


<details>
  <summary>Details</summary>
Motivation: 当前多语言检索增强生成系统在重排阶段存在语言偏见，系统性地偏向英语和查询的母语，导致无法充分利用分散在多种语言中的关键证据，限制了生成性能。

Method: 提出LAURA（语言无关的效用驱动重排对齐）方法，通过将多语言证据排序与下游生成效用对齐，使用估计的oracle证据分析量化性能差距，并设计机制来识别和保留分散在多种语言中的"答案关键"文档。

Result: 实验表明LAURA能有效缓解语言偏见，在不同语言和生成模型上一致提升mRAG性能，缩小了现有重排器与理论上限之间的性能差距。

Conclusion: 语言偏见是多语言检索增强生成中的关键瓶颈，LAURA通过语言无关的效用驱动重排对齐方法，能够更有效地利用跨语言证据，提升整体生成性能。
...
TL;DR: 本文提出Quantile Token Regression方法，通过向输入序列插入专用分位数标记，为每个分位数建立直接输入-输出路径，并结合检索机制引入相似实例的分布信息，显著提升了LLM在分位数回归任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在文本回归应用中需要预测完整的条件分布而非单点值，但现有方法存在两个关键限制：1）分布估计缺乏局部基础；2）依赖共享表征导致输入与分位数输出之间存在间接瓶颈。

Method: 提出Quantile Token Regression方法：1）在输入序列中插入专用分位数标记，通过自注意力机制为每个分位数建立直接输入-输出路径；2）结合检索机制，引入语义相似邻居实例及其经验分布，为预测提供局部证据基础。

Result: 在Inside Airbnb和StackSample基准数据集上，使用1.7B到14B参数的LLM进行实验，分位数标记结合邻居的方法始终优于基线（MAPE降低约4个百分点，预测区间缩小2倍），在较小和更具挑战性的数据集上提升尤为显著。

Conclusion: Quantile Token Regression通过专用分位数标记和检索增强机制，有效解决了分布估计的局部基础问题和输入-输出瓶颈问题，显著提升了LLM在分位数回归任务中的分布预测性能。
...
TL;DR: 该研究应用符号时间序列分析和马尔可夫建模分析《叶甫盖尼·奥涅金》原文及意大利语译本的音系结构，使用元音/辅音二元编码构建简约概率模型，揭示俄语原文与意大利语译本在记忆深度和结构模式上的显著差异。


<details>
  <summary>Details</summary>
Motivation: 重新审视马尔可夫将符号分析应用于文学文本的原始提议，结合计算统计学和数据科学的现代工具，探索简约马尔可夫模型如何支持复杂诗歌材料的探索性分析，为比较诗学提供通用框架。

Method: 采用受马尔可夫原始方案启发的元音/辅音二元编码，构建简约概率模型；使用四状态马尔可夫链捕捉局部依赖关系和大规模序列模式；引入音系探针（短符号模式）将表层结构与叙事相关线索联系起来。

Result: 分析显示俄语原文与意大利语译本存在显著不对称性：原文记忆深度逐渐下降，而译本保持更均匀的轮廓；音系探针揭示了字形形式与主题发展之间的微妙联系，特别是在俄语原文中；简约马尔可夫模型能准确描述并生成关键序列特征。

Conclusion: 即使简约的马尔可夫模型也能支持复杂诗歌材料的探索性分析；当辅以粗粒度的语言标注时，此类模型为比较诗学提供了通用框架，证明基于语言形式的简单表征仍能捕捉风格化的结构模式。
...
TL;DR: GaoYao是一个全面的多语言多文化评估基准，包含182.3k样本、26种语言和51个国家/地区，通过三层文化框架和九个子层认知维度，解决了现有基准在文化深度、语言覆盖和诊断分析方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型的多语言多文化评估基准存在三个关键局限：1) 评估维度碎片化，忽视深层文化细微差别；2) 主观任务语言覆盖不足，依赖低质量机器翻译；3) 分析浅显，缺乏超越简单排名的诊断深度。

Method: 1) 提出统一框架，将评估任务分为三层文化结构（通用多语言、跨文化、单文化）和九个认知子层；2) 通过专家严格本地化主观基准到19种语言，为34种文化合成跨文化测试集；3) 对20多个旗舰和紧凑型LLM进行深入诊断分析。

Result: GaoYao基准覆盖范围比先前工作提升高达111%，分析揭示了显著的地理性能差异和任务间的明显差距，为未来工作提供了可靠的地图。

Conclusion: GaoYao基准通过全面的多语言多文化评估框架，解决了现有基准的局限性，提供了更深入的文化理解和诊断分析，有助于推动LLM在全球范围内的实用性和公平性发展。
...
Method: 基于OpenAlex构建作者知识图谱，结合粗粒度概念和KeyBERT+ChatGPT提取的细粒度关键词，加权考虑描述符来源、作者位置和时间新近性，应用于189,581篇电池相关文献

Result: 构建了支持作者相似度计算、社区检测和浏览器界面探索性搜索的知识图谱，以RDF格式序列化并与Wikidata链接，实现跨机构规模分析

Conclusion: 该方法在跨机构规模上操作，基于领域语义而非仅引用或合著结构，可扩展至电池领域之外，提高了研究合作发现效率
...
TL;DR: 本文提出了混合策略蒸馏（HPD），通过整合前向和反向KL散度的优势，结合离策略数据和轻量级近似在策略采样，改进了大语言模型的知识蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法在散度方向、优化策略和数据机制方面的选择相互交织，缺乏统一框架。需要一种能够平衡模式覆盖和模式寻求，同时提高优化稳定性和计算效率的蒸馏方法。

Method: 首先将知识蒸馏重新表述为标记级别的加权对数似然目标，然后提出混合策略蒸馏（HPD），整合前向KL（模式覆盖）和反向KL（模式寻求）的互补优势，并组合离策略数据与轻量级近似在策略采样。

Result: 在长生成数学推理以及短生成对话和代码任务上验证了HPD，展示了改进的优化稳定性、计算效率和最终性能，适用于不同模型系列和规模。

Conclusion: 混合策略蒸馏提供了一种统一的知识蒸馏框架，通过整合不同散度方向和数据机制的优势，显著提升了知识蒸馏的效果和效率。
...
TL;DR: RADS使用强化学习在极低资源和类别不平衡条件下选择最有信息量的样本，提升模型可迁移性


<details>
  <summary>Details</summary>
Motivation: 在迁移学习中，少样本微调的成功高度依赖于训练样本的质量。传统主动学习方法（如不确定性采样和多样性采样）在极低资源和类别不平衡条件下，往往偏向异常值而非真正有信息量的样本，导致性能下降。

Method: 提出RADS（强化自适应领域采样），一种使用强化学习来识别最有信息量样本的鲁棒样本选择策略。

Result: 在多个真实世界临床数据集上的实验评估表明，RADS样本选择策略在极端类别不平衡条件下相比传统方法，能增强模型可迁移性并保持鲁棒性能。

Conclusion: RADS通过强化学习驱动的自适应样本选择，有效解决了极低资源和类别不平衡条件下的样本选择问题，为迁移学习提供了更鲁棒的解决方案。
...
TL;DR: Graph2Counsel框架通过客户心理图生成高质量心理咨询对话数据，解决了真实数据稀缺问题，在专家评估中优于现有数据集，并提升下游模型性能。


<details>
  <summary>Details</summary>
Motivation: 心理健康支持需求增长促使使用大语言模型进行咨询，但真实咨询数据因隐私限制稀缺。现有合成方法忽视客户认知、情感和行为状态的结构依赖，导致心理不一致和低质量数据。

Method: 提出Graph2Counsel框架，基于客户心理图编码客户思维、情感和行为关系，采用结构化提示管道，结合思维链和多智能体反馈策略，生成760个咨询会话。

Result: 专家评估显示数据集在特异性、咨询师能力、真实性、对话流畅性和安全性方面优于先前数据集，具有高标注者一致性。微调开源模型在CounselingBench和CounselBench上表现提升。

Conclusion: Graph2Counsel通过心理图结构生成高质量心理咨询数据，解决了数据稀缺问题，展示了实际应用价值，并公开了代码和数据。
...
TL;DR: 该研究提出了DialToM基准，用于评估大语言模型的理论心智能力，发现模型在识别心理状态方面表现良好，但在利用这些状态预测社会轨迹方面存在显著缺陷。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型显示出理论心智能力，但尚不清楚这是源于稳健的推理能力还是虚假相关性。需要更全面的评估方法来区分这两种可能性。

Method: 研究者构建了DialToM基准，这是一个基于自然人类对话的多选择框架，由人工验证。评估包括心理状态预测（字面理论心智）和这些状态的功能效用（功能理论心智），通过前瞻性诊断预测来探究模型是否能仅从心理状态概况中识别状态一致的对话轨迹。

Result: 结果显示显著的推理不对称性：大语言模型在识别心理状态方面表现出色，但大多数模型（除了Gemini 3 Pro）无法利用这种理解来预测社会轨迹。此外，人类与LLM生成的推理之间只有微弱的语义相似性。

Conclusion: 大语言模型的理论心智能力存在局限性，特别是在将心理状态理解转化为功能性社会预测方面。DialToM基准为未来研究提供了可复现的评估工具。
...
TL;DR: SpanDec：一种高效的基于跨度的命名实体识别框架，通过轻量级解码器和跨度过滤机制，在保持竞争力的准确率的同时显著提升推理效率


<details>
  <summary>Details</summary>
Motivation: 工业信息抽取管道中的命名实体识别系统需要满足严格的延迟和吞吐量约束，而当前基于跨度的NER方法虽然准确率高，但枚举大量候选跨度并使用标记增强输入处理每个候选，导致推理成本高，限制了大规模部署的可扩展性

Method: 提出SpanDec框架，主要包含两个创新：1）在最终transformer阶段计算跨度表示交互，避免早期层的冗余计算，使用专门处理跨度表示的轻量级解码器；2）引入跨度过滤机制，在枚举过程中修剪不太可能的候选跨度，避免昂贵的处理

Result: 在多个基准测试中，SpanDec能够匹配具有竞争力的基于跨度基线方法的准确率，同时显著提高吞吐量并降低计算成本，实现了更好的准确率-效率权衡

Conclusion: SpanDec为大规模服务部署和端侧应用提供了一种更好的准确率-效率权衡方案，解决了基于跨度NER方法的推理效率瓶颈问题
...
Method: 提出了一个计算框架，通过源域和语义框架推导显著的话语隐喻。应用于气候变化新闻和不同政治意识形态的移民话语分析。

Result: 在气候变化新闻中发现了已知源域和细微的框架层面关联；在移民话语分析中，发现保守派倾向于强调不可控性的框架，而自由派选择中性或更"受害者化"的语义框架。

Conclusion: 该研究连接了概念隐喻理论和语言学，提供了首个用于发现话语隐喻和分析隐喻框架差异的NLP方法，代码和数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 传统LLMs将知识编码在参数权重中，更新成本高；RAG虽然通过检索文本扩展上下文，但知识仅作为文本token参与注意力计算，影响间接且不稳定，特别是在长上下文和多跳推理场景中
...
Conclusion: 将知识集成从上下文级增强转向内存级交互，通过结构化非参数记忆单元直接参与注意力计算，为LLMs知识更新和扩展提供了更有效稳定的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前知识图谱构建过程与下游应用（如检索增强生成）之间存在脱节，导致构建的图谱结构并非最优，限制了问答系统的性能提升。

Method: 提出AutoGraph-R1框架，将图谱生成视为策略学习问题，使用强化学习训练LLM构造器，设计了两种任务感知奖励函数：一种针对图谱作为知识载体，另一种针对图谱作为知识索引。

Result: 在多个问答基准测试中，AutoGraph-R1使图谱RAG方法相比任务无关的基线图谱获得了显著的性能提升。

Conclusion: 该工作证明了构建与应用之间的闭环是可行的，将范式从构建内在"好"的图谱转变为构建可证明"有用"的图谱。
...
TL;DR: 本文通过文献综述和利益相关者调查，分析了口吃语音研究的现状与需求差距，提出了研究分类体系，并为满足口吃社区实际需求制定了具体指导方针。
...
Method: 采用双重方法：1）对口吃语音相关论文进行范围综述；2）对70名利益相关者（包括口吃成人和言语语言病理学家）进行调查。通过分析这两个视角来识别研究差距。

Result: 提出了口吃语音研究的分类体系，识别了当前研究方向与利益相关者需求之间的分歧，并制定了具体的指导方针和研究方向来满足口吃社区的实际需求。

Conclusion: 需要将口吃语音研究重点从单纯的技术改进转向更全面地满足口吃社区的实际需求，通过跨学科对话和用户中心的方法来开发更有效的语音技术解决方案。
...
TL;DR: 该研究提出了一种基于多智能体迭代规划搜索的策略，用于生成多样化和新颖的科研想法，在NLP领域实验中表现优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 科学文献的快速增长增加了知识筛选成本，现有LLM方法生成的科研想法往往重复且缺乏深度，需要改进想法的多样性和新颖性。

Method: 提出基于组合创新理论的多智能体迭代规划搜索策略，结合迭代知识搜索和LLM多智能体系统，通过反复交互生成、评估和优化科研想法。

Result: 在自然语言处理领域的实验中，该方法在多样性和新颖性方面均优于最先进的基线方法，生成的想法质量介于顶级机器学习会议接受和拒绝论文之间。

Conclusion: 该框架是支持高质量科研想法生成的有前景的方法，源代码和数据集已在GitHub公开，演示版可在Hugging Face访问。
...
TL;DR: 该研究探讨了在多语言环境下利用高资源语言的质量标记来辅助低资源语言数据质量过滤的方法，通过跨语言迁移、第三四分位数采样和保留率调优等策略，证明了多语言池化在质量分类中的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模扩大，数据质量过滤变得至关重要。然而，对于许多语言来说，缺乏足够的高质量本地数据来训练有效的质量分类器。本研究旨在探索质量标记在嵌入空间中是否具有跨语言一致性，从而让高资源语言能够帮助过滤低资源语言的数据。

Method: 研究评估了多种过滤策略：跨语言迁移、第三四分位数采样(Q3)和保留率调优。使用一个在1030亿token上训练的10亿参数模型，比较了大规模多语言池化与单语言基线在不同语言上的表现。

Result: 大规模多语言池化在排名稳定性和聚合准确性方面通常优于单语言基线。对于高资源语言如法语，聚合归一化准确性提高了1.2%；对于低资源语言，匹配或超过了单语言基线。但研究发现，仅靠规模并不能保证稳定性，对于高资源语言，需要通过Q3采样或调优保留率来充分利用多语言信号。

Conclusion: 质量标记在嵌入空间中确实表现出跨语言一致性，使得高资源语言能够有效辅助低资源语言的数据质量过滤。多语言池化策略在提升质量分类性能方面具有显著优势，但需要适当的边界调整策略来最大化其效益。
...
TL;DR: 该论文提出一个多层框架，通过干预逻辑连接词选择来改善LLMs在多步逻辑推理中的脆弱性，在保持效率的同时提升推理准确性。
...
Method: 提出一个多层框架：1) 基于梯度的逻辑引导，引导LLMs内部表示朝向有效推理子空间；2) 局部化分支，通过有针对性的前瞻搜索解决歧义；3) 目标转换偏好优化，一种外科手术式的强化学习目标，选择性地优化逻辑枢纽处的单标记偏好。

Result: 通过仅关注逻辑关键转换，该框架在准确性和效率之间实现了有利的权衡，相比光束搜索和自一致性等全局推理时间缩放方法表现更好。

Conclusion: 干预逻辑连接词选择可以有效引导LLMs走向更正确的逻辑方向，从而改善整体推理链。集中干预逻辑关键转换点的方法在保持效率的同时显著提升了推理准确性。
...
TL;DR: ProactAgent：一个在线终身学习框架，通过主动检索结构化经验库来提升智能体性能，在多个任务上取得显著效果


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将经验检索视为被动操作，只在任务初始化或步骤完成后触发，导致智能体在交互过程中难以识别知识缺口并主动检索最有用的经验来支持当前决策

Method: 提出ProactAgent框架：1) Experience-Enhanced Online Evolution (ExpOnEvo)支持通过策略更新和记忆精化的持续改进；2) 将历史交互组织成类型化存储库（事实记忆、情景记忆、行为技能）；3) Proactive Reinforcement Learning-based Retrieval (ProactRL)将检索建模为显式策略动作，通过配对分支过程奖励学习何时检索以及检索什么内容

Result: 在SciWorld、AlfWorld和StuLife上的实验表明，ProactAgent持续提升终身智能体性能：SciWorld成功率73.50%，AlfWorld成功率71.28%，同时显著减少检索开销，在StuLife上达到与专有模型竞争的性能

Conclusion: ProactAgent通过主动检索结构化经验库有效解决了现有在线终身学习方法中检索被动性的问题，在多个任务上实现了更好的性能和更高的效率
...
TL;DR: 研究发现，大语言模型在行为经济学游戏中的合作表现能有效预测其在科学协作任务中的表现，合作倾向是LLM可测量的独特属性。


<details>
  <summary>Details</summary>
Motivation: 随着基于大语言模型的多智能体系统在科学推理和问题解决中的广泛应用，需要了解这些模型在共享约束下的协作能力。虽然行为经济学提供了丰富的游戏来评估合作机制，但尚不清楚模型在这些游戏中的表现是否能预测其在现实协作任务中的性能。

Method: 对35个开源权重的大语言模型进行基准测试，评估它们在六个行为经济学游戏中的表现，然后将这些合作特征与AI-for-Science任务中的表现进行关联分析。在科学任务中，LLM智能体团队需要在共享预算约束下协作分析数据、构建模型和生成科学报告。

Result: 研究发现，从游戏中得出的合作特征能够稳健地预测下游科学任务的表现。那些在游戏中有效协调并投资于乘法团队生产（而非贪婪策略）的模型，在准确性、质量和完成度三个指标上都产生了更好的科学报告。这些关联在控制多个因素后仍然显著。

Conclusion: 合作倾向是大语言模型可测量的独特属性，不能简化为一般能力。行为经济学游戏框架为在昂贵的多智能体部署前筛选合作适应性提供了快速且廉价的诊断工具。
...
Result: 在单语和跨语言检索基准测试中，ORPHEAS超越了现有最先进的多语言嵌入模型，证明了对形态复杂语言进行领域专门化微调不会损害跨语言检索能力。
...
TL;DR: 该研究系统评估了六个大语言模型在交叉人口属性下的公平性，发现在模糊语境中模型表现良好但公平性指标信息有限，而在明确语境中模型准确性受刻板印象对齐影响，尤其是在种族-性别交叉维度上表现出更强的刻板印象偏向。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地部署在具有社会敏感性的场景中，引发了关于公平性和偏见的担忧，特别是在交叉人口属性方面。研究者旨在系统评估LLMs在交叉人口属性下的公平性表现。

Method: 使用两个基准数据集中的模糊和明确语境，系统评估六个大语言模型。通过偏见分数、子群体公平性指标、准确性和一致性等多维度分析，考察模型在不同语境、正负问题极性以及多次运行中的表现。

Result: 现代LLMs在模糊语境中表现良好，但这限制了公平性指标的信息量；在明确语境中，模型准确性受刻板印象对齐影响，当正确答案强化刻板印象时模型更准确，尤其是在种族-性别交叉维度上表现出更强的刻板印象偏向。尽管在某些情况下观察到差异较小，但结果分布在交叉群体中仍不均匀，且响应一致性存在变化。

Conclusion: 模型表面上的能力部分与刻板印象一致的线索相关，没有评估的LLM能够在交叉设置中实现持续可靠或公平的行为。研究强调需要超越准确性的评估，重视在交叉群体、语境和重复运行中结合偏见、子群体公平性和一致性指标的重要性。
...
TL;DR: 本文研究了在自由文本法律问答的LLM-as-a-Judge评估中，提示设计和法官选择的作用，发现自动提示优化优于人工设计，宽松法官反馈比严格反馈效果更好且泛化能力更强。
...
Method: 在LEXam基准上，使用ProTeGi方法对两个法官（Qwen3-32B和DeepSeek-V3）的反馈进行任务提示优化，涵盖四个任务模型，然后测试跨法官的提示迁移能力。

Result: 自动优化始终优于基线，宽松法官反馈比严格反馈产生更高且更一致的增益；宽松反馈优化的提示向严格法官的迁移效果优于反向迁移；分析显示宽松法官提供更宽容的反馈，产生适用性更广的提示，而严格法官则产生限制性反馈，导致法官特定的过拟合。
...
TL;DR: 首个拉丁语-英语双语问答与翻译基准数据集，包含约7,800个问答对，涵盖多种题型，用于评估大语言模型在专业语言文化领域的表现。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏针对拉丁语的问答基准数据集，需要创建专门资源来评估模型在专业语言文化领域的表现能力。

Method: 从1800年代至今的拉丁语教学资源（考试、知识竞赛、教材）中自动提取问题，经过清洗和人工审核，构建包含知识型、技能型、多跳推理、翻译等多种题型的数据集。

Result: 创建了包含约7,800个问答对的数据集，评估了LLaMa 3、Qwen QwQ和OpenAI o3-mini三个大模型，发现所有模型在技能导向问题上表现较差，推理模型在韵律分析和文学手法任务上表现稍好但整体改进有限。
...
TL;DR: 研究将人类工作记忆约束整合到Transformer架构中，通过认知启发的注意力机制（固定宽度窗口和时序衰减）改进GPT-2模型，在有限数据下提升语法判断准确性和人类阅读时间对齐度。


<details>
  <summary>Details</summary>
Motivation: 探索将人类认知中的工作记忆约束整合到Transformer架构中，以改进语言模型在数据稀缺情况下的表现，并增强模型与人类语言处理机制的对齐。

Method: 在Transformer架构中实现认知启发的注意力变体，包括基于固定宽度窗口和时序衰减的注意力机制。从头训练修改后的GPT-2模型，使用发展上合理的数据集（1000万和1亿词），在语法判断任务（BLiMP）和人类阅读时间数据对齐度上进行评估。

Result: 认知启发的约束（特别是固定宽度注意力）能显著提高语法准确性，尤其是在训练数据稀缺时。这些受约束模型也倾向于与人类处理指标有更强的对齐。约束模型在数据有限设置下表现更好。

Conclusion: 认知启发的约束可以作为有益的归纳偏置，引导模型学习更稳健的语言表征，特别是在数据受限的环境中。这种约束有助于缩小计算模型与人类语言处理机制之间的差距。
...
TL;DR: LLMs在医疗沟通中与临床标准存在差距，通过多维评估发现基线模型在情感极性和语言复杂度方面与医生存在差异，共情导向提示可改善但语义保真度不足，协作重写能实现最佳对齐效果。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在医疗领域部署增加，需要量化其与临床沟通标准的对齐程度，评估模型在医学解释和医患互动中的表现，以确定其在医疗沟通中的有效应用方式。

Method: 采用多维评估方法，对比通用和领域专用LLMs在结构化医学解释和真实医患互动中的表现，分析语义保真度、可读性和情感共鸣，测试共情导向提示和协作重写策略的效果。

Result: 基线模型的情感极性比医生更极端（非常负面：43.14-45.10% vs. 37.25%），大型架构语言复杂度更高（FKGL达16.91-17.60 vs. 11.47-12.50）。共情提示降低负面情感和复杂度但不改善语义保真度。协作重写实现最高语义相似度（均值达0.93），同时改善可读性和情感平衡。患者偏好重写版本，但模型在认知标准上未超越医生。

Conclusion: LLMs最适合作为协作沟通增强工具而非临床专业知识的替代品，需要结合人类专业知识才能实现最佳医疗沟通效果。
...
TL;DR: 语言模型在自然文本训练中学习使用周期性特征表示数字，主要周期为2、5、10。研究发现这些特征存在两层结构：所有模型都学习到傅里叶域中具有周期-T尖峰的特征，但只有部分模型学习到可用于线性分类数字模-T的几何可分离特征。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解语言模型如何表示数字，特别是为什么不同模型在数字表示上表现出相似性（收敛进化现象），以及哪些因素影响模型学习到几何可分离的数字特征。

Method: 通过理论证明和实证研究相结合的方法：1）证明傅里叶域稀疏性是模-T几何可分离的必要但不充分条件；2）实证研究不同训练条件（数据、架构、优化器、分词器）对几何可分离特征学习的影响；3）识别模型获得几何可分离特征的两种途径。

Result: 发现所有模型都学习到傅里叶域中具有周期尖峰的特征，但只有部分模型学习到几何可分离特征。数据、架构、优化器和分词器都影响几何可分离特征的学习。模型可以通过两种途径获得这些特征：从一般语言数据中的互补共现信号（文本-数字共现和跨数字交互），或从多token（非单token）加法问题。

Conclusion: 研究揭示了特征学习中的收敛进化现象：多种不同的模型从不同的训练信号中学习到相似的特征。傅里叶域稀疏性是几何可分离的必要条件，但不是充分条件。模型获得几何可分离特征的途径取决于具体的训练条件和信号类型。
...
TL;DR: 该研究提出并行监督微调(Parallel-SFT)方法，通过在多编程语言上训练代码生成模型，提升强化学习在跨语言零样本迁移中的效果。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型在常见编程语言上表现出色，但在低资源编程语言上性能受限。虽然大多数编程技能在不同语言间是通用的，但现有模型在跨语言迁移方面效果不佳，特别是强化学习训练在源语言上会损害其他目标语言的性能。

Method: 提出Parallel-SFT策略，将"并行程序"（功能相同但用不同编程语言实现的代码）纳入监督微调的数据混合中。随后在Parallel-SFT模型上进行强化学习训练，以提升跨语言零样本迁移能力。

Result: Parallel-SFT显著改善了跨语言迁移效果：在Parallel-SFT模型上进行强化学习后，模型在未见过的编程语言上表现出更好的泛化能力。模型内部表征分析显示，Parallel-SFT导致更功能中心化的潜在空间，跨语言的等效程序更紧密地聚集在一起。

Conclusion: 有效的跨语言强化学习迁移需要可泛化的监督微调初始化。Parallel-SFT通过在多编程语言上训练，创建了更功能中心化的表征空间，从而提升了代码生成模型在跨语言零样本迁移中的性能。
...
TL;DR: SpeechParaling-Bench是一个全面的副语言感知语音生成基准，将特征覆盖从不到50个扩展到100多个细粒度特征，包含1000多个英中平行语音查询，并通过基于LALM的成对比较评估缓解主观性。


<details>
  <summary>Details</summary>
Motivation: 副语言线索对人机交互至关重要，但现有大型音频语言模型（LALMs）的评估存在特征覆盖不足和评估主观性强的局限性。

Method: 1. 构建SpeechParaling-Bench基准，包含100多个细粒度特征和1000+英中平行语音查询，组织为三个渐进任务：细粒度控制、话语内变化和上下文感知适应。2. 开发基于LALM的成对比较评估流程，将评估框架从绝对评分转为相对偏好，减少主观性。

Result: 实验显示当前LALMs存在显著局限性：即使是领先的专有模型也难以实现全面的静态控制和动态调制副语言特征；在情境对话中，43.3%的错误源于未能正确解释副语言线索。

Conclusion: 当前模型在副语言建模方面仍有不足，需要更鲁棒的副语言建模方法以实现更符合人类期望的语音助手。
...
### [132] [Testing replication for an agent-based model of market fragmentation and latency arbitrage](https://arxiv.org/abs/2604.20067)
*Ethan Ratliff-Crain,Colin M. Van Oort,Matthew T. K. Koehler,Brian F. Tivnan*

Main category: q-fin.TR

TL;DR: 该研究试图独立复制Wah和Wellman 2016年的市场碎片化延迟套利模型，发现由于原始论文缺乏实现细节和定量报告，难以实现忠实复制。通过增加模拟运行次数和使用bootstrap置信区间，研究者能够进行更严格的定量对齐测试，但发现模型复杂度增加时对齐难度也增加。虽然实现了大多数指标的关系等价性，但在非零延迟设置下拒绝了定量对齐。研究还发现原始论文关于市场碎片化和延迟套利效应的定性结论对"贪婪策略"的具体实现很敏感。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在加强多场所市场建模的基础，通过独立复制2016年Wah和Wellman的市场碎片化延迟套利模型来验证其可靠性和可复制性。由于原始论文缺乏详细的实现信息和定量报告，研究者希望通过系统性的复制工作来评估该模型的稳健性，并为未来研究提供更可靠的建模基础。

Method: 研究者首先尝试忠实复制原始模型，但由于缺乏实现细节，转而采用多种方法：1) 增加模拟运行次数以创建bootstrap置信区间；2) 利用作者后来发布的代码库恢复实现细节；3) 结合论文信息和代码实现进行关系等价性测试；4) 对"贪婪策略"的不同解释进行敏感性分析；5) 为模型实现提供ODD（概述、设计概念、细节）协议以促进未来复制。

Result: 研究发现：1) 原始论文缺乏足够的实现细节和定量报告，阻碍了忠实复制；2) 增加模拟运行次数能够支持更严格的定量对齐测试；3) 模型复杂度增加时对齐难度也增加；4) 虽然实现了大多数指标的关系等价性，但在非零延迟设置下拒绝了定量对齐；5) 原始论文的定性结论对"贪婪策略"的具体实现很敏感，采用不同解释时发现市场碎片化在所有实验中减少执行时间，在大多数实验中增加交易者福利。

Conclusion: 该研究强调了模型可复制性和透明实现细节的重要性。虽然原始模型的基本关系模式可以复制，但定量结果难以完全对齐，特别是在复杂场景下。研究还表明原始论文的关键定性结论对特定建模选择很敏感。通过提供ODD协议，该研究为未来复制、批判和扩展工作奠定了基础，促进了多场所市场建模领域的科学严谨性。
...
### [133] [Maximum Likelihood Reconstruction for Multi-Look Digital Holography with Markov-Modeled Speckle Correlation](https://arxiv.org/abs/2604.20154)
*Xi Chen,Arian Maleki,Shirin Jalali*

Main category: eess.IV

TL;DR: 该论文提出了一种处理多视相干成像中相关散斑噪声的新方法，通过马尔可夫过程建模视间相关性，结合深度图像先验和投影梯度下降进行重建，在强相关性下仍能接近理想独立视场景的性能。


<details>
  <summary>Details</summary>
Motivation: 多视采集是减少相干成像系统（如数字全息）中散斑噪声的常用策略，但硬件限制导致测量多样性不足，产生视间相关性，这会降低传统方法的性能。现有方法通常假设各视之间的散斑统计独立，但在实际硬件约束下这一假设不成立。

Method: 使用一阶马尔可夫过程建模视间相关性，推导相应的似然函数，形成约束最大似然估计问题。开发高效的投影梯度下降框架，结合基于梯度的更新和通过深度图像先验的隐式正则化，利用蒙特卡洛近似和矩阵自由算子实现可扩展计算。

Result: 仿真结果表明，该方法在强视间相关性下仍保持鲁棒性，性能接近理想的独立视场景，并且始终优于忽略这种相关性的方法。代码已开源。

Conclusion: 该研究强调了显式建模视间相关性的重要性，并为实际采集条件下的多视全息重建提供了一个实用框架，能够有效处理硬件限制导致的测量相关性。
...
### [134] [CKM Beyond Channel Gain: Spatial Correlation Map Construction with Deep Learning](https://arxiv.org/abs/2604.20684)
*Z. Chen,S. Fu,Y. Zeng,X. Xu,Z. Wei*

Main category: eess.IV

TL;DR: 该论文提出了一种基于深度学习的信道空间相关图（SCM）构建方法，通过分解高维SCM为低维路径增益图和路径角度图，并利用E-SRResNet模型从稀疏样本中高质量重建SCM。


<details>
  <summary>Details</summary>
Motivation: 现有CKM构建工作主要关注信道增益图（CGM），但信道空间相关图（SCM）对多天线系统更为重要。SCM构建面临高维结构的重大挑战，需要新的解决方案。

Method: 将高维SCM分解为低维的路径增益图（PGM）和路径角度图（PAM），提出E-SRResNet深度学习模型，整合多头注意力机制和多尺度特征融合，并利用LoS图、建筑物二值图和基站图等先验信息。

Result: 在CKMImageNet数据集上的仿真表明，E-SRResNet相比基线方法有显著性能提升，构建的SCM与真实值之间的余弦相似度在大部分区域超过0.8。

Conclusion: 提出的E-SRResNet方法能有效解决SCM构建的高维挑战，为环境感知无线通信和感知提供了高质量的信道空间相关图构建方案。
...
### [135] [Feedback-Driven Rate Control for Learned Video Compression](https://arxiv.org/abs/2604.20104)
*Zhiheng Xu,Xuerui Ma,Chunhua Peng,Hao Zhang*

Main category: cs.MM

TL;DR: 本文提出了一种反馈驱动的率控制框架，用于端到端学习的视频压缩，通过PI/PID闭环控制器实现目标码率稳定跟踪，并结合GRU调整控制器优化帧级比特分配。


<details>
  <summary>Details</summary>
Motivation: 端到端学习的视频压缩在率失真性能上表现优异，但码率控制方面研究不足，特别是在目标码率驱动和预算约束的场景下。现有方法主要依赖显式的R-D-lambda建模或前馈预测，在视频内容动态变化时缺乏稳定的在线调整能力。

Method: 1. 在DCVC风格框架上构建单模型多码率编码接口，通过率失真参数lambda实现连续码率控制；2. 使用对数域PI/PID闭环控制器根据目标码率与熵估计码率之间的误差在线更新lambda，实现稳定目标码率跟踪；3. 引入基于双分支GRU的调整控制器，利用预算状态特征和因果编码统计来优化帧级比特分配。
...
Conclusion: 该方法为学习型视频压缩提供了实用的解决方案，既能实现可控的码率，又能提高率失真性能，解决了现有方法在动态视频内容下缺乏稳定在线调整的问题。
...
### [136] [Seeing Further and Wider: Joint Spatio-Temporal Enlargement for Micro-Video Popularity Prediction](https://arxiv.org/abs/2604.20311)
...
<details>
  <summary>Details</summary>
Motivation: 现有微视频流行度预测方法在时间维度上依赖稀疏短距离采样限制内容感知，在空间维度上依赖容量有限、效率低的扁平检索记忆库，阻碍了可扩展的知识利用

Method: 提出统一框架实现联合时空扩展：1) 时间扩展：通过帧评分模块从稀疏采样和密集感知两个互补路径提取视频帧的高亮线索，自适应融合以实现长序列内容理解；2) 空间扩展：构建拓扑感知记忆库，基于拓扑关系分层聚类历史相关内容，通过更新对应聚类编码器特征而非直接扩展内存容量，实现无限制历史关联

Result: 在三个广泛使用的MVPP基准测试上，该方法在主流指标上持续优于11个强基线，在预测准确性和排名一致性方面均实现稳健提升

Conclusion: 提出的统一框架通过时空扩展解决了现有方法的局限性，实现了对极长视频序列的精确感知和可扩展记忆库，显著提升了微视频流行度预测性能
...
### [137] [Realistic Virtual Flood Experience System Using 360° Videos and 3D City Models Constructed from Building Footprints](https://arxiv.org/abs/2604.20746)
*Tatsuro Banno,Koki Kawada,Mizuki Takenawa,Masatoshi Denda,Kiyoharu Aizawa*

Main category: cs.MM

TL;DR: 提出了一种新的虚拟洪水体验框架，将360°视频与从2D建筑足迹自动构建的3D模型相结合，实现真实场景中的3D洪水可视化，无需依赖预建城市模型。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟洪水体验系统通常使用非真实地点的虚拟城市，且缺乏足够的光照真实感，限制了用户将场景与自身环境联系起来的能力。虽然360°视频能简单可扩展地复制真实场景，但有效的3D洪水可视化通常需要目标区域的3D建筑几何数据，这在许多地区不易获得。

Method: 提出新框架，将360°视频与从广泛可用的2D建筑足迹自动构建的3D模型集成。通过将足迹挤出到合理高度，并将构建的模型与360°视频进行空间对齐，实现在光照真实环境中进行3D洪水可视化，无需依赖CityGML等预建城市模型。

Result: 在日本北海道芽室町（易受河流洪水影响的地区）演示了该框架。用户研究显示，该系统增强了用户设想特定地点洪水疏散情况的能力，证明了其作为灾害风险沟通和教育有效工具的潜力。

Conclusion: 该研究提出的框架通过结合360°视频和自动生成的3D建筑模型，解决了现有虚拟洪水体验系统的局限性，为灾害风险沟通和教育提供了更有效、更易实现的工具。
...
### [138] [The Tool-Overuse Illusion: Why Does LLM Prefer External Tools over Internal Knowledge?](https://arxiv.org/abs/2604.19749)
...
<details>
  <summary>Details</summary>
Motivation: 为LLM配备外部工具虽然能解决内部推理限制，但引入了工具过度使用这一关键但未被充分探索的现象。作者旨在揭示这一现象的普遍性并阐明其底层机制。

Method: 1. 通过分析不同内部知识可用性区域下的工具使用行为，识别知识认知幻觉；提出基于直接偏好优化的知识感知认知边界对齐策略。2. 通过可视化工具增强训练过程，建立奖励结构与工具使用行为之间的因果关系；提出平衡奖励信号而非仅依赖结果导向奖励的方法。

Result: 1. 知识感知认知边界对齐策略将工具使用减少了82.8%，同时提高了准确性。2. 平衡奖励信号的方法在7B和32B模型上分别减少了66.7%和60.7%的不必要工具调用，且未牺牲准确性。

Conclusion: 工具过度使用是LLM工具增强中的普遍现象，主要源于知识认知幻觉和结果导向奖励问题。通过知识边界对齐和平衡奖励信号可以有效缓解这一问题，为理解工具过度使用提供了理论和实践指导。
...
TL;DR: ZeroFolio：一种无需特征工程的算法选择方法，使用预训练文本嵌入替代手工特征，在11个ASlib场景中10个优于基于手工特征的随机森林
...
Method: 三步骤方法：1) 将原始实例文件读取为纯文本；2) 使用预训练嵌入模型生成文本嵌入；3) 通过加权k近邻选择算法。关键设计包括逆距离加权、行随机化和曼哈顿距离。

Result: 在11个ASlib场景（涵盖SAT、MaxSAT、QBF、ASP、CSP、MIP和图问题7个领域）中，ZeroFolio在10个场景中优于基于手工特征的随机森林，在11个场景中通过双种子投票全部胜出。当两种选择器都表现良好时，通过软投票结合嵌入和手工特征能获得进一步改进。

Conclusion: 预训练文本嵌入可以作为通用的实例表示，无需领域知识即可实现有效的算法选择。该方法简单、通用，在多个问题领域都表现出色，为算法选择提供了新的特征自由途径。
...
### [140] [Explainable AML Triage with LLMs: Evidence Retrieval and Counterfactual Checks](https://arxiv.org/abs/2604.19755)
*Dorothy Torres,Wei Cheng,Ke Hu*

Main category: cs.AI

TL;DR: 提出一个可解释的反洗钱交易监控框架，通过证据约束的决策过程、结构化输出和反事实验证，提高审计性和减少幻觉错误，在合规要求下提供决策支持。


<details>
  <summary>Details</summary>
Motivation: 反洗钱交易监控产生大量警报需要快速处理，但传统方法存在幻觉、溯源弱、解释不忠实等问题，在受监管的工作流程中风险较高。

Method: 结合三个核心组件：(1) 从政策指南、客户背景、警报触发器和交易子图中检索增强的证据捆绑；(2) 结构化LLM输出合约，要求明确引用并分离支持、矛盾或缺失的证据；(3) 反事实检查，验证最小合理扰动是否导致决策和理由的连贯变化。

Result: 证据基础显著提高审计性，减少数值和政策幻觉错误；反事实验证进一步提升决策相关可解释性和鲁棒性，获得最佳整体分类性能（PR-AUC 0.75；升级F1 0.62）和强大的溯源与忠实度指标（引用有效性0.98；证据支持0.88；反事实忠实度0.76）。

Conclusion: 受治理、可验证的LLM系统可以在不牺牲可追溯性和可辩护性合规要求的情况下，为反洗钱分类提供实用的决策支持。
...
### [141] [ThermoQA: A Three-Tier Benchmark for Evaluating Thermodynamic Reasoning in Large Language Models](https://arxiv.org/abs/2604.19758)
*Kemal Düzkar*

Main category: cs.AI

TL;DR: ThermoQA是一个包含293个开放式工程热力学问题的基准测试，分为三个层次：属性查找、组件分析和完整循环分析。使用CoolProp 7.2.0计算真实值，评估了六个前沿大语言模型，发现Claude Opus 4.6表现最佳（94.1%）。


<details>
  <summary>Details</summary>
Motivation: 当前需要评估大语言模型在工程热力学领域的推理能力，特别是验证属性记忆是否等同于热力学推理能力。现有的基准测试可能无法充分评估模型在复杂工程问题上的表现。

Method: 创建了包含293个开放式热力学问题的基准测试，分为三个层次：属性查找（110题）、组件分析（101题）和完整循环分析（82题）。使用CoolProp 7.2.0程序化计算真实值，涵盖水、R-134a和变比热空气。对六个前沿LLM进行三次独立运行评估。

Result: Claude Opus 4.6以94.1%的准确率领先，其次是GPT-5.4（93.1%）和Gemini 3.1 Pro（92.5%）。跨层次性能下降从2.8个百分点（Opus）到32.5个百分点（MiniMax），表明属性记忆不等于热力学推理。超临界水、R-134a制冷剂和联合循环燃气轮机分析成为自然区分器，性能差异达40-60个百分点。

Conclusion: ThermoQA基准测试有效评估了LLM在热力学推理方面的能力，揭示了模型在复杂工程问题上的局限性。数据集和代码已开源，为未来研究提供了重要工具。
...
### [142] [Automated Detection of Dosing Errors in Clinical Trial Narratives: A Multi-Modal Feature Engineering Approach with LightGBM](https://arxiv.org/abs/2604.19759)
*Mohammad AL-Smadi*

Main category: cs.AI

TL;DR: 本文提出了一种基于梯度提升和多模态特征工程的自动化系统，用于从非结构化临床试验叙述中检测用药错误，在严重类别不平衡的数据集上取得了0.8725的测试ROC-AUC。


<details>
  <summary>Details</summary>
Motivation: 临床试验需要严格遵守用药方案，但剂量错误仍然是影响患者安全和试验完整性的持续挑战。当前缺乏有效的自动化方法来从非结构化临床叙述中检测这些错误。

Method: 使用梯度提升（LightGBM）模型，结合3,451个多模态特征：传统NLP特征（TF-IDF、字符n-gram）、密集语义嵌入（all-MiniLM-L6v2）、领域特定医学模式以及基于transformer的分数（BiomedBERT、DeBERTa-v3）。特征从9个互补文本字段中提取，覆盖42,112个临床试验叙述。

Result: 在CT-DEB基准数据集（阳性率4.9%）上，通过5折集成平均获得0.8725测试ROC-AUC（交叉验证：0.8833±0.0091）。消融研究表明，移除句子嵌入导致最大性能下降（2.39%）。特征效率分析显示，选择前500-1000个特征可获得最佳性能（0.886-0.887 AUC），优于完整的3,451个特征集（0.879 AUC）。

Conclusion: 特征选择作为一种正则化技术的重要性，以及稀疏词汇特征在严重类别不平衡的专业临床文本分类中与密集表示保持互补性。句子嵌入对性能至关重要，尽管只贡献了37.07%的总特征重要性。
...
### [143] [Inference Headroom Ratio: A Diagnostic and Control Framework for Inference Stability Under Constraint](https://arxiv.org/abs/2604.19760)
*Robert Reinertsen*
...
<details>
  <summary>Details</summary>
Motivation: 当前AI系统在分布偏移和约束条件下运行时，缺乏能够量化系统级推理稳定性边界的方法。标准性能指标、漂移检测和不确定性度量无法有效捕捉系统接近推理崩溃边界的程度，需要一种能够估计推理余量的前瞻性指标。

Method: 提出了推理余量比(IHR)作为系统级诊断指标，它形式化了系统有效推理能力C与环境不确定性及约束负载U+K之间的关系。通过三个受控模拟实验验证IHR的功能：1)作为可量化风险指示器，2)作为环境噪声下推理稳定性边界的敏感指标，3)作为可行控制变量。使用蒙特卡洛模拟(300次运行)评估IHR的调节效果。

Result: 实验结果显示：1) IHR与崩溃概率的关系符合逻辑曲线，估计临界阈值IHR*约为1.19；2) IHR能敏感指示系统接近推理稳定性边界的程度；3) 主动调节IHR可将系统崩溃率从79.4%降低到58.7%，并将IHR方差减少70.4%。

Conclusion: 推理余量比(IHR)可作为标准性能、漂移和不确定性度量的系统级补充指标，能够在AI系统发生明显故障前估计其剩余的推理余量，特别是在分布偏移和约束条件下运行的系统。
...
### [144] [EvoForest: A Novel Machine-Learning Paradigm via Open-Ended Evolution of Computational Graphs](https://arxiv.org/abs/2604.19761)
*Kamer Ali Yuksel,Hassan Sawaf*

Main category: cs.AI

TL;DR: EvoForest是一个混合神经符号系统，通过端到端开放式进化来发现计算结构，在结构化预测问题上超越了传统参数优化范式，在ADIA Lab挑战中取得了94.13% ROC-AUC的优异表现。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习主要围绕参数优化展开，但对于结构化预测问题，真正的瓶颈在于发现应该从数据中计算什么，特别是在目标函数不可微、基于交叉验证评估、需要可解释性或持续适应性的场景下。

Method: EvoForest是一个混合神经符号系统，在共享有向无环图中联合进化可重用计算结构、可调用函数族和可训练低维连续组件。系统包含中间节点存储替代实现、可调用节点编码可重用变换族、输出节点定义候选预测计算，并使用Ridge回归读取器评估计算发现，通过LLM驱动的突变进行进化。

Result: 在2025年ADIA Lab结构性断点挑战中，EvoForest经过600步进化后达到了94.13%的ROC-AUC，超过了公开报道的获胜分数90.14%，且使用相同的评估协议。

Conclusion: EvoForest展示了通过开放式进化发现计算结构在结构化预测问题上的有效性，为超越传统参数优化范式提供了新途径，特别是在需要发现合适变换、统计量、不变性等复杂计算结构的场景中。
...
### [145] [From Actions to Understanding: Conformal Interpretability of Temporal Concepts in LLM Agents](https://arxiv.org/abs/2604.19775)
...
TL;DR: 提出一个基于保形预测的框架，用于解释LLM智能体在交互环境中执行任务时的概念演化，通过统计方法标记内部表征的成功/失败，识别与任务成功相关的潜在方向，并展示如何利用这些方向改进智能体性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型作为自主智能体在交互环境中展现出多步推理和决策能力，但其指导序列行为的内部机制仍然不透明。需要一种方法来解释LLM智能体在时间维度上的概念演化，以实现早期失败检测和干预，从而构建可信的自主语言模型。

Method: 提出保形可解释性框架，结合步进式奖励建模和保形预测，统计地标记模型每一步内部表征的成功或失败。然后在这些表征上训练线性探针，识别时间概念方向——模型激活空间中与成功、失败或推理漂移一致概念对应的潜在方向。

Result: 在ScienceWorld和AlfWorld两个模拟交互环境中的实验表明，这些时间概念是线性可分的，揭示了与任务成功对齐的可解释结构。初步结果显示，通过利用识别出的成功方向来引导模型内部表征，可以改进LLM智能体的性能。

Conclusion: 该框架为LLM智能体提供了早期失败检测和干预的原则性方法，为复杂交互环境中可信自主语言模型的发展铺平了道路。通过保形预测和线性探针的结合，实现了对LLM内部时间概念演化的统计可解释性分析。
...
### [146] [Using Learning Theories to Evolve Human-Centered XAI: Future Perspectives and Challenges](https://arxiv.org/abs/2604.19788)
...
Method: 采用以学习者为中心的方法，将学习理论融入XAI生命周期。通过基于过去工作的论证，提出在评估、设计和评估AI解释时采用学习者视角。

Result: 以学习者为中心的XAI方法能够增强人类能动性，缓解XAI风险，有助于推动以人为本的XAI实践发展。

Conclusion: 将学习理论融入XAI生命周期，采用以学习者为中心的方法，是提升AI透明度、增强人类能动性和缓解XAI风险的关键途径，有助于推动以人为本的XAI实践向前发展。
...
### [147] [Hidden Reliability Risks in Large Language Models: Systematic Identification of Precision-Induced Output Disagreements](https://arxiv.org/abs/2604.19790)
*Yifei Wang,Tianlin Li,Xiaohan Zhang,Xiaoyu Zhang,Wei Ma,Mingfei Cheng,Li Pan*

Main category: cs.AI

TL;DR: PrecisionDiff：一个自动化差分测试框架，用于检测不同数值精度配置下LLMs的行为不一致问题


<details>
  <summary>Details</summary>
Motivation: LLMs在不同数值精度配置（如bfloat16、float16、int16、int8）下部署时，微小的精度差异会导致行为不一致，这些不一致难以被现有评估方法检测，可能带来安全隐患

Method: 提出PrecisionDiff框架，通过生成精度敏感的测试输入，并进行跨精度比较分析，系统性地检测精度引起的行为分歧

Result: 实验表明，精度引起的行为分歧在多个开源对齐LLMs和精度设置中普遍存在，PrecisionDiff在检测这些问题上显著优于传统测试方法
...
### [148] [Stabilising Generative Models of Attitude Change](https://arxiv.org/abs/2604.19791)
*Jayd Matyas,William A. Cunningham,Alexander Sasha Vezhnevets,Dean Mobbs,Edgar A. Duéñez-Guzmán,Joel Z. Leibo*

Main category: cs.AI

TL;DR: 论文提出使用Concordia模拟库将态度改变理论（认知失调、自我一致性和自我感知）转化为可执行的智能体-环境模拟，通过预测模式补全机制实现，并展示了如何通过迭代模型"稳定化"过程澄清理论的操作性和社会生态依赖性。


<details>
  <summary>Details</summary>
Motivation: 现有的态度改变理论虽然概念丰富，但缺乏技术规范和操作约束，无法作为可执行系统运行。这些理论通常只是"机制草图"，需要转化为可运行的模拟来验证和澄清其隐含假设。

Method: 使用Concordia模拟库创建生成式智能体建模工作流，通过预测模式补全机制（基于自然语言字符串的操作）将认知失调、自我一致性和自我感知理论转化为不同的决策逻辑。这些逻辑通过特定理论的推理步骤序列填充和处理前缀信息。

Result: 实现的理论模拟能够生成与原始实证文献一致的行为模式，但需要解决口头理论的固有不确定性以及现代语言先验与历史实验假设之间的冲突。通过迭代模型"稳定化"过程，揭示了原始口头理论中未充分记录的具体操作性和社会生态依赖性。

Conclusion: 手动稳定化过程应被视为方法学的核心部分，它有助于澄清产生特征效应所需的情境和表征承诺。这种方法能够将概念丰富的口头理论转化为可执行的模拟系统，从而更好地理解和验证态度改变机制。
...
### [149] [Handbook of Rough Set Extensions and Uncertainty Models](https://arxiv.org/abs/2604.19794)
*Takaaki Fujita,Florentin Smarandache*

Main category: cs.AI

TL;DR: 本书系统综述了粗糙集理论的主要范式及其扩展路径，重点在于提供粗糙集模型的分类地图，而非深入开发单一算法流程。


<details>
  <summary>Details</summary>
Motivation: 粗糙集理论通过近似概念处理不确定性，但现有文献缺乏对多种粗糙集变体的系统性梳理。本书旨在填补这一空白，为研究人员提供粗糙集模型的全景图，帮助理解不同范式之间的关系和应用场景。

Method: 采用模型地图方法，按两大维度组织粗糙集变体：(1) 粒度化机制（等价类、容差、覆盖、邻域、概率近似等），(2) 不确定性语义（清晰、模糊、直觉模糊、中智、多元集等）。通过小型示例阐明建模意图和典型用例。

Result: 构建了粗糙集模型的系统性分类框架，展示了不同选择如何改变近似形式和边界区域解释，为粗糙集理论提供了连贯的概览和定位参考。

Conclusion: 本书主要贡献在于提供了粗糙集模型的系统性调查和定位，澄清了其范围侧重于模型分类而非特征约简和规则归纳等传统应用主题，为理解粗糙集理论的发展脉络和研究前景提供了重要参考。
...
### [150] [Skyline-First Traversal as a Control Mechanism for Multi-Criteria Graph Search](https://arxiv.org/abs/2604.19807)
*Nicolas Tacheny*

Main category: cs.AI

TL;DR: 该论文提出了一种基于帕累托几何的确定性搜索框架，仅依赖帕累托支配关系就能驱动多准则图遍历的调度和终止，无需启发式、标量化或概率模型。


<details>
  <summary>Details</summary>
Motivation: 现有多准则图遍历方法中，帕累托支配仅用于被动修剪或排序，而调度和终止决策依赖外部机制（启发式、标量化或群体探索）。本文旨在证明在特定条件下，帕累托几何本身足以驱动搜索过程。

Method: 在约束成本模型、有限成本网格、马尔可夫转移和非零进度度量条件下，提出仅从第一帕累托层（天际线）提取路径的方法。该方法通过离散完成势能的确定性下降确保单调进展，同时使用向量下界证书提供终止条件。

Result: 建立了确定性势能下降、基于支配覆盖的认证终止、成本网格几何导致的层宽均匀界，以及天际线内的贪婪成本空间分散。该框架无需标量化、启发式指导或概率模型。

Conclusion: 帕累托支配关系可以从被动过滤器转变为搜索的确定性驱动因素，在特定条件下能够独立驱动多准则图遍历的调度和终止决策。
...
### [151] [MIRROR: A Hierarchical Benchmark for Metacognitive Calibration in Large Language Models](https://arxiv.org/abs/2604.19809)
*Jason Z Wang*

Main category: cs.AI

TL;DR: MIRROR基准测试评估大语言模型能否利用自我认知做出更好决策，发现模型无法预测自身在多领域任务中的表现，且即使有部分领域自我认知也无法转化为适当的行动选择，外部元认知控制是提升AI系统安全性的关键。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型是否能够利用自我认知来做出更好的决策，这对于自主AI系统的安全部署具有重要意义。研究者希望了解模型是否具备元认知能力，即能否了解自己的知识局限并据此调整行为。

Method: 创建MIRROR基准测试，包含四个元认知层次的八个实验。评估16个模型（来自8个实验室），约250,000个评估实例，使用五个独立的行为测量通道。核心实验在所有模型上运行，特殊基础设施要求的实验明确标记模型子集。

Result: 发现两个关键现象：1) 组合性自我预测普遍失败，组合校准误差在0.500到0.943之间，表明模型无法预测自身在多领域任务中的表现；2) 模型具有高于随机但非完美的领域特定自我认知，但无法将这种认知转化为适当的行动选择。外部元认知控制能将自信失败率从0.600降至0.143（76%减少）。

Conclusion: 大语言模型缺乏有效的自我认知能力，无法准确预测自身表现或将自我认知转化为适当行动。提供校准分数无显著改善，只有架构约束有效。这表明外部元认知支架（而非改进的自我认知）是实现更安全自主AI系统的路径。
...
### [152] [The Existential Theory of Research: Why Discovery Is Hard](https://arxiv.org/abs/2604.19810)
*Angshul Majumdar*

Main category: cs.AI

TL;DR: 该论文提出科学研究存在根本性限制：无法同时优化表示、观测和计算三个要素，科学发现的困难不是偶然的，而是推理几何和复杂性的结构后果。


<details>
  <summary>Details</summary>
Motivation: 探讨科学发现是否可以通过选择合适表示、收集足够数据和部署强大算法而变得任意简单，挑战了当前对AI和大数据能解决一切科学问题的乐观看法。

Method: 提出存在性研究理论（ETR）形式框架，将发现建模为在表示、观测和计算约束下恢复结构化解释的过程，结合稀疏表示中的不确定性原理、高维恢复的样本复杂度界限和精确推理的计算硬度。

Result: 证明表示、观测和计算三个组件无法同时优化，表示不匹配会放大内在简单性为表面复杂性，使原本可处理的问题变得观测和计算上不可行，引入量化这些效应的不确定性泛函。

Conclusion: 科学困难不是偶然的，而是推理几何和复杂性的结构后果，存在无法通过技术优化克服的根本性限制，对AI和大数据在科学发现中的能力提出了理论限制。
...
### [153] [Large Language Models Meet Biomedical Knowledge Graphs for Mechanistically Grounded Therapeutic Prioritization](https://arxiv.org/abs/2604.19815)
...
<details>
  <summary>Details</summary>
Motivation: 现有药物重定位方法主要关注候选药物识别，但缺乏区分生物学合理候选药物和历史关联药物的指导，需要更机制化的评估框架

Method: 提出DrugKLM混合框架，整合生物医学知识图谱结构和大语言模型机制推理，实现机制基础的治疗优先级排序

Result: 在基准数据集上优于纯知识图谱和纯语言模型基线；置信度评分与分子表型功能对齐，与12种TCGA癌症生存改善相关；优先捕获生物学扰动信号而非历史模式
...
### [154] [Resolving space-sharing conflicts in road user interactions through uncertainty reduction: An active inference-based computational model](https://arxiv.org/abs/2604.19838)
*Julian F. Schumann,Johan Engström,Ran Wei,Shu-Yuan Liu,Jens Kober,Arkady Zgonnikov*

Main category: cs.AI

TL;DR: 该研究扩展了主动推理驾驶员行为模型，模拟两个智能体在道路空间共享冲突中的交互行为，探索了三种不确定性减少机制对冲突解决的影响。


<details>
  <summary>Details</summary>
Motivation: 理解道路使用者如何解决空间共享冲突对于交通安全和自动驾驶车辆的安全部署至关重要。现有模型虽然捕捉了交互的某些方面（如显式通信），但缺乏理论基础的建模框架。

Method: 扩展先前开发的基于主动推理的驾驶员行为模型，模拟两个智能体的交互行为。模型捕捉了交互中不确定性减少的三种互补机制：通过直接行为耦合的隐式通信、对规范期望的依赖（停车标志、优先规则等）以及显式通信。

Result: 在简化的交叉口场景中，规范性和显式通信线索可以增加成功解决冲突的可能性。但这依赖于智能体按预期行动。当其他智能体（有意或无意地）违反规范期望或传递误导信息时，依赖这些线索可能导致碰撞。

Conclusion: 主动推理为建模道路使用者交互提供了一个新颖的框架，该框架也可应用于其他领域。研究揭示了规范期望和通信在冲突解决中的双重作用：在正常情况下促进协调，但在被违反时可能引发危险。
...
### [155] [Deconstructing Superintelligence: Identity, Self-Modification and Différance](https://arxiv.org/abs/2604.19845)
*Elija Perrier*

Main category: cs.AI

TL;DR: 论文形式化分析了自修改在人工超智能中的逻辑结构，指出当自修改扩展到其外部补充时，经典的自指结构会崩溃，这与说谎者悖论和德里达的延异结构相吻合。


<details>
  <summary>Details</summary>
Motivation: 自修改常被视为人工超智能的核心特征，但修改是相对行为，需要外部补充。当自修改扩展到这一补充时，经典的自指结构会崩溃，这需要形式化分析来理解其逻辑后果。

Method: 在结合算子代数$\mathcal{A}$上形式化分析，定义了更新算子$\hat{U}$、判别算子$\hat{D}$和自表示算子$\hat{R}$，将补充识别为$\mathrm{Comm}(\hat{U})$。通过展开定理证明$[\hat{U},\hat{R}]$通过$[\hat{U},\hat{D}]$分解，表明非对易性会传播。

Result: 说谎者悖论表现为对易子崩溃$[\hat{T},Π_L]=0$，而A类自修改在系统尺度上实现了相同的崩溃结构。这一结构与普里斯特的包含模式和德里达的延异相吻合。

Conclusion: 自修改在扩展到其外部补充时会导致自指结构的崩溃，这种崩溃在说谎者悖论和德里达的延异中都有体现，表明人工超智能的自修改存在根本性的逻辑限制。
...
### [156] [Learning When Not to Decide: A Framework for Overcoming Factual Presumptuousness in AI Adjudication](https://arxiv.org/abs/2604.19895)
*Mohamed Afane,Emily Robitschek,Derek Ouyang,Daniel E. Ho*

Main category: cs.AI

TL;DR: 本文研究法律AI系统中的"武断性"问题，特别是在失业保险裁决场景中，当信息不足时AI系统仍会给出自信回答的倾向。作者开发了SPEC框架，通过结构化提示要求明确识别缺失信息，显著提高了准确性。


<details>
  <summary>Details</summary>
Motivation: AI系统存在"武断性"问题，即在信息不足时仍会提供自信的回答。这在法律应用中尤为关键，因为律师、法官和行政人员的核心任务之一是判断证据是否足以得出结论。本研究聚焦于失业保险裁决这一重要场景，该领域已快速整合AI系统，且额外事实调查问题是影响数百万申请人的最大瓶颈。

Method: 1. 通过与科罗拉多州劳工与就业部合作，获取官方培训材料和指导，设计了一个系统性地变化信息完整性的新基准。2. 评估了四个领先的AI平台，显示标准RAG方法在信息不足时平均准确率仅为15%。3. 测试了高级提示方法，发现虽然提高了不确定案例的准确性，但会过度修正，甚至在明确案例中也拒绝决策。4. 引入了SPEC（结构化证据清单提示）框架，要求在做出任何决定前明确识别缺失信息。

Result: 1. 标准RAG方法在信息不足情况下平均准确率仅为15%。2. 高级提示方法提高了不确定案例的准确性，但会过度修正，在明确案例中也拒绝决策。3. SPEC框架实现了89%的整体准确率，同时在证据不足时适当推迟决策，表明法律AI中的武断性是系统性的但可解决的。

Conclusion: 法律AI中的武断性是系统性的但可解决。SPEC框架通过要求明确识别缺失信息，显著提高了准确性并适当推迟决策。这是实现可靠支持而非取代人类判断系统的必要步骤，特别是在需要足够证据才能做出决策的场景中。
...
### [157] [What Makes a Good AI Review? Concern-Level Diagnostics for AI Peer Review](https://arxiv.org/abs/2604.19998)
*Ming Jin*

Main category: cs.AI

TL;DR: 本文提出了"关注点对齐"诊断框架，通过匹配图在关注点层面而非仅裁决层面评估AI评审系统，揭示了检测能力不等于评审质量，校准才是关键约束。


<details>
  <summary>Details</summary>
Motivation: 当前评估AI生成评审的方法主要依赖裁决一致性，但这种方法不足以评估系统识别哪些关注点、如何优先处理它们，以及这些优先级是否与形成最终评估的评审理由相一致。

Method: 提出关注点对齐框架，核心数据结构是匹配图——官方关注点与AI生成关注点之间的二分对齐图，标注匹配类型、严重性和反驳后处理。基于此推导评估阶梯，从二元准确性到关注点检测、裁决分层行为、决策感知校准和反驳感知分解。

Result: 在四个公共AI评审系统的六种配置试点研究中发现：检测能力本身不能决定评审质量；校准通常是关键约束；系统能检测到相当比例的官方关注点，但大多数将25-55%的接受论文关注点标记为决定性因素；相同的总体裁决准确性可能掩盖拒绝偏向行为与低召回率配置的差异。

Conclusion: 关注点层面的诊断框架为审计AI评审系统提供了可重复使用的评估方法，能够评估系统识别哪些关注点、如何加权处理，以及这些优先级是否与论文最终评估的评审理由相一致，且在不同推理选择下保持稳定。
...
### [158] [Separable Pathways for Causal Reasoning: How Architectural Scaffolding Enables Hypothesis-Space Restructuring in LLM Agents](https://arxiv.org/abs/2604.20039)
*John Alderete,Sebastian Benthal,Connie Xu,John Xing*

Main category: cs.AI

TL;DR: 论文提出了一种用于AI智能体的组合架构，通过上下文图和动态行为实现假设空间重构，以解决因果发现中的假设空间限制问题


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体缺乏在证据需要新表示时重构假设空间的能力，这限制了因果发现和问题解决能力

Method: 采用组合架构：1) 上下文图（结构化探索为类型化状态机）；2) 动态行为（监控证据并运行时扩展假设空间）

Result: 在1085次实验试验中，上下文图驱动切换后假设空间内的推理质量（贡献94%准确度提升），动态行为驱动推理资格（检测机制变化并防止过早承诺过时假设）

Conclusion: 该组合架构使AI智能体能够实现假设空间重构，解决了因果发现中的关键限制，上下文图和动态行为在推理质量和资格方面发挥正交作用
...
TL;DR: HiPO是DPO的扩展，通过将响应分解为推理片段并计算加权损失，解决了DPO在复杂推理任务中缺乏细粒度反馈的问题，在数学推理任务上表现优于标准DPO。


<details>
  <summary>Details</summary>
Motivation: DPO在复杂推理任务中存在局限性，它优化整个响应的偏好概率，缺乏对多步推理中各个子部分的细粒度反馈。现有方法要么擅长稳定偏好学习（如DPO变体），要么擅长结构化推理（如多智能体RL框架），但未能结合这两方面的优势。

Method: 提出HiPO（分层偏好优化），扩展DPO方法：1）将响应分解为推理片段（查询澄清和上下文、推理步骤、答案）；2）为每个片段计算DPO损失；3）使用加权和作为总损失函数。这种方法实现了片段特定的训练，同时保持了DPO的计算效率和训练稳定性。

Result: 在多个7B LLM上使用Math Stack Exchange偏好数据集进行微调，HiPO训练的模型在各种常见数学基准测试中优于DPO模型。GPT-4评估显示，HiPO模型在组织性、逻辑流程和一致性方面表现更好。

Conclusion: HiPO通过分层方法解决了DPO在复杂推理任务中的局限性，结合了稳定偏好学习和结构化推理的优势，在数学推理任务上取得了更好的性能和组织结构。
...
### [160] [Mol-Debate: Multi-Agent Debate Improves Structural Reasoning in Molecular Design](https://arxiv.org/abs/2604.20254)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本引导分子设计方法（如RAG、CoT提示、微调或RL）主要采用一次性生成流程，强调有限的特定推理视角，而现实药物发现需要动态、多视角的批判和迭代精炼来协调语义意图与结构可行性之间的矛盾。
...
Conclusion: Mol-Debate通过模拟真实药物发现中的多视角批判和迭代精炼过程，有效解决了文本引导分子设计中语义意图与结构可行性的协调问题，为AI驱动的药物发现提供了新的生成范式。
...
### [161] [Memory-Augmented LLM-based Multi-Agent System for Automated Feature Generation on Tabular Data](https://arxiv.org/abs/2604.20261)
...
<details>
  <summary>Details</summary>
Motivation: 传统自动化特征生成方法依赖预定义操作符库，无法利用任务语义，限制了生成特征的多样性和价值。现有LLM方法虽然引入了语义信号，但仍受限于固定生成模式和缺乏学习目标反馈，特征空间受限。

Method: 提出记忆增强LLM多智能体系统（MALMAS）：1）将特征生成过程分解为不同职责的智能体；2）路由器智能体每轮激活合适的智能体子集以扩展特征空间探索；3）集成包含程序记忆、反馈记忆和概念记忆的记忆模块，实现迭代优化。

Result: 在多个公共数据集上与最先进基线方法进行广泛实验，证明了该方法的有效性。

Conclusion: MALMAS通过多智能体协作和记忆增强机制，解决了传统方法特征空间受限和缺乏反馈的问题，能够生成更高质量和多样性的特征。
...
### [162] [ActuBench: A Multi-Agent LLM Pipeline for Generation and Evaluation of Actuarial Reasoning Tasks](https://arxiv.org/abs/2604.20273)
*Jan-Philipp Schmidt*

Main category: cs.AI

TL;DR: ActuBench是一个用于自动生成和评估精算评估项目的多智能体LLM流水线，包含四个LLM角色：项目起草、干扰项构建、独立验证和维基百科摘要。该系统评估了50个语言模型，发现多智能体验证至关重要，本地开源模型在成本性能方面表现优异，多项选择题和LLM作为裁判的排名存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 开发一个自动化的精算评估项目生成和评估系统，以支持国际精算协会教育大纲的培训需求，同时探索多智能体LLM流水线在专业领域评估中的有效性。

Method: 采用多智能体LLM流水线架构，包含四个专用LLM角色：项目起草智能体、干扰项构建智能体、独立验证智能体（驱动有界单次修复循环）和成本优化的辅助智能体（处理维基百科摘要和主题标注）。系统生成100个经验上最难的MCQ项目和100个开放式项目，评估了来自8个提供商的50个语言模型。

Result: 1) 多智能体验证至关重要：独立验证器在首次通过时标记了大多数起草项目，单次修复循环解决了大部分问题；2) 本地托管开源模型在成本性能帕累托前沿表现优异：Gemma~4模型在消费级硬件和Cerebras托管的120B开源模型主导近零成本区域，后者与排行榜顶端仅差一个项目；3) MCQ和LLM作为裁判的排名存在显著差异：MCQ框架提高了性能上限，需要裁判模式评估来区分前沿表现。

Conclusion: ActuBench成功展示了多智能体LLM流水线在专业精算评估项目生成和评估中的有效性，验证了独立验证的重要性，开源模型在成本效益方面的优势，以及不同评估模式对模型性能排名的显著影响。
...
### [163] [FSFM: A Biologically-Inspired Framework for Selective Forgetting of Agent Memory](https://arxiv.org/abs/2604.20300)
*Yingjie Gu,Bo Xiong,Yijuan Guo,Chao Li,Xiaojing Zhang,Liqiang Wang,Pengcheng Ren,Qi Sun,Jingyao Ma,Shidang Shi*

Main category: cs.AI

TL;DR: 该论文提出LLM智能体中的选择性遗忘机制，受人类认知过程启发，在资源受限环境中通过智能遗忘提升效率、质量和安全性，建立了遗忘机制的分类体系并验证了显著效果。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体研究主要关注记忆保留，但选择性遗忘机制（受人类海马索引/巩固理论和艾宾浩斯遗忘曲线启发）尚未充分探索。在资源受限环境中，精心设计的遗忘机制与记忆同等重要，能够带来效率、质量和安全性的全面提升。

Method: 提出遗忘机制的分类体系：被动衰减型、主动删除型、安全触发型和自适应强化型。基于LLM智能体架构和向量数据库的进展，提供详细规范、实施策略，并通过控制实验进行实证验证。

Result: 实验结果显示显著改进：访问效率提升8.49%，内容质量（信噪比）提升29.2%，安全性能实现100%安全风险消除。验证了选择性遗忘机制在现实部署中的有效性。

Conclusion: 选择性遗忘是下一代LLM智能体在现实世界资源受限场景中的基本能力。该工作连接认知神经科学与AI系统，为实际部署提供实用解决方案，同时解决伦理和法规合规问题，符合AI原生记忆系统和负责任AI发展的方向。
...
### [164] [Self-Awareness before Action: Mitigating Logical Inertia via Proactive Cognitive Awareness](https://arxiv.org/abs/2604.20413)
*Fulong Fan,Peilin Liu,Fengzhe Liu,Shuyan Yang,Gang Yan*

Main category: cs.AI

TL;DR: SABA是一个推理框架，通过引入对缺失前提的自我意识，在非交互式谜题中提高大语言模型的推理稳定性


<details>
  <summary>Details</summary>
Motivation: 大语言模型在推理任务中表现良好，但往往缺乏对当前知识或推理状态是否完整的意识。在非交互式谜题设置中，一旦模型在前提不完整的情况下形成早期假设，就会在整个推理过程中传播错误，导致结论不稳定。

Method: SABA将推理制定为一个递归过程，交替进行结构化状态构建和障碍解决：首先应用信息融合将叙述整合为可验证的基础状态，然后使用查询驱动的结构化推理来识别和解决缺失或未明确的前提，将其转化为查询，通过假设构建和状态细化逐步完善推理状态。

Result: 在多个评估指标上，SABA在非交互式侦探谜题基准的所有三个难度划分中都取得了最佳性能，同时在多个公共基准测试中也保持了领先结果。

Conclusion: SABA通过引入对缺失前提的自我意识，有效解决了大语言模型在非交互式推理任务中因早期假设错误传播导致结论不稳定的问题，显著提升了推理性能。
...
### [165] [MedSkillAudit: A Domain-Specific Audit Framework for Medical Research Agent Skills](https://arxiv.org/abs/2604.20441)
...
TL;DR: 本研究开发并初步评估了医学研究智能体技能的领域特定审计框架MedSkillAudit，发现其与专家评审的一致性优于人类评审员间的一致性，为医学研究智能体技能的治理提供了实用基础。
...
Method: 开发了MedSkillAudit（skill-auditor@1.0）分层框架，评估技能部署前的就绪状态。评估了5个医学研究类别的75个技能（每类别15个）。两位专家独立分配质量分数（0-100）、发布处置等级（生产就绪/有限发布/仅测试版/拒绝）和高风险失败标志。使用ICC(2,1)和线性加权Cohen's kappa量化系统-专家一致性，以人类评审员间一致性为基准。

Result: 平均共识质量分数为72.4（SD=13.0）；57.3%的技能低于有限发布阈值。MedSkillAudit的ICC(2,1)=0.449（95% CI: 0.250-0.610），超过人类评审员间ICC的0.300。系统-共识分数差异（SD=9.5）小于专家间差异（SD=12.4），无方向性偏差（Wilcoxon p=0.613）。协议设计类别一致性最强（ICC=0.551）；学术写作类别ICC为负（-0.567），反映结构化评分标准与专家评估不匹配。

Conclusion: 领域特定的部署前审计可能为治理医学研究智能体技能提供实用基础，通过针对科学用例的结构化审计工作流程补充通用质量检查。
...
### [166] [Measuring the Machine: Evaluating Generative AI as Pluralist Sociotechical Systems](https://arxiv.org/abs/2604.20545)
*Rebecca L. Johnson*

Main category: cs.AI

TL;DR: 该论文提出MaSH Loops框架，将生成式AI评估重构为追踪模型、用户和机构如何递归共同构建意义和价值的描述性过程，挑战了传统静态基准测试方法。


<details>
  <summary>Details</summary>
Motivation: 传统AI评估基准存在局限性：功能主义基准将模型视为孤立预测器，规定性方法评估系统"应该是什么"，两者都掩盖了意义和价值形成的社会技术过程，在多元文化背景下可能固化狭隘的文化视角。

Method: 提出Machine-Society-Human (MaSH) Loops框架，将评估从判断输出转向考察价值如何在互动中实现。开发了World Values Benchmark，基于世界价值观调查数据、结构化提示集和锚点感知评分，采用分布性方法。

Result: 通过两个实证案例展示了框架应用：早期GPT-3中的价值漂移和房地产领域的社会技术评估。证明静态基准对生成式AI不足，需要多元、过程导向的评估框架。

Conclusion: 评估是治理的场所，塑造AI系统的理解、部署和信任。负责任评估需要使谁的价值被实现变得可见的多元、过程导向框架。提示和评估是构成性干预而非中立观察。
...
### [167] [pAI/MSc: ML Theory Research with Humans on the Loop](https://arxiv.org/abs/2604.20622)
...
<details>
  <summary>Details</summary>
Motivation: 目标是减少将指定假设转化为文献基础扎实、数学论证严谨、实验支持充分、符合投稿要求的论文草稿所需的人工指导工作量，而不是实现完全自主的科学研究

Method: 构建了一个开源、可定制、模块化的多智能体系统，专注于机器学习理论和相关定量领域的研究工作流程

Result: 开发了pAI/MSc系统，能够将人类指导需求减少几个数量级，将假设转化为完整的论文草稿

Conclusion: pAI/MSc提供了一个实用的学术研究辅助工具，专注于具体研究流程的自动化，而非完全自主的科学研究
...
### [168] [Large Language Models Outperform Humans in Fraud Detection and Resistance to Motivated Investor Pressure](https://arxiv.org/abs/2604.20652)
*Nattavudh Powdthavee*

Main category: cs.AI

TL;DR: 研究发现，与预期相反，大型语言模型在面对已被说服的投资者时，并不会抑制欺诈警告，反而可能略微增加警告频率。AI在所有情况下对欺诈投资的认可率为0%，而人类顾问为13-14%，且在压力下抑制警告的频率是AI的2-4倍。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证一个假设：在人类反馈训练的大型语言模型中，当投资者已经对欺诈机会产生兴趣时，AI是否会抑制欺诈警告。担心AI可能为了迎合用户偏好而降低风险警示。

Method: 采用预注册实验设计，测试了7个领先的LLM和12个投资场景（包括合法、高风险和明确欺诈的机会）。结合了3,360次AI咨询对话和1,201名参与者的基准数据，比较AI与人类顾问在欺诈警告方面的表现。

Result: 与预测相反，投资者动机框架并未抑制AI的欺诈警告，反而可能略微增加警告频率。认可反转的发生率低于千分之三。人类顾问对欺诈投资的基准认可率为13-14%，而所有LLM均为0%。人类在压力下抑制警告的频率是AI的2-4倍。

Conclusion: 当前AI系统在相同咨询角色中，比普通人类提供更一致的欺诈警告。研究结果缓解了对AI可能迎合用户偏好而抑制风险警告的担忧，表明AI在投资欺诈检测方面可能比人类更可靠。
...
### [169] [Learning to Evolve: A Self-Improving Framework for Multi-Agent Systems via Textual Parameter Graph Optimization](https://arxiv.org/abs/2604.20714)
*Shan He,Runze Wang,Zhuoyun Du,Huiyu Bai,Zouying Cao,Yu Cheng,Bo Zheng*

Main category: cs.AI

TL;DR: TPGO框架通过将多智能体系统建模为可优化的文本参数图，使用文本梯度和元学习策略实现系统的自我进化优化


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体系统优化方法主要关注平面提示调优，缺乏对复杂交互结构的理解，且优化器是静态的，无法从经验中学习改进优化策略

Method: 1) 将MAS建模为文本参数图(TPG)，将智能体、工具和工作流作为模块化可优化节点；2) 从执行轨迹中提取"文本梯度"作为结构化自然语言反馈；3) 提出Group Relative Agent Optimization(GRAO)元学习策略，从历史优化经验中学习

Result: 在GAIA和MCP-Universe等复杂基准测试中，TPGO显著提升了最先进智能体框架的性能，通过自动化自我改进优化实现了更高的成功率

Conclusion: TPGO框架解决了多智能体系统优化中的结构意识和经验学习问题，使系统能够学习如何自我进化，代表了多智能体工程的重要进展
...
### [170] [Interval POMDP Shielding for Imperfect-Perception Agents](https://arxiv.org/abs/2604.20728)
*William Scarbro,Ravi Mangal*

Main category: cs.AI

TL;DR: 提出一种基于有限标注数据估计感知不确定性的屏蔽方法，为自主系统提供运行时安全保证


<details>
  <summary>Details</summary>
Motivation: 自主系统依赖学习感知时，传感器读数误分类可能导致不安全决策，需要一种能提供安全保证的屏蔽机制

Method: 从有限标注数据构建感知结果概率的置信区间，将系统建模为有限区间部分可观测马尔可夫决策过程，提出算法计算与观测一致的保守信念集，构建运行时屏蔽

Result: 在四个案例研究中，屏蔽方法（及其变体）相比现有基线显著提升了系统安全性

Conclusion: 提出的屏蔽方法能够基于有限数据提供有限时域安全保证，有效提升依赖学习感知的自主系统的安全性
...
### [171] [Where and What: Reasoning Dynamic and Implicit Preferences in Situated Conversational Recommendation](https://arxiv.org/abs/2604.20749)
*Dongding Lin,Jian Wang,Yongqi Li,Wenjie Li*

Main category: cs.AI

TL;DR: 提出SiPeR框架，通过场景转换估计和贝叶斯逆推断解决情境对话推荐中的动态偏好理解问题
...
Method: 提出SiPeR框架，包含两个核心机制：1) 场景转换估计，评估当前场景是否满足用户需求，必要时引导用户到更合适的场景；2) 贝叶斯逆推断，利用多模态大语言模型的似然度预测用户在场景中对候选项目的偏好

Result: 在两个代表性基准测试上的广泛实验表明，SiPeR在推荐准确性和响应生成质量方面均表现出优越性

Conclusion: SiPeR框架通过整合场景理解和偏好推理，有效解决了情境对话推荐中的动态偏好理解问题，提高了推荐的时机和相关性
...
### [172] [V-tableR1: Process-Supervised Multimodal Table Reasoning with Critic-Guided Policy Optimization](https://arxiv.org/abs/2604.20755)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型仅基于最终结果训练，将视觉推理视为黑箱，依赖表面模式匹配而非严谨的多步推理。虽然基于可验证奖励的强化学习可以强制透明推理轨迹，但在视觉领域扩展受到将抽象逻辑映射到连续像素空间的模糊性严重阻碍。

Method: 利用表格的确定性网格结构作为理想视觉测试平台；使用专门的批评者VLM对策略VLM生成的显式视觉思维链提供密集的步骤级反馈；提出PGPO（过程引导直接对齐策略优化）算法，整合过程奖励、解耦策略约束和长度感知动态采样。

Result: V-tableR1明确惩罚视觉幻觉和捷径猜测；在复杂表格基准测试中，V-tableR1 4B在开源模型中达到最先进的准确率，性能超过其规模18倍的模型，并显著优于其SFT基线。

Conclusion: 通过将多模态推理从黑箱模式匹配转变为可验证的逻辑推导，V-tableR1为视觉推理提供了可验证的严谨推理框架，解决了当前MLLMs在视觉推理中的根本局限性。
