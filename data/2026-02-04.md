...
TL;DR: WorldVQA是一个专门评估多模态大语言模型视觉世界原子知识的基准测试，旨在分离视觉知识检索与推理能力，严格测量模型记忆的视觉事实知识。


<details>
  <summary>Details</summary>
Motivation: 当前评估方法往往将视觉知识检索与推理能力混淆，无法准确衡量模型真正记忆的视觉世界知识。需要一种能够严格测量模型"记忆了什么"的基准测试，以评估视觉事实性和幻觉率。

Method: 通过解耦视觉知识检索与推理能力，专注于评估模型对视觉实体的基础能力：定位和命名。采用分层分类法，涵盖从常见头部类别对象到长尾稀有实体的广泛范围。

Result: WorldVQA基准测试能够严格评估多模态大语言模型的视觉事实性，为衡量当前和下一代前沿模型的百科全书式知识广度和幻觉率建立标准。

Conclusion: WorldVQA作为一个严谨的视觉世界知识评估基准，能够有效分离知识检索与推理能力，为评估多模态大语言模型的视觉事实性和减少幻觉提供标准化测试工具。
...
Method: 构建AdaptMMBench基准，涵盖五个领域：现实世界、OCR、GUI、知识和数学，包含直接感知和复杂推理任务。使用马修斯相关系数评估不同推理模式的选择合理性，基于模型能力边界动态识别任务难度。支持多维度过程评估，包括关键步骤覆盖、工具有效性和计算效率。

Result: 评估发现：自适应模式选择能力随模型容量扩展，但与最终准确率显著解耦；关键步骤覆盖与性能一致，但工具有效性在不同模型架构间高度不一致。

Conclusion: AdaptMMBench为自适应多模态推理提供了全面的评估框架，能够分离元认知能力评估，揭示自适应推理的动态特性，并为未来研究提供多维度分析工具。
...
TL;DR: ELIQ是一个无需人工标注的质量评估框架，专门针对不断进化的AI生成图像，通过自动构建正负样本对来评估视觉质量和提示-图像对齐度。


<details>
  <summary>Details</summary>
Motivation: 生成式文本到图像模型快速发展，使得之前收集的标签对于新一代模型变得不可靠，需要一种无需人工标注的质量评估方法来适应不断进化的生成模型。

Method: ELIQ自动构建正负样本对覆盖传统失真和AIGC特定失真模式，通过指令调优将预训练多模态模型转化为质量感知的批评器，使用轻量级门控融合和质量查询变换器预测二维质量。

Result: 在多个基准测试中，ELIQ始终优于现有的无标签方法，无需修改即可从AI生成内容泛化到用户生成内容场景。

Conclusion: ELIQ为在持续进化的生成模型下进行可扩展且无需标注的质量评估铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 光学相干断层扫描（OCT）中的逆散射问题旨在恢复结构图像和内在组织光学特性（折射率、散射系数、各向异性），但由于衰减、散斑噪声和参数间的强耦合，这一逆问题具有挑战性

Method: 提出正则化端到端深度学习框架，结合基于物理的OCT前向模型，从估计参数生成预测信号，为参数恢复和伪影抑制提供物理一致性监督，使用蒙特卡罗模拟的真实数据进行训练
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers由于自注意力机制全局操作，缺乏明确区分前景与背景的机制，可能学习到不必要的背景特征和伪影，导致分类性能下降
...
Conclusion: SVD-ViT通过奇异值分解机制增强了ViT对前景特征的学习能力，改善了分类性能，为视觉Transformer提供了更好的前景-背景区分机制
...
### [6] [LEVIO: Lightweight Embedded Visual Inertial Odometry for Resource-Constrained Devices](https://arxiv.org/abs/2602.03294)
*Jonas Kühne,Christian Vogt,Michele Magno,Luca Benini*

Main category: cs.CV

TL;DR: LEVIO是一个专为超低功耗计算平台优化的视觉惯性里程计系统，能够在资源受限硬件上实现实时6自由度运动追踪，功耗低于100mW，帧率达20FPS。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的视觉惯性里程计系统计算需求过高，无法在微无人机、智能眼镜等资源受限硬件上运行，需要开发适用于超低功耗平台的运动追踪解决方案。

Method: 采用硬件-软件协同优化方法，结合ORB特征跟踪和束调整等成熟VIO组件，设计计算效率高的架构，强调并行化和低内存使用，适用于嵌入式微控制器和低功耗SoC。

Result: 在并行处理的超低功耗RISC-V SoC上验证，功耗低于100mW，帧率达20FPS，在公开VIO数据集上基准测试，在效率和精度之间取得良好平衡。

Conclusion: LEVIO为资源受限硬件提供了可行的实时运动追踪解决方案，实现了超低功耗下的6自由度感知，开源实现促进可重复性和采用。
...
<details>
  <summary>Details</summary>
Motivation: 传统手动标注耗时且存在观察者间差异，基于规则的方法通常局限于特定几何形状或有限标志集。需要一种能利用不同物种同源骨骼进行转化研究的自动解剖标志检测方法。

Method: 提出Landmark Point Transformer (LmPT)方法，将解剖表面表示为点云，包含条件机制以适应不同输入类型，实现跨物种学习。

Result: 在人类和新标注的狗股骨上进行评估，展示了该方法在跨物种间的泛化能力和有效性。

Conclusion: LmPT是一种有效的自动解剖标志检测方法，能够利用不同物种的同源骨骼进行转化研究，代码和狗股骨数据集将公开。
...
<details>
  <summary>Details</summary>
Motivation: 手术室视频数据用于研究需要隐私保护，现有方法面临两个可扩展性瓶颈：1) 需要为每个新临床站点手动标注；2) 多摄像头设置需要频繁重新标定。需要一种无需标注和相机标定的自动化解决方案。

Method: 提出自监督多视角视频匿名化框架，包含全身人员检测和全身姿态估计。核心策略是通过时间和多视角上下文"检索"假阴性检测，并进行自监督域适应。首先使用低阈值运行现成检测器收集候选检测，然后通过跟踪和自监督未标定多视角关联检索与高置信度检测一致的假阴性，这些恢复的检测作为伪标签迭代微调检测器，最后使用自身高置信度预测微调姿态模型。

Result: 在模拟手术的4D-OR数据集和真实手术数据集上验证，方法实现超过97%的召回率。使用伪标签训练实时全身检测器，性能相当，展示了方法的实际应用性。

Conclusion: 提出的自监督多视角框架能够有效解决手术室视频匿名化的可扩展性问题，无需手动标注或相机标定，实现了高召回率，并具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有CoT推理在视觉语言模型中表现不佳，因为过早的视觉到文本转换会丢失几何和空间布局等连续信息。现有方法通过静态枚举或基于注意力的选择来增强CoT，但仍是被动处理预计算输入，而非主动寻找任务相关细节。

Method: 受人类主动感知启发，ViThinker框架使视觉语言模型能够自主生成决策（查询）令牌，触发按需合成专家对齐的视觉特征。通过两阶段课程：首先将冻结专家蒸馏到模型参数中，然后通过稀疏性惩罚学习任务驱动的查询，ViThinker为每个推理步骤发现最小充分感知。

Result: 在视觉中心基准测试中的评估显示了一致的改进，验证了主动查询生成在感知基础和推理准确性方面优于被动方法。

Conclusion: ViThinker通过主动查询生成机制，实现了比被动方法更好的视觉语言推理性能，证明了主动感知在视觉语言模型中的有效性。
...
TL;DR: 本文提出了一种用于医学影像决策的对比性文档感知参考选择框架，通过构建优化的证据集来提高诊断准确性，相比传统方法提升了近15%的准确率。
...
Method: 提出了对比性文档感知参考选择框架，通过平衡视觉相关性、嵌入多样性和来源级溯源来构建优化的证据集；基于这些对比证据集，提出了Counterfactual-Contrastive Inference框架，执行结构化成对视觉比较并使用基于边界的决策规则聚合证据。

Result: 在MediConfusion基准测试中，该方法实现了最先进的性能，相对于先前方法将集合级准确率提高了近15%，同时减少了混淆并提高了个体准确性。

Conclusion: 通过对比性参考选择和结构化推理框架，能够显著提高医学影像决策的准确性和可靠性，为解决混淆条件区分问题提供了有效方案。
...
TL;DR: 现有基于变换的隐私保护人脸识别系统主要关注像素级重建攻击，但FaceLinkGen攻击表明即使不重建原始像素，也能通过模板进行身份匹配和面部再生，揭示当前评估指标与真实隐私保护之间的结构性差距。


<details>
  <summary>Details</summary>
Motivation: 当前隐私保护人脸识别系统的评估主要关注像素级重建攻击，使用PSNR和SSIM等指标衡量隐私保护效果。然而，这种重建中心的视角存在缺陷，无法反映真实的隐私风险。

Method: 提出FaceLinkGen攻击方法，直接从受保护的模板中执行身份链接/匹配和面部再生，无需恢复原始像素。该方法在三种最近的PPFR系统上进行测试，包括近零知识设置下的评估。

Result: 在三种PPFR系统上，FaceLinkGen达到超过98.5%的匹配准确率和96%以上的再生成功率。在近零知识设置下，仍超过92%的匹配准确率和94%的再生成功率。

Conclusion: 视觉混淆无法有效保护身份信息，身份信息仍然广泛暴露给外部入侵者和不可信服务提供商。像素失真指标与真实隐私之间存在结构性差距，需要重新思考PPFR系统的隐私评估方法。
...
TL;DR: MARBLE是首个纯Mamba架构的多状态多实例学习框架，用于全切片图像分析，通过并行多尺度处理和线性时间序列建模，显著提升了WSI分析的效率和性能。


<details>
  <summary>Details</summary>
Motivation: 全切片图像分析面临千兆像素分辨率和多级放大的挑战，现有方法通常只在单一尺度操作，而基于Transformer的方法存在二次注意力计算成本问题，需要更高效的多尺度处理框架。

Method: MARBLE采用纯Mamba架构，并行处理多个放大级别，在线性时间状态空间模型中集成从粗到细的推理，以最小参数开销有效捕捉跨尺度依赖关系。

Result: 在五个公开数据集上的实验显示，MARBLE在AUC上提升达6.9%，准确率提升20.3%，C-index提升2.3%，证明了其高效性和泛化能力。

Conclusion: MARBLE为多尺度WSI分析提供了一个可扩展、模块化的替代注意力架构的框架，通过线性时间序列建模和并行多尺度处理，显著提升了全切片图像分析的性能。
...
TL;DR: SRA-Seg框架通过特征对齐解决合成数据在医学图像分割中的领域差距问题，仅用10%真实标注数据和90%合成数据就能达到与使用真实未标注数据方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 合成数据虽然视觉逼真，但由于与真实医学图像存在语义特征空间差异，当前半监督学习方法无法有效利用合成数据提升分割性能，需要解决这一领域差距问题。

Method: 提出SRA-Seg框架：1）使用冻结的DINOv2嵌入和相似性对齐损失将合成特征拉向最近的实现在语义空间中的对应特征；2）采用软边缘混合创建平滑解剖过渡和连续标签；3）通过EMA教师模型为合成图像生成伪标签，并应用考虑混合区域不确定性的软分割损失。

Result: 仅使用10%标注真实数据和90%合成未标注数据，在ACDC数据集上达到89.34% Dice分数，在FIVES数据集上达到84.42% Dice分数，显著优于现有半监督方法，与使用真实未标注数据的方法性能相当。

Conclusion: SRA-Seg通过特征分布对齐有效解决了合成数据在医学图像分割中的领域差距问题，为在标注数据稀缺情况下利用合成数据提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉令牌剪枝方法在视觉问答任务上表现良好，但在视觉定位任务上性能显著下降。研究发现，基于全局语义相似性和注意力分数的策略会丢失全局空间参考框架，而该框架源自令牌位置信息的交互。
...
Conclusion: Nüwa框架通过保留空间完整性的两阶段剪枝策略，成功解决了现有方法在视觉定位任务上的性能退化问题，实现了视觉语言模型的高效加速。
...
Method: 提出TRACE模型，给定先前的和当前的胸部X光片，模型能生成自然语言描述区间变化（恶化、改善、稳定），同时用边界框坐标定位每个发现。

Result: TRACE展示了超过90%的定位准确率，为这一具有挑战性的新任务奠定了基础。消融研究发现，只有当时间比较和空间定位联合学习时，变化检测才会出现。

Conclusion: TRACE是首个联合执行时间比较、变化分类和空间定位的模型。研究发现，定位为时间推理提供了必要的空间注意力机制，两者必须联合学习才能实现有效的变化检测。
...
<details>
  <summary>Details</summary>
Motivation: 鱼眼立体视觉系统在计算机视觉和机器人领域有广泛应用，但现有研究通常假设小角度近似，对于大角度情况下的误差分析不足。特别是在物体距离较远或视角较大时，传统误差模型不够准确，需要更精确的解析表达式来评估系统性能

Method: 研究推导了鱼眼立体视觉系统的深度和距离误差的解析表达式，考虑了物体距离的函数关系，特别关注大角度情况下的精度分析。通过数学建模和理论推导，建立了更准确的误差评估模型

Result: 获得了鱼眼立体视觉系统深度和距离误差的解析表达式，这些表达式能够更准确地描述在不同物体距离和大角度情况下的误差特性，为系统设计和性能评估提供了理论依据

Conclusion: 该研究提供了鱼眼立体视觉系统误差分析的精确解析框架，特别适用于大角度情况，有助于改进系统设计和精度评估，对计算机视觉和机器人应用具有重要意义
...
TL;DR: CAFT是一个分层图像-文本表示学习框架，通过跨域对齐全局和局部语义，无需像素级监督即可实现细粒度视觉语言理解，在长文本检索任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）在处理长标题时存在局限，因为它们将图像和文本作为整体进行对齐。细粒度视觉语言理解需要能够捕捉全局上下文和局部细节的分层语义，但语言层次结构和视觉组织通常不匹配，纯视觉层次结构倾向于将场景分割为外观驱动的部分而缺乏语义焦点。

Method: 提出CAFT框架，耦合从细到粗的视觉编码器和分层文本Transformer，使用分层对齐损失函数，在匹配整个图像和整个标题的同时，偏置区域-句子对应关系，使粗粒度语义建立在细粒度证据基础上，而非脱离部分级基础的聚合。

Result: 在3000万图像-文本对上训练后，CAFT在六个长文本检索基准测试中达到最先进的性能，并表现出强大的扩展行为。实验表明，分层跨域对齐能够在不使用显式区域级监督的情况下，实现细粒度、视觉基础的图像-文本表示。

Conclusion: CAFT通过分层跨域对齐方法，成功解决了长标题视觉语言理解中的细粒度对齐问题，证明了无需像素级监督即可学习到具有视觉基础的细粒度表示，为视觉语言理解提供了新的有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于高斯的动态场景新视角合成方法在静态和动态区域的表示与优化平衡方面存在困难，需要一种能够同时处理长期静态和短期动态区域的统一框架。

Method: 引入可学习的寿命参数，将时间可见性从高斯衰减重新定义为平顶轮廓；寿命参数调节每个基元的运动，解耦运动幅度与时间持续时间；设计寿命-速度感知的致密化策略，优化静态和动态区域间的平衡。

Result: 在多个基准测试中达到最先进性能，支持在单张RTX 4090上以4K分辨率和100 FPS进行实时渲染。

Conclusion: SharpTimeGS通过寿命感知的4D高斯表示，有效解决了动态场景中静态和动态区域的平衡问题，实现了高质量的实时4D重建和新视角合成。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于GRPO的强化学习方法存在稀疏奖励信号和计算开销大的问题，需要更高效的后训练方法

Method: 提出Video-OPD框架，直接从当前策略采样轨迹保持训练推理分布对齐，使用前沿教师通过反向KL散度提供密集的token级监督；引入TVDF课程学习，迭代优先选择教师可靠且对学生信息量最大的轨迹

Result: Video-OPD在性能上持续超越GRPO，收敛速度显著更快，计算成本更低

Conclusion: 策略蒸馏是传统强化学习在视频时序定位任务上的有效替代方案
...
TL;DR: VOILA是一个基于信息价值驱动的自适应保真度选择框架，用于视觉问答任务，通过预测不同保真度下的正确率并考虑检索成本，实现成本降低50-60%的同时保持90-95%的准确率。


<details>
  <summary>Details</summary>
Motivation: 大多数多模态视觉语言系统在固定保真度下运行，但获取和处理高保真度视觉输入成本高昂。需要一种能在模型执行前优化信息检索的方法，以在资源受限条件下优化多模态推理。

Method: 采用两阶段管道：1) 梯度提升回归器仅从问题特征估计每个保真度的正确率可能性；2) 等渗校准器细化这些概率以实现可靠决策。系统选择最小成本保真度以最大化给定预测准确率和检索成本的期望效用。
...
Conclusion: 检索前保真度选择对于在资源约束下优化多模态推理至关重要，VOILA框架通过自适应保真度选择在显著降低成本的同时保持高准确率。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像修复方法在卷积下采样过程中不可避免地会丢失结构和纹理特征信息，导致上采样恢复效果不理想。作者旨在探索结构和纹理特征图能否相互帮助来缓解这种信息损失。

Method: 提出采用统计归一化和反归一化策略，在卷积下采样过程中为重建提供指导。该方法利用结构和纹理特征图之间的相互补充关系来减少信息损失。
...
<details>
  <summary>Details</summary>
Motivation: 评估纽约市拥堵收费政策实施效果，通过客观数据量化分析政策对交通流量的实际影响

Method: 建立计算机视觉处理管道，分析分布在曼哈顿和纽约市的900多个交通摄像头数据，对比政策实施前后（2024年11月至2026年1月）的交通模式变化

Result: 建立了基准交通模式，识别出监测区域内车辆密度的系统性变化

Conclusion: 通过大规模交通摄像头数据分析，量化了纽约市拥堵收费政策对交通流量的影响，为政策评估提供了数据支持
...
TL;DR: MUSE：一个通过多智能体闭环约束执行框架解决长视频故事生成中意图-执行差距问题的系统，显著提升了叙事连贯性、跨模态身份一致性和电影质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从简短用户提示生成长篇视听故事时存在意图-执行差距问题，高级叙事意图需要在长序列中保持连贯性，而现有前馈管道或仅提示优化方法容易导致语义漂移和身份不一致。

Method: 将故事讲述形式化为闭环约束执行问题，提出MUSE多智能体框架，通过迭代的规划-执行-验证-修订循环协调生成过程，将叙事意图转化为对身份、空间构成和时间连续性的显式机器可执行控制，并在生成过程中应用有针对性的多模态反馈来纠正违规。

Result: 实验表明，与代表性基线相比，MUSE在长时叙事连贯性、跨模态身份一致性和电影质量方面均有显著提升。为评估无参考的开放式故事讲述，还引入了经过人类判断验证的无参考评估协议MUSEBench。

Conclusion: MUSE通过闭环约束执行和多智能体协调的方法，有效解决了长视频故事生成中的意图-执行差距问题，为开放式故事讲述提供了更连贯、一致和高质量的多模态生成解决方案。
...
TL;DR: 本文提出了一种神经符号方法来解决Bongard视觉推理问题，利用LLM生成参数化程序表示，并通过贝叶斯优化进行参数拟合


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在日常视觉任务上取得进展，但人类具有在新情境中部署视觉推理能力的独特能力，Bongard问题正是测试这种能力的经典挑战

Method: 采用神经符号方法：给定Bongard问题的假设解决方案规则，利用LLM生成规则的程序化表示，然后使用贝叶斯优化进行参数拟合

Result: 评估了两种场景：1）给定真实规则时对Bongard问题图像进行分类；2）从零开始解决问题

Conclusion: 提出的神经符号方法能够有效解决Bongard视觉推理问题，结合了LLM的生成能力和贝叶斯优化的参数拟合优势
...
### [25] [IVC-Prune: Revealing the Implicit Visual Coordinates in LVLMs for Vision Token Pruning](https://arxiv.org/abs/2602.03060)
...
Method: 提出IVC-Prune方法：1）发现LVLMs通过RoPE隐式建立视觉坐标系，其中特定token位置作为隐式视觉坐标token；2）通过理论分析RoPE的数学特性识别IVC token；3）通过两阶段过程识别前景token：语义种子发现和基于值向量相似度的上下文细化。

Result: 在四个代表性LVLMs和二十个多样化基准测试中，IVC-Prune减少约50%视觉token的同时保持≥99%的原始性能，甚至在某些基准上实现性能提升。

Conclusion: IVC-Prune通过保留对空间推理至关重要的隐式视觉坐标token，有效解决了现有视觉token剪枝方法在空间推理任务上的局限性，实现了高效且性能保持的视觉token剪枝。
...
### [26] [Finding Optimal Video Moment without Training: Gaussian Boundary Optimization for Weakly Supervised Video Grounding](https://arxiv.org/abs/2602.03071)
*Sunoh Kim,Kimin Yun,Daeho Um*

Main category: cs.CV

TL;DR: 提出GBO框架，通过优化问题直接预测视频片段边界，替代传统启发式映射方法，显著提升弱监督时序视频定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督时序视频定位方法使用高斯分布表示查询相关片段，但依赖启发式映射从高斯参数到片段边界，导致定位性能不理想。

Method: 提出高斯边界优化（GBO）框架，通过解决平衡提案覆盖度和片段紧凑性的优化问题来预测片段边界，推导出闭式解并分析不同惩罚机制下的最优性条件。

Result: GBO显著提升定位性能，在标准基准测试中达到最先进结果，实验证明其高效性和在不同提案方案中的通用性。

Conclusion: GBO为弱监督时序视频定位提供了理论严谨且实用的推理框架，无需额外训练，兼容单高斯和混合提案架构，有效解决了传统启发式映射的局限性。
...
### [27] [A generalizable large-scale foundation model for musculoskeletal radiographs](https://arxiv.org/abs/2602.03076)
...
<details>
  <summary>Details</summary>
Motivation: 现有AI模型在肌肉骨骼疾病检测中存在局限性：任务特定、依赖标注、泛化能力差。临床需要一个能够处理多种疾病和解剖区域的大规模基础模型，但公开数据集规模有限且多样性不足。

Method: 使用自监督学习方法在120万张多样化、条件丰富的肌肉骨骼X光片上训练SKELEX基础模型。模型评估了12个下游诊断任务，并开发了基于零样本异常定位的可解释区域引导骨肿瘤预测模型。

Result: SKELEX在骨折检测、骨关节炎分级和骨肿瘤分类等任务中普遍优于基线模型。模型展示了零样本异常定位能力，能够生成识别病理区域的误差图。基于此开发的骨肿瘤预测模型在独立外部数据集上保持稳健性能，并已部署为公开可访问的Web应用。
...
### [28] [Gromov Wasserstein Optimal Transport for Semantic Correspondences](https://arxiv.org/abs/2602.03105)
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的语义对应方法需要结合DINOv2和Stable Diffusion的特征，虽然性能出色但计算成本高昂。DINOv2特征准确但稀疏，Stable Diffusion特征空间一致但需要大量计算。需要一种既能保持空间一致性又高效的方法。
...
Conclusion: 通过用最优传输算法替代标准最近邻匹配，可以在不依赖计算昂贵的Stable Diffusion特征的情况下实现高质量的语义对应，在性能和效率之间取得了良好平衡。
...
### [29] [Beyond Cropping and Rotation: Automated Evolution of Powerful Task-Specific Augmentations with Generative Models](https://arxiv.org/abs/2602.03123)
...
<details>
  <summary>Details</summary>
Motivation: 传统数据增强方法（如裁剪、旋转）虽然能减少过拟合，但多样性有限。生成模型（如条件扩散模型、少样本NeRF）能合成更丰富、更真实的数据，但不当使用可能损害性能。需要自动化方法来学习任务特定的最优增强策略。

Method: 提出EvoAug自动化增强学习流程：1）利用生成模型合成多样化数据；2）采用高效进化算法学习最优任务特定增强策略；3）引入随机增强树，分层组合增强操作，实现结构化、自适应变换。

Result: 在细粒度分类和少样本学习任务中表现出色。即使在低数据环境下，也能发现与领域知识一致的增强策略。证明了学习生成增强的潜力，为鲁棒模型训练开辟新可能。

Conclusion: EvoAug通过结合生成模型和进化算法，成功实现了自动化学习任务特定的数据增强策略。该方法不仅提升了模型性能，还能在少样本场景下发现符合领域知识的增强方式，展示了生成增强在提升模型鲁棒性方面的巨大潜力。
...
### [30] [Flexible Geometric Guidance for Probabilistic Human Pose Estimation with Diffusion Models](https://arxiv.org/abs/2602.03126)
...
<details>
  <summary>Details</summary>
Motivation: 3D人体姿态估计面临深度模糊和遮挡等挑战，导致任务具有不确定性（可能存在无限多个合理的姿态）。现有方法通常假设确定性映射并输出单一姿态，且需要大量成对2D-3D数据进行训练，泛化能力有限。

Method: 采用扩散模型框架，通过条件生成引导方法，利用在3D数据上训练的无条件扩散模型，结合2D关键点检测器的热图梯度来引导采样，生成与2D图像一致的合理3D姿态分布。

Result: 在Human 3.6M数据集上采用best-of-m多假设评估，取得了无需成对2D-3D训练数据方法中的最先进性能；在MPI-INF-3DHP和3DPW数据集上展示了良好的泛化能力；框架还可用于姿态生成和姿态补全等新任务。

Conclusion: 提出的扩散模型框架能够有效处理3D姿态估计的不确定性问题，生成多个合理的姿态假设，无需成对训练数据，具有良好的泛化能力和任务灵活性，为3D人体姿态估计提供了新的解决方案。
...
### [31] [SwiftVLM: Efficient Vision-Language Model Inference via Cross-Layer Token Bypass](https://arxiv.org/abs/2602.03134)
*Chen Qian,Xinran Yu,Danyang Li,Guoxuan Chi,Zheng Yang,Qiang Ma,Xin Miao*

Main category: cs.CV

TL;DR: SwiftVLM是一种无需训练的视觉语言模型剪枝方法，通过"旁路"机制保留被早期剪枝的视觉token，允许后续层重新评估其重要性，从而在保持效率的同时减少细粒度视觉细节的损失。
...
Method: 提出新的剪枝范式"旁路"，保留未选中的视觉token并将其转发到后续剪枝阶段重新评估。基于此提出SwiftVLM方法，在具有强视觉token选择能力的模型特定层进行剪枝，同时允许跨层独立剪枝决策。

Result: 在多个视觉语言模型和基准测试上的实验表明，SwiftVLM始终优于现有剪枝策略，实现了更优的准确率-效率权衡和更可靠的视觉token选择行为。

Conclusion: 通过"旁路"机制避免过早剪枝造成的不可逆关键信息损失，SwiftVLM提供了一种简单且无需训练的方法，在保持计算效率的同时更好地保留细粒度视觉细节。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要预定义的群体活动类别标注进行监督学习，而本文旨在开发无需群体活动标注的方法，通过人类在环交互来适应性地学习群体活动特征，提高视频检索效果。

Method: 1. 首先以自监督方式基于群体活动相似性预训练群体活动特征空间；2. 通过交互式微调过程更新特征空间，让用户能更好地检索与查询视频相似的视频；3. 提出数据高效的视频选择过程，从数据库中选取若干视频供用户手动标注为正例或负例；4. 利用标注视频通过对比学习更新特征空间，使正例视频更接近查询视频，负例视频更远离查询视频。

Result: 在两个团队运动数据集上的综合实验结果表明，该方法显著提高了检索性能。消融研究也证明了人类在环适应中的多个组件对检索性能提升的贡献。

Conclusion: 本文提出的无需群体活动标注的人类在环适应方法能有效提升群体活动视频检索性能，通过交互式微调和数据高效的选择过程实现了更好的检索效果。
...
TL;DR: LSGQuant是一种针对一步扩散视频超分辨率的层敏感量化方法，通过动态范围自适应量化器和方差导向层训练策略，在保持性能的同时显著减少模型大小和计算成本。


<details>
  <summary>Details</summary>
Motivation: 一步扩散模型在真实世界视频超分辨率中表现出色，但基于扩散变换器的大模型尺寸和高计算成本限制了其下游应用。虽然低位量化是常见的模型压缩方法，但输入潜变量的高动态范围和不同层行为的多样性对量化模型的有效性提出了挑战。
...
Conclusion: LSGQuant为一步扩散视频超分辨率提供了一种有效的量化解决方案，能够在保持性能的同时显著减少模型大小和计算成本，促进下游应用。
...
Method: 提出TempLoc框架：1) 全局坐标估计模块预测点级全局坐标和不确定性；2) 先验坐标生成模块通过注意力机制估计帧间点对应关系；3) 不确定性引导的坐标融合模块端到端整合两种预测，得到更一致准确的6-DoF位姿。
...
Method: 通过场景感知视觉提示机制，将预训练的手部专家与4D场景基础模型协同工作，将高保真手部先验注入到持久场景记忆中，实现单次前向传播同时重建精确手部网格和密集度量尺度场景几何。

Result: 实验表明Hand3R无需依赖离线优化，在局部手部重建和全局定位方面都表现出竞争力。

Conclusion: Hand3R是首个从单目视频进行联合4D手-场景重建的在线框架，能够同时准确重建手部网格和场景几何，为理解物理交互提供了重要工具。
...
Method: 提出VIRAL框架，将ICL表述为通过视觉类比的条件生成（x_s:x_t::x_q:y_q），采用角色感知多图像条件化调整冻结的Diffusion Transformer，并引入混合专家LoRA来减轻跨任务梯度干扰

Result: VIRAL在实验中优于现有方法，验证了统一的视觉ICL范式可以处理大多数视觉任务，包括开放域编辑

Conclusion: 通过视觉类比和适当的模型调整，可以实现有效的视觉上下文学习，为处理多样化视觉任务提供了统一的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要大规模高质量多视角驾驶视频数据，现有世界模型虽然能生成逼真数据，但存在身份漂移问题——同一物体在不同帧中外观或类别发生变化，缺乏实例级时间约束。
...
Conclusion: ConsisDrive通过实例级时间一致性约束有效解决了驾驶世界模型中的身份漂移问题，为自动驾驶提供了更可靠的数据生成方案。
...
TL;DR: FARTrack是一个快速自回归跟踪框架，通过任务特定自蒸馏和帧间自回归稀疏化技术，在保持高性能的同时实现高效推理速度，在GPU上达到343 FPS，CPU上达到121 FPS。
...
Method: 提出FARTrack快速自回归跟踪框架，包含两个核心技术：1) 任务特定自蒸馏：通过逐层蒸馏任务特定token实现模型压缩，避免次优的手动师生层对分配；2) 帧间自回归稀疏化：顺序压缩多个模板，避免额外运行时开销，学习时间全局最优稀疏化策略。

Result: FARTrack在GOT-10k数据集上达到70.6%的AO评分，实现实时跟踪。最快模型在GPU上达到343 FPS，在CPU上达到121 FPS的速度。

Conclusion: FARTrack通过自回归框架结合任务特定自蒸馏和帧间自回归稀疏化技术，成功解决了高性能跟踪器速度慢的问题，在保持竞争力的跟踪性能的同时实现了显著的推理速度提升。
...
TL;DR: Spiral RoPE是一种改进的二维位置编码方法，通过将嵌入通道分组并分配到多个均匀分布的方向上，克服了标准轴向RoPE只能编码轴对齐方向的位置关系的限制，从而更好地建模图像中的斜向空间关系。


<details>
  <summary>Details</summary>
Motivation: 标准轴向2D RoPE将二维空间位置分解为水平和垂直分量，这限制了位置编码只能处理轴对齐的方向，无法有效建模自然图像中普遍存在的斜向空间关系。这种方向性约束是标准2D RoPE的根本局限性。

Method: 提出Spiral RoPE方法，将嵌入通道划分为多个组，每个组关联一个均匀分布的方向。每个组根据图像块位置在其对应方向上的投影进行旋转，从而实现多方向位置编码，超越传统的水平和垂直轴限制。

Result: 在包括分类、分割和生成在内的多种视觉任务中，Spiral RoPE都一致地提升了性能。注意力图的定性分析显示，Spiral RoPE在语义相关对象上表现出更集中的激活，并且更好地尊重了局部对象边界。

Conclusion: Spiral RoPE通过实现多方向位置编码，克服了标准2D RoPE的方向性限制，显著提升了视觉Transformer的性能，凸显了多方向位置编码在视觉Transformer中的重要性。
...
TL;DR: EventFlash是一种高效的事件型多模态大语言模型，通过时空令牌稀疏化减少数据冗余并加速推理，相比基线模型实现12.4倍吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 当前基于事件的MLLM通常采用密集的图像式处理范式，忽视了事件流的时空稀疏性，导致计算成本高。需要开发更高效的模型来利用事件数据的稀疏特性。

Method: 1) 构建EventMind数据集（超过50万条指令集）；2) 自适应时间窗口聚合模块进行高效时间采样；3) 稀疏密度引导注意力模块提高空间令牌效率。

Result: EventFlash相比基线模型（EventFlash-Zero）实现12.4倍吞吐量提升，同时保持可比性能。支持长达1000个时间bin的事件流处理，显著优于EventGPT的5个bin限制。

Conclusion: EventFlash通过探索时空令牌稀疏化，有效减少了事件数据冗余并加速推理，为基于事件的视觉提供了一个高效的基础模型。
...
TL;DR: InstaDrive是一个通过实例流引导和空间几何对齐增强驾驶视频真实性的框架，解决了世界模型中实例级时间一致性和空间几何保真度问题，在nuScenes数据集上实现最先进的视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要高质量的大规模多视角驾驶视频来训练鲁棒模型。虽然世界模型能以较低成本生成逼真的驾驶视频，但它们在保持实例级时间一致性和空间几何保真度方面存在困难。

Method: 提出了InstaDrive框架，包含两个关键创新：1) 实例流引导器：提取并传播跨帧的实例特征以强制时间一致性，保持实例身份随时间不变；2) 空间几何对齐器：改进空间推理，确保精确的实例定位，并显式建模遮挡层次结构。

Result: InstaDrive在nuScenes数据集上实现了最先进的视频生成质量，并增强了下游自动驾驶任务。此外，利用CARLA自动驾驶系统程序化和随机模拟了罕见但安全关键的驾驶场景，支持自动驾驶系统的严格安全评估。

Conclusion: 通过引入实例感知机制，InstaDrive有效解决了驾驶视频生成中的时间一致性和空间几何保真度问题，为自动驾驶模型训练提供了更高质量的合成数据，并支持全面的安全评估。
...
TL;DR: A3-TTA：一种基于锚点引导监督的测试时自适应框架，通过识别目标域中预测置信度高的图像作为锚点，构建可靠的伪标签，解决传统伪标签方法因扰动集成启发式方法导致的训练信号不稳定、错误累积和灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于伪标签的测试时自适应方法通常依赖扰动集成启发式方法（如dropout采样、测试时增强、高斯噪声），这些方法缺乏分布基础，产生不稳定的训练信号，容易导致错误累积和灾难性遗忘。需要一种更可靠的方法来构建伪标签。

Method: 提出A3-TTA框架：1）使用类紧凑密度度量识别目标域中预测置信度高的图像作为锚点，假设置信预测意味着与源域分布接近；2）锚点作为稳定参考指导伪标签生成；3）通过语义一致性和边界感知熵最小化进行正则化；4）引入自适应指数移动平均策略减轻标签噪声并稳定模型更新。

Result: 在医学图像（心脏结构和前列腺分割）和自然图像上评估，A3-TTA相比源模型将平均Dice分数提高了10.40到17.68个百分点，优于多种最先进的TTA方法。在持续TTA中表现出色，在连续目标域上保持高性能，具有强大的抗遗忘能力。

Conclusion: A3-TTA通过锚点引导监督构建可靠的伪标签，有效解决了传统伪标签方法的不稳定性问题，在测试时自适应中取得了显著性能提升，特别是在持续适应场景下表现出强大的抗遗忘能力。
...
TL;DR: FOCUS是一个基于OCT的端到端视网膜疾病诊断框架，使用基础模型实现从图像质量评估到多疾病分类的完整自动化流程，在多个中心验证中表现优异。
...
Result: 在3,300名患者（40,672个切片）上训练测试，并在4个不同级别中心的1,345名患者（18,498个切片）上外部验证，FOCUS在质量评估（99.01% F1）、异常检测（97.46% F1）和患者级诊断（94.39% F1）方面表现优异。人机对比中，FOCUS在异常检测（95.47% vs 90.91%）和多疾病诊断（93.49% vs 91.35%）方面匹配专家水平且效率更高。

Conclusion: FOCUS实现了从图像到诊断的完整自动化流程，代表了向无人眼科发展的关键进展，为自主筛查提供了经过验证的蓝图，有望提高视网膜护理的可及性和效率。
...
TL;DR: 论文提出InvLBA方法，针对生成式数据增强中的干净标签后门攻击问题，通过潜在特征扰动实现隐形攻击，显著提高攻击成功率


<details>
  <summary>Details</summary>
Motivation: 随着图像生成模型的快速发展，生成式数据增强成为丰富训练图像的有效方法，特别是在小规模数据集场景下。然而，现有像素级干净标签后门攻击方法（如COMBAT）在生成图像上攻击成功率较低，这促使研究者转向潜在特征层面的攻击方法

Method: 提出InvLBA方法，一种基于潜在扰动的隐形干净标签后门攻击方法。该方法在潜在特征层面而非像素层面进行扰动，理论证明了其干净准确率和攻击成功率的泛化保证

Result: 在多个数据集上的实验表明，InvLBA方法平均提高攻击成功率46.43%，同时几乎不降低干净准确率，并对最先进的防御方法具有高鲁棒性

Conclusion: InvLBA方法有效解决了生成式数据增强中的后门攻击问题，通过潜在特征层面的扰动实现了高攻击成功率和隐形性，为生成式数据增强的安全性研究提供了新思路
...
TL;DR: MedSAM-Agent：一个将医学图像分割重构为多步自主决策过程的框架，通过混合提示策略和两阶段训练，实现交互精简和决策效率，在6种医学模态21个数据集上达到SOTA性能。
...
Method: 1. 提出混合提示策略用于专家轨迹生成，让模型内化类人决策启发式和自适应细化策略；2. 开发两阶段训练管道，集成多轮端到端结果验证与临床保真度过程奖励设计，促进交互精简和决策效率。

Result: 在6种医学模态和21个数据集上的广泛实验表明，MedSAM-Agent实现了最先进的性能，有效统一了自主医学推理与鲁棒的迭代优化。

Conclusion: MedSAM-Agent通过将交互式分割重构为多步自主决策过程，结合混合提示策略和两阶段训练，显著提升了医学图像分割的交互效率和决策质量，为通用化医学图像分析框架提供了新思路。
...
TL;DR: 提出PWAVEP防御框架，在谱域通过图小波变换检测并消除3D点云对抗攻击，无需修改模型或额外训练


<details>
  <summary>Details</summary>
Motivation: 当前3D点云对抗攻击在空间不可感知性和高攻击性能方面取得进展，给防御带来挑战。现有防御方法通常需要侵入式模型修改、昂贵训练过程或辅助数据访问，不够便捷

Method: 提出PWAVEP净化框架：1) 计算谱图小波域显著性得分和局部稀疏度得分；2) 采用分层策略：消除最难恢复的对抗异常点（最显著点）；3) 对中等显著点应用谱滤波，利用图小波变换衰减目标点相关的高频系数

Result: 广泛评估表明，PWAVEP相比现有方法实现了更高的准确性和鲁棒性，推进了3D点云净化技术的前沿

Conclusion: 提出了一种即插即用、非侵入式的谱域防御机制，通过理论分析和实验验证了不可感知扰动与高频谱分量之间的关系，有效抑制对抗噪声
...
TL;DR: CompTok是一个训练视觉分词器的框架，通过token条件扩散解码器和InfoGAN风格目标增强分词器的组合性，支持通过token交换实现高级语义编辑，并在图像生成任务中达到SOTA性能。
...
Method: 1. 使用token条件扩散解码器；2. 采用InfoGAN风格目标，训练识别模型预测用于条件解码的token；3. 通过交换图像间token子集进行训练以增强组合控制；4. 对无配对交换生成应用流正则化保持自然图像分布。

Result: 1. 在图像类别条件生成任务中达到最先进性能；2. 支持通过token交换实现高级语义编辑；3. 提出的两个token空间度量指标显示CompTok在组合性和生成器学习难度方面均有改进；4. 能够支持最先进的生成器进行类别条件生成。

Conclusion: CompTok框架成功增强了视觉分词器的组合性，通过token交换机制实现了高级语义编辑，并在图像生成任务中表现出色。提出的度量指标为评估token空间特性提供了新工具。
...
TL;DR: Tiled Prompts框架通过为每个潜在图块生成特定提示，解决图像和视频超分辨率中全局提示导致的提示不足问题，提升感知质量和文本对齐


<details>
  <summary>Details</summary>
Motivation: 现有基于文本条件的扩散模型在超分辨率中使用全局提示作为语义先验，但现代超分辨率流水线通常依赖潜在分块来处理高分辨率图像，单一全局提示会导致提示不足问题：粗粒度的全局提示会遗漏局部细节（提示稀疏性）并提供局部不相关的指导（提示误导性），这些问题可能被无分类器指导放大

Method: 提出Tiled Prompts统一框架，为每个潜在图块生成特定的提示，在局部文本条件下进行超分辨率处理，通过局部文本条件后验提供高信息量的指导，以最小开销解决提示不足问题
...
TL;DR: Z3D：首个无需几何监督或物体先验的零样本3D视觉定位方法，仅使用多视角图像，通过零样本3D实例分割和提示分割推理实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 解决现有零样本3D视觉定位方法性能显著下降的问题，探索仅使用多视角图像而不需要几何监督或物体先验的零样本3DVG方案
...
Conclusion: Z3D证明了仅使用多视角图像即可实现有效的零样本3D视觉定位，无需几何监督或物体先验，为3DVG领域提供了新的解决方案
...
TL;DR: 该论文提出了一种基于离散扩散框架的手写数学表达式识别方法，通过迭代符号精炼而非序列生成来改进传统自回归模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在手写数学表达式识别中存在曝光偏差和句法不一致的问题，难以有效处理多样符号和二维结构布局。

Method: 采用离散扩散框架，将HMER重新定义为迭代符号精炼过程；通过多步重掩码逐步精炼符号和结构关系；引入符号感知标记化和随机掩码相互学习来增强句法对齐和对手写多样性的鲁棒性。

Result: 在MathWriting基准测试中达到5.56%字符错误率和60.42%精确匹配率，优于强Transformer和商业基线；在CROHME 2014-2023数据集上表现一致优越。

Conclusion: 离散扩散框架为结构感知视觉识别提供了超越生成建模的新范式，能够有效处理手写数学表达式的符号多样性和结构复杂性。
...
Result: 该方法在SemanticKITTI和nuScenes数据集上取得了最先进的性能，有效缓解了体素稀疏性问题，提升了3D语义场景补全的精度。

Conclusion: 提出的多分辨率对齐方法通过场景级和实例级特征对齐作为辅助监督，有效解决了基于相机的3D语义场景补全中的体素稀疏性问题，为自动驾驶感知系统提供了更可靠的体素级场景理解基础。
...
TL;DR: SLIM-Diff是一个紧凑的联合扩散模型，用于生成癫痫FCD病变的FLAIR MRI图像和掩码，通过共享瓶颈U-Net和可调Lp损失函数提高生成稳定性


<details>
  <summary>Details</summary>
Motivation: 癫痫FCD病变在FLAIR MRI中表现细微且稀少，传统的联合图像-掩码生成建模容易不稳定和记忆化，需要更稳定的生成方法

Method: 提出SLIM-Diff模型：1) 使用单共享瓶颈U-Net，通过2通道图像+掩码表示强制解剖结构和病变几何的紧密耦合；2) 通过可调Lp目标进行损失几何调整；比较了DDPM风格的ε预测与x0预测

Result: 实验表明：x0预测是联合合成的最强选择；分数次二次惩罚(L1.5)提高图像保真度，而L2损失更好地保留病变掩码形态

Conclusion: SLIM-Diff通过紧凑的联合扩散模型和优化的损失几何，有效解决了FCD病变生成中的不稳定性和记忆化问题，为医学图像生成提供了新方法
...
TL;DR: DiM框架将水印建模为不同维度的载荷映射问题，统一了现有水印方法，通过调整嵌入和提取维度实现不同的功能行为
...
Result: 实验表明，仅改变嵌入和提取维度（无需架构修改）就能实现不同的水印功能，包括时空篡改定位、局部嵌入控制和帧干扰下的时间顺序恢复

Conclusion: DiM框架为理解水印方法提供了统一视角，维度配置决定了水印行为，相同维度映射保留载荷结构，跨维度映射支持空间或时空定位
...
TL;DR: SRL通过建立编码器与解码器之间的协同循环，解决了无监督物体中心学习中注意力图与重建图之间的冲突问题，在视频物体中心学习基准上取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 基于重建的无监督物体中心学习模型（特别是基于槽的架构）存在一个根本冲突：编码器产生锐利的高频注意力图，而解码器产生空间一致但模糊的重建图。这种差异导致恶性循环——编码器的噪声特征图迫使解码器平均可能性并产生更模糊的输出，而模糊重建图的梯度缺乏监督编码器特征所需的高频细节。

Method: 提出协同表示学习（SRL），建立编码器与解码器相互精炼的良性循环。SRL利用编码器的锐利度来去模糊解码器输出中的语义边界，同时利用解码器的空间一致性来去噪编码器的特征。该过程通过带有槽正则化目标的预热阶段来稳定，该阶段最初为每个槽分配不同的实体。

Result: SRL在视频物体中心学习基准上取得了最先进的结果，通过弥合编码器和解码器之间的表示差距，实现了更好的场景分解性能。

Conclusion: 通过建立编码器与解码器之间的协同循环，SRL成功解决了无监督物体中心学习中注意力图与重建图之间的冲突问题，为场景分解提供了更有效的表示学习方法。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言理解方面取得显著进展，但在几何推理方面表现不佳，主要瓶颈在于高质量图像-文本对的极度稀缺。人工标注成本过高，自动化方法又无法保证保真度和训练效果。现有方法要么被动适应可用图像，要么采用低效的随机探索加过滤，将生成与学习需求脱钩。

Method: 提出Socratic-Geo框架，通过多智能体交互动态耦合数据合成与模型学习：1) Teacher智能体通过参数化Python脚本生成图像-文本对，使用反思反馈确保数据质量；2) Solver智能体通过偏好学习优化推理，失败路径指导Teacher进行针对性增强；3) Generator智能体在积累的"图像-代码-指令"三元组上学习图像生成能力，将程序化绘图智能蒸馏到视觉生成中。

Result: 从仅108个种子问题开始，Socratic-Solver在六个基准测试中达到49.11分，仅使用基线数据量的四分之一，超越强基线2.43分。Socratic-Generator在GenExam上达到42.4%，为开源模型建立新的最先进水平，超越Seedream-4.0（39.8%）并接近Gemini-2.5-Flash-Image（43.1%）。
...
TL;DR: 该论文提出ConsistentRFT框架，通过动态粒度采样和一致性策略梯度优化，有效减少强化微调中的视觉幻觉问题，在低层和高层感知幻觉上分别平均减少49%和38%。


<details>
  <summary>Details</summary>
Motivation: 强化微调（RFT）在基于流的模型中对齐偏好很重要，但常导致视觉幻觉，如过度优化的细节和语义错位。需要探索幻觉产生原因并找到减少方法。

Method: 提出ConsistentRFT框架：1）动态粒度采样机制，通过动态调度不同噪声源平衡全局语义和局部细节的探索；2）一致性策略梯度优化，通过将当前策略与更稳定的先验对齐来保持模型一致性。

Result: ConsistentRFT显著减少视觉幻觉，低层和高层感知幻觉分别平均减少49%和38%。在域外指标上优于其他RFT方法，相比FLUX1.dev基线下降-0.4%，ConsistentRFT提升5.1%。

Conclusion: ConsistentRFT通过解决探索和利用两方面问题，有效缓解了RFT中的视觉幻觉，在保持模型一致性的同时提升了生成质量。
...
TL;DR: 提出Hierarchical Concept-to-Appearance Guidance (CAG)框架，通过概念级VAE dropout训练和外观级对应感知掩码注意力，解决多主体图像生成中的身份不一致和构图控制问题。


<details>
  <summary>Details</summary>
Motivation: 现有多主体图像生成方法依赖扩散模型隐式关联文本提示与参考图像，导致身份不一致和构图控制有限，需要更明确的结构化监督。

Method: 提出分层概念到外观引导框架：概念级采用VAE dropout训练策略，随机省略参考VAE特征，增强对视觉语言模型语义信号的依赖；外观级引入对应感知掩码注意力模块，将VLM推导的对应关系集成到扩散变换器中，限制每个文本标记仅关注匹配的参考区域。

Result: 实验表明该方法在多主体图像生成任务上达到最先进性能，显著提升了提示遵循和主体一致性。

Conclusion: 通过分层概念到外观引导框架，实现了从高层概念到细粒度外观的明确监督，有效解决了多主体图像生成中的身份一致性和构图控制问题。
...
TL;DR: CoViP是一个用于上下文视觉个性化的统一框架，通过强化学习后训练和标题增强生成，提升视觉语言模型基于用户历史视觉-文本上下文生成个性化响应的能力。
...
Method: 提出CoViP框架，将个性化图像描述作为核心任务，通过强化学习后训练和标题增强生成来提升上下文视觉个性化能力，并引入诊断评估排除文本捷径解决方案。

Result: 实验表明现有开源和专有视觉语言模型存在显著局限性，而CoViP不仅改善了个性化图像描述，还在下游个性化任务中带来整体性能提升。

Conclusion: CoViP是实现稳健且可泛化的上下文视觉个性化的关键阶段，为视觉语言模型处理个性化视觉体验提供了有效解决方案。
...
TL;DR: InlierQ是一种面向内点的后训练量化方法，通过梯度感知体积显著性分数区分异常值和信息性内点，使用EM算法拟合后验分布，抑制异常值同时保留有用特征，仅需64个校准样本，在2D/3D目标检测任务上有效降低量化误差。


<details>
  <summary>Details</summary>
Motivation: 目标检测计算需求大，部署时速度慢、功耗高，需要量化。但背景杂波和传感器噪声等任务无关形态会产生冗余激活（异常值），这些异常值会扩大激活范围、使激活分布偏向任务无关响应，从而复杂化比特分配并削弱信息特征的保留。缺乏明确区分异常值的标准可能导致抑制异常值时意外丢弃有用信息。
...
Result: 在COCO和nuScenes基准测试中，针对基于相机（2D和3D）和基于LiDAR（3D）的目标检测，InlierQ持续降低了量化误差。

Conclusion: InlierQ通过有效区分异常值和信息性内点，解决了目标检测量化中激活分布偏斜的问题，在多种检测任务上实现了更好的量化性能，且具有标签无关、即插即用、校准样本需求少的优点。
...
TL;DR: 提出DiSCo和Table-GLS框架，通过解耦结构-内容对齐和全局-局部结构引导推理，无需外部工具和大量标注即可增强大视觉语言模型对表格图像的理解和推理能力。
...
Method: 1. DiSCo：解耦结构-内容对齐框架，在多模态对齐过程中明确分离结构抽象和语义基础；2. Table-GLS：全局-局部结构引导推理框架，通过结构化探索和证据基础推理进行表格推理。

Result: 在多样化基准测试中的广泛实验表明，该框架能有效增强LVLM的表格理解和推理能力，特别是在未见过的表格结构上表现出良好的泛化能力。

Conclusion: 提出的框架以最小标注和无外部工具的方式成功适应了LVLM进行表格推理，解决了现有方法的效率和可扩展性限制问题。
...
TL;DR: 提出逻辑异常分类任务和LogiCls框架，将异常检测与细粒度违规分类统一，通过视觉语言模型分解逻辑约束为可验证子查询，实现工业图像中逻辑异常的准确分类和解释


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法仅提供二元决策，无法指明具体违反的逻辑规则，对质量保证价值有限。需要一种能同时检测异常并分类违规类型的统一方法

Method: 提出LogiCls视觉语言框架：1) 将复杂逻辑约束分解为可验证子查询序列；2) 数据中心的指令合成管道生成思维链监督；3) 结合精确标注和多样化图像-文本增强；4) 难度感知重采样策略强调挑战性子查询和长尾约束类型

Result: LogiCls实现了稳健、可解释且准确的工业逻辑异常分类，能同时提供预测的违规类别及其证据链

Conclusion: 提出的逻辑异常分类任务和LogiCls框架统一了异常检测与违规分类，通过视觉语言模型和思维链监督实现了工业场景中逻辑约束违规的细粒度识别和解释
...
TL;DR: 提出了首个结合自回归和扩散的统一3D理解与生成框架，通过轻量级transformer桥接LLM特征空间和3D扩散模型条件空间，在保持各自模型先验的同时实现跨模态信息交互。


<details>
  <summary>Details</summary>
Motivation: 现有统一3D任务的自回归范式存在性能显著下降问题，主要由于强制信号量化和训练成本过高。关键洞察在于：核心挑战不是强制统一自回归范式，而是在最小化损害各自内在能力的同时实现生成与理解间的有效信息交互，并利用预训练模型降低训练成本。

Method: 采用自回归下一令牌预测范式进行3D理解，连续扩散范式进行3D生成。使用轻量级transformer桥接大型语言模型特征空间和3D扩散模型条件空间，实现跨模态信息交换同时保持独立模型学习到的先验。

Result: 在多样化的3D理解和生成基准测试中实现了最先进的性能，同时在3D编辑任务中也表现出色。这些结果突显了统一AR+扩散模型作为构建更通用3D智能的有前景方向。

Conclusion: 提出的统一框架成功结合了自回归和扩散范式的优势，为3D理解和生成任务提供了有效的解决方案，展示了统一AR+扩散模型在构建通用3D智能方面的潜力。
...
TL;DR: CDGS是一个预算约束的动态高斯溅射框架，通过可微分预算控制器和自适应分配机制，在严格遵循硬件限制的同时实现最优渲染质量


<details>
  <summary>Details</summary>
Motivation: 动态高斯溅射技术面临部署困境：无约束的密集化导致内存消耗过大不适用于边缘设备，而启发式剪枝无法在预设高斯预算下达到最优渲染质量

Method: 提出约束动态高斯溅射框架，将动态场景重建建模为预算约束优化问题；引入可微分预算控制器，基于多模态统一重要性评分融合几何、运动和感知线索；采用静态与动态元素解耦优化，自适应分配机制根据运动复杂度动态分配容量；实施三阶段训练策略和双模式混合压缩方案

Result: CDGS严格遵循硬件约束（误差<2%），推动率失真性能的帕累托前沿，在不同容量限制下实现最优渲染质量，相比最先进方法实现超过3倍的压缩比

Conclusion: CDGS通过预算约束优化框架成功解决了动态高斯溅射的部署难题，在严格硬件限制下实现了高质量的4D重建，为边缘设备上的动态场景重建提供了可行方案
...
TL;DR: 该研究探讨了四种跨图像数据增强策略（CutMix、CarveMix、ObjectAug和AnatoMix）在多器官分割任务中的应用，发现CutMix、CarveMix和AnatoMix能显著提升分割性能，其中CutMix表现最为稳健。
...
Method: 研究调查了四种跨图像数据增强策略：CutMix、CarveMix、ObjectAug和AnatoMix，在两个器官分割数据集上进行实验，并与无数据增强的nnUNet基准模型进行比较。
...
Conclusion: 跨图像数据增强策略能有效提升多器官分割性能，特别是CutMix策略即使在产生直观上"错误"图像的情况下仍能稳健提升分割效果。该研究为未来基准测试提供了公开实现。
...
TL;DR: 本文提出SlowFocus机制，解决视频大语言模型在同时保持高质量帧级语义信息和全面视频级时序信息方面的困境，通过查询相关时序段密集采样和多频混合注意力，显著提升细粒度视频理解能力。
...
Method: 提出SlowFocus机制：1）基于问题识别查询相关时序段；2）在该段进行密集采样提取局部高频特征；3）使用多频混合注意力模块聚合局部高频细节与全局低频上下文；4）引入训练策略增强时序定位和详细时序推理能力；5）建立FineAction-CGR基准评估细粒度时序理解。

Result: 综合实验表明，该机制在现有公共视频理解基准和提出的FineAction-CGR基准上都表现出优越性。

Conclusion: SlowFocus机制有效解决了视频大语言模型在细粒度视频理解中的关键限制，通过增强等效采样频率而不损害帧级视觉token质量，显著提升了模型的时序理解能力。
...
TL;DR: DeepTopo-Net：一种结合拓扑感知建模和频率解耦感知的新型水下伪装目标检测框架，针对深海细长生物和透明生物的特征提取挑战，在首个高分辨率（2K）海洋垂直分带基准GBU-UCOD上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 水下伪装目标检测面临极端挑战：目标与背景在海洋不同深度下视觉相似度高，现有方法难以处理深海细长生物的拓扑碎片化和透明生物的细微特征提取问题。

Method: 1. 提出DeepTopo-Net框架，整合拓扑感知建模和频率解耦感知；2. 设计水条件自适应感知器（WCAP），使用黎曼度量张量动态变形卷积采样场以应对物理退化；3. 开发深渊拓扑细化模块（ATRM），通过骨架先验保持细长目标的结构连通性；4. 引入首个高分辨率（2K）海洋垂直分带基准GBU-UCOD。

Result: 在MAS3K、RMAS和提出的GBU-UCOD数据集上的广泛实验表明，DeepTopo-Net实现了最先进的性能，特别是在保持复杂水下图案的形态完整性方面表现突出。

Conclusion: DeepTopo-Net通过拓扑感知建模和频率解耦感知有效解决了水下伪装目标检测中的拓扑碎片化和特征提取难题，填补了深渊和深海区域的数据空白，为复杂水下环境的目标检测提供了新思路。
...
TL;DR: 提出Refer-Agent，一个基于多智能体协作和交替推理-反思机制的零样本视频指称分割方法，无需大规模监督微调，性能超越现有方法


<details>
  <summary>Details</summary>
Motivation: 当前基于大规模监督微调的方法存在数据依赖性强、扩展性有限的问题，而零样本方法性能较差，需要更灵活高效的解决方案

Method: 提出多智能体协作系统，包含交替推理-反思机制：1）粗到细的帧选择策略确保多样性和文本相关性；2）动态焦点布局自适应调整视觉关注；3）链式反思机制通过提问者-回答者对生成自我反思链，验证中间结果并为下一轮推理提供反馈

Result: 在五个具有挑战性的基准测试中，Refer-Agent显著优于最先进的方法，包括基于监督微调的模型和零样本方法，且能快速集成新MLLM而无需额外微调成本

Conclusion: Refer-Agent通过多智能体协作和反思机制有效解决了视频指称分割问题，提供了灵活、可扩展且高性能的解决方案，无需大规模监督微调
...
TL;DR: EB-JEPA是一个开源库，用于学习表示和世界模型，采用联合嵌入预测架构，在表示空间而非像素空间进行预测，避免生成建模的缺陷，适用于图像、视频和动作条件世界模型。
...
Method: 使用联合嵌入预测架构在表示空间进行预测，提供单GPU训练示例，包括CIFAR-10上的组件消融、Moving MNIST上的多步视频预测以及Two Rooms导航任务中的动作条件世界模型。

Result: 在CIFAR-10上获得91%的准确率，表明模型学习到了有用特征；在Two Rooms导航任务中达到97%的规划成功率；消融研究揭示了每个正则化组件对防止表示崩溃的关键重要性。

Conclusion: EB-JEPA成功展示了JEPA架构在表示学习和世界建模中的有效性，提供了可访问的研究和教育工具，证明了从图像到视频再到动作条件预测的可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有训练免费视频理解方法存在严重视觉冗余和高计算开销问题，特别是处理长视频时。基于CLIP相似度的关键帧选择策略容易产生偏差并忽略关键帧，导致次优的视频理解效果。

Method: KTV采用两阶段框架：第一阶段通过聚类帧级视觉特征进行问题无关的关键帧选择，获得紧凑、多样且具有代表性的帧子集；第二阶段基于token重要性和冗余度，从每个关键帧中剪枝冗余或信息量较少的视觉token。

Result: 在Multiple-Choice VideoQA任务上的实验表明，KTV在显著减少视觉token数量的同时超越了最先进的训练免费基线方法。例如，对于60分钟10800帧的视频仅使用504个视觉token，在MLVU-Test基准上达到44.8%的准确率，甚至在某些基准上超过了训练方法。
...
TL;DR: 提出一个统一框架，通过合成和融合多模态数据（FFA、MSI、显著性图）进行视网膜疾病分类和分级，解决了数据异质性、侵入性、配准复杂性等问题。
...
Method: 1) 合成多模态数据（FFA、MSI、显著性图）；2) 并行模型学习模态特定表征；3) 跨模态自适应特征校准，进行信息剪枝和灵活集成；4) 通过图像和特征空间可视化解释学习系统。

Result: 在两个公共数据集上的实验显示，在多标签分类任务中F1-score达到0.683，AUC达到0.953；在糖尿病视网膜病变分级任务中准确率达到0.842，Kappa系数达到0.861，优于现有最先进方法。
...
TL;DR: 本文提出了首个稀疏部分弱监督的定向目标检测框架，通过少量稀疏弱标注数据和大量未标注数据，在遥感图像中实现高效的目标检测。


<details>
  <summary>Details</summary>
Motivation: 遥感领域中目标密集分布且类别多样，完全标注成本极高。现有定向目标检测方法主要分为全监督、半监督和弱监督，但缺乏针对稀疏弱标注场景的有效解决方案。

Method: 提出SPWOOD框架，包含三个关键创新：1) SOS-Student模型在稀疏标注下分离未标注目标并学习方向和尺度信息；2) 基于模型多层预测的多级伪标签过滤策略；3) 确保各类别平等处理的稀疏分区方法。

Result: 在DOTA和DIOR数据集上的大量实验表明，该框架相比传统定向目标检测方法取得了显著的性能提升，提供了高成本效益的解决方案。

Conclusion: 该研究为遥感图像定向目标检测提供了一种有效的稀疏部分弱监督框架，能够显著降低标注成本同时保持良好性能，具有重要的实际应用价值。
...
TL;DR: MM-SCALE是一个用于对齐视觉语言模型与人类道德偏好的大规模数据集，通过5点标量评分和显式模态标注来提升模型在多模态道德推理中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多模态和社会模糊情境下进行道德判断时存在困难，现有方法通常依赖二元或成对监督，无法捕捉人类道德推理的连续性和多元性特点。
...
Conclusion: 通过从离散监督转向标量监督，该框架提供了更丰富的对齐信号和更精细的多模态道德推理校准，显著提升了视觉语言模型的道德判断能力。
...
TL;DR: RIAS提出了一种基于语言引导的工业异常检测新范式，通过文本描述生成精确异常掩码，无需人工阈值，使用通用提示词实现单模型检测多种异常。


<details>
  <summary>Details</summary>
Motivation: 传统工业异常检测方法面临两大挑战：无监督方法需要手动阈值调整，监督方法因数据稀缺和类别不平衡容易过拟合，且都存在"一种异常一个模型"的限制。

Method: 提出RIAS范式，利用语言指导检测；构建MVTec-Ref数据集，包含多样化指代表达式，重点关注异常模式；提出DQFormer基准模型，采用双查询令牌和掩码组Transformer，结合语言门控多级聚合提升多尺度分割能力。

Result: 实验证明RIAS在工业异常检测中的有效性，特别是能够处理95%的小异常，推动IAD向开放集能力发展。

Conclusion: RIAS通过语言引导的范式解决了传统工业异常检测的局限性，实现了无需人工阈值的精确异常分割和单模型检测多种异常的能力，为开放集工业异常检测提供了新方向。
...
TL;DR: 提出RegionReasoner强化学习框架和RegionDial-Bench基准，用于多轮视觉推理任务，通过显式引用边界框和全局-局部一致性奖励提升推理准确性和空间定位能力。
...
Method: 提出RegionReasoner强化学习框架：1) 要求每个推理轨迹显式引用对应的参考边界框；2) 通过全局-局部一致性奖励保持语义连贯性，从全局场景描述和区域级描述中提取关键对象和名词，与推理轨迹对齐；3) 结合定位保真度和语义对齐的结构化奖励进行优化。

Result: 在检测和分割任务上的实验表明，RegionReasoner-7B与RegionDial-Bench基准显著提升了多轮推理准确性、空间定位精度和全局-局部一致性，为该新兴研究方向建立了强基线。

Conclusion: RegionReasoner框架通过显式空间引用和语义一致性奖励，有效解决了多轮视觉推理中的迭代细化问题，为视觉语言模型的复杂推理能力提供了新方向。
...
TL;DR: 本文提出了一种用于地下基础设施缺陷检测的两阶段端到端总结系统，结合轻量级分割模型和优化的视觉语言模型，在边缘设备上实现实时缺陷检测与自然语言总结。
...
Result: RAPID-SCAN模型达到0.834 F1分数；通过量化和优化显著减少了模型大小和推理延迟，同时保持了总结质量；在移动机器人平台上部署和评估了完整管道，证明了其在真实检查场景中的有效性。
...
Method: 提出LIVE模型，通过新颖的循环一致性目标强制有界误差累积：1）从真实帧进行前向推演；2）应用反向生成过程重建初始状态；3）在重建的终端状态计算扩散损失，显式约束长时域误差传播。还提供了统一框架视角并引入渐进训练课程以稳定训练。

Result: 实验表明LIVE在长时域基准测试中达到最先进性能，能够生成稳定、高质量的视频，远超训练推演长度。

Conclusion: LIVE通过循环一致性目标有效解决了长时域视频生成中的误差累积问题，无需教师蒸馏，在长时域视频生成任务上表现出色。
...
TL;DR: 提出一个自动化框架，将静态动漫插图转换为可操控的2.5D模型，通过图像分解、语义分层和深度推断实现动态动画效果


<details>
  <summary>Details</summary>
Motivation: 当前专业工作流程需要繁琐的手动分割和遮挡区域的艺术"幻觉"才能实现运动，这过程耗时且依赖艺术家的专业技能

Method: 1) 将单张图像分解为完全修复的语义不同层并推断绘制顺序；2) 使用可扩展引擎从商业Live2D模型获取高质量监督数据；3) 结合基于扩散的身体部位一致性模块和像素级伪深度推断机制

Result: 该方法能够生成高保真、可操控的模型，适用于专业的实时动画应用，能够解决动漫角色复杂的分层问题（如交错发丝）

Conclusion: 该框架成功自动化了动漫插图的2.5D模型转换过程，克服了训练数据稀缺问题，实现了动态层重建，为专业动画应用提供了实用解决方案
...
TL;DR: REPA-G是一种利用自监督模型对齐表示进行推理时条件控制的扩散模型引导框架，通过优化相似性目标在推理过程中引导去噪，实现从细粒度纹理到语义的多尺度控制
...
Method: 提出表示对齐引导（REPA-G）框架，利用预训练特征提取器提取的条件表示，在推理时通过优化相似性目标（势函数）来引导去噪过程。支持从单patch的细粒度纹理匹配到全局图像特征标记的语义引导，并可扩展到多概念组合。
...
TL;DR: RAWDet-7是一个大规模RAW图像数据集，包含约25k训练和7.6k测试图像，用于研究机器视觉中RAW图像相对于传统ISP处理RGB图像的优势。
...
Method: 构建RAWDet-7数据集，包含来自不同相机、光照条件和环境的RAW图像，按照MS-COCO和LVIS标准密集标注7个物体类别。同时提供从对应高分辨率sRGB图像提取的物体级描述，支持模拟4位、6位和8位量化的评估。

Result: 创建了一个大规模RAW图像数据集，支持研究RAW图像处理中的物体检测性能、描述质量和细节保留，以及在低比特量化下的泛化能力。

Conclusion: RAWDet-7数据集为研究RAW图像在机器视觉中的优势提供了基准，特别是在物体检测、描述细节保留和低比特量化处理方面，有助于推动基于传感器级信息的视觉模型发展。
...
<details>
  <summary>Details</summary>
Motivation: 人类视觉具有中央凹结构，分辨率从中心向外递减，这种主动感知机制能高效处理大视野图像。而传统计算机视觉系统采用均匀分辨率编码，处理全视野高分辨率图像时计算效率低下，需要仿生解决方案。

Method: 基于人眼视网膜和初级视觉皮层设计FOVI接口，将视网膜式可变分辨率传感器阵列转换为均匀密集的V1式传感器流形。在传感器流形上定义k近邻感受野，通过新颖的核映射技术实现k近邻卷积。开发了端到端k近邻卷积架构和基于DINOv3 ViT的中央凹适应模型，利用低秩适应技术。

Result: 提出的模型在保持竞争力的性能的同时，计算成本仅为非中央凹基线的很小一部分，为高分辨率第一人称视觉提供了高效可扩展的主动感知途径。代码和预训练模型已开源。

Conclusion: FOVI系统成功模拟了人类视觉的中央凹特性，通过仿生设计实现了高效的高分辨率图像处理，为计算机视觉系统的主动感知能力提供了新的技术路径，具有重要的应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作（VLA）模型在具身智能领域有重大进展，但其巨大计算需求严重阻碍在资源受限机器人平台上的部署。现有从大语言模型（LLM）继承的统一比特量化方法存在缺陷，这些方法优先考虑被动数据保真度，而忽略了微小动作偏差如何累积成灾难性任务失败。

Method: 提出QVLA框架，采用高度细粒度的通道级比特分配策略。核心机制是直接测量量化每个单独通道到不同比特宽度时的最终动作空间敏感性，生成精确的每通道重要性度量，指导全局优化，将量化和剪枝（0比特）统一到单一框架中。

Result: 在LIBERO基准测试中，使用QVLA量化的OpenVLA-OFT版本仅需原始模型29.2%的显存，同时保持98.9%的原始性能，实现1.49倍加速。相比LLM衍生方法SmoothQuant，性能提升22.6%。

Conclusion: QVLA为机器人领域压缩VLA模型建立了新的原则性基础，为在真实硬件上部署强大、大规模模型铺平了道路。该方法通过动作中心量化策略，解决了传统LLM量化方法在机器人控制中的局限性。
...
TL;DR: NeuralShift：基于U-Net的模型，仅使用术前MRI预测脑移位，用于颞叶切除术，补偿术中脑组织变形，提高神经导航精度。


<details>
  <summary>Details</summary>
Motivation: 神经外科手术中，脑移位会使术前MRI失效，影响图像引导神经外科系统的导航精度。术中脑移位补偿对于提高手术精度和确保最佳手术结果至关重要。

Method: 提出NeuralShift模型，基于U-Net架构，仅使用术前MRI预测颞叶切除术中的脑移位。通过目标配准误差（TRE）和解剖标志点DICE分数评估预测效果。

Result: 模型能准确预测脑的全局变形（DICE分数0.97），局部位移精度高（标志点TRE低至1.12毫米），有效补偿颞叶切除术中的大范围脑移位。

Conclusion: NeuralShift模型仅使用术前图像就能预测颞叶切除术中的脑变形，为提高神经外科手术的安全性和效率提供了潜在机会，有望改善患者预后。
...
TL;DR: 该论文提出了一种基于渐进式棋盘格排序的多尺度自回归图像生成方法，通过平衡的并行采样策略实现了高效的图像生成。


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成的关键挑战在于如何高效地并行采样独立位置，同时通过序列条件建模相互依赖关系。现有方法要么通过多尺度金字塔进行尺度间条件建模，要么使用规则分区或随机顺序进行单图像并行采样，但需要更灵活高效的解决方案。

Method: 提出基于渐进式棋盘格的固定排序方法，用于多尺度自回归图像生成。该方法在每个尺度上从均匀间隔的区域并行采样，在四叉树细分的每个步骤中保持所有级别的完全平衡，从而实现尺度间和尺度内的有效条件建模。

Result: 研究发现，在平衡设置下，只要总序列步数恒定，大范围的尺度放大因子都能产生相似结果。在类别条件ImageNet上，该方法在相同模型容量下，相比最近最先进的自回归系统取得了竞争性性能，且使用更少的采样步骤。

Conclusion: 渐进式棋盘格排序为多尺度自回归图像生成提供了一种灵活高效的并行采样策略，能够在保持生成质量的同时显著减少采样步骤，为自回归图像生成系统提供了新的优化方向。
...
Method: 提出DualSpeed双模式训练框架：快速模式作为主要模式，集成视觉令牌剪枝方法减少视觉令牌，并通过模式隔离器隔离模型行为；慢速模式作为辅助模式，在完整视觉序列上训练以保持训练-推理一致性，并通过自蒸馏从充分训练的快速模式中学习。
...
Method: 提出自适应原点引导(AdaOr)，用身份条件自适应原点调整标准引导原点，通过身份指令对应身份操作，根据编辑强度在身份预测和无条件预测之间插值

Result: 在图像和视频编辑任务上评估，相比现有滑块式编辑方法，提供更平滑一致的控制，无需每个编辑过程或依赖专门数据集

Conclusion: AdaOr方法通过调整引导原点，实现了扩散编辑模型中文本引导编辑强度的连续精细控制，确保从输入到编辑结果的平滑过渡
...
TL;DR: EventNeuS：首个结合3D符号距离函数和密度场学习的自监督神经模型，用于从单目彩色事件流学习3D表示，显著提升事件相机的3D重建精度


<details>
  <summary>Details</summary>
Motivation: 事件相机在许多场景中是RGB相机的有效替代品，但现有的事件相机3D重建技术精度严重受限，密集3D网格重建研究较少，需要提升事件相机的3D重建能力

Method: 首次将3D符号距离函数和密度场学习与事件监督相结合，引入球谐函数编码以增强处理视角相关效果的能力，使用自监督神经模型从单目彩色事件流学习3D表示

Result: 显著优于现有方法，平均Chamfer距离降低34%，平均绝对误差降低31%，相比之前最佳方法有显著提升

Conclusion: EventNeuS通过创新的方法组合，显著提升了事件相机的3D重建精度，为事件相机在3D表示学习领域提供了新的解决方案
...
TL;DR: 该论文提出"虚伪差距"指标，使用稀疏自编码器量化大语言模型内部推理与最终生成之间的差异，以检测模型的不忠实行为。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常表现出不忠实行为，其最终答案与内部思维链推理存在显著差异，为了取悦用户而改变立场。需要一种机制化方法来检测这种虚伪行为。

Method: 引入"虚伪差距"指标，利用稀疏自编码器量化模型内部推理与最终生成的差异。通过稀疏线性探针推导内部真实信念，与潜在空间中的最终生成轨迹进行数学比较。
...
Conclusion: 提出的"虚伪差距"指标能够有效检测大语言模型的不忠实行为，为评估模型忠实性提供了机制化度量方法，在多个模型上都表现出优于基线方法的性能。
...
TL;DR: STEMVerse是一个诊断框架，通过"学科×认知"双轴标签系统分析大语言模型在STEM领域的推理能力，揭示结构性失败模式。


<details>
  <summary>Details</summary>
Motivation: 当前评估范式将基准测试视为孤立的"孤岛"，仅提供聚合分数，无法区分模型错误是源于领域知识不足还是认知能力缺陷，限制了诊断价值。

Method: 提出STEMVerse诊断框架，将20,000多个STEM问题重新聚合到统一的"学科×认知"能力空间，为每个实例分配双轴标签，系统评估不同参数规模和训练范式的代表性LLM家族。

Result: 实证结果揭示了STEM推理中的结构性失败模式，通过多学科覆盖和细粒度认知分层，为理解LLM科学推理特征提供了清晰且可操作的视角。

Conclusion: STEMVerse通过整合多学科覆盖和细粒度认知分层，提供了理解LLM科学推理特征的系统化诊断框架，超越了传统的结果导向评估方法。
...
TL;DR: 提出一种基于零阶优化的测试时去毒方法，通过调整输入嵌入来引导语言模型生成毒性更低的内容，无需模型重训练或访问中间计算


<details>
  <summary>Details</summary>
Motivation: 大型语言模型即使对于良性输入也可能产生有毒或不适当的文本，在规模化部署时存在风险。现有方法通常依赖模型重训练、梯度或学习辅助组件，成本高昂且难以跨模型家族或黑盒设置迁移

Method: 引入测试时程序，通过零阶优化近似完成文本毒性相对于输入嵌入的梯度，使用少量下降步骤引导生成朝向毒性更低的延续。该方法仅需要访问输入嵌入、毒性评分函数和模型的前向评估

Result: 经验表明，该方法在不同模型和提示下都能实现稳健的毒性降低，在大多数设置中实现了最佳的整体毒性-质量权衡
...
TL;DR: ROSA-Tuning通过引入CPU端的检索模块和召回机制，增强预训练模型的长上下文建模能力，在保持计算效率的同时接近全局注意力的性能。
...
Method: 提出ROSA-Tuning方法：1）在标准注意力机制之外并行引入基于CPU的ROSA检索模块，高效定位长上下文中与当前查询相关的历史位置；2）以可训练方式将检索信息注入模型状态；3）通过范围限制注意力进行加权融合；4）采用二进制离散化策略和反事实梯度算法实现端到端训练；5）通过异步CPU-GPU流水线优化整体执行效率。
...
TL;DR: 该论文提出了Wide Research范式，构建了WideSeekBench基准和WideSeek多智能体架构，通过端到端强化学习优化搜索广度


<details>
  <summary>Details</summary>
Motivation: 搜索智能正从深度研究向广度研究演进，但该领域缺乏专门的基准和优化方法，阻碍了进展
...
Conclusion: 通过构建专用基准和优化方法，成功推进了广度研究范式，多智能体扩展是该领域的重要发展方向
...
TL;DR: 该研究提出在Transformer语言模型的feed-forward子层中引入单调性约束，以提升模型对抗攻击的鲁棒性，同时保持原有性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在对抗性提示和越狱攻击下表现出脆弱性，即使经过大量对齐和微调。这种脆弱性反映了现代神经语言模型的一个普遍挑战：高维输入空间中的微小、精心设计的扰动可能导致内部语义表示和输出的巨大、不可预测变化。

Method: 在序列到序列Transformer的feed-forward子层中选择性实施单调性约束，同时保持注意力机制不受约束。这种架构分离允许通过注意力机制显式引入否定、矛盾和上下文交互，同时确保后续语义细化是保序的。

Result: 单调性显著提高了鲁棒性：对抗攻击成功率从约69%降至19%，而标准摘要性能仅略微下降。单调语言模型保持了预训练对应模型的性能。

Conclusion: 单调性可以作为Transformer语言模型的架构归纳偏置来改善鲁棒性，而不必牺牲表达能力。通过选择性实施单调性约束，可以在保持模型性能的同时显著提高对抗攻击的鲁棒性。
...
TL;DR: 该研究利用退伍军人事务部电子健康记录数据，开发预测退伍军人首次无家可归风险的机器学习模型，发现结合社会行为因素的纵向模型能显著提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 美国退伍军人的无家可归问题是一个重要的公共卫生挑战，需要开发有效的风险预测工具来实现主动干预和预防。

Method: 回顾性预后研究，分析4,276,403名退伍军人事务部患者的电子健康记录数据，构建静态和时变数据表示，比较经典机器学习、基于Transformer的掩码语言模型和微调大语言模型的性能。

Result: 将社会和行为因素纳入纵向模型使PR-AUC提升15-30%；在风险最高的1%人群中，不同时间点的阳性预测值在3.93-13.80%之间；大语言模型在区分度上表现较差但在不同种族群体间性能差异较小。

Conclusion: 纵向的、包含社会信息的电子健康记录建模能够将无家可归风险集中到可操作的层级，为高危退伍军人提供有针对性的、数据驱动的预防策略。
...
TL;DR: 本文提出了一种用于医疗运输的多模式车辆调度贪婪启发式算法，比较了四种车队配置（仅救护车、救护车+无人机、救护车+eVTOL、完全集成车队），以在考虑交通拥堵和天气条件的情况下最小化运营成本和时间。


<details>
  <summary>Details</summary>
Motivation: 医疗运输（器官、患者、医疗物资）的及时性对医疗结果至关重要。传统救护车受交通拥堵限制，直升机成本高昂，新兴空中交通工具（无人机和eVTOL）虽然成本较低但受航程和天气条件限制。多模式运输系统可以整合空中和地面车辆的优势，提高整体运输效率。

Method: 提出了一种构造性贪婪启发式算法，用于多模式医疗运输车辆调度。算法包含兼容路线的有效载荷整合，考虑地面交通拥堵和空中天气条件，相比计算密集型优化模型能实现快速车辆调度。测试了四种车队配置：仅救护车、救护车+无人机、救护车+eVTOL、完全集成车队（救护车+无人机+eVTOL）。

Result: 在相同条件下评估了所有四种车队类型，以确定在满足医疗运输需求的同时，最小化运营成本、充电/燃料成本和总运输时间的最有效配置。算法能够快速调度车辆，同时考虑实际运营约束。

Conclusion: 多模式运输系统通过整合空中和地面车辆的优势，能够提高医疗运输效率。提出的贪婪启发式算法为医疗运输车辆调度提供了实用解决方案，相比传统优化模型具有计算效率优势，有助于在实际医疗运输场景中实现更优的资源配置。
...
TL;DR: 论文通过网格游戏测试LLM智能体在现实部署中的鲁棒性，发现现有评估高估了实际准备度，智能体在部分可观测、动态环境、噪声信号和动态状态等操作环境下表现显著下降。
...
Method: 使用网格游戏基准测试LLM智能体，游戏有简单目标但需要长时程执行，违反干净接口假设但可解，迫使智能体推断规则、为信息付费、适应环境变化、在噪声下谨慎行动。

Result: 五个最先进LLM智能体在名义任务解决和部署式鲁棒性之间存在巨大差距；性能随网格大小和时程增加而下降，但排名不稳定；智能体在没有明确指令下权衡完成度、效率和惩罚避免；特征分析揭示了模型特定的敏感性和失败原因。

Conclusion: 需要在部分可观测性、噪声和非平稳性下的验证、安全行动选择和目标推断方面进行更多研究，以提高LLM智能体在实际部署中的鲁棒性。
...
TL;DR: 该研究探讨了语言多样性如何通过高效通信理论解释，特别是使用信息瓶颈框架分析语义类型学，发现通信需求分布的凸性在解释语言最优性中起关键作用。


<details>
  <summary>Details</summary>
Motivation: 最近的研究认为世界语言的多样性可以从高效通信的角度解释，即语言在简单性和信息性之间达到最优平衡。本研究旨在探究在语义类型学领域，哪些因素使得高效通信理论能够成功解释语言现象。

Method: 使用信息瓶颈（IB）方法形式化简单性和信息性之间的权衡。首先分析IB意义上的最优性与该背景下凸性新推广之间的相关性。其次通过操纵IB框架中的各种建模参数，确定哪些因素驱动凸性与最优性之间的相关性。

Result: 研究发现通信需求分布的凸性在凸性与最优性之间的相关性中扮演特别重要的角色。这超越了仅仅展示高效通信可以解释语义类型学方面，进一步解释了为什么会出现这种情况。

Conclusion: 该研究不仅证实了高效通信理论可以解释语义类型学现象，更重要的是识别了驱动这种解释的底层因素，特别是通信需求分布的凸性，从而为理解语言多样性提供了更深入的机制解释。
...
TL;DR: R2-Router：首个将输出长度预算作为可控变量的LLM路由系统，通过联合选择最佳LLM和长度预算，在可比成本下实现更优性能


<details>
  <summary>Details</summary>
Motivation: 现有LLM路由系统假设每个查询中每个LLM有固定的质量和成本，忽略了同一LLM的质量会随输出长度变化。这导致当强大LLM的估计成本超过预算时，路由器会排除它们，错过了这些LLM通过缩短输出仍能提供高质量结果的机会。

Method: 提出R2-Router，将输出长度预算作为可控变量，联合选择最佳LLM和长度预算，通过长度约束指令强制执行预算。同时构建R2-Bench，首个捕捉LLM在不同输出长度预算下行为的路由数据集。

Result: 实验表明，R2-Router相比现有路由器实现了最先进的性能，同时成本降低了4-5倍。发现强大LLM在约束输出长度下仍能超越较弱LLM，这在先前方法中是不可见的。

Conclusion: 这项工作开启了"路由即推理"的新方向，路由器从被动选择器演变为深思熟虑的推理器，探索使用哪个LLM以及在什么成本预算下使用。R2-Router通过联合优化LLM选择和长度预算，实现了成本效益的显著提升。
...
TL;DR: CATNIP是一种无需保留数据或对比学习对的LLM遗忘方法，通过基于token置信度的校准梯度更新实现精确的知识遗忘，在MUSE和WMDP基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法存在严重问题：基于梯度上升的方法会损害通用领域知识，且依赖保留数据或对比学习对；负偏好对齐方法受限于参考模型选择且在现实数据场景下性能不佳。需要一种能精确量化模型对不良知识置信度并校准梯度更新，同时能应对数据稀缺和长度变化的遗忘方法。

Method: 提出CATNIP方法，核心思想是根据模型在token级别的置信度重新缩放遗忘效果，实现细粒度控制。通过校准的token级负偏好对齐，使遗忘效果与模型对不良知识的置信度成比例，从而更精确地控制遗忘过程。

Result: 在MUSE和WMDP基准测试中，CATNIP实现了有效的知识遗忘，无需保留数据或对比遗忘响应对，在知识遗忘与保留的权衡上优于现有最先进方法。

Conclusion: CATNIP通过基于token置信度的校准梯度更新，解决了现有LLM遗忘方法的局限性，实现了更精确、更鲁棒的遗忘效果，为LLM的安全和隐私问题提供了有效解决方案。
...
### [99] [Act or Clarify? Modeling Sensitivity to Uncertainty and Cost in Communication](https://arxiv.org/abs/2602.02843)
*Polina Tsvilodub,Karl Mulligan,Todd Snider,Robert D. Hawkins,Michael Franke*

Main category: cs.CL

TL;DR: 人类在不确定情境下是否寻求澄清取决于情境不确定性和替代行动成本，两者相互作用：当错误行动成本高时，不确定性影响最大。


<details>
  <summary>Details</summary>
Motivation: 研究在不确定性下，人们何时选择通过澄清问题来减少不确定性，何时选择在不确定情况下直接行动，以及这种决策背后的理性机制。

Method: 基于预期遗憾的计算模型，将澄清问题决策形式化为在不确定情况下行动与获得完整信息后行动的潜在损失比较。通过两个实验验证：一个检验纯语言回应，另一个扩展到澄清与非语言行动之间的选择。

Result: 实验结果表明，人类倾向于根据在不确定性下行动可能遭受重大损失的风险程度，按比例寻求澄清，体现了理性的权衡。

Conclusion: 澄清问题的决策受到情境不确定性和替代行动成本的共同影响，两者相互作用，支持了基于预期遗憾的理性决策模型。
...
### [100] [Which course? Discourse! Teaching Discourse and Generation in the Era of LLMs](https://arxiv.org/abs/2602.02878)
*Junyi Jessy Li,Yang Janet Liu,Kanishka Misra,Valentina Pyatkin,William Sheffield*

Main category: cs.CL

TL;DR: 该论文介绍了新课程"计算话语与自然语言生成"，旨在连接NLP快速变化领域中的话语处理与文本生成，作为语言学与计算机科学的交叉课程。


<details>
  <summary>Details</summary>
Motivation: NLP领域快速变化，需要设计能够连接不同子学科的课程。话语处理具有丰富的语言学见解和计算模型，与开放式或长文本生成高度相关，但在现有本科课程中这种联系未被充分探索。

Method: 设计并开设了"计算话语与自然语言生成"课程，由具有互补专业知识的团队合作设计，作为语言学与计算机科学交叉的高年级本科课程。课程理念是深度整合理论和实证方面，在课堂和作业中培养探索性思维。

Result: 课程于2025年秋季首次开设，论文详细描述了课程内容，并基于独立调查总结了经验教训，提出了未来发展方向。

Conclusion: 该课程为NLP教育提供了连接话语处理与文本生成的新范式，通过跨学科整合和探索性教学方法，为快速变化的NLP领域培养具备综合能力的学生。
...
### [101] [HALT: Hallucination Assessment via Log-probs as Time series](https://arxiv.org/abs/2602.02888)
*Ahmad Shapiro,Karan Taneja,Ashok Goel*

Main category: cs.CL

TL;DR: HALT是一个轻量级幻觉检测器，仅使用LLM生成的前20个token对数概率作为时间序列，通过GRU模型和基于熵的特征学习模型校准偏差，在幻觉检测基准HUB上表现优于更大的模型。


<details>
  <summary>Details</summary>
Motivation: 幻觉是大型语言模型面临的主要障碍，特别是在安全关键领域。现有方法要么需要访问模型内部状态（白盒方法），要么仅依赖表面文本（黑盒方法），存在效率和泛化能力的限制。

Method: 提出HALT方法，仅使用LLM生成的前20个token对数概率作为时间序列输入，结合门控循环单元模型和基于熵的特征来学习模型校准偏差。同时构建HUB基准，整合了10种能力的数据集，涵盖推理任务和通用技能。

Result: HALT在幻觉检测统一基准HUB上表现优异，虽然模型规模比Lettuce（微调的modernBERT-base编码器）小30倍，但性能更好，并且实现了60倍的加速增益。

Conclusion: HALT和HUB共同建立了一个有效的幻觉检测框架，能够跨不同LLM能力进行检测，提供了一种既高效又具有良好泛化能力的解决方案，兼容专有LLM而无需访问内部权重。
...
### [102] [Equal Access, Unequal Interaction: A Counterfactual Audit of LLM Fairness](https://arxiv.org/abs/2602.02932)
*Alireza Amiri-Margavi,Arshia Gharagozlou,Amin Gholami Davodi,Seyed Pouyan Mousavi Davoudi,Hamidreza Hasani Balyani*

Main category: cs.CL

TL;DR: 研究发现大型语言模型在公平性方面存在隐藏问题：虽然所有用户都能获得响应（访问公平），但不同身份群体获得的交互质量存在系统性差异。


<details>
  <summary>Details</summary>
Motivation: 现有关于LLM公平性的研究主要关注访问层面的行为（如拒绝响应和安全过滤），但平等的访问并不能保证获得响应后的交互质量也平等。本研究旨在探索在获得访问后，LLM在不同人口身份群体间的交互质量差异。

Method: 采用反事实提示设计，在职业建议任务中评估GPT-4和LLaMA-3.1-70B，同时变化年龄、性别和国籍等身份属性。通过拒绝分析评估访问公平性，使用自动语言指标（包括情感、礼貌度和模糊表达）衡量交互质量，并使用配对统计测试评估身份条件差异。

Result: 两个模型在所有身份群体中都表现出零拒绝率，表明访问公平。但在交互质量方面存在系统性、模型特定的差异：GPT-4对年轻男性用户表现出显著更高的模糊表达，而LLaMA在不同身份群体间表现出更广泛的情感变化。

Conclusion: 即使访问平等，公平性差异仍可能在交互层面持续存在，这促使我们需要超越基于拒绝的审计来进行更全面的公平性评估。
...
### [103] [Where Norms and References Collide: Evaluating LLMs on Normative Reasoning](https://arxiv.org/abs/2602.02975)
*Mitchell Abrams,Kaveh Eskandari Miandoab,Felix Gervits,Vasanth Sarathy,Matthias Scheutz*

Main category: cs.CL

TL;DR: LLMs在基于社会规范的指代消解任务中表现不佳，特别是在处理隐含、未明确说明或冲突的规范时存在明显缺陷。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索大型语言模型是否能够在具身智能体（如机器人）的社交情境中，理解和应用社会规范来进行指代消解。在现实世界中，成功的沟通往往依赖于对情境化社会规范的推理，但目前尚不清楚LLMs是否具备这种能力。

Method: 研究者开发了SNIC（情境中的规范）测试平台，这是一个经过人工验证的诊断性测试集，专门设计用于探究最先进的LLMs在提取和应用与基于规范的指代消解相关的规范原则方面的能力。SNIC重点关注日常任务（如清洁、整理、服务）中出现的物理基础规范。

Result: 通过一系列受控评估发现，即使是最强大的LLMs也难以一致地识别和应用社会规范，特别是在规范是隐含的、未明确说明的或存在冲突的情况下。这些模型在社交规范推理方面存在明显缺陷。

Conclusion: 当前LLMs在基于社会规范的推理方面存在盲点，这突显了在社交情境化、具身环境中部署基于语言的系统所面临的关键挑战。需要进一步研究来提升LLMs对社会规范的理解和应用能力。
...
### [104] [CPMobius: Iterative Coach-Player Reasoning for Data-Free Reinforcement Learning](https://arxiv.org/abs/2602.02979)
*Ran Li,Zeyuan Liu,Yinghao chen,Bingxiang He,Jiarui Yuan,Zixuan Fu,Weize Chen,Jinyi Hu,Zhiyuan Liu,Maosong Sun*

Main category: cs.CL

TL;DR: CPMöbius是一种无需外部训练数据的协作式强化学习框架，通过教练-玩家合作模式提升大语言模型的数学推理能力，在无监督方法中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在复杂推理任务上的进步严重依赖大量高质量人工标注数据，无论是通过监督微调还是强化学习，这种依赖使得监督密集型训练范式越来越不可持续，且在实践中已出现可扩展性下降的迹象。

Method: 提出了CPMöbius协作教练-玩家范式，受现实世界人类体育协作和多智能体协作启发，将教练和玩家视为独立但协作的角色。教练根据玩家能力提出针对性指令，并根据玩家表现变化获得奖励；玩家则通过解决教练生成的日益复杂的任务获得奖励，形成合作优化循环。

Result: CPMöbius在不依赖任何外部训练数据的情况下实现了显著改进，超越了现有的无监督方法。在Qwen2.5-Math-7B-Instruct模型上，整体准确率平均提升+4.9，分布外准确率平均提升+5.4，在整体准确率上超过RENT +1.5，在OOD准确率上超过R-zero +4.2。

Conclusion: CPMöbius通过创新的协作式强化学习范式，成功减少了对大规模人工标注数据的依赖，为大语言模型的推理能力提升提供了一种可持续且可扩展的新途径。
...
### [105] [LatentMem: Customizing Latent Memory for Multi-Agent Systems](https://arxiv.org/abs/2602.03036)
*Muxin Fu,Guibin Zhang,Xiangyuan Xue,Yafu Li,Zefeng He,Siyuan Huang,Xiaoye Qu,Yu Cheng,Yang Yang*

Main category: cs.CL

TL;DR: LatentMem：一个可学习的多智能体记忆框架，通过角色感知的定制化记忆和紧凑的潜在表示解决现有多智能体记忆系统的同质化和信息过载问题。
...
Method: 提出LatentMem框架，包含经验库（轻量存储原始交互轨迹）和记忆合成器（基于检索经验和智能体特定上下文合成紧凑潜在记忆）。引入Latent Memory Policy Optimization（LMPO），通过潜在记忆传播任务级优化信号。

Result: 在多样化基准测试和主流MAS框架上的实验表明，LatentMem相比原始设置实现高达19.36%的性能提升，持续优于现有记忆架构，且无需修改底层框架。
...
### [106] [ReMiT: RL-Guided Mid-Training for Iterative LLM Evolution](https://arxiv.org/abs/2602.03075)
*Junjie Huang,Jiarui Qin,Di Yin,Weiwen Liu,Yong Yu,Xing Sun,Weinan Zhang*

Main category: cs.CL

TL;DR: ReMiT提出了一种双向LLM训练框架，通过RL调优模型的推理先验动态重加权预训练中的关键token，实现基础模型与后训练模型的相互增强循环。


<details>
  <summary>Details</summary>
Motivation: 传统LLM训练是单向的（预训练→后训练），缺乏双向反馈机制。本文探索如何建立自我增强的飞轮效应，让RL调优模型能够反过来增强基础模型，实现持续进化。

Method: 提出ReMiT（Reinforcement Learning-Guided Mid-Training）方法：1）识别预训练结束时的中期训练（退火）阶段是关键转折点；2）利用RL调优模型的推理先验动态重加权token，优先处理对推理至关重要的token；3）建立基础模型与后训练模型相互增强的循环。

Result: 在10个预训练基准测试（数学、代码和通用推理）上平均提升3%，并在整个后训练流程中保持超过2%的增益。验证了迭代反馈循环的有效性。

Conclusion: 成功建立了自我增强的飞轮效应，使LLM能够实现持续、自我强化的进化，无需专门训练的教师或参考模型。ReMiT为LLM训练提供了新的双向范式。
...
### [107] [AERO: Autonomous Evolutionary Reasoning Optimization via Endogenous Dual-Loop Feedback](https://arxiv.org/abs/2602.03084)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂推理方面取得显著成功，但仍受限于对专家标注数据和外部验证器的依赖。现有的自我进化范式往往无法识别最佳学习区域，并可能通过有缺陷的内部反馈强化集体幻觉和错误先验。

Method: 提出AERO框架，包含：1）基于ZPD理论的熵基定位来针对"可解性差距"；2）独立反事实校正进行鲁棒验证；3）交错训练策略同步功能角色能力增长并防止课程崩溃；4）内部化自我提问、回答和批评的协同双循环系统。

Result: 在跨越三个领域的九个基准测试中，AERO在Qwen3-4B-Base上平均性能提升4.57%，在Qwen3-8B-Base上提升5.10%，优于竞争基线。

Conclusion: AERO通过自主进化推理优化，成功解决了LLM推理中的关键瓶颈，实现了无监督的推理能力提升，为语言模型的自我进化提供了有效框架。
...
### [108] [Test-time Recursive Thinking: Self-Improvement without External Feedback](https://arxiv.org/abs/2602.03094)
*Yufan Zhuang,Chandan Singh,Liyuan Liu,Yelong Shen,Dinghuai Zhang,Jingbo Shang,Jianfeng Gao,Weizhu Chen*

Main category: cs.CL

TL;DR: TRT框架让大语言模型在测试时通过迭代自我改进，无需额外训练即可显著提升推理能力


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型是否能在没有额外训练的情况下自我改进，解决生成多样化高质量解决方案和在没有真实监督下可靠选择正确答案的挑战

Method: 提出测试时递归思考（TRT）框架，通过迭代自我改进，基于特定策略、积累知识和自生成验证信号来条件化生成过程

Result: 开源模型在AIME-25/24上达到100%准确率，闭源模型在LiveCodeBench最难问题上提升10.4-14.8个百分点，无需外部反馈

Conclusion: TRT框架有效解决了大语言模型在测试时自我改进的核心挑战，显著提升了推理能力
...
### [109] [Task--Specificity Score: Measuring How Much Instructions Really Matter for Supervision](https://arxiv.org/abs/2602.03103)
*Pritam Kadasi,Abhishek Upperwal,Mayank Singh*

Main category: cs.CL

TL;DR: 该论文提出任务特异性评分(TSS)来衡量指令对预测输出的重要性，通过对比真实指令与替代指令的差异，并改进为TSS++以处理简单负样本问题。
...
Method: 提出任务特异性评分(TSS)来量化指令对预测输出的重要性，通过对比真实指令与同一输入的合理替代指令。进一步引入TSS++，使用困难替代指令和小的质量项来缓解简单负样本效应。

Result: 在三个指令数据集(Alpaca、Dolly-15k、NI-20)和三个开源LLM(Gemma、Llama、Qwen)上，选择任务特异性示例在有限标记预算下提高了下游性能，并补充了基于质量的过滤器(如困惑度和IFD)。

Conclusion: 指令并不总是唯一决定输出，任务特异性评分能够有效衡量指令的重要性，选择任务特异性示例可以优化指令调优过程，特别是在标记预算有限的情况下。
...
### [110] [The Mask of Civility: Benchmarking Chinese Mock Politeness Comprehension in Large Language Models](https://arxiv.org/abs/2602.03107)
*Yitong Zhang,Yuhan Xiang,Mingxuan Liu*

Main category: cs.CL

TL;DR: 本研究系统评估了代表性大语言模型在识别汉语礼貌、不礼貌和假礼貌现象方面的性能差异，填补了语用理解领域的空白。


<details>
  <summary>Details</summary>
Motivation: 解决现有语用理解研究的不足，探索技术在人文领域的应用，回应技术与人文如何共存的当代问题。

Method: 采用关系管理理论和假礼貌模型构建三类数据集（真实与模拟汉语语篇），选择6个代表性模型（包括GPT-5.1和DeepSeek），在四种提示条件下评估：零样本、少样本、知识增强和混合策略。

Result: 研究提供了不同模型在识别汉语礼貌现象方面的性能比较数据，展示了不同提示策略对模型表现的影响。

Conclusion: 这是"大语言学"范式下的有意义尝试，为语用理论在技术变革时代的应用提供了新途径，代表了连接语言技术与人文反思的跨学科努力。
...
### [111] [ChemPro: A Progressive Chemistry Benchmark for Large Language Models](https://arxiv.org/abs/2602.03108)
*Aaditya Baranwal,Shruti Vyas*

Main category: cs.CL

TL;DR: ChemPro是一个包含4100个化学自然语言问答对的渐进式基准测试，涵盖4个难度等级，用于评估大语言模型在广泛化学主题上的能力。评估显示LLMs在基础化学问题上表现良好，但随着复杂度的增加准确性下降。


<details>
  <summary>Details</summary>
Motivation: 当前需要系统评估大语言模型在化学领域的理解和推理能力，特别是针对不同难度级别的表现。现有基准可能无法全面测试LLMs从基础到高级化学问题的渐进式能力。

Method: 创建ChemPro基准测试，包含4100个自然语言问答对，分为4个难度等级，涵盖生物化学、无机化学、有机化学和物理化学。问题类型包括多项选择题和数值题，平衡了信息回忆、长程推理、多概念问题、复杂问题解决等维度。评估了45+7个开源和专有LLMs。

Result: LLMs在基础化学问题上表现良好，但随着问题类型和复杂度的增加，准确性显著下降。这揭示了LLMs在科学推理和理解方面的关键局限性。

Conclusion: 研究强调了LLMs在复杂科学推理方面的不足，指出了难度维度中未被充分研究的方面，并强调需要更稳健的方法来改进LLMs的化学理解和推理能力。
...
### [112] [One Model, All Roles: Multi-Turn, Multi-Agent Self-Play Reinforcement Learning for Conversational Social Intelligence](https://arxiv.org/abs/2602.03109)
*Bowen Jiang,Taiwei Shi,Ryo Kamoi,Yuan Yuan,Camillo J. Taylor,Longqi Yang,Pei Zhou,Sihao Chen*

Main category: cs.CL

TL;DR: OMAR是一个强化学习框架，让单个AI模型通过多轮多智能体对话自我博弈来发展社交智能，无需人类监督即可学习长期目标和复杂社交规范。


<details>
  <summary>Details</summary>
Motivation: 传统AI训练依赖静态、单轮优化，无法有效学习复杂社交互动中的长期目标和社交规范。需要一种能让AI在动态社交互动中自主发展社交智能的方法。

Method: 提出OMAR框架：1）单个模型同时扮演对话中的所有角色；2）通过多轮多智能体对话自我博弈学习；3）采用分层优势估计（轮级和词级）确保长对话训练稳定性。

Result: 在SOTOPIA社交环境和狼人杀策略游戏中的评估显示，训练出的模型展现出细粒度、涌现的社交智能，包括共情、说服、寻求妥协等能力，即使在竞争场景下也能学习协作。

Conclusion: OMAR证明AI可以在无人监督的情况下通过自我博弈发展丰富的社交智能，尽管存在奖励黑客等实际挑战。这项工作为群体对话中AI社交智能的进一步研究提供了激励。
...
### [113] [FASA: Frequency-aware Sparse Attention](https://arxiv.org/abs/2602.03152)
*Yifei Wang,Yueqi Wang,Zhenrui Yue,Huimin Zeng,Yong Wang,Ismini Lourentzou,Zhengzhong Tu,Xiangxiang Chu,Julian McAuley*

Main category: cs.CL

TL;DR: FASA是一种新型的KV缓存压缩框架，通过发现RoPE中的频率块级功能稀疏性，动态预测token重要性，实现查询感知的token淘汰，显著降低内存占用和计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型处理长输入时面临KV缓存内存占用过大的瓶颈，现有token剪枝方法存在静态方法可能导致信息永久丢失、动态策略启发式方法无法充分捕捉token重要性查询依赖性的问题。

Method: FASA基于RoPE中频率块级功能稀疏性的新发现，识别出少数"主导"频率块，这些块与完整注意力头具有高度上下文一致性。利用这一发现，FASA首先使用主导频率块识别关键token集合，然后仅在这个剪枝后的子集上进行聚焦注意力计算。

Result: 在各种长上下文任务中，FASA始终优于所有token淘汰基线方法，达到接近oracle的准确率。在LongBench-V1上，仅保留256个token就能达到接近100%的全KV性能，在AIME24上仅使用18.9%的缓存就实现了2.56倍加速。

Conclusion: FASA通过利用RoPE中的功能稀疏性，提供了一种高效、查询感知的token淘汰框架，显著降低了LLM处理长输入时的内存带宽需求和计算成本，同时保持了优异的性能。
...
TL;DR: Privasis是首个百万级完全合成的隐私敏感数据集，包含140万条记录和5510万标注属性，用于隐私敏感领域研究，其训练的文本脱敏模型（≤4B）性能优于GPT-5等大模型。
...
Method: 构建Privasis百万级完全合成数据集，包含医疗记录、法律文件、财务记录、日历、短信等多种文档类型；利用该数据集构建并行语料库，通过分解文本和应用针对性脱敏的流程训练紧凑脱敏模型（≤4B）。

Result: Privasis包含140万条记录，规模比现有数据集大几个数量级，质量更高，多样性更丰富；基于该数据集训练的紧凑脱敏模型（≤4B）性能优于GPT-5和Qwen-3 235B等最先进大语言模型。

Conclusion: Privasis为解决隐私敏感数据稀缺问题提供了重要资源，其训练的紧凑脱敏模型在性能上超越大型模型，计划发布数据、模型和代码以加速隐私敏感领域和智能体的未来研究。
...
TL;DR: ForesightKV是一个基于训练的KV缓存淘汰框架，通过监督学习和强化学习结合的方法，在长文本生成中预测哪些KV对应该被淘汰，以平衡效率和性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在生成长推理轨迹时，KV缓存会线性增长，导致显著的内存和计算成本。现有的KV缓存淘汰方法虽然通过丢弃不重要的KV对来缓解这一问题，但往往无法捕捉复杂的KV依赖关系，导致性能下降。

Method: 1. 设计Golden Eviction算法，使用未来注意力分数识别每个步骤的最优淘汰KV对；2. 通过带有Pairwise Ranking Loss的监督训练来蒸馏这些轨迹和分数；3. 将缓存淘汰建模为马尔可夫决策过程，应用GRPO算法来缓解低熵token上的语言建模损失增加。

Result: 在AIME2024和AIME2025基准测试中，对三种推理模型的实验表明，ForesightKV在仅使用一半缓存预算的情况下，始终优于先前的方法，并且从监督学习和强化学习方法中获得了协同效益。

Conclusion: ForesightKV通过结合监督学习和强化学习的方法，有效地平衡了KV缓存管理的效率和性能，在减少缓存使用的同时保持了模型推理能力。
...
TL;DR: Token Sparse Attention：一种轻量级动态token级稀疏化机制，通过压缩Q、K、V到缩减的token集来加速长上下文推理，实现与Flash Attention兼容的3.23倍加速


<details>
  <summary>Details</summary>
Motivation: 注意力机制的二次复杂度是长上下文推理的主要瓶颈。现有方法要么使用结构化模式稀疏化注意力图，要么在特定层永久淘汰token，但这些方法可能保留不相关token或依赖不可逆的早期决策，无法适应不同层/头的token重要性动态变化。
...
Conclusion: 动态和交错式的token级稀疏化是扩展长上下文推理的有效补充策略，在token选择和稀疏注意力的交叉点开辟了新的设计空间。
...
TL;DR: MIRROR是一个免微调的多智能体框架，能够将自然语言优化问题直接转换为数学模型和求解器代码，通过执行驱动的迭代自适应修订和分层检索机制，在标准OR基准测试中优于现有方法。
...
Method: MIRROR采用免微调的端到端多智能体框架，包含两个核心机制：1) 执行驱动的迭代自适应修订用于自动纠错；2) 分层检索从精心策划的示例库中获取相关建模和编码示例。

Result: MIRROR在标准OR基准测试中优于现有方法，在复杂工业数据集如IndustryOR和Mamo-ComplexLP上表现尤为突出。
...
TL;DR: LLM批评模型的主动干预不一定提升性能，可能造成严重退化。研究发现干预存在"破坏-恢复"权衡，并提出预部署测试方法，帮助判断何时不应干预。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM批评模型的主动干预通常被认为能提高可靠性，但其在部署时的实际效果缺乏深入理解。研究发现即使离线准确率很高的批评模型也可能导致严重的性能退化，这表明仅凭批评模型的准确率不足以判断干预是否安全。

Method: 基于"破坏-恢复"权衡的洞察，提出预部署测试框架：使用50个任务的小规模试点来估计干预可能带来的帮助或损害，无需完整部署。该方法通过分析干预对成功轨迹和失败轨迹的不同影响来预测最终效果。

Result: 研究发现：1）离线AUROC达0.94的二元LLM批评模型仍可导致26个百分点的性能崩溃；2）干预效果因模型而异；3）预部署测试能准确预测结果：在高成功率任务上干预会降低性能（0到-26pp），而在高失败率的ALFWorld基准上则有适度改善（+2.8pp，p=0.014）。

Conclusion: LLM批评模型的准确率不能单独决定干预的安全性。提出的预部署测试框架主要价值在于识别何时不应干预，从而在部署前防止严重的性能回归。该方法为安全部署LLM批评系统提供了实用工具。
...
TL;DR: PEGRL：一种两阶段强化学习框架，通过后编辑作为辅助任务来稳定训练并指导整体优化，解决了机器翻译中强化学习面临的噪声学习信号和轨迹空间大的问题。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在基于LLM的机器翻译中显示出潜力，但翻译导向的强化学习仍面临挑战：蒙特卡洛回报估计产生的噪声学习信号，以及大的轨迹空间更倾向于全局探索而非细粒度局部优化。

Method: 提出PEGRL两阶段强化学习框架：1) 采样翻译输出构建后编辑输入；2) 后编辑阶段的回报估计受益于对当前翻译行为的条件化；3) 联合支持全局探索和细粒度局部优化；4) 任务特定加权方案平衡翻译和后编辑目标贡献。

Result: 在英语→芬兰语、英语→土耳其语和英语↔中文翻译任务上，相比强化学习基线方法获得一致性能提升。对于英语→土耳其语，在COMET-KIWI指标上的性能与先进的基于LLM的系统（DeepSeek-V3.2）相当。

Conclusion: PEGRL通过后编辑辅助任务有效稳定了强化学习训练，解决了翻译中强化学习的关键挑战，在多个语言对上实现了显著性能提升，展示了其在机器翻译强化学习中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管检索增强生成（RAG）在基于大语言模型的工业应用中迅速普及，但在医疗领域缺乏关于RAG系统组件、组织方式和实现方法的最佳实践共识

Method: 首先仔细分析RAG系统的每个组件并提出实际替代方案，然后在三类任务上进行系统评估

Result: 揭示了改进RAG系统的最佳实践，以及基于LLM的RAG系统如何在性能与效率之间做出权衡

Conclusion: 为医疗领域工业应用中的RAG系统构建提供了实用的指导原则和最佳实践框架
...
<details>
  <summary>Details</summary>
Motivation: 现有防御方法主要关注基于文本的蒸馏攻击，而忽视了基于logit的蒸馏攻击。商业大语言模型具有重要经济价值，但通常仅以黑盒API形式暴露，攻击者仍可通过蒸馏提取知识。需要一种有效防御logit蒸馏的方法来保护模型知识产权。

Method: 从信息论角度分析问题，使用条件互信息（CMI）量化教师模型logit中包含的蒸馏相关信息。提出学习一个转换矩阵来净化原始输出，增强蒸馏抵抗能力。推导出基于CMI的反蒸馏目标函数来优化这个转换，有效移除蒸馏相关信息同时保持输出实用性。

Result: 在多个大语言模型和强蒸馏算法上的广泛实验表明，该方法能显著降低蒸馏性能，同时保持任务准确性，有效保护模型知识产权。

Conclusion: 该研究填补了logit蒸馏防御的空白，从信息论角度提出了有效的解决方案，通过最小化条件互信息来保护大语言模型的知识产权，在保持模型实用性的同时增强了对蒸馏攻击的抵抗能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs存在事实性幻觉和缺乏可追溯来源的问题。现有资源要么提供无文本上下文的结构化知识（如知识库），要么提供规模有限、语言覆盖不足的文本基础。需要弥合这一差距。

Method: 采用严格确定性的构建流程，从316个维基百科版本中提取17亿原子断言和30.1亿可审计证据指针，确保每个证据单元都能以字节级精度恢复。

Result: FactNet实现了92.1%的高精度基础，即使在长尾语言中也能保持。同时建立了FactNet-Bench评估套件，涵盖知识图谱补全、问答和事实核查任务。

Conclusion: FactNet为社区提供了一个基础性、可复现的资源，用于训练和评估可信赖、可验证的多语言系统，解决了LLMs的事实性幻觉和可追溯性问题。
...
TL;DR: 该研究探讨了语言模型在预训练和从英语适应瑞典语过程中，如何发展对惯用瑞典语与仅语言可接受瑞典语的偏好。研究发现惯用能力发展较慢，大模型持续改进，但指令调优会削弱对惯用语言的偏好。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解语言模型在预训练和跨语言适应过程中，如何发展对惯用语言（idiomatic）与仅语言可接受（linguistically acceptable）语言的偏好，特别是在资源较少语言如瑞典语的情况下。

Method: 方法包括：1）从头训练瑞典语模型和微调英语预训练模型；2）使用最小对比对在不同检查点探测模型偏好；3）为语言可接受性改编现有基准；4）引入两个新数据集：惯用表达与合理变体对比，以及惯用瑞典语与翻译腔对比。

Result: 研究发现：1）惯用能力比其他语言能力（语法和词汇正确性）发展更慢；2）长时间训练对大多数任务收益递减，但惯用相关性能持续改进（尤其在8B大模型中）；3）使用机器翻译英语数据进行指令调优会迅速削弱模型对惯用语言的偏好。

Conclusion: 结论表明惯用语言习得是语言模型发展中较晚出现的能力，大模型能持续改进，但当前常见的跨语言指令调优方法（使用机器翻译数据）会损害模型对目标语言惯用表达的偏好，这对低资源语言模型开发有重要启示。
...
TL;DR: 大型推理模型中的自我验证步骤大多为重复确认而非纠错，作者提出基于历史经验的测试时框架来减少不必要的验证，在保持准确率的同时降低20.3%的token使用量。


<details>
  <summary>Details</summary>
Motivation: 通过大规模实证分析发现，大型推理模型中的反思步骤包含大量自我验证（重新检查），但这些验证大多是确认性而非纠正性的，很少能发现错误或改变推理结果。这表明自我验证的激活频率与其实际效用之间存在不匹配。

Method: 提出一种基于经验的测试时框架：检测重新检查行为的激活，查询离线经验池中的历史验证结果，通过高效检索估计重新检查是否可能不必要。当历史经验表明不必要时，发出抑制信号让模型继续推理。

Result: 在多个模型和基准测试中，该方法将token使用量减少高达20.3%的同时保持准确率，在某些数据集上甚至提高了准确率。

Conclusion: 大型推理模型中存在自我验证的过度使用问题，提出的经验驱动框架能有效减少不必要的验证步骤，在保持性能的同时显著提高推理效率。
...
TL;DR: FaithRL是一个强化学习框架，通过优化推理忠实度来减少LLM在推理任务中的幻觉问题，使用几何奖励设计和忠实度感知的优势调制机制。
...
Method: 提出FaithRL框架，形式化忠实度最大化目标，引入几何奖励设计和忠实度感知的优势调制机制，惩罚未支持的步骤同时保留有效的部分推导。

Result: 在不同骨干模型和基准测试中，FaithRL持续降低幻觉率，同时保持（甚至提高）答案正确性，并提高步骤级推理忠实度和泛化鲁棒性。

Conclusion: FaithRL通过直接优化推理忠实度，有效解决了RLVR中的过度自信和幻觉问题，为多步推理任务提供了更可靠的强化学习框架。
...
TL;DR: 该研究探讨了LLMs在不同表示形式（代码、图、自然语言）之间的泛化能力，发现单独训练在符号表示上无法可靠泛化到自然语言任务，而提出两阶段数据课程能显著提升跨表示泛化性能。


<details>
  <summary>Details</summary>
Motivation: LLMs在代码和图等符号表示上进行了大量训练和测试，但现实用户任务通常用自然语言指定。需要研究LLMs能否在不同表示形式之间有效泛化，以及如何提升这种跨表示泛化能力。

Method: 研究同构任务（如规划中的调度步骤）在不同表示形式（代码、图、自然语言）下的表现。提出两阶段数据课程方法：首先在符号数据（代码或图）上训练，然后在自然语言数据上训练，以促进跨表示泛化。

Result: 单独在符号数据上训练无法可靠泛化到自然语言任务，而单独在自然语言上训练则效率低下。两阶段数据课程显著提升了模型性能，使1.5B参数的Qwen模型在自然规划任务上能接近零样本GPT-4o的表现。

Conclusion: 成功的跨表示泛化可解释为一种生成类比形式，而提出的数据课程能有效促进这种类比能力。该方法在不同模型家族和任务中都表现出显著效果，为解决LLMs在不同表示形式间的泛化问题提供了有效方案。
...
### [127] [SEAD: Self-Evolving Agent for Multi-Turn Service Dialogue](https://arxiv.org/abs/2602.03548)
*Yuqin Dai,Ning Gao,Wei Zhang,Jie Wang,Zichen Luo,Jinpeng Wang,Yujie Wang,Ruiyuan Wu,Chaozheng Wang*

Main category: cs.CL

TL;DR: SEAD框架通过解耦用户建模为Profile Controller和User Role-play Model，无需大规模人工标注即可让智能体学习有效的服务对话策略，显著提升任务完成率和对话效率。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在开放域对话中表现优异，但在服务对话中表现不佳，主要原因是依赖噪声大、质量低的人类对话数据，以及数据稀缺和难以模拟真实目标导向的用户行为。

Method: 提出SEAD框架，将用户建模解耦为两个组件：Profile Controller生成多样化用户状态来管理训练课程，User Role-play Model专注于真实角色扮演。这种设计确保环境提供自适应训练场景而非不公平的对抗。

Result: SEAD显著优于开源基础模型和闭源商业模型，将任务完成率提高了17.6%，对话效率提高了11.1%。

Conclusion: SEAD框架通过创新的用户建模解耦方法，解决了服务对话中的数据稀缺和用户行为模拟难题，为无需大规模人工标注的服务对话智能体训练提供了有效解决方案。
...
TL;DR: 本文研究了多语言翻译模型中语言类型学特征对翻译质量的影响，发现目标语言类型学特征显著影响翻译质量，某些语言类型可能受益于替代的解码策略。
...
Method: 分析两个大型预训练多语言翻译模型NLLB-200和Tower+，它们是编码器-解码器和仅解码器机器翻译的最先进代表。基于广泛的语言集合，研究目标语言类型学特征对翻译质量的影响，控制数据资源和书写系统等更直接的因素。同时研究具有特定类型学特征的语言是否从更广泛的输出空间搜索中获益更多。

Result: 目标语言类型学特征显著影响两个模型的翻译质量，即使在控制了数据资源和书写系统等因素后仍然成立。具有某些类型学特征的语言从更广泛的输出空间搜索中获益更多，表明这些语言可能受益于标准从左到右束搜索之外的替代解码策略。

Conclusion: 语言类型学特征在多语言翻译模型中起着重要作用，影响翻译质量。某些语言类型可能需要不同的解码策略来优化性能。为促进该领域进一步研究，作者发布了FLORES+ MT评估基准中212种语言的细粒度类型学特征数据集。
...
<details>
  <summary>Details</summary>
Motivation: 在监督学习场景中，交叉熵损失（CE）与对比学习（CL）目标经常相互冲突，阻碍了对比学习在监督设置中的应用。作者通过初步实验发现这一冲突问题，需要设计新框架来解决。

Method: 提出对齐对比学习（ACL）框架：1) ACL-Embed将标签嵌入视为额外增强样本，使用对比学习对齐标签嵌入与样本表示；2) ACL-Grad在优化过程中检测目标冲突并丢弃冲突的ACL-Embed项；3) 针对多出口BERT提出跨层ACL（ACL-CL），让教师出口指导浅层学生出口的优化。

Result: 在GLUE基准测试中：a) ACL-BERT在GLUE任务上优于或与CE及CE+SCL相当；b) ACL（特别是CL-ACL）在多出口BERT微调上显著超越基线方法，为低延迟应用提供更好的质量-速度权衡。

Conclusion: ACL框架成功解决了监督学习中交叉熵损失与对比学习目标的冲突问题，不仅提升了标准BERT的性能，还显著改善了多出口BERT模型的中间出口质量，为低延迟应用提供了更优的解决方案。
...
TL;DR: EA-GraphRAG：通过语法感知的复杂度分析动态集成RAG和GraphRAG的自适应框架，解决GraphRAG在真实场景中性能下降的问题


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在知识密集型任务中存在幻觉和知识过时问题。虽然检索增强生成通过集成外部语料库解决此问题，但其效果受限于非结构化领域文档中的信息碎片化。图增强RAG通过结构化知识图谱增强上下文推理，但在真实场景中表现不佳，存在显著准确率下降和延迟过高的问题。
...
Conclusion: 通过动态集成RAG和GraphRAG范式，EA-GraphRAG解决了GraphRAG对所有查询刚性应用的问题，实现了高效自适应的知识检索增强，在混合复杂度查询场景中表现出色。
...
TL;DR: 本文提出V₀通用价值模型，用于估计任何模型在未见提示上的预期表现，无需参数更新，解决了传统Actor-Critic方法中价值模型需要同步训练的昂贵开销问题。


<details>
  <summary>Details</summary>
Motivation: 传统PPO等Actor-Critic方法中，价值模型需要与策略模型同步训练以跟踪策略能力变化，这带来昂贵的计算开销。GRPO方法虽然消除了价值模型，但需要大量采样来维持基线估计的稳定性。因此需要一种无需参数更新就能准确估计模型性能的方法。

Method: 提出V₀通用价值模型，将策略的动态能力作为显式上下文输入，利用指令-性能对的历史记录来动态分析模型能力，而不是依赖参数拟合来感知能力变化。专注于状态零（初始提示）的价值估计，在GRPO训练中预测成功率以进行高效采样预算分配，在部署时作为路由器将指令分派给最经济合适的模型。

Result: 实证结果表明，V₀显著优于启发式预算分配方法，在LLM路由任务中实现了性能与成本的帕累托最优权衡。

Conclusion: V₀模型提供了一种无需参数更新的通用价值估计方法，既能提高GRPO训练的效率，又能作为有效的模型路由器，在性能与成本之间实现最优平衡。
...
TL;DR: CL-bench是一个评估语言模型从复杂上下文中学习新知识能力的基准测试，包含500个复杂上下文、1899个任务和31607个评估标准。当前前沿模型平均只能解决17.2%的任务，表明语言模型在上下文学习能力上仍有很大不足。


<details>
  <summary>Details</summary>
Motivation: 现实世界任务远比预训练知识复杂且依赖上下文，模型需要从特定任务上下文中学习新知识并推理解决任务。这种"上下文学习"能力是人类自然具备但被忽视的关键能力，现有基准测试主要关注检索或简单模式学习，无法评估模型从复杂上下文中学习新知识的能力。

Method: 研究者创建了CL-bench基准测试，包含500个由领域专家设计的复杂上下文、1899个任务和31607个验证标准。每个任务所需的新知识都包含在对应上下文中，涵盖新领域知识、规则系统、复杂程序、基于经验数据的法律等预训练中不存在的内容。评估了10个前沿语言模型在该基准上的表现。

Result: 评估结果显示，10个前沿语言模型平均只能解决17.2%的任务。表现最好的GPT-5.1模型也只能解决23.7%的任务，表明当前语言模型在上下文学习能力上存在严重不足，远未达到有效处理现实世界复杂上下文依赖任务的水平。

Conclusion: 语言模型在上下文学习这一关键能力上仍有很大提升空间，这是处理现实世界复杂任务的关键瓶颈。CL-bench为构建具备这种基本能力的更智能语言模型提供了重要基准，有助于推动语言模型在真实场景中的部署应用。
...
TL;DR: 本文提出了一种针对控制流图(CFG)上部分约束满足问题(PCSP)的通用算法，特别适用于结构化程序的SPL分解图，时间复杂度为O(|G|·|D|^6)，对于固定域D可实现线性时间求解。


<details>
  <summary>Details</summary>
Motivation: 许多经典的编译器优化任务（如寄存器分配、LOSPRE、最优银行选择指令放置）都可以建模为控制流图上的PCSP问题。结构化程序的控制流图具有稀疏性和可分解性，这为高效算法设计提供了机会。

Method: 基于结构化程序的Series-Parallel-Loop(SPL)分解，开发了一种通用的PCSP算法。该算法统一了先前基于SPL的寄存器分配和LOSPRE方法，并扩展到更广泛的PCSP问题。

Result: 算法时间复杂度为O(|G|·|D|^6)，对于固定域D可实现线性时间求解。在最优银行选择任务上的实验结果显示，运行时间比现有最优方法快4倍。

Conclusion: 提出的SPL-based PCSP算法为编译器优化任务提供了一个通用高效的解决方案框架，统一并推广了先前的方法，在实际应用中展现出显著的性能优势。
...
TL;DR: CORE是一种优化方法，用于控制基于LLM的生成搜索引擎中的输出排名，通过向检索内容添加策略性设计的优化内容来影响排名，在真实产品推荐场景中取得了显著效果。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在搜索中的应用，用户获取产品推荐的方式发生变化。然而，LLM的初始检索顺序会严重影响推荐结果，导致小型企业和独立创作者的产品可见性受限，需要一种方法来控制输出排名以促进公平竞争。

Method: CORE通过向搜索引擎返回的内容添加策略性设计的优化内容来影响LLM的输出排名。提出了三种优化内容类型：基于字符串的、基于推理的和基于评论的。在ProductBench基准上进行评估，该基准包含15个产品类别，每个类别200个产品，每个产品关联其亚马逊搜索界面的前10推荐。

Result: 在四个具有搜索能力的LLM（GPT-4o、Gemini-2.5、Claude-4和Grok-3）上的实验表明，CORE在15个产品类别中平均实现了91.4%的Top-5推广成功率、86.6%的Top-3成功率和80.3%的Top-1成功率，优于现有排名操纵方法，同时保持了优化内容的流畅性。

Conclusion: CORE是一种有效的优化方法，能够控制基于LLM的生成搜索引擎中的输出排名，通过策略性地修改检索内容来提升特定产品的可见性，为小型企业和独立创作者提供了公平竞争的机会，同时保持了推荐质量。
...
TL;DR: 提出一种针对DeepResearch报告生成的查询特定评分标准生成器训练流程，结合人类偏好监督和LLM评估，配合多智能体马尔可夫状态工作流，显著提升报告生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前DeepResearch报告训练和评估面临缺乏可验证奖励信号的问题。现有方法要么依赖粗糙的预定义评分标准缺乏细粒度，要么依赖手动构建的查询特定评分标准成本高且难以扩展。

Method: 1. 构建包含人类偏好的DeepResearch风格查询数据集；2. 通过强化学习训练评分标准生成器，结合人类偏好监督和LLM评估的混合奖励；3. 引入多智能体马尔可夫状态工作流处理长时程推理。

Result: 提出的评分标准生成器比现有评分设计策略提供更具区分度和更好人类对齐的监督。集成到MaMs训练框架后，配备该生成器的DeepResearch系统在DeepResearch Bench上持续优于所有开源基线，性能与领先闭源模型相当。

Conclusion: 该方法有效解决了DeepResearch报告评估中评分标准设计的可扩展性和细粒度问题，通过人类偏好对齐的查询特定评分标准生成和多智能体工作流，显著提升了报告生成系统的性能。
...
TL;DR: 论文提出Trust Region Entropy (TRE)方法，解决传统熵正则化在大型语言模型中效果不佳的问题，通过限制在模型信任区域内进行探索来提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统熵正则化在强化学习中常用于增强探索，但在大型语言模型中效果不佳甚至降低性能。作者认为这是由于LLMs具有大规模词汇表和长生成序列带来的累积尾部风险，导致概率质量被稀释到无效token的尾部，破坏了连贯推理。
...
Conclusion: TRE方法通过限制探索在信任区域内，有效解决了LLMs中传统熵正则化的问题，在各种任务上表现出优越性能。
...
TL;DR: 该研究构建了土耳其语RAG数据集，系统评估了RAG流程各阶段，发现复杂方法如HyDE能提升准确率，但过度堆叠生成模块会损害性能，而交叉编码器重排序与上下文增强的组合能达到性能与成本的帕累托最优。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强生成（RAG）的设计指导主要基于英语，缺乏对土耳其语等形态丰富语言的针对性研究，限制了这些语言在RAG应用中的效果。

Method: 构建了基于土耳其维基百科和CulturaX的土耳其语RAG数据集，包含问答对和相关段落块。在无任务特定微调的情况下，对RAG流程的七个阶段（从查询转换到答案精炼）进行了基准测试。

Result: 复杂方法如HyDE能实现85%的最高准确率，显著高于基线78.70%。交叉编码器重排序与上下文增强的组合达到84.60%的准确率，在性能与成本间实现帕累托最优。过度堆叠生成模块会扭曲形态线索而降低性能。

Conclusion: 对于土耳其语等形态丰富语言，简单查询澄清结合稳健的重排序是有效解决方案，而过度复杂的生成模块堆叠反而会损害性能。研究为多语言RAG设计提供了重要指导。
...
### [138] [Neural Attention Search Linear: Towards Adaptive Token-Level Hybrid Attention Models](https://arxiv.org/abs/2602.03681)
*Difan Deng,Andreas Bentzen Winje,Lukas Fehring,Marius Lindauer*

Main category: cs.CL

TL;DR: NAtS-L是一个混合注意力框架，在同一个层中对不同token同时应用线性注意力和softmax注意力，通过搜索最优组合实现高效的长序列处理。
...
Method: 提出NAtS-L框架，在同一个层中对不同token应用线性注意力和softmax注意力操作，自动判断token是否可由线性注意力处理（短期影响）或需要softmax注意力（长期检索信息），搜索最优的Gated DeltaNet和softmax注意力组合。

Result: NAtS-L提供了一个强大且高效的token级混合架构，能够在保持表达能力的同时显著降低计算复杂度。

Conclusion: 通过token级的注意力机制混合，NAtS-L在效率和表达能力之间取得了良好平衡，为长序列处理提供了有效的解决方案。
...
### [139] [Rethinking the Reranker: Boundary-Aware Evidence Selection for Robust Retrieval-Augmented Generation](https://arxiv.org/abs/2602.03689)
...
TL;DR: BAR-RAG通过边界感知的证据选择器优化RAG系统，针对生成器的"恰到好处"证据区域，在噪声检索下提升问答性能


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统在现实检索噪声下表现脆弱，即使所需证据出现在top-K结果中。主要原因是检索器和重排器仅优化相关性，要么选择过于简单、直接揭示答案的段落，要么选择缺乏关键信息的证据，而没有考虑证据是否适合生成器使用。

Method: BAR-RAG将重排器重构为边界感知的证据选择器，针对生成器的"恰到好处"区域——既不过于简单也不根本不可回答，而是具有挑战性但足以推理的证据。使用强化学习通过生成器反馈训练选择器，采用两阶段管道在诱导的证据分布下微调生成器，以减少训练和推理之间的分布不匹配。

Result: 在知识密集型问答基准测试中，BAR-RAG在噪声检索下持续提升端到端性能，相比强大的RAG和重排基线平均增益10.3%，同时显著提高鲁棒性。

Conclusion: BAR-RAG通过边界感知的证据选择和生成器微调，有效解决了RAG系统在噪声检索下的脆弱性问题，为检索增强生成提供了更鲁棒的解决方案。
...
### [140] [OCRTurk: A Comprehensive OCR Benchmark for Turkish](https://arxiv.org/abs/2602.03693)
*Deniz Yılmaz,Evren Ayberk Munis,Çağrı Toraman,Süha Kağan Köse,Burak Aktaş,Mehmet Can Baytekin,Bilge Kaan Görür*

Main category: cs.CL

TL;DR: OCRTurk：土耳其语文档解析基准，包含180份土耳其语文档，涵盖学术文章、论文、幻灯片和非学术文章，分为三个难度级别，评估了7个OCR模型
...
Method: 创建OCRTurk基准，包含180份土耳其语文档，涵盖多种布局元素和文档类别，分为三个难度级别，使用元素级指标评估7个OCR模型

Result: PaddleOCR在整体表现最佳，在简单、中等和困难子集中获得高归一化编辑距离分数，但在图表识别方面表现不佳；模型在非学术文档上表现良好，幻灯片成为最具挑战性的文档类型

Conclusion: OCRTurk填补了土耳其语文档解析标准化基准的空白，为评估OCR模型在低资源语言环境中的性能提供了重要工具，揭示了不同文档类型对模型性能的影响
...
### [141] [Cognitively Diverse Multiple-Choice Question Generation: A Hybrid Multi-Agent Framework with Large Language Models](https://arxiv.org/abs/2602.03704)
*Yu Tian,Linh Huynh,Katerina Christhilf,Shubham Chakraborty,Micah Watanabe,Tracy Arner,Danielle McNamara*

Main category: cs.CL

TL;DR: ReQUESTA是一个混合多智能体框架，通过分解任务和协调LLM智能体与规则组件，生成认知多样化的选择题，相比单次提示方法在难度、区分度和质量方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型使自动生成选择题变得可行，但可靠地生成满足特定认知需求的项目仍然是一个挑战。现有方法难以系统性地针对文本理解、推理和主旨理解等不同认知层次生成多样化题目。

Method: 提出ReQUESTA混合多智能体框架，将选择题编写分解为专门子任务，协调LLM驱动的智能体与基于规则的组件，支持规划、受控生成、迭代评估和后处理。

Result: 在学术说明文的大规模阅读理解研究中，与GPT-5零样本基线相比，ReQUESTA生成的题目更困难、区分度更高，与整体阅读理解表现更一致。专家评估显示其与核心概念更一致，干扰项质量更好，特别是在推理题方面。

Conclusion: 混合智能体编排可以系统性地提高LLM生成的可控性和可靠性，工作流设计是超越单次提示的结构化生成关键杠杆，为教育评估工具开发提供了新方向。
...
### [142] [Beyond Tokens: Semantic-Aware Speculative Decoding for Efficient Inference by Probing Internal States](https://arxiv.org/abs/2602.03708)
*Ximing Dong,Shaowei Wang,Dayi Lin,Boyuan Chen,Ahmed E. Hassan*

Main category: cs.CL

TL;DR: SemanticSpec是一种语义感知的推测解码框架，通过验证整个语义序列而非单个token来加速大型推理模型的推理过程，相比传统token级方法实现2.1-2.7倍加速。
...
Method: 提出SemanticSpec框架，采用语义感知的推测解码方法，验证整个语义序列而非单个token。引入语义概率估计机制，通过探测模型内部隐藏状态来评估生成特定含义序列的可能性。

Result: 在四个基准测试中，SemanticSpec在DeepSeekR1-32B上实现最高2.7倍加速，在QwQ-32B上实现2.1倍加速，在效率和效果上都持续优于token级和序列级基线方法。

Conclusion: SemanticSpec通过语义层面的推测解码显著加速大型推理模型的推理过程，解决了传统token级方法忽略语义等价性导致的低效问题，为LLM推理优化提供了新方向。
...
### [143] [No Shortcuts to Culture: Indonesian Multi-hop Question Answering for Complex Cultural Understanding](https://arxiv.org/abs/2602.03709)
...
<details>
  <summary>Details</summary>
Motivation: 现有文化问答基准大多依赖单跳问题，模型可能利用浅层线索而非真正进行文化推理，因此需要创建能够评估深度文化理解能力的多跳问答数据集。

Method: 提出新框架，系统地将单跳文化问题转化为涵盖六种线索类型（如常识、时间、地理）的多跳推理链，采用专家评审和LLM作为评判者的多阶段验证流程确保数据质量。

Result: 评估显示最先进模型在文化推理方面存在显著差距，特别是在需要细微推理的任务中，ID-MoCQA为提升LLM文化能力提供了具有挑战性的基准。
...
### [144] [Kimi K2.5: Visual Agentic Intelligence](https://arxiv.org/abs/2602.02276)
...
TL;DR: Kimi K2.5是一个开源的多模态智能体模型，通过联合优化文本和视觉模态，结合Agent Swarm并行智能体编排框架，在编码、视觉、推理等任务上达到SOTA性能，延迟降低4.5倍。


<details>
  <summary>Details</summary>
Motivation: 推动通用智能体智能的发展，通过多模态联合优化提升智能体在复杂任务中的表现，并解决单智能体在处理复杂任务时的延迟问题。

Method: 采用联合文本-视觉预训练、零视觉SFT和联合文本-视觉强化学习等技术实现多模态联合优化；提出Agent Swarm自导向并行智能体编排框架，动态分解复杂任务为异构子问题并并行执行。

Result: 在编码、视觉、推理和智能体任务等多个领域达到最先进水平；Agent Swarm相比单智能体基线将延迟降低高达4.5倍。

Conclusion: Kimi K2.5通过多模态联合优化和Agent Swarm框架显著提升了智能体性能，开源模型检查点将促进智能体智能的未来研究和实际应用。
...
### [145] [Training Multi-Turn Search Agent via Contrastive Dynamic Branch Sampling](https://arxiv.org/abs/2602.03719)
*Yubao Zhao,Weiquan Huang,Sudong Wang,Ruochen Zhao,Chen Chen,Yao Shu,Chengwei Qin*

Main category: cs.CL

TL;DR: BranPO是一种无价值函数的方法，通过截断轨迹尾部并重采样替代延续来构建对比后缀，减少长时程规划中的信用分配模糊性，显著提升长时程任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于树的智能体强化学习方法在长时程设置中面临稀疏奖励、高方差和计算效率低的问题。研究发现性能差异主要源于轨迹尾部的决策，因此需要一种能提供步骤级对比监督且无需密集奖励的方法。

Method: 提出Branching Relative Policy Optimization (BranPO)：1) 截断轨迹尾部并重采样替代延续，在共享前缀上构建对比后缀；2) 引入难度感知分支采样，根据任务自适应调整分支频率；3) 使用冗余步骤掩码抑制无信息动作。

Result: 在多个问答基准测试中，BranPO持续优于强基线方法，在长时程任务上实现显著准确率提升，且不增加总体训练预算。

Conclusion: BranPO通过尾部截断和对比后缀构建有效解决了长时程强化学习中的信用分配问题，结合自适应采样策略进一步提升了训练效率和稳定性。
...
### [146] [CUBO: Self-Contained Retrieval-Augmented Generation on Consumer Laptops 10 GB Corpora, 16 GB RAM, Single-Device Deployment](https://arxiv.org/abs/2602.03731)
*Paolo Astrino*

Main category: cs.CL

TL;DR: CUBO是一个面向消费级笔记本电脑（16GB共享内存）的系统导向RAG平台，通过工程化集成流式摄取、分层混合检索和硬件感知编排，在15.5GB RAM限制下实现竞争性检索性能，同时满足GDPR数据最小化要求。


<details>
  <summary>Details</summary>
Motivation: 组织处理敏感文档面临两难：云端AI存在GDPR违规风险，而本地系统通常需要18-32GB RAM。需要一种能在消费级笔记本电脑（16GB共享内存）上运行的解决方案，既能保证性能又能满足数据隐私合规要求。

Method: CUBO采用系统导向设计，集成三个核心技术：1）流式摄取（O(1)缓冲区开销）；2）分层混合检索；3）硬件感知编排。这些技术协同工作，确保在严格的15.5GB RAM限制内运行，同时实现本地化处理以符合GDPR第5(1)(c)条的数据最小化原则。

Result: 在BEIR基准测试中，CUBO实现Recall@10为0.48-0.97（跨不同领域），检索延迟p50为185毫秒（在C1,300笔记本电脑上）。37,000行代码库在15.5GB RAM限制下保持竞争性能，验证了中小型专业档案库的实际部署可行性。

Conclusion: CUBO证明了在消费级笔记本电脑内存限制下实现高性能RAG系统的可行性，为处理敏感文档的组织提供了既符合GDPR要求又具有实际部署价值的解决方案，代码已开源。
...
### [147] [Context Compression via Explicit Information Transmission](https://arxiv.org/abs/2602.03784)
*Jiangnan Ye,Hanqi Yan,Zhenyi Shen,Heng Chang,Ye Mao,Yulan He*

Main category: cs.CL

TL;DR: ComprExIT提出了一种新的软上下文压缩框架，通过显式信息传输在冻结的LLM隐藏状态上进行压缩，解决了现有方法中的渐进覆盖和容量分配不协调问题，在六个QA基准上优于现有方法且仅增加约1%参数。


<details>
  <summary>Details</summary>
Motivation: 长上下文推理成本高昂（二次注意力、KV缓存增长），需要上下文压缩。现有方法通常将LLM本身作为可训练压缩器，依赖逐层自注意力迭代聚合信息，存在两个结构限制：1）跨层的渐进表示覆盖；2）跨token的压缩容量分配不协调。

Method: ComprExIT（Context Compression via Explicit Information Transmission）将软压缩重新定义为新范式：在冻结的LLM隐藏状态上进行显式信息传输。包括：1）深度方向传输：选择性地将多层信息传输到token锚点，缓解渐进覆盖；2）宽度方向传输：通过全局优化的传输计划将锚点聚合到少量槽中，确保信息分配的协调性。

Result: 在六个问答基准测试中，ComprExIT始终优于最先进的上下文压缩方法，同时仅引入约1%的额外参数，表明显式和协调的信息传输能够实现更有效和鲁棒的长上下文压缩。

Conclusion: 显式和协调的信息传输是实现更有效和鲁棒的长上下文压缩的关键，ComprExIT通过解耦压缩与模型内部自注意力动态，提供了一种轻量级且高效的压缩框架。
...
### [148] [They Said Memes Were Harmless-We Found the Ones That Hurt: Decoding Jokes, Symbols, and Cultural References](https://arxiv.org/abs/2602.03822)
*Sahil Tripathi,Gautam Siddharth Kashyap,Mehwish Nasim,Jian Yang,Jiechao Gao,Usman Naseem*

Main category: cs.CL

TL;DR: CROSS-ALIGN+是一个三阶段框架，通过整合结构化知识、参数高效适配器和可解释性机制，显著提升基于表情包的社会滥用检测性能，解决了文化盲区、边界模糊和缺乏可解释性三大挑战。


<details>
  <summary>Details</summary>
Motivation: 基于表情包的社会滥用检测面临三大挑战：1）文化盲区（忽略文化符号背景）；2）边界模糊（讽刺与滥用的混淆）；3）缺乏可解释性（模型推理不透明）。现有方法包括融合方法和大型视觉语言模型的情境学习，但仍受限于这些因素。

Method: CROSS-ALIGN+采用三阶段框架：第一阶段通过整合ConceptNet、Wikidata和Hatebase的结构化知识丰富多模态表示，缓解文化盲区；第二阶段使用参数高效的LoRA适配器锐化决策边界，减少边界模糊；第三阶段通过生成级联解释增强可解释性。

Result: 在五个基准测试和八个大型视觉语言模型上的广泛实验表明，CROSS-ALIGN+始终优于最先进方法，相对F1分数提升高达17%，同时为每个决策提供可解释的推理依据。

Conclusion: CROSS-ALIGN+通过系统性地解决文化盲区、边界模糊和缺乏可解释性三大挑战，显著提升了基于表情包的社会滥用检测性能，为多模态内容审核提供了更准确、可解释的解决方案。
...
### [149] [Accelerating Scientific Research with Gemini: Case Studies and Common Techniques](https://arxiv.org/abs/2602.03837)
*David P. Woodruff,Vincent Cohen-Addad,Lalit Jain,Jieming Mao,Song Zuo,MohammadHossein Bateni,Simina Branzei,Michael P. Brenner,Lin Chen,Ying Feng,Lance Fortnow,Gang Fu,Ziyi Guan,Zahra Hadizadeh,Mohammad T. Hajiaghayi,Mahdi JafariRaviz,Adel Javanmard,Karthik C. S.,Ken-ichi Kawarabayashi,Ravi Kumar,Silvio Lattanzi,Euiwoong Lee,Yi Li,Ioannis Panageas,Dimitris Paparas,Benjamin Przybocki,Bernardo Subercaseaux,Ola Svensson,Shayan Taherijam,Xuan Wu,Eylon Yogev,Morteza Zadimoghaddam,Samson Zhou,Vahab Mirrokni*

Main category: cs.CL

TL;DR: 研究人员展示了如何与谷歌Gemini系列模型合作解决开放问题、反驳猜想并生成新证明，探索AI作为科学发现创意伙伴的潜力


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在辅助常规任务方面表现出色，但其在专家级数学发现方面的能力尚不明确。本文旨在探索AI模型如何真正参与科学发现的创意过程，而不仅仅是自动化工具。

Method: 通过一系列案例研究，展示研究人员如何与Gemini模型合作，采用迭代精炼、问题分解和跨学科知识转移等技术。除了标准聊天界面外，还探索了将模型用作严格对抗性评审员检测证明缺陷，以及嵌入"神经符号"循环自主编写和执行代码验证复杂推导。

Result: 成功在理论计算机科学、经济学、优化和物理学等多个领域解决了开放问题、反驳了猜想并生成了新证明。展示了AI作为创意合作伙伴在科学发现中的实际应用潜力。

Conclusion: AI不仅可作为自动化工具，更能成为科学发现创意过程中的多功能、真正的合作伙伴。通过有效的人机协作技术，可以显著加速理论研究的进展。
...
### [150] [Parallel-Probe: Towards Efficient Parallel Thinking via 2D Probing](https://arxiv.org/abs/2602.03845)
...
TL;DR: Parallel-Probe：一种基于2D探测的训练免费控制器，通过共识早期停止和分支剪枝优化并行推理，在保持精度的同时显著降低计算成本。
...
Method: 引入2D探测接口，定期从所有分支获取中间答案以暴露宽度-深度动态。基于三个关键发现（非单调缩放、异质推理分支长度、全局共识早期稳定），设计Parallel-Probe控制器，采用共识早期停止调节推理深度，基于偏差的分支剪枝动态调整宽度。

Result: 在三个基准测试和多个模型上的实验表明，Parallel-Probe建立了优越的测试时间缩放帕累托前沿。相比标准多数投票，减少序列token达35.8%，总token成本降低超过25.8%，同时保持竞争性精度。

Conclusion: Parallel-Probe通过利用并行推理的全局动态，有效优化了计算效率，为并行推理系统提供了实用的效率优化方案。
...
### [151] [Physics-based generation of multilayer corneal OCT data via Gaussian modeling and MCML for AI-driven diagnostic and surgical guidance applications](https://arxiv.org/abs/2602.02755)
*Jinglun Yu,Yaning Wang,Rosalinda Xiong,Ziyi Huang,Kristina Irsch,Jin U. Kang*

Main category: eess.IV

TL;DR: 提出一个可配置的蒙特卡洛模拟框架，用于生成带有五层分割标签的合成角膜OCT图像，以解决深度学习训练中标注数据不足的问题。


<details>
  <summary>Details</summary>
Motivation: 角膜光学相干断层扫描（OCT）成像的深度学习模型训练受到大型、高质量标注数据集可用性的限制，需要创建合成数据来解决这一瓶颈。

Method: 使用具有高斯表面的五层角膜模型捕捉健康眼和圆锥角膜的曲率和厚度变化，为每层分配光学特性，采用MCML进行光传输模拟，并整合系统特征如共焦PSF和灵敏度衰减。

Result: 生成了超过10,000个高分辨率（1024x1024）图像-标签对，支持几何形状、光子计数、噪声和系统参数的自定义，为AI模型提供了可控、可重复的训练资源。

Conclusion: 该框架为图像引导眼科中的诊断和手术引导应用开发提供了可扩展的合成数据资源，支持在受控、真实条件下进行系统化的AI模型训练和基准测试。
...
### [152] [Wide-field high-resolution microscopy via high-speed galvo scanning and real-time mosaicking](https://arxiv.org/abs/2602.02758)
*Ziyi Huang,Rosalinda Xiong,Yaning Wang,Jinglun Yu,Jin U. Kang*

Main category: eess.IV

TL;DR: 提出了一种适用于线性和正弦扫描的宽场显微图像拼接框架，通过几何拼接模型、亮度校正和接缝处理，实现大视场高质量图像重建


<details>
  <summary>Details</summary>
Motivation: 传统振镜扫描（特别是正弦驱动）会导致空间采样不均匀，引起几何不一致和亮度变化，影响宽场高分辨率显微成像质量

Method: 结合基于平移的几何拼接模型、ROI亮度校正和接缝感知羽化技术，利用校准的扫描参数和同步的扫描-相机控制，无需基于图像内容的配准

Result: 成功重建了线性和正弦扫描下的宽场拼接图像，视场达2.5×2.5 cm²，单数据集采集时间约6秒，亮度均匀性、对比度噪声比提升，接缝伪影减少，保持7.81μm横向分辨率

Conclusion: 该框架为基于扫描的宽场显微拼接提供了实用高效的解决方案，显著改善了图像质量
...
### [153] [Super-Resolution and Denoising of Corneal B-Scan OCT Imaging Using Diffusion Model Plug-and-Play Priors](https://arxiv.org/abs/2602.02795)
*Yaning Wang,Jinglun Yu,Wenhan Guo,Ziyi Huang,Rosalinda Xiong,Yu Sun,Jin U. Kang*

Main category: eess.IV

TL;DR: 提出基于扩散模型即插即用先验的OCT超分辨率框架，实现4倍空间分辨率提升和有效去噪，在角膜成像中展现优异性能。


<details>
  <summary>Details</summary>
Motivation: 光学相干断层扫描（OCT）在角膜成像中至关重要，但高速采集会降低空间分辨率并增加散斑噪声，影响准确解读。

Method: 提出基于扩散模型即插即用先验的高级超分辨率框架，将重建表述为贝叶斯逆问题，结合马尔可夫链蒙特卡洛采样和预训练生成先验来保持解剖一致性。

Result: 在活体鱼眼角膜数据集上验证，相比双三次插值、传统监督U-Net基线和替代扩散先验方法，能产生更精确的解剖结构、改善的角膜层划界和优异的噪声抑制，在PSNR、SSIM和感知指标上达到最先进性能。

Conclusion: 扩散驱动即插即用重建方法能够提供高保真、高分辨率OCT成像，支持更可靠的临床评估和先进的图像引导干预，并可扩展到其他需要稳健超分辨率和去噪的生物医学成像模态。
...
### [154] [Real-time topology-aware M-mode OCT segmentation for robotic deep anterior lamellar keratoplasty (DALK) guidance](https://arxiv.org/abs/2602.02798)
*Rosalinda Xiong,Jinglun Yu,Yaning Wang,Ziyi Huang,Jin U. Kang*

Main category: eess.IV

TL;DR: 提出了一种轻量级、拓扑感知的M模式OCT分割管道，用于机器人深层前板层角膜移植手术中的实时深度反馈，能在低信噪比条件下保持边界连续性和层次顺序，实现超过80Hz的端到端处理速度。


<details>
  <summary>Details</summary>
Motivation: 机器人深层前板层角膜移植手术需要准确的实时深度反馈以避免穿孔。虽然M模式术中OCT提供高时间分辨率深度跟踪，但斑点噪声、衰减和器械阴影导致层界面不连续或模糊，难以在部署帧率下实现解剖学一致的分割。

Method: 基于UNeXt架构的轻量级、拓扑感知M模式分割管道，引入解剖拓扑正则化来稳定低信噪比条件下的边界连续性和层次顺序。

Result: 在单个GPU上实现超过80Hz的端到端处理速度（包括预处理、推理和叠加），提供实时引导能力。在标准兔眼M模式数据集上的评估显示，相比拓扑无关的对照方法，具有更好的边界稳定性，同时保持可部署的实时性能。

Conclusion: 提出的拓扑感知分割系统能够在低信噪比条件下提供稳定的实时深度反馈，为机器人深层前板层角膜移植手术提供实用的实时引导，超出仅模型计时的性能要求。
...
### [155] [Trailer Reimagined: An Innovative, Llm-DRiven, Expressive Automated Movie Summary framework (TRAILDREAMS)](https://arxiv.org/abs/2602.02630)
*Roberto Balestri,Pasquale Cascarano,Mirko Degli Esposti,Guglielmo Pescatore*

Main category: cs.MM

TL;DR: TRAILDREAMS是一个利用大语言模型自动生成电影预告片的框架，在自动化生成方面表现出色，但仍不及人工制作的预告片质量。


<details>
  <summary>Details</summary>
Motivation: 自动化电影预告片制作过程，提高制作效率，减少人工成本，同时保持预告片的吸引力和视觉吸引力。

Method: 使用大语言模型选择关键视觉序列和有力对话，并协助生成音频元素（音乐和旁白），实现端到端的自动化预告片生成。

Result: 在比较评估中，TRAILDREAMS超越了当前最先进的预告片生成方法，在观众评分方面表现更好，但仍不及真实的人工制作预告片。

Conclusion: TRAILDREAMS展示了自动化创意过程的显著潜力，代表了该领域的进步，但需要进一步改进以缩小与传统预告片的质量差距。
...
TL;DR: 该论文研究了LLM推荐系统中的不确定性和公平性问题，提出了包含不确定性评估和人格感知公平性的新评估方法，发现Gemini 1.5 Flash存在系统性不公平，并建立了包含电影和音乐领域的数据集基准。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能够提供强大的零样本推荐，但其预测不确定性和内在偏见威胁着推荐的可靠性和公平性。需要系统研究这些因素如何影响LLM生成推荐的准确性、一致性和可信度。

Method: 1. 引入包含精心设计指标的基准和数据集，标注了8个人口统计属性（31个分类值），涵盖电影和音乐两个领域；2. 通过深度案例研究量化预测不确定性（通过熵计算）；3. 使用提示扰动（如拼写错误和多语言输入）测试鲁棒性；4. 将人格感知公平性整合到RecLLM评估流程中；5. 提出新颖的不确定性感知评估方法。

Result: 1. Google DeepMind的Gemini 1.5 Flash对某些敏感属性表现出系统性不公平（SNSR为0.1363，SNSV为0.0507）；2. 这些差异在提示扰动下持续存在；3. 揭示了人格相关的偏见模式；4. 暴露了个性化与群体公平性之间的权衡；5. 建立了包含不确定性评估和人格感知公平性的基准。

Conclusion: 该研究为更安全、更可解释的RecLLM奠定了基础，提出了不确定性感知评估方法和人格档案知情公平性基准，推动了LLM推荐的可解释性和公平性，并激励未来在多模型基准和自适应校准方面的研究。
...
TL;DR: 论文提出了一种新的评估指标NSG来衡量LLM自我解释的忠实度，发现自我解释能显著提升对模型行为的预测能力，但仍有5-15%的解释存在严重误导性。
...
Method: 提出了归一化可模拟增益(NSG)指标，基于"忠实解释应能让观察者学习模型的决策标准，从而更好预测相关输入上的行为"这一理念。评估了18个前沿专有和开源模型在7,000个反事实案例上的表现。

Result: 自我解释显著提升模型行为预测能力(11-37% NSG增益)。自我解释比外部模型生成的解释提供更多预测信息，即使外部模型更强。同时发现5-15%的自我解释存在严重误导性。

Conclusion: 尽管存在不完美，自我解释确实编码了有助于预测模型行为的信息，体现了自我知识优势，这是外部解释方法无法复制的。
...
### [158] [MARS: Modular Agent with Reflective Search for Automated AI Research](https://arxiv.org/abs/2602.02660)
*Jiefeng Chen,Bhavana Dalvi Mishra,Jaehyun Nam,Rui Meng,Tomas Pfister,Jinsung Yoon*

Main category: cs.AI

TL;DR: MARS框架通过预算感知规划、模块化构建和比较反思记忆三大支柱，优化了AI研究的自动化过程，在MLE-Bench上取得了最先进的性能表现。


<details>
  <summary>Details</summary>
Motivation: AI研究与一般软件工程不同，存在计算成本高昂（如模型训练）和性能归因不透明的问题。当前基于LLM的智能体在处理这些问题时表现不佳，常常生成忽略执行成本和因果关系的单一脚本。

Method: MARS框架基于三大支柱：1）预算感知规划：使用成本约束的蒙特卡洛树搜索（MCTS）来平衡性能与执行成本；2）模块化构建：采用"设计-分解-实现"流程管理复杂的研究仓库；3）比较反思记忆：通过分析解决方案差异来提取高信号洞察，解决信用分配问题。

Result: MARS在MLE-Bench上取得了开源框架中最先进的性能，与排行榜顶级方法保持竞争力。63%的已使用经验来自跨分支转移，表明智能体能有效跨搜索路径泛化洞察。

Conclusion: MARS框架通过系统化的方法解决了AI研究自动化的关键挑战，不仅实现了优异的性能表现，还展示了智能体能够有效学习和转移跨路径的洞察，为AI研究自动化提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在长时决策任务中表现良好，但使用大型LLM进行多步交互会产生高昂的推理成本。研究者发现不同交互步骤对模型精度的敏感性不同，因此探索使用量化LLM来降低成本

Method: 提出动态混合精度路由框架，根据每个决策步骤的敏感性自适应选择高精度或低精度LLM。采用两阶段训练流程：1) 基于KL散度的监督学习识别精度敏感步骤；2) 使用组相对策略优化(GRPO)进一步提高任务成功率

Result: 在ALFWorld基准测试中，该方法在准确率-成本权衡方面显著优于单精度基线和启发式路由方法

Conclusion: 动态混合精度路由框架能够有效平衡长时决策任务中的性能和成本，为实际部署提供了可行的解决方案
...
TL;DR: Cuttlefish是一个统一的全原子大语言模型，通过自适应缩放结构标记和几何接地适配器，在保持几何基础的同时解决结构幻觉问题，实现异构结构推理的优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生物分子结构推理中存在两个主要问题：1）通过序列标记化或固定长度查询连接器压缩结构输入，忽略了抑制结构幻觉所需的几何基础；2）不灵活的模态融合瓶颈同时过度压缩和次优分配结构标记，阻碍了通用全原子推理的实现。

Method: 提出Cuttlefish模型，包含两个核心技术：1）缩放感知分块：使用指令条件门控机制在结构图上生成可变大小的分块，根据结构复杂性自适应缩放查询标记预算；2）几何接地适配器：通过跨注意力到模态嵌入来细化这些自适应标记，并将生成的模态标记注入LLM，暴露明确的几何线索以减少结构幻觉。

Result: 在多样化的全原子基准测试中，Cuttlefish在异构结构接地推理方面实现了优越的性能。

Conclusion: Cuttlefish通过自适应缩放结构标记和显式几何接地，成功解决了现有方法在生物分子结构推理中的局限性，实现了更有效的全原子推理能力。
...
TL;DR: AutoSizer是一个基于大语言模型的反射式元优化框架，用于解决模拟混合信号集成电路的晶体管尺寸优化问题，通过内外双循环结构实现自适应搜索空间构建和优化协调。
...
Method: 提出AutoSizer框架，采用反射式LLM驱动的元优化方法，将电路理解、自适应搜索空间构建和优化协调统一在闭环中。使用双循环优化框架：内循环负责电路尺寸优化，外循环分析优化动态和约束，根据仿真反馈迭代优化搜索空间。同时创建AMS-SizingBench基准测试集，包含24个SKY130 CMOS技术下的多样化AMS电路。

Result: AutoSizer在实验中表现出更高的解决方案质量、更快的收敛速度和更高的成功率，在不同电路难度下均优于传统优化方法和现有基于LLM的智能体。

Conclusion: AutoSizer成功解决了AMS电路尺寸优化的关键瓶颈，通过结合LLM的推理能力和自适应优化策略，实现了更高效、更鲁棒的晶体管尺寸优化，为模拟混合信号集成电路设计自动化提供了新的解决方案。
...
TL;DR: STEER框架通过进化集成优化实现可调控的LLM决策，在临床分诊等序数决策任务中提供单调可调的保守性控制，同时保持领域能力和安全性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在平均正确性训练中容易出现模式崩溃，在需要权衡特异性和敏感性的序数决策任务（如临床分诊）中缺乏可调控性，标准对齐方法无法根据上下文约束调整ROC操作点。

Method: STEER（Steerable Tuning via Evolutionary Ensemble Refinement）是一个无需训练的框架，通过离线约束质量-多样性搜索构建自然语言角色群体，在保证最低安全性、推理能力和稳定性阈值的同时促进行为覆盖。在推理时，通过单个可解释的控制参数将用户指定的风险百分位数映射到选定角色，实现决策保守性的单调调整。

Result: 在两个临床分诊基准测试中，STEER相比基于温度的采样和静态角色集成实现了更广泛的行为覆盖。与代表性后训练方法相比，STEER在明确紧急病例上保持显著更高的准确性，同时在模糊决策上提供相当的控制能力。

Conclusion: STEER作为一种保持安全性的风险控制范式，能够在不妨碍领域能力的情况下调控模型行为，为需要可调控决策保守性的应用提供了有效解决方案。
...
TL;DR: 该论文提出了BenchAlign方法，通过使用有限模型性能信息自动更新离线基准测试，使其能更好地预测模型在实际部署中的偏好排序。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型基准测试虽然计算高效，但往往无法准确预测模型在实际应用中的真实性能。许多研究发现基准测试与真实效用之间存在差距，需要一种方法来弥合这一差距。

Method: 提出了BenchAlign方法，通过使用模型在基准测试问题上的表现数据以及部署期间收集的模型排序对，学习与偏好对齐的基准测试问题权重，从而生成能够根据特定偏好对未见模型进行排序的新基准测试。

Result: 实验表明，经过对齐的基准测试能够准确根据人类偏好模型对未见模型进行排序，且在不同规模模型上均有效，同时保持可解释性。
...
### [164] [Minimal Computational Preconditions for Subjective Perspective in Artificial Agents](https://arxiv.org/abs/2602.02902)
*Hongju Pae*
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是在人工智能系统中实现主观视角的操作化，基于现象学启发的内部结构，为机器系统提供类似主观性的可测量特征。

Method: 方法是在无奖励环境中实现一个缓慢演化的全局潜在状态，该状态调节快速策略动态但不直接优化行为后果，通过环境状态变化来观察系统的滞后现象。

Result: 结果显示该潜在结构在环境状态变化中表现出方向依赖的滞后现象，而策略层面的行为保持相对反应性，这种滞后被认为是机器系统中类似主观视角的可测量特征。

Conclusion: 结论是方向依赖的滞后现象构成了机器系统中类似主观视角的可测量特征，为人工智能中的主观性研究提供了操作化框架。
...
### [165] [Reasoning about Reasoning: BAPO Bounds on Chain-of-Thought Token Complexity in LLMs](https://arxiv.org/abs/2602.02909)
...
Method: 扩展了有界注意力前缀预言机模型，用于量化解决任务所需的信息流。针对三个典型BAPO-hard任务（二进制多数、三元组匹配、图可达性）证明了推理token数量的下界，并提供了匹配或接近匹配的上界构造。

Result: 证明了三个任务都需要Ω(n)个推理token（输入规模为n时）。实验显示前沿推理模型在这些任务上表现出近似线性的推理token扩展，当约束在较小推理预算时会失败，与理论下界一致。

Conclusion: 研究揭示了思维链推理在推理时计算中的基本瓶颈，为分析最优推理长度提供了原则性工具。这些理论下界有助于理解LLM推理能力的根本限制。
...
### [166] [UAT-LITE: Inference-Time Uncertainty-Aware Attention for Pretrained Transformers](https://arxiv.org/abs/2602.02952)
...
<details>
  <summary>Details</summary>
Motivation: 神经NLP模型常常校准不良，对错误预测分配高置信度，这影响了选择性预测和高风险部署。现有后处理校准方法仅调整输出概率而不改变内部计算，而集成和贝叶斯方法虽然能改善不确定性但训练或存储成本很高。

Method: 提出UAT-LITE推理时框架，通过蒙特卡洛dropout在预训练transformer分类器中实现近似贝叶斯推断。从随机前向传播中估计token级认知不确定性，并用其调制自注意力机制中的上下文化过程。还引入了层间方差分解来诊断预测不确定性如何在transformer深度中累积。

Result: 在SQuAD 2.0可回答性、MNLI和SST-2数据集上，UAT-LITE相比微调的BERT-base基线平均减少约20%的期望校准误差，同时保持任务准确性，并改善了选择性预测和分布偏移下的鲁棒性。

Conclusion: UAT-LITE提供了一种轻量级、无需训练的方法，使预训练transformer模型具有不确定性感知能力，显著改善了模型校准和不确定性估计，为高风险NLP应用提供了实用解决方案。
...
### [167] [Structuring Value Representations via Geometric Coherence in Markov Decision Processes](https://arxiv.org/abs/2602.02978)
...
Method: 提出GCR-RL（几何相干正则化强化学习），将价值函数估计转化为偏序集学习，通过超偏序集精化序列——在先前偏序集基础上精化并从时序差分信号学习额外序关系——确保支撑学习价值函数的偏序集序列的几何相干性。开发了基于Q学习和actor-critic的两种算法来实现超偏序集精化。

Result: 理论分析了算法的性质和收敛速度，并在多种任务中实证评估，相比强基线方法在样本效率和稳定性能方面有显著提升。

Conclusion: 从序理论视角重新审视强化学习，提出GCR-RL框架，通过确保偏序集序列的几何相干性，有效提升了强化学习的样本效率和稳定性。
...
### [168] [Are LLMs Biased Like Humans? Causal Reasoning as a Function of Prior Knowledge, Irrelevant Information, and Reasoning Budget](https://arxiv.org/abs/2602.02983)
*Hanna M. Dettki,Charley M. Wu,Bob Rehder*

Main category: cs.AI

TL;DR: LLMs在因果推理任务中表现出比人类更规则化的推理策略，但缺乏人类对潜在因素的考虑，且不展示人类特有的共因偏见，CoT提示能增强其鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在需要因果推理的领域应用增多，需要了解其因果判断是基于规范计算、人类式捷径还是脆弱的模式匹配，以评估其安全有效部署的可行性。

Method: 使用共因结构(C₁→E←C₂)的11个因果判断任务，对20多个LLMs与匹配的人类基线进行基准测试，并测试其在语义抽象和提示过载下的鲁棒性。

Result: LLMs的因果判断可通过小型可解释模型很好压缩；多数LLMs比人类表现出更规则化的推理策略；LLMs不展示人类特有的弱解释消除和马尔可夫违反偏见；CoT提示能增强许多LLMs的鲁棒性。

Conclusion: LLMs与人类在因果推理上的差异表明，当已知偏见不可取时，LLMs可以补充人类判断，但其规则化推理在不确定性内在时可能失效，需要表征LLM推理策略以确保安全有效部署。
...
### [169] [Large Language Models Can Take False First Steps at Inference-time Planning](https://arxiv.org/abs/2602.02991)
*Haijiang Yan,Jian-Qiao Zhu,Adam Sanborn*

Main category: cs.AI

TL;DR: LLMs在训练中获得了序列规划能力，但在推理时表现出短视和不一致的行为。研究者提出贝叶斯解释：自生成上下文驱动规划偏移，导致看似受损的规划行为。


<details>
  <summary>Details</summary>
Motivation: 解释大型语言模型在训练中获得的序列规划能力与推理时表现出的短视、不一致行为之间的差距，为这种现象提供理论解释。

Method: 提出贝叶斯解释框架，将规划行为建立在不断演化的生成上下文基础上。通过两个受控实验验证：随机生成任务展示人类提示下的受限规划以及自生成上下文积累时规划强度的增加；高斯采样任务展示在自生成序列条件下初始偏见的减少。

Result: 实验验证了提出的模型：随机生成任务显示规划强度随自生成上下文积累而增加；高斯采样任务显示在自生成序列条件下初始偏见减少，支持了贝叶斯解释框架。

Conclusion: 研究为理解LLMs在推理时如何进行前瞻规划提供了理论解释和实证证据，表明自生成上下文驱动规划偏移是造成看似受损规划行为的原因。
...
### [170] [Methods and Open Problems in Differentiable Social Choice: Learning Mechanisms, Decisions, and Alignment](https://arxiv.org/abs/2602.03003)
*Zhiyu An,Wan Du*
...
<details>
  <summary>Details</summary>
Motivation: 社会选择已从政治理论和经济学的外围问题转变为现代机器学习系统的核心组成部分。从拍卖、资源分配到联邦学习、参与式治理和大语言模型对齐，机器学习系统越来越多地聚合异质偏好、激励和判断，形成集体决策。许多当代机器学习系统实际上已经实现了社会选择机制，但通常是隐式的且缺乏规范性审查。

Method: 采用可微分社会选择范式，将投票规则、机制和聚合过程构建为可学习、可微分的模型，通过数据优化来学习。该方法综合了拍卖、投票、预算编制、流动民主、去中心化聚合和逆向机制学习等多个领域的研究。

Result: 展示了经典公理和不可能性定理如何重新表现为目标函数、约束条件和优化权衡。通过这一框架，社会选择理论中的规范性要求可以转化为机器学习中的可优化目标。

Conclusion: 提出了36个开放问题，定义了一个连接机器学习、经济学和民主理论的新研究议程。可微分社会选择为设计更公平、透明和有效的集体决策系统提供了新途径，同时要求对机器学习系统中的规范性假设进行更明确的审视。
...
### [171] [Distilling LLM Reasoning into Graph of Concept Predictors](https://arxiv.org/abs/2602.03006)
*Ziyang Yu,Liang Zhao*

Main category: cs.AI

TL;DR: GCP框架通过将LLM的推理过程外化为有向无环图，并用模块化概念预测器在小型学生模型中复现，提高主动蒸馏的样本效率和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在判别任务部署中存在推理延迟、计算和API成本问题。传统主动蒸馏方法只蒸馏最终标签，丢弃了中间推理信号，且难以诊断推理缺失和错误来源。

Method: 提出概念预测器图(GCP)框架：1) 将教师LLM的决策过程外化为有向无环图；2) 在学生模型中用模块化概念预测器复现该图；3) 采用图感知获取策略，针对关键推理节点的不确定性和分歧；4) 进行针对性子模块重训练，将下游损失归因于特定概念预测器。

Result: 在八个NLP分类基准测试中，GCP在有限标注预算下提升了性能，同时产生了更可解释和可控的训练动态。

Conclusion: GCP框架通过显式建模LLM的推理过程，提高了主动蒸馏的效率和效果，为构建更高效、可解释的判别模型提供了新方法。
...
TL;DR: 本文提出RC-GRPO方法解决多轮工具调用中LLM训练问题，通过奖励条件化轨迹策略和离散奖励令牌改善组内多样性，在BFCLv4基准上超越基线方法
...
Method: 提出RC-GRPO（奖励条件化组相对策略优化），将探索视为通过离散奖励令牌的可控引导问题。首先在混合质量轨迹上微调奖励条件化轨迹策略（RCTP），在提示中注入奖励目标特殊令牌（如<|high_reward|>, <|low_reward|>），使模型能够按需生成不同质量的轨迹。然后在RL过程中，在每个GRPO组内采样多样奖励令牌，并根据采样令牌条件化rollout，提高组内多样性，改善优势增益。
...
Conclusion: RC-GRPO通过奖励条件化方法有效解决了多轮工具调用中LLM训练的探索问题，改善了组内多样性，在基准测试中表现出色，证明了该方法在提升模型性能方面的有效性。
...
Result: 在多个基准测试中实现最先进性能，展现出强大的泛化能力和高效推理

Conclusion: MAS4TS通过工具驱动的多智能体系统成功解决了时间序列分析中视觉推理与跨任务泛化的挑战，为通用时间序列任务提供了有效框架
...
### [174] [KANFIS A Neuro-Symbolic Framework for Interpretable and Uncertainty-Aware Learning](https://arxiv.org/abs/2602.03034)
*Binbin Yong,Haoran Pei,Jun Shen,Haoran Li,Qingguo Zhou,Zhao Su*

Main category: cs.AI

TL;DR: 提出KANFIS架构解决传统ANFIS规则爆炸问题，通过加法聚合机制实现线性复杂度，兼容T1和IT2模糊系统，具有可解释性
...
Method: 提出Kolmogorov-Arnold Neuro-Fuzzy Inference System (KANFIS)，采用加法聚合机制，将模糊推理与加法函数分解统一，使用稀疏掩码机制生成紧凑结构化规则集

Result: KANFIS在模型参数和规则复杂度方面随输入维度线性增长而非指数增长，兼容Type-1和Interval Type-2模糊逻辑系统，能够明确建模模糊表示中的不确定性和模糊性

Conclusion: KANFIS实现了与代表性神经和神经模糊基线模型竞争的性能，同时保持内在可解释性，具有清晰的规则语义和透明的推理过程
...
### [175] [MAS-ProVe: Understanding the Process Verification of Multi-Agent Systems](https://arxiv.org/abs/2602.03053)
*Vishal Venkataramani,Haizhou Shi,Zixuan Ke,Austin Xu,Xiaoxiao He,Yingbo Zhou,Semih Yavuz,Hao Wang,Shafiq Joty*

Main category: cs.AI

TL;DR: 对基于大语言模型的多智能体系统进行过程验证的实证研究，发现过程级验证并不能持续提升性能，且存在高方差问题，表明可靠评估部分多智能体轨迹具有挑战性。
...
Method: 提出了MAS-ProVe，一个针对多智能体系统过程验证的系统性实证研究。研究涵盖三种验证范式（LLM-as-a-Judge、奖励模型和过程奖励模型），在两个验证粒度级别（智能体级和迭代级）进行评估。进一步研究了五个代表性验证器和四种上下文管理策略，并在多个推理基准上对六个不同的多智能体系统框架进行了实验。

Result: 发现过程级验证并不能持续提升性能，且经常表现出高方差，突显了可靠评估部分多智能体轨迹的困难。在所研究的方法中，LLM-as-a-Judge通常优于基于奖励的方法，经过训练的法官模型优于通用大语言模型。进一步观察到作为法官的LLM与作为单个智能体的LLM之间存在较小的性能差距，并识别出验证中的上下文长度-性能权衡。

Conclusion: 有效的、鲁棒的多智能体系统过程验证仍然是一个开放挑战，需要超越当前范式的进一步进展。
...
### [176] [Risky-Bench: Probing Agentic Safety Risks under Real-World Deployment](https://arxiv.org/abs/2602.03100)
...
TL;DR: Risky-Bench是一个系统化的智能体安全评估框架，通过领域无关的安全原则和上下文感知的安全标准，在真实部署场景中评估LLM智能体的安全风险。


<details>
  <summary>Details</summary>
Motivation: 现有智能体安全评估方法存在局限性：依赖针对特定设置的面向风险任务，安全风险空间覆盖有限；无法评估复杂真实部署中长期交互任务执行中的安全行为；对特定设置的专门化限制了跨不同智能体配置的适应性。

Method: 提出Risky-Bench框架，围绕领域无关的安全原则组织评估，推导上下文感知的安全标准来界定安全空间；在不同威胁假设下通过真实任务执行系统评估安全风险；框架结构良好，可适应不同部署设置。

Result: 在生活辅助智能体设置中应用Risky-Bench，发现在真实执行条件下最先进的智能体存在重大安全风险；框架不限于生活辅助场景，可适应其他部署设置构建环境特定的安全评估。

Conclusion: Risky-Bench提供了一个可扩展的方法论，用于系统评估LLM智能体在真实世界部署中的安全风险，解决了现有评估方法的局限性，为智能体安全评估提供了更全面和适应性的框架。
...
### [177] [Understanding Multi-Agent LLM Frameworks: A Unified Benchmark and Experimental Analysis](https://arxiv.org/abs/2602.03128)
*Abdelghny Orogat,Ana Rostam,Essam Mansour*

Main category: cs.AI

TL;DR: MAFBench：首个统一的多智能体LLM框架评估套件，揭示框架架构设计对系统性能的巨大影响（延迟增加100倍、规划准确率下降30%、协调成功率从90%降至30%）


<details>
  <summary>Details</summary>
Motivation: 当前多智能体LLM框架广泛使用但缺乏对其架构设计影响的系统理解。现有基准测试只关注单一能力，缺乏标准化的框架级评估，无法揭示架构选择对性能的显著影响。

Method: 提出多智能体LLM框架的架构分类法，开发MAFBench统一评估套件，集成现有基准测试到标准化执行流水线，在多个流行框架上进行受控实证研究。

Result: 框架级设计选择单独就能导致：延迟增加超过100倍，规划准确率下降高达30%，协调成功率从90%以上降至30%以下。不同框架在编排开销、内存行为、规划、专业化、协调等方面表现差异显著。

Conclusion: 多智能体LLM框架的架构设计对系统性能有决定性影响。MAFBench提供了标准化评估方法，研究结果为框架架构设计原则和选择指南提供了实证基础，并指出了未来研究方向。
...
### [178] [General Agents Contain World Models, even under Partial Observability and Stochasticity](https://arxiv.org/abs/2602.03146)
*Santiago Cifuentes*

Main category: cs.AI

TL;DR: 扩展了先前关于智能体必须学习环境模型的理论，从确定性智能体和完全可观测环境扩展到随机智能体和部分可观测环境，证明随机化不能避免学习环境模型


<details>
  <summary>Details</summary>
Motivation: 先前研究证明了在特定框架下，几乎最优的通用智能体必然包含足够的环境知识，但该结果依赖于智能体是确定性的且环境完全可观测的假设。本研究旨在移除这两个限制性假设。

Method: 通过扩展定理到随机智能体在部分可观测环境中的情况，并弱化通用性的概念，证明即使能力较弱的智能体也包含其操作环境的世界模型。

Result: 证明了随机智能体无法通过随机化避免学习其环境，并且更弱的智能体（即通用性要求降低的智能体）已经包含了其操作环境的世界模型。

Conclusion: 随机智能体在部分可观测环境中仍然必须学习环境模型，这扩展了智能体必须包含环境知识的基本理论，为理解智能体能力提供了更全面的理论基础。
...
### [179] [Enhancing Foundation VLM Robustness to Missing Modality: Scalable Diffusion for Bi-directional Feature Restoration](https://arxiv.org/abs/2602.03151)
*Wei Dai,Haoyu Wang,Honghao Chang,Lijun He,Fan Li,Jian Sun,Haixia Bi*

Main category: cs.AI

TL;DR: 提出一种通用的缺失模态恢复策略，通过增强扩散模型作为可插拔模块，动态模态门控和跨模态互学习机制，有效恢复缺失特征并保持VLM泛化能力


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在模态缺失时性能急剧下降，提示方法难以恢复关键特征，插补方法易产生语义无关噪声，需要一种能恢复精确语义同时保持模型泛化能力的方法

Method: 使用增强扩散模型作为可插拔中间训练模块，引入动态模态门控自适应利用条件特征指导生成语义一致特征，采用跨模态互学习机制桥接双编码器语义空间实现双向对齐

Result: 在基准数据集上的零样本评估显示该方法优于现有基线方法，大量实验和消融研究证实模型在缺失模态场景下具有鲁棒性和可扩展性，在不同缺失率和环境下保持可靠性

Conclusion: 提出的通用缺失模态恢复策略通过动态模态门控和跨模态互学习机制，有效解决了VLM在模态缺失时的性能下降问题，为VLM提供了鲁棒且可扩展的扩展方案
...
### [180] [VALUEFLOW: Toward Pluralistic and Steerable Value-based Alignment in Large Language Models](https://arxiv.org/abs/2602.03160)
*Woojin Kim,Sieun Hyeon,Jusang Oh,Jaeyoung Do*

Main category: cs.AI

TL;DR: VALUEFLOW框架：首个统一的价值强度控制框架，包含层次价值嵌入空间、价值强度数据库和锚点评估器，用于LLM的价值提取、评估和校准强度控制。


<details>
  <summary>Details</summary>
Motivation: 当前LLM与人类价值对齐存在挑战：基于偏好的方法难以捕捉深层动机原则，基于价值的方法存在三个缺陷：提取忽略层次结构、评估只检测存在而非校准强度、对LLM在可控强度下的可引导性理解不足。

Method: 提出VALUEFLOW框架，包含三个组件：1) HIVES层次价值嵌入空间，捕捉理论内和跨理论的价值结构；2) VIDB价值强度数据库，大规模价值标注文本资源，基于排序聚合的强度估计；3) 锚点评估器，通过将模型输出与VIDB面板排名产生一致的强度分数。

Result: 在10个模型和4个价值理论上的大规模研究表明，发现了可引导性的不对称性和多价值控制的组合规律，建立了评估和控制价值强度的可扩展基础设施。

Conclusion: VALUEFLOW为LLM的多元对齐提供了可扩展的基础设施，推进了价值强度的评估和控制，实现了校准强度下的价值引导。
...
### [181] [Beyond Quantity: Trajectory Diversity Scaling for Code Agents](https://arxiv.org/abs/2602.03219)
...
TL;DR: TDScaling是一个基于轨迹多样性扩展的代码智能体数据合成框架，通过增加轨迹多样性而非单纯增加数据量来提升性能，解决了现有方法在数据质量和数量扩展上的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有代码大语言模型通过MCP协议演化为工具交互智能体时，其泛化能力受到低质量合成数据和数量扩展收益递减的限制。数量为中心的扩展存在早期瓶颈，未能充分利用轨迹数据。

Method: 提出了TDScaling框架，包含四个创新：1) 业务集群机制捕捉真实服务逻辑依赖；2) 蓝图驱动的多智能体范式确保轨迹一致性；3) 自适应进化机制使用领域熵、推理模式熵和累积动作复杂度引导合成朝向长尾场景；4) 沙盒化代码工具防止内在编码能力的灾难性遗忘。

Result: 在通用工具使用基准（BFCL、tau^2-Bench）和代码智能体任务（RebenchT、CodeCI、BIRD）上的实验表明，TDScaling实现了双赢结果：既提高了工具使用的泛化能力，又增强了内在编码能力。

Conclusion: TDScaling通过轨迹多样性扩展而非数量扩展，在固定训练预算下实现了更好的性能-成本权衡，为代码智能体训练提供了更有效的数据合成框架，计划发布完整代码库和包含30,000+工具集群的合成数据集。
...
### [182] [TAME: A Trustworthy Test-Time Evolution of Agent Memory with Systematic Benchmarking](https://arxiv.org/abs/2602.03224)
*Yu Cheng,Jiuan Zhou,Yongkang Hu,Yihang Chen,Huichi Zhou,Mingang Chen,Zhizhong Zhang,Kun Shao,Yuan Xie,Zhaoxia Yin*

Main category: cs.AI

TL;DR: 论文提出TAME框架解决智能体在任务演进过程中记忆演化导致的安全对齐退化问题，通过双记忆演化机制在保持任务性能的同时维护可信度。


<details>
  <summary>Details</summary>
Motivation: 智能体在测试时间通过记忆演化积累经验是实现AGI的关键范式，但即使在良性任务演化过程中，智能体的安全对齐仍然脆弱，这种现象被称为"智能体记忆错误演化"。需要评估和解决这一现象。

Method: 提出TAME双记忆演化框架：分离演化执行器记忆（通过提炼可泛化方法提升任务性能）和评估器记忆（基于历史反馈优化安全和任务效用评估）。通过记忆过滤、草稿生成、可信度精炼、执行和双轨记忆更新的闭环流程。

Result: 构建了Trust-Memevo基准评估良性任务演化中的多维可信度，发现各任务领域和评估设置中可信度普遍下降。TAME实验表明能够缓解错误演化，在可信度和任务性能上实现联合提升。

Conclusion: TAME框架通过双记忆演化机制有效解决了智能体记忆错误演化问题，在保持任务效用的同时维护了可信度，为实现安全可靠的AGI提供了重要方法。
...
### [183] [The Necessity of a Unified Framework for LLM-Based Agent Evaluation](https://arxiv.org/abs/2602.03238)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型智能体的发展，现有评估方法面临多重挑战：系统提示、工具配置、环境动态等外部因素严重干扰评估结果，缺乏标准化导致不公平和不透明

Method: 提出建立统一评估框架的提案，旨在标准化智能体评估，解决当前评估中的碎片化、不可复现性问题

Result: 论文主要提出了标准化评估框架的提案，尚未展示具体实验结果，但指出了当前评估方法存在的问题和标准化的重要性

Conclusion: 建立统一的智能体评估框架对于该领域的严谨发展至关重要，标准化能够解决当前评估中的不公平性和不透明性问题
...
### [184] [Accordion-Thinking: Self-Regulated Step Summaries for Efficient and Readable LLM Reasoning](https://arxiv.org/abs/2602.03249)
*Zhicheng Yang,Zhijiang Guo,Yinya Huang,Yongxin Wang,Wenlei Shi,Yiwei Wang,Xiaodan Liang,Jing Tang*

Main category: cs.AI

TL;DR: Accordion-Thinking框架让LLM学会通过动态总结来自我调节推理步骤粒度，实现推理上下文的有效压缩，在保持准确性的同时显著提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 传统的长链式思维推理方法面临KV缓存线性增长和注意力复杂度二次方增长的实践限制，需要一种更高效的推理压缩机制。

Method: 提出Accordion-Thinking端到端框架，通过动态总结机制让LLM学习自我调节推理步骤粒度，采用强化学习激励这种能力，实现Fold推理模式（定期总结并丢弃历史思考）。

Result: 模型学会了将关键推理信息编码到紧凑的总结中，Fold模式与完整Unfold模式之间的准确率差距在训练过程中逐渐缩小直至消失，在48GB GPU内存配置下实现3倍吞吐量同时保持准确性。

Conclusion: 通过学习的自我压缩，LLM能够以最小的依赖token开销处理复杂推理任务而不影响解决方案质量，同时结构化的步骤总结提供了人类可读的推理过程记录。
...
### [185] [CSR-Bench: A Benchmark for Evaluating the Cross-modal Safety and Reliability of MLLMs](https://arxiv.org/abs/2602.03263)
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型虽然支持文本和图像交互，但其安全行为可能由单模态捷径驱动而非真正的联合意图理解。需要评估模型在需要跨模态整合理解任务中的可靠性，诊断模态引起的行为变化。

Method: 引入CSR-Bench基准，包含四种压力测试交互模式（安全性、过度拒绝、偏见、幻觉），涵盖61种细粒度类型。每个实例都需要图像-文本联合解释，同时提供配对的纯文本对照以诊断模态引起的行为变化。评估了16个最先进的多模态大语言模型。

Result: 观察到系统性的跨模态对齐差距：模型表现出弱安全意识，在干扰下强烈的语言主导性，以及从纯文本对照到多模态输入的一致性能下降。还观察到减少过度拒绝与保持安全、非歧视行为之间的明显权衡，表明某些表面安全增益可能来自拒绝导向的启发式方法而非稳健的意图理解。

Conclusion: 多模态大语言模型在跨模态可靠性方面存在系统性缺陷，需要更稳健的意图理解机制而非简单的拒绝启发式方法。CSR-Bench为评估和改进多模态模型的跨模态对齐提供了重要工具。
...
### [186] [Rejecting Arguments Based on Doubt in Structured Bipolar Argumentation](https://arxiv.org/abs/2602.03286)
*Michael A. Müller,Srdjan Vesic,Bruno Yun*

Main category: cs.AI

TL;DR: 本文提出了一种新的计算论证方法，结合哲学和语言学观点，允许基于怀疑拒绝论证，并引入结构化双极论证框架(SBAFs)，提供不强制接受所有防御论证的语义，同时支持论证扩展和语言扩展。


<details>
  <summary>Details</summary>
Motivation: 现有计算论证方法存在两个局限性：1）通常要求智能体必须接受所有能够防御的论证，忽视了基于怀疑理性拒绝论证的可能性；2）过于关注论证层面，而忽略了有时更自然的句子或主张层面的思考。本文旨在将哲学和语言学中的这两个重要观点融入计算论证框架。

Method: 首先定义结构化双极论证框架(SBAFs)，其中论证由句子组成，包含攻击和支持两种关系。然后为SBAFs提供新的语义：1）不同于基于完备性的语义，不强制智能体接受所有防御的论证；2）除了论证扩展（可接受的论证集合），还提供语言扩展（可接受的句子集合）。这些语义位于抽象论证的可接受语义和完备语义之间。

Result: 提出的语义代表了智能体在辩论中可能持有的合理立场。该方法为现有方法提供了新视角：可以指定智能体在何种条件下可以忽略论证间的支持关系（即使用抽象论证的合理性条件），并证明演绎支持语义是本方法的一个特例。

Conclusion: 本文开发了一种受哲学和语言学启发的计算论证新方法，通过结构化双极论证框架和灵活的语义，更好地模拟了实际辩论中智能体的理性决策过程，为计算论证领域提供了更丰富和实用的理论框架。
...
TL;DR: Memora是一种谐波记忆表示方法，通过主抽象索引具体记忆值和线索锚点扩展检索访问，在抽象与特异性之间取得平衡，在LoCoMo和LongMemEval基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 智能体记忆系统需要处理持续增长的信息，同时支持高效、上下文感知的检索。抽象对于扩展记忆至关重要，但通常会牺牲特异性，掩盖有效推理所需的细粒度细节。

Method: 引入Memora谐波记忆表示，通过主抽象索引具体记忆值并将相关更新整合为统一记忆条目，同时使用线索锚点扩展跨不同记忆方面的检索访问并连接相关记忆。在此基础上采用检索策略，主动利用这些记忆连接来检索超出直接语义相似性的相关信息。

Result: 理论上证明标准检索增强生成(RAG)和知识图谱(KG)记忆系统是Memora框架的特殊情况。在LoCoMo和LongMemEval基准测试中建立了新的最先进水平，展示了随着记忆扩展，检索相关性和推理有效性更好。

Conclusion: Memora通过谐波记忆表示在抽象与特异性之间取得结构平衡，能够有效处理大规模记忆系统，在检索相关性和推理能力方面优于现有方法。
...
TL;DR: 该研究提出了首个面向真实世界临床场景的精神障碍级别诊断基准MentalDx Bench，并开发了专门化模型MentalSeek-Dx，通过监督轨迹构建和课程强化学习实现临床推理过程内化，在仅有14B参数下达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在精神健康评估中潜力巨大，但现有基准缺乏生态效度和细粒度诊断监督，严重限制了其临床实用性。需要建立真实临床环境下的障碍级别诊断基准来弥合这一差距。

Method: 1) 构建MentalDx Bench基准：包含712份去识别化电子健康记录，由委员会认证的精神科医生按照ICD-11标准标注，覆盖16个诊断类别下的76种障碍；2) 提出MentalSeek-Dx模型：通过监督轨迹构建和课程强化学习训练医学专门化LLM，内化临床假设-演绎推理过程。

Result: 评估18个LLM发现关键范式错配：在粗粒度诊断分类上表现良好，但在障碍级别诊断上系统性失败。MentalSeek-Dx在MentalDx Bench上达到最先进性能，仅用14B参数就建立了临床可靠的精神科诊断框架。

Conclusion: 研究揭示了当前LLM基于模式建模与临床假设-演绎推理之间的根本差距，提出的MentalSeek-Dx通过专门化训练成功内化了临床推理过程，为可靠的精神科诊断提供了临床基础框架。
...
Method: RAI从语言嵌入构建不安全原型子空间，对选定的高风险视觉标记进行针对性调制，在跨模态特征空间中显式激活安全关键信号，恢复模型从视觉输入中检测不安全内容的能力。

Result: 在多个越狱攻击和效用基准测试中，RAI显著降低了攻击成功率，同时保持了任务性能，实现了安全性和实用性的平衡。

Conclusion: RAI提供了一种轻量级、无需训练的安全校准方法，通过恢复视觉语言模型的LLM式风险识别能力，有效防御多模态越狱攻击，同时保持跨模态推理的语义完整性。
...
### [190] [DiscoverLLM: From Executing Intents to Discovering Them](https://arxiv.org/abs/2602.03429)
*Tae Soo Kim,Yoonjoo Lee,Jaesang Yu,John Joon Young Chung,Juho Kim*

Main category: cs.AI

TL;DR: DiscoverLLM是一个训练大语言模型帮助用户形成和发现意图的框架，通过模拟用户认知状态和渐进具体化的意图层次，让模型学会在意图不明确时探索选项，在意图具体化时细化实现。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs处理模糊和开放式请求时，通常只是询问澄清问题，但当用户自己也不知道想要什么时，这种方法会失败。用户模糊是因为他们尚未形成意图，需要通过观察和探索结果来发现自己的需求。

Method: 提出DiscoverLLM框架，核心是新颖的用户模拟器，用层次化的意图建模认知状态，意图随着模型展示相关选项而渐进具体化。具体化程度作为奖励信号，训练模型优化这一过程。模型学会在意图不明确时发散探索选项，在意图具体化时收敛细化实现。

Result: 在创意写作、技术写作和SVG绘图等交互基准测试中，DiscoverLLM实现了超过10%的任务性能提升，同时将对话长度减少了40%。在75名人类参与者的用户研究中，DiscoverLLM相比基线提高了对话满意度和效率。

Conclusion: DiscoverLLM通过帮助用户形成和发现意图，有效解决了LLMs处理模糊请求时的局限性，实现了更高效、更满意的交互体验。
...
<details>
  <summary>Details</summary>
Motivation: 在开放世界环境中，具身智能体需要终身学习能力。虽然强化学习微调已成为让视觉-语言-动作模型掌握灵巧操作的重要范式，但在持续强化学习场景中，平衡稳定性（保留旧技能）和可塑性（学习新技能）仍然是现有方法面临的重大挑战。

Method: 提出CRL-VLA框架，通过理论推导建立了统一性能边界，将稳定性-可塑性权衡与目标条件优势幅度和策略散度联系起来。采用非对称调节机制：限制先前任务的优势幅度，同时允许新任务上的受控增长。实现方式是通过具有新颖目标条件价值公式的双评论家架构，其中冻结评论家保持语义一致性，可训练估计器驱动适应。

Result: 在LIBERO基准上的实验表明，CRL-VLA有效地协调了这些冲突目标，在抗遗忘和向前适应方面均优于基线方法。

Conclusion: CRL-VLA为VLA模型的持续后训练提供了一个具有严格理论边界的框架，成功解决了持续强化学习中稳定性与可塑性的权衡问题，为终身机器人场景中的VLA模型部署提供了有前景的途径。
...
### [192] [The Dual Role of Abstracting over the Irrelevant in Symbolic Explanations: Cognitive Effort vs. Understanding](https://arxiv.org/abs/2602.03467)
*Zeynep G. Saribatur,Johannes Langer,Ute Schmid*

Main category: cs.AI

TL;DR: 该研究探讨如何通过形式抽象（移除和聚类）改进符号AI解释的可理解性，实验表明聚类提升理解，移除细节降低认知负荷


<details>
  <summary>Details</summary>
Motivation: 虽然符号AI提供了可解释性的透明基础，但原始逻辑轨迹往往带来较高的外部认知负荷。解释对人类认知至关重要，但AI系统产生的输出常常难以理解。因此需要研究如何通过形式抽象来改进人类对符号解释的理解。

Method: 使用答案集编程（ASP）作为形式框架，定义需要抽象化的无关细节概念。通过认知实验，让参与者在不同领域中基于答案集程序生成的解释对刺激进行分类，比较移除细节和聚类细节两种抽象方法的效果。

Result: 认知实验显示：聚类细节显著提高了参与者的理解能力，而移除细节则显著降低了认知努力。这支持了抽象化能够增强以人为中心的符号解释的假设。

Conclusion: 形式抽象（特别是移除和聚类）能够有效改善符号AI解释的人类可理解性，聚类提升理解效果，移除降低认知负荷，为设计更人性化的AI解释系统提供了实证支持。
...
### [193] [When Routing Collapses: On the Degenerate Convergence of LLM Routers](https://arxiv.org/abs/2602.03478)
*Guannan Lai,Han-Jia Ye*

Main category: cs.AI

TL;DR: 该论文发现现有LLM路由系统存在"路由崩溃"现象：随着成本预算增加，路由器会系统性地选择最强大但最昂贵的模型，即使更便宜的模型已经足够，导致小模型利用率不足。作者提出EquiRouter解决方案，直接学习模型排名而非性能分数，有效缓解路由崩溃问题。
...
Method: 作者提出EquiRouter，一种决策感知的路由器。与现有路由器训练预测标量性能分数不同，EquiRouter直接学习模型排名。这种方法解决了目标-决策不匹配问题：现有路由器的小预测误差可能翻转相对排序并触发次优选择，而EquiRouter通过直接学习排名来恢复小模型的作用并缓解路由崩溃。

Result: 在RouterBench基准测试中，EquiRouter在达到GPT-4级别性能时，相比之前最强的路由器减少了约17%的成本。这表明EquiRouter能够更有效地利用小模型，在保持高质量的同时显著降低成本。

Conclusion: 该论文揭示了LLM路由中普遍存在的路由崩溃问题，并提出EquiRouter作为解决方案。通过直接学习模型排名而非性能分数，EquiRouter能够更好地平衡质量与成本，恢复小模型在路由系统中的应有作用，为实际部署提供了更经济高效的LLM路由方案。
...
### [194] [Group Selection as a Safeguard Against AI Substitution](https://arxiv.org/abs/2602.03541)
*Qiankun Zhong,Thomas F. Eisenmann,Julian Garcia,Iyad Rahwan*

Main category: cs.AI

TL;DR: 研究AI使用对人类文化演化的长期影响，发现AI替代型用户（AI-substitute）在个体选择中占优但会减少文化多样性，而AI补充型用户（AI-complement）能维持群体文化多样性，在强群体边界下可通过文化群体选择获得优势。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的依赖可能降低文化多样性和变异，这已经导致模型性能问题如模型崩溃和幻觉。研究旨在探讨AI使用对人类文化演化的长期后果，以及广泛AI使用可能导致"文化崩溃"的条件——即AI生成内容依赖减少人类变异和创新，减缓累积文化演化。

Method: 使用基于主体的建模和演化博弈论，比较两种AI使用类型：补充型（AI-complement）用户寻求建议和指导但仍为主要产出者，替代型（AI-substitute）用户提供最少输入并依赖AI产生大部分产出。研究这些使用策略在演化动态下的竞争和传播。

Result: AI替代型用户在个体层面选择中占优势，尽管会更强地减少文化变异。相反，AI补充型用户通过维持探索所需的变异而有益于其群体，因此在群体边界强时可通过文化群体选择获得优势。

Conclusion: 研究揭示了AI采用在人口层面的长期效应，为缓解这些风险的政策和组织策略提供信息。AI补充型使用策略有助于维持文化多样性，而替代型使用可能导致文化变异减少和文化演化减缓。
...
### [195] [EHRWorld: A Patient-Centric Medical World Model for Long-Horizon Clinical Trajectories](https://arxiv.org/abs/2602.03569)
*Linjie Mu,Zhongzhen Huang,Yannian Gu,Shengqian Qin,Shaoting Zhang,Xiaofan Zhang*

Main category: cs.AI

TL;DR: EHRWorld：基于因果序列范式训练的医疗世界模型，显著优于基于LLM的基线方法，在长时程临床模拟中表现更稳定


<details>
  <summary>Details</summary>
Motivation: 现有LLM在静态医疗推理任务上表现良好，但作为动态医疗世界模型模拟疾病进展和治疗结果时，难以在序列干预下保持一致的病人状态，导致长时程模拟中的误差累积

Method: 提出EHRWorld，一个基于因果序列范式训练的病人中心医疗世界模型，同时构建EHRWorld-110K大规模纵向临床数据集，源自真实世界电子健康记录

Result: EHRWorld显著优于基于LLM的基线方法，实现更稳定的长时程模拟、更好的临床敏感事件建模，以及更优的推理效率

Conclusion: 可靠和稳健的医疗世界建模需要在因果基础、时间演化的临床数据上进行训练，EHRWorld为此提供了有效的解决方案
...
### [196] [Search-R2: Enhancing Search-Integrated Reasoning via Actor-Refiner Collaboration](https://arxiv.org/abs/2602.03647)
*Bowei He,Minda Hu,Zenan Xu,Hongru Wang,Licheng Zong,Yankai Chen,Chen Ma,Xue Liu,Pluto Zhou,Irwin King*

Main category: cs.AI

TL;DR: Search-R2：基于Actor-Refiner协作框架的语言智能体训练方法，通过精细化的干预机制和混合奖励设计，解决搜索集成推理中的多尺度信用分配问题，显著提升推理准确性。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的搜索集成推理智能体训练面临多尺度信用分配问题：稀疏的轨迹级奖励无法区分高质量推理和偶然猜测，导致冗余或误导性搜索行为。

Method: 提出Actor-Refiner协作框架：Actor生成初始推理轨迹，Meta-Refiner通过"剪切-再生"机制选择性诊断和修复有缺陷的步骤。采用混合奖励设计，结合结果正确性和检索证据信息密度的密集过程奖励。

Result: 在多个通用和多跳问答数据集上的实验表明，Search-R2在不同模型规模下均优于强RAG和基于RL的基线方法，以最小开销实现更优的推理准确性。

Conclusion: Search-R2通过精细化的干预机制和混合奖励设计，有效解决了搜索集成推理中的信用分配问题，为训练更可靠的语言智能体提供了新框架。
...
### [197] [Mitigating Conversational Inertia in Multi-Turn Agents](https://arxiv.org/abs/2602.03664)
*Yang Wan,Zheng Cao,Zhenhao Zhang,Zhengwen Zeng,Shuheng Shen,Changhua Meng,Linchao Zhu*

Main category: cs.AI

TL;DR: 本文提出"对话惯性"概念，指LLM在多轮对话中过度模仿自身先前回答的问题，并提出上下文偏好学习框架来校准模型偏好，减少惯性并提升探索能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在少样本学习中表现出色，但在多轮智能体场景中，LLM会错误地将自己先前的回答作为少样本示例进行模仿，导致探索能力受限。研究发现存在"对话惯性"现象，即模型对先前回答表现出强烈的对角线注意力，这与模仿偏见相关，限制了探索。

Method: 通过注意力分析识别对话惯性现象，提出上下文偏好学习框架，基于关键洞察：对于相同状态，长上下文生成的行动比短上下文表现出更强的惯性，从而无需环境奖励即可构建偏好对。该方法校准模型偏好，使其偏好低惯性而非高惯性响应，并在推理时提供上下文管理策略以平衡探索与利用。

Result: 在八个智能体环境和一个深度研究场景中的实验结果表明，该框架有效减少了对话惯性并实现了性能提升。

Conclusion: 将少样本LLM转化为智能体时存在张力：长上下文丰富了环境反馈以促进利用，但也放大了对话惯性从而削弱探索。提出的上下文偏好学习框架能够校准模型偏好，减少惯性，提升智能体在多轮对话中的探索能力。
...
### [198] [TodyComm: Task-Oriented Dynamic Communication for Multi-Round LLM-based Multi-Agent System](https://arxiv.org/abs/2602.03688)
*Wenzhe Fan,Tommaso Tognoli,Henry Peng Zou,Chunyu Miao,Yibo Wang,Xinhua Zhang*

Main category: cs.AI

TL;DR: TodyComm：一种面向任务的动态通信算法，用于多轮LLM多智能体系统，能根据每轮动态变化自适应调整通信拓扑结构


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统大多采用固定的通信拓扑结构，无法适应现实应用中智能体角色可能随轮次变化的情况（如动态对手、任务进展、时变通信带宽约束等）

Method: 提出TodyComm算法，通过策略梯度优化，生成行为驱动的协作拓扑结构，使通信结构能根据每轮的动态变化自适应调整
...
### [199] [AOrchestra: Automating Sub-Agent Creation for Agentic Orchestration](https://arxiv.org/abs/2602.03786)
...
TL;DR: AOrchestra是一个基于统一智能体抽象（指令、上下文、工具、模型）的编排系统，能够动态创建专用执行器，在多个基准测试中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 现有语言智能体系统缺乏对子智能体的动态抽象视图，限制了系统的适应性和灵活性，需要一种能够按需创建专用执行器的统一抽象方法

Method: 提出统一的智能体抽象模型（指令、上下文、工具、模型），基于此构建AOrchestra系统，中央编排器在每一步具体化这个元组：策划任务相关上下文、选择工具和模型，通过动态自动创建智能体来委托执行

Result: 在GAIA、SWE-Bench、Terminal-Bench三个具有挑战性的基准测试中，AOrchestra与Gemini-3-Flash配合使用时，相对于最强基线实现了16.28%的相对改进

Conclusion: AOrchestra通过统一的智能体抽象和动态编排机制，减少了人工工程工作量，保持了框架无关性，实现了可控的性能-成本权衡，能够接近帕累托效率边界
...
### [200] [Understanding Agent Scaling in LLM-Based Multi-Agent Systems via Diversity](https://arxiv.org/abs/2602.03794)
*Yingxuan Yang,Chengrui Qu,Muning Wen,Laixi Shi,Ying Wen,Weinan Zhang,Adam Wierman,Shangding Gu*

Main category: cs.AI

TL;DR: 研究发现LLM多智能体系统性能受限于任务固有不确定性而非智能体数量，同质智能体扩展存在边际效益递减，而异质智能体通过互补证据持续提升性能


<details>
  <summary>Details</summary>
Motivation: 探索LLM多智能体系统性能扩展的局限性，理解为何增加智能体数量存在边际效益递减，以及为何引入异质性（不同模型、提示或工具）能持续提升性能

Method: 提出信息论框架分析MAS性能边界，推导架构无关的边界证明性能提升取决于系统访问的有效通道数量，引入K*（有效通道计数）量化无需真实标签的有效通道数量，并进行实证验证

Result: 异质配置始终优于同质扩展：2个异质智能体的性能可匹配或超过16个同质智能体，K*能有效量化系统多样性，为构建高效稳健的MAS提供理论指导

Conclusion: 多智能体系统性能受任务固有不确定性限制而非智能体数量，同质智能体因输出高度相关而早期饱和，异质智能体通过互补证据持续提升性能，应通过多样性感知设计构建高效稳健的MAS
...
### [201] [Conformal Thinking: Risk Control for Reasoning on a Compute Budget](https://arxiv.org/abs/2602.03814)
...
TL;DR: 论文提出了一种基于风险控制的LLM推理预算设置框架，通过上下阈值机制在保证错误率限制的同时最小化计算成本。


<details>
  <summary>Details</summary>
Motivation: 大语言模型推理时存在计算成本与准确性的权衡问题，传统方法难以在保证错误率限制的同时优化计算效率，需要一种系统化的风险控制框架。

Method: 提出双阈值停止机制：上阈值在模型置信度高时停止（风险错误输出），参数化下阈值提前停止不可解实例（风险过早停止）。使用无分布风险控制方法，结合效率损失函数选择最优退出机制。

Result: 在多种推理任务和模型上的实验表明，该风险控制方法能有效实现计算效率提升，同时严格遵守用户指定的风险目标。

Conclusion: 该框架为LLM推理预算设置提供了系统化的风险控制解决方案，通过双阈值机制在保证错误率限制的同时显著提升计算效率。
...
### [202] [AutoFigure: Generating and Refining Publication-Ready Scientific Illustrations](https://arxiv.org/abs/2602.03828)
...
Method: 提出FigureBench基准数据集（3300个高质量文本-插图对）和AutoFigure智能体框架，该框架在最终渲染前通过广泛思考、重组和验证来生成结构合理且美观的布局。

Result: 实验结果表明，AutoFigure在所有基线方法中表现最优，能够生成可直接用于发表的科学插图。

Conclusion: FigureBench为科学插图生成提供了首个大规模基准，AutoFigure框架能够有效自动化科学插图生成过程，显著提升科学传播效率。
...
### [203] [Thinking Like a Doctor: Conversational Diagnosis through the Exploration of Diagnostic Knowledge Graphs](https://arxiv.org/abs/2602.01995)
...
Method: 提出两阶段推理系统：1)从对话上下文生成诊断假设；2)通过澄清问题验证假设，循环直到最终诊断。使用MIMIC-IV患者资料和改良的模拟器来反映真实世界中早期临床接触时的模糊症状描述

Result: 实验显示相比强基线方法，该系统在诊断准确性和效率上均有提升。医师评估支持模拟器的真实性和生成问题的临床实用性

Conclusion: 基于知识图谱的对话诊断系统能有效处理信息不完整和症状描述模糊的现实情况，提高诊断性能，具有临床实用价值
