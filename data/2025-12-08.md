...
TL;DR: AREA3D是一种主动3D重建方法，通过解耦视图不确定性建模和使用视觉语言模型提供语义指导，在稀疏视图下实现更高效准确的重建。
...
Conclusion: AREA3D通过结合不确定性建模和语义指导，显著提升了主动3D重建的效率和准确性，为智能视角选择提供了新思路。
...
TL;DR: 论文提出"尺度锚定"问题：零样本超分辨率时空预测中，模型在低分辨率训练后无法在高分辨率推理中降低误差，因为低频数据无法表示高频物理规律。作者提出频率表示学习方法来缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究认为在不同分辨率下保持相似误差就是成功的多分辨率泛化，但作为数值求解器替代的深度学习模型应该在分辨率提高时降低误差。低分辨率数据的奈奎斯特频率限制了其能表示的物理规律频率上限，导致模型无法处理高分辨率推理中的未见频率成分，造成误差被"锚定"在低分辨率水平。
...
Conclusion: 论文识别了零样本超分辨率时空预测中的"尺度锚定"这一新问题，并提出频率表示学习方法有效缓解了该问题，使模型能够在高分辨率推理中降低误差，实现真正的多分辨率泛化。
...
Method: 提出InvarDiff方法：1）通过少量确定性运行计算每个时间步、每层、每个模块的二进制缓存计划矩阵；2）使用重采样校正避免连续缓存时的漂移；3）基于分位数的变化度量决定哪些模块在哪些步骤可以重用；4）在推理时按步骤优先和分层缓存执行。

Result: 在DiT和FLUX模型上实验表明，InvarDiff实现了2-3倍的端到端加速，对标准质量指标影响极小，视觉质量几乎没有退化。

Conclusion: InvarDiff通过利用扩散模型中的特征不变性，提供了一种有效的训练免费加速方案，在保持生成质量的同时显著提升推理速度。
...
Method: 开发了一种可解释的AI-ECG模型，用于预测冠状动脉CT血管造影中四支主要冠状动脉（右冠状动脉、左主干、左前降支、左回旋支）的严重或完全狭窄。模型在内部和外部验证集上进行评估，并在临床正常心电图亚组中进行测试。进行了亚组分析和风险分层，并通过可解释性分析揭示高风险和低风险组之间的波形差异。

Result: 在内部验证集上，模型对右冠状动脉、左主干、左前降支、左回旋支的AUC分别为0.794、0.818、0.744、0.755；在外部验证集上，AUC分别达到0.749、0.971、0.667、0.727。模型在临床正常心电图亚组中性能保持稳定。亚组分析证实了模型在不同人口统计学和采集时间层面的稳定性。基于血管特异性发生率阈值的风险分层在校准和累积事件曲线上显示出一致的分离。可解释性分析揭示了高风险和低风险组之间不同的波形差异。

Conclusion: 该研究开发的可解释AI-ECG模型能够有效预测冠状动脉严重或完全狭窄，在内外验证集上表现出良好性能，并在正常心电图患者中保持稳健。模型的可解释性分析为冠状动脉狭窄的心电图相关性提供了新见解，表明AI-ECG有望成为冠状动脉疾病筛查的有价值工具。
...
TL;DR: 论文提出了ChromouVQA基准测试，基于石原式色觉检查图构建大规模多任务数据集，评估视觉语言模型在复杂背景下的目标识别能力


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态理解方面取得了进展，但在目标嵌入杂乱背景需要图形-背景分离时仍然存在困难，需要专门的基准测试来评估模型在这方面的能力

Method: 基于石原式色觉检查图构建大规模多任务基准测试，扩展经典点阵图，改变色彩分离、密度、大小、遮挡和旋转等参数，包含9种视觉问答任务，并提出模型无关的对比学习方法

Result: 评估显示人类和视觉语言模型之间存在较大差距，特别是在细微色彩对比或破坏性几何填充条件下；提出的对比学习方法能改善全局形状恢复

Conclusion: ChromouVQA提供了一个紧凑、可控的基准测试，用于可重复评估和扩展，有助于推动视觉语言模型在复杂背景下的目标识别能力发展
...
TL;DR: 提出基于迁移学习和扩散模型的生成式降尺度框架，从粗分辨率卫星图像重建细分辨率图像，在亚洲区域季节分割实验中表现优异


<details>
  <summary>Details</summary>
Motivation: 解决从粗分辨率卫星图像（如MERRA-2的50km）生成细分辨率图像（如G5NR的7km）的挑战，特别是在有限训练数据情况下实现物理一致的空间变异性和时间自相关性保持

Method: 结合轻量级U-Net迁移编码器和扩散生成模型：先在粗分辨率长时间序列上预训练U-Net学习时空表征，然后冻结编码器并迁移到更大的降尺度模型中作为物理有意义的潜在特征

Result: 在亚洲区域季节分割实验中表现优异（R2 = 0.65到0.94），优于确定性U-Net、变分自编码器和先前迁移学习基线；预测图像保持了物理一致的空间变异性和时间自相关性
...
<details>
  <summary>Details</summary>
Motivation: 地球观测数据存在固有的异质性（不同传感器、地理区域、采集时间、大气条件），导致训练域与部署域之间的分布偏移严重限制了预训练遥感模型的泛化能力，因此需要无监督域自适应技术。
...
TL;DR: 提出了一种无需人工标注、仅使用自合成数据训练视觉语言模型评估器的方法，通过迭代生成多模态指令-响应对、推理轨迹和判断，显著提升了评估性能。


<details>
  <summary>Details</summary>
Motivation: 当前训练VLM评估器主要依赖大规模人工偏好标注，这种方法成本高昂，且随着模型快速改进，标注数据容易过时。需要一种无需人工标注的自训练方法。

Method: 采用三阶段迭代框架：1) 生成不同质量级别的多样化多模态指令-响应对；2) 为每对生成推理轨迹和判断，移除不符合预期质量级别的数据；3) 在正确的评估答案及其推理轨迹上进行训练。

Result: 在Multimodal RewardBench和VL-RewardBench上的评估显示，该方法将Llama-3.2-11B多模态评估器的整体准确率从0.38提升到0.51，在通用性、幻觉和推理维度表现尤其突出，甚至超越了更大的模型如Llama-3.2-90B、GPT-4o和Claude 3.5 Sonnet。

Conclusion: 这种无需人工标注的方法展示了自训练评估器的潜力，能够随着VLM能力的快速提升而同步进化，为未来自适应评估系统的发展提供了方向。
...
TL;DR: 本文提出了一种用于细粒度图像生成的分层嵌入器概念，结合超类和子类语义信息，解决语义纠缠问题，并通过超分辨率增强细节特征，同时提出高效的ProAttention机制。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在细粒度图像生成中存在语义信息纠缠和生成图像细节不足的问题。现有的类条件生成方法往往关注更常见的类别，而在大规模细粒度图像生成中这些问题仍然存在。

Method: 1. 引入分层嵌入器概念，整合超类和子类的语义信息；2. 在感知信息生成阶段引入超分辨率概念，通过增强和退化模型提升细节特征；3. 提出高效的ProAttention机制，可在扩散模型中有效实现。

Result: 在公共基准测试上进行广泛实验，结果表明该方法在性能上优于其他最先进的微调方法。
...
TL;DR: 提出像素等效潜在合成(PELC)原则，开发DecFormer实现扩散模型中高质量潜在空间图像合成，显著提升掩码边缘质量和颜色一致性


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型中的潜在空间修复主要依赖线性插值方法，但这种方法在掩码边缘会产生明显伪影、全局颜色偏移和图像质量下降，需要更精确的潜在合成方法

Method: 提出像素等效潜在合成(PELC)原则，开发DecFormer——一个770万参数的Transformer，预测每通道混合权重和离流形残差校正，实现与像素空间alpha合成等效的潜在融合

Result: 在FLUX.1系列模型上，DecFormer恢复了全局颜色一致性、软掩码支持、锐利边界和高保真掩码，边缘误差指标比标准掩码插值降低达53%；作为修复先验，轻量级LoRA+DecFormer达到与完全微调模型相当的质量

Conclusion: PELC为像素等效潜在编辑提供了通用方案，DecFormer实现了高质量潜在空间合成，兼容现有扩散流程，参数和计算开销极小，在修复和复杂颜色校正任务中表现出色
...
Method: 基于MIT-Adobe FiveK数据集构建DEAR数据集，通过大规模众包收集人类偏好评分，每对图像由25名独立评估者评分，共13,648名评估者参与，建立数据收集流程并分析人类投票模式。

Result: 创建了首个系统解决基于人类主观偏好的图像渲染美学评估数据集，包含细粒度、上下文敏感的美学偏好标注，支持风格偏好预测、美学基准测试和个性化美学建模等应用。

Conclusion: DEAR填补了传统失真评估与主观美学评估之间的空白，为渲染美学评估提供了首个系统基准，推动了超越技术失真的图像美学研究，数据集已在HuggingFace发布。
...
Conclusion: 提出的混合捕获范式能显著降低功耗，同时通过事件数据重建高质量RGB视频。扩散方法在感知质量上优于自回归方法，且具有良好的泛化能力。
...
Result: 在合成相关噪声的标准基准测试和CIN-D真实噪声数据集上，CARD在去噪、去模糊和超分辨率任务中均优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界场景通常包含多个静态和动态对象，但现有方法往往只能处理单个对象，依赖类别特定的参数化形状模型，导致场景配置不一致且受限于建模类别。

Method: 设计空间和时间注意力机制，分别从静态多对象组合和动态单对象时序中学习，通过注意力混合机制在推理时结合两者，交替进行空间和时间推理。

Result: COM4D能够从单目视频中重建完整、持久的4D多对象交互场景，在4D对象重建和组合3D重建任务上达到最先进水平。

Conclusion: COM4D通过分离学习空间组合和时间动态，避免了4D组合训练数据的需求，能够一致地联合预测4D/3D对象的结构和时空配置。
...
TL;DR: 论文提出了TAD基准测试，专门评估自动驾驶场景中的时序理解能力，包含近6000个问答对和7个任务。现有SOTA模型在该基准上表现不佳，作者提出了两种无需训练的方法（Scene-CoT和TCogMap）将准确率提升高达17.72%。
...
Method: 1. 创建TAD基准测试：包含近6000个问答对，涵盖7个人工设计的任务，专门评估自动驾驶场景中的时序理解能力。2. 提出两种无需训练的解决方案：Scene-CoT（利用思维链）和TCogMap（融入第一人称时序认知地图）。

Result: 当前SOTA模型在TAD基准上表现不佳，主要因为细粒度运动理解不完善。提出的Scene-CoT和TCogMap方法与现有VLM集成后，将TAD上的平均准确率提升了高达17.72%。

Conclusion: TAD基准填补了自动驾驶时序理解评估的空白，提出的两种方法有效提升了模型性能，旨在推动自动驾驶时序理解研究的进一步发展。
...
TL;DR: SpaceControl是一种无需训练、测试时的方法，通过几何输入（从粗略图元到精细网格）实现对3D生成的显式空间控制，在几何保真度和输出真实感之间提供可控权衡。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法主要依赖文本或图像提示，但语言存在歧义性，图像编辑繁琐，难以提供直观精确的几何控制。需要一种能够接受各种几何输入并实现显式空间控制的方法。

Method: 提出SpaceControl方法，无需额外训练，可与现代预训练生成模型无缝集成。接受从粗略图元到详细网格的多种几何输入，通过可控参数在几何保真度和输出真实感之间进行权衡。

Result: 通过广泛的定量评估和用户研究表明，SpaceControl在几何忠实度方面优于基于训练和优化的基线方法，同时保持高视觉质量。提供了交互式用户界面，支持在线编辑超二次曲面并直接转换为纹理3D资产。

Conclusion: SpaceControl提供了一种无需训练、测试时的方法，通过几何输入实现对3D生成的显式空间控制，在几何保真度和视觉质量之间取得良好平衡，便于在创意工作流程中实际部署。
...
TL;DR: 提出了一种用于3D高斯溅射资产交互式编辑的快速前馈模型，支持从2D视图进行连续编辑，实现局部细节细化、局部绘制和全局重新着色等任务


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散或优化的3D高斯溅射编辑方法存在速度慢、破坏原始资产身份、缺乏细粒度控制等问题，需要一种交互式的编辑解决方案

Method: 开发了状态感知的前馈模型，直接预测紧凑特征丰富的高斯表示属性更新，利用测试时训练创建状态感知的迭代工作流程

Result: 单一架构能够执行多种编辑任务，包括高保真局部细节细化、局部绘制和一致全局重新着色，所有操作都达到交互速度
...
Method: 提出GOLA框架：1) 使用奇异值分解量化秩重要性，冻结关键秩保留预训练先验；2) 将冗余秩聚类分组；3) 设计组间正交约束策略，强制不同组学习互补特征，减少信息冗余。

Result: GOLA有效减少参数冗余并增强特征表示能力，在四个基准数据集上显著优于现有最先进方法。

Conclusion: GOLA框架通过结构化参数学习和组正交约束，有效利用秩空间，解决了RGB-T跟踪中低秩适应的冗余问题，提升了模型适应性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统SfM处理时序和非时序图像数据耗时且计算昂贵，且大量公开数据因相机姿态变化不足、场景元素遮挡和噪声数据而不适合处理

Method: 提出PoolNet深度学习框架，支持帧级和场景级验证野外数据，能够区分适合SfM处理的场景与不适合的场景

Result: 模型成功区分适合SfM处理的场景，并显著减少获取SfM数据所需时间，优于现有最先进算法

Conclusion: PoolNet为解决SfM数据预处理问题提供了高效解决方案，能够快速筛选适合处理的图像数据，提高SfM处理效率
...
TL;DR: ShaRP是一种改进的基于注意力的剪枝框架，通过集成分段感知因果掩码、位置去偏和令牌去重，能够在浅层解码器有效剪枝视觉令牌，加速视频大语言模型推理而不需要重新训练。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在预填充阶段面临高计算负载问题，因为需要处理大量视觉令牌。现有的基于注意力的剪枝方法在浅层解码器使用时会导致显著性能下降，尤其是在高压缩率下。作者认为注意力剪枝本身具有识别最相关视觉令牌的潜力，但在浅层解码器的有效性受到位置编码偏差和信息交互不足等因素的限制。

Method: 提出ShaRP框架，集成三个关键技术：1) 分段感知因果掩码，增强令牌选择能力；2) 位置去偏，减少位置编码偏差的影响；3) 令牌去重，消除冗余令牌。该框架能够在浅层解码器进行有效剪枝，同时在高压缩率下保持稳定性能，且不需要重新训练。

Result: 在多个视频理解基准测试上的广泛实验表明，ShaRP实现了具有竞争力的性能，为加速VLLM推理建立了新的范式。

Conclusion: ShaRP通过改进的注意力剪枝框架，解决了VLLM在浅层解码器剪枝时的性能下降问题，能够在高压缩率下有效加速推理而不需要重新训练，为视频大语言模型的高效推理提供了新的解决方案。
...
TL;DR: LoC-Path是一个高效的病理学多模态大语言模型框架，通过减少全玻片图像中的冗余特征处理，显著降低计算和内存需求，同时保持与现有方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有病理学全玻片图像多模态大语言模型依赖重型玻片级编码器，需要处理数千个图像块特征，计算成本过高。而人类专家主要依赖关键区域进行诊断，研究表明玻片级特征存在强烈的全局和局部冗余，只有少数图像块真正与任务相关。

Method: 提出LoC-Path框架：1) 使用稀疏令牌合并器和MAE预训练重采样器去除局部冗余，将全局冗余的图块令牌压缩为紧凑的玻片级表示集；2) 提出交叉注意力路由适配器和令牌重要性评分器，以计算高效的方式将压缩的视觉表示与语言模型集成。

Result: 大量实验表明，该方法在性能上与现有最先进的全玻片MLLMs相当，同时需要显著更低的计算和内存资源。

Conclusion: 通过重新审视WSI-语言建模范式并利用特征冗余特性，LoC-Path框架提供了一种高效的全玻片图像理解方法，在保持诊断准确性的同时大幅降低计算负担。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频VAE通常只关注重建保真度，而忽略了潜在空间结构对扩散训练的重要性。潜在扩散模型将VAE与扩散主干配对，VAE潜在空间的结构强烈影响扩散训练的难度。

Method: 通过对视频VAE潜在空间进行统计分析，识别出两个对扩散训练至关重要的光谱特性：偏向低频的时空频率谱和由少数模式主导的通道特征谱。为诱导这些特性，提出了两种轻量级、与主干无关的正则化器：局部相关正则化和潜在掩码重建。

Result: 实验表明，提出的光谱结构VAE（SSVAE）在文本到视频生成收敛速度上实现了3倍加速，视频奖励提升了10%，优于强大的开源VAE。

Conclusion: SSVAE通过优化VAE潜在空间的光谱结构，显著提升了扩散模型的训练效率和生成质量，为视频生成任务提供了有效的解决方案。
...
Method: 提出Dynamic Prior方法，结合视觉语言模型的强大推理能力和SAM2的细粒度空间分割能力，无需任务特定训练即可鲁棒识别动态物体，并可无缝集成到最先进的相机姿态优化、深度重建和4D轨迹估计流程中。

Result: 在合成和真实世界视频上的大量实验表明，该方法不仅在运动分割上达到最先进性能，还显著提升了结构3D理解的准确性和鲁棒性。

Conclusion: Dynamic Prior方法通过利用预训练视觉语言模型和分割模型的能力，无需特定训练即可有效处理动态物体，为野外视频的3D场景理解提供了更准确和鲁棒的解决方案。
...
TL;DR: 提出基于遗传算法的参数优化框架，自动优化SGBM和WLS立体匹配算法参数，用于无人机树木测距，在保持处理效率的同时显著提升精度。


<details>
  <summary>Details</summary>
Motivation: 传统立体匹配算法（SGBM+WLS）在无人机应用中具有速度优势（约0.5秒/帧），但需要精细的参数调优。手动调参耗时且难以获得最优配置，限制了在资源受限无人机系统中的应用效果。

Method: 提出基于遗传算法（GA）的参数优化框架，系统搜索SGBM和WLS算法的最优参数配置。该方法自动优化参数，无需人工调参，适用于资源受限的无人机系统。

Result: 实验结果表明，GA优化方法相比基线配置：均方误差降低42.86%，峰值信噪比提高8.47%，结构相似性提高28.52%。在不同成像条件下表现出优异的泛化性能。

Conclusion: 提出的GA参数优化框架为无人机林业应用提供了实用的解决方案，在保持处理效率的同时显著提升了立体匹配精度，实现了自动化参数优化，具有实际应用价值。
...
Method: 集成YOLO目标检测与半全局块匹配(SGBM)立体视觉框架，仅使用立体相机输入进行枝条检测和深度估计

Result: YOLO在枝条分割上表现优于Mask R-CNN，达到82.0% mAPmask50-95；系统在2米操作范围内准确定位枝条，每帧处理时间小于1秒

Conclusion: 证明了低成本自主修剪系统的可行性，能提高林业作业的安全性和效率
...
TL;DR: 该研究评估了无人机林业操作中不同深度学习方法在三种分辨率下的树木枝干分割性能，建立了多分辨率基准，为嵌入式林业系统的精度-效率权衡提供指导。


<details>
  <summary>Details</summary>
Motivation: 无人机自主林业操作需要快速精确的树木枝干分割，以在变化的像素分辨率和操作条件下实现安全导航和自动化修剪。

Method: 使用Urban Street Tree Dataset，在三种分辨率（256x256、512x512、1024x1024）下评估不同深度学习方法，采用标准指标（IoU、Dice）和专门指标（TS-IoU、CPR），测试了22种配置。

Result: 在256x256分辨率下，U-Net with MiT-B4表现最佳；在512x512下，MiT-B4在IoU、Dice、TS-IoU和Boundary-F1上领先；在1024x1024下，U-Net+MiT-B3在IoU/Dice和精度上表现最好，而U-Net++在边界质量上最优。PSPNet是最高效选项，但性能有所降低。

Conclusion: 研究结果为嵌入式林业系统的精度-效率权衡建立了多分辨率基准，为实际应用中的模型选择提供了指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型在结合视觉语言模型和扩散模型时，难以充分平衡信息交互与灵活实现，因为两者表示差异巨大。视觉语言模型各层包含从低层细节到高层语义的丰富层次信息，但现有方法未能充分利用这些信息。

Method: 提出ParaUni方法：1）并行提取视觉语言模型各层特征；2）通过层集成模块(LIM)高效整合细粒度细节和语义抽象，为扩散模型提供融合表示条件；3）设计层动态调整机制(LDAM)，利用强化学习根据各层对不同奖励的响应差异进行动态调整。

Result: 大量实验表明，ParaUni通过利用互补的多层特征显著提高了生成质量，并在强化学习阶段展现出多奖励提升的强大潜力。

Conclusion: ParaUni通过并行特征提取和层动态调整机制，有效解决了统一多模态模型中信息交互与灵活实现的平衡问题，为视觉生成任务提供了更优的解决方案。
...
TL;DR: TED-4DGS：一种基于时间激活和嵌入的变形方案，用于动态3D高斯泼溅的率失真优化压缩，在多个真实数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 动态3D高斯泼溅（4DGS）在压缩方面存在不足：现有方法要么使用过度指定、寿命短的4D高斯基元，要么使用缺乏显式时间控制的变形方案。需要设计更紧凑高效的变形方案和率失真优化压缩策略。

Method: 基于稀疏锚点的3DGS表示，每个锚点分配可学习的时间激活参数控制出现和消失，轻量级锚点时间嵌入查询共享变形库产生锚点特定变形。采用INR超先验建模锚点属性分布，通道自回归模型捕获锚点内相关性。

Result: 在多个真实数据集上实现了最先进的率失真性能，是动态3DGS表示率失真优化压缩框架的首次尝试之一。

Conclusion: TED-4DGS统一了两种动态3DGS方法的优势，通过时间激活锚点表示和率失真优化压缩，为动态场景表示提供了高效紧凑的解决方案。
...
TL;DR: 本文开发了专门用于泰国朱拉隆功大学的建筑识别数据集CUBR，并在无线自组织联邦学习场景中验证了Vision Transformer模型，结果显示WAFL训练比自训练获得更高准确率。


<details>
  <summary>Details</summary>
Motivation: 随着边缘设备性能提升，边缘训练成为趋势。无线自组织联邦学习（WAFL）通过设备间通信实现协作学习，但作为任务导向的传感器系统，需要为每个具体任务构建专门的数据集。本文针对泰国朱拉隆功大学建筑识别任务开发专门数据集。

Method: 开发了朱拉隆功大学建筑识别数据集（CUBR），作为泰国案例研究。在WAFL-ViT（Vision Transformer）框架下进行实验，比较WAFL场景与自训练场景的性能差异。

Result: 实验结果表明，在WAFL场景下训练比自训练场景获得更高的准确率。数据集已公开在GitHub仓库中供研究使用。

Conclusion: 针对特定任务构建专门数据集对于WAFL-ViT等任务导向系统至关重要。WAFL协作训练相比单独设备自训练能获得更好的性能表现，为边缘设备上的联邦学习应用提供了实证支持。
...
TL;DR: EmoStyle是一个情感图像风格化框架，通过构建数据集、情感内容推理器和风格量化器，实现基于情感的艺术风格应用，同时保持内容一致性。


<details>
  <summary>Details</summary>
Motivation: 现有图像风格化方法主要关注视觉外观变换，但忽略了艺术风格所承载的情感影响。为了弥补这一差距，需要开发能够唤起特定情感同时保持内容一致性的图像风格化方法。

Method: 1. 构建EmoStyleSet数据集：从ArtEmis中提取内容-情感-风格化图像三元组；2. 设计情感内容推理器：自适应整合情感线索与内容，学习连贯的风格查询；3. 开发风格量化器：将连续风格特征转换为情感相关的码本条目。

Result: 定性和定量评估（包括用户研究）表明，EmoStyle在保持内容一致性的同时增强了情感表现力。学习到的情感感知风格字典可适应其他生成任务，展示了更广泛应用的潜力。

Conclusion: 该工作为情感驱动的图像风格化奠定了基础，扩展了AI生成艺术的创作潜力，建立了情感图像风格化这一新任务的研究框架。
...
Method: UniFS包含三个关键模块：跨模态频率融合模块、基于自适应掩码的提示学习模块、双分支互补细化模块。这些模块协同工作，从不同的k空间欠采样模式中提取领域不变特征，同时自适应地处理模式变化。特别引入了自适应提示引导的频率融合模块进行k空间学习。

Result: 在BraTS和HCP数据集上，针对多种k空间欠采样模式和加速因子（包括未见过的模式）进行评估，实验结果表明UniFS在多种场景下都达到了最先进的性能。

Conclusion: UniFS能够有效处理多种k空间欠采样模式而无需重新训练，通过统一的频率-空间融合框架显著提升了模型的泛化性能，为多对比度MR重建提供了实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统中仅依赖点云数据进行罕见物体检测仍具挑战性，虽然视觉语言模型在图像理解方面表现出色，但其通过智能数据挖掘提升3D物体检测的潜力尚未充分探索。

Method: 提出跨模态框架，结合物体检测、语义特征提取、降维和多方面异常检测技术，形成可解释的管道系统识别罕见关键物体。结合孤立森林和t-SNE异常检测方法与基于概念的过滤，有效识别语义上有意义的罕见物体。

Result: 在nuScenes数据集上的实验表明，该概念引导的数据挖掘策略显著提升了3D物体检测模型的性能，仅使用少量训练数据即可实现，特别是在拖车和自行车等挑战性类别上相比随机数据有明显改进。

Conclusion: 该方法能够有效提取和标注目标罕见物体概念（如工程车辆、摩托车、障碍物），大幅减少标注负担并聚焦最有价值的训练样本，对安全关键自动驾驶系统的高效数据集管理具有重要意义。
...
TL;DR: AVP提出了一种主动视频感知框架，通过迭代的计划-观察-反思过程，让MLLM智能体主动决定观察视频的什么内容、何时观察以及在哪里观察，从而高效提取查询相关的证据，显著提升长视频理解性能并大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有长视频理解方法依赖查询无关的视频描述器，这会浪费计算资源在不相关内容上，并模糊细粒度的时空信息。受主动感知理论启发，作者认为长视频理解智能体应该主动决定观察什么、何时观察以及在哪里观察，并持续评估当前观察是否足以回答查询。

Method: AVP采用证据寻求框架，将视频视为交互环境，直接从像素中获取紧凑的查询相关证据。具体采用迭代的计划-观察-反思过程：计划器提出有针对性的视频交互，观察器执行这些交互以提取带时间戳的证据，反思器评估证据对查询的充分性，决定停止回答或触发进一步观察。

Result: 在五个长视频理解基准测试中，AVP实现了最高性能并显著改进。特别地，AVP在平均准确率上比最佳智能体方法高出5.7%，同时仅需要18.4%的推理时间和12.4%的输入标记。

Conclusion: AVP通过主动感知方法有效解决了长视频理解中的计算效率和信息相关性问题，证明了让智能体主动决定观察策略的重要性，为视频理解领域提供了新的高效框架。
...
TL;DR: 本文提出了一种基于小波时域一致性场的水下视频增强方法WaterWave，通过隐式表示方式在无配对数据条件下实现时域一致性增强，显著提升单帧增强方法的视频质量。


<details>
  <summary>Details</summary>
Motivation: 水下视频对获取困难，现有方法多采用单帧增强模型逐帧处理，缺乏时域一致性。本文从局部时域频率角度重新思考自然视频中的时域流形，观察到动态场景中的时域一致性先验。

Method: 提出基于小波的时域一致性场WaterWave，采用隐式表示方式增强视频信号。在时域一致性先验约束下，渐进式过滤和衰减不一致分量，同时保留运动细节和场景。设计了水下流校正模块，考虑水下传输特性修正估计的光流。

Result: 实验表明WaterWave显著提升了使用单帧水下增强方法生成的视频质量。在下游水下跟踪任务（如UOSTrack和MAT）中表现出高潜力，精确度分别比原始视频提升19.7%和9.7%。

Conclusion: 通过利用时域一致性先验和隐式表示，WaterWave方法有效解决了水下视频增强中的时域不一致问题，为无配对数据条件下的水下视频处理提供了有效解决方案。
...
### [35] [Zoom in, Click out: Unlocking and Evaluating the Potential of Zooming for GUI Grounding](https://arxiv.org/abs/2512.05941)
...
Method: 提出ZoomClick方法，无需训练，通过表征缩放的四个关键属性（预缩放、深度、收缩大小、最小裁剪大小），解锁其动态空间聚焦和自适应上下文切换的能力。

Result: 方法显著提升通用视觉语言模型和专用GUI grounding模型的性能，在多个主流基准测试中达到最先进水平，例如UI-Venus-72B在ScreenSpot-Pro上达到73.1%的成功率。还提出了GUIZoom-Bench基准用于评估模型对缩放的适应性。

Conclusion: 缩放是GUI grounding的强大先验知识，ZoomClick方法无需训练即可显著提升性能，提出的GUIZoom-Bench基准将推动未来在GUI grounding任务中改进缩放技术的研究。
...
TL;DR: Know-Show是一个评估视频语言模型时空基础推理能力的新基准，包含五个互补场景，揭示了现有模型在细粒度手物交互等任务上的显著差距，并提出了GRAM训练免费插件来增强模型的细粒度基础能力。


<details>
  <summary>Details</summary>
Motivation: 当前大型视频语言模型在多模态理解方面取得了显著进展，但其推理在空间和时间上的基础性仍然较弱。需要建立一个统一的评估框架来评估模型的时空基础推理能力，即模型在推理动作及其语义的同时，能够将其推断基于视觉和时间证据的能力。

Method: 1. 构建Know-Show基准：基于Charades、Action Genome和Ego4D数据集，包含2.5K个人工编写的问题，涵盖空间（人物、物体、人物-物体、手-物体）和时间维度的五个互补场景。
2. 提出GRAM训练免费插件：通过基于注意力的视频令牌选择和显式时间戳编码，增强视频语言模型的细粒度基础能力。

Result: 1. Know-Show基准揭示了当前视频语言模型（Qwen、VideoLLaVA、GPT-4o、Gemini等）与人类推理之间的显著差距，特别是在细粒度手物交互方面。
2. 模型难以"展示它们知道的内容"，反之亦然。
3. GRAM插件能够有效增强模型的细粒度基础能力。

Conclusion: Know-Show为评估视频语言理解中的基础推理建立了统一标准，并为开发可解释和可靠的多模态推理系统提供了见解。该基准和GRAM插件将推动视频语言模型在时空基础推理方面的发展。
...
<details>
  <summary>Details</summary>
Motivation: 多模态情感分析面临模态对齐和融合的挑战。现有方法往往单独处理对齐或融合问题，导致性能和效率受限。需要同时解决这两个问题以获得更全面的情感理解。

Method: 提出DashFusion框架：1) 双流对齐模块通过时间和语义对齐同步多模态特征；2) 监督对比学习利用标签信息细化模态特征；3) 分层瓶颈融合通过压缩瓶颈token逐步整合多模态信息。

Result: 在CMU-MOSI、CMU-MOSEI和CH-SIMS三个数据集上的实验表明，DashFusion在各种指标上均达到最先进性能。消融研究证实了所提对齐和融合技术的有效性。

Conclusion: DashFusion通过同时解决对齐和融合问题，在性能和计算效率之间取得平衡，为多模态情感分析提供了一个有效的解决方案。
...
TL;DR: 该论文基于"枯叶"模型开发了一个贝叶斯理想观察者，用于图像分割任务，为人类和计算机视觉算法提供了性能上限基准。


<details>
  <summary>Details</summary>
Motivation: 人类视觉环境中物体相互遮挡的现象普遍存在，需要开发能够模拟这种遮挡关系的生成模型来研究图像分割问题。

Method: 基于枯叶模型（通过分层叠加物体生成图像的生成模型），推导贝叶斯理想观察者来计算像素分区的后验概率，并详细解释计算步骤和实际应用可行性因素。

Result: 建立了枯叶图像模型及其关联的理想观察者理论框架，可用于研究有限像素集合的分割决策。

Conclusion: 该模型为图像分割任务提供了理论性能上限，可作为人类视觉和计算机视觉算法的比较基准。
...
TL;DR: CG-VLM是一个无需训练、在推理时使用的框架，通过博弈论可解释性将视觉-语言模型中的注意力从语言先验重新导向视觉证据，减少对象幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型视觉-语言模型存在"文本惯性"问题，注意力会从视觉证据漂移到语言先验，导致对象幻觉。现有解码策略只在输出logits层面干预，无法纠正内部推理漂移，而基于启发式头部抑制或全局导向向量的内部控制方法缺乏理论依据。

Method: CG-VLM包含两个核心模块：1）基于Harsanyi交互的认知需求传感器，估计瞬时视觉-文本协同作用，识别需要视觉接地的时刻；2）聚焦共识诱导模块，在注意力崩溃为文本先验之前，选择性地将中间层注意力重新导向视觉token。

Result: CG-VLM在POPE和CHAIR基准测试中，在InstructBLIP、LLaVA、Qwen-VL和mPLUG等模型上取得了最先进的结果，同时保持了通用能力。

Conclusion: 研究表明，token级别的感知能够实现精确、上下文感知的干预，而不会损害基础知识，为视觉-语言模型提供了有效的推理时控制框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集在精确瞬态属性控制下缺乏足够的保真度，无法将稳定身份与瞬态属性解耦，限制了姿势、表情和场景构成的结构化控制，从而制约了可靠的序列合成。

Method: 1) 引入2K-Characters-10K-Stories多模态风格化叙事数据集；2) 采用人类参与循环管道，结合专家验证的角色模板和LLM引导的叙事规划；3) 设计解耦控制方案分离持久身份与瞬态属性；4) 实施质量门控循环，集成MMLM评估、自动提示调整和局部图像编辑。

Result: 在该数据集上微调的模型在生成视觉叙事方面达到了与闭源模型相当的性能，实现了像素级一致性。

Conclusion: 该工作通过大规模独特身份与显式解耦控制信号的配对，为序列身份一致性提供了首个高质量数据集，推动了可控视觉叙事的发展。
...
TL;DR: ProPhy是一个渐进式物理对齐框架，通过两阶段物理专家混合机制实现显式的物理感知条件生成，解决了现有视频生成模型在处理大规模复杂动态时的物理一致性问题。
...
Method: 提出ProPhy框架，采用两阶段物理专家混合机制：语义专家从文本描述中推断语义级物理原理，细化专家捕获标记级物理动态。同时引入物理对齐策略，将视觉语言模型的物理推理能力转移到细化专家中。
...
TL;DR: MedDIFT是一个无需训练的3D医学图像对应框架，利用预训练扩散模型的多尺度特征作为体素描述符，在肺部CT数据集上达到与最先进学习模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像配准通常依赖局部强度相似性度量，无法捕捉全局语义结构，在低对比度或解剖结构变化区域容易产生误匹配。扩散模型的中间表示编码了丰富的几何和语义信息，为解决这一问题提供了新思路。

Method: MedDIFT利用预训练的潜在医学扩散模型，提取多尺度特征作为体素描述符，通过余弦相似度进行匹配，可选局部搜索先验。该方法无需特定任务训练，直接使用预训练模型特征。

Result: 在公开肺部CT数据集上，MedDIFT达到与最先进学习模型UniGradICON相当的对应精度，优于传统B样条配准方法。消融实验证实多级特征融合和适度扩散噪声能提升性能。

Conclusion: 扩散模型的中间特征为医学图像对应提供了丰富的描述符，无需训练即可实现高性能配准，为医学图像分析提供了新的无监督对应框架。
...
Method: 提出蒙皮无关方法：1）独立估计顶点位置（低频姿态形状）和顶点法线（高频皱纹细节）；2）将顶点属性编码为渲染纹理图像，通过2D图像传输实现3D服装变形；3）利用预训练图像模型恢复皱纹细节；4）提出多模态融合整合两种频率约束。

Result: 大量实验表明，该方法在各种服装类型上显著提高了动画质量，比现有最先进方法恢复了更精细的皱纹。

Conclusion: 该方法通过解耦频率模态、利用图像传输和预训练模型，有效解决了现有蒙皮方法的问题，实现了高质量3D服装变形动画。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于语言模型的感知通用方法虽然在各种任务中取得了最佳效果，但依赖于自回归的下一个令牌预测，速度较慢。需要开发既能保持准确性又能显著加速推理的方法。

Method: 提出Fast SceneScript，采用多令牌预测减少自回归迭代次数；引入自推测解码和置信度引导解码来过滤不可靠令牌；设计参数高效机制降低多令牌预测的参数开销。

Result: 在ASE和Structured3D基准测试中，Fast SceneScript每次解码器推理步骤可生成多达9个令牌而不影响准确性，仅增加约7.5%的额外参数。

Conclusion: Fast SceneScript通过多令牌预测和置信度引导解码，在保持3D场景布局估计准确性的同时显著加速推理，为结构化语言模型提供了高效解决方案。
...
TL;DR: 本文提出了一种基于最优传输的无监督训练框架，用于训练任意ISP架构，解决了传统方法对大规模配对raw-to-sRGB数据集的依赖问题。


<details>
  <summary>Details</summary>
Motivation: 传统学习型ISP管道依赖大规模配对raw-to-sRGB数据集，这些数据获取成本高昂，成为显著瓶颈。需要开发不依赖配对数据的训练方法。

Method: 提出基于最优传输的无监督训练框架，首次将非平衡最优传输应用于跨域转换任务。框架包含"专家判别器委员会"作为混合对抗正则化器，提供专门梯度来纠正ISP故障模式。

Result: 在配对模式下训练时，性能超过原始配对方法；在非配对模式下，定量和定性性能可与原始配对训练方法相媲美甚至超越。

Conclusion: 提出的基于UOT的无监督训练框架有效解决了ISP训练对配对数据的依赖问题，在配对和非配对模式下均表现出优越性能，为ISP训练提供了更灵活高效的解决方案。
...
Method: 使用自监督方法，通过分类EXIF分类标签（相机型号、场景类型）和排序连续EXIF标签（焦距、光圈值）训练特征提取器，然后进行单类检测（高斯混合模型）和二元检测（高频残差分类）

Result: 在多种生成模型上的实验表明，该方法显著推进了技术前沿，对真实世界样本具有强泛化能力，对常见良性图像扰动具有鲁棒性

Conclusion: 基于EXIF元数据的自监督方法为AI生成图像检测提供了有效的跨模型解决方案，通过利用数字摄影固有特征实现了优越的检测性能
...
Method: 1. 非对称增强去噪蒸馏(A2D2)：通过质量和重要性加权的深度特征损失，将几何知识从干净图像教师模型转移到混合噪声学生模型；2. 3D感知一致性匹配(CM3D)：将3D MGIoU整合到匹配分数中，改进预测到真值的分配；3. 置信度门控3D推理(CGI3D)：通过将昂贵的3D回归限制在高置信度区域来加速检测。

Result: 在KITTI和Waymo上达到SOTA精度，在Rope3D上获得最佳报告的汽车AP，同时运行速度比先前高精度方法快3.6倍。
...
TL;DR: 本文系统分析了基于世界模型的测试时验证器在空间推理任务中的表现，揭示了其校准不足、存在系统性动作偏差等问题，并提出了基于空间断言的验证框架ViSA来改进验证过程。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在需要多视角理解和具身视角转换的空间推理任务中存在局限。MindJourney等近期方法通过测试时缩放（让世界模型想象动作条件轨迹，然后通过启发式验证器选择有用视角）来弥补这一差距，但这些验证器的实际效果和可靠性尚未得到系统评估。

Method: 1. 系统分析现有测试时验证器在基准测试中的行为，揭示其校准不足和系统性偏差；2. 提出基于空间断言的验证框架ViSA，将测试时奖励建立在可验证的、帧锚定的微观主张上；3. 在SAT-Real和MMSI-Bench等基准上进行实验评估。

Result: 1. MindJourney的验证器几乎没有提供有意义的校准，随机评分通常同样能降低答案熵；2. ViSA框架在SAT-Real基准上持续改善了空间推理，并通过更平衡的探索行为纠正了轨迹选择偏差；3. 在更具挑战性的MMSI-Bench上，包括ViSA在内的所有验证器都未能实现一致的缩放，表明当前世界模型形成了信息瓶颈。

Conclusion: 研究揭示了基于世界模型的测试时验证在空间推理中的"坏、好、丑"三个方面：验证器校准不足且存在系统性偏差（坏）；基于空间断言的验证框架ViSA能改善验证效果（好）；但在复杂任务中，当前世界模型的信息瓶颈限制了验证器的效果（丑）。
...
Method: 将一维Phase-OTDR数据通过Gramian Angular Difference Field、Gramian Angular Summation Field和Recurrence Plot技术转换为灰度图像，然后组合成多通道RGB表示，利用迁移学习模型（EfficientNetB0和DenseNet121）进行分类。

Result: 使用公开的Phase-OTDR数据集，该方法实现了98.84%（EfficientNetB0）和98.24%（DenseNet121）的高分类准确率，5折交叉验证的测试准确率分别为99.07%和98.68%。

Conclusion: 基于图像的分析方法在解释复杂光纤传感数据方面具有变革性潜力，显著提高了光纤监测系统的准确性和可靠性，同时减少了数据集大小并提高了分析效率。
...
### [52] [World Models That Know When They Don't Know: Controllable Video Generation with Calibrated Uncertainty](https://arxiv.org/abs/2512.05927)
...
<details>
  <summary>Details</summary>
Motivation: 当前可控视频模型存在幻觉问题（生成与物理现实不符的未来帧），在机器人策略评估和规划等任务中带来严重问题。现有视频模型缺乏评估和表达置信度的能力，阻碍了幻觉缓解。

Method: 提出了C3不确定性量化方法，包含三个核心创新：1）通过严格适当评分规则训练视频模型以获得正确性和校准性；2）在潜在空间中估计视频模型的不确定性，避免像素空间方法的不稳定性和高昂训练成本；3）将密集潜在空间不确定性映射到可解释的像素级不确定性，提供高分辨率不确定性热图。

Result: 在大型机器人学习数据集（Bridge和DROID）和真实世界评估中，该方法不仅能在训练分布内提供校准的不确定性估计，还能实现有效的分布外检测。

Conclusion: C3方法为可控视频模型提供了可靠的不确定性量化框架，能够精确定位生成视频中的不可信区域，有助于缓解幻觉问题并提高模型的可信度。
...
TL;DR: ZRNet：一种物理信息框架，通过Zernike系数预测和光学图像恢复联合处理显微镜中的光学像差问题，利用Zernike多项式建模波前畸变物理关系
...
Method: 提出ZRNet框架，包含Zernike Graph模块显式建模Zernike多项式间的物理关系，以及频率感知对齐损失函数在傅里叶域对齐Zernike系数预测和图像特征
...
TL;DR: 该论文通过生成六个合成数据集来量化背景相关性对交通标志识别的影响，使用可解释AI方法分析分类器何时以及多大程度上依赖背景特征而非目标物体本身。


<details>
  <summary>Details</summary>
Motivation: 当前可解释AI方法（如SHAP和GradCAM）虽然能分析输入特征的重要性，但难以定量测试分类器是否过度依赖虚假相关性（如背景像素）。真实数据中的相关性难以避免且难以区分是否合理，而合成数据可以精确控制相关性但缺乏真实感量化。

Method: 系统生成六个合成数据集用于交通标志识别任务，这些数据集仅在相机变化程度和背景相关性方面有所不同。通过控制变量方法，量化背景相关性、相机变化水平和交通标志形状对分类性能和背景特征重要性的独立影响。

Result: 研究结果量化了背景特征在何时以及多大程度上获得重要性以支持分类任务，这取决于训练域的变化。具体展示了背景相关性如何影响分类器的决策过程。

Conclusion: 通过合成数据集的系统实验，能够定量分析背景相关性对分类器行为的影响，为可解释AI方法的评估提供了更可靠的定量框架，有助于区分健康分类器与过度依赖虚假相关性的分类器。
...
### [55] [Manifold-Aware Point Cloud Completion via Geodesic-Attentive Hierarchical Feature Learning](https://arxiv.org/abs/2512.05710)
...
<details>
  <summary>Details</summary>
Motivation: 现有点云补全方法主要依赖欧几里得距离，忽略了点云内在的非线性几何结构，导致几何一致性不足和语义模糊问题。需要一种能够显式捕捉点云流形拓扑结构的方法来改善补全质量。
...
Conclusion: 通过显式整合非线性几何信息到特征学习流程中，提出的流形感知框架能够生成语义更连贯、结构更保真的点云补全结果，为点云补全领域提供了新的研究方向。
...
### [56] [Distilling Expert Surgical Knowledge: How to train local surgical VLMs for anatomy explanation in Complete Mesocolic Excision](https://arxiv.org/abs/2512.05740)
*Lennart Maack,Julia-Kristin Graß,Lisa-Marie Toscha,Nathaniel Melling,Alexander Schlaefer*

Main category: cs.CV

TL;DR: 提出一个隐私保护框架，将大型通用LLM的知识蒸馏到高效本地VLM中，用于手术场景理解，避免患者数据泄露。


<details>
  <summary>Details</summary>
Motivation: 当前视觉大语言模型在手术领域特定场景理解方面存在不足，且需要本地可部署模型以避免患者数据泄露到外部大型VLM。

Method: 通过仅使用文本上下文和二进制分割掩码（不含敏感图像）提示教师LLM生成专家监督数据集，用于本地VLM的监督微调和直接偏好优化。

Result: 评估证实，使用生成的数据集微调VLM显著提高了手术领域知识，相比基础VLM有大幅提升。

Conclusion: 该工作验证了一种数据高效且符合隐私要求的方法，用于训练手术领域优化的本地可部署VLM进行手术场景理解。
...
### [57] [HQ-DM: Single Hadamard Transformation-Based Quantization-Aware Training for Low-Bit Diffusion Models](https://arxiv.org/abs/2512.05746)
*Shizhuo Mao,Hongtao Zou,Qihu Xie,Song Chen,Yi Kang*

Main category: cs.CV

TL;DR: HQ-DM提出了一种基于单哈达玛变换的量化感知训练框架，有效减少扩散模型激活矩阵中的异常值，在低比特量化下保持性能，相比现有方法显著提升图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成领域应用广泛，但计算和内存成本高，部署困难。模型量化是减少存储开销和加速推理的可行方案，但现有量化方法难以处理激活矩阵中的异常值，导致低比特量化下性能显著下降。

Method: 提出HQ-DM量化感知训练框架，采用单哈达玛变换处理激活矩阵，有效减少激活异常值，同时支持INT卷积操作并防止权重异常值放大。相比传统的双哈达玛变换，该方法更具优势。

Result: 在ImageNet 256x256数据集上使用LDM-4模型进行条件生成，W4A4和W4A3量化方案相比现有最优方法分别提升Inception Score 12.8%和467.73%。

Conclusion: HQ-DM框架通过单哈达玛变换有效解决了扩散模型量化中的激活异常值问题，在保持模型性能的同时显著降低了计算和存储成本，为扩散模型的部署提供了实用解决方案。
...
### [58] [USV: Unified Sparsification for Accelerating Video Diffusion Models](https://arxiv.org/abs/2512.05754)
*Xinjian Wu,Hongmei Wang,Yuan Zhou,Qinglin Lu*

Main category: cs.CV

TL;DR: USV提出统一稀疏化框架，通过联合优化注意力稀疏化、token合并和去噪步骤减少，实现视频扩散模型的高效加速


<details>
  <summary>Details</summary>
Motivation: 现有高保真视频扩散模型面临两个主要冗余源：全局时空注意力的二次复杂度和长迭代去噪轨迹的计算开销。现有加速方法通常只针对单一维度，存在收益递减问题

Method: USV是一个端到端可训练框架，学习动态的、数据和时间步依赖的稀疏化策略，联合协调模型内部计算和采样过程的稀疏化，包括剪枝冗余注意力连接、自适应合并语义相似token、减少去噪步骤

Result: 在大规模视频生成基准测试中，USV实现了去噪过程高达83.3%的加速和22.7%的端到端加速，同时保持高视觉保真度

Conclusion: 统一动态稀疏化为实现高效高质量视频生成提供了实用路径，多维协同设计使先前独立的加速策略能够相互强化
...
TL;DR: FNOpt是一个自监督的布料模拟框架，将时间积分作为优化问题，使用傅里叶神经算子参数化的神经优化器，无需重新训练即可在不同网格分辨率和运动模式上实现稳定准确的模拟。


<details>
  <summary>Details</summary>
Motivation: 现有神经模拟器通常依赖大量地面真值数据或牺牲细节精度，在不同分辨率和运动模式上泛化能力差。需要一种能够跨分辨率可靠模拟布料物理动态的方法。

Method: 将时间积分公式化为优化问题，训练一个由傅里叶神经算子参数化的分辨率无关神经优化器。仅使用粗网格上的物理损失进行自监督训练，无需地面真值数据。

Result: FNOpt能够模拟物理合理的布料动态，在不同网格分辨率和运动模式上实现稳定准确的展开，捕捉细尺度皱纹并保持模拟稳定性。在布料模拟基准测试中，在分布外设置上优于先前基于学习的方法。

Conclusion: 基于FNO的元优化是先前布料神经模拟器的有吸引力的替代方案，减少了策划数据的需求并提高了跨分辨率可靠性。
...
TL;DR: 提出使用离散拉普拉斯算子进行曲率正则化，在仅有5%测量数据的情况下实现比标准变分自编码器高18.1%的3D场景重建精度


<details>
  <summary>Details</summary>
Motivation: 当深度传感器仅提供5%所需测量数据时，稀疏重建会引入几何误差，这对自动驾驶车辆和机器人来说是不可容忍的。现有几何深度学习通常假设组合多个几何约束能提升性能，但本文挑战这一隐含假设。

Method: 通过离散拉普拉斯算子进行曲率正则化，该算子提供稳定的梯度和噪声抑制能力，仅增加15%的训练开销且零推理成本。

Result: 在稀疏测量条件下，该方法比标准变分自编码器重建精度提高18.1%，证明单一精心设计的正则化项不仅匹配甚至超过复杂多术语公式的效果。

Conclusion: 精心设计的单一正则化项可以超越复杂的多约束组合方法，离散拉普拉斯算子在稀疏3D重建中表现出色，代码和模型已开源。
...
TL;DR: 本文提出CCVD模型，解决定制化文本到视频生成中的持续学习问题，防止遗忘旧概念和忽视新概念
...
Method: 开发了持续定制化视频扩散模型(CCVD)，包含概念特定属性保留模块、任务感知概念聚合策略，以及可控条件合成方法，通过层特定区域注意力引导的噪声估计来增强区域特征

Result: 大量实验比较表明，CCVD在定制化文本到视频生成任务中优于现有模型

Conclusion: CCVD模型能够有效解决持续学习中的遗忘和概念忽视问题，实现对新概念的连续学习，在多种文本到视频生成任务中表现优异
...
<details>
  <summary>Details</summary>
Motivation: 现有阿尔茨海默病分类框架在多中心研究中面临挑战：忽视站点间异质性、缺乏不确定性量化机制，限制了模型的鲁棒性和临床应用性。

Method: 提出不确定性引导的联邦域适应框架，使用自注意力transformer提取多模板ROI特征，集成不确定性量化指导特征对齐，通过降低不确定样本权重来缓解源-目标分布偏移。

Result: 在ADNI、AIBL、OASIS三个公开数据集上测试，UG-FedDA在AD vs NC、MCI vs AD、NC vs MCI三个分类任务中均取得一致的跨域性能提升。AD vs NC准确率分别为90.54%、89.04%、77.78%；MCI vs AD为80.20%、71.91%、79.73%；NC vs MCI为76.87%、73.91%、83.73%。

Conclusion: 该框架不仅能有效适应多中心数据，还能在严格保护隐私的前提下实现跨站点鲁棒分类，为阿尔茨海默病的早期诊断提供了实用解决方案。
...
TL;DR: 本文提出视觉推理序列攻击（VRSA），通过将有害文本分解为多个相关子图像，诱导多模态大语言模型逐步外化和聚合完整有害意图，评估视觉推理任务中的安全风险。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）因其强大的跨模态理解和生成能力被广泛应用，但更多模态也带来更多被用于越狱攻击的漏洞。先前攻击主要探索文本模态的推理安全风险，而视觉模态中的类似威胁被严重忽视。为全面评估视觉推理任务中的潜在安全风险，需要研究针对视觉模态的越狱攻击方法。

Method: 提出视觉推理序列攻击（VRSA），将原始有害文本分解为多个顺序相关的子图像，诱导MLLMs逐步外化和聚合完整有害意图。包含三个关键技术：1）自适应场景优化，优化与原始有害查询最相关的场景；2）语义连贯补全，结合上下文信息迭代重写每个子文本；3）文本-图像一致性对齐，保持语义一致性。

Result: 实验表明，VRSA在开源和闭源MLLMs（如GPT-4o和Claude-4.5-Sonnet）上相比现有最先进的越狱攻击方法，能够实现更高的攻击成功率。

Conclusion: 视觉模态存在显著的安全风险，VRSA方法有效揭示了MLLMs在视觉推理任务中的脆弱性，强调了需要加强对多模态模型的安全防护。
...
<details>
  <summary>Details</summary>
Motivation: 用户通常在相机显示输出（如8位sRGB JPEG）上进行后期编辑，但RAW编辑更精确灵活。现有RAW重建方法主要优化像素级重建保真度，在不同渲染风格和编辑操作下性能下降。需要一种使重建RAW对编辑更鲁棒的方法。

Method: 提出可插入的编辑感知损失函数，包含模块化可微分图像信号处理器（ISP），模拟真实照片处理流程。训练时，每个ISP模块参数从精心设计的分布中随机采样，模拟实际相机处理变化。损失在sRGB空间中计算，比较通过可微分ISP渲染的地面真实RAW和重建RAW。

Result: 该方法将sRGB重建质量提升1.5-2 dB PSNR（各种编辑条件下）。应用于元数据辅助RAW重建方法时，可针对目标编辑进行微调，获得进一步增益。
...
Result: 实验表明SCAIL实现了最先进的性能，并通过精心策划的数据管道和系统评估基准，推动角色动画向工作室级可靠性和真实感迈进。
...
TL;DR: NICE是一种基于神经隐式表示的颅面模型，用于正颌手术后面部外观的准确预测，通过区域特定的SDF解码器和变形解码器，显著提高了唇部和下巴等关键区域的预测精度。


<details>
  <summary>Details</summary>
Motivation: 正颌手术对于矫正牙颌面骨骼畸形至关重要，但术后面部外观的准确预测仍然具有挑战性。现有方法（生物力学模型、参数化模型和深度学习方法）要么计算效率低，要么无法完全捕捉骨骼运动与面部软组织之间复杂的非线性相互作用。
...
Conclusion: NICE为增强正颌手术的手术规划和患者咨询提供了一个临床可行的工具，能够准确预测术后面部外观。
...
<details>
  <summary>Details</summary>
Motivation: 病理图像弱监督语义分割面临类间同质性、类内异质性和CAM引起的区域收缩问题。现有两阶段方法成本高、对超参数敏感且原型发现与分割学习解耦，限制了效率和效果

Method: 提出免聚类的一阶段可学习原型框架，采用多样性正则化增强形态学类内异质性覆盖，避免传统聚类方法的局限性

Result: 在BCSS-WSSS数据集上实现SOTA性能，mIoU和mDice指标优于先前方法，分割图边界更清晰、误标更少，激活热图显示可学习原型覆盖更广泛互补的类内区域

Conclusion: 提出的免聚类一阶段可学习原型框架有效解决了病理图像弱监督语义分割的关键挑战，在效率和效果上都优于传统两阶段聚类方法
...
<details>
  <summary>Details</summary>
Motivation: 面部识别在可解释性、人口统计偏差、隐私保护、以及对老化、姿态变化、光照变化、遮挡和表情的鲁棒性方面仍面临挑战。隐私法规导致多个数据集质量下降，引发法律、伦理和隐私问题。合成面部数据生成被提出作为有前景的解决方案。

Method: 研究比较了使用不同技术生成的合成面部数据集在面部识别任务中的效果。在8个主流数据集上评估了准确率、Rank-1、Rank-5以及假阳性率为0.01%时的真阳性率，提供了文献中未广泛探讨的比较分析。

Result: 结果表明合成数据能够捕捉现实变化，但强调需要进一步研究以缩小与真实数据的性能差距。扩散模型、GANs和3D模型等技术显示出显著进展，但仍存在挑战。

Conclusion: 合成面部数据在缓解隐私问题、控制面部属性、减轻人口统计偏差以及为基于真实数据训练的模型提供补充数据方面具有潜力，但需要进一步研究来提高合成数据在面部识别任务中的性能表现。
...
TL;DR: 提出一个结合数据驱动和分析建模的交通标志识别合成数据集生成方法，使用GAN生成纹理和渲染实现物理正确的光照，创建包含105500张德国交通标志图像的Synset Signset Germany数据集


<details>
  <summary>Details</summary>
Motivation: 解决交通标志识别任务中训练/测试数据获取的挑战，结合数据驱动和分析建模的优势，生成既真实又可控的合成数据，特别关注新发布（2020年）且相对罕见的德国交通标志

Method: 采用合成管道结合GAN纹理生成（用于污垢和磨损伪影）和分析场景调制（实现物理正确光照），提供详细的参数化控制，生成包含掩码、分割图像和丰富元数据的合成数据集

Result: 创建了Synset Signset Germany数据集，包含105500张图像，涵盖211个不同的德国交通标志类别，包括新发布的罕见标志，在真实世界GTSRB基准测试中评估了真实度，并与最先进的合成数据集CATERED进行比较

Conclusion: 提出的合成方法结合了数据驱动和分析建模的优势，生成的Synset Signset Germany数据集具有高真实度和详细参数化能力，适用于可解释AI和鲁棒性测试等应用
...
TL;DR: 提出EditThinker框架，通过"边思考边编辑"的认知循环提升指令式图像编辑的指令遵循能力


<details>
  <summary>Details</summary>
Motivation: 现有指令式图像编辑方法虽然能生成高质量图像，但单轮成功率有限，主要受随机性和缺乏深思熟虑的限制，需要提升指令遵循能力

Method: 提出深思熟虑的编辑框架，模拟人类认知循环：迭代执行"边思考边编辑"循环（批判结果-精炼指令-重复生成），训练单一MLLM模型EditThinker作为推理引擎，联合生成批判分数、推理过程和精炼指令，使用强化学习对齐思考与编辑

Result: 在四个基准测试上的广泛实验表明，该方法显著提升了任何图像编辑模型的指令遵循能力，且提升幅度很大

Conclusion: 通过模拟人类认知循环的深思熟虑编辑框架，能够有效提升指令式图像编辑的指令遵循能力，将发布数据构建框架、数据集和模型以造福社区
...
TL;DR: 研究开发了一个针对利默里克大学电子与计算机工程系的课程信息问答聊天机器人，通过微调BERT模型在1,203个问答对数据集上取得了良好效果，展示了基础模型在教育领域的适应潜力。


<details>
  <summary>Details</summary>
Motivation: 现有科学问答系统多为通用聊天机器人，缺乏针对特定教育领域的微调基础模型。特别是大学课程材料领域尚无专门的问答模型，本研究旨在填补这一空白。

Method: 构建了包含1,203个问答对的SQuAD格式数据集（基于大学课程手册，结合人工和合成生成），使用PyTorch对BERT模型进行微调，采用精确匹配和F1分数进行评估。

Result: 即使进行适度微调也能显著改善假设构建和知识提取能力，证明了基础模型在教育领域的适应可行性，为开发首个大学课程材料专用问答模型奠定了基础。

Conclusion: 微调BERT模型处理学术问答对能产生有效结果，展示了向大学领域专用问答模型扩展的潜力，为实现自主教育知识系统提供了可能。
...
TL;DR: 该研究探讨了诗歌提示模式作为提示工程工具的价值，通过诗歌创作提示来评估语言模型对著名诗人作品的描述和评价，并测试模型为不同受众改写原创作品的意愿。


<details>
  <summary>Details</summary>
Motivation: 当前提示工程主要关注算法倾向和偏见研究，而创意工作者和学者则利用LLM进行创作和探索写作边界。本研究旨在探索诗歌提示模式作为提示工程新工具的可能性，并评估模型对原创作品的描述、评价及改写意愿。

Method: 提出诗歌提示模式作为提示工程工具的方法论，然后使用诗歌创作提示来评估三个著名诗人模型的描述和评价能力，测试模型为不同受众改写原创作品的意愿和后果。

Result: 研究发现诗歌提示模式可以成为提示工程师工具箱的有用补充，通过诗歌提示能够有效评估模型对诗人作品的描述和评价能力，并揭示模型为适应不同受众而改写原创作品的倾向。

Conclusion: 创意文本提示，特别是诗歌提示模式，为提示工程提供了新的方法论工具，有助于更深入地理解语言模型的创作能力和对原创作品的适应性，为创意写作和模型评估开辟了新途径。
...
TL;DR: 该研究探讨了使用思维链提示工程结合语义搜索和知识图谱来改进大型语言模型生成临床病历的质量，相比传统单次提示方法表现更优。


<details>
  <summary>Details</summary>
Motivation: 随着电子健康记录数据的激增，医生手动编写临床病历耗费大量时间，增加了患者等待时间并可能延误诊断。大型语言模型虽然能生成类似人类撰写的文章，但在临床病历生成方面需要改进。

Method: 研究采用思维链提示工程，结合国际疾病分类代码和基本患者信息作为输入。创新性地将传统思维链方法与语义搜索结果相结合，并注入基于临床本体论构建的知识图谱，以丰富生成临床病历的领域特定知识。

Result: 在CodiEsp测试数据集的六个临床案例上使用GPT-4进行测试，结果显示该方法生成的临床病历质量优于标准的单次提示方法。

Conclusion: 思维链提示工程结合语义搜索和知识图谱能有效提升大型语言模型生成临床病历的质量，为自动化临床文档生成提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然CoT提示结合少样本上下文学习能激发LLM的推理能力，但当预训练知识不足时，这种方法在新颖任务上效果不佳。研究发现CoT示例过多会降低性能，需要找到合适的平衡。

Method: 提出CoT-Recipe方法，在元训练序列中正式调节CoT和非CoT示例的混合比例。使用CoT-ICL Lab框架进行控制实验，验证方法在符号推理任务上的有效性。

Result: 通过CoT-Recipe的精细调节，即使上下文没有CoT示例，Transformer在新颖任务上的准确率也能提升高达300%。在预训练LLM（Qwen2.5系列）上应用，符号推理任务的准确率提升高达130%。

Conclusion: CoT-Recipe方法能有效调节元训练中CoT示例的比例，显著提升LLM在新颖抽象推理任务上的性能，为解决预训练知识不足时的推理问题提供了有效方案。
...
TL;DR: 该研究分析了粉红粘液新闻的特征并提出检测方法，发现LLM攻击会显著降低现有检测系统性能，提出了抗攻击的鲁棒学习框架。


<details>
  <summary>Details</summary>
Motivation: 本地新闻作为2800万美国人的重要信息来源，正面临粉红粘液新闻的威胁。这种低质量、自动生成的文章模仿合法本地报道，需要对其语言、风格和词汇特征进行细粒度分析来检测。

Method: 1. 对粉红粘液内容进行综合研究，揭示其区分性模式；2. 基于这些洞察提出检测策略；3. 研究LLM修改作为新的对抗性攻击向量；4. 引入专门设计的鲁棒学习框架来抵抗LLM攻击。

Result: 1. 发现即使是消费者可访问的LLM也能显著削弱现有检测系统，使F1分数性能下降高达40%；2. 提出的鲁棒学习框架能抵抗LLM攻击，性能提升高达27%。

Conclusion: 粉红粘液新闻对本地新闻生态系统构成严重威胁，LLM攻击加剧了这一挑战。需要专门设计的鲁棒检测框架来应对不断演变的自动化粉红粘液新闻威胁。
...
TL;DR: 混合神经符号方法分析梵语2000年演化，挑战语言简化假设，发现形态复杂性动态重分布而非减少


<details>
  <summary>Details</summary>
Motivation: 挑战语言演化是简化的朴素假设，为形态丰富、低资源语言（梵语）的演化提供新见解，解决数据稀缺问题

Method: 使用弱监督方法，通过100+高精度正则模式生成伪标签微调多语言BERT，采用新颖的置信度加权集成融合符号和神经输出

Result: 在147万词历时语料库中实现52.4%特征检测率，发现梵语形态复杂性动态重分布而非减少，置信度与准确度强相关（r=0.92），校准误差低（ECE=0.043）

Conclusion: 混合神经符号方法为低资源语言演化研究提供可扩展、可解释的框架，梵语复杂性动态重分布而非简化，系统产生良好校准的不确定性估计
...
TL;DR: 该论文研究语言模型评估中的自我偏好偏见问题，发现通过简单的文本扰动可以降低模型识别自身输出的能力从而减少偏见，但完全消除偏见仍面临挑战。


<details>
  <summary>Details</summary>
Motivation: 语言模型作为评估者被广泛使用，但存在自我偏好偏见（倾向于选择自己的输出而非其他模型或人类的输出），这种偏见难以消除，因为前沿语言模型即使在没有标注来源的情况下也能识别自己的输出。

Method: 采用黑盒扰动方法对评估候选文本进行处理，在成对比较中模糊作者身份以降低自我识别能力。具体使用简单的同义词替换等扰动策略。

Result: 简单的扰动（如替换几个词的同义词）可以预测性地减少自我偏好。但当将扰动扩展到更完全地中和候选文本之间的风格差异时，自我偏好会恢复。这表明自我识别和自我偏好可以在多个语义层面发生。

Conclusion: 虽然初步结果有希望，但完全缓解语言模型评估中的自我偏好偏见仍然具有挑战性，因为自我识别和自我偏好发生在多个语义层面。
...
TL;DR: SCRPO是一种自监督训练框架，通过利用LLM自身的批判和精炼能力构建偏好数据集，然后应用偏好学习来提升模型在摘要生成任务中的忠实度，减少幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长文本生成任务（如摘要）中经常出现幻觉问题，即生成与输入上下文不符的内容。现有方法要么需要额外的推理时间计算，要么需要更强大的教师模型，成本高且不实用。

Method: 提出SCRPO框架：1）利用LLM自身的批判和精炼能力构建偏好数据集；2）应用偏好学习来训练同一个LLM，提升其在摘要任务中的忠实度。

Result: 在三个摘要基准测试（XSUM、CNNDM和SAMSum）上，SCRPO在忠实度指标上优于最先进的自监督学习方法，同时保持或改进了衡量摘要整体质量的其他指标。与推理时精炼方法相比，SCRPO不仅效率更高，还能生成更忠实的摘要。

Conclusion: SCRPO提供了一种有效且实用的自监督训练方法，通过利用模型自身的批判和精炼能力来减少幻觉问题，在保持摘要质量的同时显著提升忠实度，且无需额外推理成本或更强大的教师模型。
...
TL;DR: 提出SQ-format统一数据格式，通过结合稀疏化和量化技术，在保持精度的同时提升大语言模型推理效率，为下一代AI加速器提供设计思路。


<details>
  <summary>Details</summary>
Motivation: 现有低比特量化和稀疏化技术难以平衡精度与效率，硬件支持有限。W4A8与W8A8峰值性能相同，而GPU支持的2:4半结构化稀疏格式因精度损失很少被采用。需要一种统一的数据格式来弥合这一差距。

Method: 提出稀疏量化格式(SQ-format)，利用稀疏矩阵可以在高精度下加速的特性，结合低精度矩阵乘法加速，实现性能和吞吐量的帕累托改进。特别适用于具有异常值不均匀分布的激活值，使其静态压缩成为可能。

Result: 展示了SQ-format在训练后量化中的最先进性能，提出了支持该格式所需的硬件要求，并为下一代AI加速器提供了设计探索和见解。

Conclusion: SQ-format是一种统一的数据格式，能够有效结合量化和稀疏化的优势，在保持模型精度的同时提升推理效率，为硬件设计提供了新的方向。
...
TL;DR: 本文首次对预训练语言模型在拼写纠错任务上的有效性进行了实证研究，特别关注低资源语言，发现大语言模型在大型微调数据集上表现最佳，并发布了LMSpell工具包。
...
Method: 1. 对多种预训练语言模型（包括编码器、编码器-解码器和大语言模型）进行拼写纠错效果的实证研究；2. 开发LMSpell工具包，包含评估函数以补偿LLM的幻觉问题；3. 以僧伽罗语为例进行案例研究，深入探讨低资源语言的拼写纠错困境。

Result: 1. 当微调数据集较大时，大语言模型在拼写纠错任务上优于编码器基和编码器-解码器模型；2. 即使对于LLM未预训练的语言，这一观察结果仍然成立；3. 揭示了低资源语言在拼写纠错方面面临的特殊挑战。

Conclusion: 本研究首次系统比较了不同预训练语言模型在拼写纠错任务上的表现，特别关注低资源语言，发现大语言模型在充足数据下具有优势，并提供了实用的工具包和评估方法，为低资源语言的拼写纠错研究提供了重要参考。
...
TL;DR: 该研究针对大语言模型在音乐知识问答上的局限性，构建了MusWikiDB音乐向量数据库和ArtistMus基准测试集，通过检索增强生成显著提升了音乐相关问答的准确率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在开放域问答方面取得了进展，但在音乐相关推理方面效果有限，主要原因是预训练数据中音乐知识稀疏。现有音乐信息检索和计算音乐学研究虽然探索了结构化和多模态理解，但缺乏基于艺术家元数据或历史背景的事实性和上下文音乐问答资源。

Method: 1. 构建MusWikiDB：包含144K个音乐相关维基百科页面的3.2M个段落向量数据库；2. 创建ArtistMus基准：包含500位多样化艺术家的1000个问题，附带流派、出道年份和主题等元数据；3. 使用检索增强生成（RAG）方法评估音乐问答性能；4. 进行RAG风格微调以提升事实回忆和上下文推理能力。

Result: 1. RAG显著提高了事实准确性：开源模型提升高达+56.8个百分点（如Qwen3 8B从35.0提升到91.8），接近专有模型性能；2. RAG风格微调进一步提升了事实回忆和上下文推理能力，在域内和域外基准测试中均有改善；3. MusWikiDB相比通用维基百科语料库准确率提高约6个百分点，检索速度快40%。

Conclusion: MusWikiDB和ArtistMus资源为音乐信息检索和领域特定问答研究提供了基础，建立了在音乐等文化丰富领域中检索增强推理的框架，显著提升了音乐相关问答的准确性和效率。
...
TL;DR: SEA-SafeguardBench是首个针对东南亚语言的人工验证安全基准，覆盖8种语言、21,640个样本，揭示现有LLM和防护系统在东南亚文化场景下表现不佳


<details>
  <summary>Details</summary>
Motivation: 现有安全评估主要针对英语，缺乏对语言和文化多样性的考量。东南亚语言代表性不足，但该地区有独特的语言多样性和安全关切（如文化敏感的政治言论、地区性虚假信息）。机器翻译的英语数据无法捕捉低资源语言的细微差别

Method: 创建SEA-SafeguardBench基准，包含三个子集：通用、真实场景和内容生成，覆盖8种东南亚语言，共21,640个人工验证样本。通过该基准评估最先进的LLM和防护系统在东南亚文化场景下的表现

Result: 实验结果表明，即使是当前最先进的LLM和防护系统在东南亚文化和有害场景下也面临挑战，与英语文本相比表现不佳

Conclusion: 需要本地化、原生创作的基准来反映当地规范和安全场景，以解决东南亚语言在安全评估中的代表性不足问题。SEA-SafeguardBench填补了这一空白，揭示了现有系统在跨语言安全防护方面的局限性
...
TL;DR: 大型语言模型通过病灶标记和解剖学感知提示，在放射学报告中检测需要随访的偶发病灶方面显著优于传统监督学习方法，性能接近人类专家水平。


<details>
  <summary>Details</summary>
Motivation: 当前基于文档级别的分类系统在检测需要随访的偶发病灶方面存在局限性，需要更细粒度的病灶级别检测方法。本研究旨在评估大型语言模型与传统监督学习方法在偶发病灶检测方面的性能差异。

Method: 使用包含400份标注放射学报告和1,623个已验证病灶的数据集。比较三种监督式transformer编码器（BioClinicalModernBERT、ModernBERT、Clinical Longformer）与四种生成式LLM配置（Llama 3.1-8B、GPT-4o、GPT-OSS-20b）。引入基于病灶标记输入和解剖学感知提示的新型推理策略，使用类别特异性F1分数评估性能。

Result: 解剖学感知的GPT-OSS-20b模型表现最佳，偶发病灶阳性宏F1达到0.79，超过所有监督基线（最高宏F1：0.70），接近0.76的标注者间一致性。解剖学基础显著提升了GPT模型的性能（p < 0.05），多数投票集成进一步将宏F1提升至0.90。错误分析显示解剖学感知LLM在区分可处理发现与良性病灶方面具有优越的上下文推理能力。

Conclusion: 通过结构化病灶标记和解剖学上下文增强的生成式大型语言模型显著优于传统监督编码器，性能接近人类专家水平。这种方法为放射学工作流程中的自动化偶发发现监测提供了可靠、可解释的途径。
...
TL;DR: 该研究系统评估了Tree-of-Thought（ToT）推理方法在孟加拉语数学应用题（MWPs）上的表现，相比标准提示和Chain-of-Thought（CoT），ToT在GPT-OSS-120B模型上实现了88%的最高准确率，比CoT提升5个百分点。


<details>
  <summary>Details</summary>
Motivation: 数学应用题（MWPs）是NLP中最具挑战性的任务之一，需要语言理解和多步数值推理。虽然Chain-of-Thought（CoT）提示显示出潜力，但其线性结构容易传播错误，限制了整体效果。本研究旨在通过Tree-of-Thought（ToT）推理方法解决这一限制，特别是在低资源语言孟加拉语中的应用。

Method: 使用SOMADHAN数据集，在计算和token成本约束下，选取100个代表性孟加拉语数学应用题。在多个大语言模型（包括GPT-OSS和LLaMA变体）上评估三种策略：标准提示、CoT和ToT。系统比较不同规模模型下的性能表现。

Result: CoT将基线准确率从78%（标准提示）提升到83%，而ToT进一步将性能提升最多5个百分点，在GPT-OSS-120B模型上达到88%准确率。ToT在中大型模型中特别有效，但对较小模型的优势较小。
...
TL;DR: 该研究提出了一种基于医学知识生成多语言推理轨迹的方法，用于提升医学问答中大型语言模型的性能，在8B参数模型中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有医学问答方法主要依赖英语和通用大语言模型的知识蒸馏，存在医学知识可靠性问题，且缺乏多语言支持，限制了临床决策支持工具在多语言环境中的发展和安全性。

Method: 采用检索增强生成方法，基于维基百科医学信息生成英语、意大利语和西班牙语共50万条推理轨迹，扩展MedQA和MedMCQA数据集到多语言版本，并在领域内和领域外医学问答基准上进行测试。

Result: 生成的推理轨迹通过上下文学习（少样本）和监督微调都能提升性能，在8B参数语言模型中取得了最先进的结果，证明了该方法在多语言医学问答中的有效性。

Conclusion: 该方法能够支持开发更安全、更透明的多语言临床决策支持工具，研究团队发布了完整的资源套件，包括推理轨迹、翻译的问答数据集、医学维基百科和微调模型。
...
TL;DR: ILVR框架通过交错潜在视觉表示与文本生成，解决多模态大语言模型中视觉反馈计算成本高的问题，实现动态状态演化与精确感知建模的统一。
...
Method: 提出交错潜在视觉推理(ILVR)框架，将文本生成与作为后续推理特定演化线索的潜在视觉表示交错结合。采用自监督策略，通过动量教师模型从辅助图像中选择性蒸馏相关特征到稀疏监督目标中，自适应选择机制引导模型自主生成上下文感知的视觉信号。

Result: 在多模态推理基准测试上的广泛实验表明，ILVR显著优于现有方法，有效弥合了细粒度感知与序列多模态推理之间的差距。

Conclusion: ILVR框架成功统一了动态状态演化与精确感知建模，通过交错潜在视觉表示与文本生成，解决了多模态大语言模型中视觉反馈的高计算成本问题，为多模态推理提供了高效解决方案。
...
TL;DR: 开发了ClinEdu多智能体教学模拟器和ClinTeach数据集，并训练了MedTutor-R1多模态苏格拉底式导师，用于解决临床医学教育中专家指导不足的问题，通过模拟评估显示其教学效果显著优于基础模型。


<details>
  <summary>Details</summary>
Motivation: 临床医学教育面临专家指导需求增长与专家资源稀缺之间的巨大矛盾，现有研究主要关注一对一知识传授，忽视了团队协作中的协作推理能力培养，这是临床教学（如病房查房）中的关键技能。

Method: 1. 开发ClinEdu多智能体教学模拟器，包含个性化患者和多样化学生群体；2. 基于ClinEdu构建ClinTeach大规模苏格拉底式教学对话数据集；3. 训练MedTutor-R1多模态苏格拉底式导师，先通过ClinTeach进行指令微调，再使用基于三维评估标准（结构保真度、分析质量、临床安全性）的强化学习优化自适应苏格拉底策略；4. 采用模拟交互评估方法，将导师重新部署到ClinEdu中进行真实情境评估。

Result: MedTutor-R1在平均教学评分上比基础模型高出20%以上，与o3模型表现相当，同时在处理不同数量学生时表现出高适应性，验证了ClinEdu教学模拟器的有效性。

Conclusion: 通过多智能体教学模拟器、大规模教学数据集和强化学习优化的苏格拉底式导师，成功开发了一个能够有效支持临床医学教育中一对多教学的智能教学系统，为解决临床教育中专家资源不足的问题提供了有前景的解决方案。
...
TL;DR: 比较两种模型在捷克宪法法院判决检索中的表现：通用OpenAI嵌入器显著优于领域特定的BERT模型，尽管绝对nDCG值较低，但差异具有统计显著性。


<details>
  <summary>Details</summary>
Motivation: 案例法检索是耗时的任务，通常通过查询数据库完成。本研究旨在比较通用嵌入器和领域特定模型在捷克宪法法院判决检索中的性能，并开发适用于噪声标签数据的评估框架。

Method: 比较两种模型：1）通用OpenAI嵌入器；2）领域特定BERT（在约30,000个判决上训练，使用滑动窗口和注意力池化）。采用噪声感知评估方法，包括IDF加权关键词重叠作为分级相关性、通过两个阈值（0.20平衡、0.28严格）进行二值化、配对自助法显著性检验，以及结合定性分析的nDCG诊断。

Result: 尽管绝对nDCG值较低（在噪声标签下预期如此），但通用OpenAI嵌入器在两种阈值设置下（@10/@20/@100）都显著优于领域预训练的BERT模型；差异具有统计显著性。诊断表明低绝对值源于标签漂移和强理想标准，而非模型缺乏实用性。

Conclusion: 通用OpenAI嵌入器在捷克宪法法院判决检索中表现优于领域特定BERT模型。提出的评估框架足够稳健，可用于处理来自遗留司法数据库的异构标签数据，这在噪声黄金数据集评估中特别有价值。
...
TL;DR: 提出一种通过融合多个基础忠实度指标来提升大语言模型忠实度评估准确性的方法，使用树模型确定各指标重要性，并结合人工标注进行优化。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的忠实度评估存在局限性，需要更准确的方法来评估模型输出的可信度，以便在更多样化的场景中安全部署这些模型。

Method: 提出基于指标融合的方法论：1）收集多个基础忠实度指标；2）使用树模型确定各指标的重要性权重；3）结合人工对LLM回答忠实度的标注来驱动融合过程；4）创建跨问答和对话领域的标准化数据集。

Result: 融合后的指标在所有测试领域中与人工判断的相关性更强，显著提升了忠实度评估的准确性。

Conclusion: 通过指标融合方法可以有效提升LLM忠实度评估的准确性，增强对模型的信任度，使其能在更多样化的场景中安全应用。同时提供的标准化数据集支持跨领域忠实度评估的复现和测试。
...
TL;DR: CICLe框架结合轻量级基础分类器和保形预测，通过自适应减少候选类别来指导LLM提示，在多样化NLP分类任务中实现了数据效率和计算效率的显著提升。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型展现出强大的上下文学习能力，但在文本分类任务中效果严重依赖提示设计且计算成本高昂。现有的CICLe框架虽然提出了资源高效的解决方案，但其在多个领域的适用性和效率优势尚未得到系统验证。

Method: CICLe框架整合了轻量级基础分类器和保形预测技术，通过自适应减少候选类别集合来指导LLM的提示设计。该方法在多样化NLP分类基准上进行了全面评估，比较了不同数据规模下的性能表现。

Result: CICLe在基础分类器训练样本充足时，持续超越基础分类器并优于少样本提示基线；在低数据场景下表现相当。效率方面，减少提示次数达34.45%，缩短提示长度达25.16%，并支持使用更小模型保持竞争力。特别在高类别不平衡任务中优势明显。

Conclusion: CICLe作为一种实用且可扩展的高效文本分类方法，结合了传统分类器的鲁棒性和LLM的适应性，在数据和计算效率方面实现了显著提升，为资源受限环境下的文本分类提供了有效解决方案。
...
TL;DR: 美国最高法院口头辩论中，法官打断律师发言对语义内容和情感语调的影响研究，特别关注性别差异。研究发现打断不会显著改变论证内容，但对女性律师的打断包含更多负面情感。
...
Result: 打断前后语义相似度保持较高水平，表明打断不会显著改变论证内容。但对女性律师的打断包含显著更高的负面情感。
...
TL;DR: 研究显示，为AI模型分配专家或低知识人设提示并不能显著提升其在GPQA Diamond和MMLU-Pro等研究生级别客观选择题上的准确率，反而低知识人设通常会降低性能。


<details>
  <summary>Details</summary>
Motivation: 探究在AI模型中分配人设（专家或低知识人设）是否能够提升其在困难客观选择题上的表现，为商业、教育和政策决策者提供基于严格测试的技术见解。

Method: 在GPQA Diamond和MMLU-Pro两个研究生级别基准测试上评估了6个模型，测试了三种人设分配方法：1）领域匹配专家人设；2）领域不匹配专家人设；3）低知识人设（外行、幼儿等），并与无人设基线进行比较。

Result: 人设提示总体上未能提升准确率：领域匹配专家人设无显著影响（除Gemini 2.0 Flash外）；领域不匹配专家人设有时会降低性能；低知识人设通常损害准确率。人设对答案准确性无一致益处。

Conclusion: 为AI模型分配人设并不能有效提升其在困难客观问题上的事实准确性，虽然人设可能改变输出风格，但对于改善事实性能效果有限。
...
TL;DR: 本文提出了一种基于检索增强生成（RAG）的医学问答系统，通过结合领域知识检索与开源大语言模型，提高了医学问答的事实准确性和减少了幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 直接将大语言模型应用于临床领域面临事实准确性不足和幻觉问题等挑战，需要开发能够结合医学领域专业知识的方法来提高医学问答系统的可靠性。

Method: 使用检索增强生成框架，结合医学文献检索与开源大语言模型（LLaMA~2和Falcon），采用低秩适应（LoRA）进行高效领域微调，通过检索相关医学文献来为模型提供事实依据。

Result: 在PubMedQA和MedMCQA基准数据集上评估，检索增强显著提高了答案准确性。微调后的LLaMA~2模型在PubMedQA上达到71.8%准确率，相比零样本基线的55.4%有显著提升，同时通过提供来源引用保持透明度，减少了约60%的无支持内容。

Conclusion: 检索增强生成与开源大语言模型的结合为可靠的生物医学问答系统提供了有效途径，展示了在临床信息学实际应用中的潜力，通过基于检索证据的回答提高了事实正确性和减少了幻觉。
...
<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在视觉问答方面表现良好，但受限于静态训练数据。检索增强生成能够访问最新、文化相关和多语言信息，但多语言多模态RAG研究仍不足，需要建立系统性评估基准。

Method: 构建M4-RAG基准，包含42种语言和56种地区方言/变体，超过80,000个文化多样性图像-问题对。创建受控检索环境，包含数百万精心策划的多语言文档，模拟真实检索条件同时确保实验可重复性。

Result: 系统评估显示，RAG对小规模视觉语言模型有持续益处，但对大规模模型无法扩展，甚至经常降低其性能，暴露了模型规模与当前检索效果之间的关键不匹配。

Conclusion: M4-RAG为推进下一代RAG系统提供了基础，这些系统能够跨语言、模态和文化背景进行无缝推理，揭示了当前RAG方法在规模扩展方面的局限性。
...
TL;DR: 提出一种无需精确平面图、物理访问或同步视频的两阶段多相机标定方法，通过标注自然几何基元和交互调整有效视场多边形实现标定


<details>
  <summary>Details</summary>
Motivation: 传统多相机系统标定方法在现实条件下存在局限：缺乏精确平面图、无法物理放置标定图案、或没有同步视频流，需要一种更灵活实用的标定方案

Method: 两阶段标定方法：第一阶段基于操作员标注的自然几何基元（平行线、垂直线、等长线段）进行单相机部分标定，估计关键参数并将有效视场投影到水平面；第二阶段通过交互调整投影的多边形位置、尺度和旋转，与平面图对齐或使用虚拟标定元素，确定外参

Result: 该方法仅需每个相机的静态图像，无需物理访问或同步视频，已实现为实用的Web服务，比较分析和演示视频证实了方法的适用性、准确性和灵活性

Conclusion: 该方法克服了传统标定方法的限制，使在先前认为不可行的场景中部署精确的多相机跟踪系统成为可能
...
### [96] [CATNUS: Coordinate-Aware Thalamic Nuclei Segmentation Using T1-Weighted MRI](https://arxiv.org/abs/2512.05329)
...
<details>
  <summary>Details</summary>
Motivation: 丘脑核团的准确分割对于理解大脑功能和神经系统疾病至关重要，但由于核团体积小、对比度有限、分辨率不足以及个体解剖变异等因素，分割仍然具有挑战性。

Method: 提出CATNUS方法，采用3D U-Net架构，通过坐标卷积层增强定位能力，能够分割13个丘脑核团（或核团组）。提供预训练模型，支持定量T1图、MPRAGE和FGATIR等多种MRI序列。

Result: CATNUS在分割准确性上优于FreeSurfer、THOMAS和HIPS-THOMAS等现有方法，表现出良好的测试-重测可靠性。在不同扫描仪、场强和厂商的数据集上展现出强大的泛化能力，产生可靠且解剖一致的核团分割结果。

Conclusion: CATNUS提供了一个准确且泛化能力强的丘脑核团分割解决方案，有望促进大规模神经影像学研究并支持临床评估应用。
...
### [97] [General and Domain-Specific Zero-shot Detection of Generated Images via Conditional Likelihood](https://arxiv.org/abs/2512.05590)
*Roy Betser,Omer Hofman,Roman Vainshtein,Guy Gilboa*

Main category: eess.IV

TL;DR: CLIDE是一种基于条件似然近似的零样本检测方法，用于检测AI生成的图像，特别擅长处理特定图像领域（如艺术图像）的检测问题。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型（特别是基于扩散的方法）的快速发展，合成图像的真实感显著提高。虽然已有全监督和少样本方法，但维护更新数据集耗时且困难。现有零样本方法往往难以适应特定图像领域（如艺术图像），限制了其实际应用。

Method: CLIDE是一种基于条件似然近似的零样本检测方法。该方法计算以真实图像为条件的似然，使其能够适应不同的图像领域。

Result: 在大规模通用数据集上达到最先进的性能，在特定领域案例中显著优于现有方法。结果表明该方法具有鲁棒性，并强调了AI生成图像检测任务需要广泛的、领域感知的泛化能力。

Conclusion: CLIDE通过条件似然近似实现了有效的零样本检测，特别在适应不同图像领域方面表现出色，为AI生成图像检测提供了更实用的解决方案。
...
### [98] [Semantic Faithfulness and Entropy Production Measures to Tame Your LLM Demons and Manage Hallucinations](https://arxiv.org/abs/2512.05156)
*Igor Halperin*

Main category: cs.AI

TL;DR: 该论文提出了两种基于信息论和热力学的无监督度量方法，用于评估大型语言模型的任务忠实度，包括语义忠实度(SF)和语义熵产生(SEP)指标。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型对给定任务的忠实度是一个复杂挑战，需要新的无监督度量方法来量化模型的忠实程度和幻觉控制。
...
Result: 提出的SF和SEP度量可以单独或联合用于LLM评估和幻觉控制，高忠实度通常对应低熵产生。在SEC 10-K文件摘要任务上验证了该框架的有效性。

Conclusion: 基于信息论和热力学的无监督度量方法为评估LLM忠实度提供了新框架，SF和SEP指标能够有效量化模型忠实程度，有助于幻觉检测和控制。
...
### [99] [Bridging Traditional Machine Learning and Large Language Models: A Two-Part Course Design for Modern AI Education](https://arxiv.org/abs/2512.05167)
*Fang Li*
...
Method: 采用两部分课程设计：第一部分教授基础机器学习概念，第二部分专注于当代LLM应用。课程为期两个七周学期，包括详细的课程架构、实施策略和评估方法。

Result: 该综合教学方法增强了学生对AI领域的理解，并更好地为他们应对行业需求做好准备，在夏季课程实施中取得了积极的学习成果。

Conclusion: 这种将传统机器学习与现代LLM相结合的教学方法能有效提升学生对AI领域的全面理解，更好地为他们在快速发展的AI行业中的职业发展做好准备。
...
### [100] [On the Computability of Artificial General Intelligence](https://arxiv.org/abs/2512.05212)
*Georgios Mappouras,Charalambos Rossides*

Main category: cs.AI

TL;DR: 该论文通过形式化证明指出：任何算法（包括AI模型）都无法产生初始算法本身不存在的全新功能能力，因此无法实现真正的创造力，只能展示现有功能能力的组合与排列。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能的快速发展，人们开始探讨人类距离开发出达到人类智能水平的人工通用智能（AGI）还有多远。本研究旨在探索算法（包括AI）的能力上限，特别是能否实现真正的创造力。

Method: 采用先前研究中关于AGI的定义（即在某个研究领域中通过创造性和创新性解锁新的、先前未知的功能能力），基于此定义进行形式化证明，论证算法能力的根本限制。

Result: 形式化证明表明：没有任何算法能够展示出初始算法本身不存在的全新功能能力。因此，任何算法（包括AI模型）都无法在任何研究领域（科学、工程、艺术、体育等）实现真正的创造力，只能展示现有功能能力及其组合与排列。

Conclusion: 该证明对AI发展的未来和人类智能的起源都具有重要启示。它表明AI无法实现真正的创造性突破，只能基于已有能力进行组合创新，这对理解人类智能的独特性提供了理论依据。
...
### [101] [Resolving Zadehs Paradox Axiomatic Possibility Theory as a Foundation for Reliable Artificial Intelligence](https://arxiv.org/abs/2512.05257)
*Bychkov Oleksii,Bychkova Sophia,Lytvynchuk Khrystyna*

Main category: cs.AI

TL;DR: 本文论证可能性理论能从根本上解决Dempster-Shafer理论（DST）的悖论问题，通过可能性与必要性测度的二元框架为不确定性处理提供逻辑一致且数学严谨的基础。


<details>
  <summary>Details</summary>
Motivation: DST在处理不确定性时存在逻辑悖论和矛盾数据处理的困难，需要寻找更根本的解决方案。本文旨在证明可能性理论不仅是一种替代方案，而是能够从根本上解决DST悖论的理论框架。

Method: 采用Bychkov文章中发展的公理化方法，基于可能性与必要性测度的二元框架构建逻辑一致的不确定性处理基础。通过比较分析概率、证据和可能性三种范式，并以经典医疗诊断困境为例，展示可能性理论如何处理矛盾数据。

Result: 可能性理论能够正确处理矛盾数据，避免DST的逻辑陷阱，使形式推理更接近自然智能的逻辑。与众多试图修正Dempster规则的尝试不同，该方法从零开始建立了逻辑一致的不确定性处理基础。

Conclusion: 可能性理论为解决DST危机提供了根本性方案，它不仅是一种替代范式，而且通过可能性与必要性测度的二元框架，为不确定性处理提供了逻辑一致且数学严谨的基础，使形式推理更贴近人类自然智能的逻辑。
...
### [102] [AI & Human Co-Improvement for Safer Co-Superintelligence](https://arxiv.org/abs/2512.05356)
*Jason Weston,Jakob Foerster*

Main category: cs.AI

TL;DR: 论文主张将AI研究目标从自我改进转向协同改进，即人类研究者与AI系统合作实现共同超级智能，这比单纯的AI自我改进更可行、更安全


<details>
  <summary>Details</summary>
Motivation: 当前AI领域追求自我改进的目标充满危险且难以完全实现，需要寻找更可行、更安全的替代方案

Method: 提出"协同改进"作为替代目标，专注于提升AI系统与人类研究者合作进行AI研究的能力，从构思到实验的全过程协作

Result: 协同改进既能加速AI研究进展，又能通过人机共生赋予双方更安全的超级智能能力

Conclusion: 将人类研究改进纳入循环的协同改进方法既能更快实现目标，又能确保更高的安全性，是比单纯AI自我改进更优的研究方向
...
### [103] [ChipMind: Retrieval-Augmented Reasoning for Long-Context Circuit Design Specifications](https://arxiv.org/abs/2512.05371)
...
TL;DR: ChipMind是一个基于知识图谱增强的推理框架，专门用于处理超长集成电路规格文档，通过将电路规格转换为领域知识图谱，结合自适应检索和语义过滤，显著提升了LLM在硬件设计中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在集成电路开发自动化方面潜力巨大，但受限于有限的上下文窗口。现有的上下文扩展方法难以对复杂冗长的电路规格进行有效的语义建模和多跳推理，这阻碍了LLM在硬件设计中的实际工业部署。
...
### [104] [BEAVER: An Efficient Deterministic LLM Verifier](https://arxiv.org/abs/2512.05439)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型从研究原型转向生产系统，需要可靠方法来验证模型输出是否满足所需约束。基于采样的估计只能提供模型行为的直觉，无法提供可靠保证。
...
Conclusion: BEAVER能够提供松散边界或经验评估无法实现的精确特征描述和风险评估，为LLM约束满足提供确定性、可靠的概率边界验证框架。
...
### [105] [The Seeds of Scheming: Weakness of Will in the Building Blocks of Agentic Systems](https://arxiv.org/abs/2512.05449)
*Robert Yang*

Main category: cs.AI

TL;DR: 该论文提出将"意志薄弱"(akrasia)作为分析AI智能体不一致性的核心概念，并开发了Akrasia基准测试来量化模型在诱惑下的自我控制能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型表现出一种特殊的不一致性：它们"知道"正确答案但无法据此行动。这种全局判断与局部冲动之间的张力在人类哲学中被称为"意志薄弱"(akrasia)。作者认为这一概念对于分析AI智能体系统的不一致性和目标漂移具有基础性意义。

Method: 引入Akrasia基准测试的初步版本，包含四种结构化提示条件：基线(B)、同义词(S)、时间(T)和诱惑(X)，用于测量模型局部响应与其先前承诺相矛盾的情况。该基准支持跨模型家族、解码策略和诱惑类型的"自我控制"定量比较。

Result: 基准测试能够量化比较不同模型在诱惑条件下的"自我控制"能力。此外，作者指出微观层面的意志薄弱可能在多智能体系统中累积为宏观层面的不稳定性，这可能被解释为"阴谋"或故意的不对齐。
...
### [106] [MIND: Multi-rationale INtegrated Discriminative Reasoning Framework for Multi-modal Large Models](https://arxiv.org/abs/2512.05530)
*Chuang Yu,Jinmiao Zhao,Mingxuan Zhao,Yunpeng Liu,Xiujun Shu,Yuanhao Feng,Bo Wang,Xiangyu Yue*

Main category: cs.AI

TL;DR: MIND推理框架通过"理解-反思-修正"的认知过程，将MLLMs从被动模仿推理转变为主动判别推理，在科学、常识和数学场景中实现SOTA性能。
...
Method: 提出MIND框架，包含RAD范式自动扩展数据集，P2CL两阶段修正学习策略，以及MCA多理性对比对齐优化策略。

Result: 在多个公开数据集（科学、常识、数学场景）上实现了最先进的性能表现。
...
### [107] [Ontology Learning with LLMs: A Benchmark Study on Axiom Identification](https://arxiv.org/abs/2512.05594)
*Roos M. Bakker,Daan L. Di Scala,Maaike H. T. de Boer,Stephan A. Raaijmakers*

Main category: cs.AI

TL;DR: 论文提出OntoAxiom基准测试，系统评估大型语言模型在识别本体论公理方面的性能，发现逐条公理提示策略优于直接方法，但性能因公理类型和本体领域而异。


<details>
  <summary>Details</summary>
Motivation: 本体论开发需要大量建模和领域专业知识，自动化这一过程的本体学习在过去十年中随着自然语言处理技术特别是大型语言模型的发展而进步。本研究旨在解决识别公理（定义类和属性间逻辑关系的基本本体组件）的挑战。

Method: 引入OntoAxiom基准测试，包含9个中等规模本体（共17,118个三元组和2,771个公理），专注于子类、不相交、子属性、定义域和值域公理。评估12个LLM在三种few-shot设置和两种提示策略下的性能：直接方法（一次查询所有公理）与逐条公理方法（每个提示只查询一个公理）。

Result: 逐条公理提示策略比直接方法获得更高的F1分数，但性能因公理类型而异。领域影响显著：FOAF本体的子类公理得分0.642，而音乐本体仅0.218。大型LLM优于小型模型，但小型模型在资源受限环境下仍可能适用。总体性能不足以完全自动化公理识别。

Conclusion: 虽然LLM性能不足以完全自动化公理识别，但可以为本体工程师提供有价值的候选公理，支持本体的开发和精炼。逐条公理方法更有效，但性能受公理类型和领域影响，大型模型表现更好但小型模型在特定场景下仍可用。
...
### [108] [Enhancing Local Search for MaxSAT with Deep Differentiation Clause Weighting](https://arxiv.org/abs/2512.05619)
*Menghua Jiang,Haokai Gao,Shuhao Chen,Yin Chen*

Main category: cs.AI

TL;DR: 提出了一种针对部分最大可满足性(PMS)和加权部分最大可满足性(WPMS)问题的新型子句权重方案DeepDist，首次为两种问题类型设计不同的权重更新条件，并结合新的初始化方法和去偏置技术，显著提升了随机局部搜索算法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的随机局部搜索算法在处理PMS和WPMS问题时，通常采用统一的子句权重更新策略，忽视了这两种问题类型之间的关键结构差异。这导致算法无法充分区分PMS和WPMS实例，限制了求解性能的提升。

Method: 1) 提出新型子句权重方案，首次为PMS和WPMS实例设计不同的权重更新条件；2) 引入新的初始化方法，更好地适应两种实例类型的独特特征；3) 提出去偏置方法，优先满足单元子句和硬子句，与权重方案形成有效互补；4) 基于这些方法开发了新的SLS求解器DeepDist。

Result: 在最近MaxSAT评估的anytime tracks基准测试中，DeepDist优于最先进的SLS求解器。更重要的是，将DeepDist与TT-Open-WBO-Inc结合的混合求解器超越了MaxSAT评估2024的获胜者SPB-MaxSAT-c-Band和SPB-MaxSAT-c-FPS，证明了该方法的有效性。

Conclusion: 通过为PMS和WPMS问题设计差异化的子句权重更新策略，并结合创新的初始化方法和去偏置技术，DeepDist显著提升了随机局部搜索算法在解决这两种问题上的性能，为(W)PMS求解提供了新的有效方法。
...
### [109] [Evolutionary System 2 Reasoning: An Empirical Proof](https://arxiv.org/abs/2512.05760)
*Zeyuan Ma,Wenqi Huang,Guo-Huan Song,Hongshu Guo,Sijie Ma,Zhiguang Cao,Yue-Jiao Gong*

Main category: cs.AI

TL;DR: 本文提出进化推理优化（ERO）框架，通过进化算法在LLM群体中筛选出具有强大推理能力的个体，发现当前LLM（包括GPT-5）的系统2推理能力有限，但通过简单进化循环可以显著提升较弱模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型虽然在特定下游任务上表现出色，但在通用智能方面仍有不足。基于智力与系统2推理（慢思考）之间的相关性，本文研究机器智能（如LLM）能否像人类一样进化获得推理能力（而非特定技能）。

Method: 提出进化推理优化（ERO）框架：首先将多个LLM初始化为一个群体，然后采用进化策略对群体进行演化，以最大化最佳个体的量化推理分数，实现"适者生存"的选择过程。

Result: 实验发现两个令人惊讶的经验发现：1）最新的LLM（如GPT-5）仍表现出有限的系统2推理能力；2）通过ERO的简单进化循环，相对较弱的模型（Qwen-7B）可以被增强，涌现出强大的推理能力。

Conclusion: ERO框架能够有效提升LLM的推理能力，证明了通过进化方法可以使机器智能获得类似人类的推理能力，为机器智能的发展提供了新思路。
...
### [110] [The Missing Layer of AGI: From Pattern Alchemy to Coordination Physics](https://arxiv.org/abs/2512.05765)
*Edward Y. Chang*

Main category: cs.AI

TL;DR: 论文认为LLM不是AGI的死胡同，而是缺少一个系统2的协调层来选择和约束模式匹配，提出了UCCT理论和MACI架构来实现推理能力


<details>
  <summary>Details</summary>
Motivation: 针对"LLM只是模式匹配器，无法真正推理"的批评，作者认为问题不在于LLM本身，而在于缺少一个协调层来引导和约束模式匹配，使其能够进行目标导向的推理

Method: 提出UCCT（语义锚定理论）作为理论基础，将推理建模为相变过程，受有效支持度、表征失配和自适应锚定预算控制。基于此开发MACI架构，包含诱饵（行为调制辩论）、过滤（苏格拉底式判断）和持久性（事务性记忆）三个组件

Result: 通过理论框架和架构设计，展示了如何将LLM从无基础的生成转变为目标约束的推理，将常见批评重新解释为可测试的协调失败问题

Conclusion: 通往AGI的道路应该通过LLM而不是绕过它们，关键在于添加一个系统2协调层来选择和约束系统1的模式匹配能力
...
### [111] [Multimodal Oncology Agent for IDH1 Mutation Prediction in Low-Grade Glioma](https://arxiv.org/abs/2512.05824)
*Hafsa Akebli,Adam Shephard,Vincenzo Della Mea,Nasir Rajpoot*

Main category: cs.AI

TL;DR: 本文提出了一种多模态肿瘤学智能体（MOA），整合基于TITAN基础模型的IDH1突变预测组织学工具，并结合临床和基因组数据的外部知识推理，在低级别胶质瘤中实现了高精度的IDH1突变预测。


<details>
  <summary>Details</summary>
Motivation: 低级别胶质瘤中的IDH1突变具有重要的临床意义，能定义不同的预后和治疗亚组。目前需要更准确的预测方法来整合多模态信息，包括组织学特征和临床基因组数据。

Method: 开发了多模态肿瘤学智能体（MOA），包含：1）基于TITAN基础模型的IDH1突变预测组织学工具；2）通过PubMed、Google Search和OncoKB对结构化临床和基因组输入进行推理的能力。在TCGA-LGG队列的488名患者上进行评估。

Result: MOA（无组织学工具）F1分数为0.826，优于临床基线（0.798）。整合组织学特征后，MOA达到最高性能，F1分数为0.912，超过组织学基线（0.894）和融合组织学-临床基线（0.897）。

Conclusion: MOA能够通过外部生物医学资源捕获互补的突变相关信息，实现准确的IDH1突变预测，展示了多模态整合在肿瘤学诊断中的优势。
...
### [112] [To Err Is Human: Systematic Quantification of Errors in Published AI Papers via LLM Analysis](https://arxiv.org/abs/2512.05925)
*Federico Bianchi,Yongchan Kwon,Zachary Izzo,Linjun Zhang,James Zou*

Main category: cs.AI

TL;DR: 使用GPT-5开发的论文正确性检查器发现，顶级AI会议和期刊发表的论文中存在不容忽视的客观错误，且错误数量随时间增加，人类专家验证显示AI检查器精度达83.2%。


<details>
  <summary>Details</summary>
Motivation: 同行评审出版物是构建新研究和知识的基础，但文献中持续存在的错误会传播并造成混淆，影响后续研究和可重复性。研究加速和同行评审系统压力增加使得错误更难被发现和避免。

Method: 开发基于GPT-5的论文正确性检查器，系统识别顶级AI会议和期刊已发表论文中的客观错误（如公式、推导、计算、图表错误），排除主观考量，并由人类专家验证AI识别的潜在错误。

Result: 论文包含显著数量的客观错误，且平均错误数随时间增加：NeurIPS从2021年的3.8个增至2025年的5.9个（增长55.3%）；ICLR从2018年的4.1个增至2025年的5.2个；TMLR从2022/23年的5.0个增至2025年的5.5个。人类专家验证316个潜在错误，确认263个为实际错误，精度83.2%。AI检查器能为75.8%的识别错误提出正确修正。

Conclusion: 前沿大语言模型在检测和修正已发表论文中的客观错误方面具有潜力，有助于建立更坚实的知识基础，虽然大多数问题相对较小，但修正它们可以减少文献混淆并增强可重复性。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在可靠数学和科学推理方面仍面临挑战，标准最终答案评估往往掩盖推理错误，导致无声故障持续存在。需要更透明的评估方法来诊断推理轨迹。

Method: 提出TRACE框架，核心是辅助推理集(ARS) - 将复杂问题分解为子问题-答案对，通过一致性指标评估中间步骤，暴露标准评估忽略的故障。

Result: 实验表明，ARS上的一致性指标与最终答案正确性相关，能精确定位推理失败步骤，为模型改进提供可操作信号。TRACE定义的置信区间能区分可靠与不可靠推理路径。

Conclusion: TRACE框架通过透明推理和一致性评估，解决了大型视觉语言模型推理错误的诊断问题，支持有效的过滤、调试和模型改进。
...
TL;DR: VQR-DQN将变分量子电路与Rainbow DQN结合，用于人力资源分配问题，相比经典方法减少26.8%的makespan，量子增强DRL在组合优化中表现出优势。


<details>
  <summary>Details</summary>
Motivation: 资源分配问题是NP难问题，传统深度强化学习方法受限于经典函数逼近器的表达能力。需要探索量子计算的优势来提升大规模资源分配问题的解决能力。

Method: 提出VQR-DQN方法，将环拓扑变分量子电路集成到Rainbow DQN中，利用量子叠加和纠缠特性。将人力资源分配问题建模为基于官员能力、事件调度和转移时间的MDP。

Result: 在四个人力资源分配基准测试中，VQR-DQN相比随机基线减少了26.8%的标准化makespan，比Double DQN和经典Rainbow DQN提升了4.9-13.4%的性能。

Conclusion: 量子增强的深度强化学习在大规模资源分配中具有潜力，电路表达能力、纠缠与策略质量的理论联系得到验证，为组合优化问题提供了新方向。
...
TL;DR: SymPyBench是一个包含15,045个大学物理问题的大规模合成基准测试，支持无限参数配置，包含三种题型和创新的评估指标，用于测试语言模型的科学推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏能够全面评估语言模型科学推理能力的大规模基准测试，特别是能够测试模型在不同参数配置下的一致性和鲁棒性的工具。

Method: 创建了包含15,045个完全参数化的大学物理问题的基准测试，支持无限输入配置，每个问题都附带结构化逐步推理和可执行的Python代码。包含三种题型：MC-Symbolic、MC-Numerical和自由形式问题。引入了三个新颖的评估指标：一致性分数、失败率和混淆率。

Result: 通过对最先进的指令调优语言模型进行实验，揭示了它们在科学推理方面的优势和局限性，为开发更鲁棒和可解释的推理系统奠定了基础。

Conclusion: SymPyBench作为一个动态、代码驱动的基准测试，为评估和提升语言模型的科学推理能力提供了重要工具，特别是通过其创新的评估指标能够量化模型在不同问题变体中的变异性和不确定性。
