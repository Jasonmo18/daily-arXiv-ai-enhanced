...
TL;DR: 提出SKETCHGAIT框架，使用无标签的素描模态进行步态识别，结合语义解析实现层次解耦的多模态学习，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别研究主要依赖轮廓和解析表示：轮廓稀疏且缺乏内部结构细节，解析依赖上游人体解析器导致性能不稳定。需要探索新的视觉模态来获取密集结构信息而不依赖语义标签。

Method: 提出SKETCH作为新的视觉模态，通过边缘检测器从RGB图像中提取高频结构线索（如肢体关节和自遮挡轮廓）。提出SKETCHGAIT框架，包含两个独立流进行模态特定学习，以及轻量级早期融合分支捕捉结构互补性。

Result: 在SUSTech1K数据集上达到92.9% Rank-1准确率，在CCPG数据集上达到93.1%平均Rank-1准确率，验证了素描模态和框架的有效性。

Conclusion: 素描作为无标签的密集结构表示，与语义解析在语义上解耦、结构上互补，SKETCHGAIT框架通过层次解耦的多模态学习实现了优越的步态识别性能。
...
TL;DR: 首个针对基于热图的关键点检测器的耦合鲁棒性验证框架，能够约束所有关键点的联合偏差，优于传统的解耦验证方法。


<details>
  <summary>Details</summary>
Motivation: 关键点检测是许多视觉任务的基础，但现代神经模型对小输入扰动仍然脆弱。由于高维输入和连续坐标输出，关键点检测器的形式化鲁棒性验证尚未充分探索。

Method: 提出耦合鲁棒性验证框架，将验证问题表述为使用混合整数线性规划（MILP）的证伪问题，结合可达热图集合和编码联合偏差约束的多面体。

Result: 实验表明，耦合方法实现了高验证率，在严格误差阈值下仍然有效，而解耦方法在这些情况下会失效。

Conclusion: 该方法为关键点检测器提供了首个耦合鲁棒性验证框架，能够捕获关键点间的相互依赖关系，为下游任务提供更准确的鲁棒性保证。
...
Method: 将每个BRep表示为参数化曲面片集合，使用可微分细分方法生成网格，通过点级监督进行大规模训练，同时重建连接且可编辑的表面

Result: 在ABC和Objaverse基准测试中，DreamCAD在文本、图像和点模态上均达到最先进性能，几何保真度提升，用户偏好超过75%

Conclusion: DreamCAD通过点级监督实现可编辑BReps生成，解决了CAD生成的可扩展性问题，并提供了大规模CAD标注数据集推动文本到CAD研究
...
TL;DR: ABRA提出对抗性批次表示增强方法，通过参数化特征统计量为结构化不确定性，在表示空间中合成最坏情况批次扰动，解决细胞成像数据中的生物批次效应问题。
...
Method: 将生物批次缓解构建为领域泛化问题，提出对抗性批次表示增强（ABRA）。通过参数化特征统计量为结构化不确定性来显式建模批次统计波动，使用最小-最大优化框架在表示空间中主动合成最坏情况生物批次扰动，并引入严格的角几何边界来保持细粒度类别可区分性。为防止对抗探索中的表示崩溃，引入协同分布对齐目标。

Result: 在RxRx1和RxRx1-WILDS大规模基准上的广泛评估表明，ABRA在siRNA扰动分类任务上建立了新的最先进性能。

Conclusion: ABRA通过对抗性批次表示增强有效缓解生物批次效应，在细胞成像数据的领域泛化问题上取得了显著进展，为高通量筛选中的批次效应校正提供了新方法。
...
TL;DR: 提出PFS（后融合稳定器），一种轻量级模块，通过处理现有BEV融合检测器的中间表示来提升在域偏移和传感器故障下的鲁棒性，无需修改原有架构或重新训练。
...
Method: 设计PFS作为近恒等变换模块，操作现有检测器的中间BEV表示，通过稳定特征统计、抑制传感器退化区域、自适应恢复弱化线索的残差校正来生成精炼特征图。

Result: 在nuScenes基准测试中，PFS在多种故障模式下取得SOTA结果，相机丢失鲁棒性提升+1.2%，低光性能提升+4.4% mAP，仅需3.3M参数。

Conclusion: PFS是一种轻量级、易于集成的解决方案，能在保持性能的同时显著提升BEV融合检测器在域偏移和传感器故障下的鲁棒性。
...
Result: iFID与扩散gFID表现出强相关性（Pearson线性相关和Spearman秩相关约0.85），是首个能有效预测扩散生成质量的指标。同时澄清了rFID与扩散细化阶段质量相关，而iFID与扩散导航阶段质量相关。
...
TL;DR: 本文提出了一种名为隐式错误计数（IEC）的新方法，用于解决在缺乏理想参考答案的多输出任务中强化学习的奖励设计问题。该方法通过枚举错误而非构建评分标准来生成校准后的奖励信号，并在虚拟试穿任务上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于评分标准的强化学习方法（如RaR）依赖于理想参考答案来构建评估标准，但许多现实任务允许多个有效输出且缺乏单一理想答案。本文识别出这一无参考设置是当前后训练方法的一个空白，需要新的奖励设计方法。

Method: 提出隐式错误计数（IEC）方法：1）枚举响应中的错误而非检查正确部分；2）在任务相关维度上应用严重性加权评分；3）通过隐式分数发射和组校准将错误转换为校准后的每方面奖励。还提出了级联错误计数（CEC）作为评估指标，并构建了Mismatch-DressCode基准来压力测试奖励设计。

Result: 在虚拟试穿任务上验证：1）CEC指标能很好跟踪人类偏好（60% top-1 vs. 30%其他）；2）在MDressBench上，IEC在所有指标上优于RaR（CEC：5.31 vs. 5.60平坦参考；5.20 vs. 5.53非平坦）；3）在VITON-HD和DressCode数据集上，IEC在8个感知指标中的6个上匹配或超越六个基线方法。

Conclusion: 当理想答案不可用时，计数错误比构建评分标准提供更强的信号。隐式错误计数方法填补了无参考设置下的奖励设计空白，为允许多个有效输出的现实任务提供了有效的强化学习奖励机制。
...
### [8] [Technical Report: Automated Optical Inspection of Surgical Instruments](https://arxiv.org/abs/2603.05987)
*Zunaira Shafqat,Atif Aftab Ahmed Jilani,Qurrat Ul Ain*

Main category: cs.CV

TL;DR: 该报告探讨了巴基斯坦制造的外科手术器械及其制造缺陷，通过深度学习架构（YOLOv8、ResNet-152、EfficientNet-b4）实现自动缺陷检测，以提高制造质量和患者安全。


<details>
  <summary>Details</summary>
Motivation: 在现代医疗动态环境中，外科手术器械的质量标准对临床成功至关重要。即使是微小缺陷也可能导致致命后果，因此制造精度至关重要。报告旨在通过自动缺陷检测方法提升巴基斯坦制造的外科器械质量，防止经济损失并保障患者生命安全。

Method: 采用深度学习方法，整合YOLOv8、ResNet-152和EfficientNet-b4架构进行自动缺陷检测。基于新开发的4,414张高分辨率图像数据集，分析裂纹、锈蚀和结构不规则等关键缺陷。与Sialkot外科器械集群的行业领导者Daddy D Pro和Dr. Frigz International合作，采用自动光学检测（AOI）工具进行质量保证。

Result: 报告提供了对巴基斯坦制造外科器械缺陷的深入分析，建立了包含4,414张高分辨率图像的数据集。通过深度学习架构实现了有效的自动缺陷检测方法，为制造商、医疗专业人员和监管机构提供了质量保证资源。

Conclusion: 通过整合行业专业知识和尖端技术，特别是深度学习架构的自动缺陷检测方法，能够显著提升外科手术器械的制造标准。这种质量保证方法有助于创造更可靠的医疗环境，确保患者安全并减少制造商的经济损失。
...
TL;DR: SemVID是一个无需训练的视觉token剪枝框架，专门针对视频时序定位任务，通过保留查询关键证据和跨帧连接性，在仅使用12.5%视觉token的情况下保持95.4%的mIoU准确率，实现5.8倍的前向加速。


<details>
  <summary>Details</summary>
Motivation: 视频时序定位需要处理长视频，使用视频语言模型计算成本极高。现有的视觉token剪枝方法直接应用于VTG会导致性能显著下降，因为VTG依赖于边界敏感证据和跨帧推理链。

Method: 提出SemVID框架，基于两个VTG专用剪枝原则：证据保留（保持查询关键patch，特别是在事件边界周围）和连接强度（保留token级跨帧连接性用于长程证据聚合）。框架首先通过平衡查询相关性和帧间变化分配每帧token预算，然后选择三种token：对象token（多样化查询关键证据）、运动token（捕捉有意义的转换并作为跨帧中继）和少量上下文token（保持场景连续性）。

Result: 在VTG基准测试中，SemVID实现了强大的准确率-效率权衡，仅使用12.5%视觉token时保留高达95.4%的mIoU，提供高达5.8倍的前向加速，在相同预算下始终优于先前方法。

Conclusion: SemVID通过专门针对VTG任务设计的训练免费剪枝框架，有效解决了视频时序定位中的计算效率问题，同时保持了边界敏感证据和跨帧推理能力，实现了显著的加速效果而不牺牲准确性。
...
### [10] [NOVA: Next-step Open-Vocabulary Autoregression for 3D Multi-Object Tracking in Autonomous Driving](https://arxiv.org/abs/2603.06254)
*Kai Luo,Xu Wang,Rui Fan,Kailun Yang*

Main category: cs.CV

TL;DR: NOVA提出了一种新的3D多目标跟踪范式，通过将轨迹重构为结构化时空语义序列，利用大语言模型的自回归能力实现跨未知目标的泛化跟踪。
...
Method: 将3D轨迹重构为结构化时空语义序列，利用大语言模型的自回归能力，将跟踪任务转化为基于原则的下一步序列补全过程，通过语言空间层次结构解决语义歧义。

Result: 在nuScenes、V2X-Seq-SPD和KITTI数据集上表现出优越性能，在nuScenes数据集上，NOVA在Novel类别上达到22.41%的AMOTA，相比基线有20.21%的绝对提升。

Conclusion: NOVA通过生成式时空语义建模范式，成功解决了3D多目标跟踪中的开放词汇泛化问题，为开放世界感知提供了新思路。
...
TL;DR: OWL是一种基于视觉运动线索的3D感知函数，通过局部视觉逼近和刚体旋转感知，实现无需先验知识的实时3D场景重建和相机航向估计。
...
Method: 基于两个基本视觉运动线索：1) 注视点附近点的感知局部视觉逼近，2) 刚体相对于注视点的感知旋转。通过像素级并行计算，直接从原始视觉运动图像序列中提取OWL值，无需测量相对瞬时方向范围和方向平移。

Result: 模拟实验表明OWL实现了3D物体的几何恒常性，并能仅凭视觉运动线索进行缩放3D场景重建。该方法提供了一种实时、最小化计算的3D点相对运动表示方法。

Conclusion: OWL填补了理论概念与机器人、自主导航实际应用之间的空白，为下一代自主系统提供了基础构建模块，并为机器感知提供了新视角，对自然感知、行为心理学和神经功能理解也有启示意义。
...
### [12] [Can we Trust Unreliable Voxels? Exploring 3D Semantic Occupancy Prediction under Label Noise](https://arxiv.org/abs/2603.06279)
*Wenxin Li,Kunyu Peng,Di Wen,Junwei Zheng,Jiale Wei,Mengfei Duan,Yuheng Zhang,Rui Fan,Kailun Yang*

Main category: cs.CV

TL;DR: 该论文针对3D语义占据预测中真实世界体素标注存在结构伪影和动态拖尾效应的问题，提出了首个面向3D占据的噪声标签基准OccNL，并设计了抗噪声框架DPR-Occ，通过双源部分标签推理在极端噪声下保持性能。


<details>
  <summary>Details</summary>
Motivation: 3D语义占据预测是机器人感知的核心任务，但真实世界的体素标注存在结构伪影和动态拖尾效应等固有噪声。这引发了一个关键但未被充分探索的问题：自主系统能否安全依赖这种不可靠的占据监督？需要系统性地研究这一问题。

Method: 提出了DPR-Occ框架，采用双源部分标签推理方法构建可靠监督。该方法通过协同时间模型记忆和表示层面的结构亲和性，动态扩展和修剪候选标签集，以保留真实语义同时抑制噪声传播。
...
TL;DR: MultiHaystack是首个评估大规模跨模态检索与推理的基准测试，包含超过46,000个多模态检索候选和747个开放可验证问题，揭示了当前多模态大语言模型在异构多模态检索方面的主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估文本、图像或视频的单独理解能力，但缺乏对从大规模异构多模态语料库中检索相关证据后再进行推理这一关键现实需求的评估。大多数现有基准将检索限制在小规模、单模态候选集中，过度简化了搜索空间并夸大了端到端可靠性。

Method: 提出MultiHaystack基准测试，包含超过46,000个跨文档、图像和视频的多模态检索候选，以及747个开放但可验证的问题。每个问题都基于检索池中唯一的已验证证据项，要求跨模态的证据定位和细粒度推理。

Result: 研究发现：1）模型在提供相应证据时表现良好，但从完整语料库中检索证据时性能急剧下降；2）最强的检索器E5-V仅达到40.8%的Recall@1；3）最先进的多模态大语言模型如GPT-5，在提供相应证据时推理准确率为80.86%，但在top-5检索下降至51.4%。

Conclusion: 异构多模态检索仍然是多模态大语言模型的主要瓶颈，MultiHaystack作为一个有价值的测试平台，揭示了小规模评估所掩盖的潜在局限性，并促进了多模态系统中以检索为中心的进展。
...
TL;DR: 本文提出Audiovisual Geolocation框架，通过视听融合解决地理定位难题，在高质量全球视频基准上显著超越单模态基线。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在基于图像的定位方面虽有进展，但精确的全球地理定位仍面临挑战，主要原因是视觉景观的固有模糊性以及听觉线索的潜力尚未充分挖掘。
...
Conclusion: 视听融合的地理定位框架通过可解释的声学感知和多模态推理，有效解决了地理模糊性问题，为高精度全球定位提供了新途径。
...
TL;DR: 将ImageNet训练集自动转换为多标签数据集，无需人工标注，通过自监督ViT进行无监督物体发现，生成高质量多标签标注，提升模型性能


<details>
  <summary>Details</summary>
Motivation: 原始ImageNet基准强制单标签假设，但许多图像包含多个物体，导致标签噪声并限制学习信号的丰富性。多标签标注能更准确地反映现实世界视觉场景，让模型学习更丰富、更鲁棒的表征

Method: 使用自监督视觉变换器进行无监督物体发现，选择与原始标签对齐的区域训练轻量级分类器，然后应用于所有区域生成整个数据集的一致多标签标注

Result: 生成的多标签标注在定性评估中与人类判断高度一致，在定量基准测试中持续提升性能。相比传统单标签方案，使用多标签监督训练的模型在不同架构上获得更好的域内准确率（在ReaL上提升达+2.0 top-1准确率，在ImageNet-V2上提升+1.5），并在下游任务中表现出更强的迁移能力（在COCO和VOC上分别提升达+4.2和+2.3 mAP）

Conclusion: 准确的多标签标注对于提升分类性能和表征学习至关重要，提出的自动化管道成功将ImageNet训练集转换为高质量多标签数据集，显著改善了模型性能
...
Conclusion: 该方法通过自动创建手术时间线和叙述，减少了外科医生手动视频审查和标注的需求，为工具辅助手术提供了更高效的解决方案
...
TL;DR: Icarus是一个全天气天空模型，能够学习全动态范围物理捕捉户外图像，实现高精度、逼真的天空环境图生成，特别解决了太阳区域建模的难题。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习天空模型在重现准确、逼真的自然天空方面存在不足，特别是在高分辨率下难以准确建模14EV+类别不平衡的太阳区域，导致视觉质量差和光照失真。

Method: 提出Icarus模型，能够学习全动态范围物理捕捉户外图像的曝光范围，支持条件生成环境图，用户可以直观地定位太阳和云层形态，并控制大气形态的纹理。

Result: Icarus可与全动态范围物理捕捉户外图像或参数化天空模型互换，在基于图像的照明中实现了前所未有的准确性、逼真度、光照方向性（阴影）和色调。

Conclusion: Icarus是一个功能强大的全天气天空模型，能够生成高质量的环境图，解决了现有模型在太阳区域建模和光照准确性方面的关键问题。
...
TL;DR: LayerBind：一种无需训练的文本到图像生成区域布局控制方法，通过分层建模和绑定实现精确的区域和遮挡控制


<details>
  <summary>Details</summary>
Motivation: 现有区域布局控制方法存在数据偏见、图像质量下降以及遮挡顺序处理困难的问题，需要一种更实用、无需训练的方法来精确控制图像生成中的区域布局和遮挡关系

Method: 采用两阶段方案：1) 实例初始化阶段，利用多模态联合注意力中的上下文共享机制，创建每个实例的分支并融合形成预布局的潜在表示；2) 语义护理阶段，通过分层注意力增强机制强化区域细节并保持遮挡顺序，使用分层透明度调度器进行更新组合

Result: LayerBind在定性和定量评估中都表现出有效性，能够实现精确的区域控制和遮挡顺序管理，同时保持图像质量，支持可编辑的工作流程

Conclusion: LayerBind作为一种无需训练、即插即用的区域和遮挡控制器，在扩散变换器中表现出强大的应用潜力，特别适合创意应用场景
...
TL;DR: BM25-V：将文本检索中的BM25算法应用于视觉词稀疏编码，实现高效可解释的图像检索


<details>
  <summary>Details</summary>
Motivation: 密集图像检索虽然准确，但可解释性差、计算成本高，且难以溯源。需要一种既高效又能提供可解释性的检索方法。

Method: 使用稀疏自编码器(SAE)从Vision Transformer的patch特征中提取稀疏视觉词激活，然后应用Okapi BM25算法进行评分。BM25的逆文档频率(IDF)权重能有效抑制常见低信息词，突出罕见判别性词。

Result: 在7个基准测试中，BM25-V的Recall@200 ≥ 0.993，作为两阶段检索的第一阶段，仅需对200个候选进行重排序，就能恢复接近密集检索的准确率（平均差距0.2%）。SAE在ImageNet-1K上训练一次即可零样本迁移到7个细粒度基准。

Conclusion: BM25-V提供了一种高效、可解释的图像检索方法，通过稀疏倒排索引操作实现高召回率，检索决策可追溯到特定视觉词及其IDF贡献，为密集检索提供了轻量级替代方案。
...
Method: 提出LIPAR框架：1) 检测并跳过重复的潜在补丁计算；2) 引入注意力恢复机制，近似被修剪token的注意力值，避免视觉伪影。
...
TL;DR: Cog2Gen3D：一个3D认知引导的扩散框架，通过语义和几何表示的双流融合来解决3D生成中的空间几何约束问题，显著提升语义保真度和几何合理性。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法通常使用几何特征作为条件，但只能建模相对关系，容易产生绝对几何尺度不一致的问题。作者认为语义信息和绝对几何共同赋能3D认知，从而实现物理世界的可控3D生成。

Method: 提出Cog2Gen3D框架，包含三个关键设计：1) 认知特征嵌入：将多模态编码为语义和几何表示，并提取逻辑表示；2) 3D潜在认知图：构建双流语义-几何图，通过基于公共的交叉注意力融合；3) 认知引导潜在扩散：使用融合的3D认知图作为条件，引导潜在扩散过程生成3D高斯。

Result: 在基于Marble World Labs构建的验证子集上进行广泛实验，Cog2Gen3D在语义保真度和几何合理性方面显著优于现有方法。

Conclusion: Cog2Gen3D通过统一的3D认知图框架，确保了3D生成的物理合理性和结构合理性，为解决3D生成中的空间几何约束问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频稳定面临几何鲁棒性和全帧一致性的基本权衡。2D方法存在过度裁剪问题，而3D技术则因脆弱的优化流程在极端运动下容易失效。
...
Conclusion: VS3R通过协同3D重建和生成式视频扩散，成功解决了视频稳定中的关键挑战，实现了更可靠、更高质量的稳定效果。
...
TL;DR: 本文提出了TumorChain框架，通过多模态交错推理实现临床肿瘤分析，在1.5M CoT标注的3D CT扫描数据集上验证了其在病灶检测、印象生成和病理分类方面的优越性能。


<details>
  <summary>Details</summary>
Motivation: 准确的肿瘤分析对临床放射学和精准肿瘤学至关重要，但现有方法缺乏可追溯的推理过程，容易产生诊断错误。需要一种能够从影像发现到临床印象再到病理结论进行逐步解释的推理框架。

Method: 提出了TumorChain多模态交错推理框架，结合3D影像编码器、临床文本理解和器官级视觉语言对齐。通过跨模态对齐和迭代交错因果推理，基于视觉证据聚合结论，经过多轮自我精炼后输出病理预测。

Result: 在病灶检测、印象生成和病理分类方面均优于强基线方法，在DeepTumorVQA基准测试上表现出强大的泛化能力，减少了幻觉风险并提高了可追溯性。

Conclusion: 多模态推理框架TumorChain为临床实践提供了可靠且可解释的肿瘤分析解决方案，通过逐步推理提高了诊断准确性和可追溯性，具有重要的临床应用价值。
...
TL;DR: PatchCue是一种基于图像块的视觉提示范式，通过将图像分割成块并生成块级视觉提示，显著提升视觉语言模型的推理能力，相比像素级边界框和点级提示更有效。


<details>
  <summary>Details</summary>
Motivation: 现有推理范式（如CoT）主要依赖文本信息，未能充分利用重要视觉线索。虽然先前工作引入了像素级视觉线索，但这些表示需要精确的空间定位，增加了学习复杂度。需要一种更符合人类感知习惯且与现代VLM的patch-tokenized输入对齐的视觉提示方法。

Method: 提出PatchCue，一种基于图像块的视觉提示范式。将图像分割成块并在块级别表示视觉线索，与现代VLM的patch-tokenized输入对齐。采用两阶段训练方法：1）冷启动监督微调以输出块级提示；2）使用过程监督提示奖励的强化学习来指导中间视觉推理步骤。

Result: 在多个VLM和多样化基准测试（包括通用视觉问答、复杂推理和文档理解）上的广泛实验表明，PatchCue持续提升整体模型性能。块级提示在性能上优于像素级边界框和点级提示。

Conclusion: PatchCue提供了一种更有效且认知对齐的视觉推理范式，通过块级视觉提示显著增强了视觉语言模型的推理能力，为视觉理解任务提供了新的解决方案。
...
TL;DR: 论文系统评估了合成新视角在视频地点识别中的应用，发现小规模合成添加能提升识别效果，而大规模添加时，视角变化幅度不如添加视图数量和数据集图像类型重要。


<details>
  <summary>Details</summary>
Motivation: 合成新视角的生成对机器人导航有积极影响：在基于图像的导航中，地面机器人拍摄的场景可以生成新的俯视视角来引导空中机器人；在视频地点识别中，空中视角的地面新视图可以帮助无人机识别地面机器人看到的地点，反之亦然。

Method: 使用五个公共VPR图像数据库和七种典型的图像相似性方法，对合成新视角在VPR中进行系统评估。

Result: 对于小规模合成添加，新视角能改善VPR识别统计；对于大规模添加，视角变化的幅度不如添加视图数量和数据集图像类型重要。

Conclusion: 合成新视角在视频地点识别中具有实用价值，特别是在小规模添加时能提升识别性能，为跨视角机器人导航提供了技术支持。
...
### [26] [CORE-Seg: Reasoning-Driven Segmentation for Complex Lesions via Reinforcement Learning](https://arxiv.org/abs/2603.05911)
*Yuxin Xie,Yuming Chen,Yishan Yang,Yi Zhou,Tao Zhou,Zhen Zhao,Jiacheng Liu,Huazhu Fu*

Main category: cs.CV

TL;DR: 该论文提出了CORE-Seg框架，通过语义引导提示适配器将推理与分割结合，用于医学图像中复杂病灶的分割，在ComLesion-14K基准上取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割正从传统的视觉模式匹配转向认知推理分析。现有的多模态大语言模型虽然具有通用常识，但缺乏复杂病灶所需的专业视觉推理能力；而传统分割模型擅长像素级分割但缺乏逻辑可解释性。

Method: 提出了CORE-Seg端到端框架，通过语义引导提示适配器将推理与分割结合。设计了从SFT到GRPO的渐进训练策略，并配备了自适应双粒度奖励机制来缓解奖励稀疏问题。

Result: 在ComLesion-14K基准上取得了37.06%的平均Dice分数（比第二好的基线高14.89%），同时将失败率降低到18.42%，达到了最先进的性能。

Conclusion: CORE-Seg框架成功地将推理能力与分割任务结合，为医学图像中复杂病灶的分割提供了有效的解决方案，在推理驱动的复杂病灶分割任务上取得了显著改进。
...
TL;DR: CylinderSplat：一种用于全景3D高斯泼溅的前馈框架，通过圆柱形Triplane表示和双分支架构解决稀疏视图下的遮挡问题，在单视图和多视图全景新视图合成中达到SOTA性能。
...
Method: 提出CylinderSplat框架，核心是新的圆柱形Triplane表示，与全景数据和曼哈顿世界假设更匹配。采用双分支架构：像素分支重建观测良好区域，体积分支利用圆柱形Triplane补全遮挡或稀疏观测区域。支持从单视图到多视图的灵活输入。

Result: 大量实验表明，CylinderSplat在单视图和多视图全景新视图合成中均达到最先进结果，在重建质量和几何精度方面均优于先前方法。

Conclusion: CylinderSplat通过圆柱形Triplane表示和双分支架构有效解决了全景3D高斯泼溅中的几何表示和遮挡问题，为全景场景的实时新视图合成提供了有效解决方案。
...
### [28] [BlackMirror: Black-Box Backdoor Detection for Text-to-Image Models via Instruction-Response Deviation](https://arxiv.org/abs/2603.05921)
*Feiran Li,Qianqian Xu,Shilong Bao,Zhiyong Yang,Xilin Zhao,Xiaochun Cao,Qingming Huang*

Main category: cs.CV

TL;DR: BlackMirror是一个用于黑盒环境下检测文本到图像模型中后门攻击的新框架，通过分析生成图像中语义模式的稳定性来识别后门行为。


<details>
  <summary>Details</summary>
Motivation: 现有检测方法主要依赖图像级相似性分析，假设后门触发的生成图像具有强一致性，但难以应对新兴的后门攻击，这些攻击产生的图像在视觉上可能具有多样性。

Method: BlackMirror包含两个组件：MirrorMatch将视觉模式与相应指令对齐以检测语义偏差；MirrorVerify评估这些偏差在不同提示下的稳定性，以区分真正的后门行为和良性响应。

Result: 综合实验表明，BlackMirror能够准确检测各种攻击，是一个无需训练、可即插即用的通用框架，适用于模型即服务应用。

Conclusion: 该论文提出了一个有效的黑盒后门检测框架，通过分析语义模式稳定性而非图像相似性，能够应对视觉多样化的后门攻击，具有实际应用价值。
...
TL;DR: PixARMesh是一种从单张RGB图像自回归重建完整3D室内场景网格的方法，无需后处理布局优化，直接生成连贯、可用的网格


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖隐式符号距离场和后处理布局优化，无法直接生成连贯的3D场景网格。需要一种能够从单张图像直接生成完整、可用3D场景网格的方法

Method: 基于点云编码器，通过像素对齐的图像特征和跨注意力机制整合全局场景上下文，实现从单图像的空间推理。采用自回归方式从统一的token流（包含上下文、姿态和网格）生成场景

Result: 在合成和真实数据集上达到最先进的重建质量，生成轻量级、高质量的网格，可直接用于下游应用

Conclusion: PixARMesh能够从单张RGB图像直接生成完整、连贯的3D室内场景网格，避免了传统方法的复杂后处理流程，为下游应用提供了高质量的3D场景重建方案
...
TL;DR: RAC（Rectified Flow Auto Coder）是一种基于Rectified Flow的新型自编码器，通过多步解码、双向推理和生成式解码机制，在保持高质量重建的同时显著提升生成性能，计算成本降低约70%。


<details>
  <summary>Details</summary>
Motivation: 传统VAE存在重建与生成之间的性能差距问题，且计算效率不高。作者希望设计一种既能保持高质量重建，又能提升生成性能，同时降低计算成本的替代方案。

Method: RAC采用Rectified Flow框架，通过将解码器应用于流时间步实现多步解码；支持双向推理，解码器通过时间反转可作为编码器使用，减少参数约41%；采用生成式解码方法，允许在路径上修正潜在变量。

Result: 实验表明RAC在重建和生成任务上均超越当前最优的VAE模型，同时计算成本降低约70%。

Conclusion: RAC作为一种新型自编码器架构，通过Rectified Flow框架成功解决了传统VAE的重建-生成差距问题，在保持高质量重建的同时显著提升生成性能并降低计算成本。
...
TL;DR: InnoAds-Composer：一种单阶段电商产品海报生成框架，通过重要性分析和路由机制实现主题、文字和风格的三条件高效控制，解决了多阶段管道中存在的主题保真度差、文字不准确和风格不一致问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的产品海报生成方法通常采用多阶段管道，难以同时控制主题、文字和风格，存在主题保真度差、文字不准确和风格不一致三大问题。

Method: 提出单阶段框架InnoAds-Composer，通过重要性分析确定各条件最敏感的层和时间步，路由每个条件到最响应位置以缩短活动token序列；设计文本特征增强模块（TFEM）整合字形图像和裁剪特征提升中文文字渲染准确性；构建首个同时包含主题、文字和风格条件的高质量电商产品海报数据集。

Result: 实验表明InnoAds-Composer显著优于现有产品海报生成方法，且没有明显增加推理延迟。

Conclusion: InnoAds-Composer通过创新的三条件控制机制和文本特征增强，有效解决了电商产品海报生成中的关键问题，为高质量海报合成提供了高效的单阶段解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然概念瓶颈模型（CBM）理论上通过将图像映射到人类可解释的概念来增强可解释性并支持公平性，但实际中CBM概念会泄露与概念语义无关的信息，且在ImSitu等数据集上仅能边际减少性别偏见，需要改进其公平性表现。

Method: 提出三种偏差缓解技术：1. 使用top-k概念过滤器减少信息泄露；2. 移除有偏概念；3. 对抗性去偏。这些方法旨在改善CBM的公平性表现。

Result: 提出的去偏CBM在公平性与性能权衡方面优于先前工作，表明该方法在实现公平且可解释的图像分类方面迈出了重要一步。

Conclusion: 通过结合三种偏差缓解技术，改进后的概念瓶颈模型在保持可解释性的同时显著提升了公平性表现，为公平且可解释的图像分类提供了有效解决方案。
...
TL;DR: 提出CollabOD轻量级协作检测框架，解决无人机图像中小目标检测的尺度变化、结构细节退化等问题，通过结构细节保留、跨路径特征对齐和定位感知轻量化设计优化无人机感知模型。


<details>
  <summary>Details</summary>
Motivation: 无人机图像中的小目标检测面临三大挑战：尺度变化、结构细节退化和有限计算资源。在高空场景中，细粒度特征在分层下采样和跨尺度融合过程中进一步减弱，导致定位不稳定和鲁棒性降低。

Method: 提出CollabOD轻量级协作检测框架，包含三个核心策略：1) 结构细节保留；2) 跨路径特征对齐；3) 定位感知轻量化设计。从图像处理、通道结构和轻量化设计三个角度优化传统无人机感知模型架构，并采用统一的细节感知检测头提高回归鲁棒性。

Result: 该框架在保持高效推理的同时增强了表示稳定性，统一的细节感知检测头在不引入额外部署开销的情况下提高了回归鲁棒性。代码已开源。

Conclusion: CollabOD框架通过协同优化结构细节保留、特征对齐和轻量化设计，有效解决了无人机图像中小目标检测的关键挑战，在保持计算效率的同时提高了检测性能。
...
TL;DR: 本文提出Art3D框架，将2D转3D的目标从几何准确重建转向艺术性视差合成，以复制专业3D电影的艺术效果和情感体验。


<details>
  <summary>Details</summary>
Motivation: 现有2D转3D方法虽然几何准确但艺术性不足，无法复制专业3D电影的沉浸感和情感共鸣。传统方法将艺术意图（如零平面偏移、局部深度塑造）误认为是数据噪声或模糊性。

Method: 提出Art3D框架，采用双路径架构分离全局深度参数（宏观意图）和局部艺术效果（视觉笔触），通过间接监督从专业3D电影数据中学习，并引入初步的电影对齐评估方法。

Result: 实验表明该方法在复制关键的局部出屏效果和与电影3D内容的全局深度风格对齐方面显示出潜力，为艺术驱动的转换工具奠定了基础。

Conclusion: 本文提出了艺术视差合成的新范式，将2D转3D从物理准确估计转向艺术一致合成，Art3D框架展示了在艺术性3D转换方面的潜力。
...
TL;DR: Pano3DComposer：一个高效的前馈框架，用于从全景图像生成3D场景，通过解耦对象生成和布局估计，在约20秒内生成高保真3D场景。
...
Method: 提出Pano3DComposer框架，包含：1）可插拔的Object-World Transformation Predictor，将现成图像到3D模型生成的3D对象从局部坐标转换到世界坐标；2）基于VGGT架构改进的Alignment-VGGT，利用目标对象裁剪、多视角对象渲染和相机参数预测变换；3）使用伪几何监督训练解决生成对象与真实对象之间的形状差异；4）针对未见域输入图像，引入从粗到精对齐机制迭代优化几何一致性。

Result: 在合成和真实数据集上的图像/文本到3D任务中实现了优越的几何精度，在RTX 4090 GPU上约20秒内生成高保真3D场景。

Conclusion: Pano3DComposer通过解耦对象生成和布局估计，结合高效的坐标变换预测和几何对齐机制，解决了现有方法在效率和全景场景生成方面的限制，实现了快速高质量的360度3D场景生成。
...
TL;DR: 提出Skeleton-to-Image Encoding (S2I)方法，将3D人体骨架序列转换为图像格式，使预训练视觉模型能直接用于骨架表示学习，解决骨架数据格式差异和数据集稀缺问题。
...
Conclusion: S2I表示首次实现了使用预训练视觉模型进行自监督骨架表示学习，将丰富的视觉领域知识转移到骨架分析中，并为异构骨架数据提供了统一的图像格式。
...
Method: 提出两阶段框架：第一阶段训练过程编码器，通过自动生成中间帧获取关键帧，并通过caption-conditioned掩码重建任务学习潜在动态；第二阶段将训练好的编码器集成到编码器-解码器模型中，引入可学习的过程查询来推断潜在过程表示，然后解码为文本描述。

Result: 在三个数据集上的实验证明了ProCap的有效性，代码和预训练模型已开源。

Conclusion: 通过将变化建模从静态图像比较重新构建为动态过程建模，ProCap能够更好地理解和描述变化过程，为变化描述任务提供了新的视角和方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有不规则采样时间序列预测方法仅使用历史观测值，缺乏对上下文语义和细粒度时间模式的学习能力，需要更全面的多模态解决方案。

Method: 提出两阶段编码机制：1)跨模态视觉-文本编码模块自动生成信息丰富的视觉图像和文本数据；2)ISTS编码提取历史观测的时序特征；3)自适应查询特征提取器压缩MLLM标记；4)多模态对齐模块缓解模态差距。

Result: 在真实数据上的大量实验证明了所提解决方案的有效性。

Conclusion: MM-ISTS通过融合时间、视觉和文本模态，成功提升了不规则采样时间序列的预测性能，实现了对复杂时间模式和上下文语义的更好理解。
...
TL;DR: RAID是一个用于驾驶员风险感知研究的大规模数据集，包含4,691个标注视频片段，涵盖多种交通场景。作者提出了一个弱监督风险目标识别框架，通过建模驾驶员意图与响应之间的关系来识别潜在风险源，并在RAID和HDDS数据集上取得了显著性能提升。
...
Method: 1. 引入RAID数据集：包含4,691个标注视频片段，涵盖驾驶员意图操作、道路拓扑、风险情况、驾驶员响应和行人注意力等标签。2. 提出弱监督风险目标识别框架：通过建模驾驶员意图操作与响应之间的关系来识别潜在风险源。3. 分析行人注意力在风险评估中的作用。

Result: 实验评估表明，该方法在RAID数据集上比现有最先进方法性能提升20.6%，在HDDS数据集上提升23.1%。验证了所提数据集的价值和方法的有效性。

Conclusion: RAID数据集为驾驶员风险感知研究提供了重要资源，提出的弱监督风险目标识别框架能够有效识别潜在风险源，行人注意力分析进一步增强了风险评估能力，为实现零碰撞移动性目标提供了技术支持。
...
TL;DR: TAR-ViTPose：一种用于视频2D人体姿态估计的时序聚合与恢复视觉Transformer，通过聚合跨帧的时序信息提升ViT在视频中的姿态估计性能


<details>
  <summary>Details</summary>
Motivation: 现有基于ViT的姿态估计器为静态图像设计，独立处理每帧，忽略了视频中的时序一致性，导致在运动模糊、遮挡或失焦等挑战性场景中预测不稳定

Method: 提出TAR-ViTPose，包含两个核心模块：1) 关节中心时序聚合(JTA)：为每个关节分配可学习的查询令牌，选择性地关注相邻帧中对应区域；2) 全局恢复注意力(GRA)：将聚合的时序特征恢复回当前帧的令牌序列，丰富姿态表示同时保留全局上下文

Result: 在PoseTrack2017基准测试上比单帧基线ViTPose提升+2.3 mAP，优于现有最先进的视频方法，同时在真实应用中实现更高的运行时帧率

Conclusion: TAR-ViTPose通过有效的时序聚合和恢复机制，显著提升了视频中2D人体姿态估计的鲁棒性和准确性，同时保持了高效的推理速度
...
### [42] [FTSplat: Feed-forward Triangle Splatting Network](https://arxiv.org/abs/2603.05932)
*Xiong Jinlin,Li Can,Shen Jiawei,Qi Zhigang,Sun Lei,Zhao Dongyang*

Main category: cs.CV

TL;DR: 提出了一种前馈三角形基元生成框架，能够从标定多视角图像直接预测连续三角形表面，实现单次前向传播生成仿真就绪模型，无需逐场景优化或后处理。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF和3D高斯泼溅方法虽然渲染质量高，但依赖耗时的逐场景优化，限制了实时部署。新兴的前馈高斯泼溅方法效率有所提升，但缺乏显式的流形几何结构，无法直接用于仿真。

Method: 提出前馈三角形基元生成框架，包含像素对齐的三角形生成模块，并引入相对3D点云监督来增强几何学习的稳定性和一致性。

Result: 实验表明该方法能够实现高效重建，同时保持与标准图形和机器人仿真器的无缝兼容性。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在自动驾驶应用中越来越普遍，旨在利用其推理和泛化能力处理长尾场景。然而，这些模型经常在自动驾驶相关的简单视觉问题上失败，且失败原因尚不明确。

Method: 创建仅针对特定视觉概念不同的反事实图像集，使用四种最先进的视觉语言模型的激活训练线性探针，分析视觉概念是否被线性编码。

Result: 1. 物体或智能体存在等概念被显式线性编码；2. 物体朝向等空间视觉概念仅通过视觉编码器保留的空间结构隐式编码；3. 识别出两种故障模式：感知失败（视觉信息未线性编码）和认知失败（信息存在但未与语言语义正确对齐）；4. 物体距离增加会快速降低相应视觉概念的线性可分性。

Conclusion: 该研究提高了对视觉语言模型在自动驾驶相关简单视觉任务中失败案例的理解，揭示了视觉信息在模型中的编码方式和故障模式，为改进模型在自动驾驶应用中的性能提供了见解。
...
TL;DR: OD-RASE框架通过本体论驱动的方法，结合大规模视觉语言模型和扩散模型，自动检测导致交通事故的道路结构并生成基础设施改进方案，以提升自动驾驶系统的安全性。
...
Method: 1) 基于道路交通系统专业知识形式化本体论；2) 使用大规模视觉语言模型生成基础设施改进方案；3) 通过本体论驱动数据过滤提升方案可靠性；4) 在事故前道路图像上自动标注改进方案构建新数据集；5) 提出OD-RASE基准方法，结合LVLM和扩散模型生成改进方案和改进后道路环境的生成图像。

Result: 实验表明，本体论驱动的数据过滤能够高精度预测导致事故的道路结构和相应的改进计划。构建了包含改进方案标注的新数据集。

Conclusion: 这项工作有助于提升整体交通环境安全性，是推动自动驾驶系统更广泛应用的重要一步。OD-RASE框架为主动识别和修复危险道路结构提供了系统化方法。
...
TL;DR: TempoSyncDiff是一个基于参考条件的潜在扩散框架，通过师生蒸馏方法实现高效音频驱动说话头生成，减少推理步骤并提高稳定性


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在说话头生成中存在高推理延迟、时间不稳定（闪烁和身份漂移）以及音频-视觉对齐不完美等问题，需要在实际计算受限环境下实现实用的说话头生成

Method: 采用参考条件的潜在扩散框架，使用师生蒸馏方法：用标准噪声预测目标训练扩散教师模型，指导轻量级学生去噪器以更少推理步骤运行；加入身份锚定和时间正则化减少身份漂移和闪烁；基于音素的音频条件提供粗粒度唇部运动控制

Result: 在LRS3数据集上的实验显示，蒸馏扩散模型能保留强教师模型的大部分重建行为，同时实现显著降低的延迟推理；包括CPU和边缘计算的延迟测量表明边缘部署可行性
...
<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法通常只能表达固定目标情感，缺乏人类在传达信息时展现的连续自然情感变化能力，需要解决动态情感表达问题

Method: 提出TIE-TFG模型，采用时序密集情感波动建模技术，根据输入文本生成对应的情感变化序列，驱动合成视频中连续的面部表情变化

Result: 广泛评估表明该方法能够产生平滑的情感过渡，在不同情感状态下保持高质量的视觉效果和运动真实性

Conclusion: EC-TFG任务和TIE-TFG模型成功解决了说话人脸生成中动态情感表达的挑战，实现了更自然、情感连续的视频合成
...
TL;DR: 提出了一种基于自适应径向投影的文档图像倾斜估计方法，并创建了DISE-2021数据集进行评估
...
Conclusion: 该方法为文档倾斜估计提供了有效的解决方案，同时创建了DISE-2021数据集用于评估，并对基于傅里叶的方法进行了全面的改进分析。
...
<details>
  <summary>Details</summary>
Motivation: 生成式真实世界图像超分辨率存在语义和结构幻觉问题，即输出可能看起来清晰但与低分辨率输入证据不忠实。基于偏好的强化学习虽然适合，但面临三个主要障碍：缺乏退化鲁棒的LR参考忠实度信号、rollout组优化瓶颈导致优势崩溃、以及真实退化覆盖有限限制了rollout多样性和偏好信号质量。
...
Conclusion: LucidNFT通过解决生成式真实世界超分辨率中的关键挑战，包括忠实度评估、优化瓶颈和退化覆盖，为基于流的真实世界超分辨率提供了有效的多奖励强化学习框架，实现了更好的感知质量与忠实度的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 少样本分割在有限监督下对新类别泛化能力有限，面临结构错位和跨视角不一致的挑战，特别是在外观或视角变化大的情况下。

Method: VINE引入空间-视角图在骨干特征上建模，空间图捕捉局部几何拓扑，视角图连接不同视角特征传播视角不变的结构语义。利用支持-查询特征差异获取判别性先验，通过掩码交叉注意力渐进整合前景增强的SAM特征和结构丰富的ResNet特征，生成类别一致的原型作为SAM解码器的自适应提示。
...
Method: 采用"思考-放置"范式：1) MLLM进行物理场景理解和交互推理，生成环境感知的思维链令牌和有效插入区域；2) 引入MLLM引导的空间直接偏好优化，通过反馈评分提升视觉自然度；3) 推理时MLLM迭代触发精炼循环，形成闭环系统；4) 提供两种用户可选模式：增强物理合理性的灵活模式和保持场景完整性的标准模式。

Result: 大量实验表明，Place-it-R1相比最先进解决方案和商业模型，能够实现物理一致性的视频对象插入，在物理合理性方面表现优异。

Conclusion: 该框架通过结合MLLM的认知推理能力和视频扩散模型的生成能力，实现了物理一致的视频对象插入，为用户提供了在合理性和保真度之间的明确控制。
...
TL;DR: OVGGT：一种训练免费的框架，通过自选择缓存和动态锚点保护技术，在固定内存和计算预算下处理任意长视频的3D几何重建


<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型使用全注意力机制导致二次计算成本，只能处理短序列；因果注意力变体如StreamVGGT虽然支持单次流式处理，但KV缓存会不断增长，在数百帧内耗尽GPU内存，无法实现真正意义上的长序列流式推理

Method: 提出OVGGT框架：1）自选择缓存：利用FFN残差幅度压缩KV缓存，同时保持与FlashAttention完全兼容；2）动态锚点保护：保护坐标关键令牌不被驱逐，抑制长轨迹中的几何漂移

Result: 在室内、室外和超长序列基准测试中，OVGGT能够在恒定VRAM预算内处理任意长视频，同时达到最先进的3D几何精度

Conclusion: OVGGT通过训练免费的方法解决了长视频3D几何重建中的内存和计算瓶颈，实现了真正的流式推理能力
...
TL;DR: VLM-RobustBench评估视觉语言模型在49种图像失真下的鲁棒性，发现当前VLM语义理解强但空间感知脆弱，几何和重采样失真对性能影响最大。
...
Method: 构建VLM-RobustBench基准，涵盖49种增强类型（噪声、模糊、天气、数字、几何扰动），在分级严重程度（低/中/高）和二元变换下评估，共133个失真设置。评估四个VLM家族（Qwen、InternVL、Molmo、Gemma）在两个基准上：MMBench（视觉基础）和MMMU-Pro（推理导向）。

Result: 视觉严重程度是性能下降的弱预测因子：低严重度的空间扰动通常比视觉上严重的光度失真对性能影响更大。低严重度的glass_blur使MMBench准确率平均下降约8个百分点，而最大下降来自重采样和几何失真（如上采样、弹性变换），可达34个百分点。

Conclusion: 当前VLM语义理解能力强但空间感知脆弱，需要定义新的鲁棒性评估协议和训练机制，强调重采样和几何不变性。
...
### [53] [Exploring Open-Vocabulary Object Recognition in Images using CLIP](https://arxiv.org/abs/2603.05962)
*Wei Yu Chen,Ying Dai*

Main category: cs.CV

TL;DR: 提出基于两阶段策略的开放词汇目标识别框架：先分割再识别，无需复杂重训练和人工标注，通过CLIP生成图像和文本嵌入，引入CNN/MLP方法增强编码灵活性，使用SVD构建共享表示空间，通过嵌入相似度匹配实现识别。
...
Result: 在COCO、Pascal VOC和ADE20K数据集上的实验表明，无需训练的CLIP编码（无SVD）获得了最高的平均AP，超越了当前最先进方法。同时证明了CNN/MLP图像编码在开放词汇目标识别中的潜力。

Conclusion: 提出的开放词汇目标识别框架通过简洁的两阶段策略，在无需复杂重训练和人工标注的情况下，实现了优异的性能，为开放词汇识别提供了新的有效解决方案。
...
TL;DR: 本文提出RF-Sampling，一种专门为流匹配模型设计的无训练推理增强框架，通过线性组合文本表示和流反演来提升生成质量和文本对齐。


<details>
  <summary>Details</summary>
Motivation: 当前推理增强策略主要适用于传统扩散模型，但在流模型上表现不佳。需要为FLUX等基于CFG蒸馏的流模型设计专门的推理增强方法。

Method: 提出反射流采样(RF-Sampling)，基于理论推导证明其隐式执行文本-图像对齐分数的梯度上升。通过线性组合文本表示并与流反演结合，探索与输入提示更一致的噪声空间。

Result: 在多个基准测试中，RF-Sampling一致提升了生成质量和提示对齐。这是首个在FLUX上展示一定程度测试时缩放能力的推理增强方法。

Conclusion: RF-Sampling为流模型提供了有效的推理增强框架，填补了传统扩散模型增强技术无法适用于流模型的空白，具有理论依据且无需额外训练。
...
TL;DR: 提出两阶段框架解决字形相似度度量问题：第一阶段在有标签人造字母表上训练教师模型，第二阶段通过师生蒸馏扩展到历史文字系统，实现无监督表示学习。


<details>
  <summary>Details</summary>
Motivation: 字形和文字系统相似度度量面临根本挑战：人造字母表中的单个字素可以可靠标注，但不同文字系统之间的历史关系不确定且有争议。

Method: 两阶段框架：1）在有标签的人造字母表上使用对比损失训练编码器，建立具有鲁棒判别特征的教师模型；2）通过师生蒸馏扩展到历史文字系统，学生在教师指导下学习无监督表示，但可以自由发现潜在的跨文字相似性。

Result: 在多样化的文字系统上实验表明，该方法实现了有效的少样本字形识别和有意义的文字聚类，无需真实进化关系标注。

Conclusion: 该框架桥接了监督对比学习和无监督发现，既能区分不同文字系统的硬边界，又能反映潜在历史影响的软相似性，解决了文字相似度度量的根本挑战。
...
TL;DR: CR-QAT框架通过课程量化和关系知识蒸馏，解决了开放词汇目标检测在极端低比特量化下视觉-语言对齐和区域关系结构退化的问题。
...
Method: 提出课程关系量化感知训练（CR-QAT）框架，结合阶段优化和关系知识蒸馏。课程QAT（CQAT）通过分区模型进行渐进量化，通过误差隔离确保稳定优化；文本中心关系KD（TRKD）构建文本锚定的成对相似度矩阵，全面传递教师模型的多维关系知识。

Result: 在LVIS和COCO零样本基准测试中，CR-QAT在激进低比特设置下始终优于现有QAT基线，分别实现了高达38.9%和40.9%的相对AP改进。

Conclusion: CR-QAT框架有效解决了开放词汇目标检测在极端量化下的性能退化问题，通过课程量化和关系知识蒸馏的结合，在保持模型压缩的同时显著提升了检测精度。
...
Method: 1. 构建新颖的深度图像-文本对数据集和深度指令数据集；2. 使用GLPN模型从视觉图像生成深度图；3. 利用GPT-4策划相应的深度指令；4. 改进CLIP中的ViT编码器以融入局部物体信息，更好地捕捉深度连续变化；5. 基于现有深度图像数据集开发深度问答基准测试。

Result: 实验结果表明，DeepSight显著增强了深度感知能力和下游任务性能，在多模态三维理解方面取得了实质性进展。

Conclusion: DeepSight作为首个专门用于深度感知的MLLM，通过创新的数据集构建和模型改进，有效解决了现有模型在深度信息理解方面的不足，推动了三维场景理解的发展。
...
TL;DR: DynUAV是一个新的无人机视角多目标跟踪基准，专注于动态场景下的挑战，包含剧烈自运动和复杂表观轨迹，包含42个视频序列和170万+标注框，涵盖车辆、行人和工业设备等类别。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角MOT基准缺乏真实世界中的动态复杂性，通常只有可预测的相机运动和线性运动模式。无人机快速移动和敏捷机动带来的显著观测挑战在现有基准中未能充分体现。

Method: 提出了DynUAV基准，包含42个视频序列，超过170万个边界框标注，涵盖车辆、行人和工业设备（挖掘机、推土机、起重机）等类别。基准特别关注由剧烈自运动带来的挑战，包括尺度剧烈变化、视角变化和运动模糊。

Result: 对现有最先进的跟踪器在DynUAV上进行全面评估，揭示了它们在动态条件下的局限性，特别是在检测和关联的相互交织挑战方面。基准被确立为一个严格的测试平台。

Conclusion: DynUAV作为一个具有挑战性的测试平台，将推动真实世界无人机视角多目标跟踪的进展。所有资源将在指定链接上公开。
...
TL;DR: CoE是一个无需训练的多模态摘要框架，通过事件链和层次事件图进行结构化推理，解决了现有方法在跨模态对齐和时序建模方面的不足，在多个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态摘要方法存在三个主要问题：1)依赖领域特定监督；2)隐式融合导致跨模态对齐弱；3)平坦的时序建模缺乏事件过渡。需要一种更结构化、无需训练的方法来改善跨模态理解和事件演化建模。

Method: 提出CoE框架，基于层次事件图(HEG)进行事件链推理。HEG将文本语义编码为显式的事件层次结构，支撑跨模态对齐和时序推理。CoE通过该结构定位关键视觉线索、建模事件演化和因果过渡，并通过轻量级风格适应进行领域对齐。

Result: 在八个不同数据集上的实验表明，CoE持续优于最先进的视频CoT基线方法，平均提升：ROUGE +3.04、CIDEr +9.51、BERTScore +1.88，显示出其鲁棒性、可解释性和跨领域泛化能力。

Conclusion: CoE通过结构化的事件链推理和层次事件图，有效解决了多模态摘要中的关键挑战，提供了一种无需训练、可解释且具有强大跨领域泛化能力的解决方案。
...
TL;DR: TaPD是一个用于自动驾驶轨迹预测的通用即插即用框架，通过渐进式知识蒸馏和时间回填机制，有效处理可变历史观测长度，在短观测情况下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测器通常假设固定长度的历史观测，但在真实场景中（如遮挡或感知范围限制），观测长度可变或极短会导致性能显著下降，需要一种能适应不同观测长度的统一预测框架。

Method: 提出TaPD框架，包含两个协同模块：观测自适应预测器（OAF）和时序回填模块（TBM）。OAF基于渐进式知识蒸馏（PKD），将长时观测"教师"的运动模式知识通过分层特征回归传递给短时观测"学生"，并引入余弦退火蒸馏权重方案平衡预测监督和特征对齐。TBM在极短历史情况下，基于场景演化回填缺失的历史片段，生成上下文丰富的轨迹以增强PKD。

Result: 在Argoverse 1和Argoverse 2数据集上的广泛实验表明，TaPD在所有观测长度下均优于强基线方法，在极短输入情况下提升尤其显著，并能以即插即用方式改进其他预测器（如HiVT）。

Conclusion: TaPD通过渐进式知识蒸馏和时序回填的协同设计，有效解决了轨迹预测中可变历史观测长度的挑战，在真实场景中具有更好的适应性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于透视图像训练的深度基础模型在全景图像上泛化能力差，因为透视域和全景域之间存在显著的几何差异。完全微调这些模型通常需要大量全景数据，这限制了实际应用。
...
Conclusion: 通过将重点从互补投影融合转移到在保留预训练透视先验下的全景域适应，RePer-360在保持强大透视先验的同时有效适应全景域，显著减少了所需训练数据量并提升了性能。
...
TL;DR: HiPP-Prune：一种分层偏好条件结构化剪枝框架，用于视觉语言模型，通过策略输出全局剪枝蓝图，在多个目标下实现可控的鲁棒性-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型剪枝面临挑战，压缩不仅影响任务效用，还会加剧物体幻觉问题，即使在相同稀疏度下也会放大视觉定位问题。

Method: 提出分层偏好条件剪枝框架，将剪枝视为多目标下的条件资源分配。策略输出全局剪枝蓝图，分解为总体稀疏度预算和分层分配。集成视觉敏感度信号，防止过度剪枝视觉关键层。使用计划级组相对策略优化在多目标回报下优化剪枝计划。

Result: 在LLaVA模型上的POPE和ScienceQA实验表明，HiPP-Prune能够发现多样化的非支配剪枝计划，在匹配稀疏度预算下提供可控的鲁棒性-效用权衡。

Conclusion: HiPP-Prune框架通过分层偏好条件剪枝，有效解决了视觉语言模型剪枝中的多目标优化问题，实现了可控的鲁棒性-效用权衡，为高效部署提供了新方法。
...
TL;DR: DEX-AR是一种针对自回归视觉语言模型的新型可解释性方法，通过动态头过滤和序列级过滤机制，生成逐令牌和序列级的热力图来解释模型决策过程。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型变得越来越复杂和广泛应用，理解其决策过程变得至关重要。传统的可解释性方法针对分类任务设计，难以处理现代自回归VLM的逐令牌生成过程以及视觉和文本模态之间的复杂交互。

Method: 提出DEX-AR方法，通过计算逐令牌生成过程中注意力图的层间梯度来解释自回归VLM。引入两个关键创新：1）动态头过滤机制识别关注视觉信息的注意力头；2）序列级过滤方法聚合逐令牌解释，区分视觉基础和纯语言令牌。

Result: 在ImageNet、VQAv2和PascalVOC数据集上的评估显示，该方法在基于扰动的新颖归一化困惑度指标和基于分割的指标上都表现出一致的改进。

Conclusion: DEX-AR成功解决了自回归视觉语言模型可解释性的挑战，通过动态过滤机制和序列级聚合，为模型决策过程提供了更准确和全面的解释。
...
TL;DR: EffectMaker是一个统一的推理-生成框架，通过多模态大语言模型解释高级特效语义，结合扩散变换器进行上下文学习，实现无需逐特效微调的参考视频特效定制化生成。


<details>
  <summary>Details</summary>
Motivation: 当前视觉特效(VFX)制作需要专业知识和高昂成本，现有AIGC系统面临特效特定数据稀缺、超自然或风格化效果建模困难等挑战，且通常需要逐特效微调，限制了系统的可扩展性和对新特效的泛化能力。

Method: EffectMaker采用统一推理-生成框架：1) 多模态大语言模型解释高级特效语义并推理如何适应目标主体；2) 扩散变换器通过上下文学习从参考视频中捕捉细粒度视觉线索；3) 语义-视觉双路径引导机制实现准确、可控、特效一致的合成；4) 构建EffectData数据集（13万视频，3000个VFX类别）提升泛化能力。

Result: 实验表明EffectMaker在视觉质量和特效一致性方面优于现有最先进基线方法，提供了一个可扩展且灵活的自定义VFX生成范式。

Conclusion: EffectMaker通过语义-视觉双路径引导机制，无需逐特效微调即可实现准确、可控的特效定制化生成，解决了现有AIGC系统在VFX生成中的可扩展性和泛化问题。
...
Result: 在新基准上，MOSIV相比适应后的基线方法显著提高了接地准确性和长时程模拟保真度，成为该新任务的强基线方法。
...
### [66] [K-MaT: Knowledge-Anchored Manifold Transport for Cross-Modal Prompt Learning in Medical Imaging](https://arxiv.org/abs/2603.06340)
*Jiajun Zeng,Shadi Albarqouni*

Main category: cs.CV

TL;DR: K-MaT是一个无需低端模态训练图像的提示学习框架，通过知识锚定流形传输将高端医学影像模型的决策结构迁移到低端模态，在跨模态基准测试中取得SOTA效果


<details>
  <summary>Details</summary>
Motivation: 大规模生物医学视觉语言模型在高分辨率影像上训练后，难以迁移到低端成像模态，容易陷入模态特定的捷径学习，导致跨模态性能下降

Method: 提出K-MaT框架：1）分解提示并锚定到临床文本描述；2）使用融合Gromov-Wasserstein最优传输将低端提示流形对齐到视觉基础的高端空间；3）无需低端训练图像

Result: 在四个跨模态基准测试中取得SOTA：平均调和准确率从42.0%提升到44.1%，宏观F1分数达到36.2%；在乳腺成像任务中有效缓解灾难性遗忘，而标准方法准确率降至27.0%
...
### [67] [StruVis: Enhancing Reasoning-based Text-to-Image Generation via Thinking with Structured Vision](https://arxiv.org/abs/2603.06032)
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像生成推理框架存在两个主要问题：纯文本推理缺乏视觉上下文导致遗漏关键空间和视觉元素；文本-图像交错推理虽然增强了视觉基础但计算成本高且受限于生成器的表示能力

Method: 提出StruVis框架，使用基于文本的结构化视觉表示作为中间推理状态，让多模态大语言模型在纯文本推理过程中有效"感知"视觉结构，通过结构化视觉引导的推理解锁T2I生成的推理潜力

Result: 在推理型文本到图像生成基准测试中取得显著性能提升，在T2I-ReasonBench上获得4.61%增益，在WISE上获得4%增益

Conclusion: StruVis作为一个生成器无关的推理框架，能够无缝集成到不同的T2I生成器中，高效提升其在推理型T2I生成中的性能，解决了现有方法的局限性
...
<details>
  <summary>Details</summary>
Motivation: 2D多目标跟踪面临部分遮挡导致的定位成本混淆问题，现有方法在处理遮挡方面存在不足，需要更有效的遮挡感知机制来提升跟踪准确性和稳定性。

Method: 提出OA-SORT框架，包含三个核心组件：遮挡感知模块（OAM）使用高斯图减少背景影响分析遮挡状态；遮挡感知偏移（OAO）和偏置感知动量（BAM）利用OAM描述的遮挡状态来缓解成本混淆并抑制估计不稳定性。

Result: 在DanceTrack、SportsMOT和MOT17数据集上进行了全面评估，在DanceTrack测试集上HOTA达到63.1%，IDF1达到64.2%。将遮挡感知框架集成到其他四个跟踪器中，HOTA和IDF1平均分别提升2.08%和3.05%。

Conclusion: 遮挡处理在多目标跟踪中至关重要，OA-SORT框架通过创新的遮挡感知机制有效解决了遮挡问题，具有即插即用、无需训练的优势，并能显著提升现有跟踪器的性能。
...
### [69] [Prompt Group-Aware Training for Robust Text-Guided Nuclei Segmentation](https://arxiv.org/abs/2603.06384)
*Yonghuang Wu,Zhenyang Liang,Wenwen Zeng,Xuan Xie,Jinhua Yu*

Main category: cs.CV

TL;DR: 提出一种基于提示组一致性训练的框架，解决SAM3等基础模型在医学图像分割中对提示词敏感的问题，显著提升文本引导细胞核分割的鲁棒性和泛化能力


<details>
  <summary>Details</summary>
Motivation: 虽然SAM3等基础模型支持灵活的文本引导医学图像分割，但其预测结果对提示词表述高度敏感，即使语义等价的描述也会产生不一致的分割结果，限制了在临床和病理工作流程中的可靠性

Method: 将提示敏感性重新定义为组间一致性问题，将语义相关的提示组织成共享相同真实掩码的提示组，提出提示组感知训练框架，包含：(1) 基于分割损失作为隐式排序信号的质量引导组正则化；(2) 采用停止梯度策略的logit级一致性约束来对齐组内预测

Result: 在多数据集细胞核基准测试中，在文本提示下获得一致提升，显著降低跨提示质量水平的性能方差；在六个零样本跨数据集任务中，平均Dice系数提升2.16个百分点

Conclusion: 该方法无需修改模型架构且保持推理不变，显著提升了计算病理学中视觉语言分割的鲁棒性和泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 受生物视觉启发的高维超列特征向量虽然能用于图像分割，但实际应用较少，主要原因是处理密集超列的计算复杂度随训练集大小线性增长，限制了其实际应用。

Method: 采用分层子采样处理VGG16基础超列，研究集成学习在稀疏超列上的性能，包括堆叠和投票集成方法，并与简单逻辑回归分类器进行比较。

Result: 在脑肿瘤数据集上，当N≤20的极端小样本情况下，简单逻辑回归分类器表现最佳；在10%分层子采样率下，N=20时最佳平均Dice得分为0.66，相比标准多尺度UNet基线有24.53%的显著提升。

Conclusion: 分层子采样和集成学习策略能有效降低超列计算复杂度，在小样本情况下显著提升图像分割性能，特别是在极端小样本场景下，简单分类器比复杂模型更有效。
...
TL;DR: PSIVG框架通过在视频扩散过程中集成物理模拟器，解决AI生成视频中物体运动违反物理规律的问题，生成更符合真实世界物理规律的视频。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的视频生成方法虽然在视觉真实性上取得了显著进展，但生成的物体运动经常违反重力、惯性和碰撞等基本物理定律，导致物体在帧间运动不一致、动力学不真实或违反物理约束，这限制了AI生成视频的真实性和可靠性。

Method: PSIVG框架包含三个主要步骤：1）从预训练扩散模型生成的模板视频中重建4D场景和前景物体网格；2）在物理模拟器中初始化这些网格并生成物理一致的轨迹；3）利用模拟轨迹指导视频生成器产生时空物理一致的运动。此外，还提出了测试时纹理一致性优化技术，基于模拟器的像素对应关系调整文本和特征嵌入以改善物体运动时的纹理一致性。

Result: 综合实验表明，PSIVG生成的视频在保持视觉质量和多样性的同时，能更好地遵循真实世界的物理规律，解决了现有方法中物体运动违反物理定律的问题。

Conclusion: PSIVG通过将物理模拟器集成到视频扩散过程中，成功提升了生成视频的物理一致性，为AI生成视频的真实性和可靠性提供了重要改进。
...
TL;DR: 该论文提出了一种数据驱动的方法来解决文本到图像模型中排版控制不足的问题，通过构建大规模排版数据集FontUse和改进训练流程，使模型能更好地遵循用户指定的字体样式和使用场景。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在排版控制方面存在不足，用户请求的排版样式经常被忽略或弱化遵循，这限制了模型在需要精确排版控制的应用场景中的实用性。

Method: 采用数据为中心的方法，构建了大规模排版数据集FontUse（约7万张图像），通过专门的标注流程自动生成用户友好的提示词、文本区域位置和OCR识别字符串。使用分割模型和多模态大语言模型自动生成标注，提示词明确结合字体样式（如衬线体、手写体、优雅体）和使用场景（如婚礼请柬、咖啡店菜单）。基于这些标注微调现有生成模型，无需修改架构。

Result: 实验表明，使用该流程训练的模型在多样化的提示词和布局下，产生的文本渲染与请求属性的对齐度优于竞争基线。还引入了基于Long-CLIP的评估指标来衡量生成排版与请求属性的一致性。

Conclusion: 该研究通过数据驱动的标注流程和专门的数据集构建，有效提升了文本到图像模型对排版控制的遵循能力，使模型能够直观地解释样式和使用场景条件，为排版敏感的文本生成应用提供了实用解决方案。
...
TL;DR: CLoPA是一种持续适应策略，通过在标注缓存上微调nnInteractive模型的一小部分参数，快速将医学图像分割性能提升到专家水平，无需改变推理流程或增加新参数。


<details>
  <summary>Details</summary>
Motivation: 现有零样本模型如nnInteractive在不同医学成像任务中无法始终达到专家级性能。由于标注活动会产生持续增长的任务特定标注数据流，在线适应分割模型成为零样本推理的自然补充。

Method: 提出CLoPA持续适应策略，通过轻量级片段调度触发，在标注缓存上微调nnInteractive模型的一小部分参数。该方法不需要新参数或改变推理流程，完全在现有标注工作流中运行。

Result: 在涵盖不同解剖目标和成像特性的八个Medical Segmentation Decathlon任务中，CLoPA快速将性能提升到专家水平，即使对于nnInteractive先前失败的任务也是如此。多数增益在单个训练片段后实现。不同参数组的调优效果取决于任务特性和数据情况。

Conclusion: CLoPA通过持续适应有效提升了医学图像交互分割性能。对于复杂几何结构的目标（如肝血管），实例归一化和低层特征调优会饱和，表明在最具挑战性的场景中需要更深层的特征表示对齐。
...
<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在视觉理解方面表现出色，但生成能力相对较弱，存在理解与生成过程的内在解耦问题。模型能准确理解细粒度视觉细节，却难以从复杂文本提示生成语义一致的图像。

Method: 提出GvU框架：1) 基于token级别的内在文本-图像对齐奖励机制，让模型同时作为教师和学生，使用理解分支评估自身输出并指导生成；2) 设计自监督强化学习框架，让模型通过基于理解的内在奖励信号迭代提升生成质量，无需外部监督。

Result: 实验结果表明，该方法显著提升了统一多模态模型的生成能力，同时增强了其细粒度视觉理解能力，缩小了视觉理解与生成之间的能力差距。

Conclusion: 通过利用模型内部理解能力来指导生成过程的自监督强化学习框架，能够有效缩小统一多模态模型中理解与生成之间的能力差距，实现两者协同提升。
...
TL;DR: GenHOI是一个轻量级增强方法，通过时间平衡和空间选择的方式向预训练视频生成模型注入参考物体信息，显著提升手-物体交互视频合成的物理合理性和物体一致性。


<details>
  <summary>Details</summary>
Motivation: 当前手-物体交互视频合成面临两个主要问题：1）专门的HOI重演方法在复杂野外场景泛化能力差；2）通用视频编辑模型在处理HOI特定问题（如物体外观不一致）时表现不佳。需要一种既能保持通用性又能专门处理HOI问题的方法。

Method: 提出GenHOI框架，包含两个核心组件：1）Head-Sliding RoPE：为参考标记分配头部特定的时间偏移，平衡参考信息在时间维度上的影响，缓解3D RoPE的时间衰减问题；2）两级空间注意力门：集中物体条件注意力于HOI区域，自适应调整注意力强度，在保持背景真实性的同时增强交互保真度。

Result: 在未见过的野外场景上进行广泛定性和定量评估，GenHOI在物理合理性、物体一致性和交互保真度方面显著优于最先进的HOI重演方法和通用视频编辑方法。

Conclusion: GenHOI通过时间平衡和空间选择机制有效解决了手-物体交互视频合成中的关键挑战，为预训练视频生成模型提供了轻量级但高效的增强方案，在复杂野外场景中展现出优越的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶VLA模型存在窄策略限制问题，模仿学习倾向于限制探索，导致后续强化学习阶段因反馈多样性不足而提前饱和，限制了模型性能的进一步提升。

Method: 提出Curious-VLA两阶段框架：1) IL阶段采用可行轨迹扩展策略生成多个物理有效轨迹，并使用逐步归一化轨迹表示来适应多样化数据；2) RL阶段引入自适应多样性感知采样优先选择高多样性样本，并提出跨度驾驶奖励配合焦点风格加权来放大奖励值跨度，提高对驾驶质量的敏感性。

Result: 在Navsim基准测试中，Curious-VLA取得了最先进的结果（PDMS 90.3，EPDMS 85.4），Best-of-N PDMS达到94.8，证明了其在释放VLA模型探索潜力方面的有效性。

Conclusion: Curious-VLA框架通过解决VLA模型中的探索-利用困境，显著提升了自动驾驶模型的性能，为VLA模型的有效训练提供了新的解决方案。
...
TL;DR: AI系统在45,139张超声图像上训练，能诊断胎儿口面裂，敏感性和特异性超过93%和95%，匹配资深放射科医生水平，显著优于初级医生，并能提升初级医生诊断能力6%以上。
...
Result: AI系统诊断胎儿口面裂的敏感性和特异性分别超过93%和95%，与资深放射科医生表现相当，显著优于初级放射科医生。作为医疗副驾驶使用时，能将初级放射科医生的敏感性提高6%以上。试点研究显示，该模型能改善罕见疾病的专业知识发展。
...
### [78] [SUREON: A Benchmark and Vision-Language-Model for Surgical Reasoning](https://arxiv.org/abs/2603.06570)
*Alejandra Perez,Anita Rau,Lee White,Busisiwe Mlambo,Chinedu Nwoye,Muhammad Abdullah Jamal,Omid Mohareri*

Main category: cs.CV

TL;DR: SUREON是一个从手术教学视频中提取专家讲解意图、原理和预测信息的大规模视频问答数据集，包含12个问题类别、206.8k个问答对，并基于此训练了两个能理解手术推理的视觉语言模型。


<details>
  <summary>Details</summary>
Motivation: 当前手术AI缺乏理解手术推理的能力，因为大规模标注包含手术推理的训练数据非常困难。然而，手术教学视频中专家讲解的内容正好包含了意图、原理和预测等推理信息，虽然这些讲解是嘈杂和非结构化的。

Method: 通过多智能体管道从手术学术视频中提取和结构化监督信息，构建SUREON数据集，包含12个问题类别（安全评估、决策原理、预测等）。基于此数据集训练了两个模型：SureonVLM（通过监督微调适应的视觉语言模型）和SureonVLM-R1（使用Group Relative Policy Optimization训练推理模型）。

Result: SUREON数据集包含134.7K个视频片段、170种手术类型、206.8k个问答对和354个专家验证的基准测试。两个模型在SUREON基准测试中准确率超过84%，在标准手术感知任务上优于大型通用领域模型。SureonVLM-R1展现出明确的推理行为，如从视觉上下文中推断手术意图。

Conclusion: 从手术教学视频中提取的监督信号能够有效训练AI模型理解手术推理，SUREON数据集和相关模型为手术AI向理解手术意图和决策的方向发展提供了重要基础。
...
Result: BEVLM使LLMs在跨视角驾驶场景中的推理准确率提升46%，通过语义蒸馏到BEV表示，在安全关键场景中的端到端闭环驾驶性能提升29%。
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦域泛化行人重识别方法依赖全局特征表示和简单平均聚合，存在两个局限：1) 仅用全局特征难以捕捉细粒度域不变局部细节；2) 统一参数平均将所有客户端等同对待，忽略了它们在鲁棒特征提取能力上的差异，稀释了高质量客户端的贡献

Method: 提出FedARKS框架，包含两个机制：RK（鲁棒知识）机制和KS（知识选择）机制。RK机制旨在提取更细粒度的域不变特征，KS机制根据客户端特征提取能力差异进行选择性知识集成

Result: 论文声称FedARKS框架能够提升模型在未见域上的泛化能力，同时保护客户端数据隐私

Conclusion: FedARKS通过鲁棒知识提取和选择性知识集成，有效解决了联邦域泛化行人重识别中的特征表示不足和聚合策略缺陷问题
...
### [81] [Cross-Resolution Distribution Matching for Diffusion Distillation](https://arxiv.org/abs/2603.06136)
...
Method: 使用logSNR曲线划分各分辨率的时间步区间，引入logSNR映射补偿分辨率引起的偏移；沿分辨率轨迹进行分布匹配，减少低分辨率生成分布与教师高分辨率分布的差距；在上采样过程中加入预测噪声重注入机制稳定训练

Result: RMD在保持高保真生成的同时显著加速推理，在SDXL上实现33.4倍加速，在Wan2.1-14B上实现25.6倍加速，同时保持高视觉保真度

Conclusion: RMD通过跨分辨率分布匹配蒸馏有效解决了多分辨率级联推理中的分布差距问题，实现了高质量、高效率的图像生成
...
TL;DR: 本文研究了视觉语言模型在结构化色彩失真下的系统性感知弱点，发现即使场景对人类仍然可识别，VLM也会产生自信但荒谬的预测，而人类在这些失真下表现远优于VLM。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在基准测试中表现良好，但它们存在系统性感知弱点：像素值的结构化大幅变化会导致模型产生自信但荒谬的预测，即使底层场景对人类来说仍然容易识别。本文旨在研究这种人类与VLM之间的感知差距。

Method: 使用空间色彩混合方法，这是一种程序化的色彩失真家族，在自然图像上叠加结构化模式（包括RGB和Ostwald色彩系统）。引入了八种空间色彩混合变体框架，评估了来自三个模型家族的九个VLM在四个数据集上的表现。同时进行了包含61名参与者的人类研究，比较人类和VLM在相同失真条件下的表现。

Result: 随着失真程度的增加，所有模型和数据集上的准确率急剧下降；扩大语言模型规模并不能可靠地缓解这种失败。在动物识别数据集上，人类在相同失真条件下显著优于VLM。简单的受人类启发的预处理步骤可以恢复几种失真类型的部分性能。

Conclusion: 视觉语言模型存在系统性的感知弱点，人类在结构化色彩失真下的表现远优于VLM。受人类启发的预处理和工具使用可以作为提高VLM鲁棒性的实用策略。
...
TL;DR: 该研究提出了一种虚拟治疗框架，用于预测非小细胞肺癌在放疗期间的肿瘤演变，通过剂量感知的多模态条件图像转换生成随访CT图像。


<details>
  <summary>Details</summary>
Motivation: 预测放疗期间肿瘤演变是临床关键挑战，特别是当纵向变化同时受解剖结构和治疗影响时。现有方法难以准确模拟放疗引起的解剖变化。

Method: 提出虚拟治疗框架，将非小细胞肺癌进展建模为剂量感知的多模态条件图像转换问题。基于CT扫描、基线临床变量和指定辐射剂量增量，生成反映治疗引起解剖变化的随访CT图像。使用222名III期非小细胞肺癌患者的895个CT扫描数据集，评估GAN和扩散模型在2D和2.5D配置下的表现。

Result: 定量和定性结果表明，基于扩散的模型从多模态、剂量感知的条件中获益更一致，比基于GAN的基线模型产生更稳定、解剖学上更合理的肿瘤演变轨迹。

Conclusion: 虚拟治疗框架作为非小细胞肺癌治疗监测和自适应放疗研究的工具具有潜力，扩散模型在该任务中表现优于GAN模型。
...
TL;DR: FreeOcc是一个无需训练的3D场景理解框架，利用预训练基础模型从多视角图像恢复语义和几何信息，实现道路场景的语义和全景占据预测。


<details>
  <summary>Details</summary>
Motivation: 当前基于相机的3D占据预测方法通常需要昂贵的密集3D监督或需要在目标域数据上训练模型，这限制了在未见环境中的部署。需要一种无需训练的方法来实现跨域3D场景理解。

Method: FreeOcc采用无需训练的流程：1) 使用可提示的基础分割模型提取每视角的全景先验；2) 使用重建基础模型恢复度量3D点；3) 通过深度和置信度感知过滤将可靠标签提升到3D；4) 随时间融合并通过确定性细化堆栈进行体素化；5) 通过拟合和合并当前视角的3D框候选来恢复实例，实现无需学习3D模型的实例感知占据。

Result: 在Occ3D-nuScenes数据集上，FreeOcc无需训练达到16.9 mIoU和16.5 RayIoU，与最先进的弱监督方法相当。作为伪标签生成流程训练下游模型时，达到21.1 RayIoU，超越之前的弱监督基线。在无需训练和弱监督的全景占据预测中分别达到3.1 RayPQ和3.9 RayPQ，建立了新的基准。

Conclusion: FreeOcc证明了基于基础模型的感知是实现无需训练3D场景理解的实用途径，为跨域部署提供了有效的解决方案。
...
TL;DR: 提出一种用于乳腺超声图像分割的半监督学习框架，通过免训练的伪标签生成和标签精炼，在仅有2.5%标注数据下达到全监督模型的性能水平。


<details>
  <summary>Details</summary>
Motivation: 传统半监督学习在乳腺超声图像分割中面临伪标签不稳定的问题，特别是在标注极其有限的情况下。现有的视觉语言模型在医学图像上的效果有限，因为领域特定的提示难以迁移。

Method: 1) 利用基于外观的简单描述（如"暗色椭圆形"）实现跨域结构迁移，让视觉语言模型生成结构一致的伪标签；2) 用这些伪标签预热静态教师模型以捕获乳腺病灶的全局结构先验；3) 结合指数移动平均教师，引入不确定性熵加权融合和自适应不确定性引导的反向对比学习来改善边界判别能力。
...
TL;DR: JOPP-3D是一个开放词汇语义分割框架，通过联合利用全景图像和点云数据，实现语言驱动的场景理解，在2D和3D开放/封闭词汇语义分割方面显著优于现有技术。
...
Result: 在Stanford-2D-3D-s和ToF-360数据集上的实验表明，JOPP-3D能够在全景和3D域产生连贯且有语义意义的分割结果，在开放和封闭词汇2D和3D语义分割方面相比SOTA有显著提升。
...
TL;DR: 本文提出了一种使用强化学习增强3D医学图像扩散模型的方法，通过多尺度反馈优化生成质量，在脑部MRI数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在3D医学图像生成中表现出强大能力，但标准训练目标与临床相关性之间存在差距，需要更有效的优化方法来提高生成质量。

Method: 首先在MRI体积数据上预训练3D扩散模型建立生成先验，然后使用近端策略优化（PPO）进行微调，通过结合2D切片评估和3D体积分析的新颖奖励系统指导优化。

Result: 在BraTS 2019和OASIS-1数据集上的验证表明，强化学习反馈有效引导生成过程向更高质量分布发展，Fréchet Inception Distance显著改善，合成数据在下游肿瘤和疾病分类任务中表现出更好的实用性。

Conclusion: 通过强化学习与多尺度反馈的结合，能够有效提升3D医学图像扩散模型的生成质量，增强合成数据在临床相关下游任务中的实用性。
...
TL;DR: 本文提出两种技术（自动聚合和逐像素重缩放）来解决训练免费扩散分割器中的注意力图不一致和得分不平衡问题，从而更好地利用生成模型的语义能力。


<details>
  <summary>Details</summary>
Motivation: 现有训练免费扩散分割方法虽然利用预训练扩散模型的交叉注意力图进行语义分割，但未能随模型生成能力增强而提升性能。研究发现存在两个问题：多头多层的注意力图与全局表示存在差异；全局图无法直接转化为准确的语义相关性，存在文本标记间的得分不平衡。

Method: 提出两种技术：1) 自动聚合：解决多头多层注意力图与全局表示之间的差异；2) 逐像素重缩放：解决不同文本标记间的得分不平衡问题，使训练免费分割能更好地利用生成能力。

Result: 在标准语义分割基准测试中评估了该方法，并进一步将其集成到生成技术中，展示了性能提升和广泛适用性。

Conclusion: 通过自动聚合和逐像素重缩放技术，训练免费扩散分割方法能够更好地利用生成模型的语义能力，解决了现有方法在注意力图一致性和得分平衡方面的局限性。
...
TL;DR: 提出了一种基于骨骼的动作分割点监督框架，仅需每个动作片段标注一帧，通过多模态骨骼数据和伪标签集成方法，在减少标注成本的同时达到与全监督方法竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 全监督的骨骼动作分割方法需要昂贵的逐帧标注，且对模糊的动作边界敏感。为了降低标注成本并提高模型鲁棒性，需要开发只需少量标注的点监督方法。

Method: 使用多模态骨骼数据（关节、骨骼、运动信息）通过预训练统一模型提取特征；提出原型相似性方法，并结合能量函数和约束K-Medoids聚类生成伪标签；采用多模态伪标签集成来指导模型训练。

Result: 在PKU-MMD（X-Sub和X-View）、MCFS-22和MCFS-130数据集上建立了新的基准，实验表明该方法在显著减少标注工作量的情况下实现了有竞争力的性能，甚至超越了一些全监督方法。

Conclusion: 提出的点监督框架为骨骼动作分割提供了一种高效的解决方案，通过多模态特征和伪标签集成方法，在降低标注成本的同时保持了高性能，为实际应用提供了可行方案。
...
TL;DR: VG3S框架通过将视觉基础模型的几何先验注入3D高斯泼溅，显著提升了自动驾驶场景的语义占用预测性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯泼溅的占用预测方法严重依赖准确的几何线索，但在纯视觉中心范式中这些线索往往不足。需要将视觉基础模型强大的几何理解能力注入到占用预测中。

Method: 提出VG3S框架，包含一个即插即用的分层几何特征适配器，通过特征聚合、任务特定对齐和多尺度重构，有效转换预训练视觉基础模型的通用特征。

Result: 在nuScenes占用基准测试中，VG3S相比基线在IoU上提升了12.6%，在mIoU上提升了7.5%。框架能无缝适配不同视觉基础模型，一致提升预测精度。

Conclusion: VG3S证明了将预训练几何基础视觉模型的先验知识整合到高斯泼溅占用预测中的巨大价值，为自动驾驶场景理解提供了有效解决方案。
...
TL;DR: 提出了一种基于特征熵优化的邻域密度化策略EntON，用于3D高斯溅射(3DGS)中实现几何精确且高质量的3D重建。该方法通过特征熵指导自适应分裂和剪枝，在保持渲染质量的同时显著提升几何精度和效率。


<details>
  <summary>Details</summary>
Motivation: 标准3DGS产生的高斯分布中心与表面与底层物体几何对齐不佳，而专注于表面的重建方法经常牺牲光度精度。需要一种既能保持渲染质量又能提高几何精度的方法。

Method: 提出特征熵优化的邻域密度化策略EntON：1) 从每个高斯中心k最近邻的协方差矩阵特征值计算3D形状特征熵，量化局部结构有序度；2) 采用交替优化框架：在标准梯度密度化（基于视图空间梯度）和特征熵感知密度化之间交替，优先在低特征熵（有序、平坦）邻域密度化以捕捉表面细节，在高特征熵（无序、球形）区域剪枝。

Result: 在DTU（小规模人造物体）和TUM2TWIN（大规模城市场景）数据集上的实验表明：几何精度提升达33%，渲染质量提升达7%，高斯数量减少达50%，训练时间减少达23%。

Conclusion: EntON通过特征熵感知的交替密度化策略，在避免不必要场景扩展的同时，实现了几何精度、渲染质量和效率之间的良好平衡。
...
TL;DR: WAFL将时序伪造定位任务从传统的时序回归和连续定位转变为离散的词级二分类，通过词锚定、特征重对齐和非对称损失函数，显著提升了定位性能并降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前时序伪造定位方法存在特征粒度不对齐和计算成本高的问题。传统方法依赖时序边界回归或连续帧级异常检测，但存在特征粒度不匹配和计算效率低下的局限性。

Method: 1. 提出词锚定时序伪造定位（WAFL）范式，将任务转化为离散的词级二分类；2. 识别最小有意义的伪造单元——词标记，并按语音自然语言边界对齐数据预处理；3. 引入法证特征重对齐（FFR）模块，将预训练语义空间特征映射到判别性法证流形；4. 设计伪影中心非对称（ACA）损失函数，动态抑制真实梯度并优先处理细微法证伪影。

Result: WAFL在数据集内和跨数据集设置下均显著优于现有最先进方法，同时需要更少的可学习参数，具有更高的计算效率。

Conclusion: WAFL通过将时序伪造定位重新定义为词级二分类任务，解决了特征粒度不对齐和计算成本高的问题，为时序伪造检测提供了更高效、更准确的解决方案。
...
TL;DR: HCF-RES是一个用于3D指代表达式分割的多模态框架，通过分层视觉语义分解和渐进多级融合，在稀疏点云基础上引入丰富的视觉语义信息，显著提升了3D场景中基于自然语言的对象定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D指代表达式分割方法仅依赖稀疏点云，缺乏丰富的视觉语义信息，难以处理细粒度描述。当描述匹配多个目标或无目标时，定位效果受限。

Method: 提出HCF-RES框架：1）分层视觉语义分解：利用SAM实例掩码指导CLIP编码，获取像素级和实例级双重粒度特征，在2D到3D投影中保持对象边界；2）渐进多级融合：通过模态内协作、2D语义与3D几何特征的跨模态自适应加权，以及语言引导的细化来整合表示。

Result: 在ScanRefer和Multi3DRefer两个基准数据集上取得了最先进的结果。

Conclusion: HCF-RES通过引入多模态视觉语义信息和渐进融合机制，有效解决了现有3D指代表达式分割方法在细粒度描述和复杂场景下的局限性，显著提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器在真实图像超分辨率中表现出潜力，但其计算昂贵的迭代采样需要单步蒸馏。现有单步蒸馏方法存在轨迹不匹配和严重网格状周期性伪影问题
...
Conclusion: StrSR通过谱和轨迹正则化有效解决了扩散变换器单步蒸馏中的关键挑战，为真实图像超分辨率提供了高效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 生成式零样本学习虽然能合成未见类特征，但面临两个内在挑战：1）类级属性无法捕捉实例特定的视觉外观（类-实例差距）；2）语义与视觉特征分布不匹配（语义-视觉领域差距）。

Method: 提出ADiVA方法，包含两个模块：1）属性分布建模模块学习可迁移的属性分布并为未见类采样实例级属性；2）视觉引导对齐模块精炼语义表示以更好反映视觉结构。
...
<details>
  <summary>Details</summary>
Motivation: 现有无训练方法依赖T2V模型中间输出构建运动引导，计算开销大且灵活性有限。本文发现早期潜在预测编码了丰富的时序信息，可直接利用流基T2V模型的预测输出来实现高效运动迁移

Method: 提出FlowMotion框架：1）流引导：基于潜在预测提取运动表示，对齐源视频与生成视频的运动模式；2）速度正则化：稳定优化并确保平滑运动演化；3）完全基于模型预测操作，无需额外训练
...
<details>
  <summary>Details</summary>
Motivation: CBCT在牙科成像中广泛应用，但高密度物体（如牙科种植体）容易产生伪影，影响图像质量和诊断准确性。现有基于扩散模型的种植体修复方法仅处理独立的2D投影，忽略了投影间的相关性，导致重建图像不一致。

Method: 提出3D牙科种植体修复方法，基于垂直分数扩散模型，每个模型在不同平面上训练并在投影域操作。通过结合两个2D分数扩散模型在采样方案中来建模投影序列的3D分布。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本异常检测方法通常依赖固定文本提示，难以捕捉复杂语义，且仅关注空间域特征，限制了检测细微异常的能力。需要解决提示适应性不足和多频特征利用不充分的问题。

Method: 提出小波增强的专家混合提示学习方法：1) 使用变分自编码器建模全局语义表示并集成到提示中；2) 通过小波分解提取多频图像特征，通过跨模态交互动态优化文本嵌入；3) 引入语义感知的专家混合模块聚合上下文信息。

Result: 在14个工业和医学数据集上进行广泛实验，证明了所提方法的有效性，能够更好地检测未见异常。

Conclusion: 该方法通过结合全局语义建模、多频特征提取和专家混合机制，显著提升了零样本异常检测的性能，特别是在捕捉复杂语义和细微异常方面表现出优势。
...
Method: 提出P-SLCR方法：1）一致性结构学习：通过选择高质量特征，在一致性点和一致性原型库之间建立结构特征学习；2）语义关系一致性推理：分别构建一致性和模糊性原型库之间的原型关系矩阵，通过该矩阵对两种原型库施加约束以保持语义一致性。

Result: 在S3DIS、SemanticKITTI和Scannet数据集上进行了广泛评估，相比无监督方法取得了最佳性能。在S3DIS数据集Area-5上达到47.1%的mIoU，比经典全监督方法PointNet高出2.5%。

Conclusion: 提出的P-SLCR方法通过原型库驱动的无监督学习策略，有效解决了点云语义分割中缺乏标注数据的问题，在多个数据集上超越了现有无监督方法，甚至在某些情况下优于全监督方法。
...
<details>
  <summary>Details</summary>
Motivation: 扩散世界模型在统一世界模拟方面表现出强大潜力，但迭代去噪过程计算成本过高，难以用于交互式使用和长时程推演。现有特征缓存方法虽然能加速推理，但由于令牌异构性和非均匀时间动态性这两个世界模型特有的障碍，导致策略迁移效果不佳。
...
Conclusion: WorldCache是针对扩散世界模型的有效缓存框架，通过专门处理令牌异构性和非均匀时间动态性，显著加速推理过程而不牺牲推演质量，为资源受限场景下的世界模型应用提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D网格生成方法通常依赖等值面提取或启发式网格化，这些方法在处理复杂几何形状和拓扑结构时存在效率和质量限制。需要一种能够直接生成高质量拓扑结构的可扩展方法。

Method: LATO将网格表示为基于表面的顶点位移场，使用稀疏体素变分自编码器压缩为结构化、拓扑感知的体素潜在表示。通过两阶段流匹配过程：首先生成结构体素，然后细化体素级拓扑特征。解码时逐步细分和修剪潜在体素来实例化精确顶点位置，专用连接头预测顶点对之间的边连接性。

Result: 与基于等值面/三角形的扩散模型和自回归生成方法相比，LATO能够生成具有复杂几何形状、良好拓扑结构的网格，同时在推理时具有高效率。

Conclusion: LATO提供了一种新颖的拓扑保持潜在表示，实现了基于流匹配的显式3D网格可扩展合成，在保持高质量拓扑结构的同时显著提高了生成效率。
...
Method: 1. 构建DentalProbe数据集（5000张图像，专家标注诊断轨迹）；2. 开发Reinspection驱动的强化学习框架，包含基于评分标准的奖励和条件诊断驱动奖励；3. 提出OralGPT-Plus智能视觉语言模型，支持迭代对称感知诊断推理

Result: OralGPT-Plus在MMOral-X基准测试和现有全景基准上相比强基线模型表现出持续可靠的改进，验证了交互式和对称感知推理的有效性

Conclusion: 智能建模在牙科影像分析中具有重要价值，为未来临床对齐的全景X光片分析研究奠定了基础，OralGPT-Plus通过迭代对称感知推理提升了诊断可靠性
...
<details>
  <summary>Details</summary>
Motivation: 密集像素级标注成本高昂，稀疏标注作为弱监督替代方案存在性能差距，需要利用3D场景重建作为辅助监督信号来提升性能

Method: 采用双学生-教师架构，利用前馈3D重建技术从2D视频恢复3D几何结构，在2D图像和重建的3D点云之间强制语义一致性，实现稀疏标注在场景中的传播

Result: 在稀疏监督任务中达到最先进性能，比现有方法提升2-7%，无需额外标注或推理开销
...
TL;DR: CHMv2是一个全球米级分辨率树冠高度图，通过基于DINOv3的深度估计模型从高分辨率光学卫星图像生成，相比现有产品显著提高了精度并减少了高森林的偏差。
...
Method: 使用基于DINOv3构建的深度估计模型，从高分辨率光学卫星图像中提取树冠高度信息。通过大规模地理多样性训练数据扩展、自动化数据整理和配准，以及针对树冠高度分布定制的损失函数和数据采样策略来实现。

Result: CHMv2相比现有产品显著提高了精度，减少了高森林的偏差，并更好地保留了细尺度结构（如树冠边缘和林窗）。通过独立ALS测试集以及数千万个GEDI和ICESat-2观测数据的验证，证明其在主要森林生物群落中表现一致。

Conclusion: CHMv2提供了一个全球米级分辨率的树冠高度图，通过改进的深度学习方法显著提升了树冠高度估计的准确性和空间分辨率，为全球森林监测和研究提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前场景图生成方法要么专注于提升关系预测准确率，要么专注于提高物体检测准确率，要么专注于降低延迟，缺乏同时平衡这三个目标的解决方案。为了支持实时应用，需要在性能与推理速度之间找到更好的权衡。

Method: 基于REACT架构，通过高效特征提取和原型空间中的主体到客体的交叉注意力机制，平衡延迟和表示能力，构建REACT++模型。

Result: REACT++在现有SGG模型中达到最高推理速度，在保持物体检测性能的同时提升了关系预测准确率。相比前代REACT版本，速度提升20%，关系预测准确率平均提升10%。

Conclusion: REACT++实现了场景图生成中性能与速度的良好平衡，为实时应用提供了有效的解决方案，代码已开源供社区使用。
...
TL;DR: 提出人机协同的拼图框架，用于解决真实考古文物碎片重组问题，结合自动求解器和人工交互指导，显著优于纯自动和纯人工方法。


<details>
  <summary>Details</summary>
Motivation: 真实考古文物碎片重组面临侵蚀、缺失区域、不规则形状和大规模模糊性等挑战，传统拼图求解器在碎片数量达数千时效果不佳，需要新方法处理文化遗产重建的复杂性和规模。

Method: 提出人机协同框架，集成自动松弛标记求解器与交互式人工指导，用户可迭代锁定已验证的放置、纠正错误，并引导系统实现语义和几何一致的组装。引入两种互补的交互策略：迭代锚定和连续交互细化。

Result: 在多个RePAIR数据集上的实验表明，该混合方法在准确性和效率上显著优于完全自动和纯人工基线，为大规模专家参与文物重组提供了实用解决方案。

Conclusion: 人机协同框架能有效处理真实考古文物碎片重组的复杂性和规模问题，结合自动求解器的效率和人工专家的判断，为文化遗产重建提供了可行的解决方案。
...
TL;DR: 研究通过合成飞镖形状的感知冲突实验，发现BEiT视觉Transformer倾向于凸性补全，并识别出特定注意力头L0H9作为早期种子驱动这一偏好


<details>
  <summary>Details</summary>
Motivation: 研究视觉Transformer在图形-背景组织中的感知模糊性，特别是当局部几何证据与全局组织先验（如格式塔凸性先验）冲突时，探索BEiT模型内部如何实现凸性偏好

Method: 使用合成飞镖形状创建感知冲突，系统性地掩蔽区域使其同时支持凹性和凸性补全；通过logit归因将内部激活投影到离散视觉词表空间，识别关键功能单元；分解注意力头的直接影响

Result: BEiT在感知冲突中可靠地偏好凸性补全；图形-背景组织在早期和中间层保持模糊，在后期层突然解决；识别出注意力头L0H9作为早期种子引入凸性偏好；下调该注意力头可改变感知决策边界

Conclusion: BEiT的凸性偏好由特定功能单元控制，特别是注意力头L0H9作为早期种子，表明视觉Transformer的感知决策可通过干预特定组件进行调控
...
TL;DR: WanderDream是首个用于心理探索模拟的大规模数据集，包含全景视频和问答对，支持在无法主动探索的情况下进行空间推理
...
Result: 实验表明：(1)心理探索对情境推理至关重要；(2)世界模型在WanderDream-Gen上表现良好；(3)想象力显著促进WanderDream-QA的推理；(4)数据在真实场景中具有良好迁移性

Conclusion: WanderDream为无需主动探索的心理模拟推理提供了首个大规模基准，展示了心理探索在空间推理中的重要性，数据具有实际应用价值
...
TL;DR: CaTok是一种1D因果图像分词器，通过MeanFlow解码器解决视觉自回归模型中的因果对齐问题，支持快速一步生成和高保真多步采样，在ImageNet重建上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉分词器要么将2D块展平为非因果序列，要么采用与"下一标记预测"模式不对齐的启发式排序方法。最近的扩散自编码器也存在问题：解码器基于所有标记缺乏因果性，而应用嵌套dropout机制则引入不平衡。

Method: 提出CaTok，一种1D因果图像分词器，采用MeanFlow解码器。通过在时间间隔内选择标记并将其绑定到MeanFlow目标，学习因果1D表示。还提出REPA-A正则化方法，将编码器特征与视觉基础模型对齐以稳定和加速训练。

Result: 在ImageNet重建上达到最先进结果：0.75 FID、22.53 PSNR和0.674 SSIM，且训练轮次更少。自回归模型性能与领先方法相当。

Conclusion: CaTok成功解决了视觉自回归模型中的因果对齐问题，学习到的因果1D表示支持快速一步生成和高保真多步采样，同时自然地捕捉了跨标记间隔的多样化视觉概念。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型虽然灵活但难以通过提示或简单推理适应来控制，不适合有严格产品要求的用例，需要更可控的系统来支持Pinterest的图像编辑和增强需求。

Method: 首先在多样化多模态数据集上训练基础扩散模型，然后针对特定下游任务在任务特定数据集上快速微调基础模型变体，产生专门化模型来处理不同用例。

Result: 在线A/B实验显示增强图像分别获得18.0%和12.5%的显著参与度提升，与人类评分者比较进一步验证了模型在背景增强和宽高比外绘任务上优于第三方模型。

Conclusion: Canvas方法通过基础模型训练和任务特定微调，能够有效支持各种图像编辑和增强用例，并可推广到多图像场景合成和图像到视频生成等广泛下游任务。
...
TL;DR: 该研究系统分析了去噪生成模型训练目标中的损失权重和输出参数化选择，探讨了这些选择与数据流形内在维度、模型架构和数据集大小的相互作用，旨在为流匹配模型提供实用的设计指导。


<details>
  <summary>Details</summary>
Motivation: 当前去噪生成模型存在多种训练目标选择（损失权重、输出参数化等），但缺乏对这些选择如何与数据特性、模型架构等因素相互作用的系统性理解。研究旨在理清这些因素，为实际训练提供指导而非提出新方法。

Method: 通过系统数值研究，分析不同训练目标（噪声、干净图像、速度等参数化）与数据流形内在维度、模型架构、数据集大小的相互作用。实验涵盖几何可控的合成数据集和图像数据，使用PSNR（去噪精度）和FID（生成质量）进行量化评估。

Result: 研究提供了关于训练目标选择如何影响模型性能的实证分析，揭示了不同参数化方案在不同数据特性下的表现差异，为流匹配模型的设计选择提供了实用见解。

Conclusion: 该研究系统性地揭示了去噪生成模型训练目标中各种因素的重要性，为实际应用中的设计选择提供了实证指导，强调了根据数据特性和任务需求选择合适训练目标的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有放射学基础模型依赖暴力扩展方法，直接迁移自然图像处理方法，导致模型脆弱且计算昂贵，临床实践中难以应用

Method: 提出GreenRFM框架，采用MUST（更多蒸馏、无处不在、语义增强、任务对齐）监督设计原则，最大化利用监督信号而非简单堆叠数据量，提供高性能和轻量级两种配置

Result: 在超过20万张图像（4个机构、2种模态）的实验中，GreenRFM在胸部和腹部CT数据集上表现优异，超越多个基线模型；在肌肉骨骼MRI图像上同样有效，证明监督原则可跨模态迁移

Conclusion: GreenRFM挑战了"规模即一切"的教条，以极低计算成本实现SOTA性能，使临床医生即使在笔记本电脑上也能公平开发先进的放射学基础模型
...
TL;DR: Match4Annotate：基于SIREN隐式神经表示和DINOv3特征的轻量级视频标注传播框架，支持点标注和掩码标注的帧内与帧间传播，在医学超声数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 专业领域（如医学影像）的视频标注成本高昂且耗时，现有标注传播方法存在局限性：视频跟踪和分割模型需要每视频初始化且无法跨视频泛化；传统对应性方法在低纹理场景表现不佳；密集特征匹配和单次分割方法缺乏时空平滑性且不支持点标注和掩码标注的统一处理。

Method: 提出Match4Annotate框架：1）在测试时使用SIREN隐式神经表示拟合DINOv3特征，生成连续高分辨率时空特征场；2）学习帧对之间的平滑隐式变形场来指导对应性匹配；3）支持点标注和掩码标注的帧内与帧间传播。

Result: 在三个具有挑战性的临床超声数据集上评估：1）在帧间传播方面达到最先进水平，优于特征匹配和单次分割基线方法；2）在帧内传播方面与专用跟踪器保持竞争力；3）展示了轻量级、测试时优化的特征匹配流程具有高效可扩展标注工作流的潜力。

Conclusion: Match4Annotate通过隐式神经表示和变形场学习，为视频标注传播提供了统一且高效的解决方案，特别适用于需要专家标注的专业领域，能够显著降低标注成本并提高可扩展性。
...
Result: 方法跨模态通用，支持多模态训练，遵循预期的缩放定律，在图像、视频和音频生成方面取得优异性能

Conclusion: Self-Flow通过将表征学习集成到生成框架中，解决了对外部模型的依赖问题，实现了语义表征学习和生成能力的协同提升
...
TL;DR: 提出一种针对漫反射直接飞行时间LiDAR的空间校准方法，通过扫描反光贴片恢复每个像素在RGB图像平面中的响应映射，实现跨模态对齐


<details>
  <summary>Details</summary>
Motivation: 漫反射直接飞行时间LiDAR通过宽视场聚集光子返回形成深度直方图，违反了标准LiDAR-RGB校准的单射线假设，需要新的校准方法

Method: 使用扫描的反光贴片结合背景减法，恢复每个LiDAR像素在共置RGB图像平面中的足迹（有效支持区域）和相对空间灵敏度，获得LiDAR到RGB的显式对应关系

Result: 在ams OSRAM TMF8828上演示了该方法，实现了跨模态对齐和融合所需的LiDAR-RGB对应关系

Conclusion: 该方法为漫反射LiDAR提供了简单的空间校准程序，解决了传统单射线假设不适用的问题，支持更准确的跨模态对齐和融合
...
TL;DR: 该研究提出了一个在扩散生成模型中处理语言否定的统一框架，通过将否定建模为语义引导的结构化可行性约束，无需重新训练模型参数。
...
Method: 将语言否定重新解释为扩散动态中语义引导的结构化可行性约束。将无分类器引导定义为语义更新方向，通过将更新投影到从语言结构导出的凸约束集上来强制执行否定。该方法无需训练，与预训练扩散主干兼容，可从图像生成扩展到时间演化的视频轨迹。

Result: 该方法在保持视觉保真度和结构连贯性的同时实现了稳健的否定遵从性。建立了扩散生成模型中语言否定的第一个统一表述，超越了表示级评估。还引入了结构化否定中心基准套件，用于隔离生成系统中的不同语言故障模式。

Conclusion: 该工作为扩散生成模型中的语言否定提供了形式化处理，通过结构化约束投影方法实现了训练无关的统一框架，能够处理多样化的否定现象，并为该领域进一步研究提供了基准工具。
...
TL;DR: 研究自动驾驶数据集中的冗余问题，发现选择性移除多源图像中的冗余标注能提升目标检测性能，多模态图像-LiDAR数据间也存在显著冗余


<details>
  <summary>Details</summary>
Motivation: 自动驾驶研究过度关注算法设计而忽视数据质量分析，特别是冗余这一基本但未被充分探索的数据质量问题。多源多模态数据中由于环境条件和传感器限制导致数据质量参差不齐，冗余问题直接影响自动驾驶性能
...
Conclusion: 冗余是可测量且可操作的数据质量因素，直接影响自动驾驶性能。研究强调了冗余作为数据质量因素在自动驾驶感知中的作用，推动了以数据为中心的视角来评估和改进自动驾驶数据集
...
### [118] [Penguin-VL: Exploring the Efficiency Limits of VLM with LLM-based Vision Encoders](https://arxiv.org/abs/2603.06569)
*Boqiang Zhang,Lei Ke,Ruihan Yang,Qi Gao,Tianyuan Qu,Rossell Chen,Dong Yu,Leoweiliang*

Main category: cs.CV

TL;DR: Penguin-VL提出了一种新的视觉语言模型架构，使用文本LLM初始化视觉编码器，替代传统的对比预训练方法，在保持轻量级的同时实现了与大型VLMs相当的性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要依赖模型规模扩展，这阻碍了在计算受限设备上的部署。同时，主流VLMs依赖对比预训练（如CLIP/SigLIP）初始化的视觉编码器存在目标不匹配问题：对比学习优化的区分性任务抑制了密集描述和复杂推理所需的细粒度视觉线索。

Method: 提出Penguin-VL，其视觉编码器从纯文本LLM初始化，而不是传统的对比预训练方法。这种方法能更好地保留细粒度的空间和时间线索，提高视觉保真度和数据效率。

Result: 在各种图像和视频基准测试中，Penguin-VL在数学推理方面与领先的VLMs（如Qwen3-VL）性能相当，在文档理解、视觉知识和多视角视频理解等任务中表现更优。这些成果是通过轻量级架构实现的，表明改进视觉表示而非模型缩放是性能提升的主要驱动力。

Conclusion: Penguin-Encoder作为传统对比预训练编码器的优越替代方案，保留了密集感知和复杂推理所需的关键细粒度线索，使其成为计算高效VLMs的强大即插即用替代方案，能够在资源受限环境中实现高性能。
...
TL;DR: Omni-Diffusion是首个完全基于掩码离散扩散模型的任意模态间多模态语言模型，统一了文本、语音和图像的理解与生成能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要采用自回归架构，在架构设计上存在改进空间。离散扩散模型在其他领域已展现潜力，但尚未作为多模态系统的主干架构被充分探索。

Method: 采用统一的掩码离散扩散模型直接捕捉离散多模态token的联合分布，支持双模态任务及更复杂的多模态场景。

Result: 在多样化基准测试中，该方法优于或与现有处理两个及以上模态的多模态系统表现相当。

Conclusion: 扩散模型在驱动下一代多模态基础模型方面展现出显著潜力，为多模态架构设计提供了有效且高效的替代方案。
...
TL;DR: 该研究发现多模态大语言模型（MLLM）的分类性能评估存在严重问题，主要源于评估协议缺陷和标注噪声。通过修正这些问题，MLLM与监督模型的性能差距显著缩小，表明先前报道的MLLM性能不足主要是评估方法问题而非模型缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前关于多模态大语言模型分类性能的研究存在矛盾结论，作者发现这些矛盾源于评估协议的问题（如输出映射不当、弱干扰项等）和标注质量不佳，需要系统性地识别和修正这些问题以准确评估MLLM的真实能力。

Method: 1. 识别并修正常见评估协议中的关键问题：模型输出超出类别列表被丢弃、弱多选干扰项导致结果虚高、开放世界设置因输出映射不佳而表现差；2. 量化常被忽视的设计选择影响：批量大小、图像顺序、文本编码器选择；3. 创建ReGT数据集（ImageNet-1k的625类多标签重标注）；4. 进行控制案例研究，评估MLLM辅助人工标注的效果。

Result: 1. 修正标注标签后MLLM性能提升高达+10.8%，显著缩小与监督模型的感知差距；2. 常被忽视的设计选择（批量大小、图像顺序、文本编码器）对准确率有实质性影响；3. MLLM对标注质量最敏感，特别是那些较少依赖监督训练信号的模型；4. 在困难案例中，标注者确认或整合MLLM预测的比例约为50%，显示其在数据集构建中的潜力。

Conclusion: MLLM在分类任务中的表现不足主要是评估协议缺陷和标注噪声造成的人工假象，而非真正的模型缺陷。通过改进评估方法和标注质量，MLLM的实际能力被低估。此外，MLLM在辅助大规模数据集构建方面具有实用价值。
...
### [121] [Verify as You Go: An LLM-Powered Browser Extension for Fake News Detection](https://arxiv.org/abs/2603.05519)
*Dorsaf Sallami,Esma Aïmeur*

Main category: cs.CL

TL;DR: Aletheia是一款基于RAG和LLM的浏览器扩展，通过证据解释、讨论社区和事实核查推送功能，在检测假新闻方面优于现有方法，用户研究证实其可用性和有效性。


<details>
  <summary>Details</summary>
Motivation: 数字时代假新闻泛滥严重威胁公众信任和民主制度，现有浏览器扩展存在模型行为不透明、解释支持有限、用户参与度不足等问题，需要更有效、透明和以用户为中心的检测工具。

Method: 提出Aletheia浏览器扩展，采用检索增强生成(RAG)和大语言模型(LLM)检测假新闻并提供基于证据的解释；包含两个交互组件：讨论中心(用户对话平台)和"保持信息"功能(推送最新事实核查)。

Result: 实验表明Aletheia在检测性能上优于最先进的基线方法；对250名参与者的用户研究证实了系统的可用性和感知有效性。

Conclusion: Aletheia作为一种透明工具在打击在线假新闻方面具有潜力，通过结合先进检测技术和用户参与功能，为解决假新闻问题提供了有效方案。
...
### [122] [Attention Meets Reachability: Structural Equivalence and Efficiency in Grammar-Constrained LLM Decoding](https://arxiv.org/abs/2603.05540)
*Faruk Alpay,Bilge Senturk*

Main category: cs.CL

TL;DR: 本文研究了语法约束解码(GCD)作为自回归下一个token分布与基于上下文无关文法(CFG)编译的下推系统可达性预言之间的耦合，证明了预言不变性定理，引入了结构歧义成本(SAC)度量，建立了引擎无关的下界，并推导了硬掩码解码的KL散度和总变差失真界限。


<details>
  <summary>Details</summary>
Motivation: 研究语法约束解码(GCD)的性能和效率问题，特别是不同语法表示对解码成本的影响，以及如何优化语法表示以减少在线解码的计算开销。

Method: 将GCD建模为自回归下一个token分布与基于CFG编译的下推系统可达性预言的耦合；证明预言不变性定理；引入结构歧义成本(SAC)度量增量打包解析森林增长；建立引擎无关的下界；定义解码成本等价类；推导硬掩码解码的KL散度和总变差失真界限。

Result: 证明了语言等价语法产生相同的下一个token集合但编译状态空间不同；对于a^n b^n语言给出了精确的控制状态膨胀计数；SAC在右递归下为O(1)每token，在连接下为Θ(t²)每token；建立了任何在线掩码引擎必须至少Ω(t²)工作的下界；找到了最小SAC代表语法；推导了硬掩码解码的失真界限。

Conclusion: 语法表示对GCD性能有重要影响，通过优化语法结构可以显著降低解码成本；建立了理论下界和失真界限，为语法优化和高效GCD实现提供了理论基础；这些结果与Transformer和MoE架构集成，可用于预测性能建模和自动化语法优化。
...
### [123] [NOTAI.AI: Explainable Detection of Machine-Generated Text via Curvature and Feature Attribution](https://arxiv.org/abs/2603.05617)
*Oleksandr Marchenko Breneur,Adelaide Danilov,Aria Nourbakhsh,Salima Lamsiyah*

Main category: cs.CL

TL;DR: NOTAI.AI是一个可解释的机器生成文本检测框架，通过整合曲率信号、神经特征和文体特征，结合XGBoost分类器和SHAP解释，提供结构化自然语言解释的交互式检测系统。


<details>
  <summary>Details</summary>
Motivation: 开发一个可解释的机器生成文本检测框架，不仅能够准确识别AI生成的文本，还能提供透明、可理解的解释，增强用户对检测结果的信任和理解。

Method: 扩展Fast-DetectGPT方法，整合17个可解释特征（包括条件概率曲率、ModernBERT检测器分数、可读性指标和文体特征），使用XGBoost元分类器进行检测，应用SHAP进行特征归因，并通过LLM生成结构化自然语言解释。

Result: 开发了一个完整的交互式Web应用程序，支持实时分析、可视化特征检查和结构化证据呈现，源代码和演示视频已公开以支持可重复性。

Conclusion: NOTAI.AI提供了一个可解释、用户友好的机器生成文本检测框架，通过整合多种信号和解释技术，实现了透明化的AI文本检测，有助于增强检测系统的可信度和实用性。
...
### [124] [Safer Reasoning Traces: Measuring and Mitigating Chain-of-Thought Leakage in LLMs](https://arxiv.org/abs/2603.05618)
*Patrick Ahrend,Tobias Eder,Xiyang Yang,Zhiyi Pan,Georg Groh*

Main category: cs.CL

TL;DR: CoT提示会加剧LLM推理中的PII泄露风险，研究通过系统框架评估不同模型和推理预算下的泄露情况，并测试了多种轻量级门控方法。


<details>
  <summary>Details</summary>
Motivation: CoT提示虽然提升LLM推理能力，但会增加隐私风险，可能将提示中的个人身份信息泄露到推理轨迹和输出中，即使模型被指示不要重述PII。需要系统研究这种推理时的直接PII泄露。

Method: 使用模型无关框架：(1)将泄露定义为跨11种PII类型的风险加权、令牌级事件；(2)追踪泄露曲线随CoT预算的变化；(3)在结构化PII数据集上比较开源和闭源模型家族，采用分层风险分类法。然后测试了四种轻量级推理时门控方法：基于规则的检测器、TF-IDF+逻辑回归分类器、GLiNER-based NER模型和LLM-as-judge。

Result: CoT持续增加泄露，特别是高风险类别；泄露强烈依赖于模型家族和推理预算。增加推理预算可能放大或减弱泄露，取决于基础模型。在门控方法测试中，没有单一方法在所有模型或预算下表现最优。

Conclusion: 需要混合、风格自适应的门控策略，在通用、可复现的协议下平衡实用性和风险。单一方法无法在所有场景下表现最佳，需要根据模型特性和推理预算动态调整防护策略。
...
### [125] [The Fragility Of Moral Judgment In Large Language Models](https://arxiv.org/abs/2603.05651)
*Tom van Nuenen,Pratik S. Sachdeva*

Main category: cs.CL

TL;DR: 研究通过扰动框架测试LLM道德判断的稳定性，发现视角转换比表面编辑更易改变判断，协议选择影响最大，表明LLM道德判断受叙述形式和任务框架共同影响。


<details>
  <summary>Details</summary>
Motivation: 人们越来越多地使用大语言模型（LLM）进行日常道德和人际指导，但这些系统无法审视缺失的上下文，只能基于呈现的困境进行判断。需要测试LLM道德判断的稳定性和可操纵性。

Method: 引入扰动框架，使用2,939个来自r/AmItheAsshole的道德困境案例，生成三类内容扰动：表面编辑（词汇/结构噪声）、视角转换（声音和立场中立化）、说服线索（自我定位、社会证明、模式承认、受害者框架）。同时改变评估协议（输出顺序、指令位置、非结构化提示）。使用四个模型（GPT-4.1、Claude 3.7 Sonnet、DeepSeek V3、Qwen2.5-72B）评估所有变体（N=129,156个判断）。

Result: 表面扰动产生低翻转率（7.5%），基本在自一致性噪声范围内（4-13%），而视角转换导致显著更高的不稳定性（24.3%）。大量困境（37.9%）对表面噪声稳健但在视角变化下翻转，表明模型将叙述声音作为实用线索。不稳定性集中在道德模糊案例中；没有一方被归咎的场景最易受影响。说服扰动产生系统性方向性变化。协议选择主导所有其他因素：结构化协议间一致性仅67.6%（kappa=0.55），仅35.7%的模型-场景单元在所有三种协议中匹配。

Conclusion: LLM道德判断是由叙述形式和任务框架共同产生的，当结果取决于呈现技巧而非道德实质时，会引发可重复性和公平性问题。
...
### [126] [FreeTxt-Vi: A Benchmarked Vietnamese-English Toolkit for Segmentation, Sentiment, and Summarisation](https://arxiv.org/abs/2603.05690)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个降低技术门槛的工具，支持越南语这种使用广泛但在NLP中代表性不足的语言的多语言文本分析，促进越南语语言资源的发展。

Method: 设计统一的双语NLP流水线，集成混合VnCoreNLP和字节对编码分词策略、微调的TabularisAI情感分类器和微调的Qwen2.5抽象摘要模型，提供语料分析功能和基于Transformer的NLP组件。

Result: 在三部分评估（分词、情感分析、摘要）中，该方法在越南语和英语上都取得了与广泛使用的基线方法竞争或更优的性能表现。

Conclusion: FreeTxt-Vi通过降低多语言文本分析的技术障碍，支持可重复研究，适用于教育、数字人文、文化遗产和社会科学等领域的大规模文本处理。
...
### [127] [Towards Robust Retrieval-Augmented Generation Based on Knowledge Graph: A Comparative Analysis](https://arxiv.org/abs/2603.05698)
*Hazem Amamou,Stéphane Gagnon,Alan Davoust,Anderson R. Avila*

Main category: cs.CL

TL;DR: 该研究使用RGB基准评估RAG系统在不同场景下的鲁棒性，比较了RGB基线方法与GraphRAG知识图谱检索系统，并测试了三种GraphRAG定制化方案以提升系统可靠性。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成(RAG)旨在增强大语言模型的能力，减少事实幻觉并提供预训练之外的新信息。然而，不一致的检索信息可能对LLM响应产生负面影响，需要评估RAG系统在此类条件下的鲁棒性。

Method: 使用RGB基准在四种场景下评估LLMs：噪声鲁棒性、信息整合、负面拒绝和反事实鲁棒性。比较RGB RAG基线与基于知识图谱的检索系统GraphRAG，并测试三种GraphRAG定制化方案。

Result: 结果显示GraphRAG定制化方案相比RGB基线有所改进，为设计更可靠的现实世界RAG系统提供了见解。

Conclusion: 该研究通过RGB基准评估了RAG系统的鲁棒性，展示了GraphRAG知识图谱方法在应对不一致检索信息方面的优势，为构建更可靠的RAG系统提供了指导。
...
### [128] [Cultural Perspectives and Expectations for Generative AI: A Global Survey Approach](https://arxiv.org/abs/2603.05723)
*Erin van Liemt,Renee Shelby,Andrew Smart,Sinchana Kumbale,Richard Zhang,Neha Dixit,Qazi Mamunur Rashid,Jamila Smith-Loud*

Main category: cs.CL

TL;DR: 全球调查显示，人们对GenAI应如何代表文化存在不同看法，研究提出了基于社区定义的文化框架和具体建议


<details>
  <summary>Details</summary>
Motivation: 目前缺乏关于全球对GenAI应如何代表文化的态度的实证证据，需要了解不同群体对文化的理解以及GenAI应如何处理文化表达

Method: 通过大规模全球调查收集数据，涵盖欧洲、北美、南美、亚洲和非洲地区，从社区直接提炼文化的工作定义，理解文化概念的复杂性及其与GenAI表达的关系

Result: 建立了基于社区定义的文化理解框架，识别了文化表达的复杂性，并提出了针对GenAI发展的具体建议

Conclusion: 提出了文化与GenAI发展的建议，包括参与式方法、超越地理维度的特定文化维度（如宗教和传统）优先考虑，以及处理文化"红线"的敏感性框架
...
### [129] [Structured Multidimensional Representation Learning for Large Language Models](https://arxiv.org/abs/2603.05727)
*Alaa El Ichi,Khalide Jbilou,Mohamed El Guide,Franck Dufrenois*

Main category: cs.CL

TL;DR: 提出基于L-积的张量Transformer架构，通过将嵌入空间进行结构化谱分解，在保持Transformer语义的同时实现参数压缩和频率归纳偏置。


<details>
  <summary>Details</summary>
Motivation: Transformer架构在多种任务中表现出色，但参数增长快且嵌入维度存在冗余。需要一种既能压缩参数又能保持性能的方法。

Method: 使用基于三阶张量L-积的结构化谱分解，将token表示重塑为谱张量切片，在变换域执行注意力机制和前馈操作，将编码器分解为p个独立的谱子Transformer。

Result: 在IMDB和AG News数据集上，提出的张量化编码器能显著减少编码器参数（p=4时最多减少75%），同时保持竞争力准确率。在BERT-base宽度下性能恢复到同等水平。

Conclusion: L-Transformer通过谱分解实现了参数压缩和频率归纳偏置，在保持Transformer语义的同时显著减少参数，为高效Transformer设计提供了新思路。
...
### [130] [Let's Talk, Not Type: An Oral-First Multi-Agent Architecture for Guaraní](https://arxiv.org/abs/2603.05743)
*Samantha Adorno,Akshata Kishore Moharir,Ratna Kandala*

Main category: cs.CL

TL;DR: 本文以巴拉圭官方语言瓜拉尼语为例，指出当前AI系统以文本为中心的设计忽视了口语优先的语言和土著社区，提出了一种口语优先的多智能体架构作为替代方案。


<details>
  <summary>Details</summary>
Motivation: 当前人工智能和人机交互系统虽然被宣传为通用解决方案，但其设计仍以文本为中心，未能充分服务于以口语为主的语言和土著社区。作者以巴拉圭广泛使用的瓜拉尼语为例，指出除非AI语言支持与真实的口语实践相结合，否则这种支持仍然不足。

Method: 提出了一种替代标准"文本到语音"流程的口语优先多智能体架构。该架构将瓜拉尼语的自然语言理解与专门的对话状态智能体和社区主导的治理智能体解耦，创建了一个尊重土著数据主权和双语现象的技术框架。

Result: 开发了一个技术框架，超越了单纯的语言识别，重点关注对话轮转、修复和共享上下文作为交互的主要场所。该框架确保数字生态系统能够赋能而非忽视多样化的语言实践。

Conclusion: 要使AI真正具有文化根基，必须从将口语语言适应文本中心系统，转变为将口语对话作为首要设计需求。只有通过尊重土著数据主权和双语现象的口语优先方法，数字生态系统才能真正赋能多样化的语言实践。
...
### [131] [NERdME: a Named Entity Recognition Dataset for Indexing Research Artifacts in Code Repositories](https://arxiv.org/abs/2603.05750)
...
### [132] [PVminerLLM: Structured Extraction of Patient Voice from Patient-Generated Text using Large Language Models](https://arxiv.org/abs/2603.05776)
*Samah Fodeh,Linhai Ma,Ganesh Puthiaraju,Srivani Talakokkul,Afshan Khan,Ashley Hagaman,Sarah Lowe,Aimee Roundtree*

Main category: cs.CL

TL;DR: PVminerLLM：一种专门用于从患者生成文本中提取结构化患者声音信息的监督微调大语言模型，在多个数据集上显著优于基于提示的方法，即使使用较小模型也能实现可靠性能。


<details>
  <summary>Details</summary>
Motivation: 患者生成文本包含关于患者生活经历、社会环境和护理参与的关键信息，这些因素强烈影响依从性、护理协调和健康公平。然而这些患者声音信号很少以结构化形式可用，限制了其在患者中心结局研究和临床质量改进中的应用。因此，可靠提取此类信息对于大规模理解和解决健康结局的非临床驱动因素至关重要。

Method: 引入PVminer基准用于结构化提取患者声音，并提出PVminerLLM——专门为此任务设计的监督微调大语言模型。该方法在多个数据集和模型规模上进行评估。

Result: PVminerLLM在多个数据集和模型规模上显著优于基于提示的基线方法，在代码预测方面达到83.82% F1分数，子代码预测达到80.74% F1分数，证据跨度提取达到87.03% F1分数。值得注意的是，即使使用较小模型也能实现强大性能，表明可靠的患者声音提取无需极端模型规模。

Conclusion: PVminerLLM实现了从患者生成文本中提取社会和经验信号的可扩展分析，为患者中心结局研究和临床质量改进提供了有效工具。代码、评估脚本和训练模型将公开发布，注释数据集可根据研究请求提供。
...
### [133] [Tutor Move Taxonomy: A Theory-Aligned Framework for Analyzing Instructional Moves in Tutoring](https://arxiv.org/abs/2603.05778)
*Zhuqian Zhou,Kirk Vanacore,Tamisha Thompson,Jennifer St John,Rene Kizilcec*

Main category: cs.CL

TL;DR: 本文提出了一个用于大规模分析辅导对话的辅导行为分类法，通过混合演绎-归纳方法开发，将辅导行为分为四类，支持AI标注和辅导策略计算建模。
...
Method: 采用混合演绎-归纳方法：首先综合认知科学、学习科学、课堂话语分析和智能辅导系统的研究构建初步框架，然后通过专家标注真实辅导转录本的迭代编码来完善分类法。

Result: 开发了一个包含四类辅导行为的分类法：辅导支持、学习支持、社会情感与动机支持、以及后勤支持。学习支持行为进一步按学生参与度谱系组织，区分激发学生推理的行为与提供直接解释或答案的行为。

Conclusion: 该分类法通过将辅导对话定义为离散的教学行为，实现了使用AI进行可扩展标注、辅导策略的计算建模，以及辅导行为与学习结果关系的实证分析。
...
### [134] [RouteGoT: Node-Adaptive Routing for Cost-Efficient Graph of Thoughts Reasoning](https://arxiv.org/abs/2603.05818)
...
<details>
  <summary>Details</summary>
Motivation: 现有图结构推理方法（如ToT、GoT、AGoT）虽然能提升某些基准测试的准确率，但引入了显著的token消耗和延迟开销，且性能增益在不同任务分布中不稳定。研究发现推理管道中存在阶段和节点异质性：高质量规划和最终综合需要强大模型，而许多中间子任务可由轻量级模型准确解决。

Method: RouteGoT是一个预算可控、节点自适应的图结构推理路由框架。它在图内进行路由，优先为规划和综合任务分配强大模型，同时根据预测的难度动态为叶子子任务分配轻量级模型和成本效益策略。框架集成了显式预算约束到全局推理调度器中，在用户指定的token预算下控制图扩展，实现可预测的性能-成本权衡。

Result: 在推理、检索和多跳问答基准测试中，RouteGoT匹配或提升了准确率，同时显著减少了token使用。具体来说，与AGoT相比，平均实现了8.1个百分点的准确率提升和79.1%的输出token减少。此外，RouteGoT通过保持更优的成本-准确率权衡，优于现有路由基线，在不同预算目标和任务下表现出改进的鲁棒性。

Conclusion: RouteGoT通过智能路由和预算控制，有效解决了图结构推理中的效率问题，实现了更好的性能-成本权衡，为实际应用中的大语言模型推理提供了实用解决方案。
...
### [135] [HART: Data-Driven Hallucination Attribution and Evidence-Based Tracing for Large Language Models](https://arxiv.org/abs/2603.05828)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型容易产生幻觉内容，严重影响其在关键应用领域的可靠性。现有的幻觉归因方法主要关注语义相似度匹配或表示级判别，难以在片段级别建立幻觉类型、错误生成机制和外部事实证据之间的结构化对应关系，限制了幻觉片段的可解释性和支持/反对证据的可追溯性。

Method: 提出HART框架，将幻觉追踪形式化为包含四个阶段的结构化建模任务：1) 片段定位，2) 机制归因，3) 证据检索，4) 因果追踪。基于此构建了首个针对幻觉追踪的结构化数据集，联合标注幻觉类型、错误机制和反事实证据集。

Result: 在提出的数据集上的实验结果表明，HART显著优于BM25和DPR等强检索基线，验证了所提出的追踪范式在幻觉分析和证据对齐方面的有效性和泛化能力。

Conclusion: HART通过结构化建模方法解决了现有幻觉归因方法的局限性，提供了更细粒度和可解释的幻觉分析框架，为大型语言模型的可靠性评估和改进提供了新的途径。
...
### [136] [ReflexiCoder: Teaching Large Language Models to Self-Reflect on Generated Code and Self-Correct It via Reinforcement Learning](https://arxiv.org/abs/2603.05863)
*Juyong Jiang,Jiasi Shen,Sunghun Kim,Kang Min Yoo,Jeonghoon Kim,Sungju Kim*

Main category: cs.CL

TL;DR: ReflexiCoder是一个基于强化学习的代码生成框架，通过内部化结构化推理轨迹（包括初始生成、bug和优化感知的反思、自我修正），使模型具备内在的自主反思和修正能力，无需依赖外部反馈，在多个代码生成基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在代码生成任务中面临性能瓶颈：标准的"System 1"单次前向生成方法在处理复杂算法任务时性能受限；现有的迭代优化策略依赖外部反馈、执行引擎或计算昂贵的提示-响应循环，缺乏内在的自主反思能力。

Method: 提出ReflexiCoder强化学习框架，采用RL-zero训练范式，使用细粒度奖励函数优化整个反思-修正轨迹。该方法将结构化推理轨迹（初始生成、bug和优化感知的反思、自我修正）直接内化到模型权重中，使模型在推理时具备完全自主的反思和修正能力，无需依赖真实反馈或执行引擎。

Result: 在7个基准测试中，ReflexiCoder-8B在1.5B-14B范围内的开源模型中达到SOTA：HumanEval (94.51%)、HumanEval Plus (87.20%)、MBPP (81.80%)、MBPP Plus (78.57%)、BigCodeBench (35.00%)、LiveCodeBench (52.21%)、CodeForces (37.34%)，性能媲美或超越GPT-5.1等专有模型。同时推理计算开销减少约40%，显著提升token效率。

Conclusion: ReflexiCoder通过将结构化推理轨迹内化到模型权重中，实现了从依赖外部反馈到内在自主反思的范式转变，显著提升了代码生成性能，同时降低了推理计算成本，为代码生成模型的自我改进能力提供了新方向。
...
### [137] [ROSE: Reordered SparseGPT for More Accurate One-Shot Large Language Models Pruning](https://arxiv.org/abs/2603.05878)
*Mingluo Su,Huan Wang*

Main category: cs.CL

TL;DR: ROSE是一种改进的SparseGPT剪枝方法，通过重新排序剪枝顺序来提升大语言模型剪枝效果，特别针对权重矩阵的列模式进行优化。


<details>
  <summary>Details</summary>
Motivation: SparseGPT等基于二阶梯度（Hessian）的大语言模型剪枝方法采用固定的从左到右剪枝顺序，当权重呈现列模式时会导致次优性能。本文研究剪枝顺序对SparseGPT框架的影响，旨在提升剪枝效果。

Method: 提出ROSE方法：1）预剪枝识别候选权重；2）估计列和块剪枝损失；3）两级重新排序：块内按列损失降序排列列，块间按块损失重新排序；4）引入块损失相对范围指标识别列层，实现全模型自适应重排序。

Result: 在主流LLM（LLaMA2-7B/13B/70B、LLaMA3-8B、Mistral-7B）上的大量实验结果表明，ROSE超越了原始SparseGPT和其他对比剪枝方法。

Conclusion: 剪枝顺序对基于Hessian的大语言模型剪枝方法有重要影响，ROSE通过重新排序剪枝顺序显著提升了剪枝性能，特别适用于具有列模式的权重矩阵。
...
### [138] [Confidence Before Answering: A Paradigm Shift for Efficient LLM Uncertainty Estimation](https://arxiv.org/abs/2603.05881)
*Changcheng Li,Jiancan Wu,Hengheng Zhang,Zhengsu Chen,Guo An,Junxiang Qiu,Xiang Wang,Qi Tian*

Main category: cs.CL

TL;DR: CoCA框架通过强化学习联合优化置信度校准和答案准确性，采用置信度优先范式，在生成答案前输出置信度，提高LLM的不确定性估计可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM不确定性估计方法多为答案优先范式，在生成答案后才输出置信度，这限制了实际应用。需要一种置信度优先的范式，让模型在回答前就能输出其正确概率。

Method: 提出CoCA框架，使用GRPO强化学习方法，通过分段信用分配联合优化置信度校准和答案准确性。为置信度和答案段分配独立的奖励和组相对优势，实现稳定联合优化并避免奖励攻击。

Result: 在数学、代码和事实问答基准测试中，CoCA显示出改进的校准效果和不确定性区分能力，同时保持了答案质量，为下游应用提供了更可靠的不确定性估计。

Conclusion: CoCA框架通过置信度优先范式和联合优化方法，有效提高了LLM不确定性估计的可靠性，为更广泛的下游应用提供了支持。
...
### [139] [VerChol -- Grammar-First Tokenization for Agglutinative Languages](https://arxiv.org/abs/2603.05883)
*Prabhu Raja*
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的分词方法BPE及其变体是脚本无关的，主要针对英语等屈折语优化，无法有效处理黏着语的特点。黏着语中单个词可能包含词根、时态、体、人称、数、性一致、格和后置词等多种语素信息，需要专门的分词方法。

Method: 论文未在摘要中明确说明具体方法，但暗示需要针对黏着语特点开发新的分词方法，可能涉及语素感知的分词策略。

Result: 统计分词器（如BPE）会将黏着语的单词分割成字节对块，破坏语素边界并增加token数量，影响语言模型处理黏着语的效率。

Conclusion: 当前主流的分词方法不适合处理黏着语，需要开发能够保留语素完整性的分词方法，以更好地服务于达罗毗荼语系、突厥语系、乌拉尔语系、韩语、日语、斯瓦希里语、巴斯克语等黏着语的语言模型处理。
...
### [140] [Lost in Stories: Consistency Bugs in Long Story Generation by LLMs](https://arxiv.org/abs/2603.05890)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成长篇叙事时经常出现自相矛盾的问题，包括事实、角色特征和世界规则的不一致，而现有的故事生成基准主要关注情节质量和流畅性，忽略了一致性错误。

Method: 提出了ConStory-Bench基准测试，包含2000个提示，涵盖四种任务场景，定义了5大类19种子类型的错误分类法。开发了ConStory-Checker自动化管道，能够检测矛盾并为每个判断提供明确的文本证据。

Result: 评估多个LLM后发现：一致性错误在事实和时间维度最常见；倾向于出现在叙事中间部分；发生在token级熵较高的文本段中；某些错误类型倾向于同时出现。

Conclusion: 这些发现可以为改进长文本叙事生成的一致性提供指导，ConStory-Bench基准和ConStory-Checker工具为评估和提升LLM在长文本生成中的一致性提供了系统框架。
...
### [141] [Building an Ensemble LLM Semantic Tagger for UN Security Council Resolutions](https://arxiv.org/abs/2603.05895)
*Hussein Ghaly*

Main category: cs.CL

TL;DR: 本文提出了一种基于LLM的集成系统，用于联合国安理会决议的语义标注，通过引入CPR和TWF评估指标来避免幻觉和内容偏差，GPT-4.1表现最佳，而GPT-4.1-mini能以20%成本达到相近性能。


<details>
  <summary>Details</summary>
Motivation: 利用LLM性能变异性构建集成系统，用于数据清洗和语义标注任务，解决在处理联合国安理会决议这类复杂文本时可能出现的幻觉、不必要添加或遗漏内容的问题。

Method: 引入两个评估指标：内容保持率（CPR）和标签格式良好度（TWF），通过多个GPT模型的多次运行，选择最佳输出构建集成系统，实现可靠的数据清洗和语义标注。

Result: GPT-4.1在两个任务中都获得最高指标（清洗：CPR 84.9%；语义标注：CPR 99.99%和TWF 99.92%）。较小的GPT-4.1-mini模型能以20%的成本达到相近性能。集成系统能够为所有LLM模型选择最优输出。

Conclusion: 通过集成设计和评估指标的使用，创建了一个可靠的LLM系统，能够在具有挑战性的文本上执行语义标注任务，平衡了性能与成本效益。
...
### [142] [Learning Next Action Predictors from Human-Computer Interaction](https://arxiv.org/abs/2603.05923)
...
<details>
  <summary>Details</summary>
Motivation: 真正主动的AI系统需要预测用户的下一步行为，这需要比稀疏提示更丰富的上下文信息。当前系统缺乏对用户完整交互历史的推理能力，需要新的数据和方法来预测用户的多模态交互行为。

Method: 1) 使用视觉语言模型标注纵向自然计算机使用数据，开发开源标注流程；2) 提出LongNAP模型，结合参数化和上下文学习，通过策略梯度方法训练生成用户特定推理轨迹，检索相关历史轨迹，并在上下文中应用来预测未来动作。

Result: 标注了20名用户一个月手机使用的36万多个动作(1800小时屏幕时间)；LongNAP在保留数据上显著优于监督微调和提示基线(分别提升79%和39%)；17.1%的预测轨迹与用户实际行为良好对齐(LLM-judge评分≥0.5)，高置信度预测时提升至26%；模型能够泛化到未见过的用户。

Conclusion: 通过学习用户行为的完整上下文来预测用户需求已成为可行任务，具有巨大潜力。LongNAP展示了在长交互历史中预测用户动作的有效性，为真正主动的AI系统奠定了基础。
...
### [143] [Addressing the Ecological Fallacy in Larger LMs with Human Context](https://arxiv.org/abs/2603.05928)
*Nikita Soni,Dhruv Vijay Kunjadiya,Pratham Piyush Shah,Dikshya Mohanty,H. Andrew Schwartz,Niranjan Balasubramanian*

Main category: cs.CL

TL;DR: 通过建模作者语言上下文（HuLM任务）解决生态谬误问题，在8B Llama模型上验证了作者语境建模对大型语言模型性能的提升效果


<details>
  <summary>Details</summary>
Motivation: 传统语言模型训练和推理忽略了语言生成的基本事实——同一作者生成的多段文本之间存在依赖关系。先前研究表明解决这种生态谬误可以显著提升小型GPT模型的性能，本研究旨在验证类似方法是否对更大规模的8B Llama模型同样有效。

Method: 提出HuLM（Human-aware Language Modeling）任务，在作者的其他时间顺序文本语境中处理其语言。研究了两种变体：1）使用HuLM目标进行预训练；2）在微调阶段使用作者语境（HuFT: Human-aware Fine-Tuning）。使用QLoRA技术进行高效微调和持续预训练。

Result: 实验表明：1）仅通过QLoRA在微调阶段解决生态谬误就能提升8B模型性能；2）基于QLoRA的持续HuLM预训练产生的人类感知模型，仅通过线性任务分类器训练就在八个下游任务上获得改进性能。

Conclusion: 建模语言在其原始生成者（作者）语境中的重要性得到验证，表明考虑作者语言上下文对提升大型语言模型性能具有实用价值和重要意义。
...
### [144] [Implicit Style Conditioning: A Structured Style-Rewrite Framework for Low-Resource Character Modeling](https://arxiv.org/abs/2603.05933)
*Chanhui Zhu*

Main category: cs.CL

TL;DR: 提出结构化风格重写框架，通过显式解耦词汇、句法和语用三个维度的风格特征，结合CoT蒸馏实现隐式风格条件化，使小模型在角色扮演中超越大模型的表现。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在角色扮演中难以捕捉复杂风格特征，标准监督微调只能学到表层语义而无法重现角色的句法和语用细微差别，导致"角色出戏"的生成问题。

Method: 提出结构化风格重写框架：1）显式解耦风格为三个可解释维度：词汇特征（通过PMI）、句法模式（基于PCFG规则）和语用风格；2）引入基于思维链蒸馏的隐式风格条件化策略，利用训练中的显式推理轨迹作为强归纳偏置。

Result: 在动漫角色这一高风格化领域的实验中，Qwen-1.7B模型在风格一致性和语义保真度上显著超越更大的基线模型（如4B标准监督微调）。

Conclusion: 该方法提供了一种数据高效的范式，能够在消费级硬件上实现高质量的风格化生成，为小模型的角色扮演能力提升提供了有效解决方案。
...
### [145] [Who We Are, Where We Are: Mental Health at the Intersection of Person, Situation, and Large Language Models](https://arxiv.org/abs/2603.05953)
*Nikita Soni,August Håkan Nilsson,Syeda Mahwish,Vasudha Varadarajan,H. Andrew Schwartz,Ryan L. Boyd*

Main category: cs.CL

TL;DR: 该研究开发了可解释的模型来预测心理健康状态，通过整合个人心理特质与情境特征，在社交媒体数据中识别适应性和非适应性自我状态。


<details>
  <summary>Details</summary>
Motivation: 心理健康不是固定特质而是动态过程，受到个人特质与情境交互作用的影响。研究旨在通过整合计算建模与心理学理论，以情境敏感且人类可理解的方式评估动态心理状态。

Method: 基于互动主义和建构主义心理学理论，整合个人层面的心理特质（如韧性、认知扭曲、内隐动机）与基于情境8 DIAMONDS框架的语言推断情境特征。比较这些理论驱动的特征与心理测量学信息语言模型的嵌入表示。

Result: 理论驱动的特征提供了具有竞争力的预测性能，同时具有更好的可解释性。定性分析进一步突出了对幸福感最具预测性特征的心理一致性。

Conclusion: 研究强调了整合计算建模与心理学理论的价值，能够以情境敏感且人类可理解的方式评估动态心理状态，为心理健康研究提供了新的方法论框架。
...
### [146] [Track-SQL: Enhancing Generative Language Models with Dual-Extractive Modules for Schema and Context Tracking in Multi-turn Text-to-SQL](https://arxiv.org/abs/2603.05996)
...
Conclusion: Track-SQL通过双提取模块有效增强生成式语言模型在多轮Text-to-SQL中的表现，显著提升执行准确率，代码已开源。
...
### [147] [MASFactory: A Graph-centric Framework for Orchestrating LLM-Based Multi-Agent Systems with Vibe Graphing](https://arxiv.org/abs/2603.06007)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的多智能体系统框架在实现复杂图工作流时需要大量手动工作，复用性有限，难以集成异构外部上下文源，需要更高效的编排解决方案。
...
Conclusion: MASFactory通过图中心框架解决了LLM-based多智能体系统工作流编排的挑战，提供了更高效、可复用和可扩展的解决方案。
...
### [148] [ViewFusion: Structured Spatial Thinking Chains for Multi-View Reasoning](https://arxiv.org/abs/2603.06024)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多视图空间推理方面表现不佳，即使有多个视角可用，模型也往往未能充分利用跨视图关系，而是依赖单图像捷径，导致在视角变换和遮挡敏感案例中性能脆弱。
...
Conclusion: 通过显式分离跨视图空间预对齐和问答的两阶段框架，ViewFusion有效解决了多视图空间推理问题，显著提升了模型在需要跨视图对齐任务上的性能。
...
### [149] [Evaluating Austrian A-Level German Essays with Large Language Models for Automated Essay Scoring](https://arxiv.org/abs/2603.06066)
*Jonas Kubesch,Lena Huber,Clemens Havas*

Main category: cs.CL

TL;DR: 研究评估了四种开源大语言模型在奥地利高中德语作文自动评分中的应用效果，发现模型在评分细则维度上最高仅达到40.6%的人工评分一致性，最终成绩匹配率仅32.8%，表明当前模型在实际评分环境中准确性不足。


<details>
  <summary>Details</summary>
Motivation: 自动作文评分系统旨在减轻教师评分负担并减少主观偏见。虽然早期系统依赖手工特征和统计模型，但大语言模型的进步为评估学生写作提供了前所未有的灵活性。本研究旨在探索最先进的开源大语言模型在奥地利高中德语文本评分中的应用，特别关注基于评分细则的评估。

Method: 研究处理了101份匿名学生考试文本，涵盖三种文本类型。评估了四种大语言模型：DeepSeek-R1 32b、Qwen3 30b、Mixtral 8x7b和LLama3.3 70b，采用不同的上下文和提示策略。重点考察模型使用标准化评分细则对德语作文进行评分的能力。

Result: 大语言模型在评分细则提供的子维度上最高达到40.6%的人工评分一致性，最终成绩匹配率仅为32.8%。结果表明，尽管较小的模型能够使用标准化评分细则进行德语作文评分，但其准确性不足以在实际评分环境中使用。

Conclusion: 即使是最先进的开源大语言模型，在奥地利高中德语作文自动评分任务中仍无法达到实际应用所需的准确性水平。模型在评分细则维度和最终成绩匹配方面的一致性都较低，表明当前技术尚未成熟到可以替代人工评分的程度。
...
### [150] [Experiences Build Characters: The Linguistic Origins and Functional Impact of LLM Personality](https://arxiv.org/abs/2603.06088)
*Xi Wang,Mengdie Zhuang,Jiqun Liu*

Main category: cs.CL

TL;DR: 该研究通过持续预训练让语言模型接触不同领域文本，模拟经验积累，发现模型能力呈双峰分布，存在"表达型通才"和"抑制型专家"两种最优类型，并揭示了"抑制优势"现象。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型开发过于注重统一性能基准，偏向特定行为倾向（如自信），忽视了人类问题解决中多样化的风格和个性特质。研究旨在探索不同经验如何塑造机器个性并影响问题解决能力。

Method: 采用持续预训练方法，以无监督方式让模型接触领域特定文本，模拟经验积累过程。使用基于大五人格框架的机器人格量表（MPI）量化模型变体的人格特质，分析其与语言风格和推理行为的关系。

Result: 发现模型能力呈双峰分布，在"表达型通才"和"抑制型专家"处达到峰值。识别出"抑制优势"现象，即降低社交特质能提升复杂推理性能。建立了训练数据语言学特征（如祈使句频率、词汇多样性）与模型个性之间的因果关系。

Conclusion: 研究为"个性工程"提供了路线图，表明通过控制训练数据的语言学特征可以系统性地塑造模型个性，从而优化特定任务性能，为开发更具多样性和适应性的人工智能系统提供了新方向。
...
### [151] [Making Implicit Premises Explicit in Logical Understanding of Enthymemes](https://arxiv.org/abs/2603.06114)
*Xuyao Feng,Anthony Hunter*

Main category: cs.CL

TL;DR: 本文提出了一种处理省略三段论（enthymemes）的集成方法，结合大语言模型生成隐含前提、翻译为逻辑公式，以及使用神经符号推理器进行蕴含判断。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的文本和对话论证通常是省略三段论（部分前提或主张是隐含的）。现有的NLP方法只能识别省略三段论但不能解码其底层逻辑，而基于逻辑的方法则需要假设有足够公式的知识库来进行溯因解码。目前缺乏将省略三段论的文本组件系统性地转换为逻辑论证并生成解码所需逻辑公式的方法。

Method: 提出一个集成管道，包含三个步骤：1）使用大语言模型基于显式前提和主张生成中间隐含前提；2）使用另一个大语言模型将自然语言翻译为逻辑公式；3）使用基于SAT求解器的神经符号推理器来确定蕴含关系。

Result: 在两个省略三段论数据集上评估了该管道，在选取正确隐含前提方面表现出有希望的性能，通过精确率、召回率、F1分数和准确率进行衡量。

Conclusion: 该研究为解决省略三段论的系统性逻辑解码问题提供了一种有效的集成方法，结合了语言模型和符号推理的优势。
...
### [152] [A Causal Graph Approach to Oppositional Narrative Analysis](https://arxiv.org/abs/2603.06135)
...
Method: 提出基于图的框架，将叙事表示为实体交互图。通过节点级因果估计，将构建的句子图提炼为最小因果子图，得到对最终分类的因果表示。基于此表示构建分类流程。

Result: 该分类流程在对立思维分类任务上优于现有方法。
...
### [153] [CRIMSON: A Clinically-Grounded LLM-Based Metric for Generative Radiology Report Evaluation](https://arxiv.org/abs/2603.06183)
...
TL;DR: CRIMSON是一个临床基础的胸部X光报告生成评估框架，通过诊断正确性、上下文相关性和患者安全性评估报告，结合完整临床上下文，防止正常或临床不显著发现对评分产生过度影响。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标未能充分纳入完整临床上下文，且常让正常或临床不显著的发现对整体评分产生不成比例的影响，缺乏对临床重要错误的优先级考虑。

Method: CRIMSON框架包含完整临床上下文（患者年龄、指征、基于指南的决策规则），将错误分类为全面的分类法（假阳性发现、遗漏发现和八种属性级错误），并为每个发现分配临床显著性级别（紧急、可行动非紧急、不可行动、预期/良性），基于与心胸放射科医生合作开发的指南进行严重性感知加权。

Result: CRIMSON在ReXVal中与六名认证放射科医生标注的临床显著错误计数高度一致（Kendall's tau = 0.61-0.71；Pearson's r = 0.71-0.84）；在RadJudge中与专家判断一致；在RadPref中与放射科医生偏好最强对齐。发布了指标、评估基准和微调的MedGemma模型。

Conclusion: CRIMSON提供了一个临床基础的评估框架，能够更准确地评估胸部X光报告生成质量，优先考虑临床重要错误，为可重复的报告生成评估提供了工具和基准。
...
### [154] [MAPO: Mixed Advantage Policy Optimization for Long-Horizon Multi-Turn Dialogue](https://arxiv.org/abs/2603.06194)
*Naifan Zhang,Ruihan Sun,Jinwei Su,Hengjie Yang,Zhengyuan Pan,Zhaohan Chen,Xiaofan Zhang*

Main category: cs.CL

TL;DR: MAPO算法利用法官模型的密集过程反馈和蒙特卡洛回报，通过混合优势估计器实现细粒度信用分配，在多轮主观对话任务中显著提升训练稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 主观多轮对话任务需要适应不断变化的用户状态并优化长期交互质量的对话策略，但强化学习面临缺乏可靠过程监督的挑战。仅基于结果的训练将多轮信用分配压缩为单一轨迹级奖励，而简单的轮级组采样在交互环境中成本过高。

Method: 提出MAPO算法：1）利用法官模型提供密集过程反馈；2）通过蒙特卡洛回报传播长期效应；3）引入混合优势估计器，结合轮级归一化和批次级归一化，实现细粒度且可扩展的信用分配。

Result: 在多个主观对话基准测试（EMPA、EmoBench、EQ-Bench）和7B到32B模型规模上，MAPO相比仅基于结果的GRPO和单级归一化基线，在训练稳定性和最终性能上均有持续提升。在EMPA上，7B基础模型的成功率提升达9个百分点，对话分数提升+43.2。尽管仅在EMPA风格环境中训练，该方法在未见过的情感智能基准测试上也能很好泛化。
...
### [155] [Wisdom of the AI Crowd (AI-CROWD) for Ground Truth Approximation in Content Analysis: A Research Protocol & Validation Using Eleven Large Language Models](https://arxiv.org/abs/2603.06197)
*Luis de-Marcos,Manuel Goyanes,Adrián Domínguez-Díaz*

Main category: cs.CL

TL;DR: AI-CROWD协议通过集成多个大语言模型的输出来近似真实标签，解决大规模内容分析中缺乏可观察真实标签的问题。


<details>
  <summary>Details</summary>
Motivation: 大规模内容分析面临缺乏可观察真实标签的挑战，因为通过人工编码创建基准在大数据集上变得不切实际，存在时间、成本和一致性方面的困难。

Method: 引入AI-CROWD协议，利用多个大语言模型输出的集合来近似真实标签。通过多数投票聚合输出，并使用诊断指标分析一致/分歧模式，识别高置信度分类并标记潜在模糊性或模型特定偏差。

Result: 该协议生成基于共识的近似标签，而不是声称这些标签是真实标签，通过聚合多个模型的收敛和发散推断来获得结果。

Conclusion: AI-CROWD协议为大规模内容分析提供了一种实用的解决方案，通过集成多个LLM的输出来近似真实标签，同时能够识别高置信度分类和潜在问题区域。
...
### [156] [LIT-RAGBench: Benchmarking Generator Capabilities of Large Language Models in Retrieval-Augmented Generation](https://arxiv.org/abs/2603.06198)
*Koki Itai,Shunichi Hasegawa,Yuta Yamamoto,Gouki Minegishi,Masaki Otsuki*

Main category: cs.CL

TL;DR: LIT-RAGBench是一个新的RAG生成器基准测试，评估LLM在五个关键能力上的表现：逻辑推理、信息整合、表格理解、多步推理和证据缺失时的弃权能力，使用虚构实体和场景确保答案基于外部文档。


<details>
  <summary>Details</summary>
Motivation: 现有RAG生成器基准测试覆盖有限，无法在统一条件下同时评估多个实际应用所需的关键能力，如长上下文整合、多步推理、表格解释和证据缺失时的弃权判断。

Method: 定义五个评估类别（整合、推理、逻辑、表格、弃权），每个类别细分为实际评估方面。使用虚构实体和场景构建114个人工构造的日语问题，通过机器翻译和人工校对生成英文版本。采用LLM-as-a-Judge进行评分，报告类别和总体准确率。

Result: 在API和开源模型测试中，没有模型总体准确率超过90%。基准测试能够测量每个类别中的强弱项，为实际RAG部署中的模型选择和RAG专用模型构建提供有价值的度量标准。

Conclusion: LIT-RAGBench填补了现有评估与实际应用之间的差距，通过系统覆盖多个能力类别和组合模式，成为RAG生成器评估的重要工具，有助于模型选择和RAG专用模型的开发。
...
### [157] [FlashPrefill: Instantaneous Pattern Discovery and Thresholding for Ultra-Fast Long-Context Prefilling](https://arxiv.org/abs/2603.06199)
*Qihang Fan,Huaibo Huang,Zhiying Wu,Juqiu Wang,Bingning Wang,Ran He*

Main category: cs.CL

TL;DR: FlashPrefill是一个通过瞬时模式发现和阈值处理实现超快速预填充的框架，在256K序列上实现27.78倍加速，在4K上下文长度上仍保持1.71倍加速。


<details>
  <summary>Details</summary>
Motivation: 长上下文建模是大型语言模型的关键能力，但注意力的二次复杂度仍然是关键瓶颈，特别是在计算密集的预填充阶段。现有的稀疏注意力机制通常存在显著的搜索延迟或稀疏性不足的问题。

Method: FlashPrefill采用快速块搜索技术同时定位动态垂直、斜线和块稀疏注意力模式，并引入动态阈值机制，避免排序或累积注意力分数的过高开销，同时有效消除长尾分布以增强稀疏性。

Result: 在广泛评估中，FlashPrefill在256K序列上实现了前所未有的27.78倍加速。即使在4K上下文长度上，仍保持1.71倍加速，表明其在不同序列规模下的鲁棒性和实用性。

Conclusion: FlashPrefill通过瞬时模式发现和动态阈值处理，显著提高了长上下文建模的效率，解决了现有稀疏注意力方法的延迟和稀疏性不足问题，为大规模序列处理提供了实用的解决方案。
...
### [158] [SPOT: Span-level Pause-of-Thought for Efficient and Interpretable Latent Reasoning in Large Language Models](https://arxiv.org/abs/2603.06222)
...
<details>
  <summary>Details</summary>
Motivation: 显式思维链虽然能提升大语言模型的推理性能，但会产生冗长的token级追踪，导致高昂的推理成本。现有方法主要通过剪枝或简洁提示来减少开销，但大多只是截断模型输出而非内化模型思考过程

Method: SPOT框架采用跨度级语义对齐的Sinkhorn最优传输目标，将每个暂停标记与整个推理段的语义进行软匹配，避免僵化的步骤末端对齐。同时引入冻结头解码约束，确保潜在状态在冻结的预训练语言模型头下可直接解码为token分布

Result: 在推理基准测试中，SPOT平均提升准确率2.3个百分点，同时减少生成token数量37.5%，并能提供潜在推理过程的忠实语义解释

Conclusion: SPOT通过灵活的潜在推理框架，在显著减少推理成本的同时提升性能，并保持潜在推理过程的可解释性，为高效推理提供了新方向
...
### [159] [Mind the Gap: Pitfalls of LLM Alignment with Asian Public Opinion](https://arxiv.org/abs/2603.06264)
*Hari Shankar,Vedanta S P,Sriharini Margapuri,Debjani Mazumder,Ponnurangam Kumaraguru,Abhijnan Chakraborty*

Main category: cs.CL

TL;DR: 该研究对GPT-4o-Mini、Gemini-2.5-Flash等当代大语言模型在印度、东亚和东南亚地区的文化对齐性进行了多语言审计，发现这些模型在宗教等敏感领域存在显著的文化偏差，特别是对少数群体的宗教观点表达不准确，并放大了负面刻板印象。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地部署在多语言、多文化环境中，但其主要依赖英语中心训练数据，可能与不同社会的多元文化价值观存在错位。研究者希望通过系统审计来评估LLMs在不同文化背景下的对齐情况，特别是关注宗教这一敏感领域。

Method: 研究对GPT-4o-Mini、Gemini-2.5-Flash、Llama 3.2、Mistral和Gemma 3等模型进行多语言审计，聚焦印度、东亚和东南亚地区。通过分析模型的内部表示（log-probs/logits），将模型的观点分布与真实公众态度进行对比。同时测试了人口统计特征提示和母语提示等轻量级干预措施的效果。

Result: 研究发现：1）流行模型在广泛社会议题上能与公众意见对齐，但在宗教观点上持续失败；2）对少数群体的宗教观点表达尤其不准确，经常放大负面刻板印象；3）轻量级干预措施能部分缓解但无法消除文化差距；4）在CrowS-Pairs、IndiBias等偏见基准测试中，模型在敏感情境下仍表现出持续伤害和代表性不足问题。

Conclusion: 研究强调了系统化、基于区域的审计对于确保大语言模型在全球范围内公平部署的紧迫性。当前模型在文化对齐方面存在显著缺陷，特别是在宗教等敏感领域，需要更深入的文化理解和本地化训练来改善这一状况。
...
### [160] [The Art That Poses Back: Assessing AI Pastiches after Contemporary Artworks](https://arxiv.org/abs/2603.06324)
*Anca Dinu,Andreiana Mihail,Andra-Maria Florescu,Claudiu Creanga*

Main category: cs.CL

TL;DR: 研究探索ChatGPT生成模仿原创艺术品的AI图像能力，通过12位艺术家评估发现AI模仿作品在色彩纹理相似度与构图概念感知相似度存在显著差距，建议使用多维度"风格转换仪表板"而非单一指标评估相似性。


<details>
  <summary>Details</summary>
Motivation: 探索人工智能在视觉创意领域的潜力，特别是ChatGPT能否有意识地模仿原创艺术品（绘画、素描、雕塑、装置等）生成新的图像，并评估AI生成作品与原始作品之间的相似性和艺术价值。

Method: 邀请12位来自罗马尼亚、保加利亚、法国、奥地利和英国的艺术家，每人提供三件原创作品；使用ChatGPT生成这些作品的模仿版本；结合人类评估（艺术家评分和评论）与计算方法（检测视觉和风格相似性或差异）进行综合分析。

Result: 研究发现色彩和纹理相似度与构图、概念和感知相似度之间存在显著差距；艺术家评论显示ChatGPT模仿当代艺术作品存在局限性，被认为缺乏维度感、背景和意图感，更像是近似引用而非能唤起情感的有价值艺术品。

Conclusion: 建议使用包含互补指标的"风格转换仪表板"来评估模仿作品与原始作品的相似性，而非依赖单一风格指标；AI在艺术模仿方面仍有局限性，特别是在传达作品的多维度、背景和意图方面。
...
### [161] [Transparent AI for Mathematics: Transformer-Based Large Language Models for Mathematical Entity Relationship Extraction with XAI](https://arxiv.org/abs/2603.06348)
*Tanjim Taharat Aurpa*

Main category: cs.CL

TL;DR: 该研究将数学问题理解转化为数学实体关系抽取任务，使用Transformer模型自动提取数学文本中的操作数和运算符关系，BERT模型达到99.39%准确率，并结合SHAP方法增强模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 数学文本理解具有挑战性，因为包含专业实体和复杂关系。需要自动提取数学问题中的实体关系，以支持自动化问题解决、知识图谱构建和教育系统应用。

Method: 将数学问题解释定义为数学实体关系抽取任务，将操作数视为实体，运算符视为关系。应用基于Transformer的模型自动提取这些关系，并整合SHAP方法进行可解释性分析。

Result: BERT模型表现最佳，达到99.39%的准确率。可解释性分析揭示了特定文本和数学特征如何影响关系预测，提供了特征重要性和模型行为的洞察。

Conclusion: 通过结合Transformer学习、任务特定数据集和可解释建模，该研究为数学实体关系抽取提供了有效且可解释的框架，支持自动化问题解决、知识图谱构建和智能教育系统的未来应用。
...
### [162] [Evaluation of Deontic Conditional Reasoning in Large Language Models: The Case of Wason's Selection Task](https://arxiv.org/abs/2603.06416)
*Hirohiko Abe,Kentaro Ozeki,Risako Ando,Takanobu Morishita,Koji Mineshima,Mitsuhiro Okada*

Main category: cs.CL

TL;DR: LLMs在规范性条件推理中表现更好，显示出类似人类的匹配偏差错误模式


<details>
  <summary>Details</summary>
Motivation: 虽然LLMs的语言能力在提升，但其推理能力特别是领域特异性推理尚未充分研究。人类推理在规范性情境中表现更好，本研究旨在探索LLMs在规范性条件推理中的表现及其错误模式。

Method: 引入新的Wason选择任务数据集，明确编码道义模态以区分规范性条件句和描述性条件句，系统检验LLMs在规范性规则下的条件推理能力，分析错误模式是确认偏差还是匹配偏差。

Result: 与人类类似，LLMs在规范性规则下推理表现更好，显示出匹配偏差类似的错误模式（倾向于忽略否定并选择与规则词汇匹配的项目）。

Conclusion: LLMs的表现随规则类型系统性变化，其错误模式可以模拟该范式中已知的人类认知偏差，表明LLMs的推理能力具有领域特异性。
...
### [163] [From Prompting to Preference Optimization: A Comparative Study of LLM-based Automated Essay Scoring](https://arxiv.org/abs/2603.06424)
*Minh Hoang Nguyen,Vu Hoang Pham,Xuan Thanh Huynh,Phuc Hong Mai,Vinh The Nguyen,Quang Nhut Huynh,Huy Tien Nguyen,Tung Le*

Main category: cs.CL

TL;DR: 本研究在IELTS写作任务2上系统比较了四种LLM驱动的自动作文评分方法，发现结合监督微调和检索增强生成的方法效果最佳，F1分数达93%，揭示了不同方法在准确性、成本和鲁棒性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型已重塑自动作文评分领域，但先前研究通常孤立地考察单项技术，限制了人们对这些方法在英语作为第二语言写作评估中相对优势的理解。本研究旨在填补这一空白。

Method: 在统一的IELTS写作任务2基准上，评估了四种主要方法：(1)基于编码器的分类微调；(2)零样本和少样本提示；(3)指令调优和检索增强生成；(4)监督微调结合直接偏好优化和检索增强生成。

Result: 研究揭示了不同方法在准确性、成本和鲁棒性之间的明确权衡。最佳配置（结合监督微调和检索增强生成）取得了最强的整体结果，F1分数达到93%。

Conclusion: 本研究首次对现代LLM驱动的英语作为第二语言自动作文评分策略进行了统一的实证比较，展示了这些方法在自动评分写作任务中的潜力。
...
### [164] [Abductive Reasoning with Syllogistic Forms in Large Language Models](https://arxiv.org/abs/2603.06428)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究表明大语言模型与人类存在相似的推理偏见，但批评大语言模型可能不公平，因为人类推理不仅涉及形式演绎，还包括从有限信息中得出试探性结论的溯因推理。需要探究大语言模型在溯因推理中的表现。

Method: 将三段论数据集转换为适合溯因推理的形式，用于评估最先进的大语言模型在溯因推理任务中的准确性和偏见。

Result: 研究发现大语言模型在溯因推理中表现出特定的偏见模式，揭示了其在形式演绎之外的情境化推理方面的局限性。

Conclusion: 大语言模型在溯因推理中存在需要改进的领域，强调超越形式演绎的情境化推理的重要性，这对弥合机器与人类认知差距、推进复杂推理任务应用至关重要。
...
### [165] [PONTE: Personalized Orchestration for Natural Language Trustworthy Explanations](https://arxiv.org/abs/2603.06485)
*Vittoria Vineis,Matteo Silvestri,Lorenzo Antonelli,Filippo Betello,Gabriele Tolomei*

Main category: cs.CL

TL;DR: PONTE是一个个性化、可验证的XAI框架，通过闭环验证和适应过程生成可靠的自然语言解释，解决传统XAI方法忽视用户差异和LLM幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有XAI方法采用"一刀切"范式，忽视用户在专业知识、目标和认知需求上的差异；同时，大型语言模型虽然能将技术解释转化为自然语言，但存在忠实性和幻觉问题。

Method: 提出PONTE框架，将个性化建模为闭环验证和适应过程，包含：1) 低维偏好模型捕捉风格需求；2) 基于结构化XAI工件的偏好条件生成器；3) 强制执行数值忠实性、信息完整性和风格对齐的验证模块（可选支持检索增强论证）。通过用户反馈迭代更新偏好状态。

Result: 在医疗和金融领域的自动和人工评估表明，验证-精炼循环显著提高了完整性和风格对齐性。人工研究进一步证实：预期偏好向量与感知风格高度一致，对生成随机性具有鲁棒性，质量评估持续积极。

Conclusion: PONTE框架通过闭环验证和适应过程，实现了可靠且个性化的XAI解释生成，解决了传统方法的局限性，为不同用户提供量身定制的可信解释。
...
### [166] [Speak in Context: Multilingual ASR with Speech Context Alignment via Contrastive Learning](https://arxiv.org/abs/2603.06505)
*Yuchen Zhang,Haralambos Mouratidis,Ravi Shekhar*

Main category: cs.CL

TL;DR: 提出一种支持多语言和口音的上下文感知ASR框架，通过轻量级投影模块连接冻结的语音编码器和仅解码器语言模型，使用对比学习对齐语音和上下文表示，在11种语言和5种英语方言上实现超过5%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前ASR系统大多局限于单语言设置和短孤立话语，上下文感知ASR存在多语言支持有限以及语音与上下文表示缺乏原则性对齐的问题。
...
### [167] [KCLarity at SemEval-2026 Task 6: Encoder and Zero-Shot Approaches to Political Evasion Detection](https://arxiv.org/abs/2603.06552)
...
<details>
  <summary>Details</summary>
Motivation: 研究政治话语中的歧义和回避技术分类，探索不同建模方法在CLARITY任务中的效果，包括直接预测清晰度标签和通过任务层次结构预测回避标签两种方案。

Method: 采用两种建模方法：(1) 直接预测清晰度标签；(2) 预测回避标签并通过任务分类层次结构推导清晰度。同时探索了多种辅助训练变体，并在零样本设置下评估基于解码器的模型。

Result: 两种建模方法表现相当。在基于编码器的模型中，RoBERTa-large在公开测试集上表现最佳；而在隐藏评估集上，零样本GPT-5.2具有更好的泛化能力。

Conclusion: 直接预测清晰度标签和通过任务层次结构预测回避标签两种方法效果相当，不同模型在不同数据集上各有优势，为政治话语分析提供了有效的技术方案。
...
### [168] [RoboLayout: Differentiable 3D Scene Generation for Embodied Agents](https://arxiv.org/abs/2603.05522)
*Ali Shamsaddinlou*
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在空间推理和3D场景布局生成方面虽有进展，但生成的布局在物理受限的室内环境中难以确保语义连贯且可供具身智能体交互操作

Method: 1. 在LayoutVLM基础上增加智能体感知推理和优化稳定性改进；2. 将显式可达性约束集成到可微分布局优化过程中；3. 提出局部细化阶段，选择性重新优化有问题的物体放置；4. 支持多种具身智能体抽象（服务机器人、仓储机器人、不同年龄段人类、动物等）

Result: RoboLayout在保持LayoutVLM强语义对齐和物理合理性的同时，增强了面向智能体中心的室内场景生成适用性，实验结果表明其在多样化场景配置中表现良好

Conclusion: RoboLayout通过集成智能体感知约束和局部优化策略，有效解决了具身智能体在室内环境中的交互可行性问题，为面向特定智能体的环境设计提供了灵活框架
...
### [169] [Reasoning Models Struggle to Control their Chains of Thought](https://arxiv.org/abs/2603.05706)
*Chen Yueh-Han,Robert McCarthy,Bruce W. Lee,He He,Ian Kivlichan,Bowen Baker,Micah Carroll,Tomek Korbak*

Main category: cs.AI

TL;DR: 该研究评估了大型语言模型对思维链（CoT）的可控性，发现模型对思维链的控制能力远低于对最终输出的控制能力，且这种能力受模型规模、训练方式、问题难度等因素影响。


<details>
  <summary>Details</summary>
Motivation: 思维链监控是检测现代推理模型不当行为和理解其动机的有前景工具。但如果模型能够控制其在思维链中的表达内容，可能会破坏思维链的可监控性。研究旨在衡量这种不良能力——思维链可控性。

Method: 研究者引入了CoT-Control评估套件，包含要求模型在遵循思维链指令的同时解决问题的任务（例如在不使用"染色体"一词的情况下推理遗传学问题）。通过多种实验设置评估模型的可控性，包括不同模型规模、RL训练程度、推理计算量、问题难度等变量。

Result: 研究发现推理模型的思维链可控性显著低于输出可控性（例如Claude Sonnet 4.5的思维链可控性仅为2.7%，而最终输出可控性为61.9%）。思维链可控性随模型规模增大而提高，但随更多RL训练、测试时计算量增加和问题难度增加而降低。即使给模型提供规避监控的激励，其可控性仍然有限。

Conclusion: 研究结果谨慎乐观地表明，目前思维链可控性不太可能成为思维链可监控性的失效模式。然而，低可控性的机制尚不清楚。鉴于其对维护思维链可监控性的重要性，建议前沿实验室在未来模型中跟踪思维链可控性。
...
Method: 提出ProEvolve框架，使用类型化关系图统一表示环境的数据、工具和模式。通过图变换来表达环境能力的添加、删除和修改，这些变换能在工具、模式和数据访问之间一致地传播更新。基于此，可以编程演化动态来生成环境，并通过子图采样和编程实例化任务沙箱。

Result: 通过将单个环境演化为200个环境和3,000个任务沙箱来验证ProEvolve，并相应地基准测试代表性智能体。
...
TL;DR: 本文提出了一种新的聚合语义家族，用于定量双极论证框架，通过三个独立步骤分别处理攻击者和支持者的影响，提高了语义的可解释性和参数化能力。


<details>
  <summary>Details</summary>
Motivation: 现有定量双极论证框架中的语义在处理攻击者和支持者时通常采用对称方式，但在实际应用中攻击者和支持者可能扮演不对称的角色。需要一种更灵活、可解释性更强的语义来更好地建模这种不对称性。

Method: 提出聚合语义家族，采用三阶段计算：1) 计算攻击者的全局权重；2) 计算支持者的全局权重；3) 将这两个值与论证的内在权重进行聚合。这种方法与模块化语义不同，将攻击者和支持者分开处理。

Result: 通过讨论三个聚合函数应满足的性质及其与经典渐进语义原则的关系，展示了聚合语义的灵活性。在最终示例中测试了500种聚合语义，展示了该框架下可能的行为范围。

Conclusion: 聚合语义通过将计算分解为三个独立且可解释的步骤，保持了双极性特征，比现有文献更进一步，产生了更可理解的渐进语义，并提高了参数化能力。
...
TL;DR: 提出CliqueFlowmer模型，将基于团的模型优化方法融入Transformer和流生成中，用于计算材料发现中的目标属性优化


<details>
  <summary>Details</summary>
Motivation: 当前流行的生成建模方法由于最大似然训练的限制，在材料空间探索方面效果有限，需要一种能够直接优化目标材料属性的替代方法

Method: 基于离线模型优化(MBO)框架，结合基于团的MBO方法、Transformer和流生成技术，开发了CliqueFlowmer模型

Result: CliqueFlowmer在材料优化能力方面表现优异，生成的材料性能显著优于生成式基线方法

Conclusion: CliqueFlowmer为计算材料发现提供了有效的优化方法，代码已开源以支持跨学科研究
...
### [173] [Artificial Intelligence for Climate Adaptation: Reinforcement Learning for Climate Change-Resilient Transport](https://arxiv.org/abs/2603.06278)
...
TL;DR: 本文提出了一种基于强化学习的决策支持框架，用于城市洪水适应规划，通过结合降雨预测、洪水建模和交通模拟，在气候不确定性下制定长期基础设施投资策略。


<details>
  <summary>Details</summary>
Motivation: 气候变化预计会加剧降雨和城市内涝，导致未来几十年城市交通系统中断增加。制定有效的适应策略面临挑战，包括基础设施投资的长期性、气候不确定性以及洪水、基础设施和交通影响之间的复杂相互作用。

Method: 提出基于强化学习（RL）的决策支持框架，构建为综合评估模型（IAM），结合降雨预测和洪水建模、交通模拟，量化基础设施和交通的直接和间接影响。RL方法学习平衡投资维护成本与避免影响的适应策略。

Result: 通过哥本哈根市中心2024-2100年的案例研究，测试多种适应方案和不同气候情景。结果显示该框架优于传统优化方法，能够发现协调的空间和时间适应路径，学习影响减少与适应投资之间的权衡，产生更具韧性的策略。

Conclusion: 强化学习作为灵活决策支持工具，在气候不确定性下进行适应性基础设施规划具有巨大潜力，能够制定更有效的长期适应策略。
...
### [174] [The EpisTwin: A Knowledge Graph-Grounded Neuro-Symbolic Architecture for Personal AI](https://arxiv.org/abs/2603.06290)
...
TL;DR: EpisTwin是一个神经符号框架，通过构建可验证的个人知识图谱来解决个人AI中数据孤岛问题，结合图检索增强生成和在线深度视觉细化实现复杂推理。


<details>
  <summary>Details</summary>
Motivation: 当前个人AI面临用户数据分散在不同孤立系统（数据孤岛）的问题，传统的检索增强生成方法依赖非结构化向量相似性，无法捕捉潜在的语义拓扑和时间依赖性，限制了整体理解能力。

Method: EpisTwin使用多模态语言模型将异构的跨应用数据转换为语义三元组，构建个人知识图谱。推理时通过智能协调器结合图检索增强生成和在线深度视觉细化，在原始视觉上下文中动态重新锚定符号实体。

Result: 开发了PersonalQA-71-100合成基准来模拟真实用户的数字足迹并评估EpisTwin性能。在一系列最先进的评判模型上展示了稳健的结果。

Conclusion: EpisTwin为可信赖的个人AI提供了一个有前景的方向，通过神经符号方法在可验证的个人知识图谱基础上实现生成式推理。
...
Method: 引入SAHOO框架，包含三个安全措施：(1) 目标漂移指数(GDI)，结合语义、词汇、结构和分布度量的多信号检测器；(2) 约束保持检查，强制执行安全关键不变性如语法正确性和非幻觉；(3) 回归风险量化，标记可能撤销先前收益的改进周期。

Result: 在189个代码生成、数学推理和真实性任务中，SAHOO实现显著质量提升：代码任务提升18.3%，推理任务提升16.8%，同时在两个领域保持约束，在真实性任务中保持低违规率。阈值在18个任务的验证集上校准。

Conclusion: SAHOO使递归自我改进期间的对齐保持变得可测量、可部署和可系统验证，绘制了能力-对齐前沿，显示早期改进周期高效但后期对齐成本上升，揭示了领域特定张力如流畅性与事实性之间的权衡。
...
TL;DR: 该论文提出了一种schema-gated orchestration方法，通过模式验证作为执行边界，解决LLM在科学工作流中确定性与灵活性之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可以将自然语言目标转化为可执行计算，但科学工作流需要确定性、可追溯性和治理保障，而LLM自主决定执行内容难以满足这些要求。研究发现工业研发中存在确定性与灵活性两个竞争需求。

Method: 提出schema-gated orchestration原则：模式作为组合工作流级别的强制执行边界，确保所有操作（包括跨步骤依赖）都通过机器可检查的规范验证。将需求操作化为执行确定性（ED）和对话灵活性（CF），使用多模型协议评估20个系统。

Result: 多模型LLM评分显示出高度一致性（ED的α=0.80，CF的α=0.98），表明可作为专家评估的可重用替代方案。现有系统存在帕累托前沿，没有系统同时实现高灵活性和高确定性，但在生成式和流程中心化极端之间存在收敛区域。

Conclusion: schema-gated架构通过分离对话和执行权限，能够解耦确定性与灵活性的权衡。提出三个操作原则：执行前澄清、受限的计划-执行编排、工具到工作流级别的门控，以指导实际应用。
...
### [177] [Clinical-Injection Transformer with Domain-Adapted MAE for Lupus Nephritis Prognosis Prediction](https://arxiv.org/abs/2603.05535)
*Yuewen Huang,Zhitao Ye,Guangnan Feng,Fudan Zheng,Xia Gao,Yutong Lu*

Main category: eess.IV

TL;DR: 首个针对儿童狼疮性肾炎的多模态计算病理学框架，仅使用常规PAS染色活检和临床数据，实现三分类治疗反应预测，准确率达90.1%


<details>
  <summary>Details</summary>
Motivation: 儿童狼疮性肾炎比成人更严重且预后更差，但现有计算病理学方法未探索儿童LN预后预测，且依赖多种昂贵染色协议，未能整合临床数据

Method: 提出多模态计算病理学框架，包含：1) 临床注入Transformer将临床特征作为条件令牌嵌入到patch级自注意力中；2) 解耦表示-知识适应策略，使用域适应掩码自编码器分离形态特征学习和病理知识提取；3) 多粒度形态类型注入机制，在实例和患者层面桥接分类知识与预后预测

Result: 在71名儿童LN患者的KDIGO标准化标签队列中，三分类准确率达到90.1%，AUC为89.4%

Conclusion: 该方法展示了作为高精度、经济有效的儿童狼疮性肾炎预后预测工具的潜力，仅需常规PAS染色活检和临床数据
...
### [178] [Longitudinal Lesion Inpainting in Brain MRI via 3D Region Aware Diffusion](https://arxiv.org/abs/2603.05693)
*Zahra Karimaghaloo,Dumitru Fetco,Haz-Edine Assemlal,Hassan Rivaz,Douglas L. Arnold*

Main category: eess.IV

TL;DR: 提出基于DDPM的伪3D纵向修复框架，用于处理脑MRI中的病变区域，通过多通道条件和区域感知扩散实现高效、稳定的纵向图像修复。


<details>
  <summary>Details</summary>
Motivation: 脑MRI纵向分析常受病变演变影响，现有深度生成模型多为横断面方法或缺乏3D解剖连续性，需要更准确的纵向修复方法。

Method: 基于去噪扩散概率模型(DDPM)的伪3D纵向修复框架，采用多通道条件整合不同时间点的纵向信息，并将区域感知扩散(RAD)扩展到医学领域，专注于病理区域修复而不改变周围健康组织。

Result: 在93名患者的纵向脑MRI上评估，模型显著优于FastSurfer-LIT基线：LPIPS距离从0.07降至0.03，时间保真度指数从1.22改善至1.024（理想值为1.0），处理速度提升约10倍（2.53分钟 vs 24.30分钟）。

Conclusion: 通过利用纵向先验和区域特定去噪，该框架为进行性神经退行性疾病研究提供了高度可靠且高效的预处理步骤。
...
### [179] [Interpretable Motion Artificat Detection in structural Brain MRI](https://arxiv.org/abs/2603.05726)
*Naveetha Nithianandam,Prabhjot Kaur,Anil Kumar Sao*

Main category: eess.IV

TL;DR: 提出了一种轻量级可解释的T1加权脑MRI运动伪影检测框架，通过扩展判别性梯度幅度直方图到三维空间，结合2D和3D特征，仅需209个可训练参数，在跨站点评估中表现出色。


<details>
  <summary>Details</summary>
Motivation: 脑结构MRI的自动质量评估对可靠的神经影像分析至关重要，但现有方法存在预处理复杂、计算成本高、泛化能力差等问题，特别是难以处理运动伪影和跨采集站点的泛化挑战。

Method: 将判别性梯度幅度直方图扩展到三维空间，通过并行决策策略整合互补的切片级（2D）和体积级（3D）DHoGM特征，使用重叠3D立方体进行体积分析，采用简单的基于阈值的分类器和低参数多层感知器。

Result: 在MR-ART和ABIDE数据集上评估，在域内评估达到94.34%准确率，在未见站点上达到89%准确率，几乎完全避免了低质量扫描的误接受，消融研究证实了2D和3D特征的互补优势。

Conclusion: 该方法为自动MRI质量检查提供了有效、高效且鲁棒的解决方案，具有集成到大规模临床和研究工作流程的强大潜力。
...
### [180] [Uni-LVC: A Unified Method for Intra- and Inter-Mode Learned Video Compression](https://arxiv.org/abs/2603.05756)
*Yichi Zhang,Ruoyu Yang,Fengqing Zhu*

Main category: eess.IV

TL;DR: Uni-LVC是一种统一的视频压缩方法，在单个模型中同时支持帧内和帧间编码，解决了现有学习型视频压缩需要分离模型和参考帧不可靠时性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有学习型视频压缩方法存在两个主要限制：1) 需要为帧内和帧间编码模式分别训练模型；2) 当时间参考不可靠时性能会下降。需要一种统一的解决方案。

Method: 基于强大的帧内编解码器，将帧间编码构建为基于参考帧提取的时间信息进行条件化的帧内编码。设计了高效的交叉注意力适配模块来整合时间线索，支持单向和双向预测模式。提出可靠性感知分类器来选择性缩放时间线索，当参考不可靠时让模型更接近帧内编码。采用多阶段训练策略来适应各种编码模式。

Result: 实验表明Uni-LVC在帧内和帧间配置下都实现了优越的率失真性能，同时保持了可比较的计算效率。

Conclusion: Uni-LVC通过统一的模型架构和创新的时间信息整合机制，成功解决了现有学习型视频压缩方法的局限性，为视频压缩提供了更灵活高效的解决方案。
...
### [181] [AI End-to-End Radiation Treatment Planning Under One Second](https://arxiv.org/abs/2603.06338)
...
<details>
  <summary>Details</summary>
Motivation: 现有AI放疗计划方法依赖多次剂量评估和修正，生成时间需要几分钟。需要开发更快速、减少规划师间差异、提高临床工作流程效率的解决方案。

Method: 提出AIRT端到端深度学习框架，包含可微分剂量反馈、对抗性通量图塑形和计划生成增强技术，在单个Nvidia A100 GPU上运行，训练数据超过10,000例前列腺病例。

Result: AIRT在1秒内生成单弧VMAT前列腺计划，在靶区覆盖和危及器官保护指标上不劣于RapidPlan Eclipse，靶区均匀性(HI=0.10±0.01)和OAR保护与参考计划相似。

Conclusion: AIRT代表了向超快速标准化放疗规划和简化临床工作流程迈出的重要一步，具有显著的时间效率和一致性优势。
...
### [182] [Omni-C: Compressing Heterogeneous Modalities into a Single Dense Encoder](https://arxiv.org/abs/2603.05528)
*Kin Wai Lau,Yasar Abbas Ur Rehman,Lai-Man Po,Pedro Porto Buarque de Gusmão*

Main category: cs.MM

TL;DR: Omni-C是一个统一的密集Transformer编码器，通过单模态对比预训练学习跨图像、音频和文本的共享表示，避免了多专家模型参数膨胀和路由开销，实现了高效的多模态学习。


<details>
  <summary>Details</summary>
Motivation: 现有多模态系统依赖分离的专家模态编码器，导致模态增加时计算复杂度线性增长；而统一的全模态模型虽然使用MoE架构，但仍存在参数膨胀和路由开销问题。需要一种更高效的多模态表示学习方法。

Method: 提出Omni-C单一密集Transformer编码器，通过在大规模未对齐数据上进行单模态对比预训练，学习跨异构模态的共享表示。采用轻量级模态特定投影头，最大化主干参数共享，避免MoE、配对监督或路由机制。

Result: Omni-C在单模态和跨模态任务上达到与专家模型相当的性能，音频和文本的零样本性能有轻微下降但可通过轻量级线性探测或参数高效微调恢复。相比多编码器基线，显著降低了推理内存使用。

Conclusion: Omni-C通过统一的密集编码器架构实现了高效可扩展的多模态学习，避免了专家模型的复杂性，支持内存受限系统的顺序模态处理和低内存推理，为多模态系统部署提供了实用解决方案。
