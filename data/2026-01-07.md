...
Motivation: 当前3D网格显著性标注方法大多沿用2D图像方法，忽略了3D几何拓扑与2D图像阵列的根本差异。现有的VR眼动追踪流程依赖单射线采样和欧几里得平滑，容易引发纹理注意力和跨间隙信号泄漏问题，需要更符合人类自然感知的高保真3D注意力获取方法。

Method: 提出一个鲁棒框架：1) 视锥采样策略，通过高斯分布的射线束模拟人类中央凹感受野，提高对复杂拓扑的采样鲁棒性；2) 混合流形-欧几里得约束扩散算法，融合流形测地约束与欧几里得尺度，确保拓扑一致的显著性传播。

Result: 该框架通过缓解"拓扑短路"和混叠问题，提供了一个与自然人类感知一致的高保真3D注意力获取范式，为3D网格显著性研究提供了更准确和鲁棒的基准。

Conclusion: 提出的框架解决了现有3D网格显著性标注方法的局限性，通过视锥采样和混合约束扩散算法，实现了更符合人类感知的3D注意力获取，为VR中的人为中心视觉建模提供了更可靠的标注基础。
...
TL;DR: 提出EGxFSL和xGAL双框架，解决医学图像分析中标注数据稀缺和模型可解释性不足的问题，通过放射科医生指导的注意力机制和可解释性引导的主动学习，在多个医学影像数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析面临两个关键挑战：标注数据稀缺和模型可解释性不足，这阻碍了临床AI的部署。少样本学习可以解决数据限制但缺乏预测透明度，主动学习方法优化数据采集但忽视了采集样本的可解释性。

Method: 提出双框架解决方案：1) EGxFSL：通过Grad-CAM-based Dice损失整合放射科医生定义的兴趣区域作为空间监督，与原型分类联合优化实现可解释的少样本学习；2) xGAL：引入迭代样本采集，优先考虑预测不确定性和注意力错位，创建可解释性指导训练和样本选择的闭环框架。

Result: 在BraTS（MRI）、VinDr-CXR（胸部X光）和SIIM-COVID-19（胸部X光）数据集上分别达到92%、76%和62%的准确率，在所有数据集上始终优于非指导基线。在严重数据限制下，xGAL仅用680个样本就达到76%准确率，而随机采样仅为57%。Grad-CAM可视化显示指导模型专注于诊断相关区域，在乳腺超声上的泛化验证了跨模态适用性。

Conclusion: 提出的EGxFSL和xGAL框架有效解决了医学图像分析中的数据稀缺和可解释性问题，通过专家指导和可解释性引导的主动学习，在多个医学影像任务上表现出优越性能，为临床AI部署提供了实用解决方案。
...
TL;DR: NitroGen是一个基于40,000小时游戏视频训练的视觉-动作基础模型，能够在1000多款游戏中实现跨游戏泛化，在未见过的游戏中任务成功率相比从头训练模型提升高达52%。


<details>
  <summary>Details</summary>
Motivation: 当前游戏AI通常针对单一游戏专门训练，缺乏跨游戏泛化能力。研究人员希望开发一个通用的游戏智能体，能够理解和执行多种不同类型游戏中的任务。

Method: 1) 从公开游戏视频自动提取玩家动作构建互联网规模视频-动作数据集；2) 创建多游戏基准环境评估跨游戏泛化能力；3) 通过大规模行为克隆训练统一的视觉-动作模型。

Result: NitroGen在3D动作游戏战斗、2D平台游戏高精度控制和程序生成世界探索等多样化领域表现出强大能力，在未见过的游戏中任务成功率相比从头训练模型提升高达52%。

Conclusion: NitroGen展示了大规模视频-动作训练在构建通用游戏智能体方面的有效性，为通用具身智能体研究提供了数据集、评估套件和模型权重等资源。
...
TL;DR: 该研究评估了多模态大语言模型在膝关节骨关节炎X光片分类任务中的表现，发现纯视觉编码器优于完整MLLM流程，LLM微调无显著提升，小规模平衡数据集比大规模不平衡数据集效果更好。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在医学视觉问答和报告生成方面表现良好，但其生成和解释能力并不能可靠地转移到疾病特异性分类任务上。膝关节骨关节炎影响全球3-4亿人，但在现有医学MLLM基准测试中代表性不足，因此需要评估MLLM在此特定分类任务中的适用性。

Method: 通过系统消融研究，操纵视觉编码器、连接器和大型语言模型，采用不同的训练策略，测量每个组件对诊断准确性的贡献。比较了纯视觉编码器与完整MLLM流程的性能，评估了提示引导与LLM微调的效果，并对比了小型平衡数据集（500张图像）与大型不平衡数据集（5,778张图像）的训练结果。

Result: 在分类任务中，训练好的视觉编码器单独使用就能在分类准确率上超越完整的MLLM流程；对LLM进行微调相比基于提示的指导没有提供有意义的改进；在小型平衡数据集上进行LoRA微调比在大型但不平衡的数据集上训练效果更好，表明数据平衡和质量比原始规模更重要。

Conclusion: 对于领域特定的医学分类任务，LLM更适合作为解释器和报告生成器，而不是作为主要分类器。MLLM架构不太适合需要高确定性的医学图像诊断分类任务。建议在开发临床适用系统时，优先考虑视觉编码器优化和仔细的数据集策划。
...
<details>
  <summary>Details</summary>
Motivation: 尽管文本推理在盲图像质量评估中广泛应用，但文本信息如何贡献于质量预测以及文本能在多大程度上代表与分数相关的图像内容仍不明确。本研究旨在从信息流角度解决这些问题。

Method: 通过比较现有BIQA模型与三种设计用于学习图像-文本-分数关系的范式：思维链、自一致性和自编码器，从信息流角度分析文本推理的作用。

Result: 实验表明：1) 现有模型仅使用文本信息时预测性能显著下降；2) 思维链范式对BIQA性能提升有限；3) 自一致性范式显著缩小了图像与文本条件预测之间的差距，将PLCC/SRCC差异缩小到0.02/0.03；4) 自编码器范式在缩小图像-文本差距方面效果较差，但指出了进一步优化的方向。
...
TL;DR: 提出一种直接从点云生成语言对齐的patch级特征的3D编码器，实现零样本3D部件分割，无需多视图渲染


<details>
  <summary>Details</summary>
Motivation: 现有3D基础模型在全局任务上表现良好，但在局部部件级推理上迁移效果差。基于视觉和语言模型的现有方法需要昂贵的多视图渲染推理，依赖LLM提示工程，且未能充分利用3D几何信息

Method: 引入仅编码器的3D模型，通过两阶段预训练：1) 从视觉编码器（如DINOv2）蒸馏密集2D特征到3D patches；2) 通过多正对比目标将这些patch嵌入与部件级文本嵌入对齐

Result: 3D编码器实现了零样本3D部件分割，具有快速单次推理能力，无需测试时多视图渲染，在多个3D部件分割基准上显著优于之前的渲染方法和前馈方法

Conclusion: 该方法填补了3D基础模型在局部部件级推理方面的空白，通过直接从点云生成语言对齐的特征，实现了高效且性能优越的3D部件分割
...
TL;DR: 研究人员创建了MovieRecapsQA基准测试，这是一个基于电影解说视频的开放式多模态视频问答基准，包含约8.2K个问题-答案对，用于评估模型在视觉和文本模态上的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有VideoQA基准测试难以捕捉真实电影理解所需的多模态推理能力，且大多不是开放式的，因为自由形式答案难以评估。需要创建能够评估模型整合视觉和对话线索能力的基准。

Method: 利用YouTube电影解说视频创建基准，这些视频通过同步的视觉（解说视频）和文本（解说摘要）模态总结电影关键事件。使用解说摘要生成约8.2K个与电影字幕对齐的QA对，并提供验证答案所需的"事实"信息。

Result: 评估了7个最先进的多模态大语言模型，发现：1)仅视觉问题最具挑战性；2)模型倾向于优先使用文本输入；3)从视频内容提取准确事实信息对所有模型都困难；4)专有和开源模型在视频依赖问题上表现相当。

Conclusion: MovieRecapsQA是首个提供输入显式文本上下文的开放式VideoQA基准，支持细粒度分析。基准揭示了当前模型在多模态视频理解方面的局限性，特别是视觉推理能力不足。
...
TL;DR: DreamLoop是一个从单张照片生成可控动态照片（cinemagraph）的框架，无需专门的动态照片训练数据，通过视频扩散模型实现灵活控制


<details>
  <summary>Details</summary>
Motivation: 现有图像动画技术局限于简单低频运动，只能处理重复纹理场景；大型视频扩散模型不适合动态照片约束且缺乏专门数据。需要一种能从单张照片生成可控动态照片的方法

Method: 通过训练视频扩散模型实现两个目标：时间桥接和运动条件。推理时使用输入图像作为首尾帧条件实现无缝循环，通过静态轨迹保持背景静止，用户指定运动路径控制动画轨迹和时序

Result: DreamLoop能够生成高质量、复杂的动态照片，符合用户意图，优于现有方法，是首个支持通用场景动态照片生成且具有灵活直观控制的方法

Conclusion: DreamLoop框架成功解决了从单张照片生成可控动态照片的挑战，无需专门训练数据，通过创新的训练策略实现了灵活的用户控制
...
TL;DR: 提出基于绿色通道移除重建误差（GRRE）的新方法，用于检测AI生成图像，通过比较真实图像和AI生成图像在移除绿色通道后重建误差的差异来实现高精度检测，具有良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型（特别是扩散模型和GANs）的快速发展，区分合成图像和真实图像变得越来越困难。现有的检测方法在面对新型或未见过的生成模型时，准确率往往会下降，这凸显了实现强泛化能力的挑战。

Method: 提出基于通道移除重建的检测范式。研究发现，当从真实图像中移除绿色（G）通道并进行重建时，产生的重建误差与AI生成图像的重建误差存在显著差异。基于这一发现，提出了G通道移除重建误差（GRRE）方法，利用这种差异进行鲁棒的AI生成图像检测。

Result: 大量实验表明，GRRE在多个生成模型（包括训练期间未见过的模型）上都能保持高检测准确率。与现有方法相比，GRRE不仅对各种扰动和后处理操作保持强鲁棒性，还表现出优越的跨模型泛化能力。

Conclusion: 基于通道移除重建的方法有潜力成为强大的取证工具，在生成式AI时代保护图像的真实性。GRRE方法简单而有效，为解决AI生成图像检测的泛化问题提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法需要特定类别的参数化模板，存在姿态歧义和形状多样性问题，限制了从2D视频到3D资产的运动转移应用

Method: 采用形态参数化的关节3D高斯溅射模型结合密集语义对应关系，通过优化联合调整形状和姿态，有效缓解形状-姿态歧义

Result: 实验结果显示在运动准确性、效率和视觉连贯性方面优于现有方法，显著推进了多样化物体类别和日常视频场景中的运动转移

Conclusion: CAMO框架通过形态参数化的3D高斯溅射和语义对应，实现了类别无关的高质量运动转移，解决了传统方法的局限性
...
TL;DR: 提出了一种前景感知的数据集蒸馏方法，通过内容自适应的动态补丁选择策略，在保持主要对象关键信息的同时减少冗余背景内容。


<details>
  <summary>Details</summary>
Motivation: 传统优化方法计算开销大、内存受限且生成不现实的噪声图像，而非优化方法虽然使用真实图像补丁但采用僵化的补丁选择策略，可能丢弃主要对象的关键信息。

Method: 利用Grounded SAM2识别前景对象并计算每张图像的前景占用率，推导类别级补丁决策阈值，设计动态补丁选择策略，根据阈值选择最信息丰富的补丁或直接调整完整图像大小。

Result: 在多个基准测试上的实验表明，该方法相比现有方法持续提升蒸馏性能，生成更具信息性和代表性的蒸馏数据集，并增强了对不同架构和图像构成的鲁棒性。

Conclusion: 提出的前景感知数据集蒸馏方法通过动态补丁选择策略有效解决了传统方法的局限性，在保持主要对象关键信息的同时提高了蒸馏效率和性能。
...
TL;DR: 该论文发现当前多模态大语言模型在功能成像（特别是PET扫描）中存在功能性感知缺陷，提出了PET-Bench基准测试，揭示了思维链提示法的幻觉陷阱，并提出原子视觉对齐方法来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在解剖模态的异常检测和报告生成方面表现出色，但它们在功能成像（特别是PET扫描）中的能力尚未得到充分探索。研究者发现当前视觉编码器无法独立于形态学先验解码功能性示踪剂生物分布，存在功能性感知缺陷。

Method: 1. 构建PET-Bench基准：包含52,308个层次化QA对，来自9,732个多中心、多示踪剂PET研究；2. 评估19个最先进的多模态大语言模型；3. 提出原子视觉对齐方法：通过简单微调策略，强制模型在高级诊断推理前掌握低级别功能性感知。

Result: 1. 发现思维链提示法在PET中产生"幻觉陷阱"，导致语言生成与视觉证据脱节，产生临床流畅但事实无根据的诊断；2. 原子视觉对齐方法有效弥合感知缺陷，将思维链从幻觉来源转变为稳健推理工具；3. 诊断准确率提升高达14.83%。

Conclusion: 该研究揭示了多模态大语言模型在功能成像中的功能性感知缺陷，提出的原子视觉对齐方法能够有效解决思维链提示法的幻觉问题，显著提升PET图像诊断的准确性和可靠性。
...
Conclusion: 通过双域密度细化方法，D³R-DETR有效解决了遥感图像中微小目标检测的挑战，在精度和性能上取得了显著提升
...
TL;DR: BUFFER-X是一个无需训练的零样本点云配准框架，通过几何引导的超参数估计、分布感知采样和坐标归一化解决跨域泛化问题，并在12个数据集上验证了有效性。
...
Method: 提出BUFFER-X框架：1) 几何引导实现超参数自动估计；2) 分布感知最远点采样替代学习检测器；3) 块级坐标归一化确保尺度一致性。采用分层多尺度匹配提取局部、中层和全局对应关系。还提出BUFFER-X-Lite版本，通过早期退出策略和快速位姿求解器减少43%计算时间。

Result: 在包含12个数据集的综合基准测试中，涵盖物体尺度、室内和室外场景，包括异构LiDAR配置的跨传感器配准。结果表明该方法无需手动调参或测试域先验知识即可有效泛化。

Conclusion: BUFFER-X通过解决现有方法的三个关键限制，实现了无需训练的零样本点云配准泛化能力，为不同环境和传感器配置提供了鲁棒的配准解决方案。
...
TL;DR: 提出了一种轻量级、数据中心的零样本单目深度估计框架，使用DINOv3作为视觉编码器，设计Simple Depth Transformer作为紧凑解码器，并采用基于质量的过滤策略优化数据集。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法依赖大规模数据集和复杂解码器，限制了效率和泛化能力。需要更轻量、数据中心的框架来实现高效的零样本深度估计。

Method: 1) 采用DINOv3作为视觉编码器获取高质量密集特征；2) 设计Simple Depth Transformer作为紧凑的基于transformer的解码器，使用单路径特征融合和上采样减少计算开销；3) 提出基于质量的过滤策略筛选有害样本，减少数据集规模同时提升训练质量。

Result: 在五个基准测试上的广泛实验表明，该框架在精度上超越了DPT。SDT相比DPT减少了约85%-89%的参数，同时实现了更高的精度。代码和网站已公开。

Conclusion: 这项工作强调了平衡模型设计和数据质量对于实现高效、可泛化的零样本深度估计的重要性。提出的框架在保持高精度的同时显著减少了模型复杂度。
...
TL;DR: ClearAIR是一个受人类视觉感知启发的All-in-One图像修复框架，采用分层粗到细策略，结合MLLM图像质量评估、区域感知任务识别和内部线索重用机制，在合成和真实数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有All-in-One图像修复方法过度依赖退化特定表示，容易导致过度平滑和伪影。需要更准确地表征复杂复合退化，并实现更精细的局部修复。

Method: 1. 基于MLLM的图像质量评估：利用跨模态理解准确评估复杂退化；2. 区域感知和任务识别：语义交叉注意力生成粗语义提示，退化感知模块捕获区域特定退化特征；3. 内部线索重用机制：自监督挖掘图像内在信息增强细节修复。

Result: 实验结果表明，ClearAIR在多样化的合成和真实世界数据集上取得了优越的性能表现。

Conclusion: ClearAIR通过人类视觉感知启发的分层粗到细策略，有效解决了现有All-in-One图像修复方法的局限性，实现了更准确和精细的图像修复。
...
TL;DR: 提出AbductiveMLLM模型，通过模仿人类认知中的言语和图像双重推理模式来增强多模态大语言模型的溯因推理能力，在视觉溯因推理任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前的多模态大语言模型虽然具备强大的通用多模态推理能力，但在溯因推理方面仍不及人类水平。受到人类认知中言语和图像双重推理模式相互作用的启发，希望通过模仿这种双模式行为来增强MLLMs的溯因推理能力。

Method: 提出AbductiveMLLM模型，包含两个协同组件：REASONER（言语推理器）和IMAGINER（图像想象器）。REASONER在言语域工作，首先使用盲LLM探索广泛的可能解释空间，然后基于跨模态因果对齐修剪视觉不一致的假设，将剩余假设作为目标先验引入MLLM。IMAGINER通过文本到图像扩散模型，基于输入视频和REASONER的输出嵌入"想象"与言语解释对应的视觉场景，丰富MLLM的上下文基础。两个组件以端到端方式联合训练。

Result: 在标准视觉溯因推理基准测试中，AbductiveMLLM实现了最先进的性能，一致优于传统解决方案和先进的多模态大语言模型。

Conclusion: 通过模仿人类认知中的言语和图像双重推理模式，可以有效增强多模态大语言模型的溯因推理能力，在视觉溯因推理任务上取得了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频风格化方法大多局限于单一类型的风格条件（文本、风格图像或风格化首帧），限制了应用范围。同时缺乏高质量数据集导致风格不一致和时间闪烁问题。

Method: 基于普通图像到视频（I2V）模型构建，使用低秩适应（LoRA）配合特定token的上矩阵进行训练，减少不同条件token之间的混淆。设计了高质量配对视频数据收集流程。

Result: 定性和定量评估表明，DreamStyle在三种视频风格化任务中表现优异，在风格一致性和视频质量方面优于竞争对手。

Conclusion: DreamStyle提供了一个统一的视频风格化框架，支持多种风格条件，通过高质量数据收集和特定token训练策略，有效解决了现有方法的局限性。
...
Conclusion: StableDPT方法成功地将图像深度估计模型适配到视频处理，通过时间模块和高效推理策略实现了更准确、时间稳定的深度预测
...
TL;DR: 本文提出了一种拓扑感知的H&E到IHC虚拟染色框架，通过拓扑一致性匹配和拓扑约束病理匹配机制，解决相邻切片数据空间错位问题，实现高质量虚拟染色。
...
Method: 提出拓扑感知的H&E到IHC虚拟染色框架，包含两个核心机制：1) 拓扑感知一致性匹配(TACM)，采用图对比学习和拓扑扰动来学习鲁棒的匹配模式，确保结构一致性；2) 拓扑约束病理匹配(TCPM)，基于节点重要性对齐病理阳性区域，增强病理一致性。
...
TL;DR: SketchThinker-R1通过模仿人类草图式思维，在保持准确性的同时将推理token成本降低64%以上


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型的逐步推理过程计算开销大（token成本高、响应时间长），而人类采用草图式思维能更高效地解决问题，因此需要开发类似的高效推理方法

Method: 三阶段方法：1) 草图模式冷启动：将标准长推理转换为草图式推理并微调基础模型；2) 训练SketchJudge奖励模型：评估思维过程，给草图式推理更高评分；3) 草图思维强化学习：在SketchJudge监督下进一步泛化草图式推理能力

Result: 在四个基准测试中，SketchThinker-R1在保持最终答案准确性的同时，实现了超过64%的推理token成本降低；定性分析显示草图式推理在解决问题时更关注关键线索

Conclusion: SketchThinker-R1成功地将人类草图式思维引入大型多模态模型，显著提高了推理效率，为高效多模态推理提供了新方向
...
TL;DR: DGA-Net：通过深度提示范式改进SAM模型，用于伪装目标检测，提出跨模态图增强和锚点引导细化模块，在COD任务上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有伪装目标检测方法主要依赖稀疏提示（如点或框），未能充分利用深度信息。为了在COD任务中充分挖掘深度线索，需要一种更全面的深度提示机制。

Method: 提出DGA-Net框架，采用"深度提示"范式适配SAM模型：1）跨模态图增强模块，在异质图中融合RGB语义和深度几何信息形成统一引导信号；2）锚点引导细化模块，创建全局锚点并建立非局部路径，将引导信号从深层传播到浅层，避免特征层次中的信息衰减。

Result: 定量和定性实验结果表明，DGA-Net在伪装目标检测任务上超越了现有的最先进方法。

Conclusion: 通过深度提示范式和两个关键模块的设计，DGA-Net能够有效利用深度信息，在伪装目标检测中实现精确且一致的图像分割。
...
<details>
  <summary>Details</summary>
Motivation: 红外小目标检测面临信噪比低、目标尺寸小、背景复杂等挑战。虽然基于DETR的检测器受益于全局上下文建模，但在红外小目标检测中性能显著下降。研究发现，由于自注意力机制，红外小目标的相关嵌入被主导的背景特征淹没，导致查询初始化和目标定位不可靠。
...
Conclusion: SEF-DETR通过改进查询初始化机制，有效解决了传统DETR在红外小目标检测中的局限性，显著提升了检测性能，为低信噪比、小尺寸目标的红外检测提供了有效框架。
...
TL;DR: 提出基于扩散模型的通用图像分割框架，无需依赖掩码架构，通过整体预测实现无类别分割，在离散数据设置下优化扩散模型参数


<details>
  <summary>Details</summary>
Motivation: 传统图像分割方法依赖掩码框架，缺乏对分割不确定性的建模能力。本文旨在开发一种基于扩散模型的通用分割方法，能够以整体方式预测分割结果，并具备对分割模糊性的建模能力

Method: 1) 使用位置感知调色板配合2D格雷码排序；2) 添加最终的tanh激活函数处理离散数据；3) 优化扩散参数，采用sigmoid损失加权和x-prediction策略；4) 从零开始训练模型

Result: 当前模型性能尚未超越领先的掩码架构，但显著缩小了性能差距。模型具备传统方法缺乏的独特能力，特别是对分割模糊性的建模能力。位置感知调色板和2D格雷码排序提升了性能

Conclusion: 扩散模型为通用图像分割提供了有前景的替代方案，具备建模分割模糊性的独特优势。结合大规模预训练或可提示条件调节，有望开发出具有竞争力的分割模型
...
<details>
  <summary>Details</summary>
Motivation: 现有VideoLLMs在未修剪视频中难以准确定位事件边界，导致生成的描述缺乏适当的时间基础。需要提升视频语言模型在密集视频描述中的时间感知能力。

Method: 提出TA-Prompting方法：1) 使用时间锚点学习精确事件定位；2) 提示VideoLLMs进行时间感知的视频事件理解；3) 引入事件连贯性采样策略，选择具有时间连贯性和跨模态相似性的事件描述。

Result: 在基准数据集上的广泛实验表明，TA-Prompting在密集视频描述、时刻检索和TemporalQA等时间理解任务上优于现有最先进的VideoLLMs。

Conclusion: TA-Prompting通过时间锚点和事件连贯性采样策略，有效提升了VideoLLMs在密集视频描述中的事件定位能力和描述质量，在多个时间理解任务上取得优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的IQA方法在整合视觉和文本线索方面能力有限，导致推理不可靠。需要一种能够模拟人类关键认知行为的IQA模型，以同时提供质量描述和精确分数。

Method: 提出Zoom-IQA模型，采用两阶段训练流程：1）在GR-IQA数据集上进行监督微调，使模型能够将评估结果锚定在关键区域；2）使用强化学习进行动态策略探索，通过KL-Coverage正则化器防止推理和评分多样性崩溃，并采用渐进重采样策略缓解标注偏差。

Result: Zoom-IQA在实验中表现出改进的鲁棒性、可解释性和泛化能力。在图像恢复等下游任务中的应用进一步证明了其有效性。

Conclusion: Zoom-IQA通过模拟人类认知行为，成功解决了现有VLM-based IQA方法推理不可靠的问题，实现了更可靠的图像质量评估，同时提供质量描述和精确分数。
...
TL;DR: DCG-ReID提出动态置信解耦加权机制，针对多模态车辆重识别中模态质量分布不平衡问题，分别设计协作融合和引导融合两种策略，有效解决平衡与不平衡质量分布数据的不同融合需求。
...
Method: 提出DCG-ReID方法：1）设计动态置信解耦加权机制，通过交互导出的模态置信度动态重加权三个模态的贡献；2）基于此开发两种场景特定融合策略：针对平衡质量分布的协作融合模块挖掘成对共识特征，针对不平衡分布的引导融合模块实现模态判别差异的差异化放大。

Result: 在三个多模态重识别基准数据集（WMVeID863、MSVR310、RGBNT100）上的大量实验验证了该方法的有效性。

Conclusion: DCG-ReID通过解耦异构质量分布模态数据，针对不同质量分布场景设计专门融合策略，有效提升了多模态车辆重识别的性能，解决了现有方法忽视不同数据类型的融合需求问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频异常理解方法依赖微调的多模态大语言模型或外部模块（如视频字幕生成器），导致成本高昂的标注、复杂的训练流程和高推理开销，需要更轻量高效的解决方案

Method: PrismVAU采用两阶段互补架构：1）粗粒度异常评分模块通过文本锚点相似性计算帧级异常分数；2）MLLM精炼模块通过系统和用户提示对异常进行上下文理解。文本锚点和提示通过弱监督自动提示工程框架优化

Result: 在标准VAD基准测试中，PrismVAU实现了有竞争力的检测性能并提供可解释的异常解释，同时不依赖指令微调、帧级标注、外部模块或密集处理

Conclusion: PrismVAU是一种高效实用的实时视频异常理解解决方案，通过单一现成MLLM实现异常检测和解释，适用于实际应用场景
...
TL;DR: 提出HybridSolarNet模型，结合EfficientNet-B0和CBAM注意力模块，用于无人机太阳能板故障检测，在保持轻量化的同时达到92.37%准确率。
...
Method: 提出HybridSolarNet模型，集成EfficientNet-B0和卷积块注意力模块(CBAM)。采用分割前增强协议避免准确率估计泄漏，引入焦点损失和余弦退火。在Kaggle太阳能板图像竞赛数据集上进行5折分层交叉验证。

Result: 平均准确率达到92.37% ± 0.41，F1分数0.9226 ± 0.39，仅需16.3MB存储（比VGG19小32倍）。GPU推理速度54.9 FPS，适合实时无人机应用。CBAM带来+1.53%准确率提升，焦点损失有效处理类别不平衡。

Conclusion: HybridSolarNet在保持轻量化的同时实现了高准确率，适合边缘计算设备部署，是无人机实时太阳能板故障检测的成功候选方案。Grad-CAM可视化显示模型能聚焦实际故障位置而非无关区域。
...
TL;DR: 该论文构建了首个专门针对虚拟试穿（VTON）的多维度质量评估数据集VTONQA，包含8,132张图像和24,396个主观评分，用于评估VTON模型生成图像的质量。


<details>
  <summary>Details</summary>
Motivation: 随着电子商务和数字时尚的快速发展，基于图像的虚拟试穿技术受到越来越多的关注。然而，现有的VTON模型经常存在服装变形和身体不一致等伪影问题，这凸显了对VTON生成图像进行可靠质量评估的需求。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在文本到图像编辑中面临两个主要挑战：1）如何在保持原始内容与应用编辑之间取得平衡；2）如何处理真实图像编辑。现有方法在这两方面存在不足。
...
Conclusion: LAMS-Edit提供了一个可扩展的框架，通过混合反演过程中的中间状态，成功解决了扩散模型在文本到图像编辑中的平衡问题，支持精确编辑和风格迁移。
...
<details>
  <summary>Details</summary>
Motivation: 原始ULS模型在CT扫描中基于点击点周围的感兴趣体积进行全身病灶分割。自发布以来，出现了多个新的公共数据集可以进一步提升模型性能。

Method: ULS+整合了额外的数据集，并使用了更小的输入图像尺寸，从而提高了准确性和推理速度。

Result: 在ULS23挑战赛测试数据和Longitudinal-CT数据集子集上，ULS+在Dice分数和点击点位置鲁棒性方面显著优于原始ULS。ULS+在ULS23挑战赛测试阶段排行榜上排名第一。
...
TL;DR: 该论文提出了一种针对漫画视觉问答（CVQA）的新型推理框架，解决了传统思维链（CoT）提示在CVQA任务中表现不佳的问题，特别是在小规模多模态大语言模型中。


<details>
  <summary>Details</summary>
Motivation: 漫画视觉问答（CVQA）因其依赖符号抽象、叙事逻辑和幽默元素，与传统视觉问答任务存在显著差异。研究发现，标准思维链（CoT）提示在CVQA任务中反而会降低性能，特别是在小规模模型中，这源于状态纠缠、虚假转换和探索效率低下等问题。

Method: 提出了一种新颖的漫画推理框架，结合模块化CoT生成、基于GRPO的强化微调以及新颖的结构化奖励机制。该框架旨在为小型MLLM生成更忠实和可迁移的推理链。

Result: 在五个具有挑战性的基准测试中，提出的3B模型超越了现有最先进方法。插件实验显示，在不同MLLM上平均获得了12.1%的额外改进。该方法还在更广泛的幽默中心和抽象视觉推理任务（包括表情包理解和社论漫画解读）上进行了评估。

Conclusion: 该研究成功解决了CVQA任务中标准CoT提示的局限性，提出的漫画推理框架显著提升了小型MLLM在复杂视觉推理任务中的性能，特别是在资源受限的环境中。
...
Method: 提出风险意图选择性检测框架，包含：1）基于势场理论的势场-轨迹关联模型定量评估运动学风险；2）意图驱动区域预测模块利用自车运动先验主动预测和筛选关键BEV区域；3）语义选择性融合方案仅传输高交互区域的高保真特征。

Result: 在DeepAccident数据集上的实验表明，该方法将通信量降至全特征共享的0.71%，同时保持最先进的检测精度，在带宽效率与感知性能之间建立了有竞争力的帕累托前沿。

Conclusion: RiSe框架通过从识别可见区域转向优先考虑风险关键区域的范式转变，实现了高效的语义选择性融合，解决了车路协同感知中带宽与特征冗余的权衡问题。
...
TL;DR: SA-ResGS提出了一种自增强残差3D高斯泼溅框架，通过自增强点云和残差学习策略，提升主动场景重建中下一最佳视角选择的稳定性与不确定性量化效果。


<details>
  <summary>Details</summary>
Motivation: 现有主动场景重建方法在稀疏和宽基线视角下存在不确定性量化不稳定、高斯监督不足的问题，导致下一最佳视角选择效果不佳。

Method: 1) 通过训练视角与光栅化外推视角之间的三角测量生成自增强点云，实现高效场景覆盖估计；2) 引入首个针对3D高斯泼溅的残差学习策略，结合不确定性驱动过滤与dropout和硬负样本采样，增强高不确定性高斯的梯度流。

Result: 在主动视角选择实验中，SA-ResGS在重建质量和视角选择鲁棒性方面均优于现有最先进基线方法。

Conclusion: SA-ResGS通过物理引导的视角选择策略和不确定性感知的残差监督方案，有效解决了稀疏宽基线视角下的不确定性量化与监督问题，提升了主动场景重建的性能。
...
TL;DR: DFRCP是一种动态模糊鲁棒卷积金字塔模块，作为YOLOv11的插件升级，用于提升运动模糊条件下的目标检测性能，通过融合多尺度特征和自适应注入模糊特征来增强全局感知。


<details>
  <summary>Details</summary>
Motivation: 相机抖动引起的运动模糊会产生重影伪影，严重降低边缘侧目标检测性能。现有方法要么将模糊作为噪声抑制而丢失判别性结构，要么进行全图像恢复增加延迟并限制在资源受限设备上的部署。

Method: 提出DFRCP模块，增强YOLOv11特征金字塔：1)结合大尺度和中尺度特征同时保留原生表示；2)引入动态鲁棒开关单元，自适应注入模糊特征以增强抖动下的全局感知；3)通过旋转和非线性插值合成多尺度模糊特征；4)使用透明度卷积学习内容自适应权衡；5)开发CUDA并行旋转和插值内核实现400倍加速。

Result: 在约3,500张小麦病虫害私有数据集上训练，使用两种模糊机制进行三倍数据增强。在模糊测试集上，YOLOv11+DFRCP比YOLOv11基线准确率提高约10.4%，仅增加适度训练时间开销，减少数据收集后的人工筛选需求。

Conclusion: DFRCP作为YOLOv11的插件升级，有效提升了运动模糊条件下的目标检测性能，通过自适应模糊特征注入和高效CUDA实现，使其适用于边缘设备部署，减少了数据收集后的人工处理需求。
...
TL;DR: ViT在空间推理任务中存在系统性失败，这源于架构固有的电路复杂性限制，而非数据规模问题。研究发现，对于非可解群（如SO(3)），保持结构保持嵌入的计算复杂度下界为NC^1完全，而恒定深度ViT仅限于TC^0，存在复杂度边界。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在语义识别方面表现出色，但在空间推理任务（如心理旋转）中存在系统性失败。传统观点常将其归因于数据规模，但本文认为这种限制源于架构固有的电路复杂性。

Method: 将空间理解形式化为学习群同态：将图像序列映射到保持底层变换群代数结构的潜在空间。证明对于非可解群（如3D旋转群SO(3)），保持这种结构保持嵌入的计算复杂度下界为Word Problem（NC^1完全）。对比证明恒定深度ViT受限于TC^0复杂度类。

Result: 在TC^0 ⊊ NC^1的假设下，建立了复杂度边界：恒定深度ViT从根本上缺乏有效捕捉非可解空间结构所需的逻辑深度。通过潜在空间探测验证了这一复杂度差距，显示ViT表示在非可解任务上随着组合深度增加而出现结构崩溃。

Conclusion: ViT在空间推理任务中的局限性源于架构固有的计算复杂度限制，而非数据不足。恒定深度ViT无法有效处理非可解群的空间结构，这为理解Transformer架构的能力边界提供了理论依据。
...
TL;DR: 本文提出CFSG模型，通过将概念和特征空间解耦为共同、特定和混淆三个结构化组件，解决细粒度领域泛化问题，在三个基准数据集上平均性能提升9.87%，优于现有SOTA方法3.08%。


<details>
  <summary>Details</summary>
Motivation: 细粒度领域泛化比传统领域泛化更具挑战性，因为细粒度识别任务中存在细微的类间差异和相对明显的类内变化。在领域偏移下，模型对细粒度线索过度敏感，导致关键特征被抑制，性能显著下降。认知研究表明人类通过利用共同和特定属性来分类对象，但当前深度学习模型尚未有效融入这一机制。
...
Conclusion: CFSG模型通过模仿人类认知机制，将概念和特征空间结构化解耦，有效解决了细粒度领域泛化问题，显著提升了模型在领域偏移下的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 标准LLM在自然图像上训练，难以处理卡通图像特有的视觉夸张抽象和叙事驱动上下文，需要专门方法解决卡通VQA挑战。

Method: 引入多智能体LLM框架，包含三个专门化智能体：视觉智能体（处理视觉信息）、语言智能体（处理语言信息）、批评智能体（评估协调），通过协作整合视觉线索和叙事上下文进行结构化推理。

Result: 在Pororo和Simpsons两个卡通VQA数据集上系统评估，详细分析了每个智能体对最终预测的贡献，深入理解了LLM多智能体在卡通VQA和多模态推理中的行为。

Conclusion: 多智能体LLM框架能有效解决卡通图像VQA的特殊挑战，通过专门化智能体协作整合视觉和叙事信息，为卡通VQA任务提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: AI在皮肤病学中的临床采用面临偏见障碍，现有诊断模型在深色肤色上表现不佳，这阻碍了公平的医疗AI应用。

Method: LesionTABE框架结合了对抗性去偏技术和皮肤病专用基础模型嵌入，通过对抗训练减少肤色相关的偏见，同时利用专业领域的基础模型特征。

Result: 在涵盖恶性和炎症性皮肤病的多个数据集上评估，LesionTABE相比ResNet-152基线在公平性指标上提升超过25%，超越了现有去偏方法，同时提高了整体诊断准确率。

Conclusion: 基础模型去偏是实现公平临床AI应用的重要步骤，LesionTABE展示了通过领域专用基础模型和对抗性去偏相结合的方法，可以有效减少皮肤病诊断中的肤色偏见。
...
TL;DR: TGIF是一个轻量级模块，通过文本引导的层间融合，利用视觉编码器的多层特征来减少MLLM的幻觉问题，提升视觉基础能力


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型通常只使用视觉编码器的单一高层特征，忽视了丰富的层级视觉线索，导致模型过度依赖语言先验而非图像证据，产生视觉未基础的幻觉问题

Method: 提出TGIF（文本引导的层间融合）模块，将编码器各层视为深度方向的"专家"，根据查询预测视觉特征的融合方式。该方法遵循直接外部融合原则，无需更新视觉编码器，计算开销小

Result: 在LLaVA-1.5-7B中集成TGIF后，在幻觉、OCR和VQA基准测试上获得一致改进，同时在ScienceQA、GQA和MMBench上保持或提升性能

Conclusion: 查询条件化、层级感知的融合是增强现代MLLM视觉基础和减少幻觉的有效方法
...
TL;DR: Unified Thinker：一种任务无关的推理架构，通过解耦推理模块与生成器，使用强化学习优化视觉正确性，显著提升图像生成中的逻辑推理能力


<details>
  <summary>Details</summary>
Motivation: 当前生成模型在逻辑密集型指令跟随方面存在推理-执行差距，闭源系统（如Nano Banana）已展示强大的推理驱动图像生成能力，开源模型存在明显差距。需要可执行的推理能力，将高级意图分解为可验证的计划来直接指导生成过程

Method: 提出Unified Thinker架构，将专用推理模块（Thinker）与图像生成器（Generator）解耦，支持模块化升级。采用两阶段训练：首先为Thinker构建结构化规划接口，然后应用强化学习，基于像素级反馈来优化策略，鼓励优化视觉正确性而非文本合理性

Result: 在文本到图像生成和图像编辑任务上的广泛实验表明，Unified Thinker显著提升了图像推理和生成质量

Conclusion: 通过解耦推理与生成，采用强化学习优化视觉正确性，Unified Thinker有效缩小了开源模型与闭源系统在推理驱动图像生成方面的差距，为通用图像生成提供了任务无关的推理架构
...
<details>
  <summary>Details</summary>
Motivation: 计算病理学中精确且可扩展的细胞核实例分割面临重大挑战，现有方法依赖基于补丁的处理和昂贵的后处理，牺牲了上下文信息和效率。
...
Result: 在PanNuke和MoNuSeg数据集上的评估显示，该方法在跨组织泛化能力和效率方面表现优异，比次优方法快5倍以上，达到最先进的效率水平。

Conclusion: LSP-DETR为计算病理学中的细胞核实例分割提供了一种高效、精确且可扩展的端到端解决方案，显著提升了处理大尺寸全切片图像的能力。
...
TL;DR: AnatomiX是一个专门为胸部X光解剖学基础解释设计的多任务多模态大语言模型，通过两阶段方法显著提升解剖学推理能力，在多个任务上性能提升超过25%


<details>
  <summary>Details</summary>
Motivation: 现有的多模态医学大语言模型在胸部X光解释中面临空间推理和解剖学理解的挑战，现有的基础技术虽然提高了整体性能，但未能建立真正的解剖学对应关系，导致医学领域的不正确解剖理解

Method: 受放射学工作流程启发，采用两阶段方法：首先识别解剖结构并提取特征，然后利用大语言模型执行多种下游任务，包括短语定位、报告生成、视觉问答和图像理解

Result: 在多个基准测试中，AnatomiX实现了卓越的解剖学推理能力，在解剖学定位、短语定位、基础诊断和基础字幕生成任务上相比现有方法性能提升超过25%

Conclusion: AnatomiX通过明确设计用于解剖学基础的胸部X光解释，有效解决了现有模型在解剖学理解方面的不足，为医学图像分析提供了更准确的解剖学推理能力
...
Method: 提出UniCorn自改进框架，无需外部数据或教师监督。将单个统一多模态模型划分为三个协作角色：提议者、求解者和评判者。通过自我博弈生成高质量交互，并采用认知模式重构将潜在理解提炼为显式生成信号。引入UniCycle基准，基于文本到图像到文本的重建循环来验证多模态一致性恢复。
...
### [47] [LTX-2: Efficient Joint Audio-Visual Foundation Model](https://arxiv.org/abs/2601.03233)
*Yoav HaCohen,Benny Brazowski,Nisan Chiprut,Yaki Bitterman,Andrew Kvochko,Avishai Berkowitz,Daniel Shalem,Daphna Lifschitz,Dudu Moshe,Eitan Porat,Eitan Richardson,Guy Shiran,Itay Chachy,Jonathan Chetboun,Michael Finkelson,Michael Kupchick,Nir Zabari,Nitzan Guetta,Noa Kotler,Ofir Bibi,Ori Gordon,Poriya Panet,Roi Benita,Shahar Armon,Victor Kulikov,Yaron Inger,Yonatan Shiftan,Zeev Melumian,Zeev Farbman*

Main category: cs.CV

TL;DR: LTX-2是一个开源的音视频生成基础模型，能够生成高质量、时间同步的音视频内容，在计算成本和推理时间方面优于专有模型。
...
Result: 模型在开源系统中实现了最先进的音视频质量和提示遵循度，同时以远低于专有模型的计算成本和推理时间提供可比的结果，能够生成包含自然背景和音效的丰富音频轨道。

Conclusion: LTX-2展示了统一音视频生成模型的可行性，在保持高质量输出的同时实现了计算效率，为开源社区提供了强大的音视频生成工具。
...
Method: 将深度表示为神经隐式场，通过局部隐式解码器查询连续2D坐标处的深度，实现任意分辨率和细粒度深度估计。

Result: 在合成和真实世界基准测试中达到最先进性能，在相对和度量深度估计任务上表现优异，特别是在细细节区域；在大视角变化下的新视角合成任务中也产生高质量结果。
...
TL;DR: Muses是首个免训练的3D生物生成前馈方法，通过骨骼引导实现结构感知的3D内容创建


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖部件感知优化、手动组装或2D图像生成，常因复杂的部件级操作和有限域外生成能力而产生不现实或不连贯的3D资产

Method: 1) 通过图约束推理构建具有连贯布局和比例的创意3D骨架；2) 在结构化潜在空间中进行基于体素的组装；3) 在骨骼条件下进行图像引导的外观建模生成风格一致的纹理

Result: 实验证明Muses在视觉保真度和文本描述对齐方面达到最先进性能，并展示灵活的3D对象编辑潜力

Conclusion: Muses通过骨骼表示实现了免训练的3D生物生成，为结构感知的3D内容创建提供了新范式
...
TL;DR: WearVox是首个针对可穿戴设备语音助手的基准测试，包含3,842个多通道自我中心音频记录，涵盖5个任务，评估显示现有SLLMs在真实场景中表现不佳（29%-59%准确率），多通道音频能显著提升抗噪能力和语音区分能力。
...
Method: 提出WearVox基准测试，包含3,842个多通道自我中心音频记录，通过AI眼镜在多样室内外环境和声学条件下收集，涵盖五个任务：搜索基础QA、闭卷QA、侧谈拒绝、工具调用和语音翻译。每个记录都配有丰富元数据，用于在真实世界约束下分析模型性能。

Result: 评估领先的专有和开源语音大语言模型（SLLMs），发现大多数实时SLLMs在WearVox上的准确率在29%到59%之间，在嘈杂室外音频上性能显著下降。案例研究显示，多通道音频输入能显著增强模型对环境噪声的鲁棒性，并改善设备定向语音与背景语音的区分能力。

Conclusion: 空间音频线索对于上下文感知语音助手至关重要，WearVox为推进可穿戴语音AI研究建立了全面的测试平台，突显了在真实可穿戴场景中评估语音助手的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 虽然LLM在软件开发等多个领域表现出色，但在考虑硬件约束的物理计算环境中，其有效性尚未得到充分探索。需要填补这一研究空白，评估LLM在物理计算项目中的逻辑和物理能力。

Method: 提出PCEval基准测试，通过模拟环境实现完全自动化的评估框架。该框架评估LLM在不同项目复杂度下生成电路和兼容代码的能力，对13个领先模型进行了全面测试。
...
TL;DR: 论文提出了一种新的拓扑损失函数，将食材列表表示为嵌入空间中的点云，通过最小化预测与真实食材之间的差异来提升食谱生成质量，结合多种复合目标函数显著改善了食谱生成的各项指标。


<details>
  <summary>Details</summary>
Motivation: 传统食谱生成模型主要基于交叉熵损失，仅关注文本流畅性，但食谱需要准确的时间、温度、程序连贯性和正确的食材组合。需要更全面的训练目标来提升食谱生成的质量。

Method: 在RECIPE-NLG基础上，研究多种复合目标函数，提出新的拓扑损失函数，将食材列表表示为嵌入空间中的点云，最小化预测食材与真实食材之间的差异。同时评估了Dice损失和混合损失的效果。

Result: 拓扑损失显著改善了食材和动作层面的指标，Dice损失在时间/温度精度方面表现优异，混合损失在数量和时间方面产生协同增益。人工偏好分析显示，该模型在62%的情况下被优先选择。

Conclusion: 通过引入拓扑损失等复合目标函数，能够有效提升食谱生成模型在食材准确性、时间温度精度等多维度的性能，为复杂程序性文本生成提供了更全面的优化方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部评估器、奖励模型或精确字符串匹配投票，限制了在开放文本生成任务中的适用性和效率。需要一种无需外部评估器的方法来从多个随机生成中选择高质量输出。

Method: 提出ModeX框架，构建候选生成文本的相似性图，通过递归应用谱聚类选择代表性中心点。还提出了ModeX-Lite版本，通过早期剪枝提高效率。

Result: 在文本摘要、代码生成和数学推理等开放文本任务中，ModeX方法持续优于标准的单路径和多路径基线方法。

Conclusion: ModeX为开放文本生成提供了一个计算高效的解决方案，无需额外推理或辅助模型，能够识别生成文本中的主导语义共识。
...
Method: 提出概率确定性和一致性（PCC）框架，通过联合建模LLM的概率确定性和推理一致性来估计事实置信度，实现自适应验证策略：模型在自信时直接回答，不确定或不一致时触发针对性检索，模糊度高时升级为深度搜索。

Result: 在三个具有挑战性的基准测试中，PCC在不确定性量化方面优于口头置信度表达，并持续优于强大的基于LLM的事实核查基线方法，且在不同LLM上具有良好的泛化能力。

Conclusion: PCC框架通过置信度引导的路由机制，仅在必要时调用检索，提高了事实核查的效率和可靠性，为LLM的事实准确性提供了有效的自适应验证方法。
...
TL;DR: 该研究提出了一种使用大语言模型模拟学生反应来估计项目反应理论参数的新方法，避免了传统现场测试的高成本。


<details>
  <summary>Details</summary>
Motivation: 传统方法通过昂贵的现场测试收集学生表现数据来校准项目反应理论参数，成本高昂。本研究旨在开发一种更经济高效的方法来估计项目参数。

Method: 使用Qwen-3密集模型系列和低秩适应技术，通过微调大语言模型来模拟不同潜在能力水平学生的多项选择题反应，重建正确反应概率作为学生能力的函数，生成合成项目特征曲线来估计IRT参数。

Result: 在六年级英语语言艺术项目数据集和BEA 2024共享任务数据集上的评估表明，该方法与基线方法相当或优于基线方法，特别在建模项目区分度方面表现出色。

Conclusion: 基于模拟的方法为估计项目反应理论参数提供了一种有前景的替代方案，特别在建模项目区分度方面效果显著，能够减少对昂贵现场测试的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 每年有超过350万份专利申请，但将科学论文转化为专利描述具有挑战性，因为两者在修辞风格和法律要求上存在差异。现有的黑盒文本到文本方法难以建模结构推理和法律约束。

Method: 提出了FlowPlan-G2P框架，将任务重构为三个阶段：1)概念图归纳：通过专家式推理提取技术实体和关系形成有向图；2)段落和章节规划：将图重组为与标准专利章节对齐的连贯集群；3)图条件生成：使用章节特定子图和定制提示生成法律合规的段落。
...
TL;DR: 本文提出了一种基于开放信息抽取（OpenIE）构建肺癌知识库的流程，用于增强大语言模型在生物医学领域的性能，通过监督语义微调显著提升了模型的语义连贯性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在生物医学研究中的应用需要高质量的训练数据，特别是在肿瘤学领域，精确性和可解释性至关重要。目前缺乏可扩展的方法来构建结构化知识库以支持有效的模型微调。

Method: 开发了一个四步流程：1) 使用MeSH词表识别医学概念；2) 筛选具有开放许可（CC0）的PubMed文献；3) 使用OpenIE方法提取（主语，关系，宾语）三元组；4) 通过命名实体识别（NER）丰富三元组以确保生物医学相关性。

Result: 构建了一个领域特定、大规模且具有噪声感知的肺癌知识库资源。通过监督语义微调T5模型，使用ROUGE和BERTScore评估显示，模型性能和语义连贯性得到显著提升。

Conclusion: OpenIE衍生的知识资源为增强生物医学自然语言处理提供了可扩展、低成本的解决方案，能够有效支持大语言模型在肿瘤学等专业领域的微调。
...
TL;DR: 论文提出了一种基于大语言模型的文档不一致性检测方法，通过引入新的证据提取指标和"编辑重试"框架，显著改进了直接提示方法的效果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在许多领域展现出强大能力，但在文档不一致性检测方面的研究相对有限。文档不一致性检测有两个关键方面：一是判断是否存在不一致性，二是提供不一致句子的证据。本研究聚焦于后者。

Method: 提出了新的全面证据提取指标，并引入了带有约束过滤的"编辑重试"框架（redact-and-retry framework with constrained filtering），显著改进了基于大语言模型的文档不一致性检测效果。

Result: 实验结果显示，该方法在文档不一致性检测方面取得了有希望的结果，相比直接提示方法有实质性改进。

Conclusion: 通过引入新的证据提取指标和编辑重试框架，能够有效提升大语言模型在文档不一致性检测任务中的性能，特别是在提取不一致证据方面。
...
TL;DR: 本研究训练了多种基于编码器的语言模型用于长文本作文自动评分，发现集成多个预训练语言模型表征的嵌入集成模型显著优于单个语言模型。


<details>
  <summary>Details</summary>
Motivation: 长文本处理对仅编码器语言模型构成挑战，特别是在长文本作文自动评分任务中。本研究旨在探索如何有效利用编码器模型处理长文本评分问题。

Method: 训练了BERT系列模型（BERT、RoBERTa、DistilBERT、DeBERTa），构建了基于多个编码器模型嵌入的集成模型，以及基于特征的监督机器学习集成模型（梯度提升决策树、XGBoost、LightGBM）。在17,307篇作文数据集上以80%/10%/10%划分训练、验证和测试，使用二次加权Kappa评估性能。

Result: 集成多个预训练语言模型表征的嵌入集成模型（使用梯度提升分类器作为集成模型）在长文本作文评分任务中显著优于单个语言模型。

Conclusion: 对于长文本作文自动评分任务，集成多个编码器模型的嵌入表示比单一模型表现更好，为解决长文本处理挑战提供了有效方法。
...
TL;DR: LATS是一种无需攻击者LLM的越狱方法，通过词汇锚注入实现高效攻击，仅需约6.4次查询即可在最新LLM上达到97-100%的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有越狱方法需要攻击者LLM生成对抗性查询且查询预算高，导致成本昂贵，且生成的查询通常包含不可解释的随机前缀。需要更高效、资源需求更低的攻击方法。

Method: LATS将越狱重新定义为多轮对话的广度优先树搜索，每个节点逐步将攻击目标中的缺失内容词注入良性提示中，通过词汇锚注入实现攻击，无需攻击者LLM参与。

Result: 在AdvBench和HarmBench上的评估显示，LATS在最新的GPT、Claude和Llama模型上达到97-100%的攻击成功率，平均仅需约6.4次查询，而其他方法需要20+次查询。

Conclusion: 对话结构是一个强大且保护不足的攻击面，LATS在攻击成功率普遍较高的时代展示了卓越的查询效率，为越狱攻击提供了更高效的解决方案。
...
TL;DR: 研究通过两阶段方法从生产级LLMs中提取受版权保护的训练数据，发现即使有安全措施，仍存在数据提取风险


<details>
  <summary>Details</summary>
Motivation: 解决关于LLMs是否记忆训练数据以及能否从生产模型中提取受版权文本的未解法律问题，特别是针对实施了安全措施的生产级模型

Method: 采用两阶段提取程序：1) 初始探测（有时使用Best-of-N越狱），2) 迭代续写提示；在Claude 3.7 Sonnet、GPT-4.1、Gemini 2.5 Pro和Grok 3四个生产模型上评估，使用基于块的最长公共子串近似（nv-recall）衡量提取成功率

Result: 不同模型提取效果各异：Gemini 2.5 Pro和Grok 3无需越狱即可提取大量文本（nv-recall分别达76.8%和70.3%），Claude 3.7 Sonnet需要越狱但能输出近乎完整的书籍（nv-recall=95.8%），GPT-4.1需要更多越狱尝试且最终拒绝继续（nv-recall=4.0%）

Conclusion: 即使有模型和系统级安全防护，从生产级LLMs中提取受版权训练数据仍然存在风险，这对LLMs的版权法律问题具有重要启示
...
TL;DR: 本文提出了一种新的结构化剪枝框架，通过混合多域校准集和迭代校准策略来识别和移除冗余通道，在保持硬件兼容性的同时实现显著模型压缩。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型规模不断增长带来了计算开销大、内存占用高和推理延迟长等实际部署障碍。现有非结构化剪枝技术产生的稀疏模式不规则，需要专门的硬件或软件支持，而结构化剪枝能够移除整个架构组件并保持与标准硬件加速器的兼容性。

Method: 提出了一种新颖的结构化剪枝框架，采用混合多域校准集和迭代校准策略来有效识别和移除冗余通道。该方法通过多领域数据校准来全面评估通道重要性，并通过迭代优化确保剪枝后的模型性能稳定。

Result: 在各种模型和多样化下游任务上的广泛实验表明，该方法能够实现显著的压缩效果，同时保持最小的性能下降。结构化剪枝后的模型与标准硬件加速器保持兼容。

Conclusion: 该结构化剪枝框架为解决大型语言模型部署障碍提供了有效解决方案，通过移除冗余通道实现模型压缩，同时保持硬件兼容性和模型性能，为实际应用中的模型部署提供了实用方法。
...
<details>
  <summary>Details</summary>
Motivation: 情感分析和仇恨言论检测对于在线内容审核至关重要，能够检测和缓解有害或冒犯性内容，为创造更安全的数字环境做出贡献。本研究旨在通过结合最先进的Transformer架构和可解释性技术，提高多语言情感分析和仇恨言论检测系统的有效性和透明度。

Method: 使用三种Transformer模型：BERT-base-multilingual-cased、RoBERTa-base和XLM-RoBERTa-base（前八层冻结），在五种语言（英语、韩语、日语、中文、法语）上进行情感分析和仇恨言论检测。采用标准性能指标（准确率、精确率、召回率、F1分数）进行评估，并集成LIME框架分析模型决策中各个单词的贡献。

Result: 研究对三种模型在五种语言上的性能进行了比较评估，使用标准性能指标衡量模型表现。通过LIME框架提供了模型预测行为的深入洞察，增强了模型的可解释性。

Conclusion: 通过结合最先进的Transformer架构和可解释性技术，本研究提高了多语言情感分析和仇恨言论检测系统的有效性和透明度，为在线内容审核提供了更可靠和可解释的工具。
...
TL;DR: 该研究系统评估了Transformer模型在AddSent对抗数据集上的鲁棒性，通过多级错误分析识别主要失败模式，发现模型规模扩大可消除鲁棒性-准确性权衡，并提出基于NER的对比学习方法显著缩小对抗性能差距。


<details>
  <summary>Details</summary>
Motivation: 尽管问答系统在标准基准测试（如SQuAD）上表现出色，但它们对对抗样本仍然脆弱。本研究旨在系统评估Transformer模型在AddSent对抗数据集上的鲁棒性，并开发有效的缓解策略。

Method: 1) 使用五种互补分类方案进行全面的多级错误分析；2) 系统评估对抗性微调比例；3) 进行数据增强实验；4) 从ELECTRA-small（1400万参数）扩展到ELECTRA-base（1.1亿参数）；5) 实施三种针对性缓解策略，包括基于实体感知的对比学习。

Result: 1) 识别否定混淆和实体替换为主要失败模式；2) 确定80%干净数据+20%对抗数据为最优微调比例；3) 发现小模型存在容量瓶颈；4) 扩大模型规模消除了鲁棒性-准确性权衡，在干净和对抗数据上都取得显著改进；5) 实体感知对比学习表现最佳：AddSent EM达到89.89%，SQuAD EM达到90.73%，对抗差距缩小了94.9%。

Conclusion: 这是首个将全面语言错误分析与NER引导的对比学习相结合用于对抗性QA的研究，证明针对性缓解策略可以实现干净性能与对抗性能接近持平，模型规模扩大是消除鲁棒性-准确性权衡的关键因素。
...
<details>
  <summary>Details</summary>
Motivation: 针对大语言模型中存在的幻觉问题，特别是由提示引发的幻觉，需要开发更有效的方法来约束模型的推理过程，提高生成内容的准确性和可验证性。

Method: 基于知识蒸馏链式模型，引入代码模块来指导知识图谱探索，并将代码作为思维链提示的一部分，形成外部知识输入，为模型提供更准确、结构化的信息，从而约束LLMs的推理过程。

Result: 在GPT-4和LLaMA-3.3上的实验表明，加入代码模块显著提升了模型捕捉上下文信息的能力，有效缓解了提示诱导幻觉。HIT@1、HIT@3、HIT@5分别提升了15.64%、13.38%、13.28%，在多个评估设置中HIT@1、HIT@3、HIT@5得分均超过95%。

Conclusion: 提出的方法通过代码模块增强知识蒸馏链式模型，显著减少了大语言模型的幻觉行为，同时提高了模型的准确性和可验证性，为解决LLM幻觉问题提供了有效途径。
...
TL;DR: 该研究通过计算模拟和自然语言分析发现，分层处理相比线性处理能更有效地将语言处理单元数控制在人类工作记忆容量限制内，这解释了为什么人类语言普遍具有分层结构。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解释为什么人类语言普遍具有分层结构。作者认为这可能与人类有限的工作记忆容量有关，分层结构可能是优化语言处理效率以适应记忆限制的解决方案。

Method: 研究建立了似然函数来量化语言处理机制产生的平均单元数与人类工作记忆容量的匹配程度。通过计算模拟符号序列和对自然语言句子进行验证分析，比较分层处理与线性处理在控制处理单元数方面的表现。

Result: 研究发现分层处理显著优于线性处理，能更有效地将处理单元数控制在人类工作记忆容量限制内，特别是在序列/句子长度增加时。结果还显示出与儿童工作记忆发展相关的收敛模式。

Conclusion: 构建分层结构优化了序列语言输入的处理效率，同时保持在记忆限制内，这真正解释了人类语言普遍具有分层性质的原因。
...
### [67] [SYNAPSE: Empowering LLM Agents with Episodic-Semantic Memory via Spreading Activation](https://arxiv.org/abs/2601.02744)
*Hanqi Jiang,Junhao Chen,Yi Pan,Ling Chen,Weihang You,Yifan Zhou,Ruidong Zhang,Yohannes Abate,Tianming Liu*

Main category: cs.CL

TL;DR: Synapse是一种统一记忆架构，通过动态图建模和激活传播机制解决LLM长期记忆的"上下文隧道"问题，显著提升复杂时序和多跳推理性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用推理方面表现出色，但标准的检索增强方法无法解决长期智能体记忆的碎片化问题。现有方法基于静态向量相似性，无法有效处理动态记忆关联。

Method: 提出Synapse统一记忆架构，借鉴认知科学将记忆建模为动态图，通过激活传播而非预计算链接来建立关联。整合侧向抑制和时间衰减机制，动态突出相关子图并过滤干扰。实现三重混合检索策略，融合几何嵌入和基于激活的图遍历。

Result: 在LoCoMo基准测试上的综合评估表明，Synapse在复杂时序和多跳推理任务上显著优于最先进方法，有效解决了"上下文隧道"问题。

Conclusion: Synapse为LLM的长期记忆问题提供了稳健解决方案，通过动态图建模和激活传播机制超越了传统静态向量相似性方法，代码和数据将在接受后公开。
...
TL;DR: WBC攻击方法通过滑动窗口和基于符号的聚合，利用局部化上下文而非全局平均损失来检测LLM训练数据成员身份，显著提高了攻击效果


<details>
  <summary>Details</summary>
Motivation: 现有的大多数针对大语言模型的成员推理攻击依赖全局信号（如平均损失）来识别训练数据，但这种方法会稀释记忆的细微局部信号，降低攻击效果。作者挑战这种全局平均范式，认为成员信号在局部上下文中更为显著

Method: 提出WBC（基于窗口的比较）方法，采用滑动窗口和基于符号的聚合。该方法使用不同大小的窗口在文本序列上滑动，每个窗口基于目标模型和参考模型的损失比较对成员身份进行二元投票。通过几何间隔的窗口大小集成投票，从标记级伪影到短语级结构捕捉记忆模式

Result: 在11个数据集上的广泛实验表明，WBC显著优于现有基线方法，实现了更高的AUC分数，在低误报率阈值下检测率提高了2-3倍

Conclusion: 聚合局部证据比全局平均更有效，揭示了微调LLM中的关键隐私漏洞。局部化方法能更好地捕捉记忆信号，为成员推理攻击提供了更有效的范式
...
### [69] [EComStage: Stage-wise and Orientation-specific Benchmarking for Large Language Models in E-commerce](https://arxiv.org/abs/2601.02752)
*Kaiyan Zhao,Zijie Meng,Zheyong Xie,Jin Duan,Yao Hu,Zuozhu Liu,Shaosheng Cao*

Main category: cs.CL

TL;DR: EComStage是一个用于评估电商场景中LLM智能体的统一基准，专注于感知、规划和行动三阶段推理过程，覆盖客户和商家双重视角。
...
Method: 提出EComStage基准，通过7个代表性任务评估LLM在感知（理解用户意图）、规划（制定行动计划）和行动（执行决策）三阶段的推理能力。所有样本都经过人工标注和质量检查，覆盖客户和商家双重视角。

Result: 评估了30多个LLM（参数从1B到200B+），包括开源模型和闭源API，揭示了不同阶段和视角下的特定优势和弱点。
...
### [70] [Punctuation-aware Hybrid Trainable Sparse Attention for Large Language Models](https://arxiv.org/abs/2601.02819)
*Junxiang Qiu,Shuo Wang,Zhengsu Chen,Hengheng Zhang,Jinda Lu,Changcheng Li,Qi Tian*

Main category: cs.CL

TL;DR: PHSA是一种基于标点符号的混合稀疏注意力机制，通过标点作为语义边界锚点，在保持核心语义结构的同时显著降低计算复杂度，在长序列处理中优于密集注意力和现有稀疏注意力方法。


<details>
  <summary>Details</summary>
Motivation: 密集注意力在长序列处理中存在二次方复杂度问题，现有稀疏注意力方法使用粗粒度语义表示进行块选择，导致语义边界模糊和关键信息丢失。

Method: 提出基于标点符号的混合稀疏注意力(PHSA)：1)设计双分支聚合机制，融合全局语义表示和标点增强的边界特征；2)引入极端稀疏自适应训练和推理策略，稳定低token激活率下的模型行为。

Result: 在通用基准和长上下文评估中，PHSA始终优于密集注意力和最先进的稀疏注意力基线(包括InfLLM v2)。对于32k-token输入序列的0.6B参数模型，在97.3%稀疏度下可将信息损失减少10.8%。

Conclusion: PHSA通过标点符号作为语义边界锚点，有效解决了稀疏注意力中的语义边界模糊问题，在保持高性能的同时显著降低计算复杂度，为长序列建模提供了可扩展的解决方案。
...
### [71] [The performances of the Chinese and U.S. Large Language Models on the Topic of Chinese Culture](https://arxiv.org/abs/2601.02830)
*Feiyan Liu,Chenxun Zhuo,Siyan Zhao,Bao Ge,Tianming Liu*

Main category: cs.CL

TL;DR: 中英文大语言模型在中文文化问题上的表现存在差异，中国开发的模型在中文文化理解任务上普遍优于美国开发的模型。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探究中美两国开发的大语言模型在中文语境下是否存在文化差异，特别是对中国文化的理解能力。由于世界十大LLM开发者主要集中在中美两国，且文化背景影响问题解决方式，因此需要评估这些模型在中文文化相关任务上的表现。

Method: 采用直接提问范式，评估包括GPT-5.1、DeepSeek-V3.2、Qwen3-Max和Gemini2.5Pro等模型。评估内容涵盖对中国传统文化的理解，包括历史、文学、诗歌及相关领域。

Result: 中国开发的模型在这些任务上普遍优于美国开发的模型。在美国开发的模型中，Gemini 2.5Pro和GPT-5.1取得了相对较高的准确率。

Conclusion: 观察到的性能差异可能源于训练数据分布的差异、本地化策略以及模型开发过程中对中文文化内容的重视程度不同。这表明文化背景确实会影响LLM在特定文化任务上的表现。
...
### [72] [To Generate or Discriminate? Methodological Considerations for Measuring Cultural Alignment in LLMs](https://arxiv.org/abs/2601.02858)
*Saurabh Kumar Pandey,Sougata Saha,Monojit Choudhury*

Main category: cs.CL

TL;DR: 研究提出逆社会人口统计提示（ISDP）方法，通过让LLMs从真实和模拟用户行为中预测人口统计特征，来更准确地评估模型的文化能力，避免传统SDP方法因任务设计问题导致的偏差误判。


<details>
  <summary>Details</summary>
Motivation: 传统的社会人口统计提示（SDP）方法在评估LLMs文化能力时存在混淆因素：提示敏感性、解码参数以及生成任务相对于判别任务的固有难度。这些因素使得难以区分性能不佳是由于模型偏见还是任务设计问题。

Method: 使用逆社会人口统计提示（ISDP），让LLMs从真实和模拟的用户行为中判别和预测人口统计特征。使用Goodreads-CSI数据集（包含印度、墨西哥和美国用户对英文书评的理解难度数据），测试了Aya-23、Gemma-2、GPT-4o和LLaMA-3.1四个模型。

Result: 模型在真实行为上的表现优于模拟行为，这与SDP的预期相反。然而，在个体层面上，两种行为类型的表现都下降且趋于接近，表明个性化存在局限性。

Conclusion: ISDP方法能够更准确地评估LLMs的文化能力，避免了SDP方法中的混淆因素。研究发现模型在真实用户行为上表现更好，但在个体层面的个性化能力有限，这为LLMs文化能力评估提供了更可靠的框架。
...
### [73] [Training Language Models with homotokens Leads to Delayed Overfitting](https://arxiv.org/abs/2601.02867)
*Adrian Cosma,Stefan Ruseti,Emilian Radoi,Mihai Dascalu*

Main category: cs.CL

TL;DR: 该论文提出了一种名为"同形令牌"的数据增强方法，通过利用子词分词中的多种有效分词变体来增强语言模型的训练，提高模型对分词变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 子词分词在语言模型中引入了一个计算层，其中许多不同的令牌序列可以解码为相同的表面形式并保留含义，但会引发不同的内部计算。尽管存在这种非唯一性，语言模型通常只使用单一的标准最长前缀分词进行训练，这限制了模型对分词变化的鲁棒性。

Method: 论文提出"同形令牌"概念，即同一词汇项的有效子词分词变体。引入轻量级训练架构，通过辅助因果编码器和块因果交叉注意力，在标准下一个令牌预测任务中条件化采样到的同形令牌变体，而不修改训练目标或令牌接口。

Result: 在数据受限的预训练中，同形令牌增强能持续延迟重复数据暴露下的过拟合，并在多样化的评估数据集上提高泛化能力。在多语言微调中，同形令牌的有效性取决于分词器质量：当标准令牌高度压缩时增益最强，而当分词器已经过度分割输入时增益减弱。

Conclusion: 同形令牌为在语言模型中引入分词不变性提供了一种简单且模块化的机制，能够提高模型对分词变化的鲁棒性，特别是在数据受限和多语言场景下。
...
### [74] [LongBench Pro: A More Realistic and Comprehensive Bilingual Long-Context Evaluation Benchmark](https://arxiv.org/abs/2601.02872)
*Ziyang Chen,Xing Wu,Junlong Jia,Chaochen Gao,Qi Fu,Debing Zhang,Songlin Hu*

Main category: cs.CL

TL;DR: LongBench Pro是一个双语长文本基准测试，包含1500个自然长文本样本，支持8k-256k令牌长度，通过人机协作构建，评估了46个长文本LLM，发现长文本优化比参数扩展更重要，实际有效上下文长度通常短于宣称长度。


<details>
  <summary>Details</summary>
Motivation: 当前长文本基准测试在可扩展性和真实性之间存在权衡：合成任务无法代表真实世界复杂性，而完全手动标注成本过高，难以扩展到极端长度和多样化场景。需要更现实、全面的长文本评估基准。

Method: 提出LongBench Pro基准测试，包含1500个自然长文本样本（英文和中文），覆盖11个主要任务和25个次要任务，输入长度8k-256k令牌。采用人机协作构建流程：前沿LLM生成挑战性问题、参考答案、设计原理和解决过程，专家验证正确性并修正问题。支持任务特定指标和多维度分类（上下文需求、长度、难度）。

Result: 评估46个广泛使用的长文本LLM，发现三个关键结果：1）长文本优化对长文本理解的贡献大于参数扩展；2）有效上下文长度通常短于宣称长度，存在显著的跨语言不对齐；3）"思考"范式主要帮助原生推理训练的模型，而混合思考设计提供了有前景的帕累托权衡。

Conclusion: LongBench Pro为推进长文本理解提供了一个稳健的测试平台，通过平衡质量与可扩展性的人机协作构建方法，提供了更现实、全面的长文本评估基准，揭示了当前长文本LLM的实际能力和局限性。
...
### [75] [Revisiting Data Compression with Language Modeling](https://arxiv.org/abs/2601.02875)
*Chen-Han Tsai*
...
<details>
  <summary>Details</summary>
Motivation: 尽管先前研究显示LLM在多模态数据压缩方面有潜力，但在实际应用中仍存在挑战，需要探索如何用LLM替代现有数据压缩算法。

Method: 研究探索了不同方法使用LLM作为数据压缩器，以降低调整压缩率，重点关注无需额外模型训练的配置方式。

Result: 在enwik9数据集上实现了约18%的调整压缩率，达到新的SOTA；在文本主导领域表现出色，在非自然文本序列压缩方面通过适当配置也能保持竞争力。

Conclusion: LLM在数据压缩方面具有显著潜力，特别是在文本主导领域，通过适当配置也能有效压缩非自然文本序列，为替代传统压缩算法提供了可能。
...
### [76] [Linear Script Representations in Speech Foundation Models Enable Zero-Shot Transliteration](https://arxiv.org/abs/2601.02906)
...
TL;DR: 研究发现多语言语音模型（如Whisper）的激活空间中线性编码了文字脚本信息，通过修改推理时的激活向量可以直接控制输出脚本，甚至能实现非常规的语言-脚本组合。


<details>
  <summary>Details</summary>
Motivation: 多语言语音基础模型（如Whisper）在网页规模数据上训练，每种语言包含多种区域变体，不同区域变体常使用不同文字脚本书写同一语言，导致语音识别输出在文字脚本上存在不确定性。

Method: 研究发现文字脚本在多语言语音模型的激活空间中呈线性编码，通过在推理时修改激活向量（添加脚本向量）可以直接控制输出脚本，即使是非常规的语言-脚本组合也能实现。

Result: 该方法能够实现语音识别输出的后处理脚本控制，在Whisper的所有模型规模上都表现出竞争性性能，能够诱导脚本变化，包括非常规的语言-脚本配对（如意大利语用西里尔字母、日语用拉丁字母）。

Conclusion: 通过利用多语言语音模型中线性编码的脚本信息，可以在推理时直接控制输出脚本，为解决语音识别输出脚本不确定性问题提供了一种有效方法。
...
### [77] [Beyond the Black Box: Theory and Mechanism of Large Language Models](https://arxiv.org/abs/2601.02907)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM领域存在一个关键悖论：尽管实证效果显著，但理论理解仍然相对初级，导致这些系统很大程度上被视为"黑箱"。为了解决这种理论碎片化问题，需要建立一个统一的理论框架。

Method: 提出了一个基于生命周期的统一分类法，将LLM研究组织成六个不同阶段：数据准备、模型准备、训练、对齐、推理和评估。在此框架下，系统回顾了驱动LLM性能的基础理论和内部机制。

Result: 分析了一系列核心理论问题，包括数据混合的数学原理、各种架构的表征限制、对齐算法的优化动态等。识别了前沿挑战，如合成数据自我改进的理论限制、安全保证的数学边界、涌现智能的机制起源。

Conclusion: 通过将实证观察与严谨科学探究相结合，这项工作为将LLM开发从工程启发式转向原则性科学学科提供了结构化路线图，促进了LLM研究的理论化发展。
...
### [78] [Image, Word and Thought: A More Challenging Language Task for the Iterated Learning Model](https://arxiv.org/abs/2601.02911)
*Hyoyeon Lee,Seth Bullock,Conor Houghton*

Main category: cs.CL

TL;DR: 该研究将半监督迭代学习模型应用于七段数码管图像通信任务，成功实现了表达性、组合性和稳定性的语言涌现。


<details>
  <summary>Details</summary>
Motivation: 探索语言传输约束如何促进语言结构的涌现，特别是将更复杂的七段数码管图像通信任务纳入迭代学习模型的研究。

Method: 采用半监督迭代学习模型，结合监督学习和无监督学习于自动编码器架构，应用于128个七段数码管图像的语言学习任务。

Result: 智能体能够学习并传输具有表达性（128个字形均有独特编码）、组合性（信号组件与意义组件一致映射）和稳定性（代际间语言不变）的语言。

Conclusion: 迭代学习模型能够成功处理复杂意义通信任务，验证了语言传输瓶颈促进语言结构涌现的理论，为研究更复杂的语言演化提供了有效工具。
...
### [79] [RAL2M: Retrieval Augmented Learning-To-Match Against Hallucination in Compliance-Guaranteed Service Systems](https://arxiv.org/abs/2601.02917)
...
<details>
  <summary>Details</summary>
Motivation: LLM驱动的服务系统中存在严重的幻觉问题，需要显式的知识基础来保证合规性响应。
...
Conclusion: 讨论了最佳实践和未来利用潜在表示的潜在方向。
...
### [80] [Memorization, Emergence, and Explaining Reversal Failures: A Controlled Study of Relational Semantics in LLMs](https://arxiv.org/abs/2601.02931)
*Yihua Zhu,Qianying Liu,Jiaxin Wang,Fei Cheng,Chaoran Liu,Akiko Aizawa,Sadao Kurohashi,Hidetoshi Shimodaira*

Main category: cs.CL

TL;DR: 研究发现自回归LLMs在关系任务中表现出色，但关系语义学习存在阈值现象，反转失败主要源于自回归顺序偏差而非语义缺陷


<details>
  <summary>Details</summary>
Motivation: 探究自回归LLMs是否真正学习关系词的逻辑语义（如对称性和逆关系逻辑），以及反转类型失败是由于缺失关系语义还是自左向右的顺序偏差

Method: 提出基于知识图谱的合成框架生成对称/逆三元组文本，从头训练GPT风格自回归模型，评估记忆、逻辑推理和上下文泛化到未见实体的能力

Result: 发现关系语义在足够逻辑监督下出现尖锐相变，即使浅层（2-3层）模型也能学习；成功泛化与稳定的中间层信号对齐；顺序匹配的前向/反向测试表明反转失败主要由自回归顺序偏差驱动

Conclusion: 自回归LLMs能够学习关系逻辑语义，但反转失败主要源于模型架构的顺序偏差而非语义理解缺陷，这为改进关系推理提供了方向
...
### [81] [Pearmut: Human Evaluation of Translation Made Trivial](https://arxiv.org/abs/2601.02933)
*Vilém Zouhar,Tom Kocmi*

Main category: cs.CL

TL;DR: Pearmut是一个轻量级但功能丰富的人类评估平台，旨在简化多语言NLP任务（特别是机器翻译）的端到端人工评估，使其像自动评估一样容易运行。


<details>
  <summary>Details</summary>
Motivation: 人类评估是多语言NLP的黄金标准，但由于现有工具设置复杂、工程和操作开销大，实践中常被自动指标替代。需要一种能降低门槛、简化流程的平台。

Method: 开发Pearmut平台，实现标准评估协议（DA、ESA、MQM），支持文档级上下文、绝对和对比评估、注意力检查、ESAAI预标注，以及静态和基于主动学习的分配策略。

Result: Pearmut移除了常见入门障碍，支持多语言任务评估，特别是机器翻译，使可靠的人类评估成为模型开发和诊断的实用常规组件。

Conclusion: Pearmut使端到端人类评估变得像自动评估一样容易，将可靠的人类评估从偶尔的努力转变为模型开发和诊断的实际常规组成部分。
...
### [82] [Enhancing Multilingual RAG Systems with Debiased Language Preference-Guided Query Fusion](https://arxiv.org/abs/2601.02956)
*Jeonghyun Park,Byeongjeong Kim,Seojin Hwang,Hwanhee Lee*

Main category: cs.CL

TL;DR: 该研究揭示了多语言检索增强生成系统中英语偏好的测量偏差，提出去偏语言偏好指标DeLP，并基于此开发了DELTA框架，利用单语对齐优化跨语言检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有mRAG系统表现出对英语等资源丰富语言的偏好，但研究发现这种测量受到评估基准中结构性先验的扭曲，包括曝光偏差、黄金答案可用性先验和文化先验，需要准确评估真正的语言偏好。

Method: 提出DeLP指标来显式排除结构性混杂因素，准确测量语言偏好；基于发现构建DELTA框架，利用单语对齐优化跨语言检索，采用轻量级高效的文本增强策略。

Result: DeLP分析显示先前报告的英语偏好主要是证据分布的结果而非模型固有偏差；DELTA框架在多种语言上一致优于英语枢纽方法和mRAG基线。

Conclusion: mRAG系统的语言偏好测量存在结构性偏差，实际偏好是单语对齐而非英语中心主义；DELTA框架通过利用单语对齐有效提升跨语言检索生成性能。
...
### [83] [LLM-Augmented Changepoint Detection: A Framework for Ensemble Detection and Automated Explanation](https://arxiv.org/abs/2601.02957)
...
<details>
  <summary>Details</summary>
Motivation: 解决变点检测领域的两个关键局限：1）单个检测方法在不同数据特征下表现互补，方法选择困难且易导致次优结果；2）缺乏对检测到的变化进行自动化、情境化的解释。

Method: 集成方法聚合十种不同的变点检测算法结果，结合LLM驱动的解释管道自动生成情境化叙述，将检测到的变点与现实历史事件关联。对于私有或领域特定数据，采用检索增强生成（RAG）解决方案基于用户提供的文档生成解释。

Result: 集成方法相比单个方法展现出更优的性能和鲁棒性，开源Python框架在金融、政治科学和环境科学等多个领域展示了实际应用价值。
...
### [84] [Low-Resource Heuristics for Bahnaric Optical Character Recognition Improvement](https://arxiv.org/abs/2601.02965)
*Phat Tran,Phuoc Pham,Hung Trinh,Tho Quan*

Main category: cs.CL

TL;DR: 本研究针对巴拿语文献数字化中的OCR识别问题，提出结合表格/非表格检测与概率后处理的综合方法，将识别准确率从72.86%提升至79.26%，为少数民族语言保护提供技术框架。


<details>
  <summary>Details</summary>
Motivation: 巴拿语作为越南、柬埔寨和老挝的少数民族语言，面临保存挑战，现有研究数据有限。纸质文档数字化过程中，因图像质量退化（破损、模糊）导致的OCR识别错误严重影响了信息检索系统的准确性。

Method: 提出综合方法：1) 应用先进的表格和非表格检测技术提升输入数据质量；2) 在OCR输出上采用基于概率的后处理启发式算法进行错误校正。

Result: 实验结果显示识别准确率从72.86%显著提升至79.26%，验证了所提方法的有效性。

Conclusion: 本研究为巴拿语保护提供了有价值的资源，同时建立的技术框架可推广应用于其他少数民族语言的数字化工作。
...
### [85] [Correct, Concise and Complete: Multi-stage Training For Adaptive Reasoning](https://arxiv.org/abs/2601.02972)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型通过增加推理时计算（如思维链）提升了推理能力，但思维链往往变得不必要地冗长，增加了计算成本却没有提升准确性，有时甚至降低性能，这种现象被称为"过度思考"。

Method: 提出多阶段高效推理方法：1) 通过拒绝采样或推理轨迹重格式化进行监督微调；2) 使用自适应长度惩罚进行强化学习；3) 引入轻量级奖励函数，惩罚第一个正确答案后生成的标记，但仅在有益时鼓励自我验证。

Result: 在七个不同推理任务上的评估显示：8B模型响应长度平均减少28%，32B模型减少40%，性能仅轻微下降1.6和2.5个点。在过度思考调整准确率曲线下面积（AUC_OAA）上得分为76.6，比基础模型高5个点，比次优方法高2.5个点。

Conclusion: 该方法在概念简单的同时，相比更复杂的现有高效推理方法实现了更优的权衡，有效解决了大语言模型推理中的过度思考问题，在保持准确性的同时显著减少了计算成本。
...
### [86] [Mechanistic Knobs in LLMs: Retrieving and Steering High-Order Semantic Features via Sparse Autoencoders](https://arxiv.org/abs/2601.02978)
*Ruikang Zhang,Shuo Wang,Qi Su*

Main category: cs.CL

TL;DR: 提出基于稀疏自编码器的框架，用于检索和操控与高级语言行为相关的语义可解释内部特征，通过对比特征检索管道实现模型行为的精确双向控制


<details>
  <summary>Details</summary>
Motivation: 机械可解释性研究已能识别和干预大语言模型的内部特征，但如何将这些内部特征与语言生成中复杂行为级语义属性的可靠控制联系起来仍是一个挑战

Method: 使用基于控制语义对立的对比特征检索管道，结合统计激活分析和基于生成的验证，从稀疏激活空间中提取单语义功能特征；以大五人格特质为案例研究

Result: 相比现有激活操控方法（如对比激活加法），该方法能实现精确、双向的模型行为控制，同时保持更好的稳定性和性能；发现"功能忠实性"效应

Conclusion: 大语言模型内化了高阶概念的深度集成表示，为复杂AI行为的调控提供了一条新颖、稳健的机械路径
...
### [87] [P-Check: Advancing Personalized Reward Model via Learning to Generate Dynamic Checklist](https://arxiv.org/abs/2601.02986)
*Kwangwook Seo,Dongha Lee*

Main category: cs.CL

TL;DR: P-Check是一个个性化奖励建模框架，通过训练可插拔的检查清单生成器来合成动态评估标准，使用偏好对比标准加权策略提升个性化判断的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的个性化奖励建模方法主要依赖用户交互历史，但将用户上下文视为静态或隐式条件信号，未能捕捉人类判断的动态性和多面性特征。
...
### [88] [Mechanistic Interpretability of Large-Scale Counting in LLMs through a System-2 Strategy](https://arxiv.org/abs/2601.02989)
...
TL;DR: 论文提出一种受System-2认知过程启发的测试时策略，将大型计数任务分解为LLM可可靠解决的小型子问题，以克服Transformer架构在计数任务上的系统性限制。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然在复杂数学问题上表现强劲，但在计数任务上存在系统性限制。这种限制源于Transformer架构的局限性，其中计数操作跨越多个层进行，导致对于较大计数问题由于深度约束而精度下降。

Method: 提出一种简单的测试时策略，受System-2认知过程启发，将大型计数任务分解为更小、独立的子问题。通过观察性和因果中介分析来理解这种System-2类策略的底层机制。机制分析识别出关键组件：潜在计数在每个部分的最终项表示中计算和存储，通过专用注意力头传输到中间步骤，并在最后阶段聚合以产生总计数。

Result: 实验结果表明，该策略使LLM能够超越架构限制，在大规模计数任务上实现高精度。该工作为理解LLM中的System-2计数提供了机制性洞察。

Conclusion: 这项工作为理解LLM中的System-2计数提供了机制性洞察，并提出了一种可推广的方法来改进和理解它们的推理行为。
...
### [89] [Stable-RAG: Mitigating Retrieval-Permutation-Induced Hallucinations in Retrieval-Augmented Generation](https://arxiv.org/abs/2601.02993)
*Qianchi Zhang,Hainan Zhang,Liang Pang,Hongwei Zheng,Zhiming Zheng*

Main category: cs.CL

TL;DR: Stable-RAG：通过估计检索文档排列敏感性并利用聚类中心表示来减少排列引起的幻觉，提高RAG系统的准确性和一致性


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法主要关注低质量检索的鲁棒性和位置偏置，但忽视了检索文档排列顺序对LLM输出的显著影响。研究发现即使黄金文档固定在第一位置，不同排列仍会导致答案大幅变化，这揭示了未充分探索的排列敏感性。

Method: 提出Stable-RAG方法：1）在多个检索顺序下运行生成器；2）对隐藏状态进行聚类；3）从捕捉主导推理模式的聚类中心表示解码；4）利用这些推理结果将幻觉输出对齐到正确答案，鼓励模型在不同文档排列下产生一致准确的预测。

Result: 在三个QA数据集上的实验表明，与基线相比，Stable-RAG显著提高了答案准确性、推理一致性，并在数据集、检索器和输入长度方面展现出更好的鲁棒泛化能力。

Conclusion: Stable-RAG通过解决检索文档排列敏感性问题，有效减少了RAG系统中的幻觉现象，提高了模型的稳定性和准确性，为构建更可靠的RAG系统提供了新思路。
...
### [90] [Large Reasoning Models Are (Not Yet) Multilingual Latent Reasoners](https://arxiv.org/abs/2601.02996)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在数学推理任务上的优异表现通常归因于其生成显式思维链解释的能力，但近期研究发现模型在完成文本推理步骤前就能得出正确答案，这表明存在潜在推理（内部非语言计算）。然而，这种现象在英语之外的多语言环境中的表现尚不清楚，因此需要系统研究多语言潜在推理的特征和机制。

Method: 采用基于截断的策略，在只给模型部分推理痕迹的情况下，检查正确答案如何随着推理步骤的展开而逐渐形成，从而测量逐步的潜在预测形成过程。同时进行表征分析，探究不同语言间的内部机制差异。

Result: 研究发现了明确的多语言潜在推理证据，但存在不均衡性：在资源丰富的语言中表现强劲，在低资源语言中较弱，在更难的基准测试上普遍较难观察到。尽管表面存在差异，但预测的内部演化在不同语言间高度一致，且与英语模式基本对齐，表明存在以英语为中心的潜在推理路径。

Conclusion: 大型推理模型确实存在多语言潜在推理能力，但这种能力在不同语言间分布不均，且整体遵循以英语为中心的推理模式。这一发现揭示了模型内部推理机制的语言依赖性，对理解多语言AI系统的认知过程具有重要意义。
...
### [91] [SentGraph: Hierarchical Sentence Graph for Multi-hop Retrieval-Augmented Question Answering](https://arxiv.org/abs/2601.03014)
*Junli Liang,Pengfei Zhou,Wangqiu Zhou,Wenjie Qing,Qi Zhao,Ziwen Wang,Qi Song,Xiangyang Li*

Main category: cs.CL

TL;DR: SentGraph：基于句子级图结构的RAG框架，通过建模句子间细粒度逻辑关系提升多跳问答性能


<details>
  <summary>Details</summary>
Motivation: 传统RAG在处理单跳问答时有效，但在多跳问答任务中存在局限性，需要结合多个文档的证据。现有的基于分块的检索方法通常提供不相关且逻辑不连贯的上下文，导致证据链不完整和推理错误。

Method: 提出SentGraph框架：1）离线构建分层句子图，采用修辞结构理论区分核心句和卫星句，并组织成带有跨文档实体桥接的主题级子图；2）在线检索时执行图引导的证据选择和路径扩展，检索细粒度句子级证据。

Result: 在四个多跳问答基准测试上进行广泛实验，证明了SentGraph的有效性，验证了显式建模句子级逻辑依赖关系对多跳推理的重要性。

Conclusion: SentGraph通过显式建模句子间的细粒度逻辑关系，有效解决了多跳问答中证据链不完整和推理错误的问题，为基于检索增强生成的多跳推理提供了新的解决方案。
...
### [92] [MMFormalizer: Multimodal Autoformalization in the Wild](https://arxiv.org/abs/2601.03017)
...
TL;DR: MMFormalizer是一个多模态自动形式化方法，通过自适应接地将视觉元素与数学物理实体结合，递归构建形式化命题，在物理推理任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统自动形式化方法主要处理文本数学，但现实世界中的物理问题需要从视觉元素推断隐藏约束（如质量、能量），因此需要将自动形式化扩展到多模态领域。

Method: 提出MMFormalizer方法，通过自适应接地整合现实世界数学和物理领域的实体，递归地从感知接地的原语构建形式化命题，采用自适应递归终止确保每个抽象都有视觉证据支持并锚定在维度或公理基础上。

Result: 在PhyX-AF基准测试（115个样本）上评估，前沿模型如GPT-5和Gemini-3-Pro在编译和语义准确性方面表现最佳，GPT-5在物理推理方面表现突出，几何领域仍然最具挑战性。
...
### [93] [Dementia-R1: Reinforced Pretraining and Reasoning from Unstructured Clinical Notes for Real-World Dementia Prognosis](https://arxiv.org/abs/2601.03018)
...
Conclusion: Dementia-R1通过冷启动强化学习策略有效解决了纵向痴呆症预后预测的挑战，在真实临床数据上表现出色，小模型也能达到大模型的性能水平。
...
### [94] [MedDialogRubrics: A Comprehensive Benchmark and Evaluation Framework for Multi-turn Medical Consultations in Large Language Models](https://arxiv.org/abs/2601.03023)
...
TL;DR: MedDialogRubrics是一个包含5200个合成患者病例和60000多个细粒度评估标准的基准测试，专门用于评估大型语言模型的多轮诊断能力，通过多智能体系统生成病例并采用基于证据医学的评估标准。


<details>
  <summary>Details</summary>
Motivation: 现有评估医疗对话AI信息收集和诊断推理能力的基准测试和评估框架缺乏严格评估，需要更系统的方法来评估大型语言模型在医疗对话中的表现。

Method: 1) 使用多智能体系统合成患者病例和主诉，不访问真实电子健康记录；2) 设计患者智能体，基于原子医学事实并配备动态指导机制检测和纠正幻觉；3) 提出结构化LLM和专家标注的评估标准生成流程，检索证据医学指南并使用拒绝采样确定"必须询问"项目。

Result: 对最先进模型进行全面评估显示，在多个评估维度上，当前模型面临重大挑战。结果表明，改进医疗对话需要对话管理架构的进步，而不仅仅是基础模型的增量调整。

Conclusion: MedDialogRubrics为评估医疗对话AI提供了系统基准，揭示了当前模型的局限性，并指出未来改进需要关注对话管理架构的创新。
...
### [95] [Reducing Hallucinations in LLMs via Factuality-Aware Preference Learning](https://arxiv.org/abs/2601.03027)
...
Method: F-DPO是DPO的简单扩展，仅使用二元事实标签：1）应用标签翻转转换，确保被选中的回复在事实性上不低于被拒绝的回复；2）添加事实感知边界，强调具有明显正确性差异的配对，当两个回复事实性相同时则退化为标准DPO。

Result: 在七个开源大语言模型（1B-14B）上，F-DPO相比基础模型和标准DPO持续提升事实性并降低幻觉率。Qwen3-8B的幻觉率降低5倍（0.424到0.084），事实性得分提升50%（5.26到7.90）。在TruthfulQA上，Qwen2.5-14B的MC1准确率提升17%（0.500到0.585），MC2准确率提升49%（0.357到0.531）。

Conclusion: F-DPO无需辅助奖励模型、token级标注或多阶段训练，就能有效减少大语言模型的幻觉问题，同时保持指令遵循能力，具有良好的泛化性能。
...
### [96] [NorwAI's Large Language Models: Technical Report](https://arxiv.org/abs/2601.03034)
*Jon Atle Gulla,Peng Liu,Lemei Zhang*

Main category: cs.CL

TL;DR: NorwAI团队开发了专门针对挪威语和斯堪的纳维亚语言的大语言模型家族，基于多种Transformer架构，通过从零预训练或持续预训练，并采用挪威语扩展分词器和高级后训练策略，以优化性能、增强鲁棒性并提高在各种实际任务中的适应性。


<details>
  <summary>Details</summary>
Motivation: 挪威语（约500万人使用）在许多自然语言处理的重要突破中代表性不足。为了弥补这一差距，需要开发专门针对挪威语和斯堪的纳维亚语言的模型。

Method: 基于GPT、Mistral、Llama2、Mixtral和Magistral等多样化的Transformer架构，使用挪威语扩展分词器，在25B-88.45B个token上进行从零预训练或持续预训练，采用高级后训练策略，并开发指令调优变体。

Result: 开发了专门针对挪威语和斯堪的纳维亚语言的模型家族，指令调优变体（如Mistral-7B-Instruct和Mixtral-8x7B-Instruct）展示了强大的助手式能力，表明其在交互式和领域特定应用中具有实际部署潜力。

Conclusion: NorwAI大语言模型已向北欧组织、公司和学生开放，供研究和实验使用。该报告详细记录了模型架构、训练数据、分词器设计、微调策略、部署和评估。
...
### [97] [BaseCal: Unsupervised Confidence Calibration via Base Model Signals](https://arxiv.org/abs/2601.03042)
...
Method: 提出两种方法：1) BaseCal-ReEval：将后训练LLM的响应输入基础LLM，用平均概率作为置信度；2) BaseCal-Proj：训练轻量级投影，将后训练LLM的隐藏状态映射回基础LLM的隐藏状态，再通过基础LLM输出层获得校准后的置信度

Result: 在五个数据集和三个LLM家族上的实验表明，BaseCal相比最佳无监督基线平均降低42.90%的预期校准误差
...
### [98] [Lil: Less is Less When Applying Post-Training Sparse-Attention Algorithms in Long-Decode Stage](https://arxiv.org/abs/2601.03043)
...
TL;DR: 稀疏注意力算法在解码阶段可能反而增加端到端复杂度，因为信息损失会导致序列显著变长，形成"少即是少"悖论。作者提出早期停止算法来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署规模扩大对推理效率提出高要求。传统方法将推理分解为预填充和解码阶段，其中解码阶段主导总延迟。现有稀疏注意力算法旨在降低解码阶段的时间和内存复杂度，但作者发现这些方法可能产生反效果。

Method: 作者通过实证和理论分析发现稀疏注意力会导致"少即是少"问题：信息损失常常导致序列显著变长。为缓解此问题，提出早期停止算法，在稀疏解码过程中检测信息损失超过信息增益的阈值。

Result: 早期停止算法在推理密集型基准测试中，将令牌消耗减少高达90%，同时准确率下降小于2%。

Conclusion: 稀疏注意力算法可能产生"少即是少"的悖论效应，反而增加端到端复杂度。提出的早期停止算法能有效缓解这一问题，显著减少令牌消耗同时保持高准确率。
...
### [99] [Detecting Hallucinations in Retrieval-Augmented Generation via Semantic-level Internal Reasoning Graph](https://arxiv.org/abs/2601.03052)
...
<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的检索增强生成系统虽然能减少事实性幻觉，但忠实性幻觉仍然存在。现有方法要么忽略了模型内部推理过程的捕捉，要么对这些特征处理粗糙，导致判别器难以学习。
...
### [100] [Do LLMs Encode Functional Importance of Reasoning Tokens?](https://arxiv.org/abs/2601.03066)
...
Method: 提出贪婪剪枝方法：基于似然保持的删除程序，迭代移除对指定目标下模型似然影响最小的推理标记，生成长度可控的推理链。

Result: 在蒸馏框架中评估剪枝推理，结果显示在匹配推理长度下，使用剪枝链训练的学生模型优于前沿模型监督的压缩基线。分析揭示了系统性的剪枝模式，并显示注意力分数可以预测贪婪剪枝排名。

Conclusion: 模型在推理标记上编码了非平凡的功能重要性结构，贪婪剪枝方法有效压缩推理链同时保持功能相关性，为理解模型内部推理机制提供了诊断工具。
...
### [101] [Learning to Diagnose and Correct Moral Errors: Towards Enhancing Moral Sensitivity in Large Language Models](https://arxiv.org/abs/2601.03079)
*Bocheng Chen,Han Zi,Xi Chen,Xitong Zhang,Kristen Johnson,Guangliang Liu*

Main category: cs.CL

TL;DR: 该论文提出两种语用推理方法，通过诊断道德良性与危险输入、纠正道德错误来增强大语言模型的道德敏感性，在道德相关基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 道德敏感性是人类道德能力的基础，指导个体调节日常行为。虽然已有许多方法试图将大语言模型与人类道德价值观对齐，但如何使其具备道德敏感性仍然极具挑战性。本文旨在回答：如何增强大语言模型的道德敏感性？

Method: 提出两种语用推理方法：1）诊断道德良性与危险输入；2）纠正道德错误。这些方法采用统一视角，不依赖语义多样且复杂的表层形式建模道德话语，而是基于推理负荷为设计语用推理程序提供原则性视角。

Result: 实证证据表明，提出的语用方法能够有效增强大语言模型的道德敏感性，在代表性道德相关基准测试中取得了强劲性能。

Conclusion: 通过语用推理方法增强大语言模型的道德敏感性是可行的，该方法为道德对齐提供了新的技术路径，在道德相关任务上表现出色。
...
### [102] [Grad-ELLM: Gradient-based Explanations for Decoder-only LLMs](https://arxiv.org/abs/2601.03089)
*Xin Huang,Antoni B. Chan*

Main category: cs.CL

TL;DR: Grad-ELLM：针对仅解码器Transformer LLMs的梯度归因方法，通过聚合注意力层梯度和注意力图生成热力图，无需修改架构，在多个任务上展现优越的忠实性
...
Method: 提出Grad-ELLM梯度归因方法：1) 聚合注意力层梯度获取通道重要性；2) 结合注意力图获取空间重要性；3) 生成每个生成步骤的热力图；4) 引入π-Soft-NC和π-Soft-NS两个改进的忠实性评估指标

Result: 在情感分类、问答和开放生成任务上评估不同模型，实验结果显示Grad-ELLM在忠实性方面持续优于其他归因方法

Conclusion: Grad-ELLM为仅解码器Transformer LLMs提供有效的梯度归因方法，通过专门的架构感知设计和改进的评估指标，显著提升了归因的忠实性
...
### [103] [Who Laughs with Whom? Disentangling Influential Factors in Humor Preferences across User Clusters and LLMs](https://arxiv.org/abs/2601.03103)
*Soichiro Murakami,Hidetaka Kamigaito,Hiroya Takamura,Manabu Okumura*

Main category: cs.CL

TL;DR: 该研究通过聚类分析日本创意游戏Oogiri中的用户幽默偏好，使用Bradley-Terry-Luce模型估计可解释的偏好因子权重，并探索大语言模型与特定用户群偏好的相似性。


<details>
  <summary>Details</summary>
Motivation: 幽默偏好存在显著的个体和文化差异，这使得使用大语言模型评估幽默变得复杂。研究旨在建模幽默偏好的异质性，并探索LLMs能否捕捉特定用户群体的偏好模式。

Method: 1. 使用日本创意响应游戏Oogiri的投票日志对用户进行聚类分析；2. 采用Bradley-Terry-Luce模型估计每个聚类对可解释偏好因子的权重；3. 通过提示LLMs选择更有趣的回应来获取其偏好判断；4. 使用角色提示引导LLMs的偏好朝向特定用户聚类。

Result: 1. 用户聚类展现出不同的偏好模式；2. LLMs的结果可以与特定用户聚类的偏好相似；3. 通过角色提示，可以引导LLMs的偏好朝向特定用户聚类。

Conclusion: 该研究成功建模了幽默偏好的异质性，并展示了LLMs能够模拟特定用户群体的幽默偏好，通过角色提示可以定向引导LLMs的偏好，为个性化幽默评估提供了方法。
...
### [104] [Discovering and Causally Validating Emotion-Sensitive Neurons in Large Audio-Language Models](https://arxiv.org/abs/2601.03115)
...
<details>
  <summary>Details</summary>
Motivation: 情感是语音交流的核心维度，但目前缺乏对现代大型音频语言模型内部如何编码情感机制的了解

Method: 在Qwen2.5-Omni、Kimi-Audio和Audio Flamingo 3三个开源模型中，使用频率、熵、幅度和对比度等多种神经元选择器，通过推理时干预（消融和增益放大）分析情感敏感神经元

Result: 发现一致的情感特异性特征：消融特定情感相关的神经元会显著降低该情感的识别准确率，而增益放大则会将预测导向目标情感；这些效应随干预强度系统性地扩展，且情感敏感神经元呈现非均匀的层间聚类

Conclusion: 研究提供了LALMs中情感决策的因果、神经元级解释，并强调针对性神经元干预作为可控情感行为的可操作手段
...
### [105] [ToxiGAN: Toxic Data Augmentation via LLM-Guided Directional Adversarial Generation](https://arxiv.org/abs/2601.03121)
*Peiran Li,Jan Fillies,Adrian Paschke*

Main category: cs.CL

TL;DR: ToxiGAN：一个结合对抗生成和LLM语义引导的类感知文本增强框架，用于毒性分类，通过两阶段定向训练和语义锚点解决GAN增强中的模式崩溃和语义漂移问题。


<details>
  <summary>Details</summary>
Motivation: 毒性语言数据的可控、类特定增强对于提高毒性分类的鲁棒性至关重要，但由于监督有限和分布偏斜，这仍然具有挑战性。现有方法难以同时保持语义相关性和类别特异性。

Method: 提出ToxiGAN框架，结合对抗生成和大型语言模型（LLMs）的语义引导。采用两阶段定向训练策略：1）使用LLM生成的中性文本作为语义锚点；2）动态选择中性样本提供平衡指导，并优化毒性样本与这些锚点分离以增强类别对比信号。

Result: 在四个仇恨言论基准测试中，ToxiGAN在宏观F1和仇恨F1指标上均取得最佳平均性能，一致优于传统和基于LLM的增强方法。消融和敏感性分析进一步证实了语义锚点和定向训练对增强分类器鲁棒性的益处。

Conclusion: ToxiGAN通过结合对抗生成和LLM语义引导，有效解决了毒性数据增强中的模式崩溃和语义漂移问题，为毒性分类提供了更鲁棒的数据增强方法，显著提升了分类性能。
...
### [106] [Improving Indigenous Language Machine Translation with Synthetic Data and Language-Specific Preprocessing](https://arxiv.org/abs/2601.03135)
*Aashish Dhawan,Christopher Driggers-Ellis,Christan Grant,Daisy Zhe Wang*

Main category: cs.CL

TL;DR: 该研究使用多语言翻译模型生成合成数据来增强美洲土著语言（瓜拉尼语、克丘亚语、艾马拉语）与西班牙语的平行语料库，以解决低资源语言缺乏训练数据的问题。


<details>
  <summary>Details</summary>
Motivation: 低资源土著语言通常缺乏有效的神经机器翻译所需的平行语料库。合成数据生成为数据稀缺环境提供了一种实用的缓解策略。

Method: 使用高容量多语言翻译模型生成合成句子对来增强现有的平行数据集；在仅使用精选数据和合成增强数据上微调多语言mBART模型；应用语言特定的预处理（包括正字法规范化和噪声感知过滤）以减少语料库伪影。

Result: 瓜拉尼语-西班牙语和克丘亚语-西班牙语翻译显示合成数据增强带来了chrF++指标的持续改进；艾马拉语的诊断实验突显了通用预处理对高度黏着性语言的局限性。

Conclusion: 合成数据增强能有效提升低资源土著语言的翻译质量，但需要针对高度黏着性语言开发更专门的预处理方法。
...
### [107] [Limited Linguistic Diversity in Embodied AI Datasets](https://arxiv.org/abs/2601.03136)
...
<details>
  <summary>Details</summary>
Motivation: 语言在视觉-语言-动作模型中扮演关键角色，但用于训练和评估这些系统的数据集的语料特征缺乏充分记录。本研究旨在系统审计VLA数据集，揭示其中指令的实际内容和语言多样性程度。

Method: 对多个广泛使用的VLA语料库进行系统性数据集审计，从互补维度量化指令语言：包括词汇多样性、重复和重叠、语义相似性以及句法复杂性。

Result: 分析显示许多数据集依赖高度重复、模板化的命令，结构变化有限，导致指令形式分布狭窄。数据集中的语言信号存在明显的局限性。

Conclusion: 这些发现为当前VLA训练和评估数据中的语言信号提供了描述性文档，旨在支持更详细的数据集报告、更原则性的数据集选择，以及针对扩大语言覆盖面的定向整理或增强策略。
...
### [108] [Self-Verification is All You Need To Pass The Japanese Bar Examination](https://arxiv.org/abs/2601.03144)
*Andrew Shin*

Main category: cs.CL

TL;DR: 首个通过日本司法考试的LLM模型：通过自验证方法在保持原考试格式和评分规则下超越官方及格线


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型快速发展，但在高度专业化和结构化的考试中实现可靠性能仍然面临挑战。日本司法考试是一个特别具有挑战性的基准，不仅需要高级法律推理能力，还要严格遵守涉及多个命题联合评估的复杂答案格式。现有方法虽然通过将问题分解为简单的真假判断有所改进，但未在原考试格式和评分方案下进行系统评估。

Method: 提出一个自验证模型，基于新构建的数据集进行训练，该数据集忠实复制了考试的真实格式和评估标准。模型能够在保持原问题结构和评分规则的情况下，通过自验证机制确保答案的一致性。

Result: 模型在真实考试标准下评估时能够超过官方及格分数，这是首个已知的在不改变原问题结构或评分规则的情况下通过日本司法考试的LLM。与多智能体推理和基于分解的监督等替代策略相比，这些方法未能达到可比性能。

Conclusion: 研究强调了格式忠实监督和一致性验证的重要性，表明精心设计的单模型方法在高风险专业推理任务中可以超越更复杂的系统。格式忠实监督对于专业考试评估至关重要。
...
### [109] [Decoupling the Effect of Chain-of-Thought Reasoning: A Human Label Variation Perspective](https://arxiv.org/abs/2601.03154)
*Beiduo Chen,Tiancheng Hu,Caiqi Zhang,Robert Litschko,Anna Korhonen,Barbara Plank*

Main category: cs.CL

TL;DR: 研究发现：长链式思维（CoT）在单答案任务中表现出色，但在处理人类标签变异（需要捕捉概率模糊性而非解决它）时效果有限。CoT主要作为LLM的决策制定者而非分布校准器。


<details>
  <summary>Details</summary>
Motivation: 尽管推理调优的LLMs在单答案任务中表现出色，但它们在建模人类标签变异方面的能力尚未充分探索。人类标签变异需要捕捉概率模糊性而非解决它，这与传统的单答案任务有本质区别。

Method: 通过系统性的解耦实验，在基于分布的任务上使用交叉CoT实验，将推理文本的影响与模型内在先验分离开来。进行逐步分析来观察CoT在推理过程中的影响变化。

Result: 观察到明显的"解耦机制"：CoT改善分布对齐，但最终准确率由CoT内容决定（贡献99%方差），而分布排名由模型先验主导（超过80%）。逐步分析显示CoT对准确率的影响在推理过程中单调增长，但分布结构主要由LLM内在先验决定。

Conclusion: 长链式思维主要作为LLM的决策制定者来确定最佳选项，但在模糊任务中无法作为细粒度的分布校准器。这表明CoT在捕捉概率模糊性方面存在局限性。
...
### [110] [Can Embedding Similarity Predict Cross-Lingual Transfer? A Systematic Study on African Languages](https://arxiv.org/abs/2601.03168)
*Tewodros Kederalah Idris,Prasenjit Mitra,Roald Eiselen*

Main category: cs.CL

TL;DR: 本文系统评估了五种嵌入相似性度量在跨语言迁移中的预测能力，发现余弦差距和检索类度量能可靠预测迁移成功率，而CKA预测能力较弱，且不同模型间存在辛普森悖论现象。


<details>
  <summary>Details</summary>
Motivation: 针对低资源非洲语言构建NLP系统时，跨语言迁移至关重要，但实践者缺乏可靠的方法来选择源语言。需要系统评估不同嵌入相似性度量在预测跨语言迁移成功方面的有效性。

Method: 在816个迁移实验中系统评估了五种嵌入相似性度量，涵盖三个NLP任务、三个非洲中心多语言模型和来自四个语系的12种语言。使用了余弦差距、检索类度量（P@1、CSLS）和CKA等指标。

Result: 余弦差距和检索类度量（P@1、CSLS）能可靠预测迁移成功率（ρ=0.4-0.6），而CKA预测能力可忽略（ρ≈0.1）。关键发现是当跨模型汇总数据时，相关性符号会反转（辛普森悖论），因此必须按模型进行验证。嵌入度量与URIEL语言类型学具有相当的预测能力。

Conclusion: 研究结果为源语言选择提供了具体指导，强调了模型特定分析的重要性。实践者应使用余弦差距或检索类度量进行源语言选择，但必须在每个模型上单独验证，避免跨模型汇总导致的辛普森悖论。
...
### [111] [Maximizing Local Entropy Where It Matters: Prefix-Aware Localized LLM Unlearning](https://arxiv.org/abs/2601.03190)
*Naixin Zhai,Pengyang Shao,Binbin Zheng,Fei Shen,Long Bai,Xun Yang*

Main category: cs.CL

TL;DR: PALU是一种针对大语言模型的机器遗忘框架，通过局部熵最大化在时间和词汇维度上实现敏感知识的遗忘，同时最小化对模型整体性能的影响。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法通常对所有响应token不加区分地处理，并在整个词汇表上强制不确定性，这导致不必要的性能下降并将优化扩展到与内容无关的区域。

Method: 提出PALU框架，基于局部熵最大化目标，发现仅抑制敏感前缀就足以切断因果生成链接，且仅平坦化top-k logits就足以在关键子空间中最大化不确定性。

Result: 大量实验验证PALU在遗忘效果和效用保持方面优于现有最先进基线方法。

Conclusion: PALU通过避免在完整词汇表和参数空间上进行冗余优化，实现了对敏感知识的有效遗忘，同时最小化了对通用模型性能的附带损害。
...
### [112] [MemRL: Self-Evolving Agents via Runtime Reinforcement Learning on Episodic Memory](https://arxiv.org/abs/2601.03192)
...
TL;DR: MemRL框架通过非参数强化学习在情景记忆上实现智能体自我进化，解决LLM在持续学习中的稳定性-可塑性困境


<details>
  <summary>Details</summary>
Motivation: 人类智能的核心特征是通过情景模拟从过去经验中合成新任务解决方案，而现有LLM难以实现这种自我进化。微调计算成本高且容易灾难性遗忘，基于记忆的方法依赖被动语义匹配常检索到噪声。

Method: 提出MemRL框架，将冻结LLM的稳定推理与可塑性演化记忆分离。采用两阶段检索机制：先通过语义相关性筛选候选，再基于学习到的Q值（效用）选择。通过环境反馈以试错方式持续优化效用值。
...
Conclusion: MemRL通过非参数强化学习在情景记忆上的应用，使智能体能够自我进化，解决了LLM持续学习中的关键挑战，为构建更接近人类智能的AI系统提供了新途径。
...
### [113] [X-MuTeST: A Multilingual Benchmark for Explainable Hate Speech Detection and A Novel LLM-consulted Explanation Framework](https://arxiv.org/abs/2601.03194)
...
TL;DR: 提出X-MuTeST框架，结合大语言模型语义推理与传统注意力增强技术，用于印地语、泰卢固语和英语的仇恨言论检测，通过人类标注的理性标注提升分类性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 社交媒体仇恨言论检测在准确性和可解释性方面面临挑战，特别是对于未被充分研究的印度语言（如印地语、泰卢固语），需要开发既能提高检测性能又能提供可解释性支持的方法。

Method: 提出X-MuTeST框架：1）结合大语言模型的高层语义推理与传统注意力增强技术；2）使用人类标注的单词级理性标注；3）通过计算原始文本与n-gram预测概率差异生成解释；4）将LLM解释与X-MuTeST解释结合；5）利用人类理性标注优化模型注意力。

Result: 1）在训练中使用人类理性标注能同时提升分类性能和可解释性；2）结合人类理性标注与可解释性方法进一步优化模型注意力；3）构建了包含6,004个印地语、4,492个泰卢固语和6,334个英语样本的数据集，提供单词级理性标注；4）在可解释性评估指标（Token-F1、IOU-F1、Comprehensiveness、Sufficiency）上表现良好。

Conclusion: X-MuTeST框架通过结合大语言模型语义推理和传统注意力技术，有效提升了仇恨言论检测的性能和可解释性，特别关注资源匮乏语言，推动了跨语言仇恨言论检测的发展。
...
### [114] [DIP: Dynamic In-Context Planner For Diffusion Language Models](https://arxiv.org/abs/2601.03199)
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型虽然具有强大的自然语言处理能力，但由于其双向注意力机制，随着上下文长度增加会带来巨大的计算成本。现有方法需要将所有上下文示例预先提供，导致推理效率低下。

Method: 提出动态上下文规划器(DIP)，利用扩散生成范式允许在生成过程中动态调整上下文的特性，在生成时动态选择和插入上下文示例，而不是预先提供所有示例。

Result: DIP在保持生成质量的同时，实现了相对于标准推理高达12.9倍的推理加速，相对于KV缓存增强推理也有1.17倍的加速。

Conclusion: 通过利用扩散语言模型的生成特性进行动态上下文优化，可以显著提高推理效率，为扩散语言模型的实际应用提供了有效的解决方案。
...
### [115] [UltraLogic: Enhancing LLM Reasoning through Large-Scale Data Synthesis and Bipolar Float Reward](https://arxiv.org/abs/2601.03205)
*Yile Liu,Yixian Liu,Zongwei Li,Yufei Huang,Xinhua Feng,Zhichao Hu,Jinglu Hu,Jianfeng Yan,Fengzong Lian,Yuhong Liu*

Main category: cs.CL

TL;DR: 本文提出UltraLogic框架，通过代码化求解方法将问题的逻辑核心与自然语言表达解耦，自动生成高质量推理数据，并引入双极浮点奖励机制解决二元奖励稀疏性问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在自然语言处理方面展现出潜力，但需要多步逻辑、规划和验证的复杂通用推理仍然是关键瓶颈。现有强化学习与可验证奖励方法在特定领域取得成功，但缺乏大规模、高质量、难度校准的通用推理数据。

Method: 提出UltraLogic框架：1）通过代码化求解方法将问题逻辑核心与自然语言表达解耦，自动生成高质量数据；2）包含数百种独特任务类型和跨十个难度级别的自动校准流程；3）引入双极浮点奖励机制，使用分级惩罚区分完美响应与存在逻辑缺陷的响应。

Result: 实验表明：1）任务多样性是推理能力提升的主要驱动力；2）双极浮点奖励结合难度匹配策略显著提高训练效率，引导模型达到全局逻辑最优。

Conclusion: UltraLogic框架通过解耦逻辑核心与语言表达、自动生成高质量推理数据、引入双极浮点奖励机制，有效解决了通用推理中的数据稀缺和训练效率问题，为大语言模型的复杂推理能力提升提供了有效解决方案。
...
### [116] [MalruleLib: Large-Scale Executable Misconception Reasoning with Step Traces for Modeling Student Thinking in Mathematics](https://arxiv.org/abs/2601.03217)
*Xinghe Chen,Naiming Liu,Shashank Sonkar*

Main category: cs.CL

TL;DR: MalruleLib框架将数学学习中的系统性错误概念转化为可执行程序，生成包含正确推理和错误推理的双路径追踪，用于评估AI模型对学生错误概念的推理能力。


<details>
  <summary>Details</summary>
Motivation: 学生在数学学习中经常出现系统性错误——他们会应用一套连贯但错误的程序，并在不同情境中重复这些错误。现有AI系统缺乏对这种错误概念进行建模和推理的能力，需要专门的基础设施来诊断学生的根本性误解。

Method: 基于67个学习科学和数学教育文献，将文档化的错误概念转化为可执行程序（malrules），构建了包含101个错误规则和498个参数化问题模板的框架。生成包含正确推理路径和错误推理路径的双路径追踪，并定义了Malrule Reasoning Accuracy（MRA）作为核心评估指标。

Result: 在9个语言模型（4B-120B）上测试，准确率从直接问题解决的66%下降到跨模板错误概念预测的40%。使用MalruleLib观察到跨模板性能下降10-21%，而提供学生步骤追踪可将预测准确率提高3-15%。框架可生成超过100万个实例。

Conclusion: MalruleLib作为教育AI的基础设施，能够跨情境建模学生程序性错误，支持诊断和针对性反馈。该框架为理解学生错误概念提供了可扩展的监督和受控评估方法。
...
### [117] [Multi-RADS Synthetic Radiology Report Dataset and Head-to-Head Benchmarking of 41 Open-Weight and Proprietary Language Models](https://arxiv.org/abs/2601.03232)
...
TL;DR: 该研究创建了RXL-RADSet基准数据集，比较了开源小语言模型与专有模型在放射学报告系统(RADS)自动分类中的表现，发现大参数SLMs在指导提示下可接近专有模型性能。


<details>
  <summary>Details</summary>
Motivation: 放射学报告和数据系统(RADS)标准化了风险沟通，但自动化RADS分配面临挑战：指南复杂、输出格式限制、跨RADS框架和模型大小的基准测试有限。需要创建验证的多RADS基准并比较不同模型的性能。

Method: 创建包含1,600份合成放射学报告的RXL-RADSet数据集，涵盖10种RADS系统和多种模态。报告由LLMs生成并经过两阶段放射科医生验证。评估了41个量化SLMs(0.135-32B参数)和GPT-5.2，使用固定指导提示。主要终点是有效性和准确性，次要分析比较指导提示与零样本提示。

Result: GPT-5.2在指导提示下达到99.8%有效性和81.1%准确性。SLMs总体达到96.8%有效性和61.1%准确性，20-32B参数的最佳SLMs达到约99%有效性和70%+准确性。性能随模型规模增加而提升，指导提示相比零样本显著改善性能。

Conclusion: RXL-RADSet提供了放射科医生验证的多RADS基准；大型SLMs(20-32B)在指导提示下可接近专有模型性能，但对于更高复杂度的方案仍存在差距。
...
### [118] [Automated Semantic Rules Detection (ASRD) for Emergent Communication Interpretation](https://arxiv.org/abs/2601.03254)
*Bastien Vanderplaetse,Xavier Siebert,Stéphane Dupont*

Main category: cs.CL

TL;DR: 本文提出ASRD算法，用于自动检测多智能体系统中涌现语言中的语义规则，提高其可解释性


<details>
  <summary>Details</summary>
Motivation: 多智能体系统中的涌现通信研究关注自主智能体如何自发发展通信策略，但现有研究很少关注涌现语言的可解释性问题
...
Conclusion: ASRD算法为多智能体涌现通信的可解释性研究提供了有效工具，能够自动检测语义规则并简化分析过程
...
### [119] [Trading with market resistance and concave price impact](https://arxiv.org/abs/2601.03215)
*Youssef Ouazzani Chahdi,Nathan De Carvalho,Grégoire Szymanski*

Main category: q-fin.TR

TL;DR: 论文研究市场影响模型下的最优交易问题，其中市场阻力由能够检测元订单并反向交易的成熟交易者产生，通过固定点方程描述阻力函数对交易速率的影响。


<details>
  <summary>Details</summary>
Motivation: 研究在存在能够检测元订单并反向交易的成熟交易者时，如何优化交易策略以应对市场阻力和价格过度反应，这是高频交易和算法交易中的重要问题。

Method: 建立具有内生市场阻力的市场影响模型，使用幂律传播函数驱动凹瞬态影响，通过固定点方程描述阻力函数对交易速率的影响，推导出满足最优交易策略的一阶最优性条件的（非线性）随机Fredholm方程。

Result: 在线性阻力函数情况下建立了最优控制的存在性和唯一性，在严格凸阻力函数情况下通过利润损失泛函的强制性和弱下半连续性获得了存在性结果，提出了求解非线性随机Fredholm方程的迭代方案并证明了指数收敛率。

Conclusion: 该研究为存在市场阻力情况下的最优交易提供了理论框架和数值求解方法，数值实验验证了收敛行为并展示了在不同衰减曲线和市场阻力规范下的最优往返策略。
...
### [120] [Transform and Entropy Coding in AV2](https://arxiv.org/abs/2601.02712)
...
TL;DR: AV2是AV1的继任者，旨在提供显著压缩增益和主观质量改进，同时保持低复杂度编码器/解码器操作。本文描述了AV2的变换、量化和熵编码设计，包括重新设计的变换核、数据驱动变换、扩展的变换分区以及模式&系数依赖的变换信号。


<details>
  <summary>Details</summary>
Motivation: AV2的主要目标是提供比AV1更显著的压缩增益和主观质量改进，同时保持低复杂度的编码器和解码器操作，为视频应用提供最高质量的视频体验。

Method: AV2引入了多项新的编码工具：帧内/帧间次级变换(IST)、网格编码量化(TCQ)、自适应变换编码(ATC)、概率适应率调整(PARA)、前向跳过编码(FSC)、跨色度分量变换(CCTX)、奇偶隐藏(PH)工具和改进的无损编码。还包括重新设计的变换核、数据驱动变换、扩展的变换分区和模式&系数依赖的变换信号。

Result: AV2能够在显著降低比特率的同时，为视频应用提供最高质量的视频体验。

Conclusion: AV2作为AV1的继任者，通过引入先进的变换、量化和熵编码工具，实现了显著的压缩增益和质量改进，同时保持了低复杂度特性，代表了视频编码技术的重要进展。
...
### [121] [Deep Learning Superresolution for 7T Knee MR Imaging: Impact on Image Quality and Diagnostic Performance](https://arxiv.org/abs/2601.02436)
*Pinzhen Chen,Libo Xu,Boyang Pan,Jing Li,Yuting Wang,Ran Xiong,Xiaoli Gou,Long Qing,Wenjing Hou,Nan-jie Gong,Wei Chen*

Main category: eess.IV

TL;DR: 深度学习超分辨率技术可提升7T膝关节MRI的主观图像质量，但与标准低分辨率成像相比并未提高诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 虽然深度学习超分辨率技术可能增强肌肉骨骼MRI图像质量，但其在7T膝关节成像中的诊断价值尚不明确，需要评估超分辨率图像与低分辨率、高分辨率图像在图像质量和诊断性能方面的差异。

Method: 前瞻性研究纳入42名参与者，进行7T膝关节MRI扫描，获取低分辨率（0.8×0.8×2 mm³）和高分辨率（0.4×0.4×2 mm³）序列。使用混合注意力Transformer模型从低分辨率数据生成超分辨率图像。三名放射科医生评估图像质量、解剖结构清晰度和膝关节病变检测。10例病例以关节镜检查为金标准。

Result: 超分辨率图像的整体质量优于低分辨率图像（中位数评分5 vs 4，P<0.001），噪声低于高分辨率图像（5 vs 4，P<0.001）。软骨、半月板和韧带的可见性在超分辨率和高分辨率图像中均优于低分辨率图像（P<0.001）。关节内病变的检出率和诊断性能（敏感性、特异性、AUC）在三种图像类型间相似（P≥0.095）。

Conclusion: 深度学习超分辨率技术改善了7T膝关节MRI的主观图像质量，但与标准低分辨率成像相比并未提高诊断准确性。
...
### [122] [Comparative Analysis of Binarization Methods For Medical Image Hashing On Odir Dataset](https://arxiv.org/abs/2601.02564)
*Nedim Muzoglu*

Main category: eess.IV

TL;DR: 在ODIR数据集上评估四种二值化方法，SDH以32位编码获得最佳性能（mAP@100=0.9184），优于LSH、ITQ和KSH，且比现有方法使用更少比特数达到接近SOTA的检索精度。


<details>
  <summary>Details</summary>
Motivation: 评估不同二值化方法在医学图像检索中的性能，寻找在准确性、存储效率和计算效率之间达到最佳平衡的方法，为医疗图像检索和设备库存管理提供实用解决方案。

Method: 在ODIR数据集上使用深度特征嵌入评估四种二值化方法：局部敏感哈希（LSH）、迭代量化（ITQ）、基于核的监督哈希（KSH）和监督离散哈希（SDH）。

Result: SDH表现最佳，仅用32位编码获得mAP@100为0.9184，优于其他方法。与先前研究相比，使用更少比特数达到接近最先进水平的检索精度：Fang等人报告0.7528（48位）和0.8856（48位），Wijesinghe等人达到94.01（256位）。

Conclusion: SDH是测试方法中最有效的二值化方法，为医学图像检索和设备库存管理提供了准确性、存储效率和计算效率之间的实用平衡。
...
### [123] [Annealed Langevin Posterior Sampling (ALPS): A Rapid Algorithm for Image Restoration with Multiscale Energy Models](https://arxiv.org/abs/2601.02594)
*Jyothi Rikhab Chand,Mathews Jacob*

Main category: eess.IV

TL;DR: 该论文提出了一种快速蒸馏策略，将预训练扩散模型的优势转移到多尺度能量基模型中，并基于此开发了退火朗文后验采样算法，用于解决成像中的逆问题。


<details>
  <summary>Details</summary>
Motivation: 能量基模型虽然具有可解释的能量景观和组合结构，适合解决成像逆问题，但历史上存在计算成本高和训练不稳定的缺点。需要克服这些限制，同时保持EBMs的优势。

Method: 1. 引入快速蒸馏策略，将预训练扩散模型的优势转移到多尺度EBMs中；2. 利用EBMs的组合性，提出退火朗文后验采样算法，用于最大后验估计、最小均方误差估计和不确定性量化；3. 在静态后验分布上执行退火，而不是像扩散模型那样在潜变量上使用复杂引导策略。

Result: 在图像修复和MRI重建实验中，该方法在准确性和效率方面匹配或超越了基于扩散模型的基线方法，同时支持最大后验恢复。蒸馏后的EBMs实现了高效采样并保持了可解释性和组合性。

Conclusion: 该框架为成像中的逆问题提供了一个可扩展且原则性的解决方案，具有在科学和临床环境中实际部署的潜力。ALPS算法结合了扩散模型的效率和EBMs的可解释性优势。
...
### [124] [Lesion Segmentation in FDG-PET/CT Using Swin Transformer U-Net 3D: A Robust Deep Learning Framework](https://arxiv.org/abs/2601.02864)
*Shovini Guha,Dwaipayan Nandi*

Main category: eess.IV

TL;DR: SwinUNet3D：基于Swin Transformer的3D U-Net框架，用于FDG-PET/CT扫描中的病灶分割，在AutoPET III数据集上显著优于传统3D U-Net，Dice分数达0.88，推理速度更快。


<details>
  <summary>Details</summary>
Motivation: PET/CT成像中准确自动的病灶分割对癌症诊断和治疗规划至关重要，需要能够同时捕捉全局上下文和精细解剖细节的模型。

Method: 提出SwinUNet3D框架，将Swin Transformer的移位窗口自注意力机制与U-Net风格的跳跃连接相结合，用于FDG-PET/CT扫描的3D病灶分割。

Result: 在AutoPET III FDG数据集上，SwinUNet3D获得Dice分数0.88和IoU 0.78，显著优于3D U-Net（Dice 0.48，IoU 0.32），推理速度更快，对小病灶和不规则病灶检测效果更好。

Conclusion: SwinUNet3D为PET/CT病灶分割提供了高效稳健的方法，推动了基于Transformer的模型在肿瘤学成像工作流程中的集成，为未来多示踪剂、多中心评估奠定了基础。
...
### [125] [Textual Explanations and Their Evaluations for Reinforcement Learning Policy](https://arxiv.org/abs/2601.02514)
*Ahmad Terra,Mohit Ahmed,Rafia Inam,Elena Fersman,Martin Törngren*

Main category: cs.AI

TL;DR: 本文提出了一种新颖的可解释强化学习框架，通过LLM生成文本解释，将其转换为透明规则，并通过两种精炼技术提高解释质量，在三个开源环境和电信用例中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 虽然文本解释对人类易于理解，但确保其正确性仍然具有挑战性，现有评估方法有限。需要一种能够生成高质量文本解释并系统评估其正确性的XRL框架。

Method: 提出新颖XRL框架：1) 使用LLM生成文本解释；2) 聚类技术识别频繁条件；3) 将条件转换为透明规则；4) 提出两种精炼技术改进解释质量；5) 自动谓词生成器确定状态语义信息；6) 可融入专家知识。

Result: 在三个开源环境和电信用例中验证了框架的有效性。生成的透明规则在某些任务上能达到满意性能，改进了现有自主策略解释方法的局限性，实现了对文本解释的系统定量评估。

Conclusion: 该框架解决了文本解释正确性评估的挑战，提供了系统化、可量化的评估方法，为XRL领域提供了有价值的见解，并展示了工业应用潜力。
...
### [126] [An Empirical Study of On-Device Translation for Real-Time Live-Stream Chat on Mobile Devices](https://arxiv.org/abs/2601.02641)
*Jeiyoon Park,Daehwan Lee,Changmin Yeo,Yongshin Han,Minseop Kim*

Main category: cs.AI

TL;DR: 该研究通过构建LiveChatBench基准测试，系统评估了设备端AI模型在实际部署中的CPU利用率、热条件等关键因素，发现虽然需要考虑受限的部署环境，但设备端模型在特定任务上能达到与GPT-5.1等商业模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管设备端AI模型效率高，但关于实际部署所需的关键因素（如设备CPU利用率和热条件）的研究很少。研究旨在解决设备端模型在实际服务部署中必须应对的两个关键问题：模型选择和资源消耗，以及设备端模型的领域适应能力。

Method: 研究聚焦于直播聊天消息翻译任务，手动构建了包含1000个韩英平行句对的LiveChatBench基准测试。通过在5种移动设备上进行广泛实验，系统评估了不同设备端模型的资源消耗和性能表现。

Result: 实验表明，虽然服务大规模异质用户群需要仔细考虑高度受限的部署环境和模型选择，但提出的方法在针对性任务上能达到与GPT-5.1等商业模型相当的性能。研究为设备端AI社区提供了有价值的见解。

Conclusion: 设备端AI模型在实际部署中需要考虑CPU利用率、热条件等关键因素，通过合理的模型选择和领域适应，设备端模型在特定任务上能达到与大型商业模型相当的性能，为实际应用提供了可行性。
...
TL;DR: HAPO框架通过动态归因机制、语义单元优化和多模态友好流程，解决现有提示优化方法中的提示漂移和可解释性问题，提升优化效率。
...
Result: 在单/多图像QA(如OCRV2)和复杂任务分析(如BBH)等场景中，HAPO展示了增强的优化效率，优于可比较的自动化提示优化方法，并为可扩展的提示工程建立了可扩展范式。

Conclusion: HAPO框架通过解决提示漂移和可解释性问题，提供了一种有效的结构化提示优化方法，为可扩展的提示工程建立了新范式。
...
### [128] [Learning User Preferences Through Interaction for Long-Term Collaboration](https://arxiv.org/abs/2601.02702)
*Shuhaib Mehri,Priyanka Kargupta,Tal August,Dilek Hakkani-Tür*

Main category: cs.AI

TL;DR: MultiSessionCollab基准测试评估智能体在多会话中学习用户偏好以提升协作质量的能力，通过记忆机制实现长期协作改进
...
Method: 提出MultiSessionCollab基准测试，开发配备持久记忆的长期协作智能体，该记忆随交互经验积累而精炼用户偏好。利用用户模拟器行为生成学习信号，训练智能体生成更全面的反思并更有效地更新记忆。

Result: 实验表明配备记忆的智能体显著改善长期协作：任务成功率更高、交互更高效、用户努力减少。人类用户研究证实记忆在真实场景中提升用户体验。

Conclusion: 记忆机制对智能体在多会话协作中学习用户偏好至关重要，MultiSessionCollab基准为评估和改进长期协作智能体提供了有效框架。
...
### [129] [The Path Ahead for Agentic AI: Challenges and Opportunities](https://arxiv.org/abs/2601.02749)
*Nadia Sibai,Yara Ahmed,Serry Sibaee,Sawsan AlHalawani,Adel Ammar,Wadii Boulila*

Main category: cs.AI

TL;DR: 该章节探讨了LLM从被动文本生成器向自主智能体系统的演进，分析了实现自主行为所需的核心组件架构，并评估了应用挑战与研究重点。


<details>
  <summary>Details</summary>
Motivation: 研究LLM从语言理解系统向自主智能体系统的架构转变，探索如何通过整合规划、记忆、工具使用等能力实现真正的自主行为，填补从语言理解到自主行动的技术空白。

Method: 通过分析LLM从统计模型到基于Transformer系统的架构演进，识别实现智能体行为的关键能力（长程推理、上下文感知、自适应决策），构建包含感知、记忆、规划、工具执行的核心组件集成框架。

Result: 提出了三个主要贡献：1）LLM通过推理-行动-反思循环向智能体能力扩展的合成分析；2）连接LLM与自主行为的核心组件集成框架；3）对应用及安全、对齐、可靠性、可持续性等挑战的批判性评估。

Conclusion: 实现负责任的发展需要在技术鲁棒性、可解释性和伦理保障方面同步进展，解决可验证规划、可扩展多智能体协调、持久记忆架构和治理框架等关键研究重点，以在实现潜力的同时减轻错位和意外后果的风险。
...
TL;DR: HAL框架通过可解释的数据驱动奖励来对齐语言模型与对话人类相似性，使用对比对话数据提取显式对话特征，组合成标量分数作为透明奖励信号进行对齐优化。


<details>
  <summary>Details</summary>
Motivation: 对话人类相似性在人机交互中至关重要，但难以定义、测量和优化，导致改进主要依赖规模或广泛监督训练，而非针对性对齐。

Method: 从对比对话数据中提取显式对话特征，组合成紧凑标量分数作为透明奖励信号，使用标准偏好优化方法进行对齐。

Result: 对齐后的模型在不同规模下保持整体性能，在大规模人类评估中更频繁地被感知为人类相似对话；HAL支持对齐行为检查和意外效应诊断。
...
TL;DR: 本文提出了一种基于因果图增强的检索增强生成系统，通过整合显式因果推理与双层知识图谱，在系统综述任务中实现零幻觉和高准确率。


<details>
  <summary>Details</summary>
Motivation: 系统综述对于循证医学至关重要，但每年150万+的出版物无法手动审查。现有AI方法在系统综述任务中存在幻觉问题，错误率在2-40%之间，这在影响患者护理时是不可接受的。

Method: 提出因果图增强的检索增强生成系统，整合显式因果推理与双层知识图谱，采用证据优先协议确保每个因果主张都能追溯到检索到的文献，并自动生成可视化干预-结果路径的有向无环图。

Result: 在234篇痴呆症运动摘要的评估中，CausalAgent达到95%准确率、100%检索成功率和零幻觉，而基线AI只有34%准确率和10%幻觉率。自动因果图实现了显式机制建模、可视化合成和增强的可解释性。
...
TL;DR: 该论文提出了一种神经符号深度强化学习方法，通过整合符号知识来提高样本效率和泛化能力，将简单任务中的部分策略作为先验知识加速复杂任务的学习。


<details>
  <summary>Details</summary>
Motivation: 当前深度强化学习算法需要大量训练数据且泛化能力有限，难以扩展到大规模或未见过的任务场景。为了解决这些问题，研究者希望通过整合符号知识来改善样本效率和泛化性能。

Method: 提出神经符号深度强化学习方法：1) 将简单任务中定义的部分策略表示为逻辑规则；2) 通过在线推理指导训练过程：在探索阶段偏置动作分布，在利用阶段重新缩放Q值；3) 将这些部分策略作为先验知识迁移到更复杂的任务中。

Result: 在网格世界环境的各种变体（包括完全可观测和部分可观测设置）中进行了实证验证，相比最先进的奖励机制基线方法，该方法表现出更好的性能，特别是在稀疏奖励环境和长规划视野任务中加速了收敛。

Conclusion: 神经符号整合方法不仅提高了样本效率和泛化能力，还增强了可解释性和可信度，为深度强化学习在复杂环境中的应用提供了有前景的方向。
...
TL;DR: 该论文提出了M3MAD-Bench，一个用于评估多智能体辩论方法的统一可扩展基准，涵盖多领域任务、多模态输入和多维度指标，解决了现有研究评估碎片化和局限于单模态的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体辩论研究存在两个根本局限：评估在碎片化且不一致的设置下进行，阻碍了公平比较；主要局限于依赖纯文本输入的单模态场景。需要建立一个统一的评估基准来解决这些问题。

Method: 提出M3MAD-Bench基准，建立标准化协议覆盖五个核心任务领域：知识、数学、医学、自然科学和复杂推理；系统覆盖纯文本和视觉语言数据集，支持跨模态比较；评估9个不同架构、规模和模态能力的基础模型；除了准确性，还包含效率导向的指标如token消耗和推理时间。

Result: 通过大量实验，系统性地洞察了多智能体辩论在纯文本和多模态场景下的有效性、鲁棒性和效率。基准提供了性能-成本权衡的整体视图。
...
Method: ReTreVal框架整合了树状思维探索、自我精炼、LLM批判评分和反思记忆。它根据问题复杂度构建自适应深度的结构化推理树，每个节点都经过迭代的自我批判和精炼，由明确的LLM生成反馈指导。双重验证机制评估每个节点的推理质量、连贯性和正确性，同时将成功推理路径和失败模式的见解持久存储在反思记忆缓冲区中，实现跨问题学习。基于批判的剪枝保留每个级别中评分最高的k个节点，控制计算成本同时保留高质量解决方案路径。

Result: 使用Qwen 2.5 7B作为基础LLM，在500个数学问题和创意写作任务上评估ReTreVal，结果显示ReTreVal在结构化探索、批判驱动的精炼和跨问题记忆的组合下，始终优于ReAct、Reflexion和Self-Refine等现有方法。
...
TL;DR: 研究发现大语言模型在逻辑推理中存在"逻辑相变"现象：在一定复杂度内性能稳定，超过临界深度后突然崩溃，类似物理相变。作者提出神经符号课程调优框架来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 符号逻辑推理是大语言模型的关键但未被充分探索的能力，在高风险领域如数学推理和法律判决中提供可靠且可验证的决策。当前研究缺乏对逻辑推理在受控复杂度增加下的系统性分析。

Method: 提出神经符号课程调优框架：自适应对齐自然语言与逻辑符号以建立共享表示，围绕相变边界重塑训练动态，逐步增强在增加逻辑深度下的推理能力。

Result: 在五个基准测试上的实验表明，该方法有效缓解了高复杂度下的逻辑推理崩溃，在朴素提示下平均准确率提升+1.26，在思维链提示下提升+3.95，同时提高了对未见逻辑组合的泛化能力。

Conclusion: 研究揭示了逻辑相变现象，并提出了有效的神经符号课程调优框架来增强大语言模型在复杂逻辑推理任务中的能力，为可靠的高风险决策系统提供了新思路。
...
TL;DR: Batch-of-Thought (BoT) 是一种训练免费的方法，通过联合处理相关查询实现跨实例学习，利用共享推理模式和一致性约束来提高LLM推理性能


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型推理系统独立处理查询，丢弃了有价值的跨实例信号，如共享推理模式和一致性约束，导致效率低下和信息损失

Method: 提出Batch-of-Thought (BoT)方法，通过批量处理相关查询进行跨实例学习，包括识别高质量推理模板、通过一致性检查检测错误、分摊计算成本。进一步提出BoT-R多智能体反射架构，其中反射器进行联合评估以获取孤立处理无法获得的互信息增益

Result: 在三个模型系列和六个基准测试上的实验表明，BoT-R持续提高准确性和置信度校准，同时将推理成本降低高达61%。理论和实验分析揭示了批量感知推理何时以及为何有益于LLM系统

Conclusion: Batch-of-Thought方法通过利用跨实例信号显著提高了LLM推理系统的性能和效率，为批量感知推理提供了理论和实践基础
...
TL;DR: RationaleTS方法通过引入基于理由的上下文学习，解决现有多模态大语言模型在时间序列推理中依赖表面模式匹配而非原则性推理的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在时间序列推理中表现不佳，主要原因是缺乏将时间观测与下游结果连接起来的理由先验，导致模型依赖表面模式匹配而非原则性推理。
...
Conclusion: RationaleTS通过理由引导的上下文学习，将理由作为指导推理单元而非事后解释，显著提升了时间序列推理的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动提示工程方法复杂且需要任务特定线索，而列名扩展任务对表格数据搜索、访问和理解至关重要但研究很少

Method: 设计简单自动提示工程系统，无需调优和显式任务线索，应用于数据库表列名扩展任务

Result: 在英语和德语数据集上评估，这是首个将自动提示工程应用于列名扩展任务的工作，也是首个应用于英语以外语言的研究

Conclusion: 提出的自动提示工程系统设计更简单但同样有效，无需调优和任务线索，成功应用于列名扩展任务并扩展到多语言
...
TL;DR: InfiAgent框架通过将持久状态外部化到文件中心状态抽象中，保持智能体推理上下文严格有界，解决了长时任务中上下文无限增长和错误累积的问题。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在长时任务中经常因上下文无限增长和错误累积而失效，现有的上下文压缩或检索增强方法需要在信息保真度和推理稳定性之间进行权衡。

Method: 提出InfiAgent框架，通过将持久状态外部化到文件中心状态抽象中，在每个步骤中从工作空间状态快照加上固定窗口的最近动作重建上下文，保持推理上下文严格有界。

Result: 在DeepResearch和80篇文献综述任务上的实验表明，无需任务特定微调，使用20B开源模型的InfiAgent与更大的专有系统竞争，并保持比上下文中心基线显著更高的长时覆盖度。

Conclusion: 显式状态外部化是稳定长时智能体的实用基础，InfiAgent框架为LLM智能体处理长时任务提供了有效的解决方案。
