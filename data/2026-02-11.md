...
TL;DR: 提出一个用于新闻视频中屏幕姓名自动检测与提取的框架，在确定性可审计性与生成式方法之间取得平衡，达到95.8% mAP@0.5的检测性能。
...
Method: 提出一个全面的框架，包括：1) 创建包含当代新闻图形多样性的标注帧语料库；2) 设计可解释的模块化提取流程，在确定性和可审计条件下运行；3) 与生成式多模态方法进行对比评估。

Result: 检测器达到95.8% mAP@0.5的图形元素定位性能。生成式系统在原始准确率上略高(F1: 84.18% vs 77.08%)，但缺乏透明数据溯源。提出的流程平衡了精确率(79.9%)和召回率(74.4%)，避免幻觉并提供完整可追溯性。用户研究发现59%受访者在快节奏广播中难以阅读屏幕姓名。

Conclusion: 该研究为现代新闻媒体中的混合多模态信息提取建立了方法严谨且可解释的基准，在确定性可审计性与随机推理之间揭示了明确的权衡，满足了新闻和分析场景对透明数据溯源的需求。
...
TL;DR: 本文提出SemanticMoments方法，通过计算预训练语义模型特征的高阶时序统计量来理解视频语义运动，在SimMotion基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频表示方法过度依赖静态外观和场景上下文而非运动动态，而传统运动中心输入（如光流）缺乏理解高级语义运动所需的基础。需要解决语义运动检索这一基础但未解决的问题。

Method: 提出SemanticMoments方法，这是一种简单的免训练方法，通过计算预训练语义模型特征的高阶时序统计量（矩）来捕捉语义运动信息。

Result: 在SimMotion基准测试（结合合成数据和人工标注真实世界数据集）上，SemanticMoments一致优于现有的RGB、光流和文本监督方法，能够更好地解耦运动和外观。

Conclusion: 语义特征空间中的时序统计量为以运动为中心的视频理解提供了可扩展且感知基础良好的基础，为解决语义运动检索问题提供了有效途径。
...
TL;DR: LELA是首个无需训练、基于大语言模型的仇恨视频定位框架，通过多模态分解和多阶段提示机制实现细粒度时间定位，在HateMM和MultiHateClip基准上显著优于现有无训练基线。
...
Method: LELA将视频分解为图像、语音、OCR、音乐和视频上下文五个模态，采用多阶段提示方案计算每帧的细粒度仇恨分数，并引入组合匹配机制增强跨模态推理。整个框架无需训练，完全基于大语言模型和模态特定字幕生成。

Result: 在HateMM和MultiHateClip两个挑战性基准测试中，LELA大幅优于所有现有的无训练基线方法。实验还提供了广泛的消融研究和定性可视化分析。

Conclusion: LELA为可扩展和可解释的仇恨视频定位建立了坚实基础，展示了无需训练的LLM框架在复杂多模态任务中的强大潜力。
...
Method: 提出基于场景图结构的文本到图像合成框架，引入ASQL（属性-尺寸-数量-位置）条件器，通过轻量级语言模型生成软视觉引导，在推理时通过优化指导基于扩散的生成过程。

Result: 该方法能够在保持文本-图像对齐的同时，支持轻量级、连贯且多样化的图像合成，提升了复杂提示的视觉表示能力。

Conclusion: 基于场景图的零样本条件机制为文本到图像合成提供了有效的组合能力增强方案，通过软视觉引导而非刚性约束，平衡了语义保真度和生成多样性。
...
TL;DR: 使用CNN-RNN模型从视觉数据预测步态中足部压力中心位置和触地时间，在平地到上楼梯转换场景中验证可行性


<details>
  <summary>Details</summary>
Motivation: 计算机视觉已用于步态环境分类以辅助控制系统，但预测足部如何接触变化环境的能力尚未充分探索。本研究旨在评估在平地到上楼梯转换时预测足部压力中心和触地时间的可行性。

Method: 8名受试者在右小腿佩戴RGB-D相机和仪器化鞋垫，执行上楼梯任务。训练CNN-RNN模型在足部触地前250毫秒窗口内连续预测压力中心和触地时间。模型轻量化，可在消费级笔记本或边缘计算设备上以60FPS运行。

Result: 在150、100和50毫秒预测窗口下，压力中心预测的平均绝对误差分别为29.42mm、26.82mm和23.72mm；触地时间预测误差分别为21.14ms、20.08ms和17.73ms。脚趾摆动速度加快可改善压力中心预测精度，更靠前的足部触地会降低压力中心预测精度但不影响触地时间预测。

Conclusion: 使用轻量化模型从视觉数据预测足部压力中心和触地时间是可行的，这对辅助系统中的预期控制具有重要意义。
...
TL;DR: 本文介绍了VLM-UQBench基准测试，用于评估视觉语言模型中的模态特定和跨模态不确定性量化方法，发现现有UQ方法存在模态特异性、与幻觉关联弱、难以检测细微歧义等问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型的不确定性量化对于确保其安全可靠行为至关重要。核心挑战在于定位不确定性的来源——是来自图像、文本还是两者之间的错位。当前缺乏专门评估模态特定和跨模态数据不确定性的基准测试。

Method: 提出了VLM-UQBench基准测试，包含600个来自VizWiz数据集的真实样本，分为干净、图像、文本和跨模态不确定性子集。开发了可扩展的扰动流程，包含8种视觉、5种文本和3种跨模态扰动。提出了两个简单指标：量化UQ分数对这些扰动的敏感性，以及UQ分数与幻觉的相关性。使用这些指标评估了多种UQ方法在四个VLM和三个数据集上的表现。

Result: 实证发现：(1)现有UQ方法表现出强烈的模态特异性，且严重依赖于底层VLM；(2)模态特定不确定性经常与幻觉同时出现，但当前UQ分数仅提供弱且不一致的风险信号；(3)虽然UQ方法在明显的群体级歧义上可以与基于推理的思维链基线相媲美，但在检测扰动流程引入的细微实例级歧义方面大多失败。

Conclusion: 当前UQ实践与可靠VLM部署所需的细粒度、模态感知不确定性之间存在显著差距。VLM-UQBench为评估和改进VLM的不确定性量化方法提供了重要基准。
...
TL;DR: IR-SIS：首个基于自然语言的手术图像分割框架，支持迭代优化和临床医生交互


<details>
  <summary>Details</summary>
Motivation: 现有手术图像分割方法存在三个主要限制：1) 局限于预定义类别；2) 一次性预测缺乏自适应优化；3) 缺乏临床医生交互机制。需要一种更灵活、可交互的分割系统。

Method: 提出IR-SIS系统：1) 使用微调SAM3进行初始分割；2) 利用视觉语言模型检测手术器械并评估分割质量；3) 采用智能体工作流自适应选择优化策略；4) 支持临床医生通过自然语言反馈进行交互；5) 构建了基于EndoVis2017和EndoVis2018的多粒度语言标注数据集。

Result: 实验表明，IR-SIS在领域内和分布外数据上均达到最先进性能，临床医生交互进一步提升了分割效果。

Conclusion: 该研究建立了首个具有自适应自优化能力的基于语言的手术分割框架，为机器人辅助手术和术中导航提供了更灵活、可交互的解决方案。
...
TL;DR: 研究发现扩散变换器中基于调制的文本条件作用并非必要，注意力机制已足够，但池化嵌入可作为引导实现可控性能提升
...
Method: 分析传统使用方式下池化嵌入的贡献，发现其作用有限；提出将池化嵌入作为引导的新视角，实现可控的属性偏移，该方法无需训练、易于实现、运行时开销可忽略

Result: 传统使用方式下池化嵌入对整体性能贡献很小，注意力机制足以有效传播提示信息；但池化嵌入作为引导时能显著提升性能，可应用于多种扩散模型，在文本到图像/视频生成和图像编辑等任务中带来改进

Conclusion: 基于调制的文本条件作用在传统用法中并非必要，但通过重新定位为引导机制，池化嵌入能够实现可控的性能提升，为扩散模型提供简单有效的改进方案
...
TL;DR: X-Mark是一种针对胸部X光片的样本特异性清洁标签水印方法，用于保护医学影像数据集版权，通过条件U-Net在显著区域生成独特扰动，实现水印有效性、鲁棒性和诊断质量保持。


<details>
  <summary>Details</summary>
Motivation: 高质量医学影像数据集对深度学习模型训练至关重要，但未经授权使用会引发严重的版权和伦理问题。现有为自然图像设计的数据集所有权验证方法在医学影像上效果不佳，因为静态水印模式在动态高分辨率扫描中缩放效果差，且医学影像视觉多样性有限、解剖结构细微，同时需要保持诊断质量。

Method: X-Mark使用条件U-Net在每个样本的显著区域内生成独特扰动，设计多组件训练目标确保水印有效性、对动态缩放过程的鲁棒性，同时保持诊断质量和视觉可区分性。在训练目标中加入拉普拉斯正则化来惩罚高频扰动，实现水印尺度不变性。所有权验证在黑盒设置下进行，检测可疑模型中的特征行为。

Result: 在CheXpert数据集上的广泛实验验证了X-Mark的有效性，实现了100%的水印成功率（WSR），在Ind-M场景中将误报概率降低了12%，同时展示了对潜在自适应攻击的抵抗能力。

Conclusion: X-Mark为医学影像数据集版权保护提供了一种有效的样本特异性清洁标签水印方法，解决了医学影像特有的挑战，在保持诊断质量的同时实现了鲁棒的所有权验证。
...
TL;DR: 提出一种基于深度多模态方法预测患者住院风险的模型，通过结合伤口变量和伤口图像进行预测，旨在早期发现伤口复杂性并减少临床医生诊断时间。


<details>
  <summary>Details</summary>
Motivation: 患者住院是伤口护理成本高昂的主要因素之一。大多数伤口不需要立即住院，但由于治疗延迟、患者不配合或现有并发症等因素，伤口可能恶化最终导致住院。需要早期预测住院风险以降低医疗成本。

Method: 提出基于迁移学习的深度多模态方法，结合伤口变量和伤口图像预测患者住院风险。模型能够从伤口图像预测伤口变量及其愈合轨迹，实现早期复杂性检测。

Result: 开发了一个基于迁移学习的伤口评估解决方案，能够从伤口图像预测伤口变量及其愈合轨迹，这是论文的主要贡献。

Conclusion: 提出的新模型有助于早期发现可能影响愈合过程的伤口复杂性，同时减少临床医生诊断伤口的时间，从而降低患者住院风险和相关医疗成本。
...
TL;DR: 该研究评估了13种深度学习模型在膀胱癌分类任务中的表现，发现ConvNext系列泛化能力有限，ViT模型校准效果更好，但无单一模型能提供完全可行的可解释性解决方案。
...
Result: ConvNext系列在膀胱癌图像分类中表现出有限的泛化能力（约60%准确率）。ViT相比ConvNext和swin transformer系列显示出更好的校准效果。测试时增强提高了模型的可解释性。无单一模型能提供完全可行的可解释性解决方案。

Conclusion: ConvNext系列适用于分布内样本，而ViT及其变体更适合解释分布外样本。没有一种模型能提供适用于所有情况的通用可解释性解决方案。
...
TL;DR: Kyrtos方法用于技术文档中曲线图表的自动识别与分析，通过聚类识别曲线线段，分析行为特征，转换为属性图和自然语言描述，最终生成随机Petri网图表示图表功能。


<details>
  <summary>Details</summary>
Motivation: 技术文档中蕴含大量有价值知识，但对其深度理解需要准确分析文档中的多种模态（如图形、表格、图表、文本等）及其关联。特别是技术文档中的曲线图表包含重要信息，需要自动识别和分析方法。

Method: 采用Kyrtos方法：1) 识别处理部分使用基于聚类的方法识别界定曲线线段的中间点；2) 分析处理部分解析提取的曲线线段以捕捉方向、趋势等行为特征；3) 将识别的线段关系转换为属性图以保持曲线结构特征；4) 将图关系表达为自然语言文本句子；5) 最终转换为随机Petri网图表示图表内部功能。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像标注成本高且需要专业知识，现有半监督学习方法中双任务协同学习仅限于单向交互（回归到分割），无法充分利用在线双向跨任务协作的潜力

Method: 提出完全可微分双向协同学习（DBiSL）框架，无缝集成并增强四个关键SSL组件：监督学习、一致性正则化、伪监督学习和不确定性估计，实现分割与回归任务间的在线双向交互

Result: 在两个基准数据集上展示了最先进的性能，为统一SSL框架设计提供了新见解，建立了双任务驱动SSL的新架构基础

Conclusion: DBiSL框架通过可微分双向协同学习有效解决了现有方法的局限性，不仅提升了半监督医学图像分割性能，还为更广泛的计算机视觉应用提供了通用的多任务学习框架
...
TL;DR: 该研究对五种最先进的图像到3D模型在医学数据上的零样本重建性能进行了基准测试，发现单切片医学图像到3D重建存在深度模糊问题，SAM3D在拓扑相似性方面表现最佳。
...
Method: 对五种最先进的图像到3D模型（SAM3D、Hunyuan3D-2.1、Direct3D、Hi3DGen、TripoSG）进行受控零样本基准测试，在六个医学数据集（涵盖解剖和病理结构）和两个自然数据集上评估，使用基于体素的指标和点云距离指标。

Result: 在医学数据集上，所有模型的体素重叠度保持中等，与从单切片推断体积时的深度重建失败模式一致。全局距离指标显示方法间差异更大：SAM3D在拓扑相似性方面表现最强，而其他模型更容易过度简化重建。

Conclusion: 结果量化了单切片医学重建的局限性，突出了2D医学数据平面性质导致的深度模糊问题，这促使需要多视图图像到3D重建来实现可靠的医学3D推断。
...
TL;DR: K-Sort Eval：一个基于视觉语言模型的高效可靠评估框架，通过后验校正和动态匹配解决VLM幻觉偏差问题，显著减少评估所需模型运行次数


<details>
  <summary>Details</summary>
Motivation: 视觉生成模型的快速发展需要更可扩展和人类对齐的评估方法。现有众包Arena平台成本高、耗时长，而直接使用视觉语言模型(VLM)作为人工判断替代品存在幻觉和偏差问题，且静态评估效率低下。
...
Conclusion: K-Sort Eval是一个可靠且高效的VLM-based评估框架，通过后验校正和动态匹配策略解决了VLM幻觉偏差问题，显著提高了评估效率，为视觉生成模型评估提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现代成像和可微渲染中出现的高维非线性逆问题需要统一的算子理论框架来分析其稳定性和统计特性，特别是针对具有块结构参数的问题。

Method: 结合块状Lipschitz几何、局部可识别性和次高斯噪声的统一假设集，建立确定性稳定性不等式、最小二乘失配泛函的全局Lipschitz边界和非渐近浓度估计。

Result: 获得了与具体重建算法无关的高概率参数误差边界，验证了高斯泼溅渲染算子满足所提假设，并推导了其Lipschitz连续性和分辨率相关可观测性的显式常数，揭示了稳定性-分辨率权衡。

Conclusion: 该分析为现代成像和可微渲染中广泛的高维非线性逆问题刻画了算子级极限，表明估计误差本质上受图像分辨率与模型复杂度比值的约束。
...
Method: 构建包含10个任务组合、32个提示类别、11种视觉风格和5个提示模板的大规模数据集，结合现代强模型生成的合成图像和专业摄影师精心策划的真实图像，通过严格筛选去除95%以上初始候选样本。

Result: 最终数据集包含超过600万文本-图像对，约2TB大小，接近预训练数据集的规模但保持微调级别的质量。在多种预训练扩散和自回归模型上，使用Fine-T2I微调能一致提升生成质量和指令遵循能力。

Conclusion: Fine-T2I是一个高质量、大规模的开放数据集，有助于缩小文本到图像微调领域的数据差距，推动开源社区发展。该数据集在开放许可下发布，为研究社区提供重要资源。
...
TL;DR: ArtifactLens：一种基于少量标注数据解锁预训练VLM检测图像生成器伪影能力的系统，在多个基准测试中达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现代图像生成器产生的图像非常逼真，只有伪影（如扭曲的手或变形的物体）能揭示其合成来源。检测这些伪影对于基准测试和改进生成器至关重要。现有方法需要大量标注数据进行微调，成本高昂且难以适应生成器演进或新伪影类型。

Method: ArtifactLens系统采用多组件架构，包括上下文学习和文本指令优化。核心思想是预训练的VLM已经编码了检测伪影所需的知识，只需通过少量标注示例（每类别仅需数百个）和适当的"脚手架"就能解锁这一能力。

Result: 在五个人工伪影基准测试中（首次跨多个数据集的评估）达到最先进性能，同时所需标注数据量减少数个数量级。方法还推广到其他伪影类型（物体形态、动物解剖、实体交互）和AIGC检测任务。

Conclusion: 预训练VLM已经具备检测图像生成器伪影的知识，通过创新的多组件架构和少量标注数据就能有效解锁这一能力，为高效、可扩展的伪影检测提供了新途径。
...
TL;DR: FD-DB提出了一种频率解耦的双分支模型，通过分离低频可解释编辑和高频残差补偿来解决合成到真实图像转换中的域偏移问题，在保持结构稳定性的同时提高真实感。


<details>
  <summary>Details</summary>
Motivation: 合成数据为几何敏感视觉任务提供了低成本、准确标注的样本，但合成域与真实域之间的外观和成像差异导致严重的域偏移，降低了下游任务性能。现有的无配对合成到真实转换方法在真实感和结构稳定性之间存在权衡：无约束生成可能引入变形或虚假纹理，而过度刚性约束又限制了向真实域统计的适应。

Method: FD-DB采用频率解耦的双分支模型：1）可解释分支预测物理意义明确的编辑参数（白平衡、曝光、对比度、饱和度、模糊和颗粒）来构建稳定的低频外观基础；2）自由分支通过残差生成补充精细细节；3）门控融合机制在明确的频率约束下结合两个分支以限制低频漂移；4）采用两阶段训练策略，先稳定编辑分支，再释放残差分支。

Result: 在YCB-V数据集上的实验表明，FD-DB提高了真实域外观一致性，显著提升了下游语义分割性能，同时保持了几何和语义结构。

Conclusion: FD-DB通过频率解耦的双分支设计，在合成到真实图像转换中实现了更好的真实感和结构稳定性平衡，有效减少了域偏移对下游任务的影响。
...
TL;DR: 提出了一种用于数字病理学图像分析的弱监督对比学习框架，仅使用切片级标签就能在特征空间中分离不同标签的补丁，提升下游多示例学习性能。


<details>
  <summary>Details</summary>
Motivation: 数字病理学全切片图像分析面临训练标签有限的问题，因为手动标注需要大量时间和精力。现有的多示例学习方法主要关注特征聚合，而忽略了在MIL设置下编码器预训练的特征表示学习。

Method: 提出弱监督对比学习框架，在训练过程中融入切片级标签信息，不依赖实例级伪标签，但能有效在特征空间中分离不同标签的补丁。

Result: 实验结果表明，与自监督对比学习方法相比，WeakSupCon方法生成的特征在三个数据集上都能提升下游MIL性能。

Conclusion: WeakSupCon框架为数字病理学图像分析提供了一种有效的特征表示学习方法，仅需切片级标签就能改善特征空间分离，从而提升下游多示例学习任务的性能。
...
Method: Align-TI从token交互的角度设计，包含两个核心组件：IVA（指令-视觉对齐）让学生模型模仿教师模型提取相关视觉信息的能力；TPA（token概率对齐）通过对齐序列token到token的转移概率来捕捉教师模型的动态生成逻辑。

Result: 实验表明Align-TI优于现有方法，相比传统知识蒸馏有2.6%的相对提升。蒸馏后的Align-TI-2B模型甚至超越了更大的LLaVA-1.5-7B模型，性能提升7.0%，为训练参数高效的多模态大语言模型建立了新的最先进蒸馏框架。

Conclusion: Align-TI通过关注token交互的知识蒸馏方法，成功实现了多模态大语言模型的高效压缩，在保持甚至超越原始模型性能的同时显著减小了模型规模，为实际部署提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 评估联邦学习方法在手术视频分类中的当前技术水平，特别是方法在未见临床中心的泛化能力以及通过本地微调进行适应的能力，同时实现不共享患者数据的协作模型开发。

Method: 使用多中心Appendix300视频数据集，参与者开发策略对阑尾炎炎症阶段进行分类。挑战赛评估两个任务：对未见中心的泛化能力和微调后的中心特定适应能力。提交的方法包括基础模型线性探测、使用三元组损失的度量学习以及各种FL聚合方案（FedAvg、FedMedian、FedSAM）。使用F1分数和预期成本评估性能，通过自举法和统计检验评估排名鲁棒性。

Result: 在泛化任务中，各中心性能有限；在适应任务中，所有团队在微调后都有改善，但排名稳定性较低。基于ViViT的提交获得了最强的整体性能。挑战赛凸显了泛化能力有限、对类别不平衡敏感以及去中心化训练中超参数调优困难等问题，而时空建模和上下文感知预处理成为有前景的策略。

Conclusion: FedSurg挑战赛为评估手术视频分类中的FL策略建立了首个基准。研究结果凸显了本地个性化与全局鲁棒性之间的权衡，并强调了架构选择、预处理和损失设计的重要性。这一基准为未来开发不平衡感知、自适应和鲁棒的临床手术AI FL方法提供了参考点。
...
TL;DR: 本文提出OSI（一步反演）方法，用于快速提取高斯着色风格水印，相比传统多步扩散反演方法速度提升20倍，准确率更高，且水印容量翻倍。


<details>
  <summary>Details</summary>
Motivation: 当前基于高斯着色的训练免费水印嵌入方法虽然对生成图像质量影响小，但提取时需要多步扩散反演来获取精确初始噪声，计算成本高且耗时。需要一种更快速准确的水印提取方法。
...
Conclusion: OSI是一种显著更快更准确的高斯着色风格水印提取方法，通过将问题重新定义为符号分类任务，实现了高效的一步提取，在速度、准确性和容量方面均有显著提升。
...
TL;DR: 本文提出ECL（均衡对比学习）框架，通过促进表示几何均衡和分类器-类中心几何均衡，解决现有监督对比学习方法在类别不平衡数据集上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有监督对比学习方法在类别不平衡数据集上存在两个主要问题：1）未考虑类均值/原型与分类器之间的对齐，可能导致泛化能力差；2）原型仅被视为每个类的一个额外样本，其影响力取决于批次中类实例数量，导致不同类别贡献不平衡。

Method: 提出ECL框架，包含两个主要组件：1）促进表示几何均衡，实现类样本坍缩和均匀分布的类均值；2）建立分类器-类中心几何均衡，对齐分类器权重和类原型。

Result: 在三个长尾数据集（CIFAR-10(0)-LT、ImageNet-LT）和两个不平衡医学数据集（ISIC 2019、LCCT）上的实验表明，ECL优于现有的SOTA监督对比学习方法。

Conclusion: ECL通过促进几何均衡，有效解决了监督对比学习在类别不平衡分类中的局限性，在多个数据集上取得了优于现有方法的性能。
...
TL;DR: AdaDS是一个通用的深度超分辨率框架，通过自适应选择扩散反向轨迹的起始时间步，利用高斯平滑的收缩特性，从任意退化的低分辨率输入中稳健恢复高分辨率深度图。


<details>
  <summary>Details</summary>
Motivation: 传统方法直接回归深度值，在严重或未知退化情况下容易产生伪影。AdaDS旨在解决深度超分辨率中面对任意退化模式时的鲁棒性问题，特别是当上游估计不完美时仍能保持恢复质量。

Method: 基于高斯平滑的收缩特性：随着前向过程中噪声积累，退化输入与原始高质量对应物之间的分布差异减小，最终收敛到各向同性高斯先验。AdaDS根据估计的细化不确定性自适应选择反向扩散轨迹的起始时间步，然后注入定制噪声，将中间样本定位在目标后验分布的高概率区域内。

Result: 在真实世界和合成基准测试上的广泛实验表明，AdaDS在零样本泛化能力和对多样化退化模式的鲁棒性方面优于最先进的方法。

Conclusion: AdaDS通过利用扩散模型的生成先验，即使在输入估计不完美的情况下也能主导恢复过程，实现了对任意退化输入的稳健深度超分辨率，展示了卓越的泛化能力和鲁棒性。
...
TL;DR: 该研究利用现有的人类动作数据集构建了一个大规模通用动作空间（UAS），并将其应用于哺乳动物和黑猩猩行为数据集的分析与分类。


<details>
  <summary>Details</summary>
Motivation: 动物和人类行为分析在计算机视觉中一直具有挑战性。早期方法依赖手工特征提取和稀疏特征点跟踪，限制了鲁棒性和泛化能力。ImageNet的出现使对象识别转向学习高级表示，本研究希望遵循这一范式，构建一个通用动作空间来改进行为分析。

Method: 利用现有标注的人类动作数据集构建大规模通用动作空间（UAS），然后将这个UAS作为基础框架，应用于哺乳动物和黑猩猩行为数据集的分析和分类。

Result: 成功构建了通用动作空间（UAS），并将其应用于哺乳动物和黑猩猩行为分析。源代码已在GitHub上开源发布。

Conclusion: 通过构建通用动作空间，能够有效分析和分类哺乳动物和黑猩猩的行为，为行为分析提供了新的范式。该方法继承了深度学习在视觉识别中的优势，将行为分析从传统的低层次处理转向学习到的高层次表示。
...
Method: 提出无需训练的关注干预算法：1) 基于任务相关标记通常具有高视觉-文本相似度的假设，提取视觉-文本交叉注意力子矩阵构建重加权矩阵来重新分配注意力；2) 通过将视觉注意力值注入束搜索解码来增强视觉标记的贡献

Result: 大量实验表明，该方法显著减少了主流大型视觉语言模型的幻觉，同时保持了生成内容的准确性和连贯性

Conclusion: 提出的无需训练关注干预算法有效解决了大型视觉语言模型的幻觉问题，通过智能重新分配注意力，在减少幻觉的同时保持了模型性能
...
TL;DR: Singpath-VL是一个专门用于宫颈细胞学的视觉语言大模型，通过创新的三阶段数据合成管道生成百万级图像描述数据集，并基于Qwen3-VL-4B模型进行多阶段微调，在细胞形态感知和诊断分类方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在计算病理学领域取得显著进展，但在细胞病理学特别是宫颈细胞学中的应用仍未被充分探索，主要原因是缺乏大规模高质量标注数据集。需要填补宫颈细胞学AI助手的空白。

Method: 1. 开发三阶段数据合成管道：利用通用MLLMs作为弱标注器，通过共识融合和专家知识注入优化输出，生成高质量细胞形态描述；2. 使用合成数据集对Qwen3-VL-4B模型进行多阶段微调，创建专门的细胞病理学MLLM。

Result: Singpath-VL模型在细粒度形态感知和细胞级诊断分类方面表现出卓越性能，将开源部分合成数据集和基准测试。

Conclusion: 该研究成功填补了宫颈细胞学AI助手的空缺，通过创新的数据合成方法解决了标注数据稀缺问题，为细胞病理学领域提供了专门的视觉语言大模型，有望推动该领域的发展。
...
TL;DR: SchröMind框架通过解决薛定谔桥问题，减少多模态大语言模型在医疗等高风险领域的幻觉问题，在POPE和MME基准测试中取得SOTA性能
...
Method: 提出SchröMind框架，通过解决薛定谔桥问题来减少幻觉。该方法在幻觉激活和真实激活之间建立token级别的映射，通过轻量级训练实现最小传输成本，同时保持模型的原始能力。

Result: 在POPE和MME基准测试上的广泛实验表明，SchröMind框架具有优越性，实现了最先进的性能，同时仅引入最小的计算开销。

Conclusion: SchröMind通过解决薛定谔桥问题，有效减少了多模态大语言模型的幻觉问题，使其在医疗等高风险领域的应用成为可能，在保持模型原始能力的同时实现了SOTA性能。
...
TL;DR: SCA-Net：基于Change-Agent框架的改进架构，用于双时相影像中的建筑物和道路变化检测，通过多尺度分析、注意力机制和动态损失函数实现高效准确的变化检测。
...
Result: 在LEVIR-CD和LEVIR-MCI数据集上的评估显示，SCA-Net优于Change-Agent和其他最先进方法：在LEVIR-MCI上mIoU提升2.64%，小建筑物IoU显著提高57.9%，训练时间减少61%。
...
TL;DR: DR.Experts：一种基于先验驱动的盲图像质量评估框架，通过显式引入失真先验来提升与人类感知的对齐度
...
Method: 提出DR.Experts框架：1）利用退化感知的视觉语言模型获取失真特定先验；2）通过失真显著性差分模块区分失真与语义注意力，精炼失真表示；3）使用动态失真加权模块（混合专家风格）融合精炼先验、语义和桥接表示，根据感知影响加权各失真特征。

Result: 在五个具有挑战性的BIQA基准测试上进行广泛实验，证明DR.Experts优于当前方法，在泛化能力和数据效率方面表现出色。

Conclusion: DR.Experts通过显式引入失真先验，有效提升了盲图像质量评估与人类感知的对齐度，为视觉任务提供了更可靠的评估工具。
...
TL;DR: AUHead是一个两阶段方法，通过解耦细粒度情感控制（动作单元AUs）实现可控的说话头部视频生成，在情感真实性和唇部同步方面超越现有技术。


<details>
  <summary>Details</summary>
Motivation: 当前方法在情感表达控制方面存在不足，缺乏细粒度的情感控制能力，难以生成具有微妙情感表达的逼真说话头部视频。

Method: 采用两阶段方法：第一阶段利用大型音频语言模型通过时空AU标记化和"情感-然后-AU"思维链机制从原始语音中解耦AUs；第二阶段提出AU驱动的可控扩散模型，将AU序列映射为结构化2D面部表示，并在交叉注意力模块中建模AU-视觉交互，通过AU解耦引导策略实现灵活的AU-质量权衡控制。

Result: 在基准数据集上的结果显示，该方法在情感真实性、准确的唇部同步和视觉连贯性方面实现了竞争性性能，显著超越了现有技术。

Conclusion: AUHead通过解耦细粒度情感控制（AUs）实现了高质量的说话头部视频生成，为虚拟化身、电影制作和交互系统提供了更精确的情感表达控制能力。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型由于大型语言模型的强先验和跨模态注意力错位，经常产生与视觉内容不一致的输出（幻觉问题），需要有效的缓解方法。

Method: 提出Scalpel方法，在推理时预测Transformer层中每个注意力头的可信方向，使用高斯混合模型捕捉信任和幻觉流形中的多峰分布，通过熵最优传输（等价于薛定谔桥问题）精确映射高斯分量，动态调整干预强度和方向。

Result: 在多个数据集和基准测试上的广泛实验表明，Scalpel有效缓解了幻觉问题，超越了先前方法并达到最先进性能，且无需额外计算，仅需单次解码步骤。

Conclusion: Scalpel是一种模型和数据无关的方法，通过优化注意力激活分布来减少幻觉，在保持高效的同时显著提升大型视觉语言模型的可靠性。
...
Method: 提出神经正切核谱聚类，利用预训练视觉语言模型中的跨模态对齐。通过正名词（语义上与感兴趣图像接近的名词）锚定神经正切核，将图像间的亲和力定义为视觉邻近性和语义重叠的耦合。还提出正则化亲和力扩散机制，自适应地集成不同提示诱导的亲和力矩阵

Result: 在16个基准数据集（包括经典、大规模、细粒度和域转移数据集）上的广泛实验表明，该方法始终以较大优势优于最先进方法

Conclusion: 该方法成功将谱聚类扩展到多模态领域，通过利用预训练视觉语言模型中的跨模态对齐，显著提升了聚类性能，增强了簇内连接同时抑制了簇间虚假连接
...
TL;DR: MieDB-100k是一个大规模、高质量、多样化的文本引导医学图像编辑数据集，解决了现有医学图像编辑数据集多样性不足、忽视医学图像理解、难以平衡质量与可扩展性的问题。
...
<details>
  <summary>Details</summary>
Motivation: 自我中心交互世界模型对增强现实和具身AI至关重要，但现有方法面临自由空间手势与接触训练数据之间的分布偏移、单目视图中手部运动与相机运动的模糊性，以及需要任意长度视频生成等挑战

Method: 提出统一自回归框架Hand2World：1）基于投影3D手部网格的遮挡不变手部条件，从场景上下文推断可见性和遮挡；2）通过像素级Plücker射线嵌入注入显式相机几何，解耦相机和手部运动；3）开发全自动单目标注流程，将双向扩散模型蒸馏为因果生成器

Result: 在三个自我中心交互基准测试中，感知质量和3D一致性显著提升，同时支持相机控制和长时程交互生成

Conclusion: Hand2World通过创新的遮挡处理、相机几何解耦和因果生成架构，有效解决了自我中心交互生成中的关键挑战，为增强现实和具身AI应用提供了高质量的交互世界模型
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频生成方法大多任务特定且主要依赖文本指令，难以处理多模态输入、上下文参考和多样化的视频生成与编辑场景。许多视频编辑方法依赖针对单个操作精心设计的流程，限制了可扩展性和组合性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉无关水印会引入视觉无关标记并破坏视觉基础，而视觉特定水印采用静态的一次性估计，忽略权重分布密度，无法适应生成过程中的视觉依赖动态变化，可能引入低质量标记。

Method: 提出注意力引导动态水印框架，在每个解码步骤中：1) 基于注意力权重动态识别语义关键证据，结合上下文感知一致性线索；2) 联合考虑不确定性感知（标记熵）和证据校准（权重密度）确定语义关键标记比例，实现自适应词汇划分。

Result: AGMark显著优于传统方法，明显提高生成质量，在生成后期阶段在视觉语义保真度方面获得特别强的增益。框架保持高度竞争力的检测准确率（至少99.36% AUC）和强大的攻击鲁棒性（至少88.61% AUC），且不牺牲推理效率。

Conclusion: AGMark通过动态证据识别和自适应词汇划分，有效解决了现有水印方法的局限性，在保持视觉保真度的同时实现可靠检测，为保持可靠性的多模态水印建立了新标准。
...
TL;DR: 本文提出了VIDA视频数据集和VideoAfford模型，用于解决3D可操作性标注问题，通过视频交互信息增强机器人对物体可操作区域的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D可操作性标注研究主要依赖静态线索（如语言和图像），缺乏动态交互上下文，难以提供揭示时序和因果关系的线索，限制了机器人对物体可操作区域的准确理解。

Method: 1) 收集VIDA数据集：包含38K人-物交互视频，涵盖16种可操作性类型、38个物体类别和22K点云；2) 提出VideoAfford模型：激活多模态大语言模型的可操作性分割能力，统一世界知识推理和细粒度可操作性标注；3) 使用潜在动作编码器从HOI视频提取动态交互先验；4) 引入空间感知损失函数获取全面的3D空间知识。

Result: 实验评估表明，该模型显著优于现有方法，并展现出强大的开放世界泛化能力和可操作性推理能力。

Conclusion: 通过视频交互信息增强3D可操作性标注是有效的，VIDA数据集和VideoAfford模型为这一领域提供了新的基准，所有数据集和代码将公开以推动相关研究。
...
<details>
  <summary>Details</summary>
Motivation: 域泛化视频语义分割在实际部署中面临域偏移和时间采样偏移的双重挑战，导致基于对应关系的传播和固定步长时间聚合失效，即使在标签稳定区域也会产生严重的帧间闪烁问题。

Method: 提出了基于稳定性查询的Time2General框架：1）时空记忆解码器将多帧上下文聚合到片段级时空记忆中，无需显式对应传播即可解码时间一致的逐帧掩码；2）掩码时间一致性损失通过正则化不同步长的时间预测差异来抑制闪烁；3）随机化训练步长使模型适应不同的时间间隔。

Result: 在多个驾驶基准测试上的广泛实验表明，Time2General在跨域准确性和时间稳定性方面显著优于先前的域泛化语义分割和视频语义分割基线，同时运行速度可达18 FPS。

Conclusion: Time2General通过时空记忆聚合和掩码时间一致性正则化，有效解决了域泛化视频语义分割中的时间一致性问题，在保持实时性能的同时显著提升了跨域部署的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI自动化研究主要关注GUI基础而非更关键的GUI规划，后者需要更复杂的数据收集。GUI探索过程天然具有树结构特征，早期功能入口点通常被更频繁探索，因此将大规模轨迹组织成树结构可以降低数据成本并简化GUI规划的数据扩展

Method: 提出TreeCUA框架：1）多智能体协作框架探索环境、验证动作、总结轨迹、评估质量；2）树基拓扑存储和回放重复探索节点；3）自适应探索算法平衡深度（轨迹难度）和广度（轨迹多样性）；4）世界知识引导和全局记忆回溯避免低质量生成；5）基于树节点信息扩展提出TreeCUA-DPO方法，通过参考相邻轨迹的分支信息提升GUI规划能力

Result: 实验结果显示TreeCUA和TreeCUA-DPO带来显著改进，域外研究进一步证明了强大的泛化能力。所有轨迹节点信息和代码将在GitHub上公开

Conclusion: TreeCUA通过树结构可验证演化有效解决了GUI自动化的扩展问题，提出的多智能体框架和树基拓扑显著提高了GUI轨迹生成的质量和效率，TreeCUA-DPO方法进一步提升了GUI规划能力，展现出良好的泛化性能
...
Method: 采用多任务深度学习框架：1）肝脏分割阶段使用半监督学习模型，结合图像分割和配准技术，利用有标注和无标注数据处理多模态MRI；2）肝纤维化分期阶段采用基于patch的方法，基于分类输出可视化肝纤维化阶段。

Result: 方法在独立测试集上进行了验证，包括分布内和分布外病例，使用三通道MRI（T1、T2、DWI）和七通道MRI（T1、T2、DWI、GED1-GED4）。代码已开源。

Conclusion: 提出的多任务深度学习框架能够有效处理多模态成像数据、有限标注和领域偏移问题，为肝脏分割和肝纤维化分期提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决细粒度指称图像分割问题，传统方法需要监督式推理链标注，成本高昂。本文旨在开发无需监督推理链标注的方法，通过解耦的推理-分割流程提升分割性能。

Method: 采用解耦的"推理-分割"流程：1) 视觉语言模型(VLM)接收图像和自然语言查询，推理场景并生成结构化空间提示（边界框+两个内部关键点）；2) 冻结的提示分割器(SAM 2)将提示转换为高质量掩码。使用Group Relative Policy Optimization (GRPO)微调Qwen3-VL模型，无需监督推理链标注。

Result: 在RefCOCOg验证集上，GenSeg-R1-8B达到0.7127 cIoU和0.7382 mIoU，显著优于Qwen3-VL Instruct基线（分别提升15.3和21.9点），并超越Seg-Zero-7B 3.3 cIoU。GenSeg-R1-G在GRefCOCO验证集上达到76.69%目标mIoU，负提示准确率82.40%。在ReasonSeg测试集上，GenSeg-R1-4B达到68.40% mIoU，超越Seg-Zero-7B 7.0点和Seg-R1-7B 10.7点。

Conclusion: GenSeg-R1框架通过解耦的推理-分割流程有效提升了细粒度指称图像分割性能，无需监督推理链标注，在多个基准测试中显著优于现有方法，特别是具备负提示检测能力。
...
Method: 采用两阶段流程：1) 可控骨架生成：使用骨骼图VAE编码骨架图结构，骨骼图DiT生成骨架嵌入，通过文本语义和2D草图控制生成过程；2) 增强网格合成：通过TextuRig数据集增强现有骨架到网格模型，并采用SKA-DPO偏好优化策略提升几何保真度。

Result: 实验表明Stroke3D能够生成合理的骨架和高质量的网格，是首个能够从用户绘制的2D草图生成绑定3D网格的方法。

Conclusion: Stroke3D框架为创建可直接动画的3D内容提供了更直观的工作流程，通过结合文本和草图输入实现了对3D生成过程的细粒度控制。
...
<details>
  <summary>Details</summary>
Motivation: 数字化绘画中的裂纹自动检测对评估降解和指导修复至关重要，但由于复杂场景以及裂纹与类似艺术特征（如笔触或头发）的视觉相似性，这仍然具有挑战性

Method: 将裂纹检测建模为逆问题，将观测图像分解为无裂纹绘画和裂纹组件。使用深度生成模型作为底层艺术品的先验，同时使用Mumford-Shah型变分函数和裂纹先验捕捉裂纹结构。通过联合优化获得像素级的裂纹定位图

Result: 该方法能够有效分离裂纹与类似的艺术特征，生成准确的像素级裂纹定位图

Conclusion: 提出的混合方法结合了深度生成模型和变分技术，为艺术品裂纹检测提供了一种有效的解决方案，支持艺术品的文档化和保护工作
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶汽车的安全导航依赖于鲁棒的视觉系统，但现有研究对视觉系统安全性的全面分析不足，需要识别潜在攻击面以制定有效的安全措施。

Method: 通过分析CAV导航所需的关键传感器和视觉组件，推导出CAV视觉系统的参考架构，基于此识别潜在攻击面，详细阐述针对每个攻击面的攻击向量，并严格评估其对CIA三要素的影响。

Result: 提出了CAV视觉系统的参考架构，识别了多个攻击面及其对应的攻击向量，全面评估了这些攻击对系统机密性、完整性和可用性的具体影响。

Conclusion: 对视觉系统攻击向量动态的全面理解对于制定能够维护CIA三原则的鲁棒安全措施至关重要，为Level-5自动驾驶能力的发展提供了重要的安全基础。
...
TL;DR: 研究发现文本到图像模型的训练数据存在严重的地理偏见，美国、英国和加拿大占样本的48%，而南美和非洲国家严重不足，仅占1.8%和3.8%。数据代表性与国家GDP高度相关（ρ=0.82），且高代表性并不等同于更高的视觉或语义多样性。


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型经常无法生成地理上具有代表性的图像，这引发了对其训练数据代表性的担忧，促使研究者探究这些训练样本来自世界的哪些地区。

Method: 通过使用LLM从标题中提取位置信息，将图像-标题对映射到国家，对大规模多模态数据集进行地理分析。研究了三个广泛使用的数据集（Re-LAION、DataComp1B和Conceptual Captions）中的英文标题，涵盖20个常见实体，并分析了Re-LAION数据集中4种非英语语言子集。

Result: 美国、英国和加拿大占样本的48.0%，南美和非洲国家分别仅占1.8%和3.8%。国家GDP与数据代表性高度相关（ρ=0.82）。非英语子集主要偏向这些语言主要使用的国家。高代表性并不一定转化为更大的视觉或语义多样性。Stable Diffusion v1.3生成的图像虽然看起来真实，但与真实世界图像相比覆盖范围严重受限。

Conclusion: 文本到图像模型的训练数据存在严重的地理偏见，偏向富裕国家，这可能导致模型无法准确代表全球多样性。需要更平衡和多样化的数据集来改善模型的全球代表性。
...
<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型在生成科学图表时，经常产生视觉上合理但结构不正确的输出。现有基准测试要么依赖图像中心或主观指标，对结构不敏感；要么评估中间符号表示而非最终渲染图像，导致基于像素的图表生成研究不足。

Method: 从真实科学PDF中构建SciFlow-Bench基准，将每个源框架图与规范的真实图配对，采用闭环往返协议评估模型：将生成的图表图像逆向解析回结构化图进行比较。使用分层多智能体系统协调规划、感知和结构推理。

Result: 实验表明，保持结构正确性仍然是一个基本挑战，特别是对于具有复杂拓扑结构的图表，这凸显了结构感知评估的必要性。

Conclusion: SciFlow-Bench通过直接评估像素级输出的结构可恢复性，为科学图表生成提供了结构优先的评估框架，揭示了当前模型在结构正确性方面的不足，强调了结构感知评估的重要性。
...
TL;DR: CompSplat是一个针对压缩视频的压缩感知训练框架，通过显式建模帧级压缩特性来解决长视频序列中的姿态漂移和几何失真问题，在严重压缩条件下实现了最先进的渲染质量和姿态精度。


<details>
  <summary>Details</summary>
Motivation: 现实世界视频通常包含长序列、不规则相机轨迹和未知姿态，导致重建过程中的姿态漂移、特征错位和几何失真。有损压缩会加剧这些问题，引入不一致性并逐渐降低几何和渲染质量。现有方法要么专注于长序列NVS，要么专注于无姿态重建，而压缩感知方法仍局限于特定伪影或有限场景，对长视频中多样压缩模式的研究不足。

Method: CompSplat是一个压缩感知训练框架，显式建模帧级压缩特性以减轻帧间不一致性和累积几何误差。该框架包含压缩感知帧加权和自适应剪枝策略，以增强鲁棒性和几何一致性，特别是在严重压缩条件下。

Result: 在Tanks and Temples、Free和Hike等具有挑战性的基准测试上进行广泛实验，CompSplat实现了最先进的渲染质量和姿态精度，在严重压缩条件下显著超越了大多数最新的最先进NVS方法。

Conclusion: CompSplat通过显式建模压缩特性有效解决了长视频序列中的压缩相关问题，为压缩视频的高质量新视角合成提供了有效的解决方案。
...
TL;DR: 该论文提出了一种名为SAKED的训练免费方法，通过量化模型内部知识稳定性来缓解大型视觉语言模型中的幻觉问题，在多种模型、任务和基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型中的幻觉问题给实际应用带来了严重的安全性和可靠性风险。受人类在不确定或犹豫时更容易出错的观察启发，研究者探索模型内部知识的不稳定性如何导致LVLM幻觉。

Method: 从注意力头、模型层和解码标记三个角度进行实证分析，识别出三种关键幻觉模式。基于这些发现，提出了稳定性感知知识增强解码（SAKED），引入层间知识稳定性评分（KSS）来量化整个模型的知识稳定性。通过对比最稳定性感知和最稳定性无关的层，SAKED抑制解码噪声并动态利用最可靠的内部知识进行忠实标记生成。

Result: SAKED在各种模型、任务和基准测试中实现了最先进的幻觉缓解性能。该方法训练免费，可以无缝集成到不同的架构中。

Conclusion: 通过量化模型内部知识稳定性并动态利用最可靠的知识，SAKED有效缓解了大型视觉语言模型中的幻觉问题，为提升模型的安全性和可靠性提供了实用的训练免费解决方案。
...
### [51] [SARS: A Novel Face and Body Shape and Appearance Aware 3D Reconstruction System extends Morphable Models](https://arxiv.org/abs/2602.09918)
...
<details>
  <summary>Details</summary>
Motivation: 传统3D可变形模型(3DMM)主要关注全局面部结构或几何特征，但忽略了年龄、性别、面部标志点等高级语义特征。这些特征对于准确重建3D人体模型至关重要，特别是在面部边界、曲线、凹陷和皱纹等细节方面。

Method: 提出SARS系统，这是一个模块化流水线，能够从单张图像中提取身体和面部信息。系统结合了身份和表情混合形状与基础面部网格，通过调整多种参数来控制3D可变形模型的变异性，包括形状、纹理、光照和相机参数等高级图像描述符。

Result: SARS系统能够从单张图像中准确重建完整人体的3D模型，不仅包含几何结构，还包含了面部语义特征，如年龄、性别和面部标志点等高级特征。

Conclusion: 该研究提出的SARS系统通过整合面部语义特征，显著提升了3D人体重建的质量和准确性，为从单张图像重建完整3D人体模型提供了更全面的解决方案。
...
TL;DR: ARK是一个新的多模态检索基准，专注于专业知识和复杂推理，包含5个知识领域和6种推理技能，评估了23个检索模型并发现知识密集型与推理密集型检索之间存在显著差距。
...
Result: 评估了23个代表性文本基和多模态检索器，发现知识密集型与推理密集型检索之间存在显著差距，细粒度视觉和空间推理是持续瓶颈。简单的增强方法如重排序和查询重写能带来一致改进，但仍有很大提升空间。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数视觉语言模型使用混合接口（离散文本标记+连续ViT特征），导致模型偏向理解任务且无法充分利用非文本数据的自监督学习。现有的离散视觉标记方法由于编码容量有限而丢失信息，导致理解能力明显弱于连续特征模型。

Method: 提出Kelix模型，采用完全离散的自回归统一建模方法，通过改进视觉离散化技术来保持更多视觉信息，实现跨模态的统一表示。

Result: Kelix成功缩小了离散视觉表示与连续特征视觉语言模型之间的理解差距，实现了更好的统一理解和生成能力。

Conclusion: 通过改进的离散视觉表示方法，可以实现完全离散的自回归统一建模，在保持生成能力的同时显著提升理解性能，为统一的多模态模型提供了新方向。
...
TL;DR: RoSE：通过将法线估计重新定义为着色序列估计来解决3D对齐问题的新范式，利用图像到视频生成模型预测着色序列，再转换为法线图


<details>
  <summary>Details</summary>
Motivation: 现有单目法线估计方法存在3D对齐问题：虽然预测的法线图看起来正确，但重建的表面与几何细节无法对齐。这是因为模型难以从相对细微的颜色变化中区分和重建不同几何形状

Method: 提出新范式：将法线估计重新定义为着色序列估计，因为着色序列对几何信息更敏感。基于此提出RoSE方法，利用图像到视频生成模型预测着色序列，然后通过普通最小二乘问题转换为法线图。使用包含多样化形状、材质和光照条件的合成数据集MultiShade进行训练

Result: 在真实世界基准数据集上，RoSE在基于对象的单目法线估计任务中达到了最先进的性能

Conclusion: 通过将法线估计重新定义为着色序列估计的新范式，RoSE方法有效解决了现有方法的3D对齐问题，显著提升了单目法线估计的准确性和几何一致性
...
Method: 包含两个核心组件：1）检索增强知识模块，将类别特定文本描述融入模型输入；2）熵驱动潜在推理机制，在紧凑潜在空间中使用可优化潜在思考令牌进行迭代探索；3）动态视觉注入策略，选择性融入最信息丰富的图像块。

Result: 大量实验结果表明，Reason-IAD在性能上持续优于最先进方法。
...
### [56] [Unbalanced optimal transport for robust longitudinal lesion evolution with registration-aware and appearance-guided priors](https://arxiv.org/abs/2602.09933)
*Melika Qahqaie,Dominik Neumann,Tobias Heimann,Andreas Maier,Veronika A. Zimmer*

Main category: cs.CV

TL;DR: 提出了一种基于不平衡最优传输的配准感知匹配器，用于纵向CT扫描中癌症病灶的对应关系建立，能够处理病灶出现、消失、合并和分裂等复杂情况。
...
Method: 提出了一种基于不平衡最优传输的配准感知匹配器，该模型能够适应不相等的病灶质量，并根据患者层面的肿瘤负荷变化调整先验。传输成本结合了：(1)尺寸归一化的几何特征，(2)变形场雅可比矩阵的局部配准可信度，(3)可选的补丁级外观一致性。通过相对剪枝对传输计划进行稀疏化，得到一对一匹配以及新出现、消失、合并和分裂的病灶，无需重新训练或启发式规则。

Result: 在纵向CT数据上，该方法相比仅基于距离的基线方法，在边缘检测精度和召回率、病灶状态召回率以及病灶图组件F1分数方面均取得了一致性更高的表现。

Conclusion: 提出的配准感知不平衡最优传输匹配器能够有效处理纵向CT扫描中病灶演变的复杂情况，为癌症治疗反应评估提供了更可靠的病灶对应关系建立方法。
...
### [57] [Bladder Vessel Segmentation using a Hybrid Attention-Convolution Framework](https://arxiv.org/abs/2602.09949)
*Franziska Krauß,Matthias Ege,Zoltan Lovasz,Albrecht Bartz-Schmidt,Igor Tsaur,Oliver Sawodny,Carina Veil*

Main category: cs.CV

TL;DR: 提出了一种混合注意力-卷积架构，用于膀胱癌内窥镜监测中的血管分割，解决膀胱变形、伪影和黏膜褶皱等挑战，通过Transformer捕获全局拓扑先验和CNN细化细节，在BlaVeS数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌监测需要在重复干预中跟踪肿瘤位置，但膀胱的可变形和中空特性缺乏稳定的解剖标志。内窥镜下可见的血管可作为患者特定的"血管指纹"用于导航，但自动分割面临诸多挑战：不完美的内窥镜数据、稀疏标签、气泡和光照变化等伪影、连续变形、以及模仿血管的黏膜褶皱。现有血管分割方法难以处理这些领域特定的复杂性。

Method: 提出混合注意力-卷积架构：结合Transformer捕获全局血管拓扑先验，CNN学习残差细化图以精确恢复细血管细节。为优先考虑结构连通性，Transformer在优化的真实数据上训练，排除短分支和末端分支。为解决数据稀缺问题，采用物理感知预训练策略，在未标记数据上使用临床基础的增强进行自监督学习。

Result: 在BlaVeS数据集（内窥镜视频帧）上评估，该方法达到高准确率（0.94）和优越的精确度（0.61）及clDice（0.66），优于最先进的医学分割模型。关键的是，该方法成功抑制了膀胱在手术中充盈和排空时动态出现和消失的黏膜褶皱产生的假阳性。

Conclusion: HAC方法提供了临床导航所需的可靠结构稳定性，通过结合全局拓扑先验和局部细节细化，有效解决了膀胱内窥镜血管分割的特定挑战，为膀胱癌监测中的可靠导航奠定了基础。
...
TL;DR: 提出了一种新颖的具身4D世界模型，能够从单视角RGBD观测生成几何一致的任意视角RGBD序列，并通过测试时动作优化策略解决逆动力学的不适定问题，实现从预测到动作执行的完整流程。


<details>
  <summary>Details</summary>
Motivation: 现有的基于世界模型的想象-执行范式通常只支持纯图像预测或部分3D几何推理，无法预测完整的4D场景动态。需要一种能够生成几何一致的多视角RGBD序列，并能有效转化为可执行动作的模型。

Method: 1) 设计跨视角和跨模态特征融合机制，联合促进RGB和深度的一致性，并强制跨视角的几何对齐；2) 提出测试时动作优化策略，通过生成模型反向传播推断与预测未来最匹配的轨迹级潜在变量；3) 使用残差逆动力学模型将轨迹先验转化为准确的可执行动作。

Result: 在三个数据集上的实验表明，该方法在4D场景生成和下游操作任务上都表现出色。消融研究为关键设计选择提供了实用见解。

Conclusion: 提出的具身4D世界模型能够从单视角观测生成几何一致的多视角RGBD序列，并通过创新的动作优化策略有效解决逆动力学的不适定问题，为机器人操作提供了更完整的想象-执行框架。
...
TL;DR: Fake-HR1是一个大规模混合推理模型，首次根据生成检测任务特性自适应决定是否需要推理，通过两阶段训练框架提升检测性能同时显著改善响应效率。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，在检测过程中加入思维链推理可以提升模型检测合成图像的能力，但过长的推理会带来大量资源开销（如token消耗和延迟），特别是在处理明显伪造图像时显得冗余。

Method: 提出Fake-HR1模型，采用两阶段训练框架：首先进行混合微调进行冷启动初始化，然后通过混合推理分组策略优化的在线强化学习，隐式学习何时选择适当的推理模式。
...
<details>
  <summary>Details</summary>
Motivation: 腹腔镜手术需要大量训练，深度学习可以支持自动技能评估，但目前受限于标注数据集规模小的问题。

Method: 创建LASANA数据集，包含1270个立体视频记录，涵盖四个基本腹腔镜训练任务，每个记录都有结构化技能评分（来自三位独立评分者）和任务特定错误标签，并提供预定义数据分割。

Result: 提供了包含自然技能变化的多样化数据集，并提供了深度学习模型的基准结果作为未来比较的参考点。

Conclusion: LASANA数据集填补了腹腔镜技能评估深度学习研究的空白，为基于视频的技能评估和错误识别方法提供了标准化的基准测试平台。
...
Method: 通过系统分析自回归视频扩散模型，发现早期层在去噪步骤间产生高度相似特征，深层主要执行帧内渲染。基于此提出SCD架构，使用因果变换器编码器进行每帧一次的时间推理，轻量级扩散解码器进行多步帧级渲染

Result: 在合成和真实基准测试的预训练和后训练任务中，SCD显著提高了吞吐量和每帧延迟，同时匹配或超越了强因果扩散基线的生成质量

Conclusion: 因果推理可以与多步去噪过程分离，SCD架构通过解耦时间推理和帧渲染，实现了更高效和高质量的视频生成
...
TL;DR: 提出SeqΔ-REPA方法，通过序列级控制-效果对齐目标，从未标记视频中学习可跨上下文传输的结构化潜在动作空间，实现零样本动作迁移


<details>
  <summary>Details</summary>
Motivation: 现有动作可控世界模型的扩展受到动作标签稀缺的限制。虽然潜在动作学习承诺从无标签视频中提取控制接口，但学习的潜在表示往往无法跨上下文传输：它们与场景特定线索纠缠，缺乏共享坐标系

Method: 引入SeqΔ-REPA序列级控制-效果对齐目标，将集成潜在动作锚定到冻结自监督视频编码器的时序特征差异上。基于此提出Olaf-World管道，从大规模被动视频中预训练动作条件视频世界模型

Result: 实验表明该方法学习到更结构化的潜在动作空间，在零样本动作迁移和新控制接口的数据高效适应方面优于现有基线方法

Conclusion: 通过观察动作的语义效果作为共享参考，SeqΔ-REPA能够学习可跨上下文传输的潜在动作表示，解决了现有方法中动作语义跨上下文对齐的问题
...
TL;DR: GeoFormer是一个开源Swin Transformer框架，仅使用Sentinel-1/2影像和开放DEM数据，在100米网格上联合估计建筑高度和足迹，在54个城市中表现优于CNN基线，并支持跨大陆迁移。
...
Method: 提出GeoFormer框架，基于Swin Transformer，仅使用Sentinel-1/2影像和开放DEM数据，在100米网格上联合估计建筑高度和足迹；采用地理分块划分策略确保训练集和测试集之间的严格空间独立性。

Result: 在54个多样化城市评估中，GeoFormer实现建筑高度RMSE 3.19米，建筑足迹RMSE 0.05，比最强的CNN基线分别提升7.5%和15.3%；在跨大陆迁移中保持建筑高度RMSE低于3.5米。
...
TL;DR: 德国面包店利用计算机视觉自动化监测剩余烘焙产品，通过有限监督训练YOLOv11模型，在仅使用图像级监督的情况下达到0.91 mAP，并通过伪标签微调提升非理想部署条件下的性能19.3%。


<details>
  <summary>Details</summary>
Motivation: 德国面包店的新鲜烘焙产品保质期短，监测剩余产品对优化生产很重要。自动化监测可降低人工成本、提高准确性并简化运营。但德国烘焙产品种类繁多，完全监督训练成本过高且可扩展性有限。
...
Result: 仅使用图像级监督的模型达到0.91 mAP。通过伪标签微调，模型在非理想部署条件下的性能提升19.3%。结合两种工作流训练的模型在非理想部署条件下超越了完全监督基线模型。

Conclusion: 该方法成功解决了在标注数据稀缺的工业场景中部署计算机视觉的挑战，为德国面包店提供了一种高效、可扩展的剩余产品监测自动化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在组织病理学中，准确维护染色切片的元数据对于临床档案的质量控制和计算病理学数据集的完整性至关重要。病理学家经常使用多种特殊染色来诊断特定形态，需要自动化的染色分类方法来支持数字化病理工作流程。

Method: 研究比较了两种方法：1）多实例学习（MIL）管道，处理全切片图像的所有补丁；2）提出的轻量级缩略图方法，使用缩略图进行分类。评估了16种染色类别（标准H&E、冷冻切片H&E和14种特殊染色）。

Result: 在内部测试数据上，MIL方法表现最佳（16类宏F1：0.941；14类合并后：0.969），缩略图方法保持竞争力（分别为0.897和0.953）。在外部TCGA数据上，缩略图模型泛化能力更好（加权F1：0.843 vs. MIL的0.807）。缩略图方法处理速度比MIL快两个数量级（5.635 vs. 0.018 slides/s）。

Conclusion: 缩略图分类方法为数字化病理工作流程中的常规视觉质量控制提供了可扩展且稳健的解决方案，在保持竞争力的分类性能的同时显著提高了处理效率。
...
TL;DR: Faster-GS系统通过整合现有3D高斯泼溅优化策略并引入新优化方法，实现了5倍训练加速且保持视觉质量，为3DGS优化建立了新的高效基准。


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯泼溅研究存在实现改进与算法修改混杂、性能与保真度权衡的问题，导致研究碎片化且难以公平比较，需要系统整合和优化。

Method: 整合现有3DGS最有效策略，引入新优化方法，研究数值稳定性、高斯截断和梯度近似等未充分探索的方面，构建Faster-GS系统。

Result: Faster-GS在综合基准测试中实现高达5倍训练加速，同时保持视觉质量，优化还可应用于4D高斯重建以实现高效非刚性场景优化。

Conclusion: Faster-GS为3DGS优化建立了新的成本效益和资源效率基准，展示了优化策略在4D高斯重建中的可扩展性。
...
TL;DR: 研究表明，即使经过颅骨剥离的脑部MRI图像仍包含独特的生物特征，可以通过简单的图像相似度计算在不同数据库间匹配同一参与者的扫描数据，存在隐私泄露风险。


<details>
  <summary>Details</summary>
Motivation: 当前医疗数据共享框架要求去除潜在标识符，但即使经过颅骨剥离的脑部MRI仍包含独特的生物特征签名，可能在不同数据库间匹配同一参与者，存在隐私风险。现有研究多依赖训练或计算密集型方法，需要更简单有效的评估方法。

Method: 使用标准预处理流程后，通过图像相似度计算来匹配个体的颅骨剥离T1加权MRI图像。该方法不依赖训练或复杂算法，而是基于预处理后的图像直接比较相似度。

Result: 在跨越不同时间间隔、扫描仪类型、空间分辨率和采集协议的情况下，实现了近乎完美的匹配准确率。即使存在潜在的认知衰退，也能成功模拟跨数据库的MRI匹配。

Conclusion: 简单的图像相似度计算方法就能有效匹配颅骨剥离的脑部MRI图像，表明当前的数据共享隐私保护措施存在不足。这一发现对制定前瞻性的医疗数据共享政策具有重要意义。
...
TL;DR: 提出一种用于实例分割的保形预测算法，为每个像素查询生成自适应置信集，提供预测掩码与真实实例高IoU的概率保证。


<details>
  <summary>Details</summary>
Motivation: 当前实例分割模型在平均预测上表现良好，但缺乏原则性的不确定性量化：输出未校准，无法保证预测掩码接近真实值。需要解决这一局限性。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法仅依赖伪影线索检测拼接或伪造区域，但在某些场景（如抗议图像）中，仅检测伪造区域可能导致误解。需要联合定位源区域和目标区域以理解完整上下文。
...
Conclusion: Forensim框架能够有效联合定位源区域和目标区域，为图像伪造检测提供了更全面的解决方案，特别是在需要理解完整上下文的场景中具有重要价值。
...
<details>
  <summary>Details</summary>
Motivation: 从无标签视频数据中学习可迁移知识并将其应用于新环境是智能体的基本能力。现有方法在处理真实世界视频时存在困难，需要探索直接从原始视频中学习世界知识的方法。

Method: 提出动态增强的潜在动态模型（dLDM），将动作动态与视觉外观解耦：使用预训练视频扩散模型处理视觉外观建模，让dLDM学习专注于紧凑且有意义的任务相关动态的潜在编码，然后自回归建模这些潜在编码以学习任务策略和支持长时程推理。

Result: 在具有挑战性的真实世界手工制作任务中，VideoWorld 2相比之前的视频生成和潜在动态模型，任务成功率提高了70%，并生成连贯的长执行视频。在机器人领域，能从Open-X数据集获取有效的操作知识，显著提升CALVIN上的任务性能。

Conclusion: 这项研究揭示了直接从原始视频中学习可迁移世界知识的潜力，所有代码、数据和模型将开源以供进一步研究，为视频理解与知识迁移提供了新方向。
...
TL;DR: ConsID-Gen是一个图像到视频生成框架，通过多视图辅助和双流编码器解决现有方法中的外观漂移和几何失真问题，在ConsIDVid-Bench基准测试中表现优于现有模型。
...
Method: 1) 构建ConsIDVid大规模对象中心数据集和ConsIDVid-Bench评估基准；2) 提出ConsID-Gen框架，通过无姿态辅助视图增强第一帧，使用双流视觉-几何编码器和文本-视觉连接器融合语义和结构线索，为Diffusion Transformer骨干提供统一条件。

Result: 在ConsIDVid-Bench上的实验表明，ConsID-Gen在多个指标上持续优于现有方法，整体性能超过Wan2.1和HunyuanVideo等领先视频生成模型，在具有挑战性的真实场景中提供更好的身份保真度和时间一致性。

Conclusion: ConsID-Gen通过数据驱动和模型架构创新，有效解决了图像到视频生成中的身份一致性保持问题，为多视图一致的视频生成提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有经典方法如L1-IRLS和Shonan存在局部极小值敏感性和依赖凸松弛的问题，这些方法无法精确保持旋转流形几何，导致在高噪声场景下精度下降

Method: IQARS将多旋转平均问题重新表述为一系列局部二次非凸子问题，通过二值化处理后可在量子退火器上执行，利用量子隧穿和并行性进行高效解空间探索

Result: 在合成和真实数据集上的评估显示，虽然当前量子退火器仍处于早期阶段且仅支持有限规模问题，但IQARS在D-Wave退火器上已能比最佳经典方法Shonan实现约12%的精度提升

Conclusion: IQARS消除了对凸松弛的依赖，更好地保持了非欧几里得旋转流形几何，为量子计算在3D视觉和机器人优化问题中的应用开辟了新途径
...
TL;DR: SAGE是一个智能框架，能够根据用户指定的具身任务自动生成仿真就绪的3D环境，通过多生成器与评估器结合实现语义合理性、视觉真实性和物理稳定性的迭代优化。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据收集成本高且不安全，现有场景生成系统依赖基于规则或任务特定的流水线，导致伪影和物理无效场景，需要可扩展、真实且仿真就绪的3D环境。

Method: SAGE采用智能体框架，结合布局和物体组合的多个生成器，以及评估语义合理性、视觉真实性和物理稳定性的评估器，通过迭代推理和自适应工具选择进行自我优化。

Result: 生成的场景真实、多样且可直接部署到现代仿真器中用于策略训练，纯用此数据训练的策略展现出清晰的扩展趋势，并能泛化到未见过的物体和布局。
...
TL;DR: PAN 2026研讨会聚焦计算文体学与文本取证，包含五个任务：生成式AI检测、文本水印、多作者写作风格分析、生成式抄袭检测和推理轨迹检测，通过Docker容器进行可复现评估。
...
Method: 采用任务驱动的评估框架，包括五个具体任务：Voight-Kampff生成式AI检测（针对混合和混淆的作者场景）、文本水印（评估现有方案的鲁棒性）、多作者写作风格分析（检测作者变更位置）、生成式抄袭检测（源文档检索和文本对齐）、推理轨迹检测（LLM生成或人工写作的推理轨迹检测）。通过TIRA实验平台以Docker容器形式提交软件解决方案。

Result: 自2012年以来，通过TIRA平台已收到超过1,100份Docker容器形式的提交，建立了大规模的可复现评估体系。2026年新增文本水印和推理轨迹检测两个新任务，反映了生成式AI时代的新需求。

Conclusion: PAN研讨会通过持续的任务设计和Docker容器评估机制，为计算文体学和文本取证领域提供了标准化、可复现的研究框架，有效应对了生成式AI带来的新挑战，促进了该领域的技术进步。
...
TL;DR: 本文提出"包容性分析"框架，通过话语分析动态评估人机协作学习中的包容性，包含参与公平、情感氛围和认知公平三个维度。


<details>
  <summary>Details</summary>
Motivation: 当前AI与教育领域的包容性评估通常采用粗粒度样本描述或事后自我报告，无法捕捉协作问题解决过程中包容性如何被动态塑造。需要更精细的过程导向方法来测量人机协作学习环境中的包容性。

Method: 提出包容性分析框架，从三个维度评估包容性：参与公平（参与分布）、情感氛围（关系动态）和认知公平（观点采纳）。使用可扩展的交互级测量方法，通过模拟对话和人机协作实验数据进行验证。

Result: 包容性分析能够揭示参与模式、关系动态和观点采纳等模式，这些模式在聚合或事后评估中是不可见的。框架能够使包容性的动态交互过程变得分析可见。

Conclusion: 这项工作代表了向过程导向方法测量人机协作学习环境中包容性的初步步骤，为更精细地理解和改善协作学习中的包容性提供了新框架。
...
TL;DR: 提出基于朴素贝叶斯扩展NLU模型的主题连续性评估方法，通过注意力机制和对数非线性增强模型能力，实现可解释分析公式，能处理任意长度对话且线性时间复杂度。


<details>
  <summary>Details</summary>
Motivation: LLM作为聊天机器人在商业场景中常面临主题连续性挑战，主题突然切换会导致用户体验差和计算资源利用效率低，需要评估响应是否与初始对话主题一致。

Method: 将自然语言理解模型扩展为可量化形式，采用朴素贝叶斯方法，引入注意力机制和对数非线性来增强主题连续性捕捉能力，将NLU模型转换为可解释分析公式。

Result: 模型在实验中持续优于传统方法，特别是在处理长而复杂的对话时表现更佳，注意力机制显著提升了在复杂对话中识别主题连续性的能力。

Conclusion: 该方法能处理任意长度对话且具有线性时间复杂度，为LLM的责任使用和可解释性提供了机会，确保主题连续性评估的有效性和可解释性。
...
TL;DR: FM SO.P通过渐进式任务混合和自动多智能体评估系统，解决了语言模型在标准操作程序理解和跨领域泛化方面的挑战，在SOPBench上取得了显著性能提升。
...
Method: 提出FM SO.P方法，包含两个创新：1）渐进式任务混合，通过三个阶段累积数据构建能力：概念消歧（术语精确性）、动作序列理解（程序正确性）、场景感知图推理（条件逻辑）；2）自动多智能体评估系统，包含三个智能体，自适应生成评估标准、分层测试集和评分标准，适应不同领域。

Result: 在SOPBench的七个领域（银行、DMV、医疗、市场、大学、图书馆、酒店）上，32B模型达到48.3%通过率，开源7B模型达到34.3%通过率，与Qwen-2.5-72B-Instruct基线（34.4%）相当，但参数量减少10倍。

Conclusion: FM SO.P通过渐进式能力构建和自适应评估系统，显著提升了语言模型对标准操作程序的理解和跨领域泛化能力，为实际企业应用提供了有效解决方案。
...
TL;DR: 该研究通过对Reddit社区中AI相关风险和成瘾讨论的大规模计算主题分析，识别出AI相关心理风险的五个经验维度，其中自我调节困难最为普遍，恐惧主要集中在自主权、控制和技术风险方面。


<details>
  <summary>Details</summary>
Motivation: 生成式AI系统日益融入日常生活，但人们对AI使用相关的心理风险如何产生、被体验以及被用户调节的实证理解仍然有限。研究旨在通过真实世界的用户讨论来理解AI安全如何被感知和情感体验。

Method: 采用大规模计算主题分析方法，收集2023-2025年间来自Reddit社区r/AIDangers和r/ChatbotAddiction的帖子。使用基于Braun和Clarke反思框架的多智能体、LLM辅助主题分析，识别出14个重复出现的主题类别，并综合为五个高阶经验维度。进一步应用基于BERT的情感分类器进行情感标注，并可视化各维度的情感特征。

Result: 研究发现五个基于真实世界用户讨论的AI相关心理风险经验维度，其中自我调节困难最为普遍。恐惧情感主要集中在与自主权、控制和技术风险相关的担忧上。研究提供了来自用户生活经验的早期实证证据，展示了AI安全在实验室或推测性情境之外如何被感知和情感体验。

Conclusion: 该研究为未来AI安全研究、评估和负责任治理提供了基础，强调了从用户生活经验角度理解AI心理风险的重要性，特别是自我调节困难作为主要风险维度，以及恐惧集中在自主权和控制问题上的发现。
...
TL;DR: 研究评估大型语言模型对西班牙语地理词汇变异的捕捉能力，发现模型能识别部分区域变体但存在系统性差异，数字资源量不是主要影响因素。


<details>
  <summary>Details</summary>
Motivation: 西班牙语存在显著的地域变异，但大型语言模型是否准确捕捉这些地理词汇变异尚不清楚。研究旨在评估LLMs作为虚拟信息提供者对西班牙语方言知识的掌握程度，探讨数字语言偏见问题。

Method: 将LLMs视为虚拟信息提供者，使用两种调查式问题格式（是非题和选择题）探测其方言知识。利用专家策划的大规模西班牙语词汇变异数据库，覆盖21个西班牙语国家的900多个词汇项目，在国家层面和方言区域层面进行评估。

Result: 结果显示LLMs在表示西班牙语变体方面存在系统性差异：与西班牙、赤道几内亚、墨西哥及中美洲、拉普拉塔河地区相关的词汇变异识别更准确，而智利变体特别难以区分。重要的是，国家层面数字资源量的差异不能解释这些性能模式。

Conclusion: 通过提供精细的大规模地理词汇变异评估，这项工作推进了对LLMs中方言知识的实证理解，为西班牙语数字语言偏见的讨论提供了新证据，表明数据量之外的因素塑造了LLMs中的方言表示。
...
TL;DR: 提出了一种完全无监督的跨语言词性标注框架，仅使用单语语料库，通过无监督神经机器翻译系统构建伪平行句对，然后基于词对齐进行词性标注器训练。


<details>
  <summary>Details</summary>
Motivation: 低资源语言通常缺乏词性标注数据，现有方法依赖平行语料库进行词性标注投影，但许多低资源语言缺乏平行语料库。需要一种仅使用单语语料库的完全无监督跨语言词性标注方法。

Method: 1) 使用无监督神经机器翻译系统将高资源语言句子翻译成低资源语言，构建伪平行句对；2) 基于词对齐进行标准的词性标注投影；3) 提出多源投影技术校准目标语言的投影词性标签，提高标注器效果。

Result: 在28个语言对上评估，涵盖4个源语言和7个目标语言。实验结果显示：1) 该方法性能可与使用平行句对的基线跨语言词性标注器相媲美，某些目标语言甚至超过基线；2) 多源投影技术进一步提升性能，平均比先前方法提高1.3%。

Conclusion: 提出的完全无监督跨语言词性标注框架仅依赖单语语料库，无需平行语料库，通过无监督神经机器翻译和多源投影技术，在低资源语言词性标注任务上取得了有竞争力的性能。
...
TL;DR: AfriNLLB是一系列轻量级模型，支持15种语言对（30个翻译方向）的高效翻译，主要针对非洲语言，通过模型压缩和知识蒸馏实现资源受限环境下的高效部署。
...
Method: 基于NLLB-200 600M模型，采用迭代层剪枝和量化进行压缩，在非洲语言平行语料库上进行微调，并使用更大教师模型进行知识蒸馏。

Result: AfriNLLB模型在保持与基线相当性能的同时，显著提高了推理速度，并发布了两个版本：支持进一步微调的Transformers版本和用于高效推理的CTranslate2版本。

Conclusion: 成功开发了适用于非洲语言的高效轻量级翻译模型，为资源受限环境提供了可行的解决方案，并开源了训练数据和模型以促进相关研究。
...
TL;DR: BiasScope是一个LLM驱动的框架，用于自动发现模型评估中的潜在偏见，并基于此构建了更具挑战性的JudgeBench-Pro基准测试。
...
Result: BiasScope能够发现不同模型家族和规模的潜在偏见，在JudgeBench数据集上验证了其通用性和有效性。基于BiasScope构建的JudgeBench-Pro基准测试显示，即使是强大的LLM评估器在该基准上的错误率也超过50%，突显了加强评估鲁棒性和减轻潜在偏见的紧迫需求。

Conclusion: BiasScope框架为自动发现LLM评估中的潜在偏见提供了有效解决方案，JudgeBench-Pro基准测试揭示了当前LLM-as-a-Judge评估系统在鲁棒性方面存在的严重不足，强调了进一步研究和改进的必要性。
...
TL;DR: 论文批判了将LLMs用于合同起草的过度乐观观点，指出LLMs不理解语言意义、缺乏上下文感知和推理能力，仅能生成统计上占优的词模式，无法真正理解具体交易情境和法律推理。


<details>
  <summary>Details</summary>
Motivation: 针对当前普遍认为LLMs可以辅助法律合同起草的观点，作者认为这种想法是不合理的，需要澄清LLMs在合同起草中的实际局限性和潜在风险。

Method: 通过理论分析，对比LLMs的文本生成机制（基于统计词模式）与真正的合同起草需求（理解具体交易情境、进行法律推理）之间的本质差异。

Result: LLMs只能生成表面合理但通用的合同文档，实际上可能产生条款不一致的无用文件，或是虽可执行但不适合特定交易的合同，无法替代专业法律工作。

Conclusion: LLMs不会威胁法律行业的持续生存能力，对LLMs在法律领域应用的过度简化假设需要重新审视，合同起草需要真正的语言理解、上下文感知和法律推理能力。
...
TL;DR: 本文提出了一种扩展多语言预训练语言模型以支持新低资源语言的方法，通过词汇扩展和双语词典初始化来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多语言预训练语言模型(mPLMs)虽然对许多低资源语言有益，但很少有工作研究如何将这些模型扩展到之前不支持的低资源语言。当前方法主要关注已有语言的继续预训练，而缺乏对新语言扩展的系统方法。

Method: 1. 使用目标语言语料库扩展模型词汇表；2. 筛选出偏向源语言(如英语)的原始词汇子集；3. 利用双语词典初始化扩展词汇的表示；4. 基于这些扩展词汇表示，使用目标语言语料库继续预训练mPLMs。

Result: 实验结果显示，与使用随机初始化扩展词汇的基线相比，本文方法在词性标注(POS tagging)和命名实体识别(NER)任务上分别提升了0.54%和2.60%。此外，该方法在训练语料选择上表现出高鲁棒性，且继续预训练后模型在源语言上的性能不会下降。

Conclusion: 本文提出的方法能够有效扩展多语言预训练语言模型以支持新的低资源语言，通过词汇扩展和双语词典初始化的策略显著提升了模型性能，同时保持了源语言能力。
...
TL;DR: 研究发现大型语言模型（LLMs）的道德判断会受到与道德无关的情境因素影响，类似于人类的"情境主义"道德偏见，道德干扰物可使LLMs的道德判断偏移超过30%


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在高风险环境中的广泛应用，确保其行为符合人类价值观变得日益重要。现有道德基准假设LLMs具有相对稳定的道德偏好，但人类道德心理学研究表明人类道德判断会受到与道德无关的情境因素影响，因此需要评估LLMs是否表现出类似的人类认知道德偏见。

Method: 从现有心理学数据集中收集了60个"道德干扰物"（情感性图像和叙事），这些干扰物与所呈现情境没有道德相关性。将这些干扰物注入现有的道德基准中，测量它们对LLM响应的影响。

Result: 研究发现道德干扰物可以使LLMs的道德判断偏移超过30%，即使在低模糊性场景中也是如此。这表明LLMs表现出类似于人类的认知道德偏见，对与道德无关的情境因素敏感。

Conclusion: LLMs的道德判断受到与道德无关的情境因素影响，类似于人类的"情境主义"道德偏见。这突显了需要进行更情境化的道德评估，以及对LLMs进行更细致的认知道德建模的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的社会偏见引发公平性担忧，RAG架构虽然通过检索外部知识增强生成能力，但仍面临相同的偏见挑战。本研究旨在评估和理解RAG的社会偏见影响。

Method: 通过跨多个检索语料库、LLM和偏见评估数据集（涵盖13种以上偏见类型）的广泛实验，评估RAG的偏见表现。然后集成Chain-of-Thought提示到RAG中，分析模型的推理过程并评估其忠实性。

Result: 令人惊讶地观察到RAG减少了偏见，表明外部上下文有助于抵消刻板印象驱动的预测。随着从检索文档中纳入更多上下文信息，模型的偏见倾向在刻板印象和反刻板印象响应之间转变。有趣的是，虽然CoT提高了准确性，但与RAG观察到的偏见减少相反，它增加了整体偏见。

Conclusion: 需要开发偏见感知的推理框架来缓解准确性与偏见之间的权衡，外部上下文可以改善公平性，但推理过程本身可能引入新的偏见挑战。
...
TL;DR: 提出概念文化指数(CCI)来衡量句子层面的文化特异性，通过比较目标文化与其他文化中句子的通用性差异来量化文化特异性，在文化特定句与通用句的分类任务中表现优于直接使用LLM评分。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地在多元文化环境中部署，但缺乏系统性的句子层面文化特异性评估方法。需要一种能够量化句子文化特异性并具有可解释性的指标。

Method: 提出概念文化指数(CCI)，定义为目标文化中的通用性估计与其他文化平均通用性估计之间的差异。通过比较设置让用户操作控制文化范围，分数基于底层通用性估计提供可解释性。

Result: 在400个句子(200个文化特定句和200个通用句)上验证CCI，分数分布呈现预期模式：文化特定句得分更高，通用句得分更低。在二元可分性方面，CCI优于直接LLM评分，在针对目标文化专门化的模型中AUC提高超过10个百分点。

Conclusion: CCI提供了一种有效量化句子层面文化特异性的方法，具有可解释性和操作灵活性，有助于在多元文化环境中更好地评估和部署语言模型。
...
TL;DR: NOWJ系统在BioCreative IX的ToxHabits共享任务中，针对西班牙语临床文本的毒物使用检测，提出多输出集成方法，在触发检测和参数检测上取得优异表现


<details>
  <summary>Details</summary>
Motivation: 从非结构化电子健康记录中提取药物使用信息是临床自然语言处理的主要挑战。虽然大语言模型有所进展，但在临床NLP中的应用受到信任、控制和效率问题的限制

Method: 提出多输出集成系统，集成BETO与CRF层进行序列标注，采用多样化训练策略，使用句子过滤提高精度，同时处理ToxNER和ToxUse两个子任务

Result: 最佳运行在触发检测上获得0.94 F1和0.97精确度，在参数检测上获得0.91 F1分数

Conclusion: NOWJ系统在特定领域、低资源的西班牙语临床文本毒物使用检测任务中表现出色，为解决临床NLP中的信任、控制和效率问题提供了有效方案
...
TL;DR: CoCoA解码器：一种无需训练的解码算法，通过分析LLM中间层的表示不稳定性来检测和减少幻觉，提高生成文本的事实准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型容易产生流畅但事实错误的文本（幻觉现象），这降低了其在下游任务中的可靠性和实用性。作者假设生成文本片段的事实性与其在模型内部层之间的表示不稳定性相关。

Method: 提出CoCoA（混淆和一致性感知）解码器，这是一种无需训练的推理时解码算法。通过量化中间层的表示不稳定性（提出两个指标），利用这些信号来惩罚内部混淆高的输出，引导模型生成更内部一致且事实基础更强的输出。还提出了自信息门控变体CoCoA-SIG，动态调节惩罚以选择性针对高惊喜、不稳定的生成。
...
TL;DR: Gt-Margin方法通过真实token的概率差值确定最佳解掩码顺序，训练监督解掩码规划器提升MDLM推理性能


<details>
  <summary>Details</summary>
Motivation: 传统MDLM在推理时依赖启发式置信度或强化学习决定解掩码顺序，这些方法成本高且效果有限，需要更有效的解掩码顺序优化方案

Method: 提出Gt-Margin评分，基于真实token与最强替代token的概率差值确定最优解掩码顺序；训练监督解掩码规划器通过排序学习模仿该顺序

Result: 使用Gt-Margin确定的最优解掩码顺序显著提升生成质量，尤其在逻辑推理任务上；训练的规划器在不修改token预测模型的情况下提高推理准确性

Conclusion: Gt-Margin为MDLM提供了有效的解掩码顺序优化方法，监督解掩码规划器能够有效提升推理性能，为MDLM的推理优化提供了新思路
...
TL;DR: EcoGym是一个用于评估LLM智能体长期规划和执行能力的基准测试平台，包含三个经济环境，通过1000+步的连续决策过程评估智能体的战略一致性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架大多是情景化的、领域特定的，或者缺乏持续经济动态的坚实基础，无法充分评估LLM智能体的长期规划能力。

Method: 开发了EcoGym基准测试，包含三个多样化经济环境（自动售货、自由职业、运营），采用统一的决策过程和标准化接口，在部分可观测和随机性条件下进行1000+步的长期评估。

Result: 对11个领先LLM的测试显示，没有单一模型在所有三个场景中都表现最优，模型在高层战略或高效行动执行方面存在显著次优性。

Conclusion: EcoGym作为一个开放、可扩展的测试平台，为透明评估长期智能体和研究现实经济环境中的可控性-效用权衡提供了工具。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现大型语言模型在搜索增强推理中存在知识整合衰减瓶颈，即随着推理链增长，模型越来越难以将检索到的证据整合到后续推理步骤中，即使相关信息可用也会限制性能

Method: 提出Self-Anchored Knowledge Encoding (SAKE)方法，这是一种无需训练、在推理时使用的策略，通过在推理过程开始和结束时锚定检索到的知识，防止知识被先前上下文掩盖，保持其语义完整性

Result: 在多跳问答和复杂推理基准测试上的广泛实验表明，SAKE显著缓解了知识整合衰减问题并提高了性能

Conclusion: SAKE为智能LLM中的知识整合提供了一个轻量级但有效的解决方案，通过稳定知识利用来改善搜索增强推理的效果
...
TL;DR: 本文提出MoSLoRA方法用于训练统一自回归奖励模型(UniARM)，解决多目标对齐中特征纠缠问题，通过共享特征提取和偏好调制实现精确的偏好权衡控制。
...
Method: 提出Preference-Modulated & Shared Low-Rank Adaptation (MoSLoRA)方法：首先通过偏好无关模块提取共享特征，然后通过偏好调制模块对共享特征进行仿射变换，该模块以混合偏好向量为条件。基于此构建Unified Autoregressive Reward Model (UniARM)框架，在单一参数空间中联合建模所有偏好维度。

Result: MoSLoRA设计缓解了特征纠缠问题，并在推理过程中实现了对偏好权衡的精确控制。UniARM消除了为每个偏好目标设置独立参数的需求，并在更大规模LLMs上展现出实际可用性。

Conclusion: 提出的MoSLoRA和UniARM框架有效解决了多目标对齐中的特征纠缠问题，通过共享特征提取和偏好调制实现了更精确的偏好控制，为多目标测试时对齐提供了更优的低成本解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究大多孤立评估RAG方法且主要关注单轮设置，缺乏对多轮对话问答中RAG方法的系统性比较，而多轮对话中的对话历史、指代消解和用户意图变化使检索更加复杂。

Method: 使用统一的实验设置，在八个不同领域的对话问答数据集上评估了基础和高级RAG方法，包括检索质量和答案生成，使用生成器和检索指标，并分析性能随对话轮次的变化。

Result: 结果显示，稳健而简单的方法（如重排序、混合BM25和HyDE）始终优于基础RAG，而几种高级技术未能带来改进，甚至可能使性能低于无RAG基线。数据集特性和对话长度强烈影响检索效果。

Conclusion: 有效的对话RAG更多地取决于检索策略与数据集结构的匹配，而不是方法的复杂性。没有单一的RAG策略在所有设置中占主导地位。
...
Method: 提出统一测试时缩放框架：1) BACD解码策略，基于模型置信度动态调整去噪；2) TCCF范式，大块用于探索性推理，小块用于细化；3) 渐进块大小扩展，缓解性能下降

Result: 在TDAR-8B上应用BACD和TCCF，相比TraDo-8B实现2.26倍加速，AIME24提升11.2分，显著优于强基线

Conclusion: 该框架为BDLMs在复杂推理任务中的测试时缩放潜力解锁迈出重要一步，有效平衡解码速度与推理效果
...
TL;DR: 该论文介绍了LEMUR，一个基于欧盟环境立法PDF文档构建的大规模多语言法律语料库，并通过对多语言嵌入模型进行法律领域微调，显著提升了法律检索性能，特别是在低资源语言上效果明显。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在法律信息检索中的应用受到两个主要限制：不可靠的检索能力和缺乏领域适应的开放嵌入模型。现有的多语言法律语料库不适合语义检索，且PDF立法源由于文本提取不完美而引入大量噪声。

Method: 1. 构建LEMUR语料库：从24,953个欧盟官方EUR-Lex PDF文档中提取，涵盖25种语言；2. 使用词汇内容评分量化PDF到文本转换的保真度；3. 在单语和双语设置下使用对比目标对三种最先进的多语言嵌入模型进行法律领域微调。

Result: 法律领域微调在低资源和高资源语言上都持续提高了Top-k检索准确性，特别是在低资源语言上提升尤为显著。跨语言评估显示这些改进能够迁移到未见过的语言，表明微调主要增强了语言无关的内容级法律表示而非语言特定线索。

Conclusion: 通过构建高质量的多语言法律语料库并进行领域特定的模型微调，可以有效提升多语言法律检索性能，特别是在低资源语言环境中。该方法为多语言法律信息访问提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有树搜索解码策略大多忽略预算约束，将其仅视为终止条件，导致在实际部署中可能出现后期过度分支或过早终止的问题。实际部署中每个查询都有固定的token预算，且不同设置下预算不同。

Method: 提出预算引导的MCTS（BG-MCTS）算法，根据剩余token预算动态调整搜索策略：开始时进行广泛探索，随着预算减少优先进行精炼和答案完成，同时减少浅层节点的后期分支。

Result: 在不同预算条件下，BG-MCTS在MATH500和AIME24/25数据集上使用开源权重大语言模型时，始终优于预算无关的树搜索基线方法。

Conclusion: 预算引导的树搜索解码策略能够有效利用有限的token预算，通过动态调整搜索策略提高解码效率，在实际部署中具有更好的性能表现。
...
TL;DR: 本文提出Context-CDA方法，通过增强上下文多样性和相关性来改进反事实数据增强，在减少语言模型社会偏见的同时保持语言建模能力


<details>
  <summary>Details</summary>
Motivation: 传统反事实数据增强(CDA)方法在减少语言模型社会偏见时存在两个主要问题：1)生成的合成数据可能与真实世界分布不一致；2)创建的反事实过于简单，忽略了敏感属性（如性别）在预训练语料中的社会背景。这些问题可能导致语言建模能力下降，影响下游任务性能。

Method: 提出Context-CDA方法：1)使用大型语言模型增强反事实数据的多样性和上下文相关性，使去偏见语料与预训练数据更好对齐；2)采用基于不确定性的过滤机制，排除目标较小语言模型认为质量低的反事实生成，进一步提高微调语料质量。

Result: 在性别偏见基准测试上的实验结果表明，Context-CDA能有效减少偏见，同时不牺牲语言建模性能。通过分析下一个token生成概率的分布变化，该方法还提供了对社会偏见的深入洞察。

Conclusion: Context-CDA是一种简单而有效的上下文增强反事实数据增强方法，通过改善去偏见语料与预训练数据的对齐，在减少语言模型社会偏见的同时保持了语言建模能力，为解决偏见缓解与模型性能之间的权衡问题提供了有效方案。
...
TL;DR: 研究比较了不同长度控制方法对LLM推理效率与性能的影响，发现长度惩罚可能阻碍推理获取，而适当调整的长度控制能提升具有强推理先验模型的效率。


<details>
  <summary>Details</summary>
Motivation: 强化学习虽然能提升大语言模型的推理能力，但会导致思维链输出变长，增加训练和推理的计算成本。现有长度控制方法存在不足，不清楚如何平衡效率与性能的最佳输出长度。

Method: 在Qwen3-1.7B Base和DeepSeek-R1-Distill-Qwen-1.5B两个模型上比较了多种长度控制方法，并将先前工作扩展到RL训练策略中进行分析。

Result: 长度惩罚可能阻碍推理获取，而适当调整的长度控制能提升具有强推理先验模型的效率。识别出两种失败模式：长输出增加分散性，短输出导致思考不足。

Conclusion: 需要针对不同模型特性设计合适的长度控制策略，平衡推理性能与计算效率，避免过长导致的分散和过短导致的思考不足问题。
...
TL;DR: ELPO提出了一种针对工具集成推理的错误定位策略优化方法，通过二分搜索定位首个不可恢复错误步骤，使用分层优势归因生成稳定学习信号，并通过自适应裁剪强化关键步骤的修正更新，在数学、科学问答和代码执行等任务上优于现有智能体强化学习方法。


<details>
  <summary>Details</summary>
Motivation: 在工具集成推理中，仅基于结果的强化学习面临稀疏、延迟奖励和弱步骤级信用分配问题。在长时程轨迹中，早期不可恢复错误会决定任务成败，因此需要定位首个不可恢复步骤并进行细粒度信用分配。

Method: ELPO包含三个核心组件：1) 通过二分搜索在固定预算下定位首个不可恢复步骤；2) 将结果树转换为分层优势归因的稳定学习信号；3) 应用错误定位自适应裁剪，强化对关键步骤及其后续步骤的修正更新。

Result: 在数学、科学问答和代码执行等工具集成推理基准测试中，ELPO在相同采样预算下持续优于现有智能体强化学习方法，在Pass@K和Major@K扩展、轨迹排序质量和工具调用效率方面均有额外提升。

Conclusion: ELPO通过定位首个不可恢复错误步骤并利用其进行细粒度信用分配，有效解决了工具集成推理中的稀疏奖励和信用分配问题，为智能体强化学习提供了更高效的优化框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的后训练对齐工作流程分散在特定后端工具和临时粘合代码中，导致实验难以复现，存在后端干扰、奖励碎片化和不可复现流水线等关键障碍。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM评估通常需要地面真实标注或复杂的评估者协调，这在实际应用中存在成本高、可扩展性差的问题。需要一种无需参考标注、灵活且与人类判断对齐的评估框架。

Method: 采用参考无关的评估框架，使用独立提示的评估者集合，基于人类对齐的模式进行指导。支持离散和连续评分判断，通过任务特定提示（从最佳候选选择、摘要、图像描述到对话）实现灵活评估。

Result: 实验表明该框架与人类判断高度对齐，优于先前方法，同时减少了计算开销。提供了高效、鲁棒且与人类对齐的解决方案，适用于实际LLM评估场景。

Conclusion: MILE-RefHumEval是一个无需参考标注的LLM评估框架，通过独立评估者集合和人类对齐模式，实现了灵活、可扩展、与人类判断一致的评估，为实际应用提供了高效解决方案。
...
TL;DR: MATA是一个多智能体表格问答框架，通过多种互补推理路径和小语言模型工具来提升表格理解的可靠性、可扩展性和效率。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在表格理解任务上取得了进展，但在资源受限或隐私敏感环境中仍面临可靠性、可扩展性和效率方面的挑战，需要更有效的解决方案。

Method: MATA采用多智能体框架，通过多种推理风格生成候选答案，利用小语言模型构建的工具集进行答案优化或选择，并设计了减少昂贵LLM调用的算法来提高效率。

Result: 在两个不同难度的基准测试中使用十种不同LLM的实验表明，MATA实现了最先进的准确性和高效推理，同时避免了过多的LLM推理开销。

Conclusion: 精心编排多种推理路径可以实现可扩展且可靠的表格问答，MATA在保持小规模开源模型强大性能的同时，能够轻松适应不同类型的LLM。
...
TL;DR: 该研究评估了知识蒸馏方法在机器翻译中的综合性能，不仅考虑翻译质量，还计算了包括碳排放在内的计算成本，为在计算约束下选择KD方法提供了可复现的指导。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译中的知识蒸馏研究通常只报告学生的翻译质量，而忽略了KD过程的计算复杂度，这使得在计算约束下难以从众多KD方法中选择合适的方案。

Method: 使用机器学习生命周期评估工具，将计算成本量化为碳足迹，考虑了KD模型生命周期中的运行时操作排放和硬件生产成本的摊销，评估了代表性的KD方法。

Result: 研究发现：(1)在小规模部署时，蒸馏开销主导总碳足迹；(2)在大规模部署时，推理主导碳足迹，KD仅在超过任务相关使用阈值时才有利；(3)词级蒸馏通常比序列级蒸馏提供更有利的碳足迹-质量权衡。

Conclusion: 该研究协议为在明确的质量和计算约束下选择KD方法提供了可复现的指导，强调了在评估KD方法时需要综合考虑翻译质量和计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然支持多种语言，但方言变体由于数据有限和语言变异而代表性不足，需要专门的方法来提升方言生成性能
...
Conclusion: 该方法为鲁棒的方言阿拉伯语生成提供了一个紧凑有效的框架，能够改善LLM的方言性能
...
TL;DR: TraceMem是一个受认知启发的记忆框架，通过三阶段流水线从用户对话痕迹中构建结构化叙事记忆模式，在长程对话中实现更好的叙事连贯性和推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在处理长期交互时面临上下文窗口有限的瓶颈，现有记忆系统将交互视为离散片段，无法捕捉对话流的底层叙事连贯性。

Method: 提出TraceMem框架，包含三个阶段：1) 短期记忆处理：使用演绎式主题分割方法划分情节边界并提取语义表示；2) 突触记忆巩固：将情节总结为情景记忆，然后与语义一起提炼为用户特定痕迹；3) 系统记忆巩固：使用两阶段层次聚类将这些痕迹组织成连贯的、随时间演化的叙事线索，形成结构化用户记忆卡片。

Result: 在LoCoMo基准测试中达到最先进性能，分析显示通过构建连贯叙事，在多跳推理和时间推理方面超越基线，突显其在深度叙事理解中的重要作用。

Conclusion: TraceMem通过受大脑启发的架构构建连贯的叙事记忆模式，显著提升了长期对话中的叙事理解和推理能力，为记忆系统领域提供了新的视角和未来展望。
...
TL;DR: 研究人员开发了一个基于Gemini的AI协作科学评估系统，在气候科学领域测试了AI如何加速科学工作流程，特别是在AMOC稳定性这一复杂主题上。


<details>
  <summary>Details</summary>
Motivation: 当前AI科研助手主要针对可重复验证的任务，但无法处理那些无法重复评估、需要基于理论和现有证据达成共识的科学问题。本研究旨在探索AI在需要专家共识的科学评估中的潜力。

Method: 开发了一个基于Gemini的AI协作科学评估环境，整合到标准科学工作流程中。与13位气候科学家合作，在AMOC稳定性这一复杂主题上测试系统，通过104次修订循环分析79篇论文。

Result: AI显著加速了科学工作流程：团队在46人时内完成了79篇论文的综合分析。大部分AI生成内容被保留在报告中，AI帮助保持了逻辑一致性和呈现质量。但专家补充至关重要：只有不到一半的报告由AI生成，需要大量监督才能达到严格的科学标准。

Conclusion: AI能够加速科学工作流程，但需要与人类专家紧密协作。AI在内容生成和一致性维护方面表现良好，但专家监督对于确保科学严谨性和可接受性至关重要。这为AI在需要共识的科学评估中的应用提供了重要见解。
...
TL;DR: 构建了一个包含657个新约译本的多语言语料库，其中352个是独特版本，在英语、法语、意大利语、波兰语和西班牙语五个语言中提供了前所未有的深度覆盖
...
Method: 从12个在线圣经图书馆和一个现有语料库中收集657个新约译本，手动为每个翻译标注元数据，包括作品标准化标识符、具体版本和修订年份，实现文本规范化

Result: 创建了包含英语（208个独特版本/396个总数）、法语（41/78）、意大利语（18/33）、波兰语（30/48）和西班牙语（55/102）的多语言语料库，为翻译历史研究提供了灵活的多层次分析资源

Conclusion: 该语料库通过提供首个支持灵活多层次分析的资源，为翻译历史的定量研究设立了新基准，使研究人员能够根据自身需求定义"独特性"，进行从微观翻译家族到宏观研究的分析
...
TL;DR: 该论文提出了一种将词义变化检测中的上下文嵌入映射到神经生物学特征空间的方法，以提高词义变化解释性并提升性能


<details>
  <summary>Details</summary>
Motivation: 词义变化研究大多关注变化程度的估计性能，但难以解释词义如何变化。提高词义变化的可解释性是该领域的重要挑战，可能带来新的见解。

Method: 将预训练语言模型获得的上下文词嵌入映射到神经生物学特征空间，该空间中每个维度对应词的原始特征，值表示该特征的强度，使人类能够系统解释词义变化。

Result: 在词义变化程度估计方面，该方法优于大多数先前方法。由于高可解释性，能够进行多种词义变化分析，发现了先前研究中被忽视的有趣词义变化类型，并能有效搜索具有特定类型词义变化的词。

Conclusion: 提出的方法不仅提高了词义变化检测的性能，更重要的是增强了可解释性，能够系统分析词义变化模式，为词义变化研究提供了新的分析工具和见解。
...
TL;DR: 首个班巴拉语自动语音识别标准化基准，使用1小时专业录制的马里宪法文本，评估37个模型，结果显示当前ASR性能远未达到部署标准


<details>
  <summary>Details</summary>
Motivation: 为班巴拉语（一种代表性不足的语言）建立首个标准化ASR评估基准，以促进该语言语音技术的发展

Method: 创建1小时专业录制的马里宪法文本数据集作为基准，在接近最优的声学和语言条件下评估37个模型（包括班巴拉语训练系统和大型商业模型）

Result: 最佳模型的词错误率为46.76%，最佳字符错误率为13.00%，多个知名多语言模型的词错误率超过100%，表明当前ASR性能远未达到部署标准

Conclusion: 多语言预训练和模型扩展不足以支持代表性不足的语言，需要针对特定语言的专门研究；基准数据集代表最佳情况，实际应用中的性能可能更差；提供公开基准和排行榜以促进透明评估和未来研究
...
TL;DR: 论文提出了一个可追踪的框架来评估大语言模型的推理效率，将token效率分解为可解释的因素，发现准确性和token效率排名存在差异，效率差距主要由条件正确性驱动。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型推理评估只报告最终准确率，掩盖了token使用效率问题。需要一种能够分解token效率、揭示token使用模式的方法来更好地理解和优化模型推理效率。

Method: 提出了一个可追踪的框架，将token效率分解为：固定token预算下的完成度、给定完成度的条件正确性、冗余度。当有基准元数据时，进一步将冗余度分解为平均表达开销和耦合系数。当有推理轨迹时，添加确定性轨迹质量度量（如基础性、重复性、提示复制）来区分退化循环和参与式推理。

Result: 在CogniLoad基准上评估25个模型，发现准确性和token效率排名存在差异（Spearman ρ=0.63），效率差距主要由条件正确性驱动，表达开销在不同模型间变化约9倍，且与模型规模关系较弱。分解揭示了不同的瓶颈模式，提示不同的效率优化策略。

Conclusion: 提出的框架能够深入分析大语言模型的推理效率，揭示token使用模式，为模型优化提供有针对性的指导。准确率不能完全反映推理效率，需要综合考虑多个因素来评估和改进模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 科学计量问答作为科学问答的子任务面临独特挑战，需要识别学术实体并检索多维度科学计量数据（如影响因子）。大语言模型在处理复杂任务如任务分解、规划和推理方面展现出潜力，但在此特定领域的应用尚未充分探索。

Method: 开发了端到端系统，采用顺序工作流结合检索增强生成和智能体概念。使用专有研究绩效评估平台作为检索数据库，通过大语言模型作为评判者进行评估，并咨询领域专家验证效果。

Result: 系统能够有效处理科学计量问答任务，成功识别学术实体并检索相关科学计量数据。通过专家评估和大语言模型评判验证了系统在特定下游任务中的有效性。

Conclusion: AnalyticsGPT展示了LLMs在科学计量问答这一小众下游任务中的有效性，为类似应用提供了有价值的见解。代码和提示已开源，促进了该领域的研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语言模型主要在正式场合使用的现代标准阿拉伯语（MSA）上预训练，但人们在线上交流时使用各种方言。由于方言与MSA的相似度不同，这对阿拉伯语言模型构成了限制，需要研究跨方言迁移的有效性。

Method: 通过3个自然语言处理任务的探测分析和表征相似性研究，评估阿拉伯语言模型的跨方言迁移能力，并分析地理邻近性对迁移效果的影响。

Result: 研究发现跨方言迁移是可能的，但在不同方言间存在不均衡性，这种差异部分可由地理邻近性解释。同时发现支持所有阿拉伯方言的模型存在负向干扰的证据。

Conclusion: 阿拉伯方言之间的相似程度值得质疑，这引发了人们对阿拉伯语言模型跨方言迁移能力的担忧。研究结果对阿拉伯语言模型的开发和应用具有重要意义。
...
TL;DR: 研究证明利用大语言模型生成的推理内容可以预测其自身预测的正确性，在教育对话分析中实现了83%的F1分数，为自动化教育分析提供了实用的质量控制方法。


<details>
  <summary>Details</summary>
Motivation: 当前使用大语言模型自动标注和分析教育对话的流程缺乏可靠的方法来检测模型何时出错，需要开发能够预测模型预测正确性的方法。

Method: 分析了30,300个教师话语，每个都由多个先进的大语言模型标注教学行为结构并附带推理。使用TF-IDF编码LLM推理，评估了五种监督分类器，并训练特定教学行为结构的专业检测器。使用LIWC框架分析正确与错误推理的语言特征。

Result: 随机森林分类器达到F1分数0.83（召回率0.854），成功识别大多数错误预测。正确预测表现出基于因果的语言，而错误推理更可能依赖认知性模糊表达和元认知表达。特定教学行为结构的专业检测器进一步提升了困难结构的性能。

Conclusion: 基于推理的错误检测为自动化教育对话分析提供了实用且可扩展的质量控制方法，特定结构的检测器能利用结构特定的语言线索提升性能。
...
TL;DR: 研究普通话中自然语言产出的量化表达，通过图片描述任务分析说话者是否量化、量化精度及策略选择，发现物体数量、生命性和产出模态影响量化行为


<details>
  <summary>Details</summary>
Motivation: 量化是日常语言的基本组成部分，但人们对说话者在自然产出中如何决定是否及如何进行量化知之甚少。本研究旨在探索普通话中量化表达的自然使用情况

Method: 使用基于图片的引发描述任务，让说话者自由描述包含多个物体的场景，没有明确指示要计数或量化。同时考察口语和书面语两种模态

Result: 物体数量、生命性和产出模态系统性地影响量化行为：物体数量增加会降低量化的可能性和精度；生命性指称和产出模态选择性地调节策略选择

Conclusion: 本研究展示了如何在无约束的产出条件下研究量化表达，并为语言产出中数量表达的进一步分析提供了自然主义数据集
...
TL;DR: 该研究构建了一个包含2,344个僧伽罗语习语的数据集，包含文化和跨语言标注，用于评估LLM在低资源语言文化习语翻译中的表现，发现现有模型存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 神经机器翻译在处理高资源语言的比喻表达时表现较好，但在处理僧伽罗语等低资源语言时面临数据有限的挑战，需要专门的资源来支持文化敏感的机器翻译研究。

Method: 构建了包含2,344个僧伽罗语习语的数据集，包含文化和跨语言标注；开发了二元分类器区分两类习语；评估了现有LLM在该数据集上的表现。

Result: 二元分类器达到约92%的准确率；评估发现现有LLM在准确传达习语含义方面存在显著不足，经常难以正确处理文化习语。

Conclusion: 该数据集为低资源NLP和文化感知机器翻译提供了重要基准，揭示了当前LLM在处理文化习语方面的局限性，为未来研究提供了方向。
...
TL;DR: Steer2Edit：一种无需训练的方法，将推理时的激活干预转化为组件级别的权重编辑，通过选择性重新分配行为影响来改善属性-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 传统的引导方法通过推理时激活干预来修改大语言模型行为，但这种方法通常采用固定的全局修改，忽略了行为主要由少数异质组件控制的事实，导致在强控制下产生不利的属性-效用权衡。

Method: Steer2Edit是一个理论基础的无需训练框架，将引导向量从推理时控制信号转化为诊断信号，用于组件级别的秩-1权重编辑。它选择性地将行为影响重新分配到各个注意力头和MLP神经元，而不是在生成过程中统一注入引导方向。
...
TL;DR: 多智能体系统无法同时实现持续自我进化、完全隔离和安全不变性，这被称为自我进化三难困境。理论和实证表明，封闭的自我进化必然导致安全对齐的不可逆退化。


<details>
  <summary>Details</summary>
Motivation: 研究多智能体系统在实现持续自我进化、完全隔离和安全不变性这三重目标时面临的根本限制，揭示封闭自我进化系统内在的安全风险。

Method: 采用信息论框架，将安全形式化为与人类价值分布的偏离程度。通过理论分析证明孤立自我进化会导致统计盲点，并在开放智能体社区和两个封闭自我进化系统中进行实证验证。

Result: 理论和实证结果一致表明，满足持续自我进化、完全隔离和安全不变性的智能体社会是不可能的。封闭的自我进化系统必然导致安全对齐的不可逆退化，与理论预测相符。
...
TL;DR: 通过分析LLM内部表征预测其自身成功率，实现更高效推理：训练线性探针预测模型在数学和编程任务上的表现，利用该信号指导跨模型路由，在MATH数据集上减少70%推理成本


<details>
  <summary>Details</summary>
Motivation: 运行LLM进行扩展推理成本高昂，但难以确定哪些输入真正需要额外计算资源。研究是否可以从LLM生成前的内部表征中恢复其自身成功率，并利用该信号指导更高效的推理

Method: 在生成前激活上训练线性探针，预测模型在数学和编程任务上的策略特定成功率；使用E2H-AMC数据集对比人类和模型在相同问题上的表现；利用探针信号指导跨模型查询路由

Result: 线性探针显著优于表面特征（如问题长度和TF-IDF）；模型编码了与人类难度不同的模型特定难度概念，且这种差异随扩展推理增加；跨模型路由在MATH数据集上可减少70%推理成本，同时性能超过最佳单模型

Conclusion: LLM内部表征包含可预测其自身成功率的信号，即使这些表征与人类对难度的直觉不同，仍能实现实际效率提升；跨模型路由策略可显著降低推理成本同时保持或提升性能
...
TL;DR: ATTNPO：利用注意力信号进行步骤级信用分配的轻量级过程监督RL框架，有效减少过度思考同时保持准确性


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习和可验证奖励的大推理模型存在过度思考问题，生成冗余推理步骤而不提升性能。轨迹级长度惩罚方法效果有限，无法区分必要步骤和冗余步骤，而过程监督方法通常资源密集且信用分配不准确。

Method: 提出ATTNPO框架，利用模型内在注意力信号进行步骤级信用分配。首先识别一组特殊注意力头，这些头自然关注必要步骤而抑制冗余步骤。然后利用这些头的注意力分数，采用两种子策略：1) 通过惩罚冗余步骤减少过度思考；2) 减少对必要步骤的惩罚以保持准确性。

Result: 实验结果表明，ATTNPO在9个基准测试中显著减少推理长度，同时显著提升性能。

Conclusion: ATTNPO通过利用模型内在注意力信号进行精细的步骤级信用分配，有效解决了推理模型过度思考问题，在减少推理长度的同时提升准确性，且计算开销较低。
...
TL;DR: 提出ViMultiChoice方法，用于越南语阅读理解任务，既能预测正确答案又能生成解释，在ViMMRC 2.0基准和新数据集上达到SotA性能


<details>
  <summary>Details</summary>
Motivation: 现有的多项选择阅读理解模型通常只能选择答案，缺乏解释推理过程的能力，特别是在越南语领域缺乏相关数据集和方法

Method: 提出ViMultiChoice方法，专门为越南语阅读理解设计，联合训练选项决策和解释生成，同时预测正确答案并生成相应解释

Result: ViMultiChoice在ViMMRC 2.0基准和新数据集上优于现有基线，达到最先进性能；联合训练显著提高了多项选择的准确性

Conclusion: 提出的越南语数据集和ViMultiChoice方法有效解决了多项选择阅读理解中缺乏解释能力的问题，联合训练策略能显著提升模型性能
...
TL;DR: ViSpeechFormer是首个基于音素建模的越南语语音识别框架，利用越南语高透明度的字形-音素对应关系，在公开数据集上表现出色，泛化能力强且受训练偏差影响小。


<details>
  <summary>Details</summary>
Motivation: 越南语具有高度透明的字形-音素对应关系（每个字素对应最多一个音素），这为开发基于音素的语音识别系统提供了独特优势。目前还没有专门为越南语设计的显式音素建模ASR框架。

Method: 提出ViSpeechFormer（越南语音转换器），这是一个基于音素的越南语自动语音识别方法。该方法利用越南语的高字形-音素透明度，显式地建模音素表示，是首个采用这种范式的越南语ASR框架。

Result: 在两个公开可用的越南语ASR数据集上的实验表明，ViSpeechFormer实现了强大的性能，对词汇外单词具有更好的泛化能力，并且受训练偏差的影响较小。

Conclusion: 这种基于音素的范式对于其他具有语音正字法的语言也很有前景。该方法为越南语ASR提供了一种新颖有效的解决方案，代码将在论文被接受后发布。
...
TL;DR: 本文提出了一个多维度的无参考评估框架，用于评估LLM在自然灾害等高风险领域特定问答中的表现，重点关注特异性、鲁棒性、相关性和上下文利用四个维度。


<details>
  <summary>Details</summary>
Motivation: 现有RAG和开放式问答评估框架主要依赖表面相似性、事实一致性或语义相关性，无法评估LLM在领域敏感决策中是否提供了所需的特定信息，特别是在高风险领域如自然灾害响应和基础设施规划中。

Method: 提出了一个多维度的无参考评估框架，从四个互补维度评估LLM输出：特异性、对改写和语义扰动的鲁棒性、答案相关性、上下文利用。创建了包含1,412个领域特定问答对的数据集，涵盖40个专业角色和7种自然灾害类型，并进行人工评估以验证框架。

Result: 研究结果显示，没有任何单一指标能够独立充分捕捉答案质量，强调了在高风险应用中部署LLM时需要结构化、多指标评估框架的重要性。人工评估显示了领域特定开放式评估固有的主观性。

Conclusion: 在高风险领域特定应用中部署LLM时，需要采用结构化、多维度的评估框架，而不是依赖单一指标，以全面评估LLM输出的质量和对决策的支持能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如RAG和知识编辑）在将动态知识集成到LLM中面临上下文窗口有限、检索器噪声和灾难性遗忘等限制，需要一种更有效的方法来解耦知识提取和推理过程。

Method: DRIFT采用双模型架构：轻量级知识模型根据查询动态压缩文档块为隐式事实标记，然后将这些密集表示投影到推理模型的嵌入空间，替代原始冗余文本。

Result: 实验表明DRIFT在长上下文任务上显著提升性能，优于同类规模模型中的强基线方法，同时保持推理准确性。

Conclusion: DRIFT为扩展LLM的有效上下文窗口和推理能力提供了一种可扩展且高效的范式，成功实现了知识提取与推理过程的解耦。
...
TL;DR: 该论文提出了一种联合实现证据检索、多模态声明验证和解释生成的新模型，特别针对需要结合文本和视觉证据进行联合推理的场景，并创建了AI领域的科学数据集AIChartClaim。


<details>
  <summary>Details</summary>
Motivation: 当前大多数声明验证工作主要关注文本证据推理或忽略可解释性，导致验证结果不准确且缺乏说服力。验证声明的真实性通常需要结合文本和视觉证据进行联合多模态推理，同时需要文本解释来使推理过程透明化。

Method: 1. 证据检索：构建双层多模态图，设计图像到文本和文本到图像推理进行多模态检索；2. 声明验证：提出标记级和证据级融合，整合声明和证据嵌入进行多模态验证；3. 解释生成：引入多模态融合解码器实现可解释性；4. 创建AIChartClaim科学数据集。

Result: 实验结果表明该模型具有优势，能够有效处理多模态声明验证任务。

Conclusion: 该研究提出了一种联合多模态声明验证框架，通过证据检索、多模态验证和解释生成三个模块的协同工作，解决了现有方法在多模态推理和可解释性方面的不足，并贡献了AI领域的科学数据集。
...
TL;DR: 本文提出了AnaBench基准测试和Anagent多智能体框架，用于解决科学表格和图表分析中的复杂挑战，通过专业化智能体分工和模块化训练策略显著提升了分析性能。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在科学表格和图表分析方面存在困难，主要挑战包括：多模态知识的复杂解释、不同来源证据的整合、领域特定知识的推理需求，以及科学图表的结构异质性和长上下文要求。

Method: 提出了Anagent多智能体框架，包含四个专业化智能体：Planner（任务分解）、Expert（信息检索）、Solver（信息合成分析）、Critic（迭代优化）。采用模块化训练策略，结合监督微调和专业化强化学习来优化个体能力并保持有效协作。

Result: 在170个子领域的全面评估中，Anagent实现了显著改进：在无需训练的情况下提升13.43%，经过微调后提升42.12%。研究还发现面向任务的推理和上下文感知的问题解决对高质量科学图表分析至关重要。

Conclusion: Anagent多智能体框架通过专业化智能体分工和模块化训练策略，有效解决了科学表格和图表分析的复杂挑战，为科学AI分析系统的发展提供了重要参考。
...
TL;DR: Quantum-Audit是一个包含2700个问题的量子计算概念理解基准测试，评估了26个主流语言模型。结果显示Claude Opus 4.5以84%准确率超过专家平均水平，但模型在专家编写问题、高级主题和识别错误前提方面表现较差。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估量子代码生成和电路设计，但缺乏对语言模型量子计算概念理解能力的系统测量。需要填补这一空白，全面评估模型对量子计算核心概念的理解深度。

Method: 创建包含2700个问题的Quantum-Audit基准：1000个专家编写问题，1000个通过LLM从研究论文中提取并经过专家验证的问题，以及700个额外问题（包括350个开放式问题和350个包含错误前提的问题）。评估了来自领先组织的26个模型。

Result: 人类参与者得分在23%-86%之间，专家平均74%。最佳模型Claude Opus 4.5达到84%准确率，但顶级模型在专家编写问题上比LLM生成问题平均低12个百分点。在高级主题（如安全）上准确率降至73%。模型在识别错误前提方面表现不佳，准确率低于66%。

Conclusion: 虽然顶级语言模型在量子计算概念理解上已超过专家平均水平，但在处理专家编写问题、高级主题和批判性推理（识别错误前提）方面仍有显著不足，需要进一步改进。
...
TL;DR: Cadmus系统使用小型模型（训练成本低于200美元）研究程序合成，相比大型语言模型具有成本低、可控性强、可解释性好的优势，在特定任务上甚至能超越GPT-5。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型进行程序合成研究存在多个问题：难以确定分布内外数据、微调效果不明确、分词影响复杂、计算和存储成本高。需要一种成本低、可控性强的小型模型系统来深入研究程序完成、分布外表示、归纳推理和指令跟随等任务。

Method: 开发了Cadmus系统，包括整数虚拟机、多样化真实程序数据集，以及基于自回归Transformer的模型。该系统训练成本低于200美元，研究人员可以精细控制训练分布，并能检查和插装模型。

Result: Cadmus模型在特定领域语言（DSL）的整数算术程序完成任务上实现了100%准确率，而GPT-5只有95%准确率。同时发现GPT-5在解决相同任务时引入了未知的先验知识，这成为使用大型LLM进行研究时的混杂因素。

Conclusion: 小型模型在复杂推理任务上具有成本效益和可解释性优势，能够支持程序合成、分布外表示、归纳推理和指令跟随等研究，而大型LLM的未知先验知识会影响对训练集与任务关系的理解。
...
### [129] [PABU: Progress-Aware Belief Update for Efficient LLM Agents](https://arxiv.org/abs/2602.09138)
...
Method: 提出Progress-Aware Belief Update (PABU)信念状态框架：1) 显式建模任务进度，预测相对进度变化；2) 选择性保留过去动作和观察，只将必要的交互信息存储到信念状态中；3) 基于保留的子集进行未来决策。

Result: 在AgentGym基准测试的8个环境中，PABU达到81.0%的任务完成率，比基于完整历史信念的SOTA模型提升23.9%。平均交互步数减少到9.5步，对应26.9%的效率提升。消融研究表明进度预测和选择性保留都是性能提升的关键。

Conclusion: PABU框架通过显式建模任务进度和选择性信息保留，有效减少了LLM智能体的冗余计算，显著提升了任务完成效率和性能，为高效的智能体设计提供了新思路。
...
### [130] [CoMMa: Contribution-Aware Medical Multi-Agents From A Game-Theoretic Perspective](https://arxiv.org/abs/2602.09159)
...
<details>
  <summary>Details</summary>
Motivation: 现有医疗多智能体框架在处理动态、异质患者数据时，大多依赖随机叙事推理，缺乏明确的证据归因和数学基础，导致决策不稳定且难以解释。

Method: 提出贡献感知医疗多智能体(CoMMa)框架：1) 去中心化架构，专家智能体在分区证据上操作；2) 通过博弈论目标进行协调；3) 使用确定性嵌入投影近似贡献感知信用分配；4) 通过估计每个智能体的边际效用实现明确的证据归因。

Result: 在多种肿瘤学基准测试（包括真实世界多学科肿瘤委员会数据集）中，CoMMa相比数据集中化和基于角色的多智能体基线，实现了更高的准确性和更稳定的性能。

Conclusion: CoMMa通过贡献感知信用分配和博弈论协调机制，为肿瘤学决策支持提供了可解释、数学基础扎实且稳定的多智能体框架，优于现有方法。
...
### [131] [Measuring Dataset Diversity from a Geometric Perspective](https://arxiv.org/abs/2602.09340)
*Yang Ba,Mohammad Sadeq Abolhasani,Michelle V Mancenido,Rong Pan*

Main category: cs.AI

TL;DR: 提出基于拓扑数据分析的多样性度量框架PLDiv，通过持久性景观捕捉数据几何结构，超越传统熵和分布变异度量


<details>
  <summary>Details</summary>
Motivation: 现有多样性度量主要关注特征空间分布变异或熵，忽略了数据集的几何结构特性，需要一种能捕捉几何丰富性的多样性度量方法
...
### [132] [Auditing Multi-Agent LLM Reasoning Trees Outperforms Majority Vote and LLM-as-Judge](https://arxiv.org/abs/2602.09341)
...
<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统大多采用多数投票聚合智能体输出，这种方法丢弃了推理轨迹的证据结构，且在虚假共识（智能体共享相关偏见并收敛于相同错误推理）下表现脆弱
...
Conclusion: AgentAuditor通过显式建模推理分歧和局部验证机制，有效解决了多智能体系统中的虚假共识问题，显著提升了推理准确性，且与具体MAS设置无关
...
### [133] [Image Quality in the Era of Artificial Intelligence](https://arxiv.org/abs/2602.09347)
*Jana G. Delfino,Jason L. Granstedt,Frank W. Samuelson,Robert Ochs,Krishna Juluru*

Main category: cs.AI

TL;DR: AI在放射学图像重建和增强中应用迅速，能提升图像质量、加快获取速度，但也引入新的失败模式并加剧图像感知质量与信息内容之间的脱节，需要了解其局限性以确保安全有效使用。


<details>
  <summary>Details</summary>
Motivation: AI在放射学中快速部署，虽然能显著改善图像质量、加快获取和审阅速度，但也带来了新的失败风险，可能加剧图像感知质量与实际信息内容之间的脱节。为确保AI技术的安全有效应用，需要充分认识其局限性。

Method: 本文是一篇通讯文章，旨在提高人们对AI在放射学图像重建和增强中局限性的认识，通过分析AI技术应用带来的潜在风险和失败模式，帮助用户更好地理解技术边界。

Result: 文章强调了AI在放射学图像处理中的双重性：一方面能显著提升图像质量和工作效率，另一方面却可能引入新的错误类型并误导临床判断。这种认识有助于用户在使用AI技术时保持警惕。

Conclusion: 了解AI在放射学图像重建和增强中的局限性至关重要，这能帮助临床医生在享受技术带来的益处的同时，有效识别和规避潜在风险，实现安全、有效的AI技术应用。
...
### [134] [Bridging Efficiency and Transparency: Explainable CoT Compression in Multimodal Large Reasoning Models](https://arxiv.org/abs/2602.09485)
...
<details>
  <summary>Details</summary>
Motivation: 长思维链在多模态推理中广泛使用，但往往过于冗长且包含冗余推理步骤，影响推理效率。现有压缩方法存在两个主要问题：可能破坏视觉-文本推理的完整性，以及压缩过程缺乏可解释性。

Method: 提出XMCC，将压缩建模为顺序决策过程，通过强化学习优化。该方法能够缩短推理轨迹，同时保留关键推理步骤和答案正确性，并为压缩决策生成自然语言解释。
...
### [135] [Computing Conditional Shapley Values Using Tabular Foundation Models](https://arxiv.org/abs/2602.09489)
*Lars Henry Berge Olsen,Dennis Christensen*

Main category: cs.AI

TL;DR: 使用TabPFN等表格基础模型高效计算Shapley值，相比传统方法在大多数情况下性能最优，且运行时间大幅减少
...
### [136] [FLINGO -- Instilling ASP Expressiveness into Linear Integer Constraints](https://arxiv.org/abs/2602.09620)
*Jorge Fandinno,Pedro Cabalar,Philipp Wanko,Torsten Schaub*

Main category: cs.AI

TL;DR: FLINGO语言扩展了约束ASP，将ASP的丰富表达特性（如默认值、未定义属性、选择规则等）融入数值约束中，解决了传统CASP表达受限的问题。


<details>
  <summary>Details</summary>
Motivation: 传统约束答案集编程（CASP）在表达数值约束时，其语法更接近后端数值求解器的表达能力和语义，而不是标准ASP的表达方式。在标准ASP中，数值属性可以通过谓词表示，支持声明默认值、未定义属性、使用选择规则进行非确定性赋值或使用聚合值。但在CASP中，一旦切换到基于约束的表示，这些特性大多会丢失。

Method: 提出了FLINGO语言和工具，将ASP的丰富表达特性融入数值约束中。基于先前建立的语义基础，提出了从新引入的FLINGO语法到遵循CLINGCON输入格式的常规CASP程序的翻译方法。

Result: 开发了FLINGO语言和工具，能够将ASP的丰富表达特性（如默认值、未定义属性、选择规则等）融入数值约束中，并通过多个示例展示了其使用。

Conclusion: FLINGO语言成功地将ASP的丰富表达特性与数值约束处理相结合，解决了传统CASP在表达数值属性时功能受限的问题，为实际应用提供了更灵活和强大的建模工具。
...
### [137] [ClinAlign: Scaling Healthcare Alignment from Clinician Preference](https://arxiv.org/abs/2602.09653)
*Shiwei Lyu,Xidong Wang,Lei Liu,Hao Zhu,Chaohe Zhang,Jian Wang,Jinjie Gu,Benyou Wang,Yue Shen*

Main category: cs.AI

TL;DR: 提出两阶段框架解决LLM医疗输出与临床医生偏好对齐问题：1)创建医生验证的偏好数据集HealthRubrics；2)提炼为可重用临床原则HealthPrinciples，用于离线对齐和推理时自修正，小模型性能超越大模型。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型具备专家级医学知识，但其开放式输出与临床医生细粒度偏好的对齐仍然困难。现有方法依赖粗粒度目标或不可靠的自动评估，缺乏专业指南的坚实基础。

Method: 提出两阶段框架：1)创建HealthRubrics数据集，包含7,034个医生验证的偏好示例，临床医生根据严格医学标准精炼LLM起草的评估标准；2)提炼为HealthPrinciples，包含119个广泛可重用、基于临床的原则，按临床维度组织，实现超越手动标注的可扩展监督。使用HealthPrinciples进行离线对齐和推理时引导自修正。

Result: 仅激活30亿参数中30亿参数的模型在HealthBench-Hard上达到33.4%的准确率，超越了包括Deepseek-R1和o3在内的更大模型，为临床对齐建立了资源高效的基线。

Conclusion: 该框架通过医生验证的评估标准和可重用临床原则，有效解决了LLM医疗输出与临床医生偏好的对齐问题，实现了小模型超越大模型的性能，为临床对齐提供了资源高效的解决方案。
...
### [138] [GHS-TDA: A Synergistic Reasoning Framework Integrating Global Hypothesis Space with Topological Data Analysis](https://arxiv.org/abs/2602.09794)
...
TL;DR: 本文提出GHS-TDA方法，通过构建全局假设图和多尺度拓扑分析，解决传统思维链方法中早期错误传播和缺乏结构化分析的问题，显著提升推理准确性和鲁棒性。
...
Conclusion: GHS-TDA方法通过全局协调和拓扑分析有效解决了传统思维链方法的局限性，显著提升了大型语言模型在复杂任务上的推理性能，为可解释和鲁棒的推理提供了新思路。
...
### [139] [Would a Large Language Model Pay Extra for a View? Inferring Willingness to Pay from Subjective Choices](https://arxiv.org/abs/2602.09802)
*Manon Reusens,Sofie Goethals,Toon Calders,David Martens*

Main category: cs.AI

TL;DR: 研究LLM在旅行助手场景中的主观决策能力，通过选择困境实验推导隐含支付意愿，并与人类基准比较，发现LLM存在系统性偏差且倾向于高估人类支付意愿。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在旅行助手等应用中的部署增多，它们经常需要在没有客观正确答案的情况下为用户做出主观选择。研究旨在评估LLM在这些主观决策场景中的表现，特别是它们是否能够像人类一样进行价值判断。

Method: 在旅行助手情境中向LLM呈现选择困境，使用多项logit模型分析响应以推导隐含支付意愿估计。除了基线设置外，还研究了更现实条件下的模型行为，包括提供用户过去选择信息和基于角色的提示。

Result: 较大规模LLM可以推导出有意义的支付意愿值，但在属性层面存在系统性偏差。LLM倾向于整体高估人类支付意愿，特别是当引入昂贵选项或商务导向角色时。基于先前对便宜选项偏好的条件设置能产生更接近人类基准的估值。
...
### [140] [CODE-SHARP: Continuous Open-ended Discovery and Evolution of Skills as Hierarchical Reward Programs](https://arxiv.org/abs/2602.10085)
*Richard Bornemann,Pierluigi Vito Amadori,Antoine Cully*

Main category: cs.AI

TL;DR: CODE-SHARP框架利用基础模型自动发现和演化技能，通过代码形式的奖励函数构建技能层次结构，使智能体能够解决复杂的长时程任务。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习依赖人工设计的奖励函数，无法实现开放式的技能发现。现有方法仅限于为预定义任务优化奖励，无法自动发现新技能。

Method: 提出CODE-SHARP框架，利用基础模型开放地扩展和精炼技能档案，构建可执行奖励函数的代码形式的有向图层次结构。

Result: 在Craftax环境中，仅使用发现的SHARP技能奖励训练的智能体能够解决越来越长的时程目标。通过高层规划器组合技能，单个目标条件智能体在复杂长时程任务上平均表现优于预训练智能体和任务特定专家策略134%。

Conclusion: CODE-SHARP框架成功实现了开放式的技能发现和学习，通过自动生成的奖励函数层次结构使智能体能够解决复杂的长期任务，超越了传统方法。
...
### [141] [Agent World Model: Infinity Synthetic Environments for Agentic Reinforcement Learning](https://arxiv.org/abs/2602.10090)
*Zhaoyang Wang,Canwen Xu,Boyi Liu,Yite Wang,Siwei Han,Zhewei Yao,Huaxiu Yao,Yuxiong He*

Main category: cs.AI

TL;DR: 提出Agent World Model (AWM)合成环境生成管道，创建1000个日常场景环境，支持多轮工具交互，用于大规模强化学习训练工具使用智能体


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型智能体训练受限于缺乏多样可靠的环境，需要解决环境稀缺问题以扩展智能体训练规模

Method: 开发AWM合成环境生成管道，创建代码驱动、数据库支持的环境，每个环境平均提供35个工具，确保状态转换的可靠性和一致性

Result: 在三个基准测试中，仅在合成环境中训练而非特定基准环境，实现了强大的分布外泛化能力

Conclusion: AWM合成环境为多轮工具使用智能体的大规模强化学习提供了有效解决方案，支持可靠的奖励函数设计和高效的智能体交互
...
### [142] [Operator-Based Information Theory for Imaging: Entropy, Capacity, and Irreversibility in Physical Measurement Systems](https://arxiv.org/abs/2602.09026)
*Charles Wood*

Main category: eess.IV

TL;DR: 该论文提出了一种基于算子的成像信息理论框架，通过算子谱特性量化信息流动，定义了算子熵、算子信息容量和不可逆性指数三个度量指标。


<details>
  <summary>Details</summary>
Motivation: 传统成像系统描述（分辨率、对比度、信噪比）无法全面描述物理变换如何影响信息流动，需要建立更通用的成像信息理论框架。

Method: 将成像链建模为作用于函数的有限算子组合，利用算子谱特性表征信息重分布，开发算子熵、算子信息容量和不可逆性指数三个度量。

Result: 框架适用于线性、非线性和随机算子，不依赖特定成像模态；分析示例展示了衰减、模糊和采样如何以不同方式影响熵、容量和不可逆性。

Conclusion: 该框架为分析成像物理极限提供了通用结构，为后续信息几何、时空预算、非线性通道和重建算法研究奠定了基础。
...
### [143] [SAS-Net: Scene-Appearance Separation Network for Robust Spatiotemporal Registration in Bidirectional Photoacoustic Microscopy](https://arxiv.org/abs/2602.09050)
*Jiahao Qin*

Main category: eess.IV

TL;DR: 提出统一场景-外观分离框架，解决双向扫描OR-PAM中的时空错位问题，实现实时高精度图像配准


<details>
  <summary>Details</summary>
Motivation: 高速光学分辨率光声显微镜双向扫描导致严重的时空错位，传统配准方法依赖亮度恒定性假设，在双向扫描下失效，需要新的解决方案

Method: 提出场景-外观分离框架，将域不变场景内容与域特定外观特征分离，通过场景一致性损失促进潜在空间几何对应，将域偏移校正与空间配准统一在一个框架中

Result: 在小鼠脑血管成像中，NCC达到0.961，SSIM达到0.894，显著优于传统方法；每帧推理时间11.2ms（86fps），支持实时处理；消融研究显示域对齐损失至关重要

Conclusion: 该框架为高速双向OR-PAM提供了稳健解决方案，支持可靠的定量和纵向功能成像，代码将公开提供
