...
TL;DR: SuperHead是一个从低质量输入生成高质量、可动画化3D头部化身的新框架，通过动态感知的3D反演方案利用预训练3D生成模型的先验知识。


<details>
  <summary>Details</summary>
Motivation: 当前创建高质量可动画3D头部化身面临的主要挑战是低质量图像或视频源导致3D重建效果差，现有超分辨率技术无法处理动态3D输入。

Method: 通过动态感知3D反演方案优化生成模型的潜在表示，生成超分辨率的3D高斯溅射头部模型，然后绑定到参数化头部模型进行动画。使用从不同面部表情和视角捕获的上采样2D面部渲染和深度图进行联合监督。

Result: SuperHead在动态运动下生成具有精细面部细节的化身，在视觉质量上显著优于基线方法。

Conclusion: 该框架成功解决了从低质量源创建高质量可动画3D头部化身的挑战，通过创新的3D反演方法实现了高质量的几何和纹理合成，同时保持了3D和时间一致性。
...
TL;DR: 本文提出了EgoAVU，一个用于自动生成第一人称视角音频-视频叙述、问答的数据引擎，以解决多模态大语言模型在理解第一人称视频时对音频信号忽视的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然能接受视觉和音频输入，但由于缺乏包含连贯联合模态信息的文本标签，这些模型在第一人称视频中是否能真正联合理解两种模态仍未被充分探索。现有模型存在严重偏向视觉信号、忽视音频线索的问题。

Method: 提出EgoAVU数据引擎，通过跨模态相关性建模自动生成第一人称音频-视频叙述；采用基于令牌的视频过滤和模块化、基于图的策划确保数据多样性和质量；构建了包含300万样本的训练数据集EgoAVU-Instruct和手动验证的评估集EgoAVU-Bench。

Result: 在EgoAVU-Bench上，现有MLLMs表现出严重偏向视觉信号的问题；在EgoAVU-Instruct上微调MLLMs后，在EgoAVU-Bench上实现了高达113%的性能提升；这种优势也迁移到其他基准测试如EgoTempo和EgoIllusion，获得高达28%的相对性能增益。

Conclusion: EgoAVU数据引擎有效解决了MLLMs在第一人称视频理解中忽视音频信号的问题，通过大规模高质量数据生成和微调，显著提升了模型对音频-视频联合模态的理解能力，且这种提升具有良好的迁移性。
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂真实场景中单视图3D重建面临的挑战：噪声干扰、物体多样性、数据集有限，需要提升重建的几何完整性、平滑度和细节保留能力。

Method: 提出MGP-KAD多模态特征融合框架：1) 通过采样和聚类真实物体数据生成几何先验，产生类别级特征并在训练中动态调整；2) 基于Kolmogorov-Arnold Networks (KAN)的混合解码器，克服传统线性解码器处理复杂多模态输入的局限性。
...
TL;DR: DwD提出使用DINOv3视觉基础模型特征作为仿真到真实驾驶视频生成的统一桥梁，解决现有方法在一致性与真实性之间的两难困境。


<details>
  <summary>Details</summary>
Motivation: 现有Sim2Real方法面临"一致性-真实性困境"：低层信号（如边缘、模糊图像）能确保精确控制但会引入合成伪影，而高层先验（如深度、语义）能实现照片级真实感但缺乏结构细节。需要一种能同时保证控制一致性和真实性的中间表示。

Method: 1) 使用DINOv3视觉基础模型特征作为仿真与真实域的统一桥梁；2) 主成分子空间投影去除导致"纹理烘焙"的高频元素；3) 随机通道尾部丢弃缓解刚性降维的结构损失；4) 可学习空间对齐模块适配高分辨率特征；5) 因果时间聚合器使用因果卷积保留历史运动上下文。

Result: DwD框架在自动驾驶视频生成中成功调和了真实性与控制一致性，有效缓解了运动模糊并保证了时间稳定性，相比现有方法在控制精度和生成质量上都有显著提升。

Conclusion: DINOv3特征作为Sim2Real的统一桥梁能够有效解决一致性-真实性困境，通过精心设计的特征处理、空间对齐和时间聚合机制，实现了高质量、可控的自动驾驶视频生成。
...
### [5] [Rethinking Multi-Condition DiTs: Eliminating Redundant Attention via Position-Alignment and Keyword-Scoping](https://arxiv.org/abs/2602.06850)
*Chao Zhou,Tianyi Wei,Yiling Chen,Wenbo Zhou,Nenghai Yu*

Main category: cs.CV

TL;DR: 本文提出PKA框架，通过位置对齐注意力和关键词范围注意力消除多条件控制中的冗余，结合条件敏感度感知采样策略，实现10倍推理加速和5.1倍显存节省。


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型在基于提示的生成方面表现出色，但缺乏对空间布局或主体外观等特定用户需求的细粒度控制。多条件控制可以解决这个问题，但在扩散变换器中的集成受到传统"连接并注意"策略的限制，该策略在条件数量增加时会产生二次计算和内存开销。

Method: 提出位置对齐和关键词范围注意力框架：1) 位置对齐注意力通过强制局部补丁对齐来线性化空间控制；2) 关键词范围注意力通过语义感知掩码修剪不相关的主题驱动交互；3) 条件敏感度感知采样策略重新加权训练目标，专注于关键去噪阶段，加速收敛并增强条件保真度。

Result: PKA框架实现了10.0倍的推理加速和5.1倍的VRAM节省，为高保真多条件生成提供了可扩展且资源友好的解决方案。

Conclusion: PKA框架通过消除多条件控制中的空间和语义冗余，显著提高了扩散变换器的效率和可扩展性，为细粒度可控图像生成提供了实用解决方案。
...
TL;DR: MetaSSP是一种用于单视图3D重建的半监督框架，通过利用大量未标注图像，在隐式SDF方法基础上实现了高质量表面重建，显著减少了标注数据需求。


<details>
  <summary>Details</summary>
Motivation: 基于隐式SDF的单视图3D重建方法虽然能生成高质量表面，但需要大量标注数据集，这限制了方法的可扩展性。研究者希望利用丰富的未标注图像来减少对标注数据的依赖。

Method: 提出MetaSSP半监督框架，包含基于梯度的参数重要性估计来正则化自适应EMA更新，以及结合增强一致性和SDF方差的SDF感知伪标签加权机制。从10%监督预热开始，统一管道联合优化标注和未标注数据。

Result: 在Pix3D基准测试中，该方法相比现有半监督基线将倒角距离减少了约20.61%，并将IoU提高了约24.09%，达到了新的最先进水平。

Conclusion: MetaSSP通过有效利用未标注图像，显著提升了单视图3D重建的性能，减少了标注数据需求，为隐式SDF方法的可扩展性提供了有前景的解决方案。
...
TL;DR: M3是一个无需训练的多模态、多智能体、多轮次框架，通过推理时迭代优化解决文本到图像生成中的复杂组合提示问题，使开源模型性能超越商业旗舰系统。


<details>
  <summary>Details</summary>
Motivation: 生成模型在文本到图像合成中取得了令人印象深刻的保真度，但在涉及多个约束的复杂组合提示方面仍然存在困难。需要一种系统性的方法来解决这些失败，而不需要昂贵的重新训练。

Method: M3采用训练免费的多智能体循环框架：规划器将提示分解为可验证的检查清单，专门的检查器、精炼器和编辑器智能体逐个修正约束，验证器确保单调改进。这是一个即插即用的模块，与任何预训练的T2I模型兼容。

Result: 在具有挑战性的OneIG-EN基准测试中，Qwen-Image+M3超越了商业旗舰系统如Imagen4(0.515)和Seedream 3.0(0.530)，达到最先进的性能(0.532)。在GenEval组合指标上也有显著改进，在强化测试集上的空间推理性能几乎翻倍。

Conclusion: 智能多智能体推理可以将开源模型提升到超越专有替代品的水平。M3为无需昂贵重新训练的组合生成建立了新范式，展示了推理时优化在解决复杂组合约束方面的强大潜力。
...
Method: 使用残差变分自编码器，仅用健康矢状位T2加权扫描训练，通过扩散生成合成数据增强。推理时通过重建误差热图检测偏离正常解剖结构的病理区域。
...
TL;DR: PhenoLIP：首个整合表型知识图谱的医学视觉语言模型预训练框架，通过结构化表型知识提升医学图像理解性能


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言模型主要依赖粗粒度的图像-文本对比学习，未能充分利用医学表型本体中编码的系统性视觉知识，限制了医学图像的结构化理解和解释能力

Method: 1. 构建PhenoKG：首个大规模表型中心多模态知识图谱，包含52万高质量图像-文本对，关联3000多个表型；2. 提出PhenoLIP框架：两阶段预训练，首先从文本本体数据学习知识增强的表型嵌入空间，然后通过教师引导的知识蒸馏将结构化知识融入多模态预训练；3. 创建PhenoBench：专家验证的表型识别基准，包含7800多个图像-描述对，覆盖1000多个表型

Result: PhenoLIP显著优于现有最佳基线：在表型分类准确率上比BiomedCLIP提升8.85%，在跨模态检索上比BIOMEDICA提升15.03%，验证了整合表型中心先验知识对医学视觉语言模型的价值

Conclusion: 通过构建表型知识图谱并设计相应的预训练框架，能够有效将结构化表型知识融入医学视觉语言模型，显著提升医学图像的结构化理解和解释能力，为医学图像分析提供了新的研究方向
...
TL;DR: DeDPO是一种改进的扩散模型对齐方法，通过整合因果推断的去偏估计技术，利用低成本合成AI反馈增强有限的人类偏好数据，解决了DPO对大规模高质量人类标签的依赖问题。


<details>
  <summary>Details</summary>
Motivation: DPO方法虽然有效，但严重依赖大规模高质量人类偏好标签，这带来了高昂的成本和可扩展性瓶颈。需要一种能够利用低成本合成反馈的方法来克服这一限制。

Method: 提出DeDPO框架，将因果推断中的去偏估计技术整合到DPO目标中，通过识别和校正合成标注器中固有的系统偏差和噪声，实现从包括自训练和视觉语言模型在内的不完美反馈源中稳健学习。

Result: 实验表明DeDPO对合成标注方法的变化具有鲁棒性，其性能能够匹配甚至偶尔超过完全使用人类标注数据训练的理论上限。

Conclusion: DeDPO通过利用廉价的合成监督，为人类-AI对齐提供了一个可扩展的解决方案，有效解决了DPO方法的成本和可扩展性瓶颈。
...
TL;DR: DroneKey++是一个无需先验信息的无人机3D姿态估计框架，通过联合关键点检测、分类和姿态估计，在合成数据集6DroneSyn上验证了其泛化能力和实时性。
...
Method: 提出DroneKey++框架：使用关键点编码器同时进行关键点检测和分类，姿态解码器通过基于射线的几何推理和类别嵌入估计3D姿态。构建大规模合成数据集6DroneSyn，包含7种无人机模型和88种户外背景。

Result: 旋转误差MAE 17.34度、MedAE 17.1度；平移误差MAE 0.135米、MedAE 0.242米；推理速度CPU 19.25 FPS、GPU 414.07 FPS，展示了跨模型的强泛化能力和实时应用潜力。

Conclusion: DroneKey++无需先验信息即可实现准确的无人机3D姿态估计，通过大规模合成数据集验证了其泛化能力，适合实时安防监控应用。
...
TL;DR: 提出一种可微分车辆模型框架，将动作序列转换为轨迹点，弥合了轨迹点模型与动作模型之间的差距，使动作模型能在轨迹点基准上训练和评估。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶系统分为轨迹点模型和动作模型两类，但当前基准协议和训练流程主要针对轨迹点模型，导致动作模型难以训练和比较，阻碍了其发展。
...
Conclusion: 该框架首次使动作模型能够在轨迹点基准上进行训练和评估，无需修改底层评估协议，弥合了轨迹点与动作之间的差距。
...
TL;DR: 该研究提出了一种对齐稀疏自编码器(SAE)来分析视觉-语言模型共享嵌入空间的几何结构，发现稀疏双模态原子承载跨模态对齐信号，单模态原子解释模态差异，移除单模态原子可消除模态差距而不损害性能。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉-语言模型在图像和文本对齐方面取得了显著成功，但其共享嵌入空间的几何结构仍然不够清晰。研究者希望探索这种几何结构，以更好地理解跨模态表示的本质。

Method: 基于等能量假设（Iso-Energy Assumption）——真正共享的概念应在不同模态中表现出相同的平均能量。研究者开发了对齐稀疏自编码器(SAE)，在训练中鼓励能量一致性同时保持重构能力，从而获得可用于几何分析的表征。

Result: 在基础视觉-语言模型上的应用揭示了清晰的结构：1）稀疏双模态原子承载全部跨模态对齐信号；2）单模态原子作为模态特定偏置，完全解释模态差距；3）移除单模态原子可消除模态差距而不损害性能；4）将向量算术限制在双模态子空间可产生分布内编辑和改进的检索效果。

Conclusion: 正确的归纳偏置既能保持模型保真度，又能使潜在几何结构变得可解释和可操作。这项研究为理解视觉-语言模型的跨模态表示提供了新的分析工具和见解。
...
TL;DR: ForeHOI是一个前馈模型，能够从单目手物交互视频中直接重建3D物体几何，无需预处理，在一分钟内完成推理，通过联合预测2D掩码修复和3D形状补全来处理严重遮挡问题。
...
Method: 提出前馈模型ForeHOI，通过联合预测2D掩码修复和3D形状补全来处理遮挡问题，2D和3D形状补全之间的信息交换提升重建质量，并创建了首个大规模高保真合成手物交互数据集用于训练。

Result: ForeHOI在物体重建方面达到最先进性能，显著优于先前方法，速度提升约100倍，能够有效处理严重的手物遮挡情况。

Conclusion: ForeHOI通过前馈框架中的2D-3D联合预测有效解决了单目手物交互视频中的严重遮挡问题，实现了高效准确的3D物体重建，为相关领域提供了有价值的工具和数据集。
...
TL;DR: 本研究开发了一个基于视觉Transformer的深度学习模型，利用指纹图像区分Kabuki综合征、Wiedemann-Steiner综合征和正常对照，为罕见遗传综合征提供非侵入性诊断辅助工具。


<details>
  <summary>Details</summary>
Motivation: Kabuki综合征和Wiedemann-Steiner综合征是两种临床表现重叠的罕见发育障碍，但由于遗传检测和专家资源的限制，许多患者未能得到及时诊断。尽管皮纹异常是多种遗传综合征的已知特征，但在分子检测时代这一信号仍未得到充分利用。

Method: 研究采用基于视觉Transformer的深度学习模型，利用指纹图像进行三种二元分类任务：正常对照vs.KS、正常对照vs.WSS、KS vs.WSS。同时应用基于注意力的可视化技术识别对模型预测最重要的指纹区域。

Result: 模型在三个分类任务中分别获得AUC分数0.80（对照vs.KS）、0.73（对照vs.WSS）和0.85（KS vs.WSS），相应的F1分数分别为0.71、0.72和0.83。注意力可视化增强了模型的可解释性。
...
TL;DR: 研究者开发了MMEarth-Bench多模态环境基准数据集，包含5个任务、12种模态和全球分布数据，用于评估多模态预训练模型的地理泛化能力，并提出了测试时多模态重建训练方法。
...
Method: 1) 创建MMEarth-Bench数据集：包含5个多模态环境任务、12种数据模态、全球分布数据，并设置分布内和分布外测试划分；2) 提出测试时多模态重建训练方法：利用测试时可用的所有模态作为辅助任务，通过地理批处理平衡正则化和专业化。

Result: 多模态预训练在有限数据设置下能提升模型鲁棒性，但地理泛化能力仍然较差；提出的TTT-MMR方法在随机和地理测试划分上均能提升模型性能；地理批处理在TTT中实现了正则化与专业化的良好平衡。

Conclusion: MMEarth-Bench填补了多模态地理空间基准的空白，揭示了当前模型地理泛化的局限性，提出的测试时训练方法为适应新任务和地理域提供了有效途径，推动了地理空间机器学习的发展。
...
Conclusion: MCPO方法有效解决了多模态医学图像配准的挑战，特别是在术前-术中配准场景中表现出色，具有实际临床应用价值。
...
TL;DR: 提出一种将Vision Transformer线性层替换为卷积操作的方法，使其能在CNN优化的BPU硬件上高效部署，实现3.8倍加速且精度损失很小


<details>
  <summary>Details</summary>
Motivation: Vision Transformer模型在计算机视觉任务中表现出色，但CNN优化的BPU硬件无法有效加速Transformer的线性层计算，存在架构不匹配问题

Method: 将Vision Transformer中的线性层和层归一化操作替换为精心设计的卷积算子，使模型能在BPU上运行，且无需重新训练即可继承原始权重

Result: 量化DeiT-Base在ImageNet上达到80.4%准确率（原81.8%），获得3.8倍推理加速；在花卉分类数据集上仅损失0.5%准确率

Conclusion: 该方法成功解决了Vision Transformer在CNN优化硬件上的部署难题，首次实现完全利用BPU加速的Transformer部署，为硬件兼容性提供了有效解决方案
...
TL;DR: 提出一种基于权重重新初始化的方法ABR，用于提升持续测试时域适应的长期性能


<details>
  <summary>Details</summary>
Motivation: 现有CTTA方法主要关注优化适应过程，但一个重要问题未解决：模型能否在长期持续变化的环境中保持良好性能？本文探索通过重新初始化方法来促进更好的长期CTTA性能

Method: 提出自适应平衡重新初始化策略（ABR），基于标签翻转变化模式确定自适应间隔进行权重重新初始化，以保持模型的长期性能

Result: 在广泛的CTTA基准测试中验证了该方法，取得了优越的性能表现

Conclusion: 通过基于标签翻转轨迹模式的自适应重新初始化策略，能够有效提升持续测试时域适应的长期性能
...
<details>
  <summary>Details</summary>
Motivation: SAM3通过可提示概念分割实现了开放词汇分割，但依赖预定义概念使其在目标域出现数据漂移（视觉分布变化）或概念漂移（条件标签分布变化）时，视觉证据与提示之间的对齐会失效。
...
Conclusion: ConceptBank提供了一个无需参数的校准框架，能够动态恢复视觉证据与概念提示之间的对齐，显著提升了开放词汇分割模型在分布漂移情况下的适应能力。
...
<details>
  <summary>Details</summary>
Motivation: SAM在实例分割中表现出色但严重依赖人工提示质量，且RGB图像缺乏深度信息，限制了模型感知空间结构和物体边界的能力。
...
Result: 在12个不同数据集上的实验结果表明，SPDA-SAM优于最先进的对比方法，自提示指导和粗到细RGB-D融合操作补偿了空间信息损失。
...
TL;DR: FlowConsist训练框架解决了快速流模型中的轨迹漂移和误差累积问题，通过使用模型自身预测的边际速度替代条件速度，并引入轨迹校正策略，在ImageNet 256×256上仅用1步采样达到1.52 FID的新SOTA。


<details>
  <summary>Details</summary>
Motivation: 当前快速流模型的训练范式存在两个根本问题：1）基于随机配对噪声-数据样本构建的条件速度引入了系统性的轨迹漂移，阻碍模型遵循一致的ODE路径；2）模型近似误差随时间步累积，导致长时间间隔下的严重偏差。

Method: 提出FlowConsist训练框架：1）用模型自身预测的边际速度替代条件速度，使优化与真实轨迹对齐；2）引入轨迹校正策略，在轨迹的每个时间步上对齐生成样本和真实样本的边际分布。

Result: 在ImageNet 256×256上建立了新的最先进水平，仅用1个采样步骤就达到了1.52的FID分数。
...
TL;DR: 该论文提出了观察者中心显著目标检测（OC-SOD），通过考虑观察者特定因素（如偏好或意图）来预测显著区域，解决了传统显著目标检测作为单一客观预测任务的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统显著目标检测方法将问题视为客观预测任务，每张图像只有一个真实分割图，这导致问题定义不足且本质上不适定。显著目标检测本质上是主观问题，不同观察者可能感知不同的显著对象。

Method: 1. 提出观察者中心显著目标检测（OC-SOD）框架，同时考虑视觉线索和观察者特定因素；2. 利用多模态大语言模型构建高效数据标注流程；3. 创建首个OC-SOD数据集OC-SODBench（33k图像，152k文本提示和对象对）；4. 设计OC-SODAgent基线系统，采用类似人类的"感知-反思-调整"过程。

Result: 在提出的OC-SODBench数据集上进行广泛实验，验证了所提方法的有效性。通过观察者中心视角，弥合了人类感知与计算建模之间的差距。

Conclusion: OC-SOD框架能够捕捉人类感知的内在模糊性和多样性，实现个性化和上下文感知的显著性预测，为"什么使对象真正显著"提供了更现实和灵活的理解。
...
TL;DR: TFusionOcc是一个面向自动驾驶的3D语义占据预测框架，采用对象中心的多传感器融合方法，结合学生t分布和T混合模型，使用可变形超二次曲面作为几何基元，在nuScenes基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D语义占据预测方法主要依赖3D体素或3D高斯表示，难以高效捕捉驾驶环境中细粒度的几何细节，限制了自动驾驶系统的感知能力。

Method: 提出TFusionOcc框架：1）采用对象中心的多传感器融合策略；2）使用学生t分布和T混合模型进行概率建模；3）引入可变形超二次曲面（带逆形变的超二次曲面）作为更灵活的几何基元；4）实现多阶段多传感器融合。

Result: 在nuScenes基准测试中达到最先进的性能，并在nuScenes-C数据集上通过大量实验证明了该方法在不同相机和激光雷达损坏场景下的鲁棒性。

Conclusion: TFusionOcc通过创新的对象中心多传感器融合框架和灵活的几何表示，显著提升了3D语义占据预测的精度和鲁棒性，为自动驾驶系统提供了更精细的环境感知能力。
...
Method: 提出MeDocVL框架，结合训练驱动的标签精炼从噪声标注构建高质量监督，采用噪声感知混合后训练策略，集成强化学习和监督微调以实现鲁棒精确提取

Result: 在医疗发票基准测试中，MeDocVL持续优于传统OCR系统和强大的VLM基线，在噪声监督下实现了最先进的性能

Conclusion: MeDocVL为医疗文档解析提供了一种有效的解决方案，能够处理复杂布局和噪声标注，在医疗发票等实际应用中表现出色
...
TL;DR: PointViT：基于Transformer的3D目标检测框架，通过选择性融合LiDAR点云和深度补全虚拟点来解决远距离物体检测问题，在KITTI基准测试中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: LiDAR点云在远距离处稀疏，导致几何线索不足，影响远场物体检测。现有方法通过RGB图像生成的深度补全虚拟点增强数据，但直接融合所有虚拟点会增加计算成本并带来融合挑战。

Method: 提出PointViT框架，联合处理原始LiDAR点和选择性采样的虚拟点。探索多种融合策略（从早期点级融合到BEV门控融合），分析精度与效率权衡。融合点云体素化后通过稀疏卷积编码为BEV表示，初始化高置信度物体查询，通过Transformer上下文聚合模块进行精炼。

Result: 在KITTI基准测试中，Car类别的3D AP达到91.16%，BEV AP达到95.94%，2D检测AP达到99.36%。

Conclusion: PointViT通过选择性融合LiDAR点和虚拟点，有效解决了远距离物体检测问题，在保持计算效率的同时显著提升了检测精度。
...
TL;DR: 提出SemGeo-AttentionNet双流架构，通过非对称跨模态融合建模人类3D视觉注意力的几何-语义二分性，并扩展到时间性扫描路径生成


<details>
  <summary>Details</summary>
Motivation: 现有3D显著性方法依赖手工几何特征或缺乏语义感知的学习方法，无法解释人类为何注视语义重要但几何不显著的区域。人类3D视觉注意力源于几何处理和语义识别的相互作用。

Method: 提出SemGeo-AttentionNet双流架构：1）几何流使用点云变换器处理几何特征；2）语义流通过几何条件多视角渲染获取基于扩散的语义先验。采用非对称跨模态融合，让几何特征查询语义内容，使自下而上的显著性指导自上而下的检索。通过强化学习扩展到时间扫描路径生成，首次引入尊重3D网格拓扑和抑制返回动态的公式。

Result: 在SAL3D、NUS3D和3DVA数据集上评估显示显著改进，验证了认知驱动架构能有效建模三维表面上的人类视觉注意力。

Conclusion: 通过明确建模几何-语义二分性的双流架构，能更有效地解释和预测人类在三维物体上的视觉注意力模式，为3D显著性检测提供了新的认知驱动框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法存在两个主要问题：1）基于结果的奖励传播到所有去噪步骤，无法区分每个步骤的局部效应；2）组级排名主要比较匹配时间步的轨迹，忽略了轨迹内依赖关系，早期去噪动作可能通过延迟的隐式交互影响后续状态

Method: TP-GRPO提出两个关键创新：1）用步级增量奖励替代基于结果的奖励，提供密集的步感知学习信号，更好隔离每个去噪动作的"纯"效应；2）识别转折点（改变局部奖励趋势并使后续奖励演化与整体轨迹趋势一致的步骤），为这些动作分配聚合的长期奖励以捕捉其延迟影响。转折点仅通过增量奖励的符号变化检测，使TP-GRPO高效且无需超参数

Result: 广泛实验表明TP-GRPO能更有效地利用奖励信号，并持续改进生成质量
...
TL;DR: POPL-KF是一种基于卡尔曼滤波的视觉惯性里程计系统，采用纯位姿几何表示处理点和线特征，减少线性化误差，在挑战性场景中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统VIO系统依赖点特征，在挑战性场景中性能下降；MSCKF-based VIO系统存在特征3D坐标线性化误差和测量更新延迟问题，需要改进。

Method: 1) 提出线特征的纯位姿几何表示；2) 开发POPL-KF系统，对点和线特征都采用纯位姿几何表示，从测量方程中消除特征坐标；3) 设计统一的基帧选择算法；4) 提出基于图像网格分割和双向光流一致性的线特征滤波器。

Result: 在公开数据集和真实世界实验中，POPL-KF优于最先进的基于滤波的方法（OpenVINS, PO-KF）和基于优化的方法（PL-VINS, EPLF-VINS），同时保持实时性能。

Conclusion: POPL-KF通过纯位姿几何表示有效减少了线性化误差，提高了VIO在挑战性场景中的性能，为基于滤波的VIO系统提供了新的解决方案。
...
Method: 提出视觉中心的具身导航框架，利用基于图像的提示进行决策制定；创建首个开源数据集，采用轨迹条件视频合成的数据生成流程。

Result: 在关键指标（成功率和路径效率）上持续优于最先进的基线方法。

Conclusion: 成功解决了室外到室内无缝导航的挑战，为实际部署场景提供了有效的解决方案。
...
TL;DR: ChatUMM是一个对话式统一多模态模型，通过创新的多轮训练策略和对话数据合成方法，解决了现有统一多模态模型仅支持单轮交互的局限性，实现了流畅的多轮多模态对话能力。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型（UMMs）虽然取得了显著进展，但受限于单轮交互范式，只能作为独立请求的求解器，无法作为连续对话中的助手。为了弥补这一差距，需要开发能够支持多轮对话的统一多模态模型。

Method: ChatUMM采用两个关键创新：1）交错多轮训练策略，将序列化的文本-图像流建模为连续对话流；2）系统化的对话数据合成流程，通过三个阶段将标准单轮数据集转化为流畅对话：构建基础状态对话、通过"干扰"轮次和依赖历史的查询重写强制长程依赖解析、合成自然交错的多模态响应。

Result: ChatUMM在开源统一模型中实现了视觉理解和指令引导编辑基准上的最先进性能，同时在文本到图像生成方面保持有竞争力的保真度。特别是在复杂多轮场景中表现出卓越的鲁棒性，确保流畅、上下文感知的对话。

Conclusion: ChatUMM成功地将统一多模态模型从单轮交互范式扩展到连续对话范式，通过创新的训练策略和数据合成方法，实现了强大的上下文跟踪能力和交错多模态生成能力，为构建真正的多模态对话助手奠定了基础。
...
TL;DR: 该论文提出了UnionST数据引擎和UnionST-S合成数据集，通过增强合成数据的多样性来缩小与真实数据的领域差距，并开发了自进化学习框架来有效标注真实数据。


<details>
  <summary>Details</summary>
Motivation: 大规模且类别平衡的文本数据对训练有效的场景文本识别模型至关重要，但收集真实数据困难。现有合成数据存在领域差距，在复杂场景下表现不佳，主要原因是语料、字体和布局多样性不足。

Method: 1. 系统分析主流基于渲染的合成数据集，识别其局限性；2. 提出UnionST数据引擎，合成覆盖挑战性样本并更好地对齐野外复杂性的文本；3. 构建UnionST-S大规模合成数据集，改进挑战性场景的模拟；4. 开发自进化学习框架进行有效的真实数据标注。

Result: 在UnionST-S上训练的模型相比现有合成数据集有显著提升，在某些场景下甚至超过真实数据性能。使用自进化学习时，仅需9%的真实数据标签就能达到有竞争力的性能。

Conclusion: 通过增强合成数据的多样性和复杂性，可以有效缩小合成与真实数据之间的领域差距。结合自进化学习框架，可以在减少真实数据标注需求的同时获得优异的场景文本识别性能。
...
TL;DR: LAB-Det：一种无需训练的单样本领域泛化目标检测方法，通过语言作为领域不变桥梁，仅需每类一个标注样本即可让冻结的检测器适应专业领域


<details>
  <summary>Details</summary>
Motivation: 基础目标检测器（如GLIP和Grounding DINO）在通用领域表现良好，但在水下图像或工业缺陷等专业领域和数据稀缺场景中性能下降。传统的跨领域少样本方法需要微调稀缺的目标数据，既成本高昂又存在过拟合风险。

Method: 提出LAB-Det方法，利用语言作为领域不变桥梁。不调整视觉特征，而是将每个示例样本投影到描述性文本中，用这些文本条件来引导冻结的检测器。这种语言条件化替代了基于梯度的适应方法。

Result: 在UODD（水下）和NEU-DET（工业缺陷）两个数据稀缺检测基准上评估，LAB-Det在不更新任何参数的情况下，相比最先进的微调基线实现了高达5.4 mAP的提升。

Conclusion: 语言适应是专业检测场景中微调的高效且可解释的替代方案，能够在数据稀缺领域实现鲁棒的泛化性能。
...
TL;DR: DreamHome-Pano是一个可控的全景室内生成框架，通过Prompt-LLM和冲突无控制架构解决布局约束与风格偏好的条件冲突问题，实现高质量室内设计合成。
...
Method: 1. 引入Prompt-LLM作为语义桥梁，将布局约束和风格参考转化为专业描述提示；2. 开发冲突无控制架构，包含结构感知几何先验和多条件解耦策略；3. 建立全景室内基准和多阶段训练管道，包括渐进式监督微调和强化学习。

Result: 实验结果表明，DreamHome-Pano在美学质量和结构一致性之间实现了优越的平衡，为全景室内可视化提供了稳健且专业级的解决方案。

Conclusion: DreamHome-Pano通过创新的语义对齐和条件解耦机制，有效解决了室内设计生成中的条件冲突问题，为可控全景室内合成提供了可靠框架。
...
TL;DR: DriveWorld-VLA是一个统一世界建模与规划的端到端自动驾驶框架，通过在潜在空间中紧密集成视觉语言动作模型与世界模型，实现场景演化与动作规划的统一，减少对密集标注监督的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶方法未能有效统一未来场景演化与动作规划，限制了视觉想象对动作决策的影响，需要一种能在单一架构中统一这两者的解决方案。

Method: 提出DriveWorld-VLA框架，在潜在空间中统一世界建模与规划，将世界模型的潜在状态作为VLA规划器的核心决策状态，支持特征级的可控、动作条件化想象，避免昂贵的像素级推演。

Result: 在NAVSIMv1上达到91.3 PDMS，NAVSIMv2上达到86.8 EPDMS，nuScenes上达到0.16的3秒平均碰撞率，实现了最先进的性能表现。

Conclusion: DriveWorld-VLA通过潜在空间中的世界建模与规划统一，显著提升了自动驾驶决策能力，使规划器能够直接受益于整体场景演化建模，并评估候选动作对未来场景的影响。
...
TL;DR: 提出MicroBi-ConvLSTM，一种超轻量级卷积-循环架构，仅需11.4K参数，比现有最佳轻量模型减少2.9-11.9倍参数，在八个HAR基准测试中保持竞争力，量化后部署仅需23.0KB内存。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的轻量级HAR模型（如TinierHAR和TinyHAR）虽然参数较少，但在考虑操作系统开销后仍超出微控制器的SRAM限制，无法在资源受限的可穿戴设备上部署。

Method: 采用两阶段卷积特征提取，包含4倍时间池化层和单层双向LSTM，实现超轻量级设计（平均11.4K参数），保持线性O(N)复杂度，支持INT8后训练量化。

Result: 在八个HAR基准测试中表现优异：UCI-HAR上93.41%宏F1，SKODA装配手势94.46%，Daphnet步态冻结检测88.98%。量化后仅损失0.21%平均F1分数，部署占用23.0KB内存。

Conclusion: MicroBi-ConvLSTM在保持竞争力的性能前提下，实现了显著的参数减少，适合内存受限的边缘设备部署，为资源受限可穿戴设备上的HAR应用提供了实用解决方案。
...
TL;DR: 本文提出AdaptOVCD，一种基于双维多级信息融合的无训练开放词汇变化检测架构，能够在零样本情况下检测任意类别变化，显著优于现有无训练方法。
...
Method: 提出AdaptOVCD架构，采用双维多级信息融合：垂直方向融合数据、特征和决策三个层次的信息，水平方向结合针对性自适应设计。具体包括：数据层的自适应辐射对齐（ARA）、特征层的自适应变化阈值（ACT）、决策层的自适应置信度过滤（ACF），并协同SAM-HQ、DINOv3和DGTRS-CLIP等预训练模型。

Result: 在九个场景的综合评估中，AdaptOVCD以零样本方式检测任意类别变化，显著优于现有无训练方法。在跨数据集评估中达到全监督性能上限的84.89%，展现出优越的泛化能力。

Conclusion: AdaptOVCD通过双维多级信息融合架构，有效缓解了异构预训练模型间的误差传播问题，实现了无需训练的开放词汇变化检测，在泛化性和适用性方面具有显著优势。
...
Method: 提出NECromancer框架，包含三个核心组件：1) 本体感知骨架图编码器(OwO)，从BVH文件中提取结构先验；2) 拓扑无关标记器(TAT)，将运动序列压缩为通用的离散表示；3) 统一BVH数据集(UvU)，聚合异构骨架的运动数据。

Result: NECromancer在高压缩率下实现高保真重建，有效解耦运动与骨架结构，支持跨物种运动迁移、组合、去噪、生成以及文本-运动检索等多种任务。

Conclusion: NECromancer建立了一个统一的运动分析与合成框架，能够处理多样化的形态学结构，为跨物种运动建模提供了通用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA基准测试存在评估协议不足的问题，无法充分捕捉真实世界的分布变化，导致对模型能力的评估有限甚至误导。

Method: 1) 设计分层评估协议，包含渐进难度级别，针对空间泛化、物体识别和任务指令理解三个核心能力；2) 通过人类遥操作收集高多样性训练数据集，每个场景支持多个细粒度操作目标，缩小训练-评估分布差距。

Result: 代表性VLA模型在累积扰动下表现出显著性能下降，暴露了在场景理解和指令基础化方面的持续局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在测试时扩展方面存在脆弱性，感知与推理在非结构化思维链中纠缠，导致长而混乱的上下文，小的感知错误可能级联成完全错误的答案，且需要昂贵的手工奖励强化学习。

Method: SPARC采用模块化两阶段流水线：第一阶段进行显式视觉搜索以定位问题相关区域，第二阶段基于这些区域进行推理生成最终答案。这种分离支持独立测试时扩展、不对称计算分配、选择性优化和压缩上下文处理。

Result: 在具有挑战性的视觉推理基准测试中，SPARC优于单体基线和强视觉基础方法。例如，SPARC将Qwen3VL-4B在V* VQA基准上的准确率提高了6.7个百分点，在具有挑战性的OOD任务上比"thinking with images"高出4.6个百分点，同时所需token预算降低200倍。

Conclusion: SPARC通过明确分离视觉感知与推理，解决了当前视觉语言模型测试时扩展的脆弱性问题，实现了更高效、更稳健的视觉推理，支持独立优化和计算资源分配，为模块化视觉语言系统设计提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 部分-部分3D形状匹配是计算机视觉中长期存在的挑战，但现有研究较少。该设置反映了最现实的情况（如3D扫描），需要同时计算准确对应关系和未知重叠区域。
...
Conclusion: 提出的整数线性规划方法有效解决了部分-部分3D形状匹配的独特挑战，为这一现实但研究不足的问题提供了实用解决方案。
...
TL;DR: 提出DAVE方法，通过ViT梯度分解生成稳定、高分辨率的归因图，解决现有方法因ViT架构特性（如patch嵌入和注意力路由）导致的伪影问题。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在计算机视觉中已成为主导架构，但为这些模型生成稳定且高分辨率的归因图仍然具有挑战性。架构组件如patch嵌入和注意力路由通常在像素级解释中引入结构化伪影，导致许多现有方法依赖粗糙的patch级归因。

Method: 提出DAVE方法，基于输入梯度的结构化分解，利用ViT的架构特性，分离出输入-输出映射的局部等变和稳定分量，将其与架构诱导的伪影和其他不稳定源隔离。

Result: 未在摘要中明确说明具体实验结果，但暗示该方法能够生成更稳定、更高分辨率的归因图。

Conclusion: DAVE是一种基于数学基础的ViT归因方法，通过梯度分解有效解决ViT归因中的伪影和不稳定性问题。
...
TL;DR: CauCLIP：基于因果关系的视觉语言框架，利用CLIP学习领域不变表示，用于无需目标域数据的外科阶段识别，显著超越现有方法


<details>
  <summary>Details</summary>
Motivation: 外科阶段识别是智能手术室中情境感知决策支持的关键组件，但训练鲁棒模型受到标注临床视频有限以及合成与真实手术数据间巨大领域差距的限制

Method: 提出CauCLIP框架：1）基于频率的数据增强策略，扰动领域特定属性同时保留语义结构；2）因果抑制损失，减轻非因果偏差并强化因果手术特征；3）统一训练框架使模型聚焦于手术工作流中的稳定因果因素
...
### [44] [Gold Exploration using Representations from a Multispectral Autoencoder](https://arxiv.org/abs/2602.06748)
*Argyro Tsandalidou,Konstantinos Dogeas,Eleftheria Tetoula Tsonga,Elisavet Parselia,Georgios Tsimiklis,George Arvanitakis*

Main category: cs.CV

TL;DR: 利用Sentinel-2卫星影像和预训练的自编码器基础模型，通过生成式表征识别含金区域，相比原始光谱输入显著提高了分类准确率。


<details>
  <summary>Details</summary>
Motivation: 由于实地矿物勘探成本高、数据有限，需要利用卫星影像进行大规模成矿预测。传统方法效果有限，需要探索基于基础模型表征的新方法。

Method: 使用在FalconSpace-S2 v1.0数据集上预训练的Isometric自编码器基础模型，从多光谱Sentinel-2影像中提取信息密集的光谱-空间表征，然后输入轻量级XGBoost分类器进行含金区域识别。

Result: 在63个已知含金和非含金区域的Sentinel-2影像数据集上，该方法将斑块级准确率从0.51提高到0.68，图像级准确率从0.55提高到0.73，表明生成式嵌入能有效捕获可迁移的矿物学模式。

Conclusion: 基础模型表征方法能够利用有限的标注数据捕获可迁移的矿物学模式，使矿物勘探更加高效、可扩展和全球适用。
...
TL;DR: PlanViz是一个评估统一多模态模型在计算机使用任务中图像生成和编辑能力的新基准，包含路线规划、工作图表和网页UI显示三个子任务，并提出了PlanScore评估指标。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在图像生成和多模态推理方面表现出色，但其在计算机使用规划任务（如空间推理和程序理解）中的潜力尚未充分探索，需要专门的评估基准来检验这些能力。

Method: 设计了PlanViz基准，包含三个需要规划步骤的日常任务子集：路线规划、工作图表和网页UI显示。通过人工标注的问题和参考图像确保数据质量，并提出任务自适应评分PlanScore来全面评估生成图像的正确性、视觉质量和效率。

Result: 通过实验揭示了统一多模态模型在计算机使用任务中图像生成和编辑的关键局限性，为未来研究指出了机会和方向。

Conclusion: PlanViz基准填补了统一多模态模型在计算机使用规划任务评估方面的空白，提出的PlanScore指标能够全面评估模型性能，为相关研究提供了重要的评估工具和方向指引。
...
TL;DR: CytoCrowd是一个新的细胞学分析公共基准数据集，包含446张高分辨率图像，每张图像都有四位独立病理学家的原始冲突标注和由资深专家建立的高质量金标准真值，可用于标准计算机视觉任务和标注聚合算法评估。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像数据集大多只提供单一干净的真值（隐藏了真实世界中的专家分歧）或多个标注但没有独立的金标准用于客观评估，这种关键差距限制了机器学习在医学图像分析中的发展。

Method: 创建CytoCrowd数据集，包含446张高分辨率细胞学图像，每张图像收集四位独立病理学家的原始冲突标注，并由资深专家建立单独的高质量金标准真值，形成双重结构数据集。

Result: 实验展示了CytoCrowd带来的挑战，并证明了其作为开发下一代医学图像分析模型资源的价值，为两项任务提供了全面的基线结果。

Conclusion: CytoCrowd填补了现有医学图像数据集的空白，既是标准计算机视觉任务的基准，也是评估标注聚合算法的真实测试平台，有助于推进医学图像分析领域的发展。
...
TL;DR: 本文提出SICA（语义诱导约束适应）方法，首次实现跨四个图像取证子领域的统一检测模型，解决了现有单模型方法性能不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 在真实取证场景中，跨多个图像取证子领域的统一检测（FID）至关重要。虽然理论上单模型方法比集成方法更有前景，但实践中性能一直较差。本文首次诊断出性能不佳的原因是：由于不同子领域伪影特征存在本质差异（异质现象），导致伪影特征空间崩溃。

Method: 提出语义诱导约束适应（SICA）方法，这是首个单模型FID范式。核心思想是利用高层语义作为结构先验，重构"统一而具区分性"的伪影特征空间。该方法在OpenMMSec数据集上进行了广泛实验。

Result: SICA在实验中超越了15种最先进方法，并以近乎正交的方式成功重构了目标伪影特征空间，有力地验证了研究假设。

Conclusion: 通过发现异质现象并诊断单模型FID性能不佳的根本原因，提出的SICA方法成功解决了"统一而具区分性"的伪影特征空间重构难题，为实际应用中的单模型图像取证检测提供了有效解决方案。
...
TL;DR: 提出ScoliGait数据集和多模态框架，用于青少年特发性脊柱侧弯的早期检测，通过视频步态分析实现无创评估，解决了现有方法的数据泄露和可解释性问题。


<details>
  <summary>Details</summary>
Motivation: 青少年特发性脊柱侧弯(AIS)的早期检测可以减缓病情进展，但传统筛查方法主观性强、难以规模化且依赖专业临床知识。基于视频的步态分析虽有潜力，但现有数据集和方法存在数据泄露问题（同一受试者的重复片段导致性能虚高），且模型过于简化、缺乏临床可解释性。
...
Conclusion: 这项工作为可扩展、无创的AIS评估提供了稳健、可解释且基于临床基础的框架，解决了现有方法的局限性，推动了基于视频步态分析的脊柱侧弯筛查技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型文本到图像扩散模型虽然视觉质量不断提升，但其规模扩大导致最先进模型与设备端解决方案之间的差距日益增大，需要开发高质量的设备端生成模型。

Method: 采用渐进式压缩管道，包括：1）基于剪枝的模型压缩策略，从12B减少到2B；2）基于ResNet的令牌下采样机制，降低延迟；3）利用去噪器早期层视觉信号的新型文本编码器蒸馏方法。

Result: NanoFLUX在移动设备上约2.5秒内生成512x512图像，证明了高质量设备端文本到图像生成的可行性。

Conclusion: 通过创新的压缩和蒸馏技术，成功开发出适用于移动设备的高质量文本到图像生成模型，缩小了最先进模型与设备端解决方案之间的差距。
...
### [50] [PANC: Prior-Aware Normalized Cut for Object Segmentation](https://arxiv.org/abs/2602.06912)
...
<details>
  <summary>Details</summary>
Motivation: 完全无监督分割方法通常寻找最显著的对象，导致分割结果对初始化、种子顺序和阈值启发式敏感，缺乏稳定性和可控性。
...
TL;DR: 提出一种将面部情绪识别从单标签分类问题转化为概率分布学习的方法，通过将VAD空间映射到复合情绪分布，自动重新标注现有数据集，以描述复杂情绪状态。


<details>
  <summary>Details</summary>
Motivation: 传统面部情绪识别将情绪简化为六种基本情绪的单标签分类，这过度简化了真实世界中复杂的、多情绪混合的自发情绪状态。需要一种能描述情绪混合和感知模糊性的方法。

Method: 提出一种自动重新标注现有数据集的方法：1）利用已有研究将基本和复合情绪映射到VAD空间的概率分布；2）给定带有VAD标注的面部图像，估计其属于每个情绪分布的可能性；3）将情绪状态描述为多种情绪的混合概率分布。

Result: 通过初步实验展示了该方法的优势，提供了新的研究方向。相关数据标注已在GitHub上公开。

Conclusion: 该方法能够丰富情绪描述，考虑情绪感知的模糊性，将情绪识别从单标签分类转向更符合真实情绪复杂性的概率分布学习框架。
...
TL;DR: 本研究开发了基于计算机视觉的自动评估方法，用于检测甘薯象鼻虫损害，包括田间分类模型（71.43%准确率）和实验室YOLO12目标检测模型（77.7% mAP），显著提高了表型分析效率。


<details>
  <summary>Details</summary>
Motivation: 甘薯象鼻虫是甘薯生产中最具破坏性的害虫之一，特别是在撒哈拉以南非洲地区。传统的损害评估方法主要依赖人工评分，劳动强度大、主观性强且结果不一致，严重阻碍了培育抗性甘薯品种的育种计划。

Method: 研究提出了基于计算机视觉的自动评估方法：1）在田间环境中收集数据训练分类模型预测根部损害严重程度；2）在实验室环境中建立数据集，设计采用YOLO12的目标检测流程，结合根部分割和分块策略的两阶段实验室流程，以提高小目标检测能力。

Result: 田间分类模型测试准确率达到71.43%；实验室YOLO12模型在识别微小象鼻虫取食孔方面达到77.7%的平均精度均值（mAP）。

Conclusion: 计算机视觉技术能够提供高效、客观、可扩展的评估工具，与当代育种工作流程无缝对接。这些进展显著提高了甘薯育种计划中的表型分析效率，在减轻象鼻虫对粮食安全的不利影响方面发挥关键作用。
...
TL;DR: 该论文推导了在两个标定视图之间映射局部图像块的仿射变换的闭式表达式，该变换是相对相机位姿、图像坐标和局部表面法向量的函数。


<details>
  <summary>Details</summary>
Motivation: 在计算机视觉中，理解不同视角下图像局部区域之间的几何关系对于特征匹配、三维重建等任务至关重要。现有方法可能缺乏对仿射变换与相机几何参数之间关系的精确数学描述。

Method: 通过数学推导，建立了从相对相机位姿、图像坐标和局部表面法向量到仿射变换的闭式表达式，基于标定相机模型和几何投影关系。

Result: 成功推导出仿射变换的闭式表达式，明确展示了变换如何依赖于相对相机位姿、图像坐标和局部表面法向量，为图像配准和三维重建提供了理论基础。

Conclusion: 该闭式表达式建立了图像局部变换与底层几何参数之间的直接联系，为计算机视觉中的特征匹配和三维重建任务提供了更精确的数学工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么关注预定义公平性类别中的偏见缓解，要么识别主导输出的多数属性，但都忽略了发现数据分布中代表性不足但仍被模型编码的稀有或少数特征（社会、文化或风格属性）。

Method: RAIGen利用Matryoshka稀疏自编码器和新颖的少数属性度量（结合神经元激活频率和语义独特性）来识别可解释的神经元，这些神经元的最高激活图像能揭示代表性不足的属性。

Result: 实验表明RAIGen能在Stable Diffusion中发现超越固定公平性类别的属性，可扩展到SDXL等更大模型，支持跨架构的系统性审计，并能在生成过程中实现稀有属性的定向增强。

Conclusion: RAIGen为无监督发现扩散模型中的稀有属性提供了首个框架，填补了现有偏见缓解和识别方法之间的空白，能够揭示被数据分布低估但仍被模型编码的多样化特征。
...
TL;DR: GaussianPOP：基于分析性高斯误差量化的3D高斯泼溅简化框架，通过直接从渲染方程推导误差准则，实现更优的紧凑性与渲染质量平衡


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅简化方法通常使用重要性评分（如混合权重或敏感度）来识别冗余高斯，但这些评分并非基于视觉误差指标，导致在紧凑性和渲染保真度之间难以达到最优平衡

Method: 提出基于分析性高斯误差量化的简化框架，核心贡献是从3DGS渲染方程直接推导出新颖的误差准则，精确测量每个高斯对渲染图像的贡献。引入高效算法，支持单次前向传播完成误差计算。框架支持训练中剪枝和训练后简化两种应用场景

Result: 实验结果表明，该方法在两种应用场景下均优于现有最先进的剪枝方法，实现了模型紧凑性与高渲染质量之间的更优平衡

Conclusion: GaussianPOP提供了一个准确且灵活的简化框架，通过分析性误差量化实现了比现有方法更好的紧凑性-质量权衡，支持训练中剪枝和训练后简化两种应用模式
...
TL;DR: RFDM是一种基于残差流的扩散模型，通过逐帧编辑实现可变长度视频编辑，计算效率接近图像模型，不依赖输入视频长度


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法需要固定长度输入和大量计算资源，而自回归视频生成虽然能高效合成可变长度视频，但在视频编辑领域应用不足。需要一种既能编辑可变长度视频又计算高效的解决方案。

Method: 从2D图像到图像扩散模型出发，通过条件化当前时间步的编辑基于前一时间步的预测，实现视频到视频编辑。提出残差流扩散模型(RFDM)，采用新的I2I扩散前向过程公式，鼓励模型预测目标输出与先前预测之间的残差，使去噪过程专注于连续帧之间的变化。

Result: 在全局/局部风格迁移和物体移除任务上，RFDM超越了基于I2I的方法，与完全时空(3D)V2V模型竞争，同时计算效率与图像模型相当，且独立于输入视频长度扩展。

Conclusion: RFDM提供了一种因果、高效的视频编辑方法，能够处理可变长度视频，计算效率高，为视频编辑任务提供了新的解决方案，并提出了更好的评估基准来排名最先进的方法。
...
TL;DR: 该研究探讨了视觉大语言模型(VLLMs)在细粒度视觉任务上表现不佳的原因，通过构建合成基准数据集分析视觉冗余问题，发现任务复杂度与视觉压缩之间存在关联。


<details>
  <summary>Details</summary>
Motivation: VLLMs的视觉能力一直落后于语言能力，尤其在需要细粒度视觉信息或空间推理的任务上表现不佳。现有研究将问题归因于视觉冗余，但缺乏对具体机制的理解。本研究旨在深入探究VLLMs如何处理不同类型视觉信息，以及哪些信息被丢弃。

Method: 1. 构建专门设计的合成基准数据集，用于探测各种视觉特征；2. 开发测量视觉冗余的指标集；3. 在多个复杂视觉任务上对VLLMs进行微调，研究冗余和压缩如何随训练数据复杂度变化。

Result: 研究发现任务复杂度与视觉压缩之间存在关联，表明拥有足够比例的高复杂度视觉数据对于改变VLLMs分配视觉表示的方式至关重要，从而能提升其在复杂视觉任务上的表现。

Conclusion: 本研究为理解VLLMs视觉处理机制提供了新见解，揭示了视觉冗余与任务复杂度的关系，为训练下一代VLLMs提供了有价值的指导。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像数据集标注困难，专家标注资源有限，类别边界模糊，导致数据集中可能存在错误标签，影响深度学习模型性能

Method: 开发了一个医学数据集错误标签检测框架，在公开的胶囊内镜数据集上验证，通过三位经验丰富的胃肠病专家重新标注识别出的潜在错误样本

Result: 提出的框架成功检测出错误标签数据，清理数据集后相比现有基线方法，异常检测性能得到提升

Conclusion: 该错误标签检测框架能有效识别医学影像数据集中的标注错误，通过数据清理可提高深度学习模型的分类性能
...
<details>
  <summary>Details</summary>
Motivation: 电影视频制作需要控制场景-主体构图和相机运动，但实景拍摄成本高昂，需要构建物理布景。为了解决这个问题，研究者提出了解耦场景上下文的电影视频生成任务：给定静态环境的多个图像，目标是合成高质量视频，包含动态主体，同时保持底层场景一致性并遵循用户指定的相机轨迹。

Method: 提出CineScene框架，利用隐式3D感知场景表示进行电影视频生成。关键创新是新颖的上下文条件机制：通过VGGT将场景图像编码为视觉表示，通过额外的上下文连接将空间先验注入预训练的文本到视频生成模型，实现相机控制的视频合成。训练时采用随机打乱输入场景图像的策略增强鲁棒性。为了解决训练数据不足的问题，使用Unreal Engine 5构建了场景解耦数据集，包含有/无动态主体的配对视频、全景图像和相机轨迹。

Result: 实验表明，CineScene在场景一致的电影视频生成方面实现了最先进的性能，能够处理大幅度的相机运动，并在不同环境中展现出良好的泛化能力。

Conclusion: CineScene通过隐式3D感知场景表示和创新的上下文条件机制，成功解决了电影视频生成中场景一致性和相机控制的问题，为低成本电影制作提供了有效的解决方案。
...
TL;DR: MedMO是一个专门针对医学领域构建的多模态大语言模型，通过多阶段训练方法在医学视觉问答、报告生成和疾病定位等任务上显著优于现有开源模型。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在医学领域的应用受到领域覆盖不足、模态对齐不充分和推理能力有限的制约，需要专门的医学基础模型来解决这些问题。

Method: 采用三阶段训练方法：1）跨模态预训练对齐视觉编码器和医学语言骨干；2）多任务指令微调涵盖图像描述、视觉问答、报告生成、检索和带边界框的疾病定位；3）强化学习结合事实性检查和边界框级GIoU奖励来增强空间定位和逐步推理能力。

Result: 在视觉问答基准上平均准确率比基线提升13.7%，接近SOTA Fleming-VL（差距1.9%）；文本问答比基线提升6.9%，比Fleming-VL提升14.5%；在医学报告生成方面语义和临床准确性均有显著提升；空间定位能力IoU比基线提升40.4%，比Fleming-VL提升37.0%。

Conclusion: MedMO通过专门的医学数据训练和多阶段优化策略，在医学多模态任务上表现出色，特别是在空间定位和推理能力方面具有显著优势，展现了跨模态泛化能力，为医学AI应用提供了有力工具。
...
TL;DR: 该论文提出了一种基于名人名言重构的广告口号生成新范式，通过模块化框架将口号生成分解为可解释的子任务，相比现有LLM基线在多样性、新颖性和情感影响方面取得边际改进。


<details>
  <summary>Details</summary>
Motivation: 广告疲劳降低了重复口号的效果，需要新颖、有创意、有洞察力的口号生成。现有LLM方法往往产生风格冗余的输出，缺乏清晰的品牌个性，显得过于机器生成。有效口号应在新颖性和熟悉性之间取得平衡。

Method: 提出基于名人名言重构的新范式，利用名人名言自然符合口号长度、使用丰富修辞手法、提供深度和洞察力的特点。引入模块化框架，将口号生成分解为可解释的子任务：名言匹配、结构分解、词汇替换和混搭生成。

Result: 通过广泛的自动和人工评估，相比三个最先进的LLM基线，在多样性、新颖性、情感影响和人类偏好方面取得了边际改进。

Conclusion: 通过重新语境化与品牌个性相关的名人名言来生成广告口号是一种有效的方法，能够在保持熟悉感的同时提供新颖性和创意深度，解决了现有LLM方法在风格冗余和缺乏品牌个性方面的问题。
...
TL;DR: 提出RMCD解码方法，通过加权聚合多个相关上下文的信息，同时抑制无关上下文的影响，提升大视觉语言模型在知识密集型视觉问答任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大视觉语言模型（LVLMs）能力显著，但仍缺乏对特定实体的详细知识。检索增强生成（RAG）通过外部知识库提供额外上下文来增强LVLMs，但现有的解码方法未能充分利用多个相关上下文，也无法有效抑制无关上下文的负面影响。

Method: 提出相关性感知的多上下文对比解码（RMCD）方法。该方法基于每个上下文与问题的相关性对预测输出进行加权，然后组合这些加权输出得到最终预测。这样既能有效聚合多个相关上下文的有用信息，又能抵消无关上下文的负面影响。

Result: 在多个LVLMs上，RMCD始终优于其他解码方法，在三个知识密集型视觉问答基准测试中取得最佳性能。RMCD无需额外训练，只需替换解码方法即可应用。分析表明RMCD对检索结果具有鲁棒性，在从最弱到最强的各种检索结果中都表现最佳。

Conclusion: RMCD是一种有效的解码方法，能够充分利用检索增强生成中的多个上下文，同时抑制无关信息的影响，显著提升大视觉语言模型在知识密集型任务上的性能，且易于部署无需额外训练。
...
### [63] [CAST: Character-and-Scene Episodic Memory for Agents](https://arxiv.org/abs/2602.06051)
*Kexin Ma,Bojun Li,Yuhua Tang,Ruochun Jin,Liting Sun*

Main category: cs.CL

TL;DR: CAST提出了一种基于戏剧理论的角色与场景记忆架构，通过构建3D场景（时间/地点/主题）并组织成角色档案来表示情景记忆，同时结合基于图的语义记忆，形成双记忆设计，显著提升了对话问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有智能体记忆系统主要强调语义回忆，将经验处理为键值对、向量或图等结构，难以表示和检索连贯的事件（情景记忆）。人类的情景记忆能够回忆基于"谁、何时、何地"的连贯事件，这是当前系统所缺乏的。

Method: 受戏剧理论启发，提出CAST架构：1) 构建3D场景（时间、地点、主题）来组织事件；2) 将这些场景组织成角色档案，总结角色的相关事件；3) 结合基于图的语义记忆，形成双记忆系统。

Result: 实验表明，CAST在多个数据集上平均提升了8.11%的F1分数和10.21%的J(LLM-as-a-Judge)分数，特别是在开放性和时间敏感的对话问题上表现尤为突出。

Conclusion: CAST通过戏剧理论启发的角色-场景记忆架构，有效解决了智能体系统中情景记忆表示和检索的挑战，结合双记忆设计显著提升了对话问答性能。
...
### [64] [Rethinking Memory Mechanisms of Foundation Agents in the Second Half](https://arxiv.org/abs/2602.06052)
...
TL;DR: 该论文是一篇关于智能体记忆系统的综述，提出了一个统一的三维框架（记忆基质、认知机制、记忆主体），分析了不同智能体拓扑中的记忆实现，并回顾了评估基准和未来挑战。


<details>
  <summary>Details</summary>
Motivation: 人工智能研究正从追求模型创新和基准分数转向强调问题定义和真实世界评估。随着领域进入"下半场"，核心挑战变为在长期、动态、用户依赖的环境中实现实际效用，智能体面临上下文爆炸，需要持续积累、管理和选择性重用大量信息。记忆因此成为填补效用差距的关键解决方案。

Method: 提出了一个统一的三维框架来分析基础智能体记忆：1) 记忆基质（内部和外部），2) 认知机制（情景、语义、感官、工作、程序记忆），3) 记忆主体（智能体中心与用户中心）。分析了不同智能体拓扑中记忆的实例化和操作方式，强调了记忆操作的学习策略。

Result: 论文提供了一个全面的记忆系统分析框架，展示了记忆如何在不同智能体架构中实现，并识别了记忆操作的学习策略。回顾了评估记忆效用的基准和指标，为领域提供了系统化的理解。

Conclusion: 记忆是解决智能体在长期、动态环境中效用差距的关键技术。论文提出的三维框架为理解和设计智能体记忆系统提供了统一视角，同时指出了评估挑战和未来研究方向，包括更好的基准、可扩展的记忆架构和更有效的学习策略。
...
TL;DR: 提出了一种基于文献感知的新颖性评估框架，通过学习同行评审报告来学习人类如何判断新颖性，并通过结构化比较现有研究来支撑这些判断。
...
Method: 使用近8万条新颖性标注的AI顶会评审报告微调大语言模型，提取论文的结构化表示（想法、方法、主张），检索语义相关论文，构建相似性图进行细粒度概念级比较。

Result: 系统能够生成校准的新颖性分数和类人解释性评估，相对于现有方法减少了高估并提高了评估一致性。

Conclusion: 该框架通过学习人类评审行为并基于结构化文献比较，为研究新颖性评估提供了更客观、一致的方法，有望改进同行评审过程。
...
TL;DR: 本文提出了一种新的标注一致性度量方法，专门用于分析不同标注者群体之间的极化现象，适用于不平衡群体和多标签设置，并在仇恨言论和毒性检测任务中发现了基于种族、宗教和教育程度的显著标注差异。


<details>
  <summary>Details</summary>
Motivation: 现有标注一致性度量方法存在三个主要问题：1) 不适合进行群体间分析；2) 对群体规模不平衡敏感；3) 仅限于单标注设置。这些限制使得它们无法充分处理主观任务（如毒性和仇恨言论检测）中的标注者群体差异问题。

Method: 作者引入了一种可量化的度量指标，并配合统计显著性检验，能够将极化现象归因于不同的标注者群体。该方法支持直接比较严重不平衡的社会人口学和意识形态子群体，适用于多标签设置，并提供了开源Python库实现。

Result: 在三个仇恨言论数据集和一个毒性检测数据集上的应用发现：1) 极化现象强烈且持续地归因于标注者的种族，特别是在仇恨言论任务中；2) 宗教标注者之间没有根本性分歧，但与其他标注者存在分歧，这种趋势在非宗教标注者中逐渐减弱并逆转；3) 教育程度较低的标注者更主观，而教育程度较高的标注者之间更倾向于广泛一致。

Conclusion: 该研究提出了一种新的度量方法，能够有效分析标注者群体间的极化现象，揭示了不同社会群体在主观任务标注中的系统性差异，并估计了获得稳健结果所需的最小标注者数量，为相关研究提供了实用工具。
...
TL;DR: COVER提出了一种新的并行扩散解码验证方法，通过KV缓存覆盖进行留一验证和稳定草稿生成，减少不必要的修订，加速推理同时保持输出质量。


<details>
  <summary>Details</summary>
Motivation: 现有并行扩散解码方法中，可撤销解码的验证方案经常触发翻转振荡（flip-flop oscillations），即令牌被重新掩码后又恢复原状。这种行为通过两种方式减慢推理：重新掩码已验证位置会削弱并行草稿生成的上下文条件；重复的重新掩码循环消耗修订预算但进展甚微。

Method: COVER（Cache Override Verification for Efficient Revision）通过KV缓存覆盖在单次前向传递中执行留一验证和稳定草稿生成。它通过KV缓存覆盖构建两个注意力视图：选定种子被掩码用于验证，同时它们的缓存键值状态被注入到所有其他查询中以保留上下文信息，通过闭式对角校正防止种子位置的自泄漏。COVER进一步使用稳定性感知评分优先处理种子，平衡不确定性、下游影响和缓存漂移，并自适应调整每步验证的种子数量。

Result: 在多个基准测试中，COVER显著减少了不必要的修订，在保持输出质量的同时实现了更快的解码速度。

Conclusion: COVER通过创新的KV缓存覆盖验证机制，有效解决了并行扩散解码中的翻转振荡问题，在减少不必要修订的同时加速了推理过程，为高效扩散语言模型推理提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究量化后训练对大型语言模型社会偏见的影响，发现现有聚合指标无法捕捉量化带来的偏见模式根本性变化，需要系统评估量化模型的偏见行为。

Method: 使用PostTrainingBiasBench基准测试，评估50个量化模型在13个封闭式和开放式偏见数据集上的表现，分析量化强度、模型不确定性对偏见变化的影响。

Result: 发现量化诱导的掩蔽偏见翻转现象：21%的响应在偏见状态间翻转；高不确定性响应翻转概率是低不确定性的3-11倍；4位量化比8位量化产生4-6倍的行为变化；不同人口群体偏见变化不对称，某些群体恶化18.6%，其他改善14.1%；大模型没有一致的鲁棒性优势。

Conclusion: 压缩从根本上改变了偏见模式，需要关键的量化后评估和干预措施来确保实际应用的可靠性，仅依赖聚合指标会产生误导性结果。
...
<details>
  <summary>Details</summary>
Motivation: 当前NLP领域的多项选择题基准缺乏严格的质量控制，存在各种缺陷，这些缺陷会降低评估的可靠性，需要系统化的检测工具来改进基准设计。

Method: 开发BenchMarker工具包，利用LLM作为评判者来检测三种MCQ缺陷：1) 污染（题目完全出现在网上）；2) 捷径（选项中的线索使猜测成为可能）；3) 写作错误（基于19条教育规则的语法/结构问题）。通过人工标注验证工具，然后审计12个基准。

Result: 1) 验证显示BenchMarker与人工标注一致；2) 审计发现：受污染的MCQ倾向于提高准确率，而写作错误倾向于降低准确率并改变排名；3) 先前的基准修复虽然解决了目标问题，但无意中引入了新的缺陷（如不合理的干扰项、多个正确答案）。
...
TL;DR: 研究发现大型语言模型可以通过提供单方面论据来引导其回答特定立场，即使面对争议性问题


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在面对争议性问题时，可能提供平衡回答、采取单一对齐立场或拒绝回答。研究者想探究是否可以通过简单直观的方式（仅提供支持特定立场的单方面论据）来引导模型的初始响应。

Method: 构建小型数据集，从三个维度进行系统研究：(i) 模型响应中诱导的立场，(ii) 争议性问题的表述方式，(iii) 论据的呈现方式。测试不同模型、论据数量和主题下的意见引导效果。

Result: 研究发现意见引导现象在三个维度上普遍存在，适用于不同模型、论据数量和主题。当切换到其他论据时，意见引导效果会一致性地降低。

Conclusion: 大型语言模型可以通过仅提供单方面论据来被引导至特定立场，这揭示了模型在争议性问题上的可操纵性，对理解LLM的立场表达机制具有重要意义。
...
TL;DR: MPIB是一个医疗提示注入基准测试套件，用于评估LLMs和RAG系统在临床环境中的安全性，通过CHER指标衡量高风险临床伤害事件。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs和RAG系统在临床工作流程中的集成增加，提示注入攻击可能导致临床不安全或误导性输出，需要系统评估临床安全性。

Method: 开发了包含9,697个实例的MPIB基准测试套件，通过多阶段质量控制和临床安全检查构建，使用CHER（临床伤害事件率）和ASR（攻击成功率）双重指标评估。

Result: 评估发现ASR和CHER指标存在显著差异，系统鲁棒性关键取决于对抗性指令出现在用户查询还是检索上下文中。

Conclusion: MPIB为临床提示注入研究提供了可重复的系统评估框架，强调需要区分指令遵从性和下游患者风险，并发布了完整的代码和数据支持进一步研究。
...
TL;DR: VowelPrompt是一个基于语言学的框架，通过提取元音级别的韵律特征（音高、能量、时长）并将其转换为自然语言描述，增强LLM在语音情感识别中的能力，结合两阶段微调方法显著提升了跨领域和跨语言的性能。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在语音情感识别中主要依赖文本转录，忽略了细粒度的韵律信息（如音高、强度、时间动态），这限制了模型的性能和可解释性。需要一种能够同时理解语言内容和声音表达性的多模态方法。

Method: 提出VowelPrompt框架：1）基于语音学证据提取时间对齐的元音片段的音高、能量和时长特征；2）将这些特征转换为自然语言描述；3）采用两阶段适应过程：监督微调（SFT）后接基于可验证奖励的强化学习（RLVR），通过组相对策略优化（GRPO）实现。

Result: 在多个基准数据集上的广泛评估表明，VowelPrompt在零样本、微调、跨领域和跨语言条件下均优于最先进的情感识别方法，同时能够生成基于上下文语义和细粒度韵律结构的可解释说明。

Conclusion: VowelPrompt通过将细粒度的元音级别韵律信息整合到LLM推理中，显著提升了语音情感识别的性能、泛化能力和可解释性，为多模态情感分析提供了有前景的方向。
...
TL;DR: 提出了一种无需人工标注的评估方法Consequence-Based Utility，通过测试候选解决方案作为上下文示例解决相关可验证问题的能力来评估其质量，在数学推理任务上显著优于现有评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前推理模型在生成研究级数学问题的解决方案方面取得进展，但验证过程仍然依赖稀缺的专家时间，成为瓶颈。作者认为，一个真正有意义的解决方案应该包含足够的方法层面信息，当应用于相关问题时，应该比错误解决方案产生更好的下游性能。

Method: 提出了Consequence-Based Utility评估方法，该方法不依赖人工标注（oracle-free），通过将每个候选解决方案作为上下文示例，测试其在解决相关但可验证问题时的价值来评分。具体来说，评估候选解决方案作为few-shot示例时，在解决相关验证问题上的表现。

Result: 在原创的研究级数学问题数据集上评估，每个问题配有一个专家编写的解决方案和九个LLM生成的解决方案。Consequence-Based Utility在排名质量上一致优于奖励模型、生成式奖励模型和LLM评判器。对于GPT-OSS-120B，将Acc@1从67.2提高到76.3，AUC从71.4提高到79.6；对于GPT-OSS-20B，AUC从69.0提高到79.2。与LLM评判器相比，还表现出更大的求解器-评估器差距，即使在底层求解器经常失败的实例上也能保持更强的正确-错误分离能力。

Conclusion: Consequence-Based Utility提供了一种有效的、无需人工标注的评估方法，能够准确评估研究级数学推理解决方案的质量，显著优于现有评估方法，为解决数学推理验证瓶颈提供了有前景的方向。
...
TL;DR: 本文研究了通过后训练提升大语言模型因果推理能力的方法，发现适当的后训练能让较小模型在因果推理任务上超越更大模型，14B参数模型在CaLM基准上达到93.5%准确率。


<details>
  <summary>Details</summary>
Motivation: 因果推理对决策至关重要，但对非专家来说仍然具有挑战性。虽然大语言模型在这一领域显示出潜力，但其精确的因果估计能力仍然有限，且后训练对这些能力的影响尚未得到充分探索。

Method: 引入CauGym数据集，包含七个核心因果任务用于训练和五个不同的测试集。系统评估了五种后训练方法：SFT、DPO、KTO、PPO和GRPO，并在五个领域内基准和四个现有基准上进行实验。

Result: 适当的后训练使较小的大语言模型能够竞争性地执行因果推理，通常超越更大的模型。14B参数模型在CaLM基准上达到93.5%准确率，而OpenAI o3仅为55.4%。后训练模型在分布偏移和噪声数据等现实条件下表现出强大的泛化能力和鲁棒性。

Conclusion: 这是首次系统证据表明，有针对性的后训练可以产生可靠且鲁棒的大语言模型因果推理器。研究结果为提升大语言模型的因果推理能力提供了有效方法。
...
TL;DR: 对比了基于提示的大语言模型与微调编码器在文本分类任务中的表现，发现微调编码器在性能、延迟和成本方面更具优势


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在开放式推理任务中表现出色，但在结构化文本分类任务中，模型选择往往只关注预测性能，忽略了生产系统中的操作约束（如延迟和成本）

Method: 系统比较了两种文本分类范式：零样本/少样本提示的大语言模型与完全微调的编码器架构。在四个基准数据集（IMDB、SST-2、AG News、DBPedia）上评估预测质量（宏观F1）、推理延迟和货币成本。将模型评估构建为多目标决策问题，使用帕累托前沿投影和参数化效用函数分析权衡

Result: 微调的BERT系列编码器模型在分类性能上具有竞争力且通常更优，同时成本比零样本/少样本LLM提示低1-2个数量级，延迟也更低

Conclusion: 盲目使用大语言模型进行标准文本分类可能导致次优的系统级结果。微调编码器是结构化NLP流程中稳健高效的组件，而LLM更适合作为混合架构中的补充元素
...
TL;DR: ReBeCA框架通过因果分析揭示自反思行为的层次机制，发现仅有少数语义行为具有因果性，且更多"积极"行为反而可能损害反思效果


<details>
  <summary>Details</summary>
Motivation: 现有自反思机制分析多为相关性研究，缺乏可解释性和泛化能力，需要揭示其真正的因果机制

Method: 提出ReBeCA框架，将自反思轨迹建模为因果图，采用三阶段不变因果预测(ICP)流程分离真正影响性能的因素

Result: 发现：1)语义行为对自反思结果有层次化影响；2)仅有少数语义行为具有因果泛化性；3)看似积极的语义行为组合可能损害反思效果。ICP验证显示稀疏因果父节点带来49.6%结构似然增益

Conclusion: ReBeCA为自反思动态提供了严谨的因果机制分析方法，能够区分真实因果关系与虚假关联
...
TL;DR: FMBench是一个评估大语言模型Markdown格式输出能力的基准测试，提出了结合监督微调和强化学习的轻量级对齐方法，在保持语义准确性的同时提高格式合规性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在用户界面和系统集成工作流中需要同时满足语义意图和格式约束，但Markdown格式（如列表、表格、标题、代码块）容易产生难以检测的细微错误，严重影响下游可用性。

Method: 提出FMBench基准测试评估模型在各种指令遵循场景下的Markdown格式能力；开发轻量级对齐流程：先进行监督微调（SFT），然后通过强化学习微调优化平衡语义保真度和结构正确性的复合目标。

Result: 在两个模型家族（OpenPangu和Qwen）上的实验表明：SFT持续改善语义对齐，而强化学习在具有挑战性的Markdown指令上提供额外的鲁棒性提升；研究揭示了语义和结构目标之间的内在权衡。

Conclusion: FMBench为评估和改进大语言模型的Markdown格式输出能力提供了系统方法，结合SFT和强化学习的对齐流程能有效提高格式合规性，同时强调了精心设计奖励函数对于可靠格式生成的重要性。
...
### [78] [On the Wings of Imagination: Conflicting Script-based Multi-role Framework for Humor Caption Generation](https://arxiv.org/abs/2602.06423)
*Wenbo Shang,Yuxi Sun,Jing Ma,Xin Huang*

Main category: cs.CL

TL;DR: 提出HOMER框架，基于GTVH幽默理论，通过多角色LLM协作和幽默检索生成图像幽默字幕，在New Yorker Cartoon数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 多模态幽默生成（如图像幽默字幕）对LLMs具有挑战性，需要视觉理解、幽默推理和创造性想象。现有基于LLM的方法依赖推理链或自我改进，存在创造力和可解释性有限的问题。

Method: 基于GTVH幽默理论，提出幽默理论驱动的多角色LLM协作框架HOMER，包含三个角色：冲突脚本提取器（定位幽默关键脚本对立）、检索增强的分层想象器（识别幽默目标并通过想象树扩展创意空间）、字幕生成器（基于获得的知识生成幽默多样的字幕）。

Result: 在两个New Yorker Cartoon基准数据集上的广泛实验表明，HOMER在多模态幽默字幕生成任务上超越了最先进的基线和强大的LLM推理策略。

Conclusion: HOMER框架通过结合幽默理论和多角色LLM协作，有效解决了多模态幽默生成的挑战，在幽默字幕生成任务上取得了优异性能。
...
### [79] [TrailBlazer: History-Guided Reinforcement Learning for Black-Box LLM Jailbreaking](https://arxiv.org/abs/2602.06440)
...
<details>
  <summary>Details</summary>
Motivation: 现有越狱技术大多未能有效利用早期交互轮次中揭示的漏洞，导致攻击效率低下且不稳定。由于越狱涉及序列交互，强化学习为此问题提供了自然框架。

Method: 提出历史感知的RL越狱框架，分析并重新加权先前步骤中的漏洞信号来指导未来决策。引入基于注意力的重新加权机制，突出交互历史中的关键漏洞，实现更高效的探索。

Result: 在AdvBench和HarmBench上的广泛实验表明，该方法实现了最先进的越狱性能，同时显著提高了查询效率。仅加入历史信息就能提高越狱成功率。

Conclusion: 历史漏洞信号在强化学习驱动的越狱策略中至关重要，为推进LLM安全防护的对抗研究提供了原则性途径。
...
### [80] [CORE: Comprehensive Ontological Relation Evaluation for Large Language Models](https://arxiv.org/abs/2602.06446)
...
TL;DR: LLMs在区分语义相关性和真正无关性方面存在严重缺陷，在无关对上的表现远差于相关对，显示出系统性生成虚假关系的问题。


<details>
  <summary>Details</summary>
Motivation: 现有评估很少评估LLMs区分有意义的语义关系和真正无关性的能力，需要填补这一评估空白。

Method: 引入CORE数据集，包含225K多选题覆盖74个学科，以及一个包含203个经过严格验证问题的通用领域基准，涵盖24种语义关系类型，相关对和无关对数量相等。

Result: 人类基线准确率92.6%（无关对上95.1%），而29个最先进LLMs总体准确率48.25-70.9%，在相关对上表现接近天花板（86.5-100%），但在无关对上严重退化（0-41.35%），校准误差增加2-4倍，平均语义崩溃率37.6%。

Conclusion: 无关性推理是LLM评估和安全的关键且未被充分评估的前沿领域，LLMs在区分语义相关性和无关性方面存在系统性缺陷。
...
### [81] [Evaluating an evidence-guided reinforcement learning framework in aligning light-parameter large language models with decision-making cognition in psychiatric clinical reasoning](https://arxiv.org/abs/2602.06449)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在精神病学应用中存在幻觉和浅层推理问题，特别是轻量级模型在临床部署中面临隐私保护和效率需求，现有训练范式过于注重语言流畅性而忽视了结构化临床逻辑，导致与专业诊断认知存在根本性不匹配

Method: 引入ClinMPO强化学习框架，使用专门奖励模型，该模型基于4,474篇精神病学期刊文章构建的数据集独立训练，并按照循证医学原则结构化，在专门设计的测试集上评估模型推理能力

Result: ClinMPO调优的Qwen3-8B模型在复杂病例诊断准确率达到31.4%，超越了300名医学生30.8%的基准表现，在领先大参数LLM持续失败的测试项目上取得突破
...
### [82] [RelayGen: Intra-Generation Model Switching for Efficient Reasoning](https://arxiv.org/abs/2602.06454)
*Jiwon Song,Yoongon Kim,Jae-Joon Kim*

Main category: cs.CL

TL;DR: RelayGen是一个无需训练的分段级运行时模型切换框架，通过利用长格式推理中的难度变化，动态将低难度推理段委派给小型模型，从而在保持大模型准确性的同时显著降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务上表现出色，但生成长推理轨迹时推理成本高昂。现有效率优化方法要么忽略生成过程中的难度变化，要么依赖监督的令牌级路由，系统复杂度高。需要一种能利用推理轨迹内部难度变化的高效方法。

Method: 通过离线分析生成不确定性（使用令牌概率边际），发现粗粒度的分段级控制足以捕捉推理轨迹中的难度转换。RelayGen识别模型特定的切换线索，动态将低难度段的继续生成委派给小型模型，同时在高难度推理上保留大型模型。

Result: 在多个推理基准测试中，RelayGen显著降低了推理延迟，同时保留了大型模型的大部分准确性。与推测解码结合时，实现了高达2.2倍的端到端加速，准确度下降小于2%，无需额外训练或学习路由组件。

Conclusion: RelayGen证明了利用推理轨迹中难度变化的粗粒度分段级控制的有效性，为降低大型推理模型部署成本提供了一种无需训练的高效解决方案，在保持准确性的同时显著提升推理速度。
...
### [83] [Diffusion-State Policy Optimization for Masked Diffusion Language Models](https://arxiv.org/abs/2602.06462)
...
Method: 提出DiSPO（扩散状态策略优化），在选定的中间掩码状态进行分支：从缓存的logits中重新采样当前掩码位置的填充，对生成的完成进行评分，并仅更新新填充的标记，无需额外的多步扩散rollout。

Result: 在LLaDA-8B-Instruct模型上，DiSPO在数学和规划基准测试中始终优于基于终端反馈的diffu-GRPO基线，且计算量和优化步骤相匹配。

Conclusion: DiSPO作为一种即插即用的信用分配层，能够有效优化扩散语言模型的中间填充决策，提升强化学习效果。
...
### [84] [Improve Large Language Model Systems with User Logs](https://arxiv.org/abs/2602.06470)
*Changyue Wang,Weihang Su,Qingyao Ai,Yiqun Liu*

Main category: cs.CL

TL;DR: UNO框架通过用户日志驱动优化，将非结构化日志转化为结构化规则和偏好对，通过聚类处理数据异质性，量化认知差距，自适应过滤噪声反馈，显著提升LLM系统性能。


<details>
  <summary>Details</summary>
Motivation: 传统LLM依赖大规模高质量数据和参数扩展的模式面临数据稀缺和计算成本收益递减的瓶颈，而用户交互日志提供了丰富的真实人类反馈和程序性知识来源，但日志的非结构化和噪声特性使得现有LLM系统难以有效利用这些数据。
...
Conclusion: UNO为从真实世界部署中持续学习提供了一个统一框架，能够有效利用用户日志中的反馈信号，克服了传统方法的局限性，为LLM系统的优化提供了新方向。
...
### [85] [Revisiting the Shape Convention of Transformer Language Models](https://arxiv.org/abs/2602.06471)
*Feng-Ting Liao,Meng-Hsi Chen,Guan-Ting Yi,Da-shan Shiu*

Main category: cs.CL

TL;DR: 本文挑战了Transformer中传统的窄-宽-窄MLP设计，提出用更深的沙漏形FFN替代传统FFN，通过减少FFN参数并增加注意力参数，在固定预算下实现更优性能。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer语言模型一直采用窄-宽-窄MLP结构的FFN，但最近研究表明残差宽-窄-宽（沙漏形）MLP具有更好的函数逼近能力。作者质疑传统MLP形状的必要性，希望探索更优的架构设计。

Method: 开发了一种Transformer变体，用更深的沙漏形FFN替代传统FFN。该沙漏形FFN由多个通过残差连接的小型沙漏子MLP堆叠而成，整体更轻量化。通过减少FFN参数并将节省的参数用于扩大模型隐藏维度或增加注意力参数。

Result: 在400M参数规模下，沙漏形FFN优于传统FFN；在1B参数规模下性能相当。在固定预算下，减少FFN参数并增加注意力参数的沙漏形变体相比传统配置持续改进。

Conclusion: 沙漏形FFN可作为传统FFN的竞争性替代方案，促使重新思考窄-宽-窄MLP惯例以及注意力与FFN之间的平衡，为构建更高效、表达能力更强的现代语言模型提供了新思路。
...
### [86] [MTQE.en-he: Machine Translation Quality Estimation for English-Hebrew](https://arxiv.org/abs/2602.06546)
*Andy Rosenbaum,Assaf Siani,Ilan Kernerman*

Main category: cs.CL

TL;DR: 发布了首个公开的英希机器翻译质量评估基准MTQE.en-he，包含959个英文段落及其希伯来语机器翻译，由三位专家标注质量分数。通过集成ChatGPT、TransQuest和CometKiwi模型，性能比最佳单模型提升6.4个百分点（Pearson）和5.6个百分点（Spearman）。


<details>
  <summary>Details</summary>
Motivation: 为希伯来语这一资源匮乏语言对建立首个公开的机器翻译质量评估基准，填补该领域的研究空白，促进未来相关研究发展。

Method: 构建包含959个英文段落及其希伯来语机器翻译的数据集，由三位专家进行直接评估标注。采用模型集成方法结合ChatGPT提示、TransQuest和CometKiwi，并尝试参数高效微调方法（LoRA、BitFit和FTHead）。

Result: 模型集成方法显著优于最佳单模型（CometKiwi），Pearson相关系数提升6.4个百分点，Spearman相关系数提升5.6个百分点。参数高效微调方法训练稳定，能带来2-3个百分点的性能提升，而全模型更新容易过拟合和分布崩溃。

Conclusion: MTQE.en-he基准和实验结果将为英希这一资源匮乏语言对的机器翻译质量评估研究提供重要基础，参数高效微调方法在该任务中表现出良好的稳定性和有效性。
...
### [87] [Baichuan-M3: Modeling Clinical Inquiry for Reliable Medical Decision-Making](https://arxiv.org/abs/2602.06570)
...
Conclusion: Baichuan-M3成功实现了从被动问答到主动临床决策支持的范式转变，模型已公开可用
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型即使面对看似无害的提示也可能产生有毒或有害内容，这带来了严重的安全挑战。毒性通常微妙且依赖于上下文，难以在标记级别或通过粗粒度句子信号检测。此外，减轻毒性往往面临安全性与生成文本连贯性、流畅性之间的权衡。

Method: 提出了一种有针对性的子空间干预策略，用于从底层模型表示中识别和抑制隐藏的毒性模式，同时保持生成安全流畅内容的整体能力。

Result: 在RealToxicityPrompts数据集上，该方法相比现有基线实现了强大的毒性缓解性能，对推理复杂度影响最小。在多个LLM上，该方法将最先进解毒系统的毒性降低了8-20%，同时保持可比的流畅性。
...
### [89] [FairJudge: An Adaptive, Debiased, and Consistent LLM-as-a-Judge](https://arxiv.org/abs/2602.06625)
...
Method: 1) 将评判行为建模为可学习和正则化的策略；2) 从数据为中心的角度构建高信息密度评判数据集，显式注入与评估行为对齐的监督信号；3) 采用课程式SFT-DPO-GRPO训练范式，逐步对齐评分标准遵守、偏见缓解和跨模式一致性，同时避免灾难性遗忘

Result: 在多个内部和公共基准测试中，FairJudge持续提高了协议一致性和F1分数，减少了非语义偏见，并且显著优于更大的指令调优LLM

Conclusion: FairJudge通过将评判行为建模为可学习策略，解决了现有LLM-as-a-Judge系统的关键局限性，实现了自适应、去偏和一致的评估，所有资源将在接受后公开发布以促进未来研究
...
### [90] [Reading Between the Waves: Robust Topic Segmentation Using Inter-Sentence Audio Features](https://arxiv.org/abs/2602.06647)
*Steffen Freisinger,Philipp Seeberger,Tobias Bocklet,Korbinian Riedhammer*

Main category: cs.CL

TL;DR: 本文提出了一种多模态主题分割方法，通过联合微调文本编码器和孪生音频编码器来捕捉句子边界周围的声学线索，显著优于纯文本和多模态基线方法。
...
Method: 提出多模态方法，联合微调文本编码器和孪生音频编码器，捕捉句子边界周围的声学线索，用于主题分割。
...
### [91] [Beyond Static Alignment: Hierarchical Policy Control for LLM Safety via Risk-Aware Chain-of-Thought](https://arxiv.org/abs/2602.06650)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM面临安全性与有用性的基本权衡问题，因为静态的、一刀切的安全策略缺乏运行时可控性，难以适应不同应用场景的需求，导致模型可能过度拒绝良性请求或对有害请求约束不足。

Method: 提出PACT框架，采用分层策略架构：不可覆盖的全局安全策略为关键风险（如儿童安全、暴力极端主义）建立不可变边界；用户定义策略可引入领域特定的非全局风险类别，并指定标签到行为的映射。框架将安全决策分解为结构化的分类→行动路径，将查询路由到适当的行动（遵守、引导或拒绝），并使决策过程透明化。

Result: 大量实验表明，PACT在全局策略评估下实现了接近最先进的安全性能，同时在用户特定策略评估下获得了最佳的可控性，有效缓解了安全性与有用性之间的权衡问题。

Conclusion: PACT框架通过动态安全控制和显式风险感知推理，为LLM提供了更好的安全可控性，解决了传统静态安全策略的局限性，并计划发布完整的模型套件、训练数据和评估协议以促进可复现研究。
...
### [92] [Evaluating Prompt Engineering Strategies for Sentiment Control in AI-Generated Texts](https://arxiv.org/abs/2602.06692)
*Kerstin Sahler,Sophie Jentzsch*

Main category: cs.CL

TL;DR: 该研究探讨了如何通过提示工程控制大语言模型生成文本的情感倾向，发现提示工程是调整AI生成文本情感的有效且经济的方法，特别是在数据受限的环境中。


<details>
  <summary>Details</summary>
Motivation: 大语言模型为情感自适应AI系统提供了新机会，但精确控制这些系统中的情感倾向仍然具有挑战性。本研究旨在探索提示工程作为资源敏感且易于访问的替代方案，以控制LLM生成文本的情感。

Method: 使用Ekman的六种基本情感（如喜悦、厌恶），研究了多种提示技术，包括零样本提示和思维链提示（使用gpt-3.5-turbo），并与微调方法进行比较。

Result: 提示工程能有效引导AI生成文本中的情感，为微调提供了实用且经济高效的替代方案。在多种技术中，使用人工编写示例的少样本提示最为有效，可能是因为提供了额外的任务特定指导。

Conclusion: 研究结果为开发情感自适应AI系统提供了有价值的见解，提示工程特别是在数据受限环境中是控制LLM生成文本情感的有效方法。
...
### [93] [R-Align: Enhancing Generative Reward Models through Rationale-Centric Meta-Judging](https://arxiv.org/abs/2602.06763)
...
TL;DR: 该论文提出了一种新的评估方法S-Corr来衡量生成奖励模型(GenRM)的推理保真度，并开发了R-Align方法来提升推理一致性，从而改善RLHF效果。


<details>
  <summary>Details</summary>
Motivation: 当前生成奖励模型(GenRM)的训练和评估主要关注标签准确性，而忽视了推理质量。研究发现推理保真度（模型偏好决策与参考决策理由的一致性）对下游RLHF结果有重要影响，但现有方法无法有效评估这一指标。

Method: 提出了Spurious Correctness (S-Corr)指标来衡量标签正确但推理错误的决策比例。开发了Rationale-Centric Alignment (R-Align)方法，通过加入黄金判断并显式监督推理对齐来提升推理保真度。

Result: 实证评估显示即使是竞争性GenRM也存在显著的S-Corr问题，且S-Corr越高，优化过程中的策略退化越严重。R-Align方法在多个基准上降低了S-Corr，并在STEM、编码、指令遵循和通用任务上带来了持续的性能提升。

Conclusion: 推理保真度是评估生成奖励模型质量的关键指标，R-Align方法通过显式监督推理对齐有效提升了模型的推理一致性，从而改善了RLHF的整体效果。
...
### [94] [Generating Data-Driven Reasoning Rubrics for Domain-Adaptive Reward Modeling](https://arxiv.org/abs/2602.06795)
*Kate Sanders,Nathaniel Weir,Sapana Chaudhary,Kaj Bostrom,Huzefa Rangwala*

Main category: cs.CL

TL;DR: 提出数据驱动方法构建细粒度推理错误分类法，增强LLM在技术领域（编程、数学、化学工程）的错误检测能力，并用于强化学习奖励函数，显著提升模型性能
...
Method: 采用数据驱动方法自动构建高度细粒度的推理错误分类法（rubrics），利用这些分类法增强LLM驱动的错误检测，并构建更强的LLM-as-judge奖励函数用于推理模型的强化学习训练

Result: 基于错误分类法的分类方法在技术领域表现出比基线方法更强的错误识别能力；使用这些奖励函数训练的模型在困难领域的任务准确率比通用LLM-as-judge训练的模型提高+45%，仅使用20%的黄金标签就能接近使用可验证奖励训练的模型性能

Conclusion: 该方法将奖励分类法从评估定性模型行为扩展到评估定量模型正确性，为在没有完整黄金标签数据集的情况下教授模型解决复杂技术问题打开了大门，这些黄金标签通常获取成本很高
...
### [95] [Visual Word Sense Disambiguation with CLIP through Dual-Channel Text Prompting and Image Augmentations](https://arxiv.org/abs/2602.06799)
...
<details>
  <summary>Details</summary>
Motivation: 词汇歧义是大型语言模型在自然语言理解中面临的持续挑战，本文旨在探索如何通过视觉领域解决词汇歧义问题，开发一个可解释的视觉词义消歧框架。

Method: 利用CLIP将歧义语言和候选图像投影到共享多模态空间；通过双通道集成（语义提示和基于照片的提示）结合WordNet同义词丰富文本嵌入；通过鲁棒的测试时增强优化图像嵌入；使用余弦相似度确定与歧义文本最匹配的图像。

Result: 在SemEval-2023 VWSD数据集上，增强嵌入使MRR从0.7227提升到0.7590，命中率从0.5810提升到0.6220；消融研究表明双通道提示提供强大低延迟性能，而激进的图像增强仅带来边际收益；WordNet定义和多语言提示实验表明噪声外部信号会稀释语义特异性。

Conclusion: 精确的CLIP对齐提示对于视觉词义消歧最为有效，双通道提示策略在性能和效率之间取得了良好平衡，而过度依赖外部噪声信号会降低语义特异性。
...
### [96] [The Representational Geometry of Number](https://arxiv.org/abs/2602.06843)
*Zhimin Hu,Lanhao Niu,Sashank Varma*

Main category: cs.CL

TL;DR: 研究发现概念表征通过共享几何关系而非概念本身实现泛化与任务特化的平衡，以数字概念为例验证了任务特定表征位于不同子空间但可通过线性映射相互转换


<details>
  <summary>Details</summary>
Motivation: 认知科学中关于概念表征是收敛到共享流形支持泛化，还是发散到正交子空间最小化任务干扰存在争议，需要机制性解释这两种属性如何共存和转换

Method: 使用数字概念作为测试平台，语言模型作为高维计算基底，分析数字表征在不同任务中保持稳定关系结构，任务特定表征嵌入不同子空间，低层特征如大小和奇偶性编码为可分离的线性方向

Result: 发现这些子空间主要通过线性映射可相互转换，表明表征尽管位于不同子空间但共享关系结构，语言模型能够平衡数字表征的共享结构与功能灵活性

Conclusion: 理解产生于任务特定变换应用于概念表征的共享底层关系结构，为概念表征如何同时实现泛化和任务特化提供了机制性视角
...
### [97] [SEMA: Simple yet Effective Learning for Multi-Turn Jailbreak Attacks](https://arxiv.org/abs/2602.06854)
*Mingqian Feng,Xiaodong Liu,Weiwei Yang,Jialin Song,Xuekai Zhu,Chenliang Xu,Jianfeng Gao*

Main category: cs.CL

TL;DR: SEMA是一种无需依赖现有策略或外部数据的多轮越狱攻击框架，通过预填充自调优和意图漂移感知的强化学习，在多种受害者模型上实现了最先进的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的多轮越狱方法面临探索复杂性和意图漂移问题，而单轮攻击只是多轮攻击的特例。需要一种更有效、更现实的多轮攻击框架来测试LLM的安全性。

Method: SEMA包含两个阶段：1) 预填充自调优：通过微调自生成的非拒绝、结构良好的多轮对抗提示来稳定学习；2) 意图漂移感知的强化学习：训练攻击者生成有效的多轮对抗提示，同时保持相同的有害目标，使用结合意图对齐、合规风险和详细程度的奖励函数。

Result: 在多个数据集、受害者模型和越狱评判标准下，SEMA实现了最先进的攻击成功率（ASR），在AdvBench上对三个闭源和开源受害者模型平均达到80.1%的ASR@1，比现有最佳方法高出33.9%。
...
### [98] [Halluverse-M^3: A multitask multilingual benchmark for hallucination in LLMs](https://arxiv.org/abs/2602.06920)
*Samir Abdaljalil,Parichit Sharma,Erchin Serpedin,Hasan Kurban*

Main category: cs.CL

TL;DR: Halluverse-M³是一个多语言、多任务、多类别幻觉数据集，涵盖英语、阿拉伯语、印地语和土耳其语四种语言，支持问答和对话摘要两种生成任务，区分实体级、关系级和句子级幻觉，用于系统评估大语言模型的幻觉检测能力。
...
Method: 构建Halluverse-M³数据集，涵盖四种语言（英语、阿拉伯语、印地语、土耳其语）和两种生成任务（问答、对话摘要），明确区分实体级、关系级和句子级幻觉。通过受控编辑过程构建幻觉输出，并由人工标注者验证，确保原始内容与幻觉生成之间的清晰对齐。

Result: 评估结果显示：问答任务比对话摘要更容易；句子级幻觉即使对最强模型也具有挑战性；英语表现最佳，低资源语言性能下降，印地语的检测准确率最低。数据集为多语言多任务幻觉研究提供了现实且具有挑战性的基准。

Conclusion: Halluverse-M³为系统研究多语言、多任务环境中的幻觉问题提供了有价值的工具，揭示了不同语言、任务和幻觉类型间的性能差异，有助于推动幻觉检测和缓解技术的未来发展。
...
### [99] [Optimal Turkish Subword Strategies at Scale: Systematic Evaluation of Data, Vocabulary, Morphology Interplay](https://arxiv.org/abs/2602.06942)
*Duygu Altinok*

Main category: cs.CL

TL;DR: 该研究首次对土耳其语等形态丰富语言的子词分词进行了全面系统研究，通过控制词汇量和训练语料大小，比较不同分词器家族，并引入形态感知诊断工具包进行多维度评估。


<details>
  <summary>Details</summary>
Motivation: 土耳其语等形态丰富语言具有高度能产性的黏着特性，这对神经语言建模中的分词设计提出了挑战。现有研究通常存在三个问题：词汇量变化时未系统控制分词器训练语料、内在诊断有限、下游任务评估范围狭窄。

Method: 研究提出了"子词宣言"方法，联合变化词汇量和分词器训练语料大小（数据和词汇耦合），在匹配参数预算下比较多个分词器家族（WordPiece、形态级别和字符基线），并跨语义、句法和形态敏感探测任务进行评估。引入了形态感知诊断工具包，包括边界级微观/宏观F1、解耦的词干原子性与表面边界命中、过/欠分割指数、字符/词编辑距离、延续率、词缀类型覆盖率和词级原子性等指标。

Result: 研究提供了词汇-语料-成功三元组的系统调查，建立了连接内在诊断与外在结果的统一形态感知评估框架，确定了字符级和形态级分词何时有效，并开源了评估代码、分词器流水线和模型。

Conclusion: 该"子词宣言"为在形态丰富语言中构建有效分词器提供了可操作的指导，并为未来研究建立了可复现的基础。这是首个此类研究，填补了该领域的空白。
...
### [100] [DAWN: Dependency-Aware Fast Inference for Diffusion LLMs](https://arxiv.org/abs/2602.06953)
*Lizhuo Luo,Zhuoran Shi,Jiajun Luo,Zhi Wang,Shen Ren,Wenya Wang,Tianwei Zhang*

Main category: cs.CL

TL;DR: DAWN是一种无需训练、依赖感知的解码方法，用于加速扩散大语言模型推理，通过建模token依赖关系实现高效并行解码，在保持生成质量的同时实现1.80-8.06倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散大语言模型推理方案受限于质量-速度权衡，采用保守的并行策略，未能充分利用效率潜力。核心挑战在于并行解码假设每个位置可以独立填充，但实际上token之间存在语义耦合，一个位置的选择会约束其他位置的有效选择。不建模这些token间依赖关系会导致并行策略产生质量下降的输出。

Method: DAWN是一种无需训练的依赖感知解码方法，通过提取token依赖关系构建依赖图。基于两个关键观察：(1)依赖于已确定位置的位置变得更加可靠；(2)同时解码强耦合的不确定位置会引发错误。DAWN利用依赖图在每次迭代中选择更可靠的解码位置，实现高并行度且生成质量损失可忽略。

Result: 在多个模型和数据集上的广泛实验表明，DAWN相比基线方法实现了1.80-8.06倍的推理加速，同时保持了生成质量。代码已开源。

Conclusion: DAWN通过建模token依赖关系，解决了扩散大语言模型并行解码中的质量-速度权衡问题，实现了高效且高质量的推理加速，为扩散大语言模型的实用化部署提供了有效解决方案。
...
### [101] [InftyThink+: Effective and Efficient Infinite-Horizon Reasoning via Reinforcement Learning](https://arxiv.org/abs/2602.06960)
*Yuchen Yan,Liang Jiang,Jin Jiang,Shuaicheng Li,Zujie Wen,Zhiqiang Zhang,Jun Zhou,Jian Shao,Yueting Zhuang,Yongliang Shen*

Main category: cs.CL

TL;DR: InftyThink+是一个端到端强化学习框架，通过优化迭代推理轨迹来解决长链思维推理中的成本、长度限制和中间信息丢失问题，显著提升推理效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有大规模推理模型通过扩展推理时的思维链获得强大性能，但面临二次成本、上下文长度限制以及"中间信息丢失"效应导致的推理质量下降。迭代推理通过定期总结中间思考来缓解这些问题，但现有方法依赖监督学习或固定启发式规则，无法优化何时总结、保留什么以及如何继续推理。

Method: 提出InftyThink+端到端强化学习框架，基于模型控制的迭代边界和显式总结来优化整个迭代推理轨迹。采用两阶段训练方案：监督冷启动后接轨迹级强化学习，使模型学习战略性总结和继续决策。

Result: 在DeepSeek-R1-Distill-Qwen-1.5B上的实验显示，InftyThink+在AIME24上准确率提升21%，明显优于传统长链思维强化学习方法，同时在分布外基准测试中泛化能力更强。此外，InftyThink+显著降低推理延迟并加速强化学习训练。

Conclusion: InftyThink+通过优化迭代推理轨迹，在提升推理性能的同时显著提高推理效率，为解决长链思维推理中的成本、长度和中间信息丢失问题提供了有效方案。
...
### [102] [ALIEN: Analytic Latent Watermarking for Controllable Generation](https://arxiv.org/abs/2602.06101)
*Liangqi Lei,Keke Gai,Jing Yu,Qi Wu*

Main category: eess.IV

TL;DR: ALIEN框架通过解析推导时间相关调制系数，实现可控水印嵌入，在质量和鲁棒性上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖计算密集的启发式优化进行迭代信号优化，导致训练开销大且容易陷入局部最优，需要更高效可控的水印嵌入方案。

Method: 提出ALIEN框架，首次解析推导时间相关调制系数，指导水印残差的扩散过程，实现可控的水印嵌入模式。

Result: ALIEN-Q在5个质量指标上比SOTA提升33.1%，ALIEN-R在15种不同条件下对生成变体和稳定性威胁的鲁棒性比SOTA提升14.0%。

Conclusion: ALIEN通过解析方法解决了传统优化方法的局限性，在保持高质量生成的同时显著提升了水印的鲁棒性和可控性。
...
### [103] [Content-Driven Frame-Level Bit Prediction for Rate Control in Versatile Video Coding](https://arxiv.org/abs/2602.06242)
*Amritha Premkumar,Prajit T Rajendran,Vignesh V Menon,Christian Herglotz*

Main category: eess.IV

TL;DR: 提出基于VCA轻量特征和随机森林回归的内容自适应比特预测框架，替代传统VVC两遍码率控制，在保持编码效率的同时减少33.3%编码时间


<details>
  <summary>Details</summary>
Motivation: 传统VVC两遍码率控制依赖解析率-QP模型，难以捕捉非线性时空变化，导致质量不稳定且复杂度高（多次试编码）

Method: 使用视频复杂度分析器（VCA）提取轻量特征，结合量化参数，通过随机森林回归预测帧级比特消耗

Result: 在VVenC编码的超高清序列上，模型与真实值强相关：I帧R²=0.93，P帧R²=0.88，B帧R²=0.77；集成到码率控制环中，编码效率与传统方法相当，总编码时间减少33.3%

Conclusion: VCA驱动的比特预测为传统率-QP模型提供了计算高效且准确的替代方案
...
### [104] [Zero-shot Multi-Contrast Brain MRI Registration by Intensity Randomizing T1-weighted MRI (LUMIR25)](https://arxiv.org/abs/2602.06292)
*Hengjie Liu,Yimeng Dou,Di Xu,Xinyi Fu,Dan Ruan,Ke Sheng*

Main category: eess.IV

TL;DR: 该论文总结了在LUMIR25挑战赛中取得第一名的零样本医学图像配准方法，该方法在仅使用T1加权MRI训练的情况下，能够泛化到多种MRI对比度的图像配准任务。


<details>
  <summary>Details</summary>
Motivation: LUMIR25挑战赛要求解决零样本配准问题，即在训练数据仅包含T1加权脑部MRI的情况下，模型需要泛化到高场强MRI、病理脑部以及各种MRI对比度的图像配准任务。这需要开发能够处理领域偏移的鲁棒配准方法。

Method: 作者首先分析了LUMIR24获胜方案的关键因素，然后提出了三种简单有效的策略：1）基于模态无关邻域描述符（MIND）的多模态损失函数；2）强度随机化的外观增强技术；3）推理时在特征编码器上进行轻量级的实例特定优化（ISO）。

Result: 该方法在LUMIR25挑战赛测试集上取得了第一名。在验证集上，实现了合理的T1-T2配准精度，同时保持了良好的形变规律性。

Conclusion: 通过结合MIND多模态损失、强度随机化增强和实例特定优化这三种策略，能够在仅使用T1加权MRI训练的情况下，实现跨多种MRI对比度的零样本图像配准，展现出良好的泛化能力。
...
### [105] [Orientation-Robust Latent Motion Trajectory Learning for Annotation-free Cardiac Phase Detection in Fetal Echocardiography](https://arxiv.org/abs/2602.06761)
*Yingyu Yang,Qianye Yang,Can Peng,Elena D'Alberti,Olga Patey,Aris T. Papageorghiou,J. Alison Noble*

Main category: eess.IV

TL;DR: ORBIT是一个自监督框架，用于从胎儿超声心动图视频中自动检测心脏舒张末期和收缩末期相位，无需手动标注，且对胎儿心脏方向具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 胎儿超声心动图对检测先天性心脏病至关重要，但缺乏胎儿心电图时，手动识别舒张末期和收缩末期帧是劳动密集型的瓶颈。现有无标注方法受限于固定的心脏方向假设，需要一种对方向鲁棒的自动化解决方案。

Method: ORBIT采用自监督框架，将配准作为自监督任务，学习心脏变形的潜在运动轨迹。通过捕捉心脏松弛和收缩之间的转折点，实现对舒张末期和收缩末期帧的准确定位，且对胎儿心脏方向具有鲁棒性。

Result: 在仅使用正常胎儿超声心动图视频训练的情况下，ORBIT在正常病例（舒张末期MAE=1.9帧，收缩末期MAE=1.6帧）和先天性心脏病病例（舒张末期MAE=2.4帧，收缩末期MAE=2.1帧）上均表现一致，优于受固定方向假设约束的现有无标注方法。

Conclusion: ORBIT展示了直接从四腔心胎儿超声心动图中实现鲁棒心脏相位检测的潜力，有助于推动完全自动化的先天性心脏病分析。
...
TL;DR: 该研究首次系统性地探讨了将多模态扩散语言模型转换为嵌入模型的可行性，发现其嵌入性能普遍低于自回归视觉语言模型，主要原因是扩散模型在图像-文本对齐方面存在不足。


<details>
  <summary>Details</summary>
Motivation: 随着大型扩散语言模型作为自回归模型的竞争替代方案出现，具有双向注意力和并行生成等优势，自然引发了一个关键但尚未探索的问题：多模态扩散语言模型能否作为有效的多模态嵌入模型？

Method: 首次系统研究将多模态扩散语言模型转换为嵌入模型的方法，评估了最先进的多模态扩散语言模型和自回归视觉语言模型在分类、视觉问答和信息检索三类嵌入任务上的表现。

Result: 多模态扩散语言模型的嵌入性能普遍低于自回归视觉语言模型。较强的扩散模型LaViDa在分类任务上落后3.5分，视觉问答落后2.5分，检索任务落后4.4分；而另一扩散模型MMaDA在所有任务上的性能差距超过20分。

Conclusion: 扩散模型在图像-文本对齐方面的不足是导致其嵌入性能受限的主要原因，多模态扩散语言模型目前还不能作为有效的多模态嵌入模型替代方案。
...
TL;DR: Jackpot框架通过最优预算拒绝采样(OBRS)减少rollout模型与策略之间的分布不匹配，实现RL训练中rollout生成与策略优化的解耦，显著提升训练效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的强化学习成本高昂，主要因为rollout过程开销大。解耦rollout生成与策略优化（如使用更高效模型进行rollout）能显著提升效率，但会引入严重的分布不匹配问题，导致学习不稳定。

Method: 提出Jackpot框架，采用最优预算拒绝采样(OBRS)直接减少rollout模型与演化策略之间的差异。框架包含：1）原则性的OBRS过程；2）联合更新策略和rollout模型的统一训练目标；3）通过top-k概率估计和批次级偏差校正实现的高效系统实现。

Result: 理论分析表明OBRS能在可控接受预算下持续使rollout分布更接近目标分布。实验显示，相比重要性采样基线，Jackpot显著提升训练稳定性，在Qwen3-8B-Base模型上训练300个更新步骤（批次大小64）时，性能达到与on-policy RL相当的水平。

Conclusion: 基于OBRS的对齐方法使RL训练中rollout生成与策略优化的解耦更接近实用和有效，为大型语言模型的强化学习提供了更高效的训练框架。
...
TL;DR: 该论文首次全面调查大语言模型中的推理失败问题，提出了新的分类框架，将推理分为具身和非具身类型，并将推理失败分为基础架构失败、应用特定限制和鲁棒性问题三类。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在各种任务中表现出色，但在看似简单的场景中仍存在显著的推理失败。为了系统性地理解和解决这些缺陷，需要进行全面的调查研究。

Method: 提出了一个新颖的分类框架：将推理分为具身推理和非具身推理，非具身推理进一步细分为非正式（直觉）推理和正式（逻辑）推理。同时，将推理失败分为三类：影响下游任务的基础架构失败、特定领域表现的应用限制、以及微小变化导致性能不一致的鲁棒性问题。

Result: 对每种推理失败提供了清晰定义，分析了现有研究，探讨了根本原因，并提出了缓解策略。通过整合分散的研究工作，为LLM推理的系统性弱点提供了结构化视角。

Conclusion: 该调查为理解LLM推理失败提供了系统框架，指导未来研究构建更强、更可靠、更鲁棒的推理能力。同时发布了GitHub资源库，为该领域提供便捷入口。
...
TL;DR: 提出基于LTLfMT逻辑框架的非马尔可夫奖励规范方法，结合奖励机器和HER解决无限状态空间MDP中的复杂任务规划问题


<details>
  <summary>Details</summary>
Motivation: 传统马尔可夫决策过程在处理大规模状态空间和非马尔可夫奖励时表达能力有限，需要手动编码谓词，缺乏统一的规范框架

Method: 1) 使用LTLfMT（基于理论的线性时序逻辑）增强表达能力；2) 识别可处理的LTLfMT片段；3) 结合奖励机器和Hindsight Experience Replay (HER)方法；4) 在非线性算术理论中实现连续控制

Result: 实验结果表明：1) LTLfMT能够自然规范复杂任务；2) 定制的HER实现对于解决复杂目标任务至关重要；3) 框架支持在非结构化异构数据域中指定复杂任务

Conclusion: 提出的LTLfMT框架为无限状态空间MDP中的非马尔可夫奖励规范提供了统一、可重用的解决方案，通过理论分析和实践方法解决了表达能力和计算复杂性之间的平衡问题
...
TL;DR: GrAlgoBench是一个新的图算法基准测试，用于评估大型推理模型，解决了现有基准测试在长上下文评估、挑战性和可编程验证方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有数学、代码和常识推理基准测试存在三个主要问题：缺乏长上下文评估、挑战性不足、答案难以通过程序验证。图算法问题特别适合评估推理能力，因为它们需要长上下文推理、允许精细控制难度、支持标准化程序化评估。

Method: 引入GrAlgoBench基准测试，包含九个图算法任务，用于系统评估大型推理模型。通过控制图的大小（节点数量）来调节上下文长度和难度。

Result: 实验揭示了当前大型推理模型的两个主要弱点：1）随着上下文长度增加，准确率急剧下降（当图超过120个节点时，准确率低于50%），主要由执行错误、弱记忆和冗余推理导致；2）存在过度思考现象，主要是大量但低效的自我验证导致推理轨迹膨胀而不提高正确性。

Conclusion: GrAlgoBench通过暴露大型推理模型的局限性，确立了图算法问题作为一个严谨、多维且实际相关的测试平台，用于推进大型推理模型中的推理研究。
...
TL;DR: DEPO框架通过在线难度估计器动态筛选训练数据，优先计算高学习潜力样本，在保持模型性能的同时将推理成本降低2倍


<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法在遇到过于简单或复杂的问题时会出现梯度信号衰减问题，而DAPO等变体虽然解决了梯度消失但计算开销巨大。需要一种既能保持收敛稳定性又能降低计算成本的新方法。

Method: 提出Difficulty-Estimated Policy Optimization (DEPO)框架，集成在线难度估计器，在rollout阶段前动态评估和筛选训练数据，优先将计算资源分配给具有高学习潜力的样本。

Result: 实验结果表明，DEPO在保持模型性能的同时，将rollout成本降低了高达2倍，显著降低了训练高性能推理模型的计算门槛。

Conclusion: DEPO为推理对齐提供了更高效和鲁棒的优化框架，为推理扩展提供了更可持续的路径，代码和数据将在接受后发布。
...
TL;DR: QA-Token是一种质量感知的分词方法，通过考虑数据可靠性来优化词汇表构建，在基因组学和金融领域显著提升下游任务性能，同时减少token数量。


<details>
  <summary>Details</summary>
Motivation: 当前的分词方法在处理序列数据时未考虑信号质量，限制了其在噪声现实世界语料库上的有效性。需要一种能够直接整合数据可靠性的分词方法。
...
Conclusion: QA-Token解锁了噪声现实世界语料库（包括数petabases的基因组序列和数TB的金融时间序列），用于基础模型训练，且推理时无额外开销。
...
TL;DR: 论文提出自回归生成存在内在稳定性极限，导致大语言模型在长视野任务中性能急剧下降，这源于过程级不稳定性而非单纯任务复杂度，需要离散分段和图状执行结构来解决。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在长视野任务中性能急剧下降，传统解释归因于任务复杂度，但作者认为这些解释不完整，即使在无分支的线性任务中，自回归执行也存在内在稳定性极限。

Method: 提出自回归生成过程级不稳定性的理论框架，推导定理A证明单路径自回归推理中的决策优势随执行长度指数衰减，通过合成环境和真实TextWorld任务的实证研究验证理论预测。

Result: 实证研究显示可观察到的性能悬崖与理论预测一致，表明长视野推理失败源于自回归架构的结构性限制，短视野评估协议可能掩盖结构不稳定性。

Conclusion: 长视野推理的根本约束是自回归生成的过程级不稳定性，需要离散分段和图状执行结构，未来推理系统可能需要从单纯扩展转向结构化治理。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的智能体系统严重依赖大规模模型，而边缘规模模型（如4B参数）的潜力尚未充分探索。本文旨在解决边缘规模模型面临的三个主要瓶颈：监督微调中的灾难性遗忘、强化学习中对奖励信号噪声的敏感性，以及长上下文场景中冗余信息导致的推理退化。

Method: 提出了AgentCPM-Explore模型，这是一个具有高知识密度和强探索能力的4B参数智能体模型。采用整体训练框架，包括：1）参数空间模型融合；2）奖励信号去噪；3）上下文信息精炼。通过深度探索策略提升模型性能。

Result: AgentCPM-Explore在4B级别模型中达到最先进性能，在四个基准测试中匹配或超越8B级别SOTA模型，在五个基准测试中甚至优于Claude-4.5-Sonnet或DeepSeek-v3.2等更大规模模型。在GAIA文本任务上达到97.09%的准确率（pass@64）。
...
### [115] [JADE: Expert-Grounded Dynamic Evaluation for Open-Ended Professional Tasks](https://arxiv.org/abs/2602.06486)
...
<details>
  <summary>Details</summary>
Motivation: 当前评估智能体AI在开放式专业任务上面临两难困境：静态评估标准虽然严谨可重复，但无法适应多样化的有效响应策略；而基于LLM的评估方法虽然能适应个体响应，但存在不稳定性和偏见问题。

Method: JADE采用两层评估框架：第一层将专家知识编码为预定义的评估技能，提供稳定的评估标准；第二层执行报告特定的、基于声明的评估，灵活评估多样化的推理策略，并通过证据依赖门控机制来使基于被反驳声明的结论无效。

Result: 在BizBench上的实验表明，JADE提高了评估稳定性，并揭示了整体性LLM评估器遗漏的关键智能体失败模式。该框架与专家编写的评估标准高度一致，并能有效迁移到医学领域基准测试中。

Conclusion: JADE框架通过结合静态评估标准的严谨性和动态评估的灵活性，有效解决了开放式专业任务评估中的两难困境，在不同专业领域都表现出良好的验证效果。
...
TL;DR: HyPER提出了一种动态扩展-缩减控制策略，用于多路径思维链推理，通过在线控制器平衡探索与利用，在固定计算预算下显著提升推理准确性并减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有多路径推理方法在探索-利用权衡上存在局限：树状搜索通过脆弱的扩展规则硬编码探索，干扰后训练推理；并行推理则过度探索冗余假设路径且答案选择能力弱。研究发现最优平衡是阶段依赖的，正确与错误推理路径往往在后期才分叉。

Method: 将测试时扩展重新表述为假设池上的动态扩展-缩减控制问题。提出HyPER：1）在线控制器根据假设池演化从探索转向利用；2）令牌级细化机制实现无需完整路径重采样的高效生成时利用；3）长度和置信度感知的聚合策略实现可靠的答案时利用。

Result: 在四个专家混合语言模型和多样化推理基准测试中，HyPER始终实现更优的准确性-计算权衡，准确率提升8-10%，同时令牌使用量减少25-40%。
...
<details>
  <summary>Details</summary>
Motivation: 当前深度研究报告生成系统严重依赖闭源或在线大模型，存在部署障碍、安全隐私问题，且现有plan-then-write范式过度依赖初始大纲质量，而构建全面大纲本身需要强大推理能力。

Method: 提出AgentCPM-Report系统，包含：1) Writing As Reasoning Policy框架，模仿人类写作过程，允许动态修订大纲；2) 8B参数深度研究智能体，交替进行Evidence-Based Drafting和Reasoning-Driven Deepening；3) Multi-Stage Agentic Training策略，包括冷启动、原子技能强化学习和整体管道强化学习。

Result: 在DeepResearch Bench、DeepConsult和DeepResearch Gym基准测试中，AgentCPM-Report超越了领先的闭源系统，在Insight指标上取得显著提升。

Conclusion: AgentCPM-Report提供了一个轻量级、高性能的本地化解决方案，通过动态大纲修订和多阶段训练策略，使小模型能够生成高质量的深度研究报告，解决了对闭源大模型的依赖问题。
...
### [118] [Same Answer, Different Representations: Hidden instability in VLMs](https://arxiv.org/abs/2602.06652)
...
TL;DR: 本文提出了一种新的VLM鲁棒性评估框架，发现传统基于输出不变性的评估方法存在不足，揭示了VLM在内部表示漂移、规模不改善鲁棒性、以及扰动对不同任务有不同影响等三种失效模式。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型（VLM）的鲁棒性评估主要基于输出层面的不变性，即假设稳定的预测反映了稳定的多模态处理过程。然而，作者认为这种假设是不充分的，需要更深入地评估模型内部表示的变化。

Method: 提出了一个表示感知和频率感知的评估框架，测量内部嵌入漂移、频谱敏感性和结构平滑性（视觉token的空间一致性），同时结合标准的基于标签的指标。该框架应用于现代VLM，并在SEEDBench、MMMU和POPE数据集上进行测试。

Result: 发现了三种不同的失效模式：1）模型经常在预测答案保持不变的情况下经历显著的内部表示漂移；2）鲁棒性不随模型规模提升而改善，更大的模型虽然准确率更高但表现出同等或更大的敏感性；3）扰动对不同任务有不同影响：在推理任务中会损害性能，但在幻觉基准测试中可能通过让模型生成更保守的答案来减少误报。

Conclusion: 传统基于输出不变性的VLM鲁棒性评估方法存在局限性，需要更全面的评估框架来揭示模型内部表示的变化。模型规模的增加并不必然带来鲁棒性的提升，不同任务对扰动的敏感性存在差异，这为未来VLM的鲁棒性改进提供了重要见解。
...
### [119] [Autoregressive Models for Knowledge Graph Generation](https://arxiv.org/abs/2602.06707)
*Thiviyan Thanapalasingam,Antonis Vozikis,Peter Bloem,Paul Groth*

Main category: cs.AI

TL;DR: ARK模型通过自回归方式将知识图谱生成为三元组序列，学习隐含的语义约束，在IntelliGraphs基准上达到89.2%-100%的语义有效性，同时能生成训练中未见的新图谱。
...
Method: 提出ARK（自回归知识图谱生成）模型家族，通过将图谱视为(head, relation, tail)三元组序列来自回归生成知识图谱。模型从数据中直接学习隐含的语义约束（类型一致性、时间有效性、关系模式），无需显式规则监督。还引入SAIL作为ARK的变分扩展，通过学习的潜在表示实现可控生成。

Result: 在IntelliGraphs基准上，模型在多样化数据集上达到89.2%到100.0%的语义有效性，同时能生成训练中未见的新图谱。分析显示模型容量（隐藏维度≥64）比架构深度对KG生成更为关键，循环架构在实现与基于Transformer的替代方案相当有效性的同时提供显著的计算效率。
...
### [120] [Semantically Labelled Automata for Multi-Task Reinforcement Learning with LTL Instructions](https://arxiv.org/abs/2602.06746)
*Alessandro Abate,Giuseppe De Giacomo,Mathias Jackermeier,Jan Kretínský,Maximilian Prokop,Christoph Weinhuber*

Main category: cs.AI

TL;DR: 该论文提出了一种基于语义LTL到自动机转换的多任务强化学习方法，能够处理复杂的线性时序逻辑规范，并在多种领域实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 研究多任务强化学习，目标是学习一个能够泛化到任意（包括未见过的）任务的通用策略。使用线性时序逻辑（LTL）公式来指定任务，这是形式化方法中常用的规范语言，最近在强化学习中得到了成功应用。

Method: 提出了一种新颖的任务嵌入技术，利用新一代语义LTL到自动机的转换方法（最初为时序综合开发）。生成的语义标记自动机在每个状态包含丰富的结构化信息，能够：(i) 高效地在线计算自动机，(ii) 提取用于调节策略的表达性任务嵌入，(iii) 自然地支持完整的LTL规范。

Result: 在多种领域的实验结果表明，该方法实现了最先进的性能，并且能够扩展到现有方法无法处理的复杂规范。

Conclusion: 基于语义LTL到自动机转换的任务嵌入技术为多任务强化学习提供了一种有效的解决方案，能够处理复杂的时序逻辑规范并实现良好的泛化性能。
...
### [121] [Wild Guesses and Mild Guesses in Active Concept Learning](https://arxiv.org/abs/2602.06818)
*Anirudh Chari,Neil Pattanaik*

Main category: cs.AI

TL;DR: 研究对比了理性主动学习器（最大化期望信息增益）和人类式积极测试策略在概念学习中的表现，发现后者在简单概念上表现更好，这可能是因为它避免了支持不匹配陷阱，维持了假设生成的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究人类概念学习中的主动学习策略如何平衡查询的信息量和学习器的稳定性，特别是在神经符号贝叶斯学习框架下，探索理性主动学习与人类认知偏见的实际效果差异。

Method: 采用神经符号贝叶斯学习器，其中假设是由大型语言模型生成的可执行程序，通过贝叶斯更新重新加权。比较两种策略：理性主动学习器（最大化近似期望信息增益）和人类式积极测试策略（查询当前最佳假设预测为正的实例）。在经典数字游戏的概念学习任务中进行测试。

Result: 期望信息增益策略在需要证伪的复杂概念（如复合规则或例外规则）中有效，但在简单概念上表现不佳。这是因为高度诊断性的边界查询导致后验分布偏向生成器产生无效或过于特定程序的区域，形成粒子近似中的支持不匹配陷阱。积极测试策略虽然信息次优，但通过选择"安全"查询维持了生成有效性，在简单规则上收敛更快。

Conclusion: "确认偏见"可能不是认知错误，而是在人类思维特有的稀疏、开放式假设空间中维持可处理推理的理性适应策略。积极测试策略通过避免支持不匹配陷阱，在简单概念学习中表现更优。
...
### [122] [ScaleEnv: Scaling Environment Synthesis from Scratch for Generalist Interactive Tool-Use Agent Training](https://arxiv.org/abs/2602.06820)
...
Result: 在未见的多轮工具使用基准（如τ²-Bench和VitaBench）上表现出显著性能提升，展示了强大的泛化能力，并提供了环境多样性扩展与模型泛化性能关系的实证证据。
...
### [123] [POP: Online Structural Pruning Enables Efficient Inference of Large Foundation Models](https://arxiv.org/abs/2602.06822)
...
<details>
  <summary>Details</summary>
Motivation: 当前的结构化剪枝方法在推理时使用固定的剪枝决策，忽略了自回归token生成过程中出现的稀疏模式。大型基础模型通过扩展获得强大性能，但需要更高效的推理方法。
...
Conclusion: POP是一种轻量级即插即用的在线结构化剪枝框架，能够实现上下文条件化的动态剪枝，在保持高精度的同时显著减少计算开销，适用于各种大型基础模型的高效推理。
...
TL;DR: 提出自适应差分隐私联邦学习框架，解决设备异构、非IID数据和隐私约束下的训练不稳定和性能下降问题


<details>
  <summary>Details</summary>
Motivation: 实际联邦学习部署中，设备异构性和非IID数据导致梯度更新不稳定且存在偏差，而差分隐私的固定梯度裁剪和高斯噪声注入会进一步放大梯度扰动，造成训练振荡和性能下降
...
Conclusion: 提出的自适应差分隐私联邦学习框架能有效应对异构和隐私约束环境下的挑战，提升模型效率和性能
...
TL;DR: 该研究比较了静态预测解释与智能体系统解释方法，发现传统特征归因方法在静态分类中有效，但在智能体多步决策场景中不可靠，需要转向轨迹层面的诊断方法。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型发展，AI系统从静态预测转向多步决策的智能体系统。传统解释方法主要针对静态预测，不清楚如何适用于智能体场景中随时间展开的行为序列。需要弥合静态与智能体可解释性之间的差距。

Method: 比较静态分类任务中的归因解释方法与智能体基准（TAU-bench Airline和AssistantBench）中的轨迹诊断方法。使用统一的XAI评估框架进行实证比较。

Result: 归因方法在静态设置中特征排序稳定（Spearman ρ=0.86），但无法可靠诊断智能体轨迹中的执行级故障。轨迹基础的评价方法能持续定位行为故障，发现状态跟踪不一致在失败运行中高2.7倍，降低成功概率49%。

Conclusion: 需要从静态预测解释转向轨迹级可解释性，以评估和诊断自主AI行为。智能体系统需要新的解释方法来理解多步决策序列中的成功与失败。
...
### [126] [Insider Purchase Signals in Microcap Equities: Gradient Boosting Detection of Abnormal Returns](https://arxiv.org/abs/2602.06198)
*Hangyi Zhao*

Main category: q-fin.ST

TL;DR: SEC Form 4内幕交易购买申报能预测美国微型股异常收益，机器学习模型在2024年样本外数据上AUC达0.70，距离52周高点的距离是最重要特征，动量模式优于均值回归直觉。


<details>
  <summary>Details</summary>
Motivation: 研究SEC Form 4内幕购买申报是否能够预测美国微型股的异常收益，特别是在市场资本规模较小、流动性较差的股票中，内幕交易信号可能具有更强的预测能力。

Method: 使用梯度提升分类器分析2018-2024年17,237笔公开市场购买交易，涉及1,343家发行人（市值3,000万至5亿美元）。模型基于内幕交易者身份、交易历史和披露时的市场条件等特征进行训练。

Result: 模型在2024年样本外数据上AUC为0.70，优化阈值0.20时精确度为0.38，召回率为0.69。距离52周高点的距离是最重要特征（占预测信号的36%）。价格涨幅超过10%后披露的交易产生最高平均累计异常收益（6.3%）和最高超额表现概率（36.7%）。

Conclusion: 内幕购买申报能有效预测微型股异常收益，动量模式优于均值回归直觉，表明在流动性较差的市场中，趋势确认可能过滤出更高信心的内幕信号，信息吸收速度较慢。
