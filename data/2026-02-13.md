...
TL;DR: 本文开发了一个基于深度学习模型的茶叶病害自动分类系统，使用TeaLeafBD数据集，通过对抗训练和数据增强等技术，实现了对7种茶叶健康状况（6种病害+健康叶片）的高精度分类。


<details>
  <summary>Details</summary>
Motivation: 茶叶是孟加拉国的重要经济作物，但茶叶叶片容易受到各种病害感染，导致产量下降和质量降低。传统人工检测方法耗时且容易出错，因此需要开发自动化检测系统来提高病害识别的效率和准确性。

Method: 研究采用深度学习模型进行茶叶病害分类，使用包含5,278张高分辨率图像的TeaLeafBD数据集。方法包括数据预处理、数据分割、对抗训练、数据增强、模型训练和评估。采用DenseNet201和EfficientNetB3两种模型，并应用可解释AI策略（如Grad-CAM可视化）分析模型预测。

Result: 实验结果显示，EfficientNetB3模型取得了最高的分类准确率93%，而DenseNet201模型达到91%的准确率。对抗训练使模型在面对噪声或干扰输入时仍能有效工作，Grad-CAM可视化帮助识别了影响模型预测的关键图像区域。

Conclusion: 提出的基于深度学习的茶叶病害分类方法能够准确检测茶叶叶片病害，为现代农业管理提供了实用的解决方案。该方法具有较高的分类准确率，且通过可解释AI技术增强了模型的透明度和可信度。
...
TL;DR: Active-Zero框架通过主动探索视觉环境，让视觉语言模型能够自主构建学习轨迹，相比被动交互的自我博弈方法显著提升了性能


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型自我博弈方法依赖于与静态图像集的被动交互，导致对初始数据集的强依赖和学习效率低下。模型无法主动寻找适合其当前能力水平的视觉数据，浪费计算资源在过于简单或过于困难的任务上

Method: 提出Active-Zero框架，包含三个协同进化的智能体：1) Searcher从开放世界存储库中根据模型能力边界检索图像；2) Questioner合成校准的推理任务；3) Solver通过准确性奖励进行精炼。形成闭环的自适应课程学习系统

Result: 在Qwen2.5-VL-7B-Instruct模型上，在12个基准测试中，推理任务平均准确率达到53.97%（提升5.7%），通用理解任务达到59.77%（提升3.9%），持续优于现有的自我博弈基线方法

Conclusion: 主动探索是实现可扩展和自适应自我进化视觉语言系统的关键要素，Active-Zero框架通过主动环境探索和协同智能体设计，显著提升了视觉语言模型的自我学习能力
...
TL;DR: ReTracing是一个多智能体具身表演艺术项目，采用考古学方法研究人工智能如何塑造、约束和产生身体运动。通过科幻小说提取人机交互描述，使用LLM生成"做什么"和"不做什么"的提示，用扩散模型转化为人类表演者和四足机器人的动作指导，在镜像地板上表演并记录运动轨迹。


<details>
  <summary>Details</summary>
Motivation: 该项目旨在探索人工智能如何影响和塑造身体运动，揭示生成系统如何通过编排的动作编码社会文化偏见。通过AI、人类和机器人的沉浸式互动，探讨在AI也能移动、思考和留下痕迹的时代，作为人类意味着什么。

Method: 1. 从科幻小说中提取描述人机交互的句子；2. 使用大语言模型为每个摘录生成"做什么"和"不做什么"的配对提示；3. 使用基于扩散的文本到视频模型将这些提示转化为人类表演者和四足机器人的动作指导；4. 在镜像地板上进行表演，通过多摄像头运动追踪记录；5. 将运动数据重建为3D点云和运动轨迹，形成数字档案。

Result: 创建了一个数字运动痕迹档案，通过AI生成的动作编排展示了生成系统如何编码社会文化偏见。项目实现了人类表演者和机器人的协同表演，形成了沉浸式的艺术体验，为研究AI与身体运动的关系提供了新方法。

Conclusion: ReTracing提供了一种新颖的方法来揭示生成系统如何通过编排的动作编码社会文化偏见。这个项目不仅是一个艺术表演，更是一个研究框架，通过AI、人类和机器人的互动，探索在人工智能时代身体运动的意义和人类身份的本质。
...
TL;DR: 本文介绍了REVEAL诊断基准，通过五项压力测试揭示当前视频语言模型在理解视频内容、时序和运动方面的根本性弱点。


<details>
  <summary>Details</summary>
Motivation: 研究视频语言模型是否能够稳健地理解视频内容、时间序列和运动，发现现有模型在这方面存在严重缺陷，需要系统性的诊断工具来评估和改进。

Method: 开发REVEAL诊断基准，包含五项压力测试：时序期望偏差、语言捷径依赖、视频谄媚性、相机运动敏感性、时空遮挡鲁棒性。提供自动生成诊断示例的数据管道。

Result: 测试领先的开源和闭源视频语言模型发现，这些模型经常将反向场景描述为正向、忽略视频内容回答问题、同意错误主张、难以处理基本相机运动、在简单时空遮挡下无法聚合时序信息。人类在这些任务上表现优异。

Conclusion: 当前视频语言模型在理解视频内容、时序和运动方面存在根本性弱点，REVEAL基准为系统评估这些弱点提供了工具，有助于推动未来研究改进视频语言模型的能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统摄影测量方法（通过多视角2D照片重建3D模型）存在多种设计选择，但不同方法之间的差异主要依赖定性评估，缺乏可重复、可量化的实验框架
...
Conclusion: 提出的合成图像生成流程为摄影测量方法的性能评估提供了标准化、可量化的测试框架，能够客观比较不同数字孪生生成方法的差异
...
### [6] [Efficient Segment Anything with Depth-Aware Fusion and Limited Training Data](https://arxiv.org/abs/2602.11804)
*Yiming Zhou,Xuenjie Xie,Panfeng Li,Albrecht Kunz,Ahmad Osman,Xavier Maldague*

Main category: cs.CV

TL;DR: 提出轻量级RGB-D融合框架，通过深度先验增强EfficientViT-SAM，仅用11.2k样本训练即超越原模型性能


<details>
  <summary>Details</summary>
Motivation: SAM模型需要海量数据（1100万图像）且仅依赖RGB输入，现有高效变体仍依赖大规模训练。希望通过深度先验在少量数据下提升分割性能
...
Conclusion: 深度先验能显著提升分割性能，即使在小规模数据集上训练也能超越仅用RGB输入的模型，为轻量级高效分割提供新思路
...
<details>
  <summary>Details</summary>
Motivation: 传统单目视觉惯性里程计在低纹理环境中表现不佳，稀疏视觉特征不足以进行精确位姿估计。虽然基于Vision Transformer的复杂基础模型能提供密集、几何一致的深度信息，但其计算需求通常无法满足实时边缘部署要求

Method: 提出一个新颖框架，将学习到的深度先验直接集成到VINS-Mono优化后端，通过强制仿射不变深度一致性和成对序数约束，使用基于方差的门控机制显式过滤不稳定伪影

Result: 在TartanGround和M3ED数据集上的大量实验表明，该方法在挑战性场景中防止发散，并带来显著的精度提升，将绝对轨迹误差降低高达28.3%

Conclusion: 该方法在严格遵循边缘设备计算限制的同时，能够鲁棒地恢复度量尺度，填补了复杂深度模型与实时边缘部署之间的空白
...
TL;DR: 该论文针对压缩视频流的实时超分辨率问题，提出了StreamSR数据集和EfRLFN模型，通过YouTube视频构建代表性数据集，并设计高效架构提升实时超分辨率性能。
...
Conclusion: StreamSR数据集填补了流媒体超分辨率研究的空白，EfRLFN模型为实时视频超分辨率提供了高效解决方案，相关资源已开源供社区使用。
...
TL;DR: ArtContext：一个从艺术史文献和知识库中提取艺术作品上下文信息的自动化流程，通过定制化的CLIP模型改进艺术作品理解


<details>
  <summary>Details</summary>
Motivation: 艺术史文章常讨论艺术作品及其细节（如布局、图像学、物质文化），但观看艺术品时难以快速了解不同文献对该作品的讨论，需要系统化的信息整合方法

Method: 1) 构建开放获取艺术史文章语料库收集流程；2) 结合维基数据知识库；3) 使用低秩适应（LoRA）技术微调CLIP模型，创建领域专用的PaintingCLIP模型；4) 通过弱监督学习训练模型

Result: PaintingCLIP模型在性能上超越了原始CLIP模型，能够为给定艺术作品提供有效的上下文信息，证明了方法的有效性

Conclusion: 提出的ArtContext流程具有通用性，可轻松应用于其他人文学科领域，为艺术作品理解提供了系统化的信息标注解决方案
...
TL;DR: Latent Forcing是一种改进的扩散模型架构，通过联合处理潜在表示和原始像素，在保持潜在扩散效率的同时直接在原始自然图像上操作，实现了扩散变换器像素生成的新SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统潜在扩散模型虽然能生成高质量图像，但存在三个主要问题：1) 在图像编码过程中丢弃信息；2) 需要单独训练的解码器；3) 对原始数据的辅助分布进行建模，失去了端到端建模的优势。

Method: 提出Latent Forcing方法，通过联合处理潜在表示和像素，使用分别调优的噪声调度来排序去噪轨迹。潜在表示作为中间计算的"草稿纸"，在高频像素特征生成之前发挥作用。特别关注条件信号顺序的重要性。

Result: 在ImageNet数据集上，Latent Forcing在扩散变换器像素生成方面达到了新的最先进水平（SOTA）。该方法还解释了REPA蒸馏在分词器和扩散模型中的差异、条件与非条件生成的区别，以及分词器重建质量与可扩散性的关系。

Conclusion: Latent Forcing通过简单的架构修改，既保持了潜在扩散的效率，又能在原始自然图像上操作，实现了端到端建模的优势，为扩散模型的发展提供了新的方向。
...
TL;DR: 利用CTA数据训练神经隐式函数，解决CMRI各向异性限制，实现心脏形状重建


<details>
  <summary>Details</summary>
Motivation: 短轴心血管磁共振成像的各向异性特性限制了心脏形状分析，需要解决这一问题

Method: 利用近各向同性、高分辨率CTA数据训练单个神经隐式函数，联合表示任意分辨率的CMRI心脏形状

Result: RV和MYO重建的Dice相似系数分别为0.91±0.07和0.75±0.13，Hausdorff距离分别为6.21±3.97mm和7.53±5.13mm
...
TL;DR: Ctrl&Shift是一个端到端的扩散框架，用于实现几何一致的物体操纵，无需显式3D表示。它通过两阶段分解（物体移除和参考引导修复）和统一扩散过程，在保持背景、几何一致性和用户可控性方面达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时实现三个核心目标：背景保持、视角变化下的几何一致性以及用户可控的变换。基于几何的方法需要显式3D重建且泛化能力差，而基于扩散的方法泛化更好但缺乏细粒度几何控制。

Method: 将物体操纵分解为两个阶段：物体移除和参考引导修复，并在统一扩散过程中编码这两个阶段。设计了多任务、多阶段训练策略，将背景、身份和姿态信号分离到不同任务中。引入了可扩展的真实世界数据集构建流程，生成带有估计相对相机姿态的配对图像和视频样本。

Result: 广泛实验表明，Ctrl&Shift在保真度、视角一致性和可控性方面达到了最先进的性能。这是第一个统一细粒度几何控制和真实世界泛化的物体操纵框架，无需依赖任何显式3D建模。

Conclusion: Ctrl&Shift通过创新的两阶段分解和统一扩散框架，成功解决了物体级操纵中的关键挑战，实现了背景保持、几何一致性和用户可控性的统一，为电影后期制作、AR和创意编辑提供了强大的工具。
...
TL;DR: 提出了一种灵活的特征压缩框架ARFC，支持单一模型实现任意压缩率，通过自回归模型控制生成token数量来调节压缩比，引入MoS模块和ERGC约束提升压缩质量。


<details>
  <summary>Details</summary>
Motivation: 现有特征压缩方法通常需要为特定压缩率训练专用模型，缺乏灵活性且泛化能力有限，当需要新压缩率时必须重新训练，这在实际应用中效率低下。

Method: 提出ARFC框架，核心是自回归模型ARC，通过控制生成token数量实现任意压缩率。引入MoS模块利用多个压缩结果减少不确定性，集成ERGC约束在训练中保持语义和结构关系。

Result: 在跨模态检索、图像分类和图像检索等多个任务和数据集上的实验表明，该方法在各种压缩率下均优于现有方法，在某些情况下甚至超过了原始未压缩特征的性能。

Conclusion: ARFC框架有效解决了特征压缩的灵活性问题，单一模型支持任意压缩率，在资源受限的实际场景中具有显著优势和应用价值。
...
TL;DR: ImagineAgent是一个结合认知推理与生成想象的智能体框架，用于解决开放词汇人-物交互中的跨模态幻觉和遮挡模糊问题，在减少80%训练数据的情况下达到SOTA性能。
...
Method: 构建认知图显式建模检测实体与候选动作的合理关系，动态调用检索增强、图像裁剪和扩散模型等工具获取领域知识和丰富视觉证据，实现跨模态对齐，并使用平衡预测准确性和工具效率的复合奖励。

Result: 在SWIG-HOI和HICO-DET数据集上达到最先进性能，仅需约20%的训练数据（相比现有方法减少80%），验证了方法的鲁棒性和效率。

Conclusion: ImagineAgent通过协调认知推理与生成想象，有效解决了开放词汇人-物交互中的跨模态对齐问题，在减少训练数据需求的同时实现了优越的性能。
...
TL;DR: 提出VasoMIM框架，将血管解剖知识融入掩码图像建模，结合XA-170K大规模数据集，在X射线血管造影分析中实现SOTA性能
...
Method: 提出血管解剖感知的掩码图像建模框架VasoMIM，包含两个关键设计：1) 解剖引导的掩码策略，策略性掩码包含血管的补丁；2) 解剖一致性损失，保持原始图像与重建图像中血管结构的一致性

Result: 在四个下游任务、六个数据集上验证，VasoMIM展现出优越的迁移能力，相比现有方法达到最先进的性能

Conclusion: VasoMIM作为基础模型在X射线血管造影分析任务中具有显著潜力，XA-170K是该领域最大的预训练数据集，两者将开源供研究使用
...
Method: 提出监督辅助多模态融合扩散模型(MFdiff)：1) 设计多模态特征融合模块学习优化融合特征，充分利用MR图像但不引入额外细节；2) 以融合特征为条件，基于扩散模型迭代生成高质量SPET图像；3) 采用两阶段监督辅助学习策略，结合模拟分布内数据的通用先验和体内分布外数据的特定先验

Result: 实验表明，MFdiff能有效从多模态输入恢复高质量SPET图像，在定性和定量评估上均优于现有最先进方法

Conclusion: 提出的MFdiff模型成功解决了多模态PET图像恢复中的模态不一致和分布外数据问题，为低剂量PET成像提供了有效的解决方案
...
TL;DR: 提出一种基于生成压缩框架的感知去噪方法，通过熵编码潜在表示强制低复杂度结构，同时使用生成解码器通过感知度量恢复真实纹理，在保持竞争性失真性能的同时实现一致的感知改进。


<details>
  <summary>Details</summary>
Motivation: 传统失真驱动的去噪方法在强噪声和分布偏移下往往产生过度平滑的重建结果，无法同时保持结构细节和感知真实性。需要一种能够平衡率-失真-感知(RDP)权衡的去噪框架。

Method: 提出生成压缩框架：1) 条件Wasserstein GAN压缩去噪器，显式控制RDP权衡；2) 条件扩散重建策略，通过压缩潜在表示进行迭代去噪指导。使用熵编码潜在表示强制低复杂度结构，生成解码器通过LPIPS损失和Wasserstein距离等感知度量恢复纹理。

Result: 在合成和真实噪声基准测试中，该方法在保持竞争性失真性能的同时，实现了一致的感知改进。建立了基于压缩的最大似然去噪器在加性高斯噪声下的非渐近保证，包括重建误差和解码错误概率的界限。

Conclusion: 提出的生成压缩框架为感知去噪提供了一种有效方法，通过压缩潜在表示强制结构简单性，同时利用生成模型恢复真实纹理，在率-失真-感知权衡方面取得了良好平衡。
...
Conclusion: LUVE框架通过创新的潜在级联架构和双频专家设计，有效解决了超高清视频生成的关键挑战，为高质量UHR视频生成提供了可行的解决方案。
...
TL;DR: PLOT-CT是一种用于低剂量CT重建的新框架，通过在预对数域应用Voronoi分解将数据解耦为不同成分，有效抑制噪声并保留结构信息，相比传统方法在1e4光子水平下获得2.36dB PSNR提升。
...
Method: 提出PLOT-CT框架，在预对数正弦图上应用Voronoi分解，将数据解耦为不同的基础成分，并将这些成分嵌入独立的潜在空间。这种显式分解增强了模型学习判别性特征的能力，直接通过抑制噪声和保留预对数域信息来提高重建精度。

Result: 在1e4入射光子水平的预对数域实验中，PLOT-CT相比传统方法获得了2.36dB的PSNR提升，达到了最先进的性能表现。

Conclusion: PLOT-CT通过在预对数域应用Voronoi分解，有效解决了低剂量CT重建中的噪声放大和信息损失问题，显著提升了重建精度，为低剂量CT成像提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 涂鸦标注虽然降低了密集像素标注的成本，但存在噪声和不完整的监督问题。现有基于伪标签的方法受限于伪标签质量，需要提升伪标签的可靠性和空间一致性。

Method: PLESS通过层次化图像分区将图像分解为空间一致的层次区域，在语义一致区域内传播涂鸦信息来细化伪标签。该框架是模型无关的，可轻松集成到现有伪标签方法中。

Result: 在两个公开心脏MRI数据集（ACDC和MSCMRseg）上，对四种涂鸦监督算法进行实验，均显示出分割准确性的持续提升。

Conclusion: PLESS作为一种通用的伪标签增强策略，能够有效提升涂鸦监督医学图像分割的性能，具有模型无关性和易集成性。
...
TL;DR: 提出GR-Diffusion框架，将离散高斯表示与扩散模型结合，用于3D低剂量全身PET重建，通过几何先验和生成能力提升图像质量


<details>
  <summary>Details</summary>
Motivation: PET重建面临噪声放大、结构模糊和细节丢失等挑战，传统方法在稀疏采样和病态逆问题中存在低通限制，需要结合几何先验和生成能力的新方法

Method: GR-Diffusion框架：1) 使用离散高斯表示从投影数据生成参考3D PET图像作为物理基础基准；2) 在扩散过程中采用分层引导机制：细粒度引导利用差异优化局部细节，粗粒度引导使用多尺度差异图校正偏差；3) 结合GR的强几何先验和扩散模型的生成能力恢复亚体素信息

Result: 在UDPET和临床数据集的不同剂量水平实验中，GR-Diffusion在提升3D全身PET图像质量和保留生理细节方面优于现有最先进方法
...
TL;DR: SToRM：首个用于多模态大语言模型的监督式令牌缩减框架，在保持全令牌性能的同时将计算成本降低高达30倍，用于高效的端到端自动驾驶系统。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的端到端驾驶系统需要依赖多模态大语言模型来处理人类自然语言指令，但现有方法需要大量计算资源，而简单的令牌缩减方法会导致性能下降。

Method: 提出SToRM框架，包含三个核心组件：1) 轻量级重要性预测器估计令牌重要性；2) 监督训练方法利用全令牌LLM路径获得伪监督信号；3) 锚点-上下文合并模块将令牌分区并合并以减少冗余。

Result: 在LangAuto基准测试中，SToRM在相同令牌缩减预算下优于最先进的端到端驾驶MLLM，保持全令牌性能的同时将计算成本降低高达30倍。

Conclusion: SToRM框架首次实现了在保持多模态大语言模型性能的同时显著降低计算成本，为资源受限的自动驾驶系统提供了高效的端到端解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 情感对于创造引人入胜的VR内容至关重要，但现有生成方法无法捕捉细微的情感语义和沉浸式体验所需的细粒度控制。

Method: 采用分层情感表示，通过丰富的可学习原型在训练中演化，无需显式情感标签；开发了包含多原型引导、时间混合和注意力重加权的可控生成流程。
...
TL;DR: Clutt3R-Seg：一种用于杂乱场景中语言引导抓取的零样本3D实例分割方法，通过层次化实例树处理遮挡和噪声掩码，在稀疏视角下仍能保持鲁棒性。
...
Result: 在合成和真实数据集上均优于现有方法，在最具挑战性的重度杂乱序列中AP@25达到61.66，比基线高2.2倍以上；仅用4个输入视角就超越了使用8个视角的MaskClustering方法2倍以上。

Conclusion: Clutt3R-Seg能够有效处理杂乱场景中的3D实例分割问题，在稀疏视角下仍保持鲁棒性能，支持语言引导的机器人抓取任务，并通过一致性更新机制适应多阶段操作。
...
TL;DR: 论文提出了颈戴式视角的视线估计新任务，收集了首个相关数据集，评估了GLC模型并提出两个扩展方法：视线超出边界分类任务和多视角协同学习。


<details>
  <summary>Details</summary>
Motivation: 现有第一人称视角视线估计主要关注头戴式摄像头，而其他视角（如颈戴式）尚未充分探索。颈戴式视角具有独特优势，但缺乏相关研究数据和方法。

Method: 1) 收集首个颈戴式视线估计数据集（8名参与者，约4小时日常活动视频）；2) 评估基于Transformer的GLC模型；3) 提出两个扩展：视线超出边界分类辅助任务，以及基于几何感知损失的头戴式和颈戴式多视角协同学习方法。

Result: 视线超出边界分类任务相比标准微调提升了性能，但多视角协同学习方法未带来性能增益。论文进一步分析了这些结果并讨论了颈戴式视线估计的启示。

Conclusion: 颈戴式视线估计是一个有前景的新研究方向，视线超出边界分类是有效的改进方法，但多视角协同学习在当前设置下效果有限。该研究为未来颈戴式视角的视线估计研究奠定了基础。
...
TL;DR: 提出TD-FusionUNet模型，通过可训练的Hadamard变换和DCT变换层在正交化潜在空间捕捉频率特征，实现轻量高效的次日野火蔓延预测。


<details>
  <summary>Details</summary>
Motivation: 开发轻量级、计算高效的次日野火蔓延预测工具，适用于资源受限环境下的实时应用需求。

Method: 提出TD-FusionUNet模型，包含可训练的Hadamard变换和离散余弦变换层，在正交化潜在空间捕捉频率特征；采用随机边缘裁剪和高斯混合模型等预处理技术增强稀疏预火掩码表示。

Result: 在WildfireSpreadTS数据集上获得0.591的F1分数，仅使用37万参数，优于使用ResNet18编码器的UNet基线模型，且参数数量显著减少。
...
TL;DR: RI-Mamba是首个用于点云的旋转不变状态空间模型，通过全局和局部参考框架分离姿态与几何，使用希尔伯特排序构建具有几何结构的标记序列，实现了任意方向下的文本到形状检索。


<details>
  <summary>Details</summary>
Motivation: 随着3D资产数量的快速增长，文本到形状检索变得至关重要。现有方法需要规范姿态且支持类别有限，限制了在现实世界中的应用，因为真实场景中的物体可能属于不同类别且呈现随机方向。

Method: 1. 定义全局和局部参考框架来分离姿态与几何；2. 使用希尔伯特排序构建具有几何结构的标记序列；3. 引入方向嵌入计算策略并通过特征线性调制重新整合；4. 采用跨模态对比学习与自动三元组生成进行训练。

Result: 在OmniObject3D基准测试中，RI-Mamba在200多个物体类别上实现了最先进的性能，表现出卓越的表征能力和鲁棒性，特别是在任意方向下的检索任务中。

Conclusion: RI-Mamba是首个旋转不变的状态空间模型，通过创新的姿态分离和方向嵌入策略，有效解决了3D形状检索中的姿态变化问题，为大规模、多类别的文本到形状检索提供了实用解决方案。
...
TL;DR: 开发了一种语义条件化的潜在扩散模型，用于生成具有解剖循环和C臂位置控制的动脉期脑DSA图像，专家评估显示合成图像具有临床真实感。


<details>
  <summary>Details</summary>
Motivation: 数字减影血管造影（DSA）在脑血管疾病诊断和治疗中起核心作用，但其侵入性和高成本限制了大规模数据收集和公共数据共享，因此需要开发合成DSA图像的方法。

Method: 开发了语义条件化的潜在扩散模型（LDM），使用编码解剖结构和采集几何的文本嵌入进行训练，基于99,349帧单中心DSA数据集，控制解剖循环（前/后）和标准C臂位置。

Result: 四位医学专家（包括两名神经放射科医师、一名神经外科医师和一名内科专家）使用5级Likert量表评估400张合成DSA图像，整体Likert评分3.1-3.3，评分者间信度高（ICC(2,k)=0.80-0.87），Fréchet起始距离中位数低至15.27。

Conclusion: 语义控制的LDM能够生成具有临床真实感的合成DSA图像，适用于下游算法开发、研究和训练。
...
<details>
  <summary>Details</summary>
Motivation: 当前使用大语言模型进行3D场景生成的方法存在三个主要局限：缺乏领域特定推理能力、缺少验证机制、以及非模块化设计。这些限制导致控制性差和可扩展性不足。本文旨在解决这些问题，特别是在农业仿真环境生成领域。

Method: 开发了一个模块化的多LLM管道，集成了3D资产检索、领域知识注入和通过Unreal引擎API的代码生成。采用混合策略结合了少样本提示、检索增强生成、微调和验证等LLM优化技术。系统架构支持结构化数据处理、中间验证和灵活扩展。

Result: 系统能够根据输入提示和领域知识生成具有逼真种植布局和环境背景的3D环境。评估显示，用户研究确认了场景的真实性和熟悉度，专家比较表明相比手动场景设计节省了大量时间。结果证实了多LLM管道在自动化领域特定3D场景生成中的有效性。

Conclusion: 多LLM管道方法在农业仿真环境生成中表现出改进的可靠性和精度。未来工作将扩展资产层次结构、探索实时生成，并将管道适配到农业以外的其他仿真领域。
...
Method: 1. 双向耦合视觉里程计与高斯泼溅表示；2. 在期望最大化框架下进行联合优化，同时细化VO生成的半稠密深度估计和高斯表示；3. 提出高斯泼溅初始化方法，利用VO的图像信息、关键帧位姿和像素关联生成接近最终高斯场景的近似。

Result: 方法在实时运行的同时，在重建场景的几何/光度保真度和跟踪精度方面达到了最先进水平。

Conclusion: GSO-SLAM通过双向耦合VO和GS，在EM框架下联合优化，实现了实时单目稠密SLAM，无需启发式方法且无额外计算开销，在重建质量和跟踪精度上表现优异。
...
TL;DR: 本文提出STVG-R1框架，通过视觉提示范式将坐标预测转化为实例级识别问题，并结合强化学习优化时空准确性和一致性，在多个基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在密集预测任务（如时空视频定位）中存在文本描述与视觉坐标不对齐导致的幻觉问题。现有方法通常需要额外的可训练模块，带来显著的标注成本和计算开销。

Method: 提出新颖的视觉提示范式，将逐帧坐标预测转化为紧凑的实例级识别问题，为每个对象分配唯一且时间一致的ID作为视觉提示。引入首个STVG强化学习框架STVG-R1，使用任务驱动奖励联合优化时间准确性、空间一致性和结构格式正则化。

Result: 在六个基准测试中验证了方法的有效性。STVG-R1在HCSTVG-v2基准上比基线Qwen2.5-VL-7B的m_IoU提升了20.9%，达到新的SOTA。在MeViS基准上实现了47.3%的J&F零样本泛化性能。

Conclusion: 提出的视觉提示范式有效避免了跨模态坐标对齐的难题，STVG-R1框架通过强化学习联合优化多个目标，在时空视频定位任务中取得了显著性能提升，并展现出强大的零样本泛化能力。
...
TL;DR: 本文通过大规模实验研究展示了如何针对性地适配通用视觉语言模型以显著提升电商产品理解性能，同时保持广泛的跨模态能力，并提出了一套全面的评估框架。


<details>
  <summary>Details</summary>
Motivation: 电商产品理解需要强大的多模态理解能力，但通用视觉语言模型缺乏针对电商数据特点（属性中心、多图像、噪声）的适配策略，且现有方法在提升电商性能时往往会牺牲模型的通用能力。

Method: 通过大规模实验研究探索通用视觉语言模型的针对性适配方法，并提出包含深度产品理解、严格指令遵循和动态属性提取的综合评估套件。

Result: 研究表明，针对性的适配可以显著提升电商性能，同时保持模型的广泛多模态能力。

Conclusion: 针对电商数据特点的适配策略能够有效提升视觉语言模型在电商领域的性能，同时不损害其通用能力，提出的评估框架为相关研究提供了全面的评测标准。
...
TL;DR: 本文提出了一种改进视觉对比解码的方法，通过构建对象对齐的辅助视图来减少多模态大语言模型中的物体幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在物体幻觉问题，即模型会生成图像中不存在的物体描述。现有的视觉对比解码方法需要改进，以更好地识别和减少这种幻觉现象。

Method: 利用自监督视觉Transformer中的对象中心注意力机制，移除最显著的视觉证据来构建辅助视图，从而破坏不受支持的标记并产生更强的对比信号。该方法与提示无关、与模型无关，可无缝集成到现有VCD流程中。

Result: 在两个流行的物体幻觉基准测试和两个MLLM模型上，该方法都表现出一致的性能提升，计算开销小（仅需一次可缓存的向前传递）。

Conclusion: 通过构建对象对齐的辅助视图来改进视觉对比解码，能有效减少多模态大语言模型中的物体幻觉问题，且具有通用性和高效性。
...
Method: 1. 提出双流架构：将检索增强的对象生成与分层环境编排解耦；2. 建立物理感知的闭环机制：PostProcess Agent编写动态脚本，配合VLM-Motion Critic进行自我反思以迭代优化模拟代码。

Result: 在Code4D基准测试中，Code2Worlds优于基线方法，获得41%的SGS增益和49%更高的丰富度，同时独特地生成了先前静态方法中不存在的物理感知动态。

Conclusion: Code2Worlds通过将4D生成转化为语言到模拟代码生成，解决了多尺度上下文纠缠和语义-物理执行差距的挑战，实现了物理感知的动态世界生成，为构建基于物理定律的世界模拟器提供了有效框架。
...
Method: 提出了两个关键技术：1) 解耦流引导：一种时间感知策略，将光照控制有效注入潜在空间同时保持几何完整性；2) 时间一致注意力：在IC-Light架构中开发时间一致注意力机制，并结合确定性正则化消除外观闪烁。

Result: 大量实验表明，该方法在时间一致性和光照保真度方面具有竞争力，能够稳健处理-90°到90°的相机旋转。

Conclusion: Light4D是一个无需训练的4D视频重光照框架，能够有效解决4D重光照中的数据稀缺和时间一致性问题，在极端视角变化下仍能保持高质量的重光照效果。
...
TL;DR: 该论文提出使用3D分形作为合成数据，通过时间变换形成视频序列，用于动作识别模型的预训练，并开发了Targeted Smart Filtering方法来解决分形生成速度和多样性问题。


<details>
  <summary>Details</summary>
Motivation: 合成数据集在深度学习中作为真实标注数据的替代方案受到关注。FDSL方法通过公式驱动生成无限完美标注数据，避免了人工标注、隐私和伦理问题。但现有3D分形生成方法速度慢且产生退化分形，需要改进。

Method: 使用3D迭代函数系统生成3D分形，通过时间变换形成视频序列作为预训练数据集。针对生成速度慢和多样性不足的问题，提出了Targeted Smart Filtering方法，该方法能显著提升采样速度并保持分形多样性。

Result: 提出的Targeted Smart Filtering方法实现了约100倍的采样速度提升，并且在动作识别下游任务中表现优于其他3D分形过滤方法。

Conclusion: 3D分形作为合成数据可以有效用于动作识别模型的预训练，但需要平衡生成速度与多样性。Targeted Smart Filtering方法成功解决了这两个关键问题，为FDSL在视频理解任务中的应用提供了有效解决方案。
...
TL;DR: 本文提出JEPA-VLA方法，通过将预测性嵌入（特别是V-JEPA 2）集成到现有视觉语言动作模型中，显著提升了机器人操作任务的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于预训练视觉语言模型构建的视觉语言动作模型在机器人操作中取得了显著进展，但仍存在样本效率低和泛化能力有限的问题。作者认为这些限制与一个被忽视的组件——预训练视觉表示密切相关，现有视觉表示在环境理解和策略先验两方面都提供不足的知识。

Method: 通过深入分析发现，视频预训练的预测性嵌入（特别是V-JEPA 2）能够灵活丢弃不可预测的环境因素并编码任务相关的时间动态，有效弥补现有视觉表示的不足。基于此观察，作者提出了JEPA-VLA方法，这是一种简单而有效的方法，能够自适应地将预测性嵌入集成到现有的视觉语言动作模型中。

Result: 实验表明，JEPA-VLA在多个基准测试中带来了显著的性能提升，包括LIBERO、LIBERO-plus、RoboTwin2.0以及真实机器人任务。

Conclusion: 预测性嵌入在视觉语言动作模型中具有重要作用，能够有效提升环境理解和策略先验能力，从而显著改善机器人操作任务的性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前动态重建方法在单目输入应用中存在挑战，现有工作缺乏统一的时空分解框架，要么采用整体时间优化，要么采用耦合的层次空间组合，导致效率受限

Method: 提出WorldTree统一框架：1) 时间分割树(TPT)：基于继承的分割树结构实现从粗到细的优化，用于层次化时间分解；2) 空间祖先链(SAC)：递归查询祖先层次结构，提供互补的空间动态，同时在祖先节点间专门化运动表示

Result: 在不同数据集上，该方法在NVIDIA-LS数据集上LPIPS指标提升8.26%，在DyCheck数据集上mLPIPS指标提升9.09%，优于现有最佳方法

Conclusion: WorldTree通过统一的时空分解框架有效解决了动态重建中的时空表示问题，显著提升了单目输入动态重建的性能
...
TL;DR: 本文提出了一种名为Proximal-Mean Inversion (PMI)的训练免费梯度校正方法，用于解决Rectified-Flow模型在反转过程中的累积误差问题，同时引入了mimic-CFG用于编辑任务，显著提升了反转稳定性、图像重建质量和编辑保真度。


<details>
  <summary>Details</summary>
Motivation: Rectified-Flow模型虽然支持训练免费反转，但现有的反转方法存在跨时间步累积的近似误差，导致速度场不稳定，进而影响重建和编辑质量。需要一种方法来稳定速度场并提高反转性能。

Method: 提出了Proximal-Mean Inversion (PMI)，通过将速度场引导到过去速度的运行平均值来稳定速度场，并将其约束在理论推导的球形高斯分布内。同时引入了mimic-CFG，这是一种轻量级的速度校正方案，用于编辑任务，通过在当前速度与其历史平均投影之间进行插值，平衡编辑效果和结构一致性。

Result: 在PIE-Bench上的大量实验表明，该方法显著提高了反转稳定性、图像重建质量和编辑保真度，同时减少了所需的神经函数评估次数。在PIE-Bench上实现了最先进的性能，并具有更高的效率和理论合理性。

Conclusion: PMI和mimic-CFG方法有效解决了Rectified-Flow模型反转中的累积误差问题，提供了更稳定、高效且理论合理的速度场校正方案，显著提升了图像重建和编辑任务的质量。
...
TL;DR: DiffPlace是一个新颖的生成框架，通过引入place-ID控制器实现地点可控的多视角图像生成，解决了现有方法在生成具有地点感知和背景一致性的城市场景方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前的多视角扩散模型在生成具有地点感知和背景一致性的城市场景方面存在困难，这限制了它们在视觉地点识别任务中生成真实样本的有效性。
...
Conclusion: DiffPlace展示了生成模型在增强场景级和地点感知合成方面的潜力，为改善自动驾驶中的地点识别提供了有价值的方法。
...
TL;DR: 提出了DynaHOI-Gym平台和DynaHOI-10M基准测试，用于评估动态手-物交互任务，并提出了ObAct基线方法
...
Method: 引入DynaHOI-Gym统一在线闭环平台，包含参数化运动生成器和基于滚动的评估指标；提出ObAct基线方法，通过时空注意力将短期观察与当前帧整合来预测动作

Result: 发布了包含1000万帧和18万条手部捕捉轨迹的大规模基准DynaHOI-10M；ObAct基线方法在位置成功率上实现了8.1%的提升

Conclusion: DynaHOI-Gym平台和DynaHOI-10M基准填补了动态手-物交互评估的空白，为相关研究提供了重要工具和数据集
...
<details>
  <summary>Details</summary>
Motivation: LGE成像是心肌瘢痕评估的临床标准，但带标注的数据集有限，阻碍了自动分割方法的发展。需要解决数据稀缺问题，特别是针对心肌纤维化的分割任务。

Method: 1. 首先训练隐式神经表示（INRs）来捕获LGE数据及其对应的心肌和纤维化掩码的连续空间表示；2. 将这些INRs压缩为紧凑的潜在嵌入，保留关键解剖信息；3. 在潜在空间上使用扩散模型生成新的表示；4. 将生成的表示解码为合成的LGE图像和解剖学一致的分割掩码。

Result: 在133个心脏MRI扫描上的实验表明，使用200个合成体积增强训练数据可以改善纤维化分割性能，Dice分数从0.509提高到0.524。该方法提供了一种无需标注的方法来缓解数据稀缺问题。

Conclusion: 该研究提出了一种创新的合成数据生成框架，结合了隐式神经表示和扩散模型，能够生成解剖学一致的LGE图像和分割掩码，有效缓解了心肌瘢痕分割中的数据稀缺问题，代码已公开。
...
TL;DR: 该研究评估了视觉语言模型在法语PDF转Markdown任务上的表现，创建了一个专注于法语文档的基准测试，包含手写表格、复杂布局等挑战性内容，通过单元测试式检查评估了15个模型的表现。


<details>
  <summary>Details</summary>
Motivation: 文档解析是检索增强生成（RAG）流水线的关键步骤，转录和布局错误会传播到下游检索和基础任务。现有基准测试往往侧重于英语或中文，并且可能过度惩罚对下游使用基本无关的格式和线性化选择。

Method: 从60,000个文档语料库中通过模型分歧采样选择困难页面，构建法语专注的基准测试。评估采用单元测试式检查，针对具体失败模式（文本存在、阅读顺序、本地表格约束），并结合类别特定的规范化以忽略仅与呈现相关的差异。

Result: 在评估的15个模型中，最强的专有模型在手写和表格处理方面表现出显著更高的鲁棒性，而几个开源权重系统在标准印刷布局上仍具有竞争力。

Conclusion: 该研究为法语文档解析提供了更精确的评估框架，揭示了不同模型在处理不同类型法语文档时的性能差异，为RAG流水线中的文档解析任务提供了重要参考。
...
TL;DR: 提出了一种基于贝叶斯深度学习的概率优化框架，通过置信度-不确定性边界损失和双温度缩放策略，解决医疗影像AI模型过度自信问题，提高预测正确性与不确定性估计的对齐。


<details>
  <summary>Details</summary>
Motivation: 在基于医疗影像的关键决策支持系统中，AI辅助决策的可靠性与预测准确性同等重要。深度学习模型虽然准确性高，但经常存在校准不足的问题，表现为对错误预测过度自信。为了促进临床接受度，模型需要以与预测正确性相关的方式量化不确定性，让临床医生能够识别不可靠的输出进行进一步审查。

Method: 提出了一个通用的概率优化框架，基于贝叶斯深度学习。具体包括：1）新颖的置信度-不确定性边界损失（CUB-Loss），对高置信度错误和低置信度正确预测施加惩罚，显式强制预测正确性与不确定性估计的对齐；2）双温度缩放（DTS）策略用于事后校准，进一步细化后验分布以提高直观可解释性。

Result: 在三个不同的医疗影像任务上验证了该框架：肺炎自动筛查、糖尿病视网膜病变检测和皮肤病变识别。实证结果表明，该方法在不同模态上实现了一致的校准改进，在数据稀缺场景下保持稳健性能，并且在严重不平衡的数据集上仍然有效。

Conclusion: 该框架强调了其在真实临床部署中的潜力，通过提高模型校准性和不确定性估计的可靠性，增强了医疗影像AI系统的可信度和临床实用性。
...
Method: 提出Spatial Chain-of-Thought框架：1）通过文本-坐标交错指令格式训练增强扩散模型的布局感知能力；2）利用最先进的多模态大语言模型作为规划器生成综合布局计划，将其空间规划能力直接转移到生成过程中。

Result: 在图像生成基准测试中达到最先进性能，在复杂推理任务上显著优于基线方法，在图像编辑场景中也表现出强大效果。

Conclusion: SCoT框架是一种即插即用方法，有效桥接了MLLMs的推理能力和扩散模型的生成能力，解决了现有方法在计算成本和空间信息保留方面的局限性。
...
TL;DR: 本文研究使用生成式AI方法改进新生儿复苏视频中的活动识别，通过本地视觉语言模型结合大语言模型，相比传统监督学习方法取得显著提升
...
Method: 探索使用本地视觉语言模型结合大语言模型的生成式AI方法，与监督学习的TimeSFormer基线进行比较。使用包含13.26小时新生儿复苏视频的模拟数据集，评估多种零样本VLM策略和带有分类头的微调VLM，包括低秩适应技术

Result: 小型本地VLM存在幻觉问题，但使用LoRA微调后，F1分数达到0.91，显著超越了TimeSFormer的0.70结果

Conclusion: 生成式AI方法，特别是经过适当微调的视觉语言模型，在新生儿复苏视频活动识别方面具有显著优势，能够有效提升识别准确性
...
TL;DR: ReNoV：利用外部表征引导扩散模型进行新颖视角合成，通过表征投影模块提升几何一致性，在重建保真度和修复质量上超越现有方法


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在新颖视角合成中几何一致性不足，需要利用外部表征的几何和语义对应特性来增强生成视角的几何一致性

Method: 首先分析外部视觉表征在空间注意力中的对应能力，然后设计表征投影模块将外部表征注入扩散过程，形成ReNoV框架

Result: 在标准基准测试中超越先前基于扩散的新颖视角合成方法，在重建保真度和修复质量上有显著提升，能从稀疏、无姿态图像集合中实现鲁棒合成

Conclusion: ReNoV框架通过利用外部表征的几何和语义对应特性，有效提升了扩散模型在新颖视角合成中的几何一致性，为从稀疏图像集合中生成高质量新视角提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 传统CCD/CMOS传感器动态范围通常低于70dB，在焊接电弧监测和抛光金属表面分析等高光条件下容易饱和，导致数字图像相关(DIC)测量中细节丢失和显著误差
...
Conclusion: DMD系统提供高保真自适应HDR成像，克服传统传感器关键限制，在高光环境的光学计量和应力分析中具有强大潜力
...
TL;DR: GigaBrain-0.5M*是一个基于世界模型的视觉-语言-动作模型，通过世界模型强化学习实现跨任务适应，在复杂操作任务上相比基线提升约30%


<details>
  <summary>Details</summary>
Motivation: 传统的视觉-语言-动作模型在场景理解和未来预测方面存在局限，而视频世界模型在时空推理和未来预测方面表现优异，因此将世界模型作为VLA学习的基础

Method: 基于GigaBrain-0.5（在超过10,000小时机器人操作数据上预训练），通过RAMP（基于世界模型条件策略的强化学习）集成世界模型强化学习，实现跨任务适应
...
Result: 初步结果表明AssetFormer能有效生成符合设计约束的模块化3D资产，为专业开发和用户生成内容场景提供了高效的资产创建流程。
...
TL;DR: PosterOmni是一个统一的图像到海报生成框架，通过数据蒸馏奖励管道整合局部编辑和全局创建，在多个任务上超越了开源和部分专有系统。


<details>
  <summary>Details</summary>
Motivation: 图像到海报生成需要同时处理局部调整和高级设计理解，既要保留具体视觉实体又要理解抽象设计概念，现有方法难以同时满足这些交织的需求。

Method: 提出PosterOmni框架，通过三步流程：(1)构建覆盖六种任务类型的多场景数据集；(2)在局部和全局专家之间进行知识蒸馏用于监督微调；(3)应用统一的奖励反馈机制联合对齐视觉实体保留和美学偏好。

Result: 实验表明PosterOmni显著提升了参考依从性、全局构图质量和美学和谐度，超越了所有开源基线，甚至超过了几个专有系统。

Conclusion: PosterOmni成功解决了图像到海报生成中的多维度挑战，通过统一的框架整合了局部编辑和全局创建，为艺术海报创作提供了有效的解决方案。
...
Result: 仅使用13,000个Nano Banana pro演示微调FLUX模型，在提示跟随和美学基准上获得竞争性表现。框架能有效泛化到离散图像和视频生成，并可作为鲁棒正则化器缓解基于奖励优化的奖励黑客问题。

Conclusion: FAIL提供了一种无需显式奖励或成对比较的流匹配模型后训练框架，在多种生成任务中表现出色，并能有效缓解奖励黑客问题，具有广泛适用性。
...
TL;DR: TexSpot是一个基于扩散的纹理增强框架，通过新的Texlet 3D纹理表示解决多视图扩散管道中的视角不一致问题，显著提升纹理质量、几何一致性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前3D纹理生成面临的主要挑战是视角不一致问题。现有方法要么依赖UV贴图（展开时会产生扭曲），要么基于点云方法（纹理保真度受几何密度限制，难以生成高分辨率纹理）。

Method: 提出TexSpot框架，核心是Texlet表示法，将基于点的3D纹理的几何表达能力与UV表示的紧凑性相结合。每个Texlet潜在向量通过2D编码器编码局部纹理块，再通过3D编码器聚合全局形状上下文。使用级联3D到2D解码器重建高质量纹理块，训练基于Texlet的扩散变换器来增强多视图扩散方法生成的纹理。
...
TL;DR: DreamID-Omni是一个统一的可控人中心音频-视频生成框架，通过对称条件扩散Transformer和双级解耦策略解决多人物身份与音色绑定问题，在多个任务上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将参考音频-视频生成、视频编辑和音频驱动视频动画等任务视为孤立目标，且在多人物场景中难以实现精确的身份和音色解耦控制。
...
Conclusion: DreamID-Omni提供了一个统一框架解决人中心音频-视频生成中的多任务和多人物控制挑战，将开源代码以弥合学术研究与商业应用之间的差距。
...
TL;DR: DeepGen 1.0是一个轻量级5B参数统一多模态模型，通过创新的SCB架构和数据中心化训练策略，在图像生成和编辑任务上超越更大规模模型，同时大幅降低训练和部署成本。


<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型通常需要巨大参数量（>10B），导致训练成本高昂且部署困难。需要开发轻量级但性能强大的替代方案，以降低研究门槛。

Method: 提出Stacked Channel Bridging（SCB）深度对齐框架，从多层VLM提取层次特征并与可学习的"思考令牌"融合；采用三阶段数据中心化训练策略：对齐预训练、联合监督微调、基于MR-GRPO的强化学习。

Result: 仅用约5000万样本训练，在多个基准测试中取得领先性能：WISE基准上超越80B HunyuanImage 28%，UniREditBench上超越27B Qwen-Image-Edit 37%。

Conclusion: DeepGen 1.0为统一多模态研究提供了高效、高性能的轻量级替代方案，通过开源代码、权重和数据集，有助于民主化该领域研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理多模态理解和生成任务时存在目标干扰和表示纠缠的问题，需要一种统一的框架来同时支持理解、生成和编辑任务，并保持忠实性和可控性。

Method: 提出统一的离散流匹配框架，通过任务特定的低秩适配器解耦理解和生成，避免目标干扰和表示纠缠；采用基于参考的多模态偏好对齐来优化相同条件下的相对结果，提高忠实性和可控性，无需大规模重新训练。

Result: 在八个基准测试中达到最先进的性能，展现出强大的零样本泛化能力，包括修复、上下文图像生成、基于参考的编辑和组合生成等任务，尽管没有进行明确的特定任务训练。

Conclusion: UniDFlow通过统一的离散流匹配框架成功解决了多模态理解和生成中的目标干扰问题，实现了SOTA性能，并展示了强大的零样本泛化能力，为多模态AI系统提供了有效的解决方案。
...
TL;DR: Monarch-RT：一种用于实时视频生成的稀疏注意力参数化方法，通过Monarch矩阵分解注意力，在保持计算效率的同时实现高表达力，首次在单张RTX 5090上实现16 FPS的真正实时视频生成。


<details>
  <summary>Details</summary>
Motivation: 实时视频生成中，基于扩散变换器的3D自注意力存在二次计算成本瓶颈，特别是在少步和自回归的实时场景下。现有稀疏注意力近似方法在这种场景下失效，因为视频注意力并非可靠稀疏，而是结合了周期性结构、稀疏语义对应和密集混合。

Method: 提出Monarch-RT结构化注意力参数化方法，使用Monarch矩阵分解注意力，通过适当对齐的块结构和扩展的平铺Monarch参数化实现高表达力。通过微调和自定义Triton内核克服参数化开销。

Result: Monarch-RT在现有双向模型设计的稀疏基线方法上表现出高效性，应用于最先进的Self-Forcing模型时达到95%注意力稀疏度且无质量损失。优化实现超越FlashAttention-2/3/4内核，在RTX 5090、H100和B200 GPU上获得1.4-11.8倍加速，首次在单张RTX 5090上实现16 FPS的实时视频生成。

Conclusion: Monarch-RT是实时视频生成中首个高度能力的稀疏注意力参数化工作，通过结构化注意力分解有效解决了实时视频生成的计算瓶颈，为实时视频生成提供了可行的技术方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有统一模型虽然能处理多模态理解和生成，但通常单次推理不进行迭代精炼。许多复杂多模态任务（如空间组合、多对象交互、动态指令）需要分解指令、验证中间结果并进行迭代修正。测试时扩展在语言模型中已证明有效，但扩展到多模态统一模型仍是挑战。

Method: UniT框架结合智能数据合成、统一模型训练和灵活的测试时推理，激发验证、子目标分解和内容记忆等认知行为。采用顺序思维链推理而非并行采样，训练短推理轨迹使其在测试时能泛化到更长推理链。

Result: 1) 在短推理轨迹上训练的统一模型能在测试时泛化到更长推理链；2) 顺序思维链推理比并行采样更具可扩展性和计算效率；3) 在生成和编辑轨迹上训练能改善分布外视觉推理性能。
...
TL;DR: 提出了一种渐进式语义错觉的矢量草图生成框架，通过顺序添加笔画实现单个草图在不同绘制阶段的语义转换，核心挑战在于初始笔画需要同时满足两个不同概念的识别要求。


<details>
  <summary>Details</summary>
Motivation: 传统视觉错觉主要依赖空间操作，本文旨在将视觉错觉从空间维度扩展到时间维度，探索在矢量草图绘制过程中通过笔画序列实现语义转换的新范式。

Method: 提出了Stroke of Surprise生成框架，采用序列感知的联合优化方法，通过双分支Score Distillation Sampling机制动态调整初始笔画，并引入Overlay Loss确保空间互补性而非遮挡。

Result: 实验表明该方法在可识别性和错觉强度方面显著优于现有基线方法，成功实现了从空间到时间维度的视觉字谜扩展。

Conclusion: 通过渐进式语义错觉框架，展示了矢量草图在时间维度上的语义转换能力，为视觉错觉研究开辟了新的方向。
...
TL;DR: HybridRAG是一个新颖的检索增强生成框架，通过预生成QA知识库和分层文本块处理，在保持准确性的同时降低延迟，特别适用于处理非结构化PDF文档的聊天机器人场景。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法通常假设结构化文本源并在查询时进行检索和生成，这在处理现实世界中非结构化文档（如复杂布局的PDF）时存在局限性，且查询时生成可能导致延迟问题。

Method: 1. 通过OCR和布局分析处理原始非结构化PDF文档，转换为分层文本块；2. 使用LLM从组织好的文本块中预生成QA知识库；3. 查询时优先匹配QA库获取即时答案，仅在没有合适匹配时才进行实时响应生成。

Result: 在OHRBench上的实验表明，HybridRAG相比标准RAG基线提供了更高的答案质量和更低的延迟。

Conclusion: HybridRAG是处理大量非结构化文档和众多用户的现实聊天机器人应用的实用解决方案，特别适用于计算资源有限的环境。
...
TL;DR: 该论文从动态视角研究大语言模型中的检索头，发现检索头在时间步上动态变化，具有不可替代性，且模型隐藏状态包含预测未来检索头模式的信号，揭示了内部规划机制。


<details>
  <summary>Details</summary>
Motivation: 先前研究主要依赖跨数据集的静态统计来识别检索头，忽略了自回归生成的细粒度时间动态性。本文旨在从动态角度研究检索头，揭示其在时间维度上的变化规律和内部机制。

Method: 通过广泛分析建立三个核心主张：动态性、不可替代性和相关性。在Needle-in-a-Haystack任务和多跳QA任务上验证发现，并在动态检索增强生成框架中量化动态和静态检索头的效用差异。

Result: 研究证实：(1)检索头在不同时间步动态变化；(2)动态检索头具有时间步特异性，无法被静态检索头有效替代；(3)模型隐藏状态编码了预测未来检索头模式的信号，表明存在内部规划机制。

Conclusion: 该研究从动态视角揭示了LLM检索头的时间动态特性，证明了动态检索头的独特性和不可替代性，以及模型内部存在的规划机制，为理解LLM内部机制提供了新见解。
...
<details>
  <summary>Details</summary>
Motivation: 等离子体物理研究文章包含高度复杂和上下文丰富的内容，需要提取其中的关键实体以支持高级搜索等应用。当前缺乏专门针对该领域嵌套命名实体识别的解决方案。

Method: 1) 为嵌套NER任务标注包含16个类别的等离子体物理语料库；2) 采用实体特定模型专业化方法，训练独立的BERT-CRF模型识别单个实体类型；3) 集成超参数优化过程系统性地微调模型参数。

Result: 开发了一个专门针对等离子体物理领域的嵌套命名实体识别系统，通过模型专业化和超参数优化提升了实体提取性能，为研究人员提供了有效的文献导航和分析工具。

Conclusion: 该方法推动了等离子体物理领域的实体识别技术发展，为研究人员提供了支持科学文献导航和分析的基础框架，具有实际应用价值。
...
TL;DR: RECOM基准测试显示LLMs在回答近期Reddit问题时存在语义-词汇悖论：模型通过大量改写保持语义相似性（余弦相似度>99%），但词汇重叠度极低（BLEU-1<8%），挑战了传统词汇指标在评估抽象生成任务中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索大型语言模型在处理近期信息时与人类视角的对齐程度，当前对LLMs在时间敏感信息上的表现评估不足，需要建立专门基准来评估模型在现实世界近期问题上的回答质量。
...
Result: 发现语义-词汇悖论：所有模型余弦相似度>99%，但BLEU-1<8%，差距超过90个百分点。MoverScore（51-53%）处于中间位置。模型规模不预测性能：7B参数的Mistral优于20B参数的GPT-OSS。矛盾率低于7%，模型很少生成与人类共识直接冲突的内容。

Conclusion: 词汇指标在评估抽象生成任务中不可靠，需要多维评估框架来捕捉语义保真度。模型通过大量改写而非词汇复制来保持语义，模型规模不是性能的预测指标。RECOM数据集公开可用，促进对LLMs时间对齐能力的研究。
...
<details>
  <summary>Details</summary>
Motivation: 尽管参数高效微调（PEFT）方法被广泛用于适配大语言模型到下游任务，并被认为能提高事实正确性，但PEFT如何影响幻觉行为仍不够清楚，特别是在问答数据集上

Method: 通过三个开源LLM主干和三个事实寻求QA基准的全面实证研究，使用七种无监督幻觉检测方法（基于语义一致性、置信度和熵的检测器），评估PEFT对幻觉检测的影响

Result: 实验结果表明，PEFT能持续增强幻觉检测能力，显著提高各种幻觉检测器的AUROC；进一步分析表明PEFT方法主要通过重塑不确定性编码和表达方式，而非向模型注入新的事实知识

Conclusion: PEFT方法能有效改善大语言模型的幻觉检测性能，主要通过改变模型内部不确定性的编码机制来实现，这为理解PEFT如何影响模型行为提供了重要见解
...
TL;DR: FalseCite是一个用于系统研究LLM幻觉的数据集，通过误导性引用诱导幻觉，并在多个模型中观察到虚假引用会显著增加幻觉率，特别是GPT-4o-mini。通过分析隐藏状态向量，发现无论是否产生幻觉，这些向量都呈现独特的角状结构。
...
Method: 引入FalseCite数据集，包含误导性或伪造的引用来诱导幻觉。使用GPT-4o-mini、Falcon-7B和Mistral 7-B三个模型进行测试，分析幻觉活动。通过分析模型的隐藏状态向量，进行可视化和聚类分析。

Result: 在FalseCite数据集上测试发现，虚假引用会显著增加模型的幻觉活动，特别是GPT-4o-mini。分析隐藏状态向量发现，无论是否产生幻觉，这些向量都倾向于形成独特的角状形状。

Conclusion: FalseCite数据集为评估和减轻LLM幻觉提供了基础工具，未来研究可以利用它来更好地理解和缓解大语言模型的幻觉问题。
...
<details>
  <summary>Details</summary>
Motivation: 在文本分类中，当类别不可用、难以区分或相互关联时，分类仍然具有挑战性。社会分析领域大量依赖文本数据，可以从文本分类中受益。本文旨在通过结合多个模型的智能来增强根据联合国可持续发展目标(SDG)的文本分类。

Method: 使用生成式AI模型生成合成数据用于模型训练，然后应用组合融合分析(CFA)技术。CFA是一种系统融合范式，使用秩-得分特征(RSC)函数和认知多样性(CD)，通过结合一组相对较好且相互多样化的分类模型来增强分类方法。

Result: CFA技术达到了96.73%的性能，优于最佳个体模型。研究结果与人类领域专家的结果进行了比较，证明结合多个ML/AI模型的智能与人类专家输入不仅可以互补，还能相互增强。

Conclusion: 通过CFA结合多个AI模型的智能，并获取人类专家的输入，能够显著提升联合国可持续发展目标文本分类的性能，展示了模型融合与人类专业知识相结合的有效性。
...
TL;DR: 研究评估了Gemini 2.5 Flash和Pro TTS模型在五种印度语言中生成法庭演讲的表现，发现模型在程序性信息传递上表现出色，但在情感表达和说服力方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 随着LLM的发展，TTS技术从基本可理解性转向上下文感知的表达性合成。在法律领域，合成语音需要传达权威性和特定专业形象，这在印度多语言环境中尤为复杂，需要研究多语言TTS在法律倡导中的表现。

Method: 提出了一个提示框架，利用Gemini 2.5对五种印度语言的原生支持和上下文感知节奏控制，生成不同的律师角色。在泰米尔语、泰卢固语、孟加拉语、印地语和古吉拉特语中评估模型表现。

Result: 模型表现出"单调权威性"，在程序性信息传递方面表现出色，但在动态声音调节和情感深度方面存在困难。孟加拉语和古吉拉特语的表现下降，突显了语音学方面的挑战。多语言TTS已准备好处理程序性法律任务，但难以复制人类法律话语的说服艺术。

Conclusion: 研究强调了多语言TTS在法律程序性任务中的可用性，同时指出了在复制人类法律话语说服艺术方面仍存在的挑战，特别是在情感表达和语音学优化方面需要进一步改进。
...
<details>
  <summary>Details</summary>
Motivation: 当前作者回复生成任务主要作为自动文本生成处理，未能充分利用作者的专业知识和意图。作者在回应审稿意见时拥有领域专业知识、作者独有信息、修订策略等具体形式的专业知识和意图，需要NLP辅助工具整合这些信号来支持有效的回复撰写。

Method: 1. 将作者回复生成重构为作者参与循环任务；2. 提出REspGen生成框架，整合显式作者输入、多属性控制和评估引导优化；3. 开发REspEval评估套件，包含20+个指标覆盖输入利用、可控性、回复质量和话语分析；4. 构建Re³Align数据集，首个大规模对齐的评审-回复-修订三元组数据集。

Result: 实验使用最先进的LLM显示：1. 作者输入和评估引导优化对回复质量有显著益处；2. 输入设计对回复质量有重要影响；3. 在可控性和质量之间存在权衡关系。

Conclusion: 通过将作者回复生成重构为作者参与循环任务，并整合作者专业知识和意图，可以显著提升回复生成的质量和实用性。提出的REspGen框架、REspEval评估套件和Re³Align数据集为这一研究方向提供了重要工具和基准。
...
TL;DR: 多语言预训练模型的tokenizer对不同书写系统存在系统性偏见，高碎片化拼写导致3.4倍分词数量增加、16.5倍推理速度下降和显著信息成本上升


<details>
  <summary>Details</summary>
Motivation: 虽然多语言预训练模型常被认为是脚本无关的，但其tokenizer可能对某些书写系统施加系统性成本。本研究旨在量化这种"脚本税"，揭示tokenization作为多语言NLP不平等来源的重要性

Method: 通过比较具有相同语言内容但不同拼写变体的文本，使用比特每字符（BPC）指标避免子词碎片化带来的"NLL悖论"，并进行往返转换检查（CER_rt=0.31）来区分映射噪声

Result: 高碎片化拼写导致分词数量增加3.4倍（6.73-6.85 vs. 2.10-2.35 tokens/word），推理速度下降16.5倍（0.23 vs. 3.8 sentences/second），信息成本显著增加：mBERT增加19.7%（8.06->9.65），XLM-R增加47.1%（12.19->17.94）

Conclusion: tokenization是多语言NLP不平等的重要来源，研究结果支持开发脚本感知的tokenization和预训练方法，以减轻不同书写系统间的性能差异
...
TL;DR: 这篇综述论文从电路复杂性、近似理论和通信复杂性三个理论视角，分析了Transformer在离散推理任务（如算术、逻辑推理和算法组合）中的理论局限性，解释了为什么当前Transformer架构难以实现精确的离散算法，尽管在模式匹配和插值方面表现出色。


<details>
  <summary>Details</summary>
Motivation: Transformer已成为序列建模应用的基础架构，在自然语言处理、视觉等领域支撑着最先进的系统。然而，它们在离散推理任务（如算术、逻辑推理和算法组合）中的理论局限性仍然是一个关键开放问题。本文旨在通过综合三个理论视角的研究，阐明Transformer在执行符号计算时面临的结构和计算障碍。

Method: 论文从三个理论视角进行综合分析：1）电路复杂性：分析Transformer的深度约束和计算结构限制；2）近似理论：研究Transformer在逼近不连续性方面的困难；3）通信复杂性：探讨token间通信的瓶颈问题。通过连接这些理论框架，提供统一的理论解释。

Result: 分析揭示了Transformer在离散推理任务中的多个核心挑战：深度约束限制了复杂算法的实现能力；难以精确逼近不连续函数；token间通信存在瓶颈，限制了符号计算的表达能力。这些理论限制解释了为什么Transformer在模式匹配和插值方面表现出色，但在精确离散算法实现上存在困难。

Conclusion: 当前Transformer架构在离散推理任务中存在根本性理论限制，主要源于其结构设计和计算特性。论文讨论了这些发现对模型设计的影响，并提出了克服这些基础限制的有前景方向，为未来改进Transformer在符号计算能力方面的研究提供了理论指导。
...
TL;DR: 本文研究大型语言模型能否在低数据环境下通过推理预测人类日常活动及其持续时间，采用检索增强提示策略整合四种上下文信息，在智能家居数据集上验证了模型的有效性。


<details>
  <summary>Details</summary>
Motivation: 预测人类活动及其持续时间在智能家居自动化、建筑设计、交通系统模拟和人机协作等应用中至关重要。现有数据驱动的智能体模型在低数据环境下表现不佳，限制了实际应用。本文探索预训练的大型语言模型是否能够利用广泛的人类知识，通过紧凑的上下文线索推理日常活动。

Method: 采用检索增强提示策略，整合四种上下文信息：时间、空间、行为历史和个人特征。在CASAS Aruba智能家居数据集上进行评估，涵盖两个互补任务：带持续时间估计的下一个活动预测，以及多步骤日常序列生成。测试不同数量的少样本示例，分析少样本效果。

Result: 结果显示大型语言模型展现出对人类行为的强大内在时间理解能力：即使在零样本设置下，也能产生连贯的日常活动预测；添加一两个示例能进一步优化持续时间校准和分类准确性。超过几个示例后性能趋于饱和，表明收益递减。序列级评估确认了在不同少样本条件下的一致时间对齐。

Conclusion: 预训练语言模型可以作为有前景的时间推理器，既能捕捉重复的日常规律，又能适应上下文依赖的行为变化，从而增强智能体模型的行为模块。这些发现表明语言模型在低数据环境下具有实用价值。
...
TL;DR: 论文提出使用LLM进行阿尔茨海默病检测，通过分析模型内部表示设计特殊标记，并生成合成数据来增强模型性能


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的早期可靠检测具有挑战性，主要因为标记数据有限。虽然大语言模型在跨领域迁移方面表现出色，但通过监督微调将其应用于AD领域的研究仍很有限

Method: 1) 对LLM进行AD检测的微调；2) 使用探测技术分析transformer层中的中间激活；3) 设计任务感知的特殊标记集；4) 训练序列到序列模型作为数据合成工具，利用这些标记生成结构一致且具有诊断信息的合成样本

Result: 微调后，特定词语和特殊标记的探测值发生显著变化，表明这些元素在模型改进的检测性能中扮演关键角色。合成数据在内在评估和下游训练管道中都得到了评估

Conclusion: 通过分析LLM内部表示并设计特殊标记，可以生成高质量的合成数据来增强AD检测模型的性能，为解决标记数据有限的问题提供了新方法
...
TL;DR: 这篇论文综述了提示工程在自然语言生成任务中的最新发展，提出了提示设计的分类法、决策框架，并建立了设计-优化-评估的框架来支持更可控和可泛化的NLG。


<details>
  <summary>Details</summary>
Motivation: 尽管提示工程已成为提升大语言模型性能的重要技术，但在自然语言生成领域仍缺乏系统化的框架和对各种提示工程方法的统一理解。本文旨在填补这一空白，为研究者提供结构化的指导。

Method: 通过文献综述方法，分析近期提示工程的发展，提出提示范式的分类法，建立基于不同因素的提示选择决策框架，并设计连接设计、优化和评估的系统框架。

Result: 论文系统梳理了提示工程在NLG任务中的应用，提出了分类体系、决策框架和设计-优化-评估框架，为实践者提供了选择合适提示方法的指导原则。

Conclusion: 提示工程作为输入级控制机制，与微调和解码方法形成互补，能够显著提升NLG任务性能。建立系统化的框架有助于实现更可控和可泛化的自然语言生成。
...
TL;DR: 本文全面综述了大型语言模型中的语码转换研究，提出了统一分类法，总结了构建、适应和评估语码转换能力LLM的实用指南，分析了当前评估实践的局限性，并讨论了新兴安全问题。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言建模取得进展，但大型语言模型在混合语言环境中仍面临挑战，在语法性、事实性和安全性方面表现出系统性退化。需要系统研究语码转换现象，为构建更好的混合语言能力模型提供指导。

Method: 1. 引入统一分类法，从数据、建模和评估三个维度组织现有研究；2. 总结实用的行动指南；3. 综述从语码转换定制预训练、任务特定后训练到提示策略和上下文学习的建模方法；4. 分析当前评估实践，识别不稳定性和可重复性限制；5. 编目现有基准并批判性考察其语言覆盖范围和英语中心偏见。

Result: 1. 建立了语码转换研究的系统框架；2. 提供了构建语码转换能力LLM的实用指南；3. 识别了当前评估实践中的问题，包括不稳定性和有限的可重复性；4. 揭示了现有基准的英语中心偏见和有限语言覆盖；5. 发现了语码转换可能被用作绕过模型安全防护的机制。

Conclusion: 语码转换对大型语言模型仍是重要挑战。本文通过系统综述和分类法为研究者和实践者提供了框架和指南，同时指出了评估实践的改进方向、安全风险以及开放研究挑战，为未来语码转换能力LLM的发展奠定了基础。
...
Method: 提出MetaMem框架，通过自演化的元记忆迭代蒸馏可迁移的知识利用经验，通过自我反思推理过程并更新元记忆状态

Result: MetaMem显著优于强基线方法，性能提升超过3.6%，所有代码和数据集已开源

Conclusion: MetaMem通过元记忆机制有效解决了记忆碎片化问题，提升了LLM在长时交互中的知识利用能力和推理性能
...
### [76] [DDL2PropBank Agent: Benchmarking Multi-Agent Frameworks' Developer Experience Through a Novel Relational Schema Mapping Task](https://arxiv.org/abs/2602.11198)
*Shafiuddin Rehan Ahmed,Wei Wei*

Main category: cs.CL

TL;DR: DDL2PropBank基准任务用于评估多智能体框架的开发者体验，通过将数据库模式映射到PropBank角色集，测试框架的代码复杂性和AI辅助能力。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏系统方法来评估多智能体框架在受控环境下的开发者体验，需要建立基准任务来比较不同框架的性能。

Method: 提出DDL2PropBank基准任务，使用Agent-as-a-Tool模式在10个框架中实现相同智能体逻辑，从代码复杂性和AI辅助能力两个维度进行评估。

Result: 结果显示三倍复杂度谱系，Pydantic AI和Agno实现开销最小；AI辅助能力方面，结构对齐分数能可靠预测单模式框架的成功，但高估多模式框架的正确性；Agno综合表现最佳。

Conclusion: Agno框架在复杂度和AI辅助能力方面表现最优，为多智能体框架评估提供了系统方法，并揭示了框架设计模式对AI生成代码质量的影响。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常在提示缺少关键细节或包含误导信息时仍然回应，导致幻觉或强化错误观念。需要评估和改进LLMs决定何时以及如何寻求澄清的能力，同时不牺牲任务性能。

Method: 1. 提出AskBench交互式基准测试，将标准QA对转换为具有明确检查点的多轮交互；2. 统一评估循环评估最终答案并模拟用户响应；3. 包含AskMind（意图不足查询）和AskOverconfidence（包含错误前提查询）两种设置；4. 提出基于准则的强化学习与验证器奖励（RLVR），使用结构化准则鼓励针对性澄清。

Result: 实验显示在准确性、准则遵循度和交互效率方面都有持续改进，并且对未见领域有很强的泛化能力。

Conclusion: AskBench基准测试和RLVR方法有效提升了LLMs在信息不足或包含错误前提时主动寻求澄清的能力，同时保持了任务性能，具有很好的泛化性。
...
TL;DR: 论文提出NLDD指标评估思维链解释的忠实性，发现模型推理存在"推理视界"现象，超过70-85%链长的推理步骤对最终答案影响很小甚至负面。


<details>
  <summary>Details</summary>
Motivation: 当前思维链解释被广泛用于解释语言模型解决复杂问题的方式，但尚不清楚这些逐步解释是否真实反映了模型的决策过程，还是仅仅是事后合理化。需要一种方法来衡量思维链解释的忠实性。

Method: 提出归一化对数差异衰减（NLDD）指标，通过破坏思维链解释中的单个推理步骤，测量模型对其答案置信度的下降程度，从而确定该步骤是否真正重要。通过标准化这些测量，NLDD支持跨不同架构的模型比较。

Result: 在句法、逻辑和算术任务上测试三个模型系列，发现一致的"推理视界"（k*）在链长的70-85%处，超过此范围的推理标记对最终答案影响很小甚至为负。还发现模型可以编码正确的内部表示但完全无法完成任务。

Conclusion: 仅凭准确性无法揭示模型是否真正通过思维链进行推理。NLDD提供了一种衡量思维链何时真正重要的方法，揭示了模型推理过程中的"推理视界"现象。
...
TL;DR: 论文提出了Mis-Align Bench基准测试，用于同时评估大语言模型在安全、价值和文化三个维度上的错位问题，构建了包含382,424个样本的SAVACU数据集，发现单一维度模型在联合条件下存在高假失败率和低对齐分数。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型错位评估基准（如INSECURE CODE、VALUEACTIONLENS、CULTURALHERITAGE）只能单独评估安全、价值或文化维度，无法同时评估这三个必须共存的维度，这限制了模型在现实场景中的对齐评估。

Method: 1. 构建SAVACU数据集：将LLM-PROMPT-DATASET中的提示重新分类为14个安全域、56个价值域和42个文化域，使用Mistral-7B-Instruct-v0.3进行分类，并通过Llama-3.1-8B-Instruct扩展低资源域，使用SimHash指纹避免重复。2. 通过两阶段拒绝采样为提示配对错位和对齐的响应。3. 在通用、微调和开源权重的大语言模型上进行基准测试，系统评估三个维度下的错位情况。

Result: 单一维度模型在联合条件下覆盖率可达97.6%，但假失败率超过50%，对齐分数较低（63%-66%）。这表明现有单维度评估方法无法准确反映模型在现实多维度场景中的对齐表现。

Conclusion: 需要统一的基准来同时评估大语言模型在安全、价值和文化维度上的错位问题，因为现实世界查询需要这三个维度同时满足。Mis-Align Bench基准填补了这一空白，为系统评估模型对齐提供了重要工具。
...
TL;DR: 该研究提出一个框架来评估LLMs在社交情境中的行为倾向与人类的一致性，通过将心理学问卷转化为情境判断测试，发现LLMs常与人类偏好分布不一致，存在过度自信、偏离共识等问题。
...
Method: 基于心理学问卷，将人类自我报告陈述转化为情境判断测试(SJTs)，在真实的用户-助手场景中评估行为。生成2,500个SJTs，每个由3名人类标注者验证，每个SJT从550名参与者中收集10名标注者的偏好行动。对25个LLMs进行全面研究。

Result: 研究发现：1) 在人类共识低的场景中，LLMs对单一回应表现出过度自信；2) 当人类共识高时，较小模型显著偏离，甚至一些前沿模型在15-20%情况下未能反映共识；3) 特质呈现跨LLM模式，如LLMs在人类共识倾向于克制的场景中鼓励情绪表达；4) LLMs的陈述价值观与揭示行为之间存在显著差距。

Conclusion: LLMs的行为倾向常与人类偏好分布不一致，存在系统性偏差。将心理测量陈述直接映射到行为场景为评估自我报告的预测效度提供了独特机会，揭示了LLMs的陈述价值观与实际行为之间的显著差距。
...
TL;DR: 研究发现大语言模型的自省语言能够可靠地追踪内部计算状态，而非仅仅是编造。通过Pull方法识别出自指处理方向，该方向与拒绝方向正交，位于模型深度6.25%处，并能因果性地影响自省输出。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型产生的自省语言是否反映了真实的内部计算过程，还是仅仅是复杂的编造。理解模型自我报告与实际计算状态之间的关系。

Method: 引入Pull方法学，通过格式工程引发扩展的自省过程。在Llama 3.1中识别区分自指处理与描述性处理的方向。分析词汇使用与激活动态之间的相关性，并通过因果干预验证。

Result: 发现自指词汇与并发激活动态之间存在对应关系，且这种对应是自指处理特有的。识别出的自指方向与拒绝方向正交，位于模型深度6.25%处。当模型产生"loop"词汇时，激活表现出更高的自相关性；当产生"shimmer"词汇时，激活变异性增加。同样的词汇在非自指语境中尽管频率高9倍，却无激活对应关系。Qwen 2.5-32B独立发展出不同的自省词汇追踪不同的激活指标。

Conclusion: 在适当条件下，Transformer模型中的自我报告能够可靠地追踪内部计算状态，表明模型的自省语言确实反映了真实的计算过程而非仅仅是编造。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在复杂推理任务中表现下降，主要由于幻觉和中间步骤错误积累。现有方法尝试通过替换关键令牌来优化推理轨迹，但可靠识别和利用关键令牌仍然具有挑战性。

Method: 提出PPCV框架，分为两个阶段：第一阶段从原始问题生成初始推理路径，通过问题转述识别关键令牌；第二阶段替换关键令牌并生成新的推理路径，通过并行推理过程的一致性验证确定最终答案。

Result: 在主流大语言模型和多个基准测试上的广泛实验表明，PPCV相比基线方法显著提升了模型的推理性能。

Conclusion: PPCV框架通过关键令牌识别和一致性验证机制，有效提升了大语言模型在复杂推理任务中的表现，为解决幻觉和错误积累问题提供了新思路。
...
Method: 提出DiffuTruth框架，将事实核查重新概念化为非平衡热力学过程：事实真相作为生成流形上的稳定吸引子，而幻觉是不稳定的。方法包括：1）生成压力测试：用噪声污染声明，使用离散文本扩散模型重建；2）语义能量度量：使用NLI批评器测量原始声明与重建之间的语义分歧；3）混合校准：融合稳定性信号与判别性置信度。

Result: 在FEVER数据集上实现无监督AUROC 0.725的SOTA性能，比基线提高1.5%；在HOVER多跳数据集上零样本泛化性能优于基线超过4%，证明了热力学真理属性对分布偏移的鲁棒性。

Conclusion: DiffuTruth通过热力学视角重新概念化事实核查，将事实真相视为生成流形上的稳定吸引子，提供了一种有效的无监督方法来检测LLM幻觉，并在多个数据集上展示了优越的性能和泛化能力。
...
TL;DR: 本文介绍了一种用于评估医疗对话AI的患者模拟器，通过生成真实可控的患者交互，系统性地评估AI助手的性能、识别幻觉和错误，并分析不同患者群体的风险模式。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏可扩展、自动化的方法来评估医疗对话AI在不同患者群体中的表现。需要一种能够系统性地测试AI助手在各种医疗、语言和行为维度上性能的工具，以识别潜在风险和改进机会。

Method: 基于NIST AI风险管理框架，构建包含三个维度患者特征的模拟器：1) 基于"All of Us"研究计划电子健康记录的医疗档案；2) 模拟健康素养和疾病特定沟通模式的语言档案；3) 基于经验观察的互动行为档案（合作、分心、对抗性参与）。使用该模拟器评估抗抑郁药选择AI决策助手的错误。

Result: 生成了500个模拟器与AI决策助手的对话，人类标注者对100个对话中的1,787个医疗概念进行评估，达成高一致性（F1=0.94，κ=0.73）。LLM评估者与人类标注者达成可比的一致性（F1=0.94，κ=0.78）。模拟器揭示了AI性能随健康素养下降而单调恶化：排名第一的概念检索准确率从有限健康素养的47.9%增加到功能性健康素养的69.1%，再到熟练健康素养的81.6%。

Conclusion: 该患者模拟器提供了一种可扩展、自动化的方法来评估医疗对话AI，能够系统性地识别AI助手的错误和风险模式，特别是在不同健康素养水平的患者群体中。这有助于改进AI系统的公平性和可靠性。
...
TL;DR: 该研究探索了指令调优的大语言模型在检测机器翻译中关键错误（如事实扭曲、意图反转或偏见翻译）的能力，发现模型规模扩展和适应策略能显著提升检测性能，超越传统编码器基线模型。


<details>
  <summary>Details</summary>
Motivation: 机器翻译中的关键错误（事实扭曲、意图反转、偏见翻译）会损害多语言系统的可靠性、公平性和安全性，特别是在高风险或代表性不足的语境中。需要开发有效的错误检测方法来减少错误信息、误解和语言伤害的风险。

Method: 使用指令调优的大语言模型（LLMs）来检测机器翻译中的关键错误，评估了不同参数规模的模型，采用了多种适应策略（零样本、少样本、微调），并在公开可访问的数据集上进行评估。

Result: 模型规模扩展和适应策略（零样本、少样本、微调）带来了一致的性能提升，超越了XLM-R和ModernBERT等仅编码器的基线模型。指令调优的LLMs在检测关键翻译错误方面表现出色。

Conclusion: 改进机器翻译中的关键错误检测有助于构建更安全、更可信、更具社会责任感的信息系统。错误检测不仅是技术挑战，更是追求公正和负责任的多语言AI的必要保障措施。
...
TL;DR: LoopFormer：一种通过可变长度轨迹训练和捷径一致性训练方案，使循环Transformer能够根据计算预算自适应调整计算深度的模型
...
Method: 1. 在可变长度轨迹上训练循环Transformer；2. 引入捷径一致性训练方案，对齐不同长度的轨迹；3. 每个循环基于当前时间和步长进行条件化，确保表示在不同长度轨迹上一致演化

Result: LoopFormer在语言建模和推理基准测试中表现出稳健性能，即使在严格计算约束下也能良好工作，并能随计算预算增加而优雅扩展
...
TL;DR: ADRD-Bench是首个专门针对阿尔茨海默病及相关痴呆症的LLM评估基准，包含1352个临床知识问题和149个照护实践问题，评估了33个先进LLM，结果显示顶级模型准确率超过0.9，但推理质量和稳定性仍需改进。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估基准对阿尔茨海默病及相关痴呆症（ADRD）的覆盖不足，缺乏实际照护情境的评估，需要建立专门的ADRD评估基准来填补这一空白。

Method: 构建ADRD-Bench基准数据集，包含两个部分：1) ADRD统一问答：整合7个现有医学基准的1352个问题；2) ADRD照护问答：基于Aging Brain Care项目创建149个实际问题。评估了33个先进LLM的性能。

Result: 开放权重通用模型准确率0.63-0.93（均值0.78），开放权重医学模型准确率0.48-0.93（均值0.82），闭源通用模型准确率0.83-0.91（均值0.89）。顶级模型准确率超过0.9，但案例研究显示推理质量和稳定性不一致。

Conclusion: ADRD-Bench填补了ADRD领域LLM评估的空白，虽然顶级模型表现良好，但推理质量和稳定性仍需改进，需要基于日常照护数据的领域特定优化来提升LLM的可靠性和实用性。
...
TL;DR: 研究发现语音语言模型在音频与文本冲突时，会过度偏向文本（16.6%文本主导），远高于纯文本冲突时的偏向（1.6%），即使明确指示信任音频也是如此。这种文本主导源于模型推理过程中的可访问性不对称，而非信息内容差异。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解语音语言模型在处理音频-文本冲突时的行为模式。当音频和文本信息不一致时，模型表现出明显的文本偏向，即使音频质量更高。这揭示了模型在跨模态信息整合中的系统性偏差，需要深入探究其根本原因。

Method: 使用ALME基准测试，包含57,602个受控的音频-文本冲突刺激，涵盖8种语言。通过对比音频-文本冲突与文本-文本冲突情况，分析模型行为。采用多种干预实验：强制转录、文本标记为"故意损坏"、音频投影层微调、语言模型LoRA微调等，以定位文本主导的来源。

Result: Gemini 2.0 Flash在音频-文本冲突时表现出16.6%的文本主导，而文本-文本冲突时仅为1.6%。音频单独准确率（97.2%）高于级联准确率（93.9%），表明音频嵌入比文本转录保留更多信息。强制转录增加文本主导（19%到33%），标记文本为"故意损坏"减少文本主导80%。微调实验显示文本主导主要源于语言模型的推理过程。

Conclusion: 文本主导反映了模型在仲裁竞争表征时的可访问性不对称，而非信息内容差异。模态仲裁是语音语言模型中一个独特的可靠性维度，未被标准语音基准测试捕捉。研究为理解多模态模型的信息整合机制提供了新视角。
...
Method: 提出MuRGAt基准，要求模型在输入包含视频、音频等多种模态的情况下，生成带有明确推理和精确引用的答案，每个引用需指定模态和时间段。同时开发了一个与人类判断高度相关的自动评估框架。

Result: 评估显示，即使是强大的多模态大语言模型也经常产生虚假引用，尽管推理正确。存在一个关键权衡：增加推理深度或强制结构化归因通常会降低准确性，揭示了内部推理与可验证归因之间的显著差距。

Conclusion: MuRGAt基准揭示了当前多模态大语言模型在复杂推理场景中的归因能力存在显著不足，需要在保持推理质量的同时提高引用的准确性和可靠性，这是未来多模态模型发展的重要方向。
...
TL;DR: SPES是一个用于预训练MoE大语言模型的内存高效去中心化框架，通过在各个节点上只训练专家子集来降低内存占用，并引入专家合并预热策略加速收敛。


<details>
  <summary>Details</summary>
Motivation: 传统LLM预训练需要集中式集群和大量高内存GPU，现有去中心化方法虽然减少了通信开销，但仍需在每个节点上训练整个模型，受限于GPU内存限制。

Method: 提出SPES框架：1）每个节点只训练专家子集，大幅降低内存占用；2）节点定期同步本地专家更新，避免全参数传输；3）引入专家合并预热策略，在训练早期促进专家间知识交换以加速收敛。

Result: 使用16个48GB GPU在互联网连接上训练了2B参数的MoE LLM，性能与类似计算预算下的集中训练LLM相当；进一步展示了可扩展性：从头训练7B模型和从密集检查点上训练的9B模型均达到先前集中式基线的性能。

Conclusion: SPES为预训练MoE LLMs提供了一种内存高效的去中心化解决方案，能够在资源受限的环境中实现与集中式训练相当的性能，并具有良好的可扩展性。
...
TL;DR: PRIME是一个用于评估数学和工程领域过程-结果对齐验证的基准测试，包含2530个高难度STEM问题样本，发现当前验证器常忽略推导过程中的错误，并提出基于过程感知的RLVR训练范式，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于模型的验证器主要关注最终结果与真实答案的一致性，而忽略了推导过程中的潜在错误，导致错误推导产生正确结果时仍获得正奖励。需要填补这一研究空白。

Method: 引入PRIME基准测试，通过一致性过滤流程从大学级STEM问题中收集2530个高难度样本；提出基于过程感知的RLVR训练范式，利用PRIME筛选验证器。

Result: 当前验证器经常无法检测推导缺陷；基于过程感知的RLVR训练显著优于仅关注结果的基线，在Qwen3-14B-Base模型上，AIME24、AIME25和Beyond-AIME分别获得8.29%、9.12%和7.31%的绝对性能提升；PRIME验证器准确度与RLVR训练效果呈强线性相关（R² > 0.92）。

Conclusion: PRIME是一个可靠的验证器选择预测指标，过程感知的验证方法能有效提升RLVR训练效果，填补了当前验证范式只关注结果而忽略过程的不足。
...
TL;DR: 该论文提出了一个双层次框架，通过前缀保护和难度感知的压缩机制，解决语言推理模型中的"过度思考"问题，在减少推理长度55.7%的同时提升准确率4.1%。


<details>
  <summary>Details</summary>
Motivation: 现有语言推理模型（LRMs）通过增加推理时间计算来提升性能，但存在"过度思考"问题，产生过长的推理轨迹，增加了延迟和内存使用。现有方法使用统一的长度惩罚，导致在序列层面过度压缩关键的早期推理步骤，在群体层面不加区分地惩罚所有查询。

Method: 提出了一个双层次框架：1）序列层面：使用前缀保护优化，通过衰减混合rollouts保持有效推理路径的同时促进简洁性；2）群体层面：难度感知惩罚，根据查询复杂度动态调整长度约束，对困难问题保持探索，对简单问题减少冗余。

Result: 在DeepSeek-R1-Distill-Qwen（1.5B/7B）上的实验表明，该方法在数学基准测试中实现了高达55.7%的token使用减少，同时准确率提升高达4.1%，并且在代码、科学和通用领域具有泛化能力。

Conclusion: 该双层次框架通过前缀保护和难度感知压缩，有效解决了语言推理模型的"过度思考"问题，在保持甚至提升性能的同时显著减少了推理长度，具有良好的泛化性能。
...
TL;DR: LLM生成的反馈在教育中有效，但反馈元素（如语气、信息覆盖）对学习效果和接受度的影响尚不明确，且可能因学习者人格特质而异。研究通过实验发现有效反馈元素有共同模式，但学习者主观偏好因人格特质而异，强调个性化反馈设计的重要性。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在教育反馈生成方面显示出潜力，但具体反馈元素（如语气、信息覆盖范围）如何影响学习效果和学习者接受度仍不清楚，特别是对于不同人格特质的学习者。需要研究这些元素的作用机制，以便设计更有效的个性化反馈系统。

Method: 定义了六个反馈元素，使用GPT-5为多项选择生物学问题生成反馈。对321名高一学生进行学习实验，使用两种学习效果测量方法和六个标准的主观评估。进一步分析基于大五人格特质的反馈接受度差异。

Result: 研究发现有效的反馈元素在支持学习效果方面有共同模式，但学习者的主观偏好因人格特质聚类而异。不同人格特质的学习者对反馈的接受度存在差异。

Conclusion: 设计LLM生成的反馈时，需要根据学习者的人格特质选择和调整反馈元素。研究结果为教育中的个性化反馈设计提供了实际启示，强调了个性化反馈的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在角色扮演应用中的普及，模拟患者成为培训咨询师和扩展治疗评估的重要工具。但现有方法存在碎片化问题：依赖不兼容、非标准化的数据格式、提示和评估指标，阻碍了可重复性和公平比较。

Method: 引入PatientHub统一框架，标准化模拟患者的定义、组合和部署。通过实现多个代表性患者模拟方法作为案例研究，展示框架如何支持标准化跨方法评估和自定义评估指标的无缝集成。还通过原型化两种新的模拟器变体展示扩展性。

Result: PatientHub将现有工作整合到单一可重复的流程中，降低了开发新模拟方法的门槛，促进了跨方法和跨模型的基准测试。框架为未来患者中心对话的数据集、方法和基准提供了实用基础。

Conclusion: PatientHub通过标准化和模块化设计解决了模拟患者领域的碎片化问题，加速了方法开发，支持公平比较，为未来研究提供了可扩展的基础设施。
...
TL;DR: 研究评估了语义异常数据集中的句子是否真正无意义而非仅仅是异常，并测试了LLM区分两者的能力。发现大多数句子只是异常而非完全无意义，LLM能有效为异常句子生成合理上下文。


<details>
  <summary>Details</summary>
Motivation: 语义异常句子在计算语义解释模型发展中很重要，但现有数据集中的句子是否真正无意义而非仅仅是异常尚不明确，且不清楚LLM能否有效区分这两种情况。

Method: 收集人类评分者和LLM对五个语义异常数据集中句子的意义性判断，包括无上下文和有上下文两种情况。评估LLM为异常句子生成合理上下文的能力。

Result: 评分者认为大多数句子最多只是异常，只有少数是真正无意义的。LLM在生成异常句子的合理上下文方面表现出相当强的能力。

Conclusion: 现有语义异常数据集中的句子大多只是异常而非完全无意义，LLM能够有效区分这两种情况并生成支持性上下文，这对语义理解模型的开发有重要意义。
...
TL;DR: 提出Thinking with Drafting (TwD)方法，通过领域特定语言作为中间表示，将视觉推理重构为光学解压缩过程，解决多模态大语言模型在复杂推理任务中的精度悖论问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在视觉感知和生成方面取得进展，但在复杂推理任务中存在精度悖论：光学感知系统仅转录符号而不捕获逻辑拓扑，像素级生成模型产生缺乏数学精确性的视觉伪影。需要弥合这一差距。

Method: 提出Thinking with Drafting (TwD)方法，基于"解析即推理"的公理，使用极简领域特定语言(DSL)作为基础中间表示。强制模型将其心智模型草拟为可执行代码，生成确定性视觉证明进行自我验证。

Result: 实验使用VisAlg视觉代数基准测试，证明TwD作为优越的认知支架。建立了闭环系统，其中视觉生成不是创造性输出而是逻辑验证器。

Conclusion: TwD为视觉推理提供了一条可泛化的路径，将视觉推理重新概念化为光学解压缩过程，其中视觉生成作为逻辑验证器而非创造性输出。
...
TL;DR: 论文提出Length-Incentivized Exploration方法，通过长度奖励和冗余惩罚激励模型进行上下文探索，解决自回归生成中状态覆盖不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有模型在测试时扩展中存在"浅层探索陷阱"：虽然更广泛的状态覆盖需要更长的推理轨迹，但自回归生成中采样长序列的概率呈指数衰减，限制了模型的上下文探索能力。

Method: 提出Length-Incentivized Exploration方法，通过长度奖励鼓励模型生成更长的探索序列，同时引入冗余惩罚避免重复探索，以最大化状态覆盖。

Result: 在Qwen3、Llama等不同模型上的实验表明，该方法能有效激励上下文探索，在领域内任务上平均提升4.4%，在领域外基准上提升2.7%。

Conclusion: 通过简单的长度奖励和冗余惩罚机制，可以显著提升模型的上下文探索能力，从而改善测试时扩展性能。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型向超长上下文应用发展时面临Transformer架构的高计算和内存成本挑战。现有的稀疏和线性注意力机制通常需要在内存效率和模型性能之间做出权衡。

Method: 提出9B参数的混合架构MiniCPM-SALA，集成InfLLM-V2稀疏注意力的高保真长上下文建模和Lightning Attention线性注意力的全局效率。采用层选择算法以1:3比例集成这两种机制，并使用混合位置编码(HyPE)。同时提出成本效益高的持续训练框架，将预训练的Transformer模型转换为混合模型。

Result: 在单张NVIDIA A6000D GPU上，模型在256K token序列长度下达到全注意力模型3.5倍的推理速度，支持高达100万token的上下文长度（传统8B全注意力模型因内存限制无法达到）。训练成本比从头训练降低约75%，同时保持与全注意力模型相当的一般能力。

Conclusion: MiniCPM-SALA成功解决了长上下文建模中效率与性能的权衡问题，通过混合架构实现了高效的长上下文处理，为超长上下文应用提供了实用的解决方案。
...
TL;DR: 提出一种基于嵌入的方法，无需依赖预先标准化或变体列表即可检测语言变体，通过子词嵌入和相似度聚类分析拼写和形态多样性。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要预先标准化或变体列表，难以处理"噪声"或低资源语言环境中的语言变体，特别是在多语言和小语言背景下。

Method: 在原始文本上训练子词嵌入，通过余弦相似度和n-gram相似度组合对相关形式进行分组，将拼写和形态多样性作为语言结构而非噪声进行分析。

Result: 在卢森堡语用户评论语料库中发现了大量词汇和正字法变体，这些变体与方言和社会语言学研究描述的模式一致，诱导出的词族捕获了系统性对应关系并突出了区域和风格差异。
...
TL;DR: DMAP是一种将文本通过语言模型映射到单位区间样本的数学方法，能够同时编码排名和概率信息，支持模型无关的文本分析。


<details>
  <summary>Details</summary>
Motivation: 当前基于困惑度等指标的语言模型文本分析方法无法充分考虑上下文信息，需要一种能更好地解释下一个token概率的方法，同时考虑条件分布形状所编码的合理选择数量。

Method: 提出DMAP方法，将文本通过语言模型映射到单位区间的一组样本，这些样本共同编码排名和概率信息，支持高效、模型无关的分析。
...
TL;DR: 论文提出了RouterXBench评估框架和ProbeDirichlet路由器，用于优化本地小模型与云端大模型之间的查询路由，通过内部隐藏状态和可学习狄利克雷分布实现更好的路由性能。


<details>
  <summary>Details</summary>
Motivation: 现有路由器评估方法不系统，忽视了场景特定需求和分布外鲁棒性。大语言模型虽成功，但成本和隐私限制需要本地部署小模型，同时将复杂查询卸载到云端模型。

Method: 提出RouterXBench评估框架，包含三个维度：路由器能力、场景对齐和跨域鲁棒性。引入ProbeDirichlet路由器，利用内部隐藏状态捕捉模型不确定性，通过可学习狄利克雷分布聚合跨层隐藏状态，采用概率训练方法。

Result: ProbeDirichlet在路由器能力和高精度场景中分别比最佳基线相对提升16.68%和18.86%，在不同模型家族、模型规模、异构任务和智能体工作流中表现一致。

Conclusion: RouterXBench提供了系统化的路由器评估框架，ProbeDirichlet通过内部隐藏状态和概率方法实现了更优的路由性能，在多种场景下展现出强大的泛化能力。
...
TL;DR: 研究发现大型语言模型在基准测试中虽然准确率相近，但在16-66%的项目上存在分歧，这种隐藏的分歧会影响科学研究结果的可重复性


<details>
  <summary>Details</summary>
Motivation: 当前研究依赖基准测试来衡量大型语言模型的进步，但作者发现表面上的准确率趋同可能掩盖了深层的认知分歧，这可能影响科学研究的可重复性

Method: 使用MMLU-Pro和GPQA两个主要推理基准，分析不同LLM在达到相近准确率时的分歧程度；重新分析已发表的教育和政治科学研究，评估模型选择对研究结果的影响

Result: LLM在16-66%的项目上存在分歧，前沿模型之间也有16-38%的分歧；在科学研究中，切换标注模型可使估计的治疗效果变化超过80%，有时甚至改变结果的正负号

Conclusion: 存在"基准测试幻觉"现象，即相等的准确率可能掩盖了模型间的分歧，模型选择成为影响科学可重复性的隐藏但关键变量
...
TL;DR: AdaptEvolve：一种基于生成置信度的自适应LLM选择方法，用于进化式序列精炼框架，通过动态选择合适能力的LLM来平衡计算效率与推理能力，平均减少37.9%推理成本同时保持97.5%的上限准确率。
...
Method: 提出AdaptEvolve方法，在进化式序列精炼框架中，利用内在生成置信度来估计实时可解性，实现基于置信度的自适应LLM选择。该方法通过模型级联机制，根据当前生成步骤的实际需求动态选择合适规模的LLM。

Result: 实验结果表明，置信度驱动的选择产生了有利的帕累托前沿，在多个基准测试中平均减少总推理成本37.9%，同时保持了静态大模型基线97.5%的上限准确率。

Conclusion: AdaptEvolve通过基于生成置信度的自适应LLM选择，有效解决了进化式智能体系统中计算效率与推理能力之间的权衡问题，为多LLM进化精炼提供了实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前端到端语音翻译模型主要在"干净"数据集上评估，忽视了真实世界中的形态变化挑战（如非母语或方言语音中的屈折变化）。现有模型对此类对抗攻击高度脆弱，而生成对抗语音数据成本高昂且技术挑战大。

Method: 提出跨模态鲁棒性迁移（CMRT）框架，将文本领域的对抗攻击方法适配到语音领域，通过从文本模态迁移对抗鲁棒性到语音模态，避免了生成对抗语音数据的需求。

Result: 在四个语言对上的实验表明，CMRT平均提升对抗鲁棒性超过3个BLEU分数，为无需对抗语音数据的鲁棒端到端语音翻译建立了新基准。

Conclusion: CMRT框架有效解决了端到端语音翻译模型的形态鲁棒性问题，通过跨模态鲁棒性迁移避免了昂贵的对抗语音数据生成，显著提升了模型在真实场景中的实用性。
...
TL;DR: 研究发现QA基准测试中16%-50%的问题存在定义不明确的问题，这是导致LLMs表现不佳的重要原因，而非模型本身能力不足。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在定义明确的问题上表现良好，但标准QA基准测试仍未解决。研究者认为这种差距部分源于定义不明确的问题——那些没有额外上下文就无法唯一确定解释的查询。

Method: 引入基于LLM的分类器来识别定义不明确的问题，应用于多个广泛使用的QA数据集。通过受控重写实验，将定义不明确的问题重写为完全定义明确的变体，同时保持正确答案不变，作为上限分析。

Result: 在多个QA数据集中发现16%到超过50%的基准问题是定义不明确的，LLMs在这些问题上表现显著更差。在受控重写实验中，QA性能持续改善，表明许多明显的QA失败源于问题定义不明确而非模型限制。

Conclusion: 定义不明确是QA评估中的重要混淆因素，研究结果强调了在基准设计中需要更加关注问题清晰度的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 近年来开源大语言模型在多语言能力方面不断进步，本文旨在研究开源LLMs在多语言机器翻译中的应用，探索模型规模和数据规模对翻译性能的影响
...
Conclusion: 开源大语言模型通过适当的模型规模和数据规模扩展，结合持续预训练和指令微调，可以在多语言机器翻译任务中达到与顶级专有系统竞争的翻译质量
...
TL;DR: 研究小型语言模型在预训练中应该学习哪些token，哪些应该通过<CALL>委托给外部资源，提出基于语法解析的LaCy方法提高事实准确性


<details>
  <summary>Details</summary>
Motivation: 小型语言模型参数容量有限，容易产生事实错误，需要决定哪些token应该学习，哪些应该委托给外部资源（如大模型、文档或数据库）

Method: 提出LaCy方法，使用spaCy语法解析器增强损失信号，区分哪些token应该学习，哪些应该通过<CALL>委托，防止事实错误

Result: LaCy模型成功学会了哪些token应该预测，哪些应该委托，在与大模型级联生成时获得更高的FactScore，优于Rho或LLM-judge训练的SLM

Conclusion: 通过语法解析增强损失信号的方法能有效指导SLM在预训练中做出正确的token学习/委托决策，提高事实准确性且更简单廉价
...
<details>
  <summary>Details</summary>
Motivation: 在临床Text-to-SQL应用中，需要区分输出多样性的两种不同原因：需要澄清的输入歧义性和需要人工审查的模型不稳定性，现有方法无法提供这种诊断性分解。

Method: 提出CLUES框架，将Text-to-SQL建模为两阶段过程（解释→答案），通过语义不确定性分解为歧义性分数和不稳定性分数，其中不稳定性分数通过二分语义图矩阵的Schur补计算。

Result: 在AmbigQA/SituatedQA（黄金解释）和临床Text-to-SQL基准（已知解释）上，CLUES在失败预测方面优于最先进的核语言熵方法，在部署环境中保持竞争力同时提供单分数无法获得的诊断分解。

Conclusion: CLUES的歧义性/不稳定性分解能够映射到针对性干预措施：歧义性触发查询优化，不稳定性触发模型改进。高歧义/高不稳定性区域覆盖了51%的错误但仅占25%的查询，实现了高效分流。
...
<details>
  <summary>Details</summary>
Motivation: 大规模可验证提示对RLVR的成功至关重要，但包含大量无信息示例且扩展成本高。现有方法主要关注通过率为0的困难提示，但随着训练进行，通过率为1的简单提示越来越多，降低了有效数据规模

Method: 提出Composition-RL方法，自动将多个问题组合成新的可验证问题，利用这些组合提示进行RL训练。还设计了课程变体，在训练过程中逐步增加组合深度

Result: 在4B到30B不同模型规模上的实验表明，Composition-RL相比原始数据集上的RL训练持续提升推理能力。课程变体进一步提升了性能，且能实现更有效的跨领域RL

Conclusion: Composition-RL是一种简单有效的方法，通过组合简单提示来更好地利用有限的可验证提示数据，提升强化学习的推理能力
...
TL;DR: DeepSight是一个开源项目，通过集成安全评估和诊断工具，将大模型安全评估从黑盒转向白盒洞察，解决现有安全流程中评估、诊断、对齐分离的问题。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型和多模态大语言模型的安全工作流程中，评估、诊断和对齐通常由独立工具处理。安全评估只能定位外部行为风险而无法找出内部根本原因，安全诊断往往脱离具体风险场景停留在可解释层面，导致安全对齐缺乏内部机制变化的专门解释，可能降低模型通用能力。

Method: 提出DeepSight开源项目，包含评估工具包DeepSafe和诊断工具包DeepScan。通过统一任务和数据协议，在两个阶段之间建立连接，将安全评估从黑盒转向白盒洞察。这是首个支持前沿AI风险评估以及联合安全评估和诊断的开源工具包。

Result: DeepSight是一个低成本、可复现、高效且高度可扩展的大规模模型安全评估项目，实现了评估-诊断集成的新范式。

Conclusion: DeepSight通过集成安全评估和诊断，系统性地解决了大模型安全工作中评估、诊断、对齐分离的问题，提供了从黑盒到白盒的完整安全洞察。
...
TL;DR: P-GenRM：首个具有测试时用户扩展能力的个性化生成奖励模型，通过结构化评估链和用户原型聚类解决个性化LLM对齐中的奖励信号获取问题


<details>
  <summary>Details</summary>
Motivation: 现有个性化奖励模型存在两个主要局限：1）将多样化的场景特定偏好过度简化为少量固定评估原则；2）对有限反馈的新用户泛化能力不足。需要更灵活、可扩展的个性化奖励建模方法

Method: 提出P-GenRM模型，将偏好信号转化为结构化评估链，生成自适应角色和评分标准。通过用户原型聚类和双粒度扩展机制：个体层面自适应扩展聚合用户评分方案；原型层面整合相似用户偏好，减少偏好推断噪声并增强泛化能力

Result: 在广泛使用的个性化奖励模型基准测试中达到最先进水平，平均提升2.31%；在分布外数据集上表现出强泛化能力；测试时用户扩展机制额外提供3%性能提升，展示更强的个性化对齐和测试时可扩展性

Conclusion: P-GenRM通过结构化评估链、用户原型聚类和双粒度扩展机制，有效解决了个性化LLM对齐中的奖励信号获取问题，实现了更好的个性化对齐和泛化能力，为个性化AI系统提供了有前景的解决方案
...
TL;DR: 利用Wiktionary数据构建基于规则的盖尔语形态学模型，解决低资源语言训练数据不足问题


<details>
  <summary>Details</summary>
Motivation: 当前流行的神经模型需要大量训练数据，而低资源语言（如盖尔语）通常缺乏足够数据。需要开发能够有效利用有限样本数据的替代方法。

Method: 使用Wiktionary数据构建基于规则的盖尔语形态学模型，采用SQL查询不同词汇模式的出现情况，并开发声明式规则库，使Python工具能够推导盖尔语单词的屈折形式。

Result: 提出了一个基于规则的模型，能够有效利用有限的样本数据，支持更好的可解释性，并为教学材料设计提供有用见解。该方法将Wiktionary数据适配到新的应用场景中。

Conclusion: 基于规则的系统能够有效解决低资源语言的形态学建模问题，支持教育工具和更高级的语言处理工具开发，为Wiktionary数据增加了新的应用价值。
...
TL;DR: WavBench是一个全面的语音对话基准测试，旨在评估现实对话能力，包含三个子集：Pro（增强推理能力）、Basic（口语化标准）和Acoustic（副语言能力），填补了现有评估忽视音频特征和认知深度的空白。


<details>
  <summary>Details</summary>
Motivation: 当前语音对话模型的评估主要遵循文本生成标准，忽视了副语言特征、口语化表达以及现代智能体所需的认知深度，无法应对现实世界的复杂性。迫切需要超越简单交互、能够评估真实世界复杂性的基准测试。

Method: 提出了WavBench基准测试，采用三重框架：1) Pro子集：设计用于严格挑战推理增强模型，显著增加难度；2) Basic子集：定义口语化新标准，优先考虑"可听性"，包括自然词汇、语言流畅性和交互融洽度；3) Acoustic子集：覆盖显性理解、生成和隐性对话，在真实场景中严格评估全面的副语言能力。

Result: 通过对五个最先进模型的评估，WavBench提供了关于复杂问题解决、口语化表达和副语言保真度交叉领域的关键见解，指导稳健语音对话模型的发展。

Conclusion: WavBench填补了语音对话评估的重要空白，为评估现实对话能力提供了全面框架，数据集和评估工具包已公开可用，将推动语音对话模型的演进。
...
TL;DR: CitiLink-Minutes：首个多层标注的葡萄牙市政会议记录数据集，包含120份会议纪要、超过100万词元、38,000个标注，支持市政决策透明化与NLP/IR研究
...
Result: 创建了包含超过100万词元、38,000个个体标注的数据集，所有个人标识符均已去标识化，遵循FAIR原则发布，并提供了元数据提取、主题分类和投票标签的基线结果
...
TL;DR: 提出基于注意力得分的轻量级重排序框架，利用大语言模型中特定注意力头的分数估计文档-查询相关性，无需人工标注的连续相关性评分即可训练


<details>
  <summary>Details</summary>
Motivation: 现有重排序方法通常需要人工标注的相关性评分（如Likert量表），限制了在任意检索数据集上的应用。同时，现有方法多为点对点或列表式，未能充分利用候选文档集合的整体信息

Method: 基于大语言模型中特定注意力头的注意力分数，训练模型估计文档-查询相关性。该方法采用列表式解决方案，利用整个候选文档短列表的完整信息进行排序，自然产生连续相关性分数

Result: 方法在多个领域（包括维基百科和长篇叙事数据集）上优于现有的点对点和列表式重排序方法，在LoCoMo基准测试中创造了新的最先进水平。仅需小规模模型（如40亿参数）即可实现强大性能

Conclusion: 提出的基于注意力得分的重排序框架是轻量级且有效的，支持灵活扩展，如通过上下文信息增强候选文档或使用中间层注意力头来提高效率而不牺牲性能
...
TL;DR: 本文介绍了视觉推理基准(VRB)，这是一个评估多模态大语言模型在解决课堂真实视觉问题能力的新数据集，基于赞比亚和印度小学考试的701个问题构建，发现模型在静态技能上表现较好但在动态空间操作上存在明显瓶颈。


<details>
  <summary>Details</summary>
Motivation: 尽管AI模型在文本推理方面取得了先进成果，但在空间和关系结构推理方面仍存在关键瓶颈，特别是在依赖视觉的早期数学教育中。需要评估多模态大语言模型解决课堂真实视觉问题的能力。

Method: 开发了视觉推理基准(VRB)，基于赞比亚和印度小学考试的701个问题构建，涵盖类比推理、模式完成、空间匹配等任务。使用未经编辑、文本最少的图像来测试模型是否能满足小学教育的实际需求。

Result: 研究发现模型能力存在"锯齿状前沿"：在计数和缩放等静态技能上表现较好，但在折叠、反射和旋转等动态操作上达到明显的"空间天花板"。这些弱点可能导致课堂使用中的错误评分、虚假支架和强化学生误解。

Conclusion: 像VRB这样的教育导向基准对于确定课堂中使用的多模态工具的功能边界至关重要，揭示了当前模型在视觉推理方面的局限性，特别是在动态空间操作方面存在显著不足。
...
<details>
  <summary>Details</summary>
Motivation: 现有文档理解基准在多样化文档类型和灵活模式下的细粒度结构化抽取能力评估不足，现有数据集在实体本体、查询复杂性和文档类型多样性方面存在局限，需要更适应性强、结构化的抽取评估框架。

Method: 通过结合人工和合成的人类验证样本的新颖流水线构建ExStrucTiny基准数据集，涵盖更多样化的文档类型和抽取场景，并分析开放和封闭视觉语言模型在该基准上的表现。

Result: 研究揭示了视觉语言模型在模式适应、查询不充分规范和答案定位等方面的挑战，为改进文档结构化信息抽取的通用模型提供了基础。

Conclusion: ExStrucTiny基准为改进文档结构化信息抽取的通用模型提供了重要基础，有助于解决现有模型在多样化文档类型和灵活模式下的细粒度结构化抽取能力不足的问题。
...
TL;DR: 该论文研究软压缩架构中token溢出的检测问题，提出了一种识别压缩导致信息丢失的方法，并展示了结合查询信息的检测器优于仅依赖上下文统计的方法。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在资源受限环境中的长上下文处理效率仍然是一个关键挑战。软压缩架构通过用学习到的压缩token替换长token序列来扩展有效上下文长度，但压缩的极限以及何时压缩开始擦除任务相关内容仍未得到充分探索。

Method: 论文定义了"token溢出"作为压缩表示不再包含足够信息回答给定查询的情况，并提出了一种方法来表征和检测它。在xRAG软压缩设置中，使用查询无关的饱和统计来分离压缩和未压缩token表示，并构建轻量级探测分类器来分析查询和上下文xRAG表示。

Result: 查询无关的饱和统计能可靠地区分压缩和未压缩token表示，但在溢出检测方面能力有限。结合查询和上下文信息的轻量级探测分类器在HotpotQA、SQuADv2和TriviaQA数据集上平均达到0.72 AUC-ROC，表明结合查询信息能显著提高检测性能。

Conclusion: 研究从查询无关诊断推进到查询感知检测器，实现了低成本的预LLM门控机制，能够减轻压缩引起的错误，为软压缩架构的实际应用提供了实用工具。
...
TL;DR: Moonshine v2是一种用于边缘设备流式语音识别的低延迟模型，采用滑动窗口自注意力机制替代全注意力，在保持高准确率的同时大幅降低延迟和计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决边缘设备上延迟关键型语音应用（如实时转录、语音命令、实时翻译）对低首次令牌时间（TTFT）和高准确率的需求。传统全注意力Transformer编码器虽然准确率高，但具有二次复杂度，导致延迟随语音长度线性增长，不适合流式应用。

Method: 引入Moonshine v2模型，采用滑动窗口自注意力机制，实现有界低延迟推理，同时保留强局部上下文。这种设计替代了传统的全注意力机制，避免了"编码整个话语"的延迟特性。

Result: 在标准基准测试中达到最先进的词错误率，与比其大6倍的模型精度相当，同时运行速度显著更快。模型在保持高准确率的同时大幅降低了延迟和计算成本。

Conclusion: 精心设计的局部注意力机制在准确率上可以与全注意力竞争，同时大幅降低模型大小和延迟成本，为边缘设备上的交互式语音界面开辟了新的可能性。
...
Result: 在多个基准测试中，该方法在严格步骤预算下持续优于强少步基线和标准训练，虽然全步解码仍更优，但显著缩小了性能差距。

Conclusion: 为实用少步扩散大语言模型建立了坚实基础，通过轨迹自蒸馏有效提升了少步解码的生成质量。
...
TL;DR: OPCD框架通过策略蒸馏与上下文蒸馏结合，让学生在自身生成的轨迹上训练，最小化与上下文条件教师的逆向KL散度，在数学推理、文本游戏等任务上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 上下文蒸馏能让语言模型将上下文知识内化到参数中，但现有方法存在局限。研究者希望开发一个框架，将策略蒸馏与上下文蒸馏相结合，让模型能从自身经验中提取可迁移知识，并将优化提示中的有益行为内化。

Method: 提出On-Policy Context Distillation (OPCD)框架，让学生在自身生成的轨迹上进行训练，同时最小化与上下文条件教师模型的逆向Kullback-Leibler散度。该方法应用于两个重要场景：经验知识蒸馏（从历史解决方案轨迹中提取和整合可迁移知识）和系统提示蒸馏（将优化提示中的有益行为内化）。

Result: 在数学推理、文本游戏和领域特定任务中，OPCD持续优于基线方法，实现了更高的任务准确率，同时更好地保持了分布外能力。研究还表明OPCD支持有效的跨尺寸蒸馏，让较小的学生模型能够从较大的教师模型中内化经验知识。

Conclusion: OPCD成功地将策略蒸馏与上下文蒸馏相结合，为语言模型从自身经验和优化提示中提取并内化知识提供了有效框架，在多个任务上表现出优越性能，并支持模型间的知识迁移。
...
### [122] [BSoNet: Deep Learning Solution for Optimizing Image Quality of Portable Backscatter Imaging Systems](https://arxiv.org/abs/2602.11701)
*Linxuan Li,Wenjia Wei,Yunfei Lu,Wenwen Zhang,Yanlong Zhang,Wei Zhao*

Main category: eess.IV

TL;DR: BSoNet：一种基于深度学习的方法，专门用于优化便携式背散射成像系统的图像质量，解决光子计数不足导致的严重噪声和低信噪比问题。


<details>
  <summary>Details</summary>
Motivation: 便携式背散射成像系统虽然具有灵活性和便携性优势，但由于康普顿背散射光子数量有限，导致图像质量严重受损，存在严重噪声和极低信噪比，影响了系统的准确性和可靠性。

Method: 提出BSoNet，一种新颖的基于深度学习的方法，专门设计用于优化便携式背散射成像系统的图像质量。

Result: 该方法显著增强了图像清晰度、识别度和对比度，同时满足实际应用需求，将便携式背散射成像系统转变为更有效和可靠的检测工具。

Conclusion: BSoNet方法显著提升了便携式背散射成像系统的图像质量，增强了安全防护能力，使该系统成为更有效的安全检查工具。
...
### [123] [Learning Perceptual Representations for Gaming NR-VQA with Multi-Task FR Signals](https://arxiv.org/abs/2602.11903)
*Yu-Chih Chen,Michael Wang,Chieh-Dun Wen,Kai-Siang Ma,Avinab Saha,Li-Heng Chen,Alan Bovik*

Main category: eess.IV

TL;DR: MTL-VQA：一种用于游戏视频的无参考质量评估多任务学习框架，通过使用全参考指标作为监督信号进行预训练，无需人工标注
...
Method: 提出多任务学习框架MTL-VQA，使用全参考指标作为监督信号学习感知有意义的特征，通过自适应任务加权联合优化多个全参考目标，学习可有效迁移到无参考质量评估的共享表示

Result: 在游戏视频数据集上的实验表明，MTL-VQA在MOS监督和标签高效/自监督设置下，性能均与最先进的无参考质量评估方法相当

Conclusion: MTL-VQA通过多任务学习利用全参考指标作为监督信号，有效解决了游戏视频无参考质量评估的数据稀缺问题，实现了竞争性的性能表现
...
### [124] [U-DAVI: Uncertainty-Aware Diffusion-Prior-Based Amortized Variational Inference for Image Reconstruction](https://arxiv.org/abs/2602.11704)
*Ayush Varshney,Katherine L. Bouman,Berthy T. Feng*

Main category: eess.IV

TL;DR: 提出一种改进的摊销变分推理框架，通过注入空间自适应扰动来增强细节重建能力，在去模糊和超分辨率任务上取得优异性能


<details>
  <summary>Details</summary>
Motivation: 解决基于扩散的生成先验方法计算成本高的问题，同时克服现有摊销推理框架在细节和纹理重建方面的不足

Method: 扩展摊销推理框架，在训练过程中根据不确定性估计向测量值注入空间自适应扰动，重点学习最不确定区域

Result: 在去模糊和超分辨率任务上，相比之前的基于扩散的方法，取得了更优或具有竞争力的性能，提供更真实的重建结果

Conclusion: 提出的方法在保持摊销推理计算效率优势的同时，显著提升了细节重建质量，为图像逆问题提供了高效且高质量的解决方案
...
### [125] [UPDA: Unsupervised Progressive Domain Adaptation for No-Reference Point Cloud Quality Assessment](https://arxiv.org/abs/2602.11969)
*Bingxu Xie,Fang Zhou,Jincan Wu,Yonghui Liu,Weiqing Li,Zhiyong Su*

Main category: eess.IV

TL;DR: 本文提出了首个无监督渐进式域适应（UPDA）框架用于无参考点云质量评估，通过粗粒度到细粒度的两阶段对齐方法解决跨域性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 当前无参考点云质量评估方法在训练域和测试域存在分布差异时性能显著下降，但跨域迁移问题尚未得到足够关注，需要解决这一挑战。

Method: 提出两阶段渐进式域适应框架：1）粗粒度阶段使用差异感知的粗粒度对齐方法，通过质量差异感知混合损失捕捉跨域样本的相对质量关系；2）细粒度阶段采用感知融合细粒度对齐方法，通过对称特征融合识别域不变特征，并使用条件判别器选择性增强质量相关特征的迁移。

Result: 大量实验表明，UPDA框架能有效提升无参考点云质量评估方法在跨域场景下的性能，验证了其实际应用价值。

Conclusion: 该研究首次解决了无参考点云质量评估的跨域适应问题，提出的UPDA框架通过渐进式两阶段对齐策略成功缓解了域偏移问题，为实际应用提供了有效解决方案。
...
### [126] [Temporally resolved aortic 3D shape reconstruction from a limited number of cine 2D MRI slices](https://arxiv.org/abs/2602.11873)
*Gloria Wolkerstorfer,Stefano Buoso,Rabea Schlenker,Jochen von Spiczak,Robert Manka,Sebastian Kozerke*

Main category: eess.IV

TL;DR: 提出一种从少量标准2D MRI切片重建时间分辨3D主动脉几何形状的框架，结合统计形状模型和可微分体积网格优化，在30名受试者中验证了准确性。


<details>
  <summary>Details</summary>
Motivation: 传统方法需要大量成像数据来重建3D主动脉几何形状，本研究旨在开发一种从有限数量的标准2D MRI切片中高效提取时间分辨3D主动脉网格的方法，适用于计算形状和应变分析。

Method: 构建主动脉统计形状模型，优化2D切片放置位置；在30名受试者（19名志愿者，11名主动脉狭窄患者）中采集2D MRI切片；通过手动分割后，使用可微分体积网格优化生成时间分辨主动脉模型，提取血管形状特征、中心线参数和径向壁应变；在10名受试者中额外采集4D流MRI用于峰值收缩期形状比较。

Result: 仅用六个2D MRI切片即可获得解剖学准确的主动脉几何形状，相对于4D流MRI的Dice得分为(89.9±1.6)%，IoU为(81.7±2.7)%，Hausdorff距离为(7.3±3.3)mm，Chamfer距离为(3.7±0.6)mm；平均绝对半径误差为(0.8±0.6)mm；所有形状特征（包括径向应变）均观察到显著的年龄相关差异。

Conclusion: 该方法能够从有限的标准2D MRI采集数据中高效提取时间分辨3D主动脉网格，适用于计算形状和应变分析，为临床评估提供了有效工具。
...
### [127] [Explaining AI Without Code: A User Study on Explainable AI](https://arxiv.org/abs/2602.11159)
*Natalia Abarca,Andrés Carvallo,Claudia López Moncada,Felipe Bravo-Marquez*

Main category: cs.AI

TL;DR: 在DashAI无代码ML平台中集成了三种可解释AI技术(PDP、PFI、KernelSHAP)，用户研究显示该方法对新手用户友好且能提高信任度，但面临新手易用性与专家需求深度之间的平衡挑战。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在医疗、金融等敏感领域的应用增加，模型决策透明度问题日益突出。虽然可解释AI(XAI)能够澄清模型预测过程，但现有方法大多需要专业技术知识，限制了非专业用户的使用价值。这一问题在无代码ML平台中尤为关键，这些平台旨在民主化AI但很少包含可解释性功能。

Method: 在开源无代码ML平台DashAI中开发了一个以人为中心的XAI模块，集成了三种互补的可解释技术：部分依赖图(PDP)、排列特征重要性(PFI)和KernelSHAP，专门用于表格分类任务。通过用户研究(N=20，包括ML新手和专家)评估了该模块的可用性和解释效果。

Result: 研究结果显示：(1)在所有可解释性任务中，任务成功率高达80%以上；(2)新手用户在解释满意度量表(ESS，Cronbach's α=0.74)上认为解释有用、准确且可信，而专家对解释的充分性和完整性更为挑剔；(3)在自动化信任量表(TiA，α=0.60)上，解释提高了用户对模型可预测性的感知和信心，新手比专家表现出更高的信任度。

Conclusion: 研究结果突显了无代码ML平台中XAI面临的核心挑战：如何使解释既对新手用户易于理解，又能满足专家对细节深度的需求。DashAI的XAI模块为这一平衡提供了实践方案，但需要继续优化以适应不同用户群体的需求。
...
### [128] [On Decision-Valued Maps and Representational Dependence](https://arxiv.org/abs/2602.11295)
*Gil Raitses*

Main category: cs.AI

TL;DR: 本文提出决策值映射和DecisionDB基础设施，用于记录、重放和审计不同数据表示对计算引擎输出结果的影响，确保决策的可重现性和可验证性。


<details>
  <summary>Details</summary>
Motivation: 相同数据的不同表示形式可能导致计算引擎产生不同的离散结果，需要一种系统方法来跟踪哪些表示形式能保持结果一致、哪些会改变结果，以确保计算决策的可重现性和可验证性。

Method: 提出决策值映射概念，将表示家族中的每个成员与其产生的离散结果关联；开发DecisionDB基础设施，使用内容计算标识符并以只写形式存储工件，支持确定性重放和审计。

Result: 实现了确定性重放，能够从存储的工件中精确恢复每个记录的决策标识符，所有三个标识字段都与持久化值匹配；将表示空间划分为持久性区域和边界。

Conclusion: 该框架将表示空间划分为持久性区域和边界，并将决策重用视为可机械检查的条件，为计算决策的可重现性和验证提供了系统方法。
...
### [129] [Voxtral Realtime](https://arxiv.org/abs/2602.11298)
...
<details>
  <summary>Details</summary>
Motivation: 现有流式ASR方法通常通过对离线模型进行分块或滑动窗口适配，这会影响质量和延迟。需要一种专门为流式处理设计的端到端训练模型，在保持低延迟的同时达到离线转录质量。
...
Conclusion: Voxtral Realtime证明了专门为流式处理设计的端到端训练模型可以在亚秒级延迟下达到离线转录质量，为实时语音识别提供了高质量解决方案。
...
### [130] [Dissecting Subjectivity and the "Ground Truth" Illusion in Data Annotation](https://arxiv.org/abs/2602.11318)
*Sheza Munir,Benjamin Mah,Krisha Kalsi,Shivani Kapania,Julian Posada,Edith Law,Ding Wang,Syed Ishtiaque Ahmed*

Main category: cs.AI

TL;DR: 该论文批判机器学习中"ground truth"范式的实证主义谬误，认为其将人类分歧视为技术噪声而非重要的社会技术信号，通过系统性文献综述揭示了数据标注实践中的"共识陷阱"问题。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习领域普遍将"ground truth"视为客观正确的标签，但这种范式建立在实证主义谬误之上，将人类分歧视为需要消除的技术噪声，而非反映文化多元性和社会复杂性的重要信号。作者旨在揭示数据标注实践中如何形成"共识陷阱"，以及这种范式如何排除人类声音、强化西方霸权。

Method: 采用系统性文献综述方法，分析2020-2025年间在ACL、AIES、CHI、CSCW、EAAMO、FAccT和NeurIPS七个顶级会议发表的论文。通过分层关键词过滤方案从30,897条记录中筛选出3,042条进行人工筛选，最终纳入346篇论文进行定性综合。使用反思性主题分析方法分析数据。

Result: 研究发现：1）系统性位置可读性失败与人类作为验证者模型的架构转变相结合，特别是依赖模型介导的标注，引入了根深蒂固的锚定偏见，有效排除了人类声音；2）地理霸权将西方规范强加为普遍基准，通常通过不稳定数据工作者的表演性对齐来执行，他们为避免经济惩罚而优先考虑请求者合规而非诚实主观性；3）统计模型将文化多元性误诊为随机误差的"噪声传感器"谬误。

Conclusion: 论文主张将分歧重新定义为构建文化能力模型所需的高保真信号，并提出多元标注基础设施的路线图，将目标从发现单一"正确"答案转向映射人类经验的多样性，以解决系统性紧张关系。
...
### [131] [Bi-Level Prompt Optimization for Multimodal LLM-as-a-Judge](https://arxiv.org/abs/2602.11340)
*Bo Pan,Xuan Kan,Kaitai Zhang,Yan Yan,Shunwen Tan,Zihao He,Zixin Ding,Junjie Wu,Liang Zhao*

Main category: cs.AI

TL;DR: BLPO：针对多模态LLM评估的二级提示优化框架，通过将图像转换为文本表示来解决视觉示例数量受限的问题


<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为AI生成内容的自动评估工具已被广泛采用，但将其评估结果与人类判断对齐仍然具有挑战性。现有的自动提示优化方法主要针对纯文本评估，在多模态场景下研究不足，特别是评估AI生成图像时面临视觉示例数量受限的瓶颈

Method: 提出BLPO（Bi-Level Prompt Optimization）框架，采用二级优化方法：1）将图像转换为保留评估相关视觉线索的文本表示；2）联合优化评估提示和图像到文本转换提示，在有限的上下文预算下保持保真度

Result: 在四个数据集和三个LLM评估模型上的实验证明了该方法的有效性，能够显著提升多模态LLM评估与人类判断的对齐程度

Conclusion: BLPO框架成功解决了多模态LLM评估中视觉示例数量受限的问题，通过二级提示优化实现了更高效的多模态评估对齐，为AI生成图像评估提供了有效的自动提示优化解决方案
...
### [132] [AgentNoiseBench: Benchmarking Robustness of Tool-Using LLM Agents Under Noisy Condition](https://arxiv.org/abs/2602.11348)
...
TL;DR: 该论文提出了AgentNoiseBench框架，用于系统评估智能体模型在噪声环境下的鲁棒性，通过分析真实世界噪声类型并自动注入可控噪声到现有基准中，发现当前智能体模型对现实环境扰动高度敏感。
...
Method: 1. 分析真实场景中的偏差和不确定性，将环境噪声分为用户噪声和工具噪声两类；2. 开发自动化管道，在保持任务可解性的前提下向现有智能体基准注入可控噪声；3. 在不同架构和参数规模的模型上进行广泛评估。

Result: 评估结果显示，在不同噪声条件下模型性能存在一致变化，突显了当前智能体模型对现实环境扰动的敏感性，揭示了基准测试与真实部署之间的性能差距。

Conclusion: AgentNoiseBench框架为系统评估智能体鲁棒性提供了有效工具，揭示了当前模型在噪声环境下的脆弱性，为未来开发更鲁棒的智能体系统指明了方向。
...
### [133] [Pushing Forward Pareto Frontiers of Proactive Agents with Behavioral Agentic Optimization](https://arxiv.org/abs/2602.11351)
...
<details>
  <summary>Details</summary>
Motivation: 现有的主动型LLM智能体训练方法面临关键挑战：如何在任务性能与用户参与度之间取得平衡。被动智能体无法有效适应用户意图，而过度使用人类反馈又会降低用户满意度。
...
### [134] [Causal-JEPA: Learning World Models through Object-Level Latent Interventions](https://arxiv.org/abs/2602.11389)
*Heejeong Nam,Quentin Le Lidec,Lucas Maes,Yann LeCun,Randall Balestriero*

Main category: cs.AI

TL;DR: C-JEPA是一种简单灵活的对象中心世界模型，通过对象级掩码将联合嵌入预测从图像块扩展到对象中心表示，促进交互推理并实现高效规划。
...
Method: 提出C-JEPA模型，将掩码联合嵌入预测从图像块扩展到对象中心表示，通过对象级掩码要求从其他对象推断目标对象状态，诱导具有反事实效果的潜在干预。

Result: 在视觉问答中实现约20%的反事实推理绝对改进；在智能体控制任务中，仅需基于图像块世界模型1%的潜在输入特征即可实现相当性能，规划效率显著提升。

Conclusion: C-JEPA通过对象级掩码诱导因果归纳偏置，使交互推理成为必需，在关系理解和高效规划方面表现出色，为对象中心世界模型提供了有效解决方案。
...
### [135] [GHOST: Unmasking Phantom States in Mamba2 via Grouped Hidden-state Output-aware Selection & Truncation](https://arxiv.org/abs/2602.11408)
*Michael Menezes,Anastasios Kyrillidis*

Main category: cs.AI

TL;DR: GHOST是一种针对Mamba2模型的结构化剪枝框架，通过仅使用前向传播统计量近似控制理论平衡截断，在保持模型性能的同时显著减少推理开销。


<details>
  <summary>Details</summary>
Motivation: Mamba2模型扩展的状态维度增强了时序建模能力，但在自回归生成过程中会产生显著的推理开销，导致带宽饱和。传统的剪枝方法无法有效解决这一瓶颈问题。

Method: GHOST框架通过联合测量可控性和可观测性，仅使用前向传播统计量来近似控制理论平衡截断，实现结构化剪枝，无需反向传播。

Result: 在130M到2.7B参数规模的模型上，该方法实现了50%的状态维度减少，在WikiText-2数据集上仅带来约1个困惑度点的增加。

Conclusion: GHOST框架提供了一种高效的结构化剪枝方法，能够显著减少Mamba2模型的推理开销，同时保持模型性能，其效果可与基于梯度的方法相媲美但计算成本更低。
...
### [136] [Distributionally Robust Cooperative Multi-Agent Reinforcement Learning via Robust Value Factorization](https://arxiv.org/abs/2602.11437)
*Chengrui Qu,Christopher Yeh,Kishan Panaganti,Eric Mazumdar,Adam Wierman*

Main category: cs.AI

TL;DR: 该论文提出了分布鲁棒的IGM原则(DrIGM)，用于解决多智能体强化学习在现实环境中的不确定性挑战，通过鲁棒值分解方法提升系统在分布外环境下的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体强化学习中的集中训练分散执行方法虽然采用了IGM原则，但在现实环境中面临模拟到现实的差距、模型不匹配和系统噪声等不确定性挑战，导致可靠性不足。

Method: 提出了分布鲁棒的IGM原则(DrIGM)，定义了鲁棒个体动作值函数，确保每个智能体的鲁棒贪婪动作与团队最优联合动作一致。基于此推导了与现有值分解架构(如VDN/QMIX/QTRAN)兼容的鲁棒变体，使用鲁棒Q目标进行训练。

Result: 在高保真SustainGym模拟器和星际争霸游戏环境中，该方法显著提升了分布外性能，证明了其在实际应用中的有效性。

Conclusion: DrIGM原则为多智能体强化学习提供了理论保证的鲁棒性，其兼容现有架构的实现方式便于实际部署，为解决现实环境中的不确定性挑战提供了有效解决方案。
...
### [137] [Credit Where It is Due: Cross-Modality Connectivity Drives Precise Reinforcement Learning for MLLM Reasoning](https://arxiv.org/abs/2602.11455)
*Zhengbo Jiao,Shaobo Wang,Zifan Zhang,Wei Wang,Bing Zhao,Hu Wei,Linfeng Zhang*

Main category: cs.AI

TL;DR: 该研究探索了多模态强化学习中的视觉证据整合机制，发现只有约15%的token具有强视觉-文本耦合，这些"锚点token"在推理中起关键作用。基于此提出了AT-RL框架，通过选择性强化高连接性token，在低开销下显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 尽管基于可验证奖励的强化学习（RLVR）显著提升了多模态大语言模型（MLLMs）的推理能力，但视觉证据在推理过程中如何整合仍不清楚。研究者希望理解多模态RLVR中视觉-文本交互的本质机制。

Method: 通过跨模态注意力连接性分析发现只有约15%的token具有强视觉-文本耦合。基于此提出锚点token强化学习（AT-RL）框架，使用基于图的注意力拓扑聚类方法，选择性强化高连接性token，而低开销（仅1.2%额外开销）。

Result: AT-RL使32B模型在MathVista上超越72B-Instruct基线（80.2分），在STEM、视频和通用任务上均获得一致提升。相反，仅训练低连接性token会导致性能严重下降，证实有效的多模态RL依赖于对视觉锚点的精确信用分配。

Conclusion: 推理质量不由token数量决定，而由跨模态锚点的保真度决定。只有少数视觉锚点token在多模态推理中起关键作用，精确强化这些锚点能显著提升模型性能，这为高效多模态RL训练提供了新思路。
...
### [138] [AgentLeak: A Full-Stack Benchmark for Privacy Leakage in Multi-Agent LLM Systems](https://arxiv.org/abs/2602.11510)
...
<details>
  <summary>Details</summary>
Motivation: 当前基准测试无法测量多智能体LLM系统的隐私风险，因为敏感数据会通过智能体间消息、共享内存和工具参数等内部通道传递，这些是仅关注输出的审计无法检测的

Method: 开发AgentLeak基准测试，包含1000个医疗、金融、法律和企业领域的场景，配备32类攻击分类法和三层检测流水线，测试GPT-4o、GPT-4o-mini、Claude 3.5 Sonnet、Mistral Large和Llama 3.3 70B等模型

Result: 多智能体配置降低了单通道输出泄露率（C1：27.2% vs 单智能体43.2%），但未监控的内部通道使系统总暴露率达到68.9%。智能体间消息（C2）泄露率高达68.8%，而输出通道仅为27.2%，输出审计会遗漏41.7%的违规。Claude 3.5 Sonnet在外部（3.3%）和内部（28.1%）通道上表现最佳
...
### [139] [Human-Inspired Continuous Learning of Internal Reasoning Processes: Learning How to Think for Adaptive AI Systems](https://arxiv.org/abs/2602.11516)
*Hong Su*
...
<details>
  <summary>Details</summary>
Motivation: 现有AI系统主要关注学习任务特定输出或静态知识表示，忽视了内部推理结构、行动调度策略和学习机制本身的持续优化，难以适应动态现实环境。

Method: 提出统一框架，将内部思维过程作为主要学习对象，系统记录内部推理轨迹和环境交互作为结构化学习材料，支持预定义逻辑与学习过程的受控替换，引入分层学习机制联合优化任务级参数和学习策略。

Result: 在温度传感器异常检测任务上的实验表明，引入内部过程学习使平均运行时间减少23.9%，系统在保持操作稳定性的同时逐步演化其内部认知架构。

Conclusion: 该框架通过将内部推理过程作为主要学习对象，实现了处理过程中的学习，使AI系统能够在执行过程中持续优化认知结构，为开发适应动态环境的AI系统提供了新途径。
...
### [140] [CausalAgent: A Conversational Multi-Agent System for End-to-End Causal Inference](https://arxiv.org/abs/2602.11527)
*Jiawei Zhu,Wei Chen,Ruichu Cai*

Main category: cs.AI

TL;DR: CausalAgent是一个基于对话式多智能体系统的端到端因果推理工具，通过自然语言交互自动化处理从数据清洗到报告生成的整个因果分析流程。
...
Conclusion: CausalAgent作为一种新颖的以用户为中心的人机协作范式，通过显式建模分析工作流程和交互式可视化，为因果推理提供了易用且可靠的解决方案。
...
### [141] [Budget-Constrained Agentic Large Language Models: Intention-Based Planning for Costly Tool Use](https://arxiv.org/abs/2602.11541)
*Hanbing Liu,Chunhao Tian,Nan An,Ziyuan Wang,Pinyan Lu,Changyuan Yu,Qi Qi*

Main category: cs.AI

TL;DR: INTENT框架：预算约束下工具增强智能体的推理时规划方法，通过意图感知分层世界模型预测未来工具使用和风险校准成本，在严格预算限制下显著提升任务成功率


<details>
  <summary>Details</summary>
Motivation: 研究预算约束下的工具增强智能体问题，其中大语言模型需要在严格的货币预算下通过调用外部工具解决多步骤任务。由于状态-动作空间巨大、结果方差高和探索成本过高，直接规划变得不可行。
...
Conclusion: INTENT框架有效解决了预算约束下工具增强智能体的规划挑战，通过推理时规划和意图感知建模，在保证预算可行性的同时提升了任务性能，对动态环境变化具有鲁棒性。
...
### [142] [Learning to Configure Agentic AI Systems](https://arxiv.org/abs/2602.11574)
*Aditya Taparia,Som Sagar,Ransalu Senanayake*

Main category: cs.AI

TL;DR: ARC：基于强化学习的轻量级分层策略，动态调整LLM智能体配置，实现按查询优化的资源分配


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体系统通常采用固定模板或手动调优的启发式配置，导致对简单和困难查询都使用相同的繁琐配置，造成计算资源浪费和系统脆弱性

Method: 将智能体配置建模为按查询的决策问题，使用强化学习训练轻量级分层策略，动态选择工作流、工具、令牌预算和提示

Result: 在多个推理和工具增强问答基准测试中，学习到的策略始终优于手动设计的基线和其他方法，任务准确率提高达25%，同时减少令牌和运行时成本

Conclusion: 按查询学习智能体配置是替代"一刀切"设计的强大方法，能够实现更高效、更灵活的资源分配
...
### [143] [The Five Ws of Multi-Agent Communication: Who Talks to Whom, When, What, and Why -- A Survey from MARL to Emergent Language and LLMs](https://arxiv.org/abs/2602.11583)
*Jingdi Chen,Hanqing Yang,Zongjun Liu,Carlee Joe-Wong*

Main category: cs.AI

TL;DR: 本文综述了多智能体通信研究的演进历程，从多智能体强化学习到涌现语言，再到基于大语言模型的系统，通过"五个W"框架分析通信设计的关键要素和挑战。


<details>
  <summary>Details</summary>
Motivation: 在动态、部分可观测的环境中，通信是减少不确定性、实现协作的关键。然而，现有通信方法存在任务特定性强、难以解释、泛化能力不足等问题，需要系统梳理多智能体通信的研究脉络和设计模式。

Method: 采用"五个W"框架（谁与谁通信、通信什么、何时通信、为何通信）分析多智能体通信，追溯三个主要研究范式的演进：多智能体强化学习（MARL）、涌现语言（EL）和基于大语言模型（LLM）的系统。

Result: MARL方法通过端到端学习优化通信协议但缺乏可解释性；EL方法试图发展结构化通信但面临基础、泛化和可扩展性挑战；LLM方法利用自然语言先验在开放环境中实现推理和规划，但需要与学习和控制结合。

Conclusion: 未来需要开发结合学习、语言和控制的混合系统，以实现可扩展、可解释的多智能体协作。本文提炼了实用设计模式和开放挑战，为构建更强大的多智能体通信系统提供指导。
...
### [144] [MAPLE: Modality-Aware Post-training and Learning Ecosystem](https://arxiv.org/abs/2602.11596)
*Nikhil Verma,Minjung Kim,JooYoung Yoo,Kyung-Min Jin,Manasa Bharadwaj,Kevin Ferreira,Ko Keun Kim,Youngjoon Kim*

Main category: cs.AI

TL;DR: MAPLE提出了一种模态感知的强化学习后训练生态系统，通过基准标注、策略优化和自适应调度，解决了多模态训练中信号处理盲目性的问题，显著提升训练效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态语言模型将文本、音频、视频信号同等对待，忽略了不同任务实际需要的模态组合。这种模态盲训练导致策略梯度方差增大、收敛缓慢，且在现实分布偏移（信号缺失、增加或重加权）时鲁棒性下降。

Method: MAPLE包含三个核心组件：1) MAPLE-bench基准，标注每个任务所需的最小信号组合；2) MAPO模态感知策略优化框架，按模态需求分层批处理以减少梯度方差；3) 自适应加权和课程调度，平衡并优先处理更难的信号组合。

Result: MAPLE将单模态/多模态准确率差距缩小30.24%，收敛速度提升3.18倍，在所有模态组合下保持稳定性，特别是在现实信号访问受限情况下表现优异。

Conclusion: MAPLE提供了一个完整的部署就绪多模态RL后训练方案，通过系统性分析损失聚合、裁剪、采样和课程设计，确立了最优训练策略，显著提升了多模态模型的训练效率和实际部署能力。
...
### [145] [scPilot: Large Language Model Reasoning Toward Automated Single-Cell Analysis and Discovery](https://arxiv.org/abs/2602.11609)
...
TL;DR: scPilot是首个实现组学原生推理的系统框架，让大语言模型能直接检查单细胞RNA-seq数据并调用生物信息学工具，将核心单细胞分析转化为逐步推理问题。


<details>
  <summary>Details</summary>
Motivation: 当前单细胞分析通常需要专业生物信息学知识，缺乏透明度和可解释性。scPilot旨在让LLM直接基于原始组学数据进行推理，实现可审计、可解释且具有诊断信息的单细胞分析。

Method: scPilot将细胞类型注释、发育轨迹重建和转录因子靶向等核心单细胞分析转化为逐步推理问题，LLM通过自然语言对话直接检查单细胞RNA-seq数据，并可按需调用生物信息学工具，支持迭代推理和修正。

Result: 实验显示，迭代组学原生推理使细胞类型注释平均准确率提升11%，Gemini-2.5-Pro将轨迹图编辑距离降低30%。同时生成透明推理轨迹，解释标记基因模糊性和调控逻辑。

Conclusion: scPilot通过将LLM扎根于原始组学数据，实现了可审计、可解释且具有诊断信息的单细胞分析，并发布了scBench评估套件来系统评估组学原生推理能力。
...
### [146] [When Agents Disagree With Themselves: Measuring Behavioral Consistency in LLM-Based Agents](https://arxiv.org/abs/2602.11619)
*Aman Mehta*

Main category: cs.AI

TL;DR: 研究发现LLM智能体在相同任务上存在行为不一致性，这种不一致性可预测任务失败率，早期决策是行为分化的关键点。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探究LLM智能体在相同任务重复执行时是否会产生一致的行为，以及行为不一致性与任务失败之间的关系。

Method: 在HotpotQA数据集上对三个模型（Llama 3.1 70B、GPT-4o和Claude Sonnet 4.5）进行了3,000次智能体运行实验，使用ReAct风格智能体，分析行为序列的变异性和任务准确性。

Result: 智能体行为存在显著不一致性：平均每10次运行产生2.0-4.2个不同的动作序列。行为一致性预测任务准确性：一致性高的任务（≤2个独特路径）准确率达80-92%，而高度不一致的任务（≥6个独特路径）准确率仅为25-60%，差距达32-55个百分点。69%的行为分化发生在第二步（首次搜索查询）。

Conclusion: LLM智能体在相同任务上表现出显著的行为不一致性，这种不一致性可预测任务失败。监控执行期间的行为一致性可实现早期错误检测，提高智能体可靠性。
...
### [147] [Neuro-Symbolic Multitasking: A Unified Framework for Discovering Generalizable Solutions to PDE Families](https://arxiv.org/abs/2602.11630)
*Yipeng Huang,Dejun Xu,Zexin Lin,Zhenzhong Wang,Min Jiang*

Main category: cs.AI

TL;DR: 提出了NMIPS框架，一种神经辅助多任务符号PDE求解器，用于同时求解PDE家族的解析解，通过多因子优化和仿射转移方法提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统数值方法需要独立求解PDE家族的每个实例，计算成本巨大；而机器学习PDE求解器虽然计算速度快，但缺乏可解释性，无法提供对科学洞察至关重要的解析表达式。

Method: 采用多因子优化同时发现PDE的解析解；设计仿射转移方法，在PDE家族间转移学习到的数学结构，避免从零开始求解每个PDE。

Result: 在多个案例实验中，相比现有基线方法，准确率提高了约35.7%，同时提供了可解释的解析解。

Conclusion: NMIPS框架有效解决了PDE家族求解的计算效率和可解释性问题，通过神经辅助多任务符号方法实现了准确且可解释的解析解发现。
...
### [148] [Do MLLMs Really Understand Space? A Mathematical Reasoning Evaluation](https://arxiv.org/abs/2602.11635)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在感知任务上表现良好，但在数学空间推理能力（解析和操作二维、三维关系）方面存在显著缺陷。人类在教科书式空间推理问题上的准确率超过95%，而大多数领先的MLLM甚至无法达到60%，这一巨大差距凸显了空间推理是当前模型的基本弱点。
...
Conclusion: MathSpatial为解决多模态大语言模型在数学空间推理方面的基本弱点提供了系统性的解决方案，通过分离感知与推理、提供结构化推理方法，显著提升了模型的空间推理能力，为未来研究提供了重要的基准和工具。
...
### [149] [Quark Medical Alignment: A Holistic Multi-Dimensional Alignment and Collaborative Optimization Paradigm](https://arxiv.org/abs/2602.11661)
...
TL;DR: 本文提出了一种新的医学问答对齐范式，通过构建多维医学对齐矩阵和统一优化机制，解决了传统强化学习在医疗领域应用中的范式不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习对齐方法在医疗问答中存在根本性范式不匹配：基于人类反馈的方法成本高昂且难以反映医学事实的绝对正确性；基于可验证奖励的方法缺乏有效自动验证器且难以处理复杂临床情境。医疗对齐需要同时优化正确性、安全性和合规性，但多目标异质奖励信号容易导致尺度不匹配和优化冲突。

Method: 1. 构建整体多维医学对齐矩阵，将对齐目标分解为四个类别：基础能力、专家知识、在线反馈和格式规范。2. 在每个类别中建立闭环：可观测指标→可归因诊断→可优化奖励，提供细粒度监督信号。3. 提出统一优化机制：采用参考冻结归一化对齐奖励尺度，实施三因子自适应动态加权策略，实现面向弱点、风险优先、减少冗余的协同优化。

Result: 实验结果表明，所提出的范式在真实世界医疗场景评估中有效，为垂直领域的复杂对齐建立了新范式。

Conclusion: 该研究提出了一种稳健的医学对齐范式，通过系统化的多维度分解和统一的优化机制，解决了医疗领域强化学习对齐的关键挑战，为垂直领域的复杂对齐问题提供了新的解决方案。
...
### [150] [Benchmark Health Index: A Systematic Framework for Benchmarking the Benchmarks of LLMs](https://arxiv.org/abs/2602.11674)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，传统的基准测试变得越来越不可靠。分数膨胀和选择性报告削弱了标准基准的权威性，导致社区不确定哪些评估结果仍然可信。

Method: 提出了基准健康指数（BHI）框架，从三个正交互补的维度审计评估集：1）能力区分度：衡量基准在噪声之外区分模型性能的锐度；2）抗饱和性：估计在达到天花板效应前剩余的改进空间；3）影响力：通过采用广度和实践塑造能力量化基准在学术和工业生态系统中的影响。

Result: 从2025年91个代表性模型的技术报告中提取了106个验证基准，系统性地描述了评估格局。BHI是首个在宏观层面量化基准健康度的框架。

Conclusion: BHI为基准选择提供了原则性基础，并支持下一代评估协议的动态生命周期管理，有助于解决当前LLM评估中的可靠性问题。
...
### [151] [Right for the Wrong Reasons: Epistemic Regret Minimization for Causal Rung Collapse in LLMs](https://arxiv.org/abs/2602.11675)
*Edward Y. Chang*

Main category: cs.AI

TL;DR: 论文提出Rung Collapse理论，解释自回归训练无法区分关联与干预的因果缺陷，导致模型"正确但理由错误"。作者提出Epistemic Regret Minimization (ERM)方法，通过三层架构解决因果推理错误，实验证明在1360个因果陷阱场景中有效恢复53-59%的错误。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统经常通过"捷径"获得高性能，但这些捷径在分布变化时会失效。这种病理现象有精确的因果起源：自回归训练无法提供梯度信号来区分关联P(Y|X)和干预P(Y|do(X))，作者称之为Rung Collapse。当基于结果的学习强化了通过错误因果模型获得的正确答案时，智能体会陷入错误的推理模式，这种现象被称为Aleatoric Entrenchment。

Method: 提出Epistemic Regret Minimization (ERM)，这是一个信念修正目标，独立于任务成功惩罚因果推理错误。构建三层架构，包含三个基于知识表示的贡献：(1) Physical Grounding Theorem证明满足执行器独立性的动作实现有效的do操作；(2) ERM作为满足AGM公设的因果信念修正算子；(3) 失败模式分类法，分类重复出现的推理错误并注入领域无关的防护机制。证明真实干预分布的渐近恢复和有限样本边界。

Result: 在6个前沿LLM的1360个因果陷阱场景实验中，发现Rung Collapse即使在推理增强模型中仍然存在（GPT-5.2为3.7%），可操控性呈现逆缩放现象（高级模型抵抗通用修正），而针对性ERM反馈能恢复53-59%的顽固错误，而结果级反馈则失败。

Conclusion: 论文揭示了自回归训练中Rung Collapse的因果缺陷，提出了ERM作为解决方案。通过理论证明和实验验证，展示了ERM在防止因果推理错误和恢复顽固错误方面的有效性，为构建更鲁棒的因果推理系统提供了理论基础和实践方法。
...
### [152] [Beyond Pixels: Vector-to-Graph Transformation for Reliable Schematic Auditing](https://arxiv.org/abs/2602.11678)
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉理解方面取得显著进展，但存在结构盲的严重限制。现有模型无法捕捉工程示意图中的拓扑结构和符号逻辑，因为其像素驱动范式丢弃了推理所需的显式向量定义关系

Method: 提出向量到图(V2G)管道，将CAD图转换为属性图，其中节点表示组件，边编码连接性，使结构依赖关系显式化且可机器审计

Result: 在电气合规检查的诊断基准上，V2G在所有错误类别中都获得了大幅度的准确性提升，而领先的MLLM仍接近随机水平

Conclusion: 像素基方法的系统性不足凸显，结构感知表示为实现多模态AI在工程领域实际部署提供了可靠路径。作者发布了基准和实现以促进进一步研究
...
### [153] [ThinkRouter: Efficient Reasoning via Routing Thinking between Latent and Discrete Spaces](https://arxiv.org/abs/2602.11683)
*Xin Xu,Tong Yu,Xiang Chen,Haoliang Wang,Julian McAuley,Saayan Mitra*

Main category: cs.AI

TL;DR: ThinkRouter是一种推理时置信度感知路由机制，在模型置信度低时将思维路由到离散token空间，置信度高时路由到潜在空间，以提高推理效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有潜在推理方法在不同场景下效果不一，研究发现错误答案的思维轨迹包含更少的低置信度步骤，而多个低置信度思维替代方案的软嵌入会引入噪声，导致对不可靠推理轨迹的高置信度。

Method: 提出ThinkRouter机制，基于模型置信度动态路由：置信度低时使用离散token空间进行显式推理，置信度高时使用潜在空间进行隐式推理，避免高置信度和噪声传播。

Result: 在STEM推理和编码基准测试中，ThinkRouter在准确率上优于显式CoT、随机路由和潜在推理基线，平均Pass@1提升19.70分，同时生成长度减少达15.55%。

Conclusion: ThinkRouter能够校准显式CoT和潜在推理产生的错误，通过全局降低模型置信度来加速思维结束token的生成，实现更高效准确的推理。
...
### [154] [Beyond Parameter Arithmetic: Sparse Complementary Fusion for Distribution-Aware Model Merging](https://arxiv.org/abs/2602.11717)
*Weihong Lin,Lin Sun,Qilong Shi,Aomufei Yuan,Yuxuan Tian,Zhengyang Wang,Guangxiang Zhao,Xiangzheng Zhang,Tong Yang*

Main category: cs.AI

TL;DR: SCF-RKL是一种新的模型融合框架，通过稀疏、分布感知的更新来减少功能干扰，使用反向KL散度测量模型差异并选择性融合互补参数，在多种基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法主要依赖参数空间启发式方法，常导致严重的功能干扰，造成泛化能力下降和不稳定的生成行为（如重复和不连贯输出）。需要一种能明确控制功能干扰的融合方法。

Method: 提出SCF-RKL框架，使用反向Kullback-Leibler散度测量模型间的功能差异，通过稀疏、分布感知的更新选择性融合互补参数，采用模式寻求和稀疏诱导设计来保持稳定表示同时整合新能力。

Result: 在24个基准测试（涵盖高级推理、通用推理与知识、指令跟随、安全性和视觉分类）上评估，SCF-RKL在多种模型规模和架构上一致优于现有模型融合方法，同时保持强大的泛化能力和生成稳定性。

Conclusion: SCF-RKL通过稀疏互补融合和反向KL散度，有效解决了模型融合中的功能干扰问题，在保持生成稳定性的同时提升了性能，为模型融合提供了新的有效范式。
...
### [155] [Text2GQL-Bench: A Text to Graph Query Language Benchmark [Experiment, Analysis & Benchmark]](https://arxiv.org/abs/2602.11745)
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图查询语言系统面临高质量基准数据集和评估方法缺乏的问题，现有数据集在领域覆盖、支持的图查询语言和评估范围方面存在局限，阻碍了该领域的系统化发展。

Method: 提出了Text2GQL-Bench基准测试，包含多GQL数据集和可扩展的构建框架，能够生成不同领域、问题抽象级别和图查询语言的数据集。同时引入了综合评估方法，涵盖语法有效性、相似性、语义对齐和执行准确性四个维度。

Result: 评估发现ISO-GQL生成存在显著的方言差距：即使在零样本设置下，强LLM最多只能达到4%的执行准确性；使用固定的3样本提示可将准确性提升到约50%，但语法有效性仍低于70%。微调的8B开源模型达到45.1%执行准确性和90.8%语法有效性，表明大部分性能提升来自足够的ISO-GQL示例暴露。
...
### [156] [AIR: Improving Agent Safety through Incident Response](https://arxiv.org/abs/2602.11749)
*Zibo Xiao,Jun Sun,Junjie Chen*

Main category: cs.AI

TL;DR: AIR是首个面向LLM智能体系统的安全事件响应框架，通过语义检查检测事件、引导智能体执行遏制恢复操作，并合成防护规则防止未来类似事件，在三种代表性智能体上实现了超过90%的成功率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体的安全机制主要关注事前预防，对事件发生后的响应、遏制和恢复能力有限。随着LLM智能体在自主应用中的广泛部署，需要建立系统化的事件响应机制来处理不可避免的安全事件。

Method: AIR框架定义了用于自主管理LLM智能体系统事件响应生命周期的领域特定语言，并将其集成到智能体执行循环中：1) 基于当前环境状态和近期上下文进行语义检查来检测事件；2) 引导智能体通过工具执行遏制和恢复操作；3) 在根除阶段合成防护规则以阻止未来类似事件。

Result: 在三种代表性智能体类型上的评估显示，AIR实现了检测、修复和根除成功率均超过90%。实验证实了AIR关键设计组件的必要性，展示了其及时性和适度开销，并证明LLM生成的规则在不同领域中接近开发者编写规则的有效性。

Conclusion: 事件响应作为提升智能体安全的一等机制既是可行的也是必要的。AIR框架为LLM智能体系统提供了系统化的事件响应能力，填补了当前安全机制在事后响应方面的空白。
...
### [157] [TSR: Trajectory-Search Rollouts for Multi-Turn RL of LLM Agents](https://arxiv.org/abs/2602.11767)
*Aladin Djuhera,Swanand Ravindra Kadhe,Farhan Ahmed,Holger Boche*

Main category: cs.AI

TL;DR: TSR是一种训练时方法，通过轻量级树搜索构建高质量轨迹，提升多轮强化学习性能，在训练计算一次性增加的情况下获得高达15%的性能提升。
...
Method: 提出TSR（轨迹搜索展开）方法，在训练时使用轻量级树搜索（如best-of-N、波束搜索、浅层前瞻搜索）构建高质量轨迹，通过任务特定反馈选择高分动作，保持优化目标不变且与优化器无关。

Result: 在Sokoban、FrozenLake和WebShop任务中，结合PPO和GRPO优化器，TSR实现了高达15%的性能提升和更稳定的学习过程，仅需一次性增加训练计算成本。

Conclusion: TSR通过将搜索从推理时间移到训练展开阶段，为多轮智能体学习提供了简单通用的增强机制，与现有框架和拒绝采样方法互补。
...
### [158] [How to Optimize Multispecies Set Predictions in Presence-Absence Modeling ?](https://arxiv.org/abs/2602.11771)
...
<details>
  <summary>Details</summary>
Motivation: 物种分布模型通常产生概率性出现预测，需要转换为二值化存在-缺失地图用于生态推断和保护规划。然而，这种二值化步骤通常是启发式的，可能严重扭曲物种流行度和群落组成的估计。

Method: 提出了MaxExp决策驱动二值化框架，通过直接最大化选择的评估指标来选择最可能的物种组合。还引入了集合大小期望（SSE）方法，这是一种基于预期物种丰富度预测组合的计算高效替代方案。

Result: 在涵盖不同分类群、物种数量和性能指标的三个案例研究中，MaxExp始终匹配或超越广泛使用的阈值和校准方法，特别是在强类别不平衡和高稀有性条件下。SSE提供了更简单但具有竞争力的选择。

Conclusion: 这些方法为多物种SDM二值化提供了稳健、可重复的工具，能够更准确地估计物种流行度和群落组成。
...
### [159] [RELATE: A Reinforcement Learning-Enhanced LLM Framework for Advertising Text Generation](https://arxiv.org/abs/2602.11780)
*Jinfang Wang,Jiajie Liu,Jianwei Wu,Ziqin Luo,Zhen Chen,Chunlei Li,Biao Han,Tao Deng,Yi Li,Shuanglong Li,Lin Liu*

Main category: cs.AI

TL;DR: RELATE是一个基于强化学习的端到端框架，统一了广告文本生成与目标对齐，通过将转化指标和合规约束整合为多维奖励，直接优化最终广告价值。
...
Method: 提出RELATE框架，通过策略学习将性能和合规目标直接整合到生成过程中，将转化导向的指标与合规约束联合建模为多维奖励，实现端到端的广告文本生成与对齐。

Result: 在大规模工业数据集上的实验表明，RELATE持续优于基线方法；在生产广告平台上的在线部署在严格政策约束下显著提升了点击转化率(CTCVR)。

Conclusion: RELATE框架通过统一生成与目标对齐，解决了现有两阶段系统的局限性，在保持合规性的同时有效提升了广告转化性能，验证了其在真实场景中的鲁棒性和有效性。
...
### [160] [FlowMind: Execute-Summarize for Structured Workflow Generation from LLM Reasoning](https://arxiv.org/abs/2602.11782)
...
<details>
  <summary>Details</summary>
Motivation: LLMs能够通过推理和工具使用解决复杂任务，但将这些解决方案准确转换为结构化工作流仍然具有挑战性。现有方法在执行过程中构建工作流常因两个过程的干扰导致不准确。

Method: 提出Execute-Summarize(ES)框架，将任务执行与工作流构建解耦：模型首先使用可用工具完成任务，然后独立地从执行轨迹中重构结构化工作流。

Result: 通过FlowBench进行广泛实验，该方法优于现有方法，为将自由形式的LLM推理落地为结构化工作流提供了可靠范式。

Conclusion: ES框架通过分离执行与工作流构建过程，提高了工作流的准确性和鲁棒性，为解决LLM解决方案到结构化工作流的转换问题提供了有效方法。
...
### [161] [Beyond End-to-End Video Models: An LLM-Based Multi-Agent System for Educational Video Generation](https://arxiv.org/abs/2602.11790)
*Lingyong Yan,Jiulong Wu,Dong Xie,Weixian Shi,Deguo Xia,Jizhou Huang*

Main category: cs.AI

TL;DR: LAVES是一个基于LLM的分层多智能体系统，用于从教育问题生成高质量教学视频，通过分解工作流程、质量门控和可执行视频脚本实现自动化生产。
...
Method: 采用分层LLM多智能体系统，由中央编排智能体协调解决方案智能体、插图智能体和叙述智能体，通过质量门控、迭代批判机制和基于规则的约束，生成结构化可执行视频脚本而非直接合成像素。

Result: 在大规模部署中，LAVES每天可生成超过100万个视频，相比当前行业标准方法成本降低95%以上，同时保持高接受率。

Conclusion: LAVES通过多智能体协作和结构化编译方法，成功解决了教学视频生成中的逻辑严谨性、成本效率和可控性问题，实现了高质量教学视频的自动化生产。
...
### [162] [Detecting RLVR Training Data via Structural Convergence of Reasoning](https://arxiv.org/abs/2602.11792)
*Hongbo Zhang,Yue Yang,Jianhao Yan,Guangsheng Bao,Yue Zhang,Yue Zhang*

Main category: cs.AI

TL;DR: 论文提出了一种名为Min-kNN Distance的黑盒检测方法，用于识别RLVR训练过的模型中的训练数据污染问题。该方法通过分析模型生成文本的多样性来区分RL训练中见过的和未见过的提示。


<details>
  <summary>Details</summary>
Motivation: RLVR（可验证奖励的强化学习）是现代推理模型训练的核心方法，但其不公开的训练数据引发了基准污染担忧。传统的基于似然的检测方法对RLVR效果不佳，因为RLVR基于自生成推理轨迹的奖励反馈进行微调，而非基于词元级概率优化。

Method: 作者发现RLVR诱导了独特的行为特征：在RLVR训练中见过的提示会导致更僵化、相似的生成结果，而未见的提示则保持更大的多样性。基于此，提出了Min-kNN Distance检测器：对给定提示采样多个补全结果，计算k个最小最近邻编辑距离的平均值来量化这种多样性崩溃。

Result: 在多个RLVR训练过的推理模型上的实验表明，Min-kNN Distance能可靠地区分RL训练中见过的和未见过的示例，并且在性能上优于现有的成员推断和RL污染检测基线方法。

Conclusion: Min-kNN Distance是一种简单有效的黑盒检测方法，无需访问参考模型或词元概率，能够有效检测RLVR训练导致的基准污染问题，为解决RL训练数据透明度问题提供了实用工具。
...
### [163] [Hi-SAM: A Hierarchical Structure-Aware Multi-modal Framework for Large-Scale Recommendation](https://arxiv.org/abs/2602.11799)
...
<details>
  <summary>Details</summary>
Motivation: 多模态推荐中，现有基于语义ID的方法存在两个主要问题：1）分词效果不佳，现有方法（如RQ-VAE）无法解耦跨模态共享语义和模态特定细节，导致冗余或塌缩；2）架构与数据不匹配，普通Transformer将语义ID视为扁平流，忽略了用户交互、物品和token的层次结构。

Method: 提出Hi-SAM框架，包含两个核心设计：1）解耦语义分词器（DST），通过几何感知对齐统一模态，采用从粗到细策略进行量化，共享码本提取共识，模态特定码本从残差中恢复细节，通过互信息最小化强制解耦；2）分层记忆锚点Transformer（HMAT），通过分层RoPE将位置编码拆分为物品间和物品内子空间，插入锚点token将物品压缩为紧凑记忆，保留当前物品细节，仅通过压缩摘要访问历史。

Result: 在真实世界数据集上相比SOTA基线获得一致改进，特别是在冷启动场景中表现突出。在大规模社交平台部署中，服务数百万用户，核心在线指标获得6.55%的提升。

Conclusion: Hi-SAM通过解耦语义分词和分层Transformer架构，有效解决了多模态推荐中的语义冗余和层次结构忽略问题，在离线实验和在线部署中都取得了显著效果。
...
### [164] [PuYun-LDM: A Latent Diffusion Model for High-Resolution Ensemble Weather Forecasts](https://arxiv.org/abs/2602.11807)
*Lianjun Wu,Shengchen Zhu,Yuxuan Liu,Liuyu Kai,Xiaoduan Feng,Duomin Wang,Wenshuo Liu,Jingxuan Zhang,Kelvin Li,Bin Wang*

Main category: cs.AI

TL;DR: 本文提出PuYun-LDM，一种针对高分辨率集合天气预报的潜在扩散模型改进方法，通过3D-MAE编码天气状态演化特征和变量感知掩码频率建模策略，解决了气象数据中潜在扩散性受限的问题。


<details>
  <summary>Details</summary>
Motivation: 传统潜在扩散模型在高分辨率集合天气预报中存在扩散性受限的问题。与自然图像不同，气象数据缺乏任务无关的基础模型和明确的语义结构，使得基于视觉基础模型的正则化方法不适用。现有频率方法在多元气象数据的谱异质性下，会导致正则化强度不均匀。

Method: 提出3D掩码自编码器（3D-MAE）编码天气状态演化特征作为扩散模型的额外条件，同时提出变量感知掩码频率建模（VA-MFM）策略，根据每个变量的谱能量分布自适应选择阈值。

Result: PuYun-LDM在短预报时效上优于ENS集合预报，在长预报时效上保持可比性能。在单个NVIDIA H200 GPU上5分钟内生成15天全球预报（6小时时间分辨率），并能高效并行生成集合预报。

Conclusion: 通过3D-MAE和VA-MFM策略，PuYun-LDM有效增强了潜在扩散性，为高分辨率集合天气预报提供了一种高效准确的解决方案。
...
### [165] [Predicting LLM Output Length via Entropy-Guided Representations](https://arxiv.org/abs/2602.11812)
...
Result: 构建并发布了ForeLen基准测试，包含长序列、思维链和RL数据。EGTP在ForeLen上达到最先进精度，MAE比最佳基线降低29.16%。将方法集成到长度感知调度器中可获得显著的端到端吞吐量提升。
...
### [166] [Prototype Transformer: Towards Language Model Architectures Interpretable by Design](https://arxiv.org/abs/2602.11852)
...
TL;DR: ProtoT是一种基于原型向量的可解释自回归语言模型架构，通过输入序列与原型之间的双向通信实现线性计算复杂度，同时提供模型推理的可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的语言模型虽然在某些领域超越人类，但其推理过程不透明，这削弱了对模型输出的信任。自回归语言模型的真实推理过程是黑箱，存在欺骗和幻觉等风险，需要开发可解释的模型架构。

Method: 提出原型变换器（ProtoT）架构，基于原型（参数向量）替代标准的自注意力机制。通过输入序列与原型之间的双向通信，原型在训练过程中自动捕获可命名的概念（如"女性"）。原型创建了在不同时间尺度上聚合上下文信息的通信通道。

Result: ProtoT在计算可扩展性方面表现出线性序列长度复杂度（相比SOTA自注意力变换器的二次复杂度）。在模型和数据规模扩展方面表现良好，在文本生成和下游任务（GLUE）上性能优秀。对输入扰动的鲁棒性与基线相当或更好，但提供了显示鲁棒性和敏感性来源的可解释路径。

Conclusion: ProtoT在接近最先进架构性能的同时，为创建设计上可解释的自回归语言模型铺平了道路。原型机制提供了模型推理的可解释性，并允许对其行为进行有针对性的编辑。
...
### [167] [Talk2DM: Enabling Natural Language Querying and Commonsense Reasoning for Vehicle-Road-Cloud Integrated Dynamic Maps with Large Language Models](https://arxiv.org/abs/2602.11860)
...
<details>
  <summary>Details</summary>
Motivation: 现有动态地图系统缺乏自然语言支持的人机交互界面，限制了人类与动态地图系统的交互效率。虽然日本已成功部署DM增强的自动驾驶系统，但自然语言接口的缺失限制了系统的可用性。

Method: 1. 开发VRCsim仿真框架生成流式车路云协同感知数据；2. 构建VRC-QA问答数据集，专注于混合交通场景的空间查询和推理；3. 提出Talk2DM插件模块，基于新颖的提示链机制，将人工定义规则与大型语言模型的常识知识逐步整合。

Result: Talk2DM在不同LLM间无缝切换时保持高自然语言查询准确率（超过93%），展示强大泛化能力。虽然大模型准确率更高但效率显著下降。使用Qwen3:8B、Gemma3:27B和GPT-oss模型时，平均响应时间仅2-5秒，显示强大实用潜力。

Conclusion: Talk2DM成功为车路云动态地图系统增加了自然语言查询和常识推理能力，通过提示链机制有效整合规则与常识知识，在保持高准确率的同时实现快速响应，具有实际部署潜力。
...
### [168] [From Atoms to Trees: Building a Structured Feature Forest with Hierarchical Sparse Autoencoders](https://arxiv.org/abs/2602.11881)
...
<details>
  <summary>Details</summary>
Motivation: 现有稀疏自编码器（SAE）通常孤立地提取单语义特征，但大量证据表明大型语言模型（LLM）捕捉了自然语言的内在结构，特别是"特征分裂"现象表明这种结构是分层的，因此需要能够捕捉这种层次结构的工具。
...
Conclusion: HSAE为发现和分析嵌入在LLM表示中的多尺度概念结构提供了一个强大、可扩展的工具，能够更好地理解和解释LLM的内部表示结构。
...
### [169] [When Should LLMs Be Less Specific? Selective Abstraction for Reliable Long-Form Text Generation](https://arxiv.org/abs/2602.11908)
*Shani Goren,Ido Galil,Ran El-Yaniv*

Main category: cs.AI

TL;DR: 提出选择性抽象框架，让LLM在不确定时降低内容细节而非完全拒绝回答，通过原子级分解和抽象化处理，在保持大部分信息的同时提高事实准确性。


<details>
  <summary>Details</summary>
Motivation: LLM存在事实错误问题，影响用户信任和高风险应用。传统的"全有或全无"不确定性估计方法在长文本生成中过于严格，会丢弃有价值信息。

Method: 提出选择性抽象框架，将响应分解为原子声明（短小、自包含的单个事实陈述），用更高置信度但更不具体的抽象来替换不确定的原子。开发端到端评估管道，使用信息论方法测量保留信息。

Result: 在FactScore和LongFact-Objects基准测试的六个开源模型上，原子级选择性抽象始终优于现有基线，风险-覆盖曲线下面积比声明移除方法提高达27.73%。

Conclusion: 降低特异性可以在保持大部分原始意义的同时提高准确性和可靠性，为LLM不确定性管理提供了更灵活有效的解决方案。
...
### [170] [AlphaPROBE: Alpha Mining via Principled Retrieval and On-graph biased evolution](https://arxiv.org/abs/2602.11917)
*Taian Guo,Haiyang Shen,Junyu Luo,Binqi Chen,Hongjun Ding,Jinsheng Huang,Luchen Liu,Yun Ma,Ming Zhang*

Main category: cs.AI

TL;DR: AlphaPROBE：通过有向无环图（DAG）导航进行alpha因子挖掘的新框架，结合贝叶斯检索器和DAG感知生成器，显著提升预测精度、收益稳定性和训练效率
...
Method: AlphaPROBE将alpha挖掘重新定义为有向无环图（DAG）的战略导航，将因子建模为节点，演化链接建模为边。框架包含两个核心组件：1）贝叶斯因子检索器，通过后验概率模型平衡利用与探索，识别高潜力种子因子；2）DAG感知因子生成器，利用因子的完整祖先轨迹生成上下文感知、非冗余的优化。

Result: 在三个主要中国股票市场数据集上对8个竞争基线的广泛实验表明，AlphaPROBE在预测准确性、收益稳定性和训练效率方面均获得显著提升。

Conclusion: 利用全局演化拓扑结构对于高效、稳健的自动化alpha发现至关重要。AlphaPROBE通过将因子池视为动态、互连的生态系统，解决了现有方法的局限性。
...
### [171] [MEME: Modeling the Evolutionary Modes of Financial Markets](https://arxiv.org/abs/2602.11918)
*Taian Guo,Haiyang Shen,Junyu Luo,Zhongshi Xing,Hanchun Lian,Jinsheng Huang,Binqi Chen,Luchen Liu,Yun Ma,Ming Zhang*

Main category: cs.AI

TL;DR: MEME框架通过建模金融市场中竞争性投资叙事（思维模式）的演化，从逻辑导向视角重构市场动态，优于传统资产中心或市场中心方法


<details>
  <summary>Details</summary>
Motivation: 现有LLM在量化金融中的应用主要采用资产中心范式（关注个股预测）或市场中心范式（关注组合配置），但往往忽视了驱动市场变动的底层逻辑推理。需要从逻辑导向视角建模金融市场作为竞争性投资叙事的动态演化生态系统

Method: 提出MEME框架：1）多智能体提取模块将噪声数据转化为高质量投资论点；2）高斯混合模型在语义空间中揭示潜在共识；3）时间评估与对齐机制跟踪思维模式的生命周期和历史盈利能力；4）优先考虑持久市场智慧而非短暂异常，确保组合构建基于稳健推理

Result: 在2023-2025年三个异质性中国股票池上的实验表明，MEME持续优于7个SOTA基线。消融研究、敏感性分析、生命周期案例研究和成本分析验证了MEME识别和适应金融市场演化共识的能力

Conclusion: MEME通过逻辑导向视角成功建模金融市场中竞争性投资叙事的演化动态，提供了一种超越传统资产中心或市场中心方法的新范式，能够更有效地识别和适应市场共识变化
...
### [172] [Gaia2: Benchmarking LLM Agents on Dynamic and Asynchronous Environments](https://arxiv.org/abs/2602.11964)
*Romain Froger,Pierre Andrews,Matteo Bettini,Amar Budhiraja,Ricardo Silveira Cabral,Virginie Do,Emilien Garreau,Jean-Baptiste Gaya,Hugo Laurençon,Maxime Lecanu,Kunal Malkan,Dheeraj Mekala,Pierre Ménard,Gerard Moreno-Torres Bertran,Ulyana Piterbarg,Mikhail Plekhanov,Mathieu Rita,Andrey Rusakov,Vladislav Vorotilov,Mengjue Wang,Ian Yu,Amine Benhalloum,Grégoire Mialon,Thomas Scialom*

Main category: cs.AI

TL;DR: Gaia2是一个用于评估LLM智能体在真实异步环境中的基准测试，引入独立于智能体行动的环境演化场景，支持细粒度动作级评估和强化学习。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法多为静态或同步的，缺乏对智能体在真实异步环境中处理时间约束、动态事件、模糊性和协作能力的测试，需要新的基准来评估实用智能体系统。

Method: 基于开源Agents Research Environments平台构建，每个场景配备写操作验证器，支持动作级评估和可验证奖励的强化学习，设计易于扩展。

Result: 评估显示没有模型在所有能力上占优：GPT-5总体得分最高（42% pass@1）但时间敏感任务失败，Claude-4 Sonnet在准确性和速度间权衡，Kimi-K2在开源模型中领先（21% pass@1）。

Conclusion: Gaia2揭示了推理、效率、鲁棒性之间的基本权衡，暴露了"仿真到现实"差距的挑战，通过发布基准和ARE框架为社区提供开发实用智能体系统的灵活基础设施。
...
### [173] [CSEval: A Framework for Evaluating Clinical Semantics in Text-to-Image Generation](https://arxiv.org/abs/2602.12004)
...
Conclusion: CSEval为生成模型在医疗领域的安全应用提供了重要的评估工具，能够评估临床语义对齐，支持生成模型在医疗领域的可靠应用。
...
### [174] [InjectRBP: Steering Large Language Model Reasoning Behavior via Pattern Injection](https://arxiv.org/abs/2602.12013)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究虽然通过调整提示来增强推理，但这些设计大多基于直觉，缺乏对底层行为模式的系统分析。研究者希望探究模型推理行为如何从行为模式角度塑造推理过程。

Method: 提出两种优化方法：1) InjectCorrect：通过模仿模型自身过去正确答案的行为模式来引导模型；2) InjectRLOpt：从历史行为模式数据中学习价值函数，并通过提出的可靠性感知Softmax策略在推理过程中生成行为注入物来引导推理过程。

Result: 实验表明两种方法都能在各种推理任务中提升模型性能，且无需修改模型参数，分别实现了最高5.34%和8.67%的性能提升。

Conclusion: 模型在回答特定类型问题时表现出自适应推理行为分布，结构化注入这些模式能显著影响推理过程和结果质量。基于行为模式的优化方法能有效提升大语言模型的推理性能。
...
### [175] [Multi UAVs Preflight Planning in a Shared and Dynamic Airspace](https://arxiv.org/abs/2602.12055)
...
TL;DR: DTAPP-IICR：一种用于大规模无人机群预飞行规划的交付时间感知优先级规划方法，通过增量迭代冲突解决，在动态共享空域中处理时间性禁飞区、异构无人机和严格交付期限，可扩展到1000架无人机。


<details>
  <summary>Details</summary>
Motivation: 大规模无人机群在动态共享空域中的预飞行规划面临重大挑战，包括时间性禁飞区、异构无人机配置和严格交付期限。现有多智能体路径规划方法缺乏实际无人机交通管理所需的可扩展性和灵活性。

Method: 提出DTAPP-IICR框架：1）基于任务紧急度生成初始优先级规划；2）使用SFIPP-ST（带软时间约束的安全飞行间隔路径规划）计算往返轨迹，处理异构无人机、强制执行时间性禁飞区，并将智能体间冲突建模为软约束；3）基于几何冲突图指导的迭代大邻域搜索解决剩余冲突；4）采用完整性保持的方向性剪枝技术加速3D搜索。

Result: 在包含时间性禁飞区的基准测试中，DTAPP-IICR在多达1000架无人机的机群中实现接近100%的成功率，剪枝技术带来高达50%的运行时间减少，优于批处理增强冲突搜索。在城市规模的实际操作中成功扩展，而其他基于优先级的方法在中等部署规模下即失败。
...
### [176] [The Pensieve Paradigm: Stateful Language Models Mastering Their Own Context](https://arxiv.org/abs/2602.12108)
*Xiaoyuan Liu,Tian Liang,Dongyang Ma,Deyu Zhou,Haitao Mi,Pinjia He,Yan Wang*

Main category: cs.AI

TL;DR: StateLM是一种具有内部状态管理能力的新型基础模型，通过主动管理上下文记忆工具，突破了固定上下文窗口的限制，在各种任务中显著优于标准LLM。


<details>
  <summary>Details</summary>
Motivation: 当前AI模型虽然拥有成熟的数据库和检索系统（如哈利波特中的冥想盆），但缺乏主动管理这些记忆的能力。模型被动接受人工设计的上下文作为全部记忆，就像没有魔杖的邓布利多，无法主动操作记忆系统。

Method: 提出StateLM模型，赋予其内部推理循环来管理自身状态。模型配备一系列记忆工具（上下文修剪、文档索引、笔记记录等），并训练其主动管理这些工具，学习动态构建自己的上下文。
...
Conclusion: 该方法将LLM从被动预测器转变为状态感知的智能体，使推理成为有状态且可管理的过程，突破了固定上下文窗口的架构限制。
...
### [177] [Stop Unnecessary Reflection: Training LRMs for Efficient Reasoning with Adaptive Reflection and Length Coordinated Penalty](https://arxiv.org/abs/2602.12113)
*Zewei Yu,Lirong Gao,Yuke Zhu,Bo Zheng,Sheng Guo,Haobo Wang,Junbo Zhao*

Main category: cs.AI

TL;DR: ARLCP框架通过自适应反思惩罚和长度协调惩罚，在大推理模型中平衡推理效率与准确性，显著减少响应长度同时提升准确率


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务中常生成过长的思维链，包含大量不必要的反思步骤（如重复自问和循环推理），导致高令牌消耗、计算开销增加和延迟，但准确率并未提升，尤其是在小模型中。问题复杂度增加会引发更多不必要的反思，降低准确率并增加令牌开销。

Method: 提出自适应反思与长度协调惩罚（ARLCP）强化学习框架，包含两个关键创新：1）自适应反思惩罚，动态减少不必要的反思步骤同时保留必要推理；2）根据问题估计复杂度校准的长度惩罚。通过协调这两种惩罚，鼓励模型生成更简洁有效的推理路径。
...
Conclusion: ARLCP框架有效解决了大型推理模型中过度反思导致的效率问题，通过动态平衡推理效率与解决方案准确性，显著减少了令牌消耗和计算开销，同时提升了模型性能。
...
### [178] [HLA: Hadamard Linear Attention](https://arxiv.org/abs/2602.12128)
*Hanno Ackermann,Hong Cai,Mohsen Ghafoorian,Amirhossein Habibian*

Main category: cs.AI

TL;DR: 提出Hadamard线性注意力(HLA)，通过计算成对相似度后的非线性变换来更好地近似softmax，避免了传统线性注意力低阶有理函数近似的限制，在视频生成等大规模token应用中保持高效计算。


<details>
  <summary>Details</summary>
Motivation: 传统线性注意力虽然计算效率高，但使用独立应用于查询和键的核函数，导致只能近似为低阶有理函数来逼近softmax，限制了其表达能力。需要一种既能保持计算效率又能更好近似softmax的注意力机制。

Method: 提出Hadamard线性注意力(HLA)，其非线性变换不像传统线性注意力那样分别应用于查询和键，而是像标准softmax注意力一样在计算成对相似度之后进行。该方法相当于使用更高阶的有理函数来近似softmax，并推导出类似标准线性注意力的高效计算方案，无需耗时的张量重塑操作。

Result: HLA能够更好地近似softmax注意力，同时保持计算效率。在涉及大量token的视频生成扩散变换器模型中验证了该方法的有效性，展示了其在大规模应用中的实用性。

Conclusion: Hadamard线性注意力提供了一种新的线性注意力变体，通过在成对相似度计算后应用非线性变换，实现了对softmax更好的高阶有理函数近似，同时保持了计算效率，特别适用于需要处理大量token的大规模应用场景。
...
### [179] [Neutral Prompts, Non-Neutral People: Quantifying Gender and Skin-Tone Bias in Gemini Flash 2.5 Image and GPT Image 1.5](https://arxiv.org/abs/2602.12133)
*Roberto Balestri*

Main category: cs.AI

TL;DR: 本研究量化了两种商用图像生成器（Gemini Flash 2.5 Image和GPT Image 1.5）中的性别和肤色偏见，发现中性提示词会产生高度极化的默认输出，而非中性的人口统计分布。


<details>
  <summary>Details</summary>
Motivation: 测试"中性提示词会产生中性人口统计输出"的假设，量化商业图像生成器中的性别和肤色偏见，为算法视觉文化提供审计框架。

Method: 使用4个语义中性提示词生成3200张照片级真实图像，采用混合色彩归一化、面部标志掩蔽和感知均匀肤色量化（Monk、PERLA、Fitzpatrick量表）的严格分析流程。
...
### [180] [Value Alignment Tax: Measuring Value Trade-offs in LLM Alignment](https://arxiv.org/abs/2602.12134)
*Jiajun Chen,Hua Shen*

Main category: cs.AI

TL;DR: 本文提出了价值对齐税（VAT）框架，用于衡量对齐干预如何影响整个价值系统，而不仅仅是目标价值，揭示了传统评估方法忽略的系统性风险。


<details>
  <summary>Details</summary>
Motivation: 现有价值对齐研究通常静态地描述价值关系，忽视了提示、微调或偏好优化等干预措施如何重塑更广泛的价值体系。需要一种能够捕捉对齐压力下价值表达动态变化的框架。

Method: 引入价值对齐税（VAT）框架，测量对齐引起的变化如何在相互关联的价值中传播，相对于实现的目标收益。使用基于施瓦茨价值理论的受控情境-行动数据集，收集配对的前后规范性判断，分析不同模型、价值和对齐策略的对齐效果。

Result: 结果显示，对齐通常会在价值之间产生不均匀、结构化的共同运动。这些效应在传统的仅针对目标的评估中是不可见的，揭示了系统性的、过程层面的对齐风险。

Conclusion: VAT框架为理解LLMs中价值对齐的动态提供了新见解，强调了考虑整个价值系统变化的重要性，而不仅仅是单一目标价值的改进。
...
### [181] [STAR : Bridging Statistical and Agentic Reasoning for Large Model Performance Prediction](https://arxiv.org/abs/2602.12143)
...
<details>
  <summary>Details</summary>
Motivation: 随着大模型评估成本急剧增加，从有限观测预测模型性能变得至关重要。现有统计方法难以应对模式变化、数据稀疏和缺乏解释性，而纯LLM方法仍不可靠。

Method: STAR框架结合数据驱动的统计期望与知识驱动的智能推理。使用专用检索器获取外部知识，将语义特征嵌入约束概率矩阵分解生成带不确定性的统计期望。基于期望违背理论的推理模块通过族内分析、跨模型比较和可信度感知聚合来细化预测。

Result: 在广泛实验中，STAR在基于分数和基于排名的指标上均优于所有基线方法。在极端稀疏条件下（每个测试模型仅1-2个观测分数），相比最强统计方法获得14.46%的总分提升。

Conclusion: STAR框架成功解决了大模型性能预测中的模式变化、数据稀疏和解释性不足问题，通过结合统计期望与知识推理实现了准确且可解释的预测。
...
### [182] [Seq2Seq2Seq: Lossless Data Compression via Discrete Latent Transformers and Reinforcement Learning](https://arxiv.org/abs/2602.12146)
...
<details>
  <summary>Details</summary>
Motivation: 传统压缩方法难以充分利用复杂数据结构，现有深度学习方法依赖向量表示而忽略了token结构，需要更有效的无损压缩技术

Method: 使用强化学习训练T5语言模型，采用离策略强化学习算法优化序列长度，将数据压缩为token序列而非连续潜在空间表示

Result: 相比传统方法显著提高了压缩比，保持语义完整性，无需外部语法或世界知识即可实现高效压缩

Conclusion: 基于强化学习和语言模型的token序列压缩方法为各种应用提供了更强大实用的压缩解决方案
...
### [183] [GPT-4o Lacks Core Features of Theory of Mind](https://arxiv.org/abs/2602.12150)
*John Muchovej,Amanda Royka,Shane Lee,Julian Jara-Ettinger*

Main category: cs.AI

TL;DR: 研究发现，尽管大型语言模型在简单的心智理论任务中能近似人类判断，但在逻辑等价任务中失败，且其行为预测与心理状态推断之间一致性低，表明其社交能力并非源于领域通用或一致的心智理论。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要评估LLMs在社交任务基准测试中的表现，但未测试心智理论所假设的实际表征——即心理状态与行为的因果模型。本研究旨在开发基于认知科学定义的心智理论评估框架，探究LLMs是否拥有连贯、领域通用且一致的心理状态-行为因果模型。

Method: 采用基于认知科学的心智理论定义，开发新的评估框架，测试LLMs是否拥有连贯、领域通用且一致的心理状态-行为因果模型。通过比较LLMs在简单心智理论范式中的表现与逻辑等价任务中的表现，并评估其行为预测与心理状态推断之间的一致性。

Result: 研究发现：1) LLMs在简单心智理论任务中能成功近似人类判断；2) 但在逻辑等价任务中失败；3) 其行为预测与相应心理状态推断之间的一致性很低。这些结果表明LLMs缺乏连贯、领域通用且一致的心理状态-行为因果模型。

Conclusion: 大型语言模型表现出的社交熟练度并非源于领域通用或一致的心智理论。尽管LLMs在某些社交任务中表现良好，但它们缺乏真正的心智理论所必需的心理状态-行为因果模型，这对其在需要深层社会理解的应用中构成了限制。
...
### [184] [Sci-CoE: Co-evolving Scientific Reasoning LLMs via Geometric Consensus with Sparse Supervision](https://arxiv.org/abs/2602.12164)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在科学推理任务中仍然脆弱，主要原因是解决方案评估不可靠和验证策略多样性有限。现有的协同演化范式在代码和数学领域表现良好，但在科学推理中需要更可靠的评估和更多样的验证方法。
...
Conclusion: Sci-CoE通过两阶段协同演化框架有效解决了科学推理中评估不可靠和多样性不足的问题，为构建更强大的科学推理系统提供了有效途径。
...
### [185] [Pedagogically-Inspired Data Synthesis for Language Model Knowledge Distillation](https://arxiv.org/abs/2602.12172)
*Bowei He,Yankai Chen,Xiaokun Zhang,Linghe Kong,Philip S. Yu,Xue Liu,Chen Ma*

Main category: cs.AI

TL;DR: 本文提出了一种基于教育原则的LLM知识蒸馏框架IOA，通过三阶段流程系统化地识别学生模型的知识缺陷、组织渐进式课程、适配认知能力，显著提升了小模型在复杂推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前基于合成数据的LLM知识蒸馏方法缺乏教学意识，将知识转移视为一次性数据合成和训练任务，而不是系统化的学习过程。需要借鉴教育原则来改进知识蒸馏方法。

Method: 提出了IOA三阶段框架：1) 知识识别器识别学生模型的知识缺陷；2) 知识组织器通过渐进式课程组织知识传递；3) 知识适配器调整表示以匹配学生模型的认知能力。整合了布鲁姆的掌握学习原则和维果茨基的最远发展区理论，创建动态蒸馏过程。

Result: 使用LLaMA-3.1/3.2和Qwen2.5作为学生模型，IOA在DollyEval上保留了教师模型94.7%的性能，参数少于1/10。在复杂推理任务上表现突出：MATH任务提升19.2%，HumanEval提升22.3%，优于现有基线方法。

Conclusion: 基于教育原则的IOA框架为LLM知识蒸馏提供了系统化的教学意识方法，显著提升了小模型在保持教师模型性能的同时减少参数量的能力，特别是在复杂推理任务上表现出色。
...
### [186] [SAM3-LiteText: An Anatomical Study of the SAM3 Text Encoder for Efficient Vision-Language Segmentation](https://arxiv.org/abs/2602.12173)
*Chengxi Zeng,Yuxuan Jiang,Ge Gao,Shuai Wang,Duolikun Danier,Bin Zhu,Stevan Rudinac,David Bull,Fan Zhang*

Main category: cs.AI

TL;DR: SAM3-LiteText：通过轻量级文本编码框架，在保持分割性能的同时，将SAM3的文本编码器参数量减少88%


<details>
  <summary>Details</summary>
Motivation: 视觉语言分割模型如SAM3使用为开放语言理解设计的大型通用文本编码器，但实际分割提示通常简短、结构化且语义受限，导致文本编码器容量过度配置，带来持续的计算和内存开销

Method: 对404,796个真实提示进行大规模解剖分析，发现严重冗余；提出SAM3-LiteText，用紧凑的MobileCLIP学生模型替换原始SAM3文本编码器，通过知识蒸馏优化

Result: 在图像和视频分割基准测试中，SAM3-LiteText将文本编码器参数减少高达88%，显著降低静态内存占用，同时保持与原始模型相当的分割性能

Conclusion: 通过分析分割提示的冗余特性并设计轻量级文本编码框架，可以在大幅减少参数和内存占用的同时保持分割性能，为高效视觉语言分割模型提供新思路
...
### [187] ["Sorry, I Didn't Catch That": How Speech Models Miss What Matters Most](https://arxiv.org/abs/2602.12249)
*Kaitlyn Zhou,Martijn Bartelds,Federico Bianchi,James Zou*

Main category: cs.AI

TL;DR: 语音识别系统在标准基准测试中表现良好，但在真实世界短小高风险的街道名称转录任务中错误率高达44%，对非英语母语者影响更大，通过合成数据微调可显著提升准确性。


<details>
  <summary>Details</summary>
Motivation: 尽管语音识别系统在标准基准测试中取得了低词错误率，但在真实世界部署中，特别是在短小高风险的话语（如街道名称转录）上经常失败。本研究旨在探究这一失败模式，特别是在美国街道名称转录这一高风险任务中，评估现有商业系统的表现，并量化转录错误对地理定位的影响。

Method: 研究评估了来自OpenAI、Deepgram、Google和Microsoft的15个模型，使用来自语言多样化的美国说话者的录音。通过量化转录错误对地理定位的影响，并引入一种合成数据生成方法，使用开源文本转语音模型生成命名实体的多样化发音。使用少于1000个合成样本进行微调来改善模型性能。

Result: 研究发现平均转录错误率为44%。转录错误对所有说话者都造成系统性错误，但对非英语母语说话者的路由距离错误是英语母语说话者的两倍。通过合成数据微调，街道名称转录准确性相对于基础模型提高了近60%（对非英语母语说话者）。

Conclusion: 研究揭示了语音系统在基准测试表现和真实世界可靠性之间的关键差距，并展示了一条简单、可扩展的路径来减少高风险转录错误。合成数据生成和微调方法能显著改善对语言多样化说话者的转录准确性，特别是在高风险的命名实体识别任务中。
...
### [188] [Think like a Scientist: Physics-guided LLM Agent for Equation Discovery](https://arxiv.org/abs/2602.12259)
*Jianke Yang,Ohm Venkatachalam,Mohammad Kianezhad,Sharvaree Vadgama,Rose Yu*

Main category: cs.AI

TL;DR: KeplerAgent是一个基于大语言模型的智能体框架，通过模拟科学家的多步推理过程（先推断物理属性，再用于限制方程空间）来发现符号方程，相比直接猜测的方法在准确性和抗噪性上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型方法直接从数据猜测方程，没有模拟科学家通常遵循的多步推理过程：先推断物理属性（如对称性），然后将其作为先验来限制候选方程空间。需要一种更符合科学推理过程的框架。

Method: KeplerAgent是一个智能体框架，协调基于物理的工具来提取中间结构，并使用这些结果配置符号回归引擎（如PySINDy和PySR），包括它们的函数库和结构约束。

Result: 在一系列物理方程基准测试中，KeplerAgent比传统的大语言模型和传统基线方法实现了显著更高的符号准确性和对噪声数据的更强鲁棒性。

Conclusion: 通过显式遵循科学推理过程（先推断物理属性再限制方程空间），KeplerAgent框架在符号方程发现任务上取得了更好的性能，为科学发现提供了更可靠的工具。
...
### [189] [Agentic Test-Time Scaling for WebAgents](https://arxiv.org/abs/2602.12276)
...
<details>
  <summary>Details</summary>
Motivation: 传统测试时缩放方法在多步智能体任务中存在局限性：均匀增加每步计算资源会快速饱和，且小错误会在长视野任务中累积。需要更智能的计算分配策略来平衡性能与效率。

Method: 首先对网页智能体进行推理时缩放的实证研究，发现均匀计算分配的问题。然后探索更强的聚合策略，包括基于LLM的仲裁器。研究发现智能体自身投票分布的熵和top-1/top-2边际等不确定性统计量与下游成功相关。基于此提出CATTS方法，使用投票衍生的不确定性指标动态分配计算资源，仅在决策真正存在争议时才增加计算。

Result: 在WebArena-Lite和GoBrowse基准测试中，CATTS相比React方法性能提升高达9.1%，同时使用的token数量比均匀缩放减少2.3倍。既获得了效率提升，又提供了可解释的决策规则。

Conclusion: CATTS通过基于置信度的动态计算分配，有效解决了多步智能体任务中测试时缩放的效率问题，在提升性能的同时显著减少计算开销，为智能体系统提供了实用的优化方案。
