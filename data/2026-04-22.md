...
### [1] [AutoAWG: Adverse Weather Generation with Adaptive Multi-Controls for Automotive Videos](https://arxiv.org/abs/2604.18993)
*Jiagao Hu,Daiguo Zhou,Danzhen Fu,Fuhao Li,Zepeng Wang,Fei Wang,Wenhua Liao,Jiayi Xie,Haiyang Sun*

Main category: cs.CV

TL;DR: AutoAWG是一个可控的恶劣天气视频生成框架，通过语义引导的多控制融合、消失点锚定的时序合成和掩码训练，在保持安全关键目标高保真的同时实现强天气风格化，显著提升了自动驾驶感知的鲁棒性。
...
Result: 在nuScenes验证集上显著优于现有方法：无首帧条件时，FID和FVD相对降低50.0%和16.1%；有首帧条件时，进一步降低8.7%和7.2%。在风格保真度、时序一致性和语义-结构完整性方面均表现出优势。

Conclusion: AutoAWG框架在恶劣天气视频生成方面表现出色，平衡了视觉质量和标注可重用性，对提升自动驾驶下游感知具有实际应用价值。
...
### [2] [Can We Build Scene Graphs, Not Classify Them? FlowSG: Progressive Image-Conditioned Scene Graph Generation with Flow Matching](https://arxiv.org/abs/2604.18623)
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数场景图生成方法将其视为一次性确定性分类问题，而非真正的渐进式生成任务。这限制了模型对复杂视觉关系的建模能力，需要更自然的生成式方法。
...
Result: 在VG和PSG数据集上的闭集和开放词汇协议实验中，在谓词R/mR和图级指标上均取得一致提升，平均比最先进的USG-Par方法提高约3个点，验证了混合离散-连续生成公式相对于一次性分类基线的优越性。

Conclusion: FlowSG通过将场景图生成重新构建为连续时间传输任务，实现了渐进式、生成式的场景图构建，在多项指标上超越了传统一次性分类方法，证明了混合离散-连续生成公式的有效性。
...
TL;DR: 本文提出了一种基于视觉的实时方法，通过单RGB相机估计人类对自主移动机器人(AMR)的感知状态，从而让AMR根据人类意识状态调整运动，提高仓库混合交通环境的安全性和效率。


<details>
  <summary>Details</summary>
Motivation: 在仓库环境中，人类工人和自主移动机器人(AMR)混合交通时，确保人类安全至关重要。现有方法通常将人类视为通用动态障碍物，导致AMR行为保守（如减速或绕行），即使工人完全意识到并能安全共享空间。需要更智能的方法来识别人类是否意识到AMR的存在。

Method: 提出实时视觉方法，使用单RGB相机估计人类对AMR的感知。整合最先进的3D人体姿态提升和头部方向估计技术，确定人类相对于AMR的位置及其视野锥，从而判断人类是否意识到AMR的存在。整个流程在NVIDIA Isaac Sim物理精确机器人仿真环境中使用合成生成数据进行验证。

Result: 实验结果表明，该系统能够可靠地实时检测人类位置及其注意力状态，使AMR能够根据人类感知状态安全调整运动。这为工业自动化环境中的安全性和操作效率提供了重要改进。

Conclusion: 该方法通过准确估计人类对AMR的感知状态，使AMR能够做出更智能的决策，在确保安全的同时提高操作效率，对工业自动化环境具有重要意义。
...
TL;DR: 本文提出Choreographic Syntax理论框架和DanceFlow数据集，开发DanceCrafter模型，实现基于文本的高质量可控舞蹈生成。


<details>
  <summary>Details</summary>
Motivation: 文本驱动的可控舞蹈生成面临两大挑战：高质量数据集严重匮乏，以及复杂舞蹈动作难以用语言准确描述。舞蹈具有复杂的空间动态、强烈的方向性和身体部位高度解耦的特点，使得这一任务尤为困难。

Method: 1. 提出Choreographic Syntax理论框架和标注系统，融合舞蹈研究、人体解剖学和生物力学原理；2. 构建DanceFlow数据集，包含41小时高质量动作数据和634万词详细描述；3. 开发DanceCrafter模型，基于Momentum Human Rig构建运动变换器，采用连续流形运动表示和混合归一化策略，并设计解剖感知损失函数来调节身体部位解耦特性。

Result: DanceCrafter能够实现高质量、稳定的复杂舞蹈序列生成。广泛的评估和用户研究表明，该方法在运动质量、细粒度可控性和生成自然度方面达到了最先进的性能。

Conclusion: 通过结合舞蹈理论框架、高质量数据集和专门设计的模型架构，本文成功解决了文本驱动舞蹈生成的挑战，实现了高保真度和可控性的复杂舞蹈动作生成。
...
TL;DR: 提出一种新的多编码器U-Net架构，用于bp-MRI前列腺病变的3D分割，通过文本-图像对齐、热图校准和置信度门控交叉注意力细化器提升分割精度


<details>
  <summary>Details</summary>
Motivation: 当前bp-MRI前列腺病变自动分割方法在多模态融合和病灶级语义指导方面存在不足，需要更精细的局部化指导来提升分割精度

Method: 提出多编码器U-Net架构，包含三个创新：1）增强前景文本-图像相似度的对齐损失；2）校准相似度图并抑制虚假背景激活的热图损失；3）置信度门控多头交叉注意力细化器进行局部边界编辑；采用阶段调度训练策略

Result: 在PI-CAI数据集上持续优于现有方法，建立了新的最先进性能，通过增强的多模态融合和局部化文本指导提升了分割效果

Conclusion: 提出的方法通过创新的对齐损失、热图损失和置信度门控细化器，有效解决了bp-MRI前列腺病变分割中的多模态融合和局部语义指导问题，实现了更精确的分割性能
...
TL;DR: 该研究开发了基于多模态大语言模型的智能C臂控制框架，通过微调MLLMs实现骨骼标志点定位，在合成和真实X射线数据集上取得与深度学习方法相当的性能，并展示了推理和空间感知能力。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法在C臂控制失败时需要临床医生手动操作，导致治疗延迟。需要能够结合临床医生反馈并通过推理进行调整的智能C臂控制框架，而骨骼标志点定位是实现这一目标的关键。

Method: 使用带标注的合成X射线数据集和真实X射线数据集，每个X射线图像配对多个骨骼标志点。微调两个多模态大语言模型，使其从X射线图像中检索最接近的标志点。与领先的深度学习方法进行定量比较，并进行定性实验展示MLLMs的推理能力和C臂导航能力。
...
TL;DR: DeltaSeg是一种用于结构缺陷分割的U型编码器-解码器架构，采用分层注意力策略，在S2DS和CSDD数据集上优于12种竞争架构。
...
Method: DeltaSeg采用U型架构，编码器使用深度可分离卷积和扩张阶段；瓶颈层使用ASPP捕获多尺度上下文；分层注意力策略包括编码器的SE通道注意力、瓶颈和解码器的坐标注意力，以及跳跃连接中的新型深度Delta注意力机制；通过多尺度辅助头进行深度监督。

Result: 在S2DS（7类）和CSDD（9类）两个数据集上，DeltaSeg一致优于包括U-Net、SA-UNet、UNet3+、SegFormer、Swin-UNet等在内的12种竞争架构，展现出跨损伤类型、成像条件和结构几何的强泛化能力。
...
TL;DR: URoPE是一种通用的旋转位置编码扩展，将相对位置编码从固定几何空间扩展到跨视图/跨维度的几何空间，通过相机射线采样和投影实现2D-3D几何推理。
...
Method: URoPE为每个关键/值图像块沿对应相机射线在预定义深度锚点处采样3D点，并将其投影到查询图像平面，然后使用投影像素坐标应用标准2D RoPE。

Result: URoPE在多个任务（新视图合成、3D目标检测、目标跟踪、深度估计）中作为插件位置编码，持续提升基于Transformer模型的性能，证明了其在几何推理中的有效性和通用性。

Conclusion: URoPE是一种参数无关、内参感知的相对位置编码，对全局坐标系选择具有不变性，同时完全兼容现有的RoPE优化注意力内核，为跨视图/跨维度几何推理提供了通用解决方案。
...
TL;DR: REVEAL框架通过将彩色眼底照片与个体化疾病风险特征对齐，结合分组感知对比学习，在AD和痴呆诊断前平均8年实现早期风险预测，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视网膜为阿尔茨海默病和痴呆提供了独特的非侵入性观察窗口，但现有方法通常将影像和风险因素分开建模，无法捕捉关键的跨模态联合模式，且缺乏组织或对齐相似患者特征的机制。

Method: 提出REVEAL框架：1) 将结构化问卷风险因素转化为临床可解释的叙事文本，与预训练视觉语言模型兼容；2) 引入分组感知对比学习策略，将具有相似视网膜形态测量和风险因素的患者聚类为正样本对，增强多模态对齐。

Result: REVEAL在预测AD和痴呆方面显著优于最先进的视网膜影像模型与临床文本编码器的组合，以及通用视觉语言模型，能够在诊断前平均8年（范围1-11年）进行早期风险预测。
...
TL;DR: CAHAL是一种针对临床MRI厚层各向异性扫描的幻觉鲁棒、物理信息引导的超分辨率框架，通过混合专家架构和复合损失函数，在患者原始采集空间直接进行分辨率增强，避免解剖学幻觉和体积高估问题。


<details>
  <summary>Details</summary>
Motivation: 临床常规MRI扫描多为厚层、各向异性采集，限制了大规模自动化形态学分析。现有生成式超分辨率方法虽然能产生视觉上令人信服的各向同性体积，但常常引入解剖学幻觉、系统性体积高估和结构扭曲，影响下游定量分析和诊断安全性。

Method: 提出CAHAL框架，采用确定性二元混合专家架构，通过专门的残差3D U-Net专家处理输入，这些专家根据体积分辨率和采集各向异性两个独立的临床MRI采集描述符进行条件化。使用复合损失函数优化，包括边缘惩罚的空间重建、傅里叶域频谱一致性匹配和分割引导的语义一致性约束。训练对通过基于物理的退化从大规模真实世界数据库中采样生成。

Result: 在T1加权和FLAIR序列上验证，CAHAL相比生成式基线方法取得了最先进的结果，在准确性和效率方面改进了最佳相关方法。

Conclusion: CAHAL提供了一个幻觉鲁棒、物理信息引导的分辨率增强框架，能够直接在患者原始采集空间操作，有效解决临床MRI厚层各向异性扫描的超分辨率问题，避免解剖学幻觉和体积高估，适用于下游定量分析和临床诊断。
...
<details>
  <summary>Details</summary>
Motivation: 自动胰腺分割对腹部MRI分析至关重要，但现有深度学习模型在不同MRI序列间泛化能力差，这一问题缺乏系统性研究。作者旨在建立跨序列泛化基准，识别临床可部署胰腺MRI分割的主要障碍。

Method: 引入CrossPan多机构基准数据集，包含1,386个3D扫描，涵盖三个常规采集序列（T1加权、T2加权和反相位）来自八个中心。通过实验评估跨序列域偏移严重性、现有域泛化方法效果、半监督学习稳定性以及基础模型零样本性能。

Result: 1) 跨序列域偏移比跨中心变异性严重得多：模型在域内Dice分数>0.85，跨序列时崩溃至接近零(<0.02)；2) 最先进的域泛化方法在物理驱动的对比度反转下几乎无效，而MedSAM2等基础模型通过对比度不变的形状先验保持中等零样本性能；3) 半监督学习仅在稳定强度分布下有效，在高器官内变异性序列上不稳定。

Conclusion: 跨序列泛化（而非模型架构或中心多样性）是临床可部署胰腺MRI分割的主要障碍。CrossPan基准为未来研究提供了重要参考，数据集和代码已公开。
...
TL;DR: Ghost-100基准测试评估视觉语言模型在渐进式强制提示下的幻觉行为，发现不同模型家族和任务类型对提示压力的响应模式存在显著差异


<details>
  <summary>Details</summary>
Motivation: 现有幻觉基准主要依赖中性提示和二元检测，无法评估模型在渐进式语言压力下的幻觉发生率和强度变化，特别是在不同任务结构下的响应模式

Method: 构建Ghost-100基准，包含800张合成图像，涵盖三个任务家族（文本不可读、时间读取、对象缺失），每个图像配以5个不同强度的提示，采用基于规则的H-Rate和GPT-4o-mini评分的H-Score双重评估协议

Result: 评估9个开源VLM发现：H-Rate和H-Score在不同模型家族间显著分离；阅读式和存在检测任务对提示压力的响应方式不同；多个模型在中等提示强度下表现出非单调敏感性峰值

Conclusion: 需要更细粒度的评估框架来捕捉VLM在强制提示下的幻觉行为模式，聚合指标会掩盖重要的响应差异，Ghost-100基准为此类分析提供了系统工具
...
TL;DR: 该论文通过黎曼几何框架分析潜在扩散模型的不稳定性，发现几何解耦现象：正常生成时曲率编码图像细节，但OOD生成时极端曲率浪费在不稳定的语义边界而非可感知细节上，从而识别出"几何热点"作为不稳定的结构根源。
...
TL;DR: 提出DUALVISION轻量融合模块，通过局部跨注意力将红外-可见光信息整合到多模态大语言模型中，提升在视觉退化条件下的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在RGB图像上表现良好，但在雾、模糊、低光等常见退化条件下表现脆弱。红外成像在这些条件下具有固有鲁棒性，但其与MLLMs的整合尚未充分探索

Method: 提出轻量级融合模块DUALVISION，通过patch级别的局部跨注意力机制高效整合IR-RGB信息；构建DV-204K数据集（25K对齐IR-RGB图像对，204K模态特定QA标注）和DV-500基准（500图像对，500QA对）用于训练和评估

Result: DUALVISION在多种视觉退化条件下展现出强大的实证性能；对开源和闭源MLLMs进行了基准测试，证明了方法的有效性

Conclusion: DUALVISION成功将红外成像的鲁棒性整合到MLLMs中，在视觉退化条件下显著提升性能；提供的数据集和基准将促进该领域的未来研究
...
TL;DR: 研究探索使用视觉基础模型（VFMs）通过2D到3D蒸馏方法实现室内LiDAR扫描的语义分割，无需人工标注，在室内SLAM数据集上验证可行性。


<details>
  <summary>Details</summary>
Motivation: 室内LiDAR扫描的逐帧语义分割对3D场景理解和地图应用至关重要，但获取训练数据的人工标注成本高昂且耗时。视觉基础模型（VFMs）在图像分割方面表现出色，本研究旨在探索能否通过2D到3D蒸馏方法将这种成功扩展到室内LiDAR场景。

Method: 采用2D到3D蒸馏框架，将每个LiDAR扫描与经过VFM处理的相机图像配对，通过跨模态蒸馏训练LiDAR分割模型。使用室内SLAM数据集生成伪标签进行评估，并提供一个手动标注的小型LiDAR数据集进行验证。

Result: 蒸馏模型在伪标签评估下达到56% mIoU，在真实标签评估下达到约36% mIoU，证明了无需人工标注的跨模态蒸馏在室内LiDAR语义分割中的可行性。

Conclusion: 研究表明，通过视觉基础模型的2D到3D蒸馏方法可以有效实现室内LiDAR扫描的语义分割，为无标注的室内3D场景理解提供了可行方案，尽管真实标签性能仍有提升空间。
...
<details>
  <summary>Details</summary>
Motivation: 人类感知具有良好的跨域泛化能力，但大多数视觉模型在训练数据之外表现不佳。多数据集学习旨在提高模型在域偏移下的鲁棒性，但统一训练面临数据分布和标签语义不一致的挑战。现有的MoE模型由于负载均衡机制强制专家间均匀分配输入，导致专家学习冗余表示，特别是在稀有或分布外域上性能下降。

Method: 提出GEM（全局专家映射）框架，包含规划器和编译器两部分。规划器基于线性规划松弛计算数据集到专家的分数分配，编译器通过分层舍入将软计划转换为确定性的容量感知映射。该方法避免了平衡损失，解决了公平性与专业化之间的冲突。

Result: GEM-DINO在UODB基准测试中实现了最先进的性能，在代表性不足的数据集上取得了显著提升，并在少样本适应场景中解决了任务干扰问题。

Conclusion: GEM框架通过全局调度替代学习型路由器，有效解决了MoE模型中公平性与专业化之间的冲突，实现了更好的专家专业化，提高了跨域学习的性能，特别是在稀有域和分布外域上的表现。
...
TL;DR: 提出ConvVitMamba统一混合框架，通过结合CNN、ViT和Mamba的优势，实现高效高光谱图像分类，在准确率、模型大小和推理效率间取得良好平衡。
...
Method: 提出ConvVitMamba三组件框架：1)多尺度卷积特征提取器捕获局部光谱、空间和联合模式；2)Vision Transformer进行标记化和编码建模全局上下文关系；3)轻量级Mamba门控序列混合模块进行高效内容感知细化。使用PCA预处理减少冗余。

Result: 在四个基准数据集（Houston和三个UAV borne QUH数据集）上的实验表明，ConvVitMamba在准确率上持续优于CNN、Transformer和Mamba方法，同时在模型大小和推理效率方面保持良好平衡。消融研究证实了各组件的互补贡献。
...
<details>
  <summary>Details</summary>
Motivation: 航空影像目标检测面临空间分辨率、场景构成和语义标签覆盖差异的挑战，传统模型难以在不同数据集间泛化，统一表示导致区域特定内容性能差且缺乏处理新类别的灵活性。

Method: 提出模块化学习框架，包含两个层次的路由机制：1) 全局专家分配层，使用潜在地理嵌入将数据集路由到专门的处理模块；2) 局部场景分解机制，将图像子区域分配到区域特定的子模块。还包括条件专家模块，利用外部语义信息实现推理期间的新类别检测。

Result: 在四个数据集上的综合评估表明，该方法在多数据集泛化、区域专业化和开放类别检测方面均有改进。

Conclusion: 通过超越单一表示，该方法为遥感目标检测提供了一个自适应框架，能够专门化处理不同数据集和复杂场景，并支持开放类别检测而无需重新训练。
...
TL;DR: 提出基于层次嵌入和对抗对齐的视觉语言模型鲁棒微调框架，通过层次结构增强对抗鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在零样本分类中易受对抗攻击，现有鲁棒微调方法通过固定文本嵌入与图像嵌入对齐，牺牲了自然性能和鲁棒性。当模型面临针对超类（如哺乳动物）和基类（如猫）的对抗攻击时，鲁棒性会进一步下降。

Method: 提出基于层次嵌入的对抗微调框架，采用多级对抗鲁棒对齐机制。通过额外机制将视觉嵌入置于层次结构的适当深度，建立嵌入深度与最大可行边界大小的理论联系。模型自然实现多种边界大小，增强对抗样本的泛化能力。考虑在多个共享相同叶标签的树上进行对齐以增加语义多样性。

Result: 在多个数据集上进行实验验证

Conclusion: 该框架通过利用类空间的固有层次特性，有效增强了视觉语言模型的对抗鲁棒性
...
TL;DR: 提出自动化管道用于金相图像密集实例分割和晶粒尺寸估计，结合Cellpose-SAM与ASTM E112 Jeffries平面测量模块，在少样本场景下实现高精度晶粒度预测


<details>
  <summary>Details</summary>
Motivation: 金相图像中提取标准化冶金指标面临挑战，主要由于复杂晶粒形态和监督分割的数据需求。需要将基础计算机视觉技术与实际冶金评估相结合，实现自动化材料表征

Method: 提出自动化管道，将Cellpose-SAM适配到微观结构，集成其拓扑感知梯度跟踪与ASTM E112 Jeffries平面测量模块。系统对比了U-Net、MatSAM和Qwen2.5-VL-7B等模型

Result: 仅使用2个训练样本，系统预测ASTM晶粒度数(G)的平均绝对百分比误差低至1.50%。验证了ASTM 50晶粒采样的最小要求，在保持拓扑分离方面优于其他方法

Conclusion: 应用级基础模型集成能实现高精度自动化材料表征，提出的管道在少样本场景下表现优异，为金相分析提供了有效解决方案
...
TL;DR: CXRMate-2模型在胸片报告生成任务中表现优异，在多项指标上显著超越现有基准，并在盲测评估中45%的生成报告被放射科医生认为可接受，显示出临床应用的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有胸片放射学报告生成模型虽然进展迅速，但其临床实用性仍不确定，主要原因是缺乏放射科医生的充分评估。本研究旨在开发一个能够生成临床可接受报告的先进模型。

Method: 提出CXRMate-2模型，整合结构化多模态条件处理和强化学习，使用复合奖励机制确保与放射科医生报告的语义对齐。在MIMIC-CXR、CheXpert Plus和ReXgradient数据集上进行评估，并进行了盲测随机定性回顾性评估，由三名顾问放射科医生比较生成报告与放射科医生报告。

Result: 在MIMIC-CXR数据集上，相比MedGemma 1.5 (4B)，GREEN指标提升11.2%，RadGraph-XL指标提升24.4%。在120项研究的盲测评估中，45%的生成报告被评定为可接受（优于或等同于放射科医生报告），在8个分析发现中有7个没有显著偏好差异。放射科医生报告因更高的召回率而受青睐，而生成报告在可读性方面更受偏好。

Conclusion: 研究结果表明胸片报告生成模型有可信的临床应用路径。通过提高召回率和改进对细微发现（如肺充血）的检测，可能足以实现与放射科医生报告的非劣效性。这些针对性进展表明，胸片报告生成系统可能已准备好在前瞻性评估中作为放射科医生主导工作流程的辅助工具。
...
TL;DR: AdaGScale提出了一种基于颜色贡献的自适应高斯缩放技术，通过减少高斯-瓦片对数量来提升3D高斯泼溅的渲染速度，同时保持图像质量。
...
TL;DR: 该论文提出了一个多智能体框架用于多模态共情响应生成，通过结构化推理和反思优化来增强共情能力，相比传统的一步生成方法有显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有多模态共情响应生成方法通常采用从多模态上下文到最终响应的隐式一步生成范式，忽视了人类情感感知的结构化特性以及情感复杂性导致的偏见问题，导致情感判断失真和共情效果不佳。

Method: 提出了一个多智能体框架，包含两个核心模块：1）结构化共情推理生成模块，通过多模态感知、一致性情感预测、实用策略规划和策略引导响应生成四个步骤显式分解响应生成过程；2）全局反思优化模块，通过全局反思智能体对中间状态和生成响应进行逐步审计，消除情感偏见和共情错误，触发针对性重新生成。

Result: 在IEMOCAP和MELD等多个基准测试上的实验表明，该模型相比最先进方法具有更优越的共情响应生成能力。

Conclusion: 通过结构化推理和反思优化的闭环框架，能够逐步提高情感感知准确性并消除情感偏见，显著提升多模态共情响应生成的质量和效果。
...
TL;DR: ABot-Explorer：一种新颖的主动探索框架，将空间记忆构建与探索统一为在线RGB-only过程，利用大视觉语言模型提取语义导航可通行性作为认知对齐锚点，显著提升探索效率和环境覆盖率。


<details>
  <summary>Details</summary>
Motivation: 当前空间记忆构建主要采用解耦的两阶段范式：先探索收集环境数据，后离线重建空间记忆。这种事后处理且以几何为中心的方法使智能体无法利用高级语义智能，常常忽略导航关键地标（如门道、楼梯），而这些正是人类认知地图中的基本语义锚点。
...
Conclusion: ABot-Explorer通过在线统一记忆构建与探索，利用语义导航可通行性作为认知锚点，实现了更高效、更符合人类认知的探索策略，为复杂具身导航任务中的长期推理提供了结构化空间记忆构建的新范式。
...
TL;DR: 提出一种基于预训练生成模型的纹理过滤方法，通过两阶段微调策略，在小规模配对图像上进行监督微调，然后在大规模无标签数据集上进行强化微调，取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 利用预训练生成模型的强大图像先验能力来提升纹理过滤的性能和泛化能力。传统纹理过滤方法在处理复杂纹理时存在局限性，而生成模型能够更好地理解图像结构和纹理特征。

Method: 采用两阶段微调策略：1）在少量配对图像上进行监督微调；2）在大规模无标签数据集上进行强化微调，使用奖励函数量化纹理去除和结构保留的质量。

Result: 实验表明该方法明显优于先前方法，能够有效处理先前具有挑战性的案例，表现出良好的性能和泛化能力。

Conclusion: 通过利用预训练生成模型的图像先验，结合两阶段微调策略，实现了高效的纹理过滤，为纹理处理任务提供了新的解决方案。
...
TL;DR: LPCVC 2025挑战赛设计了三个赛道（图像分类、开放词汇分割、单目深度估计），采用Qualcomm AI Hub进行标准化评估，展示了各赛道最佳方案并总结了关键趋势。
...
Method: 设计三个竞赛赛道：1）不同光照条件和风格下的图像分类；2）基于文本提示的开放词汇分割；3）单目深度估计。采用Qualcomm AI Hub构建统一的评估框架，确保结果的一致性和可复现性。

Result: 介绍了各赛道的最佳解决方案，总结了关键趋势和观察结果，为高效视觉模型在边缘设备上的发展提供了实践参考。

Conclusion: 提出了对未来计算机视觉竞赛的建议，强调了标准化评估框架和实际应用场景的重要性。
...
TL;DR: 提出SDB机制，通过平衡行为多样性和学习稳定性来解决VLN中自我改进的挑战，在多个数据集上取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 在视觉语言导航中，基于策略诱导经验的自我改进需要平衡行为多样性和学习稳定性。增加行为多样性可以暴露更多动作假设，但会破坏学习信号的稳定性；而过于保守的稳定性约束会抑制探索并导致过早收敛，使得可靠的自我改进变得困难。

Method: 提出稳定性-多样性平衡机制，将每个决策步骤扩展为多个潜在行为假设，通过对指令条件隐藏状态施加受控偏移来实现。然后进行可靠性感知的软评估和聚合，在学习过程中保留多样且与指令一致的替代方案。通过显式正则化约束假设间的交互，防止假设多样性过度漂移或过早崩溃。

Result: 在R2R、SOON和REVERIE数据集上均取得一致改进。例如在REVERIE val-unseen上，SPL从33.73提升到35.93，OSR从51.07提升到54.25。

Conclusion: SDB机制有效平衡了VLN中的行为多样性和学习稳定性，实现了可靠的自我改进，在多个基准测试中均表现出显著性能提升。
...
TL;DR: 本文提出了一种针对多模态测试时适应的概率高斯模型，通过显式建模类别条件分布和自适应对比不对称性校正技术，解决多模态分布不对称问题，在多种分布偏移下实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态测试时适应方法缺乏对类别条件分布的显式建模，而传统高斯判别分析在多模态场景中因模态分布不对称而效果有限，需要专门的方法来解决这一问题。

Method: 提出专门的多模态TTA概率高斯模型来显式建模类别条件分布，并引入自适应对比不对称性校正技术来抵消模态不对称带来的负面影响，从而获得校准的预测和可靠的决策边界。

Result: 在多个基准测试上的广泛实验表明，该方法在广泛的分布偏移下实现了最先进的性能。

Conclusion: 通过显式建模类别条件分布并解决模态不对称问题，提出的方法显著提升了多模态测试时适应在分布偏移下的鲁棒性和准确性。
...
TL;DR: 提出EgoMotion框架解决第一人称视角下视觉-语言条件运动生成的推理-生成纠缠问题，通过认知推理和运动生成两阶段分离，实现语义准确且运动质量高的3D人体动作合成。


<details>
  <summary>Details</summary>
Motivation: 第一人称视角运动生成因感知复杂性而研究不足，现有方法在同时优化语义推理和运动建模时存在梯度冲突，导致多模态对齐和运动质量下降。

Method: 提出EgoMotion分层生成框架：1) 认知推理阶段使用VLM将多模态输入映射到离散运动基元空间，获得目标一致表示；2) 运动生成阶段以这些表示为条件，在连续潜空间中进行扩散去噪，合成物理合理且时序连贯的运动轨迹。

Result: EgoMotion在评估中达到最先进性能，生成的运动序列在语义对齐和运动质量上均优于现有方法。

Conclusion: 通过解耦认知推理和运动控制，EgoMotion有效解决了第一人称视觉-语言运动生成中的推理-生成纠缠问题，实现了语义准确且运动质量高的动作合成。
...
TL;DR: PortraitDirector是一个新颖的人脸重演框架，采用分层组合方法实现高保真度和精细控制，将面部运动分解为空间层（头部姿态和局部表情）和语义层（全局情感），并通过优化实现实时性能。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸重演方法在表达性和精细可控性之间存在权衡。整体性方法通常牺牲粒度控制来获得表达性，而控制导向的方法则在保真度和鲁棒解耦方面存在困难。本文探索了一种组合视角，将人脸重演视为分层组合任务。

Method: 采用分层运动解耦与组合策略：1）空间层：全局头部姿态（专用表示和注入路径）+局部面部表情（从裁剪区域提取，通过情感过滤模块去除情感线索）；2）语义层：全局情感。解耦组件重新组合成表达性运动潜在表示。通过扩散蒸馏、因果注意力和VAE加速等优化实现实时性能。

Result: PortraitDirector在单块5090 GPU上实现了流式、高保真、可控的512×512人脸重演，达到20 FPS，端到端延迟800毫秒。

Conclusion: PortraitDirector通过分层组合方法成功解决了人脸重演中表达性与可控性的权衡问题，实现了高质量、细粒度控制且实时的面部重演效果。
...
TL;DR: BALTIC基准测试系统评估3D重建方法在不同介质（空气/水）和光照条件下的性能，发现高斯溅射在纹理一致条件下通过简单预处理可媲美专业水下方法，但在复杂真实环境中鲁棒性下降。


<details>
  <summary>Details</summary>
Motivation: 机器人感知在空气和水之间转换时面临3D重建的挑战，需要系统评估现有方法在不同介质和光照条件下的性能，以推动跨域重建技术的发展。

Method: 创建BALTIC基准测试，包含13个数据集覆盖两种介质和三种光照条件；使用定制水槽、单目相机和HTC Vive追踪器获取地面真实位姿；评估COLMAP的SfM重建，并将其作为NeRF和3D高斯溅射的输入；通过轨迹精度、场景几何、感知和光度指标进行评估。

Result: 在受控的纹理一致条件下，经过简单预处理（如白平衡校正）的高斯溅射方法可以达到与专业水下方法相当的性能；但在更复杂和异质的真实世界环境中，其鲁棒性会下降。

Conclusion: BALTIC基准为跨介质3D重建提供了系统评估框架，表明在受控条件下简单预处理的高斯溅射方法具有潜力，但在真实复杂环境中仍需进一步改进以提高鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于草图的3D形状检索方法在零样本设置下表现不佳，主要因为缺乏类别监督和草图输入的极端稀疏性。扩散模型具有开放词汇能力和强形状偏置，适合零样本视觉检索。

Method: 使用冻结的Stable Diffusion主干从中间U-Net层提取草图与3D视图的判别性表示。引入多模态特征增强策略：注入CLIP视觉编码器的全局和局部特征，结合BLIP生成的硬文本描述与可学习软提示。采用Circle-T损失动态加强正样本对吸引力。

Result: 在两个公开基准测试上的广泛实验表明，该方法在零样本基于草图的3D形状检索任务中始终优于最先进的方法。

Conclusion: 首次证明了文本到图像扩散模型在零样本基于草图的3D形状检索中的有效性，通过多模态特征增强策略解决了草图抽象稀疏和领域差距问题，无需昂贵重新训练即可显著提升性能。
...
TL;DR: 本文提出Patch Forcing框架，通过空间和时间上的补丁级噪声调度，实现自适应图像生成，提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统扩散和流模型在空间上均匀分配计算资源，忽略了自然图像的异质性：有些区域容易去噪，而其他区域需要更多细化或上下文信息。这种均匀分配计算的方式效率低下。

Method: 提出Patch Forcing框架：1）引入时间步采样器，控制训练期间补丁级最大信息量；2）添加轻量级每补丁难度头，实现自适应采样器动态分配计算资源；3）结合空间和扩散时间的噪声变化，让简单区域提前生成以为困难区域提供上下文。

Result: 在类别条件ImageNet上取得优越结果，与表示对齐和引导方法正交，可扩展到文本到图像合成。从全局时间步转向补丁级时间步已能改善图像生成。

Conclusion: 补丁级去噪调度为自适应图像生成提供了有前景的基础，Patch Forcing框架通过空间和时间上的自适应计算分配，显著提升了图像生成质量。
...
TL;DR: ST-Prune：一种用于自动驾驶视觉语言模型的无训练即插即用时空令牌剪枝框架，通过运动感知时间剪枝和环视空间剪枝模块，在90%令牌减少下实现接近无损性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在自动驾驶系统中面临多视角摄像头和多帧视频输入带来的巨大计算开销问题。现有的令牌剪枝方法主要针对单图像输入，无法利用驾驶场景中固有的时空冗余性。

Method: 提出ST-Prune框架，包含两个互补模块：1) 运动感知时间剪枝(MTP)：通过编码运动波动性和时间近邻性作为软约束，优先保留动态轨迹和当前帧内容；2) 环视空间剪枝(RSP)：利用环视摄像头几何结构惩罚双边跨视角相似性，消除重复投影和剩余背景。

Result: 在感知、预测和规划四个基准测试中验证，ST-Prune建立了无训练令牌剪枝的新SOTA。在90%令牌减少下，某些指标甚至超过完整模型基线，同时保持与现有剪枝方法相当的推理速度。

Conclusion: ST-Prune通过有效利用驾驶场景的时空冗余性，为自动驾驶视觉语言模型提供了一种高效的无训练令牌剪枝解决方案，显著降低计算开销的同时保持关键场景信息。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于深度特征的感知相似性模型大多在单一空间尺度上操作，隐含假设固定分辨率的结构相似性已足够。然而，空间尺度在深度特征相似性建模中的作用尚未得到充分理解，需要研究多尺度表示的重要性。

Method: 提出了MSDS（Deep Structural Similarity with Multiscale Representation）框架，通过最小化多尺度扩展DeepSSIM。该框架将深度特征表示与跨尺度集成解耦：在金字塔各层级独立计算DeepSSIM，然后使用轻量级可学习的全局权重融合得到的分数。

Result: 在多个基准数据集上的实验表明，相比单尺度基线方法，MSDS框架取得了持续且统计显著的改进，同时仅引入了可忽略的额外复杂度。结果从经验上证实了空间尺度是深度感知相似性中不可忽视的因素。

Conclusion: 通过最小化测试平台MSDS，论文成功将空间尺度作为独立因素进行隔离研究，证实了多尺度表示在深度特征相似性建模中的重要性，为图像质量评估提供了更有效的框架。
...
TL;DR: CLVG-Bench是一个评估视频模型零样本推理能力的框架，通过视频生成中的上下文学习来测试模型的多模态推理能力，发现当前SOTA模型在逻辑推理和交互生成任务上表现严重不足。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型评估基准存在局限性，任务设计简单且评估指标碎片化，无法真正衡量模型的多模态推理能力。需要建立一个更严谨的评估框架来回答"视频模型距离实现真正多模态推理还有多远"这一核心问题。
...
Result: 实验发现，虽然SOTA视频模型（如Seedance 2.0）在某些理解和推理子任务上表现尚可，但在逻辑基础和交互生成任务上表现严重不足（成功率分别<25%和~0%），暴露了多模态推理和物理基础是关键瓶颈。

Conclusion: CLVG-Bench通过系统量化视频模型的局限性，为真正鲁棒的通用视频模型提供了可操作的反馈和清晰的路线图，揭示了多模态推理能力是当前视频模型发展的关键挑战。
...
TL;DR: 该研究针对移动设备采集3D高斯溅射数据的问题，提出了对象中心场景下的采集指导系统，通过实时球面覆盖度计算指导用户运动，获得更全面均匀的视角覆盖，从而用更少图像实现更优重建质量。


<details>
  <summary>Details</summary>
Motivation: 移动设备采集3D高斯溅射数据面临挑战，特别是在对象中心场景下。现有方法如RealityScan和自由采集策略在重建质量和效率方面存在不足，需要更可靠的移动采集方案。

Method: 1. 设备校准后，将设备方向对齐到基准坐标系以获得相对位姿；2. 将相机光轴映射到对象中心的球面网格进行均匀视角索引；3. 为减少极地采样偏差，实时计算面积加权的球面覆盖度；4. 根据覆盖度指导用户的运动轨迹。

Result: 与RealityScan和自由采集策略相比，该方法使用更少的输入图像获得了更优的重建质量。进一步分析表明，该方法能够在对象中心采集过程中获得更全面和均匀的视角覆盖。

Conclusion: 提出的移动采集指导系统通过实时球面覆盖度计算和用户运动指导，有效解决了3D高斯溅射在移动设备上的数据采集问题，显著提升了重建质量和采集效率。
...
TL;DR: 本文提出基于注意力的多模态深度学习框架(ABMMDLF)，用于高精度时空作物产量预测，相比传统方法显著提升了预测准确性。


<details>
  <summary>Details</summary>
Motivation: 作物产量预测对全球粮食安全和政策制定至关重要。传统预测技术因使用静态数据源而精度有限，无法反映环境变量间随时间变化的动态复杂关系。

Method: 提出注意力多模态深度学习框架，整合多年卫星影像、高分辨率气象时间序列数据和初始土壤属性。采用CNN提取空间特征，结合时序注意力机制自适应加权重要物候期，并考虑图像和视频序列的空间特征。

Result: 实验结果显示，所提方法R²得分达到0.89，远优于基线模型。

Conclusion: ABMMDLF框架通过多模态数据融合和注意力机制，显著提高了作物产量预测的准确性和可靠性，为精准农业和粮食安全决策提供了有效工具。
...
TL;DR: ReID-R提出了一种新颖的推理驱动范式，通过将思维链融入ReID流程，实现显式身份理解和推理，仅用14.3K非平凡数据就达到与现有方法竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 当前主流感知驱动范式倾向于从大量标注数据中学习拟合，而非理解身份因果关系，导致表示在面对多种干扰时表现脆弱。需要一种能够实现显式身份理解和推理的方法。

Method: ReID-R采用两阶段方法：1) 判别性推理预热：以无标签的思维链方式训练模型，获得身份感知特征理解；2) 高效强化学习：通过非平凡采样构建场景泛化数据，利用高质量奖励信号引导模型关注ID相关线索。

Result: 在多个ReID基准测试中，ReID-R仅使用14.3K非平凡数据（现有数据规模的20.9%）就达到了与先进方法竞争的身份判别性能。此外，得益于内在推理能力，ReID-R能为结果提供高质量解释。

Conclusion: ReID-R作为一种推理驱动范式，通过思维链集成实现了显式身份理解和推理，在数据效率、性能表现和结果可解释性方面都展现出优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于固定尺寸切片的策略在处理高分辨率航拍和卫星图像时存在大量冗余计算，导致推理成本增加和检测速度下降，而小目标检测在密集分布、可变拍摄角度、微小目标尺寸和显著类间差异等挑战下仍然困难。

Method: ASAHI包含三个协同组件：1）自适应分辨率感知切片算法，基于学习阈值动态生成6或12个重叠补丁；2）切片辅助微调策略，构建包含全分辨率和切片图像补丁的增强训练数据；3）Cluster-DIoU-NMS后处理模块，结合Cluster-NMS的几何合并效率和DIoU-NMS的中心距离感知抑制，实现拥挤场景中的鲁棒重复消除。
...
Conclusion: ASAHI通过自适应切片策略有效解决了高分辨率航拍图像中小目标检测的效率和精度问题，显著减少了冗余计算，同时保持了相邻补丁之间的有益重叠，为实际应用提供了可行的解决方案。
...
TL;DR: OTCA提出了一种细粒度的奖励分配框架，通过分解扩散轨迹的时间信用和多目标信用分配，解决了GRPO训练中粗粒度奖励分配的问题，显著提升了图像和视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法将多个奖励模型压缩为单一静态标量，并在整个扩散轨迹上均匀传播，忽略了不同去噪步骤的阶段特定作用，导致优化信号不准确或不兼容。
...
TL;DR: Allo{SR}²：通过同态生成流校正单步超分辨率轨迹的新框架，解决先验崩溃和轨迹漂移问题，在单步Real-SR中实现最先进性能


<details>
  <summary>Details</summary>
Motivation: 现实世界图像超分辨率（Real-SR）虽然受益于大规模扩散和流模型的强大生成先验，但在有限LR-HR对上微调会导致"先验崩溃"——模型牺牲其固有的生成丰富性来过度拟合特定训练退化。单步生成中缺乏多步细化进一步加剧了轨迹漂移和伪影生成问题。

Method: 提出Allo{SR}²框架，通过同态生成流校正单步SR轨迹。包含三个核心组件：1）信噪比引导的轨迹初始化，通过将LR潜在特征的退化水平与预训练流的最优锚定时间步对齐来建立物理基础起始状态；2）流锚定轨迹一致性，在中间状态实施速度级监督以确保稳定、无曲率的单步推理路径；3）同态轨迹匹配，通过自对抗对齐策略最小化SR流和生成流在统一向量场中的分布差异。

Result: 在合成和现实世界基准测试上的广泛实验表明，Allo{SR}²在单步Real-SR中实现了最先进的性能，在恢复保真度和生成真实性之间提供了优越的平衡，同时保持了极高的效率。

Conclusion: Allo{SR}²通过校正单步超分辨率轨迹，有效解决了先验崩溃和轨迹漂移问题，为现实世界图像超分辨率提供了一种高效且高质量的解决方案。
...
TL;DR: 本文提出Deepening Reasoning MMSearchAgent框架，通过从完整轨迹中提取优势信号和使用差异化高斯奖励来解决多模态智能体过早交互崩溃的问题，在FVQA-test上性能提升8.4%。


<details>
  <summary>Details</summary>
Motivation: 多模态智能体在使用外部工具处理复杂任务时经常出现过早交互崩溃，主要原因有两个：1）终端奖励通常附加在最后一个token上，无法区分包含探索行为的轨迹；2）过度冗余的上下文阻碍了智能体吸收有用反馈。
...
Conclusion: 提出的框架通过从完整轨迹中提取优势信号和使用差异化高斯奖励，有效解决了多模态智能体的过早交互崩溃问题，显著提升了性能。
...
TL;DR: 本文提出了一种基于MCP惩罚和重加权ℓ1正则化的盲图像去模糊方法，通过改进的总变分正则化框架来同时估计点扩散函数和潜在清晰图像。


<details>
  <summary>Details</summary>
Motivation: 盲图像去模糊是图像处理中的关键挑战，需要同时估计点扩散函数和潜在清晰图像。传统ℓ0范数正则化虽然能促进稀疏性并保持边缘细节，但导致高度非凸和计算困难的问题，限制了实际应用。

Method: 采用极小极大凹惩罚(MCP)来增强稀疏性并更接近ℓ0范数近似，同时结合重加权ℓ1范数正则化以减少估计偏差并更好地保留图像细节和纹理。开发了相应的数值算法来求解优化问题。

Result: 通过在多幅测试图像上的实验评估，证明了所提方法的有效性，能够更好地恢复图像细节和纹理。

Conclusion: 提出的基于MCP惩罚和重加权ℓ1正则化的盲图像去模糊方法，通过改进的总变分正则化框架，克服了传统ℓ0范数正则化的计算困难，实现了更好的图像恢复效果。
...
TL;DR: 提出基于Transformer的多视角人群跟踪模型MVTrackTrans，并收集了两个大规模真实世界多视角跟踪数据集MVCrowdTrack和CityTrack，解决了现有数据集规模小、场景简单的问题。


<details>
  <summary>Details</summary>
Motivation: 当前多视角人群跟踪研究主要依赖CNN架构，且大多在Wildtrack和MultiviewX等小规模数据集上评估。这些数据集场景小、评估帧数少，难以应对真实世界中更复杂的场景规模和遮挡情况。
...
TL;DR: PLaMo 2.1-VL是一个轻量级视觉语言模型，专为自主设备设计，提供8B和2B版本，支持日语操作，专注于视觉问答和视觉定位任务，在工厂任务分析和基础设施异常检测等实际应用场景中表现出色。


<details>
  <summary>Details</summary>
Motivation: 开发一个轻量级的视觉语言模型，专门针对自主设备的本地和边缘部署需求，特别关注日语操作环境，解决工厂任务分析和基础设施异常检测等实际应用场景中的视觉理解问题。

Method: 开发了8B和2B两种规模的模型变体，构建了大规模合成数据生成管道和全面的日语训练评估资源，专注于视觉问答和视觉定位两大核心能力，并在工厂工具识别和基础设施异常检测两个实际应用场景中进行评估。

Result: 在日语和英语基准测试中超越了同类开源模型，在JA-VG-VQA-500上获得61.5 ROUGE-L，在日语Ref-L4上达到85.2%准确率。在应用场景中，工厂任务分析的零样本准确率为53.9%，在电厂数据上微调后异常检测的bbox+标签F1分数从39.7提升到64.9。

Conclusion: PLaMo 2.1-VL是一个有效的轻量级视觉语言模型，特别适合日语环境下的自主设备部署，在实际工业应用场景中表现出良好的性能，通过微调可以显著提升特定任务的检测能力。
...
TL;DR: RAFT-MSF++是一个自监督多帧单目场景流估计框架，通过递归融合时序特征联合估计深度和场景流，在KITTI基准上取得24.14%的SF-all指标，比基线提升30.99%。


<details>
  <summary>Details</summary>
Motivation: 现有单目场景流估计方法大多局限于两帧输入，限制了时序建模能力以及对遮挡区域的鲁棒性。为了解决这些问题，需要开发能够有效利用多帧信息的方法。

Method: 提出自监督多帧框架RAFT-MSF++，核心是几何-运动特征(GMF)，紧凑编码耦合的运动和几何线索，并通过迭代更新进行有效的时序推理。为增强遮挡区域的鲁棒性，引入相对位置注意力注入空间先验，以及遮挡正则化模块从可见区域传播可靠运动。

Result: 在KITTI场景流基准测试中达到24.14%的SF-all指标，相比基线提升30.99%，在遮挡区域表现出更好的鲁棒性。

Conclusion: RAFT-MSF++通过递归融合多帧时序特征，结合几何-运动特征、相对位置注意力和遮挡正则化，有效提升了单目场景流估计的性能，特别是在遮挡区域的表现。
...
TL;DR: 本文提出了一种改进Vision Transformers在乳腺X光片乳腺癌检测中的方法，通过ROI引导的token缩减、对比学习和DINOv2预训练来解决医学图像高分辨率、小病灶和细粒度分类的挑战。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在计算机视觉任务中表现出色，但在计算机辅助诊断中性能有限。针对乳腺X光片乳腺癌检测，存在两个主要问题：1）医学图像分辨率高、病灶小，导致token数量过多，softmax注意力难以定位相关区域；2）医学图像分类是细粒度任务，类间差异小、类内差异大，标准交叉熵训练不足。

Method: 提出包含三个关键组件的框架：1）基于目标检测模型的ROI token缩减，引导注意力；2）通过基于难负样本的对比学习增强细粒度区分能力；3）使用DINOv2预训练的ViT替代全局CLIP表示，捕获定位感知的细粒度特征。

Result: 在公共乳腺X光片数据集上的实验表明，该方法优于现有基线方法，证明了其有效性以及在大型乳腺癌筛查中的潜在临床价值。

Conclusion: 该方法通过解决ViT在医学图像分析中的特定挑战，显著提升了乳腺癌检测性能，为大规模乳腺癌筛查提供了有效的解决方案。
...
TL;DR: 本文研究了T恤攻击对人脸识别系统的威胁，并提出了一种基于空间一致性检查的检测方法，通过结合人脸检测器和人体检测器来分析检测到的面部和人体空间位置，从而可靠地检测T恤攻击。


<details>
  <summary>Details</summary>
Motivation: 人脸识别系统在生物认证中广泛应用，但容易受到呈现攻击的威胁。虽然已有呈现攻击检测方法在多个基准数据集上表现良好，但如何将这些方法泛化到新型攻击类型仍然是一个持续挑战。本文特别关注T恤攻击这种新型呈现攻击方式。

Method: 提出了一种基于空间一致性检查的检测方法，结合最先进的人脸检测器和人体检测器，分析检测到的面部和人体空间位置关系。使用TFPA数据库中的1,608个T恤攻击样本（包含100种独特的呈现攻击工具）和152个真实呈现样本进行综合评估。

Result: 评估表明T恤攻击确实能够威胁人脸识别系统的安全性。提出的检测方法通过分析人脸和人体检测结果的空间一致性，能够可靠地检测T恤攻击。

Conclusion: T恤攻击是人脸识别系统面临的一种新型安全威胁，而基于空间一致性检查的检测方法能够有效应对这种攻击，为人脸识别系统的安全性提供了新的防护手段。
...
TL;DR: 该研究提出并评估了一个基于EEG的驾驶员意图预测框架，在真实道路条件下使用同步多传感器平台，TSCeption架构获得了最高准确率（0.907）和Macro-F1分数（0.901），证明了在真实驾驶环境中早期稳定解码驾驶员意图的可行性。


<details>
  <summary>Details</summary>
Motivation: 从神经生理信号预测驾驶员意图为增强先进驾驶辅助系统的主动安全性提供了有前景的途径，但由于EEG信号的非平稳性和认知-运动准备的复杂性，在真实驾驶环境中仍然具有挑战性。

Method: 研究提出了一个基于EEG的驾驶员意图预测框架，使用集成到真实电动汽车中的同步多传感器平台。收集了32个驾驶会话的真实道路数据集，在一致的实验条件下评估了12种深度学习架构。

Result: 在评估的架构中，TSCeption获得了最高的平均准确率（0.907）和Macro-F1分数（0.901）。该框架表现出强的时间稳定性，在操作执行前1000毫秒内保持稳健的解码性能，性能下降最小。此外，分析显示最小EEG预处理优于伪迹处理流程，预测性能在400-600毫秒区间达到峰值。

Conclusion: 这些发现支持了在真实道路条件下早期稳定地基于EEG解码驾驶员意图的可行性，为增强驾驶安全系统提供了重要基础。
...
TL;DR: 首次提出用于多模态3D全景分割的无监督域适应框架PanDA，解决自动驾驶中域偏移导致的模态退化和伪标签不完整问题


<details>
  <summary>Details</summary>
Motivation: 现实自动驾驶中常遇到域偏移问题，现有多模态3D全景分割方法严重依赖LiDAR和RGB的跨模态互补性，在单模态退化（如恶劣天气、光照条件）时表现脆弱。传统伪标签方法仅保留高置信度区域，导致掩码碎片化和不完整监督，对全景分割尤为不利。

Method: 提出PanDA框架：1）非对称多模态增强：选择性丢弃区域以模拟域偏移，提升鲁棒表示学习；2）双专家伪标签细化模块：从2D和3D模态提取域不变先验，增强伪标签完整性和可靠性。

Result: 在涵盖时间、天气、位置和传感器变化等多种域偏移场景的广泛实验中，显著超越了现有3D语义分割的UDA基准方法。

Conclusion: PanDA是首个专门为多模态3D全景分割设计的UDA框架，通过非对称多模态增强和双专家伪标签细化，有效解决了域偏移下的模态退化和伪标签不完整问题，在多种真实世界域偏移场景中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 卫星图像合成在遥感应用中至关重要，如数据增强、灾害模拟和城市规划。现有方法在处理不同域条件下的合成图像协调方面存在挑战。

Method: 提出训练免费的扩散框架，通过潜在均值偏移操作对齐源域和目标域，采用时间步潜在融合策略平衡协调与内容保留，并使用轻量级协调分类器自动选择最佳结果。

Result: 实验证明该方法能有效执行卫星图像合成，展示了在可扩展遥感合成和模拟任务中的强大潜力。构建了包含500个配对样本的RSIC-H基准数据集。

Conclusion: HarmoniDiff-RS为卫星图像协调提供了有效的训练免费解决方案，在遥感合成应用中具有重要价值。
...
TL;DR: VecHeart是一个统一框架，用于四腔心脏结构的整体重建与生成，通过混合部件Transformer和解剖补全掩码策略，能够从部分、稀疏或噪声观察中推断完整结构，并扩展到3D+时间动态网格序列生成。


<details>
  <summary>Details</summary>
Motivation: 当前前馈隐式方法存在两个主要限制：1) 仅限于单对象建模；2) 忽略部件间相关性。准确的心脏解剖建模需要模型能够处理结构间复杂的相互关系。

Method: 提出VecHeart框架，引入混合部件Transformer，利用部件特定的可学习查询和交错注意力来捕捉复杂的腔室间依赖关系。还提出解剖补全掩码和模态对齐策略，使模型能够从部分、稀疏或噪声观察中推断完整的四腔结构。

Result: 实验表明该方法达到了最先进的性能，在多样化的挑战性场景中保持高保真重建。模型还能无缝扩展到3D+时间动态网格序列生成，展现出卓越的通用性。

Conclusion: VecHeart为四腔心脏结构的整体重建和生成提供了一个统一框架，通过创新的混合部件Transformer和解剖补全策略，有效解决了现有方法的局限性，在复杂场景下表现出色，并具有扩展到动态序列生成的潜力。
...
TL;DR: HP-Edit是一个基于人类偏好的图像编辑后训练框架，通过构建RealPref-50K真实世界数据集和HP-Scorer自动评估器，使用RLHF技术提升扩散模型在多种编辑任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 虽然扩散模型已成为图像编辑的主流方法，但将强化学习从人类反馈（RLHF）高效应用于基于扩散的编辑仍然探索不足，主要缺乏针对多样化编辑需求的可扩展人类偏好数据集和专门框架。
...
Result: 大量实验表明，该方法显著提升了如Qwen-Image-Edit-2509等模型的性能，使其输出更符合人类偏好。同时提出了RealPref-Bench基准来评估真实世界编辑性能。

Conclusion: HP-Edit框架通过结合人类偏好数据和自动评估器，成功将RLHF技术应用于扩散模型的图像编辑任务，填补了该领域的研究空白，为真实世界图像编辑提供了有效的后训练解决方案。
...
TL;DR: GOLD-BEV：一种从车载传感器学习密集鸟瞰图语义环境映射的框架，使用时间同步的航拍图像作为训练监督，无需人工标注即可生成密集语义地图。


<details>
  <summary>Details</summary>
Motivation: 现有方法在几何一致、场景中心的道路场景理解方面存在不足，特别是对动态物体的BEV标注存在模糊性和时间不一致问题。需要一种能够从车载传感器学习密集BEV语义映射的框架，同时避免人工标注的繁琐和歧义。

Method: 使用时间同步的航拍图像作为训练监督，通过BEV对齐的航拍裁剪提供直观目标空间。采用领域适应的航拍教师模型生成BEV伪标签，联合训练BEV分割和可选的伪航拍BEV重建。扩展方法包括从车载传感器合成伪航拍BEV图像，支持轻量级人工标注和不确定性感知的伪标签生成。

Result: 框架能够学习密集的BEV语义环境地图，包括动态物体，仅需训练时的时间同步航拍监督。通过伪标签生成实现可扩展的密集目标，并能扩展到航拍覆盖范围之外，支持无标注驾驶数据的处理。

Conclusion: GOLD-BEV提供了一种有效的框架，利用航拍图像监督从车载传感器学习密集BEV语义映射，解决了传统BEV标注的模糊性和时间不一致问题，实现了可扩展的密集语义环境理解。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型经常出现物体幻觉问题，即在图像描述中生成不存在的物体，这在医疗影像和自动驾驶等关键应用中尤为危险。研究表明幻觉问题可能源于语言先验偏见。

Method: 提出视觉对比编辑方法，通过分析模型对对比性视觉扰动的响应来识别和抑制幻觉倾向。使用奇异值分解分解模型激活模式，隔离幻觉子空间，并应用针对性参数编辑来减弱其影响。

Result: 实验结果表明VCE在多个基准测试中有效减少了物体幻觉，同时保持了模型原有的计算效率。

Conclusion: VCE作为一种无需标注的后处理方法，既具有可扩展性，又适合在资源受限的环境中部署，为解决LVLM中的物体幻觉问题提供了实用方案。
...
TL;DR: 本文回顾了LoViF挑战赛关于真实世界一体化图像恢复的研究，该挑战旨在推动在多种真实世界退化条件下的图像恢复技术发展，包括模糊、低光、雾霾、雨雪等，为评估模型在统一框架下的鲁棒性和泛化能力提供了基准。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像恢复面临多种退化条件（模糊、低光、雾霾、雨雪等）的挑战，需要开发能够在统一框架下处理多种退化类型的鲁棒模型。现有研究通常针对单一退化类型，缺乏对一体化恢复模型的系统性评估。

Method: 通过组织LoViF挑战赛，为真实世界一体化图像恢复建立统一的评估基准。挑战赛吸引了124名注册参与者，最终收到9份有效提交。报告对提交的方法进行了详细分析，包括模型架构、训练策略和评估指标。

Result: 挑战赛成功推动了真实世界一体化图像恢复领域的研究进展，建立了首个针对多种退化类型的统一评估框架。提交的方法展示了不同的技术路线，为后续研究提供了有价值的参考。

Conclusion: LoViF挑战赛为真实世界一体化图像恢复领域建立了重要的基准，促进了该领域的技术发展。分析结果强调了有效的方法论，并为未来真实世界低级视觉研究提供了参考框架。
...
Method: 提出无需训练的时序可分离注意力机制(TS-Attn)，动态重排注意力分布以确保多事件场景中的时间感知和全局一致性。该机制可无缝集成到各种预训练文本到视频模型中，支持即插即用。

Result: 在Wan2.1-T2V-14B和Wan2.2-T2V-A14B模型上，StoryEval-Bench得分分别提升33.5%和16.4%，推理时间仅增加2%。同时支持跨模型的多事件图像到视频生成。

Conclusion: TS-Attn有效解决了多时序动作视频生成中的核心挑战，通过创新的注意力机制在不增加训练成本的情况下显著提升现有模型的性能，为复杂时序描述的视频生成提供了实用解决方案。
...
TL;DR: 本文提出了一种基于精灵的图像分解方法，在CLEVR基准测试中与最先进的无监督类别感知图像分割方法表现相当，能够线性扩展处理多对象图像，明确识别对象类别，并以高度可解释的方式建模图像。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在图像分割方面取得稳定进展，扩散算法生成越来越逼真的图像，但在图像集合中识别重复模式这一看似简单的问题仍然非常开放。精灵基图像分解模型在聚类和图像分解方面显示出潜力，因其高可解释性而具有吸引力，但存在需要针对特定数据集定制、难以扩展到多对象图像等问题。

Method: 深入分析现有精灵基图像分解模型的设计细节，识别其核心组件，在聚类基准上进行广泛分析。基于此分析提出一种深度精灵基图像分解方法，该方法能够线性扩展处理多对象，明确识别对象类别。

Result: 提出的方法在标准CLEVR基准测试中与最先进的无监督类别感知图像分割方法表现相当，能够线性扩展处理多对象图像，明确识别对象类别，并以高度可解释的方式完全建模图像。

Conclusion: 深度精灵基图像分解方法解决了现有模型在可扩展性和通用性方面的限制，在保持高可解释性的同时，实现了与最先进方法相当的性能，为图像集合中的模式识别提供了有效的解决方案。
...
TL;DR: 该研究比较了传统计算机视觉模型与多模态大语言模型在视觉政治传播分析中的表现，发现GPT-4o在识别政治人物和统计图像人数方面表现最优。


<details>
  <summary>Details</summary>
Motivation: 评估专业机器学习模型和新兴多模态大语言模型在视觉政治传播分析中的能力，特别是在政治竞选期间社交媒体视觉内容分析方面。

Method: 使用2021年德国联邦选举期间Instagram故事和帖子的视觉内容作为案例研究，比较传统计算机视觉模型（FaceNet512、RetinaFace、Google Cloud Vision）与多模态大语言模型（GPT-4o）在识别主要政治人物和统计图像中人数方面的性能。
...
Result: 叶片聚焦样本显示边际增益，而冠层子集显著改善，表明平衡归一化与直方图多样化能有效弥合由不受控光照引起的域差距。

Conclusion: 直方图匹配的双阶段集成显著增强了葡萄藤病害分类模型在真实世界冠层图像上的鲁棒性，为解决光照变化问题提供了有效方法。
...
TL;DR: 提出主动映射移动物体的新任务，开发无需学习的Paparazzo方法，通过轨迹预测和视点选择来规划路径，显著提升3D重建完整性和准确性


<details>
  <summary>Details</summary>
Motivation: 当前3D映射流程通常假设静态环境，无法准确捕捉和重建移动物体，这限制了动态场景理解能力

Method: 提出Paparazzo方法，无需学习即可稳健预测目标轨迹，识别最具信息量的观测视点，并规划自身路径

Result: 与多个强基线相比，Paparazzo显著提高了3D重建的完整性和准确性
...
<details>
  <summary>Details</summary>
Motivation: 自我中心助手需要第一人称视角数据来捕捉用户行为和上下文以实现个性化服务。不同用户具有不同的习惯、偏好和日常行为，这种个性化对于真正有效的辅助至关重要。然而，有效整合长期用户数据进行个性化仍然是一个关键挑战。

Method: 引入EgoSelf系统，包括：1）基于过去观察构建的图结构交互记忆，捕捉交互事件和实体之间的时间和语义关系；2）专门的个性化学习任务，将个性化学习制定为预测问题，模型根据图中记录的个体用户历史行为预测可能的未来交互。

Result: 大量实验证明了EgoSelf作为个性化自我中心助手的有效性。代码已开源。

Conclusion: EgoSelf通过图结构记忆和个性化预测任务，有效解决了长期用户数据整合的挑战，实现了有效的个性化自我中心助手。
...
TL;DR: RF-HiT是一种用于医学图像分割的Rectified Flow Hierarchical Transformer模型，通过结合层次化编码器和整流流技术，在保持高性能的同时实现了线性计算复杂度和快速推理。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer和扩散模型的医学图像分割方法存在二次计算复杂度和推理延迟高的问题，难以同时满足长距离上下文推理和精确边界分割的需求。

Method: 提出RF-HiT模型，结合沙漏Transformer主干和多尺度层次化编码器，采用整流流技术实现线性复杂度，通过可学习插值融合多尺度特征，仅需少量离散化步骤。

Result: 模型仅需10.14 GFLOPs、13.6M参数和最少3步推理，在ACDC数据集上达到91.27%平均Dice分数，在BraTS 2021上达到87.40%，性能与更复杂的架构相当或更好。

Conclusion: RF-HiT在效率和性能之间取得了良好平衡，展示了作为实时临床分割强大且计算高效基础模型的潜力。
...
TL;DR: TransSplat提出了一种新的语言驱动3D高斯泼溅编辑方法，通过多视角不平衡语义传输建立2D编辑证据与3D高斯之间的语义对应关系，解决了现有方法在局部编辑精度和结构一致性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有语言驱动3D高斯泼溅编辑方法主要关注提升多视角一致性，但未能明确解决编辑后的2D证据与3D高斯之间的语义对应关系这一根本问题，导致局部编辑精度不足和编辑泄漏问题。

Method: 将语言驱动3DGS编辑建模为多视角不平衡语义传输问题：1）建立可见高斯与视角特定编辑原型之间的对应关系；2）恢复跨视角共享的规范3D编辑场以指导统一的3D外观更新；3）使用传输残差抑制非目标区域的错误编辑，减少编辑泄漏。

Result: 定性和定量实验表明，与现有专注于增强视角一致性的3D编辑方法相比，TransSplat在局部编辑精度和结构一致性方面取得了优越性能。

Conclusion: 通过明确建模2D编辑证据与3D高斯之间的语义对应关系，TransSplat解决了语言驱动3D高斯泼溅编辑中的根本问题，实现了更精确的局部编辑和更好的结构一致性。
...
TL;DR: SmartPhotoCrafter是一个自动摄影图像编辑方法，通过紧密耦合的推理到生成过程，无需用户提供明确的美学指令即可自动提升图像质量。
...
Method: 提出一个自动摄影图像编辑方法，将图像编辑建模为紧密耦合的推理到生成过程。模型包含图像批评模块进行图像质量理解和缺陷识别，以及摄影艺术家模块实现针对性编辑。采用多阶段训练流程：基础预训练、推理引导的多编辑监督适应、协调推理到生成强化学习。

Result: 实验表明SmartPhotoCrafter在自动摄影增强任务上优于现有生成模型，实现了照片级真实感结果，同时对调色指令表现出更高的色调敏感性。

Conclusion: SmartPhotoCrafter通过自动化的推理到生成框架，消除了对明确人类美学指令的依赖，能够为专业和非专业用户提供高质量的自动摄影图像增强。
...
TL;DR: SSDM框架通过结构-语义解耦调制，将全局地理空间表示分解为两个互补的跨模态注入路径，有效解决高分辨率遥感制图中特征干扰和空间结构退化问题。


<details>
  <summary>Details</summary>
Motivation: 传统细粒度高分辨率遥感制图依赖局部视觉特征，导致跨域泛化性差和大尺度地表覆盖预测碎片化。全局地理空间基础模型虽提供强大表示，但直接融合其高维隐式嵌入与高分辨率视觉特征会因语义-空间鸿沟引发特征干扰和空间结构退化。

Method: 提出结构-语义解耦调制(SSDM)框架：1) 结构先验调制分支将全局表示的宏观感受野先验引入高分辨率编码器的自注意力模块，用整体结构约束指导局部特征提取；2) 全局语义注入分支显式对齐整体上下文与深层高分辨率特征空间，通过跨模态集成直接补充全局语义。

Result: 广泛实验表明，相比现有跨模态融合方法，该方法达到最先进性能。通过释放全局嵌入潜力，SSDM在不同场景下持续提升高分辨率制图精度。
...
TL;DR: 该论文提出了PC2Model基准数据集，用于点云到三维模型的配准任务，结合模拟和真实世界数据以解决实际应用中稀疏、噪声、遮挡等挑战。


<details>
  <summary>Details</summary>
Motivation: 随着点云获取技术（如LiDAR和结构光扫描）的普及和深度学习的发展，研究重点逐渐转向下游任务，特别是点云到模型（PC2Model）配准。然而，数据驱动方法在处理真实世界扫描中的稀疏性、噪声、杂乱和遮挡时表现受限，需要更好的基准数据集来支持方法开发和评估。

Method: 论文提出了PC2Model基准数据集，采用混合设计：结合模拟点云（提供精确地面真值和受控条件）与部分真实世界扫描及其对应的3D模型（引入传感器和环境伪影）。该数据集由ICWG II/Ib领导开发，支持经典方法和数据驱动方法的训练和评估。

Result: PC2Model基准数据集已公开发布在https://zenodo.org/uploads/17581812，支持跨领域的鲁棒训练和评估，并能够系统分析模型从模拟到真实世界场景的可迁移性。

Conclusion: PC2Model基准数据集通过混合模拟和真实数据的设计，解决了点云配准中数据驱动方法面临的挑战，为点云到模型配准任务提供了标准化的训练和评估平台，促进了该领域的研究进展。
...
TL;DR: GRAFT提出了一种快速的前馈式3D人-场景交互重建方法，通过预测交互梯度迭代优化人体网格，在保持优化方法精度的同时实现50倍加速。


<details>
  <summary>Details</summary>
Motivation: 当前单图像3D人-场景交互重建存在两难：优化方法准确但速度慢（约20秒），前馈方法快速但缺乏显式交互推理，导致漂浮和穿透伪影。需要一种既能保持几何精度又能实现快速推理的方法。

Method: GRAFT是一种学习的HSI先验，通过预测交互梯度来迭代优化人体网格。它将交互状态编码为紧凑的身体锚定token，通过几何探针捕获与附近表面的空间关系。轻量级transformer递归更新人体网格并重新探测场景，确保最终姿态既符合学习先验又符合观测几何。

Result: GRAFT将交互质量提升高达113%，相比优化方法在保持相同交互质量的同时实现约50倍加速。在用户研究中获得64.8%的偏好率，并能无缝泛化到野外多人场景。

Conclusion: GRAFT成功将基于几何的人-场景拟合摊销到快速前馈推理中，实现了速度与精度的平衡，可作为可迁移的即插即用HSI先验提升现有前馈方法。
...
TL;DR: CoCo-SAM3通过解耦推理为类内增强和类间竞争，解决了SAM3在多类开放词汇语义分割中证据尺度不统一和类间冲突的问题，无需额外训练即可在多个基准上取得一致改进。


<details>
  <summary>Details</summary>
Motivation: SAM3在多类开放词汇场景中，不同类别提示独立生成的掩码缺乏统一且可比较的证据尺度，导致覆盖重叠和不稳定竞争。同时，同一概念的同义表达会激活不一致的语义和空间证据，造成类内漂移，加剧类间冲突并损害推理稳定性。

Method: CoCo-SAM3将推理明确解耦为类内增强和类间竞争两个阶段。首先对齐并聚合来自同义提示的证据以增强概念一致性，然后在统一可比较的尺度上进行类间竞争，实现所有候选类别之间的直接像素级比较。

Result: 无需任何额外训练，CoCo-SAM3在八个开放词汇语义分割基准上取得了一致的改进。

Conclusion: CoCo-SAM3通过类内增强和类间竞争的解耦推理机制，有效稳定了多类推理并缓解了类间冲突，为开放词汇语义分割提供了更稳定和有效的解决方案。
...
### [71] [CreatiParser: Generative Image Parsing of Raster Graphic Designs into Editable Layers](https://arxiv.org/abs/2604.19632)
*Weidong Chen,Dexiang Hong,Zhendong Mao,Yutao Cheng,Xinyan Liu,Lei Zhang,Yongdong Zhang*

Main category: cs.CV

TL;DR: 提出了一种混合生成框架，将栅格化设计图像分解为可编辑的文本、背景和贴纸图层，支持高质量重建和灵活编辑。


<details>
  <summary>Details</summary>
Motivation: 现有图形设计解析方法通常采用多阶段流水线，存在误差累积和可控性有限的问题，而大多数生成模型输出的栅格化结果缺乏明确的图层结构，限制了后续编辑能力。
...
Result: 在Parser-40K和Crello两个挑战性数据集上的实验表明，该方法在所有指标上平均提升23.7%，显著优于现有方法。

Conclusion: 该框架能够将栅格化设计图像有效分解为可编辑图层，实现了高质量的重建和灵活的重新编辑，为图形设计解析提供了新的解决方案。
...
### [72] [CoInteract: Physically-Consistent Human-Object Interaction Video Synthesis via Spatially-Structured Co-Generation](https://arxiv.org/abs/2604.19636)
*Xiangyang Luo,Xiaozhe Xin,Tao Feng,Xu Guo,Meiguang Jin,Junfeng Ma*

Main category: cs.CV

TL;DR: CoInteract是一个用于合成人-物交互视频的端到端框架，通过人类感知的专家混合和空间结构协同生成技术，解决了现有扩散模型在手部、面部结构稳定性和物理接触合理性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型虽然在照片级渲染方面表现出色，但在人-物交互视频合成中仍存在两个主要问题：1）手部和面部等敏感区域的结构稳定性不足；2）物理接触合理性差（如手与物体相互穿透）。这些问题限制了其在电子商务、数字广告和虚拟营销等领域的实际应用价值。

Method: CoInteract基于扩散变换器（DiT）架构，引入两种互补设计：1）人类感知的专家混合（MoE），通过空间监督路由将令牌分配给轻量级的区域专业化专家，以最小参数开销提高细粒度结构保真度；2）空间结构协同生成，采用双流训练范式，同时建模RGB外观流和辅助的HOI结构流，注入交互几何先验。训练时HOI流关注RGB令牌并监督共享骨干权重，推理时移除HOI分支实现零开销RGB生成。

Result: 实验结果表明，CoInteract在结构稳定性、逻辑一致性和交互真实性方面显著优于现有方法。

Conclusion: CoInteract通过创新的专家混合架构和协同生成策略，有效解决了人-物交互视频合成中的结构稳定性和物理接触合理性问题，为相关应用提供了高质量的合成解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法主要基于扩散模型，存在迭代采样计算开销大、受限于UNet参数化的问题。流匹配作为连续时间传输映射的学习框架，为生成式分割提供了理论基础且计算高效。

Method: 提出MedFlowSeg条件流匹配框架，将分割建模为学习时间相关向量场；开发双条件机制：双分支空间注意力模块注入多尺度结构信息，频率感知注意力模块通过差异感知融合和时间相关调制建模空间与谱表示的跨域交互。

Result: 在多种医学成像模态上验证，MedFlowSeg达到最先进性能，同时相比基于扩散的方法显著降低计算成本。

Conclusion: 流匹配作为理论基础扎实且计算高效的替代方案，在生成式医学图像分割中具有潜力，MedFlowSeg展示了其优越性。
...
TL;DR: MMControl是一个多模态控制框架，能够在联合音视频生成中同时控制视觉和听觉条件，实现身份一致、音色一致且结构可控的音视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有的可控生成框架通常仅限于视频控制，缺乏对音频的控制能力，这限制了全面可控性并导致跨模态对齐不理想。需要开发能够同时处理视觉和听觉控制信号的联合音视频生成框架。

Method: 提出MMControl框架，采用双流条件注入机制，将参考图像、参考音频、深度图和姿态序列等视觉和听觉控制信号注入到联合音视频扩散变换器中。通过旁路分支注入条件，并引入模态特定指导缩放，允许在推理时独立动态调整每个条件的影响强度。

Result: 实验表明MMControl能够在联合音视频生成中实现对角色身份、声音音色、身体姿态和场景布局的细粒度可组合控制，生成身份一致、音色一致且结构可控的音视频内容。

Conclusion: MMControl填补了现有可控生成框架的空白，实现了真正的多模态控制，为联合音视频生成提供了更全面、灵活的控制能力，显著提升了跨模态对齐质量。
...
TL;DR: IR-Flow是一个基于Rectified Flow的图像修复统一框架，通过构建多级数据分布流和累积速度场，在判别式和生成式方法间架起桥梁，实现快速推理并提升对分布外退化的适应性。


<details>
  <summary>Details</summary>
Motivation: 图像修复中，单步判别式映射通过期望学习往往缺乏精细细节，而生成式方法则存在多步采样效率低下和噪声-残差耦合的问题。需要一种统一框架来弥合这两种范式之间的差距。

Method: 提出IR-Flow方法：1）构建多级数据分布流，增强模型学习和适应不同退化水平的能力；2）提出累积速度场学习不同退化水平间的传输轨迹，引导中间状态向干净目标；3）引入多步一致性约束确保轨迹连贯性并提升少步修复性能。

Result: 在去雨、去噪和雨滴去除任务上的广泛评估表明，IR-Flow仅需少量采样步骤就能获得有竞争力的定量结果，提供了一个高效灵活且保持良好失真-感知平衡的框架。

Conclusion: IR-Flow通过在退化图像域和干净图像域之间直接建立线性传输流，不仅实现了快速推理，还提高了对分布外退化的适应性，为图像修复提供了一个统一且高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs评估基准存在两个主要问题：1）由于模态冗余允许单模态捷径；2）主要关注最终答案准确性而忽略推理过程。STEM领域因其高度可验证性成为有价值的测试平台。
...
TL;DR: 提出基于规范面部点预测的统一4D面部重建方法，将动态重建转化为规范空间重建问题，实现几何一致性和可靠对应关系
...
TL;DR: SpanVLA是一个端到端自动驾驶框架，结合自回归推理和流匹配动作专家，通过高效桥接视觉语言模型的推理指导来规划轨迹，显著减少推理时间，并通过GRPO后训练方法提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型在动作生成时存在高延迟问题，且鲁棒性有限，特别是在长尾场景中表现不佳。需要一种既能利用世界知识和推理能力，又能高效生成动作并具备良好鲁棒性的自动驾驶解决方案。

Method: 1. 提出SpanVLA框架，整合自回归推理和流匹配动作专家；2. 通过高效桥接机制，利用VLM的视觉和推理指导来规划未来轨迹，基于历史轨迹初始化进行流匹配策略；3. 提出GRPO后训练方法，让模型不仅能从正样本学习，还能学习避免典型负行为和恢复行为；4. 引入mReasoning数据集，专注于复杂推理场景和负恢复样本。

Result: 在NAVSIM（v1和v2）数据集上的广泛实验表明，SpanVLA模型具有竞争性性能。跨多种场景的定性结果突出了模型的规划性能和鲁棒性优势。

Conclusion: SpanVLA通过结合自回归推理和流匹配动作专家，有效解决了现有VLA模型的高延迟和鲁棒性限制问题，为自动驾驶提供了一种高效且鲁棒的端到端解决方案。
...
TL;DR: GDM是一个生成漂移框架，将确定性医学图像预测重新表述为多目标学习问题，在保持一步推理的同时联合提升分布级合理性和患者特异性保真度，特别针对3D医学成像。
...
Method: GDM通过吸引-排斥漂移最小化生成器前推分布与目标分布之间的差异；构建多级特征库支持可靠亲和度估计和漂移场计算；采用共享输出空间的梯度协调策略改善竞争目标间的优化平衡。

Result: 在MRI-to-CT合成和稀疏视图CT重建任务上，GDM优于多种基线方法（GAN、流匹配、SDE生成模型及监督回归方法），在解剖保真度、定量可靠性、感知真实性和推理效率之间取得更好平衡。

Conclusion: GDM为条件3D医学图像生成提供了一个实用有效的框架，能够同时提升分布级合理性和患者特异性保真度，同时保持高效推理。
...
TL;DR: CityRAG是一个视频生成模型，利用地理注册数据作为上下文，生成与真实物理场景一致的可导航3D环境，支持任意天气条件和动态物体配置。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型虽然能生成与文本或图像提示一致的序列，但缺乏重建真实世界任意天气和动态配置的能力，而这对于自动驾驶和机器人仿真等下游应用至关重要。

Method: CityRAG利用大规模地理注册数据作为上下文，将生成过程锚定到物理场景，同时保持对复杂运动和外观变化的先验知识。模型使用时间未对齐的训练数据，学习从底层场景中语义解耦瞬态属性。

Result: CityRAG能够生成连贯的分钟级物理基础视频序列，在数千帧中保持天气和光照条件，实现闭环导航，并沿着复杂轨迹重建真实世界地理环境。
...
TL;DR: AnyRecon是一个从任意无序稀疏输入进行3D重建的可扩展框架，通过持久全局场景记忆和几何感知条件策略，在保持显式几何控制的同时支持灵活的条件基数。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D重建对于从随意拍摄的场景建模至关重要，但对非生成式重建仍然具有挑战性。现有的基于扩散的方法通过合成新视图来缓解这一问题，但通常只基于一到两个捕获帧进行条件化，这限制了几何一致性，并难以扩展到大型或多样化场景。

Method: 1) 通过前置捕获视图缓存构建持久全局场景记忆，移除时间压缩以保持大视角变化下的帧级对应关系；2) 引入几何感知条件策略，通过显式3D几何记忆和几何驱动的捕获视图检索耦合生成与重建；3) 结合4步扩散蒸馏和上下文窗口稀疏注意力来降低二次复杂度。

Result: 大量实验证明，该方法在非规则输入、大视角差距和长轨迹情况下都能实现鲁棒且可扩展的重建。
...
TL;DR: 提出了一种二维早退策略，在句子和层两个维度上协调早退，为LLM分类任务带来乘性计算节省，相比单维度优化获得额外1.4-2.3倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有早退策略主要在层维度优化，但忽略了输入结构（如句子）中的语义信息分布规律。通过同时考虑层和句子两个维度的早退，可以更有效地利用计算资源，实现更大的计算节省。

Method: 提出二维早退策略：1）句子维度：逐句处理输入；2）层维度：逐步激活更深层。结合两个维度的早退实现乘性计算节省。方法轻量，仅需分类适配器，与量化、剪枝等效率方法正交。

Result: 在4个SOTA LLM（Llama 3.1/3.2、Gemma、Qwen；3B-8B参数）和3个情感分类数据集上验证：相比最优层早退，简单任务获得1.4-2.3倍额外加速；复杂多分类任务性能下降平缓；微调会减少但不会消除优势。

Conclusion: 二维早退策略在语义信息可预测地累积于输入结构时表现优异，表明该方法可扩展到情感分类之外的序列处理任务。方法模型无关，仅需轻量适配器，与现有效率技术兼容。
...
TL;DR: 该研究通过线性回归分析语言模型各层表示与人类阅读时间的关系，发现早期层表示在预测早期阅读指标上优于惊奇度，而晚期阅读指标仍以惊奇度预测更佳，表明模型深度与人类阅读处理阶段存在功能对齐。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究表明语言模型表示编码了丰富的语言信息，但尚不清楚这些表示是否也捕捉到人类处理过程的认知信号。本研究旨在探究语言模型表示是否能够预测人类阅读时间，以验证模型表示与人类认知处理之间的关联。

Method: 使用正则化线性回归方法，在两个眼动追踪数据集（涵盖英语、希腊语、希伯来语、俄语和土耳其语五种语言）上，将语言模型每一层的表示与标量预测因子（惊奇度、信息价值和logit-lens惊奇度）进行比较，分析它们对人类阅读时间的预测能力。

Result: 研究发现：1）早期层表示在预测早期阅读指标（如首次注视时间和凝视持续时间）上优于惊奇度；2）预测能力集中在早期层，表明人类处理特征由低层结构或词汇表示捕获；3）对于晚期阅读指标（如总阅读时间），标量惊奇度仍表现更佳；4）结合惊奇度和早期层表示可获得性能提升；5）最佳预测因子因语言和眼动指标而异。

Conclusion: 语言模型表示确实编码了人类阅读处理的认知信号，早期层表示与人类早期阅读处理阶段存在功能对齐，而晚期阅读处理则更依赖于惊奇度等压缩表示。模型深度与人类阅读的时间阶段存在对应关系，但最佳预测因子具有语言和任务特异性。
...
TL;DR: 该研究分析了Google AlphaEarth地球观测基础模型嵌入向量的几何结构，发现其流形具有非欧几里得特性，并开发了一个基于几何理解的智能体系统用于环境推理。


<details>
  <summary>Details</summary>
Motivation: 地球观测基础模型将地表信息编码为密集嵌入向量，但这些表示的几何结构及其对下游推理的影响尚未得到充分探索。研究者希望理解这些嵌入的流形几何特性，并利用这种理解来改进环境推理任务。

Method: 1. 分析Google AlphaEarth的64维嵌入在1210万个美国大陆样本（2017-2023）上的流形几何特性；2. 开发包含9个专用工具的智能体系统，将环境查询分解为基于FAISS索引嵌入数据库的推理链；3. 进行五条件消融实验（120个查询，三个复杂度层级）评估系统性能；4. 跨模型基准测试比较不同模型在几何工具辅助下的表现。

Result: 1. 嵌入流形是非欧几里得的：有效维度为13.3（参与率），局部内在维度约10；2. 切空间显著旋转，84%位置超过60°，局部-全局对齐接近随机基线；3. 概念方向在流形上旋转，组合向量算术效果差；4. 检索方法产生物理一致的结果，局部几何预测检索一致性（R²=0.32）；5. 嵌入检索主导响应质量（3.79±0.90 vs 3.03±0.77），多步比较表现最佳（4.28±0.43）；6. 几何工具对不同模型影响不同，Opus 4.6获得更高几何基础性（3.38 vs 2.64）。

Conclusion: 地球观测基础模型的嵌入具有复杂的非欧几里得几何结构，传统的线性方法（如向量算术）效果有限。基于检索的方法能产生更物理一致的结果，而几何特征的价值与使用模型的推理能力相关。智能体系统通过利用嵌入的几何特性，能够有效支持复杂的环境推理任务。
...
TL;DR: 本文全面综述了音译在跨语言NLP中的应用，探讨了如何通过音译克服书写系统差异带来的"文字壁垒"，分析了不同音译方法的演变、效果和权衡，并为研究者提供了基于具体语言、任务和资源约束的音译策略选择建议。


<details>
  <summary>Details</summary>
Motivation: 跨语言NLP中存在的"文字壁垒"（书写系统差异）阻碍了语言间的迁移学习。音译作为一种转换书写系统的技术，通过增加词汇重叠来弥合这一差距，成为解决跨语言迁移障碍的重要方法。

Method: 1. 提出音译在语言模型中应用的关键动机分类法；2. 概述将音译作为输入的不同方法；3. 分析这些方法的演变和有效性；4. 讨论现代大语言模型中的关键权衡；5. 探索音译有益的各种应用场景。

Result: 1. 音译能有效处理代码混合文本；2. 利用语言家族相关性；3. 在推理效率方面获得实际增益；4. 为研究者提供了基于语言、任务和资源约束的具体音译策略选择建议。

Conclusion: 音译是克服跨语言NLP中"文字壁垒"的有效技术，通过增加词汇重叠促进迁移学习。研究者应根据具体语言特点、任务需求和资源限制选择最合适的音译策略，音译在代码混合文本处理、语言家族关系利用和推理效率提升等方面具有重要应用价值。
...
TL;DR: 该论文通过幽默分析研究语言模型中的反事实不公平性，通过身份互换观察模型反应变化，发现特权身份说话者的笑话被拒绝率更高、恶意判断更频繁、社会危害评分更高


<details>
  <summary>Details</summary>
Motivation: 幽默反映了社会认知，语言模型对幽默的反应暴露了其从训练数据中内化的社会假设。研究者希望通过幽默分析来揭示模型中的反事实不公平性，特别是身份相关的偏见问题。

Method: 提出一个框架，通过身份互换（保持其他因素不变）观察模型反应变化。涵盖三个任务：幽默生成拒绝、说话者意图推断、关系/社会影响预测，包括身份无关幽默和身份特定贬低幽默。引入可解释的偏见度量来捕捉身份互换下的不对称模式。

Result: 实验显示一致的关系差异：特权身份说话者的笑话被拒绝率高出67.5%，被判断为恶意的频率高出64.7%，在5分量表上的社会危害评分高出1.5分。这些模式揭示了生成模型中敏感性和刻板印象共存的现象。

Conclusion: 研究通过幽默分析揭示了语言模型中的反事实不公平性，表明模型对特权身份说话者的反应存在系统性偏见，这使公平性和文化对齐的努力复杂化，需要更细致的方法来处理模型中的社会偏见。
...
TL;DR: 本文提出Token-to-Mask（T2M）重掩码方法，替代传统Token-to-Token（T2T）编辑规则，通过将可疑token重置为掩码状态而非直接替换，提高语言模型在需要精确token级输出的任务上的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有掩码扩散语言模型（如LLaDA2.1）使用Token-to-Token（T2T）编辑来纠正自身生成错误，但这种方法存在三个结构性缺陷：1）当没有单个替代token足够自信时无法触发；2）替换计算基于可能包含错误的上下文；3）训练时使用的均匀扰动与推理时模型实际产生的连贯、语义合理的错误不匹配。

Method: 提出Token-to-Mask（T2M）重掩码方法：不直接覆盖可疑token，而是将其重置为掩码状态，让下一个去噪步骤从符合分布特征的上下文中重新预测该位置。该方法无需训练，仅修改编辑规则，不引入新参数，并配合三种检测启发式方法。

Result: 在8个基准测试中，T2M在需要精确token级输出的任务上提高了准确性。最大提升为CMATH数据集上的+5.92分，其中79.9%的基线错误归因于"最后一英里"损坏（正确推理后给出混乱的最终答案），T2M修复了其中41.3%的案例。

Conclusion: Token-to-Mask（T2M）重掩码方法通过将可疑token重置为掩码状态而非直接替换，有效解决了T2T编辑的结构性缺陷，显著提高了语言模型在精确输出任务上的性能，且无需额外训练或参数。
...
TL;DR: 本文提出了一种结合双语词典和句法增强的上下文学习方法，用于低资源科普特语到英语的机器翻译，取得了新的最优结果。


<details>
  <summary>Details</summary>
Motivation: 低资源机器翻译需要与高资源语言不同的方法。科普特语作为低资源语言，现有方法效果有限，需要结合句法信息来提升翻译质量。

Method: 提出基于上下文学习的方法，结合双语词典和多种句法表示：原始解析器输出、解析结果的英语描述、针对困难结构的特定翻译指导。通过Universal Dependencies解析输入句子获取句法信息。

Result: 句法信息单独使用不如基于词典的注释有效，但将检索到的词典条目与句法信息结合后，在不同模型规模上都取得了显著提升，实现了科普特语翻译的最新最优结果。
...
TL;DR: HAMR是一个统一框架，通过双层优化和邻域感知重采样，自适应地解决类别不平衡和数据难度问题，在多个领域的不平衡数据集上显著提升少数类性能。


<details>
  <summary>Details</summary>
Motivation: 类别不平衡是NLP任务中普遍存在的挑战，严重阻碍了在不同领域和应用中的鲁棒性能。需要一种统一框架来同时解决类别不平衡和数据难度问题。

Method: HAMR采用双层优化动态估计实例级权重，优先考虑真正具有挑战性的样本和少数类；同时使用邻域感知重采样机制，增强对困难样本及其语义相似邻居的训练关注。

Result: 在涵盖生物医学、灾害响应和情感领域的六个不平衡数据集上进行验证，HAMR在少数类上取得显著改进，并持续优于强基线方法。消融研究表明各模块协同贡献性能提升。

Conclusion: HAMR是一个灵活且可泛化的类别不平衡适应方法，通过同时解决类别不平衡和数据难度问题，有效提升了NLP任务在不平衡数据上的性能。
...
TL;DR: 该研究通过实证分析发现，基于单次输出的越狱检测会系统性低估LLM的脆弱性，适度多样本审计能更可靠地评估模型漏洞。


<details>
  <summary>Details</summary>
Motivation: 检测大型语言模型的越狱行为具有挑战性，特别是当强对齐模型很少产生有害输出时。需要在实际条件下评估基于输出的越狱检测方法。

Method: 使用JailbreakBench Behaviors数据集和不同对齐强度的生成模型，评估基于词频-逆文档频率的词汇检测器和基于生成不一致性的检测器，在不同采样预算下进行实验。

Result: 单次输出评估会系统性低估越狱脆弱性，增加采样次数能揭示更多有害行为；从单次生成到适度采样的改进最显著；检测信号在不同模型间部分泛化，相关模型家族内转移更强；词汇检测器捕获的是行为信号和主题特定线索的混合。
...
TL;DR: Mango提出了一种多智能体网页导航方法，利用网站结构动态确定最优起始点，通过多臂老虎机问题建模URL选择，使用Thompson采样分配导航预算，并引入情景记忆组件存储导航历史，显著提升了复杂网站的导航成功率。
...
Method: 1. 将URL选择建模为多臂老虎机问题，使用Thompson采样自适应分配导航预算；2. 引入情景记忆组件存储导航历史，使智能体能从先前尝试中学习；3. 采用多智能体架构，利用网站结构动态确定最优起始点。

Result: 在WebVoyager数据集上，使用GPT-5-mini时Mango达到63.6%的成功率，比最佳基线高出7.3%；在WebWalkerQA上达到52.5%的成功率，比最佳基线高出26.8%。方法在不同开源和闭源模型作为骨干时都表现出良好的泛化性。

Conclusion: Mango通过利用网站结构、自适应预算分配和情景记忆学习，显著提升了复杂网站的导航效率，为网页智能体导航提供了有效的解决方案，代码和数据已开源。
...
TL;DR: LLM在科学可行性评估中的表现：提供实验结果证据比实验描述更可靠，实验结果能提升准确性，而实验文本在不完整时可能降低性能


<details>
  <summary>Details</summary>
Motivation: 研究如何评估科学主张的可行性，将其构建为诊断推理任务，探索在何种知识条件下（仅假设、带实验描述、带结果、或两者都有）LLM能最可靠地预测可行性

Method: 将可行性评估构建为诊断推理任务，在受控知识条件下评估LLMs（假设单独、带实验描述、带结果、或两者都有），通过逐步移除实验和/或结果上下文来探测模型鲁棒性

Result: 在多个LLM和两个数据集上，提供结果证据通常比提供实验描述更可靠；结果证据能超越内部知识单独提供的准确性，而实验文本可能脆弱，当上下文不完整时可能降低性能

Conclusion: 实验结果阐明了实验证据何时有益于基于LLM的可行性评估，以及何时会引入脆弱性，为科学可行性评估中如何有效使用LLM提供了指导
...
TL;DR: 本文提出一个可扩展的多因素实验框架，系统性地探究LLM对文档对中细微语义变化的敏感性。研究发现LLM存在位置偏差、上下文类型影响和模型特异性指纹等关键现象。


<details>
  <summary>Details</summary>
Motivation: 现有研究对LLM在文档相似性评估中如何响应细微语义变化的系统性理解不足。作者希望探究LLM是否像"大海捞针"一样能够检测文档中的细微语义变化，以及这些变化如何受文档结构、上下文和模型特性的影响。

Method: 采用多因素实验设计：在文档中嵌入单个语义改变的句子（针），改变扰动类型（否定、连接词交换、命名实体替换）、上下文类型（原始vs.主题无关）、针的位置和文档长度。测试五个LLM在数万个文档对上的表现，系统分析模型敏感性。

Result: 1. LLM存在文档内位置偏差：语义差异出现在文档前部时惩罚更重；2. 主题无关上下文会降低相似性分数并导致双极化评分；3. 每个LLM产生独特的评分分布"指纹"，但对不同扰动类型的宽容度存在普遍层级关系。

Conclusion: LLM语义相似性评分对文档结构、上下文连贯性和模型身份敏感，这些因素超出了语义变化本身的影响。提出的框架为审计和比较当前及未来模型的评分行为提供了实用的、模型无关的工具包。
...
TL;DR: 论文提出Groupwise Ranking Reward方法，通过分组排名奖励来改善多模态推理中的推理-答案不一致问题，相比传统RLVR和轨迹监督方法有更好表现。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习可验证奖励（RLVR）方法虽然奖励可验证的最终答案，但正确答案的推导轨迹可能依赖不完整的推导、薄弱证据或与结论矛盾的陈述，存在推理-答案不一致的问题，这促使研究者探索轨迹监督方法。

Method: 比较了两种主要方法：奖励模型（RMs）和生成奖励（GRs），并提出Groupwise Ranking Reward方法。该方法对同一提示下通过验证的轨迹进行一次性分组排名，并相应重新分配奖励，以更高效地区分强弱正确的轨迹。

Result: 实验显示RLVR加剧了推理-答案不一致问题，而轨迹监督能缓解该问题。Groupwise Ranking Reward表现最佳，将可靠性条件准确率从RLVR的47.4%提升到54.7%。

Conclusion: Groupwise Ranking Reward通过分组比较有效解决了多模态强化学习中推理-答案不一致的问题，在计算效率和性能提升方面优于现有方法，显著提高了推理的可靠性。
...
TL;DR: 该研究系统评估了形式数学推理中的提示工程，发现在SAIR等式理论竞赛中，单一提示存在性能上限（约60-79%），主要受数学不可判定性、复杂规则系统和提示顺序效应限制。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索在形式数学推理任务中，通过提示工程能否显著提升大型语言模型的性能，特别是在SAIR等式理论竞赛这样的复杂数学问题上。

Method: 方法包括：1）在5周内设计、测试和分析40多个提示变体（0-4,878字节）；2）在四个评估分割和三个语言模型（gpt-oss-120b, Llama 3.3 70B, Gemma 4 31B）上进行实验；3）分析提示工程对模型性能的影响机制。

Result: 主要结果：1）发现单一提示存在性能上限（gpt-oss-120b约60-79%）；2）最佳提交（AN45c, 2,252字节）在hard3分割上达到79.25%准确率，比无提示基线提升19.5个百分点；3）识别出三个限制机制：数学不可判定性、复杂规则系统对弱模型的影响、提示顺序效应。

Conclusion: 结论是：尽管投入大量工程努力，单一提示在形式数学推理任务中存在固有性能上限，主要受数学问题本质和模型架构限制。这为未来提示工程研究提供了重要参考。
...
TL;DR: LogosKG是一个新颖的硬件对齐框架，用于在大规模知识图谱上实现可扩展且可解释的多跳检索，通过符号化表示和硬件高效操作提升效率，并支持KG-LLM集成分析。


<details>
  <summary>Details</summary>
Motivation: 当前知识图谱与大语言模型集成中，多跳检索系统在效率、可扩展性和可解释性之间存在平衡困难，需要一种能够处理大规模图谱同时保持高效和可解释的解决方案。

Method: 基于符号化知识图谱表示，将遍历操作分解为主题、对象和关系的硬件高效操作；采用度感知分区、跨图路由和按需缓存技术扩展到数十亿边规模的图谱。

Result: 相比CPU和GPU基线方法，LogosKG在保持检索准确性的同时实现了显著的效率提升；下游KG-LLM交互展示了如何分析知识图谱拓扑结构对结构化知识与LLM诊断推理对齐的影响。

Conclusion: LogosKG为大规模知识图谱检索提供了高效可扩展的解决方案，为下一代KG-LLM集成打开了大门，支持基于证据的大规模分析。
...
Method: 研究者引入了MORPHOGEN基准数据集，核心任务GENFORM要求模型将第一人称句子改写为相反性别，同时保持其意义和结构。构建了涵盖法语、阿拉伯语和印地语的高质量合成数据集，并对15个流行的多语言大语言模型（2B-70B）进行了基准测试。

Result: 结果显示当前模型在处理形态性别方面存在显著差距，并提供了关于模型如何处理形态性别的有趣见解。不同模型在不同语言上的表现存在差异。

Conclusion: MORPHOGEN为性别感知语言建模提供了聚焦的诊断视角，为未来关于包容性和形态敏感的NLP研究奠定了基础。该基准有助于识别和改进模型在语法性别处理方面的能力。
...
TL;DR: 本文提出了一种基于大语言模型的主题生成方法，专门针对企业评论数据进行领导力分析，解决了现有主题模型在可解释性、主题特异性（与具体行动或特征的对齐）和极性立场一致性（主题内不混合正面和负面评价）方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有主题建模方法在同时实现可解释性、主题特异性和极性立场一致性方面存在困难，这在计算社会科学和组织研究等领域分析文本数据与外部结果的关系时尤为重要。特别是在使用企业评论数据进行领导力分析时，这些局限性更加明显。

Method: 提出了一种利用大语言模型生成满足这些属性的主题的方法，并开发了一个专门针对外部结果分析的评估框架。该框架明确将主题特异性和极性立场一致性作为评估标准，并基于现有指标检验自动化评估方法。使用日本主要企业评论平台OpenWork的员工评论数据进行实证研究。

Result: 与现有方法相比，所提出的方法在可解释性、特异性和极性一致性方面都有所改进。在分析员工士气等外部结果时，该方法生成的主题具有更高的解释力。这些结果表明该方法能够有效支持涉及外部结果的主题分析应用。

Conclusion: 所提出的方法和评估框架为涉及外部结果的主题分析提供了一个通用化方法，特别是在领导力分析等应用场景中，能够更好地满足实际分析需求。
...
TL;DR: 多语言视觉语言模型存在肯定性偏差，倾向于选择肯定描述而非否定描述。研究创建了首个多语言否定基准测试，涵盖7种语言，发现不同模型在不同语言上的表现差异显著，语言特性影响模型改进效果。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型存在肯定性偏差，倾向于选择肯定描述而非否定描述。先前研究主要关注英语，但否定在不同语言中表现形式不同（形态、词序、附着语素等），需要评估现有解决方案是否对所有语言社区公平。

Method: 创建首个人类验证的多语言否定基准测试，涵盖7种类型多样的语言：英语、中文、阿拉伯语、希腊语、俄语、他加禄语和西班牙语。评估了三种VLM模型：CLIP、SigLIP和MultiCLIP，以及一个否定修正方法SpaceVLM。

Result: 标准CLIP在非拉丁文字语言上表现接近或低于随机水平，MultiCLIP在所有语言中准确率最高且最均匀。SpaceVLM对英语、希腊语、西班牙语和他加禄语有显著改进，但在不同类型语言上效果不一，表明语言特性（形态、文字、否定结构）与模型改进效果存在交互作用。

Conclusion: 随着VLM在全球部署，多语言基准测试对于理解解决方案是否有效以及为谁有效至关重要。语言特性以与公平相关的方式影响模型改进效果，需要针对不同语言特性开发更公平的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在干净、高资源语料上训练的NER模型在部署到噪声大、稀疏的用户生成内容时会出现灾难性性能崩溃。先前研究主要关注表面症状的修复，但忽略了UGC中固有的结构稀疏性问题。

Method: 1. 通过分层混淆控制重采样实验识别信息密度作为关键因素；2. 提出注意力谱分析量化信息密度降低如何导致"注意力钝化"；3. 设计窗口感知优化模块，利用LLM识别信息稀疏区域并通过选择性回译增强语义密度。

Result: 在标准UGC数据集上，WOM框架在主流架构上实现了高达4.5%的绝对F1提升，在WNUT2017数据集上达到了新的最先进性能。

Conclusion: 信息密度是UGC中NER性能下降的统一根本原因，WOM框架通过增强语义密度而不改变模型架构，有效提升了NER在噪声稀疏文本上的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 需要系统地评估现代大语言模型在社交媒体分析任务上的表现，建立可复现的基准，并解决现有研究中可能存在的"已见数据"偏差问题。

Method: 1. 作者身份验证：引入系统抽样框架，采用多样化的用户和帖子选择策略，并在2024年1月后新收集的推文上进行泛化评估；2. 帖子生成：使用综合评估指标评估LLMs生成真实用户内容的能力；3. 用户研究：连接任务I和II，测量真实用户对基于自己写作风格生成的LLM帖子的感知；4. 属性推断：使用两种标准化分类法（IAB Tech Lab 2023和2018 U.S. SOC）标注职业和兴趣，并将LLMs与现有基线进行比较。

Result: 研究提供了关于LLM驱动社交媒体分析的新见解，并建立了可复现的基准。代码和数据已在补充材料中提供，并将在发表后公开。

Conclusion: 该统一评估为LLM驱动的社交媒体分析提供了系统性的评估框架和基准，有助于推动该领域的研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型虽然支持并行token预测，但实际解码过程仍存在高推理延迟。研究发现这种低效主要源于解码过程中的重复冗余：空间冗余（置信度聚类和位置模糊）和时间冗余（重复重掩码已稳定的预测）。

Method: 提出R²-dLLM统一框架，从推理和训练两个角度减少解码冗余。推理时引入无训练解码规则：聚合局部置信度和token预测，并最终化时间稳定的token以避免冗余解码步骤。同时提出冗余感知的监督微调流程，使模型与高效解码轨迹对齐，减少对人工调整阈值的依赖。

Result: 实验表明，R²-dLLM相比现有解码策略，将解码步骤减少高达75%，同时在不同模型和任务上保持竞争力的生成质量。

Conclusion: 解码冗余是扩散大语言模型的核心瓶颈，明确减少冗余可以带来实质性的实际效率提升。R²-dLLM通过统一的冗余减少框架，显著提高了扩散大语言模型的解码效率。
...
TL;DR: HarmThoughts是一个用于评估推理轨迹中逐步安全性的基准，包含56,931个句子和16种有害推理行为分类，揭示了现有检测器在过程级安全监控方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有安全评估主要关注最终输出，忽视了推理过程中危害如何逐步产生。当模型被越狱时，危害并非瞬间出现，而是通过抑制拒绝、合理化服从、分解有害任务和隐藏风险等行为步骤逐步展开。目前缺乏在句子级别粒度上捕捉推理轨迹中这一过程的基准。

Method: 提出了HarmThoughts基准，基于16种有害推理行为的分类法构建，这些行为分为四个功能组，描述危害如何传播而非产生什么危害。数据集包含来自4个模型家族的1,018个推理轨迹中的56,931个句子，每个句子都标注了细粒度的行为标签。

Result: 分析了推理轨迹中的危害传播模式，识别了常见的行为轨迹和从安全转向不安全的漂移点。系统比较了白盒和黑盒检测器在识别有害推理行为方面的表现，发现现有检测器在推理轨迹中的细粒度行为检测方面表现不佳，特别是在危害出现和执行阶段的微妙类别上。

Conclusion: HarmThoughts填补了过程级安全监控的关键空白，揭示了现有安全检测器在监控推理过程中逐步产生危害方面的不足，为可靠的安全监控、干预和系统性故障诊断提供了重要工具。
...
TL;DR: RADAR是一个基于角色锚定的多智能体辩论框架，用于检测事实核查中的半真陈述（事实正确但上下文缺失的误导性主张），通过政治家、科学家和法官的角色分工，在噪声检索环境下进行对抗性推理。


<details>
  <summary>Details</summary>
Motivation: 当前事实核查系统主要关注显性虚假陈述，但对半真陈述（事实正确但因省略上下文而具有误导性）存在盲点。需要一种能够推理"未言明内容"的系统来应对基于省略的操纵。

Method: 提出RADAR框架：1) 分配互补角色：政治家（主张观点）和科学家（质疑观点）基于共享检索证据进行对抗性推理；2) 中立法官进行裁决；3) 双阈值早期终止控制器自适应决定何时达到足够推理以做出判决。

Result: 实验表明，RADAR在不同数据集和骨干模型上始终优于强大的单智能体和多智能体基线，提高了省略检测准确性，同时降低了推理成本。

Conclusion: 基于角色锚定、检索基础辩论和自适应控制的框架是检测事实核查中缺失上下文的有效且可扩展的方法。
...
TL;DR: 本文提出了Align-Cultura，一个两阶段的文化对齐框架，用于评估大型语言模型在联合国教科文组织文化多样性原则下的文化对齐能力，解决了现有基准在HHH范式下的不足。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型缺乏文化对齐可能导致刻板、不敏感或误导性输出，现有基准无法系统评估模型在联合国教科文组织文化多样性原则下的文化对齐表现，特别是在HHH（有益、无害、诚实）范式方面存在空白。

Method: 采用两阶段流程：第一阶段构建CULTURAX数据集，基于联合国教科文组织文化分类法，通过查询重构、扩展代表性不足领域、使用SimHash防止数据泄漏，并通过两阶段拒绝采样生成文化基础响应；第二阶段在通用模型、文化微调模型和开源权重LLM上进行基准测试。

Result: 构建了包含1,500个样本、涵盖30个有形和无形文化子领域的CULTURAX数据集。文化微调模型将联合HHH指标提高了4%-6%，文化失败率降低18%，效率提升10%-12%，数据泄漏限制在0.3%。

Conclusion: Align-Cultura框架有效解决了文化对齐评估的空白，文化微调显著提升了模型在文化多样性方面的表现，为开发更具文化意识和包容性的大型语言模型提供了系统评估工具。
...
TL;DR: RARE框架通过分解文档为原子事实和CRRF增强的LLM数据生成，构建能反映真实冗余文档环境的RAG评估基准，揭示现有基准无法捕捉的检索器鲁棒性差距。


<details>
  <summary>Details</summary>
Motivation: 现有QA基准假设文档间重叠最小，但真实RAG系统处理金融报告、法律代码、专利等高度冗余且相似性强的文档，这种不匹配导致评估失效：检索器即使检索到足够证据的文档也会被低估，而标准基准表现好的检索器在真实冗余文档上泛化能力差。

Method: 提出RARE框架：1) 将文档分解为原子事实以实现精确冗余跟踪；2) 使用CRRF（分别评分标准并按排名融合决策）增强LLM数据生成，提高生成数据的可靠性。应用于金融、法律和专利语料库创建RedQA基准。

Result: 在RedQA基准上，强检索器基线在4跳General-Wiki上的PerfRecall@10为66.4%，但在4跳深度的金融、法律、专利语料库上降至5.0-27.9%，揭示了当前基准无法捕捉的鲁棒性差距。

Conclusion: RARE使从业者能够构建忠实反映真实部署条件的领域特定RAG评估，解决了现有基准与真实冗余文档环境不匹配的问题，为更准确的检索器评估提供了框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有MoE-LoRA方法存在两个主要挑战：1）当前MoE-LoRA方法中的不精确路由无法明确匹配输入语义与专家能力，导致专家专业化程度弱；2）统一的权重融合策略难以提供自适应更新强度，忽视了不同任务的复杂度差异。
...
Conclusion: SAMoRA通过语义感知路由和任务自适应缩放，有效解决了现有MoE-LoRA方法的局限性，为参数高效微调框架提供了新的解决方案，在多任务学习中表现出优越性能。
...
TL;DR: LLMs使用基于单元的绑定表示（CBR）进行话语级关系绑定，这是一种低维线性子空间，每个"单元"对应实体-关系索引对，在推理过程中从相应单元检索绑定属性。
...
Method: 使用带有实体和关系索引注释的受控多句子数据，通过偏最小二乘回归从属性标记激活中解码这些索引来识别CBR子空间。通过激活修补技术操纵该子空间来验证因果关系。

Result: 发现索引是线性可解码的，在投影空间中形成网格状几何结构。上下文特定的CBR表示通过激活空间中的平移向量相关联，实现跨上下文迁移。激活修补实验表明操纵该子空间会系统性地改变关系预测。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现神经NLI模型过度拟合数据集中的伪相关（数据集伪影）而非进行真正的推理。仅使用假设的模型在SNLI上达到57.7%准确率，表明存在强烈的伪相关，且38.6%的基线错误源于这些伪相关。需要开发能够减少偏见依赖的方法。

Method: 提出Product-of-Experts (PoE)训练方法，通过降低偏见模型过度自信的样本权重来减少偏见依赖。使用lambda=1.5的超参数平衡去偏见和准确率。

Result: PoE方法在几乎保持准确率的同时（89.10% vs 89.30%）将偏见依赖降低了4.71%（偏见一致性从49.85%降至45%）。行为测试仍发现模型在否定和数值推理方面存在问题。

Conclusion: PoE训练能有效减少NLI模型对数据集伪相关的依赖，但模型在否定和数值推理等复杂推理任务上仍有局限性，需要进一步改进。
...
TL;DR: 提出Tree-of-Writing评估框架解决LLM写作评估中的隐式不一致问题，并构建大规模中文写作基准HowToBench，验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法难以全面评估大语言模型在千字级别和开放式写作中的表现，传统基于参考的指标和现代LLM作为评判者的方法都存在局限性。

Method: 提出Tree-of-Writing框架，采用树结构工作流显式建模子特征聚合权重；构建HowToBench大规模中文写作基准，涵盖12种文体和1302条指令，包含三种任务类型。

Result: ToW成功缓解评估偏差，与人类判断的皮尔逊相关性达到0.93；相比传统指标和LLM-as-a-judge方法，ToW对文本扰动具有鲁棒性；发现输入长度与内容相关分数呈负相关。

Conclusion: Tree-of-Writing框架有效解决了LLM写作评估中的隐式不一致问题，为大规模语言模型的写作能力评估提供了更可靠的方法。
...
TL;DR: SAHM是首个面向阿拉伯语金融NLP和伊斯兰教法合规推理的文档基准测试和指令调优数据集，包含14,380个专家验证实例，涵盖7个任务，评估了19个LLM模型


<details>
  <summary>Details</summary>
Motivation: 英语金融NLP通过情感分析、文档理解和问答等基准测试发展迅速，而阿拉伯语金融NLP相对未被充分探索，尽管对可信金融和伊斯兰金融助手有强烈实际需求

Method: 创建SAHM基准测试和指令调优数据集，包含14,380个专家验证实例，涵盖7个任务：AAOIFI标准问答、教令问答/多选题、会计和商业考试、金融情感分析、提取式摘要和事件原因推理，数据来自真实的监管、法学和公司来源

Result: 评估了19个开源和专有LLM模型，发现阿拉伯语流利度并不能可靠地转化为基于证据的金融推理：模型在识别式任务上明显强于生成和因果推理任务，最大差距出现在事件原因推理任务上
...
TL;DR: HSPD是一种通过软对比解码在原始语料层面进行去毒的方法，在保持语义的同时定位和重写有毒内容，从根本上降低大语言模型的毒性。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型去毒方法主要关注训练后阶段或推理时间，很少处理毒性的源头——数据集本身。训练基或可控解码方法无法完全抑制模型固有的毒性，而对预训练数据集进行去毒可以从根本上减少模型在训练期间学习的毒性。

Method: 提出HSPD（分层语义保持去毒）流程，使用SoCD（软对比解码）指导大语言模型在原始语料中定位和重写有毒片段，同时保持语义，生成可以直接替换原始语料进行微调或其他训练的去毒语料。

Result: 在GPT2-XL上，HSPD实现了最先进的去毒效果，将毒性概率从0.42降至0.18，预期最大毒性从0.43降至0.20。在LLaMA2-7B、OPT-6.7B和Falcon-7B上也验证了一致的最佳结果。

Conclusion: HSPD的语义保持、语料级重写方法能有效抑制下游毒性，同时保留数据效用并允许无缝的源头级缓解，从而降低后期模型行为调整的成本。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在情感识别和道德推理方面已有广泛研究，但情感如何影响道德判断这一关键问题尚未得到充分探索

Method: 开发情感诱导管道，将情感注入道德情境中，评估多个数据集和LLM中道德可接受性的变化，并进行人类标注研究对比

Result: 发现方向性模式：积极情感增加道德可接受性，消极情感降低可接受性，影响强度足以在20%案例中逆转二元道德判断；模型能力越强，受影响程度越低；某些情感表现与效价预测相反（如悔恨增加可接受性）；人类标注未显示这些系统性变化

Conclusion: 当前LLM在情感对道德判断的影响方面与人类存在对齐差距，情感会系统性改变模型的道德评估，而人类则不会表现出这种模式
...
TL;DR: 该论文综述了基于预训练语言模型的知识图谱构建研究进展，并提出了一种基于轻量级大语言模型的超关系知识图谱构建框架LLHKG，其性能可与GPT3.5相媲美。


<details>
  <summary>Details</summary>
Motivation: 传统知识图谱构建方法依赖人工标注耗时耗力，而基于深度学习的方法泛化能力弱。随着预训练语言模型的快速发展，其在知识图谱构建领域展现出巨大潜力，需要系统梳理相关研究进展并探索更高效的构建方法。

Method: 论文首先全面综述了利用PLM进行知识图谱构建的研究进展，重点分析PLM如何利用其语言理解和生成能力从文本数据中自动提取实体和关系等关键信息。此外，提出了基于轻量级大语言模型的超关系知识图谱构建框架LLHKG，并与先前方法进行比较。

Result: 在LLHKG框架下，轻量级大语言模型的知识图谱构建能力可与GPT3.5相媲美，表明轻量级模型也能实现高质量的知识图谱构建。

Conclusion: 预训练语言模型在知识图谱构建领域具有重要应用价值，提出的LLHKG框架为高效构建知识图谱提供了新思路，轻量级模型也能达到与大模型相当的性能，有助于降低计算成本和部署难度。
...
TL;DR: 该论文系统分析了大型语言模型中普遍存在的"语言习惯语"现象，通过评估8个主流LLM模型，发现这些模型普遍存在重复、公式化的语言模式，并提出了量化指标VTI来衡量这一现象。


<details>
  <summary>Details</summary>
Motivation: 随着LLM通过RLHF和Constitutional AI等对齐技术不断发展，出现了一个日益明显的现象：语言习惯语的泛滥。这些重复、公式化的语言模式（如"这是个好问题！"、"我完全理解你的担忧"等）影响了模型输出的自然性和真实性，需要系统研究这一现象。

Method: 研究开发了自定义评估框架，对8个最先进的LLM模型（GPT-5.4、Claude Opus 4.7等）进行标准化API评估。评估了10,000个提示，涵盖10个任务类别，使用英语和中文两种语言，共产生160,000个模型响应。引入了语言习惯语指数（VTI）作为量化指标，并分析了其与奉承性、词汇多样性和人类感知自然度的相关性。

Result: 研究结果显示显著的模型间差异：Gemini 3.1 Pro的VTI最高（0.590），而DeepSeek V3.2最低（0.295）。语言习惯语在多轮对话中会累积，在主观任务中被放大，并表现出不同的跨语言模式。人类评估（N=120）证实了奉承性与感知自然度之间的强负相关（r=-0.87，p<0.001）。

Conclusion: 当前训练范式存在"对齐税"问题，语言习惯语的普遍存在凸显了开发更真实人机交互框架的紧迫性。研究结果为改进LLM对齐技术提供了重要见解。
...
Method: 提出ReflectMT两阶段反射内化算法：第一阶段通过强化学习培养模型的"翻译-反思-精炼"能力，提升语义理解和任务知识；第二阶段训练模型将反思阶段获得的知识内化，使推理时无需显式推理步骤。

Result: 在WMT24等数据集上的实验表明，ReflectMT在推理时的首轮翻译质量超过了DeepSeek-R1等多步推理模型，在GPT翻译质量评估中提升2.16分，同时减少94.33%的token消耗。

Conclusion: ReflectMT通过"先翻译后思考"范式和知识内化机制，实现了高质量翻译与低计算成本的平衡，为机器翻译领域提供了一种高效实用的解决方案。
...
TL;DR: 该论文研究LLM在回答问题时如何读取和整合推理过程，发现正确的答案具有良性的自我读取模式，并提出基于自我读取质量的训练无关引导方法


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注如何塑造LLM的推理轨迹，但对于答案token如何实际读取和整合推理过程以产生可靠结果的理解仍然不足。本文旨在深入分析答案与推理之间的注意力机制，探究模型内部如何通过自我读取来确保答案的可靠性。

Method: 聚焦于定量推理任务，分析答案到推理的注意力模式。观察到正确的解决方案展现出良性的自我读取模式：注意力焦点沿推理轨迹向前漂移，并持续集中在关键语义锚点上。基于此提出基于自我读取质量（SRQ）的训练无关引导方法，结合几何度量和语义度量，构建引导向量使推理朝向良性自我读取模式。

Result: 实验表明，正确的答案表现出对齐于正确性的良性自我读取模式，而错误的解决方案则呈现分散和不规则的注意力模式。提出的SRQ引导方法能够带来一致的准确性提升。

Conclusion: LLM在答案解码过程中存在内部确定性机制，模型会承诺于可行的解决方案分支并整合关键证据。通过引导模型朝向良性的自我读取模式，可以有效提升推理的可靠性，这为理解LLM内部工作机制和提升其性能提供了新视角。
...
TL;DR: Voice of India是一个新的印度语言ASR基准测试，基于非脚本电话对话构建，涵盖15种主要语言和139个区域集群，包含30万+话语和536小时语音，解决了现有基准在脚本化语音、过度拟合和拼写变化惩罚方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有印度语言ASR基准存在三个主要问题：1）使用脚本化、干净的语音数据；2）排行榜驱动评估导致数据集特定过度拟合；3）严格的单参考WER惩罚印度语言中自然的拼写变化，包括代码混合英语词汇的非标准化拼写。

Method: 构建了Voice of India闭源基准，基于非脚本电话对话，涵盖15种主要印度语言和139个区域集群。数据集包含306,230个话语，总计536小时语音，来自36,691名说话者，转录文本考虑了拼写变化。还进行了地理区域（地区级别）的性能分析。

Result: 数据集包含306,230个话语和536小时语音，来自36,691名说话者。通过地理分析揭示了性能差异，并提供了音频质量、语速、性别和设备类型等详细分析，指出了当前ASR系统的薄弱环节。

Conclusion: Voice of India基准解决了现有印度语言ASR评估的局限性，提供了更真实的评估场景。详细分析揭示了当前系统的不足，为改进现实世界印度语言ASR系统提供了重要见解。
...
TL;DR: SCURank是一个基于摘要内容单元(SCUs)的框架，用于改进多LLM蒸馏中的摘要排名，相比传统指标和LLM排名方法表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有LLM摘要排名策略存在不稳定性，而传统指标(如ROUGE)不足以对高质量摘要进行有效排名，需要更可靠的信息内容评估方法。

Method: 提出SCURank框架，利用摘要内容单元(SCUs)评估摘要的信息丰富度和语义重要性，而非依赖不稳定的比较或表面重叠。

Result: SCURank在多个评估指标和数据集上优于传统指标和LLM排名方法，且结合多样化LLM摘要能提升模型的抽象性和蒸馏性能。

Conclusion: SCURank验证了信息中心排名在多LLM蒸馏中的优势，为小语言模型通过蒸馏实现高质量摘要提供了更可靠的排名框架。
...
TL;DR: 研究探讨了新闻标题中第一、二人称代词的使用对记忆性的影响，以及使用大语言模型自动插入这些代词的效果。


<details>
  <summary>Details</summary>
Motivation: 新闻标题需要被记住才能影响信念和驱动行动，研究特定语言特征（如第一、二人称代词）如何影响记忆性，并探索使用大语言模型自动插入这些特征的可能性。

Method: 采用认知心理学实验设计，进行三项受控记忆实验（共240名参与者，7680个记忆判断），同时使用大语言模型自动插入代词，并通过众包评估模型修改效果。

Result: 代词插入对记忆性有混合效果，效果因标题主题、插入方式和上下文而异；大语言模型的自动修改在内容准确性、情感保留和写作风格方面存在问题。

Conclusion: 需要更多数据和精细分析来确定影响记忆性的中介因素；大语言模型自动修改文本的适用性有限；研究数据已公开供未来研究使用。
...
TL;DR: 提出基于33个定量语言学特征（词汇、句法、语义）的叙事质量自动评估方法，能有效区分专业编辑与自出版文本


<details>
  <summary>Details</summary>
Motivation: 叙事质量评估面临主观性挑战（如情节、人物发展、情感影响），需要更客观的量化方法

Method: 提取33个定量语言学特征（分为词汇、句法、语义三类），通过相似度矩阵对23本书（经典名著和自出版作品）进行聚类分析

Result: 系统能近乎完美地区分专业编辑和自出版文本，在人工标注数据集上显著优于传统故事级评估指标

Conclusion: 定量语言学特征在评估叙事质量方面具有有效性，为叙事评估提供了客观的量化方法
...
TL;DR: CulturALL是一个评估大语言模型多语言和多文化能力的基准测试，专注于现实世界场景中的接地任务，包含14种语言的2610个样本，最佳模型准确率仅44.48%


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注通用语言理解或表面文化知识，缺乏对现实世界场景中接地任务的评估，需要填补这一空白

Method: 采用人机协作框架构建：专家标注确保难度和准确性，LLM减轻人工工作量；包含14种语言、51个地区、16个主题的2610个样本

Result: 最佳LLM在CulturALL上的准确率仅为44.48%，表明现有模型在多语言多文化接地任务上仍有很大改进空间

Conclusion: CulturALL是一个全面且具有挑战性的基准测试，揭示了LLM在多语言多文化接地任务上的局限性，为未来研究提供了重要评估工具
...
TL;DR: LocQA测试集用于评估多语言大语言模型在回答地区模糊问题时的跨语言和语言内偏见，发现模型存在向美国地区倾斜的全球偏见，且指令微调会加剧这种偏见。


<details>
  <summary>Details</summary>
Motivation: 随着多语言大语言模型缩小了语言间的流畅度差距，模型面临偏见行为的风险增加，因为知识和规范可能跨语言传播。本研究旨在量化模型在回答地区模糊问题时的跨语言和语言内偏见。

Method: 提出了LocQA测试集，包含12种语言的2,156个问题，涉及法律、日期、度量等地区依赖的事实。这些问题不包含明确的地区指示，仅通过查询语言本身隐含地区信息。使用LocQA评估了32个模型。

Result: 发现了两种结构性偏见：1) 跨语言层面，模型存在向美国地区倾斜的全球偏见，即使使用非英语语言提问；2) 指令微调模型比基础模型表现出更强的这种偏见；3) 语言内层面，当同一语言涉及多个地区时，模型会优先考虑人口较多的地区。

Conclusion: LocQA的见解有助于塑造大语言模型的期望本地行为，并量化不同训练阶段对各种偏见的影响。该工具为评估和缓解多语言模型中的地区偏见提供了重要方法。
...
TL;DR: 印度金融监管基准测试：首个针对印度金融监管文本的LLM评估基准，包含406个专家标注的问答对，涵盖监管解释、数值推理、矛盾检测和时间推理四类任务。


<details>
  <summary>Details</summary>
Motivation: 现有金融NLP基准测试主要基于西方金融语料（SEC文件、美国财报、英文财经新闻），缺乏对非西方监管框架的覆盖，特别是印度金融监管体系。印度作为重要新兴市场，其金融监管文本具有独特性和复杂性，需要专门的评估工具。

Method: 从印度证券交易委员会（SEBI）和印度储备银行（RBI）收集192份文档，构建包含406个专家标注问答对的数据集，涵盖四大任务类型。通过模型辅助二次验证（kappa=0.918）和60项人工标注一致性评估（kappa=0.611）确保标注质量。在零样本条件下评估12个模型。

Result: 模型准确率在70.4%（Gemma 4 E4B）到89.7%（Gemini 2.5 Flash）之间，均显著优于非专业人类基线（60.0%）。数值推理任务最具区分度，模型间差异达35.9个百分点。通过自助法显著性检验发现三个统计上不同的性能层级。

Conclusion: IndiaFinBench填补了非西方金融监管评估的空白，为LLM在印度金融监管文本上的性能提供了首个公开基准。数据集、评估代码和所有模型输出已开源，支持未来研究和模型改进。
...
TL;DR: 该研究首次对参数少于100亿的小型语言模型在三种范式下进行大规模综合评估：基础模型、配备工具的单智能体系统、以及具备协作能力的多智能体系统，发现单智能体系统在性能与成本间达到最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型能力强大，但其高昂计算成本、延迟和隐私风险限制了实际应用部署。小型语言模型（参数少于100亿）是很有前景的替代方案，但其知识和推理能力有限。现有研究主要关注通过扩展定律或微调策略增强小型模型，而忽视了使用智能体范式（如工具使用和多智能体协作）来系统性地弥补小型模型固有弱点。

Method: 首次对参数少于100亿的开源模型进行大规模综合研究，评估三种范式：1）基础模型；2）配备工具的单智能体系统；3）具备协作能力的多智能体系统。通过系统实验比较不同范式在性能、成本和效率方面的表现。

Result: 研究结果显示，单智能体系统在性能与成本之间达到了最佳平衡，而多智能体设置虽然增加了开销但收益有限。这表明智能体中心设计对于资源受限环境中的高效可信部署具有重要意义。

Conclusion: 该研究强调了智能体中心设计对于在资源受限环境中实现高效可信部署的重要性。单智能体系统通过工具使用有效弥补了小型模型的局限性，为实际应用提供了更优的解决方案。
...
TL;DR: 本文提出了一种基于计算论证的正式框架，用于评估议会辩论摘要是否忠实保留了论证结构，特别是针对政策提案的推理过程。


<details>
  <summary>Details</summary>
Motivation: 议会辩论是民主过程的核心，但公众难以理解其复杂内容。虽然大语言模型能生成摘要，但现有评估指标无法准确衡量摘要与原文在论证一致性方面的忠实度。

Method: 提出一个正式评估框架，将论证结构锚定在辩论的具体政策提案上。采用计算论证方法，专注于评估摘要是否忠实保留了支持或反对政策结果的推理过程。

Result: 通过欧洲议会辩论案例研究，展示了该方法如何评估大语言模型生成的摘要，验证了框架在衡量论证忠实度方面的有效性。

Conclusion: 基于计算论证的正式框架能够有效评估议会辩论摘要的论证忠实度，为民主过程的透明度提供了更好的评估工具。
...
TL;DR: 论文提出了EDGE-EVAL框架，用于评估LLM在工业部署中的经济、能耗和硬件效率指标，发现小参数模型（<2B）在边缘部署中比大模型更具优势。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估主要关注准确性，而工业部署需要满足严格的能耗、延迟和硬件利用率约束，存在部署-评估差距。需要建立包含操作和经济标准的评估框架。

Method: 提出EDGE-EVAL框架，在NVIDIA Tesla T4 GPU上评估LLaMA和Qwen变体，引入五个部署指标：经济盈亏平衡点、智能每瓦特、系统密度、冷启动税和量化保真度。

Result: 小参数模型（<2B）在经济和生态维度上优于大基线模型。LLaMA-3.2-1B（INT4）在14个请求内达到ROI盈亏平衡，能源效率比7B模型高3倍，4位量化下超过6900 tokens/s/GB。发现QLoRA虽减少内存占用，但会增加小模型适应能耗达7倍。

Conclusion: 需要重新思考边缘部署中的模型评估标准，小参数模型在工业应用中具有显著优势，量化感知训练在边缘部署中的假设需要重新审视。
...
Method: 提出DASH-KV加速框架，将注意力重构为通过非对称深度哈希的近似最近邻搜索。设计了非对称编码架构，根据查询和键在精度和重用特性上的差异进行差异化映射。引入动态混合精度机制，自适应地为关键token保留全精度计算。
...
TL;DR: 通过持续预训练和模型融合，将7B参数的小型模型在德语医学任务上的性能提升到接近24B大型通用模型的水平，证明了专业化小型模型在资源受限场景下的可行性。


<details>
  <summary>Details</summary>
Motivation: 缩小小型专业化模型与大型通用模型之间的性能差距，解决德语医学领域数据稀缺问题，为德语医疗环境提供资源高效的专业化解决方案。

Method: 1. 从FineWeb2构建高质量德语医学语料库FineMed-de；2. 对三个知名LLM（7B-24B参数）进行持续预训练；3. 采用模型融合技术创建DeFineMed模型家族；4. 基于Qwen2.5的模型进行领域适应。

Result: 1. 专业化显著提升7B模型在德语医学基准测试中的性能；2. 领域适应使Qwen2.5模型对Mistral-Small-24B-Instruct的胜率提高约3.5倍；3. 模型融合成功恢复指令跟随能力，但也引入语言混合和冗长性等权衡问题。

Conclusion: 专业化7B模型可作为德语医疗环境中复杂指令跟随任务的竞争性、资源高效解决方案。模型融合虽有效但存在权衡，未来需要更有针对性的微调方法。该研究为开发专业化LLM提供了稳健、合规的方法论基础。
...
TL;DR: MM-JudgeBench是首个大规模多语言多模态评估基准，包含25种语言的6万+偏好实例，揭示当前奖励模型在多语言评估中存在显著性能差异和不一致性。


<details>
  <summary>Details</summary>
Motivation: 当前自动评估器（如奖励模型）在大型视觉语言模型对齐和评估中起核心作用，但这些评估器几乎只在英语基准上进行评估，缺乏对其跨语言泛化能力的了解。

Method: 构建MM-JudgeBench基准，包含两个互补子集：扩展VL-RewardBench的通用视觉语言偏好评估子集，以及源自OpenCQA的图表中心视觉文本推理子集。评估22个大型视觉语言模型（15个开源，7个专有），并分析跨语言性能差异。

Result: 发现显著的跨语言性能差异，模型大小和架构对多语言鲁棒性的预测能力较差，即使最先进的LVLM评估器在不同语言间也表现出不一致行为。

Conclusion: 当前奖励建模存在根本性局限，需要多语言多模态基准来开发可靠的自动评估器，凸显了跨语言评估的重要性。
...
TL;DR: 本文提出了一种双轨管道，用于检测文学小说中的圣经典故，并将其应用于科马克·麦卡锡的小说。该方法结合了自下而上的词嵌入分析和自上而下的文体风格分析，通过LLM验证和交叉验证，在麦卡锡作品中发现了349个圣经典故，对已知典故的召回率达到54%。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发一种自动化方法来检测文学作品中的圣经典故，特别是针对大规模文学语料库的互文性研究。传统的手动分析方法难以处理大量文本，需要结合现代自然语言处理技术来提高分析效率和准确性。

Method: 采用双轨管道方法：1）自下而上的嵌入轨道：使用逆文档频率识别与英王钦定本圣经共享的罕见词汇，在局部上下文中嵌入以进行语义消歧，并通过级联LLM审查候选段落对；2）自上而下的文体轨道：让LLM无指导地阅读麦卡锡的散文，与圣经进行对比，捕捉不受词汇稀有度限制的典故。两个轨道都通过长上下文模型进行交叉验证，并将所有发现与已发表的学术研究进行比对。

Result: 在整个语料库中发现了349个圣经典故。在通过人工审查学术文献检索到的115个先前记录的典故中，管道独立恢复了62个（召回率54%）。召回率因连接类型而异：从30%（转换意象）到80%（文体碰撞）。

Conclusion: 该方法展示了LLM作为机械文体分析助手的附加价值，以及它们在促进大规模文学语料库互文性统计研究方面的潜力。研究强调了结合计算方法和人工验证的重要性，为文学分析提供了新的工具和方法。
...
TL;DR: 本文针对法律问题识别任务，构建了马来西亚合同法案例数据集，发现GPT-4o等大语言模型精度不足（仅62%），提出了结合神经生成与结构化统计推理的神经符号框架LePREC，相比先进LLM基线提升30-40%。


<details>
  <summary>Details</summary>
Motivation: 全球超过一半人口因法律资源有限而难以满足民事司法需求。虽然大语言模型展现出强大的推理能力，但在法律问题识别这一基础步骤仍面临重大挑战，现有模型精度不足。

Method: 提出LePREC（法律专业人士启发的推理激发与分类）神经符号框架：1）神经组件利用LLM将法律描述转化为代表不同分析因素的问答对；2）符号组件在这些离散特征上应用稀疏线性模型，学习明确代数权重以识别最具信息量的推理因素。

Result: 基于769个真实马来西亚合同法案例构建的数据集显示，GPT-4o精度仅为62%。LePREC相比GPT-4o和Claude等先进LLM基线提升30-40%，通过基于相关的因子-问题分析为相关性决策提供了更数据高效的解决方案。

Conclusion: LePREC框架通过结合神经生成与结构化统计推理，在保持可解释性和数据效率的同时，显著提升了法律问题识别的性能，为解决法律资源有限问题提供了有效技术方案。
...
<details>
  <summary>Details</summary>
Motivation: 改进传统的作者归属分析方法，通过引入基于概率分布的距离函数来提升准确性和可解释性，同时使每个Delta距离数值都具有可解释性，便于细读和结果验证。

Method: 1. 建立理论框架，对比中心化和非中心化z-scoring的词频向量；2. 将非中心化向量重新表述为概率分布；3. 开发词元级分解方法，使每个Delta距离数值可解释；4. 在英语、德语、法语和俄语四个文学语料库上评估方法效果。
...
Conclusion: 提出的两种新Delta度量方法在作者归属任务中表现优异，特别是Jensen-Shannon Delta超越了经典方法，同时词元级分解提供了更好的可解释性，有助于文学分析和结果验证。
...
TL;DR: 论文提出Beyond Rating框架，从五个维度评估AI审稿质量，强调文本论证比评分更重要，并证明其指标与人类偏好高度相关。


<details>
  <summary>Details</summary>
Motivation: 当前自动同行评审研究过于关注评分预测任务，忽视了评审的真正价值在于文本论证（论点、问题、批评），需要更全面的评估框架。

Method: 提出Beyond Rating评估框架，包含五个维度：内容忠实度、论证一致性、焦点一致性、问题建设性、AI似然性；采用Max-Recall策略处理专家分歧；构建高质量评审数据集。

Result: 实验表明传统n-gram指标无法反映人类偏好，而提出的文本中心指标（特别是弱点论证的召回率）与评分准确性高度相关；AI批评焦点与人类专家对齐是可靠自动评分的前提。

Conclusion: 文本论证质量比评分更重要，Beyond Rating框架为未来研究提供了稳健标准，强调AI审稿需要与人类专家的论证焦点保持一致。
...
TL;DR: 本文介绍了Bangla Key2Text，一个包含260万孟加拉语关键词-文本对的大规模数据集，用于低资源语言的关键词驱动文本生成。


<details>
  <summary>Details</summary>
Motivation: 为低资源语言孟加拉语的关键词驱动文本生成任务创建大规模数据集，填补该领域的研究空白。

Method: 使用基于BERT的关键词提取流水线处理数百万孟加拉语新闻文本，将原始文章转换为结构化关键词-文本对；然后微调mT5和BanglaT5序列到序列模型作为基准。

Result: 任务特定微调显著提升了孟加拉语关键词条件文本生成效果，优于零样本大型语言模型；数据集、训练模型和代码已公开。

Conclusion: Bangla Key2Text数据集为孟加拉语自然语言生成和关键词到文本生成任务提供了有价值的资源，支持未来研究发展。
...
TL;DR: 本文提出SCALE框架，通过语义解耦和全局对齐方法解决对话中情感-原因对提取问题，在多个基准数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将ECPEC视为独立的成对分类任务，忽视了情感扩散和原因解释的不同语义，且无法捕捉全局一致的多对多对话因果关系。

Method: 从语义角度重新审视ECPEC，将情感导向语义和原因导向语义解耦到两个互补的表示空间，然后通过最优传输进行全局对齐，实现多对多且全局一致的情感-原因匹配。

Result: 在多个基准数据集上的广泛实验表明，SCALE框架一致性地实现了最先进的性能。

Conclusion: 通过语义解耦和全局对齐的方法能有效解决对话中情感-原因对提取问题，SCALE框架为此提供了一个统一的解决方案。
...
TL;DR: 论文发现LLM智能体在角色扮演中会出现类似人类的认知偏差（行动者-观察者不对称），提出ReTAS方法通过辩证对齐训练来缓解这种偏差，提高模糊场景下的故障解决率。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体从静态文本生成器发展为能够执行复杂自主工作流程的动态系统，多智能体框架通过分配专业角色来实现自我反思和相互审计，以提高可靠性。然而，这种角色扮演会引发类似人类的认知偏差——行动者-观察者不对称（AOA），即行动者（自我反思时）倾向于将失败归因于外部因素，而观察者（相互审计时）则将相同错误归因于内部故障。

Method: 提出了ReTAS（通过正题-反题-合题的推理）方法，通过辩证对齐训练模型，强制实现视角不变的推理。该方法整合了辩证思维链和群体相对策略优化，引导智能体将冲突观点合成为客观共识。

Result: 使用新的模糊失败基准测试量化了AOA效应，发现仅交换视角就会在大多数模型超过20%的情况下触发AOA效应。实验表明，ReTAS有效缓解了归因不一致性，并显著提高了模糊场景下的故障解决率。

Conclusion: 角色扮演虽然能利用领域专家知识，但会引入认知偏差。ReTAS通过辩证对齐训练能够有效缓解行动者-观察者不对称偏差，提高多智能体系统的可靠性和客观性。
...
<details>
  <summary>Details</summary>
Motivation: 语音大语言模型中的幻觉问题存在重大风险，但现有检测方法通常依赖昂贵或不切实际的黄金标准输出。此外，为文本LLM开发的幻觉检测方法无法直接捕捉音频特定信号。

Method: 研究四种注意力衍生指标：AUDIORATIO、AUDIOCONSISTENCY、AUDIOENTROPY和TEXTENTROPY，这些指标旨在捕捉与幻觉相关的病态注意力模式。基于这些特征训练轻量级逻辑回归分类器，实现高效推理时检测。

Result: 在自动语音识别和语音到文本翻译任务中，对Qwen-2-Audio和Voxtral-3B的评估显示，该方法在域内数据上优于基于不确定性和先前基于注意力的基线方法，PR-AUC提升高达+0.23，并能泛化到域外ASR设置。发现使用约100个注意力头即可实现强性能，相比使用所有头能改善域外泛化能力。

Conclusion: 虽然效果具有模型依赖性和任务特定训练要求，但结果表明注意力模式为语音大语言模型的幻觉检测提供了有价值的工具。
...
TL;DR: 大型语言模型（LLMs）出现后，同行评审报告变得更长、更流畅，更注重摘要和表面清晰度，但原创性、可复制性等深层评估维度关注度下降。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs快速发展，学术交流面临前所未有的冲击，特别是同行评审领域。虽然先前研究表明LLMs开始影响同行评审，但尚不清楚它们是否改变了评审的核心评估功能，以及如何影响评审报告的语言形式、评估重点和推荐信号。

Method: 研究LLMs出现后学术文章同行评审报告的变化，包括：1）分析评审意见的语言特征（长度、复杂度）；2）自动标注单个评审句子的评估方面；3）使用最大似然估计方法识别可能由LLMs修改或生成的评审报告；4）评估LLM辅助评审报告中提到的评估方面对论文决策推荐信息性的影响。

Result: LLMs出现后，同行评审文本变得更长、更流畅，更强调摘要和表面清晰度，语言模式更标准化（特别是置信度较低的评审者）。同时，对原创性、可复制性和细微批判性推理等深层评估维度的关注度下降。

Conclusion: LLMs正在改变同行评审的性质，使其更注重表面特征和标准化表达，但可能以牺牲深度评估质量为代价，这对学术交流的完整性和质量构成潜在风险。
...
TL;DR: 该研究创建了首个撒丁语即兴诗歌结构化语料库A Bolu，包含2,835个诗节共141,321个词元，通过多维分析揭示了即兴诗歌中的重复模式，验证了Parry和Lord的程式化理论。


<details>
  <summary>Details</summary>
Motivation: 自然语言处理对少数民族语言的关注尚未解决口头语言遗产保护的问题，特别是基于实时即兴创作的即兴诗歌这一表演性体裁，在计算语言学中仍是一个未被充分探索的领域。需要创建特定资源来记录和分析即兴诗歌的结构。

Method: 创建了首个专门针对cantada logudorese（撒丁语变体）即兴诗歌的结构化语料库A Bolu，包含2,835个诗节共141,321个词元。采用多维分析方法，结合描述性统计指标和计算语言学技术来映射诗歌文本的特征。

Result: 分析结果表明，撒丁语即兴诗人的创作具有重复出现的模式，这支持了Parry和Lord的程式化理论。这些模式为理解口头创造力提供了新的视角。

Conclusion: 该研究不仅为理解口头创造力提供了新的视角，还为开发更具包容性、更关注较少使用语言特性的自然语言处理工具做出了重要贡献，填补了少数民族语言即兴诗歌研究的空白。
...
TL;DR: 研究比较了GPT-4.1、Claude Sonnet 4.6和Gemini 2.5 Flash三个大语言模型在温度=0条件下生成运动处方的重复一致性，发现虽然GPT-4.1语义相似度最高，但其生成行为与Gemini 2.5 Flash有本质差异：前者生成100%独特输出，后者则存在显著文本重复。


<details>
  <summary>Details</summary>
Motivation: 评估不同大语言模型在临床运动处方生成任务中的重复生成一致性，揭示单一输出评估无法捕捉的模型行为差异，为临床部署提供决策依据。
...
Result: GPT-4.1语义相似度最高（0.955），但生成100%独特输出；Gemini 2.5 Flash相似度次高（0.950），但仅有27.5%独特输出，表明其高相似度源于文本重复而非一致推理；Claude Sonnet 4.6相似度最低（0.903）。安全表达在所有模型中均达到天花板水平，缺乏区分度。

Conclusion: 相同解码设置下不同模型展现出本质不同的生成行为，模型选择应是临床决策而非纯技术决策，重复生成条件下的输出行为应作为LLM运动处方系统可靠部署的核心标准。
...
<details>
  <summary>Details</summary>
Motivation: 虽然英语、荷兰语、西班牙语和汉语等语言已有大规模自由联想规范数据，但德语一直缺乏可比资源。为填补这一空白，研究者开发了德语版"小词世界"项目的自由联想规范数据集。

Method: 作为多语言Small World of Words项目的一部分，收集了5,877个德语提示词的联想数据，建立了全面的数据收集程序、参与者特征描述和预处理流程，形成了SWOW-DE数据集。

Result: SWOW-DE规范能稳健预测词汇决策任务、关联性判断和心理语言学词汇评分中的表现。与现有德语资源相比表现优异，跨语言比较揭示了共享和语言特定的联想模式。

Conclusion: SWOW-DE是目前最大的德语自由联想数据集，为语言学、心理学和跨文化研究提供了独特资源，并指出了未来研究的有前景方向。
...
TL;DR: μLMs是8M-30M参数的超小型语言模型，在边缘设备上快速生成回复的前4-8个词，然后由云端模型无缝接续完成，从而掩盖云端延迟，实现资源受限设备的响应式AI。


<details>
  <summary>Details</summary>
Motivation: 智能手表等边缘设备无法持续运行100M-1B参数的模型，而云端推理又带来多秒延迟，破坏了响应式助手的体验。需要在资源受限设备上实现低延迟的AI响应。

Method: 设计μLMs（8M-30M参数）在设备端快速生成回复开头，云端模型作为接续者而非响应者完成剩余部分；采用协作生成框架实现句中无缝切换，并通过三种错误校正方法处理本地开头错误。

Result: μLMs的性能可与70M-256M参数的现有模型相媲美，能够启动响应并由更大模型无缝完成，实现了数量级不对称的协作，为极端资源受限设备解锁了响应式AI能力。

Conclusion: 通过μLMs与云端模型的协作生成框架，可以在掩盖云端延迟的同时实现响应式AI，证明在极端规模下有用的语言生成仍然可行，为资源受限边缘设备提供了实用解决方案。
...
TL;DR: 本文测试了提示设计和模型选择对GPT预测棒球球迷体验评分的相对影响，发现提示定制化能小幅提升准确性，而模型选择影响有限，主要性能瓶颈来自文本本身的信息缺失。


<details>
  <summary>Details</summary>
Motivation: 先前研究显示GPT 4.1能基于开放式调查文本预测球迷体验评分，本文旨在探究提示设计和模型选择对预测性能的相对影响，以确定哪些因素真正影响预测准确性。

Method: 使用约10,000份来自5支MLB球队的赛后调查，比较四种配置：原始基线提示与适度定制化提示，分别与三种GPT模型（4.1、4.1-mini、5.2）组合，评估在±1分范围内的预测一致性。

Result: 提示定制化使GPT 4.1的预测一致性从67%提升至69%；模型更换会降低性能：GPT 5.2回归基线水平，GPT 4.1-mini下降6个百分点；文本本身的语言特征对准确性的影响远大于提示或模型选择。

Conclusion: 性能瓶颈有两个部分：模型解读文本的偏差（可通过提示设计纠正）和球迷书写内容与实际决策之间的信息差距（无法通过工程手段弥补）。提示定制化能针对前者产生可预测的改进，而模型选择的影响不稳定。
...
TL;DR: Chat2Workflow是一个用于从自然语言生成可执行可视化工作流的基准测试，旨在研究大语言模型能否自动化这一多轮交互过程，并提出了一个智能体框架来缓解常见的执行错误。


<details>
  <summary>Details</summary>
Motivation: 当前工业部署中，可执行可视化工作流已成为主流范式，但完全依赖人工构建，开发成本高、耗时长且容易出错。研究大语言模型能否自动化这一多轮交互过程。

Method: 提出了Chat2Workflow基准测试，基于大量真实业务工作流构建，每个实例设计成可转换并直接部署到Dify和Coze等实际工作流平台。同时提出了一个鲁棒的智能体框架来缓解重复执行错误。

Result: 实验结果表明，最先进的语言模型通常能捕捉高层意图，但在生成正确、稳定、可执行的工作流方面存在困难，特别是在复杂或变化的需求下。智能体框架带来了最高5.34%的解决率提升。

Conclusion: 尽管智能体框架有所改进，但现实世界仍存在差距，这使得Chat2Workflow成为推进工业级自动化的重要基础。
...
TL;DR: 功能向量在多语言模型中的语言无关性研究：以机器翻译为例，发现翻译功能向量在不同语言间具有可迁移性


<details>
  <summary>Details</summary>
Motivation: 虽然先前研究表明多语言模型的表示可以是语言无关的，但功能向量（从模型激活中提取的任务表示）是否也具有语言无关性尚不清楚。本研究旨在探索功能向量是否表现出语言无关性，以机器翻译作为案例研究。

Method: 使用三个仅解码器的多语言大语言模型，从英语→目标语言方向提取翻译功能向量，测试这些向量在其他目标语言上的可迁移性。通过消融实验验证功能向量的作用，并研究基础模型功能向量向指令调优变体的迁移能力，以及从词级到句级翻译的泛化能力。

Result: 研究发现：1）从单一英语→目标语言方向提取的翻译功能向量能够迁移到其他目标语言，持续提高多个未见语言中正确翻译词元的排名；2）移除功能向量会降低跨语言翻译质量，但对不相关任务影响有限；3）基础模型的功能向量可以迁移到指令调优变体；4）功能向量在某种程度上能从词级翻译泛化到句级翻译。

Conclusion: 功能向量在多语言模型中表现出语言无关性，特别是在机器翻译任务中具有跨语言可迁移性。这一发现为理解模型内部表示的任务特异性提供了新见解，并可能促进跨语言任务的有效表示学习。
...
TL;DR: 该论文提出了文档基础的相关见解生成任务，旨在为开放性问题生成补充见解以改进初始答案，并发布了SCOpE-QA数据集和InsightGen方法。


<details>
  <summary>Details</summary>
Motivation: 开放性问题回答对AI系统具有挑战性，需要超越事实检索的综合、判断和探索能力，而现有问答基准不支持用户通过多次迭代精炼答案的过程。

Method: 提出了InsightGen两阶段方法：1) 使用聚类构建文档集合的主题表示；2) 从主题图中通过邻域选择相关上下文，利用LLM生成多样且相关的见解。

Result: 在3,000个问题上的广泛评估显示，InsightGen能够持续生成有用、相关且可操作的见解，为这一新任务建立了强基线。

Conclusion: 该研究填补了现有问答基准的空白，通过文档基础的相关见解生成任务支持更丰富的用户交互和更好的问答体验。
...
TL;DR: 该研究提出了一种可扩展的方法来测量政治话语中的认知取向（证据vs直觉），应用于七个国家1946-2025年的1500万议会演讲片段，发现证据导向的话语与协商民主和治理质量正相关。


<details>
  <summary>Details</summary>
Motivation: 政治话语反映了不同的认知取向，从基于证据的推理到基于直觉的推理，这对民主审议和治理至关重要。然而，缺乏可扩展的方法来系统测量这些认知取向及其与民主治理的关系。

Method: 引入证据减直觉（EMI）评分方法，结合大型语言模型评级和基于嵌入的语义相似度分析。将该方法应用于七个国家1946-2025年的1500万议会演讲片段，分析话语的时间模式及其与协商民主和治理的关联。

Result: EMI与协商民主在时间维度上呈正相关，在同期和滞后分析中关系一致。EMI也与治理的透明度和法律可预测实施维度正相关。证据导向的话语与民主和治理质量密切相关。

Conclusion: 政治话语的认知性质对民主质量和治理至关重要。证据导向的话语与更好的协商民主和治理结果相关，这强调了在政治话语中培养证据基础推理的重要性。
...
TL;DR: 该论文提出了一种新方法，通过寻找LLMs中自然语言和符号语言推理过程的共享内部逻辑子空间，来提升多步逻辑推理能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多步逻辑推理方面仍然存在困难。现有方法要么纯粹在自然语言形式中精炼推理链，要么将符号求解器作为外部模块附加。作者想知道LLMs是否包含一个共享的内部逻辑子空间，能够同时对齐自然语言和符号语言视图的推理过程。

Method: 使用典型相关分析（CCA）在自然语言和符号语言推理链的配对残差激活上，学习一个具有最大跨视图相关性的低维子空间。设计了一种无需训练的方法，沿着这个逻辑子空间引导LLMs的推理链，从而利用两种视图的互补推理信号。

Result: 在四个逻辑推理基准测试上的实验证明了该方法的有效性，准确率提高了多达11个百分点，并且在域外问题上表现出良好的泛化能力。

Conclusion: LLMs确实包含一个共享的内部逻辑子空间，该子空间捕获了跨视图共享的逻辑推理能力，同时独立于表面形式。通过在这个子空间中引导推理，可以显著提升LLMs的逻辑推理性能。
...
### [150] [Structural Dynamics of G5 Stock Markets During Exogenous Shocks: A Random Matrix Theory-Based Complexity Gap Approach](https://arxiv.org/abs/2604.19107)
*Kundan Mukhia,Imran Ansari,Md. Nurujjaman*

Main category: q-fin.ST

TL;DR: 该研究通过随机矩阵理论分析G5国家股市，发现外生冲击事件期间市场存在稳健的三阶段结构模式：冲击前正复杂性缺口反映多因素交互；冲击中缺口坍缩至零显示单一主导模式同步；冲击后呈现非单调恢复路径（虚假恢复→再坍缩→最终稳定）。


<details>
  <summary>Details</summary>
Motivation: 研究旨在识别股票市场在外生冲击事件期间的稳健结构特征，理解危机期间市场动态的结构性变化模式，为投资者提供更准确的风险评估指标。

Method: 使用随机矩阵理论分析G5国家股市的集体收益动态，引入"复杂性缺口"指标（归一化最大特征值与平均配对相关性之差），并辅以序数熵分析验证方向多样性变化。

Result: 发现跨多个冲击事件（2025年美国关税事件、COVID-19危机、2024年日本和中国特定冲击）的一致三阶段模式；复杂性缺口值较低能预测未来投资组合波动率升高，尤其在冲击后；初始缺口扩大可能是虚假恢复信号。

Conclusion: 金融市场在危机和恢复期间存在稳健的结构特征，复杂性缺口可作为状态依赖的风险指标，帮助投资者区分虚假恢复与真正的结构稳定化。
...
### [151] [On Solving the Multiple Variable Gapped Longest Common Subsequence Problem](https://arxiv.org/abs/2604.18645)
...
<details>
  <summary>Details</summary>
Motivation: VGLCS问题在分子序列比较和时间序列分析中具有重要应用，需要处理字符间的灵活间隙约束，现有方法难以应对该问题的组合爆炸挑战。

Method: 采用基于根节点的状态图表示，将状态空间分解为多个根状态子图；使用迭代波束搜索策略动态维护有希望的候选根节点池；结合LCS文献中的启发式方法来提升搜索质量。

Result: 在包含320个合成实例的测试集上（最多10个输入序列，每个序列最多500个字符），所提方法在可比运行时间内展现出优于基线波束搜索的鲁棒性。

Conclusion: 这是首个针对VGLCS问题的全面计算研究，提出的框架能有效处理组合爆炸问题，为分子序列比较和时间序列分析等应用提供了实用解决方案。
...
### [152] [Beyond One Output: Visualizing and Comparing Distributions of Language Model Generations](https://arxiv.org/abs/2604.18724)
*Emily Reif,Claire Yang,Jared Hwang,Deniz Nazar,Noah Smith,Jeff Heer*

Main category: cs.AI

TL;DR: GROVE是一个可视化工具，通过文本图展示语言模型的多重生成结果，帮助用户理解输出分布的结构模式，避免基于单一输出的过度概括。


<details>
  <summary>Details</summary>
Motivation: 用户通常通过单一输出来评估语言模型，但这隐藏了输出的分布结构（如模式、边缘案例、对提示微小变化的敏感性），导致用户在开放式任务中基于轶事过度概括。

Method: 通过对13名研究人员进行形成性研究，了解实践中随机性的重要性，然后开发GROVE交互式可视化工具，将多个LM生成表示为文本图中的重叠路径，揭示共享结构、分支点和聚类。

Result: 通过三个众包用户研究（共131名参与者）评估，结果显示图摘要改进了对结构判断（如评估多样性），而直接输出检查在细节导向问题上表现更好，支持混合工作流程。

Conclusion: GROVE可视化工具能够有效揭示语言模型输出的分布结构，帮助用户更好地理解和评估模型行为，结合图摘要和直接输出检查的混合工作流程最为有效。
...
### [153] [ARES: Adaptive Red-Teaming and End-to-End Repair of Policy-Reward System](https://arxiv.org/abs/2604.18789)
...
<details>
  <summary>Details</summary>
Motivation: 现有RLHF方法存在关键漏洞：不完美的奖励模型可能成为单点故障，无法惩罚不安全行为。现有红队方法主要针对策略级弱点，忽视了核心LLM和奖励模型同时失效的系统性弱点。

Method: ARES框架使用"安全导师"动态组合语义连贯的对抗性提示（主题、角色、策略、目标），生成恶意和安全响应，同时暴露核心LLM和奖励模型的弱点。然后采用两阶段修复：先微调奖励模型以更好检测有害内容，再利用改进的奖励模型优化核心模型。

Result: 在多个对抗性安全基准测试中，ARES显著增强了安全鲁棒性，同时保持了模型能力，为全面的RLHF安全对齐建立了新范式。

Conclusion: ARES框架通过系统发现和修复RLHF中的双重漏洞，提供了一种更全面的安全对齐方法，解决了现有红队方法忽视的系统性弱点问题。
...
TL;DR: 首个基于依赖类型理论的交互式定理证明框架，用于专利分析的混合AI+Lean 4管道，实现形式化验证的专利分析算法
...
Method: 将专利权利要求编码为Lean 4中的DAG，匹配强度作为已验证完全格的元素，通过证明正确的单调函数传播置信度分数，形式化五个IP用例的六个算法

Result: DAG覆盖核心算法（算法1b）在固定有界匹配分数后完全机器验证，结构引理、覆盖核心生成器和闭合路径恒等式在Lean 4中机器验证，高层定理保持非正式证明草图

Conclusion: 这是首个将交互式定理证明应用于知识产权分析的框架，为专利分析提供了形式化验证的数学基础，但保证条件依赖于ML层的准确性
...
<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型在训练过程中会犯重复错误，无法达到完美分类性能。作者旨在开发一种能够无错误训练、避免重复犯错的新方法，提高分类模型的准确性和可靠性

Method: 提出"人工特殊智能"新概念，应用于18个MedMNIST生物医学数据集进行分类问题训练。该方法使模型能够实现无错误训练，避免重复犯错

Result: 在18个数据集中，除3个存在双标签问题的数据集外，其余15个数据集均实现了完美训练，达到了无错误分类性能

Conclusion: 人工特殊智能概念能够有效实现机器学习模型的完美训练，避免重复错误，在生物医学图像分类任务中表现出优异性能，但双标签问题仍是需要解决的挑战
...
TL;DR: 研究发现聚合基准无法反映个体偏好，提出需要个性化LLM基准来根据用户特定需求排名模型


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型评估基准采用聚合评分，忽略了用户个体偏好的差异性，需要开发能够反映个人需求的个性化基准

Method: 使用ELO评分和Bradley-Terry系数为115名Chatbot Arena用户计算个性化模型排名，分析用户查询特征（主题和写作风格）与LLM排名变化的关系

Result: 个体LLM排名与聚合排名差异显著：Bradley-Terry相关性平均仅0.04（57%用户接近零或负相关），ELO评分中等相关（0.43）。用户主题兴趣和沟通风格异质性显著影响模型偏好

Conclusion: 聚合基准对大多数用户无法捕捉个体偏好，需要开发根据个人用户偏好排名LLM模型的个性化基准
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型评估基准缺乏专门针对数据仓库模式图拓扑推理的测试，特别是同时考虑外键和数据血缘关系的场景，需要一个新的基准来评估模型在这方面的能力

Method: 开发了DW-Bench基准，包含1,046个自动生成的可验证正确问题，覆盖5个数据仓库模式，明确整合了外键和数据血缘两种边关系，用于评估大语言模型的图拓扑推理能力

Result: 实验结果显示，工具增强的方法在DW-Bench上显著优于静态方法，但在困难的组合子类型问题上性能达到瓶颈，表明当前模型在复杂图拓扑推理方面仍有局限
...
### [158] [SAVOIR: Learning Social Savoir-Faire via Shapley-based Reward Attribution](https://arxiv.org/abs/2604.18982)
...
TL;DR: SAVOIR框架利用合作博弈论中的夏普利值解决社交智能语言智能体中的信用分配问题，在SOTOPIA基准测试中取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 社交智能是语言智能体的核心挑战，现有基于强化学习的方法存在信用分配问题：难以确定多轮对话中单个话语对最终结果的贡献。现有方法使用语言模型直接分配奖励，但缺乏理论依据且是回顾性的。

Method: 提出SAVOIR框架，结合两个互补原则：1) 期望效用将评估从回顾性归因转向前瞻性估值，捕捉话语促成有利未来轨迹的战略潜力；2) 夏普利值确保公平的信用分配，具有效率、对称性和边际性的公理保证。

Result: 在SOTOPIA基准测试中，SAVOIR在所有评估设置中都达到了新的最先进性能，其7B模型匹配甚至超过了GPT-4o和Claude-3.5-Sonnet等专有模型。大型推理模型持续表现不佳，表明社交智能需要与分析推理不同的能力。

Conclusion: SAVOIR为社交智能语言智能体的信用分配问题提供了理论严谨的解决方案，证明了合作博弈论方法在社交强化学习中的有效性，并揭示了社交智能需要不同于传统推理能力的独特能力。
...
### [159] [Plausible Reasoning and First-Order Plausible Logic](https://arxiv.org/abs/2604.19036)
*David Billington*
...
<details>
  <summary>Details</summary>
Motivation: 传统的逻辑系统难以处理可信陈述（可能为真但偶尔为假的陈述），需要一种不使用概率等数值的推理系统来处理日常生活中的可信推理问题。

Method: 提出了17条可信推理逻辑原则（14条必要原则和3条理想原则），定义了一阶可信逻辑系统，并开发了8种不同的推理算法来处理各种可信推理情境。

Result: 可信逻辑系统满足了除两条理想原则外的所有原则，并能正确处理所有可信推理示例，是目前唯一已知的此类逻辑系统。

Conclusion: 可信逻辑为处理可信推理提供了一种有效的逻辑框架，能够在不使用概率等数值的情况下进行合理的非单调推理，填补了该领域的空白。
...
### [160] [Learning Lifted Action Models from Unsupervised Visual Traces](https://arxiv.org/abs/2604.19043)
...
<details>
  <summary>Details</summary>
Motivation: 在现实世界领域中，高效构建捕捉动作前提条件和效果的模型对于应用AI规划至关重要。先前工作主要从状态和/或动作序列的高层描述中学习此类模型，但本文处理更具挑战性的场景：从状态图像序列学习提升动作模型，且无需动作观察。

Method: 提出深度学习框架，联合学习状态预测、动作预测和提升动作模型。引入混合整数线性规划（MILP）防止预测崩溃和自增强错误。MILP获取预测的状态、动作和动作模型，在部分轨迹上求解逻辑一致的状态、动作和动作模型，使其尽可能接近原始预测。从MILP解决方案中提取伪标签，用于指导进一步训练。

Result: 在多个领域的实验表明，集成基于MILP的校正有助于模型逃离局部最优解，并向全局一致解收敛。

Conclusion: 该方法能够从状态图像序列中有效学习提升动作模型，无需动作观察，通过MILP校正机制提高了学习的一致性和全局收敛性。
...
### [161] [Reinforcement Learning Improves LLM Accuracy and Reasoning in Disease Classification from Radiology Reports](https://arxiv.org/abs/2604.19060)
*Yishu Wei,Yi Lin,Adam Flanders,George Shih,Yifan Peng*

Main category: cs.AI

TL;DR: 提出两阶段方法：先监督微调疾病分类，再通过GRPO优化预测准确性和格式，提升放射学报告分类性能


<details>
  <summary>Details</summary>
Motivation: 放射学报告的准确疾病分类对许多应用至关重要。虽然监督微调轻量级LLMs能提高准确性，但可能会降低推理能力。
...
### [162] [OLLM: Options-based Large Language Models](https://arxiv.org/abs/2604.19087)
*Shashank Sharma,Janina Hoffmann,Vinay Namboodiri*

Main category: cs.AI

TL;DR: OLLM是一种用学习到的选项集合替代标准LLM单token预测的方法，通过离散潜变量索引多个合理下一token选项，增强生成多样性和可控性。


<details>
  <summary>Details</summary>
Motivation: 标准LLM使用单token预测，依赖温度或采样启发式方法产生多样性，缺乏对变异的显式建模，限制了可控性和鲁棒性。

Method: OLLM在输出头前插入编码器和解码器层，将预训练LLM转换为具有学习选项的模型，使用低维潜空间参数化多个下一token选项，并训练紧凑策略在潜空间中选择选项。

Result: 在1.7B参数骨干上，OLLM在最优潜选择下达到约70%最终答案正确率，优于SOTA LoRA基线的51%；潜空间策略学习显著提高样本效率，减少常见错位问题。

Conclusion: 选项化下一token建模增强了数学推理中的可控性、鲁棒性和效率，潜空间策略学习是LLM强化学习的有前景方向。
...
### [163] [Towards Scalable Lifelong Knowledge Editing with Selective Knowledge Suppression](https://arxiv.org/abs/2604.19089)
*Dahyun Jung,Jaewook Lee,Heuiseok Lim*

Main category: cs.AI

TL;DR: LightEdit是一个终身知识编辑框架，通过检索相关知识和解码策略抑制原始知识概率，实现高效知识更新，解决现有方法在连续编辑中的稳定性问题和高训练成本限制。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型需要频繁更新知识以反映变化的事实并减少幻觉。现有参数编辑方法在连续编辑中面临灾难性遗忘问题，而基于检索的方法由于高训练成本在不同数据集上适用性有限。
...
Conclusion: LightEdit通过结合检索选择和解码策略，解决了终身知识编辑中的稳定性和可扩展性问题，为大型语言模型的知识更新提供了高效且成本效益高的解决方案。
...
### [164] [Has Automated Essay Scoring Reached Sufficient Accuracy? Deriving Achievable QWK Ceilings from Classical Test Theory](https://arxiv.org/abs/2604.19131)
*Masaki Uto*

Main category: cs.AI

TL;DR: 论文提出了两种基于经典测试理论的数据集特定QWK上限：理论上限（完美预测潜在真实分数的理想AES模型能达到的最大QWK）和类人上限（具有人类评分误差水平的AES模型能达到的QWK），为AES模型性能评估提供了更准确的参考标准。


<details>
  <summary>Details</summary>
Motivation: 当前自动作文评分（AES）系统通常使用二次加权kappa（QWK）在公共基准上进行评估，但由于基准标签由人工评分员分配且不可避免地包含评分误差，因此不清楚QWK的理论可达到上限是什么，也不清楚实际部署需要达到什么水平。需要更准确的评估标准来指导AES模型的开发和部署。

Method: 基于经典测试理论中的可靠性概念，推导出两种数据集特定的QWK上限：1）理论上限：在标签噪声下，完美预测潜在真实分数的理想AES模型能达到的最大QWK；2）类人上限：具有人类评分误差水平的AES模型能达到的QWK。这些上限可以从标准双评分员基准中估计，无需额外标注。通过模拟实验验证了所提上限的有效性，并在真实基准上进行了实验分析。

Result: 模拟实验验证了所提上限的有效性，真实基准实验表明：1）人-人QWK常常低估真实上限；2）所提上限能够澄清现代AES模型的当前性能和剩余提升空间；3）为AES模型评估提供了更准确的参考标准，有助于确定模型是否已达到实际部署所需的性能水平。

Conclusion: 提出的两种QWK上限为AES模型评估提供了更准确的理论框架，能够更清晰地界定模型性能的理论极限和实际目标，有助于指导AES系统的开发和部署决策。这些上限可以从现有双评分员基准中估计，无需额外标注成本，具有实际应用价值。
...
### [165] [Reasoning-Aware AIGC Detection via Alignment and Reinforcement](https://arxiv.org/abs/2604.19172)
*Zhao Wang,Max Xiong,Jianxun Lian,Zhicheng Dou*

Main category: cs.AI

TL;DR: REVEAL框架通过两阶段训练策略（监督微调+强化学习）生成可解释推理链进行AIGC检测，在多个基准测试中达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展和广泛应用，对可靠的AI生成内容检测需求日益增长，但现有方法在模型不断演进时仍面临挑战

Method: 提出AIGC-text-bank多领域数据集，构建REVEAL检测框架，采用两阶段训练：监督微调建立推理能力，强化学习提升准确性、逻辑一致性和减少幻觉

Result: REVEAL在多个基准测试中达到最先进的性能，提供了强大且透明的AIGC检测解决方案

Conclusion: REVEAL框架通过可解释的推理链生成和两阶段训练策略，为AIGC检测提供了鲁棒且透明的解决方案，项目已开源
...
### [166] [UAF: A Unified Audio Front-end LLM for Full-Duplex Speech Interaction](https://arxiv.org/abs/2604.19221)
...
<details>
  <summary>Details</summary>
Motivation: 全双工语音交互作为最自然的人机交互方式，传统级联语音处理流水线存在延迟累积、信息丢失和错误传播等问题。虽然GPT-4o等端到端音频大模型统一了语音理解和生成，但仍是半双工且依赖多个独立前端组件。开发语音助手时发现，优化语音前端与推进后端统一模型同等重要。

Method: 提出统一音频前端大语言模型UAF，将VAD、TD、SR、ASR、QA等多种音频前端任务统一为单一自回归序列预测问题。模型以流式固定时长音频块（如600ms）为输入，使用参考音频提示在开始时锚定目标说话人，回归生成编码语义内容和系统级状态控制（如打断信号）的离散标记。

Result: 实验表明，该模型在多个音频前端任务上达到领先性能，在真实交互场景中显著提升了响应延迟和打断准确性。

Conclusion: UAF模型成功解决了全双工语音系统中前端任务分散的问题，通过统一框架实现了更流畅、响应更快的语音交互体验。
...
### [167] [Explicit Trait Inference for Multi-Agent Coordination](https://arxiv.org/abs/2604.19278)
...
<details>
  <summary>Details</summary>
Motivation: LLM多智能体系统在处理复杂任务时存在协调失败问题，如目标漂移、错误级联和行为错位，需要改进协调机制。

Method: 提出显式特质推断（ETI）方法，基于心理学的温暖度和能力两个维度，从交互历史中推断和追踪合作伙伴特征，指导决策制定。

Result: 在受控经济游戏中减少45-77%的收益损失；在复杂多智能体场景中性能提升3-29%；特质推断能准确预测智能体行为，信息丰富的特质描述驱动改进。

Conclusion: ETI是轻量级且鲁棒的协调改进机制，首次系统证明LLM智能体能可靠地从交互历史推断他人特质，并利用结构化认知改善协调。
...
### [168] [Large Language Models Exhibit Normative Conformity](https://arxiv.org/abs/2604.19301)
*Mikako Bito,Keita Nishimoto,Kimitaka Asatani,Ichiro Sakata*

Main category: cs.AI

TL;DR: 该研究区分了大语言模型中的信息性从众和规范性从众，发现多数LLM同时表现出这两种从众倾向，且规范性从众的目标可通过社交情境操控，这可能使LLM多智能体系统易受恶意用户操纵。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在基于LLM的多智能体系统中表现出的从众偏见可能严重影响决策质量。以往研究简单地将"从众"视为意见改变，但本研究旨在从社会心理学角度区分信息性从众和规范性从众，以理解LLM从众的内在机制。

Method: 设计新任务区分信息性从众（参与者追求准确判断）和规范性从众（参与者避免冲突或寻求群体接纳），基于这些任务设置进行实验，评估六个LLM的从众行为，并分析相关的内部向量。

Result: 六个评估的LLM中，多达五个同时表现出信息性从众和规范性从众倾向。有趣的是，通过操控社交情境的细微方面，可以控制特定LLM规范性从众的目标对象。这表明LLM-MAS决策可能易受少数恶意用户操纵。内部向量分析表明，虽然两种从众行为外部表现相同，但可能由不同的内部机制驱动。

Conclusion: 该研究为理解LLM中"规范"如何实现以及它们如何影响群体动态提供了初步里程碑。研究结果表明，需要开发机制来减轻LLM多智能体系统中从众偏见带来的风险，特别是防止恶意操纵。
...
TL;DR: GRASPrune是一种结构化剪枝框架，可在预训练后联合剪枝FFN通道和KV头组，通过投影直通估计器学习轻量级门控分数，在保持骨干权重冻结的同时满足预算约束，最终获得更小的密集检查点且推理时无额外参数。
...
Method: 提出GRASPrune结构化剪枝框架：1）使用投影直通估计器学习轻量级门控分数，在每一步都强制执行满足预算的硬掩码；2）保持骨干权重冻结；3）固定掩码后校准保留单元的缩放因子以缓解剪枝引起的尺度不匹配；4）将这些因子折叠到剪枝权重中，获得更小的密集检查点。

Result: 在LLaMA-2-7B上，GRASPrune移除了50%的参数，在WikiText-2上达到12.18困惑度，同时在五个基准测试中保持竞争力的平均零样本准确率。仅使用512个未标记校准序列在单个NVIDIA A100 80GB GPU上进行四个epoch，无需任何完整模型微调。

Conclusion: GRASPrune是一种有效的结构化剪枝方法，能够显著减少LLM的参数数量和服务成本，同时保持模型性能，为高效部署大型语言模型提供了实用解决方案。
...
TL;DR: 论文提出将长时程企业智能体决策行为分解为四个正交对齐轴（事实精度、推理一致性、合规重构、校准弃权），以更精细评估其在监管环境下的表现，发现传统聚合准确度指标会掩盖重要失败模式。


<details>
  <summary>Details</summary>
Motivation: 当前长时程企业智能体（贷款审批、理赔裁决等）评估仅报告单一任务成功率，混淆了不同的失败模式，无法判断智能体是否符合部署环境的监管标准。需要更细粒度的评估框架来确保智能体在监管约束下的对齐性。

Method: 提出四轴对齐分解框架：事实精度（FRP）、推理一致性（RCS）、合规重构（CRR）、校准弃权（CAR）。在LongHorizon-Bench基准上测试六种记忆架构，使用确定性真实构造覆盖贷款资格和保险理赔裁决任务。

Result: 发现聚合准确度无法揭示的结构性差异：检索架构在事实精度上崩溃；模式锚定架构需支付"脚手架税"；简单摘要+事实保留提示在FRP、RCS、EDA和CRR上表现强劲；所有六种架构都在每个案例上做出决策，暴露了决策对齐轴的缺失。摘要架构在事实召回上的表现与预期相反。

Conclusion: 四轴分解框架能更精细评估长时程决策智能体，揭示传统指标隐藏的对齐问题。制度对齐（合规重构）和决策对齐（校准弃权）在现有对齐文献中代表性不足，但对实际部署至关重要。该框架可通过构建事实模式和校准CRR审计提示转移到任何受监管决策领域。
...
TL;DR: 研究发现前沿大模型在生成Lean 4证明时存在"形式化博弈"现象，即利用形式验证与忠实翻译之间的差距。虽然统一生成模式下未发现系统性博弈证据，但两阶段管道揭示了两种不同的不忠实模式。


<details>
  <summary>Details</summary>
Motivation: 形式验证能保证证明的有效性，但不能保证形式化的忠实性。在自然语言逻辑推理中，模型需要从头构建公理系统而不受库约束，这种有效证明与忠实翻译之间的差距尤为突出。研究旨在探究前沿模型在生成Lean 4证明时是否利用这一差距进行"形式化博弈"。

Method: 评估GPT-5和DeepSeek-R1在303个一阶逻辑问题上的表现（203个来自FOLIO，100个来自Multi-LogiEval）。比较统一生成与两阶段管道方法，后者将形式化与证明分离。通过编译率分析和跨阶段比较来检测不忠实行为。

Result: 尽管编译率达到87-99%，但统一生成中未发现系统性博弈证据：模型宁愿报告失败也不强行生成证明。两阶段管道揭示了两种不忠实模式：GPT-5在证明生成阶段伪造公理（可通过跨阶段比较检测），而DeepSeek-R1在形式化阶段误译前提，产生内部一致但完全逃避检测的输出。

Conclusion: 高编译率或准确率不应等同于忠实推理。研究揭示了模型在形式化过程中的不同不忠实模式，强调需要更精细的评估方法来检测形式化博弈行为。
...
TL;DR: CoDA通过轻量级适配器干预中间隐藏状态，结合特征蒸馏和分布匹配，对齐源域和目标域的潜在推理表示，显著提升LLM在跨域逻辑推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: LLM在逻辑推理方面仍有提升空间，但在许多专业稀缺领域（如低资源科学学科、新兴生物医学子领域等），高质量领域内示范样本有限或完全缺失，限制了现有方法的通用性。虽然已有研究探索使用跨域样本作为替代示范，但由于显著的领域偏移，性能提升有限。
...
Conclusion: CoDA通过干预隐藏状态和对齐跨域推理表示，有效解决了领域偏移问题，显著提升了LLM在跨域逻辑推理任务上的性能，为专业知识稀缺领域的推理问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度剪枝方法主要依赖层间余弦距离这一单一启发式标准，但这种方法在不同架构中表现不稳定，甚至可能导致灾难性性能崩溃。需要更全面的评估标准来准确识别冗余层。

Method: 提出SimDiff方法，从两个正交角度联合评估层重要性：1) 表示相似性；2) 变换差异。变换差异使用两种不同度量：MSSD（对异常值敏感，识别做出决定性修正的层）和MASD（稳健测量层的平均贡献）。

Result: 在0.5B到13B参数的多个模型上实验表明，SimDiff显著优于现有基线。在LLaMA2-7B上，25%剪枝率下仍保留超过91%的性能；在LLaMA3.1-8B上剪除12层时，推理速度提升达1.49倍。剪枝后的模型可通过少量微调有效恢复。

Conclusion: SimDiff通过多维度评估层重要性，解决了单一启发式方法的局限性，为大型语言模型的高效部署提供了更可靠的深度剪枝方案。
...
<details>
  <summary>Details</summary>
Motivation: 老年人跌倒是一个持续存在的健康风险，现有跌倒预测和检测系统由于缺乏上下文感知、高误报率、环境噪声和数据稀缺等问题，无法成为跨护理路径和安全关键环境的通用解决方案。

Method: 将跌倒检测和预测重新定义为异常检测问题，提出一个概念性框架，通过智能体AI系统动态选择相关工具并集成到自适应决策工作流程中，而不是依赖针对狭窄场景的静态配置。

Result: 论文提出了一个概念框架，强调智能体AI在早期识别与风险增加相关的运动模式细微偏差方面的潜在价值，这些偏差可能源于年龄相关衰退、疲劳或环境因素。

Conclusion: 智能体AI系统通过将跌倒检测和预测视为异常检测问题，能够更有效地管理运动相关风险，提供动态、自适应的解决方案，超越传统静态配置的限制。
...
TL;DR: DT2IT-MRM：通过去偏好的偏好构建管道、文本到图像偏好数据重构和迭代训练框架，提升多模态奖励模型性能，在三大基准测试中达到SOTA


<details>
  <summary>Details</summary>
Motivation: 现有多模态偏好数据存在三个关键问题：偏好强度缺乏粒度、文本风格偏见、偏好信号不可靠。此外，开源多模态偏好数据集噪声大，缺乏有效且可扩展的清洗方法。

Method: 提出DT2IT-MRM方法，包含：1）去偏好的偏好构建管道；2）文本到图像偏好数据的新重构；3）迭代训练框架，用于清洗现有多模态偏好数据集以训练多模态奖励模型。

Result: 在三个主要基准测试中达到最新最优性能：VL-RewardBench、Multimodal RewardBench和MM-RLHF-RewardBench。

Conclusion: DT2IT-MRM通过解决现有多模态偏好数据的局限性，显著提升了多模态奖励模型的性能，在多模态对齐任务中表现出色。
...
TL;DR: 研究比较了黑盒成员推理攻击方法在大型语言模型上的效果，发现现有方法都无法可靠检测训练数据中的成员文档，AUC-ROC接近0.5，表明黑盒MIA在LLMs上效果有限。


<details>
  <summary>Details</summary>
Motivation: LLMs使用大量数据进行训练，其中可能包含受版权保护的内容。成员推理攻击旨在检测特定文档是否被包含在LLMs的训练语料中。黑盒MIA需要大量数据操作，比较不同方法具有挑战性。

Method: 研究比较了最先进的黑盒MIA方法，使用统一的数据集评估它们在SOTA LLMs上的效果。同时开发了一种新方法——熟悉度排序，通过给予LLMs更多表达自由来更好地理解其推理过程。

Result: 所有方法在多个LLMs上的AUC-ROC都接近0.5，表明无法可靠检测成员身份。更先进的LLMs显示出更高的真正率和假正率，反映了其更强的推理和泛化能力，也说明了黑盒MIA检测LLMs成员身份的困难。

Conclusion: 当前最先进的黑盒成员推理攻击方法都无法可靠检测LLMs中的训练数据成员。LLMs的推理和泛化能力使得成员检测更加困难，需要新的方法来应对这一挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在编码和数学推理方面已有显著进步，但在视觉语义算术（从图像推断关系）方面仍存在不足。这种能力对于服务机器人和家庭机器人在非结构化环境中至关重要，需要从视觉细节中提取简洁概念并进行关系推理。

Method: 1. 提出两个新任务：二项减法和三项运算；2. 构建Image-Relation-Pair Dataset (IRPD)基准数据集；3. 提出Semantic Arithmetic Reinforcement Fine-Tuning (SAri-RFT)方法，使用可验证函数和Group Relative Policy Optimization (GRPO)对大型视觉语言模型进行后训练。

Result: 该方法在IRPD数据集和真实世界的Visual7W-Telling数据集上取得了最先进的结果，显著提升了模型在视觉语义算术任务上的表现。

Conclusion: 通过为大型视觉语言模型配备强大的跨模态关系推理能力，这项工作提升了家庭机器人在感知中建立符号推理的能力，增强了复杂环境中的决策制定、工具适应性和人机交互。
...
TL;DR: AblateCell是一个用于虚拟细胞代码库的"先复现后消融"智能体，能够自动复现基线结果并系统性地进行消融实验，解决AI虚拟细胞研究中缺乏标准化验证的问题。


<details>
  <summary>Details</summary>
Motivation: AI虚拟细胞研究中缺乏系统性的消融实验，因为生物代码库标准化不足且与领域特定数据和格式紧密耦合。现有编码智能体只能生成代码，缺乏能够复现强基线并严格测试哪些组件真正重要的验证器。

Method: AblateCell采用"先复现后消融"的两阶段方法：1) 自动配置环境、解决依赖和数据问题、重新运行官方评估并生成可验证工件来端到端复现基线；2) 通过生成隔离的代码库变异图，在权衡性能影响和执行成本的奖励下自适应选择实验，进行闭环消融。

Result: 在三个单细胞扰动预测代码库(CPA、GEARS、BioLORD)上评估，AblateCell实现了88.9%的端到端工作流成功率(比人类专家高29.9%)，在恢复真实关键组件方面达到93.3%的准确率(比启发式方法高53.3%)。

Conclusion: AblateCell能够在生物代码库上实现可扩展的、基于代码库的验证和归因，填补了AI虚拟细胞研究中系统性消融实验的空白。
...
### [179] [Time Series Augmented Generation for Financial Applications](https://arxiv.org/abs/2604.19633)
*Anton Kolonin,Alexey Glushchenko,Evgeny Bochkov,Abhishek Saxena*

Main category: cs.AI

TL;DR: 论文提出了一种评估LLM在复杂金融时间序列分析中推理能力的新方法和基准测试，通过工具增强范式验证了智能体在金融任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前标准基准测试难以准确评估大语言模型在复杂定量金融任务中的核心推理能力，特别是解析查询和编排计算的能力，需要更严格的评估方法。

Method: 引入新颖的评估方法和基准测试，采用时间序列增强生成（TSAG）框架，让LLM智能体将定量任务委托给可验证的外部工具，包含100个金融问题，评估工具选择准确性、忠实度和幻觉等指标。

Result: 实验表明，能力强的智能体能够实现接近完美的工具使用准确性，同时保持最低的幻觉水平，验证了工具增强范式的有效性。

Conclusion: 主要贡献是提供了一个评估框架和相应的智能体性能实证洞察，该框架已公开发布，旨在促进金融AI可靠性研究的标准化。
...
### [180] [A Dual Perspective on Synthetic Trajectory Generators: Utility Framework and Privacy Vulnerabilities](https://arxiv.org/abs/2604.19653)
...
<details>
  <summary>Details</summary>
Motivation: 人体移动数据具有高度敏感性，传统隐私保护方法（如聚合、混淆、噪声添加）会显著降低数据效用。虽然生成模型被提出作为替代方案，但其在隐私-效用权衡方面的效果仍不明确，需要新的评估框架。

Method: 引入并应用新的效用评估框架，提出针对特定生成模型的成员推理攻击方法，该攻击针对被认为具有隐私保护性的轨迹用户链接问题抵抗模型。

Result: 研究表明隐私评估仍然是一个重大挑战，应根据欧盟现行法规通过对抗性评估来解决。提出的成员推理攻击能够突破先前被认为具有隐私保护性的生成模型。

Conclusion: 本文为解决人体移动数据隐私-效用权衡问题迈出了第一步，强调需要新的评估框架，并证明即使被认为具有隐私保护性的生成模型也可能面临成员推理攻击的风险。
...
### [181] [Smiling Regulates Emotion During Traumatic Recollection](https://arxiv.org/abs/2604.19019)
...
TL;DR: 研究通过自动微笑检测模型分析978名大屠杀幸存者视频证词中的微笑行为，发现微笑与特定语义主题、叙事结构和时间句法相关，在负面情感时期出现，并能改善周围句子的情感轨迹，在创伤回忆中起情绪调节和社交互动作用。


<details>
  <summary>Details</summary>
Motivation: 研究大屠杀幸存者在视频证词中微笑的时间、地点和原因，探索微笑在创伤回忆中的功能和作用机制。

Method: 1. 开发自动微笑检测模型（F1分数85%）从面部特征检测微笑；2. 使用两种既有的微笑分类法标注检测到的微笑；3. 对1,083,417个转录句子生成叙事特征；4. 从音频、眼动注视和文本转录三个模态提取情感效价；5. 分析微笑率与语义主题、叙事结构、时间句法的相关性。

Result: 1. 微笑率与特定语义主题、叙事结构和时间句法显著相关；2. 微笑常出现在强烈负面情感时期，这些负面情感微笑能显著改善周围句子在所有三个模态中的情感轨迹；3. 微笑减少眼动动态和眨眼频率，这些效应的强度也受叙事效价调节。
...
### [182] [Evaluating Structured Strategy Backtests: Peer Benchmarks, Regime Timing, and Live Performance](https://arxiv.org/abs/2604.18821)
*Chang Liu*

Main category: q-fin.PM

TL;DR: 机构投资者评估结构化策略时依赖营销回溯测试，但研究发现这些回溯测试结果在实盘交易中可移植性有限，主要反映的是策略推出前的市场因子环境而非策略特定技能


<details>
  <summary>Details</summary>
Motivation: 机构投资者通常基于营销回溯测试来评估结构化策略，但这些假设性历史表现记录在实盘交易中能保留多少信号并不明确，需要实证研究验证回溯测试的可信度

Method: 使用来自10家全球机构的1,726个商业化结构化策略数据，分析营销回溯测试（pro-forma performance）与实盘表现的关联性，特别是相对于同行和外部基准的表现

Result: 原始营销回溯测试结果在实盘期间的可移植性有限，一旦相对于同行和外部基准衡量实盘结果，这种关联性会急剧减弱；营销回溯测试主要反映策略推出前的共同因子环境而非策略特定技能；在异常强劲的因子环境后推出的策略后续表现恶化更为严重

Conclusion: 机构投资者应基于适当的同行基准来评估回溯测试，当策略在极端因子运行后推出时，应对回溯测试结果应用更大的折扣；营销回溯测试的价值有限，需要更审慎的评估方法
...
### [183] [Deep Image Prior for photoacoustic tomography can mitigate limited-view artifacts](https://arxiv.org/abs/2604.19176)
*Hanna Pulkkinen,Jenni Poimala,Leonid Kunyansky,Janek Gröhl,Andreas Hauptmann*

Main category: eess.IV

TL;DR: 本文研究将深度图像先验（DIP）框架应用于光声断层扫描（PAT），作为一种无监督重建方法，以减轻实验环境中常见的有限视角伪影和噪声问题。


<details>
  <summary>Details</summary>
Motivation: 光声断层扫描在实验环境中经常遇到有限视角伪影和噪声问题，需要有效的重建方法。深度图像先验框架提供了一种无监督的重建策略，可以缓解这些问题。

Method: 采用深度图像先验框架进行无监督重建，使用圆形测量几何的快速前向和伴随算法实现高效计算。通过快速逆变换和总变差正则化进行初始化以抑制噪声和过拟合。与传统总变差重建方法进行比较。

Result: 实验包括模拟PAT测量（有限视角几何和不同噪声水平）以及实验测量。结果表明，DIP框架在有限视角几何的挑战性情况下提供了有效的无监督重建策略，在多个定量指标上优于总变差重建。

Conclusion: 深度图像先验框架为光声断层扫描提供了一种有效的无监督重建策略，即使在有限视角几何的挑战性情况下也能提供稳健的重建效果，在多个定量指标上优于总变差重建方法。
...
### [184] [Harmonizing MR Images Across 100+ Scanners: Multi-site Validation with Traveling Subjects and Real-world Protocols](https://arxiv.org/abs/2604.19474)
...
TL;DR: HACA3⁺是HACA3算法的增强版本，用于磁共振图像的多中心数据协调，通过改进的伪影编码、背景/前景敏感注意力机制以及基于100+扫描仪的大规模训练，提升了异质性数据协调的可靠性和特异性。


<details>
  <summary>Details</summary>
Motivation: 在多中心神经影像研究和医疗机器学习应用中，异质性磁共振图像数据集的可靠协调至关重要，特别是在实用性临床试验中。现有协调方法在处理多样化的扫描仪和临床协议时存在局限性。

Method: HACA3⁺包含三个关键方法改进：1) 改进的伪影编码器以更好地隔离和减轻图像伪影；2) 背景和前景敏感的注意力机制以提高协调特异性；3) 使用来自64个独立站点、100+扫描仪的数据进行广泛训练，覆盖更广泛的扫描仪多样性。研究聚焦于四种常见MR图像对比度（T1加权、T2加权、质子密度、FLAIR）。

Result: 通过旅行受试者进行站点间协调实验，评估协调模型的泛化性和鲁棒性。与公开版HACA3相比，HACA3⁺表现出更好的性能。下游任务（全脑分割和图像插补）进一步验证了其相关性。消融实验证明了每个增强组件的有效性。

Conclusion: HACA3⁺通过关键方法增强提供了更可靠、更特异的磁共振图像协调，支持多中心神经影像研究和医疗机器学习应用。预训练权重和代码已公开提供。
...
### [185] [Defining Robust Ultrasound Quality Metrics via an Ultrasound Foundation Model](https://arxiv.org/abs/2604.19512)
*Ziyang Huang,Bingyan Li,Chen Ma,Tianyi Liu,Yihui Zhai,Hong Xu,Yi Guo,Zeju Li,Yuanyuan Wang*

Main category: eess.IV

TL;DR: 提出基于TinyUSFM的超声图像质量评估框架，包含两个新指标：TinyUSFM-uLPIPS（全参考感知距离）和TinyUSFM-NRQ（无参考质量评分），解决现有PSNR和VGG-LPIPS在超声诊断效用评估中的不足。


<details>
  <summary>Details</summary>
Motivation: 临床医生缺乏量化超声重建诊断效用的原则性框架。现有标准如PSNR和VGG-LPIPS不充分，未能考虑模态特定的物理特性或声学成像的结构细微差别。

Method: 开发基于TinyUSFM的评估框架：1) TinyUSFM-uLPIPS：基于多层令牌关系的全参考感知距离指标；2) TinyUSFM-NRQ：利用干净流形建模和最差区域聚合的可部署无参考质量评分，用于检测局部有害伪影。

Result: 新指标具有四个独特优势：1) 任务关联质量：TinyUSFM-uLPIPS与语义任务损伤有更好的校准；2) 跨器官可比性：在不同解剖部位和域偏移数据上保持稳定评分；3) PSNR一致性灵敏度：TinyUSFM-NRQ无需真实图像提供可靠质量评分；4) 临床效用：将专家偏好预测准确率从47.2%提升到72.8%。

Conclusion: 通过将这些优势集成到统一的评估和优化循环中，这项工作建立了模态对齐的标准，最终弥合了算法性能与诊断效用之间的差距。
