...
TL;DR: PAVAS是一种物理感知的视频到音频生成方法，通过物理驱动音频适配器将物理推理融入潜在扩散模型，利用物体质量估计和3D运动轨迹重建来生成更符合物理现实的声音。
...
Method: 1. 使用物理参数估计器(PPE)，通过视觉语言模型估计运动物体质量，通过分割基础的动态3D重建模块恢复运动轨迹计算速度；2. 通过物理驱动音频适配器(Phy-Adapter)将这些物理线索融入潜在扩散模型；3. 提出VGG-Impact基准和音频-物理相关系数(APCC)评估指标。

Result: PAVAS在定量和定性评估中均优于现有V2A模型，能够生成物理上合理且感知上连贯的音频，特别是在物体-物体交互场景中表现出色。

Conclusion: 将物理推理融入视频到音频生成过程能够显著提升生成声音的物理现实性，为更真实的多模态生成提供了新方向。
...
Method: 使用Planet Labs的四波段卫星影像，结合深度学习模型，监测武装冲突中的火灾损害。通过苏丹的五个案例研究验证方法有效性。

Result: 相比基线方法，自动化方法能更准确地捕捉活跃火灾和烧焦区域。使用八波段影像或时间序列影像仅带来边际收益。

Conclusion: 深度学习模型结合四波段卫星影像可以有效监测武装冲突中的火灾损害，实现最小延迟的监控，为冲突地区的快速响应提供技术支持。
...
Method: 1. 使用关键帧作为条件信号注入目标身份，实现灵活可控的编辑；2. 结合关键帧条件和视频参考引导，进行时间缝合以确保稳定的身份保持和高保真重建；3. 构建配对的人脸交换数据集Face2Face，并通过反转数据对确保可靠的真实监督。

Result: 大量实验表明，该方法取得了最先进的结果，能够无缝地将目标身份与源视频的表情、光照和运动相结合，同时显著减少了制作工作流程中的手动工作量。

Conclusion: LivingSwap是首个视频参考引导的人脸交换模型，通过关键帧条件注入和视频参考引导，在长视频序列中实现了高保真度、时间一致性和稳定的身份保持，为影视制作提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 地图推理是解释地理关系的关键认知能力，在灾害响应和城市规划等任务中至关重要，但目前缺乏专门的评估基准。现有研究常将地图视为图表的特例，但地图VQA需要理解分层符号系统、空间关系以及跨地图推理，这些在图表示评估中未被充分捕捉。

Method: FRIEDA从各种领域和地理区域的文档和报告中收集真实地图图像，基于GIS文献分类，涵盖三类空间关系：拓扑关系（边界、相等、相交、包含）、度量关系（距离）和方向关系（方位）。所有问题都需要多步推理，许多需要跨地图关联和推理。评估在两种设置下进行：直接设置（提供相关地图）和上下文设置（模型需先识别相关地图）。

Result: 评估了11个最先进的LVLM，在直接设置下，最强的Gemini-2.5-Pro和GPT-5-Think分别仅达到38.20%和37.20%的准确率，远低于人类表现的84.87%。这表明在多步地图推理方面存在显著差距。

Conclusion: FRIEDA揭示了当前LVLM在复杂地图推理能力上的严重不足，为空间智能研究提供了严谨的基准，将推动该领域的发展。
...
TL;DR: 该论文提出了一种基于频域掩码的通用深度伪造检测方法，通过随机掩码和几何变换增强模型泛化能力，在保持检测精度的同时支持模型剪枝，实现资源高效的深度伪造筛查。


<details>
  <summary>Details</summary>
Motivation: 通用深度伪造检测需要能够识别各种生成模型（包括未见过的模型）生成的AI图像，这要求模型具有良好的泛化能力。同时，在绿色AI时代，需要最小化计算开销以实现大规模深度伪造筛查。现有方法过度依赖空间特征或大规模预训练模型，缺乏高效且泛化能力强的解决方案。

Method: 提出基于频域掩码的训练策略，引入随机掩码和几何变换，特别关注频域掩码因其优越的泛化特性。该方法不依赖传统空间特征或大规模预训练模型，通过频域操作增强模型对各类生成模型的检测能力。

Result: 频域掩码不仅提高了对多种生成器的检测精度，而且在显著模型剪枝下仍能保持性能。在GAN和扩散模型生成的图像数据集上实现了最先进的泛化性能，并在结构化剪枝下表现出一致的鲁棒性。

Conclusion: 频域掩码为可持续和可泛化的深度伪造检测提供了实用解决方案，在保持高性能的同时支持资源节约的计算，是迈向绿色AI时代大规模深度伪造筛查的重要一步。
...
TL;DR: M2A是一种简单的持续测试时自适应方法，通过随机掩码生成不同视图，结合一致性损失和熵最小化损失进行适配，无需复杂的不确定性校准或注意力机制。


<details>
  <summary>Details</summary>
Motivation: 测试时的分布偏移会降低图像分类器性能。现有的持续测试时自适应方法通常依赖不确定性校准或稳定的注意力分数，并引入额外复杂性。作者质疑是否需要定制化的掩码设计，还是简单的随机掩码在强干扰下就足够有效。

Method: 提出Mask to Adapt (M2A)方法：1）生成短序列的掩码视图（空间或频率域）；2）使用两个目标函数进行适配：掩码一致性损失（对齐不同视图的预测）和熵最小化损失（鼓励自信输出）；3）研究两种掩码家族：空间掩码（补丁vs像素）和频率掩码（全频vs低频vs高频）。

Result: 在CIFAR10C/CIFAR100C/ImageNetC（严重度5）上，M2A（空间）达到8.3%/19.8%/39.2%的平均错误率，优于或匹配强基线方法，而M2A（频率）表现较差。消融实验表明简单随机掩码有效且鲁棒。

Conclusion: 简单的随机掩码调度，结合一致性和熵目标，足以驱动有效的测试时自适应，无需依赖不确定性或注意力信号。空间掩码比频率掩码更有效。
...
TL;DR: 提出CVP框架，受人类中央-周边视觉启发，通过目标亲和力令牌和全局网格提升多模态模型的空间推理能力，在3D场景理解基准上达到SOTA。
...
TL;DR: 提出Fourier-RWKV去雾框架，通过多状态感知范式在保持线性计算复杂度的同时实现全面雾霾退化建模，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界非均匀雾霾条件下的图像去雾对可靠视觉感知至关重要，但Transformer方法虽然擅长捕捉全局上下文，其二次计算复杂度阻碍了实时部署。

Method: 基于多状态感知范式，提出Fourier-RWKV框架：1) 空间形式感知：通过可变形四方向令牌移位操作动态调整感受野；2) 频域感知：在傅里叶混合块中将RWKV的核心WKV注意力机制从空间域扩展到傅里叶域；3) 语义关系感知：通过语义桥接模块利用动态语义核融合精确对齐编码器-解码器特征。

Result: 在多个基准测试上的广泛实验表明，Fourier-RWKV在不同雾霾场景下提供最先进的性能，同时显著降低计算开销，在恢复质量和实际效率之间建立了有利的权衡。

Conclusion: Fourier-RWKV通过创新的多状态感知范式，在保持线性计算复杂度的同时实现了全面的雾霾退化建模，为实时高质量图像去雾提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 高质量几何图生成需要严格的空间准确性，同时提供明确的约束条件来指导生成。受几何问题求解中形式化语言和符号求解器在增强正确性和可解释性方面的启发

Method: GeoLoom包含两个核心组件：1) 自动形式化模块，将自然语言转换为专门设计的生成导向形式语言GeoLinua；2) 坐标求解器，使用高效蒙特卡洛优化将形式约束映射到精确坐标。还引入了GeoNF数据集，并提出基于约束的评估指标

Result: GeoLoom在结构保真度方面显著优于最先进的基线方法，为可解释和可扩展的图生成提供了原则性基础

Conclusion: GeoLoom框架通过形式化语言和符号求解实现了高精度几何图生成，为几何图生成领域提供了新的解决方案
...
TL;DR: 该论文提出了一个视觉知识库框架VisKnow，用于构建多模态对象知识图谱，并以AnimalKB为例展示了其在动物类别理解中的应用，包含40万张图像和2.2万条文本知识三元组。


<details>
  <summary>Details</summary>
Motivation: 当前计算机视觉中的对象理解需要超越简单的类别识别，实现更全面的感知，包括对象组件、外观特征、类别关系等。然而现有的多模态数据通常是任务导向的，缺乏系统性的组织来支持深入的对象理解。

Method: 提出了VisKnow框架，通过结合专家设计和大型模型应用，从丰富的对齐文本和图像源知识中提取多模态、对象级知识，并以图结构组织。具体构建了AnimalKB知识库，涵盖406个动物类别，包含从百科全书文档提取的文本知识三元组、图像和区域标注。

Result: AnimalKB包含22,000个文本知识三元组、420,000张图像及相应的区域标注。实验表明该知识库能够增强零样本识别和细粒度视觉问答等对象级视觉任务，同时为知识图谱补全和部件分割提供了具有挑战性的基准。

Conclusion: 视觉知识库的自动构建具有推动视觉理解及其实际应用的潜力。AnimalKB展示了结构化多模态知识在提升对象理解能力方面的价值，为未来的视觉知识表示研究提供了基础。
...
TL;DR: 本文探索了提示调优在3D目标检测中的应用，研究了基于Waymo数据集训练的模型能否作为基础模型适应其他场景，并提出了场景导向提示池(SOP²)方法。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型(如GPT-3)展现出强大的泛化能力，通过微调和提示调优等迁移学习技术，它们能够以最小参数调整适应各种下游任务。本文旨在探索常见提示调优方法在3D目标检测中的有效性，研究基于大规模Waymo数据集训练的模型能否作为基础模型适应3D目标检测领域的其他场景。

Method: 本文依次研究了提示标记和提示生成器的影响，并进一步提出了场景导向提示池(SOP²)。该方法旨在探索提示池在3D目标检测中的有效性。

Result: 论文展示了提示池在3D目标检测中的有效性，证明了基于大规模数据集训练的模型可以通过提示调优技术适应其他3D检测场景。

Conclusion: 本文的研究成果旨在启发未来研究者深入探索提示技术在3D领域的潜力，为3D目标检测领域的迁移学习提供新的思路和方法。
...
TL;DR: MM-CoT是一个诊断性基准测试，专门设计用于评估多模态模型在思维链推理中的视觉基础和逻辑连贯性，要求模型选择同时满足视觉一致性和逻辑连贯性的事件链，而非生成自由形式的解释。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试强调生成但忽视验证，无法评估多模态模型思维链推理是否真正基于视觉证据且逻辑连贯。需要填补这一空白，探究模型推理是否真正基于视觉证据并具有逻辑有效性。

Method: 引入MM-CoT基准测试，要求模型选择唯一满足两个正交约束的事件链：1) 视觉一致性（所有步骤都基于可观察证据），2) 逻辑连贯性（具有因果和常识有效性）。设计对抗性干扰项来违反其中一个约束，以暴露不同的推理失败。

Result: 评估领先的视觉语言模型发现，即使最先进的系统也在MM-CoT上表现困难，揭示了生成流畅性与真实推理保真度之间的显著差异。MM-CoT与现有基准测试相关性低，确认其测量了视觉基础和逻辑推理的独特组合。

Conclusion: MM-CoT为开发未来模型提供了基础，这些模型不仅能够进行看似合理的推理，而且能够在视觉世界中忠实且连贯地进行推理，促进真正基于视觉证据和逻辑连贯的多模态推理能力发展。
...
TL;DR: 提出一种基于法线引导的稀疏深度采样策略，通过PCA表面法线估计计算像素深度可靠性，改进深度补全模型的训练数据生成
...
Method: 提出法线引导稀疏深度采样策略：1) 在RGB-D点云上进行PCA表面法线估计；2) 计算每个像素的深度可靠性度量；3) 根据可靠性分布采样稀疏深度；4) 与Marigold-DC扩散模型集成

Result: 在NYU Depth v2数据集上的实验表明：几何感知的稀疏深度提高了精度，减少了边缘和不连续处的伪影，产生了更真实的训练条件

Conclusion: 提出的采样方法能更好地反映真实传感器行为，改进深度补全性能，特别是在几何结构复杂的区域
...
### [14] [Pose-Based Sign Language Spotting via an End-to-End Encoder Architecture](https://arxiv.org/abs/2512.08738)
*Samuel Ebimobowei Johnny,Blessed Guda,Emmanuel Enejo Aaron,Assane Gueye*

Main category: cs.CV

TL;DR: 提出了一种基于姿态关键点的端到端手语检测模型，用于在连续手语句子中检测特定查询手势的存在与否，在WSLP 2025共享任务中达到61.88%准确率和60.00% F1分数。


<details>
  <summary>Details</summary>
Motivation: 手语识别对于连接聋哑人和听力人群至关重要，但现有研究主要关注整体识别，而手语到手势检索或在连续手势序列中检测特定手势的问题尚未充分探索。作者将这一新任务定义为"手语检测"，旨在直接检测查询手势是否出现在目标序列中。

Method: 提出基于姿态关键点的端到端模型，直接从手语视频中提取姿态关键点，使用仅编码器架构和二元分类头来判断查询手势是否出现在目标序列中。相比传统依赖中间手势识别或文本匹配的方法，该方法专注于姿态表示而非原始RGB帧，降低了计算成本并减少了视觉噪声。

Result: 在WSLP 2025共享任务的Word Presence Prediction数据集上评估，达到61.88%的准确率和60.00%的F1分数。结果表明基于姿态的框架在手语检测任务中具有有效性。

Conclusion: 该方法为手语检测提供了一种有效的基于姿态的解决方案，为未来自动手语检索和验证研究奠定了坚实基础。代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 当前相机BEV感知面临性能与部署可行性之间的根本矛盾：SOTA性能依赖于计算密集的视图变换和平台特定的定制内核，导致部署困难。

Method: 采用"算法快速"和"设计可部署"两大原则：1) 通过确定性预排序策略将单块投影分解为标准索引-收集-重塑流水线，使用原生算子；2) 集成端到端深度感知融合机制，结合时间聚合和数据增强。

Result: 在nuScenes基准测试中达到0.359 NDS的SOTA性能，在Tesla T4等车载硬件上实现超过134 FPS的实时性能，无需定制插件。

Conclusion: FastBEV++提供了一个成熟可扩展的设计理念，解决了BEV感知中性能与部署的矛盾，为生产级自动驾驶系统提供了高效准确的解决方案。
...
Method: 采用混合框架，通过双通道分离语义和外观：连续通道通过ViT补丁保留细粒度细节，离散通道使用MGVQ量化将符号锚点投影为四个标记，通过解耦注意力掩码和瓶颈融合成580标记的混合序列并压缩为单个voco标记。

Result: 在七个基准测试（GQA、VQAv2、MMBench、MME、POPE、SEED-Bench、ScienceQA-Image）上平均性能保持率达到87.2%，优于领先连续基线的81.0%，实现580:1压缩比。注意力分析显示压缩标记优先考虑离散锚点，验证了其语义指导作用。

Conclusion: 研究表明，极简混合设计能够解决效率-保真度困境，推动可扩展视觉语言模型的发展，证明了通过分离语义和外观信息可以实现高效且准确的视觉表示。
...
TL;DR: 提出了一种名为Residual-SwinCA-Net的新型深度混合分割框架，结合了残差CNN和Swin Transformer，用于乳腺超声图像病灶分割，在BUSI数据集上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 解决乳腺超声图像分割中的挑战，包括提取局部相关和鲁棒特征、学习全局依赖关系、增强组织连续性、抑制超声噪声、保持恶性病灶轮廓形态完整性等。

Method: 1. 结合残差CNN模块提取局部相关特征；2. 使用内部残差路径定制Swin Transformer块学习全局依赖；3. 应用拉普拉斯-高斯区域算子增强组织连续性和噪声抑制；4. 引入边界导向算子保持病灶轮廓完整性；5. 采用阶段式收缩策略捕获尺度不变性；6. 在解码器层集成多尺度通道注意力和挤压模块；7. 使用像素注意力模块自适应加权病灶像素。

Result: 在公开BUSI数据集上，Residual-SwinCA-Net实现了99.29%的平均准确率、98.74%的IoU和0.9041的Dice系数，优于现有CNN/ViT技术。

Conclusion: 提出的Residual-SwinCA-Net框架显著提高了乳腺超声病灶分割性能，增强了临床诊断决策的及时性和准确性。
...
TL;DR: 本文提出了一种查询感知的枢纽原型学习方法，通过构建查询-支持点二分图来生成与查询相关的原型，解决现有方法中原型偏差问题，提升少样本3D点云语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于度量的原型学习方法仅从支持集生成原型，未考虑与查询数据的相关性，导致原型偏差问题。当存在分布偏移时，原型过度拟合支持集特性而无法泛化到查询分布，从而降低分割性能。

Method: 提出查询感知枢纽原型学习方法，包含两个核心模块：1) 枢纽原型生成模块，构建查询-支持点二分图，识别频繁链接的支持枢纽，生成与查询相关的原型；2) 原型分布优化模块，使用纯度重加权对比损失，将坏枢纽和边界模糊原型拉近到对应类别中心。
...
TL;DR: RLCNet：一种用于LiDAR、RADAR和摄像头多模态传感器同时在线校准的端到端可训练深度学习框架，通过加权移动平均和异常值剔除实现实时动态调整，在真实数据集上验证了优越性能。
...
Result: 在真实世界数据集上验证，RLCNet设计用于实际部署，在不同条件下表现出鲁棒性能。消融研究突出了架构选择的重要性，与现有方法的比较表明所提方法具有优越的准确性和鲁棒性。

Conclusion: RLCNet为多模态传感器在线校准提供了一种有效的深度学习解决方案，通过实时动态调整机制提高了校准的准确性和鲁棒性，适用于自动驾驶车辆的实际部署需求。
...
<details>
  <summary>Details</summary>
Motivation: 从第三人称视角视频转换为第一人称视角视频能够为沉浸式理解开辟新可能性，但由于相机姿态变化极大且视角重叠极少，这一任务极具挑战性。需要保持可见内容的同时以几何一致的方式合成未观察到的区域。

Method: 提出EgoX框架，通过轻量级LoRA适配利用大规模视频扩散模型的预训练时空知识，采用宽度和通道级拼接的统一条件策略结合第三人称和第一人称先验，并引入几何引导的自注意力机制选择性地关注空间相关区域。

Result: 该方法实现了连贯且真实的第一人称视频生成，在未见过的和野外视频上展现出强大的可扩展性和鲁棒性。

Conclusion: EgoX框架成功解决了从第三人称到第一人称视频转换的挑战，通过创新的条件策略和几何引导机制实现了高质量的沉浸式视频生成。
...
Method: 采用四阶段流水线构建数据集：1)视频筛选（分辨率和美学过滤）；2)跨帧主题挖掘与配对（基于VLM的类别共识、局部定位和多样性感知配对）；3)身份保持参考图像合成（分割图引导的外绘和框引导的内绘，结合几何感知增强和边界侵蚀）；4)验证与标注（VLM验证样本，失败样本重新合成，构建长短描述）。

Result: 构建了包含250万样本和435万图像的OpenSubject数据集，建立了覆盖主题驱动生成和编辑的基准测试，评估了身份保真度、提示遵循度、编辑一致性和背景一致性。实验表明使用OpenSubject训练能显著提升生成和编辑性能，特别是在复杂场景中。
...
TL;DR: Terrain Diffusion：一种基于扩散模型的程序化地形生成新方法，结合了扩散模型的高保真度和传统程序化噪声（如Perlin噪声）的无缝无限扩展、种子一致性和恒定时间随机访问特性。


<details>
  <summary>Details</summary>
Motivation: 传统的程序化噪声函数（如Perlin噪声）虽然快速且无限，但在真实感和大尺度连贯性方面存在根本限制。需要一种AI时代的新方法，既能保持程序化噪声的优势，又能提供更高的真实感和连贯性。

Method: 1. 提出InfiniteDiffusion算法，实现无限生成；2. 使用分层堆叠的扩散模型，将行星级上下文与局部细节耦合；3. 采用紧凑的拉普拉斯编码稳定地球尺度动态范围的输出；4. 开源无限张量框架支持无界张量的恒定内存操作；5. 使用少步一致性蒸馏实现高效生成。

Result: Terrain Diffusion能够无缝、实时地合成无边界的景观，建立扩散模型作为程序化世界生成的实用基础，能够连贯、可控且无限制地合成整个行星。

Conclusion: 该研究将扩散模型确立为程序化世界生成的实用基础，成功结合了扩散模型的高保真度和传统程序化噪声的关键特性，实现了无限、连贯、可控的行星级地形生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有分布匹配方法局限于欧几里得空间，只能捕捉线性结构而忽略了真实数据的内在几何（如曲率）。高维数据通常位于低维流形上，因此数据集蒸馏应使蒸馏数据流形与原始数据流形对齐。

Method: 提出GeoDM框架，在欧几里得、双曲和球面流形的笛卡尔积空间中操作。引入可学习的曲率和权重参数以适应底层数据几何，同时设计最优传输损失来增强分布保真度。

Result: 理论分析表明，在乘积空间中进行几何感知分布匹配比欧几里得对应方法具有更小的泛化误差界。在标准基准测试上的大量实验证明，该算法优于最先进的数据蒸馏方法，并在各种单几何分布匹配策略中保持有效性。

Conclusion: GeoDM通过在多几何空间中操作，能够更好地捕捉数据的真实内在结构，为数据集蒸馏提供了更有效的几何感知方法。
...
<details>
  <summary>Details</summary>
Motivation: 牙齿标志点检测是现代临床正畸学的关键任务，对精准诊断、个性化治疗策略制定和治疗进展监测至关重要。然而，由于牙齿几何结构复杂且个体间差异显著，开发先进技术面临挑战。

Method: 通过组织3DTeethLand挑战赛，与国际医学图像计算和计算机辅助干预会议合作，征集针对口腔内3D扫描的牙齿标志点检测算法，并发布了首个公开数据集。

Result: 创建了首个用于3D牙齿标志点检测的公开数据集，为评估该任务的最先进方法提供了宝贵资源，并鼓励研究社区提供具有重要临床意义的方法学贡献。

Conclusion: 3DTeethLand挑战赛通过提供公开数据集和促进算法开发，推动了牙齿3D标志点检测技术的发展，这对临床牙科实践具有重要价值。
...
Conclusion: GeoDiffMM通过扩散模型和几何条件化，实现了结构一致的运动放大，有效解决了小位移下光子噪声与真实微运动的分离问题。
...
TL;DR: 该论文提出了一种基于四元数的低秩支持四元数矩阵机（LSQMM）用于彩色图像分类，通过四元数代数保留RGB通道间的内在耦合关系，并引入四元数核范数正则化促进低秩结构。


<details>
  <summary>Details</summary>
Motivation: 传统彩色图像分类方法将特征表示为实数域向量、矩阵或三阶张量，无法有效保留RGB通道间的内在耦合关系。受四元数数据建模在图像恢复和去噪任务中成功的启发，作者希望开发一种能更好处理彩色图像分类的方法。

Method: 提出低秩支持四元数矩阵机（LSQMM），将RGB通道视为纯四元数，通过四元数代数保留通道间的耦合关系。为促进强相关颜色通道产生的低秩结构，在铰链损失中添加四元数核范数正则化项。设计了基于ADMM的迭代算法来有效求解该四元数优化模型。

Result: 在多个彩色图像分类数据集上的实验结果表明，与支持向量机、支持矩阵机和支持张量机等最先进方法相比，所提出的分类方法在分类精度、鲁棒性和计算效率方面均表现出优势。

Conclusion: LSQMM通过四元数表示有效保留了彩色图像通道间的内在耦合关系，结合低秩正则化提升了分类性能，为彩色图像分类提供了一种新颖有效的解决方案。
...
TL;DR: 该研究系统分析了Glaze和Nightshade等图像保护机制，发现它们通过结构化、低熵的扰动在特征层面操作，而非引入全局语义漂移，解释了为什么保护信号视觉上微妙但可检测。
...
Result: 保护机制作为结构化、低熵的扰动运行，与底层图像内容紧密耦合；保护图像保留了内容驱动的特征组织，但带有保护特定的子结构；可检测性受扰动熵、空间部署和频率对齐的交互效应控制；Glaze和Nightshade沿主导图像对齐频率轴重新分配能量而非引入扩散噪声。
...
TL;DR: PointDico是一种新颖的3D表示学习模型，通过结合扩散模型和对比学习的优点，解决了现有方法在无序、密度不均的3D点云数据上的局限性，在多个基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督表示学习方法在3D数据上面临挑战，因为3D数据具有无序性和密度不均的特性。对比模型容易过拟合，而3D掩码自编码器难以处理无序点云。这促使研究者探索结合扩散模型和对比学习优点的解决方案。

Method: 提出PointDico模型，通过知识蒸馏将去噪生成建模和跨模态对比学习相结合，其中扩散模型作为对比模型的指导。采用分层金字塔条件生成器进行多尺度几何特征提取，并使用双通道设计有效整合局部和全局上下文信息。

Result: PointDico在3D表示学习方面取得了新的最先进性能：在ScanObjectNN上达到94.32%的准确率，在ShapeNetPart上达到86.5%的实例级mIoU。

Conclusion: PointDico成功地将扩散模型和对比学习相结合，有效解决了3D点云表示学习的挑战，为无序、密度不均的3D数据提供了强大的表示学习方法。
...
Conclusion: Bi^2MAC方法有效利用不同类型区域信息并智能分配计算资源，在遥感图像全色锐化任务中实现了性能与效率的良好平衡
...
<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯溅射的方法在渲染复杂反射场景时面临渲染速度和内存存储的瓶颈，需要一种既能保持高质量反射渲染又能提升效率的解决方案。

Method: 提出混合溅射机制，包括：1）反射烘焙高斯追踪，将视角相关反射信息烘焙到高斯基元内部；2）基于瓦片的高斯溅射渲染反射；3）将反射高斯基元与基础高斯基元在统一混合溅射框架中集成；4）引入流水线级加速和反射敏感高斯剪枝来优化性能。

Result: 在Ref-NeRF和NeRF-Casting的复杂反射场景上，HybridSplat实现了约7倍的渲染加速，使用的高斯基元数量比类似基于光线追踪的高斯溅射基线少4倍，同时保持反射渲染质量。

Conclusion: HybridSplat通过创新的混合溅射机制，在复杂反射场景的渲染中实现了速度、内存和质量的平衡，成为该领域的新SOTA方法。
...
TL;DR: 提出TrackingWorld系统，通过跟踪上采样器将稀疏2D轨迹提升为密集2D轨迹，并优化估计相机位姿和3D坐标，实现世界坐标系下的密集3D跟踪


<details>
  <summary>Details</summary>
Motivation: 现有单目3D跟踪方法存在两个主要局限：1) 难以分离相机运动和前景动态运动；2) 无法密集跟踪视频中新出现的动态目标。需要一种能在世界坐标系下跟踪几乎所有像素的解决方案

Method: 1) 跟踪上采样器：将任意稀疏2D轨迹高效提升为密集2D轨迹；2) 泛化到新出现物体：将上采样器应用于所有帧，通过消除重叠区域轨迹减少2D轨迹冗余；3) 优化框架：通过估计相机位姿和2D轨迹的3D坐标，将密集2D轨迹反投影到世界中心3D轨迹

Result: 在合成和真实世界数据集上的广泛评估表明，该系统在世界坐标系下实现了准确且密集的3D跟踪

Conclusion: TrackingWorld解决了现有方法的局限性，能够分离相机运动和前景动态运动，并能密集跟踪新出现的动态目标，为单目3D跟踪提供了有效的世界中心解决方案
...
TL;DR: 提出SCU-CGAN模型，通过集成U-Net、CBAM和额外判别器，从非火图像生成逼真火图像，解决火灾数据集不足问题，显著提升火灾检测模型性能。
...
Method: 提出SCU-CGAN模型，集成U-Net架构、CBAM注意力机制和额外判别器，从非火图像生成逼真的火图像。通过评估生成图像质量，验证模型优于现有方法。

Result: SCU-CGAN在KID分数上比CycleGAN提升41.5%，生成图像质量更优。使用增强数据集显著提升火灾检测模型精度，YOLOv5 nano模型的mAP@0.5:0.95指标提升56.5%，且无需改变模型结构。
...
TL;DR: 该论文发现多模态大语言模型中视觉和文本token的norm差异导致不对称更新动态，提出通过添加LayerNorm层实现norm对齐，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型普遍采用Pre-Norm架构，导致视觉token的norm远高于文本token，这种norm差异引发"不对称更新动态"，使视觉token语义转换速度远慢于文本token，严重影响跨模态特征融合效果。

Method: 通过理论分析证明norm差异导致的不对称更新动态问题，提出在视觉投影器后添加一个精心初始化的LayerNorm层，强制实现视觉和文本token的norm对齐，消除更新速率差异。

Result: 在LLaVA-1.5架构上的实验表明，该方法不仅显著提升了多模态基准测试性能，还在纯文本评估（如MMLU）上获得明显改进，说明解决架构不平衡能全面提升模型能力。

Conclusion: 多模态大语言模型中视觉和文本token的norm差异是一个关键但被忽视的问题，通过简单的LayerNorm层插入实现norm对齐，能有效解决不对称更新动态，显著提升模型跨模态融合能力和整体性能。
...
<details>
  <summary>Details</summary>
Motivation: 在挑战性光照条件下捕获的图像通常存在各种退化问题，导致基于视觉的应用性能下降。现有方法要么显著放大固有噪声，要么仅在特定光照条件下有效，需要一种能在复杂光照条件下同时处理增强和噪声抑制的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 面部美化处理在社交媒体和广告中广泛应用，但当美化图像作为生物识别样本使用时，会对人脸识别系统构成挑战。先前研究已证明面部美化对人脸识别系统的影响，因此检测面部美化变得越来越必要。

Method: 研究分析美化图像的美感评估算法变化，评估基于人工智能的不同特征提取方法以改进美化检测，并探索是否可以利用面部美感来提升检测率。

Result: 在攻击美化算法未知的场景下，该研究在单图像检测上实现了1.1%的D-EER（等错误率）。

Conclusion: 面部美化检测对于保护生物识别系统的安全性至关重要，通过分析美感评估算法变化可以有效检测美化处理，即使在未知攻击算法的情况下也能实现较低的等错误率。
...
TL;DR: 本文开发了优化的深度学习流水线，用于自动鱼类重识别，在模拟电子监控系统的AutoFish数据集上，通过硬三元组挖掘和定制图像变换，Swin-T架构优于ResNet-50，达到90.43%的Rank-1准确率。


<details>
  <summary>Details</summary>
Motivation: 随着电子监控系统在渔业中的广泛应用，产生了大量视频数据，人工审查不可行。需要开发自动化的鱼类重识别技术来有效管理海洋资源。

Method: 使用AutoFish数据集（模拟电子监控系统的传送带场景，包含6种相似鱼类），采用优化的深度学习流水线，结合硬三元组挖掘和定制图像变换（包括数据集特定的归一化），比较了Vision Transformer的Swin-T架构和CNN的ResNet-50。

Result: Swin-T架构在关键重识别指标上显著优于ResNet-50，达到41.65%的mAP@k和90.43%的Rank-1准确率。分析发现主要挑战在于区分同一物种内视觉相似的个体，其中视角不一致比部分遮挡影响更大。

Conclusion: 通过硬三元组挖掘和定制图像变换的优化深度学习流水线，能够有效实现自动鱼类重识别，为渔业电子监控系统提供可行的自动化解决方案。Vision Transformer架构在鱼类重识别任务上表现出优于传统CNN的性能。
...
Method: 首先使用可提示视频分割模型生成身份一致的掩码片段，然后通过遮挡感知模块恢复缺失区域，最后用精炼的掩码片段指导SAM 3D Body生成一致的全身体网格轨迹

Result: 实验结果表明SAM-Body4D在具有挑战性的野外视频中实现了更好的时间稳定性和鲁棒性，且无需任何重新训练

Conclusion: SAM-Body4D通过利用视频中的人体连续性，解决了现有HMR方法的时间不一致和遮挡敏感问题，提供了一个高效且无需训练的视频人体网格恢复解决方案
...
### [39] [ContextDrag: Precise Drag-Based Image Editing via Context-Preserving Token Injection and Position-Consistent Attention](https://arxiv.org/abs/2512.08477)
*Huiguo He,Pengyu Yan,Ziqi Yi,Weizhi Zhong,Zheng Liu,Yejun Tang,Huan Yang,Kun Gai,Guanbin Li,Lianwen Jin*

Main category: cs.CV

TL;DR: ContextDrag是一种基于拖拽的图像编辑新范式，通过利用编辑模型的强大上下文建模能力，结合参考图像的VAE编码特征，实现了更连贯、保真度更高的编辑效果。
...
Method: 1. 引入Context-preserving Token Injection (CTI)，通过Latent-space Reverse Mapping (LRM)算法将无噪声的参考特征注入到正确目标位置；2. 采用Position-Consistent Attention (PCA)，对参考标记进行位置重新编码，并应用重叠感知掩码以消除不相关参考特征的干扰。

Result: 在DragBench-SR和DragBench-DR上的大量实验表明，该方法超越了所有现有的SOTA方法。

Conclusion: ContextDrag通过有效利用参考图像的上下文信息，实现了更精确的拖拽控制和语义/纹理细节的一致性保持，无需微调或反转，显著提升了基于拖拽图像编辑的质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型（MLLMs）由于内存开销过大，只能处理有限帧数的视频，无法有效理解长视频内容。需要一种既能保持知识完整性又能保证语义连贯性的高效长视频理解方法。

Method: 提出OneClip-RAG范式：1）利用视频片段进行检索增强，兼顾知识完整性和语义连贯性；2）设计查询引导的视频分块算法，将片段分块和跨模态检索统一在一个处理步骤中；3）创建SynLongVideo数据集并采用渐进训练策略提升指令跟随能力。

Result: 实验表明：1）在五个最新MLLMs上集成OneClip-RAG后，在长视频基准测试中性能显著提升，如将InternLV2 8B和Qwen2-VL 7B提升到GPT-4o水平；2）效率优势明显，如LLaVA-Video能在单张4090 GPU上2.2分钟内理解长达1小时的视频。

Conclusion: OneClip-RAG是一种有效且高效的长视频理解范式，通过创新的检索增强机制和优化算法，解决了MLLMs处理长视频的内存限制问题，在性能和效率方面均表现出色。
...
TL;DR: 提出了一种基于深度图的6D位姿估计方法，通过多视角深度图融合和稀疏TSDF表示，结合场景自适应注意力机制和密度感知稀疏Transformer，在密集工业拣选场景中实现高精度位姿估计。


<details>
  <summary>Details</summary>
Motivation: 工业拣选环境中密集堆叠、遮挡、反光和无纹理物体导致6D位姿估计困难，现有方法难以处理这些挑战，需要开发能够处理复杂场景的深度学习方法。

Method: 1. 多视角深度图融合为精细3D点云或稀疏TSDF；2. 分阶段热图机制提供场景自适应注意力先验；3. 密度感知稀疏Transformer块处理自遮挡和非均匀3D数据分布；4. 全稀疏框架实现高分辨率体素表示；5. 基于体素投票策略预测6D位姿。

Result: 在IPD和MV-YCB多视角数据集上验证，在密集工业拣选和家庭拣选场景中表现出竞争性性能，能够同时预测任意数量目标物体的6D位姿。

Conclusion: 该方法通过全稀疏框架和场景自适应注意力机制，有效解决了密集工业拣选环境中的6D位姿估计问题，为机器人近距离应用提供了新的解决方案。
...
TL;DR: ReasonBreak是一个针对多模态大推理模型的对抗性隐私保护框架，通过概念感知扰动破坏地理定位推理，相比现有方法显著提升隐私保护效果


<details>
  <summary>Details</summary>
Motivation: 多模态大推理模型（MLRMs）通过分层思维链推理能够从个人图像中推断精确地理位置，现有隐私保护技术主要针对感知模型，无法有效对抗MLRMs的复杂多步推理过程

Method: 提出ReasonBreak框架，基于概念层次对齐的扰动而非均匀噪声，战略性地针对推理链中的关键概念依赖关系，生成使特定推理步骤失效的扰动，并在后续推理阶段级联传播

Result: 在7个最先进的MLRMs（包括GPT-o3、GPT-5、Gemini 2.5 Pro）上评估，ReasonBreak在区域级保护上提升14.4%（33.8% vs 19.4%），在街区级保护上几乎翻倍（33.5% vs 16.8%）

Conclusion: ReasonBreak建立了针对基于推理的隐私威胁的新范式，通过概念感知扰动有效破坏MLRMs的分层推理能力
...
TL;DR: LapFM是一个用于腹腔镜手术分割的基础模型，通过分层概念演化预训练范式，利用大规模无标注手术图像构建了包含11.4万图像-掩码对的LapBench-114K基准，显著提升了跨手术目标的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前手术分割面临标注稀缺和跨不同手术语义不一致的挑战。现有方法通常只是对自然基础模型进行微调，仅作为领域适配器而非真正的手术基础模型，难以泛化到多样化的手术目标。

Method: 提出分层概念演化预训练范式：1）通过分层掩码解码器和父子查询嵌入建立腹腔镜概念层次结构，统一不同实体（解剖结构、组织、器械）；2）提出置信度驱动的演化标注，基于分层一致性迭代生成和过滤伪标签，逐步将可靠样本纳入训练。

Result: 构建了包含11.4万图像-掩码对的大规模基准LapBench-114K。实验表明LapFM显著优于现有最先进方法，在通用腹腔镜分割中建立了粒度自适应泛化的新标准。

Conclusion: LapFM是一个专门设计的手术基础模型，通过分层概念演化预训练有效解决了手术分割中的标注稀缺和语义不一致问题，实现了跨手术目标的强大泛化能力。
...
TL;DR: 提出统一学习框架，联合利用高分辨率RGB和低分辨率多光谱传感器进行端到端色彩校正，相比RGB-only和多光谱基线方法减少50%误差。


<details>
  <summary>Details</summary>
Motivation: 现有快照多光谱成像系统虽然能增强色彩校正等任务，但大多数方法将色彩校正流程分为多个阶段，往往在早期就丢弃多光谱数据，无法充分利用多光谱信息。

Method: 提出统一学习框架：1) 端到端色彩校正；2) 联合利用高分辨率RGB传感器和辅助低分辨率多光谱传感器数据；3) 将整个流程集成在单一模型中；4) 重构两种最先进的图像到图像架构来展示框架灵活性。

Result: 通过聚合和重新利用公开光谱数据集构建专用数据集，在多种RGB相机灵敏度下渲染。实验表明该方法显著提高色彩准确性和稳定性，相比RGB-only和多光谱基线方法减少高达50%的误差。

Conclusion: 提出的统一学习框架能够有效利用多光谱信息进行端到端色彩校正，在色彩准确性和稳定性方面显著优于现有方法，展示了框架的灵活性和通用性。
...
TL;DR: 该研究分析了基于子集选择的方法在分布外（OOD）场景下的局限性，并提出了一种结合子模子集选择和分层梯度不确定性估计的框架，以提高解释的鲁棒性和保真度。


<details>
  <summary>Details</summary>
Motivation: 基于子集选择的方法在解释深度视觉模型时被广泛使用，通过突出最有影响力的图像区域来支持对象级解释。然而，这些方法在分布内（ID）设置下表现良好，但在分布外（OOD）条件下的行为仍未被充分理解。研究发现现有方法在OOD场景下可靠性显著下降，产生冗余、不稳定且对不确定性敏感的解释。

Method: 提出了一个结合子模子集选择和分层梯度不确定性估计的框架。该方法通过自适应权重扰动估计不确定性，并使用这些估计来指导子模优化，确保多样化和信息丰富的子集选择。该框架不需要额外的训练或辅助模型。

Result: 实证评估表明，该框架不仅缓解了现有方法在OOD场景下的弱点，还在ID设置中带来了改进。研究结果突显了当前基于子集方法的局限性，并展示了不确定性驱动优化如何增强归因和对象级可解释性。

Conclusion: 该研究为现实世界视觉应用中更透明和可信赖的AI铺平了道路，通过不确定性驱动的优化提高了基于子集选择方法的鲁棒性和解释质量，特别是在分布外场景下。
...
TL;DR: 提出了一种自动化方法，通过多摄像头系统解决手术视频拍摄中的遮挡问题，自动对齐视角并选择最佳视角生成连续的手术视频。


<details>
  <summary>Details</summary>
Motivation: 手术视频对教育和研究很重要，但拍摄时外科医生经常遮挡摄像头视野。现有方法使用多个摄像头但需要手动对齐，且每次调整无影灯位置后都需要重新对齐，非常耗时。

Method: 提出全自动对齐方法：识别无影灯移动的帧，重新对齐这些帧，选择遮挡最少的摄像头视角，生成固定视角的连续手术视频。还实现了多种合成选项供用户选择。

Result: 外科医生用户研究表明，该方法生成的视频在确认手术区域和观看舒适度方面优于传统方法。视频质量也有提升。用户研究还评估了不同合成选项的偏好。

Conclusion: 该方法成功实现了手术视频拍摄的自动化对齐，提高了视频质量和观看体验，为手术教育和研究提供了更好的视频素材。
...
TL;DR: 提出一种轻量级SAM+CSRT足球运动员跟踪方法，结合SAM精确初始化、CSRT跟踪器和球衣颜色外观模型，在资源受限条件下实现足球场景下的稳定跟踪。
...
Method: 基于SAM的团队感知跟踪系统：使用SAM进行精确初始化，结合CSRT跟踪器和HSV直方图球衣颜色外观模型，通过外观重识别改进遮挡恢复能力。

Result: 在足球视频序列上达到7.6-7.7 FPS，内存使用稳定(~1880 MB)；轻度遮挡下100%跟踪成功率，拥挤禁区场景(≥5名球员)90%成功率；外观重识别能恢复50%重度遮挡，但对长时间离帧的再获取成功率仅8.66%。

Conclusion: SAM+CSRT组合在连续可见性场景表现稳定，但需要更强的重识别机制处理长时间离帧情况；该方法为资源受限下的足球跟踪系统部署提供了实用指导。
...
TL;DR: CLIP模型在少样本学习中存在模板-样本相似性偏差，导致模型依赖模板接近度而非真实样本-类别对齐。本文提出使用空提示捕获无偏模板特征来抵消这种偏差，通过预训练和微调两阶段框架提升分类准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 研究发现CLIP模型中的模板-样本相似性(TSS)会引入偏差，使模型过度依赖文本模板与图像样本的相似性，而非真正的样本-类别对齐关系，这降低了少样本分类的准确性和鲁棒性。

Method: 提出两阶段框架：1) 预训练阶段使用空提示（不含类别信息的文本输入）揭示并减少CLIP编码器中的模板诱导偏差；2) 少样本微调阶段通过偏差校准损失强制图像与其类别正确对齐，确保模型关注相关视觉线索。

Result: 在多个基准测试上的实验表明，该模板校正方法显著减少了TSS引起的性能波动，获得了更高的分类准确率和更强的鲁棒性。

Conclusion: 通过使用空提示来解耦CLIP中的模板偏差，能够有效提升模型在少样本学习中的性能，使模型更关注真实的样本-类别对齐关系而非模板相似性。
...
TL;DR: 本文提出了一种仅使用单目RGB视觉和自然语言指令的无人机视觉语言导航统一框架，通过多任务学习联合优化空间感知、轨迹推理和动作预测，显著提升了轻量级无人机的实际部署能力。


<details>
  <summary>Details</summary>
Motivation: 现有的空中视觉语言导航方法通常依赖全景图像、深度输入或里程计来支持空间推理和动作规划，这些要求增加了系统成本和集成复杂性，阻碍了轻量级无人机的实际部署。本文旨在开发一个仅使用单目RGB视觉和自然语言指令的统一框架。

Method: 1. 将导航任务建模为下一个令牌预测问题；2. 通过提示引导的多任务学习联合优化空间感知、轨迹推理和动作预测；3. 提出关键帧选择策略以减少视觉冗余；4. 引入动作合并和标签重加权机制来缓解长尾监督不平衡并促进稳定的多任务协同训练。
...
Conclusion: 本文提出的统一空中VLN框架仅使用单目RGB视觉和自然语言指令，通过创新的多任务学习架构和训练策略，实现了高效的无人机导航，为轻量级无人机的实际应用提供了可行的解决方案。
...
TL;DR: PCI框架通过分析概念插入成功率来研究扩散模型中概念形成的动态时间过程，揭示不同概念在不同时间步的锁定时机，为图像编辑提供优化时机


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注扩散模型的最终输出，但生成过程是动态的轨迹，理解这一动态过程对于掌握模型的可控性、可靠性和可预测性至关重要。需要研究噪声何时转化为特定概念并在去噪轨迹中锁定

Method: 提出PCI（Prompt-Conditioned Intervention）框架，无需训练且模型无关。核心是分析概念插入成功率（CIS），即在给定时间步插入的概念在最终图像中被保留和反映的概率，以此表征概念形成的时间动态

Result: 应用于多个最先进的文本到图像扩散模型和广泛的概念分类，PCI揭示了不同扩散模型中的多样化时间行为，某些轨迹阶段对特定概念更有利，即使在同一概念类型内也存在差异
...
<details>
  <summary>Details</summary>
Motivation: 肤色一直是歧视的焦点，但医学影像中的机器学习公平性研究通常依赖于粗糙的亚组分类，忽略了个体层面的差异。这种基于群体的方法可能会掩盖亚组内异常值面临的偏见。
...
TL;DR: 首个针对多相机阵列的实时3D高斯泼溅重建框架，通过分层相机初始化、轻量级多相机束调整、无冗余高斯采样和频率感知优化调度，在2分钟内重建数百米3D场景
...
Method: 1) 分层相机初始化方案：无需标定实现粗略的相机间对齐；2) 轻量级多相机束调整：稳定轨迹同时保持实时性能；3) 无冗余高斯采样策略：减少高斯基元数量；4) 频率感知优化调度器：减少所需优化迭代次数

Result: 仅使用原始多相机视频流，在2分钟内重建数百米3D场景，展示了前所未有的速度、鲁棒性和重建保真度，实现了漂移自由的轨迹估计和高效的在线重建

Conclusion: 该方法首次实现了多相机阵列的实时3D高斯泼溅重建，通过创新的分层初始化、束调整和优化策略，解决了单目方法视野受限的问题，为大规模场景的实时重建提供了高效解决方案
...
TL;DR: 提出自精炼扩散框架，通过XAI技术检测生成图像中的伪影和不真实区域，利用缺陷激活图在扩散过程中针对性修复，显著提升图像质量
...
Method: 提出自精炼扩散框架，使用基于可解释人工智能的缺陷高亮器生成缺陷激活图，识别伪影和不真实区域。在正向过程中放大缺陷区域的噪声，在反向过程中重点关注这些区域，从而改善重建质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前条件扩散模型存在错位和幻觉问题，需要在生成后使用自动错位检测工具（如Best-of-N方法），但这需要等待完整生成过程，计算成本高昂。本文假设文本/图像错位可以在去噪过程早期检测，实现实时对齐评估。

Method: 提出NoisyCLIP方法，在噪声潜在空间中测量语义对齐。这是首次探索和基准测试在反向扩散过程中使用双编码器进行提示到潜在错位检测的方法。

Result: NoisyCLIP在Best-of-N设置中将计算成本降低50%，同时达到98%的CLIP对齐性能。该方法实现了生成过程中的实时对齐评估，在不牺牲语义保真度的前提下降低成本。

Conclusion: NoisyCLIP通过在去噪过程的潜在空间中早期检测对齐问题，提供了一种高效的实时对齐评估方法，显著降低了计算成本，同时保持了高质量的对齐性能。
...
TL;DR: OCCDiff：一种在占用函数空间应用潜在扩散的建筑物重建方法，通过函数自编码器和潜在扩散过程生成连续占用函数，对噪声数据具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 从LiDAR点云重建建筑物时面临的主要挑战在于：在不同点密度和噪声干扰下准确捕捉建筑表面。需要灵活获取不同分辨率下的高质量3D建筑轮廓。

Method: 提出OCCDiff方法，在占用函数空间应用潜在扩散。结合潜在扩散过程和函数自编码器架构生成可在任意位置评估的连续占用函数。提出点编码器为扩散学习提供条件特征，约束最终占用预测，并为潜在编码器插入多模态特征。采用多任务训练策略增强模型性能。

Result: 实验结果表明，该方法生成物理一致的样本，对目标分布具有高保真度，并对噪声数据表现出鲁棒性。

Conclusion: OCCDiff通过潜在扩散和函数自编码器的结合，有效解决了LiDAR点云建筑物重建中的密度变化和噪声干扰问题，实现了高质量、鲁棒的3D建筑轮廓生成。
...
<details>
  <summary>Details</summary>
Motivation: 文档阴影去除对数字化文档清晰度至关重要，阴影常常会模糊或扭曲文本边缘等高频细节结构，需要有效去除阴影的同时保持这些精细结构。

Method: 提出MatteViT框架，包含两个关键策略：1) 轻量级高频放大模块(HFAM)，分解并自适应放大高频分量；2) 基于连续亮度的阴影蒙版，使用定制数据集和蒙版生成器提供精确空间指导。

Result: 在RDD和Kligler等公开基准测试中达到最先进性能，在光学字符识别等下游任务中能更好地保留文本级细节，提升识别性能。

Conclusion: MatteViT为真实世界文档阴影去除提供了鲁棒实用的解决方案，通过空间和频域信息的结合有效去除阴影并保留精细结构细节。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在领域变化时性能下降，或需要大量计算资源进行微调。为了解决这些问题，需要开发更高效、鲁棒的适应方法。

Method: 提出了训练免费的双曲适配器T-DHA，在双曲空间（使用庞加莱球模型）而非传统欧几里得空间中建模视觉-语言关系，利用双曲空间的指数体积增长特性更好地嵌入层次化数据结构，并结合负学习提高分类准确性和鲁棒性。

Result: 在多个数据集上的实验结果表明，T-DHA方法在少样本图像识别和领域泛化任务中显著优于现有的最先进方法。

Conclusion: 双曲空间建模结合负学习的T-DHA方法为视觉语言模型提供了一种高效、鲁棒的适应策略，在减少特征维度的同时提高了分类性能。
...
TL;DR: MVP框架通过多视角推理提升GUI坐标预测稳定性，无需训练即可显著提高各种模型的性能


<details>
  <summary>Details</summary>
Motivation: 现有GUI坐标预测模型存在严重的不稳定性问题，微小的视觉扰动（如裁剪几个像素）就会导致预测结果在正确和错误之间剧烈变化，这严重影响了模型性能，特别是在高分辨率和小UI元素的情况下
...
Conclusion: MVP框架通过多视角推理有效解决了GUI坐标预测的不稳定性问题，显著提升了各种模型的性能，且无需额外训练，代码已开源
...
TL;DR: InfiniteVL是一种线性复杂度的视觉语言模型架构，结合滑动窗口注意力和Gated DeltaNet，在资源受限下实现竞争性多模态性能，训练数据不到主流VLM的2%，推理速度提升3.6倍，保持恒定延迟和内存占用。
...
Method: 提出InfiniteVL架构，将滑动窗口注意力与Gated DeltaNet结合；设计三阶段训练策略：蒸馏预训练、指令微调和长序列SFT。

Result: 使用不到2%的训练数据，不仅大幅超越先前线性复杂度VLM，还匹配领先Transformer VLM性能，保持有效长期记忆；相比FlashAttention-2加速的Transformer VLM，推理速度提升3.6倍以上，延迟和内存占用恒定；在流式视频理解中保持24 FPS实时预填充速度。

Conclusion: InfiniteVL成功解决了窗口注意力和线性注意力的局限性，实现了高效、高性能的线性复杂度视觉语言模型，在资源受限下具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 油画作为融合人类抽象思维与艺术表达的高级媒介，其复杂的笔触动态和风格化特征给数字生成与编辑带来巨大挑战。现有技术受限于训练数据分布，主要关注真实照片的修改，缺乏专门的油画创作解决方案

Method: 1. 增强训练阶段：采用空间对齐和语义增强条件策略，将掩码和草图映射为空间约束，从参考图像和文本编码上下文嵌入作为特征约束，实现对象级语义对齐
2. 自监督风格转换：基于笔触渲染(SBR)构建自监督风格转换管道，模拟油画修复的修复动态，将真实图像转换为保留笔触纹理的风格化油画，构建大规模配对训练数据集
3. 推理阶段：使用AdaIN算子集成特征以确保风格一致性

Result: 广泛的实验表明，该交互式系统能够在保持油画艺术品质的同时实现细粒度编辑，在风格化油画生成与编辑中达到了前所未有的想象力实现水平

Conclusion: 提出的统一多模态框架成功解决了油画数字生成与编辑的挑战，通过空间对齐、语义增强和自监督风格转换等技术创新，实现了交互式油画创作，为艺术创作提供了新的可能性
...
TL;DR: Photo3D是一个通过GPT-4o-Image生成的图像数据驱动，提升3D生成真实感的框架，通过结构对齐的多视角合成和细节增强技术，在保持几何结构一致性的同时提升纹理细节的真实性。


<details>
  <summary>Details</summary>
Motivation: 现有3D原生生成器在几何生成方面取得了进展，但在实现真实外观方面仍不足。主要障碍在于缺乏多样化和高质量的具有丰富纹理细节的真实世界3D资产，因为由于场景尺度多样、物体非刚性运动以及3D扫描仪精度有限，捕获此类数据本质上是困难的。

Method: 1. 设计结构对齐的多视角合成流程，构建细节增强的多视角数据集与3D几何配对；2. 提出真实细节增强方案，利用感知特征适应和语义结构匹配，在保持与3D原生几何结构一致性的同时增强外观真实细节；3. 针对几何-纹理耦合和解耦的3D原生生成范式，提供专门的训练策略。

Result: 实验表明，Photo3D能够很好地泛化到不同的3D原生生成范式，并实现了最先进的真实感3D生成性能。

Conclusion: Photo3D框架通过利用GPT-4o-Image生成的图像数据，结合结构对齐的多视角合成和细节增强技术，成功解决了3D生成中真实感不足的问题，为不同3D原生生成范式提供了通用的真实感增强解决方案。
...
### [62] [No Labels, No Problem: Training Visual Reasoners with Multimodal Verifiers](https://arxiv.org/abs/2512.08889)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉推理方法存在局限：语言链式思维方法需要大规模监督数据，程序合成方法存在逻辑缺陷和定位错误。需要一种无需标注的方法来同时提升推理和定位能力。

Method: 提出无需标注的训练框架，使用AI验证器：LLM验证器通过强化学习优化推理，VLM验证器通过自动硬负例挖掘增强视觉定位，无需真实标签监督。

Result: 在多种空间推理任务上评估，方法提升了视觉推理能力，超越开源和专有模型，改进的视觉定位模型进一步优于最近的纯文本视觉推理方法。

Conclusion: VALOR框架成功结合了现代AI系统的优势：先进的语言推理模型用于分解空间查询，以及通过高性能VLM批评器改进的视觉专家模型，实现了无需标注的视觉推理和定位提升。
...
TL;DR: Fast-ARDiff提出了一种统一的AR-扩散混合框架，通过熵感知的推测解码和动态调度策略，同时加速AR和扩散组件，实现高效图像生成。
...
Method: 1) 熵感知推测策略：鼓励草稿模型产生与目标模型熵特性对齐的高熵表示，缓解熵不匹配和因草稿过度自信导致的高拒绝率。2) 动态调度器：将扩散解码集成到端到端框架中，优先优化AR部分来指导后续扩散步骤。3) 联合蒸馏框架：结合轨迹匹配和分布匹配，确保稳定训练和极少数步骤下的高质量合成。4) 推理时使用AR模块的浅层特征熵预过滤低熵草稿，避免冗余计算。

Result: 在ImageNet 256×256上，TransDiff实现了4.3倍无损加速；在文本条件生成任务中，NextStep-1实现了3倍加速，达到了最先进的加速性能。

Conclusion: Fast-ARDiff通过联合优化AR和扩散组件，有效解决了AR-扩散混合范式的延迟瓶颈，实现了高效且高质量的图像生成，为混合生成模型提供了实用的加速框架。
...
TL;DR: Astra是一个交互式通用世界模型，能够为自动驾驶、机器人抓取等多样化场景生成真实世界的未来预测，支持精确的动作交互控制，实现了交互式、一致性和通用的长期视频预测。
...
Conclusion: Astra实现了交互式、一致性和通用的长期视频预测，支持各种形式的交互，为通用世界模型的发展提供了新的解决方案。
...
TL;DR: 提出一种无需迭代的固定点估计器用于扩散反演，通过误差近似方法避免固定点迭代的高计算成本和超参数选择问题，在重建任务中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 扩散反演中的固定点迭代方法虽然能最小化重建误差，但存在计算成本高和超参数选择复杂的问题。需要一种更高效、无需迭代的反演方法。

Method: 1) 从理想反演步骤推导固定点的显式表达式；2) 引入误差近似，使用前一步可计算的误差来近似当前步的未知误差；3) 得到可计算的近似固定点表达式，这是一个无偏且低方差的估计器。

Result: 在NOCAPS和MS-COCO两个文本-图像数据集上的评估显示，该方法在重建任务中表现一致且优于DDIM反演和其他基于固定点迭代的方法，无需额外迭代或训练。

Conclusion: 提出的迭代自由固定点估计器有效解决了扩散反演中固定点迭代方法的高计算成本和超参数选择问题，实现了高效且准确的重建性能。
...
TL;DR: 该研究提出了一个大规模自动化框架，用于发现和解释人类大脑皮层中的视觉表征，通过无监督数据分解发现模式，然后识别最能激发这些模式的图像并生成自然语言描述。


<details>
  <summary>Details</summary>
Motivation: 理解人类大脑如何表示视觉概念以及在哪些脑区编码这些表征是一个长期挑战。现有研究通常规模小、依赖人工检查、关注特定区域和属性，且缺乏系统验证，需要更系统化的大规模方法。

Method: 采用两阶段方法：1）通过无监督数据驱动分解方法在fMRI活动中发现可解释模式；2）通过识别最能激发每个模式的自然图像集，并生成描述其共享视觉意义的自然语言描述来解释每个模式。引入自动化管道测试多个候选解释，分配定量可靠性分数，并为每个体素模式选择最一致的描述。

Result: 该框架揭示了数千个可解释模式，涵盖许多不同的视觉概念，包括以前未报告的细粒度表征。

Conclusion: 该研究提供了一个大规模、自动化的框架，能够系统地发现和解释人类大脑皮层中的视觉表征，克服了传统方法的局限性，揭示了丰富的视觉概念表征。
...
TL;DR: 本文提出了一种用于连续测试时自适应（CTTA）的新方法，通过自适应调整伪标签来反映每张图像内的置信度分布，并动态平衡受域偏移影响最大的类别的学习，从而在语义分割任务中实现更可靠的自适应。


<details>
  <summary>Details</summary>
Motivation: 现有CTTA方法通常依赖固定或批量级阈值，无法考虑不同类别和实例之间的难度差异，这在需要密集多类预测的语义分割中尤其成问题。需要一种能够自适应调整伪标签并平衡类别学习的细粒度方法。

Method: 提出了一种自适应方法，根据每张图像内的置信度分布动态调整伪标签，并针对受域偏移影响最大的类别进行动态平衡学习。该方法实现了细粒度的、类别和实例感知的自适应。

Result: 在八个CTTA和TTA场景（包括合成到真实和长期偏移）上的广泛实验表明，该方法始终优于最先进的技术，为演化条件下的语义分割设定了新标准。

Conclusion: 通过自适应调整伪标签和动态平衡类别学习，该方法能够产生更可靠的监督并减轻连续自适应过程中的错误累积，在语义分割的连续测试时自适应中表现出色。
...
### [68] [Automated Pollen Recognition in Optical and Holographic Microscopy Images](https://arxiv.org/abs/2512.08589)
*Swarn Singh Warshaneyan,Maksims Ivanovs,Blaž Cugmas,Inese Bērziņa,Laura Goldberga,Mindaugas Tamosiunas,Roberts Kadiķis*

Main category: cs.CV

TL;DR: 该研究应用深度学习改进和自动化花粉粒在光学和全息显微图像中的检测与分类，特别关注兽医细胞学应用。使用YOLOv8s进行目标检测，MobileNetV3L进行分类，在光学图像上取得良好性能，但全息图像性能较差。通过数据集扩展和边界框区域扩大技术显著提升了全息图像的性能。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决花粉粒在兽医细胞学中的自动化检测和分类问题，探索深度学习在光学和全息显微图像中的应用，特别是针对成本效益高的无透镜数字全息显微镜设备。

Method: 采用YOLOv8s进行目标检测，MobileNetV3L进行分类任务。针对全息图像性能差距问题，使用自动化标注和边界框区域扩大的数据集扩展技术来提升模型性能。

Result: 光学图像上检测达到91.3% mAP50，分类达到97%整体准确率。全息图像初始性能较低（检测2.49% mAP50，分类42%准确率），通过技术改进后提升到检测13.3% mAP50，分类54%准确率。

Conclusion: 研究表明，至少在图像分类任务中，可以将深度学习技术与成本效益高的无透镜数字全息显微镜设备相结合，为兽医细胞学中的花粉粒分析提供了可行的自动化解决方案。
...
Conclusion: OpenMonoGS-SLAM成功展示了结合3D高斯溅射与开放集语义理解的可行性，为开放世界环境下的单目SLAM提供了新的解决方案，具有更好的可扩展性和适应性。
...
### [70] [Trajectory Densification and Depth from Perspective-based Blur](https://arxiv.org/abs/2512.08627)
*Tianchen Qiu,Qirun Zhang,Jiajian He,Zhengyue Zhuge,Jiahui Xu,Yueting Chen*

Main category: cs.CV

TL;DR: 提出一种通过分析视频模糊模式来估计度量深度和密集轨迹的新方法，利用光学设计算法联合处理，在手持拍摄场景中实现精确的深度估计和轨迹重建。


<details>
  <summary>Details</summary>
Motivation: 在无机械稳定器的情况下，相机在拍摄过程中会发生不可避免的旋转运动，特别是在长曝光场景下会产生透视模糊。从光学角度看，这种模糊是深度位置相关的：即使在同一成像设置下，不同空间位置的物体也会产生不同程度的模糊。

Method: 提出一种新颖方法，通过分析视频流的模糊模式来估计度量深度和密集轨迹，采用联合光学设计算法。具体包括：1) 使用现成的视觉编码器和点跟踪器提取视频信息；2) 通过窗口嵌入和多窗口聚合估计深度图；3) 使用视觉语言模型对光学算法的稀疏轨迹进行稠密化处理。

Result: 在多个深度数据集上的评估表明，该方法在大深度范围内表现出色，同时保持良好的泛化能力。与手持拍摄场景中的真实轨迹相比，光学算法实现了更高的精度，稠密重建保持了较强的准确性。

Conclusion: 通过分析视频模糊模式来估计深度和轨迹的方法是有效的，特别是在无机械稳定器的手持拍摄场景中，能够实现精确的深度估计和轨迹重建，具有良好的泛化性能。
...
TL;DR: CoIG框架将图像生成重构为类似人类艺术创作的顺序语义过程，通过LLM将复杂提示分解为逐步指令，实现可监控的图像生成。


<details>
  <summary>Details</summary>
Motivation: 当前图像生成模型是"黑盒"系统，缺乏可观察性和可干预性，难以确保可靠性、安全性和控制性，且其非人类工作流程难以解释。

Method: 提出Chain-of-Image Generation框架，利用LLM将复杂提示分解为简单逐步指令，图像生成模型按计划逐步生成和编辑图像，每个步骤专注于单个语义实体。

Result: 实验结果表明CoIG显著增强了定量可监控性，同时在与基线模型相比时实现了竞争力的组合鲁棒性。框架是模型无关的，可与任何图像生成模型集成。

Conclusion: CoIG框架通过将图像生成重构为顺序语义过程，解决了当前图像生成模型的"黑盒"问题，实现了类似Chain-of-Thought在LLM中的优势，提高了可监控性和可解释性。
...
Method: 提出C-DIRA框架：1）显著性驱动的Top-K ROI池化和融合分类进行局部特征提取与集成；2）动态ROI路由仅对高难度数据样本应用ROI推理；3）伪域标注和对抗学习学习对驾驶员和背景变化鲁棒的域不变特征

Result: 在State Farm Distracted Driver Detection Dataset上，C-DIRA相比先前轻量级模型保持高准确率的同时显著减少FLOPs和延迟；在模糊和低光等视觉退化条件下表现鲁棒，在未见域上性能稳定

Conclusion: C-DIRA在紧凑性、效率和泛化能力方面有效，为边缘设备上的实时驾驶员行为识别提供了实用解决方案
...
TL;DR: 论文提出了一种名为MNAME的即插即用训练框架，通过内存银行机制提供大量负样本进行对比学习，无需外部编码器，显著提高了去噪生成模型的训练效率和生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前去噪生成模型（如扩散模型、流匹配）在视觉合成中占主导地位，但存在训练成本高和表示学习效率低的问题。虽然通过辅助对齐注入判别性表示的方法有效，但仍依赖外部预训练编码器，带来额外开销和领域偏移问题。

Method: 提出MNAME框架，采用内存银行机制维护跨训练迭代的动态更新负样本队列，将负样本数量与小批量大小解耦。使用低维投影头最小化内存和带宽开销，无需外部编码器，推理时不增加额外参数或计算成本。

Result: 在ImageNet-256上，MNAME在40万步内实现了2.40的FID分数，达到最先进水平，显著优于可比方法。框架实现了更快的收敛速度和更高效的生成质量。

Conclusion: MNAME框架通过内存银行机制提供高质量负样本，消除了对外部预训练模型的依赖，在不增加推理成本的情况下显著提高了生成模型的训练效率和生成质量，为去噪生成模型提供了一种更高效的自包含解决方案。
...
Method: 提出MoSAIC-ReID框架：1) 使用LoRA专家，每个专家对应单个属性；2) 采用oracle路由机制进行可控归因分析；3) 结合广义线性模型、统计检验和特征重要性分析。

Result: 在Market-1501和DukeMTMC数据集上取得有竞争力的性能，主要价值在于大规模量化属性重要性分析：服装颜色和内在特征贡献最大，不频繁线索（如配饰）影响有限。

Conclusion: 为可解释行人重识别提供了原则性框架，强调了在实践中整合显式语义知识的要求，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的无训练开放词汇语义分割方法在精确定位或复杂流程方面存在挑战，特别是在遥感场景中密集小目标众多。SAM 3统一了分割和识别功能，本文旨在探索其在遥感开放词汇语义分割中的应用潜力。

Method: 1. 实现掩码融合策略，结合SAM 3的语义分割头和Transformer解码器（实例头）的输出，以更好地覆盖土地覆盖类型；2. 利用存在头（presence head）的存在分数过滤场景中不存在的类别，减少由于地理空间场景中词汇量大和补丁级处理导致的误报。

Result: 在广泛的遥感数据集上进行评估，实验表明这种简单适应方法取得了有前景的性能，证明了SAM 3在遥感开放词汇语义分割任务中的潜力。

Conclusion: SAM 3在遥感开放词汇语义分割任务中具有应用潜力，通过简单的无训练适应方法即可获得良好性能，为遥感场景的语义分割提供了新的解决方案。
...
TL;DR: 提出一种结合3D渲染与约束扩散模型的工作流，自动生成高质量、逼真的双孢蘑菇合成图像及标注，用于训练分割模型，在零样本设置下达到SOTA性能。
...
Method: 提出新颖工作流，将Blender中的3D渲染与约束扩散模型相结合，自动生成高质量、逼真的双孢蘑菇合成图像及标注。该方法保持对3D场景配置和标注的完全控制，同时实现逼真效果，无需专业计算机图形学知识。

Result: 发布两个合成数据集（各含6,000张图像，描绘超过25万个蘑菇实例）。在零样本设置下评估在这些数据集上训练的Mask R-CNN模型。在两个独立真实世界数据集（包括新收集的基准）上测试，达到最先进的分割性能（在M18K上F1=0.859），尽管仅使用合成训练数据。

Conclusion: 该方法在双孢蘑菇上得到验证，但所提出的流程可轻松适应其他蘑菇物种或其他农业领域，如水果和叶片检测。合成数据生成工作流为计算机视觉在农业应用中的可扩展训练数据创建提供了有效解决方案。
...
TL;DR: 提出基于偏度指导的剪枝方法，在联邦学习环境中压缩多模态Swin Transformer模型，实现36%的模型大小缩减且保持准确率
...
Method: 提出偏度指导的剪枝方法，基于输出分布的统计偏度，选择性地剪枝多模态Swin Transformer的多头自注意力和多层感知机层。在水平联邦学习环境中验证

Result: 在紧凑型Swin Transformer上实现约36%的模型大小缩减，且没有准确率损失。证明了在联邦学习环境中保持性能的同时显著降低模型复杂度的可行性

Conclusion: 该方法展示了在多模态医学AI边缘设备上实现高效模型压缩和隐私保护分布式学习的可行性，为资源受限环境下的医学AI部署提供了解决方案
...
Result: 通过规模化训练，Wan-Move能生成5秒480p视频，其运动控制能力在用户研究中媲美Kling 1.5 Pro的Motion Brush商业产品，在MoveBench和公开数据集上均表现出优越的运动质量

Conclusion: Wan-Move实现了精确高质量的运动控制，无需辅助运动编码器，易于扩展，为视频生成模型提供了实用的运动控制解决方案，并发布了代码、模型和基准数据集
...
TL;DR: 该研究探讨生成式与非生成式视觉模型在组合泛化能力上的差异，发现生成式方法通过解码器约束和反演能更好地实现组合泛化，而非生成式方法难以通过正则化或架构约束获得必要的归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 人类视觉感知被认为需要生成式方法，但当前最成功的视觉模型是非生成式的编码器模型。这引发了一个问题：生成是否真的是机器实现人类水平视觉感知的必要条件？研究旨在通过组合泛化这一人类感知的标志性能力来探讨这个问题。

Method: 在组合数据生成过程下，形式化分析了基于解码器（生成式）和基于编码器（非生成式）方法实现组合泛化所需的归纳偏置。理论上证明了对编码器施加这些归纳偏置通常不可行，而生成式方法可以通过约束解码器并进行反演来实施这些偏置。研究了两种反演方法：在线梯度搜索和离线生成回放。在真实感图像数据集上训练了多种生成式和非生成式方法进行实证验证。

Result: 缺乏必要归纳偏置的非生成式方法通常在组合泛化上失败，需要大规模预训练或额外监督来改善泛化能力。相比之下，生成式方法通过解码器的合适归纳偏置结合搜索和回放，能在不增加额外数据的情况下显著提升组合泛化能力。
...
TL;DR: Tri-Bench是一个用于评估视觉语言模型几何推理能力的基准测试，专注于平面三角形问题，测试模型在相机姿态和场景干扰下的表现，发现当前VLM在几何推理和利用提示中的参考框架方面存在显著缺陷。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型表现出色，但在真实场景变化下经常失败。需要可验证的几何推理来构建可信和可控的智能体AI，因此创建Tri-Bench来隔离相对几何推理能力，并测试两个关键部署因素：相机姿态和场景干扰。

Method: 提出Tri-Bench基准测试，包含平面三角形问题，评估四个最近的VLM使用固定提示（明确描述周围正方形边框，可通过单应性获得正确答案）。测试六个简单任务，包括二元和连续目标，分析相机姿态（平面vs倾斜）和场景干扰（10个日常物体）的影响。

Result: 整体准确率相对于3D地面真值约为69%（最佳75%，最差64%）。在2D图像平面投影上的准确率更高（约72%）。所有四个VLM在识别少数形状类别（等边、等腰、直角三角形）时准确率降至约0%。相机倾斜导致整体准确率下降约4.1%。物体干扰对VLM准确率无显著影响。

Conclusion: 当前VLM未能正确利用提示中提供的明确参考框架提示，默认使用2D图像平面线索。模型在几何推理方面存在系统性缺陷，特别是在处理相机姿态变化和识别特定几何形状时表现不佳，这突显了可验证几何推理在可信AI中的重要性。
...
TL;DR: 该研究提出了一种新颖的结构化定位机制，通过专用控制令牌增强视觉语言模型在卫星图像中的视觉定位能力，在多个遥感基准测试中显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型正在成为遥感领域的强大通用工具，能够整合跨任务信息并通过聊天界面实现灵活的指令交互。然而，现有模型在卫星图像中的视觉定位能力仍有提升空间，特别是在复杂场景中的精确定位方面。

Method: 提出了一种结构化定位机制，在预训练的视觉语言模型基础上进行微调，使其适应多种指令跟随任务。通过专用控制令牌与专门的定位模块进行接口连接，实现语言和空间信息的联合推理。

Result: 在多个遥感基准测试中评估了该框架，一致性地改进了最先进水平，其中在视觉定位任务上相比先前方法实现了24.8%的相对提升。
...
Method: 提出GPU优化的旋转不变卷积框架，消除了传统基于矩阵乘法的卷积所需的数据降维步骤。通过利用对称旋转滤波器间的结构化数据共享，实现了内存流量和计算冗余大幅减少的多方向卷积。该方法进一步推广到加速任意旋转角度的卷积。

Result: 在广泛基准测试中，提出的卷积比CUDNN快20-55%，能耗降低15-45%，同时保持与最先进旋转不变方法相当的精度。在8方向设置下，对256×256输入实现45%加速和41%节能，对1024×1024输入实现32%加速和23%节能。集成到U-Net分割模型中，相比非旋转感知基线精度提升达6%。
...
TL;DR: UniLayDiff：首个统一的扩散Transformer模型，能够处理从无条件到各种条件约束的内容感知布局生成任务，无需为不同任务单独训练模型。


<details>
  <summary>Details</summary>
Motivation: 内容感知布局生成在平面设计自动化中至关重要，但现实应用场景多样，难以用单一模型统一处理元素类型、尺寸、关系等不同约束条件的子任务。现有方法要么只能处理部分任务，要么需要为不同条件训练独立模型，缺乏真正的统一解决方案。

Method: 提出UniLayDiff统一扩散Transformer，将布局约束视为独立模态，采用多模态扩散Transformer框架捕捉背景图像、布局元素和多样化约束之间的复杂交互。通过LoRA微调集成关系约束，在预训练其他任务后进行。

Result: 大量实验表明，UniLayDiff在从无条件到各种条件生成任务中均达到最先进性能，是首个能够统一完整内容感知布局生成任务的模型。

Conclusion: UniLayDiff首次实现了内容感知布局生成任务的真正统一，通过单一端到端可训练模型处理多样化约束条件，不仅实现了统一条件生成，还提升了整体布局质量。
...
TL;DR: EvoScene是一个无需训练的自演进框架，能够从单张图像逐步重建完整的3D场景，通过结合3D生成模型的几何推理能力和视频生成模型的视觉知识，在2D和3D域之间交替迭代，生成高质量的带纹理3D网格。


<details>
  <summary>Details</summary>
Motivation: 当前从单张图像生成高质量带纹理3D场景仍然是一个基本挑战。现有的图像到3D生成器虽然能从单视角恢复合理的几何结构，但由于其面向对象的训练方式，难以泛化到复杂、大规模的场景，无法保证结构和纹理的忠实性。

Method: EvoScene采用无需训练的自演进框架，通过三个迭代阶段：1) 空间先验初始化；2) 视觉引导的3D场景网格生成；3) 空间引导的新视角生成。该方法结合了3D生成模型的几何推理能力和视频生成模型的视觉知识，在2D和3D域之间交替迭代，逐步改进结构和外观。
...
<details>
  <summary>Details</summary>
Motivation: 夜间环境对基于摄像头的感知系统构成重大挑战，现有方法被动依赖场景照明，需要一种主动照明解决方案来提升夜间感知性能

Method: 结合现成的视觉感知模型与高清头灯，动态预测最优照明场，将光线从空区域重新分配到物体区域，形成闭环主动照明系统

Result: 在合成数据上训练并在真实世界闭环驾驶场景中零样本部署，相比标准近光灯提升18.7% mAP50和5.0% mIoU，在降低40%能耗的同时保持性能

Conclusion: LiDAS通过将现有头灯转变为主动视觉执行器，为鲁棒的夜间感知提供了经济有效的解决方案，并能与领域泛化方法互补增强鲁棒性
...
TL;DR: D4RT是一个简单但强大的前馈模型，使用统一的Transformer架构从单视频中联合推断深度、时空对应关系和完整相机参数，通过新颖的查询机制实现轻量高效的4D重建。


<details>
  <summary>Details</summary>
Motivation: 从视频中理解和重建动态场景的复杂几何与运动是计算机视觉中的一个重大挑战，需要高效解决这一任务的方法。

Method: 采用统一的Transformer架构，通过新颖的查询机制避免密集逐帧解码的计算负担和多任务特定解码器的复杂性，能够独立灵活地探测时空任意点的3D位置。

Result: 该方法实现了轻量化和高度可扩展性，在训练和推理方面都非常高效，在广泛的4D重建任务中超越了先前方法，达到了新的state-of-the-art水平。

Conclusion: D4RT为动态场景的4D重建提供了一种简单而强大的解决方案，通过统一的Transformer架构和创新的查询机制实现了高效准确的几何与运动重建。
...
<details>
  <summary>Details</summary>
Motivation: 传统NVS方法依赖显式3D归纳偏置和已知相机参数，而VGGT等视觉基础模型虽然能直接从无标定图像预测相机参数和3D表示，但其特征缺乏显式多视角几何一致性，改进这种3D特征一致性对NVS和姿态估计都有益

Method: 提出Selfi自改进3D重建流水线，通过特征对齐将VGGT骨干转换为高保真3D重建引擎。使用轻量级特征适配器，基于重投影一致性损失训练，将VGGT输出蒸馏到新的几何对齐特征空间，该空间能捕捉3D空间邻近性

Result: 在NVS和相机姿态估计任务中实现了最先进的性能，证明特征对齐是下游3D推理中非常有益的步骤

Conclusion: 特征对齐能显著提升3D特征一致性，从而改善NVS和姿态估计性能，Selfi展示了将视觉基础模型转换为高保真3D重建引擎的有效途径
...
TL;DR: 提出SEA通用框架，通过预训练模型分割视频帧序列为单个手语，嵌入共享潜在空间，再用轻量动态规划对齐字幕，支持多语言跨领域应用


<details>
  <summary>Details</summary>
Motivation: 现有字幕对齐方法通常依赖特定语言或数据集的端到端训练，限制了通用性，需要开发跨语言和跨领域的通用对齐方法

Method: SEA框架：1) 分割视频帧序列为单个手语；2) 将每个手语视频片段嵌入与文本共享的潜在空间；3) 使用轻量级动态规划程序进行对齐，CPU上分钟级处理小时级视频

Result: 在四个手语数据集上实现了最先进的对齐性能，展示了生成高质量平行数据的潜力，代码和模型已开源

Conclusion: SEA提供了一个灵活、高效的通用框架，能够适应多种场景，从小型词典到大型连续语料库，为推进手语处理提供了高质量平行数据生成能力
...
TL;DR: 该论文研究了通用对抗后缀：短令牌序列（4-10个令牌），当附加到任何输入时，能广泛降低跨任务和跨模型的准确性。通过Gumbel-Softmax松弛学习可微分的"软"形式后缀，然后离散化用于推理。
...
Method: 使用Gumbel-Softmax松弛学习可微分的"软"形式后缀，然后离散化用于推理。训练最大化标签区域的校准交叉熵，同时屏蔽黄金令牌以防止平凡泄漏，并使用熵正则化避免崩溃。

Result: 在情感分析、自然语言推理、释义检测、常识问答和物理推理等任务上，使用Qwen2-1.5B、Phi-1.5和TinyLlama-1.1B模型进行实验，证明了攻击效果的一致性和跨任务、跨模型族的可迁移性。

Conclusion: 单个在一个模型上训练的后缀能有效迁移到其他模型，持续降低准确性和校准置信度，证明了通用对抗后缀的有效性和可迁移性。
...
TL;DR: 使用强化学习框架训练对抗性后缀，通过PPO算法优化后缀策略，利用校准交叉熵奖励函数提升跨任务和跨模型的迁移能力


<details>
  <summary>Details</summary>
Motivation: 语言模型容易受到短对抗性后缀的攻击，现有方法通常使用梯度搜索或基于规则的方法，但这些方法脆弱且通常只针对单一任务或模型

Method: 采用强化学习框架，将对抗性后缀视为策略，使用近端策略优化（PPO）算法进行训练，以冻结模型作为奖励预言机，通过校准交叉熵进行奖励塑形以消除标签偏差并聚合表面形式

Result: 在五个不同的NLP基准数据集（涵盖情感分析、自然语言推理、释义和常识推理）和三个语言模型（Qwen2-1.5B Instruct、TinyLlama-1.1B Chat、Phi-1.5）上评估，RL训练的后缀能持续降低准确率，且比类似类型的先前对抗性触发器在跨任务和跨模型迁移方面更有效

Conclusion: 强化学习框架训练的对立后缀比传统方法更具鲁棒性和可迁移性，能有效攻击多种语言模型和任务
...
<details>
  <summary>Details</summary>
Motivation: 解决临床试验数据分散问题，整合ClinicalTrials.gov和PubMed研究文章，为患者、临床医生、研究人员和政策制定者提供更全面的结构化临床试验数据访问

Method: 使用GPT-5.1和Gemini-3-Pro等大语言模型，自动解析全文研究文章提取结构化试验信息，将用户查询转换为结构化数据库搜索，并提供基于证据的问答系统

Result: 相比仅使用ClinicalTrials.gov，系统将结构化临床试验数据访问量提高了83.8%，通过用户研究和自动评估验证了信息提取和问答能力

Conclusion: ClinicalTrialsHub通过整合和增强临床试验数据，显著改善了证据医学的数据可访问性，为医疗决策提供更好的支持
...
TL;DR: 研究通过概念性复制手动标注，评估多种GLLM在五个概念标注中的偏见表现，发现GLLM虽然F1分数尚可，但与手动标注存在系统性偏差，且不同GLLM之间的一致性高于与人工标注的一致性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是调查生成式大语言模型在文本标注任务中存在的偏见问题，通过概念性复制Boukes (2024)的手动标注研究，系统评估GLLM标注的可靠性和偏差程度。

Method: 使用四种不同的GLLM模型（Llama3.1:8b, Llama3.3:70b, GPT4o, Qwen2.5:72b），结合五种不同的提示词，对五个概念（政治内容、互动性、理性、不文明程度、意识形态）进行标注，并与手动标注结果进行比较分析。

Result: GLLM在F1分数方面表现尚可，但在标注流行度方面与手动标注存在显著差异，导致下游分析结果产生实质性不同。更重要的是，GLLM之间存在系统性偏见，它们彼此之间的一致性高于与手动标注的一致性，且F1分数的差异无法充分反映这种偏见程度。

Conclusion: GLLM在文本标注任务中存在系统性偏见，虽然表面指标（如F1分数）可能表现良好，但实际标注结果与人工标注存在实质性差异，不同GLLM之间的趋同性高于与人工标注的一致性，这提示研究人员需要谨慎使用GLLM进行标注任务。
...
TL;DR: 该研究通过对比解释和显著性归因分析，探索机器翻译模型中性别偏见的起源，而非仅仅测量偏见。研究使用性别模糊的自然源数据，分析源句中哪些上下文标记会影响翻译模型选择特定的目标语言性别屈折变化。


<details>
  <summary>Details</summary>
Motivation: 当前可解释性研究主要关注理解黑盒模型决策，但在性别偏见这一重要问题上研究有限。研究旨在从简单地测量偏见转向探索偏见的起源，理解翻译模型中性别决策的触发因素。

Method: 使用性别模糊的自然源数据，采用对比解释和计算显著性归因的方法。首先解决缺乏评分阈值的问题，分析不同归因水平下源词对模型性别决策的影响。将显著的源词与人类对性别的感知进行比较，并进行语言学分析。

Result: 研究发现人类感知与模型归因之间存在显著重叠，表明模型在性别决策上与人类认知有一定一致性。通过语言学分析揭示了影响性别决策的关键词汇特征。

Conclusion: 研究展示了理解模型翻译决策中性别因素的重要性，以及这与人类决策的对比。这些信息应该被用来缓解性别偏见，为偏见缓解提供了新的视角和方法。
...
<details>
  <summary>Details</summary>
Motivation: 在线游戏和社区中匿名性导致不当言论升级为言语暴力和犯罪行为，需要检测技术来构建更安全的交流环境。虽然韩语大语言模型和思维链推理受到关注，但在不当言论检测方面的应用研究仍然有限。
...
Conclusion: 提出的软归纳偏置方法通过明确定义推理视角，能够有效指导大语言模型的推理过程，在不适当言论检测任务中表现出色，为构建更安全的在线交流环境提供了有效技术方案。
...
Method: 采用分层多智能体架构，在64×64网格上部署轻量级智能体，配合选择性oracle；通过空间课程学习逐步扩展操作区域；使用负对数似然作为置信度指标；Thompson Sampling课程管理器基于能力和NLL奖励信号自适应选择训练区域

Result: 在空间化汉诺塔基准测试中，该方法展现出更好的稳定性、减少oracle使用、并通过分布式智能体协作实现更强的长距离推理能力
...
TL;DR: 这是一篇关于医疗领域NLP应用的教程论文，介绍了医疗NLP的三个层次架构：数据/资源层、NLP评估层和患者层，并包含实践环节。
...
Method: 采用三层架构组织教程内容：1) 数据/资源层（标注指南、伦理审批、治理、合成数据）；2) NLP评估层（NER、关系抽取、情感分析等任务及可解释性方法）；3) 患者层（患者参与、健康素养、翻译简化等任务）。包含实践环节让听众使用医疗NLP应用。

Result: 提出了一个结构化的医疗NLP教程框架，覆盖了从数据准备到患者应用的完整流程，为医疗NLP从业者、研究人员和学生提供了系统的学习资源。

Conclusion: 该教程为医疗NLP领域提供了全面的入门指南，特别强调了现有综述中缺失的重要方面，通过三层架构和实践环节帮助不同背景的参与者理解和应用医疗NLP技术。
...
TL;DR: QSTN是一个用于系统生成问卷式提示响应的开源Python框架，支持使用大语言模型进行计算机模拟调查和标注任务，显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 为了支持使用大语言模型进行计算机模拟调查和标注任务，需要系统化的工具来评估问卷呈现方式、提示扰动和响应生成方法对结果的影响。

Method: 开发了QSTN开源Python框架，通过系统生成问卷式提示响应，支持评估问卷结构、提示扰动和响应生成方法，并提供无需编码的用户界面。

Result: 超过4000万次调查响应的评估显示，问题结构和响应生成方法对生成调查响应与人类答案的一致性有显著影响，且计算成本大幅降低。

Conclusion: QSTN框架将支持未来基于大语言模型研究的可重复性和可靠性，为研究人员提供无需编码的实验设置能力。
...
TL;DR: 首个巴斯克语自动作文评分数据集，包含3200篇C1水平作文，专家标注评分与反馈，开源模型微调后在评分一致性和反馈质量上超越GPT-5等闭源系统


<details>
  <summary>Details</summary>
Motivation: 为低资源语言（巴斯克语）建立透明、可复现的自动作文评分和反馈生成研究基础，解决现有闭源系统缺乏透明度、难以在教育场景应用的问题

Method: 构建首个巴斯克语C1水平作文数据集（3200篇），专家标注五个维度的评分和详细反馈；微调开源模型RoBERTa-EusCrawl和Latxa 8B/70B；提出结合自动一致性指标和专家验证的新型反馈评估方法

Result: 编码器模型在评分任务上表现可靠；微调后的Latxa模型在评分一致性和反馈质量上超越GPT-5、Claude Sonnet 4.5等闭源系统；能生成与评分标准对齐、具有教学意义的反馈，识别更多错误类型

Conclusion: 该数据集和基准为巴斯克语等低资源语言的NLP研究奠定了基础，展示了开源模型在教育应用中的潜力，促进了透明、可复现的教育技术发展
...
<details>
  <summary>Details</summary>
Motivation: 低资源语言缺乏母语者编写的数据集和生成流畅合成数据的语言模型，现有偏好优化研究主要针对英语和中文，需要开发不依赖目标语言指令数据的流畅偏好对齐方法

Method: 采用策略训练方法，与两种常见方法进行比较：基于机器翻译数据的监督微调和多语言微调，以挪威语为案例研究

Result: 通过母语者评估显示，策略训练方法在保持流畅性方面优于其他方法，且不依赖难以获取的数据

Conclusion: 策略训练对于低资源语言的偏好对齐至关重要，能够在不依赖目标语言指令数据的情况下实现流畅的语言模型
...
TL;DR: 本文提出在联邦学习环境中评估大型语言模型与多样化人类偏好对齐的方法，引入自适应聚合策略以平衡对齐质量和公平性。
...
Method: 提出综合评估框架，在联邦学习环境中评估不同聚合策略；每个组本地评估生成奖励信号，服务器聚合组级奖励而不访问原始数据；评估标准聚合技术（最小值、最大值、平均值）并引入基于历史对齐性能动态调整偏好权重的自适应方案。

Result: 在基于PPO的RLHF管道的问答任务实验中，自适应方法在保持竞争性对齐分数的同时，始终实现更优的公平性。

Conclusion: 这项工作为评估LLM在不同人群中的行为提供了稳健方法，并为开发真正多元化和公平对齐的模型提供了实用解决方案。
...
TL;DR: 该研究发现逐步增加Transformer深度训练不仅能降低训练成本，还能通过改变残差流结构和形成可置换计算块来更有效地利用模型深度，克服标准模型中的深度利用限制问题。


<details>
  <summary>Details</summary>
Motivation: 虽然MIDAS研究表明逐步增加Transformer深度能降低训练成本并提升推理性能，但缺乏对这些增益的机制性理解。同时，近期研究显示非增长模型中后半部分层对最终输出分布的贡献远小于前半部分（深度诅咒现象）。

Method: 通过深度分析，研究逐步中间堆叠增长方式如何影响模型深度利用、改变残差流结构，并促进可置换计算块的形成。同时提出MIDAS的轻量级改进方案。

Result: 研究发现深度增长能更有效地利用模型深度，改变残差流结构，促进可置换计算块形成。提出的MIDAS改进方案在下游推理基准测试中取得了进一步性能提升。

Conclusion: 逐步增加模型深度能形成独特的计算电路，克服标准非增长模型中观察到的有限深度利用问题，为理解深度增长机制提供了新的见解。
...
<details>
  <summary>Details</summary>
Motivation: RAG虽然通过检索证据提高了LLM的事实性，但生成内容与源材料矛盾或超出范围的忠实性失败问题仍然严重。现有幻觉检测方法要么需要大量标注数据进行训练，要么依赖外部LLM判断导致高推理成本，而基于LLM内部表征的方法准确率有限

Method: 利用稀疏自编码器解耦LLM内部激活，识别RAG幻觉触发的特定特征。基于信息特征选择和加性特征建模的系统化流程，开发RAGLens轻量级检测器，使用LLM内部表征准确标记不忠实的RAG输出

Result: RAGLens在检测性能上优于现有方法，提供可解释的决策依据，能够有效事后缓解不忠实的RAG输出。研究还揭示了LLM中幻觉相关信号的分布新见解

Conclusion: RAGLens通过利用LLM内部表征和稀疏自编码器，实现了高效、轻量且可解释的RAG幻觉检测，为RAG系统的忠实性监控提供了新方法
...
TL;DR: 该研究对比了数据导向设计(DOD)与传统面向对象设计(OOD)在多线程环境下的性能表现，发现DOD在执行时间、系统调用和缓存未命中方面表现更优，但多线程版本因线程管理开销反而比单线程版本性能差。


<details>
  <summary>Details</summary>
Motivation: 随着多核CPU与主内存性能差距的扩大，需要硬件感知的软件设计范式。本研究旨在全面分析数据导向设计与传统面向对象设计在缓存利用和多线程环境效率方面的差异。

Method: 开发并比较了A*搜索算法的四个版本：单线程OOD、单线程DOD、多线程OOD、多线程DOD。评估指标包括执行时间、内存使用和CPU缓存未命中。

Result: 多线程测试中，DOD实现表现出显著性能优势，执行时间更快，原始系统调用和缓存未命中更少。对于A*算法这类细粒度任务，线程管理开销导致单线程版本在两个范式中都显著优于多线程版本。

Conclusion: 即使在简单算法中性能差异看似细微，但DOD在关键指标上的持续优势凸显了其基础架构的优越性，表明在复杂、大规模AI和并行计算任务中，DOD是最大化硬件效率的更有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在推理过程中会产生大量KV缓存，导致内存和吞吐量瓶颈。现有的KV缓存驱逐方法在多批次设置下无法保持准确性，且会产生更长的序列。

Method: 提出SkipKV方法：1) 使用句子级评分指标识别并删除高度相似的句子；2) 动态调整引导向量来更新隐藏激活状态，抑制冗余生成；3) 在粗粒度的句子级别进行序列移除。

Result: 在多个推理基准测试中，SkipKV在相似压缩预算下保持高达26.7%的准确性提升，相比现有方法减少1.6倍的生成长度，提高1.7倍的吞吐量。

Conclusion: SkipKV通过句子级KV压缩有效解决了推理过程中的KV缓存开销问题，在保持准确性的同时显著提高了推理效率。
...
TL;DR: 开发了一个基于大语言模型的AI辅助患者-临床试验匹配系统，通过生成结构化资格评估和可解释推理链来支持人工审查，旨在减轻协调员负担并提高匹配效率。
...
Method: 使用开源、具备推理能力的大语言模型构建安全可扩展的概念验证系统，整合异构电子健康记录数据，生成结构化资格评估和可解释推理链，支持人机协作审查。

Result: 系统能够将资格表示为动态状态而非固定判定，在可能时识别匹配，并提供使患者未来符合资格的可操作建议，同时保证所有AI生成输出的全面可审计性。

Conclusion: 该AI增强的患者-试验匹配系统有望减轻协调员负担，智能扩大每位患者考虑的试验范围，并确保全面可审计性，为临床试验筛选提供创新解决方案。
...
TL;DR: 本研究对ChatGPT和DeepSeek两大先进LLM在教育研究领域进行全面评估，包括技术分析、实证实验和用户调查，比较它们在文本生成、编程和专业问题解决方面的性能差异。


<details>
  <summary>Details</summary>
Motivation: 预训练大语言模型在教育研究领域展现出巨大潜力，ChatGPT和DeepSeek作为当前最先进的模型，在数学、科学、医学、文学和编程方面表现出色。本研究旨在全面评估这两个模型在教育研究应用中的性能差异和适用性。

Method: 采用三方面研究方法：1) 背景技术分析；2) 实证实验，在文本生成、编程和专业问题解决任务上进行基准测试；3) 真实用户调查，收集学生、教育工作者和研究人员的实际使用反馈。

Result: 实验结果显示：ChatGPT在通用语言理解和文本生成方面表现更优，而DeepSeek凭借其效率导向的设计在编程任务中表现更佳。两个模型都能提供医学上准确的诊断输出，并能有效解决复杂的数学问题。用户调查进一步揭示了这些模型在实际教育研究应用中的具体优势和限制。

Conclusion: 研究全面评估了ChatGPT和DeepSeek在教育研究领域的性能表现，发现两者各有优势：ChatGPT擅长通用语言任务，DeepSeek在编程方面更高效。两个模型都能胜任专业领域任务，为教育研究领域的AI应用提供了有价值的参考依据。
...
TL;DR: 开发了一个用于移动糖尿病预测应用的可扩展后端系统，采用水平扩展、数据库分片和异步消息队列，83%的功能达到性能目标，支持1万并发用户
...
Result: 83%的系统功能（24个中的20个）达到性能目标，用户档案管理、活动跟踪和读取密集型预测操作成功达标，系统能处理1万并发用户

Conclusion: 开发的可扩展后端系统有效支持糖尿病预测应用，异步通信机制对降低计算密集型预测请求的错误率至关重要，验证了系统的可扩展性和可靠性
...
TL;DR: 该论文提出了KPI框架，通过知识图谱增强、原型感知和可解释性方法，从患者侧信息预测疾病，解决现有方法的疾病分布不平衡和缺乏可解释性问题。


<details>
  <summary>Details</summary>
Motivation: 仅从患者侧信息（人口统计和自述症状）预测疾病具有重要价值，可提高患者意识、促进早期医疗参与和提升医疗系统效率。但现有方法面临疾病分布不平衡和缺乏可解释性等关键挑战，导致预测存在偏见或不可靠。

Method: 提出KPI框架：1）系统整合结构化可信医疗知识构建统一疾病知识图谱；2）构建临床有意义的疾病原型；3）采用对比学习提升预测准确性，特别是对长尾疾病；4）利用大语言模型生成患者特异性、医学相关的解释。

Result: 在真实世界数据集上的广泛实验表明，KPI在预测准确性方面优于最先进方法，并提供与患者叙述密切相关的临床有效解释，突显其在以患者为中心的医疗服务中的实用价值。

Conclusion: KPI框架通过整合医疗知识、构建疾病原型和增强可解释性，有效解决了疾病预测中的不平衡和可解释性问题，为以患者为中心的医疗保健提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明大型语言模型在CFA考试中表现不佳，但近期推理模型在各种专业考试中取得优异成绩，因此需要评估最新推理模型在CFA考试中的实际表现。

Method: 使用包含980道题目的模拟CFA考试数据集，涵盖Level I三个考试、Level II两个考试和Level III三个考试，评估多个先进推理模型的表现，采用与先前研究相同的通过/失败标准。

Result: 大多数模型通过了所有三级CFA考试，按总体表现排序为：Gemini 3.0 Pro、Gemini 2.5 Pro、GPT-5、Grok 4、Claude Opus 4.1和DeepSeek-V3.1。Gemini 3.0 Pro在Level I创下97.6%的最高分记录，GPT-5在Level II以94.3%领先，Gemini 2.5 Pro在Level III选择题获得86.4%最高分，而Gemini 3.0 Pro在构建回答题获得92.0%最高分。

Conclusion: 最新推理模型在CFA考试中表现出色，显著超越了先前大型语言模型的性能，证明这些模型在金融专业考试中具有强大的推理和分析能力。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型通过强化学习后训练进化为推理语言模型，其高级推理能力体现在思维链中产生"顿悟"时刻，执行自我反思和深度思考等策略。受此启发，需要一种机制使任何LLM都能成为RLM

Method: 提出强化策略注入机制(rSIM)，使用小型规划器引导LLM的思维链，通过自适应注入推理策略。基于领导者-跟随者框架，使用多智能体强化学习联合训练规划器(领导者智能体)和LLM(跟随者智能体)，采用简单的基于规则的奖励

Result: rSIM使Qwen2.5-0.5B成为RLM并显著超越Qwen2.5-14B。规划器具有通用性：只需训练一次即可作为插件大幅提升现有LLM的推理能力。规划器支持跨任务持续学习，规划能力逐步提升并泛化到更广泛问题

Conclusion: rSIM机制通过小型规划器引导LLM思维链，使用多智能体强化学习训练，能够有效提升语言模型的推理能力，使小模型超越大模型，且规划器具有通用性和持续学习能力
...
TL;DR: 该研究开发了一个机器学习框架，使用土耳其382个土壤样本的物理化学特性来预测加州承载比(CBR)，随机森林回归器表现最佳，为岩土工程提供了一种有效的传统方法替代方案。


<details>
  <summary>Details</summary>
Motivation: 传统CBR实验室测试虽然准确，但耗时、昂贵且在大规模或多样化土壤剖面中不实用。人工智能特别是机器学习的发展为快速精确建模复杂土壤行为提供了可能。
...
Result: 随机森林回归器表现最佳，获得R2分数：训练集0.95，验证集0.76，测试集0.83。显示出强大的非线性映射能力。

Conclusion: 该研究支持在岩土工程中集成智能、数据驱动的模型，为传统方法提供了有效替代方案，促进了基础设施分析和设计的数字化转型。
...
TL;DR: 该研究提出使用自动化机器学习(AutoML)方法预测土壤压实参数(最佳含水量OMC和最大干密度MDD)，相比传统实验室方法和现有ML模型，AutoML通过自动算法选择和超参数优化，在异质土壤数据集上取得了更好的预测精度和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统确定土壤压实参数的方法需要费时费力的实验室实验，而现有的经验回归模型在不同土壤类型上的适用性和准确性有限。虽然近年来AI/ML技术被用于预测这些参数，但ML模型在异质数据集上的预测准确性和泛化能力仍然不足。

Method: 采用自动化机器学习(AutoML)方法预测OMC和MDD。AutoML自动执行算法选择和超参数优化过程，通过大量实验比较不同算法的性能。

Result: 研究发现极端梯度提升(XGBoost)算法表现最佳，在独立数据集上对MDD和OMC的预测R²值分别达到80.4%和89.1%。这表明AutoML方法在不同土壤类型上具有较好的预测能力。

Conclusion: AutoML方法能有效预测不同土壤类型的压实参数，异质数据集对提高ML模型的泛化能力和性能至关重要。该研究有助于通过改进土壤压实参数预测来提升施工实践的效率和可靠性。
...
TL;DR: DuSAR框架通过双策略协同和轻量级反思机制，让单个冻结LLM实现自适应推理，在ALFWorld和Mind2Web上取得SOTA性能，同时大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体依赖外部演示或检索增强规划，导致脆弱性、泛化能力差和计算开销大。受人类问题解决启发，需要一种无需演示、能自适应推理的框架。

Method: 提出DuSAR框架：包含高层次整体规划和上下文接地局部策略的双策略协同，通过轻量级反思机制（策略适应度评分）动态评估进展，在卡住时修订全局计划或在有意义进展时细化计划。

Result: 在ALFWorld上达到37.1%成功率（Llama3.1-70B），比先前最佳结果（13.0%）提升一倍以上；在Mind2Web上达到4.02%，同样翻倍；每步token消耗减少3-9倍。消融研究证实双策略协调的必要性。

Conclusion: DuSAR通过双策略协同和反思机制实现了高效的自适应推理，显著提升性能同时降低计算成本，且兼容外部知识集成，展现了灵活性和实用性。
...
TL;DR: DeepFeature：首个LLM赋能的上下文感知特征生成框架，用于可穿戴生物信号，通过多源特征生成、迭代特征精炼和鲁棒代码翻译，在8个任务上平均AUROC提升4.21-9.67%
...
Method: 提出DeepFeature框架：1）多源特征生成机制，整合专家知识与任务设置；2）迭代特征精炼过程，基于特征评估反馈进行特征重选；3）鲁棒的多层过滤和验证方法，确保特征到代码的可靠翻译

Result: 在八个不同任务上，DeepFeature相比基线方法平均AUROC提升4.21-9.67%，在五个任务上优于最先进方法，其余任务保持相当性能

Conclusion: DeepFeature通过LLM赋能和上下文感知的特征生成，有效解决了现有生物信号特征提取方法的局限性，显著提升了医疗健康应用的性能
...
TL;DR: 该论文提出了一种从控制流为中心的代码属性图转向数据转换图的新范式，通过多智能体框架解决仓库级自动程序修复中的语义陷阱问题，实现了87.1%的修复成功率。
...
Result: 实现了自主问题解决器系统，在多个软件工程基准测试中表现出色，在SWE-Verified基准测试上达到87.1%的解决率，有效解决了现代编码智能体中RAG系统的语义陷阱问题。

Conclusion: 该研究通过数据转换图和多智能体框架，为仓库级自动程序修复提供了更稳健的基础，直接解决了当前AI代码助手工具的核心限制，满足了软件依赖日益增强的世界对更强大基础的需求。
...
<details>
  <summary>Details</summary>
Motivation: 机器人在人类环境中操作需要伦理意识，但现有自动化规划工具缺乏支持。手动指定伦理规则既费力又高度依赖具体情境，需要更实用的解决方案。

Method: 开发Principles2Plan原型系统，让领域专家提供规划领域、问题细节和相关高级原则（如仁慈、隐私），系统生成可操作的伦理规则，用户可审查、优先排序并供给规划器生成伦理知情计划。

Result: 创建了首个支持用户在经典规划情境中生成基于原则的伦理规则的系统，展示了人类-LLM协作使伦理自动化规划更实用可行的潜力。

Conclusion: Principles2Plan证明了人类与大型语言模型协作在生成情境敏感伦理规则方面的有效性，为伦理自动化规划提供了更实用的方法。
...
TL;DR: 本文提出CogMCTS框架，将LLMs的认知引导机制与MCTS紧密集成，通过多轮认知反馈、双轨节点扩展和精英启发式管理，提升自动启发式设计的稳定性和解质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的进化方法依赖种群策略易陷入局部最优，而LLM与MCTS结合的方法在多轮认知整合和搜索多样性方面仍有局限，需要克服这些限制以实现更高效的自动启发式优化。

Method: 提出认知引导的MCTS框架(CogMCTS)，包含：1)多轮认知反馈机制整合历史经验、节点信息和负面结果；2)双轨节点扩展结合精英启发式管理平衡探索与利用；3)策略性突变增强启发式形式和参数多样性。

Result: 实验结果表明，CogMCTS在稳定性、效率和解决方案质量方面优于现有的基于LLM的自动启发式设计方法。

Conclusion: CogMCTS框架通过紧密集成LLMs的认知引导与MCTS，有效解决了现有方法在探索-利用权衡和搜索多样性方面的局限，实现了更高效的自动启发式优化。
...
TL;DR: 基于Transformer的蛋白质二级结构预测模型，使用注意力机制处理氨基酸序列，通过滑动窗口数据增强技术提升性能，在CB513数据集上表现良好。
...
Method: 提出基于Transformer的模型，应用注意力机制处理蛋白质序列数据。采用滑动窗口数据增强技术在CB513数据集上扩展训练样本。模型能够处理可变长度序列，有效捕捉局部和长程残基相互作用。

Result: Transformer模型展现出强大的泛化能力，能够处理可变长度序列，同时有效捕捉残基间的局部和长程相互作用，在蛋白质二级结构预测任务中表现良好。

Conclusion: Transformer架构适用于蛋白质二级结构预测任务，其注意力机制能够有效建模氨基酸序列中的复杂模式，为蛋白质功能理解提供了有效的计算工具。
...
### [119] [See-Control: A Multimodal Agent Framework for Smartphone Interaction with a Robotic Arm](https://arxiv.org/abs/2512.08629)
*Haoyu Zhao,Weizhong Ding,Yuhao Yang,Zheng Tian,Linyi Yang,Kun Shao,Jun Wang*

Main category: cs.AI

TL;DR: See-Control是一个通过低自由度机械臂直接物理交互操作智能手机的框架，解决了现有方法依赖ADB仅限Android设备的问题，提出了ESO任务基准和数据集。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型（MLLMs）用于智能手机操作的方法依赖于Android调试桥（ADB）进行数据传输和动作执行，这限制了它们仅适用于Android设备，缺乏平台无关的解决方案。

Method: 提出了See-Control框架，包含三个关键组件：1）包含155个任务和评估指标的ESO基准；2）基于MLLM的具身智能体，无需ADB或系统后端访问即可生成机器人控制命令；3）丰富标注的操作片段数据集。

Result: See-Control通过低自由度机械臂直接物理交互操作智能手机，实现了平台无关的解决方案，为家庭机器人在真实环境中执行依赖智能手机的任务提供了具体步骤。

Conclusion: 该工作通过弥合数字智能体与物理世界之间的差距，为家庭机器人执行依赖智能手机的任务提供了平台无关的解决方案，推动了具身智能在现实环境中的应用。
...
### [120] [Multi-Agent Intelligence for Multidisciplinary Decision-Making in Gastrointestinal Oncology](https://arxiv.org/abs/2512.08674)
...
<details>
  <summary>Details</summary>
Motivation: 胃肠道肿瘤学的多模态临床推理需要整合内镜图像、放射学数据和生化标志物。尽管多模态大语言模型显示出潜力，但在处理复杂、异质的医疗历史时经常遇到上下文稀释和幻觉问题。需要模拟人类多学科团队协作的工作流程来解决这些限制。

Method: 提出分层多智能体框架，模拟人类多学科团队的协作工作流程。该框架通过多个智能体协同工作，每个智能体专注于特定类型的医疗数据或推理任务，共同完成复杂的临床决策支持。

Result: 系统获得4.60/5.00的专家评估综合得分，相比单体基线有显著提升。基于智能体的架构在推理逻辑和医学准确性方面带来了最显著的改进。

Conclusion: 模拟性的、基于智能体的合作为肿瘤学中的自动化决策支持提供了一个可扩展、可解释且临床稳健的范式。这种协作方法能够有效处理多模态医疗数据的复杂性，提高临床推理的质量和可靠性。
...
TL;DR: 基础模型在单智能体任务上表现出色，但缺乏原生多智能体智能，需要专门研究来弥补这一差距


<details>
  <summary>Details</summary>
Motivation: 基础模型正成为AI智能体的"大脑"，虽然已有研究赋予其单智能体能力（如GUI交互、工具使用），但多智能体智能是下一个前沿领域。现有假设认为多智能体能力会自然涌现，但作者发现事实并非如此

Method: 通过41个大语言模型的广泛实证研究，验证单智能体性能与多智能体智能的关系。识别基础模型在多智能体环境中的四个核心能力：理解、规划、高效通信和适应

Result: 实证证据表明，强大的单智能体性能并不能自动产生稳健的多智能体智能。多智能体能力不会自然涌现，需要专门的研究和开发

Conclusion: 需要从数据集构建、评估、训练范式和安全考虑等多个研究方向，系统地构建具有原生多智能体智能的基础模型
...
### [122] [Performance Comparison of Aerial RIS and STAR-RIS in 3D Wireless Environments](https://arxiv.org/abs/2512.08755)
...
<details>
  <summary>Details</summary>
Motivation: 虽然可重构智能表面(RIS)和同时透射反射RIS(STAR-RIS)在增强无线覆盖和容量方面具有潜力，特别是当部署在无人机上时，但缺乏对这两种空中架构的全面性能比较研究。

Method: 建立了包含定向辐射模式的精确信道模型，研究了部署高度和方向的影响，并基于加权最小均方误差和块坐标下降算法，为两种架构制定了联合优化问题并提出了高效解决方案。

Result: 仿真结果显示：STAR-RIS在低空场景中由于全空间覆盖能力而优于RIS，而RIS在基站附近的高空场景中表现更好。

Conclusion: 研究结果为未来6G通信系统中空中智能表面的部署提供了实用指导，表明应根据具体场景需求选择合适的架构。
...
TL;DR: 本文提供构建生产级Agentic AI工作流的端到端实用指南，涵盖设计、开发、部署全生命周期，提出9个核心最佳实践，并通过多模态新闻分析案例展示应用。


<details>
  <summary>Details</summary>
Motivation: 随着Agentic AI在各行业加速应用，组织面临如何设计、构建和运营生产级Agentic AI工作流的挑战，需要确保其可靠性、可观测性、可维护性，并符合安全和治理要求。

Method: 提出结构化工程生命周期，包括工作流分解、多智能体设计模式、MCP协议、工具集成、确定性编排、负责任AI考量、环境感知部署策略，并总结9个核心最佳实践。

Result: 通过多模态新闻分析和媒体生成工作流的完整案例研究，展示了所提原则的实际应用，为构建稳健、可扩展、生产就绪的Agentic AI工作流提供基础参考。

Conclusion: 本文提供了构建生产级Agentic AI系统的全面框架，结合架构指导、操作模式和实际实施见解，帮助组织设计可靠、可维护且符合治理要求的自主AI工作流。
...
<details>
  <summary>Details</summary>
Motivation: 生成组件（如LoRA）的快速扩散形成了一个庞大但无结构的生态系统。现有发现方法依赖于不可靠的用户描述或有偏见的流行度指标，阻碍了可用性。

Method: 分析了650多个LoRA，通过多种提示和种子进行图像生成来评估其行为。使用CLIP嵌入及其与基础模型生成的差异，定义了三个部分的表示：方向（定义语义偏移）、强度（量化效果显著性）和一致性（量化效果稳定性）。

Result: 开发了高效的检索框架，能够语义匹配文本查询到相关LoRA，同时过滤过强或不稳定的LoRA，在自动和人工评估中优于文本基线。该表示还支持分析强度和一致性与版权法中实质性和意志性法律概念的联系。

Conclusion: CARLoS是一个实用的系统，不仅专注于检索，其表示方法对LoRA分析具有更广泛的相关性，能够支持版权等法律概念的分析。
...
TL;DR: 该论文综述了描述逻辑和逻辑编程中Craig插值和一致插值的理论结果与计算方法，探讨了它们在知识表示中的应用。


<details>
  <summary>Details</summary>
Motivation: Craig插值和一致插值在知识表示中有多种应用，包括可解释性、遗忘、模块化和重用，甚至学习。然而许多相关的知识表示形式化方法通常不具备这些插值性质，且实际计算插值具有挑战性。

Method: 深入研究了两种主要的知识表示形式化方法：描述逻辑和逻辑编程，讨论了计算插值的理论结果和实用方法。

Result: 论文综述了这两种形式化方法中插值计算的理论基础和实践技术，但没有提供具体的实验结果。

Conclusion: 该研究为知识表示领域中插值计算的理论和实践提供了系统性的分析框架，有助于推动相关应用的发展。
...
TL;DR: 提出REST和REST+两个新基准，用于系统评估多模态大语言模型中的跨模态不一致性问题，发现现有模型在不同模态间无法保持一致的推理能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型虽然训练将视觉和语言映射到同一嵌入空间，但无法在不同模态间执行相同任务。现有评估方法缺乏对跨模态一致性的系统测试，需要专门基准来量化模型在不同模态输入下的表现差异。

Method: 创建REST和REST+基准，包含相同语义信息在三种模态（图像、文本、混合）下的样本。评估15个最先进的多模态大语言模型，分析OCR准确性、视觉特征（文本颜色、分辨率、字体）和视觉token数量对模型性能的影响。

Result: 1. 所有评估模型在不同模态间都表现出显著的不一致性；2. 模态不一致程度因模型而异，即使考虑OCR问题后仍然存在；3. 将文本渲染为图像或将图像转换为文本都无法解决不一致性问题；4. 视觉特征（文本颜色和分辨率）和视觉token数量影响模型性能；5. 一致性得分与文本和图像之间的模态差距相关。

Conclusion: 多模态大语言模型存在系统性跨模态不一致问题，这反映了模型在处理不同模态时的根本性差异。提出的REST基准为评估和改善多模态一致性提供了重要工具，揭示了模型架构中的机制性解释。
...
### [127] [Missing Wedge Inpainting and Joint Alignment in Electron Tomography through Implicit Neural Representations](https://arxiv.org/abs/2512.08113)
*Cedric Lim,Corneel Casert,Arthur R. C. McCray,Serin Lee,Andrew Barnum,Jennifer Dionne,Colin Ophus*

Main category: eess.IV

TL;DR: 提出一种完全自监督的隐式神经表示方法，用于电子断层扫描重建，无需训练数据即可处理缺失楔形伪影、数据未对齐和低剂量噪声问题


<details>
  <summary>Details</summary>
Motivation: 传统电子断层扫描重建算法存在缺失楔形伪影和数据未对齐问题，而现有的监督机器学习方法需要训练数据，难以在不同材料系统中泛化

Method: 使用隐式神经表示作为正则化器，通过姿态优化实现快速内联对齐，通过模型正则化实现缺失楔形修复和低剂量数据去噪，仅需单个数据集

Result: 在模拟和实验数据上验证了该方法能从信息有限的数据集中产生高质量断层图像，重建保真度高，用户输入和预处理需求最小

Conclusion: 基于INR的自监督重建方法提供高保真重建，用户输入和预处理需求少，可广泛应用于各种材料样本和实验参数
...
TL;DR: FlowSteer是一种无需重新训练或适配器的零样本图像修复方法，通过操作感知条件注入测量先验，在保持流模型生成能力的同时提高测量一致性


<details>
  <summary>Details</summary>
Motivation: 基于流的文本到图像模型在图像生成方面表现出色，但在图像修复任务中容易"漂移"而偏离测量真实性。现有方法需要特定数据的流或任务特定的适配器，计算量大且难以跨任务扩展

Method: FlowSteer是一种操作感知条件方案，在采样路径中注入测量先验，将冻结流的隐式指导与显式测量约束相结合。该方法利用流模型对噪声敏感的特性设计调度器

Result: 在超分辨率、去模糊、去噪和着色等任务中，FlowSteer在严格零样本设置下提高了测量一致性和身份保持能力，无需重新训练模型或适配器

Conclusion: FlowSteer虽然简单，但实现了更高保真度的重建图像，同时充分利用了流模型的丰富生成先验，为高效操纵现有流模型的生成能力提供了解决方案
...
### [129] [Does it take two to tango: Interaction between Credit Default Swaps and National Stock Indices](https://arxiv.org/abs/2512.07887)
*Yhlas Sovbetov,Hami Saka*

Main category: q-fin.ST

TL;DR: 该研究使用ARDL方法分析土耳其BIST-100指数与CDS价格在2008-2015年间的短期和长期互动关系，发现两者存在显著的负向关联，且受利率、通胀、汇率和政治事件等控制变量影响。


<details>
  <summary>Details</summary>
Motivation: 研究土耳其金融市场中股票指数与信用违约互换价格之间的动态关系，以理解土耳其企业高度美元负债的资本结构以及金融市场对政治不确定性的敏感性。

Method: 采用自回归分布滞后模型分析2008年1月至2015年5月期间BIST-100指数与CDS价格的关系，同时控制利率、通货膨胀率、汇率和政治事件等变量。

Result: 研究发现：1）CDS价格上升导致BIST指数下降；2）BIST指数上升降低CDS价格；3）利率和通胀对两者有显著影响；4）汇率变动对两者产生相反影响；5）负面政治事件对两者影响显著；6）市场存在向长期均衡收敛的调整机制。

Conclusion: 研究证实了土耳其企业高度美元负债的资本结构特征，金融市场对政治不确定性高度敏感，BIST指数与CDS价格在控制变量作用下会偏离但最终以适度速度收敛于长期均衡。
...
### [130] [Analysis of Contagion in China's Stock Market: A Hawkes Process Perspective](https://arxiv.org/abs/2512.08000)
*Junwei Yang*
...
<details>
  <summary>Details</summary>
Motivation: 探索中国股市中的传染效应，分析市场指数是否呈现趋势行为，以及行业指数之间是否存在相互影响

Method: 使用自激励和抑制性霍克斯过程拟合上证综指、深证成指、创业板指以及消费、医疗、金融等行业指数的日收益率数据，通过时空霍克斯过程和条件强度函数建模股价变动

Result: 识别出长期依赖性和趋势模式（包括上升、下降和超卖反弹趋势），发现高交易活动期间行业指数倾向于维持趋势，低活动期间则表现出强烈的行业轮动效应

Conclusion: 通过时空霍克斯过程成功建模股价变动，利用条件强度函数解释行业轮动现象，增进了对金融传染效应的理解
