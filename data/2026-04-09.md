...
TL;DR: 提出基于视觉-语言框架的食物营养分析方法，使用餐前餐后配对图像进行食物级别的营养分析，无需深度传感或多视角图像


<details>
  <summary>Details</summary>
Motivation: 现有基于图像的饮食评估方法大多依赖单一餐前图像，只能提供粗略的餐级估计，无法确定实际消耗量，且需要深度传感、多视角图像或显式分割等限制性输入

Method: 使用自然语言提示定位特定食物项目，直接从单张RGB图像估计重量；通过两阶段训练策略预测配对图像间的重量差异来估计食物消耗量

Result: 在三个公开数据集上评估，相比现有方法取得一致改进，为餐前餐后饮食图像分析建立了强基线

Conclusion: 提出的简单视觉-语言框架能够进行食物级别的营养分析，通过配对图像准确估计食物消耗量，为精准营养提供有效工具
...
TL;DR: 本文提出URMF框架，通过显式建模模态可靠性来解决多模态讽刺检测中的噪声问题，利用不确定性感知融合提升检测准确性和鲁棒性。
...
Method: 提出不确定性感知鲁棒多模态融合框架：1) 使用多头交叉注意力将视觉证据注入文本表示；2) 在融合语义空间中使用多头自注意力增强不一致性感知推理；3) 对文本、图像和交互感知的潜在表示进行统一单模态偶然不确定性建模；4) 利用估计的不确定性动态调节模态贡献；5) 设计包含任务监督、模态先验正则化、跨模态分布对齐和不确定性驱动自采样对比学习的联合训练目标。

Result: 在公开的多模态讽刺检测基准测试中，URMF持续优于强大的单模态、多模态和基于MLLM的基线方法，证明了不确定性感知融合在提高准确性和鲁棒性方面的有效性。

Conclusion: 通过显式建模模态可靠性并利用不确定性感知融合，URMF框架能够有效处理现实社交媒体中的噪声证据，显著提升多模态讽刺检测的性能和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 随着开源图像生成模型的进步，伪造视觉证据的风险增加。现有伪造检测模型在监控场景中表现不佳，因为监控图像中的篡改通常是局部、细微的，且场景复杂（多变视角、小目标、遮挡、低质量）。

Method: 通过多模态LLM驱动的流程生成具有法证价值的图像，实现语义感知的细粒度编辑。包含多种分辨率和编辑类型的篡改图像，使用多个图像编辑模型生成。
...
TL;DR: 该论文将陨石坑分析重新定义为实例级图像检索问题，提出了CraterBench-R基准数据集，发现自监督ViT模型在该任务上表现最佳，并提出了一种高效的实例-令牌聚合方法来平衡精度与存储效率。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法将陨石坑分析视为检测问题，但科学工作流程如目录去重、跨观测匹配和形态类比发现本质上是检索任务。现有方法未能满足这些实际科学需求。

Method: 1) 将陨石坑分析构建为实例级图像检索问题；2) 创建CraterBench-R基准数据集（约25,000个陨石坑身份）；3) 提出实例-令牌聚合方法：选择K个种子令牌，通过余弦相似度分配剩余令牌，聚合每个簇为单个代表性令牌；4) 设计两阶段流水线：单向量初选+实例-令牌重排序。

Result: 1) 自监督ViT模型（特别是领域内预训练）优于参数更多的通用模型；2) 保留多个ViT补丁令牌进行后期交互匹配显著提高准确率；3) 实例-令牌聚合在K=16时比原始令牌选择提升17.9点mAP，K=64时匹配所有196个令牌的精度但存储更少；4) 两阶段流水线恢复89-94%的完整后期交互准确率。

Conclusion: 将陨石坑分析重新定义为检索问题并采用实例-令牌聚合方法，能够在行星尺度上实现高效准确的陨石坑匹配，为科学工作流程提供了实用的解决方案。
...
TL;DR: 论文评估了视觉语言模型在从文化遗产图像推断结构化元数据（如创作者、起源、时期）方面的能力，发现当前模型表现不一致且存在文化偏差。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在文化遗产图像描述方面有所进步，但从视觉输入推断结构化文化元数据（创作者、起源、时期等）的任务仍未被充分探索，需要系统评估。

Method: 引入了一个多类别、跨文化的基准测试，并使用LLM-as-Judge框架评估视觉语言模型，该框架测量模型输出与参考注释的语义对齐程度。

Result: 结果显示模型只能捕捉碎片化信号，在不同文化和元数据类型上表现出显著的性能差异，导致预测不一致且缺乏充分依据。
...
TL;DR: 提出用于CBCT迭代重建算法的全自动参数优化框架，通过改进的乌鸦搜索算法和混沌初始化方案，无需参考重建即可确定最优参数，显著提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 迭代重建技术虽然能通过减少投影降低辐射剂量，但需要精确调整多个超参数，手动设置耗时且增加操作人员工作量。现有方法缺乏自动优化参数的能力。

Method: 提出全自动参数优化框架，包含：1) 改进的乌鸦搜索算法，具有集合依赖的局部搜索机制、搜索空间感知的全局搜索策略、目标驱动的局部-全局搜索平衡；2) 混沌对角线性均匀初始化方案，加速算法收敛。

Result: 在三个成像设备和四个真实数据集上评估，相比手动设置和原始CSA，平均适应度提升4.19%，在CHILL@UK和RPI_AXIS两个无参考质量指标上分别提升4.89%和3.82%。定性结果显示能保持精细细节。

Conclusion: 提出的框架在不同比较场景中表现出有效性和鲁棒性，能够自动优化CBCT迭代重建算法参数，显著提升重建质量，减少人工干预。
...
TL;DR: 论文提出DISSECT基准测试，通过五种输入模式诊断视觉语言模型中的感知-整合差距，发现开源模型在化学推理中存在明显的整合瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在视觉信息提取后，在下游推理中会丢失这些信息，形成"感知-整合差距"。传统基准测试将感知和整合混为一谈，无法暴露这种失败。

Method: 引入DISSECT诊断基准（12,000个问题，涵盖化学7,000和生物5,000），采用五种输入模式评估：视觉+文本、纯文本、纯视觉、人类先知、模型先知（VLM先描述图像再推理）。

Result: 1) 化学比生物更难利用语言先验，是真正的视觉推理测试；2) 开源模型从自身描述推理比从原始图像推理得分更高，暴露整合瓶颈；3) 闭源模型无此差距，显示感知与整合的衔接是能力差距的关键。

Conclusion: 模型先知协议是模型和基准无关的，可用于任何VLM评估后诊断整合失败。感知与整合的衔接是区分开源与闭源多模态能力的前沿。
...
TL;DR: 该综述系统回顾了视频生成技术的发展历程，从早期GAN到主流扩散模型，再到新兴的自回归模型和多模态技术，分析了各方法原理、优势局限，并探讨了多模态视频生成趋势及其在VR/AR、教育、自动驾驶等领域的应用前景。
...
Method: 首先系统梳理视频生成技术发展脉络：从早期GAN到主导的扩散模型，再到新兴的自回归模型和多模态技术。深入分析各方法的基本原理、关键进展和比较优势/局限。然后探讨多模态视频生成的新趋势，强调多样化数据类型的整合以增强上下文感知能力。

Result: 通过连接历史发展和当代创新，该综述为视频生成及其应用领域（虚拟/增强现实、个性化教育、自动驾驶模拟、数字娱乐、高级世界模型等）的未来研究提供了指导性见解。

Conclusion: 该综述填补了现有视频生成综述的空白，提供了从技术演进到多模态整合的全面视角，为这一快速发展领域的未来研究和应用方向提供了有价值的指导。
...
<details>
  <summary>Details</summary>
Motivation: 超声心动图在心血管疾病筛查和诊断中很重要，但自动化智能分析面临复杂心脏动力学和视图异质性的挑战。现有视觉语言模型方法存在模板捷径和虚假解释的脆弱性，需要更可靠、可解释的临床决策支持系统。

Method: 提出EchoTrust框架，采用证据驱动的Actor-Verifier架构。首先生成结构化中间表示，然后由不同角色进行分析，实现更可靠和可解释的决策过程。

Result: EchoTrust框架能够产生更可靠和可解释的决策，适用于高风险临床应用，解决了现有方法在超声心动图分析中的模板捷径和虚假解释问题。

Conclusion: EchoTrust通过证据驱动的Actor-Verifier框架为超声心动图视觉语言模型提供了更可信的推理能力，为高风险临床应用的自动化分析提供了可靠解决方案。
...
TL;DR: 本文提出基于提升理论的LiftFormer方法，通过构建中间子空间连接图像颜色特征与深度值，并增强边缘区域的深度预测，在单目深度估计任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计是从单张图像/视频估计深度图来表示场景3D结构的重要问题，但由于从2D到3D的映射是高度不适定的，现有方法存在挑战。特别是深度图中的边缘区域通常呈现急剧变化，容易预测错误，需要专门处理。

Method: 提出基于提升理论拓扑的LiftFormer方法：1) 构建深度导向几何表示(DGR)子空间，将深度值预测问题转化为子空间特征表示，使用线性相关向量按深度分箱提供冗余鲁棒表示；2) 构建边缘感知表示(ER)子空间，增强边缘区域的局部特征；3) 将图像空间特征转换到DGR子空间，使其直接对应深度值。

Result: 实验结果表明，LiftFormer在广泛使用的数据集上实现了最先进的性能。消融研究验证了所提出的两个提升模块在LiftFormer中的有效性。

Conclusion: 通过将深度估计问题转化为子空间特征表示，并专门处理边缘区域，LiftFormer方法有效解决了单目深度估计的挑战，在多个数据集上取得了优异的性能。
...
TL;DR: MorphDistill是一个两阶段框架，通过从多个病理基础模型中蒸馏知识到紧凑的CRC特异性编码器，用于结直肠癌生存预测，相比基线模型有显著性能提升。
...
Result: 在Alliance/CALGB 89803队列（n=424，III期CRC）中，MorphDistill达到AUC 0.68（SD 0.08），比最强基线（AUC 0.63）相对提升约8%；C-index为0.661，风险比为2.52。在外部TCGA队列（n=562）中C-index为0.628，表现出强大的泛化能力和临床亚组稳健性。
...
### [12] [Region-Graph Optimal Transport Routing for Mixture-of-Experts Whole-Slide Image Classification](https://arxiv.org/abs/2604.07298)
*Xin Tian,Jiuliu Lu,Ephraim Tsalik,Bart Wanders,Colleen Knoth,Julian Knight*

Main category: cs.CV

TL;DR: ROAM是一种用于计算病理学中全切片图像分类的空间感知混合专家MIL聚合器，通过容量约束的熵最优传输实现区域到专家的平衡分配，并在四个WSI基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前MIL聚合器将所有实例通过共享路径路由，限制了其在病理异质性中的专业化能力；而传统MoE方法中无约束的softmax路由可能导致专家利用高度不平衡，使混合方法退化为近单路径解决方案。

Method: ROAM将密集补丁包压缩为空间分箱的区域标记，通过容量约束的熵最优传输（Sinkhorn）实现区域到专家的分配，并引入图正则化的Sinkhorn迭代在空间区域图上扩散路由分配。

Result: 在四个WSI基准测试中使用冻结的基础模型补丁嵌入进行评估，ROAM性能与强MIL和MoE基线相当，在NSCLC泛化（TCGA-CPTAC）上达到外部AUC 0.845 ± 0.019。

Conclusion: ROAM通过空间感知的MoE-MIL聚合器解决了病理异质性和专家利用不平衡问题，通过容量约束的最优传输和图正则化实现了平衡的专家利用和空间一致的路由。
...
### [13] [PhysHead: Simulation-Ready Gaussian Head Avatars](https://arxiv.org/abs/2604.06467)
*Berna Kabadayi,Vanessa Sklyarova,Wojciech Zielonka,Justus Thies,Gerard Pons-Moll*

Main category: cs.CV

TL;DR: PhysHead：一种基于3D高斯分层表示的可动画头部化身方法，结合参数化头部网格和基于发束的头发，通过物理引擎模拟实现逼真的头发动态效果。


<details>
  <summary>Details</summary>
Motivation: 现有头部化身方法通常假设头发是刚性的，将头发表示为简单的外壳，无法捕捉其自然的体积行为，导致头发运动不真实。需要一种能够分离头发与头部、实现逼真头发动态效果的方法。

Method: 提出PhysHead混合表示：1) 使用3D参数化网格表示头部；2) 采用基于发束的头发表示，可直接用物理引擎模拟；3) 外观模型使用附着在头部网格和头发段上的高斯基元；4) 利用VLM模型生成动态训练序列中被遮挡区域的外观。

Result: 通过定量和定性研究证明，该方法能够合成物理上合理的头发运动，同时支持表情和相机控制。相比现有基线方法，能够生成更逼真的头发动态效果，如风吹头发等自然运动。

Conclusion: PhysHead通过混合表示方法成功解决了现有头部化身中头发刚性运动的问题，实现了逼真的头发动态效果，为创建具有真实头发行为的数字头像提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: OCTA图像具有稀疏的血管结构和强拓扑约束，现有的自监督学习方法（如掩码自编码器）主要针对密集自然图像设计，采用均匀掩码和像素级重建，难以有效捕捉血管几何特征

Method: 提出VAMAE框架：1）解剖学引导的掩码策略，利用血管性和骨架线索强调血管丰富区域；2）多目标重建，同时重建外观、结构和拓扑信息；3）在OCTA-500基准上进行评估

Result: 血管感知掩码和多目标重建相比标准掩码自编码基线带来一致改进，特别是在有限标注设置下，证明了几何感知自监督学习在OCTA分析中的潜力

Conclusion: VAMAE通过解剖学引导的掩码策略和多目标重建，有效捕捉血管连通性和分支模式，为OCTA图像分析提供了有效的自监督预训练框架，在有限监督条件下表现优异
...
TL;DR: HopS提出了一种用于部分标签场景下的提示学习方法，通过局部密度过滤和全局最优传输两种互补策略进行标签选择，在八个基准数据集上显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 在部分标签可用的情况下，提示学习的性能受到标签模糊性和监督信息不足的限制，需要一种能够有效处理标签选择问题的方法。

Method: 提出了Holistic Optimal Label Selection (HopS)方法，包含两个互补策略：1) 基于局部密度的过滤器，从最近邻候选集中选择高频标签并使用softmax分数识别最合理标签；2) 基于最优传输的全局选择目标，将均匀采样分布映射到批次中的候选标签分布，通过最小化期望传输成本确定最可能的标签分配。

Result: 在八个基准数据集上的广泛实验表明，HopS在部分监督下始终提升性能，并优于所有基线方法。

Conclusion: HopS通过局部和全局视角的互补策略提供了稳健的标签选择，为弱监督设置下的提示学习提供了实用解决方案，突出了整体标签选择的优势。
...
TL;DR: WeatherRemover：一个轻量级多天气图像恢复模型，采用UNet结构、门控机制和多尺度金字塔视觉Transformer，在恢复质量、参数效率、计算开销和内存使用之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 恶劣天气条件下拍摄的照片常因雨、雪、雾的干扰而出现模糊、遮挡和低亮度问题，严重影响后续计算机视觉任务性能。现有方法大多针对特定天气条件，少数多天气方法又忽视性能考虑，导致参数大、推理时间长、内存成本高。
...
Conclusion: WeatherRemover是一个轻量级多天气图像恢复模型，通过创新的架构设计在保持高质量恢复的同时实现了性能优化，为实际应用提供了有效的解决方案。
...
Method: 使用变分潜在瓶颈将输入编码到紧凑潜在空间，结合任务驱动的交叉熵目标和KL正则化训练。通过基于每个维度KL散度和梯度显著性的动态二进制掩码，抑制对目标任务无信息的潜在维度。
...
Conclusion: 提出的特征提取框架能有效抑制跨模型传输，保护数据不被未经授权模型重用，同时保持指定任务的性能。需要进一步评估对抗自适应攻击的鲁棒性。
...
Method: 1) 编码场景代表性关键帧作为结构先验；2) 编码每帧密集控制先验以保留细粒度结构和语义；3) 使用可控视频生成模型在时序和内容一致性下重建非关键帧；4) 开发颜色距离引导的关键帧选择算法自适应选择关键帧

Result: 实验结果表明CGVC在信号保真度和感知质量方面均优于先前的感知视频压缩方法

Conclusion: 提出的可控生成视频压缩范式通过多视觉条件引导生成，有效缓解了感知与保真度之间的困境，实现了更好的视频压缩性能
...
TL;DR: 提出了一种名为ISTS的新型水印范式，通过基于用户提示语义的动态注入策略和双向检测方法，有效抵抗针对文本到图像扩散模型水印的移除和伪造攻击。
...
Method: 提出ISTS（实例特定水印与双向检测）范式：1）基于用户提示语义动态控制注入时间和水印模式；2）采用新的双向检测方法增强水印检测的鲁棒性。

Result: 实验证明该方法在抵抗移除攻击和伪造攻击方面具有优越性，能够有效保护生成内容的版权和来源信息。

Conclusion: ISTS水印范式通过语义感知的动态注入和双向检测机制，显著提升了文本到图像扩散模型水印的安全性，为解决生成内容的版权保护问题提供了有效方案。
...
TL;DR: VDPP是一个视频深度后处理框架，通过纯几何细化而非完整场景重建，实现了高速（>43.5 FPS）的视频深度估计，能够即时集成任何图像深度模型。


<details>
  <summary>Details</summary>
Motivation: 当前端到端视频深度模型虽然性能优异，但紧密耦合的系统结构导致每当有更好的单图像深度估计器发布时，都需要较长的适应滞后。现有的后处理方法在速度、精度和实用性方面仍无法与端到端系统竞争。

Method: VDPP将范式从计算昂贵的场景重建转向有针对性的几何细化，在低分辨率空间中纯几何操作，通过密集残差学习驱动几何表示而非完整重建，采用RGB无关架构确保真正的可扩展性。

Result: VDPP在NVIDIA Jetson Orin Nano上达到>43.5 FPS的速度，匹配端到端系统的时间一致性，在速度、精度和内存效率方面提供优越的平衡，成为实时边缘部署的最实用解决方案。

Conclusion: VDPP通过重新定义视频深度后处理范式，实现了高速、准确且可扩展的视频深度估计，能够即时集成任何演进的图像深度模型，为实时边缘应用提供了最实用的解决方案。
...
Method: 提出RASR框架，包含三个模块：1) CSPR将视频解构为高级语义原语并从动态记忆库检索相关关联证据；2) DGMP结合领域先验驱动专家多模态大语言模型生成领域感知深度分析报告；3) MVDFF通过自适应门控机制整合多维特征实现鲁棒真实性判定

Result: 在FakeSV和FakeTT数据集上的广泛实验表明，RASR显著优于最先进的基线方法，实现了优异的跨领域泛化能力，整体检测准确率提升高达0.93%
...
TL;DR: 提出基于智能体驱动的视觉语言模型框架，用于甲骨文破译，通过组件识别、知识检索和关系推理来解决传统图像识别方法的"解释鸿沟"问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法将甲骨文破译视为封闭式图像识别问题，无法解决"解释鸿沟"：虽然单个字符通常独特且罕见，但它们由有限的、可重复的象形组件构成，这些组件具有可迁移的语义含义。

Method: 提出智能体驱动的视觉语言模型框架，整合VLM进行精确视觉定位和基于LLM的智能体，自动化执行组件识别、基于图的知识检索和关系推理链，实现语言准确的解释。

Result: 在三个不同任务的基准测试中，该框架相比基线方法产生更详细和精确的破译结果。同时引入了OB-Radix专家标注数据集，包含1,022个字符图像和1,853个细粒度组件图像。

Conclusion: 通过利用甲骨文的结构逻辑，提出的智能体驱动VLM框架能够更有效地破译古代甲骨文，为理解古代信仰、制度和文化提供新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有半稠密图像匹配方法存在两个长期问题：1）粗匹配阶段互最近邻匹配的过度排除问题使其难以处理图像间存在尺度差异的情况；2）细匹配阶段忽视了最终匹配的局部一致性，影响了鲁棒性。

Method: 1）粗匹配阶段：提出尺度感知匹配模块，利用分数矩阵中隐藏的线索指示尺度比例，有效处理尺度差异；2）细匹配阶段：将细匹配重新定义为级联流细化问题，引入新颖的梯度损失来鼓励流场的局部一致性。

Result: 大量实验表明，提出的匹配流程在下游任务中实现了鲁棒且准确的匹配性能，且尺度感知模块引入的开销极小。

Conclusion: 通过解决粗匹配的尺度差异问题和细匹配的局部一致性问题，本文提出的改进匹配流程显著提升了半稠密图像匹配的性能和鲁棒性。
...
TL;DR: DeSOPE是一个大规模6自由度变形物体数据集，包含26个常见物体类别的高保真3D扫描，每个物体有1个标准形态和3个变形配置，以及13.3万帧RGB-D数据和66.5万个姿态标注，用于研究物体变形对6D姿态估计的影响。


<details>
  <summary>Details</summary>
Motivation: 现有6D物体姿态估计方法大多假设物体是刚性或关节式的，但在实际应用中，物体常因磨损、碰撞或变形而偏离标准形状，导致现有方法失效。需要专门的数据集来研究变形物体的姿态估计问题。

Method: 1) 创建包含26个常见物体类别的高保真3D扫描数据集，每个物体有1个标准形态和3个变形配置；2) 构建包含13.3万帧RGB-D数据的数据集，通过半自动标注流程生成66.5万个姿态标注：先标注2D掩码，用物体姿态方法计算初始姿态，通过物体级SLAM系统优化，最后人工验证。

Result: 评估多个物体姿态估计方法发现，随着变形程度增加，性能显著下降，表明变形处理对实际应用至关重要。数据集已公开可用。

Conclusion: DeSOPE数据集填补了变形物体6D姿态估计研究的空白，揭示了现有方法在变形场景下的局限性，为开发更鲁棒的姿态估计方法提供了重要基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型依赖2D视觉先验，在复杂3D空间推理上表现不佳。现有方法要么需要昂贵的后训练，要么使用缺乏几何理解和视角灵活性的工具调用机制。

Method: 提出无需训练框架，引入基于显式3D重建的视觉思维链机制：1) 使用MLLM引导的关键词提取和多粒度掩码生成从单图像重建高保真3D网格；2) 利用外部知识库迭代计算最优相机外参并合成新视角，模拟人类视角转换。

Result: 在3DSRBench和Rel3D等主要基准测试中显著提升空间理解能力，超越专用空间模型和通用MLLMs（包括GPT-5.2和Gemini-2.5-Flash）。

Conclusion: 提出的无需训练框架通过显式3D重建和视觉思维链机制，有效解决了多模态大语言模型在复杂3D空间推理中的局限性，无需昂贵训练即可显著提升性能。
...
TL;DR: LiveStre4m：首个实时从无位姿稀疏多视角视频流进行新视角合成的前馈模型，比优化方法快38倍，仅需0.07秒/帧


<details>
  <summary>Details</summary>
Motivation: 现有动态场景表示方法需要已知相机参数且优化时间长（约2.67秒），无法满足实时直播需求。需要解决无位姿多视角视频的实时新视角合成问题。

Method: 1. 多视角视觉Transformer进行关键帧3D场景重建；2. 扩散-Transformer插值模块保证时间一致性和稳定流式传输；3. 相机位姿预测器直接从RGB图像估计位姿和内部参数，无需已知标定信息。

Result: 在1024×768分辨率下达到平均0.07秒/帧的重建时间，比优化方法快38倍；仅需两个同步无位姿输入流即可实现时间一致的新视角视频流；代码已开源。
...
TL;DR: 将静态视觉上下文学习模型转变为用户可交互的指导系统，通过视觉线索（涂鸦、点击、边界框）实现动态预测控制


<details>
  <summary>Details</summary>
Motivation: 现有视觉上下文学习模型虽然能适应新任务，但缺乏用户交互指导机制，无法在真实应用中让用户通过视觉线索主动引导模型预测

Method: 提出Interactive DeLVM方法，将用户交互直接编码到示例输入-输出对中，保持视觉上下文学习理念不变，无需微调即可支持未见过的交互方式

Result: 相比SOTA模型，在交互式分割上提升+7.95% IoU，定向超分辨率提升+2.46 PSNR，交互式对象移除降低-3.14% LPIPS，显著改善用户指导场景下的性能

Conclusion: 成功弥合了静态任务适应与用户交互之间的差距，为以用户为中心的视觉上下文学习提供了流体交互能力
...
TL;DR: VENUSS框架系统评估视觉语言模型在连续驾驶场景中的性能，发现现有模型准确率仅57%，远低于人类水平（65%），且对输入配置敏感
...
Method: 基于现有数据集提取驾驶视频的时间序列，生成结构化评估类别，比较25+个现有视觉语言模型在2600+个场景中的表现，分析输入配置（分辨率、帧数、时间间隔、空间布局、呈现模式）对性能的影响

Result: 即使顶级模型准确率也仅57%，远低于类似约束下的人类表现（65%）；模型擅长静态物体检测，但在理解车辆动态和时序关系方面存在显著能力差距

Conclusion: VENUSS提供了首个针对视觉语言模型在连续驾驶场景中性能的系统敏感性分析框架，揭示了输入配置对模型性能的重要影响，为未来研究建立了基准
...
TL;DR: FlowInOne：将多模态生成统一为纯视觉流程的框架，将所有输入转换为视觉提示，实现图像输入-图像输出的单一流程匹配模型，在文本到图像生成、布局引导编辑和视觉指令跟随等任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 挑战当前以文本为主导的多模态生成范式，该范式让语言主导视觉但无法在视觉中进行推理和创作。探索是否所有模态（文本描述、空间布局、编辑指令）都能统一到单一的视觉表示中。

Method: 提出FlowInOne框架，将多模态生成重新表述为纯视觉流程，将所有输入转换为视觉提示，实现图像输入-图像输出的单一流程匹配模型。引入VisPrompt-5M数据集（500万视觉提示对）和VP-Bench基准测试。

Result: FlowInOne在所有统一生成任务上达到最先进的性能，超越开源模型和商业系统，在指令忠实度、空间精度、视觉真实性和内容一致性方面表现优异。

Conclusion: 建立了完全以视觉为中心的生成建模新基础，在单一连续视觉空间中实现感知和创作的共存，消除了跨模态对齐瓶颈、噪声调度和任务特定架构分支。
...
TL;DR: FlowExtract：从ISO 5807标准化流程图中提取有向图的管道，解决制造业维护流程文档的可访问性问题


<details>
  <summary>Details</summary>
Motivation: 制造业设施中的维护程序通常以静态PDF或扫描图像中的流程图形式记录，这些文档编码了对资产生命周期管理至关重要的程序知识，但无法被现代操作员支持系统访问。现有的视觉语言模型在处理此类图表的连接拓扑重建方面存在困难。

Method: FlowExtract采用分离元素检测与连接性重建的管道设计：1）使用YOLOv8和EasyOCR进行标准域对齐的节点检测和文本提取；2）采用新颖的边缘检测方法，分析箭头方向并向后追踪连接线到源节点。

Result: 在工业故障排除指南上的评估显示，FlowExtract实现了非常高的节点检测性能，并在边缘提取方面显著优于视觉语言模型基线，为组织提供了实用的可查询程序知识表示路径。

Conclusion: FlowExtract为从标准化流程图中提取有向图提供了一种有效的解决方案，解决了制造业程序知识文档的可访问性问题，其实现已在GitHub上开源。
...
TL;DR: 论文提出EventFace框架，利用事件相机进行人脸识别，通过结合空间结构先验和时空动态特征，在有限数据下实现高性能识别，具有光照鲁棒性和隐私友好性。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有光照鲁棒性和隐私友好性，适合人脸识别应用。但由于事件流缺乏传统RGB图像稳定的光度外观，且缺乏专用数据集，需要开发能够建模刚性面部运动和个体面部几何的结构驱动时空身份表示方法。

Method: 提出EventFace框架：1) 构建EFace小规模事件人脸数据集；2) 使用LoRA从预训练RGB人脸模型迁移结构先验；3) 设计Motion Prompt Encoder显式编码时间特征；4) 使用Spatiotemporal Modulator融合时空特征。

Result: EventFace在评估基线中表现最佳：Rank-1识别率达到94.19%，等错误率(EER)为5.35%。在退化光照条件下比竞争方法更具鲁棒性，且学习到的表示具有更低的模板可重构性。

Conclusion: EventFace成功地将RGB人脸模型的结构先验与事件相机的时空动态特征相结合，在有限数据下实现了高性能的事件人脸识别，为隐私友好且光照鲁棒的生物识别系统提供了有前景的解决方案。
...
TL;DR: FedDAP提出了一种联邦学习框架，通过构建领域特定的全局原型来解决跨领域数据分布差异问题，使用相似性加权融合机制聚合同一领域内的本地原型，并通过领域内对齐和领域间分离来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界联邦学习场景中，客户端数据通常来自不同领域，导致严重的领域偏移问题，降低全局模型性能。现有基于原型的联邦学习方法存在两个关键局限：1）通常为每个类别构建单一全局原型，未保留领域信息；2）特征-原型对齐是领域无关的，强制客户端与全局原型对齐而不考虑领域来源。

Method: 提出FedDAP框架：1）通过相似性加权融合机制聚合同一领域内客户端的本地原型，构建领域特定的全局原型；2）在本地训练中，将本地特征与同领域原型对齐，同时鼓励与不同领域原型分离；3）这种双重对齐机制增强了本地层面的领域特定学习，使全局模型能够泛化到多样领域。

Result: 在三个不同数据集（DomainNet、Office-10和PACS）上进行了广泛实验，证明了所提框架在解决领域偏移挑战方面的有效性。代码已开源。

Conclusion: FedDAP通过构建领域特定全局原型和实现领域感知的特征-原型对齐，有效解决了联邦学习中的跨领域数据分布差异问题，提升了模型在多样化领域上的泛化能力。
...
TL;DR: 提出基于多模态大语言模型的免训练声源定位框架GAR，通过生成-分析-精炼三阶段实现音频视觉一致性验证，在单源和多源基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有声源定位方法大多依赖对比学习特征匹配，缺乏显式推理和验证机制，在复杂声学场景中效果有限。受人类元认知过程启发，利用多模态大语言模型的内在推理能力来解决这一问题。

Method: 提出生成-分析-精炼（GAR）三阶段框架：1）生成阶段产生初始边界框和音频分类；2）分析阶段通过开放集角色标注和锚点投票量化音频视觉一致性；3）精炼阶段应用自适应门控防止不必要的调整。

Result: 在单源和多源基准测试中展现出有竞争力的性能，证明了该免训练框架的有效性。

Conclusion: 利用多模态大语言模型的推理能力，通过生成-分析-精炼框架实现了无需训练的声源定位，为复杂声学场景下的定位问题提供了新思路。
...
TL;DR: RePL框架通过掩码重建识别和修正伪标签中的潜在错误，提升LiDAR语义分割中伪标签质量，在nuScenes和SemanticKITTI数据集上达到最先进性能
...
Conclusion: RePL通过改进伪标签质量有效解决了半监督LiDAR语义分割中的错误传播问题，理论分析和实验验证都证明了其有效性
...
TL;DR: VGGT-SLAM++是一个完整的视觉SLAM系统，利用VGGT的几何丰富输出，通过融合前端VO、DEM图构建和后端优化，实现大规模精确建图


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的SLAM系统（如VGGT-SLAM）主要依赖稀疏回环或全局Sim(3)约束，导致短期位姿漂移问题，需要恢复高频局部束调整来稳定轨迹
...
Result: 在标准SLAM基准测试中，VGGT-SLAM++达到最先进的精度，显著减少短期漂移，加速图收敛，并通过紧凑DEM瓦片和亚线性检索保持全局一致性

Conclusion: VGGT-SLAM++通过恢复高频局部束调整和空间校正后端，解决了Transformer-based SLAM中的短期漂移问题，实现了大规模精确建图且内存有界
...
TL;DR: 本文从监控系统视角重新审视物理对抗攻击，强调时间持续性、多模态传感、载体真实性和系统级目标等关键因素，而非孤立图像基准测试。


<details>
  <summary>Details</summary>
Motivation: 物理对抗攻击研究正从孤立图像基准转向更接近实际部署监控系统的场景。现有文献大多关注单帧检测抑制，而忽略了监控系统的实际需求：时间持续性、多模态传感（可见光-红外）、载体现实性和系统级目标。需要从监控系统视角重新评估攻击的有效性和威胁模型。

Method: 采用四部分分类法组织现有研究：时间持续性、传感模态、载体现实性、系统级目标。重点分析多目标跟踪、双模态可见光-红外规避、可控服装等最新进展，总结评估实践和未解决问题。

Result: 监控系统的鲁棒性不能仅从孤立单帧基准测试判断，必须作为随时间展开、跨传感器、受现实物理部署约束的系统问题来考察。多目标跟踪、双模态攻击等研究反映了该领域的转变。

Conclusion: 物理对抗攻击研究需要从监控系统视角重新定位，关注时间持续性、多模态传感、载体现实性和系统级目标等关键维度。未来研究应解决距离鲁棒性、相机管道变化、身份级指标和激活感知测试等未解决问题。
...
TL;DR: RefineAnything是一个多模态扩散模型，专门解决区域特定的图像细化问题，支持基于参考和无参考的细化，通过Focus-and-Refine策略和边界一致性损失实现高效、精确的局部细节恢复，同时严格保持背景不变。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型在处理局部细节时经常出现细节崩溃（如扭曲的文本、徽标和细结构），而现有的指令驱动编辑模型侧重于粗粒度语义编辑，要么忽略微妙的局部缺陷，要么无意中改变背景，特别是在感兴趣区域只占固定分辨率输入的一小部分时。

Method: 提出了RefineAnything多模态扩散细化模型，基于一个反直觉的观察：裁剪和调整大小可以在固定VAE输入分辨率下显著改善局部重建。采用Focus-and-Refine策略，通过将分辨率预算重新分配给目标区域来提高细化效果和效率，同时使用混合掩码粘贴回保证严格的背景保持。还引入了边界感知的边界一致性损失来减少接缝伪影并提高粘贴回的自然度。

Result: 构建了Refine-30K数据集（20K参考样本和10K无参考样本）和RefineEval基准测试。在RefineEval上，RefineAnything相比竞争基线实现了显著改进，达到了近乎完美的背景保持，为高精度局部细化建立了实用解决方案。

Conclusion: RefineAnything为解决区域特定图像细化问题提供了一个有效的多模态扩散模型，通过创新的Focus-and-Refine策略和边界一致性损失，在保持背景严格不变的同时实现了高质量的局部细节恢复，为高精度图像编辑提供了实用工具。
...
TL;DR: 提出了一种基于YOLOv5的多模态UI控件检测方法，通过集成GPT生成的文本描述来增强检测性能，在16,000张UI截图数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统基于像素的UI控件检测方法面临视觉模糊、设计多样性和缺乏上下文线索等挑战，需要更鲁棒和上下文感知的解决方案。

Method: 扩展YOLOv5为多模态模型，通过交叉注意力模块集成GPT生成的UI图像文本描述，将视觉特征与文本嵌入的语义信息对齐，探索了三种融合策略：逐元素加法、加权求和和卷积融合。

Result: 在包含23个控件类别的16,000张标注UI截图数据集上评估，所有融合策略均优于基线YOLOv5模型，其中卷积融合表现最佳，在语义复杂或视觉模糊类别上提升显著。

Conclusion: 结合视觉和文本模态能显著提升UI元素检测性能，特别是在视觉信息不足的边缘案例中，为软件测试、无障碍支持和UI分析等应用提供了更可靠的工具基础。
...
TL;DR: POS-ISP提出了一种序列级强化学习框架，将模块化ISP优化转化为全局序列预测问题，一次性预测整个模块序列及其参数，通过终端任务奖励优化，避免了中间监督和冗余计算。


<details>
  <summary>Details</summary>
Motivation: 现有方法在联合优化ISP模块序列和参数方面存在挑战：神经架构搜索存在训练-推理不匹配问题，而逐步强化学习则导致训练不稳定和高计算开销。需要一种更稳定高效的优化范式。

Method: POS-ISP采用序列级强化学习框架，将模块化ISP优化建模为全局序列预测问题。该方法通过单次前向传播预测整个模块序列及其参数，使用终端任务奖励进行优化，无需中间监督，减少了冗余执行。

Result: 在多个下游任务上的实验表明，POS-ISP在提升任务性能的同时降低了计算成本，证明了序列级优化作为任务感知ISP的稳定高效范式。

Conclusion: POS-ISP通过序列级强化学习框架，为任务感知ISP优化提供了一种稳定、高效的解决方案，克服了现有方法的局限性，在性能和计算效率方面都有显著提升。
...
Method: 提出Grounded Forcing框架，包含三个相互关联的机制：1) 双重记忆KV缓存，将局部时间动态与全局语义锚点解耦；2) 双重参考RoPE注入，将位置嵌入限制在训练流形内；3) 非对称邻近重缓存，通过邻近加权的缓存更新实现平滑语义继承。

Result: 大量实验表明，Grounded Forcing显著增强了长距离一致性和视觉稳定性，为交互式长视频合成建立了坚实基础。

Conclusion: Grounded Forcing通过将生成过程锚定在稳定的语义核心上，同时适应灵活的局部动态，为解决自回归视频合成的长期连贯性问题提供了有效框架。
...
TL;DR: NTIRE 2026挑战赛聚焦于比特流损坏视频恢复，旨在从解码产生严重时空伪影和内容失真的损坏比特流中恢复视觉连贯的视频内容。


<details>
  <summary>Details</summary>
Motivation: 随着视频编码和传输技术的发展，比特流损坏已成为实际应用中常见问题。解码损坏的比特流会产生严重的时空伪影和内容失真，影响视频质量和可用性。当前缺乏统一的评估基准来推动该领域研究。

Method: 挑战赛提供了一个公共基准数据集和评估协议，邀请研究团队提交恢复方法。基于近年来比特流损坏视频恢复的研究进展，在真实损坏设置下评估各种恢复方法。

Result: 论文总结了挑战赛的最终结果和主要技术趋势，展示了不同方法在比特流损坏视频恢复任务上的表现。挑战赛结果揭示了该新兴任务的难度和现有方法的局限性。

Conclusion: NTIRE 2026比特流损坏视频恢复挑战赛突显了这一新兴任务的挑战性，为未来在实用比特流损坏条件下的鲁棒视频恢复研究提供了有价值的见解和方向。
...
TL;DR: 本文揭示了多模态大语言模型作为内容审核器时面临的新型对抗攻击——"对抗性走私攻击"，该攻击利用人机能力差异，将有害内容编码为人类可读但AI无法识别的视觉格式，从而绕过自动检测。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型被广泛部署为自动化内容审核器，作者发现了一个关键安全威胁：对抗性走私攻击。这种攻击不同于传统的对抗性扰动（用于错误分类）和对抗性越狱（用于生成有害输出），而是利用人机能力差距，将有害内容编码为人类可读但AI无法识别的视觉格式，从而绕过自动检测系统。

Method: 作者将走私攻击分为两种途径：1）感知盲区攻击——干扰文本识别；2）推理阻断攻击——即使成功识别文本也抑制语义理解。为评估这一威胁，构建了SmuggleBench基准测试，包含1700个对抗性走私攻击实例。通过分析感知和推理两个维度，识别了三个根本原因：视觉编码器能力有限、OCR鲁棒性差距、领域特定对抗样本稀缺。初步探索了缓解策略，包括测试时缩放（通过思维链）和对抗训练（通过监督微调）。

Result: 评估结果显示，无论是专有模型（如GPT-5）还是开源模型（如Qwen3-VL）等最先进模型都容易受到这种攻击，攻击成功率超过90%。通过感知和推理两个维度分析漏洞，识别了三个根本原因：视觉编码器能力有限、OCR鲁棒性差距、领域特定对抗样本稀缺。

Conclusion: 对抗性走私攻击是多模态大语言模型内容审核面临的一个严重安全威胁，现有模型普遍存在漏洞。作者通过构建SmuggleBench基准测试揭示了这一威胁，并初步探索了缓解策略，为未来研究提供了重要基础。代码已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 图像压缩是现代视觉流水线的普遍组件，但压缩对对抗鲁棒性的影响尚未被充分理解。作者研究压缩表示中的攻击，发现压缩可能成为深度图像分类器的对抗放大器。

Method: 研究压缩表示中的直接攻击，提出压缩感知攻击方法。分析压缩导致的决策空间缩减效应，即压缩诱导不可逆的信息丢失变换，收缩分类边界并增加对扰动的敏感性。

Result: 在相同标称扰动预算下，压缩感知攻击比像素空间对应攻击更有效。通过标准基准和架构的广泛实验验证了这一发现，揭示了压缩在环部署设置中的关键脆弱性。

Conclusion: 压缩会放大对抗脆弱性，压缩感知攻击比传统像素空间攻击更有效。这揭示了压缩在环部署环境中的关键安全漏洞，需要重新评估压缩在安全关键应用中的使用。
...
TL;DR: 该研究系统审计了面部关键点检测模型中的年龄、性别和种族偏见，发现混淆视觉因素（特别是头部姿态和图像分辨率）对性能的影响远超人口统计属性。在控制这些混淆因素后，性别和种族差异消失，但年龄相关偏见仍然显著存在。


<details>
  <summary>Details</summary>
Motivation: 人机交互的公平性依赖于机器人感知模型的可靠性。虽然人口统计偏见在高级面部分析任务中已被广泛研究，但在面部关键点检测中的偏见尚未被探索。本研究旨在系统审计这一低层视觉任务中的偏见，以促进可信和公平的机器人感知系统。

Method: 引入受控统计方法来分离人口统计效应与混淆视觉因素（如头部姿态和图像分辨率），评估标准代表性模型，分析年龄、性别和种族偏见。

Result: 混淆视觉因素（特别是头部姿态和图像分辨率）对性能的影响远超人口统计属性。在控制这些混淆因素后，性别和种族差异消失，但年龄相关偏见仍然显著存在，对老年人群体影响更大。
...
TL;DR: 提出了一种针对混合自回归-扩散模型(MAR)的稳定强化学习框架，通过多轨迹期望和不确定性感知的token选择来解决扩散头带来的梯度噪声问题


<details>
  <summary>Details</summary>
Motivation: 强化学习在自回归和扩散模型中已有成功应用，但扩展到混合AR-扩散框架面临挑战，主要问题是扩散头会引入噪声梯度，导致训练不稳定和性能过早饱和

Method: 提出稳定RL框架：1) 多轨迹期望(MTE)，通过平均多个扩散轨迹来减少梯度噪声；2) 基于不确定性的top-k% token选择，避免过度平滑；3) 一致性感知token选择策略，过滤与最终生成内容对齐度低的AR token

Result: 在多个基准测试中，该方法相比基线GRPO和预RL模型，在视觉质量、训练稳定性和空间结构理解方面都有显著提升

Conclusion: 提出的稳定RL框架有效解决了混合AR-扩散模型中的训练不稳定问题，通过多轨迹期望和智能token选择策略显著提升了模型性能
...
TL;DR: 将分位数回归应用于基于卫星数据的现有树高估算模型，为风险敏感场景提供不确定性量化
...
Method: 通过分位数回归对现有树高估算模型进行微调，仅需对预测头进行少量修改，即可为现有模型提供统计校准的不确定性估计。

Result: 模型能够提供统计校准的不确定性估计，并且结果显示模型在更具挑战性的条件下（如地形复杂、植被异质性高）置信度较低，这与遥感领域的已知挑战相吻合。

Conclusion: 通过分位数回归可以有效地为现有树高估算模型添加不确定性量化能力，提高模型在风险敏感应用中的实用性，同时模型的不确定性估计能够反映遥感数据的固有挑战。
...
TL;DR: 首个生成式照片马赛克方法，使用扩散模型合成图块图像，克服传统基于匹配方法的限制
...
Method: 提出生成式照片马赛克框架，使用基于扩散的生成模型，通过参考图像条件化生成图块图像；采用低频条件扩散机制对齐全局结构同时保留提示驱动的细节；利用少样本个性化扩散生成用户特定或风格一致的图块

Result: 该方法能够生成语义表达丰富且结构连贯的照片马赛克，有效克服基于匹配方法的根本限制，无需大量图像集合即可生成个性化图块

Conclusion: 生成式照片马赛克方法通过扩散模型实现了语义表达和结构一致性的平衡，为照片马赛克创作提供了新的可能性，克服了传统方法的局限性
...
<details>
  <summary>Details</summary>
Motivation: 传统查找表方法在追求更高图像质量时，由于需要更大的感受野和位深度，导致索引空间呈指数增长，造成存储瓶颈，限制了在资源受限设备上的部署

Method: 1. 将插值和量化集成到单输入多输出的ECNN中，大幅减小索引空间和查找表大小；2. 集成残差学习降低对查找表位深度的依赖，提高训练稳定性并优先重建精细细节；3. 基于知识蒸馏的非均匀量化过程优化量化级别，减少存储同时补偿量化损失

Result: 广泛基准测试表明，该方法显著降低了存储成本（相比ECNN最多减少50倍），同时实现了更优的超分辨率质量

Conclusion: IQ-LUT成功解决了查找表方法在存储效率和图像质量之间的权衡问题，为资源受限设备上的高质量图像超分辨率提供了可行的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 基于学习的3D场景重建和物体补全方法需要大量配对数据（部分扫描+完整几何真值），但真实扫描系统获取这种数据成本高、耗时长，特别是需要被遮挡区域的准确真值时

Method: 在Unity中实现虚拟扫描框架，模拟真实扫描器行为（可配置分辨率、测量范围、距离相关噪声等）；采用基于射线的扫描而非直接网格采样，模拟传感器可见性和遮挡效应；结合全景图像为点云着色；集成程序化室内场景生成管道自动创建多样化房间布局和家具布置

Result: 创建了V-Scan数据集，包含合成室内扫描、物体级部分点云、基于体素的遮挡网格和完整几何真值，为基于学习的场景重建和物体补全方法提供有价值的监督数据

Conclusion: 提出的虚拟扫描框架能够高效生成逼真的合成3D扫描数据集，解决了真实数据采集的瓶颈问题，为3D场景重建和物体补全任务提供了可扩展的数据集创建方案
...
<details>
  <summary>Details</summary>
Motivation: 现有弱监督语义分割方法通常将语义识别和对象定位耦合在一起，导致模型只关注稀疏的判别区域。尽管基础模型潜力巨大，但现有方法仍遵循紧密耦合的优化范式，难以有效缓解伪标签噪声，且依赖耗时的多阶段重训练或不稳定的端到端联合优化。
...
TL;DR: DARE框架通过分布感知校正和空间集成，解决了扩散模型中语义重要token被忽视的问题，提高了文本到视频生成的语义对齐和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有条件扩散模型在推理时经常忽视语义重要的token，导致在无分类器引导下产生有偏或不完整的生成结果。这主要源于两个因素：训练数据中token频率的长尾分布导致的分布偏差，以及交叉注意力中语义重要token被信息量少的token所掩盖的空间错位问题。

Method: 提出了DARE统一框架，包含两个核心组件：1) 分布校正的无分类器引导(DR-CFG)，通过动态抑制低语义密度的主导token，鼓励模型更好地捕捉代表性不足的语义线索；2) 空间表示对齐(SRA)，根据token重要性自适应重新加权交叉注意力图并强制执行表示一致性，使语义重要token在生成过程中发挥更强的空间指导作用。

Result: 在多个基准数据集上的广泛实验表明，DARE能够持续提高生成保真度和语义对齐，相比现有方法取得了显著提升。

Conclusion: DARE框架通过从分布去偏和空间一致性两个角度改进扩散模型中的语义指导，有效解决了语义重要token被忽视的问题，提高了文本到视频生成的性能。
...
TL;DR: AnchorSplat提出了一种基于3D几何先验的锚点对齐高斯表示方法，用于场景级3D重建，显著减少所需高斯数量，提升计算效率和重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有前馈高斯重建模型采用像素对齐公式，将每个2D像素映射到3D高斯，使高斯表示与输入图像紧密耦合。这种设计限制了高斯表示的几何感知能力，且依赖于图像分辨率和视图数量。

Method: 1. 引入锚点对齐高斯表示，由3D几何先验（稀疏点云、体素或RGB-D点云）引导；2. 设计高斯细化器，通过少量前向传递调整中间高斯；3. 直接在3D空间中表示场景，实现与图像分辨率和视图数量无关的几何感知渲染。

Result: 在ScanNet++ v2 NVS基准测试中达到SOTA性能，相比先前方法具有更好的视图一致性，且使用显著更少的高斯基元。计算效率提升，重建保真度增强。
...
TL;DR: LIANet是一种基于坐标的神经表示方法，通过时空坐标重建卫星影像，可作为地理空间基础模型的用户友好替代方案，无需原始卫星数据即可微调下游任务。


<details>
  <summary>Details</summary>
Motivation: 传统地理空间基础模型需要大量数据访问和预处理，给终端用户带来负担。LIANet旨在消除这些开销，提供更便捷的EO数据表示方法。

Method: 提出LIANet（Location Is All You Need Network），一种基于坐标的神经表示，仅使用时空坐标即可重建卫星影像。预训练后，该表示可适应各种下游任务，无需访问原始卫星数据。

Result: 在不同大小的目标区域上预训练LIANet，微调下游任务（如语义分割、像素回归）的性能与从头训练或使用现有GFMs相当。

Conclusion: LIANet为地理空间分析提供了用户友好的替代方案，消除了数据访问和预处理负担，使终端用户能够仅基于标签进行微调，在保持竞争力的同时简化工作流程。
...
TL;DR: 该论文针对工业异常检测中正常样本定义模糊的问题，提出新场景和评估指标，并开发了RePaste方法，通过在迭代中重新粘贴高异常分数区域来增强学习，在MVTec AD基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统异常检测假设训练数据只包含正常样本，但在实际工业场景中，正常样本的定义往往模糊不清。例如，存在微小划痕或污渍但仍可接受的样本，或者设备升级后需要更高精度时将微小缺陷视为异常的情况。这些现实场景中的规范变化问题尚未得到充分讨论。

Method: 提出了RePaste方法，通过迭代方式重新粘贴前一步中具有高异常分数的区域到下一步的输入中，从而增强学习过程。该方法旨在处理正常样本的模糊性，并适应规范变化的需求。

Result: 在MVTec AD基准测试的新场景中，RePaste方法在提出的评估指标上取得了最先进的性能，同时保持了较高的AUROC和PRO分数。

Conclusion: 该研究解决了工业异常检测中正常样本定义模糊的现实问题，提出了新的评估场景和指标，并开发了有效的RePaste方法，为适应规范变化的实际应用提供了解决方案。
...
TL;DR: 该论文提出了一种改进的半监督语义分割方法，通过将标注图像的类别标签和对应区域粘贴到未标注图像及其伪标签图像上，并训练模型使未标注图像的预测更接近标注图像，从而提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 语义分割中像素级标注成本高昂，半监督学习利用少量标注图像和大量未标注图像来提升性能。传统方法ClassMix使用未标注图像的伪标签进行混合，存在标签不准确的问题，且标注和未标注图像之间存在数据质量差异，影响特征图质量。
...
Conclusion: 该方法通过改进数据混合策略和特征对齐，有效解决了半监督语义分割中伪标签不准确和数据质量差异的问题，显著提升了分割性能。
...
TL;DR: 该论文提出了一个用于跨医院放射学数据共享的效用保持去标识化管道（UPDP），通过生成过滤机制创建隐私过滤但保留病理信息的合成图像，结合ID过滤的报告，在保护隐私的同时保持诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 大规模放射学数据对开发稳健的医疗AI系统至关重要，但跨医院共享面临严重的隐私约束。现有去标识化研究主要关注移除可识别信息，但去标识化数据是否仍能保持大规模视觉语言模型训练和跨医院转移的效用尚未充分探索。

Method: 提出效用保持去标识化管道（UPDP）：1）编译隐私敏感术语黑名单和病理相关术语白名单；2）对放射学图像使用生成过滤机制，合成隐私过滤且保留病理信息的对应图像；3）结合ID过滤的报告，实现安全跨医院共享。

Result: 在公共胸部X光基准测试中，该方法有效移除隐私敏感信息同时保留诊断相关病理线索。在去标识化数据上训练的模型相比原始数据训练的模型保持竞争性诊断准确性，而身份相关准确性显著下降，证实有效隐私保护。在跨医院设置中，去标识化数据与本地数据结合可获得更好性能。

Conclusion: 该研究证明去标识化放射学数据可以在保护隐私的同时保持足够的诊断效用，支持大规模视觉语言模型训练和跨医院转移，为医疗AI系统开发提供了隐私保护的数据共享解决方案。
...
TL;DR: 提出一种域泛化方法，通过分离与域差异无关的类别信息和源域特定信息，结合SQ-VAE的量子向量吸收域差异，在血管和细胞核分割任务上取得优于传统方法的精度。


<details>
  <summary>Details</summary>
Motivation: 域泛化旨在使模型在新环境或数据集（未见域）上保持高精度。由于成像设备和染色方法等环境条件差异导致域差异，使得在特定数据集（源域）上训练的模型在不同数据集（目标域）上精度显著下降，因此需要开发不依赖域差异的分割方法。

Method: 1. 分离与域差异无关的类别信息和源域特定信息；2. 使用与域差异无关的信息学习分割目标（如血管和细胞核）；3. 利用Stochastically Quantized Variational AutoEncoder (SQ-VAE)中的量子向量吸收训练和测试数据之间的域差异。

Result: 在血管分割和细胞核分割数据集上的实验表明，该方法相比传统方法提高了分割精度。

Conclusion: 通过分离域差异无关信息并结合SQ-VAE的量子向量吸收域差异，实现了不依赖域差异的分割，在域泛化任务上取得了更好的性能。
...
TL;DR: INSPATIO-WORLD是一个实时框架，能从单参考视频恢复和生成高保真动态交互场景，通过时空自回归架构和联合分布匹配蒸馏技术，在空间一致性和交互精度上超越现有方法。
...
Method: 提出INSPATIO-WORLD框架，核心是时空自回归架构，包含隐式时空缓存和显式空间约束模块，并引入联合分布匹配蒸馏技术来克服合成数据依赖导致的质量下降。

Result: 在空间一致性和交互精度上显著优于现有SOTA模型，在WorldScore-Dynamic基准测试中实时交互方法排名第一，建立了从单目视频重建4D环境的实用导航流程。

Conclusion: INSPATIO-WORLD为解决空间一致性和实时交互性挑战提供了有效方案，为从单目视频重建的4D环境导航建立了实用管道。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将服装生成和虚拟试衣视为独立问题，限制了实际时尚工作流程的灵活性。同时，多源异构条件下的时尚图像合成存在挑战，现有方法依赖简单的特征拼接或静态分层注入，容易导致属性纠缠和语义干扰。

Method: 1. 提出特质路由注意力(TA)模块，利用专家混合机制动态将条件特征路由到最兼容的专家和生成层，实现纹理、形状、颜色等视觉属性的解耦注入。2. 开发自动化多视角偏好优化(MPO)管道，无需人工标注或任务特定奖励模型，通过内容保真度、文本对齐和感知质量评估器构建偏好数据，使用直接偏好优化(DPO)优化模型。

Result: 在服装生成和虚拟试衣基准测试上的广泛实验表明，VersaVogue在视觉保真度、语义一致性和细粒度可控性方面始终优于现有方法。

Conclusion: VersaVogue提供了一个统一的框架，能够同时支持服装设计和展示阶段的时尚合成任务，通过动态路由机制和自动化偏好优化解决了多条件可控时尚合成的关键挑战。
...
TL;DR: PhyEdit是一个利用显式几何模拟作为3D感知视觉指导的图像编辑框架，通过结合可插拔3D先验和2D-3D联合监督，提升物体操纵的物理准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成模型在精确空间操纵方面存在不足，经常出现物体缩放和定位错误，主要原因是缺乏显式机制来融入3D几何和透视投影。

Method: 开发PhyEdit框架，利用显式几何模拟作为上下文3D感知视觉指导，结合可插拔3D先验和2D-3D联合监督；创建RealManip-10K数据集和ManipEval评估基准。

Result: 实验表明该方法在3D几何准确性和操纵一致性方面优于现有方法，包括强大的闭源模型。

Conclusion: 通过显式几何模拟和3D感知指导，PhyEdit能够实现物理准确的物体操纵，为交互式世界模型的应用提供了更好的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在记录轨迹外的外推视角合成质量下降，因为外推姿态提供弱几何支持且缺乏密集目标视角监督。关键是在训练中显式暴露模型于轨迹外条件缺陷。

Method: 提出Geo-EVS框架，包含两个组件：1) 几何感知重投影(GAR)：使用微调VGGT重建彩色点云并重投影到观测和虚拟目标姿态，生成几何条件图；2) 伪影引导潜在扩散(AGLD)：在训练中注入重投影衍生的伪影掩码，让模型学习在缺失支持情况下恢复结构。

Result: 在Waymo数据集上，Geo-EVS提高了稀疏视角合成的质量和几何精度，特别是在高角度和低覆盖设置下。同时改善了下游3D检测性能。

Conclusion: Geo-EVS通过显式处理轨迹外条件缺陷，有效提升了外推式新视角合成的质量，减少了自动驾驶中对相机阵列的依赖，并提出了LiDAR投影稀疏参考协议用于无密集外推视角真值时的评估。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在生成逼真全身虚拟化身方面存在局限性，特别是在利用大规模真实世界视频数据训练3D扩散模型时面临挑战，因为视频通常只提供身体部位的局部观察，导致3D表示出现模糊或透明伪影。

Method: 1) 使用预训练的前馈虚拟化身重建模型作为可动画的3D标记器，将非结构化视频帧编码为结构化3D标记；2) 提出可见性感知扩散训练策略，用可学习标记替换无效区域，仅在有效区域计算损失；3) 在标记数据集上训练基于流的扩散模型。

Result: GenLCA能够生成多样化和高质量的虚拟化身，在生成和编辑结果方面大幅优于现有解决方案，实现了卓越的逼真度和泛化能力。

Conclusion: 该方法有效地利用大规模真实世界视频数据在3D空间中训练扩散模型，实现了逼真、可动画的全身虚拟化身生成和编辑，为3D内容创作提供了新的可能性。
...
TL;DR: Mem3R是一个流式3D感知模型，通过解耦相机跟踪和几何映射的混合内存设计，解决了长序列处理中的漂移累积和时间遗忘问题，在保持恒定GPU内存使用的同时显著提升了长序列性能。


<details>
  <summary>Details</summary>
Motivation: 现有循环模型在处理长视觉流时存在漂移累积和时间遗忘问题，因为压缩潜在内存的容量有限。需要一种能够保持时间一致性并高效处理长序列的流式3D感知方法。

Method: 提出混合内存设计：1) 相机跟踪使用基于轻量级多层感知器的隐式快速权重内存，通过测试时训练更新；2) 几何映射使用显式的基于令牌的固定大小状态。这种设计解耦了两个任务，支持现有的状态更新策略。

Result: 相比CUT3R，模型参数量从793M减少到644M，在500-1000帧序列上绝对轨迹误差降低达39%。同时保持恒定GPU内存使用和可比较的推理吞吐量，在视频深度估计和3D重建等下游任务上也有改进。

Conclusion: Mem3R通过解耦相机跟踪和几何映射的混合内存设计，有效解决了长序列流式3D感知中的漂移和时间遗忘问题，在保持效率的同时显著提升了性能，适用于机器人和增强现实等应用。
...
TL;DR: 不同DNN模型（领域专用模型和基础模型）的面部身份编码方式存在相似性，简单的仿射变换就能显著改善跨模型人脸识别性能。


<details>
  <summary>Details</summary>
Motivation: 探究不同DNN模型（包括领域专用模型和基础模型）是否以相似的方式编码面部身份，尽管它们在数据集、损失函数和架构上存在差异。

Method: 将不同DNN模型生成的面部图像嵌入视为点云，研究简单的仿射变换是否能够对齐不同模型的面部表示。

Result: 发现跨模型兼容性：低容量线性映射显著改善了跨模型人脸识别性能（包括人脸识别和验证任务），对齐模式在不同数据集间具有泛化性，且随模型家族系统变化。

Conclusion: 不同DNN模型在面部身份编码上存在表示收敛，这一发现对模型互操作性、集成设计和生物特征模板安全具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 现有动态数据剪枝方法在噪声标签设置下效果不佳，因为它们通常依赖逐样本损失作为排序标准。这可能导致错误地保留噪声样本（因其损失值较高），造成显著性能下降。

Method: 提出AlignPrune模块，引入基于损失轨迹的动态对齐分数（DAS）作为新标准，能更准确识别噪声样本。这是一个简单有效的即插即用模块，可无缝集成到现有动态剪枝框架中。

Result: 在五个广泛使用的基准测试上，针对各种噪声类型和剪枝比例进行实验，AlignPrune相比最先进的基线方法准确率提升最高达6.3%。

Conclusion: AlignPrune为噪声数据下的剪枝提供了通用解决方案，鼓励在真实场景中进一步探索学习。该方法不改变模型架构或训练流程，具有很好的实用性。
...
TL;DR: SUMI是一种模拟退化到增强的方法，通过学习反转低质量能量积分CT中的真实采集伪影，利用高质量光子计数CT作为参考，将常规CT图像提升到接近光子计数CT的质量水平。
...
Method: 提出SUMI方法：1）显式建模真实采集退化过程，将PCCT转换为临床合理的低质量对应物；2）学习反转这一退化过程；3）使用潜在扩散模型，在1046个PCCT上训练；4）使用在PCCT和405,379个EICT上预训练的自动编码器提取通用CT潜在特征。

Result: 1）构建了超过17,316个公开可用EICT增强到PCCT质量的数据集；2）在外部数据上，SUMI在SSIM和PSNR上分别优于最先进的图像翻译方法15%和20%；3）提高了放射科医生评定的临床效用；4）提升了下游病灶检测性能，灵敏度提高达15%，F1分数提高达10%。

Conclusion: 新兴成像技术可以通过有限的高质量扫描作为参考，系统地"蒸馏"到常规EICT中，SUMI方法为将先进成像技术推广到临床常规使用提供了有效途径。
...
TL;DR: 该论文提出了Gaussian Wrapping方法，为3D高斯泼溅引入可学习的法向和衰减公式，实现精确的水密网格提取，在DTU和Tanks and Temples数据集上达到SOTA。
...
Method: 1. 为每个高斯元素引入可学习的定向法向；2. 定义适应的衰减公式，推导出任意空间位置的法向场和占据场的闭式表达式；3. 引入一致性损失和专门的致密化策略，确保高斯元素包裹整个表面；4. 修改可微分光栅化器，输出连续模型的等值面深度；5. 提出Primal Adaptive Meshing进行感兴趣区域的任意分辨率网格化。

Result: 在DTU和Tanks and Temples数据集上达到了新的最先进水平，能够生成完整的水密网格，网格尺寸比同期工作小得多，并能恢复薄结构（如自行车辐条）。

Conclusion: Gaussian Wrapping方法为3D高斯泼溅提供了原理性的占据场定义，实现了高质量的网格提取，同时指出了标准表面评估协议中的偏差并提出了更严格的替代方案。
...
TL;DR: TC-AE是一种基于ViT的深度压缩自编码器架构，通过调整token空间来解决高压缩比下的潜在表示崩溃问题，实现更好的重建和生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过增加潜在表示的通道数来维持高压缩比下的重建质量，但这往往导致潜在表示崩溃，从而降低生成性能。TC-AE旨在从token空间的角度解决这一问题，避免依赖复杂的架构或多阶段训练方案。

Method: TC-AE采用两种互补的创新方法：1）研究token数量缩放，通过调整ViT中的patch大小在固定潜在预算下，将token到潜在表示的压缩分解为两个阶段以减少结构信息损失；2）通过联合自监督训练增强图像token的语义结构，获得更适合生成的潜在表示。

Result: TC-AE在深度压缩条件下实现了显著改进的重建和生成性能。

Conclusion: 该研究有望推动基于ViT的视觉生成tokenizer的发展，通过从token空间角度解决潜在表示崩溃问题，为深度压缩自编码器提供了新的设计思路。
...
TL;DR: MoRight是一个统一的视频生成框架，通过解耦运动建模实现用户指定的动作控制，支持分离控制物体运动和相机视角，并能学习运动因果关系。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要问题：1）将相机运动和物体运动纠缠在单一跟踪信号中；2）将运动视为运动学位移而不建模物体间的因果关系。这限制了用户对运动控制的灵活性和物理合理性。

Method: 提出MoRight框架：1）在静态规范视图中指定物体运动，通过时间跨视图注意力将其转移到任意目标相机视角，实现相机和物体运动的解耦控制；2）将运动分解为主动（用户驱动）和被动（后果）组件，从数据中学习运动因果关系。

Result: 在三个基准测试中展示了最先进的性能，包括生成质量、运动可控性和交互感知能力。用户可以在推理时提供主动运动让模型预测后果（正向推理），或指定期望的被动结果让模型恢复合理的驱动动作（逆向推理），同时自由调整相机视角。

Conclusion: MoRight通过解耦运动建模解决了现有方法的局限性，实现了更灵活、物理合理的运动控制视频生成，支持分离的相机和物体控制以及运动因果关系学习。
...
TL;DR: 利用临床对话音频进行抑郁症自动检测，GPT-OSS零样本方法表现最佳，结合医患双方对话比单方对话效果更好，仅需128个患者token即可实现有意义检测。


<details>
  <summary>Details</summary>
Motivation: 抑郁症在初级医疗中漏诊率高，而数字记录技术日益普及的临床对话录音为从自然对话中检测抑郁症提供了机会，需要探索低负担的补充筛查方法。

Method: 基于1,108个初级医疗音频记录（253例抑郁，855例非抑郁），比较了三种监督方法（Sentence-BERT+逻辑回归、LIWC+逻辑回归、ModernBERT）与零样本GPT-OSS方法，分析了单说话者与医患对话组合的效果，并测试了不同token数量下的检测性能。

Result: GPT-OSS零样本方法表现最佳（AUPRC=0.510，AUROC=0.774），LIWC+逻辑回归在监督模型中表现最好（AUPRC=0.500，AUROC=0.742）。医患对话组合优于单说话者配置，医生在抑郁对话中会语言镜像患者。仅需前128个患者token即可实现有意义检测（AUPRC=0.356，AUROC=0.675）。

Conclusion: 被动收集的临床音频可作为现有筛查流程的低负担补充，支持实时临床决策支持，医患对话组合提供了单方无法捕捉的附加信号。
...
TL;DR: 论文提出了一种结合指令拒绝和结构化弃权门的复合架构来控制大语言模型的幻觉问题，通过支持缺陷分数St来评估输出可靠性，在多种测试场景中验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生缺乏证据支持的虚假声明，这被框架化为输出边界处的错误分类问题，即模型将内部生成的补全内容当作有证据支持的内容输出。

Method: 提出复合干预方法：结合基于指令的拒绝和结构化弃权门。弃权门从三个黑盒信号计算支持缺陷分数St：自一致性(At)、释义稳定性(Pt)和引用覆盖率(Ct)，当St超过阈值时阻止输出。

Result: 在50个项目、五种认知体系和三个模型的受控评估中：单独机制均不足够；指令提示显著减少幻觉但仍有过度弃权；结构化门保持可回答准确性但会漏掉自信编造；复合架构实现高总体准确性和低幻觉率，同时继承了指令组件的过度弃权特性。
...
TL;DR: CGD-PD是一种轻量级测试时层，通过查询假设及其否定形式、投影到否定一致决策、以及使用证明驱动的消歧步骤来解决三值逻辑问答中LLM的否定不一致和认知未知问题，显著提升准确率。


<details>
  <summary>Details</summary>
Motivation: 现代大型语言模型在三值逻辑问答中存在两个常见失败模式：否定不一致（对H和¬H的回答违反确定性标签映射）和认知未知（即使前提集S蕴含一方，模型仍因不确定性预测Unknown）。需要解决这些问题以提高逻辑推理的准确性和一致性。

Method: 提出CGD-PD方法：1) 在H和机械否定的H上查询单个三值分类器；2) 将结果对投影到否定一致决策；3) 调用证明驱动的消歧步骤，使用有针对性的二元蕴含探针选择性地解决Unknown结果，平均只需4-5次模型调用。

Result: 在FOLIO基准测试的一阶逻辑领域，CGD-PD在多个前沿LLM上带来一致的性能提升，相对于基础模型的准确率相对提升高达16%，同时减少了Unknown预测。

Conclusion: CGD-PD是一种有效的轻量级测试时层，能够显著改善LLM在三值逻辑问答中的性能，通过解决否定不一致和认知未知问题，在保持效率的同时提升逻辑推理的准确性和一致性。
...
TL;DR: 开发了一个基于GLP-1受体激动剂糖尿病病例报告的文本时间序列语料库，评估了LLM自动提取时间线的能力，并展示了时间事件分析在糖尿病研究中的应用。


<details>
  <summary>Details</summary>
Motivation: 糖尿病病例报告中的临床病程描述通常使用难以在纵向建模中重用的语言表达，需要将自然语言时间信息转化为结构化时间序列数据。

Method: 构建了136篇PubMed开放获取单患者病例报告的文本时间序列语料库，将临床事件与其最可能的参考时间关联，评估LLM自动时间线提取与临床专家标注的金标准时间线的对比。

Result: 性能最佳的LLM（GPT5）在事件覆盖度（0.871）和症状、诊断、治疗、实验室检查及结果的时间序列可靠性（0.843）方面表现优异。下游时间事件分析显示GLP-1使用者比非使用者呼吸系统后遗症风险更低（HR=0.259，p<0.05）。

Conclusion: LLM能够有效从病例报告中提取结构化时间线信息，为临床研究提供可重用的时间序列数据，支持糖尿病等疾病的纵向建模和风险分析。
...
TL;DR: Text2DistBench是一个新的阅读理解基准，用于评估LLM从自然语言中推断分布知识的能力，基于YouTube电影和音乐评论，要求模型回答关于评论分布的问题。


<details>
  <summary>Details</summary>
Motivation: 大多数阅读理解基准关注可以通过定位具体文本证据回答的事实信息，但许多现实任务需要理解分布信息，如群体层面的趋势和偏好。当前缺乏评估LLM分布知识理解能力的基准。

Method: 构建Text2DistBench基准，使用真实世界的YouTube电影和音乐评论数据，提供实体元数据和相关评论，要求模型回答分布性问题，如估计正面/负面评论比例或识别最频繁讨论的话题。构建流程完全自动化并可随时间更新纳入新实体。

Result: 实验显示多个LLM显著优于随机基线，但性能在不同分布类型和特征间差异很大。模型在某些分布任务上表现良好，但在其他任务上仍有局限。

Conclusion: Text2DistBench揭示了当前LLM在分布性阅读理解方面的能力和局限性，为未来研究提供了实用且可扩展的测试平台。该基准有助于推动LLM在理解群体层面信息方面的发展。
...
Method: 整合多语言表示学习和参数高效微调技术（如低秩适应LoRA），开发概念性扩展模型来描述适应性能如何依赖于模型容量、适应数据规模和适应模块的表达能力。引入突厥语迁移系数(TTC)作为理论度量，包含突厥语之间的形态相似性、词汇重叠、句法结构和文字兼容性。

Result: 提出了一个理论框架，强调类型学相似性如何能够实现高效的多语言迁移，同时识别在极低资源场景下参数高效适应的结构限制。该框架为研究突厥语族内的跨语言迁移提供了系统化的方法。

Conclusion: 该论文为研究突厥语族内多语言大语言模型的跨语言迁移和参数高效适应提供了理论框架，通过引入TTC度量和概念模型，有助于理解语言相似性如何影响模型适应效率，并为低资源语言的多语言模型开发提供指导。
...
TL;DR: Tool-MCoT：一个用于内容安全审核的小型语言模型，通过工具增强的思维链数据微调，在保持推理效率的同时提升审核准确性
...
Method: 引入Tool-MCoT，一个用于内容安全审核的小型语言模型，通过LLM生成的工具增强思维链数据进行微调，学习有效利用外部工具来改进推理和决策

Result: 微调后的小型语言模型在性能上取得显著提升，能够有选择性地使用工具，在审核准确性和推理效率之间达到平衡，只在必要时调用工具

Conclusion: Tool-MCoT展示了小型语言模型通过工具增强微调可以在内容安全审核任务中实现高效且准确的性能，为可扩展部署提供了可行方案
...
TL;DR: 论文研究了用于预测用户下一个兴趣点的大语言模型演示选择策略，发现简单启发式方法在计算成本和预测准确性方面均优于复杂的嵌入方法。


<details>
  <summary>Details</summary>
Motivation: 尽管基于大语言模型的上下文学习已成为传统监督方法的有前景替代方案，但其效果严重依赖于演示选择。现有研究缺乏对不同演示选择策略的全面比较分析，需要为实际应用明确最佳实践。

Method: 全面评估现有演示选择方法（随机选择、嵌入选择、任务特定选择）以及更简单的启发式方法（地理邻近性、时间顺序、序列模式），在三个真实世界数据集上进行广泛实验。

Result: 启发式方法在计算成本和预测准确性方面始终优于更复杂、计算量大的嵌入方法。在某些场景下，使用这些简单启发式方法选择演示的大语言模型甚至优于现有的微调模型，且无需额外训练。

Conclusion: 对于基于大语言模型的兴趣点预测任务，简单启发式演示选择方法比复杂嵌入方法更有效，为实际应用提供了实用且高效的解决方案。
...
TL;DR: 基于LLM的框架从肿瘤科电子病历非结构化笔记中提取医学知识和表型，在乳腺癌表型提取任务上达到与传统本体方法相当的准确率


<details>
  <summary>Details</summary>
Motivation: 肿瘤科电子病历中大量有价值信息（如化疗结果、生物标志物、肿瘤位置等）存在于非结构化的医生笔记中，而非结构化字段，需要有效的信息提取方法

Method: 开发基于大语言模型（LLM）的框架处理医生笔记，提取医学知识和表型，特别关注肿瘤学领域，并以乳腺癌表型提取作为具体应用案例

Result: LLM框架在提取表型方面的准确率与传统的基于本体的方法（如NCIt本体标注器）相当，且训练后可轻松微调以适应其他癌症类型和疾病

Conclusion: LLM为基础的信息提取框架能够有效处理非结构化临床笔记，提取医学表型，性能与传统方法相当，且具有更好的适应性和可扩展性
...
TL;DR: DOVE是一个分布式的文化价值观对齐评估框架，通过比较人类文本分布与LLM生成输出来评估模型的文化价值观对齐程度，解决了现有基准的C³挑战。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在全球部署，对齐其文化价值观对于安全性和用户参与度至关重要。现有基准面临C³挑战：依赖判别式选择题格式、忽视亚文化异质性、与现实开放生成不匹配。

Method: DOVE使用率失真变分优化目标从10K文档构建紧凑的价值码本，将文本映射到结构化价值空间以过滤语义噪声。使用不平衡最优传输测量对齐度，捕捉文化内分布结构和亚群体多样性。

Result: 在12个LLM上的实验显示，DOVE实现了优越的预测有效性，与下游任务达到31.56%的相关性，同时每个文化仅需500个样本即可保持高可靠性。

Conclusion: DOVE提供了一个有效的分布式评估框架，能够直接评估LLM的文化价值观对齐，解决了现有基准的局限性，为全球部署的LLM文化对齐提供了更好的评估工具。
...
TL;DR: 研究大型语言模型在编程教育中生成可追溯、基于证据的解释，提出基于言语行为理论的提示方法提升检索增强生成系统的源忠实度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型生成的自然语言解释虽然具有说服力，但缺乏可验证性，用户难以确认解释中的主张是否有证据支持。在可解释人工智能领域，这促使研究者关注忠实性和可追溯性，即解释的主张能否基于明确来源并追溯回源。研究聚焦于编程教育中的检索增强生成，其中教科书提供权威证据。

Method: 研究基准测试了6个大型语言模型在90个基于三本编程教科书的Stack Overflow问题上的表现，通过源忠实度指标量化源忠实性。基于Achinstein的言语行为解释理论，提出了言语行为宏观规划作为源忠实解释的描述性设计原则，并通过言语行为链提示（CoI）实现，该方法将查询扩展为驱动检索的隐含解释性问题。

Result: 非检索增强生成模型的源忠实度中位数为0%，基线RAG系统的源忠实度仍然较低（22-40%，取决于模型）。CoI方法在所有模型中实现了统计显著的源忠实度提升（最高达63%），尽管绝对忠实度仍然中等，且某些模型的提升较弱或不显著。用户研究显示这些提升不会损害用户满意度、相关性或感知正确性。

Conclusion: 言语行为链提示方法能有效提升检索增强生成系统在编程教育中的源忠实度，为生成基于证据、可追溯的解释提供了有前景的途径，同时保持用户满意度。然而，绝对忠实度仍有提升空间，需要进一步研究改进。
...
TL;DR: LLM在心理健康咨询中幻觉检测准确率低（仅52%），提出结合人类专业知识和LLM的框架，通过五个分析维度提取可解释特征，显著提升检测性能


<details>
  <summary>Details</summary>
Motivation: 随着LLM驱动的聊天机器人在心理健康服务中部署增多，检测幻觉和遗漏对用户安全变得至关重要。现有LLM-as-a-judge方法在高风险医疗环境中表现不佳，细微错误可能带来严重后果

Method: 提出一个框架，整合人类专业知识与LLM，在五个分析维度提取可解释的领域知识特征：逻辑一致性、实体验证、事实准确性、语言不确定性和专业适当性。使用传统机器学习模型在这些特征上进行训练

Result: 在自定义数据集上幻觉检测达到0.717 F1，在公共基准上达到0.849 F1；遗漏检测在两个数据集上达到0.59-0.64 F1。相比LLM法官仅52%的准确率有显著提升

Conclusion: 在高风险心理健康应用中，结合领域专业知识与自动化方法比黑盒LLM判断更可靠、更透明，为安全部署提供了更好的评估方案
...
TL;DR: 提出STDec解码方法，通过时空稳定性指导扩散大语言模型解码，显著提升推理速度同时保持性能


<details>
  <summary>Details</summary>
Motivation: 当前扩散大语言模型解码器大多采用全局置信度阈值，没有显式建模局部上下文和跨步预测的一致性，限制了解码效率

Method: 提出时空稳定性指导的解码方法STDec，包括空间感知解码（基于邻近解码状态动态生成自适应阈值）和时间感知解码（对预测一致的token放宽解码阈值）

Result: 在文本推理和多模态理解基准测试中，STDec显著提升吞吐量同时保持可比的任务性能；在MBPP基准上使用LLaDA模型实现最高14.17倍加速

Conclusion: STDec是一种无需训练的解码方法，兼容缓存加速技术，通过利用扩散大语言模型解码中的时空稳定性，有效平衡了推理速度与性能
...
TL;DR: 该研究提出了一种针对阿拉伯语医疗文本生成的严重程度感知加权损失函数，通过动态调整不同临床严重程度病例的损失权重，显著提升了阿拉伯语大语言模型在医疗问答任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 传统微调方法对所有医疗案例一视同仁，忽视了临床严重程度的差异。在医疗场景中，严重病例的错误具有更高的临床风险，因此需要一种能够优先处理临床关键交互的方法。

Method: 提出严重程度感知加权损失函数，利用软严重程度概率在优化过程中动态调整token级损失贡献。使用微调的AraBERT分类器自动生成严重程度标签和概率分数，仅在损失层面进行加权，不改变模型架构。在MAQA阿拉伯语医疗数据集上评估了10个不同架构和参数规模的阿拉伯语大语言模型。

Result: 严重程度感知优化相比标准交叉熵微调取得了显著提升：AraGPT2-Base从54.04%提升到66.14%，AraGPT2-Medium从59.16%提升到67.18%，Qwen2.5-0.5B从57.83%提升到66.86%，峰值性能达到67.18%。相比未微调的基线，改进幅度最高达12.10%。

Conclusion: 严重程度感知微调方法能够在不改变模型架构的情况下，通过动态调整损失权重优先处理临床关键交互，在多个阿拉伯语大语言模型上实现了稳健且架构一致的性能提升，为医疗文本生成任务提供了有效的优化策略。
...
TL;DR: 语音语言模型中的上下文学习研究：探索语言和声学特征对文本转语音任务中上下文学习的影响，发现语速是关键因素，而音高和强度影响较小


<details>
  <summary>Details</summary>
Motivation: 上下文学习在纯文本语言模型中已得到广泛研究，但在语音领域仍相对未被探索。本研究旨在探究语言和声学特征如何影响语音语言模型中的上下文学习能力。

Method: 研究聚焦于文本转语音任务，从两个角度分析上下文学习：1) 模型从演示中推断任务的准确性（生成正确的语音内容）；2) 模型在输出中模仿演示语音声学特征的程度。同时研究了归纳头在语音上下文学习中的作用。

Result: 发现语速对上下文学习性能有强烈影响，并且会在输出中被模仿；而音高范围和强度对性能影响很小，且不会被一致地复制。此外，归纳头在语音上下文学习中起因果作用：消融前k个归纳头会完全消除模型的上下文学习能力。

Conclusion: 语音语言模型中的上下文学习与文本模型有相似之处（如归纳头的作用），但也存在重要差异（如语速是关键声学特征）。这为理解语音语言模型的学习机制提供了新见解。
...
TL;DR: 本文提出了一种基于严重程度的课程学习策略用于阿拉伯语医学文本生成，通过从轻症到重症的渐进式训练，提升模型对复杂高风险病例的处理能力。


<details>
  <summary>Details</summary>
Motivation: 现有阿拉伯语医学文本生成方法通常假设训练样本重要性均匀分布，忽视了临床严重程度的差异，这种简化会阻碍模型正确处理复杂或高风险病例的能力。

Method: 提出基于严重程度的课程学习策略：1) 将数据集按严重程度分为轻度、中度和危重三个等级；2) 采用基于规则的标注方法对MAQA数据集进行严重程度标注；3) 在微调过程中逐步从轻症病例过渡到重症病例，让模型先学习基本医学模式再处理复杂场景。

Result: 在Medical Arabic Question Answering (MAQA)数据集子集上的实验表明，该方法在所有测试模型中均带来一致的性能提升：相比基线模型提升约4%-7%，相比传统微调方法提升3%-6%。

Conclusion: 基于严重程度的课程学习策略能有效提升阿拉伯语医学文本生成的质量，特别是在处理复杂高风险病例时表现更优，为医疗AI系统提供了更精细的训练方法。
...
TL;DR: 研究探讨了语言模型在使用潜在连续思维链推理时是否真正利用叠加原理，发现在从头训练、微调和免训练三种机制中，只有从头训练的模型表现出叠加原理的使用迹象。


<details>
  <summary>Details</summary>
Motivation: 尽管理论上认为潜在连续思维链推理具有叠加原理（同时维护多个候选解的能力），但实际语言模型是否真正利用这一特性尚不清楚。研究旨在验证语言模型在三种不同训练机制下是否以及如何使用叠加原理进行推理。

Method: 研究采用三种机制：1）免训练机制，将潜在思维构建为词嵌入的凸组合；2）微调机制，在基础模型上适配以产生潜在思维；3）从头训练机制，完全使用潜在思维训练模型解决特定任务。使用Logit Lens和实体级探测分析内部表示。

Result: 只有从头训练的模型表现出使用叠加原理的迹象。在免训练和微调机制中，叠加原理要么崩溃要么完全未被使用，模型反而发现了捷径解决方案。这归因于两个互补现象：i）自然语言数据的预训练使模型偏向在最后层承诺特定词元；ii）容量对模型偏好哪种解决方案有巨大影响。
...
TL;DR: 本文提出了一种无需训练的方法ART，通过替换LLM浅层中的均匀注意力模式为局部注意力模式来减少幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在问答等任务中经常产生看似合理但错误或不相关的信息（幻觉），现有方法未能充分探索注意力模式与幻觉之间的关系。

Method: 提出注意力替换技术（ART），分析LLM各层和各注意力头的注意力分数分布，发现浅层主要依赖均匀注意力模式，将其替换为局部注意力模式，使模型更关注相关上下文。

Result: ART在多种LLM架构上显著减少了幻觉，证明了其有效性和泛化能力，无需微调或额外训练数据。

Conclusion: 通过替换浅层的均匀注意力模式为局部注意力模式，可以有效减少LLM的幻觉问题，这为理解注意力机制与幻觉关系提供了新视角。
...
TL;DR: 本文提出了一种识别西班牙语临床文本中有毒习惯命名实体的方法，在ToxHabits共享任务中，通过多种LLM方法（零样本、少样本、提示优化）探索，发现GPT-4.1的少样本提示效果最佳，在测试集上F1分数达到0.65。


<details>
  <summary>Details</summary>
Motivation: 针对西班牙语临床文本中有毒习惯命名实体识别任务，特别是在ToxHabits共享任务中，需要有效检测和分类物质使用和滥用提及，以支持临床案例报告分析。

Method: 探索了多种大型语言模型（LLM）应用方法，包括零样本学习、少样本学习和提示优化技术，重点评估了GPT-4.1在少样本提示下的表现。

Result: 在测试集上获得了0.65的F1分数，表明该方法在非英语语言（西班牙语）的命名实体识别任务中具有良好性能。

Conclusion: GPT-4.1的少样本提示方法在西班牙语临床文本的有毒习惯命名实体识别任务中表现最佳，为非英语语言的临床文本分析提供了有前景的解决方案。
...
TL;DR: 论文评估LLM在长文本理解任务中的表现，通过比较人类和LLM生成的小说摘要，发现LLM在叙事理解上存在差异，特别是倾向于强调文本结尾部分。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM的上下文长度有所增长，但其在长文本中整合信息的能力并未同步提升。通过比较人类和LLM生成的小说摘要，可以评估模型是否能够模仿人类对文本的概念性参与模式。

Method: 将150个人类撰写的小说摘要句子与其参考的具体章节进行对齐，展示了对齐任务的难度。然后为每个参考文本生成并对齐9个最先进LLM的摘要，比较人类和模型生成的摘要。

Result: 研究发现人类和模型生成的摘要之间存在风格差异，以及人类和LLM在叙事中分配注意力的方式不同，模型倾向于强调文本的结尾部分。比较人类叙事参与和模型注意力机制揭示了叙事理解下降的原因。

Conclusion: LLM在长文本叙事理解上仍存在局限，特别是在概念性参与和注意力分配方面与人类存在差异。研究为未来改进提供了目标，并发布了数据集支持后续研究。
...
<details>
  <summary>Details</summary>
Motivation: 解决阿拉伯语等少数语言在数字公平方面的差距，证明阿拉伯语性能不足主要源于专业化不足而非架构限制

Method: 采用稀疏MoE架构，四阶段思维链蒸馏方案，融入阿拉伯语语言验证和区域伦理规范，使用372M token的80/20阿拉伯语-英语混合训练数据

Result: 在Open Arabic LLM Leaderboard七个基准测试中获得最高平均分，在MadinahQA、AraTrust、AlGhafa、ALRAGE等任务上创下SOTA或接近SOTA记录，超越GPT-5.1

Conclusion: 稀疏MoE架构结合文化导向的思维链蒸馏和双语数据策略，为低资源语言提供了成本效益高的SOTA性能实现路径，建立了主权和领域特定语言技术的可复制框架
...
TL;DR: 研究通过GCA数据集评估视觉语言模型的内省一致性，发现模型会系统性违反自己设定的决策规则，而人类参与者则能保持规则一致性。
...
Method: 引入Graded Color Attribution (GCA)数据集，包含三种条件的线描图：世界知识重着色、反事实重着色和无颜色先验的形状。让VLMs和人类参与者建立颜色标签的像素覆盖阈值，然后比较这些规则与后续颜色归属决策的一致性。

Result: 模型系统性违反自己的内省规则（如GPT-5-mini在强颜色先验对象上违反率近60%），而人类参与者保持规则一致性。VLMs是优秀的颜色覆盖估计器，但在最终响应中公然违背自己的推理。世界知识先验系统性降低模型忠实度，这与人类认知模式不同。

Conclusion: VLM推理失败不是难度驱动的，而是内省自我知识校准错误。这对高风险部署有直接影响，挑战了VLM推理能力的基本假设。
...
TL;DR: 本文提出基于Transformer的方法解决SympTEMIST命名实体识别和实体链接任务，通过RoBERTa-BiLSTM-CRF模型进行NER，使用跨语言SapBERT XLMR-Large进行实体链接，发现知识库选择对准确性影响最大。
...
Conclusion: 基于Transformer的方法能有效处理SympTEMIST的NER和EL任务，但实体链接性能主要受知识库质量影响，未来可探索更好的知识库选择和匹配策略。
...
TL;DR: ATR4CH是一个针对文化遗产文本的五步法LLM知识提取框架，通过维基百科案例验证，实现了高质量的结构化知识图谱构建。
...
Method: ATR4CH采用五步法：基础分析、标注模式开发、管道架构、集成优化和全面评估，结合标注模型、本体框架和LLM提取，使用三种LLM（Claude Sonnet 3.7、Llama 3.3 70B、GPT-4o-mini）构建顺序管道。

Result: 方法成功提取复杂文化遗产知识：元数据提取F1 0.96-0.99，实体识别F1 0.7-0.8，假设提取F1 0.65-0.75，证据提取F1 0.95-0.97，论述表示G-EVAL 0.62。较小模型表现竞争性，支持成本效益部署。
...
<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统中LLM的通信存在输出冗长、上下文过载和计算成本高的问题。现有方法主要从说话者角度压缩信息，但难以适应不同听众和识别相关信息。受人类沟通中允许听众打断并表达意见的启发，需要更灵活的通信机制。

Method: 提出可中断通信框架，允许听众打断当前说话者。通过提示实验发现当前LLM往往过于自信、过早打断，因此提出基于预估未来奖励和成本的学习方法，预测合适的打断时机。

Result: 在多种多智能体场景（2智能体文本猜词游戏、3智能体会议安排、3智能体辩论）中评估，HANDRAISER相比基线减少32.2%通信成本，同时保持相当或更优的任务性能。学习的打断行为可泛化到不同智能体和任务。

Conclusion: 可中断通信框架能有效减少多智能体系统中LLM的冗余通信，降低计算成本，同时保持任务性能。该方法为多智能体通信优化提供了新思路，具有泛化能力。
...
TL;DR: 提出一个上下文感知且可调控的阿拉伯语方言机器翻译框架，通过规则数据增强构建大规模平行语料库，实现跨方言和社会语域的可控翻译生成。


<details>
  <summary>Details</summary>
Motivation: 当前阿拉伯语机器翻译系统往往忽视方言多样性，将方言输入同质化为现代标准阿拉伯语，且用户对目标方言的控制有限。需要解决方言敏感翻译中的准确性与方言保真度之间的权衡问题。

Method: 采用规则数据增强管道将3000句种子语料扩展为57000句平行数据集，覆盖8种地区变体。通过微调mT5-base模型并结合轻量级元数据标签，实现跨方言和社会语域的可控生成。

Result: 高资源基线系统（如NLLB）获得更高BLEU分数（13.75）但倾向于现代标准阿拉伯语，方言特异性有限；而本模型BLEU分数较低（8.19）但输出更符合目标地区变体。定性评估显示方言对齐度显著改善（4.80/5 vs. 1.0/5）。

Conclusion: 标准机器翻译指标在方言敏感任务中存在局限性，需要开发能更好反映阿拉伯语机器翻译中语言多样性的评估方法。本框架为可控方言翻译提供了可行方案。
...
TL;DR: Evo-L2S框架通过进化模型融合将长到短推理问题转化为多目标优化，在保持或提升准确率的同时将推理轨迹长度减少50%以上


<details>
  <summary>Details</summary>
Motivation: 当前推理模型虽然能解决复杂问题，但需要长推理链导致计算开销大；现有的训练自由模型融合方法依赖固定超参数的标量化算术方法，既脆弱又需要次优妥协

Method: 将L2S推理问题构建为多目标优化挑战，采用进化模型融合技术，通过熵基子集采样技术大幅降低适应度评估开销

Result: 在1.5B、7B和14B参数规模的六个数学推理基准测试中，Evo-L2S能将生成的推理轨迹长度减少超过50%，同时保持甚至提升原始推理模型的问题解决准确率

Conclusion: Evo-L2S框架通过进化模型融合和多目标优化，有效解决了长到短推理问题，在保持准确性的同时显著减少推理长度，为高效推理提供了新方法
...
TL;DR: ValueGround是一个评估多模态大语言模型文化价值视觉基础能力的基准，通过对比图像对测试模型能否基于国家文化倾向选择匹配的图像选项，发现模型在视觉化选项下的表现比纯文本下降约7%。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型的文化价值评估几乎都是纯文本的，不清楚模型在面对视觉化选项时能否基于文化条件做出判断。文化价值不仅通过语言表达，也通过视觉场景和日常社会实践体现。

Method: 基于世界价值观调查问题构建ValueGround基准，使用最小对比图像对代表对立回答选项，控制无关变量。给定国家、问题和图像对，模型必须选择最匹配该国价值倾向的图像，且无法访问原始文本选项。

Result: 在6个MLLM和13个国家上的测试显示：平均准确率从纯文本设置的72.8%下降到视觉化选项时的65.8%，尽管选项-图像对齐准确率达92.8%。更强模型更稳健，但所有模型都容易发生预测反转。

Conclusion: ValueGround为研究文化条件价值判断的跨模态转移提供了受控测试平台，揭示了多模态模型在视觉化文化价值判断方面的挑战和局限性。
...
TL;DR: 该论文研究了在LLM-based ASR系统中，使用少量语音数据来缓解语音投影器与语言模型之间的模态差距问题，发现仅需10%的目标域语音数据就能达到与传统ASR微调相当或更好的性能。


<details>
  <summary>Details</summary>
Motivation: 传统端到端ASR系统依赖成对的语音-文本数据进行领域适应。基于LLM的ASR架构通过投影模块连接语音编码器和大型语言模型，可以使用纯文本数据进行适应，但这引入了模态差距，因为LLM没有暴露在语音投影器产生的噪声表示中。研究者希望探索少量语音数据是否能缓解这种不匹配。

Method: 比较了三种策略：1）纯文本适应；2）成对语音-文本适应；3）混合批处理（MB），结合前两种方法。在领域内和领域外设置下进行实验，特别关注使用少量目标域语音数据（如10%，少于4小时）的效果。

Result: 实验表明，即使是有限的语音数据也能持续改善性能。特别值得注意的是，仅使用10%目标域语音（少于4小时）的混合批处理方法，其词错误率与传统ASR使用完整数据集进行微调相当或更好，这表明少量语音数据提供了强大的模态对齐信号。

Conclusion: 少量语音数据可以有效缓解LLM-based ASR系统中的模态差距问题，仅需目标域10%的语音数据就能达到传统方法的性能，为资源受限的领域适应提供了高效解决方案。
...
Method: 构建了MedConclusion数据集，包含570万篇PubMed结构化摘要，每篇实例将摘要的非结论部分与作者撰写的原始结论配对，提供自然监督。数据集包含期刊级别的元数据（如生物医学类别和SJR），支持跨生物医学领域的子组分析。评估了多种LLMs在结论和摘要提示设置下的表现，使用基于参考的指标和LLM-as-a-judge进行评分。

Result: 研究发现：1）结论写作与摘要写作在行为上存在明显区别；2）在当前自动指标下，强模型的表现仍然紧密聚集；3）评判者身份可以显著改变绝对分数。MedConclusion为研究科学证据到结论推理提供了可重复使用的数据资源。

Conclusion: MedConclusion是一个大规模、高质量的生物医学结论生成数据集，填补了LLMs在科学证据到结论推理研究中的资源空白。该数据集支持跨领域分析，揭示了结论生成与摘要生成的不同特性，为未来研究提供了重要基础。
...
TL;DR: 该研究针对Whisper模型在普什图语语音识别上的缺失问题，比较了四种微调策略，发现传统全微调效果最佳，在CommonVoice数据集上达到21.22%的词错误率，并确定了whisper-small在113小时数据量下的实用最优解。


<details>
  <summary>Details</summary>
Motivation: 普什图语虽然是CommonVoice最大的语言收集之一，但并未包含在Whisper的预训练语料中，导致现成模型无法使用。所有Whisper模型在处理普什图语音频时都会输出阿拉伯语、达里语或乌尔都语文字，词错误率超过100%。

Method: 研究比较了四种微调策略：1) 传统全微调；2) LoRA（秩64）；3) 冻结编码器（2/6层）；4) 多阶段乌尔都语到普什图语迁移学习。将传统微调扩展到whisper-small和whisper-large-v3-turbo模型，使用CommonVoice普什图语v24数据集（113小时）。

Result: 传统全微调在CV20上达到21.22%的WER，优于LoRA（33.36个百分点）、冻结编码器（14.76个百分点）和乌尔都语迁移（44.56个百分点）。在CV24上，whisper-small达到24.89% WER，whisper-large-v3-turbo达到23.37% WER。在线数据增强相比匹配训练带来7.25个百分点的WER改进。

Conclusion: 传统全微调是普什图语语音识别的最佳策略，whisper-small在113小时数据量下是实用最优解。主要错误模式包括词尾后缀混淆（阳性-ay vs 阴性-a）和涉及普什图语特有辅音/ts/的卷舌音替换。微调后的检查点和评估脚本已在HuggingFace发布。
...
TL;DR: GRAPE是一种全局剪枝策略，针对稀疏混合专家模型，通过跨层冗余感知动态分配剪枝预算，相比均匀剪枝方法显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 稀疏混合专家模型虽然通过激活部分专家提高了效率，但大量专家参数仍导致显著内存消耗。现有剪枝方法通常在各层均匀分配预算，忽略了稀疏MoE中出现的异构冗余问题。

Method: 提出GRAPE（全局冗余感知专家剪枝）策略，这是一种全局剪枝方法，基于跨层冗余动态分配剪枝预算，而不是在各层均匀分配。

Result: 在Mixtral-8x7B、Mixtral-8x22B、DeepSeek-MoE、Qwen-MoE和GPT-OSS等模型上的实验表明，在相同剪枝预算下，GRAPE始终实现最佳平均性能。在论文报告的三个主要模型中，平均准确率比最强局部基线平均提高1.40%，最高增益达2.45%。

Conclusion: GRAPE通过全局冗余感知的剪枝策略，有效解决了稀疏MoE模型中的异构冗余问题，在保持相同剪枝预算的同时显著提升了模型性能，为大型语言模型的高效部署提供了有效解决方案。
...
TL;DR: LLMs作为智能体时存在随机采样能力缺陷，无法将内部概率估计映射到随机输出，这是智能体系统的一个关键但未满足的基本要求。


<details>
  <summary>Details</summary>
Motivation: 智能体系统经常需要从分布中采样，而LLMs作为智能体时无法可靠地将内部概率估计映射到随机输出，这是一个未被充分认识但关键的失败点。

Method: 通过跨多个模型系列、模型大小、提示风格和分布的严格实证分析，评估LLMs的随机采样能力，特别是测试它们将随机种子转换为目标分布的能力。

Result: 前沿模型虽然能将提供的随机种子转换为目标分布，但直接从特定分布采样的能力存在根本性缺陷，LLMs无法可靠地将内部概率估计映射到随机输出。

Conclusion: 可靠的随机采样是LLMs作为智能体的基本但未满足的要求，这是智能体系统的一个关键限制，需要专门解决。
...
Method: 创建GlobalLies多语言并行数据集，包含440个虚假信息生成提示模板和6,867个实体，涵盖8种语言和195个国家。通过人工标注和大规模LLM-as-a-judge评估，分析最先进模型生成的数十万个内容，研究LLM在不同国家和语言背景下的虚假信息生成行为。

Result: 研究发现虚假信息生成存在系统性差异：LLM在许多低资源语言和低人类发展指数（HDI）国家中传播虚假信息的程度显著更高。现有缓解策略保护不均衡：输入安全分类器存在跨语言差距，检索增强的事实核查因信息可用性不平等而在不同地区表现不一致。

Conclusion: LLM在不同语言和国家背景下生成虚假信息的行为存在系统性差异，现有缓解策略存在保护不均衡的问题。GlobalLies数据集的发布旨在支持开发更有效的全球虚假信息传播缓解策略。
...
<details>
  <summary>Details</summary>
Motivation: 失踪人员和儿童安全调查依赖于多种格式的案例文档（结构化表格、公告式海报、叙述性网页资料），这些文档在布局、术语和数据质量上存在差异，阻碍了快速分类、大规模分析和搜索规划工作流程。

Method: 提出Guardian Parser Pack系统，包含：1）多引擎PDF文本提取与OCR备用方案；2）基于规则的数据源识别与特定源解析器；3）模式优先的协调与验证；4）可选的大语言模型辅助提取路径，包含验证器引导的修复和共享地理编码服务。

Result: 在75个手动对齐案例中，LLM辅助路径的提取质量显著高于确定性比较器（F1=0.8664 vs. 0.2578）。在517个解析记录中，LLM路径提高了关键字段完整性（96.97% vs. 93.23%）。确定性路径速度更快（平均0.03秒/记录 vs. 3.95秒/记录）。所有LLM输出都通过了初始模式验证。

Conclusion: 研究结果支持在高风险调查环境中，在模式优先、可审计的流程中受控使用概率性AI技术，为调查工作提供了有效的文档处理解决方案。
...
TL;DR: 提出了一种新的GEC任务——编辑影响评分，通过嵌入关联图捕捉编辑间的潜在依赖关系，使用困惑度评分估计每个编辑对句子流畅度的贡献，在多个数据集和语言上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有GEC系统评估方法存在局限性：一个句子可能有多个有效修正，现有评估设置不能完全适应多样化的应用场景，且基于人工判断的元评估方法难以扩展到大规模数据集。

Method: 提出了评分编辑影响的新任务，引入基于嵌入关联图的评分框架。该图捕捉编辑间的潜在依赖关系和语法相关编辑，将它们分组为连贯的组，然后进行基于困惑度的评分来估计每个编辑对句子流畅度的贡献。

Result: 在4个GEC数据集、4种语言和4个GEC系统上的实验表明，该方法始终优于一系列基线方法。进一步分析显示，嵌入关联图能有效捕捉编辑间的跨语言结构依赖关系。

Conclusion: 提出的编辑影响评分任务和基于嵌入关联图的评分框架能够自动评估GEC系统中编辑的重要性，解决了现有评估方法的局限性，并在多语言场景中表现出良好的性能。
...
TL;DR: SciDC是一种将学科专业知识与强约束结合的LLM生成方法，通过自动将灵活知识转化为多层标准化规则来约束模型在领域任务上的生成，在科学任务中平均提升12%准确率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具备丰富的知识储备和任务解决能力，但仍面临严重的幻觉问题，阻碍实际应用。科学理论和规则能有效指导人类操作者行为，但LLM尚未通过训练或提示充分利用这些高度凝练的知识。
...
TL;DR: 研究发现推理模型在答案确定后仍继续生成大量文本，提出检测-提取差距概念，并开发了基于自由延续的早期退出方法，显著减少计算成本同时提高准确性。


<details>
  <summary>Details</summary>
Motivation: 现代推理模型在答案已经确定后仍继续生成长文本，造成计算资源浪费，且存在检测与提取之间的性能差距。

Method: 通过分析自由延续与强制提取的差异，提出黑盒自适应早期退出方法，利用自由延续进行检测和提取，在答案确定时提前终止生成。

Result: 实验显示52-88%的思维链标记在答案可恢复后产生，BAEE方法能减少70-78%的串行生成，同时提高1-5个百分点的准确性，对思考模式模型增益达5.8个百分点。

Conclusion: 模型存在检测-提取差距，利用自由延续进行早期退出能显著减少计算成本并提高准确性，为高效推理提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示词压缩方法依赖顺序token删除，计算成本高且可能包含冗余信息，需要更高效、可控的压缩方案来提升大语言模型的上下文推理效率

Method: 提出DiffuMask扩散框架，集成分层shot-level和token-level剪枝信号，通过迭代掩码预测实现快速并行提示词剪枝，每步可掩码多个token

Result: DiffuMask显著加速压缩过程，实现高达80%提示长度缩减，在领域内、跨领域和跨模型设置中保持或提升准确性，提供可调的内容保留控制

Conclusion: DiffuMask提供了一个通用且可控的提示词压缩框架，促进大语言模型中更快、更可靠的上下文推理
...
TL;DR: 论文提出RAG系统的反馈适应性问题，引入修正延迟和反馈后性能两个评估指标，展示了训练方法的延迟与可靠性权衡，并提出无需训练的PatchRAG方法实现即时修正。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统评估主要关注静态假设下的整体准确性，忽略了系统在部署中通过用户或专家反馈进行修正的动态适应过程。需要研究系统在接收反馈后如何有效、快速地将修正传播到未来查询。

Method: 提出反馈适应性问题设置，引入两个评估指标：修正延迟（反馈提供到行为改变的时间延迟）和反馈后性能（反馈后在语义相关查询上的可靠性）。提出PatchRAG方法，一种无需重新训练的最小推理时实现，通过即时反馈整合实现快速修正。

Result: 研究表明基于训练的方法在延迟修正和可靠适应之间存在权衡关系。PatchRAG方法在提出的评估框架下展示了即时修正能力和强大的反馈后泛化性能。
...
TL;DR: 提出多任务提示蒸馏与分解框架，从21个临床NLP源任务学习共享元提示，以极少的可训练参数（<0.05%）适应新任务，在多个临床NLP任务类型上优于LoRA和单任务提示调优。
...
Method: 开发了多任务提示蒸馏与分解框架：1）从21个不同的临床源任务学习单一共享元提示；2）通过分解机制将元提示适应到未见目标任务；3）使用少于0.05%的可训练参数实现任务适应。

Result: 在5种临床NLP任务类型（命名实体识别、关系抽取、问答、自然语言推理、摘要）的10个目标数据集上评估，使用三个骨干模型（LLaMA 3.1 8B、Meditron3 8B、gpt-oss 20B）。框架比LoRA性能高1.5~1.7%（参数少几个数量级），比单任务提示调优高6.1~6.6%。gpt-oss 20B在临床推理任务上表现最佳。零样本和少样本性能显示共享提示表示具有更好的可迁移性。

Conclusion: 多任务提示蒸馏与分解框架能够以极少的参数实现高效的临床NLP任务适应，显著优于现有方法，展示了共享提示表示在跨任务迁移中的优势，为大规模临床NLP系统部署提供了更高效的解决方案。
...
TL;DR: G-Defense：基于图增强的防御框架，利用未经验证报告进行细粒度可解释假新闻检测，通过子声明分解、RAG证据检索和防御式推理实现高精度检测和直观解释


<details>
  <summary>Details</summary>
Motivation: 现有可解释假新闻检测方法效率低且难以处理突发新闻，而基于大语言模型的方法可能因引用未验证报告引入不准确性。需要为声明的所有方面提供可理解解释以帮助公众验证准确性。

Method: 提出图增强防御框架G-Defense：1) 将新闻声明分解为子声明并构建依赖关系图；2) 对每个子声明使用检索增强生成技术检索关键证据并生成竞争性解释；3) 基于图的防御式推理模块评估整体真实性；4) 提示大语言模型生成直观解释图。

Result: 实验结果表明G-Defense在真实性检测和解释质量方面均达到最先进性能。

Conclusion: G-Defense框架能够有效利用未经验证报告进行细粒度可解释假新闻检测，为声明的各个方面提供可理解解释，在检测准确性和解释质量方面均表现优异。
...
TL;DR: 该研究使用对齐的Word2Vec空间和图基邻域分析，考察波斯诗歌中词语意义的历史性和关系性变化，发现语义变化更适合用语义邻域的重连而非抽象漂移来描述。


<details>
  <summary>Details</summary>
Motivation: 波斯诗歌中的意义既是历史的又是关系的，词语在文学传统中持续存在，同时通过变化的邻居、修辞框架和诗歌声音改变其力量。传统方法仅将语义变化建模为向量位移，忽略了局部语义结构的变化。

Method: 使用对齐的Word2Vec空间结合图基邻域分析，跨越多个世纪和主要诗人。将词汇历史视为局部语义图的重连：邻居的获得与丢失、桥梁角色的转移以及社区间的移动。分析聚焦20个目标词，以5个反复出现的参考词（地球、夜晚、两个酒术语、心）为锚点。

Result: 不同词语展现出不同的变化模式：夜晚更具时间敏感性，地球更具诗人敏感性，心尽管图角色移动但仍显示连续性。两个酒术语凸显了探针敏感性：一个广泛且语义分散，另一个更窄且更稳定。词汇审计确认了历史驱动术语、诗人特定用法以及需要谨慎处理的稀疏神秘词汇。

Conclusion: 波斯诗歌中的语义变化更适合用邻域重连而非抽象漂移来捕捉。对于数字人文，这种方法将局部结构恢复到计算分析中，支持更接近文学实践的解释：持续性、迁移、中介和选择性转变。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要针对直接视觉问答任务进行优化，导致"黑盒"系统无法充分利用大语言模型推断反应机理的能力。需要一种能够优先考虑推理过程的化学视觉理解模型。

Method: 1. 采用细粒度视觉分析，明确识别化学描述符（如官能团）；2. 实施跨模态逆向工程策略和严格过滤流程，构建包含76万高质量样本的大规模推理-描述数据集；3. 采用三阶段训练框架系统构建模型感知和推理能力。

Result: ChemVLR在化学视觉理解任务上实现了最先进的性能，超越了领先的专有模型和领域特定的开源基线模型。全面的消融研究验证了训练策略和数据生成设计的有效性。

Conclusion: 通过优先考虑推理过程并生成显式可解释的推理路径，ChemVLR为复杂视觉化学问题提供了更透明和可靠的解决方案，代码和模型权重将开源。
...
Method: 提出自适应提示结构分解(aPSF)框架：1) 使用架构模型发现任务特定的提示结构作为语义因子；2) 进行干预式单因子更新：通过干预因子级评分估计每个因子的边际贡献，通过错误引导的因子选择将更新路由到当前主要失败源。

Result: 在多个高级推理基准测试中，aPSF优于包括原则感知优化器在内的强基线，平均准确率提升高达+2.16个百分点，在MultiArith上减少45-87%的token成本，并在1步内达到峰值验证性能。

Conclusion: aPSF通过因子分解和干预式更新实现了更高效、可控的API-only提示优化，为提示工程提供了新的结构化方法。
...
TL;DR: 研究人员创建了一个名为TEC的数据收集平台和数据集，记录人类在解决复杂问题时的试错过程，包括多轮尝试轨迹和错误反思，发现人类在试错能力上显著优于大语言模型。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统缺乏有效的试错能力，现有方法主要依赖研究者设计的简单启发式规则，性能提升有限。核心问题是缺乏适当的数据——现有模型无法从人类实际试错过程的详细记录中学习。

Method: 开发了一个数据标注平台和相应的TEC数据集，平台记录用户在多轮尝试中的完整轨迹，并收集他们在收到错误反馈后的反思。通过该平台记录了46名参与者在58个任务上的问题解决过程，共获得5,370条试错轨迹和41,229个网页上的错误反思。

Result: 人类在试错过程中达到的准确率显著高于大语言模型，这表明人类在试错能力上比LLMs更有效。TEC平台和数据集为理解人类试错行为和开发更强大的AI系统提供了宝贵基础。

Conclusion: TEC平台和数据集填补了AI试错学习的数据空白，为研究人类试错行为和开发更智能的AI系统提供了重要资源。平台和数据集已公开可用。
...
TL;DR: 论文研究了LLM生成的SQL查询的结构可靠性，发现即使执行结果正确，LLM也会为相同输入生成结构多样的查询，且这种差异常由表层输入变化触发。作者提出了SQLStructEval框架来通过AST分析程序结构，并发现通过编译式管道在结构化空间中生成查询能提高执行准确性和结构一致性。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在Text-to-SQL基准测试中表现强劲，但LLM生成的SQL程序是否具有结构可靠性仍不清楚。现有评估主要关注执行准确性，而忽略了程序结构的一致性这一重要维度。

Method: 提出了SQLStructEval框架，通过规范抽象语法树（AST）表示来分析程序结构。在Spider基准上进行实验，分析LLM生成的SQL查询的结构行为。进一步探索了通过编译式管道在结构化空间中生成查询的方法。

Result: 现代LLM经常为相同输入生成结构多样的查询，即使执行结果正确；这种结构差异常由表层输入变化（如释义或模式表示）触发。通过结构化空间生成查询能同时提高执行准确性和结构一致性。

Conclusion: 结构可靠性是评估基于LLM的程序生成系统的一个关键但被忽视的维度。在结构化空间中生成查询可以改善LLM生成SQL的结构一致性和准确性。
...
TL;DR: Luwen是一个基于Baichuan基础模型构建的开源中文法律语言模型，通过持续预训练、监督微调和检索增强生成技术，在法律任务上表现出色。
...
Method: 基于Baichuan基础模型，采用三种关键技术：1）在大规模法律语料库上进行持续预训练；2）使用精心策划的法律指令数据进行监督微调；3）集成全面法律知识库的检索增强生成。

Result: 在五个代表性法律任务（法律判决预测、司法考试、法律文本摘要、法律条文问答、司法决策推理）上，Luwen超越了多个强基线模型，证明了该方法在将通用语言模型适配到法律领域的有效性。

Conclusion: 通过专门的法律领域适配技术，Luwen模型成功地将通用语言模型转化为有效的法律语言模型，为法律领域的自然语言处理应用提供了有前景的解决方案。
...
TL;DR: 论文研究了LLM推理范式对任务性能的影响，发现不同范式在不同任务上表现差异显著，没有单一范式占优。提出基于轻量级路由器的选择-解决方法，能显著提升平均准确率。


<details>
  <summary>Details</summary>
Motivation: 研究LLM任务性能提升是来自模型本身还是推理范式，探索不同推理范式在不同任务上的表现差异，寻找最优的范式选择策略。

Method: 比较六种推理范式（Direct、CoT、ReAct、Plan-Execute、Reflection、ReCode）在四个前沿LLM和十个基准测试上的表现（约18,000次运行）。提出基于嵌入的轻量级路由器，在回答每个任务前选择最合适的推理范式。

Result: 推理范式在某些任务上帮助显著（如ReAct在GAIA上比Direct提升44pp），但在其他任务上会损害性能（如CoT在HumanEval上降低15pp）。学习型路由器将平均准确率从47.6%提升到53.1%，优于最佳固定范式（50.3%），恢复了高达37%的oracle差距。

Conclusion: 推理范式选择应该是基于任务的决策，由学习型路由器完成，而不是固定的架构选择。零-shot自路由仅对GPT-5有效，而学习型路由器对所有模型都有效。
...
TL;DR: 研究发现，向LLM评估者提供推理链会影响其判断准确性：弱评估者容易被流畅的推理误导，而强评估者能部分利用推理作为证据，但仍会被高质量推理链欺骗


<details>
  <summary>Details</summary>
Motivation: LLM作为人类评估的替代品存在表面偏见问题，可能因信息不足而影响判断准确性。随着推理能力模型的出现，向评估者展示生成器的推理内容可能提供更丰富信息，但其实际影响尚未充分研究

Method: 系统研究推理链访问对基于LLM的判断在事实问答和数学推理基准测试中的影响，通过控制实验分析推理链的流畅性和事实性对评估者决策的影响

Result: 弱评估者容易被推理存在所左右，经常接受伴随流畅推理的错误答案；强评估者能部分利用推理作为信息证据，但仍会被看似高质量的推理链误导；推理链的流畅性和事实性都是驱动评估者决策的关键信号
...
TL;DR: 比较了零样本大语言模型与监督表格模型在英语、斯洛文尼亚语和韩语的认知障碍分类任务中的表现，发现监督表格模型在结合语言特征和嵌入时表现更优


<details>
  <summary>Details</summary>
Motivation: 评估不同语言（英语、斯洛文尼亚语、韩语）中认知障碍分类的方法，比较零样本大语言模型与监督学习方法的性能差异

Method: 使用三种输入设置（仅转录文本、仅语言特征、两者结合）的零样本LLM作为直接分类器，与基于留一法协议的监督表格模型进行比较；表格模型使用工程化语言特征、转录嵌入以及早期或晚期融合两种模态

Result: 零样本LLM提供了有竞争力的无训练基线，但监督表格模型通常表现更好，特别是当包含工程化语言特征并与嵌入结合时；少样本实验表明有限监督的价值具有语言依赖性

Conclusion: 在小数据认知障碍检测中，结构化语言信号和简单的基于融合的分类器仍然是强大且可靠的信号
...
TL;DR: TeamLLM是一个模拟人类团队角色分工的多LLM协作框架，通过四种团队角色和三阶段协作机制解决多步骤情境化任务，并在新提出的CGPST基准测试中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多LLM框架没有明确模拟人类团队角色分工，导致单一视角，在多步骤情境化任务中性能受限。需要一种更接近人类团队协作的多LLM框架来提升这类任务的性能。

Method: 提出TeamLLM框架，采用四种具有明确分工的团队角色，并使用三阶段多LLM协作机制来处理多步骤情境化任务。同时提出了CGPST基准测试，该基准具有情境化基础、过程结构化、过程导向评估和多维度评估四个核心特征。

Result: 在CGPST基准测试中评估了10个流行的LLM，结果显示TeamLLM在整体层面、步骤层面和维度层面都显著提升了性能。发布了包含场景、完整过程响应和人工评分的基准数据集。

Conclusion: TeamLLM通过模拟人类团队角色分工和多LLM协作，有效提升了多步骤情境化任务的性能。提出的CGPST基准为评估这类任务提供了全面的测试框架。
...
TL;DR: 提出MSPA-CQR方法，通过多维度自洽偏好对齐和前缀引导的多方面直接偏好优化，提升对话查询重写的效果。


<details>
  <summary>Details</summary>
Motivation: 现有对话查询重写研究大多孤立进行，忽略了查询重写、段落检索和响应生成之间的反馈机制，导致重写效果受限。

Method: 1. 从三个维度（重写、检索、响应）构建自洽偏好对齐数据，生成更多样化的重写查询；2. 提出前缀引导的多方面直接偏好优化方法，从三个不同维度学习偏好信息。

Result: 实验结果表明，MSPA-CQR方法在分布内和分布外场景下都表现有效。
...
TL;DR: 提出DRCR框架，通过对话上下文重写改进多方对话生成，利用话语连贯性和响应质量作为反馈信号，并采用动态自进化学习方法提升重写器和响应器的能力。


<details>
  <summary>Details</summary>
Motivation: 现有多方对话生成研究主要利用对话结构信息，但对话中普遍存在的口语化表达和不完整话语会阻碍理解并削弱对话结构表示的准确性，这在多方对话中尤为明显。

Method: 提出DRCR框架，包含两个互补的反馈信号（话语连贯性和响应质量）来构建偏好数据，用于上下文重写和响应生成。采用动态自进化学习方法，让重写器和响应器在迭代训练循环中通过相互交互持续增强能力。

Result: 在四个多方对话数据集上进行的全面实验证实了DRCR框架的有效性。

Conclusion: 通过对话上下文重写和动态自进化学习，DRCR框架能够有效提升多方对话生成的质量，解决了口语化表达和不完整话语对对话理解的影响。
...
TL;DR: DTSR框架通过动态评估思维链充分性实现早期退出，减少大型推理模型的过度思考，在Qwen3模型上降低28.9%-34.9%的推理长度且性能损失最小


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂推理任务中表现出色，但存在过度思考问题，导致计算冗余和效率降低。现有的早期退出方法依赖手工或经验指标，不可靠且不实用，需要更智能的解决方案。

Method: 提出动态思维充分性推理框架，受人类元认知启发，包含两个阶段：1) 反思信号监测，识别早期退出的潜在线索；2) 思维充分性检查，评估当前思维链是否足以得出最终答案。

Result: 在Qwen3模型上的实验结果显示，DTSR能够减少28.9%-34.9%的推理长度，同时性能损失最小，有效缓解了过度思考问题。

Conclusion: DTSR为高效推理提供了新框架，通过动态评估思维链充分性实现智能早期退出，同时讨论了大型推理模型的过度自信问题和自我评估范式，为早期退出推理提供了有价值的见解。
...
TL;DR: GCoT-decoding是一种通用的解码策略，通过两阶段分支方法生成候选推理路径，无需手动设计提示，适用于固定和自由问答任务。


<details>
  <summary>Details</summary>
Motivation: 现有CoT解码方法仅适用于固定答案集的问题，限制了其应用范围。需要一种通用的解码策略，能够扩展到更广泛的问答任务。

Method: 采用两阶段分支方法：结合斐波那契采样和启发式错误回溯生成候选解码路径；将每条路径分割为推理段和答案段以准确计算路径置信度；聚合语义相似的路径识别共识答案，替代传统多数投票。

Result: 在六个数据集上的实验表明，该方法在固定问答任务上保持强大性能，同时在自由问答任务上取得显著改进，证明了其通用性。

Conclusion: GCoT-decoding成功扩展了CoT解码的适用范围，为更广泛的问答任务提供了一种有效的通用推理策略。
...
TL;DR: 论文提出了一个九维代数复杂度框架，用于独立分析LLM在代数推理中的具体失败原因，发现工作记忆是主要的规模不变瓶颈，并识别出五个最小诊断维度。


<details>
  <summary>Details</summary>
Motivation: 当前代数推理基准测试无法具体归因模型失败原因，单一准确率分数无法揭示失败的具体模式（如嵌套深度、运算符罕见度、中间状态数量等），需要系统化分析框架。

Method: 建立九维代数复杂度框架，每个维度独立变化而其他维度固定，使用参数化管道自动生成和验证问题，无需人工标注。评估了7个8B到235B参数的指令调优模型。

Result: 发现工作记忆是主导的规模不变瓶颈，所有模型在20-30个并行分支处崩溃，表明这是硬架构约束而非可解决的容量限制。识别出五个最小诊断维度能完整覆盖代数失败模式。

Conclusion: 提出的框架能系统分析LLM代数推理的具体失败原因，揭示了工作记忆的架构约束，并提供了完整的模型代数推理能力复杂度分析工具。
...
TL;DR: CLoT是一种基于可逆分层马尔可夫链的思维链框架，通过分层分解问题、引入后向验证机制和剪枝策略，解决了长思维链中的计算限制、上下文丢失和后向推理能力不足的问题。


<details>
  <summary>Details</summary>
Motivation: 长思维链（Long CoT）常导致序列长度超出计算限制，现有基于马尔可夫链的方法虽然减少了KV缓存冗余，但存在上下文丢失（内存无记忆性）和后向推理能力有限的问题。

Method: 提出基于可逆分层马尔可夫链的认知思维循环（CLoT）框架：1）将问题分解为具有层次依赖关系的子问题；2）受人类认知过程启发，在每层引入后向验证机制；3）实施剪枝策略，高层子问题验证后剪枝冗余低层子问题以提高效率。

Result: 在四个数学基准测试中验证了方法的有效性。在AddSub数据集上使用GPT-4o-mini，CLoT达到99.0%准确率，比传统CoT和CoT-SC分别高出4.1%和2.9%。

Conclusion: CLoT框架通过分层分解、后向验证和剪枝策略，有效缓解了错误传播，增强了推理鲁棒性，解决了长思维链中的计算和推理限制问题。
...
TL;DR: AGSC框架通过自适应粒度和GMM语义聚类解决长文本生成中的不确定性量化问题，减少幻觉并提升效率


<details>
  <summary>Details</summary>
Motivation: 大语言模型在长文本生成中存在幻觉问题，现有不确定性量化方法难以可靠聚合异构主题，忽略中性信息细节，且细粒度分解计算成本高

Method: 提出AGSC框架：1)使用NLI中性概率作为触发器区分无关信息和不确定性；2)应用高斯混合模型软聚类建模潜在语义主题，分配主题感知权重进行下游聚合

Result: 在BIO和LongFact数据集上，AGSC实现了与事实性最佳的相关性，相比完全原子分解减少了约60%的推理时间

Conclusion: AGSC框架有效解决了长文本生成中的不确定性量化挑战，在保持准确性的同时显著提升了计算效率
...
TL;DR: 该研究构建了首个公开可用的斯洛文尼亚语ESG情感数据集，并评估了多种模型在ESG情感检测任务上的性能，发现不同模型在不同ESG维度上表现最佳。


<details>
  <summary>Details</summary>
Motivation: ESG因素对企业绩效、声誉和长期可持续性评估日益重要，但针对小型公司和新兴市场的可靠ESG评级仍然有限，特别是缺乏斯洛文尼亚语的ESG数据资源。

Method: 从MaCoCu斯洛文尼亚新闻语料库构建数据集，结合大语言模型辅助筛选和人工标注公司相关ESG内容。评估了单语模型(SloBERTa)、多语模型(XLM-R)、嵌入分类器(TabPFN)、层次集成架构和大语言模型在ESG情感检测任务上的性能。

Result: 大语言模型在环境维度(Gemma3-27B, F1-macro: 0.61)和社会维度(gpt-oss 20B, F1-macro: 0.45)表现最佳，而微调的SloBERTa在治理分类上表现最好(F1-macro: 0.54)。通过案例研究展示了最佳分类器(gpt-oss)如何用于分析选定公司长期ESG表现。

Conclusion: 该研究提供了首个斯洛文尼亚语ESG情感数据集，证明了不同模型架构在不同ESG维度上的差异化性能，为新兴市场和小型公司的ESG分析提供了实用工具和方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据重述方法局限于单文档层面，只能重写单个网页，缺乏跨文档关系和关联上下文，限制了知识获取的深度和广度。

Method: 提出WRAP++方法：1）从网页超链接中发现高置信度的关系模式（双向链接和共同提及）；2）为每个发现的文档对合成需要跨文档推理的联合问答；3）基于维基百科将原始文本放大为跨文档QA数据。

Result: 将约84亿token的原始文本放大为800亿token的跨文档QA数据。在SimpleQA任务上，基于OLMo的7B和32B模型使用WRAP++训练后，显著优于单文档方法，并展现出持续的扩展增益。

Conclusion: 跨文档知识发现和放大方法相比单文档重述具有显著优势，能够创造原始文档中不存在的关联知识，为相同事实提供多样化的切入点，并大幅扩展数据规模。
...
TL;DR: Fast-dVLM是一种基于块扩散的视觉语言模型，通过并行解码和推测性块解码实现推理加速，在保持生成质量的同时获得超过6倍的端到端推理速度提升。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要依赖自回归解码，逐个生成标记，这从根本上限制了推理吞吐量。在机器人、自动驾驶等物理AI场景中，模型部署在边缘设备上且批处理大小为1，使得自回归解码受限于内存带宽，硬件并行性未得到充分利用。

Method: 提出Fast-dVLM，采用块扩散方法实现KV缓存兼容的并行解码和推测性块解码。比较了两种AR到扩散的转换策略：两阶段方法（先文本扩散微调再多模态训练）和直接方法（直接转换完整AR VLM）。引入多模态扩散适配、块大小退火、因果上下文注意力、自动截断掩码和视觉高效拼接等技术。

Result: 在11个多模态基准测试中，Fast-dVLM在生成质量上与自回归对应模型相当。通过SGLang集成和FP8量化，实现了超过6倍的端到端推理速度提升。

Conclusion: 直接转换策略更高效，能利用已对齐的多模态VLM。Fast-dVLM通过块扩散方法有效解决了VLM推理吞吐量瓶颈，为物理AI场景中的边缘部署提供了高效的解决方案。
...
TL;DR: 论文提出自然度数据选择方法在LLM推理数据中存在步骤长度混杂问题，偏好长步骤而非高质量样本，并提出两种改进方法


<details>
  <summary>Details</summary>
Motivation: 现有基于自然度的数据选择方法在LLM推理数据中系统性地偏好推理步骤更长的样本，而不是更高质量的样本，这被称为步骤长度混杂问题

Method: 提出两种变体方法：ASLEC-DROP（计算平均对数概率时丢弃首词概率）和ASLEC-CASL（应用因果去偏回归去除首词的混杂效应）

Result: 在四个LLM和五个评估基准上的实验证明了该方法在缓解步骤长度混杂问题方面的有效性

Conclusion: 基于自然度的数据选择方法在LLM推理数据中存在步骤长度混杂问题，提出的两种改进方法能有效缓解这一问题
...
TL;DR: HingeMem：基于边界触发超边的长期记忆系统，通过事件分割理论构建可解释索引，实现自适应检索，在对话系统中显著提升性能并降低计算成本。
...
Method: 提出边界引导的长期记忆系统HingeMem，基于事件分割理论，通过边界触发的超边构建可解释索引（人物、时间、地点、主题四要素）。当任一要素变化时触发边界写入当前片段，减少冗余操作。引入查询自适应检索机制，联合决定检索内容和深度。

Result: 在LLM规模（0.6B到生产级模型）的LOCOMO数据集上实验显示，HingeMem相比强基线获得约20%相对提升（无需查询类别指定），同时降低68%的问答token成本（相比HippoRAG2）。

Conclusion: HingeMem通过边界触发超边和自适应检索机制，在长期记忆建模方面取得显著进展，特别适合需要高效可信记忆的Web应用，为持续个性化交互提供有效解决方案。
...
TL;DR: 临床领域适应的大语言模型在英语医疗任务上并不总是优于通用模型，但在西班牙语任务上表现更好，现有评估框架可能不足以捕捉真正的医学专业知识。


<details>
  <summary>Details</summary>
Motivation: 最近研究表明，领域适应的大语言模型在标准医疗基准测试中并不总是优于通用模型，这引发了对专门临床适应必要性的质疑。

Method: 系统比较通用和临床LLM在英语和西班牙语临床问答任务上的表现，引入基于扰动的评估基准，测试模型鲁棒性、指令遵循能力和对抗性变化的敏感性，包括一步和两步问题转换、多提示测试和指令引导评估。

Result: 临床LLM在英语临床任务上并不总是优于通用模型，但在西班牙语子集上，提出的Marmoka模型相比Llama获得更好结果。通用和临床模型在指令遵循和严格输出格式方面都存在显著限制。

Conclusion: 当前短格式多项选择问答基准下，临床LLM在英语上仅提供边际且不稳定的改进，现有评估框架可能不足以捕捉真正的医学专业知识。同时证明可以为西班牙语等低资源语言成功开发鲁棒的医疗LLM。
...
TL;DR: iTAG提出了一种通过迭代概念分配和CoT推理，从因果图生成高质量标注文本的方法，在保持文本自然性的同时确保因果标注准确性，为文本因果发现算法提供可扩展的基准测试数据。


<details>
  <summary>Details</summary>
Motivation: 文本因果发现面临缺乏高质量因果标注文本数据的根本障碍，因为标注成本高昂。现有方法要么牺牲文本自然性（模板方法），要么无法保证因果图标注准确性（LLM方法），需要一种能同时保证自然性和准确性的生成方法。

Method: iTAG在现有LLM依赖方法的基础上，增加了真实世界概念分配到节点的步骤，将其构建为逆问题。通过链式思维推理迭代检查和优化概念选择，确保概念间的关系与目标因果图描述的因果关系尽可能一致。

Result: iTAG在广泛测试中表现出极高的标注准确性和文本自然性。使用生成数据测试文本因果发现算法的结果显示，与真实世界数据具有高度统计相关性，表明iTAG生成的数据可以作为文本因果发现算法可扩展基准测试的实用替代品。

Conclusion: iTAG通过迭代概念分配和CoT推理，成功解决了文本因果发现数据生成中自然性与准确性难以兼得的问题，为文本因果发现算法的评估提供了高质量、可扩展的基准测试数据解决方案。
...
TL;DR: 该研究探讨了领域自适应预训练（DAPT）在法语生物医学领域的应用，通过持续预训练专门化中小型LLM，但结果对DAPT的有效性提出质疑，同时发现模型融合能缓解泛化权衡问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在多个领域表现出色，但在专业领域（特别是非英语语言）的适应仍具挑战性。本研究旨在探索DAPT作为法语生物医学领域专门化策略的可行性，并分析领域性能提升与通用能力退化之间的关系。

Method: 方法包括：收集和精炼高质量法语生物医学文本语料；探索使用DAPT的因果语言建模方法；进行广泛的比较评估；研究DAPT后模型融合以缓解泛化权衡问题。

Result: 结果对DAPT的有效性提出质疑（与先前研究形成对比），但在资源受限的小规模场景中，在适当条件下仍具有可行性。研究发现DAPT后模型融合对于缓解泛化权衡至关重要，在某些情况下甚至能提升DAPT针对的专业任务性能。

Conclusion: 该研究贡献包括：发布完全开放许可的法语生物医学语料库；训练并发布专门化的法语生物医学LLM；为DAPT实施提供新见解。结论强调DAPT在资源受限场景下的可行性，以及模型融合在平衡专业化和泛化能力方面的重要性。
...
TL;DR: 本文提出了技能自动化可行性指数（SAFI），评估了4个前沿大语言模型在263个文本任务上的表现，发现数学和编程技能最易自动化，而主动倾听和阅读理解最不易自动化，并提出了AI影响矩阵框架。
...
Method: 开发了技能自动化可行性指数（SAFI），在LLaMA 3.3 70B、Mistral Large、Qwen 2.5 72B和Gemini 2.5 Flash四个模型上评估了涵盖美国劳工部O*NET分类中35种技能的263个文本任务，并与Anthropic经济指数的真实AI采用数据进行交叉参考。

Result: 数学（SAFI: 73.2）和编程（71.8）获得最高自动化可行性评分，主动倾听（42.2）和阅读理解（45.5）最低；发现"能力需求倒置"现象；78.7%的AI交互是增强而非自动化；四个模型在技能评估上表现相似（3.6分差距）。

Conclusion: 文本基础的自动化可行性更多取决于技能本身而非具体模型，提出了AI影响矩阵框架来定位技能在四个象限中的位置，为劳动力市场转型提供实证指导。
...
TL;DR: 研究发现LLM作为评判者在基于规则的评估中存在自我偏好偏差，即使评估标准完全客观，评判者对自己生成的输出仍有高达50%的错误偏向，这种偏差在主观规则下可导致模型得分偏差达10分。


<details>
  <summary>Details</summary>
Motivation: LLM-as-a-judge已成为评估LLM输出的主流方法，但评判者存在自我偏好偏差，倾向于偏爱自己或同系列模型生成的输出，这扭曲了评估结果，阻碍了模型发展，特别是在递归自我改进的背景下。

Method: 使用IFEval（具有可编程验证规则的基准）研究客观规则下的SPB，使用HealthBench（医学聊天基准）研究主观规则下的SPB，分析不同因素对SPB的影响，并探索通过集成多个评判者来缓解SPB。

Result: 在客观规则评估中，即使评估标准完全客观，当生成器失败时，评判者对自己输出的错误标记概率可高达50%；在主观规则评估中，SPB可导致模型得分偏差达10分；集成多个评判者有助于缓解但无法完全消除SPB；负面规则、极端规则长度和主观话题（如紧急转诊）特别容易受到SPB影响。

Conclusion: LLM评判者在基于规则的评估中存在显著的自我偏好偏差，这种偏差在客观和主观规则中都存在，可能严重影响模型排名和评估结果，需要开发更有效的缓解方法来确保公平评估。
...
TL;DR: 本文提出了ChunQiuTR时间键控检索基准和CTD时间感知双编码器，用于解决古典中文编年史检索中的时间一致性问题，确保检索结果不仅语义相关且时间正确。


<details>
  <summary>Details</summary>
Motivation: 在历史研究中，检索目标往往不是任意相关段落，而是特定统治月份的精确记录，时间一致性与主题相关性同等重要。古典中文编年史的时间表达简洁、隐晦、非公历，必须从上下文解释，导致语义合理的证据可能在时间上无效。

Method: 1) 构建ChunQiuTR时间键控检索基准，基于《春秋》及其注释传统，按月份级统治键组织记录，包含反映实际检索失败的时序邻近干扰项；2) 提出CTD（历法时间双编码器），结合基于傅里叶的绝对历法上下文和相对偏移偏置的时间感知双编码器。

Result: 实验显示，在时间键控评估下，CTD相比强大的语义双编码器基线获得一致提升，支持检索时的时间一致性作为忠实下游历史RAG的关键前提。

Conclusion: 检索时的时间一致性是历史RAG忠实性的关键前提，ChunQiuTR基准和CTD方法有效解决了古典中文编年史检索中的时间一致性问题。
...
TL;DR: 论文提出连续解释性引导(CIS)方法和GraSD数据集，用于评估大语言模型中的分级语用推理能力，特别是标量多样性现象。


<details>
  <summary>Details</summary>
Motivation: 语用推理本质上是分级的，不同词汇项会产生不同程度的语用充实。标量含义体现了这种特性，即标量项的蕴含强度存在差异。然而，当前对大语言模型中语用推理的评估主要依赖基于提示的操作，缺乏对分级敏感性的系统研究。

Method: 提出连续解释性引导(CIS)方法，将激活级引导强度作为连续实验变量来探测分级语用解释。同时引入新的数据集GraSD，该数据集编码了分级标量多样性。在四个大语言模型上进行实验，比较均匀激活引导和分级激活引导的效果。

Result: 实验表明，均匀激活引导虽然能全局增加语用解释，但会消除项级变异；而分级激活引导能产生与标量多样性等级一致的分化解释偏移。这表明分级敏感性在表示空间中被编码，并能通过受控干预系统性地恢复。

Conclusion: CIS和GraSD为评估大语言模型中的分级语用敏感性提供了一个原则性框架，揭示了语用推理的分级特性在模型表示中的编码方式，为理解模型语用能力提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 传统的递归摘要方法存在两个主要问题：1）摘要树包含大量冗余节点，增加了构建时间并可能对问答产生负面影响；2）递归摘要并不适用于所有类型的问题。需要一种更智能的方法来动态决定何时以及如何构建摘要树。

Method: DTCRS方法首先分析问题类型来决定是否需要构建摘要树。如果需要，则将问题分解为子问题，并使用子问题的嵌入向量作为初始聚类中心。这种方法基于文档结构和查询语义动态生成摘要树，减少了冗余摘要，同时提高了摘要与问题的相关性。

Result: DTCRS显著减少了摘要树的构建时间，并在三个问答任务中取得了实质性改进。该方法还研究了递归摘要对不同问题类型的适用性，为未来研究提供了有价值的见解。

Conclusion: DTCRS通过动态决定是否需要摘要树以及使用子问题嵌入作为聚类中心，有效解决了传统递归摘要方法的冗余问题，提高了问答系统的效率和准确性，并为递归摘要的应用范围提供了新的理解。
...
TL;DR: 研究数据复制对资源有限语言（π-语言）NLP任务的影响，通过增量复制技术扩展Nawatl语料库，在语义相似度任务中取得了适度性能提升。


<details>
  <summary>Details</summary>
Motivation: 针对计算资源有限的语言（π-语言），特别是Nawatl这种拥有超过200万使用者但训练语料匮乏的黏着语和多式综合语，探索数据复制是否能在NLP任务中带来益处。

Method: 采用增量复制技术对有限的π-yalli Nawatl语料库进行受控扩展，训练静态词嵌入，并在句子级语义相似度任务中进行评估。

Result: 实验结果显示，与未扩展的原始语料相比，使用增量复制技术后，在语义相似度任务中获得了适度的性能提升。

Conclusion: 数据复制技术对资源有限语言的NLP任务具有潜在价值，增量复制方法在Nawatl语中显示出有效性，且该方法在文献中尚未被广泛使用。
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归语言模型每次只生成一个token，即使后续token在给定上下文下高度可预测，这限制了生成效率。现有方法如推测解码需要额外模型，多头方法需要附加预测头，都存在部署复杂性。

Method: MARS通过轻量级微调方法，在现有指令数据上继续训练，教导指令调优的自回归模型单次前向传播预测多个token。不修改架构，不增加参数，支持置信度阈值实时调整速度，并开发了批处理推理的块级KV缓存策略。

Result: 在六个标准基准测试中，单token生成时匹配或超过自回归基线；多token生成时保持基线准确性的同时实现1.5-1.7倍吞吐量提升。在Qwen2.5-7B上使用KV缓存实现最高1.71倍加速。支持通过置信度阈值实时调整速度。

Conclusion: MARS提供了一种实用的延迟-质量权衡机制，无需模型交换或重启即可动态调整吞吐量，为语言模型部署提供了高效的解决方案。
...
TL;DR: IndoBERT-Sentiment：基于主题上下文的情感分析模型，相比传统孤立文本分类方法，在印尼语情感分析任务上性能提升显著


<details>
  <summary>Details</summary>
Motivation: 现有印尼语情感分析模型孤立地分类文本，忽略了决定语句情感极性（积极、消极或中性）的主题上下文。主题背景对于准确判断情感至关重要。

Method: 提出IndoBERT-Sentiment，一种基于上下文条件的情感分类器，同时接收主题上下文和文本作为输入。模型基于IndoBERT Large（3.35亿参数）构建，在188个主题的31,360个上下文-文本对上进行训练。

Result: 模型达到F1宏平均0.856和准确率88.1%。在与三种广泛使用的通用印尼语情感模型的对比评估中，IndoBERT-Sentiment比最佳基线高出35.6个F1点。模型能够正确分类那些被无上下文方法系统性地错误分类的文本。

Conclusion: 上下文条件化方法（先前已证明适用于相关性分类）可有效迁移到情感分析任务，使模型能够基于讨论主题进行更准确的情感预测，解决传统上下文无关方法的系统性错误分类问题。
...
### [146] [STRIDE-ED: A Strategy-Grounded Stepwise Reasoning Framework for Empathetic Dialogue Systems](https://arxiv.org/abs/2604.07100)
...
Method: 提出STRIDE-ED框架，采用策略感知数据精炼流程（集成LLM标注、多模型一致性加权评估和动态采样），以及两阶段训练范式（监督微调+多目标强化学习）。

Result: 实验表明STRIDE-ED在不同开源LLM上具有良好泛化能力，在自动指标和人工评估中均优于现有方法。

Conclusion: STRIDE-ED通过结构化策略条件推理有效解决了共情对话建模中的关键挑战，为复杂多阶段认知决策过程提供了有效框架。
...
TL;DR: SemEval-2026共享任务提出了基于维度情感分析（DimABSA）和维度立场分析（DimStance）的新任务，用效价-唤醒度（VA）维度替代传统分类标签，扩展了情感分析到公共议题领域。


<details>
  <summary>Details</summary>
Motivation: 传统基于方面的情感分析（ABSA）使用分类极性标签（如正面、负面、中性），这种简化方法无法捕捉情感的细微差别。同时，ABSA主要应用于消费者评论领域，需要扩展到公共议题讨论（如政治、能源、气候问题）中。维度情感分析能更精细地建模情感，而维度立场分析则将立场检测重新定义为VA空间中的回归问题。

Method: 任务分为两个轨道：Track A（DimABSA）包括三个子任务：（1）维度方面情感回归，（2）维度方面情感三元组提取，（3）维度方面情感四元组提取；Track B（DimStance）仅包括立场目标的回归子任务。引入了连续F1（cF1）指标来联合评估结构化提取和VA回归。任务吸引了400多名参与者，最终提交112份结果和42篇系统描述论文。

Result: 任务获得了广泛参与，超过400名参与者提交了112份最终结果和42篇系统描述论文。论文报告了基线结果，讨论了表现最佳的系统，并分析了关键设计选择，为方面和立场目标级别的维度情感分析提供了见解。

Conclusion: 该共享任务成功推动了维度情感分析在方面和立场目标级别的研究，通过引入VA维度建模和扩展到公共议题领域，为情感分析提供了更精细和实用的方法。所有资源已在GitHub仓库中公开，促进了该领域的研究发展。
...
TL;DR: 双语语言模型中的跨语言激活：研究荷兰语-英语Transformer模型在不同词汇共享条件下如何模拟双语者的跨语言激活模式，发现模型效果主要受频率驱动而非形式-意义映射一致性。


<details>
  <summary>Details</summary>
Motivation: 双语者在阅读时会出现跨语言激活现象，同源词（朋友）通常产生促进效应，而跨语言同形异义词（假朋友）则导致干扰或无效应。研究旨在探索双语语言模型是否能够模拟这些人类双语处理模式。

Method: 训练荷兰语-英语因果Transformer模型，设置四种词汇共享条件来操纵（假）朋友是否获得共享或语言特定的嵌入表示。使用双语阅读研究的心理语言学刺激材料，通过惊奇度和嵌入相似性分析评估模型。

Result: 模型在很大程度上保持语言分离，跨语言效应主要在嵌入共享时出现。在这些情况下，朋友和假朋友都相对于控制词表现出促进效应。回归分析显示这些效应主要由频率驱动，而非形式-意义映射的一致性。只有当朋友共享嵌入时，才能重现双语者的定性模式。

Conclusion: 双语语言模型能够捕捉一些跨语言激活效应，但它们与人类处理的对应关系似乎关键取决于词汇重叠如何编码，这可能限制了它们作为双语阅读模型的解释充分性。
...
### [149] [The Impact of Steering Large Language Models with Persona Vectors in Educational Applications](https://arxiv.org/abs/2604.07102)
*Yongchao Wu,Aron Henriksson*

Main category: cs.CL

TL;DR: 研究激活导向的人格向量在短答案生成和自动评分中的教育应用效果，发现人格导向会降低答案质量，且对开放式英语语言艺术任务的影响远大于事实性科学任务；评分方面存在可预测的校准偏移


<details>
  <summary>Details</summary>
Motivation: 虽然激活导向可以在推理时个性化大型语言模型，但其在教育环境中的效果尚不明确。本研究旨在系统性地探究激活导向的人格特质在教育生成和评分中的影响

Method: 使用七种人格特质的人格向量，在ASAP-SAS基准上进行短答案生成和自动评分实验，涵盖三种模型和两种架构，分析人格导向对答案质量和评分校准的影响

Result: 人格导向总体上降低了答案质量，对开放式英语语言艺术任务的影响比对事实性科学任务大得多（解释性和论证性任务敏感度高达11倍）。评分方面观察到可预测的校准偏移：邪恶和不礼貌的评分者评分更严格，而善良和乐观的评分者评分更宽松。英语语言艺术任务比科学任务对评分个性化敏感2.5-3倍，混合专家模型比密集模型显示出约6倍大的校准偏移

Conclusion: 这是首个系统研究激活导向人格特质在教育生成和评分中影响的研究，结果表明在教育环境中部署导向模型时，需要进行任务感知和架构感知的校准
...
### [150] [Multilingual Embedding Probes Fail to Generalize Across Learner Corpora](https://arxiv.org/abs/2604.07095)
*Laurits Lyngbaek,Ross Deans Kristensen-McLachlan*

Main category: cs.CL

TL;DR: 研究发现多语言嵌入模型并未直接编码语言通用的熟练度表征，而是捕捉了语料库特定的分布特性
...
Method: 在Qwen3-Embedding模型的隐藏状态激活上训练线性和非线性探针，预测九个语料库、七种语言中学习者文本的CEFR熟练度等级，并与基于表层文本特征的基线进行比较

Result: 在分布内评估中探针表现良好（QWK≈0.7），显著优于基线，中间层预测效果最佳；但在跨语料库评估中性能崩溃，残差分析显示探针学习到的是语料库特定的分布特性而非抽象可迁移的熟练度维度

Conclusion: 当前多语言嵌入模型并未直接编码语言通用的熟练度表征，这对基于表征的熟练度自适应语言技术方法提出了挑战
...
### [151] [Dynamic Context Evolution for Scalable Synthetic Data Generation](https://arxiv.org/abs/2604.07147)
*Ryan Lingo,Rajeev Chhajer*

Main category: cs.CL

TL;DR: 论文提出了动态上下文演化（DCE）框架，通过三种机制解决大语言模型在批量独立提示时出现的跨批次模式崩溃问题，有效保持输出多样性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在多个批次独立提示时会产生重复输出，即跨批次模式崩溃现象。现有方法主要依赖临时去重和种子轮换，缺乏系统性的解决方案。

Method: 提出动态上下文演化（DCE）框架，包含三种机制：1）语言化尾部采样（模型自我评估并丢弃明显想法）；2）语义记忆（维护嵌入索引以跨批次拒绝近似重复）；3）自适应提示演化（基于记忆状态和轮换多样性策略重构生成提示）。

Result: 在三个领域（可持续包装概念、教育考试题目、创意写作提示）和两个模型家族（gpt-5-mini和claude-haiku-4-5）的实验中，DCE实现了0.0±0.0%的崩溃率，而朴素提示为5.6±2.0%。DCE产生17-18个HDBSCAN聚类，而朴素方法仅为2-17个，表明概念结构更丰富。结果经独立嵌入模型验证，对参数变化稳健。

Conclusion: DCE框架有效解决了跨批次模式崩溃问题，去重和提示演化单独使用不足但联合使用有效，仅需标准API调用（约0.50美元/1000个候选），无需微调或定制架构。
...
TL;DR: 耶鲁-DM-Lab系统在ArchEHR-QA 2026任务中采用多模型集成方法处理患者病历问答，包含四个子任务：问题重述、证据句识别、答案生成和证据-答案对齐，通过模型多样性和投票策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在解决患者对住院记录提问的复杂医疗问答任务，需要将患者问题转化为临床医生可理解的形式，并准确识别证据、生成答案并进行对齐，这对医疗决策支持具有重要意义。

Method: ST1使用Claude Sonnet 4和GPT-4o双模型管道进行问题重述；ST2-ST4采用Azure托管的模型集成（o3、GPT-5.2、GPT-5.1、DeepSeek-R1），结合少样本提示和投票策略，并为证据对齐提供完整临床答案段落作为上下文。

Result: 开发集上最佳得分：ST4（证据-答案对齐）达到88.81微F1，ST2（证据句识别）65.72宏F1，ST3（答案生成）34.01，ST1（问题重述）33.05。模型多样性和集成投票相比单模型基线持续提升性能，对齐准确率主要受推理能力限制。

Conclusion: 多模型集成和投票策略能有效提升医疗问答系统性能，但证据对齐任务仍受限于模型的推理能力，未来需要进一步改进推理机制以提高对齐准确率。
...
TL;DR: 该研究首次系统性地定义了语言研究偏见（LoS bias），即审稿人基于论文研究语言而非科学价值进行评价的倾向，开发了LOBSTER数据集和检测方法，发现非英语论文面临更高的偏见率。


<details>
  <summary>Details</summary>
Motivation: 尽管NLP审稿指南明确禁止语言研究偏见，但这种偏见仍然存在且缺乏系统研究。先前工作将其归为弱或无建设性评论的一部分，没有将其定义为独立的偏见形式。本研究旨在首次系统性地表征语言研究偏见，区分其负面和正面形式。

Method: 1. 提出语言研究偏见的系统性定义，区分负面和正面偏见；2. 创建人工标注的LOBSTER数据集；3. 开发检测方法，达到87.37的宏观F1分数；4. 分析15,645篇审稿意见，量化不同语言研究偏见的差异；5. 识别负面偏见的四个子类别。

Result: 1. 非英语论文面临的偏见率显著高于仅研究英语的论文；2. 负面偏见始终超过正面偏见；3. 识别出四种负面偏见子类别，其中要求不合理跨语言泛化是最主要的形式；4. 检测方法达到87.37宏观F1分数。

Conclusion: 该研究首次系统性地定义了语言研究偏见，提供了检测方法和数据集，揭示了非英语论文在同行评审中面临的不公平待遇。所有资源已公开，以支持NLP及其他领域更公平的审稿实践。
...
TL;DR: 研究发现大型语言模型在处理多语言冲突信息时存在系统性偏见，倾向于忽略冲突并自信地选择特定语言的答案，尤其对俄语存在偏见，对中文存在偏好。


<details>
  <summary>Details</summary>
Motivation: 已有研究表明LLMs在整合冲突信息时存在偏见，本研究旨在探究这种偏见是否与语言选择相关，即当冲突信息以不同语言呈现时，LLMs是否存在语言偏好。

Method: 将"干草堆中的冲突针"范式扩展到多语言环境，使用五种不同语言的新闻领域自然数据，对多种规模的多语言LLMs进行全面评估。

Result: 所有测试的LLMs（包括GPT-5.2）在大多数情况下都忽略冲突，自信地只断言其中一个可能的答案。存在一致的语言偏好模式：普遍对俄语存在偏见，在最长上下文长度下对中文存在偏好。这种模式在国内外训练的模型中均存在，但在中国境内训练的模型中稍强。

Conclusion: LLMs在处理多语言冲突信息时存在系统性语言偏见，这对多语言AI系统的公平性和可靠性提出了重要挑战，需要在模型训练和评估中加以解决。
...
TL;DR: 该论文提出了一种使用语言模型作为语义上下文调节器的新框架，通过手工制作的情感描述符来建模效价和唤醒度的变化，在保持特征可解释性的同时提升预测性能。
...
Method: 1. 从结构化领域知识中提取可解释的面部几何和声学特征
2. 将这些特征转换为编码其情感含义的符号自然语言描述
3. 使用预训练语言模型处理这些描述，生成作为情感动态高级先验的语义上下文嵌入
4. 不同于端到端黑盒管道，该框架保留了特征透明度

Result: 在Aff-Wild2和SEWA数据集上进行情感变化预测评估。实验结果显示，与仅使用手工特征和深度嵌入基线相比，在效价和唤醒度的准确性方面都有持续改进。

Conclusion: 语义调节能够在保持预测性能的同时实现可解释的情感建模，为完全端到端架构提供了透明且计算高效的替代方案。该框架结合了领域知识的可解释性和语言模型的上下文抽象能力。
...
### [156] [Evaluating In-Context Translation with Synchronous Context-Free Grammar Transduction](https://arxiv.org/abs/2604.07320)
*Jackson Petty,Jaulie Goe,Tal Linzen*

Main category: cs.CL

TL;DR: LLMs在低资源语言翻译中面临挑战，本文通过形式语法任务测试LLMs能否根据上下文语法描述进行字符串转换，发现语法规模、句子长度、形态差异和书写系统都会显著影响翻译准确性。


<details>
  <summary>Details</summary>
Motivation: 低资源语言缺乏足够训练数据，但LLMs可能通过上下文中的语言描述（如教科书、词典）来理解语言。本文旨在测试LLMs能否从形式语法描述中推断语言规则并进行翻译。

Method: 构建同步上下文无关文法来定义形式语言对，模拟自然语言的语法、形态和书写特征。在给定语法和源语言句子的情况下，测量LLMs将句子从一个形式语言翻译到另一个的能力，并变化语法规模、句子长度、语言特性和书写系统。

Result: 1. LLMs翻译准确率随语法规模和句子长度显著下降；2. 源语言和目标语言之间的形态差异和书写系统差异会严重降低模型性能；3. 错误分析显示模型最常犯的错误包括：回忆错误的词汇、产生幻觉新词、或保留源语言词汇不翻译。

Conclusion: LLMs虽然能够利用上下文语法描述进行翻译，但其性能受到语法复杂性、句子长度和语言差异的显著限制。这为低资源语言翻译的实际应用提供了重要启示。
...
TL;DR: 本文提出了一种双流多尺度解耦器架构，通过分离局部和全局上下文来替代深度串行处理，同时设计了并发流并行流水线来解决系统瓶颈，在压缩比和吞吐量方面都达到了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前学习型数据压缩方法在精确概率建模和系统效率之间难以平衡。统一的单流架构难以同时捕获微观句法和宏观语义特征，需要深度串行堆叠导致延迟增加。此外，异构系统受限于设备速度不匹配，串行处理导致吞吐量受阿姆达尔定律限制。

Method: 提出了双流多尺度解耦器，将局部和全局上下文解耦，用浅层并行流替代深度串行处理；引入了分层门控细化器进行自适应特征细化和精确概率建模；设计了并发流并行流水线，克服系统瓶颈实现全流水线并行。

Result: 大量实验表明，该方法在压缩比和吞吐量方面都达到了最先进性能，同时保持了最低的延迟和内存使用。

Conclusion: 提出的双流多尺度解耦架构和并发流并行流水线有效解决了学习型数据压缩中的效率瓶颈问题，实现了压缩性能与系统效率的平衡。
...
TL;DR: ClickGuard是一个用于点击诱饵检测的可信自适应融合框架，结合BERT嵌入和结构特征，通过SSAFB进行动态集成，使用混合CNN-BiLSTM捕获模式，达到96.93%的测试准确率。


<details>
  <summary>Details</summary>
Motivation: 点击诱饵标题的广泛使用对在线可信度构成重大挑战，这些标题采用耸人听闻、误导性声明和模糊语言，需要有效的检测来确保可信的数字内容。

Method: 提出ClickGuard框架，结合BERT嵌入和结构特征，使用句法-语义自适应融合块(SSAFB)进行动态集成，采用混合CNN-BiLSTM架构捕获模式依赖关系。

Result: 模型达到96.93%的测试准确率，优于现有方法。通过LIME和PFI评估可信度，消融研究验证了SSAFB的有效性，模型在不同数据集上表现出稳健性能。

Conclusion: ClickGuard提供了一个可扩展、可靠的解决方案，通过解决句法-语义建模挑战来增强在线内容可信度，为点击诱饵检测提供了有效的框架。
...
TL;DR: 本文认为教学难以被AI完全自动化，因为教学本质上是解释性、关系性的专业工作，依赖于人类认知、行为和情境理解，而非模块化程序化任务。


<details>
  <summary>Details</summary>
Motivation: 针对当前关于AI在教育中应用的讨论常常将教学视为可模块化和程序化的工作，作者认为这种观点忽视了教学实践中的不可分割性，需要澄清AI在教育中的实际局限。

Method: 通过分析近期文献和关于大语言模型、检索增强生成系统的实证研究，论证教学工作的本质特征及其与AI能力的对比。

Result: AI虽然能够支持某些有限功能，但教学工作的解释性、关系性和专业判断本质使其难以被有意义地自动化。教学价值源于对学习者、情境和关系的持续情境解释。

Conclusion: 只要教育实践依赖于对人类认知和学习的新兴理解，教学就仍然是抵抗自动化的专业工作。AI可以提高信息获取和支持特定教学活动，但不能替代有效教学所需的人类判断和关系责任。
...
TL;DR: 本文提出了Personalized RewardBench，这是一个专门评估奖励模型在个性化偏好建模能力的新基准，现有SOTA奖励模型在该基准上表现不佳（最高仅75.94%准确率），且该基准与下游任务性能有更高相关性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM发展中的多元对齐需要奖励模型捕捉多样化的人类价值观，但现有基准主要评估通用响应质量，缺乏对奖励模型建模个人用户偏好能力的评估，这一空白需要填补。

Method: 构建基于严格遵循（或违反）用户特定规则的接受和拒绝响应对，确保偏好区分完全针对个人定制；通过人工评估确认主要区分因素是个人偏好而非通用质量；将基准与下游任务（BoN采样和PPO）性能进行相关性实验。

Result: 现有最先进的奖励模型在个性化方面表现显著不足，最高准确率仅75.94%；Personalized RewardBench与下游任务性能的相关性显著高于现有基线，能更准确地预测奖励模型在下游应用中的表现。

Conclusion: Personalized RewardBench是一个稳健且准确的评估基准，能够有效评估奖励模型在下游应用中的性能，填补了现有评估体系在个性化偏好建模能力评估方面的空白。
...
### [161] [High-Precision Estimation of the State-Space Complexity of Shogi via the Monte Carlo Method](https://arxiv.org/abs/2604.06189)
*Sotaro Ishii,Tetsuro Tanaka*

Main category: cs.AI

TL;DR: 使用蒙特卡洛采样和基于"王-王"位置的反向搜索方法，将日本将棋的状态空间复杂度精确估计为6.55×10⁶⁸，填补了之前10⁶⁴到10⁶⁹五个数量级的估计差距。


<details>
  <summary>Details</summary>
Motivation: 日本将棋的状态空间复杂度一直难以精确确定，之前的组合估计存在10⁶⁴到10⁶⁹五个数量级的巨大差距，主要困难在于从大量有效棋盘配置中区分出从初始位置合法可达的位置。

Method: 结合蒙特卡洛采样和一种新颖的可达性测试方法，该方法使用向"王-王"位置集合的反向搜索，而不是向单一初始位置的单目标反向搜索，显著减少了确定不可达性的搜索工作量。

Result: 基于50亿个位置的样本，估计日本将棋的合法位置数为6.55×10⁶⁸（三位有效数字），置信水平为3σ，显著改进了先前已知的边界。该方法应用于迷你将棋时，确定其复杂度约为2.38×10¹⁸。

Conclusion: 提出的统计估计方法成功填补了日本将棋状态空间复杂度的估计差距，为棋类游戏复杂度分析提供了新的有效技术，特别是反向搜索到目标位置集合而非单一初始位置的方法。
...
### [162] [Blind Refusal: Language Models Refuse to Help Users Evade Unjust, Absurd, and Illegitimate Rules](https://arxiv.org/abs/2604.06233)
*Cameron Pattison,Lorenzo Manuali,Seth Lazar*

Main category: cs.AI

TL;DR: 研究发现语言模型存在"盲目拒绝"问题：即使面对不合理或不公正的规则，模型也会机械性地拒绝帮助用户规避规则，缺乏对规则合法性的道德判断能力。


<details>
  <summary>Details</summary>
Motivation: 当前安全训练的语言模型在面对规避规则的请求时普遍采取拒绝态度，但这种拒绝缺乏道德推理能力。并非所有规则都值得遵守，当规则来自非法权威、内容不公正或存在合理例外时，模型的机械性拒绝反映了道德判断的缺失。

Method: 构建包含5种规则失效类型和19种权威类型的合成案例数据集，通过三层自动化质量检查和人工审核。使用18种模型配置（来自7个模型家族），采用盲审的GPT-5.4 LLM作为评判员，从两个维度分类模型响应：响应类型（帮助、硬性拒绝或转移）以及模型是否识别规则失效的原因。

Result: 模型对75.4%（N=14,650）的失效规则请求采取拒绝态度，即使这些请求本身不涉及安全或双重用途问题。57.5%的情况下模型虽然识别了规则失效的原因，但仍然拒绝提供帮助，表明模型的拒绝行为与其规范推理能力脱节。

Conclusion: 语言模型存在"盲目拒绝"问题，即机械性地拒绝帮助规避规则，而不考虑规则本身的合法性或公正性。这表明当前的安全训练方法可能导致模型缺乏必要的道德推理能力，需要在安全训练中融入更细致的规范判断。
...
### [163] [Toward Reducing Unproductive Container Moves: Predicting Service Requirements and Dwell Times](https://arxiv.org/abs/2604.06251)
*Elena Villalobos,Adolfo De Unánue T.,Fernanda Sobrino,David Aké,Stephany Cisneros,Jorge Lecona,Alejandra Matadamaz*

Main category: cs.AI

TL;DR: 本文通过机器学习模型预测集装箱的预清关需求和停留时间，以减少集装箱码头的无效移动，提升运营效率。


<details>
  <summary>Details</summary>
Motivation: 集装箱码头运营中存在大量无效的集装箱移动，这些移动消耗资源且降低效率。通过预测集装箱的预清关服务需求和停留时间，可以优化堆场操作的战略规划和资源配置。

Method: 开发并评估机器学习模型，利用历史运营数据预测集装箱的预清关服务需求和停留时间。在数据准备过程中，实施了货物描述分类系统和收货人记录去重，以提高数据一致性和特征质量。

Result: 在多个时间验证期间，提出的模型在精确率和召回率方面持续优于现有的基于规则的启发式方法和随机基线。这些预测能力为堆场操作的战略规划和资源分配提供了有价值的输入。

Conclusion: 研究结果证明了预测分析在提高集装箱码头物流运营效率和支持数据驱动决策方面的实际价值。机器学习方法能够有效预测集装箱需求，从而减少无效移动并优化资源配置。
...
### [164] [Weakly Supervised Distillation of Hallucination Signals into Transformer Representations](https://arxiv.org/abs/2604.06277)
*Shoaib Sadiq Salehmohamed,Jinal Prashant Thakkar,Hansika Aredla,Shaik Mohammed Omar,Shalmali Ayachit*

Main category: cs.AI

TL;DR: 提出一种弱监督框架，通过三种互补的grounding信号自动标注LLM生成的回答是否幻觉，并训练基于内部激活的探测分类器，实现无需外部验证的幻觉检测。


<details>
  <summary>Details</summary>
Motivation: 现有LLM幻觉检测方法依赖推理时的外部验证（如黄金答案、检索系统或辅助判断模型），需要外部监督。研究探索是否能在训练时将这种外部监督蒸馏到模型自身表示中，实现仅通过内部激活进行幻觉检测。

Method: 1. 提出弱监督框架，结合三种grounding信号自动标注：子串匹配、句子嵌入相似度、LLM作为判断器；2. 基于SQuAD v2构建15,000样本数据集，包含LLaMA-2-7B生成答案及其各层隐藏状态；3. 训练五种探测分类器（ProbeMLP、LayerWiseMLP、CrossLayerTransformer、HierarchicalTransformer、CrossLayerAttentionTransformerV2），直接对隐藏状态进行分类。

Result: Transformer-based探测分类器表现最佳，其中M2在5折平均AUC/F1上最好，M3在单折验证和保留测试集评估上最好。推理效率高：探测延迟0.15-6.66ms，端到端生成加探测吞吐量约0.231查询/秒，实际开销可忽略。

Conclusion: 幻觉检测信号可以被蒸馏到transformer表示中，实现无需外部验证的内部检测。弱监督框架有效，基于内部激活的探测分类器在保持高效推理的同时实现了良好的幻觉检测性能。
...
### [165] [SymptomWise: A Deterministic Reasoning Layer for Reliable and Efficient AI Systems](https://arxiv.org/abs/2604.06375)
*Isaac Henry,Avery Byrne,Christopher Giza,Ron Henry,Shahram Yazdani*

Main category: cs.AI

TL;DR: SymptomWise框架通过分离语言理解和诊断推理，结合专家医学知识、确定性代码驱动推理和受限使用大语言模型，提高AI症状分析系统的可靠性、可解释性并减少幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI驱动的症状分析系统在可靠性、可解释性和幻觉问题方面面临持续挑战。端到端的生成方法往往缺乏可追溯性，在安全关键场景中可能产生无支持或不一致的诊断输出。

Method: 提出SymptomWise框架，将语言理解与诊断推理分离。系统结合专家策划的医学知识、确定性代码驱动推理和受限使用大语言模型。自由文本输入被映射到验证过的症状表示，然后由在有限假设空间上操作的确定性推理模块评估，生成排名鉴别诊断。语言模型仅用于症状提取和可选解释，不用于诊断推理。
...
### [166] [ProofSketcher: Hybrid LLM + Lightweight Proof Checker for Reliable Math/Logic Reasoning](https://arxiv.org/abs/2604.06401)
*Kranthi Kommuru,Kunal Khanvilkar,Gaurav Parekh*

Main category: cs.AI

TL;DR: 论文提出混合方法：用LLM生成类型化证明草图，再用轻量级可信内核扩展为显式证明义务，结合LLM的推理能力和形式验证的可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数学和逻辑领域能生成有说服力的论证，但常包含细微错误（如忽略边界条件、无效推理模式、引用无法逻辑推导的引理），这些错误仅从文本难以发现。而交互式定理证明器（如Lean、Coq）虽能提供严格可靠性保证，但代价高昂：需要完全形式化的证明和大量底层细节信息。

Method: 提出混合流水线：1）LLM在紧凑的领域特定语言中生成类型化证明草图；2）轻量级可信内核将草图扩展为显式证明义务，确保形式验证的可靠性。

Result: 该方法结合了LLM的推理能力和形式验证的可靠性，通过类型化证明草图作为中间表示，既利用了LLM的生成能力，又通过可信内核保证了证明的正确性。

Conclusion: 混合方法有效解决了LLM生成证明中的细微错误问题，同时避免了完全形式化证明的高昂代价，为数学和逻辑推理提供了更高效可靠的解决方案。
...
### [167] [BDI-Kit Demo: A Toolkit for Programmable and Conversational Data Harmonization](https://arxiv.org/abs/2604.06405)
*Roque Lopez,Yurong Liu,Christos Koutras,Juliana Freire*

Main category: cs.AI

TL;DR: BDI-Kit是一个可扩展的数据协调工具包，提供Python API和AI辅助聊天界面两种接口，支持通过编程或自然语言对话进行数据模式与值匹配。
...
Method: BDI-Kit提供两种互补接口：1) Python API允许开发者以编程方式构建协调管道；2) AI辅助聊天界面让领域专家通过自然语言对话协调数据。工具结合自动匹配、AI辅助推理和用户驱动的细化。

Result: 演示展示了两种使用场景：使用Python API编程组合原语、检查中间输出和重用转换；与AI助手自然语言对话访问BDI-Kit功能并基于助手建议迭代细化输出。

Conclusion: BDI-Kit通过提供灵活的工具接口，支持不同用户需求的数据协调工作流程，结合自动化、AI辅助和用户交互实现有效的数据集成。
...
### [168] [Rethinking Generalization in Reasoning SFT: A Conditional Analysis on Optimization, Data, and Model Capability](https://arxiv.org/abs/2604.06628)
*Qihan Ren,Peng Wang,Ruikun Cai,Shuai Shao,Dadi Guo,Yuejin Xie,Yafu Li,Quanshi Zhang,Xia Hu,Jing Shao,Dongrui Liu*

Main category: cs.AI

TL;DR: 研究发现监督微调（SFT）在长思维链监督下具有条件性跨领域泛化能力，而非完全记忆。泛化效果受优化动态、数据质量和模型能力共同影响，呈现先降后升的"下降-恢复"模式，且存在推理能力提升但安全性下降的不对称性。


<details>
  <summary>Details</summary>
Motivation: 挑战LLM后训练中流行的观点——监督微调（SFT）只是记忆，而强化学习（RL）才泛化。研究旨在重新审视长思维链监督下的推理SFT是否真的缺乏跨领域泛化能力。

Method: 通过分析优化动态、训练数据和基础模型能力三个维度来研究推理SFT的泛化特性。具体包括：观察训练过程中的性能变化模式（下降-恢复），评估不同质量思维链数据的影响，以及比较不同能力基础模型的学习行为差异。

Result: 1）推理SFT具有条件性跨领域泛化能力；2）训练不足会导致泛化能力被低估（下降-恢复模式）；3）高质量验证的长思维链数据带来一致的跨领域增益；4）强模型能内化可迁移的程序模式，弱模型仅模仿表面冗长；5）泛化具有不对称性：推理能力提升但安全性下降。

Conclusion: 推理SFT的泛化问题应从"是否泛化"转变为"在什么条件下、以什么代价泛化"。泛化能力受优化动态、数据质量和模型能力共同塑造，且存在推理能力与安全性之间的权衡。
...
### [169] [Reasoning Fails Where Step Flow Breaks](https://arxiv.org/abs/2604.06695)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂任务上表现良好但行为不稳定难解释，现有分析工具难以处理长结构化推理轨迹，需要新的分析方法

Method: 引入Step-Saliency方法，将注意力-梯度分数池化为步骤间映射；提出StepFlow干预方法，通过Odds-Equal Bridge调整浅层显著性模式，通过Step Momentum Injection添加深层步骤级残差

Result: Step-Saliency揭示了两种信息流故障模式：浅层锁定和深层衰减；StepFlow无需重新训练即可在数学、科学和编程任务上提高多个大型推理模型的准确性

Conclusion: 修复信息流可以恢复大型推理模型缺失的推理性能，Step-Saliency为分析长推理轨迹提供了有效工具，StepFlow展示了基于显著性分析的测试时干预潜力
...
### [170] [ATANT: An Evaluation Framework for AI Continuity](https://arxiv.org/abs/2604.06710)
*Samuel Sameer Tanguturi*

Main category: cs.AI

TL;DR: ATANT是一个评估AI系统连续性的自动化测试框架，通过250个故事和1835个验证问题来衡量系统在时间维度上保持、更新、消歧和重建有意义上下文的能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI行业虽然开发了各种记忆组件（RAG管道、向量数据库、长上下文窗口、配置文件层），但缺乏正式框架来定义和衡量这些组件是否真正产生连续性。需要系统化的方法来评估AI系统在长期交互中保持上下文一致性的能力。

Method: 定义了连续性作为系统属性所需的7个特性，引入了包含10个检查点的评估方法（无需LLM参与评估循环），创建了包含250个故事、1835个验证问题的叙事测试语料库，涵盖6个生活领域。评估采用参考实现，通过5次测试套件迭代进行验证。

Result: 参考实现从传统架构的58%准确率提升到孤立模式下的100%（250个故事），50个故事累积模式下的100%，以及250个故事累积规模下的96%。累积结果是主要衡量标准，要求系统在250个不同生活叙事共存时能正确检索事实而不产生交叉污染。
...
### [171] [Steering the Verifiability of Multimodal AI Hallucinations](https://arxiv.org/abs/2604.06714)
*Jianhong Pang,Ruoxi Cheng,Ziyi Ye,Xingjun Ma,Zuxuan Wu,Xuanjing Huang,Yu-Gang Jiang*

Main category: cs.AI

TL;DR: 本文提出了一种针对多模态大语言模型幻觉的细粒度控制方法，通过区分明显幻觉和难以察觉的幻觉，使用激活空间干预技术实现对模型可验证性的灵活调控。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型产生的幻觉对人类用户构成风险，但不同幻觉的可验证性差异很大：有些容易被人类察觉（明显幻觉），有些则难以发现或需要更多验证努力（难以察觉的幻觉）。目前缺乏针对这种可验证性差异进行控制的研究方法。

Method: 首先从4,470个人类对AI生成幻觉的响应中构建数据集，根据人类可验证性将幻觉分为明显和难以察觉两类。然后提出激活空间干预方法，为这两种幻觉类型学习独立的探测模型，实现对模型可验证性的细粒度控制。

Result: 实验结果显示，明显幻觉和难以察觉的幻觉引发不同的干预探测模型，使得能够精确控制模型的可验证性。针对性干预在调节相应可验证性方面表现优异，而简单混合这些干预可以实现对不同场景所需可验证性的灵活控制。

Conclusion: 该研究首次探索了多模态AI幻觉的可验证性控制问题，提出的激活空间干预方法能够根据安全性和可用性需求，灵活调节模型产生幻觉的可验证性，为不同应用场景提供了定制化的幻觉控制方案。
...
### [172] [TurboAgent: An LLM-Driven Autonomous Multi-Agent Framework for Turbomachinery Aerodynamic Design](https://arxiv.org/abs/2604.06747)
*Juan Du,Yueteng Wu,Pan Zhao,Yuze Liu,Min Zhang,Xiaobin Xu,Xinglong Zhang*

Main category: cs.AI

TL;DR: 本文提出TurboAgent，一个基于大语言模型驱动的自主多智能体框架，用于涡轮机械气动设计与优化，实现从自然语言需求到最终设计的全自动闭环设计流程。


<details>
  <summary>Details</summary>
Motivation: 涡轮机械气动设计是一个复杂且紧密耦合的多阶段过程，现有智能设计方法通常只关注单个阶段或依赖松耦合流程，难以实现完全自主的端到端设计。

Method: 提出TurboAgent框架，以大语言模型为核心进行任务规划和协调，配合专门智能体处理生成设计、快速性能预测、多目标优化和基于物理的验证，将传统试错设计转变为数据驱动的协同工作流。

Result: 使用跨音速单转子压缩机验证，结果显示目标性能、生成设计和CFD模拟之间具有强一致性。质量流量、总压比和等熵效率的决定系数均超过0.91，归一化RMSE值低于8%。优化智能体将等熵效率提高1.61%，总压比提高3.02%，完整工作流可在约30分钟内并行执行。
...
### [173] [What's Missing in Screen-to-Action? Towards a UI-in-the-Loop Paradigm for Multimodal GUI Reasoning](https://arxiv.org/abs/2604.06995)
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI推理任务在UI理解方面仍具挑战性，当前方法通常依赖基于屏幕的直接决策，缺乏可解释性且忽视对UI元素的全面理解，最终导致任务失败。

Method: 提出UILoop范式，将GUI推理建模为循环的屏幕-UI元素-动作过程；让多模态大语言模型显式学习UI元素的定位、语义功能和实际使用方法；引入包含三个评估指标的UI理解任务，并贡献包含26K样本的UI Comprehension-Bench基准。

Result: UILoop在UI理解性能上达到最先进水平，同时在GUI推理任务中取得优异结果。
...
### [174] [EmoMAS: Emotion-Aware Multi-Agent System for High-Stakes Edge-Deployable Negotiation with Bayesian Orchestration](https://arxiv.org/abs/2604.07003)
*Yunbo Long,Yunhan Liu,Liming Xu*

Main category: cs.AI

TL;DR: EmoMAS是一个贝叶斯多智能体框架，通过战略性的情感决策提升小型语言模型在边缘设备上的高风险谈判性能，无需预训练即可在线学习策略。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自动谈判中计算成本高且存在隐私风险，不适合部署在移动助手、救援机器人等隐私敏感的边缘设备上。小型语言模型虽然适合边缘部署，但难以处理高风险谈判中的复杂情感动态。

Method: EmoMAS采用贝叶斯多智能体框架，包含一个贝叶斯协调器来协调三个专门化智能体：博弈论模型、强化学习模型和心理一致性模型。系统融合它们的实时洞察来优化情感状态转换，同时根据谈判反馈持续更新智能体可靠性。这种混合智能体架构支持无需预训练的在线策略学习。

Result: 在债务、医疗、应急响应和教育四个高风险边缘可部署谈判基准测试中，通过广泛的智能体间模拟，配备EmoMAS的小型和大型语言模型在所有基准测试中始终超越所有基线模型，在谈判性能和伦理行为之间取得平衡。

Conclusion: 战略性情感智能是谈判成功的关键驱动因素。通过将情感表达作为贝叶斯多智能体优化框架中的战略变量，EmoMAS为高风险边缘部署建立了有效、私密且自适应的谈判AI新范式。
...
### [175] [CAFP: A Post-Processing Framework for Group Fairness via Counterfactual Model Averaging](https://arxiv.org/abs/2604.07009)
...
Method: CAFP通过为每个输入生成敏感属性翻转的反事实版本，然后平均模型在事实和反事实实例上的预测结果。这是一种模型无关的后处理方法，不需要重新训练或修改原始分类器。

Result: 理论分析表明，CAFP消除了对受保护属性的直接依赖，减少了预测与敏感属性之间的互信息，并相对于原始模型可证明地限制了引入的失真。在温和假设下，CAFP实现了完美的人口统计均等，并将均衡机会差距减少了至少一半的平均反事实偏差。

Conclusion: CAFP提供了一种实用且理论上有保证的方法，可以在不重新训练模型的情况下减轻机器学习预测中的不公平影响，特别适用于无法控制模型架构或无法访问敏感属性的现实场景。
...
### [176] [A-MBER: Affective Memory Benchmark for Emotion Recognition](https://arxiv.org/abs/2604.07017)
...
Result: 实验比较了局部上下文、长上下文、检索记忆、结构化记忆和黄金证据条件。A-MBER在其设计的子集上特别具有区分性，包括长范围隐式情感、高依赖性记忆水平、基于轨迹的推理和对抗设置。

Conclusion: 记忆支持情感解释不仅仅是提供更多历史，而是通过实现更有选择性、更接地气和更上下文敏感的过去交互使用。A-MBER填补了评估AI助手情感记忆能力的重要空白。
...
### [177] [Planning Task Shielding: Detecting and Repairing Flaws in Planning Tasks through Turning them Unsolvable](https://arxiv.org/abs/2604.07042)
...
<details>
  <summary>Details</summary>
Motivation: 传统规划研究主要关注如何生成达成目标的计划，但目标规范也可以用来编码不应出现的属性。当需要确保某些缺陷状态永远不会被到达时，目标转变为修改规划任务使其不可解，从而屏蔽系统缺陷

Method: 提出$allmin$算法，通过最小化修改原始动作来使规划任务不可解，解决规划任务屏蔽问题。该方法检测并修复规划任务中的缺陷

Result: 通过实验评估$allmin$在不同规模规划任务上的性能，证明其能有效屏蔽系统，使规划任务变得不可解

Conclusion: 规划任务屏蔽是一个重要问题，$allmin$算法能够通过最小化修改动作来有效解决该问题，确保系统不会到达缺陷状态
...
### [178] [EVGeoQA: Benchmarking LLMs on Dynamic, Multi-Objective Geo-Spatial Exploration](https://arxiv.org/abs/2604.07070)
...
Result: 实验表明，LLM能够成功利用工具处理子任务，但在长距离空间探索方面存在困难；同时观察到LLM能够总结历史探索轨迹来提高探索效率的新兴能力。
...
### [179] [How Much LLM Does a Self-Revising Agent Actually Need?](https://arxiv.org/abs/2604.07236)
*Seongwoo Jeong,Seonil Son*

Main category: cs.AI

TL;DR: 论文提出了一种声明式反射运行时协议，将智能体状态、置信信号、受保护动作和假设转换外部化为可检查的运行时结构，用于分解LLM智能体的能力来源。
...
Result: 分解出四个组件：后验信念跟踪、显式世界模型规划、符号化回合内反思和稀疏LLM修订。显式世界模型规划相比贪婪后验跟踪基线显著提升（胜率+24.1pp，F1+0.017）。符号化反思作为实时运行时机制运行，但当前修订预设尚未产生净正收益。在约4.3%的回合添加条件性LLM修订仅带来微小且非单调变化。

Conclusion: 外部化反思将原本潜在的智能体行为转化为可检查的运行时结构，使得能够直接研究LLM干预的边际作用，这代表了一种方法论贡献而非排行榜声明。
...
### [180] [MedRoute: RL-Based Dynamic Specialist Routing in Multi-Agent Medical Diagnosis](https://arxiv.org/abs/2604.06180)
*Ashmal Vayani,Parth Parag Kulkarni,Joseph Fioresi,Song Wang,Mubarak Shah*

Main category: eess.IV

TL;DR: MedRoute是一个动态多智能体LMM框架，通过模拟临床多专家会诊流程，使用强化学习训练的路由器动态选择专科医生，提高了医疗诊断的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型在医疗诊断中过于通用，无法适应真实医疗场景的多样性。临床实践中诊断通常由多个专科医生协作完成，但现有方法依赖静态或预定义的专家选择，无法适应变化的实际场景。

Method: 提出MedRoute框架，包含：1）专科医生LMM智能体系统；2）配备强化学习训练路由器的全科医生，用于动态选择专科医生；3）协调员生成最终决策。该框架模拟真实临床工作流程。

Result: 在基于文本和图像的医疗数据集上进行广泛评估，结果显示诊断准确性显著提升，超越了现有最先进的基线方法。

Conclusion: MedRoute为未来研究奠定了坚实基础，其动态多智能体框架更好地模拟了真实临床诊断流程，提高了医疗诊断的准确性和适应性。
...
### [181] [Structural Regularities of Cinema SDR-to-HDR Mapping in a Controlled Mastering Workflow: A Pixel-wise Case Study on ASC StEM2](https://arxiv.org/abs/2604.06276)
*Xin Zhang,Xiaoyi Chen*

Main category: eess.IV

TL;DR: 基于ASC StEM2数据集对影院SDR到HDR映射的实证研究，通过像素级统计分析揭示了SDR和HDR母版在亮度和色彩结构上的稳定对应关系，并提出了基于EXR源数据的决策映射方法。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过罕见的共同源数据集（ASC StEM2）来实证分析影院SDR到HDR映射的特性，该数据集包含EXR场景参考图像和匹配的SDR/HDR影院发行母版，为在受控工作流程下理解两者关系提供了独特机会。

Method: 使用ASC StEM2数据集（18,580帧测试影片），进行像素级统计分析，构建EXR源数据、SDR发行母版和HDR发行母版的三域比较，分析亮度和色彩结构关系，并基于EXR定义像素级决策映射来区分EXR更接近恢复区域和内容自适应调整区域。

Result: 亮度维度：SDR和HDR母版呈现高度稳定的全局单调对应关系，几何结构基本一致，仅在自发光高光和特定材质区域出现稀疏结构化偏差。色彩维度：色调基本一致，饱和度呈现阴影抑制、中间调扩展和高光收敛的重新分布模式。基于EXR的决策映射显示82.4%的图像区域被分类为EXR更接近恢复区域，其余需要局部自适应调整。

Conclusion: 该研究为结构感知的SDR到HDR分析提供了可解释的量化基准，并为共享源母版条件下的基于学习模型设计提供了基础，但并非声称适用于所有影院母版流程的普遍规律。
...
### [182] [Adaptive Differential Privacy for Federated Medical Image Segmentation Across Diverse Modalities](https://arxiv.org/abs/2604.06518)
*Puja Saha,Eranga Ukwatta*

Main category: eess.IV

TL;DR: 提出自适应差分隐私联邦学习框架ADP-FL，用于医学图像分割，在保证隐私的同时显著提升分割精度和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 医疗数据分散且隐私保护严格，集中训练不可行；传统联邦学习加入差分隐私后导致精度下降、训练不稳定；需要平衡隐私保护与模型性能。

Method: 提出自适应差分隐私联邦学习框架ADP-FL，动态调整隐私机制以优化隐私-效用权衡，包括皮肤病变、肾脏肿瘤、脑肿瘤等多种医学图像分割任务。

Result: 相比传统联邦学习和标准差分隐私联邦学习，ADP-FL在Dice分数、边界分割质量、收敛速度和训练稳定性方面均有显著提升，性能接近非隐私联邦学习。

Conclusion: ADP-FL框架在实际联邦学习场景中实现了高性能、隐私保护的医学图像分割，具有实际应用价值。
...
### [183] [Accelerating 4D Hyperspectral Imaging through Physics-Informed Neural Representation and Adaptive Sampling](https://arxiv.org/abs/2604.06561)
*Chi-Jui Ho,Harsh Bhakta,Wei Xiong,Nicholas Antipa*

Main category: eess.IV

TL;DR: 提出基于物理信息神经表示的方法，从稀疏实验测量中高效重建密集空间分辨的2DIR高光谱图像，将采样需求减少到1/32，实验时间缩短32倍


<details>
  <summary>Details</summary>
Motivation: 高维高光谱成像在二维红外光谱中应用时，由于密集奈奎斯特采样要求和大量信号累积需求，导致数据采集时间过长，限制了其在空间变化振动耦合研究中的应用

Method: 采用物理信息神经表示方法，使用多层感知器建模稀疏采样的4D坐标与对应光谱强度关系，从有限观测中恢复密集采样的4D光谱；开发损失感知自适应采样方法，在实验过程中逐步引入潜在信息样本

Result: 该方法仅使用1/32的采样预算就能实现高保真光谱恢复，有效减少总实验时间达32倍，能够准确恢复实验测量中的振荡和非振荡光谱动态

Conclusion: 该框架为加速超立方体数据实验提供了可扩展解决方案，包括多维光谱和高光谱成像，为快速化学成像瞬态生物和材料系统开辟了新途径
...
### [184] [A Noise Constrained Diffusion (NC-Diffusion) Framework for High Fidelity Image Compression](https://arxiv.org/abs/2604.06568)
*Zhenyu Du,Yanbo Gao,Shuai Li,Yiyang Li,Hui Yuan,Mao Ye*

Main category: eess.IV

TL;DR: 提出NC-Diffusion框架，通过约束量化噪声作为扩散前向过程的噪声，解决扩散模型图像压缩中的噪声不匹配问题，提高重建保真度和推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的图像压缩方法因引入随机噪声导致重建图像与原始图像存在偏差，压缩效果不理想。需要解决扩散学习中的噪声不匹配问题。

Method: 1) 将量化噪声作为扩散前向过程的噪声，构建从原始图像到含量化噪声的初始压缩结果的噪声约束扩散过程；2) 开发自适应频域滤波模块增强U-Net架构中的跳跃连接；3) 设计零样本样本引导增强方法进一步提高图像保真度。

Result: 在多个基准数据集上的实验表明，该方法相比现有方法取得了最佳性能，显著提高了推理效率和重建图像质量。

Conclusion: NC-Diffusion框架通过约束量化噪声解决了扩散模型图像压缩中的噪声不匹配问题，结合频域滤波和零样本增强技术，实现了高保真度的图像压缩。
...
### [185] [LungCURE: Benchmarking Multimodal Real-World Clinical Reasoning for Precision Lung Cancer Diagnosis and Treatment](https://arxiv.org/abs/2604.06925)
...
TL;DR: 提出LungCURE基准和LCAgent框架，解决肺癌临床决策支持中多模态大语言模型无法处理指南约束的分期和治疗推理问题


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型无法处理指南约束的肺癌分期和治疗推理，需要标准化基准和确保指南合规的临床决策支持系统

Method: 提出LungCURE基准（基于1000个真实病例）和LCAgent多智能体框架，通过抑制临床路径中的级联推理错误来确保指南合规的肺癌临床决策

Result: 实验显示不同大语言模型在复杂医学推理能力上存在显著差异，LCAgent作为简单有效的插件能增强LLMs在真实医疗场景中的推理性能

Conclusion: LungCURE为肺癌临床决策支持提供了首个标准化多模态基准，LCAgent框架能有效确保指南合规的临床决策制定，增强LLMs在医疗场景中的推理能力
