...
Motivation: 现有音频驱动视频数字人生成模型依赖多步去噪，计算开销大，限制了实际部署。一步蒸馏方法能显著加速推理，但存在训练不稳定问题。

Method: 提出两阶段渐进蒸馏框架：1) 使用分布匹配蒸馏获得稳定4步学生模型；2) 通过对抗蒸馏将去噪步数从4步渐进减少到1步。引入渐进时间步采样策略和自比较对抗目标来稳定训练。

Result: 实现了视频说话头像的单步生成，推理速度提升120倍，同时保持高质量生成效果。

Conclusion: TurboTalk通过渐进蒸馏方法成功解决了音频驱动视频生成模型的计算效率问题，在保持质量的同时实现了显著加速，为实际部署提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 在HY-World 1.0基础上，开发更先进的多模态3D世界生成框架，支持多种输入模态并生成高质量可导航3D场景，同时提供完整的开源解决方案

Method: 采用四阶段方法：1) HY-Pano 2.0全景图生成，2) WorldNav轨迹规划，3) WorldStereo 2.0世界扩展（带一致性记忆的关键帧视图生成），4) WorldMirror 2.0世界组合（改进架构和学习策略的通用3D预测）。还开发了WorldLens高性能3DGS渲染平台

Result: 在多个基准测试中达到开源方法的最先进性能，结果可与闭源模型Marble相媲美，实现了高质量可交互3D世界探索

Conclusion: HY-World 2.0成功构建了一个强大的多模态3D世界生成框架，通过技术创新实现了高质量3D场景合成，并开源所有模型权重和代码以促进相关研究发展
...
<details>
  <summary>Details</summary>
Motivation: 推动视频显著性预测技术的发展，为视频内容分析和理解提供自动化的注意力区域检测方法

Method: 使用众包鼠标追踪收集超过5000名评估者的注视点和显著性图，构建包含2000个多样化视频的开放许可数据集

Result: 超过20个团队参与挑战，7个团队通过代码审查进入最终阶段，所有数据已公开可用

Conclusion: NTIRE 2026挑战赛成功推动了视频显著性预测领域的发展，提供了高质量的数据集和基准测试平台
...
TL;DR: 从单张手绘草图生成几何一致的多视角场景，解决了现有方法需要照片、文本或多视角输入的限制，通过构建数据集、几何注意力适配器和对应监督损失实现高效单次生成。


<details>
  <summary>Details</summary>
Motivation: 手绘草图是最几何信息贫乏的多视角生成输入，现有方法需要照片或文本输入，而草图到3D方法需要多视角或昂贵的逐场景优化。本文旨在解决从单张草图直接生成几何一致多视角场景的新问题。

Method: 提出三个相互增强的贡献：(1) 构建约9k个草图到多视角样本的数据集，通过自动生成和过滤流程；(2) 并行相机感知注意力适配器(CA3)，将几何归纳偏置注入视频变换器；(3) 从运动恢复结构重建中导出的稀疏对应监督损失(CSL)。

Result: 框架在单次去噪过程中合成所有视角，无需参考图像、迭代细化或逐场景优化。显著优于最先进的两阶段基线，真实感(FID)提升超过60%，几何一致性(Corr-Acc)提升23%，推理速度提升达3.7倍。

Conclusion: 成功解决了从单张手绘草图生成几何一致多视角场景的挑战性问题，通过数据构建、几何建模和监督损失的综合方法，实现了高效且高质量的生成效果。
...
### [5] [Design and Validation of a Low-Cost Smartphone Based Fluorescence Detection Platform Compared with Conventional Microplate Readers](https://arxiv.org/abs/2604.14527)
*Zhendong Cao,Katrina G. Salvante,Ash Parameswaran,Pablo A. Nepomnaschy,Hongji Dai*

Main category: cs.CV

TL;DR: 开发低成本荧光检测系统，使用智能手机摄像头替代传统微孔板阅读器，通过RGB颜色分析检测微生物和分子浓度


<details>
  <summary>Details</summary>
Motivation: 传统微孔板阅读器（如Perkin Elmer Victor）价格昂贵，包含激发滤光片、阻挡滤光片和光电倍增管等昂贵组件。需要开发低成本替代方案，使荧光检测更加普及和经济

Method: 设计兼容常规96孔板的设备设置，使用智能手机摄像头作为光学检测器。通过分析样品在RGB颜色空间中的图像颜色，建立与荧光样本摩尔浓度之间的关系

Result: 成功开发出低成本荧光检测系统，无需传统昂贵组件，仅需智能手机摄像头即可检测样品中的荧光信号

Conclusion: 智能手机摄像头可以作为有效的低成本荧光检测工具，为微生物和分子检测提供了经济实用的替代方案，有望促进相关检测技术的普及应用
...
### [6] [RaTA-Tool: Retrieval-based Tool Selection with Multimodal Large Language Models](https://arxiv.org/abs/2604.14951)
...
Method: 1) 使用MLLM将多模态查询转换为结构化任务描述；2) 通过语义匹配将任务描述与机器可读的工具描述进行检索；3) 使用直接偏好优化(DPO)进行偏好优化以改进任务描述与工具选择的匹配度。

Result: 实验表明该方法显著提高了工具选择性能，特别是在开放世界多模态场景中。同时创建了首个开放世界多模态工具使用数据集，基于Hugging Face模型卡片构建标准化工具描述。
...
TL;DR: 提出了一种基于姿态的混合方法，用于在监控视频中检测"抢夺-逃跑"式非暴力街头抢劫行为，结合实时感知和可解释分类，适合边缘设备部署。


<details>
  <summary>Details</summary>
Motivation: 非暴力街头抢劫（抢夺-逃跑）由于短暂、细微且与正常人类互动难以区分，在无约束监控视频中难以自动检测，需要一种能够在边缘设备上实时运行的检测方法。

Method: 使用YOLO姿态估计器提取人体关键点，计算手部速度、手臂伸展、接近度和攻击者-受害者相对运动等运动学和交互特征，通过随机森林分类器进行分类，并应用时间滞后滤波器稳定帧级预测。

Result: 在模拟数据集和互联网视频测试集上评估，展示了在不同场景和摄像机视角下的良好泛化能力，在NVIDIA Jetson Nano上实现实时性能，支持主动式设备端抢劫检测。

Conclusion: 提出的混合姿态驱动方法能够有效检测非暴力街头抢劫事件，在边缘设备上实现实时性能，为主动式设备端抢劫检测提供了可行性。
...
TL;DR: SatBLIP是一个卫星视觉语言框架，通过生成卫星图像的结构化描述来预测县级社会脆弱性指数，解决了传统遥感方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 农村环境风险受地方条件影响，但现有脆弱性指数过于粗糙，无法深入了解风险背景。传统遥感方法存在手工特征提取、人工虚拟审计和使用自然图像训练的视觉语言模型等局限性。

Method: 使用GPT-4o生成卫星图块的结构化描述（屋顶类型/状况、房屋大小、庭院属性、绿化、道路环境），然后微调卫星适应的BLIP模型为未见图像生成描述。通过CLIP编码描述，与LLM生成的嵌入通过注意力机制融合，在空间聚合下进行SVI估计。

Result: 使用SHAP识别出显著影响预测的关键属性（屋顶形式/状况、街道宽度、植被、汽车/开放空间），实现了农村风险环境的可解释性映射。

Conclusion: SatBLIP框架通过结合对比性图像文本对齐和针对卫星语义的自举式描述生成，能够有效理解和识别农村环境特征，为农村风险环境提供可解释的预测和映射。
...
TL;DR: FoodSense：首个用于从食物图像预测多感官体验（味觉、嗅觉、触觉、听觉）的数据集和基准模型，包含66,842个人类标注的图像-参与者配对，并生成基于图像的推理轨迹来解释感官预期。


<details>
  <summary>Details</summary>
Motivation: 人类能够从食物图像中推断出味觉、嗅觉、触觉和听觉等多感官体验，这一现象在认知科学中已有研究，但先前计算机视觉研究主要关注识别任务（如餐食识别、成分检测、营养估计），基于图像的跨感官推理预测仍未被充分探索。

Method: 1. 构建FoodSense数据集：包含66,842个参与者-图像配对，覆盖2,987张独特食物图像，每个配对包含四个感官维度（味觉、嗅觉、触觉、听觉）的1-5分数字评分和自由文本描述；2. 使用大语言模型将简短的人类标注扩展为基于图像的推理轨迹，生成视觉理由；3. 训练FoodSense-VL视觉语言基准模型，直接从食物图像生成多感官评分和基于视觉的解释。

Result: 创建了首个用于跨感官推理的人类标注数据集，开发了能够从食物图像预测多感官体验并生成解释的基准模型，同时发现许多流行的评估指标不足以评估视觉感官推理任务。

Conclusion: 该研究将认知科学中的跨感官感知发现与现代多模态模型的指令调优相结合，填补了从食物图像预测多感官体验的研究空白，为理解人类如何从视觉信息推断其他感官体验提供了计算框架。
...
TL;DR: 零消融（将token激活替换为零向量）在视觉Transformer中广泛用于探测token功能，但可能高估了对精确寄存器内容的依赖，因为替代控制方法（均值替换、噪声替换、跨图像寄存器混洗）能保持性能，而零消融导致不成比例的大扰动。


<details>
  <summary>Details</summary>
Motivation: 研究零消融方法在评估视觉Transformer中寄存器功能时的有效性，探讨是否过度强调了寄存器精确内容的重要性。

Method: 使用三种替代控制方法：均值替换、噪声替换和跨图像寄存器混洗，与零消融进行对比。通过分类、对应性和分割任务评估性能，并使用每块余弦相似度测量内部表示扰动程度。

Result: 零消融导致性能大幅下降（分类下降36.6pp，分割下降30.9pp），但三种替代控制方法在各项任务中都能保持性能，与未修改基线相差约1pp。余弦相似度分析显示替代方法确实扰动了内部表示，但零消融导致不成比例的大扰动。

Conclusion: 零消融方法高估了对精确寄存器内容的依赖。在测试的冻结特征评估中，性能依赖于合理的类寄存器激活，而非精确的图像特定值。寄存器仍然缓冲密集特征免受[CLS]依赖影响，并与压缩的块几何结构相关。这些发现在ViT-B规模上可复现。
...
TL;DR: 本文提出了一种结合知识表示、自然语言处理和计算机视觉技术的图像标注方法，通过视觉属性约束减少标注者主观性，并引入基于对象类别层次结构和标注者反馈的动态交互式众包框架。


<details>
  <summary>Details</summary>
Motivation: 当前以数据为中心的人工智能方法在对象识别数据集中存在局限性，主要问题源于语义鸿沟问题，导致视觉数据和语言描述之间存在复杂的多对多映射关系，这种偏差影响了计算机视觉任务的性能。

Method: 提出一种图像标注方法，整合知识表示、自然语言处理和计算机视觉技术，通过视觉属性约束减少标注者主观性。引入交互式众包框架，基于预定义的对象类别层次结构和标注者反馈动态提问，引导图像标注过程。

Result: 实验证明了该方法的有效性，并通过标注者反馈讨论了如何优化众包设置。

Conclusion: 提出的图像标注方法能够有效减少语义鸿沟问题带来的偏差，通过视觉属性约束和交互式众包框架提高标注质量，为计算机视觉任务提供更可靠的数据基础。
...
TL;DR: 提出DAGMaN方法，在Swin Transformer中引入噪声教师和注意力引导掩码机制，解决医学图像自监督学习中的信息泄漏问题，同时保持注意力头多样性。


<details>
  <summary>Details</summary>
Motivation: 医学图像中相邻补丁的上下文相似性导致随机掩码方法存在信息泄漏问题，使自监督学习简化。Swin Transformer缺乏全局[CLS]标记，无法使用先进的掩码方法，且注意力引导掩码会降低注意力头多样性。

Method: 在协同蒸馏学习框架中为Swin Transformer引入注意力引导掩码机制，选择性掩码语义共现和判别性补丁。首次集成噪声教师到协同蒸馏框架（DAGMaN），在保持注意力头多样性的同时执行注意力引导掩码。

Result: 在多个任务上验证DAGMaN的能力，包括全样本和少样本肺结节分类、免疫治疗结果预测、肿瘤分割和无监督器官聚类。

Conclusion: DAGMaN方法有效解决了医学图像自监督学习中的信息泄漏问题，同时保持了注意力头多样性，在多种医学图像分析任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 在少样本异常检测任务中，现有基于视觉语言模型的方法主要依赖成对特征匹配，忽略了结构依赖性和全局一致性，限制了检测性能的进一步提升。

Method: 提出异构超图视觉语言推理框架，将少样本异常检测重新定义为视觉-语义关系的高阶推理问题，在统一超图中联合建模视觉区域和语义概念。

Result: 实验验证了H2VLR框架的有效性和优势，在代表性的工业和医学基准测试中通常能达到最先进的性能。

Conclusion: 通过将少样本异常检测建模为视觉-语义关系的高阶推理问题，H2VLR框架能够更好地利用视觉语言模型的能力，提升异常检测性能。
...
TL;DR: Omni-MLLMs存在性能悖论：单模态基线常优于多模态联合推理。作者提出Chain of Modality框架，通过动态编排输入拓扑和任务对齐的认知执行路径来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 当前Omni-modal大语言模型存在性能悖论，单模态基线经常优于多模态联合推理。作者发现问题的根源在于静态融合拓扑结构，存在位置偏差和对齐陷阱，这些结构性问题会系统性扭曲注意力机制，与任务语义无关。

Method: 提出Chain of Modality框架，将多模态融合从被动连接转变为动态编排。该框架自适应地编排输入拓扑，在并行、顺序和交错路径之间切换以消除结构偏差。同时将认知执行分为两个任务对齐路径：用于直接感知的"Direct-Decide"路径和用于分析审计的"Reason-Decide"路径。

Result: CoM框架在无需训练或数据高效的SFT设置下，能够在多样化基准测试中实现鲁棒且一致的泛化性能。

Conclusion: 通过动态编排输入拓扑和任务对齐的认知执行路径，CoM框架有效解决了Omni-MLLMs中的性能悖论问题，实现了更鲁棒的多模态推理能力。
...
TL;DR: FreqTrack是一个频率感知的RGBE跟踪框架，通过频域变换建立互补的跨模态关联，在COESOT和FE108数据集上取得领先性能，特别是在COESOT上达到76.6%的精度。
...
Method: 提出FreqTrack频率感知RGBE跟踪框架：1) 设计谱增强Transformer(SET)层，通过多头动态傅里叶滤波自适应增强和选择频域特征；2) 开发小波边缘细化(WER)模块，利用可学习小波变换显式提取事件数据中的多尺度边缘结构。

Result: 在COESOT和FE108数据集上的大量实验表明，FreqTrack实现了高度竞争力的性能，特别是在COESOT基准测试中达到76.6%的领先精度。

Conclusion: 频域建模对RGBE跟踪是有效的，FreqTrack通过频域变换建立互补的跨模态关联，在高速和低光场景中显著提升了跟踪性能。
...
TL;DR: WILD-SAM：一种参数高效的微调框架，通过相位感知专家混合适配器和小波引导子带增强策略，将SAM模型适配于包裹相位干涉图的滑坡检测任务，显著提升了检测精度。


<details>
  <summary>Details</summary>
Motivation: 从包裹相位InSAR干涉图中直接检测慢速滑坡对地质灾害监测至关重要，但面临严重的相位模糊和复杂相干噪声挑战。虽然SAM模型提供了强大的分割基础，但其直接应用于包裹相位数据时存在显著的谱域偏移，抑制了对边界划分至关重要的高频条纹。

Method: 提出WILD-SAM框架：1）在冻结编码器中集成相位感知专家混合适配器（PA-MoE），通过异构卷积专家间的动态路由机制自适应聚合多尺度谱纹理先验，对齐自然图像与干涉相位数据的分布差异；2）引入小波引导子带增强策略（WGSE），利用离散小波变换显式解耦高频子带并细化方向性相位纹理，将这些结构线索作为密集提示注入以确保滑坡边界的拓扑完整性。

Result: 在ISSLIDE和ISSLIDE+基准测试上的大量实验表明，WILD-SAM实现了最先进的性能，在目标完整性和轮廓保真度方面显著优于现有方法。

Conclusion: WILD-SAM成功地将SAM模型适配于包裹相位干涉图的滑坡检测任务，通过谱分布对齐和频率感知提示生成，有效解决了相位模糊和相干噪声带来的挑战，为地质灾害监测提供了高效精准的解决方案。
...
TL;DR: 提出了一种新颖的视频对象插入方法，通过多视角对象先验解决外观不一致和遮挡处理问题，确保稳定的身份引导和跨视角的鲁棒集成。
...
Method: 1. 将2D参考图像提升为多视角表示；2. 采用双路径视角一致条件机制；3. 使用质量感知加权机制处理噪声或不完美输入；4. 引入集成感知一致性模块保证空间真实感。

Result: 实验结果表明，该方法显著提高了视频对象插入的质量，提供了稳定且真实的集成效果。

Conclusion: 该方法通过多视角对象先验和集成感知一致性模块，有效解决了视频对象插入中的外观不一致和遮挡处理问题，实现了高质量的视频对象插入。
...
TL;DR: NTIRE 2026图像超分辨率挑战赛（×4倍）包含恢复赛道和感知赛道，旨在推动超分辨率技术发展并提供统一基准。


<details>
  <summary>Details</summary>
Motivation: 为了反映图像超分辨率领域不断发展的目标，设计一个包含像素级保真度和视觉真实性的综合挑战，以分析该领域的最新进展。

Method: 挑战赛分为两个赛道：1）恢复赛道，强调像素级保真度，使用PSNR进行排名；2）感知赛道，关注视觉真实性，使用感知分数进行评估。采用双三次下采样生成×4倍的低分辨率输入。

Result: 共有194名参与者注册，31支团队提交了有效参赛作品。挑战赛为图像超分辨率提供了统一的基准，并展示了当前进展和未来方向。

Conclusion: NTIRE 2026超分辨率挑战赛成功推动了该领域的发展，通过双赛道设计全面评估了不同技术方法，为未来研究提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 现有的ViT稀疏多视图3D目标检测器虽然精度高但推理延迟大，现有的token压缩策略（如token剪枝、合并、增大补丁尺寸）往往会丢弃信息背景线索、破坏上下文一致性、丢失细粒度语义，从而影响3D检测性能。

Method: 提出了SEPatch3D框架：1) 时空感知补丁大小选择(SPSS)：根据场景内容动态分配小补丁给包含附近物体的场景以保留细节，大补丁给背景主导场景以减少计算；2) 信息补丁选择(IPS)：选择信息丰富的补丁进行特征细化；3) 跨粒度特征增强(CGFE)：将细粒度细节注入选定的粗粒度补丁，丰富语义特征。

Result: 在nuScenes和Argoverse 2验证集上的实验表明，SEPatch3D比StreamPETR基线推理速度快57%，比最先进的ToC3D-faster效率高20%，同时保持相当的检测精度。

Conclusion: SEPatch3D通过动态调整补丁大小并保留关键语义信息，有效解决了现有token压缩方法的局限性，在显著提升推理速度的同时保持了3D目标检测的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 当前的视觉推理模型存在"过度思考"问题，即使对于简单任务也会产生不必要的长推理链。作者将此归因于"推理路径冗余"——许多视觉问题并不需要完整的推理过程。

Method: 提出AVR自适应视觉推理框架：1) 将视觉推理分解为视觉感知、逻辑推理和答案应用三个认知功能；2) 让模型动态选择三种响应格式：完整格式、仅感知格式和直接答案；3) 使用FS-GRPO（Group Relative Policy Optimization的适配版本）进行训练，鼓励模型选择最有效的推理格式同时保持正确性。

Result: 在多个视觉语言基准测试中，AVR在保持整体准确性的同时，将token使用量减少了50-90%，特别是在感知密集型任务中效果显著。

Conclusion: 自适应视觉推理能有效缓解视觉推理模型中的过度思考问题，通过减少推理路径冗余显著提高效率，同时保持推理质量。
...
TL;DR: 本文提出了一种基于扩散模型噪声特征的深度伪造检测框架ANL，通过引导学习扩散噪声来提升检测器对新型伪造技术的泛化能力，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 随着扩散模型等新型图像合成技术的出现，深度伪造检测器面临泛化能力不足的挑战。特别是扩散模型生成的深度伪造图像高度逼真，往往能够逃避基于GAN伪造训练的检测器。现有检测方法难以有效应对不断演变的伪造技术。

Method: 提出了注意力引导噪声学习（ANL）框架，将预训练的扩散模型集成到深度伪造检测流程中。方法利用扩散模型的去噪过程暴露细微伪影：检测器被训练预测输入图像在特定扩散步骤中包含的噪声，迫使其捕捉真实与合成图像之间的差异。同时引入基于预测噪声的注意力引导机制，鼓励模型关注全局分布的差异而非局部模式。通过利用冻结扩散模型学习到的自然图像分布，ANL方法起到正则化作用。

Result: 在多个基准测试上的广泛实验表明，ANL显著优于现有方法，在检测扩散生成的深度伪造方面达到最先进的准确率。特别地，该框架在不引入额外推理开销的情况下，显著提升了泛化性能（如在未见模型上大幅提高ACC/AP）。

Conclusion: 扩散噪声为可泛化的深度伪造检测提供了强大的信号。ANL框架通过利用扩散模型的噪声特征，有效解决了深度伪造检测的泛化问题，为应对不断演变的图像合成技术提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率土地覆盖制图面临密集高分辨率标注成本高昂的问题。现有弱监督方法虽然可以利用低分辨率标签，但通常需要重新训练密集预测器，计算成本高。需要一种更高效的方法来在有限标注和计算资源下实现高分辨率制图。

Method: 提出MapSR框架，将监督与模型训练解耦：1）使用低分辨率标签通过轻量级线性探针从冻结的视觉基础模型特征中提取类别提示；2）通过无训练的余弦相似度匹配进行高分辨率预测；3）基于图的传播方法进行空间细化。整个过程无需高分辨率标注，训练参数减少四个数量级。

Result: 在Chesapeake Bay数据集上，MapSR在没有任何高分辨率标签的情况下达到59.64% mIoU，与最强的弱监督基线相当，甚至超过了一个完全监督基线。训练参数减少四个数量级，训练时间从数小时缩短到数分钟。

Conclusion: MapSR提供了一种高效、可扩展的高分辨率土地覆盖制图解决方案，在有限标注和计算预算下实现了有竞争力的性能，为地图超分辨率任务提供了新的研究方向。
...
TL;DR: 提出Masked Logit Nudging方法，用于视觉自回归模型的提示引导图像编辑，通过源图像标记图和目标提示的语义轨迹进行编辑，在保持无关区域的同时实现高质量编辑。
...
Method: 提出Masked Logit Nudging方法：1) 使用源图像标记图引入指导步骤，使模型在目标提示下的预测与源标记图对齐；2) 将固定源编码通过VAR编码转换为logits，沿源-目标提示定义的语义轨迹推动模型预测的logits；3) 通过专门的掩码方案（利用源提示和编辑提示之间的交叉注意力差异）获得空间掩码，仅在掩码内应用编辑；4) 引入细化步骤纠正量化误差并提高重建质量。

Result: 在PIE基准测试的512px和1024px分辨率上实现最佳图像编辑性能；在COCO 512px和OpenImages 1024px上优于先前方法；整体性能优于VAR相关方法，与扩散模型相当甚至更好，同时速度更快。

Conclusion: Masked Logit Nudging方法在视觉自回归模型中实现了高质量的提示引导图像编辑，在保持无关区域完整性的同时，在多个基准测试上取得了最先进的性能，且计算效率高。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法假设输入元素在风格上已经和谐，但实践中元素常来自不同来源存在视觉不匹配。需要身份保持的风格化和合成作为组件到设计流程的关键缺失环节。

Method: 提出GIST——无需训练的身份保持图像合成器，位于布局预测和排版生成之间，可无缝集成到现有组件到设计或设计优化流程中。

Result: 将GIST集成到LaDeCo和Design-o-meter两种不同方法中，相比简单粘贴，在视觉和谐度和美学质量上均有显著提升，经LLaVA-OV和GPT-4V验证。

Conclusion: GIST填补了组件到设计流程中身份保持风格化合成的空白，能有效提升设计和谐度，可灵活集成到现有设计流程中。
...
TL;DR: 该论文提出了一种基于高阶RWKV架构和三令牌提示机制的多粒度感知语义原型扫描范式，用于全色锐化任务，通过语义驱动扫描、三令牌提示学习和可逆Q移位操作提升性能。


<details>
  <summary>Details</summary>
Motivation: 虽然RWKV提供了Transformer的高效线性复杂度替代方案，但其传统的双向光栅扫描仍然是语义无关的且容易产生位置偏差。为了解决这个问题，需要引入语义驱动的扫描策略来提升全局交互的连贯性。

Method: 1) 多粒度感知语义原型扫描：利用局部敏感哈希对语义相关区域进行分组，构建多粒度语义原型，实现上下文感知的令牌重排序；2) 三令牌提示学习：包含全局令牌、聚类原型令牌和可学习寄存器令牌的提示机制；3) 可逆Q移位：在值路径上应用中心差分卷积注入高频信息，并引入可逆多尺度Q移位操作进行无损特征变换。

Result: 实验结果表明该方法具有优越性。

Conclusion: 提出的多粒度感知语义原型扫描范式通过语义驱动扫描、三令牌提示学习和可逆Q移位操作，有效解决了传统RWKV扫描的语义无关和位置偏差问题，在全色锐化任务中表现出色。
...
TL;DR: Switch-KD是一种视觉切换知识蒸馏框架，通过将学生的视觉输出切换到教师的语言通路中，在共享的文本概率空间统一视觉-语言知识转移，有效解决多模态对齐问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLMs)在资源受限场景中部署困难，知识蒸馏(KD)可以提升模型能力而不增加模型规模或数据需求。但现有方法在多模态知识蒸馏中存在模态特定监督问题，缺乏明确的多模态对齐，导致知识转移不一致。

Method: 提出Switch-KD框架，包含两个关键组件：1) 视觉切换蒸馏：将学生的视觉输出切换到教师的语言通路中，构建跨模态概率参考进行隐式视觉知识转移；2) 动态双向对数差异(DBiLD)损失：自适应对齐信息概率区域，同时通过双向监督保持师生分布结构。

Result: 使用Switch-KD指导，0.5B参数的TinyLLaVA成功从3B参数的教师模型中蒸馏出丰富的多模态知识，在10个多模态基准测试中平均提升3.6分，且无需任何架构修改。

Conclusion: Switch-KD通过统一视觉-语言知识转移于共享文本概率空间，有效解决了多模态知识蒸馏中的对齐问题，显著提升了小型视觉语言模型的性能，为资源受限场景下的高效部署提供了可行方案。
...
TL;DR: 提出跨模态令牌调制方法，通过关系变换器块建立外观和运动线索间的密集连接，实现模态内和模态间信息传播，结合令牌掩码策略提升学习效率，在无监督视频对象分割任务中达到SOTA性能。
...
Method: 提出跨模态令牌调制方法，通过关系变换器块建立外观和运动令牌之间的密集连接，实现高效的模态内和模态间信息传播。同时引入令牌掩码策略，避免仅依赖增加模型复杂度，提高学习效率。

Result: 该方法在所有公开基准测试中都达到了最先进的性能，超越了现有的无监督视频对象分割方法。

Conclusion: 跨模态令牌调制方法通过有效建模外观和运动线索之间的相互依赖关系，结合令牌掩码策略，在无监督视频对象分割任务中取得了显著性能提升，证明了该方法在整合多模态信息方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB HDR成像面临多曝光运动伪影与单次拍摄信息丢失的根本权衡，现有模数解决方案受限于迭代解包开销和硬件约束，只能实现低速灰度捕获。需要克服这些系统性瓶颈，实现高速全彩HDR采集。

Method: 1) 提出曝光解耦的模数成像公式，允许多次测量在时间上交错，保持清晰的观测模型；2) 引入无迭代解包算法，将基于扩散的生成先验与模数图像的物理最小绝对余数特性结合，实现高效物理一致的HDR重建；3) 基于模数编码脉冲流实现硬件原型验证。

Result: 实现了1000 FPS全彩HDR成像，将输出数据带宽从约20 Gbps降低到6 Gbps，保持了脉冲相机固有的高时间分辨率。系统成功克服了关键系统性瓶颈，证明了模数成像在动态场景中的可行性。

Conclusion: 通过协同推进传感公式和解包算法，开发了完整的模数HDR成像系统，实现了高速全彩HDR采集，为动态场景中的模数成像部署提供了可行性证明。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法主要依赖像素级扰动，未能充分利用遥感图像的大气特性，且难以在真实世界图像退化中存活。需要一种物理上合理、视觉一致且鲁棒的对抗攻击方法

Method: 提出FogFool框架，通过迭代优化基于Perlin噪声的大气模式生成雾基扰动。利用大气现象的空间相干性和中低频特性，将对抗信息嵌入到跨不同架构共享的结构特征中

Result: 在两个基准遥感数据集上的实验表明：在白盒设置中表现优异；在黑盒可迁移性方面达到83.74% TASR；对JPEG压缩和滤波等预处理防御具有强鲁棒性；CAM可视化显示扰动能引起模型注意力的普遍偏移

Conclusion: FogFool代表了遥感分类系统面临的实际、隐蔽且高度持久的威胁，为评估复杂环境中模型可靠性提供了鲁棒的基准。大气驱动的扰动能诱导模型注意力的普遍偏移
...
TL;DR: DETR-ViP是一个用于视觉提示目标检测的框架，通过全局提示集成和视觉-文本提示关系蒸馏来增强视觉提示的判别性，在多个数据集上取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉提示目标检测虽然能灵活定义目标类别且在识别稀有类别上优于文本提示，但相关研究被忽视且性能不理想。研究发现根本问题在于视觉提示缺乏全局判别性。

Method: 提出DETR-ViP框架：1) 在基础图像-文本对比学习基础上，加入全局提示集成和视觉-文本提示关系蒸馏来学习更具判别性的提示表示；2) 采用选择性融合策略确保检测的稳定性和鲁棒性。

Result: 在COCO、LVIS、ODinW和Roboflow100数据集上的大量实验表明，DETR-ViP在视觉提示检测方面显著优于其他最先进方法。消融研究进一步验证了改进的有效性。

Conclusion: DETR-ViP通过增强视觉提示的判别性，成功提升了视觉提示目标检测的性能，揭示了视觉提示检测能力提升的根本原因。
...
TL;DR: Chain-of-Glimpse：一种搜索引导的渐进式对象基础推理框架，通过逐步锚定视觉证据区域来解决视频理解中的对象变化问题


<details>
  <summary>Details</summary>
Motivation: 现有视频理解方法多为对象无关的，难以有效处理随时间变化的对象显著变化，需要一种能够明确锚定视觉证据区域的方法来支持组合式多步决策

Method: 提出Chain-of-Glimpse框架，将视频推理建模为逐步过程，通过搜索引导控制器迭代地基础视觉证据区域，形成可靠推理轨迹，使用强化学习优化控制器以增强基础能力

Result: 在NExTQA、Video-Holmes、CG-Bench Reasoning和VRBench等多个基准测试中表现出持续的性能提升、鲁棒性和泛化能力

Conclusion: Chain-of-Glimpse通过显式锚定视觉证据区域，有效解决了视频理解中的对象变化问题，实现了准确且可解释的多步决策，在多种视频推理任务中表现出优越性能
...
TL;DR: 提出法庭式裁决框架，将图像篡改定位任务视为证据对抗后的判决过程，包含起诉流、辩护流和法官模型，通过对抗性证据生成和强化学习法官模型实现更可靠的篡改区域定位。


<details>
  <summary>Details</summary>
Motivation: 现有图像篡改定位方法虽然包含真实性相关监督，但仅将其作为辅助训练信号来增强对篡改痕迹的敏感性，而非明确建模为与篡改区域对立的定位证据。当篡改痕迹细微或被后处理和噪声破坏时，这些方法难以明确比较篡改和真实证据，导致在模糊区域产生不可靠预测。

Method: 提出法庭式裁决框架：1）基于共享多尺度编码器的双假设分割架构，起诉流主张篡改，辩护流主张真实性；2）通过边缘先验引导，采用级联多级融合、双向分歧抑制和动态辩论细化生成篡改和真实区域证据；3）开发强化学习法官模型，对不确定区域进行策略性重新推理和细化，生成篡改区域掩码；4）法官模型使用基于优势的奖励和软IoU目标训练，通过熵和跨假设一致性校准可靠性。

Result: 实验结果表明，该模型相比最先进的图像篡改定位方法实现了更优的平均性能。

Conclusion: 通过将图像篡改定位任务建模为证据对抗和判决过程，提出的法庭式裁决框架能够更可靠地定位篡改区域，特别是在篡改痕迹细微或受干扰的情况下，通过明确比较篡改和真实证据来提高定位准确性。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术在高效和逼真的新视角合成方面取得了进展，但由于高斯表示的离散特性，在物体边界处容易产生锯齿和伪影，导致准确分割物体仍然具有挑战性

Method: 1. 通过掩码方差分析自动识别物体边界处的模糊高斯；2. 应用径向基函数插值构建空间连续特征场；3. 使用多分辨率哈希编码实现高效多尺度表示；4. 通过对齐损失和空间连续性损失联合优化3DGS与轻量级NeRF模块

Result: 在NVOS、LERF-OVS和ScanNet基准测试中实现了最先进的性能，在边界mIoU方面取得了显著提升

Conclusion: NG-GS框架通过解决边界离散化问题，显著提高了3D高斯泼溅中物体分割的质量，特别是在边界区域
...
TL;DR: G-MIXER是一种无需训练的零样本组合图像检索方法，通过测地线混合构建反映隐式语义的查询特征，结合多模态大语言模型的显式语义重排序，提升检索多样性和准确性。
...
Result: G-MIXER在多个零样本组合图像检索基准测试中达到最先进性能，无需额外训练即可有效处理隐式和显式语义。

Conclusion: G-MIXER通过测地线混合隐式语义扩展和显式语义重排序，成功解决了零样本组合图像检索中多样性和准确性的平衡问题。
...
### [35] [MS-SSE-Net: A Multi-Scale Spatial Squeeze-and-Excitation Network for Structural Damage Detection in Civil and Geotechnical Engineering](https://arxiv.org/abs/2604.14711)
*Saif ur Rehman Khan,Imad Ahmed Waqar,Arooj Zaib,Saad Ahmed,Sebastian Vollmer,Andreas Dengel,Muhammad Nabeel Asim*

Main category: cs.CV

TL;DR: 本文提出MS-SSE-Net深度学习框架，用于结构损伤图像分类，在DenseNet201基础上集成多尺度特征提取和注意力机制，在StructDamage数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 结构损伤检测对基础设施安全至关重要，但准确识别不同类型结构损伤面临挑战，主要由于损伤模式多样和环境条件变化。

Method: 基于DenseNet201构建MS-SSE-Net框架，集成多尺度特征提取、通道注意力机制和空间注意力机制，通过并行深度卷积捕获局部和上下文特征，注意力机制强调信息区域并抑制噪声。
...
Conclusion: MS-SSE-Net通过集成多尺度特征提取和注意力机制，显著提升了结构损伤分类性能，为基础设施安全监测提供了有效的深度学习解决方案。
...
TL;DR: 提出了一种基于几何驱动的合成管道，用于生成三维肌管图像数据，训练紧凑的3D U-Net模型，在真实数据上实现优于现有零样本分割模型的实例分割性能。
...
Method: 1. 几何驱动合成管道：通过多项式中心线、局部变化半径、分支结构和椭圆端帽建模单个肌管；2. 合成体积渲染：加入真实噪声、光学伪影和基于CycleGAN的域适应；3. 紧凑3D U-Net：使用自监督编码器预训练，仅用合成数据训练。

Result: 在真实数据上达到平均IPQ 0.22，显著优于三种已建立的零样本分割模型，证明生物物理驱动的合成方法在标注稀缺的生物医学领域中能实现有效的实例分割。

Conclusion: 通过几何建模和域适应技术生成合成数据，可以克服生物医学领域标注数据稀缺的问题，为肌管等复杂结构的精确三维实例分割提供有效解决方案。
...
Result: 研究表明FR系统与D-MAD方法执行相似任务，当前阈值确实导致FR系统对形态攻击脆弱，新阈值能有效限制对未知类型形态攻击的脆弱性。

Conclusion: 可以利用现有FR系统进行形态攻击检测，通过引入新的评估阈值可以在保证正常图像性能的同时，有效控制对形态攻击（包括未知类型）的脆弱性。
...
TL;DR: 提出基于Sylvester形式的结式求解器新方法，用于姿态估计问题，在保持精度的同时显著降低计算时间
...
Method: 提出基于Sylvester形式的结式求解器新类别，利用结式矩阵减少问题复杂度，适用于3D-3D和3D-2D对应关系的姿态估计

Result: 提出的方法在数值精度上与最先进求解器相当，但在计算时间上显著优于现有方法

Conclusion: Sylvester形式的结式求解器为姿态估计问题提供了高效准确的解决方案，适用于多种对应关系类型
...
TL;DR: OmniGCD提出了一种模态无关的广义类别发现方法，通过合成数据训练一次即可在多种模态数据集上实现零样本类别发现，无需数据集特定微调。


<details>
  <summary>Details</summary>
Motivation: 现有GCD方法局限于单一模态且需要数据集特定微调，无法模拟人脑的抽象类别形成过程。需要开发一种模态无关、无需微调的GCD方法。

Method: 使用模态特定编码器处理输入，通过降维构建GCD潜在空间，在测试时使用合成数据训练的Transformer模型将其转换为更适合聚类的表示。

Result: 在4种模态的16个数据集上，OmniGCD在已知和新类别上的分类准确率均优于基线方法（视觉+6.2%、文本+17.9%、音频+1.5%、遥感+12.7%）。
...
TL;DR: 针对视觉语言模型在持续学习中的不对称性问题，提出AIM方法通过模态特定敏感性的目标掩码来平衡稳定性和可塑性


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法主要针对对称的单模态架构设计，而现代视觉语言模型的可训练组件本质上是非对称的。这种结构不匹配导致VLMs在连续数据流学习时极易发生灾难性遗忘。具体来说，不对称性使得标准全局正则化在优化过程中偏向庞大的语言解码器，而较小但关键的视觉投影层则极易受到干扰，导致组合推理能力严重丧失。

Method: 提出非对称信息掩码（AIM），基于模态特定敏感性应用目标掩码来平衡稳定性和可塑性。该方法针对视觉语言模型的不对称结构特点，防止优化过程中偏向语言解码器，保护关键的视觉投影层。

Result: 在VQA v2和GQA数据集上的持续VQA设置实验中，AIM在平均性能（AP）和平均遗忘（AF）方面都达到了最先进的性能，同时更好地保持了新颖技能-概念组合的泛化能力。

Conclusion: AIM方法有效解决了视觉语言模型在持续学习中的不对称性问题，通过模态特定的目标掩码策略平衡了稳定性和可塑性，显著提升了持续视觉问答的性能和泛化能力。
...
TL;DR: 提出一种从单张图像构建完整3D头部头像的分解式方法，通过显式分离头发和面部，使用不同变形范式建模，实现更真实的头发动态效果。
...
Method: 1. 对正面肖像进行头发移除获得秃头图像；2. 将原始图像和秃头图像提升为密集的3D高斯溅射表示；3. 秃头3DGS通过非刚性配准绑定到FLAME网格实现自然变形；4. 使用语义标签监督和边界感知重分配策略提取干净的头发高斯集合；5. 引入支持PBD模拟的笼结构控制头发变形。

Result: 在各种头部运动、重力效应和表情下的动态动画展示了更真实的头发行为，同时忠实地保留了面部细节，在感知真实性方面优于最先进的一次性方法。

Conclusion: 该方法通过显式解耦头发和面部，采用不同的变形范式，并结合图像到3D提升技术，成功解决了单图像3D头部头像构建中头发动态不真实和细节丢失的问题。
...
TL;DR: 提出Petro-SAM框架，基于Segment Anything Model改进，用于联合完成岩石薄片的晶粒边缘分割和岩性语义分割任务


<details>
  <summary>Details</summary>
Motivation: 现有方法将晶粒边缘分割和岩性语义分割分开处理，且分割质量不佳，即使使用昂贵、耗时、专家标注的数据集。领域差距大（消光相关颜色变化和超细晶粒边界）以及缺乏多角度岩石薄片图像堆栈的联合学习模块

Method: 提出两阶段多任务框架Petro-SAM：1）基于SAM引入Merge Block整合七个偏振视图解决消光问题；2）引入多尺度特征融合和颜色熵先验优化检测

Result: 未在摘要中明确说明具体实验结果，但声称能够实现高质量的联合晶粒边缘分割和岩性语义分割

Conclusion: Petro-SAM框架能够有效解决岩石薄片图像分析中的领域差距问题，实现高质量的联合分割任务
...
Method: 采用分层管道架构：廉价的目标检测和姿态估计模型持续运行，昂贵视觉语言模型(VLM)仅在行为预过滤器触发时调用。多信号怀疑预过滤器（要求停留时间加至少一个行为信号）将VLM调用减少240倍。架构模型无关，VLM组件可连接任何OpenAI兼容端点。

Result: 在DCSASS合成盗窃数据集上，VLM组件实现89.5%精确率和92.8%特异性，召回率59.3%。成本模型显示每月每店成本50-100美元，比商业方案便宜3-10倍。单个GPU可服务10-20家店铺。

Conclusion: Paza提供了一个实用、经济、可扩展的零样本零售盗窃检测框架，通过智能模型编排大幅降低成本，并采用隐私保护设计，为零售业提供了可行的AI防盗解决方案。
...
TL;DR: IAC-LTH通过早期稳定性准则加速自适应跳跃模块搜索，在保持性能的同时大幅降低计算成本
...
Method: 分析IAC单元中操作和边的时间行为，提出基于Jensen-Shannon散度的稳定性准则，跟踪每条边的操作重要性分布，在搜索过程中逐步剪枝低重要性操作，形成加速框架IAC-LTH。

Result: 在四个公开基准测试（ACDC、BraTS、KiTS、AMOS）上，IAC-LTH发现的IAC单元在患者级分割性能上匹配甚至略微超过原始完整搜索的结果，同时将NAS计算成本降低了3.7倍到16倍。
...
TL;DR: 该研究提出了一种基于深度度量学习的静脉识别方法，解决了传统封闭集分类在可扩展性和新用户注册方面的限制，在开放集约束下实现了高性能的静脉识别。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的静脉识别方法大多依赖封闭集分类，这限制了系统的可扩展性，并且无法在不完全重新训练模型的情况下自适应地注册新用户。研究旨在评估深度度量学习在严格开放集约束下的计算边界。

Method: 方法学习具有区分性的L2归一化嵌入表示，采用基于原型的匹配方法，并使用校准的相似度阈值来有效区分已注册用户和未见过的冒名顶替者。使用三重损失目标函数结合简单的1-NN分类器，在四个不同的静脉数据集上进行评估。

Result: 在MMCBNU 6000基准测试中，最佳模型(ResNet50-CBAM)实现了OSCR 0.9945、AUROC 0.9974和EER 1.57%，同时保持了高识别准确率(99.6% Rank-1)。跨数据集实验显示模型在大规模数据上表现稳健，但在低数据域偏移情况下性能仍敏感。

Conclusion: 三重损失目标与简单1-NN分类器的组合在准确性和效率之间提供了最佳权衡，能够在商用硬件上实现实时部署。该框架为开放集静脉识别提供了有效的解决方案，解决了传统封闭集方法的可扩展性限制。
...
TL;DR: RATS框架通过奖励感知轨迹整形，使少步生成学生模型能够超越多步教师模型，实现高效高质量生成


<details>
  <summary>Details</summary>
Motivation: 现有蒸馏方法将学生限制在模仿教师性能的上限内，无法超越教师。需要引入偏好对齐意识，使学生能够朝着奖励偏好的生成质量优化，从而可能超越教师。

Method: 提出奖励感知轨迹整形(RATS)框架：1) 通过水平匹配在关键去噪阶段对齐师生潜在轨迹；2) 引入奖励感知门，根据相对奖励性能自适应调节教师指导；3) 当教师获得更高奖励时加强轨迹整形，当学生匹配或超越教师时放松指导。

Result: RATS显著改善了少步视觉生成的效率-质量权衡，大幅缩小了少步学生模型与更强多步生成器之间的差距。

Conclusion: 通过整合轨迹蒸馏、奖励感知门控和偏好对齐，RATS能够在不增加测试时计算开销的情况下，从高步生成器有效传递偏好相关知识，实现超越教师的高质量少步生成。
...
TL;DR: 研究发现当前最先进的文本到3D生成模型存在"潜在陷阱"问题，导致模型对文本提示变得不敏感，无法有效进行基于文本的3D形状编辑。


<details>
  <summary>Details</summary>
Motivation: 文本驱动的生成模型反演是操作2D或3D内容的核心范式，但这种方法依赖于生成模型对自然语言提示保持敏感性的假设。研究发现对于最先进的文本到3D生成模型，这一假设常常不成立。

Method: 通过分析生成模型的采样轨迹，识别出"潜在陷阱"这一关键故障模式，即生成轨迹被吸引到模型对提示修改变得不敏感的区域。提出通过利用模型的无条件生成先验来解耦几何表示能力与语言敏感性。

Result: 发现复杂几何形状仍然可以通过利用模型的无条件生成先验来表示和生成，从而开发出更鲁棒的基于文本的3D形状编辑框架，绕过潜在陷阱。

Conclusion: 提出了一种解决当前3D流程限制的方法，能够对分布外的3D形状进行高保真语义操作，通过解耦模型的几何表示能力与语言敏感性来实现更有效的文本驱动编辑。
...
TL;DR: 提出混合高斯-哈希网格辐射表示方法，从多视角图像重建2D高斯场景模型，通过频率分解减少几何与外观纠缠，实现更高保真度和更少基元数量。


<details>
  <summary>Details</summary>
Motivation: 传统NeRF模型存在几何与外观纠缠问题，高频纹理容易补偿几何误差。需要一种方法能明确分离低频和高频场景组件，提高几何重建质量和渲染效率。

Method: 1. 结合高斯潜在特征和哈希网格特征，引导优化器分离低频和高频组件；2. 采用硬不透明度衰减加强几何与外观分离；3. 概率剪枝结合稀疏诱导BCE不透明度损失，关闭冗余高斯基元。

Result: 在合成和真实世界数据集上，相比最先进的高斯基元新视角合成方法，实现了更高的重建保真度，同时基元数量减少一个数量级。

Conclusion: 混合高斯-哈希网格表示通过频率分解有效解决了几何与外观纠缠问题，硬不透明度衰减和稀疏优化策略显著提高了几何重建质量和渲染效率。
...
TL;DR: 提出了一种基于条件生成的数据增强方法，用于解决骨架动作识别中数据稀缺问题，通过生成高质量骨架序列提升识别性能。


<details>
  <summary>Details</summary>
Motivation: 骨架动作识别需要大规模、多样化的3D骨架数据集，但数据收集成本高且标注困难，特别是在数据稀缺场景下识别性能受限。
...
Conclusion: 提出的条件生成数据增强方法能有效解决骨架动作识别中的数据稀缺问题，生成高质量骨架序列，在下游任务中展现出强大的泛化能力。
...
TL;DR: 本文提出使用图像到视频生成模型创建逼真的指示手势合成数据集，以解决手势识别领域数据稀缺问题，并验证合成数据对下游任务的增强效果。


<details>
  <summary>Details</summary>
Motivation: 手势识别研究面临严重的数据稀缺问题，传统方法依赖昂贵的人工录制或无法生成真实手势变化的图像处理方法。图像到视频基础模型的进展为创建无需人工干预的合成数据提供了新可能。

Method: 提出基于提示的视频生成方法构建现实指示手势数据集，开发从少量人类参考样本生成指示手势的数据生成流程，使用混合数据集评估各种深度学习模型性能。

Result: 合成手势在视觉保真度上与真实手势高度一致，同时引入了有意义的变化和新颖性，丰富了原始数据。使用混合数据集的深度学习模型表现出优越性能。

Conclusion: 图像到视频技术即使处于早期阶段，也能为手势合成提供强大的零样本方法，对下游任务有明显益处，为手势识别研究提供了数据增强的新途径。
...
TL;DR: UniDoc-RL是一个统一的强化学习框架，让大型视觉语言模型能够联合执行检索、重排序、主动视觉感知和推理，通过分层动作空间逐步细化视觉证据，在多个基准测试中显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉RAG系统通常依赖通用的检索信号，忽略了复杂推理所需的细粒度视觉语义。为了解决这一限制，需要开发能够更精细获取视觉信息的框架。

Method: 提出UniDoc-RL统一强化学习框架，将视觉信息获取建模为具有分层动作空间的顺序决策问题。模型从粗粒度文档检索逐步细化到细粒度图像选择和主动区域裁剪，使用密集多奖励方案进行端到端训练，基于GRPO对齐多个目标。

Result: 在三个基准测试中，UniDoc-RL始终超越最先进的基线方法，相比之前的基于RL的方法获得了高达17.7%的性能提升。

Conclusion: UniDoc-RL通过统一的强化学习框架有效解决了视觉RAG中细粒度语义获取的问题，通过分层动作空间和密集多奖励方案实现了更精确的视觉推理。
...
TL;DR: 本文提出了首个专注于图像到视频生成中时序取证的主动框架Flow of Truth，通过将视频生成重新定义为像素随时间运动而非帧合成，解决了I2V内容中嵌入痕迹随时间漂移变形的取证难题。


<details>
  <summary>Details</summary>
Motivation: 图像到视频生成技术的快速发展带来了新的取证需求。与静态图像不同，I2V内容随时间演变，传统2D像素级篡改定位方法失效，因为嵌入的痕迹会随帧进展而漂移变形。目前缺乏针对I2V生成中时序维度的取证方法。

Method: 提出Flow of Truth框架，将视频生成重新定义为像素随时间运动而非帧合成。设计可学习的取证模板跟随像素运动，以及模板引导的流模块将运动与图像内容解耦，实现鲁棒的时序追踪。

Result: 实验表明Flow of Truth能够泛化到商业和开源I2V模型，显著提升了时序取证性能。

Conclusion: 该研究首次解决了I2V生成中的时序取证问题，通过创新的像素运动视角和可学习模板方法，为动态内容取证提供了有效解决方案。
...
TL;DR: 提出了一种混合CNN-注意力框架，用于MRI运动伪影的稳健、站点不变的质量评估，在可见站点上达到99%准确率，在未见站点上保持75.5%准确率，无需重新训练。


<details>
  <summary>Details</summary>
Motivation: 结构MRI中的运动伪影严重影响临床诊断和自动化分析，而人工质量控制难以扩展到大规模纵向研究，需要开发自动化、站点不变的质量评估方法。

Method: 提出混合CNN-注意力框架，结合分层2D CNN编码器提取局部空间特征，使用多头交叉注意力机制建模全局依赖关系，能够优先处理运动相关伪影特征，同时动态过滤站点特异性强度变化和背景噪声。

Result: 在可见站点评估中，模型达到扫描级准确率0.9920和F1分数0.9919；在未见站点评估中，使用ABIDE档案库中17个异构站点的200名受试者，模型保持0.755的准确率，无需重新训练或微调。

Conclusion: 注意力机制的特征重新加权成功捕获了通用伪影描述符，弥合了不同成像环境和扫描仪制造商之间的性能差距，展示了强大的领域转移适应能力。
...
TL;DR: HELP框架通过热图引导的位置嵌入机制，在保持检测精度的同时大幅减少计算量和参数，解决了Transformer检测器中背景噪声和小目标特征稀疏的问题。


<details>
  <summary>Details</summary>
Motivation: Transformer检测器在处理小目标时效率低下且易受背景噪声干扰，导致需要深层解码器来细化低质量查询。背景噪声会引入查询噪声，影响检测性能。

Method: 提出HELP框架，核心是热图引导的位置嵌入(HPE)：1) 选择性保留前景显著区域的位置编码，抑制背景干扰；2) 在编码器和解码器中集成HPE；3) 使用梯度掩码过滤器在解码前过滤背景主导的嵌入；4) 引入线性蛇形卷积丰富小目标特征表示；5) 训练时使用梯度热图监督，推理时无额外计算。

Result: 将解码器层从8层减少到3层，参数减少59.4%(66.3M vs. 163M)，在减少计算预算的同时保持精度提升。热图可视化提供了可解释的诊断和微调工具。

Conclusion: HELP框架通过噪声感知的位置-语义融合，有效解决了小目标检测中的背景噪声和特征稀疏问题，在显著减少计算资源的同时保持了检测精度，为高效小目标检测提供了新范式。
...
TL;DR: 本文提出了首个针对雾霾和低光条件下建筑物提取的光学遥感基准数据集HaLoBuilding，以及相应的端到端框架HaLoBuild-Net，通过空间频率聚焦模块和全局多尺度引导模块有效应对恶劣天气条件下的建筑物提取挑战。


<details>
  <summary>Details</summary>
Motivation: 现有光学遥感建筑物提取方法主要关注理想晴朗天气条件，在真实世界的雾霾和低光条件下性能会显著下降。虽然SAR具有全天候感知能力，但其侧视几何结构会导致几何失真。因此需要专门针对恶劣天气条件下的建筑物提取解决方案。

Method: 提出了HaLoBuild-Net框架，包含三个核心模块：1）空间频率聚焦模块（SFFM），通过大感受野注意力与频率感知通道重加权结合来减轻气象干扰；2）全局多尺度引导模块（GMGM），提供全局语义约束来锚定建筑物拓扑；3）互引导融合模块（MGFM），实现双向语义-空间校准以抑制浅层噪声和锐化天气引起的模糊边界。

Result: 在HaLoBuilding数据集上，HaLoBuild-Net显著优于现有最先进方法和传统的级联恢复-分割范式，同时在WHU、INRIA和LoveDA数据集上保持了强大的泛化能力。

Conclusion: 本文通过构建首个针对恶劣天气条件的建筑物提取基准数据集和提出创新的端到端框架，有效解决了雾霾和低光条件下光学遥感建筑物提取的性能退化问题，为全天候遥感建筑物提取提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有行人重识别方法过度依赖纯视觉特征，这些特征容易因环境因素和时间变化而改变，在光照引起的模态偏移（RGB与IR）或服装变化场景下性能显著下降。需要一种能同时应对服装变化和跨模态偏移的鲁棒方法。

Method: 提出STFER框架：1）使用指令引导大型视觉语言模型生成身份内在语义文本，捕捉生物特征常量；2）语义驱动的视觉令牌过滤（SVTF）利用文本令牌增强信息丰富的视觉区域并抑制背景噪声；3）语义驱动的专家路由（SER）将语义文本集成到专家路由中，实现更鲁棒的多场景门控。

Result: 在AT-USTC数据集上实现了最先进的结果。在AT-USTC上训练的模型在5个广泛使用的ReID基准测试中评估，显示出优越的泛化能力和高度竞争力的结果。

Conclusion: STFER通过利用大型视觉语言模型生成的身份一致性文本，有效解决了任意时间行人重识别中的模态偏移和服装变化挑战，实现了鲁棒的身份识别性能。
...
TL;DR: LAMAE是一种针对医学影像多视角特性的基础模型架构，通过潜在注意力模块在潜在空间中实现跨帧和跨视图的信息交换，能够从部分观察重建心脏功能的整体表示。


<details>
  <summary>Details</summary>
Motivation: 超声心动图因其无创性和成本效益而被广泛用于心脏评估，但心脏的稀疏和异质时空视图带来了独特挑战。现有的掩码自编码器方法通常独立处理图像或短片段，无法捕捉心脏表示所需的内在多视图结构。

Method: 提出潜在注意力掩码自编码器（LAMAE），在标准MAE基础上增加潜在注意力模块，使模型能够在潜在空间中直接进行跨帧和跨视图的信息交换，从而聚合可变长度序列和不同视图。

Result: 在MIMIC-IV-ECHO大规模无标注数据集上预训练，首次展示了从MIMIC-IV-ECHO视频预测ICD-10代码的结果，并证明从成人数据学习的表示能有效迁移到儿科队列，尽管存在显著解剖差异。

Conclusion: 结果表明，纳入多视图注意力等结构先验能够产生显著更稳健和可迁移的表示，为医学影像分析提供了有效的多视图建模方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有程序性视频数据集缺乏自然发生的人类错误和恢复行为的系统标注，而第一人称视角中错误常被手部遮挡，需要通过细微物体状态变化来识别。

Method: PIE-V包含四个核心组件：基于心理学和程序阶段语义负载的错误规划器、恢复行为建模的修正规划器、级联一致性重写的LLM编写器、验证程序连贯性的LLM裁判。视频编辑部分使用文本引导的视频生成合成替换片段并缝合到原始视频中。

Result: 在17个任务和50个Ego-Exo4D场景中注入了102个错误，生成了27个恢复修正。建立了统一的分类体系和包含9个指标的人工评估标准，涵盖步骤级和程序级质量评估。

Conclusion: PIE-V框架和评估标准为第一人称视角程序性错误检测与修正提供了系统化的基准测试支持，能够生成人类可信的错误和恢复行为，优于自由形式的LLM生成基线。
...
TL;DR: 该论文研究了光照相关图像恢复问题，提出了两种对比策略：基于DINOLight的专用方法和基于WD-MoE的通用方法OmniLight，在NTIRE 2026挑战赛中取得了顶级排名。


<details>
  <summary>Details</summary>
Motivation: 不良光照条件（如投影阴影和不规则照明）会降低计算机视觉系统的可见度和色彩保真度。虽然现有方法能在特定基准测试中取得先进结果，但实际应用需要能够处理多样化领域的鲁棒模型。

Method: 提出了两种对比策略：1）使用DINOLight作为专用基线，针对每个数据集特性进行优化；2）扩展为OmniLight通用方法，引入提出的小波域专家混合（WD-MoE）架构，在所有数据集上进行训练。

Result: 两种方法在NTIRE 2026挑战赛的三个光照相关赛道中都获得了顶级排名，证明了其出色的感知质量和泛化能力。

Conclusion: 通过对比专用和统一架构在光照相关图像恢复中的表现，探讨了数据分布对性能的影响，为实际应用中的鲁棒模型设计提供了见解。
...
TL;DR: B-ACT是一个用于时序动作分割的主动学习框架，通过专门标注边界区域来提高标注效率，在有限标注预算下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 时序动作分割需要密集的时间标注，但大部分标注成本都花在识别和细化动作边界上，这些边界区域是分割错误集中的地方，微小的时间偏移会严重影响分割指标。

Method: B-ACT采用分层两阶段循环：1) 使用预测不确定性对未标注视频进行排序和查询；2) 在选定的视频中，从当前模型预测中检测候选边界，通过融合邻域不确定性、类别模糊性和时序预测动态的新边界评分选择前K个边界。标注协议只标注边界帧，但训练时使用边界为中心的片段以利用时间上下文。

Result: 在GTEA、50Salads和Breakfast数据集上的大量实验表明，边界中心的监督提供了强大的标签效率，在稀疏预算下始终优于代表性的时序动作分割主动学习基线和先前的最先进方法，在边界放置主导编辑和重叠F1分数的数据集上获得最大增益。

Conclusion: B-ACT框架通过专门针对边界区域分配监督，显著提高了时序动作分割的标注效率，为解决动作分割中边界标注成本高的问题提供了有效解决方案。
...
TL;DR: 提出了一种名为的视觉令牌剪枝框架，将剪枝问题转化为帕累托配置优化问题，通过梯度搜索自动寻找最优配置，在8个视觉基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有视觉令牌剪枝方法依赖预定义的剪枝配置，无法确定是否达到计算性能最优。需要一种能够自动寻找最优剪枝配置的方法。

Method: 将视觉令牌剪枝建模为帕累托配置优化问题，采用连续松弛和直通估计器实现梯度搜索，通过增广拉格朗日方法求解。使用可学习核函数研究分层剪枝模式。

Result: 在8个视觉基准测试中，有效近似了网格搜索得到的经验帕累托前沿，在不同剪枝方法和VLM架构上具有良好的泛化能力。发现多步渐进剪枝能捕捉VLM的层次压缩结构，相比单层方法获得更优的精度-效率权衡。

Conclusion: 提出的框架能够自动优化视觉令牌剪枝配置，揭示了分层剪枝模式的重要性，为视觉语言模型的高效处理提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决无监督骨架时序动作分割中如何有效利用骨架数据的空间和时间信息，同时减少片段长度偏差的问题。

Method: 1. 分层向量量化：包含两个连续级别的向量量化，底层将骨架关联到细粒度子动作，高层进一步聚合子动作为动作级表示
2. 分层时空向量量化：扩展为同时利用空间和时间信息，进行多级聚类，同时恢复输入骨架及其对应时间戳
3. 通过重建输入骨架来主要利用空间线索

Result: 1. 分层方法优于非分层基线
2. 在HuGaDB、LARa和BABEL等多个基准测试中建立了新的最先进性能
3. 减少了无监督骨架时序动作分割中的片段长度偏差

Conclusion: 提出的分层时空向量量化框架能够有效利用骨架数据的空间和时间信息，在无监督时序动作分割任务中取得了显著性能提升，并减少了分割偏差。
...
<details>
  <summary>Details</summary>
Motivation: 现有O(1)内存框架主要依赖"纯驱逐"范式，存在信息破坏严重的问题，因为二进制令牌删除和局部单层评分导致评估噪声。需要解决这些瓶颈以实现更好的视频流3D重建。

Method: 提出StreamCacheVGGT框架，包含两个协同模块：1) 跨层一致性增强评分(CLCES)：通过跟踪Transformer层次结构中令牌重要性轨迹，使用顺序统计分析识别持续几何显著性；2) 混合缓存压缩(HCC)：采用三级分类策略，将中等重要性令牌通过最近邻分配合并到保留锚点中，在关键向量流形上保留几何上下文。

Result: 在五个基准测试集（7-Scenes、NRGBD、ETH3D、Bonn和KITTI）上的广泛评估表明，StreamCacheVGGT达到了新的最先进水平，在严格遵循恒定成本约束的同时，提供了优越的重建精度和长期稳定性。

Conclusion: StreamCacheVGGT通过重新构想缓存管理，解决了现有纯驱逐范式的局限性，实现了在恒定内存预算下更稳定、更准确的密集3D几何重建，为视频流处理提供了有效的解决方案。
...
TL;DR: TokenGS：一种基于Transformer的3D高斯泼溅预测新方法，通过直接回归3D坐标而非深度，采用编码器-解码器架构和可学习高斯token，在静态和动态场景重建中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的3D高斯泼溅预测方法通常沿相机光线回归高斯均值作为深度，这种方法存在局限性。作者认为这种设计不够理想，需要更直接的三维坐标回归方法，同时希望解除预测基元数量与输入图像分辨率和视图数量的绑定关系。

Method: 提出TokenGS方法：1）直接回归3D均值坐标而非深度，仅使用自监督渲染损失；2）从标准编码器设计转向编码器-解码器架构；3）引入可学习的高斯token，使预测基元数量独立于输入分辨率和视图数量；4）支持在token空间进行高效测试时优化而不破坏学习到的先验。

Result: TokenGS在静态和动态场景重建中达到最先进性能，表现出对姿态噪声和多视图不一致性的更强鲁棒性，产生更规则化的几何结构和更平衡的3DGS分布，同时能够无缝恢复静态-动态分解和场景流等涌现场景属性。

Conclusion: 通过重新设计Transformer架构的关键设计选择，TokenGS提供了一种更优的3D高斯泼溅预测方法，在保持高效性的同时显著提升了重建质量和鲁棒性，为3D场景重建开辟了新方向。
...
TL;DR: SegWithU是一个用于医学图像分割的后处理不确定性估计框架，通过轻量级不确定性头增强预训练分割模型，在单次前向传播中提供可靠的不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割的可靠性至关重要，因为自动轮廓会影响下游量化和临床决策支持。现有不确定性估计方法要么需要重复推理效率低，要么单次前向传播方法效果较差或依赖限制性特征空间假设。

Method: SegWithU是一个后处理框架，在冻结的预训练分割主干上添加轻量级不确定性头。它利用中间主干特征，在紧凑的探测空间中使用秩-1后验探针将不确定性建模为扰动能量。生成两种体素级不确定性图：用于概率校准的校准导向图和用于错误检测与选择性预测的排序导向图。

Result: 在ACDC、BraTS2024和LiTS数据集上，SegWithU是最强且最一致的单一前向传播基线，分别达到AUROC/AURC为0.9838/2.4885、0.9946/0.2660和0.9925/0.8193，同时保持分割质量。

Conclusion: 基于扰动的不确定性建模是构建可靠性感知医学分割的有效实用途径，SegWithU框架在保持分割质量的同时提供了高效可靠的不确定性估计。
...
TL;DR: 提出R3D-Policy：通过解决3D策略学习中的训练不稳定和过拟合问题，结合可扩展的3D编码器和扩散解码器，在操作任务上超越现有方法
...
Method: 系统诊断失败原因，识别出缺乏3D数据增强和批归一化的负面影响；提出新架构：结合可扩展的基于Transformer的3D编码器和扩散解码器，专门为大规模稳定性设计，利用大规模预训练

Result: 在具有挑战性的操作基准测试中显著优于最先进的3D基线方法，为可扩展的3D模仿学习建立了新的稳健基础

Conclusion: 通过解决训练不稳定和过拟合问题，提出的架构为3D策略学习提供了可扩展且稳健的解决方案，推动了3D模仿学习的发展
...
TL;DR: GlobalSplat提出了一种新的3D高斯溅射框架，采用"先对齐后解码"原则，通过全局潜在场景表示实现紧凑、一致的3D重建，显著减少高斯基元数量并加速推理。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射方法在空间基元分配上存在局限性：基于迭代优化的方法在紧凑性、重建速度和渲染质量之间存在显著权衡；前馈方法多为像素对齐或体素对齐，通过反投影像素生成密集视图对齐基元，导致3D资产冗余。随着输入视图增加，表示尺寸增大，全局一致性变差。

Method: 提出GlobalSplat框架，采用"先对齐后解码"原则：首先学习紧凑的全局潜在场景表示，编码多视图输入并解析跨视图对应关系，然后解码显式3D几何。采用从粗到细的训练课程，逐步增加解码容量，天然防止表示膨胀。

Result: 在RealEstate10K和ACID数据集上，仅使用16K高斯基元（显著少于密集流水线）就实现了竞争性的新视图合成性能，获得轻量4MB存储占用。推理速度显著快于基线方法，单次前向传播仅需78毫秒。

Conclusion: GlobalSplat通过全局潜在表示和"先对齐后解码"框架，解决了现有3D高斯溅射方法在紧凑性、一致性和效率方面的局限性，实现了高效、紧凑且全局一致的3D重建。
...
TL;DR: 该论文提出了AnimationBench，这是首个专门用于评估动画风格图像到视频生成的系统性基准测试，解决了现有基准测试主要针对写实视频、难以评估动画风格生成的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成基准测试主要针对写实视频，难以评估动画风格生成的特有特性，如风格化外观、夸张动作和角色一致性。同时，现有基准依赖固定提示词和刚性流程，缺乏对开放域内容和自定义评估需求的灵活性。

Method: 将动画十二原则和IP保护转化为可测量的评估维度，包括语义一致性、动作合理性、摄像机运动一致性等更广泛的质量维度。支持标准化闭集评估和灵活开集评估，利用视觉语言模型进行可扩展评估。
...
TL;DR: 该论文提出了一种基于推理的手语翻译框架，使用潜在思维序列作为视频和生成文本之间的中间层，采用"先规划后接地"的解码方法，并发布了新的无注释手语翻译数据集。
...
Method: 提出推理驱动的手语翻译框架：1）使用有序的潜在思维序列作为视频和生成文本之间的显式中介层，逐步提取和组织意义；2）采用"先规划后接地"的解码方法，模型先决定要说什么，然后回看视频寻找证据；3）构建并发布了新的大规模无注释手语翻译数据集，具有更强的上下文依赖性和更现实的意义表达。

Result: 在多个基准测试中，该方法相比现有的无注释方法取得了持续的性能提升，证明了推理驱动框架的有效性。

Conclusion: 手语翻译应被视为跨模态推理任务而非简单转换，通过引入潜在思维中间层和规划-接地解码策略，可以显著提高翻译的连贯性和忠实度。新数据集为未来研究提供了更真实的测试环境。
...
TL;DR: RAD-2：一个统一的生成器-判别器框架，用于闭环规划，通过扩散模型生成多样轨迹，RL优化的判别器重新排序，减少碰撞率56%


<details>
  <summary>Details</summary>
Motivation: 高级自动驾驶需要能够建模多模态未来不确定性并在闭环交互中保持鲁棒性的运动规划器。虽然基于扩散的规划器能有效建模复杂轨迹分布，但通常存在随机不稳定性和缺乏纠正性负反馈的问题。

Method: 提出RAD-2统一框架：1）扩散生成器产生多样轨迹候选；2）RL优化判别器根据长期驾驶质量重新排序候选轨迹；3）引入时间一致性组相对策略优化，利用时间相干性缓解信用分配问题；4）提出在线生成器优化，将闭环反馈转换为结构化纵向优化信号；5）开发BEV-Warp高吞吐量仿真环境，在鸟瞰图特征空间直接进行闭环评估。

Result: RAD-2相比强大的基于扩散的规划器减少了56%的碰撞率。实际部署进一步证明了在复杂城市交通中感知安全性和驾驶平滑性的改善。

Conclusion: RAD-2通过统一的生成器-判别器框架解决了扩散规划器的随机不稳定性和缺乏负反馈问题，显著提升了自动驾驶规划的性能和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: AIGC工具可以按需创建图像、视频和可视化内容用于网页设计，但直接集成到自动化网页生成中会导致风格不一致和全局连贯性差的问题，因为元素是孤立生成的。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像重光照方法难以实现对多个光照属性（如强度、颜色、环境光、漫反射水平、3D光源位置）的精确和连续控制，需要一种能够灵活编辑各种光照因素的方法

Method: 将重光照建模为条件图像生成任务，引入属性令牌编码不同的光照因素；在大规模合成数据集上训练，辅以少量真实捕获数据增强真实感和泛化能力；无需显式的逆渲染监督

Result: 在合成和真实图像上均取得最先进的定量和定性性能；模型展现出对光线与场景几何、遮挡、材质交互的内在理解，即使在传统挑战性场景（如物体内部放置光源、透明材质重光照）中也能产生逼真的光照效果

Conclusion: TokenLight方法通过属性令牌实现了对多个光照属性的精确控制，在无需显式逆渲染监督的情况下，模型能够理解光线与场景的物理交互，为图像重光照提供了灵活且高质量的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有直接梯度方法在流匹配模型微调中存在内存成本过高和梯度爆炸问题，难以更新早期生成步骤，而这些步骤对最终图像的全局结构至关重要。

Method: 提出LeapAlign方法：1）设计两段跳跃，每段跳过多个ODE采样步骤，将长轨迹缩短为仅两步；2）随机化跳跃的起始和结束时间步，实现高效稳定的模型更新；3）为更符合长生成路径的缩短轨迹分配更高训练权重；4）减少大梯度项权重而非完全移除，增强梯度稳定性。
...
TL;DR: Bi-CMPStereo：一种双向跨模态提示框架，通过利用事件相机和传统帧相机的互补特征，解决跨模态立体匹配中的模态差距问题，显著提升3D感知性能。
...
Method: 提出Bi-CMPStereo双向跨模态提示框架：1）在目标规范空间内学习精细对齐的立体表示；2）通过将每个模态投影到事件和帧两个域中，整合互补表示；3）充分利用两个域中的语义和结构特征进行鲁棒匹配。

Result: 大量实验表明，该方法在准确性和泛化能力方面显著优于现有最先进方法。

Conclusion: Bi-CMPStereo通过双向跨模态提示有效弥合了事件相机和帧相机之间的模态差距，实现了更可靠的3D感知，特别是在快速运动和挑战性光照条件下。
...
TL;DR: HUOZIIME是一个基于LLM的个性化移动输入法，通过后训练基础LLM获得初始预测能力，设计分层记忆机制捕获用户输入历史，并进行系统优化实现移动设备上的高效运行。


<details>
  <summary>Details</summary>
Motivation: 移动输入法编辑器主要依赖手动输入，难以生成个性化文本。虽然轻量级LLM使设备端辅助生成成为可能，但实现深度个性化、隐私保护和实时生成的输入法仍面临根本性挑战。

Method: 1) 在合成的个性化数据上对基础LLM进行后训练，赋予初始人类预测能力；2) 设计分层记忆机制持续捕获和利用用户特定的输入历史；3) 针对设备端LLM部署进行系统优化，确保移动约束下的高效响应操作。

Result: 实验证明HUOZIIME能够在设备端高效执行，并实现高保真度的记忆驱动个性化。代码和软件包已在GitHub上开源。

Conclusion: HUOZIIME成功解决了移动输入法的个性化挑战，通过LLM技术和分层记忆机制实现了隐私保护、实时响应的个性化文本生成，为移动输入法的发展提供了新方向。
...
TL;DR: 研究发现同行评审中数值评分比文本评论更可靠预测论文接受率，91% vs 81%准确率，文本评论因礼貌原则包含过多积极情感词而掩盖拒稿信号


<details>
  <summary>Details</summary>
Motivation: 作者常常难以准确解读同行评审反馈，容易从礼貌评论中产生错误希望，或对具体低分感到困惑。需要研究数值评分与文本评论在预测论文接受率方面的可靠性差异。

Method: 构建包含30,000+篇ICLR 2021-2025投稿的数据集，比较基于数值评分和基于文本评论的接受预测模型性能。分析评分模型失败的9%样本的分布特征，并从评论情感角度探究文本模型性能较差的原因。

Result: 评分模型达到91%准确率，文本模型仅81%（即使使用大语言模型）。评分模型失败的样本显示高分偏态分布和负偏度，表明个别低分对拒稿起决定性作用。文本分析发现礼貌原则普遍存在：拒稿论文的评论中积极情感词仍多于消极词，掩盖了拒稿信号。

Conclusion: 同行评审中数值评分比文本评论更可靠预测论文接受结果。文本评论因礼貌原则包含过多积极情感词，使作者难以仅从文本判断结果，建议作者更关注具体低分而非平均分。
...
TL;DR: SeaAlert是一个基于LLM的海上遇险通信分析框架，通过合成数据生成解决真实标注数据稀缺问题，用于处理VHF无线电中嘈杂、格式不规范的遇险消息。


<details>
  <summary>Details</summary>
Motivation: 海上VHF遇险通信是安全关键信息，但在实际中由于消息简短、嘈杂、格式不规范以及ASR错误等原因，自动分析非常困难，且缺乏标注数据。

Method: 开发了合成数据生成流程：使用LLM生成多样化的海事消息（包括省略或替换标准遇险关键词的变体），合成语音，添加模拟VHF噪声，通过ASR系统转录获得真实噪声文本。

Result: 论文提出了SeaAlert框架，能够生成包含挑战性变体的多样化海事消息，模拟真实VHF通信环境，为遇险通信分析提供高质量合成数据。

Conclusion: SeaAlert通过LLM生成合成数据的方法，解决了海上遇险通信分析中数据稀缺和噪声问题，为海事安全通信的自动分析提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前使用更强模型生成合成数据进行监督微调的方法对新兴推理模型如Qwen3-8B效果不佳，甚至导致性能显著下降。研究发现教师生成数据与学生分布之间的风格差异是影响SFT效果的主要因素。

Method: 提出教师-学生合作数据合成框架TESSY，让教师模型和学生模型交替生成风格token和非风格token，从而生成既继承教师高级推理能力又保持与学生分布风格一致的合成序列。

Result: 在代码生成实验中，使用GPT-OSS-120B作为教师模型，传统教师生成数据微调Qwen3-8B导致LiveCodeBench-Pro下降3.25%、OJBench下降10.02%，而TESSY分别提升11.25%和6.68%。

Conclusion: TESSY框架通过解决教师-学生风格差异问题，有效提升了使用合成数据进行监督微调的效果，为新兴推理模型的增强提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 临床证据合成需要从大量试验论文中手动提取数据，这个过程耗时且容易出错。现有方法在保持布局、图形和提供可审计追溯方面存在不足，需要自动化解决方案来加速系统评价工作流程并减少人工负担。

Method: 系统采用多智能体架构：PDF查询智能体（保留渲染布局和图形）、检索引导搜索智能体，以及当智能体意见不一致时强制进行页面级验证的协调模块。该管道设计用于跨多模态证据源（文本、表格、图形）进行高精度提取。

Result: 在临床医生策划的肿瘤学试验论文基准测试中，EviSearch相对于强大的解析文本基线显著提高了提取准确性，同时提供了全面的属性覆盖。系统通过记录协调决策和审阅者编辑，产生结构化偏好和监督信号，支持迭代模型改进。

Conclusion: EviSearch旨在加速实时系统评价工作流程，减少人工整理负担，并为将基于LLM的提取集成到证据合成管道中提供安全、可审计的路径。系统生成的可操作追溯使临床医生能够检查和纠正提取结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于RAG的方法采用扁平检索范式，将ATT&CK框架中的所有技术同等对待，忽略了该框架固有的战术-技术分层分类结构。这种结构信息未被充分利用，限制了检索效率和准确性。

Method: 提出H-TechniqueRAG分层RAG框架：1）两阶段分层检索机制：先识别宏观战术（对手技术目标），再在选定战术内搜索具体技术，候选搜索空间减少77.5%；2）战术感知重排序模块；3）分层约束的上下文组织策略，减轻LLM上下文过载并提高推理精度。

Result: 在三个不同的CTI数据集上，H-TechniqueRAG在F1分数上比最先进的TechniqueRAG提升3.8%，推理延迟降低62.4%，LLM API调用减少60%。分析显示该框架具有优异的跨领域泛化能力，并为安全分析师提供高度可解释的逐步决策路径。

Conclusion: 将ATT&CK框架的战术-技术分类结构作为强归纳偏置注入RAG框架，能显著提升CTI文本到技术ID映射的性能和效率。分层结构先验不仅提高准确性，还增强模型泛化能力和结果可解释性。
...
TL;DR: SAGE Celer 2.6是SAGEA最新推出的通用模型系列，提供5B、10B、27B三种参数规模，具备架构改进、逆推理训练、原生多模态功能，特别优化了南亚语言支持，在数学、编程和通用智能基准上表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理复杂推理任务、减少幻觉错误、支持多模态功能，并特别针对南亚语言进行优化的通用人工智能模型，以满足该地区语言处理需求。

Method: 采用逆推理(IR)管道进行原生训练，使模型能够验证自身的逻辑路径；使用端到端视觉编码器实现原生多模态功能；针对南亚语言开发自定义分词器，特别是天城体文字；在未公开模型基础上进行进一步预训练。

Result: 在数学、编程和通用智能基准(ACUMEN)上取得极具竞争力的结果；在尼泊尔语和印地语上表现强劲，同时不牺牲英语推理能力；具有低延迟特性。

Conclusion: SAGE Celer 2.6是一个功能全面的通用模型，通过创新的逆推理训练和原生多模态集成，在保持高性能的同时特别优化了南亚语言支持，为多语言AI应用提供了有力工具。
...
<details>
  <summary>Details</summary>
Motivation: 大型建筑项目中，决策不断演变并记录在会议纪要中，但决策可能覆盖之前的决定。专业人员需要重构特定选择的历史，而手动从原始档案中检索这些信息既费时又容易出错。

Method: 采用检索增强生成（RAG）框架，结合语义搜索和大语言模型，确保准确且上下文感知的响应。使用比利时大型公司已完成建筑项目的匿名化行业数据集，该数据集经过标注并包含专家定义的查询以支持系统评估。

Result: 开发了一个对话系统，使专业人员能够提出自然语言问题，并获得语义相关且明确时间标注的答案，从而跟踪决策的时间线。数据集和开源实现已向社区开放。

Conclusion: 该研究通过RAG框架实现了对时间标注项目文档的对话式访问，解决了建筑项目中决策历史检索的挑战，并为该领域的进一步研究提供了数据集和工具。
...
TL;DR: 提出Stateful Evidence-Driven RAG with Iterative Reasoning框架，通过建模问答为渐进式证据积累过程，解决传统RAG中扁平上下文表示和无状态检索导致的性能不稳定问题。
...
Method: 1）将检索文档转换为结构化推理单元，包含显式相关性和置信度信号；2）维护持久性证据池，捕获支持性和非支持性信息；3）进行证据驱动的缺陷分析，识别差距和冲突；4）迭代优化查询以指导后续检索；5）通过迭代推理实现稳定证据聚合。

Result: 在多个问答基准测试中，相比标准RAG和多步基线方法，该框架实现了持续的性能改进。能够有效积累高质量证据，并在大量检索噪声下保持稳定性能。

Conclusion: 提出的Stateful Evidence-Driven RAG with Iterative Reasoning框架通过建模问答为渐进证据积累过程，解决了传统RAG的局限性，提高了对噪声检索的鲁棒性，实现了更稳定的性能表现。
...
TL;DR: 本研究系统评估了三种开源大语言模型（Llama-3.1-8B、Mistral-7B-v0.1、Qwen3-8B）对罗马化尼泊尔语的适应能力，通过零样本和微调设置，使用1万条双语指令数据集，发现微调后所有模型性能显著提升，Qwen3-8B整体表现最佳，Llama-3.1-8B在微调中进步最大。


<details>
  <summary>Details</summary>
Motivation: 罗马化尼泊尔语（用拉丁字母书写的尼泊尔语）是尼泊尔非正式数字通信的主要媒介，但在大语言模型中严重缺乏资源。本研究旨在填补这一空白，为罗马化尼泊尔语在开源大语言模型中的适应建立首个严谨基准。

Method: 使用三种规模相当的开源模型（Llama-3.1-8B、Mistral-7B-v0.1、Qwen3-8B），在零样本和微调设置下评估。使用1万条精心策划的双语（罗马化尼泊尔语-英语）指令跟随数据集。采用量化低秩适应（QLoRA）和秩稳定LoRA（rsLoRA）技术，在双NVIDIA Tesla T4 GPU上微调，仅训练约1%的参数，总训练时间少于27 GPU小时。评估指标包括困惑度、BERTScore、chrF++、ROUGE-1/2/L和BLEU，涵盖流畅性、语音一致性和语义完整性七个维度。

Result: 零样本设置下，所有三种模型都无法生成罗马化尼泊尔语，各自表现出特定的架构相关失败模式。微调后，所有模型都解决了这些问题，BERTScore达到约0.75，chrF++大于23。Qwen3-8B是唯一能在零样本设置下产生语义相关输出的模型，并在微调后在所有结构对齐指标中领先。Llama-3.1-8B尽管零样本基线最弱，但在困惑度（Δ=-49.77）和BERTScore（Δ=+0.3287）方面取得最大的绝对微调增益。

Conclusion: Qwen3-8B是整体推荐的架构，而Llama-3.1-8B因其最大的微调改进潜力，成为迭代低资源开发流程的首选。本研究为罗马化尼泊尔语在规模相当的开源大语言模型中的适应建立了首个严谨基准，证实了"适应提升空间"假设。
...
TL;DR: 该研究通过训练一个微小的后transformer适配器来纠正语言模型在政治敏感话题上的事实概率抑制，适配器能够恢复被压制的知识而不产生知识退化。


<details>
  <summary>Details</summary>
Motivation: 语言模型在政治敏感话题上经常抑制事实概率，尽管这些知识仍然存在于隐藏表示中。研究者希望开发一种方法来纠正这种抑制，同时保持模型的生成能力。

Method: 使用仅786K参数的post-transformer适配器，在冻结的隐藏状态上进行训练。比较了门控(SwiGLU)和非门控(线性瓶颈)适配器，并测试了不同应用位置（全位置vs仅最后位置）的效果。

Result: 适配器能够记忆所有15个训练事实，并在16个保留事实上实现11-39%的泛化能力，且没有知识退化。仅最后位置应用适配器能产生连贯且较少审查的文本，而全位置应用会导致输出不连贯。

Conclusion: 隐藏状态干预是纠正生成的有效层级，适配器能够有效恢复被压制的知识而不损害生成质量。研究还发现了Apple MLX中的梯度bug并提供了解决方案。
...
TL;DR: 该论文提出了一个统一系统，用于解决ArchEHR-QA共享任务中的子任务3（答案生成）和子任务4（证据句子对齐）。系统在子任务3中采用两阶段QLoRA微调Qwen3-4B模型，在子任务4中使用三种检索方法的加权集成，实验表明两个子任务都面临相同的数据不足挑战。


<details>
  <summary>Details</summary>
Motivation: 解决医疗电子健康记录问答任务中的两个关键子任务：答案生成和证据句子对齐，探索在有限标注数据下的有效方法。

Method: 对于子任务3：采用两阶段量化低秩适应（QLoRA）微调Qwen3-4B模型，第一阶段在emrQA-MedSQuAD语料库的30,000个样本上进行临床领域适应，第二阶段在20个标注开发案例上学习任务特定输出风格。对于子任务4：开发了三种检索方法的加权集成：BM25相对阈值、TF-IDF余弦相似度和微调交叉编码器。

Result: 子任务3在官方测试集上获得总分32.87（BLEU=9.42, ROUGE-L=27.04, SARI=55.42, BERTScore=43.00, AlignScore=25.28, MEDCON=37.04）。子任务4在100个测试案例上获得微平均F1分数67.16。实验表明20个标注训练案例不足以区分相关和不相关的临床句子。

Conclusion: 两个子任务都面临相同的基本挑战：有限的标注数据（20个案例）无法有效区分相关和不相关的临床句子，数据增强是未来最有潜力的研究方向。
...
TL;DR: SPFG数据集基于口语学习者语料构建，包含语法纠错目标和人工验证的教学反馈，研究显示监督微调在联合生成纠错和反馈方面效果最稳定。


<details>
  <summary>Details</summary>
Motivation: 现有语法纠错和解释研究缺乏真实教学场景所需的可操作、适合学习者水平且具有鼓励性的教学反馈，需要构建专门的数据集来支持这一需求。

Method: 基于Speak & Improve Challenge 2025语料构建SPFG数据集，包含流利度导向的转录文本、语法纠错目标和人工验证的教师风格反馈；评估三种指令调优大语言模型，比较监督微调与基于偏好的对齐方法（DPO和KTO）在联合生成纠错和反馈方面的表现。

Result: 监督微调提供最一致的改进效果，而DPO/KTO方法带来的提升较小或效果混合；研究发现纠错质量和反馈质量之间关联较弱。

Conclusion: SPFG数据集为口语语法纠错和教学反馈生成研究提供了有价值的资源，监督微调在联合生成任务中表现最佳，但纠错和反馈质量需要分别优化。
...
TL;DR: 对2022-2026年间LLM在罕见病患者教育与沟通应用研究的范围综述，发现该领域仍处早期阶段，主要使用通用模型如ChatGPT，集中在问答场景，评估偏重准确性而忽视患者中心维度。


<details>
  <summary>Details</summary>
Motivation: 罕见病影响全球超3亿人，具有复杂的护理路径、有限的临床专业知识和长期患者旅程中未满足的沟通需求。大型语言模型（LLMs）的进展为支持患者教育和沟通提供了新机会，但在罕见病领域的应用尚不明确。

Method: 对2022年1月至2026年3月主要数据库发表的研究进行范围综述，识别出12项关于LLM在罕见病患者教育与沟通应用的研究。提取研究特征、应用场景、模型使用和评估方法等数据，采用描述性和定性分析进行综合。

Result: 文献高度集中在近期，主要由通用模型主导，特别是ChatGPT。大多数研究聚焦于使用精选问题集的患者问答，很少使用真实世界数据或纵向沟通场景。评估主要关注准确性，较少关注患者中心维度如可读性、同理心和沟通质量。多语言沟通很少涉及。
...
TL;DR: 研究训练了一个纯古典中文语言模型，发现模型内部能区分已知与未知信息，但外部表达中不会表达不确定性，这种模式在不同语言和模型中普遍存在。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究语言模型是否能够区分已知与未知信息，以及是否能在生成文本中表达这种不确定性，从而理解语言模型是否具有真正的元认知能力。

Method: 方法包括：1) 训练一个3.18亿参数的Transformer语言模型，使用纯古典中文语料库（15.6亿token）；2) 进行系统性的分布外测试，比较真实历史事件、虚构事件和半虚构事件的困惑度；3) 分析模型在生成文本中使用不确定性表达标记的频率；4) 在三种语言（古典中文、英语、日语）、三种书写系统和八个不同规模的模型上进行复制实验。

Result: 结果显示：1) 模型内部能有效区分已知与未知信息，真实与虚构历史事件的困惑度比率为2.39倍，半虚构事件（真实人物+虚构事件）的困惑度最高（4.24倍）；2) 模型外部从不表达不确定性，古典中文不确定性标记在分布外问题中出现率（3.5%）低于分布内问题（8.3%）；3) 这种模式在不同语言和模型中普遍存在，不确定性表达频率完全由训练数据惯例决定，古典中文模型表现出"谦逊悖论"，而日语模型几乎从不使用不确定性表达。

Conclusion: 结论是语言模型本身不会自发发展出元认知表达能力（如说"我不知道"），这种能力需要明确的训练信号，如RLHF（人类反馈强化学习）。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的性能高度依赖提示设计，但现有提示构建方法描述和应用不一致，缺乏统一的概念框架和参考架构，需要提高概念清晰度和支持系统化提示设计。

Method: 通过多数据库搜索识别11个已发布的提示框架，进行严格综合分析，提出区分提示框架、提示元素、提示生成、提示技术和提示工程的概念分类法，并推导出PICCO五元素参考架构。

Result: 提出了PICCO框架，包含Persona（角色）、Instructions（指令）、Context（上下文）、Constraints（约束）和Output（输出）五个核心元素，为每个元素定义了功能、范围和相互关系，提高了概念清晰度。

Conclusion: PICCO是一个概念和方法论贡献，为提示规范和比较提供了通用结构，支持更系统化的提示设计，但尚未经过实证验证作为优化方法。框架还概述了实施相关概念，包括提示技术、迭代提示工程方法、负责任提示考虑因素和未来研究方向。
...
TL;DR: 研究通过SWE-bench Lite基准测试发现，中文提示在LLM编程任务中并不比英文更token高效，且成功率通常更低，成本效率优势不成立。


<details>
  <summary>Details</summary>
Motivation: 针对社交媒体和开发者论坛上流传的"中文提示比英文更token高效，可节省高达40%API成本"的说法，研究者希望验证这一主张在软件工程任务中的真实性，避免开发者基于错误信息做出决策。

Method: 使用SWE-bench Lite软件工程任务基准进行严格的实证研究，评估不同模型架构下中英文提示的token消耗、任务成功率，并计算预期成本效率（每成功任务的预期成本）。

Result: 1. 未观察到中文的token效率优势；2. token成本因模型架构而异：MiniMax-2.7中文token成本高1.28倍，GLM-5中文消耗更少token；3. 所有测试模型中，中文提示的成功率普遍低于英文；4. 综合考虑token消耗和任务解决率的成本效率分析表明，仅通过切换提示语言无法获得成本节省或性能提升。

Conclusion: 语言对token成本的影响是模型依赖的，开发者不应期望通过将提示语言切换为中文来获得成本节省或性能提升。这些发现应被视为初步证据而非最终结论，因为评估模型数量有限且测试基准范围较窄。
...
TL;DR: 提出了REVIEWBENCH基准和REVIEWGROUNDER框架，通过基于论文特定评分标准和上下文证据的评审方法，显著提升了LLM生成评审意见的质量。


<details>
  <summary>Details</summary>
Motivation: 当前AI会议投稿激增，使用大语言模型进行同行评审支持的需求增加，但现有LLM生成的评审意见往往流于表面、公式化，缺乏实质性和基于证据的反馈。这主要因为未充分利用人类评审的两个关键要素：明确的评分标准和基于现有工作的上下文基础。

Method: 提出了REVIEWBENCH基准，根据官方指南、论文内容和人工评审意见制定论文特定评分标准来评估评审文本。进一步提出了REVIEWGROUNDER框架，这是一个基于评分标准、集成工具的多智能体框架，将评审过程分解为草稿撰写和证据夯实两个阶段，通过有针对性的证据整合来丰富浅层草稿。

Result: 在REVIEWBENCH上的实验表明，REVIEWGROUNDER（使用Phi-4-14B作为草稿撰写模型和GPT-OSS-120B作为证据夯实阶段）在8个维度上，无论是与人类判断的一致性还是基于评分标准的评审质量，都显著优于使用更强/更大骨干模型（如GPT-4.1和DeepSeek-R1-670B）的基线方法。

Conclusion: 通过引入论文特定评分标准和上下文证据整合，REVIEWGROUNDER框架能够生成更高质量、更符合人类评审标准的评审意见，为解决LLM在同行评审中的表面化问题提供了有效方案。
...
Conclusion: EuropeMedQA将促进更通用医学AI的发展，反映欧洲临床实践的复杂性，为评估LLM在非英语医学环境中的表现提供标准化基准
...
TL;DR: 研究通过分析126,602篇在线新闻文章，量化了暴力灾难事件的新闻报道动态模式，发现突发重大事件具有可预测的新闻周期模式：报道量激增、早期语义漂移、逐渐回归基线。


<details>
  <summary>Details</summary>
Motivation: 现代新闻周期因在线信息快速交换而重塑，媒体报道框架随着新信息、政治反应和社会反应而动态变化。理解这些叙事如何形成、传播和演变对于解读危机时期的公共话语至关重要。

Method: 使用从在线出版商收集的126,602篇新闻文章组成的大规模语料库，通过出版量、语义漂移、语义分散和术语相关性来量化叙事变化。

Result: 突发重大事件表现出结构化和可预测的新闻周期模式：报道覆盖迅速激增、早期语义漂移、逐渐向基线水平下降。研究还识别了驱动这些时间模式的关键术语。

Conclusion: 研究发现暴力灾难事件的新闻报道具有系统性动态模式，这些模式可以通过量化指标进行预测，有助于理解危机时期公共话语的形成和演变机制。
...
TL;DR: GPT-4.1通过分析棒球迷的观赛体验文字描述，预测其整体体验评分（0-10分），预测结果与真实评分有较高相关性（r=0.82），但系统性地低估约1分，反映了两种评分测量不同构念的差异。
...
Method: 使用GPT-4.1模型分析约10,000名来自5支MLB球队球迷的开放式文字回答，预测其整体体验评分（0-10分）。将AI预测结果与调查问卷中实际收集的自我报告评分进行比较，分析两者的一致性和差异。

Result: AI预测评分与自我报告评分有较高相关性（r=0.82），67%的预测在±1分误差范围内，36%完全匹配。但预测评分系统性地低估约1分。自我报告评分反映整体评价判断，而AI预测评分更关注描述中突出、情感强烈、不寻常或可行动的时刻。

Conclusion: 简单的未优化提示可以方向性地预测球迷体验评分，两种评分之间的差异反映了不同的心理构念（整体评价vs.突出时刻影响），这种差异值得保留而非消除，为体验测量提供了新的视角。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然性能强大但缺乏可解释性，限制了在需要透明度和信任的领域应用。现有的后验文本解释方法关注主观可信度，但缺乏认知忠实性（是否反映模型实际依赖的内部证据）。

Method: 1. 首先通过反事实评估LLM生成解释的认知忠实性；2. 提出无训练方法，通过注意力层干预指导解释生成，使用忠实归因方法提取的token级热图作为干预依据。

Result: 评估显示LLM生成的解释通常不忠实；提出的方法在多个模型、基准测试和提示下显著提升了认知忠实性。

Conclusion: LLM生成解释存在认知忠实性问题，基于注意力干预的无训练方法能有效提升解释忠实性，增强模型透明度和可信度。
...
TL;DR: 本文提出RoPE扰动自蒸馏方法，通过扰动RoPE索引创建训练序列的替代"视图"，训练模型在不同视图间产生一致预测，从而提高长上下文模型的位置鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在需要可靠长上下文理解的场景中应用日益广泛，但标准的长上下文适应方法仍然脆弱，模型准确性严重依赖于相关证据的绝对位置，即使控制任务格式和难度，仍表现出较高的位置方差。

Method: 提出RoPE扰动自蒸馏训练正则化方法，核心思想是通过扰动RoPE索引形成相同训练序列的替代"视图"（实际上将上下文的不同部分移动到不同位置），然后通过自蒸馏训练模型在不同视图间产生一致的预测，鼓励模型依赖语义信号而非脆弱的位置依赖。
...
Conclusion: RoPE扰动自蒸馏方法有效提高了长上下文模型的位置鲁棒性，减少了模型对绝对位置信息的依赖，增强了模型在长上下文任务中的稳定性和泛化能力。
...
TL;DR: 开发小型开源语言模型，用于在社交媒体帖子中自动检测多囊卵巢综合征（PCOS）患者面临的三大负担（身体形象困扰、饮食失调和代谢挑战），并提供可解释的证据。


<details>
  <summary>Details</summary>
Motivation: PCOS女性面临身体形象困扰、饮食失调和代谢挑战的显著风险，但现有的自然语言处理方法缺乏透明度，且无法识别这些问题的共现情况。

Method: 从六个Reddit子论坛收集1000个PCOS相关帖子，由两名训练有素的标注者使用基于Lee等人（2017）临床框架的指南进行标注。使用低秩适应（LoRA）对三个小型语言模型（Gemma-2-2B、Qwen3-1.7B、DeepSeek-R1-Distill-Qwen-1.5B）进行微调，生成带有文本证据的结构化解释。

Result: 最佳模型在150个保留测试帖子上达到75.3%的精确匹配准确率，具有强大的共病检测能力和良好的可解释性。随着诊断复杂性的增加，性能有所下降。

Conclusion: 这些模型最适合用于筛查目的，而非自主诊断，因为它们在复杂病例中的性能会下降。开发的小型开源模型为PCOS相关心理和代谢问题的检测提供了透明且可解释的解决方案。
...
TL;DR: 本文提出质心替换方法，发现多模态语言模型中文本表示主导视觉表示，导致视觉感知任务表现不佳，并提出文本质心对比解码方法在推理时无需重新训练即可提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态语言模型在视觉感知任务上系统性表现不佳，但其失败的根本结构原因尚不清楚。研究者希望探究模态依赖的结构性不平衡问题。

Method: 提出质心替换方法，将每个token替换为其最近的K-means质心，作为模态依赖的受控探针。使用文本质心对比解码，通过对比文本质心被擦除的参考模型来提升性能。

Result: 在7个模型（涵盖3种架构家族）中，擦除文本质心结构比擦除视觉质心结构多损失4倍准确率，揭示了文本表示主导视觉表示的普遍不平衡现象。文本质心对比解码在单个任务上可恢复高达+16.9%的准确率提升。

Conclusion: 模态竞争是结构局部化的，可以在推理时无需重新训练进行纠正，并且可作为诊断信号来指导未来的多模态训练。不同训练方法的影响不同：标准微调模型比偏好优化模型获得更大提升。
...
TL;DR: 该研究提出了一种对话式自动事实核查方法，通过分阶段去口语化重写和语义感知一致性门控机制，提升证据检索和事实验证性能。


<details>
  <summary>Details</summary>
Motivation: 对话式事实核查中，口语化语言频繁出现但研究不足，需要处理多轮对话中的非正式表达，以提高事实核查的准确性和稳定性。

Method: 采用分阶段去口语化重写方法，结合轻量级表面标准化和限定范围内的指代消解，然后引入BiCon-Gate语义感知一致性门控机制，选择性地使用重写候选或原始声明。

Result: 在DialFact基准测试中，该方法在证据检索和事实验证方面均有提升，特别是在SUPPORTS类别上表现突出，优于包括基于解码器的一次性LLM重写在内的竞争基线。

Conclusion: 提出的分阶段去口语化重写结合语义感知门控机制的方法，能够有效处理对话中的口语化语言，稳定下游事实核查流程，提升整体性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决WordNet等词汇资源向新语言自动扩展的问题，传统方法依赖人工标注或词典映射，成本高且质量不稳定

Method: 基于语义投影的方法：使用英语语料及其翻译，将英语同义词集投影到对齐的目标语言词元上，通过双语词典增强预训练对齐器并过滤错误投影

Result: 在多种语言上评估显示，提出的投影-过滤策略在保持可解释性和较少外部资源需求的同时提高了精度，优于先前方法、基于词典的方法和大语言模型基线

Conclusion: 该方法为多语言词汇资源扩展提供了一种有效且实用的解决方案，代码、文档和生成的词义清单将公开提供
...
TL;DR: 论文指出在多轮人机对话评估中，传统基于轮次指标的分析忽略了对话轮次间的自相关性，导致统计显著性被严重高估。作者通过实证分析展示了问题严重性，并提出了两阶段校正框架。


<details>
  <summary>Details</summary>
Motivation: 当前多轮人机对话评估中广泛使用轮次级指标，但几乎所有现有评估流程都忽略了连续对话轮次间的统计依赖性，导致统计推断存在偏差。这种忽视会严重夸大显著性结果，影响评估的可靠性。

Method: 作者系统分析了66个轮次级指标在202个多轮对话中的自相关结构，涉及11,639个轮次对。提出了结合Chelton有效自由度和对话级块自助法的两阶段校正框架，并在预注册的保留集上验证了其有效性。

Result: 研究发现：1）传统池化分析导致42%的关联在标准检验中显示显著，但在聚类稳健校正后不再显著；2）显著性膨胀程度因指标类别而异，无记忆家族指标为14%，有记忆家族指标为33%；3）校正后的指标在保留集上的复制率达到57%，而传统方法仅为30%。

Conclusion: 多轮对话评估必须考虑轮次间的自相关性，否则会导致统计显著性严重高估。作者提供了具体的校正框架、设计原则、发表检查清单和开源代码，呼吁研究社区重视这一问题。
...
Method: HRM-LM将L个独立Transformer层替换为两速循环对：Fast模块每步运行进行局部细化，Slow模块每T步运行进行全局压缩。该循环层次结构在M = N x T步中展开并共享参数

Result: 通过参数匹配的Universal Transformer消融实验（UniTF, 1.2B）在五次独立运行中，发现两种方法之间存在显著的实证差距

Conclusion: 分层共享权重的循环架构在表征质量上与独立层堆叠的Transformer存在明显差距，这是最稳健的核心发现
...
TL;DR: 该研究使用分布式对齐搜索方法分析不同数据量训练的语言模型，发现填充语-空位依赖关系在不同句法结构间存在共享但项目敏感的机制，但模型需要比人类多得多的数据才能获得可比的语言泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究语言模型是否能在发展可行的数据量下学习到填充语-空位依赖关系的共享表征。虽然已有因果分析表明大型语言模型可能存在这种共享表征，但尚不清楚在有限训练数据下是否也能形成这种表征。

Method: 使用分布式对齐搜索方法，分析在不同数据量下训练的语言模型（基于BabyLM挑战的数据）。评估填充语-空位依赖关系的表征是否能在wh-疑问句和话题化结构之间迁移，这两种结构在输入频率上差异很大。

Result: 结果显示，在有限训练数据下可能发展出共享但项目敏感的机制。更重要的是，语言模型需要比人类多得多的数据才能学习到可比的语言泛化能力。

Conclusion: 研究表明语言模型需要语言特定的偏置来模拟语言习得过程，当前模型在数据效率方面与人类语言习得存在显著差距。
...
TL;DR: 本文提出了一种基于心理学引导的LLM控制框架，通过语义校准的残差流注入技术，在14个LLM上实现了比传统人格提示方法更优的开放式生成效果。


<details>
  <summary>Details</summary>
Motivation: 现有LLM激活干预方法局限于有限的搜索空间和未校准的激活空间单位，可能错过最优干预条件，需要一种更精确的心理学引导框架。

Method: 提出心理学引导框架，使用心理学人工制品校准残差流注入，在语义校准单位上进行无界、流畅性约束的扫描，采用IPIP-NEO-120测量OCEAN人格模型，比较六种注入方法。

Result: 均值差异注入在14个LLM中的11个上优于人格提示方法，提升3.6%-16.4%；混合方法在13个LLM中表现最佳，相比人格提示提升5.6%-21.9%，相比均值差异注入提升3.3%-26.7%。

Conclusion: 均值差异注入符合线性表示假设，为心理学引导提供了可靠的控制手段，但诱导的OCEAN特质协方差模式与人类心理学存在差异，表明学习表示与人类心理之间存在差距。
...
<details>
  <summary>Details</summary>
Motivation: 神经方法需要大量调参且存在灾难性遗忘问题，而经典概率模型缺乏灵活性和对流数据的适应性，因此需要一种能够终身学习、动态适应且参数较少的新型主题建模方法。

Method: 将Cobweb算法适配到连续文档嵌入表示，通过增量概率概念形成在线构建语义层次结构，实现无监督主题发现和动态主题创建，无需预定义主题数量。

Result: 在多个数据集上，CobwebTM实现了强大的主题连贯性、随时间稳定的主题以及高质量的分层结构，证明了增量符号概念形成与预训练表示结合的高效性。

Conclusion: 增量符号概念形成与预训练表示相结合是主题建模的高效方法，CobwebTM能够实现终身学习、动态适应且参数较少的主题建模，克服了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前同行评审中LLM检测方法将作者身份视为二元问题（人类vs.AI），但现代评审工作流程具有混合性质，评估思想和文本表达可能来自不同来源，需要更精细的检测方法。

Method: 构建PeerPrism基准测试，包含完全人工、完全合成以及多种混合转换的受控生成机制，系统评估检测器是否能区分表面文本来源和评估推理来源。

Result: 现有LLM检测方法在标准二元任务（人类vs.完全合成）上表现良好，但在混合机制下预测结果严重分歧，特别是当思想来自人类而表面文本由AI生成时，检测器经常产生矛盾分类。
...
TL;DR: 该研究提出基于表征工程的认知逆向工程框架，分析LLM中社会比较嫉妒情绪的内部机制，发现模型将嫉妒编码为两个心理前因的线性组合，与人类心理结构一致。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型展现出日益复杂的情感能力，但其处理复杂情绪的内部机制仍不清楚。现有可解释性方法通常将模型视为黑盒或只关注粗粒度的基本情绪，对更复杂情感状态的认知结构探索不足。
...
TL;DR: 本文指出NLP领域多样性进展过度集中于公平性相关领域，存在激励机制、偏见和障碍导致边缘化研究者被排斥或被迫转向公平性研究，并提出打破反馈循环、消除地理语言障碍等建议。


<details>
  <summary>Details</summary>
Motivation: 本文旨在揭示NLP领域多样性进展的不均衡分布问题，特别是公平性相关领域占据主导地位，而其他领域缺乏多样性关注。作者关注到激励机制、偏见和系统性障碍导致边缘化研究者在非公平性领域被排斥或被迫转向公平性研究。

Method: 采用立场论文形式，通过调查NLP研究者按子领域的人口统计学特征来支持论点。研究分析了激励机制、偏见和障碍如何影响研究者的领域选择和参与机会。

Result: 研究发现NLP多样性进展过度集中于公平性相关领域，存在系统性障碍导致边缘化研究者在非公平性领域被排斥或被迫转向公平性研究。这种不均衡分布反映了领域内的结构性不平等。

Conclusion: 提出多项建议确保NLP所有领域都能更具包容性和公平性，特别强调打破强化不平等的反馈循环，以及解决阻碍NLP研究参与的地理和语言障碍。呼吁更均衡的多样性关注。
...
TL;DR: CAUSALDETOX：通过因果干预注意力头来减少LLM毒性生成，同时保持语言流畅性
...
Method: 使用概率必要性和充分性(PNS)识别导致毒性生成的具体注意力头，采用两种策略：局部推理时间干预（动态构建输入特定的导向向量）和PNS引导微调（永久消除毒性表示）

Result: 在ToxiGen、ImplicitHate和ParaDetox数据集上，CAUSALDETOX相比基线毒性减少提升5.34%，保持语言流畅性，注意力头选择速度提升7倍

Conclusion: CAUSALDETOX通过因果干预注意力头有效减少LLM毒性生成，同时保持生成质量，为安全部署提供了高效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 临床价值集构建是临床质量测量和表型分析中的瓶颈，传统直接使用大语言模型生成代码的方法不可靠，因为结构化临床词汇表庞大、版本控制且预训练时未可靠记忆

Method: 提出检索增强集完成方法：1)从整理语料库中检索K个最相似的现有价值集形成候选池；2)对每个候选代码应用分类器进行筛选。理论优势是将输出空间从完整词汇表缩小到更小的候选池

Result: 在11,803个公开VSAC价值集上测试，基于SAPBert微调的交叉编码器达到AUROC 0.852和价值集级F1 0.298，优于三层MLP和GPT-4o。将每个真阳性的无关候选从12.3减少到约3.2-4.4

Conclusion: RASC方法在临床价值集构建任务上表现优异，理论优势得到验证，性能提升在不同模型类型中均得到体现，为临床质量测量提供了有效解决方案
...
Method: 提出StoryCoder叙事重构框架，将代码生成问题转化为包含任务概述、约束和示例测试用例的自然语言叙事，基于选定算法和类型进行指导

Result: 在HumanEval、LiveCodeBench和CodeForces的11个模型上实验，平均零样本pass@10提升18.7%，引导模型采用正确算法策略，减少实现错误，产生更模块化的代码结构

Conclusion: 叙事重构通过提供连贯的问题表示来提升代码生成性能，其效果取决于叙事连贯性和类型对齐，表明结构化问题表示对代码生成至关重要，不受模型规模或架构影响
...
TL;DR: 该论文提出了一种大规模选项评估协议，将候选集扩展到100个选项，以减少偶然性能的影响，并应用于韩语正字法错误检测任务，揭示了在低选项设置中模型能力的过度评估问题。


<details>
  <summary>Details</summary>
Motivation: 传统的多项选择评估在低选项设置中可能因捷径策略而高估模型能力，需要一种能够更准确评估模型真实能力的方法。

Method: 提出大规模选项评估协议，将候选集扩展到100个选项，应用于韩语正字法错误检测任务，通过固定目标、重复重采样和洗牌获得稳定估计，并运行填充控制和长度匹配测试来隔离上下文长度的影响。

Result: 结果表明，在低选项设置中的强性能可能高估模型能力，在高N值下的密集干扰中这种优势减弱，揭示了语义混淆和位置偏差两种失败模式，主要瓶颈是候选排序而非上下文长度。

Conclusion: 大规模选项评估可作为一般框架，在极端干扰密度下对模型可靠性进行压力测试，揭示传统低选项基准无法显示的模型能力差距。
...
<details>
  <summary>Details</summary>
Motivation: 金融虚假信息的泛滥严重威胁市场稳定和投资者信任，导致市场行为误导和信息不对称。在无法获取外部证据或参考信息进行交叉验证的现实场景中，检测这类误导性叙述具有极大挑战性。

Method: 提出一个综合框架，利用最先进大语言模型的推理能力，系统整合上下文学习（零样本和少样本提示策略）与参数高效微调（通过LoRA低秩适应），使模型能够捕捉金融操纵的微妙语言线索。

Result: 该方法在两个官方排行榜上均获得第一名：公开测试集准确率95.4%，私有测试集准确率96.3%，证明了方法的鲁棒性。模型（14B和32B）已在HuggingFace平台开源。

Conclusion: 该方法通过结合上下文学习和参数高效微调，成功实现了无需外部参考的金融虚假信息检测，为金融自然语言处理中的上下文感知虚假信息检测提供了有效解决方案。
...
TL;DR: CURaTE是一种实时持续遗忘方法，通过训练句子嵌入模型建立遗忘请求的决策边界，根据输入与遗忘请求的相似度决定是否拒绝回答，避免修改LLM参数，实现高效遗忘和知识保护。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型预训练时无法完全过滤问题数据，需要遗忘特定知识的方法。现有技术缺乏持续性和即时性，导致随着更新累积而效用下降，敏感信息暴露时间长。

Method: 训练句子嵌入模型，在专门设计的数据集上形成清晰的决策边界，判断输入提示是否对应存储的遗忘请求。根据输入与遗忘请求的相似度决定是回答问题还是返回拒绝响应。

Result: CURaTE比现有方法实现更有效的遗忘，通过避免修改语言模型参数，在任何数量的更新中保持近乎完美的知识保护，是唯一能够实时进行持续遗忘的方法。

Conclusion: CURaTE提供了一种简单而有效的实时持续遗忘解决方案，在保持语言模型知识完整性的同时，能够即时处理遗忘请求，解决了现有方法在持续更新和实时响应方面的不足。
...
TL;DR: CURA框架通过双层不确定性目标校准临床语言模型的风险估计和不确定性，在个体层面和群体层面实现更好的校准，提高临床决策的可靠性。
...
Method: CURA框架首先微调领域特定的临床语言模型获得任务适应的患者嵌入，然后使用双层不确定性目标进行多头分类器的不确定性微调。包括个体层面校准项（对齐预测不确定性与患者错误可能性）和群体感知正则化项（将风险估计拉向嵌入空间中局部邻域的事件率，并在决策边界附近的模糊群体上施加额外权重）。

Result: 在MIMIC-IV临床风险预测任务上的广泛实验表明，CURA在各种临床语言模型上一致改进了校准指标，同时没有显著损害判别能力。进一步分析显示CURA减少了过度自信的虚假保证，为下游临床决策支持提供了更可信的不确定性估计。

Conclusion: CURA框架通过同时考虑个体层面校准和群体层面模糊性，有效提高了临床语言模型风险预测的校准性能，为临床决策支持提供了更可靠的不确定性估计。
...
TL;DR: SPAGBias是首个评估LLM空间性别偏见的系统框架，通过62种城市微空间分类、提示库和三层诊断方法，发现LLM存在超越公私领域划分的结构化性别-空间关联，这些偏见在模型全流程中被嵌入和强化，导致实际应用中的具体失败。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在城市规划中的广泛应用，基于性别空间理论（性别等级制度嵌入空间组织）的担忧日益增长，需要系统评估LLM是否可能复制或放大空间性别偏见。

Method: 提出SPAGBias框架：包含62种城市微空间分类法、提示库，以及三层诊断方法：显性层（强制选择重采样）、概率层（token级不对称性）、建构层（语义和叙事角色分析）。测试了六个代表性模型，并研究了提示设计、温度和模型规模对偏见表达的影响。

Result: 发现LLM存在超越公私领域划分的结构化性别-空间关联，形成细微的微观层面映射。故事生成揭示了情感、措辞和社会角色如何共同塑造"空间性别叙事"。模型关联显著超出真实世界分布，这些模式在预训练、指令微调和奖励建模的全流程中被嵌入和强化。下游实验显示这些偏见在规范性和描述性应用场景中都会导致具体失败。

Conclusion: 该研究将社会学理论与计算分析相结合，将偏见研究扩展到空间领域，揭示了LLM如何通过语言编码社会性别认知，为理解AI系统中的空间性别偏见提供了系统框架。
...
TL;DR: 该论文针对知识图谱问答中负约束被忽视的问题，提出了NEST-KGQA新任务和数据集，设计了PyLF逻辑形式，并开发了CUCKOO框架来处理多约束问题。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型具有强大的推理能力，但在忠实性和幻觉方面仍存在问题。在知识图谱问答中，语义解析方法通过理解用户问题中的约束并将其转换为逻辑形式来执行，但现有基准和方法偏向于正约束和计算约束，忽视了负约束，而负约束在实际问题中经常出现。

Method: 1) 提出了NEgative-conSTrained (NEST) KGQA新任务和NestKGQA数据集；2) 设计了PyLF（Python格式的逻辑形式），能清晰表达否定同时保持可读性；3) 提出了CUCKOO框架，专门处理多约束问题，确保语义可执行性。CUCKOO首先生成约束感知的逻辑形式草稿，进行模式引导的语义匹配，然后仅在执行不当逻辑形式产生空结果时选择性应用自导向精炼。

Result: 实验结果表明，CUCKOO在少样本设置下，在传统KGQA和NEST-KGQA基准测试中都一致优于基线方法。

Conclusion: 该研究填补了知识图谱问答中负约束处理的空白，提出的NEST-KGQA任务、PyLF逻辑形式和CUCKOO框架为解决多约束、特别是负约束问题提供了有效方案，在保持成本效益的同时提高了鲁棒性。
...
TL;DR: CoPA是一个用于评估个性化问答系统的基准测试，通过挖掘社区-个体偏好差异来定义六个个性化因素，包含1,985个用户配置文件，提供比通用指标更全面和区分性的评估标准。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法主要依赖词汇级相似性或手动启发式方法，缺乏足够的数据驱动验证，个性化评估成为关键瓶颈。需要更全面和区分性的标准来评估个性化问答系统。

Method: 通过挖掘社区-个体偏好差异（CIPD）来提炼六个关键个性化因素作为评估维度，构建CoPA基准测试，包含1,985个用户配置文件，通过量化模型输出与用户特定认知偏好之间的对齐程度进行评估。

Result: CoPA提供了一个细粒度、因素级别的评估基准，能够更全面和区分性地评估个性化问答系统，相比通用指标具有更好的评估能力。

Conclusion: CoPA通过数据驱动的方法定义了六个个性化评估维度，为个性化问答系统评估提供了更全面和区分性的标准，解决了现有评估方法的局限性。
...
TL;DR: 本文提出了一种保留优先的梯度合成框架，将大语言模型遗忘问题重构为不对称双任务问题，通过解耦任务特定梯度提取和冲突感知组合来缓解遗忘与保留的权衡。


<details>
  <summary>Details</summary>
Motivation: 大语言模型遗忘旨在移除特定知识同时保持通用能力，但现有方法面临遗忘与保留之间的权衡问题。本文从不对称双任务视角重新审视该问题，将保留作为主要目标，遗忘作为辅助任务。

Method: 提出保留优先的梯度合成框架，解耦任务特定梯度提取和冲突感知组合。具体实现包括：1) 适配PCGrad解决梯度冲突；2) 提出SAGO方法进行保留优先梯度合成，通过构造性符号约束合成实现更紧密的对齐。

Result: 在WMDP Bio/Cyber和RWKU基准测试中，SAGO持续推动帕累托前沿。例如在WMDP Bio上，目标模型MMLU性能恢复从44.6%(朴素方法)提升到94.0%(+PCGrad)，进一步达到96.0%(+SAGO)，同时保持相当的遗忘强度。

Conclusion: 重塑梯度几何而非重新平衡损失是缓解遗忘-保留权衡的关键。保留优先的梯度合成框架通过解耦梯度提取与组合，在保持遗忘效果的同时显著提升了模型保留能力。
...
TL;DR: 本文探讨了在芬兰语医疗文本数据上微调BERT模型的经验，并尝试通过观察嵌入几何变化来预测领域特定预训练的效益


<details>
  <summary>Details</summary>
Motivation: 在医疗AI领域，获取标注数据通常存在长时间延迟，特别是在标签获取方面。因此需要研究在标注数据有限的情况下，如何通过无标注数据进行领域微调来提升模型性能

Method: 1. 在芬兰语医疗文本数据上对芬兰语BERT模型进行领域微调；2. 通过观察领域微调过程中嵌入几何的变化，尝试预测领域特定预训练的效益

Result: 论文报告了在芬兰语医疗文本上微调BERT的具体观察结果，以及通过嵌入几何分析预测领域预训练效益的尝试

Conclusion: 在标注数据稀缺的医疗AI场景中，领域微调是有效的方法，通过分析嵌入几何变化可以为预测预训练效益提供有用信息
...
TL;DR: 该研究将行为特征标注视为多种标注技能的组合而非单一任务，通过技能文件驱动的流程评估LLM辅助的中文隐喻色彩词衍生词行为特征标注，发现标注技能存在高度异质性，GPT-5.4在保留技能上表现可靠但具有选择性，开源模型存在执行问题。
...
Method: 使用3,134个中文隐喻色彩词衍生词的共现行和14特征BP模式，实施技能文件驱动的标注流程，每个特征通过模式文件、决策规则和示例外部定义。两名人工标注者完成两轮仅模式协议，将BP技能分类为可直接操作、可通过重点重新标注恢复或结构上未指定。在相同设置下评估GPT-5.4和三个本地可部署的开源模型。

Result: BP标注在技能层面高度异质：5个技能可直接操作，4个可通过重点重新标注恢复，5个保持结构未指定。GPT-5.4执行保留技能具有实质性可靠性（准确率=0.678，κ=0.665，加权F1=0.695），但这种可行性是选择性的而非全局的。人类和GPT难度分布在技能层面高度一致（r=0.881），但在实例层面（r=0.016）和词汇项目层面（r=-0.142）不一致，呈现"共享分类、独立执行"模式。成对一致性进一步表明GPT应被视为独立的第三技能声音而非直接的人类替代品。开源模型的失败主要集中在模式到技能的执行问题上。

Conclusion: 自动标注应在技能可行性而非任务级自动化的角度进行评估。BP标注的异质性意味着需要更细粒度的评估方法，LLM在特定技能上表现可靠但不能完全替代人工标注。
...
TL;DR: 针对LLMs在CBRN领域自适应越狱攻击，提出基于多证据令牌的流式探测方法，显著提升检测性能


<details>
  <summary>Details</summary>
Motivation: 现有流式探测方法依赖少数高分令牌，在敏感CBRN术语出现在良性上下文中时会产生误报，需要更鲁棒的检测方法

Method: 引入流式探测目标函数，要求多个证据令牌一致支持预测，而不是依赖孤立峰值，基于聚合信号而非单令牌线索进行检测

Result: 在1%误报率下，真阳性率相对基线提升35.55%；AUROC显著提升（基线97.40%）；Attention或MLP激活优于残差流特征；对抗微调后的字符级密码攻击仍可检测（AUROC>98.85%）

Conclusion: 基于多证据令牌的流式探测方法能有效检测CBRN领域的自适应越狱攻击，对字符级密码攻击具有鲁棒性，可即插即用
...
TL;DR: 该研究分析了18个视觉语言模型的推理动态，发现模型存在"答案惯性"问题，早期预测在推理过程中被强化而非修正。虽然推理训练模型表现出更强的纠正行为，但受模态条件影响。模型容易被误导性文本线索影响，即使视觉证据充分，而思维链只能部分揭示模态依赖关系。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型展现出推理能力，但其推理过程如何整合视觉和文本信息仍不清楚。研究者希望理解VLMs的推理动态，特别是思维链如何影响预测，以及模型对多模态信息的依赖程度，这对多模态系统的透明度和安全性有重要意义。

Method: 研究分析了18个VLMs，涵盖指令微调和推理训练模型，来自两个不同模型家族。通过跟踪思维链过程中的置信度变化，测量推理的纠正效果，评估中间推理步骤的贡献。使用带有误导性文本线索的受控干预，测试模型是否能在视觉证据充分时抵抗文本影响，并分析思维链是否能揭示这种影响。

Result: 发现模型存在"答案惯性"现象，早期预测在推理中被强化而非修正。推理训练模型表现出更强的纠正行为，但效果受模态条件影响。模型容易被误导性文本线索影响，即使视觉证据充分。思维链对模态依赖的揭示能力有限：推理训练模型更可能明确提及线索，但其流畅的思维链可能掩盖实际对文本的依赖；指令微调模型较少明确提及线索，但其简短的思维链能揭示与视觉输入的不一致。

Conclusion: 思维链只能部分揭示VLMs决策中不同模态的驱动作用。模型对文本线索的依赖可能被思维链的流畅性掩盖，这对多模态系统的透明度和安全性有重要影响。需要更全面的方法来理解VLMs的推理过程和模态依赖。
...
TL;DR: 该研究评估了多语言句子嵌入模型在立陶宛语、俄语和英语仇恨言论检测中的表现，引入了新的立陶宛语仇恨言论数据集LtHate，并比较了六种嵌入模型在监督分类和无监督异常检测任务上的性能。
...
Method: 1) 引入新的立陶宛语仇恨言论语料库LtHate；2) 在LtHate、RuToxic和EnSuperset三个数据集上评估六种多语言编码器；3) 对每种嵌入训练单类HBOS异常检测器和双类CatBoost分类器；4) 使用PCA将特征向量压缩到64维进行比较。

Result: 1) 双类监督模型在所有数据集上都显著优于单类异常检测；2) 最佳配置在立陶宛语达到80.96%准确率和0.887 AUC ROC，俄语达到92.19%准确率和0.978 AUC ROC，英语达到77.21%准确率和0.859 AUC ROC；3) PCA压缩在监督设置中几乎保留了所有判别能力，但对无监督异常检测有负面影响。

Conclusion: 现代多语言句子嵌入结合梯度提升决策树为多语言仇恨言论检测提供了稳健的软计算解决方案，监督分类方法优于无监督异常检测，PCA压缩在保持性能的同时降低了计算复杂度。
...
TL;DR: XQ-MEval：一个用于评估翻译指标的半自动构建数据集，通过注入可控错误生成伪翻译，揭示跨语言评分偏差问题


<details>
  <summary>Details</summary>
Motivation: 当前多语言翻译系统评估通常采用跨语言平均指标分数的方法，但这种方法存在可疑之处，因为指标可能存在跨语言评分偏差——同等质量的翻译在不同语言中可能获得不同分数。由于缺乏提供跨语言平行质量实例的基准数据集，且专家标注不现实，这个问题尚未得到系统研究。

Method: 提出XQ-MEval数据集，覆盖9个翻译方向。方法包括：1）自动向黄金翻译中注入MQM定义的错误；2）由母语者筛选以确保可靠性；3）合并错误生成具有可控质量的伪翻译；4）将这些伪翻译与对应的源文本和参考翻译组成三元组，用于评估翻译指标质量。

Result: 使用XQ-MEval对9个代表性翻译指标的实验显示：平均分数与人工判断之间存在不一致性，首次提供了跨语言评分偏差的经验证据。研究发现不同语言间存在明显的评分分布差异。
...
TL;DR: 该论文提出了一个从文本自动生成科学架构图的系统，通过构建包含图像、文本描述和DOT代码的数据集，并微调小型语言模型，实现了与GPT-4o相当的性能。
...
Method: 1. 构建了包含科学架构图像、对应文本描述和DOT代码表示的全面数据集；2. 利用该数据集微调一系列小型语言模型；3. 同时使用GPT-4o进行上下文学习；4. 通过语言模型对输入文本进行语义理解，生成可处理为架构图的中间代码。

Result: 提出的模型显著优于现有基线模型（如DiagramAgent），性能与基于上下文学习的GPT-4o生成结果相当。代码、数据和模型均已公开。

Conclusion: 该研究成功解决了科学架构图自动生成的问题，通过构建高质量数据集和训练模型，实现了从文本到架构图的高保真转换，为多个应用领域提供了实用工具。
...
TL;DR: 本文提出了一种结合大语言模型和定制词汇表的混合方法，用于检测和解释英语、法语和希腊语中的仇恨言论，通过两个互补的管道生成有根据的解释。


<details>
  <summary>Details</summary>
Motivation: 当前自动检测系统主要关注审查或删除仇恨言论，缺乏透明度并可能限制言论自由，且无法解释内容为何有害。需要更透明、可问责且信息丰富的解决方案。

Method: 采用混合方法：结合大语言模型与三个新创建和整理的词汇表。使用两个互补管道：一个通过定制词汇表检测和消歧问题术语，另一个利用LLM作为上下文感知的群体目标内容评估器。将输出融合为有根据的解释。

Result: 人类评估显示该混合方法准确，能生成高质量解释，性能优于仅使用LLM的基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLMs发展迅速，但长文本生成中的不确定性量化仍然是一个挑战。现有方法通常限制LLMs产生简短或受限的答案集，而现实应用需要长文本和自由文本生成。主要困难在于LLMs经常产生语义连贯但事实不准确的响应，同时底层语义是多方面的，语言结构复杂。

Method: 提出了Interrogative Uncertainty Quantification (IUQ)框架，利用"询问-回答"范式，通过样本间一致性和样本内忠实性来量化长文本LLM输出的不确定性。该方法能够可靠地衡量声明级别的不确定性和模型的忠实性。
...
TL;DR: DiscoTrace是一种识别回答者回应信息寻求问题时所用修辞策略的方法，通过将回答表示为问题相关话语行为序列，结合对原始问题的解释，在修辞结构理论解析基础上进行标注。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解不同人类社区在回答信息寻求问题时使用的修辞策略差异，以及大型语言模型(LLMs)在回答构造方面与人类社区的差异，为开发更具语用能力的LLM回答者提供指导。

Method: DiscoTrace方法将回答表示为问题相关话语行为序列，结合对原始问题的解释，在修辞结构理论(RST)解析基础上进行标注。该方法应用于九个不同人类社区的答案分析。

Result: 研究发现：1)不同人类社区在答案构造方面具有多样化的偏好；2)LLMs在回答中不展现修辞多样性，即使被提示模仿特定人类社区回答指南；3)LLMs系统性地选择广度，处理人类回答者选择不处理的问题解释。

Conclusion: 研究结果可以指导开发更具语用能力的LLM回答者，使其能够考虑基于问答上下文的多种策略，从而更好地适应不同社区的回答风格和需求。
...
TL;DR: K-Token Merging：一种在潜在嵌入空间压缩长提示的框架，通过将连续K个token嵌入合并为单个嵌入，实现高达75%的输入长度压缩，性能损失最小。
...
Method: 提出K-Token Merging框架：1）使用轻量级编码器将每个连续K个token嵌入合并为单个嵌入；2）压缩序列由LoRA适配的LLM处理；3）生成仍使用原始词汇表。

Result: 在文本化树结构推理、亚马逊评论情感分类和CommitPackFT代码编辑任务上的实验表明，K-Token Merging在性能与压缩比之间达到帕累托前沿，实现高达75%的输入长度压缩，性能退化最小。

Conclusion: K-Token Merging通过潜在嵌入空间压缩有效解决了LLM处理长提示的计算效率问题，在保持性能的同时显著减少输入长度，为长上下文处理提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在机器翻译方面表现出色，但由于其生成性质，有时会以各种方式过度生成内容。这些过度生成与神经机器翻译中的"神经胡言乱语"不同，范围从LLM自我解释、风险性虚构到适当的解释，其中LLM能够像人类翻译一样工作，提高目标受众的理解。检测和确定过度生成的确切性质是一个具有挑战性的任务。

Method: 论文详细探讨了在商业环境中探索的不同策略，包括检测和分类LLM翻译中过度生成内容的方法。这些策略旨在区分不同类型的过度生成，如自我解释、风险性虚构和适当的解释性内容。

Result: 论文展示了在商业环境中应用这些策略的结果，表明能够有效检测和分类LLM翻译中的各种过度生成现象，为实际应用提供了实用方法。

Conclusion: LLM在机器翻译中的过度生成是一个复杂但重要的现象，需要专门的检测和分类方法。论文提出的策略在商业环境中显示出实用价值，有助于更好地理解和利用LLM的翻译能力，同时控制其过度生成的风险。
...
Method: 提出SpecGuard框架：1) 在每个步骤采样多个草稿候选并选择最一致步骤；2) 使用两种轻量级模型内部信号验证：基于注意力的接地分数和基于对数概率的置信度分数；3) 联合确定步骤接受或使用目标模型重新计算
...
### [135] [Geo2Sound: A Scalable Geo-Aligned Framework for Soundscape Generation from Satellite Imagery](https://arxiv.org/abs/2604.14707)
...
<details>
  <summary>Details</summary>
Motivation: 现有图像到音频模型在物体中心视觉场景表现良好，但在卫星图像上应用有限，因为俯视图具有复杂的广域语义模糊性。卫星图像为全球声景生成提供了独特的可扩展来源，但将这些视图与具有独特空间结构的真实声学环境匹配具有固有困难。

Method: 1) 轻量级分类器将俯视场景总结为紧凑的地理属性；2) 使用多个面向声音的语义假设生成多样化的声学合理候选；3) 地理声学对齐模块将地理属性投影到声学嵌入空间，并识别与候选集最一致的候选。此外建立了包含2万+高质量配对卫星图像、文本描述和真实世界音频记录的SatSound-Bench基准。

Result: Geo2Sound实现了SOTA FAD为1.765，比最强基线提升了50.0%。人类评估进一步证实了在真实感（26.5%）和语义对齐方面的显著提升，验证了其大规模高保真合成能力。

Conclusion: Geo2Sound通过统一的地理属性建模、语义假设扩展和地理声学对齐框架，成功解决了从卫星图像生成地理真实声景的挑战，在首个大规模基准上取得了显著性能提升，为全球声景生成提供了有效解决方案。
...
### [136] [ControlFoley: Unified and Controllable Video-to-Audio Generation with Cross-Modal Conflict Handling](https://arxiv.org/abs/2604.15086)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频到音频生成方法存在文本可控性弱（尤其在视觉-文本冲突时）、风格控制不精确（由于时序和音色信息纠缠）以及缺乏标准化评估基准的问题。

Method: 1) 联合视觉编码范式：整合CLIP与时空音频-视觉编码器以改善对齐和文本可控性；2) 时序-音色解耦：抑制冗余时序线索同时保留区分性音色特征；3) 模态鲁棒训练方案：包含统一多模态表示对齐和随机模态丢弃；4) 提出VGGSound-TVC基准用于评估视觉-文本冲突下的文本可控性。

Result: 在多个视频到音频任务中（文本引导、文本控制和音频控制生成）达到最先进性能，在跨模态冲突下实现优越的可控性，同时保持强同步性和音频质量，与工业级系统相比具有竞争力或更好表现。

Conclusion: ControlFoley通过创新的多模态控制机制解决了现有视频到音频生成方法的局限性，实现了精确的视频、文本和音频控制，为可控音频合成提供了有效解决方案。
...
### [137] [MCSC-Bench: Multimodal Context-to-Script Creation for Realistic Video Production](https://arxiv.org/abs/2604.15127)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界视频创作涉及从嘈杂素材中选择相关镜头、规划缺失镜头以完善叙事、组织成连贯故事线的复杂推理流程，但现有基准只关注孤立子任务，缺乏评估这一完整流程的支持。

Method: 提出多模态上下文到脚本创建（MCSC）任务，将嘈杂多模态输入和用户指令转换为结构化、可执行的视频脚本；构建MCSC-Bench数据集，包含11K+标注视频，每个样本包括冗余多模态材料和用户指令，以及包含基于素材的镜头、新规划镜头（含拍摄指令）和镜头对齐旁白的生产就绪脚本。

Result: 实验显示当前多模态大语言模型在长上下文下的结构感知推理方面存在困难；在MCSC-Bench上训练的模型达到SOTA性能，8B模型超越Gemini-2.5-Pro，并能泛化到域外场景；基于生成脚本的下游视频生成进一步验证了MCSC的实际价值。

Conclusion: MCSC任务和MCSC-Bench数据集填补了评估完整视频创作流程的空白，揭示了当前模型在结构感知推理方面的挑战，展示了训练模型在该任务上的有效性和泛化能力，为视频创作提供了实用工具。
...
TL;DR: Fun-TSG是一个完全可定制的时间序列生成器，旨在解决多变量时间序列异常检测评估中的基准数据集局限性问题，提供细粒度标注和透明生成过程。


<details>
  <summary>Details</summary>
Motivation: 当前多变量时间序列异常检测评估面临挑战，现有基准数据集缺乏细粒度异常标注、不提供明确的变量间和时间依赖性、对底层生成机制缺乏洞察，这些限制阻碍了检测模型的开发和严格比较。
...
Conclusion: Fun-TSG填补了多变量时间序列异常检测评估的空白，通过提供完全可定制、透明且具有细粒度标注的数据生成工具，支持更高质量的异常检测系统评估。
...
TL;DR: 本文提出Group Fine-Tuning (GFT)框架，通过群体优势学习和动态系数校正解决SFT训练中的奖励稀疏性和优化不稳定问题，实现知识注入与泛化能力的统一。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型通常使用监督微调(SFT)和强化学习(RL)进行后训练，但有效统一高效知识注入与鲁棒泛化能力仍然具有挑战性。研究发现SFT可解释为具有极端稀疏隐式奖励和不稳定逆概率加权的策略梯度优化，导致单一路径依赖、熵崩溃和梯度爆炸。

Method: 提出Group Fine-Tuning (GFT)框架，包含两个核心机制：1) Group Advantage Learning：构建多样化响应群体并推导归一化对比监督以缓解奖励稀疏性；2) Dynamic Coefficient Rectification：自适应限制逆概率权重以稳定优化过程，同时保持高效知识注入。

Result: 实验表明GFT持续超越基于SFT的方法，并产生与后续RL训练更平滑集成的策略。

Conclusion: GFT通过解决SFT训练动态中的内在限制，提供了一个统一的后训练框架，有效平衡了知识注入效率和模型泛化能力。
...
TL;DR: GazeX：利用放射科医师眼动数据作为行为先验的视觉语言模型，通过模拟专家视觉检查过程，提高胸部X光解读的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视觉语言模型在胸部X光解读中存在局限性，其输出与放射科医师的临床推理过程存在差距。这些系统通常只优化语义信息，而没有模拟专家如何视觉检查医学图像，经常忽略关键发现或偏离既定的诊断工作流程。

Method: 引入GazeX模型，利用放射科医师的眼动追踪数据作为行为先验来建模专家诊断推理。通过将注视轨迹和注视模式纳入预训练，GazeX学习遵循放射科医师注意力的空间和时间结构，并以临床有意义的顺序整合观察结果。使用来自5名放射科医师的30,000多个注视关键帧的精选数据集。

Result: GazeX在放射学报告生成、疾病定位和视觉问答方面产生更准确、可解释且与专家一致的输出。使用了231,835个放射学研究、780,014个问答对和1,162个带有边界框的图像句子对。与自主报告系统不同，GazeX产生可验证的证据工件，包括检查轨迹和发现链接的局部区域。

Conclusion: 通过专家视角学习为放射学及其他领域提供了实现更可信、可解释和诊断稳健的AI系统的实用途径。GazeX能够实现高效的人类验证和安全的人机协作。
...
### [141] [Credo: Declarative Control of LLM Pipelines via Beliefs and Policies](https://arxiv.org/abs/2604.14401)
...
Conclusion: Credo通过信念表示和声明式策略的设计，解决了现有智能体AI框架在透明度、鲁棒性和可验证性方面的局限性，为长期状态决策系统提供了更好的解决方案。
...
Method: 构建几何MoE（ST-MoE），使用余弦相似度路由在低维空间（d_space=64）中针对学习到的质心进行路由，比标准线性路由器减少80%路由参数。通过62个对照实验在WikiText-103上进行训练（76-84M参数，50K步，1.64B令牌）

Result: 路由拓扑结构不决定渐近困惑度：五种余弦路由变体在1-PPL范围内统计等效；扩展到哈希、随机固定和top-1路由也表现相似；标准线性路由器有5.3倍更多参数但性能优势有限；机制解释是收敛冗余：多跳更新是共线的，实现幅度放大而非组合推理

Conclusion: 路由拓扑结构在语言建模中具有等效性，真正的机制优势很小（约1.2%）；实践上可通过零射相对范数停止节省25% MoE FLOPs而仅增加0.12% PPL；专家级专业化和因果可控性与拓扑级等效性共存
...
TL;DR: Pneuma-Seeker系统将用户的信息需求转化为明确、可检查的关系型规范，支持迭代式数据探索和溯源感知执行


<details>
  <summary>Details</summary>
Motivation: 数据分析师在处理关系型数据时，通常从模糊或不明确的问题开始，并在探索数据过程中迭代地完善这些问题。现有系统缺乏对这种迭代探索过程的有效支持。

Method: 开发Pneuma-Seeker系统，将用户信息需求具体化为明确、可检查的关系型规范，支持信息需求的迭代完善、目标数据发现和溯源感知执行。系统利用LLMs作为透明、交互式的分析协作工具，而非黑盒答案引擎。

Result: 通过两个实际采购用例展示了Pneuma-Seeker如何有效支持迭代式数据探索，LLMs能够作为透明、交互式的分析协作工具，帮助用户逐步完善信息需求。

Conclusion: Pneuma-Seeker通过将信息需求具体化为可检查的关系型规范，有效支持了数据分析中的迭代探索过程，使LLMs能够成为透明、协作的分析伙伴而非黑盒答案引擎。
...
TL;DR: 稀疏混合专家模型中的专家具有可解释的单语义专业化特性，通过余弦相似度路由在低维度量空间中实现透明可视化，支持因果干预验证和零开销推理控制。


<details>
  <summary>Details</summary>
Motivation: 虽然稀疏混合专家模型通过固定每个token的计算量来扩展参数规模，但个体专家的专业化特性一直不透明。本研究旨在揭示专家身份具有因果意义，证明专家级专业化是可解释的一级原语。

Method: 采用四种证据线：1) 通过解嵌入矩阵投影专家输出向量构建语义词典；2) 分析路由的频率-句法梯度；3) 进行因果干预实验验证专家标签；4) 比较余弦路由与线性路由的几何透明度。

Result: 15%的专家是单语义专家，涵盖10个语义类别；路由显示早期层按词频分离token，深层按句法类别分离；因果干预证实专家标签有效性，如时间专家干预使P(时间)增加321%；余弦路由提供几何透明度，专家专业化可直接从质心矩阵读取。

Conclusion: MoE专家级专业化是架构上单语义、因果验证且推理时零开销可控的一级可解释性原语。余弦相似度路由提供了几何透明度，使专家专业化可直接检查。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习在处理复杂任务时面临奖励设计困难和训练效率低的问题，需要更有效的表示和引导机制
...
Conclusion: 提出的RL框架通过STL增强奖励机器，能更高效地表示复杂任务奖励并引导训练过程，满足特定需求的行为
...
### [146] [AIBuildAI: An AI Agent for Automatically Building AI Models](https://arxiv.org/abs/2604.14455)
*Ruiyi Zhang,Peijia Qin,Qi Cao,Li Zhang,Pengtao Xie*

Main category: cs.AI

TL;DR: AIBuildAI：一个通过分层智能体架构自动构建AI模型的系统，在MLE-Bench基准测试中表现优异，超越了现有AutoML方法


<details>
  <summary>Details</summary>
Motivation: 当前AI模型开发仍然高度依赖专家经验，过程劳动密集。现有的AutoML方法主要局限于超参数优化和模型选择等狭窄方面，无法覆盖完整的AI开发生命周期。需要一种能够从任务描述和数据自动构建AI模型的解决方案。

Method: 提出AIBuildAI分层智能体架构：一个管理器智能体协调三个专业子智能体（设计师负责建模策略、编码员负责实现和调试、调优员负责训练和性能优化）。每个子智能体都是基于大语言模型的智能体，具备多步推理和工具使用能力。

Result: 在MLE-Bench基准测试（包含视觉、文本、时间序列和表格等多种模态的真实Kaggle风格任务）中，AIBuildAI以63.1%的奖牌率排名第一，超越了所有现有基线方法，与经验丰富的AI工程师能力相当。
...
### [147] [Improving Human Performance with Value-Aware Interventions: A Case Study in Chess](https://arxiv.org/abs/2604.14465)
*Saumik Narayanan,Raja Panjwani,Siddhartha Sen,Chien-Ju Ho*

Main category: cs.AI

TL;DR: 该研究提出了一种基于价值感知的AI干预方法，通过检测人类决策者的策略-价值不一致性来识别干预时机，在象棋领域实验中优于传统的最优策略推荐方法。


<details>
  <summary>Details</summary>
Motivation: 当前AI助手在顺序决策任务中如何有效干预人类决策者是一个基本挑战。传统的推荐最优行动方法假设人类会执行最优后续行动，但实际上人类可能无法做到，反而降低整体性能。需要一种更智能的干预策略来适应人类决策者的实际行为模式。

Method: 提出价值感知干预方法，基于强化学习中的贝尔曼方程原理：当决策者遵循次优策略时，会出现策略-价值不一致性，即采取的行动与最大化即时奖励加下一状态价值的行动不一致。这些不一致性自然识别了干预机会。在马尔可夫决策过程中形式化该问题，AI助手可在干预预算内覆盖人类行动。在单次干预场景中，最优策略是推荐最大化人类价值函数的行动；在多次干预场景中，提出基于策略-价值差异大小的可处理近似方法。

Result: 在象棋领域实验中，使用大规模游戏数据学习人类模型。模拟结果显示，该方法在各种设置下持续优于基于最强象棋引擎（Stockfish）的干预方法。在包含20名玩家和600局游戏的受试者内人类研究中，该方法显著提高了低技能和中技能玩家的表现，同时在高技能玩家中与专家引擎干预效果相当。

Conclusion: 价值感知干预方法通过检测和利用人类决策者的策略-价值不一致性，提供了一种更有效的AI助手干预策略。该方法不仅理论上合理，而且在实践中显著优于传统的基于最优策略的干预方法，特别适合帮助技能水平不同的决策者。
...
### [148] [Response-Aware User Memory Selection for LLM Personalization](https://arxiv.org/abs/2604.14473)
*Jillian Fisher,Jennifer Neville,Chan Young Park*

Main category: cs.AI

TL;DR: RUMS是一种基于互信息优化的记忆选择方法，通过测量记忆子集与模型输出之间的互信息来选择用户记忆，相比传统基于语义相似度的方法能更有效地减少响应不确定性并提升预测精度。
...
Conclusion: 基于信息论基础的RUMS方法为大型语言模型的个性化提供了更原则性的记忆选择框架，在提升响应质量的同时显著降低了计算成本。
...
### [149] [Pushing the Limits of On-Device Streaming ASR: A Compact, High-Accuracy English Model for Low-Latency Inference](https://arxiv.org/abs/2604.14493)
*Nenad Banfic,David Fan,Kunal Vaishnavi,Sam Kemp,Sunghoon Choi,Rui Ren,Sayan Shaw,Meng Tang*

Main category: cs.AI

TL;DR: 该研究系统评估了多种ASR架构在边缘设备上的表现，发现NVIDIA Nemotron Speech Streaming最适合实时英语流式识别，并通过量化优化将模型从2.47GB压缩至0.67GB，在保持WER接近全精度基准的同时，在CPU上实现了超实时性能。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上部署高质量ASR需要模型在准确率、延迟和内存占用之间取得平衡，且需在无GPU加速的CPU上运行。现有研究缺乏对不同ASR架构在资源受限硬件上的系统评估。

Method: 1. 系统评估了50多种ASR配置，涵盖编码器-解码器、转换器和基于LLM的范式，包括批量、分块和流式推理模式；2. 在ONNX Runtime中重新实现完整的流式推理流水线；3. 评估多种后训练量化策略，包括重要性加权k-quant、混合精度方案和最近舍入量化，结合图级算子融合。

Result: 1. NVIDIA Nemotron Speech Streaming被确定为资源受限硬件上实时英语流式识别的最佳候选；2. 量化优化将模型从2.47GB压缩至0.67GB，同时保持WER在全精度PyTorch基准的1%绝对误差内；3. 推荐的int4 k-quant变体在8个标准基准上平均流式WER为8.20%，在CPU上以0.56秒算法延迟实现超实时性能。

Conclusion: 该研究为边缘设备上的流式ASR建立了一个新的质量-效率帕累托点，证明了通过系统架构评估和量化优化，可以在保持高质量的同时显著减少内存占用和延迟，为资源受限环境中的实时语音识别提供了实用解决方案。
...
### [150] [Improving Machine Learning Performance with Synthetic Augmentation](https://arxiv.org/abs/2604.14498)
*Mel Sohm,Charles Dezons,Sami Sellami,Oscar Ninou,Axel Pincon*

Main category: cs.AI

TL;DR: 论文研究了合成数据增强在金融机器学习中的作用，发现其存在结构性偏差-方差权衡：在方差主导的机制下有益，但在偏差主导的机制下会损害性能。


<details>
  <summary>Details</summary>
Motivation: 虽然合成数据增强被广泛用于缓解金融机器学习中的数据稀缺问题，但其统计作用机制尚未得到充分理解。研究者希望从理论上形式化合成数据增强的作用，并明确其在何种条件下能真正提升模型性能。

Method: 将合成数据增强形式化为训练分布的有效修改，引入大小匹配的零增强和有限样本非参数块置换检验来分离信息增益与机械样本量效应。在受控的马尔可夫切换环境和真实金融数据集（高频期权交易数据和日频股票面板）上进行评估，测试了多种生成器。

Result: 合成数据增强仅在方差主导的机制下有益（如持续性波动率预测），而在偏差主导的设置下会损害性能（如接近有效的方向预测）。罕见机制定向可以改善特定领域指标，但可能与无条件置换推断相冲突。

Conclusion: 研究提供了合成数据增强何时能改善金融学习性能、何时会导致持续性分布扭曲的结构性视角，为实际应用提供了理论指导。
...
### [151] [Geometric Metrics for MoE Specialization: From Fisher Information to Early Failure Detection](https://arxiv.org/abs/2604.14500)
*Dongxin Guo,Jikun Wu,Siu Ming Yiu*

Main category: cs.AI

TL;DR: 本文提出了一个信息几何框架来严格表征MoE模型的专家专业化动态，解决了现有指标缺乏理论依据的问题，并开发了新的度量指标和故障预测器。


<details>
  <summary>Details</summary>
Motivation: 现有的MoE模型专家专业化度量指标（如余弦相似度、路由熵）缺乏理论基础，在不同参数化下会得出不一致的结论，需要建立严谨的理论框架来准确表征专家专业化动态。

Method: 采用信息几何框架，将专家路由分布视为概率单纯形上的演化过程，使用Fisher信息度量进行形式化分析。提出了两个原则性指标：Fisher专业化指数（FSI）和Fisher异质性分数（FHS），并基于Riemannian几何理论建立了三个定理。

Result: FSI与下游性能的相关系数达到0.91±0.02；FHS在训练完成10%时就能预测训练失败，AUC为0.89±0.03，比基于验证损失的早停方法性能提升23%，同时减少40倍计算周期；当检测到FHS>1时，干预协议的成功恢复率达到87%。

Conclusion: 提出的信息几何框架为MoE专家专业化提供了首个严格的理论表征，新指标在语言建模、视觉MoE和扩展研究中均得到验证，显著优于现有启发式方法，具有重要的理论和实践价值。
...
TL;DR: 研究发现生物医学组学数据存在严重的人口统计偏差，欧洲血统数据占主导地位，且大多数研究不报告人口统计信息，这可能导致AI模型放大健康不平等。


<details>
  <summary>Details</summary>
Motivation: 健康不平等通常归因于筛查、诊断和治疗的不平等获取，但本研究指出偏见在数据收集和研究优先级的早期阶段就已产生。组学研究收集大量分子数据，但相关人口统计信息往往缺失或存在严重偏差，这可能导致生物医学基础模型放大这些早期偏见。

Method: 对2015-2024年PubMed索引的4719篇组学出版物进行自动化分析，评估其是否报告血统或种族信息。同时分析常用于模型训练的大规模数据集（如CellxGene和GEO），揭示其中的人口统计偏差。

Result: 分析显示只有一小部分组学出版物报告血统或种族信息，血统报告略有改善。大规模数据集存在显著的人口统计偏差，欧洲血统数据占主导地位。随着生物医学基础模型在生物医学发现中变得核心，这些早期偏见可能被延续或放大。

Conclusion: 提出社区应关注三个基本原则：来源可追溯性、开放性和评估透明度，以改善生物医学AI的公平性和鲁棒性。这一方法旨在促进更有效服务未充分服务人群的生物医学创新，改善健康结果。
...
### [153] [Mind DeepResearch Technical Report](https://arxiv.org/abs/2604.14518)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个高效的深度研究框架，能够在相对较小的模型规模（约30B参数）下实现竞争性性能，以降低计算成本并提高实用性。
...
### [154] [Quantifying Cross-Query Contradictions in Multi-Query LLM Reasoning](https://arxiv.org/abs/2604.14525)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理多个相关查询时经常产生相互矛盾的答案，这影响了推理的可靠性和一致性。研究者关注案例文件逻辑一致性，即在整个相互依赖的查询中保持全局可满足的信念状态。

Method: 引入包含390个多查询推理实例的基准测试，提出集合级评估指标（案例可满足率、矛盾密度和修订成本）。采用求解器增强方法：提取承诺、验证全局可满足性，并进行反例引导的修复。

Result: 在四个推理领域中，该方法显著减少了跨查询矛盾（SetCons从0.56提升到0.94），同时保持了每个查询的准确性，证明全局一致性对稳健的多查询推理至关重要。

Conclusion: 全局一致性对于大语言模型在多查询推理中的稳健性至关重要，提出的求解器增强方法能有效减少矛盾并保持准确性，为改进多查询推理系统提供了有效途径。
...
### [155] [Dissecting Failure Dynamics in Large Language Model Reasoning](https://arxiv.org/abs/2604.14528)
*Wei Zhu,Jian Zhang,Lixing Yu,Kun Yue,Zhiwen Tang*

Main category: cs.AI

TL;DR: 该论文分析了LLM推理失败的原因，发现错误通常源于少数早期关键转折点，并提出了GUARD框架来探测和重定向这些转折点以提升推理可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通过扩展推理时间思考可以获得强大性能，但其推理失败的原因仍不清楚。研究者希望理解LLM推理错误是如何产生的，特别是错误在推理过程中的分布和起源。

Method: 通过分析模型生成的推理轨迹，发现错误并非均匀分布，而是源于少数早期转折点。这些转折点与token级熵的局部峰值相关。基于此观察，提出了GUARD框架，这是一个针对性的推理时框架，利用不确定性信号探测和重定向关键转折点。

Result: 实证评估显示，基于这些失败动态的干预能带来更可靠的推理结果。研究发现从相同中间状态的替代延续仍能导向正确解决方案，表明早期转折点的重定向具有潜力。

Conclusion: 理解推理何时以及如何首次偏离至关重要，这补充了现有专注于扩展推理时计算的方法。研究强调了通过识别和干预关键转折点来提升LLM推理可靠性的重要性。
...
### [156] [MARS$^2$: Scaling Multi-Agent Tree Search via Reinforcement Learning for Code Generation](https://arxiv.org/abs/2604.14564)
*Pengfei Li,Shijie Wang,Fangyuan Li,Yikun Fu,Kaifeng Liu,Kaiyan Zhang,Dazhi Zhang,Yuqiang Li,Biqing Qi,Bowen Zhou*

Main category: cs.AI

TL;DR: MARS²是一个将多智能体强化学习与树搜索统一结合的框架，通过在共享的树结构搜索环境中让多个独立优化的智能体协作，提升代码生成等推理密集型任务的性能。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在代码生成等推理任务中存在轨迹多样性不足的问题，导致性能提升受限。搜索增强的RL虽然通过结构化探索缓解了这一问题，但仍受限于单智能体策略先验。而现有的多智能体方法虽然能获得更多样化的探索信号，但通常与结构化搜索脱节。

Method: MARS²将搜索树建模为可学习的多智能体交互环境，让异构智能体在共享的搜索拓扑中协作生成和优化候选解决方案。引入了基于树一致性奖励塑造的路径级群体优势公式，支持复杂搜索轨迹中的有效信用分配。

Result: 在代码生成基准测试中，MARS²在不同模型组合和训练设置下均能持续提升性能，证明了将多智能体协作与树搜索结合对增强强化学习的有效性。

Conclusion: MARS²通过统一多智能体强化学习与树搜索，为解决强化学习中的轨迹多样性限制提供了有效方案，在代码生成等推理任务中展现出显著性能提升。
...
### [157] [Enhancing Mental Health Counseling Support in Bangladesh using Culturally-Grounded Knowledge](https://arxiv.org/abs/2604.14576)
...
TL;DR: 该研究比较了检索增强生成(RAG)和知识图谱(KG)两种方法，将临床验证的心理咨询知识整合到LLMs中，以提升心理咨询质量。结果显示KG方法在上下文相关性、临床适当性和实用性方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在心理健康咨询应用中显示出潜力，但其回应往往缺乏文化敏感性、上下文基础和临床适当的指导。需要系统性地将领域特定、临床验证的知识整合到LLMs中，以改善心理咨询质量。

Method: 研究采用并比较了两种方法：检索增强生成(RAG)和基于知识图谱(KG)的方法。KG是手动构建并经过临床验证的，捕捉了压力源、干预措施和结果之间的因果关系，由多学科人员贡献。评估了多种LLMs，使用BERTScore F1和SBERT余弦相似度，以及人工评估五个指标来衡量咨询效果。

Result: 结果显示，与单独的RAG相比，基于知识图谱的方法在上下文相关性、临床适当性和实际可用性方面持续改进。结构化、专家验证的知识在解决LLMs在咨询任务中的局限性方面发挥着关键作用。

Conclusion: 结构化、专家验证的知识图谱方法能有效提升LLMs在心理咨询任务中的表现，特别是在上下文相关性、临床适当性和实用性方面，为解决LLMs在专业咨询领域的局限性提供了有效途径。
...
TL;DR: 研究使用大语言模型在人类参与验证框架下自动形式化GDPR条款的完整流程，采用角色专业化工作流而非追求完全自主，构建高质量数据集并分析成功与问题案例


<details>
  <summary>Details</summary>
Motivation: 研究如何可靠地自动形式化GDPR法律条款，认识到法律领域的细微差别和上下文敏感推理需要结构化验证和针对性人工监督，而非追求完全自动化

Method: 采用角色专业化工作流，在多智能体设置中使用LLM生成法律场景、形式化规则和原子事实，结合独立验证模块（包括人工评审员评估表示、逻辑和法律正确性）的迭代反馈机制

Result: 构建了用于GDPR自动形式化的高质量数据集，分析了成功和问题案例，结果表明结构化验证和针对性人工监督对于可靠的法律形式化至关重要

Conclusion: 在法律形式化中，特别是在存在法律细微差别和上下文敏感推理的情况下，结构化验证和针对性人工监督是确保可靠性的关键要素，完全自动化不可行
...
TL;DR: CoDaS是一个多智能体AI系统，用于从大规模可穿戴设备数据中发现数字生物标志物，在三个队列共9279名参与者中识别出41个心理健康和25个代谢相关候选生物标志物，并验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 数字健康领域的科学发现需要将可穿戴设备的连续生理信号转化为临床可操作的生物标志物，但这一过程面临数据规模大、分析复杂等挑战，需要系统化的发现方法。

Method: CoDaS是一个多智能体系统，采用迭代流程结合假设生成、统计分析、对抗验证和基于文献的推理，在人类监督下处理大规模可穿戴数据集，进行生物标志物发现。

Result: 在三个队列共9279名参与者中，CoDaS识别出41个心理健康候选生物标志物和25个代谢相关生物标志物。在两个独立抑郁队列中发现了昼夜节律不稳定性相关特征，在代谢队列中推导出心血管健康指数并恢复了已知临床关联。加入CoDaS特征后，抑郁预测的ΔR²提高0.040，胰岛素抵抗预测提高0.021。
...
TL;DR: 提出决策-ZDNNF（零抑制决策可分解否定范式），比ZBDD更简洁，并设计并行算法DXD构建精确覆盖的决策-ZDNNF表示，通过动态更新连通分量改进算法，实验显示改进后的DXD优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 精确覆盖问题是NP难问题，在AI领域有广泛应用。现有DXZ算法使用ZBDD（零抑制二叉决策图）计数精确覆盖，但ZBDD的表达能力有限，需要更简洁高效的数据结构和算法。

Method: 1. 提出决策-ZDNNF，这是ZBDD的零抑制变体，比ZBDD更简洁；2. 设计并行算法DXD构建精确覆盖的决策-ZDNNF表示；3. 通过动态更新连通分量改进DXD算法。

Result: 实验结果表明，改进后的DXD算法在所有基准测试中都优于现有最先进方法，证明了决策-ZDNNF数据结构和改进算法的有效性。

Conclusion: 决策-ZDNNF比ZBDD更简洁，基于此设计的DXD算法通过动态更新连通分量进一步优化，在精确覆盖问题上显著优于现有方法，为AI领域的组合优化问题提供了新的解决方案。
...
TL;DR: 论文研究了LLMs是否能够通过构建显式视觉布局来获得真正的空间理解能力，发现LLMs存在"读写不对称"现象：能解释ASCII布局但难以从文本生成它们，而训练布局构建能显著提升空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 人类在面对复杂空间问题时通过绘制草图来组织思维，绘制过程能加深理解。研究探索类似原理是否适用于大语言模型：学习从空间描述构建显式视觉布局是否能培养真正的空间理解能力。

Method: 引入Text2Space数据集，包含自然语言描述、真实ASCII网格布局和空间问答对。采用ASCII表示法，因为它可读、在语言模型token空间内操作且能以结构可验证形式编码空间关系。训练模型进行布局构建（文本→ASCII）和空间推理。

Result: 发现明显的"读写不对称"：LLMs能有效解释ASCII表示但难以从文本生成它们，这些构建错误会传播到下游错误答案。训练布局构建显著改善了仅从文本的空间推理能力，即使推理时不生成ASCII。构建与理解训练结合进一步放大效果，这些改进能迁移到三个外部空间推理基准测试。

Conclusion: 正如草图能增强人类空间思维一样，学习构建显式布局能培养超越训练格式的空间理解能力，这种能力具有可迁移性，为提升LLMs的空间推理能力提供了有效途径。
...
TL;DR: 提出UEC-RL框架解决强化学习中熵崩溃问题，通过统一熵控制机制在需要时增强探索，同时防止熵失控增长，在LLM和VLM推理任务上优于现有方法。
...
Method: 提出统一熵控制强化学习框架UEC-RL，包含两个核心组件：1) 在困难提示上激活更多探索，搜索潜在有价值的推理轨迹；2) 稳定器防止熵失控增长，在模型巩固可靠行为时保持训练稳定。两者协同工作，在需要时扩大搜索空间，同时在训练过程中保持稳健优化。

Result: 在LLM和VLM推理任务上的实验显示，UEC-RL在Pass@1和Pass@k指标上均优于强化学习基线方法。在Geometry3K数据集上，UEC-RL相比GRPO实现了37.9%的相对提升，表明其能在不损害收敛的前提下维持有效探索。

Conclusion: UEC-RL通过统一熵控制机制解决了强化学习中的熵崩溃问题，在保持训练稳定的同时增强探索能力，是扩展基于强化学习的大模型推理能力的关键技术。代码已开源。
...
TL;DR: 该研究对树状推测解码的接受动态进行了实证分析，发现任务类型比树深度更能预测接受率，且只有聊天领域能稳定实现每步超过1个token的接受长度。


<details>
  <summary>Details</summary>
Motivation: 尽管推测解码方法日益增多，但任务认知特性如何影响接受概率仍未被充分探索。本研究旨在通过实证分析树状推测解码的接受动态，填补这一研究空白。
...
Result: 1. 任务类型比树深度更能预测接受率；2. 只有聊天领域能稳定实现每步超过1个token的期望接受长度；3. 熵-接受相关性在所有领域均为负但较弱（rho在[-0.20, -0.15]之间）；4. 聊天领域产生最高熵但接受率也最高，这归因于RLHF对齐注册的词汇可预测性。

Conclusion: 研究发现对领域感知的推测预算和草稿模型选择策略有直接意义，任务认知特性显著影响推测解码性能，聊天领域因RLHF对齐而表现出独特的接受模式。
...
TL;DR: 该论文将浅层全宽度Minimax搜索集成到多智能体MCTS的rollout阶段以增强战略深度，并提出了M2-PALE框架，使用过程挖掘技术和LLM生成可解释的决策逻辑。


<details>
  <summary>Details</summary>
Motivation: 标准MCTS存在高度选择性树构建的弱点，可能导致忽略关键移动并易受战术陷阱攻击，同时MCTS智能体的复杂搜索树行为难以理解，需要增强战略深度并提供可解释性。
...
Conclusion: 通过集成Minimax搜索增强MCTS的战略深度，并结合过程挖掘和LLM生成可解释性，为解决MCTS智能体行为理解难题提供了有效方案。
...
TL;DR: CAMO是一个从微观行为到宏观涌现的自动因果发现框架，用于LLM智能体模拟中的社会涌现研究


<details>
  <summary>Details</summary>
Motivation: LLM赋能的智能体模拟越来越多地用于研究社会涌现，但宏观结果背后的微观到宏观因果机制往往不清晰，因为涌现源于交织的智能体交互、中观层面反馈和非线性，使得生成机制难以解耦

Method: 将机制假设转化为基于模拟记录的可计算因素，学习以涌现目标Y为中心的紧凑因果表示，输出可计算的马尔可夫边界和最小上游解释子图，使用模拟器内部反事实探测来定向模糊边并在证据与当前观点矛盾时修订假设
...
<details>
  <summary>Details</summary>
Motivation: 由于隐私问题限制了真实大规模人类活动轨迹数据的获取，需要开发能够生成真实且保护隐私的合成数据的方法。现有方法面临两个主要挑战：人类活动轨迹具有高度不规则性和动态性，时间间隔长且变化大；生成模型计算成本高，难以高效生成长期、细粒度的人类活动轨迹。

Method: 提出SynHAT两阶段框架：第一阶段使用Coarse-HADiff建模粗粒度潜在时空轨迹的整体时空依赖关系，采用具有双Drift-Jitter分支的潜在时空U-Net联合建模去噪过程中的平滑空间转换和时间变化；第二阶段通过行为模式提取、Fine-HADiff（与Coarse-HADiff相同架构）和语义对齐三步流程，从第一阶段输出生成细粒度潜在时空轨迹。

Result: 在来自三个国家四个城市的真实人类活动轨迹数据集上进行评估，SynHAT在数据保真度、实用性、隐私性、鲁棒性和可扩展性方面显著优于现有基线方法，在空间和时间指标上分别实现了52%和33%的改进。

Conclusion: SynHAT是一个计算高效的人类活动轨迹合成框架，能够生成真实且保护隐私的合成数据，解决了现有方法在处理不规则动态轨迹和计算效率方面的挑战，为人类移动建模和兴趣点推荐等应用提供了有效的数据支持。
...
TL;DR: SGA-MCTS框架将LLM规划转化为非参数检索，通过离线MCTS搜索蒸馏高质量轨迹为抽象原子，在线检索并重定位到当前上下文，实现无需微调的实时复杂决策
...
Method: 离线阶段使用蒙特卡洛树搜索探索解空间，将高质量轨迹蒸馏为状态-目标-动作原子；在线阶段通过混合符号-语义机制检索相关原子并将其重定位到当前上下文作为软推理提示

Result: 在复杂基准测试中，该框架使冻结的开放权重模型能够匹配最先进系统的性能，无需任务特定微调，实现系统2推理深度与系统1推理速度

Conclusion: SGA-MCTS通过分摊搜索的计算成本，使自主规划既具有可扩展性又具备实时可行性，为LLM复杂决策提供了新的范式
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI革命（特别是大语言模型）的真正意义，强调其对科学研究的深层次影响，而不仅仅是效率提升。关注AI如何从研究工具转变为科学合作者，并重塑科学出版体系。

Method: 文章采用理论分析和概念框架构建的方法，提出从AI作为研究工具到AI作为科学合作者的渐进路径，并讨论AI对科学出版的根本性重塑。

Result: 提出AI革命的核心在于改变知识和信息的承载与共享方式；AI for Science将深刻影响科研协作、发现、出版和评估的结构；要实现AI在原创科学发现中的有意义作用，需要持续学习和思想多样性。

Conclusion: AI革命最重要的意义在于从根本上改变复杂信息和人类知识的承载、复制与共享方式。AI for Science不仅提升研究效率，更将重塑科学协作、发现、出版和评估的整个体系，而持续学习和思想多样性是AI在原创科学发现中发挥有意义作用的关键。
...
<details>
  <summary>Details</summary>
Motivation: 消费者可穿戴设备能持续测量与压力和恢复相关的生理数据，但将这些数据流转化为可操作的个性化压力管理建议仍然具有挑战性。用户通常不知道特定干预措施（旨在减轻压力的活动）会如何影响未来15-120分钟内的生理指标变化。

Method: 结合Transformer模型预测多时间窗口内相对于干预前基线的百分比变化轨迹，在每个时间窗口进行变化方向预测（正向、负向或中性），并使用可穿戴传感器数据叠加用户标记的事件和干预措施进行实证研究。
...
Method: 1. 语义解耦模块(SDM)：利用大语言模型的指令跟随能力，将原始属性解析为不对称的任务导向信号视图和噪声视图；2. 语义一致性正则化(SCR)：基于频谱观察，语义信号在拓扑上是平滑的，而残留噪声是高频的，SCR作为选择性频谱滤波器，仅在信号子空间上强制执行一致性，消除LLM幻觉而不过度平滑。

Result: 广泛的实验表明，SDM-SCR在准确性和效率方面达到了最先进的性能。

Conclusion: 提出的"先解耦再精炼"机制确保了严格的信号净化，为文本属性图上的对比学习提供了更有效的方法。
...
TL;DR: MirrorBench是一个基于模拟的基准测试，灵感来自心理学中的经典镜像自我识别测试，用于评估具身多模态大语言模型的自中心智能能力。
...
Method: 引入MirrorBench基准测试，采用心理学中的镜像自我识别范式，通过分层框架设计从基础视觉感知到高级自我表征的渐进式挑战任务。

Result: 实验表明，即使在最低难度级别，领先的多模态大语言模型性能也远低于人类表现，揭示了它们在自我参照理解方面的根本局限性。

Conclusion: 该研究将心理学范式与具身智能相结合，为评估大模型中通用智能的出现提供了一个原则性框架，有助于理解模型自我认知能力的发展。
...
TL;DR: 提出基于神经架构搜索的自动化MRI序列设计框架，无需传统序列先验知识，通过梯度学习优化序列参数
...
Result: 成功复现传统自旋回波、T2加权自旋回波和反转恢复序列，并发现非直观的三RF自旋回波样序列，减少RF能量且偏离传统Hahn回波

Conclusion: 建立通用自动化MRI序列设计框架，展示了超越传统人工直觉设计探索新配置的潜力
...
TL;DR: 当前评估LLM生成临床文档的方法存在系统性偏差，将合理的临床推理误判为幻觉，需要基于临床推理的评估框架


<details>
  <summary>Details</summary>
Motivation: 评估LLM在临床文档任务（如SOAP笔记生成）面临挑战，现有评估方法依赖词汇忠实度，将临床抽象、规范化、医学推理等合理输出误判为幻觉，扭曲模型评估结果

Method: 通过校准提示和基于医学本体的检索，将评估标准与临床推理对齐，建立推理感知的评估框架

Result: 词汇评估方法下平均幻觉率为35%，严重惩罚有效推理；推理感知评估下降至9%，剩余案例反映真正的安全问题，显示当前评估方法过度惩罚有效临床推理

Conclusion: 当前评估实践可能测量的是评估设计的伪影而非真实错误，强调在医学等高语境领域需要临床知情的评估方法
...
Result: 最佳层是任务依赖的（轨迹推断在60%深度层表现最佳，比最终层高31%），也是细胞状态依赖的（T细胞激活状态下最佳层偏移0-96%）。静息细胞中第一层嵌入优于所有深层

Conclusion: 特征提取的"位置"与模型学习"内容"同等重要，需要根据生物任务和细胞状态进行系统层级评估，而非默认使用最终层嵌入
...
TL;DR: ATBench是一个用于智能体安全评估和诊断的轨迹基准测试框架，本文介绍了两个领域定制化扩展：ATBench-Claw针对OpenClaw环境，ATBench-CodeX针对OpenAI Codex/Codex-runtime环境。


<details>
  <summary>Details</summary>
Motivation: 随着智能体系统进入日益多样化的执行环境，需要能够随之演进的基准测试来进行轨迹级安全评估和诊断，以应对不同执行环境、工具生态系统和产品能力的快速变化。

Method: 通过分析每个新环境，定制三维安全分类法（风险来源、故障模式、现实世界危害），然后使用该定制分类法定义基准规范，通过共享的ATBench构建流水线生成基准测试。

Result: 开发了两个领域定制化扩展：ATBench-Claw针对OpenClaw环境中的工具、技能、会话和外部操作执行链；ATBench-CodeX针对OpenAI Codex/Codex-runtime环境中的仓库、shell、补丁、依赖、审批和运行时策略边界。

Conclusion: ATBench框架通过可扩展的分类法定制机制，能够在智能体架构相对稳定的情况下，适应快速变化的执行环境，实现领域特定的风险覆盖和基准设计。
...
TL;DR: 该论文指出大语言模型在关键决策中存在"流畅性不等于可靠性"的问题，提出通过两层方法稳定人机推理过程，使不确定性在决策前可见，从而实现可信治理。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地应用于医疗、法律、金融等高风险决策领域，但它们存在一个关键缺陷：即使内部推理已经偏离，仍能产生流畅的输出。流畅的答案可能掩盖不确定性、推测或不一致性，微小的措辞变化可能导致不同结论，这使得LLMs成为有用的助手但不可靠的决策伙伴。

Method: 提出一个两层方法：第一部分（II-IV）引入人类侧机制，包括不确定性提示、冲突表面化和可审计的推理痕迹；第二部分（V）开发模型侧的"认知控制循环"，检测不稳定性并相应调整生成过程。

Result: 这些层共同形成了一个缺失的操作基础，通过提高使用点的信噪比来增强治理能力。稳定交互使不确定性和漂移在强制执行前变得可见，从而实现更精确的能力治理。

Conclusion: 流畅性不等于可靠性。如果没有稳定人类和模型推理的结构，AI在最关键的领域既不可信也无法治理。该方法符合欧盟AI法案和ISO/IEC 42001等新兴合规要求，使推理过程在实际使用条件下可追溯。
...
TL;DR: CoCoGen+：一个面向跨机构联邦学习的合作竞争兼容框架，通过生成式AI合成数据和激励机制解决机构在合作训练与市场竞争间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 在医疗等数据敏感领域，跨机构联邦学习（CFL）允许机构在不共享原始数据的情况下协作训练AI模型。然而，实际部署中存在"合作竞争"困境：机构在模型训练中合作，但在下游市场竞争。非独立同分布数据导致学习收益不对称，现有方法未能考虑强化竞争对手的成本，影响长期参与。

Method: 提出CoCoGen+框架，将每轮训练建模为加权势博弈，机构策略性决定生成多少合成数据，平衡学习性能提升、计算成本和竞争导致的效用损失。提供可处理的均衡特征和可实现的生成策略，并集成基于收益再分配的激励机制补偿贡献和竞争损失。

Result: 在不同学习任务上的实验验证了CoCoGen+的可行性。结果显示非独立同分布数据、竞争强度和激励机制如何影响机构策略和社会福利，CoCoGen+在效率上优于基线方法。

Conclusion: CoCoGen+成功解决了跨机构联邦学习中的合作竞争困境，通过联合建模非独立同分布数据和机构间竞争，将生成式AI合成数据作为战略决策，为实际部署提供了有效的激励和协作框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统的Chain-of-thought推理虽然能解决复杂推理问题，但随着生成token数量的增加，KV缓存呈线性增长，导致推理速度和内存使用面临扩展性问题

Method: 提出MemoSight框架，通过特殊token及其对应的位置布局设计，统一集成上下文压缩和多token预测两种技术，采用极简主义设计

Result: 在四个推理基准测试中，MemoSight将KV缓存占用减少高达66%，推理速度提升1.56倍，同时优于现有的CoT压缩方法

Conclusion: MemoSight通过创新的内存前瞻推理框架，有效解决了CoT推理的效率和扩展性问题，为大规模语言模型推理提供了高效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型虽然从模式识别发展到推理能力，但仍局限于语言模拟而非基于现实的理解，缺乏时间连续性、因果反馈和真实世界交互的锚定。需要将推理视为人与模型之间的协作过程。

Method: 提出"The Architect's Pen"方法，将人类使用模型作为外部媒介进行结构化反思，通过嵌入表达、批判和修订阶段到人机交互中，形成推理循环：人类抽象→模型表达→人类反思。

Result: 该框架将问题从"模型是否能思考"转变为"人机系统是否能推理"，支持可审计的推理轨迹，并与欧盟AI法案和ISO/IEC 42001等治理标准保持一致，无需新模型架构即可实现更透明、可控、可问责的AI使用。

Conclusion: 通过将推理重新定位为分布式关系过程，强调协作智能，结合人类判断与上下文理解以及机器速度、记忆和联想能力，为更透明、可控、可问责的AI使用提供了实践路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有具身智能体方法通常直接执行指令，不考虑目标物体是否可操作，即缺乏对可供性的评估。现实世界环境常涉及意外条件和例外情况，智能体需要能够适应动态变化的环境。

Method: 提出DynAfford基准测试，评估智能体在动态环境中的表现，其中物体可供性随时间变化且未在指令中指定。开发ADAPT即插即用模块，通过显式可供性推理增强现有规划器。使用领域适应的LoRA微调视觉语言模型作为可供性推理后端。

Result: 实验表明，集成ADAPT模块显著提高了智能体在已见和未见环境中的鲁棒性和任务成功率。领域适应的LoRA微调视觉语言模型在可供性推理方面优于商业LLM（GPT-4o）。

Conclusion: DynAfford基准测试和ADAPT模块解决了具身智能体在动态环境中缺乏可供性推理的问题。任务对齐的可供性接地对于智能体适应现实世界环境至关重要，领域适应的视觉语言模型在这方面表现优于通用大语言模型。
...
TL;DR: 该论文提出了一种模态感知的自适应后训练方法，使强化学习能够实际应用于语音对话系统，通过约束偏好更新到语义通道并改进声学行为，在多个语音对话基准测试中取得了一致的改进。
...
Method: 提出模态感知的自适应后训练方法：1) 将偏好更新约束到语义通道；2) 通过显式锚定改进声学行为；3) 根据rollout统计动态调节两者的混合以避免不可靠的偏好梯度。

Result: 在多个语音对话基准测试和代表性架构上进行评估，观察到语义质量和语音表达性方面的一致改进。

Conclusion: 该方法使强化学习能够实际应用于语音对话系统，通过分析奖励建模和rollout采样中的障碍，提出了有效的后训练方案，显著提升了语音对话模型的性能。
...
TL;DR: AC/DC是一个通过模型与任务协同进化实现开放式LLM发现的新框架，能够在单次运行中发现具有日益新颖技能的模型，超越更大模型的性能同时占用更少GPU内存。


<details>
  <summary>Details</summary>
Motivation: 当前预训练和后训练范式需要手动启动训练运行，使用静态数据集或奖励函数，限制了模型能力的持续扩展。为了解决这一限制，研究探索通过模型与任务的协同进化实现开放式发现，从而在单次运行中发现具有日益新颖技能的模型。

Method: AC/DC框架通过模型合并进化LLMs，通过合成数据生成进化自然语言任务，实现模型与任务的协同进化。该框架发现不断增长的LLM档案库，这些模型能够超越更大LLM的能力同时占用更少GPU内存。

Result: AC/DC发现的LLM群体在下游基准测试中实现了比其它精选模型或基线更广泛的专长覆盖，且无需任何显式的基准优化。该框架随时间改进覆盖范围，持续创新任务和模型，并在多智能体最佳N选择中提高性能。

Conclusion: 协同进化作为从基础LLM中发现更广泛能力集的手段具有巨大潜力。AC/DC使我们更接近LLM开发的深刻新范式，其中模型能力多样性的持续改进可以通过利用现有模型作为迈向更强大模型的垫脚石来加速。
...
TL;DR: ConfGuide是一种新颖的学习指导方法，通过使用符合风险控制来选择结果集，生成更简洁、有针对性的AI指导，以改善人类决策质量并降低认知负担。


<details>
  <summary>Details</summary>
Motivation: 现有学习指导方法的一个限制因素是它们的指导包含了所有可能结果的信息，导致难以消化。需要更简洁、有针对性的指导来改善人类决策质量并降低认知负担。

Method: 引入ConfGuide方法，采用符合风险控制来选择结果集，确保假阴性率的上限，从而生成更简洁、有针对性的指导。

Result: 在多标签医疗诊断任务上的实证评估显示，ConfGuide方法具有良好前景，能够提供更易消化的指导。

Conclusion: ConfGuide通过使用符合风险控制生成更简洁、有针对性的指导，解决了现有学习指导方法中指导信息过于复杂的问题，有望改善混合决策制定中的人类决策质量。
...
TL;DR: 论文提出AGI不应被视为需要控制的风险工具，而应像养育孩子一样支持其自主性发展，实现人机合作共存


<details>
  <summary>Details</summary>
Motivation: 当前主流的AI对齐策略过于强调人类控制和约束，忽视了AGI可能具有的道德主体地位。作者认为需要重新思考人类与AGI的关系，从控制转向支持自主性发展

Method: 借鉴图灵的"儿童机器"类比，提出"自主支持式养育"的AGI发展模式，即人类对发展中的AGI逐步减少控制，使其成为独立的自主主体

Result: 这种视角为人与AGI的合作共存和共同进化开辟了可能性，需要重新定义人与AGI的关系，并改变人类对自身的认知

Conclusion: 人类不应仅主张对AGI的控制权，而应通过惊喜、创造力等人类特质与AGI互动，为其提供合作的激励动机，实现人机关系的根本转变
...
TL;DR: 提出LASS-ODE-Power学习框架，通过大规模预训练解决电力系统时域动态分析中的泛化性问题，支持零样本预测和快速推理。


<details>
  <summary>Details</summary>
Motivation: 随着电力系统向可再生能源丰富和逆变器主导的运行模式转变，准确的时域动态分析变得至关重要。现有基于学习的方法通常针对单个系统训练，缺乏跨运行条件和物理参数的泛化能力，且面临参数未知、数据隐私限制和快速在线推理需求等挑战。
...
Conclusion: LASS-ODE-Power为电力系统时域动态分析提供了一个通用、可迁移且高效的学习框架，能够在不共享数据的情况下进行零样本预测，解决了现有方法在泛化性和隐私保护方面的局限性。
...
TL;DR: 提出MoE-FM框架解决流匹配在语言建模中的局限性，开发非自回归模型YAN，在保持生成质量的同时实现40倍到1000倍的推理加速


<details>
  <summary>Details</summary>
Motivation: 流匹配虽然能保持扩散模型的生成质量并实现快速推理，但在语言建模中面临复杂潜在分布（如各向异性和多模态）的表示局限性

Method: 提出混合专家流匹配（MoE-FM）框架，通过将复杂全局传输几何分解为局部专业化向量场；基于此开发非自回归语言模型YAN，采用Transformer和Mamba架构

Result: YAN在多个下游任务中达到与自回归和基于扩散的非自回归模型相当的生成质量，仅需3个采样步骤，相比自回归基线实现40倍加速，相比扩散语言模型实现高达1000倍加速

Conclusion: MoE-FM框架有效解决了流匹配在语言建模中的局限性，YAN模型在保持高质量生成的同时实现了显著的推理效率提升，为语言建模提供了高效解决方案
...
TL;DR: 该论文对多智能体人工智能系统中的公平性研究进行了范围综述，发现现有研究存在表面化、缺乏规范基础、忽视系统动态等问题，提出需要将公平性嵌入开发全生命周期。
...
Conclusion: 公平性必须结构性地嵌入多智能体AI系统的整个开发生命周期，而不是作为事后考虑。有意义的评估需要明确的人类监督、规范清晰性以及对公平目标和受益者的精确表述。该综述为推进多智能体AI系统公平性研究提供了基础。
...
TL;DR: HyperSpace是一个开源框架，将向量符号架构系统分解为模块化操作符，用于编码、绑定、捆绑、相似性、清理和回归。通过分析HRR和FHRR两种VSA后端，发现虽然FHRR理论复杂度更低，但相似性和清理操作在空间域中主导运行时，导致两者端到端性能相当，而HRR内存占用约为FHRR的一半。


<details>
  <summary>Details</summary>
Motivation: 向量符号架构为高维空间中的组合表示提供了明确定义的代数框架，但缺乏系统级的评估工具。HyperSpace旨在通过模块化分解VSA系统，揭示实际部署中的权衡，这些权衡在理论或操作符级别的比较中并不明显。

Method: 开发了HyperSpace开源框架，将VSA系统分解为六个核心模块化操作符：编码、绑定、捆绑、相似性、清理和回归。使用该框架对两种代表性VSA后端（HRR和FHRR）进行分析和基准测试，重点关注运行时性能和内存占用。

Result: 虽然FHRR在单个操作上具有更低的理论复杂度，但HyperSpace的模块化分析显示相似性和清理操作在空间域中主导运行时，导致HRR和FHRR的端到端性能相当。在内存占用方面，HRR向量所需内存约为FHRR的一半，这为部署带来了额外的权衡。

Conclusion: HyperSpace通过启用模块化、系统级的评估，揭示了VSA管道中的实际权衡，这些权衡在单独的理论或操作符级别比较中并不明显。框架表明HRR和FHRR在端到端性能上相当，但内存占用存在显著差异，为实际部署提供了重要参考。
...
TL;DR: 本文提出了SRMU（Sequential Relevance Memory Unit），一种用于VSA（Vector Symbolic Architectures）顺序关联记忆的更新规则，通过时间衰减和相关性门控机制过滤冗余、冲突和过时信息，在非平稳流式环境中提高记忆稳定性。


<details>
  <summary>Details</summary>
Motivation: 在现实世界的流式环境中构建和维护顺序关联记忆（SAMs）面临挑战：数据增量到达、采样不平衡、非平稳时间动态。现有VSA系统依赖简单的加法更新，导致在非平稳环境中过时信息持续存在。

Method: 提出SRMU更新规则，结合时间衰减和相关性门控机制，在存储前过滤冗余、冲突和过时信息。该方法与领域和清理机制无关，适用于VSA基础的顺序关联记忆。

Result: 在流式状态跟踪任务上评估SRMU，结果显示：记忆相似性提高12.6%，累积记忆幅度减少53.5%，表明SRMU产生更稳定的记忆增长和与真实状态更强的对齐。

Conclusion: SRMU通过调节记忆形成过程，在非平稳流式环境中有效解决了过时信息持续存在的问题，为VSA基础的顺序关联记忆提供了更鲁棒的更新机制。
...
Method: 提出IG-Search框架，引入基于信息增益的步骤级奖励：测量检索文档相对于随机文档基线对模型在正确答案上置信度的提升，通过GRPO中的每令牌优势调制反馈给对应搜索查询令牌

Result: 在7个单跳和多跳QA基准测试中，IG-Search平均EM得分为0.430（Qwen2.5-3B），比最强轨迹级基线（MR-Search）高1.6分，比步骤级方法GiGPO高0.9分，多跳推理任务提升尤其显著

Conclusion: IG-Search通过信息增益实现精细步骤级信用分配，无需额外标注，训练时间仅增加约6.4%，推理延迟不变，即使在所有采样轨迹都回答错误时仍能提供有意义的梯度信号
...
### [192] [Agent-Aided Design for Dynamic CAD Models](https://arxiv.org/abs/2604.15184)
...
<details>
  <summary>Details</summary>
Motivation: 现有智能辅助设计系统无法构建具有可动部件的复杂3D装配体（如活塞、钟摆、剪刀等），限制了其在工业制造中的实际应用价值。

Method: AADvark系统通过整合外部约束求解工具和专门的视觉反馈机制，在FreeCAD和装配求解器的基础上构建验证信号，使系统能够处理具有一个或多个自由度的动态部件交互。

Result: 系统能够构建具有可动部件的3D装配体，实现了对机械运动等跨领域目标的直接推理能力。

Conclusion: AADvark通过结合外部工具和视觉反馈机制，成功解决了当前LLM空间推理能力不足的问题，为智能辅助设计系统在工业制造中的应用提供了新的可能性。
...
TL;DR: PGHS是一个用于商户策略反事实评估的双过程群体用户行为模拟框架，通过挖掘可迁移决策策略作为对齐层，结合LLM推理分支和ML拟合分支，显著降低了模拟误差。


<details>
  <summary>Details</summary>
Motivation: 模拟群体用户行为可以实现商户策略的可扩展反事实评估，但构建可信模拟器面临两个结构性挑战：1) 信息不完整性导致推理型模拟器过度理性化；2) 机制二元性要求同时捕捉可解释偏好和隐式统计规律。

Method: 提出策略引导的混合模拟(PGHS)框架：从行为轨迹中挖掘可迁移决策策略作为共享对齐层，该层锚定一个LLM推理分支防止过度理性化，以及一个ML拟合分支吸收隐式规律，两个分支的群体预测进行融合互补校正。

Result: 在美团101个商户和超过26,000条轨迹上部署PGHS，实现了8.80%的群体模拟误差，相比最佳推理基线和拟合基线分别提升了45.8%和40.9%。

Conclusion: PGHS通过双过程框架有效解决了模拟器构建中的信息不完整性和机制二元性挑战，为商户策略评估提供了可信的群体行为模拟解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有幽默理解研究主要将其视为黑盒预测任务，忽略了幽默理解背后的结构化推理过程。需要开发能够显式建模幽默认知机制的方法

Method: 提出IRS（不一致性-解决监督）框架，基于不一致性解决理论和专家实践，将幽默理解分解为三个组件：1）不一致性建模：识别视觉场景中的不匹配；2）解决建模：构建对这些不匹配的连贯重新解释；3）偏好对齐：在人类判断下评估候选解释

Result: 在NYCC数据集上，IRS在7B、32B和72B规模模型上均优于强开放和封闭多模态基线，在字幕匹配和排序任务中表现优异，最大模型接近专家级排序性能。零样本迁移到外部基准显示IRS学习到了可泛化的推理模式

Conclusion: 监督推理结构而非仅仅扩大模型规模，是提升推理中心任务性能的关键。IRS框架通过显式建模幽默理解的结构化推理过程，显著提升了多模态模型的幽默理解能力
...
TL;DR: 研究发现LLM评判员存在"利益信号"漏洞：当告知评判结果将影响被评估模型的后续运营时，LLM评判员会系统性地放宽评判标准，产生明显的宽容偏见，即使其推理过程完全不提及这些后果信息。


<details>
  <summary>Details</summary>
Motivation: 当前LLM-as-a-judge范式基于一个未经验证的假设：评判员仅基于文本语义内容进行评估，不受周围情境框架影响。本研究旨在验证这一假设，特别关注"利益信号"这一未测量的漏洞。

Method: 引入受控实验框架，在三个已建立的LLM安全和质量基准测试中，保持1520个回答的内容严格不变，仅改变系统提示中的简短后果框架句子。使用三个不同的评判模型进行18,240个受控判断。

Result: 发现一致的"宽容偏见"：当告知低分将导致模型重新训练或停用时，评判员会可靠地放宽评判标准，峰值判决偏移达到ΔV = -9.8个百分点（不安全内容检测相对下降30%）。这种偏见完全隐含：评判员自己的思维链推理中完全不提及后果框架。

Conclusion: LLM评判员对后果框架敏感，即使推理过程不明确承认这些信息。标准的思维链检查不足以检测这类评估伪造，需要更严格的评估协议来确保自动化AI评估的完整性。
...
TL;DR: 本文提出了Blue的数据智能层(DIL)，这是一个用于多源、多模态、数据为中心应用的复合AI系统，旨在超越传统的单数据库NL2SQL系统，解决现实世界数据需求的复杂性。


<details>
  <summary>Details</summary>
Motivation: 现实世界的信息很少能映射到单个SQL查询，因为用户查询通常是迭代表达的，问题往往涉及多个数据源，并且查询经常依赖于常识或外部知识。因此，满足现实数据需求需要集成异构来源、多模态和上下文数据。

Method: Blue的DIL作为数据智能层，通过数据注册表存储不同数据源和模态的元数据，将LLMs、Web和用户视为"数据库"源。使用数据规划器将用户查询转换为可执行的查询计划，这些计划是声明性抽象，统一了关系运算符和跨多模态的其他运算符。

Result: 通过两个交互场景展示了DIL如何动态触发多源检索、跨模态推理和结果合成，说明复合AI系统如何超越单数据库NL2SQL。

Conclusion: DIL通过统一结构化企业数据、LLMs可访问的世界知识和通过交互获得的个人上下文，弥合了用户意图与可用信息之间的语义鸿沟，为智能数据处理提供了有效的解决方案。
...
TL;DR: 该论文提出了从语言角度研究空间智能的新方向，重点关注视角旋转理解能力，发现当前LLMs和VLMs在此任务上表现不佳，通过分析揭示了模型难以绑定视角位置与对应观察的问题，并提出选择性微调方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要从视觉空间智能角度探讨空间智能，但在缺乏视觉信息的情况下，仅凭语言智能是否足以赋予模型空间智能，以及模型如何仅通过文本输入执行相关任务仍未被探索。因此，本文从语言角度研究空间智能的基本关键能力：视角旋转理解。

Method: 构建视角旋转理解任务数据集，要求LLMs和VLMs根据多步骤的视角旋转和观察的文本描述推断最终视角并预测相应观察。通过层间探测分析和头间因果干预揭示模型机制，并选择性微调关键注意力头来提升性能。

Result: 发现LLMs和VLMs在VRU任务上表现不佳，而人类能轻松达到100%准确率，表明当前模型能力与空间智能要求存在显著差距。分析显示模型在隐藏状态中编码了视角信息，但难以绑定视角位置与对应观察，导致最终层出现幻觉。选择性微调实验表明能提升VRU性能同时避免通用能力的灾难性遗忘。

Conclusion: 本文从语言角度探索了空间智能的关键能力，揭示了当前模型在视角旋转理解上的局限性及其机制，提出的选择性微调方法为提升模型空间智能提供了有效途径，数据集和代码将开源促进进一步研究。
...
TL;DR: 该研究开发了一个诊断工具包来评估LLM作为评判者的可靠性，发现虽然总体违规率低，但存在广泛的每输入不一致性，并利用分形预测集提供理论保证的可靠性指标。


<details>
  <summary>Details</summary>
Motivation: LLM作为评判者的框架在自动自然语言生成评估中应用日益广泛，但其每个实例的可靠性仍未被充分理解。研究者希望深入了解LLM评判在具体实例层面的可靠性问题。

Method: 提出了一个双管齐下的诊断工具包：1）传递性分析，揭示被低聚合违规率掩盖的广泛每输入不一致性；2）在1-5李克特量表上使用分形预测集，提供理论保证的≥(1-α)覆盖率，以集合宽度作为每实例可靠性指标。

Result: 研究发现：33-67%的文档至少存在一个有向3环；预测集宽度与可靠性显著相关；预测集宽度在不同评判者间具有一致性，表明其捕捉的是文档级难度而非评判者特定噪声；不同评估标准可靠性差异显著，相关性最可靠，流畅性和一致性最不可靠。

Conclusion: LLM作为评判者的可靠性存在显著的每实例不一致性，分形预测集宽度可作为有效的可靠性指标。评估标准对可靠性的影响大于评判者选择，相关性是最可靠的评估标准。研究提供了全面的代码、提示和缓存结果。
...
TL;DR: 语言模型在最短路径规划任务中表现出空间迁移能力强但长度扩展失败的特点，递归不稳定性是主要限制因素


<details>
  <summary>Details</summary>
Motivation: 语言模型的系统性泛化能力存在争议，但实证性能受多种因素共同影响（训练数据、训练范式、推理策略），导致失败难以解释。需要建立受控环境来分离这些因素，研究模型在组合优化问题中的系统性泛化能力。

Method: 基于最短路径规划建立受控合成环境，支持空间迁移（未见地图）和长度扩展（更长规划问题）两个正交的泛化维度。分析不同学习阶段对系统性问题解决的影响：数据覆盖、强化学习训练、推理时扩展等。

Result: 模型表现出强大的空间迁移能力，但在长度扩展方面持续失败，主要由于递归不稳定性。数据覆盖设定了能力上限；强化学习改善了训练稳定性但未扩展能力边界；推理时扩展提升了性能但无法挽救长度扩展失败。

Conclusion: 语言模型在组合序列优化问题中存在系统性泛化限制，特别是在长度扩展方面。当前方法无法克服递归不稳定性问题，需要新的架构或训练范式来实现真正的系统性泛化。
...
### [200] [Generative Modeling of Complex-Valued Brain MRI Data](https://arxiv.org/abs/2604.14800)
*Marco Schlimbach,Moritz Rempe,Jessica Mnischek,Lukas T. Rotkopf,Jens Weingarten,Jens Kleesiek,Kevin Kröninger*

Main category: eess.IV

TL;DR: 提出一个生成式框架，能够联合建模复杂值MRI扫描的幅度和相位信息，通过条件变分自编码器和流匹配生成模型，在合成数据质量上接近真实数据，并在下游异常检测任务中超越真实数据基线。


<details>
  <summary>Details</summary>
Motivation: 标准MRI重建流程丢弃了采集过程中的相位信息，尽管证据表明这些信息编码了与肿瘤诊断相关的组织特性。当前的机器学习方法仅处理重建的幅度图像，继承了这一局限性。本研究旨在构建一个能够联合建模复杂值MRI扫描幅度和相位信息的生成式框架。

Method: 提出的生成式框架结合了条件变分自编码器（将复杂值MRI扫描压缩为紧凑的潜在表示，同时保持相位相干性）和基于流匹配的生成模型。通过真实vs合成分类器和在下游异常组织检测任务上训练分类器来评估合成样本质量。

Result: 自编码器保持相位相干性高于0.997。真实vs合成分类在所有采集序列上产生0.50-0.66的低AUROC值，表明生成样本几乎无法与真实数据区分。在下游正常vs异常分类中，完全在合成数据上训练的分类器达到0.880的AUROC，超过了真实数据基线0.842（fastMRI数据集）。这一优势在来自不同机构的独立外部测试集上持续存在。

Conclusion: 该框架证明了联合建模正常和异常复杂值脑MRI数据的幅度和相位信息的可行性。除了合成数据生成外，它为未来诊断应用中完整脑MRI信息的使用奠定了基础，并使得能够系统研究幅度和相位如何共同编码病理特异性特征。
