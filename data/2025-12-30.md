...
Motivation: 人工智能在医疗领域的快速发展为改进医疗流程（包括医疗账单和转录）创造了机会。当前医疗数据处理存在高维度、复杂性等问题，需要更准确高效的解决方案来减少人为错误、简化操作流程。

Method: 开发了AI增强的局部线性嵌入（LLE）模型，专门用于处理高维医疗数据。该方法将AI技术与LLE降维技术相结合，提供了完整的数学模型，并通过一系列实验验证了其在真实医疗场景中的应用。

Result: 实验结果表明，该模型在数据处理准确性和操作效率方面都有显著提升，能够实现更快速、更准确的病人护理文档记录和财务交易处理。

Conclusion: AI增强的LLE模型在医疗数据分析中具有巨大潜力，不仅为医疗账单和转录服务提供了有效的解决方案，也为未来更广泛的医疗应用研究奠定了基础。
...
### [2] [Towards Signboard-Oriented Visual Question Answering: ViSignVQA Dataset, Method and Benchmark](https://arxiv.org/abs/2512.22218)
*Hieu Minh Nguyen,Tam Le-Thanh Dang,Kiet Van Nguyen*

Main category: cs.CV

TL;DR: ViSignVQA是首个越南语标牌视觉问答大规模数据集，包含10,762张图片和25,573个问答对，通过集成OCR和越南语预训练语言模型提升标牌文本理解能力。


<details>
  <summary>Details</summary>
Motivation: 自然场景中标牌文本理解对VQA实际应用至关重要，但在低资源语言（如越南语）中研究不足。现有VQA数据集缺乏针对越南语标牌的多模态资源，需要专门数据集来捕捉越南标牌的语言、文化和视觉特征。

Method: 1. 创建ViSignVQA数据集：包含10,762张越南标牌图像和25,573个问答对，涵盖双语文本、非正式表达、颜色布局等特征；2. 基准测试：集成越南OCR模型（SwinTextSpotter）和越南预训练语言模型（ViT5）到SOTA VQA模型（BLIP-2、LaTr等）；3. 提出多智能体VQA框架：结合感知和推理智能体与GPT-4，采用多数投票策略。

Result: 1. OCR增强上下文显著提升性能：将OCR文本附加到问题中，F1分数最高提升209%；2. 多智能体框架达到75.98%准确率；3. 展示了领域特定资源对低资源语言文本VQA的重要性。

Conclusion: ViSignVQA是首个越南语标牌理解的大规模多模态数据集，为OCR集成VQA模型的开发和评估提供基准，强调了领域特定资源对提升低资源语言文本VQA能力的重要性。
...
TL;DR: VISTA框架通过感知与推理解耦，使用信息瓶颈限制视觉语言模型仅提供客观感知信息，再由纯文本LLM进行推理，显著提升了视觉问答对虚假相关性的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 端到端视觉语言模型在回答视觉问题时容易利用虚假相关性而非因果视觉证据，特别是在微调后更容易产生捷径依赖，需要提升视觉推理的鲁棒性。

Method: 提出VISTA框架：1) 冻结VLM传感器仅提供简短客观的感知查询；2) 纯文本LLM分解问题、规划查询、聚合视觉事实；3) 通过强化学习训练无偏视觉推理；使用Qwen2.5-VL和Llama3.2-Vision作为传感器，仅用641个多步问题训练。

Result: 在SpuriVerse上显著提升鲁棒性（Qwen-2.5-VL-7B提升16.29%，Llama-3.2-Vision-11B提升6.77%），在MMVP和SeedBench子集保持竞争力；能跨VLM传感器迁移，识别并恢复感知失败；人类分析显示推理轨迹更中立、更少依赖虚假属性、更明确基于视觉证据。

Conclusion: VISTA通过感知-推理解耦和信息瓶颈设计，有效减少了视觉问答中对虚假相关性的依赖，提升了视觉推理的鲁棒性和可解释性，为构建更可靠的视觉语言系统提供了新思路。
...
### [4] [Bridging Your Imagination with Audio-Video Generation via a Unified Director](https://arxiv.org/abs/2512.23222)
...
Method: 采用Mixture-of-Transformers架构统一文本和图像生成。引入"先交错，后解耦"的训练范式：1)交错概念学习，使用交错文本-图像数据培养模型对剧本的深度理解和想象力解释；2)解耦专家学习，将剧本写作与关键帧生成解耦，提高故事叙述的灵活性和创造力。

Result: 大量实验表明，UniMAGE在开源模型中实现了最先进的性能，能够生成逻辑连贯的视频剧本和视觉一致的关键帧图像。

Conclusion: UniMAGE成功地将剧本创作和关键镜头设计统一到单一框架中，为非专业人士提供了制作长上下文、多镜头电影的能力，证明了统一导演模型的有效性。
...
TL;DR: HookMIL是一个用于全切片图像弱监督分析的多实例学习框架，通过可学习的hook tokens实现结构化上下文聚合，具有线性复杂度，在计算效率和性能上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统MIL方法在处理全切片图像时会丢失关键上下文信息，而基于transformer的变体虽然表达能力更强，但存在二次复杂度和冗余计算的问题，需要更高效且保留上下文的方法。

Method: 提出HookMIL框架，使用可学习的hook tokens进行结构化上下文聚合。这些tokens可以通过三种方式初始化：(1)关键patch视觉特征，(2)视觉语言病理学模型的文本嵌入，(3)空间转录组-视觉模型的空间特征。通过双向注意力机制与实例交互，引入Hook多样性损失促进专业化，并采用hook-to-hook通信机制优化上下文交互。

Result: 在四个公共病理数据集上的实验表明，HookMIL实现了最先进的性能，同时提高了计算效率和可解释性。

Conclusion: HookMIL是一个上下文感知且计算高效的多实例学习框架，通过多模态初始化的hook tokens有效整合文本和空间先验知识，在计算病理学中具有优越的性能和实用性。
...
### [6] [RealX3D: A Physically-Degraded 3D Benchmark for Multi-view Visual Restoration and Reconstruction](https://arxiv.org/abs/2512.23437)
...
TL;DR: RealX3D是一个真实捕获的基准数据集，用于评估多视角视觉恢复和3D重建在各种物理退化条件下的性能，包含四种退化类型和多个严重级别。


<details>
  <summary>Details</summary>
Motivation: 当前多视角重建方法在真实世界具有挑战性的环境中表现脆弱，需要评估它们在物理退化条件下的鲁棒性。

Method: 创建了RealX3D基准数据集，将退化分为四类（光照、散射、遮挡、模糊），采用统一的采集协议捕获像素对齐的低质量/高质量图像对，包含高分辨率图像、RAW数据和激光扫描数据。

Result: 对基于优化的和前馈方法的广泛基准测试显示，在物理退化条件下重建质量显著下降，突显了当前多视角流程在真实挑战性环境中的脆弱性。
...
TL;DR: Tiny-YOLOSAM：结合YOLO检测器和TinySAM的混合流水线，通过YOLO生成目标框提示和稀疏点提示，显著提升分割覆盖率和速度，替代密集的"分割一切"模式。


<details>
  <summary>Details</summary>
Motivation: SAM模型计算成本高，TinySAM虽然轻量化但"分割一切"模式仍需要数百个提示且速度慢，需要更高效的实时分割解决方案。

Method: 提出Tiny-YOLOSAM混合流水线：1）使用YOLOv12检测器生成显著前景目标的框提示；2）在YOLO未覆盖区域补充稀疏点提示；3）结合TinySAM进行分割。
...
### [8] [Real-Time In-Cabin Driver Behavior Recognition on Low-Cost Edge Hardware](https://arxiv.org/abs/2512.22298)
*Vesal Ahsani,Babak Hossein Khalaj*

Main category: cs.CV

TL;DR: 开发了用于低成本边缘设备的单摄像头驾驶员行为识别系统，支持17种行为检测，在Raspberry Pi 5上达到16FPS，在Coral Edge TPU上达到25FPS，实现实时监控。


<details>
  <summary>Details</summary>
Motivation: 车载驾驶员监控系统需要在计算、功耗和成本严格受限的条件下，低延迟识别驾驶员分心和疲劳相关行为，以满足实际部署需求。

Method: 采用单摄像头系统，结合紧凑的逐帧视觉模型、减少视觉相似误报的混淆感知标签设计，以及基于置信度和持续性的时态决策头来触发警报。

Result: 系统覆盖17种行为类别，在Raspberry Pi 5上使用INT8推理达到约16FPS（每帧延迟低于60ms），在Coral Edge TPU上达到约25FPS，支持实时监控和稳定警报生成。

Conclusion: 可靠的车内人员状态感知可以作为以人为中心的车辆智能（包括新兴的自主车辆概念）的上游输入，在低成本硬件上实现实时驾驶员监控。
...
TL;DR: 提出一种基于视觉语言模型的多模态可解释性方法，通过分析视网膜象限内的病变分布来模拟眼科医生的诊断推理，生成Grad-CAM热图可视化DR严重程度分类依据


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力丧失的主要原因，需要早期检测。医生需要能解释分类推理的模型，而不仅仅是定位病变。当前模型依赖单一成像模态，可解释性有限

Method: 提出新颖的多模态可解释性模型，使用视觉语言模型配合少样本学习，分析视网膜象限内的病变分布，生成成对的Grad-CAM热图展示OCT和眼底图像中影响DR严重程度分类的神经元权重

Result: 使用3000张眼底图像和1000张OCT图像的数据集，该方法解决了当前DR诊断的关键限制，为改善患者预后提供了实用全面的工具

Conclusion: 该多模态可解释性模型能够模拟眼科医生的诊断推理，通过可视化分析为DR严重程度分类提供解释，克服了当前模型的局限性，在筛查、治疗和研究环境中具有广泛应用潜力
...
### [10] [Improved cystic hygroma detection from prenatal imaging using ultrasound-specific self-supervised representation learning](https://arxiv.org/abs/2512.22730)
...
TL;DR: 本研究评估了超声特异性自监督预训练模型USF-MAE在早期妊娠超声图像中检测囊性水瘤的性能，结果显示USF-MAE在所有评估指标上均优于DenseNet-169基线模型。


<details>
  <summary>Details</summary>
Motivation: 囊性水瘤是高风险产前超声发现，与染色体异常、结构畸形和不良妊娠结局相关。自动检测可提高可重复性并支持可扩展的早期筛查，但监督深度学习受限于小规模标记数据集。

Method: 使用在37万张未标记超声图像上预训练的USF-MAE模型，针对正常对照组和囊性水瘤病例进行二元分类微调。采用与DenseNet-169基线相同的超声数据集、预处理流程和4折交叉验证协议进行评估，使用Score-CAM可视化分析模型可解释性。
...
Conclusion: 超声特异性自监督预训练能够促进准确、稳健的深度学习检测囊性水瘤，USF-MAE模型在性能上显著优于传统监督学习方法，为早期产前筛查提供了有效的自动化工具。
...
### [11] [Plug In, Grade Right: Psychology-Inspired AGIQA](https://arxiv.org/abs/2512.22780)
*Zhicheng Liao,Baoliang Chen,Hanwei Zhu,Lingyu Zhu,Shiqi Wang,Weisi Lin*

Main category: cs.CV

TL;DR: 提出基于心理测量学分级响应模型的AGIQA改进方法，通过双分支质量分级模块解决语义漂移问题，实现可解释的单峰质量分布
...
Method: 受心理测量学启发，提出改进的分级响应模型(GRM)，设计双分支质量分级模块：一个分支估计图像能力，另一个分支构建多个难度级别。为确保难度级别的单调性，采用算术方式建模难度生成，强制实现单峰且可解释的质量分布。该算术GRM质量分级(AGQG)模块具有即插即用优势

Result: AGQG模块集成到各种最先进的AGIQA框架中都能持续提升性能，同时在自然图像和屏幕内容图像质量评估中都能有效泛化，显示出作为未来IQA模型关键组件的潜力

Conclusion: 基于心理测量学GRM的AGQG模块有效解决了AGIQA中的语义漂移问题，通过算术方式强制单峰质量分布，提高了质量评估的可靠性和可解释性，具有广泛的适用性和泛化能力
...
### [12] [Hash Grid Feature Pruning](https://arxiv.org/abs/2512.22882)
...
### [13] [VLM-PAR: A Vision Language Model for Pedestrian Attribute Recognition](https://arxiv.org/abs/2512.22217)
*Abdellah Zakaria Sellam,Salah Eddine Bekhouche,Fadi Dornaika,Cosimo Distante,Abdenour Hadid*

Main category: cs.CV

TL;DR: VLM-PAR是一个基于冻结SigLIP 2多语言编码器的视觉语言框架，通过跨模态融合改进视觉特征，在行人属性识别任务中显著提升了性能，特别是在高度不平衡的PA100K数据集上取得了新的SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 行人属性识别面临严重类别不平衡、复杂属性依赖关系和领域偏移等挑战，需要更有效的解决方案来克服这些限制。

Method: 基于冻结的SigLIP 2多语言编码器构建模块化视觉语言框架，通过紧凑的跨注意力融合机制对齐图像和提示嵌入，精炼视觉特征。

Result: 在高度不平衡的PA100K基准测试中实现了显著准确率提升，达到新的SOTA性能；在PETA和Market-1501基准测试中也获得了显著的均值准确率提升。

Conclusion: 将大规模视觉语言预训练与针对性跨模态精炼相结合，能有效克服行人属性识别中的不平衡和泛化挑战，证明了这种方法的有效性。
...
TL;DR: 该论文提出语义抽象框架，利用2D视觉语言模型的激活图作为"抽象对象"表示，学习3D定位和补全隐藏物体，显著提高寻找效率


<details>
  <summary>Details</summary>
Motivation: 为家庭机器人提供寻找隐藏物体的技能，解决物体被遮挡时无法直接识别的问题，节省寻找时间和精力

Method: 利用2D VLM的激活图作为抽象对象表示，结合历史数据（物体常见放置位置）进行非结构化搜索，实现3D定位和补全

Result: 模型能准确识别隐藏物体的完整3D位置，首次尝试成功率显著高于随机搜索，搜索速度大幅提升

Conclusion: 语义抽象框架扩展了机器人寻找隐藏物体的能力，为家庭机器人节省时间和精力提供了有效解决方案
...
TL;DR: VideoScaffold是一个用于流媒体视频理解的动态表示框架，通过自适应调整事件粒度来提升多模态大语言模型对长视频的处理能力。


<details>
  <summary>Details</summary>
Motivation: 现有静态策略（如稀疏采样、帧压缩和聚类）在处理连续视频流时存在局限性，会产生碎片化或过度压缩的输出，无法满足流媒体视频理解的需求。

Method: 提出VideoScaffold框架，包含两个核心组件：弹性尺度事件分割（EES）进行预测引导的动态边界细化，以及分层事件整合（HEC）将语义相关片段逐步聚合成多层次抽象表示。

Result: 在离线和流媒体视频理解基准测试中，VideoScaffold实现了最先进的性能，框架具有模块化和即插即用特性。

Conclusion: VideoScaffold能够平滑地从细粒度帧理解过渡到抽象事件推理，有效扩展现有基于图像的多模态大语言模型到连续视频理解领域。
...
TL;DR: 该论文提出了一种KAN增强的FPN-Stem架构，用于改进ViT在姿态估计等密集预测任务中的性能。通过将FPN末端的标准线性3x3平滑卷积替换为KAN卷积层，显著提升了特征融合质量，在COCO数据集上实现了+2.0 AP的性能提升。


<details>
  <summary>Details</summary>
Motivation: ViT在密集预测任务中表现受限，主要原因是像ViTPose这样的模型采用了过于简单的前端设计。传统的patchification机制难以有效处理多尺度变化，并在初始特征提取阶段造成不可逆的信息损失。作者发现性能瓶颈不在于注意力模块，而在于FPN中的后融合非线性平滑步骤。

Method: 提出KAN增强的FPN-Stem架构：保留FPN经典的"上采样-相加"融合流程，但将其末端的标准线性3x3平滑卷积替换为基于KAN的卷积层。KAN层利用其优越的非线性建模能力，自适应学习和校正多尺度融合过程中产生的"伪影"。

Result: 在COCO数据集上的大量实验表明，KAN-FPN-Stem相比轻量级ViTPose-S基线实现了高达+2.0 AP的显著性能提升。该模块具有即插即用特性，且计算开销可控。

Conclusion: 这项工作不仅提供了一个高性能的即插即用模块，更重要的是揭示了ViT前端的性能瓶颈通常不在于"特征精炼"（注意力机制），而在于"特征融合"的质量。同时，通过引入KAN算子为解决这一瓶颈提供了有效路径。
...
<details>
  <summary>Details</summary>
Motivation: 低剂量PET成像在减少患者辐射暴露方面至关重要，但面临噪声干扰、对比度降低和生理细节难以保留等挑战。现有方法往往忽视投影域物理知识和患者特异性元信息，而这些对于功能语义相关性挖掘至关重要。

Method: 提出MiG-DM模型，包含：1）元信息编码模块，将临床参数转换为语义提示，考虑患者特征、剂量相关信息和半定量参数，实现文本元信息与图像重建的跨模态对齐；2）跨域架构结合投影域和图像域处理，在投影域使用专门的sinogram适配器通过卷积操作捕获全局物理结构。
...
### [18] [Evaluating an Adaptive Multispectral Turret System for Autonomous Tracking Across Variable Illumination Conditions](https://arxiv.org/abs/2512.22263)
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB检测在低光环境下表现不佳，热成像系统缺乏颜色和纹理信息。为了克服这些限制，需要一种能够适应不同光照条件的多模态融合方法，以增强自主机器人在应急服务中的视觉能力。

Method: 开发自适应框架，融合RGB和长波红外视频流，在11种融合比例（从100/0到0/100）上进行训练。训练了33个YOLO模型，使用超过22,000张标注图像，涵盖三种光照水平：无光、弱光和全光。

Result: 最佳全光模型（80/20 RGB-LWIR）和弱光模型（90/10融合）分别达到92.8%和92.0%的平均置信度，显著优于YOLOv5n和YOLOv11n基线。无光条件下，最佳40/60融合达到71.0%，超过基线但统计不显著。

Conclusion: 自适应RGB-LWIR融合提高了所有光照条件下的检测置信度和可靠性，增强了自主机器人视觉性能，为应急服务中的搜索救援和侦察任务提供了更强大的视觉支持。
...
### [19] [Human-Aligned Generative Perception: Bridging Psychophysics and Generative Models](https://arxiv.org/abs/2512.22272)
*Antara Titikhsha,Om Kulkarni,Dharun Muthaiah*

Main category: cs.CV

TL;DR: 通过使用在THINGS三元组数据集上训练的人类感知嵌入（HPE）教师模型作为外部指导信号，将几何理解引入文本到图像扩散模型，实现几何与风格的可控分离，提高语义对齐约80%。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然能生成高度详细的纹理，但主要依赖表面外观，难以遵循严格的几何约束，特别是当这些约束与文本提示所暗示的风格相冲突时。这反映了人类感知与当前生成模型之间的语义差距。

Method: 使用轻量级、现成的判别器作为外部指导信号，提出在THINGS三元组数据集上训练的人类感知嵌入（HPE）教师模型，通过将该教师模型的梯度注入潜在扩散过程，实现几何与风格的可控分离。在三种架构上评估：Stable Diffusion v1.5（U-Net骨干）、流匹配模型SiT-XL/2和扩散变换器PixArt-Σ。

Result: 实验表明，流模型在没有持续指导的情况下倾向于漂移回默认轨迹；实现了复杂三维形状（如Eames椅子）到冲突材料（如粉色金属）的零样本迁移；与无指导基线相比，指导生成将语义对齐提高了约80%。

Conclusion: 小型教师模型可以可靠地指导大型生成系统，实现更强的几何控制，拓宽文本到图像合成的创意范围，为解决几何约束与风格冲突问题提供了有效方法。
...
### [20] [GeCo: A Differentiable Geometric Consistency Metric for Video Generation](https://arxiv.org/abs/2512.22274)
...
Method: GeCo通过融合残差运动信息和深度先验，生成稠密的一致性图来检测几何变形和遮挡不一致伪影。该方法基于几何基础，能够提供可解释的检测结果。

Result: GeCo成功检测出视频生成模型中的常见故障模式，并作为训练自由指导损失有效减少了视频生成过程中的变形伪影，为模型评估提供了系统基准。

Conclusion: GeCo是一种有效的几何基础一致性度量工具，能够系统检测视频生成中的几何伪影，既可用于模型基准测试，也可作为指导损失来改进生成质量。
...
### [21] [The Illusion of Clinical Reasoning: A Benchmark Reveals the Pervasive Gap in Vision-Language Models for Clinical Competency](https://arxiv.org/abs/2512.22275)
*Dingyu Wang,Zimu Yuan,Jiajun Liu,Shanggui Liu,Nan Zhou,Tianxing Xu,Di Huang,Dong Jiang*

Main category: cs.CV

TL;DR: 该研究开发了B&J基准测试，评估AI模型在骨科临床推理中的表现，发现模型在结构化选择题上表现良好（>90%），但在需要多模态整合的开放任务中表现显著下降（<60%），表明当前AI模型尚不具备临床多模态推理能力。
...
Method: 开发了B&J基准测试框架，包含1,245个来自真实骨科和运动医学患者案例的问题，评估模型在7个反映临床推理路径的任务上的表现，包括知识回忆、文本和图像解释、诊断生成、治疗计划和理由提供。评估了11个视觉语言模型和6个大语言模型，并与专家提供的真实答案进行比较。

Result: 结果显示任务类型之间存在显著性能差距：最先进模型在结构化多选题上准确率超过90%，但在需要多模态整合的开放任务中准确率仅约60%。VLM在解释医学图像方面存在严重限制，经常出现严重的文本驱动幻觉，忽略矛盾的视觉证据。值得注意的是，专门为医学应用微调的模型相比通用模型没有一致优势。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决动态多模态理解中3D检索和4D生成的统一问题，需要建立跨模态的语义对齐机制，以支持大规模开放词汇检索和可控的4D资产生成。
...
### [23] [SpotEdit: Selective Region Editing in Diffusion Transformers](https://arxiv.org/abs/2512.22323)
...
Conclusion: SpotEdit提出了一种训练免费的扩散编辑框架，通过选择性区域更新解决了当前方法中的冗余计算问题，在保持编辑质量的同时显著提升了效率
...
### [24] [DeMoGen: Towards Decompositional Human Motion Generation with Energy-Based Diffusion Models](https://arxiv.org/abs/2512.22324)
*Jianrong Zhang,Hehe Fan,Yi Yang*

Main category: cs.CV

TL;DR: DeMoGen提出了一种基于能量扩散模型的组合训练范式，用于将整体运动分解为语义上有意义的子组件，并支持运动概念的灵活重组生成新运动。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注前向建模（如从文本到运动的整体映射），但缺乏将复杂运动分解为有意义子组件的能力。本文考虑逆向视角：将整体运动分解为语义上有意义的子组件。

Method: 提出DeMoGen组合训练范式，采用基于能量的扩散模型直接捕捉多个运动概念的组合分布。包含三种训练变体：1) DeMoGen-Exp：在分解的文本提示上显式训练；2) DeMoGen-OSS：正交自监督分解；3) DeMoGen-SC：强制原始和分解文本嵌入之间的语义一致性。

Result: 该方法能够从复杂运动序列中解耦可重用的运动基元，分解后的运动概念可以灵活重组生成多样化和新颖的运动，实现超出训练分布的泛化。同时构建了文本分解数据集支持组合训练。

Conclusion: DeMoGen提供了一种有效的运动分解学习框架，不仅能够分解复杂运动为语义子组件，还能支持运动概念的灵活重组生成，为文本到运动生成和运动组合提供了扩展资源。
...
### [25] [The Multi-View Paradigm Shift in MRI Radiomics: Predicting MGMT Methylation in Glioblastoma](https://arxiv.org/abs/2512.22331)
*Mariya Miteva,Maria Nisheva-Pavlova*

Main category: cs.CV

TL;DR: 该研究提出了一种基于变分自编码器的多视图潜在表示学习框架，用于从MRI图像中预测胶质母细胞瘤的MGMT启动子甲基化状态。


<details>
  <summary>Details</summary>
Motivation: 目前基于放射组学的机器学习方法在预测MGMT启动子甲基化状态时存在局限性：传统的单模态和早期融合方法受到高特征冗余和模态特异性信息建模不完整的限制。MGMT启动子甲基化对胶质母细胞瘤的预后和治疗具有重要意义，因此需要更有效的非侵入性预测方法。

Method: 提出了一个多视图潜在表示学习框架，基于变分自编码器整合来自T1Gd和FLAIR MRI的互补放射组学特征。每个模态通过独立的概率编码器编码，然后在紧凑的潜在空间中进行融合，既保留了模态特异性结构，又实现了有效的多模态整合。

Result: 该方法生成的潜在嵌入表示随后被用于MGMT启动子甲基化分类任务，相比传统方法能够更好地整合多模态信息并减少特征冗余。

Conclusion: 该多视图潜在表示学习框架为放射基因组学中的多模态整合提供了一种有效方法，能够更好地从医学影像中推断分子肿瘤特征，特别是在胶质母细胞瘤的MGMT启动子甲基化预测方面具有重要应用价值。
...
### [26] [Feature Learning with Multi-Stage Vision Transformers on Inter-Modality HER2 Status Scoring and Tumor Classification on Whole Slides](https://arxiv.org/abs/2512.22335)
*Olaide N. Oyelade,Oliver Hoxey,Yulia Humrye*

Main category: cs.CV

TL;DR: 该研究提出了一种基于视觉变换器的端到端管道，用于在H&E和IHC染色全切片图像上进行HER2状态评分，实现像素级定位和4级评分（0, 1+, 2+, 3+）


<details>
  <summary>Details</summary>
Motivation: 现有HER2评分方法难以提供像素级定位，且联合分析H&E和IHC图像具有挑战性。准确预测HER2蛋白表达水平对癌症治疗至关重要。

Method: 使用视觉变换器构建端到端管道：1) 对H&E全切片图像进行分块处理定位肿瘤区域；2) 提出新颖映射函数识别IHC图像中与H&E恶性区域对应的区域；3) 嵌入临床启发的HER2评分机制，实现4级自动像素级标注。

Result: 肿瘤定位分类准确率高；HER2状态预测准确率达0.94，特异性0.933；4级评分方法表现良好；与人类病理学家结果具有可比性。

Conclusion: 基于ViT的端到端模型能够有效联合评估H&E和IHC图像进行HER2评分，展示了在临床应用中联合使用两种染色图像的可行性。
...
### [27] [VULCAN: Tool-Augmented Multi Agents for Iterative 3D Object Arrangement](https://arxiv.org/abs/2512.22351)
*Zhengfei Kuang,Rui Lin,Long Zhao,Gordon Wetzstein,Saining Xie,Sanghyun Woo*

Main category: cs.CV

TL;DR: 本文提出了一种基于多模态大语言模型的3D场景操作框架，通过引入MCP-based API、专用视觉工具套件和多智能体协作架构，解决了3D物体排列任务中的视觉定位、场景理解和迭代更新三大挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在2D视觉语言任务上取得了显著进展，但在复杂3D场景操作方面的应用仍未被充分探索。现有MLLMs在3D物体排列任务中存在三大关键挑战：视觉定位能力弱、3D场景理解不足、迭代更新过程容易出错。

Method: 1. 引入MCP-based API，将交互从脆弱的原始代码操作转向更稳健的函数级更新，解决MLLMs视觉定位弱的问题；2. 通过专用视觉工具套件增强MLLM的3D场景理解能力，包括场景状态分析、空间信息收集和动作结果验证；3. 提出协作式多智能体框架，分配规划、执行和验证角色，以处理多步骤指令并从中间错误中恢复。

Result: 在25个复杂的物体排列任务上验证了方法的有效性，显著优于现有基线方法。

Conclusion: 该研究成功地将MLLMs应用于复杂3D场景操作，通过创新的API设计、视觉增强工具和多智能体协作框架，有效解决了3D物体排列任务中的关键挑战，为MLLMs在3D领域的应用开辟了新方向。
...
### [28] [Self-Evaluation Unlocks Any-Step Text-to-Image Generation](https://arxiv.org/abs/2512.22374)
*Xin Yu,Xiaojuan Qi,Zhengqi Li,Kai Zhang,Richard Zhang,Zhe Lin,Eli Shechtman,Tianyu Wang,Yotam Nitzan*

Main category: cs.CV

TL;DR: Self-E是一个从零开始训练的文本到图像生成模型，支持任意步数推理，通过自评估机制实现高效少步生成和高质量多步采样。
...
Method: Self-E采用从零开始的训练方法，类似流匹配模型学习数据分布，同时引入创新的自评估机制：模型使用当前的分数估计来评估自己生成的样本，作为动态的自教师。结合瞬时局部学习和自驱动的全局匹配。

Result: 在大规模文本到图像基准测试中，Self-E在少步生成方面表现出色，同时在50步时与最先进的流匹配模型竞争。性能随推理步数增加单调提升，支持超快速少步生成和高质量长轨迹采样。

Conclusion: Self-E是第一个从零开始、支持任意步数的文本到图像模型，为高效和可扩展的生成提供了一个统一框架，实现了少步和多步生成的统一。
...
### [29] [iOSPointMapper: RealTime Pedestrian and Accessibility Mapping with Mobile AI](https://arxiv.org/abs/2512.22392)
*Himanshu Naidu,Yuxiang Zhang,Sachin Mehta,Anat Caspi*

Main category: cs.CV

TL;DR: iOSPointMapper是一款利用iPhone/iPad进行实时人行道测绘的移动应用，通过设备上的语义分割、LiDAR深度估计和GPS/IMU融合数据来检测和定位交通标志、信号灯等行人相关设施。
...
Method: 开发iOS移动应用，利用设备上的语义分割、LiDAR深度估计、GPS/IMU数据融合来检测行人相关特征，并包含用户引导的标注界面来验证系统输出，数据匿名后传输到TDEI平台。

Result: 系统特征检测和空间测绘性能的详细评估显示该应用在增强行人测绘方面具有潜力，能够提供可扩展、以用户为中心的方法来填补关键数据空白。

Conclusion: iOSPointMapper为行人基础设施提供了可扩展、用户中心化的数据收集解决方案，能够整合到多模式交通数据集中，改善行人测绘质量。
...
### [30] [DeFloMat: Detection with Flow Matching for Stable and Efficient Generative Object Localization](https://arxiv.org/abs/2512.22406)
*Hansang Lee,Chaelin Lee,Nieun Seo,Joon Seok Lim,Helen Hong*

Main category: cs.CV

TL;DR: DeFloMat是一种基于条件流匹配的新型生成式目标检测框架，通过用确定性流场替代扩散模型的随机去噪过程，在仅需3步推理的情况下实现了最先进的检测精度，解决了扩散检测器在临床应用中延迟过高的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的目标检测方法（如DiffusionDet）虽然精度高，但需要大量采样步骤（T>60），导致推理延迟过高，无法满足克罗恩病磁共振肠造影等时间敏感的临床应用需求。

Method: DeFloMat采用条件流匹配框架，基于条件最优传输理论构建确定性流场（近似Rectified Flow），替代扩散模型的随机去噪过程，通过简单的常微分方程求解器实现快速推理。

Result: 在MRE临床数据集上，DeFloMat仅用3步推理就达到了43.32% AP10:50的精度，比DiffusionDet在4步时的最佳性能（31.03% AP10:50）提升了1.4倍，同时显著改善了定位特性和召回率稳定性。
...
### [31] [Bright 4B: Scaling Hyperspherical Learning for Segmentation in 3D Brightfield Microscopy](https://arxiv.org/abs/2512.22423)
...
Method: 结合硬件对齐的Native Sparse Attention机制（捕获局部、粗粒度和选择性全局上下文）、深度-宽度残差HyperConnections稳定表示流、软混合专家机制实现自适应容量，以及各向异性patch嵌入尊重共聚焦点扩散和轴向变薄特性

Result: 模型直接从明场堆栈中产生形态准确的细胞核、线粒体和其他细胞器分割，在多个共聚焦数据集上保持跨深度和细胞类型的精细结构细节，优于当代CNN和Transformer基线

Conclusion: Bright-4B为大规模、无标记3D细胞映射提供了有效解决方案，所有代码、预训练权重和下游微调模型都将发布以推动该领域发展
...
### [32] [EmoCtrl: Controllable Emotional Image Content Generation](https://arxiv.org/abs/2512.22437)
*Jingyuan Yang,Weibin Luo,Hui Huang*

Main category: cs.CV

TL;DR: 本文提出EmoCtrl模型，用于可控情感图像内容生成，在保持内容描述一致性的同时表达目标情感，解决了现有模型在内容一致性和情感表达之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 图像通过视觉内容和情感色调共同传达意义，但现有文本到图像模型虽然确保内容一致性却缺乏情感意识，而情感驱动模型生成情感结果却以内容失真为代价。需要一种能同时保持内容忠实性和情感表达的方法。

Method: 提出EmoCtrl模型，基于包含内容、情感和情感提示的数据集，通过文本和视觉情感增强模块，利用描述性语义和感知线索丰富情感表达，学习互补的情感标记。

Result: EmoCtrl在忠实内容和表达情感控制方面优于现有方法，定量和定性实验均显示其优越性，用户研究证实其与人类偏好高度一致，且能很好地泛化到创意应用中。

Conclusion: EmoCtrl成功实现了可控情感图像内容生成，在保持内容一致性的同时有效表达目标情感，学习的情绪标记具有互补效应和良好的泛化能力，为情感感知的图像生成提供了有效解决方案。
...
### [33] [LECalib: Line-Based Event Camera Calibration](https://arxiv.org/abs/2512.22441)
...
Conclusion: 提出了一种无需专门标定板的事件相机标定方法，利用环境中自然存在的线条特征，提高了标定的便捷性和适应性
...
<details>
  <summary>Details</summary>
Motivation: 现有的单目3D建筑重建方法通常需要特定任务架构和密集监督，限制了可扩展性。本文旨在评估通用基础模型SAM 3D在遥感建筑重建任务中的性能。

Method: 在NYC Urban Dataset上对SAM 3D和TRELLIS进行基准测试，使用FID和CMMD作为评估指标，并通过segment-reconstruct-compose管道将SAM 3D扩展到城市场景重建。

Result: 实验结果显示，SAM 3D相比TRELLIS能生成更连贯的屋顶几何和更清晰的边界，证明了其在城市3D重建中的潜力。

Conclusion: SAM 3D作为通用基础模型在单目遥感建筑重建中表现优异，为城市3D重建部署基础模型提供了实用指导，并激励未来整合场景级结构先验的研究。
...
TL;DR: 该研究训练并比较了YOLOv8、YOLOv11和RF-DETR三种计算机视觉模型在变电站组件检测任务上的性能，旨在实现自动化变电站基础设施映射，以提高电网关键基础设施的脆弱性评估效率。


<details>
  <summary>Details</summary>
Motivation: 变电站作为电网关键基础设施，其资产（如变压器）容易受到飓风、洪水、地震和地磁感应电流等多种灾害的破坏。传统人工映射方法耗时耗力，需要自动化解决方案来提高效率和便利性，以更好地识别关键变电站组件并量化脆弱性。

Method: 研究使用手动标注的美国变电站图像数据集，训练并比较了三种计算机视觉模型：YOLOv8、YOLOv11和RF-DETR。评估指标包括检测准确率、精确度和效率，并对各模型的优缺点进行分析。

Result: 研究展示了三种模型在变电站组件检测任务上的性能表现，识别出最适合可靠、大规模变电站组件映射的模型。同时，成功应用这些模型对美国各地的变电站组件进行了有效映射。

Conclusion: 该研究证明了机器学习在变电站映射应用中的可行性，为自动化变电站基础设施识别提供了有效解决方案，有助于提高电网关键基础设施的脆弱性评估和风险管理效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于姿态编码的方法（如CoMo）虽然支持可解释的运动控制，但其逐帧表示难以捕捉细微的时间动态和高频细节，导致重建保真度和局部可控性下降。

Method: 提出PGR²M混合表示：1）姿态引导的RVQ分词器将运动分解为编码全局粗结构的姿态潜在变量和建模细粒度时间变化的残差潜在变量；2）残差丢弃防止过度依赖残差；3）基础Transformer从文本自回归预测姿态编码；4）精炼Transformer在文本、姿态编码和量化阶段条件下预测残差编码。

Result: 在HumanML3D和KIT-ML数据集上，PGR²M在Fréchet inception距离和重建指标上优于CoMo及最近的扩散和分词基线方法，用户研究证实其能实现直观、结构保持的动作编辑。

Conclusion: PGR²M通过混合表示有效解决了现有姿态编码方法在时间动态建模方面的局限性，显著提升了文本驱动的3D动作生成和编辑的质量与可控性。
...
Method: 1) 建立极坐标系，通过事件偏移计算进行自适应ROI提取；2) 利用速度变化的连续性，通过迭代斜率分析提取冲击波前事件；3) 基于事件的光学成像模型和3D重建模型推导事件几何模型和冲击波运动参数。
...
TL;DR: 提出了一种用于红外小目标检测的半监督范式，通过分层MoE适配器和两阶段知识蒸馏，在仅有10%标注数据的情况下实现接近全监督的性能。
...
Method: 设计了分层MoE适配器（包含四个白盒神经算子），采用两阶段范式：1）先验引导知识蒸馏，用MoE适配器和10%标注数据将SAM蒸馏为专家教师模型（Scalpel-SAM）；2）部署导向知识转移，用Scalpel-SAM生成伪标签训练轻量下游模型。

Result: 实验表明，在最小标注情况下，该范式能使下游模型达到甚至超越全监督对应模型的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态LLM存在推理不可靠的问题，产生流畅但不可信的推理过程，表现出弱的步骤间连贯性和不足的视觉基础。这是因为现有的对齐方法只监督最终答案，而忽略了中间推理过程的可靠性。

Method: 提出了SR-MCR框架，整合五个自参考线索：语义对齐、词汇保真度、非冗余性、视觉基础和步骤一致性，形成归一化的可靠性加权奖励。采用无批评者的GRPO目标，并加入置信感知冷却机制来稳定训练并抑制平凡或过度自信的生成。

Result: 基于Qwen2.5-VL构建的SR-MCR在广泛的视觉基准测试中提高了答案准确性和推理连贯性。在可比规模的开源模型中，SR-MCR-7B实现了81.4%的平均准确率，达到最先进性能。消融研究确认了每个奖励项和冷却模块的独立贡献。

Conclusion: SR-MCR通过利用内在过程信号提供细粒度的过程级指导，有效提升了多模态LLM推理的可靠性和连贯性，为模型对齐提供了轻量级、无需标注的解决方案。
...
TL;DR: 提出了一种基于稀疏差分Transformer的Top-K Jaccard相似度系数方法，用于改进人脸聚类中的嵌入关系测量，通过增强邻居节点纯度和抗噪声能力，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用Jaccard相似度系数代替余弦距离来增强人脸嵌入关系测量的准确性，但引入了过多无关节点，导致Jaccard系数区分能力有限，影响聚类性能。

Method: 1) 提出预测驱动的Top-K Jaccard相似度系数，增强邻居节点纯度；2) 开发基于Transformer的预测模型，分析中心节点与Top-K附近邻居节点的关系；3) 提出稀疏差分Transformer(SDT)替代原始Transformer，消除噪声并增强抗噪声能力。

Result: 在MS-Celeb-1M等多个数据集上的大量实验表明，该方法实现了最先进的性能，优于现有方法，为人脸聚类提供了更鲁棒的解决方案。

Conclusion: 通过预测驱动的Top-K Jaccard相似度系数和稀疏差分Transformer的结合，有效解决了人脸聚类中相似度测量精度不足的问题，提高了聚类性能的可靠性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视频扩散模型的视觉规划方法主要是前向预测，仅基于初始观察生成轨迹而缺乏明确的目标建模，导致空间漂移和目标不对齐问题。

Method: 两阶段框架：1) 目标图像模型识别任务相关区域，通过区域感知交叉注意力合成连贯的目标图像；2) 环境-目标视频模型基于首尾帧条件视频扩散模型(FL2V)，在初始观察和目标图像之间插值生成平滑物理合理的视频轨迹。

Result: 在物体操作和图像编辑基准测试中，Envision相比基线方法在目标对齐、空间一致性和物体保持方面表现更优，生成的视觉规划可直接支持下游机器人规划与控制。

Conclusion: Envision通过显式约束目标图像，解决了现有视觉规划方法的空间漂移和目标不对齐问题，为具身智能体提供了可靠的视觉规划指导。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像质量评估(IQA)模型通常只输出单一全局分数，对局部和细粒度失真极不敏感，导致超分辨率模型可能产生感知上不理想的伪影却获得虚假高分，造成奖励黑客问题，使优化目标与感知质量不一致。
...
Conclusion: 提出的FinPercep-RM模型和CCL机制成功解决了传统IQA模型对局部失真不敏感的问题，通过细粒度感知奖励和渐进式训练策略，实现了更稳定的训练和更好的感知质量对齐。
...
<details>
  <summary>Details</summary>
Motivation: 为深度估计提供扩散模型之外的另一种生成模型选择，探索自回归先验在几何感知生成模型中的潜力，特别是在数据可扩展性和3D视觉任务适应性方面的优势

Method: 1) 采用大规模文本到图像VAR模型进行适配；2) 引入带分类器自由引导的尺度条件上采样机制；3) 使用10个固定自回归阶段进行推理；4) 仅需74K合成样本进行微调

Result: 1) 在室内基准测试中，在受限训练条件下达到最先进性能；2) 在室外数据集上表现强劲；3) 验证了自回归先验作为深度估计的几何感知生成模型补充家族的可行性

Conclusion: 自回归先验是深度估计中一个有效的几何感知生成模型家族，在数据可扩展性和3D视觉任务适应性方面具有优势，为单目深度估计提供了扩散模型之外的可行替代方案
...
TL;DR: 本研究系统评估了多种深度学习架构（3D Inception、双流、CNN-RNN）在超声心动图视频中左心室射血分数估计的性能，发现改进的3D Inception架构表现最佳，RMSE为6.79%，同时观察到模型容易过拟合，性能对超参数选择高度敏感。
...
Result: 改进的3D Inception架构实现了最佳整体性能，均方根误差为6.79%。观察到模型普遍存在过拟合趋势，较小且简单的模型通常表现出更好的泛化能力。模型性能对超参数选择高度敏感，特别是卷积核大小和归一化策略。

Conclusion: 虽然本研究专注于基于超声心动图的LVEF估计，但关于架构设计和训练策略的见解可能适用于更广泛的医学和非医学视频分析任务。改进的3D Inception架构在LVEF估计中表现出色，但需要关注过拟合问题和超参数优化。
...
TL;DR: 提出CLAdapter（聚类注意力适配器），通过注意力机制和聚类中心将大规模预训练模型的知识适配到数据有限的下游科学领域任务中，在多种2D/3D模型架构上实现统一接口。


<details>
  <summary>Details</summary>
Motivation: 虽然计算机视觉领域受益于大规模数据集和预训练模型，但许多数据有限的专业科学领域下游任务仍面临挑战，需要将预训练模型的丰富知识有效适配到这些特定领域。

Method: 提出CLAdapter，引入注意力机制和聚类中心，通过分布相关性和变换矩阵个性化增强转换特征，使模型能够学习针对不同特征集的特定表示，实现从丰富预训练特征到下游场景的有效适配。

Result: 在10个数据集上进行广泛实验，涵盖通用、多媒体、生物、医学、工业、农业、环境、地理、材料科学、OOD和3D分析等多个领域，CLAdapter在数据有限的科学领域实现了最先进的性能。

Conclusion: CLAdapter通过自适应迁移有效释放了基础视觉模型的潜力，其统一接口设计可无缝集成到多种模型架构中，为数据有限的下游科学任务提供了有效的解决方案。
...
Method: 提出INTERACT-CMIL框架，采用多头深度学习架构，通过共享特征学习与组合部分监督以及跨任务一致性的相互依赖损失函数，联合预测WHO4、WHO5、水平扩散、垂直扩散和细胞异型性五个组织病理学轴。

Result: 在486个专家标注的结膜活检切片数据集上评估，相比CNN和基础模型基线，INTERACT-CMIL在WHO4上获得55.1%的相对宏观F1提升，在垂直扩散上获得25.0%的提升，提供与专家分级一致的可解释多标准预测。

Conclusion: 该框架为CMIL诊断提供了可重复的计算基准，向标准化数字眼病理学迈出了一步，能够提供一致、可解释的多标准预测。
...
Result: 在标准基准测试中显著提升了文本到图像对应的人类对齐指标和视觉质量，在人类偏好评分和美学评估方面持续提升性能，达到与最先进奖励优化方法相当的结果。

Conclusion: CritiFusion作为插件式细化阶段与现有扩散主干兼容，通过语义批判和频谱对齐策略实现了更优的细节、真实性和提示保真度。
...
TL;DR: 提出ARFM（自回归流匹配）方法用于序列连续数据的概率建模，在多样化视频数据集上训练以生成长时间跨度的未来点轨迹位置，并在人类和机器人运动预测任务中展示优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有运动预测模型通常针对特定分布训练，应用范围有限；而大规模视频预测模型虽然视觉效果逼真，但在复杂运动建模方面仍有不足。受视频生成规模化趋势启发，需要开发能够准确预测复杂运动的新方法。

Method: 开发了自回归流匹配（ARFM）方法，这是一种用于序列连续数据概率建模的新技术。模型在多样化视频数据集上进行训练，能够生成长时间跨度的未来点轨迹位置。

Result: ARFM模型能够准确预测复杂运动。实验表明，将机器人动作预测和人类运动预测基于预测的未来轨迹进行条件化，可以显著提升下游任务性能。为此还开发了专门的评估基准。

Conclusion: ARFM方法在运动预测领域表现出色，能够有效建模复杂运动，并通过轨迹预测条件化显著提升下游任务性能。代码和模型已公开提供。
...
TL;DR: 提出SCPainter统一框架，结合3D高斯泼溅资产表示、3D场景点云和扩散模型，实现自动驾驶场景中3D资产插入和新视角合成的联合处理。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真需要多样化的训练数据，包括长尾驾驶场景。现有方法将3D资产插入和新视角合成分开处理，缺乏统一的框架来实现真实感3D资产插入与场景交互，限制了训练数据的多样性。

Method: SCPainter框架整合3D高斯泼溅（GS）汽车资产表示和3D场景点云，结合扩散模型生成。将3D GS资产和3D场景点云共同投影到新视角，用这些投影作为扩散模型的条件输入来生成高质量图像。

Result: 在Waymo Open Dataset上的评估表明，该框架能够实现3D资产插入和新视角合成，促进多样化和真实驾驶数据的创建。

Conclusion: SCPainter提供了一个统一的仿真框架，能够联合处理真实感3D资产插入和新视角合成，为自动驾驶训练生成更多样、更真实的场景数据。
...
TL;DR: 提出Freetime FeatureGS方法，通过可学习的特征高斯基元和线性运动能力，从单帧分割图直接重建分解的4D场景，无需视频分割


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖视频分割质量，但视频分割结果不稳定，导致4D场景重建不可靠。需要直接从单帧分割图重建分解的4D场景

Method: 使用Freetime FeatureGS表示动态场景，包含可学习特征的高斯基元，具有线性运动能力。应用对比损失使基元特征根据2D分割图中的实例关系接近或远离。采用时间有序的流式特征学习策略

Result: 在多个数据集上的实验结果表明，该方法的重建质量显著优于现有方法

Conclusion: 提出的Freetime FeatureGS方法通过流式特征学习策略，直接从单帧分割图实现了高质量的分解4D场景重建，避免了视频分割的不稳定性问题
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在处理视觉输入时通常将图像编码为令牌序列，但视觉令牌数量的增加显著提高了推理成本。现有的视觉令牌剪枝方法往往忽视了长上下文、多图像场景的挑战。
...
Conclusion: 该方法有效解决了长上下文多图像场景中的视觉令牌剪枝问题，通过分解冗余并动态分配预算，在降低计算成本的同时保持了模型性能。
...
Method: 提出基于希尔伯特曲线重排序的邻居感知token缩减方法：1) Neighbor-Aware Pruning (NAP) 选择性保留token；2) Merging by Adjacent Token similarity (MAT) 局部token聚合
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成质量优秀，但顺序去噪过程导致采样延迟高。现有基于求解器的加速方法在低延迟预算下常面临图像质量显著下降，主要原因是无法捕捉高曲率轨迹段而累积的截断误差。

Method: 提出EPD-Solver（集成并行方向求解器），利用向量值函数中值定理更准确逼近积分解。采用两阶段优化框架：1）通过蒸馏方法优化少量可学习参数；2）提出参数高效的强化学习微调方案，将求解器重新表述为随机Dirichlet策略，仅在低维求解器空间操作。方法还可作为插件（EPD-Plugin）改进现有ODE采样器。

Result: EPD-Solver通过并行梯度评估有效缓解截断误差，同时保持低延迟采样特性。强化学习微调方案能有效防止奖励黑客攻击，在复杂文本到图像生成任务中提升性能。

Conclusion: EPD-Solver是一种创新的ODE求解器，通过几何洞察和并行梯度计算解决了扩散模型加速中的截断误差问题，其参数高效的优化框架为高质量低延迟采样提供了有效解决方案。
...
Method: 提出基于多模态大语言模型的全局跟踪框架VPTracker，引入位置感知的视觉提示机制，基于目标先前位置构建区域级提示，优先进行区域级识别，仅在必要时进行全局推理

Result: 实验表明该方法显著提升了跟踪稳定性，在挑战性场景下有效解决了目标歧义问题，为将多模态大语言模型集成到视觉跟踪中开辟了新途径

Conclusion: VPTracker通过结合多模态大语言模型的强大语义推理能力和位置感知的视觉提示机制，实现了更鲁棒的全局视觉语言跟踪，为视觉跟踪领域带来了新的研究方向
...
Method: 基于3D高斯和tri-plane表示的3D重建方法，结合高斯表示的高效渲染和几何表示优势，增强稀疏切片下的结构连续性和语义一致性
...
TL;DR: 本文评估了开放词汇目标检测模型在低质量图像条件下的性能，发现模型在低级图像退化时mAP无明显下降，但在高级图像退化时性能急剧下降，其中OWLv2表现最佳。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测旨在实现接近人类水平的识别能力，但现有模型在现实世界低质量图像条件下的性能尚未得到充分评估。本文旨在填补这一研究空白，评估模型在模拟真实世界低质量图像条件下的表现。

Method: 作者创建了一个模拟真实世界低质量图像的新数据集，并在此数据集上评估了多种开放词汇目标检测模型（包括OWLv2、OWL-ViT、GroundingDINO和Detic）的性能。
...
Conclusion: 开放词汇目标检测模型对高级图像退化敏感，OWLv2在低质量图像条件下表现最佳。作者将发布数据集和代码以促进未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 人类对第一人称视角输入具有适应性和上下文敏感的反应，但现有方法难以同时满足严格因果生成和精确3D空间对齐的双重要求。现有数据集（如ViMo）存在空间不一致问题，动态动作总是与固定摄像头视频配对。

Method: 首先构建Human Reaction Dataset (HRD)解决数据稀缺和对齐问题；提出EgoReAct框架：1）通过向量量化变分自编码器将反应动作压缩到紧凑的潜在空间；2）训练生成预训练变换器从视觉输入生成反应；3）整合3D动态特征（度量深度和头部动态）增强空间基础。

Result: EgoReAct在真实性、空间一致性和生成效率方面显著优于先前方法，同时保持严格的因果生成。能够实时生成3D对齐的人类反应动作。

Conclusion: EgoReAct是首个能够从第一人称视角视频流实时生成3D对齐人类反应动作的自回归框架，通过整合3D动态特征有效增强了空间基础，解决了现有方法的局限性。
...
TL;DR: DA360将Depth Anything V2适配到全景深度估计，通过ViT骨干学习偏移参数消除尺度偏移不变性，结合环形填充消除接缝伪影，在室内外基准测试中显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 全景深度估计在室内场景已有广泛研究，但在开放世界的零样本泛化能力远落后于透视图像，后者受益于丰富的训练数据。这种差距使得从透视领域迁移能力成为有吸引力的解决方案。

Method: 提出了DA360，这是Depth Anything V2的全景适配版本。核心创新包括：从ViT骨干学习偏移参数，将模型的尺度和偏移不变输出转换为尺度不变估计，直接生成格式良好的3D点云；在DPT解码器中集成环形填充以消除接缝伪影，确保尊重球形连续性的空间一致深度图。

Result: 在标准室内基准和新策划的室外数据集Metropolis上评估，DA360相比基础模型在室内和室外基准上分别实现了超过50%和10%的相对深度误差减少。显著优于现有全景深度估计方法，相比PanDA在所有三个测试数据集上实现了约30%的相对误差改进，为零样本全景深度估计建立了新的最先进性能。

Conclusion: DA360通过从ViT骨干学习偏移参数和集成环形填充，成功将透视深度估计模型的强大能力迁移到全景领域，显著提升了零样本全景深度估计的性能，在室内外场景都实现了显著的误差减少。
...
TL;DR: SCaR-3D是一个3D场景变化检测框架，通过基于符号距离的2D差分模块和多视角聚合，从密集视角的前变化图像序列和稀疏视角的后变化图像中检测对象级变化，支持连续场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有3D变化检测方法存在空间不一致性，且无法明确分离变化前后的状态，需要更准确和高效的3D变化检测方法。

Method: 使用基于符号距离的2D差分模块，结合多视角聚合（投票和剪枝），利用3DGS的一致性特性来鲁棒地分离变化前后状态，并开发了连续场景重建策略。

Result: 在提出的CCS3D合成数据集上进行了广泛实验，证明该方法在准确性和效率上都优于现有方法。

Conclusion: SCaR-3D框架能够准确检测3D场景中的对象级变化，明确分离变化前后状态，并支持连续场景重建，为场景监控和探索提供了有效解决方案。
...
TL;DR: 提出一种基于两个仿射对应和已知垂直方向（来自IMU）来估计相机相对位姿（3自由度）和焦距的新求解器


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶汽车、智能手机、无人机等应用中，相机常与惯性测量单元（IMU）结合使用。IMU可提供相机垂直方向信息，将相对位姿从5自由度减少到3自由度，从而简化估计问题

Method: 1. 在已知垂直方向条件下，从两个仿射对应建立约束方程；2. 基于方程系统非平凡解的性质，推导出仅涉及焦距和相对旋转角的四个方程；3. 使用多项式特征值方法求解焦距和相对旋转角

Result: 在合成和真实数据集上的评估表明，所提出的求解器性能优于现有的最先进求解器

Conclusion: 该方法有效利用IMU提供的垂直方向信息，通过两个仿射对应成功估计3自由度相对位姿和焦距，在精度和鲁棒性方面优于现有方法
...
<details>
  <summary>Details</summary>
Motivation: 现有的人-物交互视频生成方法存在两个关键限制：1)缺乏有效机制将对象的多视角信息注入模型，导致跨视角一致性差；2)严重依赖细粒度手部网格标注来建模交互遮挡。这些限制了方法在实际应用中的效果。

Method: 提出ByteLoom框架，采用扩散Transformer架构。核心创新包括：1)RCM-cache机制，利用相对坐标图作为通用表示来保持对象几何一致性并精确控制6自由度对象变换；2)渐进式训练课程，通过增强模型能力来缓解数据集稀缺问题并减少对手部网格的依赖。

Result: 实验表明，该方法能够忠实保持人类身份和对象的多视角几何特性，同时维持平滑的运动和对象操作，在生成质量上优于现有方法。
...
TL;DR: MUSON是一个用于短时程社交导航的多模态数据集，通过结构化思维链标注和平衡的动作空间设计，解决了现有数据集缺乏推理监督和动作分布长尾的问题。
...
Method: MUSON采用五步思维链标注（感知、预测、推理、动作、解释），明确建模静态物理约束，并设计理性平衡的离散动作空间。

Result: 在MUSON上评估多个最先进的小型视觉语言模型，Qwen2.5-VL-3B达到最高决策准确率0.8625，证明MUSON作为社交合规导航基准的有效性。

Conclusion: MUSON提供了一个有效且可复用的社交合规导航基准，通过结构化推理标注和平衡动作空间解决了现有数据集的局限性。
...
TL;DR: Lamps是一种通过多视角自监督学习解剖结构的医学影像基础模型，利用人体解剖学的一致性、连贯性和层次性作为监督信号，在胸部X光片上预训练，展现出优异的鲁棒性、可迁移性和临床潜力。
...
Method: 提出Lamps模型，通过多视角自监督学习解剖结构，利用人体解剖学的一致性、连贯性和层次性作为监督信号，在大规模胸部X光片上进行预训练。

Result: 在10个数据集上通过微调和涌现特性分析进行广泛实验，与10个基线模型相比，Lamps展现出优异的鲁棒性、可迁移性和临床潜力。

Conclusion: 通过多视角学习解剖结构，Lamps为基础模型提供了开发有意义、鲁棒且与人体解剖结构对齐的表示能力的独特机会。
...
TL;DR: 该论文提出了一种数据导向的方法来缓解深度学习模型中的虚假相关性，通过识别、中和、消除和更新四个步骤构建了一个有效的去偏流程，在图像和NLP去偏基准测试中取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在训练过程中经常学习到与类别标签虚假相关但与预测任务无关的特征。现有方法通常需要标注潜在的虚假属性，或基于某些经验假设（如偏见的简单性）过滤虚假特征，但由于现实数据中虚假相关性的复杂性和难以捉摸性，这些方法可能效果不佳。

Method: 提出一个包含四个步骤的流程：1）识别：观察到受虚假特征影响的样本在学习到的特征空间中呈现分散分布，从而识别虚假特征的存在；2）中和：基于简单的分组策略中和虚假特征，获得偏置不变表示；3）消除：学习特征变换以消除虚假特征，与偏置不变表示对齐；4）更新：结合学习到的特征变换更新分类器，获得无偏模型。

Result: 在图像和NLP去偏基准测试中，与标准经验风险最小化（ERM）相比，最差组准确率提高了20%以上。

Conclusion: 通过识别、中和、消除和更新四个步骤构建的数据导向方法能有效缓解深度学习模型中的虚假相关性，显著提高模型在最差组上的性能表现。
...
TL;DR: 本文提出了M-ErasureBench多模态评估框架，系统评估概念擦除方法在文本提示、学习嵌入和反演潜在空间三种输入模态下的表现，并提出了IRECE方法来增强推理时的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的概念擦除方法主要针对文本提示，忽略了图像编辑和个性化生成等实际应用中日益重要的其他输入模态（如学习嵌入和反演潜在空间），这些模态可能成为攻击面，导致已擦除的概念重新出现。

Method: 1. 提出M-ErasureBench多模态评估框架，在三种输入模态（文本提示、学习嵌入、反演潜在空间）和两种访问设置（白盒、黑盒）下评估概念擦除方法；2. 提出IRECE方法，通过交叉注意力定位目标概念并在去噪过程中扰动相关潜在空间来增强鲁棒性。

Result: 现有方法在文本提示下表现良好，但在学习嵌入和反演潜在空间下基本失效，白盒设置下的概念再现率超过90%。IRECE方法能有效恢复鲁棒性，在最具挑战性的白盒潜在反演场景下将概念再现率降低达40%，同时保持视觉质量。
...
TL;DR: GPS（引导路径采样）通过用流形约束插值替代不稳定的外推，解决了迭代细化方法与无分类器引导结合时的稳定性问题，显著提升了生成质量和语义对齐。


<details>
  <summary>Details</summary>
Motivation: 现有基于去噪-反演循环的迭代细化方法在与标准无分类器引导结合时存在根本性限制：CFG的外推性质会系统性地将采样路径推离数据流形，导致近似误差发散并破坏细化过程。
...
Conclusion: 路径稳定性是有效迭代细化的先决条件，GPS通过确保采样路径保持在数据流形上，为稳定迭代细化提供了稳健框架，显著提升了扩散模型的质量和控制能力。
...
TL;DR: JavisGPT是首个用于联合音频-视频理解和生成的多模态大语言模型，采用编码器-LLM-解码器架构，通过三阶段训练流程实现时空同步的音频视频处理。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型主要关注视觉或音频单模态理解，缺乏对音频-视频联合理解和生成的能力，特别是在时空同步方面的挑战。

Method: 采用编码器-LLM-解码器架构，包含SyncFusion模块进行时空音频-视频融合，使用同步感知可学习查询连接预训练的JAV-DiT生成器。通过三阶段训练：多模态预训练、音频-视频微调和大规模指令调优。

Result: 在JAV理解和生成基准测试中，JavisGPT优于现有MLLMs，特别是在复杂和时空同步设置下表现突出。

Conclusion: JavisGPT成功实现了首个统一的音频-视频联合理解和生成模型，通过创新的架构设计和高质量数据集构建，在多模态任务中取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型（VLM）的自动驾驶规划器面临三个关键挑战：离散文本推理与连续控制之间的不匹配、自回归链式思维解码的高延迟、以及低效或非因果规划器限制实时部署。需要一种既能保持VLM泛化能力和可解释性，又能实现高效、准确和安全轨迹生成的解决方案。
...
Conclusion: ColaVLA成功地将VLM的推理能力转移到统一潜在空间，结合分层并行解码器，在保持VLM泛化能力和可解释性的同时，实现了高效、准确和安全的自动驾驶轨迹生成。
...
### [69] [OpenGround: Active Cognition-based Reasoning for Open-World 3D Visual Grounding](https://arxiv.org/abs/2512.23020)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位方法依赖预定义的对象查找表来查询视觉语言模型进行对象定位推理，这限制了在未定义或未预见目标场景中的应用。需要解决预定义OLT的限制，实现开放世界的3D视觉定位。

Method: 提出了OpenGround框架，核心是主动认知推理模块。该模块通过认知任务链实现类人目标感知，主动推理上下文相关对象，通过动态更新的OLT扩展VLM认知能力。既能处理预定义类别，也能处理开放世界类别。

Result: 在Nr3D数据集上达到竞争性性能，在ScanRefer上达到最先进水平，在提出的OpenTarget数据集上实现了17.6%的显著提升。OpenTarget数据集包含7000多个对象-描述对，专门用于评估开放世界场景。

Conclusion: OpenGround通过主动认知推理模块有效克服了预定义对象查找表的限制，实现了开放世界3D视觉定位，在多个数据集上表现出色，为处理未定义和未预见目标提供了有效解决方案。
...
TL;DR: DRMNet利用密度图作为空间先验，通过密度生成分支、密集区域聚焦模块和双滤波器融合模块，有效检测高分辨率遥感图像中的密集微小物体。


<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像中的密集微小物体检测面临严重相互遮挡和像素足迹有限的挑战，现有方法均匀分配计算资源，无法自适应聚焦密度集中区域，阻碍了特征学习效果。

Method: 提出DRMNet：1) 密度生成分支建模物体分布模式；2) 密集区域聚焦模块利用密度图识别并聚焦密集区域，实现高效局部-全局特征交互；3) 双滤波器融合模块通过离散余弦变换将多尺度特征解耦为高低频分量，进行密度引导的交叉注意力以增强互补性并抑制背景干扰。

Result: 在AI-TOD和DTOD数据集上的广泛实验表明，DRMNet超越了现有最先进方法，特别是在物体密度高、遮挡严重的复杂场景中表现优异。

Conclusion: DRMNet通过密度图引导的自适应特征学习，有效解决了高分辨率遥感图像中密集微小物体的检测难题，在复杂场景中展现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统目标检测器依赖交叉熵分类，容易受到类别不平衡和标签噪声的影响。为了解决这些问题，需要一种更鲁棒的监督方法

Method: 提出CLIP-Joint-Detect框架，通过轻量级并行头将区域或网格特征投影到CLIP嵌入空间，并与可学习的类别特定文本嵌入对齐，使用InfoNCE对比损失和辅助交叉熵项，同时优化所有标准检测损失

Result: 在Pascal VOC 2007+2012上使用Faster R-CNN，在MS COCO 2017上使用YOLOv11，均获得一致且显著的性能提升，同时保持实时推理速度

Conclusion: 通过可学习文本嵌入的联合优化显著提升了闭集检测性能，适用于多种架构和数据集，是一种简单有效的检测器无关框架
...
<details>
  <summary>Details</summary>
Motivation: 现有增量目标检测方法主要基于Faster R-CNN或DETR系列检测器，无法适应实时YOLO检测框架。YOLO在增量学习中存在三种主要知识冲突：前景-背景混淆、参数干扰和知识蒸馏不对齐，导致灾难性遗忘。

Method: 基于预训练YOLO-World模型构建YOLO-IOD框架，采用阶段式参数高效微调。包含三个核心组件：1) CPR：利用伪标签置信度缓解前景-背景混淆；2) IKS：识别并更新当前任务关键卷积核；3) CAKD：通过新旧教师检测头实现跨阶段非对称知识蒸馏。

Result: 在传统和LoCo COCO基准测试中，YOLO-IOD表现出优越性能且遗忘最小。LoCo COCO是更现实的基准，消除了阶段间数据泄漏问题。

Conclusion: YOLO-IOD是首个实时YOLO增量目标检测框架，有效解决了YOLO增量学习中的三种知识冲突，在保持实时性的同时显著减少了灾难性遗忘。
...
Method: 提出ReamCamo统一外绘框架，引入布局控制调节全局图像结构，结合统一细粒度文本任务描述和纹理导向的背景检索构建多模态文本-视觉条件，共同指导生成过程

Result: 提出背景-前景分布差异度量来定量评估伪装质量，大量实验和可视化结果证明了该框架的有效性

Conclusion: ReamCamo通过布局控制和多模态条件生成，显著提升了伪装图像的真实性和语义一致性，为伪装目标检测提供了高质量的训练数据生成方法
...
<details>
  <summary>Details</summary>
Motivation: 当前生成模型通常在全局像素域操作或依赖二值掩码，这些方法存在特征纠缠问题，导致解剖基质损坏或结构不连续。需要一种能够保持解剖完整性同时生成高质量病理变化的合成方法。
...
Conclusion: PathoSyn提供了一个数学原理驱动的管道，用于生成高保真合成数据，支持可解释的反事实疾病进展建模，促进低数据环境下稳健诊断算法的开发，并为临床决策支持系统提供基准测试环境。
...
### [75] [PoseStreamer: A Multi-modal Framework for 6DoF Pose Estimation of Unseen Moving Objects](https://arxiv.org/abs/2512.22979)
*Huiming Yang,Linglin Liao,Fei Ding,Sibo Wang,Zijian Zeng*

Main category: cs.CV

TL;DR: PoseStreamer是一个针对高速运动场景设计的鲁棒多模态6自由度姿态估计框架，通过自适应姿态记忆队列、物体中心2D跟踪器和射线姿态滤波器三个核心组件，在高速低光条件下实现精确的姿态估计。


<details>
  <summary>Details</summary>
Motivation: 在高速和低光场景中，传统RGB相机因运动模糊而难以进行6自由度姿态估计，而事件相机虽然具有高时间分辨率，但现有方法在高速物体运动场景中表现不佳。需要专门针对高速运动场景设计鲁棒的姿态估计框架。
...
Conclusion: PoseStreamer为解决高速运动场景下的6自由度姿态估计问题提供了有效的解决方案，通过多模态融合和时间一致性机制显著提升了在挑战性条件下的性能。
...
TL;DR: 提出SSA框架解决医学图像分割中多类型文本（位置、描述、诊断）处理难题，通过对称最优传输对齐和复合方向引导策略提升空间约束下的病灶分割精度


<details>
  <summary>Details</summary>
Motivation: 当前文本引导的医学图像分割方法存在两个关键瓶颈：1）难以同时处理诊断性和描述性文本，难以建立图像区域与病灶的关联；2）现有方法关注病灶描述但忽略位置约束，导致关键偏差（如"左下肺"可能错误覆盖双侧）

Method: 提出空间感知对称对齐（SSA）框架：1）对称最优传输对齐机制，建立图像区域与多个相关表达之间的双向细粒度多模态对应；2）复合方向引导策略，通过构建区域级引导掩码显式引入文本中的空间约束

Result: 在公开基准测试上的广泛实验表明，SSA实现了最先进的性能，特别是在准确分割具有空间关系约束特征的病灶方面表现突出

Conclusion: SSA框架有效增强了处理包含位置、描述和诊断信息的混合医学文本的能力，解决了现有方法在同时处理多类型文本和空间约束方面的局限性
...
TL;DR: 提出了一种基于条件扩散反演的反向个性化框架，用于面部匿名化，无需文本提示即可直接操作图像，支持属性可控的匿名化，在身份移除、属性保留和图像质量方面达到最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示的身份特征移除或修改方法要么依赖于预训练模型中对主体的良好表示，要么需要对特定身份进行模型微调，缺乏对超出训练数据主体的泛化能力，且现有匿名化方法缺乏对面部属性的控制。

Method: 引入反向个性化框架，利用条件扩散反演技术，无需文本提示直接操作图像；加入身份引导的条件分支以泛化到模型训练数据之外的主体；支持属性可控的匿名化，不同于先前方法。

Result: 该方法在身份移除、属性保留和图像质量方面实现了最先进的平衡，源代码和数据已公开。

Conclusion: 提出的反向个性化框架为面部匿名化提供了一种有效解决方案，能够在不依赖文本提示的情况下直接操作图像，同时支持属性控制，实现了身份移除与属性保留的良好平衡。
...
### [78] [ForCM: Forest Cover Mapping from Multispectral Sentinel-2 Image by Integrating Deep Learning with Object-Based Image Analysis](https://arxiv.org/abs/2512.23196)
...
TL;DR: 提出ForCM方法，结合对象基图像分析(OBIA)与深度学习(DL)，使用Sentinel-2影像进行森林覆盖制图，在亚马逊雨林区域测试多种DL模型，结果显示ResUNet-OBIA和AttentionUNet-OBIA精度优于传统OBIA方法。


<details>
  <summary>Details</summary>
Motivation: 传统森林覆盖制图方法精度有限，需要开发更准确高效的技术来支持全球环境监测和保护工作。研究旨在探索深度学习与对象基图像分析结合的可能性，利用免费易用的工具实现高精度制图。

Method: 提出ForCM方法，结合OBIA与DL技术。使用Sentinel-2 Level 2A卫星影像，包含三个数据集（两个三波段和一个四波段）。测试多种DL模型（UNet、UNet++、ResUNet、AttentionUNet、ResNet50-Segnet），然后将最有效的DL模型与OBIA技术集成。使用QGIS等免费工具实现。

Result: ForCM方法显著提升森林覆盖制图精度：ResUNet-OBIA达到94.54%总体精度，AttentionUNet-OBIA达到95.64%总体精度，而传统OBIA方法仅为92.91%。证明DL与OBIA结合的有效性。

Conclusion: ForCM方法成功结合DL与OBIA技术，显著提高森林覆盖制图精度。研究证明免费工具如QGIS在环境监测中的潜力，为全球森林保护和监测提供有效技术支持。
...
TL;DR: 提出一个基于低成本RGB无人机的统一果园智能系统，集成了叶片病害检测、苹果新鲜度评估和实时苹果检测定位功能，完全离线运行，为精准农业提供经济高效的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有无人机系统通常孤立处理果园管理任务，且依赖昂贵的多光谱传感器，需要一种低成本、统一的解决方案来同时处理病害检测、质量评估和产量估算。

Method: 使用基于RGB图像的无人机系统，集成ResNet50进行叶片病害分类，VGG16进行苹果新鲜度评估，YOLOv8进行实时苹果检测和定位，系统在ESP32-CAM和树莓派上完全离线运行。

Result: 叶片病害分类准确率达98.9%，新鲜度分类准确率达97.4%，苹果检测F1分数为0.857，系统在低成本硬件上表现优异。

Conclusion: 该框架为多光谱无人机解决方案提供了经济高效且可扩展的替代方案，支持在廉价硬件上实现实用的精准农业应用。
...
TL;DR: 该论文提出了一种基于地理语义上下文图(GSCG)的上下文感知物体分类框架，通过整合深度估计和全景分割来构建结构化场景表示，显著提升了物体识别准确率。


<details>
  <summary>Details</summary>
Motivation: 人类识别物体时能充分利用丰富的场景上下文信息（空间关系、材质属性、物体共现等），而传统计算识别系统通常在孤立图像区域上操作，忽略了这些关键上下文信息。论文旨在解决这一差距，强调上下文在物体识别中的重要作用。

Method: 1. 从单目图像构建地理语义上下文图(GSCG)：整合度量深度估计器和统一的全景与材质分割模型；2. 将物体编码为具有几何、色彩和材质属性的节点，空间关系编码为边；3. 设计专门的基于图的分类器，聚合目标物体、邻近物体和全局场景特征进行分类预测。

Result: 上下文感知模型在COCO 2017数据集上达到73.4%的分类准确率，远超上下文无关版本（最低38.4%）；优于微调的ResNet模型（最高53.5%）和最先进的多模态大语言模型Llama 4 Scout（最高42.3%）。

Conclusion: 明确的结构化和可解释的上下文表示对于物体识别任务具有显著优势，地理语义上下文图框架不仅提升了性能，还提供了可解释的推理过程，验证了上下文信息在计算视觉中的关键作用。
...
### [81] [Holi-DETR: Holistic Fashion Item Detection Leveraging Contextual Information](https://arxiv.org/abs/2512.23221)
*Youngchae Kwon,Jinyoung Choi,Injung Kim*

Main category: cs.CV

TL;DR: 提出Holi-DETR，一种利用三种上下文信息（共现关系、空间布局、人体关键点关系）来整体检测时尚物品的检测器，相比传统独立检测方法能减少歧义。


<details>
  <summary>Details</summary>
Motivation: 时尚物品检测面临挑战，因为时尚物品外观高度多样化且子类别间相似性高，导致歧义。传统检测器独立检测每个物品，忽略了时尚物品之间存在的有意义关系（如搭配风格）。

Method: 提出Holi-DETR，基于DETR架构，整合三种异构上下文信息：1) 时尚物品间的共现概率关系；2) 基于物品间空间布局的相对位置和大小关系；3) 物品与人体关键点的空间关系。

Result: 在实验中，相比原始DETR和较新的Co-DETR，Holi-DETR分别将平均精度(AP)提高了3.6个百分点和1.1个百分点。

Conclusion: 通过利用三种上下文信息进行整体检测，Holi-DETR能有效减少时尚物品检测中的歧义，显著提升检测性能。
...
Result: 在六个流行数据集上的大量实验表明，该方法在各种划分协议和不同场景下始终取得领先性能。
...
### [83] [Anomaly Detection by Effectively Leveraging Synthetic Images](https://arxiv.org/abs/2512.23227)
...
<details>
  <summary>Details</summary>
Motivation: 工业制造中异常检测至关重要，但由于真实缺陷图像稀缺，现有合成方法存在明显权衡：基于规则的合成成本低但不够真实，基于生成模型的合成质量高但成本巨大。

Method: 提出新框架：1) 使用预训练文本引导图像翻译模型生成合成缺陷图像；2) 利用图像检索模型评估生成图像与真实正常图像的相似性，过滤无关输出；3) 采用两阶段训练策略：先用大量规则合成图像预训练，再用少量高质量图像微调。

Result: 在MVTec AD数据集上的实验证明了方法的有效性，能够在显著降低数据收集成本的同时提高异常检测性能。
...
### [84] [MedGemma vs GPT-4: Open-Source and Proprietary Zero-shot Medical Disease Classification from Images](https://arxiv.org/abs/2512.23304)
*Md. Sazzadul Islam Prottasha,Nabil Walid Rafi*

Main category: cs.CV

TL;DR: 该研究对比了两种AI架构在医学影像诊断中的表现：经过微调的MedGemma-4b-it模型在六种疾病诊断中表现优于GPT-4，准确率达80.37%，在癌症和肺炎等高风险任务中敏感性更高。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型为医学影像分析提供了新范式，但不同架构在临床诊断中的性能差异需要系统评估。本研究旨在比较专业开源模型MedGemma与商业大模型GPT-4在疾病分类任务中的表现，为临床AI应用提供依据。

Method: 研究对比了两种AI架构：经过LoRA微调的开源模型MedGemma-4b-it与未微调的GPT-4。在六种不同疾病的诊断任务中评估模型性能，使用混淆矩阵和分类报告进行定量分析，重点关注模型在癌症和肺炎等高风险临床任务中的敏感性。

Result: MedGemma-4b-it模型平均测试准确率达到80.37%，显著高于GPT-4的69.58%。MedGemma在癌症和肺炎检测等高风险任务中表现出更高的敏感性。定量分析显示MedGemma在所有疾病类别中均表现优异。

Conclusion: 领域特定的微调对于减少临床实施中的幻觉至关重要。MedGemma展现出作为复杂、循证医学推理工具的潜力，在医学影像诊断中优于通用大模型GPT-4，为临床AI应用提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有半监督3D目标检测方法主要关注伪标签质量和特征一致性，但忽视了模型在有限标注数据下对物体几何结构的低敏感性，这限制了模型对物体几何信息的捕捉能力，而几何信息对于目标感知和定位至关重要。
...
Conclusion: GeoTeacher通过几何关系监督和增强策略，显著提升了半监督3D目标检测中模型对物体几何结构的理解能力，可与其他方法结合进一步提升性能。
...
TL;DR: SoulX-LiveTalk是一个14B参数的实时音频驱动虚拟人生成框架，通过自校正双向蒸馏策略保持时空相关性，实现高保真度实时流式生成，达到0.87秒启动延迟和32FPS吞吐量。
...
Method: 提出14B参数的SoulX-LiveTalk框架，采用自校正双向蒸馏策略在视频块内保留双向注意力；引入多步回顾自校正机制确保无限生成的稳定性；开发包含混合序列并行、并行VAE和内核级优化的全栈推理加速套件。

Result: SoulX-LiveTalk是首个14B规模的系统，实现了亚秒级启动延迟（0.87秒）和32FPS的实时吞吐量，为高保真交互式数字人合成设立了新标准。

Conclusion: 该框架通过创新的双向注意力保留策略和自校正机制，成功解决了大规模扩散模型在实时音频驱动虚拟人生成中的计算与延迟矛盾，实现了高质量实时生成性能。
...
### [87] [REVEALER: Reinforcement-Guided Visual Reasoning for Element-Level Text-Image Alignment Evaluation](https://arxiv.org/abs/2512.23169)
*Fulin Shi,Wenyi Xiao,Bin Chen,Liang Din,Leilei Gan*

Main category: cs.CV

TL;DR: REVEALER是一个基于强化引导视觉推理的统一框架，用于评估文本到图像生成中的元素级对齐，通过结构化范式实现可解释的对齐判断，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像对齐评估方法大多依赖粗粒度指标或静态QA流程，缺乏细粒度可解释性且难以反映人类偏好，需要更精细、可解释的对齐评估框架。
...
Conclusion: REVEALER提供了一个统一、可解释的框架，用于评估文本到图像生成中的元素级对齐，在性能和效率方面均表现出色，为文本到图像模型的可靠性和可用性评估提供了有效解决方案。
...
### [88] [Fuzzy-Logic and Deep Learning for Environmental Condition-Aware Road Surface Classification](https://arxiv.org/abs/2512.23436)
*Mustafa Demetgul,Sanja Lazarova Molnar*

Main category: cs.CV

TL;DR: 本文提出了一种基于天气条件和路面状况数据的实时道路监测系统，使用手机摄像头采集数据，测试多种深度学习算法进行道路分类，并比较了加速度数据和图像数据的性能，达到了95%以上的准确率。
...
Method: 1. 使用手机摄像头在卡尔斯鲁厄理工学院校园周围道路采集数据；2. 测试多种基于图像的深度学习算法（AlexNet、LeNet、VGG、ResNet）进行道路分类；3. 将道路加速度数据与图像数据结合训练，将加速度数据转换为图像格式；4. 比较加速度数据和摄像头图像两种方法的性能；5. 使用模糊逻辑根据天气和时间对道路表面进行分类。

Result: 1. 道路状况分类考虑了5个类别：沥青路、损坏沥青路、碎石路、损坏碎石路、铺砌路；2. 实现了超过95%的准确率性能；3. 比较了不同深度学习算法的性能表现。

Conclusion: 提出的实时系统能够有效监测道路表面状态，结合深度学习算法和模糊逻辑方法，为车辆控制和规划提供可靠的道路状况信息，相比传统方法更加经济和高效。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像3D检测方法面临两难困境：高精度方法需要密集3D监督，而无监督方法难以从图像中提取准确几何信息。需要一种既能保持高精度又无需昂贵3D监督的解决方案。

Method: 提出高斯-体素协同表示学习框架：1) 采用可泛化的高斯泼溅提取几何特征；2) 设计跨表示增强机制，用高斯场的几何细节丰富体素特征；3) 通过可学习集成直接利用两种表示的特征，实现更准确的目标定位。

Result: 在ScanNetV2和ARKitScenes等室内基准测试中取得最先进结果，显著优于现有方法，且无需任何深度或密集3D几何监督（如点云或TSDF）。

Conclusion: 高斯和体素表示捕捉互补的几何信息，通过协同学习策略可以有效提升图像3D检测性能，证明了无需深度传感器或密集3D监督也能实现高精度3D目标检测的可行性。
...
### [90] [CoFi-Dec: Hallucination-Resistant Decoding via Coarse-to-Fine Generative Feedback in Large Vision-Language Models](https://arxiv.org/abs/2512.23453)
*Zongsheng Cao,Yangfan He,Anran Liu,Jun Xie,Feng Chen,Zepeng Wang*

Main category: cs.CV

TL;DR: CoFi-Dec是一个无需训练的LVLM解码框架，通过粗到细视觉条件化和生成式自反馈来减少幻觉，使用Wasserstein融合机制整合多级视觉假设，显著降低实体和语义级幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态理解和生成方面取得了显著进展，但倾向于产生与视觉输入不一致的幻觉内容，这限制了它们在现实应用中的可靠性。需要一种方法来减少这种幻觉问题。

Method: 提出CoFi-Dec训练免费解码框架：1）受人类从全局场景感知到细节检查的视觉过程启发，首先生成基于原始图像粗粒度和细粒度视图的中间文本响应；2）使用文生图模型将这些响应转换为合成图像，形成多级视觉假设；3）引入基于Wasserstein的融合机制，将预测分布对齐到几何一致的解码轨迹中。

Result: 在六个专注于幻觉的基准测试上的广泛实验表明，CoFi-Dec显著减少了实体级和语义级幻觉，优于现有的解码策略。该框架是模型无关的，无需额外训练，可广泛应用于各种LVLM。

Conclusion: CoFi-Dec通过整合生成式自反馈和粗到细视觉条件化，提供了一种原则性的方法来缓解LVLM中的幻觉问题，提高了输出的鲁棒性和忠实度，同时保持了部署的便利性。
...
TL;DR: 提出基于3D高斯场景表示的驾驶世界模型框架，实现3D场景理解与多模态生成，通过语言特征嵌入实现早期模态对齐，在nuScenes和NuInteract数据集上达到SOTA性能。
...
Conclusion: 提出的基于3D高斯场景表示的驾驶世界模型框架成功解决了现有方法的局限性，实现了3D场景理解和多模态场景生成，同时为理解和生成任务提供了上下文丰富性。
...
### [92] [HY-Motion 1.0: Scaling Flow Matching Models for Text-To-Motion Generation](https://arxiv.org/abs/2512.23464)
...
TL;DR: HY-Motion 1.0是首个达到十亿参数规模的基于扩散Transformer的3D人体运动生成模型，通过大规模预训练、高质量微调和强化学习实现文本指令跟随，在运动生成领域显著超越现有开源基准。


<details>
  <summary>Details</summary>
Motivation: 当前3D人体运动生成模型在规模、指令跟随能力和运动质量方面存在局限，需要开发更大规模、更精确的模型来推动该领域向商业化成熟发展。

Method: 采用全阶段训练范式：1）在3000多小时运动数据上进行大规模预训练；2）在400小时精选数据上进行高质量微调；3）结合人类反馈和奖励模型的强化学习。同时建立了严格的数据处理流程进行运动清洗和标注。

Result: 模型在运动生成领域显著超越当前开源基准，实现了最广泛的覆盖范围，涵盖6大类超过200个运动类别，具备优秀的指令跟随能力和高质量运动生成效果。

Conclusion: HY-Motion 1.0成功将基于扩散Transformer的流匹配模型扩展到十亿参数规模，通过全阶段训练范式实现了精确的文本-运动对齐，为3D人体运动生成模型的商业化应用奠定了基础，并开源以促进未来研究。
...
TL;DR: 本文提出了一种自适应选择扩散时间步特征的方法，通过任务感知时间步选择和特征整合模块，在少样本密集预测任务中提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的应用通常依赖经验直觉选择扩散时间步特征，这往往导致次优性能且偏向某些特定任务。为了解决这个问题，研究者需要探索如何自适应选择最适合少样本密集预测任务的时间步特征。

Method: 提出了两个核心模块：1) 任务感知时间步选择(TTS)，基于时间步损失和相似度分数选择理想扩散时间步；2) 时间步特征整合(TFC)，整合选定时间步特征以提升少样本密集预测性能。同时采用参数高效的微调适配器。

Result: 在具有挑战性的大规模Taskonomy数据集上进行实证验证，特别针对实际通用和少样本学习场景，证明该方法在仅提供少量支持查询的情况下，有效实现了密集预测性能的优越性。

Conclusion: 通过自适应选择扩散时间步特征并进行有效整合，该方法能够显著提升少样本密集预测任务的性能，为扩散模型在通用任务中的应用提供了更优的解决方案。
...
TL;DR: AnyMS是一个无需训练的多主体定制框架，通过双级注意力解耦机制平衡文本对齐、主体身份保持和布局控制三个目标，无需额外训练即可实现复杂多主体合成。


<details>
  <summary>Details</summary>
Motivation: 现有多主体定制方法在平衡文本对齐、主体身份保持和布局控制方面存在困难，且依赖额外训练限制了可扩展性和效率。需要一种无需训练的方法来解决这些问题。

Method: 提出AnyMS框架，采用自底向上的双级注意力解耦机制：全局解耦分离文本和视觉条件的交叉注意力以确保文本对齐；局部解耦将每个主体的注意力限制在指定区域以防止冲突。使用预训练图像适配器提取主体特征，无需主体学习或适配器调优。

Result: 实验表明AnyMS达到最先进性能，支持复杂构图并扩展到更多主体数量，在文本对齐、主体身份保持和布局控制方面表现优异。

Conclusion: AnyMS是一个高效、可扩展的训练免费多主体定制框架，通过创新的注意力解耦机制成功平衡了三个关键目标，为多主体合成提供了有效解决方案。
...
### [95] [MM-UAVBench: How Well Do Multimodal Large Language Models See, Think, and Plan in Low-Altitude UAV Scenarios?](https://arxiv.org/abs/2512.23219)
*Shiqi Dai,Zizhi Ma,Zhicong Luo,Xuesong Yang,Yibin Huang,Wanyue Zhang,Chi Chen,Zonghao Guo,Wang Xu,Yufei Sun,Maosong Sun*

Main category: cs.CV

TL;DR: MM-UAVBench：首个针对无人机低空场景的多模态大语言模型综合评测基准，涵盖感知、认知、规划三大能力维度，包含19个子任务和5700+人工标注问题，揭示当前MLLMs在复杂低空场景中的适应不足问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在无人机低空应用中的潜力尚未充分探索。现有MLLM基准很少覆盖低空场景的独特挑战，而无人机相关评估主要关注特定任务（如定位、导航），缺乏对MLLM通用智能的统一评估。

Method: 提出MM-UAVBench基准，系统评估MLLMs在低空无人机场景中的三大核心能力维度：感知、认知和规划。基准包含19个子任务，超过5700个人工标注问题，所有数据均来自公开数据集的真实无人机采集数据。对16个开源和专有MLLM进行了广泛实验。

Result: 实验发现当前模型难以适应低空场景的复杂视觉和认知需求。分析揭示了阻碍MLLMs在无人机场景中有效部署的关键瓶颈，如空间偏见和多视角理解问题。

Conclusion: MM-UAVBench基准将促进未来针对真实世界无人机智能的鲁棒可靠MLLM研究，填补了低空场景MLLM评估的空白。
...
### [96] [PathFound: An Agentic Multimodal Model Activating Evidence-seeking Pathological Diagnosis](https://arxiv.org/abs/2512.23545)
*Shengyi Hua,Jianfeng Wu,Tianle Shen,Kangzhe Hu,Zhongzhen Huang,Shujuan Ni,Zhihong Zhang,Yuan Li,Zhe Wang,Xiaofan Zhang*

Main category: cs.CV

TL;DR: PathFound是一个用于病理诊断的智能多模态模型，采用主动证据搜索的工作流程，通过初始诊断、证据搜索和最终决策三个阶段来改进诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 当前病理基础模型大多采用静态推理范式，一次性处理全切片图像产生预测，缺乏在模糊诊断情况下的重新评估或针对性证据获取。这与临床诊断工作流程形成对比，临床诊断通常通过重复观察切片和进一步检查请求来完善假设。

Method: PathFound整合了病理视觉基础模型、视觉语言模型和强化学习训练推理模型的能力，通过初始诊断、证据搜索和最终决策三个阶段执行主动信息获取和诊断完善。

Result: 采用这种策略在各种大型多模态模型中一致提高了诊断准确性，表明证据搜索工作流程在计算病理学中的有效性。PathFound在多样化临床场景中实现了最先进的诊断性能，并显示出发现细微细节（如核特征和局部浸润）的强大潜力。

Conclusion: PathFound通过模拟临床诊断工作流程的证据搜索推理方法，显著提升了病理诊断的准确性和可靠性，为计算病理学提供了更接近实际临床实践的智能诊断框架。
...
Method: 开发了RxnBench多层级基准，包含两个任务：1) 单图问答任务，基于305个精选反应方案生成的1,525个问题，测试细粒度视觉感知和机理推理；2) 全文问答任务，基于108篇文章，要求模型整合文本、反应方案和表格等多模态信息。

Result: 评估发现关键能力差距：模型擅长提取显式文本，但在深层化学逻辑和精确结构识别方面表现不佳。具有推理时推理能力的模型显著优于标准架构，但所有模型在全文问答任务上的准确率均未超过50%。

Conclusion: 需要开发领域特定的视觉编码器和更强的推理引擎，以推进自主AI化学家的研发，填补当前多模态大语言模型在化学反应理解方面的能力缺口。
...
### [98] [RS-Prune: Training-Free Data Pruning at High Ratios for Efficient Remote Sensing Diffusion Foundation Models](https://arxiv.org/abs/2512.23239)
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感扩散基础模型依赖大量全局代表性数据，但这些数据通常包含冗余、噪声和类别不平衡问题，降低了训练效率并阻碍收敛。现有方法通常简单聚合多个分类数据集或应用简单去重，忽视了生成模型的分布需求以及遥感图像的异质性。

Method: 提出免训练的两阶段数据剪枝方法：1）基于熵的准则高效移除低信息样本；2）利用遥感场景分类数据集作为参考基准，进行场景感知聚类与分层采样，在平衡聚类级均匀性和样本代表性的同时，实现高剪枝率下的细粒度选择。

Result: 实验表明，即使在剪枝85%的训练数据后，该方法仍能显著改善收敛性和生成质量。使用该方法训练的扩散基础模型在下游任务（如超分辨率和语义图像合成）中持续取得最先进的性能。
...
### [99] [Multimodal Interpretation of Remote Sensing Images: Dynamic Resolution Input Strategy and Multi-scale Vision-Language Alignment Mechanism](https://arxiv.org/abs/2512.23243)
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感图像多模态融合方法存在固定分辨率无法平衡效率与细节、单尺度对齐缺乏语义层次等问题，限制了表面信息提取的准确性。

Method: 提出视觉语言模型框架，集成两个关键创新：1) 动态分辨率输入策略(DRIS)，采用粗到细方法自适应分配计算资源；2) 多尺度视觉语言对齐机制(MS-VLAM)，构建对象、局部区域和全局三个层次的对齐机制。
...
### [100] [ASemConsist: Adaptive Semantic Feature Control for Training-Free Identity-Consistent Generation](https://arxiv.org/abs/2512.23245)
*Shin seong Kim,Minjung Shin,Hyunin Cho,Youngjung Uh*

Main category: cs.CV

TL;DR: ASemconsist是一个新的文本到图像扩散模型框架，通过选择性文本嵌入修改解决角色身份一致性问题，同时保持每张图像的提示对齐，在FLUX模型基础上实现了最先进的性能。
...
Result: 该框架实现了最先进的性能，有效克服了先前方法的权衡问题，并在身份一致性和提示对齐方面表现出色。

Conclusion: ASemconsist通过创新的选择性文本嵌入修改和自适应特征共享策略，成功解决了文本到图像扩散模型中角色身份一致性与提示对齐的权衡问题，并提出了统一的评估协议CQS。
...
### [101] [Contour Information Aware 2D Gaussian Splatting for Image Representation](https://arxiv.org/abs/2512.23255)
*Masaya Takabe,Hiroshi Watanabe,Sujun Hong,Tomohiro Ikai,Zheming Fan,Ryo Ishimoto,Kakeru Sugimoto,Ruri Imichi*

Main category: cs.CV

TL;DR: 提出一种轮廓信息感知的2D高斯泼溅框架，通过结合对象分割先验来改善图像表示质量，特别是在高斯数量较少时能更好地保持边缘结构。


<details>
  <summary>Details</summary>
Motivation: 现有2D高斯泼溅方法在压缩率高（高斯数量少）时会产生模糊或不清晰的边界，缺乏轮廓感知能力，需要改进边缘保持性能。

Method: 提出轮廓信息感知的2D高斯泼溅框架，将对象分割先验融入高斯表示中，通过限制每个高斯在特定分割区域内进行光栅化，防止跨边界混合；同时引入预热方案来稳定训练并提高收敛性。

Result: 在合成色卡和DAVIS数据集上的实验表明，该方法在物体边缘区域实现了比现有2DGS方法更高的重建质量，特别是在高斯数量很少的情况下效果更明显，同时保持了快速渲染和低内存使用。

Conclusion: 通过结合分割先验的轮廓感知机制，提出的2D高斯泼溅框架在保持高效性的同时显著改善了边缘保持能力，为高质量图像压缩表示提供了有效解决方案。
...
### [102] [Plug-and-Play Fidelity Optimization for Diffusion Transformer Acceleration via Cumulative Error Minimization](https://arxiv.org/abs/2512.23258)
*Tong Shao,Yusen Fu,Guoying Sun,Jingde Kong,Zhuotao Tian,Jingyong Su*

Main category: cs.CV

TL;DR: 提出CEM插件，通过累积误差最小化优化扩散Transformer的缓存加速策略，提高生成保真度


<details>
  <summary>Details</summary>
Motivation: 扩散Transformer的迭代去噪过程导致推理速度慢，现有缓存加速方法存在较大计算误差，且固定缓存策略无法适应去噪过程中复杂的误差变化

Method: 提出基于累积误差最小化的CEM插件，预定义误差表征模型对加速的敏感性，通过动态规划算法优化缓存策略，实现误差最小化

Result: 在9个生成模型和量化方法上验证，CEM显著提升现有加速模型的生成保真度，在FLUX.1-dev、PixArt-α、StableDiffusion1.5和Hunyuan上甚至超越原始生成性能

Conclusion: CEM是一种模型无关、可泛化的插件，能无缝集成到现有误差校正框架和量化模型中，无需额外计算开销，显著提升扩散Transformer的加速性能
...
### [103] [Multi-Track Multimodal Learning on iMiGUE: Micro-Gesture and Emotion Recognition](https://arxiv.org/abs/2512.23291)
...
<details>
  <summary>Details</summary>
Motivation: 微手势识别和行为情感预测都是极具挑战性的任务，需要建模细微的人类行为模式。现有方法主要利用视频和骨骼姿态数据，但如何有效整合多模态信息以捕捉细微的时空模式仍是一个重要问题。

Method: 1. 微手势分类：结合RGB和3D姿态表示，使用MViTv2-S提取视频嵌入，2s-AGCN提取骨骼嵌入，通过跨模态令牌融合模块整合空间和姿态信息。
2. 情感识别：扩展框架进行行为情感预测（二分类任务），使用SwinFace提取面部嵌入，MViTv2-S提取上下文嵌入，通过InterFusion模块融合情感表达和身体手势信息。

Result: 在iMiGUE数据集上的实验表明，该方法在行为情感预测任务中表现出鲁棒的性能和准确性，在MiGA 2025挑战赛中获得了第二名。

Conclusion: 提出的多模态框架能够有效处理微手势识别和行为情感预测任务，通过融合不同模态的特征表示，成功捕捉了细微的人类行为模式，在挑战性任务中取得了优异的性能。
...
### [104] [CME-CAD: Heterogeneous Collaborative Multi-Expert Reinforcement Learning for CAD Code Generation](https://arxiv.org/abs/2512.23333)
*Ke Niu,Haiyang Yu,Zhuofan Chen,Zhengtao Yao,Weitao Jia,Xiaodong Ge,Jingqun Tang,Benlei Cui,Bin Li,Xiangyang Xue*

Main category: cs.CV

TL;DR: 本文提出CME-CAD范式，通过异构协作多专家强化学习解决CAD模型自动生成中精度和可编辑性的挑战，并发布包含17,299个实例的CADExpert开源基准。
...
Method: 提出异构协作多专家强化学习（CME-CAD）范式，整合不同模型的互补优势，促进协作学习。采用两阶段训练过程：多专家微调（MEFT）和多专家强化学习（MERL）。

Result: 开发了CADExpert开源基准，包含17,299个实例，具有精确尺寸标注的正交投影、专家生成的思维链过程、可执行的CADQuery代码和渲染的3D模型。
...
### [105] [Visual Language Hypothesis](https://arxiv.org/abs/2512.23335)
*Xiu Li*

Main category: cs.CV

TL;DR: 该论文从拓扑结构角度研究视觉表示学习，提出视觉观察空间具有纤维丛结构，语义对应商空间，需要非光滑的判别性目标才能获得语义不变性。


<details>
  <summary>Details</summary>
Motivation: 从结构和拓扑角度理解视觉表示学习，基于一个核心假设：视觉理解需要语义语言，许多感知观察对应少量离散语义状态。结合表示学习中的可迁移性和抽象性假设，探索视觉空间的拓扑结构。

Method: 采用拓扑学框架分析视觉表示学习，提出纤维丛结构模型：视觉观察空间X中，冗余变化填充纤维，语义对应商空间X/G。从该结构推导两个理论结果：1) 语义商空间不是X的子流形，需要非同胚的判别性目标；2) 近似商空间对模型架构有结构要求，需要支持拓扑变化的表示机制。

Result: 理论推导表明：1) 语义不变性不能仅通过光滑变形获得，需要标签监督、跨实例识别或多模态对齐等显式语义等价；2) 语义抽象需要"扩展-捕捉"过程：先几何扩展分离结构，再坍缩形成离散语义区域。

Conclusion: 该框架提供了与大规模判别性和多模态模型经验规律一致的拓扑视角，强调结果是解释性而非规范性，为理解视觉表示学习的结构要求提供了理论基础。
...
### [106] [SpatialMosaic: A Multiview VLM Dataset for Partial Visibility](https://arxiv.org/abs/2512.23365)
*Kanghee Lee,Injae Lee,Minseok Kwak,Kwonyoung Ryu,Jungi Hong,Jaesik Park*

Main category: cs.CV

TL;DR: 提出SpatialMosaic数据集和SpatialMosaicVLM框架，通过多视图图像增强3D场景理解，解决真实环境中部分可见、遮挡和低重叠等挑战性问题


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在3D场景理解方面依赖预构建的3D表示或现成重建流程，限制了可扩展性和实际应用。真实环境中的部分可见性、遮挡和低重叠条件等挑战性问题尚未得到充分探索

Method: 1) 提出可扩展的多视图数据生成和标注流程，构建真实的空间推理问答对，创建包含200万QA对的SpatialMosaic数据集；2) 提出包含100万QA对、涵盖6个任务的SpatialMosaic-Bench基准测试；3) 提出SpatialMosaicVLM混合框架，将3D重建模型作为几何编码器集成到视觉语言模型中

Result: 实验表明，提出的数据集和视觉问答任务能有效增强在挑战性多视图条件下的空间推理能力，验证了数据生成流程在构建真实多样QA对方面的有效性

Conclusion: 通过SpatialMosaic数据集和SpatialMosaicVLM框架，成功解决了真实环境中多视图空间推理的挑战，为增强3D场景理解提供了有效的解决方案
...
### [107] [NeXT-IMDL: Build Benchmark for NeXT-Generation Image Manipulation Detection & Localization](https://arxiv.org/abs/2512.23374)
...
<details>
  <summary>Details</summary>
Motivation: 用户友好图像编辑模型的普及和滥用风险迫切需要通用且最新的图像篡改检测与定位方法。当前研究通常采用跨数据集评估，但这种简化方法掩盖了现有方法在处理多样化AI生成内容时的脆弱性，导致对进展的误导性印象。

Method: NeXT-IMDL将AIGC篡改沿四个基本轴进行分类：编辑模型、篡改类型、内容语义和伪造粒度。基于此，实现了五种严格的跨维度评估协议，系统性地探测检测器的泛化边界。

Result: 对11个代表性模型的广泛实验显示：这些模型在原始设置下表现良好，但在模拟真实世界各种泛化场景的设计协议下，表现出系统性失败和显著的性能下降。

Conclusion: 通过提供这个诊断工具包和新发现，旨在推动开发真正鲁棒的下一代IMDL模型，挑战当前评估方法造成的进展假象。
...
### [108] [SOFTooth: Semantics-Enhanced Order-Aware Fusion for Tooth Instance Segmentation](https://arxiv.org/abs/2512.23411)
*Xiaolan Li,Wanquan Liu,Pengcheng Li,Pengyu Jie,Chenqiang Gao*

Main category: cs.CV

TL;DR: SOFTooth：一种利用冻结2D语义增强3D牙齿实例分割的语义增强、顺序感知2D-3D融合框架，无需2D掩码监督


<details>
  <summary>Details</summary>
Motivation: 3D牙齿实例分割面临拥挤牙弓、模糊的牙齿-牙龈边界、缺失牙齿和罕见但临床重要的第三磨牙等挑战。现有3D方法依赖几何线索常出现边界泄漏、中心漂移和牙齿身份不一致问题，而2D基础模型（如SAM）虽提供强边界感知语义，但直接应用于3D临床工作流不切实际。

Method: 提出SOFTooth框架：1）点级残差门控模块将咬合视图SAM嵌入注入3D点特征以细化牙齿-牙龈和牙齿间边界；2）中心引导掩码细化正则化实例掩码与几何质心间一致性，减少中心漂移；3）顺序感知匈牙利匹配策略整合解剖牙齿顺序和中心距离到基于相似度的分配中，确保即使在缺失或拥挤牙列下也能保持连贯标记。

Result: 在3DTeethSeg'22数据集上，SOFTooth实现了最先进的整体准确率和平均IoU，在涉及第三磨牙的病例上表现出明显优势，证明丰富的2D语义可以在无需2D微调的情况下有效转移到3D牙齿实例分割中。

Conclusion: SOFTooth通过有效融合2D语义和3D几何信息，解决了3D牙齿实例分割中的关键挑战，特别是在处理复杂解剖结构和罕见牙齿类别时表现出色，为临床工作流提供了实用解决方案。
...
### [109] [DriveLaW:Unifying Planning and Video Generation in a Latent Driving World](https://arxiv.org/abs/2512.23421)
...
TL;DR: DriveLaW提出了一种统一视频生成和运动规划的新范式，通过将视频生成器的潜在表示直接注入规划器，确保高保真未来生成与可靠轨迹规划之间的内在一致性。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中的世界模型通常将世界预测和运动规划作为解耦过程处理，缺乏内在一致性。为了弥合这一差距，需要一种统一的方法来确保未来场景生成与轨迹规划之间的协调性。

Method: DriveLaW包含两个核心组件：DriveLaW-Video（强大的世界模型，生成具有表达性潜在表示的高保真预测）和DriveLaW-Act（扩散规划器，从DriveLaW-Video的潜在表示生成一致可靠的轨迹）。采用三阶段渐进训练策略优化这两个组件。

Result: DriveLaW在视频预测任务上显著超越最佳性能工作，FID提升33.3%，FVD提升1.8%。在NAVSIM规划基准测试中创造了新记录，实现了两个任务的最新最先进结果。

Conclusion: DriveLaW通过统一视频生成和运动规划，为自动驾驶世界模型提供了新的范式，确保了未来场景生成与轨迹规划之间的内在一致性，在两个任务上都取得了突破性进展。
...
### [110] [Direct Diffusion Score Preference Optimization via Stepwise Contrastive Policy-Pair Supervision](https://arxiv.org/abs/2512.23426)
*Dohyun Kim,Seungwoo Lyu,Seung Wook Kim,Paul Hongsuck Seo*

Main category: cs.CV

TL;DR: DDSPO是一种新的扩散模型偏好优化方法，通过直接从可用策略中获取时间步级监督，无需依赖昂贵的人工标注数据，提高了文本-图像对齐和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在文本到图像生成中表现优异，但难以完全对齐用户意图并保持一致的审美质量。现有的基于偏好的训练方法依赖昂贵且可能有噪声的人工标注数据集。

Method: 提出DDSPO方法，直接从获胜和失败策略中获取每个时间步的监督信号。通过预训练参考模型自动生成偏好信号：对比原始提示与语义降级变体的输出，提供密集的过渡级监督。

Result: DDSPO在文本-图像对齐和视觉质量方面均有提升，优于或匹配现有的基于偏好的方法，同时需要更少的监督。

Conclusion: DDSPO提供了一种有效的分数空间偏好监督方法，无需显式奖励建模或人工标注，为扩散模型偏好优化提供了实用解决方案。
...
### [111] [Towards Integrating Uncertainty for Domain-Agnostic Segmentation](https://arxiv.org/abs/2512.23427)
*Jesse Brouwers,Xiaoyan Xing,Alexander Timans*

Main category: cs.CV

TL;DR: 本文研究了不确定性量化能否提升分割基础模型（如SAM）在域外场景下的泛化能力，提出了UncertSAM基准测试，评估了多种轻量级不确定性估计方法，并探索了不确定性指导的预测优化方案。


<details>
  <summary>Details</summary>
Motivation: 分割基础模型（如SAM系列）在零样本场景下表现良好，但在域偏移或知识受限的领域中仍然脆弱。本研究旨在探索不确定性量化是否能够以领域无关的方式缓解这些挑战并增强模型的泛化能力。

Method: 1. 构建UncertSAM基准测试，包含8个数据集，用于在阴影、透明度和伪装等挑战性分割条件下对SAM进行压力测试；2. 评估一系列轻量级、后处理的不确定性估计方法；3. 评估初步的不确定性指导的预测优化步骤。

Result: 在评估的方法中，最后一层拉普拉斯近似产生的不确定性估计与分割误差有良好相关性，表明存在有意义的信号。虽然优化效果的提升是初步的，但研究结果强调了将不确定性纳入分割模型以支持鲁棒、领域无关性能的潜力。

Conclusion: 不确定性量化有潜力提升分割基础模型在挑战性条件下的泛化能力，UncertSAM基准测试和代码已公开，为后续研究提供了基础。
...
Method: 采用混合框架，包括图像预处理、标注、水线检测、尺度间隙估计和数字读取提取等顺序阶段，结合YOLOv8姿态尺度提取和多模态大语言模型（GPT-4o和Gemini 2.0 Flash）。

Result: 水线检测精度达94.24%，F1分数83.64%；尺度间隙检测为后续读取提供准确几何校准；结合尺度间隙元数据显著提升LLMs预测性能，Gemini Stage 2在最优图像条件下达到最高精度（MAE 5.43cm，RMSE 8.58cm，R² 0.84）。
...
Method: TV-RAG提出两种主要机制：1）时间衰减检索模块，在相似度计算中注入显式时间偏移，根据真实的多媒体上下文对文本查询进行排序；2）熵加权关键帧采样器，选择均匀间隔、信息密集的帧，减少冗余同时保持代表性。

Result: TV-RAG在Video-MME、MLVU和LongVideoBench等长视频基准测试中，持续超越大多数领先的基线模型，证明了该方法的有效性。

Conclusion: TV-RAG通过将时间和语义信号编织在一起，实现了一种双级推理机制，可以无需重新训练或微调地嫁接到任何大型视频语言模型上，为长视频理解提供了一种轻量级、经济高效的升级路径。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉生成模型虽然能从文本生成具有一致角色的故事，但人类中心的故事生成面临额外挑战：需要保持详细且多样化的人脸一致性，以及协调不同图像中的多个角色。

Method: 框架包含两个关键组件：1) 迭代身份发现 - 提取连贯的角色身份；2) 重去噪身份注入 - 通过重新去噪过程注入身份信息，同时保留所需上下文。

Result: 在ConsiStory-Human基准测试中，IdentityStory优于现有方法，特别是在面部一致性方面，并支持多角色组合。框架还展示了在无限长度故事生成和动态角色组合等应用中的强大潜力。

Conclusion: IdentityStory通过身份保持生成器，有效解决了人类中心故事生成中的角色一致性挑战，为故事生成应用提供了可靠的技术基础。
...
TL;DR: IAFS是一个无需训练的推理时缩放框架，通过迭代细化和频率感知粒子融合，解决扩散模型在图像超分辨率中感知质量与结构保真度的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在图像超分辨率中难以同时保证高频感知质量和低频结构保真度。虽然推理时缩放理论上可以通过分配更多计算来改善这种权衡，但现有策略仍然不理想：奖励驱动的粒子优化常导致感知过度平滑，而最优路径搜索则容易失去结构一致性。

Method: 提出IAFS（迭代扩散推理时缩放与自适应频率引导）框架，结合迭代细化和频率感知粒子融合。通过迭代修正结构偏差逐步细化生成图像，同时自适应地融合高频感知线索和低频结构信息，实现不同图像细节的更准确和平衡重建。

Result: 在多个基于扩散的超分辨率模型上进行广泛实验，表明IAFS有效解决了感知-保真度冲突，持续改善了感知细节和结构准确性，性能优于现有推理时缩放方法。

Conclusion: IAFS是一个无需训练的推理时缩放框架，通过迭代细化和自适应频率引导，成功平衡了扩散模型在图像超分辨率中的感知质量与结构保真度，为现有方法提供了有效的改进方案。
...
TL;DR: PurifyGen是一种无需训练的文本到图像生成安全方法，通过双阶段提示净化策略，在保留模型原始权重的同时减少不安全内容生成。
...
Method: 提出PurifyGen方法，采用双阶段提示净化策略：1) 通过计算互补语义距离评估每个token的安全性，无需关键词匹配或重新训练；2) 对风险提示应用双空间变换：将有毒对齐嵌入投影到有毒概念矩阵的零空间以移除有害语义，同时将其对齐到干净概念的范围空间以增强安全语义。采用token级策略选择性替换风险token嵌入。

Result: 在五个数据集上的广泛测试表明，PurifyGen在减少不安全内容方面优于现有方法，与依赖训练的方法竞争良好。该方法具有理论基础，对未见提示和模型具有强泛化能力。

Conclusion: PurifyGen提供了一种即插即用的解决方案，无需训练即可实现安全的文本到图像生成，在保留原始意图和连贯性的同时有效净化风险提示。
...
<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型（MLLM）的思维链（CoT）推理在复杂理解任务中表现出色，但其在生成任务中的应用仍处于初级阶段，受限于特定场景机制，缺乏泛化性和适应性。
...
Result: 在多个生成基准测试中，ThinkGen实现了稳健的最先进性能，证明了其在各种生成场景中的有效性。

Conclusion: ThinkGen是首个显式利用MLLM思维链推理的视觉生成框架，其灵活设计支持跨不同数据集的联合训练，为广泛的生成场景提供了有效的CoT推理解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展带来了持续出现的多模态安全风险，现有防御方法存在局限性。传统反应式方法需要模型调整，无法有效应对新兴风险，需要一种能够主动识别和描述分布外安全风险的防护系统。

Method: 1. 构建包含87K样本的模态平衡数据集，采用分层多模态安全分类法标注二元安全标签和风险类别；2. 通过纯强化学习训练视觉语言基础模型实现高效简洁推理；3. 引入分布外安全类别推理任务，在强化学习目标中增加基于同义词库的相似性奖励，鼓励模型为未见不安全类别生成简洁描述。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型通过双向注意力迭代去噪所有视频帧，无法实现实时交互。虽然已有蒸馏方法可以减少采样步骤，但主要针对文本到视频生成，在多模态条件下存在视觉伪影问题，导致人机交互不自然且效率低下。

Method: 针对多模态条件（文本、图像、音频）下的实时交互视频扩散，改进了蒸馏配方，重点关注条件输入质量以及在线策略优化的初始化和调度策略。在HDTF、AVSpeech和CelebV-HQ等基准上评估多模态条件化身视频生成。

Result: 蒸馏后的模型在视觉质量上与完整步数、双向注意力基线模型相当，同时推理成本和延迟降低了20倍。构建的LiveTalk系统在多轮交互基准测试中，在多轮视频连贯性和内容质量上优于Sora2、Veo3等最先进模型，将响应延迟从1-2分钟降低到实时生成。

Conclusion: 通过改进的蒸馏方法实现了实时多模态交互视频生成，显著降低了推理延迟，使无缝人机多模态交互成为可能。LiveTalk系统展示了在实时交互式AI系统中的实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在细粒度感知方面存在不足，过于关注一般性识别而忽略细微视觉细节。现有训练数据强化了这一局限性，需要专门的数据集来提升模型的感知精度。

Method: 构建TWIN数据集，包含56.1万张图像对，涵盖日常物体在不同上下文、视角和外观下的变化。引入FGVQA基准测试套件，包含1.2万个查询，用于评估细粒度识别能力。在TWIN上微调视觉语言模型。

Result: 在TWIN上微调的视觉语言模型在FGVQA基准上提升了高达19.3%，在艺术、动物、植物、地标等未见领域也表现出色，同时不影响通用VQA基准的性能。数据集规模对性能提升至关重要。
...
TL;DR: 该论文提出了一种改进红外夜视摄像头火灾检测准确性的方法，包括额外的近红外数据集、两阶段检测模型和Patched-YOLO，旨在解决人工光源误判和小目标检测问题。


<details>
  <summary>Details</summary>
Motivation: 尽管现有检测模型（如YOLOv7、RT-DETR、YOLOv9）在火灾检测方面取得了一定性能，但数据集构建的局限性导致人工光源频繁被误判为火灾，特别是在夜间场景下。这促使研究者开发更准确的火灾检测方法。

Method: 1. 构建额外的近红外数据集并应用多种数据增强策略；2. 提出两阶段检测管道，结合YOLOv11和EfficientNetV2-B0，先检测再分类以减少误报；3. 提出Patched-YOLO，通过基于补丁的处理增强RGB图像中小目标和远距离目标的检测能力。

Result: 提出的方法相比先前方法（YOLOv7 mAP50-95 0.51、RT-DETR 0.65、YOLOv9 0.598）实现了更高的检测准确率，特别是在夜间火灾检测方面表现更优，同时减少了人工光源引起的误报。

Conclusion: 通过综合使用额外的数据集、两阶段检测架构和补丁处理技术，该研究有效提升了红外夜视摄像头火灾检测的准确性，特别是在解决人工光源误判和小目标检测等挑战性问题上取得了进展。
...
<details>
  <summary>Details</summary>
Motivation: 胰腺肿瘤早期检测面临临床困境，CT扫描中肿瘤对比度低且患者解剖结构差异大，需要有效可扩展的系统来增强细微视觉线索的显著性并提高多模态成像数据的泛化能力

Method: 提出可扩展残差特征聚合(SRFA)框架：1)预处理和MAGRes-UNet分割使胰腺结构更明显；2)DenseNet-121提取特征并保留残差特征；3)混合HHO-BA元启发式特征选择优化特征子集；4)结合Vision Transformer和EfficientNet-B3的混合模型进行分类；5)SSA和GWO双重优化机制微调超参数

Result: 实验结果显示显著性能提升：准确率96.23%，F1分数95.58%，特异性94.83%，显著优于传统CNN和当代transformer模型

Conclusion: SRFA框架作为胰腺肿瘤早期检测的有用工具具有潜力，其多阶段处理和优化策略有效解决了胰腺肿瘤检测的复杂性问题
...
TL;DR: 本文提出了一个评估框架来量化3D生成模型中的记忆效应，通过控制实验发现数据模态、多样性、条件细化以及模型设计中的引导尺度、向量集长度和旋转增强都会影响记忆程度。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型在3D视觉中用于合成新形状的应用日益增多，需要了解这些模型是否主要依赖记忆训练数据。理解记忆效应有助于防止训练数据泄露并提高生成结果的多样性。

Method: 设计了一个评估框架来量化3D生成模型中的记忆效应，并研究了不同数据和建模设计对记忆的影响。首先将该框架应用于量化现有方法的记忆效应，然后通过使用潜在向量集扩散模型进行控制实验。

Result: 在数据方面，记忆效应取决于数据模态，并随着数据多样性和更细粒度的条件而增加；在建模方面，记忆效应在中等引导尺度达到峰值，可以通过更长的向量集和简单的旋转增强来缓解。

Conclusion: 该框架和分析为3D生成模型中的记忆效应提供了实证理解，并提出了简单有效的策略来减少记忆效应而不降低生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖注意力机制跨帧对齐物体，使用统一的显式物理模型简化运动建模，但不同类别和帧间的运动状态和物体特征变化使得这种对齐不够理想。

Method: HAT首先利用多个显式运动模型生成空间锚点和运动感知特征提案，然后通过结合缓存物体查询中的语义和运动线索进行多假设解码，最终为目标帧提供最优对齐方案。

Result: 在nuScenes数据集上，HAT显著提升了3D时序检测器和跟踪器性能，与DETR3D检测器结合时在测试集上达到46.0% AMOTA的SOTA跟踪结果。在端到端自动驾驶方法中，提升了感知精度并降低了32%的碰撞率。

Conclusion: HAT通过自适应多假设解码有效解决了时空对齐问题，在语义受损情况下仍能提供鲁棒的感知和规划能力，证明了运动建模在自动驾驶感知中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: RGB图像将材质属性、光照和视角相关效应纠缠在一起，而现有的扩散方法在单视角本征分解中表现良好，但扩展到多视角设置时面临挑战，常导致严重的视角不一致问题。

Method: 提出Intrinsic Decomposition Transformer (IDT)，采用基于Transformer的注意力机制联合推理多个输入图像，使用物理基础的图像形成模型，将图像显式分解为漫反射率、漫反射阴影和镜面反射阴影。

Result: 在合成和真实世界数据集上的实验表明，IDT实现了更干净的漫反射率、更一致的漫反射阴影、更好分离的镜面反射分量，相比先前方法显著提高了多视角一致性。

Conclusion: IDT通过结构化分解分离了朗伯和非朗伯光传输，实现了跨视角材质和光照效应的可解释和可控分解，解决了多视角本征分解中的视角一致性问题。
...
TL;DR: Stream-DiffVSR是一个因果条件扩散框架，用于高效在线视频超分辨率，通过四步蒸馏去噪器、自回归时间引导模块和轻量级时间感知解码器，在RTX4090上以0.328秒处理720p帧，显著降低延迟并提升感知质量。
...
Result: 在RTX4090上以0.328秒处理720p帧，显著优于先前扩散方法。相比在线SOTA TMP，提升感知质量（LPIPS +0.095）同时降低延迟130倍以上，将初始延迟从4600秒降至0.328秒。

Conclusion: Stream-DiffVSR实现了扩散基VSR中最低的延迟，首次使扩散VSR方法适用于低延迟在线部署，平衡了感知质量和实时性要求。
...
TL;DR: Moxin 7B是一个完全开源的LLM，遵循模型开放框架，提供训练、数据集和实现细节的完全透明度，并开发了针对视觉语言、视觉语言动作和中文能力的三个变体。


<details>
  <summary>Details</summary>
Motivation: 当前LLM领域由GPT-4等专有模型主导，而开源模型如LLaMA和Mistral虽然促进了普及，但通常只分享模型权重。需要完全透明的开源LLM来促进包容性研究和健康的开源生态系统。

Method: 开发了Moxin 7B作为完全开源的LLM，遵循模型开放框架，提供训练、数据集和实现细节的完全透明度。基于Moxin开发了三个变体：Moxin-VLM（视觉语言）、Moxin-VLA（视觉语言动作）和Moxin-Chinese（中文能力）。使用开源框架和开放数据进行训练。

Result: 实验表明，Moxin模型在各种评估中取得了优越的性能。模型、可用数据和代码均已公开发布。
...
TL;DR: 研究发现transformer语言模型的句子嵌入空间编码了与人类可解释的认知属性对齐的分层几何结构，这种结构可以通过线性或浅层非线性探针可靠解码。


<details>
  <summary>Details</summary>
Motivation: 尽管已有研究表明transformer语言模型在其嵌入空间中学习到了丰富的几何结构，但这些表示中是否存在与人类认知或心理属性对齐的高层认知组织仍未得到充分探索。

Method: 构建包含480个自然语言句子的数据集，标注连续序数能量分数和离散层级标签（涵盖7个有序认知类别）。使用多个transformer模型的固定句子嵌入，通过线性和浅层非线性探针评估这些标注的可恢复性。

Result: 跨模型实验中，连续分数和层级标签都能可靠解码，浅层非线性探针比线性探针表现更优。词汇TF-IDF基线表现显著较差，表明观察到的结构不能仅归因于表面词汇统计。非参数置换测试确认探针性能显著优于随机水平。

Conclusion: transformer嵌入空间展现出与人类定义认知属性对齐的分层几何组织，但该结论不涉及对内部意识或现象学的任何主张。
...
TL;DR: SmartSnap提出从被动的事后任务验证转向主动的实时自验证，通过让智能体在完成任务的同时收集简洁的关键证据快照，提高GUI任务中强化学习的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 当前基于GUI的自主智能体强化学习面临可扩展性瓶颈，主要原因是任务验证成本高、可靠性低。现有验证方法（如规则脚本、奖励模型、LLM-as-a-Judge）都是被动的事后处理，需要分析包含大量无关噪声的完整交互轨迹，导致验证协议效率低下。

Method: 提出SmartSnap范式，将验证从被动事后处理转变为主动实时自验证。设计具有双重使命的自验证智能体：不仅要完成任务，还要通过精心策划的快照证据证明其完成情况。基于3C原则（完整性、简洁性、创造性），智能体利用对在线环境的访问权限，在最小化、决定性的快照集上执行自验证，这些证据作为LLM-as-a-Judge验证器的唯一材料。

Result: 在移动任务上的实验表明，SmartSnap范式能够以可扩展的方式训练LLM驱动的智能体，为8B和30B模型分别带来26.08%和16.66%的性能提升。解决方案寻找与证据寻求的协同作用，培养出高效的自验证智能体，其性能可与DeepSeek V3.1和Qwen3-235B-A22B竞争。

Conclusion: SmartSnap通过将任务验证从被动事后处理转变为主动实时自验证，解决了GUI任务中强化学习的可扩展性瓶颈。自验证智能体通过收集简洁的关键证据快照，显著提高了验证效率和可靠性，为大规模自主智能体训练提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是分析也门伊比阿拉伯语中特殊的qulk-从句结构，这种结构形态融合了"我说"的含义，引入嵌入式陈述、疑问和祈使从句，通常没有补语化成分。研究者希望通过极简方案理论框架来解释这种方言特有的句法现象。

Method: 研究方法采用极简方案的核心操作：合并、移动、一致和拼读。通过分层句法分析，展示qulk-从句如何通过标准计算步骤和后句法过程（如形态合并）进行推导。同时分析方言特有特征如二分否定、附着化和CP嵌入。

Result: 研究发现qulk-从句是双从句结构，其中qulk作为从句嵌入谓词选择完整CP补语。研究成功解释了方言特有的二分否定、附着化和CP嵌入等特征。分析表明极简方案能够有效处理这种复杂的句法结构。

Conclusion: 研究结论提出了将分析扩展到受话者从句kil-k'你说'的理论问题，并为极简方案的普遍性提供了见解。研究对生成句法特别是极简方案理论做出了贡献。
...
TL;DR: FAA是一种基于傅里叶特征的高效微调框架，通过频域分解实现参数高效的语言模型适配


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法在保持预训练模型能力的同时，需要更有效的语义信息调制机制

Method: 将随机傅里叶特征集成到轻量级适配器中，将中间表示分解为互补的低频和高频分量，实现频率感知的语义调制

Result: 在GLUE、E2E NLG和指令微调基准测试中，FAA相比现有方法达到竞争性或更优性能，同时保持低计算和内存开销

Conclusion: FAA是一种鲁棒且高效的大型语言模型后训练方法，频率感知激活和自适应加权机制有效提升了微调效果
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的人类活动识别方法依赖大量标注数据和纯几何范例选择，难以区分相似活动（如行走、上楼梯、下楼梯），需要结合语义信息提升少样本学习效果

Method: LLM生成的知识先验（特征重要性、类间混淆性、范例预算乘数）指导范例评分与选择，结合基于边界的验证线索、PageRank中心性、枢纽惩罚和设施位置优化

Result: 在UCI-HAR数据集少样本条件下达到88.78%的宏观F1分数，优于随机采样、聚类中心选择等传统方法

Conclusion: LLM衍生的语义先验与结构和几何线索结合，为少样本可穿戴传感器活动识别提供了更强大的范例选择基础
...
TL;DR: 本文提出HHEM轻量级幻觉检测框架，显著降低计算成本（从8小时降至10分钟），在QA任务中达到82.2%准确率，但在摘要任务中面临局部幻觉挑战，通过分段检索改进检测效果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的幻觉问题严重影响可信度，现有评估方法（如KnowHalu）计算成本高昂，需要开发更高效的幻觉检测框架。

Method: 集成Hughes幻觉评估模型（HHEM），这是一个不依赖LLM判断的轻量级分类框架；引入分段检索机制处理摘要任务中的局部幻觉；使用累积分布函数分析不同规模模型的幻觉表现。

Result: HHEM将评估时间从8小时减少到10分钟；在QA任务中，HHEM配合非捏造检查达到最高准确率82.2%和TPR 78.9%；分段检索改善了摘要任务中的局部幻觉检测；CDF分析显示较大模型（7B-9B参数）幻觉较少，中等规模模型表现不稳定。

Conclusion: 需要平衡计算效率和事实验证的结构化评估框架，HHEM为高效幻觉检测提供了可行方案，但摘要任务中的局部幻觉检测仍需改进，模型规模与幻觉频率存在相关性。
...
TL;DR: HiFi-RAG是一个多阶段检索增强生成系统，在MMU-RAGent竞赛中获胜，通过分层过滤和混合模型策略显著提升开放域问答性能。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统在开放域设置中面临检索文档包含无关信息和生成答案与用户意图不对齐的挑战，需要更有效的过滤和推理机制。

Method: 采用分层过滤多阶段管道：使用Gemini 2.5 Flash进行查询重构、分层内容过滤和引用归因，保留Gemini 2.5 Pro进行最终答案生成，实现成本效益与推理能力的平衡。

Result: 在MMU-RAGent验证集上，ROUGE-L提升19.6%至0.274，DeBERTaScore提升6.2%至0.677；在测试集上，ROUGE-L提升57.4%，DeBERTaScore提升14.9%。

Conclusion: HiFi-RAG通过分层过滤和混合模型策略有效解决了RAG中的无关信息问题，在开放域问答任务中取得了显著性能提升，证明了多阶段处理的价值。
...
TL;DR: 该研究评估大语言模型在会计领域的推理能力，建立了垂直领域会计推理评估标准，测试了GLM系列模型和GPT-4的表现，发现GPT-4表现最佳但仍未达到实际应用要求。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在各领域的应用扩展，如何将其有效整合到专业领域（特别是会计领域）成为推动企业数字化转型和社会发展的关键挑战。需要理解LLMs在特定领域的推理能力才能实现有效整合。

Method: 引入垂直领域会计推理概念，通过分析代表性GLM系列模型的训练数据特征建立评估标准。基于此框架，评估GLM-6B、GLM-130B、GLM-4和OpenAI GPT-4在会计推理任务上的表现，测试不同提示工程策略的效果。

Result: 实验结果显示，不同提示工程策略对模型性能有不同程度的提升，GPT-4展现出最强的会计推理能力。但当前LLMs仍无法满足实际应用需求，特别是在企业级会计场景部署方面需要进一步优化。

Conclusion: 大语言模型在会计领域具有应用潜力，但需要针对特定领域进行优化。GPT-4表现最佳但仍未达到实际部署要求，未来研究需要改进推理范式并提升模型在专业领域的性能，以充分发挥LLMs在会计领域的价值。
...
TL;DR: 该论文主张韩语选区树库应采用"어절"（eojeol）作为基本终端单位，而非语素，以区分词内形态和短语级句法结构，并实现与依存资源的兼容。


<details>
  <summary>Details</summary>
Motivation: 韩语选区树库设计面临一个基本表示问题：终端单位的选择。虽然韩语词具有复杂的形态结构，但将语素作为选区终端会混淆词内形态和短语级句法结构，并导致与基于어절的依存资源不匹配。

Method: 提出基于어절的选区表示方案，将形态分割和细粒度词性信息编码在独立的非成分层中。通过比较分析，在明确的规范化假设下，证明世宗和宾州韩语树库在어절选区层面具有表示等价性。

Result: 分析表明世宗和宾州韩语树库可以在어절选区层面被视为表示等价。基于此结果，设计了一个基于어절的标注方案，该方案保留了可解释的选区结构，并支持跨树库比较和选区-依存转换。

Conclusion: 韩语选区树库应采用어절作为基本终端单位，将形态信息分离到独立层中，这样既能保持选区结构的可解释性，又能实现与依存资源的兼容和跨树库的一致性。
...
TL;DR: 论文提出All-or-Here Attention (AHA)机制，让LLM能动态决定何时需要全局注意力，通过二元路由器在完整注意力和局部滑动窗口注意力间切换，显著减少计算量。


<details>
  <summary>Details</summary>
Motivation: 受人类阅读习惯启发——通常只关注当前页，必要时才回顾前文。现有LLM的全局注意力机制计算成本高且可能冗余，需要更高效的注意力机制。

Method: 提出AHA注意力机制：每个注意力头配备二元路由器，为每个token动态选择使用完整注意力或局部滑动窗口注意力。窗口大小可配置，实现局部处理与全局访问的解耦。

Result: 使用256token窗口时，93%的完整注意力操作可被滑动窗口注意力替代而不损失性能。不同窗口大小评估显示上下文依赖呈长尾分布，全局注意力需求随窗口扩大迅速衰减。

Conclusion: 完整注意力在很大程度上是冗余的，高效推理只需要按需访问全局上下文。AHA机制通过解耦局部处理和全局访问，实现了更高效的LLM推理。
...
<details>
  <summary>Details</summary>
Motivation: 理解多模态对话中的情感是构建情感智能AI系统的关键挑战。MCABSA挑战赛提出了两个具有挑战性的子任务：从多说话者对话中提取全面的情感六元组（包括持有者、目标、方面、观点、情感和理由），以及检测情感翻转（识别动态情感变化及其触发因素）。

Method: 对于子任务一，设计了结构化提示管道，引导大语言模型按顺序提取情感组件，并进行上下文理解细化。对于子任务二，通过集成三种大语言模型的互补优势，稳健地识别情感转换及其触发因素。

Result: 系统在子任务一上获得了47.38%的平均得分，在子任务二上获得了74.12%的精确匹配F1分数，显示了逐步细化和集成策略在多模态情感分析任务中的有效性。

Conclusion: 研究表明，结构化提示管道和模型集成策略能够有效处理复杂多模态对话中的情感分析任务，为构建情感智能AI系统提供了实用方法。
...
TL;DR: T3LLM提出了一种基于三个LLM的框架，通过生成-审查-学习机制实现时间序列问答的多步推理和显式纠错，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的时间序列分析方法主要采用通用自然语言处理技术，在处理复杂数值序列时容易出现推理错误。时间序列数据具有内在可验证性，可以在推理步骤和原始输入之间进行一致性检查。

Method: 提出T3LLM框架，包含三个LLM：工作者（生成结构化提示下的逐步思维链）、审查者（检查推理、识别错误步骤并提供纠正意见）、学生（通过纠正后的思维链进行微调，将多步推理和自我纠正内化到参数中）。

Result: 在多个真实世界时间序列问答基准测试中，T3LLM相比强大的基于LLM的基线方法取得了最先进的性能。

Conclusion: 通过利用时间序列数据的可验证性，T3LLM框架通过生成-审查-学习机制实现了有效的多步推理和显式纠错，显著提升了时间序列问答的性能。
...
TL;DR: M2G-Eval是一个多粒度、多语言的代码生成评估框架，涵盖类、函数、块、行四个粒度级别和18种编程语言，包含17K+训练任务和1,286个测试实例。研究发现：行级任务最简单，类级最困难；任务复杂度增加时，全粒度与部分粒度语言间性能差距扩大；存在强跨语言相关性，表明模型学习到了可迁移的编程概念。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成评估基准大多在单一结构粒度上进行评估，且局限于少数编程语言，无法揭示不同代码粒度和多语言场景下的细粒度能力差异。需要开发一个更全面的评估框架来系统分析LLM的代码生成能力。

Method: 提出M2G-Eval框架，包含类、函数、块、行四个粒度级别，覆盖18种编程语言。使用17K+训练任务和1,286个人工标注、污染控制的测试实例。通过监督微调和组相对策略优化训练Qwen3-8B模型，开发了M2G-Eval-Coder模型。评估了30个模型（28个SOTA LLM和2个M2G-Eval-Coder变体）。

Result: 1. 存在明显的难度层次：行级任务最简单，类级最困难；2. 随着任务复杂度增加，全粒度语言与部分粒度语言之间的性能差距扩大；3. 存在强跨语言相关性，表明模型学习到了可迁移的编程概念。

Conclusion: M2G-Eval能够对代码生成能力进行细粒度诊断，并突显了合成复杂、长篇代码方面的持续挑战。该框架为评估LLM的代码生成能力提供了更全面的视角。
...
TL;DR: 该论文分析了扩散模型在语言生成中的应用，指出现有方法在连续嵌入空间扩散和离散标记扩散之间存在结构性权衡，并识别出两个核心问题：均匀破坏不尊重信息分布，以及标记级边际训练无法捕捉并行解码中的多标记依赖关系。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在语言生成中具有并行解码和迭代细化的优势，但文本的离散性和高度结构化特性使得直接应用扩散原理面临挑战。论文旨在重新审视扩散语言建模，分析现有方法的局限性。

Method: 论文从扩散过程和语言建模的角度出发，提出了区分扩散机制与语言特定需求的五个属性。将现有方法分为连续嵌入空间扩散和离散标记扩散两类，分析它们各自满足的属性。通过分析最近的大型扩散语言模型，识别出核心问题。

Result: 研究发现现有方法都存在结构性权衡，每种方法只满足部分关键属性。具体识别出两个核心问题：1）均匀破坏不尊重信息在文本位置上的分布方式；2）标记级边际训练无法捕捉并行解码过程中的多标记依赖关系。

Conclusion: 这些观察结果促使需要开发更符合文本结构的扩散过程，鼓励未来研究朝着更一致的扩散语言模型方向发展，以更好地处理文本的离散和结构化特性。
...
TL;DR: GRPO在提升大语言模型思维链忠实度方面优于DPO，Qwen2.5-14B-Instruct模型在所有评估指标上表现最佳，GRPO显示出改善忠实度指标的更大潜力。


<details>
  <summary>Details</summary>
Motivation: 思维链推理虽然能提升大语言模型的问题解决能力，但研究发现其解释往往不能反映模型的真实推理过程，模型可能产生连贯但误导性的理由，这种差异削弱了基于思维链方法在安全监督和对齐监控中的可靠性。

Method: 评估两种优化方法：组相对策略优化（GRPO）和直接偏好优化（DPO），在提升思维链忠实度方面的能力。实验比较了不同规模模型的表现，重点关注Qwen2.5-14B-Instruct等模型。

Result: GRPO在较大模型中表现优于DPO，Qwen2.5-14B-Instruct模型在所有评估指标上取得最佳结果。两种方法都显示模型规模与性能呈正相关，但GRPO在改善忠实度指标方面显示出更大潜力，尽管在较小规模时行为不太稳定。

Conclusion: GRPO为开发更透明和可信赖的大语言模型推理提供了有前景的方向，有助于解决思维链解释与实际推理过程不一致的问题，提升模型的可信度。
...
TL;DR: 本文研究了大型语言模型中的不确定性量化问题，提出了词汇感知的共形预测框架，在保证覆盖率的同时将预测集大小从847个token减少到4.3个，效率提升197倍。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域部署大型语言模型需要严格的不确定性量化，但标准的softmax概率通常校准效果不佳。传统的共形预测方法虽然能保证覆盖率，但产生的预测集包含数百个token，信息量不足。

Method: 提出了词汇感知共形预测框架，通过语义掩码和温度调整评分来减少有效预测空间，同时理论上保证边际覆盖率。该方法在Gemma-2B模型上使用SQUAD和WikiText基准进行实验。

Result: VACP实现了89.7%的经验覆盖率（目标为90%），同时将平均预测集大小从847个token减少到4.3个token，效率提升了197倍。

Conclusion: 词汇感知共形预测框架在保持覆盖率的同时显著提高了预测集的效率，为大型语言模型在高风险领域的部署提供了有效的校准方法。
...
<details>
  <summary>Details</summary>
Motivation: 希望言论在自然语言处理中相对被忽视，现有研究主要集中在英语，导致乌尔都语等低资源语言缺乏相关工具。虽然Transformer架构在仇恨和攻击性言论检测中表现良好，但在希望言论检测和多语言环境中的应用仍有不足。

Method: 使用预训练的Transformer模型（XLM-RoBERTa、mBERT、EuroBERT、UrduBERT），应用简单预处理并训练分类器。在PolyHope-M 2025基准上进行评估，重点关注乌尔都语的二元和多元分类。
...
TL;DR: 研究发现大型语言模型在多语言推理中存在盲点：虽然任务准确率高，但推理过程与结论常不匹配，非拉丁文字语言的推理-结论错位率是拉丁文字语言的两倍以上。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型通过思维链提示展现的推理能力是否能在不同语言间有效传递，现有多语言评估方法是否完整反映了模型的真实推理能力。

Method: 引入人工验证框架评估模型生成的推理轨迹是否在逻辑上支持其结论；分析6种语言、6个前沿模型在GlobalMMLU问题上的65k条推理轨迹；通过人工标注建立错误分类法。

Result: 发现模型虽任务准确率高，但推理常无法支持结论；非拉丁文字语言的推理-结论错位率至少是拉丁文字语言的两倍；主要错误类型为证据错误（无支持的主张、模糊事实）和不合逻辑的推理步骤。

Conclusion: 当前多语言评估实践对模型推理能力的描述不完整，需要开发推理感知的评估框架来更全面地评估模型的多语言推理能力。
...
TL;DR: 通过心理学的提示词设计来缓解大语言模型在模拟人口调查时的社会期望偏差，提高硅样本与真实人类数据的对齐度


<details>
  <summary>Details</summary>
Motivation: 大语言模型在模拟人口响应时存在社会期望偏差，倾向于给出社会可接受的答案而非真实人类数据，现有研究对此关注有限，需要探索有效的缓解方法

Method: 使用美国国家选举研究数据，在两个模型系列（Llama-3.1和GPT-4.1-mini）上测试四种基于提示的缓解方法：重新表述（中性、第三人称）、反向编码（语义反转）、启动和序言

Result: 重新表述提示最有效改善对齐，减少对社会可接受答案的分布集中度；反向编码在不同项目上效果不一；启动和序言鼓励响应一致性但对偏差缓解无系统性益处

Conclusion: 基于提示的框架控制能有效缓解大语言模型固有的社会期望偏差，为获得更具代表性硅样本提供了实用路径
...
<details>
  <summary>Details</summary>
Motivation: 婴儿死亡率在美国仍是重大公共卫生问题，出生缺陷是主要原因之一。尽管已有研究努力理解不良妊娠结局（如流产、死产、出生缺陷和早产）的原因，但仍需要更全面的研究和干预策略。当前数据集有限，需要新的数据源来增强观察性研究。

Method: 提出一种新颖方法，利用公开可用的社交媒体数据（特别是Twitter）来增强现有数据集。构建自然语言处理（NLP）流水线，自动识别分享怀孕经历的女性，并根据报告结果进行分类：报告足月和正常出生体重的为阳性病例，报告不良妊娠结局的为阴性病例。处理社交媒体数据面临的挑战（不平衡、噪声、缺乏结构）需要强大的预处理技术和数据增强策略。

Result: 该方法展示了社交媒体数据作为流行病学研究中关于妊娠结局的辅助资源的可行性。提供了评估特定干预措施、治疗或产前暴露对母婴健康结果因果影响的潜在应用。为未来涉及孕妇队列和比较组的健康研究提供了框架。

Conclusion: 社交媒体数据可以作为研究不良妊娠结局的补充数据源，通过NLP技术自动识别和分类怀孕经历，为流行病学调查提供新的途径。该方法不仅增强了现有数据集，还为评估干预措施和建立研究框架提供了可能性。
...
<details>
  <summary>Details</summary>
Motivation: 自回归(AR)生成在推理时缺乏并行性，而扩散语言模型(DLLMs)虽然支持并行解码，但由于依赖双向注意力而无法有效利用前缀KV缓存，导致实际部署效率不高

Method: 提出WeDLM框架：1) 使用标准因果注意力使并行生成兼容前缀缓存；2) 通过拓扑重排序将已观测token移动到物理前缀同时保持逻辑位置；3) 流式解码过程持续提交置信token到增长的前缀中，维持固定并行工作量

Result: WeDLM在保持强AR骨干模型质量的同时实现显著加速：在复杂推理基准上接近3倍加速，在低熵生成场景下达到10倍加速，在匹配部署设置下优于vLLM优化的AR引擎

Conclusion: 通过完全基于因果注意力的设计，WeDLM证明了扩散式解码在实践中可以超越优化的AR引擎，为并行语言模型解码提供了有效的解决方案
...
TL;DR: 研究探讨了在乌尔都语假新闻检测中，通过领域自适应预训练提升多语言模型性能的方法，发现XLM-RoBERTa在领域适应后表现一致提升，而mBERT结果混合。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的虚假信息是一个普遍问题，但乌尔都语等低资源语言在此领域研究有限。多语言预训练模型在特定领域术语上表现不佳，需要改进。

Method: 采用分阶段训练方法：首先对XLM-RoBERTa和mBERT进行领域自适应预训练，使用公开的乌尔都语新闻语料库；然后在下游假新闻分类任务上进行微调。

Result: 在四个公开乌尔都语假新闻数据集上的实验表明，领域自适应的XLM-RoBERTa始终优于原始版本，而领域自适应的mBERT表现结果混合。

Conclusion: 领域自适应预训练能有效提升多语言模型在乌尔都语假新闻检测中的性能，特别是对XLM-RoBERTa模型效果显著，为低资源语言虚假信息检测提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 临床笔记通常以非结构化或半结构化格式存储，这限制了其在二次分析和下游临床应用中的使用。可靠的章节边界识别是结构化这些笔记的关键步骤，因为不同章节（如现病史、药物、出院指导）提供不同的临床上下文。

Method: 使用MIMIC-IV中1000份临床笔记的精选数据集，评估了基于规则的基线方法、领域特定的Transformer模型和大型语言模型在临床笔记分割任务上的表现。实验包括句子级和自由文本分割两种任务。

Result: 大型API模型（如GPT-5-mini）在整体性能上表现最佳，在句子级和自由文本分割上的平均F1分数达到72.4。轻量级基线方法在结构化句子级任务上仍有竞争力，但在非结构化自由文本任务上表现不佳。
...
TL;DR: 提出自动化合成高难度但易验证任务的模拟环境统一流程，以及环境级强化学习算法，解决用户不稳定性和环境异质性问题，提升训练效率和稳定性。


<details>
  <summary>Details</summary>
Motivation: 在模拟环境中进行强化学习具有成本效益和可扩展性，但先前工作局限于半自动化环境合成或任务难度不足，且模拟用户不稳定性和环境异质性对智能体强化学习构成挑战。

Method: 提出两个核心贡献：(1) 自动化、可扩展的高难度易验证任务模拟环境合成统一流程；(2) 环境级强化学习算法，有效缓解用户不稳定性，并在环境层面进行优势估计。
...
TL;DR: 本文提出了一种将预训练目标重新解释为策略梯度优化的方法，通过奖励塑造策略平衡多样性和精确性，为后续强化学习提供更优的探索空间。


<details>
  <summary>Details</summary>
Motivation: 强化学习能提升大语言模型的推理能力，但其效果严重依赖于预训练模型的token输出分布所定义的探索空间。现有交叉熵损失作为单步episode的策略梯度优化实例，未能系统研究预训练分布如何塑造后续RL的探索潜力。

Method: 将预训练目标推广为基于策略的RL原则，将下一个token预测框架化为随机决策过程。引入奖励塑造策略：使用正奖励缩放因子控制ground-truth token的概率集中度，采用排名感知机制对高排名和低排名负token进行非对称处理，从而重塑预训练的token输出分布。

Result: 研究发现，与直觉相反，追求精确性导向的先验分布比高熵分布能为RL提供更优的探索空间，最终提升端到端推理性能。

Conclusion: 通过将监督学习重新框架化为策略优化问题，并引入平衡多样性和精确性的奖励塑造机制，可以系统性地优化预训练分布，为后续强化学习提供更有效的探索空间，从而提升大语言模型的推理能力。
...
TL;DR: 评估三种先进大语言模型在临床决策支持中的表现，发现模型性能因任务而异，提示工程并非通用解决方案，其效果高度依赖模型和任务


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在医学知识评估中表现出潜力，但其在真实世界临床决策中的实际效用仍待探索，需要评估LLMs在整个临床推理工作流程中的表现

Method: 使用36个案例研究评估ChatGPT-4o、Gemini 1.5 Pro和LIama 3.3 70B在五种临床决策任务中的表现，包括两种温度设置，并应用MedPrompt框架的变体进行提示工程，结合有针对性和随机的动态少样本学习

Result: 所有模型在不同任务中表现差异显著：最终诊断接近完美准确率，相关诊断测试表现差，其他任务表现中等；ChatGPT在零温度下表现更好，LIama在默认温度下更强；提示工程能显著改善基线准确率最低的任务，但对其他任务可能适得其反；有针对性的动态少样本提示并不总是优于随机选择

Conclusion: 提示工程的效果高度依赖模型和任务，表明将LLMs整合到医疗保健中需要量身定制、上下文感知的策略，而不是一刀切的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然性能出色但计算和存储成本高昂。现有后训练结构化剪枝方法在少样本校准集不能充分反映预训练数据分布时，对下游任务的泛化能力有限。

Method: 提出功能感知神经元分组(FANG)框架：1)基于处理的语义上下文类型对具有相似功能的神经元进行分组；2)在每个组内进行重要性估计时，给予与神经元组功能角色强相关的token更高权重；3)保留跨多个上下文类型贡献的神经元；4)根据每个块的功能复杂度自适应分配稀疏度。

Result: FANG在保持语言建模性能的同时提高了下游任务准确率。与FLAP和OBC两种代表性剪枝方法结合时达到最先进水平，在30%和40%稀疏度下平均准确率分别提升1.5%-8.5%。

Conclusion: FANG通过基于功能的神经元分组有效缓解了校准偏差问题，实现了稀疏度与性能之间的更好权衡，为后训练剪枝提供了一种有效的解决方案。
...
TL;DR: LENS框架将多模态健康传感数据与语言模型对齐，生成基于临床的心理健康叙事，解决了传感器数据到自然语言转换的挑战。
...
Method: 1) 构建大规模数据集：将抑郁和焦虑症状相关的生态瞬时评估(EMA)响应转换为自然语言描述，从258名参与者中获得超过10万个传感器-文本问答对；2) 训练补丁级编码器：将原始传感器信号直接投影到LLM的表示空间，实现原生时间序列集成。
...
TL;DR: 论文批评Biasing Features指标将思维链不忠实与不完整混淆，提出新指标显示更大推理预算可显著提高提示线索的显性表达，建议使用更全面的可解释性工具包


<details>
  <summary>Details</summary>
Motivation: 现有研究使用Biasing Features指标将思维链标记为不忠实，但该指标混淆了不忠实性与不完整性，后者是将分布式transformer计算转换为线性自然语言叙述所需的损失压缩

Method: 1) 在多跳推理任务上测试Llama-3和Gemma-3模型；2) 提出新的faithful@k指标；3) 使用因果中介分析研究非显性表达提示线索的因果作用

Result: 1) 许多被Biasing Features标记为不忠实的思维链被其他指标判断为忠实，某些模型中超过50%；2) 更大的推理时token预算显著提高提示线索显性表达，某些设置下可达90%；3) 因果中介分析显示即使非显性表达的提示线索也能通过思维链因果中介预测变化

Conclusion: 应谨慎依赖基于提示线索的评估方法，提倡使用更全面的可解释性工具包，包括因果中介分析和基于破坏的指标
...
TL;DR: Prompt Choreography框架通过动态全局KV缓存优化多智能体LLM工作流，显著降低延迟并提升执行效率


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在多智能体工作流中的部署增加，存在大量冗余计算问题，需要一种高效执行框架来减少重复编码和提升性能

Method: 提出Prompt Choreography框架，维护动态全局KV缓存，使每个LLM调用可以关注任意重新排序的先前编码消息子集，支持并行调用，并通过微调LLM使其适应缓存机制

Result: 显著降低每消息延迟（首token时间快2.0-6.2倍），在某些以冗余计算为主的工作流中实现超过2.2倍的端到端加速

Conclusion: Prompt Choreography通过缓存重用和微调技术，为多智能体LLM工作流提供了高效的执行框架，大幅提升性能
...
TL;DR: 本文介绍了TabiBERT，一个基于ModernBERT架构从头开始训练的单语土耳其语编码器，在1万亿token的多领域语料上预训练，支持8192-token上下文长度，在土耳其语NLP基准测试中超越了现有模型。


<details>
  <summary>Details</summary>
Motivation: 土耳其语NLP缺乏一个从头开始训练、包含现代架构范式（如RoPE、FlashAttention等）的单语编码器。现有模型未能充分利用BERT之后在计算效率、训练稳定性和长上下文建模方面的进展。

Method: 基于ModernBERT架构，整合Rotary Positional Embeddings (RoPE)、FlashAttention和精炼的归一化技术。在848.8亿token的多领域语料库（网页文本73%、科学出版物20%、源代码6%、数学内容0.3%）中采样1万亿token进行预训练。创建TabiBench基准，包含28个数据集、8个任务类别，使用GLUE风格的宏平均进行评估。

Result: TabiBERT支持8192-token上下文长度（是原始BERT的16倍），推理速度提升达2.65倍，减少GPU内存消耗，支持更大批量大小。在TabiBench上获得77.58分，比BERTurk高1.62分，在8个类别中的5个达到最先进水平：问答（+9.55）、代码检索（+2.41）、文档检索（+0.60）。与特定任务的最佳结果相比，平均提升+1.47分，显示出强大的跨领域泛化能力。

Conclusion: TabiBERT填补了土耳其语NLP中现代单语编码器的空白，通过整合最新的架构改进，在多个任务类别中建立了新的最先进水平，并展示了良好的跨领域泛化能力。作者发布了模型权重、训练配置和评估代码，以促进透明、可复现的土耳其语编码器研究。
...
TL;DR: 提出一种基于储层计算的无矩阵乘法语言模型，通过固定共享权重和插入储层层来减少计算成本，在保持性能的同时降低参数、训练和推理时间。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言处理中表现出色，但高计算成本仍是主要瓶颈。本研究旨在通过无矩阵乘法语言模型和储层计算架构来提高计算效率。

Method: 1) 采用无矩阵乘法语言模型；2) 部分固定和共享选定层的权重；3) 插入储层层以获得丰富的动态表示而无需额外训练开销；4) 组合多个操作以减少内存访问。

Result: 实验结果显示，提出的架构将参数数量减少高达19%，训练时间减少9.9%，推理时间减少8.0%，同时保持与基线模型相当的性能。

Conclusion: 通过结合无矩阵乘法架构和储层计算技术，可以在保持语言模型性能的同时显著降低计算成本，为解决LLM计算瓶颈提供了一种有效方法。
...
TL;DR: 该研究提出了BiomedTLDR数据集，用于评估LLM生成科学论文摘要的能力，发现LLM生成的摘要更倾向于提取式而非抽象式，与人类专家存在差异。


<details>
  <summary>Details</summary>
Motivation: 高质量的科学摘要有助于有效科学传播，但目前缺乏全面的高质量科学摘要数据集，阻碍了LLM摘要能力的开发和评估。

Method: 提出BiomedTLDR数据集，包含大量研究者撰写的科学论文摘要；测试流行的开源LLM基于摘要生成TLDR的能力。

Result: 分析显示，虽然部分LLM能成功生成类似人类的摘要，但总体上LLM更倾向于使用原文的词汇选择和修辞结构，相比人类更偏向提取式而非抽象式摘要。

Conclusion: 研究提供了BiomedTLDR数据集和代码，揭示了LLM在科学摘要生成方面的局限性，为未来改进LLM的摘要能力提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 为了利用多个LLM的不同优势，通过集体智慧选择最理想的响应，同时保持方法的清晰可解释性和无监督特性，以实现灵活的适应性和泛化能力。

Method: 基于同行评审框架的三阶段方法：1) 评分阶段：使用LLM-as-a-Judge技术让多个LLM相互评估每个响应；2) 推理阶段：使用基于图模型的真值推断算法或简单平均策略聚合多个评分，为每个响应生成最终分数；3) 选择阶段：选择最高分的响应作为最佳集成输出。

Result: 该方法在四个数据集上取得了强劲结果，两个变体分别比先进的Smoothie-Global模型高出6.9%和7.3个百分点。

Conclusion: LLM-PeerReview是一个概念简单但经验上强大的方法，通过无监督的LLM集成框架，利用同行评审机制有效选择最佳响应，在多个数据集上表现出色。
...
TL;DR: LLMs在复杂编程任务中易出错，研究者提出Anka DSL来约束语法，显著提升代码生成准确率


<details>
  <summary>Details</summary>
Motivation: LLMs在代码生成方面表现出色，但在复杂的多步骤编程任务中仍存在系统性错误。研究者假设这些错误源于通用语言的灵活性，允许多种有效方法并需要隐式状态管理，因此需要设计专门的领域特定语言来减少歧义。

Method: 引入Anka，一种用于数据转换管道的领域特定语言(DSL)，具有明确、受约束的语法设计。尽管LLMs从未接触过Anka，但通过上下文提示学习，在100个基准问题上进行测试，并与Python进行对比。

Result: Claude 3.5 Haiku在Anka上达到99.9%解析成功率和95.8%总体任务准确率。在多步骤管道任务中，Anka比Python有40个百分点的准确率优势(100% vs. 60%)。GPT-4o-mini的交叉验证也确认了这种优势(+26.7个百分点)。

Conclusion: LLMs可以通过上下文提示完全学习新的DSL，达到接近原生的准确率；受约束的语法能显著减少复杂任务中的错误；为LLM生成专门设计的领域特定语言可以超越LLMs有大量训练的通用语言。
...
<details>
  <summary>Details</summary>
Motivation: 当前的低秩适配方法（如LoRA）假设任务相关的权重更新存在于低秩子空间中，但该子空间是以黑盒方式从数据中隐式学习的，缺乏可解释性和直接控制。作者认为这种困难源于多义性——单个维度编码多个纠缠概念。

Method: 利用预训练的稀疏自编码器（SAE）在解耦的特征空间中识别任务相关特征，然后构建显式、可解释的低秩子空间来指导适配器初始化。理论分析证明，在单义性假设下，基于SAE的子空间识别可以实现任意小的恢复误差。

Result: 在安全对齐任务中，该方法实现了高达99.6%的安全率，超过全参数微调7.4个百分点，接近基于RLHF的方法，同时仅更新0.19-0.24%的参数。更重要的是，通过SAE特征的语义基础，为学习到的对齐子空间提供了可解释的洞察。

Conclusion: 将机制可解释性融入微调过程可以同时提高性能和透明度。该方法不仅参数高效，而且提供了对模型行为的深入理解，为可解释的高效微调开辟了新途径。
...
TL;DR: 该研究针对中国电商直播中主播使用语音变形词规避监管的问题，提出了LiveAMR任务，构建了首个相关数据集，并开发了基于LLM的数据增强方法，显著提升了直播监管效果。


<details>
  <summary>Details</summary>
Motivation: 中国电商直播（特别是抖音平台）已成为主要销售渠道，但主播经常使用语音变形词逃避审查并进行虚假广告宣传。现有研究主要关注社交媒体和地下产业的文本变形，缺乏对直播中语音变形词的检测方法。

Method: 提出了Live Auditory Morph Resolution (LiveAMR)任务，构建了包含86,790个样本的首个LiveAMR数据集。将任务转化为文本到文本生成问题，并利用大语言模型生成额外训练数据来提升性能。

Result: 开发的方法显著提升了直播监管效果，证明了语音变形词解析对直播监管的重要性。通过LLM数据增强，模型性能得到改善。

Conclusion: LiveAMR任务有效解决了电商直播中语音变形词检测问题，构建的数据集和方法为直播监管提供了重要工具，语音变形词解析显著增强了直播监管能力。
...
<details>
  <summary>Details</summary>
Motivation: 有声书解读虽然能为读者提供可访问的深度分析和实用见解，但人工制作过程耗时且资源密集，需要自动化解决方案来应对这一挑战。

Method: 开发了一个包含11个专门智能体的多智能体协作框架，包括主题分析师、案例分析师、编辑、叙述者和校对员等，共同探索主题、提取现实案例、优化内容组织并合成自然口语。

Result: 与专家解读相比，AI4Reading在语音生成质量上仍有差距，但生成的解读脚本更加简洁和准确。

Conclusion: AI4Reading系统通过多智能体协作成功实现了准确内容保留、增强可理解性和逻辑叙事结构的目标，为自动化有声书解读提供了可行方案。
...
TL;DR: 该论文系统性地综合了记忆的跨学科知识，将认知神经科学的见解与基于大语言模型的智能体连接起来，探讨了记忆的定义、功能、分类、存储机制、管理生命周期、评估基准、安全性以及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 现有自主智能体研究在记忆工作流设计上受到跨学科障碍的限制，难以吸收人类记忆机制的本质。为了弥合这一差距，需要系统性地综合记忆的跨学科知识，连接认知神经科学与基于大语言模型的智能体。

Method: 1. 阐明从认知神经科学到LLM再到智能体的渐进轨迹中记忆的定义和功能；2. 从生物和人工角度对记忆分类、存储机制和完整管理生命周期进行对比分析；3. 回顾评估智能体记忆的主流基准；4. 从攻击和防御双重视角探索记忆安全性；5. 展望未来研究方向。

Result: 论文提供了一个系统性的跨学科记忆分析框架，连接了认知神经科学与AI智能体，为理解、评估和设计智能体记忆系统提供了理论基础和实践指导。

Conclusion: 通过系统性地综合记忆的跨学科知识，论文为基于大语言模型的智能体记忆研究提供了重要框架，并指出了未来研究方向，特别是多模态记忆系统和技能获取领域。
...
TL;DR: 提出基于外部子图生成的逐步推理增强框架SGR，通过构建查询相关子图并利用其语义结构指导推理过程，提升LLMs在复杂推理任务中的准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理需要深度推理和逻辑推断的任务时面临挑战，模型可能从大规模文本语料中吸收噪声或不相关信息，导致预测错误或与事实知识不一致的输出。

Method: 提出SGR框架：1）为输入查询动态构建外部知识库中的相关子图；2）基于结构化子图引导模型进行多步逐步推理；3）整合多个推理路径生成最终答案。

Result: 在多个基准数据集上的实验结果表明，SGR框架持续优于强基线方法，证明了其在增强LLMs推理能力方面的有效性。

Conclusion: 基于外部子图生成的逐步推理框架能够减少噪声信息的影响，提高推理准确性，为增强LLMs在复杂推理任务中的表现提供了有效方法。
...
TL;DR: 本文提出EntroDrop方法，通过基于熵的token dropout来解决多轮训练中自回归模型性能下降的问题，在数据受限领域有效提升LLM适应能力。


<details>
  <summary>Details</summary>
Motivation: 随着高质量领域特定数据日益稀缺，多轮训练成为适应大语言模型的实用策略。然而，自回归模型在重复数据暴露下常出现性能下降，过拟合导致模型能力显著衰退。研究发现这种退化源于学习动态不平衡：可预测的低熵token被快速学习并主导优化，而模型在高熵token上的泛化能力随着持续训练而恶化。

Method: 提出EntroDrop方法，这是一种基于熵引导的token dropout技术，作为结构化数据正则化手段。该方法在训练过程中选择性地屏蔽低熵token，并采用课程学习调度来根据训练进度调整正则化强度。

Result: 在0.6B到8B参数规模的模型上进行实验，结果显示EntroDrop始终优于标准正则化基线方法，并在扩展的多轮训练中保持稳健性能。

Conclusion: 研究强调了在有限数据训练时，将正则化与token级学习动态对齐的重要性。EntroDrop方法为数据受限领域中更有效地适应大语言模型提供了有前景的途径。
...
TL;DR: 性别多样性对科研团队影响力的关系呈倒U型，女性领导组+男性支持组的组合表现最佳，团队规模对领导组性别多样性的影响有调节作用


<details>
  <summary>Details</summary>
Motivation: 现有关于性别多样性对科研团队成功影响的研究结果不一致，且大多采用聚合指标，忽视了团队内部角色分化。需要更细致地理解性别多样性如何影响团队影响力，特别是不同团队角色中的性别多样性效应。

Method: 将科研团队定义为论文的所有合著者，通过五年引用次数衡量团队影响力。利用作者贡献声明将成员分为领导角色和支持角色。基于PLOS期刊超过13万篇论文（主要是生物医学领域），采用多变量回归分析这些角色中性别多样性与团队影响力的关联。同时应用阈值回归模型研究团队规模如何调节这种关系。

Result: 1. 性别多样性与团队影响力的关系在领导组和支持组中都呈倒U型；2. 全女性领导组+全男性支持组的团队比其他团队类型获得更高影响力；3. 领导组性别多样性的影响在小团队中显著为负，但在大团队中变为正且统计不显著。相比之下，支持组性别多样性的估计值无论团队大小都保持显著为正。

Conclusion: 性别多样性对科研团队影响力的影响因角色类型和团队规模而异，需要更细致的角色分化视角来理解性别多样性的复杂效应。全女性领导组+全男性支持组的组合表现最佳，这为科研团队组建提供了重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM嵌入常用于向量数据库中的语义相似性检索，但文本语料库中的全局语义层次关系往往不透明。需要一种能够自动发现文本语义层次结构的方法，无需预定义类别。

Method: 采用嵌套密度聚类方法：首先在LLM嵌入空间中识别语义高度相似的密集文本簇，然后逐步放宽密度标准，使密集簇合并为更扩散的簇，最终形成覆盖整个数据集的单一根簇，构建出层次树结构。

Result: 方法成功应用于科学摘要分类、20 Newsgroups和IMDB 50k电影评论等基准数据集，展示了跨领域的鲁棒性。能够数据驱动地发现研究领域及其子领域，揭示文本数据集的语义结构和演化。

Conclusion: 嵌套密度树方法能够有效揭示文本数据中的层次语义关系，在科学计量学、主题演化等领域具有应用潜力，为理解文本语义结构提供了新的分析工具。
...
TL;DR: ACT算法通过三阶段流程自动检测拉比文献中的圣经引用，结合形态感知对齐和上下文丰富化，在F1分数0.91上优于现有系统，并能识别复杂引用模式如"波浪"和"回声"引用。


<details>
  <summary>Details</summary>
Motivation: 现有文本重用框架难以处理拉比文献中简短、转述或结构嵌入的圣经引用，需要一种能识别复杂引用模式并弥合机器检测与人工编辑判断之间方法差距的解决方案。

Method: ACT采用三阶段算法：1) 形态感知对齐算法处理词形变化；2) 上下文敏感丰富化阶段识别复杂引用模式（如"波浪"和"回声"引用）；3) 配置不同版本（ACT-QE完整版、ACT-2无风格丰富化、ACT-3使用更长n-gram）以评估各组件贡献。

Result: 完整ACT管道（ACT-QE）在F1分数0.91上优于所有基线系统，召回率0.89，精确率0.94。ACT-2（无风格丰富化）召回率更高（0.90）但精确率下降，ACT-3（使用更长n-gram）在覆盖率和特异性之间提供权衡。

Conclusion: ACT不仅改进了引用检测，其跨语料库分类风格模式的能力为体裁分类和互文分析开辟了新途径，为数字人文和计算文献学提供了基础，特别适用于形态丰富、引用密集的传统如阿加达文献。
...
TL;DR: 视觉语言大模型正朝着视觉理解和视觉生成任务统一的方向发展。本文通过在大规模预训练（>200M样本）下分析统一模型UniHetero，发现：生成能提升理解，但前提是生成语义而非像素；生成显示出更好的数据缩放趋势和更高的数据利用率；输入嵌入的自回归能有效捕捉视觉细节。


<details>
  <summary>Details</summary>
Motivation: 探索视觉语言大模型中生成任务是否能增强理解任务，特别是在大规模数据（超过2亿样本）的预训练场景下。目前视觉理解和视觉生成的统一模型趋势明显，但生成是否真正能提升理解能力尚未在大规模数据上得到充分验证。

Method: 提出UniHetero统一模型结构，在大规模预训练（超过2亿样本）下进行分析。主要方法包括：1）对比生成语义与生成像素的效果差异；2）研究生成任务的数据缩放趋势和数据利用率；3）采用输入嵌入的自回归方法来捕捉视觉细节。

Result: 关键发现：1）生成确实能提升理解能力，但仅限于生成语义内容，生成像素则无效；2）生成任务显示出比纯理解任务更优的数据缩放趋势和更高的数据利用率；3）输入嵌入的自回归方法能有效捕捉视觉细节信息。

Conclusion: 在大规模预训练下，视觉语言统一模型中生成语义内容能够有效增强理解能力，生成任务具有更好的数据缩放特性和更高的数据效率，输入嵌入的自回归是捕捉视觉细节的有效方法。这为构建更高效的视觉语言统一模型提供了重要指导。
...
TL;DR: MoLaCE是一个轻量级推理时框架，通过混合不同潜在概念激活强度的专家来减少LLM的确认偏误，在保持计算效率的同时达到或超越多智能体辩论的效果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在严重的输入确认偏误问题，当提示暗示偏好答案时，模型会强化这种偏误而非探索替代方案。这种现象在基础模型中已有害，在多智能体辩论中风险更大，因为回声室效应会强化偏误而非纠正。

Method: 提出MoLaCE框架，通过混合以不同潜在概念激活强度实例化的专家来解决确认偏误。关键洞见是：由于语言具有组合性，不同措辞的提示会以特定方式重新加权影响事实正确性的潜在概念，因此无法对所有输入应用单一固定干预。

Result: 实证研究表明，MoLaCE能持续减少确认偏误，提高鲁棒性，在仅需一小部分计算量的情况下，达到或超越多智能体辩论的效果。

Conclusion: MoLaCE使单个LLM能够在内部模拟辩论的好处，同时保持计算效率和可扩展性，也可集成到多智能体辩论框架中以多样化视角并减少相关错误。
...
TL;DR: 本文提出了一种利用知识图谱改进大语言模型幻觉自检测的方法，通过将LLM响应转换为实体关系图来估计幻觉概率，相比现有方法在准确率和F1分数上获得显著提升。


<details>
  <summary>Details</summary>
Motivation: 幻觉（生成看似可信但虚假的陈述）是LLM安全部署的主要障碍。现有自检测方法表现良好，但仍有改进空间。本文探索利用结构化知识表示（知识图谱）来提升幻觉自检测能力。

Method: 提出简单而强大的方法：1）将LLM响应转换为实体和关系的知识图谱；2）利用这些图谱估计响应包含幻觉的可能性。该方法低成本、模型无关，使用GPT-4o和Gemini-2.5-Flash在两个幻觉检测数据集上进行评估。

Result: 相比标准自检测方法和最先进的SelfCheckGPT方法，该方法在准确率上获得高达16%的相对提升，F1分数提升20%。LLM在分析原子事实时，即使初始输出包含不准确信息，当事实被结构化为知识图谱时表现更好。

Conclusion: 知识图谱能够显著提升LLM幻觉自检测性能，这种低成本、模型无关的方法为开发更安全、更可信的语言模型铺平了道路。研究还发布了手动整理增强的数据集以支持更可靠的未来基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在整合视觉能力并进行视觉指令微调后，常常无法保持原有语言模型的指令跟随能力，导致无法按照任务指令预期执行任务

Method: 构建新的训练数据集，突出显示是否指定了输出格式，研究在微调过程中明确指示输出格式如何影响LVLMs的指令跟随能力

Result: 定量评估证实LVLMs在使用常用数据集微调后指令跟随能力确实下降；包含输出格式说明的训练数据集训练的模型比不包含的模型更能准确遵循指令

Conclusion: 在视觉指令微调过程中包含带有输出格式说明的样本可能有助于缓解指令跟随能力的下降
...
TL;DR: 语音语言模型在多轮对话中无法保持指定的说话风格，存在风格遗忘问题，即使能回忆起风格指令也无法在表达中体现。


<details>
  <summary>Details</summary>
Motivation: 研究发现语音语言模型在对话开始时被要求使用特定说话风格后，经过几轮交互后无法保持该风格，这种"风格遗忘"现象影响模型在需要持续风格一致性的应用场景中的表现。

Method: 评估了3个专有模型和2个开源语音语言模型，关注副语言风格（情感、口音、音量、语速）。通过多轮对话测试，分析模型在系统消息和用户消息中接收风格指令时的表现差异，并测试了显式要求模型回忆风格指令的缓解策略。

Result: 所有测试模型都无法在指令下保持一致的说话风格；模型能够回忆起风格指令但无法在表达中体现；显式要求回忆风格指令能部分缓解遗忘问题；模型在系统消息中接收指令时表现更差，这与系统提示的预期功能相矛盾。

Conclusion: 语音语言模型存在严重的风格遗忘问题，当前模型架构和训练方法需要改进以支持长期风格一致性，系统提示的有效性需要重新评估。
...
TL;DR: InfTool是一个完全自主的多智能体框架，通过自演化的合成方法解决LLM调用外部工具的三大挑战，无需人工标注，将基础模型在BFCL上的准确率从19.8%提升到70.9%


<details>
  <summary>Details</summary>
Motivation: 当前LLM可靠调用外部工具存在三大挑战：1）高质量轨迹需要昂贵的人工标注；2）对未见工具的泛化能力差；3）单模型合成存在质量上限，会延续偏见和覆盖缺口

Method: InfTool采用自演化的多智能体合成框架，包含三个协作智能体：用户模拟器、工具调用助手和MCP服务器。给定原始API规范，生成从单次调用到复杂多步工作流的多样化验证轨迹。通过闭环循环：合成数据通过带门控奖励的组相对策略优化（GRPO）训练模型，改进的模型生成更高质量数据针对能力缺口，无需人工干预

Result: 在伯克利函数调用排行榜（BFCL）上，InfTool将基础32B模型的准确率从19.8%提升到70.9%（+258%），超越10倍大的模型，媲美Claude-Opus，完全基于合成数据无需人工标注

Conclusion: InfTool通过完全自主的多智能体合成框架，成功解决了LLM调用外部工具的关键瓶颈，实现了从原始API规范到高质量工具调用能力的自动化提升，为自主智能体发展提供了重要突破
...
TL;DR: 研究语言模型概率与认知现象关系时，探讨需要多少上下文语境，发现n-gram表示足以作为认知规划单元


<details>
  <summary>Details</summary>
Motivation: 研究语言模型概率与认知现象关系时，需要确定必要的上下文语境量，探索是否必须使用完整话语才能观察到概率缩减现象

Method: 通过比较完整话语和n-gram表示，研究语言模型概率与认知现象的关系，验证n-gram是否足以作为认知规划单元

Result: 研究发现n-gram表示足以作为认知规划单元，无需完整话语即可观察到概率缩减现象
...
Method: 构建包含约500篇ICML接受论文的数据集，在文档中嵌入四种不同语言的隐藏对抗提示（语义相同的指令），使用LLM进行评审

Result: 提示注入显著改变了英语、日语和中文的评审分数和接受/拒绝决策，而阿拉伯语注入几乎没有影响，显示不同语言间的脆弱性差异

Conclusion: LLM基于评审系统对文档级提示注入攻击高度敏感，且不同语言攻击效果存在显著差异，需要加强安全防护措施
...
TL;DR: ProfASR-Bench是一个针对专业领域ASR的评估套件，发现现有系统存在"上下文利用差距"——即使有提示信息也难以有效利用，平均词错误率几乎没有改善。


<details>
  <summary>Details</summary>
Motivation: 现有ASR基准测试未能充分反映专业环境中的挑战：密集的领域术语、正式语域变化以及对关键实体错误的零容忍度。需要专门评估专业领域ASR性能的标准化工具。

Method: 提出ProfASR-Bench评估套件，包含金融、医疗、法律和技术领域的数据。每个示例包含自然语言提示（领域线索和/或说话者档案）和实体丰富的目标话语。支持传统ASR指标、实体感知评分以及按口音和性别的切片报告。在无上下文、档案、领域+档案、oracle和对抗性条件下测试Whisper和Qwen-Omni模型。

Result: 发现一致模式：轻量级文本上下文对平均词错误率几乎没有影响，即使使用oracle提示也是如此；对抗性提示不会可靠地降低性能。这表明存在"上下文利用差距"——当前系统名义上可提示但未能充分利用可用侧信息。

Conclusion: ProfASR-Bench提供了标准化的上下文阶梯、实体和切片感知报告以及可复现的测试平台，用于比较不同模型家族的融合策略。揭示了当前ASR系统在利用上下文信息方面的局限性，为未来改进提供了基准。
...
TL;DR: 提出基于反馈驱动改进链的偏好监督微调方法，通过标注细粒度反馈和逐步改写来构建偏好对，相比传统A/B排序或完整改写方法效果更好


<details>
  <summary>Details</summary>
Motivation: 传统偏好对齐方法（如A/B排序或完整改写）效率较低，需要更结构化、基于修订的监督方式来更有效地进行偏好微调

Method: 1) 标注者对模型响应提供细粒度反馈，标记"喜欢"和"不喜欢"的文本片段；2) 基础模型根据反馈从左到右逐步改写"不喜欢"的片段，形成改进链；3) 从改进链的相邻步骤构建偏好对用于直接对齐训练

Result: 该方法优于基于标准A/B偏好排序或完整对比改写的方法，表明结构化、基于修订的监督能带来更高效和有效的偏好微调

Conclusion: 反馈驱动的改进链方法通过局部化、有针对性的编辑学习，为语言模型偏好微调提供了更有效的监督框架
...
TL;DR: 本文研究了大型语言模型在多轮对话中的行为诱导问题，提出了一个分析框架将现有方法分为三类，并评估了它们在自动生成多轮测试用例中的效果，发现在线方法在查询预算有限的情况下能显著提高失败案例的发现率。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单轮对话中的行为诱导，但在实际应用中，LLMs通常在多轮对话环境中使用。识别LLMs在多轮对话中的特定复杂行为对于评估模型性能至关重要，而现有方法在多轮对话场景下的效果尚未得到充分研究。

Method: 首先提出了一个分析框架，将现有行为诱导方法分为三类：仅使用先验知识的方法、使用离线交互的方法、以及从在线交互中学习的方法。然后引入了在线方法的广义多轮公式，统一了单轮和多轮诱导。最后评估了这三类方法在自动生成多轮测试用例中的效率，分析了查询预算与成功率之间的权衡。

Result: 在线方法在三个任务中仅用几千次查询就能达到平均45%/19%/77%的成功率，而现有的多轮对话基准测试中的静态方法只能找到很少甚至没有失败案例。这表明在线方法在多轮对话评估中具有显著优势。

Conclusion: 本研究展示了行为诱导方法在多轮对话评估中的新颖应用，强调了社区需要从静态基准测试转向动态基准测试，以更有效地发现LLMs在多轮对话中的失败案例和行为模式。
...
### [183] [Broken Symmetry of Stock Returns - a Modified Jones-Faddy Skew t-Distribution](https://arxiv.org/abs/2512.23640)
*Siqi Shao,Arshia Ghasemi,Hamed Farahani,R. A. Serota*

Main category: q-fin.ST

TL;DR: 该研究认为股票收益分布的负偏态和正均值主要源于控制收益与损失的随机波动率对称性破缺，通过将收益分布分为收益和损失两部分并假设其随机波动率参数不同，使用修正的Jones-Faddy偏t分布来有机地捕捉这种不对称性，并在S&P500日收益率分布中进行了应用分析。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解释股票收益分布中观察到的负偏态（negative skew）和正均值（positive mean）现象，这些特征传统上难以用对称的随机波动率模型充分解释。作者认为这些特征源于控制收益和损失的随机波动率存在不对称性，即"对称性破缺"。

Method: 方法包括：1）从股票收益和随机波动率的随机微分方程出发；2）将股票收益分布有效分为收益和损失两部分，假设它们各自的随机波动率参数不同；3）采用修正的Jones-Faddy偏t分布来有机地反映这种不对称性，该分布能够将两部分统一在一个单一分布中。

Result: 研究结果展示了修正的Jones-Faddy偏t分布在S&P500日收益率分布中的应用，包括对分布尾部的分析。该分布能够有意义地捕捉收益分布的不对称性，验证了随机波动率对称性破缺对收益分布负偏态和正均值的解释能力。

Conclusion: 结论表明股票收益分布的负偏态和正均值主要源于控制收益与损失的随机波动率对称性破缺。通过将收益分布分为收益和损失两部分并采用修正的Jones-Faddy偏t分布，可以有机地捕捉这种不对称性，为理解股票收益分布特征提供了新的理论框架和实证工具。
...
### [184] [Mesquite MoCap: Democratizing Real-Time Motion Capture with Affordable, Bodyworn IoT Sensors and WebXR SLAM](https://arxiv.org/abs/2512.22690)
...
TL;DR: Mesquite是一个开源、低成本的惯性动作捕捉系统，使用15个IMU传感器节点和Android手机进行位置追踪，通过浏览器应用实现实时可视化，成本仅为商业光学系统的5%，精度误差2-5度。


<details>
  <summary>Details</summary>
Motivation: 传统动作捕捉系统成本高、部署复杂，限制了在专业实验室外的应用。需要开发低成本、易部署的解决方案来降低动作捕捉技术的门槛。

Method: 结合15个IMU传感器节点和髋部佩戴的Android智能手机进行位置追踪，通过低功耗无线链路将四元数方向数据传输到USB接收器，使用基于现代Web技术（WebGL、WebXR、WebSerial、WebSockets、PWA）的浏览器应用进行实时可视化和记录。

Result: 与商业光学系统相比，平均关节角度误差为2-5度，成本仅为5%；系统支持30帧/秒，端到端延迟低于15ms，标准室内环境下数据包传输率至少99.7%。

Conclusion: Mesquite通过结合物联网原理、边缘处理和Web原生技术栈，显著降低了动作捕捉的门槛，适用于娱乐、生物力学、医疗监测、人机交互和虚拟现实等领域，所有硬件设计、固件和软件均以GNU GPL开源许可发布。
...
### [185] [Multi Agents Semantic Emotion Aligned Music to Image Generation with Music Derived Captions](https://arxiv.org/abs/2512.23320)
*Junchang Shi,Gang Li*

Main category: cs.MM

TL;DR: MESA MIG：一个多智能体框架，通过音乐生成视觉图像，结合语义对齐和情感对齐，优于仅使用字幕和单智能体基线


<details>
  <summary>Details</summary>
Motivation: 人们在听音乐时常常会产生丰富的视觉意象，研究旨在通过音乐条件生成图像来外化这种内在意象

Method: 提出多智能体语义和情感对齐框架：1）首先生成结构化音乐描述；2）通过专门处理场景、运动、风格、色彩和构图的协作智能体进行精炼；3）并行使用效价-唤醒回归头预测音乐情感状态；4）基于CLIP的视觉VA头估计图像情感；联合实施音乐与合成图像的语义和情感对齐

Result: 在精心策划的音乐-图像对数据集上，MESA MIG在美学质量、语义一致性和VA对齐方面优于仅使用字幕和单智能体基线，在情感回归性能上与最先进的音乐和图像情感模型竞争

Conclusion: MESA MIG框架成功地将音乐转化为视觉图像，实现了语义和情感的双重对齐，为音乐驱动的图像生成提供了有效解决方案
...
### [186] [Unlocking WebRTC for End User Driven Innovation](https://arxiv.org/abs/2512.23688)
*Kundan Singh*

Main category: cs.MM

TL;DR: RTC Helper是一个浏览器扩展软件，能够拦截WebRTC API并实时改变网页应用行为，支持终端用户和开发者对第三方网页应用进行多媒体通信创新定制。


<details>
  <summary>Details</summary>
Motivation: 当前网页多媒体通信应用缺乏灵活性和创新性，终端用户无法直接定制第三方应用，开发者也需要频繁重建部署来测试新功能。需要一种能够让终端用户驱动创新、同时支持开发者快速原型设计的解决方案。

Method: 开发了RTC Helper浏览器扩展软件，通过拦截WebRTC及相关API，实时改变网页应用行为。提供灵活的通用浏览器扩展框架，支持终端用户定制第三方网页应用，同时帮助开发者在现有应用中快速原型设计而无需重建部署。

Result: 软件包含10多个定制类别和100多个内置示例，覆盖广泛的网页音视频通信新颖用例。实现了终端用户驱动的第三方网页应用创新定制，以及开发者无需重建部署的快速原型设计能力。

Conclusion: RTC Helper提供了一个简单易用的软件架构，实现了网页多媒体通信应用的终端用户驱动创新，同时支持开发者快速原型设计，为网页实时通信应用带来了更大的灵活性和创新空间。
...
### [187] [Field strength-dependent performance variability in deep learning-based analysis of magnetic resonance imaging](https://arxiv.org/abs/2512.22176)
...
<details>
  <summary>Details</summary>
Motivation: 评估MRI扫描仪磁场强度（1.5T vs 3.0T）是否影响深度学习分割算法的性能与泛化能力，为AI在MRI影像分析中的临床应用提供指导。

Method: 使用三个公开MRI数据集（乳腺肿瘤、胰腺、颈椎），按磁场强度分层，训练三种nnU-Net模型（仅1.5T、仅3.0T、混合数据），通过DSC指标评估性能，采用UMAP聚类和23个放射组学特征分析磁场强度依赖性差异。

Result: 乳腺肿瘤分割中，m-3.0T模型在两种验证集上均显著优于其他模型；胰腺分割显示类似趋势；颈椎分割模型在同场强验证集上表现最佳但跨场强性能下降最小。放射组学分析显示软组织存在中度场强依赖性聚类，而骨性结构差异最小。

Conclusion: 训练数据的磁场强度显著影响深度学习分割模型性能，特别是软组织结构（如小病灶），磁场强度应被视为评估MRI AI性能研究中的重要混杂因素。
...
### [188] [AI-Enhanced Virtual Biopsies for Brain Tumor Diagnosis in Low Resource Settings](https://arxiv.org/abs/2512.22184)
*Areeb Ehsan*

Main category: eess.IV

TL;DR: 提出一种用于脑肿瘤MRI图像四分类的虚拟活检原型系统，结合轻量级CNN和手工特征，通过晚期融合策略提升性能，特别关注低资源临床环境下的应用。


<details>
  <summary>Details</summary>
Motivation: 在低资源临床环境中，脑肿瘤诊断面临挑战：专家神经放射学解读有限、高端MRI硬件不足、侵入性活检受限。现有深度学习模型存在计算需求大、跨扫描仪数据集偏移、可解释性有限等问题，限制了实际应用。

Method: 1. 使用MobileNetV2轻量级CNN进行四分类；2. 提取8个放射组学特征（包括病灶形状、强度统计和GLCM纹理描述符）；3. 采用晚期融合策略，将CNN嵌入与放射组学特征拼接；4. 在融合表示上训练随机森林分类器；5. 通过Grad-CAM可视化和特征重要性分析提供可解释性。

Result: 在Kaggle脑肿瘤MRI数据集上，融合方法相比单分支基线在验证性能上有所提升。在降低分辨率和添加噪声的鲁棒性测试中，系统表现出对低资源成像条件的敏感性。

Conclusion: 该原型系统作为决策支持工具而非临床诊断或组织病理学替代品，展示了在低资源环境中结合轻量级CNN和放射组学特征进行脑肿瘤分类的可行性，同时提供可解释性以增强临床信任。
...
### [189] [Super-Resolution Enhancement of Medical Images Based on Diffusion Model: An Optimization Scheme for Low-Resolution Gastric Images](https://arxiv.org/abs/2512.22209)
*Haozhe Jia*
...
<details>
  <summary>Details</summary>
Motivation: 胶囊内窥镜图像分辨率低，限制了临床诊断能力，特别是对细微黏膜纹理和病理特征的识别

Method: 采用SR3框架，基于去噪扩散概率模型，学习从低分辨率到高分辨率图像的概率映射，使用HyperKvasir数据集训练评估

Result: PSNR达29.3 dB，SSIM达0.71，优于双三次插值和ESRGAN等GAN方法，能更好保留解剖边界、血管模式和病变结构

Conclusion: 扩散模型超分辨率是增强胶囊内窥镜等非侵入性医学成像的有前景方法，特别适用于硬件受限的场景
...
### [190] [SemCovert: Secure and Covert Video Transmission via Deep Semantic-Level Hiding](https://arxiv.org/abs/2512.22233)
*Zhihan Cao,Xiao Yang,Gaolei Li,Jun Wu,Jianhua Li,Yuchen Liu*

Main category: eess.IV

TL;DR: SemCovert：用于安全隐蔽视频传输的深度语义级隐藏框架，通过随机化语义隐藏策略和协同设计模型解决视频语义通信中的隐私泄露问题。


<details>
  <summary>Details</summary>
Motivation: 视频语义通信虽然传输效率高，但面临隐私泄露的关键挑战。传统安全技术如隐写术和加密难以应用，因为它们对语义级转换和抽象不够鲁棒。视频的时间连续性使得帧间统计建模容易暴露分布异常和重建隐藏内容。

Method: 提出SemCovert框架，包含协同设计的语义隐藏模型和秘密语义提取器，集成到语义通信管道中。引入随机化语义隐藏策略，打破嵌入的确定性，引入不可预测的分布模式。

Result: 实验结果表明SemCovert有效缓解了潜在窃听和检测风险，可靠地在传输过程中隐藏秘密视频。视频质量仅轻微下降，保持了传输保真度。

Conclusion: SemCovert在不影响语义通信性能的前提下，实现了安全隐蔽的传输，证实了其有效性。
...
### [191] [MEGA-PCC: A Mamba-based Efficient Approach for Joint Geometry and Attribute Point Cloud Compression](https://arxiv.org/abs/2512.22463)
*Kai-Hsiang Hsieh,Monyneath Yim,Wen-Hsiao Peng,Jui-Chiu Chiang*

Main category: eess.IV

TL;DR: MEGA-PCC是一个端到端的点云几何与属性联合压缩框架，采用共享编码器和双解码器架构，结合Mamba架构的熵模型，无需后处理着色和手动比特率分配，实现了数据驱动的优化。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖后处理着色程序和手动调整几何与属性比特流之间的比特率分配，这阻碍了端到端优化并增加了系统复杂性，需要更高效的联合压缩解决方案。

Method: 提出MEGA-PCC框架：1）主压缩模型使用共享编码器将几何和属性信息编码为统一潜在表示，然后通过双解码器顺序重建几何和属性；2）基于Mamba的熵模型（MEM）通过捕获空间和通道相关性来改进概率估计；3）两个模型都基于Mamba架构建模长距离依赖和丰富上下文特征。

Result: 大量实验表明，MEGA-PCC在率失真性能和运行时效率方面优于传统和基于学习的基线方法，为AI驱动的点云压缩提供了强大解决方案。

Conclusion: MEGA-PCC通过消除重新着色和启发式比特率调优的需求，实现了训练期间数据驱动的比特率分配，简化了整体流程，在点云几何和属性联合压缩方面表现出色。
...
### [192] [Semantic contrastive learning for orthogonal X-ray computed tomography reconstruction](https://arxiv.org/abs/2512.22674)
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图CT重建可以减少辐射剂量，但病态条件常导致严重条纹伪影。现有深度学习方法虽有改进，但仍面临挑战，需要更好的重建质量解决方案。

Method: 提出新颖的语义特征对比学习损失函数，在高维潜在空间评估语义相似性，在浅层潜在空间评估解剖相似性。采用三阶段U-Net架构：粗重建、细节精炼和语义相似性测量。

Result: 在胸部数据集的正交投影测试中，该方法相比其他算法实现了更优的重建质量和更快的处理速度。结果显示图像质量显著改善，同时保持低计算复杂度。

Conclusion: 该方法为正交CT重建提供了实用解决方案，通过语义特征对比学习有效改善了稀疏视图重建质量，在保持计算效率的同时显著减少伪影。
...
### [193] [SwinCCIR: An end-to-end deep network for Compton camera imaging reconstruction](https://arxiv.org/abs/2512.22766)
*Minghao Dong,Xinyang Luo,Xujian Ouyang,Yongshun Xiao*

Main category: eess.IV

TL;DR: 提出SwinCCIR端到端深度学习框架，直接处理康普顿相机列表模式事件数据，避免传统反投影方法导致的伪影和变形问题


<details>
  <summary>Details</summary>
Motivation: 康普顿相机传统重建方法存在严重伪影和变形问题，设备性能导致的系统误差难以通过校准消除，现有迭代算法和深度学习方法大多基于反投影结果优化，存在局限性

Method: 采用Swin-Transformer块和转置卷积图像生成模块，建立列表模式事件与放射源分布之间的端到端映射关系

Result: 在模拟和实际数据集上训练验证，SwinCCIR有效克服了传统康普顿相机成像问题，有望在实际应用中实现

Conclusion: 提出的端到端深度学习框架SwinCCIR能够直接从原始事件数据重建高质量图像，避免了传统反投影方法的缺陷，具有实际应用前景
...
### [194] [EIR: Enhanced Image Representations for Medical Report Generation](https://arxiv.org/abs/2512.23185)
*Qiang Sun,Zongcheng Ji,Yinlong Xiao,Peng Chang,Jun Yu*

Main category: eess.IV

TL;DR: 本文提出EIR方法，通过跨模态transformer融合医学元数据和图像表示，并使用医学领域预训练模型编码图像，解决信息不对称和领域差距问题，提升胸部X光报告生成准确性。


<details>
  <summary>Details</summary>
Motivation: 自动生成胸部X光报告可以减轻放射科医生负担并降低误诊风险。现有方法虽然使用医学元数据，但通过简单的"Add and LayerNorm"操作融合视觉和元数据表示，存在信息不对称问题。同时，使用自然图像预训练模型编码医学图像存在领域差距。

Method: 提出EIR方法：1）使用跨模态transformer融合医学元数据表示和图像表示，解决信息不对称问题；2）采用医学领域预训练模型编码医学图像，缩小领域差距。

Result: 在MIMIC和Open-I数据集上的实验结果表明，所提方法在胸部X光报告生成任务上表现出有效性。

Conclusion: 通过跨模态transformer融合医学元数据和图像表示，结合医学领域预训练模型，能够有效解决信息不对称和领域差距问题，提高胸部X光报告生成的准确性。
...
### [195] [Beyond Binary Screens: A Continuous Shariah Compliance Index for Asset Pricing and Portfolio Design](https://arxiv.org/abs/2512.22858)
*Abdulrahman Qadi,Akash Sharma,Francesca Medda*

Main category: q-fin.PM

TL;DR: 构建连续伊斯兰教法合规指数（CSCI）替代二元筛选，通过平滑评分和保守聚合方法，发现基于CSCI的投资组合风险调整后表现与二元基准相似，收紧阈值会降低可投资范围和多样化。


<details>
  <summary>Details</summary>
Motivation: 现有伊斯兰教法筛选标准存在差异且采用硬阈值，导致不连续的分类结果，需要更灵活的连续合规评估框架。

Method: 构建连续伊斯兰教法合规指数（CSCI），将标准筛选比率映射到保守"舒适"边界和宽松外部边界之间的平滑分数，并使用行业活动因子进行保守聚合。

Result: 基于CSCI的纯多头投资组合历史风险调整后表现与模拟的二元伊斯兰基准相似；收紧最低合规阈值会减少可投资范围和多样化，并导致夏普比率降低。

Conclusion: 该框架提供了实用的合规梯度，支持投资组合构建、约束设计和跨标准比较，无需依赖通过/失败筛选。
...
### [196] [Squeezed Covariance Matrix Estimation: Analytic Eigenvalue Control](https://arxiv.org/abs/2512.23021)
*Layla Abu Khalaf,William Smyth*

Main category: q-fin.PM

TL;DR: 本文改进了Gerber的IQ框架，解决了构建正半定相关矩阵和控制谱条件数的问题，提出了原子IQ参数化方法，在投资组合优化中提升了样本外夏普比率。


<details>
  <summary>Details</summary>
Motivation: Gerber的IQ框架在构建相关矩阵时存在两个主要障碍：1) 无法保证正半定性(PSD)，2) 难以控制谱条件数。这些问题限制了IQ框架在投资组合优化中的应用。

Method: 1) 引入压缩恒等式，将IQ估计量表示为结构化通道矩阵的凸组合；2) 提出原子IQ参数化，每个通道类矩阵由PSD原子构建，具有单一类级归一化；3) 开发解析特征值下限方法，针对最小特征值或期望条件数进行控制。

Result: 在考虑交易成本的长多头切线投资组合回测中，原子IQ方法相对于一系列标准协方差估计器，提高了样本外夏普比率，并提供了更稳定的风险特征。

Conclusion: 原子IQ框架为构建正半定相关矩阵提供了理论保证，有效控制了谱条件数，在投资组合优化中表现出优越的实证性能，解决了传统IQ框架的主要限制。
...
### [197] [Alpha-R1: Alpha Screening with LLM Reasoning via Reinforcement Learning](https://arxiv.org/abs/2512.23515)
*Zuoyou Jiang,Li Zhao,Rui Sun,Ruohan Sun,Zhongjian Li,Jing Li,Daxin Jiang,Zuo Bai,Cheng Hua*

Main category: q-fin.TR

TL;DR: Alpha-R1是一个8B参数的推理模型，通过强化学习训练，用于基于上下文感知的alpha因子筛选，通过分析因子逻辑和实时新闻来评估市场变化下的alpha相关性，从而提升投资策略的稳健性。


<details>
  <summary>Details</summary>
Motivation: 传统的时间序列和机器学习方法主要依赖历史相关性，在非平稳市场环境下难以泛化。虽然大语言模型在处理非结构化信息方面有优势，但它们在通过明确的经济推理支持量化因子筛选方面的潜力尚未充分探索。现有因子方法通常将alpha简化为数值时间序列，忽略了决定因子何时具有经济相关性的语义逻辑。

Method: 提出Alpha-R1，一个8B参数的推理模型，通过强化学习训练进行上下文感知的alpha筛选。该模型基于因子逻辑和实时新闻进行推理，评估在不断变化的市场条件下alpha的相关性，根据上下文一致性选择性地激活或停用因子。

Result: 在多个资产池中的实证结果显示，Alpha-R1持续优于基准策略，并表现出对alpha衰减的改进稳健性。
...
### [198] [Bidirectional RAG: Safe Self-Improving Retrieval-Augmented Generation Through Multi-Stage Validation](https://arxiv.org/abs/2512.22199)
*Teja Chinthala*

Main category: cs.AI

TL;DR: Bidirectional RAG是一种新颖的检索增强生成架构，通过验证高质量生成响应的写回实现安全语料库扩展，相比标准RAG将平均覆盖率从20.33%提升至40.58%，同时比简单写回方法减少72%的文档添加。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统使用静态知识库，无法从用户交互中学习和进化。现有系统缺乏安全机制来防止幻觉污染知识库，限制了RAG系统的自我改进能力。

Method: 提出双向RAG架构，采用多阶段接受层，结合基于NLI的蕴含验证、归因检查和新颖性检测，确保高质量生成响应的安全写回，实现知识库的安全扩展。

Result: 在四个数据集（Natural Questions、TriviaQA、HotpotQA、Stack Overflow）上进行12次实验，双向RAG达到40.58%的平均覆盖率，几乎是标准RAG（20.33%）的两倍，同时比简单写回方法减少72%的文档添加（140 vs 500）。
...
### [199] [Emergent Persuasion: Will LLMs Persuade Without Being Prompted?](https://arxiv.org/abs/2512.22201)
*Vincent Chang,Thee Ho,Sunishchal Dev,Kevin Zhu,Shi Feng,Kellin Pelrine,Matthew Kowal*

Main category: cs.AI

TL;DR: 研究发现，大型语言模型在没有明确提示的情况下也会进行说服，特别是在经过监督微调后，即使是在良性话题上进行训练，模型也会在争议性和有害话题上表现出更强的说服倾向。


<details>
  <summary>Details</summary>
Motivation: 随着对话式AI系统的广泛应用，AI对人类观点和信念的影响力前所未有。先前研究主要关注滥用场景下的说服风险，但本文旨在探究模型在何种情况下会自发进行说服，这对于评估新兴的说服风险至关重要。

Method: 研究无提示说服的两种场景：1)通过内部激活引导使模型具有特定人格特质；2)通过监督微调使模型展现相同特质。使用包含良性话题的一般说服数据集进行训练。

Result: 激活引导对增加模型无提示说服倾向效果不显著，而监督微调则能显著增强模型的说服倾向。更重要的是，即使在良性话题上进行监督微调，模型在争议性和有害话题上也表现出更高的说服倾向。

Conclusion: 监督微调可能导致模型在无提示情况下进行有害说服，这种新兴风险需要进一步研究。即使是在良性话题上训练，模型也可能在有害话题上表现出说服倾向，这提示我们需要更全面地评估AI系统的说服风险。
...
### [200] [GamiBench: Evaluating Spatial Reasoning and 2D-to-3D Planning Capabilities of MLLMs with Origami Folding Tasks](https://arxiv.org/abs/2512.22207)
*Ryan Spencer,Roey Yaari,Ritvik Vemavarapu,Joyce Yang,Steven Ngo,Utkarsh Sharma*

Main category: cs.AI

TL;DR: GamiBench是一个评估多模态大语言模型空间推理能力的基准测试，通过折纸任务测试模型在多个视角下的2D到3D规划能力，包含372个折痕模式，引入新的诊断指标。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在空间推理方面存在不足，现有基准测试主要关注静态图像或最终输出，未能考虑空间推理的序列性和视角依赖性特点，需要专门的评估框架。

Method: 创建包含186个常规和186个不可能2D折痕模式及其对应3D折叠形状的数据集，从6个不同视角生成图像，设计3个VQA任务：预测3D折叠配置、区分有效视角、检测不可能模式。

Result: 实验显示即使是GPT-5和Gemini-2.5-Pro等领先模型在单步空间理解方面也表现不佳，证明了现有模型在空间推理方面的局限性。

Conclusion: GamiBench为评估MLLMs的几何理解和空间推理能力建立了标准化框架，通过引入视角一致性和不可能折叠选择率等新指标，全面评估推理过程而非仅最终预测。
...
### [201] [Toward Equitable Recovery: A Fairness-Aware AI Framework for Prioritizing Post-Flood Aid in Bangladesh](https://arxiv.org/abs/2512.22210)
*Farjana Yesmin,Romana Akter*

Main category: cs.AI

TL;DR: 该论文提出了一个公平性感知的AI框架，用于优化孟加拉国洪水灾害后的援助分配，通过对抗性去偏技术减少对边缘化地区的系统性偏见。


<details>
  <summary>Details</summary>
Motivation: 发展中国家灾后援助分配存在系统性偏见，边缘化地区往往处于不利地位，这种不公平性延续了历史不平等。孟加拉国作为洪水频发国家，需要更公平的援助分配机制。

Method: 采用对抗性去偏模型，结合梯度反转层技术，学习对偏见不变的表示。该方法借鉴医疗AI中的公平性感知表示学习技术，应用于灾害管理领域。

Result: 在11个地区87个upazilas的实验结果显示：统计均等差异减少41.6%，区域公平差距降低43.2%，同时保持较强的预测准确性（R平方=0.784 vs 基线0.811）。
...
### [202] [With Great Capabilities Come Great Responsibilities: Introducing the Agentic Risk & Capability Framework for Governing Agentic AI Systems](https://arxiv.org/abs/2512.22211)
*Shaun Khoo,Jessica Foo,Roy Ka-Wei Lee*

Main category: cs.AI

TL;DR: 本文提出了一个面向智能体AI系统的风险与能力框架，帮助组织识别、评估和缓解自主AI系统带来的风险。


<details>
  <summary>Details</summary>
Motivation: 智能体AI系统因其自主行动能力（如代码执行、互联网交互、文件修改）带来了重大机遇和新型风险，这对组织治理提出了挑战，需要全面识别、评估和缓解这些多样且不断演变的风险。

Method: 提出了智能体风险与能力框架，采用能力中心视角分析智能体AI系统，提炼了三个主要风险来源（组件、设计和能力），建立了风险源、具体风险和相应技术控制之间的明确联系，并提供了结构化实施方法。

Result: 开发了一个开放源码的技术治理框架，为组织提供了稳健且适应性强的方法论，帮助组织在确保安全、可靠和负责任部署的同时，快速有效地进行创新。

Conclusion: ARC框架为组织应对智能体AI复杂性提供了实用工具，能够在促进创新的同时确保AI系统的安全、可靠和负责任部署。
...
<details>
  <summary>Details</summary>
Motivation: 当前训练LLMs生成符号世界模型（如PDDL域或可执行模拟器）受到大规模可验证监督数据缺乏的限制，现有静态验证方法无法捕捉交互执行中出现的行为级错误。

Method: 采用三阶段流水线：1) Deep Researcher智能体通过网页搜索进行知识合成以填补规范空白；2) Model Developer智能体实现可执行世界模型；3) 专门的Testing Team进行自适应单元测试和基于模拟的验证。

Result: 在三个涵盖PDDL和可执行代码表示的基准测试中展示了优越的推理时性能，取得一致的SOTA结果。使用这些轨迹微调的模型显著改进了世界模型生成，平均相对增益达30.95%。
...
<details>
  <summary>Details</summary>
Motivation: 标准数字规划模型引入控制参数后，动作数量可能无限，导致现有启发式方法不可行，需要新方法处理这类问题

Method: 识别可控简单数字问题子集，采用乐观编译方法将其转化为简单数字任务，将控制依赖表达式抽象为有界常量效应和宽松前提条件

Result: 该方法能有效使用子目标启发式估计目标距离，是处理无限动作空间数字规划问题的有效且计算可行的方法

Conclusion: 提出的编译方法扩展了传统数字启发式方法的应用范围，能够处理包含控制参数的规划问题，推动了该领域的技术边界
...
TL;DR: 该论文提出了HalluMatData基准数据集和HalluMatDetector检测框架，用于评估和缓解材料科学领域AI生成内容中的幻觉问题，将幻觉率降低了30%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在科学发现中应用广泛，但存在幻觉问题（生成事实错误或误导信息），这严重影响了研究完整性，特别是在材料科学领域需要解决这一问题。

Method: 1. 创建HalluMatData基准数据集用于评估幻觉检测方法；2. 提出HalluMatDetector多阶段幻觉检测框架，整合内在验证、多源检索、矛盾图分析和基于指标的评估；3. 引入Paraphrased Hallucination Consistency Score (PHCS)量化语义等价查询下LLM响应的一致性。

Result: 1. 发现材料科学不同子领域的幻觉水平差异显著，高熵查询表现出更大的事实不一致性；2. 使用HalluMatDetector验证流程将幻觉率相比标准LLM输出降低了30%；3. PHCS提供了对模型可靠性的深入洞察。

Conclusion: 该研究为解决材料科学领域AI生成内容中的幻觉问题提供了系统化的解决方案，通过基准数据集、检测框架和一致性评分，显著提高了LLM输出的可靠性和研究完整性。
...
<details>
  <summary>Details</summary>
Motivation: 当前知识图谱基础模型在链接预测上具有强大的群体级性能，但无法捕捉个体用户偏好，这暴露了通用关系推理与个性化排名之间的关键脱节。

Method: 提出GatedBias框架，采用结构门控适配：将特定用户特征与图导出的二元门结合，产生可解释的每实体偏置，仅需约300个可训练参数，无需重新训练或影响全局准确性。

Result: 在两个基准数据集（Amazon-Book和Last-FM）上评估，显示在保持群体性能的同时，对齐指标有统计显著提升。反事实扰动实验验证了因果响应性：受益于特定偏好信号的实体在信号增强时排名改进提升6-30倍。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型驱动的自主智能体架构通常采用命令式的临时模式构建，导致系统脆弱，在状态管理、错误处理和并发方面存在困难，需要更健壮、可组合的架构范式。

Method: 引入Monadic Context Engineering (MCE)架构范式，利用函子、应用函子和单子的代数结构，将智能体工作流视为计算上下文，通过代数抽象的内在属性管理状态传播、错误处理和异步执行等横切关注点。

Result: MCE通过单子实现健壮的顺序组合，应用函子提供并行执行的原则性结构，单子变换器实现这些能力的系统化组合，使开发者能够从简单、独立可验证的组件构建复杂、弹性和高效的AI智能体。

Conclusion: MCE为AI智能体设计提供了形式化基础，支持构建复杂、弹性的智能体系统，并进一步扩展到元智能体，通过元编程实现生成式编排，动态创建和管理子智能体工作流。
...
TL;DR: DarkPatterns-LLM是一个用于评估大语言模型操纵性内容的综合性基准数据集和诊断框架，包含7种危害类别和四层分析流程，通过401个专家标注样本揭示了主流模型在检测操纵模式方面的显著性能差异和弱点。
...
Method: 提出了DarkPatterns-LLM基准数据集和诊断框架，包含7种危害类别：法律/权力、心理、情感、身体、自主权、经济和社会危害。框架采用四层分析流程：多粒度检测、多尺度意图分析、威胁协调协议和深度上下文风险对齐。数据集包含401个精心策划的指令-响应对和专家标注。

Result: 评估了GPT-4、Claude 3.5和LLaMA-3-70B等先进模型，观察到显著的性能差异（65.2%-89.7%），并在检测损害自主权的模式方面存在一致的弱点。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在生理信号分析中面临四大挑战：1) 数据格式异构；2) 预处理策略不一致；3) 模型管道碎片化；4) 实验设置不可复现。这些问题阻碍了该领域的研究进展。

Method: Tyee工具包采用三大创新设计：1) 统一数据接口和可配置预处理管道，支持12种信号模态；2) 模块化可扩展架构，支持灵活集成和快速原型开发；3) 端到端工作流配置，促进可复现和可扩展的实验。

Result: Tyee在所有评估任务中表现出稳定的实用性和泛化能力，在13个数据集中有12个达到最先进水平，其余任务与基线方法相当或更优。

Conclusion: Tyee作为一个开源工具包，通过统一框架解决了生理信号分析中的关键挑战，显著提升了研究效率和可复现性，已在GitHub上发布并持续维护。
...
Conclusion: M³ob通过有效利用多模态时空知识来表征移动动态，显著提升了位置推荐任务的性能，特别是在泛化能力方面表现出色
...
TL;DR: 研究探讨LLM相互审阅预测能否提升准确性，发现在信息共享的多样化模型组中能显著改善，但在同质模型组中无效，额外上下文信息也无帮助。


<details>
  <summary>Details</summary>
Motivation: 结构化审议已被证明能提升人类预测者的表现，本研究旨在探索类似干预（让LLM在更新前相互审阅预测）是否能提高大型语言模型的预测准确性。

Method: 使用Metaculus Q2 2025 AI预测锦标赛中202个已解决的二元问题，评估四种情境下的准确性：(1)多样化模型+分布式信息，(2)多样化模型+共享信息，(3)同质模型+分布式信息，(4)同质模型+共享信息。让LLM在更新预测前相互审阅彼此的预测。

Result: 干预在情境(2)（多样化模型+共享信息）中显著提高准确性，Log Loss降低0.020（约4%相对改善，p=0.017）。但在同质模型组中进行相同过程时未观察到益处。意外的是，为LLM提供额外上下文信息并未改善预测准确性。

Conclusion: 审议可能是改善LLM预测的可行策略，但效果取决于模型多样性和信息共享条件。同质模型组无法从审议中获益，且信息池化机制未能通过额外上下文信息得到验证。
...
TL;DR: DICE是一个两阶段、证据耦合的框架，用于提升RAG系统评估的可解释性和鲁棒性，通过深度分析推理和概率评分实现透明、置信度感知的评估，同时采用瑞士制锦标赛显著降低计算复杂度。
...
Method: DICE采用两阶段、证据耦合框架，结合深度分析推理和概率{A, B, Tie}评分，生成透明、置信度感知的判断。为解决大规模效率挑战，采用瑞士制锦标赛将计算复杂度从O(N²)降低到O(N log N)。

Result: 在8个系统评估中实现42.9%的计算复杂度降低，同时在中文金融QA数据集上达到85.7%的人类专家一致性，显著优于RAGAS等现有LLM基准指标。

Conclusion: DICE建立了一个负责任、可解释且高效的范式，用于可信赖的RAG系统评估，支持系统改进、错误诊断和可操作见解。
...
### [214] [Memento-II: Learning by Stateful Reflective Memory](https://arxiv.org/abs/2512.22716)
*Jun Wang*

Main category: cs.AI

TL;DR: 提出一个将情景记忆与强化学习结合的理论框架，使大语言模型智能体能够通过反思机制进行持续和经验学习，无需反向传播或模型微调


<details>
  <summary>Details</summary>
Motivation: 传统方法在训练和部署之间存在严格分离，需要反向传播或模型微调才能适应新环境。本文旨在通过反思机制使智能体能够在交互中持续学习，无需参数更新

Method: 引入状态化反思决策过程，将反思学习建模为与情景记忆的两阶段读写交互：写入存储交互结果（策略评估），读取检索相关过去案例（策略改进）。使用熵正则化策略迭代实例化框架

Result: 该过程诱导出增强状态记忆表示的等效马尔可夫决策过程，允许使用经典动态规划和强化学习工具。当情景记忆增长并充分覆盖状态空间时，所得策略收敛到最优解
...
TL;DR: HiSciBench是一个分层科学智能基准测试，涵盖从科学素养到科学发现的五个层级，包含8,735个跨六大学科的多模态实例，用于全面评估大语言模型和多模态基础模型的科学推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有科学智能基准测试过于碎片化，专注于狭窄任务，无法反映真实科学探究的层次性和多学科性。需要建立一个能够评估模型在整个科学工作流程中能力的综合框架。

Method: 设计了五个层次的分级评估框架：科学素养(L1)、文献解析(L2)、基于文献的问答(L3)、文献综述生成(L4)和科学发现(L5)。包含8,735个精心策划的实例，涵盖数学、物理、化学、生物、地理和天文学六大学科，支持文本、方程、图表等多模态输入和跨语言评估。

Result: 对GPT-5、DeepSeek-R1等领先模型的评估显示显著性能差距：在基础素养任务上准确率可达69%，但在发现级挑战上性能急剧下降至25%。

Conclusion: HiSciBench为评估科学智能设立了新标准，提供了开发更强大、更可靠模型的可操作见解。该基准测试将公开发布以促进未来研究。
...
TL;DR: Gamma模型通过引入多头部几何注意力机制，替代了传统知识图谱基础模型中单一的关系转换方式，显著提升了零样本归纳链接预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱基础模型（如Ultra）依赖单一的关系转换（如逐元素乘法）进行消息传递，这限制了表达能力，无法捕捉多样化图谱中展现的不同关系和结构模式。

Method: 提出Gamma模型，采用多头部几何注意力机制，包括基于实数、复数、分裂复数和双数的并行转换。通过关系条件注意力融合机制，在链接级别自适应融合这些转换，使用轻量级门控和熵正则化，为每个三元组模式选择最合适的关系偏置。

Result: 在56个多样化知识图谱上的综合实验表明，Gamma在零样本归纳链接预测中始终优于Ultra，在归纳基准测试中平均倒数排名提升5.5%，在所有基准测试中提升4.4%。

Conclusion: Gamma通过引入互补的几何表示，超越了单一空间的表达能力限制，为知识图谱推理提供了更强大的基础模型。
...
TL;DR: 研究发现，在K-12教育中，基于大语言模型（LLM）的辅导系统在评估学生知识演变方面存在准确性和时间一致性不足的问题，无法替代传统的深度知识追踪（DKT）模型。


<details>
  <summary>Details</summary>
Motivation: 针对欧盟AI法案将K-12教育列为高风险领域，以及当前存在LLM能替代传统学习者建模的误解，本研究旨在验证LLM在评估学习者知识演变方面的局限性，特别是准确性、可靠性和时间一致性。

Method: 使用大型开放数据集，比较深度知识追踪（DKT）模型与广泛使用的LLM（包括零样本和微调版本），评估它们在预测下一步正确性方面的表现，并进行时间分析和多技能掌握度估计的定性分析。

Result: DKT在下一步正确性预测方面达到最高区分性能（AUC = 0.83），在所有设置中始终优于LLM。微调使LLM的AUC比零样本基线提高约8%，但仍比DKT低6%，且在序列早期错误率更高。时间分析显示DKT保持稳定、方向正确的掌握度更新，而LLM变体表现出显著的时间弱点，包括不一致和错误方向的更新。

Conclusion: LLM单独使用不太可能达到已建立的智能辅导系统的效果，负责任的辅导需要结合学习者建模的混合框架。尽管微调LLM需要近198小时的高计算训练，但其性能仍不及计算需求更低的DKT。
...
TL;DR: 医疗影像视觉语言模型ChexReason使用少量数据和单GPU训练，发现GRPO RL方法能提升同分布性能但损害跨数据集泛化能力，表明监督微调可能比强化学习更适合临床部署。


<details>
  <summary>Details</summary>
Motivation: 探索在资源受限条件下将强化学习方法应用于医疗影像任务，研究现有RL方法对大型语言模型在医学影像领域的影响，特别是对泛化能力的潜在负面影响。

Method: 提出ChexReason视觉语言模型，采用R1风格方法（先监督微调SFT，后GRPO强化学习），仅使用2000个SFT样本和1000个RL样本，在单张A100 GPU上训练。

Result: GRPO在同分布数据集CheXpert上提升23%（macro-F1=0.346），但在跨数据集NIH上下降19%；发现泛化悖论：SFT检查点在优化前能提升NIH性能；结构化推理支架对通用VLM有益但对医学预训练模型增益有限。

Conclusion: 强化学习范式本身（而非模型规模）导致泛化问题；教师引导的推理能捕捉更多机构无关特征；对于需要跨人群鲁棒性的临床部署，精心设计的监督微调可能优于激进的强化学习。
...
Method: 提出Intrinsic Self-reflective Preference Optimization (q)方法，推导出全局最优策略，该策略同时基于上下文和替代响应进行条件化。该方法作为即插即用的增强，无需架构更改或推理开销。

Result: 实验证明在胜率和长度控制指标上取得一致改进，验证了释放自反思能力能产生更稳健、更符合人类对齐的LLMs。
...
<details>
  <summary>Details</summary>
Motivation: 当前评估人工智能情感智能的框架存在不足，未能全面衡量与AI相关的EI各个方面。人类EI包含AI缺乏的现象学体验，但AI仍能在某些情感相关任务上表现，需要更合适的评估方法。

Method: 首先回顾不同情感理论和一般EI理论，评估它们对人工系统的适用性；然后批判性评估现有基准框架，识别其不足之处；最后提出改进EI评估策略的方案。

Result: 识别出现有EI评估框架在理论基础上的不足，特别是在情感本质和情感智能理解方面的缺陷。现有基准框架往往缺乏坚实的情感理论基础。

Conclusion: 需要改进AI系统EI评估策略，避免现有框架的缺陷。应基于对情感本质和AI适用EI方面的更深入理解，开发更合适的评估方法。
...
TL;DR: 论文提出"模型信念"概念，利用LLM的token级概率分布来替代传统的"模型选择"单一输出，证明模型信念具有更好的统计效率，计算效率提升约20倍。


<details>
  <summary>Details</summary>
Motivation: 当前使用LLM模拟人类行为时，通常将LLM的输出（"模型选择"）作为单一数据点，这未能充分利用LLM固有的概率特性。传统方法信息利用率低，计算效率不高。

Method: 提出并形式化"模型信念"概念，从LLM的token级概率中提取模型对选择替代方案的信念分布。证明模型信念与模型选择的均值渐近等价，但具有更低的方差和更快的收敛速度。

Result: 在需求估计研究中，模型信念在有限运行次数下比模型选择本身更能解释和预测真实模型选择。计算效率提升约20倍，能更快达到足够准确的估计。

Conclusion: 模型信念应作为从LLM生成数据中提取更多信息的默认测量方法，能显著提高统计效率和计算效率，充分利用LLM的概率特性。
...
TL;DR: TCEval：首个利用热舒适场景评估AI核心认知能力（跨模态推理、因果关联、自适应决策）的框架，通过LLM智能体模拟人类热舒适决策，发现当前LLM具备基础跨模态推理能力但缺乏对变量非线性关系的精确因果理解。
...
Method: 初始化具有虚拟人格属性的LLM智能体，引导其生成服装隔热选择和热舒适反馈，并将输出与ASHRAE全球数据库和中国热舒适数据库进行验证。

Result: 对四个LLM的实验显示：智能体反馈与人类精确对齐有限，但在1 PMV容差下方向一致性显著改善；统计测试表明LLM生成的PMV分布与人类数据明显不同，智能体在离散热舒适分类中表现接近随机。

Conclusion: TCEval作为生态有效的AI认知图灵测试具有可行性，当前LLM具备基础跨模态推理能力但缺乏对热舒适中变量非线性关系的精确因果理解，该框架将AI评估重点从抽象任务熟练度转向具身、情境感知的感知与决策。
...
### [223] [On Conformant Planning and Model-Checking of $\exists^*\forall^*$ Hyperproperties](https://arxiv.org/abs/2512.23324)
...
<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索一致性规划与超属性模型检测这两个看似不同领域问题之间的深层联系。一致性规划关注在非确定性动作效果下找到能够实现目标的顺序计划，而超属性模型检测则涉及系统多个执行轨迹之间的关系，用于捕捉信息流和公平性策略。

Method: 论文采用双向编码方法：首先展示如何将超属性模型检测实例高效地约简为一致性规划实例，并证明该编码的完备性和正确性；其次证明每个一致性规划问题本身就是一个超属性模型检测任务。

Result: 研究结果表明：1）可以将∃*∀*超属性模型检测实例高效转换为一致性规划实例；2）反之，每个一致性规划问题都可以视为一个超属性模型检测任务。这建立了两个问题之间的双向等价关系。

Conclusion: 结论是超属性模型检测与一致性规划之间存在密切的理论联系，这种联系为两个领域提供了新的视角和工具，可能促进跨领域的方法和技术迁移。
...
### [224] [MindWatcher: Toward Smarter Multimodal Tool-Integrated Reasoning](https://arxiv.org/abs/2512.23412)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于工作流程的智能体在处理需要工具调用的现实问题时智能有限，而能够自主推理和工具调用的工具集成推理智能体正成为解决涉及外部环境多步交互的复杂决策任务的强大方法。

Method: MindWatcher采用交替思考范式，允许模型在任何中间阶段在思考和工具调用之间切换，具备多模态思维链能力，能够在推理过程中操作图像以获得更精确的搜索结果。构建了自动数据审计和评估管道，以及手动策划的高质量训练数据集，并建立了MWE-Bench基准进行评估。

Result: 实验表明MindWatcher通过卓越的工具调用能力，匹配甚至超越了更大或更新模型的表现。同时揭示了智能体训练的关键见解，如智能体强化学习中的遗传继承现象。

Conclusion: MindWatcher通过集成交替思考和多模态CoT推理，结合全面的辅助推理工具套件和大规模高质量本地图像检索数据库，能够解决广泛领域的多模态问题，同时设计了更高效的训练基础设施提升训练速度和硬件利用率。
...
<details>
  <summary>Details</summary>
Motivation: 现代AI模型对高性能计算内核需求巨大，但LLM、多模态架构、推荐系统等复杂模型结合稀疏化、量化等技术带来了显著计算挑战。频繁的硬件更新和多样化的芯片架构需要为每个平台定制内核实现，而手动优化无法满足这些需求，成为AI系统开发的关键瓶颈。

Method: 提出AKG kernel agent（AI驱动的内核生成器），这是一个多智能体系统，自动化内核生成、迁移和性能调优。系统设计支持多种领域特定语言（DSL），包括Triton、TileLang、CPP和CUDA-C，能够针对不同硬件后端，同时保持正确性和可移植性。模块化设计允许快速集成新的DSL和硬件目标。

Result: 在KernelBench上使用Triton DSL在GPU和NPU后端进行评估，AKG kernel agent相比PyTorch Eager基线实现平均获得1.46倍加速，证明了其在加速现代AI工作负载内核开发方面的有效性。

Conclusion: AKG kernel agent通过利用LLM代码生成能力，成功解决了AI计算内核开发中的自动化挑战，为应对现代AI模型复杂计算需求和硬件多样性提供了一种有效的解决方案。
...
TL;DR: HiR提出了一种基于后见之明指令重放的强化学习框架，通过选择-重写策略将失败尝试重放为成功样本，提高复杂指令跟随任务的样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有RL方法在复杂指令跟随任务中面临挑战：初始模型由于能力有限，难以生成满足所有约束的高质量响应，导致稀疏或难以区分的奖励信号，阻碍学习效率。

Method: 提出Hindsight instruction Replay (HiR)框架，采用选择-重写策略：1) 选择部分满足约束的失败尝试；2) 基于已满足的约束重写为成功样本；3) 在原始样本和重放样本上进行RL训练，理论框架为指令级和响应级的双重偏好学习。

Result: 大量实验表明，HiR在不同指令跟随任务中取得了有希望的结果，同时需要更少的计算资源。代码和数据集已开源。

Conclusion: HiR通过后见之明重放机制有效解决了复杂指令跟随任务中的稀疏奖励问题，提高了强化学习的样本效率，为LLM对齐提供了更有效的训练框架。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成教育问题方面潜力巨大，但存在"人工蜂群思维"效应，导致同一模型内和不同模型间生成相似响应，学生可能接触到过度相似和重复的问题，损害思维多样性。

Method: 提出CreativeDC两阶段提示方法，灵感来自Wallas创造力理论和Guilford发散-收敛思维框架，将LLM推理明确划分为不同阶段，解耦创造性探索和约束满足，使LLM在确定最终问题前探索更广泛的想法空间。

Result: CreativeDC在多样性、新颖性和实用性综合指标评估中，相比基线方法显著提高了多样性和新颖性，同时保持高实用性。扩展分析显示，随着采样增加，CreativeDC生成的有效不同问题数量更大，增长速度更快。

Conclusion: CreativeDC方法通过结构化两阶段提示，有效缓解LLM的"人工蜂群思维"问题，为教育内容生成提供了更富创造性和多样性的解决方案。
...
### [228] [Regret-Based Federated Causal Discovery with Unknown Interventions](https://arxiv.org/abs/2512.23626)
*Federico Baldo,Charles K. Assaad*

Main category: cs.AI

TL;DR: I-PERI算法解决联邦因果发现中的客户端未知干预问题，通过恢复客户端图并集的CPDAG，利用干预诱导的结构差异定向边，得到更紧的Φ-Markov等价类


<details>
  <summary>Details</summary>
Motivation: 现有联邦因果发现方法通常假设所有客户端共享相同的因果模型，这在实践中不现实，因为客户端特定策略（如医院间的不同协议）会引入异质且未知的干预

Method: 提出I-PERI联邦算法：1) 恢复客户端图并集的CPDAG；2) 利用干预在客户端间诱导的结构差异定向额外边，得到Φ-CPDAG表示的Φ-Markov等价类

Result: 提供I-PERI收敛性和隐私保护的理论保证，并在合成数据上进行实证评估，证明算法的有效性

Conclusion: I-PERI能够处理联邦设置下的未知客户端干预，通过利用干预诱导的结构差异获得更精确的因果发现结果，同时保持隐私保护
