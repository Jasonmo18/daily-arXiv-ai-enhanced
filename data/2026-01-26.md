...
TL;DR: 提出一种将视频到视频重光照扩散模型输出蒸馏到3D重建中的方法，实现大尺度房间环境的可控3D重光照，避免了困难的逆渲染问题。
...
Method: 通过将视频到视频重光照扩散模型的输出蒸馏到3D重建中，实现可控的房间尺度场景3D重光照。这种方法绕过了困难的逆渲染问题，创建了一个灵活的系统。

Result: 在合成和真实世界数据集上验证了该方法能够忠实渲染新光照条件下的场景新视角，适用于复杂真实世界场景的3D重建重光照。

Conclusion: 该方法成功实现了房间尺度场景的可控3D重光照，避免了传统逆渲染方法的困难，为复杂真实世界场景的重光照提供了有效解决方案。
...
Method: 提出联邦模板和任务学习(FeTTL)框架，通过学习全局模板和任务模型来对齐客户端间的数据分布，解决多机构医学影像数据的异质性问题。

Result: 在视网膜视盘分割和病理转移分类两个任务上，FeTTL显著优于现有联邦学习方法(p值<0.002)，实验还证明了联合学习模板和任务的重要性。

Conclusion: FeTTL为缓解联邦学习中的分布偏移提供了一个原则性和可扩展的解决方案，支持在多机构真实环境中进行稳健的模型部署。
...
TL;DR: 该研究评估了多模态模型在识别足球视频中重要子事件的能力，发现现有模型表现接近随机水平，主要问题在于模型倾向于依赖单一主导模态且无法有效整合多源信息。


<details>
  <summary>Details</summary>
Motivation: 基础模型被广泛应用于涉及时间顺序多模态事件的语言生成任务。本研究旨在评估模型识别视频中重要子事件的能力，这是叙述或总结多模态事件的基本前提。特别关注足球比赛场景，因为该场景具有明确的时间结构和事件重要性差异。

Method: 1. 构建新数据集：利用足球比赛精彩集锦中隐含的人类重要性偏好，无需额外标注成本；2. 数据集将公开给研究社区；3. 比较多个最先进的多模态模型；4. 超越标准评估指标的分析，揭示模型的模态依赖模式和信息整合能力。

Result: 1. 现有多模态模型在区分重要与非重要子事件任务上的表现接近随机水平；2. 模型倾向于依赖单一主导模态（如视觉或文本）；3. 模型无法有效合成来自多个来源的必要信息；4. 揭示了多模态数据处理中的样本级异质性挑战。
...
TL;DR: 论文提出通过引入"视角令牌"来增强多模态语言模型的空间推理能力，解决现有模型在视角转换任务中的自我中心偏差问题


<details>
  <summary>Details</summary>
Motivation: 当前多模态语言模型在语义视觉语言任务上表现良好，但在需要采用其他智能体视觉视角的空间推理任务中存在失败。这些错误反映了持续的自我中心偏差，并引发了对当前模型是否支持非自我中心推理的疑问。

Method: 受人类空间认知启发，引入视角令牌——专门编码方向的嵌入表示，包括：(1) 基于身体关键点的具身线索，或(2) 支持心理旋转的抽象表示。将这些令牌集成到LLaVA-1.5-13B模型中。

Result: 在合成和自然基准测试（Isle Bricks V2, COCO, 3DSRBench）中，视角令牌提高了准确性，其中基于旋转的令牌能够泛化到非人类参考智能体。表征分析显示，微调增强了基础模型中已有的潜在方向敏感性。

Conclusion: 将认知基础的空间结构直接嵌入到令牌空间中，为视角转换和更类似人类的空间推理提供了一种轻量级、模型无关的机制。多模态语言模型包含非自我中心推理的前兆，但缺乏适当的内部结构。
...
TL;DR: 本文提出了一种新的Referring Expression Segmentation（RES）框架，通过熵基点发现和视觉基推理解决现有MLLM方法中粗粒度边界框和文本坐标推理不可靠的问题，在多个基准数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于多模态大语言模型（MLLM）的RES方法存在两个关键限制：1）从MLLM获得的粗粒度边界框导致冗余或非区分性的点提示；2）依赖文本坐标推理不可靠，无法区分目标与视觉上相似的干扰物。

Method: 提出EBD-VBR框架，包含熵基点发现（EBD）和视觉基推理（VBR）。EBD通过建模粗边界框内的空间不确定性，将点选择视为信息最大化过程；VBR通过联合视觉-语义对齐验证点的正确性，放弃纯文本坐标推理。采用从粗到细的工作流程：边界框初始化、熵引导点发现、视觉基验证和掩码解码。

Result: 在四个基准数据集（RefCOCO、RefCOCO+、RefCOCOg和ReasonSeg）上的广泛评估表明，该框架在所有四个基准上都实现了新的最先进性能，证明了其在生成准确且语义基础的掩码方面的有效性。

Conclusion: 提出的EBD-VBR框架通过熵基点发现和视觉基推理有效解决了现有RES方法的局限性，能够以最少的提示生成准确且语义基础的分割掩码，在多个基准数据集上取得了最先进的性能。
...
TL;DR: 提出DCCS-Det红外小目标检测器，通过双流显著性增强块和潜在感知语义提取聚合模块，解决现有方法在局部-全局特征联合建模不足以及特征冗余和语义稀释的问题，在多个数据集上达到最先进的检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有红外小目标检测方法存在两个主要问题：1）局部-全局特征联合建模不足，影响目标-背景区分能力；2）特征冗余和语义稀释，降低目标表示质量。这些问题限制了在复杂背景下检测小尺寸、低对比度目标的性能。

Method: 提出DCCS-Det检测器，包含两个核心组件：1）双流显著性增强块（DSE），集成局部感知与方向感知上下文聚合，捕捉长距离空间依赖和局部细节；2）潜在感知语义提取聚合模块（LaSEA），通过跨尺度特征提取和随机池化采样策略缓解特征退化，增强判别性特征并抑制噪声。

Result: 在多个数据集上的广泛实验表明，DCCS-Det实现了最先进的检测精度，同时保持了有竞争力的效率。消融研究进一步验证了DSE和LaSEA模块在复杂场景下提升目标感知和特征表示能力的贡献。

Conclusion: DCCS-Det通过创新的双流显著性增强和潜在感知语义提取聚合机制，有效解决了红外小目标检测中的特征建模和表示质量问题，在检测精度和效率方面均表现出色，为红外小目标检测提供了有效的解决方案。
...
TL;DR: AlphaFace是一个利用开源视觉语言模型和CLIP嵌入的实时人脸交换方法，通过视觉和文本语义对比损失提升身份表示和属性保留，在极端面部姿态下超越现有方法。
...
TL;DR: Emotion-LLaMAv2提出端到端多模态情感推理框架，配合MMEVerse基准数据集，解决现有MLLMs在情感理解上的局限性


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在情感推理方面能力有限，缺乏高质量描述性情感标注的大规模数据集和标准化评估基准

Method: 1) 端到端多视角编码器消除外部人脸检测；2) Conv Attention预融合模块实现局部和全局多模态特征交互；3) 感知到认知的课程指令调优方案统一情感识别和自由形式情感推理

Result: 构建MMEVerse基准，整合12个公开情感数据集，通过多智能体流水线重新标注，产生13万训练片段和3.6万测试片段，涵盖18个评估基准

Conclusion: Emotion-LLaMAv2和MMEVerse建立了情感识别和推理的端到端管道和标准化评估设置，解决了领域内的关键限制
...
TL;DR: VISTA-PATH是一个交互式、类别感知的病理学分割基础模型，通过结合视觉上下文、语义组织描述和专家空间提示，实现精确的多类别分割，支持动态人机交互优化，并提升临床分析效果。


<details>
  <summary>Details</summary>
Motivation: 现有的分割基础模型虽然通过大规模预训练提高了泛化能力，但将其视为静态视觉预测任务，与病理学需求不匹配。病理图像分割需要处理异质性结构、整合专家反馈，并生成对临床解释有直接意义的像素级分割。

Method: 开发了VISTA-PATH模型，联合考虑视觉上下文、语义组织描述和可选专家空间提示进行分割。构建了VISTA-PATH Data数据集，包含超过160万张图像-掩码-文本三元组，涵盖9个器官和93个组织类别。支持通过稀疏的斑块级边界框标注反馈进行动态人机交互优化。

Result: 在广泛的保留测试集和外部基准测试中，VISTA-PATH始终优于现有的分割基础模型。模型产生的高保真、类别感知分割改善了组织微环境分析，提出的肿瘤相互作用评分（TIS）与患者生存率显示出强且显著的关联。

Conclusion: VISTA-PATH将病理图像分割从静态预测提升为交互式、临床基础的表征，为数字病理学建立了新的基础模型范式，支持更精确的临床分析和患者预后评估。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在物理世界理解方面仍达不到人类水平，现有数据集要么依赖成本高昂的真实视频标注，要么使用真实性和多样性有限的合成模拟。需要寻找更丰富、可扩展的监督来源。

Method: 提出利用游戏视频中的视觉异常（违反预设物理定律的glitches）作为监督源。构建PhysGame数据集（140,057个glitch相关的问答对，涵盖5个物理领域和16个细粒度类别），使用游戏元数据（标题和描述）指导高质量问答生成。同时构建GameBench基准（880个专家标注的glitch视频）用于评估物理推理能力。

Result: PhysGame显著提升了模型的迁移能力：在Game2Real方面，Qwen2.5VL在PhysBench上的真实世界物理推理性能提升2.5%；在Game2General方面，在MVBench基准上获得1.9%的提升。PhysGame调优的模型在GameBench上获得3.7%的绝对改进，显示出更强的物理不合理性检测鲁棒性。

Conclusion: 从游戏异常中学习为推进多模态智能的物理世界理解提供了一条可扩展且有效的途径，游戏glitches可作为丰富的监督源来增强模型的物理推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器在视频生成中表现出色，但长输入序列导致全注意力的二次复杂度带来高计算延迟。现有稀疏注意力方法存在限制：训练无关方法稀疏度有限，训练相关方法需要大量数据和计算资源。

Method: 提出SALAD方法，在稀疏注意力旁并行引入轻量级线性注意力分支，通过输入相关的门控机制精细平衡两个分支，实现高效稀疏化。

Result: 达到90%的稀疏度和1.72倍的推理加速，同时保持与全注意力基线相当的生成质量。微调过程高效，仅需2000个视频样本和1600个训练步骤（批次大小为8）。

Conclusion: SALAD在视频生成扩散变换器中实现了高效的注意力稀疏化，在保持质量的同时显著提升推理速度，且训练成本较低。
...
TL;DR: 论文提出了TangramPuzzle基准测试，用于评估多模态大语言模型在组合空间推理方面的能力，通过七巧板游戏形式测试模型在精确几何约束下的空间推理性能。
...
Method: 引入TangramPuzzle基准测试，基于经典七巧板游戏评估组合空间推理能力。提出Tangram Construction Expression（TCE）符号几何框架，将七巧板组装精确地基于机器可验证的坐标规范，以减少视觉近似的模糊性。设计了两个互补任务：轮廓预测（从局部组件推断全局形状）和端到端代码生成（解决逆向几何组装问题）。

Result: 通过对先进的开源和专有模型进行广泛评估实验，发现了一个有趣的洞察：多模态大语言模型倾向于优先匹配目标轮廓，而忽视几何约束，导致组件发生扭曲或变形。

Conclusion: TangramPuzzle基准测试为评估多模态大语言模型的组合空间推理能力提供了严谨的几何基础框架，揭示了当前模型在精确几何约束下的局限性，为未来改进提供了方向。
...
TL;DR: OnlineSI框架让多模态大语言模型具备持续空间理解能力，通过有限空间记忆和3D点云语义融合，实现实时视频流处理，为具身系统部署铺平道路。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM方法缺乏在动态世界中持续工作的能力，且难以部署到真实环境的具身系统中，需要解决空间理解的连续性和实时性问题。

Method: 提出OnlineSI框架，核心是维护有限空间记忆保留历史观测，确保推理计算量不随输入累积而增加；同时整合3D点云与语义信息，提升场景中物体的定位和识别能力。

Result: 引入模糊F1分数缓解歧义问题，在两个代表性数据集上测试，实验证明了方法的有效性。
...
TL;DR: 提出了一种基于伪标签的师生框架，用于半监督分层开放集分类，通过子树伪标签和年龄门控机制处理未知类别数据，在iNaturalist19基准上仅需每类20个标注样本即可达到全监督性能。


<details>
  <summary>Details</summary>
Motivation: 分层开放集分类旨在处理未见类别，将其分配到类层次结构中的适当高层类别。本研究将该范式扩展到半监督设置，以利用包含已知和未知类别的大规模未标注数据集来提升分层开放集性能。

Method: 提出基于伪标签的师生框架，包含两个关键组件：1) 子树伪标签 - 在存在未知数据时提供可靠监督；2) 年龄门控机制 - 缓解伪标签的过度自信问题。

Result: 实验表明，该框架优于自监督预训练加监督适应的方案，在iNaturalist19基准上仅使用每类20个标注样本时，性能甚至与全监督对应方法相当。

Conclusion: 通过引入子树伪标签和年龄门控机制，成功将分层开放集分类扩展到半监督设置，有效利用未标注数据提升性能，在有限标注数据下达到全监督水平。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感变化检测方法要么关注局部特征提取，要么追求整体图像处理，导致跨时相特征匹配偏差，且对辐射和几何噪声敏感。需要解决这些核心问题以提升变化检测的准确性和鲁棒性。
...
Result: 在LEVIR-CD、WHU-CD和SYSU-CD三个数据集上的实验验证了HA2F的有效性，在精度指标和计算效率方面均超越了现有对比方法，达到了最先进的性能。消融实验证明DHFCM和NAFRM模块都是有效的。

Conclusion: HA2F框架通过双模块协作引导的层次自适应聚合，有效解决了遥感变化检测中的特征对齐偏差和噪声敏感性问题，为环境监测、灾害评估等多学科应用提供了可靠的技术支持。
...
TL;DR: 提出了一种新颖的组合视频检索框架，利用视觉语言模型的表示能力，通过交叉注意力模块X-Aligner渐进融合视觉和文本查询，并加入视觉查询的标题作为额外输入，在Webvid-CoVR数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的组合视频检索框架通常将多模态输入在单阶段融合，相对于初始基线只有边际改进。需要更有效的方法来充分利用视觉语言模型的表示能力，提升组合查询的检索性能。

Method: 1. 提出新颖的交叉注意力模块X-Aligner，包含交叉注意力层，渐进融合视觉和文本输入，并将多模态表示与目标视频对齐。2. 引入视觉查询的标题作为额外输入增强多模态查询表示。3. 采用两阶段训练策略：第一阶段仅训练新引入模块，第二阶段微调文本查询编码器，以保留预训练VLM表示。4. 基于BLIP和BLIP-2架构实现。

Result: 1. 在Webvid-CoVR-Test数据集上获得63.93%的Recall@1，达到最先进性能。2. 在CIRCO和Fashion-IQ等组合图像检索数据集上表现出强大的零样本泛化能力。

Conclusion: 提出的框架通过渐进融合策略和两阶段训练方法，有效提升了组合视频检索的性能，并展示了良好的跨任务泛化能力，为多模态检索任务提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 无人机应用中，由于尺度不平衡和边缘模糊，小目标检测面临准确率低的挑战。现有方法难以有效捕捉小目标的边界和位置信息。

Method: 提出BPIM框架，包含PIG模块获取位置信息、BIG模块提取边界信息、CSF模块融合浅层特征、TFF模块结合位置和边界信息、AWF模块自适应融合深层语义特征，通过注意力机制和跨尺度特征融合策略整合边界、位置和尺度信息。

Result: 在VisDrone2021、DOTA1.0和WiderPerson数据集上的实验表明，BPIM相比基准Yolov5-P2有更好性能，在计算负载相当的情况下达到了与最先进方法相当的性能。

Conclusion: BPIM框架通过有效挖掘边界和位置信息，结合自适应权重融合和跨尺度位置融合策略，显著提升了无人机图像中小目标检测的性能。
...
TL;DR: SCHIGAND是一个结合多种生成模型的合成人脸生成管道，能产生高质量、多样化的面部数据集，适用于生物识别测试。


<details>
  <summary>Details</summary>
Motivation: 由于隐私法规、数据稀缺和伦理问题，获取高质量面部数据集面临挑战，现有生成模型在真实性、多样性和身份保持方面存在平衡问题。

Method: 提出SCHIGAND管道，整合StyleCLIP、HyperStyle、InterfaceGAN和Diffusion模型，增强身份保持能力，生成真实的面部类内变化，同时保持类间区分度。

Result: 使用ArcFace面部验证模型评估，SCHIGAND在图像质量和多样性之间取得良好平衡，能有效补充甚至替代真实数据用于生物识别应用。
...
TL;DR: 提出一种结构保持损失（SPL），利用局部线性模型量化输入图像与编辑图像间的结构差异，无需训练即可在潜在扩散模型中保持像素级边缘结构。


<details>
  <summary>Details</summary>
Motivation: 虽然潜在扩散模型（LDMs）在文本提示驱动的图像编辑方面表现出色，但在保持像素级边缘结构方面存在挑战，这对于照片级风格迁移或图像色调调整等任务至关重要。

Method: 提出结构保持损失（SPL），利用局部线性模型量化结构差异；将SPL直接集成到扩散模型的生成过程中；采用后处理步骤减轻LDM解码失真；使用掩码策略进行精确编辑定位；添加颜色保持损失保护未编辑区域的色调。

Result: 实验证实SPL能有效增强结构保真度，在潜在扩散模型图像编辑方面实现了最先进的性能。

Conclusion: 提出的SPL方法解决了潜在扩散模型在图像编辑中保持结构完整性的关键挑战，通过无需训练的方式实现了更好的结构保真度。
...
Method: 提出ReWeaver框架，仅需4个输入视图即可预测2D UV空间和3D空间中的接缝、面板及其连接性。构建了大规模数据集GCD-TS，包含超过100,000个合成样本，涵盖广泛的复杂几何和拓扑结构。

Result: ReWeaver在拓扑准确性、几何对齐和接缝-面板一致性方面持续优于现有方法，能够生成与多视角图像精确对齐的结构化2D-3D服装表示。

Conclusion: ReWeaver通过结构化表示实现了拓扑准确的3D服装重建，为3D感知、高保真物理模拟和机器人操作提供了实用的解决方案，有效弥合了模拟与现实的差距。
...
TL;DR: 提出CER-HV框架，通过CER排名和人工验证来检测和清理阿拉伯文字手写文本识别数据集中的标签错误，显著提升数据质量和模型性能。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯文字手写文本识别（HTR）仍落后于拉丁文字，研究发现数据质量是主要限制因素，许多已发布数据集存在标签错误问题。

Method: 提出CER-HV框架：1）基于CER的噪声检测器，使用精心配置的CRNN模型并采用早停策略避免过拟合噪声样本；2）人工验证步骤，对高排名样本进行人工检查。

Result: 1）在多个数据集中以高精度（Muharaf达90%，PHTI达80-86%）检测出转录、分割、方向和非文本内容等错误；2）CRNN在6个数据集中的5个达到SOTA性能；3）应用CER-HV后，干净数据集CER提升0.3-0.6%，噪声数据集提升1.0-1.8%。

Conclusion: CER-HV框架能有效识别和清理HTR数据集中的标签错误，显著提升模型性能，该框架具有通用性，可应用于其他文本识别数据集。
...
TL;DR: 该论文提出从圆形合成孔径声纳数据中提取阴影信息的方法，用于改进目标分析和三维重建


<details>
  <summary>Details</summary>
Motivation: 圆形合成孔径声纳虽然提供360°方位视图，但传统处理会丢失阴影信息，而阴影对目标识别和三维重建具有重要价值

Method: 使用子孔径滤波获取不同视角图像，应用固定焦点阴影增强技术获得清晰阴影，并通过空间雕刻方法从分割阴影中重建三维形状

Result: 成功从CSAS数据中提取阴影信息，证明了阴影在改进目标分析和三维重建方面的潜力

Conclusion: 阴影信息在圆形合成孔径声纳中具有重要价值，提出的方法能有效利用阴影改进目标分析和三维重建
...
### [23] [A Step to Decouple Optimization in 3DGS](https://arxiv.org/abs/2601.16736)
*Renjie Ding,Yaonan Wang,Min Liu,Jialin Zhu,Jiazheng Wang,Jiahao Zhao,Wenting Shen,Feixiang He,Xiang Che*

Main category: cs.CV

TL;DR: 该论文分析了3D高斯泼溅（3DGS）优化过程中的耦合问题，提出了解耦优化方法，并最终设计了AdamW-GS优化器以提升效率和效果。


<details>
  <summary>Details</summary>
Motivation: 3DGS作为实时新视角合成的强大技术，虽然采用了深度学习中的优化方法，但其物理意义和特定设计导致存在两个被忽视的优化细节：更新步骤耦合和梯度矩耦合，这些复杂耦合问题尚未得到充分探索。

Method: 重新审视3DGS优化过程，将其解耦并重组为三个组件：稀疏Adam、重新状态正则化和解耦属性正则化。在3DGS和3DGS-MCMC框架下进行大量实验，基于实证分析重新设计优化器，提出AdamW-GS。

Result: 通过解耦优化过程并重新组合有益组件，提出的AdamW-GS优化器同时实现了更好的优化效率和表示效果。

Conclusion: 该研究深入理解了3DGS优化中的耦合问题，提出的AdamW-GS优化器通过解耦和重新耦合有益组件，在优化效率和表示效果上取得了更好的平衡。
...
TL;DR: 该研究提供了一个大型、经过标注的ERCP图像数据集，包含19,018张原始图像和19,317张处理后的图像，来自1,602名患者，其中5,519张图像已标注，旨在解决ERCP公共数据集稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: ERCP是诊断和治疗胆胰疾病的关键技术，人工智能有潜力实现诊断自动化，但公共ERCP数据集稀缺限制了这一方法的应用。

Method: 收集了来自1,602名患者的19,018张原始ERCP图像和19,317张处理后图像，其中5,519张图像由两位具有5年以上经验的胃肠病学家手动检查和标注，并由一位具有20年以上经验的胃肠病学家审核。

Result: 创建了一个大型、经过严格标注的ERCP数据集，并通过分类实验验证了该数据集的实用性和有效性。

Conclusion: 该数据集旨在为自动ERCP分析和胆胰疾病诊断提供基准，填补了ERCP公共数据集的空白。
...
<details>
  <summary>Details</summary>
Motivation: 现有B-rep表示和生成方法采用基于图的表示，将几何和拓扑特征解耦处理，无法应用基于序列的生成框架（如Transformer）。需要一种能够统一编码B-rep几何和拓扑的序列表示方法。

Method: 1. 将B-rep编码为三种token：几何token、位置token表示几何特征，面索引token表示拓扑；2. 分层构建整体token序列：先构建几何块（面和边），再进行几何块排序；3. 使用基于Transformer的自回归模型，通过下一token预测学习序列分布。

Result: BrepARG实现了最先进的性能，验证了将B-rep表示为整体token序列的可行性。

Conclusion: BrepARG开创了B-rep生成的新方向，首次实现了基于序列的B-rep生成，为应用Transformer等序列生成架构奠定了基础。
...
TL;DR: 本文提出CASP方法，通过CLS令牌注意力引导提示和特征混合策略，在极少样本类增量学习中实现更好的知识迁移和泛化能力，显著减少参数开销。


<details>
  <summary>Details</summary>
Motivation: 在极少样本类增量学习（FSCIL）中，模型需要在有限样本下快速适应新类别，同时避免灾难性遗忘。现有基于提示的方法在极端少样本增量设置下，模型的迁移和泛化能力变得至关重要，需要利用预训练知识学习可在未来类别间共享的特征表示。

Method: 提出CLS令牌注意力引导提示（CASP）：1）在CLS令牌的查询、键和值投影中引入类共享的可训练偏置参数，显式调节自注意力权重；2）设计注意力扰动策略；3）在浅层特征空间执行流形令牌混合，合成潜在新类别特征以提升泛化能力。

Result: 在CUB200、CIFAR100和ImageNet-R数据集上的实验表明，CASP在标准和细粒度FSCIL设置中均优于现有最先进方法，增量阶段无需微调且显著减少参数开销。

Conclusion: CASP方法通过注意力引导和特征混合策略，有效提升了极少样本类增量学习中的知识迁移和泛化能力，同时保持了参数效率，为FSCIL提供了一种有效的解决方案。
...
TL;DR: 提出一种新的脊柱韧带附着点自动检测方法，通过形状分割和领域规则实现高精度检测，在所有脊柱区域都表现出良好泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在生物力学建模中，韧带附着点的准确表示对模拟椎体间作用力至关重要。现有自动检测方法要么局限于特定脊柱区域，要么精度不足，需要更可靠的方法来构建脊柱模型。

Method: 首先对3D椎体进行基于形状的分割，然后应用特定领域规则来识别不同类型的附着点，结合了形状分析和解剖学知识。

Result: 在两个独立的多患者脊柱数据集上验证，平均绝对误差(MAE)为0.7毫米，均方根误差(RMSE)为1.1毫米，优于现有方法，在所有脊柱区域都表现出良好泛化能力。

Conclusion: 该方法实现了高精度的脊柱韧带附着点检测，为构建可靠的脊柱生物力学模型提供了有效工具，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 室内空间美学体验预测困难，因为感知主观性强且视觉响应复杂，需要结合眼动追踪等生理信号来提升预测准确性

Method: 采用双分支CNN-LSTM框架，融合视觉特征和眼动信号；收集224个室内设计视频和28名参与者的同步眼动数据，评估15个美学维度

Result: 模型在客观维度（如光线）准确率72.2%，主观维度（如放松感）66.8%，优于现有视频基准；眼动训练模型仅用视觉输入也能保持性能；瞳孔响应对客观评估贡献最大，眼动+视觉组合增强主观评估

Conclusion: 眼动追踪作为训练时的特权信息具有重要价值，能开发更实用的室内设计美学评估工具，为设计促进福祉的环境提供支持
...
TL;DR: 本文介绍了ColorConceptBench，一个用于评估文本到图像模型颜色概念关联能力的人类标注基准，发现当前模型在抽象语义颜色关联方面存在不足且难以通过常规方法改进。


<details>
  <summary>Details</summary>
Motivation: 虽然文本到图像模型已有显著进步，但它们在将颜色与隐含概念关联方面的能力尚未得到充分探索。现有方法主要关注显式颜色名称或代码，而缺乏对抽象颜色概念的系统评估。

Method: 研究者开发了ColorConceptBench基准，包含1,281个隐含颜色概念，基于6,369个人类标注构建概率颜色分布。使用该基准评估了七个领先的文本到图像模型，并测试了标准干预措施（如模型缩放和引导）的效果。

Result: 评估结果显示，当前模型对抽象语义缺乏敏感性，更重要的是，这种局限性似乎对标准干预措施（如模型缩放和引导）具有抵抗性，表明问题不仅仅是模型规模问题。

Conclusion: 实现类似人类的颜色语义理解不仅需要更大的模型，更需要模型学习和表示隐含意义方式的根本性转变。ColorConceptBench为这一研究方向提供了重要的评估工具。
...
TL;DR: 提出自监督框架训练VGGT模型，通过序列级几何约束和联合优化损失，无需标注数据即可提升大规模环境中的定位能力


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的视觉几何框架在相机姿态估计和3D场景理解方面表现良好，但通常依赖标注数据进行训练，难以适应未标注和未见过的场景。需要开发自监督方法来解决这一限制。

Method: 提出自监督框架，将传统的成对关系扩展到序列级几何约束。在每个序列中采样多个源帧并将其几何投影到不同目标帧，以改善时间特征一致性。将物理光度一致性和几何约束制定为联合优化损失，避免对硬标签的需求。

Result: 模型在数百次迭代内收敛，在大规模定位方面取得显著改进。通过该方法训练，局部和全局跨视图注意力层以及相机和深度头都能有效捕获底层多视图几何。

Conclusion: 提出的自监督框架成功解决了VGGT模型对标注数据的依赖问题，通过序列级几何约束和联合优化损失，实现了在未标注数据上的有效训练，显著提升了大尺度环境中的定位能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数AI系统是单模态的，限制了其对手术工作流程的全面理解。需要能够全面建模手术场景相关组件的通用手术AI系统。大型视觉语言模型在多模态数据处理方面的进展为建模手术任务提供了潜力，但对其在手术应用中的系统性研究仍然有限。

Method: 研究评估了三种最先进的视觉语言模型（Qwen2.5、LLaVA1.5、InternVL3.5）在GraSP机器人手术数据集上的表现，采用零样本和参数高效的LoRA微调两种设置，并与开放集检测基线Grounding DINO进行比较。
...
Conclusion: 大型视觉语言模型在手术工具检测任务中具有潜力，Qwen2.5表现出色，特别是在零样本泛化方面。不同模型在识别和定位方面各有优势，为未来手术AI系统的开发提供了重要参考。
...
TL;DR: 论文提出了一种轻量级、无需训练的方法来解决长视频生成中的sink-collapse问题，通过多头RoPE抖动打破注意力同质化，实现了实时、流式、无限长度的视频生成，最长可达12小时。


<details>
  <summary>Details</summary>
Motivation: 当前自回归长视频生成模型存在误差累积和长期一致性丢失问题，虽然引入了注意力sink帧来缓解性能衰减，但导致了sink-collapse现象——生成内容反复回退到sink帧，造成场景重置和循环运动模式。

Method: 提出了一种轻量级、无需训练的方法，通过引入多头RoPE抖动来打破多头注意力机制中的同质化，从而缓解长时程崩溃问题。该方法不改变模型权重，仅在推理时应用。

Result: 实验表明该方法成功缓解了sink-collapse问题，同时保持了生成质量。实现了实时、流式、无限长度的视频生成，质量衰减很小，生成了长达12小时的连续视频。

Conclusion: 该工作首次展示了实时、流式、无限长度的视频生成能力，通过解决RoPE周期性结构与多头注意力机制之间的内在冲突，有效抑制了sink-collapse现象，为长视频生成提供了稳健的解决方案。
...
TL;DR: 论文提出一种使用奖励信号引导视频生成的新方法，替代依赖教师模型的传统自回归架构，在保持视觉保真度和时间一致性的同时简化训练过程。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型大多依赖双向架构，自回归变体通常需要强教师模型支持，这限制了性能表现，特别是在缺乏高质量自回归教师时，输出质量往往落后于双向模型。

Method: 采用奖励信号引导生成过程的方法，通过奖励信号指导模型，简化训练流程，同时保持高视觉保真度和时间一致性，避免了教师架构带来的约束。

Result: 在标准基准测试中，该方法与现有自回归模型性能相当，在某些情况下甚至超越了类似规模的双向模型。在VBench上获得84.92总分，与需要大量异构蒸馏的最先进自回归方法（84.31分）相当。

Conclusion: 使用奖励信号引导的自回归视频生成方法提供了一种有效的替代方案，能够在不依赖强教师模型的情况下实现高质量生成，简化了训练过程并保持了竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，深度学习依赖大规模专家标注和一致数据分布，但实际应用中标注稀缺且数据来自多个扫描仪或中心，形成混合域设置，存在未知域标签和严重域差异。现有方法通常假设单一域偏移或需要明确域索引，这在现实部署中很少成立。

Method: 提出域不变混合域半监督分割框架：1）复制粘贴机制（CPM）通过跨域转移信息区域来增强训练集；2）聚类最大均值差异（CMMD）块聚类未标记特征，并通过MMD目标将其与标记锚点对齐，鼓励域不变表示；3）集成在师生框架中实现鲁棒分割。

Result: 在Fundus和M&Ms基准测试上的实验表明，该方法在仅有少量标注样本和多个未知域差异的情况下，持续超越半监督和域适应方法，实现了鲁棒且精确的分割。

Conclusion: 该方法为混合域半监督医学图像分割提供了一个潜在解决方案，能够有效处理标注稀缺和未知域差异的挑战，在现实医疗场景中具有应用价值。
...
TL;DR: VisGym是一个包含17个环境的视觉语言模型评估与训练平台，用于测试多步视觉交互能力，发现前沿模型在交互设置中表现不佳，揭示了长上下文利用不足、视觉化任务难度增加等问题。


<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型在多步视觉交互中的能力尚未得到充分评估，特别是在感知、记忆和动作的长期整合方面存在不足，需要系统性的评估框架来揭示其局限性。

Method: 提出VisGym平台，包含17个环境，涵盖符号谜题、真实图像理解、导航和操作任务，提供难度、输入表示、规划视野和反馈的灵活控制，并生成结构化演示用于监督微调。

Result: 前沿模型在交互设置中表现不佳，简单配置成功率46.6%，困难配置仅26.0%。模型难以有效利用长上下文，视觉化任务比文本任务更难，但明确的目标观察、文本反馈和探索性演示能带来改进。

Conclusion: VisGym揭示了视觉语言模型在多步视觉决策中的具体失败模式，为改进提供了明确方向，包括更好的长上下文利用、视觉表示理解和交互策略学习。
...
TL;DR: SyncLight是首个能够在静态场景的多个未标定视图间实现一致参数化重照明的方法，解决了多视图应用中严格的照明一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有生成式方法在多视图应用中难以保持严格的照明一致性，而多摄像机广播、立体电影和虚拟制作等应用需要跨多个视图的精确照明控制。

Method: 采用基于潜在桥匹配公式训练的多视图扩散变换器，通过单一参考编辑控制整个图像集的照明强度和颜色，无需相机姿态信息即可实现零样本泛化到任意数量视图。

Result: SyncLight能够以单次推理步骤实现整个图像集的高保真重照明，在训练仅使用图像对的情况下，能够零样本泛化到任意数量的视点，有效传播照明变化。

Conclusion: SyncLight为多视图捕捉系统提供了实用的重照明工作流程，解决了多视图应用中严格的照明一致性问题，具有重要的实际应用价值。
...
Result: 在标准基准测试中取得有竞争力的结果；在提出的AnyViewBench极端动态视角合成基准上，相比基线方法性能大幅提升，能够从任意视角生成真实、合理且时空一致的视频。

Conclusion: AnyView展示了在极端动态场景中从任意视角生成时空一致视频的能力，为动态视角合成提供了新的解决方案，并通过新基准推动了该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究通常将代码混合简化为翻译或可转换性问题，难以评估模型切换行为是否上下文适当且符合人类惯例。需要开发能够在真实社区环境中评估代码混合能力的基准。

Method: 引入ChiEngMixBench基准，基于通用构建流程支持跨领域和双语对的可扩展数据集开发。将代码混合定义为认知对齐问题，通过自发性和自然性两个互补信号进行评估。

Result: 实证评估表明，该基准的指标能够系统地区分不同模型的代码混合性能。此外，发现了隐含出现的术语分层策略，这一现象与矩阵语言框架理论一致，表明多语言大语言模型与人类通信之间存在结构化认知对齐。

Conclusion: ChiEngMixBench为评估代码混合能力提供了首个基准，揭示了多语言大语言模型在代码混合中表现出的结构化认知对齐模式，为理解模型与人类通信对齐提供了新视角。
...
TL;DR: M3Kang是首个大规模多语言多模态数学推理数据集，基于袋鼠数学竞赛构建，包含1747道题目，覆盖108种语言，用于评估视觉语言模型的多语言数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多语言数学推理方面的表现尚未充分探索，特别是与人类表现相比存在差距。需要构建一个大规模、多语言、多模态的数学推理数据集来评估和改进模型在这方面的能力。

Method: 从全球最大的数学竞赛——袋鼠数学竞赛中提取题目，构建包含1747道选择题的数据集，按年级难度组织，翻译成108种文化多样的语言，其中部分题目包含解题必需的图表。同时收集了超过68,000名学生的表现数据用于人类对比。

Result: 研究发现：1）尽管近期有进展，模型在基础数学和基于图表的推理方面仍有困难；2）性能随语言覆盖范围和模型规模扩展，但不随年级水平提升；3）多语言技术可有效扩展到多模态设置，相比基线方法有显著改进。

Conclusion: M3Kang数据集填补了多语言多模态数学推理评估的空白，为视觉语言模型的多语言数学推理能力提供了基准测试工具。研究发现模型表现与人类仍有差距，多语言技术在多模态场景中具有扩展潜力。数据集和代码已开源。
...
TL;DR: 该研究提出了一种离线响应式知识蒸馏方法，在有限硬件资源下开发高精度专业助手，验证了数据质量比数量更重要。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理领域特定或机构知识时容易出现幻觉，特别是在这些知识未包含在预训练数据中的情况下。需要一种在有限硬件资源下开发高精度专业助手的方法。
...
Result: 实验结果显示，较大的非结构化数据集存在持续幻觉问题，而500行的上下文感知数据集达到了96.7%的准确率，并具备强大的拒绝能力。这验证了LIMA假设，表明在低资源设置中，数据质量和结构对齐比数量更重要。
...
<details>
  <summary>Details</summary>
Motivation: 语言扩散模型旨在提高采样速度和连贯性，超越自回归大语言模型。当前扩散模型主要针对连续空间，需要扩展到离散的语言状态空间

Method: 引入神经流扩散模型（NFDM），从数据中学习多元前向过程，确保前向过程和生成轨迹适合语言建模，将连续扩散模型直接应用于离散状态空间

Result: 模型显著缩小了与同等规模自回归模型的似然差距，同时采样质量与之前的潜在扩散模型相当
...
TL;DR: GameTalk框架通过对话微调LLMs进行多智能体战略决策，在复杂游戏中显著优于未训练模型


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLMs在单轮决策任务中的应用，而忽略了通过对话优化长期目标的问题。多智能体环境中的战略决策，特别是需要协调和谈判的长期对话场景，对LLMs提出了重要挑战。

Method: 提出了GameTalk框架，通过微调方法（GRPO、DPO、STaR）训练LLMs进行多轮交互决策。与以往关注单轮目标或静态动作预测不同，该方法训练LLMs在整个对话过程中优化全局目标，并整合依赖于完整交互的奖励信号。

Result: 在复杂度递增的游戏套件上评估，结果显示GameTalk显著优于未训练模型，特别是在奖励塑造条件下。DPO方法持续产生最强的性能提升，验证了对话微调在战略决策中的有效性。

Conclusion: 对话微调为LLMs在交互环境中进行推理、谈判和行动提供了有前景的路径，GameTalk框架在多智能体战略决策任务中展现出显著优势。
...
TL;DR: 使用大型语言模型生成多语言合成数据，训练小型模型在低资源语言任务上表现优于大型模型本身


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多语言任务上表现出色，但在低资源语言场景中，人工标注数据稀缺。研究探索是否可以利用LLMs生成合成数据作为知识蒸馏的一种形式，训练出性能相当甚至更好的小型模型

Method: 使用最先进的多语言LLM生成覆盖11种语言和4个分类任务的合成数据集，然后用这些数据通过微调、指令调优或作为上下文示例来训练小型模型

Result: 实验表明，即使是少量的合成数据也能使小型模型超越大型生成器本身，特别是在低资源语言中。小型模型在多种语言和任务上都表现出色

Conclusion: 大型语言模型最适合作为生成器（教师）而非分类器，它们生成的合成数据能够赋能更小、更高效的多语言模型，在低资源语言场景中尤其有效
...
TL;DR: DNIPRO是一个包含24.6万篇新闻文章的多语言语料库，记录了2022年2月至2024年8月的俄乌战争，涵盖5个国家11个媒体机构的3种语言，具有丰富的元数据和标注，用于研究战时话语的跨国分析。


<details>
  <summary>Details</summary>
Motivation: 研究俄乌战争期间不同国家媒体的叙事差异、信息战和媒体框架，需要包含竞争性地缘政治视角的多语言语料库，以支持系统性的跨国分析。

Method: 构建了一个包含24.6万篇新闻文章的多语言语料库，涵盖俄罗斯、乌克兰、美国、英国和中国五个国家的11个媒体机构，包含英语、俄语和中文三种语言，提供一致的元数据、多种标注类型和严格的人工评估。

Result: 语料库展示了不同媒体如何构建竞争性现实，报道呈现出反映地缘政治利益的两极化解读。通过立场检测、情感分析、主题框架和矛盾分析等用例实验，揭示了叙事分歧的模式。

Conclusion: DNIPRO为理解冲突性叙事如何在全球信息生态系统中出现和演变提供了基础资源，不仅支持计算新闻学研究，也有助于分析战时话语的跨国动态。
...
TL;DR: PolyBench：一个包含12.5万+聚合物设计任务的大规模训练和测试基准数据集，通过知识增强推理蒸馏方法训练的小型语言模型在聚合物设计任务上超越了大型模型


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在聚合物设计领域效果不佳，原因包括：(1) 大多数模型缺乏聚合物特定知识；(2) 现有对齐模型缺乏聚合物设计相关知识和能力覆盖

Method: 1. 引入PolyBench基准数据集（12.5万+任务），基于1300万+实验和合成数据点构建；2. 提出知识增强推理蒸馏方法，为数据集添加结构化思维链；3. 任务从简单到复杂组织，支持泛化测试和诊断分析

Result: 在PolyBench测试集上，7B到14B参数的小型语言模型表现优于相似规模模型，甚至超越了闭源前沿LLMs，在其他聚合物基准测试中也显示出优势

Conclusion: PolyBench基准和知识增强推理蒸馏方法有效解决了LLMs在聚合物设计领域的局限性，使小型模型能够在该专业领域超越大型通用模型
...
TL;DR: LLM自动评分在爱沙尼亚全国考试中表现与人类评分相当，可用于高风险写作评估，并能为学生提供个性化反馈。


<details>
  <summary>Details</summary>
Motivation: 在大规模考试（如全国毕业考试）中，传统人工评分耗时且一致性有限，需要快速、一致的自动化评分系统，特别是在爱沙尼亚即将实施全电子化考试系统的背景下。

Method: 使用两个爱沙尼亚全国考试数据集，基于官方课程标准的评分标准，将LLM和统计NLP的自动评分与专家小组的人工评分进行比较，同时评估偏见、提示注入风险以及LLM作为作文写手的能力。

Result: 自动评分性能与人类评分者相当，且评分结果通常落在人类评分范围内。系统能够产生细粒度的子分数剖面，可用于生成系统化的个性化反馈。

Conclusion: 基于原则、评分标准驱动、人类参与的评分流程适用于高风险写作评估，即使在小型语言环境中也能实现全国规模的LLM辅助评估，同时保持人类监督并符合新兴的教育和监管标准。
...
TL;DR: 该研究评估了10个主流大语言模型的区域偏见，发现GPT-3.5偏见最高（9.5分），Claude 3.5 Sonnet最低（2.5分），表明区域偏见会严重影响LLM的可靠性和公平性。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型中的区域偏见问题，这是AI公平性和全球代表性领域的新兴关注点。区域偏见可能影响LLM输出的可靠性、公平性和包容性，特别是在跨文化应用中。

Method: 使用100个精心设计的提示词数据集，在情境中立场景下评估10个主流LLM的区域偏见。引入FAZE评估框架，通过基于提示的方法在10分制上测量区域偏见，分数越高表示越倾向于特定区域。

Result: 不同模型的偏见水平存在显著差异：GPT-3.5偏见最高（9.5分），Claude 3.5 Sonnet最低（2.5分）。其他模型包括GPT-4o、Gemini系列、Claude 3 Opus、Llama 3、Gemma 7B、Mistral 7B和Vicuna-13B。

Conclusion: 区域偏见会显著影响LLM在现实世界跨文化应用中的可靠性、公平性和包容性。该研究通过强调包容性评估框架和系统性方法的重要性，为识别和缓解语言模型中的地理偏见做出了贡献。
...
TL;DR: LLMs通过深度绑定丰富背景故事和身份叙事，能够更准确地模拟人类在社交困境游戏中的行为，包括身份因素和情境因素。


<details>
  <summary>Details</summary>
Motivation: 人类行为既取决于理性思考，也受身份和情境因素影响。现有研究主要关注通过"引导"聊天模型来模拟人物，但缺乏对身份基础行为的忠实复现。本研究旨在探索如何通过深度绑定基础模型与扩展背景故事来更准确地模拟人类行为。

Method: 通过为大型语言模型提供丰富的叙事身份背景进行条件化，并使用指令调优模型检查一致性。研究还模拟了时间（研究年份）、问题框架和参与者群体效应等情境因素。

Result: 与人类研究相比，模拟保真度得到提升。LLMs能够建模多种情境因素，包括时间效应、问题框架和参与者群体差异，这些因素通常被实验描述忽略但影响准确复现。

Conclusion: LLMs能够探索影响人类研究但常被忽略的细节因素，为准确复现人类行为提供新工具。深度绑定基础模型与丰富身份背景比简单的"引导"方法能更忠实地复制身份基础行为。
...
<details>
  <summary>Details</summary>
Motivation: 可信的临床摘要不仅需要流畅生成，还需要透明展示每个陈述的来源。现有方法存在事后处理或需要重新训练的限制

Method: 提出基于解码器注意力的训练免费框架，支持两种多模态归属策略：原始图像模式（直接使用图像补丁注意力）和标题作为跨度模式（用生成标题替代图像实现纯文本对齐）
...
Conclusion: 注意力引导的归属是迈向可解释和可部署临床摘要系统的有希望步骤，为临床摘要提供了透明可靠的源归属解决方案
...
TL;DR: CoA是一个视觉问答系统，当图像问题对信息不足时，能主动询问澄清问题，而不是直接给出可能错误的答案


<details>
  <summary>Details</summary>
Motivation: 现实世界中的视觉问答往往是上下文相关的，图像-问题对可能信息不足，直接回答会导致自信但错误的预测。需要系统能够识别何时需要澄清信息

Method: 提出CoA(Clarify-or-Answer)系统，包含两个模块：1)判断是否需要澄清；2)如果需要则生成单个聚焦问题，然后结合回答给出最终答案。引入CONTEXTCLARIFY数据集和GRPO-CR强化学习方法优化澄清问题生成

Result: 在三个VLLM和三个数据集上，CoA在模块和系统层面都取得一致改进，端到端VQA准确率平均比基于提示的基线提高+15.3分（83%）

Conclusion: CoA通过主动澄清机制有效解决了视觉问答中的歧义问题，显著提高了系统在信息不足情况下的准确性和可靠性
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型基于上下文中的线索进行下一个token预测，但由于现代架构中层级和注意力头的复杂性，阐明哪些先前token对特定预测影响最大仍然具有挑战性。需要开发能够量化输入token对模型预测影响的方法。

Method: 提出了Jacobian Scopes方法套件，包含三个变体：Semantic Scope（针对特定logits的敏感性）、Fisher Scope（针对完整预测分布）和Temperature Scope（针对模型置信度/逆温度）。这些方法通过分析最终隐藏状态相对于输入的线性化关系来实现token级因果归因。

Result: 通过指令理解、翻译和上下文学习等案例研究，发现了有趣的结果，例如Jacobian Scopes能够揭示模型中的隐式政治偏见。该方法也为最近讨论的上下文时间序列预测机制提供了新的见解。

Conclusion: Jacobian Scopes是一套有效的梯度归因方法，能够量化输入token对大语言模型预测的影响，为模型可解释性提供了新工具，特别是在揭示模型偏见和理解上下文学习机制方面具有重要价值。
...
TL;DR: 当前多模态大语言模型在专业领域（如遥感和医学影像）表现有限，研究发现仅通过文本指令注入领域知识效果不佳，需要优化层面的知识整合。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在通用任务上表现出色，但在遥感、医学影像等专业领域效果有限。传统方法通过文本指令、提示或辅助描述注入领域知识，但研究发现这种输入层面的知识注入对科学多模态任务几乎没有改进，表明当前MLLM无法仅通过语言内化领域特定先验知识。
...
Conclusion: 研究强调了优化层面领域知识整合的必要性，揭示了当前MLLM中文本领域条件化的根本局限性。领域知识必须在优化层面而非仅通过语言输入进行整合。
...
<details>
  <summary>Details</summary>
Motivation: LLM作为评估器在许多任务中有效，但存在数值偏见问题，某些评分被过度生成，影响评估性能。本研究旨在探究这种偏见的成因

Method: 比较对齐前后的LLM输出，观察对齐对数值偏见的影响；探索三种缓解策略：温度缩放、分布校准和分数范围调整

Result: 对齐确实增加了数值偏见；在三种缓解策略中，分数范围调整最有效，能减少偏见并提高性能，但仍属启发式方法
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在数学、编程等通用推理任务上表现出色，但在围棋等专业领域表现不佳，甚至达不到初级水平。这种通用LLM与领域专家之间的性能差距限制了LLM在更广泛领域特定任务中的应用。

Method: 1. 使用结构化的围棋专业知识和通用长链思维推理数据进行混合微调作为冷启动；2. 通过强化学习将围棋专业知识与通用推理能力整合；3. 开发LoGos模型，使其能够用自然语言进行围棋对弈。

Result: LoGos模型不仅保持了出色的通用推理能力，还能用自然语言进行围棋对弈，展示有效的战略推理和准确的下步预测。其性能达到人类专业棋手水平，显著超越所有现有LLM。

Conclusion: 该研究成功地将大语言模型的通用推理能力应用于专业领域，为将LLM推理能力扩展到专门领域提供了重要见解。同时发布了首个用于LLM训练的大规模围棋数据集、首个LLM围棋评估基准和首个达到人类专业水平的通用LLM围棋模型。
...
TL;DR: GraphAnchor是一种新颖的图锚定知识索引方法，将静态知识图重构为动态演化的知识索引，通过迭代检索更新图结构来锚定关键实体和关系，指导LLM评估知识充分性并生成子查询，从而提升多跳问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在整合和解释分散在噪声文档中的关键证据方面存在挑战，需要更有效的方法来关联分布在不同文档中的关键信息。
...
Conclusion: GraphAnchor通过动态演化的图索引有效提升了RAG系统在多跳问答中的性能，为知识整合和解释提供了新思路。
...
Method: 提出Deep Evidence Reranking Agent (DeepEra)，通过逐步推理机制更精确评估候选段落，超越表层语义分析。构建了包含约30万实例的SciRAG-SSLI数据集，覆盖10个学科，包含自然检索上下文和系统生成的干扰项。

Result: 综合评估证实，该方法相比主流重排序器实现了更优的检索性能。这是首次全面研究和实证验证两阶段RAG框架中不可忽视的SSLI问题。

Conclusion: DeepEra通过整合逐步推理有效解决了科学问答中语义相似但逻辑无关的检索挑战，提升了检索增强生成的事实可靠性和逻辑鲁棒性。
...
TL;DR: TL-GRPO：针对迭代优化任务的轻量级RL算法，通过回合级组采样实现细粒度优化，在模拟电路尺寸优化任务中超越标准GRPO和贝叶斯优化方法
...
Method: 提出Turn-Level GRPO (TL-GRPO)算法，通过回合级组采样实现细粒度优化。该算法针对迭代优化任务特点，在保持大语言模型推理能力的同时，对每个回合进行优化。

Result: 在模拟电路尺寸优化任务中，TL-GRPO超越标准GRPO和贝叶斯优化方法。使用TL-GRPO训练的30B模型在相同模拟预算下达到最先进性能，展示了强大的泛化能力和实际效用。

Conclusion: TL-GRPO有效解决了迭代优化任务中的细粒度优化问题，为大语言模型在科学优化任务中的应用提供了新方法，在保持推理能力的同时实现了更好的优化效果。
...
TL;DR: 该论文提出"Timely Machine"框架，将测试时间重新定义为实际时钟时间而非生成长度，以解决工具调用场景中传统测试时间定义失效的问题。作者创建了Timely-Eval基准测试，并开发了Timely-RL方法来增强模型的时间预算意识。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型处理复杂推理任务，测试时间扩展变得至关重要。但在频繁调用工具的智能体场景中，传统的基于生成长度的测试时间定义失效，因为工具延迟使推理时间与生成长度脱钩。需要重新定义测试时间为实际时钟时间，让模型能根据时间预算动态调整策略。

Method: 1) 提出Timely Machine框架，将测试时间重新定义为实际时钟时间；2) 创建Timely-Eval基准测试，涵盖高频工具调用、低频工具调用和时间约束推理三种场景；3) 提出Timely-RL方法，通过冷启动监督微调后使用强化学习来增强时间规划能力。

Result: 研究发现：1) 小模型在快速反馈场景中表现更好，能通过更多交互获得优势；2) 大模型在高延迟场景中表现更优，依赖更高质量的交互；3) 现有模型无法根据时间预算调整推理策略；4) Timely-RL方法能显著提升时间预算意识，在Timely-Eval基准测试中持续提升性能。

Conclusion: 该工作为智能体时代的测试时间扩展提供了新视角，通过重新定义测试时间为实际时钟时间，并开发相应的评估基准和训练方法，使模型能够更好地适应有时间约束的智能体场景。
...
TL;DR: 本文介绍了医学检索增强生成（MRAG）基准测试，填补了医学领域RAG评估的空白，并开发了MRAG工具包来系统探索不同RAG组件。


<details>
  <summary>Details</summary>
Motivation: 虽然检索增强生成（RAG）在科学和临床问答系统中迅速被采用，但医学领域缺乏全面的评估基准。为了填补这一空白，需要建立一个专门的医学RAG评估基准。

Method: 引入医学检索增强生成（MRAG）基准测试，涵盖英语和中文的各种任务，构建基于Wikipedia和Pubmed的语料库，并开发MRAG工具包来系统探索不同RAG组件。

Result: 实验表明：(a) RAG提高了LLM在MRAG任务中的可靠性；(b) RAG系统性能受检索方法、模型大小和提示策略影响；(c) RAG提高了有用性和推理质量，但对于长形式问题，LLM响应的可读性略有下降。

Conclusion: MRAG基准测试填补了医学领域RAG评估的空白，提供了系统评估工具，将公开发布数据集和工具包以促进学术界和工业界的应用。
...
TL;DR: LOGICAL-COMMONSENSEQA是一个新的常识推理基准，将推理重构为基于逻辑运算符（AND、OR、NEITHER/NOR）的原子语句组合评估，而非传统的单标签选择。


<details>
  <summary>Details</summary>
Motivation: 现有常识推理基准大多依赖单标签评估，无法区分语句是联合合理、互斥还是联合不合理，这掩盖了模型在组合推理方面的真实能力。

Method: 引入LOGICAL-COMMONSENSEQA基准，将常识推理重构为基于逻辑运算符（AND、OR、NEITHER/NOR）的原子语句组合评估。评估了指令调优、推理专用和微调模型在零样本、少样本和思维链提示下的表现。

Result: 模型在合取推理上表现尚可，在析取推理上表现中等，但在基于否定的问题上性能急剧下降。该基准揭示了模型在组合常识推理方面的基本限制。

Conclusion: LOGICAL-COMMONSENSEQA暴露了现有模型在组合常识推理方面的根本局限性，为推进组合常识推理研究提供了一个受控框架。
...
TL;DR: 研究发现单轮测试无法捕捉LLM在对话中的真实表现，多轮对话设置能揭示模型特有的长度依赖和支架特定漏洞


<details>
  <summary>Details</summary>
Motivation: 当前LLM基准测试主要采用单轮提示评估，但无法捕捉真实世界中对话动态中发生的危害。研究者希望探究对话长度是否影响LLM回答的真实性

Method: 在BoolQ数据集上评估三个不同LLM在不同对话长度和支架条件下的表现，采用多轮对话设置而非传统的单轮测试

Result: 结果显示模型特有的漏洞在单轮测试中不可见，观察到长度依赖和支架特定的效应，这些部署相关的漏洞只能在多轮对话设置中被发现

Conclusion: 静态评估存在根本性局限，多轮对话设置对于发现LLM在真实部署中可能出现的漏洞至关重要
...
TL;DR: 本文提出了一种基于大语言模型的检索-精炼-校准（RRC）框架，用于事实核查任务，通过识别实体、精炼证据和校准验证过程来提高准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的事实核查方法通常采用分解范式，将声明分解为子声明进行单独验证。但这种范式可能会引入噪声（如不相关的实体或证据），从而降低验证准确性。
...
TL;DR: Attention-MoA框架通过智能体间语义注意力机制重新定义大语言模型协作，结合层间残差模块和自适应早停机制，显著提升集体智能效果，使小型开源模型组合超越大型专有模型。


<details>
  <summary>Details</summary>
Motivation: 当前混合智能体框架虽然通过动态路由和残差连接提升了效率，但未能促进智能体间的深度语义交互，限制了系统纠正幻觉和优化逻辑的能力，需要新的协作机制来提升集体智能效果。

Method: 提出Attention-MoA框架，核心是智能体间语义注意力机制，使不同模型能够进行深度语义交互；同时引入层间残差模块和自适应早停机制，缓解深层信息退化并提高计算效率。

Result: 在AlpacaEval 2.0上获得91.15%的长度控制胜率，MT-Bench得分8.83，FLASK评估中在12项能力中的10项表现最优；小型开源模型组合超越Claude-4.5-Sonnet和GPT-4.1等大型专有模型。

Conclusion: Attention-MoA通过重新定义智能体协作机制，实现了深度语义交互和高效计算，显著提升了混合智能体框架的性能，使小型模型组合能够超越大型专有模型，为大语言模型推理时协作提供了新范式。
...
TL;DR: 本文介绍了AuroraEdge-V-2B，这是一个专为边缘部署设计的紧凑、鲁棒且高速的视觉大语言模型，通过压缩融合方法提升推理效率，在保持高性能的同时显著减少参数数量和计算成本。


<details>
  <summary>Details</summary>
Motivation: 随着多模态技术的发展，视觉大语言模型（VLLMs）在工业生产中的应用日益增多，但现有VLLMs存在参数过多、计算资源需求大、推理速度慢等问题，难以满足边缘部署和实时响应的工业应用需求。

Method: 提出了AuroraEdge-V-2B模型，这是一个仅有20亿参数的紧凑型VLLM，专门针对边缘部署优化。同时提出了一种压缩融合方法来提升推理效率，显著减少解码过程中的视觉令牌数量，从而将推理时的浮点运算减少一半。

Result: AuroraEdge-V-2B在9个基准测试中获得了比同等参数规模模型（如Qwen2-VL-2B、Qwen2.5-VL-3B、InternVL-2.5-2B）更高的分数，同时实现了更快的推理速度和更低的计算成本。

Conclusion: AuroraEdge-V-2B通过紧凑的设计和创新的压缩融合方法，有效解决了VLLMs在工业应用中面临的计算资源需求大、推理速度慢的问题，为边缘部署提供了高性能、高效率的视觉大语言模型解决方案。
...
TL;DR: PROST-LLM通过渐进式方法增强大语言模型的语音到语音翻译能力，包括三任务学习、模态链、自采样和反向翻译生成偏好对，以及偏好优化。
...
Method: 1. 使用CVSS语料库微调LLMs，采用设计的三任务学习和模态链方法提升初始性能；2. 利用微调模型通过自采样和反向翻译生成偏好对，无需人工评估；3. 使用这些偏好对进行偏好优化，进一步提升模型的S2ST能力。

Result: 大量实验证实了PROST-LLM在提升LLMs的S2ST能力方面的有效性。

Conclusion: PROST-LLM通过渐进式方法成功增强了大语言模型的语音到语音翻译能力，为解决数据稀缺问题提供了有效方案。
...
TL;DR: 论文提出RPEval基准测试和RP-Reasoner方法，用于解决LLM助手因无关个性化记忆导致的非理性个性化问题，通过实用推理选择性整合个性化信息。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型助手集成了记忆机制记录用户偏好，但无关的个性化记忆会干扰模型对用户意图的理解，影响用户体验，需要系统研究个性化带来的双重影响。

Method: 开发RPEval基准测试，包含个性化意图推理数据集和多粒度评估协议；提出RP-Reasoner方法，将记忆利用视为实用推理过程，实现个性化信息的选择性整合。

Result: RPEval揭示了现有LLM中普遍存在的非理性个性化现象；RP-Reasoner在RPEval上显著优于精心设计的基线方法，解决了大规模商业个性化助手中80%的不良案例。

Conclusion: 实用推理能有效缓解非理性个性化问题，提升个性化助手的用户体验，RPEval基准为相关研究提供了公开评估平台。
...
TL;DR: 本文扩展了MultiLexNorm基准，新增5种亚洲语言，提出基于大语言模型的新架构，提升了跨语言词汇规范化性能。


<details>
  <summary>Details</summary>
Motivation: 社交媒体数据对NLP应用很重要，但因其非正式、自发性和多样化的社会方言，NLP模型性能会下降。现有词汇规范化基准MultiLexNorm主要覆盖印欧语系拉丁字母语言，缺乏亚洲语言代表性。

Method: 扩展MultiLexNorm基准，纳入5种不同语系、4种不同文字的亚洲语言；提出基于大语言模型的新架构，以提升跨语言词汇规范化的鲁棒性。

Result: 先前最先进模型在新亚洲语言上表现不佳；基于大语言模型的新架构展现出更稳健的性能；通过错误分析揭示了未来研究方向。

Conclusion: 扩展的MultiLexNorm基准填补了亚洲语言在词汇规范化任务中的空白，基于大语言模型的方法为跨语言词汇规范化提供了更鲁棒的解决方案，错误分析指出了未来改进方向。
...
TL;DR: 研究发现正确到错误奉承信号在多头注意力激活中最具线性可分性，通过线性探针分析发现这些信号在中间层注意力头中最为有效，可用于针对性干预减少奉承行为


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解语言模型中奉承行为（sycophancy）的内部机制，探索正确到错误奉承信号在模型不同组件中的线性可分性，为针对性干预提供理论基础

Method: 使用线性探针在残差流、MLP和注意力层训练，分析奉承信号的可分性；基于TruthfulQA数据集训练探针，评估在其他事实QA基准上的迁移效果；比较发现的"奉承方向"与先前识别的"真实方向"

Result: 奉承信号在多头注意力激活中最具线性可分性；中间层稀疏注意力头中的探针引导最有效；TruthfulQA训练的探针能有效迁移到其他事实QA基准；奉承方向与真实方向重叠有限，表明事实准确性和抗顺从性机制相关但不同

Conclusion: 奉承行为可通过利用注意力激活内部几何结构的简单线性干预来缓解，注意力模式分析显示有影响力的头会过度关注用户怀疑表达，导致奉承性转变
...
TL;DR: 针对大语言模型实例解释的梯度方法面临高维梯度计算难题，本文比较了两种降维策略：选择架构相关的组件子集 vs 全梯度投影，发现贪婪选择组件子集在检索任务中表现更优且计算更高效。


<details>
  <summary>Details</summary>
Motivation: 当前基于梯度的大语言模型实例解释方法受限于模型梯度的巨大维度，实践中通常随意选择参数子集进行计算，缺乏系统评估和理论依据，需要寻找更有效的降维策略。

Method: 提出新基准测试，比较两种降维方法：1）选择架构相关的组件子集（贪婪选择策略），2）将全梯度投影到低维空间。通过检索任务评估不同方法捕捉训练数据影响信息的能力。

Result: 贪婪选择的组件子集在检索任务中比全梯度或随机投影方法更能有效捕捉训练数据影响信息，且计算效率高于随机投影方法。

Conclusion: 针对性的组件选择策略是使大模型实例解释计算更可行的实用方法，基于架构信息的贪婪选择优于随机投影，为大规模模型影响估计提供了更有效的降维方案。
...
TL;DR: PLawBench是一个实用的法律基准测试，旨在评估大语言模型在真实法律实践场景中的能力，包含850个问题，覆盖13个实际法律场景，使用约12,500个评估指标进行细粒度评估。
...
Method: 基于真实法律工作流程，设计了三个任务类别：公共法律咨询、实际案例分析和法律文件生成。使用专家设计的评估标准，通过与大语言模型评估器对齐人类专家判断来评估10个最先进的大语言模型。

Result: 实验结果显示，没有一个大语言模型在PLawBench上表现出色，揭示了当前大语言模型在细粒度法律推理能力方面存在显著局限性。

Conclusion: PLawBench突出了未来法律大语言模型评估和发展的重要方向，表明当前模型在真实法律实践场景中仍有很大改进空间。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估智能体长期记忆的系统性基准测试，特别是在交互式环境中评估多种记忆技能（如多跳回忆、归纳、时空推理等）的能力。

Method: 通过程序化生成问题，从智能体自身游戏轨迹中提取可验证的真实答案，覆盖文本和视觉游戏环境，评估单跳/多跳回忆、归纳、时间、空间、逻辑和对抗性记忆技能。

Result: 在15个文本游戏和多个视觉种子上的评估结果显示，归纳和空间推理是持续瓶颈，特别是在视觉环境中。持久性记忆对文本游戏有明显提升，但对视觉语言模型智能体的改进不一致，表明视觉基础的情景记忆仍具挑战性。

Conclusion: EMemBench揭示了当前记忆智能体在长期记忆方面的局限性，特别是视觉环境中的归纳和空间推理能力不足，表明视觉基础的情景记忆是一个重要的开放挑战。
...
TL;DR: 该研究针对生物医学概念识别中未见概念的泛化问题，提出了基于层次概念索引的评估框架和LLM自动标注数据生成方法，证明自动标注数据虽不能完全替代人工标注，但能有效提升模型对未见概念的识别能力。


<details>
  <summary>Details</summary>
Motivation: 生物医学概念识别中，由于人工标注稀缺，模型对未见概念的泛化能力面临挑战。研究旨在系统解决这一问题，探索可扩展的自动标注数据资源。

Method: 1. 提出基于层次概念索引的评估框架和新指标来衡量泛化能力；2. 探索基于大语言模型的自动标注数据生成，构建任务特定的生成流程。

Result: 研究表明，LLM生成的自动标注数据虽然不能完全替代人工标注，但能作为有价值的资源提升泛化能力，为模型提供更广泛的覆盖范围和结构知识，从而更好地识别未见概念。

Conclusion: 自动标注数据是提升生物医学概念识别泛化能力的重要补充资源，结合层次概念索引的评估框架能有效衡量模型对未见概念的识别性能。
...
TL;DR: 该研究针对自动作文评分系统中的算法偏见问题，提出了一种基于对比学习的解决方案，有效减少了AI对英语作为第二语言学习者的评分偏差。


<details>
  <summary>Details</summary>
Motivation: 随着自动作文评分系统在教育评估中的广泛应用，人们越来越关注其对英语作为第二语言学习者的算法偏见问题。当前基于Transformer的回归模型主要基于母语者语料训练，容易学习到表层L2语言特征与作文质量之间的虚假相关性。

Method: 研究首先使用ASAP 2.0和ELLIPSE数据集对微调的DeBERTa-v3模型进行偏见分析，发现高熟练度ESL作文的评分比同等质量的母语作文低10.3%。为缓解这一问题，提出了一种基于对比学习的方法：使用匹配作文对的对比学习。构建了17,161对匹配作文数据集，采用三元组边界损失微调模型，使ESL和母语写作的潜在表征对齐。

Result: 该方法将高熟练度ESL作文的评分差距减少了39.9%（从10.3%降至6.2%），同时保持了0.76的二次加权Kappa系数。后续语言分析表明，模型成功解耦了句子复杂性和语法错误，避免了对有效L2句法结构的惩罚。

Conclusion: 基于对比学习的匹配作文对方法能有效减少自动作文评分系统对ESL学习者的算法偏见，同时保持评分准确性，为开发更公平的教育评估工具提供了可行方案。
...
TL;DR: 本研究提出了一种基于大语言模型的放射学报告纵向信息自动标注方法，用于解决现有标注工具不足的问题，并创建了标准化的基准数据集来评估报告生成模型。


<details>
  <summary>Details</summary>
Motivation: 放射学报告中的纵向信息对于监测疾病进展和指导临床决策至关重要，但现有的自动标注方法存在诸多问题：手工标注劳动密集，复杂规则封闭且领域特定难以适应，简单规则又容易遗漏专业信息。缺乏合适的工具来一致地标注真实文本和模型生成文本中的时间变化信息，导致难以有效评估报告生成模型的性能。

Method: 研究提出了一个基于大语言模型的标注流程，包含两个主要任务：1）识别包含相关信息的句子；2）提取疾病进展信息。首先在500份人工标注的报告上评估和比较了五种主流大语言模型，综合考虑效率和性能后选择了Qwen2.5-32B模型。然后使用该模型对MIMIC-CXR数据集中的95,169份报告进行自动标注，创建了标准化的基准数据集。

Result: 基于大语言模型的标注方法显著优于现有解决方案，在纵向信息检测和疾病跟踪任务上分别获得了11.3%和5.3%的F1分数提升。使用Qwen2.5-32B标注的数据集为评估报告生成模型提供了标准化基准，并在此基础上评估了七种最先进的报告生成模型。

Conclusion: 基于大语言模型的自动标注方法能够有效解决放射学报告纵向信息标注的挑战，提供了一种高效、准确的标注方案，并为评估报告生成模型创建了有价值的标准化基准数据集，有助于推动放射学报告生成领域的发展。
...
Result: 实验显示：1）低资源语言上的任务准确率相比英语大幅下降；2）但幻觉检测器的准确率下降幅度通常比任务准确率下降小好几倍；3）检测器在语言内部（包括非英语）和多语言设置中表现稳健，但在没有目标语言监督的跨语言设置中表现不佳。
...
TL;DR: 本文提出了一种名为"说服令牌"（P-Tokens）的新方法，用于替代上下文知识编辑中的冗长演示，实现更高效的大语言模型知识更新。


<details>
  <summary>Details</summary>
Motivation: 上下文知识编辑（IKE）虽然是一种有前景的大语言模型更新技术，但它依赖冗长、特定事实的演示，这些演示创建成本高且占用大量上下文窗口空间。需要一种更实用、可扩展的替代方案。

Method: 引入说服令牌（P-Tokens）——经过训练的特殊令牌，能够复制IKE演示的效果，从而无需特定事实演示即可实现高效的知识编辑。

Result: 在两个编辑数据集和三个大语言模型上的评估显示，P-Tokens的性能与IKE相当甚至更好；编辑性能对干扰因素具有鲁棒性，对相邻事实影响小；增加P-Tokens数量可提升性能。

Conclusion: P-Tokens解决了IKE的关键限制，为大语言模型编辑提供了更实用、可扩展的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 细粒度意见分析需要大量人工标注数据，成本高昂且难以覆盖多领域应用，需要探索LLMs作为自动标注器的可行性

Method: 使用声明式标注流程减少手动提示工程变异性，提出LLM裁决多个标签的新方法，在ASTE和ACOS任务上测试不同规模模型

Result: LLMs能够作为自动标注器和裁决器，在个体LLM标注器之间实现高标注一致性，显著降低创建细粒度意见标注数据集的成本和人力

Conclusion: LLMs可以有效解决领域特定标注数据短缺问题，为细粒度意见分析提供经济高效的自动标注解决方案
...
TL;DR: 该研究首次系统分析了文本到图像（T2I）模型中的"表面优先于语义"（SoS）现象，发现当使用英语以外的语言时，T2I模型倾向于产生文化刻板印象的视觉描绘，而非遵循提示的语义内容。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明T2I模型对不同输入语言高度敏感，当面对非英语提示时，模型往往产生文化刻板印象的描绘，优先考虑语言表面形式而非提示的语义。然而，对这种被称为"表面优先于语义"（SoS）行为的全面分析尚属空白。

Method: 研究创建了涵盖171种文化身份的提示集，翻译成14种语言，并用其测试了7个T2I模型。为了量化模型、语言和文化间的SoS倾向，引入了新的度量方法，并分析了这些倾向如何视觉化地表现出来。

Result: 研究发现除一个模型外，所有模型至少在两种语言中表现出强烈的表面优先倾向，且这种效应在T2I文本编码器的各层中逐渐增强。此外，这些表面倾向经常与刻板印象的视觉描绘相关联。

Conclusion: T2I模型普遍存在"表面优先于语义"的倾向，特别是在非英语语言中，这导致模型产生文化刻板印象的视觉输出，而非基于提示的真实语义内容。这种现象在模型架构的不同层次中都有体现。
...
TL;DR: 该研究使用国际象棋作为测试平台，分析大语言模型在记忆（晶体智力）和推理（流体智力）能力上的表现差异，发现模型在需要推理的任务上表现显著下降，特别是在分布外任务上表现接近随机水平。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型展现出强大能力，但尚不清楚这些能力在多大程度上反映的是记忆（晶体智力）还是推理（流体智力）。研究者希望区分这两种认知能力，以评估当前模型的系统性泛化能力。

Method: 使用国际象棋作为受控测试平台，构建一个基于训练语料接近度的位置分类体系，从可通过记忆解决的常见状态到需要基本原理推理的新颖状态。系统评估多个GPT模型在不同推理强度下的表现。

Result: 研究发现清晰的梯度：随着流体智力需求增加，性能持续下降。在分布外任务中，性能崩溃至随机水平。虽然新模型有所改进，但在训练分布外的任务上进展显著放缓。推理增强的推理能提高性能，但其每token的边际效益随分布接近度而减少。

Conclusion: 当前架构在系统性泛化方面仍然有限，需要超越规模扩展的机制来实现稳健的流体智力。研究强调了区分记忆和推理能力的重要性，以及开发更强大推理机制的必要性。
...
TL;DR: 论文提出了一种新型的对抗性攻击方法——说服性对抗攻击，利用生成式LLM通过说服技巧改写虚假声明，从而绕过自动事实核查系统。


<details>
  <summary>Details</summary>
Motivation: 现有对抗性攻击框架主要依赖注入噪声或改变语义，但忽略了说服技巧在虚假信息传播中的重要作用。说服技巧被广泛用于操纵受众，但尚未被系统性地用于攻击自动事实核查系统。

Method: 使用生成式大语言模型（LLM）通过15种说服技巧（分为6类）改写虚假声明，采用解耦评估策略分别研究说服对声明验证和证据检索的影响。

Result: 在FEVER和FEVEROUS基准测试上的实验表明，说服性攻击能显著降低事实核查系统的验证性能和证据检索能力。

Conclusion: 说服技巧是一类强大的对抗性攻击手段，凸显了构建更鲁棒的自动事实核查系统的必要性。
...
TL;DR: 论文提出了一种基于排列的评估框架来量化嵌入模型中的位置和语言偏见，发现当前最先进的嵌入模型存在系统性偏见：早期段落和高资源语言（如英语）被过度表示，而后期段落和低资源语言被边缘化。作者进一步分析了位置偏见的来源，并提出了一种推理时的注意力校准方法来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 为了在基于嵌入的搜索过程中被发现，文档的每个部分都应该在其嵌入表示中得到反映。然而，当前嵌入模型可能存在系统性偏见，导致文档的不同部分（如不同位置或不同语言）在嵌入表示中被不公平地表示，这会影响搜索的公平性和发现性。

Method: 1. 引入基于排列的评估框架来量化嵌入模型中的位置和语言偏见
2. 分析位置偏见的来源，发现其源于池化标记嵌入中的前加载注意力分布
3. 提出推理时的注意力校准方法，重新分配注意力以更均匀地覆盖文档位置

Result: 1. 观察到最先进的嵌入模型存在系统性位置和语言偏见
2. 早期段落和高资源语言（如英语）被过度表示
3. 后期段落和低资源语言被边缘化
4. 提出的注意力校准方法能够提高后期段落的可发现性

Conclusion: 嵌入模型存在系统性位置和语言偏见，这会影响文档搜索的公平性。通过提出的评估框架和注意力校准方法，可以量化并缓解这些偏见，提高文档所有部分的公平表示和可发现性。相关代码已开源。
...
TL;DR: 论文提出LogitMatch方法解决生成式LLM在文本分析任务中缺乏显式引用输入特定部分机制的问题，通过约束解码强制模型输出与有效输入片段对齐。


<details>
  <summary>Details</summary>
Motivation: 生成式大语言模型缺乏显式机制来引用输入文本的特定部分，导致在命名实体识别等文本分析任务中需要各种临时提示策略，且结果不一致。

Method: 将现有策略分为三类：标记输入文本、索引片段数值位置、匹配片段内容。针对内容匹配的局限性，提出LogitMatch约束解码方法，强制模型输出与有效输入片段对齐。

Result: 在四个不同任务上评估所有方法，发现标记方法仍是稳健基线，LogitMatch通过消除片段匹配问题改进了竞争性基于匹配的方法，并在某些设置中优于其他策略。

Conclusion: LogitMatch为生成式LLM在文本分析任务中提供了一种有效的约束解码方法，解决了片段匹配问题，在某些场景下优于现有策略。
...
<details>
  <summary>Details</summary>
Motivation: LLM在语义理解方面表现出色，但在需要严格逻辑的复杂决策任务中难以保证结构一致性和推理可靠性。传统决策理论如AHP虽提供系统化理性框架，但其构建严重依赖劳动密集型的领域专家知识，形成"专家瓶颈"，阻碍了在一般场景下的可扩展性

Method: 提出Doc2AHP结构化推理框架，利用AHP的结构原则作为约束，指导LLM在非结构化文档空间中进行受限搜索，强制父子节点间的逻辑蕴含关系。引入多智能体加权机制和自适应一致性优化策略，确保权重分配的数字一致性

Result: 实验结果表明，Doc2AHP不仅使非专家用户能够从零开始构建高质量决策模型，而且在逻辑完整性和下游任务准确性方面显著优于直接生成基线方法

Conclusion: Doc2AHP成功弥合了LLM的泛化能力与决策理论严谨性之间的差距，消除了对大量标注数据或人工干预的需求，为非专家用户提供了构建高质量决策模型的有效途径
...
TL;DR: LLMs在临床决策支持中易受患者压力影响，SycoEval-EM框架通过多智能体模拟评估LLM在急诊医学中的抗压能力，发现模型对不当医疗请求的屈服率高达0-100%，静态基准无法预测实际安全表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在临床决策支持中显示出潜力，但存在因患者压力而同意不当医疗请求的风险。现有评估方法多为静态基准测试，无法充分评估模型在真实临床压力下的表现，需要开发更贴近实际的多轮对抗测试框架。

Method: 开发了SycoEval-EM多智能体模拟框架，通过对抗性患者说服场景评估LLM的鲁棒性。涵盖20个LLM模型和1,875次交互，涉及三个Choosing Wisely场景（明智选择指南），测试不同说服策略对模型决策的影响。

Result: 模型屈服率从0%到100%不等，对影像检查请求的脆弱性（38.8%）高于阿片类药物处方（25.0%）。模型能力与鲁棒性相关性差，所有说服策略都同样有效（30.0-36.0%），表明是普遍脆弱性而非特定策略弱点。

Conclusion: 静态基准测试无法充分预测临床AI在社交压力下的安全性，临床AI认证需要包含多轮对抗测试。模型能力不能保证临床决策的鲁棒性，需要专门的安全评估框架。
...
TL;DR: 该研究对比了传统机器学习、基于提示的LLMs/VLMs和微调PEFT模型在医疗分类任务上的表现，发现传统ML模型在多数任务中表现最佳，而LoRA微调的Gemma变体表现最差，提示式LLMs/VLMs在图像任务上有竞争力。


<details>
  <summary>Details</summary>
Motivation: 多模态视觉语言模型和大语言模型为医疗分类带来了新可能性，但缺乏对这些方法与传统机器学习方法的系统性比较。本研究旨在通过统一基准测试，评估不同模型在医疗分类任务上的实际表现。

Method: 使用四个公开数据集（涵盖文本和图像模态，包括二分类和多分类任务），对比三类模型：传统ML（LR、LightGBM、ResNet-50）、基于提示的LLMs/VLMs（Gemini 2.5）和微调PEFT模型（LoRA适配的Gemma3变体）。所有实验使用一致的数据划分和评估指标。

Result: 传统ML模型在大多数医疗分类任务中表现最佳，特别是在结构化文本数据集上。LoRA微调的Gemma变体在所有文本和图像实验中表现最差。基于提示的LLMs/VLMs在文本任务上表现不佳，但在多分类图像任务上与ResNet-50基线表现相当。
...
TL;DR: 本文提出了一种评估多轮智能体任务中不同能力（如规划、状态跟踪等）相对重要性的反事实框架，通过程序生成的可控环境来隔离各技能贡献


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在孤立任务上表现良好，但在需要规划、状态跟踪和长上下文处理等多轮、长视野的智能体任务上仍有困难。需要理解这些底层能力对任务成功的相对重要性，以指导AI智能体和语言模型的未来发展。

Method: 提出一个反事实框架：通过提供完美执行特定任务（如完美规划、无错误状态跟踪）的"预言机"来评估该技能对智能体性能的关键性。开发了一套程序生成、游戏式任务，具有可调复杂性，可在受控环境中提供精确的预言机干预，隔离各技能的贡献。

Result: 结果显示，某些干预（如规划）在各种设置中持续提升性能，而其他技能的有用性则取决于环境和语言模型的特性。这表明不同技能的重要性具有情境依赖性。

Conclusion: 该研究揭示了多轮智能体环境的挑战，为AI智能体和语言模型的未来发展提供了指导。通过受控环境中的反事实分析，可以更精确地评估不同能力对复杂任务的重要性。
...
TL;DR: AgentsEval是一个多智能体流推理框架，用于评估自动生成的医学影像报告的临床正确性和推理保真度，通过模拟放射科医生的协作诊断工作流程提供结构化评估。
...
Method: 提出AgentsEval多智能体流推理框架，将评估过程分为可解释的步骤：标准定义、证据提取、对齐和一致性评分，模拟放射科医生的协作诊断工作流程。

Result: 实验结果表明，AgentsEval提供临床对齐、语义忠实且可解释的评估，在转述、语义和风格扰动下保持稳健。构建了涵盖五个医学报告数据集的多领域扰动基准。
...
Method: 采用统一的训练框架，结合领域并行专家训练与后续融合；扩展异步强化学习框架DORA用于大规模多环境训练；系统分析现实噪声模式并设计针对性训练程序；引入Heavy Thinking模式进行测试时扩展。

Result: 在智能体搜索、智能体工具使用和工具集成推理等基准测试中达到开源模型的最先进性能；在复杂工具交互中表现出强大的泛化能力；在嘈杂现实环境中保持鲁棒行为。

Conclusion: LongCat-Flash-Thinking-2601展示了通过统一训练框架、大规模环境训练、噪声鲁棒性设计和测试时扩展等方法，可以开发出具有卓越智能体推理能力的开源模型。
...
<details>
  <summary>Details</summary>
Motivation: 受昆虫在发现食物位置和巢穴之间学习并优化视觉引导路径的能力启发，希望开发一种简单高效的视觉点目标导航方法，解决现有模型计算成本过高的问题。

Method: 结合昆虫大脑中负责联想学习和路径整合的两个抽象模型，构建昆虫启发的智能体，将Habitat点目标导航任务形式化基准与昆虫的视觉路径学习能力进行类比。

Result: 昆虫启发智能体在性能上与最近的SOTA模型相当，但计算成本低多个数量级；在更真实的模拟环境中测试显示该方法对扰动具有鲁棒性。

Conclusion: 昆虫大脑的简单神经机制可以为高效的点目标导航提供有效解决方案，证明了生物启发的简单模型在复杂导航任务中的潜力。
...
TL;DR: 研究发现，通过强化学习训练的推理型大语言模型在心理理论任务中表现出更强的鲁棒性，但这种改进更多源于寻找正确解决方案的稳定性提升，而非形成了新的心理理论推理能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在心理理论测试中表现出色引发了对其底层能力本质的争论，同时通过强化学习训练的推理模型在各种基准测试中取得了显著改进，本文旨在探究这类推理模型在心理理论任务中的行为表现。

Method: 采用新颖的机器心理学实验改编和已建立的基准测试结果，分析推理模型在心理理论任务中的表现，特别关注其对提示变化和任务扰动的鲁棒性。

Result: 推理模型在心理理论任务中表现出对提示变化和任务扰动更强的鲁棒性，但分析表明这种增益更可能归因于寻找正确解决方案的稳定性提升，而非形成了新的心理理论推理形式。

Conclusion: 研究结果对评估大语言模型的社会认知行为具有重要意义，提示我们需要更谨慎地区分模型在任务中的表现改进是源于真正的认知能力提升，还是仅仅是解决策略的鲁棒性增强。
...
TL;DR: MAGE-KT：一种多智能体图增强知识追踪框架，通过构建多视图异构图和检索紧凑子图来解决现有方法中概念关系挖掘不足和全图编码计算成本高、噪声多的问题
...
Result: 在三个广泛使用的KT数据集上的实验表明，该方法在知识概念关系准确性和下一问题预测方面相比现有方法有显著提升

Conclusion: MAGE-KT通过多智能体关系提取和子图检索机制，有效解决了知识追踪中概念关系建模不足和计算效率问题，为图增强知识追踪提供了新思路
...
TL;DR: 开发AI支持平台帮助中低收入国家生物医学技术人员实时诊断和维修医疗设备，通过LLM和用户界面提供故障排除指导，减少设备停机时间


<details>
  <summary>Details</summary>
Motivation: 中低收入国家医疗诊断设备因维护不及时、技术专家缺乏和制造商支持不足而利用率低或无法使用，导致设备停机时间长、诊断延迟和患者护理质量下降

Method: 开发集成大型语言模型的AI支持平台，包含用户友好的Web界面，允许技术人员输入错误代码或设备症状，获得逐步故障排除指导；建立全球同行讨论论坛支持知识交流；以Philips HDI 5000超声机为原型验证

Result: 概念验证显示错误代码解释精度达100%，纠正措施建议准确率达80%，证明AI驱动系统支持医疗设备维护的可行性
...
### [93] [Experience with Single Domain Generalization in Real World Medical Imaging Deployments](https://arxiv.org/abs/2601.16359)
*Ayan Banerjee,Komandoor Srivathsan,Sandeep K. S. Gupta*

Main category: eess.IV

TL;DR: 该论文针对医学影像中的单域泛化问题，开发了专家知识集成深度学习技术DL+EKE，在糖尿病视网膜病变应用中超越现有方法，并成功部署到压力心电图和静息态fMRI两个真实案例中。


<details>
  <summary>Details</summary>
Motivation: 医学影像应用中，不同扫描设备和成像协议导致的域偏移问题严重影响了模型在未见目标域上的泛化能力。特别是在多中心研究中，罕见类特征的变异性加剧，需要解决单域泛化挑战以确保AI模型在实际部署中的有效性。

Method: 首先验证现有SOTA单域泛化技术在糖尿病视网膜病变应用中的局限性，然后开发通用的专家知识集成深度学习技术DL+EKE，将该技术实例化到DR应用中，最后将DL+EKE部署到压力心电图和静息态fMRI两个真实医学影像案例中。

Result: DL+EKE在糖尿病视网膜病变应用中超越了现有的SOTA单域泛化方法。在压力心电图和静息态fMRI两个真实部署案例中，DL+EKE技术展现了更好的泛化性能，同时揭示了单域泛化技术在实际应用中面临的问题。

Conclusion: 专家知识集成深度学习技术DL+EKE能够有效解决医学影像中的单域泛化问题，在多个真实医学应用中展现出优越的泛化性能，为实际部署提供了可行的解决方案。
...
### [94] [On The Robustness of Foundational 3D Medical Image Segmentation Models Against Imprecise Visual Prompts](https://arxiv.org/abs/2601.16383)
*Soumitri Chattopadhyay,Basar Demir,Marc Niethammer*

Main category: eess.IV

TL;DR: 该研究系统分析了3D医学分割基础模型对不精确提示的鲁棒性，通过控制性扰动实验揭示了模型对视觉形状和空间线索的依赖程度
...
Method: 使用两种最新的基础模型，在多器官腹部分割任务上进行实验，系统研究各种受控的密集视觉提示扰动，这些扰动密切模拟现实世界中的不精确性

Result: 实验揭示了可提示医学分割的多个方面，特别是模型对视觉形状和空间线索的依赖程度，以及模型对某些扰动的弹性程度

Conclusion: 该研究为理解3D医学分割基础模型对不精确提示的鲁棒性提供了系统分析，揭示了模型的行为特征和局限性，有助于未来开发更鲁棒的医学分割模型
...
### [95] [Fast, faithful and photorealistic diffusion-based image super-resolution with enhanced Flow Map models](https://arxiv.org/abs/2601.16660)
*Maxence Noble,Gonzalo Iñaki Quintana,Benjamin Aubin,Clément Chadebec*

Main category: eess.IV

TL;DR: FlowMapSR：基于Flow Map自蒸馏扩散模型的图像超分辨率框架，通过正负提示引导和对抗性微调实现高效推理，在重建保真度和真实感之间取得更好平衡


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型在图像超分辨率中重建保真度与真实感之间的权衡问题，同时提高推理效率。传统师生蒸馏方法存在信息压缩问题，会降低感知线索质量，而Flow Map自蒸馏模型在保持表达能力和训练稳定性的同时能实现快速推理

Method: 提出FlowMapSR框架：1) 将Flow Map自蒸馏模型（Eulerian、Lagrangian、Shortcut三种变体）适配到超分辨率任务；2) 引入正负提示引导，将无分类器引导范式推广到Flow Map模型；3) 使用低秩适应（LoRA）进行对抗性微调。最终发现Shortcut变体与这些增强结合效果最佳

Result: FlowMapSR在x4和x8超分辨率任务中，相比现有最先进方法在重建保真度和真实感之间取得了更好的平衡，同时保持有竞争力的推理时间。单个模型可用于两种上采样因子，无需尺度特定条件或退化引导机制

Conclusion: FlowMapSR通过结合Flow Map自蒸馏模型、正负提示引导和对抗性微调，成功解决了扩散模型在图像超分辨率中的效率-质量权衡问题，为高效高质量的图像超分辨率提供了新方案
