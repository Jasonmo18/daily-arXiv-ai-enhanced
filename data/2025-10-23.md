...
Motivation: 地球观测数据量不断增长，自动提取信息对于解决社会、经济和环境挑战至关重要。然而，高维数据带来了稀疏性、低效性和维度灾难等问题，限制了机器学习模型的有效性。

Method: 通过降维技术，特别是特征提取方法，在保持数据基本属性的同时减少复杂性，增强数据压缩、清洗、融合、可视化、异常检测和预测等任务的效果。

Result: 论文提供了在遥感数据价值链中应用降维技术的全面手册，识别了未充分探索的降维算法及其在未来研究中的应用机会。

Conclusion: 降维技术是解决地球观测高维数据挑战的关键工具，未来研究应进一步探索未充分利用的降维算法在遥感领域的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶中高层次感知、预测和规划问题的问答需求，利用预训练视觉语言模型但需要更好的提示工程和上下文基础来提升性能。

Method: 第一阶段：使用Qwen2.5-VL-32B模型，输入六摄像头数据、历史帧窗口和思维链提示；第二阶段：增加nuScenes场景元数据和任务特定指令。采用自一致性集成提高可靠性。

Result: 在驾驶QA基准测试中，第一阶段使用5个历史帧和10-shot提示达到65.1%准确率（零-shot为62.61%），自一致性提升至66.85%；第二阶段达到67.37%总体准确率。在严重视觉损坏下仍保持96%准确率。

Conclusion: 精心设计的提示工程和上下文基础可以显著增强预训练视觉语言模型在高级驾驶问答中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 解决纵向放射影像序列分析中的基本数据挑战：如何有效建模不规则时间间隔捕获的高分辨率图像序列，充分利用其中的空间和时间线索。

Method: 提出连续时间选择性扫描机制，将真实检查时间差明确整合到状态转换中，并辅以多尺度3D邻域融合模块来稳健捕捉时空关系。

Result: 在乳腺癌风险预测基准测试中，验证c-index提高2-5个百分点，1-5年AUC得分高于现有的循环、变换器和状态空间模型变体。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频扩散模型在生成时间一致和物理合理的运动方面存在不足，主要原因是模型对复杂运动的理解不够充分。现有方法通过将扩散模型特征与预训练视频编码器特征对齐，但这些编码器的特征混合了视频外观和动态信息，限制了对齐效果。

Method: 提出运动中心对齐框架：1）从预训练视频编码器中学习解耦的运动子空间，该子空间通过预测真实光流进行优化；2）将文本到视频扩散模型的潜在特征与这个新的运动子空间对齐，使生成模型能够内化运动知识。

Result: 该方法在VideoPhy、VideoPhy2、VBench和VBench-2.0等基准测试上进行了实证评估，并进行了用户研究，结果表明该方法提高了最先进视频扩散模型的物理常识性，同时保持了对文本提示的遵循。

Conclusion: 通过解耦运动子空间并与扩散模型特征对齐，能够有效提升文本到视频生成的运动合理性和物理常识性，而不影响文本遵循能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图像描述评估指标（如CIDEr、SPICE）是为短文本设计的，无法有效评估长文本描述中的属性和关系错误。需要一种能够定位特定文本跨度错误的新评估方法。

Method: PoSh使用场景图作为结构化评分标准，指导LLM作为评判者来评估详细图像描述。该方法还引入了DOCENT数据集，包含艺术作品、专家编写的参考描述和模型生成的描述，并由艺术史学生进行质量评估。

Result: PoSh在DOCENT数据集上与人类判断的相关性比最佳开放权重替代方案强0.05 Spearman ρ，对图像类型具有鲁棒性，并且能够作为有效的奖励函数，优于标准监督微调。

Conclusion: PoSh和DOCENT为详细图像描述评估提供了新的标准，揭示了基础模型在描述具有丰富场景动态的图像时难以实现完整、无错误的覆盖，为VLM进展设立了具有挑战性的新任务。
...
<details>
  <summary>Details</summary>
Motivation: 在多模态融合和生成应用中，人体姿态表示作为人本应用的重要组成部分，在多个下游任务中至关重要。目前缺乏对这些不同表示之间相关性的清晰研究，特别是在对比学习范式下。

Method: 提出UniHPR统一人体姿态表示学习管道，使用基于奇异值的对比学习损失来同时对齐多个数据表示（图像、2D和3D人体姿态）。
...
<details>
  <summary>Details</summary>
Motivation: 标准U-Net模型在处理不规则形状和模糊边界的脑肿瘤区域时存在挑战，且高分辨率MRI数据训练需要大量计算资源并面临类别不平衡问题。

Method: 将注意力机制集成到3D U-Net模型中，使模型能够捕捉复杂细节并优先处理信息丰富的区域；同时使用基于数字图像处理的肿瘤检测算法来解决训练数据不平衡问题。

Result: 在BraTS 2020数据集上，模型表现优于相关研究，获得Dice系数0.975、特异性0.988和敏感性0.995的优异结果。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺癌是女性主要死因，需要早期检测和准确诊断。医疗数据的敏感性使得开发准确且私密的AI模型具有挑战性，联邦学习虽然能保护隐私，但在非独立同分布数据上训练会影响模型准确性和泛化能力。

Method: 采用联邦近端方法处理非独立同分布的超声乳腺癌图像数据集，并结合改进的U-Net模型和注意力机制来增强肿瘤分割准确性。

Result: 该方法训练出的全局模型达到了96%的准确率，有效提升了肿瘤分割精度，同时保护了患者隐私。

Conclusion: 联邦近端方法在非独立同分布的本地医疗数据集上训练精确机器学习模型方面具有潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频理解研究主要依赖第三人称视角，忽略了多智能体学习的同步性和自我中心特性，需要从个体视角理解团队战术。

Method: 引入X-Ego-CS数据集（124小时专业级CS2比赛录像）和跨自我中心对比学习（CECL），对齐队友的自我中心视觉流以培养团队战术意识。
...
Method: 提出DARE框架，基于变形场梯度范数动态调整弹性正则化，集成应变和剪切能项，并包含折叠预防机制惩罚负变形雅可比区域。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常只能生成一两种人体运动动力学测量指标，缺乏能够从视觉输入直接联合预测多种运动动力学参数的统一方法。

Method: 提出FootFormer跨模态方法，直接从视觉输入联合预测人体运动动力学，包括足部压力分布、足部接触图和质心等参数。

Result: 在多个数据集上，FootFormer在足部压力分布、足部接触图和质心估计方面显著优于或等同于现有方法，并在稳定性预测组件（压力中心、质心、支撑面）估计方面达到最先进性能。

Conclusion: FootFormer能够有效从视觉输入联合预测多种人体运动动力学参数，在稳定性分析相关指标估计方面表现优异，为运动分析提供了有效的工具。
...
TL;DR: PruneHal是一种无需训练的KV缓存剪枝方法，通过减少冗余视觉token的注意力分散来缓解多模态大语言模型的幻觉问题，在多个基准测试中取得优异效果且几乎不增加推理成本。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在严重幻觉问题，现有解决方案需要额外训练或引入外部信息，带来额外计算成本。研究发现幻觉与视觉token注意力分配不足密切相关，冗余视觉token分散了模型对关键信息的注意力。

Method: 提出PruneHal方法，采用自适应KV缓存剪枝技术，在推理过程中动态剪枝冗余视觉token，增强模型对关键视觉信息的关注，无需额外训练且模型无关。

Result: 在多个主流幻觉评估基准上对四种主流MLLMs进行测试，获得了稳健且优异的结果，证明了方法的有效性和优越性。

Conclusion: PruneHal是首个将token剪枝应用于MLLMs幻觉缓解的方法，无需训练、几乎无额外推理成本，可无缝集成到不同解码策略中，为幻觉问题提供了简单有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统的奖励函数主要关注整个生成视频序列的质量（如美学吸引力和整体一致性），但在图像到视频生成任务中，生成视频的时间一致性往往受到影响，需要专门解决时序连贯性问题。

Method: 提出VCD度量标准，在视频帧特征频率空间中定义，通过频域分析有效捕捉帧信息，并基于奖励微调框架对视频生成模型进行微调。

Result: 在多个图像到视频数据集上的实验结果表明，使用VCD微调视频生成模型能显著增强时间一致性，且不会降低其他性能表现。

Conclusion: VCD是一种有效的度量标准，能够专门解决视频生成中的时间一致性问题，提升时序连贯性而不影响其他质量指标。
...
Result: Dream4Drive能够以前所未有的灵活性大规模生成多视角边缘案例，显著提升自动驾驶中的边缘案例感知。同时贡献了DriveObj3D大规模3D资产数据集，覆盖典型驾驶场景类别，支持多样化的3D感知视频编辑。
...
<details>
  <summary>Details</summary>
Motivation: 现有动态场景重建方法在不同场景下性能不一致，缺乏统一处理所有动态挑战的有效方法。

Method: 集成多个专家模型，使用体积感知像素路由器通过可微分权重溅射将高斯级权重投影到像素空间，实现自适应混合。

Result: 在N3V和Technicolor数据集上持续优于最先进方法，并提高了效率。

Conclusion: MoE-GS是首个将混合专家技术引入动态高斯溅射的方法，通过统一框架解决了动态场景重建的挑战。
...
Method: 使用小波变换增强单帧图像中的目标高频特征并抑制背景噪声；提出基于匈牙利算法的多帧时间轨迹补全方案进行跨帧匹配；设计包含时间匹配与插值补全、基于时间一致性的噪声过滤和渐进轨迹细化的后处理流程。

Result: 在公开SpotGEO数据集上的实验结果表明，该方法有效，F1分数达到90.14%。

Conclusion: 所提出的方法能够有效检测地球静止轨道空间目标，在复杂背景下实现高精度的目标检测和轨迹跟踪。
...
<details>
  <summary>Details</summary>
Motivation: 解决车联网带宽限制下协作感知的挑战，现有方法丢弃背景信息导致上下文缺失，影响感知性能。

Method: 采用课程学习策略，早期利用背景线索但逐步剪除，强制模型将上下文信息内化到前景表示中，无需传输背景本身。

Result: 在模拟和真实世界基准测试中，FadeLead在不同带宽设置下均优于现有方法。

Conclusion: 通过上下文丰富的前景共享，FadeLead有效提升了协作感知性能，证明了背景上下文内化的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 随着神经场、几何与运动深度学习以及3D生成AI的快速发展，4D生成与重建领域进展迅速。本文旨在从4D表示的角度提供一个独特的视角，帮助研究者选择合适的4D表示方法。

Method: 采用选择性方法，重点分析代表性工作而非全面列举，基于几何、运动和交互三个支柱对4D表示进行分类，涵盖NeRF、3D高斯泼溅等流行方法以及结构化模型、长程运动等较少探索的表示。

Result: 系统性地分析了不同4D表示方法的特性和挑战，讨论了LLM和视频基础模型在4D应用中的作用与局限，并提供了现有4D数据集的评估。

Conclusion: 本文的核心价值在于指导研究者如何根据具体任务选择和定制合适的4D表示方法，并指出了当前领域的局限性和未来发展方向。
...
Method: 使用ControlNet机制进行语义边缘引导，为单步推理提供动态结构控制；引入结合L2、LPIPS和边缘感知AME损失的混合损失函数。

Result: 实验表明该方法有效提升了结构完整性和真实感，同时保持了单步生成的高效性，在输出质量和推理速度之间实现了优越的平衡。

Conclusion: 提出的框架通过语义边缘引导和混合损失优化，成功解决了单步扩散模型在真实图像超分辨率中的结构不准确问题，实现了质量与效率的良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像扩散模型在语义对齐方面表现良好，但在生成提示中指定数量的对象方面仍然存在困难。现有方法只能使用可微的回归模型作为批评器，无法利用计数能力更强的检测器模型。

Method: 设计了自定义激活函数，将检测器logits转换为软二进制指示器，在推理时使用预训练的T2I模型优化噪声先验。

Result: 在SDXL-Turbo、SD-Turbo和Pixart-DMD等多个模型上的实验表明，在四个不同复杂度的基准测试中，对象计数准确性得到一致且显著的提升（例如在D2D-Small基准上提升高达13.7%），且图像质量和计算开销几乎没有下降。

Conclusion: D2D框架成功地将不可微检测模型的计数能力引入到文本到图像生成中，显著提升了对象计数的准确性，同时保持了图像质量。
...
<details>
  <summary>Details</summary>
Motivation: 程序性活动中的错误分析在工业自动化、物理康复、教育和人机协作等领域具有重要应用价值，需要系统性地研究基于视觉的错误检测和预测方法。

Method: 利用计算机视觉技术（包括动作识别、预测和活动理解）来识别任务执行中的偏差，如错误排序、不当技术使用或时间错误；对现有方法按程序结构使用、监督水平和学习策略进行分类。

Result: 建立了基于视觉的程序性活动错误分析的统一视角，系统梳理了该领域的技术方法、数据集和评估体系。

Conclusion: 基于视觉的错误分析在提高安全性、效率和任务性能方面具有巨大潜力，未来需要解决区分允许变化与真实错误、建模错误传播等挑战，并探索神经符号推理和反事实状态建模等方向。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型取得了显著进展，但其大规模特性使其在资源受限环境中不实用。因此需要开发高效训练算法来创建轻量级但性能强大的视觉语言模型。

Method: RIL算法结合强化学习和对抗模仿学习，使用基于LLM的判别器区分学生和教师输出，并利用多个大型教师VLM提供多样化指导。这种统一学习策略同时利用强化和模仿方法。

Result: 在多种视觉语言基准测试上的广泛实验表明，RIL显著缩小了与最先进开源和闭源VLM的性能差距，并在多个实例中超越了它们。

Conclusion: RIL通过统一强化和模仿学习，使学生模型实现了显著的性能提升，使其能够与领先的闭源VLM竞争，为资源受限环境提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的从遥感图像中推导水深或海底类别的方法将这些任务孤立处理，丧失了它们相互作用的共同好处，阻碍了深度学习方法的广泛应用。

Method: Seabed-Net采用双分支编码器进行水深估计和基于像素的海底分类，通过注意力特征融合模块和窗口化Swin-Transformer融合块整合跨任务特征，并通过动态任务不确定性加权平衡目标。

Result: 在两个异质沿海地点的广泛评估中，Seabed-Net始终优于传统经验模型和传统机器学习回归方法，实现了高达75%的RMSE降低。与最先进的单任务和多任务基线相比，水深RMSE降低了10-30%，海底分类准确率提高了8%。

Conclusion: 联合建模水深与海底底质和栖息地会产生协同增益，为集成浅水测绘提供了一个稳健的开放解决方案。
...
Method: 通过系统实验分析尺度偏移影响，建立ScaleBench基准测试20种先进DG算法，提出Causal Feature Decomposition and Anisotropic Processing (Catto)算法来缓解尺度偏移问题。

Result: 实验表明现有算法在尺度偏移问题上存在局限性，Catto算法能有效缓解尺度偏移影响，并揭示了四个重要的研究见解。

Conclusion: 尺度偏移是人群定位领域泛化中一个重要但研究不足的问题，本文提出的Scale Shift Domain Generalization为未来研究提供了新的方向和理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将fMRI映射到CLIP的最终语义层，忽略了中间层的丰富物体信息，且与大脑功能层次结构不符。添加VAE管道会增加大量参数，因此需要一种更高效的方法。

Method: 基于人类视觉系统功能层次结构，将不同视觉区域（低/高级）的fMRI信号对齐到CLIP的对应中间层和最终层，引入跨重建策略和多粒度损失函数。

Result: 在高层语义指标上达到或超越SOTA方法性能，同时参数减少71.7%，有效捕获了CLIP-only方法常遗漏的视觉细节。

Conclusion: BrainMCLIP通过利用CLIP中间特征，在语义准确性和细节保真度之间取得了良好平衡，无需单独的VAE管道，为fMRI图像解码提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决开集视觉识别中需要大量标注数据和训练成本高的问题，开发无需训练即可实现高质量分割和识别的框架。

Method: 采用两阶段流程：1) 使用EfficientNetB0提取特征并通过SVD和层次聚类进行无监督分割；2) 利用CLIP的视觉和文本编码器进行跨模态对齐，通过相似度计算实现开集识别。

Result: 在COCO、ADE20K和PASCAL VOC等基准测试中，在匈牙利mIoU、精确率、召回率和F1分数方面均达到最先进性能。

Conclusion: 该框架展示了无需训练即可实现有效、灵活和通用的开集图像分割与识别能力。
...
<details>
  <summary>Details</summary>
Motivation: 随着NeRF成为3D场景表示和视图合成的强大工具，保护其知识产权免受未经授权使用变得越来越重要。现有方法避免几何扰动或限制在显式空间中，因为这会严重降低渲染质量。

Method: 提出AegisRF框架，包含扰动场和敏感性场。扰动场在NeRF模型的预渲染输出中注入对抗性扰动来欺骗未经授权的下游目标模型；敏感性场学习敏感性来自适应约束几何扰动，在保持渲染质量的同时破坏未经授权使用。

Result: 实验评估表明AegisRF在多种下游任务和模态中具有通用适用性，包括多视图图像分类和基于体素的3D定位，同时保持高视觉保真度。

Conclusion: AegisRF成功解决了NeRF知识产权保护问题，通过联合优化扰动和敏感性场，在保持渲染质量的同时有效破坏未经授权使用。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型评估主要关注单视角设置，而多摄像头设置已成为机器人平台标准配置，但VLMs能否有效利用多视角输入进行机器人推理仍是一个开放问题。

Method: 构建包含1.7k个手动策划QA项的MV-RoboBench基准测试，涵盖8个子任务，分为空间理解和机器人执行两大类别，评估了包括开源和闭源模型在内的多种VLMs。

Result: 最先进的模型性能仍远低于人类水平，空间智能与机器人任务执行在多视角机器人场景中呈正相关，现有通用单视角空间理解基准的强性能不能可靠转化为机器人空间任务的成功。
...
<details>
  <summary>Details</summary>
Motivation: 随着电子商务的普及，物流市场在全球范围内增长，提高仓库运营效率至关重要。数字孪生技术为此提供了可能，但需要准确收集工人在仓库中的位置并在虚拟空间中反映。

Method: 在物流仓库天花板上安装19个向下拍摄的广角摄像头，基于地板表面对齐相机坐标与实际位置关系。针对广角摄像头边缘失真问题，基于检测到的工人脚部位置进行对齐，减少图像失真影响。

Result: 通过基于脚部位置的对齐方法，确认跟踪准确率提高了20%以上。比较了多种外观特征利用方法，验证了所提方法的有效性。

Conclusion: 基于脚部位置的对齐方法能有效减少广角摄像头图像失真的影响，实现跨摄像头的准确位置对齐，显著提高工人跟踪精度。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在客观多模态感知任务中表现出色，但在主观、情感细腻的领域（如心理分析）应用较少，特别是在HTP测试等临床心理评估中。

Method: 首先将包含多个实例的绘图分解为语义上有意义的子绘图，构建层次化表示；然后分析每个层次的子绘图，提取心理或情感洞察；引入HTP知识库和特征提取模块，通过强化学习训练生成心理档案；最后整合多方面信息生成专业评估。

Result: 实验结果表明，PICK显著增强了多模态大语言模型在心理分析中的能力，并通过扩展到情感理解任务验证了其作为通用框架的有效性。

Conclusion: PICK框架弥合了多模态大语言模型与专业领域之间的差距，为通过视觉表达理解人类心理状态提供了结构化和可解释的框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度分类技术和实验室自动光学检测模型在应用于真实世界IC缺陷分类任务时面临挑战，主要原因是真实世界数据分布比公开不平衡数据集更加倾斜，且样本包含类别特定属性和领域相关特征的混合。

Method: 提出ReCAME-Net模型，采用多专家分类器框架，集成了区域通道注意力模块、度量学习损失、困难类别挖掘策略和知识蒸馏过程。

Result: 在IC-Defect-14数据集上，ReCAME-Net超越了先前的最先进模型，同时在通用公共数据集上保持了可比较的性能和竞争力。

Conclusion: ReCAME-Net能够有效处理高度不平衡IC缺陷数据中的类内多样性和类间相似性挑战，为真实世界IC缺陷分类提供了有效解决方案。
...
Method: 使用生成模型预测目标对比度图像作为数据驱动先验，然后重建高度欠采样的数据。生成模型可基于多种数据源进行条件预测，包括其他对比度图像、历史扫描图像、采集参数和患者信息。
...
<details>
  <summary>Details</summary>
Motivation: 卫星图像中系统性缺失像素（如云层遮挡）会导致非随机缺失数据，如果不处理，会在自动化特征提取模型中产生表示偏差。特别是在甲烷羽流检测中，标签与缺失值数量之间的虚假关联会导致模型将图像覆盖率与标签关联，从而在低覆盖率图像中检测性能下降。

Method: 1. 评估多种插补方法以消除覆盖率与标签之间的依赖关系；2. 提出加权重采样方案，通过在每个覆盖率区间内强制类别平衡来消除标签与覆盖率之间的关联。

Result: 重采样和插补方法都能显著减少表示偏差，同时不会损害平衡准确率、精确率或召回率。在实际操作场景中，使用这些技术去偏的模型在低覆盖率图像中检测羽流的概率更高。

Conclusion: 通过插补和重采样方法可以有效解决卫星图像中系统性缺失像素导致的表示偏差问题，提高甲烷羽流检测模型在低覆盖率图像中的性能，为实际应用提供了有效的去偏解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决单源域泛化目标检测中模型因领域偏移和有限领域知识而陷入伪相关性的问题，即模型过度依赖简单分类特征而非本质的领域不变表示。

Method: 提出Cauvis方法：1）交叉注意力提示模块，通过视觉提示与交叉注意力结合减轻伪特征偏差；2）双分支适配器，通过高频特征提取实现领域适应并分离因果-伪特征。

Result: 在SDGOD数据集上，Cauvis相比现有领域泛化方法获得15.9-31.4%的性能提升，在复杂干扰环境中展现出显著鲁棒性优势。

Conclusion: Cauvis方法有效解决了单源域泛化目标检测中的伪相关性问题，实现了最先进的性能表现和鲁棒性。
...
Method: 使用紧凑VLM（350M）提取特征并预测目标预训练VLM响应何时收敛到其峰值回答能力，训练为离散分类器但推理时可插值连续分辨率实现细粒度控制。

Result: 在五个涵盖文档和自然图像的多模态基准测试中，CARES在保持任务性能的同时，计算量减少高达80%。

Conclusion: CARES通过智能选择最小足够分辨率，有效解决了视觉语言模型中视觉token过多导致的高计算成本问题，实现了计算效率与性能的良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理小重叠或无重叠图像对时表现不佳，视频插值生成的中间帧模糊且选择策略效率低下，无法满足姿态估计需求。

Method: 提出混合视频生成(HVG)结合视频插值模型和姿态条件新视角合成模型生成清晰中间帧，并使用基于特征对应的特征匹配选择器(FMS)选择适合姿态估计的帧。

Result: 在Cambridge Landmarks、ScanNet、DL3DV-10K和NAVI数据集上的实验表明，相比现有SOTA方法，PoseCrafter显著提升了姿态估计性能，尤其在小重叠或无重叠场景。

Conclusion: PoseCrafter通过混合视频生成和智能帧选择策略，有效解决了稀疏重叠图像对的相机姿态估计挑战。
...
Method: 在受控实验条件下研究7个最先进视觉语言模型在计数任务中的推理能力，进行层间分析以定位错误来源，并针对性地仅微调输出层进行训练。

Result: 实验表明视觉语言模型对物体数量、类型、空间排列和干扰物高度敏感。层间分析揭示错误主要源于最后一层表示到输出空间的映射问题。仅微调输出层即可将准确率提升高达21%，在真实数据集上也取得了一致的改进效果。

Conclusion: 视觉语言模型的计数能力缺陷主要源于输出映射问题，而非深层表示能力不足。通过针对性地微调输出层，可以显著提升模型性能，这为改进视觉语言模型的推理能力提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 虽然T2I模型具有生成无限合成数据的潜力，但提示工程作为主要交互方式，其对数据效用关键维度（质量、多样性、一致性）的系统影响尚未充分探索。

Method: 首先进行合成实验验证提示复杂度泛化的难度，并通过理论推导解释观察到的困难；然后引入新的评估框架比较真实数据和合成数据的效用，在CC12M、ImageNet-1k和DCI等多个数据集上分析提示复杂度的影响，并评估不同的推理时干预方法。

Result: 合成实验显示向更一般条件泛化比相反方向更难；实证实验表明增加提示复杂度导致条件多样性和提示一致性降低，但减少合成与真实数据的分布差异；当前推理时干预方法以偏离真实数据支持为代价增加生成多样性，其中提示扩展方法在图像多样性和美学质量上表现最佳。

Conclusion: 提示复杂度对T2I模型生成的合成数据效用有显著影响，提示扩展方法通过使用预训练语言模型作为似然估计器，在保持数据质量的同时有效提升多样性，甚至在某些方面超越真实数据。
...
<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型是否具备时间感知能力，即能否将视觉内容定位到时间维度上，这对于理解模型的时空理解能力具有重要意义。

Method: 引入了包含10,000多张图像的时间基准数据集TIME10k，评估了37个VLMs的时间感知能力，并基于时间信息在嵌入空间中的结构化特性，提出了从嵌入空间提取显式时间线表示的方法。

Result: 发现时间信息在VLM嵌入空间中沿低维非线性流形结构化，提出的时间线方法在时间推理任务中相比基于提示的基线方法达到竞争性甚至更优的准确率，且计算效率高。

Conclusion: 视觉语言模型确实具备时间感知能力，时间信息在嵌入空间中具有结构化特征，提出的时间线表示方法能够有效促进时间推理任务，为理解模型的多模态时空理解能力提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 环绕视角自动驾驶场景重建面临确保泛化能力同时提升新视角质量的核心挑战，现有方法由于环绕视角重叠区域最小，通常无法保证几何一致性和新视角重建质量。

Method: 设计轻量级VGGT变体从预训练VGGT中提取几何先验到几何分支；设计高斯头融合多尺度几何tokens预测新视角渲染的高斯参数；集成几何和高斯头分支的多尺度特征联合监督语义细化模型，通过特征一致性学习优化渲染质量。
...
TL;DR: 提出了一种新颖的多模态协同学习框架，能够在训练阶段利用多种传感器数据提升单模态模型在推理阶段的性能，适用于地球观测领域的分类和回归任务。
...
Result: 在四个地球观测基准测试中，涵盖不同传感器模态的分类和回归任务，相比现有最先进方法取得了持续的性能提升。

Conclusion: 该框架在单模态推理场景下，能够有效提升地球观测应用的预测性能，验证了方法的有效性和通用性。
...
TL;DR: 提出VAEEDOF方法，使用蒸馏变分自编码器进行多焦点图像融合，能同时处理7张图像，并创建MattingMFIF合成数据集解决数据稀缺问题，实现无伪影的融合效果。


<details>
  <summary>Details</summary>
Motivation: 解决光学镜头景深限制问题，传统方法和深度学习方法在多焦点图像融合中仍面临训练数据有限、合成数据集领域差距以及缺乏信息区域处理困难等挑战。

Method: 使用蒸馏变分自编码器进行高保真、高效的图像重建，融合模块能同时处理最多7张图像，并引入MattingMFIF合成4K数据集模拟真实景深效果。
...
Method: 采用全自动模块化框架，处理扫描或拍摄的心电图图像，包括处理透视畸变、褶皱和污渍等常见伪影。

Result: 在37,191张心电图图像上验证，在Akershus大学医院收集的1,596张图像上获得19.65 dB的平均信噪比，在Emory数据集上所有子类别均优于现有技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像伪造定位方法通常生成单一确定性定位图，缺乏高精度和可靠性，无法满足法医分析和安全监控等高风险应用的需求。需要提高预测可信度并降低错误风险。

Method: 提出条件伯努利扩散模型（CBDiff），在扩散过程中引入伯努利噪声以更好地反映伪造掩码的二元稀疏特性，并设计时间步交叉注意力机制（TSCAttention）利用语义特征与时序步骤指导改进操纵检测。

Result: 在八个公开基准数据集上的广泛实验表明，CBDiff显著优于现有最先进方法，显示出在真实世界部署的强大潜力。
...
TL;DR: 本文提出了首个系统性基准XBench，用于评估胸部X射线中跨模态可解释性，发现当前视觉语言模型在医学图像定位方面存在局限性，特别是在小病变和弥散病变上表现不佳。
...
Result: 研究发现：(1)所有VLM变体对大型和明确病理的定位表现合理，但对小型或弥散病变的定位性能显著下降；(2)在胸部X射线特定数据集上预训练的模型比对通用领域数据训练的模型具有更好的对齐效果；(3)模型的整体识别能力与定位能力高度相关。

Conclusion: 当前视觉语言模型尽管具有较强的识别能力，但在临床可靠的定位方面仍有不足，强调了在医学实践中部署前需要进行有针对性的可解释性基准测试的重要性。
...
TL;DR: AMR是一个零外部依赖的增强时刻检索框架，旨在解决现有方法面临的三个关键瓶颈：数据稀缺、边界模糊和细粒度语义区分不足。该框架通过两阶段训练（冷启动和蒸馏适应）来增强边界和语义区分能力，并在多个基准测试中优于现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 现有时刻检索方法面临三个关键瓶颈：(1) 数据稀缺导致模型陷入浅层关键词特征关联；(2) 相邻事件过渡区域的边界模糊；(3) 对细粒度语义（如区分"踢球"与"扔球"）的区分能力不足。

Method: 提出AMR框架，包含两阶段训练：冷启动阶段使用课程学习在增强数据上建立基础边界/语义意识；蒸馏阶段引入双查询集（原始查询和主动查询），通过跨阶段蒸馏损失确保知识一致性。

Result: 在多个基准测试中，AMR实现了比先前最先进方法更好的性能。

Conclusion: AMR框架通过解决现有标注中的模糊边界信息和语义混淆问题，同时保持增强的边界和语义区分能力，显著提高了时刻检索的性能。
...
TL;DR: 本文提出了MedReason-R1医学视觉语言模型，通过构建CT-RATE-VQA数据集和引入显式推理过程，解决了医学图像诊断中缺乏高质量数据集和忽略从粗到细诊断过程的问题。
...
Method: 构建CT-RATE-VQA数据集（84K QA对），提出MedReason-R1模型，嵌入放大病灶区域图像，并引入GRPO强化学习框架实现无需人工标注的有效推理。
...
TL;DR: 本文提出ReAct-GS方法，通过重新激活机制解决3D高斯泼溅中的过重建伪影问题，包括重要性感知的致密化准则和自适应参数扰动机制。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在复杂场景中会出现过重建伪影，表现为局部模糊和针状扭曲。作者发现这源于梯度幅度稀释和基元冻结现象，即复杂区域的高斯致密化被抑制，而次优尺度的高斯陷入局部最优。
...
### [49] [From Forecasting to Planning: Policy World Model for Collaborative State-Action Prediction](https://arxiv.org/abs/2510.19654)
*Zhida Zhao,Talas Fu,Yifan Wang,Lijun Wang,Huchuan Lu*

Main category: cs.CV

TL;DR: 提出了Policy World Model (PWM)新范式，将世界建模与轨迹规划统一，通过无动作未来状态预测方案利用学习的世界知识辅助规划，实现类人前瞻性感知。
...
Method: 提出PWM统一架构，集成世界建模和轨迹规划；采用无动作未来状态预测方案；引入动态增强并行令牌生成机制，包含上下文引导分词器和自适应动态焦点损失。

Result: 仅使用前视摄像头输入，性能达到或超过依赖多视角和多模态输入的最先进方法。

Conclusion: PWM通过协同状态-动作预测实现了类人前瞻性感知，提供了更可靠的规划性能，展示了世界建模与规划统一的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 呈现攻击是严重的安全威胁，深度学习模型虽然有效但通常作为黑箱运行，用户无法理解其决策过程。需要视觉解释技术来理解DL模型的行为和决策原因。

Method: 提出了Ensemble-CAM技术，用于为基于深度学习的面部呈现攻击检测系统生成视觉解释，识别导致系统将生物特征图像判定为真实或伪造的关键区域。

Result: 该方法能够提供详细的视觉解释，帮助用户理解深度学习模型在面部呈现攻击检测中的决策依据。

Conclusion: Ensemble-CAM技术增强了基于深度学习的面部呈现攻击检测系统的透明度和可信度，通过视觉解释使用户能够更好地理解系统行为。
...
TL;DR: LyTimeT是一个两阶段框架，通过时空注意力机制和稳定性约束从高维视频中提取可解释的动态系统变量，能够抑制视觉干扰因素并学习鲁棒的潜在表示。


<details>
  <summary>Details</summary>
Motivation: 从高维视频中提取真实动态变量面临背景运动、遮挡和纹理变化等视觉干扰因素的挑战，需要开发能够抑制这些干扰并学习稳定潜在表示的框架。

Method: 第一阶段使用时序Transformer自编码器，通过全局注意力关注动态相关区域；第二阶段使用线性相关性分析选择物理意义维度，并通过李雅普诺夫稳定性正则化器优化转移动态。

Result: 在5个合成基准和4个真实世界动态系统上的实验表明，LyTimeT在互信息和内在维度估计上最接近真实值，对背景扰动保持不变性，在CNN和Transformer基线中达到最低分析均方误差。

Conclusion: 结合时空注意力与稳定性约束可以产生不仅准确且物理可解释的预测模型，证明了该框架在动态系统变量提取中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有量化方法面临两个主要挑战：激活分布高度非均匀，以及权重量化中使用的静态、不匹配的码本。

Method: 提出自适应分布感知量化(ADQ)框架，包含三个关键创新：基于分位数的码本初始化、基于指数移动平均的在线码本自适应机制、以及基于敏感度的混合精度分配策略。对于激活，集成硬件友好的非均匀到均匀映射方案。

Result: 在ImageNet上，ADQ使ResNet-18在平均比特宽度仅为2.81位的情况下达到71.512%的Top-1准确率，在可比条件下优于最先进方法。在CIFAR-10上的消融研究验证了各创新组件的贡献。

Conclusion: ADQ框架通过自适应权重量化和硬件友好的激活映射，有效解决了量化训练中的关键挑战，在低比特量化下实现了优异的性能表现。
...
TL;DR: 本文介绍了Pico-Banana-400K数据集，这是一个包含40万张图像的大规模指令引导图像编辑数据集，旨在解决当前多模态模型研究中高质量开放数据集的缺乏问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在文本引导图像编辑方面取得了显著进展，但研究社区缺乏大规模、高质量、开放可访问的真实图像数据集，这限制了进一步的研究发展。

Method: 利用Nano-Banana从OpenImages集合的真实照片生成多样化的编辑对，采用细粒度图像编辑分类法确保编辑类型的全面覆盖，通过基于MLLM的质量评分和精心策划来保持精确的内容保存和指令忠实度。

Result: 构建了包含40万张图像的Pico-Banana-400K数据集，包含三个专门子集：7.2万示例的多轮编辑集合、5.6万示例的偏好子集，以及配对的长期短期编辑指令。
...
TL;DR: 该论文分析了单目深度估计的评估指标问题，发现现有指标对曲率扰动不敏感，提出了基于相对表面法线的新指标、新的深度可视化工具以及创建更好符合人类判断的复合指标的合理方法。
...
Method: 对现有指标进行定量分析，考察它们对地面真实值各种扰动的敏感性，并与人类判断进行比较；引入基于相对表面法线的新指标、新的深度可视化工具以及创建复合指标的合理方法。

Result: 分析显示现有指标对曲率扰动严重不敏感；提出的新指标和复合指标方法能更好地与人类判断对齐。

Conclusion: 论文揭示了现有深度估计评估指标的局限性，特别是对曲率扰动的敏感性不足，并提供了改进的评估工具和方法，有助于推动该领域的标准化评估。
...
TL;DR: olmOCR 2是一个强大的OCR系统，使用7B视觉语言模型，通过强化学习和可验证奖励进行训练，在复杂布局文档转换方面达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够将数字化打印文档（如PDF）转换为干净、自然排序的纯文本的OCR系统，特别关注数学公式转换、表格解析和多列布局等挑战性任务。

Method: 使用专门训练的7B视觉语言模型（olmOCR-2-7B-1025），采用基于可验证奖励的强化学习（RLVR），奖励来自多样化的二进制单元测试。通过生成具有多样化布局的合成文档来扩展单元测试创建。

Result: 在olmOCR-Bench英语OCR基准测试中达到最先进性能，在数学公式转换、表格解析和多列布局方面相比先前版本有最大改进。

Conclusion: olmOCR 2在复杂文档OCR任务中表现出色，模型、数据和代码已根据宽松开源许可发布。
...
TL;DR: 提出一种联合实体识别与消歧的统一框架，利用大语言模型丰富实体上下文，在跨域数据集上达到最先进性能。
...
Result: 在基准数据集上的评估显示，该方法在跨域数据集上达到了最先进的性能水平。

Conclusion: 提出的联合框架显著提升了实体链接性能，特别是在跨域场景下表现出色。
...
TL;DR: 提出使用小型语言模型（MiniLMs）从地球科学文献中精确、快速、经济地检索信息的框架，替代计算成本高的大型语言模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在科学文献处理中存在信息偏见和计算成本高的问题，需要开发更精确、高效的信息检索方法。

Method: 构建包含约7700万句高质量句子的地球科学文献语料库，使用MiniLMs通过语义搜索和句子级索引进行信息提取，并结合情感分析和无监督聚类分析。

Result: MiniLMs能够有效提取专家验证的领域特定信息，特别擅长处理定量研究结果，并能跟踪地球科学社区中结论演变、研究重点和新兴问题。

Conclusion: MiniLMs在地球科学社区具有巨大潜力，可用于事实检索、趋势分析、矛盾分析和教育目的，提供比大型语言模型更精确和经济的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大规模语言模型广泛部署，但系统评估其指令遵循能力仍然具有挑战性。现有基准可能被模型记忆，需要新的评估方法来评估真实能力而非记忆性能。

Method: 开发了一个紧凑的测试套件，包含20个精心设计的提示，针对指令遵循的不同方面（格式合规、内容约束、逻辑序列、多步骤任务执行），并在2025年10月14日对256个经过验证的模型进行了大规模实证研究。

Result: 研究揭示了持续存在的失败模式，并识别出特定指令类型带来的特殊挑战，提供了主要提供商和新兴实现模型的比较性能分析。

Conclusion: 这项工作既提供了一个实用的评估工具，也提供了当代LLM生态系统中指令遵循能力最全面的实证分析之一。
...
### [60] [A Graph Signal Processing Framework for Hallucination Detection in Large Language Models](https://arxiv.org/abs/2510.19117)
*Valentin Noël*

Main category: cs.CL

TL;DR: 提出基于谱分析的框架，将Transformer层建模为注意力诱导的动态图，通过图信号处理定义诊断指标，用于区分事实推理与幻觉。实验发现事实陈述呈现一致的低频收敛模式，而不同类型的幻觉具有不同的谱特征。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在取得显著成果的同时，区分事实推理与幻觉仍然具有挑战性。需要开发新的方法来检测和识别模型输出中的幻觉现象。

Method: 提出谱分析框架，将Transformer层建模为由注意力诱导的动态图，令牌嵌入作为图上的信号。通过图信号处理定义Dirichlet能量、谱熵和高频能量比等诊断指标，并与计算稳定性建立理论联系。

Result: 实验发现普遍的光谱模式：事实陈述呈现一致的"能量山"行为并具有低频收敛，而不同类型的幻觉显示不同的特征。逻辑矛盾使光谱不稳定（效应量g>1.0），语义错误保持稳定但显示连接性漂移，替换幻觉显示中间扰动。基于光谱特征的简单检测器达到88.75%的准确率，优于基于困惑度的基线方法（75%）。

Conclusion: 谱几何可能捕捉推理模式和错误行为，为大型语言模型中的幻觉检测提供了一个潜在框架。这些发现表明光谱特征在识别模型输出质量方面具有实用价值。
...
TL;DR: 本文研究了孟加拉语到锡尔赫特语的机器翻译，通过微调多语言Transformer模型并与零样本大语言模型比较，发现微调模型显著优于大语言模型。
...
Conclusion: 这些发现强调了针对代表性不足语言进行任务特定适应的重要性，有助于推动包容性语言技术的发展。
...
### [62] [Training-Free Spectral Fingerprints of Voice Processing in Transformers](https://arxiv.org/abs/2510.19131)
*Valentin Noël*

Main category: cs.CL

TL;DR: 该研究通过图信号处理分析transformer模型在语音转换任务中的注意力图，发现不同架构具有独特的"计算指纹"，可通过代数连通性变化检测模型的计算特征和语言偏向。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索不同transformer架构如何通过不同的连接模式实现相同的语言计算，并开发一种无需训练的诊断方法来揭示模型架构偏见。

Method: 使用图信号处理分析注意力诱导的token图，在20种语言和3个模型家族中跟踪代数连通性（Fiedler值）在语音转换期间的变化，重点关注早期层（2-5层）。

Result: 发现清晰的架构特征：Phi-3-Mini在英语中显示显著的早期层中断，其他19种语言影响最小；Qwen2.5-7B对形态丰富语言显示小规模分布式变化；LLaMA-3.2-1B显示系统性但温和的响应。这些光谱特征与行为差异强相关。

Conclusion: 训练重点会在模型中留下可检测的计算印记，表现为句法转换期间可测量的连接模式。该方法可作为简单、无需训练的诊断工具，用于揭示架构偏见和支持模型可靠性分析。
...
TL;DR: 使用小型语言模型（SLMs）进行微调，在机器生成内容检测任务中显著优于大型语言模型（LLMs），同时大幅降低计算成本和延迟。


<details>
  <summary>Details</summary>
Motivation: 当前基于零样本方法的机器生成内容检测器存在计算成本高或准确率不足的问题，需要在准确性和效率之间进行权衡，因此需要改进方案。

Method: 对预训练的RoBERTa和CodeBERTa等编码器专用小型语言模型进行微调，使用专门的源代码和自然语言数据集进行二元分类任务。

Result: 编码器模型在AUROC指标上达到0.97-0.99，宏F1分数达到0.89-0.94，同时将延迟降低8-12倍，峰值VRAM使用减少3-5倍。在跨生成器转换和对抗性变换下，性能保持原始AUROC的92%以上。
...
TL;DR: TM-Rephrase是一个模型无关的框架，利用大语言模型将原始推文重述为更标准化的语言，以改进社交媒体短文本的主题建模效果，特别是在COVID-19相关推文分析中表现优异。


<details>
  <summary>Details</summary>
Motivation: 社交媒体短文本（如推文）的简洁性、非正式性和噪声阻碍了传统主题建模的有效性，导致产生不连贯或冗余的主题，难以解释。

Method: 开发TM-Rephrase框架，使用大语言模型对原始推文进行重述（包括通用重述和口语到正式语言重述两种策略），然后应用多种主题建模方法进行分析。

Result: TM-Rephrase提高了主题建模性能的三个指标（主题连贯性、主题独特性和主题多样性），同时减少了大多数主题建模算法的主题冗余，其中口语到正式语言重述策略对LDA算法的性能提升最为显著。
...
TL;DR: 本文提出了正交多样性感知选择（ODiS）算法，通过主成分分析将多维度评分解相关，确保预训练数据在质量和多样性上的平衡选择，显著提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于分数的数据选择方法存在维度相关性偏差，导致选择高分数据时忽视多样性，直接选择高分数据反而降低模型性能，需要从更广范围采样才能恢复结果。

Method: 1. 从语言质量、知识质量和理解难度等多维度评估数据；2. 使用PCA对多维评分进行解相关，得到正交评估维度；3. 为每个维度训练Roberta评分器进行大规模推理；4. 在每个正交维度上选择高分数据构建训练集。
...
TL;DR: 提出了一种动态可逆的剪枝框架，通过检测上下文感知的神经元激活并应用自适应掩码来调节生成过程中的影响，实现细粒度、内存感知的偏见缓解。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型内部表示中存在不良行为，影响公平性、一致性，并放大有害内容。现有训练时方法计算成本高且不可逆，静态剪枝方法无法适应对话上下文变化。

Method: 采用动态可逆的剪枝框架，在推理时检测上下文感知的神经元激活，应用自适应掩码来调节其影响，保持知识完整性。

Result: 该方法在多语言单轮和多轮对话中实现了更一致的行为，支持动态公平控制，适用于现实世界对话AI。

Conclusion: 提出的推理时解决方案提供了灵活、透明的偏见缓解方法，能够适应不断变化的对话上下文，同时保持模型的知识完整性。
...
Method: 开发了高质量、多样化的全模态基准测试MMAO-Bench，包含1880个人工标注样本，涵盖44种任务类型，并引入了创新的多步开放式问题类型来更好评估复杂推理任务。
...
TL;DR: Re:Member是一个基于情感表达和记忆的第二语言学习系统，通过用户个人视频和风格化语音提问来促进情感回忆和对话参与。
...
Method: 使用WhisperX转录对齐、3帧视觉采样和Style-BERT-VITS2情感合成的模块化生成管道，将情感语调与视觉上下文对齐，采用耳语或深夜音调等表达性语音风格。

Result: 系统被设计为风格化交互探针，能够激发特定情绪，促进情感回忆和对话参与。

Conclusion: Re:Member突出了情感和个人媒体在学习者中心教育技术中的重要作用，为第二语言学习提供了新的交互范式。
...
TL;DR: ProfBench是一个包含7000多个由专业领域专家评估的响应-标准对的数据集，用于评估大语言模型在处理专业文档、信息合成和生成综合报告方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型的进展受到验证响应困难的限制，主要局限于数学、编程和简短问答等任务。然而，许多实际应用需要评估LLMs在处理专业文档、合成信息和生成综合报告方面的能力。

Method: 构建了ProfBench数据集，包含物理博士、化学博士、金融MBA和咨询MBA等专业领域的专家评估。开发了稳健且经济实惠的LLM-Judges来评估ProfBench标准，通过减轻自我增强偏见并将评估成本降低2-3个数量级。

Result: ProfBench对最先进的LLMs构成了显著挑战，表现最佳的模型如GPT-5-high仅达到65.9%的整体性能。发现了专有模型和开源模型之间的显著性能差异，并深入分析了扩展思维在处理复杂专业领域任务中的作用。

Conclusion: ProfBench为评估LLMs在专业领域的表现提供了一个公平且可访问的基准，揭示了当前模型在处理复杂专业任务时的局限性，并为未来模型开发提供了重要见解。
...
TL;DR: 该论文提出了两个用于评估大型语言模型在心理健康支持方面表现的基准：MentalBench-100k包含10万对对话响应，MentalAlign-70k包含7万条评分，通过情感认知一致性框架分析LLM评估者与人类专家的可靠性差异。


<details>
  <summary>Details</summary>
Motivation: 现有心理健康对话评估基准存在规模小、可靠性不足的问题，缺乏评估自动评估者可信度的框架，需要大规模对话数据集和评估者可靠性评估方法。

Method: 构建MentalBench-100k基准（整合1万轮真实场景对话，每轮配9个LLM生成响应）和MentalAlign-70k基准（比较4个高性能LLM评估者与人类专家在7个属性上的评分），采用情感认知一致性框架通过组内相关系数量化一致性、可靠性和偏差。

Result: 分析发现LLM评估者存在系统性评分偏高，认知属性（指导和信息性）可靠性强，共情能力精度降低，安全和相关性存在不可靠性。
...
### [71] [Dynamic Evaluation for Oversensitivity in LLMs](https://arxiv.org/abs/2510.19005)
*Sophia Xiao Pu,Sitao Cheng,Xin Eric Wang,William Yang Wang*

Main category: cs.CL

TL;DR: 该论文提出了OVERBENCH基准，用于动态评估语言模型的过度敏感性（过度防御良性提示），通过生成模型特定的挑战性数据集来捕捉新兴防御模式。


<details>
  <summary>Details</summary>
Motivation: 解决语言模型过度防御良性提示的问题，以及现有静态基准因模型演变导致数据污染和评估能力下降的局限性。

Method: 开发动态框架生成模型特定的挑战性数据集，构建OVERBENCH基准，包含来自25个模型的450,000个样本。

Result: OVERBENCH提供了动态演进的视角来监控过度敏感性，能够捕捉静态数据集忽略的漏洞。

Conclusion: 该框架能够持续监控模型防御触发器的演变，为语言模型的过度敏感性提供更有效的评估方法。
...
### [72] [Are they lovers or friends? Evaluating LLMs' Social Reasoning in English and Korean Dialogues](https://arxiv.org/abs/2510.19028)
*Eunsu Kim,Junyeong Park,Juhyun Oh,Kiwoong Park,Seyoung Song,A. Seza Dogruoz,Najoung Kim,Alice Oh*

Main category: cs.CL

TL;DR: 本文介绍了SCRIPTS数据集，用于评估大语言模型在人际对话中的社会推理能力，发现现有模型在英语和韩语数据集上表现差异显著，且存在选择不可能关系的倾向，思维模型和思维链提示对社会推理帮助有限。
...
Method: 构建了包含1,000个英语和韩语对话的SCRIPTS数据集，源自电影剧本。每个对话由母语者标注概率关系标签（高度可能、较不可能、不可能）。评估了9个模型在该任务上的表现。

Result: 当前专有LLM在英语数据集上达到75-80%准确率，但在韩语上降至58-69%。模型在10-25%的响应中选择不可能关系。思维模型和思维链提示对社会推理帮助有限，有时还会放大社会偏见。
...
TL;DR: 该研究评估了大型语言模型在生物医学术语标准化任务中的表现，发现微调效果取决于标识符的流行度和词汇化程度。基因标识符表现最佳，而GO和HPO等非词汇化标识符的泛化能力有限。
...
Result: GO术语映射显示强记忆增益（术语到标识符准确率提升达77%），而HPO改进最小。只有基因映射显示出泛化能力（13.9%增益）。GPT-4o在所有术语系统中都优于Llama变体。嵌入分析显示基因符号和蛋白质名称间语义对齐紧密，但GO和HPO的术语与标识符对齐较弱。

Conclusion: 微调成功取决于两个相互作用因素：标识符流行度和词汇化程度。流行标识符在预训练中更常见，增强记忆能力；词汇化标识符（如基因符号）支持语义泛化。而GO和HPO的任意标识符限制模型只能进行机械学习。
...
TL;DR: 该论文研究大型语言模型在参数知识与提示中冲突信息存在差异时的行为，特别关注代码生成领域中的知识冲突检测和激活引导方法。


<details>
  <summary>Details</summary>
Motivation: 扩展知识冲突研究到代码生成领域，探索LLMs如何处理参数知识与外部提示信息之间的冲突，并开发领域无关的框架来构建和解释这类冲突。

Method: 提出领域无关的框架来构建和解释知识冲突，开发针对代码冲突场景的新型评估方法和数据集，使用激活级引导技术来改善模型行为。

Result: 实验表明足够大的LLMs在其参数中编码了知识冲突的概念，能够以80.65%的准确率检测知识冲突；激活级引导相比随机基线可提升12.6%的引导成功率，但效果取决于模型大小、任务领域和引导方向的平衡。

Conclusion: LLMs能够检测知识冲突，激活引导技术可有效改善模型行为，但需要仔细平衡模型大小、任务领域和引导方向以获得最佳效果。
...
TL;DR: 本文对藏语AI研究现状进行全面调查，涵盖文本和语音数据资源、NLP任务、机器翻译、语音识别及大语言模型发展，旨在为低资源语言AI研究提供参考。
...
Conclusion: 本调查旨在为未来藏语AI研究提供基础参考，鼓励合作建立包容和可持续的低资源语言AI生态系统。
...
TL;DR: TSSS是一个高效的多跳检索增强生成框架，通过模板化推理和基于检索器的终止器来减少token使用并提高推理稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有的迭代提示方法在多跳检索增强生成中效率低下，存在重复生成可预测token序列和随机停止的问题，导致token使用过多和终止不稳定。

Method: TSSS采用模板化推理来缓存重复前缀并将子查询锚定到主问题，同时使用基于检索器的终止器在子查询重复时确定性地停止推理。

Result: 在HotpotQA、2WikiMultiHop和MuSiQue数据集上，TSSS在RAG-CoT方法中实现了最先进的准确性和有竞争力的效率。

Conclusion: TSSS通过分离结构化推理和终止控制，在效率受限场景（如设备端推理）中实现了高效且可靠的多跳检索增强生成。
...
TL;DR: 提出了evolveQA基准测试，专门用于评估LLM处理时间演化知识的能力，基于三个真实世界的时间戳语料库构建，发现相比静态知识问题性能下降高达31%。
...
Result: 通过对12个开源和闭源LLM在3种知识探测格式上的广泛评估，在evolveQA上相比静态知识问题性能下降高达31%。

Conclusion: evolveQA基准测试能有效评估LLM处理时间演化知识的能力，揭示了现有LLM在处理动态知识方面的显著局限性。
...
TL;DR: 提出了一种不依赖大语言模型的知识图谱问答系统，使用小型复述模型处理从知识图谱检索的实体关系边，通过两阶段流程实现问答。


<details>
  <summary>Details</summary>
Motivation: 开发不依赖检索增强生成和大语言模型的知识图谱问答系统，避免大模型的计算开销和依赖。
...
Conclusion: 该方法展示了不依赖大语言模型的知识图谱问答系统的可行性，在保持性能的同时降低了计算需求。
...
TL;DR: 本文提出了一种类型匹配的语言距离框架，解决了现有语言知识库中向量表示不适用于多样化语言数据结构以及缺乏统一聚合方法的问题。


<details>
  <summary>Details</summary>
Motivation: 现有语言知识库如URIEL+虽然提供了有价值的地理、谱系和类型学距离信息，但存在两个关键限制：一是其通用向量表示不适用于多样化的语言数据结构，二是缺乏将这些信号聚合为单一综合得分的系统方法。

Method: 提出了类型匹配语言距离框架，为每种距离类型设计了结构感知的表示方法：地理距离使用说话者加权的分布表示，谱系距离使用双曲嵌入，类型学距离使用潜变量模型，并将这些信号统一为稳健的任务无关复合距离。

Result: 在选择迁移语言时，所提出的表示方法和复合距离在广泛的NLP任务中持续提升了性能表现。

Conclusion: 该框架为多语言研究提供了一个更加系统和有效的工具包。
...
<details>
  <summary>Details</summary>
Motivation: 难度可控的阅读理解问题生成在教育领域作为自适应学习支持工具具有重要意义。传统神经问题生成方法存在两大局限：无法直接生成最常用的多选题类型，以及没有明确优化难度控制的准确性。

Method: 利用大语言模型，采用直接偏好优化技术进行训练，以提高难度控制的准确性，实现难度可控的多选题生成。

Result: 该方法能够直接生成难度可控的多选题，相比传统方法在难度控制精度方面有所提升。

Conclusion: 提出的方法有效解决了传统难度可控问题生成的两个主要局限，为教育领域的自适应学习提供了更实用的工具。
...
TL;DR: 本文提出了JointCQ框架，通过联合生成声明和查询来解决LLM幻觉检测中的上下文丢失和查询特异性不足问题，在多个开放域QA幻觉检测基准上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型存在幻觉问题，现有幻觉检测方法在声明提取和查询生成阶段存在局限性，如上下文丢失和查询特异性低，导致整个检测流程性能下降。

Method: 提出JointCQ框架，设计精心评估标准筛选合成训练数据，微调语言模型进行联合声明提取和查询生成，为下游搜索和验证提供可靠输入。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型预训练权重中的知识是静态的，无法跟上现实世界发展，阻碍了持续知识获取。需要同时实现知识适应（注入新知识）和知识保留（保留旧知识）两个目标。

Method: KORE自动将单个知识项转换为结构化知识，确保模型准确学习新知识；同时将先前知识存储在LMM线性层激活的协方差矩阵中，通过将原始权重投影到矩阵零空间来初始化适配器，定义最小化与先前知识干扰的微调方向。

Result: 在LLaVA-v1.5-7B、LLaVA-v1.5-13B和Qwen2.5-VL-7B等多种LMM上的广泛实验表明，KORE实现了优越的新知识注入性能，并有效减轻了灾难性遗忘。

Conclusion: KORE方法能够有效解决大型多模态模型的知识更新问题，在注入新知识的同时保持对旧知识的记忆，为持续学习提供了可行方案。
...
TL;DR: 本文提出了一个包含11个类别的幻觉综合分类法，并开发了HAD模型，将幻觉检测、跨度级识别和纠正集成到单一推理过程中。模型在约90K合成样本上训练，在多个基准测试中表现优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 随着自然语言生成模型特别是大语言模型的广泛应用，其输出可靠性和准确性引发担忧，其中幻觉问题（模型生成看似合理但错误的信息）尤为关键，因此需要有效的幻觉检测方法。

Method: 提出了HAD模型，将幻觉检测、跨度级识别和纠正集成到单一推理过程；构建了包含11个类别的幻觉综合分类法；使用约90K合成样本训练模型；创建了包含2,248个样本的HADTest测试集。

Result: 在领域内和领域外测试集上的评估显示，HAD模型普遍优于现有基线，在HaluEval、FactCHD和FaithBench基准测试中取得了最先进的结果，证明了其鲁棒性和通用性。

Conclusion: HAD模型通过集成检测、识别和纠正功能，在多个NLG任务中展现出卓越的幻觉检测性能，为解决大语言模型幻觉问题提供了有效解决方案。
...
TL;DR: 本文提出了一种基于超体积优化的强化学习策略（HVO），用于优化文本摘要中的多目标问题，在多个数据集上表现优于GRPO方法，7B模型性能接近GPT-4。
...
Result: 在多个代表性摘要数据集上的实验表明，HVO方法在总体得分上优于GRPO，在不同维度上表现更均衡，7B基础模型增强后性能与GPT-4相当且生成长度更短。

Conclusion: HVO方法能有效生成多个目标平衡的摘要，为基于LLM的多目标强化学习优化提供了新思路。
...
TL;DR: 该论文提出将推理能力集成到语音大语言模型中，用于端到端的槽填充任务。通过使用思维链框架将任务分解为多个推理步骤，创建推理数据集并应用监督微调策略，展示了引入推理步骤带来的性能提升。


<details>
  <summary>Details</summary>
Motivation: 受最近推理LLM发展的启发，研究者希望将推理能力集成到语音大语言模型中，以改进端到端槽填充任务的性能。

Method: 使用思维链框架将槽填充任务分解为多个推理步骤，创建推理数据集，并应用监督微调策略到语音LLM。区分常规和推理语音LLM，实验不同类型和规模的LLM作为文本基础模型。

Result: 引入推理步骤带来了性能改进，但主要为数学、逻辑和编码领域开发的推理文本LLM作为语音LLM的基础模型可能表现较差。混合语音LLM（基于混合文本基础LLM并微调以保留直接和推理两种操作模式）比仅使用单一操作模式微调的模型性能更好。

Conclusion: 将推理能力集成到语音LLM中可以提升槽填充任务的性能，但需要选择合适的文本基础模型，混合操作模式的语音LLM表现最佳。
...
TL;DR: 该研究探讨了在大型语言模型中注入标注者身份特征如何影响其对仇恨言论的敏感性，特别是关于标注者与目标群体身份相似或不同时的偏见问题。使用Gemini和GPT-4.1-mini模型，通过浅层和深层身份提示方法分析群体内和群体外身份对检测性能和公平性的影响。


<details>
  <summary>Details</summary>
Motivation: 将心理学中的群体身份理论与先进NLP技术结合，研究如何通过将社会人口属性融入LLMs来解决自动仇恨言论检测中的偏见问题。

Method: 使用Google的Gemini和OpenAI的GPT-4.1-mini模型，采用两种身份提示方法：浅层身份提示和基于检索增强生成(RAG)的深度上下文身份开发，以纳入更丰富的身份配置文件。
...
TL;DR: LOGICAL是一个基于微调GLiNER模型的本地可部署PII移除系统，在临床笔记中高效移除个人身份信息，性能优于大型语言模型，适合资源受限环境。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在临床笔记PII移除中存在高计算成本和数据隐私风险，特别是在资源受限环境中应用受限。

Method: 使用1515份临床文档，定义9个PII类别，微调GLiNER模型并在376个测试实例上评估，与多个基准系统比较。

Result: 微调GLiNER模型达到0.980的F1分数，显著优于Gemini-Pro-2.5（0.845），95%的文档完全正确清理，在标准笔记本电脑上高效运行。

Conclusion: 微调的专业Transformer模型如GLiNER为临床笔记PII移除提供了准确、计算高效且安全的解决方案，是资源密集型LLMs的实用替代方案。
...
TL;DR: 本文介绍了DnD Gesture++数据集，通过集成文本、音频和手势的多模态方法研究对话中的轮换预测，发现语义手势能持续提升性能。
...
Method: 扩展DnD Gesture语料库，添加2,663个语义手势标注，使用专家混合框架集成文本、音频和手势进行轮换预测建模。

Result: 实验表明，加入语义引导的手势相比基线模型能带来持续的性能提升。

Conclusion: 语义手势在多模态轮换预测中发挥补充作用，验证了手势作为重要对话线索的价值。
...
<details>
  <summary>Details</summary>
Motivation: 虽然现有模型在语音和文本理解方面表现良好，但在自然对话中理解谁在什么时候说了什么（说话者归属推理）方面仍然存在困难。

Method: 从四个开放语料库（CHiME-6、MELD、MultiDialog和AMI）构建，包含超过12,000个经过验证的实例，配有音频、转录文本和元数据。包含两个任务：说话者归属问答和通过话语匹配的说话者归属。

Result: 基线结果显示，模型能够捕捉说了什么，但经常无法识别谁说的，揭示了说话者感知对话理解的关键差距。
...
### [90] [AgenticMath: Enhancing LLM Reasoning via Agentic-based Math Data Generation](https://arxiv.org/abs/2510.19361)
*Xianyang Liu,Yilin Liu,Shuai Wang,Hao Cheng,Andrew Estornell,Yuzhi Zhao,Jiaheng Wei*

Main category: cs.CL

TL;DR: 提出AgenticMath方法，通过四阶段流程生成高质量数学问答对，显著提升LLM数学推理能力，仅需3-6万样本即可超越使用更大规模数据的基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前方法生成的数据质量低、答案错误、信息丰富度有限，需要开发更有效的数据生成方法来提升LLM的数学推理能力。

Method: 四阶段流程：1)种子问题筛选；2)多智能体问题重述；3)答案增强；4)问答对评估。使用思维链推理增强答案正确性，不依赖人工标注。

Result: 在3B-8B参数LLM上，仅使用3-6万数学样本进行微调，在多种数学推理基准测试中达到或超越了使用40万或230万样本的基线模型性能。

Conclusion: 针对性的高质量数据生成比大规模低质量数据更有效地提升LLM数学推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 长上下文推理对大型语言模型至关重要，但现有强化学习方法主要针对短上下文，缺乏针对长上下文的高级推理模式探索，且高难度强化学习数据稀缺。

Method: 提出LoongRL方法，核心是KeyChain合成方法：通过插入UUID链将短多跳问答转换为高难度长上下文任务，模型需要追踪正确链、识别真实问题、检索相关事实并进行推理。

Result: 在Qwen2.5-7B和14B上，LoongRL显著提升长上下文多跳问答准确率，分别获得+23.5%和+21.1%的绝对增益。LoongRL-14B达到74.2分，与更大前沿模型性能相当，并通过所有128K针在干草堆压力测试。

Conclusion: LoongRL诱导出计划-检索-推理-复查的推理模式，能够泛化到远超训练长度的任务，同时保持短上下文推理能力，为长上下文推理提供了有效的强化学习解决方案。
...
TL;DR: MLEB是一个大规模、多样化的开源法律信息检索基准，包含10个专家标注的数据集，涵盖多个司法管辖区、文档类型和任务类型，其中7个是新构建的数据集。


<details>
  <summary>Details</summary>
Motivation: 填补开源法律信息检索领域在司法管辖区和领域方面的空白，为法律信息检索提供更全面、多样化的评估基准。

Method: 构建包含10个专家标注数据集的基准，涵盖美国、英国、欧盟、澳大利亚、爱尔兰和新加坡等多个司法管辖区，文档类型包括案例、立法、监管指南、合同和文献，任务类型包括搜索、零样本分类和问答。

Result: 创建了MLEB基准，包含7个新构建的数据集，填补了现有开源法律信息检索资源的空白，并公开了代码、结果和数据以支持可重复评估。

Conclusion: MLEB是目前最大、最多样化、最全面的开源法律信息检索基准，为法律AI研究提供了重要的评估资源。
...
TL;DR: MoE-Prism通过模型-系统协同设计将刚性MoE模型转换为弹性服务，提供细粒度控制，在保持质量的同时显著提高资源利用效率。
...
Method: 分为两个阶段：离线重构引擎将单体专家分解为细粒度'子专家'，使用元启发式分组算法保持功能局部性；在线调度引擎实现QoS感知调度，包括云部署吞吐量最大化和内存受限设备的延迟优化卸载。

Result: 在三种不同MoE模型上的评估显示，MoE-Prism提供超过基线4倍的稳定操作点，在严格延迟预算下吞吐量提升19.9%，在有限资源下延迟降低10.36%。
...
TL;DR: 提出一种使用目标语句子嵌入替代gloss注释的手语翻译新方法，无需手动标注，在德语和英语手语数据集上显著优于其他无gloss方法


<details>
  <summary>Details</summary>
Motivation: 现有手语翻译系统依赖gloss注释，但gloss标注数据稀缺且不同数据集间差异大，需要一种无需手动gloss标注的监督方法

Method: 使用目标语句子嵌入作为训练时的监督信号，替代传统gloss角色，支持单语和多语言句子嵌入及翻译系统

Result: 在德语(PHOENIX-2014T)和美国手语(How2Sign)数据集上显著优于其他无gloss方法，缩小了无gloss与依赖gloss系统之间的差距

Conclusion: 该方法为缺乏gloss标注的手语翻译提供了新的最先进解决方案，无需额外SLT数据集预训练
...
<details>
  <summary>Details</summary>
Motivation: 传统手语翻译通常使用单一语言的文本进行训练，这限制了可扩展性和跨语言泛化能力。现有方法虽然用基于文本的句子嵌入替换了gloss监督，但仍受限于特定语言和模态。

Method: 采用在多种语言的文本和语音上训练的语言无关多模态嵌入来监督SLT；提出耦合增强方法，结合多语言目标增强（翻译成多种语言）和视频级扰动，提高模型鲁棒性。

Result: 实验显示相比仅使用文本句子嵌入监督，该方法在BLEURT指标上获得了一致的提升，在低资源设置下改进更为显著。
...
<details>
  <summary>Details</summary>
Motivation: 实体提及检测是信息提取的基础但性能瓶颈，研究旨在探索从早期LLM层中高效恢复结构化实体表示的可能性。

Method: 开发ToMMeR轻量级模型，通过探测早期LLM层进行实体提及检测，使用LLM作为判断器评估预测质量，并进行跨模型架构分析。

Result: 在13个NER基准测试中达到93%零样本召回率，90%以上精确度；跨模型分析显示不同架构（1400万-150亿参数）在提及边界上高度一致（DICE >75%）；添加跨度分类头后达到接近SOTA的NER性能（80-87% F1）。
...
TL;DR: DFKI-MLT团队在WMT-SLT 2022手语翻译任务中提出了一种端到端架构，从瑞士德语手语视频直接翻译为德语文本，但测试集性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 传统手语翻译方法使用通用seq2seq架构，但往往无法充分利用时间特征，需要改进时空特征表示和翻译的统一建模。

Method: 开发了学习时空特征表示和翻译的单一模型，实现真正的端到端架构，预期能更好地泛化到新数据集。

Result: 最佳系统在开发集上达到5±1 BLEU分，但在测试集上性能大幅下降至0.11±0.06 BLEU分。

Conclusion: 虽然端到端架构在开发集上表现良好，但在测试集上泛化能力不足，表明需要进一步改进模型的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 弥补性能导向基准测试与认知启发模型评估之间的差距，提供更符合人类语言习得模式的评估工具。

Method: 从280万自然学习者句子构建136,867个控制三元组（修正版、学习者版、人工版），采用选择性容忍度测试范式。

Result: 实验表明选择性容忍度是区别于标准语法性的独立能力，模型性能按训练范式强烈聚类。

Conclusion: BLiSS验证了作为测量不同训练目标如何影响模型与人类语言习得系统模式对齐的稳健工具。
...
TL;DR: MINED是一个评估大型多模态模型时间感知能力的基准，包含6个关键维度和11个任务，涵盖2104个时间敏感知识样本。评估显示Gemini-2.5-Pro表现最佳，而大多数开源LMM缺乏时间理解能力。知识编辑方法可以有效更新LMM中的时间敏感知识。
...
Method: 从维基百科构建MINED基准，由两名专业标注者创建，包含2104个时间敏感知识样本，涵盖6种知识类型。评估15个广泛使用的LMMs，并研究通过知识编辑方法更新时间敏感知识的可行性。

Result: Gemini-2.5-Pro获得最高平均CEM分数63.07，大多数开源LMMs缺乏时间理解能力。LMMs在组织知识上表现最好，在体育知识上表现最弱。知识编辑方法在单次编辑场景中能有效更新LMMs的知识。

Conclusion: MINED基准揭示了LMMs在时间敏感知识理解方面的局限性，知识编辑是更新LMMs时间敏感知识的可行方法。
...
TL;DR: MBR解码在语音转文本任务（ASR和ST）中表现优于传统的束搜索方法，在大多数实验设置中提高了准确性。


<details>
  <summary>Details</summary>
Motivation: 考虑到MBR解码在文本到文本生成任务中表现出色，研究人员期望它在语音到文本任务中同样有效，以替代当前主流的束搜索方法。

Method: 使用Whisper及其衍生模型在英语和日语上评估MBR解码在ASR和ST任务中的表现。

Result: 在大多数实验设置中，MBR解码的准确性超过了束搜索，显示出其在离线ASR和ST任务中的优势。

Conclusion: MBR解码是高精度离线语音转文本任务的有前途方法。
...
TL;DR: 本文研究了成员推理攻击（MIA）和机器生成文本检测之间的可迁移性，证明了两个任务的最优性能度量相同，并通过大规模实验验证了跨任务性能的强相关性。


<details>
  <summary>Details</summary>
Motivation: 尽管MIA和机器生成文本检测方法都基于语言模型的概率分布信号，但它们被独立研究，可能忽略了另一任务中更强的方法和有价值的见解。

Method: 从理论上证明两个任务的最优性能度量相同，并通过大规模实验（包括7种MIA方法和5种机器文本检测器，覆盖13个领域和10个生成器）验证跨任务性能的相关性。

Result: 发现跨任务性能存在非常强的秩相关性（rho > 0.6），特别是Binoculars方法在MIA基准测试中也达到了最先进性能。

Conclusion: 研究结果强调了两个研究社区之间需要更大的跨任务意识和合作，并提出了MINT统一评估套件来促进跨任务发展和公平评估。
...
TL;DR: 提出Lookahead路由框架，通过预测潜在输出来指导模型选择，在七大基准测试中平均性能提升7.7%


<details>
  <summary>Details</summary>
Motivation: 现有LLM路由器仅基于输入查询进行分类，忽略了潜在输出中的有价值信息，无法捕捉隐含意图和上下文细微差别，导致复杂或模糊查询的路由决策不理想

Method: 提出Lookahead框架，通过预测模型输出的潜在表示来"预见"可能的输出，使用这些预测指导模型选择。实现基于因果和掩码语言模型的两种方法

Result: 在七个公共基准测试（指令遵循、数学推理和代码生成）上，Lookahead始终优于现有路由基线，相比最先进方法平均性能提升7.7%

Conclusion: Lookahead框架通过预见潜在输出实现了更明智的路由决策，无需完整推理即可利用输出信息，显著提升了多模型系统的路由效率
...
TL;DR: 该论文提出了一个统一的分类法来评估语音基础模型，定义了三个正交轴：评估方面、模型能力和任务要求，用于系统地对齐模型与合适的评估方法。
...
Method: 提出一个包含三个正交轴的分类法：评估方面（测量的内容）、模型能力（完成任务所需的能力）和任务要求（执行任务所需的协议要求），并对现有评估和基准进行分类。

Result: 分类法能够系统地对齐模型与合适的评估方法，并揭示了现有评估在韵律、交互和推理等方面的系统性空白。

Conclusion: 这项工作为选择、解释和扩展语音模型评估提供了概念基础和实践指南，有助于指导未来基准设计。
...
TL;DR: 本文通过系统实验研究发现，多语言模型在特定语言微调时出现灾难性遗忘的主要决定因素是模型与数据规模的相对比例，而非模型架构。指令跟随能力比架构对保留多语言知识更重要，参数高效微调在缓解遗忘方面并无明显优势，而跨语言对齐可以减轻遗忘并促进对未见目标语言的正面迁移。


<details>
  <summary>Details</summary>
Motivation: 多语言基础模型在特定语言微调时经常出现灾难性遗忘，降低在未见语言上的性能。虽然这种现象被广泛记录，但文献对何时发生遗忘的结果呈现碎片化。为解决这种模糊性，需要进行系统实证研究。

Method: 使用机器翻译作为测试平台，通过在不同模型架构、数据规模和微调方法上的受控实验，识别触发多语言微调中灾难性遗忘的条件。

Result: 发现模型与数据规模的相对比例是遗忘的主要决定因素；模型的指令跟随能力对保留多语言知识比架构更关键；参数高效微调在缓解遗忘方面没有明显优势；跨语言对齐可以减轻遗忘并促进对未见目标语言的正面迁移。

Conclusion: 多语言微调中的灾难性遗忘主要由模型与数据规模的相对比例决定，指令跟随能力比模型架构更重要，跨语言对齐是缓解遗忘的有效策略。
...
TL;DR: 本文提出从混合语言历史文档中提取拉丁语片段的新任务，评估大型基础模型在724页标注数据集上的性能，证明当代模型可实现可靠的拉丁语检测。


<details>
  <summary>Details</summary>
Motivation: 解决从混合语言历史文档中提取拉丁语片段的挑战，填补该领域的研究空白。
...
Method: 通过250名不同人口统计学个体的问卷调查，与社会科学专家密切合作开发PBBQ数据集，包含16个文化类别，并对多个开源、闭源和波斯语微调模型进行基准测试。

Result: 研究发现当前LLMs在波斯文化中表现出显著的社会偏见，模型输出与人类响应对比显示LLMs经常复制人类偏见模式。

Conclusion: PBBQ数据集为波斯语语言模型的偏见评估和缓解提供了基础，揭示了学习表示与文化刻板印象之间的复杂相互作用。
...
Method: 开发了可扩展、可解释的众包流程来收集跨语言新闻相似性数据，构建了CrossNews-UA数据集，包含乌克兰语与相关语言（波兰语、俄语、英语）的新闻对，使用4W标准（Who, What, Where, When）进行语义相似性标注和详细解释。测试了从传统词袋模型、Transformer架构到大型语言模型（LLMs）的各种模型。

Result: 研究结果突显了多语言新闻分析中的挑战，并提供了关于模型性能的见解。

Conclusion: 提出的众包流程和CrossNews-UA数据集为跨语言新闻分析提供了可扩展的解决方案，模型测试揭示了多语言新闻分析的难点和性能差异。
...
TL;DR: 本文提出了一种基于字体风格的攻击方法SAD，利用人类与NLP模型对风格化文本的感知差异，在情感分类和机器翻译等任务中实现有效攻击。


<details>
  <summary>Details</summary>
Motivation: 随着社交媒体的发展，用户使用风格化字体和字体表情来表达个性，这些视觉上吸引人的文本对人类可读，但对NLP模型构成隐藏漏洞，因为模型将这些字符处理为不同标记，导致干扰。

Method: 提出Style Attack Disguise (SAD)攻击方法，设计轻量版和强力版两种规模：轻量版注重查询效率，强力版追求更优攻击性能。

Result: 在情感分类和机器翻译任务上的实验表明，SAD在传统模型、大语言模型和商业服务中都具有强大的攻击性能，并且在文本到图像和文本到语音等多模态任务中也显示出潜在威胁。

Conclusion: 风格化文本在人类与NLP模型之间存在感知差距，SAD攻击方法有效利用这一差距，对多种NLP任务构成安全威胁。
...
TL;DR: LlavaCode框架通过将代码压缩为紧凑的语义表示来提升代码补全性能，在减少检索上下文的同时提高生成质量，显著降低延迟。
...
Conclusion: LlavaCode框架通过代码压缩有效解决了检索增强生成在代码补全中的延迟问题，在保持生成质量的同时显著提升推理速度。
...
TL;DR: 本文比较了微调和提示工程在情感检测中的效果，发现微调预训练模型在情感识别中达到70%以上指标，而LLMs需要结构化提示工程和情感分组来提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer模型在情感识别方面取得显著进展，但在开放文本中自动情感分析仍面临上下文模糊性、语言变异性和复杂情感表达解释困难等挑战，这使得通用模型的直接应用变得困难。
...
Conclusion: 这些进展改善了情感分析、人机交互以及跨领域用户行为的理解，表明结构化提示和情感分组是提升LLMs情感检测性能的关键因素。
...
TL;DR: 本文提出DiffAdapt框架，通过分析推理轨迹的熵模式来识别LLM的"过度思考"现象，并基于问题难度自适应选择推理策略，在不微调基础模型的情况下显著减少计算开销。
...
Method: 基于推理轨迹熵分析发现U型熵模式，提出DiffAdapt轻量框架，通过小探针分类LLM隐藏状态来为每个问题选择Easy/Normal/Hard推理策略（包含固定提示、温度和最大令牌长度）。

Result: 在5个模型和8个基准测试上的评估显示，该方法在保持或提高准确率的同时，将令牌使用量减少高达22.4%。

Conclusion: DiffAdapt为计算高效的推理提供了一条实用路径，通过难度自适应策略显著提升LLM推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 在干扰环境中部署大模型时，需要同时考虑语义理解、隐私保护和可预测延迟的平衡，传统方法难以在边缘设备上满足这些要求。

Method: 采用SenseFusion轻量编码器对齐传感器嵌入与语言，Edge-RAG本地混合检索层，PromptRouter成本感知策略选择生成路径，以及Secure Execution确保数据最小化。

Result: 在家庭、办公室和诊所部署中，实现亚秒级端到端延迟，减少跨层令牌和带宽成本，通过本地检索提高事实一致性，降低矛盾率。

Conclusion: 边缘优先设计能够在干扰环境中有效平衡语义理解、隐私保护和延迟要求，为大模型部署提供可行方案。
...
TL;DR: 该论文研究了大型语言模型是否具备动机警觉能力，即能否像人类一样考虑信息来源的动机和意图来评估内容可信度。研究发现LLMs在受控实验中表现出类似人类的动机敏感性，但在真实广告场景中表现不佳，不过简单的干预可以显著提升其表现。


<details>
  <summary>Details</summary>
Motivation: 人类交流具有动机性，人们会根据信息来源的意图和动机来决定信任哪些陈述。为了让LLMs在现实世界中有效工作，它们也必须具备批判性评估内容的能力，考虑信息来源的动机，比如评估销售宣传中声明的可信度。

Method: 首先采用认知科学中的受控实验验证LLMs行为是否与理性学习模型一致，然后扩展到更自然的在线赞助广告场景进行评估，并测试简单的干预措施是否能提升LLMs的动机警觉能力。

Result: 在受控实验中，LLMs能够像人类一样成功折扣来自偏见来源的信息；但在在线广告场景中，LLMs的推理与理性模型预测不太一致，部分原因是额外信息分散了注意力。简单的干预措施显著提高了LLMs与理性模型的一致性。
...
TL;DR: 本研究通过实验分析提示对语言模型内部表示质量的影响，发现提示相关性并不总是与表示质量正相关，挑战了相关提示必然带来更好表示的假设。
...
Method: 进行一系列探测实验，分析零样本分类中不同提示模板组合的嵌入表示，评估提示对表示质量的影响。

Result: 研究发现提示确实影响表示质量，但这些变化与提示对目标任务的相关性并不一致相关，更相关的提示不一定产生更好的表示。

Conclusion: 研究挑战了提示相关性必然提升表示质量的假设，揭示了提示机制中更复杂的动态关系，需要进一步分析影响这种意外行为的潜在因素。
...
<details>
  <summary>Details</summary>
Motivation: 欧盟虽然提供了开放的法律信息访问，但这些资源对普通公民来说仍然难以理解和探索，存在法律信息可及性的挑战。

Method: 构建了包含880个对话轮次的EUDial数据集，并提出LexGuide框架，利用检索增强生成和分层主题组织来结构化对话进展。
...
TL;DR: 提出了SmartSwitch推理框架，通过检测和干预来解决大语言模型在长链思维推理中的"欠思考"问题，提升推理深度和性能。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在长链思维推理中存在的"欠思考"问题，即模型频繁切换思路而缺乏充分探索，这限制了性能和token效率。

Method: 提出SmartSwitch推理框架，包含感知模块和干预模块。感知模块检测思路切换点并使用现成的过程奖励模型评估前一个思路的潜力；干预模块在发现高潜力思路被过早放弃时，回溯到切换点前并插入"深化提示"来鼓励进一步探索。
...
TL;DR: AdaSPEC通过选择性令牌过滤改进知识蒸馏过程，提高推测解码中的令牌接受率，在多种任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法旨在最小化草稿模型和目标模型在所有令牌上的KL散度，这与推测解码最大化令牌接受率的目标不一致，导致草稿模型因容量限制难以充分吸收目标模型知识。

Method: 提出AdaSPEC方法，在知识蒸馏过程中引入选择性令牌过滤，使用参考模型识别并过滤难以拟合的令牌，使草稿模型在简单令牌上更好地与目标模型对齐。

Result: 在算术推理、指令跟随、编码和摘要等多样化任务上，使用31M/1.4B和350M/2.7B参数配置，AdaSPEC始终优于最先进的DistillSpec方法，在所有任务中实现更高的接受率（最高提升15%）。

Conclusion: AdaSPEC通过选择性令牌过滤改进了知识蒸馏过程，在不影响生成质量的前提下提高了令牌接受率，为推测解码提供了更有效的草稿模型训练方法。
...
TL;DR: 该论文提出了一种模型合并方法作为代码混合NLP的实用适应策略，通过在多语言基础模型上进行持续预训练、模型合并和下游任务微调，在英语-印地语和英语-西班牙语的句子分类任务中取得了优于传统方法的性能。
...
Method: 从多语言基础模型出发：(i)在未标记的代码混合文本上进行持续预训练获得适应检查点，(ii)将检查点与基础模型合并，(iii)在下游任务数据上进行微调。使用XLM-R和Llama-3.2-1B模型在英语-印地语和英语-西班牙语的句子分类任务上进行评估。

Result: 合并模型始终优于完全微调和CPT->FT方法，F1分数比完全微调高2-5分，比CPT->FT高约1-2分。在跨语言对迁移测试中，合并检查点的迁移能力更强，TV/TIES变体达到0.65-0.68 F1，而完全微调为0.61-0.63。

Conclusion: 模型合并能更有效地利用未标记数据，代码混合知识是低资源语言对更可靠的基板。论文提供了针对不同数据情况的适应方案，并讨论了在更广泛任务和更大模型中的扩展考虑。
...
TL;DR: ToolDreamer是一个框架，通过使用LLM生成假设性的工具描述来改进工具检索，使检索器能更自然地对齐查询和工具，从而提升稀疏和密集检索器的性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索模型基于用户查询与工具描述的相似性进行排名，但用户请求往往与工具描述语言不匹配，导致检索效果不佳。需要一种方法使检索器能更好地理解查询意图并检索相关工具。

Method: 提出ToolDreamer框架，使用LLM生成假设性的合成工具描述，这些描述是LLM认为可能对查询有用的工具描述。通过这种方式在工具描述的语言空间内实现更自然的查询-工具对齐。

Result: 在ToolRet数据集上应用ToolDreamer，结果显示该方法提高了稀疏和密集检索器的性能，无论是否进行训练，展示了其灵活性。

Conclusion: ToolDreamer框架旨在将部分推理负担转移到检索器上，使LLM能够有效处理大量工具集合而不会超出其上下文窗口限制。
...
TL;DR: 本文提出了一种轻量级的提示空间优化框架，通过系统性地将翻译提示转换为更自然、文化适应和难度增强的形式，显著提升了多语言大语言模型的性能。
...
Method: 引入轻量级提示空间优化框架，对翻译提示进行自然性、文化适应性和难度增强的系统性转换，在12种语言上应用这些转换。

Result: 在相同数据条件下，相比仅翻译的基线方法，在Global-MMLU准确率上提升4.7%，Flores XCometXL提升2.4%，mArenaHard偏好胜率提升35.3%。

Conclusion: 提示空间优化是构建更鲁棒、文化接地和全球能力更强的多语言大语言模型的简单而强大的范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于可验证奖励的强化学习方法面临"学习悬崖"问题：当问题远超模型当前能力时，模型持续失败产生零奖励信号，导致优势计算归零，学习梯度消失，阻碍进步。
...
TL;DR: Hubble是一个用于研究LLM记忆机制的开源模型套件，包含标准模型和扰动模型，通过控制文本插入来模拟记忆风险，发现记忆风险与敏感数据在训练语料中的相对频率相关。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型的记忆机制和风险，特别是敏感数据如何被模型记忆以及如何缓解这种风险。

Method: 开发标准模型和扰动模型，在训练过程中控制插入文本（如书籍段落、传记、测试集），分析不同参数规模和训练token数量下的记忆行为。

Result: 发现记忆风险由敏感数据在训练语料中的相对频率决定，敏感数据在较小语料中更容易被记忆，且没有持续暴露的敏感数据会被遗忘。

Conclusion: 提出了缓解记忆风险的最佳实践：通过增加训练语料规模来稀释敏感数据，以及让敏感数据在训练早期出现；Hubble为记忆研究、成员推断和机器遗忘提供了理想的测试平台。
...
### [124] [CityAQVis: Integrated ML-Visualization Sandbox Tool for Pollutant Estimation in Urban Regions Using Multi-Source Data (Software Article)](https://arxiv.org/abs/2510.18878)
*Brij Bridhin Desai,Yukta Arvind,Aswathi Mundayatt,Jaya Sreevalsan-Nair*

Main category: cs.HC

TL;DR: CityAQVis是一个交互式机器学习工具，用于预测和可视化城市地面污染物浓度，整合多源数据并支持模型比较和空间分布分析。


<details>
  <summary>Details</summary>
Motivation: 现有空气质量可视化工具缺乏预测能力，需要开发能够整合多样化数据集并有效沟通复杂时空污染模式的交互式工具。

Method: 开发CityAQVis交互式ML沙盒工具，整合卫星观测、气象参数、人口密度、高程和夜间灯光等多源数据，通过图形用户界面支持用户构建和比较预测模型。

Result: 工具通过预测大都市区域二氧化氮浓度的案例研究验证了适应性，能够进行不同城市情景下污染物浓度空间分布的对比可视化。
...
### [125] [Towards Better Health Conversations: The Benefits of Context-seeking](https://arxiv.org/abs/2510.18880)
...
TL;DR: 该研究通过4项混合方法研究发现，在健康咨询场景中，主动寻求上下文信息的AI（Wayfinding AI）比基线AI更受用户欢迎，被认为更有帮助、相关性和针对性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在健康信息咨询中可能存在不准确、偏见或误导的风险，需要研究如何设计更有效的对话AI来帮助用户导航健康话题。

Method: 采用4项混合方法研究（共163名参与者），包括定性研究分析用户与LLMs的交互模式，并开发了主动寻求上下文的"Wayfinding AI"，通过随机盲法研究比较其与基线AI的效果。

Result: 定性研究发现上下文寻求在对话AI中很重要，即使这意味着延迟回答。在随机研究中，参与者认为Wayfinding AI比基线AI更有帮助、更相关、更贴合他们的关切。

Conclusion: 主动寻求上下文对对话动态有强烈影响，为帮助导航健康话题的对话AI提供了设计模式。
...
### [126] [EasyVitessce: auto-magically adding interactivity to Scverse single-cell and spatial biology plots](https://arxiv.org/abs/2510.19532)
*Selena Luo,Mark S. Keller,Tabassum Kakar,Lisa Choy,Nils Gehlenborg*

Main category: cs.HC

TL;DR: EasyVitessce是一个Python包，通过添加一行代码将静态的Scanpy和SpatialData图转换为交互式可视化。


<details>
  <summary>Details</summary>
Motivation: 简化交互式可视化的创建过程，使研究人员能够轻松地将现有的静态图转换为交互式版本，而无需深入了解Vitessce的技术细节。

Method: 使用Vitessce内部渲染交互式图，并抽象化Vitessce配置的技术细节。通过单行Python代码实现转换。

Result: 生成的交互式图可以在计算笔记本环境中查看，也可以导出配置用于其他上下文（如Web应用程序），增强了Scverse Python绘图API的实用性。

Conclusion: EasyVitessce通过简化交互式可视化的创建过程，提高了生物信息学数据分析的可访问性和效率。
...
### [127] [Detecting AI-Assisted Cheating in Online Exams through Behavior Analytics](https://arxiv.org/abs/2510.18881)
*Gökhan Akçapınar*

Main category: cs.HC

TL;DR: 本研究分析在线考试中的AI辅助作弊行为，通过聚类方法识别可疑行为模式，发现约33%学生表现出不同程度的可疑行为，且这些学生平均分数比无可疑行为学生高30-40分。


<details>
  <summary>Details</summary>
Motivation: AI辅助作弊已成为在线考试的重大威胁，高级浏览器扩展使大语言模型能在几秒内回答考试问题，危及考试安全性。

Method: 分析52名学生在监考面对面在线考试中的行为，定义可疑行为包括：选择问题区域文本、右键点击、失去考试页面焦点，使用k-Means聚类分析这些行为频率。

Result: 学生被分为6个聚类，其中4个聚类（约33%样本）表现出不同程度的可疑行为。有可疑行为的学生平均分数比无行为学生高30-40分。
...
### [128] [SocializeChat: A GPT-Based AAC Tool Grounded in Personal Memories to Support Social Communication](https://arxiv.org/abs/2510.19017)
*Wei Xiang,Yunkai Xu,Yuyang Fang,Zhuyu Teng,Zhaoqu Jiang,Beijia Hu,Jinguo Yang*

Main category: cs.HC

TL;DR: SocializeChat是一个为有语言障碍的老年人设计的AAC工具，通过利用用户的个人记忆记录生成句子建议，结合话题偏好和人际亲密度，为不同的社交情境和对话伙伴定制建议。


<details>
  <summary>Details</summary>
Motivation: 有语言障碍的老年人使用传统的AAC工具时，主要只能满足基本需求，难以进行有意义的社交沟通。有效的对话往往依赖于个人记忆，但这些记忆难以提取和重复利用。

Method: 开发SocializeChat系统，通过整合用户的个人记忆记录、话题偏好和人际亲密度，生成适合不同社交情境和对话伙伴的句子建议。系统不仅利用过去的经验支持互动，还将对话视为创造新记忆的机会。

Result: 用户研究表明，SocializeChat有潜力增强AAC支持的社交互动的包容性和相关性。

Conclusion: SocializeChat在记忆和沟通之间建立了动态循环，通过重用过去的经验和创造新的记忆，提升了有语言障碍老年人的社交沟通质量。
...
### [129] [Examining the Impact of Label Detail and Content Stakes on User Perceptions of AI-Generated Images on Social Media](https://arxiv.org/abs/2510.19024)
*Jingruo Chen,TungYen Wang,Marie Williams,Natalia Jordan,Mingyi Shao,Linda Zhang,Susan R. Fussell*

Main category: cs.HC

TL;DR: 本研究通过实验发现，AI生成图像标签的详细程度（基础、中等、最大）能提升用户对标签透明度的感知，但不影响用户参与度；而内容重要性（高vs低）显著影响用户参与度和信任度，用户在低重要性内容中表现出更高的参与度和信任。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成图像在社交媒体上的普及，引发了关于信任和真实性的担忧，需要研究不同标签详细程度和内容重要性如何影响用户对AI生成图像的参与和感知。

Method: 采用被试内实验设计，105名参与者参与，研究不同标签详细程度（基础、中等、最大）和内容重要性（高vs低）对用户参与度和感知的影响。

Result: 增加标签详细程度能增强用户对标签透明度的感知，但不影响用户参与度；内容重要性显著影响用户参与度和信任度，用户在低重要性内容中表现出更高的参与度和信任。

Conclusion: 社交媒体平台可以采用详细标签来提高透明度而不损害用户参与度，为AI生成内容提供有效的标签策略建议。
...
### [130] [CLiVR: Conversational Learning System in Virtual Reality with AI-Powered Patients](https://arxiv.org/abs/2510.19031)
*Akilan Amithasagaran,Sagnik Dakshit,Bhavani Suryadevara,Lindsey Stockton*

Main category: cs.HC

TL;DR: CLiVR是一个结合大型语言模型、语音处理和3D虚拟形象的VR对话学习系统，用于模拟医患互动，作为标准化患者培训的可扩展补充方案。


<details>
  <summary>Details</summary>
Motivation: 传统医学教育使用标准化患者和高仿真人体模型需要大量资源，限制了可及性和可扩展性，需要更经济高效的替代方案。

Method: 在Unity中开发并部署在Meta Quest 3平台，整合LLM、语音处理和3D虚拟形象，从症状数据库动态生成模拟场景，并通过情感分析提供沟通反馈。

Result: 医学教师专家研究(n=13)显示系统具有强用户接受度、高教育潜力信心，并获得有价值的改进反馈。

Conclusion: CLiVR提供了一个可扩展、沉浸式的标准化患者培训补充方案，在医学教育中具有良好应用前景。
...
### [131] ["Over-the-Hood" AI Inclusivity Bugs and How 3 AI Product Teams Found and Fixed Them](https://arxiv.org/abs/2510.19033)
*Andrew Anderson,Fatima A. Moussaoui,Jimena Noa Guevara,Md Montaser Hamid,Margaret Burnett*

Main category: cs.HC

TL;DR: 本文研究了AI产品中的"表面包容性偏见"，即用户界面中存在的偏见，这些偏见会不成比例地排除具有特定问题解决方法的用户。通过实地研究，识别了6种AI包容性缺陷，提出了GenderMag-for-AI方法来检测和修复这些缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注AI的"底层"偏见（如算法、训练数据等），但对"表面"包容性偏见（用户界面中的偏见）研究不足。这些偏见会排除具有特定问题解决方法的用户，需要系统性的方法来检测和修复。

Method: 与3个AI产品团队进行实地研究，使用现有的包容性设计方法（GenderMag）来识别AI产品中的包容性缺陷，并开发了专门针对AI的变体方法GenderMag-for-AI。

Result: 识别出6种AI包容性缺陷，共出现83次，修复了47个缺陷实例。新开发的GenderMag-for-AI方法在检测特定类型的AI包容性缺陷方面特别有效。

Conclusion: AI产品中存在显著的表面包容性偏见，需要专门的工具和方法来检测和修复。GenderMag-for-AI方法为AI产品团队提供了一种有效的包容性设计工具，能够帮助识别和解决用户界面中的偏见问题。
...
### [132] [LLMartini: Seamless and Interactive Leveraging of Multiple LLMs through Comparison and Composition](https://arxiv.org/abs/2510.19252)
*Yingtian Shi,Jinda Yang,Yuhan Wang,Yiwen Yin,Haoyu Li,Kunyu Gao,Chun Yu*

Main category: cs.HC

TL;DR: LLMartini是一个交互式系统，支持无缝比较、选择和直观的跨模型组合工具，通过分解响应为语义对齐的段、自动合并共识内容并突出模型差异，显著提高了多LLM交互的效率。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型(LLM)的多样性增加，用户需要比较和组合不同模型的输出来获得更高质量或更全面的响应，但切换界面和手动集成输出效率低下，带来高认知负担和碎片化工作流程。

Method: 系统基于任务特定标准将响应分解为语义对齐的段，自动合并共识内容，通过颜色编码突出模型差异，同时保留独特贡献。
...
### [133] [Learning To Defer To A Population With Limited Demonstrations](https://arxiv.org/abs/2510.19351)
*Nilesh Ramgolam,Gustavo Carneiro,Hsiang-Ting,Chen*

Main category: cs.HC

TL;DR: 提出了一种上下文感知的半监督框架，使用元学习从少量演示中生成专家特定嵌入，解决学习延迟(L2D)系统的数据稀缺问题。
...
Method: 使用元学习生成专家特定嵌入，采用双重用途机制：首先生成大量伪标签用于训练，然后在测试时实现对新专家的即时适应。

Result: 在三个不同数据集上的实验结果表明，使用合成标签训练的模型快速接近oracle级别性能，验证了方法的数据效率。
...
### [134] [Unmanned Aerial Vehicles Control in a Digital Twin: Exploring the Effect of Different Points of View on User Experience in Virtual Reality](https://arxiv.org/abs/2510.19604)
...
TL;DR: 本研究探讨了在VR环境中使用数字孪生技术时，不同虚拟视角对无人机操控体验和性能的影响。通过比较四种视角（基线视图、第一人称视角、追逐视角、第三人称视角），发现第一人称视角带来更高的心理负荷和轨迹偏差，但控制更平滑；而第三人称视角最受用户青睐。


<details>
  <summary>Details</summary>
Motivation: 无人机操控是一项认知要求高的任务，事故常源于情境意识不足、训练不充分和用户体验差。通过数字孪生技术提供更直观的视觉反馈，有望提升飞行员意识和整体体验质量。

Method: 开发VR应用程序，让参与者在数字孪生环境中控制物理DJI Mini 4 Pro无人机，测试四种不同相机视角：基线视图（静态外部）、第一人称视角、追逐视角和第三人称视角。19名参与者完成基于圆环的障碍课程，收集客观飞行数据和主观用户体验评估。

Result: 定量分析显示，第一人称视角与显著更高的心理需求和努力、更大的轨迹偏差相关，但控制输入更平滑。偏好数据显示第三人称视角最受青睐，而第一人称视角引发两极分化反应。

Conclusion: 不同虚拟视角对无人机操控体验和性能有显著影响，第三人称视角在用户体验和偏好方面表现最佳，而第一人称视角虽然控制更平滑但带来更高认知负荷。
...
### [135] [Sentiment Analysis of Social Media Data for Predicting Consumer Behavior Trends Using Machine Learning](https://arxiv.org/abs/2510.19656)
*S M Rakib Ul Karim,Rownak Ara Rasul,Tunazzina Sultana*

Main category: cs.HC

TL;DR: 该研究使用机器学习方法对Twitter数据进行情感分析，预测消费者趋势。使用Sentiment140数据集，以"汽车"为例分析消费者偏好变化。BERT模型表现最佳（准确率83.48%），LSTM和BERT能有效捕捉语言和上下文模式。


<details>
  <summary>Details</summary>
Motivation: 在技术快速发展的时代，Twitter等社交媒体平台已成为收集消费者洞察、捕捉多样化意见和理解公众态度的重要工具。

Method: 使用结构化工作流程清洗和准备数据，应用SVM、朴素贝叶斯、LSTM和BERT等机器学习模型进行情感分类和趋势预测，使用准确率、精确率、召回率和F1分数评估模型性能。

Result: BERT模型取得最高性能（准确率：83.48%，精确率：79.37%，召回率：90.60%，F1：84.61）。LSTM和BERT能有效捕捉语言和上下文模式，提高预测准确性。时间分析显示情感随时间变化，命名实体识别识别出相关术语和主题。

Conclusion: 该研究解决了讽刺检测和多语言数据处理等挑战，提供了一个可扩展的框架来生成可操作的消费者洞察。
...
### [136] [Directive, Metacognitive or a Blend of Both? A Comparison of AI-Generated Feedback Types on Student Engagement, Confidence, and Outcomes](https://arxiv.org/abs/2510.19685)
*Omar Alsaiari,Nilufar Baghaei,Jason M. Lodge,Omid Noroozi,Dragan Gašević,Marie Boden,Hassan Khosravi*

Main category: cs.HC

TL;DR: 本研究比较了三种AI生成反馈方法（指令性、元认知和混合型）在编程课程中的效果，发现混合反馈最能促进学生修订行为，而所有方法在自信度和作品质量方面表现相当。


<details>
  <summary>Details</summary>
Motivation: 反馈对学习有重要影响，但指令性反馈和元认知反馈在AI教育环境中的比较效果尚不明确，需要探索哪种方法能更好地平衡学习效率和自我调节能力发展。

Method: 在入门设计和编程课程中进行为期一学期的随机对照试验，329名学生被分配到接收指令性、元认知或混合型AI生成反馈的组别，使用自适应教育平台。

Result: 混合反馈组的学生修订行为最多，指令性和元认知组次之；所有组别的自信度评分都很高，作品质量结果在各组间相当。

Conclusion: AI在提供平衡清晰度和反思性的反馈方面具有潜力，混合方法特别有前景，能够结合即时改进的行动指导和自我反思的机会。
...
### [137] [LifeSync-Games: Toward a Video Game Paradigm for Promoting Responsible Gaming and Human Development](https://arxiv.org/abs/2510.19691)
*R. González-Ibáñez,J. Macías-Cáceres,M. Villalta-Paucar*

Main category: cs.HC

TL;DR: LifeSync-Games框架通过简化数字孪生技术将虚拟游戏与现实生活活动连接起来，旨在增强游戏的发展价值，促进自我调节和在身体、心理、社交领域的成长。


<details>
  <summary>Details</summary>
Motivation: 当前游戏在支持人类发展和福祉方面的潜力未被充分利用，公众讨论和监管更关注风险而非益处。为纠正这种不平衡，需要建立连接虚拟游戏与现实生活的框架。

Method: 利用简化数字孪生技术创建LifeSync-Games框架，包括理论基础、技术组件、设计指南和评估方法，并在新游戏和畅销游戏中进行了早期应用验证。

Result: 提出了一个能够连接虚拟游戏与现实生活活动的完整框架，展示了其在不同类型游戏中的适用性和实践相关性。

Conclusion: LifeSync-Games框架通过建立虚拟游戏与现实生活的互惠关系，能够增强游戏的发展价值，促进自我调节和全面发展。
...
### [138] [Multi-scale topology optimization of porous heat sinks with voided lattice structure using a two-layer Darcy-Forchheimer model](https://arxiv.org/abs/2510.18882)
*Tatsuki Saito,Yuto Kikuchi,Kuniharu Ushijima,Kentaro Yaji*

Main category: physics.comp-ph

TL;DR: 提出了一种结合空隙晶格结构的水冷散热片拓扑优化框架，使用双层Darcy-Forchheimer模型，显著降低压降并提高散热性能。


<details>
  <summary>Details</summary>
Motivation: 传统多孔散热片由于复杂几何结构导致压降过大，限制了实际应用，需要开发能同时优化空隙和多孔区域的新方法。

Method: 采用多材料优化框架，显式表示空隙和多孔区域，结合梯度晶格密度，使用双层Darcy-Forchheimer模型进行降阶仿真，并在固定压降条件下通过热流体有限元分析验证。

Result: 优化后的空隙晶格结构相比传统板翅式和均匀晶格散热片，努塞尔数提高20-30%，同时保持更低的压力损失。

Conclusion: 提出的拓扑优化框架能有效设计高性能水冷散热片，空隙晶格结构在散热性能和压降控制方面均优于传统设计。
...
### [139] [Machine-learned domain partitioning for computationally efficient coupling of continuum and particle simulations of membrane fabrication](https://arxiv.org/abs/2510.19051)
*Matthias Busch,Gregor Häfner,Jiayu Xie,Marius Tacke,Marcus Müller,Christian J. Cyron,Roland C. Aydin*

Main category: physics.comp-ph

TL;DR: 提出了一种基于机器学习的决策模型，用于动态选择不同分辨率的模拟方法，通过MLP预测粒子模拟与连续介质模拟之间的预期差异，实现高效的混合多尺度模拟。


<details>
  <summary>Details</summary>
Motivation: 解决分子级粒子模拟在大时空域中计算可扩展性的限制，通过混合方法结合不同分辨率的模拟来克服计算需求快速增长的问题。

Method: 构建多层感知器(MLP)决策模型，预测粒子模拟与连续介质模拟结果的预期差异，在运行时动态选择模拟方法，仅在预期能增加价值的地方局部使用高保真粒子模拟。

Result: 该并发方法应用于膜制造过程模拟，成功将粒子模拟与连续介质模型耦合，实现了高效、可扩展和自适应的多尺度模拟。

Conclusion: 基于机器学习的决策模型能够有效集成到模拟工作流中，为大规模多尺度模拟提供了一种有前景的解决方案。
...
### [140] [Calibrated Principal Component Regression](https://arxiv.org/abs/2510.19020)
*Yixuan Florence Wu,Yilun Zhu,Lei Cao and,Naichen Shi*

Main category: stat.ML

TL;DR: 提出校准主成分回归（CPCR）方法，通过结合主成分子空间学习低方差先验和原始特征空间校准，在过参数化广义线性模型中减少方差同时控制截断偏差。


<details>
  <summary>Details</summary>
Motivation: 传统主成分回归（PCR）在过参数化设置中通过降维减少方差，但当真实回归向量在保留主成分之外有分量时会产生截断偏差。需要一种方法既能减少方差又能控制这种偏差。

Method: CPCR首先在主成分子空间学习低方差先验，然后通过中心化Tikhonov步骤在原始特征空间校准模型，利用交叉拟合并软化PCR的硬截断。

Result: 理论分析显示在随机矩阵机制下，当回归信号在低方差方向有不可忽略分量时，CPCR优于标准PCR。实证表明CPCR在多个过参数化问题中持续改善预测性能。

Conclusion: CPCR在现代过参数化设置中表现出稳定性和灵活性，能有效平衡方差减少和偏差控制。
...
### [141] [Signature Kernel Scoring Rule as Spatio-Temporal Diagnostic for Probabilistic Forecasting](https://arxiv.org/abs/2510.19110)
*Archer Dodson,Ritabrata Dutta*

Main category: stat.ML

TL;DR: 本文提出了一种基于粗糙路径理论的签名核评分规则，用于改进数据驱动的天气预报模型的训练和评估，能够更好地捕捉时空依赖关系。


<details>
  <summary>Details</summary>
Motivation: 传统评分规则（如MSE）忽略了天气和大气系统中高度相关的数据结构，限制了概率天气预报模型的训练和评估效果。

Method: 引入签名核评分规则，将天气变量重新定义为连续路径，通过迭代积分编码时空依赖关系，并使用路径增强保证唯一性。

Result: 在WeatherBench 2模型上的实证评估显示，签名核评分规则具有高判别能力和独特的路径依赖交互捕捉能力。使用轻量级模型进行训练，在15个时间步的预测路径上优于气候学基准。

Conclusion: 签名核评分规则为天气预报验证和模型训练提供了理论鲁棒的度量标准，能够有效改进数据驱动天气预报模型的性能。
...
### [142] [Extreme Event Aware ($η$-) Learning](https://arxiv.org/abs/2510.19161)
*Kai Chang,Themistoklis P. Sapsis*

Main category: stat.ML

TL;DR: 本文提出了一种名为Extreme Event Aware (e2a或η-learning)的新方法，用于在训练数据中缺乏极端事件的情况下预测罕见极端事件。该方法通过强制模型符合可观测的极端事件统计特性来减少不确定性，即使在没有极端事件数据的区域也能生成前所未有的极端事件。


<details>
  <summary>Details</summary>
Motivation: 量化预测罕见极端事件对理解复杂动力系统至关重要，但现有方法通常假设训练数据中存在多个极端事件，导致在极端事件区域具有较高的认知不确定性。本文旨在克服这一限制。

Method: 提出η-learning方法，通过在训练过程中强制模型符合指示极端性的可观测统计特性，进行统计正则化。该方法不需要假设可用数据中存在极端事件，利用定性论证或未标记数据估计的统计特性来减少不确定性。

Result: 基于最优传输的理论结果为该方法提供了严格的理论依据并突出了其最优性。大量数值实验表明，η-learning框架在多个原型问题和实际降水降尺度问题上表现出优越性能。

Conclusion: η-learning方法能够在训练数据缺乏极端事件的情况下，通过统计正则化减少极端事件区域的不确定性，成功生成前所未有的极端事件，为罕见极端事件预测提供了有效解决方案。
...
### [143] [Topology of Currencies: Persistent Homology for FX Co-movements: A Comparative Clustering Study](https://arxiv.org/abs/2510.19306)
*Pattravadee de Favereau de Jeneret,Ioannis Diamantis*

Main category: stat.ML

TL;DR: 该研究比较了拓扑数据分析与传统统计方法在聚类货币行为方面的表现，发现TDA特征能产生更紧凑和分离度更好的聚类，特别是在外汇市场这种复杂系统中。


<details>
  <summary>Details</summary>
Motivation: 外汇市场是一个复杂的系统，通常表现出非线性和高维动态，传统统计方法可能无法完全捕捉这些特征。研究旨在探索TDA是否能提供超越传统方法的额外洞察。

Method: 使用13种主要货币兑欧元的月度对数收益率，比较基于TDA衍生特征和传统统计特征的聚类结果。应用k-means和层次聚类两种算法，通过轮廓系数和Calinski-Harabasz指数评估聚类质量。

Result: 基于TDA特征的聚类产生了更紧凑和分离度更好的聚类，特别是在Calinski-Harabasz得分上显著更高。但所有聚类方法的轮廓系数都较为一般，反映了外汇时间序列聚类的内在难度。

Conclusion: TDA能够捕捉传统方法可能忽略的货币联动中的结构模式，是分析金融时间序列的有价值补充工具，在风险管理等领域具有应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型的出现为法律文档分析自动化带来了革命性变革，特别是在合同管理系统中的合同审查环节。本文旨在解决合同审查中法律条款自动检测和标注的挑战，减少合同审查的时间和成本。

Method: 结合公开的Contract Understanding Atticus Dataset和专有合同数据集，采用三种关键优化策略：鲁棒的文本转换、战略性的分块选择，以及包括思维链提示和结构化工具调用的高级LLM特定技术。

Result: 实验结果显示，该方法在法律条款识别准确性和效率方面取得了显著提升，能够有效减少合同审查的时间和成本，同时保持较高的识别准确性。

Conclusion: 经过精心优化的大语言模型系统可以作为法律专业人士的有价值工具，有望为各种规模的组织提供高效的合同审查服务。
...
### [145] [On the hardness of RL with Lookahead](https://arxiv.org/abs/2510.19372)
*Corentin Pla,Hugo Richard,Marc Abeille,Nadav Merlis,Vianney Perchet*

Main category: stat.ML

TL;DR: 本文研究了具有转移前瞻的强化学习，其中智能体可以在决定行动前观察执行任意ℓ步行动后会访问的状态。研究发现一步前瞻（ℓ=1）可以通过线性规划在多项式时间内求解，但当ℓ≥2时，问题变为NP难问题。


<details>
  <summary>Details</summary>
Motivation: 研究转移前瞻信息对强化学习性能的影响，以及使用这种信息的最优规划的计算复杂性。

Method: 提出了一种新颖的线性规划公式来解决一步前瞻（ℓ=1）的最优规划问题，并分析了ℓ≥2情况下的计算复杂性。

Result: 证明了一步前瞻（ℓ=1）可以在多项式时间内求解，而ℓ≥2时问题变为NP难问题。

Conclusion: 研究结果精确界定了强化学习中转移前瞻规划问题的可处理与难处理情况之间的边界。
...
### [146] [Learning Upper Lower Value Envelopes to Shape Online RL: A Principled Approach](https://arxiv.org/abs/2510.19528)
*Sebastian Reboul,Hélène Halconruy,Randal Douc*

Main category: stat.ML

TL;DR: 本文提出了一种利用离线数据加速在线强化学习的双阶段框架，通过数据驱动的价值函数上下界学习来改进在线算法性能。


<details>
  <summary>Details</summary>
Motivation: 研究如何利用离线数据加速在线强化学习，这是一个具有潜力但理论基础有限的方向，旨在建立离线预训练与在线微调之间的正式桥梁。

Method: 提出双阶段框架：第一阶段使用离线数据推导价值函数的上界和下界；第二阶段将这些学习到的边界融入在线算法中。该方法通过解耦上下界实现更灵活和紧密的近似，并使用过滤参数确保各阶段独立性。

Result: 在表格MDP上的实证结果显示，与UCBVI和先前方法相比，该方法显著减少了遗憾值。

Conclusion: 该方法为离线预训练和在线微调提供了正式的理论桥梁，通过数据驱动的价值包络学习实现了显著的性能提升。
...
### [147] [TinyUSFM: Towards Compact and Efficient Ultrasound Foundation Models](https://arxiv.org/abs/2510.19239)
*Chen Ma,Jing Jiao,Shuyu Liang,Junhu Fu,Qin Wang,Zeju Li,Yuanyuan Wang,Yi Guo*

Main category: eess.IV

TL;DR: TinyUSFM是首个轻量级超声基础模型，通过知识蒸馏和精心策划的小数据集，在保持大模型性能的同时显著提升计算效率，仅需6.36%参数和6.40%计算量即可达到原始模型性能。


<details>
  <summary>Details</summary>
Motivation: 医学影像基础模型虽然性能优越但计算资源需求大，限制了在资源受限临床环境中的部署，需要开发轻量级解决方案。

Method: 提出特征梯度驱动的核心集选择策略筛选高质量训练数据，开发域分离掩码图像建模辅助的一致性驱动动态蒸馏框架，通过不同域掩码下的教师模型一致性自适应传输知识。

Result: 仅使用20万图像进行蒸馏，TinyUSFM在分类任务上比基线模型提升9.45%，分割任务提升7.72%，平均分类准确率达84.91%，分割Dice分数达85.78%。

Conclusion: TinyUSFM成功实现了在保持大模型性能的同时显著降低计算需求，为资源受限临床环境提供了可行的轻量级超声基础模型解决方案。
...
### [148] [Automated Morphological Analysis of Neurons in Fluorescence Microscopy Using YOLOv8](https://arxiv.org/abs/2510.19455)
*Banan Alnemri,Arwa Basbrain*

Main category: eess.IV

TL;DR: 该论文提出了一种基于YOLOv8的神经元实例分割和形态测量管道，在干细胞衍生神经元的高分辨率数据集上实现了超过97%的分割准确率和75.32%的形态测量准确率。


<details>
  <summary>Details</summary>
Motivation: 荧光显微镜图像中神经元细胞的准确分割和形态分析在神经科学和生物医学成像应用中至关重要，但传统方法劳动密集且耗时，需要大量人工努力和专业知识。

Method: 使用在手动标注的显微镜图像上训练的YOLOv8模型进行神经元实例分割，并利用真实标注和预测掩码提取细胞长度、宽度、面积和灰度强度值等生物学重要特征。

Result: 模型实现了超过97%的高分割准确率，提取的形态学测量总体准确率达到75.32%。

Conclusion: 该集成框架为细胞成像和神经科学研究提供了自动化分析的有价值工具，减少了手动标注需求，实现了可扩展、精确的神经元形态量化。
...
### [149] [Local Guidance for Configuration-Based Multi-Agent Pathfinding](https://arxiv.org/abs/2510.19072)
*Tomoki Arita,Keisuke Okumura*

Main category: cs.MA

TL;DR: 该论文研究了一种为多智能体路径规划(MAPF)提供局部指导的方法，通过在智能体附近提供时空线索来改善解决方案质量，而不超过适度的时间预算。


<details>
  <summary>Details</summary>
Motivation: 探索为MAPF算法提供局部指导的替代方法，以改善协调效率，尽管这种方法涉及智能体移动时的重新计算，但可能显著提高解决方案质量。

Method: 为每个智能体在其附近提供局部指导，提供信息性的时空线索给规划器，应用于领先的配置求解器LaCAM。

Result: 这种局部指导形式显著提高了解决方案质量，为MAPF建立了新的性能前沿，且没有超过适度的时间预算。

Conclusion: 局部指导方法可以有效改善MAPF性能，通过提供智能体附近的时空线索，在不显著增加计算成本的情况下提高解决方案质量。
...
### [150] [Modeling realistic human behavior using generative agents in a multimodal transport system: Software architecture and Application to Toulouse](https://arxiv.org/abs/2510.19497)
*Trung-Dung Vu,Benoit Gaudou,Kamaldeep Singh Oberoi*

Main category: cs.MA

TL;DR: 本文提出了一种基于大语言模型和智能体模拟的架构，用于在复杂多模式交通系统中建模真实的人类移动行为，通过法国图卢兹的案例研究验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 建模真实人类行为以理解人们的出行方式选择，从而提出个性化移动解决方案仍然具有挑战性。

Method: 将大语言模型应用于基于智能体的模拟中，结合GAMA仿真平台、GTFS公共交通数据和OpenTripPlanner多模式路径规划，在真实城市环境中捕捉决策过程。

Result: 在一个月的模拟中，智能体不仅能够做出情境感知的交通决策，还会随时间形成出行习惯。

Conclusion: 将大语言模型与基于智能体的模拟相结合，为推进智能交通系统和个性化多模式移动解决方案提供了有前景的方向。
...
### [151] [Polynomial-time Configuration Generator for Connected Unlabeled Multi-Agent Pathfinding](https://arxiv.org/abs/2510.19567)
*Takahiro Suzuki,Keisuke Okumura*

Main category: cs.MA

TL;DR: 本文提出PULL算法解决连通无标签多智能体路径规划问题，该算法在多项式时间内完成，保证智能体间始终保持连通性。


<details>
  <summary>Details</summary>
Motivation: 在群体机器人应用中，如自重构和行进，标准多智能体路径规划无法保证智能体间的连通性，而连通无标签多智能体路径规划即使在高度受限的图类上也是NP难问题。

Method: 提出基于规则的单步函数算法PULL，计算保持连通性并朝向目标配置的后续配置，在2D网格中每步运行时间为O(n²)。

Result: PULL算法在随机生成的实例中，针对数百个智能体能够找到与平凡解相竞争的解决方案质量。

Conclusion: PULL是一种轻量级、多项式时间的完整算法，并开发了将PULL集成到现有搜索基MAPF算法中的最终最优求解器，为小规模实例提供了有价值的工具。
...
### [152] [ECKO: Explainable Clinical Knowledge for Oncology](https://arxiv.org/abs/2510.18929)
...
TL;DR: ECKO是一个专为肿瘤学设计的可解释临床知识图谱，整合了33个生物医学本体论和多个研究数据，旨在通过连接患者特异性分子数据与药理学知识来支持个性化药物推荐。


<details>
  <summary>Details</summary>
Motivation: 传统肿瘤治疗模式未能充分考虑患者的个体差异，个性化肿瘤学需要整合和解释大量异质性生物医学数据，以制定更精准的治疗策略。

Method: 构建ECKO知识图谱，整合33个生物医学本体论，聚合多个研究数据，创建统一资源，支持数据驱动的临床应用，提供透明可解释的药物推荐。

Result: ECKO能够通过连接患者特异性分子数据与相关药理学知识，促进最佳治疗方案的识别，为药物推荐提供透明可解释的依据。

Conclusion: ECKO代表了肿瘤学个性化医学向可解释、可扩展和临床可操作方向的重要进展，在生物标志物发现、治疗优化和转化研究方面具有潜在应用价值。
...
### [153] [BATIS: Bayesian Approaches for Targeted Improvement of Species Distribution Models](https://arxiv.org/abs/2510.19749)
*Catherine Villeneuve,Benjamin Akera,Mélisande Teng,David Rolnick*

Main category: cs.LG

TL;DR: 本文提出BATIS框架，从贝叶斯视角改进物种分布模型，通过迭代更新先验预测来应对数据空间偏差问题，在数据稀缺地区显著提升模型可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习物种分布模型在处理复杂异构数据时表现良好，但受到数据空间偏差的限制，需要更有效地结合局部观测与生态模式。

Method: 引入BATIS框架，采用贝叶斯深度学习方法，迭代更新先验预测，同时捕捉偶然性和认知不确定性，在eBird公民科学数据集上进行了广泛的不确定性量化方法基准测试。

Result: 实证研究表明贝叶斯深度学习方法能大幅提高物种分布模型在数据稀缺地区的可靠性。

Conclusion: BATIS框架通过贝叶斯深度学习方法有效提升了物种分布模型的可靠性，特别是在数据稀缺区域，有助于生态理解和保护工作。
...
### [154] [ADPO: Anchored Direct Preference Optimization](https://arxiv.org/abs/2510.18913)
*Wang Zixian*
...
<details>
  <summary>Details</summary>
Motivation: 标准DPO假设硬二元标签和成对比较，在存在不确定性和噪声时表现不佳。ADPO旨在通过软偏好概率、参考策略锚定和列表式扩展来解决这些问题。

Method: ADPO引入：(i)软偏好概率编码不确定性并减轻梯度漂移；(ii)任意参考策略锚定通过组间平移不变性和隐式KL正则化稳定训练；(iii)通过Plackett-Luce分布进行列表式偏好建模。

Result: 在上下文赌博机中，锚定比标准DPO提升WinMass 38-63%，KDE平滑在重尾噪声下达到0.68 vs 0.32（相对增益112%）。在顺序强化学习中，锚定在噪声偏好下性能提升15-29%。

Conclusion: ADPO将DPO、Bradley-Terry目标和Top-1-vs-Rest公式统一为特例，为不同噪声环境提供实用指导：干净或中等噪声使用成对锚定Soft-DPO，极端污染使用基于KDE的列表式ADPO。
...
### [155] [Scalable LinUCB: Low-Rank Design Matrix Updates for Recommenders with Large Action Spaces](https://arxiv.org/abs/2510.19349)
*Evgenia Shustova,Marina Sheshukova,Sergey Samsonov,Evgeny Frolov*

Main category: cs.LG

TL;DR: 本文提出了Scalable LinUCB算法，通过动态低秩参数化逆Cholesky因子，实现了快速且内存高效的线性上下文多臂老虎机操作，解决了传统LinUCB在特征维度和动作空间增大时的计算瓶颈。


<details>
  <summary>Details</summary>
Motivation: 线性上下文多臂老虎机（特别是LinUCB）在推荐系统中广泛应用，但其训练、推理和内存成本随特征维度和动作空间大小而增长。主要瓶颈是需要更新、求逆和存储吸收交互历史上下文信息的设计矩阵。

Method: 通过动态低秩参数化逆Cholesky式因子，推导出数值稳定的秩1和批量更新方法，无需直接形成整个矩阵。为控制内存增长，采用投影-分裂积分器进行动态低秩逼近。

Result: 实现了平均每步更新成本O(dr)和内存O(dr)，其中r为逼近秩。每个动作评估的推理复杂度为O(dr)。在推荐系统数据集上的实验证明了算法的有效性。

Conclusion: Scalable LinUCB算法成功解决了传统LinUCB的计算瓶颈，提供了可扩展的解决方案，适用于大规模推荐系统应用。
...
### [156] [Policy Learning with Abstention](https://arxiv.org/abs/2510.19672)
*Ayush Sawarni,Jikai Jin,Justin Whitehouse,Vasilis Syrgkanis*

Main category: cs.LG

TL;DR: 该论文研究策略学习中的弃权机制，允许策略在不确定时选择弃权并采用默认安全选项，提出两阶段学习器并建立了快速后悔保证。


<details>
  <summary>Details</summary>
Motivation: 传统策略学习算法在预测不确定时仍强制决策，这在高风险环境中存在风险。需要研究允许策略弃权的机制，以降低风险并提高安全性。

Method: 提出两阶段学习器：首先识别一组近似最优策略，然后基于这些策略之间的分歧构建弃权规则。使用双重稳健目标处理未知倾向得分情况。

Result: 当倾向得分已知时，获得了O(1/n)类型的快速后悔保证；在未知倾向得分情况下，通过双重稳健目标扩展了这些保证。

Conclusion: 弃权是策略学习中的多功能工具，可应用于边界条件下的改进保证、分布式鲁棒策略学习以及安全策略改进等核心问题。
...
### [157] [Statistical Inference for Linear Functionals of Online Least-squares SGD when $t \gtrsim d^{1+δ}$](https://arxiv.org/abs/2510.19734)
...
Method: 提出基于在线SGD的程序，在O(td)时间内运行，仅需O(d)内存，并开发了在线方差估计器。

Result: 证明当迭代次数t ≳ d^(1+δ)时SGD迭代满足CLT，相比现有方法的t ≳ d^(3/2)要求显著改善。

Conclusion: 这些结果为在接近最优的缩放机制t ≳ d^(1+δ)下构建SGD迭代的置信区间提供了首个完全在线和数据驱动的框架。
...
### [158] [From Prototypes to Sparse ECG Explanations: SHAP-Driven Counterfactuals for Multivariate Time-Series Multi-class Classification](https://arxiv.org/abs/2510.19514)
*Maciej Mozolewski,Betül Bayrak,Kerstin Bach,Grzegorz J. Nalepa*

Main category: cs.LG

TL;DR: 提出一个基于原型的框架，为12导联ECG分类模型生成稀疏反事实解释，通过SHAP阈值识别关键信号段，使用DTW和聚类提取原型，保持81.3%的有效性并提高43%的时间稳定性。


<details>
  <summary>Details</summary>
Motivation: 解决时间序列可解释性AI中，特别是医疗领域ECG分类模型解释性的挑战，提供可操作且可解释的洞察。

Method: 使用SHAP阈值识别关键信号段并转换为区间规则，采用动态时间规整和中心聚类提取代表性原型，将原型与查询R峰对齐以确保与待解释样本的一致性。

Result: 生成的反事实解释仅修改原始信号的78%，在所有类别中保持81.3%的有效性，时间稳定性提高43%。心肌梗死类别有效性达98.9%，但肥厚性心肌病检测面临挑战（13.2%）。

Conclusion: 该方法支持近实时生成临床有效的反事实解释，为交互式解释平台奠定基础，建立了AI诊断系统中生理感知反事实解释的设计原则，并为临床部署的用户控制解释接口指明了方向。
...
### [159] [3D Optimization for AI Inference Scaling: Balancing Accuracy, Cost, and Latency](https://arxiv.org/abs/2510.18905)
*Minseok Jung,Abhas Ricky,Muhammad Rameez Chatni*

Main category: cs.LG

TL;DR: 提出了一个3D优化框架，联合校准准确率、成本和延迟，在统一决策空间内实现约束感知的推理扩展。通过蒙特卡洛模拟评估了四种优化方法，发现knee-point优化能达到最佳平衡。
...
Method: 使用蒙特卡洛模拟在三个代表性场景和九个模拟大语言模型上，评估四种解决3D多目标优化问题的优化方法。

Result: MOO框架形成了1D和2D优化无法捕获的可行空间，knee-point优化实现最佳平衡，当精度优先时准确率最大化方法仍具优势。
...
### [160] [Large Connectome Model: An fMRI Foundation Model of Brain Connectomes Empowered by Brain-Environment Interaction in Multitask Learning Landscape](https://arxiv.org/abs/2510.18910)
*Ziquan Wei,Tingting Dan,Guorong Wu*

Main category: cs.LG

TL;DR: 提出了一种基于多任务学习的脑功能成像基础模型，通过标记化大脑-环境交互进行多任务预训练，并使用伪标签进行半监督微调，在性别预测、行为识别和多种疾病早期诊断中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前AI模型在临床应用中因样本量有限而性能受限，需要建立可靠的脑功能成像基础模型。自监督学习与脑-结果关系不一定对齐，导致现有基础模型对下游任务不够优化。

Method: 利用丰富的环境变量和人口统计数据，将脑建模构建为多任务学习：1）通过标记化多个大脑-环境交互进行多任务预训练；2）使用预训练BEI的伪标签进行半监督微调。

Result: 在性别预测、人类行为识别以及自闭症、帕金森病、阿尔茨海默病和精神分裂症的早期诊断等多种应用中评估，显示出促进当前神经影像学临床应用的良好潜力。

Conclusion: 该方法通过多任务预训练和半监督微调，为脑功能成像建立了更优化的基础模型，有望显著促进神经影像学在临床常规中的应用。
...
### [161] [Benchmarking On-Device Machine Learning on Apple Silicon with MLX](https://arxiv.org/abs/2510.18921)
*Oluwaseun A. Ajayi,Ogundepo Odunayo*

Main category: cs.LG

TL;DR: 本文对苹果硅设备上的MLX框架进行性能评估，主要比较MLX与PyTorch在Transformer模型推理延迟方面的表现，结果显示MLX在苹果生态系统内能实现高效、易用的设备端机器学习应用。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型和机器学习的广泛应用，研究如何在笔记本电脑和移动设备等小型设备上部署这些模型变得日益重要，这需要能够充分利用设备端硬件的框架和方法。

Method: 创建了MLX-transformers框架，包含MLX中的不同Transformer实现，并下载PyTorch模型检查点转换为MLX格式。在两个苹果硅MacBook设备上对BERT、RoBERTa和XLM-RoBERTa模型进行基准测试，与NVIDIA CUDA GPU进行性能对比。

Result: 研究结果显示MLX在苹果硅设备上表现出良好的性能，能够实现高效的Transformer模型推理，为苹果生态系统内的设备端机器学习应用提供了可行方案。

Conclusion: MLX框架在苹果硅设备上具有显著潜力，能够支持高效且更易访问的设备端机器学习应用，特别是在Transformer模型的推理任务中表现优异。
...
### [162] [Noise-corrected GRPO: From Noisy Rewards to Unbiased Gradients](https://arxiv.org/abs/2510.18924)
*Omar El mansouri,Mohamed El Amine Seddik,Salem Lahlou*

Main category: cs.LG

TL;DR: 提出了一种噪声鲁棒的Group Relative Policy Optimization (GRPO)和Done Right GRPO (Dr.GRPO)框架，通过建模奖励噪声为伯努利噪声并进行噪声校正，在数学和代码任务上显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 强化学习从人类反馈(RLHF)或可验证奖励(RLVR)对不一致或错误奖励产生的噪声高度敏感，而噪声与广泛使用的基于群体的策略优化方法之间的相互作用尚未充分探索。
...
### [163] [BAPO: Stabilizing Off-Policy Reinforcement Learning for LLMs via Balanced Policy Optimization with Adaptive Clipping](https://arxiv.org/abs/2510.18927)
...
TL;DR: 本文提出BAPO方法解决离线强化学习中策略熵下降、优化不稳定和崩溃的问题，通过动态调整裁剪边界来平衡正负贡献，在多个基准测试中取得优异表现。
...
Method: 提出BAPO方法，基于熵裁剪规则，动态调整裁剪边界来平衡正负贡献，保持策略熵，稳定强化学习优化过程。

Result: 在AIME 2024和2025基准测试中，7B BAPO模型超越SkyWork-OR1-7B等开源模型，32B BAPO模型在同规模模型中达到最优，甚至超过o3-mini和Gemini-2.5-Flash-Thinking等专有系统。

Conclusion: BAPO方法通过自适应裁剪机制有效解决了离线强化学习的稳定性问题，实现了快速、稳定且数据高效的训练，在多个基准测试中表现出色。
...
TL;DR: 开发了一个集成LLM、CTGAN、T-learner和上下文多臂赌博机的综合系统，用于提供个性化的临床治疗建议，在结肠癌数据集上表现优于其他方法。
...
Method: 使用LLM处理非结构化医疗叙述为结构化数据（93.2%准确率），CTGAN生成真实合成患者数据（55%准确率），T-learner预测患者特异性治疗反应（84.3%准确率），上下文多臂赌博机平衡探索与利用来优化治疗选择。

Result: 在III期结肠癌数据集测试中，KernelUCB方法在5000轮中获得0.60-0.61平均奖励分数，优于其他参考方法。

Conclusion: 该系统克服了在线学习环境中的冷启动限制，提高了计算效率，在个体化医疗方面取得了显著进展。
...
TL;DR: 本文通过统一的离线评估框架，将课程难度分解为五个维度，研究了课程学习在大型语言模型推理任务中的效果。研究发现没有通用的最优课程策略，前向与反向课程学习的相对效果取决于模型能力和任务复杂度，不同难度样本在不同任务中产生不同收益。


<details>
  <summary>Details</summary>
Motivation: 现有课程学习研究使用不同的难度指标和训练设置，导致对课程学习何时有效、哪种方向更好等基本问题缺乏统一认识。本文旨在通过系统性的评估框架解决这些问题。

Method: 采用统一的离线评估框架，将课程难度分解为五个维度：问题难度、模型惊奇度、置信度边界、预测不确定性和决策变异性。在数学推理基准上对Llama3.1-8B、Mistral-7B和Gemma3-4B进行受控后训练实验。

Result: 发现没有通用的最优课程策略，前向与反向课程学习的相对效果取决于模型能力和任务复杂度；同一指标内不同难度样本在不同任务中产生不同收益；任务对齐课程主要影响最终表示和泛化能力，而内部状态课程调节置信度和不确定性等内部状态。

Conclusion: 研究挑战了通用课程策略的概念，为不同模型和任务场景提供了实用指导，某些指标表明优先处理决策不确定样本可以进一步改善学习效果。
...
### [166] [Steering Autoregressive Music Generation with Recursive Feature Machines](https://arxiv.org/abs/2510.19127)
*Daniel Zhao,Daniel Beaglehole,Taylor Berg-Kirkpatrick,Julian McAuley,Zachary Novack*

Main category: cs.LG

TL;DR: MusicRFM是一个基于递归特征机（RFMs）的框架，能够在无需重新训练的情况下对预训练音乐模型进行细粒度、可解释的控制，通过直接引导模型内部激活来实现对音符、和弦等音乐属性的精确控制。
...
Method: 训练轻量级RFM探针来发现MusicGen隐藏状态中的概念方向（对应音乐属性），在推理过程中将这些方向注入回模型以实时引导生成过程，无需逐步骤优化。

Result: 能够将目标音符生成的准确率从0.23提高到0.82，同时文本提示的遵循度保持在未引导基线的约0.02范围内，实现了有效控制且对提示保真度影响最小。

Conclusion: MusicRFM成功平衡了控制与生成质量之间的权衡，展示了在音乐领域中RFMs的有效应用，为可控音乐生成提供了新的解决方案。
...
### [167] [InvarGC: Invariant Granger Causality for Heterogeneous Interventional Time Series under Latent Confounding](https://arxiv.org/abs/2510.19138)
*Ziyi Zhang,Shaogang Ren,Xiaoning Qian,Nick Duffield*

Main category: cs.LG

TL;DR: 提出InvarGC方法，利用跨环境异质性解决传统Granger因果关系分析中的潜在混杂和干预环境识别问题，在合成和真实数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统Granger因果关系基于线性模型，难以检测非线性因果关系，且依赖因果充分性和已知干预目标两个关键假设。现实中存在潜在混杂因素，且时间序列数据通常来自异质环境，缺乏干预先验知识。

Method: 提出不变Granger因果关系(InvarGC)，利用跨环境异质性减轻潜在混杂影响，以边级粒度区分干预和非干预环境，从而恢复不变因果关系。

Result: 在合成和真实数据集上的广泛实验表明，该方法相比最先进方法具有竞争优势。

Conclusion: InvarGC能够有效解决传统Granger因果关系分析的局限性，在存在潜在混杂和未知干预环境的情况下恢复不变因果关系。
...
### [168] [Imbalanced Gradients in RL Post-Training of Multi-Task LLMs](https://arxiv.org/abs/2510.19178)
...
TL;DR: 论文发现多任务后训练中存在梯度不平衡问题，某些任务会产生显著更大的梯度，导致优化偏向这些任务，但大梯度并不一定带来更大的学习收益。


<details>
  <summary>Details</summary>
Motivation: 传统的多任务后训练假设所有任务贡献相似大小的梯度，但本文发现这个假设在RL后训练中不成立，某些任务会产生显著更大的梯度，导致优化偏差。

Method: 通过分析多任务后训练中的梯度分布，研究不同任务产生的梯度大小与学习收益之间的关系。

Result: 研究发现大梯度任务并不一定获得更大的学习收益，梯度不平衡不能通过典型的训练统计量（如训练奖励或优势）来解释，而是源于任务间的固有差异。

Conclusion: 研究警示不要简单地混合数据集进行多任务训练，呼吁未来研究开发基于梯度级别的修正方法。
...
### [169] [SPOT: Scalable Policy Optimization with Trees for Markov Decision Processes](https://arxiv.org/abs/2510.19241)
*Xuyuan Xiong,Pedro Chumpitaz-Flores,Kaixun Hua,Cheng Hua*

Main category: cs.LG

TL;DR: SPOT是一种新颖的决策树策略计算方法，通过混合整数线性规划优化MDP中的决策树策略，采用降维分支定界法提高效率，在保持可解释性的同时显著提升计算速度和可扩展性。
...
Method: 将优化问题建模为混合整数线性规划，采用降维分支定界方法将MDP动态与树结构约束解耦，实现高效并行搜索。

Result: 在标准基准测试中，SPOT实现了显著的速度提升，可扩展到具有更多状态的更大MDP，决策树策略保持可解释性和紧凑性。
...
### [170] [Every Attention Matters: An Efficient Hybrid Architecture for Long-Context Reasoning](https://arxiv.org/abs/2510.19338)
...
<details>
  <summary>Details</summary>
Motivation: 解决长上下文推理场景中传统注意力机制的高I/O和计算开销问题，寻求更高效的模型架构。

Method: 采用线性注意力和softmax注意力的混合架构，通过系统探索不同注意力机制的比例确定最优结构，并使用自研的高性能FP8算子库linghe提升训练效率。

Result: Ring-mini-linear-2.0包含16B参数和957M激活，Ring-flash-linear-2.0包含104B参数和6.1B激活；相比32B密集模型推理成本降至1/10，相比原Ring系列成本降低超50%；训练效率提升50%，在多个复杂推理基准测试中保持SOTA性能。

Conclusion: Ring-linear模型系列通过混合注意力架构和高效算子库实现了显著的推理成本降低和性能提升，为长上下文推理提供了高效的解决方案。
...
### [171] [Foundation Model Forecasts: Form and Function](https://arxiv.org/abs/2510.19345)
*Alvaro Perez-Diaz,James C. Loach,Danielle E. Toutoungi,Lee Middleton*

Main category: cs.LG

TL;DR: 该论文分析了时间序列基础模型（TSFMs）的预测类型与实际应用价值的关系，指出虽然TSFMs在预测精度上表现良好，但预测类型（点预测、分位数预测、参数预测或轨迹集成）决定了其支持的操作任务。研究发现三分之二的TSFMs只产生点预测或参数预测，而许多操作任务需要保留时间依赖性的轨迹集成预测。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探讨预测类型如何影响时间序列基础模型的实际应用价值，而不仅仅是预测精度。当前大多数TSFMs产生的预测类型无法满足需要时间依赖性信息的操作任务需求。

Method: 方法包括：调查近期TSFMs的预测类型；分析不同预测类型之间的转换可能性；证明边际分布无法确定路径依赖事件概率；将六种基本预测任务映射到最小充分预测类型；提供任务对齐的评估框架。

Result: 研究发现：轨迹集成可以通过边际化转换为更简单的形式，但反向转换需要施加时间依赖性；边际分布无法唯一确定路径依赖事件概率；不同预测类型支持不同的操作任务。

Conclusion: 结论是预测类型而非精度决定了实际效用，需要根据具体任务选择合适的预测类型，并提出了任务对齐的评估框架来指导模型选择和应用。
...
### [172] [MetaCluster: Enabling Deep Compression of Kolmogorov-Arnold Network](https://arxiv.org/abs/2510.19105)
*Matthew Raffel,Adwaith Renjith,Lizhong Chen*

Main category: cs.LG

TL;DR: MetaCluster框架通过元学习器和聚类技术，显著压缩Kolmogorov-Arnold Networks (KANs)的参数存储，实现高达80倍的压缩率且不损失精度。


<details>
  <summary>Details</summary>
Motivation: KANs用向量权重替代标量权重提高了表达能力和精度，但导致参数和内存呈乘性增长，需要高效的压缩方法。
...
Conclusion: MetaCluster框架成功解决了KANs参数存储问题，通过聚类和共享机制实现了高效的模型压缩。
...
### [173] [FrogDeepSDM: Improving Frog Counting and Occurrence Prediction Using Multimodal Data and Pseudo-Absence Imputation](https://arxiv.org/abs/2510.19305)
*Chirag Padubidri,Pranesh Velmurugan,Andreas Lanitis,Andreas Kamilaris*

Main category: cs.LG

TL;DR: 本研究通过深度学习和数据插补技术改进蛙类物种分布模型，数据平衡显著提升模型性能，多模态集成模型在蛙类计数和栖息地分类任务中表现优异。
...
Method: 应用深度学习和数据插补技术，采用数据平衡和特征选择方法，构建多模态集成模型整合土地覆盖、NDVI等环境变量。

Result: 数据平衡将平均绝对误差从189降低到29，多模态模型在栖息地分类中达到84.9%准确率和0.90 AUC，表现出良好的泛化能力。

Conclusion: 多模态学习和数据预处理技术能显著改善生态预测模型，为稀疏或不完整数据条件下的生物多样性监测提供更精确和可扩展的解决方案。
...
### [174] [FairNet: Dynamic Fairness Correction without Performance Loss via Contrastive Conditional LoRA](https://arxiv.org/abs/2510.19421)
*Songqi Zhou,Zeyuan Liu,Benben Jiang*

Main category: cs.LG

TL;DR: FairNet是一个新颖的框架，通过动态实例级公平性校正来解决机器学习模型公平性问题，结合偏置检测器和条件低秩适应(LoRA)，仅在检测到偏置的实例上激活公平性校正机制。


<details>
  <summary>Details</summary>
Motivation: 现有去偏方法往往牺牲性能、依赖静态校正策略、难以处理数据稀疏问题，特别是在少数群体中，且对敏感属性的利用不够优化。

Method: FairNet整合偏置检测器与条件LoRA，使用新的对比损失函数训练LoRA模块，最小化不同敏感群体间的类内表示差异，灵活处理完全、部分或缺失敏感属性标签的情况。

Result: 理论分析表明，在偏置检测器具有中等TPR/FPR时，FairNet能提升最差群体性能而不降低整体模型性能，甚至可能带来轻微性能提升。
...
Method: 提出两部分安全案例：1) 确保模型在没有思维链时缺乏危险能力；2) 确保思维链启用的危险能力可被监控检测。系统分析监控性威胁（神经语言和编码推理），评估现有和新颖的思维链忠实性维护技术。

Result: 建立了预测市场来评估思维链监控安全案例的可行性，并分析了关键技术里程碑。
...
### [176] [Graph Unlearning Meets Influence-aware Negative Preference Optimization](https://arxiv.org/abs/2510.19479)
...
<details>
  <summary>Details</summary>
Motivation: 现有的图遗忘模型使用梯度上升在遗忘集上实现遗忘，但这种方法导致模型效用在遗忘过程中急剧下降，原因是梯度上升的快速散度速度。

Method: 1. 分析NPO具有较慢的散度速度，理论上提出遗忘高影响力边可以减少遗忘的影响；2. 设计影响力感知的消息函数来放大遗忘边的影响力并减轻遗忘集与保留集之间的紧密拓扑耦合；3. 使用基于移除的方法快速估计每条边的影响力；4. 从拓扑角度提出拓扑熵损失以避免在遗忘过程中局部结构过度信息损失。

Result: 在五个真实世界数据集上的广泛实验表明，基于INPO的模型在所有遗忘质量指标上均达到最先进的性能，同时保持了模型的效用。

Conclusion: INPO框架通过减缓散度速度和提高模型效用鲁棒性，在图遗忘任务中实现了优异的性能表现。
...
### [177] [Optimizing the Unknown: Black Box Bayesian Optimization with Energy-Based Model and Reinforcement Learning](https://arxiv.org/abs/2510.19530)
*Ruiyao Miao,Junren Xiao,Shiya Tsang,Hui Xiong,Yingnian Wu*

Main category: cs.LG

TL;DR: 提出REBMBO方法，结合高斯过程局部指导和能量基模型全局信息，使用PPO进行自适应多步前瞻，克服传统贝叶斯优化的单步偏差问题。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯优化方法存在显著的单步偏差，容易收敛到局部最优且在复杂高维任务中表现不佳。黑盒优化在科学工程领域取得成功，特别是在函数评估成本高且梯度不可用时。

Method: 将贝叶斯优化迭代定义为马尔可夫决策过程，使用近端策略优化进行自适应多步前瞻，结合高斯过程提供局部指导和能量基模型捕获全局结构信息。

Result: 在合成和真实世界基准测试上进行广泛实验，证实REBMBO的优越性能。在各种高斯过程配置下的额外分析进一步突显其适应性和鲁棒性。

Conclusion: REBMBO通过动态调整探索深度和方向，有效克服传统贝叶斯优化方法的局限性，在复杂优化任务中表现出色。
...
### [178] [Insights into the Unknown: Federated Data Diversity Analysis on Molecular Data](https://arxiv.org/abs/2510.19535)
*Markus Bujotzek,Evelyn Trautmann,Calum Hand,Ian Hales*

Main category: cs.LG

TL;DR: 本文研究了联邦学习在分子数据聚类中的应用，比较了三种联邦聚类方法与传统集中式方法的性能，并引入了化学领域特定的评估指标SF-ICF。
...
Method: 在8个分子数据集上对三种联邦聚类方法（Fed-kMeans、Fed-PCA+Fed-kMeans、Fed-LSH）与集中式方法进行大规模基准测试，使用标准数学指标和新提出的化学指标SF-ICF进行评估。

Result: 联邦聚类方法能够有效表示分布式分子数据，结合化学领域知识的评估指标和客户端可解释性分析对分子数据的联邦多样性分析至关重要。

Conclusion: 联邦聚类方法在保护隐私的同时能够有效分析分子数据的多样性，领域知识的融入和可解释性分析对于联邦学习在药物发现中的应用具有重要意义。
...
TL;DR: 该论文提出了一种新的语言模型遗忘方法，通过引导框架解决传统梯度上升方法导致的概率质量重新分配到高似然区域的问题，实现更彻底的遗忘同时保持模型效用。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型遗忘方法依赖梯度上升来降低特定目标响应的概率，但这种方法会产生挤压效应——概率质量被重新分配到语义相关的高似然区域，导致虚假遗忘。

Method: 提出引导框架，将挤压效应与模型自身高置信度生成相关联。通过联合抑制目标响应和模型信念，BS-T方法衰减高概率token，BS-S方法移除整个高置信度生成，共同对抗挤压效应。

Result: 在多样化基准测试和多种模型家族上的广泛实验证实了该方法的有效性，能够实现更彻底的遗忘同时保持模型效用。

Conclusion: 通过将模型信念纳入遗忘目标，引导框架直接对抗挤压效应，为语言模型安全遗忘提供了更有效的解决方案。
...
### [180] [GaLLoP: Gradient-based Sparse Learning on Low-Magnitude Parameters](https://arxiv.org/abs/2510.19778)
*Anand Choudhary,Yasser Sulaıman,Lukas Mauch,Ghouthi Boukli Hacene,Fabien Cardinaux,Antoine Bosselut*

Main category: cs.LG

TL;DR: GaLLoP是一种基于梯度的稀疏学习技术，仅微调在目标任务上梯度幅度最大且预训练幅度最小的模型参数，在保持性能的同时减少计算成本。
...
Result: 在LLaMA3 8B和Gemma 2B上的实验表明，GaLLoP在分布内和分布外性能上持续改进或匹配其他领先的参数高效微调技术（LoRA、DoRA、SAFT），并减轻灾难性遗忘和任务数据记忆问题。

Conclusion: GaLLoP通过保持重要预训练参数不变，稳定性能并实现跨随机种子的鲁棒泛化，是一种有效的稀疏微调方法。
...
### [181] [Blackbox Model Provenance via Palimpsestic Membership Inference](https://arxiv.org/abs/2510.19796)
*Rohith Kuditipudi,Jing Huang,Sally Zhu,Diyi Yang,Christopher Potts,Percy Liang*

Main category: cs.LG

TL;DR: 该论文研究如何证明Bob使用了Alice训练的开源语言模型，通过测试Bob的模型或文本与Alice训练数据顺序之间的相关性来检测模型派生关系。


<details>
  <summary>Details</summary>
Motivation: 动机是解决开源模型被他人使用但无法证明的问题，特别是在查询设置和观察设置下验证模型派生关系。

Method: 方法包括：1）查询设置中通过提示估计Bob模型对Alice训练数据顺序的似然性；2）观察设置中通过文本重叠和重新训练模型版本来检测相关性。

Result: 结果：查询设置在40多个微调模型上获得p值≤1e-8；观察设置中第二种方法仅需几百个token就能可靠区分，第一种方法需要几十万个token。

Conclusion: 结论：通过训练数据顺序相关性可以有效证明模型派生关系，为开源模型版权保护提供了统计证据支持。
...
TL;DR: 提出Enhanced TimeAutoDiff框架，通过合成ICU时间序列数据来训练和评估预测模型，在人口和细粒度亚组层面显著减少真实-合成数据评估差距，提高医疗AI的可信度。


<details>
  <summary>Details</summary>
Motivation: 为了解决医疗AI模型评估中数据隐私保护和细粒度亚组性能分析的问题，需要开发能够生成高质量合成数据的方法，以便在不暴露敏感电子健康记录的情况下进行可靠的模型评估。

Method: 基于先前的扩散和VAE生成器（TimeDiff、HealthGen、TimeAutoDiff），引入Enhanced TimeAutoDiff，在潜在扩散目标中增加了分布对齐惩罚项，并在MIMIC-III和eICU数据集上对24小时死亡率和住院时间任务进行广泛基准测试。

Result: Enhanced TimeAutoDiff将真实-合成数据评估差距减少了70%以上，达到ΔTRTS ≤ 0.014 AUROC，同时保持训练效用（ΔTSTR ≈ 0.01）。对于32个交叉亚组，大型合成队列将亚组级AUROC估计误差减少了50%，在72-84%的亚组中优于小型真实测试集。

Conclusion: 这项工作为重症监护中可信、细粒度的模型评估提供了实用且保护隐私的路线图，能够在不同患者群体中进行稳健可靠的性能分析，而无需暴露敏感EHR数据，有助于提高医疗AI的整体可信度。
...
### [183] [Semantic World Models](https://arxiv.org/abs/2510.19818)
*Jacob Berg,Chuning Zhu,Yanda Bao,Ishan Durugkar,Abhishek Gupta*

Main category: cs.LG

TL;DR: 该论文提出了一种新的世界建模方法，将世界建模视为视觉问答问题，只预测任务相关的语义信息而非像素重建，从而提升机器人规划性能。
...
Method: 将世界建模转化为视觉问答问题，使用视觉语言模型作为语义世界模型，通过监督微调在图像-动作-文本数据上训练。

Result: 该方法在开放机器人任务上实现了策略改进，相比基于重建的世界建模方法获得了显著泛化性能提升。

Conclusion: 语义世界建模方法能够继承预训练视觉语言模型的泛化和鲁棒性，为机器人决策规划提供更有效的解决方案。
...
### [184] [Cardiocirculatory Computational Models for the Study of Hypertension](https://arxiv.org/abs/2510.19302)
*Simone Celora,Andrea Tonini,Francesco Regazzoni,Luca Dede',Gianfranco Parati,Alfio Quarteroni*

Main category: math.NA

TL;DR: 开发患者特异性心血管循环模型用于高血压数字孪生，包含0D电路模型和3D-0D机电模型，通过敏感性分析识别关键参数，研究三种高血压场景并开发校准策略。


<details>
  <summary>Details</summary>
Motivation: 构建高血压数字孪生，提供比纯0D模型更详细的生理机制表征，实现实时高血压控制和临床洞察。

Method: 采用0D RLC电路模型（含/不含毛细血管）和3D-0D机电模型（左心室3D模型耦合0D循环模型），进行单参数和多参数敏感性分析，研究系统性、肺性和肾血管性高血压三种场景。

Result: 3D-0D模型能准确表征高血压下的心血管循环动力学，提供比0D模型更精细的临床洞察。

Conclusion: 3D-0D模型是实现高血压实时控制数字孪生的重要进展，提供超越纯0D模型的精细化临床信息。
...
### [185] [Energy dissipation and global convergence of a discrete normalized gradient flow for computing ground states of two-component Bose-Einstein condensates](https://arxiv.org/abs/2510.19392)
*Zixu Feng,Lunxu Liu,Qinglin Tang*

Main category: math.NA

TL;DR: 本文证明了半隐式离散梯度流算法在计算多组分玻色-爱因斯坦凝聚体基态时的能量耗散性质，特别针对含约瑟夫森结和旋转项的双组分模型，并建立了全局收敛性。


<details>
  <summary>Details</summary>
Motivation: 虽然数值实验表明GFSI算法在计算多组分BEC基态时具有能量耗散特性，但严格的数学证明一直是个开放性问题。

Method: 通过引入拉格朗日乘子，将GFSI算法重新表述为等价形式，从而证明其在含约瑟夫森结和旋转项的双组分BEC模型中的能量耗散性质。

Result: 成功证明了GFSI算法在特定模型中的能量耗散特性，建立了全局收敛到稳态的数学理论，并通过数值实验验证了时间步长上界与粒子相互作用强度的关系。

Conclusion: 本文为GFSI算法在多组分BEC基态计算中的能量耗散特性提供了严格的数学证明，填补了理论空白，并通过数值实验验证了理论结果。
...
### [186] [Interpolatory Dynamical Low-Rank Approximation: Theoretical Foundations and Algorithms](https://arxiv.org/abs/2510.19518)
*Benjamin Carrel,Daniel Kressner,Hei Yin Lam,Bart Vandereycken*

Main category: math.NA

TL;DR: 本文提出了一种结合离散经验插值方法(DEIM)的动态低秩逼近(DLRA)框架，通过使用斜投影替代正交投影来降低非线性问题的计算成本，并开发了新的投影积分器PRK-DEIM。


<details>
  <summary>Details</summary>
Motivation: 传统DLRA方法在处理非线性问题时，虽然能显著减少存储需求，但计算成本降低有限，因为需要评估整个速度场。本文旨在通过DEIM方法解决这一挑战。

Method: 使用DEIM选择数据稀疏的斜投影替代正交切线空间投影，构建DLRA-DEIM微分包含系统，并开发PRK-DEIM投影积分器结合显式Runge-Kutta方法和DEIM投影。

Result: 建立了DLRA-DEIM的存在性、精确性和误差界结果，与现有DLRA结果匹配；提出的PRK-DEIM方法在保持精度的同时显著降低了计算成本。

Conclusion: DLRA-DEIM框架为大规模矩阵微分方程提供了高效解决方案，PRK-DEIM积分器在计算效率和精度方面表现出色，并可扩展到指数Runge-Kutta方法和低阶张量微分方程。
...
### [187] [Dictionary learning methods for brain activity mapping with MEG data](https://arxiv.org/abs/2510.19702)
*Daniela Calvetti,Erkki Somersalo*

Main category: math.NA

TL;DR: 本文评估了一种贝叶斯字典学习（BDL）算法在MEG脑区识别中的性能，通过模拟激活脑区并分析算法在识别活跃脑区方面的表现。


<details>
  <summary>Details</summary>
Motivation: 研究旨在识别在观察窗口中被激活的脑区，MEG因其优异的时间分辨率和直接测量脑电活动的特性成为fMRI的有价值替代方案。

Method: 采用贝叶斯字典学习算法，将源空间划分为148个感兴趣区域，设计模拟协议随机激活每个ROI中的小块，计算MEG信号并使用BDL算法解决逆问题。算法包含字典压缩和贝叶斯压缩误差分析的第一阶段，以及基于第一阶段输出构建缩减字典进行字典编码的第二阶段。

Result: 通过混淆矩阵的概率解释和多类分类器的不同纯度度量来评估算法性能。

Conclusion: BDL算法在MEG脑区识别任务中表现出良好的性能，为脑功能研究提供了一种有效的替代方法。
...
### [188] [An active-flux-type scheme for ideal MHD with provable positivity and discrete divergence-free property](https://arxiv.org/abs/2510.19721)
*Mengqing Liu,Dongwen Pang,Remi Abgrall,Kailiang Wu*

Main category: math.NA

TL;DR: 提出了一种保持正性的PAMPA方案，用于理想磁流体动力学，在笛卡尔网格上强制执行离散无散磁场。该方法结合了无限制器的界面点值更新和局部无散投影，确保单元平均值在温和条件下保持正性。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够保持物理量正性（如密度和压力）且保持磁场无散性的高精度数值方法，以解决理想磁流体动力学中的强激波和高马赫数问题。

Method: 将一维不变域保持PAMPA框架扩展到多维多波磁流体动力学，采用新的非守恒重构、局部无散投影、几何拟线性化技术，结合Lax型熵故障单元指示器和凸振荡消除机制。

Result: 测试表明该方法具有高阶精度、清晰的磁流体结构分辨率和强激波鲁棒性，包括在等离子体β≈2.51×10⁻⁶的爆炸波和高达马赫10⁴的射流中表现良好。

Conclusion: 这是第一个严格保持单元平均值和界面点值正性，同时在整个计算过程中维持离散无散磁场的主动通量型理想磁流体动力学方法。
...
### [189] [Test-time Verification via Optimal Transport: Coverage, ROC, & Sub-optimality](https://arxiv.org/abs/2510.18982)
*Arpan Mukherjee,Marcello Bullo,Debabrota Basu,Deniz Gündüz*

Main category: cs.AI

TL;DR: 本文提出了一个统一框架来分析验证器在测试时缩放中的作用，将可验证测试时缩放建模为传输问题，揭示了次优性-覆盖率曲线的三个区域，并分析了顺序和批量采样算法的计算复杂性。


<details>
  <summary>Details</summary>
Motivation: 虽然带验证的测试时缩放显示出改善大语言模型性能的潜力，但验证器的作用及其缺陷仍未得到充分探索。现有研究只捕捉了这些因素的部分子集，缺乏量化它们相互作用几何结构的统一框架。

Method: 将可验证测试时缩放构建为传输问题，通过分析生成器覆盖率、验证器收敛区域和采样算法次优性三个量的相互作用，提出了顺序和批量两类采样算法。

Result: 理论分析表明次优性-覆盖率曲线存在三个区域：传输区域（次优性随覆盖率增加而增加）、策略改进区域（次优性可能随覆盖率增加而减少，取决于验证器的收敛区域）和饱和区域（次优性趋于平稳，不受覆盖率影响）。在Qwen、Llama和Gemma模型上的实证结果验证了理论发现。

Conclusion: 通过传输问题框架揭示了验证器在测试时缩放中的关键作用，提供了量化覆盖率、验证器收敛区域和采样算法次优性相互作用的统一视角，为设计更高效的验证测试时缩放方法提供了理论基础。
...
### [190] [Timely Clinical Diagnosis through Active Test Selection](https://arxiv.org/abs/2510.18988)
...
<details>
  <summary>Details</summary>
Motivation: 当前机器学习诊断方法依赖静态完整数据集，无法反映临床实践中顺序性、资源感知的推理过程。诊断在高压或资源有限环境中仍然复杂且容易出错，需要能够帮助临床医生做出及时且成本效益高的决策的框架。

Method: ACTMED将贝叶斯实验设计与大型语言模型集成，在每个步骤选择预期能为特定患者带来最大诊断不确定性减少的测试。LLMs作为灵活的模拟器，生成合理的患者状态分布并支持信念更新，无需结构化、任务特定的训练数据。
...
TL;DR: 该论文提出了MUSE基准测试，用于评估多模态大语言模型在音乐理解中的关系推理能力，发现当前SOTA模型在基本音乐感知方面存在严重缺陷，与人类专家存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在音频理解方面的评估可能掩盖了关系推理的基本弱点，需要更全面的基准测试来揭示模型在音乐感知方面的真实能力。

Method: 开发了MUSE基准测试，包含10个任务来探测基本音乐感知技能，评估了4个SOTA模型（Gemini Pro和Flash、Qwen2.5-Omni、Audio-Flamingo 3）并与200人的基准进行比较。

Result: 发现SOTA模型能力差异很大，与人类专家存在持续差距。Gemini Pro在基本感知上表现良好，但Qwen和Audio Flamingo 3表现接近随机水平，显示出严重的感知缺陷。思维链提示提供不一致且通常有害的结果。
...
TL;DR: 本研究通过行为和元认知分析方法，比较了两种代表性大语言模型在三种提示条件下评估临床试验报告CONSORT标准的能力，发现模型在推理风格、不确定性表达和解释模式方面存在明显差异。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在医疗领域快速扩张，但这些系统根据CONSORT标准评估临床试验报告的能力尚不明确，特别是在认知和推理策略方面。

Method: 采用行为和元认知分析方法，使用专家验证数据，系统比较两种代表性大语言模型在三种提示条件下的表现。

Result: 模型在处理不同CONSORT项目和提示类型时表现出明显差异，包括推理风格转变、明确的不确定性表达和替代解释模式。
...
TL;DR: 本文研究了推理模型中的模式选择问题，将其识别为早期退出问题的更具挑战性变体，旨在通过零步思考在推理过程开始时决定使用长链思维还是短链思维模式，以减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 推理模型在数学和逻辑推理任务中表现出色，但由于逐步思考过程往往导致过度思考，产生不必要的计算开销。为了解决这个问题，需要开发能够自动选择推理模式的方法。

Method: 将模式选择识别为早期退出问题的变体，比较了九种基线方法，包括基于提示的方法和利用模型内部信息的方法，重点关注在推理过程开始时基于预定义虚假思考做出决策的零步思考方法。

Result: 基于提示的方法由于分类能力有限且仅提供少量手工制作信息而经常失败；利用内部信息的方法在大多数场景中表现更好，但仍存在稳定性问题。现有方法仅依赖模型提供的信息不足以有效解决信息有限场景下的模式选择问题。

Conclusion: 模式选择是早期退出问题的更具挑战性变体，现有方法在信息有限场景下表现不足，突显了该任务的持续挑战性。需要开发更有效的方法来解决这一关键问题。
...
TL;DR: WebGraphEval是一个将多个智能体的网页交互轨迹抽象为统一加权行动图的框架，能够捕捉基准数据集中的结构多样性，超越传统的二元成功指标评估方法。
...
Method: 将多个智能体的轨迹抽象为统一的加权行动图，规范编码行动，合并重复行为，应用奖励传播和成功加权边统计等结构分析。

Result: 在来自六个网页智能体的数千条轨迹评估中，图抽象捕捉了跨模型规律性，突显了冗余和低效行为，识别了基于结果指标忽略的关键决策点。

Conclusion: 通过将网页交互构建为图结构数据，WebGraphEval为网页智能体建立了多路径、跨智能体和效率感知的通用评估方法学。
...
TL;DR: 本研究通过法律领域的实验评估ChatGPT的性能，与基于正则表达式的基线方法进行比较。研究发现ChatGPT即使具备必要的知识和能力，也无法有效整合这些能力进行推理以得出全面结果，揭示了其在法律领域的重大局限性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估ChatGPT在法律领域的实际表现，特别是与传统的正则表达式方法进行比较，而不仅仅关注与人类表现的对比。法律领域中最关键的任务之一是阅读法律判决并提取法律原则的关键段落，这对AI系统提出了严峻挑战。

Method: 采用实验方法，在法律领域测试ChatGPT的性能，并将其与基于正则表达式的基线方法进行比较。通过分析AI系统在处理法律决策和提取法律原则关键段落时的表现来评估其能力。

Result: 研究结果显示，ChatGPT即使拥有必要的知识和能力，也无法有效整合这些能力进行推理以得出全面结果。在法律文本分析和法律原则提取任务中，AI系统缺乏全面理解和推理能力，表现不如预期。

Conclusion: 研究表明ChatGPT在法律领域存在重大局限性，无法像人类那样分解复杂问题并根据多种所需能力进行解决。真正的智能仍然是人类独有的特质，至少在法律这一特定领域如此。
...
TL;DR: 本文扩展了推导状态论证框架（DSA框架），为能够处理不一致先例的广义理由模型提供论证性解释方法。


<details>
  <summary>Details</summary>
Motivation: 传统先例约束假设先例集必须一致，而广义理由模型放松了这一假设。目前已有基于传统一致理由模型的论证性解释方法，但缺乏针对广义推理框架的相应解释方法。

Method: 扩展推导状态论证框架（DSA-framework），使其能够解释基于广义理由模型的推理过程。

Result: 开发了一个能够解释广义理由模型推理的论证框架扩展版本。

Conclusion: 成功扩展了DSA框架，为处理不一致先例的广义理由模型提供了论证性解释能力，填补了该领域的方法空白。
...
### [197] [Continual Knowledge Adaptation for Reinforcement Learning](https://arxiv.org/abs/2510.19314)
...
Result: 在三个基准测试中，CKA-RL优于现有最先进方法，整体性能提升4.20%，前向迁移提升8.02%。
...
### [198] [NeSyPr: Neurosymbolic Proceduralization For Efficient Embodied Reasoning](https://arxiv.org/abs/2510.19429)
*Wonje Choi,Jooyoung Kim,Honguk Woo*

Main category: cs.AI

TL;DR: NeSyPr是一个新颖的具身推理框架，通过神经符号过程化将知识编译为可组合的过程表示，使基于语言模型的智能体能够在动态环境中进行结构化、自适应和及时推理，无需依赖外部符号规划器。


<details>
  <summary>Details</summary>
Motivation: 解决在动态环境中采用语言模型进行具身任务时面临的挑战，包括大规模推理引擎或符号规划器的在线访问受限（由于延迟、连接性和资源限制）。

Method: 首先使用符号工具生成任务特定计划，然后将这些计划转换为编码计划隐含产生规则的可组合过程表示，将这些组合过程无缝集成到语言模型的推理过程中。

Result: 在PDDLGym、VirtualHome和ALFWorld等具身基准测试中，NeSyPr展示了比大规模推理模型和符号规划器更高效的推理能力，同时使用更紧凑的语言模型。

Conclusion: 神经符号过程化能够将多步符号结构化路径查找和推理抽象并泛化为单步语言模型推理，类似于人类知识编译，适合在延迟敏感和资源受限的物理系统中部署。
...
### [199] [AgentSense: LLMs Empower Generalizable and Explainable Web-Based Participatory Urban Sensing](https://arxiv.org/abs/2510.19661)
*Xusen Guo,Mingxing Peng,Xixuan Hao,Xingchen Zou,Qiongyan Wang,Sijie Ruan,Yuxuan Liang*

Main category: cs.AI

TL;DR: AgentSense是一个基于大语言模型的多智能体演化框架，用于提升参与式城市感知系统的适应性和可解释性，通过迭代优化任务分配来适应动态城市条件和异构工作者偏好。


<details>
  <summary>Details</summary>
Motivation: 现有城市感知系统在多样化城市场景中泛化能力有限，决策过程缺乏可解释性，需要开发更适应动态环境和更透明的解决方案。

Method: 采用混合训练免费框架，结合经典规划器生成基线解决方案，然后通过多智能体演化系统迭代优化，使感知任务分配适应动态城市条件和异构工作者偏好，同时生成自然语言解释。

Result: 在两大移动数据集和七种动态干扰上的实验显示，AgentSense在适应性和可解释性方面优于传统方法，相比单智能体LLM基线在性能和鲁棒性上表现更优，提供更合理透明的解释。
...
### [200] [A Graph Engine for Guitar Chord-Tone Soloing Education](https://arxiv.org/abs/2510.19666)
*Matthew Keating,Michael Casey*

Main category: cs.AI

TL;DR: 开发了一个基于图的引擎，为吉他学生提供和弦音独奏建议，通过构建和弦音琶音图并计算最优路径来生成和弦音独奏线。


<details>
  <summary>Details</summary>
Motivation: 和弦音独奏是即兴演奏的基础练习，但学习和练习难度较大，需要为吉他学生提供有效的学习工具。

Method: 首先生成和弦音琶音，构建加权图（节点代表和弦琶音），计算连续和弦节点间的最优过渡音边权重，寻找最短路径并重构和弦音独奏线。

Result: 开发了一个用户友好的系统来处理引擎的输入输出，帮助吉他学生练习和弦音独奏。
...
### [201] [Explainable e-sports win prediction through Machine Learning classification in streaming](https://arxiv.org/abs/2510.19671)
*Silvia García-Méndez,Francisco de Arriba-Pérez*

Main category: cs.AI

TL;DR: 该论文提出了一种可解释的电子竞技流式胜率预测分类解决方案，通过在多个滑动窗口上控制输入数据来反映相关游戏变化，实验准确率超过90%，超越了现有解决方案。


<details>
  <summary>Details</summary>
Motivation: 电子竞技观众和玩家数量的增长，以及优化通信解决方案和云计算技术的发展，推动了在线游戏行业的持续增长。尽管基于AI的电子竞技分析传统上被定义为从相关数据中提取有意义模式并进行可视化以增强决策，但专业胜率预测的大部分工作都集中在批处理的分类方面，同时忽略了可视化技术。

Method: 提出了一种可解释的胜率预测分类解决方案，在流式处理中通过多个滑动窗口控制输入数据以反映相关游戏变化。

Result: 实验结果显示准确率超过90%，超越了文献中竞争解决方案的性能。

Conclusion: 该系统可被排名和推荐系统利用进行明智决策，得益于可解释性模块，该模块促进了对结果预测的信任。
...
### [202] [RLIE: Rule Generation with Logistic Regression, Iterative Refinement, and Evaluation for Large Language Models](https://arxiv.org/abs/2510.19698)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的方法忽视了规则间的相互作用，且未充分探索将LLM与概率规则学习结合进行稳健推理的机会。

Method: RLIE框架包含四个阶段：1) LLM生成和筛选候选规则；2) 逻辑回归学习概率权重进行全局选择和校准；3) 基于预测误差迭代优化规则集；4) 评估加权规则集作为直接分类器的性能。

Result: 在真实数据集上的评估显示，直接使用学习到的权重应用规则可获得优越性能，而将规则、权重和逻辑模型输出注入LLM反而会降低准确性。
...
TL;DR: 提出Memo架构，一种基于transformer的强化学习方法，通过引入记忆创建和检索机制来解决长时程任务中的上下文限制问题。


<details>
  <summary>Details</summary>
Motivation: 现有transformer策略在具身决策任务中面临视觉输入超出上下文限制的问题，而人类能够通过记忆压缩和利用终身经验来保持环境上下文。

Method: Memo在训练期间通过定期插入总结标记来整合记忆创建和检索，构建transformer架构和训练方法。

Result: 在网格世界元强化学习基准和真实室内多目标导航任务中，Memo优于简单的长上下文transformer基线，同时计算和存储效率更高。

Conclusion: Memo在推理时能更好地泛化到更长上下文，并在流式设置中保持鲁棒性，其中历史上下文必须被截断以适应推理约束。
...
### [204] [Beyond Reactivity: Measuring Proactive Problem Solving in LLM Agents](https://arxiv.org/abs/2510.19771)
*Gil Pasternak,Dheeraj Rajagopal,Julia White,Dhruv Atreja,Matthew Thomas,George Hurn-Maloney,Ash Lewis*

Main category: cs.AI

TL;DR: PROBE是一个评估LLM智能体主动性的新基准，将主动性分解为三个核心能力：搜索未指定问题、识别具体瓶颈和执行适当解决方案。测试显示即使最先进的模型也难以解决该基准，最佳端到端性能仅为40%。


<details>
  <summary>Details</summary>
Motivation: 当前评估主动性的基准局限于局部上下文，无法测试跨来源和长时间跨度的推理能力。需要一个新的评估框架来填补这一空白。

Method: 提出PROBE基准，将主动性分解为三个核心能力管道：(1)搜索未指定问题，(2)识别具体瓶颈，(3)执行适当解决方案。应用该基准评估领先的LLM和流行智能体框架。

Result: 测试结果显示，即使最先进的模型也难以解决该基准，GPT-5和Claude Opus-4.1实现了40%的最佳端到端性能。分析了各模型的相对能力和共同的失败模式。
...
### [205] [Benchmarking World-Model Learning](https://arxiv.org/abs/2510.19788)
...
Method: 提出WorldTest协议，包含无奖励探索、衍生测试和行为评分。实例化为AutumnBench，包含43个交互式网格世界环境和129个任务，涵盖三个任务族：掩码帧预测、规划和因果动态变化预测。

Result: 比较了517名人类参与者和三个前沿模型，发现人类表现优于模型，扩展计算只在某些环境中提高性能而非所有环境。

Conclusion: WorldTest提供了一个新颖模板来评估智能体对环境动态的学习，AutumnBench揭示了世界模型学习的显著提升空间。
...
### [206] [AI Signal Processing Paradigm for Movable Antenna: From Geometric Optimization to Electromagnetic Reconfigurability](https://arxiv.org/abs/2510.19209)
*Yining Li,Ziwei Wan,Chongjia Sun,Kaijun Feng,Keke Ying,Wenyan Ma,Lipeng Zhu,Xiaodan Shao,Zhenyu Xiao,Zhen Gao*

Main category: eess.SP

TL;DR: 本文综述了6G无线通信系统中几何可移动天线(GMA)和电磁可重构天线(ERA)的集成技术，提出了可移动可重构天线(MARA)的统一建模框架，并系统分析了基于人工智能的信号处理方法在高维混合优化中的优势。


<details>
  <summary>Details</summary>
Motivation: 传统固定天线在6G智能可重构通信系统中的局限性日益突出，GMA和ERA作为突破这一瓶颈的关键技术，其集成的"几何-电磁双重构"范式给信号处理带来了高维混合优化的严峻挑战。

Method: 首次集成GMA的几何优化和ERA的电磁重构，提出MARA的统一建模框架，研究GMA、ERA和MARA的信道建模与频谱效率优化，并系统综述基于人工智能的解决方案。

Result: 填补了现有文献在几何-电磁双重构下AI驱动信号处理范式缺乏全面综述的空白，为高频谱效率和灵活性的6G无线系统设计与优化提供了理论支持。

Conclusion: AI方法在高维非凸优化计算中相比传统算法具有显著优势，能够有效解决几何-电磁双重构带来的信号处理挑战，推动6G无线通信系统向智能化和高可重构性发展。
...
### [207] [Generalized Modified Blake-Zisserman Robust Spline Adaptive Filter for Generalized Gaussian Noise](https://arxiv.org/abs/2510.19256)
*Haiquan Zhao,Bei Xu*

Main category: eess.SP

TL;DR: 提出了一种广义修正Blake-Zisserman鲁棒样条自适应滤波(SAF-GMBZ)算法，解决了传统SAF算法在广义高斯噪声和脉冲噪声环境下的性能下降问题，并在主动噪声控制应用中开发了FcGMBZ算法。


<details>
  <summary>Details</summary>
Motivation: 现有的SAF算法在广义高斯噪声环境下性能下降，在脉冲噪声下存在显著的稳态失准，且未能有效处理异常值的不利影响。

Method: 提出了SAF-GMBZ算法，并基于该算法进一步开发了FcGMBZ算法用于主动噪声控制应用。通过引入常用假设计算了步长的均值收敛范围和稳态均方误差。

Result: 仿真结果验证了理论稳态均方误差的准确性，SAF-GMBZ算法在广义高斯噪声环境下的非线性系统辨识中表现出优越性，FcGMBZ算法在脉冲噪声环境下的主动噪声控制应用中有效。

Conclusion: SAF-GMBZ算法在广义高斯噪声环境下具有优越的学习性能，FcGMBZ算法在主动噪声控制应用中表现出良好的收敛精度和噪声消除能力。
...
### [208] [A Study on Delay Assessment for Heterogenous Traffic in VANET](https://arxiv.org/abs/2510.19267)
*Shama Siddiqu,Indrakshi Dey*

Main category: eess.SP

TL;DR: 本文比较了VANET中的802.11p标准协议与FROG-MAC协议的性能。802.11p使用EDCA机制优先处理安全关键消息，但当非关键消息开始传输时，关键数据节点仍需等待。FROG-MAC通过将普通数据包分段传输并在间隙中允许紧急数据包访问信道，显著减少了延迟。


<details>
  <summary>Details</summary>
Motivation: VANET中的多优先级异构节点产生不同类型的数据，需要基于优先级提供差异化服务。现有802.11p协议在非关键消息传输时会阻塞紧急消息的访问，需要改进MAC协议来更好地处理数据优先级。

Method: 通过仿真评估802.11p和FROG-MAC协议在高优先级和低优先级数据下的延迟和吞吐量性能。FROG-MAC采用将普通数据包分段传输并在间隙中允许紧急数据包访问信道的机制。

Result: 仿真结果显示，FROG-MAC在延迟和吞吐量两个性能参数上都优于802.11p协议，因为它为紧急流量提供了更早的信道访问机会。

Conclusion: FROG-MAC协议通过分段传输机制有效减少了VANET中紧急数据包的访问延迟，在数据优先级管理方面优于标准的802.11p协议。
...
### [209] [IoT-Enabled Sleep Monitoring and Cognitive Assessment for Evaluating Teacher Well-Being](https://arxiv.org/abs/2510.19269)
*Anwar Ahmed Khan,Shama Siddiqui,Mehar Ullah,Indrakshi Dey*

Main category: eess.SP

TL;DR: 本研究使用物联网技术和智能手表监测巴基斯坦高中教师的睡眠质量，结合认知评估问卷分析睡眠质量与认知功能的关系，发现大多数教师睡眠质量和认知功能较差，两者存在明显关联。


<details>
  <summary>Details</summary>
Motivation: 高中教师因工作压力大和多任务要求，常面临睡眠质量和认知功能问题，这对教学能力产生负面影响，因此需要研究改善教师福祉的方法。

Method: 使用嵌入脉搏率和血氧传感器的智能手表收集数据，将睡眠质量分为"差"、"一般"或"好"三个等级，同时使用认知评估问卷进行教师认知功能的自我评估。研究覆盖巴基斯坦208名高中教师。

Result: 研究发现大多数教师睡眠质量和认知功能较差，睡眠质量与认知功能之间存在明显关联。

Conclusion: 教师的工作负荷和其他因素需要改善，以确保他们的福祉，这将反过来对他们的教学质量产生积极影响。
...
### [210] [Multi-code rate Task-Oriented Communication for Multi-Edge Cooperative Inference](https://arxiv.org/abs/2510.19360)
*Dongwon Kim,Jiwan Seo,Joonhyuk Kang*

Main category: eess.SP

TL;DR: 提出了一种动态调整特征提取码率的框架，通过速率自适应量化方案和动态规划方法，在有限带宽下优化多边缘协作推理系统的通信效率和推理性能。


<details>
  <summary>Details</summary>
Motivation: 物联网与人工智能融合的多边缘协作推理系统中，同时传输所有设备的固定压缩比特征会导致通信资源利用效率低下，同时面临隐私问题和有限通信带宽的挑战。

Method: 采用速率自适应量化方案动态调整特征提取码率，基于特征对下游推理任务的重要性；利用动态规划方法在离散码率选项间分配码率，满足有限带宽约束。

Result: 在多视图数据集上的实验表明，所提框架显著优于使用固定速率量化的框架，在有限带宽条件下实现了通信效率和推理性能的良好平衡。

Conclusion: 该框架通过动态码率调整和优化分配，有效解决了多边缘协作推理系统中的通信效率问题，为AI驱动的任务提供了更高效的解决方案。
...
### [211] [Ray-Tracing Based Narrow-Beam Channel Simulation, Characterization and Performance Evaluation for 5G-R Systems](https://arxiv.org/abs/2510.19401)
*Tao Zhou,Liying Geng,Yiqun Liang,Kaifeng Bao,Tianyun Feng,Liu Liu,Bo Ai*

Main category: eess.SP

TL;DR: 本文基于射线追踪仿真研究了5G-R系统的窄波束信道特性和性能评估，分析了三种高铁场景下的信道特征和系统性能。


<details>
  <summary>Details</summary>
Motivation: 研究5G-R系统在高铁环境下的窄波束信道特性，为高铁5G系统设计和优化提供指导。

Method: 建立三种高铁场景模型，采用射线追踪进行动态窄波束信道仿真，使用波束跟踪方案确保与移动列车的连续对准，并开发硬件在环仿真平台。

Result: 分析了路径损耗、阴影衰落、衰落严重性、时频空色散和平稳区间等信道特性，评估了误块率、吞吐量、频谱效率等系统性能指标。

Conclusion: 结果为高铁环境下5G-R系统的设计和优化提供了有价值的指导。
...
### [212] [A Novel Delay-Doppler Domain Channel Sounding Method for 6G High-Mobility Scenarios](https://arxiv.org/abs/2510.19402)
*Kaifeng Bao,Tao Zhou,Chaoyi Li,Liu Liu,Bo Ai*

Main category: eess.SP

TL;DR: 本文提出了一种针对6G高速移动场景的延迟-多普勒域信道探测方法，包括波形设计、同步和信道扩展函数估计，并通过城市环境中的车对基础设施场景测量验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 在6G系统中，传统时域或频域信道探测方法无法直接获取高速移动场景引起的多普勒信息，而延迟-多普勒域信道探测方法尚未得到充分探索。

Method: 设计了探测信号的波形并分析其探测能力，详细描述了延迟-多普勒域信道探测的方法论，包括同步和信道扩展函数估计，并提出了提高测量精度的算法。

Result: 建立了一个适用于6G高速移动场景的延迟-多普勒域信道探测系统，在城市环境中进行了车对基础设施场景的测量，获得了信道扩展函数、功率延迟分布、多普勒功率谱密度、多径分量数量等特征。

Conclusion: 测量结果证实了所提方法的有效性，为推进6G高速移动通信研究提供了有益见解。
...
### [213] [On the Robustness of AFDM and OTFS Against Passive Eavesdroppers](https://arxiv.org/abs/2510.19525)
*Vincent Savaux,Hyeon Seok Rou,Zeping Sui,Giuseppe Thadeu Freitas de Abreu,Zilong Liu*

Main category: eess.SP

TL;DR: 该论文比较了AFDM和OTFS波形在被动窃听者进行暴力解调时的安全性，分析表明AFDM具有更好的抗窃听能力。


<details>
  <summary>Details</summary>
Motivation: 研究AFDM和OTFS波形在面对不知道啁啾参数（AFDM）或延迟多普勒网格配置（OTFS）的被动窃听者时的安全性，评估两种波形在暴力解调攻击下的鲁棒性。

Method: 通过分析窃听者暴力解调的复杂度，并对比AFDM和OTFS在相同条件下的比特误码率（BER）性能，使用数学分析和仿真验证。

Result: 分析结果显示，OTFS的暴力解调复杂度为O(√N)，而AFDM为O(N²)，其中N是子载波数量。BER仿真表明AFDM信号在窃听端几乎不可解码，而OTFS在同等条件下允许部分信号恢复。

Conclusion: AFDM在对抗被动窃听者的暴力解调攻击方面比OTFS具有更强的安全性，复杂度更高使得窃听者更难成功解调信号。
...
### [214] [Micro-Doppler Energy-Based Robust Multi-Target Vital Signs Monitoring Using 77-GHz FMCW Radar with Spatiotemporal Adaptive Processing](https://arxiv.org/abs/2510.19639)
*Chenxing Tan,Yuguan Hou,Hao Wang,Zhonghao Yuan*

Main category: eess.SP

TL;DR: 提出基于微多普勒能量的77GHz FMCW雷达多目标生命体征监测框架，相比传统相位方法具有更好的抗噪性和鲁棒性，可同时监测4个目标的呼吸和心率。
...
Method: 集成空间-时间自适应处理(STAP)进行目标检测跟踪，MUSIC算法进行高分辨率角度估计，创新自适应谱滤波技术提取生命体征，建立微多普勒能量变化与生理活动的数学关系。

Result: 在5米范围内准确检测和分离4个目标的生命体征，呼吸率和心率的平均绝对误差分别为1.2次/分钟和2.3次/分钟，性能优于传统相位方法。

Conclusion: 微多普勒能量方法在相位噪声和运动伪影方面具有固有鲁棒性，在多目标场景下表现出优越性能。
...
### [215] [Interpretable machine learning for cardiogram-based biometrics](https://arxiv.org/abs/2510.19775)
...
TL;DR: 该研究评估了心电图(ECG)和阻抗心动图(ICG)特征在生物识别中的表现，发现QRS相关ECG特征在情绪变化下具有稳健的识别能力，而ICG特征提供补充但稳定性较低。通过特征选择和机器学习方法，实现了99%的识别准确率。


<details>
  <summary>Details</summary>
Motivation: 研究ECG和ICG特征在生物识别中的判别能力和对情绪变化的稳健性，探索这些生理信号作为身份识别工具的潜力。

Method: 使用随机森林模型结合多种可解释性方法，评估29个跨四个领域（时间、幅度、斜率、形态）的特征，采用特征重要性分析、相关性分析和特征选择方法。

Result: ECG特征特别是QRS相关描述符在特征重要性排名中最高，ICG BCX特征提供补充但稳定性较低。特征选择得到14个特征的子集，准确率达到99%，与完整特征集相当。

Conclusion: 可靠的生物识别个体性主要编码在QRS相关的ECG特征中，这些特征对情绪变化具有韧性，可能源于心室质量、传导通路和胸廓几何的个体差异。
...
### [216] [AI in Proton Therapy Treatment Planning: A Review](https://arxiv.org/abs/2510.19213)
...
TL;DR: 本文综述了人工智能在质子治疗计划中的作用，AI在图像重建、配准、剂量计算、计划优化和质量评估等方面显示出潜力，能够提高效率、减少人工工作量，支持个性化和适应性计划。


<details>
  <summary>Details</summary>
Motivation: 质子治疗相比光子治疗具有更好的剂量适形性，但其治疗计划面临解剖结构变化、设置/范围不确定性和计算复杂性的挑战。本文旨在评估人工智能如何改善质子治疗计划。

Method: 通过回顾和总结近期关于AI在图像重建、图像配准、剂量计算、计划优化和质量评估等应用领域的研究，按应用领域和验证策略进行分类分析。

Result: AI在自动勾画轮廓、增强剂量计算成像、预测剂量分布和加速鲁棒优化方面显示出潜力。这些方法减少了人工工作量，提高了效率，支持更个性化的计划和适应性计划。

Conclusion: AI正在成为高效、一致和患者特异性质子治疗计划的关键推动者。解决验证和实施中的挑战对于将其转化为常规临床实践至关重要。

Abstract: Purpose: Proton therapy provides superior dose conformity compared to photon
