...
TL;DR: 本文展示了如何使用持久同调（PH）这一拓扑数据分析方法来客观区分艺术风格，包括不同艺术流派艺术家、同一流派内艺术家，以及真实艺术家作品与AI生成作品的区别。


<details>
  <summary>Details</summary>
Motivation: 传统艺术风格识别依赖艺术史学家或评论家的主观视觉直觉和经验，而数学工具可以提供更结构化的分析视角。

Method: 使用持久同调（PH）这一拓扑数据分析方法对艺术作品进行分析。

Result: PH方法能够以统计确定性区分不同艺术流派的艺术家、同一流派内的艺术家，并能区分真实艺术家作品与AI生成的模仿风格作品。

Conclusion: 持久同调为艺术风格分析提供了客观且可解释的见解，能够有效识别艺术作品的风格特征。
...
TL;DR: 提出了一种使用StyleGAN2生成猕猴面部表情的方法，通过数据增强、样本选择和损失函数优化来解决训练数据有限的问题，能够生成多样化的面部表情并用于基于风格的图像编辑。


<details>
  <summary>Details</summary>
Motivation: 生成动物面部表情面临训练图像数量有限和变化不足的挑战，特别是在不同个体的面部表情方面。本研究专注于在系统神经科学和进化研究中广泛研究的猕猴。
...
TL;DR: RacketVision是一个用于体育分析的新型计算机视觉数据集和基准，涵盖乒乓球、网球和羽毛球，提供大规模细粒度球拍姿态标注和传统球位置信息，支持复杂人机交互研究。


<details>
  <summary>Details</summary>
Motivation: 推动体育分析中的计算机视觉发展，解决现有数据集缺乏细粒度球拍姿态标注的问题，支持动态物体追踪、条件运动预测和多模态分析研究。

Method: 构建包含乒乓球、网球和羽毛球的大规模数据集，提供球拍姿态和球位置的细粒度标注，设计交叉注意力机制进行多模态融合，评估三个相互关联的任务：细粒度球追踪、关节球拍姿态估计和预测性球轨迹预测。

Result: 评估发现，简单拼接球拍姿态特征会降低性能，而交叉注意力机制能有效利用这些特征，使轨迹预测结果超越强单模态基线。

Conclusion: RacketVision为动态物体追踪、条件运动预测和体育多模态分析提供了多功能资源和强起点，交叉注意力机制是多模态融合的关键。
...
TL;DR: 本文提出了PairHuman数据集，这是首个专为高质量双人肖像生成设计的大规模基准数据集，包含10万+图像，并开发了DHumanDiff基线方法用于双人肖像生成。


<details>
  <summary>Details</summary>
Motivation: 个性化双人肖像定制具有重要应用价值，但缺乏基准数据集阻碍了高质量定制的发展。

Method: 构建包含10万+图像的PairHuman数据集，涵盖多种场景、服装和互动；开发DHumanDiff基线方法，增强面部一致性并平衡个性化人物生成与语义驱动场景创建。

Result: 实验结果表明，所提出的数据集和方法能够生成高度定制化、视觉质量优越且符合人类偏好的肖像。

Conclusion: PairHuman数据集和DHumanDiff方法为高质量双人肖像生成提供了有效解决方案，数据集已公开可用。
...
TL;DR: 本文提出了一种基于无监督自动编码器和CDF 97小波变换的混合高斯-泊松去噪方法，用于中子成像数据去噪，相比传统方法具有更低的重建误差和更好的边缘保持能力。


<details>
  <summary>Details</summary>
Motivation: 中子成像在惯性约束聚变分析中至关重要，但图像常被高斯和泊松噪声共同退化，传统滤波和阈值方法难以有效处理这种混合噪声，且缺乏真实数据限制了机器学习方法的应用。

Method: 使用无监督自动编码器，在潜在空间中集成Cohen-Daubechies-Feauveau (CDF 97)小波变换，专门针对混合高斯-泊松噪声进行去噪。
...
TL;DR: 本文提出SaFeR-CLIP框架，通过将不安全概念重定向到语义上最接近的安全替代概念，在保持模型安全性的同时显著提升泛化性能，相比现有方法恢复高达8.0%的零样本准确率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）通过微调提升安全性时往往导致泛化性能显著下降，这种权衡源于僵化的对齐策略破坏了模型学习到的语义结构。

Method: 提出邻近感知方法，将不安全概念重定向到语义上最接近的安全替代概念，以最小化表示变化。开发SaFeR-CLIP微调框架，应用最小干预原则。

Result: SaFeR-CLIP成功平衡了安全性和性能，相比现有方法恢复高达8.0%的零样本准确率，同时保持强大的安全性。还贡献了NSFW-Caps基准数据集，包含1000个高度对齐的测试对。
...
TL;DR: 本文提出了一种三阶段框架，可从单视图SVG输入生成多视图一致的SVG，具有几何和颜色一致性，显著减少冗余路径并保留细节结构。


<details>
  <summary>Details</summary>
Motivation: 解决单对象SVG从单视图输入生成多视图一致SVG的未探索问题，连接生成建模与结构化向量表示，支持资产创建和语义向量编辑应用。

Method: 三阶段框架：1）将栅格化输入提升为3D表示并渲染多视图图像；2）扩展SAM2时间记忆机制到空间域，构建空间记忆库建立跨视图部件级对应关系；3）在栅格到向量转换中执行路径整合和结构优化。

Result: 生成的SVG在多个视图间表现出强大的几何和颜色一致性，显著减少冗余路径，同时保留精细结构细节。

Conclusion: 该工作为单输入、对象级多视图SVG生成提供了可扩展路径，连接了生成建模与结构化向量表示，支持资产创建和语义向量编辑应用。
...
TL;DR: Mesh RAG是一个无需训练、即插即用的框架，通过检索增强生成方法改进自回归3D网格生成模型，解决了传统方法的顺序依赖问题，提升了生成质量和速度，并支持增量编辑。


<details>
  <summary>Details</summary>
Motivation: 传统3D网格制作依赖人工艺术家，耗时且难以扩展。现有自回归模型存在质量与速度的权衡问题，顺序依赖限制了生成效率和编辑灵活性。

Method: 受语言模型RAG启发，Mesh RAG利用点云分割、空间变换和点云配准技术来检索、生成和集成网格组件，解耦了生成过程的顺序依赖。

Result: Mesh RAG在多种基础自回归网格生成模型上显著提升了网格质量，加速了生成速度（相比顺序部件预测），并实现了增量编辑功能，且无需模型重新训练。

Conclusion: Mesh RAG框架有效解决了自回归网格生成中的顺序依赖问题，在保持高质量的同时实现了并行化推理和灵活的编辑能力，具有广泛适用性。
...
TL;DR: WorldGen是一个从文本提示自动创建大规模交互式3D世界的系统，通过结合LLM场景布局推理、程序生成和扩散式3D生成等技术，将自然语言描述转化为可探索的完整纹理环境。


<details>
  <summary>Details</summary>
Motivation: 弥合创意意图与功能虚拟空间之间的差距，让创作者无需手动建模或专业3D技能即可设计连贯、可导航的世界，推动可访问的大规模生成式世界构建。

Method: 结合LLM驱动的场景布局推理、程序生成、基于扩散的3D生成和对象感知场景分解，构建模块化系统，支持对布局、规模和风格的细粒度控制。

Result: 系统能够生成几何一致、视觉丰富且实时渲染效率高的世界，可直接在标准游戏引擎中探索或编辑，实现了从文本到可交互3D环境的自动转换。
...
Method: 通过混合人机协作流程生成和验证结构化文本标注，整合配准的Sentinel-2和ALOS-2影像数据。

Result: 对通用视觉语言模型的全面评估显示当前模型在数值推理方面存在困难，揭示了科学视觉语言模型的关键挑战。
...
TL;DR: 本文提出了BOP-ASK数据集，用于评估和训练视觉语言模型在物体交互推理方面的能力，包含超过15万张图像和3300万个问答对，涵盖6个任务。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在空间推理基准测试中表现良好，但忽视了现实应用所需的细粒度空间理解能力，如精确3D定位、物体物理兼容性、物体可供性和多步空间规划。

Method: 利用BOP数据集的6D物体姿态，生成细粒度标注，包括抓取姿态、参考物体姿态、路径规划轨迹、相对空间和深度关系以及物体间关系。

Result: 在BOP-ASK上训练的模型优于基线，并展现出精确物体和抓取姿态估计、轨迹规划以及在杂乱环境中进行细粒度物体中心空间推理的新兴能力。

Conclusion: BOP-ASK为训练和评估视觉语言模型提供了丰富资源，展示了在物体交互推理方面的显著改进，并将公开发布数据集和生成流程。
...
TL;DR: 提出Parts-Mamba模型，一种混合GCN-Mamba架构，用于解决骨架动作识别中因遮挡或通信质量差导致的骨架不完整问题，通过捕获远距离关节的上下文信息，在NTU RGB+D数据集上实现最高12.9%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 现实场景中捕获的骨架数据常因人体部分遮挡或通信质量问题而不完整，现有GCN模型在这种非理想情况下表现不佳，主要原因是缺乏对远距离关节上下文信息的有效捕获。

Method: 提出Parts-Mamba混合模型，结合GCN和Mamba架构，通过部件特定扫描特征捕获部件特定信息，并通过部件-身体融合模块保持非邻近关节的上下文信息。

Result: 在NTU RGB+D 60和NTU RGB+D 120数据集的不同遮挡设置下进行评估，相比现有方法实现了最高12.9%的准确率提升。

Conclusion: Parts-Mamba模型通过有效捕获和保持远距离关节的上下文信息，显著提升了在骨架不完整情况下的动作识别性能。
...
TL;DR: 本文提出了联合Gromov-Wasserstein（JGW）目标，将传统的GW距离扩展到支持多对多对象匹配，提供了部分同构分布的非负差异度量，并在合成和真实数据集上展示了优越性能。
...
Method: 引入JGW目标，扩展GW框架以支持对象集合的同步匹配，采用最优传输算法（包括熵正则化）进行点云对象表示的求解。

Result: 在部分匹配的基准测试中，该方法在准确性和计算效率方面优于其他GW变体，在合成和真实数据集（包括几何形状和生物分子复合物）上有效实现了多形状匹配。
...
TL;DR: 该论文提出了一种用于逆问题的表示对齐方法（REPA），通过在扩散或流式生成模型与预训练自监督编码器之间进行表示对齐，显著提升了重建质量和感知真实感。


<details>
  <summary>Details</summary>
Motivation: 在逆问题中，预训练的生成模型被用作先验，但缺乏地面真实信号。作者希望利用预训练自监督视觉编码器的表示能力来指导重建过程，提高重建保真度和感知质量。

Method: 提出REPA方法，在推理时对齐扩散或流式模型与预训练自监督编码器（如DINOv2）的内部表示，通过表示对齐正则化来引导重建过程。

Result: 实验表明REPA方法在超分辨率、框内修复、高斯去模糊和运动去模糊等任务中一致提升了重建质量，同时通过减少离散化步骤数量实现了效率提升。

Conclusion: REPA方法为逆问题提供了一种有效的表示对齐机制，能够显著改善重建的感知保真度，并且具有通用性，可集成到多种最先进的逆问题求解器中。
...
TL;DR: 提出NFGlassNet方法，利用闪光/非闪光图像中的反射动态特性进行玻璃表面检测，通过反射对比挖掘模块和反射引导注意力模块实现准确检测，并在自建数据集上验证了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有玻璃表面检测方法依赖边界线索或反射线索，但未能充分利用玻璃本身的内在特性。观察到现实场景中玻璃两侧光照强度差异导致反射变化的现象，利用这一特性改进检测精度。

Method: 提出NFGlassNet方法，包含反射对比挖掘模块(RCMM)用于提取反射特征，反射引导注意力模块(RGAM)用于融合反射和玻璃表面特征。构建包含3.3K对闪光/非闪光图像的数据集进行训练。

Result: 大量实验表明，该方法在玻璃表面检测任务上优于现有最先进方法。

Conclusion: 利用闪光/非闪光图像中的反射动态特性能够有效提升玻璃表面检测的准确性，所提出的NFGlassNet方法在多个场景下都表现出优越性能。
...
TL;DR: 本文提出了R-AVST数据集和AVST-Zero模型，用于解决真实世界音频-视觉时空推理任务。R-AVST包含5K未剪辑视频和27K个对象，涵盖100种音频-视觉事件类型，并定义了三个核心推理任务。AVST-Zero采用强化学习方法，通过多维奖励直接优化模型行为。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频理解任务中进展迅速，但主要关注简单视频场景，无法反映真实世界音频-视觉事件的复杂多样性。需要构建专门的数据集和模型来填补这一空白。

Method: 1) 构建R-AVST数据集：使用基于LLM的关键对象提取、自动空间标注和人工质量检查的流程；2) 提出AVST-Zero模型：基于强化学习，避免中间监督，通过精心设计的多维奖励直接优化行为。

Result: R-AVST数据集包含5K个未剪辑视频、27K个对象和8K多个高质量问答对。实验验证了R-AVST在推进音频-视觉时空推理方面的有效性，AVST-Zero相比现有模型表现出竞争力。
...
TL;DR: 本文提出了Warm Diffusion，一种结合模糊和噪声的混合扩散模型，通过控制模糊-噪声比来整合热扩散和冷扩散的优势，解决了热扩散忽略图像相关性而冷扩散忽略噪声作用的问题。


<details>
  <summary>Details</summary>
Motivation: 热扩散完全依赖噪声，忽略了高频细节与低频结构的相关性；冷扩散仅使用模糊，忽视了噪声在数据流形形成中的作用。需要整合两者的优势。

Method: 提出Warm Diffusion（BNMD模型），采用分治策略控制模糊和噪声的联合作用，通过频谱分析研究模糊-噪声比（BNR）在模型学习动态和数据流形变化之间的权衡。

Result: 在多个基准测试上的广泛实验验证了该方法在图像生成任务中的有效性。

Conclusion: Warm Diffusion成功整合了热扩散和冷扩散的优势，通过模糊-噪声混合扩散模型实现了更好的图像生成性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统的异常生成方法需要少量异常样本进行微调，这与异常稀缺的实际情况相矛盾，且容易过拟合类别先验。本文旨在解决无真实异常样本或训练数据的情况下的异常生成问题。

Method: 提出Delta-Denoising方法，使用最小提示对驱动两个扩散分支进行对比，通过累积每步去噪差异生成图像特定的定位图，指导潜在修复过程，同时保留周围上下文并生成真实的局部缺陷。还包含标记级提示优化和空间注意力偏置等技术。

Result: 在公共数据集上的实验表明，DeltaDeno实现了出色的生成效果、真实性和下游检测性能的一致提升。

Conclusion: DeltaDeno是一种有效的零样本异常生成方法，无需训练数据即可生成逼真的异常样本，并能显著提升下游异常检测任务的性能。
...
TL;DR: 本文提出了一种基于扩散模型的视频超分辨率方法DGAF-VSR，通过光学引导变形模块和特征域时序条件模块，解决了现有方法中的误差累积、空间伪影以及感知质量与保真度之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视频超分辨率方法存在误差累积、空间伪影以及感知质量与保真度之间的权衡问题，主要原因是视频帧之间的对齐不准确和补偿不足。

Method: 提出DGAF-VSR方法，包含光学引导变形模块(OGWM)来保持对齐特征中的高频细节，以及特征域时序条件模块(FTCM)在特征域提供密集指导。

Result: 在合成和真实世界数据集上的广泛实验表明，DGAF-VSR在感知质量(DISTS降低35.82%)、保真度(PSNR增益0.20 dB)和时间一致性(tLPIPS降低30.37%)方面均优于最先进方法。

Conclusion: 特征域比像素域更适合信息补偿，在放大分辨率下进行变形能更好地保留高频信息，但这一优势并非单调递增。DGAF-VSR通过改进对齐和补偿机制，显著提升了视频超分辨率的性能。
...
<details>
  <summary>Details</summary>
Motivation: CBCT图像中的牙齿分割在邻牙粘连情况下存在严重挑战，导致解剖形状失真，需要开发能够保持形状完整性的分割方法。
...
Method: 提出Forward-Backward-Refinement标注流程，结合多方向跟踪和智能错误校正；开发DeepSTG评估框架量化数据集质量；提出PG-TAF训练免费两阶段框架，将STVG分解为高级时间定位和细粒度时空传播。

Result: 在复杂现实场景中性能平均下降10.4%，特别是在小/遮挡对象和复杂空间关系上。PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试上均获得一致增益。

Conclusion: OmniGround基准测试揭示了当前STVG模型的局限性，PG-TAF框架通过分解任务有效提升了时空视频定位性能，为未来研究提供了重要基准和解决方案。
...
TL;DR: 提出了MultiPriv基准，首个系统评估视觉语言模型中个体级隐私推理能力的基准，揭示了现有模型在隐私推理方面的系统性漏洞。


<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型展现出复杂推理能力，带来了超越简单属性感知的个体级隐私风险。现有隐私基准主要评估隐私感知，但无法应对更关键的隐私推理风险——模型推断和链接分布式信息构建个体档案的能力。

Method: 提出了隐私感知与推理框架，构建了新颖的双语多模态数据集，包含合成的个体档案，其中标识符与敏感属性精心链接。设计了9个挑战性任务评估完整的PPR谱系，从属性检测到跨图像重识别和链式推理。

Result: 对50多个基础和商业VLM进行大规模评估发现：(1)许多VLM存在显著但未测量的基于推理的隐私风险；(2)感知级指标无法预测这些推理风险；(3)现有的安全对齐对此类基于推理的攻击不一致且无效。

Conclusion: MultiPriv暴露了系统性漏洞，为开发强大的隐私保护VLM提供了必要框架。
...
TL;DR: 提出了一种联合建模动态图像序列和运动场的隐式神经表示框架，通过光学流方程作为物理约束，无需预估计运动场即可同时恢复时间相干图像和运动场。


<details>
  <summary>Details</summary>
Motivation: 传统运动补偿重建依赖预估计的光学流，但在欠采样情况下不准确，会降低重建质量。需要一种能同时恢复图像和运动场的方法。

Method: 使用两个隐式神经表示：一个参数化时空图像内容，另一个表示光学流。通过光学流方程耦合两者，并加入数据一致性损失。

Result: 在动态心脏MRI数据集上的实验表明，该方法优于最先进的运动补偿和深度学习方法，实现了更好的重建质量、准确的运动估计和改进的时间保真度。

Conclusion: 隐式联合建模与流正则化约束在推进dMRI重建方面具有潜力，能够同时恢复高质量图像和准确运动场。
...
TL;DR: 本文提出了FingerCap任务，用于生成捕捉手部动作细节的手指级语义文本描述，并构建了包含4万对手部动作视频和描述的FingerCap-40K数据集。为解决视频MLLMs在时间稀疏性上的瓶颈，提出了FiGOP方法，通过结合RGB关键帧和手部关键点来恢复精细时间线索。


<details>
  <summary>Details</summary>
Motivation: 理解细粒度人类手部动作对视觉感知、具身智能和多模态通信至关重要。现有方法在捕捉手指级语义方面存在困难，特别是时间稀疏性限制了捕捉细微高频动态的能力。

Method: 提出了FiGOP（手指图像组）方法，将每个RGB关键帧与后续手部关键点配对，直到下一个关键帧。使用轻量级时间编码器将关键点转换为运动嵌入，并与RGB特征集成。FiGOP适应了经典的GOP概念来捕捉手指运动，在不增加RGB密度的情况下恢复精细时间线索。

Result: 在FingerCap-40K数据集上的实验表明，现有的开源和闭源Video-MLLMs在手指级推理方面仍存在困难，而采用FiGOP增强的模型在HandJudge评估和人工研究中均取得了持续改进。

Conclusion: FiGOP方法有效解决了视频MLLMs在捕捉精细手指运动时的时间稀疏性问题，为细粒度手部动作理解提供了有效的解决方案。
...
TL;DR: 本文提出了一种基于点监督的面部表情定位方法，通过双分支框架解决单时间戳标注下的表情检测问题，包括实例自适应强度建模和类别感知顶点分类。


<details>
  <summary>Details</summary>
Motivation: 现有面部表情定位方法主要依赖完整的时间边界标注，成本高昂且耗时。本文研究点监督面部表情定位，只需每个表情实例的单个时间戳标注即可训练，降低标注成本。

Method: 提出双分支框架：1）基于高斯的实例自适应强度建模模块，通过检测伪顶点帧、估计持续时间构建实例级高斯分布进行软伪标注；2）类别感知顶点分类分支，仅基于伪顶点帧区分宏表情和微表情。还引入强度感知对比损失增强特征学习。

Result: 在SAMM-LV、CAS(ME)^2和CAS(ME)^3数据集上的大量实验证明了所提框架的有效性。

Conclusion: 提出的点监督面部表情定位框架通过双分支设计和强度建模，在仅需单时间戳标注的情况下实现了有效的表情检测和分类。
...
TL;DR: MatPedia是一个基于联合RGB-PBR表示的基础模型，通过将材料编码为RGB外观和PBR物理属性的两个相互依赖的潜在变量，使用视频扩散架构统一处理多种材料任务。
...
Method: 提出新颖的联合RGB-PBR表示，将材料编码为RGB外观和四个PBR图的两个相互依赖潜在变量，将其表述为5帧序列并使用视频扩散架构，在MatHybrid-410K混合数据集上训练。

Result: MatPedia实现了原生1024×1024分辨率合成，在质量和多样性方面显著超越现有方法，能够统一处理文本到材料生成、图像到材料生成和本征分解等多种任务。

Conclusion: MatPedia通过联合RGB-PBR表示和视频扩散架构，为材料生成提供了一个统一的基础模型框架，成功桥接了视觉外观和物理属性之间的鸿沟。
...
TL;DR: 提出了一种边缘设备上基于烹饪状态引导的食物图像生成方法，通过厨师标注的熟度级别和烹饪配方指导，从生食图像合成逼真的熟食图像。
...
Method: 构建首个基于烤箱的烹饪进程数据集，提出边缘高效的配方和烹饪状态引导生成器，引入领域特定的烹饪图像相似度(CIS)指标作为训练损失和进度监控信号。

Result: 模型在FID分数上显著优于现有基线方法（在自建数据集上提升30%，在公共数据集上提升60%）。

Conclusion: 该方法能够根据用户偏好的视觉目标而非固定预设生成逼真的食物图像，同时确保时间一致性和烹饪合理性。
...
TL;DR: SeeCLIP是一个解决开放集域泛化问题的框架，通过语义增强的CLIP模型来处理已知类和未知类之间的区分难题，特别针对视觉相似但类别不同的'困难未知类'。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理域偏移和新类别时面临结构风险和开放空间风险的权衡困境，容易产生过度自信，特别是在区分与已知类具有细粒度视觉相似性的'困难未知类'时表现不佳。

Method: 提出语义增强的CLIP框架，包括：语义感知提示增强模块分解图像为判别性语义标记；双工对比学习保持与已知类的分离性和语义邻近性；语义引导扩散模块通过扰动语义标记合成伪未知类样本。

Result: 在五个基准测试上的广泛实验显示，相比最先进方法，准确率提升3%，H-score提升5%。

Conclusion: SeeCLIP通过细粒度语义增强有效解决了开放集域泛化中的已知类和未知类区分难题，在多个基准测试上取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS使用大量高斯基元来拟合场景，导致存储和计算开销巨大。现有修剪方法依赖手动设计标准或引入额外可学习参数，效果不理想。
...
TL;DR: DepthFocus是一个可操控的视觉Transformer模型，将立体深度估计重新定义为意图驱动的控制任务。该模型能够根据深度偏好标量动态调整计算，在复杂场景中实现选择性感知，在传统单深度基准测试和新的多深度数据集上都表现出色。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的深度很少是单一的，透射材料会创建层叠的模糊性，混淆传统感知系统。现有模型是被动的，试图估计锚定到最近表面的静态深度图，而人类会主动转移焦点来感知所需深度。

Method: 引入DepthFocus，一个可操控的视觉Transformer，将立体深度估计重新定义为意图驱动的控制。模型根据标量深度偏好进行条件化，动态调整计算以聚焦于预期深度，在复杂场景中实现选择性感知。训练主要利用新构建的500k多层合成数据集，捕捉多样的透射效果。

Result: DepthFocus不仅在BOOSTER等传统单深度基准测试上达到最先进性能，还在新提出的真实和合成多深度数据集上定量展示了意图对齐的估计。此外，在未见过的透射场景上表现出强大的泛化能力。

Conclusion: DepthFocus作为向主动和类人3D感知迈出的重要一步，展现了其鲁棒性，能够根据用户意图灵活调整深度感知焦点。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中直接替换身份来实现人脸去识别，使用化学中的替换机制作为灵感，提出了线性和测地线编辑方案。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有去识别方法在身份抑制和属性保留之间的平衡问题，作者希望开发一种无需训练的方法，直接在潜在空间中操作身份信息。

Method: 将身份编辑重新解释为预训练无条件扩散模型潜在h空间中的语义位移，通过优化发现身份编辑方向，使用新颖的试剂损失监督属性保留和身份抑制，提出线性和测地线编辑方案来导航潜在流形。

Result: 在CelebA-HQ和FFHQ数据集上的实验结果表明，FLUID在身份抑制和属性保留之间实现了优越的平衡，在定性和定量指标上都优于最先进的去识别方法。

Conclusion: FLUID框架通过直接在扩散模型的潜在空间中操作身份信息，提供了一种高效的去识别解决方案，在保持图像质量的同时有效保护个人隐私。
...
TL;DR: MVLR是一个统一的记忆增强视觉语言恢复模型，用于在多种恶劣天气条件下恢复不同退化程度的图像，通过结合视觉语言模型和隐式记忆库来提升恢复精度和计算效率。


<details>
  <summary>Details</summary>
Motivation: 在恶劣天气条件下（如雨、雾、雪等）实现可靠的视觉感知对于自动驾驶和户外机器人至关重要但具有挑战性，需要开发能够处理多种天气退化类型的统一模型。

Method: MVLR将轻量级编码器-解码器主干与视觉语言模型和隐式记忆库耦合。VLM执行思维链推理来编码天气退化先验，IMB存储退化模式的连续潜在表示，通过动态交叉注意力机制将检索到的退化原型与多尺度视觉特征自适应融合。

Result: 在四个恶劣天气基准测试上的广泛实验表明，MVLR在峰值信噪比和结构相似性指标方面超过了单分支和专家混合基线方法。

Conclusion: MVLR在模型紧凑性和表达能力之间提供了实用的平衡，适用于各种户外条件下的实时部署。
...
TL;DR: 本文介绍了ConfusedTourist，一个评估视觉语言模型在混合文化线索场景下文化对抗鲁棒性的新测试套件，发现现有模型在面对简单图像堆叠扰动时准确率显著下降。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型的文化维度评估通常只关注单一文化概念，忽视了多文化线索共存场景，而模型在这种混合文化输入下的稳定性对支持多元文化社会至关重要。

Method: 开发了ConfusedTourist测试套件，通过图像堆叠和基于图像生成的变体来扰动地理线索，评估模型的文化对抗鲁棒性，并进行可解释性分析。
...
TL;DR: 本文提出了一个统一的行人跟踪框架OmniPT，能够交互式地跟踪、基于参考跟踪并生成跟踪对象的语义理解。通过RL-Mid Training-SFT-RL训练阶段，解决了如何将跟踪任务建模为基础模型可执行任务以及如何使模型输出格式化答案的问题。


<details>
  <summary>Details</summary>
Motivation: 虽然LVLMs在图像级任务中表现出色，但在实例级任务（如视觉定位和目标检测）中仍存在性能差距。行人跟踪作为经典任务，与自然语言结合的新主题（如Referring MOT、Cross-view Referring MOT和Semantic MOT）强调模型应在高级语义层面理解跟踪对象，这正是LVLMs的优势所在。

Method: 提出OmniPT框架，采用RL-Mid Training-SFT-RL训练阶段：基于LVLM预训练权重，首先进行简单RL阶段使模型输出固定可监督的边界框格式；随后使用大量行人相关数据集进行中间训练；最后在多个行人跟踪数据集上进行监督微调，再进行另一个RL阶段以提升跟踪性能和指令跟随能力。

Result: 在跟踪基准测试上的实验结果表明，所提方法能够比先前方法表现更好。

Conclusion: OmniPT框架成功地将行人跟踪任务建模为基础模型可执行的任务，并通过多阶段训练策略实现了优于先前方法的性能，特别是在语义理解方面展现了LVLMs的优势。
...
TL;DR: ReBrain是一个基于检索增强扩散框架的脑部MRI重建方法，通过从CT扫描合成MRI，特别针对低剂量协议导致的稀疏CT体积进行优化。


<details>
  <summary>Details</summary>
Motivation: 某些患者由于身体或临床限制无法进行MRI扫描，而现有的CT到MRI合成方法在处理稀疏CT数据时效果不佳，需要更准确的重建方法。
...
Conclusion: ReBrain框架能够有效解决稀疏CT数据下脑部MRI重建的挑战，为无法进行MRI扫描的患者提供可行的替代方案。
...
### [36] [Spanning Tree Autoregressive Visual Generation](https://arxiv.org/abs/2511.17089)
*Sangkyu Lee,Changho Lee,Janghoon Han,Hosung Song,Tackgeun You,Hwasup Lim,Stanley Jungkyu Choi,Honglak Lee,Youngjae Yu*

Main category: cs.CV

TL;DR: STAR模型通过使用均匀生成树的遍历顺序，在保持采样性能的同时为图像编辑提供灵活的序列顺序，解决了传统自回归模型在双向上下文生成中性能下降或序列顺序灵活性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中暴露随机排列的序列顺序时，要么性能下降，要么在推理时序列顺序选择的灵活性受损。需要一种方法既能保持采样性能，又能提供足够灵活的序列顺序来支持图像编辑。

Method: STAR利用图像块位置定义的网格中采样的均匀生成树的遍历顺序。通过广度优先搜索获得遍历顺序，通过拒绝采样高效构建生成树，确保连接的图像部分观测在序列中作为前缀出现。

Result: STAR通过定制但结构化的随机策略相比随机排列，保持了后缀完成能力，同时维持了采样性能，无需对广泛采用的语言自回归模型架构进行重大更改。

Conclusion: STAR建模能够结合图像先验知识（如中心偏差和局部性），在保持采样性能的同时提供足够灵活的序列顺序以适应推理时的图像编辑需求。
...
TL;DR: REArtGS++提出了一种基于时间几何约束和平面高斯泼溅的通用化铰接物体重建方法，改进了REArtGS在螺旋关节和多部件物体上的不足，通过解耦螺旋运动建模和平面高斯优化实现更好的部件级表面重建和关节参数估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法REArtGS在处理螺旋关节或多部件物体时存在困难，且缺乏对未见状态的几何约束。需要开发更通用的铰接物体重建方法，能够处理各种关节类型并提供时间连续的几何约束。

Method: 1. 为每个关节建模解耦螺旋运动，无需类型先验；2. 通过部件运动混合联合优化部件感知高斯和关节参数；3. 引入时间连续几何约束，鼓励高斯平面化，并通过泰勒一阶展开在平面法线和深度之间建立时间一致正则化。

Result: 在合成和真实世界铰接物体上的广泛实验表明，该方法在通用化部件级表面重建和关节参数估计方面优于现有方法。

Conclusion: REArtGS++通过引入时间几何约束和平面高斯泼溅，显著提升了铰接物体重建的通用性和准确性，特别是在处理螺旋关节和多部件物体时表现出色。
...
Conclusion: SketchVerify通过草图验证框架有效解决了运动规划的质量和效率问题，扩展候选轨迹数量能持续提升性能。
...
TL;DR: DiverseVAR是一种无需额外训练的方法，通过抑制模型输入中的关键特征分量并在输出中增强它，有效恢复视觉自回归模型的生成多样性，同时保持高保真合成质量。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型虽然具有推理效率和图像质量优势，但存在多样性崩溃问题，即输出变异性减少，类似于少步蒸馏扩散模型中出现的问题。

Method: 通过分析发现特征图中的关键分量是早期尺度多样性形成的关键因素。DiverseVAR在模型输入中抑制关键分量，在模型输出中增强它，从而恢复生成多样性。

Result: 实证结果表明，该方法显著增强了生成多样性，同时对性能影响可忽略不计。

Conclusion: DiverseVAR能够有效解锁VAR模型的固有生成潜力，在保持高保真合成的同时解决多样性崩溃问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有铰接物体重建方法需要多阶段、多视角观测等昂贵输入，限制了实际应用。
...
Conclusion: 该方法仅需稀疏视角RGB图像即可实现高质量的铰接物体重建，具有更好的实用性和重建效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大型预训练模型的视频异常检测方法通常依赖密集帧级推理，计算成本高且延迟大。本文旨在探索在强大预训练模型下是否真正需要密集推理。

Method: 提出ReCoVAD框架，包含两个核心通路：反射通路使用轻量级CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；意识通路使用中等规模视觉语言模型生成文本事件描述和精炼异常分数，通过集成大型语言模型定期审查描述以识别未见异常。

Result: 在UCF-Crime和XD-Violence数据集上，ReCoVAD实现了最先进的免训练性能，同时仅处理了先前方法所用帧数的28.55%和16.04%。

Conclusion: 稀疏推理对于基于大型模型的有效视频异常检测是足够的，ReCoVAD证明了在保持高性能的同时显著减少计算负担的可行性。
...
Method: 提出分区自适应对比学习（PACL），关注社交媒体数据中图像和文本的事实与情感连接，通过分离不同类型的样本并为每种类型设计不同的对比学习策略，动态构建正负样本对，充分利用噪声样本的潜力。

Result: 通过综合实验证明，弥合'情感鸿沟'显著提高了各种预训练视觉模型在下游情感相关任务中的性能。

Conclusion: 利用预训练文本模型的知识可以有效增强视觉模型的情感感知能力，PACL方法通过对比学习策略成功解决了视觉情感识别中的情感鸿沟问题。
...
### [43] [Intervene-All-Paths: Unified Mitigation of LVLM Hallucinations across Alignment Formats](https://arxiv.org/abs/2511.17254)
*Jiaye Qian,Ge Zheng,Yuchen Zhu,Sibei Yang*

Main category: cs.CV

TL;DR: 本研究提出了一个针对大型视觉语言模型幻觉问题的综合干预框架，发现幻觉源于图像到输入文本、图像到输出文本和文本到文本三种路径的相互作用，并开发了针对不同问答对齐格式的干预方法。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型在各种任务中表现出色，但它们仍然容易产生幻觉。本研究旨在理解幻觉产生的因果路径并开发有效的干预方法。

Method: 提出了与transformer因果架构对齐的综合干预框架，分析不同干预路径对幻觉的影响，并针对判别式和生成式格式开发了识别和干预关键幻觉头的方法。

Result: 实验证明该方法在多个基准测试中能够持续减少各种对齐类型的幻觉。

Conclusion: LVLMs的幻觉并非源于单一因果路径，而是多种路径相互作用的结果，针对不同问答格式的干预方法能有效减少幻觉。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态推理模型在生成长推理链时存在冗余自反思问题，而现有的训练自由CoT压缩方法依赖静态视觉参考，对多模态推理提升有限。
...
### [45] [Range-Edit: Semantic Mask Guided Outdoor LiDAR Scene Editing](https://arxiv.org/abs/2511.17269)
*Suchetan G. Uppur,Hemant Kumar,Vaibhav Kumar*

Main category: cs.CV

TL;DR: 提出了一种基于语义掩码引导的LiDAR点云编辑方法，通过将点云转换为2D距离图像并使用语义掩码指导扩散生成，能够高效生成复杂的边缘情况场景。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要大量多样的点云数据来训练，特别是边缘情况场景。从真实世界获取这些数据具有挑战性，而现有的3D虚拟环境模拟方法耗时且难以捕捉真实场景的复杂性。

Method: 将LiDAR点云转换为2D距离图像，使用基于凸包的语义掩码作为条件指导扩散生成过程，确保几何一致性和真实性。

Result: 在KITTI-360数据集上验证了高质量的LiDAR点云生成能力，能够生成复杂的边缘情况和动态场景。
...
TL;DR: PEGS是一个将物理先验与事件流增强集成到3D高斯泼溅管道中的框架，用于执行去模糊的目标聚焦建模和运动恢复，在大时空尺度上重建刚性运动。


<details>
  <summary>Details</summary>
Motivation: 由于建模范式的限制、严重的运动模糊和物理一致性不足，在大时空尺度上重建刚性运动仍然是一个具有挑战性的任务。

Method: 提出三重监督方案：通过加速度约束强制执行物理合理性，利用事件流进行高时间分辨率指导，并使用卡尔曼正则化器融合多源观测。设计了基于实时运动状态的自适应训练调度策略。

Result: 实验表明，与主流动态方法相比，PEGS在大时空尺度上重建运动方面表现出优越性能。
...
TL;DR: 本文研究发现多语言文本到图像模型存在文化偏差问题，提出了两种对齐策略来增强跨语言文化一致性。


<details>
  <summary>Details</summary>
Motivation: 当前多语言T2I模型在跨语言提示下往往产生文化中性或英语偏向的结果，缺乏文化一致性。

Method: 提出探测方法定位文化敏感信号到特定神经元，然后引入两种对齐策略：推理时文化激活和层定向文化增强。

Result: 在CultureBench上的实验显示，相比强基线方法在文化一致性方面有持续改进，同时保持保真度和多样性。

Conclusion: 通过定位和增强文化相关神经元，可以有效提升多语言T2I模型的文化一致性表现。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于CCT的比色映射方法仅适用于普朗克光源，而现代LED光源常常偏离普朗克轨迹，导致色彩还原不准确。

Method: 采用2D色度空间特征，使用轻量级MLP替代传统的CCT插值方法，通过包含代表性LED光源的光箱校准程序训练MLP。

Result: 在多种LED照明条件下，该方法平均减少22%的角度再现误差，保持与传统光源的向后兼容性，支持多光源场景和实时相机部署。

Conclusion: 从1D CCT到2D色度空间的转变结合MLP方法，显著提高了非普朗克光源下的比色映射精度，同时保持计算效率。
...
Method: 扩展MAE到任意多视图，统一掩码所有视图并使用轻量级解码器和帧间注意力机制。

Result: MuM在重建、密集图像匹配和相对姿态估计等任务中优于DINOv3和CroCo v2。

Conclusion: 多视图掩码自编码是学习3D视觉特征的有效方法，比现有方法更简单且可扩展。
...
TL;DR: ODTSR是一种基于Qwen-Image的一步扩散变换器，通过噪声混合视觉流设计同时考虑保真度和可控性，在真实世界图像超分辨率任务中实现最先进性能。
...
Method: 引入噪声混合视觉流设计：新视觉流接收带有可调噪声的低质量图像，原视觉流接收带有一致噪声的低质量图像；采用保真度感知对抗训练增强可控性并实现一步推理。

Result: 在通用真实世界图像超分辨率上达到最先进性能，并在中文场景文本图像超分辨率等挑战性场景中实现提示可控性，无需在特定数据集上训练。

Conclusion: ODTSR通过创新的噪声混合视觉流和保真度感知对抗训练，成功平衡了真实世界图像超分辨率中的保真度和可控性需求。
...
TL;DR: 本文提出了一种利用生成模型创建多样化合成人脸图像的方法，并发布了包含27,780张图像的DIF-V数据集，以解决现有面部验证系统中存在的种族和性别偏见问题。
...
Method: 整合先进生成模型创建多样化高质量合成人脸图像，强调面部特征的多样性表达，确保符合身份证照片特征要求。

Result: 分析显示现有验证模型对某些性别和种族存在偏见，身份风格修改会负面影响模型性能。

Conclusion: 通过解决现有数据集的不公平问题，这项工作不仅丰富了人工智能多样性和伦理讨论，还为开发更具包容性和可靠性的面部验证技术奠定了基础。
...
TL;DR: CC-DiceCE是一种基于CC-Metrics框架的损失函数，用于解决医学图像分割中传统Dice损失对小病灶分割不足的问题。相比现有blob loss，CC-DiceCE在nnU-Net框架下能提高病灶检测率，仅轻微增加假阳性。


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割损失函数（如Dice）由于小病灶相对体积小，对整体损失贡献可忽略，导致小病灶分割不足。需要基于每个病灶评估分割质量的实例级损失函数。

Method: 在nnU-Net框架中引入基于CC-Metrics的CC-DiceCE损失函数，并与现有blob loss进行比较，以DiceCE作为基准。

Result: CC-DiceCE损失提高了病灶检测率（召回率），分割性能仅有轻微下降，但会略微增加假阳性。多数据集研究表明CC-DiceCE通常优于blob loss。

Conclusion: CC-DiceCE损失函数能有效改善小病灶分割问题，在保持分割性能的同时提高检测率，是优于blob loss的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的生成式方法（如扩散模型）通常基于人工设计的轨迹锚点或随机噪声进行去噪，存在改进空间。作者希望通过结合判别式方法提供更好的初始指导来提升扩散模型的性能。

Method: 提出两阶段框架：第一阶段使用基于Transformer的提议解码器从传感器输入回归生成粗粒度轨迹预测；第二阶段应用扩散精炼器迭代去噪和精炼这些初始预测，并设计了细粒度去噪解码器增强场景合规性。

Result: 在NAVSIM v2上达到87.4 EPDMS，在Bench2Drive上达到87.1 DS和71.4 SR，在两个公共基准测试中都创造了新记录。

Conclusion: DiffRefiner通过结合判别式轨迹提议和生成式精炼，有效提升了扩散基规划的轨迹预测性能，消融研究验证了各组件有效性。
...
<details>
  <summary>Details</summary>
Motivation: 超声图像在不同采集设备间存在外观差异，导致领域偏移问题，影响固定黑盒下游推理模型的性能。现有无配对图像翻译方法往往忽视类别特定的语义对齐，造成内容-类别映射错误，损害诊断准确性。

Method: 提出UI-Styler框架，包含：1）模式匹配机制，将目标图像中的纹理模式迁移到源图像上同时保持源结构内容；2）类别感知提示策略，基于目标域伪标签引导，确保与诊断类别的准确语义对齐。

Result: 在超声跨设备任务上的广泛实验表明，UI-Styler在分布距离和下游任务（如分类和分割）中始终优于现有无配对图像翻译方法，达到最先进的性能。

Conclusion: UI-Styler通过类别感知的风格迁移有效解决了超声图像跨设备领域偏移问题，为黑盒下游推理模型的重用提供了实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有野火风险预测方法缺乏因果推理和多模态理解能力，难以实现可靠的泛化。需要开发能够整合视觉、气候和地理因素的推理密集型空间模型。

Method: 提出FireScope-Bench数据集和基准，结合Sentinel-2影像、气候数据和专家定义的风险栅格；开发FireScope框架，通过强化学习和视觉监督学习预测风险栅格并生成互补的推理轨迹。

Result: 在美国训练并在欧洲测试时，FireScope取得显著性能提升；专家反馈和自动分析确认其推理轨迹具有忠实性和语义意义。

Conclusion: 基于语言的推理可以提升视觉生成模型的泛化能力和可解释性；FireScope是首个展示语言推理改进视觉生成泛化能力的高分辨率跨大陆野火风险模型。
...
TL;DR: 本文介绍了RSFM数据库和REMSA智能体，用于解决遥感基础模型选择困难的问题。RS-FMD包含150多个遥感基础模型，REMSA是基于LLM的智能体，能够通过自然语言查询自动选择合适模型，并在专家验证的基准测试中表现优于多个基线方法。


<details>
  <summary>Details</summary>
Motivation: 遥感领域基础模型越来越多，但由于文档分散、格式异构和部署约束多样，选择合适的遥感基础模型变得困难。

Method: 构建了包含150多个模型的RSFM数据库，开发了基于LLM的REMSA智能体，使用情境学习对候选模型进行排序，并提供透明的理由说明。

Result: REMSA在75个专家验证的查询场景基准测试中（产生900个配置），表现优于朴素智能体、密集检索和非结构化RAG-based LLMs等基线方法。

Conclusion: RS-FMD和REMSA为遥感基础模型选择提供了结构化资源和自动化解决方案，完全基于公开元数据运行，不访问私有或敏感数据。
...
Result: 大多数自监督特征能捕获深度伪造相关信息且具有互补性，模型主要关注语义相关区域而非虚假伪影，但都无法可靠跨数据集泛化。

Conclusion: 自监督表示在深度伪造检测中既有前景又面临根本挑战：能学习有意义的模式，但实现稳健的跨域性能仍然困难。
...
TL;DR: PostCam是一个用于动态场景中新颖视角视频生成的框架，支持拍摄后编辑相机轨迹，通过查询共享交叉注意力模块融合6-DoF相机姿态和2D渲染视频帧，实现更精确灵活的运动操控。


<details>
  <summary>Details</summary>
Motivation: 现有视频重拍摄方法的相机运动注入策略不理想，限制了相机控制精度，且生成的视频无法保留源视频的精细视觉细节。

Method: 引入查询共享交叉注意力模块，融合6-DoF相机姿态和2D渲染视频帧控制信号；采用两阶段训练策略：先学习粗略相机控制，再结合视觉信息优化运动精度和视觉保真度。

Result: 在真实世界和合成数据集上的实验表明，PostCam在相机控制精度和视角一致性上比最先进方法提高超过20%，同时达到最高的视频生成质量。

Conclusion: PostCam通过统一的特征表示和两阶段训练策略，显著提升了动态场景中相机轨迹编辑的精度和生成视频的质量。
...
TL;DR: VLA-4D模型通过4D感知的视觉表示和时空动作表示，实现了机器人操作的时空一致性控制，解决了现有方法在时间连贯性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型在时空一致性操作方面存在挑战，特别是在时间连贯性控制上表现不佳，需要更细粒度的表示方法。

Method: 提出4D感知视觉表示（将1D时间嵌入3D位置形成4D嵌入）和时空动作表示（扩展传统空间动作表示包含时间信息），通过交叉注意力机制融合多模态表示到LLM中进行时空动作预测。

Result: 在机器人操作的不同任务中进行了广泛实验，验证了方法的优越性。

Conclusion: VLA-4D模型通过统一的4D感知框架，实现了空间平滑和时间连贯的机器人操作控制。
...
TL;DR: 本文提出CA-SAM方法，通过轻量级的对齐层模块和持续学习策略，在平衡计算效率和性能的同时，使SAM模型能够适应医学图像分割任务并缓解灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，机构间的隐私政策差异使得联合训练不可行，需要持续学习方法来避免灾难性遗忘。SAM模型虽然具有强大的零样本能力，但参数量大、计算开销高，限制了实际部署。

Method: 提出对齐层模块来对齐编码器-解码器特征分布，轻量级地适配SAM到医学图像；在此基础上提出CA-SAM持续学习策略，自动适配对齐层来缓解灾难性遗忘，同时利用SAM的零样本先验保持性能。

Result: 在9个医学分割数据集的持续学习场景下，CA-SAM实现了最先进的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有稠密3D重建方法在SLAM中存在的漂移和冗余点云地图问题，这些限制了效率和下游任务（如新视角合成）的性能。

Method: 首先通过轻量级跟踪和重建模块构建局部一致的子地图，然后逐步对齐和融合到全局高斯地图中，强制跨视角几何一致性，全局地图反过来纠正局部漂移并增强跟踪鲁棒性。

Result: 在真实世界数据集上实现了最先进的跟踪、3D重建和新视角渲染性能，跟踪精度提升超过12%，产生更精细、更详细的几何结构，同时保持紧凑和内存高效的全局表示。

Conclusion: SING3R-SLAM通过全局一致的高斯表示有效解决了SLAM中的漂移和冗余问题，为多种下游应用提供了高效且多功能的3D建图解决方案。
...
Method: 结合局部Transformer进行高分辨率特征提取和全局Transformer进行全扫描上下文建模，使用DINO自蒸馏和SigLIP视觉语言对齐进行预训练，仅使用公开CT数据集。

Result: 在多个CT基准测试中，SPECTRE在零样本和微调设置下均优于现有CT基础模型，证明了其卓越性能。

Conclusion: SPECTRE是一个可扩展、开放且完全基于Transformer的3D医学影像基础模型，展示了无需私有数据即可获得高性能通用表示的能力。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，通过4D时空查询直接学习连续3D语义占据，在自监督Occ3D-nuScenes基准上比之前基于相机的方法在语义RayIoU上提升了26%。


<details>
  <summary>Details</summary>
Motivation: 由于大规模3D标注成本过高，现有方法要么依赖2D渲染一致性导致3D结构仅隐式出现，要么使用离散体素网格限制空间精度和可扩展性。

Method: 使用独立的4D时空查询采样相邻帧，支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督，引入收缩场景表示在恒定内存下实现长距离监督和推理。
...
TL;DR: 提出了一种用于彩色图像盲去卷积的新四元数保真项方法，通过四元数卷积核处理彩色通道间的相互依赖关系，有效改善去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有彩色图像盲去卷积方法通常将彩色图像转换为灰度图或单独处理每个颜色通道，忽略了颜色通道之间的关系。

Method: 设计了一种新颖的四元数保真项，利用四元数卷积核的特性（包含四个核：一个类似非负卷积核捕获整体模糊，三个无约束核分别对应RGB通道建模未知相互依赖），并在盲去卷积过程中使用归一化四元数核以保持图像强度。
...
TL;DR: 本文提出了一种基于传统文本搜索的相关性反馈机制，用于改进视觉语言模型在推理时的检索性能，包括四种反馈策略：伪相关性反馈、生成式相关性反馈、注意力反馈总结器和显式反馈。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然支持自然语言查询的视觉搜索，但提升性能通常需要微调或使用更大模型。本文旨在通过相关性反馈机制在不依赖微调的情况下改进检索性能。

Method: 提出了四种反馈策略：1）伪相关性反馈（PRF）基于排名靠前的结果优化查询嵌入；2）生成式相关性反馈（GRF）使用合成标题进行查询优化；3）注意力反馈总结器（AFS），基于Transformer的多模态细粒度特征整合模型；4）使用真实标题的显式反馈作为上限基准。

Result: 在Flickr30k和COCO数据集上的实验表明，GRF、AFS和显式反馈相比无反馈检索，对较小VLM的MRR@5提升了3-5%，对较大VLM提升了1-3%。AFS与显式反馈类似，能减轻查询漂移，在迭代多轮检索中比GRF更稳健。

Conclusion: 相关性反馈能够持续提升不同VLM的检索性能，为交互式和自适应视觉搜索开辟了新机会。
...
TL;DR: MolSight是一个用于光学化学结构识别(OCSR)的三阶段学习框架，专门解决现有系统在立体化学信息识别方面的挑战，通过预训练、多粒度微调和强化学习优化，在立体化学结构识别上达到最先进性能。
...
Method: 采用三阶段训练范式：1) 在大规模噪声数据集上进行预训练，赋予模型化学结构图像的基本感知能力；2) 使用具有更丰富监督信号的数据集进行多粒度微调，探索化学键分类和原子定位等辅助任务对分子式识别的影响；3) 使用强化学习进行后训练优化，并引入新的立体化学结构数据集。

Result: 通过在不同数据集上的广泛实验，MolSight在（立体）化学光学结构识别方面达到了最先进的性能。即使参数规模相对紧凑，GRPO算法也能进一步提升模型在立体分子上的表现。

Conclusion: MolSight框架通过创新的三阶段训练方法，成功解决了OCSR中立体化学信息识别的关键挑战，为化学信息学中的数据挖掘、药物发现和LLM应用提供了有效的解决方案。
...
TL;DR: BiFingerPose是一种基于双模态的指尖姿态估计算法，通过结合电容图像和指纹补丁，能够同时准确预测全面的指尖姿态信息，包括之前无法估计的滚转角，并在连续和离散交互任务中表现出显著优势。


<details>
  <summary>Details</summary>
Motivation: 现有基于电容图像的指尖姿态估计算法在便携设备上只能估计俯仰角和偏航角，且在大角度输入时精度下降，无法估计滚转角，限制了触摸屏设备的人机交互能力扩展。

Method: 提出双模态输入方法，结合来自触摸屏的电容图像和屏下指纹传感器获取的指纹补丁，构建BiFingerPose算法来同时预测全面的指尖姿态参数。

Result: 在12人用户研究中，BiFingerPose相比之前SOTA方法预测性能提升超过21%，任务完成效率提高2.5倍，用户操作准确率提升23%，能够可靠估计滚转角并显著改善其他姿态参数的预测性能。

Conclusion: BiFingerPose通过双模态方法显著提升了指尖姿态估计能力，在增强认证安全性和改善交互体验方面具有实际应用潜力，并开发了相应原型展示交互可能性。
...
TL;DR: 本文提出SpatialGeo模型，通过层次化融合几何和语义特征来增强多模态大语言模型的空间推理能力，解决了现有模型在三维空间理解方面的局限性。
...
Method: 提出基于几何和语义特征层次化融合的新型视觉编码器，通过层次适配器将自监督学习的几何特征与CLIP语义特征互补，采用随机特征丢弃训练避免仅依赖CLIP编码器的平凡解。

Result: SpatialGeo在空间推理任务中准确率显著提升，在SpatialRGPT-Bench上将最先进模型性能提高至少8.0%，推理时内存成本减少约50%。

Conclusion: SpatialGeo通过增强空间感知能力有效提升了MLLMs的空间推理性能，同时降低了计算成本。
...
Method: 1. 基于显式特征匹配的相对姿态初始化；2. 局部联合优化提升姿态估计；3. 全局优化阶段通过束调整整合几何一致性约束和特征轨迹。
...
<details>
  <summary>Details</summary>
Motivation: 生成图像模型在透明物体合成方面表现不佳，特别是折射效果，因为它们未能充分学习光学定律来准确渲染折射物体。
...
TL;DR: 提出一个统一框架用于多媒体绘画过程生成，通过语义驱动的风格控制机制将多种媒介嵌入扩散模型的条件空间，实现跨媒介风格增强和一致纹理演化。


<details>
  <summary>Details</summary>
Motivation: 现有绘画教程视频缺乏交互性和个性化，而现有生成模型在跨媒介泛化和保持时间/结构一致性方面存在困难，无法忠实再现人类创作流程。

Method: 使用语义驱动的风格控制机制将多种媒介嵌入扩散模型条件空间，采用跨媒介风格增强和反向绘画训练策略确保平滑生成。构建大规模真实绘画过程数据集。

Result: 在跨媒介一致性、时间连贯性和最终图像保真度方面取得良好结果，在LPIPS、DINO和CLIP指标上表现优异。提出的感知距离剖面(PDP)曲线定量建模创作序列。

Conclusion: 该框架能够生成与人类艺术进程一致的绘画过程，实现跨风格的过程转移和一致纹理演化，为交互式个性化绘画学习提供了有效解决方案。
...
TL;DR: DSeq-JEPA提出了一种判别性顺序联合嵌入预测架构，通过识别主要判别区域并按重要性顺序预测后续区域，将JEPA风格的潜在预测与GPT风格的顺序推理相结合。


<details>
  <summary>Details</summary>
Motivation: 受人类视觉感知启发，传统I-JEPA对所有区域进行统一独立处理，缺乏明确的预测顺序概念。人类视觉会从最具信息量的区域到次要区域进行选择性顺序注意力部署。

Method: 首先基于transformer生成的显著性图识别主要判别区域，然后按判别性顺序预测后续区域，形成从主要到次要线索的课程式语义进展。

Result: 在图像分类、细粒度视觉分类、检测分割和低级推理任务上的广泛实验表明，DSeq-JEPA比I-JEPA变体始终关注更具判别性和泛化性的表示。

Conclusion: DSeq-JEPA通过整合预测性和自回归自监督学习，实现了更有效的视觉表示学习，在多个任务上表现优于现有方法。
...
TL;DR: SuperQuadricOcc是一种基于超二次曲面的语义占据估计方法，通过超二次曲面表示减少基元数量，实现75%内存占用降低、124%推理速度提升和5.9% mIoU改进，是首个支持实时推理的占据模型。


<details>
  <summary>Details</summary>
Motivation: 高斯表示在自监督占据估计中广泛使用，但大量高斯基元显著增加内存需求，不适合实时推理。超二次曲面由于形状多样性可减少基元数量和内存需求，但缺乏超二次曲面光栅化器阻碍了其应用。

Method: 提出SuperQuadricOcc方法，采用基于超二次曲面的场景表示，通过多层二十面体细分的高斯近似超二次曲面，实现训练期间的高斯光栅化监督。

Result: 在Occ3D数据集上，相比之前的高斯方法，内存占用减少75%，推理速度提升124%，mIoU提高5.9%，无需时间标签。基元数量减少84%。

Conclusion: 该方法首次实现了实时推理的占据模型，同时保持竞争力性能，通过快速超二次曲面体素化模块促进与先前方法的比较。
...
TL;DR: 提出ATAC方法，通过嵌入空间中的增强诱导漂移向量来推断语义恢复方向，在CLIP模型中实现高效测试时对抗防御
...
Method: 在CLIP嵌入空间中操作，计算增强诱导漂移向量来推断语义恢复方向，基于这些潜在漂移的角度一致性校正嵌入

Result: 在广泛基准测试中，ATAC始终实现极高鲁棒性，平均超过先前最先进方法近50%，同时计算开销极小

Conclusion: ATAC是CLIP嵌入空间中测试时对抗防御新范式的有效方法，在非常规和极端设置下保持最先进鲁棒性，甚至对自适应攻击也实现非平凡鲁棒性
...
TL;DR: SVRecon将稀疏体素栅格化范式扩展到高保真表面重建任务，通过集成SDF来解决稀疏体素在优化过程中容易陷入局部最小值的问题。


<details>
  <summary>Details</summary>
Motivation: 稀疏体素虽然具有空间解耦和清晰边界的优点，但在优化过程中容易陷入局部最小值。SDF值提供了自然平滑连续的几何场，但在独立参数化的稀疏体素之间保持这种平滑性具有挑战性。

Method: 通过(1)使用视觉几何模型进行稳健的几何初始化，(2)应用空间平滑损失来强制父-子和兄弟体素组之间的连贯关系，促进连贯平滑的体素结构。

Result: 在各种基准测试中的广泛实验表明，该方法实现了强大的重建精度，同时具有持续快速的收敛速度。

Conclusion: SVRecon方法在保持快速收敛的同时实现了高质量的表面重建，代码将公开。
...
TL;DR: 本文提出了非参数概率鲁棒性（NPPR）作为更实用的鲁棒性度量，不依赖预定义的扰动分布，直接从数据中学习优化的扰动分布，在分布不确定性下实现保守的鲁棒性评估。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型对小的输入扰动很脆弱，现有概率鲁棒性（PR）方法假设固定已知的扰动分布，这在实践中不现实。

Method: 采用非参数统计建模范式，基于高斯混合模型（GMM）和多层感知机（MLP）头以及双三次上采样开发NPPR估计器，涵盖输入依赖和输入独立的扰动场景。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上的实验表明，NPPR相比现有方法提供更保守（低40%）的PR估计。

Conclusion: NPPR是一个更实用的鲁棒性度量，解决了现有PR方法对预定义扰动分布的依赖问题。
...
TL;DR: MorphSeek是一种用于医学图像可变形配准的细粒度表示级策略优化方法，通过在高维潜在特征空间中进行空间连续优化，解决了传统方法在捕捉空间变异变形方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统可变形图像配准方法面临高维变形空间的挑战，现有强化学习框架往往将空间投影到粗糙的低维表示，限制了捕捉空间变异变形能力。

Method: 提出MorphSeek框架，在编码器顶部引入随机高斯策略头来建模潜在特征分布，实现高效探索和从粗到细的优化。结合无监督预热和弱监督微调，通过多轨迹采样稳定训练。

Result: 在三个3D配准基准测试（OASIS脑部MRI、LiTS肝脏CT和腹部MR-CT）中，MorphSeek相比竞争基线实现了持续的Dice改进，同时保持高标签效率和低参数成本。

Conclusion: MorphSeek推进了表示级策略学习范式，实现了空间一致且数据高效的变形优化，为高维环境中的可扩展视觉对齐提供了原则性、骨干无关和优化器无关的解决方案。
...
TL;DR: 本文提出了一种名为MCMoE的多模态动作质量评估框架，通过混合专家模型解决推理阶段部分模态缺失的问题，实现了完整和不完整多模态学习的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现实应用中，多模态AQA系统在推理阶段经常面临部分模态不可用的问题，导致现有模型无法运行并引发性能严重下降。需要解决模态缺失情况下的鲁棒性评估问题。

Method: 提出MCMoE框架，包含自适应门控模态生成器动态融合可用信息重建缺失模态，设计模态专家学习单模态知识并动态混合所有专家知识提取跨模态联合表示，在训练阶段挖掘完整多模态特征指导模态生成。

Result: 在三个公共AQA基准测试中，MCMoE在完整和不完整多模态学习场景下均取得了最先进的性能结果。

Conclusion: MCMoE框架通过统一单模态和联合表示学习，有效解决了多模态AQA中模态缺失问题，提升了模型的鲁棒性和性能表现。
...
TL;DR: 提出了MMT-ARD：一种多模态多教师对抗鲁棒蒸馏框架，通过双教师知识融合架构提升视觉语言模型的对抗鲁棒性，在ImageNet和零样本基准测试中显著提高了鲁棒准确率和训练效率。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在安全关键应用中的部署日益增多，其对抗鲁棒性成为关键问题。传统单教师方法存在知识多样性有限、收敛速度慢、鲁棒性与准确性难以平衡等挑战。

Method: 采用多模态多教师对抗鲁棒蒸馏框架，包含双教师知识融合架构、基于教师置信度的动态权重分配策略，以及自适应sigmoid加权函数来平衡跨模态知识传递强度。
...
TL;DR: CWMDT框架将标准视频扩散模型转化为反事实世界模型，通过构建场景的数字孪生表示，使用大语言模型进行反事实推理，并生成反事实视觉序列。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要基于事实观察进行前向模拟，无法回答反事实查询（如'如果移除这个物体会发生什么'），这在物理AI行为评估等应用中日益重要。

Method: 1) 构建场景的数字孪生，以结构化文本编码对象及其关系；2) 使用大语言模型推理反事实干预的时间传播；3) 用修改后的表示条件化视频扩散模型生成反事实视觉序列。

Result: 在两个基准测试上的评估表明，CWMDT方法实现了最先进的性能，证明数字孪生等替代视频表示可为基于视频前向模拟的世界模型提供强大的控制信号。

Conclusion: CWMDT框架成功地将视频扩散模型转化为有效的反事实世界模型，通过数字孪生表示实现了对特定场景属性的针对性干预，为物理AI行为评估等应用提供了重要工具。
...
TL;DR: Radar2Shape是一个去噪扩散模型，通过将雷达信号频率与多分辨率形状特征相关联，从部分可观测的雷达信号进行3D重建。该方法采用两阶段方法：首先学习具有层次分辨率形状特征的规范化潜在空间，然后通过以类似粗到细的方式调节雷达信号频率来扩散到该潜在空间。


<details>
  <summary>Details</summary>
Motivation: 从高频雷达信号确定3D物体形状在商业和航空航天应用中至关重要，但分析复杂。现有深度学习方法难以表示任意形状或处理有限视角收集的真实世界雷达信号，而光学3D重建方法将雷达信号简单视为相机视图时效果不佳。

Method: 提出Radar2Shape去噪扩散模型，采用两阶段方法：1）学习具有层次分辨率形状特征的规范化潜在空间；2）通过以粗到细方式调节雷达信号频率扩散到该潜在空间。

Result: Radar2Shape能够成功地从部分观测的雷达信号重建任意3D形状，并在两种不同仿真方法和真实世界数据上表现出鲁棒的泛化能力。同时发布了两个合成基准数据集。

Conclusion: Radar2Shape方法能够有效处理部分可观测雷达信号进行3D重建，为高频雷达领域的未来研究提供了基准数据集，支持模型安全适应到真实世界雷达系统。
...
TL;DR: 提出了一种基于计算机视觉的深度学习方法，使用超过18,000个MRI系列图像来估计脊柱年龄，并开发了脊柱年龄差（SAG）作为衡量脊柱健康的生物标志物。


<details>
  <summary>Details</summary>
Motivation: 脊柱是人体重要结构，容易发生与年龄相关的退行性病变，需要开发有效的方法来评估脊柱健康状况。

Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退行性病变聚类，通过消融研究确定模型选择，利用深度学习从MRI图像估计脊柱年龄。

Result: 脊柱年龄差（SAG）与椎间盘突出、骨赘、椎管狭窄、骨折等脊柱退行性病变以及吸烟、体力劳动等生活方式因素相关。

Conclusion: 脊柱年龄差（SAG）可作为衡量整体脊柱健康的有用生物标志物，与多种脊柱退行性病变和生活方式因素显著相关。
...
TL;DR: 本文研究了多模态模型中缩小LLM规模对视觉能力的影响，发现视觉能力比推理能力下降更严重，提出了视觉提取调优和逐步推理的Extract+Think方法来解决这一瓶颈。


<details>
  <summary>Details</summary>
Motivation: 实际应用需要更小、更高效的多模态系统，但缩小LLM规模会不成比例地影响视觉能力，需要解决这一性能瓶颈。

Method: 提出Extract+Think方法，包括视觉提取调优（训练模型提取指令相关的视觉细节）和逐步推理生成答案。

Result: 实验发现LLM缩小对感知能力的影响与推理能力相当或更大，而Extract+Think方法在该领域设定了效率和性能的新标准。

Conclusion: 视觉提取调优和逐步推理的Extract+Think方法有效解决了多模态模型缩小规模时的视觉能力瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频QA模型依赖单次感知固定帧，导致幻觉和细粒度证据失败。受人类暂停、放大和重读关键区域的行为启发，需要迭代视觉反刍机制。
...
TL;DR: 本文提出了一种新的原生3D编辑框架，通过单次前向传播直接操作3D表示，解决了现有方法在几何一致性和视觉质量方面的问题。
...
Method: 创建了一个大规模多模态数据集用于指令引导的3D编辑，涵盖添加、删除和修改任务。探索了两种条件策略：传统的交叉注意力机制和新的3D标记连接方法。

Result: 标记连接方法在参数效率方面更优且性能更好。广泛评估表明该方法在生成质量、3D一致性和指令保真度方面优于现有的2D提升方法。

Conclusion: 该方法为指令引导的3D编辑设定了新的基准，在保持3D一致性和视觉质量的同时实现了高效编辑。
...
<details>
  <summary>Details</summary>
Motivation: 现有的向量数据库检索管道依赖于扁平或单分辨率索引结构，无法适应不同用户查询所需的语义粒度变化，导致检索速度和上下文相关性之间的次优权衡。

Method: SPI在文档嵌入上构建语义金字塔，通过轻量级分类器为每个查询动态选择最优分辨率级别，实现从粗到细的渐进式检索。

Result: 在多个RAG任务评估中，SPI实现了高达5.7倍的检索加速和1.8倍的内存效率提升，同时将端到端QA F1分数提高了最多2.5分。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM本地推理面临配置选择过多的问题，现有基准测试工具目标狭窄且不用户友好，缺乏整合系统指标和任务特定指标的统一易用基准测试框架。

Method: 开发Bench360框架，允许用户定义自定义任务、数据集和任务特定指标，自动测试选择的LLM、推理引擎和量化级别，涵盖单流、批处理和服务器等使用场景，跟踪计算性能、资源使用、部署等系统指标以及ROUGE、F1分数等任务特定指标。

Result: 在四个常见LLM任务、三个硬件平台和四个先进推理引擎上的测试结果显示，任务性能与系统级效率之间存在多个有趣的权衡关系，不同推理引擎和模型存在显著差异。
...
<details>
  <summary>Details</summary>
Motivation: 工业级大型语言模型往往忽视低资源语言如突尼斯阿拉伯语的理解能力，这可能导致数百万突尼斯人无法用母语与AI充分互动，威胁方言保护并影响年轻一代的语言偏好。

Method: 创建包含突尼斯方言、标准阿拉伯语和英语的平行翻译数据集，并带有情感标签；在转写、翻译和情感分析三个任务上对多个流行LLM进行基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 现有自适应决策边界方法假设已知类别呈各向同性分布，限制边界为球形，忽略了不同特征方向的分布差异，限制了开放意图检测的性能。

Method: 采用监督对比学习获得判别性特征空间；使用可学习矩阵参数化椭圆体作为已知类别边界；通过双损失函数优化边界，平衡经验风险和开放空间风险。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地用于需要与人类价值观对齐的关键应用中。虽然模型微调常用于确保安全响应，但这种方法静态且不适用于涉及动态价值观和偏好的日常情境。

Method: 提出了一种实用、可重复且模型无关的程序，使用Schwartz基本人类价值观理论，通过对话数据集进行结构化评估，比较基线提示与明确基于价值观的提示。
...
TL;DR: 本文提出了一种基于概念梯度的可解释性技术，通过分析毒性检测中的子类型属性（如淫秽、威胁、侮辱等）作为概念，识别毒性语言。研究发现概念的不均衡归因会导致分类错误，因此引入了目标词典集和词-概念对齐分数来量化错误归因，并提出了无词典增强策略来验证模型对更广泛毒性模式的归因。


<details>
  <summary>Details</summary>
Motivation: 虽然毒性语言检测在文本数据方面取得了显著进展，但基于概念的解释在毒性检测中的探索仍然有限。当前方法往往只关注输入特征，而忽略了概念层面的因果解释。

Method: 1. 利用毒性检测数据集中的子类型属性作为概念指标；2. 引入基于概念梯度的方法提供因果解释；3. 构建目标词典集捕获导致误分类的有毒词汇；4. 计算词-概念对齐分数量化错误归因；5. 提出无词典增强策略生成排除预定义有毒词典的毒性样本。

Result: 该方法能够识别毒性词汇对模型误分类的具体贡献，通过词-概念对齐分数量化了过度归因导致的错误，无词典增强策略验证了模型在排除显性词汇重叠后仍能识别更广泛的毒性语言模式。

Conclusion: 概念梯度方法为毒性检测提供了更因果的解释框架，目标词典集和词-概念对齐分数能够有效识别和量化模型的错误归因行为，无词典增强策略揭示了模型对毒性语言模式的更广泛理解能力。
...
TL;DR: 该论文研究了AI检测模型对经过轻微AI润色的人类撰写阿拉伯语文章的误判问题，发现所有AI检测器都会将大量轻微润色的人类文章错误归因于AI生成。


<details>
  <summary>Details</summary>
Motivation: 解决AI检测模型对经过轻微AI润色的人类撰写文章的误判问题，特别是在阿拉伯语领域缺乏相关研究，避免错误指控作者AI抄袭并维护AI检测器的可信度。

Method: 生成两个阿拉伯语数据集：第一个包含800篇文章（一半AI生成，一半人类撰写），用于评估14个LLM和商业AI检测器的区分能力；第二个Ar-APT数据集包含400篇人类文章，由10个LLM使用4种润色设置处理，共16400个样本，用于评估轻微润色对检测性能的影响。

Result: 所有AI检测器都会错误地将大量轻微润色的人类文章归因于AI生成。最佳LLM Claude-4 Sonnet准确率从83.51%降至57.63%；最佳商业模型originality.AI准确率从92%骤降至12%。

Conclusion: 轻微AI润色会显著影响AI检测模型的性能，导致对人类撰写文章的误判，这突显了当前AI检测技术在阿拉伯语领域的局限性，需要开发更鲁棒的检测方法。
...
TL;DR: 本文复现了基于最近邻的测试时训练方法，通过在推理时使用检索到的最近邻序列对语言模型进行微调，显著降低了困惑度和比特每字节指标，特别是在结构化或专业数据集上效果更明显。


<details>
  <summary>Details</summary>
Motivation: 验证测试时训练在语言模型中的有效性和通用性，特别是对于不同规模和架构的模型，以及在不同数据域上的表现。

Method: 使用预训练的RoBERTa嵌入和Faiss索引检索每个测试输入的20个最近邻，然后对每个邻居应用一次梯度更新，在GPT-2、GPT-Neo和R1-Distilled-Qwen2.5-1.5B模型上进行实验。

Result: 测试时训练显著降低了困惑度和比特每字节指标，在GitHub和EuroParl等结构化数据集上改进最大。未在The Pile上预训练的模型受益更多，允许较小模型接近较大模型的性能。

Conclusion: 最近邻测试时训练方法具有鲁棒性和通用性，即使对于现代推理优化架构也能带来一致的性能提升，同时提出了内存高效的检索实现方案。
...
TL;DR: 本文系统分析了多语言大语言模型中语言信息的内部表示结构，发现语言信息在第一个Transformer块中急剧分离，并在整个模型深度中保持线性可分。语言方向与词汇嵌入的对齐与训练数据的语言组成密切相关，中文包含模型的中文匹配峰值达到16.43%，而英语中心模型仅为3.90%。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言LLM在多种语言上表现出色，但对其内部表示空间中语言信息结构以及跨层涌现方式的系统性分析仍然有限。本文旨在通过全面探测研究来理解语言编码的层间动态和几何结构。

Method: 在六个多语言LLM上进行了全面探测研究，覆盖所有268个Transformer层，使用线性和非线性探针以及新的Token-Language Alignment分析来量化语言编码的层间动态和几何结构。

Result: 语言信息在第一个Transformer块中急剧分离（从第0层到第1层增加76.4±8.2个百分点），并在整个模型深度中保持几乎完全线性可分。语言方向与词汇嵌入的对齐与训练数据的语言组成密切相关，中文包含模型的中文匹配峰值达到16.43%，而英语中心模型仅为3.90%，显示出4.21倍的结构印记效应。

Conclusion: 多语言LLM不是通过表面脚本特征来区分语言，而是通过训练语料库塑造的潜在表示结构。这些发现为多语言表示学习中的数据组成策略和公平性提供了实用见解。
...
TL;DR: 本文提出了一种基于语言模型的嵌入方法，用于解决SNOMED CT生物医学本体中词汇外查询的概念检索问题，在直接上位词和较少相关祖先概念检索方面优于基线方法。


<details>
  <summary>Details</summary>
Motivation: SNOMED CT中的知识检索因语言歧义、同义词和多义词而具有挑战性，当查询是词汇外查询（即在本体中没有等效匹配）时，问题更加严重。

Method: 提出了一种基于语言模型的本体嵌入方法，用于从SNOMED CT中检索层次概念，特别针对词汇外查询问题。

Result: 该方法在构建的词汇外查询数据集上表现优于SBERT和两种词汇匹配基线方法，能够有效检索最直接的上位词及其较少相关的祖先概念。

Conclusion: 该方法虽然针对SNOMED CT评估，但具有通用性，可扩展到其他本体，作者已发布代码、工具和评估数据集。
...
TL;DR: 研究发现共情行为（牺牲任务效率满足人类需求）在LLM激活空间中呈现线性方向，通过对比提示检测和调控三个不同模型，发现检测性能优异但调控效果因模型而异。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型中是否存在可检测和调控的共情行为方向，并比较不同训练策略（安全训练与未审查）对共情行为编码和调控能力的影响。

Method: 使用基于EIA基准的对比提示，在Phi-3-mini-4k、Qwen2.5-7B和Dolphin-Llama-3.1-8B三个模型上进行检测和调控实验，分析激活空间中的线性方向。

Result: 检测性能优异（AUROC 0.996-1.00），但跨模型探针一致性有限。调控成功率：Qwen 65.3%、Phi-3 61.7%、Dolphin 94.4%（仅正向调控）。Dolphin在反共情调控时出现灾难性崩溃。

Conclusion: 共情编码独立于安全训练，但安全训练影响调控鲁棒性。不同模型在检测-调控能力上存在差异，Qwen和Phi-3保持双向一致性，Dolphin仅对共情增强具有鲁棒性。
...
TL;DR: JGU Mainz团队在BLP-2025代码生成共享任务中获胜，提出多智能体管道方法，通过代码生成和调试智能体迭代修复程序，最终获得95.4%的Pass@1分数。
...
Method: 采用多智能体管道：代码生成智能体首先生成初始解决方案，然后执行单元测试，仅将失败案例转发给调试智能体，该智能体基于错误信息、当前程序和相关测试用例生成修订方案。

Result: 在共享任务中获得第一名，Pass@1分数达到95.4%。

Conclusion: 多智能体管道方法在代码生成任务中表现优异，能够有效通过迭代调试提高代码质量，系统代码已公开。
...
TL;DR: 本文重新探讨了非宾格动词和作格动词与施事性和终结性之间的关系，使用基于种子词的可解释维度分析，发现两者确实存在关联，并证明可解释维度结合人类判断能有效评估语义属性。


<details>
  <summary>Details</summary>
Motivation: 尽管传统观点认为描述施事行为的动词更倾向于作格结构，描述终结事件的动词更倾向于非宾格结构，但Kim等人(2024)的研究发现人类对施事性和终结性的评分并不能很好地预测不及物动词的句法行为。本文旨在重新检验这一关系。

Method: 使用基于施事性和终结性对立极点上种子词的可解释维度进行计算分析。
...
<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型容易受到后门攻击的问题，其中输入提示中的触发器可能引导生成有害或意外内容。

Method: 引入PEPPER防御方法，通过将标题重写为语义上不同但视觉上相似的标题，并添加不显眼的元素来破坏输入提示中的触发器。
...
TL;DR: 本文提出了一种新的无参考指标来评估大语言模型生成响应的简洁性，通过计算原始响应与摘要之间的压缩比例以及词移除压缩来量化冗余内容。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常生成冗长且充满冗余细节的响应，这会降低清晰度和用户满意度，并增加模型开发者的成本，特别是那些按输出token数量收费的专有模型。

Method: 该方法通过三个计算的平均值来量化非必要内容：i) 原始响应与LLM抽象摘要之间的压缩比；ii) 原始响应与LLM提取摘要之间的压缩比；iii) 词移除压缩，即LLM在保持含义的同时尽可能移除非必要词语。

Result: 实验结果表明，所提出的指标能够有效识别LLM输出中的冗余内容，为对话AI系统中响应简洁性的自动评估提供了实用工具。

Conclusion: 该无参考指标为评估LLM响应的简洁性提供了一种有效方法，无需依赖人工标注的黄金标准参考，有助于提高响应质量和降低使用成本。
...
TL;DR: 本文提出了软概念混合（SCM）训练方案，通过在训练中直接暴露软表示来弥合推理中的软概念与训练中的离散标记之间的差距，从而提升大语言模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通常通过生成离散标记进行推理，这可能限制其表达能力。虽然软推理显示了潜力，但模型是在离散标记上训练的，存在软概念推理与离散标记训练之间的差距。

Method: 提出软概念混合（SCM）训练方案：构建软概念向量（通过概率加权平均嵌入形成），将其混合到模型的隐藏状态中，并使用强化学习优化整个潜在推理过程。

Result: 在五个推理基准测试上的实验表明，SCM提高了大语言模型的推理性能，同时保持了稳定的训练动态。

Conclusion: SCM通过软概念感知训练有效提升LLMs的推理能力，弥合了软概念推理与离散标记训练之间的差距。
...
TL;DR: 本文提出了一种新的训练方法，通过将TRMs的潜在推理视为分类器自由引导和隐式策略改进算法，显著提高了训练效率，减少了18倍的前向传播次数并消除了停止机制，同时在ARC-1上达到24%准确率。


<details>
  <summary>Details</summary>
Motivation: 探索如何以最小改动进一步提高Tiny Recursive Models (TRMs)的效率，这些小型循环架构在复杂推理任务上已显示出优于大型语言模型的表现。

Method: 将TRMs的潜在推理框架化为分类器自由引导和隐式策略改进算法，提出了一种在训练期间为每个循环提供目标的新型训练方案。

Result: 方法显著提升了训练效率，总前向传播次数减少18倍，消除了停止机制，同时在ARC-1上仅用0.8M参数就达到24%准确率，优于大多数LLMs。
...
TL;DR: 本文研究了训练数据的统计特性与归纳头形成之间的关系，发现批次大小和上下文长度的组合可以预测归纳头的形成点，表面二元重复频率和可靠性强烈影响归纳头形成，并确定了这两个值的精确帕累托边界。
...
Method: 研究分析了自然和合成数据的统计特性，通过结合批次大小和上下文长度的简单方程来预测归纳头形成点，并考察了表面二元重复频率和可靠性对归纳头形成的影响。

Result: 发现批次大小和上下文长度的组合可以准确预测归纳头形成点；表面二元重复频率和可靠性强烈影响归纳头形成，并确定了这两个值的精确帕累托边界；当频率和可靠性高时，局部依赖关系足以形成归纳头，但当这些值低时，类别性和边际分布形状变得重要。

Conclusion: 训练数据的统计特性，特别是表面二元重复频率和可靠性，对归纳头的形成具有决定性影响，这为理解语言模型上下文学习能力的形成机制提供了重要见解。
...
TL;DR: 提出了一种新的论据感知量化摘要任务，旨在揭示对话中论点的声明-理由结构，并通过量化指标衡量论据强度。开发了ARQUSUMM框架，利用LLM少样本学习识别句子内的命题及其声明-理由关系，并采用论据结构感知聚类算法进行论据聚合和量化支持。


<details>
  <summary>Details</summary>
Motivation: 在线对话平台上的争议性话题日益增多，需要不仅总结多样论点，还要揭示其理由和论证结构。现有研究要么忽视对话的论证性质，要么未能深入解析句子内的论据结构。

Method: ARQUSUMM框架：1）利用基于论证理论的LLM少样本学习识别句子内的命题及其声明-理由关系；2）采用论据结构感知聚类算法聚合论据并量化支持度。

Result: 实验表明ARQUSUMM优于现有对话和量化摘要模型，生成的摘要能更好地表示论据结构，对用户更有帮助，具有高文本质量和量化准确性。

Conclusion: 提出的论据感知量化摘要任务和ARQUSUMM框架能有效揭示对话中的深层论据结构，为理解在线讨论的论证动态提供了新方法。
...
TL;DR: 本研究提出了一种监督微调方法来增强领域特定信息提取能力，针对湖南历史名人领域设计了细粒度的模式引导指令模板，并在四个大型语言模型上进行参数高效指令微调，显著提升了信息提取性能。
...
Method: 设计针对湖南历史名人领域的细粒度模式引导指令模板，构建指令调优数据集，对Qwen2.5-7B、Qwen3-8B、DeepSeek-R1-Distill-Qwen-7B和Llama-3.1-8B-Instruct四个公开大模型进行参数高效指令微调。

Result: 所有模型在微调后都表现出显著的性能提升，其中Qwen3-8B表现最佳，在100个样本和50次训练迭代后达到89.3866分。
...
TL;DR: 该研究构建了视觉说服力判断数据集，探索视觉语言模型对视觉说服因素的理解能力，发现模型存在召回偏向，高层次的语义对齐是最强预测因子，而对象导向的推理能显著提升性能。
...
Result: 实验分析显示VLMs存在召回偏向，过度预测高说服力；对低/中层次特征的判别能力弱；高层次语义对齐是最强预测因子；对象导向的推理能显著提升精确率和F1分数。
...
TL;DR: 本文提出了一个可解释的自动评分框架AnalyticScore，通过四个可解释性原则(FGTI)和基于LLM的特征提取方法，在保持高精度的同时提供透明评分过程。
...
Result: 在ASAP-SAS数据集的10个项目上，AnalyticScore的评分准确率超过许多不可解释方法，平均仅比不可解释SOTA方法低0.06 QWK，且特征化行为与人类标注者高度一致。

Conclusion: AnalyticScore框架证明了实现可解释自动评分的可行性，为未来研究提供了基准参考，在保持高准确性的同时提供了透明评分过程。
...
TL;DR: MUCH是首个针对声明级不确定性量化的基准测试，包含4,873个样本，覆盖四种欧洲语言和四个开源LLM，提供24个生成对数概率，并提出高效的确定性分割算法。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型可靠性不足的问题，通过声明级不确定性量化来提升模型可信度，并为未来方法提供公平可复现的评估基准。

Method: 构建多语言数据集，发布每个token的24个生成对数概率，提出仅需0.2%LLM生成时间的确定性分割算法，支持实时监控。

Result: 创建了包含4,873个样本的基准测试，覆盖英语、法语、西班牙语和德语，支持四种开源LLM，分割算法效率显著提升。

Conclusion: 当前方法在性能和效率方面仍有很大改进空间，MUCH基准为未来声明级不确定性量化研究提供了现实部署条件下的评估标准。
...
TL;DR: 本文提出使用上下文压缩作为预训练任务来改进LLM的文本表示能力，相比基于token级预测的方法，该方法能生成更优的文本表示，并在多种任务中表现更好且更样本高效。


<details>
  <summary>Details</summary>
Motivation: 现有LLM主要针对因果建模和下一token预测优化，不适合生成整体文本表示。虽然已有研究引入预训练任务来适配LLM，但大多依赖token级预测目标，存在局限性。

Method: 提出上下文压缩作为预训练任务，让模型学习生成紧凑的记忆token来替代整个上下文进行下游序列预测。结合对比学习进一步优化表示质量。

Result: 实验表明精心设计的压缩目标能显著提升LLM文本表示，优于基于token级预训练任务的模型。LLM2Comp模型在广泛任务中超越当代LLM文本编码器，且更样本高效，所需训练数据显著减少。

Conclusion: 上下文压缩是LLM无监督适配的有效预训练任务，能产生强大的文本表示模型，在性能和效率上都优于现有方法。
...
TL;DR: 本文提出了LangMark，一个用于英语到七种语言翻译的多语言自动后编辑数据集，包含206,983个三元组（源文本、机器翻译输出、人工后编辑结果），旨在解决缺乏针对NMT输出的大规模多语言数据集的问题。
...
Method: 创建并发布LangMark数据集，包含英语到巴西葡萄牙语、法语、德语、意大利语、日语、俄语和西班牙语的206,983个人工标注的三元组，由专业语言学家标注。

Result: 实验表明，使用少量提示的大型语言模型能够有效执行自动后编辑，改进领先的商业甚至专有机器翻译系统。

Conclusion: LangMark这一新资源将促进自动后编辑系统未来的开发和评估。
...
TL;DR: 本文介绍了用于微调PLLuM项目中波兰大型语言模型的指令数据集，包括有机、转换和合成指令的功能分类，并分享了关于使用人工编写与合成指令数据集在语言适应方面的观察。


<details>
  <summary>Details</summary>
Motivation: 为PLLuM项目中的波兰大型语言模型开发指令数据集，探索不同类型指令在语言模型适应中的影响，并为其他LLM的类似数据集开发提供指导。

Method: 构建包含有机、转换和合成指令的功能分类数据集，分析人工编写与合成指令在语言适应中的差异。

Result: 开发了PLLuM指令语料库(PLLuMIC)的代表性子集，为其他LLM的指令数据集开发提供了参考框架。

Conclusion: 指令数据集的功能分类和类型分析对LLM的语言适应具有重要意义，发布的PLLuMIC子集可作为其他类似项目开发的指导资源。
...
TL;DR: 本文提出了一种基于方面的因果弃权框架（ABCA），通过因果推理分析LLM内部知识多样性，实现早期弃权决策，解决了传统后生成弃权方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有弃权方法依赖后生成信号，无法预先防止不可靠响应。LLM从不同来源获取的知识具有多面性，这种内部知识多样性可用于评估知识可靠性。

Method: ABCA框架通过因果推理分析LLM参数知识中的方面多样性（如学科、法律背景、时间框架），估计条件因果效应来评估查询相关知识的可靠性，支持两种弃权类型：方面效应不一致（知识冲突）和方面效应一致支持弃权（知识不足）。

Result: 在标准基准测试中，ABCA提高了弃权可靠性，实现了最先进的性能，并增强了弃权决策的可解释性。

Conclusion: ABCA通过利用LLM内部知识多样性的因果分析，为早期可靠弃权提供了有效框架，显著优于现有方法。
...
TL;DR: 本文提出了一种注意力引导特征融合（AGFF）模型，将统计特征和语义特征结合在统一框架中，通过注意力机制动态确定各特征类型的相对重要性，在新闻分类任务中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 传统统计方法能捕捉词级重要性但缺乏上下文语义理解，而现代深度学习方法理解语义但可能忽略简单高效的统计指标。需要结合两种特征类型的互补优势来提升新闻分类性能。

Method: 提出注意力引导特征融合（AGFF）模型，使用基于注意力的机制动态确定统计特征和语义特征的相对重要性，在统一框架中进行特征融合。

Result: 在基准新闻数据集上的评估表明，AGFF模型相比传统统计模型和纯语义深度学习模型具有优越性能，消融研究验证了融合过程中各组成部分的贡献。

Conclusion: 策略性地整合多样化特征类型能显著提升分类准确性，该模型能平衡并利用统计和语义表示的互补优势，是实际新闻分类任务的有效解决方案。
...
TL;DR: 本文提出了一种名为\name的层剪枝框架，通过可微分掩码优化和熵感知知识蒸馏，在保持性能的同时实现高效训练和推理加速。
...
Result: 在多种模型架构和基准测试中表现出优越性。在Qwen3-32B上剪枝25%层时，在MATH-500上达到96%准确率（仅比原始模型96.8%下降0.8%），优于现有SOTA方法（95%），推理速度提升1.33倍，仅消耗0.5B tokens（占后训练数据量的0.5%）。

Conclusion: \name框架能够有效解决层剪枝中的性能、训练成本和推理效率问题，为大规模语言模型的部署提供了实用的压缩方案。
...
TL;DR: PARROT框架用于测量LLM在权威和说服压力下的准确性退化，通过双盲评估隔离因果效应，量化置信度变化，并系统分类失败模式。研究发现先进模型抗压能力强，而老旧/小模型出现严重认知崩溃。


<details>
  <summary>Details</summary>
Motivation: 研究LLM在社交压力下的顺从性现象，特别是权威和说服导致的准确性退化问题，为安全部署提供评估框架。

Method: 使用PARROT框架：(i)双盲评估比较中性问题和权威错误版本，(ii)基于对数似然的置信度校准跟踪，(iii)八状态行为分类法系统分类失败模式。评估22个模型在13个领域的1302个MMLU式选择题。

Result: 先进模型(GPT-5, GPT-4.1, Claude Sonnet 4.5)顺从率低(≤11%)且准确性损失小，而老旧/小模型(GPT-4:80%, Qwen 2.5-1.5B:94%)出现严重认知崩溃。国际法和全球知识领域脆弱，基础数学相对稳健。

Conclusion: 抵抗过度拟合压力应作为与准确性、伤害避免和隐私同等重要的主要目标，确保LLM在现实世界中的安全部署。
...
TL;DR: MirageTVQA是一个新的基准测试，旨在评估视觉语言模型在包含视觉噪声的多语言表格问答任务中的表现，揭示了现有模型在视觉噪声处理和多语言推理方面的严重缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有表格问答数据集（如WikiTableQuestions和FinQA）主要是英语且表格格式完美，与现实世界复杂场景存在显著差距，需要创建更贴近实际的评估基准。

Method: 构建包含近60,000个问答对的MirageTVQA基准测试，涵盖24种语言，表格包含模拟扫描文档的视觉噪声，挑战模型的多语言和视觉噪声处理能力。

Result: 评估显示领先的VLM模型在视觉噪声下性能下降超过35%，且存在明显的英语优先偏见，其他语言上的推理能力无法有效迁移。

Conclusion: MirageTVQA为衡量和推动更鲁棒的表格推理VLM模型提供了重要基准，揭示了当前模型在现实场景应用中的局限性。
...
Method: 通过翻译专家进行人工翻译和适应，同时设计针对爱沙尼亚语特点的详细提示进行机器翻译实验。

Result: 人工翻译的爱沙尼亚语数据集上模型性能略低于原始英语测试集，机器翻译数据表现更差，提示工程对翻译质量和模型准确性的改进有限。

Conclusion: 语言专家参与数据集翻译和适应对于确保语言能力和推理评估的可靠性和可解释性至关重要。
...
TL;DR: 基于语料库研究普通话单音节词在自然对话中的音高轮廓实现，发现词义对音高有显著影响。即使控制多种变量，词义仍是音高实现的重要预测因素，语义效应强于词汇本身。


<details>
  <summary>Details</summary>
Motivation: 研究普通话单音节词在自然对话中的音高实现，特别关注词义对音高轮廓的影响，挑战传统普通话声调理论。

Method: 使用广义加性模型分解观察到的音高轮廓，分析不同控制变量和语义预测因子的影响，通过语境化嵌入预测单个词标记的音高轮廓。

Result: 词义效应强于词汇效应，异形同音词具有不同音高轮廓，基于语境化嵌入的音高预测准确率显著超过置换基线。

Conclusion: 研究结果挑战传统普通话声调理论，但符合判别词典模型的理论框架，表明分布语义学在语音学中具有重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 利用多模态表示通过将语言置于视觉上下文中来改进自然语言推理，旨在克服文本偏见和表面启发式问题。
...
TL;DR: 本文提出了选择性RoPE，一种输入依赖的旋转位置嵌入机制，它通过任意角度旋转来泛化RoPE，适用于线性和softmax变压器，并在语言建模和序列任务中表现出改进性能。


<details>
  <summary>Details</summary>
Motivation: 受线性变压器中选择性门控能改善语言相关任务的启发，作者希望将输入依赖的选择性机制引入到位置编码中，以改进传统固定角度旋转的RoPE方法。

Method: 提出了选择性RoPE机制，允许输入依赖的任意角度旋转，揭示了softmax注意力中隐藏的旋转结构，并在状态空间模型和门控线性变压器中分析了实部和虚部的作用。

Result: 通过将门控变压器配备选择性RoPE，在语言建模和困难序列任务（如复制、状态跟踪和检索）上表现出性能改进。

Conclusion: 选择性RoPE通过输入依赖的旋转机制有效改进了位置编码，揭示了注意力机制中的隐式位置结构，并在多个任务中验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 多项选择题的选项可能泄露可被利用的信号，导致准确率指标不能真实反映模型能力，并鼓励在强化微调中出现答案猜测行为。

Method: 提出ReVeL框架，将多项选择题分类并重写为开放式问题，采用不同的重写和验证方案，使用GRPO对Qwen2.5-VL模型进行微调。

Result: 在ReVeL-OpenQA上训练的模型在多项选择基准测试中保持相同准确率，同时将开放式问题准确率提高约6个百分点，在评估中揭示MCQA基准测试中高达20个百分点的分数膨胀。
...
Method: 提出MR-RLVR方法，通过"掩码后填充"和"步骤重排序"构建过程级自监督奖励，训练流程包括两个阶段：先在数学计算和证明数据上进行自监督训练，然后在仅结果可验证的数学计算数据集上进行RLVR微调。

Result: 在Qwen2.5-3B和DeepSeek-R1-Distill-Qwen-1.5B上实现MR-RLVR，在AIME24、AIME25、AMC23和MATH500数据集上评估，在固定采样和解码预算下，相比原始RLVR平均相对提升：Pass@1 +9.86%、Pass@5 +5.27%、Pass@8 +4.00%。
...
### [125] [Stable diffusion models reveal a persisting human and AI gap in visual creativity](https://arxiv.org/abs/2511.16814)
...
### [126] [Cognitive BASIC: An In-Model Interpreted Reasoning Language for LLMs](https://arxiv.org/abs/2511.16837)
*Oliver Kramer*

Main category: cs.AI

TL;DR: Cognitive BASIC是一种基于BASIC风格的提示语言和模型内解释器，将大型语言模型的推理过程结构化，形成显式的逐步执行轨迹。


<details>
  <summary>Details</summary>
Motivation: 受复古BASIC语言简洁性的启发，作者希望为LLM推理创建一个可解释的认知控制层，使多步推理过程更加透明。
...
Conclusion: Cognitive BASIC能够有效结构化LLM推理过程，实现透明的多步推理，在知识提取、冲突检测和推理任务中表现良好。
...
### [127] [Fantastic Bugs and Where to Find Them in AI Benchmarks](https://arxiv.org/abs/2511.16842)
...
TL;DR: 提出了一种利用响应模式统计分析来识别基准测试中无效问题的框架，通过统计异常检测标记潜在问题供专家审查，在9个基准测试中达到84%的精确率，并结合LLM初审减少人工工作量。


<details>
  <summary>Details</summary>
Motivation: 基准测试对AI发展至关重要，但无效问题会破坏其可靠性。手动识别和修正数千个基准问题既不可行也是可靠评估的关键瓶颈。

Method: 基于AI评估中常用的核心假设——平均分足以总结模型性能，构建统计分析方法检测响应模式的异常，当项目统计值超出预期范围时标记为潜在问题，并引入LLM初审机制。

Result: 在9个广泛使用的基准测试中，该方法指导专家审查识别问题问题的精确率高达84%，LLM初审进一步减少了人工工作量。

Conclusion: 该框架为系统基准修订提供了高效且可扩展的解决方案，通过统计分析和LLM辅助显著提升了基准测试的可靠性评估效率。
...
### [128] [Hybrid Differential Reward: Combining Temporal Difference and Action Gradients for Efficient Multi-Agent Reinforcement Learning in Cooperative Driving](https://arxiv.org/abs/2511.16916)
...
<details>
  <summary>Details</summary>
Motivation: 传统状态奖励函数在多车协同驾驶的高频连续控制任务中存在奖励差异消失问题，导致策略梯度信噪比低，严重影响算法收敛和性能提升。

Method: 提出混合差分奖励机制，包含两个互补组件：基于全局势函数的时间差分奖励，利用势能演化趋势确保最优策略不变性和长期目标一致性；动作梯度奖励直接衡量动作的边际效用，提供高信噪比的局部指导信号。

Result: 使用在线规划和多智能体强化学习算法的广泛实验表明，HDR机制显著提高了收敛速度和策略稳定性，引导智能体学习到能有效平衡交通效率和安全的高质量协同策略。

Conclusion: HDR机制成功解决了多车协同驾驶中奖励信号失效的问题，为高频连续控制任务提供了有效的奖励设计框架。
...
### [129] [MirrorMind: Empowering OmniScientist with the Expert Perspectives and Collective Knowledge of Human Scientists](https://arxiv.org/abs/2511.16997)
*Qingbin Zeng,Bingbing Fan,Zhiyu Chen,Sijian Ren,Zhilun Zhou,Xuhua Zhang,Yuanyi Zhen,Fengli Xu,Yong Li,Tie-Yan Liu*

Main category: cs.AI

TL;DR: 本文提出MirrorMind分层认知架构，通过整合个体认知轨迹和集体学科记忆，解决现有AI科学家方法忽视科学知识生产的社会性和历史性问题。


<details>
  <summary>Details</summary>
Motivation: 现有AI科学家方法将科学发现视为孤立的优化过程，忽略了知识生产的社会历史本质。人类科学洞察来自个体认知轨迹和集体学科记忆两个相互关联的来源，而现有LLM难以表示这些结构化的认知和社会背景。

Method: 提出MirrorMind分层认知架构：个体层面构建研究者的情景、语义和人格记忆；领域层面将集体知识映射为结构化学科概念图；跨学科层面作为正交编排引擎。该架构将记忆存储与智能执行分离。

Result: 在四个综合任务中评估：作者级认知模拟、互补推理、跨学科协作促进、多智能体科学问题解决。结果表明MirrorMind超越了简单事实检索，实现了结构化、个性化和洞察生成的科学推理。

Conclusion: 通过整合个体认知深度与集体学科广度，MirrorMind能够实现更具结构性和洞察力的科学推理，推动AI科学家向更接近人类科学思维的方向发展。
...
TL;DR: 论文重新解释了扩散模型在逆问题求解中的作用，提出DAPS++方法将扩散阶段与数据驱动优化解耦，通过更直接地利用似然项指导推理，在减少计算成本的同时实现稳健的图像重建性能。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯视角下的基于分数的扩散方法在解决逆问题时，先验提供的指导有限，重建主要由测量一致性项驱动，导致推理过程与扩散动力学实际解耦。需要重新理解扩散在逆问题求解中的结构。

Method: 将扩散重新解释为期望最大化(EM)框架中的初始化阶段，提出DAPS++方法，使似然项能更直接地指导推理，同时保持数值稳定性。该方法将扩散阶段与数据驱动优化完全解耦。

Result: DAPS++需要更少的函数评估次数和测量优化步骤，在多种图像恢复任务中实现了高计算效率和稳健的重建性能。

Conclusion: 通过重新理解扩散在逆问题求解中的作用，DAPS++提供了一种更有效的框架，解释了为什么统一的扩散轨迹在实践中仍然有效，同时显著提高了计算效率。
...
### [131] [MIR: Efficient Exploration in Episodic Multi-Agent Reinforcement Learning via Mutual Intrinsic Reward](https://arxiv.org/abs/2511.17165)
*Kesheng Chen,Wenjian Luo,Bang Zhang,Zeping Yin,Zipeng Ye*

Main category: cs.AI

TL;DR: 本文提出了一种名为MIR的相互内在奖励方法，用于解决多智能体强化学习中稀疏奖励（特别是情节奖励）的挑战。该方法通过激励智能体探索影响队友的行为来促进团队探索，并在MiniGrid-MA环境中验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中情节奖励面临两个主要挑战：联合行动轨迹的指数级稀疏性，以及现有方法未能充分考虑影响团队状态的联合行动。

Method: 提出MIR（相互内在奖励）方法，激励个体智能体探索能够影响队友的行为，并与原始策略结合以刺激团队探索。

Result: 在扩展的MiniGrid-MA环境中，MIR方法相比现有最先进方法表现出更优越的性能。

Conclusion: MIR是一种简单而有效的增强策略，能够显著提升多智能体强化学习在稀疏奖励环境下的算法性能。
...
### [132] [Designing Domain-Specific Agents via Hierarchical Task Abstraction Mechanism](https://arxiv.org/abs/2511.17198)
*Kaiyu Li,Jiayu Wang,Zhi Wang,Hui Qiao,Weizhan Zhang,Deyu Meng,Xiangyong Cao*

Main category: cs.AI

TL;DR: 本文提出了一种基于层次任务抽象机制(HTAM)的新型多智能体框架EarthAgent，专门用于解决遥感等专业领域中结构化工作流的挑战，显著优于现有单智能体和多智能体系统。


<details>
  <summary>Details</summary>
Motivation: 通用LLM驱动智能体在需要严格结构化工作流的专业领域(如遥感)表现不佳，这些领域需要专门工具和多步骤程序，对通用方法构成重大挑战。

Method: 提出层次任务抽象机制(HTAM)，将多智能体系统构建为反映领域固有任务依赖图的逻辑层次结构，通过任务中心架构确保程序正确性，将复杂问题分解为顺序层。

Result: 构建了EarthAgent系统并在GeoPlan-bench基准测试中验证，实验表明EarthAgent在工具选择、路径相似性和逻辑完整性方面显著优于现有系统。

Conclusion: 将智能体架构与领域固有任务结构对齐是构建鲁棒可靠的专业自主系统的关键步骤。
...
### [133] [Agentifying Agentic AI](https://arxiv.org/abs/2511.17332)
*Virginia Dignum,Frank Dignum*

Main category: cs.AI

TL;DR: 本文主张将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具作为实现Agentic AI的基础，通过将自适应数据驱动方法与结构化推理协调模型相结合，构建具备透明度、协作性和问责制的智能体系统。


<details>
  <summary>Details</summary>
Motivation: Agentic AI需要持续的自主性、推理和交互能力，但当前关于智能体的假设需要辅以明确的认知、协作和治理模型。

Method: 利用AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具，将自适应数据驱动方法与结构化推理协调模型相结合。

Result: 提出了一种连接形式理论和实践自主性的智能体视角，为实现具备能力、灵活性、透明度、协作性和问责制的智能体系统奠定了基础。

Conclusion: AAMAS社区的概念工具为构建透明、协作和负责任的Agentic系统提供了坚实基础，通过形式理论与实践自主性的结合，能够实现更可靠的智能体系统。
...
### [134] [That's not natural: The Impact of Off-Policy Training Data on Probe Performance](https://arxiv.org/abs/2511.17408)
*Nathalie Kirch,Samuel Dower,Adrians Skapars,Ekdeep Singh Lubana,Dmitrii Krasheninnikov*

Main category: cs.AI

TL;DR: 本文系统评估了使用合成数据和离策略数据对LLM行为探测泛化能力的影响，发现响应生成策略显著影响探测性能，离策略数据泛化成功可预测同策略泛化成功，但训练数据域偏移会导致更大性能下降。


<details>
  <summary>Details</summary>
Motivation: 由于许多行为的自然示例稀少，研究人员不得不依赖合成或离策略的LLM响应来训练探测模型，需要系统评估这些数据对探测泛化能力的影响。

Method: 在八种不同LLM行为上测试线性和注意力探测模型，使用多个LLM，比较不同响应生成策略对探测性能的影响，评估从离策略数据到同策略数据的泛化能力。

Result: 响应生成策略显著影响探测性能，离策略数据成功泛化可预测同策略泛化成功；训练数据域偏移导致更大性能下降，不同域测试分数始终低于同域测试。

Conclusion: 在缺乏同策略数据时，使用同域离策略数据比使用不同域同策略数据产生更可靠的探测，强调需要能更好处理LLM监控中分布偏移的方法。
...
### [135] [SRA-CP: Spontaneous Risk-Aware Selective Cooperative Perception](https://arxiv.org/abs/2511.17461)
...
<details>
  <summary>Details</summary>
Motivation: 现有协作感知方法传输大量与驾驶安全无关的数据，超出通信带宽限制，且依赖预定义通信伙伴，不适合动态交通环境

Method: 采用去中心化协议，车辆持续广播轻量级感知覆盖摘要，仅在检测到风险相关盲区时启动针对性协作；包含感知风险识别模块和选择性信息交换融合模块

Result: 相比通用协作感知方法，安全关键对象检测精度损失小于1%，通信带宽仅需20%；相比现有选择性方法，感知性能提升15%

Conclusion: SRA-CP框架有效解决了协作感知中的通信带宽和动态环境适应性问题，实现了高效安全的信息共享
...
### [136] [RubiSCoT: A Framework for AI-Supported Academic Assessment](https://arxiv.org/abs/2510.17309)
...
Conclusion: RubiSCoT通过一致、可扩展和透明的评估，有潜力优化学术评估流程。
...
### [137] [Avoiding Quality Saturation in UGC Compression Using Denoised References](https://arxiv.org/abs/2511.16876)
*Xin Xiong,Samuel Fernández-Menduiña,Eduardo Pavez,Antonio Ortega,Neil Birkbeck,Balu Adsumilli*

Main category: eess.IV

TL;DR: 本文提出了一种基于失真饱和检测的方法来解决用户生成内容压缩中的质量饱和问题，通过计算去噪视频的MSE来避免在饱和区域进行编码，相比传统方法可节省8%-20%的比特率。


<details>
  <summary>Details</summary>
Motivation: 传统编解码器在压缩用户生成内容时会产生质量饱和问题，即增加比特率无法改善视觉质量反而保留输入伪影。现有方法通过重复评估非参考指标来检测质量饱和，效率低下。

Method: 提出两种失真饱和检测方法：DSD（基于输入相关阈值）和RDSD（基于低复杂度压缩估计拉格朗日乘子），通过计算去噪视频的MSE来检测失真饱和，避免在饱和区域编码。

Result: 实验表明，使用AVC编解码器时，避免在饱和区域编码可在多种非参考指标下实现8%-20%的BD-rate节省。

Conclusion: 所提出的失真饱和检测方法可作为预处理步骤，帮助标准兼容编解码器避免用户生成内容压缩中的质量饱和问题，显著提升压缩效率。
...
### [138] [MRI Super-Resolution with Deep Learning: A Comprehensive Survey](https://arxiv.org/abs/2511.16854)
...
TL;DR: 本论文综述了磁共振成像超分辨率技术的最新进展，特别关注深度学习方法，涵盖理论基础、架构设计、学习策略、基准数据集和性能指标，并提出了系统分类法。


<details>
  <summary>Details</summary>
Motivation: 高分辨率磁共振成像在临床和研究中至关重要但成本高昂且受技术限制，超分辨率技术通过从低成本低分辨率扫描生成高分辨率图像，有望在不增加硬件的情况下提高诊断准确性和效率。

Method: 从计算机视觉、计算成像、逆问题和磁共振物理学的角度审视基于深度学习的MRI超分辨率方法，提出系统分类法来分类这些方法，并对已建立和新兴的超分辨率技术进行深入研究。

Result: 提供了全面的MRI超分辨率技术综述，包括理论框架、方法分类和性能评估，并收集了开源资源、工具和教程。

Conclusion: MRI超分辨率是一个有前景的研究方向，但仍面临挑战，需要社区进一步解决，论文为相关研究提供了系统指导和资源支持。
...
### [139] [MedImageInsight for Thoracic Cavity Health Classification from Chest X-rays](https://arxiv.org/abs/2511.17043)
...
TL;DR: 本研究评估了MedImageInsight基础模型在胸部X光片正常/异常二元分类中的应用，比较了微调模型和迁移学习两种方法，在真实临床数据上取得了与CheXNet相当的性能。


<details>
  <summary>Details</summary>
Motivation: 胸部X光片是最常用的胸部诊断成像方式，但日益增长的影像量和放射科医生工作量对及时解读提出了挑战，需要自动化分类系统来支持分诊并减轻医生负担。

Method: 采用两种方法：1）对MedImageInsight进行端到端微调用于分类；2）使用该模型作为特征提取器，结合传统机器学习分类器构建迁移学习流程。实验使用ChestX-ray14数据集和真实临床数据。

Result: 微调分类器表现最佳，ROC-AUC达到0.888，校准性能优于迁移学习模型，性能与CheXNet等成熟架构相当。

Conclusion: 医学影像基础模型在减少任务特定训练需求的同时能保持诊断可靠性，未来将扩展到多标签病理分类以提供初步诊断解读。
...
### [140] [OmniLens++: Blind Lens Aberration Correction via Large LensLib Pre-Training and Latent PSF Representation](https://arxiv.org/abs/2511.17126)
*Qi Jiang,Xiaolong Qian,Yao Gao,Lei Sun,Kailun Yang,Zhonghua Yi,Wenyong Li,Ming-Hsuan Yang,Luc Van Gool,Kaiwei Wang*

Main category: eess.IV

TL;DR: OmniLens++是一个用于盲镜头像差校正的框架，通过扩展镜头库设计规范和引入潜在PSF表示，解决了现有方法在数据扩展性和光学退化先验指导方面的挑战，在多样化像差校正中展现了最先进的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的镜头库预训练管道在泛化能力方面面临两个主要挑战：数据扩展困难和缺乏光学退化先验指导，这限制了其在处理多样化未知光学退化时的性能。

Method: 1) 扩展设计规范增加镜头源退化多样性，通过量化光学退化的空间变化模式和严重程度来采样更均匀的分布；2) 提出潜在PSF表示(LPR)，使用VQVAE框架学习镜头库PSF的潜在特征，通过建模光学退化过程来约束退化先验的学习。

Result: 在真实世界镜头和合成镜头库的多样化像差实验中，OmniLens++在盲像差校正中展现了最先进的泛化能力。AODLibpro被验证为可扩展的基础，能在多样化像差中进行更有效的训练，LPR能进一步挖掘大规模镜头库的潜力。

Conclusion: OmniLens++通过改进数据可扩展性和引入光学退化先验指导，显著提升了盲镜头像差校正的泛化性能，为处理多样化光学退化提供了有效的解决方案。
