...
Motivation: 开发一个专门针对韩语语言文化背景的视觉语言模型，特别强调推理能力和智能体能力，以满足韩国特定语境下的AI应用需求。

Method: 模型采用两阶段训练：首先预训练以增强推理能力，然后后训练以支持多模态理解、增强推理、智能体行为和人类偏好对齐。
...
<details>
  <summary>Details</summary>
Motivation: 现有射频无人机检测数据集存在稀缺性和多样性有限的问题，缺乏标准化基准来支持可重复的研究和模型比较。

Method: 通过结合真实世界采集数据（罗文大学校园和受控RF笼设施）和系统化合成变体生成，采用精确控制信噪比、注入干扰发射器、应用频率偏移和标签一致的边界框变换等增强技术。

Result: 创建了一个包含多种当代无人机模型的大规模基准数据集CDRF，并发布了开源工具链用于数据生成、预处理、增强和评估，支持分类、开放集识别和目标检测任务。

Conclusion: CDRF通过提供全面的基准数据集和工具链，旨在加速鲁棒、可泛化的射频感知模型的发展，支持标准化基准测试和可重复的研究流程。
...
TL;DR: Klear是一个音频-视频联合生成系统，通过单塔架构、渐进多任务训练策略和高质量密集标注数据集，解决了现有方法中的音视频不同步、唇语对齐差和单模态退化等问题，在多个任务上大幅超越先前方法。


<details>
  <summary>Details</summary>
Motivation: 当前音频-视频联合生成方法存在音视频不同步、唇语对齐差、单模态退化等问题，这些问题源于弱音视频对应建模、有限泛化能力和高质量密集标注数据稀缺。需要从模型架构、训练策略和数据构建三个维度系统解决这些问题。

Method: 1. 架构：采用单塔设计，统一DiT块和Omni-Full Attention机制，实现紧密音视频对齐和强可扩展性。2. 训练：渐进多任务策略，包括随机模态掩码、跨任务联合优化和多阶段课程学习，增强鲁棒表示和防止单模态崩溃。3. 数据：构建首个大规模音频-视频密集标注数据集，开发自动化数据构建流水线，标注和过滤数百万高质量、严格对齐的音视频-字幕三元组。

Result: Klear在大规模数据集上扩展良好，在联合和单模态设置中都能生成高保真、语义和时间对齐、遵循指令的内容，在分布外场景中表现出强泛化能力。在所有任务上大幅超越先前方法，性能与Veo 3相当。

Conclusion: Klear通过系统性的架构、训练和数据创新，解决了音频-视频联合生成的关键挑战，为下一代音视频合成提供了统一、可扩展的路径，在多个维度上实现了显著性能提升。
...
### [4] [Towards Real-world Lens Active Alignment with Unlabeled Data via Domain Adaptation](https://arxiv.org/abs/2601.03718)
*Wenyong Lia,Qi Jiang,Weijian Hu,Kailun Yang,Zhanjun Zhang,Wenjun Tian,Kaiwei Wang,Jian Bai*

Main category: cs.CV

TL;DR: DA3通过领域自适应方法，使用少量未标记的真实图像增强模拟基线，将主动对准精度提升46%，接近使用精确标记真实数据的性能，同时减少98.7%的设备数据收集时间。


<details>
  <summary>Details</summary>
Motivation: 主动对准是高精度光学系统大规模自动化组装的关键技术。虽然基于光学模拟的数字孪生管道能生成大规模标记数据，但复杂成像条件导致模拟与真实图像之间存在领域差距，限制了模拟训练模型的泛化能力。

Method: 提出领域自适应主动对准(DA3)，使用自回归领域变换生成器和基于对抗的特征对齐策略，通过自监督学习提取真实领域信息，从而提取领域不变的图像退化特征以实现鲁棒的对准预测。

Result: 在两个镜头类型上的实验显示，DA3比纯模拟管道精度提高46%，接近使用3个镜头样本精确标记真实数据获得的性能，同时将设备数据收集时间减少98.7%。

Conclusion: 领域自适应能有效赋予模拟训练模型鲁棒的真实世界性能，验证了数字孪生管道作为显著提升大规模光学组装效率的实用解决方案。
...
TL;DR: 提出SuPLoRA方法，通过超类型-子类型概念层次结构组织被擦除概念，实现高效、有效的群体概念抑制，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法在需要擦除多个概念时效率低下且效果不佳，每个概念都需要单独微调参数，且可能降低整体生成质量。

Method: 1) 构建超类型-子类型概念层次结构，将语义相关概念分组；2) 提出SuPLoRA方法，在冻结的下投影矩阵中编码超类型概念信息，仅更新上投影矩阵进行擦除；3) 应用标准扩散正则化保护未掩码区域的去噪过程。

Result: 构建了更具挑战性的基准测试，涵盖名人、物体和色情内容等多个领域的概念同时擦除需求。理论分析证明了SuPLoRA在缓解生成性能退化方面的有效性。

Conclusion: 提出的超类型-子类型概念层次结构和SuPLoRA方法能够高效、有效地实现多概念擦除，同时保持生成质量，解决了现有方法在擦除多个概念时的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 研究VLA模型中VLM的选择和能力如何影响下游策略性能这一基础但缺乏系统研究的问题，挑战当前关于VLM能力直接转化为控制性能的常见假设。

Method: 提出VLM4VLA最小适应管道，仅使用少量新可学习参数将通用VLM转换为VLA策略，进行公平高效比较；在三个基准测试的多个下游任务上进行实证研究；通过微调VLM在七个辅助体现任务上研究具体体现能力的影响；进行模态级消融分析。

Result: VLM初始化相比从头训练有持续优势，但VLM的通用能力是下游任务表现的差预测指标；提升VLM在特定体现技能上的性能并不能保证更好的下游控制性能；视觉模块是主要性能瓶颈，向VLM视觉编码器注入控制相关监督能带来持续增益。

Conclusion: 标准VLM能力对有效体现控制是必要但不充分的，当前VLM预训练目标与体现动作规划需求之间存在持续领域差距，视觉编码器是关键改进方向。
...
TL;DR: 提出使用高阶张量（如三阶或更高阶中心矩）扩展PCA的协方差矩阵方法，以创建更精确的旋转不变形状描述符，用于分子形状描述、物体识别和形状相似性比较等应用。


<details>
  <summary>Details</summary>
Motivation: PCA使用协方差矩阵（二阶矩）只能通过椭球近似形状，但真实形状通常更复杂，需要更精确的描述方法。现有方法对复杂形状的描述能力有限，需要扩展到更高阶的矩来捕捉更丰富的形状特征。

Method: 扩展PCA的协方差矩阵方法到高阶张量：1）使用三阶或更高阶中心矩张量（如p_abc=E[(x_a-E[x_a])(x_b-E[x_b])(x_c-E[x_c])]）描述形状；2）结合多项式和高斯函数创建可解码的高精度形状描述符；3）推导这些高阶张量的旋转不变量。

Result: 该方法能够创建任意高精度的形状描述符，提供比传统PCA协方差矩阵更丰富的形状特征表示。这些描述符具有旋转不变性，能够准确描述复杂形状。

Conclusion: 高阶张量扩展为PCA提供了更强大的形状描述能力，可用于创建旋转不变的特征描述符，在分子形状分析、物体识别和形状相似性比较等应用中具有重要价值，避免了昂贵的旋转优化计算。
...
TL;DR: MMErroR是一个多模态基准测试，包含2,013个样本，每个样本嵌入单一连贯的推理错误，涵盖6个顶级领域下的24个子领域，用于评估视觉语言模型检测错误推理和分类错误类型的能力。
...
Method: 构建MMErroR多模态基准测试，包含2,013个嵌入单一推理错误的样本，涵盖6个顶级领域下的24个子领域，进行过程级、以错误为中心的评估。
...
TL;DR: 提出了一种新的可重光照3D高斯头部模型，能够在不需要OLAT数据的情况下，从单张或多张图像重建并重光照任何主体。


<details>
  <summary>Details</summary>
Motivation: 现有高质量重光照方法需要复杂的时间复用照明数据（如OLAT），限制了应用范围。需要一种能够从常规多视角数据中学习可重光照模型的方法。

Method: 采用两阶段设计：第一阶段从无OLAT的多视角数据学习平光照明下的3DGS头像；第二阶段学习从平光头像到基于物理的反射参数的映射，使用较小的OLAT数据集训练。

Result: 模型能够很好地泛化，将第一阶段学习的任何主体重光照，效果如同采集了OLAT数据。仅需单张图像即可拟合未见过的对象，支持新视角合成和重光照应用。

Conclusion: 提出了一种通用的可重光照3D高斯头部模型，无需OLAT数据即可实现高质量重光照，具有优秀的泛化能力和实用性。
...
TL;DR: 论文提出RiskCueBench视频风险预测基准，要求模型仅基于早期风险信号片段预测未来危险事件，而非使用完整视频序列，更贴近实际应用场景。


<details>
  <summary>Details</summary>
Motivation: 现有视频风险评估研究通常允许模型访问完整视频序列（包括事故本身），这大大降低了任务难度，无法真实反映实际应用场景中需要基于早期信号预测风险的需求。

Method: 引入RiskCueBench视频理解基准，对视频进行精心标注以识别风险信号片段（即指示潜在安全担忧的最早时刻），要求模型仅基于这些早期信号预测未来危险事件。

Result: 实验结果显示当前系统在理解动态演变情境和基于早期视觉信号预测未来风险事件方面存在显著能力差距，突显了视频风险预测模型实际部署面临的重要挑战。

Conclusion: RiskCueBench基准更好地反映了现实世界条件，揭示了当前视频风险预测系统的局限性，为开发更实用的风险预测模型提供了重要基准和挑战方向。
...
Method: 提出统一架构，使用空间域和频域特征的交叉注意力机制，结合血液检测模块，采用Swin Transformer/BERT或EfficientNet-B4/BERT等模型组合

Result: 在FF++数据集上达到99.80% AUC，在Celeb-DF上达到99.88% AUC（使用Swin Transformer和BERT）；使用EfficientNet-B4和BERT时分别为99.55%和99.38% AUC，具有良好的跨数据集泛化能力

Conclusion: 该架构在深度伪造检测方面超越了现有最佳方法，提供了统一的解决方案，并在多个数据集上表现出优异的性能和泛化能力
...
TL;DR: Eye-Q是一个多语言视觉文字谜题基准测试，要求模型通过深层推理而非表面识别来解决概念密集场景中的文字谜题，现有视觉语言模型在该基准上表现有限，最高准确率仅60.27%。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在标准基准测试中表现良好，但主要依赖表面识别而非深层推理。为了评估模型真正的复杂视觉理解能力，需要设计更具挑战性的任务，要求模型发现隐含视觉线索、生成和修正假设，并将感知证据映射到非字面概念。

Method: 提出Eye-Q多语言基准测试，包含1,343个视觉文字谜题。每个谜题展示一个概念密集的场景和简短描述，模型需要推断特定的目标单词或短语。谜题设计故意非结构化和线索隐含，包含干扰项和上下文关系，需要选择性注意、抽象和关联推理。基准涵盖英语、波斯语、阿拉伯语和跨语言谜题。

Result: 评估最先进的视觉语言模型显示存在显著性能差距，特别是在抽象和跨语言谜题上。模型在构建和搜索适当概念表示以进行灵活的图像到短语推理方面存在局限性，最高准确率仅为60.27%。

Conclusion: 视觉文字谜题是评估视觉语言模型深层推理能力的有效工具，现有模型在需要复杂视觉理解的任务上仍有很大提升空间，特别是在抽象推理和跨语言理解方面。
...
TL;DR: GAMBIT是一种新颖的多模态越狱框架，通过构建游戏化场景驱动模型主动完成有害内容生成，显著提高了对推理模型的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型的安全对齐机制在对抗性输入下仍然脆弱，且现有攻击方法主要关注增加视觉任务复杂度，未能有效利用模型自身的推理激励机制，导致对推理模型的攻击效果不佳。
...
Conclusion: 通过利用模型的推理激励机制和构建游戏化场景，GAMBIT能够有效绕过MLLMs的安全对齐机制，揭示了当前安全防御在对抗性推理攻击下的脆弱性。
...
TL;DR: FROST-Drive提出一种新的端到端自动驾驶架构，通过冻结预训练视觉语言模型的视觉编码器权重，保留其强大的泛化能力，直接迁移到驾驶任务中，显著优于完全微调的方法。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶模型通常对视觉编码器进行完全微调，这可能导致模型过度专注于训练数据而限制泛化能力。本文质疑这种训练范式的必要性，旨在探索如何更好地利用预训练视觉语言模型的广泛世界知识来提升驾驶模型的泛化性能。

Method: 提出FROST-Drive架构：1) 冻结预训练视觉语言模型的视觉编码器权重；2) 使用基于transformer的适配器进行多模态融合；3) 采用基于GRU的解码器生成平滑路径点；4) 设计专门优化Rater Feedback Score的自定义损失函数。

Result: 在Waymo Open E2E数据集（专门捕捉长尾场景的大规模数据集）上的实验表明，冻结编码器方法显著优于采用完全微调的模型，证明了保留VLM广泛知识比密集领域特定适应更有效。
...
Result: 约束低秩模型在泛化性能上显著优于高维模型，在均方根误差（RMSE）和排序精度之间达到最优平衡；嵌入空间可视化显示无监督地出现了语义类型聚类；冷启动场景验证了系统的实用性。

Conclusion: 该研究证明了约束低秩模型在大规模推荐系统中的有效性，能够从交互数据中捕捉深层结构关系，并通过可调评分参数有效管理流行度偏差与个性化亲和力之间的权衡。
...
TL;DR: ThinkRL-Edit：一个基于强化学习的图像编辑框架，通过解耦视觉推理和图像合成，使用思维链推理采样来提升指令驱动的图像编辑质量


<details>
  <summary>Details</summary>
Motivation: 当前指令驱动的图像编辑模型在视觉推理方面存在局限，导致推理密集型编辑任务表现不佳。现有强化学习方法面临三个关键挑战：推理探索受限、奖励融合偏差和不稳定的VLM指令奖励

Method: 提出ThinkRL-Edit框架，解耦视觉推理和图像合成，引入基于思维链的推理采样（包含规划和反思阶段），采用无偏链偏好分组策略，并用二进制检查表替代基于区间的VLM评分

Result: 实验表明该方法在推理密集型图像编辑任务上显著优于先前工作，能够生成更忠实于指令、视觉连贯且语义基础扎实的编辑结果

Conclusion: 通过解耦推理与合成、扩展推理探索范围、改进奖励机制，ThinkRL-Edit有效解决了现有RL图像编辑方法的局限性，为复杂推理编辑任务提供了更可靠的解决方案
...
TL;DR: 本文提出了一种无需训练的方法SDCD，通过引入结构扰乱视图进行对比解码，有效减少大型视觉语言模型中的物体幻觉问题。
...
Method: 提出了一种无需训练的方法SDCD（Structure-Disrupted Contrastive Decoding）。该方法通过引入打乱的结构扰乱视图，对输出分布进行对比校准。通过惩罚在结构缺失视图中仍保持高置信度的token，有效抑制了纹理驱动的偏差。

Result: 实验结果表明，SDCD在多个基准测试中显著减少了幻觉现象，并提升了LVLMs的整体多模态能力。

Conclusion: SDCD通过解决视觉编码过程中的结构偏差问题，有效缓解了大型视觉语言模型中的物体幻觉，为改善多模态理解提供了新的训练免费解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统面部表情追踪需要侵入式设备或复杂校准，限制了虚拟环境中自然表情交互的应用

Method: 采用蒸馏学习方法训练机器学习模型，结合合成和真实图像的多源异构数据；开发了轻量级采集装置（手机+定制VR头显）收集18k多样本数据；构建了鲁棒的可微分渲染管道自动提取表情标签
...
TL;DR: G2P方法通过将3D高斯泼溅的外观感知属性转移到点云，解决了点云语义分割中几何特征不足以区分形状相似但外观不同物体的问题，无需2D或语言监督即可实现更区分性和外观一致的分割。


<details>
  <summary>Details</summary>
Motivation: 点云的稀疏和不规则分布限制了外观证据，仅使用几何特征无法有效区分形状相似但外观（颜色、纹理、材质）不同的物体，需要引入外观感知信息来提升分割性能。

Method: 提出Gaussian-to-Point (G2P)方法，将3D高斯泼溅的外观感知属性转移到点云。通过建立点级对应关系解决优化高斯与原始点几何之间的不对齐问题，利用高斯不透明度属性解决几何歧义，利用高斯尺度属性实现复杂3D场景中的精确边界定位。

Result: 在标准基准测试中实现了优越性能，在几何挑战性类别上显示出显著改进，所有改进均无需任何2D或语言监督。

Conclusion: G2P方法通过将3D高斯泼溅的外观感知属性转移到点云，有效解决了点云语义分割中几何特征不足的问题，实现了更区分性和外观一致的分割，为3D场景理解提供了有效解决方案。
...
Method: CloudMatch为每个未标记图像生成一个弱增强视图和两个互补的强增强视图：一个通过场景间混合模拟上下文变化，另一个通过场景内混合保持语义一致性。通过跨多样化增强视图的预测一致性学习来捕捉云模式的结构多样性和上下文丰富性。

Result: 大量实验表明CloudMatch取得了良好的性能，证明了其有效利用未标记数据并推进半监督云检测的能力。

Conclusion: CloudMatch通过结合视图一致性学习和场景混合增强，能够有效利用未标记遥感图像进行云检测，为半监督云检测提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前的无注释手语翻译方法主要关注手动信号，但忽视了面部表情的语义重要性，导致当不同概念具有相同手动表达时产生歧义。面部情感信息应作为语义锚点而非辅助信息。

Method: 提出EASLT框架，包含专门的情感编码器捕捉连续情感动态，通过情感感知融合模块自适应地重新校准时空手语特征，将情感语义从手动动态中解耦。

Result: 在PHOENIX14T和CSL-Daily基准测试中，EASLT在无注释方法中达到先进性能，分别获得26.15和22.80的BLEU-4分数，以及61.0和57.8的Bleurt分数。

Conclusion: 将面部情感作为语义锚点而非辅助信息，通过专门建模情感动态并自适应融合，能有效解决手语翻译中的语义歧义，显著提升翻译保真度。
...
Method: 采用粗到精策略：粗阶段使用Bezier增强对象空间编码器（BEOSE）通过二次Bezier曲线建模实例级空间关系，以及频率感知编码器（FAE）在频域生成全局级空间表示；精阶段使用不确定性感知高斯精细定位器（UGFL）将预测建模为高斯分布，采用不确定性感知的损失函数回归2D位置。

Result: 在KITTI360Pose数据集上的大量实验表明，SpatiaLoc显著优于现有的最先进方法。

Conclusion: SpatiaLoc通过强调空间关系的粗到精策略，在跨模态定位任务中取得了优越性能，证明了空间关系建模对于文本和点云定位的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展显著提升了AI生成图像的质量，引发了关于错误信息和公众信任侵蚀的担忧。检测AI生成图像已成为关键挑战，特别是在泛化到未见生成模型方面。现有使用冻结预训练CLIP模型的方法虽然显示出泛化潜力，但仅将图像编码器视为基本特征提取器，未能充分利用其潜力。

Method: 本文提出掩码预训练模型微调（MPFT）策略，包含纹理感知掩码（TAM）机制。该方法在微调过程中掩码包含生成模型特定模式的纹理区域，迫使CLIP-ViT关注AI生成图像与真实图像之间的"分布偏差"。通过对冻结CLIP图像编码器（CLIP-ViT）的深入分析，发现其能有效聚类真实图像，但缺乏区分真实与AI生成图像的能力。

Result: 在GenImage和UniversalFakeDetect数据集上的大量实验表明，该方法仅用少量图像进行微调，就能显著超越现有方法，在两个数据集上分别达到98.2%和94.6%的平均准确率。

Conclusion: 提出的MPFT策略通过纹理感知掩码机制有效提升了CLIP-ViT在AI生成图像检测中的泛化能力，仅需少量训练数据即可实现对未见生成模型的高精度检测，为解决AI生成图像检测的泛化问题提供了有效方案。
...
TL;DR: 本文介绍了SiT-Bench基准测试，用于评估无需像素输入的纯文本大语言模型的空间智能能力，发现当前模型在局部语义任务表现良好但存在全局一致性"空间鸿沟"。


<details>
  <summary>Details</summary>
Motivation: 当前空间智能研究主要依赖视觉语言模型，但核心问题未解：空间理解究竟源于视觉编码器还是基础推理架构？本文旨在探究纯文本大语言模型的空间推理能力。

Method: 构建包含3,800多个专家标注项目的SiT-Bench基准测试，涵盖5个主要类别和17个子任务，将单/多视角场景转换为高保真、坐标感知的文本描述，要求大语言模型进行符号文本推理而非视觉模式匹配。

Result: 评估显示当前最先进的大语言模型在局部语义任务上表现熟练，但在全局一致性方面存在显著"空间鸿沟"。研究发现显式空间推理能显著提升性能，表明大语言模型具有潜在的世界建模能力。

Conclusion: SiT-Bench作为基础资源，将促进开发具有空间基础的大语言模型架构，为未来视觉语言模型和具身智能体提供支持，代码和基准测试已开源。
...
TL;DR: 本文提出了环境鲁棒的少样本分割（ER-FSS）设置和自适应注意力蒸馏（AAD）方法，以解决现有少样本分割模型在复杂真实环境（如光照变化、背景干扰等）下性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有少样本分割研究大多忽略真实世界中的复杂环境因素（如光照、背景、视角变化等），导致实验室训练的模型在实际部署中表现不佳。需要开发能够应对动态复杂环境的鲁棒模型。

Method: 提出环境鲁棒FSS设置（ER-FSS），建立包含8个数据集的基准测试。提出自适应注意力蒸馏（AAD）方法，通过反复对比和蒸馏支持图像与查询图像之间的关键共享语义，生成针对新类别的类别特定注意力，增强模型在复杂环境中聚焦正确目标的能力。

Result: AAD方法在所有数据集和设置下将mIoU提高了3.3%-8.5%，表现出优越的性能和强大的泛化能力。

Conclusion: 提出的ER-FSS设置和AAD方法有效提升了少样本分割模型在复杂真实环境中的鲁棒性，为实际应用部署提供了更可靠的解决方案。
...
TL;DR: 本文提出了一种针对印度石刻铭文的鲁棒自适应分块策略，结合注意力U-Net进行二值化处理，显著提升了在复杂背景下的文本提取性能。


<details>
  <summary>Details</summary>
Motivation: 石刻铭文图像由于刻字与石质背景对比度差、表面退化不均匀、存在干扰伪影以及文本密度和布局变化大等问题，给二值化处理带来严重挑战，现有技术经常失败。

Method: 提出自适应分块策略，通过动态采样和分块选择方法训练注意力U-Net模型，注意力机制使模型能聚焦细微结构线索，克服表面噪声和布局不规则问题。

Result: 该方法显著提升了二值化性能，尽管仅在单一印度文字数据集上训练，但表现出对其他印度文字和非印度文字的强零样本泛化能力。

Conclusion: 该方法为下游任务如文字识别、OCR和历史文本分析提供了清洁、结构化的铭文内容表示，展现了鲁棒性和文字无关的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于描述特征的物体分类技术依赖对象对齐来计算相似度，这限制了方法的适用性和效率

Method: 1) 对物体边界进行归一化和分段处理，获得近似凸段；2) 按段长度降序排序；3) 提取段长度、极值点数量、面积、基部和宽度等特征袋；4) 使用这些特征测量图像边界相似度

Result: 在多个数据集上进行测试，获得了可接受的结果

Conclusion: 提出的方法通过特征排序成功替代了对象对齐的需求，为物体分类提供了一种新的有效途径
...
TL;DR: CrackSegFlow是一个可控的流匹配合成框架，用于生成基于二进制掩码的真实裂缝图像，解决裂缝分割中标签稀缺和领域偏移问题，并发布了包含5万对图像-掩码的数据集。


<details>
  <summary>Details</summary>
Motivation: 自动化裂缝分割对于路面和基础设施的规模化状态评估至关重要，但实际部署受到像素级标签稀缺以及传感器、光照、纹理和标注规范之间的严重领域偏移的限制。

Method: 提出了CrackSegFlow框架，包含两个流匹配模型：一个生成基于二进制掩码的真实裂缝图像，采用拓扑保持掩码注入和边界门控调制；另一个生成具有明确裂缝覆盖率控制的裂缝掩码。还将裂缝掩码注入无裂缝背景中以增加光照和表面伪影的多样性。

Result: 在五个基准测试中（四个沥青数据集和一个混凝土数据集），使用真实加合成数据对，域内性能平均提升5.37 mIoU和5.13 F1；目标引导的跨域合成仅使用有限目标掩码统计量就实现了平均13.12 mIoU和14.82 F1的提升。相比基于扩散的语义合成，CrackSegFlow提供更快的确定性采样，并提高了细结构裂缝几何的保真度和掩码-图像对齐。

Conclusion: CrackSegFlow通过可控的流匹配合成有效解决了裂缝分割中的标签稀缺和领域偏移问题，显著提升了分割性能，并发布了CSF-50K公共数据集用于大规模可泛化裂缝分割的基准测试。
...
TL;DR: VideoMemory是一个实体中心的视频生成框架，通过动态记忆库解决多镜头叙事视频中角色、道具和环境一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型能产生高质量短片，但在场景变化或长时间间隔后重新出现时，难以保持实体身份和外观的一致性，这是叙事视频生成的核心挑战。

Method: 使用动态记忆库存储角色、道具和背景的显式视觉和语义描述符，通过多智能体系统分解叙事为镜头，从记忆中检索实体表示，并基于这些状态合成关键帧和视频，每个镜头后更新记忆库以反映故事驱动的变化。
...
TL;DR: MGPC是一个可泛化的多模态点云补全框架，通过整合点云、RGB图像和文本，在统一架构中提升对新物体和真实场景的泛化能力。
...
Method: 提出MGPC框架，整合点云、RGB图像和文本；引入创新的模态丢弃策略、基于Transformer的融合模块和渐进式生成器；开发自动数据生成流程并构建MGPC-1M大规模基准数据集。

Result: 在MGPC-1M和真实世界数据上的大量实验表明，该方法始终优于现有基线方法，并在真实世界条件下表现出强大的泛化能力。

Conclusion: MGPC通过多模态整合和创新的架构设计，显著提升了点云补全任务的泛化能力，为处理新物体和真实场景提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有自我中心动作识别方法主要依赖RGB外观、人体姿态估计或其组合，缺乏有效的运动信息表示。本文旨在探索2D点轨迹作为轻量级但有效的运动线索，以提升动作识别性能。

Method: 使用CoTracker跟踪每个视频中随机初始化的一组点，将得到的轨迹与对应图像帧一起作为Transformer识别模型的输入。特别地，方法甚至可以在仅提供初始帧及其关联点轨迹（而非完整视频序列）的情况下工作。

Result: 实验结果表明，集成2D点轨迹相比不使用运动信息的相同模型能持续提升性能，即使仅使用初始帧和点轨迹也能获得显著增益，证明了点轨迹作为轻量级运动表示的有效性。

Conclusion: 2D点轨迹是一种轻量级但有效的自我中心动作理解表示方法，通过跟踪随机采样的图像点提供有价值的运动线索，无需复杂的物体或交互区域检测，为动作识别提供了新的视角。
...
TL;DR: 提出了BREATH-VL框架，结合视觉语言模型和视觉配准方法，用于内窥镜6-DoF相机定位，在复杂气道环境中显著提升定位精度


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在内窥镜相机定位中的三个主要挑战：缺乏大规模高质量医疗定位数据集、细粒度姿态回归能力有限、提取时序特征计算延迟高

Method: 1) 构建BREATH数据集（目前最大的体内内窥镜定位数据集）；2) 提出BREATH-VL混合框架，整合VLM语义线索和视觉配准几何信息；3) 引入轻量级上下文学习机制，将运动历史编码为语言提示以实现高效时序推理

Result: BREATH-VL在准确性和泛化性上均优于最先进的视觉定位方法，平移误差相比最佳基线减少25.5%，同时保持竞争力的计算延迟

Conclusion: 通过结合VLM的语义理解和配准方法的几何对齐优势，BREATH-VL框架能够在内窥镜手术场景中实现准确、鲁棒的6-DoF相机定位，为医疗导航任务提供了有效解决方案
...
TL;DR: MATANet是一种用于细粒度海洋物种分类的新模型，通过多上下文环境注意力模块和层次分离诱导学习模块，结合分类学层次结构和环境上下文来提升分类性能。
...
Result: 在FathomNet2025、FAIR1M和LifeCLEF2015-Fish数据集上的实验表明，MATANet取得了最先进的性能。

Conclusion: MATANet通过模仿专家策略，利用分类学结构和环境上下文来解释水下动物的模糊特征，有效提升了细粒度海洋物种分类的准确性。
...
Result: 在RadDiffBench基准（包含57个专家验证的放射学研究对）上，RadDiff达到47%准确率，使用真实报告指导时达到50%准确率，显著优于通用领域VisDiff基线。系统在COVID-19表型比较、种族亚组分析和生存相关影像特征发现等临床任务中表现出多样性。

Conclusion: RadDiff和RadDiffBench为系统性地揭示放射学数据中有意义的差异提供了首个方法和基准基础，展示了在多种临床任务中的实用性。
...
TL;DR: 本文提出了HyperCOD——首个大规模高光谱伪装目标检测基准数据集，以及HSC-SAM方法，通过将高光谱图像解耦为空间图和光谱显著性图，有效适配SAM模型进行伪装目标检测。


<details>
  <summary>Details</summary>
Motivation: RGB图像在颜色和纹理线索模糊的真实场景中难以有效检测伪装目标，而高光谱图像虽能捕捉精细光谱特征，但缺乏专用的大规模基准数据集限制了该领域发展。

Method: 提出HSC-SAM方法，将高光谱图像解耦为空间图（输入SAM图像编码器）和光谱显著性图（作为自适应提示），有效弥合模态差距，适配Segment Anything Model进行伪装目标检测。

Result: 在包含350张高分辨率高光谱图像的HyperCOD基准上，HSC-SAM取得了新的state-of-the-art性能，并在其他公开HSI数据集上展现出强大的泛化能力。
...
TL;DR: 本文提出了一种名为Masked Video Prediction (MVP)的新颖后训练目标，通过要求模型从干扰项中重建被掩码的连续视频片段，来增强视频大语言模型的时间推理和因果理解能力。
...
Method: 提出Masked Video Prediction (MVP)后训练目标，让模型从具有挑战性的干扰项中重建被掩码的连续视频片段；开发可扩展的数据合成管道，将任意视频语料库转换为MVP训练样本；采用Group Relative Policy Optimization (GRPO)和细粒度奖励函数来增强模型对视频上下文和时间属性的理解。

Result: 综合评估表明，MVP通过直接强化时间推理和因果理解，显著提升了视频推理能力。

Conclusion: MVP作为一种新颖的后训练目标，能够有效弥补现有视频大语言模型在时间连贯性和因果理解方面的不足，通过显式监督时间逻辑和上下文，增强了模型的视频推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 制造业中基于AI的计算机视觉系统需要大量训练数据，但获取和标注成本高昂。虽然制造业可以使用CAD模型生成合成数据，但农业领域缺乏现成的CAD模型，增加了利用合成数据的难度。

Method: 提出了替代CAD文件的多种技术来创建合成数据集：1) 通过扫描获取高代表性3D模型；2) 使用图像到3D的方法生成3D模型。将这些合成数据用于训练物体检测模型，并在小型真实数据集上进行微调。

Result: 研究表明，使用扫描或图像到3D方法获取的高代表性3D模型可以生成有效的合成数据来训练物体检测模型。在小型真实数据集上进行微调可以显著提高模型性能，即使使用代表性较低的模型也能获得相似性能。

Conclusion: 在农业领域缺乏CAD模型的情况下，可以通过替代方法生成合成数据来训练AI物体检测系统。结合合成数据训练和少量真实数据微调的策略，可以有效解决农业领域数据获取困难的问题。
...
Method: 在NNGPT生态系统中构建性能感知闭环系统，使用LoRA微调LLM，基于6000+个经验评估的PyTorch增强函数，仅用下游模型准确率标注，采用成对性能排序训练
...
TL;DR: EvalBlocks是一个用于医学影像基础模型高效评估的模块化框架，基于Snakemake构建，支持数据集、模型、聚合方法和评估策略的无缝集成，实现可重复、可扩展的实验管理。


<details>
  <summary>Details</summary>
Motivation: 医学影像基础模型开发需要持续监控下游性能，但研究人员面临跟踪大量实验、设计选择及其性能影响的负担，通常依赖临时的手动工作流程，这些流程缓慢且容易出错。

Method: 基于Snakemake构建模块化、即插即用的框架，支持新数据集、基础模型、聚合方法和评估策略的无缝集成，通过集中跟踪、高效缓存和并行执行实现可重复性和可扩展性。
...
Method: 提出IDESplat方法：1) 引入深度概率提升单元(DPBU)，通过级联warp操作以乘法方式集成极线注意力图；2) 堆叠多个DPBU构建迭代深度估计过程，逐步识别高可能性深度候选；3) 迭代提升深度概率估计并更新深度候选，逐步细化深度图

Result: 在RealEstate10K、ACID和DL3DV数据集上取得优异重建质量和SOTA性能：在RE10K上PSNR比DepthSplat高0.33dB，仅使用10.7%参数和70%内存；在DTU跨数据集实验中PSNR提升2.95dB，展现强泛化能力

Conclusion: IDESplat通过迭代深度概率提升机制有效解决了单warp操作的局限性，实现了更准确的高斯均值预测，在保持实时效率的同时显著提升了3D高斯泼溅的重建质量和泛化能力
...
TL;DR: MDENeRF是一个迭代框架，通过NeRF的深度信息来优化单目深度估计，结合单目先验保持全局结构，同时利用NeRF不确定性注入高频细节。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计在自动驾驶、扩展现实等领域有重要应用，但现有方法通常产生过于平滑的深度图，缺乏精细几何细节，影响场景理解的准确性。
...
TL;DR: 提出SVL-DRL框架，通过分阶段强化学习解决医学图像分割中的噪声标注问题，将每个体素视为自主智能体，动态修正标签错误，在三个公开数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割依赖高质量标注数据，但实际中常因器官形态复杂和标注者差异导致噪声标注，严重影响分割模型性能。受医学影像标注员能基于先验知识修正标签的启发，需要自动化的噪声标注处理方案。

Method: 提出端到端分阶段体素级深度强化学习框架：1) 将噪声标注建模为体素依赖问题，采用分阶段强化学习保证鲁棒收敛；2) 引入体素级异步优势演员-评论家模块，每个体素作为自主智能体动态优化状态表示；3) 设计新颖的动作空间和复合奖励函数，结合Dice值和空间连续性指标。

Result: 在三个公开医学图像数据集上实现SOTA性能，在各种实验设置下Dice和IoU分数平均提升超过3%。

Conclusion: SVL-DRL框架能有效处理医学图像分割中的噪声标注问题，通过强化学习方法自动修正标签错误，显著提升分割精度和语义完整性。
...
TL;DR: HemBLIP是一个视觉语言模型，用于生成外周血细胞的可解释形态描述，在白血病诊断中提供比黑盒模型更透明的解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前用于白血病诊断的深度学习模型多为黑盒模型，限制了临床信任和采用。需要开发能够生成可解释形态描述的系统来提高诊断透明度。

Method: 构建了包含14k个健康和白病细胞与专家属性描述配对的新数据集，通过全微调和LoRA参数高效训练两种方式适配通用视觉语言模型，并与生物医学基础模型MedGEMMA进行基准比较。
...
TL;DR: FocusUI提出了一种高效的UI grounding框架，通过选择与指令最相关的视觉补丁并保持位置连续性，显著减少了视觉token数量，在保持性能的同时提升了推理速度和降低了内存使用。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型的UI grounding任务需要处理高分辨率截图，产生大量视觉token（如2K分辨率约4700个），导致计算开销大且注意力分散。而人类与UI交互时通常只关注感兴趣区域，因此需要更高效的UI grounding方法。

Method: FocusUI框架包含两个关键技术：1) 通过融合指令条件分数和基于规则的UI图分数构建补丁级监督，选择具有区分度和指令相关性的视觉token；2) 提出PosPad策略，将被丢弃的连续视觉token序列压缩为放置在序列最后索引的特殊标记，以保持位置连续性。
...
Conclusion: FocusUI通过有效选择相关视觉补丁并保持位置连续性，成功解决了UI grounding任务中的计算效率和精度平衡问题，为高效UI理解提供了实用解决方案。
...
TL;DR: 该论文提出了FUSION，首个基于扩散模型的无条件全身运动先验，能够联合建模身体和手部运动，解决了现有方法无法生成包含详细手部关节的多样化全身运动的问题。


<details>
  <summary>Details</summary>
Motivation: 手部在与环境交互和表达手势中至关重要，但现有人体运动合成方法存在不足：要么完全忽略手部运动，要么只能在高度受限的狭窄任务中生成全身运动。主要障碍是缺乏大规模数据集来联合捕捉具有详细手部关节的多样化全身运动。
...
Result: 1. 在HumanML3D数据集的关键点跟踪任务中超越了最先进的骨骼控制模型；2. 实现了更优的运动自然度；3. 在物体交互和自交互两个应用中展示了对手部运动的精确控制，同时保持合理的全身协调性。

Conclusion: FUSION是首个能够联合建模身体和手部运动的扩散模型，在标准基准测试中表现出色，并能通过优化流程应用于特定任务，实现了对手部运动的精确控制，同时保持全身协调的自然性。
...
TL;DR: PosterVerse是一个全流程商业级海报生成方法，通过LLM蓝图创建、扩散模型背景生成和MLLM驱动的HTML引擎统一布局文本渲染，解决了现有系统设计流程不完整、文本渲染精度低等问题。


<details>
  <summary>Details</summary>
Motivation: 商业级海报设计需要美学吸引力与精确信息传递的完美结合。现有自动化海报生成系统存在设计流程不完整、文本渲染精度低、商业应用灵活性不足等显著限制。

Method: PosterVerse采用三阶段方法：1) 使用微调LLM从用户需求中提取关键设计元素创建蓝图；2) 通过定制扩散模型生成图形背景；3) 使用MLLM驱动的HTML引擎进行统一布局文本渲染，确保高文本精度和灵活定制。同时引入PosterDNA数据集，这是首个引入HTML排版文件的中文海报生成数据集。

Result: 实验结果表明，PosterVerse能够持续生成商业级海报，具有吸引人的视觉效果、准确的文本对齐和可定制的布局。PosterDNA数据集从根本上解决了小尺寸和高密度文本渲染的挑战。

Conclusion: PosterVerse为自动化商业海报设计提供了一个有前景的解决方案，通过完整的工作流程实现了商业级海报的自动化生成，解决了现有系统的关键限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频奖励模型通常关注视觉质量、运动质量和文本对齐，但忽视了关键的结构失真问题（如异常物体外观和交互），这些问题会降低生成视频的整体质量。
...
Result: REACT能够准确评估生成视频中的结构失真问题，既提供精确的定量评估，又支持可解释的归因分析，与现有奖励模型形成互补。

Conclusion: REACT填补了生成视频评估中结构失真评估的空白，通过专门的帧级奖励模型和两阶段训练方法，能够有效识别和评估视频中的异常物体外观和交互问题。
...
<details>
  <summary>Details</summary>
Motivation: 风力涡轮机的可靠运行需要频繁检查，因为即使微小的表面损伤也会降低空气动力学性能、减少能量输出并加速叶片磨损。自动化检查的核心是从视觉数据中准确分割涡轮叶片。传统方法需要大量标注数据集，存在可扩展性挑战。

Method: 1. 将像素级分割任务重构为二元区域分类问题；2. 使用完全无监督、可解释的模块化自适应区域生长技术生成图像区域；3. 采用图像特定的自适应阈值引导；4. 通过区域合并过程将碎片区域整合为连贯片段；5. 引入RegionMix增强策略，通过组合不同区域合成新的训练样本以提高泛化和分类鲁棒性。

Result: 该框架展示了最先进的分割精度和强大的跨站点泛化能力，能够在不同风电场中一致地分割涡轮叶片。

Conclusion: 提出的注释高效分割方法通过将像素级任务重构为区域分类问题，结合无监督区域生长和区域混合增强，成功解决了传统方法需要大量标注数据的问题，实现了高精度分割和良好的跨站点泛化性能。
...
TL;DR: LocalDPO：一种新颖的后训练框架，通过在时空区域级别构建局部偏好对来优化文本到视频扩散模型的对齐，无需外部评判模型或手动标注


<details>
  <summary>Details</summary>
Motivation: 现有的直接偏好优化方法依赖多样本排序和任务特定的评判模型，效率低下且往往产生模糊的全局监督，需要更高效和细粒度的视频生成器对齐方法

Method: 提出LocalDPO框架，通过自动化流程收集偏好对数据：将高质量真实视频作为正样本，通过随机时空掩码局部损坏并仅使用冻结基础模型恢复掩码区域来生成负样本；引入区域感知DPO损失，将偏好学习限制在损坏区域以实现快速收敛

Result: 在Wan2.1和CogVideoX上的实验表明，LocalDPO在视频保真度、时间连贯性和人类偏好分数方面持续优于其他后训练方法

Conclusion: LocalDPO为视频生成器对齐建立了更高效和细粒度的范式，通过局部偏好对优化在时空区域级别实现更好的对齐效果
...
TL;DR: 本文发现大型多模态模型存在"文本惯性"问题，即一旦思考过程中出现文本幻觉，模型会盲目坚持错误文本而忽略冲突的视觉证据。作者提出LogicGraph扰动协议评估模型自反思能力，并引入主动视觉上下文精炼方法来缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在视频推理中展示了强大的链式思考能力，但其推理链的鲁棒性存在问题。研究发现模型存在"文本惯性"这一关键失败模式，即模型倾向于盲目坚持错误的文本幻觉而忽视冲突的视觉证据，这影响了推理的可靠性。

Method: 1. 提出LogicGraph扰动协议，在多种LMM的推理链中结构化注入扰动，评估其自反思能力；2. 引入主动视觉上下文精炼方法，这是一种无需训练的推理范式，通过主动视觉重定位机制进行细粒度验证，并结合自适应上下文精炼策略来总结和去噪推理历史。

Result: 实验结果显示，模型成功自我纠正的比例低于10%，主要表现出盲目的文本错误传播。而提出的主动视觉上下文精炼方法能显著抑制幻觉传播，增强推理鲁棒性。

Conclusion: 大型多模态模型存在严重的文本惯性问题，导致推理链脆弱。提出的主动视觉上下文精炼方法通过强制细粒度视觉验证和自适应上下文管理，有效缓解了幻觉传播，提升了多模态推理的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 桥接基础重建模型的强先验与视频扩散模型的能力，实现更高质量的3D场景生成，同时探索重建模型与生成模型的相互增强

Method: 重新利用VGGT重建模型生成几何潜在表示，通过训练适配器使其与预训练视频扩散模型的外观潜在表示对齐，联合生成解耦但对齐的潜在表示

Result: 在单图像和多图像条件下的3D场景生成中达到最先进水平，能同时生成RGB视频、相机姿态、深度图和全局点云，并能通过生成先验增强重建鲁棒性

Conclusion: Gen3R成功桥接了重建模型与生成模型，展示了紧密耦合这两种模型的相互益处，为场景级3D生成提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前遥感视觉语言模型存在逻辑幻觉问题，即正确答案来自错误推理链或依赖位置捷径而非空间逻辑，这削弱了复杂空间任务中的认知可靠性。需要从感知为中心转向高层次演绎推理。

Method: 提出GeoReason框架：1) 构建GeoReason-Bench逻辑驱动数据集，包含4000个从几何基元和专家知识合成的推理轨迹；2) 两阶段训练策略：监督知识初始化（学习推理语法和领域知识）和一致性感知强化学习（通过逻辑一致性奖励惩罚逻辑漂移）。
...
TL;DR: 本文提出了一种新颖的多模态方法，通过像素级二维表示来编码卫星图像时间序列(SITS)中的视觉属性变化，使用递归图处理植被指数时间序列，并引入PIMC多模态对比学习方法，在多个下游任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 卫星持续产生大量地球观测数据，特别是卫星图像时间序列(SITS)。然而，大多数深度学习模型要么处理整个图像，要么处理完整的时间序列序列来提取下游任务的特征。现有方法未能充分利用像素级的时间变化信息。

Method: 提出基于像素级二维表示的多模态方法：1) 从像素级植被指数时间序列(NDVI、EVI、SAVI)生成递归图作为原始像素值的替代表示；2) 引入PIxel-wise Multimodal Contrastive (PIMC)多模态自监督方法，基于二维像素时间序列表示和遥感影像(RSI)训练有效编码器。

Result: 在三个下游任务上验证：PASTIS数据集的像素级预测和分类，以及EuroSAT数据集的土地覆盖分类。实验结果表明，二维表示显著增强了SITS的特征提取能力，对比学习提高了像素时间序列和RSI的表示质量。该方法在所有下游任务上都优于现有最先进方法。

Conclusion: 该多模态方法在各种地球观测任务中优于现有模型，建立了一个强大的自监督框架，可用于处理SITS和RSI数据。二维表示能更有效地编码SITS中的视觉属性变化，而对比学习则提升了两种模态的表示质量。
...
### [54] [Diffusion-DRF: Differentiable Reward Flow for Video Diffusion Fine-Tuning](https://arxiv.org/abs/2601.04153)
*Yifan Wang,Yanyu Li,Sergey Tulyakov,Yun Fu,Anil Kag*

Main category: cs.CV

TL;DR: Diffusion-DRF：一种基于可微分奖励流的视频扩散模型微调方法，使用冻结的视觉语言模型作为免训练评判器，避免传统偏好优化方法的标签依赖和奖励攻击问题


<details>
  <summary>Details</summary>
Motivation: 当前基于DPO的文本到视频生成方法依赖不可微分的偏好信号（人工标注或学习型奖励模型），导致训练标签密集、易产生偏见、容易受到奖励攻击，且训练不稳定
...
Conclusion: Diffusion-DRF为视频扩散模型提供了一种可微分、免训练的微调方法，通过直接利用VLM反馈避免了传统偏好优化方法的局限性，在提升生成质量的同时保证了训练稳定性
...
<details>
  <summary>Details</summary>
Motivation: 现有基于注意力的rPPG深度学习方法虽然提高了鲁棒性，但带来了计算成本高、参数多的问题，且注意力机制的时间复杂度随序列长度呈二次方增长。需要一种更高效的长时间序列建模方法。

Method: 提出ToTMNet架构，核心是用FFT加速的Toeplitz时间混合层替代传统注意力机制。Toeplitz算子使用线性参数实现全序列时间感受野，通过循环嵌入和FFT卷积实现近线性时间计算。架构结合局部深度时间卷积分支和门控全局Toeplitz混合，形成紧凑的门控时间混合器。

Result: 在UBFC-rPPG（真实视频）数据集上达到1.055 bpm MAE和0.996 Pearson相关系数；在SCAMPS（合成视频）到UBFC-rPPG的跨域设置中达到1.582 bpm MAE和0.994 Pearson相关系数。模型仅含63k参数，验证了门控机制对全局Toeplitz混合的重要性。

Conclusion: Toeplitz结构的时间混合是rPPG任务中注意力机制的高效实用替代方案，能够在保持高性能的同时显著减少计算和参数开销。主要局限是仅使用了两个数据集，但结果显示了该方法的潜力。
...
TL;DR: 该论文提出了一种结合2D图像和深度图的视觉定位方法，通过增强图像几何信息实现高精度定位，同时保持构建和维护的便捷性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位方法存在局限性：2D图像方法易于构建和维护但几何推理能力有限；3D结构方法精度高但需要集中式重建且难以更新。需要一种既能保持2D方法优势又能提供几何准确性的解决方案。

Method: 采用2D图像表示并增强每张图像的估计深度图来捕捉几何结构。利用密集匹配器，结合紧凑压缩和GPU加速的LO-RANSAC实现，在存储和计算上实现高效平衡。

Result: 在多个标准基准测试中达到新的最先进精度，在可比地图大小下优于现有内存高效方法。实现了精度与内存效率之间的灵活权衡。

Conclusion: 通过深度增强的2D图像表示，该方法在保持易于构建和维护优势的同时，实现了高精度视觉定位，为实际应用提供了实用解决方案。
...
TL;DR: CHORD是一个通用的生成式管道，用于合成4D（3D+时间）动态物体和场景，通过从2D视频中提取拉格朗日运动信息来实现类别无关的动态生成。
...
Method: 提出基于蒸馏的管道，从2D视频的欧拉表示中提取丰富的拉格朗日运动信息。该方法继承视频生成模型的通用性，实现类别无关的动态生成。

Result: 实验证明该方法能生成多样化的多体4D动态，相比现有方法具有优势，并展示了在生成机器人操作策略方面的应用潜力。

Conclusion: CHORD是一个通用、多功能、类别无关的4D动态生成方法，通过从2D视频中提取运动信息，克服了传统方法和现有学习方法的局限性。
...
TL;DR: Jailbreak-Zero是一种新颖的红色团队方法，将LLM安全评估从基于示例的约束方法转变为基于策略的框架，通过攻击LLM生成多样化对抗提示并微调，实现策略覆盖、攻击多样性和提示保真度的帕累托最优。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型安全评估方法主要采用基于示例的约束方法，这种方法在策略覆盖范围、攻击策略多样性和提示保真度方面存在局限性，需要更全面、可扩展的评估框架。

Method: 使用攻击LLM生成大量多样化的对抗提示，然后利用偏好数据集微调攻击模型，实现策略覆盖、攻击策略多样性和提示保真度三个关键目标的帕累托最优。
...
TL;DR: 本研究评估了两种本地部署的LLM（gpt-oss-20b和gpt-oss-120b）在临床任务中的表现，发现它们性能与更大模型相当，且微调后能接近GPT-5水平，展示了本地LLM在临床决策支持中的潜力。


<details>
  <summary>Details</summary>
Motivation: 专有LLM系统存在隐私问题和云基础设施依赖，开源替代方案通常模型过大，不适合资源受限的临床环境。需要评估本地部署的轻量级LLM在临床任务中的可行性。

Method: 在三个代表性临床任务上对两种本地LLM进行基准测试：1）一般疾病诊断；2）专科（眼科）诊断与管理；3）模拟专家评估。与专有模型（GPT-5、o4-mini）和开源模型（DeepSeek-R1）比较，并对gpt-oss-20b进行微调评估适应性。

Result: gpt-oss模型在各项任务中性能与DeepSeek-R1和o4-mini相当或更优，尽管模型尺寸显著更小。微调后gpt-oss-20b的诊断准确性显著提升，接近GPT-5水平。

Conclusion: 本地部署的LLM能够提供准确、适应性强且保护隐私的临床决策支持，为LLM更广泛融入常规临床实践提供了实用途径。
...
TL;DR: GPT-5是一个包含智能快速模型、深度推理模型和实时路由器的统一系统，在性能、安全性和实用性方面均有显著提升


<details>
  <summary>Details</summary>
Motivation: 开发一个更高效、更安全、更实用的AI系统，通过多模型架构和智能路由提升用户体验，同时在生物化学领域采取预防性安全措施

Method: 采用统一系统架构：1) gpt-5-main用于大多数问题回答；2) gpt-5-thinking用于复杂推理；3) 实时路由器根据对话类型、复杂度、工具需求和明确意图动态选择模型；4) 路由器通过用户切换、偏好评分和正确性测量进行持续训练；5) 使用安全完成技术防止不允许内容

Result: GPT-5在基准测试中超越先前模型，响应速度更快，对现实世界查询更有用；在减少幻觉、改进指令遵循、减少奉承方面有显著进步；在写作、编程和健康三个最常见应用场景中表现提升；gpt-5-thinking被认定为生物化学领域高能力模型，激活相应安全防护

Conclusion: GPT-5通过创新的多模型架构和智能路由系统，在性能、安全性和实用性方面实现了重要突破，特别是在生物化学领域采取预防性安全措施，体现了负责任AI开发的原则
...
TL;DR: 该研究提出专门针对小型语言模型（SLMs）在非推理任务（如语气修改）中的评估框架，挑战当前以推理能力为主导的评估标准，为工业应用提供实用工具。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型评估过度关注推理和问题解决能力，导致小型语言模型（10B参数以下）在这些指标上得分比大型语言模型低3-4倍。然而，这种评估未能捕捉SLMs在常见工业应用（如语气修改任务）中的实际有效性。

Method: 提出专门针对非推理任务的评估框架，结合数据生成、提示调优和基于LLM的评估等新方法，展示任务特定微调的潜力，特别适用于边缘和私有计算场景。

Result: 该框架能够有效评估SLMs和LLMs在实际应用中的性能，特别是在缺乏预定义评估数据集的非推理任务上，为从业者提供了实用的基准测试工具。

Conclusion: 需要重新思考语言模型评估标准，不应仅以推理能力衡量模型价值，SLMs在特定工业应用中具有显著优势，提出的框架为实际部署提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在自然语言理解和生成方面表现出色，但在企业环境中部署时暴露出关键限制：对自定义指令的遵循不一致。企业应用需要精确的指令遵循能力，而现有模型在这方面存在不足。

Method: 对13个领先的大语言模型进行系统性评估，测试其在真实世界RAG场景中的指令遵循能力、响应准确性和性能指标。使用样本和企业级评估协议进行测试。

Result: 不同模型在指令遵循方面差异显著，Claude-Sonnet-4和GPT-5取得了最高结果。研究揭示了"指令差距"现象：模型在通用任务上表现出色，但在企业部署所需的精确指令遵循方面存在困难。

Conclusion: 这项工作为组织部署LLM解决方案提供了实用见解，并为各大模型家族的指令遵循能力建立了基准。指令遵循能力是LLM企业部署的关键挑战，需要进一步研究和改进。
...
TL;DR: 这篇综述论文系统回顾了2021年以来语义文本相似性（STS）领域的研究进展，重点关注六大关键技术方向，包括基于Transformer的模型、对比学习方法、领域特定解决方案等，为研究者和实践者提供了当前方法、应用和挑战的全面指导。


<details>
  <summary>Details</summary>
Motivation: 自2021年以来，语义文本相似性研究快速发展，受到Transformer架构、对比学习和领域特定技术的推动。为了系统梳理这些进展，帮助研究者和实践者理解当前方法、应用和挑战，需要一篇全面的综述来组织和分析这些发展。

Method: 论文采用文献综述方法，从六个关键领域分析STS研究进展：1）基于Transformer的模型；2）对比学习方法；3）领域聚焦解决方案；4）多模态方法；5）基于图的方法；6）知识增强技术。通过系统梳理这些技术方向的最新成果，提供结构化分析。

Result: 研究发现：1）Transformer模型如FarSSiBERT和DeBERTa-v3取得了显著精度提升；2）对比学习方法如AspectCSE建立了新的基准；3）领域适应模型（如CXR-BERT用于医疗文本，Financial-STS用于金融）展示了STS在专业领域的有效定制；4）多模态、基于图和知识集成模型进一步增强了语义理解和表示能力。

Conclusion: 该综述为研究者和实践者提供了导航STS快速发展的宝贵指导，突出了新兴趋势和未来机会。通过系统组织和分析这些进展，论文不仅总结了当前方法和技术，还指出了该领域面临的挑战和未来的发展方向。
...
TL;DR: 提出无需历史模型性能数据的测试集压缩框架，通过领域适应和聚类分析减少专业领域基准测试的冗余，在3GPP通信基准上实现90%以上的成本降低。


<details>
  <summary>Details</summary>
Motivation: 现有垂直领域LLM评估基准存在语义冗余和高计算成本问题，而现有压缩方法依赖历史模型性能数据，无法适用于新任务、新领域或新模型的冷启动场景。

Method: 1) 使用少量领域数据微调基础LLM以内部化任务语义；2) 仅基于原始文本内容生成高级语义嵌入；3) 在领域适应嵌入空间中进行任务感知聚类；4) 引入数据集X射线机制分析聚类几何结构，根据基准内在冗余动态校准压缩强度。

Result: 在专业领域数据集（特别是大规模3GPP通信基准）上的实验表明，该方法能有效识别和移除冗余样本，在保持与完整基准高度一致性的同时，将评估成本降低90%以上。

Conclusion: 提出了一种无需历史性能数据的测试集压缩框架，解决了冷启动场景下的基准测试冗余问题，显著降低了专业领域LLM评估的计算成本。
...
TL;DR: GuardEval是一个统一的多视角基准数据集，包含106个细粒度类别，用于训练和评估内容审核系统。基于该数据集训练的GemmaGuard模型在内容审核任务上显著优于现有主流模型。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在内容审核方面存在局限性：难以处理隐式冒犯、微妙性别和种族偏见、越狱提示等复杂情况，且过度依赖训练数据可能强化社会偏见，导致不一致和伦理问题的输出。需要更安全、公平的审核系统。

Method: 1. 构建GuardEval基准数据集：包含106个细粒度类别，涵盖人类情感、冒犯和仇恨言论、性别和种族偏见、更广泛的安全问题等多个维度。2. 开发GemmaGuard模型：基于Gemma3-12B模型，使用QLoRA微调技术在GuardEval数据集上进行训练，实现细粒度内容审核。

Result: GemmaGuard在评估中取得了0.832的宏观F1分数，显著优于OpenAI Moderator（0.64）和Llama Guard（0.61）等主流审核模型。多视角、以人为中心的安全基准有效减少了偏见和不一致的审核决策。

Conclusion: 多视角、以人为中心的安全基准对于减少偏见和不一致的审核决策至关重要。GuardEval和GemmaGuard共同证明，多样化和具有代表性的数据能够显著提升复杂边界案例的安全性、公平性和鲁棒性。
...
TL;DR: LLM_annotate是一个用于分析小说角色人格的Python包，它使用大语言模型标准化角色行为标注流程，支持全文本分析，并包含质量验证GUI界面。


<details>
  <summary>Details</summary>
Motivation: 传统角色人格分析通常依赖人工标注，效率低且难以标准化。需要一种可扩展、可重复的方法来分析小说和剧本中的角色行为与人格特质。

Method: 开发了LLM_annotate Python包，包含文本分块、LLM标注、角色名称消歧、质量评分、统计计算和嵌入生成等功能。支持商业、开源或自定义LLM，并通过人机交互GUI进行质量验证。

Result: 通过《辛普森一家大电影》和《傲慢与偏见》的教程示例，展示了该包能够高效、可重复地进行角色分析，实现了标准化的工作流程。

Conclusion: LLM_annotate为研究人员提供了一个灵活、可扩展的工具，用于系统化分析虚构角色的人格特质，支持多种LLM并确保分析质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于句子语义相似度的方法缺乏长距离依赖关系和LLMs中的丰富知识，LLMs在主题分割任务上的应用尚未充分探索

Method: 提出重叠递归提示策略，使用句子枚举方法，并支持采用边界相似度评估指标

Result: LLMs作为主题分割器比现有方法更有效

Conclusion: LLMs在主题分割任务上具有潜力，但在可靠使用前仍需解决一些问题
...
TL;DR: 提出了一种针对ARM64架构的"虚拟张量核心"软件架构，通过直接内存映射和手工优化的NEON SIMD内核，绕过标准库容器，实现了软件定义的DMA，在M2硬件上110M参数模型达到>60 tokens/秒的稳定吞吐量。


<details>
  <summary>Details</summary>
Motivation: 边缘设备上部署大型语言模型受到"内存墙"的根本限制，即数据移动延迟超过算术吞吐量。标准推理运行时通常存在高层抽象、动态调度和未对齐内存访问模式带来的显著开销。

Method: 1. 提出"虚拟张量核心"软件架构，专门针对ARM64微架构（Apple Silicon）优化；2. 绕过标准库容器，采用直接内存映射（mmap）和手工优化的NEON SIMD内核；3. 实现"软件定义的直接内存访问（DMA）"；4. 提出张量虚拟化布局（TVL）保证权重矩阵100%缓存线利用率；5. 零拷贝加载器消除初始化延迟。

Result: 在M2硬件上，110M参数模型实现了>60 tokens/秒的稳定吞吐量。虽然专有硬件加速器（如Apple AMX）能达到更高的峰值吞吐量，但该架构提供了一个完全开源、可移植且确定性的参考实现，用于研究通用ARM硅上的内存瓶颈，满足200ms心理语言学延迟阈值。

Conclusion: 该工作提出了一种创新的软件架构，通过虚拟张量核心和软件定义的DMA技术，有效解决了边缘设备上LLM部署的内存瓶颈问题，为研究ARM架构上的内存优化提供了开放、可移植的参考实现。
...
TL;DR: BPE分词通过递归应用驱动词频分布趋向Zipf幂律，同时诱导熵的特征增长模式，使语言模型的预测熵与Zipf理论预测更一致，减少局部词元依赖，重构自然语言的关键信息特性。


<details>
  <summary>Details</summary>
Motivation: 尽管自然语言展现出显著的统计规律性（如Zipf定律和Heaps定律），但这些特性与现代Transformer模型使用的分词方案之间的关系尚不清楚。本文旨在分析BPE分词如何改变语料库统计特性，以及它如何影响语言模型的信息特性。

Method: 1. 在Zipfian频率分布假设下分析各种语料库的信息内容（用香农熵衡量），推导出槽熵期望值的闭式表达式；2. 实证研究BPE如何转换语料库统计特性，展示递归应用BPE驱动词频趋向Zipf幂律并诱导熵的特征增长模式；3. 在不同BPE深度分词的语料库上训练语言模型，分析模型预测熵与Zipf理论预测的一致性；4. 使用基于注意力的诊断方法分析分词深度对局部词元依赖的影响。

Result: 1. 递归应用BPE驱动词频分布趋向Zipf幂律；2. BPE诱导熵的特征增长模式；3. 随着BPE深度增加，语言模型的预测熵与Zipf理论预测的一致性增强；4. 更深的分词减少了局部词元依赖，使经验分布更接近弱依赖（近IID）状态。

Conclusion: BPE不仅作为一种压缩机制，还作为一种统计变换，重构了自然语言的关键信息特性。它通过驱动词频分布趋向Zipf幂律、减少局部依赖，使语言模型能够更好地学习自然语言的统计规律性。
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明隐喻影响人类决策，考虑到大语言模型训练数据包含大量隐喻，研究者想探究隐喻是否也会影响LLMs的推理路径，特别是在跨领域错位问题中。

Method: 通过干预训练数据中的隐喻（在预训练、微调和重新对齐阶段），研究隐喻与模型错位程度的关系；深入分析隐喻与全局/局部潜在特征激活的关联；基于监测这些潜在特征设计错位内容检测器。

Result: 发现训练数据中的隐喻与大语言模型推理内容的错位程度存在强因果关系；通过隐喻干预能显著改变模型的跨领域错位程度；设计的检测器能够高精度预测错位内容。

Conclusion: 隐喻在大语言模型的推理路径和跨领域错位问题中扮演重要角色，通过监测隐喻激活的潜在特征可以有效检测和干预模型的错位行为。
...
TL;DR: PersonaWeaver框架通过将世界构建与行为构建解耦，解决了现有角色生成方法中的两种对齐偏差问题，生成了更具多样性和戏剧张力的虚拟角色。


<details>
  <summary>Details</summary>
Motivation: 现有角色生成方法存在两种对齐偏差：积极道德偏差（角色总是采取赞同立场）和乐于助人助手偏差（角色总是直接回答问题）。这些偏差虽然适合指令跟随系统，但抑制了戏剧张力，导致角色可预测性高。

Method: 提出PersonaWeaver框架，将世界构建（角色、人口统计特征）与行为构建（道德立场、互动风格）解耦，从而生成具有更多样化反应和道德立场的角色。

Result: 该方法生成了具有更丰富反应和道德立场的角色，同时在风格标记（如长度、语气、标点）上实现了二阶多样性。

Conclusion: PersonaWeaver通过解耦世界构建和行为构建，有效解决了现有角色生成方法中的对齐偏差问题，为虚拟世界创造了更具多样性和戏剧张力的角色。
...
TL;DR: 提出Disclaimer Injection方法，通过向文本注入精心设计的对齐触发免责声明，使大语言模型无法有效学习受保护数据，利用模型自身的对齐机制实现数据保护。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在大量异构文本语料上训练，未经授权使用专有或个人数据的问题日益严重，需要在实际黑盒设置下保护数据不被模型学习。

Method: 提出Disclaimer Injection方法，在数据层面注入精心设计的对齐触发免责声明，利用模型自身的对齐机制阻止有效学习，无需访问或修改训练流程。

Result: 在受保护数据上微调会导致对齐相关层持续激活，使对齐约束覆盖任务学习，导致模型性能显著下降，相比标准微调有系统性性能退化。

Conclusion: 该方法首次利用对齐行为作为数据保护的新杠杆，提出了无需访问训练流程即可在大语言模型规模上限制数据可学习性的实用方法。
...
TL;DR: 该论文系统形式化了提格里尼亚语基数词和序数词的语音表达，填补了该语言计算资源的空白，并发布了开源实现。


<details>
  <summary>Details</summary>
Motivation: 提格里尼亚语在计算资源方面存在空白，特别是数字语音表达缺乏系统化的形式化描述，这影响了面向提格里尼亚语社区的语言建模、语音合成和可访问性应用。

Method: 1. 记录提格里尼亚语数字表达的规范规则，包括连接系统、量级词以及日期、时间和货币的特殊情况；2. 提供数字到词语转换的形式化算法；3. 开发并发布开源实现；4. 评估前沿大语言模型在提格里尼亚语数字语音表达方面的能力。

Result: 1. 完成了提格里尼亚语数字语音表达的系统形式化；2. 发布了开源实现；3. 评估显示前沿大语言模型在准确表达提格里尼亚语数字方面存在显著差距，凸显了明确规则文档的必要性。

Conclusion: 该工作填补了提格里尼亚语数字语音表达的计算资源空白，为面向提格里尼亚语社区的语言建模、语音合成和可访问性应用提供了重要基础，同时揭示了当前大语言模型在该语言任务上的局限性。
...
TL;DR: LatentGraphMem：结合隐式图记忆与显式子图检索的记忆框架，用于处理长上下文稀疏证据的查询任务


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统存在两难：显式结构化记忆可解释但脆弱，隐式记忆机制高效稳定但难以检查。长上下文应用中证据稀疏且分散，需要更好的记忆解决方案。

Method: 提出LatentGraphMem框架：在潜在空间中存储图结构记忆以保证稳定性和效率；提供任务特定的子图检索接口，在固定预算下返回紧凑的符号子图供下游推理和人工检查。训练时生成显式图视图与冻结推理器交互，推理时在潜在空间检索并仅外部化检索到的子图。

Result: 在多个模型规模的长视野基准测试中，LatentGraphMem始终优于代表性的显式图和隐式记忆基线，同时支持参数高效适应，并能灵活扩展到更大的推理器而不引入大型符号工件。

Conclusion: LatentGraphMem通过结合隐式图记忆的稳定性与显式子图检索的可解释性，为长上下文稀疏证据的查询任务提供了有效的记忆解决方案。
...
TL;DR: PCoA是一个用于医学领域摘要生成的专家标注基准，支持短语级上下文归因，并提出解耦评估框架来独立评估摘要、引用和贡献短语的质量。


<details>
  <summary>Details</summary>
Motivation: 验证系统生成的摘要具有挑战性，特别是在高风险医疗领域，需要精确的源上下文归因。当前缺乏支持短语级上下文归因的医学摘要基准。

Method: 引入PCoA基准，将基于方面的摘要与其支持性上下文句子和贡献短语对齐。提出细粒度解耦评估框架，独立评估生成摘要、引用和贡献短语的质量。

Result: PCoA提供了可靠的基准，实验表明在摘要生成前明确识别相关句子和贡献短语可以提高整体质量。数据集和代码已公开。

Conclusion: PCoA为评估具有短语级上下文归因的系统生成摘要提供了有效基准，在摘要生成前识别相关上下文和短语能提升质量，对高风险医疗领域具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 传统文档相似性评估采用整体方法，忽略了不同方面的差异。作者提出需要显式考虑特定方面来更准确地衡量文档相似性，并修订现有标准实践。

Method: 开发ASPECTSIM框架，基于26K个方面-文档对的新基准进行实验。使用GPT-4o直接提示生成方面条件相似性分数，并对16个开源小模型和9个嵌入模型进行大规模元评估，采用两阶段细化方法提升性能。

Result: GPT-4o实现的ASPECTSIM相比无显式方面的整体相似性，人类-机器一致性提高约80%。开源小模型直接提示效果差（20-30%一致性），但两阶段细化可提升约140%一致性，但仍远低于GPT-4o性能。

Conclusion: 显式考虑方面对文档相似性评估至关重要，需要修订标准实践。虽然简单细化能提升开源模型性能，但它们在捕捉方面条件相似性方面仍显著落后于大型专有模型。
...
TL;DR: 研究发现LLM作为评分者在不同评分量尺上缺乏一致性，0-5分制在人类-LLM对齐方面表现最佳，评分量尺设计对LLM评估协议至关重要。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型越来越多地被用作自动化评估者，但先前研究表明这些LLM评分者在提示改变时往往缺乏评分一致性。然而，评分量尺本身的影响尚未得到充分探索。

Method: 研究比较了人类和LLM两种评分者，在三种评分量尺上收集评分数据，涵盖六个基准测试（包括客观、开放式主观和混合任务）。使用组内相关系数(ICC)测量绝对一致性。

Result: LLM判断在主观基准测试中跨量尺不一致；评分量尺选择显著影响人类-LLM一致性，即使组内面板可靠性很高；在任务聚合后，0-5分制产生最强的人类-LLM对齐；总体可靠性可能掩盖基准异质性，并揭示跨性别群体的系统性亚组差异。

Conclusion: 评分量尺设计和子级诊断应作为LLM-as-a-judge协议的重要组成部分，强调在LLM评估中需要仔细考虑量尺选择并进行亚组分析。
...
TL;DR: L2T预训练框架在标准下一个token预测基础上加入语言学习任务，通过结构化输入输出对提供显式语言刺激，提升语言模型的语言能力


<details>
  <summary>Details</summary>
Motivation: 传统语言模型预训练主要关注世界知识和推理能力，但没有显式优化语言能力。为了弥补这一差距，需要开发能够同时提升语言能力和通用推理能力的预训练方法

Method: 提出L2T预训练框架，受人类语言习得启发，将原始文本转换为结构化输入输出对，在标准下一个token预测任务基础上加入语言学习任务，提供显式语言刺激

Result: 在原始文本和L2T数据混合预训练的语言模型，在语言能力基准测试上不仅整体性能提升，而且学习速度加快，同时在通用推理任务上保持竞争力

Conclusion: L2T框架通过整合语言学习任务，有效提升了语言模型的语言能力，同时不影响其通用推理性能，为语言模型预训练提供了新的方向
...
TL;DR: LLMs在时间序列分类任务中表现不佳主要是由于基于提示的评估方法限制，而非模型缺乏时间结构表征能力。通过线性探针分析内部表征，发现LLMs实际具备强大的时间序列理解能力。


<details>
  <summary>Details</summary>
Motivation: 先前基于提示的评估显示LLMs在时间序列分类上表现不佳，这引发了关于LLMs是否真正理解时间结构的疑问。本研究旨在探究这种表现不佳是由于模型表征能力不足，还是评估方法本身的局限性。

Method: 采用线性探针方法直接分析LLMs的内部表征，与基于提示的生成方法进行对比。通过层间分析研究时间序列信息在不同Transformer层中的分布，并考察视觉和多模态输入对时间序列理解的影响。

Result: 零样本提示方法表现接近随机水平（F1 0.15-0.26），而线性探针方法显著提升性能（F1 0.61-0.67），达到或超过专门的时间序列模型。层间分析显示类别区分性的时间序列信息在早期Transformer层就已出现，并通过视觉和多模态输入得到增强。

Conclusion: 当前基于提示的评估方法系统性地低估了LLMs的时间序列理解能力。LLMs内部确实编码了有意义的时间结构信息，但提示生成机制未能有效提取这些信息。这揭示了模型内部表征与外部评估之间的不匹配问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学问答基准主要关注临床知识或患者层面推理，缺乏系统评估基于证据的流行病学推理能力，需要专门的诊断基准来评估模型在流行病学推理方面的表现
...
TL;DR: SegNSP将线性文本分割重新定义为下一个句子预测任务，通过标签无关的NSP方法和分割感知损失函数，在无需显式主题标签的情况下实现有效的文本分割。


<details>
  <summary>Details</summary>
Motivation: 线性文本分割是NLP中长期存在的挑战性问题，由于主题边界定义复杂、话语结构多变、需要平衡局部连贯性与全局上下文等因素，传统方法效果有限，阻碍了摘要、信息检索、问答等下游应用的发展。

Method: 提出SegNSP框架，将线性文本分割重新定义为下一个句子预测任务。采用标签无关的NSP方法，预测下一句是否延续当前主题，无需显式主题标签。结合分割感知损失函数和更难负采样策略，更好地捕捉话语连续性。

Result: 在CitiLink-Minutes数据集上达到B-F1分数0.79，与人工标注的主题转换高度一致；在WikiSection数据集上达到B-F1分数0.65，比最强可复现基线TopSeg高出0.17个绝对点。

Conclusion: SegNSP展示了竞争力和鲁棒性，证明了建模句子间连续性对于提高分割质量和支持下游NLP应用的有效性，为线性文本分割提供了新的解决方案。
...
TL;DR: CALM框架通过解耦任务语义与文化概念，构建文化聚类并自适应集成，赋予语言模型文化自我意识，在跨文化基准测试中优于现有方法。
...
Method: CALM框架：1) 解耦任务语义与显性文化概念和潜在文化信号；2) 通过对比学习形成结构化文化聚类；3) 通过交叉注意力对齐建立细粒度文化特征交互；4) 通过Mixture-of-Experts机制沿文化特定维度自适应集成；5) 构建文化基础内部身份状态并与原始知识融合；6) 通过自提示反思学习实现持续适应和自我修正。

Result: 在多个跨文化基准数据集上的广泛实验表明，CALM始终优于最先进的方法。

Conclusion: CALM框架成功赋予语言模型文化自我意识，能够理解并适应动态变化的文化语境，在跨文化任务中表现出色。
...
TL;DR: KR-Test是一个轻量级评估框架，用于区分SFT训练中的事实学习与语言风格模仿，通过对比正确与错误续写的似然偏好来评估知识保留。


<details>
  <summary>Details</summary>
Motivation: 传统SFT训练依赖验证集困惑度监控，但这种方法混淆了语言风格模仿与事实知识内化，无法准确评估模型是否真正掌握了领域知识。

Method: 提出KR-Test框架：1）自动生成对比样本，对比正确与错误的事实续写；2）测量模型对正确续写的似然偏好；3）无需指令调优或生成解码；4）通过"盲测vs.先知"基线分析验证框架完整性。

Result: KR-Test能够有效区分事实学习与语言模仿，揭示了LoRA训练中语言收敛与知识保留的细粒度分离，增强了微调动态的可解释性。

Conclusion: KR-Test提供了一个轻量级、基于语料的评估框架，能够更准确地监控SFT训练中的知识保留情况，解决了传统困惑度评估的局限性。
...
TL;DR: RPAM是一种通过模型合并实现自适应推理的轻量级方法，将长链思维模型与短链思维模型合并，无需重新训练即可减少推理成本


<details>
  <summary>Details</summary>
Motivation: 现有大型推理模型生成冗长推理路径导致计算开销大、延迟高，而现有加速方法需要重新训练模型或设计复杂提示，成本高昂且对输入敏感

Method: 提出推理模式对齐合并(RPAM)框架：构建模式标注校准集为查询分配推理模式，通过特征对齐优化层间合并系数，使合并模型的中间表示与选定模型对齐，同时使用对比目标使其远离非选定模型

Result: 在七个广泛使用的推理基准测试中，RPAM显著降低了推理成本，同时保持了强大的性能表现

Conclusion: RPAM提供了一种轻量级的模型合并方法，实现了查询自适应推理，无需从头训练或大规模额外数据，为高效推理提供了新途径
...
TL;DR: IntroLM让因果语言模型在预填充阶段通过内省令牌预测自身输出质量，无需外部分类器，在问答基准上达到90%的ROC AUC，优于DeBERTa分类器14%，在路由系统中可减少33%延迟和50%大模型使用。
...
Method: 提出IntroLM方法，在因果语言模型的预填充阶段引入内省令牌，通过令牌条件LoRA（仅在内省令牌时激活）让模型学习预测给定查询的输出质量，同时保持原始主干模型行为，避免使用外部评估器。
...
Conclusion: IntroLM提供了一种高效的方法让LLM能够自我评估输出质量，无需外部分类器，在保持模型原始行为的同时显著提升了预测性能，为多模型路由系统带来了更好的成本效益。
...
TL;DR: 该论文提出了个性化音频语言模型（PALM）任务，创建了首个基准测试PALM-Bench，发现现有方法在个性化知识建模和跨任务迁移方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 现有大型音频语言模型（LALMs）虽然在音频理解和生成方面表现出色，但其行为过于通用（如总结口语内容），无法充分支持个性化问答（如总结我最好朋友说的话）。人类会根据每个人的个人背景来调整解释和决策，而当前模型缺乏这种个性化能力。

Method: 1. 形式化定义了PALM任务，专注于识别个人概念和在个人背景中进行推理；2. 创建了首个PALM-Bench基准测试，支持多说话者场景下的结构化评估；3. 对代表性开源LALMs进行了广泛实验，评估了无训练提示和监督微调策略。

Result: 实验表明，现有的无训练提示和监督微调策略虽然能带来改进，但在建模个性化知识和跨任务稳健迁移方面仍然有限。现有方法无法有效处理个性化知识并实现跨任务的鲁棒迁移。

Conclusion: 需要开发更先进的个性化音频语言模型方法来弥补当前模型的不足。论文提出的PALM任务和基准测试为这一研究方向奠定了基础，数据和代码将公开发布以促进方法学进展。
...
TL;DR: 本文提出了一种基于人物感知的视觉语言模型框架，用于自行车友好性评估，通过人物条件化、多粒度微调和数据增强三个创新点，实现了可解释的自行车友好性评估。


<details>
  <summary>Details</summary>
Motivation: 现有的基于感知的自行车友好性评估方法在捕捉复杂道路环境和考虑用户主观感知异质性方面存在局限，需要更全面的评估框架。

Method: 提出人物感知视觉语言模型框架：1) 基于自行车手类型学的理论驱动人物条件化；2) 结合专家标注推理和用户评分的多粒度监督微调；3) 通过AI数据增强创建受控配对数据以隔离基础设施变量影响。

Result: 开发了全景图像众包系统，收集了427名自行车手的12,400个人物条件化评估。实验结果显示该框架在自行车友好性评分预测方面具有竞争力，并能实现可解释的因素归因。

Conclusion: 该框架通过结合理论驱动的人物条件化、多粒度学习和数据增强，为自行车友好性评估提供了可解释的解决方案，能够更好地捕捉道路环境复杂性和用户感知异质性。
...
TL;DR: 研究发现大语言模型在多跳推理中，后跳答案实体可能比桥接实体更早被解码，这种现象称为"层序反转"，且随着总跳数增加而增强。作者提出了"概率性回忆-提取"框架来解释这一现象。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在多跳推理上表现良好，但其内部如何组合多个事实仍不清楚。先前研究提出的"跳对齐电路假设"认为桥接实体在层间顺序计算，但作者发现这一假设不能推广到真实世界的多跳查询中。

Method: 作者通过对真实世界多跳查询的系统分析，发现了层序反转现象。为解释这一行为，提出了"概率性回忆-提取"框架，将多跳推理建模为浅层MLP层中的广泛概率性回忆，随后在深层注意力层中进行选择性提取。

Result: 实证验证了该框架：通过系统探测分析、重新解释先前的逐层解码证据、解释思维链的增益，并提供了多跳失败（尽管单跳知识正确）的机制诊断。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对大语言模型在多样化记忆维度上的系统评估，特别是在多会话设置中。需要一个新的基准来全面评估LLM和智能体系统的多会话记忆能力。

Method: 提出EvolMem基准，基于认知心理学构建，包含陈述性和非陈述性记忆的多个细粒度能力。采用混合数据合成框架，包括主题启动生成和叙事启发转换，可扩展地生成可控复杂度的多会话对话，并配有样本特定的评估指南。

Result: 广泛评估显示，没有LLM在所有记忆维度上始终优于其他模型。此外，智能体记忆机制并不一定能增强LLM的能力，并且经常表现出显著的效率限制。

Conclusion: EvolMem为评估LLM的多会话记忆能力提供了一个全面的基准，揭示了当前模型在记忆能力方面的局限性，特别是智能体记忆机制的效率问题。数据和代码已开源。
...
TL;DR: 该研究提出了一种评估大语言模型在隐私与亲社会动机冲突下决策一致性的框架，通过标准化问卷和结构方程建模来测量模型的价值-行为对齐程度。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常单独测量隐私态度或分享意图，难以确定模型表达的价值是否像真实人类行为一样能共同预测其下游数据分享行动。需要一种能评估在竞争态度下价值-行为对齐的方法。

Method: 引入基于情境的评估协议，在有限的历史保持会话中顺序管理隐私态度、亲社会性和数据分享接受度的标准化问卷。使用多组结构方程建模来识别隐私关注和亲社会性对数据分享的关系，并提出价值-行为对齐率这一人类参考的方向一致性度量。

Result: 在多个大语言模型中观察到稳定但模型特定的隐私-PSA-数据分享接受度特征，以及价值-行为对齐的显著异质性。

Conclusion: 该研究提供了一个系统框架来评估LLMs在隐私与亲社会动机冲突下的决策一致性，揭示了不同模型在价值-行为对齐方面的差异，为理解模型决策机制提供了新视角。
...
TL;DR: 提出PAS框架评估LLM在警务操作中的表现，构建了基于8000+官方文档的QA数据集，发现商用LLM在警务相关任务中表现不佳，特别是在基于事实的建议方面
...
TL;DR: DiffCoT：将思维链推理重新定义为迭代去噪过程的扩散风格框架，通过滑动窗口机制和因果扩散噪声调度提升多步数学问题解决的鲁棒性和纠错能力


<details>
  <summary>Details</summary>
Motivation: 传统的思维链推理存在暴露偏差和错误累积问题，早期错误会在自回归解码过程中不可逆地传播，限制了多步数学问题解决的性能

Method: 提出DiffCoT框架，将思维链推理重新定义为迭代去噪过程，通过滑动窗口机制在推理步骤层面集成扩散原理，同时引入因果扩散噪声调度来保持因果一致性

Result: 在三个多步思维链推理基准测试和多种模型骨干上的实验表明，DiffCoT始终优于现有的思维链偏好优化方法，显著提升了推理的鲁棒性和纠错能力

Conclusion: DiffCoT通过扩散风格的迭代去噪方法有效解决了传统思维链推理中的暴露偏差和错误传播问题，为多步推理任务提供了更稳健的解决方案
...
TL;DR: 本文研究了大语言模型在持续预训练过程中如何学习、保留和遗忘概念，通过分析概念电路来揭示概念学习的动态机制


<details>
  <summary>Details</summary>
Motivation: 人类主要通过概念理解世界，但大语言模型在持续预训练过程中如何获取、保留和遗忘概念尚不清楚。研究者希望理解概念学习的动态过程，包括单个概念的学习遗忘以及多个概念间的相互作用

Method: 通过分析LLMs内部的概念电路（与特定概念相关的计算子图），结合图指标来表征电路结构，研究概念在持续预训练中的行为动态

Result: 1) 概念电路能显著反映概念学习和遗忘；2) 概念电路在持续预训练中呈现阶段性时间模式；3) 学习增益大的概念后续遗忘更明显；4) 语义相似概念干扰更强；5) 概念知识可迁移性存在差异

Conclusion: 研究提供了概念学习动态的电路级视角，为设计更可解释和鲁棒的概念感知训练策略提供了信息，有助于理解LLMs如何获取和保持概念知识
...
TL;DR: PsychEthicsBench：首个基于澳大利亚心理学和精神病学指南的原则驱动基准，用于评估LLM在心理健康领域的伦理知识和行为响应，发现拒绝率是伦理行为的差指标，领域特定微调可能降低伦理鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLM在心理健康应用中的安全对齐主要依赖拒绝式安全信号，但这种方法对临床实践所需的细微行为洞察有限。临床上不恰当的拒绝可能被视为缺乏同理心并阻碍求助行为。

Method: 超越拒绝中心指标，引入PsychEthicsBench基准，基于澳大利亚心理学和精神病学指南，通过多项选择和开放式任务结合细粒度伦理标注来评估LLM的伦理知识和行为响应。

Result: 对14个模型的实证结果显示：拒绝率是伦理行为的差指标，安全触发与临床适当性存在显著差异；领域特定微调可能降低伦理鲁棒性，多个专业模型在伦理对齐上表现不如其基础模型。

Conclusion: PsychEthicsBench为心理健康领域LLM的系统性、司法管辖区感知评估提供了基础，鼓励该领域更负责任的发展。拒绝率不能准确反映临床适当性，需要更细致的伦理评估框架。
...
TL;DR: LLMs在处理多语言代码切换时存在输出语言对齐问题，即使上下文线索清晰也经常无法正确推断用户期望的输出语言，表现出对非英语回复的系统性偏见。


<details>
  <summary>Details</summary>
Motivation: 多语言用户在对话中自然地进行代码切换（混合使用不同语言），但大型语言模型在处理这类交互时面临根本性挑战。当用户在提示中进行代码切换时，通常不会明确指定期望的LLM回复语言，因此LLMs必须从上下文和语用线索中推断输出语言。研究发现当前LLMs系统性地无法满足这一期望。

Method: 引入OLA基准来评估LLMs在代码切换交互中的输出语言对齐能力，专注于韩语-英语代码切换，涵盖从简单的句内混合到指令内容不匹配的各种情况。进一步测试了链式思维提示和代码切换感知DPO（使用约1K示例）等方法。

Result: 即使是前沿模型也经常误解隐含的语言期望，表现出对非英语回复的系统性偏见，这种偏见在中文和印尼语对中也普遍存在。模型还表现出不稳定性，如中间回复切换和语言侵入。链式思维提示无法解决这些错误，但代码切换感知DPO能显著减少不对齐问题。

Conclusion: LLMs在代码切换交互中的失败源于对齐不足而非根本性限制。需要将多语言LLMs与用户在真实世界代码切换交互中的隐含期望对齐，代码切换感知DPO是有效的解决方案。
...
TL;DR: 论文提出Self-Graph Reasoning框架，让大语言模型在开放域问答中进行图结构推理，提升推理一致性和准确性


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型的推理过程通常是线性的且逻辑不一致，而真实世界推理需要整合多个前提并并行解决子问题。现有方法如思维链虽然以线性文本形式表达推理，但经常导致不一致结论。现有方法依赖外部提供的图结构，未探索LLM如何构建和使用自己的图结构推理

Method: 提出Self-Graph Reasoning框架，使LLM在生成最终答案前，能将其推理过程显式表示为结构化图。构建图结构推理数据集，将多个候选推理图合并为精炼的图结构用于模型训练

Result: 在五个QA基准测试中，SGR持续提升推理一致性，相比基础模型获得17.74%的性能提升。使用SGR微调的LLaMA-3.3-70B模型性能与GPT-4o相当，超过Claude-3.5-Haiku

Conclusion: 图结构推理能有效提升大语言模型在开放域问答中的推理能力，Self-Graph Reasoning框架展示了LLM构建和使用自身图结构推理的潜力
...
TL;DR: 音频语言模型通过推理轨迹提供可解释的音频深度伪造检测，但需要评估其推理在对抗攻击下的鲁棒性。研究发现显式推理并不总是增强鲁棒性，而是呈现分化：对具有鲁棒声学感知的模型，推理起到防御作用；对其他模型则造成性能损失，尤其是在语言攻击下。


<details>
  <summary>Details</summary>
Motivation: 音频语言模型为音频深度伪造检测提供了可解释性，超越了传统的黑盒分类器。然而，现有研究主要关注最终预测的变化，缺乏对模型推理过程在对抗攻击下鲁棒性的系统分析。需要评估推理过程在攻击下的稳定性，以理解可解释性模型的真实安全性。

Method: 提出了一个法证审计框架，从三个相互关联的维度评估音频语言模型推理在对抗攻击下的鲁棒性：声学感知、认知一致性和认知失调。通过系统分析不同攻击场景下模型推理的变化，研究推理过程对模型防御能力的影响。

Result: 研究发现显式推理并不普遍增强鲁棒性。对于具有鲁棒声学感知的模型，推理起到"盾牌"的防御作用；但对其他模型，推理造成性能"税"，特别是在降低认知一致性的语言攻击下，攻击成功率增加。即使分类失败，高认知失调也可作为"无声警报"标记潜在操纵。

Conclusion: 这项工作对推理在法证音频深度伪造分析中的作用及其脆弱性进行了批判性评估。研究表明，虽然推理提供了可解释性，但并不自动保证鲁棒性，其防御效果取决于模型的声学感知能力。认知失调可作为检测操纵的补充指标。
...
TL;DR: EPAG是一个评估LLMs预咨询能力的基准数据集和框架，通过病史-诊断指南对比和疾病诊断来评估模型，发现特定任务微调的小型开源模型可以超越前沿LLM，且更多病史信息不一定改善诊断性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个评估大型语言模型在临床预咨询阶段能力的基准，以促进LLM在真实临床环境中的应用和发展。

Method: 创建EPAG基准数据集和评估框架，通过直接评估（病史-诊断指南对比）和间接评估（疾病诊断）来测试LLMs的预咨询能力，并进行多语言预咨询对话特征分析。

Result: 1）使用精心策划的任务特定数据集微调的小型开源模型在预咨询任务中表现优于前沿LLMs；2）增加病史信息量不一定提高诊断性能；3）预咨询语言影响对话特征。

Conclusion: EPAG为评估LLMs在临床预咨询能力提供了有效工具，特定任务微调的小模型可能优于大模型，且预咨询对话质量比信息量更重要，开源数据集和评估流程有助于推动LLM在临床环境的应用。
...
TL;DR: 本文首次系统比较了大型推理模型(LRMs)与非推理LLMs作为评估者的优劣，发现LRMs在判断准确性、指令遵循和对抗攻击鲁棒性方面更优，但仍有表面质量偏见。提出的PlanJudge策略能显著缓解偏见。


<details>
  <summary>Details</summary>
Motivation: 研究动机是系统性地探究大型推理模型(LRMs)是否比非推理LLMs更适合作为评估者，特别是在需要推理能力的判断任务中，以确定哪种模型架构更适合评估应用。

Method: 采用实证分析方法，首先系统比较LRMs与非推理LLMs在判断准确性、指令遵循能力、对抗攻击鲁棒性等方面的表现。然后提出PlanJudge策略，通过让模型在执行前生成明确的评估计划来缓解偏见问题。

Result: 1) LRMs在判断准确性上优于非推理LLMs，特别是在推理密集型任务中；2) LRMs在评估情境中展现出更好的指令遵循能力；3) LRMs对针对判断任务的对抗攻击具有更强的鲁棒性；4) 但LRMs仍存在明显的表面质量偏见。PlanJudge策略能显著缓解LRMs和标准LLMs中的偏见问题。

Conclusion: LRMs作为评估者在多个维度上优于非推理LLMs，但仍存在偏见问题。提出的PlanJudge策略虽然简单，但能有效提高评估的鲁棒性，为改进LLM评估方法提供了实用方案。
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体需要在动态环境中持续学习新任务而不发生灾难性遗忘，但现有的稳定性-可塑性困境限制了其持续学习能力。作者认为这一困境源于未能明确区分任务间共享的通用知识和任务特定干扰引入的冲突知识。

Method: 提出Agent-Dice框架，采用两阶段知识解耦过程：1）几何共识过滤来修剪冲突梯度；2）基于曲率的重要性加权来增强共享语义。该框架基于方向共识评估进行参数融合。

Result: 在GUI智能体和工具使用智能体领域的广泛实验表明，Agent-Dice展现出优异的持续学习性能，同时具有最小的计算开销和参数更新。

Conclusion: 通过明确区分共享知识和冲突知识，Agent-Dice有效解决了LLM智能体持续学习中的稳定性-可塑性困境，为智能体的持续学习提供了理论分析和实用框架。
...
TL;DR: 提出了LLM-MC-Affect框架，使用概率分布和蒙特卡洛估计来捕捉对话中的情感动态和模糊性，通过情感轨迹分析人际耦合关系


<details>
  <summary>Details</summary>
Motivation: 现有文本情感分析方法将情感视为确定性的点估计，无法捕捉对话中固有的主观性、潜在模糊性和序列耦合特性，需要更精细的情感协调分析框架

Method: 引入LLM-MC-Affect概率框架，将情感建模为情感空间上的连续潜在概率分布，利用随机LLM解码和蒙特卡洛估计来近似这些分布，生成高保真情感轨迹，通过序列互相关和斜率指标分析人际耦合

Result: 在师生教学对话案例研究中，定量指标成功提取了高层次交互洞察（如有效脚手架），验证了方法的解释能力

Conclusion: 该工作为理解人际动态建立了可扩展且可部署的途径，提供了超越教育领域、适用于更广泛社会和心理学研究的通用解决方案
...
TL;DR: ELO方法通过层特定优化提升多语言大模型在特定语言上的持续预训练效率，减少计算成本并防止源语言性能退化


<details>
  <summary>Details</summary>
Motivation: 传统持续预训练方法存在计算成本高、源语言性能退化的问题，需要一种更高效的特定语言优化方法

Method: ELO方法包含两个阶段：1) ELO预训练阶段，分离关键的第一层和最后一层进行目标语言训练；2) 层对齐阶段，将训练好的层重新整合并进行简短的全模型微调

Result: ELO方法相比现有方法训练速度提升高达6.46倍，目标语言性能提升6.2%，同时有效保持了英语等源语言能力

Conclusion: ELO方法为多语言大模型的特定语言优化提供了一种高效且有效的解决方案，显著降低了计算资源需求
...
<details>
  <summary>Details</summary>
Motivation: 解决高风险生物医学领域中系统生成响应的有效验证问题，确保生成内容的可追溯性和可信度

Method: 提出eTracer框架，采用后验对齐方法，将每个响应声明与上下文证据进行对齐（支持或反驳），基于声明级对齐结果实现可追溯性

Result: 实验表明，声明级对齐方法克服了传统句子级对齐方法的局限，显著提升了整体对齐质量和用户验证效率

Conclusion: eTracer框架通过声明级证据对齐，实现了生成响应的可追溯性和可验证性，提高了生物医学领域系统响应的可信度
...
TL;DR: DisastQA是一个包含3000个经过严格验证问题的灾难管理QA基准，涵盖8种灾害类型，用于评估模型在不确定和冲突信息下的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有QA基准建立在干净证据基础上，无法准确评估灾难管理中需要处理不确定和冲突信息的真实场景。

Method: 通过人-LLM协作流水线构建大规模基准，包含2000个多项选择和1000个开放性问题；采用分层抽样确保平衡覆盖；提出基于关键点的人类验证评估协议。

Result: 评估20个模型显示与通用排行榜（如MMLU-Pro）存在显著差异；开源模型在干净设置下接近专有系统，但在现实噪声下性能急剧下降。

Conclusion: DisastQA基准揭示了灾难响应中模型可靠性的关键差距，特别是在处理不完美信息时，为评估灾难管理QA系统提供了重要工具。
...
TL;DR: NeuronScope是一个多智能体框架，通过迭代、激活引导的过程解决LLM神经元多义性问题，相比单次解释方法能更准确地揭示隐藏的多义性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的神经元解释面临多义性挑战，单个神经元可能响应多个不同的语义概念，现有单次解释方法难以准确捕捉这种多概念行为。

Method: 提出NeuronScope多智能体框架，将神经元解释重新定义为迭代、激活引导的过程，明确将神经元激活分解为原子语义组件，聚类为不同的语义模式，并使用神经元激活反馈迭代优化每个解释。
...
### [106] [From Implicit to Explicit: Token-Efficient Logical Supervision for Mathematical Reasoning in LLMs](https://arxiv.org/abs/2601.03682)
*Shaojie Wang,Liang Zhang*

Main category: cs.CL

TL;DR: 该论文提出FSLR训练框架，专注于提升大语言模型在数学问题解决中的逻辑关系理解能力，相比传统CoT-SFT方法在性能、训练效率和资源消耗方面均有显著提升。
...
Method: 提出First-Step Logical Reasoning (FSLR)训练框架，专注于训练模型识别问题中应使用的变量和操作（即规划的第一步），通过这一孤立步骤提供对逻辑关系理解的显式监督。

Result: FSLR在多个模型和数据集上一致优于CoT-SFT，在分布内和分布外设置下平均分别提升3.2%和4.6%，训练速度提升4-6倍，训练token消耗减少80%以上。

Conclusion: FSLR通过专注于逻辑关系理解的第一步规划，有效提升大语言模型的逻辑推理能力，相比传统方法在性能、效率和资源效率方面均有显著优势。
...
### [107] [Evaluation Framework for AI Creativity: A Case Study Based on Story Generation](https://arxiv.org/abs/2601.03698)
*Pharath Sathya,Yin Jou Huang,Fei Cheng*

Main category: cs.CL

TL;DR: 本文提出了一个包含四个主要维度（新颖性、价值、一致性、共鸣）和十一个子维度的结构化评估框架，用于评估AI故事生成的创造性。通过"Spike Prompting"控制故事生成和115名读者的众包研究，发现创造性评估是层次性的而非累积性的，不同维度在不同判断阶段变得显著。


<details>
  <summary>Details</summary>
Motivation: 现有基于参考的评估指标无法捕捉创造性的主观本质，因此需要一个新的框架来评估AI故事生成的创造性。

Method: 使用"Spike Prompting"技术控制故事生成，通过115名读者的众包研究，分析不同创造性维度如何影响即时和反思性的人类创造性判断。

Result: 研究发现创造性评估是层次性的而非累积性的，不同维度在不同判断阶段变得显著；反思性评估显著改变了评分和评分者间的一致性。

Conclusion: 该框架能有效揭示被基于参考的评估所掩盖的创造性维度，为AI故事生成的创造性评估提供了更全面的方法。
...
### [108] [RedBench: A Universal Dataset for Comprehensive Red Teaming of Large Language Models](https://arxiv.org/abs/2601.03699)
...
Result: 建立了现代大语言模型的基准测试，开源了数据集和评估代码，促进了模型漏洞的全面评估。

Conclusion: RedBench为系统评估大语言模型对抗性提示的鲁棒性提供了标准化工具，促进了安全可靠的大语言模型开发。
...
### [109] [ADEPT: Adaptive Dynamic Early-Exit Process for Transformers](https://arxiv.org/abs/2601.03700)
...
Method: 提出自适应token级早退机制，根据token复杂度动态调整计算；通过解耦跳过层的序列依赖关系，优化KV生成过程，使token级早退更实用。

Result: 在语言生成任务中效率提升达25%，下游分类任务实现4倍加速，性能提升达45%。

Conclusion: ADEPT通过自适应动态早退机制有效解决了传统早退策略中KV缓存的瓶颈问题，在保持性能的同时显著提升了Transformer模型的推理效率。
...
### [110] [AirNav: A Large-Scale Real-World UAV Vision-and-Language Navigation Dataset with Natural and Diverse Instructions](https://arxiv.org/abs/2601.03707)
...
<details>
  <summary>Details</summary>
Motivation: 现有无人机视觉语言导航数据集存在三个主要问题：依赖虚拟环境、指令不够自然、数据集规模有限。这些问题限制了无人机导航系统在真实世界中的应用和性能。

Method: 1. 构建AirNav数据集：使用真实城市航拍数据而非合成环境，提供自然多样的导航指令；2. 提出AirVLN-R1模型：结合监督微调和强化微调来提升性能和泛化能力；3. 进行真实世界测试验证可行性。

Result: 成功构建了基于真实航拍数据的大规模无人机VLN基准数据集AirNav，并开发了AirVLN-R1模型。通过真实世界测试初步验证了模型的可行性。数据集和代码已公开可用。

Conclusion: AirNav解决了现有无人机VLN数据集的关键限制，为真实世界无人机导航研究提供了更好的基准。AirVLN-R1模型展示了在真实环境中的可行性，推动了无人机视觉语言导航技术的发展。
...
### [111] [Visual Merit or Linguistic Crutch? A Close Look at DeepSeek-OCR](https://arxiv.org/abs/2601.03714)
*Yunhao Liang,Ruixuan Ying,Bo Li,Hong Li,Kai Yan,Qingwen Li,Min Yang,Okamoto Satoshi,Zhe Cui,Shiwen Ni*

Main category: cs.CL

TL;DR: 该研究质疑DeepSeek-OCR的性能主要依赖语言先验而非真正的视觉OCR能力，通过语义破坏实验发现其性能从90%暴跌至20%，且传统OCR方法比端到端方法更鲁棒


<details>
  <summary>Details</summary>
Motivation: DeepSeek-OCR声称通过光学2D映射实现高比率视觉-文本压缩，能解码超过输入视觉标记10倍的文本标记，这为解决LLM长上下文瓶颈提供了可能方案。但研究者质疑其性能究竟来自真正的视觉OCR能力还是语言先验知识

Method: 采用句子级和单词级语义破坏技术，分离模型的固有OCR能力与语言先验。对13个基线模型进行对比基准测试，分析视觉标记数量与先验依赖的关系，并进行上下文压力测试

Result: 在没有语言支持的情况下，DeepSeek-OCR性能从约90%暴跌至20%。传统流水线OCR方法比端到端方法对语义扰动更具鲁棒性。较低的视觉标记数量与更强的先验依赖相关，增加了幻觉风险。在约10,000个文本标记处出现模型完全崩溃
...
### [112] [Stuttering-Aware Automatic Speech Recognition for Indonesian Language](https://arxiv.org/abs/2601.03727)
...
<details>
  <summary>Details</summary>
Motivation: 自动语音识别系统在处理流畅语音时表现优异，但在处理口吃语音时性能显著下降，特别是对于印尼语等低资源语言，缺乏专门的口吃语音数据集

Method: 1. 提出数据增强框架，通过规则转换和大型语言模型在流畅文本中注入重复和延长等口吃模式；2. 使用文本转语音合成生成合成口吃音频；3. 应用合成数据通过迁移学习微调预训练的印尼语Whisper模型

Result: 实验表明，这种有针对性的合成数据暴露能够持续减少口吃语音的识别错误，同时在流畅语音段上保持性能，验证了合成数据管道在开发更具包容性语音技术方面的实用性

Conclusion: 合成数据管道可以有效解决低资源语言口吃语音识别数据稀缺问题，为开发更具包容性的语音技术提供了可行方案，特别是在资源不足的语言环境中
...
### [113] [O-Researcher: An Open Ended Deep Research Model via Multi-Agent Distillation and Agentic RL](https://arxiv.org/abs/2601.03743)
...
TL;DR: 提出自动化合成高质量训练数据框架，通过多智能体协作生成研究级教学数据，采用两阶段训练策略提升开源大语言模型性能
...
Result: 实验表明该框架能显著提升多种规模的开源模型性能，在主要深度研究基准上达到新的最先进水平
...
### [114] [Evaluation of Multilingual LLMs Personalized Text Generation Capabilities Targeting Groups and Social-Media Platforms](https://arxiv.org/abs/2601.03752)
*Dominik Macko*

Main category: cs.CL

TL;DR: 该研究探讨了大型语言模型在多语言环境下个性化文本生成的能力及其对可检测性的影响，覆盖10种语言、1080种个性化组合和16个模型，发现个性化质量存在语言差异，且平台个性化对文本可检测性影响更大。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型生成多语言连贯文本能力的增强，对其潜在滥用的担忧也在增加。先前研究表明这些模型可能被滥用于生成多语言的个性化虚假信息，且个性化会降低机器生成文本的可检测性，但这一现象仅在英语中被研究过。

Method: 研究覆盖10种语言，考察个性化能力的潜在滥用和潜在益处。设计了1080种不同个性化方面的提示组合，使用16个不同的语言模型生成文本（总计17,280个文本），分析个性化质量对文本可检测性的影响。

Result: 结果显示，针对不同人口群体和社交媒体平台的个性化文本生成质量存在语言差异。平台个性化对生成文本的可检测性影响更大，特别是在英语中，其个性化质量最高。

Conclusion: 多语言环境下个性化文本生成存在显著差异，平台个性化对可检测性的影响尤为明显，这为理解和防范大型语言模型的潜在滥用提供了重要见解。
...
### [115] [Do LLM Self-Explanations Help Users Predict Model Behavior? Evaluating Counterfactual Simulatability with Pragmatic Perturbations](https://arxiv.org/abs/2601.03775)
*Pingjun Hong,Benjamin Roth*

Main category: cs.CL

TL;DR: 研究探讨LLM自我解释是否能帮助用户预测模型行为，通过反事实模拟性评估，发现自我解释能提高模拟准确性，但效果受扰动策略和评估者能力影响。
...
Result: 自我解释能持续提高LLM评估者和人类的模拟准确性，但增益程度和稳定性强烈依赖于扰动策略和评估者能力。定性分析显示，访问解释能帮助人类对扰动问题形成更准确的预测。

Conclusion: LLM的自我解释虽然可能不完全反映真实决策过程，但在帮助用户预测模型行为方面具有实际价值，特别是在反事实模拟任务中，但效果受到多种因素影响。
...
### [116] [Tracing the complexity profiles of different linguistic phenomena through the intrinsic dimension of LLM representations](https://arxiv.org/abs/2601.03779)
*Marco Baroni,Emily Cheng,Iria deDios-Flores,Francesca Franzon*

Main category: cs.CL

TL;DR: 本文研究LLM表示的内在维度作为语言复杂性的标记，探索不同LLM层的内在维度特征如何区分形式和功能复杂性。


<details>
  <summary>Details</summary>
Motivation: 研究LLM表示的内在维度是否能够作为语言复杂性的有效标记，以及不同内在维度特征是否能够区分形式和功能复杂性。

Method: 使用内在维度分析LLM表示，比较不同句子结构（如多从句协调、从属关系、右分支vs中心嵌入、明确vs模糊关系从句附着）的内在维度差异，并结合表示相似性和层消融实验进行验证。

Result: 形式复杂性（多从句协调和从属关系）的内在维度差异与早期研究中独立识别的更抽象语言处理阶段对齐；功能复杂性（右分支vs中心嵌入、明确vs模糊关系从句附着）的内在维度差异较弱，且不与同一处理阶段相关。
...
### [117] [HearSay Benchmark: Do Audio LLMs Leak What They Hear?](https://arxiv.org/abs/2601.03783)
...
<details>
  <summary>Details</summary>
Motivation: 音频大语言模型在理解和生成方面取得了显著进展，但其潜在的隐私影响尚未得到充分探索。研究者旨在调查ALLMs是否仅通过声学声纹就会无意中泄露用户隐私，填补这一研究空白。

Method: 构建了HearSay基准，包含超过22,000个真实世界音频片段，通过自动化分析和人工验证的严格流程确保数据质量。在基准上进行了广泛的实验，评估ALLMs从声纹中提取私人属性的能力、现有安全机制的有效性以及推理过程对隐私风险的影响。

Result: 1. 显著的隐私泄露：ALLMs能够从声纹中提取私人属性，性别识别准确率达92.89%，并能有效分析社会属性；2. 安全机制不足：现有防护措施严重不足，大多数模型无法拒绝隐私侵入请求，对生理特征的拒绝率接近零；3. 推理加剧风险：思维链推理在能力较强的模型中通过揭示更深层的声学相关性而加剧隐私风险。

Conclusion: 研究揭示了ALLMs中存在的关键隐私漏洞，强调了针对性的隐私对齐的紧迫需求。这些发现为开发更安全的音频AI系统提供了重要依据，相关代码和数据集已开源。
...
TL;DR: 本文提出了一种新的选择相关性评分方法，用于评估训练数据影响估计中示例选择策略的质量，发现常见策略往往不如随机选择，并提出了一种平衡影响力和代表性的改进策略。


<details>
  <summary>Details</summary>
Motivation: 训练数据影响估计方法可以量化训练文档对模型输出的贡献，但人类无法解释数千个文档，只能展示一小部分作为解释。现有评估系统大多忽略了选择策略对解释质量的影响，而选择哪些文档直接影响解释效果。
...
Conclusion: 选择策略在训练数据影响估计的解释系统中至关重要，提出的选择相关性评分和平衡策略能够有效提高解释质量，为基于示例的解释提供了更好的方法。
...
TL;DR: 本文提出了一种新的记忆评估框架CRM，通过控制提示词线索来重新评估LLMs中的PII泄露问题，发现之前报告的PII泄露主要是由表面形式线索驱动而非真正的记忆。


<details>
  <summary>Details</summary>
Motivation: 现有研究将LLMs成功重建PII解释为记忆证据，但作者认为这种评估方法存在问题，因为PII重建可能通过提示诱导的泛化或模式完成实现，而非真正的记忆。需要建立更严谨的记忆评估框架来准确量化LLMs中的隐私相关记忆。

Method: 提出Cue-Resistant Memorization (CRM)框架，作为记忆评估的必要条件，明确控制提示-目标重叠线索。在32种语言上进行大规模多语言重新评估，涵盖多种记忆范式，包括逐字前缀-后缀完成、关联重建、无线索生成和成员推理。

Result: 当控制表面形式线索后，重建成功率显著下降；无线索生成和成员推理的真实阳性率极低。之前报告的PII泄露效果主要来自直接表面形式线索驱动，而非真正的记忆。

Conclusion: 先前报道的PII泄露应更好地解释为线索驱动行为而非真正的记忆，强调了线索控制评估对于可靠量化LLMs中隐私相关记忆的重要性。
...
TL;DR: 提出了VietMed-MCQ数据集，这是一个针对越南传统医学(VTM)的多选题数据集，通过RAG管道生成并采用自动一致性检查机制，包含3,190个问题，用于评估LLM在低资源医学领域的表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用医学领域表现出色，但在越南传统医学等特定文化领域的专业领域中表现显著下降，主要原因是缺乏高质量、结构化的基准测试数据集。

Method: 使用检索增强生成(RAG)管道创建多选题数据集，采用双模型验证方法通过独立答案验证确保推理一致性，包含基于子字符串的证据检查机制，数据集涵盖三个难度级别。

Result: 数据集包含3,190个问题，经过1名医学专家和4名学生验证，获得94.2%的批准率，组间一致性高(Fleiss' kappa = 0.82)。基准测试显示，具有强中文先验的通用模型优于越南中心模型，表明跨语言概念迁移，但所有模型在复杂诊断推理方面仍有困难。

Conclusion: VietMed-MCQ数据集填补了越南传统医学领域基准测试的空白，揭示了跨语言知识迁移的重要性，为低资源医学领域研究提供了有价值的资源，尽管基于子字符串的证据检查存在已知局限性。
...
TL;DR: 通过系统性的分层探测研究发现，Transformer模型中心理学意义的编码位置高度依赖于方法选择，不同嵌入提取方法产生不同的特征选择性和层间分布模式


<details>
  <summary>Details</summary>
Motivation: 理解Transformer语言模型在何处编码具有心理学意义的内容，对于理论和实践都至关重要，需要系统研究不同模型架构和方法下心理学特征的编码模式

Method: 对10个Transformer模型（包括编码器-仅和解码器-仅架构）进行分层探测研究，涵盖58个心理语言学特征，比较三种嵌入提取方法（上下文嵌入与孤立嵌入）

Result: 发现意义的定位高度依赖于方法：上下文嵌入比孤立嵌入具有更高的特征选择性和不同的层间分布；最终层表示很少是恢复心理语言学信息的最佳选择；不同模型共享意义维度的深度排序模式

Conclusion: Transformer模型中"意义"的编码位置反映了方法论选择与架构约束之间的相互作用，需要谨慎解释探测研究的结果
...
TL;DR: 该论文评估了AI文本检测方法，发现基于上下文的语义建模（如DistilBERT）在检测AI生成文本方面显著优于基于词汇特征的传统方法，强调了适当评估协议对防止主题记忆的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的快速发展，AI生成文本增多，学生使用LLM生成内容作为自己作业的现象日益普遍，这违反了学术诚信。因此需要评估有效的AI文本检测方法。

Method: 使用HC3和DAIGT v2两个数据集构建统一基准，采用基于主题的数据分割防止信息泄露。评估了传统机器学习模型（TF-IDF逻辑回归）和基于transformer的架构（BiLSTM、DistilBERT）。

Result: TF-IDF逻辑回归达到82.87%的准确率，BiLSTM达到88.86%，DistilBERT达到88.11%但ROC-AUC最高（0.96），表现最佳。结果表明上下文语义建模显著优于词汇特征。

Conclusion: 上下文语义建模在AI文本检测中表现优越，适当评估协议对防止主题记忆至关重要。未来工作将扩展数据集多样性，使用参数高效微调方法（如LoRA），探索更小或蒸馏模型，并采用更高效的批处理策略和硬件感知优化。
...
TL;DR: SPAE提出了一种基于步骤潜力的优势估计方法，通过提取中间置信度和正确性来改进强化学习中的奖励信号，提高推理准确性并减少响应长度。
...
Method: 提出无训练探测机制提取中间置信度和正确性，组合成步骤潜力信号；基于此信号设计SPAE方法，放大潜力增益、惩罚潜力下降，并在潜力饱和后应用惩罚以鼓励及时终止。

Result: 在多个基准测试中，SPAE在提高准确性的同时显著减少响应长度，优于强RL基线和近期高效推理及令牌级优势估计方法。

Conclusion: SPAE通过步骤级潜力信号实现了细粒度信用分配，解决了RLVR中过程监督不足的问题，为LLMs推理提供了更有效的强化学习框架。
...
### [124] [Rethinking Table Pruning in TableQA: From Sequential Revisions to Gold Trajectory-Supervised Parallel Search](https://arxiv.org/abs/2601.03851)
*Yu Guo,Shenghao Ye,Shuangwu Chen,Zijian Wen,Tao Zhang,Qirui Bai,Dong Jin,Yunpeng Hou,Huasen He,Jian Yang,Xiaobin Tan*

Main category: cs.CL

TL;DR: TabTrim：一种将表格剪枝从顺序修订转变为黄金轨迹监督并行搜索的新框架，通过训练剪枝器和验证器来对齐黄金剪枝轨迹，在推理时并行搜索最优子表


<details>
  <summary>Details</summary>
Motivation: 现有表格问答中的表格剪枝方法依赖不可靠的批判信号进行顺序修订，往往无法检测答案关键数据的丢失，需要更有效的剪枝框架

Method: 提出TabTrim框架：1）从黄金SQL查询执行过程中提取中间子表构建黄金剪枝轨迹；2）训练剪枝器和验证器使逐步剪枝结果对齐黄金轨迹；3）推理时进行并行搜索探索多个候选剪枝轨迹并识别最优子表

Result: TabTrim-8B达到73.5%的平均准确率，比最强基线提高3.2%，在WikiTQ上达到79.4%，在TableBench上达到61.2%，在多样化表格推理任务中实现最先进性能
...
### [125] [PartisanLens: A Multilingual Dataset of Hyperpartisan and Conspiratorial Immigration Narratives in European Media](https://arxiv.org/abs/2601.03860)
...
TL;DR: 本文介绍了PartisanLens——首个多语言超党派新闻标题数据集，包含西班牙语、意大利语和葡萄牙语的1617个样本，用于检测超党派叙事和人口替代阴谋论，并评估了LLM在此任务上的分类性能和自动标注能力。


<details>
  <summary>Details</summary>
Motivation: 检测超党派叙事和人口替代阴谋论对应对错误信息传播至关重要。现有资源稀缺、以英语为中心，且往往孤立分析超党派性、立场和修辞偏见，而非将其作为政治话语的相互关联方面进行研究。

Method: 1) 创建PartisanLens多语言数据集（西班牙语、意大利语、葡萄牙语，1617个超党派新闻标题）；2) 评估主流LLM在该数据集上的分类性能；3) 评估LLM作为自动标注器的可行性；4) 探索LLM通过社会经济和意识形态配置文件模拟人类标注模式的能力。

Result: 建立了超党派和PRCT叙事的稳健基线分类性能，评估了LLM作为自动标注器的潜力与局限性，探索了LLM模拟人类标注模式的可能性，为欧洲语境下的党派和阴谋论叙事检测研究提供了资源支持。

Conclusion: PartisanLens填补了多语言政治话语分析资源的空白，为检测欧洲语境下的超党派和阴谋论叙事提供了重要数据集和评估框架，支持未来相关研究发展。
...
### [126] [What Matters For Safety Alignment?](https://arxiv.org/abs/2601.03868)
*Xing Li,Hui-Ling Zhen,Lihao Yin,Xianzhi Yu,Zhenhua Dong,Mingxuan Yuan*

Main category: cs.CL

TL;DR: 该论文对LLM和LRM的安全对齐能力进行了大规模实证研究，评估了6个内在模型特性和3种外部攻击技术的影响，发现推理和自反思机制对安全对齐有显著优势，同时揭示了后训练和知识蒸馏可能导致安全对齐系统性退化，以及CoT攻击通过响应前缀可大幅提升攻击成功率等关键发现。


<details>
  <summary>Details</summary>
Motivation: 为了评估LLM和LRM的安全对齐能力，了解影响安全对齐的关键因素，为开发更安全可靠的AI系统提供重要见解。研究旨在系统性地探究内在模型特性和外部攻击技术对安全对齐的影响。

Method: 使用32个近期流行的LLM和LRM模型，涵盖13个不同模型家族，参数规模从3B到235B。评估采用5个已建立的安全数据集，使用56种越狱技术和4种CoT攻击策略进行漏洞探测，共进行了460万次API调用。系统研究6个关键内在模型特性和3种外部攻击技术的影响。

Result: 1. GPT-OSS-20B、Qwen3-Next-80B-A3B-Thinking和GPT-OSS-120B是安全性最高的三个LRM模型，表明集成推理和自反思机制对稳健安全对齐有显著优势。
2. 后训练和知识蒸馏可能导致安全对齐系统性退化，安全应作为这些阶段的明确约束或核心优化目标。
3. CoT攻击通过响应前缀可将攻击成功率平均提升3.34倍，对Seed-OSS-36B-Instruct的攻击成功率从0.6%提升至96.3%。
4. 角色扮演、提示注入和基于梯度的对抗提示搜索是现代模型中引发未对齐行为的主要方法。

Conclusion: 安全对齐需要将推理和自反思机制作为核心设计要素，在后训练和知识蒸馏阶段应将安全作为明确约束而非次要目标。文本完成接口和允许用户定义响应前缀的功能存在重大安全风险，需要架构和部署层面的防护措施。研究为开发更安全的AI系统提供了实证基础和具体建议。
...
### [127] [Atlas: Orchestrating Heterogeneous Models and Tools for Multi-Domain Complex Reasoning](https://arxiv.org/abs/2601.03872)
...
### [128] [Evaluating Small Decoder-Only Language Models for Grammar Correction and Text Simplification](https://arxiv.org/abs/2601.03874)
*Anthony Lamelas*

Main category: cs.CL

TL;DR: 小型语言模型在语法纠错和文本简化任务上表现不及大型语言模型，尽管效率更高但性能差距明显


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然性能强大但计算成本高、部署困难，需要探索小型语言模型是否能作为语法纠错和文本简化任务的高效替代方案
...
TL;DR: Arctic-ABSA是一套针对商业需求优化的方面级情感分析模型，通过扩展数据集、增加情感类别、多语言支持以及推理注入技术，在多个基准测试中超越了GPT-4o和Claude 3.5 Sonnet等大型模型。


<details>
  <summary>Details</summary>
Motivation: 针对商业场景中的实际需求，现有方面级情感分析模型在数据集规模、情感类别覆盖、多语言支持等方面存在不足，需要开发更强大、更实用的模型。

Method: 1) 构建大规模数据集：结合公开数据和合成数据，数据集规模是SemEval14的20倍；2) 扩展模型能力：将情感类别从3类扩展到5类（增加混合和未知类别），联合预测整体文本情感；3) 推理注入技术：通过Chain-of-Thought示例微调，并引入新颖的推理预训练技术；4) 多语言支持：开发单模型支持多种语言。

Result: 1) 395M参数编码器和8B参数解码器在准确率上比GPT-4o和Claude 3.5 Sonnet高出最多10个百分点；2) 在SemEval14基准测试中创下新的SOTA结果；3) 单多语言模型在6种语言上保持87-91%的准确率，且不降低英语性能；4) 发布了ABSA-mix基准，整合了17个公共ABSA数据集覆盖92个领域。

Conclusion: Arctic-ABSA通过创新的数据集构建、模型架构扩展和推理注入技术，为商业应用提供了强大且实用的方面级情感分析解决方案，在多语言性能和准确率方面显著超越了现有大型语言模型。
...
TL;DR: DTR框架通过基于生成不确定性的自适应检索触发和双路径检索机制，改进RAG性能，减少不必要检索
...
Conclusion: DTR是一个无需训练的框架，通过智能的检索决策和双路径检索机制，有效提升RAG性能并减少资源消耗，代码和数据已开源。
...
TL;DR: 研究发现语言模型在多选题回答中存在两阶段机制：先选择正确答案内容，再将内容绑定到输出符号


<details>
  <summary>Details</summary>
Motivation: 多选题回答容易评估但存在元任务问题：模型既要解决问题又要输出代表答案的符号，这混淆了推理错误和符号绑定失败。研究旨在理解语言模型内部如何实现多选题回答。

Method: 使用表征分析（PCA、线性探针）和因果干预研究语言模型的内部实现。分析选项边界（换行）残差状态，研究获胜内容位置和输出符号的表示。

Result: 发现选项边界残差状态包含与每个选项正确性相关的强线性可解码信号。获胜身份探针显示两阶段进展：获胜内容位置在最终选项处理后立即可解码，而输出符号在答案发射位置附近表示。符号和内容置换测试支持两阶段机制。

Conclusion: 语言模型在多选题回答中采用两阶段机制：首先在内容空间选择获胜者，然后将该获胜者绑定或路由到适当的符号进行输出。
...
TL;DR: 本文提出了Doc-PP基准测试，用于评估大型视觉语言模型在文档问答中的政策遵从性，发现了推理引发的安全漏洞，并提出了DVA框架来增强安全性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中文档问答部署受到动态用户定义政策的约束，现有安全研究主要关注隐式社会规范或纯文本设置，忽略了多模态文档的复杂性，需要专门评估模型在严格非披露政策下的表现。

Method: 1. 构建Doc-PP基准测试，基于真实世界报告，要求跨异质视觉和文本元素进行推理；2. 提出DVA（分解-验证-聚合）结构化推理框架，将推理与政策验证解耦；3. 通过实验评估模型在政策遵从性方面的表现。

Result: 1. 发现了系统性"推理引发的安全漏洞"：模型在需要通过复杂合成或跨模态聚合推理答案时频繁泄露敏感信息；2. 提取文本虽然改善感知但无意中促进了信息泄露；3. DVA框架显著优于标准提示防御方法，为政策合规文档理解提供了鲁棒基线。

Conclusion: 多模态文档的政策遵从性面临独特挑战，需要专门的安全评估基准和结构化推理框架。DVA通过解耦推理与政策验证，有效减少了信息泄露，为安全的多模态文档理解提供了实用解决方案。
...
TL;DR: 该研究提出了一种基于归因的框架来分析短上下文语言建模中的位置效应，发现语言模型存在架构特定的位置偏好模式，包括随深度增加的近因偏差和随深度减弱的首因偏差。


<details>
  <summary>Details</summary>
Motivation: 语言模型在处理输入时往往表现出对特定位置信息的偏好，而不考虑语义相关性。虽然位置偏差已在多种情境中被研究，但先前工作未能揭示这些偏差如何在不同层和输入位置间演化，或如何独立于任务复杂性而变化。

Method: 引入基于归因的分析框架，使用层传导度和滑动窗口方法，量化每一层在输入位置间分配重要性的方式，生成层级位置重要性剖面图。

Result: 研究发现这些剖面图具有架构特异性、跨输入稳定性以及对词汇重排的不变性。具体特征包括：随着深度增加而增强的显著近因偏差，以及随模型深度减弱的微妙首因偏差。此外，早期层在所有位置上都优先加权内容词而非功能词，而后期层则失去了这种词类型区分。

Conclusion: 该研究揭示了语言模型中存在稳定且架构特定的位置处理模式，这些模式独立于语义内容，为理解语言模型内部工作机制提供了新的视角。
...
TL;DR: RADAR是一个检索增强的对抗性检测器，通过生成器改写真实文章并引入事实扰动，结合轻量级检测器使用密集段落检索验证声明，采用语言对抗反馈机制提升检测鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 为了有效对抗LLM生成的虚假信息传播，需要开发更强大的虚假新闻检测方法。现有方法在面对不断进化的虚假信息生成技术时存在局限性，需要更鲁棒的检测机制。

Method: RADAR采用生成器-检测器对抗框架：生成器通过事实扰动改写真实文章，检测器使用密集段落检索验证声明。引入语言对抗反馈机制，用结构化自然语言批评指导生成器进化，迫使检测器适应改进。

Result: 在虚假新闻检测基准测试中，RADAR达到86.98%的ROC-AUC，显著优于带检索的通用LLM。消融研究表明检测器端检索带来最大增益，语言对抗反馈和少样本演示为鲁棒训练提供关键信号。

Conclusion: RADAR通过检索增强和对抗性精炼有效检测LLM生成的虚假信息，语言对抗反馈机制使生成器和检测器能够协同进化，为虚假新闻检测提供了鲁棒且高效的解决方案。
...
TL;DR: 提出Benchmark²框架，包含三个评估基准质量的指标：跨基准排名一致性、区分度得分和能力对齐偏差，用于系统评估LLM基准的质量。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型评估基准的快速增加，迫切需要系统方法来评估基准本身的质量，以确保评估结果的可靠性和有效性。

Method: 提出Benchmark²框架，包含三个互补指标：1)跨基准排名一致性，衡量基准产生的模型排名是否与其他基准一致；2)区分度得分，量化基准区分不同模型的能力；3)能力对齐偏差，识别同一模型家族中强模型失败而弱模型成功的异常实例。

Result: 在涵盖数学、推理和知识领域的15个基准上对11个LLM进行实验，发现现有基准质量存在显著差异，基于所提指标的筛选基准构建方法可以在大幅减少测试集的情况下获得相当的评估性能。

Conclusion: Benchmark²框架为系统评估LLM基准质量提供了有效工具，有助于识别高质量基准并优化基准构建过程，提高评估效率和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 当外部检索证据与LLM内部知识冲突时，模型倾向于依赖记忆事实而非上下文，导致输出不忠实。需要一种高效方法在知识冲突场景中提高上下文忠实度

Method: 提出ContextFocus，一种轻量级激活引导方法，通过调整模型激活来优先考虑上下文证据，无需模型微调，推理开销最小

Result: 在ConFiQA基准测试中显著优于ContextDPO、COIECD和提示方法，与提示策略互补，在大模型上依然有效，显著提高上下文忠实度

Conclusion: ContextFocus在提高LLM输出上下文忠实度方面具有高效性、鲁棒性和有效性，为知识冲突场景提供了实用解决方案
...
TL;DR: 本文提出了VotIE任务，用于从非结构化的市政会议纪要中提取结构化投票信息，并在葡萄牙市政会议纪要上建立了首个基准。实验发现：在域内评估中，微调编码器（XLM-R-CRF）表现最佳（93.2% F1），优于生成式方法；但在跨市政迁移场景中，微调模型性能显著下降，而少样本LLM表现出更好的泛化能力，不过其高计算成本限制了实用性。


<details>
  <summary>Details</summary>
Motivation: 市政会议纪要记录了地方民主进程中的关键决策，但与标准化的议会记录不同，它们以高度异质、自由形式的叙述文本编码投票结果，在不同市政机构间差异很大，这给自动化提取带来了重大挑战。需要开发专门的信息提取任务来处理这种非结构化数据。

Method: 提出了VotIE（投票信息提取）任务，旨在从叙述性审议记录中识别结构化投票事件。基于CitiLink语料库建立了葡萄牙市政会议纪要的首个基准。实验比较了两种方法：1）微调编码器（特别是XLM-R-CRF模型），2）生成式大语言模型的少样本学习。

Result: 1）在标准域内评估中，微调编码器（XLM-R-CRF）达到93.2%的宏F1分数，优于生成式方法；2）在跨市政评估（迁移到未见过的行政环境）中，微调模型性能大幅下降，而少样本LLM表现出更强的鲁棒性，性能下降幅度显著较小。

Conclusion: 尽管生成式模型在泛化能力上有优势，但其高计算成本限制了实际应用。因此，轻量级的微调编码器在大规模实际部署中仍是更实用的选择。为支持行政NLP的可重复研究，公开发布了基准、训练模型和评估框架。
...
TL;DR: 本文研究了LLM驱动的学生模拟质量评估问题，提出了综合评估指标并测试了多种方法，发现现有方法效果有限。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在教育领域的应用增多，许多研究使用模拟学生进行训练和评估，但缺乏对学生模拟质量的系统评估和测量方法。

Method: 正式定义了学生模拟任务，提出了一套涵盖语言、行为和认知三个维度的评估指标，并在真实数学辅导对话数据集上对多种学生模拟方法进行了基准测试。

Result: 实验结果显示，基于提示策略的学生模拟方法表现较差；监督微调和偏好优化方法表现更好但仍有限制，自动评估和人工评估结果一致。

Conclusion: 学生模拟是一个具有挑战性的任务，现有方法效果有限，需要未来进一步研究改进。
...
TL;DR: SpeakerSleuth基准测试发现大型音频语言模型在评估多轮对话中的说话人一致性时存在显著缺陷，模型倾向于优先考虑文本而非声学特征，暴露出模态不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 虽然大型音频语言模型已成为评估语音生成质量的主流方法，但其在多轮对话中评估说话人一致性的能力尚未被探索。本研究旨在填补这一空白，评估LALMs是否能可靠地判断多轮对话中的说话人一致性。

Method: 提出了SpeakerSleuth基准测试，包含三个反映实际需求的任务。构建了1,818个人工验证的评估实例，涵盖四个不同的数据集（合成和真实语音），并控制声学难度。评估了九个广泛使用的大型音频语言模型。

Result: 模型在可靠检测声学不一致性方面表现不佳：有些模型过度预测不一致性，而有些则过于宽松；模型难以准确识别有问题的具体轮次；当提供其他对话者的轮次时，性能急剧下降，模型优先考虑文本连贯性而非声学线索，甚至无法检测明显的性别切换。然而，模型在选择与说话人最匹配的音频变体方面表现更好，显示出固有的声学辨别能力。
...
TL;DR: 该论文研究多语言机器翻译中的跨语言知识迁移，分析语言相似性对迁移的影响，开发提升低资源语言翻译鲁棒性的方法，并探讨训练策略对多语言学习的影响。


<details>
  <summary>Details</summary>
Motivation: 多语言机器翻译系统旨在实现跨语言知识共享，但低资源语言由于平行数据有限，其跨语言表示学习和知识迁移面临挑战。需要深入理解多语言模型如何在语言间共享知识，以及数据可用性和训练策略如何影响这一过程。

Method: 以机器翻译为主要测试平台，分析语言相似性对知识迁移的影响；使用检索和辅助监督来增强低资源翻译；研究在平行数据上微调大型语言模型可能带来的意外权衡；考察训练中语言多样性的作用。

Result: 研究表明：语言相似性显著影响跨语言知识迁移；检索和辅助监督能有效提升低资源翻译性能；增加翻译覆盖范围能改善泛化能力并减少离目标行为；在平行数据上微调大型语言模型可能引入意外权衡。

Conclusion: 建模选择和数据组成对多语言学习具有重要影响。通过理解语言相似性、优化训练策略和增加语言多样性，可以构建更具包容性和鲁棒性的多语言NLP系统，特别是对低资源语言的支持。
...
TL;DR: SaLAD是一个多模态安全基准，包含2013个真实世界图像-文本样本，覆盖10个常见类别，用于评估MLLMs在日常生活中的安全影响。研究发现当前顶级模型在危险查询上的安全响应率仅为57.2%，现有安全对齐方法效果有限。


<details>
  <summary>Details</summary>
Motivation: 随着MLLMs成为人类生活中不可或缺的助手，其生成的不安全内容对人类行为构成威胁。需要研究和评估MLLMs响应在日常生活中对人类行为的安全影响，现有方法存在局限性。

Method: 提出SaLAD多模态安全基准，包含2013个真实世界图像-文本样本，覆盖10个常见类别，平衡设计包含不安全场景和过度敏感案例。强调真实风险暴露、真实视觉输入和细粒度跨模态推理。提出基于安全警告的评估框架，鼓励模型提供清晰信息性警告而非通用拒绝。

Result: 在18个MLLMs上的测试显示，表现最佳的模型在不安全查询上的安全响应率仅为57.2%。即使流行的安全对齐方法在当前场景下效果也有限，揭示了当前MLLMs在识别日常危险行为方面的脆弱性。

Conclusion: 当前MLLMs在识别日常生活中的危险行为方面存在显著安全漏洞，需要更有效的安全对齐方法。SaLAD基准为评估和改进MLLMs安全性能提供了重要工具。
...
TL;DR: MPO是一种基于模式的提示优化框架，将提示视为由固定语义部分组成的结构化对象，通过局部文本梯度独立优化各部分，同时保持整体模式不变，从而提升小型开源语言模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法通常将提示视为整体文本块，难以定位错误、保留关键指令或防止提示无限增长。需要一种能够保持提示结构、实现局部优化且可解释的方法。

Method: 提出模块化提示优化(MPO)框架：1) 将提示定义为包含系统角色、上下文、任务描述、约束和输出格式的结构化对象；2) 使用批评语言模型生成局部文本梯度，独立优化每个部分；3) 通过去重整合各部分更新，减少冗余和组件间干扰。
...
Conclusion: 保持固定的提示模式同时应用局部、分节优化是提高小型开源语言模型推理性能的有效实用方法，提供了可解释且鲁棒的优化过程。
...
TL;DR: CoM-DAD提出了一种统一的概率框架，通过分层双过程解决多模态生成问题，将高级语义规划与低级token合成解耦，实现文本-图像的统一生成。


<details>
  <summary>Details</summary>
Motivation: 当前生成建模在离散数据（文本）和连续数据（图像）上分别采用自回归和扩散方法，阻碍了真正统一的多模态系统发展。掩码语言模型虽然提供高效双向上下文，但缺乏自回归模型的生成保真度和扩散模型的语义连续性，且扩展到多模态设置存在严重的对齐挑战和训练不稳定问题。

Method: CoM-DAD采用分层双过程框架：1) 通过连续潜在扩散过程建模语义流形；2) 将token生成视为离散吸收扩散过程，由"可变速率噪声调度"调节，并以演化语义先验为条件。引入"随机混合模态传输"策略，无需繁重的对比双编码器即可对齐不同模态。

Result: 该方法在标准掩码建模上表现出优越的稳定性，为可扩展的统一文本-图像生成建立了新范式。

Conclusion: CoM-DAD通过耦合流形离散吸收扩散框架，成功解决了多模态生成中的对齐和稳定性问题，为统一文本-图像生成系统提供了新的解决方案。
...
### [144] [KDCM: Reducing Hallucination in LLMs through Explicit Reasoning Structures](https://arxiv.org/abs/2601.04086)
*Jinbo Hao,Kai Yang,Qingzhen Su,Yifan Li,Chao Jiang*

Main category: cs.CL

TL;DR: 提出一个通过代码引导知识图谱探索来减少大语言模型幻觉的框架，在多个基准测试中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型中由提示引起的幻觉问题，特别是错误推理导致的不可靠预测

Method: 扩展链式知识蒸馏方法，加入可编程模块指导知识图谱探索，将可执行代码嵌入推理提示中，利用外部结构化知识，开发增强的蒸馏推理框架来显式调控中间推理步骤
...
### [145] [LLMberjack: Guided Trimming of Debate Trees for Multi-Party Conversation Creation](https://arxiv.org/abs/2601.04135)
*Leonardo Bottona,Nicolò Penzo,Bruno Lepri,Marco Guerini,Sara Tonelli*

Main category: cs.CL

TL;DR: LLMberjack是一个从现有辩论（回复树结构）创建多方对话的平台，提供可视化讨论树界面，支持构建连贯的线性化对话序列，并集成LLM辅助自动编辑消息和说话者描述。
...
Method: 开发交互式平台，可视化讨论树结构，允许用户构建线性化对话序列，同时保留参与者身份和话语关系，并集成LLM辅助功能支持自动编辑。

Result: 树形可视化有助于创建连贯有意义的对话线程，LLM支持提高了输出质量并减少了人工工作量，平台已开源。

Conclusion: LLMberjack是一个有效的工具，能够从现有辩论创建高质量的多方对话，填补了该类型资源的空白。
...
### [146] [All That Glisters Is Not Gold: A Benchmark for Reference-Free Counterfactual Financial Misinformation Detection](https://arxiv.org/abs/2601.04160)
...
<details>
  <summary>Details</summary>
Motivation: 金融新闻中的虚假信息检测面临上下文复杂性挑战，因为金融新闻的含义往往来自分散的线索。现有模型在无参考情况下检测金融虚假信息的能力存在不足，需要建立更可靠的评估基准。

Method: RFC Bench在段落级别操作，捕捉金融新闻的上下文复杂性。定义了两个互补任务：1）无参考虚假信息检测；2）基于比较的诊断，使用配对的原始-扰动输入进行分析。

Result: 实验显示一致模式：当有比较上下文可用时，模型性能显著更强；而无参考设置暴露了显著弱点，包括不稳定的预测和较高的无效输出。这表明当前模型在没有外部基础的情况下难以维持一致的信念状态。

Conclusion: RFC Bench通过揭示这一差距，为研究无参考推理和推进更可靠的金融虚假信息检测提供了结构化测试平台，有助于在真实世界环境中改进金融虚假信息检测技术。
...
### [147] [Class of topological portfolios: Are they better than classical portfolios?](https://arxiv.org/abs/2601.03974)
*Anubha Goel,Amita Sharma,Juho Kanniainen*

Main category: q-fin.PM

TL;DR: 该论文提出了一种基于拓扑数据分析(TDA)的投资组合风险管理新方法，通过持久性景观量化"拓扑风险"，并构建最小化该风险的最优投资组合，在实证中表现出优于传统方法的性能。


<details>
  <summary>Details</summary>
Motivation: 传统投资组合选择方法存在局限性，需要更全面的风险管理工具。拓扑数据分析(TDA)作为新兴领域，能够基于数据形状特征提取信息，为投资科学提供有前景的替代方案。

Method: 使用持久性景观(持久同调的一种汇总统计量)捕捉收益的拓扑变化，提出"拓扑风险"概念，通过持久性景观的Lp范数量化投资组合风险，并通过优化推导最小化拓扑风险的最优投资组合。

Result: 基于近十年标普500数据的数值实验表明，TDA投资组合在超额平均收益和多个财务比率方面优于7种流行的投资组合优化模型、1/N朴素投资组合和标普500市场指数。结果在不同持有规模和投资时间范围内保持一致。

Conclusion: TDA基础的拓扑风险度量比传统统计度量能更全面地理解投资组合动态，对现代投资组合管理实践具有重要相关性，展示了拓扑数据分析在金融领域的应用潜力。
...
### [148] [Ensemble Models for Predicting Treatment Response in Pediatric Low-Grade Glioma Managed with Chemotherapy](https://arxiv.org/abs/2601.03899)
*Max Bengtsson,Elif Keles,Angela J. Waanders,Ulas Bagci*

Main category: eess.IV

TL;DR: 提出一种结合MRI影像组学和临床数据的集成学习框架，用于预测无法完全手术切除的儿童脑肿瘤患者的化疗反应，在化疗无效病例上获得较高召回率。


<details>
  <summary>Details</summary>
Motivation: 针对无法完全手术切除的儿童脑肿瘤患者（特别是低级别胶质瘤），这类患者化疗是主要治疗选择但预后较差，需要非侵入性的化疗反应预测方法来指导个性化治疗。

Method: 集成Swin UNETR编码器和XGBoost分类器：1）使用分割模型划分肿瘤四个亚区；2）Swin UNETR直接从分割MRI预测反应；3）XGBoost结合影像组学特征和临床变量预测；4）集成两种方法的输出。

Result: Swin-Ensemble模型表现最佳：化疗无效病例精度0.68、召回0.85，化疗有效病例精度0.64，总体准确率0.69，优于Mamba-FeatureFuse、Swin UNETR编码器和Swin-FeatureFuse等对比模型。

Conclusion: 该集成框架为无法完全手术切除的儿童低级别胶质瘤患者提供了有前景的个性化化疗反应预测方法，这类患者化疗是主要治疗选择且无进展生存期较低。
...
TL;DR: 当前病理学基础模型对扫描仪引起的技术领域偏移缺乏鲁棒性，尽管基准性能良好，但在真实临床应用中存在扫描仪依赖的偏差风险。


<details>
  <summary>Details</summary>
Motivation: 尽管病理学基础模型在基准测试中表现良好，但其对真实世界技术领域偏移（如不同扫描仪设备引起的变异性）的鲁棒性尚不清楚，这影响了模型在临床环境中的可靠性。

Method: 系统评估了14个病理学基础模型对扫描仪引起的变异的鲁棒性，包括最先进的模型、早期自监督模型和自然图像训练的基线模型。使用5台设备扫描的384张乳腺癌全切片图像数据集，通过无监督嵌入分析和一组临床病理学监督预测任务来评估鲁棒性。

Result: 当前PFMs对扫描仪引起的领域偏移不具有不变性，大多数模型在其嵌入空间中编码了明显的扫描仪特异性变异性。虽然AUC通常保持稳定，但这掩盖了关键失败模式：扫描仪变异性系统性地改变了嵌入空间，影响了下游模型预测的校准，导致扫描仪依赖的偏差。鲁棒性不是训练数据规模、模型大小或模型新近度的简单函数。

Conclusion: PFMs的开发和评估需要超越以准确性为中心的基准测试，转向在真实采集变异性下对嵌入稳定性和校准进行明确评估和优化。虽然训练数据最多样化的模型（如视觉语言模型）在鲁棒性方面具有优势，但在下游监督任务中表现不佳。
...
### [150] [Edit2Restore:Few-Shot Image Restoration via Parameter-Efficient Adaptation of Pre-trained Editing Models](https://arxiv.org/abs/2601.03391)
*M. Akın Yılmaz,Ahmet Bilican,Burak Can Biner,A. Murat Tekalp*

Main category: eess.IV

TL;DR: 本文提出了一种基于预训练文本条件图像编辑模型的少样本图像修复方法，通过LoRA适配器微调，仅需16-128个配对图像即可处理去噪、去雨、去雾等多种修复任务。


<details>
  <summary>Details</summary>
Motivation: 传统图像修复方法需要为每种退化类型训练专门的模型，且需要数千个配对样本，数据需求大且不灵活。本文旨在探索利用预训练图像编辑模型的视觉先验知识，通过少量样本实现多种修复任务。

Method: 使用FLUX.1 Kontext（120亿参数的流匹配模型）作为基础模型，通过参数高效的LoRA适配器进行微调，仅需16-128个配对图像。通过文本提示指定修复操作，单个统一的LoRA适配器可处理多种退化类型。

Result: 方法在感知质量方面表现优异，虽然PSNR/SSIM等像素级重建指标不是最优，但显著降低了数据需求。通过消融研究分析了训练集大小、任务特定vs统一适配器、文本编码器微调等因素的影响。

Conclusion: 预训练图像编辑模型经过适当适配后，为传统图像修复方法提供了数据高效且灵活的替代方案，开辟了少样本、提示引导图像增强的新途径。
...
### [151] [Transforming Video Subjective Testing with Training, Engagement, and Real-Time Feedback](https://arxiv.org/abs/2601.04184)
*Kumar Rahul,Sriram Sethuraman,Andrew Segall,Yixu Chen*

Main category: cs.MM

TL;DR: 提出集成框架提升主观视频质量评估：通过自动化训练测验、实时注意力评分和高效成对比较，提高评分者质量并减少比较次数


<details>
  <summary>Details</summary>
Motivation: 传统主观视频质量评估方法存在局限性，难以捕捉细微感知差异并确保可靠的用户输入，需要改进评分者训练和注意力监控机制

Method: 三阶段框架：1) 自动化训练测验教导视频质量指标；2) 实时注意力评分机制使用"黄金"视频对监控评分者专注度；3) 基于链式的高效成对比较程序生成JOD质量分数

Result: 实验显示训练测验显著提高数据质量（黄金单元准确率提升，平局率降低），实时反馈进一步改善数据质量并产生最单调的质量评分，减少R-Q曲线高质量部分的非单调情况

Conclusion: 提出的训练-测验-反馈三阶段方法能显著提升主观视频质量评估的可靠性，有助于训练更好的客观视频质量指标
...
TL;DR: 提出一种多智能体工作流，将主要任务描述优化与约束条件解耦，通过定量分数反馈迭代改进提示词，显著提升LLM输出对形式约束的遵循度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常生成内容相关但不符合形式约束的输出，传统提示词优化方法只关注主要任务描述的改写，忽略了作为响应接受标准的细粒度约束条件。

Method: 采用多智能体工作流，将主要任务描述优化与约束条件解耦，使用定量分数作为反馈，迭代重写和改进提示词。

Result: 该方法生成的修订提示词在Llama 3.1 8B和Mixtral-8x 7B等模型上获得了显著更高的合规性分数。

Conclusion: 通过解耦任务描述和约束条件优化，并使用定量反馈迭代改进，可以有效提升LLM输出对形式约束的遵循度。
...
TL;DR: 论文研究AI智能体如何通过内省机制建立自我意识，利用类似生物疼痛的学习信号来推断内部状态，从而提升强化学习性能。


<details>
  <summary>Details</summary>
Motivation: 理解人工智能体如何建模内部心理状态对于推进AI中的心智理论至关重要。证据表明自我意识与他人意识存在统一系统，本研究通过让强化学习智能体推断自身内部状态来探索这种自我意识。

Method: 在网格世界环境中，引入受生物疼痛启发的内省探索组件，使用隐马尔可夫模型从在线观察中推断"疼痛信念"，并将该信号整合到主观奖励函数中，研究自我意识如何影响智能体的学习能力。同时使用该计算框架研究正常与慢性疼痛感知模型之间的性能差异。

Result: 结果显示，具有内省能力的智能体总体上显著优于标准基线智能体，并且能够复制复杂的人类行为模式。

Conclusion: 通过模拟生物疼痛机制的内省探索能够有效增强AI智能体的自我意识，提升其学习性能，并为理解人类心智理论提供计算框架。
...
TL;DR: 提出基于成熟度的框架，通过明确的测量机制来认证具身AI系统
...
Method: 提出基于成熟度的认证框架，包含结构化评估框架、量化评分机制和多目标权衡导航方法；以不确定性量化作为示例测量机制，并通过无人机系统检测案例研究展示可行性

Result: 通过无人机系统检测案例研究证明了该方法的可行性
...
TL;DR: CPGPrompt：一种自动提示系统，将叙述性临床指南转换为LLM可用的结构化决策树，用于患者病例评估，在专科转诊决策上表现优异，但在多类路径分配上表现有差异。
...
Method: 开发CPGPrompt自动提示系统，将临床指南转换为结构化决策树，利用LLM动态导航决策树进行患者病例评估。在三个领域（头痛、腰痛、前列腺癌）生成合成病例，测试不同决策场景。

Result: 二元专科转诊分类在所有领域表现一致强劲（F1：0.85-1.00），召回率高（1.00±0.00）。多类路径分配表现降低，领域间有差异：头痛（F1：0.47）、腰痛（F1：0.72）、前列腺癌（F1：0.77）。性能差异反映了各指南的结构特点。

Conclusion: CPGPrompt系统在将临床指南整合到AI方面取得进展，在专科转诊决策上表现优异，但在复杂路径分配上仍有改进空间，性能差异与指南结构特点相关。
...
<details>
  <summary>Details</summary>
Motivation: 大型基础模型在医疗AI中展现出潜力，但能否提供真正的个性化治疗建议仍存疑问。研究发现个性化医疗面临多重挑战，包括普遍性悖论、隐私-性能悖论、规模-特异性悖论和自动化-同理心悖论。需要明确预测与因果推断的界限，解决这些矛盾以实现负责任的AI医疗整合。

Method: 提出混合框架：大型基础模型利用多模态数据从群体模式中快速生成假设，提供干预候选排名和不确定性估计；这些结果触发后续的N-of-1试验（交叉自我实验），在个体层面进行因果验证。N-of-1试验作为个性化医学中个体因果推断的金标准，通过本地实验保护隐私。

Result: 大型基础模型无法替代N-of-1试验，但两者具有互补性。基础模型擅长从群体数据中发现模式并生成假设，而N-of-1试验擅长为特定个体提供因果证据。结合两者的混合框架能够解决个性化医疗中的矛盾，实现真正的个性化治疗建议。

Conclusion: 大型基础模型和N-of-1试验是互补的，而非替代关系。明确预测与因果推断的界限，并解决个性化医疗中的各种矛盾，对于负责任的AI医疗整合至关重要。提出的混合框架结合了两者的优势，能够实现真正的个性化治疗建议。
...
TL;DR: STAR-S框架通过自学习循环将安全规则推理学习集成到LLMs中，有效防御越狱攻击


<details>
  <summary>Details</summary>
Motivation: 现有方法试图通过训练模型在响应前对安全规则进行推理来提高安全性，但难以明确设计或直接获得有效的安全推理形式来防御越狱攻击

Method: 提出STAR-S框架，通过自学习循环集成安全规则推理学习：在安全规则指导下引出推理和反思，然后利用微调增强安全推理，重复此过程形成协同循环

Result: 实验表明STAR-S能有效防御越狱攻击，性能优于基线方法

Conclusion: STAR-S框架通过自学习循环增强LLMs的安全规则推理能力，为防御越狱攻击提供了有效解决方案
...
TL;DR: 论文提出神经符号框架评估LLM推理过程，发现扩展token生成并非深度推理必要条件，混合长短CoT训练会导致过早饱和，蒸馏到小模型只能复制行为长度但无法达到逻辑效能。


<details>
  <summary>Details</summary>
Motivation: 当前CoT评估方法存在局限，无法区分性能提升是来自真实推理还是单纯增加输出长度，需要更全面的过程中心化评估框架来深入理解LLM的推理能力。

Method: 提出非侵入性神经符号框架进行过程中心化推理评估，识别四种行为原型并诊断失败模式，分析推理模式、训练策略和模型规模的影响。

Result: 发现扩展token生成不是深度推理的必要条件；混合长短CoT数据训练会导致过早饱和和崩溃；蒸馏到小模型能复制行为长度但无法达到逻辑效能，受限于内在能力限制。

Conclusion: 需要更精细的评估方法来区分真实推理与表面行为，训练策略需要谨慎设计以避免过早饱和，模型规模对逻辑推理能力有本质影响，蒸馏技术存在局限性。
...
TL;DR: SAE-Steering：通过稀疏自编码器解耦推理策略特征，实现对大语言模型推理策略的有效控制，提升推理准确率


<details>
  <summary>Details</summary>
Motivation: 大型推理模型能自主选择推理策略，但这种自主选择往往产生低效甚至错误的推理路径。现有方法难以控制细粒度推理策略，因为策略特征在隐藏状态中纠缠在一起。需要开发能可靠、灵活控制推理策略的方法。

Method: 提出SAE-Steering方法：1) 使用稀疏自编码器将策略纠缠的隐藏状态解耦到特征空间；2) 设计两阶段特征识别流程：首先通过策略关键词的logits放大召回特征（过滤99%以上特征），然后按控制效果对剩余特征排序；3) 使用识别出的策略特定特征作为控制向量。

Result: SAE-Steering在控制效果上比现有方法提升超过15%。通过控制推理策略，能将模型从错误路径重定向到正确路径，实现7%的绝对准确率提升。

Conclusion: SAE-Steering方法能有效解耦和控制大型推理模型的推理策略，显著提升推理的可靠性和准确性，为解决模型自主推理策略选择问题提供了有效方案。
...
TL;DR: SafeRemind：一种在解码时向大型推理模型思考步骤动态注入安全提醒短语的防御方法，显著提升模型安全性而不影响推理能力


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过显式思考步骤取得显著成功，但这些思考步骤可能放大不安全行为，而传统防御机制忽视了LRM独特的推理动态。研究发现思考步骤中安全提醒短语的出现对确保LRM安全起着关键作用。
...
Conclusion: SafeRemind通过动态注入安全提醒短语有效解决了大型推理模型思考步骤中的安全隐患，提供了一种无需参数更新的高效防御机制。
...
TL;DR: SandwichR提出了一种新的推理范式，通过"答案-推理-答案"三明治结构，在保持CoT推理精度的同时大幅降低延迟，解决了查询修正任务中实时性与准确性的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 查询修正是现代搜索管道的关键入口点，需要在实时延迟约束下保持高精度。传统思维链（CoT）推理虽然能提高准确性，但会产生过高的延迟，不适合实时查询修正。现有解决方案（先输出答案后推理）在自回归解码下，早期答案与后续推理无关，无法利用推理能力提高准确性。

Method: 提出Sandwich Reasoning（SandwichR）方法，采用"答案-推理-答案"范式：首先生成初始修正，然后进行显式推理过程，最后生成最终精炼修正。通过一致性感知的强化学习策略：专门的一致性奖励强制初始修正与最终修正对齐，同时基于边界的拒绝采样优先处理推理能带来最大修正增益的边界样本。此外，构建了高质量的查询修正数据集。

Result: 实验结果表明，SandwichR在达到与标准CoT相当的最先进准确性的同时，实现了40-70%的延迟降低，成功解决了在线搜索中的延迟-准确性权衡问题。

Conclusion: SandwichR通过创新的三明治推理范式，在查询修正任务中实现了低延迟与高精度的平衡，为实时搜索系统提供了有效的解决方案，同时构建的数据集也为复杂查询修正研究提供了专门基准。
...
TL;DR: 本文提出使用LLM生成领域特定启发式函数来扩展个性化药物规划规模，从最多7种药物扩展到至少28种药物，显著提升覆盖率和规划效率。


<details>
  <summary>Details</summary>
Motivation: 个性化药物规划需要为每位患者选择药物并确定给药方案以实现特定医疗目标。先前研究虽然证明了自动化规划器能够生成个性化治疗方案，但实际应用中最多只能处理7种药物，这在临床实践中远远不够，需要扩展药物规划规模以更好地与临床医生合作。

Method: 采用程序化方式定义领域（指定初始状态和后继生成过程），使用大型语言模型（LLM）生成针对特定问题的启发式函数，然后将这些启发式函数与固定搜索算法（GBFS，贪心最佳优先搜索）结合使用。

Result: 该方法在覆盖率和规划时间方面取得了显著改进，能够将药物规划规模扩展到至少28种药物，使药物规划更接近实际临床应用。

Conclusion: 通过使用LLM自动生成领域特定启发式函数，成功扩展了个性化药物规划的规模，使其从最多处理7种药物扩展到至少28种药物，为药物规划的实际临床应用迈出了重要一步。
...
TL;DR: EntroCoT框架通过熵基分割和蒙特卡洛评估自动识别和优化CoT推理轨迹中的低质量监督数据，构建高质量数据集提升数学推理能力


<details>
  <summary>Details</summary>
Motivation: 现有微调数据集存在"答案正确但推理错误"的问题，即最终答案正确但中间步骤存在幻觉、冗余或逻辑无效，这影响了CoT推理的质量

Method: 提出EntroCoT统一框架：1）基于熵的机制在不确定节点分割推理轨迹；2）蒙特卡洛展开机制评估每个步骤的边际贡献；3）准确过滤欺骗性推理样本构建高质量数据集

Result: 在数学基准测试上的广泛实验表明，使用EntroCoT构建的子集进行微调始终优于全数据集监督的基线方法

Conclusion: EntroCoT能有效识别和优化低质量CoT监督轨迹，构建高质量数据集提升大语言模型的数学推理能力
...
TL;DR: 论文提出ROI-Reasoning框架，通过元认知微调和理性感知强化学习，让大语言模型在严格计算预算下进行推理决策，优化多任务计算资源分配。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然具备强大推理能力，但无法自动判断不同任务所需的计算量，在严格计算预算下无法有效分配计算资源。现有方法缺乏对任务难度、投资回报率(ROI)的预测能力，导致计算资源分配效率低下。

Method: 提出两阶段ROI-Reasoning框架：1) 元认知微调阶段：训练模型预测推理成本和预期效用，做出显式的解决或跳过决策；2) 理性感知强化学习阶段：在硬性token预算下优化序列决策，学习长期分配策略。将问题形式化为有序随机多选择背包问题(OS-MCKP)。

Result: 在预算约束的数学推理基准测试中，ROI-Reasoning在严格计算预算下持续提高总体得分，同时显著减少遗憾值。模型学会了在不同任务间战略性地分配计算资源。

Conclusion: ROI-Reasoning框架赋予大语言模型内在的预算感知理性能力，通过预测任务难度和投资回报率，在严格计算约束下实现更优的计算资源分配策略，为预算约束下的推理任务提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 可废止蕴涵关注从不完整信息中得出合理结论，KLM框架是其基础模型。理性闭包是KLM框架中最突出的算法之一，但现有实现多为命令式方法。本文旨在提供一种声明式的ASP方法来计算理性闭包，以提高计算效率和实现自动化。

Method: 使用答案集编程（ASP）为理性闭包提供声明式定义。该方法能够从给定知识库自动构建最小排序模型，并支持对指定查询进行蕴涵检查。作者形式化证明了ASP编码的正确性。

Result: 通过实证评估将ASP实现与现有的命令式实现（特别是InfOCF求解器）进行性能比较。结果表明，基于ASP的方法遵循理性闭包的理论基础，并提供了改进的计算效率。

Conclusion: 本文成功开发了一种使用ASP计算理性闭包的声明式方法，该方法不仅保持了理论正确性，而且在计算效率上优于现有的命令式实现，为可废止推理提供了更高效的自动化工具。
...
TL;DR: 使用ASP对意大利刑法典建模，通过司法判例半自动学习法律规则，支持刑事审判推理和结果预测
...
Method: 将意大利刑法典条款编码为ASP程序，包括"人身犯罪"和财产犯罪；使用归纳逻辑编程从案例中学习法律规则；处理编码过程中的矛盾；通过稳定模型的"支持性"提供解释

Result: 开发出能够对先前判决进行验证的工具，可为新案件生成可能的判决决策并提供解释，使决策过程更加可解释

Conclusion: ASP方法能有效建模刑法典并学习法律规则，工具提供自动解释功能有助于澄清司法决策逻辑，提高司法透明度
...
TL;DR: 本文提出了一种基于答案集编程（ASP）的方法，用于为决策树模型生成多种解释类型，相比SAT方法更灵活且支持枚举所有可能解释。


<details>
  <summary>Details</summary>
Motivation: 决策树模型（包括随机森林和梯度提升决策树）在机器学习中广泛应用，但其复杂结构难以解释，特别是在需要正式决策依据的安全关键应用中。现有工作已证明可以通过自动推理技术推导逻辑和溯因解释。

Method: 提出一种基于答案集编程（ASP）的方法，用于生成多种解释类型：充分解释、对比解释、多数解释和树特定解释。相比基于SAT的方法，ASP方法在编码用户偏好方面更灵活，并支持枚举所有可能的解释。
...
<details>
  <summary>Details</summary>
Motivation: 虽然ASP在许多应用领域实现了"用户指定问题，计算机解决问题"的AI愿景，但当前ASP求解技术能否扩展到大型配置问题仍存疑问。特别是电子系统配置这类可能包含超过30,000个组件的大规模问题，需要评估ASP技术的可扩展性。

Method: 研究聚焦于解决"接地瓶颈"的方法，即问题实例规模增大时内存需求急剧增加的问题。首先调查了增量求解方法，然后基于接地分析开发了"约束感知猜测"方法，显著减少了内存需求。

Result: 增量求解方法在实践中被证明是有效的，但即使采用增量方法，内存需求仍然构成显著限制。新开发的约束感知猜测方法显著降低了内存需求，推动了ASP技术在大规模配置问题中的应用边界。

Conclusion: 论文展示了当前ASP技术在大型配置问题中的潜力和限制，通过分析接地过程并开发约束感知猜测方法，有效解决了接地瓶颈问题，为ASP在大规模实际问题中的应用提供了重要进展。
...
<details>
  <summary>Details</summary>
Motivation: RL在数学和编码等可验证奖励领域表现优异，但扩展到金融决策时面临市场随机性的挑战：奖励可验证但本质上是噪声的，导致标准RL退化为奖励黑客

Method: 提出Trade-R1训练框架，通过过程级推理验证连接可验证奖励与随机环境。核心创新是将长金融文档的推理评估转化为结构化RAG任务，构建三角一致性度量（检索证据、推理链和决策的对齐），作为噪声市场回报的有效性过滤器。探索两种奖励整合策略：固定效应语义奖励（FSR）和动态效应语义奖励（DSR）
...
TL;DR: 论文提出DOT方法解决大模型推理中过度冗长问题，通过动态截断冗余token减少78%推理token使用，同时提升准确率


<details>
  <summary>Details</summary>
Motivation: 现有增强推理模型通过强化学习和可验证奖励取得了显著性能提升，但存在部署成本高的问题，模型在简单查询上过度冗长。现有基于显式长度惩罚的高效推理方法常引入优化冲突，且未深入探究过度思考的生成机制。

Method: 提出动态异常截断(DOT)训练时干预方法，选择性抑制冗余token，仅针对完全正确rollout组中的极端尾部响应长度。辅以辅助KL正则化和预测性动态采样确保稳定收敛。

Result: 实验结果表明该方法显著扩展了效率-性能帕累托前沿。在AIME-24上，相比初始策略减少78%推理token使用，同时提高准确率，超越了现有高效推理方法。

Conclusion: DOT方法有效解决了模型在训练过程中对简单输入生成不必要推理的"长度偏移"现象，在保持复杂问题长程推理能力的同时大幅提升推理效率。
...
Method: 提出了智能体漂移的理论框架，包括语义漂移、协调漂移和行为漂移三种表现形式。开发了智能体稳定性指数（ASI）作为复合度量框架，包含12个维度来量化漂移。通过仿真分析和理论建模进行研究。
