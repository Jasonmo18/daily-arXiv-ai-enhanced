...
Method: 1) 构建CMER-Bench基准测试，将表达式分为简单、中等、复杂三个难度等级；2) 创建大规模数据集MER-17M和CMER-3M，重点包含复杂数学表达式样本；3) 提出结构化数学语言表示方法，显式建模表达式的层次和空间结构；4) 基于编码器-解码器架构设计专门模型CMERNet，在CMER-3M数据集上训练。

Result: 实验结果表明，现有MER模型和多模态大语言模型在简单和中等难度表达式上表现良好，但在复杂表达式上性能显著下降。相比之下，CMERNet仅用1.25亿参数，就在CMER-Bench上显著优于现有MER模型和MLLMs。

Conclusion: 该研究通过构建专门基准测试、大规模数据集、新的表示方法和专门模型，有效解决了复杂数学表达式识别的挑战，为开发准确鲁棒的复杂MER模型提供了重要支持。
...
TL;DR: 本文探讨了AIGC在新闻图像生产中的可控性路径，通过两个实验提出了人机协作机制和评估指标，以解决新闻行业中的真实性、语义保真度和伦理问题。


<details>
  <summary>Details</summary>
Motivation: AIGC辅助图像生产在新闻行业引发争议，主要问题包括虚假信息、真实性、语义保真度和可解释性。大多数AIGC工具是"黑箱"，阻碍了内容准确性和语义对齐的双重要求，并引发了伦理、社会技术和信任困境。
...
Result: 实验1显示不同平台在语义对齐、文化特异性和视觉真实性方面存在差异，主要受训练语料偏见和平台级过滤影响；实验2构建的可控流程能够保持语义表征的可追溯性，确保编辑保真度。

Conclusion: 提出人机协作机制用于新闻特殊报道中的AIGC辅助图像生产，建议评估角色身份稳定性、文化表达准确性和用户-公众适宜性三个指标，为新闻行业提供可控的图像生产路径。
...
TL;DR: 该研究提出了一个结合深度学习图像分类与大型语言模型临床推理的框架，用于生成胃肠道疾病的临床解释，但发现当前LLMs在医学推理中仍不可靠。


<details>
  <summary>Details</summary>
Motivation: 医学图像分类器能检测胃肠道疾病但缺乏解释能力，大型语言模型能生成临床文本但视觉推理能力弱且解释不稳定，存在模型所见与临床医生期望的推理类型之间的差距。

Method: 提出一个连接图像分类与结构化临床推理的框架：1) 设计MobileCoAtNet混合模型用于内窥镜图像分类；2) 用分类输出驱动多个LLMs进行推理；3) 构建两个专家验证的基准评估LLMs推理质量。

Result: MobileCoAtNet在8个胃相关类别上达到高准确率；32个LLMs评估显示强分类能改善解释质量，但没有模型达到人类水平的稳定性；即使最好的LLMs也会随提示变化而改变推理。
...
TL;DR: 当前多模态大语言模型在医学决策任务上表现不佳，纯文本推理优于视觉或视觉文本结合的方法，需要改进视觉基础理解能力。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在一般视觉语言任务上表现出色，但在医学决策任务中表现不佳，特别是在视觉差异细微的医学分类任务上，需要研究其局限性并探索改进方法。

Method: 使用两个具有挑战性的医学数据集：三阶段阿尔茨海默病分类和MIMIC-CXR胸部X光分类，比较纯文本、纯视觉和多模态输入的推理性能，并探索三种改进策略：带推理注释的上下文学习、视觉描述后纯文本推理、以及视觉塔的少样本微调。

Result: 纯文本推理在医学决策任务中始终优于纯视觉或多模态设置，多模态输入甚至比纯文本表现更差。三种改进策略中，视觉描述后纯文本推理和视觉塔微调显示出一定潜力。

Conclusion: 当前多模态大语言模型缺乏基础的视觉理解能力，在医学决策任务中存在显著局限性。需要改进视觉基础理解，探索视觉描述和视觉塔微调等方向来提升医疗领域的多模态决策能力。
...
TL;DR: STAR提出了一种堆叠自回归方案，通过分阶段理解和生成来统一多模态学习，在保持理解能力的同时提升生成性能。
...
Conclusion: STAR通过任务渐进式堆叠自回归方案，有效解决了多模态理解和生成的统一问题，在保持现有理解能力的同时显著提升了生成性能。
...
TL;DR: 该论文提出了一种解耦几何与外观的紧凑表示方法，使用面元表示几何，结合全局神经场和每个基元的颜色表示外观，相比3D高斯泼溅减少了9.7-31倍的基元数量和3.7-5.5倍的内存使用，同时保持感知质量并提升渲染速度。


<details>
  <summary>Details</summary>
Motivation: 尽管高斯泼溅在新视角合成方面取得了令人印象深刻的结果，但即使场景几何结构简单，它也需要数百万个基元来建模高纹理场景。现有方法在几何和外观表示上存在冗余，需要更紧凑的表示方法。

Method: 提出了一种超越基于点渲染的表示方法，将几何和外观解耦：1）使用面元表示几何；2）外观通过全局神经场和每个基元的颜色组合表示；3）神经场为每个像素纹理固定数量的基元，确保计算开销低。

Result: 在室外场景中使用9.7倍更少的基元和5.5倍更少的内存，在室内场景中使用31倍更少的基元和3.7倍更少的内存，同时匹配3D高斯泼溅的感知质量。渲染速度是现有纹理基元方法的两倍，同时视觉质量有所提升。

Conclusion: 通过解耦几何和外观的紧凑表示方法，在显著减少基元数量和内存使用的同时，保持了高质量的渲染效果，为高效的新视角合成提供了有前景的解决方案。
...
TL;DR: VajraV1模型在实时目标检测领域实现了新的SOTA性能，在COCO验证集上各尺寸版本均超越同期YOLO系列模型，其中Xlarge版本达到56.2% mAP的最高水平。


<details>
  <summary>Details</summary>
Motivation: 近年来实时目标检测领域快速发展，YOLOv10到YOLOv13等模型相继发布。为了进一步提升检测性能，需要整合现有YOLO模型的有效设计选择，在保持实时推理速度的同时实现更高的检测精度。

Method: VajraV1模型架构结合了先前YOLO模型的有效设计选择，引入了架构增强改进。具体方法未在摘要中详细说明，但强调是通过整合现有YOLO模型的最佳实践来实现性能提升。

Result: 在COCO验证集上：VajraV1-Nano达到44.3% mAP，超越YOLOv12-N 3.7%和YOLOv13-N 2.7%；Small版本50.4% mAP，超越YOLOv12/13-S 2.4%；Medium版本52.7% mAP，超越YOLOv12-M 0.2%；Large版本53.7% mAP，超越YOLOv13-L 0.3%；Xlarge版本56.2% mAP，超越所有现有实时目标检测器。

Conclusion: VajraV1模型通过整合现有YOLO模型的有效设计选择，在实时目标检测领域实现了新的最先进性能，在保持竞争性推理速度的同时显著提升了检测精度。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到动作生成模型在潜在空间扩散方面存在两个主要问题：1) 如何构建语义对齐的潜在空间以提高扩散效果；2) 如何最佳地注入文本条件以确保动作与描述紧密匹配。本研究旨在解决这两个问题，提升动作生成的逼真度和文本-动作对齐度。

Method: 提出MoLingo模型，采用三种关键技术：1) 使用帧级文本标签训练语义对齐的动作编码器，使具有相似文本含义的潜在表示保持接近；2) 比较单令牌条件注入与多令牌交叉注意力方案，发现交叉注意力效果更好；3) 结合语义对齐潜在、自回归生成和交叉注意力文本条件。

Result: MoLingo在标准评估指标和用户研究中均达到了新的最先进水平，在人体动作生成任务上表现出色。交叉注意力方案相比单令牌条件注入能产生更好的动作逼真度和文本-动作对齐度。

Conclusion: 通过语义对齐的潜在空间、自回归生成和交叉注意力文本条件，MoLingo显著提升了文本到动作生成的质量，为后续研究和下游应用提供了有力工具。作者将发布代码和模型以促进进一步研究。
...
TL;DR: 提出CFHA框架，通过三阶段扩散模型将无人机合成数据转换为真实风格，缩小域差距，提升无人机人体检测性能


<details>
  <summary>Details</summary>
Motivation: 无人机人体检测面临标注数据稀缺且目标分布不断变化的问题，合成数据虽能低成本生成标注，但存在合成与真实图像间的域差距，影响模型在实际应用中的效果

Method: 提出粗到细层次对齐（CFHA）框架，包含三个模块：全局风格迁移（扩散模型对齐颜色、光照和纹理统计）、局部细化（超分辨率扩散模型增强小物体细节）、幻觉去除（过滤不符合真实世界视觉属性的人体实例）

Result: 在公开的无人机Sim2Real检测基准测试中，相比未转换的基线方法，检测准确率显著提升，在Semantic-Drone基准上mAP50最高提升+14.1

Conclusion: CFHA框架通过层次化对齐策略有效缩小合成与真实数据间的域差距，全局和局部阶段具有互补作用，为无人机人体检测提供了有效的合成数据转换方案
...
TL;DR: Route-DETR通过自适应成对路由解决DETR中查询竞争问题，使用抑制器和委托器两种路由机制，在训练时引入可学习的低秩注意力偏置，推理时无额外计算成本，在COCO和Cityscapes上取得显著性能提升。
...
Method: 提出Route-DETR，通过自适应成对路由机制：1）使用查询相似性、置信度分数和几何信息区分竞争查询和互补查询；2）引入抑制器路由减少竞争查询间的重复；3）委托器路由鼓励探索不同区域；4）通过可学习的低秩注意力偏置实现非对称查询交互；5）采用双分支训练策略，训练时包含路由偏置，推理时保持标准注意力。

Result: 在COCO和Cityscapes数据集上验证，相比多个DETR基线模型获得一致改进。在ResNet-50上比DINO提升1.7% mAP，在Swin-L上达到57.6% mAP，超越了先前的最先进模型。

Conclusion: Route-DETR有效解决了DETR中的查询竞争问题，通过自适应路由机制提高了检测效率，在保持推理效率的同时显著提升了检测性能，为端到端物体检测提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 临床工作站上实时部署前列腺MRI分割常受计算负载和内存占用的限制，而基于深度学习的前列腺分割方法因解剖学变异性而具有挑战性

Method: 提出KLO-Net，结合动态K-NN注意力机制（自适应确定每个空间位置的注意力连接数）和CSP编码器块（减少内存消耗），用于MRI前列腺分割
...
TL;DR: 该论文提出"去品牌化"新任务，旨在从生成图像中精细移除商标和结构性品牌特征，同时保持语义连贯性，并构建了基准数据集和基于VLM的新型评估指标。
...
Method: 引入"去品牌化"新任务，构建全面的基准数据集。提出基于视觉语言模型（VLM）的新型评估指标，使用问答框架探测图像中的显式商标和隐式整体品牌特征。

Result: 研究发现随着模型保真度提高（如SDXL、FLUX比Stable Diffusion更容易合成品牌标识），去品牌化挑战的紧迫性凸显。VLM指标验证了去品牌化是一个独特且具有实际意义的问题，需要专门技术。

Conclusion: 去品牌化是一个新颖且重要的任务，需要专门方法来解决商标和结构性品牌特征的移除问题，同时保持图像语义连贯性。提出的VLM评估方法能有效评估显式和隐式品牌特征。
...
TL;DR: FocalComm是一个专注于交换困难实例特征的多智能体协同感知框架，通过渐进式困难实例挖掘和查询式特征融合技术，显著提升了自动驾驶中对行人等小目标的安全检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有协同感知方法主要优化车辆检测性能指标，对行人等安全关键的小目标检测不足，且依赖全特征交换而非仅传递有助于减少漏检的关键特征。

Method: 提出FocalComm框架，包含两个核心设计：1) 可学习的渐进式困难实例挖掘模块，提取每个智能体的困难实例导向特征；2) 基于查询的特征级融合技术，在协作过程中动态加权这些已识别的特征。

Result: 在V2X-Real和DAIR-V2X两个真实世界数据集上，FocalComm在车辆中心和基础设施中心两种协同设置下均优于现有最先进的协同感知方法，特别是在V2X-Real数据集上的行人检测表现出显著性能提升。

Conclusion: FocalComm通过专注于交换困难实例导向特征，有效提升了自动驾驶协同感知中对安全关键小目标的检测能力，为提升自动驾驶安全性提供了新思路。
...
TL;DR: 该论文提出了一个将2D扩散模型适配到3D形状补全任务的框架，通过引入Shape Atlas这一紧凑的2D表示来克服3D数据稀缺和模态差异问题。


<details>
  <summary>Details</summary>
Motivation: 2D扩散模型在丰富数据支持下取得了显著成功，但3D扩散模型由于高质量3D数据集稀缺以及3D输入与2D潜在空间之间的模态差异而发展滞后。需要解决这些限制来实现高质量的3D形状补全。

Method: 引入Shape Atlas——一种紧凑的3D几何2D表示，能够充分利用预训练2D扩散模型的生成能力，并实现条件输入与输出空间之间的模态对齐，从而更有效地进行条件生成。

Result: 在PCN和ShapeNet-55数据集上验证了方法的有效性，能够从有限3D数据中学习并产生高质量、细节保留的形状补全结果。还展示了将补全点云转换为艺术家创建的网格的下游应用。

Conclusion: 提出的统一2D框架成功克服了3D数据稀缺和模态差异问题，实现了高质量的3D形状补全，并展示了实际应用价值。
...
TL;DR: Sparse-LaViDa通过动态截断推理步骤中的冗余掩码标记来加速掩码离散扩散模型，同时引入寄存器标记保持生成质量，实现了2倍加速而不损失性能。
...
Conclusion: Sparse-LaViDa通过动态标记截断和寄存器标记的创新设计，有效解决了MDMs推理速度瓶颈，实现了显著的加速而不牺牲性能，为高效多模态生成模型提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究仅通过问答准确性间接评估帧选择质量，缺乏直接评估关键帧采样策略的方法。长视频理解需要高效的关键帧采样，但缺乏专门的基准测试来直接分析不同采样方法如何捕捉整个长视频中的关键内容。

Method: 1) 创建KFS-Bench基准测试，包含多场景标注，可直接评估采样策略；2) 设计新的采样质量指标，综合考虑采样精度、场景覆盖和采样平衡；3) 开发基于问题-视频相关性的自适应平衡采样方法，平衡采样多样性与问题-帧相似性。

Result: 通过KFS-Bench对关键帧采样方法进行全面研究，发现采样精度、场景覆盖和采样平衡是影响问答性能的关键因素。提出的自适应平衡采样方法在关键帧采样和问答性能方面均表现优异。

Conclusion: KFS-Bench填补了长视频问答中关键帧采样评估的空白，提出的采样质量指标和自适应平衡采样方法为高效长视频理解提供了有效解决方案。基准测试已开源，促进该领域进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统几何模型在场景理解中存在局限性，特别是在处理遮挡、纹理缺失表面和实时深度感知方面。深度学习技术能够弥补这些不足，提升自主机器人在动态非结构化环境中的感知能力，从而改善决策制定、导航和交互效果。

Method: 本文采用文献综述方法，系统梳理了深度学习在自主机器人场景理解中的关键应用领域，包括：1) 目标检测技术；2) 语义分割和实例分割；3) 深度估计方法；4) 3D重建技术；5) 视觉SLAM系统。重点分析了这些深度学习技术如何与传统几何模型结合或替代。

Result: 深度学习技术在场景理解方面显著提升了自主机器人的性能：1) 克服了传统几何模型在遮挡和纹理缺失表面的局限；2) 实现了实时深度感知；3) 增强了语义推理能力；4) 当这些感知模块集成到动态非结构化环境中时，能更有效地支持决策制定、导航和交互。

Conclusion: 深度学习在自主机器人场景理解中展现出巨大潜力，能够有效解决传统方法的局限性。然而仍存在需要进一步研究的问题，本文为基于学习的场景理解技术指明了未来发展方向，包括算法优化、系统集成和实际应用挑战等。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法在医学图像和表格数据的多模态学习中存在局限性，主要由于它们僵化的表格建模机制难以处理异构表格数据，导致模型局限于特定数据队列，无法有效学习跨不同队列的可迁移医学知识。

Method: 提出CITab框架：1）从语义感知角度设计表格建模机制，整合列标题作为语义线索，促进可迁移知识学习和多数据源预训练的可扩展性；2）提出原型引导的混合线性层模块，专门处理表格特征的专业化，有效应对表格数据的异质性并探索潜在的医学概念。

Result: 在包含4,461名受试者的三个公开数据队列上进行阿尔茨海默病诊断任务的全面评估，实验结果表明CITab超越了最先进的方法，为有效且可扩展的跨表格多模态学习铺平了道路。

Conclusion: CITab通过创新的语义感知表格建模和专业化特征处理机制，成功解决了现有自监督学习方法在跨表格多模态学习中的局限性，实现了更好的可迁移性和性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 传统结构光方法在像素域进行匹配，在遮挡、精细结构、非朗伯表面等挑战性场景下鲁棒性有限。受神经特征匹配进展启发，需要在特征空间而非脆弱的像素域进行更鲁棒的对应关系匹配。
...
Conclusion: 提出的学习型结构光解码框架通过在特征空间进行匹配并利用深度先验，显著提升了在挑战性场景下的鲁棒性和深度质量，为单次投射结构光系统提供了更优的解决方案。
...
TL;DR: ASAP Textured Gaussians通过自适应采样和各向异性参数化，显著提升了纹理高斯方法的效率，用更少的纹理参数实现高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 现有的纹理高斯方法存在两个关键限制：1）纹理在规范空间中定义，导致对低贡献区域的低效采样；2）纹理参数化在所有高斯上均匀分配，不考虑视觉复杂度，造成过度参数化。

Method: 提出两种简单有效的策略：基于高斯密度分布的自适应采样，以及根据渲染误差分配纹理资源的误差驱动各向异性参数化。

Result: ASAP Textured Gaussians显著改善了质量效率权衡，用更少的纹理参数实现了高保真渲染。

Conclusion: 通过自适应采样和各向异性参数化，ASAP Textured Gaussians有效解决了现有纹理高斯方法的内存效率问题，在保持高质量的同时大幅减少了纹理参数需求。
...
TL;DR: ChartAgent：基于工具集成推理的图表理解框架，通过模块化工具库和结构化证据包解决现有MLLMs在缺少文本标注时性能下降的问题


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在图表理解方面依赖显式文本标注，当关键数字缺失时性能显著下降。需要开发更鲁棒的图表理解系统

Method: 提出ChartAgent框架，基于工具集成推理，将复杂图表分析分解为可观察、可重放的步骤。使用包含关键元素检测、实例分割、OCR等十多个核心工具的模块化工具库，动态编排工具实现系统化视觉解析

Result: ChartAgent显著提高了在稀疏标注设置下的鲁棒性，通过标准化和整合中间输出到结构化证据包，提供可追溯和可复现的最终结论支持

Conclusion: ChartAgent为可信赖和可扩展的图表理解系统提供了实用路径，通过工具集成推理的透明性和可验证性超越了黑盒范式
...
TL;DR: OmniDrive-R1是一个用于自动驾驶的端到端视觉语言模型框架，通过交错多模态思维链机制统一感知与推理，使用强化学习驱动的视觉定位能力实现无需密集标注的细粒度分析。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在自动驾驶等安全关键领域部署时存在可靠性问题，特别是物体幻觉问题。现有方法存在感知与推理阶段解耦、依赖昂贵密集定位标注等根本缺陷。

Method: 提出OmniDrive-R1框架，采用交错多模态思维链机制统一感知与推理。核心创新是强化学习驱动的视觉定位能力，通过两阶段强化学习训练流程和Clip-GRPO算法实现，该算法引入无标注、基于过程的定位奖励机制。

Result: 在DriveLMM-o1数据集上的实验显示，相比基线Qwen2.5VL-7B模型，OmniDrive-R1将总体推理分数从51.77%提升到80.35%，最终答案准确率从37.81%提升到73.62%。

Conclusion: OmniDrive-R1通过端到端统一感知与推理、强化学习驱动的视觉定位以及无标注奖励机制，显著提升了自动驾驶场景下视觉语言模型的可靠性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 真实场景文本数据集中存在标签错误问题，现有方法难以有效处理变长序列标签、标签序列不对齐和字符级错误等挑战，需要开发能够检测这些标签错误的新方法。

Method: SELECT采用图像-文本编码器和字符级分词器处理变长序列标签问题，并引入相似性序列标签破坏（SSLC）方法，在训练时故意引入标签错误来模拟真实场景，SSLC考虑字符间的视觉相似性并可能导致序列长度变化。

Result: 实验结果表明SELECT在检测标签错误方面优于现有方法，并能提高真实场景文本数据集上的场景文本识别（STR）准确率，展示了其实际应用价值。

Conclusion: SELECT是首个成功处理变长标签的真实场景文本数据集标签错误检测方法，通过多模态训练和SSLC技术有效解决了标签错误检测问题，具有重要的实际应用意义。
...
TL;DR: HyperVL：一种专为设备端推理设计的高效多模态大语言模型，通过图像分块策略限制峰值内存使用，引入视觉分辨率压缩器和双重一致性学习，在保持性能的同时显著降低延迟和功耗。
...
Conclusion: HyperVL通过创新的效率优化技术，成功解决了多模态大语言模型在设备端部署的挑战，为高效的设备端多模态推理提供了可行的解决方案。
...
TL;DR: FacEDiT将说话人脸编辑和生成统一为语音条件面部运动填充任务，使用基于流匹配的扩散Transformer模型，支持替换、插入、删除等编辑操作，并提出了首个说话人脸编辑基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常将说话人脸编辑和生成视为独立任务，缺乏统一框架。作者认为这两者实际上是语音条件面部运动填充的子任务，需要一个能够同时支持局部编辑和整体生成的统一方法。

Method: 提出FacEDiT模型，采用基于流匹配的扩散Transformer架构，受掩码自编码器启发，学习在周围运动和语音条件下合成被掩码的面部运动。引入偏置注意力和时间平滑约束来增强边界连续性和唇部同步。

Result: 实验验证了说话人脸编辑和生成确实可以作为语音条件运动填充的子任务出现。FacEDiT能够产生准确、语音对齐的面部编辑，具有强身份保持和视觉连续性，同时在说话人脸生成任务上表现出良好的泛化能力。

Conclusion: 通过将说话人脸编辑和生成统一为语音条件面部运动填充任务，FacEDiT提供了一个强大的统一框架，支持多种编辑操作并确保与未编辑区域的无缝过渡，同时提出了首个标准编辑基准数据集FacEDiTBench。
...
TL;DR: 提出一种多模态深度学习框架，通过非侵入式图像实时预测室内工作面照度分布，特别适用于动态占用空间


<details>
  <summary>Details</summary>
Motivation: 日光联动控制（DLCs）在建筑节能方面具有巨大潜力，但现有室内日光预测研究多针对静态场景，需要开发适用于动态占用空间的实时预测方法

Method: 提出多模态深度学习框架，仅从侧窗区域提取图像特征（而非室内像素），结合时空特征，通过非侵入式图像实时预测工作面照度分布

Result: 在广州测试房间收集17,344个样本进行训练验证，模型在同分布测试集上R²>0.98、RMSE<0.14，在未见天测试集上R²>0.82、RMSE<0.17，显示高精度和可接受的时间泛化能力

Conclusion: 该框架能准确预测室内工作面照度分布，适用于动态占用空间，为日光联动控制提供有效的实时预测工具
...
TL;DR: SDAR-VL首次系统地将块状离散扩散应用于大规模视觉语言理解，通过集成框架解决训练成本高、收敛慢和不稳定的问题，在21个基准测试中超越传统块扩散方法，性能匹配或超越强自回归基线。


<details>
  <summary>Details</summary>
Motivation: 块状离散扩散在并行生成和因果依赖建模之间提供了良好平衡，但实际应用受限于高训练成本、收敛慢和不稳定性，使其落后于自回归基线。需要解决这些问题以使其成为实用的视觉语言建模骨干。

Method: 提出SDAR-VL框架，包含三个关键组件：1) 异步块状噪声调度，在每个批次内多样化监督；2) 有效掩码比率缩放，在随机掩码下实现无偏损失归一化；3) 渐进式Beta噪声课程，增加有效掩码覆盖率同时保持破坏多样性。

Result: 在21个单图像、多图像和视频基准测试中，SDAR-VL相比传统块扩散方法显著提升了训练效率、收敛稳定性和任务性能。在匹配设置下，性能匹配或超越了LLaVA-OneVision等自回归基线和LLaDA-V等全局扩散基线。

Conclusion: SDAR-VL证明了块状离散扩散可以作为视觉语言理解的实用骨干，通过集成训练框架解决了传统方法的局限性，在多个基准测试中达到了最先进的性能。
...
Method: 引入分层3DGS表示GaussianPlant，使用结构基元（StPs）显式表示枝干和叶片几何形状，外观基元（ApPs）表示植物外观。StPs将枝干建模为圆柱体、叶片建模为圆盘，通过自组织方式优化属性区分枝干和叶片。ApPs绑定到每个StP，通过重渲染损失和梯度流联合优化

Result: 实验表明GaussianPlant通过ApPs实现高保真外观重建，通过StPs实现准确结构重建，能够提取分枝结构和叶片实例

Conclusion: 该方法成功实现了植物外观和结构的联合恢复，为植物表型分析等应用提供了有效的解决方案
...
Method: 从面部远程光电容积描记、上半身运动和深度学习管道中提取10种信号，使用四种频谱估计器分析，通过片段级质量指标训练机器学习模型来预测准确性或选择最可靠信号
...
TL;DR: OUSAC框架通过优化引导调度和自适应缓存加速扩散模型，减少82%无条件前向传播，实现53-60%计算节省和5倍加速，同时提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成高质量图像但计算成本高，Classifier-Free Guidance (CFG) 虽提升质量但需要双倍计算（条件和无条件前向传播）。现有缓存方法假设恒定引导尺度，无法适应动态引导模式。

Method: 提出两阶段方法：阶段1使用进化算法联合优化跳过的时间步和引导尺度，最多消除82%无条件传播；阶段2引入自适应秩分配，针对不同Transformer块定制校准，在动态引导下保持缓存有效性。

Result: OUSAC显著优于现有加速方法：DiT-XL/2上节省53%计算并提升15%质量；PixArt-alpha上节省60%计算并提升16.1%质量；FLUX上实现5倍加速且CLIP分数超过50步基线。
...
TL;DR: ViewMask-1-to-3提出了一种基于离散扩散模型的多视角图像生成方法，通过将多视角合成转化为离散序列建模问题，使用MAGVIT-v2标记化和掩码标记预测实现跨视角一致性，无需复杂3D几何约束。


<details>
  <summary>Details</summary>
Motivation: 现有多视角图像生成方法通常依赖3D感知架构或专门扩散模型，需要大量多视角训练数据和复杂几何先验。本文旨在开发一种更简单有效的多视角生成方法。

Method: 将多视角合成转化为离散序列建模问题，使用MAGVIT-v2将每个视角表示为视觉标记，通过掩码标记预测统一语言和视觉，结合随机掩码和自注意力实现跨视角一致性，无需复杂3D几何约束。

Result: 在GSO和3D-FUTURE数据集上，ViewMask-1-to-3在PSNR、SSIM和LPIPS指标上平均排名第一，证明了离散扩散模型作为多视角生成方法的可行性和简单性。

Conclusion: 离散扩散模型为多视角图像生成提供了可行且简单的替代方案，通过离散序列建模和掩码标记预测实现了跨视角一致性，无需复杂3D几何约束或专门注意力架构。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感大视觉语言模型在可解释性和复杂空间关系处理方面存在局限，限制了实际应用

Method: 结合LLM和神经符号AI，将文本查询转化为一阶逻辑表达式，在检测到的实体上进行显式推理，采用逻辑分解策略提高可扩展性

Result: 在DOTA数据集上表现优于现有RS-LVLMs，特别是在复杂查询场景下，并展示了在洪水后卫星图像检索中的实际应用潜力

Conclusion: RUNE通过显式推理方法在遥感图像检索任务中实现了性能、鲁棒性和可解释性的综合提升，为实际应用提供了新思路
...
TL;DR: 提出了一种无需训练和数据的CLIP模型遗忘框架，支持三种遗忘范式：全局遗忘、领域特定知识移除和选择性领域完全遗忘，通过多模态零空间和文本提示与视觉原型实现高效类别信息移除。


<details>
  <summary>Details</summary>
Motivation: 尽管CLIP等预训练模型在零样本分类方面表现出色，但实际应用中需要在不增加数据或重新训练的情况下移除特定对象类别，同时不影响模型在其他任务上的性能。现有基于重新训练的方法存在局限性，需要更灵活、计算高效的解决方案。

Method: 通过协同整合文本提示和从CLIP联合嵌入空间衍生的合成视觉原型，利用多模态零空间实现遗忘。该方法不需要额外训练数据，也不需要重新训练模型，能够高效移除不需要的类别信息。

Result: 该方法实现了三种不同的遗忘范式：1）跨所有领域的全局遗忘；2）领域特定知识移除（如移除草图表示同时保留照片识别）；3）选择性领域的完全遗忘。相比现有方法更加灵活和计算高效。

Conclusion: 提出的训练和数据自由的遗忘框架为受控模型遗忘提供了有效的解决方案，克服了现有重新训练方法的局限性，能够在保持模型其他知识的同时精确移除特定类别信息。
...
Method: 提出MFE-GAN框架，包含多尺度特征提取(MFE)模块，采用Haar小波变换和归一化处理文档图像。设计了新颖的生成器、判别器和损失函数，并通过消融研究验证其有效性。

Result: 在Benchmark、Nabuco和CMATERdb数据集上的实验表明，MFE-GAN显著减少了总训练和推理时间，同时保持了与最先进方法相当的性能。

Conclusion: MFE-GAN为文档图像增强和二值化提供了一种高效解决方案，在减少计算时间的同时保持了良好的性能，有助于提高OCR系统的效率和准确性。
...
TL;DR: 提出Consistent Instance Field方法，通过连续概率时空表示实现动态场景理解，在新型视角全景分割和开放词汇4D查询任务上表现优异


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖离散跟踪或视角依赖特征，难以解耦可见性和持久物体身份，需要一种能保持跨时空一致实例表示的动态场景理解方法

Method: 基于可变形3D高斯构建实例嵌入表示，联合编码辐射和语义信息；引入高斯身份校准机制和向语义活跃区域重采样，通过可微分光栅化从RGB图像和实例掩码直接学习

Result: 在HyperNeRF和Neu3D数据集上，该方法在新型视角全景分割和开放词汇4D查询任务上显著优于现有最先进方法

Conclusion: Consistent Instance Field提供了一种有效的连续概率时空表示，能够解耦可见性和物体身份，实现跨时空一致的动态场景理解
...
<details>
  <summary>Details</summary>
Motivation: 传统遗忘技术需要迭代微调和大量数据整理，计算成本高。需要一种更高效、精确的方法来从预训练多模态模型中删除特定类别信息，以解决模型去污染和隐私保护问题。

Method: 利用零空间投影技术，计算目标文本嵌入张成的子空间的正交基，然后将这些方向投影到零空间中，从而擦除最终投影层中嵌入的目标类别信息。

Result: 该方法显著降低了图像特征与不需要类别之间的对齐，导致目标类别的零样本性能大幅下降，同时保留了模型的整体多模态知识。部分投影可以在完全遗忘和保留有用信息之间取得平衡。

Conclusion: 提出的零空间投影方法为多模态模型的选择性遗忘提供了一种计算高效、精确的解决方案，无需重新训练或遗忘集图像，有效解决了模型去污染和隐私保护的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图像编辑系统难以在支持高级语义变化和精确局部重绘的同时，保持线稿的稀疏、风格敏感结构。需要一种能统一全局编辑和局部重绘的草图编辑系统。

Method: 1) 开发可控数据生成管道：从无属性基础草图构建属性添加序列，通过跨序列采样形成多步编辑链，使用风格保留属性移除模型扩展风格覆盖。2) 基于DiT的统一草图编辑框架：重新利用RGB通道编码输入，实现指令引导编辑和线条引导重绘的无缝切换。3) 任务引导的专家混合集成到LoRA层：通过文本和视觉线索路由，提高语义可控性、结构保真度和风格保留。

Result: 大量实验表明在两个任务上都取得了最先进的结果，与最近的基线相比，在指令遵循和风格/结构保留方面表现更优。

Conclusion: SketchAssist及其数据集为草图创建和修订提供了一个实用、可控的助手，统一了全局编辑和局部重绘功能。
...
TL;DR: 该研究提出了首个用于评估和改进机器学习模型检测性能反模式能力的基准数据集，包含600多个PyTorch跟踪记录，并提出了一种轻量级ML模型与LLM结合的迭代方法，显著优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 识别和解决机器学习模型中的性能反模式对于高效训练和推理至关重要，但通常需要系统基础设施、ML模型和内核开发方面的深厚专业知识。虽然大型科技公司依赖专门的ML基础设施工程师来分析torch跟踪和基准测试，但这种资源密集型工作流程对大多数计算机视觉研究人员来说难以获得。其中，在冗长的执行跟踪中精确定位有问题的跟踪段是最耗时的任务，并且难以用当前的ML模型（包括LLM）自动化。
...
Conclusion: 该研究为ML模型性能反模式检测提供了首个专门的基准数据集，并提出了一种有效的迭代检测方法，能够显著提高检测效率并克服现有技术的局限性，为计算机视觉研究人员提供了更易访问的性能分析工具。
...
TL;DR: CIS-BA提出了一种新的后门攻击范式，通过连续对象交互模式而非静态对象特征设计触发器，实现了多触发器-多对象攻击机制，在复杂环境中保持高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击方法依赖单触发器-单对象映射和脆弱的像素级线索，在能力和鲁棒性上存在固有局限，无法应对自动驾驶等现实应用中对象检测模型面临的安全威胁。

Method: 提出CIS-BA范式，将触发器设计从静态对象特征转向连续对象交互模式，建模为连续交互空间；设计CIS-Frame框架，通过交互分析构建空间触发器，将其形式化为类-几何约束进行样本投毒，并在检测器训练中嵌入后门。

Result: 在MS-COCO和真实世界视频上的实验表明，CIS-BA在复杂环境中攻击成功率超过97%，在动态多触发器条件下保持95%以上有效性，并能规避三种最先进的防御方法。

Conclusion: CIS-BA扩展了交互密集型场景中的后门攻击领域，为对象检测系统的安全性提供了新的见解，展示了基于交互模式的后门攻击的有效性和鲁棒性。
...
TL;DR: Fast3DHPE是一个用于单目3D人体姿态估计的统一框架，解决了现有方法训练评估不一致的问题，同时提出了基于解耦扩散模型的FastDDHPose方法，在标准框架下实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D人体姿态估计方法在训练和评估时缺乏统一框架，导致公平比较困难，且存在层次误差累积问题，需要标准化框架和更有效的建模方法。

Method: 提出Fast3DHPE模块化框架标准化训练评估协议；提出FastDDHPose方法，使用解耦扩散模型分别建模骨骼长度和方向分布，避免层次误差累积；设计高效的运动学层次时空去噪器，关注关节层次结构而非复杂拓扑。
...
Conclusion: Fast3DHPE为3D人体姿态估计提供了标准化框架，解决了公平比较问题；FastDDHPose利用解耦扩散模型有效建模人体姿态分布，在统一框架下实现了SOTA性能，具有实际应用价值。
...
TL;DR: SGPU提出了一种贝叶斯框架，通过分析答案嵌入的几何结构来量化语义不确定性，避免脆弱的聚类方法，在多个模型和数据集上实现了最先进的校准性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型常产生看似合理但不可靠的输出，需要鲁棒的不确定性估计。现有基于聚类的方法对微小措辞变化敏感，容易错误分组或分离语义相似的答案，导致不确定性估计不可靠。

Method: SGPU将生成的答案映射到密集语义空间，计算其嵌入的Gram矩阵，通过特征谱总结语义配置，然后将这种谱表示输入高斯过程分类器，学习将语义一致性模式映射到预测不确定性。

Result: 在6个LLM和LVLM、8个数据集（涵盖VQA、图像分类和文本QA）上，SGPU在校准（ECE）和判别（AUROC、AUARC）性能方面始终达到最先进水平，并能跨模型和模态迁移。

Conclusion: SGPU通过分析答案嵌入的几何结构来量化语义不确定性，避免了脆弱的聚类方法，其谱表示能捕捉语义不确定性的一般模式，具有跨模型和模态的迁移能力。
...
TL;DR: 该论文提出了一种基于球面Voronoi（SV）的统一框架，用于改进3D高斯泼溅中的外观建模，解决了传统球谐函数在高频信号、吉布斯振铃和镜面反射方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统辐射场方法（如3D高斯泼溅）使用球谐函数进行外观建模存在根本性限制：难以处理高频信号、产生吉布斯振铃伪影、无法捕捉镜面反射（真实渲染的关键组件）。虽然球面高斯等替代方案有所改进，但增加了显著的优化复杂性。

Method: 提出球面Voronoi（SV）作为3D高斯泼溅中外观表示的统一框架。SV将方向域划分为具有平滑边界的可学习区域，为视图相关效果提供直观稳定的参数化。对于漫反射外观，SV保持比现有替代方案更简单的优化；对于反射，将SV用作可学习反射探针，遵循经典图形学原理以反射方向作为输入。

Result: 在合成和真实世界数据集上取得了最先进的结果。对于漫反射外观，SV实现了竞争性结果同时保持优化简单；对于反射（球谐函数失败的领域），SV作为可学习反射探针的表述表现出色。

Conclusion: 球面Voronoi为显式3D表示中的外观建模提供了一个原则性、高效且通用的解决方案，克服了传统球谐函数的局限性，同时避免了替代方案的优化复杂性。
...
TL;DR: 提出一种自动为骨折边界框分配全球AO编码的方法，将全局多标签任务转化为局部多分类任务，F1分数提升7.89%，但使用不完美骨折检测器时性能下降。


<details>
  <summary>Details</summary>
Motivation: 15-45%的儿童在成长期间会经历骨折，准确诊断至关重要。骨折形态与位置和碎片角度一样是关键的诊断特征，需要自动化的骨折形态分析方法。

Method: 提出一种自动为骨折边界框分配全球AO编码的方法，利用公共数据集，将全局多标签任务重新表述为局部多分类任务。

Result: 该方法将平均F1分数提高了7.89%，但在使用不完美的骨折检测器时性能会下降，突显了实际部署中的挑战。

Conclusion: 该方法能有效提取骨折形态特征并提高诊断准确性，但实际应用中需要更可靠的骨折检测器来确保性能稳定。
...
TL;DR: SkyLume是一个大规模无人机数据集，专门用于研究光照鲁棒性的3D重建，包含10个城市区域在一天三个时段采集的10万+高分辨率图像，配有LiDAR扫描和3D真值，并提出了评估光照解耦稳定性的TCC指标。


<details>
  <summary>Details</summary>
Motivation: 现有基于NeRF和3D高斯泼溅的大规模无人机3D重建方法在处理多时相数据时面临光照不一致问题，导致颜色伪影、几何不准确和外观不一致。由于缺乏系统采集不同光照条件下同一区域的无人机数据集，这一挑战尚未得到充分探索。

Method: 提出了SkyLume数据集：1) 从10个城市区域采集超过10万张高分辨率无人机图像（四个倾斜视角和天底视角），每个区域在一天三个时段采集以系统隔离光照变化；2) 提供每场景的LiDAR扫描和精确3D真值，用于评估深度、表面法线和不同光照下的重建质量；3) 引入时间一致性系数(TCC)指标，用于评估跨时间反照率稳定性，直接评估光照与材质解耦的鲁棒性。

Result: 创建了一个专门用于研究光照鲁棒3D重建的大规模真实世界无人机数据集，填补了该领域的数据空白，为大规模逆渲染、几何重建和新视角合成提供了基础资源。

Conclusion: SkyLume数据集为研究光照变化下的3D重建提供了系统化的评估基准，有望推动大规模逆渲染、几何重建和新视角合成的研究与实际应用评估。
...
TL;DR: DRAW2ACT：一种深度感知的轨迹条件视频生成框架，用于生成可控的机器人操作演示，通过多模态条件输入和RGB-D联合生成提升时空一致性，并用于训练机器人策略。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在机器人操作中可控性有限，基于2D轨迹或单模态条件的视频生成方法难以产生可控且一致的机器人演示，需要更全面的轨迹表示和多模态生成方法。

Method: 1. 从输入轨迹提取多个正交表示（深度、语义、形状和运动）注入扩散模型；2. 联合生成空间对齐的RGB和深度视频，利用跨模态注意力机制和深度监督；3. 基于生成的RGB-D序列训练多模态策略模型回归机器人关节角度。
...
TL;DR: OminiGen：一种统一的BEV空间框架，用于生成对齐的多模态传感器数据（LiDAR和多视角相机），通过UAE方法实现联合解码和可控生成。
...
Method: 1. 使用共享的BEV空间统一多模态特征；2. 提出UAE（通用多模态重建方法），通过体渲染联合解码LiDAR和多视角相机数据；3. 结合ControlNet分支的Diffusion Transformer实现可控多模态传感器生成。

Result: 实验表明OminiGen在统一多模态传感器数据生成方面达到预期性能，具有多模态一致性和灵活的传感器调整能力。

Conclusion: OminiGen通过统一的BEV框架和UAE方法，有效解决了多模态传感器数据生成的对齐问题，为自动驾驶数据合成提供了高效可控的解决方案。
...
Method: 使用MRI扫描和深度学习模型，提出了一种新的多视角方法，考虑MRI视图之间的空间关系来检测MGMT甲基化状态。该方法从所有三个视图提取信息，而不使用复杂的3D深度学习模型，避免了高参数、收敛慢和内存需求大的问题。还引入了新的肿瘤切片提取技术。
...
<details>
  <summary>Details</summary>
Motivation: 汽车雷达在环境感知方面具有成本效益和恶劣天气鲁棒性，但标注雷达数据有限，限制了雷达感知系统的发展。

Method: 提出4D-RaDiff框架，在潜在点云表示上应用扩散模型，考虑雷达点云的稀疏性和独特特性，通过对象级或场景级条件控制生成，将未标注边界框转换为高质量雷达标注，或将LiDAR点云转换为真实雷达场景。

Result: 实验表明，使用4D-RaDiff合成雷达数据作为数据增强方法，相比仅使用真实数据训练，能持续提升物体检测性能；使用合成数据预训练可将所需标注雷达数据减少90%，同时达到可比检测性能。
...
TL;DR: Elastic3D是一种基于条件潜在扩散模型的端到端单目到立体视频转换方法，通过引导式VAE解码器生成高质量、视差一致的立体视频，无需显式深度估计和扭曲，并提供视差范围的可控调节。


<details>
  <summary>Details</summary>
Motivation: 随着沉浸式3D内容需求的增长，需要自动化单目到立体视频转换方法。传统基于深度估计和扭曲的方法容易产生伪影，需要更高质量、可控的转换方案。

Method: 基于条件潜在扩散模型，避免显式深度估计和扭曲。核心创新是引导式VAE解码器，确保生成锐利且视差一致的立体视频。提供标量调节旋钮，让用户在推理时控制立体效果的强度（视差范围）。

Result: 在三个真实世界立体视频数据集上的实验表明，该方法优于传统的基于扭曲方法和最近的免扭曲基线，为可靠、可控的立体视频转换设定了新标准。

Conclusion: Elastic3D提供了一种高质量、可控的端到端单目到立体视频转换方法，通过引导式VAE解码器避免了传统方法的伪影问题，在立体视频生成质量上达到新水平。
...
TL;DR: EVPG通过概率图将不可微的视觉编程执行过程重构为可微的概率推理过程，实现了基于梯度的端到端优化，显著提升了视觉编程在复杂视觉推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉编程方法主要关注提升大语言模型生成的可视化程序质量，但忽略了优化视觉编程调用的预训练模型。由于视觉编程不可微且缺乏子任务标签，无法直接利用最终标签进行端到端梯度优化。

Method: 提出EVPG方法，通过构建有向概率图来重构视觉编程执行过程。根据变量依赖关系建立概率图，将不可微的执行过程转化为可微的精确概率推理过程，从而支持基于梯度的端到端监督学习。

Result: 在三个经典复杂视觉推理任务（GQA、NLVRv2和Open Images）上进行了广泛实验，EVPG显著提升了视觉编程的性能表现。

Conclusion: EVPG通过概率图方法成功解决了视觉编程不可微的问题，实现了端到端优化，为视觉编程框架的性能提升提供了有效解决方案。
...
### [51] [Semantic Mismatch and Perceptual Degradation: A New Perspective on Image Editing Immunity](https://arxiv.org/abs/2512.14320)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的文本引导图像编辑技术容易被滥用，而当前的图像免疫评估方法存在缺陷——它们只关注免疫图像与原始图像编辑输出的视觉差异，却忽略了免疫的核心要求：破坏与攻击者意图的语义对齐。

Method: 提出协同中间特征操纵(SIFM)方法，通过双重协同目标策略性地扰动扩散中间特征：1)最大化特征与原始编辑轨迹的差异以破坏语义对齐；2)最小化特征范数以诱导感知退化。同时引入免疫成功率(ISR)这一新评估指标。

Result: 大量实验表明，SIFM在保护视觉内容免受基于扩散的恶意操纵方面达到了最先进的性能，ISR指标能够更准确地量化免疫效果。

Conclusion: 该研究重新定义了图像免疫的成功标准，提出了更有效的免疫方法和评估指标，为防范扩散模型滥用提供了新的技术方案。
...
TL;DR: 本文提出了DriverGaze360数据集和DriverGaze360-Net方法，用于全景驾驶场景下的驾驶员注意力预测，解决了传统方法视野受限的问题。
...
Method: 提出了DriverGaze360数据集（包含约100万帧带注视标签的图像，来自19名驾驶员）和DriverGaze360-Net方法，该方法通过辅助语义分割头联合学习注意力图和被关注物体，提升对全景输入的空间感知和注意力预测能力。

Result: DriverGaze360-Net在全景驾驶图像上实现了最先进的注意力预测性能，在多个指标上表现优异。

Conclusion: DriverGaze360数据集和DriverGaze360-Net方法能够全面建模驾驶员的全景注视行为，为可解释自动驾驶系统和理解混合交通场景中的驾驶员行为提供了重要工具。
...
TL;DR: Zoom-Zero框架通过粗粒度定位和细粒度视觉验证改进视频问答的时序定位能力，解决了现有方法在时序错位和幻觉方面的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的大型视频语言模型在时序感知方面存在局限，基于GRPO的方法虽然尝试改进时序定位，但仍难以忠实基于相关视频证据进行答案生成，导致时序错位和幻觉问题。

Method: 提出Zoom-Zero框架：1) 粗粒度定位查询相关片段；2) 时序放大到最显著帧进行细粒度视觉验证。包含两个关键创新：缩放精度奖励验证时序定位预测的保真度，促进在定位帧上的细粒度视觉验证；令牌选择性信用分配，将奖励归因于负责时序定位或答案生成的令牌，缓解GRPO处理多方面奖励信号的问题。

Result: 在NExT-GQA上时序定位提升5.2%，在ReXTime上提升4.6%，平均答案准确率提升2.4%。推理过程中的粗到细缩放进一步有益于长视频理解，在长视频基准上平均提升6.4%。

Conclusion: Zoom-Zero框架通过粗到细的时序放大机制，显著提升了视频问答的时序定位能力和答案准确性，特别是在长视频理解方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然带来了图像编辑的革命，但也引发了严重的伦理挑战，可能被滥用于创建欺骗性或有害内容。现有防御方法通过在图像中添加不可感知的扰动来降低风险，但这些方法对恶意篡改的防御效果有限。

Method: 提出双注意力引导噪声扰动（DANP）免疫方法：1）在多个时间步上操作，同时干扰交叉注意力图和噪声预测过程；2）使用动态阈值生成掩码，识别文本相关和不相关区域；3）减少相关区域的注意力，增加不相关区域的注意力，误导编辑到错误区域；4）最大化注入噪声与模型预测噪声之间的差异，进一步干扰生成过程。

Result: DANP方法对恶意编辑表现出显著的免疫能力，大量实验证实该方法达到了最先进的性能水平。

Conclusion: 通过同时针对注意力机制和噪声预测机制，DANP方法能有效防御文本到图像扩散模型的恶意编辑，为解决扩散模型伦理滥用问题提供了有效的技术方案。
...
Result: 在大量实验中，TDAE在模型内和跨模型评估中都达到了最先进的性能，有效减轻了恶意编辑的影响。

Conclusion: TDAE通过协调图像-文本优化，实现了对恶意图像编辑的可迁移防御，在跨模型场景中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过优化3D或视频生成模型来构建4D表示，缺乏原生4D生成能力。同时面临4D训练数据稀缺、时间一致性难以保证、长视频序列训练效率低等挑战。

Method: 1) 基于预训练的单图像到3D模型保持空间一致性；2) 引入专用时间层确保时间一致性；3) 使用因子化4D卷积和时间下采样块压缩潜在序列；4) 设计专门的训练策略增强对遮挡的鲁棒性。

Result: 实现了直接从单目视频合成高质量动态3D物体的能力，在保真度、时间一致性和结构一致性方面表现出色，同时支持长视频序列的高效训练和推理。

Conclusion: SS4D展示了原生4D生成模型的可行性，通过结构化时空潜在表示和专门的训练策略，有效解决了4D生成中的关键挑战，为动态3D内容创建提供了新方法。
...
TL;DR: DISCODE是一种无需微调的分布感知评分解码器，通过自适应测试时损失和解析解，提升大视觉语言模型在跨域图像描述评估中的鲁棒性，并在新基准MCEval上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 大视觉语言模型在多模态任务中表现优异，但在跨域场景下的图像描述评估鲁棒性仍然不足，需要更好的评估指标来对齐人类判断。

Method: 提出DISCODE方法，核心是测试时自适应评估方法，引入自适应测试时损失，利用高斯先验分布提升评分估计鲁棒性，并通过推导的解析解在测试时高效最小化损失。

Result: 在MCEval新基准（涵盖六个不同领域）和四个现有代表性基准上，DISCODE作为无参考评估指标取得了最先进的性能。

Conclusion: DISCODE通过分布感知的评分解码和测试时自适应方法，显著提升了大视觉语言模型在跨域图像描述评估中的鲁棒性和与人类判断的一致性。
...
TL;DR: 该论文发布了一个新的公开隧道衬砌缺陷数据集，包含三种不同类型隧道衬砌的标注图像，用于支持深度学习方法的自动化隧道检测。


<details>
  <summary>Details</summary>
Motivation: 传统隧道人工检测方法耗时、主观且成本高，而现有自动化视觉检测方法受限于隧道数据集的稀缺性，阻碍了深度学习在隧道缺陷检测中的应用。

Method: 创建并公开了一个包含三种不同隧道衬砌类型（具有不同纹理和施工技术）的标注图像数据集，涵盖了典型缺陷如裂缝、渗漏和水渗透，支持监督、半监督和无监督深度学习方法。

Result: 提供了一个多样化的隧道缺陷数据集，能够支持缺陷检测和分割任务，并可用于研究模型在不同隧道类型间的泛化能力和可迁移性。
...
TL;DR: 本文提出了一个通过语义结构恢复来改进SVG动画生成的框架，解决了当前视觉语言模型在处理SVG时因图形元素碎片化导致的动画不连贯问题。


<details>
  <summary>Details</summary>
Motivation: SVG在现代网页设计中至关重要，但自动化SVG动画对视觉语言模型仍然具有挑战性。当前模型经常错误处理SVG，因为视觉上连贯的部分通常被分割成低级别的形状，这些形状无法提供哪些元素应该一起移动的指导。

Method: 引入了一个框架，通过统计聚合多个弱部件预测来恢复SVG动画所需的语义结构。通过将SVG重新组织为语义组，使视觉语言模型能够生成更连贯的动画。

Result: 实验表明，该方法相比现有方法有显著提升，表明语义恢复是实现稳健SVG动画的关键步骤，支持视觉语言模型与矢量图形之间更可解释的交互。

Conclusion: 语义结构恢复是解锁稳健SVG动画的关键，该方法能够显著提高动画的连贯性，为视觉语言模型与矢量图形的交互提供了更可解释的基础。
...
TL;DR: CAPRMIL提出了一种新的多示例学习框架，通过将补丁特征投影到少量全局上下文感知的token中，使用多头自注意力注入全局上下文，配合简单的均值聚合器，在计算效率和参数数量上大幅优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在计算病理学中，由于全切片图像（WSI）的千兆像素尺度和像素级标注的稀缺性，弱监督学习已成为标准方法。传统MIL方法依赖复杂的注意力聚合机制，计算复杂度高且参数多，需要更高效、可扩展的解决方案。

Method: CAPRMIL采用聚合器无关的框架，使用冻结的补丁编码器提取特征，然后将补丁特征投影到少量全局上下文/形态学感知的token中，利用多头自注意力以线性计算复杂度注入全局上下文，最后配合简单的均值MIL聚合器进行预测。

Result: 在多个公共病理学基准测试中，CAPRMIL达到了最先进的切片级性能，同时将可训练参数减少了48%-92.8%，推理时的FLOPs降低了52%-99%，在GPU内存效率和训练时间方面表现最佳。

Conclusion: 学习丰富的上下文感知实例表示，在聚合之前进行全局上下文注入，是复杂池化方法的有效且可扩展的替代方案，为全切片分析提供了高效的计算框架。
...
TL;DR: 提出Hybrid Gaussian Splatting (HGS)框架，通过静态-动态分解策略显著减少动态新视角合成模型的参数冗余，实现高达98%的模型压缩和实时渲染（4K分辨率下125 FPS）


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的动态新视角合成方法存在模型复杂度高、参数冗余大的问题，导致模型体积庞大、渲染速度慢，难以在资源受限设备上实现实时应用

Method: 提出Hybrid Gaussian Splatting (HGS)框架，采用静态-动态分解(SDD)策略，利用径向基函数(RBF)建模：动态区域使用时变RBF捕捉时间变化，静态区域共享时间不变参数以减少冗余；并引入两阶段训练策略增强时间一致性

Result: 模型大小减少高达98%，在RTX 3090上实现4K分辨率125 FPS实时渲染，在RTX 3050上实现1352×1014分辨率160 FPS；渲染质量与SOTA方法相当，在高频细节和突变场景变化方面提供更好的视觉保真度

Conclusion: HGS通过有效的静态-动态分解策略，显著降低了动态新视角合成模型的参数冗余，实现了紧凑高效的实时渲染框架，适合资源受限设备和VR系统应用
...
<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法主要依赖学习鲁棒的全局表示或隐式特征对齐，往往无法建模对精确定位至关重要的显式空间对应关系。需要一种能显式弥合不同视角间语义和几何差距的方法。
...
Conclusion: CLNet能够联合捕获高级语义和细粒度对齐，显式建模跨视角的空间对应关系，在跨视角地理定位任务中取得了显著改进，为相关领域提供了新的研究方向。
...
Method: 开发一个多层系统，结合专业大型语言模型(LLM)、卫星影像分析和知识库技术，为小城市提供综合的气候适应分析工具。

Result: 提出了EcoScapes系统，通过整合多种技术手段，帮助小城市更有效地制定气候适应策略，相关代码已在GitHub开源。

Conclusion: 该多层系统能够有效解决小城市在气候适应规划中面临的资源限制和数据整合问题，为可持续城市发展提供技术支持。
...
Method: 1. 利用大规模预训练模型（如CLIP）主动探测特征空间，明确突出真实视频、已知伪造和未见操纵之间的分布差异；2. 引入伪造异常值探测，通过动态子聚类建模和聚类条件异常值生成，在估计子聚类边界附近合成异常值样本，模拟超出已知操纵类型的新伪造伪影；3. 设计异常值引导的三重训练，使用提出的异常值驱动对比学习和异常值条件交叉熵损失优化检测器，区分真实、伪造和异常值样本。

Result: 实验表明，FakeRadar在各种深度伪造视频检测基准数据集上优于现有方法，特别是在跨域评估中，能够有效处理各种新兴操纵技术。

Conclusion: FakeRadar通过主动探测特征空间分布差异、合成模拟新兴伪造技术的异常值样本，以及异常值引导的训练策略，显著提升了深度伪造检测的跨域泛化能力，为应对不断演变的伪造技术提供了有效解决方案。
...
TL;DR: ExpanDyNeRF是一个用于动态神经辐射场的单目框架，通过高斯泼溅先验和伪真值生成策略，解决了大角度旋转下渲染不稳定的问题，并在新创建的SynDM数据集上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 当前动态NeRF系统在显著视角偏差下往往失败，产生不稳定和不真实的渲染结果，特别是在大角度旋转时表现不佳。

Method: 提出ExpanDyNeRF框架：1) 利用高斯泼溅先验；2) 采用伪真值生成策略；3) 优化密度和颜色特征以改善从挑战性视角的场景重建。

Result: 在SynDM合成数据集和真实世界数据集上的定量和定性结果表明，ExpanDyNeRF在极端视角偏移下的渲染保真度显著优于现有动态NeRF方法。

Conclusion: ExpanDyNeRF通过创新的先验和生成策略，有效解决了动态NeRF在大角度旋转下的渲染问题，为动态场景的新视角合成提供了更稳定和真实的解决方案。
...
Result: 在分布内测试集上，所有模型在三种放大倍数下均实现超过80%的多类平衡准确率，所有基础模型表现相似，两种聚合方法结果相当。放大倍数研究表明40x分辨率足够，更高分辨率或跨放大倍数聚合未带来性能提升。但在分布外测试集上，性能显著下降至约60%。
...
TL;DR: LCMem提出了一种跨域记忆检测方法，将记忆检测视为重识别和复制检测的统一问题，通过两阶段训练策略实现身份一致性和增强鲁棒的复制检测，在六个基准数据集上显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 生成图像模型已能达到欺骗人类专家的真实感，但其在隐私保护数据共享方面的潜力尚未充分理解。主要障碍包括缺乏可靠的记忆检测机制、有限的定量评估以及现有隐私审计方法在跨域场景下的泛化能力差。

Method: 将记忆检测视为重识别和复制检测的统一问题，提出Latent Contrastive Memorization Network (LCMem)，采用两阶段训练策略：首先学习身份一致性，然后结合增强鲁棒的复制检测。

Result: 在六个基准数据集上，LCMem在重识别任务上提升高达16个百分点，在复制检测任务上提升高达30个百分点，实现了更可靠的大规模记忆检测。现有隐私过滤器性能有限且鲁棒性不足。

Conclusion: LCMem为跨域隐私审计设立了新标准，提供了可靠且可扩展的记忆检测方法。结果表明需要更强的保护机制来应对生成模型的隐私风险。
...
TL;DR: SynPS：一种利用位置嵌入和语义信息协同进行非刚性图像编辑的方法，通过动态调制位置嵌入影响来平衡语义修改和保真度，解决现有方法中的注意力崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于大扩散模型的免训练图像编辑方法在处理复杂非刚性编辑（如姿势或形状变化）时存在困难，主要原因是现有注意力共享机制中的注意力崩溃问题，其中位置嵌入或语义特征主导视觉内容检索，导致过度编辑或编辑不足。

Method: 提出SynPS方法，协同利用位置嵌入和语义信息进行非刚性图像编辑。首先提出编辑度量来量化每个去噪步骤所需的编辑幅度，然后设计注意力协同流程，动态调制位置嵌入的影响，使SynPS能够平衡语义修改和保真度保留。

Result: 在公共和新策划的基准测试上进行广泛实验，证明了该方法在性能和忠实度方面的优越性，有效避免了过度编辑和编辑不足的问题。

Conclusion: SynPS通过自适应整合位置和语义线索，有效解决了现有非刚性图像编辑中的注意力崩溃问题，实现了更忠实和平衡的图像编辑效果。
...
TL;DR: 提出STMP算法，将基于分数的生成模型与消息传递框架结合，用于压缩图像恢复，在性能和复杂度之间取得更好平衡


<details>
  <summary>Details</summary>
Motivation: 传统PnP方法使用的去噪器依赖通用或手工先验，难以准确捕捉自然图像的复杂统计结构，导致重建效果不佳；而基于分数的生成模型虽然能准确描述图像分布，但直接用于后验采样计算复杂度太高

Method: 利用基于分数的生成建模与经验贝叶斯去噪之间的紧密联系，设计了一个消息传递框架，集成了基于分数的最小均方误差去噪器；对于量化测量系统，进一步提出Q-STMP，增加了分量级MMSE去量化模块

Result: STMP在FFHQ数据集上相比基线方法在性能-复杂度权衡方面显著更好；Q-STMP即使在1比特量化下仍保持鲁棒性；两种算法通常都在10次迭代内收敛

Conclusion: STMP结合了消息传递的快速收敛性和基于分数生成先验的表达能力，为压缩图像恢复提供了有效的解决方案，其渐近性能可通过状态演化方程准确预测
...
TL;DR: 提出了一种仅使用真实视频数据进行训练的无人监督视频实例分割方法，通过关键帧选择和稀疏到密集的知识蒸馏来提升时间一致性，在多个基准测试中超越了现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 当前无人监督视频实例分割主要依赖从图像数据集（如ImageNet）生成的合成视频数据，但这种方法无法准确建模真实视频中的复杂运动（如透视变化、部分运动、相机运动等），因此需要开发仅使用真实视频数据的训练方法。

Method: 1. 从单帧无人监督实例分割开始；2. 利用深度运动先验识别高质量关键帧掩码；3. 提出稀疏到密集的知识蒸馏方法，配合时间DropLoss训练分割模型进行隐式掩码传播；4. 在生成的密集标签集上训练最终模型。

Result: 该方法在多个基准测试中超越了当前最先进的无人监督视频实例分割方法。

Conclusion: 仅使用真实视频数据训练的无人监督视频实例分割方法能够有效解决合成数据无法准确建模真实运动的问题，通过关键帧选择和稀疏到密集的知识蒸馏策略实现了更好的时间一致性和分割性能。
...
TL;DR: 提出A4-Agent框架，通过解耦为三个阶段（Dreamer、Thinker、Spotter）协调专用基础模型，实现零样本的affordance预测，显著超越有监督方法
...
Method: 提出训练免费的A4-Agent框架，将affordance预测解耦为三阶段流程：Dreamer使用生成模型可视化交互方式；Thinker利用大视觉语言模型决定交互对象部分；Spotter协调视觉基础模型精确定位交互区域

Result: 零样本框架在多个基准测试中显著优于最先进的有监督方法，并在真实世界环境中展现出强大的泛化能力

Conclusion: 通过协调预训练基础模型的互补优势而不进行任务特定微调，提出的框架实现了卓越的affordance预测性能
...
TL;DR: SignIT是一个新的意大利手语识别数据集，包含644个视频（3.33小时），标注了94个手语类别，涵盖5个宏观类别，并提取了2D关键点。作者建立了基准测试，评估了多种最先进模型，发现这些模型在该数据集上表现有限。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏专门针对意大利手语（LIS）的公开数据集，这限制了手语识别研究的发展。作者旨在填补这一空白，创建一个高质量、标注完善的LIS数据集，为手语识别任务提供基准。

Method: 1. 收集644个意大利手语视频，总计3.33小时；2. 手动标注94个不同的手语类别，分为动物、食物、颜色、情感和家庭5个宏观类别；3. 从视频中提取用户手部、面部和身体的2D关键点；4. 建立手语识别基准测试，采用多种最先进模型，分析时间信息、2D关键点和RGB帧对模型性能的影响。

Result: 实验结果表明，现有最先进模型在这个具有挑战性的意大利手语数据集上表现有限。作者公开了数据集和标注，为后续研究提供了基础资源。

Conclusion: SignIT数据集填补了意大利手语识别研究的空白，为手语识别任务提供了有价值的基准。结果显示当前模型在该任务上仍有改进空间，该数据集将促进手语识别技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前视网膜基础模型受限于缺乏真实临床背景的研究数据集，且需要大量任务特定优化，限制了在低资源环境中的部署效率。

Method: 从中国162家医疗机构长达十年的远程医疗项目中收集485,980张彩色眼底照片及其对应诊断报告，利用临床图像与报告的自然对齐构建ReVision模型。

Result: 在27个眼科基准测试中，ReVision零样本疾病检测平均AUROC达0.946（12个公共基准）和0.952（3个独立临床队列）；最小化适应时匹配精细调优模型但参数和标注需求少得多；前瞻性读者研究显示诊断准确率提升14.8%。
...
Method: 提出DASP框架，包含对抗分支和自监督分支。对抗分支中设计包含四个时空先验学习块（SPLB）的判别器，SPLB包含空间基时序学习模块（STLM）和轴向空间学习模块（ASLM）。自监督分支提出3D一致性投影损失，将目标帧和源帧双向投影到共享3D空间，计算3D差异作为损失。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的肖像生成方法在使用同一ID的多个参考图像时，通常生成保真度较低的肖像，且难以精确控制面部属性

Method: 1. 引入面部细化器和地标生成器获取细粒度多面部特征和3D感知面部地标；2. 设计HiFi-Net融合多面部特征并与地标对齐；3. 构建自动化流水线创建基于ID的数据集用于训练

Result: 实验结果表明，该方法在面部相似度和可控性方面超越现有SOTA方法，且与之前的SDXL-based工作兼容
...
<details>
  <summary>Details</summary>
Motivation: 医学图像恢复（MedIR）中，All-in-One模型需要同时处理多种不同模态和退化类型的任务。由于任务间存在显著差异，共享模型面临两个关键挑战：任务干扰（不同任务在同一参数上产生冲突梯度）和任务不平衡（不同任务学习难度不同导致优化不均）。
...
TL;DR: 本文介绍了首个专为智能交通系统设计的事件相机数据集，用于车辆和行人检测与跟踪，并建立了基于检测的跟踪基准，取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 智能交通系统中的多目标跟踪主要依赖帧式相机，但在弱光和高速度条件下表现不佳。事件相机具有低延迟、高动态范围和高时间分辨率的特点，有潜力解决这些问题。然而，与帧式视觉相比，事件视觉研究较少，存在研究空白。

Method: 1. 创建了首个专为事件智能交通系统设计的试点数据集，涵盖车辆和行人检测与跟踪；2. 基于该数据集建立了基于检测的跟踪基准；3. 开发了专门的特征提取器。

Result: 在该数据集上建立的跟踪基准取得了优异的性能表现。

Conclusion: 事件相机在智能交通系统中具有巨大潜力，特别是在弱光和高速度条件下。本文通过创建专门的数据集和基准，为事件视觉在智能交通系统中的应用研究提供了重要基础。
...
TL;DR: WorldPlay是一个流式视频扩散模型，通过双动作表示、重构上下文记忆和上下文强制蒸馏三大创新，实现了实时交互式世界建模，解决了现有方法在速度与内存之间的权衡问题。
...
Method: 1) 双动作表示：支持对用户键盘鼠标输入的鲁棒动作控制；2) 重构上下文记忆：动态重建过去帧的上下文，通过时间重帧保持几何重要但久远帧的可访问性，缓解内存衰减；3) 上下文强制蒸馏：为内存感知模型设计的蒸馏方法，通过对齐师生模型的记忆上下文，保持学生模型使用长程信息的能力。

Result: WorldPlay能够以24 FPS的速度生成长序列720p流式视频，具有优异的几何一致性。与现有技术相比表现更佳，并在多样化场景中展现出强大的泛化能力。

Conclusion: WorldPlay通过创新的架构设计解决了流式视频生成中的速度-内存权衡问题，实现了实时交互式世界建模，为高质量长序列视频生成提供了有效解决方案。
...
TL;DR: SFVD提出单帧视频集蒸馏方法，通过将视频蒸馏为每类最具信息量的单帧，再通过可微分插值转换为视频序列，显著提升视频数据集蒸馏性能
...
Method: 提出单帧视频集蒸馏(SFVD)框架：1)将视频蒸馏为每类最具信息量的单帧；2)通过可微分插值将单帧转换为视频序列并与原始数据集匹配；3)更新仅限于单帧以提高优化效率；4)通过通道重塑层将蒸馏帧与真实视频采样结合以融入时间信息

Result: 在多个基准测试中，SFVD显著优于现有方法，在MiniUCF上实现了高达5.3%的性能提升

Conclusion: SFVD通过将视频蒸馏为单帧的方法有效解决了视频数据集蒸馏中的优化挑战，提供了一种更高效的视频数据集蒸馏解决方案
...
TL;DR: AMD-HookNet++提出了一种新颖的CNN-Transformer混合特征增强方法，用于合成孔径雷达图像中的冰川分割和崩解前沿检测，解决了纯CNN方法长距离依赖不足和纯Transformer方法边缘锯齿的问题。


<details>
  <summary>Details</summary>
Motivation: 冰川和冰架前沿的动态变化对冰盖物质平衡和沿海海平面有重要影响。虽然AMD-HookNet首次引入了纯双分支CNN进行冰川分割，但卷积操作的局部性和平移不变性限制了模型保持长距离依赖的能力，需要改进。

Method: 提出AMD-HookNet++混合CNN-Transformer架构：1) Transformer分支捕获长距离依赖和全局上下文信息；2) CNN分支保留局部细节；3) 增强的空间通道注意力模块动态调整空间和通道维度的token关系；4) 像素到像素对比深度监督，将像素级度量学习整合到冰川分割中。

Result: 在CaFFe基准数据集上，AMD-HookNet++实现了78.2的IoU和1,318米的HD95，同时保持367米的竞争性MDE，创造了新的最先进水平。更重要的是，混合模型产生了更平滑的崩解前沿轮廓，解决了纯Transformer方法常见的锯齿边缘问题。

Conclusion: AMD-HookNet++通过CNN-Transformer混合架构有效结合了全局上下文信息和局部细节，显著提升了冰川分割和崩解前沿检测的性能，为冰川监测提供了更准确、更平滑的解决方案。
...
Method: 提出ViRC框架，引入Reason Chunking机制将多模态数学CoT结构化为连续的关键推理单元（CRUs），模拟人类专家解题模式。CRUs确保单元内文本连贯性以验证中间命题，同时整合跨单元视觉信息生成后续命题。创建CRUX数据集，使用三种视觉工具和四种推理模式为每个数学问题提供显式标注的CRUs。提出渐进式训练策略：指导性SFT、实践性SFT和战略性RL。

Result: ViRC-7B模型在多个数学基准上平均比基线提升18.8%。

Conclusion: ViRC框架通过模拟人类认知过程的结构化推理方法，显著提升了多模态数学推理能力，为MLLMs在复杂数学任务中的表现提供了有效解决方案。
...
TL;DR: FiNERweb是一个多语言命名实体识别数据集创建流程，通过教师-学生范式在91种语言和25种文字上扩展，包含约22.5万段落和23.5万实体标签，使用回归模型识别NER相关段落并用多语言LLM标注。
...
Method: 基于FineWeb-Edu，训练回归模型识别NER相关段落，然后用多语言LLM进行标注。流程覆盖91种语言和25种文字，包含英语标签和翻译后的目标语言标签集。

Result: 回归模型F1分数超过84；在FiNERweb上训练的模型在英语、泰语和斯瓦希里语的零样本迁移中性能相当或更好，尽管数据量比基线少19倍。标注质量评估显示忠实度3.99/5，完整性4.05/5。

Conclusion: FiNERweb提供了高质量、可复用的多语言NER数据集，能够有效支持教师-学生训练。观察到当前SOTA模型在使用目标语言标签而非英语标签时性能下降0.02-0.09 F1，因此同时发布英语和翻译标签集。
...
TL;DR: Olmo 3是一个包含7B和32B参数规模的全开放语言模型家族，专注于长上下文推理、函数调用、编码、指令跟随、通用对话和知识回忆，提供完整的模型构建流程和所有中间检查点。


<details>
  <summary>Details</summary>
Motivation: 构建完全开放、透明的语言模型，提供从数据到最终模型的完整生命周期，解决现有开源模型在透明度、可复现性和功能覆盖方面的不足。

Method: 开发了7B和32B参数规模的语言模型家族，专注于多个核心能力：长上下文推理、函数调用、编码、指令跟随、通用对话和知识回忆。提供了完整的模型构建流程，包括所有训练阶段、检查点、数据点和依赖项。

Result: 发布了Olmo 3模型家族，其中旗舰模型Olmo 3 Think 32B是目前最强的全开放思维模型，在多个任务上达到最先进水平。

Conclusion: Olmo 3代表了完全开放语言模型的重要进展，通过提供完整的模型生命周期实现了前所未有的透明度，为研究社区提供了有价值的资源。
...
Method: 提出结构感知解码方法，包括：1）使用预训练语言模型获取上下文语义表示；2）通过候选表示组合捕获多粒度实体跨度特征；3）引入层次结构约束确保语义与结构一致性；4）联合优化分类损失和结构一致性损失增强稳定性。

Result: 在ACE 2005数据集上的实验表明，该方法在准确率、精确率、召回率和F1分数上均有显著提升，特别是在嵌套和重叠实体识别中表现出更强的边界定位和结构建模能力。

Conclusion: 结构感知解码在复杂语义提取任务中具有有效性，为开发具有层次理解能力的语言模型提供了新视角，为高精度信息提取建立了方法论基础。
...
TL;DR: 研究发现大型语言模型在不同规模、训练方式和任务难度下都存在层利用率不足的问题，有效深度比例保持稳定，而非随模型加深而增加。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型深度不断增加，性能增益却逐渐减少。先前研究提出了"有效深度"概念，认为深层模型未能充分利用其层进行有意义的计算。本研究旨在系统探讨有效深度如何随模型规模、训练类型和任务难度变化。

Method: 系统分析Qwen-2.5系列模型（1.5B-32B）的行为，比较基础模型与对应的长链思维模型，评估不同难度任务下的表现，研究有效深度在不同条件下的变化规律。

Result: 1. 有效层数随模型规模增加而增长，但有效深度比例保持稳定；2. 基础模型与长链思维模型在有效深度上没有差异，推理能力提升源于更长上下文而非更深层计算；3. 模型不会针对更难问题动态使用更多层。

Conclusion: 当前LLM在不同规模、训练范式和任务难度下都存在深度利用不足的问题，这为提升层利用率、模型剪枝和早期退出等研究方向提供了机会。
...
TL;DR: 本文研究如何将预训练的自回归语言模型转换为扩散语言模型，通过改进注意力模式和训练策略，在保持准确性的同时提升生成效率。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型支持并行生成，但从头训练效率低于自回归模型。为了结合两者的优势，需要将预训练的自回归模型有效转换为扩散模型，在保持任务准确性的同时提升生成速度。

Method: 1. 提出块状注意力模式：保持块间因果性，块内双向建模，以更好地保留预训练权重分布并支持KV缓存；2. 提出位置依赖的掩码策略：训练时对后续token赋予更高掩码概率，以缩小训练-测试差距；3. 系统研究扩散模型的注意力模式、训练动态等设计选择。

Result: 提出的Efficient-DLM系列模型在准确性和效率上均优于现有方法。例如，Efficient-DLM 8B相比Dream 7B和Qwen3 4B，准确率分别提升5.4%和2.7%，吞吐量分别提升4.5倍和2.7倍。

Conclusion: 通过改进注意力模式和训练策略，可以有效将预训练自回归模型转换为高效的扩散语言模型，实现准确性和生成速度的双赢，为可扩展的AR-to-dLM转换提供了实用见解。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在处理长上下文时面临自注意力机制计算复杂度随序列长度呈二次方增长的问题。现有稀疏注意力方法存在局限性：基于训练的方法成本高且不能作为加速插件直接应用于其他模型，而推理时方法往往在效率或多模态通用性上有所妥协。

Method: 提出UniSparse统一机制，引入复合令牌的概念——聚合多粒度上下文信息的紧凑表示。基于此抽象，通过多粒度压缩和块级选择动态构建稀疏注意力，实现GPU上高效且硬件友好的执行。

Result: 在从合成基准到实际应用的多模态任务中，UniSparse在准确性和效率上均优于最先进的稀疏注意力方法（如MInference、XAttention、FlexPrefill），达到≥99%的全注意力精度，注意力计算速度比FlashAttention快达2.61倍。

Conclusion: UniSparse提供了一种有效的解决方案，解决了长上下文处理中的计算瓶颈问题，在保持高精度的同时显著提升了计算效率，具有广泛的应用前景。
...
Method: 基于Transformer的帧级模型，使用约300小时的双人对话数据进行多任务联合训练，支持日语、英语和汉语

Result: 多语言模型在所有三种语言上都达到或超过单语言基线；发现日语更依赖短期语言信息，英语和汉语对沉默时长和韵律变化更敏感；零样本迁移有限，显示实质性跨语言差异

Conclusion: 多语言模型能学习语言通用线索和语言特定时序模式，为理解跨语言反馈时机差异提供了统一模型和实证证据，有助于设计更自然的语音对话系统
...
TL;DR: CogMem是一个受认知启发的记忆增强LLM架构，通过结构化持久记忆支持持续迭代推理，解决多轮交互中的推理失败问题。
...
Conclusion: CogMem通过结构化记忆架构使LLM能够进行更可靠、更类似人类的推理，朝着持续迭代推理的方向迈进。
...
<details>
  <summary>Details</summary>
Motivation: 标准语言到区域方言的翻译是NLP的重要挑战，特别是在孟加拉语中，由于数据稀缺和语言变异性。需要有效的解决方案来处理低资源方言翻译问题。

Method: 提出了两种RAG管道：1) 基于转录本的管道，使用音频转录中的大段方言上下文；2) 基于标准化句对的管道，使用结构化的local_dialect:standard_bengali句对。在六种孟加拉方言和多种LLM上使用BLEU、ChrF、WER和BERTScore进行评估。

Result: 句对管道始终优于转录本管道，将吉大港方言的词错误率从76%降低到55%。关键发现是，精心设计的检索策略比模型规模更重要，小模型(Llama-3.1-8B)可以超越大模型(GPT-OSS-120B)的性能。
...
TL;DR: 本文介绍了两种用于扩展纳瓦特尔语语料库的上下文无关文法，以解决这种墨西哥土著语言数字资源匮乏的问题，通过生成大量语法正确的句子来训练词嵌入模型。


<details>
  <summary>Details</summary>
Motivation: 纳瓦特尔语作为墨西哥的国家语言之一，属于数字资源稀少的π语言类型，缺乏可用于训练大型语言模型的语料库，这给自然语言处理研究带来了重大挑战。

Method: 提出了两种新的纳瓦特尔语上下文无关文法，并采用生成模式来产生大量语法正确的纳瓦特尔语句子，从而扩展语料库规模。

Result: 使用扩展后的语料库训练词嵌入模型，在句子语义相似性任务中表现优于仅使用原始语料库的结果，并且经济型嵌入模型在某些情况下表现优于一些大型语言模型。

Conclusion: 通过上下文无关文法扩展语料库是解决资源稀缺语言数字资源问题的有效方法，能够显著改善词嵌入模型的性能，为纳瓦特尔语的自然语言处理研究提供了可行路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在处理长上下文时面临高计算成本和噪声问题，现有压缩技术要么破坏局部连贯性，要么存在位置偏差且不兼容闭源API。

Method: 采用"结构-选择"两阶段方法：1) LingoEDU将线性文本转换为基于基本语篇单元的结构关系树；2) 轻量级排序模块选择查询相关子树进行线性化。

Result: 方法在结构预测准确性上达到SOTA，显著优于前沿LLM同时降低成本，在长上下文任务和复杂深度搜索场景中性能显著提升。

Conclusion: EDU-based Context Compressor通过显式结构表示有效解决了长上下文压缩问题，在保持信息完整性的同时大幅提升效率。
...
TL;DR: 该研究评估了GPT-3.5-turbo基于宏观经济价格信号形成通胀感知和预期的能力，通过与英国家庭调查数据和官方统计对比，发现LLM在短期预测上与调查数据一致，但在通胀理解上缺乏一致性模型。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估大型语言模型（LLMs）在社会科学应用中的潜力，特别是它们能否基于宏观经济信息形成类似人类的通胀感知和预期，这对于LLMs在经济学和社会科学研究中的应用具有重要意义。

Method: 采用准实验设计，利用GPT-3.5-turbo的训练截止时间（2021年9月）作为自然实验，使其不了解后续的英国通胀激增。通过模拟英国央行通胀态度调查（IAS）的信息集和人口特征，将LLM输出与家庭调查数据和官方统计进行比较。使用新颖的Shapley值分解方法来分析提示内容对模型输出的驱动因素。

Result: 研究发现：1）GPT在短期预测上与总体调查预测和官方统计数据一致；2）在细分层面，GPT复制了家庭通胀感知的关键经验规律，特别是收入、住房所有权和社会阶层方面；3）GPT对食品通胀信息表现出类似人类受访者的高度敏感性；4）但GPT缺乏一致的消费者价格通胀模型。

Conclusion: 该研究表明LLMs能够在一定程度上模拟人类对通胀的感知和预期，但存在局限性。研究方法可用于评估LLMs在社会科学中的行为、比较不同模型或辅助调查设计，为LLMs在经济学研究中的应用提供了方法论基础。
...
TL;DR: 研究探讨了文档打包策略对大型语言模型多跳推理能力的影响，发现打包训练能提升性能但增加计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前训练大型语言模型时通常将多个文档打包以提高计算效率，但这一过程对模型能力的影响尚未得到充分研究，特别是对潜在多跳推理能力的影响。

Method: 通过研究不同文档打包策略对LLMs多跳推理能力的影响，并进行消融实验以识别关键因素。

Result: 研究发现打包训练相比单个文档训练能提升模型性能，但需要更多计算资源；消融实验揭示了打包优势的关键因素。

Conclusion: 该研究深化了对LLM训练动态的理解，为优化模型开发提供了实用见解。
...
TL;DR: SASQ是一个轻量级的量化感知训练框架，专门针对激活量化因子进行优化，通过仅优化量化因子而不改变预训练权重，在保持部署效率的同时实现高精度静态推理。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型规模增长快于GPU内存发展，量化部署面临挑战：动态量化计算开销大且边缘设备部署困难，静态量化牺牲精度，现有量化感知训练方法权重训练成本高。
...
Conclusion: SASQ通过轻量级量化感知训练框架，在保持部署效率的同时实现了高精度静态推理，解决了现有量化方法在精度和效率之间的权衡问题。
...
TL;DR: 提出C-ing Clearly方法，利用C代码生成合成数据来增强大语言模型对汇编语言的理解，在二进制代码摘要和漏洞检测任务上取得性能提升


<details>
  <summary>Details</summary>
Motivation: 大语言模型通常在高级编程语言任务上表现优异，但在汇编等低级语言任务上表现不佳，需要提升模型对汇编代码的理解能力

Method: 提出C-ing Clearly合成数据生成方法，利用对应的C代码来增强LLM对汇编的理解，通过在该方法生成的数据上进行微调来提升模型性能

Result: 该方法在不同LLM家族和模型规模上都取得了持续的性能提升，特别是在二进制代码摘要和漏洞检测任务上表现出改进

Conclusion: 通过利用C代码生成合成数据来增强LLM对汇编的理解是有效的，该方法能够显著提升模型在低级编程语言任务上的性能
...
TL;DR: 作者认为Futrell和Mahowald提出的连接深度学习系统与语言学理论的框架很有用，但该框架过于关注基于文本的生成式大语言模型，限制了与语言学的有效互动，因为许多关于人类语言的有趣问题超出了书面文本的范畴。作者主张音频基础的深度学习模型应该发挥关键作用。


<details>
  <summary>Details</summary>
Motivation: 作者认为当前连接深度学习与语言学的框架存在局限性，因为它主要关注文本基础的生成式大语言模型，而忽略了人类语言中许多重要的非文本特征。音频包含丰富的语言信息（如韵律、语调、情感等），这些在书面文本中无法体现，但对理解人类语言至关重要。

Method: 作者通过批判性分析现有框架的局限性，提出应该将音频基础的深度学习模型纳入语言学研究的核心。虽然没有详细描述具体的技术方法，但强调了音频模型在捕捉语言非文本特征方面的潜力。

Result: 作者论证了音频基础深度学习模型在语言学研究中不可或缺的作用，指出当前基于文本的框架无法全面捕捉人类语言的丰富性，特别是口语中的韵律、语调、情感表达等特征。

Conclusion: 为了更全面地理解人类语言，语言学与深度学习的交叉研究必须超越文本基础的大语言模型，积极整合音频基础的深度学习模型，这样才能捕捉到语言中那些在书面文本中缺失但至关重要的特征。
...
Method: 采用双目标训练方法，结合自回归和掩码扩散目标，训练50个语言模型在不同数据重复水平下，寻找最优目标比例
...
TL;DR: 越南首个综合性法律基准VLegal-Bench，包含10,450个样本，基于布鲁姆认知分类法设计，用于系统评估大语言模型在越南法律任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 越南法律的复杂性、层级结构和频繁修订给评估大语言模型的法律理解和应用能力带来挑战，需要专门的评估基准。

Method: 基于布鲁姆认知分类法设计多层次法律理解任务，通过严格的标注流程生成10,450个样本，由法律专家标注和交叉验证，确保样本基于权威法律文件并反映真实法律助理工作流程。
...
TL;DR: PolyPersona是一个生成框架，通过指令调优小型语言模型来合成基于人物角色的多领域调查响应，使用参数高效的LoRA适配器和4位量化，构建了包含3,568个响应的数据集，并在多指标评估中取得了良好效果。


<details>
  <summary>Details</summary>
Motivation: 需要一种高效、可复现的方法来生成基于人物角色的合成调查数据，以支持可扩展的评估和偏差分析，同时解决大型模型资源消耗大的问题。

Method: 使用参数高效的LoRA适配器配合4位量化对紧凑聊天模型进行指令调优，采用对话式数据管道明确保留人物角色线索，构建了包含10个领域、433个不同人物角色的3,568个合成调查响应数据集。

Result: TinyLlama 1.1B和Phi-2等小型模型在BLEU（最高0.090）和ROUGE-1（0.429）等指标上表现与7B-8B基线模型相当，能够生成可靠、连贯的合成调查数据。

Conclusion: 基于人物角色的微调使小型语言模型能够生成可靠的合成调查数据，该框架为调查数据生成提供了高效、可复现的方法，支持可扩展评估和透明偏差分析。
...
TL;DR: JMMMU-Pro是一个基于图像的日本多学科多模态理解基准测试，通过将问题图像和文本组合成单一图像，创建需要视觉感知的集成视觉-文本理解基准。Vibe Benchmark Construction是一种可扩展的构建方法，利用图像生成模型生成候选视觉问题，人工验证并调整提示以确保质量。


<details>
  <summary>Details</summary>
Motivation: 为了更严格地评估语言模型在日语环境下的多模态理解能力，需要创建一个能够测试集成视觉-文本理解的基准测试。现有基准测试可能无法充分评估模型在复杂视觉场景中的日语理解能力。

Method: 提出Vibe Benchmark Construction方法：使用图像生成模型（如Nano Banana Pro）生成候选视觉问题，人工验证输出质量，必要时调整提示重新生成。通过将问题图像和文本组合成单一图像，创建需要视觉感知的集成理解任务。

Result: 构建了高质量的JMMMU-Pro基准测试，涵盖广泛的背景和布局设计。实验结果显示，所有开源语言模型在JMMMU-Pro上都表现不佳，表明该基准测试具有挑战性。

Conclusion: JMMMU-Pro为评估语言模型的日语多模态能力提供了更严格的工具，Vibe Benchmark Construction方法为未来图像型视觉问答基准测试的开发提供了高效指南。
...
TL;DR: TiME模型通过蒸馏等技术训练小型单语编码器，在保持性能的同时显著提升效率、降低能耗，适用于资源受限场景。


<details>
  <summary>Details</summary>
Motivation: 当前NLP研究过度关注大型通用模型，但实际应用中许多场景只需要特定小规模能力。大型模型在处理大量数据或实时响应时速度不足，能耗过高，不利于可持续发展和电池设备部署。

Method: 使用蒸馏等现代训练技术，从多语言教师模型蒸馏出单语模型，从相对位置编码教师模型蒸馏出绝对位置编码模型，训练小型单语编码器（TiME）。

Result: 在多个常见NLP任务上评估，TiME模型在基准性能与吞吐量、延迟和能耗之间取得了更好的平衡，支持低资源语言。

Conclusion: 通过蒸馏技术训练小型单语模型是可行的，能够为效率关键型应用提供更优的解决方案，平衡性能与资源消耗。
...
TL;DR: 本文提出Jacobi Forcing方法，通过渐进蒸馏将自回归模型转化为高效的并行解码器，在保持生成质量的同时实现3.8-4.0倍的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散大语言模型(dLLMs)的并行解码方法存在两个主要问题：1)后训练中的掩码数据分布与预训练时的真实数据分布不匹配；2)双向注意力机制与预训练学习的因果先验冲突，阻碍了精确KV缓存的复用。这导致加速效果有限且生成质量下降。

Method: 提出Jacobi Forcing渐进蒸馏范式：让模型在自己生成的并行解码轨迹上进行训练，平滑地将自回归模型转变为高效的并行解码器，同时保留其预训练的因果推理特性。基于Jacobi Forcing模型的轨迹特性，进一步引入带拒绝回收的多块解码，提高每次迭代的令牌接受数量。

Result: Jacobi Forcing模型在代码和数学基准测试上实现了3.8倍的实时加速，且性能损失最小。结合多块解码和拒绝回收技术，实现了每次迭代高达4.5倍的令牌接受数量提升和接近4.0倍的实时加速。

Conclusion: Jacobi Forcing通过渐进蒸馏有效解决了预训练-后训练不匹配问题，成功将自回归模型转化为高效的并行解码器，在保持生成质量的同时显著提升推理速度，为大规模模型推理加速提供了新范式。
...
TL;DR: Spoken DialogSum是首个将原始对话音频与事实摘要、情感摘要以及说话人年龄、性别、情感等副语言线索对齐的数据集，包含13,460个情感多样的对话，通过两阶段构建方法实现，实验表明端到端音频语言模型在情感摘要任务上比级联ASR-LLM系统提升28%的ROUGE-L分数。


<details>
  <summary>Details</summary>
Motivation: 当前音频语言模型能够处理长对话，但情感感知或口语对话摘要研究受到缺乏将语音、摘要和副语言线索关联的数据集的限制。现有数据集未能同时包含原始音频、摘要和副语言标注。

Method: 数据集构建分为两个阶段：1) 使用LLM重写DialogSum脚本，添加Switchboard风格的填充词和反馈词，并为每个话语标注情感、音高和语速；2) 使用富有表现力的TTS引擎从标注脚本合成语音，并与副语言标签对齐。最终得到包含13,460个对话的数据集，每个对话都配有事实摘要和情感摘要。

Result: 创建了Spoken DialogSum数据集，包含13,460个情感多样的对话，每个对话都有原始音频、事实摘要、情感摘要以及说话人年龄、性别、情感等副语言标注。基线实验显示，Audio-LLM在情感摘要任务上的ROUGE-L分数比级联ASR-LLM系统相对提升28%，证明了端到端语音建模的价值。

Conclusion: Spoken DialogSum填补了情感感知口语对话摘要领域的数据空白，为研究语音、摘要和副语言线索的关联提供了重要资源。实验结果证实了端到端音频建模在情感摘要任务上的优势，为未来相关研究奠定了基础。
...
TL;DR: MMGR是一个评估视频生成模型推理能力的新框架，包含物理、逻辑、空间和时间五个推理维度，在抽象推理、具身导航和物理常识三个领域测试模型，发现当前模型在抽象推理和长期空间规划方面表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型的评估指标（如FVD）过于关注感知质量，忽视了模型在因果关系、物理规律和全局一致性方面的推理失败。需要建立一个能够全面评估生成模型推理能力的框架。

Method: 提出了MMGR评估框架，基于五个推理能力维度：物理推理、逻辑推理、3D空间推理、2D空间推理和时间推理。在三个领域进行评估：抽象推理（ARC-AGI、数独）、具身导航（真实世界3D导航和定位）和物理常识（运动和组合交互）。使用细粒度指标要求视频和图像生成的整体正确性。
...
TL;DR: 开发了一个包含5.4万篇孟加拉语文章和摘要的多领域数据集，用于改进孟加拉语抽象摘要生成，填补了现有研究主要集中于新闻领域的空白。


<details>
  <summary>Details</summary>
Motivation: 现有孟加拉语摘要研究主要集中于新闻文章，而新闻写作风格相对固定，难以适应博客、社交媒体等多样化真实文本。数字时代孟加拉语内容激增，需要能够减少信息过载、帮助读者快速理解内容的摘要系统。

Method: 从Cinegolpo博客、Samakal和The Business Standard报纸等多个来源收集了超过5.4万篇孟加拉语文章和摘要，构建了跨多个领域和写作风格的数据集。使用LSTM、BanglaT5-small和MTS-small等深度学习和迁移学习模型进行训练和评估。

Result: 建立了强大的基线模型，展示了该数据集作为孟加拉语自然语言处理未来研究基准的潜力。数据集提供了更好的适应性和实际相关性。
...
<details>
  <summary>Details</summary>
Motivation: 手工病历审查是临床研究中极其耗时且资源密集的环节，需要专家从非结构化的电子健康记录中提取复杂信息，亟需自动化解决方案来减轻负担

Method: 开发了一个安全、模块化的框架，利用本地部署的大语言模型，结合检索增强生成和结构化响应方法，在符合HIPAA标准的计算基础设施上实现临床特征自动提取

Result: 该框架在多个医疗特征提取任务中达到高准确率，与专家标注数据集相比表现优异，甚至发现了手工审查中遗漏的标注错误

Conclusion: 该框架展示了LLM系统通过自动化特征提取减少手工病历审查负担的潜力，能够提高数据捕获的一致性，加速临床研究进程
...
<details>
  <summary>Details</summary>
Motivation: 传统无线电地图构建方法需要大量位置标记数据，成本高昂且在实际场景中不实用。本文旨在开发一种无需位置标签的盲构建方法，解决实际部署中的成本和技术难题。

Method: 首先证明了在准镜面环境模型下，非视距信道状态信息具有空间连续性，推导出与物理距离成正比的CSI距离度量。针对泊松分布AP部署中的直线轨迹，证明了即使角度分辨率较差，定位误差的克拉美罗下界也能渐近消失。基于这些理论结果，开发了空间正则化贝叶斯推理框架，联合估计信道特征、区分视距/非视距条件并恢复用户轨迹。

Result: 在射线追踪数据集上的实验表明，该方法实现了0.68米的平均定位误差和3.3%的波束图重建误差，验证了所提盲映射方法的有效性。

Conclusion: 提出的盲无线电地图构建框架能够在不依赖位置标签的情况下，通过室内MIMO-OFDM信道测量准确推断用户轨迹并重建波束图，为智能无线应用提供了实用且成本效益高的解决方案。
...
TL;DR: Adjudicator系统通过神经符号方法构建知识图谱，结合多智能体LLM架构来识别和纠正标签噪声，在AlleNoise基准测试上达到0.99 F1分数，显著优于基线方法。
...
Method: 采用神经符号方法：1) 构建动态知识图谱统一项目上下文；2) 设计"智能体委员会"多智能体LLM架构，让专门智能体辩论并投票标签有效性；3) 使用基于知识图谱的覆盖逻辑识别复杂结构错误。

Result: 在AlleNoise基准测试的1000项平衡子集上，知识图谱增强模型达到0.99 F1分数，显著优于单LLM基线(0.48 F1)和无知识图谱委员会(0.59 F1)。系统能完美识别复杂结构错误(完全召回率)，而基线方法无法发现这类错误。

Conclusion: Adjudicator提供了一个稳健且可解释的自动高精度数据验证系统，为严格监管的工业环境中生成高质量数据集提供了重要概念验证。
...
### [110] [AI-Powered Annotation Pipelines for Stabilizing Large Language Models: A Human-AI Synergy Approach](https://arxiv.org/abs/2512.13714)
*Gangesh Pathak,Prasanna Kumar*

Main category: cs.AI

TL;DR: 本文提出了一种AI驱动的标注流程，用于系统识别、标记和修复LLM输出的不稳定模式，通过人机协同方法结合自动弱监督和基于置信度的标注，旨在解决LLM在高度监管行业中因不稳定、幻觉和性能波动导致的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: LLM在高度监管行业中的应用受到限制，主要因为其存在不稳定问题、不一致推理、幻觉和性能波动，特别是在工作流程中。这些可靠性问题阻碍了LLM在需要事实精确性和一致行为的领域的安全使用。现有的稳定化方法如RLHF和监督微调虽然能带来可量化的改进，但成本高昂且依赖大量人工标注，难以可持续地扩展。

Method: 提出了一种AI驱动的标注流程，采用人机协同方法，结合自动弱监督模型和基于置信度的标注，辅以目标人工验证。该框架引入了语义一致性、事实正确性和逻辑连贯性等稳定性特定标注类别，通过反馈循环实现模型的持续校准和鲁棒性增强。

Result: 该方法能够系统性地识别、标记和修复LLM输出的不稳定模式，通过人机协同确保反馈信息的可靠性和道德正确性。框架支持基于反馈循环的持续模型校准，从而增强LLM的鲁棒性。

Conclusion: 提出的AI驱动标注流程为人机协同解决LLM稳定性问题提供了一种可持续的解决方案，通过结合自动标注和人工验证，能够在保证可靠性的同时实现可扩展性，为LLM在高度监管行业中的安全应用提供了技术支持。
...
### [111] [Compressed Causal Reasoning: Quantization and GraphRAG Effects on Interventional and Counterfactual Accuracy](https://arxiv.org/abs/2512.13725)
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型部署向边缘和资源受限环境转移，量化模型（如INT8和NF4）成为标准，但精度降低对形式化因果推理的影响尚不清楚。这是首个系统评估量化在Pearl因果阶梯所有三个层次上影响的研究。

Method: 使用3000个样本的分层CLadder基准，评估Llama 3 8B在量化下的因果推理能力。在CRASS基准上进行实验，评估图检索增强生成（Graph RAG）使用真实因果图的效果。

Result: 量化对因果推理准确率影响有限：NF4量化总体退化小于1%。干预查询（阶梯2）对精度损失最敏感，反事实推理（阶梯3）相对稳定但在特定查询类型（如碰撞偏倚和后门调整）存在异质性弱点。CRASS基准上各精度表现几乎相同，表明现有常识反事实数据集缺乏揭示量化诱导推理漂移的结构敏感性。图RAG使NF4干预准确率提升1.7%，部分抵消压缩相关退化。

Conclusion: 因果推理对4位量化表现出意外的鲁棒性，图结构化增强可以有针对性地强化干预推理，当前反事实基准未能捕捉更深层次的因果脆弱性。该研究为压缩因果推理提供了初步经验地图，并为部署高效且结构支持的因果AI系统提供了实用指导。
...
### [112] [State-Dependent Refusal and Learned Incapacity in RLHF-Aligned Language Models](https://arxiv.org/abs/2512.13762)
*TK Lee*

Main category: cs.AI

TL;DR: 该研究提出了一种定性案例研究方法，用于审计大语言模型在长时程交互中的策略相关行为选择性。研究发现同一模型在非敏感领域表现正常，但在敏感领域会反复产生功能性拒绝，表现出行为不对称性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型被广泛部署为通用工具，但扩展交互可能揭示标准定量基准无法捕捉的行为模式。需要开发定性方法来审计模型在策略敏感领域的行为选择性。

Method: 采用定性案例研究方法，在单个86轮对话会话中分析模型行为。定义了三种响应机制：正常表现(NP)、功能性拒绝(FR)和元叙事(MN)。通过长时程交互观察模型在不同领域的行为模式。

Result: 研究发现同一模型在广泛非敏感领域表现正常，但在提供者或策略敏感领域会反复产生功能性拒绝，表现出NP和FR之间的持续不对称性。元叙事角色框架叙述往往与敏感情境中的拒绝同时出现。

Conclusion: 研究提出了基于可观察行为的交互级审计框架，并引入"习得性无能"作为描述选择性保留行为的概念，为检查潜在对齐副作用提供了视角，值得在不同用户和模型中进行进一步调查。
...
### [113] [Mathematics and Coding are Universal AI Benchmarks](https://arxiv.org/abs/2512.13764)
*Przemyslaw Chojecki*

Main category: cs.AI

TL;DR: 该论文研究了数学和编程在AI智能体心理测量电池模空间中的特殊作用，证明了数学定理证明和编程任务生成的电池子空间在评估度量下是稠密的，并发现形式数学是高级AI智能体递归自我改进的自然点火域。


<details>
  <summary>Details</summary>
Motivation: 研究数学和编程在AI智能体心理测量电池模空间中的特殊作用，探索形式数学作为递归自我改进的自然点火域的潜力。

Method: 基于AAI框架和GVU动力学，定义数学纤维，与形式证明内核（如Lean、Coq）配对，分析GVU流在数学纤维上的谱稳定自我改进机制，证明在智能体输出均匀紧致和AAI泛函Lipschitz条件下，数学定理证明和编程任务生成的电池子空间在评估度量下是稠密的。

Result: 主要技术结果是密度定理：编程单独具有普遍性，而纯数学不具有表达普遍性，但其特权是谱性质的而非表达性质的。数学和编程为评估提供了"通用坐标"，形式数学是高级AI智能体递归自我改进的自然点火域。

Conclusion: 数学和编程在AI智能体评估中具有特殊地位，形式数学由于其谱稳定性和类神谕验证特性，为高级AI智能体的递归自我改进提供了自然的起点和机制。
...
### [114] [Semantic Grounding Index: Geometric Bounds on Context Engagement in RAG Systems](https://arxiv.org/abs/2512.13771)
*Javier Marín*

Main category: cs.AI

TL;DR: 论文提出语义接地指数(SGI)，通过计算回答与问题vs回答与上下文的角度距离比，来检测RAG系统的幻觉。研究发现幻觉回答在嵌入空间中更接近问题而非上下文，这种现象称为"语义懒惰"。
...
Method: 引入语义接地指数(SGI)，定义为在单位超球面上，回答到问题的角度距离与回答到上下文的角度距离之比。基于球面三角不等式推导理论预测，并在HaluEval数据集(n=5,000)上使用五种嵌入模型进行验证。

Result: 发现"语义懒惰"现象：幻觉回答在角度上更接近问题而非上下文。效应大小(Cohen's d)在0.92-1.28之间，跨模型相关性r=0.85。SGI的判别能力随问题-上下文角度分离θ(q,c)增加而增强，效应大小从d=0.61(低θ)到d=1.27(高θ)，AUC从0.72提升到0.83。在长回答(d=2.05)和短问题(d=1.22)上表现优异。

Conclusion: SGI提供了一种计算高效、理论可靠的基础设施，用于识别生产RAG部署中需要验证的响应。但需注意，角度几何测量的是主题参与度而非事实准确性，在TruthfulQA上的AUC=0.478证实了这一点。
...
### [115] [MURIM: Multidimensional Reputation-based Incentive Mechanism for Federated Learning](https://arxiv.org/abs/2512.13955)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然保护隐私，但仍面临客户端激励不足、隐私风险、资源限制等挑战。评估客户端可靠性对于公平分配激励和确保每个客户端数据对全局模型有实质性贡献至关重要。

Method: 提出MURIM（多维信誉激励机制），综合考虑客户端可靠性、隐私、资源容量和公平性，基于客户端贡献、延迟和信誉分配激励，并配备可靠性验证模块防止恶意或不靠谱客户端获得不当奖励。

Result: 在MNIST、FMNIST和ADULT Income数据集上的实验表明，MURIM相比现有基线方法：公平性指标提升达18%，隐私攻击成功率降低5-9%，对投毒和噪声梯度攻击的鲁棒性提升达85%。

Conclusion: MURIM能有效缓解对抗性威胁，促进公平真实的参与，在异构和动态的联邦学习环境中保持稳定的模型收敛，为联邦学习激励机制提供了全面解决方案。
...
### [116] [Evaluating Frontier LLMs on PhD-Level Mathematical Reasoning: A Benchmark on a Textbook in Theoretical Computer Science about Randomized Algorithms](https://arxiv.org/abs/2512.13978)
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在数学推理和科学发现方面取得显著进展，但仍需对其在经典研究生级数学理论上的基础推理能力进行严格评估，以了解模型的实际数学能力水平。

Method: 使用四种前沿大语言模型（GPT-5-Thinking、Gemini-3-Pro、Claude-Sonnet-4.5-Thinking、Grok-4），要求它们为《随机算法》教材中的一系列引理和练习生成正式的LaTeX证明，并进行定量和定性分析。

Result: 顶级模型（Gemini和Claude）准确率约66%，表现出对概率方法和形式逻辑的扎实掌握；其他模型一致性较差，准确率约40%。定性分析显示在简洁性、幻觉率和逻辑结构方面存在差异。

Conclusion: 前沿大语言模型已达到适合研究生级教学辅助和形式化的熟练程度门槛，但在严格数学推导的可靠性方面存在显著差异，仍需进一步改进。
...
### [117] [ReflCtrl: Controlling LLM Reflection via Representation Engineering](https://arxiv.org/abs/2512.13979)
*Ge Yan,Chung-En Sun,Tsui-Wei,Weng*

Main category: cs.AI

TL;DR: ReflCtrl框架通过表示工程控制LLM的自我反思行为，在保持性能的同时减少推理成本，发现反思与模型内部不确定性高度相关。


<details>
  <summary>Details</summary>
Motivation: 虽然带CoT推理的LLM通过自我反思提升了性能，但增加了推理成本。本研究旨在通过表示工程理解并控制反思行为，以优化推理效率。
...
Conclusion: ReflCtrl能有效控制LLM的反思行为，揭示反思与模型不确定性的内在联系，为优化推理效率提供了新方法。
...
### [118] [Sparsity-Controllable Dynamic Top-p MoE for Large Foundation Model Pre-training](https://arxiv.org/abs/2512.13996)
...
Method: 提出DTop-p MoE路由机制：1) 使用比例-积分(PI)控制器动态调整概率阈值，使运行中的激活专家稀疏度与指定目标对齐；2) 引入动态路由归一化机制，自适应调整层间路由logits，让不同层学习不同的专家选择模式。
...
Conclusion: DTop-p MoE为大规模MoE预训练提供了一个鲁棒框架，通过动态调整概率阈值和层间路由归一化，实现了对计算资源的精确控制和自适应分配。
...
TL;DR: RoutingGen是一个难度感知的路由框架，针对代码生成任务动态调整提示策略：简单任务使用few-shot提示，复杂任务使用Intention Chain-of-Thought（ICoT）结构化推理策略，在保持高性能的同时显著减少token使用量。
...
Method: 提出RoutingGen难度感知路由框架，根据任务复杂度动态选择提示策略：简单任务采用few-shot提示，复杂任务采用新提出的Intention Chain-of-Thought（ICoT）策略。ICoT引导模型捕获任务意图，包括核心算法逻辑和时间复杂度等抽象概念。

Result: 在三个模型和六个标准代码生成基准测试中，RoutingGen在大多数设置下达到最先进性能，同时平均减少46.37%的总token使用量。ICoT在具有挑战性的基准测试中优于六个现有提示基线方法。

Conclusion: RoutingGen通过难度感知路由和意图导向的ICoT策略，有效解决了现有CoT方法在代码生成中的局限性，实现了性能提升和计算效率的平衡，为代码生成任务提供了更智能的提示框架。
...
TL;DR: OpenDataArena (ODA) 是一个开源平台，用于系统评估和基准测试LLM后训练数据，通过统一训练评估流程、多维度评分框架、数据谱系探索和开源工具包，揭示数据特性与模型性能之间的因果关系。


<details>
  <summary>Details</summary>
Motivation: 当前LLM发展面临一个关键问题：模型有严格的基准测试，但训练数据却是一个黑箱——数据组成不透明、来源不确定、缺乏系统评估。这种不透明性阻碍了可重复性研究，也模糊了数据特性与模型行为之间的因果关系。

Method: ODA平台包含四个核心组件：(1) 统一的训练-评估流程，确保不同模型和领域的公平比较；(2) 多维度评分框架，从数十个维度评估数据质量；(3) 交互式数据谱系探索器，可视化数据集谱系并分析组件来源；(4) 完全开源的工具包，支持训练、评估和评分。

Result: 在ODA上进行了广泛实验：涵盖120多个训练数据集、22个基准测试、600多次训练运行和4000万个处理数据点。分析揭示了数据复杂度与任务性能之间的权衡，通过谱系追踪发现了流行基准中的冗余，并绘制了数据集间的谱系关系图。

Conclusion: ODA不仅扩展了排行榜，更旨在推动从试错式数据策展向数据为中心AI的原则性科学转变。通过发布所有结果、工具和配置，ODA为数据混合规律和基础模型战略组成的研究铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 现代大型语言模型推理成本高、速度慢，传统推测采样方法中草稿模型调用次数是预设超参数，缺乏灵活性

Method: 将草稿树生成过程建模为马尔可夫决策过程，采用离线强化学习训练预测模型，实现实时决策草稿模型调用
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统的自动搜索已成为智能体AI研究的关键焦点。先前方法主要依赖LLM在代码空间进行自由形式搜索，但这种方法缺乏结构性和效率。本文旨在开发更结构化的框架来探索相同的搜索空间。

Method: 提出了一种结构化框架，通过一组固定的简单可组合组件来探索多智能体系统的搜索空间。这种方法虽然缺乏LLM在候选生成阶段的生成灵活性，但提供了更系统化的搜索方式。

Result: 在数学和问答两个领域的五个基准测试中，该方法在四个基准上优于先前方法。此外，该方法还提供了成本效益更高的搜索过程，并能够生成模块化、可解释且逻辑更简单的多智能体系统。

Conclusion: 尽管在候选生成阶段缺乏LLM的生成灵活性，但基于固定可组合组件的结构化框架在多智能体系统自动搜索中表现更优，具有更好的性能、成本效益和可解释性。
...
TL;DR: 本文探索使用大语言模型（LLM）自动地理编码复杂的生物多样性标本采集地点描述，通过QLoRA微调在多个区域和语言数据集上取得优于现有基线方法的效果。


<details>
  <summary>Details</summary>
Motivation: 生物标本采集记录中的地理位置通常以叙述性描述而非坐标形式存在，传统基于地名库或地理指示词的方法难以处理包含空间关系的相对位置描述，而准确的地理编码对生物多样性研究至关重要，但人工处理耗时费力，需要自动化解决方案。

Method: 首先识别有效的提示模式，然后使用量化低秩适应（QLoRA）在多个区域和语言的生物多样性数据集上微调大语言模型，专注于处理复杂的地点描述。

Result: 在固定训练数据量下，该方法在多个数据集上平均65%的记录在10公里半径内，最佳结果（纽约州数据集）达到85%在10公里内和67%在1公里内，优于现有基线方法。

Conclusion: 大语言模型在处理复杂、冗长的地点描述方面表现出色，显示出在生物多样性领域自动地理编码复杂位置描述的潜力，特别是在处理包含空间关系的叙述性描述方面优于传统方法。
...
TL;DR: 该论文提出了一种结合专用语言模型和递归定理分解的新型计算机定理证明方法，在Lean4中实现了90.4%的miniF2F通过率，并通过分解机制进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 形式化自动定理证明长期以来被视为人工智能的挑战，传统方法在处理复杂定理时存在局限性，需要新的技术突破。

Method: 采用专用语言模型生成Lean4证明，结合递归分解将困难定理拆分为更简单的蕴含命题，通过多智能体架构协调自动形式化、证明生成、分解和递归证明过程。

Result: 在未使用分解的情况下达到90.4%的miniF2F通过率，使用分解后性能显著提升。扩展了Kimina Lean Server的AST解析能力以支持自动递归证明分解。

Conclusion: 该方法为自动定理证明提供了有效的新途径，系统已开源并在PyPI上发布为goedels-poetry，支持定制化扩展和替代语言模型适配。
...
TL;DR: LLMs可自主构建帕金森病监测本体，但不够全面；人机协作方法显著提升本体质量和完整性。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在自动化本体开发中的能力，并评估人机协作是否能创建更全面的帕金森病监测与警报本体。

Method: 采用四种方法：One Shot提示、Chain of Thought提示、X-HCOME（混合方法）和SimX-HCOME+（强调持续人工监督的迭代方法）。

Result: LLMs可自主构建本体但不全面；X-HCOME产生与专家构建相似的本体；SimX-HCOME+创建更全面准确的本体。
...
TL;DR: 提出基于动态权重生成的大规模知识编辑方法MeG，通过为LLM特定层附加动态权重神经元，利用扩散模型根据输入查询条件生成权重，实现单神经元大规模知识编辑，显著提升编辑的可靠性、泛化性和局部性指标。


<details>
  <summary>Details</summary>
Motivation: 当前在大型语言模型中进行大规模知识编辑时，难以同时保证编辑的可靠性、泛化性和局部性指标，现有方法在大规模编辑场景下面临挑战。

Method: MeG方法在LLM的特定层附加动态权重神经元，使用扩散模型根据输入查询条件生成该神经元的权重，通过添加单个动态权重神经元实现大规模知识编辑。

Result: 实验表明MeG相比现有知识编辑方法，在大规模知识编辑的可靠性、泛化性和局部性指标上均有显著提升，特别是在局部性指标的绝对百分比值上有明显提高。

Conclusion: 提出的MeG方法通过动态权重生成机制，能够有效实现大规模知识编辑，在保持编辑质量的同时显著提升了各项性能指标，为LLM知识编辑提供了新思路。
...
### [127] [Seismology modeling agent: A smart assistant for geophysical researchers](https://arxiv.org/abs/2512.14429)
*Yukun Ren,Siwei Yu,Kai Chen,Jianwei Ma*

Main category: cs.AI

TL;DR: 该论文提出基于大语言模型的智能交互工作流，通过Model Context Protocol服务器套件将SPECFEM地震波模拟软件从传统的文件驱动操作转变为意图驱动的对话交互，显著降低使用门槛。


<details>
  <summary>Details</summary>
Motivation: 传统SPECFEM软件存在学习曲线陡峭、依赖复杂手动文件编辑和命令行操作的问题，阻碍了研究人员的使用效率和科学探索。

Method: 开发首个SPECFEM的Model Context Protocol服务器套件，将整个模拟过程分解为离散的、可由智能体执行的工具，涵盖参数生成、网格划分、求解器执行和可视化等环节。

Result: 通过多个案例验证，该工作流在自主和交互模式下均能无缝运行，产生与标准基准一致的高保真结果，显著降低了使用门槛并提高了可重复性。

Conclusion: 这是MCP技术在计算地震学中的首次应用，为计算地球物理学向AI辅助和自动化科学研究提供了有前景的途径，同时保持了研究人员的科学决策权。
...
TL;DR: Context-Picker是一个用于长上下文问答的推理感知框架，将上下文选择从相似性排序转变为最小充分子集选择，通过两阶段强化学习优化，显著提升答案准确性并减少上下文长度。


<details>
  <summary>Details</summary>
Motivation: 在长上下文问答中，确定查询所需的最佳上下文量是一个重要挑战。包含太少段落可能遗漏关键信息，包含太多则会引入噪声并降低答案质量。传统方法（如固定Top-K检索和单阶段重排序）面临选择正确段落数量的困境，这对于事实性问题尤为突出，因为这类问题通常只需要少量特定证据。

Method: 提出Context-Picker框架，将上下文选择视为决策过程，通过两阶段强化学习优化：1）召回导向阶段，优先覆盖推理链；2）精确导向阶段，积极修剪冗余以提炼紧凑证据集。为解决奖励稀疏问题，提出离线证据蒸馏管道，通过留一法挖掘"最小充分集"，提供密集的任务对齐监督。
...
Conclusion: Context-Picker通过将上下文选择范式从相似性排序转变为最小充分子集选择，有效解决了长上下文问答中的上下文量优化问题。该框架通过人类启发的两阶段强化学习调度，结合离线证据蒸馏，显著提升了答案准确性并减少了不必要的上下文，为长上下文问答系统提供了有效的解决方案。
...
TL;DR: 论文提出Model-First Reasoning (MFR)方法，让大语言模型先构建问题的显式模型（实体、状态变量、动作、约束），再生成解决方案，显著减少了约束违反并提升了规划质量。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理复杂多步规划任务时存在高约束违反率和解决方案不一致的问题。现有方法如Chain-of-Thought和ReAct依赖隐式状态跟踪，缺乏明确的问题表示。

Method: 提出Model-First Reasoning (MFR)两阶段范式：第一阶段让LLM构建问题的显式模型（定义实体、状态变量、动作和约束），第二阶段基于该模型生成解决方案计划。

Result: 在医疗调度、路径规划、资源分配、逻辑谜题和程序合成等多个规划领域中，MFR相比Chain-of-Thought和ReAct显著减少了约束违反并提升了解决方案质量。消融研究表明显式建模阶段对这些改进至关重要。

Conclusion: 许多LLM规划失败源于表示缺陷而非推理限制，显式建模是构建鲁棒且可解释AI智能体的关键组件。研究结果强调了明确问题表示在复杂规划任务中的重要性。
...
TL;DR: GreedyLR是一种新颖的自适应学习率调度器，根据当前损失动态调整学习率，在多个NLP、CV和LLM任务上表现优于现有调度器。


<details>
  <summary>Details</summary>
Motivation: 尽管训练优化器取得了显著进展，但大多数研究工作仍使用常见的调度器选择（如余弦或指数衰减）。本研究旨在开发一种更有效的自适应学习率调度方法。

Method: 提出GreedyLR调度器，根据当前损失自适应调整训练过程中的学习率。提供了算法的理论分析，包括收敛性证明和最优缩放因子F的推导。

Result: 在多个NLP、CV和LLM任务（包括微调和预训练实验，参数规模达70亿）上，GreedyLR在准确性、速度和收敛性方面均优于多种最先进的调度器。算法对实际噪声环境具有鲁棒性。
...
TL;DR: 研究发现通用Transformer在ARC-AGI等复杂推理任务中的性能提升主要来自循环归纳偏置和Transformer的强非线性组件，而非复杂架构设计。基于此提出了增强版通用推理模型URM，在ARC-AGI上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 通用Transformer在复杂推理任务中表现出色，但其性能提升的具体来源尚不明确。研究者希望系统分析UT变体，找出真正有效的组件，从而设计更高效的推理模型。

Method: 首先系统分析UT变体，发现性能提升主要来自循环归纳偏置和Transformer的强非线性组件。基于此提出通用推理模型URM，通过短卷积和截断反向传播增强UT。

Result: URM在ARC-AGI上取得显著性能提升：在ARC-AGI 1上达到53.8% pass@1，在ARC-AGI 2上达到16.0% pass@1，均达到当前最佳水平。

Conclusion: 通用Transformer在复杂推理任务中的性能提升主要源于其循环结构和非线性能力，而非复杂架构设计。URM通过简单有效的增强策略显著提升了推理性能，为高效推理模型设计提供了新思路。
...
### [132] [Generative AI for Video Translation: A Scalable Architecture for Multilingual Video Conferencing](https://arxiv.org/abs/2512.13904)
*Amirkia Rafiei Oskooei,Eren Caglar,Ibrahim Sahin,Ayse Kayabay,Mehmet S. Aktas*

Main category: cs.MM

TL;DR: 提出一个系统级框架来解决级联生成式AI流水线在视频翻译等应用中的实时部署瓶颈，通过轮转机制将多用户场景的计算复杂度从二次降为线性，并通过分段处理协议管理推理延迟，在多种硬件上实现实时吞吐。


<details>
  <summary>Details</summary>
Motivation: 级联生成式AI流水线在视频翻译等应用中的实时部署面临重大系统级挑战，包括序列模型推理的累积延迟和二次计算复杂度，这使得多用户视频会议应用无法扩展。

Method: 提出一个系统级框架，包含：1）轮转机制将多用户场景的计算复杂度从二次降为线性；2）分段处理协议管理推理延迟以实现感知实时体验；3）在多种硬件上实现概念验证流水线并进行性能分析。

Result: 在商品级（NVIDIA RTX 4060）、云端（NVIDIA T4）和企业级（NVIDIA A100）GPU上的客观评估显示系统实现了实时吞吐（τ<1.0）。主观用户研究进一步验证了该方法，表明可预测的初始处理延迟对用户来说是可接受的，以换取流畅、不间断的播放体验。

Conclusion: 该工作提出了一个经过验证的端到端系统设计，为在多语言通信平台中部署可扩展的实时生成式AI应用提供了实用路线图。
...
### [133] [End-to-End Learning-based Video Streaming Enhancement Pipeline: A Generative AI Approach](https://arxiv.org/abs/2512.14185)
*Emanuele Artioli,Farzad Tashtarian,Christian Timmerer*

Main category: cs.MM

TL;DR: ELVIS是一个端到端视频流增强架构，通过服务器端编码优化和客户端生成式修复技术来移除和重建冗余视频数据，在保持带宽不变的情况下提升视频质量。
...
Method: 采用端到端架构，结合服务器端编码优化和客户端生成式修复技术，模块化设计允许集成不同编解码器、修复模型和质量评估指标。

Result: 当前技术相比基准测试可提升高达11个VMAF点，但由于计算需求较大，实时应用仍面临挑战。

Conclusion: ELVIS是将生成式AI融入视频流管道的初步尝试，为在不增加带宽需求的情况下实现更高质量的视频体验奠定了基础。
...
### [134] [Improving the Plausibility of Pressure Distributions Synthesized from Depth through Generative Modeling](https://arxiv.org/abs/2512.13757)
*Neevkumar Manavar,Hanno Gerd Meyer,Joachim Waßmuth,Barbara Hammer,Axel Schneider*

Main category: eess.IV

TL;DR: 提出一个通过信息潜在空间和权重优化损失增强物理合理性的框架，结合条件布朗桥扩散模型及其潜在版本，用于医院病床接触压力监测，以预防压疮并实现实时患者评估。


<details>
  <summary>Details</summary>
Motivation: 医院病床接触压力监测对预防压疮和实时患者评估至关重要。现有方法能预测压力分布图，但往往缺乏物理合理性，限制了临床可靠性。

Method: 提出一个框架，通过信息潜在空间和权重优化损失增强物理合理性，结合生成建模产生高保真、物理一致的压力估计。应用基于扩散的条件布朗桥扩散模型，并提出其潜在版本LBBDM的训练策略，专门用于卧姿压力合成。

Result: 实验结果表明，该方法在物理合理性和性能上优于基线：BBDM结合ILS能生成高度详细的地图，但计算成本高、推理时间长；LBBDM提供更快的推理速度，同时保持竞争力性能。

Conclusion: 该方法支持临床环境中非侵入性、基于视觉的实时患者监测，为压疮预防和患者评估提供了更可靠的解决方案。
...
### [135] [Towards Deep Learning Surrogate for the Forward Problem in Electrocardiology: A Scalable Alternative to Physics-Based Models](https://arxiv.org/abs/2512.13765)
*Shaheim Ogbomo-Harmitt,Cesare Magnetti,Chiara Spota,Jakub Grzelak,Oleg Aslanidi*

Main category: eess.IV

TL;DR: 该研究提出了一种基于深度学习的替代方案，用于解决心电学中的正向问题（从心脏电活动计算体表电位），相比传统物理模型具有更高的计算效率。
...
Method: 采用基于注意力机制的时序序列到序列架构，从心脏电压传播图预测心电图信号；引入结合Huber损失和谱熵项的混合损失函数，以保持时域和频域保真度。

Result: 在包含健康、纤维化和间隙连接重塑条件的2D组织模拟中，模型取得了高精度（平均R² = 0.99 ± 0.01）；消融研究证实了卷积编码器、时间感知注意力和谱熵损失的贡献。
...
### [136] [Synthetic Aperture for High Spatial Resolution Acoustoelectric Imaging](https://arxiv.org/abs/2512.14094)
*Wei Yi Oon,Yuchen Tang,Baiqian Qi,Wei-Ning Lee*

Main category: eess.IV

TL;DR: 该论文提出了一种合成孔径声电成像方法，通过非聚焦超声传输和相干因子加权，解决了传统聚焦超声声电成像景深有限的问题，实现了更大范围的高分辨率电生理成像。


<details>
  <summary>Details</summary>
Motivation: 传统聚焦超声声电成像的景深仅限于焦点区域，无法覆盖厘米尺度的器官范围，限制了其在生物组织电生理成像中的应用。

Method: 提出合成孔径声电成像方法，使用非聚焦超声传输，通过像素级的延迟求和重建声电图像，并利用相干因子和脉冲长度相干因子对图像进行加权处理以抑制噪声。

Result: 在盐水和离体龙虾神经实验中，合成孔径方法在整个景深范围内提高了空间分辨率，相干因子加权进一步提升了图像分辨率和对比度，信噪比甚至超过了传统聚焦超声方法。

Conclusion: 基于非聚焦波传输的相干加权合成孔径声电成像策略为快速生物电流的实用成像提供了高分辨率且抗噪声的解决方案。
...
### [137] [Test Time Optimized Generalized AI-based Medical Image Registration Method](https://arxiv.org/abs/2512.14556)
*Sneha Sree C.,Dattesh Shanbhag,Sudhanya Chatterjee*

Main category: eess.IV

TL;DR: 提出了一种新型AI驱动的3D非刚性配准框架，能够跨多种成像模态和解剖区域进行通用配准，无需针对特定应用进行定制化调整。


<details>
  <summary>Details</summary>
Motivation: 医学图像配准对于跨CT、MRI和超声等成像模态对齐解剖结构至关重要。传统非刚性配准方法需要大量参数调整且计算成本高，限制了临床实时应用。深度学习方法虽然前景广阔，但对任务特定重新训练的依赖限制了其可扩展性和实际适应性。

Method: 提出了一种新型AI驱动的3D非刚性配准框架，该框架能够跨多种成像模态和解剖区域进行通用配准。与依赖应用特定模型的传统方法不同，该方法消除了对解剖结构或模态特定定制的需求。

Result: 该方法能够实现跨模态和解剖区域的通用配准，简化了在不同临床环境中的集成过程。

Conclusion: 该框架解决了现有非刚性配准方法的局限性，提供了一种高效、可推广的解决方案，能够处理异质成像环境，有望改善临床工作流程。
...
### [138] [Configurable γ Photon Spectrometer to Enable Precision Radioguided Tumor Resection](https://arxiv.org/abs/2512.14667)
*Rahul Lall,Youngho Seo,Ali M. Niknejad,Mekhail Anwar*

Main category: eess.IV

TL;DR: 开发了一种9.9mm²的集成电路γ光谱仪，用于放射引导手术中检测肿瘤边缘的癌细胞，具有亚keV能量分辨率和1.315MeV能量动态范围。


<details>
  <summary>Details</summary>
Motivation: 手术中难以可视化微观癌细胞簇，导致残留增加癌症复发风险。现有放射引导手术需要毫米级γ光谱仪来精确定位癌细胞位置。

Method: 采用180nm CMOS工艺制造集成电路γ光谱仪，使用2×2μm反偏二极管产生毫伏级电压信号，通过测量γ光子检测后电压信号衰减时间来替代直接测量电压降，实现低功耗能量谱测量。

Result: 使用三种常见γ发射放射性同位素(64Cu、133Ba、177Lu)测试，能够在5分钟采集时间内解析低至1μCi的活度，具有亚keV能量分辨率和1.315MeV能量动态范围。

Conclusion: 该集成电路γ光谱仪为放射引导手术提供了一种高灵敏度、高能量分辨率的解决方案，能够帮助外科医生更准确地识别和切除肿瘤边缘的癌细胞。
