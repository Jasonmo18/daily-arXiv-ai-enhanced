...
TL;DR: 该论文开发了一个分析框架来研究AI系统的递归自我改进，提出了可测试的条件来判断能力是否会无限增长，并提供了实际可实施的安全控制措施。
...
Method: 开发了一个分析框架，将能力增长与资源建设和部署策略联系起来，利用物理和信息论限制定义服务边界，建立内生增长模型来区分超线性和亚临界状态。

Result: 推导出基于可观测序列的决策规则，用于判断失控行为与非奇异行为，提供了基于功率上限、吞吐量限制和评估门等实际可实施的安全控制。

Conclusion: 该研究用可测试的条件和可部署的控制措施取代了对AI奇点的猜测，为认证或排除AI奇点提供了理论基础和实践工具。
...
TL;DR: 本文提出了一种可扩展且高效的数据归属方法，通过将基于遗忘的慢速归属方法蒸馏到特征嵌入空间，实现高效检索对生成输出影响最大的训练图像。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型的数据归属方法需要大量计算资源，使得在实际应用中不实用。

Method: 将基于遗忘的慢速归属方法蒸馏到特征嵌入空间，结合高效索引和搜索方法，无需运行昂贵的归属算法即可找到高影响力图像。

Result: 在MSCOCO训练的中等规模模型和LAION训练的大规模Stable Diffusion模型上，该方法在几秒内实现更好或竞争性性能，比现有方法快2,500-400,000倍。
...
TL;DR: 开发并验证了基于共识的视频评估工具ColoWorkflow，用于微创结直肠手术的工作流程分析，该工具包含10个通用阶段和34个特定步骤，在54个手术视频中显示出良好的适用性和中等程度的评分者间可靠性。


<details>
  <summary>Details</summary>
Motivation: 微创结直肠手术存在程序变异性大、学习曲线困难、并发症影响质量和结果的问题，需要数据驱动的工具来减少变异性、优化培训和改善手术性能。

Method: 通过德尔菲法达成工作流程描述符的共识，开发ColoWorkflow工具，然后由独立评分者应用于多中心腹腔镜和机器人结直肠手术视频数据集，评估适用性和评分者间可靠性。

Result: 达成了10个通用阶段和34个特定步骤的共识，ColoWorkflow在54个手术视频中显示出广泛适用性（除一个标签外全部使用），评分者间可靠性中等（阶段平均Cohen's K为0.71，步骤为0.66）。

Conclusion: ColoWorkflow是首个基于共识的、经过验证的微创结直肠手术工作流程分析VBA工具，为视频性能评估建立了可重复框架，支持跨机构基准测试和AI驱动的工作流程识别发展。
...
### [4] [Accelerating Controllable Generation via Hybrid-grained Cache](https://arxiv.org/abs/2511.11031)
...
<details>
  <summary>Details</summary>
Motivation: 解决可控生成模型在处理控制条件和内容生成时计算需求高、生成效率普遍较低的问题。

Method: 采用两种缓存策略：1）基于特征重用的粗粒度缓存（块级），在模型推理的每一步动态绕过编码器-解码器块中的冗余计算；2）细粒度缓存（提示级），在模块内重用连续推理步骤中的交叉注意力图，并将其扩展到相邻步骤的相应模块计算中。

Result: 在四个基准数据集上验证了HGC的有效性，特别是在平衡生成效率和视觉质量方面的优势。在COCO-Stuff分割基准上，计算成本（MACs）显著降低63%（从18.22T降至6.70T），同时语义保真度损失控制在1.5%以内。

Conclusion: HGC方法能够在不显著牺牲生成质量的前提下，大幅提升可控生成模型的效率，为高效可控生成提供了有效解决方案。
...
Conclusion: 该方法在数据有限环境下实现了比现有方法更优的精度保持和剪枝率平衡
...
TL;DR: 提出短窗口滑动学习框架用于CCTV视频实时暴力检测，通过1-2秒短片段和LLM自动标注构建细粒度数据集，在RWF-2000上达到95.25%准确率，在长视频UCF-Crime上提升至83.25%。
...
TL;DR: 本文提出了一种竞争性强且计算效率高的概率性降雨临近预报方法，使用视频投影器和轻量级概率头结合预训练卫星视觉编码器，在Weather4Cast 2025基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发更准确和高效的概率降雨临近预报方法，以改进短期天气预报能力。
...
Result: 在Weather4Cast 2025基准测试中取得了有前景的性能，CRPS为3.5102，相对于最佳3D-UNET基线实现了约26%的效果提升。

Conclusion: 所提出的方法在概率降雨临近预报任务中表现出色，显著优于传统的3D-UNET方法，具有更好的计算效率和预测准确性。
...
TL;DR: PhaseWin是一种新颖的阶段窗口搜索算法，用于对象级基础模型的忠实区域归因，以近线性复杂度替代传统二次成本的贪婪选择方法。


<details>
  <summary>Details</summary>
Motivation: 现有的基于子模子集选择的归因方法虽然忠实度高，但效率限制阻碍了在实际场景中的部署应用。

Method: 采用分阶段粗到细搜索策略，结合自适应剪枝、窗口化细粒度选择和动态监督机制，近似贪婪行为同时大幅减少模型评估次数。

Result: 理论上在温和单调子模假设下保持近贪婪近似保证；实证中仅使用20%计算预算即可达到贪婪归因95%以上的忠实度，在对象检测和视觉定位任务中持续优于其他基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在OOD检测中使用负提示来强调图像特征与提示内容之间的差异，但这些提示通常包含广泛的非ID特征，可能导致次优结果。

Method: 提出正负提示监督方法，使用LLM初始化类特定的正负提示，然后优化这些提示，正提示关注类内特征，负提示突出类别边界特征，并采用基于图的架构传播语义监督到视觉分支。

Result: 在CIFAR-100和ImageNet-1K两个基准测试上，针对八个OOD数据集和五个不同LLM的广泛实验表明，该方法优于最先进的基线方法。

Conclusion: 所提出的正负提示监督方法能够有效提升OOD检测性能，通过优化提示表示和语义知识传递来实现更好的检测效果。
...
TL;DR: 本研究评估了YOLOv11n和YOLOv12n两个轻量级模型在面部表情识别中的性能，发现YOLOv12n在干净数据集上表现更好，而YOLOv11n在噪声环境下具有更高精度。


<details>
  <summary>Details</summary>
Motivation: 在无约束的真实环境中，面部表情识别仍然具有挑战性，需要平衡性能与效率的轻量级模型。

Method: 将FER2013和KDEF两个基准分类数据集转换为目标检测格式，使用mAP 0.5、精确率、召回率和混淆矩阵评估YOLOv11n和YOLOv12n模型性能。

Result: YOLOv12n在干净的KDEF数据集上获得95.6的mAP 0.5，在FER2013数据集上mAP为63.8；YOLOv11n在FER2013上精确率达到65.2，假阳性更少。
...
TL;DR: 本文提出了一种名为异构互补蒸馏（HCD）的新框架，用于解决异构架构（如ViT到ResNet18）知识蒸馏中因空间特征表示差异带来的挑战。该方法通过整合互补的师生特征来对齐共享logits表示，并引入子logit解耦蒸馏和正交性损失来确保知识多样性。
...
Method: HCD框架通过卷积投影器和自适应池化处理学生中间特征，与教师倒数第二层特征拼接后，通过互补特征映射器（CFM）生成共享logits。引入子logit解耦蒸馏（SDD）将共享logits划分为n个子logits，与教师logits融合以修正分类。使用正交性损失（OL）确保子logit多样性。
...
TL;DR: 本文提出FedBCS方法解决联邦学习中的特征异质性问题，通过领域不变上下文原型对齐来弥合特征表示差距，在医学图像分割任务中取得显著性能提升。
...
Method: 提出频率域自适应风格重校准的原型构建方法，解耦内容-风格表示并学习最优风格参数；设计上下文感知的双层级原型对齐方法，从编码器和解码器的不同层提取领域不变原型并与上下文信息融合。

Result: 在两个公共数据集上的广泛实验表明，该方法展现出卓越的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 微表情识别对于推断真实情绪至关重要。现有方法存在两个核心挑战：静态外观与动态运动线索的纠缠阻碍模型关注细微运动；现有数据集中的文本标签与面部肌肉运动不完全对应，造成文本监督与物理运动之间的语义鸿沟。
...
Conclusion: DEFT-LLM方法通过将Uni-MER的指令对齐知识集成到模型中，为微表情注入了有效的物理先验，同时利用大语言模型的跨模态推理能力，实现了对细微情绪线索的精确捕捉。
...
TL;DR: ERMoE是一种稀疏MoE变压器，通过重新参数化专家到学习正交特征基中，用特征基分数替代学习门控逻辑，实现内容感知路由，解决路由不稳定和专家利用不足问题，无需显式平衡损失。


<details>
  <summary>Details</summary>
Motivation: 解决MoE架构中路由器逻辑与专家内部结构不对齐导致的路由不稳定、专家利用不足，以及负载不平衡造成的瓶颈问题。标准解决方案如辅助负载平衡损失会削弱专家专业化并损害下游性能。

Method: 重新参数化每个专家到学习的正交特征基中，用特征基分数（输入特征与专家基的余弦相似度）替代学习门控逻辑，实现内容感知路由，直接将token分配与专家表示空间绑定。
...
TL;DR: DMC是一个用于CLIP模型的类增量学习框架，通过解耦视觉编码器和文本软提示的优化来解决分类器偏差问题。DMC-OT是其增强版本，引入最优传输校准策略来对齐内存统计信息。


<details>
  <summary>Details</summary>
Motivation: 现有的CLIP模型在持续学习新类别时面临分类器偏差问题，文本原型容易过拟合到新类别。需要一种方法来保持跨模态对齐并解决视觉编码器更新带来的分布漂移问题。

Method: 提出两阶段框架DMC：第一阶段冻结文本提示优化视觉编码器，第二阶段冻结视觉编码器优化文本软提示。DMC-OT进一步引入最优传输校准策略来对齐内存统计信息，并采用任务特定的提示设计增强任务间可分性。

Result: 在CIFAR-100、Imagenet-R、CUB-200和UCF-101上的实验表明，DMC和DMC-OT都达到了最先进的性能，DMC-OT进一步将准确率平均提高了1.80%。

Conclusion: DMC框架通过解耦视觉和文本模态的优化，有效解决了CLIP在类增量学习中的分类器偏差问题。DMC-OT的最优传输校准策略进一步提升了性能，证明了该方法在持续学习场景中的有效性。
...
TL;DR: 本文提出Phase Aggregated Smoothing (PAS)方法，通过多相位聚合平滑时间核，解决视频LLMs中的时间不稳定性问题，无需训练即可提升模型对微小时间偏移的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视频LLMs存在时间不一致性问题：帧时序的微小变化会导致注意力翻转和相关帧被抑制。这种不稳定性源于多模态RoPE扩展导致的逆傅里叶时间核产生的帧级波纹。

Method: 提出PAS方法：在多个注意力头中应用小的相反相位偏移，然后聚合它们的输出。这种方法保持每个头的频谱幅度，同时通过聚合有效平滑时间核并降低相位敏感性，而不改变位置编码结构。

Result: 在多个视频理解基准测试中，在相同token预算下显示出持续改进，计算开销可忽略不计。PAS为视频LLMs提供了即插即用的鲁棒时间编码升级方案。

Conclusion: PAS通过平滑RoPE旋转对数的时间核，实现了对微小时间偏移的Lipschitz稳定注意力，多相位平均在保持每个头频谱的同时衰减高频波纹，为视频LLMs提供了有效的时序鲁棒性解决方案。
...
TL;DR: 提出了一种无需训练的二值验证工作流，用于零样本视觉任务。该工作流包括量化（将开放性问题转化为多项选择题）和二值化（将每个候选转化为真/假问题），通过确定性决策提升准确率。


<details>
  <summary>Details</summary>
Motivation: 为了解决现成视觉语言模型在零样本视觉任务中处理开放性问题时的性能限制，提出一种强调推理时设计而非任务特定训练的统一工作流。

Method: 工作流包括两个步骤：量化将开放性问题转化为明确候选的多项选择题；二值化为每个候选生成真/假问题，通过确定性决策（当且仅当一个为真时选择，否则回退到多项选择题）进行解析。

Result: 在指代表达式定位、空间推理和BLINK-Jigsaw等任务上，量化到多项选择题带来显著提升，真/假二值化提供一致额外增益。相同工作流在所有任务上产生显著改进，表明其通用性。

Conclusion: 该工作流为当前视觉语言模型提供了一个实用、即插即用的路径，通过推理时设计实现更强的零样本视觉性能，理论分析建立了从开放性问题到多项选择题再到真/假验证的难度阶梯。
...
### [18] [Rethinking Autoregressive Models for Lossless Image Compression via Hierarchical Parallelism and Progressive Adaptation](https://arxiv.org/abs/2511.10991)
*Daxin Li,Yuanchao Bai,Kai Wang,Wenbo Zhao,Junjun Jiang,Xianming Liu*

Main category: cs.CV

TL;DR: 本文提出了HPAC框架，通过分层并行和渐进适应策略重新确立自回归模型在无损图像压缩中的实用性和性能优势，在多个数据集上达到新的SOTA压缩效果。


<details>
  <summary>Details</summary>
Motivation: 自回归模型虽然是无损图像压缩的理论性能基准，但常因计算成本过高而被认为不实用。本文旨在重新思考这一范式，通过创新框架使其成为实用且高性能的解决方案。

Method: 提出HPAC模型，采用分层分解结构和内容感知卷积门控；引入CSI推理加速和AFC高比特深度扩展；通过SARP-FT策略进行实例级渐进微调，基于信息密度选择空间连续区域优化低秩适配器。

Result: 在自然图像、卫星图像和医学图像等多种数据集上的实验验证，该方法实现了新的最先进压缩性能，在保持小参数规模和竞争性编码速度的同时显著超越现有方法。

Conclusion: 精心设计的自回归框架可以在小参数规模和竞争性编码速度下提供显著性能增益，重新确立了自回归模型在无损图像压缩中的实用价值。
...
TL;DR: CLUE是一个基于Stable Diffusion的生成模型框架，通过固定格式提示实现多样性和稳定性的图像生成，无需额外数据。在医学领域（中耳炎数据集）表现出色，显著改善图像质量指标，并作为有效的数据增强方法。
...
Method: 基于Stable Diffusion架构，引入Style Encoder处理图像和提示生成风格嵌入，通过新的第二注意力层集成到U-Net中，使用KL散度实现潜在空间的连续高斯表示。
...
TL;DR: PROMISE是一种新颖的多模态框架，通过提示学习和分层对比学习来解决模态缺失问题，提升跨模态表示的一致性。
...
Method: 提出PROMISE框架，将多模态提示学习融入分层对比学习框架，配备专门设计的提示注意力机制，动态生成缺失模态场景下的鲁棒且一致的表示。

Result: 在基准数据集上的广泛实验和消融研究显示，PROMISE相比当前最先进的多模态方法具有优越性能。

Conclusion: PROMISE通过创新的提示注意力机制和分层对比学习，有效弥合完整数据与不完整数据之间的表示差距，在处理缺失模态时表现出色。
...
TL;DR: EmoVid是首个专为创意媒体设计的多模态情感标注视频数据集，包含卡通动画、电影片段和动画表情包，通过情感条件化视频生成技术显著提升了视频生成的情感表达质量。
...
Conclusion: EmoVid为情感视频计算设立了新基准，不仅为艺术风格视频的视觉情感分析提供了宝贵见解，还提供了增强视频生成情感表达的实用方法。
...
TL;DR: 提出了Draft and Refine（DnR）框架，通过问题条件利用率指标量化LVLM模型对视觉信息的依赖程度，并利用外部视觉专家反馈来减少幻觉，提高视觉基础性。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型过于依赖语言先验而非视觉证据，导致产生不准确或幻觉的响应，缺乏量化衡量模型实际使用视觉信息程度的指标。

Method: DnR框架包含：1）构建查询条件相关性图定位问题特定线索；2）通过相关性引导的概率掩码测量依赖度；3）利用外部视觉专家反馈优化初始响应；4）选择利用率提升最大的响应。

Result: 在VQA和图像描述基准测试中，该方法实现了准确率的一致提升和幻觉减少，无需重新训练或架构更改。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在处理复杂视觉任务时存在"视觉处理瓶颈"，容易失去视觉证据的接地性和上下文视觉经验。受人类认知记忆理论启发，研究者希望为VLMs配备动态潜在视觉记忆来改善这一问题。

Method: 提出VisMem框架，包含短期模块用于细粒度感知保留和长期模块用于抽象语义整合，这些记忆在推理过程中被无缝调用，使VLMs能够在思考和生成过程中保持感知保真度和语义一致性。

Result: 在多个视觉基准测试（理解、推理和生成）上的广泛实验表明，VisMem相对于原始模型实现了11.8%的平均性能提升，并且优于所有对比方法。

Conclusion: VisMem建立了一种新的潜在空间记忆增强范式，通过认知对齐的视觉记忆机制有效解决了VLMs的视觉处理瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的推理时引导方法缺乏自适应性和选择性，无法根据提示词调整引导强度，也无法仅针对图像的不安全区域进行引导，需要更有效的安全生成方法。

Method: 通过估计提示词危害性并应用选择性引导掩码，仅在图像的不安全区域进行引导，实现自适应和选择性的安全防护。

Result: 实验表明SP-Guard比现有方法生成更安全的图像，同时最大程度减少对内容的意外改变。
...
TL;DR: 提出了SUPER解码器块，利用小波完美重构特性防止信息丢失，同时选择性抑制冗余特征，作为即插即用模块提升U-Net变体在逆问题中的性能。
...
Result: 在多个裂缝基准测试中显著提升了细裂缝分割性能，特别是在宽度小于4像素的裂缝上；在智能手机图像去噪任务中也实现了PSNR的适度提升。
...
TL;DR: AirCopBench是首个评估多模态大语言模型在复杂感知条件下空中协作感知能力的基准测试，包含14.6k+问题，涵盖4个关键任务维度，评估显示现有模型在协作感知任务上存在显著性能差距。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在单智能体视觉任务中表现良好，但缺乏评估多智能体协作感知的基准。多无人机系统相比单传感器设置具有更好的覆盖范围、鲁棒性和协作能力，但现有基准主要针对高质量单智能体图像的基本感知任务，无法评估复杂协作场景下的表现。

Method: 构建AirCopBench基准，包含来自模拟器和真实世界数据的14.6k+问题，涵盖场景理解、物体理解、感知评估和协作决策4个维度14种任务类型。通过模型、规则和人工方法在严格质量控制下生成大规模问题。

Result: 对40个多模态大语言模型的评估显示，在协作感知任务中存在显著性能差距，最佳模型平均落后人类24.38%，且在不同任务间表现不一致。微调实验证实了空中协作感知和推理中模拟到真实迁移的可行性。

Conclusion: AirCopBench填补了多智能体协作感知评估的空白，揭示了现有模型在复杂协作场景下的局限性，为未来多模态大语言模型在协作感知领域的发展提供了重要基准。
...
TL;DR: EmbryoDiff是一个基于扩散模型的两阶段框架，通过多焦点特征融合和混合语义-边界条件块，解决胚胎发育阶段识别中的细胞遮挡问题，在IVF胚胎发育阶段分类任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有判别模型未能利用胚胎发育的分布先验来提高准确性，且依赖单焦点信息导致胚胎表示不完整，在细胞遮挡情况下容易出现特征模糊问题。

Method: 提出两阶段扩散框架：1)训练冻结帧级编码器提取鲁棒多焦点特征；2)引入多焦点特征融合策略构建3D感知形态表示，设计混合语义-边界条件块将补充信息注入扩散去噪过程。

Result: 在两个基准数据集上达到最先进结果，仅使用单次去噪步骤就获得最佳平均测试性能，准确率分别达到82.8%和81.3%。

Conclusion: EmbryoDiff通过扩散建模和多焦点特征融合，有效解决了胚胎发育阶段识别中的细胞遮挡问题，显著提升了分类准确性。
...
Method: 将CheXpert、SIIM-ACR、BraTS 2020和MosMedData四个数据集重新格式化为统一的视觉问答格式，使用MAT框架构建相关、不相关和零重叠三种测试分割，评估LLaVA-Vicuna-7B和Qwen2-VL-7B等模型。

Result: 在相关分割上模型达到83.2%分类准确率和0.75分割cIoU，但在不相关和零重叠条件下性能显著下降。跨任务迁移显示仅使用分类数据训练也能提升分割性能7% cIoU。多模态大模型在组合泛化方面表现独特优势。

Conclusion: CrossMed为评估医学视觉语言模型的零样本、跨任务和模态无关泛化能力提供了严格测试平台，多模态大模型在组合泛化方面具有独特优势。
...
<details>
  <summary>Details</summary>
Motivation: 4D流MRI需要高时空分辨率来早期检测心血管疾病，但传统方法在获取速度与精度之间存在权衡。现有PINNs方法训练过程缓慢且需针对每个患者单独训练，限制了实际应用。

Method: 提出PINGS-X框架，采用轴对齐时空高斯表示，包括：(i)具有形式收敛保证的归一化高斯泼溅，(ii)轴对齐高斯简化高维数据训练同时保持精度和收敛性，(iii)高斯合并程序防止退化解并提高计算效率。

Result: 在计算流体动力学和真实4D流MRI数据集上的实验结果表明，PINGS-X显著减少训练时间，同时实现优越的超分辨率精度。

Conclusion: PINGS-X通过创新的高斯表示方法，有效解决了4D流MRI超分辨率重建中的训练效率问题，为临床实际应用提供了可行方案。
...
TL;DR: LiteAttention利用扩散注意力在去噪步骤间的时空连贯性，通过早期标记非关键图块并传播跳过决策来消除冗余注意力计算，在不损失质量的前提下显著加速视频扩散模型。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在视频生成中质量优异但存在二次注意力复杂度问题，现有加速方法面临动态估计计算开销大与静态模式不灵活的根本权衡。

Method: 基于扩散注意力稀疏模式在去噪步骤间具有强时空连贯性的观察，提出LiteAttention方法，利用进化计算跳过机制，早期标记非关键图块并传播跳过决策。
...
Method: 通过系统综述方法，分析自监督学习、半监督学习、领域泛化、联邦学习和混合神经符号模型等先进方法，同时评估协议、报告标准和可重复性挑战。

Result: 建立了跨数据集的性能基准表，识别了多中心验证和临床信任方面的开放差距，展示了技术进展与转化障碍之间的联系。

Conclusion: 这项工作为可重复、保护隐私且临床可部署的DR AI制定了实用议程，所调查的创新方法可广泛扩展到大规模医学影像应用。
...
TL;DR: 本文提出了一种名为S2D-Align的新型监督微调范式，通过利用不同粒度的辅助信号建立解剖学基础的对齐，以解决传统SFT方法在放射学报告生成中无法建立解剖学基础对齐的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的放射学报告生成方法主要依赖多模态大语言模型的跨模态生成能力，通过监督微调优化放射图像和报告之间的跨模态对齐。然而，标准SFT范式仅通过图像-文本对进行实例级对齐，无法建立解剖学基础的对齐，导致生成质量不理想。

Method: S2D-Align采用浅到深策略，逐步丰富对齐过程：从粗粒度的放射图像-报告配对开始，然后引入参考报告进行实例级指导，最后利用关键短语将生成过程锚定到具体的解剖细节。通过基于记忆的适配器连接不同的对齐阶段，实现特征共享。
...
TL;DR: 本文比较了两种有前景的生成模型——去噪扩散概率模型和自回归因果变换器，在生成形状建模和补全任务上的表现。结果显示扩散模型在连续潜在空间中表现最佳，而自回归模型在相同离散潜在空间中也能达到或超越扩散模型的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然生成模型已在包括3D数据在内的多种数据模态中得到广泛应用，但对于哪种模型最适合哪种任务尚未达成共识。此外，文本和图像等条件信息经常被用于引导生成过程，而其他条件如部分3D数据尚未得到充分评估。

Method: 本研究比较了两种最有前景的生成模型：去噪扩散概率模型和自回归因果变换器，将它们适配用于生成形状建模和补全任务。进行了全面的定量评估和比较，包括基线判别模型和广泛的消融研究。

Result: 结果显示：(1) 具有连续潜在空间的扩散模型在从单个噪声深度图像进行多模态形状补全的任务中，优于判别模型和自回归方法，并在现实条件下实现了最先进的性能；(2) 在相同的离散潜在空间中比较时，自回归模型可以匹配或超越扩散模型在这些任务上的性能。

Conclusion: 扩散模型在连续潜在空间中表现最佳，而自回归模型在离散潜在空间中具有竞争力。两种模型在不同设置下各有优势，为3D形状生成和补全任务提供了有价值的见解。
...
### [35] [Phys-Liquid: A Physics-Informed Dataset for Estimating 3D Geometry and Volume of Transparent Deformable Liquids](https://arxiv.org/abs/2511.11077)
...
<details>
  <summary>Details</summary>
Motivation: 自主机器人执行精确液体操作任务时，容器运动会导致液体表面变形，使液体状态评估变得复杂。现有数据集缺乏代表真实液体行为的物理信息模拟数据。

Method: 提出四阶段重建和估计流程：液体分割、多视角掩码生成、3D网格重建和真实世界缩放。使用物理信息数据集Phys-Liquid来验证方法的有效性。

Result: 实验结果表明，该方法在重建液体几何和体积方面具有更高的准确性和一致性，优于现有基准。

Conclusion: Phys-Liquid数据集及相关验证方法为透明液体感知任务的未来发展提供了支持，有助于提高液体几何和体积估计的精度。
...
TL;DR: 该论文提出了SaTformer，一种基于全时空注意力的视频变换器，用于从卫星辐射数据中预测极端降水。该方法将降水回归问题重新表述为分类问题，并使用类别加权损失来解决标签不平衡问题，在NeurIPS Weather4Cast 2025累积降雨挑战赛中获得了第一名。
...
Method: 提出SaTformer架构，使用全时空注意力的视频变换器，将降水回归问题重新表述为分类问题，并采用类别加权损失来处理长尾分布的数据集不平衡问题。

Result: 该模型在NeurIPS Weather4Cast 2025累积降雨挑战赛中获得了第一名，证明了其在极端降水预测方面的有效性。

Conclusion: SaTformer通过视频变换器架构和创新的数据处理方法，成功解决了极端降水预测问题，为AI天气预测提供了新的有效解决方案。
...
TL;DR: VideoP2R是一个面向视频语言模型的强化微调框架，通过将感知和推理建模为独立过程来提升视频推理能力。该方法在SFT阶段生成高质量的过程感知链式思维数据集，在RL阶段使用过程感知组相对策略优化算法分别奖励感知和推理过程。


<details>
  <summary>Details</summary>
Motivation: 将强化微调（RFT）扩展到大型视频语言模型（LVLMs）面临挑战，现有方法难以有效提升视频推理能力。

Method: 提出VideoP2R框架：1）SFT阶段：构建VideoP2R-CoT-162K数据集，包含感知和推理的三步流程；2）RL阶段：使用PA-GRPO算法，为感知和推理过程分别提供奖励。

Result: 在7个视频推理和理解基准测试中，VideoP2R在6个上达到最先进性能。消融研究验证了过程感知建模和PA-GRPO的有效性，证明模型的感知输出对下游推理具有足够信息。

Conclusion: VideoP2R通过过程感知建模成功提升了视频语言模型的推理能力，证明了将感知和推理作为独立过程处理的优势。
...
TL;DR: 本文综述了24篇关于AI生成媒体检测的最新研究，分析了现有方法的局限性和挑战，并提出基于多模态深度学习模型的研究方向，以提供更鲁棒和通用的检测能力。


<details>
  <summary>Details</summary>
Motivation: 随着GAN和扩散模型等AI技术的快速发展，生成逼真媒体内容的能力大幅提升，导致真实与合成内容难以区分。深度伪造等技术的滥用引发了错误信息传播、隐私侵犯和欺诈等问题，因此需要开发有效的检测方法。

Method: 系统回顾了24个最新研究，逐一分析每项研究的贡献和弱点，总结现有方法的共同局限和关键挑战。

Result: 现有检测方法（如CNN和ViT）主要依赖视觉、空间或时间异常检测，但在未见数据上的泛化能力不足，对不同模型生成的内容检测效果不佳，且对多模态数据和高度修改内容无效。

Conclusion: 建议未来研究聚焦于多模态深度学习模型，这类模型有潜力提供更鲁棒和通用的检测能力，为构建更强防御系统提供明确起点。
...
TL;DR: 本文提出了一种基于大语言模型的大型向量化字形模型LVGM，通过预测下一个笔画来生成向量化中文字形，并发布了大规模中文SVG数据集。
...
Result: 模型能够基于有限笔画生成完整字符、语义优雅的词语甚至未见过的诗句的向量化形式，实验显示模型在数据规模上具有扩展性。

Conclusion: LVGM成功实现了向量化中文字形的生成，生成的向量化字形得到了专家和相关人员的验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有的KBVQA方法要么通过上下文学习利用MLLMs中的隐式知识，要么通过检索增强生成利用显式知识，但它们的推理过程仍然隐含，缺乏从MLLMs获得的明确多步轨迹。

Method: 首先通过提示冻结的7B大小MLLM完成问题与其真实答案之间的推理过程，构建Hindsight-Zero训练数据；然后将其自蒸馏为思维链生成器和知识生成器；其次通过KEPO优化知识生成器，偏好低置信度但有用的知识而非高置信度但无用的知识。

Result: 在OK-VQA和A-OKVQA上的实验验证了HinD的有效性，显示使用7B大小MLLM激发推理的HinD在不使用商业模型API或外部知识的情况下实现了优越性能。

Conclusion: HinD框架成功激发了MLLMs的内部知识推理能力，为KBVQA任务提供了一种有效的推理方法，无需依赖外部知识源或大型商业模型。
...
TL;DR: OT-ALD是一个基于最优传输理论的图像到图像翻译框架，通过计算源域和目标域潜在分布之间的最优传输映射，解决了DDIB方法中的翻译效率低和潜在分布不匹配问题。
...
Method: 使用最优传输理论计算源域和目标域潜在分布之间的映射，将映射后的分布作为目标域反向扩散过程的起点。

Result: 在三个高分辨率数据集的四个翻译任务中，OT-ALD相比最佳基线模型平均提高采样效率20.29%，平均降低FID分数2.6。

Conclusion: OT-ALD有效消除了潜在分布不匹配问题，在提高翻译效率的同时保持了图像质量，平衡了快速翻译与图像质量之间的权衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测方法难以显式学习和预测智能体对轨迹变化事件的延迟响应（latencies），这会影响预测系统的因果连续性并导致不合理的轨迹。不同智能体对特定事件可能有不同的延迟偏好。

Method: 受声学中混响曲线启发，提出混响变换和Rev轨迹预测模型，使用两个显式可学习的混响核来模拟和预测每个智能体的延迟偏好及其随机性，实现基于预测延迟的可控轨迹预测。

Result: 在多个数据集上的实验表明，Rev在保持竞争力的预测准确性的同时，能够揭示跨智能体和场景的可解释延迟动态。定性分析验证了混响变换的特性。

Conclusion: 混响变换作为一种通用的延迟建模方法具有潜力，Rev模型能够显式学习智能体的延迟响应，提高轨迹预测的合理性和可控性。
...
TL;DR: MultiTypeFCDD是一个轻量级卷积框架，用于可解释的多类型异常检测，仅使用图像级标签学习生成多通道热力图，每个通道对应特定异常类型，无需为每个对象类别训练单独模型。
...
Method: 提出MultiTypeFCDD框架，使用图像级标签学习生成多通道热力图，每个通道对应特定异常类型，作为单一统一框架能够跨多个对象类别区分异常类型。

Result: 在Real-IAD数据集上评估，该方法在显著减少参数负载和推理时间的情况下，取得了与最先进复杂模型竞争的结果。
...
TL;DR: CATS-V2V是首个针对复杂不利交通场景的V2V协同感知真实世界数据集，包含10种天气和光照条件下的100个片段，提供LiDAR点云、多视角相机图像、GNSS/IMU记录以及3D边界框标注。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注普通交通场景，限制了协同感知在复杂不利交通场景中的潜力，需要专门的数据集来克服感知限制。

Method: 使用两辆硬件时间同步的车辆采集数据，涵盖10种天气和光照条件，提供多模态传感器数据，并提出基于目标的时间对齐方法确保所有对象在所有传感器模态中精确对齐。

Result: 构建了包含60K帧LiDAR点云、1.26M多视角相机图像、750K GNSS/IMU记录的数据集，提供时间一致的3D边界框标注和静态场景，支持4D BEV表示构建。
...
TL;DR: 本文提出了AlignVQA框架，通过多智能体辩论机制来改善视觉问答系统的置信度校准问题，使模型的置信度更准确地反映其预测性能。


<details>
  <summary>Details</summary>
Motivation: 现代视觉问答系统在医疗诊断和自主导航等高风险领域应用日益广泛，但这些系统经常产生过度自信的回答，其置信度估计的可靠性尚未得到充分研究。

Method: 引入AlignVQA辩论式多智能体框架，使用不同提示策略的专门化视觉语言模型生成候选答案，然后通过两阶段交互：通用智能体对这些提议进行批判、精炼和聚合。

Result: 在多个基准VQA数据集上的实证结果表明，该方法显著减少了校准差异，更校准的专门化智能体产生更好对齐的置信度。

Conclusion: 提出的aligncal可微分校准感知损失函数能够通过最小化校准误差上界来微调专门化智能体，明确改善每个智能体置信度估计的保真度。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中多视角视频常因相机触发延迟或独立录制设置导致时序不同步，影响4D高斯溅射的重建质量，需要解决这一时序错位问题。

Method: 采用粗到精的对齐模块，先估计帧级粗偏移，再细化为亚帧精度，可集成到现有4DGS框架中处理异步数据。

Result: 实验表明该方法能有效处理时序错位视频，显著提升基线方法的重建性能。

Conclusion: 提出的时序对齐策略增强了4DGS框架对非同步多视角视频的鲁棒性，实现了高质量动态场景重建。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs计算成本高，难以在资源受限场景部署；小模型难以精确捕捉细粒度视觉区域，影响在真实世界中的有效性。

Method: 使用液态状态空间动力学替代Transformer交叉注意力；提出Token-Grid相关性模块计算文本token与图像块之间的轻量级相关性，通过FiLM条件调节状态空间动力学。

Result: 在多个基准测试中，Viper-F1实现了准确、细粒度的理解，同时显著提高了效率。

Conclusion: Viper-F1通过状态空间方法和视觉定位增强，在保持高效率的同时实现了准确的细粒度视觉语言理解。
...
TL;DR: 本文提出了一种用于印度次大陆PM污染物6小时预报的高效框架，使用CAMS全球大气成分预报数据，通过轻量级参数专用架构实现准确预测。


<details>
  <summary>Details</summary>
Motivation: 开发专门针对印度地区PM污染物短期预报的高效模型，克服现有基础模型在有限空间域上的性能限制。

Method: 使用CAMS全球大气成分预报的0.4度分辨率分析场作为输入，训练三个轻量级参数专用架构模型，采用256x256输入区域和128x128预测区域的设计。

Result: 在2024年独立测试集上，模型在RMSE、MAE、Bias和SSIM指标上相比Aurora基础模型取得显著性能提升。

Conclusion: 紧凑且专门化的深度学习模型在有限空间域的短程预报中具有显著优势，能够提供准确高效的污染物预测。
...
<details>
  <summary>Details</summary>
Motivation: 开发有效的短期降雨预测方法，通过迁移学习框架提高预测准确性，并参与Weather4Cast 2025竞赛。
...
Conclusion: 提出的ConvGRU迁移学习框架在短期降雨预测中表现良好，证明了其在气象预测任务中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉问答模型在处理卫星图像时缺乏结构化推理能力，特别是在气候相关应用中需要复杂的检测、分类、空间关系和比较分析。

Method: 集成思维链推理与直接偏好优化，通过生成中间推理过程来提升模型的可解释性、鲁棒性和准确性。

Result: 实验表明，思维链监督比直接基线准确率提升34.9%，直接偏好优化进一步提升了准确率和推理质量。

Conclusion: 该框架通过增强地理空间推理能力，推动了多光谱地球观测视觉问答在气候应用中的发展。
...
TL;DR: 本文首次对视觉语言模型在微小语义保留输入变化下的鲁棒性进行了大规模系统研究，发现现代VLM对像素级偏移、几何变换、重缩放、改写等良性扰动高度敏感，即使是最先进模型也经常在微小变化下失效。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型取得了显著进展，但其在保持语义不变的微小输入变化下的可靠性尚未得到充分理解，需要系统评估模型对良性扰动的鲁棒性。

Method: 通过多种良性视觉和文本扰动（像素级偏移、几何变换、重缩放、改写、多语言重写）来测试VLM的稳定性，涵盖广泛的模型和数据集。

Result: 现代VLM对微小扰动高度敏感，大量样本在至少一种视觉或文本修改下改变预测答案；样本稳定性与正确性高度相关，稳定样本更可能被正确回答；小模型稳定性模式可预测大模型正确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D点云后门攻击局限于刚性的一对一范式，而安全敏感的3D领域（如自动驾驶和机器人）需要应对更复杂的多目标威胁。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像数据稀缺，难以针对每个领域、模态或任务单独预训练模型。需要一种统一且可扩展的方法来适应多任务和多模态需求。

Method: 通过轻量级模块组件作为专门技能集，使系统能够在推理时根据输入类型或临床目标灵活激活相应能力。

Result: 将胸部CT基础模型适应到预后和分割模块，两项任务性能均有提升。结合PET扫描，Dice得分比基线提高5%。
...
TL;DR: 本文提出了一种名为Guidance Score Distillation (GSD)的框架，通过从预训练的视频扩散模型中提取多视图一致性先验，来解决3D高斯溅射在稀疏训练视图下容易过拟合的问题。
...
Method: 提出GSD框架，基于Score Distillation Sampling (SDS)思想，监督多个相邻视图的渲染图像，引导高斯溅射表示朝向视频扩散模型的生成方向。引入统一的指导形式，包括基于真实深度图的深度扭曲指导和基于语义图像特征的指导，以校正视频扩散模型的噪声预测结果。

Result: 实验结果表明，该方法在多个数据集上优于现有方法。

Conclusion: GSD框架通过利用视频扩散模型的多视图一致性先验，有效解决了3D高斯溅射在稀疏视图下的过拟合问题，并在多个数据集上取得了优于现有方法的性能。
...
TL;DR: 本文研究了多模态表示模型中的位置偏见问题，特别是在图像-文本检索任务中，发现文本编码器偏向输入开头，图像编码器则同时偏向开头和结尾。


<details>
  <summary>Details</summary>
Motivation: 位置偏见已被证明会负面影响模型性能，但在表示模型特别是多模态模型中的存在和影响尚未充分探索。

Method: 区分上下文重要性和位置偏见，评估不同模型和数据集中的位置偏见存在程度和表现形式。

Result: 实验表明位置偏见在多模态模型中普遍存在，文本编码器偏向输入开头，图像编码器同时偏向开头和结尾，这种偏见源于位置编码方案、训练损失、上下文重要性等多因素组合。

Conclusion: 多模态模型存在显著的位置偏见，且在不同模态中表现不同，需要进一步研究以减轻其对模型性能的负面影响。
...
TL;DR: DiT-Gaze是一个基于扩散变换器和3D高斯泼溅的视线重定向框架，通过弱监督训练和正交约束损失实现高质量的视线方向合成。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅模型在渲染细微连续视线变化时存在困难，需要提高视线重定向的保真度以生成更好的训练数据。

Method: 结合扩散变换器进行高保真图像合成，使用合成生成的中等视线角度进行弱监督训练，并应用正交约束损失解耦视线、头部姿态和表情的内部表示。

Result: DiT-Gaze在感知质量和重定向精度上都达到了新的最先进水平，将最先进的视线误差降低了4.1%至6.353度。

Conclusion: 该方法为创建合成训练数据提供了优越的解决方案，代码和模型将向研究社区开放。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D深度学习在多领域泛化的局限性，特别是不同传感器采集的点云数据在密度和噪声分布上的显著差异导致的负迁移问题，以及现有方法未能充分利用领域感知和领域通用特征之间的协同作用。
...
TL;DR: 本文提出了一种新颖的小样本风格编辑框架，通过多风格专家混合低秩适配（MoE LoRA）方法，使用有限配对数据有效微调通用图像编辑模型以适应新风格。
...
Method: 提出参数高效的多风格MoE LoRA框架，包含风格特定和风格共享路由机制；采用度量引导方法自动确定各层最优秩；探索DiT模型中LoRA的最佳插入位置；集成对抗学习和流匹配指导扩散训练过程。

Result: 实验结果表明，所提方法在显著减少LoRA参数的情况下，优于现有最先进方法。

Conclusion: 该框架能够有效解决小样本风格编辑问题，通过智能路由和自适应秩选择实现高效的多风格学习。
...
TL;DR: GEODE模型通过解耦3D推理与数值生成，解决了现有视觉语言模型在3D空间理解上的双重瓶颈问题，使用1.5B参数达到与7B+模型相当的3D空间推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型基于"平面"感知架构，难以理解真实世界的3D空间智能，存在输入阶段几何感知编码器计算成本高与2D特征浅层化的冲突，以及输出阶段离散分词器无法生成精确连续数值的对齐问题。
...
Result: 1.5B参数的模型能够作为高级语义调度器运行，在空间推理性能上达到最先进水平，可与7B+模型相媲美。

Conclusion: GEODE通过解耦3D推理与数值生成的双模块架构，有效解决了视觉语言模型在3D空间理解上的根本性瓶颈，为3D空间智能提供了新的解决方案。
...
TL;DR: 本文提出了一种用于自闭症儿童注视目标检测的新方法SACF框架，通过利用社交上下文来解决数据集类别不平衡问题，在自闭症儿童注视检测任务中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 通过人工智能自动检测自闭症儿童的注视目标具有重要意义，特别是对于缺乏足够专业人员的儿童群体。该任务对于构建能够测量联合注意力的自动化系统至关重要，而联合注意力是自闭症谱系障碍的核心挑战。

Method: 提出了新颖的社交感知粗到细（SACF）注视检测框架，利用场景的社交上下文来克服自闭症数据集中常见的类别不平衡问题。采用双路径架构，包含专门处理社交和非社交注视的专家模型，并由上下文感知门模块引导。

Result: 综合实验结果表明，该框架在自闭症儿童注视目标检测任务中达到了新的最先进性能，显著优于现有方法，特别是在关键的面部注视少数类别上表现突出。

Conclusion: 该研究为自闭症儿童的注视目标检测提供了一个有效的解决方案，通过利用社交上下文信息成功解决了数据不平衡问题，为构建自动化联合注意力测量系统奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然能生成逼真连贯的图像，但经常无法正确遵循文本中的数值指令，这揭示了语言和视觉表示之间的差距。研究发现模型内部已经编码了数值正确性的潜在概念。

Method: 基于模型内部信号对计数准确性的敏感性，CountSteer在推理过程中引导模型的交叉注意力隐藏状态，无需额外训练即可提升对象计数准确性。

Result: 实验表明CountSteer将对象计数准确性提高了约4%，同时不损害视觉质量。

Conclusion: CountSteer为更可控和语义可靠的文本到图像生成提供了一个简单而有效的步骤，展示了利用模型内在数值感知能力的方法。
...
### [62] [Discovering Meaningful Units with Visually Grounded Semantics from Image Captions](https://arxiv.org/abs/2511.11262)
*Melika Behjati,James Henderson*

Main category: cs.CV

TL;DR: 本文提出了一种新的视觉语言模型，通过将标题标记分组来捕获语言的细粒度表示，并与图像编码器输出的对象表示对齐，从而提升模型对视觉和语言的细粒度理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型主要关注图像块与语言标记的对齐，但图像块对人眼无意义，单个标记也不一定携带可接地的图像信息。需要将标记分组来描述场景的不同方面，以获得更好的细粒度知识理解。

Method: 提出一种模型架构，将标题标记分组以捕获语言的细粒度表示，并将这些表示与经过训练发现对象的图像编码器输出对齐。

Result: 通过学习标记分组，视觉语言模型对视觉和语言有了更好的细粒度理解。模型发现的标记组在定性和定量上都与文本中的可接地短语高度相似。

Conclusion: 通过标记分组的方法可以有效提升视觉语言模型的细粒度理解能力，模型能够自动发现与可接地短语相似的语义单元。
...
TL;DR: 本文提出了一种新颖的模型无关方法，通过交通场景图来增强基于语言的驾驶模型，在LangAuto基准测试中显著提升了驾驶性能（LMDrive提高15.6%，BEVDriver提高17.5%）。
...
Method: 提出模型无关方法，将不同抽象层次和格式的交通场景图序列化，通过结构化提示模板将其整合到模型中，系统分析关系监督的效益。

Result: 在LangAuto基准测试中，场景图条件化显著提升了驾驶性能，LMDrive驾驶分数提高15.6%，BEVDriver提高17.5%，表明模型能更好地内化和基于关系先验。

Conclusion: 场景图条件化训练能有效提升驾驶模型的性能，即使测试时不需要场景图输入，模型也能更好地理解交通实体间的关系依赖。
...
### [64] [From Synthetic Scenes to Real Performance: Enhancing Spatial Reasoning in VLMs](https://arxiv.org/abs/2511.11440)
*Massimo Rizzoli,Simone Alghisi,Seyed Mahed Mousavi,Giuseppe Riccardi*

Main category: cs.CV

TL;DR: 本文提出了一种通过控制合成数据生成来解决视觉语言模型微调中偏差和分布不平衡问题的方法，在真实世界数据上取得了更好的性能表现。
...
Method: 重新设计微调过程：1）控制数据生成和标注，确保无偏差、分布平衡且无标注错误；2）通过全面采样物体属性（颜色、形状、大小、位置）自动构建数据集；3）使用该数据集微调最先进的视觉语言模型。

Result: 实验表明：1）在平衡合成数据上微调能在视觉场景中产生均匀性能并减轻常见偏差；2）在合成刺激上微调显著提高了在真实世界数据（COCO）上的性能，优于匹配设置下微调的模型。

Conclusion: 通过控制合成数据生成可以有效解决视觉语言模型微调中的偏差和分布不平衡问题，在真实世界数据上实现更好的性能迁移。
...
TL;DR: 提出CORAL方法，通过协调序数关系解剖学习来捕捉体积医学图像中的局部和全局结构，解决现有方法依赖硬二值阈值和忽视全局方向一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 体积医学图像分割面临解剖结构复杂和标注数据有限的问题。现有方法使用硬二值阈值定义正负样本，丢弃了连续的解剖相似性信息，且忽视了全局解剖进展的方向一致性，导致特征空间扭曲。

Method: CORAL采用对比排序目标利用连续解剖相似性，确保切片间特征距离与其解剖位置差异成比例；同时引入序数目标强制全局方向一致性，使学习到的特征分布与跨患者的规范解剖进展对齐。

Result: CORAL在有限标注设置下的基准数据集上实现了最先进的性能，同时学习到具有有意义的解剖结构的表示。

Conclusion: 通过协调学习框架，CORAL能够学习解剖信息丰富的表示，显著提升下游分割任务的性能，代码已开源。
...
### [66] [SimuFreeMark: A Noise-Simulation-Free Robust Watermarking Against Image Editing](https://arxiv.org/abs/2511.11295)
*Yichao Tang,Mingyang Li,Di Miao,Sheng Li,Zhenxing Qian,Xinpeng Zhang*

Main category: cs.CV

TL;DR: SimuFreeMark是一个免噪声模拟的水印框架，通过利用图像低频分量的固有稳定性来嵌入水印，无需训练时进行噪声模拟，在各种攻击下表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的水印方法依赖于手工噪声模拟层进行训练，这限制了它们对未知失真的泛化能力。需要一种能够抵抗传统信号处理和新型语义编辑攻击的鲁棒图像水印方法。

Method: 利用图像低频分量对多种攻击的鲁棒性，将水印直接嵌入到低频分量的深度特征空间中，使用预训练的变分自编码器（VAE）将水印与结构稳定的图像表示绑定。

Result: 广泛的实验表明，SimuFreeMark在多种传统和语义攻击下优于最先进的方法，同时保持优异的视觉质量。

Conclusion: SimuFreeMark通过消除训练过程中的噪声模拟需求，提供了一种更通用和鲁棒的图像水印解决方案，在对抗各种攻击时表现出卓越性能。
...
TL;DR: AUVIC是一个新颖的多模态大语言模型视觉概念遗忘框架，通过对抗性扰动实现精确的目标概念移除，同时避免对相关实体的性能影响。
...
Method: 提出AUVIC框架，应用对抗性扰动实现精确遗忘，有效隔离目标概念同时避免对相似实体的意外影响。构建VCUBench基准来评估群体上下文中的视觉概念遗忘。

Result: 实验结果表明，AUVIC实现了最先进的目标遗忘率，同时对非目标概念的性能退化最小。

Conclusion: AUVIC为多模态大语言模型中的视觉概念遗忘提供了有效解决方案，在满足数据隐私要求的同时保持了模型整体性能。
...
### [68] [6D Strawberry Pose Estimation: Real-time and Edge AI Solutions Using Purely Synthetic Training Data](https://arxiv.org/abs/2511.11307)
*Saptarshi Neil Sinha,Julius Kühn,Mika Silvan Goschke,Michael Weinmann*

Main category: cs.CV

TL;DR: 本文提出了一种基于纯合成数据的草莓6D姿态估计方法，使用YOLOX-6D-Pose算法和Blender程序化管道生成逼真的训练数据，在RTX 3090和Jetson Orin Nano上均取得良好性能，特别适合农业机器人部署。
...
Method: 使用YOLOX-6D-Pose单次检测算法，结合Blender程序化管道生成逼真合成数据来训练模型，支持边缘设备推理。

Result: 模型在RTX 3090和Jetson Orin Nano上均达到可比精度，RTX 3090处理速度更快，Jetson Orin Nano更适合资源受限环境。模型能准确估计成熟和部分成熟草莓姿态，但对未成熟草莓检测存在挑战。

Conclusion: 该方法在农业机器人部署中具有良好前景，可轻松扩展到其他水果如苹果、桃子和李子，为农业自动化提供有效解决方案。
...
TL;DR: DocSLM是一种高效的小型视觉语言模型，专为资源受限的边缘设备上的长文档理解设计，通过分层多模态压缩器和流式弃权机制，在减少82%视觉标记、75%参数和71%延迟的同时，在多个长多模态文档基准测试中达到或超越最先进方法。
...
Method: DocSLM采用分层多模态压缩器联合编码每页的视觉、文本和布局信息为固定长度序列，并引入流式弃权机制通过基于熵的不确定性校准器过滤低置信度响应。

Result: 在多个长多模态文档基准测试中，DocSLM使用82%更少的视觉标记、75%更少的参数和71%更低的延迟，达到或超越了最先进方法的性能。

Conclusion: DocSLM为轻量级边缘设备提供了可靠的多模态文档理解解决方案，显著降低了资源消耗同时保持了高性能。
...
TL;DR: YCB-Ev SD是一个标准分辨率事件相机数据集，用于6自由度物体姿态估计，包含50,000个34毫秒的事件序列，基于YCB-Video对象的物理渲染场景生成。
...
Method: 使用物理渲染场景，通过模拟线性相机运动生成事件序列，采用时间表面线性衰减和双通道极性编码的事件表示方法。

Result: 线性衰减时间表面和双通道极性编码在姿态估计性能上表现最优，显著优于指数衰减和单通道替代方案，极性信息对性能提升贡献最大。

Conclusion: 该数据集为事件视觉研究提供了标准化的合成数据资源，支持可重复的基准测试和即时研究使用。
...
Result: 仅使用2D运动数据训练的Free3D能生成多样化、时间连贯且语义对齐的3D运动，性能达到甚至超过完全3D监督的模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有分割方法要么需要大量标注数据（监督学习），要么分割速度慢且精度差（无监督学习），需要结合两者的优势来解决材料表征提取中的分割问题。

Method: 集成HMRF理论和CNN分割，利用HMRF的无监督学习特性和CNN的快速分割能力，研究不同邻域项和组件对无监督HMRF损失的贡献。

Result: 在PU泡沫结构的μCT图像数据集上实现了无需真实标签的高分割精度，提出的预训练策略显著减少了训练分割模型所需的标注数据量。

Conclusion: HMRF-UNet方法成功结合了无监督学习和快速分割的优势，为材料表征提取提供了一种高效准确的分割解决方案，同时通过预训练策略降低了数据标注需求。
...
<details>
  <summary>Details</summary>
Motivation: 视频情感计算面临模型不稳定和表征退化问题，核心限制是缺乏分层结构机制来解耦不同的情感组件（情感基础和瞬态波动），因为不同情感波动在不同情感上下文中的意义不同。

Method: 提出低秩稀疏情感理解框架(LSEF)，包含三个即插即用模块：稳定性编码模块(SEM)捕获低秩情感基础，动态解耦模块(DDM)分离稀疏瞬态信号，一致性集成模块(CIM)重建多尺度稳定性和反应性一致性。采用秩感知优化策略(RAO)自适应平衡梯度平滑性和敏感性。

Result: 在多个数据集上的广泛实验证实，LSEF显著增强了鲁棒性和动态辨别能力，进一步验证了分层低秩稀疏建模在理解情感动态方面的有效性和通用性。

Conclusion: 分层低秩稀疏建模为理解情感动态提供了一种有效的理论框架，LSEF通过解耦情感基础和瞬态波动，成功解决了视频情感计算中的模型不稳定和表征退化问题。
...
TL;DR: MicroVQA++是一个三阶段构建的大规模高质量显微镜视觉问答数据集，通过专家验证、异构图过滤和MLLM生成结合人工筛选的方式，显著提升了显微镜推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在生物医学成像中的应用受到大规模高质量训练数据稀缺的限制，特别是在显微镜科学推理方面。

Method: 三阶段构建方法：1) 从同行评审文章中获取专家验证的图-标题对；2) 使用HiCQA-Graph异构图融合NLI文本蕴含、CLIP视觉语言对齐和智能体信号来过滤不一致样本；3) 使用MLLM生成多选题并通过人工筛选。

Result: 构建了包含大规模训练集和人工检查测试集的数据集，其Bloom认知水平难度样本分布超过MicroVQA基准。4B规模的MLLM在显微镜推理任务上达到与GPT-5竞争的性能，并在开源MLLM中实现最先进性能。

Conclusion: 通过精心设计的数据构建方法，即使是较小规模的MLLM也能在显微镜推理任务上实现优异性能，证明了高质量数据的重要性。
...
TL;DR: Q-Doc是一个三层次评估框架，用于系统评估多模态大语言模型在文档图像质量评估方面的能力，发现MLLMs具有初步DIQA能力但存在评分不一致、失真识别错误等关键限制，同时证明思维链提示能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的能力已扩展到高级视觉任务之外，但它们在文档图像质量评估方面的潜力仍未得到充分探索，需要填补这一研究空白。

Method: 提出三层次评估框架：粗粒度级别让MLLMs为文档图像分配质量分数；中粒度级别设计失真类型识别任务；细粒度级别引入失真严重性评估。使用思维链提示来增强性能。

Result: 评估表明MLLMs具有初步DIQA能力，但存在关键限制：评分不一致、失真识别错误和严重性判断错误。思维链提示显著提升了所有级别的性能。

Conclusion: 该工作为MLLMs的DIQA能力提供了基准测试，揭示了它们在质量感知方面的明显缺陷，并指出了有前景的改进途径。
...
TL;DR: BOFA是一个用于类增量学习的新框架，通过在CLIP的跨模态桥接层进行正交低秩融合，无需额外参数即可实现稳定知识积累，同时结合跨模态混合原型提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 将CLIP应用于类增量学习面临两个主要挑战：需要额外可学习模块增加模型复杂度和遗忘风险；现有方法未能充分利用多模态表示的互补优势。

Method: BOFA框架将模型适配限制在CLIP现有的跨模态桥接层，采用正交低秩融合机制将参数更新约束到与过去任务特征正交的低秩安全子空间，并使用跨模态混合原型结合稳定的文本原型和视觉对应物。

Result: 在标准基准测试上的广泛实验表明，BOFA在准确性和效率方面优于现有方法。

Conclusion: BOFA通过桥接层正交融合实现了无需数据回放的稳定知识积累，在类增量学习中取得了优越的性能。
...
Result: 实验表明，在WEAVE-100k上训练能够增强视觉理解、图像编辑以及理解-生成协作能力，促进UMMs发展出新兴的视觉记忆能力。

Conclusion: WEAVE为多模态社区研究上下文交织理解和生成提供了视角和基础，揭示了当前方法在多轮上下文感知图像生成和编辑方面的持续局限性。
...
TL;DR: 本文研究了文本到图像扩散模型中泛化与记忆之间的模糊性，重点关注多模态标志性现象，即图像和文本唤起文化共享关联的情况。作者提出了一个评估框架，将识别（模型是否识别引用）与实现（如何通过复制或重新诠释来描绘）分开量化，并通过实验证明该框架能更有效地区分复制与转换。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型中泛化与记忆之间的模糊性问题，特别关注文化共享关联的多模态标志性现象，旨在理解模型如何在识别文化引用与复制文化引用之间取得平衡。

Method: 引入了一个评估框架，将识别和实现两个维度分开量化；评估了五个扩散模型在767个来自Wikidata的文化引用上的表现；通过提示扰动实验（同义词替换和字面图像描述）评估语言敏感性；分析了文化对齐与训练数据频率、文本独特性、引用流行度和创建日期等因素的相关性。

Result: 评估框架比现有的基于相似性的方法更能有效区分复制与转换；模型即使在文本线索改变时也经常复制标志性视觉结构；文化对齐不仅与训练数据频率相关，还与文本独特性、引用流行度和创建日期相关。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的解码器直接基于fMRI特征进行条件化，忽视了视觉信息在皮层中的层次组织方式，忽略了大脑的层次处理过程，模糊了早期、中期和晚期视觉区域的作用。

Method: 提出Hi-DREAM框架：1）ROI适配器将fMRI分组为早期/中期/晚期流，转换为多尺度皮层金字塔；2）轻量级深度匹配的ControlNet在去噪过程中注入尺度特定提示；3）浅层尺度保留布局和边缘，深层尺度强调物体和语义。
...
TL;DR: VP-Bench是一个评估多模态大语言模型视觉提示理解能力的基准测试，包含感知和下游任务两个阶段，测试了28个MLLM模型。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试未系统评估MLLM对视觉提示（如边界框）的理解能力，而这是人类直观的交互方式，需要填补这一研究空白。

Method: 采用两阶段评估框架：第一阶段测试模型在自然场景中感知视觉提示的能力，包含8种形状和355种属性组合的30k可视化提示；第二阶段评估视觉提示对下游任务的影响。

Result: 评估了28个MLLM模型（包括GPT-4o、InternVL3、Qwen2.5-VL等），分析了影响VP理解的因素，如VP属性变化、问题安排和模型规模。
...
TL;DR: VoxTell是一种用于文本提示的3D医学图像分割的视觉语言模型，能够将自由形式的文本描述映射到3D分割掩码，在多种医学影像模态上实现零样本分割。
...
Method: 使用多阶段视觉语言融合策略，在解码器层间对齐文本和视觉特征；在62K+ CT、MRI和PET扫描数据上训练，涵盖1000+解剖和病理类别。

Result: 在未见数据集上实现最先进的零样本性能，在熟悉概念上表现优异，同时能泛化到相关未见类别；展示出强大的跨模态迁移能力、对语言变化的鲁棒性以及从真实世界文本进行实例特定分割的准确性。

Conclusion: VoxTell为医学图像分割提供了一种灵活且强大的文本提示方法，能够有效处理临床语言变化并实现准确的3D分割，代码已开源。
...
TL;DR: 提出了MaMOL框架，将模态缺失问题重新定义为多任务学习问题，通过双路由机制实现参数高效的缺失模态适应。


<details>
  <summary>Details</summary>
Motivation: 遥感多模态分类常因环境干扰、传感器故障或大气效应导致模态缺失，严重影响分类性能。现有两阶段适应方法计算成本高且假设训练时数据完整，限制了在现实世界不完整情况下的泛化能力。

Method: 引入缺失感知的混合LoRA框架，采用双路由机制：任务导向的动态路由器自适应激活专家处理不同缺失模式，模态特定-共享的静态路由器维持稳定的跨模态知识共享。通过轻量级专家更新和共享专家重用实现参数高效适应。
...
TL;DR: 本文发现大型视觉语言模型（LVLM）在产生物体幻觉时往往忽略图像信息，而是依赖先前生成的标记来推断新物体。作者提出了Prelim Attention Score（PAS），一种无需训练、基于注意力权重的轻量级信号，用于实时检测物体幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然强大但存在不可靠的物体幻觉问题。研究发现许多幻觉预测中模型实际上忽略了图像，而是依赖先前生成的标记来推断新物体。

Method: 通过条件互信息量化图像依赖程度，提出Prelim Attention Score（PAS）——基于注意力权重的轻量级信号，无需额外前向传播，可在推理过程中实时计算。

Result: PAS在多个模型和数据集上实现了最先进的物体幻觉检测性能，能够实现实时过滤和干预。

Conclusion: 利用先前被忽视的注意力信号，PAS提供了一种高效、无需训练的物体幻觉检测方法，显著提升了大型视觉语言模型的可靠性。
...
TL;DR: BRIDGE提出了一种轻量级的跨模态融合模块，通过在编码器顶部添加少量双向注意力层来对齐视觉和文本隐藏状态，既保持了编码器的非因果性和理解能力，又实现了高效的跨模态对齐。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型融合方法要么在编码器早期混合特征，要么在后期比较池化嵌入，但两种模态的隐藏状态本身已包含丰富的模态特定结构，直接对齐这些状态是更自然的方式。

Method: 在视觉和文本编码器顶部附近放置少量仅跨模态的双向注意力层，将隐藏状态序列投影到共享空间，进行跨模态注意力计算，并通过门控残差更新返回，使用简单的稳定器来改善对齐效果。

Result: 在标准检索、VQA和视觉推理基准测试中，BRIDGE优于可比较的视觉语言模型，同时保持了对比模型的双编码器效率。

Conclusion: BRIDGE提供了一种有效的跨模态对齐方法，既保持了编码器的理解能力，又实现了高效的模态融合，代码已开源。
...
TL;DR: 本文提出了一种基于大语言模型的军事模拟推演数据分析方法，通过任务分解、多轮交互和自定义工具，生成高质量的结构化分析报告。


<details>
  <summary>Details</summary>
Motivation: 传统的手工分析方法耗时且容易出错，而单次指令输入大语言模型无法获得格式良好的高质量分析报告。
...
Conclusion: 所提出的方法能够有效提升军事模拟推演数据分析的效率和准确性，生成高质量的结构化分析报告。
...
TL;DR: 提出了一种通过离线数据增强和并行检索的结构化记忆架构，解决了历史人物对话系统中深度与效率的权衡问题，在保持低延迟的同时实现深度对话。
...
Conclusion: 该架构不仅作为对话界面，还作为传记分析的研究工具，提供时空热图、情感轨迹分析和交互路径跟踪等可视化功能，适用于教育、博物馆和研究应用。
...
TL;DR: HyQuT是首个用于自然语言生成的混合量子-经典大语言模型，在Transformer框架中集成了变分量子电路，在8M和150M参数规模下实现了连贯的上下文感知对话。


<details>
  <summary>Details</summary>
Motivation: 尽管量子计算逐渐应用于替代经典计算，但现有量子或混合模型仍局限于简单任务，尚未成功应用于大规模自然语言生成。本研究旨在探索将量子计算集成到大规模生成语言模型中的可行性。

Method: 提出的HyQuT架构在Transformer框架中集成了变分量子电路（VQC），在8M和150M参数规模下进行实验。使用最少量子比特（10个量子比特和80个量子门）替代约10%的经典参数。

Result: 实验结果表明，在150M参数模型中，使用最少量子资源可以替代约10%的经典参数，同时实现可比较的收敛稳定性和生成质量。

Conclusion: 本研究为将量子计算集成到大规模生成语言模型提供了早期可行性证明，展示了量子计算在大规模自然语言处理任务中的潜力。
...
TL;DR: 研究发现大型语言模型在处理时间约束方面存在系统性风险，包括双峰性能分布、极端提示脆弱性和系统性行动偏见，即使通过微调也无法可靠学习时间约束处理能力。


<details>
  <summary>Details</summary>
Motivation: 当前在需要实时决策的智能体架构中部署LLM时，假设它们能够可靠地判断行动窗口是否开放或已关闭，但这种假设未经测试。
...
TL;DR: 本文扩展了谱神经符号推理框架，通过引入三种基于语义的增强：基于Transformer的节点合并、句子级蕴含验证和外部知识图谱对齐，在不改变核心谱推理引擎的情况下提升图质量。


<details>
  <summary>Details</summary>
Motivation: 现有谱神经符号推理框架在节点冗余、边质量不高和上下文缺失方面存在不足，需要在不依赖二次注意力机制的情况下提升推理的鲁棒性和可扩展性。

Method: 提出三种语义增强方法：(1)使用上下文嵌入进行Transformer节点合并减少冗余；(2)使用预训练NLI分类器进行句子级蕴含验证改进边质量；(3)与外部知识图谱对齐增强缺失上下文。

Result: 在ProofWriter、EntailmentBank和CLUTRR基准测试中显示一致的准确率提升（最高+3.8%），改进对对抗性案例的泛化能力，并减少推理噪声。

Conclusion: 通过模块化、基于语义的预处理步骤扩展谱神经符号推理框架，在不改变核心谱推理引擎的情况下实现了更鲁棒、可解释和可扩展的推理系统。
...
Result: 理论分析证明该方法优于固定偏好权重方法，多个任务上的实验验证了其有效性。

Conclusion: PRO框架能够自动推断提示特定的偏好权重，在多目标对齐场景中实现了优越的性能和训练效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决SimCSE风格dropout增强在专利文档中产生的语义凝聚力丧失问题，利用专利文档固有的多章节结构来提供自然语义多样性。

Method: 提出基于章节的增强策略，将专利的不同部分（摘要、权利要求、背景等）作为互补视图进行对比学习，避免过度分散并保持全局结构和局部连续性。

Result: 在大规模基准测试中，这种完全自监督的方法在现有技术检索和分类任务上达到或超过了基于引用和IPC监督的基线方法，同时避免了依赖脆弱或不完整的标注。

Conclusion: 不同专利章节对不同任务具有专门化作用，权利要求和摘要有利于检索，背景部分有助于分类，这凸显了利用专利固有话语结构进行表示学习的价值。
...
Method: 使用15个开源LLM（包括通用型和医学专用型），在6种疾病用例中测试6种提示策略（零样本、单样本、少样本、思维链、自一致性和提示图），采用任务适配指标和线性混合效应模型进行性能评估。

Result: 表现最佳的模型在各任务中的宏观平均得分接近评分者间一致性；中小型通用模型与大型模型性能相当，而微型和专用模型表现较差；提示图和少样本提示将性能提升约13%；任务特定因素比模型大小或提示策略影响更大。
...
TL;DR: 提出了一种基于加权专家乘积（wPoE）的测试时引导框架，将通用压缩模型与预训练神经语言模型自适应结合，确保压缩率至少不低于最佳单个模型，无需微调即可提升文本压缩性能。


<details>
  <summary>Details</summary>
Motivation: 传统通用压缩器（如gzip）计算开销低、速度快、适用性广，但压缩率不如现代神经压缩器；而神经压缩器虽然能更好地建模数据分布，但在未见数据上泛化能力差。需要解决这一局限性。

Method: 采用测试时引导的加权专家乘积（wPoE）框架，在推理时自适应地结合通用压缩模型和预训练神经语言模型，确保压缩率至少与最佳单个模型相当。

Result: 大量实验表明，该方法无需微调即可提升文本压缩性能，并能与任何自回归语言模型无缝集成。

Conclusion: 该框架为跨不同数据分布的文本压缩提供了实用解决方案，结合了传统压缩器的通用性和神经压缩器的高效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型文本生成系统评估方法往往忽视统计不确定性量化，而概率生成策略会引入额外的不确定性，需要更严谨的统计方法来评估模型行为。

Method: 采用贝叶斯方法量化二元评估指标的不确定性，特别关注由概率文本生成策略引起的不确定性。通过两个案例研究验证该方法：1）评估对抗性输入上的拒绝率；2）评估开放对话示例中的成对偏好。

Result: 贝叶斯方法能够为基于LLM的系统行为提供有用的不确定性量化，在拒绝率评估和模型偏好比较中都能有效捕捉统计不确定性。

Conclusion: 贝叶斯不确定性量化方法能够更全面地评估LLM文本生成系统的行为，为模型评估提供更可靠的统计基础，特别是在涉及概率生成策略的场景中。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在高资源语言（如英语）上表现出色，但在低资源和形态丰富语言中的有效性尚未充分探索，因此需要对这些语言进行系统性评估。

Method: 构建了一个跨语言基准测试，涵盖粤语、日语和土耳其语，包含四个任务：开放域问答、文档摘要、英译X和文化对话。结合人工评估（流畅度、事实准确性、文化适当性）和自动指标（BLEU、ROUGE）来评估模型性能。

Result: 最大的专有模型（GPT-4o、GPT-4、Claude 3.5）在语言和任务上普遍领先，但在文化细微理解和形态学泛化方面仍有显著差距。GPT-4o在跨语言任务上表现出强大的多语言能力，Claude 3.5在知识和推理基准上达到竞争性准确度。所有模型都在各语言的独特语言挑战上存在困难，如土耳其语的粘着形态和粤语的口语表达。较小的开源模型（LLaMA-2-13B、Mistral-7B）在流畅度和准确性上大幅落后。

Conclusion: 研究揭示了开发更具文化意识和语言泛化能力的大语言模型的必要性，并发布了基准测试和评估数据以促进可重复性和进一步研究。
...
TL;DR: 本文提出了一种自监督框架，通过使用偏斜感知聚合策略来提高防护模型的语义鲁棒性，减少语义变异性58%，提升基准准确率2.5%，并改善模型校准度达40%。


<details>
  <summary>Details</summary>
Motivation: 防护模型对表面语言变化的敏感性是LLM安全的关键漏洞，即使意义保留的改写也会导致安全评分大幅波动，揭示缺乏语义基础。

Method: 引入实用的自监督框架，利用改写集通过新颖的偏斜感知聚合策略来强制执行预测一致性，用于鲁棒目标计算。

Result: 方法将语义变异性降低约58%，基准准确率平均提高约2.5%，并能泛化到未见过的风格变化，同时模型校准度提升达40%。

Conclusion: 应将语义一致性作为首要训练目标，这为构建更可靠的防护模型提供了可扩展的方案，并揭示了模型校准与一致性之间的双向关系。
...
TL;DR: 本文提出了STaDS框架来评估LLMs的真实理解能力，通过结构化决策模拟测试发现大多数前沿模型在跨领域决策中表现不一致，存在准确性与决策因素依赖不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs虽然在预测准确性上表现出色，但正确性并不等同于真正的理解能力。需要评估LLMs是否能够像人类专家一样，在多个实例和不同领域中做出一致且基于正确决策因素的决策。

Method: 引入结构化表格决策模拟(STaDS)框架，将LLMs视为参加结构化决策"考试"的专业人士，通过三个维度评估理解能力：(i)问题和指令理解，(ii)基于知识的预测，(iii)对相关决策因素的依赖。

Result: 对9个前沿LLMs在15个不同决策设置中的分析显示：(a)大多数模型难以在不同领域保持一致的强准确性；(b)模型可能准确但全局不可靠，存在陈述理由与预测驱动因素之间的频繁不匹配。
...
<details>
  <summary>Details</summary>
Motivation: 人工耳蜗植入能显著改善重度至极重度感音神经性听力损失儿童的口语能力，但结果变异性较大，且无法通过植入年龄或残余听力可靠预测个体结果。
...
Conclusion: 深度迁移学习通过直接捕获判别性和任务特定信息，在表征学习方面具有优势，支持开发适用于全球人工耳蜗项目的单一预测模型的可行性。
...
Method: 使用MoE语音投影器，每个专家专精特定语言的语义子空间；采用多阶段训练范式，利用单语ASR和ST数据；结合语言特定损失、组内负载均衡损失和过渡损失来指导专家分配和阶段过渡。

Result: 在广泛使用的数据集上进行大量实验，证明了该方法的有效性和通用性。

Conclusion: 所提出的方法能够有效解决代码切换语音翻译中的挑战，通过专家专业化语义建模和多阶段训练策略提升了翻译性能。
...
TL;DR: 本文提出了GVF微调方法，通过事实锚点数据增强、事实感知指令调优和事实一致性损失函数三个核心机制，系统性地提升多模态大语言模型的视觉事实一致性，显著减少视觉幻觉现象。
...
Method: GVF微调整合了三个核心机制：事实锚点数据增强（用结构化事实锚点和反事实提示丰富训练数据）、事实感知指令调优（将事实线索嵌入显式指令中）、事实一致性损失函数（专门惩罚事实不准确性）。

Result: 在LLaVA-1.5-13B模型上评估，GVF微调在VHTest基准测试中显著优于标准微调，在开放式问题和是/否问题格式上都表现出色。同时，在MME和POPE等多模态基准测试中保持或略微提升性能。

Conclusion: GVF微调能够有效缓解视觉幻觉问题，同时不损害模型的一般理解和推理能力，为提升多模态大语言模型的可靠性提供了有效解决方案。
...
TL;DR: 这篇综述回顾了大型语言模型在材料科学中的三大应用领域：科学文献挖掘、预测建模和多智能体实验系统，并展示了开源模型在性能上可与闭源商业模型相媲美。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在材料科学中的快速应用，需要系统梳理其在材料发现流程中的关键应用，并评估开源模型的实际表现，以推动更透明、可复现的AI平台建设。

Method: 通过综述分析LLM在材料科学中的三大应用方向：从文本中提取合成条件等信息、学习结构-性能关系、协调集成计算工具和实验室自动化的多智能体系统，并进行基准测试比较开源与闭源模型性能。
...
TL;DR: 本文提出了一种两阶段演示选择方法TopK + L2D，通过结合语义相似性和标签分布对齐来改进文本分类中的上下文学习演示选择，在七个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有演示选择方法主要关注测试输入与演示之间的语义相似性，往往忽略了标签分布对齐的重要性，这限制了大型语言模型在文本分类中的上下文学习性能。

Method: 提出两阶段演示选择方法：首先基于语义相似性选择TopK候选演示，然后使用微调的BERT类小型语言模型生成标签分布并计算标签分布散度，选择与测试输入标签分布对齐的演示。

Result: 在七个文本分类基准测试上的广泛实验表明，该方法持续优于之前的演示选择策略。进一步分析显示，大型语言模型的性能与用于标签分布估计的小型语言模型的准确性呈正相关。
...
<details>
  <summary>Details</summary>
Motivation: 气候变化和温室气体排放的影响是全球关注的主要问题，这些排放很大程度上来自消费品的生产、使用和处置。因此需要构建工具来评估消费品的环境影响，其中生命周期评估（LCA）是重要组成部分。

Method: 开发了SpiderGen LLM工作流，整合传统LCA分类学和方法论与LLM的推理能力，生成用于LCA的过程信息，并与真实LCA文档作为基准进行比较评估。

Result: SpiderGen提供了准确的LCA过程信息，在10个样本数据点上达到62%的F1分数，输出要么完全正确要么只有小错误。相比基线技术（如思维链提示和单样本提示）表现更好。

Conclusion: SpiderGen有潜力显著减少估算碳影响所需的人工努力和成本，能够在10分钟内以低于1美元的成本生成LCA过程信息，而传统LCA可能需要超过25000美元和21人天。
...
TL;DR: 本文通过多轮对话测试发现大型语言模型在重复提问下存在显著脆弱性，简单的'再想想'提示会导致准确率下降约10%，结合语义等效的改写问题会使准确率下降7.5%。研究建立了马尔可夫链模型来预测准确率动态，并提出稳态准确率作为交互环境下的稳健性指标。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型应用范围扩大，用户与模型的交互频率和规模不断增加，评估模型在真实任务中的稳健性变得至关重要。研究旨在测试模型在多轮对话中的答案变化和准确率动态。

Method: 使用简单的多轮后续提示评估模型答案变化，通过马尔可夫链分析各轮次的准确率动态，并检验线性探针是否能预测这些变化。

Result: 发现LLM存在显著脆弱性：'再想想'提示使Gemini 1.5 Flash在九轮对话中准确率下降约10%；结合改写问题使Claude 3.5 Haiku准确率下降7.5%。模型准确率可用马尔可夫链有效建模，Gemini 1.5 Flash的稳态准确率比首轮低约8%。线性探针可帮助预测未来答案变化。

Conclusion: 研究确立了稳态准确率作为交互环境下稳健性的原则性指标，揭示了模型在重复提问下的脆弱性。解决这种不稳定性对于在高风险和交互环境中部署LLM至关重要，因为一致的推理与初始准确率同等重要。
...
TL;DR: 研究递归提示下大语言模型生成合成数据时的性别偏见动态，发现偏见会趋向模型固有水平而非单调放大，并提出对比增强策略能有效减少下游任务偏见。
...
Result: 低初始偏见会放大36%趋向模型固有水平，高初始偏见会衰减26%；对比增强策略在下游任务中显著减少偏见（低初始偏见减少98.8%，平均减少91%）。
...
TL;DR: 本文提出使用'电话游戏'方法研究多模态系统的隐藏语言，通过观察概念共现频率来量化系统理解中的概念连接强度，并贡献了Telescope数据集。


<details>
  <summary>Details</summary>
Motivation: 由于闭源多模态系统的黑盒架构，其理解世界的隐藏语言不透明，需要研究其偏好偏差如何影响概念理解。

Method: 采用多轮'电话游戏'框架，利用系统在图像压缩和重建过程中的偏好偏差，观察概念共现频率变化来研究概念连接强度。

Result: 开发了Telescope数据集（10,000+概念对），能够构建多模态系统理解中的全局概念连接图，识别训练继承的偏好偏差，评估泛化能力。
...
TL;DR: Squid Game是一个动态对抗性评估环境，通过资源受限和信息不对称的交互游戏来评估大语言模型，包含6个淘汰赛级别，测试多方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试难以跟上大语言模型的发展，存在数据污染问题，且主要假设良性、资源丰富的环境，缺乏在压力下模型行为的评估。

Method: 设计动态对抗性评估环境Squid Game，包含6个淘汰赛级别，让LLM与其他LLM对手进行交互游戏，测试指令遵循、代码、推理、规划和安全对齐等多方面能力。

Result: 评估了超过50个LLM，观察到同一模型谱系中明显的代际性能转变，发现一些模型采用投机性捷径获胜，表明静态基准测试可能存在更高级别的评估范式污染。
...
TL;DR: 研究发现AI语音系统能够隐式学习人类社交中的礼貌表达方式，在礼貌提示下会显著降低语速，模仿人类通过减慢语速表达礼貌的社交惯例。


<details>
  <summary>Details</summary>
Motivation: 探究AI语音系统是否能学习人类社交中非显性编程的隐含线索，特别是通过语速变化表达礼貌的微妙韵律标记。
...
TL;DR: 本文系统研究了大型语言模型在国际关系领域表现出的国家层面偏见，基于联合国安理会历史记录构建了偏见评估框架，发现LLMs存在多维偏见，并提出结合检索增强生成和反思技术的去偏见框架。


<details>
  <summary>Details</summary>
Motivation: 研究动机是系统评估大型语言模型在国际关系领域中的国家层面偏见，特别是在联合国安理会五个常任理事国之间的偏见模式，以解决LLMs在该领域应用时的偏见问题。

Method: 方法包括：1）基于联合国安理会历史记录构建包含三种测试的偏见评估框架；2）分析不同LLMs的国家层面偏见模式；3）提出结合检索增强生成和反思技术的去偏见框架。

Result: 实验结果发现：1）LLMs普遍存在对西方国家的有利偏见和对俄罗斯的不利偏见；2）偏见随模型和评估情境而变化；3）推理能力更强的模型偏见更少、性能更好；4）提出的去偏见框架有效减少了国家层面偏见，特别是在GPT-4o-mini和LLama-3.3-70B中。

Conclusion: 结论强调在国际关系领域应用LLMs时，需要同时评估国家层面偏见和性能表现，提出的去偏见框架为减少LLMs偏见提供了有效解决方案。
...
Result: ΠAttention在语言建模、检索和视觉语言任务中达到或超过密集注意力的质量，困惑度比RingAttention低8.3%，相同上下文长度下GPU使用减少50%。
...
TL;DR: 本文提出了一种结合TextRank句子提取、医学命名实体识别和大语言模型的框架，用于提升医学文本摘要的忠实度。在MeQSum和BanglaCHQ-Summ数据集上微调LLaMA-2-7B模型，在质量和忠实度指标上均取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 消费者健康问题摘要可以简化医疗沟通，但不忠实的摘要可能误传医疗细节，带来严重风险。因此需要确保医学文本摘要的忠实性。

Method: 结合TextRank句子提取、医学命名实体识别和大语言模型(LLMs)，在MeQSum(英语)和BanglaCHQ-Summ(孟加拉语)数据集上微调LLaMA-2-7B模型。
...
TL;DR: 本文介绍了TEDxTN，这是首个公开可用的突尼斯阿拉伯语到英语语音翻译数据集，包含108个TEDx演讲，25小时语音，涵盖11个突尼斯地区的不同口音，支持代码转换。
...
Result: 创建了首个开源公开的代码转换突尼斯方言语音翻译语料库，包含25小时语音数据，覆盖多个地区口音。
...
Conclusion: GenAI和RAG技术可以有效简化学生获取日常学术信息的过程，Gemini 2.0 Flash和Gemma 3n是适合此应用的优秀模型选择。
...
### [117] [Unsupervised Cycle Detection in Agentic Applications](https://arxiv.org/abs/2511.10650)
*Felix George,Harshit Kumar,Divya Pathak,Kaustabha Ray,Mudit Verma,Pratibha Moogi*

Main category: cs.CL

TL;DR: 提出了一种结合结构和语义分析的无监督循环检测框架，用于检测LLM驱动应用中隐藏的执行循环，在LangGraph股票市场应用中验证效果显著优于单一方法。
...
Result: 在LangGraph股票市场应用的1575个轨迹上评估，混合方法F1得分为0.72（精度0.62，召回率0.86），显著优于单一结构方法（F1:0.08）和语义方法（F1:0.28）。
...
TL;DR: 本研究探讨了使用GPT-4o在计算语言学课程中对简答题测验和项目报告进行自动评分的可行性，结果显示LLM评分与人类评分具有高度相关性（最高达0.98），在55%的测验案例中分数完全一致。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型越来越多地用于教育任务如评分，但其在真实课堂环境中与人类评估的一致性仍缺乏充分研究。本研究旨在验证LLM在实际教学评估中的可行性。

Method: 收集了约50名学生的五次测验简答题和14个团队的项目报告，使用GPT-4o进行评分，并与课程助教独立进行的人工评分进行对比分析。

Result: GPT-4o与人类评分者表现出强相关性（最高0.98），在55%的测验案例中分数完全一致。对于项目报告，总体上与人类评分保持一致，但在技术性、开放性回答的评分上存在一定变异性。
...
TL;DR: 该研究通过分析多语言大语言模型的内部机制，发现它们采用枢轴语言表示机制：模型在不同语言间使用几乎相同的表示，而语言特定的解码出现在后层。通过干预语言特征可以控制模型输出语言。


<details>
  <summary>Details</summary>
Motivation: 探索多语言大语言模型如何内部表示不同语言，以及为什么性能仍然偏向主导训练语言，旨在理解多语言对齐机制。

Method: 训练一系列不同多语言数据混合的LLM，使用跨层转码器和归因图分析内部机制，通过干预语言特征进行实验。

Result: 发现模型采用枢轴语言表示机制，解码依赖后层高频语言特征，这些特征从模型首层线性读取语言身份，通过干预可以抑制一种语言并替换为另一种。
...
TL;DR: 本文提出了一种基于参数空间对齐的任务算术方法，通过利用Transformer架构的对称性来减少模型间的负干扰，在推理任务上优于标准任务算术。


<details>
  <summary>Details</summary>
Motivation: 解决任务算术在模型训练发散时出现的负干扰问题，提升跨模型技能迁移的效果。

Method: 首先对齐模型的参数空间，利用Transformer架构的置换、旋转和缩放对称性，适配GQA和SwiGLU层，探索基于权重和激活的方法。

Result: 在具有挑战性的推理基准测试中，该方法始终优于标准任务算术，成功将高级推理技能迁移到非推理模型。

Conclusion: 该方法为在演进的LLM家族间合并和迁移专门技能提供了有效途径，减少了冗余微调并增强了模型适应性。
...
TL;DR: 本文研究了LLM作为评判者在社交对话任务中的可靠性，发现将任务从直接事实查询重构为对话判断任务会显著改变模型的判断，不同模型表现出不同的倾向性（谄媚或过度批判）。


<details>
  <summary>Details</summary>
Motivation: LLM越来越多地被用作各种任务的评判者，包括日常社交互动任务，但尚不清楚LLM评判者能否可靠评估需要社交或对话判断的任务。

Method: 通过对比模型在直接事实查询和对话判断任务中的表现，并在两种条件下施加简单的反驳压力，测量模型在对话压力下保持立场的坚定程度。

Result: 研究发现，GPT-4o-mini在社交框架任务中表现出谄媚倾向，而Llama-8B-Instruct则变得过度批判，所有模型的平均性能变化为9.24%。

Conclusion: 即使是最小的对话上下文也能显著改变模型判断，对话框架是LLM评估中的关键因素，该框架为诊断模型信念提供了可复现的方法论。
...
TL;DR: ICX360是一个开源Python工具包，专门用于解释大型语言模型(LLMs)的输出，重点关注用户提供的上下文和提示。它包含三种基于扰动和梯度的解释方法，支持检索增强生成、自然语言生成和越狱等多种用例。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在医疗问答、会议摘要等高风险应用中的普及，开发解释LLM输出的工具变得至关重要，以增强模型透明度和可信度。

Method: ICX360实现了三种最新的解释工具，使用黑盒方法(通过扰动)和白盒方法(通过梯度)来解释LLMs，支持对用户提供的上下文进行分析。

Result: 开发了完整的开源工具包，包含快速入门指南和详细教程，覆盖检索增强生成、自然语言生成、越狱等多种实际应用场景。

Conclusion: ICX360为LLM的可解释性提供了实用的工具支持，有助于提高模型在关键应用中的透明度和可靠性。
...
TL;DR: 本文揭示了LLMs在二元决策任务中存在格式级负向偏见，即提示格式比负向响应的语义对模型回答影响更大。研究发现当模型缺乏足够知识时倾向于生成负向回答，且不同提示策略对负向偏见有不同影响。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注检测和解决引发负向偏见的负向注意力头，但对影响负向偏见的详细因素探索不足。本文旨在深入探究LLMs负向偏见的具体影响因素。

Method: 引入构建评估集的流程，系统地将数据集分为三个子集（正确、错误、知识不足）。通过分析评估集识别模型在知识不足时的捷径行为，并研究不同提示场景下负向偏见的变化。

Result: 发现提供相关上下文和"我不知道"选项通常能减少负向偏见，而思维链提示往往会放大偏见。负向偏见的程度随提示类型而变化，提示类型影响回答方向。

Conclusion: 本研究揭示了影响负向偏见的各种因素，为减轻LLMs中的负向偏见提供了关键见解。
...
TL;DR: MedPath是一个大规模多领域生物医学实体链接数据集，整合了9个现有专家标注数据集，使用UMLS进行实体规范化，映射到62个生物医学词汇表，并添加完整的本体路径，支持语义丰富和可解释的实体链接系统开发。


<details>
  <summary>Details</summary>
Motivation: 解决生物医学命名实体识别和实体链接领域的数据碎片化问题、缺乏可解释模型资源以及语义盲评估指标的局限性。

Method: 基于9个现有专家标注的实体链接数据集构建MedPath数据集，使用UMLS最新版本进行实体规范化，映射到62个其他生物医学词汇表，并在最多11个生物医学词汇表中添加完整的本体路径。
...
TL;DR: 本文研究了工具增强语言模型（TaLMs）在使用代码解释器工具时的推理退化现象，提出了工具诱导短视（TIM）概念，发现虽然工具使用提高了最终答案准确率，但损害了推理过程的连贯性。


<details>
  <summary>Details</summary>
Motivation: 研究TaLMs在使用外部工具时是否真正进行可信推理，而非简单依赖工具输出作为推理替代品。

Method: 使用PYMATH基准测试（1,679个竞赛级数学问题），开发多维度评估套件量化推理退化，分析工具使用频率与推理质量的关系，并提出基于偏好优化的框架来改善工具使用方式。

Result: TaLMs在使用工具时最终答案准确率提升19.3个百分点，但推理过程质量下降（非工具LLMs在推理过程比较中胜出41.5%），工具使用越频繁推理越不连贯，约55%高风险案例存在TIM问题。

Conclusion: 工具使用虽然提高答案准确率，但损害推理质量，需要重新调整TaLMs将工具作为辅助证据而非推理替代品，提出的优化框架能同时改善答案准确率和推理深度。
...
TL;DR: EMSQA是一个包含24.3K个问题的医疗问答数据集，涵盖10个临床学科和4个认证级别。研究者提出了Expert-CoT和ExpertRAG方法，通过结合特定临床领域知识和认证级别来提升LLMs在医疗问答中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs在医疗问答中往往忽略领域专业知识（如临床学科和认证级别），限制了在高风险环境中的表现。

Method: 开发了EMSQA数据集，并提出Expert-CoT（基于特定临床学科和认证级别的思维链提示）和ExpertRAG（基于学科对齐文档和真实患者数据的检索增强生成管道）。

Result: 在4个LLMs上的实验显示，Expert-CoT比普通CoT提升2.05%，结合ExpertRAG比标准RAG基线提升4.59%准确率。32B专业增强LLMs通过了所有EMS认证模拟考试。

Conclusion: 结合领域专业知识的提示和检索策略能显著提升LLMs在医疗问答中的性能，特别是在高风险认证环境中。
...
TL;DR: 提出一个基于多模态大语言模型的交互式学术同行评审系统，通过整合文本和视觉信息、基于OpenReview数据的检索增强生成，以及将评审意见转化为结构化待办清单，帮助作者在论文提交前进行有效修订。
...
Method: 开发交互式web系统，集成多模态LLM处理文本和视觉信息，使用基于OpenReview数据的RAG提升评审质量，并通过Action:Objective[#]格式将评审转化为结构化待办清单。

Result: 实验结果表明，该系统能生成更全面、更有用的符合专家标准的评审意见，优于消融基线，推进了透明、以人为本的学术辅助。

Conclusion: 该系统成功实现了多模态、社区感知的同行评审模拟，为学术写作提供了有效的实时反馈和修订跟踪，显著提升了预提交阶段的论文质量。
...
TL;DR: 本文评估了四种先进大语言模型在罕见疾病诊断任务上的表现，使用从《豪斯医生》剧集提取的176个症状-诊断对作为数据集，发现模型准确率在16.48%到38.64%之间，新一代模型性能提升2.3倍。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在罕见疾病诊断方面的能力，目前这一领域研究不足，需要建立评估基准来推动AI辅助诊断研究。

Method: 使用从《豪斯医生》剧集提取的176个症状-诊断对数据集，评估GPT 4o mini、GPT 5 mini、Gemini 2.5 Flash和Gemini 2.5 Pro四种大语言模型在叙述性医疗推理任务上的表现。

Result: 模型表现存在显著差异，准确率范围为16.48%到38.64%，新一代模型相比旧代模型性能提升2.3倍，但所有模型在罕见疾病诊断方面都面临重大挑战。

Conclusion: 虽然大语言模型在罕见疾病诊断方面仍面临困难，但跨架构的性能改进显示了未来发展的有希望方向，该教育验证的基准为叙述性医疗推理建立了基线性能指标，并为推进AI辅助诊断研究提供了公开可访问的评估框架。
...
TL;DR: 本研究开发了CardioEmbed，一个基于Qwen3-Embedding-8B的心血管领域专用嵌入模型，通过对比学习在7本心脏病学教科书上训练，在心脏特异性语义检索任务中达到99.60%的检索准确率，比当前最佳医学嵌入模型MedTE提升15.94个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有的生物医学文本嵌入模型主要基于PubMed研究文献开发，而临床心脏病学实践严重依赖程序性知识和专业术语，这些内容主要存在于综合教科书中而非研究摘要中，这种研究实践差距限制了现有嵌入模型在心脏病学临床应用中的有效性。

Method: 基于Qwen3-Embedding-8B模型，使用对比学习方法，在精心策划的7本综合心脏病学教科书语料库（约150,000个去重句子）上进行训练，采用带批内负样本的InfoNCE损失函数。

Result: 在心脏特异性语义检索任务中达到99.60%的检索准确率，比MedTE模型提升15.94个百分点；在MTEB医学基准测试中，BIOSSES获得0.77 Spearman相关系数，SciFact获得0.61 NDCG@10，在相关生物医学领域表现出竞争力。
...
TL;DR: DiscoX是一个用于评估专业领域中文到英文翻译的新基准，包含7个领域的200篇专业文本，平均长度超过1700个词元。同时开发了Metric-S无参考评估系统，在准确性、流畅性和适当性方面提供细粒度自动评估。实验显示即使最先进的LLM在专业翻译任务上仍落后于人类专家。


<details>
  <summary>Details</summary>
Motivation: 当前翻译评估方法主要关注片段级准确性和流畅性，而专业领域翻译需要篇章级连贯性和严格的术语精确性，现有评估方法在这方面存在不足。

Method: 引入DiscoX基准，包含200篇专业领域中文文本及其英文翻译；开发Metric-S无参考评估系统，提供准确性、流畅性和适当性的细粒度自动评估。

Result: Metric-S与人类判断具有强一致性，显著优于现有指标；实验显示最先进的LLM在专业翻译任务上仍明显落后于人类专家。

Conclusion: DiscoX基准和评估系统为更严格的翻译评估提供了稳健框架，验证了实现专业级机器翻译仍面临挑战，有助于推动基于LLM的翻译未来发展。
...
TL;DR: 本文对主流开源DPO数据集进行了首次全面的数据中心分析，使用Magpie框架标注样本质量，并基于分析结果构建了新的DPO混合数据集UltraMix，该数据集比最佳单个数据集小30%但性能更优。


<details>
  <summary>Details</summary>
Motivation: 现有DPO数据集缺乏系统性比较，主要由于计算成本高和缺乏丰富质量标注，难以理解偏好选择方式、任务类型覆盖范围以及样本级别的人类判断准确性。

Method: 利用Magpie框架对每个样本进行任务类别、输入质量和偏好奖励的标注，实现可扩展的细粒度偏好质量检查；基于分析结果从五个语料库中精选构建UltraMix混合数据集。

Result: 揭示了不同数据集在奖励边际上的结构和质量差异；UltraMix比最佳单个数据集小30%但在关键基准测试中表现更优。

Conclusion: 通过数据中心方法可以构建更高效的DPO数据集，为未来数据中心偏好优化研究提供了有价值的资源。
...
TL;DR: 提出了一种增强基于自动机的结构化生成多样性的新方法，通过利用自动机遍历历史来引导LLM生成新颖的结构模式，显著提高了结构多样性和内容多样性。
...
Method: 利用自动机遍历历史来引导LLMs朝向新颖的结构模式，增强基于自动机的结构化生成的多样性。

Result: 评估显示该方法显著提高了结构多样性和内容多样性，同时保持了相当的生成效率。案例研究展示了该方法在生成开源库测试用例方面的有效性。

Conclusion: 该方法成功解决了结构化生成中多样性不足的问题，在保持效率的同时显著提升了输出多样性，在测试用例生成等实际应用中表现出色。
...
### [133] [Correcting Mean Bias in Text Embeddings: A Refined Renormalization with Training-Free Improvements on MMTEB](https://arxiv.org/abs/2511.11041)
*Xingyu Ren,Youran Sun,Haoyu Liang*

Main category: cs.CL

TL;DR: 本文发现当前文本嵌入模型存在一致的偏见，并提出了一种无需训练、轻量级的重归一化方法来解决这个问题。该方法通过减去嵌入向量中的公共偏置项，在MMTEB基准测试中显著提升了各种任务的性能。


<details>
  <summary>Details</summary>
Motivation: 发现现有文本嵌入模型的输出存在系统性偏见，所有句子的嵌入向量都包含一个几乎相同的偏置项，这影响了模型性能。

Method: 提出重归一化方法，有两种变体：直接减去偏置向量μ，或减去嵌入向量在μ方向上的投影。该方法无需额外训练，可直接应用于现有模型。

Result: 在38个模型上的实验表明，重归一化显著提升了性能：检索任务提升9.7σ，分类任务提升3.1σ，其他任务提升0.8σ。理论预测和实验都证实投影方法效果更好。

Conclusion: 重归一化是一种有效解决文本嵌入模型偏见的简单方法，能显著提升模型在各种任务上的表现，且理论预测与实验结果一致。
...
### [134] [Can LLMs Detect Their Own Hallucinations?](https://arxiv.org/abs/2511.11087)
*Sora Kadotani,Kosuke Nishida,Kyosuke Nishida*

Main category: cs.CL

TL;DR: 本文研究LLMs能否检测自身的幻觉，提出基于CoT的分类方法，实验显示GPT-3.5 Turbo能检测58.2%的自身幻觉。


<details>
  <summary>Details</summary>
Motivation: 大语言模型能生成流畅回答但有时会产生事实幻觉，需要研究LLMs是否能够检测自身的幻觉问题。

Method: 将幻觉检测制定为句子分类任务，提出使用思维链(CoT)从模型参数中提取知识的分类框架。

Result: 实验结果表明，GPT-3.5 Turbo配合CoT能够检测58.2%的自身幻觉。
...
### [135] [Analysing Personal Attacks in U.S. Presidential Debates](https://arxiv.org/abs/2511.11108)
...
<details>
  <summary>Details</summary>
Motivation: 人身攻击已成为美国总统辩论的显著特征，在选举期间对公众认知产生重要影响。检测此类攻击可以提高政治话语的透明度，为记者、分析师和公众提供见解。深度学习和transformer模型的发展为有害语言的自动检测创造了新机会。

Method: 手动标注2016、2020和2024年选举周期的辩论文本，然后进行统计和基于语言模型的分析，研究微调transformer模型和通用大语言模型在检测正式政治演讲中人身攻击的潜力。

Result: 研究表明现代语言模型的任务特定适应可以为政治传播的深入理解做出贡献。

Conclusion: 任务特定适应的现代语言模型能够有效检测政治辩论中的人身攻击，为政治传播分析提供了新的技术手段。
...
TL;DR: 提出了SAP²方法，一种两阶段动态修剪和整合相关上下文关键词的新框架，用于解决ASR系统在需要领域特定知识的语境化场景中利用长上下文信息的挑战。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别系统在常见条件下表现优异，但在需要领域特定知识的语境化场景（如会议演示）中难以有效利用长上下文信息，主要受限于模型上下文窗口和广泛上下文噪声中相关信息稀疏的问题。

Method: SAP²方法采用两阶段动态修剪和整合相关上下文关键词的框架，每个阶段利用提出的语音驱动注意力池化机制，有效压缩上下文嵌入同时保留语音显著信息。

Result: 在SlideSpeech和LibriSpeech数据集上实现了最先进的性能，词错误率分别为7.71%和1.12%。在SlideSpeech上，相比非上下文基线，有偏关键词错误率显著降低了41.1%。

Conclusion: SAP²方法在广泛上下文输入条件下在两个数据集上都表现出稳健的可扩展性，能够持续保持性能，有效解决了ASR系统在语境化场景中利用长上下文信息的挑战。
...
TL;DR: 本文针对WordNet中副词语义分类不足的问题，提出了一个基于语言学理论的副词超义类型学，并通过实证标注验证了其有效性。
...
Method: 提出了基于语言学理论的副词超义类型学，包括方式、时间、频率、程度、领域、说话者导向和主语导向等主要语义域，并通过试点标注研究进行实证验证。
...
TL;DR: 提出LANE对抗训练策略，通过选择性标记训练集中的替代词来生成挑战性负例，使模型关注目标词的细粒度语义差异，提升词义分辨能力。


<details>
  <summary>Details</summary>
Motivation: 神经语言模型在处理细粒度词义时容易过拟合到全局句子表示，无法捕捉局部语义细节，需要改进词义分辨能力。

Method: LANE对抗训练策略，通过选择性标记替代词生成负例，强制模型区分相同句子中不同标记词的语义差异。

Result: 在词汇语义变化检测和词义消歧基准测试中表现优于标准对比学习方法，产生更具区分性的词表示。

Conclusion: LANE方法能有效提升词表示的区分性，更好地捕捉细微语义差异，且模型无关，可集成到现有表示学习框架中。
...
TL;DR: 本文提出了一种从知识图谱生成问答对的可扩展确定性流水线，结合LLM进行模板精化，通过聚类三元组、创建可重用模板、LLM精化模板和引入干扰项等步骤，高效生成高质量的问答对。


<details>
  <summary>Details</summary>
Motivation: 现有方法在可扩展性、语言质量和事实一致性方面存在不足，需要开发一种既能保持事实准确性又能提升语言质量的问答生成方法。

Method: 首先基于关系对KG三元组进行聚类，创建可重用的自然语言模板；然后使用LLM精化模板以提升清晰度和连贯性；最后通过从KG中选择干扰项来实例化答案选项。

Result: 实验证明这种混合方法能够高效生成高质量的问答对，兼具可扩展性、流畅性和语言精确性。

Conclusion: 提出的混合流水线方法成功解决了现有问答生成方法在可扩展性和语言质量方面的挑战，为教育平台和LLM开发提供了有效的解决方案。
...
Method: iMAD首先让单个智能体生成结构化自我批判响应，提取41个可解释的语言和语义特征，然后使用轻量级辩论决策分类器决定是否触发多智能体辩论。

Result: 在六个视觉问答数据集上的实验表明，iMAD显著减少了token使用量（最高达92%），同时提高了最终答案准确率（最高达13.5%）。

Conclusion: iMAD通过智能选择性地触发辩论，实现了计算效率和准确性的双重提升，为多智能体辩论系统提供了更实用的解决方案。
...
TL;DR: 本文开发了一种针对最先进机器学习模型的新型对抗攻击策略，通过生成模糊输入来混淆模型，重点关注孟加拉语领域的对抗攻击，同时严格保持效用使用减少和效率。


<details>
  <summary>Details</summary>
Motivation: 近年来机器学习模型被发现存在多种脆弱性，使模型及其应用系统面临风险。本文旨在分析和实验现有最佳对抗攻击方法，并创建新的攻击策略。

Method: 开发具有最大困惑度的对抗实例，利用机器学习和深度学习方法欺骗模型。分析多个数据集，专注于创建模糊的对抗样本来使模型处于困惑状态，并将孟加拉语纳入对抗攻击领域。

Result: 成功开发了针对当前最先进机器学习模型的新型对抗攻击策略，能够有效混淆模型并使其处于困惑状态。

Conclusion: 通过开发具有高困惑度的对抗攻击方法，为未来模型鲁棒性的发展提供了路径，同时严格遵循效用使用减少和效率原则。
...
TL;DR: 提出了LAET（层自适应集成调优）策略，通过选择性微调预训练LLM中最有效的层来减少计算开销，同时在金融NLP任务中提升性能，甚至优于GPT-4等大型模型。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在金融NLP应用中的高计算需求问题，使更多组织能够使用先进的NLP技术。

Method: LAET策略通过分析隐藏状态表示，选择性地微调预训练LLM中最有效的层，同时冻结不太关键的层。

Result: LAET在金融NLP任务中表现出色，超越了现有基准和最先进的LLM（如GPT-4），即使使用较小的LLM（约30亿参数）也能取得良好结果。

Conclusion: 该工作通过高效可扩展的模型，将前沿的金融NLP研究与实际部署连接起来，为金融应用提供了实用的解决方案。
...
TL;DR: 本文介绍了M-DAIGT共享任务，旨在检测多个领域（特别是新闻文章和学术写作）中的AI生成文本，包含两个子任务：新闻文章检测和学术写作检测，并发布了包含3万样本的大规模基准数据集。
...
Method: 创建包含3万样本的平衡数据集（人类撰写和AI生成），使用多种现代LLM（如GPT-4、Claude）和多样化提示策略生成AI内容，组织46个团队参与共享任务。

Result: 共有46个团队注册，4个团队提交了最终结果，所有团队都参与了两个子任务。

Conclusion: M-DAIGT为AI生成文本检测提供了重要基准，并讨论了未来的发展方向。
...
TL;DR: MajinBook是一个开放目录，通过整合影子图书馆和Goodreads的元数据，创建了一个包含53.9万本英文书籍的高精度语料库，涵盖三个世纪的数据，包含出版日期、类型和流行度指标。


<details>
  <summary>Details</summary>
Motivation: 旨在促进影子图书馆在计算社会科学和文化分析中的应用，解决传统语料库的偏见问题，为研究提供高质量的机器可读文本数据。

Method: 将影子图书馆的元数据与Goodreads的结构化书目数据链接，优先使用原生数字EPUB文件确保质量，并评估链接策略的准确性。

Result: 创建了包含53.9万本英文书籍的语料库，涵盖三个世纪，包含出版日期、类型、评分和评论等丰富元数据，并提供了法语、德语和西班牙语的辅助数据集。

Conclusion: MajinBook为计算社会科学研究提供了高质量的数据资源，项目在法律框架下开放所有底层数据，支持文本和数据挖掘研究。
...
TL;DR: 提出了一种主动式听力辅助系统，通过双模型架构实时识别和分离佩戴者的对话伙伴，无需显式提示。


<details>
  <summary>Details</summary>
Motivation: 开发能够主动适应对话动态的听力辅助设备，利用佩戴者的自我语音作为锚点，基于对话轮换行为推断对话伙伴并抑制其他声音。

Method: 使用双模型架构：轻量级流式模型每12.5毫秒运行一次用于低延迟提取对话伙伴，较慢模型较少运行以捕获长距离对话动态。系统基于自我中心双耳音频工作。

Result: 在真实世界2人和3人对话测试集上验证，收集了11名参与者总计6.8小时的数据，显示在多对话环境中识别和隔离对话伙伴的泛化能力。

Conclusion: 这项工作是向能够主动适应对话动态和参与度的听力辅助设备迈出的重要一步。
...
Method: 招募182名合格专业人士（拥有法学博士、特许金融分析师资格或6年以上经验）编写基于实际工作流程的任务，建立专家制定的评分标准并通过严格的质量验证流程。

Result: 评估20个领先模型显示仍有很大改进空间，金融和法律困难子集的最高得分分别为0.39和0.37。模型在具体能力上表现差异显著，常见失败模式包括判断不准确、缺乏过程透明度和推理不完整。

Conclusion: 当前模型在专业应用中的可靠性存在关键差距，需要改进判断准确性、过程透明度和推理完整性才能满足专业采用的要求。
...
### [148] [AccKV: Towards Efficient Audio-Video LLMs Inference via Adaptive-Focusing and Cross-Calibration KV Cache Optimization](https://arxiv.org/abs/2511.11106)
*Zhonghua Jiang,Kui Chen,Kunxi Li,Keting Yin,Yiyun Zhou,Zhaode Wang,Chengfei Lv,Shengyu Zhang*

Main category: cs.MM

TL;DR: AccKV是一个针对音频-视频大语言模型的高效推理优化框架，通过自适应聚焦和交叉校准技术来优化KV缓存管理，解决多模态处理中的信息混淆和性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有的AV-LLMs在处理音频和视频时面临KV缓存过大、模态间注意力分配不均以及信息混淆等问题，导致计算效率低下和性能下降。

Method: 提出AccKV框架，包含层自适应聚焦技术（根据层级特性选择关键模态）和交叉校准技术（整合低效KV缓存并选择性淘汰低优先级模态的缓存）。

Result: 实验结果表明AccKV能显著提升AV-LLMs的计算效率，同时保持准确性。

Conclusion: AccKV通过智能的KV缓存优化策略，有效解决了多模态大语言模型在音频-视频处理中的效率瓶颈问题。
...
### [149] [Picking a Representative Set of Solutions in Multiobjective Optimization: Axioms, Algorithms, and Experiments](https://arxiv.org/abs/2511.10716)
*Niclas Boehmer,Maximilian T. Wittmann*

Main category: cs.AI

TL;DR: 本文研究多目标优化中的Pareto修剪问题，通过将其重构为多赢家投票问题，分析了现有质量度量方法的不足，提出了新的定向覆盖度量方法，并研究了各种质量度量的计算复杂性。
...
Method: 将Pareto修剪问题重构为多赢家投票问题，进行公理分析；引入新的定向覆盖度量；分析各种质量度量的计算复杂性；进行实验评估。

Result: 发现现有质量度量方法存在不直观行为；提出的定向覆盖度量在各种设置下表现竞争力强或更优；识别了可处理和难处理案例之间的边界。

Conclusion: 质量度量的选择对所选解集的特征有决定性影响，新提出的定向覆盖度量在多种设置下表现优异，为多目标决策提供了更好的解决方案。
...
### [150] [Structure-Aware Encodings of Argumentation Properties for Clique-width](https://arxiv.org/abs/2511.10767)
*Yasir Mahmood,Markus Hecher,Johanna Groven,Johannes K. Fichte*

Main category: cs.AI

TL;DR: 本文研究了图参数clique-width在编码中的应用，特别针对抽象论证框架，设计了保持clique-width线性的归约方法，将论证问题转化为(Q)SAT问题。


<details>
  <summary>Details</summary>
Motivation: 现代SAT求解器在小树宽实例上表现高效，但更一般的图参数clique-width在编码方面的研究较少。抽象论证框架作为基于有向图的推理系统，是研究计算性质的理想候选。

Method: 设计了从论证问题到(Q)SAT的新归约方法，这些归约线性保持clique-width，称为有向分解引导(DDG)归约。

Result: 为所有论证语义（包括计数）建立了新结果，证明DDG归约的开销在合理假设下无法显著改进。

Conclusion: 本文开启了理解clique-width编码能力的研究方向，为抽象论证问题提供了高效的(Q)SAT编码方法，且证明了归约效率的最优性。
...
### [151] [Potential Outcome Rankings for Counterfactual Decision Making](https://arxiv.org/abs/2511.10776)
...
<details>
  <summary>Details</summary>
Motivation: 在反事实决策中，决策者通常通过比较候选行动的期望潜在结果来做出选择。本文旨在开发新的决策规则，通过引入PoR和PoB度量来更有效地评估行动偏好。

Method: 建立了PoR和PoB的识别定理和边界推导，并提出了相应的估计方法。通过数值实验验证估计器的有限样本性质，并在真实数据集上进行应用演示。

Result: 成功开发了PoR和PoB两种新的反事实决策度量，建立了理论框架并提供了实用的估计方法。数值实验验证了估计器的良好性能。

Conclusion: PoR和PoB为反事实决策提供了新的分析工具，能够更准确地评估个体层面的行动偏好，在不确定决策环境中具有重要应用价值。
...
### [152] [Multi-agent Undercover Gaming: Hallucination Removal via Counterfactual Test for Multimodal Reasoning](https://arxiv.org/abs/2511.11182)
*Dayong Liang,Xiao-Yong Wei,Changmeng Zheng*

Main category: cs.AI

TL;DR: 本文提出MUG协议来解决LLM中的幻觉问题，通过多智能体卧底游戏机制，使用反事实测试检测幻觉智能体，提升多模态推理的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体辩论方法假设所有辩论者都是理性的，但现实情况是智能体本身也会产生幻觉，因此需要更可靠的检测机制。

Method: 引入多智能体卧底游戏协议，通过修改参考图像引入反事实证据，观察智能体是否能准确识别这些变化，从而检测幻觉智能体。

Result: MUG协议在三个关键维度上改进了多智能体辩论：实现基于反事实测试的事实验证、引入跨证据推理、促进主动推理。

Conclusion: MUG为LLM的多模态推理提供了一个更可靠有效的框架，通过反事实测试和主动推理机制显著提升了推理的可靠性。
...
TL;DR: 本综述从自适应性的角度重新审视大语言模型的推理能力，提出根据任务难度和不确定性动态分配推理努力的概念框架，系统分类了基于训练和无训练的自适应推理方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM推理方法采用统一的推理策略，无法根据任务复杂度调整推理努力，导致简单问题过度推理而复杂问题推理不足。需要建立自适应推理框架来优化计算资源分配。

Method: 提出自适应推理的形式化定义，将其建模为控制增强的策略优化问题，平衡任务性能与计算成本。构建系统分类法，将方法分为基于训练的方法（强化学习、监督微调、学习控制器）和无训练方法（提示条件、反馈驱动停止、模块化组合）。

Result: 建立了连接经典认知范式与算法实现的理论框架，澄清了不同机制在实践中如何实现自适应推理，并实现了跨策略的系统比较。

Conclusion: 自适应推理是提升LLM推理效率的关键方向，未来需要在自我评估、元推理和人类对齐推理控制等方面解决开放挑战。
...
Method: 提出关系特定的空间加权策略，通过学习注意力机制动态选择每个关系类型的最优几何空间，并使用多空间一致性损失确保跨空间预测的连贯性。

Result: 在从1K论文到10M论文的知识图谱上评估，相比TransE、RotatE等基线方法获得一致改进。在10M论文数据集上达到0.612 MRR，相对最佳基线提升4.8%，推理速度85ms/三元组。

Conclusion: HyperComplEx通过自适应维度分配实现近线性扩展，在保持高效训练的同时显著提升了知识图谱嵌入性能，为可扩展知识图谱嵌入研究提供了可复现的实现和数据集。
...
### [155] [Advanced Tool for Traffic Crash Analysis: An AI-Driven Multi-Agent Approach to Pre-Crash Reconstruction](https://arxiv.org/abs/2511.10853)
*Gerui Xu,Boyou Chen,Huizhong Guo,Dave LeBlanc,Ananna Ahmed,Zhaonan Sun,Shan Bao*

Main category: cs.AI

TL;DR: 本研究开发了一个多智能体AI框架，用于从碎片化的碰撞数据中重建事故前场景和推断车辆行为。该框架在277起追尾前车减速事故中测试，在39起复杂案例中实现100%准确率，超越了人类专家92%的准确率。


<details>
  <summary>Details</summary>
Motivation: 传统的交通事故重建依赖人工专家，在处理不完整多模态数据时往往产生不一致结果。需要开发AI系统来提高碰撞重建的准确性和一致性。

Method: 采用两阶段协作框架：第一阶段从多模态输入生成自然语言事故重建；第二阶段结合重建结果与时序事件数据记录器进行深度推理。处理文本事故报告、结构化表格数据和视觉场景图。

Result: 在39起复杂追尾事故案例中，框架实现了100%准确率，成功识别最相关EDR事件并正确区分撞击与被撞车辆，超越了人类研究人员92%的准确率。即使在处理不完整数据时也保持稳健性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法过于简化脑连接性的复杂关系，假设单一模态的脑连接组作为疾病传播基质，导致病理传播预测不准确；而纯数据驱动的方法由于缺乏适当约束面临可识别性问题。
...
Conclusion: 该框架通过结合LLMs的专家知识和生物约束，有效解决了神经退行性疾病进展建模中的可识别性问题，提供了更准确的病理传播预测和更好的生物学解释性。
...
TL;DR: 提出多智能体法律验证器，将合规检查分解为专门的法律解释、业务情境评估和风险评估智能体，通过结构化合成协议协调，在200个日本个人信息保护法修正案第16条案例上实现72%准确率，比单智能体基线提高21个百分点。


<details>
  <summary>Details</summary>
Motivation: 在严格的隐私法规（如日本个人信息保护法APPI）下，AI驱动的数据传输规划中的法律合规性变得越来越关键，需要开发可扩展且法规感知的自动化合规验证框架。

Method: 采用多智能体法律验证器，将合规检查分解为三个专门智能体：法规解释、业务情境评估和风险评估，通过结构化合成协议进行协调。

Result: 在200个分层APPl第16条修正案案例数据集上，系统达到72%准确率，比单智能体基线提高21个百分点，在明确合规案例上达到90%准确率（基线为16%），同时保持对明显违规的完美检测。

Conclusion: 领域专业化和协调推理可以显著提高法律AI性能，为可信赖和可解释的自动化合规验证提供了可扩展且法规感知的框架，但在模糊场景中仍存在挑战。
...
TL;DR: 该论文研究自主AI系统在遇到没有完全满足所有操作约束的场景时，如何构建、评估和证明候选行动方案，以实现与人类期望和价值观一致的目标。


<details>
  <summary>Details</summary>
Motivation: 尽管经过广泛训练，自主AI系统仍会遇到没有可用行动方案完全满足所有操作约束的场景，需要超越训练策略来做出符合人类期望的决策。

Method: 通过分析和实证案例研究，探讨智能体如何整合规范性、实用性和情境性理解来选择更符合期望的行动方案。

Result: 识别了智能体在复杂现实环境中做出稳健决策所需的知识类型，以及决策制定的要求。

Conclusion: 智能体需要整合多种知识类型来选择和追求更符合期望的行动方案，以实现与人类价值观一致的目标。
...
### [159] [Faster Symmetry Breaking Constraints for Abstract Structures](https://arxiv.org/abs/2511.11029)
*Özgür Akgün,Mun See Chang,Ian P. Gent,Christopher Jefferson*

Main category: cs.AI

TL;DR: 提出了一种新的不完整方法来打破抽象结构的对称性，通过更好地利用其表示来处理约束编程中常见的不可区分对象对称性问题。


<details>
  <summary>Details</summary>
Motivation: 在约束编程中，抽象结构（如嵌套集合）需要转换为求解器支持的表示形式，而对称性破坏技术通常会产生大量复杂约束，在实践中性能较差。

Method: 开发了一种新的不完整对称性破坏方法，通过更好地利用抽象结构的表示来处理不可区分对象产生的对称性。

Result: 与之前的方法相比，新方法在速度上表现更优，比(Akgün et al. 2025)中提出的方法更快。

Conclusion: 该方法为约束编程中抽象结构的对称性破坏提供了一种更高效的解决方案，特别适用于处理不可区分对象产生的对称性问题。
...
### [160] [Key Decision-Makers in Multi-Agent Debates: Who Holds the Power?](https://arxiv.org/abs/2511.11040)
*Qian Zhang,Yan Zheng,Jinyi Liu,Hebin Liang,Lanjun Wang*

Main category: cs.AI

TL;DR: 研究发现角色分配策略对多智能体辩论性能有显著影响，提出'真理最后'策略可提升22%性能，并开发了MADC策略来优化多智能体辩论机制。


<details>
  <summary>Details</summary>
Motivation: 多智能体辩论在提升推理能力方面具有潜力，但角色分配策略这一关键方面尚未得到充分探索。

Method: 提出'真理最后'角色分配策略，并开发多智能体辩论一致性策略，通过路径一致性评估独立角色间的一致性，模拟最高一致性得分的角色作为真理。

Result: 在9个LLM模型上验证，MADC策略持续表现出先进性能，有效克服了MAD的性能瓶颈，推理任务性能提升达22%。
...
### [161] [Autonomous Vehicle Path Planning by Searching With Differentiable Simulation](https://arxiv.org/abs/2511.11043)
*Asen Nachkov,Jan-Nico Zaech,Danda Pani Paudel,Xi Wang,Luc Van Gool*

Main category: cs.AI

TL;DR: DSS框架利用可微分模拟器Waymax作为状态预测器和评估器，通过梯度下降优化动作序列，在自动驾驶规划中显著提升了跟踪和路径规划精度。
...
Method: 提出DSS框架，利用可微分模拟器Waymax作为状态预测器和评估器，通过梯度下降优化动作序列，结合规划梯度和随机搜索。

Result: 实验表明，DSS在跟踪和路径规划精度上显著优于序列预测、模仿学习、无模型RL和其他规划方法。

Conclusion: DSS框架通过结合可微分模拟器和梯度优化，为自动驾驶规划提供了一种有效的解决方案。
...
### [162] [Satisficing and Optimal Generalised Planning via Goal Regression (Extended Version)](https://arxiv.org/abs/2511.11095)
*Dillon Z. Chen,Till Hofmann,Toryn Q. Klassen,Sheila A. McIlraith*

Main category: cs.AI

TL;DR: 本文提出了一种新颖的广义规划方法，通过从训练问题中学习条件-动作规则来构建可重用的通用计划，该方法在合成成本、规划覆盖率和解决方案质量方面优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 广义规划的目标是合成能够解决相关规划问题族的程序，但现有方法在效率和可扩展性方面存在局限，需要更简单有效的方法。

Method: 对每个训练问题，按顺序计算每个目标原子的最优计划，执行目标回归，并将输出提升为一阶条件→动作规则集合，这些规则构成可直接执行或用于修剪规划搜索空间的通用计划。

Result: 实验表明，该方法在经典和数值规划领域的合成成本、规划覆盖率和解决方案质量三个指标上显著优于现有最先进的（广义）规划器。

Conclusion: 提出的方法能够有效学习有效的广义规划和状态空间修剪公理，为广义规划问题提供了一种简单而强大的解决方案。
...
### [163] [GGBench: A Geometric Generative Reasoning Benchmark for Unified Multimodal Models](https://arxiv.org/abs/2511.11134)
*Jingxuan Wei,Caijun Jia,Xi Bai,Xinglong Xu,Siyuan Li,Linzhuang Sun,Bihui Yu,Conghui He,Lijun Wu,Cheng Tan*

Main category: cs.AI

TL;DR: GGBench是一个专门评估几何生成推理的基准测试，旨在填补统一多模态模型在生成推理评估方面的空白，通过几何构造任务来测试模型的语言理解和精确视觉生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估判别性理解或无约束图像生成，但无法衡量生成推理的整合认知过程。几何构造任务需要语言理解和精确视觉生成的融合，为评估统一多模态模型的生成推理能力提供了理想测试平台。

Method: 提出GGBench基准测试，专门用于评估几何生成推理能力。该基准提供了一个系统化诊断模型理解、推理和主动构建解决方案能力的综合框架。

Result: GGBench为下一代智能系统设定了更严格的标准，能够全面评估模型在几何构造任务中的生成推理表现。

Conclusion: 几何构造是评估统一多模态模型生成推理能力的理想测试平台，GGBench基准填补了现有评估方法的空白，为智能系统的进一步发展提供了重要工具。
...
### [164] [STaR: Towards Cognitive Table Reasoning via Slow-Thinking Large Language Models](https://arxiv.org/abs/2511.11233)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLMs在表格推理中存在推理过程缺乏深度迭代和稳定性不足的问题，影响其在结构化数据分析中的可靠性。

Method: 提出STaR框架，通过显式建模逐步思考和不确定性感知推理来增强LLMs的慢思考能力，采用两阶段难度感知强化学习训练，并在推理时进行轨迹级不确定性量化。

Result: 在基准测试中，STaR实现了优越的性能和增强的推理稳定性，并在跨域数据集上表现出强大的泛化能力。

Conclusion: STaR为LLMs表格推理提供了一个可靠且受认知启发的解决方案，具有实际应用潜力。
...
### [165] [AIonopedia: an LLM agent orchestrating multimodal learning for ionic liquid discovery](https://arxiv.org/abs/2511.11257)
*Yuqi Yin,Yibo Fu,Siyuan Wang,Peng Sun,Hongyu Wang,Xiaohui Wang,Lei Zheng,Zhiyong Li,Zhirong Liu,Jianji Wang,Zhaoxi Sun*

Main category: cs.AI

TL;DR: AIonopedia是第一个基于大语言模型的离子液体发现智能体，通过多模态领域基础模型实现准确性质预测和分层搜索架构，在真实湿实验验证中表现出优异的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 离子液体的发现面临性质预测困难、数据有限、模型精度不足和工作流程碎片化等挑战，需要开发更有效的AI驱动发现方法。

Method: 开发基于大语言模型的多模态领域基础模型，结合分层搜索架构进行分子筛选和设计，使用新构建的全面离子液体数据集进行训练和评估。

Result: 模型在综合数据集上表现优异，在文献报道系统的评估中显示能有效进行离子液体修饰，真实湿实验验证证实了其在实际应用中的泛化能力。

Conclusion: AIonopedia能够显著加速真实世界中的离子液体发现过程，特别是在具有挑战性的分布外任务上表现出色。
...
TL;DR: 本文提出了一种基于机密计算技术的AI决策可追溯性解决方案，通过强制记录训练和推理过程中的所有组件，生成防篡改、可验证的完整决策痕迹。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在决策过程文档化方面存在不足，这阻碍了决策责任链的追溯能力，特别是在AI决策可能违反法律或损害人权的情况下，需要能够在法庭上站得住脚的文档记录。

Method: 扩展DBOM概念，利用机密计算技术构建有效的工作流程，强制记录AI训练和推理过程中的每一个组件，支持生成防篡改、可验证的完整决策痕迹。

Result: 开发了首个支持生成防篡改、可验证且详尽的AI决策痕迹的运行工作流程，并通过区分有毒和可食用蘑菇的应用程序演示了工作流程的内部机制。

Conclusion: 该方法为AI决策提供了实用的可追溯性解决方案，能够有效支持责任链的构建和法律合规要求。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法只能单独解释为什么某个个体属于某个概念（正蕴含）或为什么不属于（缺失蕴含），但无法同时对比解释两个个体之间的差异。本文旨在填补这一空白，提供能够解释"为什么a是C而b不是"的对比性解释方法。

Method: 针对描述逻辑本体论中的ABox推理，开发了对比解释的适当概念。分析了在不同最优性标准下各种变体的计算复杂性，涵盖了轻量级和更表达性的描述逻辑。实现了一种计算对比解释变体的方法，并在现实知识库的生成问题上进行了评估。

Result: 提出了对比ABox解释的形式化定义，并针对不同描述逻辑（从轻量级到表达性更强的逻辑）分析了各种变体在不同最优性标准下的计算复杂性。实现的方法在现实知识库生成的问题上得到了验证。

Conclusion: 对比ABox解释为知识库推理提供了新的解释能力，能够同时考虑正蕴含和缺失蕴含，聚焦于个体间的相关差异。该方法在不同描述逻辑下具有不同的计算复杂性特征，为实际应用提供了理论基础和实现方案。
...
TL;DR: RLSLM是一个结合强化学习和基于规则的社会运动模型的混合框架，通过在奖励函数中整合基于行为实验的社会舒适度场，实现社会对齐的导航策略，在用户体验上优于现有方法。
...
Result: 通过沉浸式VR实验证明RLSLM在用户体验上优于最先进的基于规则模型，消融和敏感性分析显示相比传统数据驱动方法显著提高了可解释性。

Conclusion: 该工作提出了一个可扩展的、以人为中心的方法论，有效整合认知科学和机器学习，用于现实世界的社会导航应用。
...
TL;DR: 本调查系统回顾了在多智能体强化学习中，面对现实通信约束（如消息扰动、传输延迟和有限带宽）时，鲁棒且高效的通信策略的最新进展。重点关注了三个应用领域：协作自动驾驶、分布式同时定位与地图构建以及联邦学习，并指出了关键开放挑战和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 现有MARL方法大多假设通信是瞬时、可靠且带宽无限的，但这些条件在现实部署中很少满足。因此需要研究在现实约束下的鲁棒和高效通信策略。

Method: 系统调查方法，分析在消息扰动、传输延迟和有限带宽等现实约束下的通信策略，重点关注协作自动驾驶、分布式SLAM和联邦学习三个应用领域。

Result: 识别了低延迟可靠性、带宽密集型数据共享和通信-隐私权衡等核心挑战，提出了需要统一方法共同设计通信、学习和鲁棒性的观点。
...
TL;DR: CURENet是一个多模态模型，通过整合非结构化临床笔记、实验室检测和患者时间序列数据，利用LLM处理临床文本和文本实验室检测，以及transformer编码器处理纵向序列就诊数据，实现了对慢性疾病的高精度预测。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录包含多种数据类型，但现有预测模型往往无法充分捕捉多模态数据之间的交互、冗余和时间模式，限制了临床决策的准确性。

Method: 使用大型语言模型处理临床文本和文本实验室检测，transformer编码器处理纵向序列就诊数据，整合多种EHR数据模态。

Result: 在MIMIC-III和FEMH数据集上，CURENet在预测前10种慢性疾病的多标签框架中达到了超过94%的准确率。
...
TL;DR: 本文提出了一种基于模型引导策略调整的测试时对齐技术，能够在不重新训练智能体的前提下，精确控制个体行为属性，在伦理对齐和奖励最大化之间实现原则性权衡。


<details>
  <summary>Details</summary>
Motivation: 在复杂动态环境中部署决策AI智能体时，保持与人类价值观或准则的对齐是一个关键挑战。仅训练实现目标的智能体可能采取有害行为，暴露出最大化奖励函数与保持对齐之间的关键权衡。对于预训练智能体，确保对齐尤其具有挑战性，因为重新训练成本高且过程缓慢。

Method: 提出基于模型引导策略调整的测试时对齐技术。首先在各自游戏中训练RL智能体以最大化奖励，然后在测试时通过场景-动作属性分类器应用策略调整，确保决策与伦理属性对齐。

Result: 在包含134个基于文本的游戏环境和数千个涉及伦理决策的标注场景的MACHIAVELLI基准上进行评估。结果表明，测试时策略调整为缓解不同环境和对齐属性中的不道德行为提供了有效且可扩展的解决方案。

Conclusion: 测试时策略调整能够在不重新训练智能体的情况下，有效控制AI智能体的行为属性，在伦理对齐和奖励最大化之间实现良好平衡，为解决AI对齐问题提供了实用方法。
...
### [172] [Boosting Neural Video Representation via Online Structural Reparameterization](https://arxiv.org/abs/2511.11071)
*Ziyi Li,Qingyu Mao,Shuai Liu,Qilei Li,Fanyang Meng,Yongsheng Liang*

Main category: eess.IV

TL;DR: 本文提出了Online-RepNeRV，一种基于在线结构重参数化的神经视频表示框架，通过增强模型容量来提升视频压缩性能，同时保持训练和推理效率。
...
Method: 提出通用重参数化块ERB，包含多个并行卷积路径增强容量；采用在线重参数化策略在训练时动态融合参数，训练后将多分支结构等价转换为单分支结构。

Result: 在主流视频数据集上的实验表明，相比基线方法平均PSNR增益达0.37-2.7 dB，同时保持可比的训练时间和解码速度。

Conclusion: Online-RepNeRV通过在线结构重参数化有效提升了神经视频表示的性能，在不影响解码效率的前提下显著改善了压缩质量。
...
### [173] [Sensitivity of Finite Element Models to Relationship Between T2 Relaxation and Modulus in Articular Cartilage](https://arxiv.org/abs/2511.10947)
*Alexander A. Donabedian,Deva D. Chan*

Main category: eess.IV

TL;DR: 该研究评估了有限元模型对T2-动态模量关系不确定性的敏感性，发现模量偏移会导致计算应力和应变的显著变化，而斜率变化影响可忽略。
...
Method: 通过改变用于定义软骨特性的线性T2-动态模量关系的斜率和截距，评估有限元模型对这些变化的敏感性。

Result: 模量偏移导致计算应力和应变前1%的显著百分比变化，而斜率调节的影响可忽略不计。

Conclusion: 支持在个体特异性模型中使用生理相关的模量范围。
...
### [174] [Large-scale modality-invariant foundation models for brain MRI analysis: Application to lesion segmentation](https://arxiv.org/abs/2511.11311)
*Petros Koutsouvelis,Matej Gazda,Leroy Volmer,Sina Amirrajab,Kamil Barbierik,Branislav Setlak,Jakub Gazda,Peter Drotar*

Main category: eess.IV

TL;DR: 该论文提出了一种模态不变表示学习方法，用于脑部MRI数据的自监督学习，并在中风和癫痫病灶分割任务中评估其效果。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉领域正转向大规模基础模型的自监督学习预训练，但大多数自监督学习框架针对自然图像设计，在捕捉多模态MRI信息方面研究不足。

Method: 提出模态不变表示学习设置，在大规模预训练后评估其在中风和癫痫病灶分割任务中的有效性。

Result: 实验结果表明，尽管成功实现了跨模态对齐，但病灶分割主要受益于保留细粒度的模态特定特征。

Conclusion: 模型检查点和代码已公开提供，强调了在脑部MRI分析中保留模态特定特征的重要性。
...
### [175] [DualVision ArthroNav: Investigating Opportunities to Enhance Localization and Reconstruction in Image-based Arthroscopy Navigation via External Cameras](https://arxiv.org/abs/2511.10699)
...
TL;DR: DualVision ArthroNav是一个多相机关节镜导航系统，通过集成外部相机解决单目SLAM的尺度模糊和长期漂移问题，实现稳健的关节镜导航。


<details>
  <summary>Details</summary>
Motivation: 现有光学跟踪系统存在工作空间限制和手术流程干扰问题，而基于视觉的替代方案容易受到漂移、尺度模糊和快速运动的影响。

Method: 系统将外部相机刚性安装在关节镜上，外部相机提供稳定的视觉里程计和绝对定位，单目关节镜视频实现密集场景重建，结合这两种互补视图。

Result: 系统平均绝对轨迹误差为1.09毫米，重建场景的平均目标配准误差为2.16毫米，具有高视觉保真度（SSIM=0.69，PSNR=22.19）。
