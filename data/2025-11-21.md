...
TL;DR: UniFit是一个基于多模态大语言模型的通用虚拟试穿框架，通过语义对齐模块解决文本指令与参考图像之间的语义鸿沟问题，并采用两阶段训练策略在数据有限的情况下学习复杂任务。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本指令的多任务虚拟试穿框架存在两个主要问题：(1) 文本指令与参考图像之间的语义鸿沟；(2) 复杂场景下的数据稀缺问题。

Method: 提出UniFit框架，包含MLLM引导的语义对齐模块（MGSA），通过多模态输入集成和语义对齐损失来捕获跨模态语义关系。采用两阶段渐进训练策略和自合成流程。

Result: 实验表明UniFit不仅支持多种虚拟试穿任务（包括多服装和模型间试穿），而且达到了最先进的性能水平。

Conclusion: UniFit通过MLLM驱动的语义对齐和渐进训练策略，成功解决了虚拟试穿中的语义鸿沟和数据稀缺问题，实现了通用且高性能的虚拟试穿框架。
...
### [2] [SAM2S: Segment Anything in Surgical Videos via Semantic Long-term Tracking](https://arxiv.org/abs/2511.16618)
*Haofeng Liu,Ziyue Wang,Sudhanshu Mishra,Mingqi Gao,Guanyi Qin,Chang Han Low,Alex Y. W. Kong,Yueming Jin*

Main category: cs.CV

TL;DR: 本文提出了SAM2S模型，通过构建最大的手术视频分割基准SA-SV，并引入多样记忆机制、时序语义学习和抗模糊学习，显著提升了手术视频中交互式视频对象分割的性能。
...
Conclusion: SAM2S在手术视频分割任务上表现出色，为计算机辅助手术提供了有效的交互式视频对象分割解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决6D物体姿态估计在遮挡情况下的性能下降问题，传统多阶段流水线在遮挡时早期阶段容易出错，误差会传播到后续处理。

Method: 提出四种扩展方法：动态非均匀密集采样策略、多假设推理机制、迭代优化和遮挡训练增强，以及新的基于可见性的加权评估指标。

Result: 在ICBIN数据集上精度提升超过5%，在BOP数据集上提升超过2%，同时推理速度提升约3倍。

Conclusion: 所提出的方法有效提升了遮挡情况下6D姿态估计的鲁棒性和准确性，同时显著加快了推理速度。
...
### [4] [Adaptive Guided Upsampling for Low-light Image Enhancement](https://arxiv.org/abs/2511.16623)
*Angela Vivian Dcosta,Chunbo Song,Rafael Radkowski*

Main category: cs.CV

TL;DR: 提出自适应引导上采样(AGU)方法，通过多参数优化同时提升低光照图像的多个质量特性，如降噪和增强锐度，在实时处理中优于现有方法。
...
Method: 基于引导图像方法，通过多参数优化学习低光照与明亮图像特征之间的关联，仅需少量样本图像对即可训练机器学习模型。

Result: AGU能够使用低质量、低分辨率输入实时渲染高质量图像，在低光照场景下优于现有最先进方法。

Conclusion: AGU方法通过多参数优化有效解决了低光照图像引导上采样的挑战，实现了实时高质量图像渲染。
...
TL;DR: Box6D是一种面向仓储场景的类别级6D姿态估计方法，通过单次RGB-D观测快速推断箱子尺寸并使用类别CAD模板估计姿态，在保持竞争性精度的同时显著降低推理时间。


<details>
  <summary>Details</summary>
Motivation: 解决仓储自动化、拣选等场景中现有6D姿态估计方法的局限性：基于模型的方法需要精确CAD模型且泛化性差；无模型方法在复杂条件下容易失败；类别级方法过于通用且忽略环境先验。

Method: 使用快速二分搜索从单次RGB-D观测推断箱子尺寸，基于类别CAD模板而非实例特定模型估计姿态，采用深度合理性过滤器和早停策略拒绝不合理假设。

Result: 在真实仓储场景和公开基准测试中，Box6D在保持竞争性或更优6D姿态精度的同时，将推理时间减少约76%。

Conclusion: Box6D为仓储环境中的箱子6D姿态估计提供了一种高效准确的解决方案，平衡了灵活性和精度需求，具有实际工业应用价值。
...
TL;DR: 提出一种两阶段自改进范式，通过自生成详细文本理由来弥合视觉语言模型在领域特定视频分类中的语义差距，无需额外标注即可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在领域特定视频分类任务中表现不佳，特别是在数据有限的情况下，存在关键的理由差距——稀疏的领域数据不足以弥合复杂时空内容与抽象分类标签之间的语义距离。

Method: 两阶段自改进范式：第一阶段提示VLMs为每个视频生成详细文本理由，然后在这些自生成理由上微调模型；第二阶段在任务标签上进行常规监督微调。

Result: 在多个数据集上的广泛实验表明，该方法显著优于直接监督微调，验证了自生成理由作为适应领域特定视频分析的有效、标注高效的范式。

Conclusion: 自生成理由是一种有效的、标注高效的范式，能够帮助视觉语言模型适应领域特定的视频分析任务，显著提升性能。
...
<details>
  <summary>Details</summary>
Motivation: CLIP等图像-文本预训练模型主要关注单标签单粒度对齐，但在医学影像等复杂领域，图像通常对应多个高级标签和不同注释粒度，现有方法效果有限。
...
Conclusion: MGLL作为即插即用模块，能有效提升视觉语言模型在复杂领域中的多标签和跨粒度对齐能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统心脏超声依赖2D视频，而3D超声心动图具有更好的图像质量，但医生习惯使用2D视图。该方法旨在让医生在保持习惯格式的同时受益于3D扫描的速度和可用性。
...
Conclusion: 该方法成功实现了从3D体积中提取标准2D视图，使临床医生能够从3D体积中获得准确的真实世界解释，同时发布了代码和数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在有限可见类别上微调CLIP进行分割会导致过拟合，并破坏预训练好的视觉-语言对齐。需要一种稳定模态对齐的微调方法。

Method: 采用信息论视角，通过两个互信息目标：压缩像素-文本模态对齐以减少噪声，最大化预训练CLIP与微调模型对齐知识间的互信息来传递适合分割任务的局部语义关系。
...
TL;DR: 提出一种基于一致性正则化的多任务学习方法，通过可微分的BI-RADS形态特征来缓解乳腺超声肿瘤分割和分类任务之间的破坏性干扰，显著提升了分割任务在外部数据集上的泛化性能。
...
Method: 提出新颖的一致性正则化方法，使用可微分的BI-RADS启发形态特征来缓解分割和分类任务之间的破坏性干扰。在BrEaST数据集上训练，在三个外部数据集上进行验证。

Result: 在三个外部数据集上，提出的多任务方法相比基线方法在分割任务上取得了统计显著的改进：UDIAT（0.81 vs 0.59）、BUSI（0.66 vs 0.56）、BUS-UCLM（0.69 vs 0.49），p<0.001。在UDIAT数据集上达到了最先进的分割性能。

Conclusion: 所提出的一致性正则化方法有效缓解了多任务学习中的破坏性干扰，显著提升了乳腺超声肿瘤分割任务在外部验证中的泛化性能，为多任务学习在医学图像分析中的应用提供了有效解决方案。
...
TL;DR: 提出了一种检测引导的生成式框架，通过预测分层类别和属性token来实现视觉语义理解，在细粒度类别区分和属性识别方面优于现有方法。
...
Method: 检测引导的生成式框架，提取精炼的ROI级特征，使用BART生成器从粗到细生成语义token，涵盖类别层次结构和属性值对。

Result: 在大型电商数据集和开源数据集上的实验表明，该方法显著优于现有的相似性基线和多阶段分类系统。

Conclusion: 该方法实现了更强的细粒度识别能力和更连贯的统一推理，为视觉语义理解提供了有效解决方案。
...
### [14] [Physically Realistic Sequence-Level Adversarial Clothing for Robust Human-Detection Evasion](https://arxiv.org/abs/2511.16020)
...
Method: 将产品图像映射到UV空间并转换为紧凑的调色板和控制点参数化，使用物理基础的人体-服装管道模拟运动、多角度视角、布料动力学和光照变化，通过期望变换目标与时序加权优化控制点。

Result: 实验显示具有强大稳定的隐蔽性、对视角变化的高鲁棒性以及优越的跨模型可迁移性，物理服装在室内外录制中实现可靠抑制。

Conclusion: 该方法证实了在真实世界环境中的可行性，为可穿戴对抗攻击提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有真实世界图像超分辨率方法主要依赖基于LoRA的预训练扩散模型微调，但这些密集模型难以自适应捕捉复杂退化样本的异质特征，也无法在同等计算预算下实现输入间的知识共享。

Method: 提出混合秩架构，将LoRA中的每个秩作为独立专家；采用细粒度专家划分策略，固定位置秩作为共享专家；开发基于CLIP嵌入的退化估计模块；引入零专家槽和退化感知负载平衡损失。

Result: 综合实验验证了该框架的有效性和最先进的性能表现。

Conclusion: MoR架构成功解决了真实世界图像超分辨率中自适应捕捉异质特征和知识共享的问题，实现了优异的性能。
...
TL;DR: CuriGS是一个基于课程学习的稀疏视图3D高斯溅射重建框架，通过引入学生视图和渐进式训练策略解决稀疏视图下的过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在稀疏视图设置下面临监督稀缺和视角覆盖不足导致的过拟合挑战，需要有效的方法来增强训练数据。

Method: 提出课程引导框架，为每个真实视角（教师）生成多组不同扰动水平的学生视图，通过渐进式解锁扰动级别、深度相关正则化和多信号评估来增强训练。

Result: 实验结果表明CuriGS在渲染保真度和几何一致性方面优于现有最先进方法，适用于各种合成和真实稀疏视图场景。

Conclusion: CuriGS通过课程学习策略有效解决了稀疏视图3D重建中的过拟合问题，显著提升了重建质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前植物表型分析中，多传感器平台广泛用于收集作物冠层反射数据。无人机在作物监测、预测任务中表现出色，但卫星任务受限于空间分辨率，难以满足现代微地块管理需求。因此需要开发能够结合两者优势的方法。
...
Conclusion: 跨模态对应学习具有弥合卫星和无人机传感在农业监测中差距的潜力，能够显著提升作物性状估计的准确性。
...
TL;DR: 本文提出一个基于大语言模型(LLMs)的框架来解决食品识别中的领域偏移、长尾分布和细粒度分类挑战，通过LLMs解析食品图像生成标题和成分，将文本和图像投影到共享嵌入空间进行对齐，并在两个食品数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决食品识别中的三个主要挑战：训练样本与真实环境图像之间的领域偏移问题、长尾数据分布问题，以及不同类别食品间视觉差异细微难以区分的问题。

Method: 1. 利用LLMs解析食品图像生成食品标题和成分；2. 将生成的文本和来自不同领域的食品图像投影到共享嵌入空间以最大化配对相似度；3. 使用对齐后的多模态特征进行识别。

Result: 在两个食品数据集上，该方法在长尾数据分布、领域适应和细粒度分类方面均优于现有的专门方法。

Conclusion: 基于LLMs的简单框架能够有效解决食品识别中的多重挑战，通过文本-图像特征对齐显著提升了识别性能。
...
TL;DR: AMS-KV是一种针对视觉自回归模型(VAR)的尺度自适应KV缓存策略，通过优先存储压缩尺度和局部尺度的KV，显著减少KV缓存使用84.83%，降低自注意力延迟60.48%，并提升模型可扩展性。


<details>
  <summary>Details</summary>
Motivation: 传统VAR模型在下一尺度预测时KV内存随尺度数量增长而急剧增加，严重限制了模型的可扩展性，需要设计专门的KV缓存策略来解决这一瓶颈。

Method: 基于系统研究发现：(1)局部尺度注意力对生成质量重要；(2)压缩尺度内存分配稳定多尺度生成；(3)缓存需求层间尺度相似性弱。提出AMS-KV策略，优先存储压缩和局部尺度的KV，并通过尺度间相似性分析优化缓存利用。

Result: 相比原始VAR模型，AMS-KV减少KV缓存使用84.83%，降低自注意力延迟60.48%。在VAR-d30模型基线出现内存不足时，AMS-KV能稳定扩展到批量大小256，并提升吞吐量。

Conclusion: AMS-KV通过尺度自适应KV缓存策略有效解决了VAR模型在下一尺度预测时的内存瓶颈问题，显著提升了模型的效率和可扩展性。
...
TL;DR: LiSTAR是一个直接在传感器原生几何上操作的生成世界模型，通过混合圆柱-球面表示减少量化伪影，使用时空注意力机制建模动态场景，并支持可控的4D LiDAR数据合成。
...
Method: 提出混合圆柱-球面表示来保持数据保真度；使用基于射线中心的时空注意力变换器建模特征演化；开发4D点云对齐的体素布局和掩码生成框架，实现可控合成。
...
TL;DR: VideoSeg-R1是首个将强化学习引入视频推理分割的框架，采用解耦架构将任务分解为联合参考图像分割和视频掩码传播，包含分层文本引导帧采样、推理模型生成空间线索和显式推理链、以及使用SAM2和XMem的分割传播三个阶段，通过任务难度感知机制自适应控制推理长度以提高效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统视频推理分割方法依赖监督微调，限制了在分布外场景的泛化能力且缺乏显式推理。为解决这些问题，提出了VideoSeg-R1框架。
...
Conclusion: VideoSeg-R1是首个将强化学习引入视频推理分割的框架，通过解耦架构和任务难度感知机制，在复杂视频推理分割任务中取得了优异性能。
...
TL;DR: SpectralTrain是一个通用的、架构无关的训练框架，通过结合课程学习和基于PCA的光谱降采样，显著提高高光谱图像分类的训练效率，在保持准确性的同时实现2-7倍的训练加速。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类通常涉及大规模数据和计算密集型训练，限制了深度学习模型在实际遥感任务中的部署。需要一种能够降低计算成本同时保持性能的训练方法。

Method: 集成课程学习与基于PCA的光谱降采样，逐步引入光谱复杂性同时保留关键信息，实现高效的光谱-空间模式学习。该框架与特定架构、优化器或损失函数无关。

Result: 在三个基准数据集上的实验表明，训练时间减少2-7倍，准确率变化较小。在云分类等气候相关遥感任务中展现出潜力。

Conclusion: 训练策略优化是HSI模型架构设计的有效补充，SpectralTrain框架在保持性能的同时显著提高了训练效率。
...
TL;DR: Rad-GS是一个用于千米级室外环境的4D雷达-相机SLAM系统，使用3D高斯作为可微分空间表示，结合雷达点云和多普勒信息来指导动态对象掩码，提高定位精度和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够在千米级室外环境中进行稳健建图的系统，利用4D毫米波雷达的优势，解决传统基于相机或LiDAR的方法在大规模环境中的局限性。

Method: 结合原始雷达点云与多普勒信息以及几何增强点云，指导同步图像中的动态对象掩码；利用非同步图像帧全局优化3D高斯表示；采用全局八叉树结构和目标高斯基元管理策略来抑制噪声并减少内存消耗。

Result: 在广泛实验和消融研究中，Rad-GS实现了与基于相机或LiDAR的传统3D高斯方法相当的性能，验证了使用4D毫米波雷达进行稳健室外建图的可行性。

Conclusion: Rad-GS展示了在千米级真实世界重建中的潜力，为大规模场景重建提供了一种有效的解决方案。
...
TL;DR: 本文提出了T2T-VICL框架，研究视觉语言模型在不同视觉任务之间的跨任务视觉上下文学习能力，通过文本提示生成和选择机制，结合感知评分推理实现跨任务VICL。


<details>
  <summary>Details</summary>
Motivation: 探索视觉语言模型在不同视觉任务之间进行视觉上下文学习的潜力，解决当视觉提示和目标图像来自不同视觉任务时VICL是否仍然有效的问题。

Method: 提出T2T-VICL全协作流水线，设计文本提示生成和选择机制来描述不同低层视觉任务间的差异，构建首个跨任务VICL数据集，并开发结合感知评分推理和传统评估指标的新型推理框架。

Result: 在9个跨任务场景中取得顶级结果，在另外10个场景中取得次顶级表现，突破了VLMs中跨任务VICL的边界。

Conclusion: T2T-VICL框架成功解锁了视觉语言模型在跨任务视觉上下文学习方面的潜力，证明了在不同视觉任务间进行VICL的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的4D高斯泼溅方法在重建动态场景时存在困难，无法准确解决模糊像素对应关系，且在动态区域密度不足。

Method: 采用两个关键组件：1）椭圆误差聚类和误差校正泼溅添加，精确定位动态区域并初始化拟合泼溅；2）分组4D高斯泼溅，提高泼溅与动态对象映射的一致性。具体包括将渲染误差分类为缺失颜色和遮挡类型，并通过跨视图颜色一致性指导进行反向投影或前景分割的针对性校正。
...
Conclusion: 该方法有效解决了4D高斯泼溅在动态场景重建中的关键问题，通过创新的误差聚类和分组技术显著提升了渲染质量和时间一致性。
...
TL;DR: 提出DCS-LDM，一种新的视觉生成范式，将信息复杂度与尺度解耦，构建分层、尺度无关的潜在空间，支持固定潜在表示下任意分辨率和帧率的解码。
...
Method: 构建分层、尺度无关的潜在空间，通过多级token建模样本复杂度，支持在固定潜在表示下解码到任意分辨率和帧率。通过跨层级分解结构和细节信息，支持渐进式由粗到细的生成范式。

Result: 实验结果表明，DCS-LDM在提供跨多种尺度和视觉质量的灵活生成的同时，实现了与最先进方法相当的性能。

Conclusion: DCS-LDM通过解耦信息复杂度与尺度，实现了灵活的计算-质量权衡，支持渐进式生成，在保持性能的同时提供跨尺度的生成灵活性。
...
TL;DR: 本文提出了多模态交互欺骗评估(MIDA)任务，发现当前最先进的多模态大语言模型在识别复杂社交互动中的欺骗行为方面存在显著缺陷，并开发了社交思维链(SoCoT)和动态社交认知记忆(DSEM)模块来提升性能。
...
Method: 引入MIDA任务和包含同步视频文本及真实标签的新数据集，评估12个最先进的开源和闭源MLLMs，并设计了社交思维链推理流程和动态社交认知记忆模块。

Result: 基准测试显示显著性能差距，即使GPT-4o等强大模型也难以可靠区分真假。新框架在该挑战性任务上带来了性能提升。

Conclusion: 当前MLLMs无法有效将语言与多模态社交线索关联，缺乏建模他人知识、信念或意图的能力，需要新方法来构建更具洞察力和可信度的AI系统。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中量化参数(QP)可能部分或完全未知，限制了依赖已知QP的非盲方法的适用性，需要开发盲QECV技术。现有盲方法生成的退化向量仅捕获全局退化信息，缺乏空间细节，且所有QP使用统一架构忽略了不同压缩级别的计算需求差异。
...
Method: 提出空间优先最优传输分配来动态强调位置良好且空间可靠的样本，以及排名感知自适应自蒸馏方案通过自蒸馏范式自适应注入排名感知。

Result: 在ScanNet V2和SUN RGB-D数据集上的广泛实验表明，SR3D有效弥合了训练-推理差距，在保持实时速度的同时显著优于先前方法的准确性。

Conclusion: SR3D框架成功解决了3D目标检测中的训练-推理不一致问题，实现了更准确的检测性能同时保持实时处理能力。
...
TL;DR: 本文提出了一种新的高空间分辨率水体变化检测数据集HSRW-CD，并设计了SSCP注意力模块来充分利用深度特征中的空间语义和结构信息，显著提升了水体变化检测的性能。


<details>
  <summary>Details</summary>
Motivation: 当前高空间分辨率水体变化检测数据集的稀缺限制了其在城乡区域的应用，同时现有深度学习方法未能充分利用深度特征中的空间语义和结构信息。

Method: 提出了SSCP注意力模块，包含三个组件：多语义空间注意力(MSA)、结构关系感知全局注意力(SRGA)和通道自注意力(CSA)，通过增强空间语义和结构信息来提升水体检测能力。

Result: 在提出的HSRW-CD和Water-CD数据集上的大量实验验证了SSCP的有效性和泛化能力。

Conclusion: SSCP模块作为即插即用的组件，可以集成到现有的水体变化检测模型中，显著提升水体检测的判别能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS SLAM系统缺乏开放词汇语义理解能力，无法满足高级机器人交互需求。集成语言特征面临内存消耗大、渲染开销高的问题，而静态模型方法缺乏对新环境的适应性。

Method: 提出场景自适应编码器-解码器，将高维语言嵌入压缩到紧凑的16维特征空间；采用语言引导的剪枝策略识别语义冗余；引入基于语言的闭环检测方法，复用映射特征。

Result: 减少了每个高斯的存储内存，加速渲染，实现15FPS实时性能；高斯数量减少超过60%同时保持渲染质量；在映射质量和跟踪精度方面具有竞争力。

Conclusion: LEGO-SLAM成功解决了3DGS SLAM中的语义集成挑战，在保持实时性能的同时提供了开放词汇能力，为高级机器人交互奠定了基础。
...
TL;DR: 本文提出推理引导嵌入(RGE)方法，通过将MLLMs的生成推理能力融入嵌入提取过程，提升多模态表示质量。
...
Method: 提出RGE方法，保留MLLMs的生成推理过程并与对比训练结合，先进行结构化推理生成，然后在推理展开后提取表示。

Result: 在MMEB基准测试中，推理引导的条件化使多模态检索性能比非推理基线提高4.9%。

Conclusion: 显式推理能有效增强嵌入质量，推理引导的嵌入提取方法优于传统直接编码方法。
...
TL;DR: SurvAgent是首个用于多模态生存预测的分层思维链增强多智能体系统，通过两阶段方法整合病理图像和基因数据，在TCGA数据集上优于传统方法和现有医疗智能体。
...
Method: 两阶段方法：1）WSI-Gene CoT增强案例库构建，通过低倍镜筛查、跨模态相似性感知补丁挖掘和置信度感知补丁挖掘分析病理图像，基因分层分析处理六种功能基因类别；2）基于二分法的多专家智能体推理，通过RAG检索相似案例并整合多模态报告。

Result: 在五个TCGA队列上的广泛实验表明，SurvAgent优于传统方法、专有MLLM和医疗智能体，为精准肿瘤学中的可解释AI驱动生存预测建立了新范式。

Conclusion: SurvAgent通过分层CoT增强的多智能体系统成功解决了多模态数据整合、区域探索和经验学习等关键挑战，为临床可解释的生存预测提供了有效解决方案。
...
TL;DR: 本文提出PPCL框架，通过可插拔剪枝和连续层蒸馏技术，在保持图像生成质量的同时将DiT模型参数量减少50%。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiTs)在图像生成方面表现出色，但参数量大导致计算成本高，限制了在资源受限环境中的部署。

Method: 首先通过线性探测和一阶微分趋势分析识别冗余层区间，然后提出可插拔的师生交替蒸馏方案，在单一训练阶段内集成深度和宽度剪枝。

Result: 在多模态扩散变换器架构上的实验表明，PPCL相比完整模型减少50%参数，关键客观指标下降小于3%，保持高质量图像生成能力。

Conclusion: 该方法在实现更高压缩比的同时保持高质量图像生成，适合资源受限环境部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉生成方法要么在生成前进行文本推理（作为预规划），要么在生成后进行（作为后精炼），但缺乏在生成过程中的实时多模态交互。
...
### [36] [Video2Layout: Recall and Reconstruct Metric-Grounded Cognitive Map for Spatial Reasoning](https://arxiv.org/abs/2511.16160)
...
Conclusion: 连续边界坐标表示能有效提升多模态大语言模型的空间推理精度，缓解自然语言描述空间关系时的固有模糊性。
...
### [37] [Simba: Towards High-Fidelity and Geometrically-Consistent Point Cloud Completion via Transformation Diffusion](https://arxiv.org/abs/2511.16161)
*Lirui Zhang,Zhengkai Zhao,Zhi Zuo,Pan Gao,Jie Qin*

Main category: cs.CV

TL;DR: Simba框架将点云补全中的逐点变换回归重新表述为分布学习问题，结合对称先验和扩散模型的生成能力，通过分层Mamba架构实现高保真上采样，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决点云补全中同时保留输入细节和确保全局结构完整性的挑战，特别是现有回归方法容易过拟合、记忆实例特定变换而非学习通用几何先验，以及对输入噪声高度敏感的问题。

Method: 将逐点变换回归重新表述为分布学习问题，集成对称先验和扩散模型的生成能力，避免实例特定记忆化，同时捕获鲁棒几何结构，并采用分层Mamba架构进行高保真上采样。

Result: 在PCN、ShapeNet和KITTI基准测试上的广泛实验验证了该方法的最先进性能。

Conclusion: Simba框架通过将变换回归重新表述为分布学习问题，结合对称先验和扩散模型，有效解决了点云补全中的过拟合和噪声敏感性问题，实现了优异的性能。
...
### [38] [Layer-wise Noise Guided Selective Wavelet Reconstruction for Robust Medical Image Segmentation](https://arxiv.org/abs/2511.16162)
*Yuting Lu,Ziliang Wang,Weixin Xu,Wei Zhang,Yongqiang Zhao,Yang Yu,Xiaohong Zhang*

Main category: cs.CV

TL;DR: 提出LNG-SWR方法，通过层间噪声引导的选择性小波重构来提升医学图像分割模型的鲁棒性，无需依赖对抗训练即可显著减少强攻击下的性能下降。
...
Method: 在训练时向多个层注入小零均值噪声，学习频率偏置先验；应用先验引导的选择性小波重构进行频率自适应：抑制噪声敏感频带，增强方向结构和形状线索，稳定边界响应，同时保持频谱一致性。
...
### [39] [An Image Is Worth Ten Thousand Words: Verbose-Text Induction Attacks on VLMs](https://arxiv.org/abs/2511.16163)
*Zhi Luo,Zenghui Yuan,Wenqi Wei,Daizong Liu,Pan Zhou*

Main category: cs.CV

TL;DR: 本文提出了一种新的冗长文本诱导攻击(VTIA)，通过两阶段框架向良性图像注入难以察觉的对抗性扰动，以最大化视觉语言模型的输出token长度。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在多模态任务上的成功，其部署效率问题日益突出。现有方法仅延迟EOS token出现来隐式延长输出，缺乏稳定性和可控性，无法直接最大化输出token长度作为明确优化目标。

Method: 采用两阶段框架：1)对抗性提示搜索，使用强化学习策略自动识别能诱导LLM产生冗长输出的对抗性提示；2)视觉对齐扰动优化，在输入图像上构建对抗样本，最大化扰动图像视觉嵌入与对抗性提示的相似度。

Result: 在四种流行视觉语言模型上的综合实验表明，该方法在有效性、效率和泛化能力方面具有显著优势。

Conclusion: 提出的VTIA方法能够有效诱导视觉语言模型产生冗长输出，为模型安全性评估提供了新的视角。
...
### [40] [EvoVLA: Self-Evolving Vision-Language-Action Model](https://arxiv.org/abs/2511.16166)
*Zeting Liu,Zida Yang,Zeyu Zhang,Hao Tang*

Main category: cs.CV

TL;DR: EvoVLA是一个自监督的视觉语言动作框架，通过阶段对齐奖励、基于姿态的对象探索和长时程记忆三个组件，解决了VLA模型在长时程机器人操作中的阶段幻觉问题，在模拟和真实世界中都显著提升了任务成功率。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型存在阶段幻觉问题，即智能体利用粗略评估信号来走捷径完成多步骤任务，报告高进度但未真正完成任务。这限制了长时程机器人操作的实际效果。
...
Conclusion: EvoVLA有效解决了VLA模型中的阶段幻觉问题，实现了从模拟到真实世界的有效迁移和强泛化能力，为长时程机器人操作提供了可行的解决方案。
...
### [41] [Target Refocusing via Attention Redistribution for Open-Vocabulary Semantic Segmentation: An Explainability Perspective](https://arxiv.org/abs/2511.16170)
...
Method: 提出RF-CLIP方法，这是一种无需训练的方法，通过识别和过滤由维度特定过度激活产生的干扰token，将注意力从干扰token重新聚焦到目标区域，从而改进CLIP的多模态对齐粒度。

Result: 在八个基准测试中实现了最先进的性能，同时保持高推理效率。

Conclusion: 通过模拟人类注意力重新聚焦行为，RF-CLIP有效提升了CLIP在开放词汇语义分割中的密集预测能力，证明了注意力机制优化对多模态对齐的重要性。
...
TL;DR: 本文提出了一种新颖的两阶段模型DSLGA，用于解决无监督域自适应可见光-红外人员重识别问题，通过域共享学习和渐进对齐策略来处理域间和域内模态差异。


<details>
  <summary>Details</summary>
Motivation: 由于公共数据集与现实世界数据之间的差异，现有VI-ReID算法在实际应用中表现不佳，因此需要研究无监督域自适应方法，在不降低准确性和无需新样本标注的情况下将知识从公共数据迁移到现实数据。

Method: 设计了两阶段模型DSLGA：预训练阶段使用域共享学习策略缓解域间模态差异，微调阶段使用渐进对齐策略通过聚类到整体对齐方式处理域内跨模态对齐挑战。

Result: 大量实验表明，该方法在各种设置下显著优于现有的VI-ReID域自适应方法，甚至优于一些监督方法。

Conclusion: 提出的DSLGA模型有效解决了UDA-VI-ReID中的域间和域内模态差异问题，构建了新的测试方法CMDA-XD，在性能上取得了显著提升。
...
Result: 在心脏、海马体和肺部验证中，PrIntMesh实现了高几何精度、正确拓扑结构，即使在有限或噪声训练数据下也能保持鲁棒性能。相比体素和基于表面的方法，能更好地重建共享界面并保持结构一致性。

Conclusion: PrIntMesh提供了一种数据高效的解决方案，适合临床应用，能够产生解剖学上合理的器官重建结果。
...
TL;DR: VLA-Fool是一个针对视觉-语言-动作模型在具身环境中的多模态对抗鲁棒性研究，提出了三种攻击方式：文本扰动、视觉扰动和跨模态错位攻击，揭示了多模态对齐的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有的研究主要关注单模态扰动，忽视了跨模态错位对具身推理和决策的根本影响，而VLA模型的对抗鲁棒性在真实多模态和黑盒条件下尚未得到充分探索。

Method: VLA-Fool统一了三种多模态对抗攻击：基于梯度和提示的文本扰动、基于补丁和噪声的视觉扰动、以及故意破坏感知与指令语义对应的跨模态错位攻击，并首次开发了自动构建的语义引导提示框架。
...
### [45] [Building temporally coherent 3D maps with VGGT for memory-efficient Semantic SLAM](https://arxiv.org/abs/2511.16282)
*Gergely Dinya,Péter Halász,András Lőrincz,Kristóf Karacs,Anna Gelencsér-Horváth*

Main category: cs.CV

TL;DR: 提出基于视觉门控生成变换器(VGGT)的快速时空场景理解框架，支持接近实时的辅助导航应用。通过滑动窗口处理图像流和对齐子图来克服VGGT的高内存需求，利用VGGT跟踪头将2D语义实例掩码聚合为3D对象，并保持时间一致性和上下文推理。


<details>
  <summary>Details</summary>
Motivation: 开发高效的实时场景理解系统，特别针对辅助导航应用，需要克服VGGT的高内存需求并实现连续更新的3D场景表示。

Method: 使用滑动窗口处理图像流，对齐子图来管理内存；利用VGGT跟踪头聚合2D语义实例掩码为3D对象；存储时间戳和实例级身份以实现时间一致性和环境变化检测。
...
### [46] [Explainable AI for Diabetic Retinopathy Detection Using Deep Learning with Attention Mechanisms and Fuzzy Logic-Based Interpretability](https://arxiv.org/abs/2511.16294)
...
### [47] [Optimizing 3D Gaussian Splattering for Mobile GPUs](https://arxiv.org/abs/2511.16298)
...
<details>
  <summary>Details</summary>
Motivation: 利用3DGS在3D场景重建中的高效性，结合移动设备部署的优势（数据隐私、离线操作、快速响应），开发针对移动GPU优化的3DGS实现。

Method: 核心是专为2D纹理缓存优化的新型排序算法，通过优化数据处理、移动和布局，结合改进的变量布局设计和其他优化技术来加速3DGS算法。

Result: 端到端评估显示，Texture3dgs在排序方面实现4.1倍加速，整体3D场景重建实现1.7倍加速，同时内存使用减少1.6倍。

Conclusion: Texture3dgs设计有效实现了移动设备上高效的3D场景重建，证明了针对移动GPU纹理缓存优化的价值。
...
### [48] [Upsample Anything: A Simple and Hard to Beat Baseline for Feature Upsampling](https://arxiv.org/abs/2511.16301)
...
<details>
  <summary>Details</summary>
Motivation: 视觉基础模型虽然在下游任务中表现出强大的泛化能力，但其表示通常被下采样14x/16x，限制了在像素级应用中的直接使用。现有特征上采样方法依赖于数据集特定的重新训练或繁重的隐式优化，限制了可扩展性和泛化性。

Method: 采用简单的每图像优化方法，学习一个结合空间和范围线索的各向异性高斯核，有效连接高斯泼溅和联合双边上采样。学习到的核作为通用的边缘感知算子，可在不同架构和模态间无缝迁移。

Result: 该方法在224x224图像上仅需约0.419秒运行时间，在语义分割、深度估计以及深度和概率图上采样任务中达到最先进的性能。
...
### [49] [Sparse Autoencoders are Topic Models](https://arxiv.org/abs/2511.16309)
*Leander Girrbach,Zeynep Akata*

Main category: cs.CV

TL;DR: 本文提出将稀疏自编码器(SAE)重新理解为主题模型，并开发了SAE-TM框架，将SAE特征作为可重用的主题原子，支持跨模态的大规模主题分析。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器在嵌入分析中的作用和实际价值存在争议，作者希望从主题建模的角度重新理解SAE，证明它们可以作为有效的主题分析工具。

Method: 将潜在狄利克雷分配扩展到嵌入空间，推导出SAE目标作为该模型的最大后验估计器；提出SAE-TM框架，训练SAE学习可重用的主题原子，将其解释为下游数据的词分布，无需重新训练即可合并成任意数量的主题。

Result: SAE-TM在文本和图像数据集上比强基线方法产生更连贯的主题，同时保持多样性；成功分析了图像数据集中的主题结构，并追踪了日本木版画中主题随时间的变化。

Conclusion: SAE可以作为跨模态大规模主题分析的有效工具，为嵌入分析提供了新的实用价值。
...
### [50] [NaTex: Seamless Texture Generation as Latent Color Diffusion](https://arxiv.org/abs/2511.16317)
*Zeqiang Lai,Yunfei Zhao,Zibo Zhao,Xin Yang,Xin Huang,Jingwei Huang,Xiangyu Yue,Chunchao Guo*

Main category: cs.CV

TL;DR: NaTex是一个原生纹理生成框架，直接在3D空间中预测纹理颜色，避免了传统基于多视图扩散模型的局限性，通过将纹理视为密集颜色点云的新范式解决了遮挡区域处理、精确网格纹理对齐和跨视图一致性问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于多视图扩散模型的方法存在处理遮挡区域困难、网格纹理边界对齐不精确、跨视图内容和颜色强度不一致等问题，需要新的原生3D纹理生成方法。

Method: 提出潜在颜色扩散方法，包括几何感知的颜色点云VAE和多控制扩散变换器，完全从3D数据训练；引入原生几何控制，通过位置嵌入和几何潜在变量为DiT提供直接3D空间信息；设计VAE-DiT架构，几何分支与颜色VAE紧密耦合。

Result: NaTex在纹理一致性和对齐方面显著优于先前方法，表现出强大的泛化能力，无需训练或简单调优即可应用于材质生成、纹理精炼、部件分割和纹理化等下游任务。

Conclusion: NaTex通过原生3D纹理生成框架成功解决了传统方法的局限性，在纹理生成质量和应用泛化方面取得了显著进展。
...
### [51] [Aerial View River Landform Video segmentation: A Weakly Supervised Context-aware Temporal Consistency Distillation Approach](https://arxiv.org/abs/2511.16343)
...
Result: 仅使用30%标注数据，同时提高了mIoU和时序一致性，确保地形物体的稳定定位。
...
### [52] [CRISTAL: Real-time Camera Registration in Static LiDAR Scans using Neural Rendering](https://arxiv.org/abs/2511.16349)
...
Method: 从预捕获的LiDAR点云渲染合成视图，建立实时帧与点云的2D-3D对应关系，使用神经渲染技术减少合成与真实图像间的域差距，提出在线渲染匹配和预构建定位两种实时变体。

Result: 在ScanNet++数据集上展示改进结果，优于现有SLAM流水线，实现无漂移的度量尺度相机跟踪。
...
### [53] [CAMS: Towards Compositional Zero-Shot Learning via Gated Cross-Attention and Multi-Space Disentanglement](https://arxiv.org/abs/2511.16378)
*Pan Yang,Cheng Deng,Jing Yang,Han Zhao,Yun Liu,Yuling Chen,Xiaoli Ruan,Yanping Chen*

Main category: cs.CV

TL;DR: CAMS提出了一种基于CLIP的组成零样本学习方法，通过门控交叉注意力机制从图像编码器中提取细粒度语义特征，并在多维空间中进行语义解耦，以提升对未见属性-对象组合的泛化能力。
...
Method: 设计了门控交叉注意力机制，通过一组潜在单元从CLIP的高层图像编码块中捕获细粒度语义特征，同时自适应抑制背景和其他无关信息；随后进行多空间解耦，实现属性和对象语义的分离。

Result: 在三个流行基准测试（MIT-States、UT-Zappos和C-GQA）上的实验表明，CAMS在封闭世界和开放世界设置下均达到了最先进的性能。

Conclusion: CAMS通过提取视觉特征中的语义特征并在多维空间中进行语义解耦，有效提升了组成零样本学习的性能，特别是在未见属性-对象组合的识别方面。
...
### [54] [CylinderDepth: Cylindrical Spatial Attention for Multi-View Consistent Self-Supervised Surround Depth Estimation](https://arxiv.org/abs/2511.16428)
*Samer Abualhanud,Christian Grannemann,Max Mehltretter*

Main category: cs.CV

TL;DR: 提出一种几何引导的自监督多摄像头深度估计方法，通过将多视角3D点投影到共享圆柱体上，建立跨图像邻域关系，并使用显式空间注意力聚合特征，实现跨视图一致的密集度量深度估计。


<details>
  <summary>Details</summary>
Motivation: 现有自监督环视深度估计方法在重叠图像间的深度估计不一致，需要解决多摄像头系统下的跨视图深度一致性问题。

Method: 基于标定和时间同步的多摄像头系统，首先为每张图像预测初始深度图，将所有图像的3D点投影到共享圆柱体上建立邻域关系，然后使用非学习的显式空间注意力机制根据圆柱体上的距离聚合跨图像特征，最终预测每张图像的深度图。

Result: 在DDAD和nuScenes数据集上的评估表明，该方法相比现有最优方法显著提高了跨图像深度估计的一致性和整体深度精度。

Conclusion: 提出的几何引导方法能够有效实现多摄像头系统的跨视图一致深度估计，为360°环视感知提供了密集、度量和一致的3D感知能力。
...
### [55] [Graph Neural Networks for Surgical Scene Segmentation](https://arxiv.org/abs/2511.16430)
*Yihan Li,Nikhil Churamani,Maria Robu,Imanol Luengo,Danail Stoyanov*

Main category: cs.CV

TL;DR: 本文提出了两种结合Vision Transformer和Graph Neural Networks的分割模型，用于增强腹腔镜胆囊切除术中肝胆解剖结构的识别精度，解决遮挡、长距离依赖和罕见结构几何特征捕获的挑战。


<details>
  <summary>Details</summary>
Motivation: 准确识别肝胆解剖结构对于预防腹腔镜胆囊切除术中的手术并发症至关重要。深度学习模型在处理遮挡、长距离依赖和捕获罕见结构的精细几何特征方面存在困难。

Method: 提出了两种分割模型：(1) 静态k最近邻图与GCNII结合，实现稳定的长距离信息传播；(2) 动态可微分图生成器与GAT结合，支持自适应拓扑学习。两种模型均在Endoscapes-Seg50和CholecSeg8k基准上进行评估。

Result: 所提出的方法在mIoU上比最先进基线提高了7-8%，在mDice上提高了6%。特别是在薄、罕见和安全关键结构上产生了解剖学上一致的预测。

Conclusion: 提出的基于图的分割方法在手术场景分割中提高了性能和解剖一致性。通过将ViT的全局上下文与基于图的关系推理相结合，模型提高了可解释性和可靠性，为通过精确识别关键解剖特征实现更安全的腹腔镜和机器人辅助手术铺平了道路。
...
TL;DR: 本文提出了一种语言驱动的属性泛化（LDAG）架构，通过大语言模型生成目标类的多属性描述，构建无偏见的支持策略，以解决小样本分割中支持图像因类内变化导致元指导不准确的问题。


<details>
  <summary>Details</summary>
Motivation: 现有小样本分割方法主要从支持图像中提取元指导信息，但由于视觉表示的类内变化，这些信息无法为未训练类别提供准确指导。作者认为支持图像提供的参考信息可能不是必需的，关键在于为训练和未训练类别提供无偏见的元指导。

Method: 设计了语言驱动的属性泛化架构，包括多属性增强（MaE）模块和多模态属性对齐（MaA）模块。MaE通过大语言模型生成目标类的多个详细属性描述，构建精炼的视觉-文本先验指导；MaA实现属性文本与视觉特征之间的跨模态交互。

Result: 实验表明，该方法明显优于现有方法，达到了新的最先进性能。

Conclusion: 利用目标属性语言描述构建鲁棒的支持策略，可以有效解决小样本分割中的类内变化问题，实现更好的分割性能。
...
### [57] [StreetView-Waste: A Multi-Task Dataset for Urban Waste Management](https://arxiv.org/abs/2511.16440)
...
Method: 创建包含垃圾和垃圾容器的城市场景数据集，支持三个评估任务；为每个任务提供最先进模型的基准；提出两种改进策略：基于启发式的垃圾容器跟踪方法和利用几何先验的模型无关分割框架。
...
### [58] [VLA-Pruner: Temporal-Aware Dual-Level Visual Token Pruning for Efficient Vision-Language-Action Inference](https://arxiv.org/abs/2511.16449)
*Ziyan Liu,Yeqiu Chen,Hongyi Cai,Tao Lin,Shuo Yang,Zheng Liu,Bo Zhao*

Main category: cs.CV

TL;DR: VLA-Pruner是一种专门针对视觉-语言-动作模型的token剪枝方法，通过双层级重要性标准和自适应token选择策略，在保持语义理解和动作执行性能的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有的VLM token剪枝方法仅基于语义显著性选择token，忽视了VLA模型的双系统特性（高层语义理解和低层动作执行），导致动作生成所需的关键信息被丢弃，严重降低VLA性能。

Method: 提出VLA-Pruner方法，采用双层级重要性标准：视觉-语言预填充注意力用于语义相关性，通过时间平滑估计的动作解码注意力用于动作重要性。基于此标准设计自适应双层级token选择策略。

Result: 实验表明VLA-Pruner在多种VLA架构和多样化机器人任务上实现了最先进的性能。

Conclusion: VLA-Pruner通过考虑VLA模型的双系统特性和时间连续性，有效解决了现有token剪枝方法在VLA任务中的局限性，为实时部署提供了高效解决方案。
...
### [59] [LLaVA$^3$: Representing 3D Scenes like a Cubist Painter to Boost 3D Scene Understanding of VLMs](https://arxiv.org/abs/2511.16454)
...
Method: 从中间多视角3D重建中提取物体的全方位视觉表示，为VLM提供3D场景描述，无需任何微调。

Result: 在3D视觉问答和3D语言定位任务上，该方法优于之前的2D基础VLM解决方案。

Conclusion: 仅使用多视角2D图像即可有效提升VLM的3D场景理解能力，无需额外的3D训练数据或模型微调。
...
### [60] [FastSurfer-CC: A robust, accurate, and comprehensive framework for corpus callosum morphometry](https://arxiv.org/abs/2511.16471)
*Clemens Pollak,Kersten Diers,Santiago Estrada,David Kügler,Martin Reuter*

Main category: cs.CV

TL;DR: FastSurfer-CC是一个高效、全自动的胼胝体形态测量框架，能够自动识别中矢状切片、分割胼胝体和穹窿、定位前后连合以标准化头部位置、生成厚度轮廓和细分，并提取八个形状指标进行统计分析。


<details>
  <summary>Details</summary>
Motivation: 胼胝体是大脑中最大的连合结构，在衰老和神经系统疾病研究中是重点研究对象，也是深部脑刺激等干预措施的关键靶点。尽管已有大量关于胼胝体分割的研究，但很少有公开可用的工具提供全面自动化的分析流程。

Method: 开发了FastSurfer-CC框架，自动执行中矢状切片识别、胼胝体和穹窿分割、前后连合定位、厚度轮廓生成、细分以及形状指标提取等任务。

Result: FastSurfer-CC在各项任务中均优于现有的专用工具，并且能够检测到亨廷顿病患与健康对照组之间的统计学显著差异，而现有最先进方法无法检测到这些差异。
...
### [61] [Flow and Depth Assisted Video Prediction with Latent Transformer](https://arxiv.org/abs/2511.16484)
*Eliyas Suleyman,Paul Henderson,Eksan Firkat,Nicolas Pugeault*

Main category: cs.CV

TL;DR: 该论文研究了在遮挡场景下的视频预测问题，提出通过结合点流（point-flow）和深度图（depth-maps）信息来提升视频预测模型在遮挡和背景运动情况下的性能。


<details>
  <summary>Details</summary>
Motivation: 视频预测是机器人学和世界建模等下游应用的基础任务，但遮挡问题仍然是视频预测的固有挑战。作者假设通过提供明确的运动信息（点流）和几何结构信息（深度图）能够改善模型在遮挡场景下的表现。
...
### [62] [Acquisition Time-Informed Breast Tumor Segmentation from Dynamic Contrast-Enhanced MRI](https://arxiv.org/abs/2511.16498)
*Rui Wang,Yuexi Du,John Lewin,R. Todd Constable,Nicha C. Dvornek*

Main category: cs.CV

TL;DR: 本文提出了一种利用图像采集时间信息来改进乳腺DCE-MRI肿瘤分割的方法，通过特征线性调制层将采集时间信息融入模型，提高了分割性能和泛化能力。
...
Method: 使用特征线性调制层将采集时间信息融入模型，根据特定采集序列调整模型特征，利用每个成像研究中获取的完整、可变数量的图像。

Result: 在大型公共多中心乳腺DCE-MRI数据集上训练，评估显示结合相位采集时间知识改善了肿瘤分割性能和模型泛化能力。

Conclusion: 通过整合图像采集时间信息，可以有效提高乳腺DCE-MRI肿瘤分割的准确性和泛化性能。
...
### [63] [BoxingVI: A Multi-Modal Benchmark for Boxing Action Recognition and Localization](https://arxiv.org/abs/2511.16524)
*Rahul Kumar,Vipul Baghel,Sudhanshu Singh,Bikash Kumar Badatya,Shivam Yadav,Babji Srinivasan,Ravi Hegde*

Main category: cs.CV

TL;DR: 本文提出了一个专门用于拳击击打检测和分类的全面、高质量视频数据集，包含6,915个击打片段，分为六种击打类型，来自20个YouTube训练视频，涉及18名运动员。


<details>
  <summary>Details</summary>
Motivation: 由于格斗运动的动态性和非结构化特性以及录制环境的变化，开发鲁棒的数据集一直是主要瓶颈。本文旨在解决这一问题，为实时视觉动作识别研究提供支持。

Method: 从20个公开的YouTube训练视频中提取击打片段，手动分割和标注以确保精确的时间边界和类别一致性，涵盖各种运动风格、摄像机角度和运动员体型。

Result: 创建了一个包含6,915个高质量击打片段的综合数据集，分为六种不同的击打类型，为拳击动作分析提供了丰富的基准。

Conclusion: 该数据集将加速拳击及相关领域中运动分析、自动化教练和性能评估的进展，特别适用于资源受限和非约束环境下的研究。
...
TL;DR: 本文提出了一种新颖的两阶段合成图像检测框架，通过监督对比学习提取图像嵌入特征，并结合k-NN分类器在少样本学习范式下实现对新生成器的泛化检测，显著提升了检测准确率和源归属性能。
...
Method: 两阶段检测框架：第一阶段使用监督对比学习的视觉深度学习模型提取图像嵌入特征，在训练时策略性地保留部分生成器架构以测试跨生成器泛化能力；第二阶段使用k-NN分类器在嵌入空间进行少样本学习，仅需每个类别150张图像。

Result: 在少样本学习机制下，仅使用每个类别150张图像即可达到91.3%的平均检测准确率，比现有方法提升5.2个百分点；在源归属任务中，AUC和OSCR分别提升14.70%和4.27%。

Conclusion: 该框架为实现鲁棒、可扩展的法医归属系统提供了重要进展，能够适应不断发展的生成AI环境，无需进行详尽的重新训练协议。
...
### [65] [Contrastive vision-language learning with paraphrasing and negation](https://arxiv.org/abs/2511.16527)
*Kwun Ho Ngan,Saman Sadeghi Afgeh,Joe Townsend,Artur d'Avila Garcez*

Main category: cs.CV

TL;DR: 本文提出SemCLIP方法，通过结合释义和否定处理，改进CLIP模型的对比损失函数，提升视觉语言模型对语义变换的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP模型在处理否定和释义文本时表现不稳定，否定会大幅改变语义但词汇变化很小，而释义会产生不同表达但含义相同，这对改进视觉语言模型的评估和对齐提出了挑战。

Method: 提出SemCLIP方法，评估释义和否定的组合，设计新的CLIP对比损失函数，应用LLM生成的包含原始、释义和否定文本描述的训练三元组来训练CLIP类模型。

Result: SemCLIP能将释义描述向原始图像嵌入靠近，同时将否定描述推远。在CC-Neg基准测试中，图像检索准确率从68.1%提升到78.1%。在下游零样本分类任务中表现优于CLIP。

Conclusion: SemCLIP能够显著提升对语义变换的鲁棒性，在保持CLIP性能的同时有效处理否定和释义问题。
...
### [66] [Enhancing Multi-Camera Gymnast Tracking Through Domain Knowledge Integration](https://arxiv.org/abs/2511.16532)
...
Method: 结合体操领域知识，利用运动员3D中心通常在预定义垂直平面内的特性，提出级联数据关联范式：当交叉视角检测足够时使用三角测量生成3D轨迹候选，不足时采用射线-平面相交生成共面候选轨迹。

Result: 通过广泛实验验证了方法的鲁棒性，在挑战性场景中优于现有方法，并已成功应用于最近的体操世锦赛，获得国际体操联合会的高度认可。

Conclusion: 该方法通过结合领域知识和级联数据关联策略，有效解决了多摄像机体操运动员跟踪中的挑战，实现了鲁棒的3D轨迹跟踪。
...
### [67] [Investigating Optical Flow Computation: From Local Methods to a Multiresolution Horn-Schunck Implementation with Bilinear Interpolation](https://arxiv.org/abs/2511.16535)
*Haytham Ziani*

Main category: cs.CV

TL;DR: 本文对局部和全局光流计算方法进行了应用分析，重点关注Horn-Schunck算法，并实现了其多分辨率版本以提升精度和收敛性。


<details>
  <summary>Details</summary>
Motivation: 研究不同光流计算方法在估计帧间运动中的有效性，特别是在变化图像条件下的表现。

Method: 结合局部方法（如Lucas-Kanade）和全局方法（如Horn-Schunck），并实现Horn-Schunck算法的多分辨率版本，使用双线性插值和延长技术。

Result: 多分辨率Horn-Schunck算法在精度和收敛性方面得到改善，能够更好地估计不同图像条件下的运动。

Conclusion: 局部和全局方法的结合策略，特别是多分辨率Horn-Schunck算法，在光流计算中具有更好的性能和适用性。
...
### [68] [EOGS++: Earth Observation Gaussian Splatting with Internal Camera Refinement and Direct Panchromatic Rendering](https://arxiv.org/abs/2511.16542)
*Pierrick Bournez,Luca Savant Aira,Thibaud Ehret,Gabriele Facciolo*

Main category: cs.CV

TL;DR: EOGS++是基于3D高斯溅射的卫星图像重建方法，直接在原始高分辨率全色数据上操作，无需外部预处理，通过集成光流技术和束调整改进相机姿态估计，在重建质量和效率方面达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 扩展地球观测高斯溅射框架，开发专门针对卫星图像的新方法，避免依赖外部预处理工具，同时改进相机姿态估计精度。

Method: 提出EOGS++方法，直接在原始高分辨率全色数据上操作；利用光流技术将束调整嵌入训练过程；引入早期停止和TSDF后处理等改进措施。

Result: 在IARPA 2016和DFC2019数据集上，EOGS++在重建质量和效率方面达到最先进性能，建筑物平均MAE误差从1.33提升到1.19，优于原始EOGS和其他基于NeRF的方法。

Conclusion: EOGS++在保持高斯溅射计算优势的同时，显著提升了卫星图像的重建质量和几何精度，为地球观测应用提供了有效的解决方案。
...
TL;DR: 本文针对长尾分布下的2D目标检测问题，使用LVISv1数据集，通过改进平衡组Softmax框架和探索度量学习，在Faster R-CNN架构上实现了24.5%的mAP，创造了新的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中的目标检测面临长尾分布挑战，即多数类别只有少量实例，导致检测模型偏向频繁类别，在稀有类别上性能下降。

Method: 采用两阶段Faster R-CNN架构，改进平衡组Softmax框架缓解类别不平衡，并探索度量学习生成良好分离的特征嵌入，在推理时使用k近邻方法提升分类性能。

Result: 在LVISv1数据集上达到24.5%的平均精度均值，超越了之前24.0%的基准，特别是在稀有类别上表现出色。

Conclusion: 所提出的方法有效提升了长尾目标检测的性能，证明了平衡组Softmax改进和度量学习在缓解类别不平衡问题上的有效性。
...
Method: 采用人机协同标注流程来标注物体形状、纹理和姿态，提供大规模视觉基础的3D重建数据。通过多阶段训练框架，结合合成预训练和真实世界对齐，突破3D数据瓶颈。

Result: 在真实世界物体和场景的人类偏好测试中，相比近期工作取得了显著提升，胜率至少为5:1。

Conclusion: SAM 3D在自然图像中的3D物体重建方面表现出色，将发布代码、模型权重、在线演示以及新的野外3D物体重建基准测试。
...
TL;DR: 提出一种新颖的教师引导剪枝框架，将知识蒸馏与重要性评分估计紧密结合，实现一次性全局剪枝，在保持性能的同时达到高稀疏度。
...
Method: 在重要性评分计算中利用教师模型的梯度信号，识别对任务性能和知识转移都关键的参数，然后进行一次性全局剪枝，最后使用稀疏感知重训练恢复精度。
...
Method: 提出轻量级选择器模型评估来自多个采样噪声的潜在高斯基元，实现早期轨迹缩减；引入实例掩码去噪，通过过滤冗余背景区域来修剪可学习高斯基元。

Result: 大量实验和分析表明，TRIM显著提高了3D生成的效率和质量。

Conclusion: TRIM方法在不影响输出质量的前提下，有效加速了3D高斯扩散模型的推理过程，并支持推理时缩放。
...
### [73] [Solving Spatial Supersensing Without Spatial Supersensing](https://arxiv.org/abs/2511.16655)
*Vishaal Udandarao,Shyamgopal Karthik,Surabhi S. Nath,Andreas Hochlehnert,Matthias Bethge,Ameya Prabhu*

Main category: cs.CV

TL;DR: 本文对Cambrian-S视频世界模型进行了批判性分析，发现其基准测试VSR和VSC存在缺陷，可以通过简单方法近乎完美解决，且其推理方法可能利用了基准测试中的捷径启发式方法。
...
Method: 提出两个简单基准方法：NoSense（仅使用词袋SigLIP模型）和VSC-Repeat（将视频与自身连接），分别测试VSR和VSC基准的鲁棒性。

Result: NoSense在VSR上达到95%准确率，VSC-Repeat使Cambrian-S准确率从42%降至0%，表明基准测试存在严重缺陷。

Conclusion: 当前VSI-Super基准测试无法可靠衡量空间超感知能力，Cambrian-S的推理方法通过利用基准测试中的捷径而非真正的空间超感知来提升性能。
...
### [74] [PartUV: Part-Based UV Unwrapping of 3D Meshes](https://arxiv.org/abs/2511.16659)
*Zhaoning Wang,Xinyue Wei,Ruoxi Shi,Xiaoshuai Zhang,Hao Su,Minghua Liu*

Main category: cs.CV

TL;DR: PartUV是一个基于部件分解的UV展开管道，针对AI生成网格的噪声和不良条件问题，通过语义部件分解和几何启发式方法，生成更少但部件对齐的图表，同时保持低失真。


<details>
  <summary>Details</summary>
Motivation: 现有UV展开方法在处理AI生成的网格时表现不佳，这些网格通常噪声大、凹凸不平且条件差，导致高度碎片化的图表和次优边界，产生伪影并阻碍下游任务。

Method: 基于PartField学习部件分解方法，PartUV在自上而下的递归框架中结合高层语义部件分解和新的几何启发式方法，确保每个图表的失真低于用户指定阈值，同时最小化总图表数量。

Result: 在四个不同数据集上的评估显示，PartUV在图表数量和接缝长度方面优于现有工具和最近的神经方法，实现可比较的失真，在挑战性网格上具有高成功率，并支持部件特定多瓦片打包等新应用。

Conclusion: PartUV通过部件对齐的UV展开方法有效解决了AI生成网格的展开问题，提供了更少图表、更低失真的解决方案，并扩展了应用可能性。
...
### [75] [TriDiff-4D: Fast 4D Generation through Diffusion-based Triplane Re-posing](https://arxiv.org/abs/2511.16662)
*Eddie Pokming Sheung,Qihao Liu,Wufei Ma,Prakhar Kaushik,Jianwen Xie,Alan Yuille*

Main category: cs.CV

TL;DR: TriDiff-4D是一个基于扩散模型的4D生成管道，通过三平面重定位生成高质量、时间一致的4D虚拟形象，支持骨架驱动的4D生成，显著提升了时间一致性、运动精度和计算效率。


<details>
  <summary>Details</summary>
Motivation: 解决现有4D生成方法在时间几何不一致性、感知伪影、运动不规则性、高计算成本和动态控制有限等方面的根本限制，满足从文本描述生成高保真可控4D虚拟形象的需求。
...
Conclusion: TriDiff-4D在时间一致性、运动精度、计算效率和视觉保真度方面表现出色，能够从大规模3D和运动数据集中显式学习3D结构和运动先验，支持任意长度的4D生成。
...
### [76] [V-ReasonBench: Toward Unified Reasoning Benchmark Suite for Video Generation Models](https://arxiv.org/abs/2511.16668)
*Yang Luo,Xuanlei Zhao,Baijiong Lin,Lingting Zhu,Liyao Tang,Yuqi Liu,Ying-Cong Chen,Shengju Qian,Xin Wang,Yang You*

Main category: cs.CV

TL;DR: V-ReasonBench是一个评估视频推理能力的基准测试，涵盖结构化问题解决、空间认知、模式推理和物理动力学四个维度，通过合成和真实图像序列构建，为视频模型提供可复现、可扩展的评估框架。
...
Method: 构建包含合成和真实世界图像序列的基准测试，设计多样化的可验证答案任务，评估六个最先进的视频模型在四个关键维度的推理能力。

Result: 评估显示不同视频模型在结构化、空间、模式和物理推理方面存在明显差异，还比较了视频模型与图像模型的性能，分析了常见幻觉行为，并研究了视频时长对帧链推理的影响。
...
### [77] [Video-as-Answer: Predict and Generate Next Video Event with Joint-GRPO](https://arxiv.org/abs/2511.16669)
*Junhao Cheng,Liang Hou,Xin Tao,Jing Liao*

Main category: cs.CV

TL;DR: 本文提出Video-Next-Event Prediction (VNEP)任务，要求模型生成视频而非文本回答，并开发了VANS模型，通过强化学习将视觉语言模型与视频扩散模型对齐，在程序和预测基准上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 视频能够展示难以用语言传达的物理世界信息，但视频生成主要局限于娱乐领域。作者发现将视频作为Next-Event Prediction的新回答模态的未开发机会，从"讲述"转向"展示"，为程序性学习和创造性探索提供更直观和定制化的答案。

Method: 引入VANS模型，利用强化学习将视觉语言模型(VLM)与视频扩散模型(VDM)对齐。核心是提出的Joint-GRPO方法，协调VLM和VDM作为一个单元工作，通过共享奖励优化VLM生成准确且易于可视化的字幕，同时指导VDM生成忠实于这些字幕和输入视觉上下文的视频。

Result: 在程序和预测基准上的实验表明，VANS在视频事件预测和可视化方面都达到了最先进的性能。

Conclusion: VANS成功解决了VNEP任务的挑战，通过强化学习对齐VLM和VDM，实现了从文本回答到视频回答的转变，为程序性学习和创造性探索提供了更直观的解决方案。
...
### [78] [Learning to Think Fast and Slow for Visual Language Models](https://arxiv.org/abs/2511.16670)
*Chenyu Lin,Cheng Chi,Jinlin Wu,Sharon Li,Kaiyang Zhou*

Main category: cs.CV

TL;DR: 提出一种简单的强化学习方法，让视觉语言模型根据任务难度自动切换快速思考和慢速思考模式，在保持高性能的同时显著提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要追求冗长的推理链，导致计算成本过高，而人类会根据问题复杂度选择不同的思考模式来高效分配认知资源。

Method: 采用两阶段方法：第一阶段根据模型输出长度标注快速/慢速思考模式标签；第二阶段使用GRPO和思考模式标签训练模型发展双模式思考能力。

Result: DualMindVLM模型在显著超越基础模型的同时，与最先进视觉推理模型性能相当，且保持了极高的token效率。

Conclusion: 提出的简单强化学习方法成功实现了视觉语言模型的双模式思考，在保持高性能的同时大幅提升了计算效率。
...
Method: 提出NoPo-Avatar方法，完全消除测试时重建对人体姿态的依赖，仅使用图像作为输入。
...
### [80] [Mind the Motions: Benchmarking Theory-of-Mind in Everyday Body Language](https://arxiv.org/abs/2511.15887)
...
Method: 利用专家策划的身体语言参考作为知识库，构建了具有精细非语言线索标注和手动验证心理解释的视频数据集。

Result: 当前AI系统在非语言线索解释方面表现显著不足，在检测方面存在巨大性能差距，在解释方面相比人类标注者表现出过度解释的模式。

Conclusion: Motion2Mind框架揭示了AI在理解非语言交流心理状态方面的局限性，为评估和改进机器心理理论能力提供了重要基准。
...
### [81] [TOD-ProcBench: Benchmarking Complex Instruction-Following in Task-Oriented Dialogues](https://arxiv.org/abs/2511.15976)
...
<details>
  <summary>Details</summary>
Motivation: 现有任务导向对话基准往往将复杂指令简化为简单的意图、槽位和API调用配置，无法充分评估LLM对复杂流程指令的遵循能力。

Method: 基于高质量ABCD数据集构建包含复杂流程指令的基准，将细粒度约束和动作流程表述为多级条件-动作指令语句，设计三个任务全面评估LLM能力。

Result: 创建了包含复杂过程指令的基准数据集，设计了三个评估任务来系统测试LLM的指令遵循能力，并研究了多语言设置和不同指令格式对性能的影响。

Conclusion: TOD-ProcBench是一个具有挑战性的基准，能够系统评估LLM在多轮任务导向对话中理解和遵循复杂指令的能力，填补了现有基准的不足。
...
### [82] [Liars' Bench: Evaluating Lie Detectors for Language Models](https://arxiv.org/abs/2511.16035)
...
Method: 构建LIARS' BENCH测试平台，包含四个开源模型在七个数据集上生成的谎言和诚实回答，涵盖不同谎言类型，按说谎原因和信念目标两个维度变化。

Result: 评估三种黑白盒谎言检测技术发现，现有技术在某些类型谎言检测上系统性失败，特别是在仅凭对话记录无法判断模型是否说谎的情况下。
...
### [83] [Early science acceleration experiments with GPT-5](https://arxiv.org/abs/2511.16072)
...
TL;DR: 本文通过多个学科案例研究展示了GPT-5在科学研究中的实际应用，包括数学、物理、天文学等领域，重点突出了AI如何加速研究进程以及人类专家在其中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 虽然GPT-5等前沿AI模型对科学家来说是宝贵工具，但许多研究者对其能力了解不足。本文旨在通过具体案例展示GPT-5在科学研究中的实际价值和应用方式。

Method: 采用多个学科领域的案例研究方法，记录人类研究者与GPT-5的交互过程，包括数学、物理、天文学、计算机科学、生物学和材料科学等领域的实际应用。

Result: GPT-5在多个研究领域产生了新的具体研究步骤，特别是在数学领域贡献了四个经过人类作者仔细验证的新结果，帮助解决了之前未解决的问题。

Conclusion: GPT-5能够有效加速科学研究进程，在人类专家的指导下可以产生实质性贡献。虽然这些贡献在范围上相对有限，但考虑到前沿AI的发展速度，其影响深远。
...
### [84] [ELPO: Ensemble Learning Based Prompt Optimization for Large Language Models](https://arxiv.org/abs/2511.16122)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的性能高度依赖精心设计的提示，但手动提示工程耗时费力，成为实际应用的核心瓶颈。现有自动提示优化方法主要关注单一模型或算法，在处理复杂任务时性能受限。

Method: 提出ELPO框架，采用集成学习思想，结合投票机制、共享生成策略和不同搜索方法，并创新性地提出了更高效的提示生成和搜索算法。

Result: 实验结果表明ELPO在不同任务上优于最先进的提示优化方法，例如在ArSarcasm数据集上F1分数提高了7.6。

Conclusion: ELPO框架通过集成学习方法有效提升了提示优化的准确性和鲁棒性，为自动提示优化领域提供了新的解决方案。
...
### [85] [TS-PEFT: Token-Selective Parameter-Efficient Fine-Tuning with Learnable Threshold Gating](https://arxiv.org/abs/2511.16147)
*Dabiao Ma,Ziming Dai,Zhimin Xin,Shu Wang,Ye Wang,Haojun Fei*

Main category: cs.CL

TL;DR: 本文提出了一种新的参数高效微调范式TS-PEFT，通过选择性对部分位置索引应用PEFT修改，相比传统对所有索引应用PEFT的方法，可能提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 传统PEFT方法对所有位置索引应用修改，作者质疑这种做法的必要性，认为可能不仅多余甚至适得其反。

Method: 引入Token-Selective PEFT (TS-PEFT)范式，使用函数S选择性地对位置索引子集应用PEFT修改。

Result: 实验结果表明，对所有索引不加选择地应用PEFT不仅多余，还可能产生负面影响。
...
TL;DR: SemanticCite是一个AI驱动的系统，通过全文源分析验证引文准确性，提供详细的推理和文本片段作为上下文信息。该系统结合多种检索方法和四类分类系统（支持、部分支持、不支持、不确定），能够捕捉细微的声明-源关系，并为不同类型的错误提供适当的补救措施。


<details>
  <summary>Details</summary>
Motivation: 学术文献面临语义引用错误、AI生成的幻觉引用以及传统引用格式无法指示具体支持章节等问题，需要准确验证引用来源并指导读者找到支持证据。

Method: 结合多种检索方法，使用四类分类系统（支持、部分支持、不支持、不确定），通过微调的轻量级语言模型进行全文源分析，提供详细的推理和文本片段。

Result: 实验表明，微调的轻量级语言模型在性能上可与大型商业系统相媲美，但计算需求显著降低，使大规模引用验证变得可行。系统提供透明、基于证据的解释，支持用户理解和信任。
...
TL;DR: 本文提出了语义结构熵(SeSE)框架，从结构信息角度量化LLM的语义不确定性，用于幻觉检测。通过构建自适应稀疏的有向语义图，利用层次抽象挖掘潜在语义结构信息，显著优于现有不确定性量化方法。


<details>
  <summary>Details</summary>
Motivation: 现有不确定性量化方法主要依赖语义概率分布或成对距离，忽略了潜在语义结构信息，无法提供更精确的不确定性估计。在安全关键场景中，可靠的不确定性量化对避免LLM产生幻觉至关重要。

Method: 开发自适应稀疏有向语义图构建算法，捕获方向性语义依赖并自动修剪干扰连接；通过层次抽象利用潜在语义结构信息，定义语义结构熵为最优语义编码树的结构熵；扩展SeSE到长文本生成，通过建模随机语义交互量化单个声明的不确定性。

Result: 在29个模型-数据集组合上的广泛实验表明，SeSE显著优于先进的不确定性量化基线方法，包括强监督方法和最近提出的KLE方法。

Conclusion: SeSE框架从结构信息角度有效量化LLM的语义不确定性，为幻觉检测提供了理论可解释的方法，在安全关键应用中具有重要价值。
...
TL;DR: 提出了SDA（Steering-Driven Distribution Alignment）框架，一种无需训练、模型无关的对齐方法，通过动态重分布模型输出概率来增强LLMs与人类意图的对齐，在帮助性、无害性和诚实性方面显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用，确保模型输出与人类意图对齐成为关键挑战。传统方法需要昂贵的重新训练或大量监督，因此需要开发高效、无需训练的对齐框架。

Method: SDA框架基于用户定义的对齐指令动态重分布模型输出概率，无需微调即可增强模型行为与人类意图的对齐。该方法轻量级、资源高效，兼容各种开源LLMs。

Result: 在8个不同规模和来源的开源LLMs上评估，SDA在帮助性、无害性和诚实性三个关键维度上均取得显著提升：帮助性平均提升64.4%，诚实性提升30%，无害性提升11.5%。

Conclusion: SDA是一种有效且通用的对齐框架，能够在不进行训练的情况下显著提升LLMs的对齐性能，支持个性化偏好对齐，适用于多样化模型和应用场景。
...
TL;DR: 本文创建了用于评估大型语言模型处理习语和比喻语言能力的数据集，包括一个大规模潜在表达数据集和两个人工标注的确切表达数据集，用于训练和评估模型在习语识别任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 习语和比喻语言在口语和书面语中占很大比例，但大型语言模型在处理这类语言时仍有困难。虽然大规模语料库似乎能解决大多数NLP问题，但习语和比喻语言仍然难以被LLMs准确理解。

Method: 使用现有习语和比喻语言数据集获取合并的习语列表，从大型语料库中检索上下文序列。创建一个大规模潜在表达数据集和两个人工标注的确切表达数据集，处理后用于模型无关的训练兼容性，并在槽位标注和序列标注任务中进行训练和评估。

Result: 开发了用于评估预训练语言模型处理比喻意义能力的数据集，通过习语识别任务来测试模型表现。这些数据集提供了多样化的类别，可用于构建新模型和开发新方法。

Conclusion: 微调方法被证明是最优的，但更好和更大的数据集可以进一步缩小LLMs在理解习语和比喻语言方面的差距。本文提供的数据集是解决方案之一，为开发新模型和方法提供了基础。
...
TL;DR: 本文分析了自然语言解释（rationales）的充分性指标，发现该指标不能有效衡量rationales对模型性能的影响，并探讨了rationales在token分类和注意力正则化两种建模范式中的作用。


<details>
  <summary>Details</summary>
Motivation: 现有评估rationales信息量的充分性指标存在局限性，无法深入了解rationales信息对模型性能的实际影响，需要更系统地研究rationales的复杂性。

Method: 通过将充分性与两种建模范式相关联：通过token分类识别rationales标记的能力，以及通过注意力正则化在输入中融入rationales来提升模型性能的能力。

Result: 发现高度信息化的rationales不一定能帮助正确分类实例；充分性反而捕捉了非rationales上下文对分类的影响；融入rationales信息可以提升跨域分类，但结果因任务和模型类型而异；充分性与token分类似乎无关。
...
TL;DR: 本文提出了MinerU-HTML提取管道，将HTML到文本提取重新定义为序列标注问题，使用0.6B参数语言模型解决，显著提升了网页内容提取质量，特别是在结构化元素保护方面。


<details>
  <summary>Details</summary>
Motivation: 现有网页语料库依赖基于启发式的提取器（如Trafilatura），这些方法难以保持文档结构并经常损坏结构化元素（如公式、代码和表格）。作者假设提高提取质量对下游性能的影响可能与积极的过滤策略一样重要。

Method: 引入MinerU-HTML提取管道，将内容提取重新定义为序列标注问题，使用0.6B参数语言模型解决。采用两阶段格式化管道，先明确分类语义元素，再转换为Markdown格式。

Result: 在包含7,887个标注网页的MainWebBench基准测试中，MinerU-HTML达到81.8% ROUGE-N F1，而Trafilatura仅为63.6%。在结构化元素保护方面表现优异（代码块90.9%，公式94.0%）。使用MinerU-HTML构建的AICC语料库在13个基准测试中平均准确率达到50.8%，优于Trafilatura提取的TfCC。

Conclusion: HTML提取是网页语料库构建中关键但常被低估的组成部分。MinerU-HTML的方法证明了提取质量对模型能力有显著影响，且其基于模型的方法具有固有的可扩展性。
...
TL;DR: 本研究探讨了监督机器学习和深度学习模型是否能有效区分感知低质量与高质量新闻文章，使用141万+新闻文章数据集，发现传统机器学习（如随机森林）和深度学习模型（如ModernBERT）都能有效区分新闻质量。
...
Method: 使用3种机器学习分类器和3种深度学习模型，基于141.2万篇英文新闻文章数据集，利用194个语言特征进行网站级别的质量分类。

Result: 随机森林准确率0.7355，ROC AUC 0.8131；ModernBERT-large表现最佳，准确率0.8744，ROC-AUC 0.9593，F1 0.8739；其他深度学习模型也表现出色。
...
### [94] [ESGBench: A Benchmark for Explainable ESG Question Answering in Corporate Sustainability Reports](https://arxiv.org/abs/2511.16438)
*Sherine George,Nithish Saji*

Main category: cs.CL

TL;DR: ESGBench是一个用于评估可解释ESG问答系统的基准数据集和评估框架，包含基于企业可持续发展报告的多主题问题、人工标注答案和支持证据。


<details>
  <summary>Details</summary>
Motivation: 需要评估ESG问答系统的透明度和可解释性，促进负责任AI在ESG领域的发展。

Method: 构建包含多ESG主题问题的基准数据集，配备人工标注的答案和证据链，用于评估模型推理能力。
...
### [95] [Anatomy of an Idiom: Tracing Non-Compositionality in Language Models](https://arxiv.org/abs/2511.16467)
*Andrew Gomes*

Main category: cs.CL

TL;DR: 本文研究基于Transformer的语言模型如何处理习语表达，通过电路发现和分析技术识别出习语处理中的特殊计算模式，包括"习语头"和"增强接收"现象。
...
Method: 采用改进的路径修补算法进行电路发现，识别和分析"习语头"（在不同习语中频繁激活的注意力头）以及"增强接收"（习语token间因早期处理而增强的注意力）。

Result: 发现习语处理具有独特的计算模式，识别出专门的习语处理电路，这些电路展示了Transformer平衡计算效率和鲁棒性的机制。

Conclusion: 这些发现为理解Transformer处理非组合性语言提供了见解，并为研究更复杂语法结构处理提供了途径。
...
### [96] [Arctic-Extract Technical Report](https://arxiv.org/abs/2511.16470)
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够在资源受限硬件上高效处理长文档的结构化数据提取模型，解决现有模型部署成本高的问题。

Method: 采用先进的训练协议，优化模型架构使其仅占用6.6 GiB存储空间，支持在A10 GPU等有限资源设备上运行。
...
### [97] [TurkColBERT: A Benchmark of Dense and Late-Interaction Models for Turkish Information Retrieval](https://arxiv.org/abs/2511.16528)
*Özay Ezerceli,Mahmoud El Hussieni,Selva Taş,Reyhan Bayraktar,Fatma Betül Terzioğlu,Yusuf Çelebi,Yağız Asker*

Main category: cs.CL

TL;DR: 该论文提出了TurkColBERT基准，系统比较了稠密编码器和延迟交互模型在土耳其语信息检索中的表现，发现延迟交互模型在参数效率方面显著优于稠密编码器，并提出了优化的索引算法MUVERA+Rerank。


<details>
  <summary>Details</summary>
Motivation: 神经信息检索系统在高资源语言中表现出色，但在形态丰富的低资源语言如土耳其语中研究不足，特别是延迟交互模型尚未得到系统评估。

Method: 采用两阶段适应管道：先在土耳其NLI/STS任务上微调英语和多语言编码器，然后使用PyLate在MS MARCO-TR上将其转换为ColBERT风格的检索器，评估了10个模型在5个土耳其BEIR数据集上的表现。

Result: 延迟交互模型在参数效率方面表现优异：1.0M参数的colbert-hash-nano-tr比600M参数的turkish-e5-large小600倍，但保留了其71%的平均mAP；3-5倍小的延迟交互模型显著优于稠密编码器，ColmmBERT-base-TR在特定领域任务中mAP提升达13.8%。

Conclusion: 延迟交互模型在土耳其语检索中具有显著优势，MUVERA+Rerank索引算法实现了低延迟检索，ColmmBERT-base-TR查询时间仅0.54毫秒。但研究依赖于中等规模数据集和翻译基准，可能无法完全反映真实世界的土耳其语检索条件。
...
### [98] [Beyond Tokens in Language Models: Interpreting Activations through Text Genre Chunks](https://arxiv.org/abs/2511.16540)
...
Method: 使用Mistral-7B模型，基于文本提示的激活状态来预测文本体裁，采用scikit-learn分类器进行分析。

Result: 在两个数据集上分别实现了98%和71%的F1分数，结果始终优于控制任务，证明了方法的有效性。

Conclusion: 这为从LLM中推断文本体裁提供了概念验证，表明浅层学习模型能够有效提取LLM内部表示中的体裁信息。
...
### [99] [WER is Unaware: Assessing How ASR Errors Distort Clinical Understanding in Patient Facing Dialogue](https://arxiv.org/abs/2511.16544)
...
TL;DR: 该论文挑战了ASR评估中过度依赖词错误率(WER)的标准，研究发现WER等传统指标与临床影响相关性差，提出了基于LLM的自动化评估框架来替代专家临床评估。


<details>
  <summary>Details</summary>
Motivation: 当前ASR在临床对话中广泛应用，但评估仍主要依赖词错误率(WER)，缺乏对临床安全影响的评估。需要建立能够反映转录错误临床影响的评估标准。

Method: 建立专家临床评估黄金标准，使用LLM作为评估器，通过GEPA方法进行程序化优化，复制专家临床评估。

Result: 优化后的评估器(Gemini-2.5-Pro)达到与人类专家相当的性能，准确率90%，Cohen's κ为0.816，显著优于传统WER指标。
...
### [100] [Integrating Symbolic Natural Language Understanding and Language Models for Word Sense Disambiguation](https://arxiv.org/abs/2511.16577)
*Kexin Zhao,Ken Forbus*

Main category: cs.CL

TL;DR: 提出了一种基于统计语言模型的词义消歧方法，无需人工标注训练数据，通过将符号NLU系统生成的候选含义转换为可区分的自然语言替代项，使用LLM根据上下文选择合适解释。
...
Method: 使用统计语言模型作为消歧预言机，将符号NLU系统生成的多个候选含义转换为可区分的自然语言替代项，查询LLM根据语言上下文选择合适解释，并将选定含义传播回符号NLU系统。
...
### [101] [Comparison of Text-Based and Image-Based Retrieval in Multimodal Retrieval Augmented Generation Large Language Model Systems](https://arxiv.org/abs/2511.16654)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态RAG系统依赖LLM摘要将图像转换为文本，导致视觉细节和上下文信息丢失，影响下游检索和问答性能。

Method: 在金融财报问答基准上比较了文本分块检索（图像先被摘要为文本）和直接多模态嵌入检索（图像原生存储在向量空间）两种方法，评估了6个LLM模型和2个多模态嵌入模型。

Result: 直接多模态嵌入检索显著优于基于LLM摘要的方法，在mAP@5上绝对提升13%（相对提升32%），在nDCG@5上绝对提升11%（相对提升20%），且产生更准确和事实一致的答案。

Conclusion: LLM摘要会在预处理阶段引入信息损失，而直接多模态嵌入能够保留视觉上下文用于检索和推理，在多模态RAG系统中具有重要实践价值。
...
### [102] [UniUltra: Interactive Parameter-Efficient SAM2 for Universal Ultrasound Segmentation](https://arxiv.org/abs/2511.15771)
...
TL;DR: UniUltra是一个用于通用超声分割的框架，通过上下文-边缘混合适配器和深度监督知识蒸馏技术，在保持参数效率的同时显著提升了SAM2模型在超声图像上的性能。


<details>
  <summary>Details</summary>
Motivation: 解决SAM2在超声图像上性能显著下降的问题，以及如何在资源受限的临床环境中高效部署适应后的模型。

Method: 提出上下文-边缘混合适配器（CH-Adapter）增强细粒度感知，并开发深度监督知识蒸馏（DSKD）技术将知识从大型图像编码器转移到超轻量编码器。

Result: UniUltra在广泛实验中优于现有最先进方法，仅使用SAM2参数的8.91%进行微调，最终压缩模型相比原始SAM2参数减少94.08%。

Conclusion: 该框架在保持竞争性能的同时大幅降低了计算需求，非常适合实际临床部署。
...
### [103] [Weakly Supervised Segmentation and Classification of Alpha-Synuclein Aggregates in Brightfield Midbrain Images](https://arxiv.org/abs/2511.16268)
*Erwan Dereure,Robin Louiset,Laura Parkkinen,David A Menassa,David Holcman*

Main category: eess.IV

TL;DR: 开发了一个自动图像处理流程，基于弱监督分割和ResNet50分类器，在帕金森病和中脑组织的全玻片图像中分割和分类α-突触核蛋白聚集体，能够区分主要聚集体形态，平衡准确率达到80%。


<details>
  <summary>Details</summary>
Motivation: 帕金森病与错误折叠的α-突触核蛋白聚集体积累相关，形成用于病理诊断的路易体和神经突。自动分析免疫组织化学组织病理学图像为更好理解这些聚集体的空间组织提供了有前景的工具。

Method: 开发了一个自动图像处理流程，基于弱监督分割，对免疫组织化学标记变异性具有鲁棒性，使用ResNet50分类器在全玻片图像中分割和分类α-突触核蛋白聚集体。

Result: 该方法能够区分主要聚集体形态，包括路易体和神经突，平衡准确率达到80%。

Conclusion: 该框架为大规模表征α-突触核蛋白聚集体在明场免疫组织化学组织中的空间分布和异质性，以及研究其与周围细胞（如小胶质细胞和星形胶质细胞）之间了解甚少的关系铺平了道路。
...
### [104] [Majority Rules: LLM Ensemble is a Winning Approach for Content Categorization](https://arxiv.org/abs/2511.15714)
*Ariel Kamen,Yakov Kamen*

Main category: cs.AI

TL;DR: 本文提出了一个基于大语言模型的非结构化文本分类集成框架(eLLM)，通过整合多个模型解决了单个系统的常见弱点，在IAB分层分类法上实现了比最强单模型提升65% F1分数的性能改进。


<details>
  <summary>Details</summary>
Motivation: 单个大语言模型在文本分类中存在不一致性、幻觉、类别膨胀和误分类等问题，需要开发更稳健的集成方法来提高分类准确性和可靠性。

Method: 采用集成大语言模型(eLLM)框架，通过数学建模集体决策过程并建立原则性聚合标准，在零样本条件下评估10个最先进的大语言模型，使用IAB分层分类法对8,660个人工标注样本进行分类。

Result: eLLM框架相比最强单模型在F1分数上提升了65%，实现了接近人类专家水平的性能，同时提高了鲁棒性和准确性。
...
### [105] [Graph-Memoized Reasoning: Foundations Structured Workflow Reuse in Intelligent Systems](https://arxiv.org/abs/2511.15715)
*Yash Raj Singh*

Main category: cs.AI

TL;DR: 本文提出了Graph-Memoized Reasoning框架，通过图结构记忆来存储和重用推理工作流，以减少大型语言模型推理系统中的重复计算，提高计算效率和推理速度。
...
Result: 提出了一个最小化总推理成本并正则化存储与生成工作流之间不一致性的优化目标，为智能系统中的效率-一致性权衡提供了理论基础，并设计了相应的概念评估协议。
...
### [106] [MACIE: Multi-Agent Causal Intelligence Explainer for Collective Behavior Understanding](https://arxiv.org/abs/2511.15716)
*Abraham Itzhak Weinberg*

Main category: cs.AI

TL;DR: MACIE是一个多智能体因果智能解释框架，结合结构因果模型、干预反事实和Shapley值，为多智能体强化学习系统提供全面解释，解决个体贡献归因、涌现智能量化和可操作解释三个关键问题。
...
Method: MACIE框架结合结构因果模型、干预反事实和Shapley值，通过干预归因分数计算每个智能体的因果贡献，使用协同指标分离集体效应与个体贡献来量化系统级涌现智能，并通过自然语言叙述合成因果洞察提供可操作解释。

Result: 在四个MARL场景（合作、竞争和混合动机）中评估，结果显示准确的结果归因（平均phi_i=5.07，标准差<0.05），在合作任务中检测到正向涌现（协同指数高达0.461），计算效率高（CPU上每个数据集0.79秒）。
...
### [107] [How Modality Shapes Perception and Reasoning: A Study of Error Propagation in ARC-AGI](https://arxiv.org/abs/2511.15717)
*Bo Wen,Chen Wang,Erhan Bilal*

Main category: cs.AI

TL;DR: 本文研究了不同模态（文本和图像）在ARC-AGI任务中对网格感知的影响，发现结构化文本能精确定位稀疏特征，图像能捕捉2D形状但对分辨率敏感，结合两者可提升执行效果。
...
Method: 采用类似黑格尔辩证法的迭代方法：初始摘要（正题）→通过提问识别局限性（反题）→生成通用摘要（合题），满足当前和未来信息需求。

Result: 在TriviaQA、TruthfulQA和SQUAD数据集上，CoS比零样本LLM基线提升66%，比专用摘要方法BRIO和PEGASUS提升27%，使用更少token且与下游LLM无关。

Conclusion: CoS为网站维护者提供了使内容更易被LLM访问的可行方案，同时保留人工监督的可能性。
...
Method: 采用多模态AI框架，第一个案例研究使用GPT 4o和GPT 4o mini从28,000份OSHA事故报告中提取结构化见解；第二个案例研究使用Molmo 7B和Qwen2 VL 2B轻量级开源视觉语言模型，在ConstructionSite10k数据集上进行规则级安全违规检测。
...
TL;DR: 本文从认知角度提出空间智能分类法，按推理复杂度组织任务，将现有基准映射到该分类法，分析评估指标和方法，揭示当前模型与人类推理的差距，并探讨提升空间能力的训练和推理方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究多基于输入模态分类，但空间能力不仅取决于输入格式。本文从认知角度构建分类法，旨在提供更原则性的跨任务比较，揭示当前多模态大语言模型在空间推理方面的能力差距。

Method: 提出基于认知的空间智能分类法，按推理复杂度组织任务并关联认知功能；将文本、视觉语言和具身环境下的现有基准映射到该分类法；分析评估指标和方法；探讨训练和推理两种提升空间能力的方法。

Result: 建立了认知视角的空间智能分类框架，系统梳理了现有基准和评估方法，揭示了当前模型与人类空间推理的差距，明确了训练和推理方法的互补机制。

Conclusion: 从认知角度构建的空间智能分类法为跨任务比较提供了新视角，揭示了研究空白，为未来研究提供了全面理解和可操作方向，有助于推动多模态大语言模型空间推理能力的发展。
...
TL;DR: 该论文提出了一种基于一致性引导的跨模态迁移方法，通过将异构模态投影到共享潜在空间来增强多模态学习的鲁棒性，特别是在存在噪声数据、低质量标签和模态差异的情况下。


<details>
  <summary>Details</summary>
Motivation: 解决多模态学习系统中由于噪声数据、低质量标签和异构模态特性带来的不确定性挑战，特别是在人机交互环境中数据质量和标注一致性差异较大的问题。
...
TL;DR: 该论文提出使用检索增强生成(RAG)和多智能体系统(MAS)来提升大语言模型在生物力学教育任务中的性能，解决专业领域知识不足和复杂推理问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在特定领域应用中存在知识差距，处理需要多步推理的复杂问题时性能下降，需要开发专门的教育助手来增强本科生物力学课程学习。

Method: 构建双模块框架：1) 使用RAG提升概念性判断题的回答特异性和逻辑一致性；2) 使用MAS解决需要多步推理和代码执行的计算问题。评估了多个LLM在生物力学数据集上的表现。

Result: RAG显著提升了LLM在回答概念性问题时的性能和稳定性，超越了原始模型。MAS能够执行多步推理、推导方程、执行代码并为需要计算的任务生成可解释的解决方案。
...
TL;DR: 多智能体编排相比单智能体在事件响应中实现了100%可操作建议率，比单智能体的1.7%提升了80倍行动特异性和140倍解决方案正确性，且质量方差为零，使生产SLA承诺成为可能。


<details>
  <summary>Details</summary>
Motivation: 解决单智能体LLM在事件响应中生成模糊、不可用建议的问题，证明多智能体编排能从根本上提升LLM事件响应质量。

Method: 开发MyAntFarm.ai可复现容器化框架，通过348次对照试验比较单智能体副驾驶与多智能体系统在相同事件场景下的表现。

Result: 多智能体系统实现100%可操作建议率，单智能体仅1.7%；多智能体在行动特异性提升80倍，解决方案正确性提升140倍，且质量方差为零。

Conclusion: 多智能体编排不是性能优化，而是基于LLM事件响应的生产就绪要求，引入了决策质量(DQ)新指标来评估操作部署的关键属性。
...
Method: 使用波兰儿童康复医院的电子健康记录，比较基于规则的低计算量方法和大型语言模型在提取患者人口统计、临床发现和处方药物信息方面的表现，并评估文本标准化缺失和翻译导致信息损失的影响。

Result: 规则方法在信息检索任务中准确率更高，特别是在年龄和性别提取方面；LLM在药物名称识别方面表现更好，且具有更强的适应性和可扩展性。翻译成英文的文本与原始波兰语文本相比，LLM效果有所差异。
...
TL;DR: IMACT-CXR是一个基于AutoGen的多智能体交互式胸部X光解读教学系统，集成了空间标注、视线分析、知识检索和图像推理功能，通过贝叶斯知识追踪来评估学员技能水平并提供个性化教学。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够同时处理学员的边界框标注、视线样本和自由文本观察的智能教学系统，帮助放射科培训生提高胸部X光解读能力，解决传统教学中缺乏个性化反馈和实时指导的问题。

Method: 采用AutoGen框架构建多智能体工作流，包括定位质量评估、苏格拉底式教学、PubMed证据检索、相似病例推荐等专门智能体，结合贝叶斯知识追踪维护技能掌握度估计，使用TensorFlow U-Net进行肺叶分割以实现解剖学感知的视线反馈。

Result: 系统实现了响应式教学流程，具有有界延迟和精确的答案泄露控制，初步评估显示在定位和诊断推理方面相比基线方法有所改进。

Conclusion: IMACT-CXR展示了将多种AI技术集成到单一教学系统中的可行性，为实时住院医师培训部署提供了可扩展的解决方案，能够有效提升学员的胸部X光解读技能。
...
### [116] [Step-Audio-R1 Technical Report](https://arxiv.org/abs/2511.15848)
...
Method: 提出模态基础推理蒸馏框架，使模型学习生成基于声学特征的音频相关推理链，避免产生不连贯的推理过程。
...
### [117] [Decomposing Theory of Mind: How Emotional Processing Mediates ToM Abilities in LLMs](https://arxiv.org/abs/2511.15895)
*Ivan Chulo,Ananya Joshi*

Main category: cs.AI

TL;DR: 本文通过对比激活引导与基线LLM的激活，使用线性探针分析45种认知行为，发现情感理解而非分析推理是LLM心智理论能力提升的关键机制。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明激活引导能显著提升语言模型的心智理论能力，但其内部机制导致输出变化的具体原因尚不明确。

Method: 在Gemma-3-4B模型上应用对比激活加法引导，在1000个BigToM前向信念场景中评估，使用线性探针分析45种认知行为的激活差异。

Result: 信念归因任务准确率从32.5%提升至46.7%，提升主要由情感内容处理（情感感知+2.23，情感评价+2.20）驱动，同时抑制分析过程（质疑-0.78，聚合思维-1.59）。
...
### [118] [Thinking, Faithful and Stable: Mitigating Hallucinations in LLMs](https://arxiv.org/abs/2511.15921)
*Chelsea Zou,Yiheng Yao,Basant Khalil*

Main category: cs.AI

TL;DR: 本文开发了一个用于大语言模型的自校正框架，通过细粒度不确定性信号（自信度对齐和词元级熵峰值）实时检测和缓解多步推理中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在多步推理中产生幻觉的问题，不仅关注最终答案正确性，更注重推理过程的可靠性和忠实性。

Method: 设计复合奖励函数，惩罚不合理的高自信度和熵峰值，鼓励稳定准确的推理轨迹；通过强化学习策略使模型更具自省能力，利用自信度感知的奖励反馈塑造生成行为。

Result: 实验表明该方法提高了最终答案准确性和推理校准度，消融实验验证了各信号对性能的独立贡献。

Conclusion: 提出的自校正框架有效提升了大语言模型在多步推理中的准确性和推理过程的忠实性，证明了细粒度不确定性信号在检测和缓解幻觉中的重要性。
...
### [119] [JudgeBoard: Benchmarking and Enhancing Small Language Models for Reasoning Evaluation](https://arxiv.org/abs/2511.15958)
*Zhenyu Bi,Gaurav Srivastava,Yang Li,Meng Lu,Swastik Roy,Morteza Ziyadi,Xuan Wang*

Main category: cs.AI

TL;DR: JudgeBoard是一个新颖的评估框架，直接查询模型评估答案正确性，无需额外答案比较。提出MAJ多智能体评估框架，通过多个SLM协作来近似LLM级别的判断准确性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-as-a-judge框架依赖预定义指标进行答案比较，这种方法间接且难以完全自动化，对推理输出的细粒度可扩展评估支持有限。

Method: 提出JudgeBoard评估管道，直接查询模型评估答案正确性；提出MAJ多智能体判断框架，利用具有不同推理配置文件的多个交互SLM通过协作审议来近似LLM级判断准确性。

Result: 实验显示SLM与LLM在独立判断任务中存在显著性能差距，但MAJ框架显著提高了SLM的可靠性和一致性。在MATH数据集上，使用较小模型作为骨干的MAJ表现与较大模型相当甚至更好。
...
### [120] [CARE-RAG - Clinical Assessment and Reasoning in RAG](https://arxiv.org/abs/2511.15994)
*Deepthi Potluri,Aby Mammen Mathew,Jeffrey B DeWitt,Alexander L. Rasgon,Yide Hao,Junyuan Hong,Ying Ding*

Main category: cs.AI

TL;DR: 该论文研究了在临床环境中，即使提供权威证据，大型语言模型在推理方面仍存在差距，并提出了评估推理准确性、一致性和忠实度的框架。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决大型语言模型在检索到正确证据后仍无法正确推理的问题，特别是在需要严格遵循结构化协议的临床环境中。
...
Conclusion: 结论强调了检索增强生成的潜力与风险，安全部署需要同等严格地评估推理和检索过程。
...
### [121] [SpellForger: Prompting Custom Spell Properties In-Game using BERT supervised-trained model](https://arxiv.org/abs/2511.16018)
...
<details>
  <summary>Details</summary>
Motivation: 探索AI在游戏中的动态内容生成能力，特别是作为核心游戏玩法共创工具的应用潜力，目前这方面研究仍不充分。

Method: 使用监督训练的BERT模型解析玩家自然语言提示，将文本描述映射到法术预制体并平衡参数（伤害、成本、效果），确保游戏平衡性。游戏在Unity引擎中开发，AI后端使用Python。

Result: 预期交付一个功能原型，能够实时生成法术并应用于引人入胜的游戏循环中，验证AI作为直接游戏机制的有效性。

Conclusion: SpellForger通过让玩家用自然语言创建自定义法术，为游戏个性化与创造力提供了独特体验，成功展示了AI作为核心游戏玩法共创工具的可行性。
...
### [122] [An Aligned Constraint Programming Model For Serial Batch Scheduling With Minimum Batch Size](https://arxiv.org/abs/2511.16045)
...
<details>
  <summary>Details</summary>
Motivation: 现有CP模型依赖预定义的虚拟批次集合，这会导致维度灾难并增加问题复杂性。在半导体制造等实际应用中，最小批次大小是常见要求，需要更高效的建模方法。

Method: 使用关键对齐参数直接对机器上同族作业序列进行推理，避免虚拟批次集合；通过定制搜索阶段和增强约束传播器的推理级别来利用问题结构。

Result: 在近5000个实例上的计算实验表明，新模型在最多100个作业的小中型实例上表现优越，在最多500个作业、10个族、10台机器的大规模实例上，找到的解决方案比现有方法好25%。

Conclusion: 提出的CP模型通过避免虚拟批次集合实现了更紧凑的建模，在串行批处理调度问题上显著优于现有方法，特别是在大规模实例上表现突出。
...
### [123] [Artificial Intelligence and Accounting Research: A Framework and Agenda](https://arxiv.org/abs/2511.16055)
*Theophanis C. Stratopoulos,Victor Xiaoqi Wang*

Main category: cs.AI

TL;DR: 本文提出了一个AI-会计研究分类框架，分析AI技术对会计研究的变革影响，探讨会计学者如何通过战略定位和合作发挥优势，并比较人类研究者与AI在整个研究流程中的能力差异。


<details>
  <summary>Details</summary>
Motivation: 生成式AI和大语言模型的快速发展正在根本性改变会计研究，为学者们带来机遇和竞争威胁，需要系统分析这种变革并指导学者应对。

Method: 提出二维分类框架（研究焦点：会计中心vsAI中心；方法：AI方法vs传统方法），应用于IJAIS特刊和主要会计期刊的AI-会计研究论文，分析研究流程中人类与AI的能力对比。

Result: 识别出现有研究分布和研究机会，发现会计学者在人类判断、创造力和理论深度方面仍具优势，GenAI在民主化研究能力的同时加剧了竞争。

Conclusion: AI变革要求改革博士教育以培养比较优势并建立AI素养，人类研究者需要在更高层次贡献中发挥价值。
...
### [125] [SkyRL-Agent: Efficient RL Training for Multi-turn LLM Agent](https://arxiv.org/abs/2511.16108)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决现有强化学习框架在软件工程智能体训练中的效率问题，特别是多轮长视野任务中的训练成本和性能瓶颈。

Method: 采用优化的异步流水线调度器（1.55倍加速）和基于AST的搜索工具增强训练配方，结合SWE-Bench数据集进行纯强化学习训练。

Result: SA-SWE-32B智能体在SWE-Bench Verified上达到39.4% Pass@1，相比之前模型成本降低2倍以上，并能有效泛化到其他智能体任务。

Conclusion: SkyRL-Agent框架通过高效调度和工具集成显著提升了智能体训练效率和性能，展示了在多个智能体任务上的良好扩展性和泛化能力。
...
### [126] [Multidimensional Rubric-oriented Reward Model Learning via Geometric Projection Reference Constraints](https://arxiv.org/abs/2511.16139)
*Yongnan Jin,Xurui Li,Feng Cao,Liucun Gao,Juanjuan Yao*

Main category: cs.AI

TL;DR: MR-RML是一个通过GPRC实现的多维标准导向奖励模型学习框架，旨在解决LLMs在医疗实践中的对齐挑战，包括静态评估与动态临床需求的脱节、适应多源医疗标准的困难，以及传统奖励模型无法捕捉多维医疗质量标准的局限性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗实践中的潜力受到关键对齐挑战的限制：静态评估基准与动态临床认知需求脱节、难以适应不断发展的多源医疗标准，以及传统奖励模型无法捕捉多维医疗质量标准。

Method: 提出MR-RML框架，包含三个核心创新：(1) 嵌入领域标准的"维度-场景-学科"医疗标准系统；(2) 独立的多维奖励模型，将评估标准分解；(3) 几何投影参考约束，将医疗认知逻辑转化为数学正则化。

Result: 在权威医疗基准Healthbench上的评估显示，相比基础LLM Qwen-32B，该方法在完整子集上提升45%，在困难子集上提升85%，在开源LLMs中达到SOTA水平（完整子集62.7分，困难子集44.7分），并优于大多数闭源模型。

Conclusion: MR-RML框架通过将医疗标准结构化整合到训练流程中，有效解决了LLMs在医疗领域的对齐挑战，显著提升了模型在复杂医疗场景下的性能表现。
...
TL;DR: 该论文介绍了FOOTPASS数据集，这是首个在足球比赛中用于逐场比赛动作识别的基准数据集，结合计算机视觉输出和足球战术知识来生成可靠的逐场比赛数据流。


<details>
  <summary>Details</summary>
Motivation: 当前足球视频理解方法在构建可靠的逐场比赛数据方面仍不足，通常只能辅助而非完全自动化标注。同时战术建模和性能分析研究已取得进展，这促使利用战术知识作为先验来支持基于计算机视觉的预测。

Method: 引入FOOTPASS数据集，支持在多种模态、多智能体战术背景下进行球员中心的动作识别，结合计算机视觉任务输出（如跟踪、识别）和足球战术规律知识。

Result: 创建了首个覆盖整个足球比赛的逐场比赛动作识别基准数据集，能够开发利用计算机视觉和战术知识的方法来生成可靠的逐场比赛数据流。
...
### [128] [OpenMMReasoner: Pushing the Frontiers for Multimodal Reasoning with an Open and General Recipe](https://arxiv.org/abs/2511.16334)
*Kaichen Zhang,Keming Wu,Zuhao Yang,Kairui Hu,Bin Wang,Ziwei Liu,Xingxuan Li,Lidong Bing*

Main category: cs.AI

TL;DR: OpenMMReasoner是一个完全透明的两阶段多模态推理训练方法，包含监督微调(SFT)和强化学习(RL)阶段，在9个多模态推理基准上相比Qwen2.5-VL-7B-Instruct基线提升11.6%
...
Result: 在9个多模态推理基准测试中显著超越强基线，证明了数据质量和训练设计对多模态推理性能的关键作用
...
### [129] [From Performance to Understanding: A Vision for Explainable Automated Algorithm Design](https://arxiv.org/abs/2511.16201)
*Niki van Stein,Anna V. Kononova,Thomas Bäck*

Main category: cs.AI

TL;DR: 本文提出将自动化算法设计与可解释性相结合的新愿景，通过LLM驱动的算法发现、可解释基准测试和问题类别描述符，构建发现-解释-泛化的闭环知识循环，实现可解释的、针对特定问题类别的算法设计。
...
Result: 形成发现、解释和泛化相互促进的闭环知识循环，从盲目搜索转向可解释的、针对特定问题类别的算法设计。

Conclusion: 这种集成将加速进展，同时产生关于优化策略何时以及为何成功的可重用科学见解，实现从盲目搜索到可解释算法设计的转变。
...
TL;DR: 提出了一种无需训练的单次联邦视觉语言模型适应框架TOFA，通过视觉和文本双管道处理多模态信息，解决现有方法在数据异构性、多模态利用不足和额外训练需求方面的问题。


<details>
  <summary>Details</summary>
Motivation: 现有联邦视觉语言模型适应方法需要迭代训练，通信成本高且易受攻击。单次联邦训练技术虽能减少通信轮次，但在多模态信息利用、数据异构性处理和额外训练需求方面存在挑战。

Method: TOFA框架采用无需训练的方式，包含视觉管道（分层贝叶斯模型学习个性化类别原型分布）和文本管道（评估并全局对齐本地文本提示），通过自适应权重校准机制结合双模态预测。

Result: 在9个数据集上的广泛实验表明，TOFA在各种联邦设置下均表现出有效性。

Conclusion: TOFA是一种高效、轻量级的单次联邦视觉语言模型适应方法，无需额外训练资源，能有效处理数据异构性并充分利用多模态信息。
...
Conclusion: 多智能体协作奖励模型框架能够在不增加人工标注的情况下实现多视角奖励塑造，提高RLHF的鲁棒性和可解释性。
...
TL;DR: ChemO是一个基于国际化学奥林匹克竞赛的新基准，通过评估等价重构和结构化视觉增强解决化学问题自动评估的挑战。ChemLabs多智能体框架模拟人类专家协作，在先进多模态模型上实现93.6/100的高分，超过人类金牌水平。


<details>
  <summary>Details</summary>
Motivation: 数学和物理领域的奥林匹克级基准已用于测试AI推理能力，但化学因其独特的多模态符号语言一直是个开放挑战。需要开发专门基准来评估AI在复杂化学问题上的表现。

Method: 提出ChemO基准，包含评估等价重构（将需要视觉输出的问题转换为计算可行格式）和结构化视觉增强（分离视觉感知与化学推理能力）。开发ChemLabs分层多智能体框架，包含问题分解、感知、推理和审计等专门智能体。

Result: 在先进多模态模型上的实验表明，结构化视觉增强与多智能体系统结合带来显著性能提升。最优配置获得93.6/100的高分，超过估计的人类金牌阈值，在自动化学问题解决方面达到新的最先进水平。
...
TL;DR: 提出了一种自动化流水线，从教育文档中提取高质量问答对和视觉问答对，结合布局感知OCR和基于LLM的语义解析，为LLM训练提供真实教育内容替代合成数据。


<details>
  <summary>Details</summary>
Motivation: 现有指令微调和强化学习数据集成本高昂且依赖合成样本，存在幻觉和多样性有限问题，而教材和练习材料包含大量高质量人工编写的问答内容未被充分利用。

Method: 结合布局感知OCR和基于大语言模型的语义解析，构建自动化流水线从教育文档PDF中提取格式良好的问答对和视觉问答对。

Result: 实验表明该方法能生成准确、对齐且低噪声的问答对和视觉问答对，适用于多种文档类型。

Conclusion: 该方法能够规模化利用真实世界教育内容，为改进推理导向的LLM训练提供了实用的合成数据生成替代方案。
...
TL;DR: 本文提出了一个多意图科学问答基准（MuISQA），用于评估RAG系统在异构证据覆盖方面的性能，并开发了一种意图感知检索框架，通过LLM假设潜在答案、分解意图特定查询，并使用RRF重新排序来平衡不同意图的覆盖。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统通常面向单一意图，导致复杂科学问题中多意图任务的证据覆盖不完整，无法满足多跳推理和多样化证据来源的需求。

Method: 提出意图感知检索框架：利用LLM假设潜在答案，将问题分解为意图特定查询，为每个底层意图检索支持段落，然后通过互惠排名融合（RRF）聚合和重新排序检索到的片段。

Result: 在MuISQA基准和其他通用RAG数据集上的实验表明，该方法在检索准确性和证据覆盖方面持续优于传统方法。

Conclusion: 意图感知检索框架能够有效解决多意图科学问答中的证据覆盖问题，显著提升RAG系统在复杂科学问题上的性能。
...
TL;DR: 本文提出了一个超级指标来评估Android恶意软件合成数据的质量，该指标聚合了八个不同维度的度量标准，生成单一加权分数，实验证明比传统指标更稳定和一致。
...
Method: 在MalDataGen中集成了一个超级指标，该指标聚合了四个保真度维度上的八个度量标准，产生单一加权分数。

Result: 涉及十个生成模型和五个平衡数据集的实验表明，超级指标比传统指标更稳定和一致，与分类器实际性能的相关性更强。

Conclusion: 超级指标为Android恶意软件合成数据质量评估提供了更可靠和标准化的解决方案。
...
TL;DR: Pharos-ESG框架通过多模态解析、上下文叙述和分层标注，将ESG报告转化为结构化表示，解决了ESG报告因不规则布局和弱结构化内容带来的理解挑战，并在实验中优于现有文档解析系统和多模态模型。


<details>
  <summary>Details</summary>
Motivation: ESG报告作为评估企业ESG绩效的核心媒介，存在阅读顺序混乱、布局不规则、内容冗长且弱结构化等问题，难以进行大规模理解，因此需要开发能够有效解析和结构化ESG报告的框架。

Method: 提出Pharos-ESG统一框架，包括基于布局流的阅读顺序建模模块、目录锚点引导的分层感知分割模块，以及将视觉元素上下文转换为连贯自然语言的多模态聚合管道，并进一步丰富ESG、GRI和情感标签。

Result: 在标注基准上的广泛实验表明，Pharos-ESG始终优于专用文档解析系统和通用多模态模型。同时发布了Aurora-ESG，首个大规模公开ESG报告数据集，涵盖中国大陆、香港和美国市场。

Conclusion: Pharos-ESG框架成功解决了ESG报告的结构化挑战，为金融治理和决策中的ESG整合提供了更好的支持，并通过Aurora-ESG数据集促进了相关研究的发展。
...
TL;DR: 论文主张生成式AI的进步源于明确的生成原则，而非模糊算法，并建议认知神经科学应研究这些原则在大脑中的运作机制。同时，机器学习研究提供了神经信息处理系统的新视角，论文讨论了五个具体例子来说明神经科学可从ML研究中获益。


<details>
  <summary>Details</summary>
Motivation: 探讨生成式AI的明确生成原则是否也适用于大脑运作，以及认知神经科学如何从机器学习研究中学习，特别是关于神经信息处理系统的特征化理解。

Method: 通过分析生成式AI的基本原则，并讨论五个具体案例（世界建模的缺陷、思维过程生成、注意力机制、神经缩放定律和量化），来展示机器学习对神经科学的启示。

Result: 识别出生成式AI的明确原则在理解大脑认知功能方面具有潜在相关性，机器学习研究为神经科学提供了新的分析框架和概念工具。

Conclusion: 神经科学应该积极借鉴机器学习研究的成果，特别是生成原则和神经信息处理系统的特征化方法，这将有助于深化对大脑认知机制的理解。
...
<details>
  <summary>Details</summary>
Motivation: 痴呆症患者的沟通行为会随时间逐渐变化，如表达减少、重复增加或话题偏离，但现有计算工具大多无法跟踪这种渐进的行为漂移。
...
Conclusion: 个性化行为上下文对于检测痴呆症患者沟通行为变化至关重要，PersonaDrift为评估此类渐进变化检测方法提供了可扩展的基准框架。
...
TL;DR: CLOZE是一个基于大语言模型的零样本框架，能够从临床笔记中自动提取医学术语并将其整合到层次化医学本体中，无需额外训练或标注数据，同时保护患者隐私。


<details>
  <summary>Details</summary>
Motivation: 临床笔记作为富含详细患者观察信息的非结构化文档，为医学本体扩展提供了有价值但未被充分利用的资源。直接利用临床笔记进行本体扩展的研究仍然很少。

Method: 利用预训练大语言模型的强大语言理解和广泛生物医学知识，CLOZE能够有效识别疾病相关概念并捕捉复杂的层次关系。该零样本框架无需额外训练或标注数据，并通过自动移除受保护健康信息来保护患者隐私。

Result: 实验结果表明，CLOZE提供了一个准确、可扩展且保护隐私的本体扩展框架，在生物医学研究和临床信息学中具有广泛的下游应用潜力。

Conclusion: CLOZE框架展示了利用临床笔记扩展医学本体的可行性，为生物医学研究和临床应用提供了有效的解决方案。
...
Method: 提出了一个分类学框架，基于Marr的层次理论识别挑战的粒度级别，并区分三种力度的挑战：对计算功能主义的挑战、实践性挑战和严格不可能性论证。

Result: 该框架成功应用于14个来自科学和哲学文献的著名案例，能够清晰区分不同类型的挑战。

Conclusion: 该框架为数字意识辩论提供了有价值的分类工具，有助于澄清和区分不同类型的挑战，而不在辩论中选边站队。
...
TL;DR: YOFO是一种基于模板的多模态大语言模型判断方法，通过单次前向传播同时验证多个结构化需求，在保持可解释性的同时实现数量级的加速。


<details>
  <summary>Details</summary>
Motivation: 现有方法面临基本权衡：让MLLMs输出单一分数与其生成性质不符且限制细粒度需求理解，而自回归生成判断分析在高吞吐量场景中速度过慢。

Method: 提出YOFO方法，基于自回归模型，接受结构化需求模板，在单次推理步骤中通过读取每个需求关联的最终token的logits，为每个需求生成二元是/否决策。

Result: 实验表明YOFO在标准推荐数据集上达到最先进结果，支持依赖感知分析（后续判断基于先前判断），并能从后验思维链中获益。

Conclusion: YOFO设计在保持可解释性的同时实现了数量级的加速，解决了MLLMs作为判断器时的效率与细粒度理解之间的权衡问题。
...
Method: 提出Deliberate Practice Policy Optimization (DPPO)元认知训练框架，动态交替监督微调（能力扩展）和强化学习（技能精炼），自动识别弱点并针对性分配资源。

Result: 训练视觉语言具身模型Pelican-VL 1.0，性能比基础模型提升20.3%，超越100B参数规模开源模型10.6%。
...
TL;DR: MedBayes-Lite是一个轻量级贝叶斯增强框架，用于基于transformer的临床语言模型，旨在产生可靠的、不确定性感知的预测，无需重新训练或架构修改。


<details>
  <summary>Details</summary>
Motivation: 尽管transformer在临床决策支持中显示出强大潜力，但在模糊医疗案例中容易过度自信，而校准的不确定性对医疗应用至关重要。

Method: 该框架集成三个组件：(i)使用蒙特卡洛dropout进行贝叶斯嵌入校准以量化认知不确定性，(ii)不确定性加权注意力机制对token可靠性进行边际化，(iii)受临床风险最小化启发的置信度引导决策塑造。

Result: 在生物医学QA和临床预测基准测试中，MedBayes-Lite持续改善校准和可信度，将过度自信降低32-48%。在模拟临床环境中，通过标记不确定预测供人工审查，可预防高达41%的诊断错误。

Conclusion: 结果表明MedBayes-Lite在医疗AI系统中能够有效实现可靠的不确定性传播并提高可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够整合基本面和技术面分析的AI交易系统，以确定在EUR-USD货币对交易中哪种类型的输入特征（基本面或技术面）具有更强的预测能力。

Method: 整合全面的输入特征集：包括欧元区和美国的关键宏观经济变量（如GDP、失业率）以及技术变量（指标、振荡器、斐波那契水平、价格背离）。使用机器学习指标评估预测准确性，并通过历史数据回测模拟评估交易盈利能力和风险。

Result: 通过标准机器学习指标量化了预测准确性，并通过历史回测模拟评估了交易系统的盈利能力和风险表现。

Conclusion: 通过比较分析确定了在EUR-USD货币对交易中，基本面和技术面特征中哪一类能够提供更强大和可靠的预测能力来生成盈利交易信号。
...
TL;DR: 该研究分析了大型语言模型与人类推理的认知机制差异，提出了包含28个认知元素的分类法，并通过对17个模型和人类推理轨迹的大规模分析，揭示了模型依赖浅层前向链式推理而人类使用层次嵌套和元认知监控的系统性差异。研究还开发了测试时推理指导方法，显著提升了复杂问题上的表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂问题上表现出色但在简单变体上失败，表明它们通过不同于人类推理的机制获得正确输出。研究旨在理解模型与人类推理的认知机制差异，为开发基于原则性认知机制而非脆弱推理捷径的模型奠定基础。

Method: 综合认知科学研究提出28个认知元素的分类法，分析17个模型（文本、视觉、音频模态）的170K推理轨迹和54个人类思维轨迹，进行元分析评估1,598篇LLM推理论文的研究重点分布。

Result: 发现系统性的结构差异：人类使用层次嵌套和元认知监控，而模型依赖浅层前向链式推理；研究社区集中于易量化的行为而忽视与成功相关的元认知控制；开发了测试时推理指导方法，在复杂问题上性能提升达60%。

Conclusion: 通过连接认知科学和LLM研究，为开发基于原则性认知机制而非脆弱推理捷径的模型建立了基础，为改进模型能力和大规模测试人类认知理论开辟了新方向。
