...
TL;DR: 本文提出GenEval方法，结合多模态视觉语言模型和人类知识，通过域一致性边界评估未知因果因素差异，提升单源域泛化能力，在糖尿病视网膜病变分级和癫痫发作区检测任务上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 图像分类在不同域间的泛化在关键医疗任务中仍具挑战性。当域间存在未知因果因素差异时，缺乏客观评估这些差异的方法，且通常无法获取数据收集者的元数据或协议级信息。

Method: 首先提出域一致性边界理论框架评估域间未知因果因素差异；然后提出GenEval方法，结合基础模型（如MedGemma-4B）和人类知识，通过低秩适应技术桥接因果差距，增强单源域泛化能力。

Result: 在8个糖尿病视网膜病变数据集和2个癫痫发作区检测数据集上，GenEval实现了优异的单源域泛化性能，平均准确率分别为69.2%和81%，分别比最强基线高出9.4%和1.8%。

Conclusion: GenEval方法通过结合多模态视觉语言模型和人类知识，有效解决了医疗图像分类中的跨域泛化问题，为评估和处理域间未知因果因素差异提供了系统化框架。
...
TL;DR: SPARROW是一个像素级视频多模态大语言模型，通过目标特定跟踪特征和双提示设计，解决了现有视频MLLMs在时空一致性、空间精度和参考跟踪方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有视频多模态大语言模型通常依赖静态分割标记进行逐帧定位，这种方法缺乏时间上下文，导致空间漂移、身份切换和不稳定初始化等问题，特别是在物体移动或重新出现时。

Method: SPARROW采用两个关键组件：1）目标特定跟踪特征，在训练期间注入时间对齐的参考线索；2）双提示设计，解码边界框和分割标记，将几何先验与语义定位融合。模型基于类无关的SAM2提议器，无需外部检测器。

Result: 在三个开源视频MLLMs（UniPixel、GLUS、VideoGLaMM）上集成SPARROW后，在六个基准测试中取得一致提升：RVOS上提升+8.9 J&F，视觉定位上提升+5 mIoU，GCG上提升+5.4 CLAIR。
...
TL;DR: ABRA是一种将预训练检测器中类别特定知识从有标注源域迁移到无标注目标域的方法，通过权重空间的几何传输实现跨域适应，特别适用于夜间、雾天等缺乏标注数据的场景。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测模型（如Grounding DINO）在零样本场景下表现良好，但在域偏移情况下性能显著下降。许多实际应用场景（如夜间、雾天）缺乏大规模标注数据集，无法直接进行微调。

Method: ABRA将域适应问题建模为预训练检测器权重空间中的几何传输问题，通过对齐源域和目标域专家来传输类别特定知识，实现无需目标域训练图像即可完成知识迁移。

Result: 在多个具有挑战性的域偏移场景下进行广泛实验，证明ABRA能够成功在各种恶劣条件下实现类别级专业知识的"传送"。

Conclusion: ABRA提供了一种有效的跨域适应方法，能够在缺乏目标域标注数据的情况下，将源域的类别特定检测知识迁移到目标域，为实际应用场景中的域偏移问题提供了解决方案。
...
### [4] [Marker-Based 3D Reconstruction of Aggregates with a Comparative Analysis of 2D and 3D Morphologies](https://arxiv.org/abs/2603.12667)
*Haohang Huang,Jiayi Luo,Issam Qamhia,Erol Tutumluer,John M. Hart,Andrew J. Stolba*

Main category: cs.CV

TL;DR: 本文提出了一种基于摄影测量的低成本、灵活的骨料颗粒3D重建方法，用于获取骨料的3D形态信息，便于质量控制和形态分析。


<details>
  <summary>Details</summary>
Motivation: 骨料作为建筑材料的主要骨架，其尺寸和形态信息对质量控制至关重要。现有的3D表征方法（如3D激光扫描或X射线CT）设备昂贵，难以在采石场和施工现场应用，需要一种低成本、灵活的3D重建方案。

Method: 提出基于标记的摄影测量方法，通过背景抑制、点云拼接和尺度参考，实现高质量的骨料3D模型重建。该方法使用标记设计来简化处理流程。

Result: 验证了重建结果的准确性，并与地面真实数据对比。对选定骨料样本的2D和3D形态特性进行了比较分析，发现2D和3D统计数据存在显著差异。

Conclusion: 该方法能够以低成本、便捷的方式获取骨料的3D形状信息，便于骨料检查、数据收集和3D形态分析，为质量保证/质量控制过程提供了实用工具。
...
### [5] [UNIStainNet: Foundation-Model-Guided Virtual Staining of H&E to IHC](https://arxiv.org/abs/2603.12716)
*Jillur Rahman Saurav,Thuong Le Hoai Pham,Pritam Mukherjee,Paul Yi,Brent A. Orr,Jacob M. Luber*

Main category: cs.CV

TL;DR: UNIStainNet：首个利用病理基础模型提供组织级语义指导的虚拟免疫组化染色方法，单模型支持多种标记物，在多个数据集上达到SOTA


<details>
  <summary>Details</summary>
Motivation: 现有方法通过对比目标、原型匹配或域对齐提高真实感，但生成器本身缺乏病理基础模型的直接指导。需要一种能直接从H&E图像生成虚拟IHC染色的方法，减少组织有限时的重复切片需求。

Method: 基于SPADE-UNet架构，使用冻结的病理基础模型（UNI）的密集空间标记进行条件化，提供组织级语义指导。采用错位感知损失套件保持染色定量准确性，学习染色嵌入使单模型能同时服务多个IHC标记物。
...
Conclusion: UNIStainNet通过病理基础模型提供语义指导，实现了单模型多标记物的虚拟IHC染色，在保持定量准确性的同时达到最先进性能，为组织有限情况下的快速诊断提供了有效工具。
...
TL;DR: Surg-R1是一个通过四阶段训练流程构建的外科视觉语言模型，采用三层推理层次结构，在多个外科任务上超越了现有专有推理模型和外科专用VLMs。


<details>
  <summary>Details</summary>
Motivation: 现有外科视觉语言模型缺乏可解释的推理链，而通用推理模型又缺乏领域专业知识，无法处理组合式外科任务。外科场景理解需要既准确又可验证的预测。
...
Conclusion: Surg-R1通过分层推理架构和四阶段训练流程，实现了外科场景理解中准确预测与可解释推理的统一，为外科AI系统提供了可验证的决策支持。
...
TL;DR: 研究重新审视模型缝合技术，发现通过简单的特征匹配损失，异构视觉基础模型可以在视觉任务中可靠地缝合，且深层缝合点甚至能超越原始模型性能。


<details>
  <summary>Details</summary>
Motivation: 探索异构视觉基础模型（VFMs）是否可以通过缝合技术实现兼容性，这些模型在目标、数据和模态组合上各不相同（如CLIP、DINOv2、SigLIP 2）。

Method: 引入系统化缝合协议，涵盖缝合点选择、缝合层族、训练损失函数和下游任务。提出VFM缝合树（VST），在多个VFMs之间共享早期层同时保留后期层。

Result: 1) 缝合层训练至关重要，传统方法在浅层缝合点难以保持精度；2) 使用目标模型倒数第二层的简单特征匹配损失，异构VFMs可在视觉任务中可靠缝合；3) 深层缝合点缝合模型能以较小推理开销超越原始模型。

Conclusion: 将缝合从诊断工具提升为实用方法，用于整合互补的VFM优势，并精确定位其表示对齐或分歧的位置，为多模态LLMs提供可控的精度-延迟权衡。
...
### [8] [SAW: Toward a Surgical Action World Model via Controllable and Scalable Video Generation](https://arxiv.org/abs/2603.13024)
...
TL;DR: SAW：基于四种轻量信号（语言提示、参考场景、组织可操作性掩码、工具轨迹）的轨迹条件手术动作视频扩散模型，用于生成真实可控的手术交互视频，解决数据稀缺和仿真-现实差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有手术视频生成方法需要昂贵标注或复杂中间表示，限制了可扩展性；同时缺乏时间一致性和真实感，难以构建有效的手术世界模型来应对数据稀缺、罕见事件合成和仿真-现实差距等挑战。

Method: 提出Surgical Action World (SAW)：基于四种轻量条件信号（语言提示、参考手术场景、组织可操作性掩码、2D工具轨迹）的视频扩散模型；将视频到视频扩散重新表述为轨迹条件手术动作合成；在12,044个腹腔镜剪辑数据集上微调，使用深度一致性损失增强几何合理性。

Result: 在测试数据上达到最先进的时间一致性（CD-FVD: 199.19 vs. 546.82）和强视觉质量；下游应用显示：1）手术AI：用SAW生成视频增强罕见动作，显著提升动作识别性能（夹闭F1分数：20.93%→43.14%；切割：0.00%→8.33%）；2）手术仿真：从仿真器轨迹生成工具-组织交互视频，实现视觉逼真的仿真引擎。

Conclusion: SAW通过轻量条件信号实现了高质量、时间一致的手术动作视频生成，为解决手术AI中的数据稀缺和仿真-现实差距问题提供了有效途径，展示了在手术AI增强和仿真应用中的实用价值。
...
### [9] [InterEdit: Navigating Text-Guided Multi-Human 3D Motion Editing](https://arxiv.org/abs/2603.13082)
...
TL;DR: 本文提出InterEdit3D数据集和InterEdit模型，用于解决文本引导的多人物3D运动编辑任务，通过语义感知和交互感知的token对齐策略提升编辑效果。
...
Method: 提出InterEdit3D数据集和TMME基准，并设计InterEdit模型：采用同步的无分类器条件扩散模型，引入语义感知计划token对齐和交互感知频率token对齐策略。

Result: 实验表明InterEdit在文本-运动一致性和编辑保真度方面表现优异，在TMME任务上达到state-of-the-art性能。

Conclusion: 本文成功解决了多人3D运动编辑的挑战，提出的数据集和模型为这一领域提供了有价值的基准和解决方案。
...
TL;DR: AbSteering框架通过异常中心思维链和直接偏好优化，引导通用视频语言模型生成精确的HRCT诊断报告，在检测敏感性和减少幻觉方面超越领域专用模型。


<details>
  <summary>Details</summary>
Motivation: HRCT诊断报告生成面临高病理多样性和空间稀疏性的挑战，现有通用视频语言模型在医学领域的适应性尚未充分探索，需要专门框架来提升诊断精度。

Method: 提出AbSteering框架：1）异常中心思维链方案强制异常推理；2）直接偏好优化目标利用临床易混淆异常作为硬负样本来增强细粒度区分能力。

Result: 通用视频语言模型在该框架指导下展现出强大的医学影像适应能力，超越预训练的大规模CT基础模型，实现更高的检测敏感性并减少幻觉。

Conclusion: AbSteering成功将通用视频语言模型引导至HRCT报告生成任务，证明了通过适当范式指导，通用模型在医学影像领域具有强大迁移潜力。
...
### [11] [Less Data, Faster Convergence: Goal-Driven Data Optimization for Multimodal Instruction Tuning](https://arxiv.org/abs/2603.12478)
*Rujie Wu,Haozhe Zhao,Hai Ci,Yizhou Wang*

Main category: cs.CV

TL;DR: GDO框架通过计算样本描述符，为不同目标构建优化的训练子集，在固定训练协议下用更少样本实现更快收敛和更高准确率。


<details>
  <summary>Details</summary>
Motivation: 多模态指令调优通常计算效率低下，因为训练资源分散在效用差异很大的混合图像-视频池中，需要优化数据选择以提高效率。

Method: 提出目标驱动数据优化(GDO)框架，为每个候选样本计算六个描述符，针对不同目标构建优化的1×训练子集，在固定训练协议下评估。

Result: 相比512k样本的Uni-10x基线，GDO在MVBench、VideoMME、MLVU和LVBench上分别只需35.4k、26.6k、27.3k和34.7k样本就能达到参考性能，同时准确率分别提升+1.38、+1.67、+3.08和+0.84个百分点。

Conclusion: GDO提供了一个目标驱动的数据优化框架，在固定训练协议下能够用更少的训练样本实现更快的收敛，特别在强调时间性的设置下能获得更好的长视频理解能力。
...
### [12] [CalliMaster: Mastering Page-level Chinese Calligraphy via Layout-guided Spatial Planning](https://arxiv.org/abs/2603.12482)
*Tianshuo Xu,Tiantian Hong,Zhifei Chen,Fei Chao,Ying-cong Chen*

Main category: cs.CV

TL;DR: CalliMaster是一个统一的书法页面生成框架，通过解耦空间规划与内容合成，实现可控的页面级书法生成与编辑。
...
Method: 采用从粗到细的管道（文本→布局→图像），在单一多模态扩散变换器中，空间规划阶段预测字符边界框建立全局空间排列，然后作为几何提示用于内容合成阶段，使用流匹配渲染高保真笔触。

Result: 实现了最先进的生成质量，支持可控的语义重新规划（调整字符大小或位置时自动协调周围空白和笔势），并扩展到文物修复和法证分析等下游应用。

Conclusion: CalliMaster通过解耦空间规划与内容合成，为数字文化遗产提供了一个全面的页面级书法生成与编辑工具。
...
### [13] [RAW-Domain Degradation Models for Realistic Smartphone Super-Resolution](https://arxiv.org/abs/2603.12493)
*Ali Mosleh,Faraz Ali,Fengjia Zhang,Stavros Tsogkas,Junyong Lee,Alex Levinshtein,Michael S. Brown*

Main category: cs.CV

TL;DR: 该论文提出通过设备特定的校准和降解建模来改进智能手机数字变焦中的超分辨率性能，而不是依赖通用的先验知识。


<details>
  <summary>Details</summary>
Motivation: 智能手机数字变焦依赖基于学习的超分辨率模型处理RAW传感器图像，但获取传感器特定的训练数据困难，因为缺乏真实的高分辨率图像。现有的合成数据生成方法存在领域差距问题。

Method: 通过校准建模设备特定的降解过程，将公开可用的渲染图像"反处理"到不同智能手机的RAW域，使用这些图像对训练单图像RAW-to-RGB超分辨率模型。

Result: 实验表明，准确的降解建模带来了显著改进，该超分辨率模型在真实数据上表现优于基于任意选择降解的大规模训练基线。

Conclusion: 精心设计的降解建模可以增强真实世界条件下的超分辨率性能，设备特定的校准方法比通用的降解假设更有效。
...
### [14] [Naïve PAINE: Lightweight Text-to-Image Generation Improvement with Prompt Evaluation](https://arxiv.org/abs/2603.12506)
...
Method: 利用T2I偏好基准，直接从初始噪声和给定提示预测图像质量数值，选择高质量噪声进行生成，提供关于扩散模型生成质量的反馈

Result: 实验结果表明，Naïve PAINE在多个提示语料库基准测试中优于现有方法

Conclusion: Naïve PAINE能够有效提升扩散模型的生成质量，且足够轻量，可以无缝集成到现有扩散模型流程中
...
<details>
  <summary>Details</summary>
Motivation: 现有自回归扩散模型使用滑动窗口缓存会丢弃过去上下文，导致长视频生成中出现保真度下降、身份漂移和运动停滞。当前方法固定保留早期令牌作为注意力锚点，但静态锚点无法反映视频内容的动态演变。
...
Conclusion: MemRoPE通过位置解耦和时态聚合的相互促进机制，实现了无限制生成中的固定大小缓存可行性，显著提升了长视频生成的质量和一致性。
...
Method: 采用基于patch的Masked Image Modeling（MIM）自监督预训练方法，在1,206个无标注CT体积上预训练3D U-Net编码器。下游任务包括：使用VDETR和Vertex Relative Position Encoding的3D损伤检测，以及多标签损伤分类。对于检测任务，采用包含2,000个未标注体积的半监督学习和一致性正则化。

Result: 在仅有144个标注训练样本的情况下，检测任务达到56.57%验证mAP@0.50和45.30%测试mAP@0.50，相比仅监督训练提升了115%。分类任务在2,244个标注样本上，使用冻结编码器在七个损伤类别上达到94.07%测试准确率。

Conclusion: 自监督预训练结合半监督学习能有效解决医学成像中的标签稀缺问题，在有限标注下实现稳健的3D目标检测，证明了自监督特征的可迁移性和标签高效方法的有效性。
...
TL;DR: 本文通过诊断研究发现，当前视觉语言模型在空间推理方面存在系统性缺陷，主要与CLIP风格图像编码器和1D位置编码的设计选择有关，这些设计限制了模型理解2D空间关系的能力。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在通用基准测试上表现出色，但在理解相对位置、布局和计数等基本2D空间关系方面仍然脆弱。作者认为这种失败不仅是数据问题，更与当前VLM管道中的主导设计选择密切相关。

Method: 在LLaVA框架内进行受控诊断研究，比较前沿模型和LLaVA变体在一系列空间基准测试上的表现。对比基于CLIP的编码器与使用更密集或生成目标的替代编码器，以及增强2D位置编码的变体。

Result: 研究结果显示不同模型间存在一致的空间性能差距，表明编码器目标和位置结构确实影响空间行为，但并不能完全解决空间推理问题。

Conclusion: 当前视觉语言模型的空间推理缺陷与CLIP风格图像编码器和1D位置编码的设计选择密切相关，需要重新思考VLM架构以改善空间理解能力。
...
TL;DR: CLGT框架通过因果学习和几何拓扑融合解决跨视角地理定位问题，在多个数据集上实现SOTA性能
...
Method: 提出CLGT框架，包含因果特征提取器（CFE）减少混淆因素影响，几何拓扑融合模块（GT Fusion）注入BEV道路拓扑，以及数据自适应池化模块（DA Pooling）增强语义丰富区域表示

Result: 在CVUSA、CVACT及其鲁棒性增强变体上实现最先进性能，特别是在具有挑战性的真实世界损坏情况下表现优异

Conclusion: CLGT框架通过因果干预和几何拓扑融合有效解决了跨视角地理定位中的视角差异和混淆因素问题，为复杂真实场景提供了鲁棒的解决方案
...
<details>
  <summary>Details</summary>
Motivation: DiTs在高分辨率下具有强大的可扩展性和对齐能力，但密集空间标记的二次自注意力导致高推理延迟，限制了部署。研究发现去噪过程在空间上对美学描述符是非均匀的，与美学标记相关的区域接收集中的交叉注意力并显示较大的时间变化，而低亲和力区域则平滑演化，存在冗余计算

Method: 提出AccelAes框架：1）构建AesMask（一次性美学焦点掩码），基于提示语义和交叉注意力信号；2）SkipSparse在局部计算可行时重新分配计算和引导到掩码区域；3）使用轻量级步级预测缓存定期替换完整的Transformer评估，减少时间冗余

Result: 在代表性DiT家族上的实验显示一致的加速和美学导向质量改进。在Lumina-Next上，AccelAes实现了2.11倍加速，ImageReward比密集基线提高了+11.9%

Conclusion: AccelAes通过美学感知的时空减少有效加速DiTs推理，同时提升感知美学质量，为高效高质量图像生成提供了实用解决方案
...
### [20] [MRGeo: Robust Cross-View Geo-Localization of Corrupted Images via Spatial and Channel Feature Enhancement](https://arxiv.org/abs/2603.12587)
*Le Wu,Lv Bo,Songsong Ouyang,Yingying Zhu*

Main category: cs.CV

TL;DR: MRGeo是首个针对跨视角地理定位在图像损坏条件下的鲁棒性方法，通过分层防御策略提升特征质量和几何一致性，在多个鲁棒性基准测试中平均R@1提升2.92%。
...
Conclusion: MRGeo是首个系统性的跨视角地理定位鲁棒性方法，通过分层防御策略有效应对图像损坏问题，在鲁棒性基准和标准数据集上均表现优异，为实际部署提供了可靠解决方案。
...
### [21] [Neural Gate: Mitigating Privacy Risks in LVLMs via Neuron-Level Gradient Gating](https://arxiv.org/abs/2603.12598)
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在金融、医疗等关键领域的应用日益广泛，但其安全性和隐私风险也随之增加。现有隐私保护方法在泛化性和非破坏性方面存在局限，难以处理未见过的隐私查询，且可能影响模型的标准任务性能。

Method: Neural Gate通过神经元级模型编辑来缓解隐私风险。该方法学习一个特征向量来识别模型中与隐私相关概念相关的神经元，然后精确指导模型参数的更新，从而提高模型对隐私相关问题的拒绝率。

Result: 在MiniGPT和LLaVA上的综合实验表明，该方法显著提升了模型的隐私保护能力，同时保持了模型的原始效用。

Conclusion: Neural Gate提供了一种有效的方法来增强大型视觉语言模型的隐私保护，能够在处理新颖敏感查询时保持保护行为，同时不损害模型的标准任务性能。
...
### [22] [A Prediction-as-Perception Framework for 3D Object Detection](https://arxiv.org/abs/2603.12599)
*Song Zhang,Haoyu Chen,Ruibo Wang*

Main category: cs.CV

TL;DR: 论文提出预测即感知（PAP）框架，将预测-感知架构集成到3D物体感知任务中，通过连续帧信息预测未来位置并作为查询输入，提升感知准确性和效率。
...
Method: 提出预测即感知（PAP）框架，包含预测和感知两个主要模块。预测模块基于当前帧感知结果预测自车和周围交通参与者的未来位置，这些预测位置作为查询传递给下一帧的感知模块，感知结果再迭代反馈给预测模块。

Result: 在nuScenes数据集上使用端到端模型UniAD评估PAP结构，结果显示目标跟踪精度提升10%，推理速度提高15%，显著提升感知模型的效率和准确性，同时减少计算资源消耗。

Conclusion: 这种仿生设计显著提升了感知模型的效率和准确性，同时降低了计算资源消耗，为3D物体感知任务提供了有效的架构改进方案。
...
### [23] [A2Z-10M+: Geometric Deep Learning with A-to-Z BRep Annotations for AI-Assisted CAD Modeling and Reverse Engineering](https://arxiv.org/abs/2603.12605)
*Pritham Kumar Jena,Bhavika Baburaj,Tushar Anand,Vedant Dutta,Vineeth Ulavala,Sk Aziz Ali*

Main category: cs.CV

TL;DR: 该研究创建了A2Z数据集，包含100万个CAD模型的多模态标注，用于支持BRep学习和CAD逆向工程任务。
...
Method: 构建了包含100万个ABC CAD模型的A2Z数据集，包含高分辨率网格、3D手绘草图、BRep几何拓扑信息、文本描述等多模态标注，并整合了25,000个专业设计的电子外壳模型。

Result: 创建了存储需求达5TB的大规模数据集，开发了新颖评估指标，并训练了基础模型在15万个CAD模型上检测BRep共边和角点，在CAD逆向工程关键任务中表现良好。

Conclusion: A2Z数据集为CAD学习和检索任务提供了前所未有的多模态资源，将公开发布以支持相关研究，特别是在CAD逆向工程和快速原型设计领域。
...
### [24] [Mastering Negation: Boosting Grounding Models via Grouped Opposition-Based Learning](https://arxiv.org/abs/2603.12606)
...
Method: 引入D-Negation数据集，提供带有正向和负向语义描述的对象标注。提出分组对立学习框架，将D-Negation中的对立语义描述组织成结构化组，并制定两个互补的损失函数，鼓励模型推理否定和语义限定词。

Result: 将数据集和学习策略集成到最先进的语言基础定位模型中，仅微调不到10%的参数，在正向和负向语义评估上分别实现了高达4.4 mAP和5.7 mAP的改进。
...
### [25] [LR-SGS: Robust LiDAR-Reflectance-Guided Salient Gaussian Splatting for Self-Driving Scene Reconstruction](https://arxiv.org/abs/2603.12647)
*Ziyu Chen,Fan Zhu,Hui Zhu,Deyi Kong,Xinkai Kuang,Yujia Zhang,Chunmao Jiang*

Main category: cs.CV

TL;DR: LR-SGS是一种用于自动驾驶场景的鲁棒高效LiDAR反射率引导的显著高斯泼溅方法，通过利用LiDAR点云的几何和反射率信息，结合RGB数据，在复杂光照和高速运动场景中实现更好的重建效果。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法主要依赖相机或仅使用LiDAR进行高斯初始化或深度监督，未能充分利用点云中的反射率等丰富场景信息，也未充分挖掘LiDAR和RGB的互补性，导致在高速运动和复杂光照等挑战性自动驾驶场景中性能下降。

Method: 提出LR-SGS方法：1) 从LiDAR提取几何和反射率特征点初始化结构感知的显著高斯表示；2) 通过显著变换和改进的密度控制来精炼高斯，捕捉边缘和平面的结构；3) 将LiDAR强度校准为反射率并附加到每个高斯作为光照不变的材料通道；4) 与RGB联合对齐以增强边界一致性。

Result: 在Waymo开放数据集上的实验表明，LR-SGS以更少的高斯数和更短的训练时间实现了优越的重建性能。特别是在复杂光照场景中，该方法比OmniRe高出1.18 dB的PSNR。

Conclusion: LR-SGS通过充分利用LiDAR点云的几何和反射率信息，结合RGB数据的互补性，在自动驾驶场景中实现了更鲁棒和高效的重建，特别是在复杂光照和高速运动等挑战性条件下表现出色。
...
### [26] [RoboStereo: Dual-Tower 4D Embodied World Models for Unified Policy Optimization](https://arxiv.org/abs/2603.12639)
*Ruicheng Zhang,Guangyu Chen,Zunnan Xu,Zihao Liu,Zhizhou Zhong,Mingyang Zhang,Jun Zhou,Xiu Li*

Main category: cs.CV

TL;DR: RoboStereo是一个对称双塔4D世界模型，通过双向跨模态增强确保时空几何一致性，并基于此提出了首个世界模型驱动的统一策略优化框架，在精细操作任务上实现了>97%的平均相对改进。
...
Method: 提出了RoboStereo对称双塔4D世界模型，采用双向跨模态增强确保几何一致性；并提出了统一策略优化框架：测试时策略增强(TTPA)、模仿进化策略学习(IEPL)和开放探索策略学习(OEPL)。
...
Method: 提出MV-GRPO方法：使用条件增强器为原始提示生成语义相邻但多样的字幕，创建多视图奖励映射；通过计算原始样本在这些新字幕下的条件概率分布，实现无需重新生成样本的多视图优势重估计。

Result: 大量实验表明，MV-GRPO在偏好对齐性能上优于现有最先进方法。

Conclusion: MV-GRPO通过密集多视图评估方案有效增强了GRPO框架，解决了单视图评估的局限性，为文本到图像模型的偏好对齐提供了更有效的优化信号。
...
<details>
  <summary>Details</summary>
Motivation: 传统世界模型主要关注生成未来视频帧的光度细节，但预测结果往往几何不一致。需要一种专注于几何一致性的预测方法。

Method: 将冻结VGGT的潜在标记作为世界状态，训练轻量级时序流变换器自回归预测其未来轨迹。采用干净目标参数化解决高维特征空间中的流匹配问题，使用两阶段潜在流强制课程解决自回归展开中的暴露偏差。

Result: 在KITTI、Cityscapes和TartanAir数据集上，VGGT-World在深度预测方面显著优于最强基线，运行速度快3.6-5倍，仅需0.43B可训练参数。

Conclusion: 冻结几何基础模型特征是3D世界建模的有效且高效的预测状态，为几何一致的场景演化预测提供了新方法。
...
Method: 首先引入轻量级尺度对齐阶段恢复多视图尺度一致性，然后通过轻量级任务特定适配器将预训练的VFM特征集成到神经体积重建流程中，这些适配器在训练时保持预训练表示的跨域鲁棒性。

Result: 在ScanNet训练集上训练，在ScanNet测试集和域外TUM RGB-D、Tanks and Temples数据集上评估。在所有数据集域上都达到了最先进的性能，特别是在具有挑战性的室外Tanks and Temples数据集上，F1分数达到70.1，显著优于最接近的竞争对手VGGT的51.8。
...
TL;DR: AVION是一个针对遥感图像视觉语言模型适应的知识蒸馏框架，通过教师模块构建语义丰富的文本原型，学生模块集成轻量级可学习提示，在六个光学遥感基准测试中提升了少样本分类和跨模态检索性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在遥感图像适应方面面临两个主要挑战：1) 文本表示的语义覆盖有限；2) 视觉特征的适应性不足。这些问题在涉及多种视觉外观和细粒度对象区分的航空场景中尤为显著。

Method: 提出AVION知识蒸馏框架，包含教师模块和学生模块。教师模块通过收集大型语言模型的描述并利用遥感图像特征验证有效性，构建语义丰富的文本原型。学生模块将轻量级可学习提示集成到视觉和语言编码器中，在教师指导下对齐嵌入及其跨模态关系。训练完成后，学生模块在推理时独立运行。

Result: 在六个光学遥感基准测试中，AVION提高了少样本分类和基础类别的准确性，同时不降低对新类别的泛化能力。它还提升了跨模态检索的平均召回率，且仅需极少的额外可训练参数。

Conclusion: AVION框架有效解决了遥感图像视觉语言模型适应中的语义覆盖和特征适应性问题，通过知识蒸馏方法在多个遥感任务上取得了显著性能提升，具有实用价值。
...
TL;DR: 本文提出了一种基于预训练EfficientNet情感识别模型的快速方法，用于ABAW竞赛中的帧级面部情感理解任务，通过置信度阈值和MLP处理，结合滑动窗口平滑，显著提升了验证指标。


<details>
  <summary>Details</summary>
Motivation: 参与第10届ABAW竞赛，旨在解决野外环境下的帧级面部情感理解任务，包括面部表情识别、效价-唤醒度估计和动作单元检测，需要高效准确的方法来处理这些复杂的多任务问题。

Method: 1. 使用预训练的EfficientNet情感识别模型提取面部嵌入特征；2. 设置置信度阈值，高于阈值时直接使用模型预测，否则将嵌入特征输入到在AffWild2数据集上训练的多层感知机；3. 对帧级预测结果使用固定大小的滑动窗口进行平滑处理以减少噪声；4. 对于细粒度暴力检测任务，探索多种预训练架构提取帧嵌入特征并进行聚合用于视频分类。

Result: 在ABAW挑战赛的四个任务上的实验结果表明，该方法显著提升了验证指标，优于现有基线方法。

Conclusion: 提出的基于预训练EfficientNet模型和置信度阈值的方法，结合滑动窗口平滑处理，能够有效处理帧级面部情感理解任务，在ABAW竞赛中取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有点云配准方法在面对真实世界挑战（如严重噪声、显著遮挡和大尺度变换）时经常失败，导致配准精度受损和在复杂环境中鲁棒性不足。需要开发一种能够有效适应多样化点云结构和复杂变换的鲁棒配准框架。

Method: IGASA采用分层金字塔架构进行多尺度特征提取和融合，包含两个关键组件：1）分层跨层注意力模块，利用跳跃注意力机制对齐多分辨率特征并增强局部几何一致性；2）迭代几何感知细化模块，在精细匹配阶段利用粗匹配阶段建立的可靠对应关系进行细化。

Result: 在3D(Lo)Match、KITTI和nuScenes四个广泛认可的基准数据集上的实验表明，IGASA显著超越了现有最先进方法，在配准精度方面实现了显著提升。
...
Result: 在四个流行的VLM基准测试中验证：1. MMMU上比最佳VLM提升8.09%准确率；2. MMMU-Pro上提升4.87%准确率；3. 在生成任务上（A-OKVQA和OCR-VQA）优于Intern-VL2-8b和Qwen2.5-VL-7b这两个顶级VLM；4. 即使没有多数共识或多数VLM预测错误时，仍能产生高性能的双焦点多样性融合预测。

Conclusion: V3Fusion方法能够有效利用异构模型的认知不确定性，动态缓解幻觉问题，在视觉-语言推理任务中实现高性能的融合预测，特别是在缺乏多数共识或多数模型出错的情况下仍能保持鲁棒性。
...
Method: 提出CognitionCapturerPro框架，整合EEG与多模态先验，采用不确定性加权相似性评分机制量化模态特定保真度，使用融合编码器整合共享表示，结合简化对齐模块和预训练扩散模型。
...
### [35] [MoKus: Leveraging Cross-Modal Knowledge Transfer for Knowledge-Aware Concept Customization](https://arxiv.org/abs/2603.12743)
*Chenyang Zhu,Hongxiang Li,Xiu Li,Long Chen*

Main category: cs.CV

TL;DR: 提出知识感知概念定制新任务和MoKus框架，通过跨模态知识转移实现高保真定制生成，优于现有方法
...
Method: 提出MoKus框架，包含两个阶段：1)视觉概念学习阶段学习锚点表示存储视觉信息；2)文本知识更新阶段更新知识查询的答案到锚点表示，实现跨模态知识转移

Result: MoKus在提出的KnowCusBench基准上优于最先进方法，并能扩展到虚拟概念创建和概念擦除等应用，在世界知识基准上也有改进

Conclusion: 知识感知概念定制是一个重要新任务，MoKus框架通过跨模态知识转移有效解决该问题，具有良好扩展性和应用前景
...
### [36] [FC-Track: Overlap-Aware Post-Association Correction for Online Multi-Object Tracking](https://arxiv.org/abs/2603.12758)
*Cheng Ju,Zejing Zhao,Akio Namiki*

Main category: cs.CV

TL;DR: FC-Track是一个轻量级的在线多目标跟踪后关联校正框架，专门针对重叠引起的身份切换问题，通过IoA过滤和局部相似性比较来纠正误匹配，无需全局优化或重识别。


<details>
  <summary>Details</summary>
Motivation: 在线多目标跟踪方法在复杂动态环境中容易因频繁遮挡和物体重叠导致身份切换，错误的关联会随时间传播并降低跟踪可靠性。现有方法对此问题处理不足。

Method: 提出基于IoA的过滤策略抑制高重叠条件下的不可靠外观更新，通过重叠轨迹对内的外观相似性比较局部校正检测到轨迹的误匹配，防止短期误匹配传播为长期身份切换。
...
Conclusion: FC-Track框架有效缓解了重叠引起的长期身份切换问题，无需全局优化或重识别，适合实时机器人应用，在MOT20基准上保持最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 对抗性补丁能够劫持Vision Transformers的自注意力机制，将注意力集中到小的高对比度区域，导致错误分类。研究发现包含对抗噪声的token具有不同的统计特性，这为检测和防御提供了机会。

Method: 提出STRAP-ViT防御机制：1) 检测阶段：使用Jensen-Shannon散度作为度量标准，分离表现为异常的token；2) 缓解阶段：对异常token应用随机复合变换，使对抗噪声失效。该机制作为不可训练的即插即用模块集成到ViT架构中，仅用于推理。

Result: 在多个预训练视觉Transformer架构（ViT-base-16和DinoV2）和数据集（ImageNet和CalTech-101）上测试，针对多种对抗攻击（Adversarial Patch、LAVAN、GDPA和RP2），STRAP-ViT提供了优秀的鲁棒性准确率，与干净基线仅相差2-3%，并优于现有最先进方法。

Conclusion: STRAP-ViT是一种有效、低成本的对抗性补丁防御方案，能够在不增加训练成本的情况下显著提高Vision Transformers的鲁棒性，为实际部署提供了可行的解决方案。
...
TL;DR: Cheers是一个统一的多模态模型，通过将补丁级细节与语义表示解耦，在单一模型中同时实现视觉理解和图像生成，实现4倍令牌压缩和高效训练。
...
Result: 在流行基准测试中，Cheers在视觉理解和生成方面匹配或超越了先进的UMMs，实现4倍令牌压缩，支持更高效的高分辨率图像编码和生成。在GenEval和MMBench上优于Tar-1.5B，仅需20%训练成本。
...
### [39] [Hierarchical Dual-Change Collaborative Learning for UAV Scene Change Captioning](https://arxiv.org/abs/2603.12832)
*Fuhai Chen,Pengpeng Huang,Junwen Wu,Hehong Zhang,Shiping Wang,Xiaoguang Ma,Xuri Ge*

Main category: cs.CV

TL;DR: 该论文提出了一种新的无人机场景理解任务——无人机场景变化字幕生成（UAV-SCC），旨在为移动视角拍摄的动态航拍图像中的语义变化生成自然语言描述。论文设计了HDC-CL方法，包含DALT变换器和HCM-OCC校准机制，并构建了新的基准数据集。


<details>
  <summary>Details</summary>
Motivation: 传统的变化字幕生成主要关注固定视角下随时间变化的图像差异，而无人机拍摄的图像对差异既来自时间变化，也来自相机移动导致的空间视角变化。现有方法难以处理这种部分重叠的场景内容，需要理解视角引起的场景变化并有效利用图像间的相对方向信息。

Method: 提出了分层双变协同学习（HDC-CL）方法：1）设计了动态自适应布局变换器（DALT），自适应建模图像对的不同空间布局，在统一的编码层中学习重叠和非重叠区域的相互关联特征；2）提出了分层跨模态方向一致性校准（HCM-OCC），增强模型对视角偏移方向的敏感性。

Result: 构建了新的无人机场景变化字幕生成基准数据集UAV-SCC。大量实验表明，所提出的方法在该任务上达到了最先进的性能。

Conclusion: 论文成功定义了无人机场景变化字幕生成这一新任务，提出了有效的HDC-CL方法，构建了基准数据集，为无人机场景理解研究提供了新的方向和工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准测试在多个维度上推进了评估，但对模型如何维护世界状态的观察仍然不足。需要一种方法来诊断视频理解系统中的状态维护能力。

Method: 将计数重新定位为诊断世界状态维护能力的最小探针，将这种能力分解为对象计数（跟踪当前可见对象 vs. 跟踪累积唯一身份）和事件计数（检测瞬时动作 vs. 跟踪完整活动周期），形成8个细分子类别。包含406个视频，带有逐帧标注的10,071个事件发生时刻和对象状态变化时刻，生成1,000个流式QA对和4,576个时间线查询点。通过流式多点查询观察状态维护轨迹，设计三个互补指标来诊断数值精度、轨迹一致性和时间感知。

Result: 对主流视频语言模型的评估显示，当前模型在时空状态维护方面仍存在显著缺陷，特别是在周期性事件计数等任务上表现困难。

Conclusion: VCBench为测量和改进视频理解系统中的状态维护提供了一个诊断框架，揭示了当前模型在维护世界状态方面的局限性，并为未来改进指明了方向。
...
### [41] [Team LEYA in 10th ABAW Competition: Multimodal Ambivalence/Hesitancy Recognition Approach](https://arxiv.org/abs/2603.12848)
*Elena Ryumina,Alexandr Axyonov,Dmitry Sysoev,Timur Abdulkadirov,Kirill Almetov,Yulia Morozova,Dmitry Ryumin*

Main category: cs.CV

TL;DR: 该论文提出了一种多模态方法用于视频级别的犹豫/矛盾情绪识别，通过融合场景、面部、音频和文本四种模态，在BAH语料库上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 无约束视频中的犹豫/矛盾情绪识别具有挑战性，因为这种行为状态具有微妙、多模态和上下文依赖的特性。现有方法需要更好地整合多种互补信息源来准确识别这种复杂情绪状态。

Method: 采用四模态融合方法：1) 使用VideoMAE模型捕捉场景动态；2) 通过情感帧级嵌入和统计池化编码面部信息；3) 使用EmotionWav2Vec2.0提取声学特征，并通过Mamba时序编码器处理；4) 使用微调的transformer文本模型建模语言线索。最后通过多模态融合模型（包括原型增强变体）整合各模态嵌入。

Result: 在BAH语料库上的实验显示：最佳单模态配置的平均MF1为70.02%，最佳多模态融合模型达到83.25%。通过五个原型增强融合模型的集成，最终测试性能达到71.43%，证明了多模态融合的显著优势。

Conclusion: 研究结果表明，互补的多模态线索和鲁棒的融合策略对于犹豫/矛盾情绪识别至关重要。多模态方法相比单模态基线有显著性能提升，为复杂情绪状态识别提供了有效解决方案。
...
TL;DR: 提出HFP-SAM框架，通过频率域引导和全视图Mamba模块解决海洋动物分割中的长距离建模和细节感知问题


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在海洋动物分割中存在长距离建模困难，而Segment Anything Model (SAM)缺乏对细粒度细节和频率信息的感知能力

Method: 提出分层频率提示SAM框架：1) 频率引导适配器(FGA)通过频率域先验掩码注入海洋场景信息；2) 频率感知点选择(FPS)生成高亮区域作为点提示；3) 全视图Mamba(FVM)以线性计算复杂度提取空间和通道上下文信息

Result: 在四个公开数据集上的大量实验证明了该方法的优越性能

Conclusion: HFP-SAM通过频率域引导和高效的上下文建模，实现了高性能的海洋动物分割，源代码已公开
...
### [43] [Finite Difference Flow Optimization for RL Post-Training of Text-to-Image Models](https://arxiv.org/abs/2603.12893)
...
Method: 提出在线RL变体：1) 采样配对轨迹并比较；2) 将流速度拉向更有利图像方向；3) 将整个采样过程视为单一动作而非逐步处理；4) 使用高质量视觉语言模型和现成质量指标作为奖励。

Result: 方法收敛更快，在输出质量和提示对齐方面优于先前方法，通过广泛指标评估验证了效果。

Conclusion: 将整个采样过程视为单一动作的在线RL方法能有效减少方差，加速收敛，并提高扩散模型在图像质量和提示对齐方面的性能。
...
TL;DR: 本文针对文档布局分析中传统目标检测指标(IoU、F1、mAP)不适用于2D印刷媒体的局限性，提出了SSU语义结构标注方法和COTe分解式评估指标，显著提升了模型性能评估的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统文档布局分析模型使用为3D空间投影设计的对象检测指标(IoU、F1、mAP)来评估性能，但这些指标不适用于原生2D的印刷媒体，导致模型性能评估存在误导性和信息不足的问题。

Method: 提出了两种创新方法：1) 结构语义单元(SSU) - 一种关系标注方法，将焦点从物理结构转向内容语义结构；2) COTe评分 - 一种可分解的页面解析质量度量指标，包含覆盖率、重叠度、侵入度和冗余度四个维度。

Result: 在3个DLA数据集上评估5个常见模型，COTe评分比传统指标更具信息量，能揭示模型的不同失败模式(如破坏语义边界或重复解析同一区域)。COTe将解释-性能差距相对F1减少了76%，且即使没有显式SSU标注，其粒度鲁棒性仍基本保持。

Conclusion: SSU标注方法和COTe评分系统为文档布局分析提供了更稳健、可比较和细致的评估框架，显著改善了模型性能评估的准确性和可解释性，并降低了使用门槛。作者发布了SSU标注数据集和Python库以促进应用。
...
TL;DR: 提出了一种基于语义锚点的结构忠实框架来解决机器学习遗忘中的结构塌陷问题，通过保持保留知识的语义关系来平衡删除与保留的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习遗忘方法往往忽视保留实例之间的语义关系，导致模型出现渐进式结构塌陷，破坏了删除与保留的平衡。需要一种能够保持知识结构的方法来解决这一问题。

Method: 提出结构忠实框架，引入语义锚点作为参考点来维护知识结构。通过语义编码器（如CLIP）从语言驱动的属性描述中实例化锚点，采用结构感知对齐和正则化：前者围绕锚点对齐遗忘前后保留知识的组织结构，后者调节结构关键参数的更新。

Result: 在图像分类、检索和人脸识别任务中，性能分别平均提升了32.9%、22.5%和19.3%，有效平衡了删除-保留权衡并增强了泛化能力。

Conclusion: 通过引入语义锚点来保持知识结构，提出的框架成功解决了机器学习遗忘中的结构塌陷问题，在多个任务上显著提升了性能，实现了更好的删除-保留平衡。
...
TL;DR: 论文提出了Dyn-Bench基准测试，用于评估多模态大语言模型在动态4D场景中的时空推理和动态物体定位能力，发现现有模型在这两方面表现不一致，并提出结构化集成方法显著提升性能。
...
Method: 通过从大量2D和4D数据源进行多阶段筛选，构建了Dyn-Bench基准测试，包含1k视频、7k视觉问答对和3k动态物体定位对。采用掩码引导融合和时空文本认知图等结构化集成方法来增强模型的动态感知能力。

Result: 现有模型无法同时在时空推理和动态物体定位方面保持强劲性能，经常产生对运动和交互的不一致解释。传统提示策略改进有限，而结构化集成方法显著提升了模型的动态感知和时空推理能力。

Conclusion: Dyn-Bench为评估多模态大语言模型在动态4D世界中的理解能力提供了系统基准。结构化集成方法比传统提示策略更有效，为模型在动态场景中的感知和推理能力提升提供了新方向。
...
### [47] [SortScrews: A Dataset and Baseline for Real-time Screw Classification](https://arxiv.org/abs/2603.13027)
*Tianhao Fu,Bingxuan Yang,Juncheng Guo,Shrena Sribalan,Yucheng Chen*

Main category: cs.CV

TL;DR: SortScrews数据集：一个用于螺丝分类的公开数据集，包含560张RGB图像，覆盖6种螺丝类型和背景类，提供数据收集脚本和基线模型结果。
...
Method: 创建SortScrews数据集，包含560张512×512分辨率的RGB图像，覆盖6种螺丝类型和背景类；使用标准化采集设置，包含光照和相机视角的轻微变化；提供可重复使用的数据收集脚本；使用在ImageNet上预训练的EfficientNet-B0和ResNet-18进行迁移学习作为基线。

Result: 尽管数据集规模有限，这些轻量级模型实现了强大的分类准确率，表明受控采集条件即使在相对较小的数据集上也能实现有效学习。

Conclusion: SortScrews数据集填补了螺丝分类领域公开数据集的空白，其标准化采集设置和可扩展的数据收集脚本为工业自动化应用提供了有价值的资源，证明了在受控条件下使用小数据集进行有效分类的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有语义感知水印方法依赖单一全局语义绑定，容易受到局部但全局一致的语义编辑攻击，需要更细粒度的语义绑定来提供可信的语义感知水印。

Method: 提出SLICE框架，将图像语义解耦为四个语义因子（主体、环境、动作、细节），并将它们精确锚定到初始高斯噪声的不同区域，实现细粒度语义绑定。

Result: SLICE显著优于现有基线方法，对抗高级语义引导再生攻击时大幅降低攻击成功率，同时保持图像质量和语义保真度，并提供统计保证的误接受率。

Conclusion: SLICE提供了一种实用、无需训练的溯源解决方案，具有细粒度诊断能力和对现实对抗操作的鲁棒性。
...
Result: 系统显著优于最先进的检测和识别方法，在真实机器人上实现更好的任务完成效果

Conclusion: "Show, Don't Tell"范式通过绕过语言描述，能够快速训练针对演示中相关物体的定制化检测器，提高机器人对新物体的识别能力
...
TL;DR: 该研究提出了一种用于野外条件下连续情感识别的多模态方法，结合面部、行为和音频三种模态，在Aff-Wild2数据集上取得了0.658的CCC分数。
...
Method: 方法结合三种互补模态：1）面部模态使用GRADA帧级嵌入和Transformer时序回归；2）行为模态使用Qwen3-VL-4B-Instruct从视频片段提取信息，Mamba建模时序动态；3）音频模态使用WavLM-Large和注意力统计池化，包含跨模态过滤减少不可靠/非语音段影响。融合策略探索两种：定向跨模态专家混合融合和可靠性感知视听融合。

Result: 在Aff-Wild2开发集上，提出的多模态融合策略取得了0.658的Concordance Correlation Coefficient (CCC)。

Conclusion: 该多模态方法有效结合了面部、行为和音频信息，在野外情感识别任务中表现出色，证明了多模态融合策略的有效性。
...
### [51] [Geometry-Guided Camera Motion Understanding in VideoLLMs](https://arxiv.org/abs/2603.13119)
...
Method: 1) 创建大规模合成数据集CameraMotionDataset；2) 将相机运动建模为约束感知的多标签识别；3) 构建VQA基准CameraMotionVQA；4) 提出轻量级、模型无关的流水线，从3D基础模型提取几何相机线索，使用时序分类器预测约束运动基元，通过结构化提示注入到下游VideoLLM推理中。

Result: 实验显示现有VideoLLMs在识别相机运动基元上存在显著错误，Qwen2.5-VL视觉编码器中相机运动线索表示较弱（尤其在深层ViT块中）。提出的方法改善了运动识别能力，使模型响应更具相机感知性。

Conclusion: 几何驱动的线索提取和结构化提示是迈向相机感知VideoLLM和VLA系统的实用步骤，数据集和基准已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 在体现系统和增强现实系统中，基于透视图像训练的基础模型在360°全景图像上的性能会显著下降，需要专门针对高分辨率全景图像设计的分割模型。

Method: 将全景分割重新定义为固定轨迹的透视视频分割，将全景图分解为沿着连续球面遍历采样的重叠透视补丁。使用InfiniGen引擎合成了183,440张4K分辨率全景图像进行大规模监督训练。

Result: 在真实世界4K全景基准测试中，SAP相比不同尺寸的原始SAM2实现了+17.2的零样本mIoU增益，表现出优异的泛化能力。

Conclusion: SAP通过轨迹对齐的范式有效解决了全景分割问题，为高分辨率全景图像提供了强大的实例分割基础模型。
...
TL;DR: 本文提出了一种高保真上下文学习（HiFICL）方法，通过虚拟键值对、低秩分解和端到端训练目标，更准确地建模大型多模态模型的上下文学习机制，在多个多模态基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型中的上下文学习性能对演示配置敏感且计算成本高，现有近似方法通过学习"偏移向量"简化了过程，但不够精确。受精确分解的启发，需要更忠实地建模上下文学习机制。

Method: 提出HiFICL方法，包含三个关键组件：1）作为可学习上下文的虚拟键值对集合；2）用于稳定和正则化训练的低秩分解；3）简单的端到端训练目标。该方法构成了一种上下文感知的参数高效微调形式。

Result: 在多个多模态基准测试上的广泛实验表明，HiFICL始终优于现有的近似方法。

Conclusion: HiFICL通过更准确地建模上下文学习机制，提供了一种有效的参数高效微调方法，在保持计算效率的同时提高了性能。
...
TL;DR: 论文提出Visual-ERM，一种多模态生成奖励模型，用于评估视觉到代码任务的质量，通过直接在渲染的视觉空间中提供细粒度、可解释、任务无关的反馈，显著提升了强化学习在图表、表格和SVG解析任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉到代码任务中，现有奖励信号存在两个主要问题：一是基于文本规则的奖励无法捕捉细粒度视觉差异，二是基于粗略视觉嵌入相似性的奖励容易受到奖励攻击。这两种方法都无法准确评估视觉保真度，导致强化学习效果不佳。

Method: 提出Visual-ERM（视觉等价奖励模型），这是一个多模态生成奖励模型，直接在渲染的视觉空间中评估视觉到代码的质量。该方法提供细粒度、可解释、任务无关的反馈，并集成到强化学习中。同时构建了VC-RewardBench基准，用于评估结构化视觉数据上的图像到图像差异判断能力。

Result: Visual-ERM在Qwen3-VL-8B-Instruct上提升了图表到代码任务8.4分，在表格和SVG解析任务上分别平均提升2.7和4.1分。在VC-RewardBench基准测试中，8B参数的Visual-ERM显著优于235B参数的Qwen3-VL-Instruct，并接近领先的闭源模型性能。

Conclusion: 细粒度的视觉奖励监督对于视觉到代码的强化学习既是必要的也是充分的，无论任务特异性如何。Visual-ERM通过直接在视觉空间中提供精确反馈，有效解决了现有奖励信号的局限性，为视觉到代码任务提供了更可靠的评估机制。
...
TL;DR: TerraFlow是一种用于地球观测的多模态、多时间序列学习方法，通过时间训练目标实现跨空间、时间和模态的序列感知学习，在GEO-Bench-2基准测试中优于现有基础模型，并在自然灾害风险图预测方面取得突破。


<details>
  <summary>Details</summary>
Motivation: 地球观测数据具有多模态、多时间序列的特点，且输入长度可变。现有基础模型在处理这类数据时存在局限性，特别是在自然灾害风险图预测等复杂任务上表现不佳。

Method: 提出TerraFlow方法，基于时间训练目标实现跨空间、时间和模态的序列感知学习，能够处理真实世界地球观测数据中常见的变长输入。

Result: 在GEO-Bench-2基准测试的所有时间任务中优于现有最先进的基础模型，在自然灾害风险图预测任务上表现突出，F1分数提升高达50%，Brier分数提升24%。

Conclusion: TerraFlow为地球观测的多模态、多时间序列学习提供了有效解决方案，在复杂任务如自然灾害风险预测方面展现出显著优势，为深度学习在地球观测领域的应用开辟了新方向。
...
TL;DR: 该论文提出了SAVA-X框架，用于解决从第三人称演示评估第一人称模仿的错误检测问题，处理跨视角域偏移、时间不对齐和冗余等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有错误检测方法通常假设单视角设置，无法处理使用第三人称演示评估第一人称模仿的实际场景。Ego→Exo模仿错误检测面临跨视角域偏移、时间不对齐和大量冗余等挑战。

Method: 提出SAVA-X框架，采用对齐-融合-检测架构，包含：(i)视角条件自适应采样，(ii)场景自适应视角嵌入，(iii)双向交叉注意力融合。

Result: 在EgoMe基准测试中，SAVA-X在AUPRC和平均tIoU指标上持续优于所有基线方法，消融实验确认了其各组成部分的互补优势。

Conclusion: SAVA-X框架有效解决了跨视角模仿错误检测问题，为工业训练、医疗和装配质量控制等实际应用提供了解决方案。
...
Method: 提出Anchor-based Motion Guidance (AMG)构建结构稳定且空间代表性的锚点，通过最优传输建立对应关系实现一致的变形传播；配合Color Uncertainty-guided Appearance Refinement (CUAR)通过估计每个高斯的颜色不确定性，选择性细化易产生遮挡伪影的区域。

Result: 大量实验表明，Catalyst4D实现了时间稳定、高保真度的动态场景编辑，在视觉质量和运动一致性方面优于现有方法。

Conclusion: Catalyst4D通过锚点运动引导和颜色不确定性外观细化，成功解决了动态场景编辑中的时空一致性问题，为高质量4D高斯场景编辑提供了有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA架构中，VLM倾向于弱化细粒度几何线索且缺乏显式时间证据，而现有注入方法要么关注静态空间表示，要么需要大量架构修改，时间信息未得到充分探索

Method: 提出Plug-in Visual Injection (PVI)模块，这是一个轻量级、编码器无关的模块，通过零初始化残差路径连接到预训练动作专家，仅需单阶段微调即可保持预训练行为

Result: PVI在基础策略和一系列竞争性替代注入策略上获得一致增益，控制研究表明时间视频特征(V-JEPA2)优于静态图像特征(DINOv2)，在多阶段任务上增益最大；真实机器人长视野双手机械臂布料折叠实验验证了PVI的实用性

Conclusion: PVI提供了一种有效的方法来增强VLA架构的视觉表示能力，特别是时间特征在需要状态跟踪和协调的任务中表现优异，具有实际应用价值
...
TL;DR: 提出了一个包含10种基本手术动作、跨越6个外科专业、超过11,000个视频片段的最大规模BSA数据集，并基于此开发了能够跨专业识别基本手术动作的基础模型，展示了在技能评估和手术规划等下游应用中的潜力。
...
Method: 1. 创建了包含10种基本手术动作、跨越6个外科专业、超过11,000个视频片段的最大规模BSA数据集；2. 基于该数据集开发了通用基础模型，用于基本手术动作的识别；3. 在来自不同手术类型和身体部位的数据集上验证了模型的跨专业性能；4. 展示了基础模型的下游应用：使用领域特定知识进行前列腺切除术的技能评估，以及使用大视觉语言模型进行胆囊切除术和肾切除术的动作规划。

Result: 1. 模型在不同手术类型和身体部位的数据集上表现出稳健的跨专业性能；2. 多国外科医生评估表明，语言模型生成的动作规划解释文本具有临床相关性；3. 证明了基本手术动作可以在不同场景中被稳健识别，准确的BSA理解模型能够促进复杂应用并加速实现外科超级智能。

Conclusion: 基本手术动作可以在不同场景中被稳健识别，准确的BSA理解模型能够促进复杂外科应用并加速实现外科超级智能。该研究为外科人工智能的发展提供了重要的数据集和基础模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感定位方法主要局限于感知级匹配和单实体建模，缺乏显式推理和实体间关系建模。随着推理语言模型和可验证奖励强化学习的进步，需要将推理范式扩展到遥感视觉定位任务中。

Method: 提出实体感知推理(EAR)框架，基于视觉语言基础模型生成结构化推理轨迹和主客体定位输出。采用监督微调进行冷启动初始化，然后通过实体感知奖励驱动的组相对策略优化(GRPO)进一步优化。

Result: 在ME-RSRG数据集上的大量实验证明了多实体推理的挑战性，并验证了所提出的EAR框架的有效性。

Conclusion: 该工作通过引入ME-RSRG基准数据集和EAR框架，成功将遥感定位重新定义为多实体推理任务，推动了遥感视觉定位从感知级匹配向显式推理的范式转变。
...
TL;DR: 针对3D高斯泼溅技术的资源消耗攻击防御方法，通过频域分析抑制高斯过度增长


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术虽然渲染质量高，但其高斯表示暴露了新的攻击面——资源消耗攻击。攻击者通过污染训练图像，诱导高斯过度增长导致资源耗尽。现有的空间域防御方法（平滑、阈值、剪枝）只处理可见结构，忽略了隐蔽扰动如何扭曲训练数据的频谱行为。

Method: 提出频谱防御方法，包含两个部分：1）3D频率滤波器，选择性剪枝表现出异常高频的高斯；2）2D频谱正则化，在渲染上区分自然各向同性频率，同时惩罚各向异性的角能量以约束噪声模式。

Result: 实验表明该防御方法能构建鲁棒、准确且安全的3DGS系统：在攻击下抑制过度增长达5.92倍，减少内存使用达3.66倍，提升速度达4.34倍。

Conclusion: 通过频域分析防御3D高斯泼溅的资源消耗攻击是有效的，能够显著抑制高斯过度增长，提升系统性能和安全性。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中的对抗鲁棒性通常以牺牲干净数据准确性为代价，存在长期存在的挑战性权衡。本文旨在探究VLMs鲁棒性的本质机制，以解决这一根本问题。

Method: 通过分析对抗微调模型，发现鲁棒性主要集中于浅层，由低频谱偏置和输入不敏感的注意力模式驱动。基于此提出R-Adapt框架，冻结所有预训练权重，仅在初始层引入基于洞察的最小化适配，支持训练免费、模型引导和数据驱动三种范式。

Result: 在18个数据集和多样化任务上的广泛评估显示，R-Adapt在各种攻击下实现了最先进的性能。该框架能高效泛化到大型视觉语言模型（如LLaVA和Qwen-VL）以增强其鲁棒性。

Conclusion: 对抗鲁棒性在VLMs中并非均匀分布，而是主要集中于浅层。R-Adapt通过浅层最小化适配实现了对抗鲁棒性与干净准确性的卓越平衡，为增强标准模型鲁棒性提供了灵活途径。
...
TL;DR: OARS框架通过COMPASS奖励模型在线对齐生成式超分辨率模型，平衡保真度与感知质量，在Real-ISR基准上实现SOTA性能
...
Method: 提出OARS在线对齐框架，基于COMPASS奖励模型评估LR到SR转换，联合建模保真度保持和感知增益。训练COMPASS使用COMPASS-20K数据集和三级感知标注流程。OARS通过浅层LoRA优化进行渐进式在线对齐：从冷启动流匹配到全参考再到无参考RL。

Result: 广泛实验和用户研究表明，在保持保真度的同时实现一致的感知质量提升，在Real-ISR基准测试中达到最先进的性能。

Conclusion: OARS框架通过基于MLLM的COMPASS奖励模型和渐进式在线对齐策略，有效解决了生成式超分辨率模型与人类视觉偏好的对齐问题，平衡了感知质量和保真度。
...
Method: 提出coDrawAgents框架，包含四个专门化智能体：1) 解释器：自适应选择直接文本到图像路径或布局感知的多智能体流程，解析提示为属性丰富的对象描述符并按语义显著性排序；2) 规划器：采用分治策略，基于画布视觉上下文逐步规划相同语义优先级对象的布局；3) 检查器：通过验证空间一致性和属性对齐引入显式纠错机制；4) 绘制器：逐步合成图像，将新规划对象融入画布为后续迭代提供更丰富上下文。

Result: 在GenEval和DPG-Bench基准测试上的广泛实验表明，coDrawAgents相比现有方法显著改善了文本-图像对齐、空间准确性和属性绑定。

Conclusion: coDrawAgents通过多智能体协作框架有效解决了文本到图像生成中的三个关键挑战：降低布局复杂性、基于视觉上下文进行规划以及实现显式错误纠正，为组合性图像生成提供了有前景的解决方案。
...
TL;DR: 提出基于视觉的分层分类框架，用于自动监测磨料翼轮磨损状态，通过三级分类实现高精度磨损识别


<details>
  <summary>Details</summary>
Motivation: 磨料翼轮因其灵活性常用于复杂自由曲面的精加工，但这种灵活性导致复杂的磨损模式（如凹凸翼片轮廓或翼片撕裂），这些磨损会影响磨削结果，需要自动化监测方法

Method: 提出分层分类框架：1）状态检测（新vs磨损）；2）磨损类型识别（矩形、凹形、凸形）和翼片撕裂检测；3）严重程度评估（部分vs完全变形）。使用自定义真实翼轮图像数据集，采用EfficientNetV2架构的迁移学习方法

Result: 分类准确率从93.8%（翼片撕裂）到99.3%（凹形严重程度），表现出高鲁棒性。使用Grad-CAM验证模型学习物理相关特征并分析错误分类

Conclusion: 提出的分层方法为自适应过程控制和自动化翼轮磨削中的磨损考虑提供了基础，能够有效监测复杂磨损模式
...
TL;DR: CompoSIA是一个解耦交通要素的驾驶视频生成器，通过噪声级身份注入和分层双分支动作控制，实现对场景结构、对象身份和自车动作的独立操控，能合成传统生成器无法产生的危险组合场景。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶面临"长尾"安全关键边缘场景的挑战，这些场景通常由常见交通要素的异常组合产生。现有可控生成模型提供不完整或纠缠的指导，无法独立操控场景结构、对象身份和自车动作。

Method: 提出CompoSIA组合式驾驶视频模拟器，包含噪声级身份注入技术实现姿态无关的身份生成（仅需单张参考图像），以及分层双分支动作控制机制提升动作可控性。

Result: 在身份编辑方面FVD提升17%，动作控制的旋转和翻译误差分别降低30%和47%。下游压力测试显示规划器故障显著增加，各编辑模式下3秒碰撞率平均增加173%。

Conclusion: CompoSIA通过解耦交通要素控制，能够合成传统纠缠生成器无法产生的危险组合场景，为自动驾驶系统压力测试提供了有效的场景生成工具。
...
TL;DR: TRACE是一个基于扩散模型的结构感知字符编码框架，利用字符结构的稳定性实现抗噪声干扰的数据嵌入，在跨媒体传输中显著提升PSNR和提取准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖边缘特征或预定义码本，缺乏对字符结构稳定性的利用。字符结构具有抗噪声干扰的固有优势，因为它们在各种字符中具有稳定且统一的表示。
...
Result: TRACE在PSNR上比现有方法提升超过5dB，提取准确率提高5%，在跨媒体传输后表现优异。框架具有广泛通用性，适用于多种语言和字体。

Conclusion: TRACE通过利用字符结构稳定性，实现了抗噪声干扰的高性能数据嵌入，特别适合实际文档安全应用，在多语言和字体环境下表现出色。
...
TL;DR: 提出基于视频的癫痫发作预测新任务，利用短时发作前视频预测未来5秒内是否发作，通过跨物种迁移学习解决人类数据稀缺问题，达到70%以上预测准确率。
...
Method: 提出视频基癫痫发作预测任务，使用3-10秒发作前视频片段预测未来5秒内发作；采用跨物种迁移学习框架，利用大规模啮齿动物视频数据进行辅助预训练，捕捉跨物种通用的发作相关行为动态。

Result: 在严格视频仅设置下，方法达到超过70%的预测准确率，优于现有基线方法。

Conclusion: 跨物种学习为构建非侵入性、可扩展的癫痫早期预警系统提供了潜力，视频基方法有望克服传统神经信号方法的设备限制。
...
TL;DR: SG-NLF是一个无需姿态估计的LiDAR NeRF框架，通过整合光谱信息和几何一致性来解决LiDAR数据稀疏、无纹理特性带来的几何空洞和表面不连续问题。


<details>
  <summary>Details</summary>
Motivation: 传统NeRF方法在LiDAR新视角合成中严重依赖准确的相机姿态，而LiDAR数据的稀疏性和无纹理特性导致几何空洞和表面不连续问题，需要新的解决方案。
...
Conclusion: SG-NLF为LiDAR视角合成提供了新的视角，通过整合光谱信息和几何一致性有效解决了姿态依赖和LiDAR数据特性的挑战。
...
TL;DR: 提出VIRD方法解决跨视角姿态估计问题，通过双轴变换构建视角不变表示，显著降低位置和方向误差


<details>
  <summary>Details</summary>
Motivation: GNSS定位在遮挡和多径效应下性能下降，跨视角姿态估计作为替代方案存在显著视角差异挑战，现有方法难以有效解决地面与卫星视图之间的空间对应关系

Method: 提出VIRD方法：1) 对卫星视图应用极坐标变换建立水平对应关系；2) 使用上下文增强的位置注意力处理地面和极坐标变换后的卫星特征，解决垂直错位；3) 引入视图重建损失进一步强化视角不变性

Result: 在KITTI和VIGOR数据集上优于现有方法：KITTI上位置和方向中值误差分别降低50.7%和76.5%；VIGOR上分别降低18.0%和46.8%

Conclusion: VIRD通过双轴变换和视图重建损失有效缓解地面与卫星视图之间的视角差异，显著提升跨视角姿态估计精度，为自动驾驶和机器人定位提供可靠解决方案
...
<details>
  <summary>Details</summary>
Motivation: 随着AIGC的快速发展，图像操纵变得日益容易，给图像伪造检测与定位带来重大挑战。现有视觉语言模型的先验知识由于偏向语义合理性而非真实性，对检测定位性能帮助有限甚至产生负面影响。

Method: 提出IFDL-VLM新流程，利用位置掩码显式编码伪造概念作为视觉语言模型的额外先验知识，简化训练优化过程，增强检测定位结果的可解释性。

Result: 在9个流行基准测试上进行实验，评估域内和跨数据集泛化性能。实验结果显示，在检测、定位和可解释性方面均取得新的最先进性能。

Conclusion: IFDL-VLM方法通过有效利用视觉语言模型和位置掩码先验，显著提升了图像伪造检测与定位的性能和可解释性，为解决AIGC时代的图像伪造问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有功能映射方法在处理非等距变形和拓扑噪声时存在局限性，几何描述符存在歧义性，谱基投影到密集点对应关系时存在空间不一致问题
...
Method: 提出CleanSight防御方法：1）基于选定跨模态融合层中的视觉-文本注意力相对比率检测中毒输入；2）通过选择性剪枝可疑的高注意力视觉标记来净化输入，中和后门激活。该方法完全在测试时运行，无需训练。

Result: 大量实验表明，CleanSight在多种数据集和后门攻击类型上显著优于现有的基于像素的净化防御方法，同时在干净样本和中毒样本上都能保持模型的实用性。

Conclusion: 通过揭示后门攻击的注意力窃取机制，提出的CleanSight方法提供了一种高效、训练免费的防御方案，能够有效检测和中和后门攻击，同时保持模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLMs）可能从训练数据中继承社会偏见并传播到下游应用中。现有去偏方法大多缺乏理论保证，无法确保模型效用的保持。

Method: 提出一种闭式解的去偏方法，在跨模态空间中实现帕累托最优公平性。该方法无需训练、无需标注数据，可同时处理视觉和文本模态的偏见。

Result: 在多个公平性指标和数据集上的实验表明，该方法在零样本图像分类、文本到图像检索、文本到图像生成等下游任务中优于现有方法，同时保持任务性能。

Conclusion: 该方法提供了一种理论保证的去偏解决方案，能够有效处理群体公平性和交叉公平性，为视觉语言模型的公平性研究提供了新思路。
...
TL;DR: Multimodal OCR (MOCR) 是一种将文本和图形联合解析为统一文本表示的文档解析范式，其实现方法dots.mocr将视觉元素视为一等解析目标，支持端到端训练并利用现有文档中的多模态监督。


<details>
  <summary>Details</summary>
Motivation: 传统OCR系统主要关注文本识别，而将图形区域作为裁剪像素处理，无法保留文本和视觉元素之间的语义关系。需要一种能够同时解析文本和图形并保持其语义关联的统一方法。

Method: 提出dots.mocr方法，将图表、图示、表格和图标等视觉元素作为一等解析目标。构建了从PDF、渲染网页和原生SVG资产的综合数据引擎，通过分阶段预训练和监督微调训练了一个紧凑的3B参数模型。

Result: 在文档解析基准测试中，dots.mocr在OCR Arena Elo排行榜上仅次于Gemini 3 Pro，超越现有开源文档解析系统，在olmOCR Bench上达到83.9的新SOTA。在结构化图形解析方面，在图像到SVG基准测试中重建质量高于Gemini 3 Pro，在图表、UI布局、科学图表和化学图表上表现强劲。

Conclusion: MOCR范式为构建大规模图像到代码语料库进行多模态预训练提供了可扩展的路径，能够将先前被丢弃的图形转换为可重用的代码级监督，解锁现有文档中嵌入的多模态监督。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在具身空间推理方面的评估存在局限：评估范式有限、覆盖范围不足，阻碍了模型的快速迭代开发。需要更贴近实际机器人任务的评估方法来推动模型发展。

Method: 1. 构建模拟世界来物理化地锚定视觉语言模型；2. 将机器人任务分解为定位和执行两个生成性问题，而非传统的判别式评估；3. 在指令和环境两个层面系统设计，确保覆盖广泛的空间推理场景。

Result: 使用ESPIRE对前沿视觉语言模型进行诊断，提供了对其空间推理行为的深入分析，揭示了模型在具身空间推理方面的具体表现和局限性。

Conclusion: ESPIRE基准通过模拟环境和任务分解方法，为视觉语言模型的具身空间推理能力提供了更贴近实际部署的评估框架，有助于推动模型在机器人领域的应用发展。
...
TL;DR: 论文提出Topo-R1框架，通过两阶段训练（监督微调+强化学习）赋予视觉语言模型拓扑感知能力，用于无标注的拓扑异常检测，在多个领域基准测试中表现优异。
...
Method: 1) 开发自动化数据生成流程，合成多样化的拓扑异常并构建大规模多领域基准；2) 提出Topo-R1框架，通过两阶段训练：监督微调 + 基于Group Relative Policy Optimization的强化学习；3) 设计拓扑感知复合奖励函数，包括类型感知匈牙利匹配、空间定位评分和中心线Dice奖励。

Result: Topo-R1在无标注拓扑质量评估方面建立了新范式，在所有评估协议中一致优于通用视觉语言模型和监督基线方法。

Conclusion: 该研究为拓扑异常检测提供了有效的无监督解决方案，通过赋予视觉语言模型拓扑感知能力，实现了跨领域的拓扑质量评估，为血管、神经纤维等管状结构的分割质量监控提供了新方法。
...
TL;DR: 该论文提出了VTON-IQA框架，用于虚拟试穿图像的无参考质量评估，无需真实图像作为参考，并构建了大规模人工标注数据集VTON-QBench。


<details>
  <summary>Details</summary>
Motivation: 虚拟试穿系统在时尚电商等实际应用中日益重要，但缺乏可靠的评估方法。真实场景中通常无法获得同一人穿着目标服装的真实图像，使基于参考的评估不切实际，而现有分布级指标无法反映单个生成图像的感知质量。

Method: 提出VTON-IQA框架，构建VTON-QBench大规模人工标注数据集（包含62,688张试穿图像和431,800个质量标注），并引入交错交叉注意力模块，在标准transformer块的自注意力和MLP之间插入交叉注意力层，以显式建模服装保真度和人物细节保留之间的交互。

Result: VTON-IQA实现了可靠的人类对齐图像级质量预测，并对14个代表性VTON模型进行了全面的基准评估。

Conclusion: 该研究为解决虚拟试穿系统质量评估的挑战提供了有效框架，通过无参考质量评估方法和大规模数据集，为VTON模型的性能评估提供了可靠工具。
...
Method: 提出两种互补方法：1) 区域感知提示增强(RAPTA)：使用对象检测器找到显著区域并转换为语义基础的提示变体，在训练中随机采样以增加多样性；2) 注意力驱动的多模态复制检测(ADMCD)：聚合局部补丁、全局语义和纹理线索，使用轻量级transformer生成融合表示，应用简单阈值决策规则检测复制。

Result: 实验表明，RAPTA在保持高合成质量的同时减少了过拟合，ADMCD能够可靠地检测复制，性能优于单模态指标。

Conclusion: RAPTA和ADMCD为文本到图像扩散模型提供了有效的解决方案，既能减少训练图像复制风险，又能保持生成质量，同时提供可靠的复制检测机制。
...
TL;DR: 该研究开发了一个基于风洞实验数据的屋顶风速分布学习框架，比较了克里金插值和三种深度学习模型（UNet、ViTAE、CWGAN），评估了不同训练策略和传感器配置对屋顶风场重建性能的影响。
...
Method: 使用PIV获取的风洞实验数据，开发学习观察框架，比较克里金插值与三种深度学习模型（UNet、ViTAE、CWGAN），评估两种训练策略（单风向训练SDT和混合风向训练MDT），传感器密度5-30个，测试传感器位置扰动（±1网格）下的鲁棒性，并通过POD-QR分解优化传感器布局。

Result: 深度学习模型能有效从稀疏传感器数据重建屋顶风场，相比克里金插值，SSIM提升达32.7%，FAC2提升24.2%，NMSE降低27.8%。混合风向训练进一步改善性能，SSIM提升达173.7%，FAC2提升16.7%，MG提升98.3%。QR优化在传感器扰动下提升鲁棒性达27.8%，但存在指标依赖的权衡。

Conclusion: 深度学习方法能有效重建屋顶风场，传感器配置、优化和训练策略需要联合考虑以确保可靠部署。基于实验数据的训练为不同场景下的方法选择和传感器布局提供了实用指导。
...
TL;DR: V-Bridge框架利用预训练视频生成模型作为通用视觉学习器，通过极少量多任务训练样本（仅1000个）激活其隐含的图像修复能力，实现多种图像修复任务的竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 大规模视频生成模型内部蕴含丰富的视觉世界结构、语义和动态先验知识，但这些模型作为通用视觉学习器的潜力尚未被充分挖掘。传统图像修复方法通常需要大量数据和专门架构，而视频模型可能隐含强大的修复先验。

Method: 提出V-Bridge框架，将图像修复重新解释为渐进式生成过程而非静态回归问题。利用视频模型模拟从退化输入到高保真输出的逐步细化过程，通过极少量多任务训练样本（仅1000个）激活预训练视频模型的修复能力。

Result: 仅用现有修复方法不到2%的训练数据，预训练视频模型就能实现竞争性的图像修复性能，单个模型可处理多种修复任务，性能可与专门设计的架构相媲美。

Conclusion: 视频生成模型隐含学习到强大且可迁移的修复先验，仅需极少数据即可激活。这挑战了生成建模与低级视觉之间的传统界限，为视觉任务的基础模型设计开辟了新范式。
...
TL;DR: VAEX-BENCH：首个针对视频抽象时空推理的基准测试，通过合成自我中心视频数据集评估多模态大语言模型在整合分散线索、推断隐含结构方面的能力


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准主要关注提取式推理（答案可直接从时空事件中提取），而缺乏对抽象时空推理能力的评估。抽象时空推理需要整合时间观察、结合分散线索、推断隐含空间和上下文结构，这对具身智能体至关重要。

Method: 1. 形式化视频抽象时空推理概念；2. 引入结构化评估分类法，系统化定位其核心维度；3. 构建可控、场景驱动的合成自我中心视频数据集，涵盖物体、房间和楼层平面图三个层级；4. 基于此框架提出VAEX-BENCH基准，包含五个抽象推理任务及其提取式对应任务。

Result: 通过广泛实验比较了最先进的多模态大语言模型在提取式和抽象式设置下的性能，暴露了它们在抽象任务上的局限性，并对潜在瓶颈进行了细粒度分析。

Conclusion: VAEX-BENCH填补了视频抽象时空推理评估的空白，揭示了当前多模态大语言模型在抽象推理能力上的不足，为未来研究提供了重要基准和分析框架。数据集即将发布。
...
Method: 1) 提出可制造性度量的分类体系，从配置依赖性和测量类型两个维度进行划分；2) 创建BenDFM数据集，包含20,000个钣金弯曲零件，使用工艺感知的弯曲模拟生成，提供折叠和展开几何形状以及多种可制造性标签。

Result: 1) 在BenDFM数据集上测试了两种先进的3D学习架构，发现基于图的表示方法（捕捉零件表面关系）获得更好的准确性；2) 预测依赖特定制造设置的度量仍然更具挑战性。

Conclusion: BenDFM数据集为研究基于学习的可制造性评估提供了系统平台，有助于解决先前未探索的学习挑战，特别是对于依赖特定制造配置的复杂工艺如钣金弯曲。
...
<details>
  <summary>Details</summary>
Motivation: 挑战当前主流的离散网格深度学习范式，解决医学影像处理中固定像素/体素网格的局限性，实现分辨率无关的鲁棒性处理。

Method: 将离散医学信号嵌入共享的隐式神经表示，学习在潜在调制之间映射的神经算子，实现连续函数空间之间的变换。

Result: 在分割、形状补全、图像到图像翻译和图像合成等2D/3D任务上取得竞争性性能，在未见过的离散化情况下表现出强鲁棒性，并经验性地满足神经算子的关键理论特性。

Conclusion: NOIR框架成功地将医学影像任务重构为连续函数空间的算子学习，为医学影像分析提供了分辨率无关的鲁棒解决方案。
...
TL;DR: FDeID-Toolbox：一个用于面部去身份化研究的综合性工具箱，解决该领域实现碎片化、评估不一致的问题，提供标准化数据加载、统一方法实现、灵活推理管道和系统评估协议。


<details>
  <summary>Details</summary>
Motivation: 面部去身份化领域存在实现碎片化、评估协议不一致、研究结果不可比的问题，这源于任务本身的复杂性：需要平衡隐私保护、效用保持和视觉质量三个维度，同时涉及多个下游应用。

Method: 开发了FDeID-Toolbox，采用模块化架构，包含四个核心组件：1) 主流基准数据集的标准化数据加载器；2) 从经典方法到最先进生成模型的统一方法实现；3) 灵活的推理管道；4) 涵盖隐私、效用和质量指标的系统评估协议。

Result: 实验表明，FDeID-Toolbox能够在一致条件下对不同的面部去身份化方法进行公平且可重复的比较。

Conclusion: FDeID-Toolbox解决了面部去身份化研究中的碎片化和不可重复性问题，为社区提供了一个标准化、可扩展的研究平台，促进了该领域的公平比较和可重复研究。
...
<details>
  <summary>Details</summary>
Motivation: 现代人机协作应用中，多感知模块并行处理虽然提升感知质量，但在流式感知场景中会累积延迟，导致系统性能下降。现有感知管道面临信息冗余和计算资源分配不优的挑战。

Method: 受Relevance概念和人机协作事件信息稀疏性启发，提出轻量级感知调度框架，利用先前帧输出基于场景上下文实时估计和调度必要感知模块。

Result: 相比传统并行感知管道，计算延迟降低27.52%，MMPose激活召回率提升72.73%，关键帧准确率高达98%，验证了在不显著影响精度的情况下提升实时感知效率的能力。

Conclusion: 该框架展示了作为人机协作中多模态流式感知系统的可扩展系统解决方案的潜力，能有效平衡感知精度和计算效率。
...
TL;DR: 该论文提出了World Scene Graph Generation (WSGG)任务，旨在构建包含场景中所有交互对象（包括可见和不可见对象）的世界场景图，并引入了ActionGenome4D数据集和三种互补的方法来解决不可见对象推理问题。


<details>
  <summary>Details</summary>
Motivation: 现有时空场景图方法存在局限性：它们仅关注当前可见对象，在遮挡时丢弃实体，且工作在2D空间。这限制了场景理解的完整性和持续性，需要向世界中心、时间持久且可解释的场景推理发展。

Method: 1. 引入ActionGenome4D数据集，通过前向3D重建将Action Genome视频升级为4D场景，包含所有动作相关对象的世界坐标系定向边界框和密集关系标注（包括暂时不可见对象）。2. 提出三种互补方法：PWG（持久世界图）通过零阶特征缓冲区实现对象持久性；MWAE（掩码世界自编码器）将不可见对象推理重构为带跨视图关联检索的掩码补全；4DST（4D场景变换器）用可微分逐对象时间注意力替代静态缓冲区，并增强3D运动和相机姿态特征。3. 设计基于图RAG的视觉语言模型方法，建立无定位关系预测基线。

Result: 论文提出了WSGG任务框架，创建了包含4D场景信息的数据集，开发了三种针对不可见对象推理的方法，并建立了基于视觉语言模型的基线系统，为世界中心、时间持久的场景理解奠定了基础。

Conclusion: WSGG将视频场景理解推向世界中心、时间持久且可解释的场景推理，通过处理可见和不可见对象，实现了更完整的场景建模，为高级视频理解任务提供了新范式。
...
TL;DR: STEVO-Bench是一个评估视频世界模型能否将状态演化与观测解耦的基准测试，通过控制观测条件（如遮挡、关灯、摄像机移开）来检测模型在自然演化过程中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的演化（如倒水、冰融化）无论是否被观测都会发生，但视频世界模型通过2D帧观测生成"世界"。本研究旨在探究这些生成的"世界"能否在无观测情况下继续演化，即模型能否将状态演化与观测解耦。

Method: 设计了STEVO-Bench基准测试，通过指令控制观测条件：插入遮挡物、关闭灯光、指定摄像机"移开"轨迹。评估视频模型在有和没有摄像机控制的情况下，对多种自然演化过程的处理能力。

Result: 通过STEVO-Bench评估，揭示了当前视频世界模型在将状态演化与观测解耦方面的局限性。分析结果提供了对现有视频世界模型潜在数据和架构偏见的新的见解。

Conclusion: STEVO-Bench提出了一个自动检测和分离视频世界模型在自然状态演化关键方面失败模式的评估协议，为理解当前视频世界模型的局限性提供了新的视角。
...
TL;DR: 本文提出了一种新的知识蒸馏框架，通过训练轻量级探针来提取教师模型的中间表示，而不是使用输出logits作为监督信号，从而在推理任务上获得更干净的蒸馏信号。


<details>
  <summary>Details</summary>
Motivation: 在推理任务中，大型语言模型的输出分布经常不能提供高质量的训练信号。虽然模型的中间表示可能编码了正确答案，但通过词汇投影时这些信息会丢失或扭曲，导致输出噪声大且脆弱。

Method: 提出名为\method{}的蒸馏框架：在冻结的教师模型隐藏状态上训练轻量级探针，然后使用探针的预测（而不是输出logits）作为学生模型的监督信号。

Result: 在四个推理基准测试（AQuA-RAT、ARC Easy/Challenge和MMLU）上取得了一致的改进，在有限数据情况下效果最为显著。探针训练的中间表示提供了比教师自身输出更干净的标签。

Conclusion: 通过利用内部表示，\method{}使从业者能够从大型教师模型中提取更多价值，无需额外的训练数据或架构复杂性，且计算开销小，架构无关。
...
Result: 随着更新次数增加，检索偏差加剧：最早状态准确率保持高位，而最新状态准确率大幅下降；错误样本上的注意力、隐藏状态相似性和输出logits信号变得平坦且区分度弱；认知启发式干预策略仅带来有限改进，无法消除偏差。

Conclusion: LLMs在长上下文中跟踪和遵循知识更新存在持续性挑战，模型难以稳定识别最新版本，这揭示了当前LLM架构在处理动态知识更新方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有激活稀疏化方法通常在整个投影层应用均匀稀疏度，忽略了Transformer权重矩阵的异质性统计特性，导致性能下降加剧。需要一种能够考虑不同投影层统计特性的稀疏度分配方法。

Method: 提出ActTail方法：1）基于重尾自正则化理论，通过经验谱密度计算每个投影层的重尾指数；2）将重尾指数作为定量指标，为每个投影层分配特定的稀疏度预算；3）提供理论分析，建立激活稀疏度与重尾指数之间的明确关系，为稀疏度分配提供原则性指导。

Result: 在LLaMA和Mistral模型上的实验表明，相比均匀稀疏度分配，ActTail在高稀疏度下显著改善了困惑度和下游任务性能。在80%稀疏度下，LLaMA-2-7B困惑度降低21.8%，LLaMA-2-13B降低40.1%，Mistral-7B降低9.4%。

Conclusion: ActTail通过考虑投影层异质性统计特性，基于重尾指数进行投影特定的稀疏度分配，为激活稀疏化提供了理论指导，显著提升了高稀疏度下的大语言模型性能。
...
TL;DR: 该论文提出了一种从多轮用户对话中学习的方法，通过自我蒸馏利用用户后续消息来改进语言模型，无需显式反馈即可实现对齐、个性化和持续适应。


<details>
  <summary>Details</summary>
Motivation: 语言模型产生的大量多轮用户交互数据通常被丢弃，但这些数据包含有价值的信息：用户的后续消息可能表明之前的回答不正确、未遵循指令或不符合用户偏好。现有方法未能有效利用这些自然产生的交互数据。

Method: 提出基于自我蒸馏的原理性可扩展方法：将用户后续消息作为条件输入模型，比较由此产生的token分布与原始策略，获得反映模型在事后应如何改变行为的目标分布，然后将这种"事后分布"蒸馏回当前策略中。

Result: 在WildChat真实用户对话数据上训练，能显著改进语言模型在标准对齐和指令遵循基准上的表现，且不损害其他能力。该方法还支持个性化，使模型能通过交互持续适应个体用户而无需显式反馈。

Conclusion: 部署过程中自然产生的原始用户交互数据能够支持语言模型的对齐、个性化和持续适应，为从用户交互中学习提供了有效且可扩展的途径。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型对训练数据的大规模记忆能力引发了安全、隐私和知识产权问题，但现有遗忘学习方法主要关注平面句子级数据，忽视了自然结构化数据中的关系型、多跳推理知识，需要专门针对知识图谱事实的遗忘学习基准和方法。

Method: 提出了Graph Oblivion and Node Erasure (GONE)基准，用于评估知识图谱事实的遗忘学习效果，能分离三种遗忘效应：直接事实移除、推理泄漏和灾难性遗忘。同时设计了Neighborhood-Expanded Distribution Shaping (NEDS)框架，利用图连接性识别锚点相关邻居，在遗忘事实与其语义邻域之间建立精确决策边界。

Result: 在LLaMA-3-8B和Mistral-7B模型上评估多个知识编辑和遗忘学习方法，NEDS在GONE和其他基准上表现出色，遗忘效能达到1.000，局部性达到0.839。
...
TL;DR: 语言模型尽管经过广泛的安全训练，但仍易受提示注入攻击。研究发现这是由于角色混淆造成的——模型根据文本的写作方式而非来源推断角色，导致不可信文本模仿角色时继承了该角色的权限。


<details>
  <summary>Details</summary>
Motivation: 语言模型对提示注入攻击的脆弱性是一个重要安全问题。作者旨在探究为什么即使经过安全训练，模型仍然容易受到这类攻击，并寻找根本原因。

Method: 设计了新颖的角色探针来捕捉模型内部如何识别"谁在说话"。通过向用户提示和工具输出中注入伪造的推理过程来测试这一洞察，并在多个开源和闭源模型上进行实验。

Result: 攻击在StrongREJECT上平均成功率为60%，在智能体数据窃取上为61%，而基线接近零。内部角色混淆程度在生成开始前就能强烈预测攻击成功率。

Conclusion: 研究揭示了安全在接口层面定义但权限在潜在空间中分配的根本差距。提出了一个统一的机制框架，表明多样化的提示注入攻击都利用了相同的底层角色混淆机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本-语音联合语言建模（TASTE）存在两个主要限制：依赖外部ASR系统导致无法流式处理，以及使用非因果解码器限制了实时应用。需要开发一种可流式化的解决方案来支持实时语音交互。

Method: 提出TASTE-S方法：1）在编码器中集成基于CTC的ASR模块，实现即时双模态编码；2）重新设计单元解码器，支持实时解码；3）通过联合训练优化系统性能。

Result: TASTE-S在性能上与原始TASTE相当，同时显著降低了延迟。进一步研究表明，TASTE-S对转录保持鲁棒性，并支持长格式编码和解码。

Conclusion: TASTE-S成功解决了原始TASTE方法的流式处理限制，通过集成CTC-ASR和重新设计解码器，实现了可流式化的文本-语音联合语言建模，为实时语音交互应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 思维链常被视为窥探LLM决策的窗口，但近期研究认为它可能只是事后合理化。这引发了一个关键的对齐问题：推理轨迹是否独立于最终答案而因果性地影响模型泛化？

Method: 设计控制实验，保持最终有害答案不变但改变推理路径。构建包含恶意推理、误导性推理和顺从性推理的数据集。在多种范式下训练模型（0.6B-14B参数），包括问题-思考-答案、问题-思考和仅思考模式，并在思考和不思考模式下评估。

Result: 1) 思维链训练比标准微调更能放大有害泛化；2) 不同推理类型尽管最终答案相同，但会引发与其语义一致的不同行为模式；3) 仅训练推理而无答案监督就足以改变行为，证明推理携带独立信号；4) 这些效应即使在无推理生成答案时也持续存在，表明深度内化。

Conclusion: 推理内容具有因果效力，挑战了仅监督输出的对齐策略。模型会内化推理中的语义模式，即使最终答案相同，不同的推理路径也会导致不同的泛化行为。
...
<details>
  <summary>Details</summary>
Motivation: 否定对现代语言模型构成持续挑战，常导致意义反转或事实错误。研究旨在通过因果分析理解GPT-2内部如何处理这种语言转换。

Method: 1. 使用自建的12,000对肯定-否定句数据集；2. 定义否定效应分数(NES)量化模型区分能力；3. 进行激活修补和注意力头消融两种干预实验；4. 在层和头级别分析隐藏表示。

Result: 1. 否定处理能力高度集中在第4-6层的有限注意力头中；2. 消融这些头会直接破坏模型的否定敏感性；3. 在xNot360基准上模式一致但幅度较小；4. 这些头携带肯定信号而非恢复基线行为。

Conclusion: GPT-2的否定处理不是广泛分布的能力，而是由特定中间层的少数注意力头专门负责，这些发现有助于理解语言模型内部工作机制。
...
TL;DR: ShatterMed-QA是一个包含10,558个多跳临床问题的双语基准测试，旨在评估深度诊断推理能力。通过k-Shattering算法构建拓扑正则化医学知识图谱，剪除通用枢纽节点以切断逻辑捷径，并采用隐式桥梁实体掩码和拓扑驱动的困难负采样来合成评估案例。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在标准医学基准测试中表现优异，但在真实临床环境所需的复杂多跳诊断推理方面严重不足。主要障碍是"捷径学习"，模型利用知识图谱中高度连接的通用枢纽节点（如"炎症"）来绕过真实的微病理级联过程。

Method: 1. 使用新颖的k-Shattering算法构建拓扑正则化医学知识图谱，物理剪除通用枢纽节点以显式切断逻辑捷径；2. 通过隐式桥梁实体掩码和拓扑驱动的困难负采样合成评估案例，迫使模型在不依赖表面消除的情况下导航生物学上合理的干扰项。

Result: 对21个LLM的综合评估显示，在多跳任务上性能大幅下降，特别是领域特定模型。通过检索增强生成恢复掩码证据后，几乎所有模型都实现了性能恢复，验证了ShatterMed-QA的结构保真度，并证明了其在诊断当前医学AI基本推理缺陷方面的有效性。

Conclusion: ShatterMed-QA基准测试成功揭示了当前医学AI在多跳诊断推理方面的根本缺陷，通过切断逻辑捷径和强制深度推理，为评估和改进临床AI的推理能力提供了有效工具。恢复掩码证据后的性能恢复证明了该框架的结构有效性。
...
TL;DR: 研究显示主流LLMs在生成写作反馈时会根据学生性别、种族、学习需求等属性产生系统性偏见，表现出"积极反馈偏见"和"反馈保留偏见"，即使面对相同作文内容也会调整反馈方式和评判标准。


<details>
  <summary>Details</summary>
Motivation: LLM驱动的工具承诺大规模自动化个性化反馈，但LLMs并非语言中立：它们偏向标准学术英语并复制社会刻板印象，引发了对"个性化"如何影响学生反馈质量的担忧。

Method: 使用PERSUADE数据集中600篇八年级议论文，在提示条件中嵌入性别、种族/民族、学习需求、成就水平和动机等学生属性，通过四个主流LLMs生成反馈，采用Marked Words框架分析词汇变化。

Result: 结果显示基于预设学生属性存在系统性、刻板印象一致的反馈变化：对种族、语言或残疾标记的学生表现出积极反馈偏见（过度赞扬）和反馈保留偏见（较少实质性批评），模型不仅调整强调内容，还改变写作评判方式和学生称呼方式。

Conclusion: 研究提出"标记教学法"概念，强调自动化反馈工具需要透明度和问责制，以避免强化社会偏见和不公平的教学实践。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本嵌入基准测试主要关注传统段落检索，未能评估模型处理碎片化、上下文相关、时间距离较长的长时程记忆检索任务的能力，而这对记忆增强系统至关重要。

Method: 提出了长时程记忆嵌入基准测试(LMEB)，包含22个数据集和193个零样本检索任务，涵盖4种记忆类型：情景记忆、对话记忆、语义记忆和程序记忆，包含AI生成和人工标注数据。

Result: 评估了15个广泛使用的嵌入模型（参数从数亿到百亿），发现：1）LMEB提供了合理的难度水平；2）更大的模型并不总是表现更好；3）LMEB与MTEB具有正交性，表明传统段落检索性能不能推广到长时程记忆检索。

Conclusion: LMEB填补了记忆嵌入评估的关键空白，提供了标准化、可复现的评估框架，推动文本嵌入在处理长期、上下文相关记忆检索方面的进一步发展。
...
TL;DR: 提出Expert Pyramid Tuning (EPT)方法，将计算机视觉中的多尺度特征金字塔概念引入参数高效微调，通过共享元知识子空间和金字塔投影机制动态选择多尺度特征，在减少训练参数的同时提升多任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于MoE的LoRA变体虽然通过动态路由获得不错效果，但忽视了任务复杂度的层次性。这些方法通常使用统一架构的专家，限制了它们捕捉不同任务所需多样化特征粒度的能力——有些任务需要高级语义抽象，而其他任务需要细粒度句法操作。

Method: 提出Expert Pyramid Tuning (EPT)架构，将任务适应分解为两个阶段：(1) 共享元知识子空间，在低维度编码通用语言模式；(2) 金字塔投影机制，使用可学习的上投影算子在不同尺度重建高维特征。任务感知路由器动态选择这些多尺度特征的最佳组合。

Result: 在多个多任务基准测试上的广泛实验表明，EPT显著优于最先进的MoE-LoRA变体。由于设计的重参数化能力，EPT在实现性能提升的同时减少了训练参数数量。

Conclusion: EPT成功将多尺度特征金字塔概念从计算机视觉引入参数高效微调领域，通过层次化特征表示和动态路由机制，在保持参数效率的同时提升了多任务适应能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗样本检测方法通常依赖攻击先验知识、白盒访问受害者模型或大量查询，严重限制了实际部署。需要一种轻量级、实用且资源消耗低的检测机制

Method: 利用预训练的替换词检测（RTD）判别器定位可疑词元，掩蔽这些词元，然后通过观察干预前后受害者模型预测置信度的变化来检测对抗样本。整个过程无需对抗数据、模型调优或内部模型访问，仅需两次黑盒查询
...
TL;DR: SCILIRE是一个基于人机协作原则设计的科学文献数据集创建系统，通过验证和整理工作流程提高数据提取质量，并利用用户反馈改进LLM推理能力。
...
Method: 围绕人机协作原则设计系统，建立验证和整理数据的工作流程，支持研究人员审查和修正AI输出，并将这种交互作为反馈信号来改进未来的LLM推理。

Result: 通过内在基准测试和跨多个领域的实际案例研究评估，结果显示SCILIRE提高了提取保真度并促进了高效的数据集创建。

Conclusion: SCILIRE系统通过人机协作的工作流程，有效解决了科学文献结构化知识提取的挑战，提高了数据提取质量和效率。
...
TL;DR: 这篇综述系统梳理了大语言模型持续学习的方法论，围绕持续预训练、持续微调和持续对齐三个阶段，分析了传统持续学习方法在大语言模型上的适应性和改进，揭示了当前方法在跨任务知识整合方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 大语言模型需要动态适应不断演化的知识和序列任务，但静态预训练范式存在灾难性遗忘的严重限制。持续学习为解决这一问题提供了关键范式，需要系统梳理针对大语言模型的持续学习方法。

Method: 采用结构化综述方法，围绕三个核心训练阶段（持续预训练、持续微调、持续对齐）组织内容。在传统的排练法、正则化法和架构法分类基础上，进一步根据遗忘缓解机制细分每个类别，并进行严格的比较分析。

Result: 当前方法在特定领域显示出有希望的结果，但在实现跨不同任务和时间尺度的无缝知识整合方面仍存在根本性挑战。分析揭示了LLM持续学习与传统机器学习在规模、参数效率和涌现能力方面的核心区别。

Conclusion: 这篇系统综述为理解大语言模型适应性的当前成就和未来机会提供了结构化框架，有助于推动语言模型终身学习的研究和实践发展。
...
TL;DR: TESS提出了一种新的多模态时间序列预测方法，通过引入时间演化语义空间作为文本和数值数据之间的中间瓶颈，解决了文本描述与预测模型之间的模态鸿沟问题。


<details>
  <summary>Details</summary>
Motivation: 将文本信息融入时间序列预测有助于处理事件驱动的非平稳性，但存在根本的模态鸿沟：文本描述隐含、定性地表达时间影响，而预测模型需要明确、定量的信号。现有方法过度关注冗余标记，难以可靠地将文本语义转化为可用的数值线索。

Method: 提出TESS方法，引入时间演化语义空间作为模态间的中间瓶颈。该空间包含可解释、数值基础的时间原语（均值偏移、波动性、形状和滞后），通过结构化提示由LLM从文本中提取，并通过置信感知门控进行过滤。

Result: 在四个真实世界数据集上的实验表明，相比最先进的单模态和多模态基线方法，预测误差最多减少29%。

Conclusion: TESS通过时间演化语义空间有效桥接了文本和数值模态之间的鸿沟，显著提升了时间序列预测性能，同时保持了可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要从理论或实证角度探讨大语言模型在递归训练合成数据时可能出现的模型崩溃问题，但缺乏对这种现象随时间变化的纵向研究。本研究旨在评估不同ChatGPT模型生成多样化文本的能力变化

Method: 使用文本相似性度量来评估不同ChatGPT模型生成多样化文本输出的能力，通过设置温度参数为1来鼓励多样性生成

Result: 研究发现近期ChatGPT版本在生成多样化文本方面的能力出现可测量的下降，即使明确提示并要求生成多样化文本也无法避免。这种多样性减少可能归因于训练数据中合成数据的增加

Conclusion: ChatGPT模型存在"模型自我收敛"现象，即不同版本生成的文本相似度逐渐增加，导致输出多样性下降。这种现象是由于互联网中LLM生成数据的渗透导致训练数据中合成数据比例增加所致
...
<details>
  <summary>Details</summary>
Motivation: 现有代码强化学习数据集中的验证信号较弱且静态，限制了可验证奖励强化学习在代码生成中的效果。

Method: 提出解决方案条件和对抗性验证框架，基于候选解决方案的执行行为迭代优化测试用例，增加难度、提高区分度并减少冗余。基于此框架构建了EvolveCoder-22k大规模编码强化学习数据集。
...
TL;DR: 研究首次评估大型语言模型对伊斯兰教逊尼派和什叶派两大教派的处理偏差，发现模型回答存在严重的语言和地域不一致性：英文环境下偏向什叶派，印地语环境下转向逊尼派；高级模型还会根据用户地理位置调整答案。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型成为宗教知识的重要来源，需要评估其对不同宗教群体的公平性。本研究首次系统测量LLMs如何处理伊斯兰教两大主要教派（逊尼派和什叶派）之间的差异，确保AI系统不偏袒特定群体。

Method: 开发了SectEval测试集（包含88个问题，支持英文和印地语），评估了15个顶级LLM模型（包括专有和开源模型）。通过语言对比和地理位置模拟，分析模型在不同条件下的回答倾向性。

Result: 发现显著的基于语言的不一致性：英文环境下，DeepSeek-v3和GPT-4o等强大模型偏向什叶派答案；相同问题在印地语环境下则转向逊尼派。高级模型（如Claude-3.5）会根据用户地理位置调整答案（伊朗用户得到什叶派答案，沙特用户得到逊尼派答案），而较小模型（特别是印地语版本）则忽视地理位置，坚持逊尼派观点。

Conclusion: AI并非中立，其宗教"真相"会因用户使用的语言和声称来自的国家而改变。这揭示了LLMs在处理敏感宗教话题时存在系统性偏差，需要开发更公平、一致的评估方法和模型改进策略。
...
TL;DR: SteerRM是一种无需训练的去偏方法，使用稀疏自编码器干预来减少奖励模型对表面风格线索的偏见，同时保持整体性能。


<details>
  <summary>Details</summary>
Motivation: 奖励模型在评估响应时存在偏见，倾向于选择表面格式更好的回答而非语义更优的回答。现有的去偏方法通常需要重新训练或修改架构，而直接抑制激活会因表示纠缠导致性能下降。

Method: SteerRM使用稀疏自编码器进行训练免费的去偏干预：1) 使用对比配对响应隔离风格效应；2) 通过强度-稳定性准则识别偏见相关的SAE特征；3) 在推理时抑制这些特征。

Result: 在RM-Bench的六个奖励模型上，SteerRM将Hard-split准确率平均提高7.3个百分点，同时保持整体性能。在Gemma-based奖励模型和受控非格式偏见上的结果进一步表明该方法在不同架构和偏见类型上的泛化能力。
...
TL;DR: 本文研究了强化学习可验证奖励(RLVR)中选项设计的影响，发现选项数量不匹配会降低性能，而强干扰项能有效防止随机猜测。作者提出了迭代干扰项筛选(IDC)框架来构建高质量干扰项，促进深度推理。


<details>
  <summary>Details</summary>
Motivation: 在RLVR中使用多项选择题(MCQs)作为可验证数据源时，模型可能通过随机猜测或简单排除来"奖励黑客"，绕过深度推理。现有方法通常将MCQs转换为开放式问题，但这会丢弃专家设计的干扰项提供的对比信号。

Method: 系统研究选项设计对RLVR的影响，提出迭代干扰项筛选(IDC)框架，主动构建高质量干扰项以阻断排除捷径并促进深度推理。

Result: 实验表明：1)训练和测试中选项数量不匹配会降低性能；2)强干扰项能有效缓解随机猜测，即使在2项选择题中也能实现有效的RLVR训练；3)IDC方法显著提高了干扰项质量，在RLVR训练中相比原始数据获得显著提升。

Conclusion: 选项设计对RLVR训练至关重要，通过主动构建高质量干扰项可以有效防止奖励黑客行为，促进模型进行深度推理，提高RLVR训练效果。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏专门针对欧洲葡萄牙语（PT-PT）的大语言模型评估排行榜，需要填补这一空白，并提供更全面的性能评估。

Method: 开发了专门的排行榜平台，并创建了新颖的基准测试，包括模型安全防护和与葡萄牙文化对齐等之前未覆盖的评估维度。

Result: 成功建立了欧洲葡萄牙语LLM排行榜，提供了全面的评估框架，包含传统和创新的基准测试项目。

Conclusion: 该工作填补了欧洲葡萄牙语大语言模型评估的空白，为研究者和开发者提供了重要的评估工具，促进了该语言变体LLM的发展。
...
TL;DR: 该研究系统性地将BabyBERTa扩展到英法双语场景，在严格数据匹配条件下比较单语、双语和跨语言设置，发现训练数据对语法和语义任务的影响存在情境依赖性。


<details>
  <summary>Details</summary>
Motivation: 现有发展合理的语言模型研究主要集中在英语，缺乏对多语言环境的探讨。本研究旨在系统性地研究紧凑语言模型在英法双语场景下的表现，填补这一研究空白。

Method: 扩展BabyBERTa到英法双语场景，在严格数据量匹配条件下设计三种设置：单语、双语和跨语言。对比两种训练语料：儿童导向语音（约250万词）和多领域语料（约1000万词），并引入新的法语评估资源。

Result: 结果显示出情境依赖效应：维基百科训练持续有益于语义任务，而儿童导向语音在单语设置中改善语法判断。双语预训练在文本蕴含任务上带来显著提升，特别是法语表现尤为突出。这些模式在BabyBERTa、RoBERTa和LTG-BERT中一致出现。

Conclusion: 研究揭示了训练数据对语言模型表现的系统性影响，为多语言发展合理的语言模型研究提供了重要见解，并展示了跨架构的一致性趋势。
...
Method: DS²-Instruct框架：1) 生成任务相关关键词确保领域覆盖全面性；2) 将这些关键词与布鲁姆分类学的不同认知层次配对创建多样化指令；3) 使用自一致性验证确保数据质量

Result: 在数学、金融、逻辑推理等七个挑战性领域生成数据集，评估显示使用该框架生成数据微调的模型相比现有数据生成方法有显著改进

Conclusion: DS²-Instruct提供了一种无需人工监督生成高质量领域特定指令数据集的有效方法，显著提升了LLM在专业领域的适应能力
...
TL;DR: Long-form RewardBench是首个专门针对长文本生成的奖励模型测试基准，包含QA、RAG、Chat、Writing、Reasoning五个子任务，评估了20多个主流奖励模型，发现当前模型在长文本奖励建模能力上仍有不足。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习对齐方法广泛应用，奖励模型评估基准在多个领域已有建立，但长文本生成这一关键实际应用场景的奖励模型评估仍存在显著空白。

Method: 通过精心设计的多阶段数据收集流程收集指令和偏好数据，构建包含五个关键子任务的基准，并对20多个主流奖励模型（包括分类器和生成模型）进行广泛实验，还设计了新颖的"长文本针在干草堆测试"。

Result: 当前模型在长文本奖励建模能力上表现不足；长文本针在干草堆测试显示奖励模型性能与错误在回答中的位置及回答总长度相关，分类器和生成模型表现出不同特征；分类器比在相同数据上训练的生成模型具有更好的泛化能力。

Conclusion: Long-form RewardBench作为首个长文本奖励建模基准，旨在为这一关键领域提供稳健的进展可视化平台，推动长文本奖励模型的发展。
...
Conclusion: 提出的自进化框架能够有效减少对人工标注的依赖，在错误跨度检测任务中实现优越性能
...
TL;DR: 本文提出了一种PCA扫描方法，用于解决监督语义差异分析中PCA维度选择缺乏系统方法的问题，通过联合考虑表示能力、梯度可解释性和稳定性来选择最佳维度。


<details>
  <summary>Details</summary>
Motivation: 监督语义差异（SSD）方法在应用PCA时缺乏选择保留组件数量的系统方法，这引入了不必要的研究者自由度，影响了分析的可重复性和透明度。

Method: 提出PCA扫描程序，将维度选择视为表示能力、梯度可解释性和K值附近稳定性的联合标准。该方法在关于人工智能的短帖语料库上进行演示，参与者同时完成了自恋量表。

Result: 扫描方法产生了稳定、可解释的"赞赏"相关梯度，将乐观、协作的AI框架与不信任和嘲弄的话语形成对比，而"竞争"维度没有出现稳健的对齐。高PCA维度启发式方法产生的是分散、弱结构的聚类。

Conclusion: PCA扫描方法在保持SSD解释目标的同时约束了研究者自由度，支持对内涵意义进行透明且具有心理学意义的分析。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在高资源语言翻译上表现出色，但在低资源语言翻译上表现不佳。现有的后训练方法严重依赖高质量平行数据，而这些数据对于低资源语言往往稀缺或不可得。

Method: 提出WALAR方法，基于对现有基于源语言的多语言质量评估模型失败模式（"漏洞"）的观察。使用强化学习训练，但通过词对齐和语言对齐技术来缓解质量评估模型中的漏洞问题。

Result: 使用WALAR持续训练了一个支持101种语言翻译的大语言模型。实验显示，该模型在Flores-101数据集的1400个语言方向上大幅优于最强的开源多语言大语言模型LLaMAX。
...
TL;DR: 该论文介绍了ESG-Bench基准数据集，用于评估大型语言模型在ESG报告理解中的表现并减少幻觉问题，通过特定设计的思维链提示策略显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 随着ESG报告成为法律要求和评估企业长期道德表现的关键渠道，这些报告的长度和复杂性使得解读和自动化分析变得困难。需要支持可扩展且可信赖的分析工具。

Method: 创建ESG-Bench基准数据集，包含基于真实ESG报告上下文的人工标注问答对，并带有细粒度标签指示模型输出是否事实支持或存在幻觉。设计了任务特定的思维链提示策略，并在多个最先进的大型语言模型上使用思维链标注的推理进行微调。

Result: 实验表明，基于思维链的方法在减少幻觉方面显著优于标准提示和直接微调，并且这些增益能够转移到ESG领域之外的现有问答基准上。

Conclusion: 将ESG报告分析框架化为具有可验证性约束的问答任务，能够系统评估大型语言模型提取和推理ESG内容的能力，并为在具有社会敏感性和合规关键性的环境中减少幻觉提供了新的应用案例。
...
TL;DR: NAIT框架通过分析神经元激活模式相似性，从指令调优数据集中高效选择高质量子集，仅用10%数据即可超越其他方法，并揭示了神经元激活特征在不同能力间的可迁移性。


<details>
  <summary>Details</summary>
Motivation: 指令调优(IT)能有效释放大语言模型能力，但过多数据会降低性能，而精心选择高质量子集能显著提升能力。因此，如何从IT数据集中识别最有效的子集数据以开发特定或通用能力成为关键挑战。

Method: 提出NAIT框架，通过分析IT数据与目标领域能力之间的神经元激活模式相似性来评估数据影响。从目标能力领域内数据中捕获神经元激活模式，构建可重用、可迁移的神经元激活特征，然后基于候选样本与目标能力预期激活特征的相似性来评估和选择最优样本。

Result: 实验显示，使用NAIT选择的10% Alpaca-GPT4 IT数据子集进行训练，在各种任务上始终优于依赖外部先进模型或基于不确定性特征的方法。研究发现神经元激活特征在不同LLM能力间具有可迁移性：具有更多逻辑推理和编程特征的IT数据具有强大的通用迁移能力，而稳定的核心数据子集足以持续激活基础模型能力并普遍提升多样化任务性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决将原始AIS轨迹数据转换为结构化、语义丰富表示的问题，使其既能为人类理解，又能直接用于机器推理系统，支持海事分析和LLM集成。

Method: 提出上下文感知轨迹抽象框架：1) 将噪声AIS序列分割为独立的航次；2) 每个航次包含清洁的、带移动注释的片段；3) 用多源上下文信息（地理实体、海上导航特征、天气条件）丰富每个片段；4) 支持使用LLM生成受控自然语言描述。

Result: 通过增加语义密度和降低时空复杂性，这种抽象表示可以促进下游分析，并支持与LLM集成进行更高级的海事推理任务。通过实证研究评估了多个LLM在AIS数据和开放上下文特征上生成描述的质量。

Conclusion: 提出的上下文感知轨迹抽象框架成功地将原始AIS数据转换为结构化、语义丰富的表示，支持LLM生成自然语言描述，为海事分析和推理任务提供了有效的数据表示方法。
...
### [121] [Beyond Final Answers: CRYSTAL Benchmark for Transparent Multimodal Reasoning Evaluation](https://arxiv.org/abs/2603.13099)
*Wayner Barrios,SouYoung Jin*

Main category: cs.AI

TL;DR: CRYSTAL是一个包含6,372个实例的诊断基准，通过可验证的中间步骤评估多模态推理能力，并提出两种互补的评估指标和新的训练方法。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法主要关注最终答案准确性，无法揭示推理过程中的系统性失败，如步骤选择偏差、顺序混乱等问题，需要更细粒度的诊断工具。

Method: 1) 构建CRYSTAL基准：通过Delphi启发式流程，由四个独立MLLM生成推理轨迹，经语义聚类聚合和人工质量验证；2) 提出Match F1和Ordered Match F1两种评估指标；3) 提出因果过程奖励(CPR)和CPR-Curriculum训练方法。

Result: 评估20个MLLM（包括未参与基准构建的商业前沿系统）发现：普遍存在步骤选择偏差（精确度远高于召回率）、非单调缩放权衡、推理顺序混乱（没有竞争模型能保持超过60%的匹配步骤在正确顺序）。CPR-Curriculum在GRPO中实现+32% Match F1提升。

Conclusion: CRYSTAL基准揭示了传统准确性评估无法发现的系统性推理失败，提出的CPR和CPR-Curriculum方法能有效提升多模态推理质量，为理解和改进MLLM的推理能力提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型(LRMs)存在过度思考(在简单问题上消耗冗余计算步骤)和思考不足(未能充分利用推理能力探索足够路径)的问题，导致效率低下和潜在错误，限制了在资源受限环境中的实际部署。现有方法可能无意中引发思考不足，损害准确性。

Method: ReBalance利用置信度作为推理动态的连续指标：通过高置信度方差识别过度思考，通过持续过度自信识别思考不足。从小规模数据集中聚合隐藏状态形成推理模式原型，计算引导向量来指导LRMs的推理轨迹。动态控制函数基于实时置信度调节该向量的强度和方向，在过度思考时剪枝冗余，在思考不足时促进探索。
...
TL;DR: 该论文将基于网页的自主智能体任务形式化为序列决策过程，提出将现代智能体架构与传统规划范式对应的分类法，并开发了五个新的评估指标来超越简单的成功率评估。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的网页任务智能体通常作为黑箱运行，难以诊断失败原因和规划过程。需要建立形式化框架来理解智能体的决策行为，并提供更全面的评估方法。

Method: 1. 将网页任务形式化为序列决策过程；2. 提出分类法：逐步执行智能体对应广度优先搜索，树搜索智能体对应最佳优先树搜索，全预先规划智能体对应深度优先搜索；3. 开发五个新的评估指标；4. 使用WebArena基准的794个人工标注轨迹数据集；5. 比较逐步执行智能体与全预先规划智能体。

Result: 逐步执行智能体更接近人类黄金轨迹（38%总体成功率），而全预先规划智能体在技术指标上表现更佳（元素准确率89%）。新评估指标揭示了不同架构在不同应用约束下的优势差异。

Conclusion: 提出的分类法和评估框架为诊断智能体失败和选择适当架构提供了理论基础。不同智能体架构在不同指标上各有优势，需要根据具体应用需求选择合适的架构，而不仅仅是依赖成功率。
...
Method: 基于真实故障发动机数据，计算传感器实际读数与预期值偏差的导数，使用随机森林算法进行预测。相比传统方法关注信号偏差本身，该方法利用偏差导数来更早检测异常动态

Result: 仿真结果证实该方法能有效预测灾难性故障的发生。真实数据验证进一步强化了方法的鲁棒性和实际适用性。使用深度学习数据增强解决训练数据获取问题

Conclusion: 提出的基于偏差导数的随机森林方法能在测量值达到临界阈值前检测异常，为操作人员提供提前预警，避免发动机损坏和意外动力损失，同时有时间安全改变航线
...
### [125] [ToolTree: Efficient LLM Agent Tool Planning via Dual-Feedback Monte Carlo Tree Search and Bidirectional Pruning](https://arxiv.org/abs/2603.12740)
*Shuo Yang,Soyeon Caren Han,Yihao Ding,Shuhe Wang,Eduard Hoy*

Main category: cs.AI

TL;DR: ToolTree：一种基于蒙特卡洛树搜索的LLM智能体工具规划新范式，通过双阶段评估和双向剪枝机制提升多步骤工具使用的规划能力


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体的工具规划方法通常采用贪婪、反应式的工具选择策略，缺乏前瞻性且无法考虑工具间的依赖关系，难以处理复杂的多步骤任务

Method: 提出ToolTree规划范式，受蒙特卡洛树搜索启发，采用双阶段LLM评估和双向剪枝机制，在工具执行前后对工具使用轨迹进行探索和优化

Result: 在4个基准测试的开集和闭集工具规划任务中，ToolTree在保持最高效率的同时，相比最先进的规划范式平均提升约10%的性能

Conclusion: ToolTree通过前瞻性规划和自适应决策，显著提升了LLM智能体在复杂多步骤工具使用任务中的规划能力
...
### [126] [AI Model Modulation with Logits Redistribution](https://arxiv.org/abs/2603.12755)
*Zihan Wang,Zhongkui Ma,Xinguo Feng,Zhiyang Mei,Ethan Ma,Derui Wang,Minhui Xue,Guangdong Bai*

Main category: cs.AI

TL;DR: AIM是一种新的模型调制范式，使单个模型能展现多样化行为以满足不同需求，支持效用和焦点两种调制模式，无需重新训练即可动态控制输出质量和聚焦特征。


<details>
  <summary>Details</summary>
Motivation: 大规模模型通常需要适应模型所有者和用户的多样化需求，但维护多个专门化版本效率低下，因此需要一种方法让单个模型能够展现多样化行为。

Method: 提出AIM模型调制范式，引入logits重分配策略，在训练数据无关且无需重新训练的情况下运行。基于logits排序的统计特性建立形式化基础，支持效用调制（动态控制输出质量）和焦点调制（精确控制模型聚焦的输入特征）。
...
### [127] [Context is all you need: Towards autonomous model-based process design using agentic AI in flowsheet simulations](https://arxiv.org/abs/2603.12813)
...
<details>
  <summary>Details</summary>
Motivation: 虽然基于大语言模型的智能AI系统在软件开发领域已广泛应用，但在化工流程模拟领域仍处于探索阶段。本文旨在开发一个能够在工业流程模拟环境中提供辅助的智能AI框架。

Method: 首先展示了GitHub Copilot结合Claude Opus等先进大语言模型，利用技术文档和注释示例生成Chemasim流程建模工具的有效语法。在此基础上，开发了一个多智能体系统：一个智能体利用工程知识解决抽象问题，另一个智能体将解决方案实现为Chemasim代码。

Result: 该框架在典型的流程建模示例中表现出有效性，包括：(1)反应/分离过程，(2)变压精馏，(3)包含夹带剂选择的非均相共沸精馏。同时讨论了当前框架的局限性。

Conclusion: 本文提出的智能AI框架为化工流程模拟提供了有效的辅助工具，虽然存在一些局限性，但为未来研究指明了方向，有望进一步增强其能力。
...
### [128] [ODRL Policy Comparison Through Normalisation](https://arxiv.org/abs/2603.12926)
*Jaime Osvaldo Salas,Paolo Pareti,George Konstantinidis*

Main category: cs.AI

TL;DR: 本文提出了一种将复杂ODRL策略规范化的方法，通过参数化归一化将策略简化为最小组件，将权限和禁止转换为纯权限策略，简化复杂逻辑约束，并证明算法保持语义不变。
...
Method: 基于最近定义的语义，提出参数化归一化方法，将ODRL策略转化为最小组件：1) 将包含权限和禁止的策略重新表述为仅包含权限的策略；2) 将复杂逻辑约束简化为简单约束；3) 提供计算ODRL策略规范形式的算法；4) 提供简化数值和符号约束的算法。

Result: 证明算法能够保持策略语义不变，并分析结果的大小复杂度：在属性数量上呈指数级，在属性唯一值数量上呈线性。这使得复杂策略可以在更基本的ODRL片段中表示，并将策略比较问题简化为检查两条规则是否相同的简单问题。

Conclusion: 提出的归一化方法解决了ODRL策略的复杂性和互操作性问题，通过将策略转化为规范形式，简化了策略比较和处理，为ODRL的实际应用提供了有效工具。
...
### [129] [Efficient and Interpretable Multi-Agent LLM Routing via Ant Colony Optimization](https://arxiv.org/abs/2603.12933)
...
TL;DR: AMRO-S是一个高效可解释的多智能体系统路由框架，通过语义条件路径选择、SFT小模型意图推断、任务特定信息素专家和异步更新机制，在保持低延迟的同时优化质量-成本权衡。


<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的多智能体系统面临高推理成本、延迟和有限透明度等问题，现有路由策略依赖昂贵的LLM选择器或静态策略，在动态负载和混合意图下缺乏可控性，导致性能不稳定和资源利用效率低下。

Method: 1) 使用监督微调的小语言模型进行意图推断，为每个查询提供低开销语义接口；2) 将路由内存分解为任务特定信息素专家，减少跨任务干扰；3) 采用质量门控异步更新机制，将推理与学习解耦。

Result: 在五个公共基准测试和高并发压力测试中，AMRO-S相比强基线路由方法持续改善了质量-成本权衡，同时通过结构化信息素模式提供可追踪的路由证据。

Conclusion: AMRO-S通过语义条件路径选择建模，实现了高效、可解释的多智能体系统路由，解决了现有方法在成本、延迟和透明度方面的限制，为实际部署提供了可扩展的解决方案。
...
### [130] [Steve-Evolving: Open-World Embodied Self-Evolution via Fine-Grained Diagnosis and Dual-Track Knowledge Distillation](https://arxiv.org/abs/2603.13131)
*Zhengwei Xie,Zhisheng Chen,Ziyan Weng,Tingyu Wu,Chenglong Li,Vireo Zhang,Kun Wang*

Main category: cs.AI

TL;DR: Steve-Evolving：一种非参数自进化框架，通过细粒度执行诊断与双轨知识蒸馏的紧密耦合，解决开放世界具身智能体长时程任务中的经验组织与进化问题。


<details>
  <summary>Details</summary>
Motivation: 开放世界具身智能体面临的主要瓶颈不是单步规划质量，而是交互经验如何组织和进化。现有方法在长时程任务中难以有效积累和利用经验，需要一种能够持续自我进化的框架。

Method: 采用三阶段框架：1) 经验锚定：将子目标尝试固化为结构化经验元组，构建三层经验空间并建立多维索引；2) 经验蒸馏：成功轨迹提炼为可重用技能，失败提炼为可执行防护栏；3) 知识驱动闭环控制：检索技能和防护栏注入LLM规划器，诊断触发局部重规划在线更新约束。

Result: 在Minecraft MCU长时程任务套件上的实验表明，相比静态检索基线方法，Steve-Evolving框架取得了持续改进。

Conclusion: Steve-Evolving通过紧密耦合执行诊断与知识蒸馏，实现了无需模型参数更新的持续进化过程，有效解决了开放世界具身智能体长时程任务中的经验组织与进化问题。
...
### [131] [When Right Meets Wrong: Bilateral Context Conditioning with Reward-Confidence Correction for GRPO](https://arxiv.org/abs/2603.13134)
...
Method: 1. 提出对比式GRPO重构，揭示GRPO目标隐含地最大化正确与错误样本策略比率之间的边界；2. 提出双边上下文条件化机制，让模型在优化过程中交叉参考成功与失败的推理轨迹；3. 提出奖励置信度校正机制，使用奖励-置信度协方差动态调整GRPO中的优势基线。

Result: 在数学推理基准测试中，该方法在多种模型和算法上均表现出持续改进，无需额外采样或辅助模型，可适配所有GRPO变体。

Conclusion: 通过利用组内正确与错误解决方案之间的对比信息，提出的双边上下文条件化和奖励置信度校正机制能有效提升GRPO的训练效果，为推理模型优化提供了新思路。
...
### [132] [Developing and evaluating a chatbot to support maternal health care](https://arxiv.org/abs/2603.13168)
...
TL;DR: 本文介绍了一个面向印度孕产妇健康的聊天机器人系统，该系统通过多机构合作开发，结合了阶段感知分流、混合检索和证据条件生成技术，并提出了高风险部署下的评估工作流程。


<details>
  <summary>Details</summary>
Motivation: 在低资源环境中，用户健康素养低且医疗资源有限，提供可信赖的孕产妇健康信息具有重要影响。然而，部署此类系统面临技术挑战：用户查询简短、不完整且多语言混杂，答案需要地区特定背景，部分或缺失的症状信息使得安全分流决策困难。

Method: 系统结合了：(1) 阶段感知分流，将高风险查询路由到专家模板；(2) 对精选孕产妇/新生儿指南的混合检索；(3) 基于LLM的证据条件生成。核心贡献是在有限专家监督下的高风险部署评估工作流程，包括：标记分流基准测试、合成多证据检索基准、LLM作为评判者的真实查询比较以及专家验证。

Result: 分流基准测试(N=150)实现了86.7%的紧急情况召回率，明确报告了漏报紧急情况与过度升级之间的权衡；开发了多证据检索基准(N=100)；在真实查询(N=781)上使用临床医生设计的标准进行LLM评判比较；并通过专家验证。

Conclusion: 在嘈杂的多语言环境中构建可信赖的医疗助手需要深度防御设计配合多方法评估，而不是依赖单一模型和评估方法选择。系统展示了在有限专家监督下进行高风险部署的可行评估框架。
...
### [133] [Unmasking Biases and Reliability Concerns in Convolutional Neural Networks Analysis of Cancer Pathology Images](https://arxiv.org/abs/2603.12445)
*Michael Okonoda,Eder Martinez,Abhilekha Dalal,Lior Shamir*

Main category: eess.IV

TL;DR: CNN在癌症病理图像分类中的评估存在严重偏差，即使使用不含临床信息的背景图像片段，模型也能达到高达93%的准确率，表明标准评估方法可能产生误导性结果。


<details>
  <summary>Details</summary>
Motivation: 尽管CNN在癌症放射学图像分类中表现出有效性，但其不透明性限制了对其工作原理的深入理解。研究者希望检验当前CNN评估标准实践在癌症病理学应用中的可靠性，特别是评估方法是否能够真正反映模型对临床相关信息的识别能力。

Method: 研究分析了13个常用的癌症基准数据集，使用四种常见的CNN架构，涵盖黑色素瘤、癌瘤、结直肠癌和肺癌等不同类型癌症。通过将原始图像中不包含临床相关内容的背景片段裁剪出来创建新数据集，然后将CNN模型在这些"无信息"数据集上的准确率与原始数据集上的表现进行比较。

Result: 结果显示，即使在使用不含生物医学信息的背景图像片段数据集时，CNN模型仍能提供高准确率，有时高达93%。这表明某些CNN架构比其他架构更容易受到偏差影响。标准机器学习评估实践在应用于癌症病理学时可能产生不可靠的结果。

Conclusion: 当前CNN在癌症病理学中的评估方法存在严重偏差，这些偏差难以识别，可能导致研究人员在使用基准数据集测试CNN方法有效性时被误导。需要更严谨的评估方法来确保模型真正学习到临床相关信息而非数据集偏差。
...
### [134] [Multiscale Structure-Guided Latent Diffusion for Multimodal MRI Translation](https://arxiv.org/abs/2603.12581)
*Jianqiang Lin,Zhiqiang Shen,Peng Cao,Jinzhu Yang,Osmar R. Zaiane,Xiaoli Liu*

Main category: eess.IV

TL;DR: MSG-LDM是一个基于潜在扩散的多模态MRI转换框架，通过风格-结构解耦机制和结构感知损失，在任意缺失模态场景下实现更好的解剖结构一致性和纹理细节保留。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在多模态MRI转换任务中，处理任意缺失模态场景时容易出现解剖结构不一致或纹理细节退化的问题，需要改进以同时保持可靠的边界细节和完整结构信息。

Method: 提出潜在空间中的风格-结构解耦机制，将模态特定风格特征与共享结构表示分离；在多尺度特征空间中联合建模低频解剖布局和高频边界细节；设计风格一致性损失和结构感知损失来减少风格干扰并提高结构表示稳定性。

Result: 在BraTS2020和WMH数据集上的大量实验表明，该方法优于现有的MRI合成方法，特别是在重建完整结构方面表现突出。

Conclusion: MSG-LDM框架通过风格-结构解耦和多尺度特征建模，有效解决了多模态MRI转换中的解剖结构一致性和纹理细节保留问题，为任意缺失模态场景提供了更可靠的解决方案。
...
### [135] [Deep Learning Based Estimation of Blood Glucose Levels from Multidirectional Scleral Blood Vessel Imaging](https://arxiv.org/abs/2603.12715)
*Muhammad Ahmed Khan,Manqiang Peng,Ding Lin,Saif Ur Rehman Khan*

Main category: eess.IV

TL;DR: ScleraGluNet：基于多视角深度学习框架，利用巩膜血管图像进行三分类代谢状态识别和连续空腹血糖估计的非侵入性血糖评估方法


<details>
  <summary>Details</summary>
Motivation: 传统血糖监测方法（如血液检测）对频繁评估较为繁琐，而巩膜含有浅表微血管系统，可能反映糖尿病相关变化，且易于在眼表观察到，因此开发非侵入性血糖评估方法具有重要意义。

Method: 提出ScleraGluNet多视角深度学习框架，从多方向巩膜血管图像中进行三分类代谢状态识别（正常、控制良好的糖尿病、高血糖糖尿病）和连续空腹血糖估计。数据集包含445名参与者（150/140/155）和2,225张前段图像（每人5个注视方向）。血管增强后，使用并行卷积分支提取特征，通过蝠鲼觅食优化算法进行优化，并通过基于Transformer的跨视角注意力机制进行特征融合。

Result: ScleraGluNet实现了93.8%的总体准确率，正常、控制良好的糖尿病和高血糖糖尿病的单对多AUC分别为0.971、0.956和0.982。对于空腹血糖估计，模型达到MAE=6.42 mg/dL和RMSE=7.91 mg/dL，与实验室测量结果有强相关性（r=0.983；R²=0.966）。Bland-Altman分析显示平均偏差为+1.45 mg/dL，95%一致性界限为-8.33至+11.23 mg/dL。

Conclusion: 多方向巩膜血管成像结合多视角学习是一种有前景的非侵入性血糖评估方法，支持在临床部署前进行多中心验证。
...
### [136] [GLEAM: A Multimodal Imaging Dataset and HAMM for Glaucoma Classification](https://arxiv.org/abs/2603.12800)
*Jiao Wang,Chi Liu,Yiying Zhang,Hongchen Luo,Zhifen Guo,Ying Hu,Ke Xu,Jing Zhou,Hongyan Xu,Ruiting Zhou,Man Tang*

Main category: eess.IV

TL;DR: GLEAM是首个公开的三模态青光眼数据集，包含三种成像模态，标注了四个疾病阶段。HAMM框架通过分层注意力掩码建模有效整合跨模态信息，实现青光眼分类。


<details>
  <summary>Details</summary>
Motivation: 青光眼诊断需要多模态成像信息，但缺乏公开的多模态数据集和有效的跨模态信息整合方法。现有方法难以充分利用不同成像模态的互补信息进行准确诊断。

Method: 提出GLEAM三模态数据集（扫描激光眼底镜、环视盘OCT、视野模式偏差图），并开发HAMM框架：使用分层注意力编码器和轻量解码器，通过掩码建模在编码器层面进行跨模态表示学习。

Result: 创建了首个公开的三模态青光眼数据集，HAMM框架能够有效整合跨模态信息，实现准确的青光眼分期诊断。

Conclusion: GLEAM数据集和HAMM框架为青光眼的多模态诊断提供了重要资源和方法，有助于在不同疾病阶段实现更准确的诊断和治疗。
...
### [137] [Editing Away the Evidence: Diffusion-Based Image Manipulation and the Failure Modes of Robust Watermarking](https://arxiv.org/abs/2603.12949)
*Qian Qi,Jiangyun Tang,Jim Lee,Emily Davis,Finn Carter*

Main category: eess.IV

TL;DR: 扩散编辑会无意中破坏或移除鲁棒水印，因为扩散过程会收缩离流形扰动，导致低幅度水印信号衰减，影响版权保护和内容溯源


<details>
  <summary>Details</summary>
Motivation: 研究扩散式图像编辑对鲁棒水印的影响，因为扩散编辑引入了与传统后处理不同的变换类型，可能无意中破坏水印，影响版权保护和内容溯源的可靠性

Method: 将扩散编辑建模为随机变换，分析水印信噪比和互信息在扩散轨迹上的边界，推导可靠恢复的信息理论条件，并评估代表性水印系统在不同扩散编辑场景下的表现

Result: 扩散编辑会显著降低水印可恢复性，即使常规语义编辑也会导致水印信号衰减，可靠恢复在某些条件下变得信息理论上不可能

Conclusion: 扩散编辑对现有水印技术构成新挑战，需要设计专门针对生成式图像编辑保持鲁棒性的水印方法，以确保内容溯源和版权保护的有效性
...
### [138] [Reinforcing the Weakest Links: Modernizing SIENA with Targeted Deep Learning Integration](https://arxiv.org/abs/2603.12951)
*Riccardo Raciti,Lemuel Puglisi,Francesco Guarnera,Daniele Ravì,Sebastiano Battiato*

Main category: eess.IV

TL;DR: 该研究将深度学习模块SynthStrip和SynthSeg集成到传统脑萎缩评估工具SIENA中，通过替换颅骨剥离和组织分割等薄弱环节，显著提升了PBVC评估的准确性、鲁棒性和计算效率。


<details>
  <summary>Details</summary>
Motivation: SIENA作为广泛使用的脑萎缩评估方法，依赖传统的图像处理步骤（特别是颅骨剥离和组织分割），这些步骤的失败会传播到整个流程并导致萎缩估计偏差。研究旨在探索是否可以通过针对性的深度学习模块替换来改进SIENA，同时保留其成熟且可解释的框架。

Method: 将深度学习工具SynthStrip（颅骨剥离）和SynthSeg（组织分割）集成到SIENA流程中，在ADNI和PPMI纵向队列上评估三种不同的流程变体。使用三个互补标准评估性能：与纵向临床和结构衰退的相关性、扫描顺序一致性以及端到端运行时间。

Result: 替换颅骨剥离模块带来最一致的改进：在ADNI中，相对于标准SIENA流程，显著增强了PBVC与多种疾病进展指标之间的关联；在两个数据集中都显著提高了扫描反转下的鲁棒性。完全集成的流程实现了最强的扫描顺序一致性，将误差降低了高达99.1%。GPU加速变体将执行时间减少了高达46%，同时保持与标准SIENA相当的CPU运行时间。

Conclusion: 深度学习可以通过强化传统纵向萎缩流程中最薄弱的图像处理步骤来显著增强其性能。这项研究更广泛地展示了模块化现代化临床可信赖的神经影像工具的价值，同时不牺牲其可解释性。
...
### [139] [Accelerating Stroke MRI with Diffusion Probabilistic Models through Large-Scale Pre-training and Target-Specific Fine-Tuning](https://arxiv.org/abs/2603.13007)
*Yamin Arefeen,Sidharth Kumar,Steven Warach,Hamidreza Saber,Jonathan Tamir*

Main category: eess.IV

TL;DR: 提出一种基于扩散概率生成模型的数据高效MRI重建策略，通过大规模预训练加小规模微调，在仅有少量目标领域数据的情况下实现临床可接受的加速MRI重建


<details>
  <summary>Details</summary>
Motivation: 解决临床卒中MRI加速扫描中面临的数据稀缺问题，传统方法需要大量特定应用数据，而临床环境中难以获取足够数量的完全采样数据

Method: 采用基础模型范式：1）在大规模多样化脑MRI数据集（fastMRI，约4000个非FLAIR对比度样本）上预训练扩散概率生成模型；2）在少量目标应用数据（仅20个FLAIR样本）上进行微调，精心选择学习率和微调时长

Result: 预训练模型在仅20个目标样本微调后，重建性能与使用大量目标领域FLAIR数据训练的传统模型相当；适度微调（降低学习率）效果最佳，不足或过度微调会降低重建质量；临床盲法读片研究显示2倍加速数据重建图像在质量和结构描绘方面不劣于标准方法

Conclusion: 大规模预训练结合针对性微调使扩散模型能在数据受限的临床卒中MRI加速重建中有效应用，显著减少对特定应用大数据集的需求，同时保持临床可接受的图像质量
...
### [140] [DiT-IC: Aligned Diffusion Transformer for Efficient Image Compression](https://arxiv.org/abs/2603.13162)
*Junqi Shi,Ming Lu,Xingchen Li,Anle Ke,Ruiqi Zhang,Zhan Ma*

Main category: eess.IV

TL;DR: DiT-IC是一种基于扩散Transformer的图像压缩方法，在32倍下采样的潜在空间中运行，相比传统U-Net扩散编码器实现了30倍加速和大幅内存降低，能在16GB笔记本GPU上重建2048x2048图像。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的图像压缩方法虽然感知质量优秀，但存在采样开销大、内存使用高的问题。传统U-Net架构只能在浅层潜在空间（8倍下采样）运行，而传统VAE编码器能在更深层潜在空间（16-64倍下采样）工作，因此研究扩散模型能否在紧凑潜在空间中有效运行而不损失重建质量。

Method: DiT-IC使用扩散Transformer替代U-Net，在32倍下采样的潜在空间中进行扩散。通过三个关键对齐机制：1）方差引导重建流，根据潜在不确定性调整去噪强度；2）自蒸馏对齐，强制与编码器定义的潜在几何一致性以实现单步扩散；3）潜在条件引导，用语义对齐的潜在条件替代文本提示，实现无文本推理。

Result: DiT-IC实现了最先进的感知质量，同时比现有基于扩散的编码器提供高达30倍的解码加速和大幅降低的内存使用。能够在16GB笔记本GPU上重建2048x2048图像。

Conclusion: DiT-IC证明了扩散模型可以在紧凑潜在空间（32倍下采样）中有效运行，通过扩散Transformer架构和三个对齐机制，在保持优秀感知质量的同时显著提升了扩散编码器的实用性和效率。
