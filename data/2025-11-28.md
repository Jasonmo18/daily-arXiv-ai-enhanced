...
TL;DR: G²VLM是一个几何基础的视觉语言模型，通过将3D视觉几何特征与语言模型结合，解决了现有视觉语言模型在空间智能方面的不足，实现了空间3D重建和空间理解任务的统一处理。
...
Method: 提出G²VLM模型，通过利用学习到的3D视觉几何特征来直接预测3D属性，并通过上下文学习和交错推理增强空间推理任务。该统一设计能够在大规模多视角图像和视频数据上训练，同时利用通常需要难以收集的标注才能获得的3D视觉先验。

Result: 实验结果表明G²VLM在两项任务上都表现出色：在3D重建任务上达到与最先进前馈模型相当的结果，在空间理解和推理任务上取得更好或具有竞争力的结果。

Conclusion: 通过将语义强大的视觉语言模型与低级3D视觉任务统一，G²VLM可以作为社区的强基线，并为未来应用（如3D场景编辑）开辟更多可能性。
...
TL;DR: Qwen3-VL是Qwen系列中最强大的视觉语言模型，支持256K令牌的文本、图像和视频交错上下文，在多种多模态基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够无缝集成文本、图像和视频，并支持长上下文理解的多模态模型，以满足现实工作流程中图像推理、智能决策和多模态代码智能的需求。

Method: 采用增强的交错MRoPE进行时空建模，集成DeepStack以加强视觉语言对齐，以及基于文本的时间对齐技术。模型包括密集（2B/4B/8B/32B）和专家混合（30B-A3B/235B-A22B）变体。

Result: 在MMMU、MathVista和MathVision等综合评估中表现领先，纯文本理解能力超越同类文本模型，长上下文理解能力出色。

Conclusion: Qwen3-VL作为基础引擎，在图像推理、智能决策和多模态代码智能方面具有广泛应用前景，在密集和MoE架构下均实现优越性能。
...
<details>
  <summary>Details</summary>
Motivation: AI模型在日常设备中的普及凸显了一个关键挑战：预测错误会降低用户体验。现有解决方案主要关注错误检测，但很少提供高效的纠正机制，特别是在资源受限的设备上。

Method: 系统包含两个关键组件：(1) 服务器端管道，利用知识蒸馏将稳健特征表示从基础模型转移到设备兼容架构；(2) 设备端机制，通过原型适配实现超高效错误纠正。

Result: 在Food-101和Flowers-102数据集上，一次学习场景下实现了超过50%的错误纠正率，同时保持极小的遗忘（小于0.02%）和可忽略的计算开销。Android演示应用验证了系统的实用性。

Conclusion: 该系统证明了在现实场景中通过原型更新而非模型重训练来实现高效AI错误纠正的可行性，为资源受限设备上的AI纠错提供了实用解决方案。
...
TL;DR: 提出了ADVLA框架，通过在视觉编码器投影到文本特征空间的特征上直接应用对抗扰动，在低幅度约束下高效破坏下游动作预测，注意力引导使扰动既集中又稀疏。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法需要昂贵的端到端训练且通常生成明显的扰动补丁，为解决这些限制而开发ADVLA。

Method: 在视觉编码器投影到文本特征空间的特征上直接应用对抗扰动，引入三种策略增强敏感性、强制稀疏性和集中扰动，结合Top-K掩码修改少于10%的补丁。

Result: 在L∞=4/255约束下，ADVLA实现近100%的攻击成功率，扰动集中在关键区域，整体图像几乎不可察觉，单步迭代仅需约0.06秒，显著优于传统基于补丁的攻击。
...
TL;DR: MoGAN是一个专注于提升视频生成运动真实性的后训练框架，通过光学流判别器和分布匹配正则化器，在不牺牲视觉质量的情况下显著改善运动质量。


<details>
  <summary>Details</summary>
Motivation: 现有的视频扩散模型在帧级保真度上表现良好，但在运动连贯性、动态性和真实性方面存在不足，经常产生抖动、重影或不合理的动态。标准的去噪MSE目标缺乏对时间一致性的直接监督，导致模型即使损失较低仍可能生成较差的运动效果。

Method: 在3步蒸馏视频扩散模型基础上，训练基于DiT的光学流判别器来区分真实与生成的运动，并结合分布匹配正则化器以保持视觉保真度。

Result: 在Wan2.1-T2V-1.3B上的实验显示，MoGAN显著提升了运动质量：在VBench上运动得分比50步教师模型提升+7.3%，比3步DMD模型提升+13.3%；在VideoJAM-Bench上分别提升+7.4%和+8.8%，同时保持相当或更好的美学和图像质量得分。人类研究也证实MoGAN在运动质量上更受偏好。

Conclusion: MoGAN在不牺牲视觉保真度或效率的情况下，提供了显著更真实的运动效果，为快速高质量视频生成提供了实用路径。
...
TL;DR: CaFlow是一个用于长时动作质量评估的统一框架，结合了反事实去混淆和双向时间条件流，通过自监督方式解耦因果特征和混淆特征，并利用双向建模提升长期表示的稳定性和连贯性。


<details>
  <summary>Details</summary>
Motivation: 长时动作质量评估（如花样滑冰、艺术体操）面临建模长期时间动态和抵抗上下文混淆的挑战。现有方法依赖昂贵标注或单向时间建模，容易受到伪相关和不稳定长期表示的影响。

Method: 提出CaFlow框架，包含因果反事实正则化（CCR）模块和双向时间条件流（BiT-Flow）模块。CCR通过反事实干预自监督解耦因果和混淆特征，BiT-Flow通过前向和后向动态建模与循环一致性约束生成更平滑连贯的表示。

Result: 在多个长时动作质量评估基准测试上的广泛实验表明，CaFlow达到了最先进的性能。

Conclusion: CaFlow通过结合反事实去混淆和双向时间建模，有效解决了长时动作质量评估中的长期动态建模和上下文混淆问题，取得了优异的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型作为多模态评估评判者时，其遵循多样化、细粒度评估标准的能力尚未得到充分探索，需要系统评估模型在多元标准下的表现。

Method: 通过严格的数据收集流程构建Multi-Crit基准，收集具有多标准人工标注的挑战性响应对，并引入三个新指标来系统评估多元标准遵循、标准切换灵活性和识别标准级别偏好冲突的能力。

Result: 对25个多模态模型的综合分析显示：专有模型在遵循多元标准方面仍有困难（尤其在开放式评估中）；开源模型在灵活遵循多样化标准方面更落后；基于整体判断信号的批评微调增强了视觉基础能力但未能泛化到多元标准级别判断。
...
TL;DR: 本文首次系统研究仅通过相机轨迹（而非像素）来感知视频内容的可能性，发现相机轨迹是揭示视频内容的有效信号，提出了CamFormer对比学习框架将相机轨迹与自然语言对齐。


<details>
  <summary>Details</summary>
Motivation: 探索仅通过相机运动轨迹而不依赖像素信息来理解视频内容的可行性，验证"如何移动"能否揭示"在做什么"或"观察什么"。

Method: 提出对比学习框架训练CamFormer编码器，将相机位姿轨迹投影到联合嵌入空间，与自然语言对齐。

Result: 相机轨迹是揭示视频内容的有效信号，CamFormer嵌入在跨模态对齐、分类和时序分析等下游任务中表现优异，且对不同相机位姿估计方法具有鲁棒性。

Conclusion: 相机轨迹是一种轻量、鲁棒且多功能的视频内容感知模态，仅通过运动路径就能有效理解视频内容。
...
TL;DR: Canvas-to-Image是一个统一的框架，将文本提示、主题参考、空间布局、姿态约束和布局注释等多种控制信号整合到单一画布界面中，通过多任务画布训练策略实现高保真的组合和多模态图像生成。


<details>
  <summary>Details</summary>
Motivation: 现代扩散模型在生成高质量多样化图像方面表现出色，但在高保真组合和多模态控制方面仍存在困难，特别是当用户同时指定文本提示、主题参考、空间排列、姿态约束和布局注释时。
...
Conclusion: 联合训练使Canvas-to-Image能够跨多个控制模态进行推理，而不是依赖特定任务的启发式方法，在推理过程中能很好地泛化到多控制场景。
...
TL;DR: 本文提出了一种简单轻量级的方法，通过关联神经元激活与外部指标来识别编码特定技能的神经元，在文本生成和推理任务中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型表现出广泛能力，但其内部机制仍不透明，需要开发可解释性方法来理解模型如何编码特定技能。

Method: 基于软提示训练识别技能神经元的方法，扩展到复杂多技能场景，通过关联神经元激活与外部标签和模型置信度等辅助指标，无需手动标记聚合。

Result: 在开放文本生成和自然语言推理任务上验证了方法有效性，不仅检测到驱动已知技能的神经元，还发现了BigBench算术推理中先前未识别的捷径。

Conclusion: 该方法能够有效识别编码特定技能的神经元，为理解大语言模型内部机制提供了可解释性工具。
...
TL;DR: 该研究探索了在大型语言模型预训练中使用多种元数据类型（如文档质量指标）来加速训练，发现细粒度元数据编码信息更有效，并提出了元数据追加和可学习元标记等方法来提高训练效率。


<details>
  <summary>Details</summary>
Motivation: 先前的研究仅关注URL作为元数据信号，本研究旨在探索其他元数据类型是否能带来更大的训练加速效益，特别是那些编码细粒度信息的元数据。

Method: 研究了多种元数据类型，引入元数据追加作为辅助任务，使用可学习元标记配合掩码损失训练，并通过探针分析潜在表示来理解元数据如何影响学习过程。

Result: 发现细粒度文档质量指标等元数据能够加速预训练，元数据追加和可学习元标记都能有效提高训练效率，元数据通过诱导质量感知的潜在结构来发挥作用。

Conclusion: 研究结果为整合元数据以提升LLM预训练效率和效果提供了实用指南，强调了细粒度元数据的重要性。
...
TL;DR: 研究探讨捷克语AI生成诗歌与人类创作诗歌的感知差异，发现参与者无法区分两者，且存在作者偏见——当认为诗歌是AI生成时评价更低，尽管AI诗歌实际评价相当或更高。


<details>
  <summary>Details</summary>
Motivation: 大多数AI生成诗歌研究集中在英语上，本研究旨在考察在捷克语这种形态复杂、资源较少的斯拉夫语言中，AI诗歌的感知和审美评价。

Method: 让捷克母语者识别和评价AI与人类创作的诗歌，使用逻辑回归模型分析识别准确性与审美评价的关系。

Result: 参与者识别准确率接近随机水平（45.8%），AI诗歌与人类诗歌难以区分；存在作者偏见，认为AI生成的诗歌评价更低；诗歌喜爱程度与识别准确性呈负相关。

Conclusion: AI能在捷克语等复杂低资源语言中生成令人信服的诗歌，读者的作者信念与诗歌审美评价相互关联。
...
<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在处理深度复杂问题（如Humanity's Last Exam）时面临的概念挑战和计算成本高的问题，探索通过小型编排器管理其他模型和工具来提升智能上限和效率。

Method: ToolOrchestra方法训练小型编排器，使用强化学习结合结果感知、效率感知和用户偏好感知的奖励机制，协调多种智能工具的使用。

Result: Orchestrator在HLE上获得37.1%得分，超越GPT-5（35.1%）且效率提高2.5倍；在tau2-Bench和FRAMES上大幅超越GPT-5，仅使用约30%成本；在性能与成本权衡中表现最佳，并能稳健泛化到未见过的工具。

Conclusion: 通过轻量级编排模型组合多样化工具比现有方法更高效且更有效，为实用和可扩展的工具增强推理系统铺平道路。
...
TL;DR: 本文通过系统评估大语言模型在不同任务难度下的泛化能力，发现跨难度泛化通常有限，训练数据难度选择无法在全部难度范围内实现一致改进。
...
Result: 跨难度泛化能力有限，无论使用简单还是困难数据进行训练，都无法在全部难度范围内实现一致的性能提升。

Conclusion: 在LLM的训练和评估数据中需要包含不同难度级别的示例，在难度选择上走捷径存在风险。
...
<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在规划和状态推理方面的实际能力，特别是在不依赖代码执行或其他工具的情况下，通过经典的8拼图任务来精确评估。

Method: 测试了四种模型在零样本、思维链和算法思维等提示条件下，并采用分层纠正反馈和外部移动验证器来评估模型表现。

Result: 反馈提高了某些模型-提示组合的成功率，但成功运行通常冗长且计算昂贵。即使有外部移动验证器提供有效移动，所有模型都无法解决任何拼图。

Conclusion: 当前LLMs在规划方面存在显著限制，需要开发维护显式状态和执行结构化搜索的机制才能取得进一步进展。
...
TL;DR: 该论文将系统动力学和结构方程建模整合到一个共同的数学框架中，用于生成系统分布、开发方法并比较结果，以支持数据科学和AI/ML应用中的系统动力学认识论。


<details>
  <summary>Details</summary>
Motivation: AI/ML模型在解决未解决问题方面具有创新性，但也可能放大人类偏见。负责任AI/ML倡导者希望利用更丰富的系统动力学因果模型来指导开发，但不同方法基于不同基本假设的整合存在困难。

Method: 将系统动力学和结构方程建模整合到一个共同的数学框架中，该框架能够生成系统分布、开发方法并比较结果。

Result: 提出了一个统一的数学框架，能够弥合系统动力学和结构方程建模之间的方法论差异。

Conclusion: 该共同框架有助于为数据科学和AI/ML应用提供系统动力学的认识论基础，促进负责任AI/ML的发展。
...
### [18] [Entropy Coding for Non-Rectangular Transform Blocks using Partitioned DCT Dictionaries for AV1](https://arxiv.org/abs/2511.21609)
*Priyanka Das,Tim Classen,Mathias Wien*

Main category: eess.IV

TL;DR: 本文提出了一种针对非矩形分区变换系数的熵编码方法，该方法通过有效建模系数特性来提高编码效率，在实验设置中显示出显著的理论码率节省。


<details>
  <summary>Details</summary>
Motivation: 当前视频编码标准如VVC和AV1使用非矩形分区结合预测信号，但缺乏相应的变换技术支持。虽然已有方法在实验环境中显示出潜力，但现有的熵编码方案主要针对DCT系数设计，无法最优编码这些变换系数。

Method: 提出一种新的熵编码方法，通过有效建模非矩形分区变换系数的特性来高效编码这些系数。该方法利用条件熵估计理论码率节省，特别针对与DCT差异较大的场景。

Result: 该方法在实验设置中显示出显著的理论码率节省，特别是对于与DCT特性差异较大的场景。

Conclusion: 所提出的熵编码方法能够有效编码非矩形分区变换系数，为视频编码标准提供了改进的编码效率，特别是在非矩形分区场景下。
