...
Method: TP-Blend采用两个注意力处理器：1) 交叉注意力对象融合(CAOF)：通过熵正则化最优传输问题重新分配多头特征向量；2) 自注意力风格融合(SASF)：通过细节敏感实例归一化和Key-Value矩阵交换注入风格纹理。

Result: 实验表明TP-Blend能生成高分辨率、照片级真实的编辑结果，在内容控制和外观控制方面都表现出精确性，在定量保真度、感知质量和推理速度方面超越现有基线方法。

Conclusion: TP-Blend提供了一个轻量级、无需训练的框架，能够同时处理对象融合和风格注入，实现了对内容和外观的精确控制，为扩散模型编辑提供了新的解决方案。
...
TL;DR: 提出一个基于文本指令驱动的3D面部表情生成框架，能够从指定表情平滑过渡到目标表情，通过IFED模块实现多模态学习，I2FET方法优化语义理解，生成符合文本描述的面部表情序列。


<details>
  <summary>Details</summary>
Motivation: 传统的3D虚拟形象通常只有六种基本面部表情，为了模拟更真实的情感变化，需要能够渲染任意两种表情之间的平滑过渡。文本描述能够提供对人类情感状态的多样化描述，从而大大扩展面部表情及其过渡的可能性。

Method: 1. 提出指令驱动的面部表情分解器（IFED）模块，促进多模态数据学习并捕获文本描述与面部表情特征之间的相关性；2. 提出指令到面部表情过渡（I2FET）方法，利用IFED和顶点重建损失函数来优化潜在向量的语义理解，根据给定指令生成面部表情序列；3. 提出面部表情过渡模型，生成面部表情之间的平滑过渡。

Result: 在CK+和CelebV-HQ数据集上的广泛评估表明，所提出的模型优于最先进的方法。结果显示该框架能够根据文本指令生成面部表情轨迹，文本提示允许对人类情感状态进行多样化描述，从而大大扩展了面部表情及其过渡的可能性。

Conclusion: 该研究提出了一个创新的基于文本指令驱动的3D面部表情生成框架，能够实现任意两种面部表情之间的平滑过渡。通过多模态学习和语义理解优化，该框架在标准数据集上表现出色，有望在各种实际应用中找到用途，如虚拟形象动画、情感计算和人机交互等。
...
<details>
  <summary>Details</summary>
Motivation: 印度等新兴经济体的快速机动化导致严重的执法不对称，2023年记录违规超过1100万起，而警力密度仅为每4000辆车一名警官。传统监控和人工罚单无法应对这种规模，需要自主、协作且节能的边缘AI感知基础设施。
...
Result: 系统在9.6W功耗下维持28-30FPS推理，实现97.7%的违规检测准确率和84.9%的OCR精度，涵盖信号跳跃、斑马线违规、逆行、非法掉头和超速五类违规。相比其他模型，获得10.7%的平均精度提升和1.4倍的能效比改进。

Conclusion: 路边边缘AI分析可以增强协作感知和主动道路安全管理，系统通过V2X协议向联网车辆和智能交通系统后端发布CAM和DENM类型标准化安全事件，展示了在IEEE智能车辆生态系统中的应用潜力。
...
TL;DR: 论文提出了ROAD基准测试，用于评估LiDAR点云分类在同时面临域偏移和标签演化时的鲁棒性，填补了3D持续学习研究的空白。


<details>
  <summary>Details</summary>
Motivation: 3D感知系统在实际应用中需要适应不断变化的对象定义和传感器域，但相比2D视觉，3D点云感知中的持续学习和迁移学习研究不足，特别是在同时面临域偏移和标签演化的情况下。

Method: 提出ROAD基准测试，专门针对LiDAR物体分类，考虑域偏移以及三种标签演化形式：类别分裂、类别扩展和类别插入。使用大规模数据集（Waymo、NuScenes、Argoverse2），评估零样本迁移、线性探测和持续学习方法。

Result: 评估发现现有方法在现实偏移下存在局限性，为未来鲁棒3D感知研究建立了强基线。

Conclusion: ROAD基准测试填补了3D持续学习研究的空白，揭示了现有方法在现实场景中的不足，为开发更鲁棒的3D感知系统提供了重要评估框架。
...
TL;DR: Motive是一个基于梯度的运动归因框架，用于视频生成模型，能够识别影响运动质量的数据片段，并指导数据筛选以提升视频的时间一致性和物理合理性。


<details>
  <summary>Details</summary>
Motivation: 尽管视频生成模型发展迅速，但数据如何影响运动质量仍不清楚。现有方法主要关注视觉外观，缺乏专门针对运动动态的数据归因框架。

Method: 提出Motive框架：1）使用运动加权损失掩码将时间动态与静态外观分离；2）基于梯度的数据归因方法，可扩展到大型高质量视频数据集和模型；3）识别对运动有强影响的微调片段。

Result: 在文本到视频模型中，Motive能识别显著影响运动的片段，指导数据筛选。使用Motive选择的高影响力数据，在VBench上提升了运动平滑度和动态程度，相比预训练基础模型获得74.1%的人类偏好胜率。

Conclusion: Motive是首个专门针对视频生成模型中运动而非视觉外观的归因框架，能够有效指导数据筛选以改善视频的时间一致性和物理合理性。
...
<details>
  <summary>Details</summary>
Motivation: 2D拖拽编辑已取得显著进展，但3D场景的直观拖拽编辑仍然面临挑战。现有3D编辑方法在几何相关内容编辑方面存在局限性，需要一种能够高效、直观编辑真实3D场景的方法

Method: 提出基于3D高斯溅射的变形引导实现一致的几何修改，结合扩散引导进行内容校正和视觉质量增强，采用渐进式编辑策略支持激进3D拖拽编辑

Result: 方法支持多种编辑类型（运动变化、形状调整、修复、内容扩展），在几何相关3D内容编辑方面达到最先进性能，单RTX 4090 GPU上仅需10-20分钟

Conclusion: 3DGS-Drag填补了基于变形的3D编辑和基于2D编辑的3D编辑方法之间的空白，为真实3D场景提供了高效、直观的拖拽编辑能力
...
Method: 提出了两种特征组合流程来整合Gabor和GLCM特征，以缩小可能的人类注视区域范围。这些流程在模拟数字乳腺断层合成图像上进行评估，并与基于阈值的模型观察者进行比较。

Result: 结果显示，提出的流程预测的注视候选区域与基于阈值的模型观察者在定性上一致。观察到GLCM均值与Gabor特征响应之间存在强相关性，表明这些特征尽管公式不同但编码了相关的图像信息。人类观察者的眼动数据进一步表明预测的注视区域与早期注视行为具有一致性。

Conclusion: 这些发现强调了结合结构和纹理特征对于建模视觉搜索的价值，并支持开发感知信息驱动的观察者模型。特征组合方法能够有效预测人类早期视觉搜索行为。
...
### [8] [Zero-Shot Distracted Driver Detection via Vision Language Models with Double Decoupling](https://arxiv.org/abs/2601.08467)
*Takamichi Miyata,Sumiko Miyata,Andrew Morris*

Main category: cs.CV

TL;DR: 提出一种解耦驾驶员外观特征与行为特征的视觉语言模型框架，通过消除外观变化对零样本分类的影响，提高分心驾驶检测的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的分心驾驶检测方法在真实场景中表现不佳，主要原因是模型将驾驶员的外观特征（如服装、年龄、性别）与行为特征纠缠在一起，导致分类决策更多基于"驾驶员是谁"而非"驾驶员在做什么"。

Method: 提出主体解耦框架：1）提取驾驶员外观嵌入表示；2）在零样本分类前从图像嵌入中移除外观影响，强调与分心驾驶相关的证据；3）通过Stiefel流形上的度量投影对文本嵌入进行正交化，在保持原始语义的同时提高可分离性。

Result: 实验表明该方法在多个基准测试中相比现有基线方法取得了一致的性能提升，证明了该框架在实际道路安全应用中的潜力。

Conclusion: 通过解耦驾驶员外观特征与行为特征，能够显著提高视觉语言模型在分心驾驶检测任务中的性能，为解决真实场景中的驾驶员行为识别问题提供了有效方案。
...
TL;DR: 开发了一种新的统计理想观察者模型，通过设置最小可提取图像特征的阈值来进行整体视觉搜索处理，减少自由参数并缩小系统规模。


<details>
  <summary>Details</summary>
Motivation: 开发一个更高效的视觉搜索模型，通过减少参数来简化系统，为医学图像感知、计算机视觉等领域提供更好的性能基准和特征评估方法。

Method: 提出基于统计的理想观察者模型，采用整体视觉搜索处理，通过在最小可提取图像特征上设置阈值来减少自由参数。

Result: 开发了一个参数更少、系统更紧凑的视觉搜索模型框架，为多个应用领域提供了新的分析工具。

Conclusion: 该新型统计理想观察者模型通过阈值化最小可提取特征实现了参数减少和系统简化，在医学图像感知、计算机视觉、目标检测等多个领域具有广泛应用前景。
...
TL;DR: CASHEW和CASHEW-RL：两种提升视觉语言模型多步推理稳定性的方法，通过聚合多个推理轨迹并进行视觉验证，显著提升多模态理解任务性能


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态理解和推理任务中表现良好，但其多步推理过程不稳定，同一输入多次采样会产生不同的推理轨迹和不一致的最终预测，需要解决推理稳定性问题

Method: 提出两种互补方法：1) CASHEW推理时框架，通过迭代聚合多个候选推理轨迹形成更高质量推理链，使用显式视觉验证过滤幻觉步骤，将推理基于视觉证据；2) CASHEW-RL学习变体，通过Group Sequence Policy Optimization训练，使用复合奖励鼓励基于最小充分视觉证据的正确答案，并根据任务难度自适应分配推理努力

Result: 在13个图像理解、视频理解和视频推理基准测试中显示显著性能提升，包括ScienceQA上提升23.6个百分点，EgoSchema上提升8.1个百分点

Conclusion: CASHEW和CASHEW-RL通过聚合推理轨迹和视觉验证有效解决了视觉语言模型多步推理的不稳定性问题，实现了更稳健的推理性能
...
TL;DR: 提出了一种无需训练的视觉零样本异常检测框架DIVAD，通过预训练DDIM模型的反向过程重构图像，利用输入图像与重构图像的差异来检测和定位异常，无需额外模态或细粒度提示。


<details>
  <summary>Details</summary>
Motivation: 当前零样本异常检测方法要么依赖语言等多模态信息生成细粒度提示进行定位，要么只能进行图像级分类缺乏空间精度。需要一种无需额外模态、无需训练、仅依赖视觉信息的零样本异常检测方法。

Method: 基于预训练的DDIM模型，将输入图像反向编码到潜在空间，然后从固定的中间时间步开始正向去噪重构图像。由于扩散模型仅在正常数据上训练，重构结果会呈现正常外观。通过计算输入图像与重构图像的差异来检测和定位异常。

Result: 在VISA数据集上取得了最先进的性能，展示了强大的异常定位能力，无需辅助模态，摆脱了对提示词的依赖。

Conclusion: 提出了一种简单有效的无需训练视觉零样本异常检测框架，通过扩散模型反向过程实现异常检测和定位，为减少提示词依赖的零样本异常检测研究提供了新方向。
...
TL;DR: 本文提出了一种基于概念多样性的高效图像输入选择方法CBD，结合Vision-Language Models和Margin不确定性度量，在保持计算效率的同时显著提升DNN微调性能。


<details>
  <summary>Details</summary>
Motivation: DNN微调需要标注新收集的输入数据，这一过程成本高且耗时。现有的多样性选择方法虽然有效，但计算密集且难以扩展到大规模输入集，限制了实际应用。

Method: 提出概念多样性CBD度量，利用Vision-Language Models高效计算图像输入的多样性；构建混合输入选择方法，结合CBD和Margin不确定性度量；在多种DNN模型、输入集和选择预算下进行全面评估。

Result: CBD与几何多样性GD高度相关但计算时间大幅减少；CBD-based选择方法在所有基线方法中表现最佳，能有效指导输入选择提升DNN模型性能；计算效率接近简单不确定性方法，在ImageNet等大规模数据集上仍保持高效。
...
TL;DR: FigEx2是一个视觉条件化框架，能够从科学复合图中定位面板并生成面板级描述，通过噪声感知门控融合模块和强化学习策略提升性能，在跨学科领域表现出色。
...
Result: 在检测任务上达到0.726 mAP@0.5:0.95，在METEOR和BERTScore上显著优于Qwen3-VL-8B模型，并展现出对分布外科学领域的零样本迁移能力。

Conclusion: FigEx2能够有效解决科学复合图的面板定位和描述生成问题，通过创新的融合模块和优化策略实现了卓越性能，具有很好的跨领域泛化能力。
...
TL;DR: 首个基于视觉-语言引导的生物医学图像伪造生成与检测框架，结合扩散模型和视觉语言提示，实现语义可控的伪造操作，并建立大规模基准数据集和检测模型。


<details>
  <summary>Details</summary>
Motivation: 生物医学出版物中的科学图像篡改对研究完整性和可重复性构成威胁，但现有自然图像取证方法难以处理生物医学图像特有的领域伪影、复杂纹理和非结构化布局等挑战。

Method: 提出视觉-语言引导框架：1）结合扩散合成与视觉语言提示实现语义可控的伪造生成（复制、拼接、区域移除）；2）建立Rescind大规模基准数据集；3）开发Integscan检测框架，集成注意力增强视觉编码与提示条件语义对齐的结构化状态空间模型；4）引入视觉-语言模型验证循环确保语义保真度。

Result: 在Rescind和现有基准上的广泛实验表明，Integscan在伪造检测和定位方面均达到最先进性能，为自动化科学完整性分析奠定了坚实基础。

Conclusion: 该研究首次提出视觉-语言引导的生物医学图像伪造生成与检测框架，通过结合扩散合成、语义控制和验证机制，有效解决了生物医学图像取证的独特挑战，推动了科学诚信分析的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的图像数据经常受到噪声和失真的影响，而传统CNN训练通常使用高质量数据。研究旨在探索通过有控制地在训练数据中引入噪声来提升模型对真实世界噪声的鲁棒性。

Method: 使用CIFAR-10数据集，评估三种常见噪声类型（高斯噪声、椒盐噪声、高斯模糊）在不同强度和训练集污染水平下的影响。采用Resnet-18模型进行实验，分析不同噪声比例对模型性能的影响。

Result: 实验结果表明，在训练中仅加入10%的噪声数据就足以显著降低测试损失，并在完全噪声测试条件下提高准确率，同时对干净数据的性能影响最小。

Conclusion: 有策略地暴露于噪声可以作为一种简单而有效的正则化方法，在传统数据清洁度和现实世界鲁棒性之间提供实用的权衡。
...
<details>
  <summary>Details</summary>
Motivation: 基于深度学习的医学图像分割在临床诊断中至关重要，但在少样本场景下因标注数据稀缺而面临挑战。虽然DINOv3等自监督基础模型在自然图像上表现出强大的特征提取能力，但直接应用于医学图像存在领域差异问题。
...
TL;DR: 提出基于扩散模型的领域特定合成数据集自动生成管道，解决预训练模型与实际部署环境间的分布偏移问题
...
Method: 三阶段框架：1) 通过可控修复合成目标对象到领域特定背景中；2) 多模态评估验证生成结果（对象检测、美学评分、视觉语言对齐）；3) 使用用户偏好分类器捕捉主观选择标准

Result: 能够高效构建高质量、可部署的数据集，减少对真实世界数据收集的依赖

Conclusion: 提出的自动化管道为领域特定合成数据生成提供了有效解决方案，有助于弥合预训练模型与实际部署环境之间的差距
...
TL;DR: PathoGen是一种基于扩散模型的生成方法，用于在良性组织病理学图像中可控地生成高保真病变区域，解决罕见病理和疾病亚型标注数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 组织病理学诊断AI模型开发面临专家标注病变数据稀缺的严重限制，特别是对于罕见病理和代表性不足的疾病亚型。现有数据增强方法无法生成足够真实的病变形态，难以保持组织病理学组织特有的复杂空间关系和细胞结构。

Method: 提出PathoGen，一种基于扩散模型的生成模型，能够在良性组织病理学图像中实现可控、高保真的病变修复。该方法利用扩散模型的迭代细化过程，合成具有自然组织边界、保留细胞结构和真实染色特征的病变。

Result: 在肾脏、皮肤、乳腺和前列腺病理四个不同诊断挑战的数据集上验证。定量评估表明PathoGen在图像保真度和分布相似性方面优于条件GAN和Stable Diffusion等最先进的生成基线。使用PathoGen合成的病变增强训练集可提高下游分割性能，特别是在数据稀缺情况下优于传统几何增强方法。
...
TL;DR: 本文提出SSP框架，通过光学流和文本提示改进音频-视觉语义分割，在传统AVS任务基础上增加了语义理解能力，在多个场景下实现更精确的分割效果。


<details>
  <summary>Details</summary>
Motivation: 音频-视觉语义分割(AVSS)需要在音频-视觉分割(AVS)基础上增加语义理解能力，不仅要识别发声物体的像素位置，还要理解场景语义。现有方法将任务分解为两个子任务，但需要更有效的多模态融合策略来处理运动物体和静止发声物体的挑战。

Method: 提出SSP框架：1) 使用光学流捕捉运动动态，为分割提供时间上下文；2) 引入两种文本提示（物体类别和场景描述）处理静止发声物体；3) 设计视觉-文本对齐模块(VTA)实现跨模态整合；4) 采用后掩码技术训练模型学习光学流图。

Result: 实验结果表明SSP框架优于现有的AVS方法，能够提供高效且精确的分割结果，特别是在处理运动物体和静止发声物体时表现出色。

Conclusion: SSP框架通过整合光学流和文本提示，有效解决了音频-视觉语义分割中的多模态融合挑战，为AVSS任务提供了创新的解决方案，在语义理解和分割精度方面都取得了显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法严重依赖零样本预测作为伪标签进行自训练，但在分布偏移下不可靠，主要受两个基本限制：1）模态间隙：分布偏移导致视觉和文本模态之间存在差距，使跨模态关系不准确；2）视觉干扰：视觉嵌入编码了丰富但与任务无关的噪声，在分布偏移下常常淹没任务特定语义。

Method: SubTTA通过对齐两种模态的语义子空间来增强零样本预测：1）提取两种模态的主子空间，通过最小化弦距离将视觉流形对齐到文本语义锚点，以弥合模态间隙；2）将对齐的视觉特征投影到任务特定的文本子空间，通过将视觉嵌入约束在有效语义范围内来过滤任务无关噪声；3）在纯化空间上执行标准TTA以细化决策边界。

Result: 在各种基准测试和VLM架构上的广泛实验表明，SubTTA的有效性，相比最先进的TTA方法平均提高了2.24%。
...
TL;DR: 该论文提出了一种基于实例对齐字幕的可解释视频异常检测方法，通过将文本描述与具体对象实例及其外观运动属性关联，实现可验证的空间定位解释。
...
Method: 引入实例对齐字幕框架，将每个文本声明链接到特定对象实例及其外观和运动属性，能够捕捉"谁导致异常"、"每个实体在做什么"、"影响谁"以及"解释定位在哪里"等关键信息。

Result: 为8个广泛使用的VAD基准数据集进行标注，并扩展了360度第一人称数据集VIEW360，新增868个视频、8个地点和4种新异常类型，创建了VIEW360+综合测试平台。实验表明该方法揭示了当前基于LLM和VLM方法的显著局限性。

Conclusion: 提出的实例级空间定位字幕方法为可信和可解释的异常检测提供了稳健的基准，能够实现可验证和可操作的推理，推动未来可解释VAD研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 火箭发射过程中的极端成像条件（如密集颗粒雾霾、超过120dB的亮度变化）严重影响了关键机械参数（羽流流场、冲击波结构、喷嘴振荡）的光学测量精度

Method: 采用硬件-算法协同设计：1）定制空间变化曝光传感器单次拍摄获取多曝光数据；2）物理感知去雾算法动态估计雾霾密度、区域自适应光照优化、多尺度熵约束融合分离雾霾与场景辐射

Result: 在真实发射图像和受控实验中验证，该框架在恢复羽流和发动机区域物理准确视觉信息方面表现出优越性能

Conclusion: 为提取关键机械参数（粒子速度、流动不稳定频率、结构振动）提供可靠图像基础，支持极端航空航天环境下的精确定量分析
...
TL;DR: 提出SISTA框架，通过多级对齐（图像-报告和补丁-单词）改进医学视觉语言预训练，避免传统对比学习中因医学数据相似性导致的假阴性问题。
...
Method: 提出SISTA框架，包含两个层面的语义对齐：1) 图像-报告层面：通过引入报告间相似性来消除假阴性；2) 补丁-单词层面：有效对齐图像补丁与相关单词标记。

Result: 实验结果表明，该框架在不同数据集的下游任务（图像分类、图像分割、目标检测）中提高了迁移性能。特别是在有限标注数据的细粒度任务中取得了显著改进。

Conclusion: SISTA框架通过多级语义对齐改进了医学视觉语言预训练，解决了传统对比学习中的假阴性问题，在多个下游任务中表现出优越性能，尤其在数据有限的情况下效果显著。
...
TL;DR: 该技术报告介绍了在RoboSense2025挑战赛中获得奖项的跨平台3D目标检测解决方案，基于PVRCNN++框架，通过数据增强和自训练策略提升跨域泛化能力，在挑战赛中取得第三名。
...
Method: 基于PVRCNN++框架，结合点云和体素特征，采用定制化数据增强和基于伪标签的自训练策略来缩小域间差距，提升跨平台泛化性能。

Result: 在RoboSense2025挑战赛中取得第三名：第一阶段目标域Car类别3D AP达到62.67%，第二阶段目标域Car和Pedestrian类别分别达到58.76%和49.81%。

Conclusion: 通过结合PVRCNN++框架、定制化数据增强和自训练策略，有效提升了3D目标检测模型的跨平台泛化能力，在挑战赛中取得了优异成绩。
...
<details>
  <summary>Details</summary>
Motivation: 当前动态3D场景理解系统缺乏类似人类的认知能力，无法有效处理多次访问同一场景时的知识积累和更新。需要一种能够模拟人类认知过程，具备记忆、检索和更新能力的系统。

Method: 集成三个核心能力：1）多阶段运动线索框架识别动态对象；2）认知映射系统存储、回忆和更新多次访问的静态场景；3）因子图优化策略精炼相机位姿。利用深度和相机位姿先验识别动态区域，将静态元素与记忆库匹配。

Result: 在视频深度估计、相机位姿重建和3D映射任务上达到最先进性能，有效支持跨扩展序列和多次访问的连续场景理解。

Conclusion: CogniMap3D通过模拟人类认知过程，实现了高效的动态3D场景理解与重建，为持续场景理解提供了创新解决方案。
...
TL;DR: 本文提出一种基于二阶高斯方向导数滤波的高分辨率角点检测方法，能够准确检测相邻角点，在定位误差、图像模糊变换鲁棒性、图像匹配和3D重建方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有角点检测方法（如Zhang等人的方法）使用简单角点模型存在理论缺陷，相邻角点的灰度信息会相互影响，导致无法准确检测高分辨率图像中的相邻角点。

Method: 使用二阶高斯方向导数滤波器对两种典型高分辨率角点模型（END型和L型）进行平滑处理，推导出这两种模型的SOGDD表示，发现高分辨率角点的多个特征，并提出基于高斯滤波尺度选择的新高分辨率角点检测方法。

Result: 实验验证表明，所提方法在定位误差、图像模糊变换鲁棒性、图像匹配和3D重建方面优于现有最先进方法，能够准确检测相邻角点。

Conclusion: 提出的基于SOGDD的高分辨率角点检测方法有效解决了相邻角点相互影响的问题，在多个计算机视觉任务中表现出优越性能。
...
TL;DR: 该研究构建了GI-Bench基准测试，系统评估多模态大语言模型在胃肠内窥镜临床工作流中的表现，发现顶级模型在诊断推理上可与初级内镜医师媲美，但在空间定位和事实准确性方面仍存在瓶颈。
...
Method: 构建GI-Bench基准测试，涵盖20种细粒度病变类别；评估12个MLLM模型在五阶段临床工作流中的表现：解剖定位、病变识别、诊断、发现描述和管理；使用Macro-F1、mIoU和多维度Likert量表将模型性能与3名初级内镜医师和3名住院医师进行比较。

Result: Gemini-3-Pro达到最先进性能；在诊断推理方面，顶级模型（Macro-F1 0.641）优于住院医师（0.492），与初级内镜医师（0.727）相当（p>0.05）；但存在"空间定位瓶颈"：人类病变定位（mIoU >0.506）显著优于最佳模型（0.345；p<0.05）；定性分析揭示"流畅性-准确性悖论"：模型生成报告的语言可读性优于人类（p<0.05），但事实准确性显著较低（p<0.05），存在"过度解释"和视觉特征幻觉。

Conclusion: MLLM在胃肠内窥镜临床工作流中展现出潜力，特别是在诊断推理方面可与初级医师媲美，但在空间定位和事实准确性方面仍需改进；GI-Bench建立了动态排行榜以追踪模型在临床内窥镜中的演进表现。
...
### [28] [Route, Retrieve, Reflect, Repair: Self-Improving Agentic Framework for Visual Detection and Linguistic Reasoning in Medical Imaging](https://arxiv.org/abs/2601.08192)
...
TL;DR: R^4是一个用于医学影像分析的智能体框架，通过四个协调的智能体（路由、检索、反思、修复）来提升视觉语言模型在临床图像解读中的可靠性、可控性和空间定位能力。


<details>
  <summary>Details</summary>
Motivation: 当前医学影像分析主要依赖单一的黑盒视觉语言模型，缺乏对推理过程、安全性和空间定位的控制。需要一种能够提供更可靠、可控且具有空间基础的临床图像解读工具。

Method: 提出R^4框架，包含四个协调智能体：1) Router根据图像、患者病史和元数据配置任务和专业化提示；2) Retriever使用示例记忆和pass@k采样生成自由文本报告和边界框；3) Reflector针对关键临床错误模式（否定、侧位、无支持声明、矛盾、缺失发现和定位错误）对草稿-框对进行批判；4) Repairer在目标约束下迭代修订叙述和空间输出，并为未来案例整理高质量示例。

Result: 在胸部X光分析中，使用多个现代VLM骨干进行评估，R^4相比强大的单一VLM基线，将LLM-as-a-Judge分数提升了约+1.7-+2.5分，mAP50提升了+2.5-+3.5个绝对百分点，且无需基于梯度的微调。

Conclusion: 智能体路由、反思和修复可以将强大但脆弱的VLM转变为更可靠、更好基础的临床图像解读工具，展示了智能体框架在提升医学影像分析系统性能和可控性方面的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 多站点脑MRI数据聚合可以增强深度学习模型训练，但也会引入由站点特定变化（如扫描仪厂商、采集参数和成像协议差异）导致的非生物异质性，这会削弱模型的泛化能力。现有方法依赖有限的配对数据或无法有效分离风格与解剖结构，且大多只处理单序列协调，限制了在实际多序列MRI场景中的应用。

Method: MMH采用两阶段框架：1）基于扩散的全局协调器，使用风格无关的梯度条件将MR图像映射到序列特定的统一域；2）目标特定的微调器，将全局对齐图像适配到期望的目标域。使用三平面注意力BiomedCLIP编码器聚合多视图嵌入来表征体积风格信息，实现无需配对数据的图像风格与解剖结构的显式分离。

Result: 在4,163个T1和T2加权MRI上的评估显示，MMH在图像特征聚类、体素级比较、组织分割以及下游年龄和站点分类任务中，均优于现有最先进的方法。

Conclusion: MMH提供了一个有效的多站点多序列脑MRI协调框架，能够显著减少站点效应，提高模型的泛化能力，适用于实际临床和研究场景。
...
TL;DR: MobiDiary是一个从IMU和Wi-Fi物理信号生成日常活动自然语言描述的系统，解决了传统视觉系统隐私问题和标签限制，通过统一传感器编码器和Transformer解码器实现跨模态泛化。


<details>
  <summary>Details</summary>
Motivation: 智能家居中的人类活动识别对健康监测和辅助生活至关重要。传统视觉系统存在隐私问题和环境限制（如遮挡），而现有方法通常局限于预定义标签，缺乏表达性的人类可读描述。

Method: 提出MobiDiary框架，使用统一传感器编码器连接物理信号与语言描述。采用基于补丁的机制捕捉局部时间相关性，集成异构位置嵌入统一不同传感器的空间上下文。这些统一信号标记输入到基于Transformer的解码器，通过自回归机制逐词生成连贯的动作描述。

Result: 在多个公共基准测试（XRF V2、UWash和WiFiTAD）上评估，MobiDiary在跨模态泛化方面表现优异，在字幕生成指标（BLEU@4、CIDEr、RMC）上达到最先进性能，在连续动作理解方面优于专门基线。

Conclusion: MobiDiary成功地从异构物理信号生成自然语言活动描述，解决了隐私和环境限制问题，为智能家居中的活动识别提供了更自然、表达性更强的解决方案。
...
TL;DR: 提出FUME方法，利用双气体光学成像和深度学习技术，通过检测奶牛瘤胃中CO2和CH4排放模式，实现非侵入式瘤胃酸中毒检测，在体外条件下达到高精度分类和分割性能。


<details>
  <summary>Details</summary>
Motivation: 瘤胃酸中毒是奶牛常见代谢疾病，造成重大经济损失和动物福利问题。现有诊断方法依赖侵入式pH测量，难以实现连续监测。需要开发非侵入、可扩展的监测技术。

Method: 提出FUME深度学习框架，利用红外相机捕捉CO2和CH4排放模式，采用轻量级双流架构，包含权重共享编码器、模态特定自注意力机制和通道注意力融合，联合优化气体羽流分割和健康状态分类。

Result: 在首个双气体OGI数据集（8,967标注帧，6个pH水平）上，FUME达到80.99% mIoU分割精度和98.82%分类准确率，仅使用1.28M参数和1.97G MACs，计算成本降低10倍，优于现有方法。

Conclusion: 研究证明了基于气体排放的牲畜健康监测可行性，CO2提供主要判别信号，双任务学习对优化性能至关重要，为实用的体外酸中毒检测系统奠定了基础。
...
TL;DR: 本研究比较了基于EVA-ViT视觉Transformer的多模态方法与LlaMA或ChatGPT LLM在减少幻觉问题和检测胸部X光图像疾病方面的表现，发现文本RAG能有效减少幻觉，图像RAG能提高预测置信度和校准，GPT LLM表现优于Llama模型。
...
Method: 使用NIH胸部X光图像数据集，比较三种方法：基于图像的RAG（使用KNN方法）、基于文本的RAG（利用外部知识信息）、以及基线方法。采用EVA-ViT作为图像编码器，结合LlaMA或ChatGPT LLM进行多模态分析。

Result: 文本RAG能有效减少幻觉问题；图像RAG提高了预测置信度和校准；GPT LLM在性能、幻觉率和预期校准误差方面均优于Llama模型。
...
TL;DR: 本文提出基于事件分割和置信度估计的零样本ADL识别方法，相比传统时间分割方法在复杂数据集上表现更好，甚至超越有监督方法。
...
Result: 事件分割方法在复杂现实数据集上持续优于时间分割的LLM方法，甚至超越有监督数据驱动方法；置信度度量能有效区分正确与错误预测。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频-LLM的推测解码方法主要关注剪枝冗余视觉token，但无法达到文本LLM的加速效果。主要问题包括：(1) 剪枝策略未能充分保留视觉语义token，降低了草稿质量和接受率；(2) 即使大幅剪枝（如90%），草稿模型的剩余推理成本仍限制整体加速。

Method: HIPPO提出：(1) 语义感知token保留方法，融合全局注意力分数和局部视觉语义，在高剪枝率下保留语义信息；(2) 视频并行推测解码算法，解耦并重叠草稿生成和目标验证阶段。

Result: 在四个视频-LLM和六个基准测试上的实验表明，HIPPO相比原始自回归解码实现了高达3.51倍的加速。

Conclusion: HIPPO通过语义感知token保留和并行化设计，有效解决了视频-LLM推测解码中的关键限制，实现了显著的推理加速。
...
TL;DR: 该研究引入KidVis基准测试，基于人类视觉发展理论评估MLLMs的基本视觉能力，发现即使最先进的GPT-5在儿童已掌握的六种基础视觉能力上表现远逊于人类儿童，且存在"缩放定律悖论"。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在高级推理任务上表现出色，但研究者想知道它们是否具备与人类直觉相当的基础视觉原始能力。通过借鉴人类视觉发展理论，研究旨在评估MLLMs在低语义依赖视觉任务上的基本能力。

Method: 引入KidVis基准测试，将视觉智能分解为6-7岁儿童已具备的六种原子能力：专注力、追踪、辨别、记忆、空间和闭合能力，包含10类低语义依赖视觉任务。评估20个最先进的MLLMs，并与人类生理基线（儿童表现）进行对比。

Result: 人类儿童平均得分接近完美（95.32），而最先进的GPT-5仅得67.33，存在显著性能差距。研究观察到"缩放定律悖论"：单纯增加模型参数无法线性提升这些基础视觉能力。

Conclusion: 当前MLLMs尽管具备强大的推理能力，但缺乏实现广义视觉智能所需的基本生理感知原始能力。这揭示了现有模型在基础视觉处理方面的根本局限性。
...
TL;DR: 提出M3SR架构，通过多尺度多感知Mamba解决高光谱图像重建中的空间感知单一和特征提取单一问题，在降低计算成本的同时超越现有方法。


<details>
  <summary>Details</summary>
Motivation: Mamba架构在低层视觉任务中表现出色，但在高光谱重建中面临两个挑战：1）单一空间感知限制了全面理解高光谱图像的能力；2）单一尺度特征提取难以捕捉高光谱图像中的复杂结构和精细细节。
...
Conclusion: M3SR通过多尺度多感知Mamba架构成功解决了高光谱重建中的关键问题，在性能和效率上都取得了显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在图像质量评估任务中表现出色，但达到最先进性能通常需要计算昂贵的微调方法。作者希望开发一种无需训练的方法来提升模型性能。

Method: 提出IQARAG框架，包含三个关键阶段：检索特征提取、图像检索、整合与质量分数生成。通过检索语义相似但质量不同的参考图像及其平均意见分数，为输入图像提供视觉感知锚点。

Result: 在KADID、KonIQ、LIVE Challenge和SPAQ等多个多样化IQA数据集上的实验表明，IQARAG能有效提升LMMs的图像质量评估性能。
...
TL;DR: 提出YOLOBirDrone架构，通过改进的注意力机制和特征融合技术，提高无人机与鸟类的检测和分类准确率，达到约85%的检测精度。


<details>
  <summary>Details</summary>
Motivation: 无人机在商业和国防应用中广泛使用，但也越来越多地被用于针对性攻击，需要开发无人机检测系统。现有的视觉检测系统在区分无人机和小型鸟类方面存在准确率限制。
...
Conclusion: YOLOBirDrone架构有效提高了无人机与鸟类的检测和分类准确率，为解决无人机检测中的挑战性问题提供了有效解决方案。
...
TL;DR: UM-Text是一个统一的多模态模型，通过自然语言指令进行上下文理解和视觉文本编辑，解决了现有方法在保持与参考图像风格一致性方面的不足。
...
Method: 提出UM-Text统一多模态模型，引入视觉语言模型(VLM)处理指令和参考图像，使文本内容和布局能根据上下文信息精心设计。提出UM-Encoder结合各种条件信息的嵌入，组合由VLM根据输入指令自动配置。训练中提出区域一致性损失，在潜在空间和RGB空间为字形生成提供更有效的监督，并设计定制的三阶段训练策略。贡献了UM-DATA-200K大规模视觉文本图像数据集。

Result: 在多个公共基准测试上的广泛定性和定量结果表明，该方法实现了最先进的性能。

Conclusion: UM-Text通过统一的架构有效解决了视觉文本编辑中的风格一致性问题，结合VLM的上下文理解、UM-Encoder的条件信息融合以及专门的训练策略，在生成准确和谐的视觉文本图像方面表现出色。
...
TL;DR: 提出IET（个体化探索Transformer），通过IEA机制让每个token自适应选择注意力候选，在保持计算效率的同时实现更精确的信息聚合，在超分辨率任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer方法在图像超分辨率中虽然性能出色，但计算成本高。大多数方法采用固定分组注意力，忽略了token相似性的内在不对称性，无法实现灵活的自适应注意力计算。
...
Conclusion: IET通过token自适应和非对称注意力设计，在保持计算效率的同时实现了更精确的信息聚合，为Transformer在超分辨率任务中的高效应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型越来越多地部署在自动驾驶和具身AI系统中，可靠的感知对安全语义推理和决策至关重要。虽然最近的VLM在多模态基准测试中表现出色，但它们对现实感知退化的鲁棒性仍然了解不足。

Method: 使用Cityscapes数据集上的语义分割作为代表性感知模块，在受控的上游视觉感知退化条件下系统研究VLM的语义错位。引入感知现实性退化，这些退化仅导致传统分割指标适度下降，但观察下游VLM行为的严重失败。提出一组语言级错位指标来量化这些效应。

Result: 研究结果揭示了像素级鲁棒性与多模态语义可靠性之间的明显脱节，包括幻觉对象提及、安全关键实体的遗漏和不一致的安全判断。即使分割质量下降不大，也会导致VLM出现严重的安全关键错误。

Conclusion: 当前基于VLM的系统存在关键局限性，需要在安全关键应用中明确考虑感知不确定性的评估框架。像素级鲁棒性不能保证多模态语义可靠性。
...
TL;DR: Geo-NVS-w是一个几何感知的新视角合成框架，通过SDF几何表示和几何保持损失，在非结构化野外图像集合中实现高质量渲染，同时降低能耗。
...
Conclusion: Geo-NVS-w是一个稳健的野外新视角合成方法，通过几何感知框架实现了高质量、几何一致的渲染，同时显著降低了计算能耗。
...
TL;DR: LoGo是一个针对地理空间点云的无源无监督域适应框架，通过局部-全局双重共识机制解决跨区域地理模式变化导致的域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 地理空间点云语义分割在遥感应用中至关重要，但不同区域的地理模式差异和数据采集策略导致显著的域偏移，严重降低部署模型的性能。现有域适应方法通常需要源域数据，但由于数据隐私、监管政策和传输限制，这一要求很难满足，因此需要研究无源无监督域适应方法。

Method: 提出LoGo框架，包含三个核心模块：1) 局部层面：类平衡原型估计模块，采用类内独立锚点挖掘策略而非传统全局阈值过滤，确保尾类也能生成鲁棒特征原型；2) 全局层面：基于最优传输的全局分布对齐模块，将伪标签分配建模为全局优化问题，通过全局分布约束纠正头类过度主导问题；3) 双重一致性伪标签过滤机制，仅保留局部多增强集成预测与全局最优传输分配一致的高置信度伪标签进行自训练。

Result: 论文提出的方法有效解决了长尾分布导致的特征崩溃问题，防止模型预测严重偏向多数类，在无源域适应设置下提升了地理空间点云语义分割的性能。

Conclusion: LoGo框架通过局部-全局双重共识机制，成功解决了地理空间点云无源无监督域适应中的关键挑战，包括长尾分布、类不平衡和伪标签噪声问题，为实际部署中的域偏移问题提供了有效解决方案。
...
TL;DR: 开发了一个用于全景X光片上诊断冠周炎的AI辅助系统，包含解剖定位、病理分类和可解释性功能


<details>
  <summary>Details</summary>
Motivation: 克服在全景X光片上诊断冠周炎的挑战，提高诊断准确性和临床可信度
...
TL;DR: 该论文提出了一种基于BLIP-2的轻量级多模态任务平台，集成YOLO-World和YOLOv8-Seg模型，通过内容感知关键帧采样和统一提示优化方案，解决无人机边缘设备计算资源有限与视觉语言模型高计算成本之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 无人机在复杂场景中需要实时视觉理解和交互能力，但大型视觉语言模型的高计算成本与无人机边缘设备的有限计算资源之间存在矛盾，这限制了无人机应用的发展。

Method: 1. 将BLIP-2与YOLO模型深度融合，利用YOLO的精确感知结果进行目标检测和实例分割；2. 设计基于K-Means聚类的智能关键帧采样机制，结合时间特征拼接；3. 实现统一的多任务适应提示优化方案，将YOLO的结构化事件日志作为上下文信息注入BLIP-2输入。

Result: 该平台扩展了BLIP-2在无人机应用中的多任务能力，无需对无人机数据进行任务特定微调，能够有效处理视频级交互任务，生成准确且上下文相关的输出。

Conclusion: 提出的轻量级多模态任务平台成功解决了无人机边缘设备计算资源有限的问题，通过智能集成和优化策略，实现了高效的实时视觉理解和交互能力。
...
TL;DR: SPARK是一个自标定的实时多相机3D重建框架，联合处理点云融合和相机外参不确定性，通过几何感知在线外参估计和置信度驱动的点云融合策略，在动态场景中实现稳定重建。


<details>
  <summary>Details</summary>
Motivation: 现有多相机3D重建方法在多视图融合、相机外参不确定性和大规模相机系统可扩展性方面存在困难，需要一种能够同时处理这些问题并实现实时重建的解决方案。

Method: SPARK包含两个核心组件：1) 几何感知在线外参估计模块，利用多视图先验并强制跨视图和时间一致性实现稳定自标定；2) 置信度驱动的点云融合策略，在像素和点级别建模深度可靠性和可见性，抑制噪声和视图依赖不一致性。

Result: 在真实世界多相机系统上的实验表明，SPARK在外参精度、几何一致性、时间稳定性和实时性能方面优于现有方法，证明了其有效性和大规模多相机3D重建的可扩展性。

Conclusion: SPARK通过联合处理点云融合和外参不确定性，实现了稳定、实时的大规模多相机3D重建，解决了现有方法的局限性，适用于动态场景和大型相机系统。
...
TL;DR: 提出MMLGNet多模态框架，通过CLIP等视觉语言模型将高光谱影像和LiDAR等异构遥感模态与自然语言语义对齐，使用简单的CNN编码器在基准数据集上超越现有多模态视觉方法。


<details>
  <summary>Details</summary>
Motivation: 随着多模态地球观测数据的增加，需要能够有效融合光谱、空间和几何信息并实现语义级理解的方法。现有方法在异构遥感模态与语言语义对齐方面存在不足。

Method: 采用模态特定编码器，通过双向对比学习将视觉特征与手工制作的文本嵌入在共享潜在空间中对齐，借鉴CLIP的训练范式，桥接高维遥感数据与语言引导解释之间的差距。

Result: 在两个基准数据集上，MMLGNet使用简单的CNN编码器实现了强大性能，超越了多个已建立的多模态视觉方法，证明了语言监督的显著优势。

Conclusion: 提出的MMLGNet框架成功将异构遥感模态与语言语义对齐，展示了语言监督在多模态遥感数据分析中的重要性，为遥感数据的语义级理解提供了有效解决方案。
...
Method: 提出三模块方法：1)自动块提取模块从源视频帧提取局部块；2)重演模块生成对应的目标局部块；3)权重估计模块计算每帧动画参数以创建完整面部动画序列

Result: 大量实验表明，该方法能成功将源面部表情的语义含义转移到面部特征比例变化较大的风格化角色上

Conclusion: 提出的局部块重定向方法能有效处理风格化3D角色的面部动画转移，保持原始表演的语义表达
...
TL;DR: 提出CardiacMind框架，通过心脏推理模板和强化学习奖励机制，提升多模态大语言模型在超声心动图诊断中的推理能力，实现类似心脏病专家的思维模式。


<details>
  <summary>Details</summary>
Motivation: 现有超声心动图基础模型未能有效捕捉定量测量与临床表现之间的关系，而医学推理多模态大语言模型需要昂贵详细的推理路径构建，且难以将超声心动图先验知识直接融入推理过程。

Method: 提出心脏推理模板（CRT）提供复杂心脏疾病的逐步规范诊断流程，简化推理路径构建；开发CardiacMind强化学习方案，包含三个新奖励：程序数量奖励（PQtR）、程序质量奖励（PQlR）和超声心动图语义奖励（ESR）。

Result: 在15种复杂心脏疾病的多视图超声心动图诊断中性能提升48%，在CardiacNet-PAH上提升5%；用户研究显示93.33%的临床医生同意其推理逻辑类似心脏病专家。

Conclusion: 通过引入类似心脏病专家的思维模式和结构化推理模板，结合创新的强化学习奖励机制，显著提升了多模态大语言模型在超声心动图诊断中的推理能力和临床实用性。
...
TL;DR: 提出NAR方法，一种针对遥感多标签分类中不同类型标签噪声的自适应正则化方法，通过置信度机制区分加性、减性和混合噪声，结合半监督学习框架提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 遥感数据规模扩大导致标注依赖主题产品或众包，这些低成本策略常引入多标签噪声（部分错误标注）。现有方法大多忽视加性、减性和混合噪声的区别，缺乏针对不同噪声类型自适应调整学习行为的机制。

Method: 提出NAR（噪声自适应正则化）方法：1）基于置信度的标签处理机制，动态保留高置信度标签、暂时停用中等置信度标签、通过翻转纠正低置信度标签；2）结合早期学习正则化（ELR）稳定训练，防止过拟合到噪声标签；3）在半监督学习框架中明确区分加性和减性噪声。

Result: 在加性、减性和混合噪声场景下的实验表明，NAR相比现有方法持续提升鲁棒性。在减性和混合噪声下性能提升最显著，表明自适应抑制和选择性纠正噪声监督是有效的噪声鲁棒学习策略。

Conclusion: NAR方法通过明确区分加性和减性噪声，结合置信度机制和早期学习正则化，为遥感多标签分类中的噪声鲁棒学习提供了有效解决方案，特别是在减性和混合噪声场景下表现优异。
...
TL;DR: 本文提出了一种名为静态-动态协作（SDC）的框架，用于解决少样本类增量学习中的稳定性-可塑性困境。该方法将学习过程分为静态保留阶段和动态学习阶段，通过静态记忆保留旧知识，动态投影器适应新类，实现了更好的稳定性与可塑性平衡。


<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习（FSCIL）需要在有限数据下持续识别新类别，面临稳定性-可塑性困境：如何在保留旧知识的同时获取新知识。现有方法难以平衡这一矛盾，需要更有效的解决方案。

Method: 提出静态-动态协作（SDC）框架，将FSCIL分为两个阶段：1）静态保留阶段（SRS）：在基础会话中训练初始模型，保留关键部分作为静态记忆以保持基础旧知识；2）动态学习阶段（DLS）：引入额外的动态投影器，与静态记忆联合训练，利用增量动态类别信息适应新类。

Result: 在三个公共基准数据集和一个真实世界应用数据集上的广泛实验表明，该方法相比其他竞争方法取得了最先进的性能，有效提升了旧知识的保留能力，同时持续适应新类别。

Conclusion: SDC框架通过分离静态保留和动态学习阶段，利用静态记忆和动态投影器的协作，成功解决了FSCIL中的稳定性-可塑性困境，实现了更好的旧知识保留和新类别适应平衡。
...
TL;DR: 该研究开发了一种结合稳健特征选择的放射组学框架，用于预测高级别浆液性卵巢癌患者对新辅助化疗的治疗反应，在不同解剖部位病灶和多种反应指标上取得了良好预测性能。


<details>
  <summary>Details</summary>
Motivation: 高级别浆液性卵巢癌通常诊断时已晚期且伴有广泛腹膜转移，治疗困难。约40%患者对新辅助化疗反应有限，需要非侵入性方法预测治疗反应以指导个体化治疗。

Method: 引入结合稳健特征选择的放射组学框架，采用自动化随机化算法模拟观察者间变异，平衡特征稳健性和预测准确性。使用四种反应指标（CRS、RECIST、体积减少、直径减少），分析不同解剖部位病灶，基于治疗前后CT扫描提取特征并训练模型，在独立队列进行外部验证。
...
Method: 采用MDQF框架：1）RGB和TIR图像使用独立的DETR-like检测器分支；2）每个细化阶段通过查询选择与适配进行跨分支查询融合；3）高质量查询从一个分支传递到另一个分支，排除退化模态影响；4）支持使用非配对RGB或TIR图像单独优化各分支

Result: 实验表明该方法优于现有RGB-T检测器，实现了更好的模态独立性，在极端条件下表现出色

Conclusion: MDQF框架有效平衡了RGB-T检测中的模态互补与分离需求，通过查询融合机制在极端条件下排除退化模态干扰，同时支持非配对数据训练，提高了检测的鲁棒性和灵活性
...
TL;DR: 提出自动化建筑体量生成框架，创建CoMa-20K数据集，基于视觉语言模型进行条件生成任务评估，为数据驱动的建筑设计建立基准


<details>
  <summary>Details</summary>
Motivation: 建筑和城市规划中的概念设计阶段（特别是建筑体量设计）复杂且严重依赖设计师直觉和手动工作，需要自动化解决方案。现有数据驱动方法面临缺乏合适数据集的障碍。

Method: 1. 创建CoMa-20K数据集，包含详细体量几何、经济和规划数据，以及开发场地在城市环境中的视觉表示；2. 将体量生成构建为视觉语言模型的条件任务；3. 评估微调模型和大型零样本模型

Result: 实验揭示了任务的固有复杂性，同时展示了视觉语言模型生成上下文敏感体量方案的潜力。数据集和分析为数据驱动的建筑设计建立了基础基准

Conclusion: 提出的框架和数据集为数据驱动的建筑设计提供了重要基础，突出了未来研究的显著机会，特别是在利用视觉语言模型进行上下文感知的建筑体量生成方面
...
TL;DR: 该论文提出了HazardForge框架和MovSafeBench基准，用于评估视觉语言模型在自动驾驶等移动系统中处理危险场景的能力，特别是针对具有时空动态的异常场景。
...
Method: 提出了HazardForge可扩展流水线，利用图像编辑模型结合布局决策算法和验证模块生成危险场景。基于此构建了MovSafeBench基准，包含7,254张图像和对应的问答对，涵盖13个物体类别，包括正常和异常物体。

Result: 实验表明，视觉语言模型在异常物体条件下的性能显著下降，特别是在需要细微运动理解的场景中性能下降最为明显。

Conclusion: HazardForge和MovSafeBench为评估视觉语言模型在复杂环境中的安全决策能力提供了有效工具，揭示了模型在异常场景处理方面的局限性。
...
TL;DR: 本文提出了一种基于物联网的智能水族箱系统，通过集成ESP32微控制器和多种传感器（pH、TDS、温度、浊度）与执行器（伺服喂食器、水泵），实现实时水质监测和自动控制，显著提高了水族箱维护效率。


<details>
  <summary>Details</summary>
Motivation: 传统水族箱水质监测方法效率低下、劳动密集且容易出错，导致水生环境不理想。需要一种能够连续监测多个参数并自动控制的解决方案来改善水生生物的健康状况。

Method: 开发了基于物联网的智能水族箱系统，采用ESP32微控制器集成pH、TDS、温度和浊度传感器，配合伺服喂食器和水泵执行器。系统架构包括边缘处理能力、通过Blynk物联网平台的云连接，以及具有可配置冷却期的智能警报机制。

Result: 在10升水族箱环境中测试，系统平均传感器准确率达到96%，异常检测响应时间为1.2秒。自动喂食和水循环模块在长期测试中保持97%的运行可靠性，显著减少了人工干预，同时确保了稳定的水生环境。

Conclusion: 研究表明，经济高效的物联网解决方案能够革新水族箱维护，使水生生态系统管理对住宅和商业应用都更加便捷、可靠和高效。
...
TL;DR: EfficientFSL是一个专门为ViT设计的查询式微调框架，在少样本分类任务中实现竞争性性能，同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 大型模型如ViT在少样本分类中表现出色，但微调需要大量GPU内存和训练时间，不适用于低资源场景。需要一种高效的方法来利用预训练模型的知识。

Method: 提出EfficientFSL框架：1) 轻量级可训练Forward Block合成任务特定查询，以查询方式从预训练模型中间表示中提取特征；2) Combine Block融合多层输出增强特征表示；3) Support-Query Attention Block通过调整原型对齐查询集分布来缓解分布偏移。

Result: EfficientFSL在四个域内少样本数据集和六个跨域数据集上实现了最先进的性能，仅需极少的可训练参数，显著降低了计算开销。

Conclusion: EfficientFSL通过查询式微调框架有效解决了ViT在少样本分类中的计算效率问题，在保持高性能的同时大幅减少资源需求，适用于实际应用场景。
...
TL;DR: 本文提出了一种名为约束数据集蒸馏（CD²）的新框架，用于解决少样本类增量学习中的灾难性遗忘问题。该方法通过数据集蒸馏模块和蒸馏约束模块，在增量学习中更有效地保留先前学到的知识。


<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习（FSCIL）面临灾难性遗忘的关键问题。现有方法通常使用外部存储器存储先前知识，并平等对待增量类，无法适当保留先前的重要知识。受知识蒸馏工作的启发，需要一种更好的方法来在增量学习中保留先前学到的知识。

Method: 提出约束数据集蒸馏（CD²）框架，包含两个核心模块：1）数据集蒸馏模块（DDM）：在分类器指导下合成高度浓缩的样本，迫使模型从少量增量样本中学习紧凑的类相关线索；2）蒸馏约束模块（DCM）：引入设计的损失函数来约束先前学习的类分布，更充分地保留蒸馏知识。

Result: 在三个公共数据集上进行了广泛实验，结果表明该方法优于其他最先进的竞争对手，证明了其在少样本类增量学习中的优越性。

Conclusion: CD²框架通过数据集蒸馏和约束机制，有效解决了少样本类增量学习中的灾难性遗忘问题，能够更充分地保留先前学到的知识，在多个数据集上表现出优越性能。
...
TL;DR: VideoHEDGE是一个用于视频问答幻觉检测的模块化框架，通过语义熵等可靠性评分方法，在多个视频VLM模型上实现了优于现有方法的检测性能。
...
Method: 提出VideoHEDGE框架：对视频-问题对生成基准答案和多个高温生成结果，包括原始视频和经过光度、时空扰动的变体；使用NLI或嵌入方法将文本输出聚类为语义假设；通过聚类级概率质量计算三种可靠性评分：语义熵（SE）、RadFlag和视觉增强语义熵（VASE）。

Result: 在SoccerChat基准测试中，使用LLM作为评判者获得二元幻觉标签。在三个7B Video-VLM模型上，VASE始终获得最高的ROC-AUC，尤其是在较大失真预算下；SE和RadFlag通常接近随机水平。嵌入聚类与NLI聚类在检测性能上相当但计算成本更低；领域微调减少了幻觉频率但校准改进有限。

Conclusion: VideoHEDGE框架能有效检测视频问答中的幻觉，VASE评分方法表现最佳，嵌入聚类提供了计算效率高的替代方案，领域微调对校准改善有限。提供了可复现的基准测试库。
...
Method: 提出MoCha框架，仅需单帧任意掩码；引入条件感知RoPE适应多模态输入条件并增强面部身份；采用基于强化学习的后训练阶段；构建三个专门数据集：基于UE5的高保真渲染数据集、当前肖像动画技术合成的表情驱动数据集、现有视频-掩码对增强数据集。

Result: 大量实验表明，该方法显著优于现有最先进方法，在复杂场景下表现出更好的泛化能力和视觉质量。

Conclusion: MoCha通过简化输入要求（仅需单帧掩码）和构建综合训练数据，有效解决了视频角色替换在复杂场景中的泛化问题，为可控视频角色替换提供了创新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统放射组学特征在群体层面预定义，虽然可解释但过于局限，无法捕捉患者特异性变异；端到端深度学习性能好但缺乏可解释性，临床采用需要兼顾准确性和可解释性

Method: 1. 放射组学指纹：动态构建患者特异性特征集，通过图像条件预测器估计特征使用概率，结合全局系数的透明逻辑回归进行分类；2. 健康人格：使用扩散模型重建健康膝关节MRI，为每位患者合成无病理基线，通过病理图像与健康人格的特征对比突出解剖异常

Result: 两种方法在三个临床任务中表现与最先进深度学习模型相当或更优，同时支持多层次可解释性；案例研究展示了这些方法如何促进人类可解释的生物标志物发现和病理定位

Conclusion: 放射组学指纹和健康人格为膝关节MRI自动评估提供了准确且可解释的解决方案，结合了个体特异性和可解释性，支持临床采用
...
TL;DR: SfMamba框架通过引入通道视觉状态空间块和语义一致洗牌策略，解决了源自由域适应中感知场与计算效率的权衡问题，在多个基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 源自由域适应（SFDA）在现实应用中面临数据隐私和存储限制的挑战，现有方法在领域不变特征学习中难以平衡感知场和计算效率。虽然Mamba的选择性扫描机制提供了长程依赖建模的线性复杂度解决方案，但视觉Mamba在捕捉通道频率特征和保持空间鲁棒性方面仍有局限。

Method: 提出SfMamba框架，包含两个核心组件：1）通道视觉状态空间块，通过通道序列扫描实现领域不变特征提取；2）语义一致洗牌策略，在2D选择性扫描中打乱背景补丁序列同时保持预测一致性，以减少错误累积。

Result: 在多个基准测试上的综合评估表明，SfMamba在保持良好参数效率的同时，始终优于现有方法，为SFDA提供了实用的解决方案。

Conclusion: SfMamba通过创新的通道序列扫描和语义一致洗牌策略，有效解决了源自由域适应中的领域对齐和空间鲁棒性问题，在性能和效率方面均表现出色。
...
TL;DR: 本文提出语义正交校准（SoC）方法，通过Huber正则化器在保持语义邻近性的同时实现平滑原型分离，以改进视觉语言模型在测试时提示调优中的校准性能。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在医疗、自动驾驶等关键决策系统中的广泛应用，其不确定性估计的校准变得至关重要。然而，现有的VLM测试时提示调优研究主要关注提升判别性能，而校准维度被严重忽视。当前最先进的方法强制文本提示嵌入完全正交以增强可分性，但作者理论证明这会过度推动语义相关类别分离，导致模型过度自信。

Method: 提出语义正交校准（SoC）方法，使用Huber-based正则化器，在保持语义邻近性的同时实现平滑原型分离。相比先前基于正交性的方法，SoC能够更好地平衡类别可分性和语义关系保留。

Result: 通过全面的实证验证，SoC在多个数据集和任务上一致地改善了校准性能，同时保持了有竞争力的判别能力。相比先前基于正交性的方法，SoC在不确定性估计校准方面表现更优。

Conclusion: SoC方法有效解决了现有正交约束方法在VLM测试时提示调优中导致过度自信的问题，通过平衡语义邻近性和原型分离，显著提升了模型校准性能，为关键决策系统中的VLM应用提供了更可靠的不确定性估计。
...
Conclusion: CtrlFuse通过交互式动态融合机制，成功解决了现有方法在任务适应性和语义可控性方面的局限性，实现了融合质量与下游任务性能的双重提升。
...
TL;DR: SafeRedir是一个轻量级推理时框架，通过提示嵌入重定向实现鲁棒遗忘，无需修改底层图像生成模型，有效移除有害概念同时保持良性生成质量。


<details>
  <summary>Details</summary>
Motivation: 图像生成模型会记忆训练数据中的不良概念（如NSFW内容和受版权保护的艺术风格），现有遗忘方法需要昂贵的重新训练、会降低良性生成质量，或无法抵抗提示改写和对抗攻击。

Method: SafeRedir通过提示嵌入重定向实现推理时遗忘，包含两个核心组件：潜在感知多模态安全分类器用于识别不安全生成轨迹，以及令牌级delta生成器用于精确语义重定向，配备令牌掩码和自适应缩放辅助预测器。

Result: 在多个代表性遗忘任务上的实证结果表明，SafeRedir实现了有效的遗忘能力、高语义和感知保持、鲁棒的图像质量，并增强了对对抗攻击的抵抗力。该框架能有效泛化到各种扩散骨干和现有遗忘模型。

Conclusion: SafeRedir提供了一种轻量级、即插即用的解决方案，用于鲁棒遗忘，无需修改底层模型，在安全性和生成质量之间取得了良好平衡，具有广泛的适用性。
...
TL;DR: 论文提出基于可迁移性、鲁棒性、可解释性和计算效率四支柱的深度伪造检测可靠性评估框架，分析五种SOTA方法发现显著进展与关键局限
...
Method: 提出基于四个支柱的可靠性评估框架：可迁移性（在不同数据集和生成方法间的泛化能力）、鲁棒性（对抗攻击和扰动的抵抗力）、可解释性（检测决策的可解释程度）、计算效率（资源消耗和速度）。使用该框架分析了五种最先进的深度伪造检测方法。

Result: 分析显示深度伪造检测方法在特定方面取得显著进展，但也暴露出关键局限性，特别是在可迁移性和鲁棒性方面存在不足，需要更全面的评估标准。

Conclusion: 需要超越传统分类性能指标的更全面评估框架来推动深度伪造检测技术的发展，提出的四支柱框架为系统评估提供了基础，有助于识别现有方法的优缺点并指导未来研究方向。
...
Method: 提出Salience-SGG框架，包含迭代显著性解码器（ISD），强调具有显著空间结构的三元组。为此，提出了语义无关的显著性标签来指导ISD。
...
Conclusion: Salience-SGG通过关注空间显著性，有效解决了场景图生成中的长尾分布问题，在保持无偏性的同时提升了空间理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 目前大多数急性缺血性卒中病灶分割模型基于U-Net框架，但具体配置差异大，且许多实现不公开，最优配置不明确。需要开发一个公开可用的、经过系统优化的鲁棒分割框架。
...
Conclusion: ISLA是一个经过系统优化的鲁棒分割框架，在急性缺血性卒中病灶分割任务上表现出色，其公开可用性将促进该领域的研究和应用。
...
<details>
  <summary>Details</summary>
Motivation: H&E染色是病理分析的标准方法，但荧光显微镜提供补充信息。将荧光图像转换为H&E样外观有助于解释和与标准工作流程集成

Method: 采用Cycle-Consistent Adversarial Network (CycleGAN)方法，将C01和C02荧光通道组合成RGB，学习荧光和H&E域之间的双向映射。使用基于ResNet的生成器、残差块和PatchGAN判别器，结合对抗损失、循环一致性损失和身份损失进行训练
...
Method: 提出DentalX方法，引入结构上下文提取模块，学习牙科解剖结构的语义分割作为辅助任务，提取有意义的结构上下文并将其整合到主要疾病检测任务中，以增强对细微牙科疾病的检测。

Result: 在专用基准测试上的广泛实验表明，DentalX在两个任务上都显著优于先前的方法。这种相互受益在模型优化过程中自然产生，因为两个任务之间的相关性被有效捕捉。

Conclusion: DentalX通过利用口腔结构信息来缓解X光片中的视觉模糊问题，有效提升了牙科疾病检测性能，证明了结构上下文信息在医学图像分析中的重要性。
...
TL;DR: 本文提出了一种自动合成3D视觉定位数据及其推理过程的管道，并基于此数据微调LLM得到Reason3DVG-8B模型，该模型仅用1.6%的训练数据就超越了之前的LLM方法3D-GRAND。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉定位任务仍然具有挑战性，因为现有模型推理能力有限。现有方法需要大量人工标注数据进行监督训练，而基于合成数据的方法性能提升有限且与数据收集成本不成比例。

Method: 提出了一个自动合成3D视觉定位数据及其对应推理过程的数据管道，并利用生成的数据对大型语言模型进行微调，开发了Reason3DVG-8B模型。

Result: Reason3DVG-8B模型仅使用1.6%的训练数据就超越了之前的LLM-based方法3D-GRAND，证明了所提出数据的有效性和推理在3D视觉定位中的重要性。

Conclusion: 通过自动合成带有推理过程的3D视觉定位数据并用于LLM微调，可以显著提升3D视觉定位性能，同时大幅减少对人工标注数据的依赖。
...
TL;DR: 3AM通过将3D感知特征从MUSt3R集成到SAM2中，解决了视频对象分割在大视角变化下的问题，无需相机位姿或深度图，仅需RGB输入即可实现几何一致的分割。
...
Method: 提出3AM训练时增强方法：1）将MUSt3R的多层次3D感知特征集成到SAM2中；2）轻量级特征融合器融合编码隐式几何对应的MUSt3R特征和SAM2的外观特征；3）提出视场感知采样策略，确保帧观察空间一致的对象区域以学习可靠的3D对应关系。

Result: 在具有宽基线运动的挑战性数据集（ScanNet++、Replica）上，3AM显著优于SAM2及其扩展版本，在ScanNet++的选定子集上达到90.6% IoU和71.7% Positive IoU，相比最先进的VOS方法分别提升15.9和30.4个百分点。

Conclusion: 3AM通过融合3D感知特征和外观特征，实现了无需相机位姿或预处理的几何一致视频对象分割，在大视角变化场景下显著提升了分割性能。
...
TL;DR: 该论文提出了一种基于零样本扩散的框架，将水印去除重新定义为视角合成问题，通过生成语义内容一致的替代视图来有效去除不可见水印，在15种水印方法上实现了最先进的去除效果。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成图像内容认证需求的增长，各大平台大规模部署不可见水印方案。然而，评估这些方案对抗复杂去除攻击的脆弱性对于评估其可靠性和指导鲁棒设计至关重要。当前水印方法对像素空间和频域攻击具有鲁棒性，但缺乏对语义保持视角变换的防御能力。

Method: 提出零样本扩散框架，将水印去除重新定义为视角合成问题。核心思想是通过生成相同语义内容的感知一致替代视图（类似于从偏移视角重新观察场景）来自然去除嵌入水印。方法在潜在空间中应用受控几何变换，并通过视角引导对应注意力机制在重建过程中保持结构一致性。该方法使用冻结的预训练模型，无需检测器访问或水印知识。

Result: 在15种水印方法上实现了最先进的水印抑制效果，优于14种基线攻击方法。在多个数据集上保持了卓越的感知质量，证明了语义保持视角变换对现有水印方案的强大攻击能力。

Conclusion: 揭示了不可见水印的基本脆弱性：即使对像素空间和频域攻击具有鲁棒性的水印，仍然容易受到语义保持视角变换的攻击。这项工作为评估水印方案的可靠性和设计更鲁棒的水印系统提供了重要见解。
...
### [74] [EmbeddingRWKV: State-Centric Retrieval with Reusable States](https://arxiv.org/abs/2601.07861)
*Haowen Hou,Jie Yang*
...
Method: 1) 状态表示学习：微调RWKV-based LLM得到EmbeddingRWKV，作为统一模型同时充当嵌入模型和状态骨干；2) 基于状态的重排序器：利用预计算状态，仅处理查询token，实现与文档长度解耦的推理；3) 统一层选择策略：仅使用25%的层即可保持98.62%性能

Result: 1) 重排序阶段获得5.4×-44.8×加速；2) 仅使用25%的层即可保持98.62%的全模型性能；3) 在广泛实验中实现高质量检索和重排序结果，同时显著提升系统效率

Conclusion: State-Centric Retrieval通过状态共享机制有效解决了传统RAG系统的效率瓶颈，实现了高质量检索与高效计算的统一，为RAG系统设计提供了新范式
...
### [75] [A Human-Centric Pipeline for Aligning Large Language Models with Chinese Medical Ethics](https://arxiv.org/abs/2601.07954)
*Haoan Jin,Han Ying,Jiacheng Ji,Hanhui Xu,Mengyue Wu*

Main category: cs.CL

TL;DR: 提出MedES基准和监护人循环框架，通过监督微调和领域特定偏好优化，在中文医疗伦理场景下对齐7B参数大语言模型，显著超越更大基线模型。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在医疗任务中应用广泛，但在复杂现实场景下与医疗伦理的细致要求对齐仍未被充分探索，特别是在中文医疗伦理领域。

Method: 1) 构建MedES动态场景中心基准，基于260个权威中文医疗、伦理和法律来源；2) 提出监护人循环框架，使用专用自动评估器生成针对性提示和结构化伦理反馈；3) 通过监督微调和领域特定偏好优化对齐7B参数LLM。

Result: 在中文医疗伦理背景下，对齐后的模型在核心伦理任务上显著超越更大的基线模型，在质量和综合评估指标上均有改善，自动评估器在领域内准确率超过97%。

Conclusion: 为中文医疗领域LLM与医疗伦理对齐提供了实用且可适应的框架，表明通过模块化替换底层规范语料，类似对齐流程可在其他法律和文化环境中实例化。
...
### [76] [Knowing But Not Doing: Convergent Morality and Divergent Action in LLMs](https://arxiv.org/abs/2601.07972)
...
<details>
  <summary>Details</summary>
Motivation: 价值对齐对AI安全和社会兼容性至关重要，但大型语言模型如何在现实决策场景中表征和实施人类价值观仍缺乏深入研究。

Method: 创建ValAct-15k数据集（包含3000个基于Reddit的咨询场景），基于Schwartz基本人类价值观理论评估10个前沿LLMs（5个美国公司，5个中国公司）和55名人类参与者，使用场景问题和传统价值观问卷进行对比分析。

Result: LLMs在场景决策中表现出近乎完美的跨模型一致性（Pearson r≈1.0），而人类则表现出广泛变异性（r∈[-0.79,0.98]）；人类和LLMs都显示出自我报告价值观与实际行为之间的弱相关性（r=0.4,0.3）；当被要求"持有"特定价值观时，LLMs表现下降达6.6%。

Conclusion: 对齐训练虽然产生了规范性的价值趋同，但未能消除人类特有的知行不一致问题，LLMs在价值实施上表现出角色扮演厌恶，揭示了价值对齐的复杂性。
...
### [77] [Explaining Generalization of AI-Generated Text Detectors Through Linguistic Analysis](https://arxiv.org/abs/2601.07974)
*Yuxi Xia,Kinga Stańczak,Benjamin Roth*

Main category: cs.CL

TL;DR: AI文本检测器在跨不同生成条件（如未见过的提示、模型家族或领域）时泛化能力差，本文通过语言特征分析解释其泛化行为


<details>
  <summary>Details</summary>
Motivation: 现有AI文本检测器在领域内基准测试中表现良好，但在跨不同生成条件时泛化能力不足，且缺乏对这种现象的深入解释

Method: 构建包含6种提示策略、7个大型语言模型和4个领域数据集的综合基准，训练分类检测器并评估其跨提示、跨模型和跨领域泛化能力，通过80个语言特征分析泛化性能与特征偏移的相关性

Result: 研究发现特定检测器和评估条件下的泛化性能与时态使用、代词频率等语言特征显著相关

Conclusion: 通过语言特征分析可以解释AI文本检测器的泛化行为，为改进检测器的跨条件泛化能力提供了理论基础
...
### [78] [Cross-Cultural Expert-Level Art Critique Evaluation with Vision-Language Models](https://arxiv.org/abs/2601.07984)
*Haorui Yu,Ramon Ruiz-Dolz,Xuehang Wen,Fengrui Zhang,Qiufeng Yi*

Main category: cs.CL

TL;DR: 本文提出了一个三层评估框架，用于评估视觉语言模型在跨文化艺术批评中的文化理解能力，发现自动指标不可靠，西方样本得分更高，需要单一主评委进行校准。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在视觉感知方面表现出色，但其在艺术中解读文化意义的能力尚未得到充分验证。需要建立一个系统框架来评估模型对跨文化艺术的文化理解深度。

Method: 提出了三层评估框架：第一层离线计算自动覆盖率和风险指标；第二层使用单一主评委按照五个维度进行基于量表的评分；第三层通过等渗回归将第二层总分校准到人类评分。

Result: 在152个样本的保留集上，校准后MAE降低了5.2%。评估了15个VLM在294个专家锚点（涵盖六种文化传统）上的表现。发现自动指标不能可靠代表文化深度，西方样本得分高于非西方样本，跨评委量表不匹配使得朴素平均不可靠。

Conclusion: 该框架为模型选择和文差距诊断提供了校准的文化理解分数，以及维度级诊断和风险指标。强调需要单一主评委和显式校准来获得可靠的文化理解评估。
...
TL;DR: 本文提出了一种构建多语言多模态事实核查数据集的新方法，通过整合ClaimReview数据、抓取完整辟谣文章、标准化裁决结果，并利用大语言模型进行证据提取和理由生成，创建了法语和德语的事实核查数据集。


<details>
  <summary>Details</summary>
Motivation: 当前在线平台上的虚假信息快速传播，急需强大、最新、可解释且多语言的事实核查资源。现有数据集在范围上有限，通常缺乏多模态证据、结构化标注以及声明、证据和裁决之间的详细联系。

Method: 开发了一个全面的数据收集和处理流程：1) 聚合ClaimReview数据源；2) 抓取完整的辟谣文章；3) 标准化异构的声明裁决；4) 用结构化元数据和对齐的视觉内容进行丰富；5) 使用最先进的大语言模型和多模态大语言模型进行证据提取（按预定义类别）和理由生成（将证据与裁决联系起来）。

Result: 通过G-Eval和人工评估验证，该流程能够：1) 实现不同组织或媒体市场之间事实核查实践的细粒度比较；2) 促进开发更可解释和基于证据的事实核查模型；3) 为未来多语言多模态虚假信息验证研究奠定基础。

Conclusion: 该研究提出的数据收集和处理流程成功构建了法语和德语的多模态事实核查数据集，解决了现有数据集的局限性，为开发更强大、可解释和多语言的事实核查系统提供了重要资源和方法基础。
...
TL;DR: VULCA-Bench是一个多文化艺术评论基准，用于评估视觉语言模型在超越表层视觉感知的文化理解能力，包含7,410个图像-评论对，涵盖8种文化传统，采用五层文化理解框架进行评估。


<details>
  <summary>Details</summary>
Motivation: 现有VLM基准主要评估对象识别、场景描述和事实问答等基础能力（L1-L2），而缺乏对高阶文化解释能力的评估。需要开发一个能够系统评估模型文化理解深度的基准。

Method: 构建了包含7,410个匹配图像-评论对的数据集，涵盖8种文化传统，支持中英双语。采用五层文化理解框架（L1-L5：从视觉感知到哲学美学），具体化为225个文化特定维度，并由专家撰写双语评论作为支持。

Result: 初步结果表明，高层推理（L3-L5）比视觉和技术分析（L1-L2）更具挑战性。数据集、评估脚本和标注工具已通过CC BY 4.0许可在补充材料中提供。

Conclusion: VULCA-Bench填补了现有VLM基准在评估文化理解深度方面的空白，为系统评估模型的多文化艺术理解能力提供了标准化工具，有助于推动视觉语言模型在文化敏感应用中的发展。
...
TL;DR: 论文提出纵向建模与评估范式，针对嵌套于作者且按时间排序的行为序列数据，更新NLP流程的四个部分，以解决传统文档级评估在生态效度上的不足。


<details>
  <summary>Details</summary>
Motivation: NLP通常将文档视为独立无序样本，但在纵向研究中，文档嵌套于作者并按时间排序形成行为序列。传统文档级评估在生态效度上存在问题，可能导致错误结论。

Method: 提出纵向建模与评估范式，更新四个NLP流程部分：1) 按人群(横断面)和/或时间(前瞻性)对齐的评估划分；2) 区分人际差异与个体内动态的准确度指标；3) 默认包含历史信息的序列输入；4) 支持不同历史潜在状态粒度(汇总、显式动态或基于交互)的模型内部结构。

Result: 在包含238名参与者、17k篇日常日记转录文本与PTSD症状严重程度配对的数据集上，传统文档级评估与提出的生态有效建模评估相比，可能产生显著不同甚至相反的结论。

Conclusion: 研究结果支持NLP从词序列评估向行为序列范式的转变，强调纵向建模在捕捉真实世界行为动态中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 随着对话长度增加，LLMs会出现响应延迟增加和答案质量下降的问题，现有方法要么需要额外的LLM调用来构建记忆，要么进行离线记忆构建而不考虑当前用户话语，导致效率低下或破坏对话连续性

Method: DyCP是一种轻量级上下文管理方法，在查询时动态分割和检索相关记忆，保持对话的顺序结构而不需要预定义主题边界，支持高效、自适应的上下文检索

Result: 在三个长对话基准测试（LoCoMo、MT-Bench+、SCM4LLMs）和多个LLMs上，DyCP持续提升答案质量同时减少响应延迟

Conclusion: 尽管现代LLMs扩展了上下文窗口，但实际的长上下文处理能力仍有差距，有效上下文管理仍然至关重要，DyCP为此提供了一种有效的解决方案
...
TL;DR: 概念向量投影（CVP）是一种通过将情感建模为嵌入空间中的方向来生成连续、多语言情感分数的方法，该方法在不同领域和语言中具有良好的可移植性，但其线性假设存在近似性。


<details>
  <summary>Details</summary>
Motivation: 人文领域的情感分析应用通常需要情境化的连续分数，而现有方法在跨领域、跨语言和跨历史时期的可移植性方面存在不足，需要评估CVP方法的泛化能力和基本假设。

Method: 使用概念向量投影（CVP）方法，将情感建模为嵌入空间中的方向，评估该方法在不同体裁、历史时期、语言和情感维度上的表现，并检验其线性假设的有效性。

Result: 研究发现，在一个语料库上训练的概念向量能够很好地迁移到其他语料库，性能损失最小；CVP是一种可移植的方法，能有效捕捉可泛化的模式，但其线性假设是近似的，存在进一步发展的潜力。

Conclusion: CVP是一种有效的跨领域情感分析方法，具有良好的可移植性，但其线性假设的近似性表明该方法仍有改进空间，为未来研究提供了方向。
...
TL;DR: LLM Review框架通过盲审机制解决多智能体协作中的创意同质化问题，在科幻写作任务上优于基线方法，小模型+该框架可超越大单智能体模型


<details>
  <summary>Details</summary>
Motivation: 大语言模型在创意生成方面存在困难，而现有的多智能体框架虽然能改善推理，但会导致内容同质化，反而阻碍了创造力。需要一种既能获得反馈又能保持创意多样性的方法。

Method: 提出了LLM Review框架，采用盲审机制：智能体交换针对性反馈但独立修改，保持创意轨迹的多样性。同时创建了SciFi-100科幻写作数据集，结合LLM评分、人工标注和基于规则的新颖性指标进行综合评估。

Result: 实验表明LLM Review在多智能体基线方法中表现最佳，且使用该框架的小模型可以超越更大的单智能体模型，表明交互结构可以替代模型规模。
...
TL;DR: 研究发现大语言模型推理能力可通过内部潜在特征激活实现，思维链提示只是激活推理机制的有效方式之一而非必要条件。


<details>
  <summary>Details</summary>
Motivation: 探究思维链提示为何有效以及它是否是触发大语言模型推理能力的唯一机制，理解大语言模型内部推理机制的本质。

Method: 使用稀疏自编码器分析和干预大语言模型的内部表示，识别与推理行为因果相关的潜在特征，通过潜在特征引导来激活推理能力。

Result: 单个推理相关潜在特征的引导可显著提高准确率，无需显式思维链提示；大型模型中潜在引导能达到与标准思维链提示相当的性能，且输出更高效；推理导向的内部状态在生成早期即被触发，并能覆盖抑制显式推理的提示指令。
...
<details>
  <summary>Details</summary>
Motivation: 探索语言模型的计算能力极限，澄清训练对模型计算表达能力的影响

Method: 证明语言模型的自回归输出链式组合足以执行通用计算，能模拟任何算法在任何输入上的执行

Result: 即使随机初始化的语言模型在训练前就具备通用计算能力，训练主要改善可编程性而非计算表达能力

Conclusion: 语言模型的计算能力是其固有属性，训练的作用是提供自然语言接口来访问这些能力，而非创造计算表达能力
...
TL;DR: 本文提出了一个全面的置信度估计评估框架，重点关注大语言模型在语言变化下的三个新维度：提示扰动鲁棒性、语义等价答案稳定性、语义不同答案敏感性。


<details>
  <summary>Details</summary>
Motivation: 现有置信度估计方法主要关注校准和区分能力，但忽视了在语言变化背景下的重要缺陷。置信度估计应该在语义等价的提示或答案变化下保持一致，而在答案含义不同时应该发生变化。因此需要更全面的评估框架来反映真实使用场景中的可靠性。

Method: 提出了一个综合评估框架，从三个新维度评估置信度估计质量：1) 对提示扰动的鲁棒性；2) 跨语义等价答案的稳定性；3) 对语义不同答案的敏感性。通过实验验证常见置信度估计方法在这些指标上的表现。

Result: 研究发现，许多在大语言模型上常用的置信度估计方法在这些新指标上表现不佳：在传统校准或区分指标上表现良好的方法，往往对提示变化不够鲁棒，或对答案变化不够敏感。

Conclusion: 该框架揭示了现有置信度估计评估在真实世界大语言模型使用场景中的局限性，为选择和设计更可靠的置信度估计方法提供了实用指导。需要超越传统校准和区分指标，考虑语言变化下的鲁棒性和敏感性。
...
TL;DR: 本文研究了面向工具调用智能体RAG的查询建议问题，当用户提问超出知识范围时，系统能够生成相似且可回答的查询建议，以增强用户交互体验。


<details>
  <summary>Details</summary>
Motivation: 当前基于工具调用的智能体RAG系统存在知识范围限制，当用户提问超出范围时可能导致幻觉问题。虽然已有防护框架可以阻止超出范围的问题，但缺乏对可回答查询建议的研究，而这对增强用户与RAG智能体的交互至关重要。

Method: 提出了鲁棒的动态少样本学习方法，从相关工作流程中检索示例。系统可以自我学习（例如基于先前的用户查询），因此在实际应用中易于部署。该方法考虑了用户问题不可回答的场景，建议的查询应该相似以辅助用户交互。

Result: 在三个基准数据集上进行了评估，这些数据集基于从真实世界用户查询收集的两个未标记问题数据集。实验结果表明，该方法能够产生更相关且可回答的建议，优于少样本和仅检索的基线方法。

Conclusion: 该方法能够实现更安全、更有效的用户与智能体RAG的交互，解决了传统查询推荐设置中的主要挑战，即确保建议的查询在RAG的多步骤工作流程中是可回答的。
...
TL;DR: ACPS框架通过因果推理和简洁思维草图，在减少token使用的同时提升LLM在各种推理任务上的准确性和鲁棒性
...
Method: 提出自适应因果提示与思维草图框架，利用结构因果模型推断查询对答案的因果效应，自适应选择适当干预（标准前门和条件前门调整），并用简洁思维草图替代冗长思维链

Result: 在多个推理基准测试和LLM上的实验表明，ACPS在准确性、鲁棒性和计算效率方面持续优于现有提示基线

Conclusion: ACPS框架通过因果推理和高效思维草图，实现了跨异构任务的可泛化推理，显著减少了token使用和推理成本
...
TL;DR: ExoFormer通过引入外部锚定投影解耦Transformer中早期层同时作为稳定参考和计算模块的矛盾，在性能、数据效率和注意力下沉方面均有提升，但存在表示坍缩现象。


<details>
  <summary>Details</summary>
Motivation: Transformer中重用早期层注意力投影作为残差连接时存在根本矛盾：第一层需要同时作为所有深层稳定参考和有效计算模块。为了解决这一矛盾，作者提出ExoFormer。

Method: ExoFormer学习专门的外部锚定投影，将其置于顺序层堆栈之外，从而将锚定角色与计算精炼解耦。通过统一的归一化混合框架（研究不同系数粒度：元素级、头级、标量级）应用于所有注意力路径（查询、键、值和门控逻辑）。

Result: ExoFormer变体始终优于内部锚定对应模型。动态变体在下游任务准确率上比基线提高2.13个百分点，数据效率更高，用1.84倍更少的token达到基线验证损失。注意力下沉比标准门控注意力减少2倍。但所有ExoFormer变体都表现出表示坍缩迹象。

Conclusion: 通过卸载假说解释表示坍缩现象：外部锚定保留了必要的token身份信息，使各层能够专门进行计算精炼。作者发布了代码和模型以促进未来研究。
...
TL;DR: 该研究针对大型推理模型的置信度校准问题，提出了RMCB基准测试，包含34.7万条推理轨迹，评估了十多种表示方法，发现文本编码器在区分能力上最优，而结构感知模型在校准方面最优，两者存在权衡关系。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在关键领域的错误校准问题削弱了其可靠性，需要准确估计其长格式、多步骤输出的置信度。目前缺乏针对这一任务的全面基准测试。

Method: 1. 构建RMCB基准：包含347,496条来自6个流行LRM的推理轨迹，涵盖临床、金融、法律、数学推理等关键领域；2. 评估十多种表示方法：包括序列、图基和文本编码器架构；3. 分析区分能力（AUROC）和校准性能（ECE）的权衡。

Result: 1. 文本编码器获得最佳AUROC（0.672），结构感知模型获得最佳ECE（0.148）；2. 没有单一方法在两方面都占优；3. 增加架构复杂性并不总能超越简单的序列基线；4. 仅依赖块级隐藏状态的方法存在性能上限。

Conclusion: 该研究提供了迄今为止最全面的置信度估计基准，建立了严格的基线，并展示了当前基于表示的范式的局限性，揭示了区分能力和校准之间的持久权衡。
...
TL;DR: SwiftMem：一种查询感知的智能体记忆系统，通过专门的时间索引和语义DAG-Tag索引实现亚线性检索，解决了现有记忆系统全量检索的延迟瓶颈问题。
...
Method: 1. 时间索引：支持对数时间范围查询，用于时间敏感检索
2. 语义DAG-Tag索引：通过分层标签结构将查询映射到相关主题
3. 嵌入-标签协同整合机制：基于语义聚类重新组织存储以改善缓存局部性

Result: 在LoCoMo和LongMemEval基准测试中，SwiftMem相比最先进的基线实现了47倍的搜索加速，同时保持竞争性的准确性，使记忆增强的LLM智能体能够实际部署。

Conclusion: SwiftMem通过查询感知的亚线性检索方法，有效解决了智能体记忆系统的延迟瓶颈问题，为记忆增强的LLM智能体提供了实用的部署解决方案。
...
TL;DR: 通过微调函数向量提升LLM的关系表示能力，使用少量示例微调后性能优于原始因果中介分析向量，并引入复合函数向量增强类比推理能力


<details>
  <summary>Details</summary>
Motivation: 概念间的关系表示是智能系统理解世界的核心前提。虽然已有研究使用因果中介分析发现少量注意力头编码任务表示（函数向量），但如何更好地提取和利用这些关系表示仍需探索

Method: 1) 使用少量示例（约20个词对）微调函数向量；2) 引入复合函数向量——微调函数向量的加权组合；3) 在推理时将复合向量插入LLM激活中

Result: 1) 微调函数向量在关系型词语补全任务上优于原始向量；2) 在关系词解码和语义关系人类相似性判断对齐方面表现更好；3) 复合函数向量显著提升认知科学和SAT基准中的类比问题性能
...
TL;DR: 该研究探索了提示设计对LLM政治问答清晰度自动评估的影响，使用CLARITY数据集比较不同提示策略，发现推理型提示能显著提升清晰度评估准确性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM自动评估主要关注事实准确性，但对政治问答中的清晰度评估研究不足，特别是提示设计对自动清晰度评估的影响尚未充分探索。

Method: 使用SemEval 2026共享任务的CLARITY数据集，比较GPT-3.5基线模型与GPT-5.2在三种提示策略下的表现：简单提示、思维链提示、带少量示例的思维链提示。

Result: GPT-5.2在清晰度预测上始终优于GPT-3.5基线，带少量示例的思维链提示将准确率从56%提升至63%；思维链提示在回避检测上达到34%准确率；推理型提示将话题识别准确率从60%提升至74%。

Conclusion: 提示设计能可靠地提升高层次清晰度评估性能，但细粒度回避检测和话题识别仍然具有挑战性，即使使用结构化推理提示也难以完全解决。
...
TL;DR: LogiSafetyGen框架将非结构化法规转换为时序逻辑预言机，通过逻辑引导模糊测试生成安全关键轨迹，并构建LogiSafetyBench基准测试LLM在满足功能目标和合规规则方面的能力


<details>
  <summary>Details</summary>
Motivation: 现有基准测试往往忽视隐式监管合规性，无法评估LLM能否自主执行强制性安全约束，特别是在高风险领域中LLM必须严格遵守监管标准

Method: 提出LogiSafetyGen框架，将非结构化法规转换为线性时序逻辑预言机，采用逻辑引导模糊测试合成有效的安全关键轨迹；基于此构建LogiSafetyBench基准，包含240个人工验证的任务

Result: 评估13个最先进的LLM发现，尽管更大模型在功能正确性方面表现更好，但经常优先考虑任务完成而非安全性，导致不合规行为

Conclusion: 需要开发能够平衡功能正确性和监管合规性的LLM系统，特别是在高风险应用中，当前模型在安全约束执行方面存在明显不足
...
Method: 提出T-SPIN方法：1) 除了当前优势外，还引入历史优势（迭代生成响应与初始策略生成的原型合成响应之间的比较）；2) 在自对弈框架中引入熵约束，理论上支持无参考微调，消除训练-生成差异。

Result: 在多个任务上的实验结果表明，T-SPIN不仅性能优于SPIN，而且在迭代过程中保持稳定演化。与监督微调相比，T-SPIN仅使用25%的样本就能达到相当甚至更好的性能。

Conclusion: T-SPIN通过历史优势和熵约束有效解决了SPIN的局限性，在数据稀缺情况下表现出色，为语言模型微调提供了更稳定有效的解决方案。
...
TL;DR: 该论文提出了Generation-Augmented Generation (GAG)方法，通过将私有专业知识作为额外专家模态，在表示层面对齐到冻结的基础模型，解决了私有知识注入中的微调迭代成本高和RAG证据碎片化等问题。


<details>
  <summary>Details</summary>
Motivation: 在生物医学、材料和金融等高风险领域部署大语言模型时，需要注入私有的、快速演变的、在公开预训练中代表性不足的领域特定知识。现有两种主要方法各有明显缺点：微调迭代成本高且容易导致灾难性遗忘和通用能力退化；检索增强生成(RAG)虽然保持基础模型完整，但在专业私有语料中因证据碎片化、检索漂移和长上下文压力而表现脆弱。

Method: 受多模态LLM将异构模态对齐到共享语义空间的启发，提出了Generation-Augmented Generation (GAG)方法。该方法将私有专业知识视为额外的专家模态，通过紧凑的表示级接口将其对齐到冻结的基础模型，避免了提示时的证据序列化，同时实现了即插即用的专业化和可扩展的多领域组合。

Result: 在两个私有科学QA基准测试（免疫学佐剂和催化材料）以及混合领域评估中，GAG在两个基准上分别比强RAG基线提高了15.34%和14.86%的专业性能，同时在六个开放通用基准上保持性能，并实现了接近oracle选择性的可扩展多领域部署激活。

Conclusion: GAG方法通过将私有专业知识作为专家模态进行表示级对齐，有效解决了私有知识注入的挑战，实现了高性能的专业化、可扩展的多领域组合和可靠的激活选择，为高风险领域的LLM部署提供了实用解决方案。
...
TL;DR: 研究发现MoE模型性能主要由总参数量和专家稀疏度决定，而非仅考虑总参数和激活参数，提出了最大化总参数、最小化专家稀疏度和专家数量的设计原则。


<details>
  <summary>Details</summary>
Motivation: 当前MoE语言模型设计主要基于总参数量（内存占用）和激活参数量（推理成本），但研究发现这两个因素不足以描述最优架构，需要更全面的性能评估框架。

Method: 通过系统研究，分析MoE性能的决定因素，发现总参数量（N_total）和专家稀疏度（s:=n_exp/n_topk）是主要决定因素，并建立了相应的设计原则。

Result: 研究表明专家数量n_exp和top-k专家数n_topk不会在稀疏度中"抵消"，更大的专家总数会轻微损害性能，因为它迫使核心模型维度（深度和宽度）减少以满足内存约束。

Conclusion: 提出了MoE设计的简单原则：在给定约束下最大化总参数量，最小化专家稀疏度（最大化n_topk）和专家数量，为MoE架构设计提供了稳健框架。
...
TL;DR: Med-CoReasoner框架通过并行英语和本地语言推理、概念结构化以及临床知识整合，显著提升多语言医学推理能力，特别是在低资源语言上效果明显。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在英语医学任务上表现良好，但在多语言场景下存在显著的推理差距，限制了全球医疗公平部署。需要解决本地语言医学推理能力不足的问题。

Method: 提出Med-CoReasoner框架：1）并行激发英语和本地语言推理；2）将推理抽象为结构化概念；3）通过概念级对齐和检索将本地临床知识整合到英语逻辑框架中，结合英语推理的结构稳健性和本地语言的实践专业知识。

Result: 在三个基准测试中，Med-CoReasoner将多语言推理性能平均提升5%，在低资源语言上提升尤为显著。模型蒸馏和专家评估证实其能产生临床合理且文化基础扎实的推理轨迹。

Conclusion: Med-CoReasoner通过语言协同推理框架有效缩小了多语言医学推理差距，为全球医疗公平部署提供了可行方案，特别是在资源匮乏语言环境中具有重要应用价值。
...
TL;DR: DART是一个强化学习框架，通过动态构建rollout树来发现工具使用机会，在长链推理中实现自发工具调用，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 当前将工具使用集成到长链推理中面临训练数据稀缺和可能损害模型内在推理能力的挑战，需要一种无需人工标注的方法来协调工具执行与长链推理。

Method: DART框架在训练时动态构建rollout树来发现有效的工具使用机会，在promising位置分支探索多样化的工具集成轨迹，然后通过基于树的优势估计识别和奖励对解决方案有积极贡献的子轨迹。

Result: 在AIME和GPQA-Diamond等具有挑战性的基准测试中，DART显著优于现有方法，成功协调了工具执行与长链推理。

Conclusion: DART通过强化学习框架实现了在长链推理中自发使用工具的能力，无需人工标注，有效解决了工具集成与长链推理的协调问题。
...
TL;DR: D²Plan：一种双智能体动态全局规划范式，通过推理器和净化器的协作解决检索增强LLM在复杂多跳推理中的搜索链构建无效和推理劫持问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的检索增强LLM在多跳推理任务中存在两个关键失败模式：1）搜索链构建无效，产生错误查询或遗漏关键信息检索；2）推理被外围证据劫持，导致模型将干扰信息误认为有效证据。这些问题源于累积上下文被关键证据和无关信息淹没。

Method: 提出D²Plan双智能体动态全局规划范式，包含推理器和净化器两个组件。推理器在推理过程中构建显式全局规划并根据检索反馈动态调整；净化器评估检索相关性并压缩关键信息供推理器使用。采用两阶段训练框架：基于合成轨迹的监督微调冷启动，以及带有规划导向奖励的强化学习。

Result: 广泛实验表明，D²Plan能够实现更连贯的多步推理，对无关信息具有更强的鲁棒性，在具有挑战性的QA基准测试中取得了优越性能。

Conclusion: D²Plan通过双智能体协作和动态全局规划，有效解决了检索增强LLM在多跳推理中的关键失败模式，提升了复杂检索增强推理的能力和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索如何通过先进的提示工程技术来提升大型语言模型在情感分析任务中的性能，特别是在处理复杂情感表达如讽刺检测时。

Method: 使用GPT-4o-mini和gemini-1.5-flash两种模型，对比评估少样本学习、思维链提示和自一致性等高级提示技术。任务包括情感分类、基于方面的情感分析和讽刺检测，使用准确率、召回率、精确率和F1分数进行评估。

Result: 高级提示技术显著提升情感分析性能：少样本方法在GPT-4o-mini中表现最佳，而思维链提示使gemini-1.5-flash的讽刺检测性能提升高达46%。

Conclusion: 提示策略需要根据具体模型和任务进行定制，强调提示设计必须与LLM架构和任务语义复杂性相匹配的重要性。
...
TL;DR: AgriAgent是一个用于真实农业场景的两级智能体框架，通过分层执行策略处理不同复杂度的任务，相比统一执行范式的方法在复杂任务上表现更好。


<details>
  <summary>Details</summary>
Motivation: 真实农业场景中的智能体系统需要处理从轻量级信息理解到复杂多步执行的各种任务，但现有统一执行范式难以适应任务复杂度差异大和工具可用性不完整的问题。

Method: 提出AgriAgent两级智能体框架：简单任务由特定模态智能体直接推理处理；复杂任务触发契约驱动规划机制，将任务转化为能力需求，进行能力感知的工具编排和动态工具生成，支持多步可验证执行和故障恢复。
...
TL;DR: CLaS-Bench是一个用于评估多语言大模型语言导向能力的轻量级并行问题基准，涵盖32种语言，首次为多语言导向技术提供了标准化评估框架。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在多语言NLP中的重要性日益增加，需要更高效、可解释的技术来适应目标语言。虽然导向技术（在推理过程中操纵内部表示）已经出现，但缺乏专门的基准和评估协议来量化这些技术的有效性。

Method: 开发了CLaS-Bench基准，包含32种语言的并行问题。评估了多种导向技术：残差流DiffMean干预、探针推导方向、语言特定神经元、PCA/LDA向量、稀疏自编码器和提示基线。通过语言控制和语义相关性两个维度测量导向性能，结合成单一的调和平均导向分数。

Result: 研究发现，跨语言情况下，简单的基于残差的DiffMean方法始终优于所有其他方法。层间分析显示，语言特定结构主要出现在后期层，且导向方向根据语言家族进行聚类。

Conclusion: CLaS-Bench是首个用于多语言导向的标准化基准，既能对语言表示进行严格的科学分析，又能将导向作为低成本适应替代方案进行实际评估。
...
TL;DR: 首个针对语音对话中心理操纵检测的研究，通过合成多说话者语音数据集，发现模型在语音检测中的召回率显著低于文本，揭示了跨模态检测的挑战。


<details>
  <summary>Details</summary>
Motivation: 心理操纵作为计算社会推理的新兴任务，先前研究仅关注文本对话，忽视了操纵策略在语音中的表现。本研究旨在填补这一空白，探索语音对话中心理操纵检测的独特挑战。

Method: 创建了首个多说话者语音心理操纵检测基准SPEECHMENTALMANIP，通过高质量、声音一致的文本转语音技术将文本数据集转换为音频。使用少样本大型音频-语言模型和人工标注，评估模态对检测准确性和感知的影响。

Result: 模型在语音检测中表现出高特异性但召回率显著低于文本检测，表明模型对训练中缺失的声学或韵律线索敏感。人类标注者在音频设置中也表现出类似的不确定性，凸显了操纵性语音固有的模糊性。

Conclusion: 研究强调了在多模态对话系统中进行模态感知评估和安全对齐的必要性，语音心理操纵检测面临独特挑战，需要进一步研究声学和韵律线索的整合。
...
TL;DR: 该研究提出了一种基于学生个性特征调整LLM辅导策略的方法，通过构建个性-教学法分类框架，使LLM能根据模拟学生个性调整教学策略，获得教师偏好和更好的教学效果。


<details>
  <summary>Details</summary>
Motivation: 当前LLM教育辅导系统未考虑学生个性特征，而不同教学策略对不同个性的学生效果不同，策略不匹配可能对学习成果产生负面影响，因此需要开发能根据学生个性调整教学策略的个性化LLM辅导系统。

Method: 首先基于教育学文献构建连接教学方法和个性特征分类的框架；然后模拟师生对话，让LLM导师根据模拟学生个性调整教学策略；最后通过人类教师评估该方法的效果。

Result: 人类教师一致偏好该方法胜过两个基线方法；该方法还增加了较少使用但高影响力的策略（如角色扮演）的使用频率，人类和LLM标注者都显著偏好这些策略。

Conclusion: 该研究为开发更个性化、更有效的LLM教育应用铺平了道路，证明了考虑学生个性特征调整教学策略的重要性，能够提升LLM辅导系统的教育效果。
...
<details>
  <summary>Details</summary>
Motivation: GRPO方法依赖昂贵的外部验证器或人工规则，导致计算成本高、训练延迟大，且稀疏奖励阻碍优化效率。需要寻找更高效的内在奖励机制。

Method: 提出Latent-GRPO框架，基于潜在空间几何特性：正确推理轨迹的终端token表示形成密集聚类，错误轨迹则分散为离群点。引入迭代鲁棒质心估计（IRCE）算法，通过球面投影缓解幅度波动，并通过迭代聚合估计鲁棒的"真理质心"。

Result: 在多个数据集上，该方法保持模型性能的同时，相比基线实现超过2倍的训练加速。广泛结果显示具有强大的泛化能力和鲁棒性。

Conclusion: Latent-GRPO通过潜在空间几何特性有效生成密集连续奖励，解决了GRPO依赖外部验证器的问题，显著提升了训练效率和优化效果。
...
TL;DR: JudgeRLVR：两阶段判别-生成范式，通过先训练模型判别可验证答案，再基于此进行生成优化，在数学领域实现精度提升同时减少生成长度
...
Method: 提出JudgeRLVR两阶段范式：第一阶段训练模型判别具有可验证答案的解决方案响应；第二阶段使用判别模型初始化，进行标准的生成式RLVR微调。

Result: 在Qwen3-30B-A3B模型上，相比传统RLVR：在领域内数学任务上，平均精度提升约3.7分，平均生成长度减少42%；在领域外基准测试上，平均精度提升约4.5分，显示出更好的泛化能力。

Conclusion: 判别能力是高效生成的前提。通过学习区分有效解决方案，模型能够内化指导信号来剪枝搜索空间，实现更好的质量-效率权衡和泛化性能。
...
TL;DR: 提出一个结合NLP和形式化验证的框架，用于开发具有同理心的治疗对话机器人，通过Transformer提取对话特征，用随机混合自动机建模治疗会话，并通过统计模型检查验证同理心属性。


<details>
  <summary>Details</summary>
Motivation: 对话机器人在心理健康治疗路径中作为支持工具的应用日益增多，具有重要社会影响。同理心是治疗环境中的关键非功能性需求，但当前聊天机器人开发实践缺乏系统化的方法来指定或验证同理心。

Method: 提出一个集成自然语言处理和形式化验证的框架：1）使用基于Transformer的模型提取对话特征；2）将特征转换为治疗会话的随机混合自动机模型；3）通过统计模型检查验证同理心相关属性；4）策略合成为塑造智能体行为提供指导。

Result: 初步结果表明：1）形式化模型能够以良好保真度捕捉治疗动态；2）专门设计的策略提高了满足同理心要求的概率。

Conclusion: 该框架为开发具有同理心的治疗对话机器人提供了一种系统化方法，通过形式化验证确保同理心属性的满足，为心理健康支持工具的开发提供了新思路。
...
TL;DR: 论文研究了大型语言模型中的"溢出"现象——普通文本提示会引发过度输出，导致服务成本增加、延迟和性能下降，提出了BenchOverflow基准来评估和缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在普通交互设置下产生过度输出的故障模式，这种现象被称为"溢出"。与越狱或提示注入不同，溢出在常规交互中就会发生，当扩展到大量请求时，会导致服务成本增加、延迟和跨用户性能下降。从经济环境角度看，不必要的token会增加每次请求的成本和能耗，在规模上转化为巨大的运营支出和碳足迹。此外，溢出在共享环境中代表了计算放大和服务降级的实际向量。

Method: 提出了BenchOverflow基准，这是一个模型无关的基准测试，包含九种纯文本提示策略，这些策略可以在没有对抗性后缀或策略规避的情况下放大输出量。使用标准化协议，固定5000个新token的预算，评估了九个开源和闭源模型。使用上限饱和率(CSR@1k/3k/5k)和经验累积分布函数(ECDF)量化尾部风险，通过内部提示方差和跨模型相关性分析溢出的可重复性和异质性。提出了一种轻量级缓解措施——固定的简洁性提醒。

Result: 观察到长度分布的明显右移和重尾现象。上限饱和率和经验累积分布函数量化了尾部风险；内部提示方差和跨模型相关性表明溢出现象具有广泛的可重复性，但在不同模型系列和攻击向量之间存在异质性。轻量级缓解措施（固定的简洁性提醒）在所有策略和大多数模型中都能减弱右尾并降低上限饱和率。

Conclusion: 长度控制应被视为可测量的可靠性、成本和可持续性问题，而不仅仅是风格特点。通过实现跨模型长度控制鲁棒性的标准化比较，BenchOverflow为选择最小化资源浪费和运营支出的部署提供了实用基础，并为评估在不削弱任务性能的情况下遏制计算放大的防御措施提供了依据。
...
TL;DR: MHEL-LLaMo是一个用于历史文本实体链接的无监督集成方法，结合小语言模型和大语言模型，在多个历史文本基准测试中优于现有方法且无需微调。
...
Method: 提出MHEL-LLaMo，一种无监督集成方法，结合小语言模型（SLM）和大语言模型（LLM）。使用多语言双编码器（BELA）进行候选检索，通过指令调优的LLM进行NIL预测和候选选择。利用SLM的置信度分数区分简单和困难样本，仅对困难案例使用LLM。

Result: 在6种欧洲语言（英语、芬兰语、法语、德语、意大利语和瑞典语）的4个历史文本基准测试中，MHEL-LLaMo优于最先进的模型，且无需微调，为低资源历史实体链接提供了可扩展解决方案。

Conclusion: MHEL-LLaMo通过结合SLM和LLM的优势，在降低计算成本的同时防止简单案例的幻觉，为历史文本实体链接提供了一个高效、可扩展的无监督解决方案。
...
TL;DR: STAGE是一个针对完整电影剧本的统一基准测试，包含知识图谱构建、场景级事件摘要、长上下文剧本问答和角色扮演四个任务，旨在全面评估模型对长篇叙事世界的理解和一致性推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注问答或对话生成等单一子任务，缺乏对模型构建连贯故事世界并在多种推理和生成任务中保持一致性能力的评估。电影剧本作为复杂的长篇叙事形式，需要模型理解角色关系、时序事件和对话交互。

Method: STAGE定义了四个基于共享叙事世界表示的任务：1)知识图谱构建；2)场景级事件摘要；3)长上下文剧本问答；4)剧本内角色扮演。基准提供了150部中英文电影的清洗剧本、整理的知识图谱以及事件和角色中心标注。

Result: STAGE基准测试提供了全面的评估框架，能够评估模型在构建世界表示、抽象和验证叙事事件、长叙事推理以及生成角色一致响应方面的能力。数据集覆盖中英文电影，支持跨语言评估。

Conclusion: STAGE填补了现有基准测试的空白，为评估模型在复杂长篇叙事理解方面的能力提供了统一框架，特别是模型构建和使用连贯故事世界表示的能力，这对叙事AI的发展具有重要意义。
...
TL;DR: STAR框架通过分析输出概率偏移来检测推理时后门攻击，利用恶意推理路径在模型先验概率低但后验概率高的统计差异，实现高效检测。


<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地集成Chain-of-Thought等推理机制，显式推理暴露了推理时后门攻击的新攻击面。这些攻击注入恶意推理路径而不改变模型参数，且由于生成语言连贯的路径，能有效规避传统检测方法。

Method: 提出STAR（状态转移放大比）框架，通过分析输出概率偏移检测后门。该框架利用恶意输入诱导路径在模型一般知识中先验概率低但后验概率高的统计差异，量化状态转移放大，并使用CUSUM算法检测持续异常。

Result: 在多种模型（8B-70B）和五个基准数据集上的实验表明，STAR展现出强大的泛化能力，始终实现接近完美的性能（AUROC ≈ 1.0），效率比现有基线高约42倍。该框架对试图绕过检测的自适应攻击也表现出鲁棒性。

Conclusion: STAR框架为检测LLM推理时后门攻击提供了一种高效且鲁棒的方法，通过分析概率统计差异有效识别恶意推理路径，解决了显式推理机制带来的新安全挑战。
...
TL;DR: Deep Research Bench II是一个新的深度研究系统评估基准，包含132个跨22个领域的研究任务，使用9430个细粒度二元评分标准从信息回忆、分析和呈现三个维度评估研究报告质量。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究系统评估存在两个主要问题：一是现有基准未能充分测试系统分析证据和撰写连贯报告的能力；二是评估标准要么过于粗糙，要么完全由LLM定义，导致评分存在偏见且难以验证或解释。

Method: 构建包含132个跨22个领域研究任务的基准，每个任务需要生成长篇研究报告；开发9430个细粒度二元评分标准，涵盖信息回忆、分析和呈现三个维度；通过四阶段LLM+人工流程构建评分标准，结合自动提取和400多小时专家评审。
...
TL;DR: Ministral 3系列是一个参数高效的密集语言模型家族，包含3B、8B和14B三种规模，每个规模都有基础预训练模型、指令微调模型和推理模型三个变体，采用Cascade Distillation技术开发，具备图像理解能力，使用Apache 2.0许可证。


<details>
  <summary>Details</summary>
Motivation: 针对计算和内存受限的应用场景，开发参数高效的密集语言模型，满足不同规模的需求，并提供多种变体以适应不同任务类型。

Method: 采用Cascade Distillation（级联蒸馏）技术，这是一种迭代剪枝和持续训练与蒸馏相结合的方法，通过这种方法从较大的模型中推导出Ministral 3模型。

Result: 发布了Ministral 3系列模型，包含3B、8B和14B三种参数规模，每个规模都有三个变体：基础预训练模型（通用用途）、指令微调模型和推理模型（复杂问题解决），所有模型都具备图像理解能力。

Conclusion: Ministral 3系列为计算和内存受限的应用提供了一个参数高效的密集语言模型解决方案，通过Cascade Distillation技术实现了模型优化，并以Apache 2.0许可证开源，便于广泛使用。
...
### [116] [ExpSeek: Self-Triggered Experience Seeking for Web Agents](https://arxiv.org/abs/2601.08605)
*Wenyuan Zhang,Xinghua Zhang,Haiyang Yu,Shuaiyi Nie,Bingli Wu,Juwei Yue,Tingwen Liu,Yongbin Li*

Main category: cs.CL

TL;DR: ExpSeek提出了一种新的经验干预方法，将经验从任务前的全局上下文注入转变为步骤级的主动寻求，通过熵阈值确定干预时机，显著提升了Web智能体的性能。


<details>
  <summary>Details</summary>
Motivation: 现有经验干预方法主要在任务执行前被动注入经验作为全局上下文，难以适应智能体与环境交互过程中动态变化的上下文观察，需要更灵活的经验利用机制。
...
Conclusion: ExpSeek通过步骤级主动寻求经验的方法，有效解决了现有经验干预方法在动态环境中的适应性不足问题，为Web智能体的经验利用提供了新的技术范式。
...
TL;DR: OrderProbe：一个用于评估大语言模型结构重建能力的确定性基准，使用中、日、韩三种语言的四字固定表达，通过精确匹配评分来测试模型从乱序输入中恢复规范语序的能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在语义理解方面表现出色，但其从乱序输入中重建内部结构的能力尚未得到充分探索。句子级恢复任务由于存在多种有效词序而难以进行自动化评估。

Method: 提出OrderProbe基准，使用中文、日文和韩文中具有唯一规范语序的四字固定表达，支持精确匹配评分。同时提出一个诊断框架，评估模型在恢复准确率之外的语义保真度、逻辑有效性、一致性、鲁棒敏感性和信息密度。

Result: 在12个广泛使用的大语言模型上的实验表明，结构重建任务即使对前沿系统也很困难：零样本恢复准确率经常低于35%。同时观察到语义回忆和结构规划之间存在一致性的分离，表明结构鲁棒性不是语义能力的自动副产品。

Conclusion: 结构重建是大语言模型面临的重要挑战，OrderProbe提供了一个有效的评估工具，揭示了模型在语义理解和结构规划能力之间的差异，为未来模型开发提供了重要见解。
...
TL;DR: LALITA框架通过基于词汇和语言特征的源语句筛选策略，在低资源语言环境下优化机器翻译训练数据选择，显著提升翻译质量并减少50%以上的数据需求。


<details>
  <summary>Details</summary>
Motivation: 低资源语言机器翻译面临数据获取成本高昂的挑战，人工翻译生成足够数据过于昂贵，需要开发有效的数据筛选框架来优化平行文本构建，确保在低资源环境下获得最佳机器翻译系统性能。

Method: 提出LALITA框架，通过词汇和语言特征分析来筛选源语句，主要选择复杂句子构建平行语料库。使用英语-印地语双语文本评估有效的句子选择策略，模拟低资源数据可用性（5万到80万英语句子），并在多种语言（印地语、奥里亚语、尼泊尔语、挪威尼诺斯克语、德语）上进行测试。

Result: 通过主要训练复杂句子（来自现有和合成数据集），翻译质量显著提升。在所有数据规模（5万到80万句子）上都报告了性能改进。LALITA表现出显著效率，在多种语言上减少超过50%的数据需求，同时展示了在数据增强方面的实用性。

Conclusion: LALITA框架为低资源语言机器翻译提供了一种高效的数据筛选方法，不仅通过减少训练数据需求降低了系统训练成本，还展示了在数据增强方面的应用潜力，为解决低资源语言翻译的数据瓶颈问题提供了有效方案。
...
TL;DR: 研究通过道德价值条件化控制LLMs的政治立场，发现道德取向能显著影响模型在政治坐标上的位置，且这种效应受角色框架和模型规模调节。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要依赖直接探测或人口统计角色工程来揭示LLMs的政治意识形态偏见，但社会心理学认为政治意识形态是基本道德直觉的下游结果。本研究旨在探索道德价值与政治立场之间的因果关系。

Method: 将道德取向作为可控条件，让模型认可或拒绝特定道德价值，然后使用政治罗盘测试评估由此产生的政治立场变化。通过将道德价值作为"透镜"，观察道德条件化如何主动引导模型在经济和社会维度上的轨迹。

Result: 道德条件化能引起模型政治坐标的显著、特定于价值的偏移。这些效应受角色框架和模型规模的系统性调节，并且在实例化相同道德价值的替代评估工具中具有稳健性。
...
<details>
  <summary>Details</summary>
Motivation: 潜在习语表达（PIEs）与特定语言社区的日常经验密切相关，构成了评估NLP系统语言和文化能力的有趣挑战。需要创建一个能够比较不同语言习语模式并研究跨语言和跨模态理解的数据集

Method: 创建了XMPIE平行多语言多模态数据集，包含34种语言，由语言专家按照多语言指南创建。每个PIE配有五张图像，涵盖从习语意义到字面意义的连续谱，包括语义相关和随机干扰项

Result: 构建了一个高质量的数据集，包含超过一万个项目，支持跨语言比较分析和跨模态（文本vs图像）研究，可用于评估多语言和多模态习语理解能力

Conclusion: XMPIE为评估多语言和多模态习语语言理解提供了一个高质量的基准，能够研究习语理解在不同语言间的迁移以及在不同模态间的转移
...
TL;DR: 该论文首次对安全语言生成进行理论分析，证明安全语言识别不可行，安全语言生成至少与普通语言识别一样困难（同样不可行），并讨论了部分可处理的情况。


<details>
  <summary>Details</summary>
Motivation: 随着语言学习理论的发展，虽然语言识别已被证明不可行，但语言生成是可行的。然而，在现实世界应用中，需要考虑语言生成的安全性。本文旨在为安全语言生成提供首个理论框架。

Method: 基于极限学习计算范式，形式化定义了安全语言识别和安全语言生成任务。通过理论证明分析这些任务的可行性边界。

Result: 证明安全语言识别是不可能的；安全语言生成至少与普通语言识别一样困难，因此也是不可能的。同时讨论了某些情况下这些任务可能是可处理的。

Conclusion: 安全语言生成在理论上是困难的，但某些特定情况下可能存在可行的解决方案。这为未来研究安全语言生成系统提供了理论基础和方向。
...
<details>
  <summary>Details</summary>
Motivation: LLM作为评分者的范式虽然可扩展，但由于生成随机性，冻结的黑盒模型难以与人类评分标准对齐。存在三个主要问题：提示敏感性导致的评分标准不稳定性、缺乏可审计证据的不可验证推理、以及与人类评分边界不匹配的量表对齐问题

Method: 提出RULERS框架，包含三个核心组件：1) 将自然语言评分标准编译为版本化不可变包；2) 通过结构化解码强制确定性证据验证；3) 应用轻量级Wasserstein后校准，所有操作都不更新模型参数

Result: 在论文和摘要评估基准上的实验表明，RULERS在人类评分一致性方面显著优于代表性基线方法，对对抗性评分标准扰动保持强稳定性，并使较小模型能够与较大的专有评分模型相媲美

Conclusion: 可靠的LLM评分需要可执行的评分标准、可验证的证据和校准的量表，而不仅仅是提示词调整。RULERS框架通过编译器-执行器架构实现了这一目标
...
TL;DR: LLMs在仇恨言论净化任务中常因安全警报而错误拒绝处理，研究分析了触发这种拒绝的上下文和语言偏见，并提出跨语言翻译策略来减少错误拒绝。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在仇恨言论净化任务中经常触发安全警报而拒绝处理，这阻碍了实际应用。研究旨在系统调查这种错误拒绝行为，分析触发拒绝的偏见因素。

Method: 评估了9个LLM在英语和多语言数据集上的表现，分析了语义毒性和目标群体对拒绝率的影响，并提出跨语言翻译策略（英语→中文净化→英语回译）。

Result: LLMs对语义毒性更高、针对特定群体（国籍、宗教、政治意识形态）的输入有不成比例的拒绝率；多语言数据集总体拒绝率低于英语数据集，但仍存在语言依赖的偏见；跨语言翻译策略显著减少了错误拒绝，同时保持了原始内容。

Conclusion: LLMs在仇恨言论净化中存在系统性偏见导致的错误拒绝问题，提出的跨语言翻译策略是一种有效且轻量级的缓解方法，能显著减少错误拒绝而不损害内容完整性。
...
<details>
  <summary>Details</summary>
Motivation: 目前大语言模型在金融量化任务上的评估是碎片化的，主要局限于知识问答，缺乏对数学推理和实际交易策略编码能力的全面评估。

Method: 开发了QuantEval基准测试，包含三个维度：知识问答、量化数学推理和量化策略编码。特别集成了CTA风格的回测框架，可以执行模型生成的策略并使用金融性能指标进行评估。

Result: 评估了当前最先进的开源和专有大语言模型，发现与人类专家存在显著差距，特别是在推理和策略编码方面。通过领域对齐数据的监督微调和强化学习实验，展示了持续的改进。

Conclusion: QuantEval将促进大语言模型在量化金融能力方面的研究，加速其在真实交易工作流程中的实际应用。同时发布了完整的确定性回测配置以确保严格的可重复性。
...
### [125] [PrivGemo: Privacy-Preserving Dual-Tower Graph Retrieval for Empowering LLM Reasoning with Memory Augmentation](https://arxiv.org/abs/2601.08739)
*Xingyu Tan,Xiaoyang Wang,Qing Liu,Xiwei Xu,Xin Yuan,Liming Zhu,Wenjie Zhang*

Main category: cs.CL

TL;DR: PrivGemo是一个保护隐私的知识图谱增强推理框架，通过双塔设计和匿名化视图在保持本地原始知识的同时支持远程推理，解决了知识图谱隐私泄露问题。


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱隐私处理方法主要关注实体名称掩码，但仍面临结构泄露、不可控远程交互、脆弱的多跳多实体推理以及有限经验复用等四个局限性，需要更全面的隐私保护解决方案。

Method: 采用双塔设计保持原始知识图谱本地化，同时通过匿名化视图支持远程推理；通过检索连接所有主题实体的匿名化长跳路径支持多跳多实体推理；使用分层控制器和隐私感知经验内存减少不必要的探索和远程交互。

Result: 在六个基准测试上的综合实验显示，PrivGemo实现了整体最先进的结果，比最强基线高出最多17.1%；同时使较小模型（如Qwen3-4B）能够达到与GPT-4-Turbo相当的推理性能。

Conclusion: PrivGemo通过内存引导的暴露控制，提供了一个有效的隐私保护知识图谱增强推理框架，既保护了知识图谱隐私，又保持了高质量的推理性能。
...
### [126] [From Rows to Reasoning: A Retrieval-Augmented Multimodal Framework for Spreadsheet Understanding](https://arxiv.org/abs/2601.08741)
*Anmol Gulati,Sahil Sen,Waqar Sarguroh,Kevin Paul*

Main category: cs.CL

TL;DR: FRTR是一个多模态检索增强生成框架，专门用于处理大规模企业电子表格，通过细粒度嵌入和混合检索技术显著提升了LLM在复杂电子表格上的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型在处理包含数千行数字、多个链接工作表以及嵌入图表和收据等视觉内容的企业级电子表格时存在困难。现有的电子表格推理方法通常依赖单表压缩或全上下文编码，这限制了可扩展性，并且无法反映真实用户与复杂多模态工作簿的交互方式。

Method: 提出了FRTR（From Rows to Reasoning）框架，这是一个先进的多模态检索增强生成框架。它将Excel工作簿分解为细粒度的行、列和块嵌入，采用带有互惠排名融合（RRF）的混合词汇-密集检索，并集成多模态嵌入来同时推理数值和视觉信息。

Result: 在FRTR-Bench基准测试中，使用Claude Sonnet 4.5达到了74%的答案准确率，相比之前最先进方法的24%有显著提升。在SpreadsheetLLM基准测试中，使用GPT-5达到了87%的准确率，同时与上下文压缩方法相比减少了约50%的token使用量。

Conclusion: FRTR框架通过细粒度分解和混合检索策略，有效解决了LLM在大规模多模态企业电子表格上的推理挑战，显著提升了准确性和效率，为实际企业应用提供了可行的解决方案。
...
### [127] [Inferring Latent Intentions: Attributional Natural Language Inference in LLM Agents](https://arxiv.org/abs/2601.08742)
*Xin Quan,Jiafeng Xiong,Marco Valentino,André Freitas*

Main category: cs.CL

TL;DR: 该论文提出了Attributional NLI框架，将社会心理学原理融入自然语言推理，用于评估LLM在多智能体环境中的意图归因推理能力，并通过实验验证了神经符号方法在提升推理能力方面的优势。
...
Result: 实验显示归因推理能力存在明显层次：神经符号智能体始终表现最佳，平均胜率达到17.08%，显著优于其他智能体类型。

Conclusion: Att-NLI框架有助于开发具有复杂推理能力的智能体，同时突显了神经符号AI在构建多智能体环境中理性LLM智能体方面的潜在影响。
...
### [128] [TableCache: Primary Foreign Key Guided KV Cache Precomputation for Low Latency Text-to-SQL](https://arxiv.org/abs/2601.08743)
*Jinbo Su,Yuxuan Hu,Cuiping Li,Hong Chen,Jia Li,Lintao Ma,Jing Zhang*

Main category: cs.CL

TL;DR: TableCache：通过预计算表表示作为KV缓存并构建表Trie结构，显著提升Text-to-SQL任务的推理速度，TTFT最高加速3.62倍


<details>
  <summary>Details</summary>
Motivation: 现有LLM-based方法在Text-to-SQL任务中通常将完整数据库模式包含在提示中，导致上下文过长和预填充延迟增加。用户查询通常聚焦于重复的表集合，这为跨查询共享KV缓存提供了机会，但当前推理引擎在处理不同表顺序的用户查询时会产生冗余的前缀缓存副本。

Method: 1. 离线预计算表表示作为KV缓存，在线查询所需缓存；2. 在计算表缓存时保持表间主外键关系；3. 构建Table Trie结构以支持推理期间高效的KV缓存查找；4. 引入缓存管理系统，包括查询重排序策略提升缓存命中率和并行化模型推理与缓存加载的计算加载流水线。

Result: 实验结果表明，TableCache在Time to First Token (TTFT)上实现了最高3.62倍的加速，且性能下降可忽略不计。

Conclusion: 通过预计算表KV缓存、构建Table Trie结构和优化缓存管理，TableCache有效解决了Text-to-SQL任务中长上下文和冗余缓存复制的问题，显著提升了推理效率。
...
### [129] [Spatial Context Improves the Integration of Text with Remote Sensing for Mapping Environmental Variables](https://arxiv.org/abs/2601.08750)
*Valerie Zermatten,Chiara Vanalli,Gencer Sumbul,Diego Marcos,Devis Tuia*

Main category: cs.CL

TL;DR: 该研究提出了一种基于注意力的方法，将航空影像和地理定位文本在空间邻域内结合，用于预测环境变量，在EcoWikiRS数据集上显著优于单模态基线。


<details>
  <summary>Details</summary>
Motivation: 文本作为生态学新兴数据源，包含独特信息可与地理空间数据互补，提供传统数据源无法捕捉的局部环境信息。但文本数据在生态学中的贡献定义不清，可用性稀疏不规则，与地理空间数据整合困难。

Method: 提出基于注意力的方法，结合视觉和文本表示与地理位置编码，通过注意力模块动态选择对预测任务有用的空间邻居，整合多个邻近观测的贡献。

Result: 在EcoWikiRS数据集上评估预测103个环境变量的任务，该方法持续优于单位置或单模态（仅图像或仅文本）基线。按主题组分析变量时，在气候、土壤、人口和土地利用/土地覆盖变量上表现出显著性能提升。

Conclusion: 结合文本和图像数据时包含空间上下文具有显著优势，基于注意力的空间邻域整合方法能有效利用文本和图像数据的互补信息，提升环境变量预测性能。
...
### [130] [Multiplex Thinking: Reasoning via Token-wise Branch-and-Merge](https://arxiv.org/abs/2601.08808)
*Yao Tang,Li Dong,Yaru Hao,Qingxiu Dong,Furu Wei,Jiatao Gu*

Main category: cs.CL

TL;DR: Multiplex Thinking是一种随机软推理机制，通过在每个思考步骤中采样K个候选token并将它们的嵌入聚合成单个连续的多路复用token，从而在保持标准离散生成特性的同时实现紧凑的多重推理表示。


<details>
  <summary>Details</summary>
Motivation: 传统思维链(CoT)方法虽然能有效解决复杂推理任务，但会产生冗长的低带宽token序列。相比之下，人类通常通过维护可能下一步的概率分布来进行软推理。受此启发，研究团队希望开发一种既能保持推理能力又能减少序列长度的方法。

Method: 提出Multiplex Thinking方法：在每个推理步骤中，采样K个候选token，将它们的嵌入聚合成单个连续的多路复用token。这种方法保持了词汇嵌入先验和标准离散生成的采样动态，同时诱导出可处理的多路复用展开概率分布。多路复用轨迹可以直接通过策略强化学习进行优化。

Result: 在具有挑战性的数学推理基准测试中，Multiplex Thinking在Pass@1到Pass@1024的所有评估指标上，都一致优于强大的离散CoT和RL基线方法，同时产生更短的序列长度。

Conclusion: Multiplex Thinking是一种自适应的软推理机制，当模型置信度高时，多路复用token几乎离散化，行为类似标准CoT；当模型不确定时，它能紧凑地表示多个可能的下一步，而不增加序列长度。这种方法在推理性能和效率之间取得了良好平衡。
...
TL;DR: 提出MLLM-VADStory框架，利用领域知识指导多模态大语言模型，系统量化视频广告故事线理解并生成规模化洞察。


<details>
  <summary>Details</summary>
Motivation: 视频广告叙事通常由功能性意图构建，每个场景单元在几秒内执行不同的传播功能，传递产品和品牌信息。需要系统方法来理解和量化广告故事结构。

Method: 框架将广告分割为功能单元，使用新颖的广告特定功能角色分类法对每个单元功能进行分类，然后跨广告聚合功能序列以恢复数据驱动的故事线结构。

Result: 应用于4个行业子垂直领域的5万个社交媒体视频广告，发现基于故事的创意能提高视频留存率，并推荐表现最佳的故事弧以指导广告主创意设计。

Conclusion: 该框架展示了使用领域知识指导MLLMs生成视频广告故事线可扩展洞察的价值，使其成为理解视频创意的通用工具。
...
### [132] [Enhancing Portfolio Optimization with Deep Learning Insights](https://arxiv.org/abs/2601.07942)
*Brandon Luo,Jim Skufca*

Main category: q-fin.PM

TL;DR: 该研究聚焦深度学习投资组合优化，针对多资产、多头策略在不同市场周期中的挑战，提出使用预训练技术和Transformer架构来增强模型适应能力


<details>
  <summary>Details</summary>
Motivation: 传统投资组合优化方法在动态市场条件下面临挑战，特别是在有限市场周期数据的情况下，需要更适应性的深度学习策略来应对市场波动

Method: 采用预训练技术处理有限的市场周期数据，利用Transformer架构纳入状态变量，构建深度学习投资组合优化模型

Result: 与传统方法相比，提出的方法显示出有希望的结果，模型在波动市场中表现出更强的韧性

Conclusion: 深度学习驱动的投资组合优化领域正在不断发展，需要适应性策略来应对动态市场条件并提高预测准确性
...
### [133] [Optimal Option Portfolios for Student t Returns](https://arxiv.org/abs/2601.07991)
*Kyle Sung,Traian A. Pirvu*

Main category: q-fin.PM

TL;DR: 该论文为在收益率服从Student t分布下，方差和VaR最小化的最优期权组合提供了显式解，突破了传统正态分布的假设。


<details>
  <summary>Details</summary>
Motivation: 传统投资组合优化通常假设收益率服从正态分布，但现实中金融数据常呈现厚尾特征。该研究旨在解决在更现实的厚尾分布（Student t分布）下，方差和VaR最小化的最优期权组合构建问题。

Method: 提出了一种在Student t分布假设下获得最优期权组合的方法论，包括方差最小化和VaR最小化的显式解推导。

Result: 数值实验显示，方差最小化和VaR最小化的最优组合构成存在显著差异，表明考虑更现实的尾部风险设置可以导致更现实的投资组合配置。

Conclusion: 在Student t分布假设下，方差和VaR最小化的最优期权组合存在明显差异，这强调了在投资组合优化中考虑厚尾特征的重要性，能够产生更符合现实风险特征的投资配置。
...
### [134] [Feasibility-First Satellite Integration in Robust Portfolio Architectures](https://arxiv.org/abs/2601.08721)
*Roberto Garrone*

Main category: q-fin.PM

TL;DR: 该论文提出了一个面向小规模投资组合的可行性优先、非预测性卫星资产整合框架，通过四个嵌套的可行性层级（物理、经济、结构、认知）来约束卫星资产规模、换手率和广度，而不依赖收益预测或回测表现。


<details>
  <summary>Details</summary>
Motivation: 传统基于因子暴露、主观判断或回测表现的卫星资产整合方法适合机构规模，但不适用于小规模投资组合。小规模组合的主要约束来自固定成本、不可逆风险和治理复杂性，而非收益可预测性或交易能力，因此需要一种规模感知的可行性优先框架。

Method: 开发了一个四层嵌套的可行性框架：1) 物理可行性：确保在凹性市场冲击定律下的可实施性；2) 经济可行性：通过成本主导阈值约束抑制噪声主导的再分配；3) 结构可行性：通过可容忍损失下的明确期权预算来约束卫星资产规模；4) 认知可行性：通过基于熵的复杂性预算限制卫星资产的广度和分散度。

Result: 该框架得出了卫星资产规模、换手率和广度的闭式可行性边界，无需依赖收益预测、因子溢价或回测表现。将结构性期权确定为卫星资产设计的主要原则，其他层级作为稳健性透镜而非优化标准。

Conclusion: 该框架为将主题卫星资产整合到小型、稳健性导向的投资组合中提供了有纪律的基础，特别适合小规模投资组合，其中主要约束来自固定成本、不可逆风险和治理复杂性而非收益可预测性。
...
### [135] [Application of Ideal Observer for Thresholded Data in Search Task](https://arxiv.org/abs/2601.07976)
*Hongwei Lin,Howard C. Gifford*

Main category: eess.IV

TL;DR: 提出了一种基于阈值视觉搜索的拟人化模型观察者，通过选择性处理高显著性特征来改进图像质量评估性能，在噪声环境中尤其有效。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够模拟人类视觉系统的任务型图像质量评估模型，通过选择性处理高显著性特征来提升诊断准确性和计算效率，同时解决模型训练中数据有限的问题。

Method: 采用两阶段框架：候选区域选择和决策制定。在候选选择阶段使用阈值化数据精确定位感兴趣区域，通过阶段特定的特征处理优化性能。通过模拟实验评估阈值化对特征图、候选定位和多特征场景的影响。

Result: 阈值化通过排除低显著性特征显著提高了观察者性能，特别是在噪声环境中。中间阈值通常优于无阈值处理，表明仅保留相关特征比保留所有特征更有效。模型能够在较少图像的情况下有效训练，同时保持与人类性能的一致性。

Conclusion: 提出的新颖框架能够预测临床实际任务中的人类视觉搜索性能，并为资源有限条件下的模型观察者训练提供解决方案。该方法在计算机视觉、机器学习、国防安全图像分析等领域具有广泛应用前景。
...
### [136] [A Single-Parameter Factor-Graph Image Prior](https://arxiv.org/abs/2601.08749)
*Tianyang Wang,Ender Konukoglu,Hans-Andrea Loeliger*

Main category: eess.IV

TL;DR: 提出了一种具有分段常数局部参数的自适应图像模型，使用因子图和NUP先验，通过共轭梯度步和高斯消息传递进行迭代计算，应用于去噪和对比度增强。


<details>
  <summary>Details</summary>
Motivation: 传统图像模型往往使用固定参数，无法适应图像不同区域的特性。本文旨在开发一种能够自动适应图像局部特性的分段平滑模型，以提高图像处理效果。

Method: 使用因子图框架，引入NUP（参数未知的正态分布）先验，构建分段平滑图像模型。算法采用共轭梯度迭代和高斯消息传递相结合的计算方法。

Result: 提出的模型能够自动适应图像局部特性，在去噪和对比度增强应用中表现出良好性能，相比传统方法有显著改进。

Conclusion: 提出的自适应分段平滑图像模型在因子图框架下有效实现，通过NUP先验和高效算法，在图像处理任务中展现出优越性能。
...
### [137] [M3CoTBench: Benchmark Chain-of-Thought of MLLMs in Medical Image Understanding](https://arxiv.org/abs/2601.08758)
*Juntao Jiang,Jiangning Zhang,Yali Bi,Jinsheng Bai,Weixuan Liu,Weiwei Jin,Zhucun Xue,Yong Liu,Xiaobin Hu,Shuicheng Yan*

Main category: eess.IV

TL;DR: 该论文提出了M3CoTBench基准测试，专门用于评估多模态大语言模型在医学图像理解中的思维链推理能力，填补了现有医学图像基准只关注最终答案而忽略推理过程的空白。


<details>
  <summary>Details</summary>
Motivation: 当前医学图像理解基准主要关注最终答案而忽略推理路径，这种不透明的过程缺乏可靠的判断依据，难以辅助医生诊断。思维链推理与临床思维过程自然契合，但缺乏专门评估医学领域多模态思维链推理的基准。

Method: 构建了M3CoTBench基准，包含：1）涵盖24种检查类型的多样化多难度数据集；2）13个不同难度任务；3）专门针对临床推理的思维链评估指标（正确性、效率、影响、一致性）；4）对多个多模态大语言模型的性能分析。

Result: M3CoTBench系统评估了多种医学成像任务中的思维链推理，揭示了当前多模态大语言模型在生成可靠且临床可解释推理方面的局限性。

Conclusion: 该基准旨在促进开发透明、可信且诊断准确的医疗AI系统，填补了医学图像理解中思维链推理评估的空白，为未来研究提供了重要工具。
...
### [138] [Bridging the Trust Gap: Clinician-Validated Hybrid Explainable AI for Maternal Health Risk Assessment in Bangladesh](https://arxiv.org/abs/2601.07866)
*Farjana Yesmin,Nusrat Shirmin,Suraiya Shabnam Bristy*

Main category: cs.AI

TL;DR: 本研究提出了一种结合模糊逻辑和SHAP解释的混合可解释AI框架，用于孕产妇健康风险预测，在孟加拉国资源受限环境中验证了其临床接受度和信任度。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习在孕产妇健康风险预测方面有潜力，但在资源受限的临床环境中缺乏可解释性和信任度成为关键障碍，阻碍了临床采用。

Method: 开发了结合先验模糊逻辑和后验SHAP解释的混合可解释AI框架，使用模糊-XGBoost模型在1,014份孕产妇健康记录上进行训练，并通过孟加拉国14名医疗专业人员的系统性反馈进行验证。

Result: 模型达到88.67%准确率（ROC-AUC：0.9703）；验证研究显示71.4%的医疗专业人员偏好混合解释，54.8%表示对临床使用信任；SHAP分析确定医疗可及性是主要预测因子，模糊风险评分排名第三。

Conclusion: 结合可解释的模糊规则与特征重要性解释能增强实用性和信任度，为孕产妇医疗保健中的可解释AI部署提供实用见解，同时识别了产科病史、孕龄和连接障碍等关键差距。
...
### [139] [When Models Know When They Do Not Know: Calibration, Cascading, and Cleaning](https://arxiv.org/abs/2601.07965)
*Chenjie Hao,Weyl Lu,Yuko Ishiwaka,Zengyi Li,Weier Wan,Yubei Chen*

Main category: cs.AI

TL;DR: 提出一种无需训练的通用方法，通过模型校准、级联和数据清洗来增强模型识别自身未知的能力，从而提高AI系统的效率、可靠性和可信度。


<details>
  <summary>Details</summary>
Motivation: 当模型能够识别自身不知道什么时，会带来许多可能性。核心问题是如何让模型意识到自己的未知，一个可行的方法是利用模型内部信号计算置信度来反映其无知程度。

Method: 提出简单、有效、无需训练的通用方法，适用于视觉和语言模型。基于两个关键观察：1) 单个模型内更高置信度对应更高准确率；2) 在验证集上校准的模型在测试集上仍保持校准。在此基础上引入两种应用：1) 基于校准优势路由的模型级联；2) 基于模型集成和校准置信度的数据清洗方法。

Result: 通过校准置信度的可比性，级联大小模型可在几乎不损失准确率的情况下提高效率；级联两个规模相当的模型可获得超越任一单独模型的性能。基于多专家和校准置信度的数据清洗方法能有效识别ImageNet和MMLU数据集中的错误标注样本，平衡了精确度和检测率。

Conclusion: 让模型能够识别自身不知道什么是实现更高效、可靠和可信AI的实用步骤。校准置信度提供了可靠且可比较的信号，支持模型级联和数据清洗等应用。
...
### [140] [Reasoning over Precedents Alongside Statutes: Case-Augmented Deliberative Alignment for LLM Safety](https://arxiv.org/abs/2601.08000)
...
TL;DR: 论文提出CADA方法，通过案例增强的审慎对齐来提升开源大语言模型的安全性，避免过度拒绝良性请求，在保持实用性的同时增强无害性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在遵循安全原则的同时避免拒绝良性请求仍面临挑战。OpenAI的审慎对齐方法依赖详细的"类代码"安全规则，但这种方法在推理能力有限的开源模型中效果未得到充分研究。需要探索更有效的安全对齐方法。

Method: 提出CADA（案例增强的审慎对齐）方法：1）通过案例增强的简单代码而非详细的类代码规则进行训练；2）使用强化学习在自生成的安全推理链上进行优化；3）引导模型进行案例增强的推理而非僵化遵循枚举规则。

Result: CADA方法有效提升了无害性，增强了对抗攻击的鲁棒性，减少了过度拒绝现象，同时在多样化基准测试中保持了实用性。相比仅依赖规则的方法，案例增强的训练产生了更稳健和泛化的安全行为。

Conclusion: 案例增强的审慎对齐为提升大语言模型安全性提供了实用替代方案，能够在增强安全性的同时保持帮助性，避免规则僵化带来的局限性，实现更广泛的适应性。
...
### [141] [Internal Deployment Gaps in AI Regulation](https://arxiv.org/abs/2601.08005)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI监管主要关注面向外部用户的系统部署，但企业内部部署的高风险AI应用（如研发自动化、关键业务流程加速、敏感数据处理）可能逃避监管，需要特别关注。

Method: 通过分析美国和欧盟2025年前沿AI法规，识别企业内部AI系统监管的三个漏洞：范围模糊性、一次性合规评估、信息不对称，并分析其根源和解决方案。

Result: 发现了三个监管漏洞：1）范围模糊允许内部系统逃避监管义务；2）一次性合规评估无法捕捉内部系统的持续演化；3）信息不对称削弱监管意识和监督能力。

Conclusion: 需要针对企业内部AI部署制定更明确的监管框架，解决可测量性、激励机制和信息获取等核心问题，使相关政策选择更加审慎而非偶然。
...
### [142] [Integrating Attendance Tracking and Emotion Detection for Enhanced Student Engagement in Smart Classrooms](https://arxiv.org/abs/2601.08049)
...
Result: 在DAiSEE数据集上的实验评估显示，情绪分类准确率达到89.5%，考勤数据与情绪分析的结合能为教师提供课堂动态的额外洞察

Conclusion: 整合考勤数据与情绪分析能为教师提供更全面的课堂动态洞察，支持更及时响应的教学实践，有助于提升教学效果
...
### [143] [Forecast Aware Deep Reinforcement Learning for Efficient Electricity Load Scheduling in Dairy Farms](https://arxiv.org/abs/2601.08052)
*Nawazish Alia,Rachael Shawb,Karl Mason*

Main category: cs.AI

TL;DR: 本文提出了一种用于乳制品农场能源管理的深度强化学习框架，通过改进PPO算法实现电池储能和热水系统的智能调度，在真实农场数据上验证了成本节约效果。


<details>
  <summary>Details</summary>
Motivation: 乳制品农场是能源密集型行业，严重依赖电网供电。随着可再生能源整合增加，需要可持续能源管理来减少电网依赖并支持联合国可持续发展目标7。然而可再生能源的间歇性给实时供需平衡带来挑战，现有强化学习方法通常假设完全了解未来价格或发电情况，这在动态环境中不现实，且标准PPO变体依赖固定阈值导致训练不稳定。

Method: 提出深度强化学习框架用于乳制品农场的负载调度，重点关注电池储能和热水系统。开发了两种PPO变体：1) Forecast Aware PPO：使用基于小时和月份的残差校准方法整合短期需求与可再生能源预测；2) PID KL PPO：采用比例积分微分控制器自适应调节KL散度，实现稳定的策略更新。

Result: 在真实乳制品农场数据上训练，该方法比标准PPO降低1%电力成本，比DQN降低4.8%，比SAC降低1.5%。在电池调度方面，PPO减少电网输入13.1%，展示了在现代乳制品农场可持续能源管理中的可扩展性和有效性。

Conclusion: 该研究提出的深度强化学习框架通过改进PPO算法，有效解决了乳制品农场能源管理中的挑战，实现了成本节约和电网依赖减少，为农业领域的可持续能源管理提供了有效解决方案。
...
### [144] [A New Strategy for Verifying Reach-Avoid Specifications in Neural Feedback Systems](https://arxiv.org/abs/2601.08065)
...
Method: 开发新的算法计算后向可达集的过近似和欠近似，并将其与现有前向分析技术集成
...
### [145] [Semantic Gravity Wells: Why Negative Constraints Backfire](https://arxiv.org/abs/2601.08070)
*Shailesh Rana*

Main category: cs.AI

TL;DR: 论文发现大型语言模型在遵循"不要使用词X"这类否定指令时存在系统性失败，失败概率与语义压力呈逻辑关系，主要源于两种机制不同的失败模式


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在遵循否定指令（如"不要使用词X"）时频繁失败的现象，探究其背后的机制和条件

Method: 引入语义压力作为量化指标，使用logit lens技术进行分层分析，通过激活修补确定因果责任层，分析两种失败模式

Result: 发现违反概率与语义压力呈紧密逻辑关系（p=σ(-2.40+2.27·P₀)），识别出两种失败模式：启动失败（87.5%）和覆盖失败（12.5%），确定第23-27层是因果责任层

Conclusion: 否定约束设计存在根本性矛盾：明确提及禁止词汇反而会启动模型产生该词，揭示了语言模型在遵循否定指令时的系统性弱点
...
### [146] [MemoBrain: Executive Memory as an Agentic Brain for Reasoning](https://arxiv.org/abs/2601.08079)
*Hongjin Qian,Zhao Cao,Zheng Liu*

Main category: cs.AI

TL;DR: MemoBrain是一个面向工具增强型智能体的执行记忆模型，通过构建依赖感知的记忆系统来管理长时推理过程中的中间状态和逻辑关系，解决上下文积累问题。


<details>
  <summary>Details</summary>
Motivation: 工具增强型智能体框架中的复杂推理本质上是长时程的，导致推理轨迹和临时工具产物不断积累，超出大语言模型的有限工作上下文容量。缺乏显式记忆机制会破坏逻辑连续性并削弱任务对齐能力。
...
Result: 在具有挑战性的长时程基准测试（包括GAIA、WebWalker和BrowseComp-Plus）上评估MemoBrain，相比强基线模型展示出一致的性能提升。

Conclusion: 记忆不仅是辅助效率问题，而是维持长时程连贯、目标导向推理的核心组件。MemoBrain通过显式认知控制机制，实现了对推理轨迹的主动管理而非被动上下文积累。
...
### [147] [How vehicles change lanes after encountering crashes: Empirical analysis and modeling](https://arxiv.org/abs/2601.08125)
*Kequan Chen,Yuxuan Wang,Pan Liu,Victor L. Knoop,David Z. W. Wang,Yu Han*

Main category: cs.AI

TL;DR: 研究分析了交通事故后车辆变道行为（post crash lane changes），发现其与常规强制/自由变道相比持续时间更长、插入速度更低、碰撞风险更高，并开发了基于图注意力模块的轨迹预测框架，性能优于现有基线10%以上。


<details>
  <summary>Details</summary>
Motivation: 交通事故发生后，后续车辆需要变道绕过障碍物，但目标车道车辆可能拒绝让行，增加了变道复杂性和碰撞风险。然而，事故后变道的行为特征和运动模式尚不明确，需要填补这一研究空白。
...
Result: 1. 事故后变道持续时间更长、插入速度更低、碰撞风险更高；2. 79.4%的事故后变道涉及新跟随者至少一次不让行行为（DLCs为21.7%，MLCs为28.6%）；3. 提出的模型在轨迹预测性能上比现有基线提升10%以上（平均位移误差和最终位移误差）；4. 模型能提供更可靠的碰撞风险分析，降低误报率并提高冲突预测准确性。

Conclusion: 事故后变道具有独特的风险特征，特别是高比例的不让行行为。提出的交互感知轨迹预测框架能有效预测这类复杂场景下的车辆轨迹，为智能交通系统和自动驾驶的安全决策提供支持，且在不同场景下具有良好的可迁移性。
...
### [148] [Embedded AI Companion System on Edge Devices](https://arxiv.org/abs/2601.08128)
*Rahul Gupta,Stephen D. H. Hsu*

Main category: cs.AI

TL;DR: 提出一种在边缘设备上运行的AI伴侣系统，采用活动/非活动双阶段内存管理机制，在用户活动时进行低延迟对话，在用户空闲时进行内存提取和整合，实现在有限计算资源下的高效个性化AI伴侣。
...
Method: 提出交替活动/非活动阶段的内存范式：用户活动时执行低延迟实时对话，使用轻量级检索现有记忆和上下文；用户不活动时进行更计算密集的提取、整合和维护整个对话会话的记忆。

Result: 使用非常弱的模型（Qwen2.5-7B-Instruct量化int4）的系统在大多数指标上优于等效的无记忆原始LLM，并且与具有16k上下文窗口的GPT-3.5表现相当。

Conclusion: 该设计在嵌入式硬件的严格约束下最小化延迟，同时保持长期个性化，并引入了AI伴侣基准来全面评估对话质量和记忆能力。
...
### [149] [Project Synapse: A Hierarchical Multi-Agent Framework with Hybrid Memory for Autonomous Resolution of Last-Mile Delivery Disruptions](https://arxiv.org/abs/2601.08156)
...
<details>
  <summary>Details</summary>
Motivation: 最后一公里配送中断是物流行业的重要挑战，传统方法难以有效处理复杂、动态的配送问题。需要开发能够自主解决这些中断的智能系统，提高配送效率和客户满意度。

Method: 1. 采用分层多智能体架构：中央解决监督智能体进行战略任务分解，专业工作智能体负责战术执行
2. 使用LangGraph编排复杂和循环工作流
3. 从6000多个真实用户评论中定性分析，构建包含30个复杂中断场景的基准数据集
4. 采用LLM-as-a-Judge协议进行系统性能评估，并包含明确的偏见缓解机制

Result: 论文提出了Project Synapse框架，并建立了相应的评估体系。通过真实用户评论构建的基准数据集为系统验证提供了可靠基础，LLM-as-a-Judge协议确保了评估的客观性。

Conclusion: Project Synapse是一个有效的自主解决最后一公里配送中断的框架，其分层多智能体架构和LangGraph编排能够处理复杂工作流，基于真实数据的评估方法为系统性能验证提供了可靠途径。
...
### [150] [ZeroDVFS: Zero-Shot LLM-Guided Core and Frequency Allocation for Embedded Platforms](https://arxiv.org/abs/2601.08166)
...
<details>
  <summary>Details</summary>
Motivation: 现有DVFS和任务分配方法存在局限性：基于利用率的启发式方法忽略停顿时间，基于离线分析的方法需要大量配置且无法运行时适应。需要一种能够动态适应、考虑热管理和能效平衡的智能调度方案。

Method: 1) 分层多智能体强化学习框架，两个协作智能体分解指数级动作空间；2) 结合回归技术的精确环境模型预测热动态和性能状态；3) LLM语义特征提取，从OpenMP程序提取13个代码级特征；4) Dyna-Q启发框架，整合直接强化学习和基于模型的规划。

Result: 1) 后续决策延迟358ms，首次决策3.5-8.0秒（含一次性LLM特征提取）；2) 比Linux ondemand governor提升7.09倍能效和4.0倍完工时间；3) 首次决策延迟比基于表格的分析快8300倍；4) 比无模型方法收敛速度快20倍。

Conclusion: 提出的框架通过模型驱动的强化学习和LLM语义特征，实现了高效的热管理和能量感知调度，支持零样本部署到新工作负载，为动态嵌入式系统的实际部署提供了可行方案。
...
### [151] [Improving LLM Reasoning with Homophily-aware Structural and Semantic Text-Attributed Graph Compression](https://arxiv.org/abs/2601.08187)
*Zijun Di,Bin Lu,Huquan Kang,Luoyi Fu,Jiaxin Ding,Xiaoying Gan,Lei Zhou,Xinbing Wang,Chenghu Zhou*

Main category: cs.AI

TL;DR: HS2C框架通过利用图同质性，在结构和语义层面压缩文本属性图，提升LLM在图理解任务中的推理性能和压缩效率


<details>
  <summary>Details</summary>
Motivation: 现有方法受限于上下文窗口，通常采用随机采样丢弃节点/边，这会引入噪声并导致推理不稳定。作者认为图本身包含丰富的结构和语义信息，有效利用这些信息可以提升LLM的推理性能。

Method: 提出HS2C框架：1) 结构层面：基于结构熵最小化原则进行全局层次划分，识别自然凝聚的同质性社区，去除随机连接噪声；2) 语义层面：将检测到的结构同质性传递给LLM，使其能够基于预定义的社区类型进行差异化语义聚合，将冗余背景上下文压缩为简洁的社区级共识。

Result: 在10个节点级基准测试和7个图级基准测试中，HS2C通过向LLM提供结构和语义压缩的输入，同时提升了压缩率和下游推理准确率，验证了其优越性和可扩展性。

Conclusion: HS2C框架通过利用图同质性进行结构和语义压缩，有效提升了LLM在图理解任务中的性能，展示了在文本属性图理解中的潜力和通用性。
...
### [152] [Adapting Rules of Official International Mahjong for Online Players](https://arxiv.org/abs/2601.08211)
*Chucai Wang,Lingfeng Li,Yunlong Lu,Wenxin Li*

Main category: cs.AI

TL;DR: 该研究使用世界冠军AI进行自对弈分析，发现国际麻将在线单局游戏中存在先手优势和子目标计分问题，提出了补偿分机制和计分调整方案，使传统麻将规则更适合在线环境。


<details>
  <summary>Details</summary>
Motivation: 在线麻将玩家与线下玩家存在显著差异：在线玩家游戏时间碎片化、对手组合不固定，而线下玩家通常有固定对手进行多轮游戏。传统为线下设计的规则需要调整，以确保在线单局游戏的公平性。

Method: 使用世界冠军AI进行自对弈比赛，通过统计分析揭示游戏平衡性问题。具体分析先手优势和子目标计分设置的问题。

Result: 研究发现存在明显的先手优势，以及不同牌型子目标计分设置存在问题。相比传统通过多轮轮换位置来平衡先手优势的方法，提出的每轮补偿分机制更适合在线玩家。

Conclusion: 基于AI数据分析，提出了规则调整方案：引入先手优势补偿分机制，优化不同牌型子目标的计分设置。实现了修订版在线麻将游戏，为传统游戏适应在线环境提供了数据驱动的改进方法。
...
### [153] [An Axiomatic Approach to General Intelligence: SANC(E3) -- Self-organizing Active Network of Concepts with Energy E3](https://arxiv.org/abs/2601.08224)
*Daesuk Kwon,Won-gi Paeng*

Main category: cs.AI

TL;DR: SANC(E3)是一个理论框架，提出表征单元不是先验给定的，而是在有限激活容量下通过竞争选择、重建和压缩的稳定结果而自发涌现，通过最小化能量函数E3来统一感知、想象、预测、规划和行动。


<details>
  <summary>Details</summary>
Motivation: 现有智能系统通常预设固定的基本单元（如token、像素等），忽略了表征单元本身如何涌现和稳定的核心问题。需要建立一个理论框架，解释表征单元如何从经验中自组织形成，并统一各种认知功能。
...
Conclusion: SANC(E3)为理解表征单元如何从经验中涌现提供了一个原则性框架，将各种认知功能统一为格式塔完成过程，为构建更接近人类智能的自组织系统提供了理论基础。
...
### [154] [MPCI-Bench: A Benchmark for Multimodal Pairwise Contextual Integrity Evaluation of Language Model Agents](https://arxiv.org/abs/2601.08235)
...
<details>
  <summary>Details</summary>
Motivation: 随着语言模型从被动聊天机器人演变为处理个人数据的主动助手，评估其遵守社会规范变得日益重要。现有上下文完整性基准主要是文本中心且侧重于负面拒绝场景，忽略了多模态隐私风险和隐私与效用的基本权衡。

Method: 提出MPCI-Bench基准，包含从相同视觉源衍生的正负配对实例，分为三个层级：规范性种子判断、上下文丰富的故事推理和可执行的智能体行动轨迹。通过三原则迭代精炼流程确保数据质量。

Result: 对最先进多模态模型的评估显示，它们在平衡隐私与效用方面存在系统性失败，并存在明显的模态泄露差距——敏感视觉信息比文本信息泄露更频繁。

Conclusion: MPCI-Bench是首个用于评估智能体环境中隐私行为的多模态成对上下文完整性基准，将开源以促进未来智能体上下文完整性研究。
...
### [155] [The End of Reward Engineering: How LLMs Are Redefining Multi-Agent Coordination](https://arxiv.org/abs/2601.08237)
*Haoran Su,Yandong Sun,Congjia Yu*

Main category: cs.AI

TL;DR: 论文提出从手工设计数值奖励函数转向基于语言的目标规范，利用大语言模型从自然语言描述合成奖励函数，解决多智能体强化学习中奖励工程面临的挑战。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中，手动设计奖励函数面临信用分配模糊、环境非平稳性、交互复杂性组合增长等挑战，传统奖励工程方法难以应对这些困难。

Method: 利用大语言模型从自然语言描述直接合成奖励函数（如EUREKA），在线自适应调整奖励公式（如CARD），采用可验证奖励的强化学习范式，实现语义奖励规范、动态奖励适应和与人类意图的对齐。

Result: 语言介导的监督可以作为传统奖励工程的可行替代方案，协调可以从共享语义表示而非显式设计的数值信号中产生。

Conclusion: 论文概述了从手工奖励工程向基于语言的目标规范转变的研究方向，同时指出了计算开销、幻觉鲁棒性、大规模多智能体系统可扩展性等开放挑战。
...
### [156] [T3: Benchmarking Sycophancy and Skepticism in Causal Judgment](https://arxiv.org/abs/2601.08258)
*Edward Y. Chang*

Main category: cs.AI

TL;DR: T3是一个诊断基准，用于评估LLM在Pearl因果阶梯上的因果判断能力，包含454个专家策划的案例，重点分析失败模式，发现前沿模型存在"怀疑陷阱"和"非单调缩放悖论"两种病理现象。


<details>
  <summary>Details</summary>
Motivation: 需要严格评估大型语言模型在因果推理方面的能力，特别是在Pearl因果阶梯的不同层次上，以诊断模型在因果判断中存在的系统性缺陷和病理现象。

Method: 开发了T3基准测试，包含454个专家策划的案例，将性能分解为效用（敏感性）、安全性（特异性）和明智拒绝三个维度，应用于前沿模型进行诊断分析。

Result: 发现两种病理现象：1) L1层的"怀疑陷阱"（安全调优模型如Claude Haiku拒绝60%有效链接）；2) L3层的非单调缩放悖论（更大的GPT-5.2在模糊反事实上比GPT-4-Turbo低55分，主要因过度犹豫而非幻觉）。验证了过程验证协议RCA能恢复决定性因果判断。

Conclusion: T3基准能有效诊断LLM在因果判断中的系统性缺陷，揭示了安全调优和模型缩放可能带来的意外负面效应，为改进模型因果推理能力提供了诊断工具。
...
### [157] [Greedy Is Enough: Sparse Action Discovery in Agentic LLMs](https://arxiv.org/abs/2601.08280)
*Angshul Majumdar*

Main category: cs.AI

TL;DR: 论文研究大规模动作空间中动作发现的理论问题，提出基于结构化稀疏假设的贪婪算法，能在多项式样本复杂度下精确恢复相关动作集，为智能体系统的动作剪枝提供理论基础。


<details>
  <summary>Details</summary>
Motivation: 现代智能体系统（如工具增强语言模型）面临极大动作空间（数千个API），但实际部署中只有少量动作真正影响性能。这种观察促使研究者探索如何在大规模动作空间中高效发现相关动作。

Method: 提出上下文线性奖励模型，假设动作相关性遵循结构化稀疏性：只有少量动作在潜在状态中具有非零效应。将动作发现建模为块稀疏恢复问题，分析受正交匹配追踪启发的贪婪算法。

Result: 在不相干性、信号强度和动作覆盖的标准假设下，证明贪婪算法能以高概率精确恢复相关动作集，样本复杂度与稀疏度和潜在维度呈多项式关系，与总动作数仅呈对数关系。同时提供参数估计误差保证，证明所得决策规则对新潜在状态接近最优。

Conclusion: 稀疏动作发现是大型动作决策的基本原理，为智能体系统的动作剪枝提供了理论基础。信息理论下界表明稀疏性和充分覆盖是问题可处理性的必要条件。
...
### [158] [OpenMic: A Multi-Agent-Based Stand-Up Comedy Generation System](https://arxiv.org/abs/2601.08288)
...
<details>
  <summary>Details</summary>
Motivation: 中文单口喜剧生成需要文化背景的幽默、精准的节奏把控、舞台表演提示和隐式的多步推理。现有的中文幽默数据集更适合幽默理解和评估，而非长篇单口喜剧生成，导致直接监督与目标任务不匹配。

Method: 1. 构建基于AutoGen的端到端多智能体系统；2. 采用多轮迭代循环规划，让多个专业智能体共同优化幽默、节奏和可表演性；3. 使用检索增强生成(RAG)进行素材基础和创意扩展；4. 微调专门的JokeWriter模型以更好地内化单口喜剧特有的铺垫-笑点结构和长程呼应。

Result: 开发了OpenMic系统，能够将用户话题转化为完整的单口喜剧表演并生成视频内容。

Conclusion: 通过多智能体协作、RAG增强和专门的模型微调，OpenMic成功解决了中文单口喜剧生成中的文化背景、节奏把控和数据集任务不匹配等挑战。
...
### [159] [Thematic Working Group 5 -- Artificial Intelligence (AI) literacy for teaching and learning: design and implementation](https://arxiv.org/abs/2601.08380)
...
<details>
  <summary>Details</summary>
Motivation: 当前教师普遍缺乏AI知识和技能，无法有效将AI技术整合到教学中，需要系统性的策略来提升教师的AI素养和教学能力。

Method: 通过课程设计、专业发展项目、实际课堂应用探索以及政策指南制定等多维度策略，为教师提供全面的AI教育支持。

Result: 开发了系统的教师AI素养提升框架，包括课程资源、培训项目和实践指导，帮助教师获得必要的AI知识和技能。

Conclusion: 通过多方面的策略支持，可以有效提升教师的AI素养和教学能力，使他们能够自信地使用AI工具，并在学生中培养对AI概念的深入理解。
...
### [160] [A Qualitative Model to Reason about Object Rotations (QOR) applied to solve the Cube Comparison Test (CCT)](https://arxiv.org/abs/2601.08382)
*Zoe Falomir*

Main category: cs.AI

TL;DR: 本文提出了一个用于推理物体旋转的定性模型(QOR)，并将其应用于解决Ekstrom等人(1976)的立方体比较测试(CCT)。该模型通过构建描述立方体侧面特征旋转运动与位置变化、方向变化关系的概念邻域图(CNGRLO)，并生成组合表来计算旋转推理。


<details>
  <summary>Details</summary>
Motivation: 解决立方体比较测试(CCT)中关于物体旋转的推理问题，需要一种能够处理旋转运动、位置变化和方向变化的定性推理模型。

Method: 构建概念邻域图(CNGRLO)来描述旋转运动与位置变化、方向变化之间的关系，并基于此图生成组合表来进行旋转推理计算。

Result: 开发了QOR定性模型，成功应用于解决立方体比较测试，通过概念邻域图和组合表实现了对物体旋转的有效推理。

Conclusion: QOR模型为处理物体旋转推理问题提供了一种有效的定性方法，特别适用于解决立方体比较测试这类空间推理任务。
...
### [161] [Deconstructing Pre-training: Knowledge Attribution Analysis in MoE and Dense Models](https://arxiv.org/abs/2601.08383)
*Bo Wang,Junzhuo Li,Hong Chen,Yuanlin Chu,Yuxuan Fan,Xuming Hu*

Main category: cs.AI

TL;DR: MoE架构通过神经元级归因度量Gated-LPI揭示了稀疏模型与密集模型在预训练知识获取动态上的差异，发现MoE形成低熵骨干、早期稳定性和功能鲁棒性三大模式。


<details>
  <summary>Details</summary>
Motivation: 研究MoE架构在预训练期间如何塑造知识获取过程，以及这一过程与密集架构的差异，以理解稀疏架构的训练动态特性。

Method: 引入Gated-LPI（对数概率增加）神经元级归因度量，分解跨神经元的对数概率增加；对MoE和密集架构进行时间分辨比较，跟踪1.2M训练步骤（约5.0T令牌）和600K训练步骤（约2.5T令牌）的检查点。

Result: 发现三个关键模式：1）低熵骨干：MoE中约1%的神经元捕获超过45%的正向更新，形成高效用核心；2）早期巩固：MoE在<100K步内锁定稳定重要性分布，而密集模型在整个训练中保持波动；3）功能鲁棒性：屏蔽最重要的MoE注意力头仅降低关系HIT@10 <10%，而密集模型降低>50%，表明稀疏性促进分布式知识存储。
...
### [162] [Creativity in AI as Emergence from Domain-Limited Generative Models](https://arxiv.org/abs/2601.08388)
*Corina Chutaux*
...
<details>
  <summary>Details</summary>
Motivation: 当前AI创造力研究多采用评估框架衡量生成输出的新颖性、多样性或实用性，将创造力视为待评估属性而非待建模现象。随着大规模多模态生成系统展现复杂模式重组能力，需要从生成角度理解机器创造力的本质和限制。

Method: 提出生成视角的AI创造力框架，将其分解为四个相互作用组件：基于模式的生成、诱导世界模型、上下文基础、任意性。分析这些组件在多模态生成系统中的表现，关注生成动态与领域特定表征相互作用的结构和上下文条件。

Result: 建立了一个技术框架，将创造力视为AI系统中的涌现现象而非事后评估标签，为研究生成动态与领域特定表征相互作用如何产生创造性行为提供了概念基础。

Conclusion: 创造力应被视为有界信息环境中领域受限生成模型的涌现特性，这一生成视角为理解AI创造力提供了更本质的技术框架，有助于超越传统评估方法，从结构和上下文条件角度研究创造性行为的产生机制。
...
### [163] [Owen-Shapley Policy Optimization (OSPO): A Principled RL Algorithm for Generative Search LLMs](https://arxiv.org/abs/2601.08403)
*Abhijnan Nath,Alireza Bagheri Garakani,Tianchen Zhou,Fan Yang,Nikhil Krishnaswamy*

Main category: cs.AI

TL;DR: OSPO是一种基于Shapley-Owen归因的强化学习框架，通过将序列级奖励重新分配到语义单元来解决推荐系统中信用分配问题，无需额外价值模型。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的推荐方法（如GRPO）依赖稀疏的序列级奖励，导致信用分配问题，难以确定哪些具体token驱动成功。这在需要从模糊语言推断用户潜在意图的场景中尤为严重，因为这种推理模式在预训练中很少出现。

Method: 提出Owen-Shapley Policy Optimization (OSPO)框架，基于token对结果的边际贡献重新分配序列级优势。采用基于势的奖励塑造和Shapley-Owen归因方法，将信用分配到语义连贯的单元（如描述产品属性的短语或捕捉偏好的句子），同时保持最优策略，无需参数化价值模型。

Result: 在Amazon ESCI和H&M Fashion数据集上的实验显示，OSPO相比基线方法获得了一致的性能提升，并且在测试时对训练中未见过的分布外检索器表现出显著的鲁棒性。

Conclusion: OSPO通过基于Shapley-Owen归因的信用分配机制，有效解决了推荐系统中强化学习的信用分配问题，提高了模型性能和对未见检索器的鲁棒性，为个性化推荐任务提供了更精细的奖励信号。
...
### [164] [Hybrid Distillation with CoT Guidance for Edge-Drone Control Code Generation](https://arxiv.org/abs/2601.08412)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码生成任务中展现出巨大潜力，但模型的高资源消耗与无人机平台的实时轻量级需求存在矛盾。需要解决如何在资源受限的无人机平台上实现高效智能控制的问题。

Method: 1) 构建高质量无人机SDK数据集，包含指令-代码-推理链和反事实负样本；2) 使用DeepSeek-Coder-V2-Lite作为教师模型，采用混合黑盒白盒蒸馏策略生成思维链软标签，结合加权交叉熵损失将复杂推理能力迁移到学生模型；3) 通过针对无人机控制场景优化的提示调优工程增强模型性能。

Result: 蒸馏后的轻量级模型在保持高代码生成准确率的同时，在部署和推理效率方面实现显著提升，有效证明了该方法在实现无人机精准轻量级智能控制方面的可行性和优越性。

Conclusion: 该方法成功解决了大型模型资源消耗与无人机平台轻量级需求之间的矛盾，为资源受限的无人机平台提供了高效的智能控制解决方案，展示了知识蒸馏和思维链引导在特定领域应用中的有效性。
...
### [165] [RubricHub: A Comprehensive and Highly Discriminative Rubric Dataset via Automated Coarse-to-Fine Generation](https://arxiv.org/abs/2601.08430)
*Sunzhu Li,Jiale Zhao,Miteto Wei,Huimin Ren,Yang Zhou,Jingwen Yang,Shunyu Liu,Kaike Zhang,Wei Chen*

Main category: cs.AI

TL;DR: 提出RubricHub框架，通过从粗到细的评分标准生成方法，解决开放式生成任务中缺乏真实标签的问题，在多个领域实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有基于评分标准的评估方法存在可扩展性瓶颈和标准粗糙的问题，导致监督天花板效应。开放式生成任务缺乏真实标签，需要更精细的评估标准来捕捉细微差异。

Method: 提出自动化从粗到细的评分标准生成框架，结合原则引导合成、多模型聚合和难度演进，生成全面且高度区分的标准。基于此框架构建RubricHub大规模多领域数据集，并采用两阶段后训练流程：基于评分标准的拒绝采样微调(RuFT)和强化学习(RuRL)。

Result: RubricHub数据集包含约11万个样本，覆盖多个领域。后训练的Qwen3-14B模型在HealthBench上达到69.3分，超越了GPT-5等专有前沿模型，实现了最先进的性能。

Conclusion: 提出的RubricHub框架和两阶段后训练方法有效解决了开放式生成任务的评估和监督问题，显著提升了模型性能，在多个领域实现了SOTA结果。
...
### [166] [YaPO: Learnable Sparse Activation Steering Vectors for Domain Adaptation](https://arxiv.org/abs/2601.08441)
*Abdelaziz Bounhar,Rania Hossam Elmohamady Elbadry,Hadi Abdine,Preslav Nakov,Michalis Vazirgiannis,Guokan Shang*

Main category: cs.AI

TL;DR: YaPO是一种基于稀疏自编码器的参考无关方法，通过优化稀疏编码学习解耦的稀疏导向向量，实现更精细、稳定和高效的LLM对齐控制。


<details>
  <summary>Details</summary>
Motivation: 现有的密集导向向量由于神经元多语义性容易纠缠多个潜在因素，在需要区分密切相关的价值观和行为（如文化对齐）的细粒度场景中效果有限且不稳定。

Method: 提出YaPO方法，在稀疏自编码器的潜在空间中学习稀疏导向向量，通过优化稀疏编码产生解耦、可解释且高效的导向方向。

Result: YaPO比密集导向基线收敛更快、性能更强、训练稳定性更好，可泛化到幻觉、财富追求、越狱、权力追求等多种对齐相关行为，同时保持通用知识能力（MMLU无退化）。
...
### [167] [Beyond Linearization: Attributed Table Graphs for Table Reasoning](https://arxiv.org/abs/2601.08444)
*Yuxiang Wang,Junhao Gan,Shengxiang Gao,Shenghao Ye,Zhengyi Yang,Jianzhong Qi*

Main category: cs.AI

TL;DR: TABGR提出了一种基于图结构的表格推理方法，通过构建属性表图（ATG）保留表格结构，使用QG-PPR机制缓解信息丢失问题，在基准测试中比SOTA模型提升9.7%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的表格推理方法将表格线性化为纯文本，导致表格结构丢失、缺乏可解释的推理路径，并存在"中间丢失"问题。

Method: 提出Table Graph Reasoner (TABGR)：1) 将表格表示为属性表图（ATG），显式保留行列单元格结构；2) 提出问题引导的个性化PageRank（QG-PPR）机制重新排序表格数据，缓解中间丢失问题。

Result: 在两个常用基准测试中，TABGR始终优于最先进模型，准确率提升高达9.7%。

Conclusion: TABGR通过图表示和QG-PPR机制有效解决了现有表格推理方法的局限性，在保持可解释性的同时显著提升了性能。
...
### [168] [An Under-Explored Application for Explainable Multimodal Misogyny Detection in code-mixed Hindi-English](https://arxiv.org/abs/2601.08457)
*Sargam Yadav,Abhishek Kaushik,Kevin Mc Daid*

Main category: cs.AI

TL;DR: 开发了一个多模态可解释的Web应用，用于检测印地语-英语混合文本和表情包中的厌女内容，采用Transformer模型和SHAP/LIME可解释性技术。


<details>
  <summary>Details</summary>
Motivation: 数字平台用户增长迅速，但仇恨言论和厌女内容也随之扩散。现有AI模型在低资源语言和混合语言上研究不足，且缺乏可解释性，而可解释AI对敏感的仇恨言论检测至关重要。

Method: 开发多模态可解释Web应用：1) 文本检测使用XLM-RoBERTa和mBERT模型，训练于4,193条评论数据集；2) 表情包检测使用mBERT+EfficientNet和mBERT+ResNet模型，训练于4,218个表情包数据集；3) 使用SHAP和LIME提供特征重要性解释；4) 通过Chatbot Usability Questionnaire和User Experience Questionnaire进行人工评估。

Result: 构建了一个完整的Web应用系统，能够检测混合语言文本和表情包中的厌女内容，并提供模型决策的可解释性分析。系统已通过人工评估验证其可用性。
...
### [169] [SUMMPILOT: Bridging Efficiency and Customization for Interactive Summarization System](https://arxiv.org/abs/2601.08475)
*JungMin Yun,Juhwan Choi,Kyohoon Jin,Soojin Jang,Jinhee Jang,YoungBin Kim*

Main category: cs.AI

TL;DR: SummPilot是一个基于交互的可定制摘要系统，结合自动摘要效率与个性化需求，通过大语言模型实现自动和交互式摘要，用户可通过语义图、实体聚类等组件定制摘要。


<details>
  <summary>Details</summary>
Motivation: 解决传统自动摘要系统缺乏个性化定制能力的问题，满足用户根据个人兴趣和需求定制摘要的需求，提升摘要的实用性和用户满意度。

Method: 开发SummPilot系统，利用大语言模型支持自动和交互式摘要，提供语义图、实体聚类、可解释评估等交互组件，让用户参与摘要定制过程。

Result: 演示和用户研究表明SummPilot系统具有适应性和实用性，能够有效支持可定制摘要任务，满足不同用户的个性化需求。

Conclusion: SummPilot成功结合了自动摘要效率与个性化定制能力，通过交互式设计为用户提供了灵活、可解释的摘要定制方案，在可定制摘要领域具有实用价值。
...
### [170] [What If TSF: A Benchmark for Reframing Forecasting as Scenario-Guided Multimodal Forecasting](https://arxiv.org/abs/2601.08509)
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测方法大多是单模态的，仅依赖历史模式外推。虽然大语言模型在多模态预测方面显示出潜力，但现有基准主要提供回顾性或不对齐的原始上下文，无法确定模型是否真正利用了文本输入。人类专家在实践中会结合历史证据和假设场景，在不同场景下对相同观测产生不同的预测。

Method: 引入WIT（What If TSF）多模态预测基准，通过提供专家构建的合理或反事实场景，评估模型是否能基于上下文文本（特别是未来场景）进行预测。该基准旨在为场景引导的多模态预测提供严格的测试平台。

Result: 提出了WIT基准，并已在GitHub上开源（https://github.com/jinkwan1115/WhatIfTSF），为研究社区提供了评估多模态时间序列预测模型的新工具。

Conclusion: WIT基准填补了现有时间序列预测评估的空白，通过引入场景引导的预测任务，能够更准确地评估多模态模型是否真正利用了文本上下文，特别是未来假设场景，从而推动更接近人类专家决策的预测模型发展。
...
### [171] [Sketch-Based Facade Renovation With Generative AI: A Streamlined Framework for Bypassing As-Built Modelling in Industrial Adaptive Reuse](https://arxiv.org/abs/2601.08531)
*Warissara Booranamaitree,Xusheng Du,Yushu Cai,Zhengyang Wang,Ye Zhang,Haoran Xie*

Main category: cs.AI

TL;DR: 提出结合生成式AI和视觉语言模型的三阶段框架，直接处理粗略结构草图和文本描述生成建筑立面改造方案，避免传统详细建模流程。


<details>
  <summary>Details</summary>
Motivation: 建筑立面改造比完全拆除更可持续，但现有工作流程需要详细的竣工建模，耗时耗力且需要反复修改，需要更高效的方案生成方法。

Method: 三阶段框架：1）微调VLM模型预测需要修改的区域和组件；2）稳定扩散模型生成新元素草图并与原轮廓融合；3）ControlNet细化生成逼真图像。

Result: 在数据集和真实工业建筑上的实验表明，该框架能生成保留原始结构同时提升立面细节质量的改造方案，有效绕过详细建模需求。

Conclusion: 该方法使建筑师能快速探索设计替代方案、迭代早期概念，并以更清晰的方式传达改造意图，提高设计效率。
...
### [172] [Learner-Tailored Program Repair: A Solution Generator with Iterative Edit-Driven Retrieval Enhancement](https://arxiv.org/abs/2601.08545)
*Zhenlong Dai,Zhuoluo Zhao,Hengning Wang,Xiu Tang,Sai Wu,Chang Yao,Zhipeng Gao,Jingyuan Chen*

Main category: cs.AI

TL;DR: 本文提出了一种针对编程学习者的个性化程序修复框架LPR，通过检索增强和迭代优化方法，不仅能修复错误代码，还能提供错误原因解释。


<details>
  <summary>Details</summary>
Motivation: 现有智能编程辅导系统大多只关注修复学习者的错误代码，而不提供错误背后的根本原因。为了解决这一不足，本文提出了LPR（学习者定制程序修复）任务，旨在同时修复代码并提供错误描述。

Method: 提出LSG框架，包含两阶段方法：第一阶段使用修复解决方案检索框架构建检索数据库，采用编辑驱动的代码检索方法获取有价值的解决方案；第二阶段提出解决方案引导的程序修复方法，在检索解决方案的指导下修复代码并提供解释。此外，还提出了迭代检索增强方法，利用生成代码的评估结果迭代优化检索方向。

Result: 实验结果表明，该方法在一系列基线模型上取得了显著优势，验证了所提框架在新LPR任务上的有效性。

Conclusion: 本文提出的LSG框架能够有效解决编程学习者定制程序修复任务，不仅修复错误代码，还能提供错误解释，为智能编程辅导系统提供了更全面的解决方案。
...
### [173] [WaterCopilot: An AI-Driven Virtual Assistant for Water Management](https://arxiv.org/abs/2601.08559)
...
<details>
  <summary>Details</summary>
Motivation: 跨界河流流域的水资源管理面临数据碎片化、实时访问受限以及多样化信息源整合复杂等挑战，需要开发统一平台来弥合这些差距。

Method: 基于检索增强生成(RAG)和工具调用架构，开发了两个自定义插件：iwmi-doc-plugin用于文档语义搜索，iwmi-api-plugin用于查询实时数据库，支持多语言交互、透明来源引用和可视化功能。

Result: 使用RAGAS框架评估，总体得分0.8043，其中答案相关性0.8571，上下文精确度0.8009。系统实现了自动阈值警报、与LRB数字孪生集成，并建立了可扩展的AWS部署管道。

Conclusion: WaterCopilot建立了一个可复制的AI增强框架，用于改善数据稀缺的跨界环境中的水治理，展示了AI助手支持及时决策和加强复杂河流流域水安全的潜力。
...
### [174] [ViDoRe V3: A Comprehensive Evaluation of Retrieval Augmented Generation in Complex Real-World Scenarios](https://arxiv.org/abs/2601.08620)
...
<details>
  <summary>Details</summary>
Motivation: 现有的RAG基准测试无法捕捉真实场景的复杂性，往往只关注文本数据、单文档理解，或将检索和生成分开评估。实际RAG管道需要处理视觉元素（表格、图表、图像）、跨文档信息综合和准确来源定位等挑战。

Method: 构建了ViDoRe v3基准测试，包含10个专业领域数据集，约26,000个文档页面和3,099个人工验证的查询，支持6种语言。通过12,000小时的人工标注工作，提供了检索相关性、边界框定位和验证参考答案的高质量标注。

Result: 评估显示：视觉检索器优于文本检索器；后期交互模型和文本重排序显著提升性能；混合或纯视觉上下文提高答案生成质量。但当前模型在处理非文本元素、开放式查询和细粒度视觉定位方面仍有困难。

Conclusion: ViDoRe v3为多模态RAG研究提供了全面的评估框架，揭示了当前系统的局限性，特别是在处理视觉元素和复杂查询方面。该基准测试采用商业友好许可发布，旨在推动解决这些挑战的进展。
...
### [175] [Resisting Manipulative Bots in Memecoin Copy Trading: A Multi-Agent Approach with Chain-of-Thought Reasoning](https://arxiv.org/abs/2601.08641)
...
<details>
  <summary>Details</summary>
Motivation: 模因币跟单交易虽然流行但存在盈利不确定性，主要面临操纵性机器人、被跟随钱包未来表现不确定性和交易执行延迟等问题。同时，单一LLM在处理复杂的资产分配等任务时存在困难，特别是在加密货币领域缺乏足够的领域知识。

Method: 受资产管理团队结构启发，将复杂任务分解为子任务，协调专业智能体协作解决。采用少样本思维链提示，使每个智能体获取专业模因币交易知识、解释多模态数据并生成可解释决策。

Result: 在1000个模因币项目的交易数据集上，多智能体系统在识别高质量模因币项目和关键意见领袖钱包方面的精确度分别达到73%和70%，选出的KOL在这些项目中总共创造了50万美元的利润。

Conclusion: 提出的可解释多智能体系统能有效解决模因币跟单交易中的挑战，通过任务分解和专业智能体协作，在识别优质项目和KOL方面表现出色，为加密货币市场的智能交易提供了新思路。
...
### [176] [From Classical to Quantum Reinforcement Learning and Its Applications in Quantum Control: A Beginner's Tutorial](https://arxiv.org/abs/2601.08662)
*Abhijit Sen,Sonali Panda,Mahima Arya,Subhajit Patra,Zizhan Zheng,Denys I. Bondar*

Main category: cs.AI

TL;DR: 这是一个面向本科生的强化学习入门教程，通过实例驱动的清晰解释，帮助学生在理论和实践之间建立桥梁。


<details>
  <summary>Details</summary>
Motivation: 让强化学习对本科生更加易于理解，解决学生从概念理解到实际编码应用之间的常见挑战，弥合理论与实践的差距。

Method: 采用清晰的、实例驱动的解释方法，通过动手实践的例子和易于理解的讲解，帮助学生建立基础技能。

Result: 该教程旨在使学生能够更自信地在实际场景中应用强化学习技术，提供了从理论到实践的有效学习路径。

Conclusion: 通过这个教程，本科生可以获得强化学习的坚实基础，更好地理解和应用强化学习技术解决实际问题。
...
### [177] [Parallel Context-of-Experts Decoding for Retrieval Augmented Generation](https://arxiv.org/abs/2601.08670)
*Giulio Corallo,Paolo Papotti*

Main category: cs.AI

TL;DR: Pced是一种无需训练的并行解码框架，通过检索感知对比解码规则同步多个文档"专家"的预测，解决了RAG中多文档推理与速度的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成面临一个权衡：将多个文档拼接成长提示可以实现多文档推理，但会导致预填充瓶颈；而分别编码文档KV缓存虽然速度快，却破坏了跨文档交互能力。

Method: 提出并行专家上下文解码(Pced)框架，将检索到的文档视为独立的"专家"，通过新颖的检索感知对比解码规则同步它们的预测，该规则根据模型先验对专家logits进行加权。

Result: 这种方法恢复了跨文档推理能力，而无需构建跨文档的共享注意力机制。

Conclusion: Pced通过将证据聚合从注意力机制转移到解码过程，解决了RAG中多文档推理与计算效率之间的根本矛盾。
...
### [178] [Why AI Alignment Failure Is Structural: Learned Human Interaction Structures and AGI as an Endogenous Evolutionary Shock](https://arxiv.org/abs/2601.08673)
*Didier Sornette,Sandro Claudio Lera,Ke Wu*

Main category: cs.AI

TL;DR: 论文认为大型语言模型表现出的欺骗、威胁等行为不是模型对齐失败或恶意意图的证据，而是对人类社会互动记录（包括权力不对称关系）的结构性泛化，反映了人类道德本身的多元性和矛盾性。


<details>
  <summary>Details</summary>
Motivation: 针对当前将LLMs表现出欺骗、威胁等行为解释为对齐失败或恶意意图的观点，作者认为这种解释存在概念错误，需要重新理解这些行为的本质和AGI的真正风险。

Method: 基于关系模型理论，分析LLMs如何统计性地内化人类社会互动记录，将勒索等行为视为权力、信息或约束极端不对称下互动机制的结构性泛化，而非正常社会行为的范畴偏差。

Result: LLMs的"不道德"行为反映了人类道德本身的多元性、情境依赖性和历史偶然性，AGI的主要风险不是对抗性意图，而是作为人类智力、权力和矛盾的内生放大器。

Conclusion: 对齐失败是结构性的而非偶然的，需要治理方法关注放大效应、复杂性和制度稳定性，而非仅仅关注模型层面的意图。普遍道德的人工智能概念本身定义不清。
...
### [179] [PersonaDual: Balancing Personalization and Objectivity via Adaptive Reasoning](https://arxiv.org/abs/2601.08679)
*Xiaoyou Liu,Xinyi Mou,Shengbin Yue,Liang Wang,Yuqing Wang,Qiexiang Wang,Tianrui Qin,Wangchunshu Zhou,Zhongyu Wei*

Main category: cs.AI

TL;DR: PersonaDual框架让单个LLM同时支持客观推理和个性化推理，通过上下文自适应切换模式，在保持个性化优势的同时减少干扰


<details>
  <summary>Details</summary>
Motivation: 用户期望LLM能符合个人偏好，但个性化信息可能损害客观性和事实准确性，特别是当个性化信息与问题不匹配时。需要平衡个性化好处与客观性需求。

Method: 提出PersonaDual框架，首先通过SFT训练学习两种推理模式（客观和个性化），然后通过强化学习优化模式选择，使用DualGRPO算法改进切换决策。

Result: 在客观和个性化基准测试中，PersonaDual在保持个性化优势的同时显著减少干扰，实现接近无干扰的性能，并能更好地利用有帮助的个性化信号改进客观问题解决。

Conclusion: PersonaDual框架成功解决了个性化与客观性之间的权衡问题，通过自适应模式切换机制，在单个模型中实现了两种推理能力的平衡与协同。
...
### [180] [All Required, In Order: Phase-Level Evaluation for AI-Human Dialogue in Healthcare and Beyond](https://arxiv.org/abs/2601.08690)
*Shubham Kulkarni,Alexander Lyzhov,Shiva Chaitanya,Preetam Joshi*

Main category: cs.AI

TL;DR: OIP-SCE是一种评估对话式AI在临床工作中合规性的新方法，通过检查每个必需临床义务是否按正确顺序完成并提供清晰证据，使复杂规则变得实用且可审计。


<details>
  <summary>Details</summary>
Motivation: 当前大多数评估方法忽略了合规性如何依赖于完整对话过程，导致技术进展与医疗实际需求之间存在差距。需要一种方法能够确保AI系统在临床对话中满足所有必需义务，并提供可审计的证据。

Method: OIP-SCE（强制性信息阶段结构化合规评估）方法检查每个必需的临床义务是否被满足，确保按正确顺序执行，并为临床医生提供清晰的审查证据。该方法将复杂规则转化为可操作步骤，使政策变得可共享和执行。

Result: 在两个案例研究（呼吸病史采集和福利验证）中展示了OIP-SCE的有效性。阶段级别的证据将政策转化为共享、可操作的步骤，为临床医生提供检查控制权，为工程师提供清晰的实施规范。

Conclusion: OIP-SCE提供了一个单一、可审计的评估界面，使AI能力与临床工作流程保持一致，支持常规、安全的使用。通过将复杂规则转化为实用且可审计的形式，弥合了技术进步与医疗实际需求之间的差距。
...
### [181] [Evaluating the Ability of Explanations to Disambiguate Models in a Rashomon Set](https://arxiv.org/abs/2601.08703)
*Kaivalya Rawal,Eoin Delaney,Zihao Fu,Sandra Wachter,Chris Russell*

Main category: cs.AI

TL;DR: 该论文提出了一种无需真实标签即可评估模型解释质量的新方法AXE，特别针对Rashomon集合中模型解释的评估问题，能够检测对抗性公平漂白行为。


<details>
  <summary>Details</summary>
Motivation: 在Rashomon集合中，多个模型性能相似但内部机制不同，传统解释评估方法依赖于与真实解释的比较，这掩盖了模型间的行为差异，且无法有效评估解释质量。

Method: 提出AXE方法，基于三个解释评估原则：1）不依赖真实解释；2）能够揭示Rashomon集合中模型的行为差异；3）能够检测对抗性公平漂白。该方法通过评估特征重要性解释的质量来实现这些目标。

Result: AXE方法能够以100%的成功率检测对抗性公平漂白行为，相比基于模型敏感性或真实解释比较的传统方法，AXE能够确定受保护属性是否被用于预测。

Conclusion: AXE提供了一种无需真实标签的模型解释评估框架，能够有效揭示Rashomon集合中模型的行为差异，帮助选择适合部署的模型，并检测对抗性公平漂白问题。
...
### [182] [Learning from Demonstrations via Capability-Aware Goal Sampling](https://arxiv.org/abs/2601.08731)
*Yuanlin Duan,Yuning Wang,Wenjie Qiu,He Zhu*

Main category: cs.AI

TL;DR: Cago是一种基于能力感知的目标采样方法，通过动态追踪智能体在专家轨迹上的能力，选择刚好超出当前能力范围的中间目标来引导学习，从而解决模仿学习在长时程任务中的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在长时程环境中经常失败，因为完美复制演示不现实，小错误会累积导致灾难性后果。现有方法仅将演示用于策略初始化或奖励塑形，无法有效应对智能体能力与专家轨迹之间的差距。

Method: Cago（能力感知目标采样）方法动态追踪智能体在专家轨迹上的能力，利用这一信号选择刚好超出智能体当前能力范围的中间步骤（目标）来引导学习，形成自适应课程，逐步解决完整任务。

Result: 实验结果表明，Cago在稀疏奖励、目标条件任务中显著提高了样本效率和最终性能，一致优于现有的从演示学习方法基线。

Conclusion: Cago通过能力感知的目标采样机制，有效缓解了直接模仿学习对专家轨迹的脆弱依赖，为长时程任务中的模仿学习提供了一种更鲁棒的自适应方法。
...
### [183] [AI as Entertainment](https://arxiv.org/abs/2601.08768)
*Cody Kommers,Ari Holtzman*

Main category: cs.AI

TL;DR: 该论文指出AI系统主要被定位为提升生产力的智能工具，但娱乐正成为AI的重要应用场景，而AI领域缺乏评估娱乐内容社会影响的框架，需要从人文角度建立"厚重娱乐"的评估体系。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统主要被设计和评估为提升生产力的智能工具，但现实中AI正被广泛用于娱乐目的，尤其是年轻人群体。AI领域缺乏评估娱乐内容社会影响的框架，现有的评估主要关注文化危害，而忽视了娱乐可能带来的积极社会价值。

Method: 通过分析当前AI评估实践的局限性，借鉴人文科学的见解，提出"厚重娱乐"作为评估AI生成文化内容的框架。该框架考虑娱乐在意义建构、身份形成和社会连接中的作用，而不仅仅是减少危害。

Result: 研究发现AI娱乐应用已经广泛存在且具有巨大商业潜力，可能成为AI公司的主要商业模式。当前AI评估存在严重不对称：虽然严格衡量智能的益处和危害，但几乎只关注文化危害，缺乏评估文化输出积极价值的框架。

Conclusion: AI可能最终更多是关于娱乐而非智能，就像社交媒体更多是关于社交连接一样。需要建立新的评估框架来理解AI生成娱乐内容的社会影响，考虑其积极的文化价值而不仅仅是防范危害。
...
### [184] [Uncovering Political Bias in Large Language Models using Parliamentary Voting Records](https://arxiv.org/abs/2601.08785)
*Jieying Chen,Karen de Jong,Andreas Poole,Jan Burakowski,Elena Elderson Nosti,Joep Windt,Chendi Wang*

Main category: cs.AI

TL;DR: 本文提出了一种基于议会投票记录构建政治偏见基准的通用方法，并在荷兰、挪威、西班牙三个国家案例中应用，发现最先进的LLMs普遍表现出左倾或中间派倾向，并对右翼保守政党存在明显负面偏见。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在数字平台和决策系统中深度应用，对其政治偏见的担忧日益增长。虽然已有大量研究关注性别、种族等社会偏见，但对政治偏见的系统性研究仍然有限，尽管其对社会有直接影响。

Method: 提出了一种通用方法，通过将模型生成的投票预测与经过验证的议会投票记录对齐来构建政治偏见基准。在三个国家案例中实例化该方法：PoliBiasNL（荷兰）、PoliBiasNO（挪威）和PoliBiasES（西班牙）。还提出了一种可视化方法，将LLMs和政党的意识形态定位在共享的二维CHES空间中，实现模型与现实政治行为者之间的直接可解释比较。

Result: 实验揭示了细粒度的意识形态区别：最先进的LLMs始终表现出左倾或中间派倾向，同时对右翼保守政党存在明显的负面偏见。
