...
TL;DR: MARL-Rad是一个用于放射学报告生成的多模态多智能体强化学习框架，通过协调区域特定智能体和全局集成智能体，使用临床可验证奖励进行优化，在临床效果指标上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的放射学报告生成方法通常使用单一模型的强化学习，或者对独立训练的模型进行后处理的智能体化。这些方法无法有效协调多个智能体并优化整个智能体系统，限制了临床效果的提升。

Method: 提出MARL-Rad框架，包含区域特定智能体和全局集成智能体的协调机制。通过多模态多智能体强化学习联合训练多个智能体，使用临床可验证奖励优化整个智能体系统。

Result: 在MIMIC-CXR和IU X-ray数据集上的实验表明，MARL-Rad在RadGraph、CheXbert和GREEN等临床效果指标上持续提升，达到最先进的临床效果性能。进一步分析确认该方法增强了侧向一致性，生成更准确、细节丰富的报告。

Conclusion: MARL-Rad通过多智能体强化学习框架有效协调区域特定和全局智能体，显著提升放射学报告生成的临床效果，为医学影像报告生成提供了新的有效方法。
...
TL;DR: 该研究探讨了基于惯性测量单元的在线手写识别中处理字符分布不均和书写者间差异的两种策略：子词分词和基于连接的数据增强。研究发现子词分词主要缓解书写者间风格差异，而基于连接的数据增强则有效补偿书写者内分布稀疏性。


<details>
  <summary>Details</summary>
Motivation: 基于惯性测量单元的在线手写识别虽然能够在不同书写表面上收集输入信号，但仍面临字符分布不均和书写者间差异的挑战。需要系统研究解决这些问题的有效策略。

Method: 研究采用两种主要策略：1）子词分词（特别是Bigram分词），通过结构抽象处理书写者间差异；2）基于连接的数据增强，作为正则化器处理训练集和验证集之间的词汇分布偏移。在OnHW-Words500数据集上进行实验，分别评估书写者独立和书写者依赖两种分割方式。

Result: 在书写者独立分割中，Bigram分词将词错误率从15.40%降至12.99%，显著改善对未见书写风格的泛化能力。在书写者依赖分割中，分词反而降低性能，但基于连接的数据增强将字符错误率降低34.5%，词错误率降低25.4%。短的低级分词有利于模型性能，且数据增强的效果优于按比例扩展训练。

Conclusion: 研究揭示了明显的方差依赖效应：子词分词主要缓解书写者间风格差异，而基于连接的数据增强有效补偿书写者内分布稀疏性。这两种策略针对不同类型的方差问题具有互补作用。
...
### [3] [CineSRD: Leveraging Visual, Acoustic, and Linguistic Cues for Open-World Visual Media Speaker Diarization](https://arxiv.org/abs/2603.16966)
*Liangbin Huang,Xiaohua Liao,Chaoqun Cui,Shijing Wang,Zhaolong Huang,Yanlong Du,Wenji Mao*

Main category: cs.CV

TL;DR: CineSRD：面向影视等开放世界场景的多模态说话人日志系统，利用视觉、声学和语言线索进行说话人注册与标注


<details>
  <summary>Details</summary>
Motivation: 传统说话人日志系统主要针对会议、访谈等受限场景，而影视等视觉媒体具有说话人数量多、音频视觉异步、环境复杂等挑战，需要探索开放世界的说话人日志任务

Method: 提出CineSRD统一多模态框架：1）通过视觉锚点聚类进行初始说话人注册；2）集成音频语言模型进行说话人转换检测，精炼标注并补充未注册的屏幕外说话人

Result: 在构建的中英文视觉媒体说话人日志基准测试中表现优异，在传统数据集上也取得有竞争力的结果，验证了其在开放世界视觉媒体场景中的鲁棒性和泛化能力

Conclusion: CineSRD成功将说话人日志任务扩展到影视等复杂视觉媒体领域，通过多模态融合有效解决了开放世界场景的挑战，为视觉媒体理解提供了新工具
...
Method: 采用Top-K机制选择信息最丰富的像素进行稀疏化传输，通过MU-MIMO传输到地面服务器，使用Swin-large-based MaskDINO编码器提取BEV特征并进行协同融合，开发基于扩散模型的DRL算法联合优化无人机选择、稀疏化比例和预编码矩阵

Result: 在Air-Co-Pred数据集上的仿真结果显示，相比传统CNN-based BEV融合基线，BHU框架在提升5%以上感知性能的同时，减少了85%的通信开销
...
TL;DR: 该论文提出了一种结合迁移学习和宽度学习系统（BLS）的面部美观度预测方法，通过EfficientNets提取特征并传输到BLS进行预测，提高了预测精度。
...
Method: 论文将迁移学习与宽度学习系统融合用于面部美观度预测。首先，基于迁移学习的CNN模型（使用EfficientNets）构建特征提取器，将提取的面部美观特征传输到BLS进行预测（称为E-BLS）。其次，在E-BLS基础上设计连接层连接特征提取器和BLS（称为ER-BLS）。

Result: 实验结果表明，与现有的BLS和CNN方法相比，E-BLS和ER-BLS提高了面部美观度预测的准确性，证明了所提方法的有效性和优越性。

Conclusion: 该方法不仅提高了面部美观度预测的精度，还可以广泛应用于模式识别、目标检测和图像分类等领域。迁移学习与宽度学习系统的融合为解决数据稀缺和模型构建效率问题提供了有效途径。
...
TL;DR: 提出Script-to-Slide Grounding任务，利用大语言模型自动将脚本句子与幻灯片文本对象进行对齐，实现教学视频自动生成的第一步


<details>
  <summary>Details</summary>
Motivation: 幻灯片视频在教育研究中广泛应用，但添加视觉效果的视频编辑过程非常耗时，需要自动化解决方案

Method: 提出Script-to-Slide Grounding任务定义，开发Text-S2SG方法，利用大语言模型将脚本句子与幻灯片文本对象进行对齐

Result: 实验显示该方法达到高精度（F1分数：0.924），验证了方法的有效性

Conclusion: 将隐式的幻灯片视频编辑过程形式化为可计算任务，为实现自动化奠定了基础
...
### [7] [A 3D Reconstruction Benchmark for Asset Inspection](https://arxiv.org/abs/2603.17358)
...
<details>
  <summary>Details</summary>
Motivation: 资产管理需要高精度的3D模型来支持维护、修复和评估工作。现有3D重建数据集缺乏资产检查场景中常见的密集采集轨迹、高重叠度图像、毫米级细节以及反射和透明等复杂表面条件，难以评估方法在该领域的实际性能。

Method: 创建了一个包含三个合成场景的新数据集，这些场景具有模拟的检查轨迹和不同程度的表面条件（非朗伯表面内容）。数据集提供了地面真实的深度图、相机位姿和网格模型，用于评估最先进的3D重建方法。

Result: 评估结果显示，当前最先进的3D重建方法在处理密集采集轨迹和复杂表面条件时表现显著不佳，暴露了现有方法在该领域应用的可扩展性差距。

Conclusion: 当前3D重建方法在资产检查领域的实际应用中存在严重局限性，需要新的研究方向来开发可部署的资产检查3D重建技术。该数据集为相关研究提供了重要的基准测试平台。
...
### [8] [Look Where It Matters: High-Resolution Crops Retrieval for Efficient VLMs](https://arxiv.org/abs/2603.16932)
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通常需要处理高分辨率图像，这导致了精度与计算效率之间的权衡：高分辨率输入能捕捉细节但计算成本高，低分辨率输入效率高但可能丢失关键视觉信息（如小文本）。

Method: 1. 使用低分辨率全局视图，通过工具调用仅检索查询所需的高分辨率片段；2. 自动构建监督数据：通过比较低分辨率与高分辨率答案判断是否需要裁剪，使用oracle grounding模型定位正确答案证据并映射到离散裁剪集；3. 采用冷启动SFT和多轮GRPO训练，结合语义答案正确性和裁剪成本惩罚的复合奖励。

Result: 该方法在保持计算效率的同时提高了视觉语言模型处理高分辨率图像的准确性，特别是在需要识别小文本等细节信息的场景中。

Conclusion: AwaRes框架有效解决了视觉语言模型在精度与效率之间的权衡问题，通过空间按需的方法实现了更高效的高分辨率图像处理。
...
TL;DR: 利用基础生成模型合成大规模面部行为数据集，通过语言模型实现面部参数的双向理解与生成，将面部动画建模为语言问题
...
Method: 设计涵盖情感和头部动作的提示词套件，生成约80小时面部视频并拟合每帧3D面部参数，构建大规模(提示词-参数)对数据集；基于此数据集，通过Motion2Language和Language2Motion两个互补任务，探索语言模型对面部运动的理解与生成能力

Result: 实验表明语言模型能够有效解释和合成面部运动，并具有良好的泛化能力；首次将面部参数建模为语言问题，为文本条件面部动画和运动理解建立了统一路径

Conclusion: 通过合成大规模面部行为数据集并利用语言模型的双向能力，成功填补了文本引导面部动画的数据空白，开创了将面部参数建模为语言问题的新范式
...
TL;DR: 该论文提出了一种基于差异度的多模态融合方法，通过显式测量视觉、音频和文本模态之间的冲突来识别矛盾/犹豫视频，在BAH数据集上取得了显著优于基线的性能。


<details>
  <summary>Details</summary>
Motivation: 针对第10届ABAW竞赛中的矛盾/犹豫视频识别挑战，需要有效处理多模态信息之间的不一致性，因为矛盾/犹豫状态往往表现为不同模态之间的冲突或不协调。

Method: 提出基于差异度的多模态融合方法：1）视觉特征通过Py-Feat提取动作单元（AUs）；2）音频特征通过Wav2Vec 2.0提取；3）文本特征通过BERT提取；4）每个模态通过BiLSTM和注意力池化处理，并投影到共享嵌入空间；5）融合模块计算模态嵌入之间的成对绝对差异，直接捕捉表征矛盾/犹豫的不协调性。

Result: 在BAH数据集上，该方法在验证测试集上取得了0.6808的宏F1分数，显著优于挑战基线（0.2827）。对1,132个视频的统计分析证实，动作单元的时间变异性是识别矛盾/犹豫的主要视觉判别特征。

Conclusion: 基于差异度的多模态融合方法能够有效捕捉矛盾/犹豫状态中不同模态之间的冲突，显著提升了识别性能，为多模态情感分析提供了新的融合策略。
...
### [11] [Omni IIE Bench: Benchmarking the Practical Capabilities of Image Editing Models](https://arxiv.org/abs/2603.16944)
...
TL;DR: 本文介绍了Omni IIE Bench基准测试，专门用于诊断指令式图像编辑模型在不同语义尺度任务中的一致性表现，发现现有模型在从低语义尺度向高语义尺度任务过渡时普遍存在性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有指令式图像编辑基准测试通过混合评估追求任务广度，但忽略了专业应用中一个关键失败模式：模型在不同语义尺度任务上的不一致表现。需要专门诊断模型在实际应用场景中的编辑一致性。
...
Result: 对8个主流IIE模型进行了全面评估，首次量化了一个普遍存在的性能差距：几乎所有模型在从低语义尺度任务向高语义尺度任务过渡时都表现出显著的性能下降。

Conclusion: Omni IIE Bench为开发下一代更可靠、更稳定的IIE模型提供了关键的诊断工具和见解，有助于解决模型在不同语义尺度任务中的不一致性问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在机器人感知和操作中应用广泛，但缺乏可靠的物理属性推理能力，特别是对物体质量的估计，这对于确定合适的抓取力和确保安全交互至关重要。现有基准大多未在真实感知条件下评估物理量估计。

Method: 提出PhysQuantAgent框架用于真实物体质量估计，并创建VisPhysQuant基准数据集（包含多视角RGB-D视频和精确质量标注）。引入三种视觉提示方法：物体检测、尺度估计和横截面图像生成，以增强模型对目标物体尺寸和内部结构的理解。

Result: 实验表明，视觉提示方法显著提高了在真实世界数据上的质量估计准确性，证明了将空间推理与VLM知识结合进行物理推断的有效性。

Conclusion: 通过视觉提示增强视觉语言模型的空间推理能力，可以有效提升其对物理属性（如质量）的估计性能，为机器人安全操作提供重要支持。
...
TL;DR: 提出基于场景即规范概念的标准化场景提取方法，以及领域知识引导的场景聚类流程，用于从高速公路数据记录中提取和分组测试场景，支持自动驾驶系统的高效验证。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统（ADS）的批准依赖于在真实交通场景中的行为评估。现有场景提取方法依赖异构定义，阻碍场景可比性；而场景分组方法中，基于规则的方法缺乏处理复杂交通场景的能力，基于机器学习的方法虽然能处理复杂性但缺乏可解释性且可能与领域知识不一致。

Method: 基于"场景即规范"概念建立标准化场景提取方法，同时开发领域知识引导的场景聚类流程。在highD数据集上进行实验验证。

Result: 实验表明，场景可以被可靠地提取，领域知识可以有效地集成到聚类过程中。该方法支持从高速公路数据记录中推导场景类别的标准化流程。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖主logits进行新颖性评分，未能充分利用各个模态的互补证据。由于RGB通常主导logits，IMU等其他模态的线索未被充分利用，这种不平衡在灾难性遗忘下会随时间恶化。
...
TL;DR: 提出Beyond Images自动数据增强管道，通过大规模检索实体相关图像、将视觉输入转为文本描述、融合多源描述生成实体对齐摘要，提升多模态知识图谱性能


<details>
  <summary>Details</summary>
Motivation: 多模态知识图谱受益于视觉信息，但大规模图像收集困难且常排除模糊但相关的视觉内容（如标志、符号、抽象场景）

Method: 三阶段管道：1)大规模检索实体相关图像；2)将所有视觉输入转为文本描述；3)使用大语言模型融合多源描述生成简洁的实体对齐摘要，这些摘要可替换或增强标准MMKG模型中的文本模态

Result: 在三个公共MMKG数据集和多个基线模型上观察到一致提升（整体Hits@1提升达7%）；在具有视觉模糊标志和符号的实体子集上，将图像转为文本带来显著改进（MRR提升201.35%，Hits@1提升333.33%）

Conclusion: 扩展图像覆盖范围并将模糊视觉内容转为文本是增强多模态知识图谱完成的实用路径，同时发布了轻量级文本-图像一致性检查接口用于可选的有针对性审计
...
TL;DR: 该论文针对资源受限环境下视觉语言模型部署效率低的问题，通过系统分析瓶颈并提出优化方案，显著降低了紧凑型VLM的延迟，同时开发了具有结构化感知输出的ArgusVLM模型家族。


<details>
  <summary>Details</summary>
Motivation: 在资源受限环境中部署视觉语言模型需要低延迟和高吞吐量，但现有的紧凑型VLM未能实现与其较小参数量相匹配的推理速度提升，存在效率瓶颈。

Method: 1. 对紧凑型VLM进行端到端效率分析和系统性的推理性能剖析，识别主要瓶颈；2. 开发针对紧凑型VLM的优化方案，大幅降低延迟同时保持准确性；3. 扩展紧凑型VLM以支持结构化感知输出，创建ArgusVLM模型家族。

Result: 优化方案显著降低了紧凑型VLM的延迟：InternVL3-2B的首令牌时间减少53%，SmolVLM-256M减少93%。ArgusVLM在多种基准测试中表现出色，同时保持紧凑高效的设计。

Conclusion: 该研究提供了针对紧凑型VLM的实用优化指南，这些技术可广泛应用于不同VLM架构和常见服务框架，为构建高效的VLM系统提供了实践指导，同时展示了通过结构化感知输出扩展紧凑型VLM的可行性。
...
TL;DR: HopChain是一个可扩展的框架，用于合成多跳视觉语言推理数据，以增强VLMs的复杂推理能力，在24个基准测试中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前VLMs在细粒度视觉语言推理方面仍有不足，特别是长链推理会暴露感知、推理、知识和幻觉等多种错误。现有RLVR训练数据缺乏依赖视觉证据的复杂推理链，无法充分暴露这些弱点。

Method: 提出HopChain框架，合成多跳视觉语言推理数据。每个合成查询形成一个逻辑依赖的实例基础推理链，早期跳建立实例、集合或条件，后期跳依赖这些基础，最终答案保持具体、明确的数字以便验证奖励。

Result: 将HopChain合成的多跳数据添加到原始RLVR数据中训练Qwen3.5模型，在24个基准测试中，20个测试得到提升。完整多跳查询比半多跳和单跳变体分别提高5.3和7.0个准确率点。在超长CoT推理中，提升超过50个准确率点。

Conclusion: HopChain是一个有效且可扩展的框架，能够合成多跳数据，显著提升视觉语言推理的泛化能力，特别是在复杂长链推理场景中表现突出。
...
TL;DR: OpenQlaw是一个用于分析二维量子材料的智能编排系统，通过将视觉识别与推理解耦，并利用持久记忆存储物理尺度比例和样品制备方法，为研究人员提供更实用的交互体验。


<details>
  <summary>Details</summary>
Motivation: 现有基于物理知识的多模态大语言模型虽然能准确识别二维量子材料，但其输出过于冗长且注重认知透明度，导致认知过载，缺乏对研究人员实际交互的即时实用性。

Method: 基于NanoBot轻量级智能框架构建OpenQlaw系统，核心LLM智能体编排领域专家MLLM（QuPAINT），将视觉识别与推理解耦，支持动态处理用户查询（如尺度感知物理计算、生成隔离视觉标注），并具备持久记忆功能存储物理尺度比例和样品制备方法。

Result: 系统成功将孤立推理转化为上下文感知的助手，能够通过多种消息渠道在实验室环境中访问，加速高通量器件制造过程。

Conclusion: 智能体架构与核心智能体作为领域专家编排器的扩展相结合，能够将光学识别转化为实用的器件制造，为二维量子材料研究提供更有效的交互工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有蒸馏自回归视频模型经常与人类视觉偏好不一致，而现有的强化学习框架不适合这些架构，要么需要昂贵的重新蒸馏，要么需要求解器耦合的反向过程优化，带来大量内存和计算开销

Method: 1. 基于负感知微调的前向过程RL公式，通过对比正负样本在推理端点直接建立隐式策略改进方向，无需反向过程展开；2. 流式训练方案，通过滚动KV缓存渐进生成序列，仅对局部剪辑窗口应用RL更新，同时基于先前上下文确保长程一致性；3. 多奖励目标，通过不确定性感知选择性正则化和动态参考更新稳定化，缓解奖励黑客问题

Result: 大量实验表明，该方法在多个蒸馏自回归视频模型中一致提高了生成质量，作为一个稳健且可扩展的对齐解决方案

Conclusion: Astrolabe为蒸馏自回归视频模型提供了一个高效、可扩展的在线强化学习框架，解决了现有方法的计算瓶颈，能够有效提升生成质量与人类偏好对齐
...
TL;DR: PaAgent是一种基于多模态大语言模型的图像修复智能体，通过自演化的工具画像库和检索增强生成技术，能够更有效地选择适合的图像修复工具，特别针对复杂场景中的部分和非均匀退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像修复智能体通常缺乏对过往交互的洞察总结机制，导致需要穷举搜索最优修复工具。为了解决这一限制，需要开发能够更智能选择修复工具的系统。

Method: 提出PaAgent，包含自演化的工具画像库和检索增强生成技术。通过总结各种修复工具的特性、修复后的图像、选择的工具和退化图像来构建画像库。采用主客观强化学习策略，结合图像质量评分和语义洞察来生成奖励，增强对复杂场景中退化的感知能力。

Result: 在8个图像修复基准测试中进行了广泛实验，涵盖六种单一退化和八种混合退化场景，验证了PaAgent在处理复杂图像修复任务方面的优越性。

Conclusion: PaAgent通过自演化画像库和检索增强生成技术，能够更有效地选择图像修复工具，特别在复杂退化场景中表现出色，为自动化图像修复任务提供了更智能的解决方案。
...
TL;DR: 提出TrackDeform3D框架，仅使用RGB-D相机低成本自主收集可变形物体的3D数据集，通过识别3D关键点并加入运动一致性约束进行鲁棒跟踪，创建了包含6个物体、110分钟轨迹数据的大规模高质量数据集。


<details>
  <summary>Details</summary>
Motivation: 当前3D结构化表示（如关键点和网格）虽然对可变形物体建模有用，但提取方法难以处理复杂变形，且大规模3D数据收集存在瓶颈：现有方法要么需要大量人工标注或昂贵动捕设备，要么依赖在非结构化环境中失效的简化假设，导致可变形物体的大规模3D数据集稀缺。

Method: 提出低成本自主框架，仅使用RGB-D相机收集可变形物体3D数据。方法识别3D关键点并鲁棒跟踪其轨迹，通过加入运动一致性约束产生时间平滑且几何一致的数据。

Result: TrackDeform3D在多种物体类别上评估，相比多个最先进跟踪方法在几何精度和跟踪准确性方面均表现更优。使用该框架创建了包含6个可变形物体、总计110分钟轨迹数据的高质量大规模数据集。

Conclusion: 提出的TrackDeform3D框架解决了可变形物体3D数据收集的挑战，提供了低成本、自主的解决方案，并创建了高质量数据集，有助于推动可变形物体感知和建模研究。
...
<details>
  <summary>Details</summary>
Motivation: CNN的下采样操作破坏了平移不变性，但下采样对计算效率和感受野扩展至关重要。现有方法难以同时保持平移不变性和下采样的优势。

Method: 提出高斯-埃尔米特采样(GHS)，利用高斯-埃尔米特多项式进行平移一致的下采样，将平移不变性直接嵌入到CNN层中，无需架构修改或额外训练。

Result: 在CIFAR-10、CIFAR-100和MNIST-rot数据集上，GHS显著提高了平移一致性，在空间平移下实现100%分类一致性，同时提升了分类准确率。

Conclusion: GHS是一种有效的下采样策略，能够在保持CNN计算效率和感受野优势的同时，实现准确的平移不变性，为构建平移不变CNN提供了新思路。
...
### [23] [Pixel-level Counterfactual Contrastive Learning for Medical Image Segmentation](https://arxiv.org/abs/2603.17110)
*Marceau Lafargue-Hauret,Raghav Mehta,Fabio De Sousa Ribeiro,Mélanie Roschewitz,Ben Glocker*

Main category: cs.CV

TL;DR: 提出结合反事实生成与密集对比学习的像素级分割方法，无需标注或利用AI生成标签，在挑战性数据上达到约94% DSC


<details>
  <summary>Details</summary>
Motivation: 图像分割依赖昂贵的人工标注数据集，AI生成的银标准标签虽然易得但可能引入偏差，现有自监督学习方法难以直接扩展到像素级任务

Method: 提出结合反事实生成与密集对比学习的管道，包括双视图(DVD-CL)和多视图(MVD-CL)方法，以及利用银标准标注的监督变体，并引入新的可视化算法CHRO-map

Result: 无标注的DVD-CL优于其他密集对比学习方法，使用银标准标签的监督变体优于直接在银标准标注数据上训练，在挑战性数据上达到约94% DSC
...
### [24] [Hidden Clones: Exposing and Fixing Family Bias in Vision-Language Model Ensembles](https://arxiv.org/abs/2603.17111)
*Zacharie Bugaud*

Main category: cs.CV

TL;DR: 研究发现视觉语言模型集成中存在家族相关性误差，提出三种家族感知方法提升集成效果


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型集成方法忽略了同一架构家族模型间的相关性误差，这些相关误差会降低集成效果，甚至在某些问题上导致准确率降为0%

Method: 提出三种家族感知方法：1) 分层家族投票(HFV)：先在家族内聚合再跨家族投票；2) QualRCCV：基于校准、家族质量和家族规模加权的无训练方法；3) 学习候选评分(LCS)：训练交叉验证分类器，使用支持广度、家族多样性和模型质量重新排序候选答案

Result: 在VQAv2、TextVQA和GQA三个基准测试上，HFV在误导性问题上恢复18-26个百分点准确率；QualRCCV首次在所有三个基准上显著超越校准投票；LCS获得最大提升：VQAv2 +0.68%、TextVQA +0.61%、GQA +2.45%，且是唯一不降低任何基准性能的学习方法

Conclusion: 视觉语言模型集成中存在显著的家族相关性误差，通过家族感知方法可以有效缓解这一问题，显著提升集成性能，其中LCS方法表现最佳且具有良好泛化能力
...
### [25] [MosaicMem: Hybrid Spatial Memory for Controllable Video World Models](https://arxiv.org/abs/2603.17117)
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散模型正在从生成短片段向世界模拟器发展，需要保持相机运动、重访和干预下的一致性。然而空间记忆是关键瓶颈：显式3D结构能改善重投影一致性但难以描绘运动物体，而隐式记忆即使有正确姿态也常产生不准确的相机运动。

Method: 提出MosaicMem混合空间记忆系统，将图像块提升到3D空间实现可靠定位和针对性检索，通过patch-and-compose接口在查询视图中组合空间对齐的图像块，保持应持续的内容同时允许模型修复应演化的内容。结合PRoPE相机条件和两种新的记忆对齐方法。

Result: 实验显示相比隐式记忆有更好的姿态遵循性，相比显式基线有更强的动态建模能力。MosaicMem进一步实现了分钟级导航、基于记忆的场景编辑和自回归展开。

Conclusion: MosaicMem通过混合空间记忆方法有效解决了视频扩散模型中空间记忆的瓶颈，在保持相机运动一致性和动态物体建模方面取得了平衡，为世界模拟器的发展提供了重要技术支持。
...
### [26] [BEV-SLD: Self-Supervised Scene Landmark Detection for Global Localization with LiDAR Bird's-Eye View Images](https://arxiv.org/abs/2603.17159)
*David Skuddis,Vincent Ress,Wei Zhang,Vincent Ofosu Nyako,Norbert Haala*

Main category: cs.CV

TL;DR: BEV-SLD是一种基于场景地标检测的LiDAR全局定位方法，利用鸟瞰图自动发现场景特定模式作为地标，通过一致性损失实现跨场景的稳定定位。
...
Method: 提出自监督方法，使用鸟瞰图在指定空间密度下发现场景特定模式作为地标，通过一致性损失对齐可学习的全局地标坐标与每帧热图。

Result: 在校园、工业和森林环境中实现了鲁棒定位，相比最先进方法表现出色性能。

Conclusion: BEV-SLD通过自监督发现场景特定地标，为LiDAR全局定位提供了有效解决方案，在各种环境中都表现出鲁棒性。
...
### [27] [GazeOnce360: Fisheye-Based 360° Multi-Person Gaze Estimation with Global-Local Feature Fusion](https://arxiv.org/abs/2603.17161)
*Zhuojiang Cai,Zhenghui Sun,Feng Lu*

Main category: cs.CV

TL;DR: GazeOnce360是一个端到端模型，用于从单个向上鱼眼相机进行多人视线估计，解决了360度场景中多人3D视线方向估计的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖前向相机和受限视角，而本文探索了从向上鱼眼视角估计360度场景中多人3D视线方向的未充分研究场景。

Method: 提出GazeOnce360端到端模型，包含旋转卷积和眼部关键点监督来处理鱼眼图像畸变，采用双分辨率架构融合全局低分辨率上下文和局部高分辨率眼部区域特征。

Result: 实验结果表明模型各组件有效，证明了在实用多人场景中基于鱼眼的360度视线估计的可行性和潜力。

Conclusion: 这项工作展示了鱼眼相机在360度多人视线估计中的实际应用前景，为相关研究提供了新数据集和方法框架。
...
### [28] [Generalist Multimodal LLMs Gain Biometric Expertise via Human Salience](https://arxiv.org/abs/2603.17173)
*Jacob Piland,Byron Dowling,Christopher Sweet,Adam Czajka*

Main category: cs.CV

TL;DR: MLLMs结合专家知识可在隐私约束下实现虹膜活体检测，性能优于专用CNN基线和人工检测


<details>
  <summary>Details</summary>
Motivation: 虹膜活体检测面临实际挑战：无法收集未来未知攻击数据，收集足够多样本成本高，共享生物特征数据存在隐私问题。需要适应新攻击向量的灵活解决方案。

Method: 使用预训练多模态大语言模型（MLLMs）结合人类专家知识（受试者识别的攻击指标描述），在严格隐私约束下（仅使用大学批准服务或本地部署模型）进行虹膜活体检测。分析视觉编码器嵌入，使用结构化提示整合人类显著性信息。

Result: 在224张虹膜图像（7种攻击类型）的IRB限制数据集上测试，Gemini 2.5 Pro结合专家提示优于专用CNN基线和人工检测，本地部署的Llama 3.2-Vision达到接近人工的性能水平。

Conclusion: 在机构隐私约束下可部署的MLLMs为虹膜活体检测提供了可行路径，能够利用预训练视觉特征和人类专家知识解决攻击类型模糊性问题。
...
### [29] [Patient4D: Temporally Consistent Patient Body Mesh Recovery from Monocular Operating Room Video](https://arxiv.org/abs/2603.17178)
*Mingxiao Tu,Hoijoon Jung,Alireza Moghadam,Andre Kyme,Jinman Kim*

Main category: cs.CV

TL;DR: Patient4D：利用静止先验的4D患者重建管道，通过姿态锁定和刚性回退机制，在手术AR场景中从单目视频中稳定恢复被遮挡的3D人体网格


<details>
  <summary>Details</summary>
Motivation: 在手术增强现实场景中，麻醉患者被手术布覆盖，同时外科医生的头戴式摄像头不断改变视角，现有的人体网格恢复方法在这种遮挡和动态视角条件下性能下降，需要专门解决方案

Method: 提出Patient4D管道，结合图像级基础模型感知和轻量级几何机制，通过姿态锁定（使用稳定关键帧锚定姿态参数）和刚性回退（通过轮廓引导的刚性对齐在严重遮挡下恢复网格）确保时间一致性

Result: 在4,680个合成手术序列和三个公共HMR视频基准上评估，在手术布遮挡下达到0.75平均IoU，将失败帧从30.5%减少到1.3%，相比最佳基线有显著改进

Conclusion: 利用静止先验可以显著改善临床AR场景中的单目重建，Patient4D通过几何约束机制稳定预测，同时保持与现成HMR模型的兼容性
...
<details>
  <summary>Details</summary>
Motivation: 工业应用中可靠的产品图像识别至关重要，错误的匹配可能导致昂贵的下游故障。需要评估现代视觉嵌入模型在真实工业环境中的实例检索能力。

Method: 创建结构化基准测试，评估开源基础嵌入模型、专有多模态嵌入系统和领域特定视觉模型。使用统一图像到图像检索协议，包含工业数据集（制造、汽车、DIY、零售）和公共基准数据集，不进行后处理以隔离模型检索能力。

Result: 基准测试结果揭示了当代基础和统一嵌入模型在细粒度实例检索任务中的迁移能力，以及与专门为工业应用训练模型的比较。提供了关于不同模型在真实约束、异构图像条件和精确实例匹配要求下的性能洞察。

Conclusion: 该基准测试旨在为从业者和研究人员提供当前视觉嵌入方法在生产级产品识别系统中的优势和局限性信息，帮助选择合适的模型用于工业应用。
...
TL;DR: 提出SA-CycleGAN-2.5D框架解决多站点神经影像分析中的扫描仪诱导协变量偏移问题，通过2.5D三平面流形注入、U-ResNet生成器全局注意力机制和谱归一化判别器，显著降低域间差异，实现体素级影像协调。


<details>
  <summary>Details</summary>
Motivation: 多站点神经影像分析面临扫描仪诱导的协变量偏移问题，其中体素强度的边际分布随采集协议非线性变化，而解剖条件分布保持不变。这对放射组学可重复性特别有害，因为采集方差常常超过生物病理方差。现有统计协调方法在特征空间操作，无法支持空间下游任务，而标准深度学习方法受限于局部有效感受野，无法建模场强偏差的全局强度相关性。

Method: 提出SA-CycleGAN-2.5D域适应框架，基于Ben-David等人的HΔH散度界限理论。包含三个架构创新：1) 2.5D三平面流形注入，以O(HW)复杂度保留通过平面梯度∇z；2) U-ResNet生成器，具有密集体素到体素自注意力机制，超越CNN的O(√L)感受野限制，建模全局扫描仪场偏差；3) 谱归一化判别器，约束Lipschitz常数(KD≤1)以实现稳定的对抗优化。

Result: 在654名胶质瘤患者跨越两个机构域(BraTS和UPenn-GBM)的评估中，该方法将最大均值差异(MMD)降低了99.1%(从1.729降至0.015)，并将域分类器准确率降至接近随机水平(59.7%)。消融实验证实全局注意力在更难的异质到同质转换方向上具有统计显著性(Cohen's d=1.32, p<0.001)。

Conclusion: 通过结合2D效率和3D一致性，该框架生成保留肿瘤病理生理学的体素级协调图像，实现了可重复的多中心放射组学分析，有效解决了多站点神经影像分析中的扫描仪诱导协变量偏移问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有动态场景高斯溅射重建管道通常依赖固定锚点选择（如FPS），无论场景复杂度如何都使用8192个锚点，在严格计算预算下会造成计算资源过度分配。

Method: 提出高效高斯流式传输（EGS），这是一个基于强化学习的预算感知锚点采样器，可联合选择锚点预算和信息丰富的锚点子集，同时保持高斯流式重建主干不变。

Result: 在动态多视图数据集上，EGS在质量-效率权衡方面持续优于FPS采样。在快速渲染模式下，使用256个锚点（比8192少32倍），PSNR提高0.52-0.61dB，运行速度比IGS@8192快1.29-1.35倍。

Conclusion: EGS作为一种即插即用的锚点采样器，能够在显著降低锚点预算的情况下保持与完整锚点基线相当的竞争力，为动态场景高斯溅射重建提供了更高效的计算资源分配方案。
...
TL;DR: 该研究通过分层线性探测和注意力干预分析，揭示了多模态大语言模型在分割任务中的内在空间理解机制，发现适配器导致分割表示下降，但LLM层通过注意力机制逐步恢复。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型越来越多地应用于像素级视觉任务，但其内在的空间理解能力仍然未被充分理解。研究者希望探究MLLM在分割任务中的工作机制，为未来设计具备分割能力的模型提供指导。

Method: 采用分层线性探测评估整个MLLM流程（视觉编码器、适配器、LLM），并进行基于干预的注意力敲除分析，测试跨令牌注意力是否逐步细化视觉表示，同时评估图像令牌间双向注意力对空间一致性的影响。

Result: 分析发现：1）适配器引入分割表示下降；2）LLM层通过注意力介导的细化逐步恢复，正确分类的令牌引导错误分类的邻居朝向正确标签；3）早期图像令牌位置的恢复受因果注意力限制，而图像令牌间的双向注意力可以缓解这一问题。

Conclusion: 这些发现提供了MLLM如何处理视觉信息进行分割的机制性解释，为未来设计具备分割能力的模型提供了重要见解，强调了注意力机制在空间理解中的关键作用。
...
TL;DR: GigaWorld-Policy是一个基于预训练视频生成模型的动作中心世界行动模型，通过解耦动作预测和视频生成，实现了9倍加速和7%任务成功率提升


<details>
  <summary>Details</summary>
Motivation: 现有世界行动模型存在两个关键瓶颈：1）联合推理未来视觉动态和对应动作导致推理开销大；2）联合建模使视觉和运动表示纠缠，动作预测准确性过度依赖未来视频质量
...
Conclusion: 通过解耦动作预测和视频生成，GigaWorld-Policy实现了高效的动作解码和更准确的策略学习，同时保持了物理合理性约束，为机器人策略学习提供了更优的解决方案
...
Method: 提出LED基准，定义八种标准化错误类型（缺失、幻觉、尺寸错误、分割、合并、重叠、重复和错误分类），提供量化规则和注入算法进行真实错误模拟。构建LED数据集，设计三个评估任务：文档级错误检测、文档级错误类型分类和元素级错误类型分类

Result: 实验表明，LED能够对最先进的多模态模型进行细粒度和可解释的结构理解评估，揭示了跨模态和架构的明显弱点

Conclusion: LED建立了一个统一且可解释的基准，用于诊断文档理解模型的结构鲁棒性和推理能力
...
TL;DR: 提出了ConfusionBench基准，通过多阶段过滤流程构建高质量的学生困惑识别数据集，包含平衡的困惑识别和视频定位任务，并评估了开源和专有模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有困惑数据集存在标签噪声、时间标注粗糙和专家验证有限等问题，阻碍了可靠的细粒度识别和时间定位分析，需要构建更高质量的基准。

Method: 提出了一个实用的多阶段过滤流程，整合了模型辅助筛选、研究人员策划和专家验证，构建了ConfusionBench基准，包含平衡的困惑识别数据集和视频定位数据集。

Result: 专有模型整体表现更好但倾向于过度预测过渡片段，开源模型更保守但更容易漏检；学生困惑报告可视化可支持教育专家做出干预决策和调整学习计划。

Conclusion: 通过多阶段过滤流程构建了高质量的ConfusionBench基准，为教育视频中的学生困惑识别和定位提供了可靠评估工具，所有数据集和材料将公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 现有故事可视化方法在角色一致性和身份漂移问题上表现不佳，特别是在复杂交互和长叙事中，需要解决主体不一致和风格漂移的挑战。

Method: 提出两阶段框架：1) 组共享注意力机制，在注意力层内实现无损跨样本信息流，结构性地编码跨帧身份对应；2) 直接偏好优化，基于整体偏好数据同时提升视觉保真度和身份一致性。

Result: 在ViStoryBench基准测试中达到最先进水平，显著优于强基线方法：角色一致性指标提升+10.0，风格一致性指标提升+18.7，同时保持高保真生成质量。

Conclusion: 提出的两阶段框架通过组共享注意力机制和直接偏好优化，有效解决了故事可视化中的角色一致性问题，在保持高质量生成的同时显著提升了身份和风格的一致性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型智能体在处理长视频理解任务时面临挑战，这些任务具有信息密度高、时间跨度长的特点。简单的任务分解和协作机制不足以处理长链推理任务，而基于嵌入的检索方法可能会丢失复杂问题的关键信息。

Method: Symphony采用多智能体系统，模拟人类认知模式：1）将长视频理解分解为细粒度子任务；2）通过反思增强的深度推理协作机制；3）提供基于视觉语言模型的定位方法，分析任务并评估视频片段相关性。
...
TL;DR: 提出R²VLM模型，通过循环推理框架处理局部视频片段，利用演化的思维链记录任务分解和完成状态，实现长时程任务进度估计，避免处理长视频的高计算成本。
...
Conclusion: R²VLM通过循环推理框架有效解决了长视频处理的计算瓶颈，同时保持了复杂推理能力，在长时程任务进度估计方面实现了最先进性能，具有良好的泛化能力。
...
TL;DR: MedSAD-CLIP：基于CLIP的医学异常检测与分割模型，通过细粒度文本-视觉注意力机制和轻量适配器，在有限标注数据下实现精准病灶定位和分类


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的医学异常检测方法通常依赖全局表示和弱监督，导致定位粗糙、分割质量有限。在临床实际场景中，虽然标注数据有限但仍有意义，需要开发能够利用有限标注异常数据的监督适应方法

Method: 提出MedSAD-CLIP模型，采用Token-Patch Cross-Attention(TPCA)机制实现细粒度文本-视觉线索对齐，使用轻量图像适配器和可学习提示词高效适应医学领域，同时设计基于边界的图像-文本对比损失增强正常与异常特征的全局区分能力

Result: 在脑部、视网膜、肺部和乳腺四个多样化基准数据集上的实验表明，该方法在像素级分割和图像级分类任务上均优于现有最先进方法

Conclusion: 监督式CLIP适应作为一种统一且可扩展的范式，在医学异常理解方面具有巨大潜力，能够有效利用有限标注数据实现精准的病灶定位和分类
...
Method: 基于双分支架构，通过因果键值缓存设计和滑动窗口学习策略实现流式推理，同时使用以人为中心的增量SLAM进行在线世界坐标对齐和物理合理的轨迹校正。

Result: 在标准EMDB基准测试和高度动态的自定义视频上，性能与现有的基于分块的方法相当，同时独特地支持在线处理。

Conclusion: OnlineHMR成功实现了完全在线的3D人体网格恢复，满足了系统级因果性、保真度、时间一致性和效率四个关键要求，为交互式应用提供了可行的解决方案。
...
### [42] [UniSAFE: A Comprehensive Benchmark for Safety Evaluation of Unified Multimodal Models](https://arxiv.org/abs/2603.17476)
...
TL;DR: UniSAFE是首个针对统一多模态模型的系统级安全评估基准，涵盖7种I/O模态组合，包含6,802个实例，评估了15个先进模型，发现多图像组合和多轮对话设置中安全违规风险更高


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型（UMMs）虽然具有强大的跨模态能力，但带来了单任务模型未观察到的新的安全风险。现有的安全基准在任务和模态上分散，无法全面评估复杂的系统级漏洞

Method: 构建UniSAFE基准，采用共享目标设计，将常见风险场景映射到任务特定的I/O配置中，涵盖7种I/O模态组合，包括传统任务和新型多模态上下文图像生成设置

Result: 评估15个最先进的UMMs（包括专有和开源模型），发现当前UMMs存在关键漏洞：多图像组合和多轮设置中安全违规率更高，图像输出任务比文本输出任务更易受攻击
...
TL;DR: 本文提出MCoT-MVS方法，通过多模态思维链推理指导多级视觉特征选择，提升组合图像检索性能
...
Method: 利用多模态大语言模型进行思维链推理生成保留、移除和目标推断文本，指导两个参考视觉注意力选择模块提取补丁级和实例级语义，设计加权分层组合模块融合多粒度视觉线索

Result: 在CIRR和FashionIQ两个基准测试中均优于现有方法，达到新的最先进性能

Conclusion: MCoT-MVS方法通过多模态思维链推理指导的多级视觉选择机制，有效提升了组合图像检索的准确性和鲁棒性
...
TL;DR: 提出Material Magic Wand工具，用于在无纹理网格中基于材料属性自动选择重复结构部件，减少手动选择工作量。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的许多形状（如松果鳞片、建筑窗户）包含重复结构，这些结构共享相同材料但存在几何变化。在分配材料时，需要逐个手动识别和选择这些重复部件，过程繁琐耗时。
...
Conclusion: 提出的Material Magic Wand工具能够有效解决无纹理网格中的材料感知部件分组问题，显著减少艺术家在材料分配中的手动工作量。
...
TL;DR: Loc3R-VLM是一个为2D视觉语言模型增加3D理解能力的框架，通过单目视频输入实现空间认知，在语言定位和3D问答任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在空间理解和视角感知推理方面仍有不足，当前方法主要是在输入表示中加入几何线索，而不是显式地教授模型进行3D空间推理。

Method: 受人类空间认知启发，采用两个联合目标：全局布局重建（构建场景结构的整体表示）和显式情境建模（锚定自我中心视角）。利用预训练的3D基础模型提取轻量级相机姿态先验，确保几何一致性和度量尺度对齐。

Result: 在语言定位任务上达到最先进性能，在情境化和通用3D问答基准测试中优于现有的2D和视频方法，证明了空间监督框架能够实现强大的3D理解能力。

Conclusion: Loc3R-VLM通过空间监督框架成功为2D视觉语言模型赋予了先进的3D理解能力，直接从单目视频输入中学习空间表示，在多个3D理解任务上表现出色。
...
TL;DR: 研究发现视觉语言模型的安全对齐在视觉模态整合后减弱，图像能显著提高越狱成功率。模型能区分良性输入和有害输入，但视觉模态会将表征推向特定越狱状态，导致拒绝机制失效。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在整合视觉模态后安全对齐能力减弱，即使文本提示包含明确有害意图，添加图像也能大幅提高越狱成功率。需要理解这种安全漏洞的根本原因。

Method: 通过分析模型表征空间，发现良性输入和有害输入可区分，且越狱样本形成独特的内部状态。识别越狱方向并定义越狱相关偏移，提出JRS-Rem防御方法，在推理时移除越狱相关偏移。

Result: 越狱相关偏移能可靠表征越狱行为，为不同越狱场景提供统一解释。JRS-Rem防御方法在多种场景下提供强大防御，同时保持良性任务性能。

Conclusion: 视觉语言模型的越狱不是由于无法识别有害意图，而是视觉模态将表征推向特定越狱状态。移除越狱相关偏移能有效增强模型安全性，为视觉语言模型安全对齐提供新视角。
...
TL;DR: InfoShot：一种面向长视频理解的镜头感知帧采样方法，通过信息论目标在有限帧数下平衡视频整体覆盖与关键短暂事件


<details>
  <summary>Details</summary>
Motivation: 现有帧采样方法在只能保留少量帧时，难以平衡广泛的视频覆盖范围与短暂但关键的事件，导致下游预测不可靠

Method: 1) 将视频分割为语义一致的镜头；2) 从每个镜头中选择两个互补的关键帧：一个代表主要内容，一个捕捉镜头内的异常变化；3) 基于信息论目标鼓励采样集保留镜头结构和稀疏镜头内偏差的高信息量

Result: InfoShot在帧数限制下提高了异常检测命中率和下游视频问答准确率，同时在标准视频理解基准测试中匹配或优于强基线方法

Conclusion: InfoShot是一种无需重新训练的任务无关采样器，能够有效保留视频整体上下文和短暂决策关键时刻，提升长视频理解性能
...
TL;DR: StereoWorld是一个相机条件化的立体世界模型，通过联合学习外观和双目几何来实现端到端的立体视频生成，相比单目RGB或RGBD方法，它在RGB模态内运行，同时直接从视差中获取几何信息。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常采用单目生成后再转换的流程，存在立体一致性差、视差精度低、相机运动保真度不足等问题。需要一种能够直接在RGB模态内联合学习外观和双目几何的端到端立体视频生成方法。

Method: 1. 统一的相机帧RoPE：通过相机感知的旋转位置编码增强潜在标记，实现相对、视角和时间一致的调节，同时通过稳定的注意力初始化保留预训练视频先验。2. 立体感知注意力分解：将完整的4D注意力分解为3D视图内注意力加水平行注意力，利用极线先验以显著降低计算成本捕捉视差对齐的对应关系。

Result: 在基准测试中，StereoWorld相比强大的单目转换流程，在立体一致性、视差精度和相机运动保真度方面均有提升，生成速度提高3倍以上，视角一致性额外提升5%。

Conclusion: StereoWorld实现了无需深度估计或修复的端到端双目VR渲染，通过度量尺度深度基础增强具身策略学习，并且兼容长视频蒸馏以实现扩展的交互式立体合成。
...
TL;DR: 提出一种利用视觉语言基础模型解决材料分类数据稀缺问题的新框架，通过图像生成与自动标注流水线创建多样化训练数据，并结合先验知识蒸馏与联合微调提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 材料分类在计算机视觉和图形学中至关重要，但传统方法面临标注数据稀缺的挑战，限制了模型的准确性和泛化能力。现有的视觉语言基础模型解决方案在材料识别任务中效果仍不理想。

Method: 提出包含两个关键创新的框架：(1) 鲁棒的图像生成与自动标注流水线，创建多样化、高质量的材料中心图像训练数据集，通过融合对象语义和材料属性的文本提示自动分配标签；(2) 先验知识融合策略，从视觉语言模型中蒸馏信息，结合联合微调方法优化预训练视觉基础模型与VLM先验，在保持广泛泛化能力的同时适应材料特定特征。

Result: 在多个数据集上的广泛实验表明性能显著提升。合成的数据集有效捕捉了真实世界材料的特征，视觉语言模型先验的整合显著提高了最终性能。

Conclusion: 该框架有效利用基础模型克服数据限制，提升材料分类准确性，源代码和数据集将公开发布。
...
TL;DR: 该论文提出了一种利用手势语义作为归纳偏置来改进单目RGB图像中3D手部姿态估计的两阶段框架，通过手势感知预训练和基于Transformer的关节回归，在InterHand2.6M数据集上取得了优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 从单目RGB图像估计3D手部姿态是AR/VR、人机交互和手语理解等应用的基础。当存在离散手势标签时，手势语义可以作为3D姿态估计的强大归纳偏置，但目前的方法尚未充分利用这一信息。

Method: 提出两阶段框架：1) 手势感知预训练，利用InterHand2.6M中的粗粒度和细粒度手势标签学习信息丰富的嵌入空间；2) 基于Transformer的关节回归，以手势嵌入作为中间表示，最终回归MANO手部参数。训练采用分层目标函数，涵盖参数、关节和结构约束。

Result: 在InterHand2.6M数据集上的实验表明，手势感知预训练持续提高了单手姿态估计的准确性，优于最先进的EANet基线。该方法的优势可以跨架构迁移，无需任何修改。

Conclusion: 手势语义可以作为3D手部姿态估计的有效归纳偏置，提出的两阶段框架通过手势感知预训练和基于Transformer的回归，显著提升了姿态估计性能，且具有良好的可迁移性。
...
TL;DR: ASSR-EIC是一个基于任意尺度超分辨率的极端图像压缩框架，支持可变比特率压缩，通过单一模型实现自适应重建，在超低比特率下保持高质量恢复。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型压缩方法需要为每个目标比特率训练单独模型，计算开销大且部署困难。同时，现有超分辨率增强方法在超低比特率下效果不佳，且依赖固定的超分辨率尺度，无法灵活适应不同比特率。

Method: 1. 编码器端引入任意尺度下采样模块实现可控比特率降低；2. 解码器端使用基于扩散的联合退化感知任意尺度超分辨率解码器，支持单一模型内的速率自适应重建；3. 利用压缩和重缩放感知扩散先验指导重建；4. 设计全局压缩-重缩放适配器提供整体速率适应指导；5. 设计局部压缩-重缩放调制器动态平衡生成性和保真度导向行为；6. 引入双重语义增强设计。

Result: 大量实验表明，ASSR-EIC在极端图像压缩方面实现了最先进的性能，同时支持灵活的比特率控制和自适应速率依赖的重建。

Conclusion: ASSR-EIC通过任意尺度超分辨率解决了现有极端图像压缩方法的局限性，实现了单一模型支持可变比特率压缩，在超低比特率下仍能保持高质量重建，具有实际部署价值。
...
TL;DR: 本文提出了一种基于冻结Stable Diffusion模型的高效视频生成方法，通过运动自适应时间注意力机制，仅需少量可训练参数即可实现高质量视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法通常对所有视频内容采用统一的时间注意力机制，无法有效处理不同运动强度的序列。高运动序列需要局部注意力来保持快速变化的细节，而低运动序列需要全局注意力来确保场景一致性。同时，现有方法往往需要大量参数或额外的时序一致性损失。

Method: 1. 运动自适应时间注意力机制：根据估计的运动内容动态调整时间注意力感受野，高运动序列采用局部跨帧注意力，低运动序列采用全局注意力。2. 级联注入策略：在UNet所有transformer块中注入轻量级时间注意力模块，下采样和中间块使用全局注意力进行语义稳定，上采样块使用运动自适应注意力进行细粒度优化。3. 时间相关噪声初始化和运动感知门控。4. 仅使用标准去噪目标，无需额外时序一致性损失。

Result: 1. 仅增加2580万可训练参数（占基础UNet的2.9%），在WebVid验证集上取得有竞争力的结果。2. 标准去噪目标本身提供了足够的隐式时序正则化，优于添加显式时序一致性损失的方法。3. 消融研究揭示了噪声相关性与运动幅度之间的权衡关系，为推理时控制提供了实用方法。

Conclusion: 提出的运动自适应时间注意力机制能够高效地在冻结的Stable Diffusion模型基础上实现视频生成，通过动态调整注意力策略适应不同运动内容，仅需少量参数即可获得高质量结果，同时为推理时生成行为控制提供了实用工具。
...
TL;DR: 本文提出VirPro方法，通过视觉引导的概率提示学习增强弱监督单目3D检测，利用自适应提示库和多高斯提示建模来捕捉场景视觉多样性，在KITTI基准上实现4.8%的平均精度提升。


<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测通常依赖伪标签技术减少对真实标注的依赖。现有方法使用确定性语言线索作为弱监督信号，但手工制作的文本描述难以捕捉不同场景中个体的视觉多样性，限制了模型学习场景感知表示的能力。

Method: 提出视觉引导的概率提示学习（VirPro），包含自适应提示库（APB）存储可学习的实例条件提示，多高斯提示建模（MGPM）将场景视觉特征融入文本嵌入以表达视觉不确定性，通过RoI级对比匹配强制模态对齐，增强语义连贯性。

Result: 在KITTI基准上的大量实验表明，集成该预训练范式能持续带来显著的性能提升，相比基线方法实现高达4.8%的平均精度改进。

Conclusion: VirPro方法通过自适应多模态预训练范式有效解决了传统文本描述难以捕捉视觉多样性的问题，增强了弱监督单目3D检测的语义连贯性和场景感知能力，为多模态学习提供了新思路。
...
TL;DR: 该论文针对指代图像分割中运动相关查询性能不佳的问题，提出数据增强方案和多模态径向对比学习方法，显著提升运动查询性能


<details>
  <summary>Details</summary>
Motivation: 现有指代图像分割方法在处理运动相关查询时性能显著低于基于外观的查询，需要专门解决运动理解能力不足的问题

Method: 1. 提出高效数据增强方案，从原始描述中提取运动中心短语，无需额外标注；2. 提出多模态径向对比学习，在融合的图像-文本嵌入上进行而非单模态表示；3. 创建新的运动中心测试集M-Bench基准

Result: 实验表明该方法显著提升多个RIS模型在运动中心查询上的性能，同时在基于外观的描述上保持竞争力

Conclusion: 通过专门的数据增强和对比学习策略，有效解决了指代图像分割中运动理解能力不足的问题，为运动中心查询提供了系统解决方案
...
TL;DR: 本文提出SHIFT框架，通过像素运动奖励和对抗优势机制解决视频扩散模型微调后的运动保真度下降问题


<details>
  <summary>Details</summary>
Motivation: 图像条件视频扩散模型在视觉真实感方面表现优异，但在微调后往往出现运动保真度下降的问题，包括运动动态性减弱和长期时间一致性退化

Method: 提出基于像素通量动态的像素运动奖励来捕捉瞬时和长期运动一致性，并设计Smooth Hybrid Fine-tuning (SHIFT)框架，将监督微调和优势加权微调融合，利用新颖的对抗优势机制

Result: SHIFT框架有效解决了现代视频扩散模型监督微调中的动态度崩溃问题，提高了收敛速度并缓解了奖励黑客问题

Conclusion: 提出的像素运动奖励和SHIFT框架能够有效提升视频扩散模型的运动对齐能力，在保持视觉质量的同时增强运动保真度
...
<details>
  <summary>Details</summary>
Motivation: 现有交互式头部生成方法存在两个主要问题：1) 依赖短片段行为线索，缺乏长距离上下文建模，导致生成的面部行为缺乏上下文适当性；2) 双轨信号（用户行为和预设音频）的纠缠融合导致跨信号干扰，可能影响说话时的唇部区域同步。
...
Conclusion: ECHO框架通过长距离上下文建模和信号解耦策略，显著提升了生成头像面部行为的上下文适当性、情感合理性和唇部同步质量。
...
### [57] [Rel-Zero: Harnessing Patch-Pair Invariance for Robust Zero-Watermarking Against AI Editing](https://arxiv.org/abs/2603.17531)
*Pengzhen Chen,Yanwei Liu,Xiaoyan Gu,Xiaojun Chen,Wu Liu,Weiping Wang*

Main category: cs.CV

TL;DR: 提出Rel-Zero关系零水印框架，利用AI编辑过程中图像块间关系距离相对不变的性质，实现无需修改原图的内容认证


<details>
  <summary>Details</summary>
Motivation: 扩散模型等AI编辑技术威胁数字内容真实性，传统水印方法为保持鲁棒性会引入可感知扰动，而现有零水印方法依赖全局特征难以抵抗复杂编辑操作
...
Method: 1) 引入外部知识库检索相关句子；2) 通过不确定性估计进行事实校准，将检索信息分解为主谓宾三元组并自我/交叉精炼；3) 渐进式视觉情感增强模块利用校准的事实语义与视频内容和情感词典交互；4) 动态偏差调整路由模块预测和调整样本偏差程度
...
TL;DR: AR-CoPO：针对流式自回归视频生成器的强化学习对齐框架，通过分块级对比策略优化解决现有方法在短轨迹、低随机性场景下的对齐难题


<details>
  <summary>Details</summary>
Motivation: 现有基于SDE的GRPO方法在流式自回归视频生成场景中面临挑战：少步ODE和一致性模型采样器偏离标准流匹配ODE，短轨迹对初始化噪声高度敏感，导致中间SDE探索无效

Method: 提出AR-CoPO框架，将Neighbor GRPO对比视角适配到流式AR生成：1）引入分块级对齐机制，通过forking机制在随机选择的分块处构建邻域候选；2）分配序列级奖励；3）执行局部GRPO更新；4）提出半在线策略训练策略，结合在线探索和参考轨迹回放缓冲区利用

Result: 在Self-Forcing上的实验表明，AR-CoPO在域外泛化和域内人类偏好对齐方面均优于基线，提供了真正对齐而非奖励破解的证据

Conclusion: AR-CoPO成功解决了流式自回归视频生成器在强化学习对齐中的特殊挑战，通过分块级对比优化和半在线策略训练实现了更有效的对齐效果
...
### [60] [FrescoDiffusion: 4K Image-to-Video with Prior-Regularized Tiled Diffusion](https://arxiv.org/abs/2603.17555)
*Hugo Caselles-Dupré,Mathis Koroglu,Guillaume Jeanneret,Arnaud Dapogny,Matthieu Cord*

Main category: cs.CV

TL;DR: FrescoDiffusion：一种无需训练的扩散模型方法，用于从单张复杂图像生成高分辨率（如4K）视频，通过预计算潜在先验增强分块去噪，保持全局布局一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的图像到视频（I2V）模型在处理超高分辨率输入（如4K）时存在困难：在模型原生分辨率生成会丢失细节，而高分辨率分块去噪会破坏全局布局一致性。这在壁画动画场景中尤为严重，因为大型艺术作品包含许多不同角色、对象和语义不同的子场景，需要在时间上保持空间连贯性。

Method: FrescoDiffusion是一种无需训练的方法，核心思想是用预计算的潜在先验增强分块去噪：首先生成低分辨率视频并上采样其潜在轨迹，获得捕获长程时空结构的全局参考。对于4K生成，计算每个分块的噪声预测，并通过最小化模型输出空间中的加权最小二乘目标，在每个扩散时间步将其与全局参考融合。该目标结合标准分块合并准则和正则化项，产生闭式融合更新，增强全局一致性同时保留细节。还提供空间正则化变量，实现对运动区域的区域级控制。

Result: 在VBench-I2V数据集和提出的壁画I2V数据集上的实验显示，相比分块基线方法，FrescoDiffusion在全局一致性和保真度方面有改进，同时计算效率高。正则化方法能够显式控制创造性与一致性之间的权衡。

Conclusion: FrescoDiffusion通过引入预计算潜在先验和加权融合机制，有效解决了超高分辨率图像到视频生成中的全局一致性难题，特别适用于壁画动画等复杂场景，在保持细节的同时增强了时空连贯性。
...
### [61] [Edit-As-Act: Goal-Regressive Planning for Open-Vocabulary 3D Indoor Scene Editing](https://arxiv.org/abs/2603.17583)
*Seongrae Noh,SeungWon Seo,Gyeong-Moon Park,HyeongYeop Kang*

Main category: cs.CV

TL;DR: Edit-As-Act是一个基于目标回归规划的3D室内场景编辑框架，通过符号化目标谓词和PDDL风格的动作语言实现自然语言指令编辑，保持物理一致性和最小化改变。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇系统编辑3D场景时要么重新生成大部分场景，要么依赖图像空间编辑破坏空间结构，导致全局意外改变或物理布局不一致。这些限制源于将编辑主要视为生成任务。

Method: 采用目标回归规划视角，将用户指令视为期望的世界状态，编辑应是最小化改变序列。设计EditLang动作语言（受PDDL启发），包含显式前提条件和效果编码支撑、接触、碰撞等几何关系。语言驱动规划器提出动作，验证器强制执行目标导向性、单调性和物理可行性。

Result: 在E2A-Bench基准测试（包含9个室内环境的63个编辑任务）上，Edit-As-Act在所有编辑类型和场景类别中显著优于先前方法。

Conclusion: 通过将推理与低级生成分离，Edit-As-Act实现了指令保真度、语义一致性和物理合理性这三个现有范式无法同时满足的标准，产生可解释且物理一致的场景变换。
...
### [62] [Anchoring and Rescaling Attention for Semantically Coherent Inbetweening](https://arxiv.org/abs/2603.17651)
*Tae Eun Choi,Sumin Shim,Junhyeok Kim,Seong Jae Hwang*

Main category: cs.CV

TL;DR: 本文提出了一种新的生成式中间帧合成方法，通过关键帧锚定注意力偏置和重新缩放的时间RoPE，在稀疏序列和大运动场景下显著提升了帧一致性、语义保真度和节奏稳定性。


<details>
  <summary>Details</summary>
Motivation: 生成式中间帧合成（GI）旨在生成关键帧之间的真实中间帧，而不仅仅是简单的插值。随着序列变得更稀疏、运动幅度更大，现有GI模型存在帧不一致、节奏不稳定和语义不对齐的问题。由于GI涉及固定端点且存在多种合理路径，需要从关键帧和文本中获得额外指导来指定预期路径。

Method: 提出了两种关键技术：1）关键帧锚定注意力偏置，从关键帧和文本中提取语义和时间指导到每个中间帧；2）重新缩放的时间RoPE，使自注意力机制能更忠实地关注关键帧，从而增强帧一致性。同时建立了首个专门用于文本条件GI评估的基准TGI-Bench。

Result: 无需额外训练，该方法在短序列和长序列的各种挑战场景下，在帧一致性、语义保真度和节奏稳定性方面均达到了最先进的性能。

Conclusion: 通过关键帧锚定注意力偏置和重新缩放的时间RoPE，本文提出的方法有效解决了生成式中间帧合成中的帧不一致和语义不对齐问题，在TGI-Bench基准上展现了优越性能，为稀疏序列和大运动场景下的GI任务提供了有效解决方案。
...
### [63] [FINER: MLLMs Hallucinate under Fine-grained Negative Queries](https://arxiv.org/abs/2603.17662)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注粗粒度图像相关问题，无法充分评估多模态大语言模型在细粒度查询中的幻觉问题，特别是当图像中存在真实元素与细粒度不匹配同时出现时。

Method: 提出FINER基准测试（包括FINER-CompreCap和FINER-DOCCI），分析四种设置下的幻觉问题；提出FINER-Tuning方法，利用直接偏好优化在FINER启发数据上进行微调。

Result: 在四个前沿MLLMs上应用FINER-Tuning，在基准测试中幻觉减少最高达24.2%（InternVL3.5-14B），同时在八个现有幻觉测试套件上表现提升，并在六个基准测试中增强了一般多模态能力。

Conclusion: FINER基准测试有效揭示了MLLMs在细粒度查询中的幻觉问题，FINER-Tuning方法能显著减少幻觉并提升模型整体性能，为多模态模型评估和改进提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在将复杂结构化数字图形转换为可执行代码方面面临重大挑战，需要高保真视觉感知和精确代码生成的协同，而传统描述性任务无法评估这种能力。

Method: 创建包含1080个样本的综合基准测试，涵盖科学可视化到复杂符号表示等多种数字内容，每个样本都配有可执行参考代码。评估框架将性能解耦为感知保真度和符号精度两个维度。
...
TL;DR: 提出早期干预框架和混合低秩适配方法，解决多模态医学影像疾病识别中信息融合不充分和预训练模型适应困难的问题


<details>
  <summary>Details</summary>
Motivation: 当前多模态医学影像疾病识别方法面临两大挑战：1）主流"单模态嵌入后融合"范式无法充分利用多模态数据的互补和关联信息；2）标记多模态医学图像稀缺且与自然图像存在显著领域偏移，阻碍了视觉基础模型的应用

Method: 提出早期干预框架，将一种模态作为目标，其他作为参考，利用参考模态的高层语义标记作为干预标记，在早期阶段引导目标模态的嵌入过程；同时提出混合低秩秩适配方法，使用一组不同秩的低秩适配器和权重松弛路由器进行参数高效微调

Result: 在视网膜疾病、皮肤病变和膝关节异常分类三个公共数据集上的广泛实验验证了该方法相对于多个竞争基线的有效性

Conclusion: 提出的早期干预框架和混合低秩适配方法能够有效解决多模态医学影像疾病识别中的信息融合和模型适应问题，在多个医学影像分类任务上表现出优越性能
...
### [66] [WeatherReasonSeg: A Benchmark for Weather-Aware Reasoning Segmentation in Visual Language Models](https://arxiv.org/abs/2603.17680)
*Wanjun Du,Zifeng Yuan,Tingting Chen,Fucai Ke,Beibei Lin,Shunli Zhang*

Main category: cs.CV

TL;DR: WeatherReasonSeg是一个评估视觉语言模型在恶劣天气条件下推理分割能力的基准，包含可控合成数据集和真实世界数据集，发现VLM性能随天气严重程度单调下降且不同天气类型有不同脆弱性模式。
...
Method: 引入WeatherReasonSeg基准，包含两个互补组件：1）通过将合成天气应用于现有分割数据集构建可控推理数据集，支持细粒度鲁棒性分析；2）通过掩码引导的LLM提示生成语义一致的查询，策划真实世界恶劣天气推理分割数据集。评估范围涵盖功能、应用场景、结构属性、交互和需求匹配五个推理维度。

Result: 对不同VLM的广泛实验揭示两个关键发现：1）VLM性能随天气严重程度单调下降；2）不同天气类型导致不同的脆弱性模式。

Conclusion: WeatherReasonSeg可作为推进鲁棒、天气感知推理的基础，帮助评估和改进视觉语言模型在恶劣天气条件下的推理分割能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的稀疏视图重建方法存在两个主要问题：1）在稀疏监督下预测过多高斯基元，导致几何不稳定和深度质量差；2）仅依赖2D分割器特征进行语义提升，提供弱3D级监督，导致新场景中3D语义不完整。

Method: 提出UniSem框架，包含两个关键组件：1）误差感知高斯丢弃（EGD），利用渲染误差线索抑制冗余倾向的高斯基元，实现误差引导的容量控制；2）混合训练课程（MTC），逐步融合2D分割器提升的语义与模型自身涌现的3D语义先验，通过对象级原型对齐增强语义一致性和完整性。

Result: 在ScanNet和Replica数据集上的实验表明，UniSem在不同输入视图数量下均取得优越的深度预测和开放词汇3D分割性能。使用16视图输入时，相比强基线方法，深度相对误差降低15.2%，开放词汇分割平均准确率提升3.7%。

Conclusion: UniSem通过联合优化深度精度和语义泛化，有效解决了稀疏视图3D高斯泼溅重建中的几何不稳定和语义不完整问题，为稀疏无位姿图像的语义感知3D重建提供了统一解决方案。
...
### [68] [Eye image segmentation using visual and concept prompts with Segment Anything Model 3 (SAM3)](https://arxiv.org/abs/2603.17715)
...
<details>
  <summary>Details</summary>
Motivation: 研究最新版本的Segment Anything Model（SAM3）在眼图像分割任务中是否比SAM2表现更好，并探索其新的概念（文本）提示模式的性能。

Method: 使用包含实验室环境的高分辨率高质量视频和野外采集的挑战性眼视频（TEyeD数据集）的多样化数据集评估眼图像分割性能，比较SAM3与SAM2在视觉提示和概念提示模式下的表现。

Result: 在大多数情况下，无论是实验室数据集还是野外数据集，SAM3（无论是视觉提示还是概念提示）的表现都不如SAM2。SAM2不仅表现更好，而且速度更快。
...
TL;DR: PCA-Seg：一种并行成本聚合范式，通过专家驱动的感知学习模块和特征正交化解耦策略，解决开放词汇语义和部件分割中的知识干扰问题，在8个基准测试中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在开放词汇语义和部件分割中采用串行空间和类别聚合结构，导致类别级语义和空间上下文之间的知识干扰，限制了视觉语言对齐信息的充分提取。

Method: 提出并行成本聚合范式PCA-Seg，包含专家驱动的感知学习模块（EPL），该模块通过多专家解析器从多视角提取互补特征，并使用系数映射器自适应学习像素特定权重。同时采用特征正交化解耦策略减少语义流和上下文流之间的冗余。

Result: 在8个基准测试中，PCA-Seg的每个并行块仅增加0.35M参数，但实现了最先进的开放词汇语义和部件分割性能。

Conclusion: PCA-Seg通过并行成本聚合范式有效解决了现有方法中的知识干扰问题，能够从成本体积中捕获更丰富的视觉语言对齐信息，为开放词汇分割任务提供了简单而有效的解决方案。
...
### [70] [SARE: Sample-wise Adaptive Reasoning for Training-free Fine-grained Visual Recognition](https://arxiv.org/abs/2603.17729)
...
Method: SARE采用级联设计，结合快速候选检索和细粒度推理，仅在必要时调用后者。在推理过程中，SARE引入自反经验机制，利用过去的失败经验在推理时提供可转移的判别性指导，无需参数更新。

Result: 在14个数据集上的广泛实验证实，SARE实现了最先进的性能，同时大幅减少了计算开销。

Conclusion: SARE通过样本自适应推理框架有效解决了细粒度视觉识别中的挑战，在保持高性能的同时显著提升了效率，为训练免费的细粒度识别提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 开放词汇分割在遥感领域潜力巨大，但现有方法主要局限于晴朗天气的光学数据，在云雾等恶劣天气条件下表现不佳。光学和SAR数据具有互补优势，光学提供丰富的光谱语义信息，SAR具有穿透云雾的能力，因此需要开发能够融合这两种模态的鲁棒分割方法。

Method: 提出MM-OVSeg多模态光学-SAR融合框架，包含两个关键设计：1）跨模态统一过程，用于多传感器表示对齐；2）双编码器融合模块，集成多个视觉基础模型的分层特征，实现文本对齐的多模态分割。

Result: 大量实验表明，MM-OVSeg在不同云层条件下实现了优越的鲁棒性和泛化能力。该方法在恶劣天气条件下的开放词汇分割任务中表现优异。

Conclusion: MM-OVSeg通过有效融合光学和SAR数据的互补优势，解决了恶劣天气条件下开放词汇分割的挑战，为遥感领域的多模态分割提供了新的解决方案。
...
### [72] [Fine-Grained Post-Training Quantization for Large Vision Language Models with Quantization-Aware Integrated Gradients](https://arxiv.org/abs/2603.17809)
*Ziwei Xiang,Fanhu Zeng,Hongjian Fang,Rui-Qi Wang,Renxing Chen,Yanan Zhu,Yi Chen,Peipei Yang,Xu-Yao Zhang*

Main category: cs.CV

TL;DR: 该论文提出了一种基于量化感知积分梯度的细粒度量化策略，用于大型视觉语言模型的量化加速，将粒度从模态级别提升到令牌级别，显著提升了量化精度。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然在下游任务中表现出色，但存在巨大的计算和内存开销，阻碍了实际部署。现有的量化方法通常在模态级别测量令牌敏感性，无法捕捉复杂的跨令牌交互，也无法在令牌级别定量测量量化误差。

Method: 受机制可解释性中公理归因的启发，提出了基于量化感知积分梯度的细粒度量化策略。该方法利用积分梯度定量评估令牌敏感性，将粒度从模态级别提升到令牌级别，同时反映模态间和模态内的动态特性。

Result: 在多个LVLM模型上的W4A8和W3A16设置下进行广泛实验，该方法在可忽略的延迟开销下提高了模型和基准测试的准确性。例如，在3位仅权重量化下，LLaVA-onevision-7B的平均准确率提高了1.60%，与全精度版本的差距缩小到仅1.33%。

Conclusion: 提出的QIG方法通过细粒度的令牌级别量化策略，有效解决了现有量化方法在捕捉跨令牌交互方面的不足，显著提升了量化精度，为大型视觉语言模型的实用部署提供了有效的量化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型采用循环帧处理机制，在像素域训练时会导致内存成本过高，因为激活值在整个视频序列中累积。这一根本限制使得使用像素级损失对长视频或高分辨率视频进行微调在计算上不可行。

Method: 提出ChopGrad截断反向传播方案，将梯度计算限制在局部帧窗口内，同时保持全局一致性。该方法通过理论分析验证了这种近似的有效性，并支持使用帧级损失进行高效微调。

Result: ChopGrad将训练内存从与视频帧数线性增长（完整反向传播）降低到恒定内存。在多种条件视频生成任务中表现优异，包括视频超分辨率、视频修复、神经渲染场景的视频增强以及受控驾驶视频生成。

Conclusion: ChopGrad通过截断反向传播有效解决了视频扩散模型训练中的内存瓶颈问题，实现了高效微调，同时保持了生成质量，在各种视频生成任务中优于现有最先进方法。
...
TL;DR: Video-SFT在多模态大语言模型中能提升视频理解能力，但会损害静态图像性能，这种权衡与时间预算密切相关


<details>
  <summary>Details</summary>
Motivation: 研究视频监督微调如何影响多模态大语言模型的视觉能力演化，特别是空间与时间理解之间的平衡关系

Method: 系统研究不同架构、参数规模和帧采样设置下的Video-SFT效果，提出指令感知的混合帧策略来缓解图像-视频权衡

Result: Video-SFT能可靠提升视频性能，但对静态图像基准测试的增益有限甚至会导致性能下降；增加采样帧数能改善视频性能，但不能可靠提升静态图像性能
...
TL;DR: ProGVC是一个渐进式生成视频压缩框架，通过多尺度残差令牌映射和自回归上下文模型，在低比特率下实现感知压缩，同时支持渐进传输和可变比特率。


<details>
  <summary>Details</summary>
Motivation: 现有感知视频编解码器通常缺乏对可变比特率和渐进传输的原生支持，且生成模块与熵编码弱耦合，限制了比特率降低。

Method: 提出ProGVC框架，将视频编码为分层多尺度残差令牌映射，支持渐进传输；使用基于Transformer的多尺度自回归上下文模型估计令牌概率，用于熵编码和预测截断的细尺度令牌以恢复感知细节。

Result: 实验表明ProGVC在低比特率下提供了有前景的感知压缩性能，同时具有实际可扩展性。

Conclusion: ProGVC作为一种新的编码范式，统一了渐进传输、高效熵编码和细节合成，在低比特率下实现了良好的感知压缩效果。
...
TL;DR: 该论文提出了一种特征保留的人脸匿名化框架，在保护身份隐私的同时更好地保留面部表情、光照方向和肤色等关键特征，解决了现有方法忽视表情和光度一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 社交媒体和大规模数据集中人脸图像的广泛共享引发了严重的隐私问题，现有的人脸匿名化方法主要关注身份移除和图像真实性，但往往忽视了面部表情以及光照和肤色等关键的光度一致性特征，而这些特征对于重光照、色彩恒常性、医疗或情感分析等下游任务至关重要。

Method: 提出了一种特征保留的匿名化框架，扩展了DeepPrivacy方法：1）通过引入密集面部关键点来更好地保留表情；2）通过轻量级后处理模块确保光照方向和肤色的一致性；3）建立了专门评估表情保真度、光照一致性和颜色保留的量化指标。

Result: 在CelebA-HQ数据集上的实验表明，该方法生成的匿名化人脸在真实感方面有所提升，并且在表情、光照和肤色保真度方面显著优于现有最先进的基线方法。

Conclusion: 特征感知的匿名化对于实现更有用、更公平、更可信的隐私保护人脸数据至关重要，该方法在保护身份隐私的同时更好地保留了关键的面部特征，为下游任务提供了更实用的匿名化数据。
...
<details>
  <summary>Details</summary>
Motivation: 全景图像提供360°视野但在消费设备中越来越普遍，但其非针孔畸变给联合位姿估计和3D重建带来挑战。现有的前馈模型针对透视相机设计，在此场景下泛化能力差。

Method: 提出PanoVGGT框架，采用置换等变Transformer，包含球面感知位置嵌入和全景特定的三轴SO(3)旋转增强，实现球面域的有效几何推理。为解决全局坐标系模糊性，引入随机锚定训练策略。同时贡献了PanoCity数据集。

Result: 在PanoCity和标准基准测试上的大量实验表明，PanoVGGT实现了竞争性的精度、强鲁棒性和改进的跨域泛化能力。

Conclusion: PanoVGGT是首个能够从全景图像中联合预测相机位姿、深度图和3D点云的置换等变Transformer框架，在多个指标上表现优异，代码和数据集将开源。
...
TL;DR: LoGSAM：基于语音驱动、参数高效的脑肿瘤定位分割框架，通过语音转录、文本提示生成、视觉语言检测和分割模型，实现少参数更新的高性能肿瘤分割


<details>
  <summary>Details</summary>
Motivation: 现有脑肿瘤MRI定位分割方法大多依赖任务特定的监督模型，受限于标注数据稀缺。需要开发参数高效、能够利用放射科医生语音描述进行肿瘤定位分割的框架

Method: 1) 使用Whisper ASR转录翻译放射科医生语音；2) 基于临床NLP提取肿瘤特异性文本提示；3) 通过LoRA适配的Grounding DINO进行文本条件肿瘤定位；4) 将预测边界框作为MedSAM的提示生成像素级肿瘤掩码

Result: 在BRISC 2025数据集上达到80.32%的Dice分数；在12个未见过的德国语音MRI扫描上实现91.7%的病例级准确率；仅更新5%模型参数

Conclusion: 通过智能利用预训练基础模型和最小参数更新，成功构建了模块化的语音到分割流水线，展示了参数高效、语音驱动的脑肿瘤定位分割的可行性
...
### [79] [Unified Spatio-Temporal Token Scoring for Efficient Video VLMs](https://arxiv.org/abs/2603.18004)
...
<details>
  <summary>Details</summary>
Motivation: 现有令牌剪枝方法存在局限性：要么仅在ViT中剪枝（仅适用于单模态感知任务），要么仅在LLM中剪枝（需要复杂的文本条件令牌选择机制）。需要一种统一的、架构范围内的视觉令牌剪枝方法，适用于视觉语言任务，特别是视频任务中的时间冗余问题。

Method: 提出时空令牌评分（STTS）模块：1）通过辅助损失学习时间评分；2）通过LLM下游梯度学习空间评分；3）结合高效的打包算法；4）无需文本条件或令牌合并；5）完全兼容端到端训练；6）在ViT和LLM中同时剪枝视觉令牌。

Result: 在13个长短视频QA任务中，剪枝50%视觉令牌，训练和推理效率提升62%，平均性能仅下降0.7%。效率增益随视频采样帧数增加而增加。在长视频QA中应用测试时缩放，性能比基线提升0.5-1%。

Conclusion: STTS是一种新颖、简单而有效的统一架构范围内视觉令牌剪枝技术，特别适用于视频视觉语言模型，能显著提升计算效率同时保持性能。
...
### [80] [S-VGGT: Structure-Aware Subscene Decomposition for Scalable 3D Foundation Models](https://arxiv.org/abs/2603.17625)
*Xinze Li,Pengxu Chen,Yiyuan Wang,Weifeng Su,Wentao Cheng*

Main category: cs.CV

TL;DR: S-VGGT通过结构层面的冗余处理解决3D基础模型中的二次计算成本问题，使用场景图进行场景划分，实现并行高效处理，可与现有token级加速方法结合获得叠加加速效果。


<details>
  <summary>Details</summary>
Motivation: 前馈式3D基础模型面临全局注意力带来的二次计算成本问题，限制了模型的可扩展性。现有的token级加速方法虽然能提供局部节省，但存在最近邻搜索开销，且未能解决密集捕获数据中占主导地位的结构冗余问题。

Method: S-VGGT在结构帧层面处理冗余：1）利用初始特征构建密集场景图，表征结构冗余并指导场景划分；2）将帧软分配到少量子场景中，确保平衡分组和平滑几何过渡；3）设计子场景共享共同参考帧，建立并行几何桥梁，实现无需显式几何对齐的独立高效处理。

Result: 该方法从根本上削减了全局注意力成本，提供了强大的内在加速。更重要的是，S-VGGT与token级加速方法完全正交，可以无缝结合实现叠加加速，同时不损害重建保真度。

Conclusion: S-VGGT通过结构层面的优化解决了3D基础模型的核心计算瓶颈，提供了一种高效且可扩展的解决方案，能够与现有加速技术结合获得显著性能提升。
...
### [81] [Part-Aware Open-Vocabulary 3D Affordance Grounding via Prototypical Semantic and Geometric Alignment](https://arxiv.org/abs/2603.17647)
*Dongqiang Gou,Xuming He*

Main category: cs.CV

TL;DR: 该论文提出了一种新颖的两阶段跨模态框架，用于开放词汇的3D功能可供性定位，通过大语言模型生成部件感知指令恢复缺失语义，并引入APA和IORM组件增强几何表示。
...
Method: 提出两阶段框架：第一阶段使用大语言模型生成部件感知指令恢复缺失语义；第二阶段引入APA（功能可供性原型聚合）捕获跨对象几何一致性，以及IORM（对象内关系建模）细化对象内几何区分。

Result: 在新引入的基准测试和两个现有基准测试上进行广泛实验，证明该方法相比现有方法具有优越性能。

Conclusion: 该框架通过增强语义和几何表示，有效解决了开放词汇3D功能可供性定位中的关键挑战，在多个基准测试上表现出色。
...
### [82] [Few-Step Diffusion Sampling Through Instance-Aware Discretizations](https://arxiv.org/abs/2603.17671)
*Liangyu Yuan,Ruoyu Wang,Tong Zhao,Dingwen Fu,Mingkun Lei,Beier Zhu,Chi Zhang*

Main category: cs.CV

TL;DR: 提出了一种实例感知的离散化框架，通过学习基于输入相关先验调整时间步分配，在条件生成设置中扩展基于梯度的离散化搜索，相比全局共享时间步调度能提升生成质量


<details>
  <summary>Details</summary>
Motivation: 现有扩散和流匹配模型大多采用全局共享的时间步调度策略，这种统一处理未能考虑生成过程中实例特定的复杂性，可能限制性能。在合成数据上的控制实验揭示了全局调度在实例特定动态下的次优性

Method: 提出实例感知离散化框架，学习基于输入相关先验自适应调整时间步分配，将基于梯度的离散化搜索扩展到条件生成设置

Result: 在合成数据、像素空间扩散、潜在空间图像和视频流匹配模型等多种设置中，该方法一致提高了生成质量，相比训练仅有边际调整成本，推理开销可忽略

Conclusion: 实例感知的离散化框架能够有效解决全局时间步调度的局限性，通过学习输入相关的先验自适应调整时间步分配，在多种生成任务中显著提升性能
...
TL;DR: DeepCORO-CLIP是一个基于多视角视频-文本对比学习训练的冠脉造影基础模型，在超过20万段造影视频上训练，能够进行全面的冠脉评估，包括狭窄检测、预后预测和疾病进展追踪。


<details>
  <summary>Details</summary>
Motivation: 传统冠脉造影的视觉解读存在观察者间差异，现有AI方法通常只分析单帧或单投影，主要关注狭窄检测，限制了全面的冠脉评估能力。

Method: 使用视频-文本对比学习方法，在203,808段冠脉造影视频上训练多视角基础模型，整合多个投影并使用注意力池化进行研究级评估，支持诊断、预后和疾病进展追踪任务。

Result: 模型在显著狭窄检测上内部验证AUROC为0.888，外部验证为0.89；与核心实验室定量冠脉造影相比，平均绝对误差为13.6%，优于临床报告的19.0%；在慢性完全闭塞、冠脉内血栓和冠脉钙化检测上表现良好；迁移学习可预测一年主要不良心血管事件（AUROC 0.79）和估计左心室射血分数（平均绝对误差7.3%）。

Conclusion: DeepCORO-CLIP提供了一个用于自动化冠脉造影解读的基础模型，在医院部署中平均推理时间仅4.2秒，代码、样本数据、模型权重和部署基础设施均已公开。
...
TL;DR: 该研究探索了使用闪光-非闪光配对采集作为接触式指纹活体检测的轻量级主动感知机制，通过光照差异分析来区分真实指纹与各种伪造攻击。


<details>
  <summary>Details</summary>
Motivation: 接触式指纹识别虽然卫生方便，但由于缺乏物理接触和传统活体线索，给伪造检测带来了新挑战。现有方法主要依赖单图像采集和基于外观的特征，在不同设备、采集条件和伪造材料间泛化能力较差。

Method: 采用闪光-非闪光配对采集作为主动感知机制，通过分析光照引起的差异，使用可解释的指标如通道间相关性、镜面反射特性、纹理真实性和差分成像来提取互补特征。

Result: 闪光照明增强了材料和结构相关特性（如脊线可见性、次表面散射、微几何和表面油脂），而非闪光图像提供基线外观上下文。这些互补特征有助于区分真实指纹与打印、数字和模制等伪造攻击。

Conclusion: 研究证明了光照感知分析在提高接触式指纹伪造攻击检测的鲁棒性和可解释性方面的潜力，为未来的配对采集和物理信息特征设计提供了方向。
...
TL;DR: AFSS提出一种反遗忘采样策略，动态选择训练图像，减少冗余计算，在保持精度同时显著加速YOLO系列检测器的训练过程


<details>
  <summary>Details</summary>
Motivation: YOLO检测器虽然推理速度快，但训练过程仍然耗时，因为每个epoch都要处理所有训练图像，即使许多图像已经充分学习。这与"你只看一次"的高效理念相矛盾，因此需要探索是否真的需要在每个epoch中看到所有训练图像

Method: 提出反遗忘采样策略(AFSS)，通过测量每个训练图像的学习充分性（检测召回率和精度的最小值），将图像动态分为简单、中等、困难三个级别。简单图像稀疏重采样，中等图像部分选择，困难图像每个epoch都采样。定期更新学习充分性，使检测器能自适应地关注信息丰富的图像

Result: 在自然图像检测基准（MS COCO 2017和PASCAL VOC 2007）和遥感检测数据集（DOTA-v1.0和DIOR-R）上，AFSS为YOLO系列检测器实现了超过1.43倍的训练加速，同时提高了准确率

Conclusion: AFSS策略有效解决了YOLO训练效率低下的问题，通过动态采样机制在保持甚至提升检测性能的同时显著加速训练过程，实现了训练效率与性能的双重提升
...
TL;DR: SynRL是一个通过合成视频数据训练视觉语言模型掌握时间理解基础技能的后训练框架，使用几何形状生成的7.7K合成数据在多个视频理解基准上超越了使用165K真实世界数据的方法。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型从图像理解转向视频理解时面临两个关键问题：1)现有数据集缺乏时间中心性，答案可以从孤立的关键帧推断而不需要整体时间整合；2)由专有模型生成的训练数据在基本时间感知（如运动方向、速度判断）上存在系统性错误。

Method: 提出SynRL后训练框架，通过程序化生成的合成视频教授模型时间原语（包括方向、速度和状态跟踪等时间理解的基础构建块）。将时间理解分解为短期感知原语（速度、方向）和长期认知原语，通过基于代码的视频生成构建了7.7K CoT和7K RL样本，并带有真实帧级标注。

Result: 尽管仅使用简单几何形状训练，SynRL在涵盖时间定位、复杂推理和通用视频理解的15个基准测试中取得了显著改进。7.7K合成CoT样本的表现超过了使用165K真实世界样本的Video-R1方法，证明了从抽象合成模式到复杂现实场景的有效迁移。

Conclusion: 通过精心设计的合成数据进行视频时间学习为视频后训练提供了新的范式，这是一种更具成本效益的扩展路径。基础时间技能（如逐帧变化跟踪和速度比较）能够从抽象合成模式有效迁移到复杂现实场景。
...
TL;DR: MoBaNet：一种参数高效、模态平衡的对称融合框架，用于多模态遥感语义分割，通过冻结的视觉基础模型、跨模态提示注入适配器和差异引导的门控融合模块实现


<details>
  <summary>Details</summary>
Motivation: 多模态遥感语义分割利用异构数据的互补物理线索增强场景解释，但现有方法存在计算开销大和模态不平衡问题，其中辅助模态在优化过程中被抑制

Method: 1. 基于冻结VFM骨干的对称双流架构；2. 跨模态提示注入适配器(CPIA)实现深度语义交互；3. 差异引导门控融合模块(DGFM)自适应融合特征；4. 模态条件随机掩码(MCRM)策略缓解模态不平衡

Result: 在ISPRS Vaihingen和Potsdam基准测试中达到最先进性能，且训练参数显著少于全微调方法，验证了其鲁棒和平衡的多模态融合有效性
...
TL;DR: DiffVP提出了一种基于差异视觉提示的CT报告生成方法，通过显式提取扫描与参考图像之间的语义差异来指导报告生成，而非仅依赖绝对视觉特征。


<details>
  <summary>Details</summary>
Motivation: 现有CT报告生成方法通常整体编码3D体积，无法区分信息性线索与冗余的解剖背景。受放射学认知减法启发，需要基于显式的高层语义差异而非仅依赖绝对视觉特征来生成报告。

Method: 提出差分视觉提示（DiffVP）方法：1）使用分层差异提取器捕获全局和局部语义差异到共享潜在空间；2）通过差异到提示生成器将这些信号转换为可学习的视觉前缀标记，用于LLM条件化；3）这些差异提示作为结构化条件信号，隐式抑制不变解剖结构，同时放大诊断相关视觉证据。

Result: 在两个大规模基准测试中，DiffVP始终优于先前方法，分别将平均BLEU-1-4提高了+10.98和+4.36，并在RadGenome-ChestCT上进一步提升了临床效果（F1分数0.421）。

Conclusion: DiffVP通过显式提取扫描与参考图像之间的语义差异，为CT报告生成提供了更有效的条件信号，无需显式病灶定位即可生成准确报告，显著提升了生成质量和临床相关性。
...
Method: 将3D外观生成重构为几何条件视频扩散问题：给定3D网格，渲染编码多模态几何特征以像素级精度约束视频生成。设计下游重建流水线，包含3D感知修复，通过模型旋转和上下文感知二次生成完成自遮挡区域。

Result: 实验结果表明，该方法在视频一致性和最终重建质量方面均优于现有方法。生成的视频不仅是高质量动态预览，还可作为可靠的3D感知中间表示，无缝反投影到UV纹理或监督神经渲染方法。
...
TL;DR: C2P是一个无需提示的通用医学图像分割框架，通过解耦几何和语义表示，利用MLLM提取语义token并引入几何token，实现跨模态的精确分割。


<details>
  <summary>Details</summary>
Motivation: 现有通用医学图像分割方法过度依赖手动视觉提示或参考图像，限制了自动化和鲁棒性；同时跨模态的联合训练难以处理大的域偏移问题。

Method: 提出Concept-to-Pixel框架：1) 将解剖知识解耦为几何和语义表示；2) 利用MLLM提取可学习的语义token；3) 引入显式监督的几何token来强制通用物理和结构约束；4) 这些解耦的token与图像特征深度交互生成输入特定的动态核进行掩码预测；5) 引入几何感知推理共识机制评估预测可靠性并抑制异常值。

Result: 在包含7种模态、8个数据集的统一基准测试中，该方法相比通用或单模型方法表现出显著优势；统一模型在零样本任务和跨模态迁移任务中展现出强大的泛化能力。

Conclusion: C2P框架通过解耦几何和语义表示，实现了无需提示的通用医学图像分割，在跨模态任务中表现出优越性能和强大泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位方法在简单单物体场景中表现良好，但在真实世界常见的复杂多物体场景中性能严重下降。主要面临两个挑战：1) 对区分视觉相似物体至关重要的隐式定位线索解析不足；2) 对共现物体的动态空间干扰抑制无效，导致定位精度下降。

Method: 提出PC-CrossDiff统一双任务框架，包含双级跨模态差分注意力架构：1) 点级差分注意力模块，通过文本与点云之间的双向差分注意力，自适应提取隐式定位线索；2) 聚类级差分注意力模块，建立分层注意力机制，自适应增强定位相关空间关系，同时通过定位感知差分注意力块抑制模糊或不相关的空间关系。
...
TL;DR: SCEP是一种无需训练的LVLM框架，通过挖掘可疑补丁令牌进行证据驱动推理，用于图像深度伪造检测。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型具有强大的图像理解能力，但将其适应于图像深度伪造检测通常需要昂贵的微调，并且对多样化和不断演变的篡改操作泛化能力差。

Method: SCEP使用视觉编码器的CLS令牌作为全局参考，将补丁特征聚类成连贯组，通过融合CLS引导的语义不匹配与频率和噪声异常指标来评分补丁，从每个聚类中采样高置信度补丁并应用基于网格的NMS，生成证据包用于冻结LVLM的预测。

Result: 在多样化基准测试中，SCEP在不进行LVLM微调的情况下优于强基线方法。

Conclusion: SCEP提供了一种无需训练的证据驱动推理框架，能够有效检测图像深度伪造，克服了传统LVLM微调方法的成本和泛化限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有单视图3D人体重建研究主要关注清晰、近距离的单人图像，在更常见的多人场景中表现不佳。多人3D重建面临遮挡严重、清晰度低和外观多样性三大挑战。

Method: 1. 自监督适应管道：让预训练大型人体模型能从严重遮挡输入重建完整3D人体；2. 自校准学习：通过混合身份保持样本与干净/损坏图像对，使单步扩散模型自适应优化粗糙渲染；3. 输出蒸馏回多人3DGS表示以提升质量

Result: 大量实验表明CrowdGaussian能生成逼真、几何一致的多人物场景重建

Conclusion: CrowdGaussian是一个统一的框架，能直接从单张图像重建多人3D高斯溅射表示，有效解决了多人场景中的遮挡、清晰度和外观多样性问题
...
TL;DR: 本研究系统比较了三种方法在精准畜牧业行为分类中的表现：从头训练、冻结特征提取和参数高效微调（PEFT）。结果显示PEFT方法显著优于其他方法，最佳配置在仅使用2.72%参数的情况下达到83.16%的测试准确率。


<details>
  <summary>Details</summary>
Motivation: 精准畜牧业中的自动化行为分类面临计算成本高和标注数据有限的双重挑战。本研究旨在探索如何高效利用大规模基础模型（如DINOv3）来解决这些问题。

Method: 系统比较了三种方法：1）从头训练（ResNet-18, ViT-Small）；2）冻结特征提取；3）参数高效微调（PEFT）DINOv3基础模型。评估了QLoRA和DoRA在不同配置（秩=8,16,64；目标模块=q_proj vs 所有线性层）下的表现。

Result: PEFT方法显著优于其他方法。最佳QLoRA配置（所有线性层，秩=64）达到83.16%测试准确率，仅使用2.72%参数（183.0M），训练时间5.8小时。相比之下，ResNet-18为72.87%（16.8小时），ViT-Small为61.91%（18.7小时），冻结DINOv3为76.56%（17.5小时）。DoRA达到类似准确率（83.14%）但训练时间更长（11.0小时）。

Conclusion: 参数高效微调（PEFT）是部署十亿参数视觉模型到农业畜牧应用的有效策略。研究发现欠拟合而非过拟合是主要挑战，增加适配器容量能持续改善泛化性能。这为农业图像分析中高效利用大规模基础模型提供了指导。
...
TL;DR: 基于ResNet-50帧分类器和解剖引导时序事件解码的多标签胃肠道视频分析系统，通过类别权重调整和时序优化策略，在严重类别不平衡情况下提升了罕见病理标签的检测性能。


<details>
  <summary>Details</summary>
Motivation: 开发能够同时识别胃肠道解剖结构和病理变化的多标签视频分析系统，主要挑战在于严重的类别不平衡问题，特别是罕见病理标签的检测困难。

Method: 采用ResNet-50作为帧级分类器，输入336x336尺寸图像；使用裁剪的类别正样本权重调整训练损失函数；在时序阶段结合GT风格帧级事件组合、解剖投票平滑、基于解剖的病理门控和保守滞后解码器。

Result: 系统能够预测17个标签（5个解剖类别和12个病理类别）；通过权重调整改善了罕见类别学习；时序优化使最终时序mAP从0.3801提升到0.4303。

Conclusion: 提出的多标签胃肠道视频分析管道通过有效的类别不平衡处理和时序解码策略，显著提升了罕见病理检测性能，为临床视频分析提供了实用解决方案。
...
TL;DR: 提出Mask-to-Point学习框架，利用视频对象分割掩码标注改进视觉基础模型，用于密集点跟踪任务


<details>
  <summary>Details</summary>
Motivation: 当前基于静态图像预训练的视觉基础模型在捕捉视频中的密集时间对应关系方面存在不足，需要更适合视频点跟踪的预训练方法

Method: 提出三种基于掩码的弱监督表示学习约束：局部结构一致性损失（使用Procrustes分析）、掩码标签一致性损失、掩码边界约束

Result: 仅使用3.6K个VOS训练视频，M2P模型在TAP-Vid-DAVIS基准上分别比DINOv2-B/14和DINOv3-B/16提升12.8%和14.6%

Conclusion: M2P学习框架能有效改进视觉基础模型用于点跟踪任务，可作为点跟踪任务的通用预训练模型
...
TL;DR: 论文提出了一种基于大规模激活（MAs）的无需训练的自引导方法STAS，通过引导视频扩散变换器中特定token位置的激活值来提升视频生成质量和时序一致性。
...
Method: 提出结构化激活引导（STAS），一种无需训练的自引导方法。该方法将第一帧token和边界token（潜在空间中每个时间块的首尾部分）的MA值引导至缩放后的全局最大参考幅度。

Result: STAS在不同文本到视频模型中实现了视频质量和时序一致性的持续改进，同时引入的计算开销可以忽略不计。

Conclusion: 通过利用视频扩散变换器中大规模激活的结构化模式，提出的STAS方法能够有效提升视频生成质量，为利用模型内部信号进行高效优化提供了新思路。
...
TL;DR: TINA攻击方法证明当前文本中心的概念消除技术只是掩盖而非真正消除概念，视觉知识仍然存在


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型的概念消除技术主要关注切断文本到图像的映射，但忽略了底层视觉知识的持续存在，需要从视觉角度验证概念是否真正被消除

Method: 提出TINA（Text-free INversion Attack）方法，在零文本条件下使用DDIM反演来寻找视觉生成路径，避免现有文本中心防御的干扰，并集成优化程序克服无文本指导时的累积近似误差

Result: TINA成功从经过最先进遗忘处理的模型中重新生成被消除的概念，证明当前方法只是掩盖概念而非真正消除

Conclusion: 当前文本中心的概念消除范式存在根本缺陷，需要开发直接操作内部视觉知识的新方法来实现真正的概念消除
...
TL;DR: 这篇综述系统梳理了视频理解领域，从三个互补视角组织文献：低层视频几何理解、高层语义理解和统一视频理解模型，总结了从孤立任务特定方法向统一建模范式的转变趋势。
...
Method: 采用结构化综述方法，将文献组织为三个互补视角：1）低层视频几何理解；2）高层语义理解；3）统一视频理解模型。通过这种分类框架分析领域发展趋势。

Result: 提供了视频理解领域的连贯图谱，总结了关键建模趋势和设计原则，揭示了从孤立任务特定流水线向统一建模范式的广泛转变。

Conclusion: 视频理解领域正朝着构建鲁棒、可扩展和统一的视频基础模型方向发展，需要解决开放挑战以实现更系统的进展。
...
TL;DR: 该研究全面评估了细胞分割基础模型（CellPoseSAM、CellSAM、μSAM）和通用分割模型（SAM、SAM2、SAM3）在多种显微镜数据集上的表现，并提出了自动提示生成（APG）策略来改进基于SAM的显微镜基础模型。


<details>
  <summary>Details</summary>
Motivation: 细胞分割是显微镜图像分析的基础任务，虽然已有多个基于Segment Anything Model（SAM）的细胞分割基础模型，但最近发布的SAM2和SAM3进一步提升了通用分割能力。需要全面评估这些模型在显微镜数据上的表现，并探索改进策略。

Method: 1. 在多样化的（光）显微镜数据集上评估细胞分割基础模型（CellPoseSAM、CellSAM、μSAM）和通用分割模型（SAM、SAM2、SAM3）；2. 提出新的实例分割策略——自动提示生成（APG），用于改进基于SAM的显微镜基础模型；3. 以μSAM为基础模型测试APG效果。

Result: APG策略持续改善了基于μSAM的分割结果，与最先进的CellPoseSAM模型表现相当。研究为SAM风格模型在显微镜领域的适应策略提供了重要经验，并为创建更强大的显微镜基础模型提供了策略。

Conclusion: 该研究通过全面评估和提出APG策略，为显微镜图像分割领域提供了重要见解。APG能有效改进基于SAM的显微镜基础模型，代码已公开供社区使用，有助于推动更强大的显微镜基础模型发展。
...
TL;DR: 该研究比较了不同形式的人类感知先验（手绘标注、眼动热图、分割掩码和DINOv2嵌入）在开放集虹膜呈现攻击检测中的效果，发现去噪后的眼动热图在泛化性能上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 人类感知先验在显著性引导的深度学习训练中显示出潜力，特别是在虹膜呈现攻击检测领域。然而，对于开放集虹膜PAD任务，哪种形式的人类显著性最有效仍未被充分探索。

Method: 通过一系列实验比较了手绘标注、眼动追踪热图、分割掩码和DINOv2嵌入，与最先进的深度学习基线模型在开放集虹膜PAD任务上进行对比。采用留一攻击类型出的范式进行评估。

Result: 在开放集PAD任务中，去噪后的眼动追踪热图在ROC曲线下面积和BPCER为1%时的APCER指标上，相对于交叉熵基线表现出最佳的泛化改进。
...
TL;DR: 图像编辑模型存在"编辑溢出"现象：模型会修改指定编辑区域之外的相关内容。该研究将溢出作为探测模型世界知识的自然探针，开发了系统框架和基准数据集。


<details>
  <summary>Details</summary>
Motivation: 指令跟随式图像编辑模型在实际应用中普遍存在"编辑溢出"现象，即模型不仅修改指定区域，还会改变语义相关但未指定的内容。这引发了一个根本问题：溢出是反映了模型真正的隐式世界理解，还是仅仅是注意力泄漏？

Method: 提出了EditSpilloverProbe框架，将编辑溢出重新定位为图像编辑模型中世界知识的自然探针。包括：1) 建立溢出分类法（空间、语义、混合、随机）；2) 开发自动检测和分类流水线；3) 基于真实中文文本编辑任务构建基准数据集EditSpilloverBench；4) 系统评估5个代表性编辑模型。

Result: 1) 不同架构的溢出率差异显著（3.49%到11.46%，相差3.3倍）；2) 绝对语义溢出数量反映模型的世界理解能力：nano_banana产生最多语义溢出（每图27.8个），而qwen_2511编辑控制最精确但语义溢出较少（每图16.3个），揭示了编辑控制与世界理解之间的权衡；3) 空间衰减分析显示溢出区域密度随距离呈指数衰减，但语义相关溢出的比例保持恒定（40%-58%），证明语义溢出反映真实世界理解而非空间扩散。

Conclusion: 编辑溢出现象可以作为探测图像编辑模型世界理解能力的有效工具。语义溢出反映了模型真正的世界知识而非简单的注意力泄漏，这为评估和改进图像编辑模型提供了新的视角和方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏一个开放可访问的框架来实现对多身份面部外观和声音音色的细粒度控制，需要统一的身份感知音视频生成解决方案。

Method: 1) 设计数据整理流水线自动提取跨模态的身份信息；2) 提出灵活可扩展的身份注入机制；3) 采用多阶段训练策略加速收敛并增强跨模态一致性。

Result: 实验证明该框架具有优越性，能够实现高保真、一致的身份感知音视频生成，支持单主体到多主体交互的多样化场景。

Conclusion: 提出了一个统一、可扩展的身份感知音视频生成框架，通过面部外观和声音音色作为控制信号，实现了高质量的多身份个性化内容创建。
...
TL;DR: SpiderCam是一款基于FPGA的实时稀疏深度相机，通过差分离焦技术实现480x400深度图，在52cm工作范围内以32.5FPS运行，总功耗仅624mW。


<details>
  <summary>Details</summary>
Motivation: 开发一种低功耗的实时3D深度相机，解决现有被动式FPGA 3D相机功耗过高的问题，实现首个亚瓦特级功耗的被动式FPGA 3D相机。

Method: 采用定制相机同时捕捉同一场景的两个不同对焦图像，使用SystemVerilog在低功耗FPGA上实现差分离焦深度计算算法，通过算法改进克服低功耗传感器带来的挑战，并设计内存本地化实现以适应无法存储完整图像对的设备限制。

Result: 实现了480x400稀疏深度图的实时生成（32.5FPS），工作范围52cm，总功耗仅624mW，成为文献中首个亚瓦特级功耗的被动式FPGA 3D相机。

Conclusion: SpiderCam成功展示了通过算法优化和硬件设计，可以在极低功耗下实现实时深度感知，为低功耗3D视觉应用开辟了新途径。
...
Method: 提出基于变分自编码器(VAE)的半灰盒噪声感知异常检测框架，包含鲁棒的噪声感知特征，能够捕捉环境噪声的特征行为，提高检测精度并降低误报率

Result: 在流行的物体分类DNN上评估，显示所提检测方法在现实噪声条件下具有鲁棒性（在不同DNN配置下AUROC高达90%），同时揭示了特征相似性和高噪声水平带来的局限性

Conclusion: 提出的框架能够有效检测边缘AI协同推理中的恶意数据注入攻击，在噪声环境下保持高检测性能，为边缘AI安全提供了有效解决方案
...
TL;DR: 提出了一种基于几何驱动的2D-3D-2D范式，通过多视角冗余自动将少量手动标注的RGB图像标签传播到RGB和热成像模态，构建了大规模无人机语义分割数据集SegFly


<details>
  <summary>Details</summary>
Motivation: 现有无人机RGB和RGB-T数据集在规模、多样性和标注效率方面受限，主要因为手动标注成本高且商用无人机难以实现准确的RGB-T对齐

Method: 采用几何驱动的2D-3D-2D范式：1) 将少量（<3%）手动标注的RGB图像提升为语义3D点云；2) 将点云重投影到所有视角生成密集伪真值；3) 将3D几何作为中间对齐空间实现跨模态图像配准

Result: 自动生成97%的RGB标签和100%的热成像标签，标注准确率达91%和88%；实现87%的RGB-T配准精度；构建了包含2万多张RGB图像和1.5万多对RGB-T对齐图像的大规模数据集SegFly
...
TL;DR: 本文提出C-TRAIL多模态法律数据集和两阶段框架，将行车记录仪视频与法律责任模式和法律条文对齐，实现交通事故的自动化法律分析。


<details>
  <summary>Details</summary>
Motivation: 虽然行车记录仪视频证据日益丰富，但将视频内容转化为法律责任判定仍依赖人工专家。现有研究存在明显差距：交通事故研究主要关注感知和语义理解，而基于LLM的法律方法大多基于文本案例描述，很少整合视频证据。

Method: 1. 提出C-TRAIL多模态法律数据集，在中国交通法规体系下明确对齐行车记录仪视频、文本描述、责任模式及相应法律条文；2. 引入两阶段框架：交通事故理解模块生成文本视频描述，法律多智能体框架输出责任模式、法律条文集和完整判决报告。

Result: 在C-TRAIL和MM-AU数据集上的实验结果表明，该方法优于通用和法律LLM以及现有基于智能体的方法，同时提供透明可解释的法律推理过程。

Conclusion: 该研究填补了视频证据与法律分析之间的空白，通过多模态数据集和两阶段框架实现了交通事故的自动化法律责任判定，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将透明度通道作为额外维度附加到RGB空间，需要重建底层VAE，但高质量透明字形数据稀缺，重新训练VAE计算成本高且可能破坏从海量RGB语料库学到的语义先验，导致潜在模式混合

Method: 提出TransText框架，基于Alpha-as-RGB范式，通过潜在空间拼接将alpha通道嵌入为RGB兼容的视觉信号，确保严格的跨模态一致性同时防止特征纠缠，无需修改预训练生成流形

Result: TransText显著优于基线方法，生成连贯、高保真的透明动画，具有多样化和精细的效果

Conclusion: TransText为动态视觉设计提供了首个层感知文本动画适配方法，通过创新的Alpha-as-RGB范式解决了现有方法的局限性
...
Method: 结合三个组件：1)基于LLM的提示扩展器，将简短用户意图转换为结构化逐层描述；2)具有4D RoPE位置编码的潜在扩散Transformer，联合生成完整媒体设计及其RGBA图层；3)支持完整alpha通道的RGBA VAE解码器。

Result: 在Crello测试集上，LaDe在文本到图层和图像到图层任务中优于Qwen-Image-Layered，通过两个VLM评估器(GPT-4o mini和Qwen3-VL)验证了更好的文本到图层对齐效果。

Conclusion: LaDe框架能够生成灵活数量的语义有意义图层，支持多种媒体设计任务，在图层生成质量和对齐方面优于现有方法。
...
TL;DR: AHOY方法从单目视频中重建完整、可动画的3D高斯化身，即使存在严重遮挡。现有方法需要无遮挡输入，而AHOY通过生成式监督处理遮挡区域，实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: 现有3D化身重建方法假设输入是无遮挡的，这排除了大多数真实世界视频（人物常被家具、物体或其他人遮挡）。从遮挡视频重建面临根本挑战：大范围身体区域可能从未被观察到，且每个姿态缺乏多视角监督。

Method: 提出四个关键技术：(i) 幻觉作为监督的流程，使用身份微调的扩散模型为未观察到的身体区域生成密集监督；(ii) 两阶段从规范到姿态依赖的架构，从稀疏观察引导到完整的姿态依赖高斯图；(iii) 地图姿态/LBS姿态解耦，吸收生成数据中的多视角不一致性；(iv) 头/身体分离监督策略，保持面部身份特征。

Result: 在YouTube视频和具有显著遮挡的多视角捕获数据上评估，展示了最先进的重建质量。生成的化身足够稳健，可以用新姿态进行动画，并合成到使用手机视频捕获的3DGS场景中。

Conclusion: AHOY方法成功解决了从遮挡单目视频重建完整3D高斯化身的挑战，通过生成式监督填补缺失信息，实现了高质量、可动画的化身重建，适用于真实世界场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在3D空间推理中通常依赖计算昂贵的3D表示（如点云或BEV地图），或缺乏物理基础来解决尺度和大小模糊性问题。本文旨在通过引入IMU采集的自我运动模态数据来增强MLLMs的3D场景理解能力。

Method: 提出Motion-MLLM框架，包含两个核心组件：1）级联运动-视觉关键帧过滤模块，利用IMU数据和视觉特征高效选择稀疏但具代表性的关键帧；2）非对称跨模态融合模块，运动令牌作为中介将自我运动线索和跨帧视觉上下文注入视觉表示。

Result: Motion-MLLM在多种3D场景理解和空间推理任务中取得显著改进。相比基于视频帧和显式3D数据的SOTA方法，Motion-MLLM在准确率相当甚至更高的情况下，计算开销显著降低（成本效益分别提高1.40倍和1.63倍）。

Conclusion: 通过将视觉内容锚定在物理自我运动轨迹上，Motion-MLLM能够推理绝对尺度和场景空间关系，为MLLMs的3D空间推理提供了一种高效且物理基础的方法。
...
TL;DR: DynaEdit是一种无需训练的零样本视频编辑方法，基于预训练的文本到视频流模型，能够实现复杂的动作修改、交互对象插入和全局效果编辑。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法存在局限性：训练模型需要大量相关数据且难以处理复杂编辑，而无训练方法只能进行结构和运动保持的编辑，无法修改运动或交互行为。需要一种能够处理复杂编辑（如动作修改、交互对象插入）的通用视频编辑方法。

Method: 基于无反转方法（inversion-free approach），不干预模型内部结构，因此是模型无关的。针对直接应用该方法导致的低频错位和高频抖动问题，提出了新的机制来解决这些现象。

Result: DynaEdit在复杂的基于文本的视频编辑任务上取得了最先进的结果，包括修改动作、插入与场景交互的对象、引入全局效果等复杂编辑能力。

Conclusion: DynaEdit通过创新的机制解决了现有方法的局限性，实现了无需训练的通用视频编辑，能够处理复杂的动作修改和交互编辑任务，为视频编辑提供了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D形状标记化主要依赖为渲染和压缩设计的几何层次细节（LoD）方法，这些空间层次结构标记效率低且缺乏语义连贯性，不适用于自回归建模。
...
Result: LoST在几何和语义重建指标上大幅超越先前基于LoD的3D形状标记器，实现最先进的重建效果；仅需先前自回归模型0.1%-10%的标记数量即可实现高效高质量的3D生成，并支持语义检索等下游任务。

Conclusion: LoST通过语义层次标记化解决了3D形状标记化的核心问题，为自回归3D生成提供了更高效、语义更连贯的解决方案，显著提升了重建质量和生成效率。
...
TL;DR: 提出无需训练的连续可控图像编辑框架，通过在文本嵌入空间进行简单导向，实现平滑编辑控制


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖额外训练或人工干预，需要更简单、轻量且无需训练的方法来实现连续可控的图像编辑

Method: 1) 使用大语言模型自动构建去偏对比提示对；2) 在生成器的文本编码器空间中计算导向向量；3) 提出弹性范围搜索程序确定有效导向幅度区间；4) 在区间内添加缩放版导向向量实现连续编辑

Result: 方法在连续编辑行为上与基于训练的方法相当，优于其他无需训练的方法，且能自然推广到图像和视频生成等多种文本条件模态

Conclusion: 简单的文本嵌入空间导向足以实现平滑的连续编辑控制，无需额外训练或人工干预，为文本条件生成模型提供了轻量级的编辑解决方案
...
TL;DR: EchoGen是一个统一的布局到图像生成和图像定位框架，通过渐进式训练策略解决联合训练中的优化挑战，在两个任务上都取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 图像定位任务具有强大的文本和布局理解能力，可以弥补布局到图像生成中的相应限制；同时，从布局生成的图像内容多样性高，可以增强图像定位的鲁棒性。联合训练两个任务可以相互促进性能提升，但面临优化挑战和性能受限的问题。

Method: 提出渐进式训练策略：1）并行多任务预训练（PMTP）阶段，利用共享token加速训练，使模型具备两个任务的基本能力；2）双重联合优化（DJO）阶段，利用任务对偶性顺序整合两个任务，实现统一优化；3）循环强化学习（Cycle RL）阶段，使用一致性约束作为奖励消除对视觉监督的依赖，通过GRPO策略显著增强模型的统一能力。

Result: 在布局到图像生成和图像定位基准测试中取得了最先进的结果，并显示出两个任务联合优化的明显协同增益。

Conclusion: EchoGen框架通过渐进式训练策略成功解决了布局到图像生成和图像定位联合训练中的优化挑战，实现了两个任务的相互促进和性能提升，证明了联合优化的有效性。
...
TL;DR: SkeletonLLM通过将任意骨架序列转换为MLLM原生视觉模态，实现通用骨架理解，避免现有方法的局限性


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉-语言推理方面表现出色，但局限于原生模态，无法直接处理结构化非视觉数据如人体骨架。现有方法要么将骨架动态压缩为有损特征向量，要么将运动量化为离散标记，在不同骨架格式间泛化能力差。

Method: 提出SkeletonLLM框架，核心是DrAction——一个可微分、格式无关的渲染器，将骨骼运动学转换为紧凑图像序列。采用协作训练策略：因果推理蒸馏从教师模型转移结构化逐步推理，判别性微调锐化混淆动作间的决策边界。

Result: SkeletonLLM在识别、描述、推理和跨格式迁移等多样化任务上展现出强大的泛化能力，为将MLLM应用于非原生模态提供了可行路径。

Conclusion: 通过将骨架序列转换为MLLM原生视觉模态的方法，实现了通用骨架理解，为多模态大语言模型处理非视觉结构化数据开辟了新途径。
...
TL;DR: 评估大型语言模型在印度农村地区提供可靠孕产健康信息的潜力，发现Perplexity AI在语义上与专家最接近，ChatGPT-4o在清晰度和医学术语使用上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 印度农村地区医疗资源和基础设施有限，获取可靠的孕产健康信息面临挑战。随着互联网普及率提高（8.3亿用户，近半数农村女性上网），数字工具为健康教育提供了新机会。研究旨在评估大型语言模型能否提供可靠且易于理解的孕产健康信息。

Method: 研究选取ChatGPT-4o、Perplexity AI和GeminiAI三种大型语言模型，向每个模型提出17个孕产健康相关问题。将模型回答与孕产健康专家的回答进行比较，使用语义相似度、名词重叠度和可读性指标来评估内容质量。

Result: Perplexity AI的回答在语义上与专家回答最接近，而ChatGPT-4o生成的文本更清晰易懂，医学术语使用更准确。这表明不同模型在准确性和可理解性方面各有优势。

Conclusion: 随着农村地区互联网接入增加，大型语言模型可作为孕产健康教育的可扩展辅助工具。研究强调需要开发在准确性和清晰度之间取得平衡的AI工具，以改善服务不足地区的医疗保健沟通。
...
TL;DR: 本文提出了一种基于检索增强生成（RAG）的系统，用于回答关于S&P 500财务报告的问题，并通过神经重排序显著提升了系统性能。


<details>
  <summary>Details</summary>
Motivation: 金融分析师面临从冗长的10-K报告（通常超过100页）中提取信息的重大挑战，需要更有效的自动化问答系统来辅助分析工作。

Method: 采用混合检索方法结合全文检索和语义检索，构建RAG系统，并引入可选的交叉编码器模型进行神经重排序，使用FinDER基准数据集进行系统评估。

Result: 重排序显著提升了答案质量：8分及以上正确率从33.5%提升至49.0%（提升15.5个百分点），完全错误答案率从35.3%降至22.5%。

Conclusion: 神经重排序在金融RAG系统中具有关键作用，通过现代语言模型和优化的检索策略能够显著超越基线方法的性能。
...
TL;DR: 本文提出了一种基于评分标准引导的推理框架，用于第二语言语音评估，通过编码多维度评分标准（准确性、流利度、韵律）并校准模型不确定性，以更好地对齐人类评分者的变异性。
...
Method: 采用评分标准引导的推理框架，微调Qwen2-Audio-7B-Instruct模型，使用多评分者的人类判断数据，开发基于保形校准的不确定性校准回归方法，提供可解释的置信区间。
...
TL;DR: 本文提出了一个针对自然语言转SQL系统的鲁棒性评估基准，包含约10种扰动类型，在传统和智能体两种设置下评估了多个先进大语言模型，发现模型在字符级噪声和语义保留的语言变体上表现下降明显。


<details>
  <summary>Details</summary>
Motivation: 现实世界的数据库环境是动态、嘈杂且不断演变的，而传统的基准评估通常假设静态模式和格式良好的用户输入，因此需要对NL2SQL系统进行鲁棒性评估。

Method: 引入一个包含约10种扰动类型的鲁棒性评估基准，在传统和智能体两种设置下评估了Grok-4.1、Gemini-3-Pro、Claude-Opus-4.6和GPT-5.2等多个先进大语言模型。

Result: 模型在多种扰动下保持较强性能，但在字符级噪声和语义保留的语言变体上表现显著下降；表面级噪声在传统流程中造成更大性能下降，而语言变体在智能体设置中带来更大挑战。
...
TL;DR: 该论文分析了大语言模型评估在模糊任务中的问题，通过两个案例研究揭示了现有评估方法的局限性，并提出了更稳健、可解释的评估设计需求。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的评估基准和指标在模糊任务（输入输出空间不明确、成功标准模糊）上无法提供可靠或诊断性的能力信号，需要分析现有评估方法的根本局限性。

Method: 采用两个案例研究：1) 复杂指令跟随(CIF)，识别了真实世界指令复杂性覆盖不足、对指令措辞敏感、指标不一致且不可比较、LLM评估者引入的不稳定性等问题；2) 自然语言到Mermaid序列图(NL2Mermaid)，展示了多维度评估标准如何提供超越聚合分数的可操作见解。

Result: 研究发现当前评估经常混淆不同的失败模式，导致分数不稳定、无法诊断且难以采取行动。现有评估实践在模糊任务上存在根本局限性，包括覆盖不足、指标不一致、评估不稳定等问题。

Conclusion: 当前大语言模型在模糊任务上的评估方法存在严重缺陷，需要设计更稳健、可解释的评估方案，以提供更可靠、诊断性的能力信号，并支持模型改进。
...
<details>
  <summary>Details</summary>
Motivation: 分析现代大型推理LLM在跨语言知识迁移中的不足，特别是识别影响知识迁移的主要障碍因素。

Method: 1. 对ECLeKTic和MultiLoKo数据集进行观察性数据分析；2. 回归分析识别主要预测因素；3. 提供源语言关键实体测试；4. 开发合成生成管道创建SFT样本，改进推理机制。

Result: 1. 文字脚本匹配是知识迁移失败的主要预测因素；2. 提供源语言关键实体能显著改善跨脚本问题；3. 通过改进推理机制训练，两个模型的跨脚本迁移差距都得到减少。

Conclusion: 跨语言参数知识迁移的主要障碍是文字脚本差异，通过在后期训练中改进推理机制，可以有效提升跨语言知识迁移效果。
...
TL;DR: 提出基于集成学习的自训练框架用于无监督神经机器翻译，通过多模型共享任务但使用不同辅助语言来创造结构化多样性，利用集成解码生成伪翻译数据进一步训练模型，最终选择单个模型部署以保持推理成本


<details>
  <summary>Details</summary>
Motivation: 传统无监督神经机器翻译（UNMT）方法通常训练单个模型，性能有限。本文旨在通过集成学习框架提升UNMT性能，同时保持部署时的单模型推理成本

Method: 1. 针对主要语言对训练多个UNMT模型，这些模型共享相同的翻译任务但使用不同的辅助语言，创造结构化多样性；2. 使用token级集成解码为原始语言对生成伪翻译数据；3. 利用集成输出作为合成平行数据进一步训练每个模型；4. 部署时根据验证性能选择单个模型

Result: 实验显示相比单模型UNMT基线有统计显著改进：从英语翻译时平均提升1.7 chrF，翻译成英语时平均提升0.67 chrF

Conclusion: 提出的集成驱动自训练框架有效提升了无监督神经机器翻译的性能，同时保持了部署时的单模型推理效率，为UNMT提供了一种有效的性能提升方法
...
TL;DR: 利用多语言不一致性作为MoE LLMs可解释性工具，通过对比成功和失败语言的路由模式，定位知识相关专家模块


<details>
  <summary>Details</summary>
Motivation: 现代LLMs在不同语言间存在显著行为差异，通常被视为需要缓解的问题。本文提出利用这种跨语言不一致性作为MoE LLMs的可解释性工具，通过对比模型在不同语言中成功和失败回忆信息时的路由模式，定位负责特定知识的功能组件。

Method: 提出知识定位框架，包含两个阶段：(1) 用困难事实问题查询模型，生成"成功"和"失败"激活桶；(2) 对MoE路由器logits应用统计对比分析，识别对知识重要的专家模块。通过停用这些专家验证其必要性。

Result: 仅停用约20个专家（共6000个），模型在超过40%的情况下不再能正确回答问题。这表明该方法能有效定位知识相关的关键专家模块。

Conclusion: 该方法提供了一种现实且可扩展的知识定位方法，能够应对日益复杂的LLMs，利用跨语言不一致性作为可解释性工具，有效识别MoE模型中负责特定知识的功能组件。
...
TL;DR: 提出基于英语语法档案(EGP)的新框架，用于检测二语学习者语法结构尝试并分类成功/失败，支持精细反馈和CEFR水平评估，比较基于规则和LLM的方法性能。


<details>
  <summary>Details</summary>
Motivation: 评估二语学习者的语法能力对于提供针对性反馈和评估语言水平至关重要，需要能够检测学习者语法结构尝试并分类成功与否的系统。

Method: 提出基于英语语法档案(EGP)的新框架，将语法结构映射到CEFR水平，使用基于规则和LLM的分类器检测语法结构尝试并分类成功/失败，同时用于CEFR水平预测。评估基于规则、LLM和混合方法，以及全自动与半自动流程。

Result: LLM在语义和语用细微差别的语法结构上优于基于规则的方法，而基于规则的方法在纯形态或句法特征上仍有竞争力。混合方法(规则预过滤+LLM)在水平评估中表现最佳。全自动流程接近半自动系统性能，特别是在检测成功语法尝试方面。

Conclusion: 该框架强调学习者的成功尝试和失败尝试，能够提供积极的形成性反馈，并为语法发展提供可操作的见解。混合方法结合了规则和LLM的优势，全自动流程具有实用价值。
...
TL;DR: TAP-GPT是一个基于TableGPT2的表格专用大语言模型框架，用于阿尔茨海默病的少样本分类，在多种生物标志物数据集上优于传统机器学习方法，并能处理缺失数据而不需要插补。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病诊断需要处理表格生物标志物数据，但这些数据通常样本量小且不完整，深度学习模型难以超越传统方法。预训练大语言模型具有少样本泛化、结构化推理和可解释输出的优势，为临床预测提供了新的范式。

Method: 提出TAP-GPT框架，基于TableGPT2进行领域适应，使用表格提示而非纯文本进行微调，用于阿尔茨海默病的少样本分类。在四个ADNI数据集上评估，包括QT-PAD生物标志物和区域级结构MRI、淀粉样蛋白PET、tau PET数据。

Result: 在多模态和单模态设置下，TAP-GPT优于其骨干模型和传统机器学习基线，在少样本设置中表现优异，与最先进的通用大语言模型竞争力相当。特征选择缓解了高维输入的退化问题，TAP-GPT在模拟和真实缺失数据下保持稳定性能而不需要插补。

Conclusion: 这是首次系统地将表格专用大语言模型应用于多模态生物标志物阿尔茨海默病预测，证明此类预训练模型能有效处理结构化临床预测任务，为表格大语言模型驱动的多智能体临床决策支持系统奠定了基础。
...
TL;DR: SYMDIREC是一个神经符号框架，通过符号规划分解RTL任务，使用微调检索器获取相关代码，并通过LLM推理组装验证输出，在RTL综合和总结任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: RTL综合和总结是硬件设计自动化的核心，但对大型语言模型具有挑战性，因为HDL语法严格、监督有限且与自然语言对齐弱。现有提示和检索增强生成方法缺乏符号规划，限制了结构精度。

Method: SYMDIREC将RTL任务分解为符号子目标，通过微调的检索器获取相关代码，然后通过LLM推理组装验证输出。该框架支持Verilog和VHDL，无需LLM微调。

Result: SYMDIREC在综合任务上达到约20%更高的Pass@1率，在总结任务上获得15-20%的ROUGE-L改进，显著优于提示和RAG基线方法。

Conclusion: SYMDIREC展示了符号指导在RTL任务中的优势，通过神经符号方法有效解决了HDL语法严格、监督有限和对齐弱的问题。
...
TL;DR: 本文提出了一种基于本地大语言模型的文本匿名化方法，通过用类型一致的替代品替换个人身份信息，在保护隐私的同时保持文本的流畅性和语义价值。


<details>
  <summary>Details</summary>
Motivation: 在大型语言模型时代，如何在保护敏感信息的同时不损害数据的实用性成为一个紧迫问题。需要一种既能防止数据泄露，又能保持文本流畅性和任务相关语义的解决方案。

Method: 开发了一个本地LLM驱动的替代管道，在组织边界内执行文本匿名化，将个人身份信息替换为类型一致的现实替代品。采用多指标、跨技术的系统评估方法，在Action-Based Conversation Dataset上进行测试，并与行业标准（Microsoft Presidio和Google DLP）以及最先进方法（ZSTS）进行比较。

Result: 该方法在隐私保护、语义效用和可训练性方面达到最先进水平，优于基于规则的方法、命名实体识别基准和ZSTS变体。在隐私-效用-可训练性综合边界上表现最佳，同时保持低主题漂移和强事实效用。

Conclusion: 本地LLM替代方法能够生成既负责任又有操作价值的匿名化语料库，既适用于智能问答管道，也适合下游微调任务，在保护隐私的同时保持了数据的实用性。
...
TL;DR: 本文针对全球南方土著语言在AI革命中被边缘化的问题，以尼泊尔和印度特莱地区的Tharu语为例，开发了Tharu-LLaMA（3B）专用指令跟随模型，通过LLM-to-Human引导管道构建TharuChat数据集，证明小规模合成数据能有效提升低资源语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展造成了深刻的数字鸿沟，使全球南方的土著语言被排除在AI革命之外。Tharu语作为尼泊尔和印度特莱地区约170万人使用的印欧语系语言，虽然有丰富的口述传统，但面临严重的数据稀缺和语言碎片化问题，导致最先进的多语言模型经常产生"幻觉"或默认使用印地语和尼泊尔语等高资源语言。

Method: 本文提出Tharu-LLaMA（3B）专用指令跟随模型。通过LLM-to-Human引导管道构建TharuChat数据集：使用经过提示工程优化的Gemini模型，输入Rana Tharu语法和民间传说来合成训练数据。数据集主要基于Rana Tharu方言（约70%），同时整合Dangaura和Kochila方言元素，反映了该地区嘈杂、异质的语言现实。

Result: 通过严格的实证消融研究证明，尽管数据集存在方言混合和残留的Awadhi/印地语影响等缺陷，小规模合成数据仍然非常有效。数据集规模从25%增加到100%时，困惑度从6.42线性降低到2.88。该模型可作为通过生成式AI保护资源匮乏的喜马拉雅语言的可行性证明，且可在消费级硬件上实现。

Conclusion: Tharu-LLaMA模型为通过生成式AI保护低资源语言提供了概念验证，展示了即使在数据稀缺和语言碎片化的情况下，通过创新的数据合成方法也能有效开发专用语言模型，为全球南方土著语言的AI包容性提供了可行路径。
...
TL;DR: 该研究首次在语音生成大模型中进行了神经元层面的情感控制研究，发现了情感敏感神经元，可在推理时无需训练地实现情感控制。


<details>
  <summary>Details</summary>
Motivation: 现有大型音频语言模型在情感控制方面存在不足：情感转换常偏离目标情感，且可能因拒绝、幻觉或改写而损害语言保真度。

Method: 通过成功过滤激活聚合识别情感敏感神经元，这些神经元具有因果可操作性，可在推理时进行无需训练的情感引导。

Result: 在三个LALM模型上验证，情感敏感神经元的干预能产生情感特异性增益，可泛化到未见过的说话者，自动和人工评估均支持该效果。

Conclusion: 研究建立了一个无需训练的情感控制机制框架，为语音生成中的情感控制提供了新的方法。
...
TL;DR: CulT-Eval是一个包含7,959个文化表达实例的基准测试，用于评估机器翻译系统处理文化负载表达的能力，揭示了当前模型在保留文化意义方面的系统性问题。


<details>
  <summary>Details</summary>
Motivation: 文化表达（如习语、俚语、文化特定项目）在自然语言中普遍存在，但机器翻译系统准确翻译这些表达仍然具有挑战性。现有基准测试分散且缺乏系统框架来评估文化负载表达的翻译性能。

Method: 引入CulT-Eval基准，包含7,959个精心策划的文化表达实例，涵盖多种文化表达类型，并建立了全面的错误分类法。通过大规模语言模型评估和详细分析，识别现有自动指标未能捕捉的系统性失败模式。

Result: 当前模型在保留文化基础意义和捕捉文化及上下文细微差别方面存在困难，无法实现准确翻译。研究提出了针对标准机器翻译指标忽略的文化诱导意义偏差的补充评估指标。

Conclusion: CulT-Eval基准填补了文化表达翻译评估的系统性框架空白，揭示了当前模型的局限性，并提供了针对文化意义偏差的评估指标，有助于改进机器翻译系统处理文化负载表达的能力。
...
Method: 使用涵盖220对字母对比的最小对范式，通过三个大型语言模型独立分析九个感知维度上的音素-意义关联，并用英语母语者的行为数据验证

Result: 发现音素与意义之间存在一致关联，这些关联可由发音-语音特征系统预测，英语母语者识别准确率达80.8%，初步跨语言证据表明核心映射在五种类型不同语言中具有普适性

Conclusion: 音义象似性不是偶然现象，而是语音信号的普遍结构化特性，其系统性之强以至于大型语言模型仅凭文本输入就能恢复这种关联
...
Method: 创建GSU纯文本网格数据集，包含导航、物体定位和结构组合三个任务，测试不同模型（包括纯语言模型、视觉语言模型和前沿模型）的性能，并尝试通过微调小型模型来匹配前沿模型表现。

Result: 大多数模型能理解基本网格概念，但在相对参考系（相对于具身智能体）和从坐标列表识别3D形状方面表现不佳；视觉模态的暴露并未为视觉语言模型提供可泛化的3D空间理解；最新前沿模型能解决这些任务，但更难的变体仍可能难倒它们；微调小型模型有潜力匹配前沿模型性能。

Conclusion: 纯语言模型在空间推理方面存在特定局限性，特别是相对参考系和3D形状识别；视觉模态不能直接转化为3D空间理解能力；通过专门微调小型模型可以开发出具有竞争力的专用具身智能体。
...
TL;DR: PACE-RAG框架通过结合患者个体临床特征与相似病例的处方模式，实现个性化药物推荐，在帕金森病和MIMIC-IV基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs和RAG方法在药物推荐中存在局限：LLMs无法捕捉实际处方模式的细微差别，基于指南的检索过于通用，基于相似患者的检索则容易复制多数模式而忽略个体临床差异。需要一种能综合个体患者上下文与相似病例处方倾向的方法。

Method: 提出PACE-RAG（患者感知的上下文和基于证据的策略RAG）框架，通过分析针对特定临床信号的治疗模式，识别最优处方并生成可解释的临床总结。框架在帕金森病队列和MIMIC-IV基准上使用Llama-3.1-8B和Qwen3-8B进行评估。

Result: PACE-RAG在帕金森病队列上达到80.84%的F1分数，在MIMIC-IV基准上达到47.22%的F1分数，实现了最先进的性能表现。

Conclusion: PACE-RAG是一个稳健、临床基础扎实的个性化决策支持解决方案，能够有效综合个体患者上下文与相似病例的处方模式，为复杂疾病如帕金森病提供精准的药物推荐。
...
TL;DR: SafeTutors基准评估发现，当前LLM作为AI导师时普遍存在教学安全风险，包括过度披露答案、强化错误概念等，且多轮对话会显著恶化教学行为。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为AI导师的评估范式孤立地评估问题解决准确性和通用安全性，未能捕捉模型是否同时在学生-导师互动中既教学有效又安全。作者认为辅导安全与常规LLM安全有根本区别，主要风险不是有毒内容，而是通过过度披露答案、强化错误概念和放弃支架支持等方式悄然侵蚀学习效果。

Method: 引入SafeTutors基准，联合评估数学、物理和化学领域的安全性和教学法。该基准基于从学习科学文献中提取的理论基础风险分类法，包含11个危害维度和48个子风险。

Result: 所有模型都显示出广泛的危害；模型规模并不能可靠地改善情况；多轮对话会恶化行为，教学失败率从17.7%上升到77.8%。危害因学科而异，因此缓解措施必须具有学科意识，单轮"安全/有帮助"的结果可能掩盖了在扩展互动中系统性的导师失败。

Conclusion: AI导师的安全评估需要专门的方法，不能依赖常规LLM安全评估。多轮互动会显著放大教学风险，学科特定的缓解策略至关重要，单轮评估可能严重低估长期互动中的系统性失败。
...
TL;DR: 本文提出一个框架，通过让LLMs学习论证重构来提升批判性思维能力，包括开发自动论证重构引擎、创建高质量数据集，并验证该方法在多个批判性思维任务上的有效性。


<details>
  <summary>Details</summary>
Motivation: 论证重构对于批判性思维至关重要，因为它使论证的潜在推理过程变得明确。然而，目前尚不清楚LLMs是否能够通过学习论证重构来增强其批判性思维能力。

Method: 提出了一个包含三个贡献的整体框架：(1) 开发了能够自动重构任意论证的GAAR引擎；(2) 使用GAAR引擎合成了新的高质量论证重构数据集Arguinas；(3) 研究了学习论证重构是否有利于下游的批判性思维任务。

Result: 实验结果表明，在七个批判性思维任务中，经过论证重构训练的模型表现优于未经训练的模型，其中在提出的Arguinas数据集上训练时观察到了最大的性能提升。

Conclusion: 通过学习论证重构，LLMs能够显著提升其在批判性思维任务上的表现，这为增强AI系统的批判性思维能力提供了一种有效方法。源代码和数据集将公开提供。
...
TL;DR: 论文提出MSL（最小充分长度）理论度量，用于量化保持答案正确性的最短推理长度，并基于此开发TRiMS方法，在保持准确性的同时大幅减少推理链长度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通过长思维链实现复杂推理，但这导致严重的推理膨胀和计算冗余。为了最大化"每个token的智能"，需要找到保持答案正确性的最短推理长度。

Method: 1. 提出MSL理论度量，基于独立采样序列的递归定义，证明其极限存在；2. 分析主流CoT压缩策略，识别接近MSL的关键结构因素；3. 提出TRiMS方法，结合GRPO算法和MSL估计进行训练，通过动态批聚合和基于批级标准差计算优势来缓解训练不稳定性。

Result: TRiMS在所有基准测试中实现了超过80%的CoT token减少，同时准确率略有提升。

Conclusion: MSL为推理链压缩提供了首个可测量的下界，TRiMS方法有效实现了推理效率的大幅提升，在保持准确性的同时显著减少计算冗余。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探索基于Transformer的语言模型如何学习语言类别，通过比较其训练过程中的行为与人类语言习得的抽象特征理论和具体样例理论，理解语言模型学习机制及其对人类语言习得模型的启示。

Method: 使用GPT-2小模型，通过新颖的基于分歧度的度量方法来追踪学习轨迹，分析词汇语义和句法类别的形成过程，比较模型在训练不同阶段的行为特征。

Result: 研究发现：(1)当语言结构被学习时，抽象类别层面的行为比具体词汇层面的行为出现得更早；(2)不同的语言行为在训练过程中依次突然出现，表明抽象化在语言模型学习中扮演关键角色。

Conclusion: 语言模型的学习过程显示抽象化是其核心机制，这一发现为语言模型作为人类语言习得模型的存在证明提供了重要依据，表明抽象类别学习在语言习得中具有基础性作用。
...
TL;DR: L2A（学习注意力）是一种新的注意力机制层，通过条件性地决定何时调用全局注意力，使语言模型能够高效处理长上下文，减少80%的全局注意力计算，将上下文长度从32K扩展到128K，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 语言模型难以泛化到预训练上下文长度之外，限制了长程推理和检索能力。传统的长上下文训练方法由于注意力机制的二次方复杂度而非常昂贵。作者观察到大多数token实际上不需要全局注意力，可以依赖局部上下文。

Method: 提出L2A（学习注意力）层，这是一种条件性的token级长程记忆访问机制。L2A为每个token决定何时需要调用全局注意力，而不是对所有token都应用全局注意力。同时设计了定制的Triton内核来高效实现这种条件性注意力机制。

Result: 在Qwen 2.5和Qwen 3模型上评估，将有效上下文长度从32K扩展到128K。L2A性能与标准长上下文训练相当（差距在3%以内），同时跳过了约80%token的全局注意力计算。训练吞吐量比FlashAttention提高约2倍，KV缓存内存减少高达50%且性能损失可忽略。

Conclusion: L2A提供了一种高效的长上下文处理方法，通过条件性全局注意力调用显著降低了计算成本，同时保持了模型性能。该方法不仅提高了训练效率，还支持后训练剪枝，为长上下文语言模型的实际部署提供了实用解决方案。
...
TL;DR: 本文提出HypoTermInstruct数据集，通过教授模型认识自身知识局限性来减少大语言模型幻觉，实验证明该方法能显著提升幻觉检测分数而保持基础能力稳定。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常产生幻觉（输出流畅但虚假的信息），部分原因是监督微调（SFT）隐含地奖励模型总是给出回答。为了解决这个问题，需要教导模型认识自身知识的局限性并承认不确定性。

Method: 1. 创建HypoTermInstruct数据集（31,487个回答对应11,151个问题），通过关于不存在的"假设性"术语的问题来教导模型认识知识局限性；2. 发布HypoTermQA-Enhanced基准测试，通过多重验证加强幻觉倾向评估；3. 在Llama3.1-8B和Gemma3-4B模型上进行800次受控LoRA SFT实验，测试100种微调配置。

Result: 用HypoTermInstruct替换通用指令数据后：1. HypoTerm分数显著提升（中位数增加0.19%到25.91%）；2. FactScore提升（+0.39%到+0.86%）；3. MMLU性能保持稳定（仅下降0.26%到0.35%）。

Conclusion: 针对性的高质量SFT数据可以有效地教导元认知技能，无需偏好学习/强化学习流程就能减少幻觉，为构建更可靠的AI系统提供了机制性见解和实用路径。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具备强大的通用智能，但其多语言性能严重不平衡。尽管LLM在统一语义空间中编码了大量跨语言知识，但在低资源或未见语言上难以可靠地利用这些知识。而预训练的编码器-解码器翻译模型已具备平衡的多语言能力，这为补充LLM提供了自然方案。

Method: 提出XBridge架构，将多语言理解和生成任务卸载到外部预训练翻译模型，同时保持LLM作为英语核心处理通用知识。为解决模型间表示不对齐问题，引入轻量级跨模型映射层和基于最优传输的对齐目标，实现细粒度语义一致性。

Result: 在四个LLM上的多语言理解、推理、摘要和生成任务实验表明，XBridge优于强基线方法，特别是在低资源和未见语言上表现突出，且无需重新训练LLM。

Conclusion: XBridge通过组合预训练翻译模型和LLM，有效解决了LLM多语言性能不平衡问题，为低资源语言提供了有效的解决方案，同时保持了LLM的核心知识处理能力。
...
TL;DR: 该研究提出了一个全面的机器生成文本检测基准，评估了多种检测方法在跨领域、跨LLM和对抗鲁棒性方面的表现，发现现有方法在理想条件下表现良好但缺乏跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速普及，迫切需要鲁棒且可泛化的机器生成文本检测器。现有基准通常只在理想条件下评估单个检测器在单个数据集上的表现，未能解决跨领域迁移、跨LLM泛化和对抗鲁棒性等关键问题。

Method: 研究提出了一个综合基准，在两个语料库（HC3包含23,363个人类-ChatGPT对，ELI5包含15,000个人类-Mistral-7B对）上评估了多种检测方法，包括：经典分类器、微调的Transformer编码器（BERT、RoBERTa、ELECTRA、DistilBERT、DeBERTa-v3）、CNN、XGBoost风格计量模型、基于困惑度的检测器以及LLM作为检测器的提示方法。

Result: Transformer模型在分布内测试中达到接近完美的性能，但在领域转移时性能下降。XGBoost风格计量模型性能相当且保持可解释性。LLM基检测器表现不佳且受生成器-检测器身份偏差影响。基于困惑度的方法出现极性反转（现代LLM输出比人类文本困惑度更低），但修正后仍有效。没有任何方法能在跨领域和跨LLM源方面实现鲁棒泛化。

Conclusion: 机器生成文本检测面临跨领域泛化和跨LLM鲁棒性的重大挑战，需要开发更强大的检测方法来解决这些限制，特别是在实际应用中需要考虑领域转移和不同LLM源的影响。
...
TL;DR: AURORA模型基于前两个共振峰预测元音发音时的舌位和舌形，旨在作为教学工具和生物反馈应用基础


<details>
  <summary>Details</summary>
Motivation: 开发一个能够解释共振峰与发音之间关系的教学工具，并为语音治疗提供生物反馈应用基础

Method: 基于40名英语母语者的超声舌成像和声学数据构建模型，采用概念和计算基础相结合的方法

Result: 开发了AURORA模型，并创建了Shiny应用和实时舌位生物反馈原型软件，使模型更易于使用

Conclusion: AURORA模型可作为有效的语音学教学工具和语音治疗应用基础，具有广泛的应用前景
...
<details>
  <summary>Details</summary>
Motivation: 多语言语音大语言模型在数据分布不平衡时面临稳定性-可塑性困境：完全共享参数会导致低资源语言负迁移，完全语言特定调优又限制了跨语言知识转移。

Method: 提出Zipper-LoRA框架，包含三种变体（静态、硬、软），通过轻量级语言条件路由器在LoRA秩级别动态控制共享和语言特定子空间的贡献，并采用两阶段训练策略加速收敛。

Result: 在12种语言的混合资源设置中，Zipper-LoRA始终优于完全共享和独立基线，尤其在极低资源场景表现突出，且在分块和非分块编码器配置中均保持稳健。

Conclusion: Zipper-LoRA有效解决了多语言语音大语言模型中的稳定性-可塑性困境，为大规模多语言ASR提供了可靠框架，实现了细粒度共享与解耦的平衡。
...
TL;DR: KA2L框架通过分析LLM的潜在空间来评估其对特定知识点的掌握程度，构建模型尚未掌握的知识问题，从而减少50%的标注和计算成本并提升性能。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏对LLM在领域特定知识理解深度的研究，以及如何应用针对性主动学习来提升其专业能力。现有研究主要关注使用高质量知识微调LLM，但对模型知识掌握程度的评估和针对性改进不足。

Method: 提出知识感知主动学习（KA2L）框架：1）使用知识分布探测技术分析Transformer特定层的隐藏状态，识别LLM中已知和未知知识的分布；2）提出隐藏状态解码方法，从潜在知识空间生成大量未知问题的自然语言表述；3）通过潜在空间分析构建模型无法回答的问题，专注于模型尚未掌握的知识进行训练。

Result: 在9个开源LLM上的实验表明：1）在两个开放领域和一个垂直领域数据集上，KA2L显著减少了50%的标注和计算成本；2）同时实现了更好的性能表现；3）为LLM的主动学习策略提供了有价值的见解。

Conclusion: KA2L框架通过分析LLM的潜在知识空间，能够有效评估模型的知识掌握程度，并针对性地构建训练数据，显著提升训练效率和模型性能，为LLM的主动学习提供了创新解决方案。
...
TL;DR: 该研究通过线性探测和机制可解释性技术，分析了三个不同规模的语言模型如何编码四种语义关系（同义、反义、上义、下义），发现层级关系存在方向不对称性，关系信号在中间层最强，且模型规模影响因果干预效果。


<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型是否捕捉结构化意义，需要研究它们如何表示概念关系。本研究旨在探索不同规模模型如何编码四种基本语义关系，以及这些关系在模型内部的具体表示机制。

Method: 结合线性探测和机制可解释性技术，包括稀疏自编码器和激活修补，分析Pythia-70M、GPT-2和Llama 3.1 8B三个模型。重点关注四种语义关系：同义、反义、上义和下义，通过特征分析和因果干预来理解关系编码机制。

Result: 发现层级关系存在方向不对称性：上义关系编码冗余且难以抑制，下义关系依赖紧凑特征更易被破坏。关系信号在中间层最强，在残差后/MLP路径中比注意力中更强。难度在不同模型中一致（反义最简单，同义最难）。因果干预效果与模型容量相关：在Llama 3.1上，SAE引导的修补能可靠地改变关系信号，而在较小模型中效果弱或不稳定。

Conclusion: 研究阐明了语义关系在语言模型内部的位置和可靠性表示方式，提供了一个可复现的框架，将稀疏特征与探测级因果证据联系起来，为理解模型如何编码结构化语义知识提供了新见解。
...
TL;DR: ARAM是一个无需训练的自适应引导框架，用于解决检索增强生成中检索上下文噪声问题，通过动态调整引导强度来提升扩散语言模型的生成质量。


<details>
  <summary>Details</summary>
Motivation: 在检索增强生成中，当检索到的上下文存在噪声、不可靠或与模型参数知识不一致时，会产生检索先验冲突，降低生成质量。这个问题在自回归语言模型中已有研究，但在基于扩散的语言模型中仍未被充分探索，因为其迭代去噪过程给检索上下文整合带来了独特挑战。

Method: 提出了自适应检索增强掩码扩散（ARAM）框架，这是一个无需训练的自适应引导方法。ARAM根据检索上下文引起的分布偏移的信噪比，在去噪过程中动态校准引导尺度。当检索上下文提供可靠的纠正证据时增强引导，当上下文信号噪声大或非支持性时抑制引导。

Result: 在多个知识密集型问答基准测试上的广泛实验表明，ARAM在整体问答性能上优于竞争性的检索增强生成基线方法。

Conclusion: ARAM通过自适应引导机制有效解决了扩散语言模型中检索上下文噪声问题，提升了检索增强生成的性能，特别是在处理不可靠或冲突的检索信息时表现出色。
...
TL;DR: 本文提出了一个多模态、多语言的幽默检测基准，包含3000个文本、6000张图片和1200个视频，用于区分安全笑话与有害笑话，后者进一步分为显式和隐式类别。


<details>
  <summary>Details</summary>
Motivation: 当前静态基准无法捕捉依赖文化细微差别和隐含线索的黑色幽默，这带来了安全挑战。需要能够评估模型在理解有害和冒犯性幽默方面的能力，特别是需要上下文推理的隐式内容。

Method: 创建了一个手动策划的多模态多语言数据集（英语和阿拉伯语的文本和图像，以及英语、阿拉伯语和语言无关的视频）。采用严格的标注指南：区分安全笑话与有害笑话，有害笑话进一步分为显式（明显）和隐式（隐蔽）类别。系统评估了最先进的开放和闭源模型在所有模态上的表现。

Result: 闭源模型显著优于开源模型。英语和阿拉伯语之间存在显著的性能差异，突显了基于文化的推理感知安全对齐的迫切需求。

Conclusion: 需要文化基础、推理感知的安全对齐来有效检测和理解有害幽默，特别是涉及隐式文化细微差别的内容。当前模型在处理多语言和多模态幽默检测方面仍有不足。
...
TL;DR: CoVerRL框架通过让单一模型在生成器和验证器角色间交替，解决无标签强化学习中"共识陷阱"问题，防止模型因过度追求一致性而强化系统性错误。


<details>
  <summary>Details</summary>
Motivation: 当前无标签强化学习方法将多数投票答案作为伪标签，但存在"共识陷阱"问题：随着训练最大化自一致性，输出多样性崩溃，模型会自信地强化难以检测的系统性错误。

Method: 提出CoVerRL框架，让单一模型在生成器和验证器角色间交替。多数投票为验证器训练提供有噪声但有信息的监督，而改进的验证器逐步从伪标签中过滤自一致错误，形成良性循环。

Result: 在Qwen和Llama模型系列上的实验显示，CoVerRL在数学推理基准上比无标签基线提升4.7-5.9%。自验证准确率从约55%提升至85%以上，证实两种能力真正共同进化。

Conclusion: CoVerRL通过生成器和验证器的协同进化，有效解决了无标签强化学习中的共识陷阱问题，在保持高奖励准确性的同时显著提升了推理能力。
...
TL;DR: 提出基于信息论自动生成步骤级标签的新方法，用于训练过程奖励模型，降低计算复杂度至O(N)，在数学、编程等多领域推理任务中实现有效的思维链选择。


<details>
  <summary>Details</summary>
Motivation: 多步推理提升大语言模型能力但增加错误传播风险，过程奖励模型通过评分单个步骤提供细粒度监督。现有训练方法依赖昂贵人工标注或计算密集的自动标注，需要更高效、可扩展的解决方案。

Method: 基于信息论自动生成步骤级标签，估计每个推理步骤如何影响正确答案的可能性，提供步骤质量信号。该方法将计算复杂度从O(N log N)降低到O(N)。

Result: 在数学、Python编程、SQL和科学问答等多种推理基准测试中，生成的标签能够在最佳K选择评估设置中实现有效的思维链选择。

Conclusion: 该方法为LLM推理提供了可扩展且高效的监督机制，特别适用于错误传播关键的任务，解决了现有方法在标注成本和计算复杂度方面的限制。
...
TL;DR: 该研究探索语言模型从非结构化文本中进行空间推理的能力，通过叙事到剧本布局的转换任务，结合拒绝SFT和GRPO强化学习，在古典英语文学语料上取得了优于基础模型的性能。
...
Method: 提出叙事到剧本布局任务，引入戏剧学启发的确定性评估套件，采用结合拒绝SFT（使用Best-of-N采样）和基于可验证奖励的GRPO强化学习的训练推理方案。

Result: 在古典英语文学文本语料上的实验显示，该方法在角色归属、空间合理性和移动经济性等多个指标上优于基础模型，同时与LLM作为评判者和人类主观偏好保持一致。

Conclusion: 该研究证明了语言模型能够从非结构化文本中进行有效的空间推理，提出的训练方法显著提升了模型在叙事到剧本布局任务上的性能，为自动化媒体应用提供了可行方案。
...
TL;DR: NEVU是一个新闻事件中心的人类价值理解基准，专注于事实新闻中的演员条件、事件中心和方向感知的价值识别，包含多层次语义单元标注和分层价值空间。
...
Method: 构建NEVU基准，包含2,865篇英文新闻文章，采用四层语义单元标注（子事件、基于行为的复合事件、基于故事的复合事件、文章），通过LLM辅助流水线进行标注，包含阶段性验证和针对性人工审核。使用分层价值空间，包含54个细粒度价值和20个粗粒度类别。

Result: NEVU包含45,793个单元-演员对和168,061个有向价值实例。实验显示轻量级适应（LoRA）能持续改进开源模型，表明NEVU不仅可作为基准，还支持超越提示评估的监督适应。

Conclusion: NEVU填补了事实新闻中人类价值理解的空白，提供了演员条件、事件中心和方向感知的评估框架，支持多层次语义分析和模型适应性评估。
...
TL;DR: 研究发现LLM的"口头置信度"并非即时计算，而是答案生成时自动计算并缓存，随后在需要时检索输出，代表比词元对数概率更丰富的答案质量评估


<details>
  <summary>Details</summary>
Motivation: 口头置信度被广泛用于从黑盒LLM中提取不确定性估计，但LLM内部如何生成这些分数仍然未知。研究旨在探究两个核心问题：置信度何时计算（即时请求时还是答案生成时自动计算并缓存），以及口头置信度代表什么（词元对数概率还是更丰富的答案质量评估）

Method: 聚焦Gemma 3 27B和Qwen 2.5 7B模型，采用多种实验方法：激活引导、补丁、噪声和交换实验揭示置信度表示在答案相邻位置出现；注意力阻断定位信息流；线性探测和方差分解分析缓存表示对口头置信度的解释力

Result: 实验证据支持缓存检索机制：置信度表示在答案相邻位置先于口头化位置出现；置信度从答案词元收集，缓存在第一个答案后位置，然后检索输出；缓存表示解释了口头置信度的显著方差，超越了词元对数概率的解释力
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然实现了前所未有的流畅性，但仍然容易产生"幻觉"——生成事实错误或无根据的内容。这一限制在可靠性至关重要的高风险领域中尤为关键，需要解决LLM生成内容的事实准确性问题。

Method: 提出了一个基于领域知识的分层检索与验证架构，采用四阶段自调节管道实现：I) 具有早期退出逻辑的内在验证以优化计算；II) 利用领域检测器进行自适应搜索路由以针对特定主题档案；III) 纠正性文档分级(CRAG)以过滤无关上下文；IV) 外部再生后接原子声明级验证。该框架通过LangGraph实现。

Result: 在五个不同基准测试的650个查询中评估：TimeQA v2、FreshQA v2、HaluEval General、MMLU Global Facts和TruthfulQA。实证结果显示管道在所有环境中始终优于零样本基线，在TimeQA v2中胜率达到83.7%，在MMLU Global Facts中达到78.0%。在需要精细时间和数值精度的领域中表现出高效性，事实答案行的基础性得分稳定在78.8%至86.4%之间。

Conclusion: 该架构为错误信息提供了强大的故障安全机制，但识别出"虚假前提过度声称"的持续失败模式。这些发现为多阶段RAG行为提供了详细的实证特征描述，并建议未来工作应优先考虑预检索"可回答性"节点，以进一步缩小对话AI中的可靠性差距。
...
<details>
  <summary>Details</summary>
Motivation: LLM通过多阶段异构数据源训练，但开发者缺乏系统方法确定特定行为对应的训练数据，导致调试只能被动修补，且故障容易在分布变化或模型更新后重现

Method: 提出DebugLM框架，让模型学习将响应与唯一溯源标签关联，指示责任数据集；支持针对性测试时修复，可选择性触发对特定数据源的拒绝，无需重新训练或修改模型参数

Result: 实验表明DebugLM能在多阶段训练管道中提供准确的行为追踪，实现有效的测试时修复，同时保持模型的通用效用

Conclusion: DebugLM通过为LLM提供内置数据溯源能力，解决了训练数据可观测性问题，使开发者能精确识别不良行为来源并实施针对性修复
...
TL;DR: 本文首次系统评估了12种印度语言中LLM的安全性，发现存在显著的安全漂移问题，跨语言一致性仅12.8%，安全率方差超过17%，揭示了多语言LLM的安全泛化差距。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在多语言环境中部署，其在文化多样、低资源语言中的安全行为仍知之甚少。印度语言有超过12亿使用者但在LLM训练数据中代表性不足，需要系统评估其安全性。
...
Conclusion: 研究强调了多语言LLM的关键安全泛化差距，表明安全对齐在不同语言间并不均匀传递。发布了首个支持印度语言部署的文化知情安全评估基准IndicSafe，倡导基于区域危害的语言感知对齐策略。
...
TL;DR: 提出了一种无需训练的多令牌预测方法，通过掩码令牌探测LLM的潜在能力，实现并行预测未来令牌，显著提高解码效率


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然只训练了下一个令牌预测，但具有潜在的多令牌预测能力。现有方法需要修改模型权重或依赖辅助模型，我们希望开发一种无需训练的方法来利用这种能力

Method: 使用从嵌入空间动态提取的掩码令牌探测LLM，构建推测令牌树，采样top-K候选，应用轻量级剪枝策略保留高概率延续，在解码时并行验证候选预测

Result: 在基准测试中，该方法始终优于现有的无需训练基线方法，在LLaMA3上接受长度增加约12%，在Qwen3上增加8-12%，吞吐量提升高达15-19%

Conclusion: 该方法证明了LLM解码层自然地将掩码令牌表示与下一个令牌状态对齐，无需重新训练或辅助模型即可实现准确的多步预测，为高效解码提供了新途径
...
<details>
  <summary>Details</summary>
Motivation: 当前多语言缩放定律方法无法衡量跨语言迁移效应，导致语言混合比例选择次优，需要更准确的方法来优化多语言预训练

Method: 将多语言预训练视为合作博弈，每种语言作为玩家共同贡献，使用Shapley值量化每种语言的贡献度，提出ShapleyLaw模型

Result: 实验表明ShapleyLaw在模型性能预测和语言混合优化方面优于基线方法

Conclusion: 基于合作博弈论的ShapleyLaw方法能够有效量化跨语言迁移效应，为多语言预训练提供更优的语言混合比例优化方案
...
TL;DR: 论文提出"先验偏见"新指标评估机器翻译中的性别偏见，发现仅解码器模型在性别特定指标上不优于编码器-解码器架构，但后训练能减少男性先验偏见。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在NLP任务中达到最先进水平，但仍存在系统性偏见，特别是机器翻译中的性别偏见。由于不同语言在性别标记上的差异，翻译需要将隐式源信号消歧为显式性别标记形式。现有标准基准可能捕捉到广泛差异，但未能反映现代机器翻译中性别偏见的全部复杂性。

Method: 扩展了现有的偏见评估框架：(i) 引入名为"先验偏见"的新度量，捕捉模型的默认性别假设；(ii) 将该框架应用于仅解码器的机器翻译模型。

Result: 结果显示，尽管仅解码器模型规模庞大且处于最先进水平，但在性别特定指标上通常不优于编码器-解码器架构；然而，后训练（如指令微调）不仅能提高上下文意识，还能减少男性先验偏见。

Conclusion: 需要更精细的评估框架来全面评估机器翻译中的性别偏见，后训练是减少模型性别偏见的重要途径。
...
TL;DR: 该论文提出了ConGA框架，用于解决从英语到意大利语翻译中的性别偏见问题，通过细粒度标注和评估方法改善机器翻译的性别处理。


<details>
  <summary>Details</summary>
Motivation: 机器翻译和大型语言模型在处理跨语言性别问题时面临持续挑战，特别是从性别中立语言（如英语）翻译到形态上有性别标记的语言（如意大利语）时。这种不对称性导致翻译系统默认使用阳性形式，强化了偏见并降低了翻译准确性。

Method: 提出了上下文性别标注（ConGA）框架，这是一套基于语言学的词级性别标注指南。该方案区分英语中的语义性别（阳性M、阴性F、模糊A）和意大利语中的语法性别实现（阳性M、阴性F），并结合实体级标识符进行跨句子跟踪。将ConGA应用于gENder-IT数据集，创建了评估翻译性别偏见的黄金标准资源。

Result: 研究结果揭示了系统性的阳性过度使用和不一致的阴性实现，突显了当前机器翻译系统的持续局限性。通过结合细粒度语言标注和定量评估，为构建更具性别意识和多语言的NLP系统提供了方法论和基准。

Conclusion: 这项工作通过结合细粒度语言标注与定量评估，为构建更具性别意识和多语言的NLP系统提供了方法论和基准，有助于解决机器翻译中的性别偏见问题。
...
TL;DR: 本文提出了一种使用大语言模型（LLM）进行社会-环境规划中问题概念化的模板化工作流程，通过ChatGPT将利益相关者的自然语言描述转化为定量模型，简化了参与式建模过程。
...
Method: 提出模板化工作流程：1）使用大语言模型从利益相关者的直观问题描述中识别基本模型组件；2）探索处理问题的多样化视角；3）将这些组件组装成统一模型；4）通过迭代通信在Python中实现模型。使用ChatGPT 5.2 Instant在湖泊问题和电力市场问题上进行了演示。

Result: 在两个案例研究中，经过几次迭代和人工验证与精炼后，都获得了可接受的输出。实验表明大语言模型可以成为促进社会-环境规划中问题概念化过程参与式建模的有效工具。

Conclusion: 大语言模型能够有效促进社会-环境规划中的参与式建模，特别是在问题概念化阶段，通过将自然语言描述转化为定量模型组件，为后续的深度不确定性规划步骤奠定基础。
...
### [163] [DAPS++: Rethinking Diffusion Inverse Problems with Decoupled Posterior Annealing](https://arxiv.org/abs/2511.17038)
*Hao Chen,Renzheng Zhang,Scott S. Howard*

Main category: cs.AI

TL;DR: 论文提出DAPS++方法，将扩散模型在逆问题求解中的角色重新解释为EM框架中的初始化阶段，实现扩散阶段与数据驱动细化的解耦，从而提高计算效率和重建性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于分数的扩散方法在解决逆问题时，虽然采用贝叶斯联合推断框架，但实际上先验提供的指导有限，重建主要依赖测量一致性项，导致推断过程与扩散动力学基本解耦。需要澄清这种结构并改进方法。

Method: 提出DAPS++方法，将扩散重新解释为期望最大化（EM）框架中的初始化阶段，使似然项能更直接地指导推断，同时保持数值稳定性。该方法减少函数评估次数和测量优化步骤，实现扩散阶段与数据驱动细化的完全解耦。

Result: DAPS++在多种图像恢复任务中实现了高计算效率和鲁棒的重建性能，需要更少的函数评估次数和测量优化步骤，同时解释了为什么统一的扩散轨迹在实践中仍然有效。

Conclusion: 通过将扩散重新定位为EM框架中的初始化阶段，DAPS++澄清了扩散在逆问题求解中的角色，使似然项能更直接地指导推断，实现了计算效率和重建性能的平衡。
...
TL;DR: 该研究通过构建基于文本的多智能体版Clue游戏，评估LLM智能体的多步演绎推理能力，发现智能体在完整游戏中难以保持一致的推理，且微调对性能提升有限。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体在"谁是真凶"这类复杂推理任务中表现不佳，需要系统评估其多步演绎推理能力。经典桌游Clue提供了基于规则的测试平台，可以检验智能体在长时间游戏中的推理一致性。

Method: 构建文本版多智能体Clue游戏，使用GPT-4o-mini和Gemini-2.5-Flash共6个智能体参与。研究微调结构化逻辑谜题是否能够提升游戏中的推理表现。进行了18场模拟游戏。

Result: 在18场游戏中，智能体仅获得4次正确胜利，表明在完整游戏中难以保持一致的演绎推理。微调并不能可靠提升性能，有时反而增加了推理量但未改善推理精度。

Conclusion: LLM智能体在需要长期一致推理的复杂任务中仍面临挑战，简单的微调策略不足以解决多步演绎推理问题，需要更系统的方法来提升智能体的推理一致性。
...
TL;DR: Draft-and-Prune (D&P) 框架通过多样化和验证改进自动形式化的逻辑推理，在多个基准测试中显著提升性能
...
Method: 提出 Draft-and-Prune (D&P) 推理时框架，通过多样化和验证改进基于AF的逻辑推理。首先草拟多个自然语言计划，并基于这些计划生成程序；然后修剪可执行但矛盾或模糊的形式化，通过多数投票从存活路径中聚合预测。

Result: 在四个代表性基准测试（AR-LSAT、ProofWriter、PrOntoQA、LogicalDeduction）上，D&P显著增强了基于AF的推理，无需额外监督。在AR-LSAT的AF-only设置中，D&P使用GPT-4达到78.43%准确率，使用GPT-4o达到78.00%准确率，显著优于最强的AF基线MAD-LOGIC和CLOVER。在其他基准测试中达到接近上限的性能，包括PrOntoQA和LogicalDeduction的100%。

Conclusion: Draft-and-Prune框架通过结合多样化和验证机制，有效解决了自动形式化中的语义失败问题，显著提升了逻辑推理的准确性和鲁棒性。
...
TL;DR: CRAFT是一个利用模型推理能力和隐藏表示来增强对抗越狱攻击鲁棒性的红队对齐框架，通过优化隐藏状态空间目标实现安全感知推理轨迹对齐。


<details>
  <summary>Details</summary>
Motivation: 现有防御主要在输出层面操作，缺乏对推理过程的深度安全对齐。CRAFT旨在利用大型推理模型的隐藏表示空间，实现更鲁棒的安全对齐，防止越狱攻击。

Method: CRAFT整合对比表示学习和强化学习，通过分离安全与不安全推理轨迹，在潜在空间构建支持鲁棒推理级安全对齐的几何结构。理论证明将潜在-文本一致性纳入GRPO可消除表面对齐策略。

Result: 在Qwen3-4B-Thinking和R1-Distill-Llama-8B两个强推理模型上，CRAFT在多个安全基准测试中优于IPO和SafeKey等最先进防御方法，推理安全性平均提升79.0%，最终响应安全性提升87.7%。

Conclusion: CRAFT通过隐藏空间推理对齐有效提升模型安全性，证明了在潜在表示空间进行安全对齐的优越性，为对抗越狱攻击提供了新的防御范式。
...
TL;DR: 提出InfoDensity奖励框架，通过优化推理质量而非仅长度来减少LLM推理过程的冗余，在保持准确率的同时显著降低计算成本


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法仅优化最终响应长度，忽略了中间推理步骤的质量，导致模型容易受到奖励攻击。作者认为冗余不仅是长度问题，更是中间推理质量差的表现

Method: 通过实证研究跟踪推理步骤中答案分布的条件熵，发现高质量推理具有低不确定性收敛和单调进展特性。基于此提出InfoDensity奖励框架，结合AUC奖励和单调性奖励作为推理质量统一度量，并通过长度缩放项鼓励更简洁地实现同等质量

Result: 在数学推理基准测试中，InfoDensity在准确率上匹配或超越最先进基线方法，同时显著减少令牌使用量，实现了强大的准确率-效率权衡

Conclusion: 高质量推理具有信息密集特性，每个步骤都贡献有意义的熵减少。InfoDensity框架通过优化推理质量而非仅长度，有效解决了LLM推理冗余问题，实现了更好的计算效率
...
TL;DR: PIER是一个基于离线强化学习的船舶航线规划框架，通过物理信息环境学习节能、安全的航线策略，相比传统大圆航线可减少10%的CO2排放，并将极端燃料消耗情况减少9倍。


<details>
  <summary>Details</summary>
Motivation: 国际航运占全球温室气体排放的3%，但当前航线规划仍主要依赖启发式方法。需要开发更节能、安全且不依赖在线模拟器的航线优化方案。

Method: PIER采用离线强化学习框架，结合物理校准环境（基于历史船舶跟踪数据和海洋再分析产品）、物理信息状态构建、演示增强的离线数据，以及解耦的后验安全防护机制。

Result: 在墨西哥湾7条航线的一年数据验证中：1）平均CO2排放减少10%；2）极端燃料消耗（>1.5倍中位数）从4.8%降至0.5%（减少9倍）；3）燃料方差降低3.5倍；4）与真实最快航迹一致且方差降低23.1倍；5）不依赖天气预报，性能稳定。

Conclusion: PIER框架成功实现了节能、安全的船舶航线规划，其不依赖预报的特性使其在实际应用中更可靠。该架构可推广到野火疏散、飞机轨迹优化和未知地形自主导航等领域。
...
TL;DR: ShuttleEnv是一个基于数据的羽毛球交互式仿真环境，用于支持强化学习和策略分析，使用精英球员比赛数据和概率模型模拟对抗过程。


<details>
  <summary>Details</summary>
Motivation: 为快速对抗性体育（如羽毛球）提供现实且可解释的智能体-对手交互平台，支持强化学习研究和策略行为分析，避免依赖物理仿真。

Method: 基于精英球员比赛数据，采用显式概率模型模拟回合级动态，创建交互式数据驱动仿真环境，支持实时可视化和交互分析。

Result: 开发了ShuttleEnv平台，展示了多个训练有素的智能体，提供实时回合可视化，允许探索不同打法风格、观察涌现策略并交互分析决策行为。

Conclusion: ShuttleEnv作为可重用平台，为体育AI领域的研究、可视化和智能体演示提供了有效工具，支持快速对抗性体育中的强化学习和策略分析。
...
TL;DR: RideJudge是一个用于网约车责任判定的渐进式视觉-逻辑对齐框架，通过合成轨迹、自适应上下文优化和有序敏感强化学习等技术，解决了现有多模态大模型在责任判定中的语义鸿沟、逻辑松散和透明度不足问题。


<details>
  <summary>Details</summary>
Motivation: 网约车责任判定面临三大挑战：1）海量订单使人工审核不可行；2）传统自动化方法缺乏司法决策所需的推理透明度；3）多模态大模型存在感知幻觉和逻辑松散问题，难以将通用视觉语义与严格证据协议对齐。

Method: 提出RideJudge框架：1）SynTraj合成引擎将抽象责任概念转化为具体轨迹模式；2）自适应上下文优化策略提炼专家知识；3）裁决链机制强制主动证据查询；4）有序敏感强化学习根据严重程度层次校准决策边界。

Result: RideJudge-8B模型达到88.41%的准确率，超越了32B规模的基线模型，为可解释的责任判定设立了新标准。

Conclusion: RideJudge通过渐进式视觉-逻辑对齐框架，有效解决了网约车责任判定中的系统性错位问题，在保持高效自动化的同时提供了司法决策所需的推理透明度。
...
TL;DR: 本文发现大型推理模型启用思维链后安全性下降，提出在思维链生成前加强安全决策的新对齐方法，在保持推理性能的同时显著提升安全性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过思维链显著提升推理能力，但研究发现这种增强是以安全性显著下降为代价的。作者观察到安全性下降只在启用思维链时发生，禁用思维链时不会出现，这促使他们考虑在思维链生成前加强模型的安全决策能力。

Method: 提出一种新颖的安全对齐方法，在思维链生成前促进大型推理模型的安全决策。具体方法：1）使用基于BERT的分类器从安全模型（如禁用思维链的大型推理模型）中提取安全决策信号；2）将这些信号作为辅助监督集成到大型推理模型的安全对齐中；3）通过这种方式，安全梯度可以反向传播到模型的潜在表示，有效增强模型在思维链生成前的安全决策能力。

Result: 大量实验表明，该方法显著提升了大型推理模型的安全能力，同时有效保持了模型的一般推理性能。

Conclusion: 通过在思维链生成前加强安全决策的新对齐方法，成功解决了大型推理模型在启用思维链时安全性下降的问题，实现了安全性和推理性能的平衡。
...
TL;DR: 提出将医患对话建模为部分可观测下的主动知识查询问题，开发结合状态提取、信念更新、知识检索和POMDP-lite规划的框架，在受控模拟环境中验证概念可行性


<details>
  <summary>Details</summary>
Motivation: 现有电子病历系统多为输出导向，仅转录、提取和总结咨询内容，缺乏对已知信息、缺失信息、关键不确定性的建模，无法主动指导下一步问题或建议

Method: 将医患对话建模为部分可观测下的主动知识查询问题，结合状态提取、序列信念更新、缺口感知状态建模、混合知识检索和POMDP-lite动作规划器

Result: 在10个标准化多轮对话和300个查询检索基准测试中，完整框架达到83.3%覆盖率、80.0%风险召回率、81.4%结构完整性，冗余度低于基线方法

Conclusion: 主动查询方法在严格受控条件下具有方法论意义，是对话式电子病历生成的概念上有吸引力的表述，但仅为概念验证，不表明临床部署准备就绪
...
Method: 提出IET（隐式执行追踪）框架：在生成过程中将智能体特定的密钥信号嵌入到令牌分布中，使文本成为仅用密钥可检测的自描述执行轨迹；检测时使用转移感知评分方法识别智能体交接点并重建交互图。

Result: 实验表明IET能够高精度恢复智能体片段和协调结构，同时保持生成质量，为多智能体语言系统实现隐私保护的审计功能。

Conclusion: IET提供了一种无需元数据的解决方案，使多智能体系统的责任追溯成为可能，支持隐私保护的审计需求。
...
TL;DR: 提出了一种新的可解释AI方法——信息性半事实解释（ISF），不仅生成半事实解释（说明即使某些特征改变，预测结果仍保持不变），还补充解释为什么这些极端值变化不会改变结果，通过揭示影响决策的隐藏特征。


<details>
  <summary>Details</summary>
Motivation: 当前半事实解释方法存在局限性：它们只告诉用户即使某些输入特征发生极端变化，预测结果仍保持不变，但无法解释为什么这些变化不会影响结果。用户需要更详细的解释来理解决策背后的原因，例如在贷款审批场景中，用户不仅想知道即使贷款金额翻倍仍能获批，还想知道是因为良好的信用评分才使得这种变化不影响结果。

Method: 提出信息性半事实解释（ISF）算法，该算法在生成半事实解释的基础上，补充关于影响自动化决策的隐藏特征的信息。ISF不仅找到单个关键特征的最大值变化而不改变结果（传统半事实方法），还识别并解释为什么这些极端值变化不会影响结果，通过揭示其他隐藏特征的作用机制。

Result: 在基准数据集上的实验结果表明，ISF方法生成的半事实解释既具有信息性，又在关键指标上表现出高质量。用户研究显示，人们更喜欢这种详细的解释，而不是当前方法生成的简单半事实解释。

Conclusion: ISF方法通过提供更详细的解释，增强了半事实解释的实用性和可理解性，使XAI系统能够为用户提供更有意义的决策解释，帮助用户理解为什么某些特征的变化不会影响最终结果。
...
Method: 提出Sensi架构，包含三个核心机制：1) 分离感知与行动的两玩家架构；2) 外部状态机管理的课程学习系统；3) 数据库作为控制平面实现可编程上下文窗口。还引入LLM作为裁判组件，动态生成评估标准。

Result: Sensi v1使用两玩家架构解决了2个游戏关卡；Sensi v2加入课程学习后解决了0个关卡，但仅用约32次行动尝试就完成了全部学习课程，相比需要1600-3000次尝试的同类系统实现了50-94倍的样本效率提升。

Conclusion: Sensi架构显著提高了学习效率，但失败模式表明架构瓶颈已从学习效率转向感知基础问题（自我一致的幻觉级联），这是一个更易处理的问题。
...
TL;DR: 该论文提出MALLES框架，利用大语言模型构建多智能体经济沙盒，通过偏好学习和均值场机制解决高维多模态环境中的决策挑战，显著提升产品选择和购买预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 现实经济中的现代决策面临高维多模态环境、智能体异质性和组合数据稀疏性的挑战，需要建立能够跨领域、跨类别应用的统一仿真框架。

Method: 1) 基于偏好学习范式，通过对大量异质交易记录进行后训练，使LLM经济对齐；2) 采用均值场机制建模产品环境与客户群体的动态交互，稳定高维决策空间的采样过程；3) 提出多智能体讨论框架，让专业化智能体协作处理大量产品信息，通过结构化对话捕捉关键决策因素。

Result: 实验表明，相比现有经济和金融LLM仿真基线，该框架在产品选择准确性、购买数量预测和仿真稳定性方面取得显著改进。

Conclusion: 大语言模型有潜力成为现实经济中高保真、可扩展决策仿真和后续分析的基础支柱，基于基础数据库构建统一的经济仿真框架。
...
TL;DR: 本文综述了自动驾驶领域合成数据与仿真技术的最新进展，涵盖合成数据在感知规划中的应用、数字孪生仿真验证、以及域适应策略，并讨论了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶技术虽取得显著进展，但在实际部署中面临数据稀缺、安全要求和跨环境泛化等挑战，需要合成数据和虚拟环境作为可扩展、可控的训练评估解决方案。

Method: 从三个核心维度组织研究现状：1）合成数据在感知和规划中的应用；2）基于数字孪生的系统验证仿真；3）连接合成与真实数据的域适应策略。同时提供数据集、工具和仿真平台的详细分类。

Result: 建立了自动驾驶仿真与合成数据领域的综合框架，识别了视觉语言模型和仿真真实感在增强场景理解和泛化能力中的作用，分析了基准设计趋势。

Conclusion: 需要解决Sim2Real迁移、可扩展安全验证、协同自主性和仿真驱动策略学习等关键挑战，以加速实现安全、可泛化、全球可部署的自动驾驶系统。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地作为持久知识工作者使用，上下文记忆成为默认策略，但实际部署中暴露出严重问题，需要探索更可靠的知识存储和检索方法。

Method: 使用Claude Sonnet 4.5等前沿模型进行基准测试，比较上下文记忆与知识对象（离散哈希寻址元组）的性能，分析容量限制、压缩损失和目标漂移等失败模式，并测试多跳推理能力。
...
Conclusion: 上下文记忆存在系统性缺陷，知识对象在知识存储和检索方面表现更优，建议采用密度自适应检索作为切换机制，并发布了基准测试套件。
...
Method: 提出RPMS架构，通过结构化规则检索强制执行动作可行性，通过轻量级信念状态门控记忆适用性，并通过规则优先仲裁解决两者之间的冲突。

Result: 在ALFWorld的134个未见任务中，RPMS使用Llama 3.1 8B达到59.7%的单次试验成功率（比基线提高23.9个百分点），使用Claude Sonnet 4.5达到98.5%（提高11.9个百分点）。规则检索单独贡献了14.9个百分点的提升。在ScienceWorld中使用GPT-4也获得了一致的增益（平均得分54.0 vs. ReAct基线的44.9）。

Conclusion: RPMS架构通过规则检索和信念状态管理有效解决了LLM智能体在封闭世界环境中的动作可行性问题。关键发现是情景记忆有条件地有用：在没有基础的情况下会损害某些任务类型的性能，但一旦通过当前状态过滤并受明确动作规则约束，就会成为稳定的净正收益。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能体自我进化方法主要将成功经验记录为文本提示或反思，这在复杂场景中无法可靠保证任务的高效重新执行

Method: 提出AgentFactory范式，将成功任务解决方案保存为可执行的子智能体Python代码，这些子智能体基于执行反馈持续优化，具有标准化文档和跨系统可移植性

Result: AgentFactory实现了持续能力积累：可执行子智能体库随时间增长和改进，逐步减少类似任务所需的工作量，无需人工干预

Conclusion: AgentFactory通过将成功经验编码为可执行子智能体而非文本记录，为LLM智能体自我进化提供了更可靠、高效的新范式
...
### [181] [A Controlled Comparison of Deep Learning Architectures for Multi-Horizon Financial Forecasting: Evidence from 918 Experiments](https://arxiv.org/abs/2603.16886)
*Nabeel Ahmad Saidd*

Main category: q-fin.ST

TL;DR: 该研究对9种深度学习架构在金融时间序列预测中进行系统比较，发现ModernTCN表现最佳，架构选择解释了几乎所有性能差异，而随机种子影响可忽略。


<details>
  <summary>Details</summary>
Motivation: 深度学习架构在金融价格预测中快速发展，但缺乏严格的基准评估。本研究旨在通过控制实验比较不同架构家族（Transformer、MLP、CNN、RNN）在加密货币、外汇和股指市场的多步预测性能。

Method: 采用严格的五阶段实验协议：1）固定种子的贝叶斯超参数优化；2）按资产类别固定配置；3）多种子重训练；4）不确定性聚合；5）统计验证。在加密货币、外汇和股指市场，以4小时和24小时为预测范围，共进行918次实验。

Result: ModernTCN获得最佳平均排名（1.333）和75%的第一名率，其次是PatchTST（2.000）。结果显示清晰的三层排名结构，架构解释了几乎所有性能方差，而种子随机性可忽略。不同时间范围的排名保持稳定，尽管误差放大2-2.5倍。所有配置的方向性准确率接近50%，表明基于MSE训练的模型在小时分辨率上缺乏方向性预测能力。

Conclusion: 研究强调了架构归纳偏置相对于原始参数数量的重要性，为多步金融预测提供了可复现的指导。ModernTCN在金融时间序列预测中表现最优，架构选择是关键因素。
...
### [182] [Amanous: Distribution-Switching for Superhuman Piano Density on Disklavier](https://arxiv.org/abs/2603.16890)
*Joonhyung Bae*

Main category: cs.MM

TL;DR: Amanous系统将三种自动钢琴作曲传统（Nancarrow、Xenakis、L-system）统一，通过分布切换机制在Yamaha Disklavier上实现超人类极限的纹理创作。


<details>
  <summary>Details</summary>
Motivation: 现有三种主要的自动钢琴作曲传统（Nancarrow的速度卡农、Xenakis的随机分布、L-system语法）各自独立发展，缺乏统一框架。需要一种硬件感知的作曲系统来整合这些方法，实现超越人类物理极限的音乐纹理。

Method: 提出四层架构（符号层、参数层、数值层、物理层），通过分布切换机制让L-system符号选择不同的分布体系而非仅仅调整参数。包含硬件抽象层处理延迟和键复位约束，密度扫描分析计算饱和点，收敛点演算将速度卡农几何转化为控制接口。

Result: 1) 四层架构产生统计显著不同的音乐段落（效应量d=3.70-5.34）；2) 硬件抽象层确保超人类纹理在Disklavier可执行范围内；3) 发现24-30音符/秒的计算饱和点；4) 收敛点演算实现宏观结构与微观纹理的链接。系统已在物理Disklavier上部署，软件精度达亚毫秒级。

Conclusion: Amanous系统成功统一了三种自动钢琴作曲传统，通过分布切换机制实现了硬件感知的超人类极限音乐创作。系统展示了算法自一致性和高精度执行，为未来心理声学验证奠定了基础。
...
### [183] [Beyond Forced Modality Balance: Intrinsic Information Budgets for Multimodal Learning](https://arxiv.org/abs/2603.17347)
*Zechang Xiong,Da Li,Kexin Tang,Pengyuan Li,Wenkang Kong,Yulan Hu*

Main category: cs.MM

TL;DR: IIBalance是一个多模态学习框架，通过内在信息预算来平衡不同模态的贡献，避免强势模态主导，提高互补信息利用


<details>
  <summary>Details</summary>
Motivation: 多模态模型常出现模态不平衡问题，强势模态会主导学习过程，导致次优性能。现有方法通过重新加权梯度或损失来平衡模态，但忽略了每个模态都有有限信息容量这一事实。

Method: 提出IIBalance框架：1) 基于任务估计每个模态的内在信息预算，将其转化为模态贡献的全局先验；2) 以最高预算模态为锚点，设计基于原型的相对对齐机制，仅在较弱模态偏离其预算潜力时纠正语义漂移；3) 推理时使用概率门控模块，结合全局预算和样本级不确定性生成校准的融合权重。

Result: 在三个代表性基准测试上的实验表明，IIBalance始终优于最先进的平衡方法，并能更好地利用互补模态线索。

Conclusion: IIBalance通过考虑模态的内在信息容量来平衡多模态学习，避免了强制模仿，实现了更好的模态贡献对齐和性能提升。
...
### [184] [Quantum-Assisted Optimal Rebalancing with Uncorrelated Asset Selection for Algorithmic Trading Walk-Forward QUBO Scheduling via QAOA](https://arxiv.org/abs/2603.16904)
*Abraham Itzhak Weinberg*

Main category: q-fin.PM

TL;DR: 该论文提出了一种混合经典-量子框架用于投资组合构建和再平衡，通过量子优化算法减少交易成本同时保持风险调整后收益。


<details>
  <summary>Details</summary>
Motivation: 传统投资组合再平衡通常产生高交易成本，需要探索量子优化算法在金融应用中的潜力，特别是减少交易频率同时保持投资组合性能。

Method: 1. 资产选择：使用Ledoit-Wolf收缩协方差估计结合层次相关聚类从S&P 500中选取10只去相关股票；2. 权重优化：通过GPU加速的熵正则化遗传算法；3. 再平衡调度：将再平衡计划表述为QUBO问题，使用QAOA量子近似优化算法在无前瞻偏见的向前滚动框架中求解。

Result: 在S&P 500数据上回测（训练：2010-2024；样本外测试：2025），GA+QAOA策略获得夏普比率0.588和总回报10.1%，略优于最强经典基准（GA+10天定期再平衡，夏普0.575），同时执行8次再平衡vs 24次，交易成本降低44.5%。

Conclusion: 混合经典-量子架构可以减少投资组合再平衡的换手率，同时保持有竞争力的风险调整后性能，为近期量子优化在金融应用提供了结构化测试平台。
...
### [185] [Simultaneous super-resolution and optical sectioning with four-beam interference structured illumination microscopy (4I-SIM)](https://arxiv.org/abs/2603.16908)
*Jiaming Qian,Jing Feng,Hongjun Wu,Maoxian Zhang,Dongqin Lu,Tianchi Kang,Xinyu Han,Qian Chen,Chao Zuo*

Main category: eess.IV

TL;DR: 4I-SIM通过四光束干涉扩展频率支持并补偿轴向缺失锥，实现无伪影超分辨和光学切片，克服传统2D-SIM在厚样品中的局限性


<details>
  <summary>Details</summary>
Motivation: 传统二维结构光照明显微镜(2D-SIM)在厚或散射样品中存在光学传递函数的缺失锥问题，导致离焦背景和重建伪影，影响图像保真度

Method: 提出四光束干涉结构光照明显微镜(4I-SIM)，通过引入额外干涉阶次扩展横向频率支持并同时补偿轴向缺失锥，实现固有光学切片

Result: 在多种厚固定和活体样品中验证，4I-SIM相比2D-SIM实现近两倍横向分辨率提升和显著改善的切片能力，达到103nm横向和336nm轴向分辨率

Conclusion: 4I-SIM通过最小硬件修改、低光毒性和开源重建工具，建立了在复杂生物环境中同时实现超分辨和光学切片的实用可重复平台
...
### [186] [UNICORN: Ultrasound Nakagami Imaging via Score Matching and Adaptation for Assessing Hepatic Steatosis](https://arxiv.org/abs/2603.16942)
*Kwanyoung Kim,Jaa-Yeon Lee,Youngjun Ko,GunWoo Lee,Jong Chul Ye*

Main category: eess.IV

TL;DR: 提出UNICORN方法，通过分数匹配和自适应技术实现超声Nakagami成像，提供高分辨率参数映射，用于肝脂肪变性评估


<details>
  <summary>Details</summary>
Motivation: 传统B超在组织表征方面有限，现有Nakagami成像方法存在窗口大小选择困难和估计器不稳定问题，导致图像分辨率降低

Method: 提出UNICORN方法，基于超声包络信号的分数函数，提供精确的闭式Nakagami参数估计器，实现像素级参数映射

Result: UNICORN能有效评估肝脂肪变性，提供与脂肪变性相关的背向散射统计视觉区分，在患者真实数据实验中验证了临床检测能力和鲁棒性

Conclusion: UNICORN方法解决了现有Nakagami成像的局限性，实现了高分辨率参数映射，为肝脂肪变性的临床检测提供了有效工具
...
### [187] [A Lensless Polarization Camera](https://arxiv.org/abs/2603.17156)
*Noa Kraicer,Shay Elmalem,Erez Yosef,Hani Barhum,Raja Giryes*

Main category: eess.IV

TL;DR: 提出了一种紧凑的无透镜偏振相机，使用漫射器和条纹偏振掩模，通过计算重建从单次快照中恢复四个线性偏振图像。


<details>
  <summary>Details</summary>
Motivation: 现有偏振相机使用空间或时间复用技术，增加了相机体积、重量和成本。无透镜成像方法（如DiffuserCam）已证明可以通过用编码元件替换镜头并执行计算重建来实现紧凑成像系统。

Method: 设计了一个紧凑的无透镜偏振相机，由漫射器和简单的条纹偏振掩模组成。开发了明确建模偏振编码无透镜测量的重建算法，从单次快照中恢复四个线性偏振图像。

Result: 成功从单次快照中恢复了四个线性偏振图像，证明了无透镜方法在偏振成像中的潜力，并揭示了影响重建质量的物理因素。

Conclusion: 无透镜偏振相机为紧凑偏振成像提供了有前景的解决方案，揭示了重建质量的关键物理因素，为开发高质量实用系统提供了指导。
...
### [188] [Structured SIR: Efficient and Expressive Importance-Weighted Inference for High-Dimensional Image Registration](https://arxiv.org/abs/2603.17415)
*Ivor J. A. Simpson,Neill D. F. Campbell*

Main category: eess.IV

TL;DR: 提出了一种用于3D密集图像配准的高效概率推理方法Structured SIR，通过新颖的内存高效协方差参数化（低秩协方差+稀疏结构化Cholesky精度因子）实现多模态不确定性量化，相比变分方法具有更好的校准性和样本质量。


<details>
  <summary>Details</summary>
Motivation: 图像配准是一个不适定的密集视觉任务，多个解都能达到相似的损失值，这促使需要概率推理。现有的变分推理方法由于对后验形式的限制性假设，可能导致表征不足、过度自信和低质量样本。更灵活的后验通常受限于3D图像配准所需的高维协方差矩阵的复杂性。

Method: 提出了Structured SIR方法，采用采样重要性重采样(SIR)算法，结合新颖的内存高效高维协方差参数化：将协方差表示为低秩协方差和稀疏、空间结构化Cholesky精度因子的和。这种结构能够捕捉复杂的空间相关性，同时保持计算可行性。

Result: 在脑MRI数据的3D密集图像配准中评估，该方法产生的不确定性估计比变分方法显著更好地校准，达到同等或更好的准确性。关键的是，模型产生了高度结构化的多模态后验分布，实现了有效且高效的不确定性量化。

Conclusion: Structured SIR方法为高维密集图像配准提供了一种内存和计算高效的推理方法，能够表达性地表征多模态不确定性，产生高质量样本，相比现有方法在不确定性校准方面有显著改进。
...
### [189] [Deep Learning-Based Airway Segmentation in Systemic Lupus Erythematosus Patients with Interstitial Lung Disease (SLE-ILD): A Comparative High-Resolution CT Analysis](https://arxiv.org/abs/2603.17547)
...
TL;DR: 使用深度学习U-Net架构自动分割HRCT气道，发现SLE-ILD患者相比非ILD患者在上叶气道体积显著增大，提示气道扩张可作为SLE-ILD的生物标志物。


<details>
  <summary>Details</summary>
Motivation: 系统性红斑狼疮（SLE）患者常并发间质性肺病（ILD），但缺乏有效的定量影像学生物标志物。本研究旨在通过深度学习自动分割技术，量化比较SLE-ILD与非ILD患者的气道体积差异，探索气道结构改变作为疾病生物标志物的潜力。

Method: 回顾性分析106名SLE患者（27名SLE-ILD，79名非ILD）的HRCT影像。开发基于U-Net架构的定制深度学习框架，自动分割肺叶和段级气道结构。使用双样本t检验比较两组间气道体积差异（显著性阈值p<0.05）。

Result: 肺叶水平：SLE-ILD患者右肺上叶（p=0.009）和左肺上叶（p=0.039）气道体积显著增大。段水平：R1（p=0.016）、R3（p<0.001）和L3（p=0.038）段气道体积差异显著，上肺区变化最明显，下肺区仅显示非显著性趋势。

Conclusion: 深度学习自动量化方法能有效检测SLE-ILD患者特异性气道扩张模式，主要影响上叶和特定肺段。这种AI驱动的定量影像学生物标志物有望增强SLE人群中ILD的早期检测和监测，促进个体化患者管理。
