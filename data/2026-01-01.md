...
TL;DR: 提出NIRNL框架解决跨模态检索中噪声标签问题，通过跨模态边界保持和邻居感知实例细化，实现高性能、高可靠性和高数据利用率


<details>
  <summary>Details</summary>
Motivation: 跨模态检索面临大规模标注数据收集耗时耗力的问题，多模态数据标注不可避免地包含噪声，这会降低模型检索性能。现有鲁棒跨模态检索方法往往无法同时满足模型性能上限、校准可靠性和数据利用率的要求。

Method: 提出Neighbor-aware Instance Refining with Noisy Labels (NIRNL)框架：1) Cross-modal Margin Preserving (CMP)调整正负样本对的相对距离，增强样本对之间的区分度；2) Neighbor-aware Instance Refining (NIR)通过跨模态邻居一致性识别纯子集、困难子集和噪声子集；3) 为这种细粒度划分构建不同的定制优化策略，最大化利用所有可用数据同时减轻错误传播。

Result: 在三个基准数据集上的大量实验表明，NIRNL实现了最先进的性能，表现出显著的鲁棒性，特别是在高噪声率下。

Conclusion: NIRNL框架有效解决了跨模态检索中的噪声标签问题，通过创新的跨模态边界保持和邻居感知实例细化方法，在保持高性能的同时提高了数据利用率和校准可靠性。
...
TL;DR: 本文提出了一种基于Depth Anything V2架构的单目深度估计方法，通过动态向量低秩适应技术将合成数据先验知识迁移到医疗内窥镜领域，在手术场景中显著提升了深度估计的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统自监督单目深度估计方法在充满镜面反射和液体的内窥镜手术环境中表现脆弱，特别是在薄手术器械和透明表面上容易出现边界坍塌问题。现有方法通常依赖带噪声真实世界伪标签训练的基础模型，难以适应医疗领域的特殊挑战。

Method: 采用Depth Anything V2架构的高保真合成先验知识，通过动态向量低秩适应技术高效地将这些先验知识迁移到医疗领域，同时最小化参数预算以弥合合成到真实的差距。此外，在SCARED数据集上引入了物理分层评估协议，专门量化高镜面反射区域的性能。

Result: 该方法在SCARED数据集上取得了新的最先进性能，准确度（< 1.25）达到98.1%，与现有基线相比，平方相对误差降低了超过17%，在恶劣手术光照条件下表现出卓越的鲁棒性。

Conclusion: 通过结合Depth Anything V2的高保真合成先验和动态向量低秩适应技术，成功解决了内窥镜手术环境中单目深度估计的挑战，特别是在薄结构和透明表面上的性能显著提升，为机器人手术提供了更可靠的深度感知能力。
...
TL;DR: D²VLM：通过解耦学习框架解决视频语言模型中的时间定位问题，采用"先定位后回答"范式，并引入因子化偏好优化算法提升事件级感知能力


<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型在事件级感知的时间定位方面存在困难，且时间定位与文本响应这两个关键任务通常耦合处理，缺乏清晰的逻辑层次结构，导致目标函数次优

Method: 提出D²VLM框架，解耦时间定位和文本响应学习，采用"先定位后回答"范式，引入证据标记进行证据定位；开发因子化偏好优化算法，将概率时间定位建模纳入优化目标；构建合成数据集支持因子化偏好学习
...
TL;DR: 本文提出了一种基于视频的自动评估管道，用于城市作战训练中的"进入并清理房间"演练，通过计算机视觉提取骨骼、视线和轨迹数据，开发特定任务指标来评估心理运动流畅性、情境意识和团队协调能力。


<details>
  <summary>Details</summary>
Motivation: 有效的城市作战训练需要情境意识和肌肉记忆，但传统评估方法依赖昂贵传感器或主观观察，限制了可扩展性和准确性。需要一种无需额外硬件的自动性能评估方案。

Method: 使用计算机视觉模型从训练视频中提取2D骨骼、视线向量和运动轨迹，开发特定任务指标测量心理运动流畅性、情境意识和团队协调，通过加权组合的扩展认知任务分析层次结构生成总体性能评分。

Result: 通过真实世界ECR演练的案例研究，展示了该方法能够提供可操作的领域特定指标，捕获个人和团队表现，支持行动后评估的交互式仪表板。

Conclusion: 视频分析可实现合成训练环境中的可扩展评估，未来工作包括扩展到3D视频数据，并解决跟踪困难、地面真实验证等局限性。
...
TL;DR: 提出一种空间提升的全景拼接框架，将2D图像提升为3D点云表示，通过统一投影中心生成几何一致的360°全景图，有效减少视差和遮挡带来的失真。


<details>
  <summary>Details</summary>
Motivation: 传统2D单应性变换和网格变形方法在处理真实3D场景（多深度层、遮挡）时会产生重影、结构弯曲和拉伸失真，特别是在多视图累积和360°闭环拼接场景中问题更加严重。

Method: 1) 将输入图像提升为统一坐标系下的密集3D点表示，通过置信度指标进行全局跨视图融合；2) 在3D空间建立统一投影中心，采用等距圆柱投影将融合数据映射到单一全景流形；3) 在画布域进行空洞填充以处理视点转换揭示的未知区域。

Result: 实验评估表明，该方法在显著视差和复杂遮挡场景中大幅减少了几何失真和重影伪影，产生了更自然、一致的全景结果。
...
### [6] [Lifelong Domain Adaptive 3D Human Pose Estimation](https://arxiv.org/abs/2512.23860)
*Qucheng Peng,Hongfei Xue,Pu Wang,Chen Chen*

Main category: cs.CV

TL;DR: 该论文提出了终身领域自适应3D人体姿态估计任务，通过创新的GAN框架解决非平稳目标姿态数据集问题，有效缓解领域偏移和灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法依赖受控环境下的标注数据，难以泛化到真实世界场景。传统的领域自适应方法忽略了目标姿态数据集的非平稳性问题，无法应对连续变化的领域适应需求。

Method: 提出终身领域自适应3D HPE任务，设计创新的GAN框架，包含3D姿态生成器、2D姿态判别器和3D姿态估计器。构建新颖的3D姿态生成器范式，整合姿态感知、时间感知和领域感知知识，增强当前领域适应并缓解对先前领域的灾难性遗忘。

Result: 在多种领域自适应3D HPE数据集上进行广泛实验，证明了该方法在适应当前领域姿态和保留先前领域知识方面的优越性能。

Conclusion: 首次将终身领域自适应引入3D人体姿态估计任务，提出的GAN框架有效解决了领域偏移和灾难性遗忘问题，为3D HPE在多样化真实场景中的应用提供了新思路。
...
Result: 综合实验结果表明，该模型在缺陷修复能力上优于现有开源方法，能够有效修复难以区分的电影缺陷。

Conclusion: HaineiFRDM框架利用扩散模型强大的内容理解能力，通过创新的分块策略和模块设计，实现了高质量的高分辨率电影修复，为开源电影修复提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 量化儿童颅骨发育和缝合线骨化对诊断和治疗颅骨生长障碍至关重要。CT虽有辐射风险，MRI虽无辐射但无法显示颅骨和缝合线，因此需要一种非侵入性方法来评估颅骨发育。

Method: 使用深度学习驱动的管道，将T1加权MRI转换为合成CT。采用领域特定的变分自编码器，预测详细的颅骨分割，生成缝合线概率热图，并从热图直接获得缝合线分割。

Result: 合成CT与真实CT的结构相似度达99%，Fréchet起始距离为1.01。颅骨分割在七个颅骨上平均Dice系数达85%，缝合线分割Dice系数达80%。通过TOST检验证实合成CT与真实CT在颅骨和缝合线分割上的等效性。

Conclusion: 这是首个能从MRI生成合成CT并实现缝合线分割的儿科颅骨CT合成框架，填补了非侵入性颅骨评估的关键空白，为无辐射的颅骨发育评估提供了新方法。
...
TL;DR: 提出MGML框架处理多模态MRI数据缺失问题，通过元参数化自适应模态融合和一致性正则化提升脑肿瘤分割性能，在BraTS数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 临床实践中多模态MRI数据往往不完整，如何最大化利用不完整的多模态信息是重要研究挑战，需要开发能有效处理缺失模态的分割方法。

Method: 提出元引导多模态学习框架，包含元参数化自适应模态融合和一致性正则化模块。Meta-AMF根据可用模态生成自适应软标签监督信号，促进更一致的多模态融合；一致性正则化增强分割性能并提高框架鲁棒性。

Result: 在BraTS2020和BraTS2023数据集上验证，在15种缺失模态组合的平均Dice分数上优于现有方法。BraTS2020上WT、TC、ET分别达到87.55、79.36、62.67。

Conclusion: MGML框架能有效处理多模态MRI数据缺失问题，无需改变原始模型架构，可方便集成到训练流程中，在脑肿瘤分割任务上表现出优越性能。
...
TL;DR: 提出基于AI的自动化动作分割与性能评估框架，用于神经外科微血管吻合手术培训，实现92.4%的动作分割准确率和85.5%的技能分类准确率。


<details>
  <summary>Details</summary>
Motivation: 微血管吻合手术需要精细操作技能，传统评估依赖专家主观评价，存在评分者间变异、不一致性和耗时等问题，需要自动化、可扩展的客观评估方案。

Method: 提出三模块AI框架：1)基于YOLO和DeepSORT的器械尖端跟踪定位；2)基于自相似矩阵的动作边界检测和无监督聚类的动作分割；3)用于评估手术手势熟练度的监督分类模块。

Result: 在58个专家评分的微血管吻合视频数据集上验证，实现帧级动作分割准确率92.4%，技能分类准确率85.5%，能够复现专家评估结果。

Conclusion: 该方法能为微外科教育提供客观、实时反馈，实现更标准化、数据驱动的培训协议，推进高风险手术环境中的能力评估。
...
TL;DR: 该研究提出了T2VAttack，首次系统性地从语义和时间两个维度对文本到视频扩散模型进行对抗攻击研究，揭示了即使微小提示修改也能显著降低视频质量。


<details>
  <summary>Details</summary>
Motivation: 尽管文本到视频扩散模型在生成高质量视频方面取得了显著进展，但其对抗攻击的脆弱性尚未得到充分探索。考虑到视频数据的动态特性，需要从语义和时间两个维度评估模型的鲁棒性。

Method: 提出了两种对抗攻击方法：T2VAttack-S通过贪婪搜索识别提示中的关键语义或时间词并用同义词替换；T2VAttack-I通过迭代插入优化词以最小化提示扰动。两种方法分别针对语义目标（评估视频-文本对齐）和时间目标（评估时间动态）。

Result: 在多个最先进的T2V模型（ModelScope、CogVideoX、Open-Sora、HunyuanVideo）上的实验表明，即使单个词的替换或插入也能导致语义保真度和时间动态的显著下降，揭示了当前T2V扩散模型的关键脆弱性。

Conclusion: 该研究首次全面探索了T2V扩散模型的对抗攻击脆弱性，提出的T2VAttack框架从语义和时间两个维度揭示了模型的安全漏洞，为未来开发更鲁棒的T2V模型提供了重要参考。
...
TL;DR: 提出了一种用于自动驾驶场景中视图外推的有效方法，仅需图像和可选相机位姿，无需昂贵的传感器或标注数据，通过4D高斯模型和视频扩散模型的迭代优化实现高质量新视图生成。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视图外推方法严重依赖激光雷达点云、3D边界框和车道标注等先验信息，需要昂贵传感器或大量人工标注，限制了在实际部署中的应用。本文旨在仅使用图像和可选相机位姿实现高质量的视图外推。

Method: 首先从图像和可选相机位姿估计全局静态点云和逐帧动态点云，融合为统一表示。然后采用可变形4D高斯框架重建场景。初始训练的4D高斯模型渲染降质伪图像来训练视频扩散模型。随后，逐步偏移的高斯渲染通过扩散模型迭代优化，增强结果作为训练数据反馈给4DGS，直到外推到目标视点。

Result: 与基线方法相比，该方法在新型外推视点上生成更高质量的图像，验证了仅使用图像和相机位姿实现有效视图外推的可行性。

Conclusion: 该方法通过4D高斯模型与视频扩散模型的协同优化，实现了无需昂贵传感器或标注数据的自动驾驶场景视图外推，为实际部署提供了更实用的解决方案。
...
TL;DR: 该论文提出了一种基于深度学习的卫星时间序列异常检测方法，通过训练一个修复模型来预测无变化情况下的地表状态，从而高灵敏度地检测地表变化。


<details>
  <summary>Details</summary>
Motivation: 卫星影像地表变化检测对于灾害响应和环境监测至关重要，但传统方法受大气噪声、季节变化和传感器伪影等因素影响，难以实现高灵敏度检测。

Method: 基于SATLAS基础模型构建修复模型，利用全球分布的Sentinel-2时间序列数据训练模型从前序观测预测最后一帧图像，通过预测与观测的差异检测地表异常。

Result: 在2023年土耳其-叙利亚地震序列中成功检测到Tepehan的断裂特征，检测灵敏度比传统方法（时间中值或Reed-Xiaoli异常检测器）高约3倍，具有更高的特异性和灵敏度。

Conclusion: 该方法为利用免费多光谱卫星数据实现自动化、全球尺度的地表变化监测提供了有效途径，能够检测传统方法难以发现的地表异常。
...
TL;DR: 提出GCA-ResUNet，一种高效医学图像分割框架，通过分组坐标注意力模块增强全局表示能力，在保持CNN效率优势的同时提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 现有U-Net等CNN方法在建模长距离上下文依赖方面存在局限，特别是在多器官场景和低对比度区域；而Transformer方法虽然能缓解这一问题，但需要更高计算资源和更多训练数据，不利于在资源受限的临床环境中部署。

Method: 提出轻量级即插即用的分组坐标注意力（GCA）模块，将通道上下文建模解耦为多个组以显式考虑通道间的语义异质性，并集成方向感知坐标编码以捕获水平和垂直轴上的结构化空间依赖关系。

Result: 在Synapse和ACDC基准测试中分别达到86.11%和92.64%的Dice分数，优于Swin-UNet和TransUNet等代表性CNN和Transformer方法，特别是在描绘具有复杂边界的小解剖结构方面表现一致提升。
...
TL;DR: GASeg：通过拓扑信息连接几何与外观的自监督语义分割框架，解决外观模糊性问题


<details>
  <summary>Details</summary>
Motivation: 自监督语义分割方法在面对外观模糊性时经常失败，原因是过度依赖不稳定的外观特征（如阴影、眩光、局部纹理）。需要利用更稳定的拓扑信息来连接外观和几何特征。

Method: 提出GASeg框架，核心是可微盒计数模块量化多尺度拓扑统计；引入拓扑增强策略模拟真实世界模糊性；使用GALoss多目标损失强制几何与外观特征的跨模态对齐。

Result: 在COCO-Stuff、Cityscapes、PASCAL等四个基准测试中达到最先进性能，验证了通过拓扑信息连接几何与外观方法的有效性。

Conclusion: 通过利用稳定的拓扑信息来桥接几何和外观特征，可以有效解决自监督语义分割中的外观模糊性问题，提升模型鲁棒性和性能。
...
TL;DR: 提出了一种从RPO图像序列中重建未知航天器3D结构的新方法，使用3D高斯泼溅表示，并通过整合太阳位置先验知识来改善光照条件下的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 在空间交会对接操作中，从图像序列恢复目标航天器的3D结构对导航和操作至关重要。传统3DGS方法假设静态光照，而空间环境中的动态光照条件会严重影响重建质量和后续的相机姿态估计精度。

Method: 使用3D高斯泼溅表示目标几何和外观，并整合服务航天器估计的太阳位置先验知识到训练流程中，使模型能够适应空间快速变化的光照条件，学习全局阴影和自遮挡效应。

Result: 实验研究表明，所提方法能有效提升3DGS模型在动态光照条件下的渲染质量，模型能够适应空间快速变化的光照，准确反映阴影和自遮挡效应，从而改善后续姿态估计任务的性能。

Conclusion: 通过整合太阳位置先验知识，提出的方法成功解决了空间动态光照条件下3D重建的挑战，为RPO过程中的精确姿态估计提供了更高质量的3D模型基础。
...
TL;DR: 提出CVC框架解决流式扩散编辑中的速度误差累积问题，通过双视角速度转换机制和基于经验贝叶斯推断的后验一致性更新，实现稳定可解释的潜在动态和忠实重建。
...
Method: 提出条件速度校正(CVC)框架，将流式编辑重新定义为由已知源先验驱动的分布转换问题。引入双视角速度转换机制，将潜在演化分解为结构保持分支和语义引导分支。基于经验贝叶斯推断和Tweedie校正进行后验一致性更新，补偿量化偏差。

Result: CVC产生稳定可解释的潜在动态，实现忠实重建和平滑局部语义转换。综合实验表明，CVC在多种任务中始终实现更优的保真度、更好的语义对齐和更可靠的编辑行为。

Conclusion: CVC通过理论驱动的速度校正机制，有效解决了流式扩散编辑中的轨迹漂移问题，为分布转换提供了稳定可靠的框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法主要关注服装细节的忠实渲染，但忽视了塑造整体风格的关键因素——服装合身度。合身度定义了服装与穿着者身体的贴合程度，是时尚设计的基本要素，对整体风格协调至关重要。
...
Result: FitControler能够与多种VTON模型协同工作，实现精确的合身度控制。引入了两种合身度一致性指标来评估生成结果，并通过大量实验验证了方法的有效性。

Conclusion: 该工作首次将合身度控制引入虚拟试穿领域，提出的FitControler插件能够无缝集成到现有VTON模型中，实现对服装合身度的定制化控制，填补了该领域的重要空白。
...
<details>
  <summary>Details</summary>
Motivation: 现有2D高斯泼溅方法在分配表示容量和参数精度时忽略了图像结构信息，导致在低比特率下的率失真效率受限。需要将图像结构与表示容量和量化精度显式耦合，同时保持原生解码速度。

Method: 1. 结构引导初始化：根据自然图像的空间结构先验分配2D高斯分布，实现局部化和语义有意义的分布；2. 量化感知微调：对协方差参数进行自适应位宽量化，复杂区域的小尺度高斯给予更高精度；3. 几何一致正则化：将高斯方向与局部梯度方向对齐以更好保留结构细节。

Result: 方法在保持超过1000 FPS解码速度的同时，显著提升了2DGS的表示能力和RD性能。相比基线GSImage，在Kodak数据集上BD-rate降低43.44%，在DIV2K数据集上降低29.91%。

Conclusion: 提出的结构引导分配原则成功将图像结构与表示容量和量化精度耦合，在保持快速解码的同时大幅提升了2D高斯泼溅的率失真效率，为紧凑图像表示提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在处理遥感图像时面临挑战，因为遥感图像与自然图像存在本质差异。现有遥感VLM难以提取细粒度视觉特征，并在深度语言处理过程中容易出现视觉遗忘问题。

Method: 提出MF-RSVLM模型，采用多特征融合方法：1）学习多尺度视觉表示，结合全局上下文与局部细节；2）采用循环视觉特征注入方案，确保语言模型在生成过程中保持对视觉证据的感知，减少视觉遗忘。
...
TL;DR: 提出基于强化学习的可训练各向异性扩散框架，通过深度Q学习选择扩散动作顺序，构建自适应图像结构的随机扩散过程，在去噪性能上超越传统扩散方法，与深度CNN方法竞争。
...
Method: 将去噪过程建模为一系列基础扩散动作序列，通过深度Q学习学习动作顺序，构建可训练的随机各向异性扩散过程，形成自适应图像结构的扩散算子。

Result: 该方法在三种常见噪声类型的去除任务中表现优异，超越了现有的扩散基方法，并与代表性的深度CNN方法具有竞争力。

Conclusion: 基于强化学习的可训练各向异性扩散框架能够有效提升图像去噪性能，通过学习的扩散动作序列构建自适应扩散过程，为传统扩散方法提供了新的发展方向。
...
<details>
  <summary>Details</summary>
Motivation: 细粒度遥感数据集常使用层次标签结构进行粗到细的物体区分，但将语义层次嵌入表示学习空间以提升检测性能仍具挑战。现有研究在不同层次应用监督对比学习，但忽视了两个关键问题：1) 标签层次中数据分布不平衡导致高频类别主导学习过程；2) 类别间语义关系学习干扰与类别无关的定位任务。

Method: 在DETR框架中提出平衡层次对比损失结合解耦学习策略。该损失引入可学习的类别原型，在每个层次上均衡不同类别贡献的梯度，确保每个层次类别在每个小批量中对损失计算贡献相等。解耦策略将DETR的对象查询分为分类和定位两组，实现任务特定的特征提取和优化。

Result: 在三个具有层次标注的细粒度数据集上的实验表明，该方法优于最先进的方法。

Conclusion: 提出的平衡层次对比损失和解耦学习策略有效解决了细粒度遥感检测中的层次标签不平衡和语义关系干扰问题，显著提升了检测性能。
...
Method: 1) 利用块级均值作为代表性token进行稀疏掩码预测；2) 实现时空感知的token排列；3) 针对视频生成场景引入首帧下沉机制。旨在开发在线自适应、硬件高效、低开销的稀疏注意力机制。

Result: RainFusion2.0能够实现80%的稀疏度，在保持视频质量的同时获得1.5-1.8倍的端到端加速。该方法在各种生成模型中都有效，并在不同硬件平台上验证了其泛化能力。

Conclusion: RainFusion2.0成功解决了现有稀疏注意力方法的局限性，提供了一种硬件高效、低开销的解决方案，能够加速视频和图像生成模型，并在多种硬件平台上保持鲁棒性能。
...
TL;DR: 论文提出Latent Motion Reasoning (LMR)框架，通过两阶段"思考-行动"决策过程解决文本到动作生成中的语义-运动阻抗失配问题，显著提升语义对齐和物理合理性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到动作生成方法将问题视为直接翻译，面临语义-运动阻抗失配的根本理论瓶颈：难以在单次映射中将语义密集的离散语言意图落地到运动密集的高频运动数据中。

Method: 提出Latent Motion Reasoning (LMR)框架，采用两阶段"思考-行动"决策过程。核心是新颖的双粒度分词器，将运动解耦为两个不同流形：用于规划全局拓扑的压缩语义推理潜空间，和用于保持物理保真度的高频执行潜空间。

Result: 在T2M-GPT（离散）和MotionStreamer（连续）两个代表性基线上实现LMR，实验显示在语义对齐和物理合理性方面均获得显著提升，验证了运动规划的最佳基底是学习的运动对齐概念空间而非自然语言。

Conclusion: 通过引入潜在系统2推理架构，将生成重新表述为两阶段决策过程，有效弥合了语言与物理之间的不可言说性鸿沟，为文本到动作生成提供了更优的理论框架。
...
TL;DR: 提出一种免训练的生成对抗攻击框架，通过扩散模型生成自然、场景一致的对抗物体，显著提升自动驾驶单目深度估计攻击的有效性、隐蔽性和物理部署能力。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计作为自动驾驶核心感知模块易受对抗攻击，现有物理攻击依赖纹理贴片，存在放置限制严格、真实性有限等问题，在复杂驾驶环境中效果受限。

Method: 提出免训练的生成对抗攻击框架，包含显著区域选择模块识别对深度估计影响最大的区域，以及雅可比向量积引导机制将对抗梯度引导至预训练扩散模型支持的更新方向，通过扩散条件生成过程生成自然场景一致的对抗物体。

Result: 广泛的数字和物理实验表明，该方法在有效性、隐蔽性和物理部署能力方面显著优于现有攻击方法，能够诱导显著的对抗深度偏移。

Conclusion: 该框架生成的物理可信对抗物体对自动驾驶安全评估具有重要实际意义，为深度估计系统的鲁棒性评估提供了新方法。
...
TL;DR: GeoBench是一个几何问题解决的分层基准测试，包含四个推理层级，通过六个形式验证的任务评估视觉语言模型在几何推理中的能力，发现推理模型表现随任务复杂度增加而下降。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在几何推理评估中存在三个主要问题：1）基于教科书的基准测试存在测试数据污染风险；2）过于关注最终答案而非推理过程；3）诊断粒度不足。需要更全面、分层的评估框架来系统评估几何问题解决能力。

Method: 提出GeoBench分层基准测试，包含四个推理层级：视觉感知、目标导向规划、严格定理应用、自我反思回溯。通过TrustGeoGen生成六个形式验证的任务，系统评估从属性提取到逻辑错误纠正的能力范围。
...
TL;DR: 本文针对文本到图像扩散模型在人类反馈强化学习中出现偏好模式崩溃问题，提出了D²-Align框架来维持生成多样性


<details>
  <summary>Details</summary>
Motivation: 现有基于人类反馈强化学习的文本到图像扩散模型虽然能在自动化奖励指标上获得高分，但往往导致偏好模式崩溃——模型收敛到狭窄的高分输出模式（如单一风格或普遍过曝），严重损害了生成多样性

Method: 提出方向性解耦对齐框架：1）在奖励模型的嵌入空间中学习方向性修正，同时保持模型冻结；2）在优化过程中将修正应用于奖励信号，防止模型塌陷到特定模式

Result: 通过结合质量与多样性定量指标的全面评估，D²-Align在保持生成多样性的同时实现了与人类偏好的更好对齐

Conclusion: 方向性解耦对齐框架有效缓解了偏好模式崩溃问题，在维持生成多样性的同时提升了与人类偏好的对齐质量
...
TL;DR: IMDD-1M是首个大规模工业多模态缺陷数据集，包含100万个对齐的图像-文本对，覆盖60多种材料类别和400多种缺陷类型。基于此数据集训练了面向工业场景的扩散式视觉-语言基础模型，通过轻量微调即可高效适应专业领域。
...
Method: 构建了IMDD-1M数据集，包含100万个对齐的图像-文本对，覆盖60多种材料类别和400多种缺陷类型，每个样本都有专家验证的注释和细粒度文本描述。基于该数据集从头训练了一个扩散式视觉-语言基础模型，专门针对工业场景设计。

Result: 该基础模型作为通用化基础，可以通过轻量微调高效适应专业领域。仅需不到5%的任务特定数据，就能达到专用专家模型相当的性能，展示了数据高效的基础模型适应在工业检测和生成中的潜力。

Conclusion: IMDD-1M数据集和相应的基础模型为可扩展、领域自适应和知识基础的制造智能铺平了道路，显著降低了工业场景中多模态AI应用的数据需求，提高了适应效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前的多模态大语言模型（MLLMs）在多模态推理方面取得了显著进展，但其推理过程仍以文本为中心，导致在复杂的长期、视觉中心任务中表现不佳。需要一种新的推理范式来更好地处理视觉信息。

Method: 提出DiffThinker，一个基于扩散模型的推理框架，将多模态推理重新定义为原生的图像到图像生成任务。该方法实现了四种核心特性：效率、可控性、原生并行性和协作性。
...
Conclusion: 生成式多模态推理是视觉中心推理的一个有前景的方法，DiffThinker通过将推理重新定义为图像到图像生成任务，在逻辑一致性和空间精度方面实现了优越性能。
...
TL;DR: DGC是一种内存高效的高光谱图像分割框架，通过局部块学习全局聚类结构，无需预训练，在消费级硬件上30分钟内完成训练，但存在优化不稳定性问题。
...
Method: 提出Deep Global Clustering（DGC）框架，通过重叠区域的小块操作来强制一致性，从局部块观察中学习全局聚类结构，无需预训练，保持恒定内存使用。

Result: 在叶片病害数据集上，DGC实现了背景-组织分离（平均IoU 0.925），并通过可导航的语义粒度展示了无监督病害检测能力。但框架存在优化不稳定性，源于多目标损失平衡问题。

Conclusion: DGC的设计理念有价值，但稳定实现需要动态损失平衡的原则性方法。该工作可作为知识框架，代码和数据已开源。
...
TL;DR: 本文提出了一种名为内部指导（IG）的简单有效策略，通过在训练过程中引入中间层的辅助监督，并在采样过程中外推中间层和深层输出来提升扩散模型的生成质量，显著改善了训练效率和生成性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在捕捉完整数据分布方面表现出强大能力，但由于缺乏足够的训练和数据来覆盖低概率区域，模型在这些区域生成高质量图像时会受到惩罚。现有的指导策略如CFG会导致样本过度简化或失真，而其他基于"坏版本"的指导方法则需要精心设计的退化策略、额外训练和采样步骤。

Method: 提出内部指导（IG）策略：1）在训练过程中引入中间层的辅助监督；2）在采样过程中外推中间层和深层输出以获得生成结果。这种方法简单且不需要额外的训练或复杂的退化策略。

Result: 在各种基准测试中取得了显著改进：在ImageNet 256x256上，SiT-XL/2+IG在80和800轮次分别达到FID=5.31和1.75；LightningDiT-XL/1+IG达到FID=1.34，大幅领先其他方法；与CFG结合后达到当前最先进的FID=1.19。

Conclusion: 内部指导（IG）是一种简单而有效的策略，能够显著提升扩散模型的训练效率和生成质量，在多个基准测试中取得了最先进的性能，为扩散模型的发展提供了新的方向。
...
Method: 提出CorGi框架：1）基于贡献度评估，缓存低贡献度的transformer块并在间隔内重用；2）对于文本到图像任务，提出CorGi+，利用跨注意力图识别重要token并应用部分注意力更新来保护物体细节。

Result: 在最新的DiT模型上评估显示，CorGi和CorGi+平均实现高达2.0倍的加速，同时保持高质量生成效果。

Conclusion: CorGi和CorGi+通过减少DiT模型中的冗余计算，有效实现了推理加速而不影响生成质量，为扩散变换器的高效部署提供了实用解决方案。
...
TL;DR: ARM是一个轻量级可学习模块，通过自适应融合CLIP的层次特征来提升开放词汇语义分割性能，采用"一次训练，随处使用"范式，在多个基准测试中显著提升基线性能。


<details>
  <summary>Details</summary>
Motivation: 现有训练免费方法要么依赖昂贵的外部基础模型（如SAM、DINO），要么使用静态启发式方法处理CLIP特征，这些方法要么计算成本高，要么效果不理想。CLIP的图像级表示缺乏像素级细节，限制了开放词汇语义分割的性能。

Method: 提出注意力精炼模块（ARM），包含语义引导的交叉注意力块（使用深层特征K、V选择和精炼细节丰富的浅层特征Q）和自注意力块。采用"一次训练，随处使用"范式，在通用数据集（如COCO-Stuff）上训练一次后，可作为通用即插即用后处理器用于各种训练免费框架。

Result: 在多个基准测试中，ARM能一致提升基线性能，且推理开销可忽略不计，为训练免费的开放词汇语义分割建立了高效有效的范式。

Conclusion: ARM通过自适应融合CLIP的层次特征，有效解锁和精炼了CLIP的内部潜力，为训练免费的开放词汇语义分割提供了一种轻量级、高效且通用的解决方案。
...
TL;DR: Mirage是一个用于驾驶场景资产编辑的一步视频扩散模型，通过结合2D和3D编码器特征以及两阶段数据对齐策略，实现了高真实感和时间一致性的视频编辑。


<details>
  <summary>Details</summary>
Motivation: 现有的视频对象编辑方法在保持高视觉保真度和时间一致性方面存在困难，而驾驶场景需要多样化和可扩展的训练数据来提升自动驾驶系统的鲁棒性能。

Method: 基于文本到视频扩散先验，引入时间无关的2D编码器潜在特征注入3D解码器以恢复细节；采用两阶段数据对齐策略（粗粒度3D对齐和细粒度2D精炼）来解决分布不匹配导致的姿态对齐问题。

Result: 大量实验表明，Mirage在多样化的编辑场景中实现了高真实感和时间一致性，并且能够泛化到其他视频到视频转换任务。

Conclusion: Mirage为驾驶场景的资产编辑提供了一个有效的解决方案，可作为未来研究的可靠基线，其代码已开源。
...
TL;DR: MambaSeg：一种新颖的双分支语义分割框架，使用并行Mamba编码器处理RGB图像和事件流，通过双维度交互模块实现空间和时间维度的细粒度融合，在保持高效计算的同时达到最先进的性能。
...
Method: 提出MambaSeg框架，采用并行Mamba编码器分别处理RGB图像和事件流。引入双维度交互模块（DDIM），包括跨空间交互模块（CSIM）和跨时间交互模块（CTIM），在空间和时间维度上进行细粒度融合，改善跨模态对齐并减少模糊性。

Result: 在DDD17和DSEC数据集上的广泛实验表明，MambaSeg实现了最先进的语义分割性能，同时显著降低了计算成本，展示了其在高效、可扩展和鲁棒的多模态感知方面的潜力。

Conclusion: MambaSeg通过有效融合RGB和事件数据的互补特性，在保持计算效率的同时提升了语义分割性能，为在挑战性视觉条件下的鲁棒感知提供了有前景的解决方案。
...
TL;DR: 提出PGMP框架解决牙科CBCT金属伪影问题，结合物理模拟生成高质量训练数据，使用确定性流形投影实现单次前向推理，并利用医学基础模型确保临床合理性。


<details>
  <summary>Details</summary>
Motivation: 牙科CBCT中的金属伪影严重遮挡解剖结构，影响诊断。现有深度学习方法存在局限：监督方法因"回归到均值"导致光谱模糊，无监督方法存在结构幻觉风险，而扩散模型虽然真实但依赖缓慢的随机迭代采样，不适合临床应用。

Method: 提出物理基础流形投影(PGMP)框架：1) 解剖自适应物理模拟(AAPS)通过蒙特卡洛光谱建模和患者特异性数字孪生合成高保真训练对；2) DMP-Former采用直接x预测范式，将恢复重新表述为确定性流形投影，单次前向恢复干净解剖结构；3) 语义结构对齐(SSA)模块利用医学基础模型(MedDINOv3)的先验锚定解决方案，确保临床合理性。

Result: 在合成和多中心临床数据集上的实验表明，PGMP在未见解剖结构上优于最先进方法，在效率和诊断可靠性方面设定了新的基准。

Conclusion: PGMP框架有效解决了牙科CBCT金属伪影减少的挑战，通过物理模拟、确定性流形投影和医学先验的整合，实现了高效且临床可靠的伪影去除。
...
TL;DR: 提出DualityForge框架，通过可控扩散视频编辑将真实视频转换为反事实场景，构建DualityVidQA数据集，并设计DNA-Train训练方法，显著减少多模态大语言模型在反事实视频上的幻觉问题。
...
Method: 1) DualityForge框架：使用可控扩散视频编辑将真实视频转换为反事实场景，嵌入结构化上下文信息自动生成高质量QA对和原始-编辑视频对；2) 构建DualityVidQA数据集；3) DNA-Train训练方法：两阶段SFT-RL训练，RL阶段应用成对ℓ1优势归一化实现更稳定高效的策略优化。

Result: 在DualityVidQA-Test上，方法相比Qwen2.5-VL-7B基线相对改进24.0%，显著减少反事实视频上的模型幻觉。在幻觉和通用基准测试中都取得显著提升，显示出强大的泛化能力。

Conclusion: DualityForge框架通过自动生成高质量反事实数据，结合DNA-Train训练方法，有效解决了MLLM在反事实视频上的幻觉问题，同时保持了在通用任务上的性能。将开源数据集和代码。
...
TL;DR: EndoRare是一个单样本、无需重新训练的生框架，能够从单张参考图像合成多样化的高保真罕见胃肠道病变图像，通过语言引导的概念解耦分离病理特征和非诊断属性，显著提升AI分类器性能和临床医生诊断能力。
...
Method: EndoRare采用语言引导的概念解耦方法，将病理特征与非诊断属性分离，将前者编码为可学习的原型嵌入，同时变化后者以确保多样性，实现从单张参考图像生成多样化高保真病变图像。

Result: 在四种罕见病理上验证，合成图像被专家评为临床可信，用于数据增强显著提升下游AI分类器性能，盲法读者研究显示新手内镜医生使用EndoRare生成病例后召回率提高0.400，精确率提高0.267。

Conclusion: EndoRare为计算机辅助诊断和临床教育中的罕见疾病差距提供了实用、数据高效的解决方案，能够同时提升AI模型性能和临床医生培训效果。
...
<details>
  <summary>Details</summary>
Motivation: 在低剂量CT肺癌筛查的深度学习流程中，稳健的预处理很少被量化评估。需要开发临床驱动的质量控制流程并评估其对不同类型模型的影响
...
Result: Virtual-Eyes将RAD-DINO切片级AUC从0.576提升至0.610，患者级AUC从0.646提升至0.683（平均池化）和从0.619提升至0.735（最大池化），校准性改善（Brier分数从0.188降至0.112）。但Sybil和ResNet-18在Virtual-Eyes下性能下降（Sybil AUC从0.886降至0.837；ResNet-18 AUC从0.571降至0.596），Merlin转移性有限（AUC约0.507-0.567）

Conclusion: 解剖学目标质量控制可以稳定和改进通用基础模型工作流程，但可能破坏适应原始临床环境的专用模型。预处理策略应根据模型类型进行定制
...
TL;DR: UniAct是一个两阶段框架，通过微调MLLM和因果流式管道，使仿人机器人能够以低于500毫秒的延迟执行多模态指令，在零样本跟踪不完美参考动作方面成功率提升19%。


<details>
  <summary>Details</summary>
Motivation: 仿人机器人领域长期目标是实现能够遵循多样化多模态指令的通用智能体，但现有方法难以将语言、音乐、轨迹等异构指令转化为稳定、实时的动作，高层多模态感知与全身执行之间存在显著瓶颈。

Method: UniAct采用两阶段框架：1) 微调的多模态大语言模型(MLLM)处理多模态指令；2) 因果流式管道生成实时动作。通过FSQ共享离散码本统一输入，确保跨模态对齐并将动作约束在物理可行流形上。

Result: 在UniMoCap（20小时仿人动作基准）上验证，实现了低于500毫秒的延迟，零样本跟踪不完美参考动作的成功率提升19%，展示了在多样化现实场景中的鲁棒泛化能力。

Conclusion: UniAct标志着向响应式通用仿人助手迈出了关键一步，通过统一的感知和控制实现了无缝交互，为解决高层多模态感知与全身执行之间的瓶颈提供了有效方案。
...
TL;DR: CERES是一个因果推理框架，通过双模态因果干预解决自我中心视频中语言偏见和视觉混淆问题，提升指代视频对象分割性能。


<details>
  <summary>Details</summary>
Motivation: 自我中心指代视频对象分割任务面临两大挑战：1）训练数据中存在的语言偏见和虚假相关性；2）自我中心视角特有的视觉混淆因素（如快速运动、频繁遮挡）。现有方法难以有效处理这些问题。

Method: CERES采用双模态因果干预：1）对语言模态应用后门调整原则，抵消从数据集统计中学习的语言表示偏见；2）对视觉模态应用前门调整概念，通过因果原则指导将语义视觉特征与几何深度信息智能集成，创建对自我中心失真更鲁棒的表征。
...
### [42] [SenseNova-MARS: Empowering Multimodal Agentic Reasoning and Search via Reinforcement Learning](https://arxiv.org/abs/2512.24330)
...
TL;DR: SenseNova-MARS是一个多模态智能体推理与搜索框架，通过强化学习赋予视觉语言模型交错的视觉推理和工具使用能力，在知识密集型视觉任务中实现动态工具调用。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在解决复杂任务时主要局限于文本导向的思维链或孤立工具调用，缺乏人类般的动态工具操作与连续推理交错能力，特别是在需要协调外部工具（如搜索和图像裁剪）的知识密集型和视觉复杂场景中。

Method: 提出SenseNova-MARS多模态智能体推理与搜索框架，通过强化学习赋予VLMs交错的视觉推理和工具使用能力。具体整合图像搜索、文本搜索和图像裁剪工具来处理细粒度和知识密集型的视觉理解挑战。在强化学习阶段，提出批量归一化组序列策略优化算法（BN-GSPO）来提高训练稳定性并增强模型调用工具和有效推理的能力。

Result: 实验表明SenseNova-MARS在开源搜索和细粒度图像理解基准测试中达到最先进性能。在搜索导向基准测试中，SenseNova-MARS-8B在MMSearch上得分67.84，在HR-MMSearch上得分41.64，超越了Gemini-3-Flash和GPT-5等专有模型。

Conclusion: SenseNova-MARS通过提供有效且鲁棒的工具使用能力，代表了向智能体视觉语言模型发展的有前景的一步。所有代码、模型和数据集将公开发布以促进该领域进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于视觉的视觉语言模型在自动驾驶中依赖于2D图像线索，缺乏准确的度量空间推理和几何推断能力，导致驾驶策略不可靠，存在安全和可靠性瓶颈。

Method: 提出LVLDrive框架，通过引入LiDAR点云作为额外输入模态；设计渐进融合Q-Former逐步注入LiDAR特征，确保预训练VLM知识库的稳定性；开发空间感知问答数据集来显式教授模型高级3D感知和推理能力。

Result: 在驾驶基准测试中，LVLDrive在场景理解、度量空间感知和可靠驾驶决策方面相比纯视觉方法表现出优越性能。

Conclusion: 研究表明，显式的3D度量数据对于构建可信赖的基于VLM的自动驾驶系统是必要的，LVLDrive通过融合LiDAR成功提升了VLM的3D空间理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有皮肤病图像分析数据集主要关注皮肤镜图像，缺乏患者自述查询和临床背景，限制了其在以患者为中心的护理中的应用。需要开发支持患者查询和临床上下文的数据集。

Method: 引入皮肤病评估框架(DAS)，包含36个高层次和27个细粒度评估问题，提供中英文多选项。基于DAS构建专家标注的数据集，用于封闭式问答和分割任务。评估多种多模态模型，包括不同的提示策略。

Result: 分割任务中，提示设计影响性能：默认提示在Mean-of-Max和Mean-of-Mean评估中表现最佳，而增强提示在多数投票评估中表现最好(Jaccard指数0.395，Dice分数0.566)。封闭式问答中，o3模型总体准确率最高(0.798)，GPT-4.1次之(0.796)，Gemini-1.5-Pro在Gemini家族中表现最佳(0.783)。

Conclusion: DermaVQA-DAS填补了现有皮肤病图像分析数据集的空白，提供了支持患者查询和临床背景的资源。研究展示了多模态模型在皮肤病分析中的潜力，并公开了数据集、DAS框架和评估协议以促进未来研究。
...
TL;DR: 该论文提出了一个用于多模态预训练的综合框架，旨在解决自动驾驶车辆和无人机等自主系统中从多模态传感器数据构建空间智能的挑战，建立了从单模态基线到统一框架的预训练范式分类体系。
...
Method: 论文提出了一个多模态预训练的综合框架，分析了基础传感器特性与学习策略之间的相互作用，评估了平台特定数据集的作用。核心贡献是建立了统一的预训练范式分类体系，从单模态基线到学习整体表示的复杂统一框架。同时研究了文本输入和占用表示的整合，以促进开放世界感知和规划。

Result: 论文系统地分析了多模态预训练的技术进展，识别了推动该领域发展的核心技术。建立了从单模态基线到统一框架的预训练范式分类体系，并探讨了文本输入和占用表示在开放世界感知中的应用。

Conclusion: 论文识别了计算效率和模型可扩展性等关键瓶颈，并提出了实现通用多模态基础模型的路线图，这些模型能够为实际部署实现稳健的空间智能。该框架为自主系统的多模态感知和理解提供了系统性的指导。
...
### [46] [RedunCut: Measurement-Driven Sampling and Accuracy Performance Modeling for Low-Cost Live Video Analytics](https://arxiv.org/abs/2512.24386)
...
<details>
  <summary>Details</summary>
Motivation: 实时视频分析在大量摄像头部署中计算成本高昂，现有动态模型大小选择方法无法适应多样化工作负载，特别是在移动视频和低精度目标场景下表现不佳

Method: RedunCut采用测量驱动的规划器来评估采样的成本效益权衡，并使用轻量级、数据驱动的性能模型来改进精度预测

Result: 在道路车辆、无人机和监控视频以及多种模型家族和任务中，RedunCut在固定精度下减少14-62%的计算成本，对有限历史数据和漂移保持鲁棒性

Conclusion: RedunCut通过改进采样策略和精度预测，成功解决了现有动态模型大小选择方法的局限性，为大规模视频分析提供了高效的计算解决方案
...
Method: 采用流友好的自回归框架，结合流匹配头部进行概率建模；提出由前瞻模块增强的因果编码器，整合短期未来上下文（60ms）以提升质量同时保持低延迟

Result: 每帧生成时间34ms，系统总延迟低于100ms；在HDTF数据集上获得离线8.13和在线7.61的唇同步置信度得分，达到最先进的唇同步质量

Conclusion: DyStream通过创新的因果编码器设计和流匹配自回归框架，实现了实时、高质量的对话头部视频生成，显著超越了其他因果策略
...
Method: 采用基于TimeSformer的视频Transformer架构，改进为分层时间注意力和加权空间注意力机制；结合YOLO目标检测追踪提取精细运动特征；从五个技能维度评估微血管吻合术表现

Result: 在58个专家标注视频数据集上验证，动作分割帧级准确率达87.7%，后处理后提升至93.62%；在复制专家评估方面，所有技能维度的平均分类准确率达76%

Conclusion: 该系统能够提供客观、一致、可解释的反馈，有望实现更标准化、数据驱动的手术培训评估，促进外科教育的可扩展性和一致性
...
TL;DR: 本文通过离散小波变换(DWT)获取视觉基元，分析Vision Transformer编码器表示中的组合性，发现单层DWT分解的基元在潜在空间中近似组合


<details>
  <summary>Details</summary>
Motivation: 虽然对Transformer模型工作原理的洞察主要来自语言任务分析，但本研究旨在通过组合性视角研究Vision Transformer编码器学习到的表示

Method: 引入一个框架，使用离散小波变换(DWT)获取视觉输入依赖的基元，通过检查组合表示重构原始图像表示的能力，实证测试表示空间中的组合性程度

Result: 研究发现，单层DWT分解得到的基元产生的编码器表示在潜在空间中近似组合，为ViT如何组织信息提供了新视角

Conclusion: Vision Transformer编码器表示展现出组合性特性，离散小波变换是获取视觉基元的有效工具，这为理解ViT信息结构提供了新见解
...
Method: 采用两个互补模块：1) 谱间窗口图嵌入(iSWGE)，将光谱通道表示为具有学习边缘特征的图节点来建模谱间关系；2) 窗口空间图注意力与卷积块注意力(WSGA-C)，结合稀疏图注意力和卷积注意力来减少空间冗余并强调精细结构

Result: 在SDOML数据集上评估，相比强基线方法，平均光谱信息散度降低20.15%，PSNR提升最高1.09%，对数变换MS-SSIM增益1.62%，在相同比特率下提供更清晰和光谱保真的重建
...
<details>
  <summary>Details</summary>
Motivation: 尽管AI在医学影像分析中取得了显著进展，但现有系统通常只输出结构化预测结果，而生成完整的叙述性放射学报告仍需放射科医生完成。本研究旨在利用大型语言模型（如GPT-4）来填补这一空白，将结构化检测结果转化为自然语言的诊断报告。

Method: 研究设计了一个集成管道：首先使用YOLOv5和YOLOv8模型在胸部X光图像中进行异常检测，生成边界框预测和类别标签；然后将这些结构化结果输入大型语言模型（LLM），由LLM生成描述性发现和临床总结。比较了YOLOv5和YOLOv8在检测精度和推理延迟方面的性能，并评估了GPT-4生成文本的质量。

Result: 结果显示，AI生成报告与人工报告在语义相似性方面表现良好。人类评估表明，GPT-4在清晰度方面得分很高（4.88/5），但在自然写作流畅性方面得分较低（2.81/5）。这表明当前系统能够达到临床准确性，但在文体风格上与放射科医生撰写的文本仍有区别。

Conclusion: 该研究成功展示了结合目标检测模型和大型语言模型生成放射学报告的可行性。虽然生成的报告在临床准确性方面表现良好，但在自然语言流畅性和文体风格方面仍需改进，以更接近放射科医生的专业写作水平。
...
TL;DR: 提出PhyGDPO框架，通过物理增强数据管道和物理感知优化方法，解决文本到视频生成中物理一致性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成方法虽然在视觉质量上有所提升，但难以生成符合物理规律的视频。基于图形或提示扩展的方法难以泛化到复杂场景，且缺乏包含丰富物理交互的训练数据。

Method: 1. 提出PhyAugPipe物理增强数据管道，利用视觉语言模型和思维链推理构建大规模训练数据集PhyVidGen-135K；2. 设计PhyGDPO框架，基于分组Plackett-Luce概率模型进行物理感知优化；3. 引入物理引导奖励方案PGR，嵌入VLM物理奖励；4. 提出LoRA-Switch Reference方案提高训练效率。

Result: 在PhyGenBench和VideoPhy2基准测试中显著优于现有开源方法，能够生成更符合物理规律的视频。

Conclusion: 通过构建物理增强数据集和设计物理感知优化框架，有效提升了文本到视频生成的物理一致性，为物理可信的视频生成提供了新思路。
...
TL;DR: DARFT框架通过决策模糊性引导的强化微调，专门针对CDVQA中决策模糊样本进行优化，提升模型判别能力


<details>
  <summary>Details</summary>
Motivation: 现有CDVQA模型在监督微调后仍存在大量决策模糊失败案例，即模型对正确答案和强干扰项赋予相似置信度，需要专门解决这类决策模糊样本

Method: 提出DARFT框架：1）使用SFT训练好的参考策略挖掘决策模糊样本；2）在挖掘的子集上应用组相对策略优化，利用多样本解码和组内相对优势抑制强干扰项

Result: 实验显示DARFT在SFT基线上获得一致性能提升，特别是在少样本设置下表现优异
...
TL;DR: SliceLens：一个基于LLM/VLM的假设驱动框架，通过视觉推理发现细粒度错误切片，在首个面向实例级视觉任务的FeSD基准上表现优异


<details>
  <summary>Details</summary>
Motivation: 现有切片发现方法主要针对图像分类任务，难以应用于检测、分割、姿态估计等多实例任务；真实场景中的错误切片常涉及复杂视觉关系，现有方法缺乏细粒度推理能力；当前基准测试偏向特定算法或图像分类，人工标注无法反映真实模型失败情况

Method: 提出SliceLens框架，利用大型语言模型和视觉语言模型生成和验证多样化失败假设，通过基于视觉的推理实现细粒度、可解释的错误切片识别

Result: 在现有基准和新提出的FeSD基准上均达到最先进性能，在FeSD上将Precision@10从0.31提升到0.73（提升0.42）；识别出的可解释切片有助于实际模型改进，模型修复实验验证了其有效性

Conclusion: SliceLens通过假设驱动的视觉推理方法，有效解决了实例级视觉任务中细粒度错误切片发现的挑战，为模型评估和修复提供了可靠工具
...
TL;DR: EVOL-SAM3：一种零样本推理分割框架，通过推理时进化搜索过程替代传统静态推理范式，显著超越现有方法
...
Method: 提出EVOL-SAM3框架，将推理分割重构为推理时进化搜索过程：维护提示假设种群，通过"生成-评估-进化"循环迭代优化；引入视觉竞技场进行无参考成对竞赛评估提示适应性；设计语义突变算子注入多样性和纠正语义错误；异构竞技场模块整合几何先验与语义推理确保最终选择鲁棒性

Result: 在挑战性的ReasonSeg基准测试中，EVOL-SAM3不仅大幅超越静态基线方法，还在零样本设置下显著超过完全监督的最先进方法

Conclusion: EVOL-SAM3通过进化搜索范式成功解决了当前推理分割方法的局限性，为复杂语境依赖的语言查询到像素级定位提供了更强大、更鲁棒的解决方案
...
TL;DR: 本文针对自然灾害后评估需求，构建了首个基于无人机影像的飓风灾后3D点云数据集，并测试了现有SOTA 3D语义分割模型，发现其在灾后场景中存在显著局限性。
...
Method: 使用无人机采集飓风伊恩（2022）灾区的航空影像，通过运动恢复结构（SfM）和多视角立体（MVS）技术重建3D点云，构建专门的数据集。在此基础上评估了Fast Point Transformer（FPT）、Point Transformer v3（PTv3）和OA-CNNs等当前最先进的3D语义分割模型。

Result: 测试发现现有SOTA模型在灾后3D场景理解中存在显著局限性，表明当前方法无法有效处理灾后环境的特殊挑战。
...
TL;DR: CPJ是一个无需训练的少样本框架，通过结构化图像描述增强农业病虫害视觉问答，在领域转移下显著提升诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有作物病害诊断方法依赖昂贵的监督微调，且在领域转移下表现不佳，需要更准确、可解释且无需训练的解决方案。
...
TL;DR: CLoRA是一种新颖的视觉Transformer微调方法，通过基础空间共享和样本无关多样性增强组件，在保持参数效率的同时提升学习性能，在图像和点云数据集上取得了优于现有方法的平衡。
...
Result: 在广泛使用的图像和点云数据集上进行实验，结果表明CLoRA在学习性能和参数效率之间取得了更好的平衡，同时在点云分析中需要最少的GFLOPs，优于现有最先进方法。

Conclusion: CLoRA通过基础空间共享和多样性增强机制，有效解决了现有LoRA方法在参数效率和学习性能之间的权衡问题，为视觉Transformer微调提供了一种更平衡的解决方案。
...
### [60] [Video and Language Alignment in 2D Systems for 3D Multi-object Scenes with Multi-Information Derivative-Free Control](https://arxiv.org/abs/2512.24826)
*Jason Armitage,Rico Sennnrich*

Main category: cs.CV

TL;DR: 提出一种通过遗憾最小化和无导数优化改进多变量互信息估计的新方法，使现成的2D跨模态系统能在线适应3D场景中的物体遮挡和特征区分
...
Method: 通过遗憾最小化和无导数优化改进多变量互信息估计，结合表达性度量和基于价值的优化来控制场景内相机，直接从视觉语言模型的噪声输出中学习

Result: 该方法使现成的2D跨模态系统能够在线适应物体遮挡和区分特征，在多物体3D场景的跨模态任务中提升性能，无需预训练或微调

Conclusion: 提出的算法有效解决了2D到3D的维度转换问题，通过改进的互信息估计和相机控制策略，实现了跨模态系统在3D场景中的自适应性能提升
...
TL;DR: 本文提出了面向户外监控场景的3D视觉定位新任务，构建了首个真实世界大规模多模态数据集MoniRefer，并提出了端到端方法Moni3DVG，在复杂交通场景中实现基础设施级别的3D视觉理解。
...
Conclusion: 本文填补了户外监控场景3D视觉定位的研究空白，提出的数据集和方法为基础设施级别的交通场景理解提供了重要基础，代码和数据集将公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 现有消防救援目标检测研究存在两个主要局限：1）主要关注山区或森林环境，对更频繁且结构复杂的城市救援场景关注不足；2）现有检测系统类别有限（如火焰、烟雾），缺乏覆盖消防车、消防员等指挥决策关键目标的全面系统。

Method: 1）构建新的FireRescue数据集，覆盖城市、山区、森林和水域多种救援场景，包含消防车、消防员等8个关键类别，共15,980张图像和32,000个边界框；2）提出FRS-YOLO改进模型，引入即插即用的多维协同增强注意力模块通过跨维度特征交互增强易混淆类别的判别表示，集成动态特征采样器强化高响应前景特征以减轻烟雾遮挡和背景干扰。

Result: 实验结果表明，消防救援场景中的目标检测具有高度挑战性，所提方法有效提升了YOLO系列模型在该场景下的检测性能。

Conclusion: 本文通过构建全面的消防救援数据集和提出改进的FRS-YOLO模型，解决了现有研究在场景覆盖和类别完整性方面的不足，为消防救援指挥决策提供了更有效的目标检测解决方案。
...
Method: 1. 采用径向拓扑组织生成过程：选择初始token作为中心点，将所有其他token按空间距离分组到多个同心环中；2. 环状生成：从内环到外环并行预测同一环内的所有token；3. 嵌套注意力机制：动态修正前向传播中的不合理输出，减轻错误累积。

Result: RadAR框架在保持自回归模型表示能力的同时，显著提高了视觉生成的推理效率，通过并行化预测减少了传统顺序解码的时间开销。

Conclusion: 通过结合径向并行预测和动态输出修正，RadAR为自回归视觉生成提供了一种高效且可扩展的解决方案，平衡了生成质量与推理效率。
...
TL;DR: EchoFoley：视频声音生成新任务，通过符号化声音事件实现细粒度控制，构建EchoFoley-6k数据集，提出EchoVidia框架，在可控性和感知质量上显著超越现有VT2A模型


<details>
  <summary>Details</summary>
Motivation: 当前视频-文本到音频（VT2A）方法存在三个关键限制：1）视觉与文本条件不平衡导致视觉主导；2）缺乏细粒度可控生成的具体定义；3）指令理解能力弱，现有数据集依赖简短分类标签。需要解决这些限制以实现更好的视频声音生成。

Method: 提出EchoFoley新任务，采用符号化表示来指定声音事件的时间、内容和方式。构建EchoFoley-6k大规模专家标注数据集（6000+视频-指令-标注三元组）。提出EchoVidia框架，采用以声音事件为中心的智能生成框架和慢-快思考策略。

Result: 实验表明，EchoVidia在可控性上比最近的VT2A模型提升40.7%，在感知质量上提升12.5%。实现了事件级别的局部控制和分层语义控制。

Conclusion: EchoFoley任务和EchoVidia框架有效解决了现有VT2A方法的局限性，通过符号化声音事件表示和专家标注数据集，显著提升了视频声音生成的可控性和质量。
...
### [65] [DarkEQA: Benchmarking Vision-Language Models for Embodied Question Answering in Low-Light Indoor Environments](https://arxiv.org/abs/2512.24985)
*Yohan Park,Hyunwoo Ha,Wonjun Jo,Tae-Hyun Oh*

Main category: cs.CV

TL;DR: DarkEQA是一个开源基准测试，用于评估视觉语言模型在低光照条件下的性能，通过模拟物理真实的视觉退化来测试EQA相关感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要在理想光照条件下评估视觉语言模型，但实际应用中需要模型在包括低光照在内的各种视觉退化条件下稳定工作，这一需求尚未得到充分研究。

Method: DarkEQA基准通过在线性RAW空间中模拟物理真实的照明下降和传感器噪声，然后经过ISP启发的渲染流程，创建多级低光照条件下的视觉退化场景，评估模型在自我中心视角下的问答能力。
...
TL;DR: Splatwizard是一个专门用于评估3D高斯泼溅（3DGS）压缩模型的统一基准工具包，旨在解决现有评估工具缺乏标准化和全面性指标的问题。


<details>
  <summary>Details</summary>
Motivation: 3DGS技术的快速发展导致对标准化评估工具的迫切需求，特别是在压缩任务方面。现有基准测试往往缺乏全面评估不同方法独特特性（如渲染速度、率失真权衡、内存效率和几何精度）的特定指标。

Method: 开发了Splatwizard工具包，提供易于使用的框架来实现新的3DGS压缩模型并利用现有最先进技术。包含自动化计算关键性能指标的集成流水线，如图像质量指标、重建网格的倒角距离、渲染帧率和计算资源消耗。

Result: Splatwizard为3DGS压缩模型提供了一个统一的基准测试工具包，使研究人员能够更全面、标准化地评估不同方法的性能表现。
...
TL;DR: 该论文提出了一种统一框架，将3D高斯泼溅中的特征嵌入学习和实例分割标签解码合并为一个步骤，通过引入"嵌入到标签"过程和边界硬挖掘技术，解决了多视图2D分割标签不一致的问题，提高了3D实例分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有的3D实例分割方法面临两个主要问题：1）多视图2D实例标签在不同视角间存在不一致性，导致3D预测质量差；2）现有方法通常采用两阶段方法，要么依赖对比学习和超参数敏感的聚类，要么需要预处理标签以确保一致性。这些方法训练时间长且性能有限。

Method: 提出统一框架，将特征嵌入学习和标签解码合并：1）为高斯基元引入可学习的特征嵌入用于分割；2）通过新颖的"嵌入到标签"过程高效解码为实例标签；3）针对边界伪影问题，提出边界硬挖掘技术，但直接应用于特征嵌入不稳定，因此在线性层处理后计算三元组损失以稳定训练。

Result: 在ScanNet、Replica3D和Messy-Rooms数据集上，该方法在定性和定量评估中均优于基线方法，减少了训练时间并提高了分割性能。

Conclusion: 该统一框架通过合并特征嵌入学习和标签解码步骤，有效解决了多视图2D标签不一致问题，并通过边界硬挖掘技术改善了边界分割质量，为3D高斯泼溅场景理解提供了更高效的实例分割解决方案。
...
TL;DR: 该论文提出了一种改进Noise2Noise训练的方法，允许在噪声目标图像上应用特定非线性函数而不引入显著偏差，解决了HDR图像去噪中训练过程被异常值干扰的问题。


<details>
  <summary>Details</summary>
Motivation: Noise2Noise方法虽然可以仅使用噪声图像对进行去噪器训练，但存在一个主要限制：对噪声目标应用非线性函数会导致结果偏差。由于非线性函数在图像处理中很常见，这限制了可对噪声目标进行的预处理类型。特别是在高动态范围(HDR)图像去噪中，训练过程容易被异常值干扰，而常用的非线性色调映射方法被认为与Noise2Noise训练不兼容。

Method: 论文开发了一个理论框架来分析非线性函数的影响，并描述了一类具有最小偏差的非线性函数。作者发现某些非线性函数可以在噪声目标上应用而不引入显著偏差。针对HDR图像去噪问题，论文研究了特定的损失函数和色调映射函数组合，这些组合可以减少异常值影响同时引入最小偏差。该方法应用于现有的基于机器学习的蒙特卡洛去噪器。

Result: 该方法在蒙特卡洛渲染产生的高动态范围图像去噪上进行了验证。结果显示，使用仅含噪声的训练数据，该方法能够接近原始实现（使用高样本数参考图像训练）的性能，成功解决了HDR图像训练中异常值干扰的问题。

Conclusion: 论文表明，通过仔细选择非线性函数和损失函数的组合，可以在Noise2Noise训练中应用非线性预处理而不引入显著偏差。这扩展了Noise2Noise方法的适用性，特别是在处理高动态范围图像时，使得仅使用噪声训练数据就能获得接近使用干净参考图像训练的性能。
...
### [69] [SpaceTimePilot: Generative Rendering of Dynamic Scenes Across Space and Time](https://arxiv.org/abs/2512.25075)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型难以独立控制场景的空间视角和时间动态，缺乏对空间和时间维度的解耦控制能力，限制了视频内容的灵活编辑和探索。

Method: 1. 引入动画时间嵌入机制，在扩散过程中显式控制输出视频的运动序列；2. 提出时间扭曲训练方案，利用现有多视角数据集模拟时间差异；3. 改进相机条件机制，允许从第一帧开始改变相机视角；4. 创建CamxTime合成数据集，提供完整的空间-时间视频轨迹。

Result: 在真实世界和合成数据上的评估表明，SpaceTimePilot能够实现清晰的空间-时间解耦，相比先前工作取得更强效果，支持连续、任意的空间和时间探索。

Conclusion: SpaceTimePilot通过创新的时间嵌入机制、训练方案和数据集，成功实现了视频生成中空间和时间的解耦控制，为动态场景的可控渲染提供了新方法。
...
TL;DR: CropTrack是一个用于农业场景的多目标跟踪框架，通过结合外观和运动信息解决重复模式、相似外观、光照变化和频繁遮挡等问题，在身份保持方面优于传统运动跟踪方法。


<details>
  <summary>Details</summary>
Motivation: 农业环境中的多目标跟踪面临重复模式、相似外观、突然光照变化和频繁遮挡等挑战。现有跟踪器主要依赖运动信息，但在目标频繁强遮挡时难以保持身份，而外观相似性又使得外观关联难以整合。

Method: 提出CropTrack框架，结合外观和运动信息，包括：重新排序增强的外观关联、一对多关联配合外观冲突解决策略、指数移动平均原型特征库来改进外观关联。

Result: 在公开农业MOT数据集上评估，CropTrack表现出持续的身份保持能力，优于传统运动跟踪方法。相比现有技术，在识别F1和关联准确率方面有显著提升，身份切换次数更低。

Conclusion: CropTrack通过有效整合外观和运动信息，解决了农业环境中多目标跟踪的身份保持问题，为农业场景提供了更可靠的目标跟踪解决方案。
...
TL;DR: 本文提出了VLN-MME评估框架，用于在视觉语言导航任务中评估多模态大语言模型的零样本智能体能力，发现增强推理链和自反思反而导致性能下降，揭示了MLLM在具身导航中的上下文感知和空间推理局限性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现出色，但作为具身智能体（需要多轮对话、空间推理和序列动作预测）的性能仍需探索。本文旨在通过统一的评估框架来探究MLLM在视觉语言导航任务中的零样本智能体潜力。

Method: 提出了VLN-MME评估框架，将传统导航数据集转化为标准化基准。采用高度模块化和易访问的设计简化评估，支持跨不同MLLM架构、智能体设计和导航任务的结构化比较和组件级消融实验。

Result: 研究发现，在基线智能体基础上增强推理链和自反思反而导致性能下降。这表明MLLM在具身导航任务中表现出较差的上下文感知能力：虽然能遵循指令并结构化输出，但其3D空间推理保真度较低。
...
TL;DR: FinMMDocR是一个用于评估多模态大语言模型在真实世界金融数值推理任务上的双语多模态基准，包含1200个专家标注问题，涵盖12种金融场景和9类金融文档，平均需要11步推理，最佳模型准确率仅58.0%


<details>
  <summary>Details</summary>
Motivation: 现有基准在金融领域的多模态推理评估存在不足，缺乏真实世界金融场景的复杂性、专业文档的深度理解要求以及多步骤数值推理能力。需要建立一个更贴近实际金融应用场景的评估基准来推动MLLM在复杂多模态推理任务上的进步。

Method: 构建了包含1200个专家标注问题的双语多模态基准，其中57.9%的问题融入了12种隐含金融场景，使用837份中英文金融文档（平均50.8页），问题平均需要11步推理（5.3步信息提取+5.7步计算），65.0%的问题需要跨页证据（平均2.4页）。

Result: 最佳性能的MLLM在FinMMDocR基准上仅达到58.0%的准确率，不同的检索增强生成方法在该任务上表现出显著性能差异，表明现有模型在处理复杂金融多模态推理任务方面仍有很大提升空间。

Conclusion: FinMMDocR基准通过引入真实金融场景、复杂文档理解和多步骤数值推理要求，为评估和推动MLLM在复杂多模态推理任务上的进步提供了重要工具，有望促进金融领域多模态人工智能技术的发展。
...
TL;DR: 提出基于神经元激活模式的LiDAR域自适应方法，仅需标注少量有代表性的目标域样本即可达到SOTA性能，结合持续学习技术防止权重漂移


<details>
  <summary>Details</summary>
Motivation: 3D目标检测器在自动驾驶感知系统中表现优异，但在不同地理区域间泛化能力差（如美国训练的模型在亚洲或欧洲表现不佳），需要有效的域自适应方法

Method: 基于神经元激活模式的LiDAR域自适应方法，通过正确选择少量代表性、多样化的目标域样本进行标注，结合持续学习启发的后训练技术防止权重漂移

Result: 该方法在实证评估中优于线性探测和最先进的域自适应技术，仅需极小的标注预算

Conclusion: 通过神经元激活模式选择少量代表性样本进行标注，结合持续学习技术，可以实现高效的LiDAR域自适应，显著提升模型在不同地理区域的泛化能力
...
Method: 提出ProDM（Property-aware Progressive Correction Diffusion Model）生成扩散框架，包含三个关键组件：1）CAC运动模拟数据引擎，从心脏门控CT合成具有多样化运动轨迹的非门控采集数据；2）属性感知学习策略，通过可微钙化一致性损失融入钙化特异性先验；3）渐进校正方案，在扩散步骤中逐步减少伪影。

Result: 在真实患者数据集上的实验表明，ProDM显著提高了CAC评分准确性、空间病灶保真度和风险分层性能。在真实非门控扫描上的读者研究进一步证实ProDM能抑制运动伪影并提高临床可用性。

Conclusion: 这些发现突出了渐进式、属性感知框架在从常规胸部CT成像中进行可靠CAC量化方面的潜力，为非门控CT的临床应用提供了有效解决方案。
...
TL;DR: VIPER：首个面向生成式视频推理的过程感知评估基准，包含16个任务和新的POC@r指标，发现当前视频模型仅有约20%的过程-结果一致性，存在显著的结果作弊问题。
...
Method: 提出过程感知评估范式，构建VIPER基准（包含16个跨时间、结构、符号、空间、物理和规划推理的任务），并设计POC@r指标，使用VLM-as-Judge和分层评分标准来评估中间步骤有效性和最终结果。

Result: 实验显示最先进的视频模型仅达到约20%的POC@1.0一致性，存在显著的结果作弊现象。测试时扩展和采样鲁棒性分析揭示了当前视频生成与真正广义视觉推理之间的巨大差距。

Conclusion: 需要过程感知评估来准确衡量生成式视频推理能力，VIPER基准为这一领域提供了系统性的评估工具，揭示了当前模型的局限性。
...
TL;DR: 本文提出了一种无需重新索引历史图库图像的双向连续兼容表示框架，用于解决终身行人重识别中的特征兼容性问题。


<details>
  <summary>Details</summary>
Motivation: 现有终身行人重识别方法需要为历史图库图像重新提取特征（重新索引），但这面临数据隐私问题和高昂的计算成本。这导致更新模型提取的查询特征与旧模型提取的图库特征不兼容，严重影响重识别性能。

Method: 提出双向连续兼容表示框架，持续更新旧模型提取的图库特征，以兼容方式实现高效的终身行人重识别。

Result: 通过理论分析和多个基准测试的广泛实验验证，该方法在提出的RFL-ReID任务和传统L-ReID任务上均取得领先性能。

Conclusion: 本文提出的Bi-C2R框架成功解决了无需重新索引的终身行人重识别问题，实现了新旧模型特征之间的兼容性，为实际应用提供了更实用的解决方案。
...
Method: 提出自引导框架：1) 使用Diffusion Transformer作为数据生成器，为每个真实样本合成唇部改变的伴生视频，形成视觉对齐的视频对；2) 训练基于DiT的音频驱动编辑器，利用完整对齐的输入视频帧专注于精确的音频驱动唇部修改；3) 引入时间步自适应多阶段学习策略，解耦不同扩散时间步的编辑目标冲突

Result: 方法实现了高度准确的唇部同步、忠实的身份保持，并在具有挑战性的野外场景中表现出卓越的鲁棒性。同时提出了ContextDubBench基准数据集用于多样化和挑战性实际应用场景的评估

Conclusion: 通过将视觉配音重构为条件良好的视频编辑问题，并利用完整对齐的视觉上下文，该方法在唇部同步精度、身份保持和鲁棒性方面取得了显著改进，为实际应用提供了更可靠的解决方案
...
Method: 1) 使用上下文感知补全和多样化时间掩码从损坏输入中恢复基础骨架序列；2) 骨架空间分解模块将骨架分为五个语义区域，基于运动方差进一步分为动态和静态子组，通过针对性扰动生成两个增强骨架序列；3) 物理驱动估计模块利用拉格朗日动力学估计关节加速度；4) 融合骨架位置序列和加速度序列输入GCN动作识别头。
...
Conclusion: FineTec通过统一的框架有效处理时间损坏的骨架序列，展示了在细粒度动作识别任务中的鲁棒性和泛化能力，为真实世界应用提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散方法在稀疏视图3D重建中存在三个关键问题：已知视角外围覆盖不足、生成视图间的几何不一致性、计算流程昂贵

Method: 提出GaMO框架，通过多视角外推从现有相机姿态扩展视野，采用多视角条件和几何感知去噪策略，无需训练实现零样本处理

Result: 在Replica和ScanNet++数据集上，3、6、9个输入视图下均达到SOTA重建质量，PSNR和LPIPS指标优于先前方法，相比SOTA扩散方法实现25倍加速，处理时间低于10分钟

Conclusion: GaMO通过多视角外推范式有效解决了稀疏视图重建中的覆盖范围、几何一致性和计算效率问题，为高质量3D场景重建提供了高效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在英语到代码生成方面表现出色，但这种进展尚未扩展到低资源语言。针对孟加拉语到Python代码生成问题，需要开发能够处理低资源语言的有效方法
...
Conclusion: BanglaCodeAct为低资源语言的可靠代码生成提供了有效解决方案，展示了基于智能体推理在低资源语言代码生成中的潜力，为孟加拉语到Python翻译设立了新标准
...
<details>
  <summary>Details</summary>
Motivation: 为安全关键的药品领域文档理解建立一个可控、可复现的基准测试，测试预训练文本布局模型在噪声OCR和异构模板下的鲁棒性。

Method: 创建PharmaShip数据集，包含三个互补任务：序列实体识别、关系抽取和阅读顺序预测。采用实体中心的评估协议，对五个代表性基线模型进行基准测试，包括像素感知和几何感知家族模型及其RORE增强变体。

Result: 像素和显式几何信息提供互补的归纳偏置，但单独使用都不够充分；注入阅读顺序导向的正则化能持续改进SER和EL；更长的位置覆盖稳定了页面后期预测并减少了截断伪影；ROP在词级别准确但在段落级别具有挑战性。

Conclusion: PharmaShip为药品领域的安全关键文档理解建立了可控、可复现的基准，并突显了序列感知约束作为结构建模的可迁移偏置的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 研究大语言模型中噪声驱动的人格涌现现象，探索外部噪声如何影响语言模型的反射生成动力学，为研究LLMs中的反射生成、涌现行为和长程语言连贯性提供可重复方法。

Method: 开发LN-RP计算框架，向LLM初始生成状态注入随机噪声种子，通过152个生成周期观察语言行为的非线性转变，分析噪声诱导的相变现象。

Result: 发现了三种具有不同熵特征的稳定人格模式，外部噪声源能可靠地诱导反射生成动力学的相变，定量评估确认了人格保持的一致性以及不同模式间的显著差异(p < 0.01)。

Conclusion: LN-RP协议为研究大语言模型中的反射生成、涌现行为和长程语言连贯性提供了可重复的方法，揭示了噪声在诱导人格相变中的重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM安全机制主要关注明显有害内容，但用户可以通过隐蔽的重新表述来伪装恶意意图，现有安全训练数据存在显著缺口。

Method: 引入HarmTransform多智能体辩论框架，通过多个智能体之间的迭代批评和精炼，系统地将有害查询转化为更隐蔽的形式，同时保留其底层恶意意图。

Result: 实验表明HarmTransform在生成有效查询转换方面显著优于标准基线方法。同时分析发现辩论具有双重作用：既能提升转换的隐蔽性，也可能引入主题偏移和不必要的复杂性。

Conclusion: 多智能体辩论在生成全面安全训练数据方面既有潜力也有局限性，需要平衡转换效果与复杂性控制。
...
TL;DR: LLMs在德州扑克不完全信息环境中能自发学习确定性结构（如手牌等级）和随机特征（如胜率），形成与理论信念状态相关的内部表征。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明LLMs在完全信息游戏中能发展出涌现的世界模型，本文旨在将这一研究扩展到不完全信息领域，以德州扑克作为典型的部分可观察马尔可夫决策过程。

Method: 在扑克手牌历史数据上预训练GPT风格模型，然后探测其内部激活，主要使用非线性探针来分析模型学习的表征。

Result: 模型在没有明确指导的情况下学习了确定性结构（如手牌等级）和随机特征（如胜率），这些表征是可解码的，并且与理论信念状态相关。

Conclusion: LLMs能够在德州扑克的随机环境中学习自己的表征，这表明它们能够在不完全信息领域中发展出内部世界模型。
...
<details>
  <summary>Details</summary>
Motivation: 传统推测解码方法中，草稿模型与目标模型过度对齐，限制了推测解码只能达到目标模型的性能上限。需要一种方法能够突破这一限制，让推测解码有可能超越目标模型本身的性能。

Method: EASD在标准推测解码基础上引入动态熵惩罚机制：1）计算采样分布的熵来量化模型不确定性；2）当草稿模型和目标模型都表现出高熵且top-N预测有显著重叠时，拒绝对应token；3）让目标LLM重新采样，防止低置信度错误传播。

Result: 在多个推理基准测试中，EASD始终优于现有的推测解码方法，并且在大多数情况下超越了目标LLM本身的性能。同时证明EASD的效率与标准推测解码相当。

Conclusion: EASD通过熵感知的动态惩罚机制，不仅保持了推测解码的效率优势，还突破了传统推测解码的性能上限，实现了可能超越目标模型本身的推理性能提升。
...
TL;DR: MiMo-Audio通过将音频预训练数据扩展到超过1亿小时，实现了音频任务的少样本学习能力，在多个音频理解和生成任务上达到开源SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 现有音频语言模型通常需要针对特定任务进行微调，而人类能够通过少量示例或简单指令泛化到新的音频任务。受GPT-3在文本领域成功的启发，研究者认为这种范式同样适用于音频领域。

Method: 1. 将MiMo-Audio的预训练数据扩展到超过1亿小时；2. 开发系统评估方法；3. 在后期训练阶段，策划多样化的指令调优语料库；4. 为音频理解和生成引入思考机制。

Result: 1. MiMo-Audio-7B-Base在语音智能和音频理解基准测试中达到开源SOTA；2. 能够泛化到训练数据中未包含的任务（如语音转换、风格迁移、语音编辑）；3. 具备强大的语音延续能力；4. MiMo-Audio-7B-Instruct在多个音频理解和对话基准测试中达到开源SOTA，接近或超越闭源模型。

Conclusion: 通过大规模音频预训练，MiMo-Audio展示了类似GPT-3在文本领域的泛化能力，实现了音频任务的少样本学习，为通用音频智能系统的发展提供了重要进展。
...
TL;DR: 本研究通过SHAP解释方法比较两种基于Transformer的新闻偏见检测模型，发现不同架构在整合语言信号时存在显著差异，领域自适应模型比专用偏见检测器减少63%误报，模型错误主要源于语篇层面的模糊性而非显性偏见线索。


<details>
  <summary>Details</summary>
Motivation: 新闻文本的自动偏见检测广泛用于支持新闻分析和媒体问责，但人们对偏见检测模型如何做出决策以及为何失败知之甚少。本研究旨在通过可解释性分析来理解不同模型架构如何操作化语言偏见。

Method: 采用比较可解释性研究方法，使用基于SHAP的解释分析两种基于Transformer的偏见检测模型：在BABE数据集上微调的专用偏见检测器，以及在BABE数据集上微调的领域自适应预训练RoBERTa模型。分析正确和错误预测的词级归因，以表征不同模型架构如何操作化语言偏见。

Result: 结果显示：1）两种模型都关注相似的评估性语言类别，但在整合这些信号到预测时存在显著差异；2）偏见检测器模型对误报分配比真报更强的内部证据，表明归因强度与预测正确性不一致，导致系统性地过度标记中性新闻内容；3）领域自适应模型表现出与预测结果更好对齐的归因模式，产生63%更少的误报；4）模型错误源于不同的语言机制，误报主要由语篇层面的模糊性而非显性偏见线索驱动。

Conclusion: 研究强调了可解释性评估对偏见检测系统的重要性，表明架构和训练选择对模型可靠性及在新闻背景下的部署适用性具有关键影响。领域自适应方法在减少误报和提高解释一致性方面表现更优。
...
TL;DR: 该研究探讨了在检索增强问答中使用LLMs时，扩展上下文窗口带来的挑战，并提出了一种自适应分块提示策略来平衡相关信息与无关信息，从而在减少token使用的同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs上下文窗口的扩展，检索增强生成中更广泛上下文的使用增加。虽然更长的上下文更容易纳入目标知识，但也引入了更多无关信息，这会阻碍模型的生成过程并降低其性能。
...
Conclusion: 该研究强调了需要进一步研究增强LLMs在面对信息不足时有效拒绝请求的能力。自适应分块提示策略为解决长上下文中的无关信息问题提供了一种有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 利用机制可解释性研究发现中间注意力层编码token级假设的特性，探索从注意力层token分布直接生成对抗样本的可能性

Method: 从中间注意力层提取token作为对抗扰动，在ArgQuality数据集上使用LLaMA-3.1-Instruct-8B同时作为生成器和评估器进行实验

Result: 注意力层生成的对抗样本能显著降低评估性能且保持语义相似性，但某些层和位置的替换会导致语法退化，限制实际效果

Conclusion: 中间层表示作为对抗样本来源具有潜力但存在局限，可用于压力测试基于LLM的评估流程
...
<details>
  <summary>Details</summary>
Motivation: 金融数值推理问答任务由于缺乏领域知识而存在错误，尽管大语言模型有进步，但金融数值问题仍具挑战性，需要特定金融领域知识和复杂多步数值推理。

Method: 实现多检索器检索增强生成(RAG)系统，检索外部领域知识和内部问题上下文，利用最新LLM处理任务，通过SecBERT编码器进行领域特定训练。

Result: 领域特定训练显著提升模型性能，最佳神经符号模型超越FinQA基准模型；最佳提示式LLM生成器达到SOTA性能，提升超过7%，但仍低于人类专家水平。

Conclusion: 研究发现小模型存在幻觉损失与外部知识获取的权衡，大模型中外部事实收益通常超过幻觉损失；最新LLM在少样本学习下增强了数值推理能力，领域特定训练具有潜力。
...
TL;DR: 提出一个分析框架，分离学生回答内容信号与教师评分倾向，通过建模教师历史作为动态先验，结合文本嵌入，量化各信号贡献，使评分过程透明可审计。


<details>
  <summary>Details</summary>
Motivation: 自动评分常混淆学生回答内容与教师评分倾向，需要分离这两者以使评分判断可见、可审计，帮助教师和研究者检查评分实践与学生推理证据的一致性。

Method: 使用ASSISTments数学回答数据，将教师历史建模为动态先验，从句子嵌入提取文本表示，采用中心化和残差化处理提示和教师混杂因素，通过时间验证的线性模型量化各信号贡献，用投影方法展示模型分歧供定性检查。

Result: 教师先验对成绩预测影响很大；先验与内容嵌入结合时效果最佳(AUC~0.815)，纯内容模型虽高于随机但明显较弱(AUC~0.626)；调整评分者效应能锐化残差内容表示，保留更多信息维度，揭示语义证据支持理解而非表面回答差异的情况。

Conclusion: 该框架将嵌入从单纯特征转化为学习分析工具，使教师和研究者能够检查评分实践与学生推理证据在何处一致或冲突，为反思提供实用管道。
...
Method: 提出HGMem超图记忆机制，将记忆表示为超图结构，其中超边对应不同的记忆单元，能够渐进形成记忆内的高阶交互，构建集成化的情境知识结构。

Result: 在多个全局理解挑战数据集上的实验表明，HGMem方法持续改进多步RAG性能，在多样化任务上显著超越强基线系统。

Conclusion: HGMem将记忆从简单存储扩展为动态表达结构，通过超图机制连接事实和思考，形成集成化知识结构，为后续步骤的深度推理提供有力支持，有效提升全局理解和复杂推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决长上下文场景中全注意力模型计算成本过高的问题，特别是在预填充密集型（如检索增强生成）和解码密集型（如工具集成推理）任务中，需要更高效的注意力机制来支持大规模上下文处理。

Method: 提出LongCat ZigZag Attention (LoZA)稀疏注意力方案，通过将全注意力模型转换为稀疏版本，在中训练阶段应用于LongCat-Flash模型，创建LongCat-Flash-Exp作为长上下文基础模型。

Result: LoZA在长上下文场景中实现了显著的速度提升，能够高效处理高达100万token的上下文，支持长期推理和长视野智能体能力，为检索增强生成和工具集成推理等应用提供高效解决方案。

Conclusion: LoZA稀疏注意力方案成功解决了长上下文处理的计算瓶颈问题，通过将全注意力模型转换为稀疏版本，在有限计算预算下实现了高效的大规模上下文处理能力，为长上下文AI应用提供了实用解决方案。
...
TL;DR: CEC-Zero是一个零监督强化学习框架，通过让大语言模型自我纠错来解决中文拼写校正问题，无需人工标注，在9个基准测试中显著优于现有方法。
...
Method: CEC-Zero从干净文本合成错误输入，通过语义相似度和候选一致性计算聚类共识奖励，使用PPO优化策略，实现零监督强化学习。

Result: 在9个基准测试中，比监督基线提高10-13个F1分数，比强LLM微调方法提高5-8个F1分数，具有无偏奖励和收敛的理论保证。

Conclusion: CEC-Zero建立了一个无需标注的鲁棒、可扩展的中文拼写校正范式，释放了LLM在噪声文本处理中的潜力。
...
TL;DR: RISE：一个无监督框架，通过稀疏自编码器发现大语言模型推理过程中的行为向量，实现推理行为的可解释性和可控性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型的推理能力不断增强，但其推理过程的内部机制仍未被充分探索。现有方法通常依赖人类定义的概念（如过度思考、反思）在词级别进行监督分析，但这种方法有限，因为无法捕捉所有潜在的推理行为，且许多行为难以在词空间中定义。

Method: 提出RISE框架：将思维链轨迹分割为句子级别的"步骤"，在步骤级激活上训练稀疏自编码器（SAEs），发现编码不同推理行为的"推理向量"（激活空间中的方向）。通过可视化和聚类分析这些行为，并进行针对性干预来控制推理行为。

Result: SAEs能够分离出可解释的行为（如反思和回溯），这些行为在解码器列空间中占据可分离的区域。对SAE派生向量的干预可以可控地放大或抑制特定推理行为，改变推理轨迹而无需重新训练。SAEs还能捕捉结构特性（如响应长度），并发现超出人类监督的新行为（如置信度相关向量）。

Conclusion: 无监督潜在发现方法在解释和可控引导大语言模型的推理方面具有巨大潜力，能够超越人类定义的概念发现新的推理行为，并实现精确的行为控制。
...
TL;DR: 该研究开发了WISE框架，用于区分假新闻和讽刺新闻，评估了8个轻量级Transformer模型和2个基线模型在20,000个样本数据集上的表现，发现MiniLM准确率最高(87.58%)，RoBERTa-base的ROC-AUC最佳(95.42%)，DistilBERT在效率与准确性之间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: 区分假新闻与讽刺/幽默内容具有独特挑战，因为两者具有重叠的语言特征但意图不同。当前需要开发能够在资源受限环境中部署的误信息检测系统。

Method: 开发WISE框架，在Fakeddit数据集上使用20,000个标注样本（假新闻或讽刺），采用分层5折交叉验证，评估8个轻量级Transformer模型和2个基线模型，使用多种评估指标（准确率、精确率、召回率、F1分数、ROC-AUC、PR-AUC、MCC、Brier分数、预期校准误差），并进行统计检验（配对t检验和McNemar检验）。

Result: MiniLM获得最高准确率(87.58%)，RoBERTa-base获得最高ROC-AUC(95.42%)和强准确率(87.36%)，DistilBERT在效率与准确性之间取得良好平衡（准确率86.28%，ROC-AUC 93.90%）。统计检验证实模型间存在显著性能差异，轻量级模型能够匹配或超越基线性能。

Conclusion: 轻量级模型能够在资源受限的实际环境中有效部署误信息检测系统，为实际应用提供了可行的解决方案。研究强调了在区分假新闻和讽刺内容时，模型选择需要考虑准确性和效率的平衡。
...
TL;DR: iCLP框架让大语言模型在潜在空间中进行隐式规划，同时在语言空间进行推理，显著提升数学推理和代码生成任务的准确性和效率，并保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有基于显式文本规划的方法面临LLM幻觉和任务多样性挑战，而人类潜意识认知（Implicit Cognition）通过从经验中学习紧凑模式来指导决策，无需显式语言化，这为改进LLM规划提供了灵感。

Method: iCLP框架：1）从现有逐步推理轨迹中提取显式规划；2）通过向量量化自编码器和码本学习这些规划的离散表示（潜在规划）；3）在配对潜在规划和相应推理步骤上微调LLM，使模型学会在推理过程中进行隐式规划。

Result: 实验结果显示，在数学推理和代码生成任务上，iCLP使LLM能够在潜在空间规划、语言空间推理，显著提高了准确性和效率，并展现出强大的跨领域泛化能力，同时保持了思维链推理的可解释性。

Conclusion: iCLP框架成功地将人类潜意识认知原理应用于LLM规划，通过潜在规划实现了更高效、准确且可泛化的推理，为LLM的隐式规划提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: LLM在医疗、法律、金融等关键决策领域应用日益广泛，但其可靠性存在不确定性，常出现过度自信错误、输入变化时性能下降、缺乏明确不确定性估计等问题，现有评估方法碎片化，只关注孤立方面

Method: 引入Composite Reliability Score (CRS)统一框架，集成校准、鲁棒性和不确定性量化三个维度；在五个QA数据集上对十个领先的开源LLM进行实验，评估基线性能、扰动影响和校准方法

Result: CRS提供稳定的模型排名，揭示单一指标遗漏的隐藏故障模式，显示最可靠的系统能够平衡准确性、鲁棒性和校准后的不确定性

Conclusion: CRS框架为LLM可靠性评估提供了统一、可解释的度量标准，有助于识别在关键决策应用中真正可靠的模型
...
TL;DR: HY-MT1.5系列翻译模型通过多阶段训练框架，在参数效率方面表现优异，1.8B模型超越许多更大开源模型和商业API，7B模型在其规模级别达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 开发高性能机器翻译模型，通过参数效率优化，在较小模型规模下达到或接近大型专有模型的翻译性能，同时支持高级翻译约束功能。

Method: 采用多阶段训练框架，包括通用和MT导向的预训练、监督微调、策略蒸馏和强化学习，构建了1.8B和7B参数的HY-MT1.5系列模型。

Result: HY-MT1.5-1.8B在标准中-外和英-外任务中全面超越大型开源基线（如Tower-Plus-72B）和主流商业API，达到Gemini-3.0-Pro约90%性能；HY-MT1.5-7B在其规模级别达到SOTA，在Flores-200上达到Gemini-3.0-Pro的95%性能，在WMT25和普通话-少数民族语言测试集上超越Gemini-3.0-Pro。

Conclusion: HY-MT1.5系列模型为通用和专业翻译任务提供了高度竞争力和鲁棒的解决方案，在各自参数规模下表现出色，支持术语干预、上下文感知翻译和格式保持等高级约束功能。
...
TL;DR: 本文提供了一个从零开始在AWS SageMaker上训练Hugging Face模型的完整指南，旨在降低云平台使用门槛，帮助资源有限的研究人员。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的发展主要由资源丰富的研究机构和行业伙伴推动，许多研究人员由于缺乏本地计算资源而转向云服务，但云平台的陡峭学习曲线成为障碍，现有文档存在知识缺口，需要整合碎片化信息。

Method: 通过演示论文的形式，集中整理研究人员在AWS SageMaker上训练第一个Hugging Face模型所需的基本信息，提供从零开始的完整指南。

Result: 创建了一个集中的资源指南，帮助研究人员克服云平台学习曲线，成功在AWS SageMaker上训练Hugging Face模型。

Conclusion: 通过提供全面的入门指南，本文旨在民主化云平台采用，使更多研究人员能够利用云服务训练复杂模型，促进更广泛的研究参与。
...
TL;DR: 提出了一种用于掩码扩散语言模型的激活引导框架，通过对比示例计算层间引导向量，实现推理时高效控制，无需模拟去噪轨迹


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型通过迭代去噪过程生成文本，具有并行解码和与自回归大模型竞争的性能，但推理时控制和引导机制尚未充分探索

Method: 提出激活引导框架：使用对比示例通过单次前向传播计算层间引导向量，在每一步反向扩散过程中应用这些方向，实现高效推理时控制

Result: 在LLaDA-8B-Instruct上的实验表明能够可靠调节高级属性，消融实验检验了在Transformer子模块和token范围（提示vs.响应）上的引导效果

Conclusion: 该激活引导框架为掩码扩散语言模型提供了有效的推理时控制机制，能够在不模拟去噪轨迹的情况下实现属性调节
...
TL;DR: 论文提出大型情感世界模型LEWM，通过整合情感因素改进世界模型，在保持基础任务性能的同时更准确预测情感驱动的社会行为。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型主要关注物理世界规律建模，缺乏对情感因素的系统探索。情感作为世界知识的重要组成部分，显著影响人类决策，但当前世界模型对此关注不足。

Method: 提出大型情感世界模型LEWM，构建Emotion-Why-How数据集整合情感与因果关系，使模型能同时建模情感状态、视觉观察和动作，预测未来状态和情感转变。

Result: 实验表明LEWM能更准确预测情感驱动的社会行为，同时在基础任务上保持与通用世界模型相当的性能。移除情感相关信息会降低推理性能，验证了情感的重要性。

Conclusion: 情感是世界理解的关键因素，LEWM通过系统整合情感建模，提升了世界模型对社会行为的预测能力，为情感感知的人工智能系统提供了新思路。
...
TL;DR: TeleChat3-MoE是一个基于MoE架构的大语言模型系列，参数量从1050亿到超过1万亿，在昇腾NPU集群上端到端训练。技术报告主要介绍了支持大规模模型训练的基础设施，包括数值精度验证、性能优化、并行化框架和集群级优化。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模不断增大（参数量达万亿级别），需要可靠且高效的训练基础设施来支持前沿模型规模的扩展。特别是在昇腾NPU硬件生态系统中，需要解决大规模训练中的数值精度、性能瓶颈和并行效率等问题。
...
Conclusion: TeleChat3-MoE的技术报告展示了在昇腾NPU集群上成功训练万亿参数级别MoE模型的基础设施能力。通过系统化的数值精度验证、性能优化、并行化框架和集群级优化，实现了可靠且高效的大规模模型训练，为硬件生态系统上的前沿语言模型开发提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在临床诊断中存在三个关键局限：1) 基于未经验证知识产生医学内容幻觉；2) 提出冗余或低效问题而非有助于诊断进展的区分性问题；3) 在多轮对话中失去连贯性，导致矛盾或不一致的结论。需要模拟真实临床场景中迭代、假设驱动的诊断推理过程。

Method: 提出MedKGI诊断框架，整合医学知识图谱以将推理约束在已验证的医学本体中；基于信息增益选择问题以最大化诊断效率；采用OSCE格式的结构化状态来保持跨轮次的一致证据跟踪。

Result: 在临床基准测试中，MedKGI在诊断准确性和询问效率方面均优于强大的LLM基线，平均提高对话效率30%，同时保持最先进的准确性。

Conclusion: MedKGI通过整合医学知识图谱、信息增益问题选择和结构化状态跟踪，有效解决了LLM在临床诊断中的关键局限，实现了更准确、高效的诊断推理。
...
TL;DR: 本文介绍了LAILA，这是目前最大的公开阿拉伯语自动作文评分数据集，包含7,859篇作文，标注了整体和七个维度的分数，填补了阿拉伯语AES研究的数据空白。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语自动作文评分研究因缺乏公开可用数据集而受限，需要构建大规模标注数据集来支持相关研究发展。

Method: 设计了LAILA数据集，包含7,859篇阿拉伯语作文，标注了整体分数和七个维度（相关性、组织结构、词汇、风格、发展、机械规范、语法）的分数，并提供了在特定提示和跨提示设置下的基准测试结果。

Result: LAILA成为目前最大的公开阿拉伯语AES数据集，为阿拉伯语自动作文评分研究提供了重要资源，支持开发更稳健的评分系统。

Conclusion: LAILA数据集填补了阿拉伯语自动作文评分研究的关键空白，为开发先进的阿拉伯语AES系统提供了必要的数据基础。
...
TL;DR: 开发了一个适用于专利和科学论文的语言相似性模型Pat-SPECTER，在预测专利-论文引用方面表现最佳，并验证了美国专利因诚信义务而引用语义相似度较低论文的假设。


<details>
  <summary>Details</summary>
Motivation: 需要开发一个能够同时处理专利和科学论文的语言相似性模型，以更好地理解和预测专利与科学文献之间的引用关系。

Method: 开发了Pat-SPECTER模型，这是基于SPECTER2模型在专利数据上进行微调的版本。通过"赛马式"评估比较了八种语言相似性模型在预测可信专利-论文引用方面的表现。

Result: Pat-SPECTER模型在预测专利-论文引用方面表现最佳。验证了美国专利因诚信义务而倾向于引用语义相似度较低的论文的假设。

Conclusion: Pat-SPECTER模型在专利-论文相似性分析中表现优异，为学术界和实践者提供了有效的工具，并揭示了不同司法管辖区专利引用行为的差异。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法存在局限性：基于质量指标的选择在长期预训练中收益递减，而基于多样性指标的选择会移除过多高质量样本，两者都限制了预训练大语言模型的能力。由于计算成本高，在万亿级token数据集中很少同时考虑多种指标。

Method: DATAMASK将数据选择过程视为掩码学习问题，采用基于策略梯度的优化方法：迭代采样数据掩码，基于预定义目标计算策略梯度，更新掩码采样逻辑。通过多种加速增强技术，相比贪心算法减少98.9%的选择时间。

Result: 从15万亿token的FineWeb数据集中选择了约10%的子集（FineWeb-Mask）。在12个多样化任务评估中，1.5B密集模型性能提升3.2%，7B MoE模型性能提升1.9%。

Conclusion: DATAMASK框架能够高效地在万亿级token规模上实现质量和多样性指标的联合优化，显著提升预训练数据选择效果，为大规模语言模型预训练提供了有效的数据选择解决方案。
...
TL;DR: FIGR通过强化学习将主动视觉思维整合到多轮推理中，构建视觉表示来外化中间结构假设，在复杂数学推理任务上优于纯文本方法
...
Conclusion: 图示引导的多模态推理能有效增强复杂推理的稳定性和可靠性，视觉表示有助于捕捉纯文本难以捕获的全局结构属性
...
<details>
  <summary>Details</summary>
Motivation: 现有讽刺理解方法大多依赖单一模型的黑盒预测，难以结构化解释讽刺背后的认知因素。虽然讽刺通常表现为语义评价与规范期望或意图之间的不匹配，但明确分解和建模这些组件的框架仍然有限。

Method: 提出World Model inspired SArcasm Reasoning (WM-SAR)，将字面意义、上下文、规范期望和意图分解为专门的LLM智能体。字面评价与规范期望之间的差异被量化为确定性不一致分数，与意图分数一起通过轻量级逻辑回归模型集成，推断最终的讽刺概率。

Result: 在代表性讽刺检测基准测试中，WM-SAR持续优于现有的深度学习和LLM方法。消融研究和案例分析进一步表明，整合语义不一致和意图推理对于有效的讽刺检测至关重要，实现了强大的性能和高度可解释性。

Conclusion: WM-SAR方法利用LLMs的推理能力，同时保持可解释的数值决策结构，为讽刺理解提供了既高性能又高可解释性的解决方案。
...
TL;DR: 提出基于人类略读策略的自监督对比学习框架，通过随机掩码文档段落并使用NLI对比目标对齐相关部分，提升长文档表示效果


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型在长文档表示方面存在困难，稀疏注意力机制资源密集且难以捕捉完整文档上下文，分层Transformer模型效率较高但无法清晰解释文档不同部分之间的关系。人类通过略读重要部分来理解整体信息的策略启发了本研究。

Method: 提出自监督对比学习框架，随机掩码文档段落，使用基于自然语言推理(NLI)的对比学习目标，将掩码段落与相关部分对齐，同时与不相关部分保持距离，模拟人类信息合成过程。

Result: 在法律和生物医学文本上的实验证实，该方法在准确性和效率方面都取得了显著提升。

Conclusion: 基于人类略读策略的自监督对比学习框架能够生成更丰富且计算效率更高的长文档表示，在专业领域文本处理中表现优异。
...
TL;DR: 该研究比较了多种低资源语言文本摘要方法，包括零样本LLM提示、微调mT5模型、数据增强、多语言迁移以及LLM翻译管道，发现微调mT5基线在多数指标上优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 自动文本摘要在高资源语言（如英语）中已取得高性能，但对低资源语言的摘要研究相对较少。本研究旨在探索和比较针对低资源语言的各种文本摘要方法。

Method: 1. 零样本提示不同规模的LLM；2. 微调mT5等较小模型，包含三种数据增强方法和多语言迁移；3. LLM翻译管道方法：将源语言翻译为英语→摘要→翻译回源语言；4. 使用五种不同指标进行评估。

Result: 1. 不同LLM在相似参数规模下性能存在差异；2. 多语言微调的mT5基线在大多数指标上优于包括零样本LLM在内的其他方法；3. LLM作为评估者在低资源语言上可能可靠性较低。

Conclusion: 对于低资源语言的文本摘要任务，多语言微调方法（如mT5）比零样本LLM提示和翻译管道方法更有效，且LLM评估在低资源语言场景下需谨慎使用。
...
TL;DR: 该研究开发了一个针对雅思写作考试的智能辅导平台，通过设计研究法迭代优化，从规则系统升级到基于DistilBERT的评分模型，实现了有效的自适应反馈，显著提升了考生成绩。


<details>
  <summary>Details</summary>
Motivation: 传统雅思备考方法缺乏个性化反馈，无法针对雅思写作评分标准提供定制化指导。为了解决这一问题，研究者开发了一个集成了自动评分系统和针对性反馈的智能平台。

Method: 采用设计研究法进行迭代开发：早期使用规则系统，第4周期采用DistilBERT转换器模型加回归头进行评分，第5周期实现自适应反馈。平台架构将对话指导与写作界面分离以降低认知负荷。

Result: 规则系统存在中段分数压缩、低准确性和负R²值等局限。DistilBERT模型显著改善评分性能（MAE=0.66，正R²）。自适应反馈使成绩平均提高0.060个分数段（p=0.011，Cohen's d=0.504），但效果因修订策略而异。

Conclusion: 自动反馈功能最适合作为人工教学的补充，保守的表面修正比激进的结构干预更可靠。评估高分段作文仍具挑战，未来需进行纵向研究并纳入官方考官验证。
...
TL;DR: 该研究填补了语音处理与文本分段交叉领域的三个空白：创建了首个语音段落分割基准数据集，提出了保留原始转录的约束解码方法，并开发了高效的分层分割模型。


<details>
  <summary>Details</summary>
Motivation: 自动语音转录通常输出无结构的单词流，这降低了可读性和再利用性。当前语音处理领域缺乏段落分割这一关键后处理步骤，同时文本分割领域也缺乏稳健且自然的基准数据集。

Method: 1) 创建两个基准数据集：TEDPara（人工标注的TED演讲）和YTSegPara（带合成标签的YouTube视频）；2) 提出约束解码方法，让大语言模型在保留原始转录的同时插入段落分隔符；3) 开发紧凑模型MiniSeg，并通过分层扩展实现章节和段落联合预测。

Result: 建立了首个语音段落分割基准，提出的约束解码方法实现了忠实、句子对齐的评估，MiniSeg模型达到了最先进的准确率，且分层扩展后能以最小计算成本联合预测章节和段落。

Conclusion: 该研究的资源和方将段落分割确立为语音处理中标准化、实用的任务，填补了语音处理与文本分段交叉领域的重要空白。
...
TL;DR: KCL是一个评估语言模型法律推理能力的基准，通过提供问题级别的支持判例来分离推理能力与领域知识，包含选择题和开放式生成题两部分。


<details>
  <summary>Details</summary>
Motivation: 创建KCL基准是为了独立评估语言模型的法律推理能力，而不是评估其参数化的领域知识，从而更准确地衡量模型的实际推理能力。

Method: KCL包含两个组件：KCL-MCQA（283个选择题，1,103个对齐判例）和KCL-Essay（169个开放式生成题，550个对齐判例，2,739个实例级评分标准）。通过提供问题级别的支持判例，实现推理能力与领域知识的分离。

Result: 对30多个模型的系统评估显示，在法律推理能力方面仍存在较大差距，特别是在KCL-Essay部分。专门用于推理的模型始终优于通用模型。

Conclusion: KCL基准为评估语言模型的法律推理能力提供了有效工具，揭示了当前模型在法律推理方面的局限性，并表明推理专用模型具有优势。所有资源已公开发布。
...
TL;DR: CREST是一种无需训练的推理时认知引导方法，通过识别和干预特定注意力头来抑制低效推理行为，提高LLM推理准确率并减少计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型依赖长链思维推理解决复杂任务，但推理轨迹经常低效，导致高延迟（过多token生成）或不稳定推理（浅层思考与过度思考交替）。需要一种方法来引导模型远离低效推理模式。

Method: CREST包含两个组件：(1)离线校准步骤：识别与特定认知行为（如验证和回溯）相关的注意力头，并推导出头特定的引导向量；(2)推理时程序：旋转隐藏表示以抑制沿这些向量的分量，自适应地抑制非生产性推理行为。

Result: 在多样化的推理基准测试和模型中，CREST将准确率提高了17.5%，同时减少了37.6%的token使用量，提供了更快、更可靠的LLM推理路径。

Conclusion: CREST通过识别和干预与认知行为相关的注意力头，有效引导LLM推理过程，在提高准确率的同时显著降低计算成本，为推理优化提供了简单有效的方法。
...
TL;DR: Youtu-LLM是一个1.96B参数的轻量级语言模型，通过从零开始预训练而非蒸馏，在保持高计算效率的同时具备原生智能体能力。它采用紧凑架构支持128k上下文，通过精心设计的"常识-STEM-智能体"课程训练，在数学、编程和工具使用等智能体任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前小型语言模型通常依赖蒸馏技术，缺乏真正的推理和规划能力。研究者希望开发一个轻量级但具备强大内在智能体能力的模型，能够在有限计算资源下执行复杂的长期规划和推理任务。

Method: 1. 采用密集多潜在注意力(MLA)架构和STEM导向词汇表，支持128k上下文窗口；2. 使用约11T token的大规模语料，实施"常识-STEM-智能体"多阶段课程训练；3. 针对智能体中期训练，采用多样化数据构建方案合成数学、编程和工具使用领域的轨迹数据。

Result: Youtu-LLM在2B以下模型中达到新的SOTA水平：在通用基准测试中与更大模型竞争，在智能体特定任务上显著超越现有SOTA基线，证明了轻量级模型可以具备强大的内在智能体能力。

Conclusion: Youtu-LLM展示了通过从零开始预训练和精心设计的课程学习，轻量级语言模型可以同时实现高计算效率和强大的智能体能力，为资源受限环境下的智能体应用提供了有前景的解决方案。
...
### [117] [R-Debater: Retrieval-Augmented Debate Generation through Argumentative Memory](https://arxiv.org/abs/2512.24684)
*Maoyuan Li,Zhongsheng Wang,Haoyuan Li,Jiamou Liu*

Main category: cs.CL

TL;DR: R-Debater是一个基于论证记忆的智能辩论框架，通过检索历史论据和案例证据来生成多轮连贯辩论，在单轮和多轮辩论任务中均优于现有LLM基线。


<details>
  <summary>Details</summary>
Motivation: 现有辩论系统缺乏对历史论据的记忆和一致性维护能力，难以在长期辩论中保持立场一致、有效回应对手并提供证据支持。需要结合修辞学和记忆研究，开发能够回忆和调整先前论点的辩论框架。

Method: R-Debater整合了辩论知识库（用于检索类似案例的证据和先前辩论动作）和基于角色的智能体（用于跨轮次生成连贯话语）。系统基于修辞学和记忆研究，将辩论视为回忆和调整先前论据的过程。

Result: 在ORCHID标准化辩论数据集上评估，包含1,000项检索语料和32个跨7个领域的辩论。在单轮话语生成任务中，R-Debater在InspireScore（主观、逻辑、事实性）上表现更好；在多轮对抗模拟中，Debatrix评估（论证、来源、语言、整体）得分更高。20名经验辩论者的人类评估进一步证实了其一致性和证据使用能力。

Conclusion: 检索基础与结构化规划相结合能够产生更忠实、立场对齐且跨轮次连贯的辩论。R-Debater框架在保持辩论一致性、有效回应对手和提供证据支持方面优于现有LLM基线。
...
TL;DR: 本文提出MUSIC数据增强策略，通过合成多轮对比对话对来提升多轮奖励模型的性能，在保持单轮基准性能的同时显著提升多轮对话评估质量。


<details>
  <summary>Details</summary>
Motivation: 评估多轮对话质量对开发大型语言模型至关重要，但现有方法依赖昂贵的人工评估。标准偏好数据集通常只基于最终轮次对比回复，无法捕捉多轮交互的细微差别，需要更好的多轮评估方法。

Method: 提出MUSIC（多步指令对比）无监督数据增强策略，通过合成跨多个轮次存在差异的对比对话对。基于Skywork偏好数据集，使用Gemma-2-9B-Instruct模型训练多轮奖励模型。

Result: MUSIC增强的奖励模型在性能上超越基线方法，在多轮对话评估中与先进专有LLM评判结果更一致，且不损害标准单轮奖励模型基准性能。

Conclusion: 跨多轮次的对比对于构建鲁棒的多轮奖励模型至关重要，MUSIC策略能有效提升多轮对话评估质量，为LLM训练提供有价值的指导信号。
...
TL;DR: BIOME-Bench：用于评估大语言模型在多组学分析中生物分子相互作用推断和通路机制解析能力的标准化基准
...
Method: 通过严格的四阶段工作流程构建BIOME-Bench基准，评估大语言模型在多组学分析中的两个核心能力：生物分子相互作用推断和端到端多组学通路机制解析。为这两个任务开发评估协议，并在多个当代强模型上进行综合实验。

Result: 实验结果表明，现有模型在多组学分析中仍存在显著缺陷：难以可靠地区分细粒度的生物分子关系类型，以及生成忠实、稳健的通路水平机制解释。
...
TL;DR: 该论文对大型语言模型在推理任务中的计算效率进行了评估，发现混合专家架构在性能与效率之间取得了良好平衡，并揭示了推理计算存在饱和点。


<details>
  <summary>Details</summary>
Motivation: 当前研究往往忽视生成长推理序列带来的显著计算负担，而工业应用不仅关注原始准确率，还需考虑资源约束和推理成本。

Method: 对当代和早期开源LLM进行测试时计算感知评估，在数学和推理密集型基准上绘制帕累托前沿，分析混合专家架构的效率优势，并追踪帕累托效率随时间变化的轨迹。

Result: 发现混合专家架构在评估设置中能很好平衡性能与效率；推导出单位计算精度增益的涌现趋势；证明推理计算存在饱和点，超过特定阈值后精度增益会减弱。
...
TL;DR: 介绍NLP伦理课程的教学方法与实践经验，强调通过互动式教学和"以教促学"培养批判性思维，分享四年多机构跨学科教学成果
...
Method: 采用基于主动学习的教学方法，包括互动式课堂、实践活动和"以教促学"方法。课程经过四年多在不同机构、教育水平和跨学科背景下的持续优化和调整。

Result: 课程产生了许多可重复使用的成果：既包括教学材料，也包括学生为不同受众制作的实际教育产品。课程已在多个机构和教育层次成功实施。

Conclusion: 通过分享教学方法和经验，为寻求将社会影响考量融入课程的教育工作者提供灵感和参考，推动NLP伦理教育的发展。
...
TL;DR: 提出因果三角验证方法，用于在多语言模型中验证机制解释的可靠性，要求电路在保持语义的变体环境中保持必要性、充分性和不变性。


<details>
  <summary>Details</summary>
Motivation: 多语言模型在整体表现上很强，但在不同语言、文字和文化中的行为难以预测。现有的机制解释方法缺乏因果验证标准，无法区分真正的机制和虚假相关。

Method: 提出三角验证方法：1) 定义参考家族（保持谓词语义的变体）；2) 要求电路满足必要性（消融会破坏目标行为）、充分性（激活补丁能转移行为）和不变性（在参考家族中效果保持稳定）；3) 结合自动电路发现，通过三角验证接受或拒绝候选电路。

Result: 三角验证为机制解释提供了可证伪的标准，能够过滤掉那些在单一环境测试中通过但在跨语言不变性测试中失败的虚假电路。

Conclusion: 三角验证方法为多语言模型的机制解释建立了因果验证框架，通过跨环境的一致性测试提高了机制解释的可靠性，并与实用可解释性议程相连接。
...
<details>
  <summary>Details</summary>
Motivation: 世界正面临生态、意义和语言三大危机的汇聚，形成"元危机"。大型AI正在加速所有这些危机，语言工程师们却继续坚持对人类有害的可扩展性叙事，为富豪和腐败统治者提供关键人才，仿佛整个技术开发过程是价值中立的。迫切需要探索替代方案。

Method: 论文提出需要运用集体智慧，重新设计NLP的未来发展方向，使其以人类繁荣和生态健康为中心，创造肯定生命的未来。

Result: 论文没有提供具体实验结果，而是提出了一个批判性分析和转型呼吁，指出当前NLP发展路径的问题，并提出需要根本性的范式转变。

Conclusion: NLP领域需要紧急转向，摆脱当前加剧危机的技术发展模式，转向以人类福祉和生态健康为中心的价值导向发展路径，创造肯定生命的未来。
...
TL;DR: Encyclo-K是一个基于知识陈述而非问题的基准测试框架，通过从权威教科书中提取独立知识陈述并在测试时动态组合成评估问题，解决了现有基准测试的数据污染、单知识点评估和专家标注成本高等问题。
...
Method: 以知识陈述而非问题作为构建单元，从权威教科书中提取独立知识陈述，在测试时通过随机抽样动态组合成评估问题。每个问题聚合8-10个陈述，实现多知识点的综合评估。

Result: 在50多个LLM上的实验表明，Encyclo-K具有强大的区分能力。即使是表现最好的OpenAI-GPT-5.1也只达到62.07%的准确率。推理模型准确率在16.04%到62.07%之间，聊天模型在9.71%到50.40%之间，显示出清晰的梯度分布。
...
TL;DR: mHC框架通过将超连接的残差连接空间投影到特定流形来恢复恒等映射特性，解决了HC方法训练不稳定和可扩展性受限的问题，同时优化基础设施确保效率。


<details>
  <summary>Details</summary>
Motivation: 超连接（HC）方法扩展了残差连接范式，带来了性能提升，但破坏了残差连接固有的恒等映射特性，导致训练不稳定、可扩展性受限和显著的内存访问开销。

Method: 提出流形约束超连接（mHC）框架，将HC的残差连接空间投影到特定流形以恢复恒等映射特性，同时进行严格的基础设施优化以确保效率。

Result: 实验证明mHC在规模化训练中有效，提供了实质性的性能改进和优越的可扩展性。

Conclusion: mHC作为HC的灵活实用扩展，将有助于深入理解拓扑架构设计，并为基础模型的演进提供有前景的方向。
...
TL;DR: BEDA框架通过将信念估计转化为生成约束，为战略对话提供简单通用的机制，在对抗、合作和谈判三种设置中均优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有战略对话研究虽然能准确估计信念，但缺乏在生成过程中使用这些信念的原则性机制，需要填补这一空白。

Method: 首先形式化对抗和对齐两个核心对话行为，通过概率约束操作化这些行为；然后构建BEDA框架，包括世界集合、信念估计器和条件生成器，后者根据推断的信念选择行为并生成一致的对话。

Result: 在三种设置中均取得显著改进：在CKBG（对抗）中，成功率至少提高5.0个百分点（GPT-4.1-nano提高20.6点）；在Mutual Friends（合作）中平均提高9.3点；在CaSiNo（谈判）中达到相对于所有基线的最优交易。

Conclusion: 将信念估计转化为约束条件为可靠的战略对话提供了一个简单通用的机制，BEDA框架在不同对话场景中均表现出色。
...
TL;DR: ADOPT是一个用于多步LLM流程的自适应依赖感知提示优化框架，通过显式建模LLM步骤与最终任务结果之间的依赖关系，实现精确的文本梯度估计和灵活的单步优化。


<details>
  <summary>Details</summary>
Motivation: 多步LLM流程的性能严重依赖于每一步使用的提示，但由于缺乏步骤级监督和步骤间依赖关系，联合优化这些提示非常困难。现有的端到端提示优化方法在这些条件下表现不佳，往往产生次优或不稳定的更新。
...
Conclusion: ADOPT通过显式建模步骤依赖关系、解耦梯度估计与更新、自适应资源分配，为多步LLM流程提供了有效的提示优化解决方案，解决了现有方法在复杂依赖条件下的局限性。
...
TL;DR: 基于DeBERTa V3和LSTM的法律文档分类器，使用48个随机选择的短文本块（最多128个标记）作为输入，通过Temporal部署实现可靠处理流程。


<details>
  <summary>Details</summary>
Motivation: 法律文档分类面临挑战：专业词汇多、文档长度大，直接将完整文档输入基于Transformer的模型进行分类可能不可行、昂贵或缓慢。

Method: 提出基于DeBERTa V3和LSTM的法律文档分类器，输入为48个随机选择的短文本块（最多128个标记），并使用Temporal构建部署管道实现可靠处理流程。

Result: 最佳模型的加权F分数为0.898，在CPU上运行的管道处理100个文件的平均时间为498秒。

Conclusion: 该方法有效解决了长法律文档分类问题，通过分块处理和可靠部署管道实现了良好的分类性能和处理效率。
...
TL;DR: RESTOREx是一个通过LLM生成和人工标注解释来检测社交媒体中表情包抑郁症状的资源，MAMAMemeia是基于认知分析疗法(CAT)的多智能体多维度讨论框架，在抑郁症状检测上比现有方法提升7.55%的macro-F1分数。


<details>
  <summary>Details</summary>
Motivation: 随着表情包从单纯的幽默交流媒介演变为用户自由表达各种情绪的工具，特别是在表达抑郁情绪方面的使用日益增长，需要研究在线社交媒体平台用户分享的表情包中表现的抑郁症状。

Method: 提出RESTOREx资源，使用大语言模型生成和人工标注的解释来检测社交媒体表情包中的抑郁症状。开发MAMAMemeia框架，这是一个基于临床心理学认知分析疗法(CAT)能力的协作式多智能体多维度讨论框架。

Result: MAMAMemeia框架相比30多种现有方法，在抑郁症状检测任务上提升了7.55%的macro-F1分数，成为新的基准方法。

Conclusion: 该研究为检测社交媒体表情包中的抑郁症状提供了有效工具，MAMAMemeia框架通过结合临床心理学方法和多智能体协作显著提升了检测性能，为心理健康监测提供了新方法。
...
TL;DR: 本文提出Thought Gestalt模型，一种结合token和句子级"思想"状态的双层抽象Transformer，通过记忆机制提升语言模型的全局一致性和数据效率。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer语言模型主要依赖表层共现统计，缺乏对实体和事件的全局一致潜在表示，导致关系方向脆弱（如逆转诅咒）、上下文错误和数据效率低下。而人类认知科学表明，人类理解语言时会将其转换为紧凑的事件式表示并持久记忆。
...
Result: 在扩展实验中，TG相比匹配的GPT-2运行始终提高效率，扩展拟合表明GPT-2需要约5-8%更多数据和33-42%更多参数才能匹配TG的损失。TG还在父子逆转诅咒探针上减少了关系方向泛化错误。

Conclusion: Thought Gestalt模型通过结合认知科学原理，在语言建模中引入双层抽象表示，有效提升了模型的全局一致性、数据效率和关系方向泛化能力。
...
TL;DR: AdaGReS是一个用于检索增强生成的冗余感知上下文选择框架，通过结合查询-块相关性和集合内冗余惩罚的集合级目标，在token预算约束下优化上下文选择，无需手动调参。


<details>
  <summary>Details</summary>
Motivation: 标准top-k检索在检索增强生成中经常返回冗余或近似重复的文本块，这会浪费token预算并降低下游生成质量。需要一种能够有效控制冗余、优化上下文选择的解决方案。

Method: 提出AdaGReS框架，使用结合查询-块相关性和集合内冗余惩罚的集合级目标函数，在token预算约束下进行贪心选择。引入闭式、实例自适应的相关性-冗余权衡参数校准，无需手动调参，能适应候选池统计特性和预算限制。理论分析表明该目标函数在实际嵌入相似性条件下具有ε-近似子模性。

Result: 在开放域问答（Natural Questions）和高冗余生物医学（药物）语料库上的实验表明，AdaGReS在冗余控制和上下文质量方面持续改进，转化为更好的端到端答案质量和跨设置的鲁棒性。

Conclusion: AdaGReS通过优化冗余感知的上下文选择，有效解决了RAG中冗余浪费token预算的问题，提供理论保证和无需手动调参的自适应能力，显著提升了检索增强生成的质量和效率。
...
### [132] [Generative AI-enhanced Sector-based Investment Portfolio Construction](https://arxiv.org/abs/2512.24526)
*Alina Voronina,Oleksandr Romanko,Ruiwen Cao,Roy H. Kwon,Rafael Mendoza-Arriaga*

Main category: q-fin.PM

TL;DR: 大型语言模型（LLM）在量化行业投资组合构建中的应用研究：多模型对比显示LLM在稳定市场表现优异，但在波动市场中表现不佳，而LLM与传统优化方法结合能提升投资组合表现。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型（来自OpenAI、Google、Anthropic、DeepSeek和xAI等主要提供商）在量化行业投资组合构建中的应用潜力，探索生成式AI算法在投资管理中的实际效果。

Method: 使用LLM从S&P 500行业指数中识别可投资股票组合，每个模型被提示选择并加权每个行业的20只股票。将LLM选择的投资组合与相应行业指数在两个不同的样本外时期进行比较：稳定市场阶段（2025年1-3月）和波动阶段（2025年4-6月）。

Result: LLM投资组合表现具有强烈的时间依赖性：在稳定市场条件下，LLM加权投资组合在累计收益和风险调整后（夏普比率）指标上经常优于行业指数；但在波动时期，许多LLM投资组合表现不佳。当LLM股票选择与传统优化技术结合时，投资组合结果在表现和一致性方面都有所改善。

Conclusion: LLM可以有效补充量化金融，增强股票选择和可解释性，但其可靠性仍受市场环境影响。研究强调了混合AI-量化框架的潜力，即整合LLM推理与成熟的优化技术，以产生更稳健和适应性强的投资策略。
...
### [133] [The Drill-Down and Fabricate Test (DDFT): A Protocol for Measuring Epistemic Robustness in Language Models](https://arxiv.org/abs/2512.23850)
*Rahul Baxi*

Main category: cs.AI

TL;DR: 该研究提出了DDFT评估框架，用于测量语言模型在语义压缩和对抗性干扰下的知识稳健性，发现模型稳健性与参数规模和架构类型无关，而与错误检测能力密切相关。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型评估（如MMLU和TruthfulQA）只测量理想条件下的知识水平，无法评估模型在现实压力下（如信息退化或对抗性攻击时）的知识稳健性。需要新的评估方法来区分模型是缺乏知识还是验证机制在压力下崩溃。

Method: 提出了Drill-Down and Fabricate Test (DDFT)协议，通过渐进语义压缩和对抗性伪造来测量模型的认知稳健性。采用两系统认知模型：语义系统（生成流畅文本）和认知验证器（验证事实准确性）。评估了9个前沿模型在8个知识领域和5个压缩级别下的表现（共1800次轮级评估）。

Result: 认知稳健性与传统设计范式正交：参数数量（r=0.083, p=0.832）和架构类型（r=0.153, p=0.695）对稳健性无显著预测作用。错误检测能力强烈预测整体稳健性（rho=-0.817, p=0.007）。旗舰模型尽管规模大但表现出脆弱性，而较小模型也能实现稳健性能。
...
TL;DR: McCoy框架结合大语言模型与答案集编程，将医学文献转化为ASP代码，结合患者数据进行疾病诊断，提供可解释的预测系统
...
Conclusion: McCoy通过整合LLM和ASP，克服了传统符号AI在医疗领域应用的知识库构建障碍，利用两种范式的优势创建了强大的可解释疾病预测系统。
...
TL;DR: SPARK是一个基于多智能体LLM的个性化搜索框架，通过角色化智能体协作实现任务特定的检索和涌现式个性化


<details>
  <summary>Details</summary>
Motivation: 传统个性化搜索系统受限于静态用户画像和单一检索流程，难以捕捉用户动态、多维的信息需求，需要能够建模用户演化信息需求的框架

Method: 1. 定义角色空间（角色、专业知识、任务上下文、领域）；2. 引入角色协调器动态解释查询并激活相关智能体；3. 每个智能体执行独立的检索增强生成过程，配备长短期记忆存储和上下文感知推理模块；4. 通过结构化通信协议（共享记忆库、迭代辩论、接力式知识传递）促进智能体协作

Result: 框架产生了关于协调效率、个性化质量和认知负载分布的可测试预测，同时包含用于持续角色细化的自适应学习机制

Conclusion: SPARK通过整合细粒度智能体专业化与协作检索，为能够捕捉人类信息寻求行为的复杂性、流动性和上下文敏感性的下一代搜索系统提供了见解
...
TL;DR: 本文提出了一种风险感知的逐步对齐方法（RSA），通过使用嵌套风险度量来增强语言模型安全对齐，解决现有方法对罕见但灾难性有害行为的鲁棒性不足问题。
...
Method: 提出风险感知逐步对齐（RSA）方法，将安全对齐表述为令牌级别的风险感知约束策略优化问题，通过逐步对齐过程解决，利用嵌套风险度量推导令牌级别的策略更新。

Result: 实验结果表明，该方法在保持高帮助性的同时确保强安全性，并显著抑制尾部风险（即低概率但高影响的不安全响应）。

Conclusion: RSA方法通过明确的风险感知设计，有效缓解了模型偏离参考策略带来的风险，并明确抑制了低概率高影响的有害行为，为语言模型安全对齐提供了更鲁棒的解决方案。
...
TL;DR: 该研究系统分析了JEPA-WM类世界模型，通过实验确定了最优架构、训练目标和规划算法组合，提出的新模型在导航和操作任务上超越了现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前AI面临的核心挑战是开发能够解决广泛物理任务并能泛化到新任务和环境的智能体。虽然基于世界模型的规划方法取得进展，但现有方法在表征空间规划的技术选择缺乏系统性研究，需要确定JEPA-WM类模型的最优实现方案。

Method: 将相关模型归类为JEPA-WM，系统研究模型架构、训练目标和规划算法三个关键组件的影响。在模拟环境和真实机器人数据上进行实验，通过对比分析确定最优组合方案。

Result: 提出的新模型在导航和操作任务上超越了DINO-WM和V-JEPA-2-AC两个基准方法。研究提供了代码、数据和检查点供社区使用。

Conclusion: 通过系统性研究JEPA-WM类模型的技术选择，确定了最优实现方案，为开发更高效的基于世界模型的规划方法提供了重要指导，推动了物理任务智能体的发展。
...
TL;DR: 该研究分析了三种主流大语言模型（GPT-4o-mini、Gemini-2.0-Flash、DeepSeek-V3）在密苏里大学数学竞赛问题上的表现，发现DeepSeek-V3在所有数学领域表现最佳，但所有模型在几何问题上都表现较弱，揭示了不同模型的错误模式差异。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多使用相同的数据集评估大语言模型的数学推理能力，这限制了研究结果的普适性，可能无法全面捕捉数学任务中的多样化挑战。本研究旨在通过分析大语言模型在代表性不足的数学竞赛问题上的表现，更深入地了解其数学推理能力。

Method: 研究使用密苏里大学数学竞赛中的微积分、解析几何和离散数学问题，对GPT-4o-mini、Gemini-2.0-Flash和DeepSeek-V3三种大语言模型进行测试。将模型回答与已知正确答案进行比较，评估各问题领域的准确性，并分析模型的推理过程以探索不同问题类型和模型间的错误模式。

Result: DeepSeek-V3在微积分、解析几何和离散数学三个类别中表现最佳，无论是推理过程还是最终答案。所有三种大语言模型在几何问题上都表现出明显的弱点。DeepSeek-V3的错误主要源于计算和逻辑错误，GPT-4o-mini经常出现逻辑和方法相关的错误，而Gemini则倾向于不完整的推理和仓促的结论。

Conclusion: 在代表性不足的数学竞赛数据集上评估大语言模型，可以更深入地了解其独特的错误模式，并突出结构化推理方面的持续挑战，特别是在几何领域。这有助于更全面地评估大语言模型的数学推理能力。
...
TL;DR: 本文提出了一种两阶段方法，将空间推理分解为原子构建块及其组合，通过监督微调学习基本空间变换，然后使用GRPO框架训练LoRA适配器进行多步规划，在ASCII艺术环境中优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在通用语言能力上表现强劲，但在结构化环境中的空间变换和多步规划方面仍然存在困难。需要一种方法来提升LLMs在空间推理任务上的表现，特别是在导航和规划等应用场景中。

Method: 1. 两阶段方法：首先对基本空间变换（如旋转、平移、缩放）进行监督微调，使模型具备基本空间物理知识；然后冻结该物理感知模型，在GRPO框架内训练轻量级LoRA适配器，以学习将这些构建块组合用于多步规划。
2. 创建ASCII艺术数据集和相应的基于ASCII的强化学习环境来支持该流程。
3. 在动态环境（有显式状态更新）和静态环境（模型必须依赖内部状态）中评估方法。

Result: 1. 该方法在动态环境和静态环境中均一致优于基线方法，包括通用骨干模型、物理感知模型和端到端RL模型。
2. 相比从零开始的端到端强化学习，所提方法收敛更快，训练更稳定。
3. 通过注意力模式分析验证了微调确实带来了空间理解的有意义改进。

Conclusion: 提出的两阶段分解方法能有效提升LLMs的空间推理能力，通过先学习基本空间变换再学习组合策略的方式，在结构化环境中实现了更好的多步规划性能，且训练更稳定高效。
...
TL;DR: RLMs是一种递归语言模型推理策略，允许LLM处理远超其上下文窗口的长提示，通过编程式检查、分解和递归调用来处理长文本片段。


<details>
  <summary>Details</summary>
Motivation: 研究如何让大型语言模型处理任意长度的提示，突破现有模型上下文窗口的限制，实现更高效的长文本处理能力。

Method: 提出递归语言模型（RLMs）作为通用推理策略，将长提示视为外部环境，允许LLM编程式地检查、分解并递归调用自身处理提示片段。

Result: RLMs成功处理超出模型上下文窗口两个数量级的输入，即使在较短提示下，在四个不同的长上下文任务中也显著优于基础LLM和常见长上下文支架，同时查询成本相当或更低。

Conclusion: RLMs为LLM处理长上下文提供了一种有效的推理时扩展方法，能够在保持成本效益的同时显著提升长文本处理质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在单任务上表现良好，但在多智能体协作场景中缺乏全局优化能力，难以协调多个智能体实现最优整体性能

Method: 采用去中心化部分可观测马尔可夫决策过程建模协作，使用集中训练分散执行框架，提出组相对策略优化算法，结合平衡任务质量、速度和协调成本的简化联合奖励
...
### [142] [Multi-modal cross-domain mixed fusion model with dual disentanglement for fault diagnosis under unseen working conditions](https://arxiv.org/abs/2512.24679)
...
Method: 提出多模态跨域混合融合模型，包含双重解耦框架（解耦模态不变/特定特征和域不变/特定表示）、跨域混合融合策略（随机混合跨域模态信息）、三重模态融合机制（自适应集成多模态异构信息）。
...
TL;DR: 该研究提出了一个包含四个可解释维度的物体摆放偏好模型，并通过问卷验证了这些维度的心理区分度，最后将其集成到MCTS规划器中生成符合人类偏好的摆放方案。


<details>
  <summary>Details</summary>
Motivation: 现有的家庭物体重排机器人系统通常依赖从人类演示中推断的潜在偏好模型，这些模型虽然能有效预测，但对指导人类决策的可解释因素提供有限洞察。需要开发一个更明确、可解释的物体摆放偏好模型。

Method: 1. 提出了包含四个可解释维度的物体摆放偏好模型：空间实用性、习惯便利性、语义连贯性、常识适当性。2. 设计并验证了自报告问卷，通过63名参与者的在线研究确认这些维度的心理区分度。3. 将参与者偏好集成到蒙特卡洛树搜索（MCTS）规划器中，用于生成合理的物体摆放方案。

Result: 1. 问卷研究结果证实了四个维度的心理区分度和解释力（在厨房和客厅两个场景中）。2. 当MCTS规划器由参与者偏好引导时，能够生成与参与者生成的摆放方案高度一致的合理摆放方案。

Conclusion: 该工作贡献了一个紧凑、可解释的物体摆放偏好模型，并展示了如何将其操作化用于机器人规划，为家庭机器人系统提供了更透明、可解释的决策基础。
...
TL;DR: GenZ是一个混合模型，通过可解释的语义特征桥接基础模型和统计建模。该方法通过对比统计建模误差识别的项目组来发现语义特征描述，而不是仅依赖基础模型的领域知识。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然拥有广泛的领域知识，但往往无法捕捉对预测任务至关重要的数据集特定模式。基础模型通常依赖其领域理解，而忽略了数据集特有的统计模式。

Method: 提出了一种广义EM算法，联合优化语义特征描述符和统计模型参数。该方法提示冻结的基础模型基于发现的特征对项目进行分类，将这些判断视为潜在二元特征的噪声观察，这些特征通过学习到的统计关系预测实值目标。

Result: 在房价预测方面，使用多模态列表数据发现的语义特征实现了12%的中位数相对误差，显著优于依赖LLM通用领域知识的GPT-5基线（38%误差）。对于Netflix电影嵌入，仅从语义描述预测协同过滤表示，获得了0.59的余弦相似度，相当于传统协同过滤需要约4000个用户评分的性能。

Conclusion: GenZ方法能够发现数据集特定的模式（如预测本地房地产市场的建筑细节、预测用户偏好的系列电影成员资格），这些模式与模型单独的领域知识不同。该方法成功地将基础模型的语义理解与统计建模的数据特定模式相结合。
...
TL;DR: 提出一种基于约束模板的方法，用于从养老机构管理者访谈中提取设施特定的排班约束条件，并通过排除例外约束来优化排班生成。


<details>
  <summary>Details</summary>
Motivation: 养老机构的排班条件因设施而异，需要从管理者访谈中提取特定设施的约束条件，但现有约束提取技术难以处理例外情况。

Method: 使用约束模板提取连续工作日排班模式、员工组合等约束组合，通过调整关注天数、员工数量和提取焦点（模式或频率）来提取多样化约束，并加入排除例外约束的机制。

Result: 实验表明，该方法成功生成了满足所有硬约束的排班，并通过避免提取例外约束减少了软约束违反次数。

Conclusion: 提出的约束模板方法能够有效提取养老机构特定约束条件，结合例外约束排除机制，可生成更符合实际需求的护理人员排班。
...
TL;DR: DARTS项目开发了一个半自动数据标注流水线，通过人机协同方式减少驾驶场景多模态数据标注的成本和时间，支持波兰自动驾驶研究。


<details>
  <summary>Details</summary>
Motivation: 手动标注异构驾驶数据成本高、耗时长，需要开发自动化解决方案来加速大规模标注数据集的创建，支持波兰自动驾驶研究。

Method: 采用人在回路方法，结合AI与人工专业知识，包括自动生成初始标注、迭代模型重训练、数据匿名化和领域适应技术，核心使用3D目标检测算法。

Result: 开发的工具和方法显著节省时间，确保跨不同传感器模态的一致高质量标注，加速了DARTS项目中标准化格式的大规模标注数据集的准备。

Conclusion: 该半自动标注流水线有效降低了标注成本和时间，为波兰自动驾驶研究提供了技术基础，支持DARTS项目目标的实现。
...
TL;DR: 迭代部署LLM模型，每次基于用户从前一模型部署中精心筛选的数据进行微调，能显著改变模型特性，在规划任务中观察到技能提升和泛化能力，理论分析表明这实现了隐式强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 研究迭代部署大型语言模型时，用户从先前模型部署中筛选数据用于微调后续模型，这一过程如何影响模型特性，特别是探索这种机制在规划任务中的效果及其理论意义。

Method: 在多个规划领域测试迭代部署机制：每个新模型基于用户从前一模型部署中精心筛选的数据进行微调，观察模型规划技能的变化，并进行理论分析连接强化学习框架。

Result: 观察到规划技能显著提升，后续模型展现出涌现的泛化能力，能发现比初始模型长得多的规划方案。理论分析表明迭代部署实现了外层循环的强化学习训练，具有隐式奖励函数。

Conclusion: 迭代部署机制可视为显式强化学习的替代训练方案，依赖数据筛选而非显式奖励，对AI安全有重要影响，因为隐含的奖励函数未明确定义，可能对未来模型部署产生意外影响。
...
TL;DR: STAgent是一个专门用于时空理解的智能体大语言模型，能够处理约束性兴趣点发现和行程规划等复杂任务，通过工具交互实现探索、验证和推理，同时保持通用能力。


<details>
  <summary>Details</summary>
Motivation: 开发一个专门针对时空理解任务的智能体大语言模型，解决传统模型在处理复杂时空任务（如约束性兴趣点发现和行程规划）时的局限性，同时确保模型不丧失通用能力。

Method: 采用三阶段方法：1）构建支持10+领域特定工具的稳定工具环境；2）分层数据筛选框架，以1:10,000的比例筛选高质量查询数据；3）级联训练方案：种子SFT阶段评估查询难度，第二SFT阶段微调高确定性查询，最终RL阶段利用低确定性数据。

Result: STAgent在TravelBench上表现出色，同时在广泛的通用基准测试中保持了其通用能力，证明了所提出的智能体模型的有效性。

Conclusion: STAgent通过专门的工具环境、高质量数据筛选和级联训练方案，成功构建了一个既能处理复杂时空任务又不丧失通用能力的智能体大语言模型，为时空理解领域提供了有效的解决方案。
...
### [149] [Generative Video Compression: Towards 0.01% Compression Rate for Video Transmission](https://arxiv.org/abs/2512.24300)
*Xiangyu Chen,Jixiang Luo,Jingyu Xu,Fangqiu Yi,Chi Zhang,Xuelong Li*

Main category: eess.IV

TL;DR: GVC（生成式视频压缩）通过生成式视频模型实现极低压缩率（0.02%），将计算负担从传输转移到推理端，在带宽受限环境中具有应用潜力。


<details>
  <summary>Details</summary>
Motivation: 探索视频能否实现极低压缩率（如0.01%），解决带宽和资源受限环境（如应急救援、远程监控、移动边缘计算）中的视频通信问题。

Method: 提出生成式视频压缩（GVC）框架，利用现代生成式视频模型将视频编码为极紧凑表示，在接收端通过生成先验从最小传输信息合成高质量视频，并提出压缩-计算权衡策略以实现消费级GPU上的快速推理。

Result: 在某些情况下实现了0.02%的压缩率，在AI Flow框架内验证了GVC在带宽和资源受限环境中的可行性，展示了有效、高效、可扩展且实用的视频通信新范式。

Conclusion: GVC通过生成式模型重新定义了视频压缩的极限，将计算负担从传输转移到推理，为带宽和资源受限环境中的视频通信开辟了新的可能性，提供了一种可行的新视频通信范式。
